 ...
 */
/* $Id: xlate.c 40502 2009-04-14 16:06:07Z tkreuzer $
 *
...
IntCreateXlateForBlt(PDC pDCDest, PDC pDCSrc, SURFACE* psurfDest, SURFACE* psurfSrc)
{
	...
	if (psurfDest->SurfObj.iBitmapFormat == BMF_1BPP)
	{
		...
		{
			XlateObj = IntEngCreateMonoXlate(0, DestPalette, SourcePalette, pDCSrc->crBackgroundClr);
		}
	}
	else
	{
...
#endif
			{
				XlateObj = IntEngCreateSrcMonoXlate(DestPalette, pDCDest->crForegroundClr, pDCDest->crBackgroundClr);
			}
		}
		else
		{
