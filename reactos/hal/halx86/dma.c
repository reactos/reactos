/* $Id: dma.c,v 1.9.6.1 2004/10/24 22:57:51 ion Exp $
 *
 ...
 */
{
  ...
  switch (DeviceDescription->InterfaceType)
    {
      case PCIBus:
        if (DeviceDescription->Master == FALSE) return NULL;

      case Isa:
        /* There are only 8 DMA channels on ISA, so any request above this
	   should not get any channel setup (FIXME: BUT SHOULD NOT RETURN NULL!) */
        if (DeviceDescription->DmaChannel >= 8) return NULL;
	
	/* Channel 4 is Reserved for Chaining, so you cant use it */
	if (DeviceDescription->DmaChannel == 4) return NULL;
