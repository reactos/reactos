                   ...
                   WORD Hint)
{
    ...
    if (Hint < ExportDir->NumberOfNames)
    {
        ...
        if (strcmp(ExName, (PCHAR)SymbolName) == 0)
        {
            ...
            if (((ULONG)Function >= (ULONG_PTR)ExportDir) &&
                    ((ULONG)Function < (ULONG_PTR)ExportDir + (ULONG_PTR)ExportDirSize))
            {
                DPRINT("Forward: %s\n", (PCHAR)Function);
                Function = LdrFixupForward((PCHAR)Function);
                if (Function == NULL)
                ...
                !(Module->Flags & LDRP_UNLOAD_IN_PROGRESS))
        {
            ...
            if (Module->Flags & LDRP_PROCESS_ATTACH_CALLED)
            {
                ...
                LdrpCallDllEntry(Module,
                                 DLL_PROCESS_DETACH,
                                 (PVOID)(INT_PTR)(Module->LoadCount == LDRP_PROCESS_CREATION_TIME ? 1 : 0));
            }
            else
            {
                TRACE_LDR("Unload %wZ\n", &Module->BaseDllName);
