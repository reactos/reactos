    ...
    void **ppvOut)
{
    ...
    if (IsEqualIID(riid, IID_IShellFolder))
    {
        ...
        {
            NtPidlSymlinkData * symlink = (NtPidlSymlinkData*) (((PBYTE) info) + sizeof(NtPidlEntry) + info->entryNameLength + sizeof(WCHAR));
    ...
    if (pidl)
    {
        ...
        if (IsEqualGUID(pscid->fmtid, storage))
        {
            ...
            else if (pscid->pid == PID_STG_STORAGETYPE)
            {
                ...
                {
                    NtPidlTypeData * td = (NtPidlTypeData*) (((PBYTE) info) + sizeof(NtPidlEntry) + info->entryNameLength + sizeof(WCHAR));
