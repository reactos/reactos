...
static void uhci_insert_tds_in_qh(struct uhci_qh *qh, struct urb *urb, __le32 breadth)
{
	...
	plink = &qh->element;
	list_for_each_entry(td, struct uhci_td, &urbp->td_list, list) {
...
static void uhci_insert_qh(struct uhci_hcd *uhci, struct uhci_qh *skelqh, struct urb *urb)
{
	...
	if (lqh->urbp) {
		list_for_each_entry(turbp, struct urb_priv, &lqh->urbp->queue_list, queue_list)
...
static void uhci_remove_qh(struct uhci_hcd *uhci, struct uhci_qh *qh)
{
	...
	if (!list_empty(&qh->list)) {
		...
		if (pqh->urbp) {
			...
			list_for_each_entry(turbp, struct urb_priv, &pqh->urbp->queue_list,
					queue_list)
...
static void uhci_append_queued_urb(struct uhci_hcd *uhci, struct urb *eurb, struct urb *urb)
{
	...
	furbp = eurbp;
	if (eurbp->queued) {
		list_for_each_entry(furbp, struct urb_priv, &eurbp->queue_list, queue_list)
			if (!furbp->queued)
				break;
	}
...
static void uhci_delete_queued_urb(struct uhci_hcd *uhci, struct urb *urb)
{
	...
	if (usb_pipebulk(urb->pipe) || usb_pipeint(urb->pipe)) {
		...
		list_for_each_entry(turbp, struct urb_priv, &urbp->queue_list, queue_list) {
			if (!turbp->queued)
				break;
...
static void uhci_destroy_urb_priv(struct uhci_hcd *uhci, struct urb *urb)
{
	...
	list_for_each_entry_safe(td, struct uhci_td, tmp, struct uhci_td, &urbp->td_list, list) {
...
static int isochronous_find_limits(struct uhci_hcd *uhci, struct urb *urb, unsigned int *start, unsigned int *end)
{
	...
	list_for_each_entry(up, struct urb_priv, &uhci->urb_list, urb_list) {
		struct urb *u = up->urb;
...
static void uhci_unlink_generic(struct uhci_hcd *uhci, struct urb *urb)
{
	 ...
	 */
	head = &urbp->td_list;
	list_for_each_entry(td, struct uhci_td, head, list) {
		unsigned int ctrlstat = td_status(td);
...
static void uhci_free_pending_qhs(struct uhci_hcd *uhci)
{
	...
	list_for_each_entry_safe(qh, struct uhci_qh, tmp, struct uhci_qh, &uhci->qh_remove_list, remove_list) {
...
static void uhci_free_pending_tds(struct uhci_hcd *uhci)
{
	...
	list_for_each_entry_safe(td, struct uhci_td, tmp, struct uhci_td, &uhci->td_remove_list, remove_list) {
...
static void uhci_finish_completion(struct uhci_hcd *uhci, struct pt_regs *regs)
{
	...
	list_for_each_entry_safe(urbp, struct urb_priv, tmp, struct urb_priv, &uhci->complete_list, urb_list) {
...
static void uhci_scan_schedule(struct uhci_hcd *uhci, struct pt_regs *regs)
{
 ...
 rescan:
	...
	list_for_each_entry_safe(urbp, struct urb_priv, tmp, struct urb_priv, &uhci->urb_list, urb_list) {
		struct urb *urb = urbp->urb;
