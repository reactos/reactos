...
static int uhci_irq(struct usb_hcd *hcd, struct pt_regs *regs)
{
	...
	if (status & ~(USBSTS_USBINT | USBSTS_ERROR | USBSTS_RD)) {
		if (status & USBSTS_HSE)
			err("%x: host system error, PCI problems?", uhci->io_addr);
		if (status & USBSTS_HCPE)
			err("%x: host controller process error. something bad happened", uhci->io_addr);
		if (status & USBSTS_HCH) {
			spin_lock_irqsave(&uhci->lock, flags);
			if (uhci->rh_state >= UHCI_RH_RUNNING) {
				err("%x: host controller halted. very bad", uhci->io_addr);
				hc_died(uhci);
...
static int uhci_reset(struct usb_hcd *hcd)
{
	...
	uhci->io_addr = (unsigned long) hcd->rsrc_start.LowPart;
