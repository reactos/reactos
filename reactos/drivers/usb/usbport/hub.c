...
DPRINT1("Start of the loop, hub_event_list=%p. __DELETE\n", hub_event_list);
		spin_lock_irq(&hub_event_lock);
		if (list_empty(&hub_event_list)) {
			spin_unlock_irq(&hub_event_lock);
			break;
		}
DPRINT1("List is not empty. __DELETE\n");
		tmp = hub_event_list.next;
		list_del_init(tmp);
DPRINT1("list_del_init_done. __DELETE\n");
		hub = list_entry(tmp, struct usb_hub, event_list);
DPRINT1("hub_events: hub %p\n", hub);
		hdev = hub->hdev;
...
DPRINT1("before spin_unlock_irq %p __DELETE\n", &hub_event_lock);
		spin_unlock_irq(&hub_event_lock);
DPRINT1("passed spin_unlock_irq, i=%d\n", i);
...
#else
static struct usb_device_id hub_id_table [] = {
    { USB_DEVICE_ID_MATCH_DEV_CLASS, 0,0,0,0, USB_CLASS_HUB},
    { USB_DEVICE_ID_MATCH_INT_CLASS, 0,0,0,0,0,0,0, USB_CLASS_HUB},
    { 0 }						/* Terminating entry */
};
#endif
...
{
	KeInitializeEvent(&khubd_wait, NotificationEvent, FALSE); // Linux-wrapper specific
