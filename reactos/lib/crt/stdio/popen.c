...
FILE *_popen (const char *cm, const char *md) /* program name, pipe mode */
{
  ...
  if ( *md == 'r' )
    {
      pf = _fdopen(__fileno_alloc(hReadPipe,  __fmode) , "r");
      CloseHandle(hWritePipe);
    }
  else
    {
      pf = _fdopen( __fileno_alloc(hWritePipe, __fmode) , "w");
      CloseHandle(hReadPipe);
    }
...
FILE *_wpopen (const wchar_t *cm, const wchar_t *md) /* program name, pipe mode */
{
  ...
  if ( *md == L'r' )
    {
      pf = _wfdopen(__fileno_alloc(hReadPipe,  __fmode) , L"r");
      CloseHandle(hWritePipe);
    }
  else
    {
      pf = _wfdopen( __fileno_alloc(hWritePipe, __fmode) , L"w");
      CloseHandle(hReadPipe);
    }
