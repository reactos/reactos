...
WsNpAllocate(VOID)
{
    ...
    /* Set non-null data */
    Provider->RefCount = 1;
    Provider->cbSize = sizeof(NSP_ROUTINE);
               ...
               IN LPGUID ProviderId)
{
    ...
    /* Call it */
    (*NSPStartupProc)(ProviderId, (LPNSP_ROUTINE)&Provider->cbSize);
...
WsNpNSPCleanup(IN PNS_PROVIDER Provider)
{
    ...
    if (Provider->DllHandle)
    {
        ...
        lpNSPCleanup = InterlockedExchangePointer((PVOID*)&Provider->NSPCleanup,
                                                  NULL);
...
{
    /* Call the NSP */
    return Provider->NSPLookupServiceEnd(LookupHandle);
}
...
{
    /* Call the NSP */
    return Provider->NSPLookupServiceNext(LookupHandle,
                                          ControlFlags,
                                          BufferLength,
                                          Results);
}
...
{
    /* Call the NSP */
    return Provider->NSPLookupServiceBegin(&Provider->ProviderId,
                                           Restrictions,
                                           ServiceClassInfo,
                                           ControlFlags,
                                           LookupHandle);
