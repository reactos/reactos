     ...
     IN INT namelen)
{
    ...
    if ((ErrorCode = WsQuickProlog()) == ERROR_SUCCESS)
    {
        ...
        {
            /* Make the call */
            Status = Socket->Provider->lpWSPBind(s,
                                                 name, 
                                                 namelen,
                                                 &ErrorCode);
...
closesocket(IN SOCKET s)
{
    ...
    if ((ErrorCode = WsQuickProlog()) == ERROR_SUCCESS)
    {
        ...
        {
            /* Make the call */
            Status = Socket->Provider->lpWSPCloseSocket(s, &ErrorCode);
          ...
          IN DWORD dwCallbackData)
{
    ...
    if ((ErrorCode = WsApiProlog(&Process, &Thread)) == ERROR_SUCCESS)
    {
        ...
        if (Socket = WsSockGetSocket(s))
        {
            ...
            /* Make the call */
            Status = Socket->Provider->lpWSPAccept(s,
                                                   addr,
                                                   addrlen,
                                                   lpfnCondition,
                                                   dwCallbackData,
            ...
            IN DWORD dwFlags)
{
    ...
    if ((ErrorCode = WsApiProlog(&Process, &Thread)) == ERROR_SUCCESS)
    {
        ...
        if (Socket = WsSockGetSocket(s))
        {
            ...
            /* Make the call */
            Status = Socket->Provider->lpWSPJoinLeaf(s,
                                                     name,
                                                     namelen,
                                                     lpCallerData,
                                                     lpCalleeData,
...
DoLookup:
    ...
    if (ErrorCode == ERROR_SUCCESS)
    {
        ...
        Status = CatalogEntry->Provider->lpWSPSocket(af,
                                                     type,
                                                     protocol,
                                                     ProtocolInfo,
                                                     g,
