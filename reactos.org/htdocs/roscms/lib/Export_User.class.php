    ...
    if ($thisuser->isMemberOfGroup('transmaint') || $thisuser->securityLevel() == 3) {
      if ($usage == 'usrtbl') {
        ...
        if ($thisuser->isMemberOfGroup('transmaint')) {
          $stmt=DBConnection::getInstance()->prepare("SELECT lang_id FROM ".ROSCMST_LANGUAGES." WHERE id = :user_id LIMIT 1");
        ...
        switch ($flag) {
          case 'addmembership':
            ...
            $stmt=DBConnection::getInstance()->prepare("SELECT 1 FROM ".ROSCMST_MEMBERSHIPS." m JOIN ".ROSCMST_GROUPS." g ON m.group_id = g.id WHERE m.user_id = :user_id AND m.group_id = :group_id AND g.security_level <= :security_level LIMIT 1");
        ...
        switch ($flag) {
          ...
          case 'detail':
            ...
            if ($thisuser->securityLevel() == 3) {
              ...
              $stmt=DBConnection::getInstance()->prepare("SELECT g.id, g.name FROM ".ROSCMST_MEMBERSHIPS." m JOIN ".ROSCMST_GROUPS." g ON g.id!=m.group_id WHERE g.security_level  <= :sec_level AND m.user_id != :user-id ORDER BY g.name ASC");
