...
<?php
	// Count the pack entries
	$query_count_pack=mysql_query("SELECT COUNT('pack_id') FROM rsdb_item_pack WHERE pack_groupid = '". $result_page["grpentr_id"] ."' ;");	
	$result_count_pack = mysql_fetch_row($query_count_pack);
	
	if ($result_count_pack[0]) {
      ...
      <tr>
        <td><div align="center"><span class="Stil2"><a name="ver"></a>Sort by:
				...
				if ($RSDB_SET_sort == "item") {
					echo "<b>Item version</b> | <a href='".$RSDB_intern_link_group_sort."ros#ver'>Package Manager version</a>";
					
					$RSDB_intern_sortby_SQL_a_query = "SELECT * 
														FROM `rsdb_item_pack` 
														WHERE `pack_groupid` = " . $RSDB_SET_group . "
														ORDER BY `pack_appversion` DESC ;";
					
					
					$RSDB_intern_sortby_headline_field = "pack_appversion";
					$RSDB_intern_sortby_linkname_field = "pack_packmgrversion";
				}
				if ($RSDB_SET_sort == "ros") {
					echo "<a href='".$RSDB_intern_link_group_sort."item#ver'>Item version</a> | <b>Package Manager version</b>";
					
					$RSDB_intern_sortby_SQL_a_query = "SELECT * 
														FROM `rsdb_item_pack` 
														WHERE `pack_groupid` = " . $RSDB_SET_group . "
														ORDER BY `pack_packmgrversion` DESC ;";
					
					
					$RSDB_intern_sortby_headline_field = "pack_packmgrversion";
					$RSDB_intern_sortby_linkname_field = "pack_appversion";
				}
		...
		if ($result_sortby_a[$RSDB_intern_sortby_headline_field] != $RSDB_intern_TEMP_version_saved_a) {
				...
				if ($RSDB_SET_sort == "item") {
							$RSDB_intern_sortby_SQL_b_query = "SELECT * 
														FROM `rsdb_item_pack` 
														WHERE `pack_groupid` = " . $RSDB_SET_group . "
														AND `pack_appversion` = '" . $result_sortby_a['pack_appversion'] . "'
														ORDER BY `pack_packmgrversion` DESC ;";
				}
				if ($RSDB_SET_sort == "ros") {
							$RSDB_intern_sortby_SQL_b_query = "SELECT * 
														FROM `rsdb_item_pack` 
														WHERE `pack_groupid` = " . $RSDB_SET_group . "
														AND `pack_packmgrversion` = '" . $result_sortby_a['pack_packmgrversion'] . "'
														ORDER BY `pack_appversion` DESC ;";
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <td width="40%"><font color="#FFFFFF" face="Arial, Helvetica, sans-serif"><strong>&nbsp;<?php
						...
						if ($RSDB_SET_sort == "item") {
							echo $result_sortby_a['pack_name']." [";
...
?>
            <tr bgcolor="<?php
								 ...
								 ?>">
              <td width="40%" bgcolor="<?php echo $farbe; ?>">&nbsp;<a href="<?php echo $RSDB_intern_link_item.$result_sortby_b['pack_id']; ?>"><?php

					if ($RSDB_SET_sort == "ros") {
						echo $result_sortby_b['pack_name']." [";
