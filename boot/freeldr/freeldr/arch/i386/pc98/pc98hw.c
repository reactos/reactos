...
DetectBiosFloppyPeripheral(PCONFIGURATION_COMPONENT_DATA ControllerKey)
{
    ...
    for (FloppyNumber = 0; FloppyNumber < Pc98GetFloppyCount(); FloppyNumber++)
    {
        ...
        FloppyData = (PVOID)(PartialDescriptor + 1);
...
DetectBiosFloppyController(PCONFIGURATION_COMPONENT_DATA BusKey)
{
    ...
    Size = FIELD_OFFSET(CM_PARTIAL_RESOURCE_LIST, PartialDescriptors[7]);
    PartialResourceList = FrLdrHeapAlloc(Size, TAG_HW_RESOURCE_LIST);
    if (PartialResourceList == NULL)
    {
...
//  PartialResourceList->PartialDescriptors[0].Flags =
    ...
    DiskGeometry = (PVOID)&PartialResourceList->PartialDescriptors[1];
    ...
    PCONFIGURATION_COMPONENT_DATA BusKey)
{
    ...
    Int13Drives = (PVOID)&PartialResourceList->PartialDescriptors[1];
...
DetectPointerController(PCONFIGURATION_COMPONENT_DATA BusKey)
{
    ...
    /* Set 'Configuration Data' value */
    Size = FIELD_OFFSET(CM_PARTIAL_RESOURCE_LIST, PartialDescriptors[HiResoMachine ? 7 : 6]);
    PartialResourceList = FrLdrHeapAlloc(Size, TAG_HW_RESOURCE_LIST);
    if (PartialResourceList == NULL)
    {
        ERR("Failed to allocate resource descriptor\n");
...
DetectParallelPorts(PCONFIGURATION_COMPONENT_DATA BusKey)
{
    ...
    /* Set 'Configuration Data' value */
    Size = FIELD_OFFSET(CM_PARTIAL_RESOURCE_LIST, PartialDescriptors[8]);
    PartialResourceList = FrLdrHeapAlloc(Size, TAG_HW_RESOURCE_LIST);
    if (PartialResourceList == NULL)
    {
        ERR("Failed to allocate resource descriptor\n");
...
DetectSerialPorts(PCONFIGURATION_COMPONENT_DATA BusKey)
{
    ...
    if (CpDoesPortExist((PUCHAR)0x30))
    {
        ...
        /* Set 'Configuration Data' value */
        Size = FIELD_OFFSET(CM_PARTIAL_RESOURCE_LIST, PartialDescriptors[HasFifo ? 11 : 4]) +
               sizeof(*SerialDeviceData);
        PartialResourceList = FrLdrHeapAlloc(Size, TAG_HW_RESOURCE_LIST);
        if (PartialResourceList == NULL)
        {
            ERR("Failed to allocate resource descriptor\n");
