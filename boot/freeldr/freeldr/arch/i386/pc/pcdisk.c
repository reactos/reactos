    ...
    IN USHORT BufferSize)
{
    ...
    *Ptr = BufferSize;
    ...
    if (BufferSize >= sizeof(EXTENDED_GEOMETRY))
    {
         ...
         * Any of these must be set if the drive removable (bit 6 if CD-ROM),
...
/**
 * @brief
 * Determine the size of interest of a CD-ROM media, using the ISO-9660
 * volume size from its ISO Primary Volume Descriptor, if any.
 *
 * @note
 * Based on lib/fs/iso.c!IsoMount().
 * This is also what MS CDFS does with the CdRvdVolSz() macro,
 * in cdfs/strucsup.c!CdUpdateVcbFromVolDescriptor().
 **/
static BOOLEAN
GetCDROMSizeViaIso9660(
    ...
    _Inout_ PULONG BytesPerSector)
{
    ...
    *Sectors = 0;
     ...
     * One SECTORSIZE sector at sector offset 16 */
    Success = PcDiskReadLogicalSectorsLBA(BiosDrive, 16ULL, 1, DiskReadBuffer);
    if (!Success /*|| Count < sizeof(PVD)*/)
        return FALSE;

    /* Check whether the PVD is valid. If so, retrieve the volume size. */
     ...
     */
    *Sectors = Pvd->VolumeSpaceSizeL; // In LBN (Logical Block Numbers)
    *BytesPerSector = Pvd->LogicalBlockSizeL; // == 2048;
    return TRUE;
}

/**
 * @brief
 * Attempts to determine the size of interest of a CD-ROM media,
 * by bisecting over valid and invalid sectors, when all else fails.
 *
    ...
    _Inout_ PULONG BytesPerSector)
{
    ...
    *Sectors = 0;
    ...
    // /* Assume standard 2048 bytes per sector */
    // *BytesPerSector = 2048;
    ...
    IN PPC_DISK_DRIVE DiskDrive)
{
    ...
    if (Success && (DiskDrive->ExtGeometry.Size == sizeof(DiskDrive->ExtGeometry)))
    {
        ...
        if (IsEnhRemovable && (Flags & 0x40))
        {
            ...
            SizeSuccess = GetCDROMSizeViaIso9660(DriveNumber, &Sectors, &BytesPerSector);
            if (SizeSuccess)
            {
                TRACE("GetCDROMSizeViaIso9660(0x%x) returned:\n"
                      "  Sectors: %llu\n"
                      "  BytesPerSector: %lu\n",
