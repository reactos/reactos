    ...
    ULONG* BytesRead)
{
    ...
    *BytesRead = 0;
    ...
    Volume = File->Volume;
    FileSystem = FatFile->obj.fs;
    Offset = f_tell(FatFile);

    /*
     * FatFs deliberately clips each disk_read at a cluster boundary. exFAT
     * marks files with a NoFatChain allocation as physically contiguous, so
     * sector-aligned reads from those files can be issued as one device read.
     * Fragmented files and partial leading sectors stay on the FatFs path.
     */
    if (Volume &&
        FileSystem == &Volume->FileSystem &&
        FileSystem->fs_type == FS_EXFAT &&
        FatFile->obj.stat == 2 &&
        FatFile->obj.sclust >= 2 &&
        FatFile->obj.sclust < FileSystem->n_fatent &&
        FileSystem->csize != 0 &&
        Offset < f_size(FatFile) &&
        Offset % Volume->BytesPerSector == 0)
    {
        Remaining = f_size(FatFile) - Offset;
        if (Size > Remaining)
            Size = (ULONG)Remaining;

        DirectSize = Size - Size % Volume->BytesPerSector;
        SectorOffset = (LBA_t)(FatFile->obj.sclust - 2) * FileSystem->csize +
                       Offset / Volume->BytesPerSector;
        SectorCount = DirectSize / Volume->BytesPerSector;

        if (DirectSize != 0 &&
            FileSystem->database <= Volume->SectorCount &&
            SectorOffset <= Volume->SectorCount - FileSystem->database)
        {
            Sector = FileSystem->database + SectorOffset;
            if (SectorCount <= Volume->SectorCount - Sector)
            {
                if (!ExFatDeviceRead(Volume, Buffer, Sector, DirectSize))
                    return EIO;

                Result = f_lseek(FatFile, Offset + DirectSize);
                if (Result != FR_OK)
                    return ExFatMapStatus(Result);

                *BytesRead = DirectSize;
                Buffer = (PUCHAR)Buffer + DirectSize;
                Size -= DirectSize;
            }
        }
    }
