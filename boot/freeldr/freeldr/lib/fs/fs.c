    ...
    _Out_ PULONGLONG VolumeSize)
{
    ...
    *VolumeSize = 0;
     ...
     * and there is a "service" (file-system) name associated */
    FuncTable = pDevice->FileFuncTable;
    ...
    if (!_wcsicmp(ServiceName, L"cdfs"))
        return IsoGetVolumeSize(DeviceId, VolumeSize);
    else
#endif
    if (!_wcsicmp(ServiceName, L"fastfat") || !_wcsicmp(ServiceName, L"vfatfs"))
        return FatGetVolumeSize(DeviceId, VolumeSize);
    else
    if (!_wcsicmp(ServiceName, L"btrfs"))
        return BtrFsGetVolumeSize(DeviceId, VolumeSize);
    else
#ifndef _M_ARM
    if (!_wcsicmp(ServiceName, L"ntfs"))
        return NtfsGetVolumeSize(DeviceId, VolumeSize);
    else
    if (!_wcsicmp(ServiceName, L"ext2fs"))
        return ExtGetVolumeSize(DeviceId, VolumeSize);
    else
#endif
