...
BOOLEAN ExtLookupFile(PEXT_VOLUME_INFO Volume, PCSTR FileName, PEXT_FILE_INFO ExtFileInfo)
{
    ...
    // Alternatively, use PathPart.
    RtlStringCbCopyNA(ExtFileInfo->FileName, sizeof(ExtFileInfo->FileName),
                      DirectoryEntry.Name, DirectoryEntry.NameLen);
    ExtFileInfo->FileNameLength = strlen(ExtFileInfo->FileName);
