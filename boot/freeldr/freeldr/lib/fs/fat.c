...
ARC_STATUS FatLookupFile(PFAT_VOLUME_INFO Volume, PCSTR FileName, PFAT_FILE_INFO FatFileInfoPointer)
{
    ...
    RtlStringCbCopyA(FatFileInfoPointer->FileName, sizeof(FatFileInfoPointer->FileName), PathPart);
    FatFileInfoPointer->FileNameLength = strlen(FatFileInfoPointer->FileName);
...
ARC_STATUS FatGetFileInformation(ULONG FileId, FILEINFORMATION* Information)
{
    ...
    /* Copy the file name, perhaps truncated, and NUL-terminated */
    RtlStringCbCopyNA(Information->FileName, sizeof(Information->FileName),
                      FileHandle->FileName, FileHandle->FileNameLength);
