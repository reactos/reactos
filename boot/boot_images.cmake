...
# ISO image boot parameters
set(ISO_BOOT_PARAMETERS "")
...
if(DEFINED EFI_PLATFORM_ID)
  ...
  string(APPEND ISO_BOOT_PARAMETERS "-eltorito-alt-boot -eltorito-platform efi -eltorito-boot loader/efisys.bin -no-emul-boot")
endif()
...
separate_arguments(ISO_BOOT_PARAMETERS WINDOWS_COMMAND "${ISO_BOOT_PARAMETERS}")
...
add_custom_target(bootcd
    COMMAND native-mkisofs -quiet -o ${REACTOS_BINARY_DIR}/bootcd.iso -iso-level 4
        ...
        -eltorito-boot loader/isoboot.bin -no-emul-boot -boot-load-size 4 ${ISO_BOOT_PARAMETERS} -hide boot.catalog
...
add_custom_target(bootcdregtest
    COMMAND native-mkisofs -quiet -o ${REACTOS_BINARY_DIR}/bootcdregtest.iso -iso-level 4
        -publisher ${ISO_MANUFACTURER} -preparer ${ISO_MANUFACTURER} -volid ${ISO_VOLNAME} -volset ${ISO_VOLNAME}
        -eltorito-boot loader/isobtrt.bin -no-emul-boot -boot-load-size 4 ${ISO_BOOT_PARAMETERS} -hide boot.catalog
        -sort ${CMAKE_CURRENT_BINARY_DIR}/bootfiles.sort
