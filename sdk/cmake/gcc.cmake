...
function(generate_import_lib _libname _dllname _spec_file)
    ...
    # With this, we let DLLTOOL create the object files
    set(LIBRARY_PRIVATE_DIR ${CMAKE_CURRENT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/${_libname}.dir)
    add_custom_command(
        OUTPUT ${LIBRARY_PRIVATE_DIR}/${_libname}t.o ${LIBRARY_PRIVATE_DIR}/${_libname}h.o
        COMMAND ${CMAKE_COMMAND} -E rm -f ${LIBRARY_PRIVATE_DIR}/${_libname}s*.o # Remove all previously generated files
        COMMAND ${CMAKE_DLLTOOL} --def ${CMAKE_CURRENT_BINARY_DIR}/${_libname}_implib.def --kill-at --output-lib=${NULDEV} -t ${_libname} -n -n # Produce obj files
        DEPENDS ${CMAKE_CURRENT_BINARY_DIR}/${_libname}_implib.def
        WORKING_DIRECTORY ${LIBRARY_PRIVATE_DIR})
    set_source_files_properties(
        ${LIBRARY_PRIVATE_DIR}/${_libname}t.o
        ${LIBRARY_PRIVATE_DIR}/${_libname}s.o
        PROPERTIES
        EXTERNAL_OBJECT TRUE)

    # We create a static library with all the object files thus created
    _add_library(${_libname} STATIC EXCLUDE_FROM_ALL
        ${LIBRARY_PRIVATE_DIR}/${_libname}t.o
        ${LIBRARY_PRIVATE_DIR}/${_libname}h.o)
    #list all files generated by dlltool into a command file
    add_custom_command(TARGET ${_libname} PRE_LINK
        COMMAND ${CMAKE_COMMAND} -E echo ${LIBRARY_PRIVATE_DIR}/${_libname}s*.o > ${LIBRARY_PRIVATE_DIR}/objs.lst)
    set_target_properties(${_libname}
        PROPERTIES
        LINKER_LANGUAGE "C"
        PREFIX ""
        STATIC_LIBRARY_OPTIONS @${LIBRARY_PRIVATE_DIR}/objs.lst)
    ...
    _add_library(${_libname}_delayed STATIC EXCLUDE_FROM_ALL
        ...
        ${LIBRARY_PRIVATE_DIR}/${_libname}h.o)
    #list all files generated by dlltool into a command file
    add_custom_command(TARGET ${_libname}_delayed PRE_LINK
        COMMAND ${CMAKE_COMMAND} -E echo ${LIBRARY_PRIVATE_DIR}/${_libname}s*.o > ${LIBRARY_PRIVATE_DIR}/objs.lst)
