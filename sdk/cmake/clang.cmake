...
function(CreateBootSectorTarget _target_name _asm_file _binary_file _base_address)
    ...
    # Boot sector files are 16-bit real mode assembly. Assemble as ELF with
    # Clang/LLVM to avoid COFF's unsupported 16-bit relocations, then have LLD
    # emit the flat binary at the requested load address.
    ...
    # Step 3: Link a flat binary at the expected real-mode load address
    add_custom_command(
        OUTPUT ${_binary_file}
        COMMAND ${CMAKE_LINKER} -m elf_i386 --image-base=0 -Ttext=0x${_base_address} --oformat=binary -o ${_binary_file} ${_object_file}
...
# Clang/LLVM runtime libraries. Every path is resolved by the compiler itself,
# so there is no toolchain root to locate or configure.

function(_query_target_runtime _outvar _compiler)
    execute_process(
        COMMAND ${_compiler} --target=${_target_tool_triplet} ${ARGN}
        OUTPUT_VARIABLE _runtime_path
        OUTPUT_STRIP_TRAILING_WHITESPACE
        ERROR_QUIET)
    if(NOT IS_ABSOLUTE "${_runtime_path}" OR NOT EXISTS "${_runtime_path}")
        message(FATAL_ERROR "llvm-mingw runtime not found: ${_compiler} --target=${_target_tool_triplet} ${ARGN}")
    endif()
    ...
    # libc++ headers live next to the lib: <triple>/lib/libc++.a -> <triple>/include/c++/v1
    get_filename_component(_libcxx_prefix "${_llvm_libcxx}" DIRECTORY)
    get_filename_component(_libcxx_prefix "${_libcxx_prefix}" DIRECTORY)
    set(_reactos_cppstl_pre_include_flags " -I${_libcxx_prefix}/include/c++/v1")
