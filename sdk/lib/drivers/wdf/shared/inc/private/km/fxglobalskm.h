...
typedef
BOOLEAN
(NTAPI *PFN_KD_REFRESH)(
    );

typedef
VOID
(NTAPI *PFN_KE_FLUSH_QUEUED_DPCS)(
    VOID
    );

typedef
NTSTATUS
(NTAPI *PFN_IO_SET_COMPLETION_ROUTINE_EX)(
...
typedef
BOOLEAN
(NTAPI *PFN_KE_REGISTER_BUGCHECK_REASON_CALLBACK) (
...
typedef
VOID
(NTAPI *PFN_IO_DISCONNECT_INTERRUPT)(
...
typedef
BOOLEAN
(NTAPI *PFN_PO_GET_SYSTEM_WAKE)(
    __in PIRP Irp
    );

typedef
VOID
(NTAPI *PFN_PO_SET_SYSTEM_WAKE)(
    __inout PIRP Irp
    );

typedef
KAFFINITY
(NTAPI *PFN_KE_QUERY_ACTIVE_PROCESSORS)(
    VOID
    );

typedef
VOID
(NTAPI *PFN_KE_SET_TARGET_PROCESSOR_DPC)(
    __in PRKDPC  Dpc,
    __in CCHAR  Number
    );
...
typedef
ULONG
(NTAPI *PFN_KE_GET_CURRENT_PROCESSOR_NUMBER)(
    VOID
    );

typedef
ULONG
(NTAPI *PFN_KE_GET_CURRENT_PROCESSOR_NUMBER_EX)(
    __out_opt PPROCESSOR_NUMBER ProcNumber
    );

typedef
ULONG
(NTAPI *PFN_KE_QUERY_MAXIMUM_PROCESSOR_COUNT_EX)(
    __in USHORT GroupNumber
    );

typedef
ULONG
(NTAPI *PFN_KE_QUERY_MAXIMUM_PROCESSOR_COUNT)(
    VOID
    );

typedef
BOOLEAN
(NTAPI *PFN_KE_ARE_APCS_DISABLED)(
    VOID
    );

typedef
ULONG
(NTAPI *PFN_KE_GET_RECOMMENDED_SHARED_DATA_ALIGNMENT)(
    VOID
    );

typedef
NTSTATUS
(NTAPI *PFN_IO_UNREGISTER_PLUGPLAY_NOTIFICATION_EX)(
    __in PVOID NotificationEntry
    );

typedef
NTSTATUS
(NTAPI *PFN_POX_REGISTER_DEVICE) (
...
typedef
VOID
(NTAPI *PFN_POX_START_DEVICE_POWER_MANAGEMENT) (
    __in POHANDLE Handle
    );

typedef
VOID
(NTAPI *PFN_POX_UNREGISTER_DEVICE) (
    __in POHANDLE Handle
    );

typedef
VOID
(NTAPI *PFN_POX_ACTIVATE_COMPONENT) (
    __in POHANDLE Handle,
...
typedef
VOID
(NTAPI *PFN_POX_REPORT_DEVICE_POWERED_ON) (
    __in POHANDLE Handle
    );

typedef
VOID
(NTAPI *PFN_POX_COMPLETE_IDLE_STATE) (
    __in POHANDLE Handle,
    __in ULONG Component
    );
...
typedef
VOID
(NTAPI *PFN_POX_COMPLETE_IDLE_CONDITION) (
    __in POHANDLE Handle,
    __in ULONG Component
    );
...
typedef
VOID
(NTAPI *PFN_POX_COMPLETE_DEVICE_POWER_NOT_REQUIRED) (
    __in POHANDLE Handle
    );

typedef
VOID
(NTAPI *PFN_POX_SET_DEVICE_IDLE_TIMEOUT) (
    __in POHANDLE Handle,
...
typedef
VOID
(NTAPI *PFN_IO_REPORT_INTERRUPT_ACTIVE) (
    _In_ PIO_REPORT_INTERRUPT_ACTIVE_STATE_PARAMETERS Parameters
    );

typedef
VOID
(NTAPI *PFN_IO_REPORT_INTERRUPT_INACTIVE) (
    _In_ PIO_REPORT_INTERRUPT_ACTIVE_STATE_PARAMETERS Parameters
    );

typedef
VOID
(NTAPI *PFN_VF_CHECK_NX_POOL_TYPE) (
