...
typedef void (*PFNOUTHEADER)(FILE *, char *);
typedef int (*PFNOUTLINE)(FILE *, EXPORT *);
typedef void (*PFNOUTFOOTER)(FILE *);

typedef struct _OUTFILE
{
    FILE *file;
    PFNOUTHEADER OutputHeader;
    PFNOUTLINE   OutputLine;
    PFNOUTFOOTER OutputFooter;
} OUTFILE, *POUTFILE;
...
void
OutputFooter_asmstub(FILE *file)
{
    fprintf(file, "\n    END\n");
}
...
int
ParseFile(char* pcStart, POUTFILE outFiles, unsigned int numOutFiles)
{
    char *pc, *pcLine;
    int nLine;
    EXPORT exp;
    ...
    for (pcLine = pcStart; *pcLine; pcLine = NextLine(pcLine), nLine++)
    {
        ...
        {
            if (outFiles[i].file && outFiles[i].OutputLine)
                outFiles[i].OutputLine(outFiles[i].file, &exp);
        }
        gbDebug = 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (pszDefFileName)
    {
        ...
        outFiles[numOutFiles].file = file;
        outFiles[numOutFiles].OutputHeader = OutputHeader_def;
        outFiles[numOutFiles].OutputLine   = OutputLine_def;
        outFiles[numOutFiles].OutputFooter = NULL;
        ++numOutFiles;
    }
    ...
    if (pszStubFileName)
    {
        ...
        outFiles[numOutFiles].file = file;
        outFiles[numOutFiles].OutputHeader = OutputHeader_stub;
        outFiles[numOutFiles].OutputLine   = OutputLine_stub;
        outFiles[numOutFiles].OutputFooter = NULL;
        ++numOutFiles;
    }
    ...
    if (pszLibStubName)
    {
        ...
        outFiles[numOutFiles].file = file;
        outFiles[numOutFiles].OutputHeader = OutputHeader_asmstub;
        outFiles[numOutFiles].OutputLine   = OutputLine_asmstub;
        outFiles[numOutFiles].OutputFooter = OutputFooter_asmstub;
        ++numOutFiles;
    }
    ...
    for (i = 0; i < numOutFiles; ++i)
    {
        if (outFiles[i].file && outFiles[i].OutputHeader)
            outFiles[i].OutputHeader(outFiles[i].file, pszDllName);
    }

    /* Parse the input file and write to the output files */
    result = ParseFile(pszSource, outFiles, numOutFiles);

    /* Write the footers and close the files */
    for (i = 0; i < numOutFiles; ++i)
    {
        if (outFiles[i].file)
        {
            if (outFiles[i].OutputFooter)
                outFiles[i].OutputFooter(outFiles[i].file);
            fclose(outFiles[i].file);
        }
    }
