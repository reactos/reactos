...
public:
  ...
  {
    return NotImplemented;  // FIXME: not available: SetStatus(DllExports::GdipGetEncoderParameterList(nativeImage, clsidEncoder, size, buffer));
  }
...
//     return size;
  ...
  {
    return SetStatus(DllExports::GdipGetImageDimension(nativeImage, size ? &size->Width : NULL, size ? &size->Height : NULL));
  }
  ...
  Image *GetThumbnailImage(UINT thumbWidth, UINT thumbHeight, GetThumbnailImageAbort callback, VOID *callbackData)
  {
    ...
    SetStatus(DllExports::GdipGetImageThumbnail(nativeImage, thumbWidth, thumbHeight, thumbImage ? &thumbImage->nativeImage : NULL, callback, callbackData));
    return thumbImage;
  }
  ...
  {
    return NotImplemented;  // FIXME: not available: SetStatus(DllExports::GdipSaveAddImage(nativeImage, newImage ? newImage->nativeImage : NULL, encoderParams));
  }
