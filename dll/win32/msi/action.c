...
const WCHAR *msi_get_command_line_option(const WCHAR *cmd, const WCHAR *option, UINT *len)
{
    ...
    while (*cmd)
    {
        ...
        if(!_wcsnicmp(cmd, option, opt_len))
            found = TRUE;
...
static LPWSTR resolve_keypath( MSIPACKAGE* package, MSICOMPONENT *cmp )
{
    ...
    if (cmp->Attributes & msidbComponentAttributesRegistryKeyPath)
    {
        ...
        if (deformated_name)
            swprintf(buffer,fmt2,root,deformated,deformated_name);
        else
            swprintf(buffer,fmt,root,deformated);
    ...
    LIST_FOR_EACH_ENTRY( comp, &package->components, MSICOMPONENT, entry )
    {
        ...
        if (action == INSTALLSTATE_LOCAL || action == INSTALLSTATE_SOURCE)
        {
            ...
            else
            {
                ...
                swprintf(source, fmt, MSI_RecordGetInteger(row, 1));
                                       ...
                                       LPWSTR lpszName, LONG_PTR lParam)
{
    ...
    else
    {
        ...
        swprintf(tl_struct->path,fmt,tl_struct->source, lpszName);
    }
...
static void register_dll( const WCHAR *dll, BOOL unregister )
{
    ...
    if (unregister) swprintf( cmd, unregW, dll );
    else swprintf( cmd, regW, dll );
...
static UINT msi_publish_install_properties(MSIPACKAGE *package, HKEY hkey)
{
    ...
    swprintf(date, date_fmt, systime.wYear, systime.wMonth, systime.wDay);
...
UINT ACTION_ForceReboot(MSIPACKAGE *package)
{
    ...
    swprintf(buffer, msiexec_fmt, sysdir, squashed_pc);
    ...
    swprintf( buffer, install_fmt, package->ProductCode, squashed_pc );
...
WCHAR *msi_create_component_advertise_string( MSIPACKAGE *package, MSICOMPONENT *component, const WCHAR *feature )
{
    ...
    if (ret) swprintf( ret, fmt, productid_85, feature, component ? '>' : '<', component_85 );
    return ret;
}
...
static UINT ITERATE_InstallODBCDriver( MSIRECORD *rec, LPVOID param )
{
    ...
    len = swprintf(ptr, driver_fmt, driver_file->FileName);
    ptr += len + 1;

    if (setup_file)
    {
        len = swprintf(ptr, setup_fmt, setup_file->FileName);
        ptr += len + 1;
    }
...
static UINT ITERATE_InstallODBCTranslator( MSIRECORD *rec, LPVOID param )
{
    ...
    len = swprintf(ptr, translator_fmt, translator_file->FileName);
    ptr += len + 1;
...
static UINT ITERATE_InstallODBCDataSource( MSIRECORD *rec, LPVOID param )
{
    ...
    len = swprintf(attrs, attrs_fmt, desc);
    attrs[len + 1] = 0;
...
static UINT ITERATE_RemoveExistingProducts( MSIRECORD *rec, LPVOID param )
{
    ...
    swprintf( cmd, fmtW, product, features ? features : szAll );
    msi_free( product );
    msi_free( features );
