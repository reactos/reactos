...
UINT MSIREG_OpenProductKey(LPCWSTR szProduct, LPCWSTR szUserSid, MSIINSTALLCONTEXT context, HKEY *key, BOOL create)
{
    ...
    else
    {
        ...
        swprintf( keypath, szInstaller_LocalManagedProd_fmt, szUserSid, squashed_pc );
        LocalFree(usersid);
    }
                            ...
                            HKEY *key, BOOL create)
{
    ...
    else
    {
        ...
        swprintf( keypath, szInstaller_LocalManagedFeat_fmt, szUserSid, squashed_pc );
        LocalFree(usersid);
    }
    if (create) return RegCreateKeyExW(root, keypath, 0, NULL, 0, access, NULL, key, NULL);
    return RegOpenKeyExW(root, keypath, 0, access, key);
                                    ...
                                    HKEY *key, BOOL create)
{
    ...
    if (context == MSIINSTALLCONTEXT_MACHINE)
    {
        swprintf( keypath, szUserDataFeatures_fmt, szLocalSid, squashed_pc );
    }
    else
    {
        if (!szUserSid)
        ...
        swprintf( keypath, szUserDataFeatures_fmt, szUserSid, squashed_pc );
        LocalFree(usersid);
    }
    if (create) return RegCreateKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, NULL, 0, access, NULL, key, NULL);
    return RegOpenKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, access, key);
...
UINT MSIREG_OpenUserDataComponentKey(LPCWSTR szComponent, LPCWSTR szUserSid, HKEY *key, BOOL create)
{
    ...
    if (!szUserSid)
    {
        ...
        swprintf( keypath, szUserDataComp_fmt, usersid, squashed_comp );
        LocalFree(usersid);
    }
    else
        swprintf( keypath, szUserDataComp_fmt, szUserSid, squashed_comp );
...
UINT MSIREG_DeleteUserDataComponentKey(LPCWSTR szComponent, LPCWSTR szUserSid)
{
    ...
    if (!szUserSid)
    {
        ...
        swprintf(keypath, szUserDataComponents_fmt, usersid);
        LocalFree(usersid);
    }
    else
        swprintf(keypath, szUserDataComponents_fmt, szUserSid);
...
UINT MSIREG_OpenUserDataProductKey(LPCWSTR szProduct, MSIINSTALLCONTEXT dwContext, LPCWSTR szUserSid, HKEY *key, BOOL create)
{
    ...
    if (dwContext == MSIINSTALLCONTEXT_MACHINE)
        swprintf( keypath, szUserDataProd_fmt, szLocalSid, squashed_pc );
    else if (szUserSid)
        swprintf( keypath, szUserDataProd_fmt, szUserSid, squashed_pc );
    else
    {
        if (!(usersid = get_user_sid()))
        {
        ...
        swprintf( keypath, szUserDataProd_fmt, usersid, squashed_pc );
        LocalFree(usersid);
    }
    if (create) return RegCreateKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, NULL, 0, access, NULL, key, NULL);
    return RegOpenKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, access, key);
...
UINT MSIREG_OpenUserDataPatchKey(LPCWSTR szPatch, MSIINSTALLCONTEXT dwContext, HKEY *key, BOOL create)
{
    ...
    if (dwContext == MSIINSTALLCONTEXT_MACHINE)
        swprintf( keypath, szUserDataPatch_fmt, szLocalSid, squashed_patch );
    else
    {
        if (!(usersid = get_user_sid()))
        {
        ...
        swprintf( keypath, szUserDataPatch_fmt, usersid, squashed_patch );
        LocalFree(usersid);
    }
    if (create) return RegCreateKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, NULL, 0, access, NULL, key, NULL);
    return RegOpenKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, access, key);
...
UINT MSIREG_DeleteUserDataPatchKey(LPCWSTR patch, MSIINSTALLCONTEXT context)
{
    ...
    if (context == MSIINSTALLCONTEXT_MACHINE)
        swprintf(keypath, szUserDataPatches_fmt, szLocalSid);
    else
    {
        if (!(usersid = get_user_sid()))
        {
        ...
        swprintf(keypath, szUserDataPatches_fmt, usersid);
        LocalFree(usersid);
    }
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, access, &hkey)) return ERROR_SUCCESS;
...
UINT MSIREG_OpenUserDataProductPatchesKey(LPCWSTR product, MSIINSTALLCONTEXT context, HKEY *key, BOOL create)
{
    ...
    if (context == MSIINSTALLCONTEXT_MACHINE)
        swprintf( keypath, szUserDataProductPatches_fmt, szLocalSid, squashed_product );
    else
    {
        if (!(usersid = get_user_sid()))
        {
        ...
        swprintf( keypath, szUserDataProductPatches_fmt, usersid, squashed_product );
        LocalFree(usersid);
    }
    if (create) return RegCreateKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, NULL, 0, access, NULL, key, NULL);
    return RegOpenKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, access, key);
...
UINT MSIREG_OpenInstallProps(LPCWSTR szProduct, MSIINSTALLCONTEXT dwContext, LPCWSTR szUserSid, HKEY *key, BOOL create)
{
    ...
    if (dwContext == MSIINSTALLCONTEXT_MACHINE)
        swprintf( keypath, szInstallProperties_fmt, szLocalSid, squashed_pc );
    else if (szUserSid)
        swprintf( keypath, szInstallProperties_fmt, szUserSid, squashed_pc );
    else
    {
        if (!(usersid = get_user_sid()))
        {
        ...
        swprintf( keypath, szInstallProperties_fmt, usersid, squashed_pc );
        LocalFree(usersid);
    }
    if (create) return RegCreateKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, NULL, 0, access, NULL, key, NULL);
    return RegOpenKeyExW(HKEY_LOCAL_MACHINE, keypath, 0, access, key);
...
UINT MSIREG_DeleteUserDataProductKey(LPCWSTR szProduct, MSIINSTALLCONTEXT context)
{
    ...
    if (context == MSIINSTALLCONTEXT_MACHINE)
        swprintf(keypath, szUserDataProducts_fmt, szLocalSid);
    else
    {
        if (!(usersid = get_user_sid()))
        {
        ...
        swprintf(keypath, szUserDataProducts_fmt, usersid);
        LocalFree(usersid);
    }
...
UINT MSIREG_OpenPatchesKey(LPCWSTR szPatch, HKEY *key, BOOL create)
{
    ...
    swprintf( keypath, szInstaller_Patches, squashed_pc );
