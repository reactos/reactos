...
static MSIPROGID *load_progid( MSIPACKAGE* package, MSIRECORD *row )
{
    ...
    if (!MSI_RecordIsNull(row,6))
    {
        ...
        swprintf(progid->IconPath,fmt,FilePath,icon_index);
...
static MSICLASS *load_class( MSIPACKAGE* package, MSIRECORD *row )
{
    ...
    if (!MSI_RecordIsNull(row,9))
    {
        ...
        swprintf(cls->IconPath,fmt,FilePath,icon_index);
                ...
                MSIVERB* verb, INT* Sequence )
{
    ...
    if (verb->Argument)
     ...
     if (verb->Argument)
         swprintf(command, fmt, component->FullKeypath, verb->Argument);
     else
         swprintf(command, fmt2, component->FullKeypath);
