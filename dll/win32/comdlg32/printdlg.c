	...
	if(GetDlgItem(hDlg, cmb1))
	{
		...
		for(i = 0; i < (numResolutions * 2); i += 2)
		{
			...
			if(dpiX == Resolutions[i])
			    IsDefault = TRUE;
#ifdef __REACTOS__ //FIXME: This should complile with long support something is interfering
                ...
                for (i = 0; i < (num_resolutions * 2); i += 2)
                {
                    ...
                    if (resolutions[i] == resolutions[i+1])
                    {
                        if (dpiX == resolutions[i])
                            is_default = TRUE;
                        #ifdef __REACTOS__
                        swprintf(buf, L"%ld dpi", resolutions[i]);
                        #else
                    ...
                    } else
                    {
                        if (dpiX == resolutions[i] && dpiY == resolutions[i+1])
                            is_default = TRUE;
                        #ifdef __REACTOS__
                        swprintf(buf, L"%ld dpi x %ld dpi", resolutions[i], resolutions[i+1]);
                        #else
