    ...
    OUT PULONG ProfileBufferLength)
{
    ...
    *ProfileBufferLength = 0;
    ...
    BufferLength = sizeof(MSV1_0_LM20_LOGON_PROFILE) +
                   ComputerNameUCS.Length + sizeof(WCHAR);

    Status = NtlmAllocateClientBuffer(ClientRequest,
                                      BufferLength,
                                      &Buffer);
    if (!NT_SUCCESS(Status))
    {
...
/*
 * Handle Interactive, Batch and Service logon
 */
NTSTATUS
NTAPI
LsaApLogonUserEx2_Interactive()
{
    //TODO move code from LsaApLogonUserEx2
    return STATUS_NOT_IMPLEMENTED;
}
...
LsaApLogonUserEx2_Network(
    ...
    OUT PUNICODE_STRING* LogonUserRef,
    OUT PUNICODE_STRING* LogonDomainRef,
    IN OUT PLSA_SAM_PWD_DATA LogonPwdData,
    ...
    OUT PMSV1_0_LM20_LOGON_PROFILE *LogonProfile,
    OUT PULONG LogonProfileSize,
    OUT PNTSTATUS SubStatus)
{
    ...
    *SpecialAccount = FALSE;
    LogonInfo = (PMSV1_0_LM20_LOGON)ProtocolSubmitBuffer;
    ...
    {
        Status = BuildLm20LogonProfileBuffer(ClientRequest, *UserInfoPtr,
            LogonPwdData, LogonProfile, LogonProfileSize);
        if (!NT_SUCCESS(Status))
        {
                  ...
                  OUT PSECPKG_SUPPLEMENTAL_CRED_ARRAY *SupplementalCredentials) /* Not supported yet */
{
    ...
    LSA_SAM_PWD_DATA LogonPwdData = { FALSE, NULL, NULL, NULL };
    ...
    *AuthenticatingAuthority = NULL;
    ...
    #else
    {
        PNTLMSSP_GLOBALS_SVR gsvr = lockGlobalsSvr();
        {
            ...
            RtlCopyUnicodeString(&ComputerName, (PUNICODE_STRING)&gsvr->NbMachineName);
            ComputerNameSize = ComputerName.Length / sizeof(WCHAR);
        doneCN:
            unlockGlobalsSvr(&gsvr);
            if (!NT_SUCCESS(Status))
                goto done;
        }
    }
    ...
    {
        Status = LsaApLogonUserEx2_Network(
                    ClientRequest,
                    ProtocolSubmitBuffer,
                    ClientBufferBase,
                    SubmitBufferSize,
        ...
        *MachineName = DispatchTable.AllocateLsaHeap(sizeof(UNICODE_STRING));
        if (*MachineName != NULL)
        {
            ...
            if ((*MachineName)->Buffer != NULL)
            {
                ...
                RtlCopyMemory((*MachineName)->Buffer, ComputerName.Buffer, (*MachineName)->MaximumLength);
            }
        }
