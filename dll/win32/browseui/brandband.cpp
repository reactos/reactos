...
void CBrandBand::SelectImage()
{
	...
	GetObject(fImageBitmap, sizeof(bitmapInfo), &bitmapInfo);
...
HRESULT STDMETHODCALLTYPE CBrandBand::SetSite(IUnknown* pUnkSite)
{
	...
	hwnd = SHCreateWorkerWindow(0, parentWindow, 0, WS_VISIBLE | WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, NULL, 0);
	if (hwnd == NULL)
		return E_FAIL;
	SubclassWindow(hwnd);
...
HRESULT STDMETHODCALLTYPE CBrandBand::CloseDW(unsigned long dwReserved)
{
    ShowDW(FALSE);
