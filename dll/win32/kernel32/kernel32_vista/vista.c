    ...
    _Inout_ PDWORD lpdwSize)
{
    ...
    if (dwFlags && dwFlags != PROCESS_NAME_NATIVE)
    {
    ...
    Status = NtQueryInformationProcess(hProcess,
                                       dwFlags ? ProcessImageFileName : ProcessImageFileNameWin32,
                                       pBuffer,
                                       dwBufferSize,
                                       NULL);
    ...
    {
        uCch = pBuffer->Length / sizeof(WCHAR);
        if (uCch >= *lpdwSize)
        {
            BaseSetLastNTError(STATUS_BUFFER_TOO_SMALL);
        }
