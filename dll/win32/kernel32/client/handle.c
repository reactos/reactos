                ...
                IN DWORD dwOptions)
{
    ...
    { // check if this belongs to AFD, if so it's a winsock socket
		...
		if (!winsock_module) 
        { // only do once. we do not load, must already be laded
		...
		if(isSocket && canDupe && winsock_module ) 
        { 
			SOCKET targetSocket;
        ...
        {
			if (isSocket && winsock_module && !canDupe) 
            {
						...
						if (result != 0) 
                        {
							...
							if (targetSocket != INVALID_SOCKET) 
                            {
								...
								*lpTargetHandle = (HANDLE)targetSocket;							
								...
								DPRINT1("DuplicateHandle: WSASocketW on cross process call returned INVALID_SOCKET, returning input handle(%lx) (BADIMPL FIXME).\n", hSourceHandle);						
						}
					}			
					_SEH2_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
					{
