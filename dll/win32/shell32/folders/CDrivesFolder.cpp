...
enum drives_columns
{
    DRIVES_COL_NAME,
    DRIVES_COL_TYPE,
    DRIVES_COL_TOTAL_SIZE,
    DRIVES_COL_FREE_SIZE,
    DRIVES_COL_COMMENTS,
    DRIVES_COL_COUNT
};
...
HRESULT WINAPI CDrivesFolder::CompareIDs(LPARAM lParam, PCUIDLIST_RELATIVE pidl1, PCUIDLIST_RELATIVE pidl2)
{
    ...
    if (!_ILIsDrive(pidl1) || !_ILIsDrive(pidl2) || LOWORD(lParam) >= DRIVES_COL_COUNT)
        return E_INVALIDARG;
...
HRESULT WINAPI CDrivesFolder::GetDefaultColumnState(UINT iColumn, DWORD * pcsFlags)
{
    ...
    if (!pcsFlags || iColumn >= DRIVES_COL_COUNT)
        return E_INVALIDARG;
...
HRESULT WINAPI CDrivesFolder::GetDetailsOf(PCUITEMID_CHILD pidl, UINT iColumn, SHELLDETAILS *psd)
{
    ...
    if (!psd || iColumn >= DRIVES_COL_COUNT)
        return E_INVALIDARG;
