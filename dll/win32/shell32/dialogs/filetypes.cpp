...
FileTypesDlg_InsertToLV(HWND hListView, LPCWSTR Assoc, INT iItem, PFILE_TYPE_GLOBALS g)
{
    ...
    Entry->EditFlags = 0;
    DWORD editflags;
    dwSize = sizeof(editflags);
    if (!RegQueryValueExW(hKey, L"EditFlags", NULL, &dwType, (BYTE*)&editflags, &dwSize) &&
        (dwType == REG_DWORD || dwType == REG_BINARY) && dwSize == sizeof(DWORD))
    {
        Entry->EditFlags = editflags;
    }
    ...
    BOOL defaultIcon = !GetFileTypeIconsByKey(hKey, Entry, g->IconSize);
    ...
    {
        iSmallImage = ImageList_AddIcon(g->himlSmall, Entry->hIconSmall);
    }
    ...
    lvItem.pszText = Assoc[0] == L'.' ? _wcsupr(&Entry->FileExtension[1]) : g->NoneString;
            ...
            switch (LOWORD(wParam))
            {
                ...
                case IDC_FILETYPES_CHANGE:
                    ...
                    if (pEntry)
                    {
                        ...
                        Info.pcszClass = NULL;
                        if (SUCCEEDED(SHOpenWithDialog(hwndDlg, &Info)))
                        {
                            pEntry->InvalidateDefaultApp();
        ...
        case WM_NOTIFY:
            ...
            switch (lppl->hdr.code)
            {
                case LVN_GETDISPINFO:
                {
                    ...
                    PFILE_TYPE_ENTRY p = (PFILE_TYPE_ENTRY) pLVDI->item.lParam;
                    if (p && (pLVDI->item.mask & LVIF_TEXT))
                    {
                        if (pLVDI->item.iSubItem == 1)
                        {
                            pLVDI->item.pszText = GetTypeName(p, pGlobals);
