...
static RECT BUTTON_GetTextRect(const BUTTON_INFO *infoPtr, HDC hdc, const WCHAR *text, LONG maxWidth)
{
    ...
    /* TODO: do not ignore WM_SETTHEME */
    if (theme)
    {
...
#ifdef __REACTOS__
    /* Add margin from the theme data to fix start button */
    HTHEME theme = GetWindowTheme(infoPtr->hwnd);
    if (theme)
    {
        RECT bgRect = {0};
        ...
        HDC hdc = GetDC(infoPtr->hwnd);
        GetThemeBackgroundContentRect(theme, hdc, BP_PUSHBUTTON, infoPtr->state, &bgRect, &labelRect);
        ReleaseDC(infoPtr->hwnd, hdc);

        size->cx += (bgRect.right - bgRect.left) - (labelRect.right - labelRect.left);
        size->cy += (bgRect.bottom - bgRect.top) - (labelRect.bottom - labelRect.top);
    }
    else
    {
        size->cx += 4;
