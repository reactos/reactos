...
LoadSystemCursors(VOID)
{
    ...
    for (i = 0; i < _countof(g_SysCursors); i++)
    {
        ...
        g_SysCursors[i].cursor = LoadImageW(NULL, g_SysCursors[i].resource_name, IMAGE_CURSOR, 0, 0, LR_SHARED | LR_DEFAULTSIZE);
    ...
    if (result == ERROR_SUCCESS)
    {
        for (i = 0; i < _countof(g_SysCursors); i++)
        {
            ...
            if (result == ERROR_SUCCESS && type == REG_EXPAND_SZ)
            {
                ...
                g_SysCursors[i].cursor = LoadImageW(NULL, szExpanded, IMAGE_CURSOR, 0, 0, LR_LOADFROMFILE | LR_DEFAULTSIZE);
