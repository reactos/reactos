...
IntCalcWallpaperCoordinates(
    _Out_ PSIZE lpszSrc,
    _Out_ PSIZE lpszDst,
    _Out_ PPOINT lpptSrc,
    _Out_ PPOINT lpptDst)
{
    int x, y;
    POINT ptSrc, ptDst;
    SIZE sz, szSrc, szDst;
...
HBITMAP
IntStretchWallpaper(HBITMAP hBitmap)
{
    if (hBitmap)
    {
        HDC hWallpaperDC;
        ...
        IntCalcWallpaperCoordinates(&szSrc, &szDst, &ptSrc, &ptDst);

        hNewBitmap = NtGdiCreateCompatibleBitmap(ScreenDeviceContext, sz.cx, sz.cy);
        hWallpaperDC = NtGdiCreateCompatibleDC(ScreenDeviceContext);
        if (hWallpaperDC && hNewBitmap)
        {
            ...
            /*
             * Stretch bitmap to the required coordinates
    ...
    else if (gspv.hbmWallpaper)
    {
        GreSetBitmapOwner(gspv.hbmWallpaper, GDI_OBJ_HMGR_POWNED);
        NtGdiDeleteObjectApp(gspv.hbmWallpaper);
        gspv.hbmWallpaper = NULL;
    }
    return NULL;
}

static BOOL
IntPaintWallpaper(HDC hDC)
{
