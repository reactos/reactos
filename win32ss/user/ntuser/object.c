...
// Grow the handle table by 1 PAGE by commiting memory in the shared section
static
BOOL
IntUserGrowHandleTable(VOID)
{
    NTSTATUS status;
    SIZE_T AllocatedSize, CommitSize;
    PVOID CommitAddress;

    if (gHandleTable->allocated_handles >= MAX_USER_HANDLES)
    {
        ERR("Out of user handles! Used -> %lu, NM_Handle -> %d\n", gpsi->cHandleEntries, gHandleTable->nb_handles);
        return FALSE;
    }

    /* Calculate how much is already committed */
    AllocatedSize = gHandleTable->allocated_handles * sizeof(USER_HANDLE_ENTRY);
    AllocatedSize = ALIGN_UP_BY(AllocatedSize, PAGE_SIZE);

    /* Commit one more page */
    CommitAddress = (PVOID)((ULONG_PTR)gHandleEntries + AllocatedSize);
    CommitSize = PAGE_SIZE;
    status = IntUserHeapCommitRoutine(GlobalUserHeap, &CommitAddress, &CommitSize);
    if (!NT_SUCCESS(status))
    {
        ERR("Failed to commit memory for handle table: %lx\n", status);
        return FALSE;
    }

    /* Update the number of allocated handle entries */
    AllocatedSize += CommitSize;
    gHandleTable->allocated_handles = AllocatedSize / sizeof(USER_HANDLE_ENTRY);
...
VOID UserInitHandleTable(PUSER_HANDLE_TABLE ht, PVOID mem)
{
   ht->freelist = NULL;
   ht->handles = mem;

   ht->nb_handles = 0;
   ht->allocated_handles = 0;
}
...
BOOL FASTCALL UserCreateHandleTable(VOID)
{
   ...
   UserInitHandleTable(gHandleTable, gHandleEntries);

   if (!IntUserGrowHandleTable())
   {
       ERR("Failed growing handle table\n");
       return FALSE;
   }
