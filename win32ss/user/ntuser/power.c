...
IntDeployPowerCallout(VOID)
{
     ...
     * the _THREADINFO structure) but ReactOS lacks in code implementation to
     * handle this. Suppose a thread happens to get into this fate, the power
     * callout would never get signaled...
     */
    ...
    _In_ PWIN32_POWEREVENT_PARAMETERS pWin32PwrEventParams)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        ERR("Could not deploy power callout, invalid Win32 power parameters detected!\n");
        return Status;
    }
