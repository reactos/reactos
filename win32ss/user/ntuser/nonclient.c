...
{
   if (thickframe)
       UserDrawWindowFrame(hdc, rect, UserGetSystemMetrics(SM_CXFRAME), UserGetSystemMetrics(SM_CYFRAME));
   else
...
LONG FASTCALL DefWndStartSizeMove(PWND Wnd, WPARAM wParam, POINT *capturePoint)
{
    ...
    else  // SC_SIZE
    {
        ...
        while (!hittest)
        {
            if (!co_IntGetPeekMessage(&msg, 0, 0, 0, PM_REMOVE, TRUE))
                return 0;
            if (IntCallMsgFilter(&msg, MSGF_SIZE))
                continue;
...
void FASTCALL DefWndDoSizeMove(PWND pwnd, WORD wParam)
{
    ...
    if (((Style & WS_MAXIMIZE) && syscommand != SC_MOVE) || !IntIsWindowVisible(pwnd))
        return;
    ...
    if (IntGetCapture() != UserHMGetHandle(pwnd))
        co_UserSetCapture(UserHMGetHandle(pwnd));
    ...
    for (;;)
    {
        ...
        else if (g_bWindowSnapEnabled && msg.message == WM_LBUTTONUP)
        {
            ...
            if (!IsTaskBar && hittest == HTCAPTION && thickframe && (ExStyle & WS_EX_MDICHILD) == 0)
            {
                ...
                if (doSideSnap)
                {
                    co_WinPosSetWindowPos(pwnd, NULL,
                                          snapRect.left,
                                          snapRect.top,
    ...
    //// This causes the mdi child window to jump up when it is moved.
    //if (pWndParent)
    //    IntMapWindowPoints(0, pWndParent, (POINT *)&sizingRect, 2);
...
PCURICON_OBJECT FASTCALL NC_IconForWindow(PWND pWnd)
{
    ...
    if (!hIcon && !(pWnd->ExStyle & WS_EX_DLGMODALFRAME))
    {
        ...
        if (!hIcon)
            hIcon = gpsi->hIconWindows;   // Reg size.
    }
    if (hIcon)
    {
        pIcon = (PCURICON_OBJECT)UserGetObjectNoErr(gHandleTable, hIcon, TYPE_CURSOR);
    }
    return pIcon;
}
...
void NC_DrawFrame(HDC hDC, RECT *CurrentRect, BOOL Active, DWORD Style, DWORD ExStyle)
{
    ...
    {
        LONG Width = (UserGetSystemMetrics(SM_CXFRAME) - UserGetSystemMetrics(SM_CXDLGFRAME)) *
                      UserGetSystemMetrics(SM_CXBORDER);

        LONG Height = (UserGetSystemMetrics(SM_CYFRAME) - UserGetSystemMetrics(SM_CYDLGFRAME)) *
...
void UserDrawCaptionBar(PWND pWnd, HDC hDC, INT Flags)
{
    ...
    if (!(Flags & DC_NOVISIBLE) && !IntIsWindowVisible(pWnd))
        return;
    ...
    if (Flags & DC_FRAME)
        NC_DrawFrame(hDC, &CurrentRect, (Flags & DC_ACTIVE), Style, ExStyle);
    ...
    if (!(Style & WS_MINIMIZE))
    {
        ...
        {
            PMENU menu = UserGetMenuObject(UlongToHandle(pWnd->IDMenu)); // FIXME! Use pWnd->spmenu.
            if (menu)
            {
                TempRect = CurrentRect;
...
LRESULT NC_DoNCPaint(PWND pWnd, HDC hDC, INT Flags)
{
    ...
    if (Flags & DC_FRAME)
        NC_DrawFrame(hDC, &CurrentRect, Active ? Active : (Flags & DC_ACTIVE), Style, ExStyle);
...
LRESULT NC_HandleNCActivate(PWND Wnd, WPARAM wParam, LPARAM lParam)
{
    ...
    if (lParam != -1 && (Flags = NC_DoNCActive(Wnd)) != 0)
    {
        HDC hDC;
...
void NC_DoButton(PWND pWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (wParam)
    {
        case HTCLOSE:
            ...
            // In case of error MenuState==0xFFFFFFFF.
            MenuState = IntGetMenuState(SysMenu ? UserHMGetHandle(SysMenu) : NULL,
                                        SC_CLOSE, MF_BYCOMMAND);
    ...
    for (;;)
    {
        if (!co_IntGetPeekMessage(&Msg, 0, WM_MOUSEFIRST, WM_MOUSELAST, PM_REMOVE, TRUE))
            break;
        if (IntCallMsgFilter(&Msg, MSGF_MAX))
            continue;
    ...
    switch (wParam)
    {
        case HTCAPTION:
        {
            ...
            while (1)
            {
                ...
                if (!parent || UserIsDesktopWindow(parent))
                    break;
                TopWnd = parent;
            }
...
LRESULT NC_HandleNCRButtonDown(PWND pwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (hittest)
    {
        case HTCAPTION:
        case HTSYSMENU:
            if (!IntGetSystemMenu(pwnd, FALSE))
                break;
            ...
            for (;;)
            {
                ...
                if (UserHMGetHandle(pwnd) != IntGetCapture())
                    return 0;
            }
            IntReleaseCapture();
...
LRESULT DefWndNCHitTest(HWND hWnd, POINT Point)
{
    ...
    {
        if (ExStyle & WS_EX_STATICEDGE)
            InflateRect(&WindowRect, -GetSystemMetrics(SM_CXBORDER), -GetSystemMetrics(SM_CYBORDER));
        if (!PtInRect(&WindowRect, Point))
           return HTBORDER;
    }
    ...
    if (!(Style & WS_MINIMIZE))
    {
            ...
            (WindowRect.bottom - WindowRect.top) > GetSystemMetrics(SM_CYHSCROLL))
        {
            ...
            if (PtInRect(&TempRect, Point) &&
                HASSIZEGRIP(Style, ExStyle,
                            GetWindowLongPtrW(Parent, GWL_STYLE),
                            OrigWndRect, ParentRect))
            {
                if ((ExStyle & WS_EX_LEFTSCROLLBAR) != 0)
                    return HTBOTTOMLEFT;
                else
...
DWORD FASTCALL GetNCHitEx(PWND pWnd, POINT pt)
{
    ...
    if (HAS_THICKFRAME(Style, ExStyle))
    {
        ...
        if (!RECTL_bPointInRect(&rcWindow, pt.x, pt.y))
        {
            ...
            {
                if (pt.x < rcWindow.left+UserGetSystemMetrics(SM_CXSIZE))
                    return HTTOPLEFT;
                if (pt.x >= rcWindow.right-UserGetSystemMetrics(SM_CXSIZE))
                    return HTTOPRIGHT;
                return HTTOP;
            }
            ...
            {
                if (pt.y < rcWindow.top+UserGetSystemMetrics(SM_CYSIZE))
                    return HTTOPLEFT;
                if (pt.y >= rcWindow.bottom-UserGetSystemMetrics(SM_CYSIZE))
                    return HTBOTTOMLEFT;
                return HTLEFT;
            }
