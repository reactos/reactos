...
HBITMAP MakeMonochrome(HBITMAP hBitmap)
{
    ...
    /* FIXME: Drawing monochrome bitmap is not supported yet */
    hNewBitmap = CreateColorDIB(bm.bmWidth, bm.bmHeight, RGB(0, 0, 0));
    //hNewBitmap = CreateMonoBitmap(bm.bmWidth, bm.bmHeight, FALSE);
    if (!hNewBitmap)
        return NULL;

    hdc1 = CreateCompatibleDC(NULL);
    hdc2 = CreateCompatibleDC(NULL);
    hbm1Old = SelectObject(hdc1, hBitmap);
    hbm2Old = SelectObject(hdc2, hNewBitmap);
    for (y = 0; y < bm.bmHeight; ++y)
    {
        for (x = 0; x < bm.bmWidth; ++x)
        {
            rgbColor = GetPixel(hdc1, x, y);
