    ...
    _In_reads_opt_(argc) PWSTR* argv)
{
    ...
    if (!bResult)
    {
        DPRINT1("TransactNamedPipe(%S, %d) returned FALSE\n", pServiceName, dwControl);
...
ScmControlService(
    ...
    _In_ SERVICE_STATUS_HANDLE hServiceStatus)
{
    return ScmControlServiceEx(pServiceName,
                               hControlPipe,
                               dwControl,
                               hServiceStatus,
                               0, 0, NULL);
...
Quit:
    ...
    return ScmControlServiceEx(Service->lpServiceName,
...
ScmAutoShutdownServices(VOID)
{
    ...
    while (ServiceEntry != &ServiceListHead)
    {
             ...
             CurrentService->Status.dwCurrentState == SERVICE_START_PENDING))
        {
            ...
            ScmControlService(CurrentService->lpServiceName,
                              CurrentService->lpImage->hControlPipe,
