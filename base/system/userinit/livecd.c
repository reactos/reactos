    ...
    _Out_ LIVECD_UNATTEND* pUnattend)
{
    ...
    /* If the user presses CTRL+ALT+SHIFT, disable unattended setup */
    if ((GetKeyState(VK_CONTROL) & GetKeyState(VK_SHIFT) & GetKeyState(VK_MENU)) < 0)
    {
        TRACE("Unattended setup is disabled by keypress\n");
        return;
    }
    ...
    PSTATE pState)
{
    ...
    if (*UnattendInf)
    {
         ...
         * If there is none, we will NUL the path. */
