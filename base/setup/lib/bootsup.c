...
InstallMbrBootCodeToDisk(
    IN PCUNICODE_STRING SystemRootPath,
    IN PCUNICODE_STRING SourceRootPath,
...
InstallFatBootcodeToPartition(
    IN PCUNICODE_STRING SystemRootPath,
    IN PCUNICODE_STRING SourceRootPath,
    IN PCUNICODE_STRING DestinationArcPath,
    ...
    _In_ ULONG_PTR Options)
{
    ...
    typedef struct _DISK_GEOMETRY_EX_INTERNAL
    {
        ...
        // DISK_DETECTION_INFO Detection; // Unused here.
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("Cannot open %wZ for bootloader (Status 0x%lx)\n",
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("IOCTL_DISK_GET_PARTITION_INFO_EX failed (Status 0x%08lx)\n", Status);
        goto Quit;
    }

    IsSuperFloppy = (NT_SUCCESS(Status) &&
                     IsDiskSuperFloppy2(&DiskGeoEx.Partition,
    ...
    _In_ PCUNICODE_STRING DestinationArcPath)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        if (Status == STATUS_NOT_SUPPORTED)
            ...
            DPRINT1("FormatFileSystem(%s) failed: Status %lx\n", FileSystemName, Status);
        return Status;
    }
