    ...
    IN PVOID Parameter OPTIONAL)
{
    ...
    // UNREFERENCED_PARAMETER(NtOsInstall);
#if 1
    if (NtOsInstall)
    {
    ...
    _In_ PPARTLIST PartList)
{
         ...
         Entry = Entry->Flink)
    {
        ...
        PartEntry = Volume->PartEntry; // FIXME
        if (!PartEntry)
            continue; // We don't support non-basic volumes
        ASSERT(Volume == PartEntry->Volume);

        CheckVolume = (!PartEntry->New && !Volume->New &&
                       (Volume->FormatState == Formatted));

        DPRINT("Volume %S (%C%c) on Disk #%d, Partition #%d (%s), "
               "index %d - Type 0x%02x, IsPartNew = %s, IsVolNew = %s, FormatState = %lu -- Should I check it? %s\n",
               Volume->Info.DeviceName,
               !Volume->Info.DriveLetter ? L'-' : Volume->Info.DriveLetter,
