//Кнопка скрывает и возвпащает запущенные приложения на рабочем столе
...
#define TSDB_CLICK (WM_USER + 100)
  ...
  /*  VOID OnDraw(HDC hdc, LPRECT prc);*/
    VOID OnDraw(HDC hdc, LPRECT prc) // Кнопка показать скрыть окна на рабочем столе(справо внизу на таскбаре)
    {
        ...
        else
        {
            ...
            if (m_bHovering) // Draw a hot button
            {
                HBRUSH hbrHot = ::CreateSolidBrush(RGB(255, 255, 255));
            ...
            {
                HBRUSH hbrHot2 = ::CreateSolidBrush(RGB(0, 0, 0));
