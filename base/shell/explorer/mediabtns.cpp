...
CMultimediaBackend::GetMasterMixer()
{
    ...
    m_dwVolumeStep = (m_dwMasterRanges.dwMaximum - m_dwMasterRanges.dwMinimum) / VOLUME_STEPS_COUNT;
...
CMultimediaBackend::AdjustVolume(_In_ UINT direction, _In_ HANDLE hProcessHeap)
{
    ...
    /* Check if current volume is either at floor or ceiling value.
     * If either of the channels is at maximum or minimum value, we do not change volume. */
    for (DWORD i = 0; i < m_dwMasterChannels; ++i)
    {
        if ((direction == APPCOMMAND_VOLUME_DOWN && mxcdVolume[i].dwValue == m_dwMasterRanges.dwMinimum) ||
            (direction == APPCOMMAND_VOLUME_UP   && mxcdVolume[i].dwValue == m_dwMasterRanges.dwMaximum))
        {
    ...
    for (DWORD i = 0; i < m_dwMasterChannels; ++i)
    {
        ...
        else if (direction == APPCOMMAND_VOLUME_UP)
        {
            ...
            if (m_dwMasterRanges.dwMaximum - mxcdVolume[i].dwValue < m_dwVolumeStep)
    ...
    TRACE("Volume values were updated:\n"
          "   Left channel volume: %lu\n"
          "  Right channel volume: %lu\n",
          mxcdVolume[0].dwValue, mxcdVolume[1].dwValue);
