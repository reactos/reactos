...
/*
 * $Source: /cygdrive/c/RCVS/CVS/ReactOS/reactos/lib/kjs/src/main.c,v $
 * $Id$
 */
  ...
  if (do_load)
      ...
      for (; optind < argc; optind++)
	{
	  if (strcmp (argv[optind], "-f") == 0
	      || strcmp (argv[optind], "--file") == 0)
	    {
	      optind++;
	      if (optind >= argc)
		goto no_argument_for_file;

	      break;
	    }

	  if (!js_eval_file (interp, argv[optind]))
	    {
	      fprintf (stderr, "%s: loading of file `%s' failed:\n%s\n",
		       program, argv[optind], js_error_message (interp));
	      exit (1);
	    }
	}
    }
...
#if 0
  /* Use with JS_DEBUG_MEMORY_LEAKS in jsint.h. */
