...
};  /* @author: Klemens Friedl; TEXT() casts */
...
void RenderQueue_Add(DisplayModel *dm, int pageNo) {
    ...
    assert(dm);
    if (!dm) return; //goto Exit; /* @author: Klemens Friedl; because of crosses initialization of [...] */
...
static void AddFileMenuItem(HMENU menuFile, FileHistoryList *node)
{
    ...
    AppendMenu(menuFile, MF_ENABLED | MF_STRING, newId, (TCHAR*)txt); /* @author: Klemens Friedl; TCHAR* cast */
    node->menuId = newId;
}
...
static bool runningFromProgramFiles(void)
{
    ...
    BOOL fOk = SHGetSpecialFolderPath(NULL, (TCHAR*)programFilesDir, CSIDL_PROGRAM_FILES, FALSE); /* @author: Klemens Friedl; TCHAR* cast */
...
static void AppGetAppDir(DString* pDs)
{
    ...
    SHGetSpecialFolderPath(NULL, (TCHAR*)dir, CSIDL_APPDATA, TRUE); /* @author: Klemens Friedl; TCHAR* cast */
...
{
    assert(0 == pDs->length);
    AppGenDataFilename((char*)PREFS_FILE_NAME, pDs); /* @author: Klemens Friedl; char* cast */
}
...
static void Prefs_Save(void)
{
    ...
    write_to_file((TCHAR*)path.pString, (void*)prefsStr.pString, prefsStr.length); /* @author: Klemens Friedl; TCHAR* cast */
...
static WindowInfo* LoadPdf(const char *fileName, bool ignoreHistorySizePos = true, bool ignoreHistory = false)
{
    ...
    if (!win->dm) {
        ...
        //goto Exit; /* @author: Klemens Friedl; because of "crosses initialization of [...]" */
...
static HFONT Win32_Font_GetSimple(HDC hdc, char *fontName, int fontSize)
{
    ...
    strcpy_s((char*)lf.lfFaceName, LF_FACESIZE, fontName); /* @author: Klemens Friedl; char* cast */
...
void DisplayModel::pageChanged(void)
{
    ...
    if (pageCount <= 0)
        win_set_text(win->hwndFrame, (TCHAR*)baseName); /* @author: Klemens Friedl; TCHAR* cast */
    else {
        char titleBuf[256];
        HRESULT hr = StringCchPrintfA(titleBuf, dimof(titleBuf), "%s page %d of %d", baseName, currPageNo, pageCount);
        win_set_text(win->hwndFrame, (TCHAR*)titleBuf); /* @author: Klemens Friedl; TCHAR* cast */
	/* @author: Klemens Friedl; FIXME: hr is unused ATM */
    }
}
...
static bool AlreadyRegisteredForPdfExtentions(void)
{
    ...
    if (ERROR_SUCCESS != RegOpenKeyEx(HKEY_CLASSES_ROOT, TEXT(".pdf"), 0, KEY_QUERY_VALUE, &key)) /* @author: Klemens Friedl; TEXT() cast */
        return false;
...
static void AssociateExeWithPdfExtensions()
{
    ...
    /* key\.pdf */                                              /* @author: Klemens Friedl; TEXT() && TCHAR* casts */
...
static void OnDropFiles(WindowInfo *win, HDROP hDrop)
{
    ...
    {
        DragQueryFile(hDrop, i, (TCHAR*)filename, MAX_PATH); /* @author: Klemens Friedl; TCHAR* cast */
        LoadPdf(filename);
    }
    DragFinish(hDrop);
...
{    
    ...
    DrawText(hdc, (TCHAR*)txt, strlen(txt), r, DT_CENTER | DT_VCENTER | DT_SINGLELINE); /* @author: Klemens Friedl; TCHAR* cast */
}
...
static void PaintTransparentRectangle(WindowInfo *win, HDC hdc, RectI *rect) {
    ...
    /* @author: Klemens Friedl; error: 'AlphaBlend' was not declared in this scope; even with WINVER: 0x0500 set in rbuild file, weird; @FIXME */
...
static void DrawAbout(HWND hwnd, HDC hdc, PAINTSTRUCT *ps)
{
    ...
    GetTextExtentPoint32(hdc, (TCHAR*)txt, strlen(txt), &txtSize); /* @author: Klemens Friedl; TCHAR* cast */
    ...
    txt = SUMATRA_TXT;
    TextOut(hdc, x, y, (TCHAR*)txt, strlen(txt)); /* @author: Klemens Friedl; TCHAR* cast */
...
static void DrawAnim2(WindowInfo *win, HDC hdc, PAINTSTRUCT *ps)
{
    ...
    TextOut(hdc, curTxtPosX, curTxtPosY, (TCHAR*)txt.pString, txt.length); /* @author: Klemens Friedl; TCHAR* cast */
...
static void OnPaint(WindowInfo *win)
{
    ...
    if (WS_ABOUT == win->state) {
        ...
        HFONT origFont = (HFONT)SelectObject(hdc, fontRightTxt); /* Just to remember the orig font */
        FillRect(hdc, &ps.rcPaint, gBrushBg);
	/* @author: Klemens Friedl; translation function and it's related macros need some care */
...
static void OnMenuSaveAs(WindowInfo *win)
{
    ...
    ofn.lpstrFilter = TEXT("PDF\0*.pdf\0All\0*.*\0"); /* @author: Klemens Friedl; TEXT() casts */
    ofn.nFilterIndex = 1;
    ...
    BOOL ok = CopyFileEx((TCHAR*)srcFileName, (TCHAR*)realDstFileName, NULL, NULL, &cancelled, COPY_FILE_FAIL_IF_EXISTS);  /* @author: Klemens Friedl; TCHAR* cast */
    if (!ok) {
        SeeLastError();
	 /* @author: Klemens Friedl; translation needs some care */
...
static void OnMenuOpen(WindowInfo *win)
{
    ...
    ofn.lStructSize = sizeof(ofn);
    ofn.hwndOwner = win->hwndFrame;
    ofn.lpstrFile = (TCHAR*)fileName; /* @author: Klemens Friedl; TCHAR* cast */
...
{
    AssociateExeWithPdfExtensions();
    /* @author: Klemens Friedl; translation need some care */
...
static void OnMenuAbout() {
    ...
    gHwndAbout = CreateWindow(
            ...
            NULL, NULL,
            ghinst, NULL); /* @author: Klemens Friedl; TCHAR* cast */
    if (!gHwndAbout)
        return;
...
BOOL PrivateIsAppThemed() {
    BOOL isThemed = FALSE;
    HMODULE hDll = LoadLibrary(TEXT("uxtheme.dll")); /* @author: Klemens Friedl; TEXT() cast */
    if (!hDll) return FALSE;

    FARPROC fp = GetProcAddress(hDll, TEXT("IsAppThemed")); /* @author: Klemens Friedl; TEXT() cast */
    if (fp)
        isThemed = fp();
