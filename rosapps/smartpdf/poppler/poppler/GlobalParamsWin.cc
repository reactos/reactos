...
static void GetWindowsFontDir(char *winFontDir, int cbWinFontDirLen)
{
    ...
    HMODULE hLib = LoadLibrary(L"shell32.dll"); /* @note: "L" cast */
    if (hLib) {
        ...
        if (SHGetFolderPathFunc)
            (*SHGetFolderPathFunc)(NULL, CSIDL_FONTS, NULL, SHGFP_TYPE_CURRENT, (TCHAR*)winFontDir); /* @note: TCHAR* cast */
        ...
        if (!winFontDir[0]) {
            ...
            if (SHGetSpecialFolderPathFunc)
                (*SHGetSpecialFolderPathFunc)(NULL, (TCHAR*)winFontDir, CSIDL_FONTS, FALSE); /* @note: TCHAR* cast */
        }
        FreeLibrary(hLib);
    }
    if (winFontDir[0])
    ...
    // Try older DLL
    hLib = LoadLibrary(L"SHFolder.dll"); /* @note: "L" cast */
    if (hLib) {
        SHGetFolderPathFunc = (HRESULT (__stdcall *)(HWND, int, HANDLE, DWORD, LPTSTR))
                              GetProcAddress(hLib, "SHGetFolderPathA");
        if (SHGetFolderPathFunc)
    ...
    GetWindowsDirectory((TCHAR*)winFontDir, cbWinFontDirLen); /* @note: TCHAR* cast */
    if (winFontDir[0]) {
