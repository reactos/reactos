...
/* You can disable getBuf() on File/Flate/Embed streams by commenting those out.
This might be helpful when tracking down problems (i.e. if you suspect getBuf()
handling has a bug, you can easily disable it to verify that it's gone when
getBuf() is not used anywhere */
#define ENABLE_FILE_GET_BUF 1
#define ENABLE_FLATE_GET_BUF 1
#define ENABLE_EMBED_GET_BUF 1
...
// FIXME: add more names when you need
const char *getStreamKindName(StreamKind kind)
{
    if (strFile == kind)
        return "FileStream";
    else if (strFlate == kind)
        return "FlateStream";
    else
        return "Unknown";
}
...
Stream *Stream::addFilters(Object *dict) {
  ...
  if (obj.isName()) {
    str = makeFilter(obj.getNameC(), str, &params);
  } else if (obj.isArray()) {
    for (i = 0; i < obj.arrayGetLength(); ++i) {
      ...
      if (params.isArray())
        ...
        params2.initNull();
      if (obj2.isName()) {
        str = makeFilter(obj2.getNameC(), str, &params2);
      } else {
...
Stream *Stream::makeFilter(char *name, Stream *str, Object *params) {
  ...
  if (!strcmp(name, "ASCIIHexDecode") || !strcmp(name, "AHx")) {
    ...
    early = 1;
    if (params->isDict()) {
      params->dictLookupInt("Predictor", NULL, &pred);
      params->dictLookupInt("Columns", NULL, &columns);
      params->dictLookupInt("Colors", NULL, &colors);
      params->dictLookupInt("BitsPerComponent", NULL, &bits);
      params->dictLookupInt("EarlyChange", NULL, &early);
    }
    ...
    black = gFalse;
    if (params->isDict()) {
      params->dictLookupInt("K", NULL, &encoding);
      params->dictLookupBool("EndOfLine", NULL, &endOfLine);
      params->dictLookupBool("EncodedByteAlign", NULL, &byteAlign);
      params->dictLookupInt("Columns", NULL, &columns);
      params->dictLookupInt("Rows", NULL, &rows);
      params->dictLookupBool("EndOfBlock", NULL, &endOfBlock);
      params->dictLookupBool("BlackIs1", NULL, &black);
    }
...
Guchar *ImageStream::getLine() {
  ...
  if (nBits == 1) {
    Guchar *tmpImgLine = imgLine;
    if (str->hasGetBuf()) {
      int leftBytes = nVals / 8;
      if (nVals % 8 > 0)
          ++leftBytes;
      char *buf;
      int bufSize;
      while (leftBytes > 0) {
        hasData = str->getBuf(&buf, &bufSize, leftBytes);
        if (!hasData)
            return imgLine;
        leftBytes -= bufSize;
        while (bufSize > 0) {
          ...
          ++buf;
          bufSize--;
          *tmpImgLine++ = (Guchar)((c >> 7) & 1);
          *tmpImgLine++ = (Guchar)((c >> 6) & 1);
          *tmpImgLine++ = (Guchar)((c >> 5) & 1);
          *tmpImgLine++ = (Guchar)((c >> 4) & 1);
          *tmpImgLine++ = (Guchar)((c >> 3) & 1);
          *tmpImgLine++ = (Guchar)((c >> 2) & 1);
          *tmpImgLine++ = (Guchar)((c >> 1) & 1);
          *tmpImgLine++ = (Guchar)(c & 1);
        }
      }
    } else {
      Guchar *tmpImgLine = imgLine;
    ...
    if (str->hasGetBuf()) {
      ...
      Guchar *tmpImgLine = imgLine;
      while (left > 0) {
        hasData = str->getBuf(&buf, &bufSize, left);
        if (!hasData)
            return imgLine;
        left -= bufSize;
        while (bufSize > 0) {
...
GBool FileStream::hasGetBuf() { 
#ifdef ENABLE_FILE_GET_BUF
  return gTrue; 
#else
  return gFalse;
#endif
...
GBool FileStream::getBuf(char **bufOut, int *bufSizeOut, int maxSize)
{
  int size;
  if (bufPtr >= bufEnd) {
    if (!fillBuf()) {
  ...
  assert(bufEnd > bufPtr);
  size = bufEnd - bufPtr;
  if (Stream::NO_SIZE_LIMIT != maxSize) {
    assert(maxSize > 0);
    if (size > maxSize)
      size = maxSize;
  }
...
void FileStream::ungetBuf(int sizeToGoBack) {
  bufPtr -= sizeToGoBack;
  assert(bufPtr >= buf);
}
...
GBool EmbedStream::hasGetBuf() {
#ifdef ENABLE_EMBED_GET_BUF
  return str->hasGetBuf(); 
#else
  return gFalse;
#endif
}

void EmbedStream::ungetBuf(int sizeToGoBack) {
  str->ungetBuf(sizeToGoBack);
  if (limited) {
    length += sizeToGoBack;
  }
}

GBool EmbedStream::getBuf(char **bufOut, int *bufSizeOut, int maxSize) {
  if (limited && (0 == length)) {
    return gFalse;
  }
  ...
  if (limited) {
    if (Stream::NO_SIZE_LIMIT == maxSize) {
      ...
      if (maxSize > (int)length)
        maxSize = (int)length;
    }
  }
  GBool hasData = str->getBuf(bufOut, bufSizeOut, maxSize);
  if (!hasData)
    return gFalse;

  if (limited) {
    assert((int)length >= *bufSizeOut);
    length -= *bufSizeOut;
  }
  assert(*bufSizeOut > 0);
  return gTrue;   
...
GBool FlateStream::hasGetBuf() {
#ifdef ENABLE_FLATE_GET_BUF
  if (pred)
    return gFalse;
  return gTrue;
#else
...
void FlateStream::ungetBuf(int sizeToGoBack) {
  remain += sizeToGoBack;
  index -= sizeToGoBack;
  if (index < 0)
    index = flateWindow + index;
}

GBool FlateStream::getBuf(char **bufOut, int *bufSizeOut, int maxSize) {
  int sizeToReturn;
  assert(hasGetBuf());
  while (remain == 0) {
    if (endOfBlock && eof)
  ...
  // buf wraps, for simplicity we return only the part that doesn't wrap
  sizeToReturn = remain;
  if (index + remain > flateWindow) {
    sizeToReturn = flateWindow - index;
  }
  if (Stream::NO_SIZE_LIMIT != maxSize) {
    assert(maxSize > 0);
    if (sizeToReturn > maxSize)
      sizeToReturn = maxSize;
  }
  assert(sizeToReturn > 0);
  remain -= sizeToReturn;
  *bufOut = (char*)&(buf[index]);
  *bufSizeOut = sizeToReturn;
  index = (index + sizeToReturn) & flateMask;
  return gTrue;
}
