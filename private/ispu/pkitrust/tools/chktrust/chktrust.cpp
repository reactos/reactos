...
extern "C" int __cdecl wmain(int argc, WCHAR **wargv)
{
    ...
    if (!(pArgs->Fill(argc, wargv)) || (pArgs->GetValue(IDS_PARAM_HELP)))
    {
    ...
    if (((pwszCatalogFile) && !(pwszCatalogMember)) || (!(pwszCatalogFile) && (pwszCatalogMember)))
    {
        wprintf(L"%s", pArgs->GetUsageString());
        goto ParamError;
    }
...
HRESULT _CallWVT(WCHAR *pwszFilename)
{
    ...
    sWTFI.hFile             = CreateFileU(pwszFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
HRESULT _ExplodeCatalog(WCHAR *pwszCatalogFile)
{
    ...
    {
        hrReturn |= _CallCatalogWVT(pwszCatalogFile, psMember->pwszReferenceTag, psMember->pwszReferenceTag);
    }
...
HRESULT _CallCatalogWVT(WCHAR *pwszCatalogFile, WCHAR *pwszMemberTag, WCHAR *pwszMemberFile)
{
    ...
    sWTCI.hMemberFile           = CreateFileU(pwszMemberFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
BOOL OpenSIP(const WCHAR* pwsFileName, SIP_SUBJECTINFO** ppSubjectInfo, SIP_DISPATCH_INFO** ppDispatchInfo, GUID* pgSubject)
{
    ...
    *ppDispatchInfo = (SIP_DISPATCH_INFO*) new(BYTE[sizeof(SIP_DISPATCH_INFO)]);
    ...
    if ((pgSubject == NULL)        || (*ppSubjectInfo == NULL)    || (*ppDispatchInfo == NULL))
    {
        return FALSE;
    }
    ...
    // Get the type of SIP
    if (!CryptSIPRetrieveSubjectGuid(pwsFileName, NULL, pgSubject))
    {
        goto ErrorReturn;
    }
    ...
    // Load the SIP
    if (!CryptSIPLoad(pgSubject, 0, *ppDispatchInfo))
    {
        goto ErrorReturn;
    }
...
BOOL ReadMsgBlob(const WCHAR* pwsFileName, CRYPT_DATA_BLOB* pData)
{
    if ((pwsFileName == NULL)    || (pData == NULL))
    {
        return FALSE;
    }
    ...
    // Get the SIP
    if (!OpenSIP(pwsFileName, &pSubjectInfo, &pDispatchInfo, &gSubject))
    {
        return FALSE;
    }
    ...
    if (!pDispatchInfo->pfGet(pSubjectInfo, &dwEncodingType, 0, &(pData->cbData), NULL))
    {
        delete[](pSubjectInfo);
    ...
    if (!pDispatchInfo->pfGet(pSubjectInfo, &dwEncodingType, 0, &(pData->cbData), pData->pbData))
    {
        delete[](pData->pbData);
        delete[](pSubjectInfo);
        delete[](pDispatchInfo);
...
BOOL CheckForTimeStamp(WCHAR *pwszFile)
{
    BOOL        fRet = TRUE;
    ...
    if (!(hMsg = CryptMsgOpenToDecode(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING , 0, 0, 0, NULL, NULL)))
    {
        goto ErrorReturn;
    }
    ...
    if (!CryptMsgGetParam(hMsg, CMSG_SIGNER_UNAUTH_ATTR_PARAM, 0, (void *) pMsgAttr, &cbMsgAttr))
    {
        goto ErrorReturn;
    }
