...
BOOL _SetCertErrorAndHygiene(PCCERT_CONTEXT pSubjectContext, PCCERT_CONTEXT pIssuerContext, DWORD dwCurrentConfidence, DWORD *pdwError);
PCCERT_CONTEXT _GetExternalIssuerCert(PCCERT_CONTEXT pContext, DWORD dwEncoding, DWORD *pdwRetError, DWORD *pdwConfidence, FILETIME *psftVerifyAsOf);
void _SetConfidenceOnIssuer(DWORD dwEncoding, PCCERT_CONTEXT pChildCert, PCCERT_CONTEXT pTestIssuerCert,
                            DWORD dwVerificationFlag, FILETIME *psftVerifyAsOf, DWORD *pdwConfidence, DWORD *pdwError);
...
PCCERT_CONTEXT WINAPI TrustFindIssuerCertificate(PCCERT_CONTEXT pChildContext,
                                                 ...
                                                 DWORD dwFlags)
{
    if (!(pChildContext) || !(pahStores) || !(psftVerifyAsOf) || (dwFlags != 0))
    {
    ...
    for (int i = 0; i < (int)chStores; i++)
    {
        ...
        while (pCertContext = CertGetIssuerCertificateFromStore(pahStores[i], pChildContext, pCertContext, &fdwWork))
        {
            _SetConfidenceOnIssuer(dwEncoding, pChildContext, pCertContext, fdwWork, psftVerifyAsOf, &dwConfidence, &dwError);
    ...
    {
        if (pCertContext = _GetExternalIssuerCert(pChildContext, dwEncoding, &fdwRetError, &dwConfidence, psftVerifyAsOf))
        {
...
BOOL WINAPI TrustOpenStores(HCRYPTPROV hProv, OUT DWORD *pchStores, HCERTSTORE *pahStores, DWORD dwFlags)
{
    BOOL        fRet;
    DWORD       cs;
...
BOOL WINAPI TrustIsCertificateSelfSigned(PCCERT_CONTEXT pContext, DWORD dwEncoding, DWORD dwFlags)
{
    ...
    if (!(CertVerifySubjectCertificateContext(pContext, pContext, &dwFlag)) || (dwFlag & CERT_STORE_SIGNATURE_FLAG))
    {
        return(FALSE);
    }
...
PCCERT_CONTEXT _GetExternalIssuerCert(PCCERT_CONTEXT pContext, DWORD dwEncoding, DWORD *pdwRetError, DWORD *pdwConfidence, FILETIME *psftVerifyAsOf)
{
    *pdwConfidence  = 0;
    ...
    if (NULL == (pfnCryptGetObjectUrl = (PFN_CRYPT_GET_OBJECT_URL) GetProcAddress(hDll, sz_CryptGetObjectUrl)))
        goto CryptGetObjectUrlProcAddressError;

    if (NULL == (pfnCryptRetrieveObjectByUrlW = (PFN_CRYPT_RETRIEVE_OBJECT_BY_URLW) GetProcAddress(hDll, sz_CryptRetrieveObjectByUrlW)))
        goto CryptRetrieveObjectByUrlWProcAddressError;

    if (!(pfnCryptGetObjectUrl(URL_OID_CERTIFICATE_ISSUER, (void *)pContext, 0, NULL, &cbUrlArray, NULL, NULL, NULL)) || (cbUrlArray < 1))
    {
    ...
    for (i = 0; i < pUrlArray->cUrl; i++)
    {
        ...
        if (pfnCryptRetrieveObjectByUrlW(pUrlArray->rgwszUrl[i], CONTEXT_OID_CERTIFICATE, 0, 0, (void **)&pIssuer, NULL, NULL, NULL, NULL))
        {
            if (!(CertCompareCertificateName(X509_ASN_ENCODING, &pContext->pCertInfo->Issuer, &pIssuer->pCertInfo->Subject)))
            {
                continue;
            }
            ...
            dwError = 0;
            _SetConfidenceOnIssuer(dwEncoding, pContext, pIssuer, dwStatus, psftVerifyAsOf, &dwConfidence, &dwError);
...
BOOL WINAPI TrustDecode(DWORD dwModuleId, BYTE **ppbRet, DWORD *pcbRet, DWORD cbHint,
                        DWORD dwEncoding, const char *pcszOID, const BYTE *pbEncoded, DWORD cbEncoded, DWORD dwDecodeFlags)
{
    ...
    *pcbRet = cbHint;
    ...
    if (!(CryptDecodeObject(dwEncoding, pcszOID, pbEncoded, cbEncoded, dwDecodeFlags, *ppbRet, pcbRet)))
    {
...
BOOL _CompareAuthKeyId2(DWORD dwEncoding, PCCERT_CONTEXT pChildContext, PCCERT_CONTEXT pParentContext)
{
    ...
    if (!(pExt = CertFindExtension(szOID_AUTHORITY_KEY_IDENTIFIER2, pChildContext->pCertInfo->cExtension, pChildContext->pCertInfo->rgExtension)))
    {
        goto NoExtensions;
    }
    ...
    {
        if (pIdInfo->AuthorityCertIssuer.rgAltEntry[i].dwAltNameChoice == CERT_ALT_NAME_DIRECTORY_NAME)
        {
            break;
        }
    }
    ...
    if (!(CertCompareCertificateName(dwEncoding, &pIdInfo->AuthorityCertIssuer.rgAltEntry[i].DirectoryName, &pParentContext->pCertInfo->Issuer)))
    {
    ...
    if (!(CertCompareIntegerBlob(&pIdInfo->AuthorityCertSerialNumber, &pParentContext->pCertInfo->SerialNumber)))
    {
        goto IncorrectIssuer;
    }
...
BOOL _CompareAuthKeyId(DWORD dwEncoding, PCCERT_CONTEXT pChildContext, PCCERT_CONTEXT pParentContext)
{
    ...
    if (!(pExt = CertFindExtension(szOID_AUTHORITY_KEY_IDENTIFIER, pChildContext->pCertInfo->cExtension, pChildContext->pCertInfo->rgExtension)))
    {
        goto NoExtensions;
    }

    if (!(TrustDecode(WVT_MODID_WINTRUST, (BYTE **)&pChildKeyIdInfo, &cbKeyIdInfo, 104,
                      dwEncoding, X509_AUTHORITY_KEY_ID, pExt->Value.pbData, pExt->Value.cbData, CRYPT_DECODE_NOCOPY_FLAG)))
    {
        goto DecodeFailed;
    }

    if ((pChildKeyIdInfo->CertIssuer.cbData < 1) || (pChildKeyIdInfo->CertSerialNumber.cbData < 1))
    {
        goto NoKeyId;
    }

    //  issuer certificate's issuer name must match
    if (!(CertCompareCertificateName(dwEncoding, &pChildKeyIdInfo->CertIssuer, &pParentContext->pCertInfo->Issuer)))
    {
        goto IncorrectIssuer;
    }

    //  issuer certificate's serial number must match
    if (!(CertCompareIntegerBlob(&pChildKeyIdInfo->CertSerialNumber, &pParentContext->pCertInfo->SerialNumber)))
    {
        goto IncorrectIssuer;
    }
