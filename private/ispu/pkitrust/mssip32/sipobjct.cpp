...
BOOL SIPObjectCatalog_::GetSignedDataMsg(SIP_SUBJECTINFO *pSI,DWORD dwIdx, DWORD *pdwDLen,BYTE *pbData, DWORD *pdwEncodeType)
{
    if ((dwIdx != 0) || (!(pdwEncodeType)))
    {
    ...
    {
        if (!(pSI->psBlob) || !(_ISINSTRUCT(MS_ADDINFO_BLOB, pSI->psBlob->cbStruct, pbMemSignedMsg)))
        {
            SetLastError((DWORD)ERROR_INVALID_PARAMETER);
            return(FALSE);
        }
...
BOOL SIPObjectCatalog_::PutSignedDataMsg(SIP_SUBJECTINFO *pSI,DWORD *dwIdx, DWORD dwDLen,BYTE *pbData, DWORD dwEncodeType)
{
    if (!(pbData))
    {
        SetLastError((DWORD)ERROR_INVALID_PARAMETER);
    ...
    if (this->FileHandleFromSubject(pSI, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ))
    {
        ...
        DWORD   cbWritten;
        cbWritten = 0;
        if (!(WriteFile(this->hFile, pbData, dwDLen, &cbWritten, NULL)) || (cbWritten != dwDLen))
        {
            return(FALSE);
        }
...
BOOL SIPObjectCatalog_::CreateIndirectData(SIP_SUBJECTINFO *pSI,DWORD *pdwDLen, SIP_INDIRECT_DATA *psData)
{
    if (pdwDLen)
    {
        *pdwDLen = 0;
