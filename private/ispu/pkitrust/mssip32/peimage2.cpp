...
BOOL CalculateImagePtrs(PLOADED_IMAGE LoadedImage)
{
    ...
    __try {
        ...
        if ((DosHeader->e_magic != IMAGE_DOS_SIGNATURE) && (DosHeader->e_magic != IMAGE_NT_SIGNATURE)) {
            __leave;
        }
...
BOOL MapIt(HANDLE hFile, PLOADED_IMAGE LoadedImage)
{
    HANDLE hMappedFile;
...
BOOL imagehack_IsImagePEOnly(IN HANDLE           FileHandle)
/*
