...
class SIPObjectPE_ : public SIPObject_
{
    public:
        ...
        BOOL            CreateIndirectData(SIP_SUBJECTINFO *pSI,DWORD *pdwDLen, SIP_INDIRECT_DATA *psData);
        BOOL            VerifyIndirectData(SIP_SUBJECTINFO *pSI, SIP_INDIRECT_DATA *psData);
    ...
    protected:
        ...
        virtual BOOL    PutMessageInFile(SIP_SUBJECTINFO *pSI, WIN_CERTIFICATE *pWinCert,DWORD *pdwIndex);

        virtual BOOL    GetMessageFromFile(SIP_SUBJECTINFO *pSI, WIN_CERTIFICATE *pWinCert,DWORD dwIndex, DWORD *pcbCert)
                            { return(ImageGetCertificateData(this->hFile,dwIndex,pWinCert,pcbCert)); }

        virtual BOOL    GetDigestStream(DIGEST_DATA *pDigestData, DIGEST_FUNCTION pfnCallBack, DWORD dwFlags);
