...
BOOL SIPObjectSS_::GetMessageFromFile(SIP_SUBJECTINFO *pSI, WIN_CERTIFICATE *pWinCert, DWORD dwIndex,DWORD *pcbCert)
{
    if (!(pTopStg))
    {
        return(FALSE);
    ...
    if ((pTopStg->OpenStream(Ids[dwIndex].pwszName, NULL, STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pStream) != S_OK) || !(pStream))
    {
    ...
    if (pWinCert)
    {
        ...
        cbRead = 0;
        cbCert -= WVT_OFFSETOF(WIN_CERTIFICATE, bCertificate);
        if ((pStream->Read(pWinCert->bCertificate, cbCert, &cbRead) != S_OK) || (cbRead != cbCert))
        {
            SetLastError(ERROR_BAD_FORMAT);
            pStream->Release();
            return(FALSE);
...
BOOL SIPObjectSS_::PutMessageInFile(SIP_SUBJECTINFO *pSI, WIN_CERTIFICATE *pWinCert,DWORD *pdwIndex)
{
    if ((pWinCert->dwLength <= OFFSETOF(WIN_CERTIFICATE,bCertificate))  || (pWinCert->wCertificateType != WIN_CERT_TYPE_PKCS_SIGNED_DATA))
    {
    ...
    if ((pTopStg->CreateStream(Ids[*pdwIndex].pwszName, STGM_CREATE | STGM_WRITE | STGM_SHARE_EXCLUSIVE, 0, 0, &pStream) != S_OK) || !(pStream))
    {
        SetLastError(ERROR_BAD_FORMAT);
        return(FALSE);
    }

    if (pStream->Write(pWinCert->bCertificate, pWinCert->dwLength - WVT_OFFSETOF(WIN_CERTIFICATE, bCertificate), NULL) != S_OK)
    {
        pStream->Release();
        SetLastError(ERROR_BAD_FORMAT);
        return(FALSE);
...
BOOL SIPObjectSS_::GetDigestStream(DIGEST_DATA *pDigestData, DIGEST_FUNCTION pfnCallBack, DWORD dwFlags)
{
    ...
    for (int i = 0; i < (int)cSortStg; i++)
    {
        switch (pSortStg[i].type)
        {
            case STGTY_STORAGE:
                ...
                if ((pStg->OpenStorage(pSortStg[i].pwcsName, NULL, STGM_READ | STGM_SHARE_EXCLUSIVE, 0, 0, &pInnerStg) != S_OK) || !(pInnerStg))
                {
            ...
            case STGTY_STREAM:
                ...
                if ((pStg->OpenStream(pSortStg[i].pwcsName, NULL, STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pInnerStm) != S_OK) || !(pInnerStm))
                {
                    this->FreeElements(&cSortStg, &pSortStg);
                    SetLastError(ERROR_BAD_FORMAT);
                    return(FALSE);
...
BOOL SIPObjectSS_::FileHandleFromSubject(SIP_SUBJECTINFO *pSubject, DWORD dwAccess, DWORD dwShared)
{
  ...
  /*  if ((hr = StgOpenStorage((const WCHAR *)pSubject->pwsFileName, NULL,
                        (dwAccess & GENERIC_WRITE) ? (STGM_READWRITE | STGM_SHARE_EXCLUSIVE | STGM_DIRECT) : (STGM_READ | STGM_SHARE_DENY_NONE | STGM_TRANSACTED),
                        NULL,
                        0,
                        &pTopStg)) != S_OK)
    {
    ...
    if ((hr = StgOpenStorage((const WCHAR *)pSubject->pwsFileName, NULL,
                        (dwAccess & GENERIC_WRITE) ? (STGM_READWRITE | STGM_SHARE_EXCLUSIVE | STGM_DIRECT) : (STGM_READ | STGM_SHARE_DENY_WRITE | STGM_DIRECT),
                        NULL,
                        0,
                        &pTopStg)) != S_OK)
    {
...
BOOL SIPObjectSS_::SortElements(IStorage *pStg, DWORD *pcSortStg, STATSTG **ppSortStg)
{
    ...
    *ppSortStg  = NULL;
    ...
    if (*pcSortStg > 0)
    {
        ...
        if ((pEnum->Next(*pcSortStg, *ppSortStg, &celtFetched) != S_OK) || (celtFetched != *pcSortStg))
        {
