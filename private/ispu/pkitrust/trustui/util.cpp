...
void TUIGoLink(HWND hwndParent, WCHAR *pszWhere)
{
    ...
    if (!(hURLMon))
    {
        ...
        cb = WideCharToMultiByte(0, 0, pszWhere, -1, NULL, 0, NULL, NULL);
            if (NULL == (psz = new char[cb]))
            {
                return;
            }

            WideCharToMultiByte(0, 0, pszWhere, -1, psz, cb, NULL, NULL);
    ...
    else
    {
        ...
        if (SUCCEEDED(CoInitialize(NULL)))       // Init OLE if no one else has
        {
            ...
            typedef void (WINAPI *pfnHlinkSimpleNavigateToString)(LPCWSTR, LPCWSTR, LPCWSTR, IUnknown *, IBindCtx *, IBindStatusCallback *, DWORD, DWORD);
