...
{
    if (!(pProvData->padwTrustStepErrors) || (pProvData->padwTrustStepErrors[TRUSTERROR_STEP_FINAL_WVTINIT] != ERROR_SUCCESS))
    {
        return(S_FALSE);
    }
    ...
    if (!(_ISINSTRUCT(CRYPT_PROVIDER_DATA, pProvData->cbStruct, fRecallWithState)) || (!(pProvData->fRecallWithState)))
    {
        ...
        if (!_ISINSTRUCT(CRYPT_PROVIDER_DATA, pProvData->cbStruct, dwProvFlags) || pProvData->dwProvFlags & WTD_USE_IE4_TRUST_FLAG)
        {
            //  open "known" stores
