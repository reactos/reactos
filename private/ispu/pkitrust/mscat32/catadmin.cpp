...
//  default system guid for apps that just make calls to CryptCATAdminAddCatalog with hCatAdmin == NULL...
...
HCATINFO WINAPI CryptCATAdminEnumCatalogFromHash(HCATADMIN hCatAdmin, BYTE *pbHash, DWORD cbHash, DWORD dwFlags, HCATINFO *phPrev)
{
    ...
    while (dwIdx < psCatAdmin->cHashDB)
    {
        ...
        if (!(psCatAdmin->fUseDefSubSysId))//  if we are not using the default ID, we only want to check "our" subsystem.
        {
            break;
        }
