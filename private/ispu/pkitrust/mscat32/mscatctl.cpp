...
BOOL CatalogLoadFileData(CRYPTCATSTORE *pCat)
{
    ...
    pCTLContext = (PCCTL_CONTEXT) CertCreateContext(
        ...
        pbData,
        cbData,
        CERT_CREATE_CONTEXT_NOCOPY_FLAG | CERT_CREATE_CONTEXT_NO_HCRYPTMSG_FLAG,
        NULL                                        // pCreatePara
        );
    if (pCTLContext)
    {
...
BOOL CatalogSaveP7UData(CRYPTCATSTORE *pCat)
{
    ...
    if (CatalogFillCTL(pCat, &sCTLInfo))
    {
        ...
        if (cbEncoded > 0)
        {
            ...
            fRet = CryptMsgEncodeAndSignCTL(    pCat->dwEncodingType, &sCTLInfo, &sSignInfo, 0, pbEncoded, &cbEncoded);
            CatalogFreeCTL(&sCTLInfo);
            if (fRet)
            {
                HANDLE  hFile;
                                        ...
                                        NULL)) != INVALID_HANDLE_VALUE)
                {
                    ...
                    if (!(WriteFile(hFile, pbEncoded, cbEncoded, &cbWritten, NULL)) || (cbEncoded != cbWritten))
                    {
                        fRet = FALSE;
                    }
    ...
    if (pCTL->cCTLEntry > 0)
    {
        ...
        for (int iEntry = 0; iEntry < (int)pCTL->cCTLEntry; iEntry++)
        {
            ...
            if (pCTL->rgCTLEntry[iEntry].cAttribute > 0)
            {
                ...
                {
                    if (!(CatalogFillMemAttr(pCat, pMember, &pCTL->rgCTLEntry[iEntry].rgAttribute[iAttr])))
                    {
                        return(FALSE);
                    }
                }
...
BOOL CatalogFillCTL(CRYPTCATSTORE *pCat, CTL_INFO *pCTL)
{
    ...
    if (pCat->hReserved)
    {
        ...
        if (pCTL->cCTLEntry > 0)
        {
            if (!(pCTL->rgCTLEntry = (PCTL_ENTRY)CatalogNew(sizeof(CTL_ENTRY) * pStackMember->Count())))
            {
                return(FALSE);
            }
        ...
        while (dwMember < pStackMember->Count())
        {
            ...
            if (!(pCTLEntry->SubjectIdentifier.pbData = (BYTE *)CatalogNew(dwSize)))
            {
                return(FALSE);
            }

            memcpy(pCTLEntry->SubjectIdentifier.pbData, pMember->pwszReferenceTag,dwSize);
            ...
            if (!(pCTLEntry->rgAttribute = (PCRYPT_ATTRIBUTE)CatalogNew(sizeof(CRYPT_ATTRIBUTE) * pCTLEntry->cAttribute)))
            {
                return(FALSE);
            }
            ...
            if ((pMember->gSubjectType.Data1 == 0) && (pMember->gSubjectType.Data2 == 0) && (pMember->gSubjectType.Data3 == 0))
            {
    ...
    if (pCat->hAttrs)
    {
        ...
        if (!(pCTL->rgExtension = (CERT_EXTENSION *)CatalogNew(sizeof(CERT_EXTENSION) * pCTL->cExtension)))
        {
            return(FALSE);
        }
...
BOOL WINAPI IsCatalogFile(HANDLE hFile, WCHAR *pwszCatalogFile)
{
    ...
    if ((hFile == NULL) || (hFile == INVALID_HANDLE_VALUE))
    {
        ...
        if ((hFile = CreateFileU(pwszCatalogFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)) == INVALID_HANDLE_VALUE)
        {
            goto IsCatFileError;
        }
    ...
    if (((cbFile = GetFileSize(hFile, NULL)) == 0xffffffff) || (cbFile < 1))
    {
        goto FileSizeError;
    }
    ...
    pCTLContext = (PCCTL_CONTEXT) CertCreateContext(
        ...
        pbFile,
        cbFile,
        CERT_CREATE_CONTEXT_NOCOPY_FLAG | CERT_CREATE_CONTEXT_NO_HCRYPTMSG_FLAG | CERT_CREATE_CONTEXT_NO_ENTRY_FLAG,
        NULL                                        // pCreatePara
        );
    if (pCTLContext)
    {
        ...
        {
            if (strcmp(pCTLContext->pCtlInfo->SubjectUsage.rgpszUsageIdentifier[0], pszCatalogListUsageOID) == 0)
            {
                fRet = TRUE;
                goto CommonReturn;
           }
