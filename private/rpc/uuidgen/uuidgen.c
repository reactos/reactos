...
This command-line program simply uses the procedure I_UuidStringGenerate to output one or more UUIDs in one of the formats normal, IDL, or C struct.
...
// The order in which these includes are performed seems, after extensive testing and analysis, to be highly crucial to an NT build.
...
main(int argc, char **argv)
{
    ...
    {
        Result = I_UuidStringGenerate(Flag, MyUuidString, "INTERFACENAME");
        ...
        case RPC_S_OK:
            fprintf(OutputFile, MyUuidString);
        } // end switch
    } // end for
