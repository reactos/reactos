...
Abstract:
This module contains I_UuidStringGenerate, which coalesces the procedures UuidCreate and UuidToString to create a UUID in one of the formats IDL, C struct, or plain.
...
// The order in which these includes are performed seems, after extensive testing and analysis, to be highly crucial to a successful NT build.
#include <sysinc.h>
#include <rpc.h>
#include <uuidfmt.h>
#include <stdio.h>
    ...
    )
{
    ...
    ASSERT(   (Flag == UUIDGEN_FORMAT_IDL) || (Flag == UUIDGEN_FORMAT_CSTRUCT) || (Flag == UUIDGEN_FORMAT_PLAIN));
