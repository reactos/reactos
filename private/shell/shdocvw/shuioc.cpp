...
STDMETHODIMP CShellUIHelper::ShowChannel(IChannelMgrPriv *pChMgrPriv, LPWSTR pwszURL, HWND hwnd)
{
    ...
    {
#ifndef UNIX
        hres = CoCreateInstance(CLSID_InternetExplorer, NULL, CLSCTX_LOCAL_SERVER, IID_IWebBrowser2, (void**)&pWebBrowser2);
#else
        hres = CoCreateInternetExplorer( IID_IWebBrowser2, CLSCTX_LOCAL_SERVER, (LPVOID*) &pWebBrowser2 );
#endif
    }
...
STDMETHODIMP CShellUIHelper::AddChannel(BSTR bstrURL)
{
    ...
    if (hwnd)
    {
        if (!SHIsRestricted2W(hwnd, REST_NoChannelUI, NULL, 0) && !SHIsRestricted2W(hwnd, REST_NoAddingChannels, NULL, 0))
        {
            ...
            hres = JITCoCreateInstance(CLSID_ChannelMgr, NULL, CLSCTX_INPROC_SERVER, IID_IChannelMgrPriv, (void **)&pChMgrPriv, hwnd, FIEF_FLAG_FORCE_JITUI);
            if (S_OK == hres)
            {
                ASSERT(pChMgrPriv);
...
STDMETHODIMP ZoneCheck(IUnknown *punkSite, BSTR bstrReqUrl)
{
    ...
    if (SUCCEEDED(GetHTMLDoc2(punkSite, &pHtmlDoc)))
    {
        ...
        if (SUCCEEDED(pHtmlDoc->get_URL(&bstrDocUrl)))
        {
            ...
            if (SUCCEEDED(CoCreateInstance(CLSID_InternetSecurityManager, NULL, CLSCTX_INPROC_SERVER, IID_IInternetSecurityManager, (void **)&pSecMgr)))
            {
                ASSERT(pSecMgr);
...
STDMETHODIMP CShellUIHelper::GetIDsOfNames(REFIID riid, OLECHAR **rgszNames, UINT cNames, LCID lcid, DISPID * rgdispid)
{
...
// 2. set _fWaitingToFindText so that on DocumentComplete, mimic the find dialog and select/highlight the specified text
{
    HRESULT hr;
    IWebBrowser2* pwb2;
    ...
    if (IsSpecialUrl(URL))
    {
        ...
        // Security:  Don't allow javascript on one web page to automatically execute in the zone of the target frame URL.
    ...
    return SUCCEEDED(hr) ? S_OK : S_FALSE;//script runtime error protection
}
