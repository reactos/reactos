...
// As this is marshalled over to the main shell process hopefully this will take care of most of the serialization problems.
// Probably still need a way to handle the case better where a window is coming up at the same time the last one is going down...
...
BOOL WinList_Init(void)
{
    ...
    else
    {
        ...
        // All of the main processing moved to first call to WinList_GetShellWindows as the creating of OLE object across processes screwed up DDE timing.
        return TRUE;
    }
