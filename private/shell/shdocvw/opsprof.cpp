...
HRESULT     CIEFrameAuto::COpsProfile::Init()
{
    ...
    return CAutomationStub::Init( SAFECAST(this, IHTMLOpsProfile*), IID_IHTMLOpsProfile, CLSID_COpsProfile, pauto );
}
...
BOOL CIEFrameAuto::COpsProfile::_IsUsingSSL()
{
    BOOL fSecure = FALSE;
...
CIEFrameAuto::COpsProfile::_LegalDomain(TCHAR *pszHostName, TCHAR *pszDomain)
{
    ...
    if (pszHostName==NULL || pszDomain==NULL || EMPTY_STRING(pszHostName) || EMPTY_STRING(pszDomain))
        return FALSE;
...
BOOL CIEFrameAuto::COpsProfile::_ApplySettings(AccessSettings *pac, URL_COMPONENTS *puc, HDPA hdpaReqList, DWORD *pdwLast)
{
    if (!_DomainMatch(puc->lpszHostName, pac->m_rgchDomain)  || !_PathMatch(puc->lpszUrlPath, pac->m_rgchPath))
        return FALSE;
