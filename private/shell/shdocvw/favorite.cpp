...
void OrgFavSynchronize(HWND hwnd, VARIANT *pvarUrlsToSynch)
{
         ...
         *(pvarUrlsToSynch->bstrVal) )
    {
        ...
        if (SUCCEEDED(JITCoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&psm, hwnd, FIEF_FLAG_FORCE_JITUI)))
        {
                    ...
                    if (lstrlen(pbffFS->patf->pszFile) < MAX_PATH)
                    {
                        ...
                        if (QualifyFileName(pbffFS->patf))
                        {
                            ...
                            else if (pbffFS->patf->bStartSubscribed)
                            {
                                ...
                                if (SUCCEEDED (CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&pSubsMgr)))
                                {
...
INT_PTR CALLBACK SubscribeFavoriteDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            ISubscriptionMgr* pSubsMgr;
            if (SUCCEEDED (JITCoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&pSubsMgr, hDlg, FIEF_FLAG_FORCE_JITUI)))
            {
                //url is in patf->pidlFavorite
                TCHAR szURL[MAX_URL_STRING];
                IEGetDisplayName(patf->pidlFavorite, szURL, SHGDN_FORPARSING);
...
BOOL IsSubscribed(ADDTOFAV *patf)
{
    ...
    if (SUCCEEDED(IEGetDisplayName(patf->pidlFavorite, szURL, SHGDN_FORPARSING)))
    {
        ...
        if (SUCCEEDED(CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&pSubscriptionMgr)))
        {
            BSTR bstrURL = SysAllocStringT(szURL);
            if (bstrURL)
            {
...
BOOL IsSubscribed(LPWSTR pwzUrl)
{
    ...
    ISubscriptionMgr * pSubscriptionMgr;
    if (FAILED(CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&pSubscriptionMgr)))
    {
        return FALSE;
    }
