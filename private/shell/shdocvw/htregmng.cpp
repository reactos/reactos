    ...
    IN DWORD  dwFlags)          // REF_ flags
{
    ...
    if (NO_ERROR == dwRet)
    {
        ...
        // Are the any subkeys?
        if (NO_ERROR == RegQueryInfoKey(hkeyNew, NULL, NULL, NULL, &ckeys, NULL, NULL, &cvalues, NULL, NULL, NULL, NULL) && 0 == ckeys)
        {
            ...
            if (!bRet && IsFlagSet(dwFlags, REF_EDITFLAGS))
            {
                ...
                bRet = ((1 == cvalues && NO_ERROR == dwRetEdit) || (2 == cvalues && NO_ERROR == dwRetEdit && NO_ERROR == dwRetDef));
            }
        }
        RegCloseKey(hkeyNew);
    }
...
HRESULT CheckWebSettings(void)
{
    ...
    retval = RegOpenKeyEx(HKEY_LOCAL_MACHINE, IE_VERIFY_REGKEY, NULL, KEY_READ, &hkey);
    ...
    retval = RegQueryValueEx(hkey, IE_VERIFY_REGVALUE, NULL, &dwType, (LPBYTE)szDataFromReg, &dwSize);
    if (retval != ERROR_SUCCESS)
    {
        RegCloseKey(hkey);
        return E_FAIL;
