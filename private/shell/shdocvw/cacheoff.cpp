...
class COfflinePagesCacheCleaner : public IEmptyVolumeCache2
{
    private:
        ...
        static HRESULT CALLBACK RunningTotalSizeCallback(LPINTERNET_CACHE_ENTRY_INFO lpCacheEntryInfo, void * pv);
        static HRESULT CALLBACK DeleteEntryCallback(LPINTERNET_CACHE_ENTRY_INFO lpCacheEntryInfo, void * pv);
        static VOID IncrementFileSize(LPINTERNET_CACHE_ENTRY_INFO lpCacheEntryInfo, DWORDLONG * pdwlSize);
...
STDMETHODIMP COfflinePagesCacheCleaner::ShowProperties(HWND hwnd)
{
    ...
    if (SHGetValue(HKEY_LOCAL_MACHINE, REGSTR_PATH_SUBSCRIPTION, REGSTR_VAL_DIRECTORY, NULL, (LPBYTE)szOfflinePath, &dwSize) != ERROR_SUCCESS)
    {
...
HRESULT COfflinePagesCacheCleaner::WalkOfflineCache(LPFCALLBACK     lpfCallBack, void *          pv)
{
    HRESULT hr = S_OK;
    ...
    if ((lpCacheEntryInfo = (LPINTERNET_CACHE_ENTRY_INFO) LocalAlloc(LPTR, MAX_CACHE_ENTRY_INFO_SIZE)) == NULL)
    {
        hr = E_OUTOFMEMORY;
    }
    else
    {
        ...
        if ((hCacheEntryInfo = FindFirstUrlCacheEntry(NULL, lpCacheEntryInfo, &dwCacheEntryInfoSize)) == NULL)
        {
        ...
        if (SUCCEEDED(hr))
        {
            do
            {
                ...
                dwCacheEntryInfoSize = MAX_CACHE_ENTRY_INFO_SIZE;
            } while ((E_ABORT != hr) && FindNextUrlCacheEntry(hCacheEntryInfo, lpCacheEntryInfo, &dwCacheEntryInfoSize));
    ...
    )
{
    ...
    {
        hr =  pdecbStruct->picb->PurgeProgress(pdecbStruct->dwlSpaceFreed, pdecbStruct->dwlTotalSpace - pdecbStruct->dwlSpaceFreed, NULL, NULL);
    }
...
VOID COfflinePagesCacheCleaner::IncrementFileSize(LPINTERNET_CACHE_ENTRY_INFO lpCacheEntryInfo, DWORDLONG                   * pdwlSize)
{
