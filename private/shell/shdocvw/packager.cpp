...
const GUID CLSID_IMimeInternational = {0xfd853cd9, 0x7f86, 0x11d0, {0x82, 0x52, 0x0, 0xc0, 0x4f, 0xd8, 0x5a, 0xb4}};
const GUID IID_IMimeInternational = {0xc5588349, 0x7f86, 0x11d0, {0x82, 0x52, 0x0, 0xc0, 0x4f, 0xd8, 0x5a, 0xb4}};
const GUID IID_IMimeBody = {0xc558834c, 0x7f86, 0x11d0, {0x82, 0x52, 0x0, 0xc0, 0x4f, 0xd8, 0x5a, 0xb4}};
...
HRESULT HrGetCombinedURL( IHTMLElementCollection *pCollBase, LONG cBase, LONG lElemPos, BSTR bstrRelURL, BSTR bstrDocURL, BSTR *pbstrBaseURL);
...
public:
    ...
    {
        ASSERT(m_bstrValue==NULL || !StrCmpIW(m_bstrValue, c_bstr_BLANK) || !StrCmpIW(m_bstrValue, bstrValue));
...
public:
    ...
    virtual HRESULT InitFromCollection(IHTMLElementCollection *pColl, ULONG *pcElems = NULL) = 0;
    virtual HRESULT PackageData(CWebArchive *pwa, BOOL *pfCancel = NULL, CThicketProgress *ptp = NULL, ULONG progLow = 0, ULONG progHigh = 100)
    {
...
protected:
    ...
    HRESULT _InitSubCollection(IHTMLElementCollection *pAll, BSTR bstrTagName, IHTMLElementCollection **ppSub, ULONG *pcElems = NULL);
    ...
    virtual HRESULT _PackageElement(CWebArchive *pwa, IHTMLElement *pElem);
...
public:
    ...
    virtual HRESULT InitFromCollection(IHTMLElementCollection *pColl, ULONG *pcElems = NULL);
protected:
...
public:
    ...
    HRESULT InitFromCollection(IHTMLElementCollection *pColl, ULONG *pcElems = NULL, IHTMLDocument2 *pDoc = NULL);
    virtual HRESULT PackageData(CWebArchive *pwa, BOOL *pfCancel = NULL, CThicketProgress *ptp = NULL, ULONG progLow = 0, ULONG progHigh = 100);
...
public:
    ...
    virtual HRESULT Init(IHTMLElementCollection *pColl, ULONG *pcElems, IHTMLDocument2 *pDoc);
...
public:
    ...
    virtual HRESULT PackageData(CWebArchive *pwa, BOOL *pfCancel, CThicketProgress *ptp = NULL, ULONG progLow = 0, ULONG progHigh = 100);
protected:
    virtual BSTR _GetTargetAttribute(void);
};
...
public:
    ...
    virtual HRESULT Init(IHTMLElementCollection *pColl, ULONG *pcElems = NULL, IHTMLDocument2 *pDoc = NULL);
protected:
    virtual BSTR _GetTargetAttribute(void);
                                  ...
                                  IMimeBody *pBody)
{
    ...
    hr = CoCreateInstance(CLSID_IMimeInternational, NULL, CLSCTX_INPROC_SERVER, IID_IMimeInternational, (LPVOID*)&pimi);
    if (SUCCEEDED(hr))
    {
...
HRESULT CRelativeURLPackager::Init(IHTMLElementCollection *pColl, ULONG *pcElems, IHTMLDocument2 *pDoc)
{
    HRESULT hr = S_OK;
...
HRESULT CScriptPackager::Init(IHTMLElementCollection *pColl, ULONG *pcElems, IHTMLDocument2 *pDoc)
{
                                            ...
                                            BOOL fFrameDoc)
{
    ...
    {
        pIOCT->Exec(NULL, OLECMDID_DONTDOWNLOADCSS, OLECMDID_DONTDOWNLOADCSS, NULL, NULL);
        pIOCT->Release();
    }
    ...
    {
        if (SHQueryValueExA(hkey, REGVALUE_DOWNLOAD_IMAGES, 0, &dwType, abBuffer, &dwSize) == NO_ERROR)
        {
...
HRESULT CDocumentPackager::_GetDesignDoc( IHTMLDocument2 *pDocSrc, IHTMLDocument2 **ppDocDesign, BOOL *pfCancel, CThicketProgress *ptp, UINT cpDst )
{
    HRESULT            hr;
    DWORD              dwFlags;
    BSTR               bstrURL = NULL;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr) && cpDst != CP_ACP)
            {
                ...
                if ( SUCCEEDED(pMultiLanguage->GetCodePageInfo(cpDst, langid, &cpInfo)) && (bstrCharSet = SysAllocString(cpInfo.wszWebCharset)) != NULL )
...
HRESULT CSSPackager::Init(IHTMLElementCollection *pColl, ULONG *pcElems, IHTMLDocument2 *pDoc)
{
...
HRESULT CSSPackager::PackageStyleSheets(IHTMLDocument2 *pDoc2, CWebArchive *pwa)
{
    HRESULT hr = S_OK;
...
HRESULT CSSPackager::_PackageSSCollection(IHTMLStyleSheetsCollection *pssc, CWebArchive *pwa)
{
    HRESULT hr;
    LONG cSS;
...
HRESULT CSSPackager::_PackageSS(IHTMLStyleSheet *pss, CWebArchive *pwa)
{
    HRESULT     hr;
