    ...
    IN CHAR *    pszBuf)
{
    ...
    *ppwsz = (LPWSTR)LocalAlloc(LPTR, SIZEOF(WCHAR) * MAX_PATH);
    if (*ppwsz)
    {
        ...
        cch = SHGetIniString(c_szIntshcut, ISHCUT_INISTRING_ICONFILE, *ppwsz, MAX_PATH, pszFile);
        if (0 != cch)
        {
            TCHAR szIndex[MAX_BUF_INT];
            // The icon index is all ASCII so don't need SHGetIniString
            cch = GetPrivateProfileString(c_szIntshcut, ISHCUT_INISTRING_ICONINDEX, c_szNULL, szIndex, SIZECHARS(szIndex), pszFile);
            if (0 != cch)
            {
    ...
    IN WORD *  pwHotkey)
{
    ...
    *pwHotkey = 0;
    ...
    cch = GetPrivateProfileString(c_szIntshcut, TEXT("Hotkey"), c_szNULL, szHotkey, SIZECHARS(szHotkey), pszFile);
    if (0 != cch)
    {
        int nVal;
    ...
    OUT PINT    pnShowCmd)
{
    ...
    *pnShowCmd = SW_NORMAL;
    ...
    cch = GetPrivateProfileString(c_szIntshcut, TEXT("ShowCommand"), c_szNULL, szT, SIZECHARS(szT), pszFile);
    if (0 != cch)
    {
