...
void WriteCleanBrowserShutdown(BOOL fClean)
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, REG_STR_IE, 0, KEY_WRITE, &hkey) == ERROR_SUCCESS)
    {
        RegSetValueEx(hkey, REG_STR_CLEAN, 0, REG_DWORD, (LPBYTE)&dwValue, sizeof(dwValue));
        RegCloseKey(hkey);
    }
}
...
BOOL ReadCleanBrowserShutdown()
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, REG_STR_IE, 0, KEY_READ, &hkey) == ERROR_SUCCESS)
    {
