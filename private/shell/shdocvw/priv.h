...
#define _WIN32_WINDOWS      0x0400// This stuff must run on Win95
...
// Win95 doesn't have full support for NTDLL would accidentally call NTDLL functions and cause us to crash on Win95.
#undef NTSYSAPI
#define NTSYSAPI
#include <ntrtl.h>
#include <nturtl.h>
...
// (Re-use TF_CUSTOM1 and TF_CUSTOM2 by defining a TF_ value in your local file to one of these values while you have the file checked out.)
...
STDAPI CMyHlinkSrc_OleCreate(CLSID rclsid, REFIID riid, DWORD renderOpt, FORMATETC* pFormatEtc, IOleClientSite* pclient, IStorage* pstg, LPVOID* ppvOut);
...
//  critical section is not taken.  FindWindow sends interthreadmessages, which is not obvious.
...
//  called it FindWindowSW.  In fact, there should be some law against giving a method the same name as a Windows API.
#ifdef DEBUG
...
// some of them will take Ansi strings on Windows 95 and will take unicode strings on NT.
#include "runonnt.h"
...
HRESULT IECreateInstance(REFCLSID rclsid, LPUNKNOWN pUnkOuter, DWORD dwClsContext, REFIID riid, LPVOID FAR* ppv);
#endif
