...
void CDocObjectHost::_DumpMenus(IN LPCTSTR pszMsg, IN BOOL    bBreak)
{
    if (IsFlagSet(g_dwDumpFlags, DF_DEBUGMENU))
    {
        ...
        TraceMsg(TF_ALWAYS, "   _hmenuBrowser = %x, _hmenuSet = %x, _hmenuFrame = %x", _hmenuBrowser, _hmenuSet, _hmenuFrame);
        TraceMsg(TF_ALWAYS, "   _hmenuCur = %x, _hmenuMergedHelp = %x, _hmenuObjHelp = %x", _hmenuCur, _hmenuMergedHelp, _hmenuObjHelp);
...
{
    if (SHRegGetBoolUSValue(c_szRegKeyCoolbar, TEXT("ShowDiscussionButton"), FALSE, TRUE) && _pmsoctBrowser) {
        OLECMD rgcmds[] = {
...
{
    TraceMsg(TF_SHDUIACTIVATE, "DOH::UIActivate called %d->%d (this=%x)", _uState, uState, this);
...
HRESULT _GetDefaultLocation(LPWSTR pszPath, DWORD cchPathSizeIn, UINT id)
{
    ...
    HOMEMSG("_GetStdLocation returning", (SUCCEEDED(hres) ? pszPath : TEXT("Error")), hres);
    return hres;
}
...
HRESULT CDocObjectHost::_RegisterMediaTypeClass(IBindCtx* pbc)
{
    ...
    if (_BuildClassMapping() && DSA_GetItemCount(g_hdsaCls)) {
        ...
        hres = RegisterMediaTypeClass(pbc, DSA_GetItemCount(g_hdsaCls), (LPCSTR*)DSA_GetItemPtr(g_hdsaStr, 0), (CLSID*)DSA_GetItemPtr(g_hdsaCls, 0), 0);
...
void CDocObjectHost::_StartPicsQuery(void)
{
    if (IS_RATINGS_ENABLED() && ::RatingEnabledQuery() == S_OK) {
        ...
        if (fEnforce) {
            ...
            _fbPicsWaitFlags = PICS_WAIT_FOR_ASYNC | PICS_WAIT_FOR_INDOC | PICS_WAIT_FOR_END | PICS_WAIT_FOR_ROOT;
...
void CDocObjectHost::_StartPicsRootQuery(LPCTSTR pszURL)
{
    if (_fbPicsWaitFlags & PICS_WAIT_FOR_ROOT) {
        ...
        if (SUCCEEDED(_GetCurrentPageW(&pwszURL, TRUE))) {
            hres = CoInternetParseUrl(pwszURL, PARSE_ROOTDOCUMENT, 0, wszRootURL, ARRAYSIZE(wszRootURL), &cchResult, 0);
            OleFree(pwszURL);
        }
        ...
        if (pszURL != NULL && (hres == INET_E_DEFAULT_ACTION || hres == E_FAIL)) {
             ...
             */
            hres = CoInternetParseUrl(pszURL, PARSE_ROOTDOCUMENT, 0, wszRootURL, ARRAYSIZE(wszRootURL), &cchResult, 0);
        }
...
BOOL IsAssociatedWithIE(LPCWSTR szPath)
{
    ...
    if (SHVerbExistsNA(szExt, "open", szBuf, SIZECHARS(szBuf))) {
        ...
        LPCSTR pszFound;
        if ((pszFound = StrStrIA(szBuf, IEXPLORE_EXE)) || (pszFound = StrStrIA(szBuf, EXPLORER_EXE))) {
...
HRESULT CDocObjectHost::_InsertMenus(/* [in] */ HMENU hmenuShared, /* [out][in] */ LPOLEMENUGROUPWIDTHS lpMenuWidths)
{
    HRESULT hres = S_OK;
    int nMenuOffset = 0;
...
STDMETHODIMP CScriptErrorList::setDetailsPaneOpen(BOOL fDetailsPaneOpen)
{
    ...
    SHRegSetUSValue(szRegKey_SMIEM, szRegVal_ErrDlgDetailsOpen, REG_SZ, pszVal, cbSize, SHREGSET_HKCU | SHREGSET_FORCE_HKCU);
...
STDMETHODIMP CScriptErrorList::setPerErrorDisplay(BOOL fPerErrorDisplay)
{
    TCHAR   szYes[] = TEXT("yes");
    TCHAR   szNo[] = TEXT("no");
    LPTSTR  pszVal;
    ...
    SHRegSetUSValue(szRegKey_SMIEM, szRegVal_ErrDlgPerErr, REG_SZ, pszVal, cbSize, SHREGSET_HKCU | SHREGSET_FORCE_HKCU);
...
HRESULT CScriptErrorList::_CScriptErrInfo::Init(LONG lLine, LONG lChar, LONG lCode, BSTR strMsg, BSTR strUrl)
{
    ASSERT(_strMsg == NULL);
    ASSERT(_strUrl == NULL);
