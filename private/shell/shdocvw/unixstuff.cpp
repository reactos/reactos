...
EXTERN_C BOOL CheckAndExecNewsScript( HWND hwnd )
{
     ...
     if (! dwUseOENews )
     {
         hres = RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_NEWSCLIENTS, 0, KEY_READ, &hkey);
         if (hres == ERROR_SUCCESS)
         {
             dwSize = sizeof( tszExpandedCommand );
             hres = RegQueryValueEx(hkey, REGSTR_PATH_CURRENT, NULL, NULL, (LPBYTE)tszCommandLine, &dwSize);
             if (hres == ERROR_SUCCESS)
             {
                  STARTUPINFO stInfo;
                  memset(&stInfo, 0, sizeof(stInfo));
                  stInfo.cb = sizeof(stInfo);
                  SHExpandEnvironmentStrings(tszCommandLine, tszExpandedCommand, INTERNET_MAX_URL_LENGTH);
                  if( CreateProcess(tszExpandedCommand, tszCommandLine, NULL, NULL, FALSE, DETACHED_PROCESS, NULL, NULL, &stInfo, NULL) )
                      fRet = TRUE;
             }
     ...
     if( !dwUseOENews && !fRet )
     {
         ShellMessageBox(MLGetHinst(), hwnd, MAKEINTRESOURCE(IDS_NEWS_SCRIPT_ERROR), MAKEINTRESOURCE(IDS_NEWS_SCRIPT_ERROR_TITLE), MB_ICONWARNING|MB_OK);
         fRet = TRUE;
     }
...
BOOL CheckForOutlookExpress(LPTSTR szDllPath)
{
    ...
    dwErr = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szDefMailKey, 0, KEY_ALL_ACCESS, &hKeyMail);
    ...
    if(!StrCmpI(szBuf, szOEName))
    {
        ...
        // Get the DLL Path
        dwErr = RegOpenKeyEx(hKeyMail, szOEName, 0, KEY_ALL_ACCESS, &hKeyOE);
        if(dwErr != ERROR_SUCCESS)
        {
            goto out;
        }
...
STDAPI UnixSendDocToMailRecipient(LPCITEMIDLIST pidl, UINT uiCodePage, DWORD grfKeyState)
{
    ...
    hres = RegCreateKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_MAILCLIENTS, 0, NULL, 0, KEY_READ, NULL, &hkey, &dw);
    if (hres == ERROR_SUCCESS)
    {
        dw = INTERNET_MAX_URL_LENGTH;
        hres = RegQueryValueEx(hkey, REGSTR_PATH_CURRENT, NULL, NULL, (LPBYTE)tszCommandLine, &dw);
        RegCloseKey(hkey);

        if (hres == ERROR_SUCCESS)
        {
            dw = SHExpandEnvironmentStrings(tszCommandLine, tszExpandedCommand, INTERNET_MAX_URL_LENGTH);
             ...
             if (!pidl)
             {
                 ...
                 bMailed = CreateProcess(tszExpandedCommand, tszCommandLine, NULL, NULL, FALSE, DETACHED_PROCESS, NULL, NULL, &stInfo, NULL);
                 return hres;
             }
         }
         else
...
HRESULT UnixRegisterBrowserInActiveSetup()
{
    ...
    hr = RegCreateKeyEx(HKEY_LOCAL_MACHINE, szKeyName, 0, NULL, 0, KEY_ALL_ACCESS, NULL, &hKey, &dw);
    if (hr != ERROR_SUCCESS)
       goto Cleanup;

    wnsprintf(szVersion,ARRAYSIZE(szVersion), TEXT("%d,%02d,%04d,%04d"), VER_MAJOR_PRODUCTVER, VER_MINOR_PRODUCTVER, VER_PRODUCTBUILD, VER_PRODUCTBUILD_QFE);
    hr = RegQueryValueEx(hKey, TEXT("Version"), NULL, &dwType, (LPBYTE)szOldVersion, &dwLen);
    if (hr == ERROR_SUCCESS)
    {
       hr = RegSetValueEx(hKey, TEXT("OldVersion"), NULL, REG_SZ, (LPBYTE)szVersion, ((1+lstrlen(szVersion))*sizeof(TCHAR)));
    }

    hr = RegSetValueEx(hKey, TEXT("Version"), NULL, REG_SZ, (LPBYTE)szVersion, ((1+lstrlen(szVersion))*sizeof(TCHAR)));
