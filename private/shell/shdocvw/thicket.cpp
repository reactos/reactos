...
INT_PTR CALLBACK SaveAsWarningDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam);
HRESULT SaveToThicket( HWND hwnd, LPCTSTR pszFileName, IHTMLDocument2 *pDoc, UINT codepageSrc, UINT codepageDst, UINT iPackageStyle );
HRESULT FormsGetFileName(
        ...
        DWORD *pnFilterIndex,
        BOOL bForceHTMLOnly);
HRESULT GetFileNameFromURL( LPWSTR pwszURL, LPTSTR pszFile, DWORD cchFile);
...
public:
    ...
    // CThicketUI methods
    HRESULT SaveDocument( HWND hWnd, LPCTSTR pszFileName, IHTMLDocument2 *pDoc, UINT codepageSrc, UINT codepageDst, UINT iPackageStyle );
...
HRESULT CThicketUI::SaveDocument( HWND hWnd, LPCTSTR pszFileName, IHTMLDocument2 *pDoc, UINT codepageSrc, UINT codepageDst, UINT iPackageStyle)
{
...
#ifndef NO_MARSHALLING
    // We don't do anything with pDoc until we're on the worker thread, so marshall it.
...
#endif
    {
        ...
        DialogBoxParam(MLGetHinst(), MAKEINTRESOURCE(IDD_SAVETHICKET), hWnd, CThicketUI::ThicketUIDlgProc, (LPARAM)this);

     //   HWND hwnd = MLCreateDialogParamWrap(MLGetHinst(), MAKEINTRESOURCE(IDD_SAVETHICKET), NULL, CThicketUI::ThicketUIDlgProc, (LPARAM)this);
     //   if (!hwnd)
     //       _hrDL = E_FAIL;
    }
...
BOOL_PTR CThicketUI::ThicketUIDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    BOOL        fRet = FALSE;
...
BOOL CThicketUI::DlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    BOOL fRet = TRUE;
    ...
    case WM_INITDIALOG:
        ...
        if (!(_fThreadStarted = SHQueueUserWorkItem(ThicketUIThreadProc, this, 0, (DWORD_PTR)NULL, (DWORD_PTR *)NULL, "shdocvw.dll", TPS_LONGEXECTIME)))
            _hrDL = E_FAIL;
#else
...
DWORD WINAPI CThicketUI::ThicketUIThreadProc( LPVOID ppv )
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = S_FALSE;
            hr = docPkgr.PackageDocument( pDoc, ptui->_pszFileName, &ptui->_fCancel, &tprog, 0, 100, ptui->_codepageDst );
...
void SaveBrowserFile( HWND hwnd, LPUNKNOWN punk )
{
    ...
    hr = pDoc->QueryInterface(IID_IOleCommandTarget, (void **)&pOleCommandTarget);
    if (FAILED(hr))
    {
        goto Cleanup;
    }
    ...
    if (pwzExt > bstrURL) {
        ...
        {
            hr = pOleCommandTarget->Exec(&CGID_MSHTML, IDM_SAVEPICTURE, 0, NULL, NULL);
    ...
    if (pCmd[0].cmdf & OLECMDF_LATCHED)
    {
        ...
        if (RegOpenKeyEx(HKEY_CURRENT_USER, REGKEY_SAVEAS_WARNING_RESTRICTION, 0, KEY_READ, &hkeySaveAs) == ERROR_SUCCESS)
        {
            dwSize = sizeof(DWORD);

            if (RegQueryValueEx(hkeySaveAs, REGVALUE_SAVEAS_WARNING, NULL, NULL, (LPBYTE)&dwValue, &dwSize) == ERROR_SUCCESS)
            {
                if (dwValue)
                {
        ...
        iFlags = DialogBoxParam(MLGetHinst(), MAKEINTRESOURCE(DLG_SAVEAS_WARNING), hwnd, SaveAsWarningDlgProc, (LPARAM)0);
        if (!(iFlags & SAVEAS_OK))
        {
            goto Cleanup;
        }
        ...
        if (iFlags & SAVEAS_NEVER_ASK_AGAIN)
        {
            ...
            if (RegOpenKeyEx(HKEY_CURRENT_USER, REGKEY_SAVEAS_WARNING_RESTRICTION, 0, KEY_ALL_ACCESS, &hkey) == ERROR_SUCCESS)
            {
                RegSetValueEx(hkey, REGVALUE_SAVEAS_WARNING, 0, REG_DWORD, (CONST BYTE *)&dwNeverAsk, sizeof(dwNeverAsk));
                RegCloseKey(hkey);
            }
        }
    }
...
Continue:
    ...
    hr = FormsGetFileName(hwnd, szFileDst, ARRAYSIZE(szFileDst), (LONG_PTR)&tcpi, &iFilter, bForceHTMLOnly);
    if (hr==S_OK)
...
void ReportThicketError( HWND hwnd, HRESULT hr )
{
    ...
    if ( lpstrMsg )
    {
        MLShellMessageBox(hwnd, lpstrMsg, MAKEINTRESOURCE(IDS_THICKETERRTITLE), MB_OK | MB_ICONERROR);
    }
}
...
UINT_PTR APIENTRY SaveOFNHookProc(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
    ULONG i, iCurSel;
    ...
    switch (uiMsg)
    {
...
#endif /* UNIX */
            if (pMultiLanguage->EnumCodePages( MIMECONTF_SAVABLE_BROWSER | MIMECONTF_VALID, langid, &pEnumCodePage) == S_OK)
            {
                MIMECPINFO cpInfo;
                ULONG      ccpInfo;
                UINT       cpDefault;

                if (pMultiLanguage->GetCodePageInfo(csetInfo.uiInternetEncoding, langid, &cpInfo) == S_OK && !(cpInfo.dwFlags & MIMECONTF_SAVABLE_BROWSER))
                {
                ...
                if (cpDefault == CODEPAGE_UNICODE && pofn->nFilterIndex == PACKAGE_MHTML) {
                ...
                for (i = 0; pEnumCodePage->Next(1, &cpInfo, &ccpInfo) == S_OK; ++i)
                {
                    ...
                    if (cpInfo.uiCodePage == CODEPAGE_UNICODE && pofn->nFilterIndex == PACKAGE_MHTML) {
                        i--;
                        continue;
                    }
                    ...
                    {
                       StrCpyNW(wzEncoding, cpInfo.wszDescription, lstrlen(cpInfo.wszDescription) + 1);
                       bFoundEncoding = TRUE;
                    }

                    lpszDesc = cpInfo.wszDescription;

                    iIdx = SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_ADDSTRING, 0, (LPARAM)lpszDesc);
                    SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_SETITEMDATA, iIdx, (LPARAM)cpInfo.uiCodePage);
                }
                ...
                if (bFoundEncoding)
                {
                    ...
                    SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_SETCURSEL, (WPARAM)iIndex, 0);
                }
                else
                {
                    ...
                    SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_SETCURSEL, 0, 0);
                }
            }
            ...
            switch (phdr->hdr.code)
            {
                case CDN_FILEOK:
                {
                    ...
                    ptcpi->cpDst = //*(UINT *)phdr->lpOFN->lCustData =
                        (UINT)SendDlgItemMessage (hdlg, IDC_SAVE_CHARSET, CB_GETITEMDATA, (WPARAM)iCurSel, (LPARAM)0);
                }
                ...
                case CDN_TYPECHANGE:
                {
                    ...
                    {
                        iCurSel = (int)SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_GETCURSEL, 0, 0);
                        uiCPSel = (UINT)SendDlgItemMessage (hdlg, IDC_SAVE_CHARSET, CB_GETITEMDATA, (WPARAM)iCurSel, (LPARAM)0);
                        ...
                        i = (int) SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_FINDSTRINGEXACT, (WPARAM)0, (LPARAM)UNICODE_TEXT);
                        if (i != CB_ERR)
                        {
                            SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_DELETESTRING, i, (LPARAM)0);
                        }

                        iCount = (int)SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_GETCOUNT, 0, 0);

                        // Set selected item back

                        for (i = 0; i < iCount; i++)
                        {
                            uiCP = (UINT)SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_GETITEMDATA, (WPARAM)i, (LPARAM)0);
                            if (uiCP == uiCPSel)
                            {
                                iSet = i;
                            }
                        ...
                        if (iSet != 0xffffffff)
                        {
                            SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_SETCURSEL, (WPARAM)iSet, (LPARAM)0);
                        }
                    }
                    else
                    {
                        ...
                        if (i == CB_ERR) {
                            ...
                            i = (int) SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_ADDSTRING, 0, (LPARAM)UNICODE_TEXT);
                            SendDlgItemMessage(hdlg, IDC_SAVE_CHARSET, CB_SETITEMDATA, i, (LPARAM)CODEPAGE_UNICODE);
        ...
        case WM_HELP:
        {
            SHWinHelpOnDemandWrap((HWND)((LPHELPINFO) lParam)->hItemHandle, c_szHelpFile, HELP_WM_HELP, (DWORD_PTR)aSaveAsHelpIDs);
        }
        break;
        case WM_CONTEXTMENU:
        {
            SHWinHelpOnDemandWrap((HWND) wParam, c_szHelpFile, HELP_CONTEXTMENU, (DWORD_PTR)aSaveAsHelpIDs);
        }
        break;
    }
        ...
        BOOL bForceHTMLOnly)
{
    ...
    if ( SHGetValue(HKEY_CURRENT_USER, REGSTR_PATH_MAIN, REGSTR_VAL_SAVEDIRECTORY, &dwType, achPath, &cbData) != ERROR_SUCCESS || !PathFileExists(achPath))
    {
...
HRESULT GetFileNameFromURL( LPWSTR pwszURL, LPTSTR pszFile, DWORD cchFile)
{
    ...
    if (cchUrl)
    {
        ...
        if (SUCCEEDED(ParseURLW(pwszURL, &puw)))
        {
            ...
            if ( !GetUrlCacheEntryInfoW( pwszURL, &ceiT, &dwSize ) && GetLastError() == ERROR_INSUFFICIENT_BUFFER &&
                 (pcei = (LPINTERNET_CACHE_ENTRY_INFOW)new BYTE[dwSize]) != NULL && GetUrlCacheEntryInfoW( pwszURL, pcei, &dwSize ) )
            {
...
INT_PTR CALLBACK SaveAsWarningDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_INITDIALOG:
            ...
            fRet = TRUE;
        case WM_COMMAND:
            bChecked = SendDlgItemMessage(hDlg, IDC_SAVEAS_WARNING_CB, BM_GETCHECK, 0, 0 );
