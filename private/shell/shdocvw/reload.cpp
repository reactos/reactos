...
// inserts an IHTMLLoadOptions object that contains the codepage into the bind context
HRESULT InsertCodepageIntoBindCtx(UINT codepage, IBindCtx * pbc)
{
    ...
    if (pbc == NULL) {
        ...
        if (codepage != CP_ACP) {
            ...
            // We got a codepage that wasn't the ansi one create an HTMLLoadOptions object and set the code page in it.
            ...
            if (SUCCEEDED(hr) && phlo) {
                hr = phlo->SetOption(HTMLLOADOPTION_CODEPAGE, &dwcp, sizeof(dwcp));
                if (SUCCEEDED(hr)) {
...
STDMETHODIMP CUrlDownload::QueryInterface(REFIID riid, void ** ppv)
{
    ...
    *ppv = NULL;
    ...
    else if ((IID_IUnknown == riid) || (IID_IHlinkFrame == riid))
    ...
    *pwzUrlBuf = 0;
    ...
    do {
        ...
        case PRC_SEMI_URL_EQL_ANY:
            if (!wch || (wchDel && wch == wchDel) || (!wchDel && wch == L';')) {
                *pwzUrl = 0;
