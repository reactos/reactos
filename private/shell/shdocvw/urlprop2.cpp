...
*/
STDMETHODIMP_(void) CUrlHistoryProp::Dump(void)
{
...
STDMETHODIMP CUrlHistoryProp::InitFromDB(LPCTSTR pszURL, Intshcut *pintshcut, BOOL fPrivObj)
{
    ...
    // Initialize the in-memory property storage from the file and database
    HRESULT hres = Init();
    if (SUCCEEDED(hres))
    {
        if (NULL == m_pintshcut)
...
HRESULT CUrlHistoryProp::LoadFromDB(IN LPCTSTR pszURL)
{
    ...
    if (buf.pcei) {
        ...
        if (phdPrev) {
            ...
            for (const HISTEXTRA* phextPrev = phdPrev->_GetExtra(); !phextPrev->IsTerminator(); phextPrev = phextPrev->GetNextFast())
            {
                TraceMsg(DM_HISTPROP, "CUHP::InitFromDB found HISTEXTRA (id=%d, vt=%d)", phextPrev->idExtra, phextPrev->cbExtra);
    ...
    IN LPARAM        lParam)
{
    ...
    case PID_INTSITE_FLAGS:
        if (ppropvar->vt == VT_UI4 && phdNew) {
            TraceMsg(DM_HISTPROP, "CUHP::s_CommitHistItem updating PID_INSITE_FLAGS (%x to %x)", phdNew->dwFlags, ppropvar->lVal);
    ...
    case PID_INTSITE_WATCH:
        if (ppropvar->vt == VT_UI4 && phdNew) {
            TraceMsg(DM_HISTPROP, "CUHP::s_CommitHistItem updating PID_INSITE_WATCH (%x to %x)", phdNew->dwFlags, ppropvar->lVal);
    ...
    default:
        ...
        case VT_I4:
            ...
            break;
        case VT_LPWSTR:
            cbRequired = WideCharToMultiByte(CP_ACP, 0, ppropvar->pwszVal, -1, NULL, 0, NULL, NULL);
            ...
            if (peparam->phextCur)
            {
                ...
                WideCharToMultiByte(CP_ACP, 0, ppropvar->pwszVal, -1, (LPSTR) peparam->phextCur->abExtra, cbRequired, NULL, NULL);
                peparam->fDirty = TRUE;
            }
            break;
        case VT_EMPTY:
        ...
        if (peparam->phextCur) {
            ...
            TraceMsg(DM_HISTEXTRA, "s_CommitHistItem moving phextCur forward %d bytes", peparam->phextCur->cbExtra);
            ...
            if (peparam->phdPrev) {
                ...
                HISTEXTRA* phextPrev = (HISTEXTRA*) peparam->phdPrev->_FindExtra(propid);
                if (phextPrev) {
                    TraceMsg(DM_HISTEXTRA, "s_CommitHistItem invalidate an old one id=%d %d bytes", phextPrev->idExtra, phextPrev->cbExtra);
...
HRESULT CUrlHistoryProp::Commit(IN DWORD dwFlags)
{
    ...
    if (phdNew) {
        ...
        if (eparam.fDirty)
        {
            ...
            if ((dwFlagsPrev & PIDISF_RECENTLYCHANGED) != (phdNew->dwFlags & PIDISF_RECENTLYCHANGED))
            {
