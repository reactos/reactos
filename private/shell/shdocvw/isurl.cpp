...
STDMETHODIMP Intshcut::InvokeCommand(IN LPCMINVOKECOMMANDINFO pici)
{
    ...
    if (pici && SIZEOF(*pici) <= pici->cbSize)
    {
        ...
        case IDCMD_ISCM_SYNC:
        {
            ...
            {
                ISubscriptionMgr * pMgr;
                if (SUCCEEDED(JITCoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void **)&pMgr, pici->hwnd, FIEF_FLAG_FORCE_JITUI)))
                {
    ...
    if (IsIESchemeHandler(pszVerb, pszScheme))
    {
        ...
        if (varIn.bstrVal)
        {
            ...
            || FAILED(hr = _IEExecFile_TryRunningWindow(&varIn, cid)))
            {
                ...
                if (SUCCEEDED(CoCreateInstance(CLSID_InternetExplorer, NULL, CLSCTX_ALL, IID_IOleCommandTarget, (LPVOID *)&poct)))
                {
