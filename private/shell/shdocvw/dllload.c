...
// we still use ordinal numbers but on unix we will use the new paramater as the function name.
...
DELAY_LOAD_SHELL(g_hinstShell32,
                 shell32,
...
DELAY_LOAD_IE_ORD(g_hinstBrowseui,
    ...
    DELAY_LOAD_IE_HRESULT(g_hinstBrowseui,
                          BROWSEUI,
...
(LPCWSTR lpszUrlName, LPCWSTR lpszLocalFileName, FILETIME ExpireTime, FILETIME LastModifiedTime,
 ...
 (lpszUrlName, lpszLocalFileName, ExpireTime, LastModifiedTime,
  CacheEntryType, lpHeaderInfo, dwHeaderSize, lpszFileExtension, lpszOriginalUrl))
    ...
    DELAY_LOAD(g_hinstWININET, WININET, BOOL, DeleteUrlCacheContainerA, (LPCSTR Name, DWORD dwOptions), (Name, dwOptions))
    ...
    DELAY_LOAD(g_hinstWININET, WININET, DWORD, InternetAttemptConnect, (DWORD dwReserved), (dwReserved))

    DELAY_LOAD(g_hinstWININET,
               WININET,
               BOOL,
               GetUrlCacheConfigInfoA,
               (LPINTERNET_CACHE_CONFIG_INFOA lpCacheConfigInfo, LPDWORD lpdwCacheConfigInfoBufferSize, DWORD dwFieldControl),
    ...
    (HINTERNET hinternet, DWORD dwOptions, LPVOID lpBuffer, DWORD dwBufferLength), (hinternet, dwOptions, lpBuffer, dwBufferLength))

    DELAY_LOAD(g_hinstWININET, WININET, BOOL, InternetGetConnectedState, (LPDWORD lpdwFlags, DWORD dwReserved), (lpdwFlags, dwReserved))
