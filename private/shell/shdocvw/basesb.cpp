...
// stuff that should be turned off while in here, but on when it goes to commonsb or shbrows2.cpp
#define IN_BASESB2
...
// BUGBUG: Remove this #include by defining _bbd._pauto as IWebBrowserApp, just like Explorer.exe.
#include "hlframe.h"
 ...
 * Used to help distribute WM_TIMER messages during OLE operations when we are busy.
 * If we don't install the CoRegisterMessageFilter then OLE can PeekMessage(PM_NOREMOVE) the timers such that they pile up and fill the message queue.
*/
...
public:
    ...
    virtual DWORD __stdcall HandleInComingCall(IN DWORD dwCallType, IN HTASK htaskCaller, IN DWORD dwTickCount, IN LPINTERFACEINFO lpInterfaceInfo)
    {
        if (_lpMFOld)
    ...
    virtual DWORD __stdcall RetryRejectedCall(IN HTASK htaskCallee, IN DWORD dwTickCount, IN DWORD dwRejectType)
    {
        if (_lpMFOld)
    ...
    virtual DWORD __stdcall MessagePending(IN HTASK htaskCallee, IN DWORD dwTickCount, IN DWORD dwPendingType)
    {
        DWORD dw;
        MSG msg;
        ...
        // fault when we dereference _lpMFOld... Make sure this doesn't happen by increasing our refcount.
        AddRef();
    ...
    void UnInitialize()
    {
        ...
        // of automation driving we once hit a function call above and we dereferenced this old pointer and page faulted.
...
CBaseBrowser2::CBaseBrowser2(IUnknown* punkAgg) : CAggregatedUnknown(punkAgg), _bptBrowser(BPT_DeferPaletteSupport)
{
...
HRESULT CBaseBrowser2::ReleaseShellView()
{
    ...
    {
        TraceMsg(TF_WARNING, "CBB(%x)::ReleaseShellView _cRefUIActivateSV(%d)!=0  _bbd._psv=%x ABORTING", this, _cRefUIActivateSV, _bbd._psv);
        return S_OK;
    }
    ...
    if (_bbd._psv) {
        ...
        if (psv)
        {
            ...
            if (_cRefUIActivateSV)
            {
                TraceMsg(TF_WARNING, "CBB(%x)::ReleaseShellView setting _bbd._psv = NULL (was %x) while _cRefUIActivateSV=%d", this, psv, _cRefUIActivateSV);
            }
...
// we no longer need to postpone. In that case, we unblock it by posting a WMC_ASYNCOPERATION.
...
HRESULT CBaseBrowser2::_UIActivateView(UINT uState)
{
    if (_bbd._psv)
    {
        ...
        if (_cRefUIActivateSV)
        {
            TraceMsg(TF_WARNING, "CBB(%x)::_UIActivateView(%d) entered reentrantly!!!!!! _cRefUIActivate=%d", this, uState, _cRefUIActivateSV);
        ...
        TraceMsg(TF_SHDUIACTIVATE, "CBaseBrowser2(%x)::_UIActivateView(%d) about to call _bbd._psv(%x)->UIActivate", this, uState, _bbd._psv);
        ...
        TraceMsg(TF_SHDUIACTIVATE, "CBaseBrowser2(%x)::_UIActivateView(%d) back from _bbd._psv(%x)->UIActivate", this, uState, _bbd._psv);
        _cRefUIActivateSV--;
    ...
    if (SVUIA_ACTIVATE_FOCUS == uState && !_bbd._psv && !_bbd._hwndView &&  _bbd._psvPending &&  _bbd._hwndViewPending)
    {
    ...
    {
        TraceMsg(TF_SHDUIACTIVATE, "CBaseBrowser2(%x)::_UIActivateView processing deferred UIDeactivate, _bbd._psv=%x", this, _bbd._psv);
    ...
    {
        TraceMsg(TF_SHDUIACTIVATE, "CBaseBrowser2(%x)::_UIActivateView processing deferred OnDestroy", this);
...
void CBaseBrowser2::_GetNavigationInfo(WORD * pwNavTypeFlags)
{
    *pwNavTypeFlags = 0;
    ...
    for (;;)
    {
        ...
        {
            *pwNavTypeFlags |= (NAVTYPE_PageIsChanging | NAVTYPE_SiteIsChanging);

            // Make sure we don't do transitions when
            // just navigating between non-web PIDLs.
            if ((_bbd._pidlCur && !fCurIsWeb) || (_bbd._pidlPending && !fPendingIsWeb))
            {
...
void CBaseBrowser2::_MayPlayTransition(IShellView* psvNew, HWND hwndViewNew, BOOL bSiteChanging)
{
    ...
    SHRegGetUSValue(REGSTR_PATH_MAIN, REGSTR_VAL_PAGETRANSITIONS, NULL, (void *)&dwEnabled, &dwSize, FALSE, (void *)&bDefault, SIZEOF(bDefault));
...
HRESULT CBaseBrowser2::_SwitchActivationNow()
{
    ...
    if (_ptrsite)
    {
        if (!(wNavTypeFlags & NAVTYPE_ShellNavigate) && (wNavTypeFlags & NAVTYPE_PageIsChanging) && (_bbd._psv && psvNew))
        {
    ...
    //     instead of IsURLChild() because it doesn't work in Folder Shortcuts and doesn't
    //     work in NSEs outside of the "IE" name space (like Web Folders).
    if ((_bbd._pidlCur && IsURLChild(_bbd._pidlCur, FALSE)) || (pidlNew && IsURLChild(pidlNew, FALSE)))
    ...
    if (_bbd._psv)
    {
        ...
        if (_bbd._pctView && SUCCEEDED(_bbd._pctView->Exec(&CGID_ShellDocView, SHDVID_CANDOCOLORSCHANGE, 0, NULL, NULL)))
        {
...
// Returning S_FALSE indicate that the user hit CANCEL when it is prompted to save the changes (if any).
HRESULT CBaseBrowser2::_MaySaveChanges(void)
{
    HRESULT hres = S_OK;
    if (_bbd._pctView) // we must check!
    {
        hres = _bbd._pctView->Exec(&CGID_Explorer, SBCMDID_MAYSAVECHANGES, OLECMDEXECOPT_PROMPTUSER, NULL, NULL);
    }
...
HRESULT CBaseBrowser2::_DisableModeless(void)
{
    if (_cRefCannotNavigate == 0)
    {
        ...
        if (SUCCEEDED(IUnknown_QueryStatus(_bbd._psvPending, &CGID_ShellDocView, 1, &rgCmd, NULL)) && (rgCmd.cmdf & MSOCMDF_SUPPORTED) && !(rgCmd.cmdf & MSOCMDF_ENABLED))
        {
...
HRESULT CBaseBrowser2::GetViewRect(RECT* prc)
{
    ...
    TraceMsg(DM_UIWINDOW, "ief GetViewRect _rcBorderDoc=%x,%x,%x,%x", _rcBorderDoc.left, _rcBorderDoc.top, _rcBorderDoc.right, _rcBorderDoc.bottom);
    TraceMsg(DM_UIWINDOW, "ief GetViewRect prc=%x,%x,%x,%x", prc->left, prc->top, prc->right, prc->bottom);
    return S_OK;
}
            ...
            else
            {
                ...
                if (S_FALSE == _DisableModeless())
                {
                    ...
                    if (!_bbd._pidlCur)
                    {
                        ...
                        hres = MLBuildResURLWrap(TEXT("shdoclc.dll"), HINST_THISDLL, ML_CROSSCODEPAGE, TEXT("navcancl.htm"), szResURL, ARRAYSIZE(szResURL), TEXT("shdocvw.dll"));
                        if (SUCCEEDED(hres))
                        {
...
HRESULT CBaseBrowser2::QueryService(REFGUID guidService, REFIID riid, void **ppvObj)
{
    ...
    *ppvObj = NULL;
    ...
    //  dispatch those. See comments on CWebBrowserSB::QueryService for detail. (SatoNa)

    if (IsEqualGUID(guidService, SID_SWebBrowserApp) || IsEqualGUID(guidService, SID_SContainerDispatch) || IsEqualGUID(guidService, IID_IExpDispSupport))
    {
        return _bbd._pautoSS->QueryInterface(riid, ppvObj);
    }

    if (IsEqualGUID(guidService, SID_SHlinkFrame) || IsEqualGUID(guidService, IID_ITargetFrame2) || IsEqualGUID(guidService, IID_ITargetFrame))
    {
        return _ptfrm->QueryInterface(riid, ppvObj);
    }
    else if (IsEqualGUID(guidService, SID_STopLevelBrowser) || IsEqualGUID(guidService, SID_STopFrameBrowser))
    {
        if (IsEqualGUID(riid, IID_IUrlHistoryStg))
        {
            ...
            if (!_pIUrlHistoryStg)
            {
                hres = CoCreateInstance(CLSID_CUrlHistory, NULL, CLSCTX_INPROC_SERVER, IID_IUrlHistoryStg, (void **)&_pIUrlHistoryStg);
            }
            if (_pIUrlHistoryStg)
            {
    ...
    else if (IsEqualGUID(guidService, SID_SShellBrowser) || IsEqualGUID(guidService, SID_SVersionHost))
    {
                         ...
                         UINT * puiDelay, LPWSTR pwzUrlBuf, UINT cchUrlBuf)
{
    ...
    enum {
        PRC_START, PRC_DIG, PRC_DIG_WS, PRC_SEMI, PRC_SEMI_URL,
...
CBaseBrowser2::QueryVersionVector(IVersionVector *pVersion)
{
    ...
    if (RegOpenKey(HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Internet Explorer\\Version Vector"), &hkey) == ERROR_SUCCESS)
    {
        for (int iValue = 0; ; iValue++)
        {
            ...
            if (SHEnumValueW(hkey, iValue, wszValue, &cchValue, &dwType, wszVersion, &cchVersion) == ERROR_SUCCESS)
            {
...
#ifdef LIGHT_FRAMES
HRESULT CBaseBrowser2::SaveObject(void)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::GetMoniker(DWORD dwAssign, DWORD dwWhichMoniker, IMoniker **ppmk)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::GetContainer(IOleContainer **ppContainer)
{
...
HRESULT CBaseBrowser2::ShowObject(void)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::OnShowWindow(BOOL fShow)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::RequestNewObjectLayout(void)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::CanInPlaceActivate(void)
{
    return S_OK;
}


HRESULT CBaseBrowser2::OnInPlaceActivate(void)
{
    return S_OK;
}


HRESULT CBaseBrowser2::OnUIActivate(void)
{
    return S_OK;
}


HRESULT CBaseBrowser2::GetWindowContext(
...
HRESULT CBaseBrowser2::Scroll( /* [in] */ SIZE scrollExtant)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::OnUIDeactivate( /* [in] */ BOOL fUndoable)
{
    return S_OK;
}


HRESULT CBaseBrowser2::OnInPlaceDeactivate(void)
{
    return S_OK;
}


HRESULT CBaseBrowser2::DiscardUndoState(void)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::DeactivateAndUndo(void)
{
    return E_NOTIMPL;
}


HRESULT CBaseBrowser2::OnPosRectChange( /* [in] */ LPCRECT lprcPosRect)
{
    return E_NOTIMPL;
}
