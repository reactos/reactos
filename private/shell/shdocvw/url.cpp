 ...
 * 0x00000000   Stop running any further IURLSearchHooks and pass this URL back to the browser for browsing.
 ...
 *              BUGBUG: This is not fully implemented, yet, making IURLQualify return this involves too much change.
...
void InitURLSearchHooks()
{
    ...
    // The installer needs to install the values into LOCAL_MACHINE so they are accessable to all users.
    // Then anyone wanting to modify the value, will need to determine if they want to add it to a specific user's CURRENT_USER or modify the LOCAL_MACHINE value to
    ...
    if ((hdpa) && (SHRegOpenUSKey(c_szURLSearchHook, KEY_READ, NULL, &hkeyHooks, FALSE) == ERROR_SUCCESS))
    {
        ...
        do {
            ...
            lEnumReturn = SHRegEnumUSValue(hkeyHooks, dwiValue, szCLSID, &dwccCLSIDLen, NULL, NULL, NULL, SHREGENUM_DEFAULT);
            if (lEnumReturn == ERROR_SUCCESS)
            {
                ...
                if (SUCCEEDED(SHCLSIDFromString(szCLSID, &clsidHook)))
                {
                    ...
                    HRESULT hr = CoCreateInstance(clsidHook, NULL, CLSCTX_INPROC_SERVER | CLSCTX_LOCAL_SERVER, IID_IURLSearchHook, (LPVOID *)&pusHook);
                    if (SUCCEEDED(hr))
...
Purpose: This function qualifies a string as a URL.  Strings
         such as "www.foo.com" would have the scheme guessed if the correct flags are given.
         Local paths are converted to "file:" URLs.

         pszTranslatedURL may point to the same buffer as pcszURL.

         If the given string is already a URL (not necessarily canonicalized, though), this function will not touch it,
         unless UQF_CANONICALIZE is set, in which case the string will be canonicalized.
    ...
    LPBOOL      pbWasCorrected)
{
    ...
    // relative path \data\ftp\docs, would wrongly be turned into "ftp://\data\ftp\docs".
    ...
    else
    {
            ...
            pcszURL[1] == TEXT(':') || pcszURL[1] == TEXT('|') || pcszURL[0] == TEXT('\\')))
        {
            ...
            if(SUCCEEDED(hres))
            {
                //  SHSTRs have a size granularity, so the size will be equal to or greater than what was set.
                ...
                //  put a default drive letter on the path if necessary.
        ...
        else if (SUCCEEDED(hres = strOut.SetSize(MAX_URL_STRING)))
        {
            ...
            if (hres == S_FALSE && IsFlagSet(dwFlags, UQF_AUTOCORRECT))
            {
                hres = UrlFixup(pcszURL, (LPTSTR)strOut, strOut.GetSize());
                bWasCorrected = (hres == S_OK);
            }

            if (hres == S_FALSE && IsFlagSet(dwFlags, UQF_USE_DEFAULT_PROTOCOL))
            {
