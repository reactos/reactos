...
void CUrlHistory::s_SetDaysToKeep(DWORD dwDays)
{
    ...
    Error = RegSetValueEx(hk, c_szRegValDaysToKeep, 0, REG_DWORD, (LPBYTE) &dwDays, sizeof(dwDays));
...
DWORD CUrlHistory::s_GetDaysToKeep(void)
{
    ...
    DWORD Error = RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_URLHISTORY, 0, KEY_READ, &hk);
    if(Error)
    {
        Error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_URLHISTORY, 0, KEY_READ, &hk);
    }

    if(!Error)
    {
        Error = RegQueryValueEx(hk, c_szRegValDaysToKeep, 0, &dwType, (LPBYTE) &dwDays, &cbDays);
        RegCloseKey(hk);
    }
