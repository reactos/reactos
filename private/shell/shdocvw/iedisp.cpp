...
// Define a new internal class that is used to manage a set of simple properties that we manage as part of the object.
// This is mainly used such that pages (or objects that manage a page can save state across pages.
...
HRESULT CIEFrameAuto::Navigate(BSTR URL, VARIANT * Flags, VARIANT * TargetFrameName, VARIANT * PostData, VARIANT * Headers)
{
    ...
    if ((pwzTargetFrameName && pwzTargetFrameName[0])) {
        ...
        if (SUCCEEDED(hres))
        {
            ...
            hres = pOurTargetFrame->FindFrame(pwzTargetFrameName, FINDFRAME_JUSTTESTEXISTENCE, &punkTargetFrame);
    ...
    if (pwzHeaders || pPostData || dwFlags || grBindFlags) {
        ...
        hres = NavigateHack(dwFlags, pBindCtx, pStubCallback, fOpenWithFrameName ? pwzTargetFrameName:NULL, wszPath, pwzLocation);
    } else {
        ASSERT(dwFlags==0);
...
HRESULT CIEFrameAuto::ShowBrowserBar(VARIANT * pvaClsid, VARIANT *pvaShow, VARIANT *pvaSize)
{
    ...
    if (pvaClsid->vt == VT_I2 && (pvaClsid->iVal == FCW_ADDRESSBAR || pvaClsid->iVal == FCW_TOOLBAND || pvaClsid->iVal == FCW_LINKSBAR ))
    {
        return IUnknown_Exec(_pbs, &CGID_Explorer, SBCMDID_SHOWCONTROL, MAKELONG(pvaClsid->iVal, pvaShow ? pvaShow->boolVal : 1), NULL, NULL);
    }
    else {
        return IUnknown_Exec(_pbs, &CGID_ShellDocView, SHDVID_SHOWBROWSERBAR, pvaShow ? pvaShow->boolVal : 1, pvaClsid, NULL);
    }
}
...
CIEFrameClassFactory::CIEFrameClassFactory(IUnknown* punkAuto, REFCLSID clsid, UINT uFlags) : _cRef(1), _dwRegister((DWORD)-1), _uFlags(uFlags)
{
    AssertParking();
    ...
    HRESULT hres = CoRegisterClassObject(clsid, this, CLSCTX_LOCAL_SERVER | CLSCTX_INPROC_SERVER, REGCLS_MULTIPLEUSE, &_dwRegister);
    if (FAILED(hres))
    {
        _dwRegister = (DWORD)-1;
    }
...
void FireEvent_NavigateComplete(IUnknown* punk, IWebBrowser2* pwb2, LPCITEMIDLIST pidl, HWND hwnd)
{
    ...
    if (pcp1)
    {
        ...
        // or vbscript: URL.
        if (GetUrlSchemeW(sstrPath.wsz) != URL_SCHEME_JAVASCRIPT && GetUrlSchemeW(sstrPath.wsz) != URL_SCHEME_VBSCRIPT)
        {
...
HRESULT CIEFrameAuto::CAutomationStub::ResolveTypeInfo2( )
{
    ...
    if (!IEFrameAuto()->_hinstMSHTML)
    {
        ...
        {
            IEFrameAuto()->_pfnMEGetIDsOfNames = (PFN_MatchExactGetIDsOfNames)GetProcAddress(IEFrameAuto()->_hinstMSHTML, "MatchExactGetIDsOfNames");
        }
    }
...
HRESULT CIEFrameAuto::CAutomationStub::GetIDsOfNames(REFIID riid, OLECHAR **rgszNames, UINT cNames, LCID lcid, DISPID *rgdispid )
{
...
STDMETHODIMP CIEFrameAuto::CAutomationStub::GetDispID(BSTR bstrName, DWORD grfdex, DISPID *pid)
{
    HRESULT       hr;
    ...
    if ( IEFrameAuto()->_pfnMEGetIDsOfNames )
    {
        ...
        hr = (IEFrameAuto()->_pfnMEGetIDsOfNames)(_pInterfaceTypeInfo2, IID_NULL, &bstrName, 1, 0, pid, grfdex & fdexNameCaseSensitive);
    }
    else
    {
...
HRESULT CIEFrameAuto::COmWindow::showModelessDialog(BSTR strUrl, VARIANT * pvarArgIn, VARIANT * pvarOptions, IHTMLWindow2 ** ppDialog)
{
    ...
    if( SUCCEEDED(hr) )
    {
        hr = pWindow->showModelessDialog(strUrl, pvarArgIn, pvarOptions, ppDialog);
        pWindow->Release( );
    }
    ...
    if (enabled)
    {
        ...
        *enabled =  VARIANT_FALSE;
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(ZoneCheckUrlExW(strUrl, &dwPolicy, SIZEOF(dwPolicy), NULL, NULL, URLACTION_COOKIES, PUAF_NOUI, NULL)) && (URLPOLICY_DISALLOW != dwPolicy))
            {
...
STDMETHODIMP CIEFrameAuto::COmNavigator::javaEnabled(VARIANT_BOOL* enabled)
{
            ...
            StrStr((LPCTSTR)strUrl, L"/exchange/calendar/pick.asp"))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(ZoneCheckUrlExW(strUrl, &dwPolicy, SIZEOF(dwPolicy), NULL, NULL, URLACTION_JAVA_PERMISSIONS, PUAF_NOUI, NULL)) && (URLPOLICY_JAVA_PROHIBIT != dwPolicy))
            {
                *enabled = VARIANT_TRUE;
            }
...
CIEFrameAuto::CCommonCollection::CCommonCollection( ) :  CAutomationStub( MIN_BROWSER_DISPID, MAX_BROWSER_DISPID, TRUE )
{
}
...
HRESULT CIEFrameAuto::CMimeTypes::Init()
{
    ...
    return CAutomationStub::Init( SAFECAST(this, IHTMLMimeTypesCollection*), IID_IHTMLMimeTypesCollection,  CLSID_CMimeTypes, pauto );
}
...
HRESULT CIEFrameAuto::COmNavigator::get_appMinorVersion(BSTR * p)
{
    ...
    *p = NULL;
    ...
    lResult = RegOpenKey(HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings"), &hkInetSettings );
...
HRESULT CIEFrameAuto::CPlugins::Init()
{
    ...
    return CAutomationStub::Init( SAFECAST(this, IHTMLPluginsCollection*), IID_IHTMLPluginsCollection, CLSID_CPlugins, pauto );
}
...
/*
                    Window Open Support
*/
CIEFrameAuto::COmHistory::COmHistory( ) : CAutomationStub( MIN_BROWSER_DISPID, MAX_BROWSER_DISPID, TRUE )
{
}
...
BOOL GetNextOption( BSTR& bstrOptionString, BSTR* optionName, int* piValue )
{
    ...
    *optionName = GetNextToken(bstrOptionString, L"=,", L" \t\n\r", &delimiter);
    ...
    bstrOptionString=delimiter;// Advance the option string to the delimiter
    return TRUE;
}
...
BSTR GetNextToken( BSTR bstr, BSTR delimiters, BSTR whitespace, BSTR *nextPos )
{
    ...
    *nextPos = curPos;// return the delimiter
    return result;
}
