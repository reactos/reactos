        ...
        if(SUCCEEDED(hres))
        {
            ...
            if(SUCCEEDED(hres))
            {
                ...
                if(SUCCEEDED(hres) && Component.wszSubscribedURL[0])
                {
                    ...
                    hres = CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (LPVOID*)&pSubsMgr);
                    if(SUCCEEDED(hres))
                    {
...
void CDocObjectHost::CDOHBindStatusCallback::_CheckForCodePageAndShortcut(void)
{
    ...
    if (SUCCEEDED(hres)) {
        ...
        if ((codepage != CP_ACP || fHasShortcut) && _pbc) {
            ...
            HRESULT hres = CoCreateInstance(CLSID_HTMLLoadOptions, NULL, CLSCTX_INPROC_SERVER, IID_IHtmlLoadOptions, (void**)&phlo);
