...
ULONG RegisterNotify(HWND hwnd, UINT nMsg, LPCITEMIDLIST pidl, DWORD dwEvents, UINT uFlags, BOOL fRecursive)
{
    ...
    if (g_pfnSHChangeNotifyDeregister == NULL)
    {
        ...
        g_pfnSHChangeNotifyRegister = (PFNSHCHANGENOTIFYREGISTER)GET_PRIVATE_PROC_ADDRESS(hmodShell32, "NTSHChangeNotifyRegister", (LPSTR)640);
        if (g_pfnSHChangeNotifyRegister && (WhichPlatform() == PLATFORM_INTEGRATED))
        {
            g_pfnSHChangeNotifyDeregister = (PFNSHCHANGENOTIFYDEREGISTER)GET_PRIVATE_PROC_ADDRESS(hmodShell32, "NTSHChangeNotifyDeregister", (LPSTR)641);
            g_fNewNotify = TRUE;
        }
        else
        {
            g_pfnSHChangeNotifyRegister = (PFNSHCHANGENOTIFYREGISTER)GET_PRIVATE_PROC_ADDRESS(hmodShell32, "SHChangeNotifyRegister", (LPSTR)2);
...
DWORD SHRestricted2W(BROWSER_RESTRICTIONS rest, LPCWSTR pwzUrl, DWORD dwReserved)
{
    ...
    if (pwzUrl && InRange(rest, REST_WITHACTION_FIRST, REST_WITHACTION_LAST))
    {
        ...
        HRESULT hr;
        hr = CoCreateInstance(CLSID_InternetSecurityManager, NULL, CLSCTX_INPROC_SERVER, IID_IInternetSecurityManager, (void**)&pism);
...
void IEWriteErrorLog(const EXCEPTION_RECORD* pexr)
{
    ...
    _try
    {
        ...
        if (hfile != INVALID_HANDLE_VALUE)
        {
            ...
            HRESULT hres = CoCreateInstance(CLSID_CUrlHistory, NULL, CLSCTX_INPROC_SERVER, IID_IUrlHistoryStg, (void **)&pUrlHistStg);
            if (SUCCEEDED(hres)) {
                IEnumSTATURL* penum;
...
HRESULT JITCoCreateInstance(REFCLSID rclsid, LPUNKNOWN pUnkOuter, DWORD dwClsContext, REFIID riid, LPVOID FAR* ppv, HWND hwndParent, DWORD dwJitFlags)
{
    uCLSSPEC ucs;
    QUERYCONTEXT qc = { 0 };
    HRESULT hr;
...
// Check if it is safe to create a shortcut for the given url. 
// Used by add to favorites code.
{
    ASSERT(pidl);
    ...
    if (IsURLChild(pidl, FALSE))
    {
        ...
        if (URL_SCHEME_JAVASCRIPT == nScheme || URL_SCHEME_VBSCRIPT == nScheme)
        {
            ...
            MLLoadString(IDS_ADDTOFAV_WARNING + ilsFlag, szText, ARRAYSIZE(szText));

            fRet = (IDYES == MLShellMessageBox(hwnd, szText, szTitle, MB_YESNO | MB_ICONWARNING | MB_APPLMODAL | MB_DEFBUTTON2));
        }
    }
