...
LPHEIPIDL _CreateHCacheFolderPidl(BOOL fOleMalloc, LPCTSTR pszUrl, FILETIME ftModified, LPSTATURL lpStatURL, __int64 llPriority = 0, DWORD dwNumHits = 0);
...
BOOL      GetDisplayNameForTimeInterval( const FILETIME *pStartTime, const FILETIME *pEndTime, TCHAR *szBuffer, int cbBufferLength );
BOOL      GetTooltipForTimeInterval( const FILETIME *pStartTime, const FILETIME *pEndTime, TCHAR *szBuffer, int cbBufferLength );
...
HRESULT HistCacheFolderView_OnAddPropertyPages(DWORD pv, SFVM_PROPPAGE_DATA * ppagedata)
{
    ...
    HRESULT hres = CoCreateInstance(CLSID_FileTypes, NULL, CLSCTX_INPROC_SERVER, IID_IShellPropSheetExt, (void **)&pspse);
    if (SUCCEEDED(hres))
    {
...
HRESULT CALLBACK CacheFolderView_ViewCallback(IShellView *psvOuter, IShellFolder *psf, HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    return HistCacheFolderView_ViewCallback(psvOuter, psf, hwnd, uMsg, wParam, lParam, FOLDER_TYPE_Cache);
}

HRESULT CALLBACK HistFolderView_ViewCallback(IShellView *psvOuter, IShellFolder *psf, HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    return HistCacheFolderView_ViewCallback(psvOuter, psf, hwnd, uMsg, wParam, lParam, FOLDER_TYPE_Hist);
}

HRESULT CALLBACK IntervalFolderView_ViewCallback(IShellView *psvOuter, IShellFolder *psf, HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    return HistCacheFolderView_ViewCallback(psvOuter, psf, hwnd, uMsg, wParam, lParam, FOLDER_TYPE_HistInterval);
}

HRESULT CALLBACK DomainFolderView_ViewCallback(IShellView *psvOuter, IShellFolder *psf, HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
