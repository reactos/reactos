    ...
    if (_hwnd) {
        int iDC = SaveDC(hdcDraw);
          ...
          // to rcBounds. Ugly.
           ...
           SendMessage(_hwnd, WM_PRINT, (WPARAM)hdcDraw, PRF_NONCLIENT|PRF_CLIENT|PRF_CHILDREN|PRF_ERASEBKGND);
...
HRESULT CShellEmbedding::Freeze(DWORD dwDrawAspect, LONG lindex, void *pvAspect, DWORD *pdwFreeze)
{
    IVOMSG(TEXT("Freeze"));
    *pdwFreeze = 0;
    return S_OK;
...
HRESULT CShellEmbedding::SetAdvise(DWORD aspects, DWORD advf, IAdviseSink *pAdvSink)
{
...
HRESULT CShellEmbedding::GetAdvise(DWORD *pAspects, DWORD *pAdvf, IAdviseSink **ppAdvSink)
{
...
// ** IViewObject2 **
HRESULT CShellEmbedding::GetExtent(DWORD dwDrawAspect, LONG lindex, DVTARGETDEVICE *ptd, LPSIZEL lpsizel)
{
...
HRESULT CShellEmbedding::_CreateWindowOrSetParent(IOleWindow* pwin)
{
    ...
    if (_hwnd) {
        ...
        _hwnd = CreateWindowEx(
            ...
            (hwndParent ? (WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_TABSTOP) : (WS_POPUP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_TABSTOP)),
...
HRESULT CShellEmbedding::SetHostNames(LPCOLESTR szContainerApp, LPCOLESTR szContainerObj)
{
...
HRESULT CShellEmbedding::Close(DWORD dwSaveOption)
{
    ...
    BOOL fSave = FALSE;
    if (_fDirty && ((OLECLOSE_SAVEIFDIRTY==dwSaveOption) || (dwSaveOption==OLECLOSE_PROMPTSAVE))) {
        fSave = TRUE;
    }
...
HRESULT CShellEmbedding::SetMoniker(DWORD dwWhichMoniker, IMoniker *pmk)
{
...
HRESULT CShellEmbedding::GetMoniker(DWORD dwAssign, DWORD dwWhichMoniker, IMoniker **ppmk)
{
...
HRESULT CShellEmbedding::InitFromData(IDataObject *pDataObject, BOOL fCreation, DWORD dwReserved)
{
...
HRESULT CShellEmbedding::GetClipboardData(DWORD dwReserved, IDataObject **ppDataObject)
{
...
void CShellEmbedding::_OnInPlaceActivate()
{
    ...
    _pipsite->GetWindowContext(&_pipframe, &_pipui, &_rcPos, &_rcClip, &_finfo);
    ...
    SetWindowPos(_hwnd, 0, _rcPos.left, _rcPos.top, _rcPos.right-_rcPos.left, _rcPos.bottom-_rcPos.top, SWP_SHOWWINDOW | SWP_NOZORDER);
...
void CShellEmbedding::_OnUIActivate(void)
{
    ...
    for (HWND hwndFocus = GetFocus(); hwndFocus && hwndFocus!=_hwnd; hwndFocus = GetParent(hwndFocus))
    {}
...
HRESULT CShellEmbedding::EnumVerbs(IEnumOLEVERB **ppEnumOleVerb)
{
        ...
        && (pformatetcIn->tymed & TYMED_ENHMF))
    {
        if (_hwnd)
        {
            ...
            if (hdc)
            {
                ...
                Draw(DVASPECT_CONTENT, LINDEX_INTERNAL, NULL, pformatetcIn->ptd, hdcTargetDevice, hdc, &rectBounds, NULL, NULL, 0);
    ...
    else if (pformatetcIn->cfFormat == CF_METAFILEPICT && (pformatetcIn->tymed & TYMED_MFPICT))
    {
        ...
        if (hmem)
        {
            ...
            if (hdc)
            {
                ...
                Draw(DVASPECT_CONTENT, LINDEX_INTERNAL, NULL, pformatetcIn->ptd, hdcTargetDevice, hdc, &rectBounds, &rectBounds, NULL, 0);
                ...
                if (pmf->hMF)
                {
                    ...
                    TraceMsg(TF_SHDCONTROL, "sdv TR ::GetData (%d,%d)-(%d,%d)", _size.cx, _size.cy, _sizeHIM.cx, _sizeHIM.cy);
...
HRESULT CShellEmbedding::QueryGetData(FORMATETC *pformatetc)
{
    ...
    else if (pformatetc->cfFormat == CF_METAFILEPICT && (pformatetc->tymed & TYMED_MFPICT))
    {
        hres = S_OK;
    }
...
HRESULT CShellEmbedding::SetObjectRects(LPCRECT lprcPosRect, LPCRECT lprcClipRect)
{
    ...
    if (_hwnd)
    {
        SetWindowPos(_hwnd, NULL, _rcPos.left, _rcPos.top, _size.cx, _size.cy, SWP_NOZORDER | SWP_NOACTIVATE);
    }
...
HRESULT CShellEmbedding::ResizeBorder(LPCRECT prcBorder, IOleInPlaceUIWindow *pUIWindow, BOOL fFrameWindow)
{
...
LRESULT CShellEmbedding::v_WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_WINDOWPOSCHANGED:
        if (_hwndChild)
        {
            ...
            {
                SetWindowPos(_hwndChild, NULL, 0, 0, lpwp->cx, lpwp->cy, SWP_NOZORDER|SWP_NOMOVE|SWP_NOACTIVATE| (lpwp->flags&(SWP_NOREDRAW|SWP_NOCOPYBITS)));
            }
        }
        goto DoDefault;
