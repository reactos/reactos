...
{
    // flag combinations
    ALL_ISL_RESOLVE_FLAGS = (SLR_NO_UI | SLR_ANY_MATCH | SLR_UPDATE)
}
ISL_RESOLVE_FLAGS;
...
*/
STDMETHODIMP Intshcut::SetPath(LPCTSTR pcszPath)
{
    HRESULT hr;
...
Note:
    1. SetIDList also does SetPath implicitly to update the path (URL) to match the pidl.
...
Note:
    1. If we don't have a pidl from SetIDList, generate a pidl from path.
*/
...
STDMETHODIMP Intshcut::_ComputeDescription()
{
    ...
    if (S_OK == hres)
    {
        ...
        // This is a bad assumption but if this assumption is violated then there is a good chance that the URL probably cannot even be navigated to
...
STDMETHODIMP Intshcut::GetDescription(LPTSTR pszDescription, int cchBuf)
{
    ...
    ASSERT(IS_VALID_STRUCT_PTR(this, CIntshcut));
    ASSERT(hr == S_OK && (cchBuf <= 0 || (IS_VALID_STRING_PTR(pszDescription, -1) && EVAL(lstrlen(pszDescription) < cchBuf))));
...
STDMETHODIMP Intshcut::SetWorkingDirectory(LPCTSTR pcszWorkingDirectory)
{
    ...
    ASSERT(IS_VALID_STRUCT_PTR(this, CIntshcut));
    ASSERT(!pcszWorkingDirectory || IS_VALID_STRING_PTR(pcszWorkingDirectory, -1));
    ...
    if (pcszWorkingDirectory)
    {
        ...
        if (GetFullPathName(pcszWorkingDirectory, SIZECHARS(rgchNewPath), rgchNewPath, &pszFileName) > 0)
    ...
    if (hres == S_OK)
    {
        ...
        if (SUCCEEDED(hres))
        {
            ...
            bChanged = !((!pcszWorkingDirectory && S_FALSE == hres) ||
                (pcszWorkingDirectory && S_OK == hres && !StrCmp(pcszWorkingDirectory, szDir)));
...
*/
STDMETHODIMP Intshcut::GetWorkingDirectory(IN LPTSTR pszBuf, IN int    cchBuf)
{
    HRESULT hres;
...
*/
STDMETHODIMP Intshcut::SetHotkey(IN WORD wHotkey)
{
    HRESULT hres;
...
*/
STDMETHODIMP Intshcut::GetHotkey(PWORD pwHotkey)
{
    HRESULT hres;
...
*/
STDMETHODIMP Intshcut::SetShowCmd(IN int nShowCmd)
{
    HRESULT hres;
...
*/
STDMETHODIMP Intshcut::GetShowCmd(OUT int *pnShowCmd)
{
    HRESULT hres;
...
*/
STDMETHODIMP Intshcut::SetIconLocation(IN LPCTSTR pszFile, IN int     niIcon)
{
    HRESULT hres = S_OK;
    BOOL bNewMeat;
    ...
    if (hres == S_OK)
    {
        ...
        hres = GetIconLocation(0, szOldPath, SIZECHARS(szOldPath), &niOldIcon, &uFlags);
        if (SUCCEEDED(hres))
        {
            BOOL bOldMeat;
            BOOL bChanged = FALSE;
            ...
            ASSERT(!*szOldPath || bOldMeat);

            bChanged = ((!bOldMeat && bNewMeat) ||
                (bOldMeat && !bNewMeat) ||
                        (bOldMeat && bNewMeat && (StrCmp(szOldPath, szNewPath) != 0 || niIcon != niOldIcon)));
...
VOID UrlMunge(TCHAR *lpszSrc, TCHAR *lpszDest, UINT   cchDestBufSize, BOOL fRecentlyChanged)
{
...
*------------------*/
STDMETHODIMP Intshcut::_GetIconLocationWithURLHelper(
    IN  LPTSTR pszBuf,
    IN  int    cchBuf,
    OUT int *  pniIcon,
    ...
    BOOL fRecentlyChanged)
{
    ...
    *pszBuf = TEXT('\0');
    ...
    if (SUCCEEDED(hres))
    {
        ...
        hres = HelperForReadIconInfoFromPropStg(pszBuf, cchBuf, pniIcon, m_pprop, rgpropspec, pszActualUrl, cchActualUrlBuf, fRecentlyChanged);
    }
    ...
    if (TEXT('\0') == *pszBuf)
    {
        ...
        if (S_OK == hres)
        {
            ...
            hres = HelperForReadIconInfoFromPropStg(pszBuf, cchBuf, pniIcon, ppropstg, rgpropspec, pszActualUrl, cchActualUrlBuf, fRecentlyChanged);
            ppropstg->Release();
        }
        ...
        if ((S_OK == hres) && (*pszBuf) && pszActualUrl && (*pszActualUrl))
        {
            ...
            if (var.bstrVal)
            {
                hres = WritePropertyNPB(ISHCUT_INISTRING_SECTIONW, ISHCUT_INISTRING_ICONFILEW, &var);
                ...
                if (S_OK == hres)
                {
                    ...
                    if (var.bstrVal)
                    {
                        hres = WritePropertyNPB(ISHCUT_INISTRING_SECTIONW, ISHCUT_INISTRING_ICONINDEXW, &var);
                        SysFreeString(var.bstrVal);
                    }
                }
            }
