...
BOOL WINAPI CharToOemWrapW(LPCWSTR lpszSrc, LPSTR lpszDst)
{
...
DWORD WINAPI CharUpperBuffWrapW(LPWSTR pch, DWORD cchLength)
{
...
HWND WINAPI CreateWindowExWrapW(
    DWORD       dwExStyle,
    LPCWSTR     lpClassName,
    ...
    va_list *   Arguments)
{
    ...
    if (((dwFlags == FORMAT_MESSAGE_FROM_STRING) ||
        ...
        if (!(dwFlags & FORMAT_MESSAGE_ALLOCATE_BUFFER)) {
            ...
            FormatMessageA(dwFlags, strSource, dwMessageId, dwLanguageId, str, str.BufSize(), Arguments);         // We don't handle Arguments != NULL
...
#ifdef NEED_KERNEL32_WRAPPER
DWORD WINAPI GetCurrentDirectoryWrapW(DWORD nBufferLength, LPWSTR lpBuffer)
{
...
#ifdef NEED_KERNEL32_WRAPPER
VOID WINAPI OutputDebugStringWrapW(LPCWSTR lpOutputString)
{
...
HHOOK FORWARD_API WINAPI SetWindowsHookExWrapW(int idHook, HOOKPROC lpfn, HINSTANCE hmod, DWORD dwThreadId)
{
...
BOOL WINAPI CreateProcessWrapW(LPCWSTR lpApplicationName,
                               ...
                               LPPROCESS_INFORMATION lpProcessInformation)
{
    ...
    if (UseUnicodeShell32()) {
        fRet = CreateProcessW(lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
    } else {
        ...
        if (NULL == lpStartupInfo) {
            fRet = CreateProcessA(striApplicationName, striCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, striCurrentDirectory, NULL, lpProcessInformation);
        } else {
            ...
            fRet = CreateProcessA(striApplicationName, striCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, striCurrentDirectory, &si, lpProcessInformation);
        }
    }
...
UINT WINAPI DdeInitializeWrapW(LPDWORD pidInst, PFNCALLBACK pfnCallback, DWORD afCmd, DWORD ulRes)
{
    UINT uRet;
...
void SHChangeNotifyWrap(LONG wEventId, UINT uFlags, LPCVOID dwItem1, LPCVOID dwItem2)
{
    ...
    if (UseUnicodeShell32() || !SHCNF_HAS_WSTR_PARAMS(uFlags)) {
        ...
        _SHChangeNotify(wEventId, uFlags, (void*)(LPSTR)striItem1, (void*)(LPSTR)striItem2);
    }
}
...
HGLOBAL DevNamesAFromDevNamesW(HGLOBAL hdnw)
{
    HGLOBAL         hdna = NULL;
    ...
    if (hdnw) {
        ...
        if (lpdnw) {
            ...
            hdna = GlobalAlloc(GHND, sizeof(DEVNAMES) + cchDriver + cchDevice + cchOutput);
            if (hdna) {
...
HGLOBAL DevNamesWFromDevNamesA(HGLOBAL hdna)
{
    HGLOBAL         hdnw = NULL;
    ...
    if (hdna) {
        ...
        if (lpdna) {
            ...
            hdnw = GlobalAlloc(GHND, sizeof(DEVNAMES) + sizeof(WCHAR) * (cchDriver + cchDevice + cchOutput));
            if (hdnw) {
                LPDEVNAMES lpdnw = (LPDEVNAMES)GlobalLock(hdnw);
                if (!lpdnw) {
                    GlobalFree(hdnw);
                    ...
                    SHAnsiToUnicode((LPSTR)lpszDriver, (LPWSTR)lpdnw + lpdnw->wDriverOffset, cchDriver);
                    SHAnsiToUnicode(lpszDevice, (LPWSTR)lpdnw + lpdnw->wDeviceOffset, cchDevice);
                    SHAnsiToUnicode(lpszOutput, (LPWSTR)lpdnw + lpdnw->wOutputOffset, cchOutput);
                    GlobalUnlock(hdnw);
                }
            }
...
#ifdef UNIX
HGLOBAL DevModeAFromDevModeW(HGLOBAL hdmw)
{
    HGLOBAL         hdma = NULL;
    if (hdmw) {
        ...
        if (lpdmw) {
            ...
            if (hdma) {
                ...
                if (lpdma) {
                    ...
                    memcpy(&lpdma->dmSpecVersion, &lpdmw->dmSpecVersion, offsetof(DEVMODEW, dmFormName) - offsetof(DEVMODEW, dmSpecVersion));
                    memcpy(lpdma->dmFormName, strFormName, CCHFORMNAME);
                    memcpy(&lpdma->dmLogPixels, &lpdmw->dmLogPixels, sizeof(DEVMODEW) - offsetof(DEVMODEW, dmLogPixels));
                    GlobalUnlock(hdma);
                } else {
                    GlobalFree(hdma);
                    hdma = NULL;
        ...
        if (lpdma) {
            ...
            if (hdmw) {
                ...
                if (lpdmw) {
                    ...
                    memcpy(&lpdmw->dmSpecVersion, &lpdma->dmSpecVersion, offsetof(DEVMODEA, dmFormName) - offsetof(DEVMODEA, dmSpecVersion));
                    lstrcpyA(strFormName, (LPCSTR)lpdmw->dmFormName);
                    strFormName.ConvertIncludingNul();
                    memcpy(&lpdmw->dmLogPixels, &lpdma->dmLogPixels, sizeof(DEVMODEA) - offsetof(DEVMODEA, dmLogPixels));
                    GlobalUnlock(hdmw);
                } else {
                    GlobalFree(hdmw);
                    hdmw = NULL;
...
BOOL WINAPI PrintDlgWrapW(LPPRINTDLGW lppd)
{
    BOOL        fRet;
...
BOOL WINAPI PageSetupDlgWrapW(LPPAGESETUPDLGW lppsd)
{
    BOOL fRet;
    ...
    DWORD cchBuffer)
{
    ...
    if (lpszLongPath == NULL) {    // No OUT buffer provided ==> counting mode.
        bCountMode = TRUE;
    } else {    // Initialize OUT buffer.
        *lpszLongPath = '\0';
    }
    ...
    if (!bCountMode && cchBuffer >= 3) {   // Copy the drive letter.
        StrCpyNA(lpszLongPath, lpszShortPath, 3);
    } else {   // Supplied buffer is so small, can't even copy the drive letter!!
        bCountMode = TRUE;
    }
    dwReturnLen += 2;
    ...
    while (*pTmp) {
        ...
        *pTmp = '\0';
        ...
        if (hFind != INVALID_HANDLE_VALUE) {
            ...
            if (!bCountMode && dwReturnLen < cchBuffer) {   // Add the LFN to the path
                PathCombineA(lpszLongPath, lpszLongPath, Find_Data.cFileName);
            } else {   // We are out of buffer space. Continue loop to find space reqd.
                bCountMode = TRUE;
                *lpszLongPath = '\0';
            }
        } else {
...
Done:
    ...
    if (dwReturnLen && bCountMode) {   // If everything OK (dwReturnLen!=0) and Counting Mode, add one.
        return (dwReturnLen + 1);
    } else {   // Return error (dwReturnLen==0) Or
    ...
    DWORD cchBuffer)
{
    ...
    if (dwRet != 0) {   // If the call succeeded, thunk back the size.
        if (dwRet < (DWORD)cchBuffer) {   // Succeeded in getting LFN value in the OUT buffer. Thunk it back
            // to Unicode.
