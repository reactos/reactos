...
int CaseConvertPathExceptDBCS(LPTSTR pszPath, int cch, BOOL fUpper)
{
    ...
    // BUGBUG !!! (ccteng)
    // Do we need to check for Memphis? Is Memphis shipping a kernel compiled with new headers?
...
// BUGBUG, we should validate the sizes of all path buffers by filling them with MAX_PATH fill bytes.
...
Returns: TRUE if it exists
  ...
  then you have to call PathFileExistsAndAttributes, as this function will fail on the UNC server and server\share case!
*/
STDAPI_(BOOL)
...
*/
{
    ...
    if (lpsz)
    {
        ...
        if (lpsz[0] == TEXT('"') && lpsz[cch-1] == TEXT('"'))// Are the first and last chars quotes?
        {// Yep, remove them.
            lpsz[cch-1] = TEXT('\0');
