...
// private prototypes
DWORD InitializeWorkerThreadPool(VOID);
...
--*/
{
    ...
    TerminateWorkers();// kill all I/O worker threads. Queued work items will be lost
    TerminateTimers();// kill all timer threads
    TerminateWaiters();// kill all wait threads
...
LWSTDAPI_(BOOL) SHQueueUserWorkItem(
    ...
    dwFlags     - flags modifying request:
                    ...
                    TPS_EXECUTEIO
                        - execute work item in I/O thread. If set, work item
                          cannot be tagged (and therefore cannot be subsequently cancelled) and cannot have an associated priority
                    ...
                    TPS_DEMANDTHREAD
                        - a thread will be created for this work item if one is
                          not currently available. DEMANDTHREAD work items are queued at the head of the work queue.
                          That is, they get the highest priority

                    TPS_LONGEXECTIME
                        - caller expects this work item to take relatively long time to complete (e.g. it could be in a UI loop).
                          Work items thus described remove a thread from the pool for an indefinite amount of time
...
--*/
{
    ...
    if (!(dwFlags & TPS_EXECUTEIO)) {
        error = g_ThreadPool.QueueWorkItem((FARPROC)pfnCallback, (ULONG_PTR)pContext, lPriority, dwTag, pdwId, pszModule, dwFlags);
        ...
        if ((g_NumIoWorkerThreads == 0) || ((g_NumIoWorkRequests > threshold) && (g_LastIoThreadCreationTickCount + g_ThreadCreationDelta < GetTickCount()))) {
