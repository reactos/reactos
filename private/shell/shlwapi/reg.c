    ...
    IN  DWORD   cbData)
{
    ...
    if (pwszSubKey && pwszSubKey[0])
    {
        if (g_bRunningOnNT)
        {
            dwRet = RegCreateKeyExW(hkey, pwszSubKey, 0, TEXTW(""), 0, KEY_SET_VALUE, NULL, &hkeyNew, &dwDisp);
        }
        else
        {
            CHAR szSubKey[MAX_PATH];
            ...
            dwRet = RegCreateKeyExA(hkey, pszSubKey, 0, "", REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hkeyNew, &dwDisp);
        }
    }
    else
        hkeyNew = hkey;
