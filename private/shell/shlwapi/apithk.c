...
BOOL IsTSRemoteAdministrationMode()
{
    ...
    if (!s_bTSRemoteAdminCached)
    {
        ...
        if (IsOS(OS_WIN2000TERMINAL))
        {
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("System\\CurrentControlSet\\Control\\Terminal Server"), 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
            {
                DWORD dwRegValue;
                DWORD cbValue = sizeof(dwRegValue);

                // "TSAppCompat" == REG_TERMSRV_APP_COMPAT (from sdk\inc\hydra\regapi.h)
                if (RegQueryValueEx(hkey, TEXT("TSAppCompat"), NULL, NULL, (LPBYTE)&dwRegValue, &cbValue) == ERROR_SUCCESS)
                {
