...
HWND SHCreateWorkerWindowW(WNDPROC pfnWndProc, HWND hwndParent, DWORD dwExStyle, DWORD dwFlags, HMENU hmenu, void * p)
{
    ...
    HWND hwnd = CreateWindowExW(dwExStyle, L"WorkerW", NULL, dwFlags, 0, 0, 0, 0, hwndParent, (HMENU)hmenu, HINST_THISDLL, NULL);
    if (hwnd) {
        SetWindowPtr(hwnd, 0, p);
    ...
    if (hinst)
    {
        ...
        if (lRes == ERROR_SUCCESS)
        {
            ...
            else if (lRes != ERROR_SUCCESS && uInstall == PLATFORM_INTEGRATED)
            {
                ...
                RegSetValueEx(hKey, REGVAL_INTEGRATEDBROWSER, (DWORD)NULL, REG_DWORD, (LPBYTE)&fValue, cbSize);
