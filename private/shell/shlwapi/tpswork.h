...
// CThreadPool - maintains lists of work items, non-IO worker threads and IO worker threads
...
private:    // private classes
    ...
    public:
        ...
        CWorkItem(FARPROC lpfn, ULONG_PTR context, LONG priority, DWORD_PTR tag, DWORD_PTR * pid, LPCSTR pszModule, DWORD flags) : CPrioritizedListEntry(priority)
        {
            m_function = lpfn;
            m_context = context;
            m_tag = tag;
        ...
        BOOL Match(DWORD_PTR Tag, BOOL IsTag) {
            return IsTag ? ((m_flags & TPS_TAGGEDITEM) && (m_tag == Tag)) : ((m_flags & TPS_ID) && (m_id == Tag));
        }
...
//#endif
    ...
    {
        // add a work item to the queue at the appropriate place and create a thread to handle it if necessary
        ...
        // demand-thread work-items have the highest priority. Put at head of queue, else insert based on priority
    ...
    DWORD RemoveWorkItem(FARPROC * ppfnFunction, ULONG_PTR * pContext, HMODULE* hModuleToFree, DWORD * pdwFlags, DWORD dwTimeout)
    {
        ...
        while (TRUE) {
            ...
            // BUGBUG - we want MsgWaitForMultipleObjectsEx() here, but Win95 doesn't support it
            ...
            if ((status == WAIT_OBJECT_0) || (status == WAIT_IO_COMPLETION)) {
                ...
                if (dwTimeout != INFINITE) {
                    ...
                    if (dwElapsedTime > dwTimeout) {
                        // waited longer than requested. Don't wait again if we find there's nothing in the FIFO
                        dwWaitTime = 0;
                    } else {
                        // amount of time to wait next iteration is amount of time until expiration of originally specified period
    ...
    DWORD CreateWorkerThread(VOID) {
        ...
        error = StartThread((LPTHREAD_START_ROUTINE)WorkerThread, &hThread, FALSE);
