...
//  Takes a variable number of parameters for IDispatch, packages them up.
//  pdispparams  - The DISPPARAMS structure that receives the result of the packaging.
...
//                 first (2 * cArgs) of them.  See SHPackDispParams for details.
{
    HRESULT hr = S_OK;
    ...
    // the parameters are pushed from right to left.  Put the first argument we encounter at the highest index.
...
//  Takes a variable number of generic parameters, packages them up.
...
//                 The first is the type of the argument, and the second is the corresponding value.

//                 As a special case, a null VT_BSTR can be passed
//                 as a NULL pointer and we will turn it into a genuine null BSTR.
...
//  pinv        - SHINVOKEPARAMS structure that describes the invoke that is about to occur.
...
//  A client can support a "Cancel" flag by returning E_FAIL once the cancel has occurred.
...
//  of the subsequent callback (plus one final bout of work when the entire enumeration completes).
...
//  The Explorer Band responds only to IID_IDispatch and not to the dispatch ID.
...
//  Enumerate the connection point sinks, calling the callback for each one found.
//  The callback function is called once for each sink. 
//  The IUnknown is whatever interface we could get from the sink (either piidCB or piidCB2).
{
    HRESULT hr;
...
//  Send out the callback (if applicable) and then do the invoke if the callback said that was a good idea.
//  Parameters:
...
HRESULT CALLBACK EnumInvokeCallback(IUnknown *psink, LPARAM lParam)
{
    ...
    if (pinv->Callback)
    {
        ...
        // It can return S_FALSE to skip this callback or E_FAIL to stop the invoke altogether
...
//                      If this parameter is NULL, the function does nothing.
//      pinv         -  Structure containing parameters to INVOKE.
//                      The pdispparams field can be NULL; we will turn it into a real DISPPARAMS for you.
...
//      IPFL_USEDEFAULT     - Many fields in the SHINVOKEPARAMS will be set to default values to save the caller effort:
...
//                  pexcepinfo      =   NULL
//                  puArgErr        =   NULL
LWSTDAPI IConnectionPoint_InvokeIndirect(IConnectionPoint *pcp, SHINVOKEPARAMS *pinv)
{
    HRESULT hr;
    DISPPARAMS dp = { 0 };
    IID iidCP;
    ...
    // IDispatch, so we have to try both.  (Sigh.  Too many buggy components in the system.)
...
//  Wrapper around IConnectionPoint_InvokeIndirect with special Cancel semantics.
...
HRESULT CALLBACK InvokeWithCancelProc(IDispatch *psink, SHINVOKEPARAMS *pinv)
{
    ...
    if ((piwc->pfCancel && *piwc->pfCancel) || (piwc->ppvCancel && *piwc->ppvCancel))
        return E_FAIL;
...
LWSTDAPI IConnectionPoint_InvokeWithCancel(IConnectionPoint *pcp, DISPID dispidMember, DISPPARAMS * pdispparams, LPBOOL pfCancel, void **ppvCancel)
{
...
//  Takes a variable number of parameters for IDispatch, packages them up, and invokes them.
...
//  ap           - va_list of parameters to package.  We package up the
//                 first (2 * cArgs) of them.  See SHPackDispParams for details.
LWSTDAPI IConnectionPoint_InvokeParamV(IConnectionPoint *pcp, DISPID dispidMember, VARIANTARG *rgvarg, UINT cArgs, va_list ap)
{
    HRESULT hr;
...
//  Example:
//      VARIANTARG args[1];
//      IConnectionPoint_InvokeParam(pcp, DISPID_PROPERTYCHANGE, args, 1, VT_BSTR, bstrProperty);
LWSTDAPIV IConnectionPoint_InvokeParam(IConnectionPoint *pcp, DISPID dispidMember, VARIANTARG *rgvarg, UINT cArgs, ...)
{
    HRESULT hr;
    va_list ap;
...
//                      This parameter may be NULL, in which case the operation fails.
//      riidCP       -  The connection point interface to locate.
//      pcpOut       -  Receives the IConnectionPoint, if any.
LWSTDAPI IUnknown_FindConnectionPoint(IUnknown *punk, REFIID riidCP, IConnectionPoint **pcpOut)
{
    HRESULT hr;
...
//  invoke parameters, and call the IDispatch::Invoke for each connected sink.
...
LWSTDAPI IUnknown_CPContainerInvokeIndirect(IUnknown *punk, REFIID riidCP, SHINVOKEPARAMS *pinv)
{
...
//  Example:
//      IUnknown_CPContainerInvokeParam(punk, DIID_DShellFolderViewEvents, DISPID_SELECTIONCHANGED, NULL, 0);
...
//  Given an IUnknown, query for its connection point container,
//  find the corresponding connection point, and call the IPropertyNotifySink::OnChanged for each connected sink.
//  Parameters:
//      punk         -  Object that might be an IConnectionPointContainer
//      dispid       -  To pass to IPropertyNotifySink::OnChanged.
