...
//    TRUE - indicates that the OS is correct and Clustering Service may be installed.
{
   ...
   fReturnValue = VerifyVersionInfo( &osiv,
                                     VER_MAJORVERSION | VER_MINORVERSION | VER_SERVICEPACKMAJOR | VER_SUITENAME,
                                     dwlConditionMask );
...
//    TRUE - indicates that Terminal Services is installed.
{
   ...
   fReturnValue = VerifyVersionInfo( &osiv, VER_SUITENAME, dwlConditionMask );
...
BOOL InstallDNS(BOOL bWait)
{
    ...
    StrCatN(szUnattendFile, TEXT("\\cysdnsu.inf"), ARRAYSIZE(szUnattendFile));
    ...
    // create command line
    pszCmdLine = MyFormatString(IDS_INSTALL_DNS_COMMAND_LINE, szSysDir, szInfFile, szUnattendFile);
    if (!pszCmdLine)
        goto ERROR_RETURN;

    // execute the command
    if (!CreateProcess(NULL, pszCmdLine, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
        goto ERROR_RETURN;
...
BOOL InstallDHCP(BOOL bWait)
{
    ...
    dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_SRVWIZ, TEXT("DomainDNSName"), NULL, (LPVOID)szDomainName, &cbSize);
    ...
    // create unattend file
    StrCatN(szUnattendFile, TEXT("\\cysdhcpu.inf"), ARRAYSIZE(szUnattendFile));
    pszText = MyFormatStringAnsi(IDS_INSTALL_DHCP_UNATTEND_TEXT, szDomainName);
    if (!pszText)
        goto ERROR_RETURN;
    ...
    // create command line
    pszCmdLine = MyFormatString(IDS_INSTALL_DHCP_COMMAND_LINE, szSysDir, szInfFile, szUnattendFile);
    if (!pszCmdLine)
        goto ERROR_RETURN;
...
BOOL InstallAD(BOOL bWait)
{
    ...
    dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_SRVWIZ, TEXT("DomainNetBiosName"), NULL, (LPVOID)szDomainNetBiosName, &cbSize);
    if (NO_ERROR != dwRet || cbSize <= sizeof(TCHAR))
        goto ERROR_RETURN;

    // get domain DNS name from registry
    cbSize = sizeof(szDomainDNSName);
    dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_SRVWIZ, TEXT("DomainDNSName"), NULL, (LPVOID)szDomainDNSName, &cbSize);
    if (NO_ERROR != dwRet || cbSize <= sizeof(TCHAR))
        goto ERROR_RETURN;

    // create answer file
    StrCatN(szAnswerFile, TEXT("\\cysad.inf"), ARRAYSIZE(szAnswerFile));
    pszText = MyFormatStringAnsi(IDS_INSTALL_AD_ANSWER_TEXT, szDomainNetBiosName, szDomainDNSName);
    if (!pszText)
        goto  ERROR_RETURN;
    ...
    // create command line
    pszCmdLine = MyFormatString(IDS_INSTALL_AD_COMMAND_LINE, szSysDir, szAnswerFile);
    if (!pszCmdLine)
        goto ERROR_RETURN;
...
VOID GetTcpIpInterfaceFriendlyName(LPWSTR wszFriendlyName, DWORD cchSize)
{
    ...
    *wszFriendlyName = 0;
    ...
    if (NOERROR == hr)
    {
        ...
        if (NO_ERROR == dwRet)
        {
            dwRet = MprConfigGetFriendlyName(hMprConfig, wszGuidName, wszFriendlyName, sizeof(WCHAR) * cchSize);
            if (NO_ERROR != dwRet)
            {
                *wszFriendlyName = 0;
            }
