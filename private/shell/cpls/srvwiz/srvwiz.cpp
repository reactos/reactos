...
STDMETHODIMP CSrvWiz::get_Home(DWORD *pdwHome)
{
    ...
    else
    {
        ...
        if (dwDsRole)
        {
            ...
            DWORD dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_SRVWIZ, TEXT("home"), NULL, (LPVOID)pdwHome, &cbSize);
...
STDMETHODIMP CSrvWiz::put_Home(DWORD dwHome)
{
    ...
    SHSetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_SRVWIZ, TEXT("home"), REG_DWORD, (LPVOID)&dwHome, sizeof(dwHome));
...
STDMETHODIMP CSrvWiz::get_ShowAtStartup(BOOL *pbShow)
{
    ...
    *pbShow = TRUE; // always show it by default
    ...
    if (NO_ERROR == SHGetValue(HKEY_CURRENT_USER, SZ_REGKEY_WELCOME, TEXT("show"), NULL, &dwShow, &cbShow))
    {
        *pbShow = (dwShow != 0);
    }
...
STDMETHODIMP CSrvWiz::put_ShowAtStartup(BOOL bShow)
{
    ...
    SHSetValue(HKEY_CURRENT_USER, SZ_REGKEY_WELCOME, TEXT("show"), REG_DWORD, (LPVOID)&dwShow, sizeof(dwShow));
...
STDMETHODIMP CSrvWiz::put_DomainDNSName(BSTR bstrDomainDNSName)
{
    ...
    SHSetValueW(HKEY_LOCAL_MACHINE, SZ_REGKEY_SRVWIZ, L"DomainDNSName", REG_SZ, bstrDomainDNSName, (lstrlenW(bstrDomainDNSName)+1)*sizeof(WCHAR));
...
STDMETHODIMP CSrvWiz::put_DomainNetBiosName(BSTR bstrDomainNetBiosName)
{
    ...
    SHSetValueW(HKEY_LOCAL_MACHINE, SZ_REGKEY_SRVWIZ, L"DomainNetBiosName", REG_SZ, bstrDomainNetBiosName, (lstrlenW(bstrDomainNetBiosName)+1)*sizeof(WCHAR));
...
STDMETHODIMP CSrvWiz::get_ProductRegistered(BOOL *pbReg)
{
    ...
    *pbReg = FALSE; // Assume it is not registered
    ...
    dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_NTCV, TEXT("Regdone"), NULL, (LPVOID)szData, &cbSize);
...
STDMETHODIMP CSrvWiz::DsRole(DWORD dwInfoLevel, DWORD *pdwRole)
{
    ...
    *pdwRole = 0;
    ...
    if (dwInfoLevel == 0)
    {
        ...
        dwRet = DsRoleGetPrimaryDomainInformation(NULL, DsRolePrimaryDomainInfoBasic, (PBYTE *)&pdib);
        if ((ERROR_SUCCESS == dwRet) && pdib)
        {
            if ((DsRole_RoleBackupDomainController == pdib->MachineRole) || (DsRole_RolePrimaryDomainController == pdib->MachineRole))
            {
    ...
    else if (dwInfoLevel == 1)
    {
        ...
        dwRet = DsRoleGetPrimaryDomainInformation(NULL, DsRoleUpgradeStatus, (PBYTE *)&pusi);
...
// For others that have more states, it is up to the caller to interprete the return value
{
    ...
    *pdwState = 0;
    ...
    {
        // If reg value [HKLM\Software\Microsoft\MSMQ\Parameters,MaxSysQueue] exists, we assume Message Queue is installed.

        DWORD dwRet;
        DWORD dwMaxSysQueue;
        DWORD cbSize = sizeof(dwMaxSysQueue);

        dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_MSMQ, TEXT("MaxSysQueue"), NULL, (LPVOID)&dwMaxSysQueue, &cbSize);
        if (NO_ERROR == dwRet)
        {
            *pdwState = 1;
        }
    }
    else if (!StrCmpIW(bstrService, L"RemoteAccess") || !StrCmpIW(bstrService, L"Routing"))
    {
        ...
        dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_RRAS, TEXT("ConfigurationFlags"), NULL, (LPVOID)&dwCfgFlags, &cbSize);
    ...
    else if (!StrCmpIW(bstrService, L"NetShow"))
    {
        ...
        dwRet = SHGetValue(HKEY_LOCAL_MACHINE, SZ_REGKEY_NETSHOW, TEXT("InstallDir"), NULL, (LPVOID)szPath, &cbSize);
...
// BUGBUG: we can't wait for sysocmgr or dcpromo to finish and then check its return value for sucess because it
...
// BUGBUG: We can't wait for sysocmgr or dcpromo to finish and check the return value because it can cause a reboot.
// So here we register winlogon notify event first, launch both sysocmgr and dcpromo without wait and blindly assume success.
{
    if (!pbRet)
        return E_INVALIDARG;
