...
//  time that we store internally. We send the clock control our TimeProvider function.
WORD g_Modified = 0;
...
void AdjustAMPMPosition(HWND hwnd)
{
   ...
   if (!fMoved && szTimePrefix[0] == TEXT('1'))
   {
        ...
        for (i = 0; i < ARRAYSIZE(rgMoveTimeControls); i++)
        {
            ...
            MoveWindow(hwndCurr, pt.x + width, pt.y, rCurrCtl.right - rCurrCtl.left, rCurrCtl.bottom - rCurrCtl.top, TRUE);
        }
        ...
        pt.y = rAMPMCtl.top;
        ScreenToClient(hwnd, &pt);
        MoveWindow(hwndAMPM, pt.x, pt.y, rAMPMCtl.right - rAMPMCtl.left, rAMPMCtl.bottom - rAMPMCtl.top, TRUE);
   }
}
...
int GetMaxSubstitutedCharWidth(HDC hDC)
//  Determine the widest digit (safety against variable pitch fonts), but
//  do it using strings so that if number substitution is on, we will get the width of the number based on what will actually be displayed
{
    char sz[2] = "0";
    TCHAR szAMPM[12];
    LONG i, nMaxNumWidth;
...
#else
        WideStrToStr(name,  (TimeZoneId == TIME_ZONE_ID_STANDARD) ? info.StandardName : info.DaylightName );
#endif //UNICODE
...
//  Otherwise it returns FALSE.
{
    ...
    if (GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_ICALENDARTYPE, tchCalendar, ARRAYSIZE(tchCalendar)))
    {
...
INT_PTR CALLBACK DateTimeDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
//  Main dialog proc.
{
    int i;
    ...
    switch (uMsg)
    {
        ...
        case ( WM_VSCROLL ) :
        {
            switch (GET_WM_VSCROLL_CODE(wParam, lParam))
            {
                ...
                //  else if (hBuddy == GetDlgItem(hDlg, DATETIME_AMPM))
                    ...
                    PropSheet_Changed(GetParent(hDlg), hDlg);//  Light the apply now button.
                    TimeProvider(&SystemTime, hDlg);//  Force the clock to reflect this setting.
            ...
            switch (GET_WM_COMMAND_ID(wParam, lParam))
            {
                ...
                case ( DATETIME_DAY ) :
                {
                    switch (GET_WM_COMMAND_CMD(wParam, lParam))
                    {
                        case ( EN_CHANGE ) :
                        {
                            CheckNum( hDlg, GET_WM_COMMAND_ID(wParam, lParam) <= DATETIME_SECOND ? DATETIME_TARROW : DATETIME_YARROW, GET_WM_COMMAND_HWND(wParam, lParam) );
        ...
        case ( WM_HELP ) :             // F1
        {
            WinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD_PTR)(LPTSTR)aDateTimeHelpIds );
            break;
        }
        case ( WM_CONTEXTMENU ) :      // right mouse click
        {
            WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD_PTR)(LPTSTR)aDateTimeHelpIds );
            break;
        }
        default :
        {
...
//  Reads the data for a time zone from the registry.
{
    ...
    //  Every time zone must have a standard name, therefore, we save registry space by using the Standard
                ...
                case ( 0 ) :
                {
                    switch (((NMHDR *)lParam)->code)
                    {
                        case ( PSN_APPLY ) :
                        {
                            ...
                            // if the user had modified the time as well as the timezone, then we should honor the time that they gave us since they
                            // explicitly said this was the time.  If we don't then the time they entered will be offset by the timezone change
                            if (g_WasModified)
                            {
                                g_WasModified = FALSE;
                                SetTime();
