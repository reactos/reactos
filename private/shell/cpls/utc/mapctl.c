...
void MapControlRotateTo(HWND window, int x, BOOL animate)
{
    ...
    ctldata->totaldelta = (ctldata->map.size.cx / 2) - WorldMapGetDisplayedLocation(&ctldata->map, x);
    ...
    if (ctldata->totaldelta)
    {
        ...
        if (animate)
        {
            ctldata->timer = SetTimer( ctldata->window, MAPCTL_ANIMATION_TIMER, MAPCTL_ANIMATION_INTERVAL, NULL );
            if (ctldata->timer)
            {
