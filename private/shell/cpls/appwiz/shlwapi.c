        ...
        if (NO_ERROR == dwRet)
        {
            ...
            else if (REG_EXPAND_SZ == dwType)
            {
                ...
                else
                {
                    ...
                    if (NO_ERROR == dwRet)
                    {
                        DWORD cbExpand = CbFromCchW(ExpandEnvironmentStringsW(lpsz, szBuff, ARRAYSIZE(szBuff)));
                        cbSize = max(cbExpand, cbSize);
                    }
                }
...
*/
int SHAnsiToUnicodeNativeCP(UINT uiCP, LPCSTR pszSrc, int cchSrc, LPWSTR pwszDst, int cwchBuf)
{
    ...
    if (cwchRc) {
        ...
        if (cwchNeeded) {
            LPWSTR pwsz = (LPWSTR)LocalAlloc(LMEM_FIXED, cwchNeeded * SIZEOF(WCHAR));
            if (pwsz) {
                cwchRc = MultiByteToWideChar(uiCP, 0, pszSrc, cchSrc, pwsz, cwchNeeded);
                if (cwchRc) {
...
int SHAnsiToUnicodeCP_ACP(LPCSTR pszSrc, LPWSTR pwszDst, int cwchBuf)
{
    int cwchRc = 0;             /* Assume failure */
...
STDAPI_(int) SHStringFromGUIDA(UNALIGNED REFGUID rguid, LPSTR   psz, int     cchMax)
{
    int i;
...
STDAPI_(int) SHStringFromGUIDW(UNALIGNED REFGUID rguid, LPWSTR  psz, int     cchMax)
{
    int i;
    const BYTE * pBytes = (const BYTE *) rguid;
