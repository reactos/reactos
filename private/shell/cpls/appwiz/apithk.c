...
HANDLE _SetJobCompletionPort(HANDLE hJob)
{
    ...
    if ( hIOPort != NULL )
    {
        ...
        if (SetInformationJobObject( hJob,JobObjectAssociateCompletionPortInformation, &CompletionPort, sizeof(CompletionPort) ) )
        {
            hRet = hIOPort;
        }
    }
...
STDAPI_(DWORD) WaitingThreadProc(void *pv)
{
    ...
    while (TRUE)
    {
        ...
        if (!GetQueuedCompletionStatus( hIOPort, &dwCompletionCode, (PULONG_PTR) &pCompletionKey, &lpOverlapped, INFINITE ) || (dwCompletionCode == JOB_OBJECT_MSG_ACTIVE_PROCESS_ZERO))
        {
            break;
        }
    }
    ...
    if (hJob)
    {
        ...
        if (hIOPort)
        {
            ...
            if (bWx86Enabled && bForceX86Env) {
                cchArch = GetEnvironmentVariableW(ProcArchName, szArchValue, sizeof(szArchValue));
            ...
            if (fWorked)
            {
                ...
                if (bAssigned)
                {
                    ...
                    hThread = CreateThread(NULL, 0, WaitingThreadProc, (LPVOID)hIOPort, 0, &idThread);
                    if (hThread)
                    {
