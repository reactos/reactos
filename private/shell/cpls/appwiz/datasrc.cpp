...
*/
STDMETHODIMP CDataSrc::FireOnDataReady(DBROWCOUNT iRow)
{
...
DWORD CDataSrc::_ThreadStartProc()
{
    ...
    _EnumAppItems(_dwEnum, _cbstrCategory);// Enumerate the applications, this function does the real work

    // Claim to the world that we are done
    _fAppsEnumed = TRUE;
    _fInEnumOp = FALSE;

    PostWorkerMessage(WORKERWIN_FIRE_DATASETCHANGED, 0, 0);// Tell Trident that dataset has changed
    return CWorkerThread::_ThreadStartProc();// Call our base class and do clean up.
}
...
STDMETHODIMP CDataSrc::getRWStatus(DBROWCOUNT iRow, DB_LORDINAL iCol, OSPRW *prwStatus)
{
    ...
    if ((_IsValidRow(iRow) || -1 == iRow) && (_IsValidCol(iCol) || -1 == iCol))
    {
        if (iRow == -1)
        {
