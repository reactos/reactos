...
void PutIconsInList(HWND hLB, LPWIZDATA lpwd)
{
    ...
    SetWindowPos(hLB, NULL, 0, 0, rc.right-rc.left, rc.bottom-rc.top, SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
...
BOOL_PTR CALLBACK PickIconDlgProc(HWND hDlg, UINT message , WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
        ...
        case WM_INITDIALOG:
            ...
            break;
        case WM_COMMAND:
            if ((GET_WM_COMMAND_ID(wParam, lParam) == IDC_ICONLIST) && ((GET_WM_COMMAND_CMD(wParam, lParam) == LBN_DBLCLK)))
            {
                PropSheet_PressButton(GetParent(hDlg), (lpwd->dwFlags & WDFLAG_SINGLEAPP) ? PSBTN_NEXT : PSBTN_FINISH);
            }
            break;
