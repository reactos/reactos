...
void FillInItem(TV_DISPINFO * ptvdi)
{
                      ...
                      SHGFI_ICON | SHGFI_DISPLAYNAME | SHGFI_SMALLICON))
    {
        ...
        ptvdi->item.pszText = fi.szDisplayName;

        DestroyIcon(fi.hIcon);

        ptvdi->item.mask = TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE;

        TreeView_SetItem(ptvdi->hdr.hwndFrom, &(ptvdi->item));
    }
}
...
void WaitForThreadDeath(HWND hDlg, HANDLE * phThread, DWORD dwMilliseconds)
{
    ...
    if (hThread)
    {
        *phThread = 0;
        ...
        while(TRUE)
        {
            DWORD result = MsgWaitForMultipleObjects(1, &hThread, FALSE, dwMilliseconds, QS_SENDMESSAGE);
            ...
            case WAIT_TIMEOUT:
                #ifdef DEBUG
                ShellMessageBox(g_hinst, hDlg, TEXT("Forced to terminate background thread"), 0, MB_OK | MB_ICONEXCLAMATION);
                #endif
        ...
        case WM_NOTIFY:
            ...
            switch(lpnm->code)
            {
                ...
                case TVN_KEYDOWN:
                    ...
                    {
                WaitForThreadDeath(hDlg, &(pfti->hThread), 5000);
