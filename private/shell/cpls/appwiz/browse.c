...
#ifdef UNICODE
        if (FAILED(pdesktop->lpVtbl->ParseDisplayName(pdesktop, lpwd->hwnd, NULL, lpwd->szExeName, &chEaten, &pidlStartBrowse, NULL)))
#else
        WCHAR szTmp[MAX_PATH];

        SHAnsiToUnicode(lpwd->szExeName, szTmp, MAX_PATH);

        if (FAILED(pdesktop->lpVtbl->ParseDisplayName(pdesktop, lpwd->hwnd, NULL, szTmp, &chEaten, &pidlStartBrowse, NULL)))
#endif
        {
...
BOOL_PTR CALLBACK BrowseDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NOTIFY:
        ...
        case PSN_SETACTIVE:
            ...
            break;
        case PSN_WIZNEXT:
            if (!NextPushed(lpwd) || ((lpwd->dwFlags & WDFLAG_SETUPWIZ) && !SetupCleanupExePath(lpwd)))
            {
                SetDlgMsgResult(hDlg, WM_NOTIFY, -1);
            }
            break;
        case PSN_WIZFINISH:
        {
            ...
            if (!SetupCleanupExePath(lpwd) || !ExecSetupProg(lpwd, ForceWx86, TRUE))
            {
                BrowseSetActive(lpwd);
                SetDlgMsgResult(hDlg, WM_NOTIFY, -1);
            }
...
BOOL_PTR CALLBACK SetupBrowseDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_NOTIFY:
        ...
        case PSN_WIZNEXT:
            ...
            bForceWx86 = FALSE;
#endif
            if (!NextPushed(lpwd) || !SetupCleanupExePath(lpwd) || !ExecSetupProg(lpwd, bForceWx86, FALSE))
            {
                SetDlgMsgResult(hDlg, WM_NOTIFY, -1);
            }
            break;
        case PSN_WIZFINISH:
        {
            ...
            if (!SetupCleanupExePath(lpwd) || !ExecSetupProg(lpwd, bForceWx86, TRUE))
            {
                BrowseSetActive(lpwd);
                SetDlgMsgResult(hDlg, WM_NOTIFY, -1);
            }
