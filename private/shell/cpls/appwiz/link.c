...
//  Cretates a link.
{
    ...
    if (lpwd->lpszOriginalName)
    {
        if (PathFileExists(szLinkName))
        {
            ...
            SHChangeNotify(SHCNE_DELETE, SHCNF_FLUSH | SHCNF_PATH, lpwd->lpszOriginalName, NULL);
        }
        else
        {
...
ExitNoFree:
    ...
    if (bWorked)
    {
        SHChangeNotify(SHCNE_UPDATEITEM, SHCNF_FLUSH | SHCNF_PATH, szLinkName, NULL);
    }
    SetCursor(hcurOld);
    ...
    {
        ShellMessageBox(g_hinst, lpwd->hwnd, MAKEINTRESOURCE(IDS_NOSHORTCUT), 0, MB_OK | MB_ICONEXCLAMATION);
    }
