...
void ValidateINIEntry(LPWIZDATA lpwd, LPTSTR lpszKeyName)
{
    ...
    if (GetPrivateProfileString(INST_SECTION, lpszKeyName, TEXT(""), szFileName, ARRAYSIZE(szFileName), lpwd->szIniFile))
    {
...
void AppList_InitListBox(HWND hDlg, LPPROPSHEETPAGE lpp)
{
    ...
    if (lpwd->szIniFile[0] != 0)
    {
        ...
        if (lpszKeys)
        {
            ...
            if (GetPrivateProfileString(INST_SECTION, NULL, NULL, lpszKeys, MAX_KEY_SIZE, lpwd->szIniFile))
            {
...
BOOL GetListSel(HWND hLB, int iCurSel, LPWIZDATA lpwd)
{
    ...
    if ((iCurSel != LB_ERR) && (ListBox_GetTextLen(hLB, iCurSel) <= ARRAYSIZE(szKeyName)))
    {
        ListBox_GetText(hLB, iCurSel, szKeyName);

        if (GetPrivateProfileString(INST_SECTION, szKeyName, TEXT(""), lpwd->szExeName, ARRAYSIZE(lpwd->szExeName), lpwd->szIniFile))
        {
