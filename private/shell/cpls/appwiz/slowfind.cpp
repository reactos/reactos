...
// Todo: Remember the find result somewhere in the registry or cache it in code so that we don't waste time repeatedly computing it.
...
protected:
    ...
    LPTSTR  _pszFolder;// The Result

    // Best match found
    int _iBest;
    int _iCurDepth;

    BOOL * _pfFound;// found it or not?
    BOOL _fRootSearch;// are we searching from root dirs like c:?
    TCHAR _szSystemDir[MAX_PATH];// system directory used by the root search
};
...
HRESULT CProgFilesAppFinder::EnterFolder(LPCWSTR pwszFolder, LPTREEWALKERSTATS ptws, WIN32_FIND_DATAW * pwfd)
{
...
#else
    WideCharToMultiByte(CP_ACP, 0, pwszFolder, -1, szFolder, ARRAYSIZE(szFolder), NULL, NULL);
#endif
    ...
    if (_fRootSearch)
    {
        if (!lstrcmpi(pszName, TEXT("Program Files")) || !lstrcmpi(pszName, TEXT("Windows")) ||
            !lstrcmpi(pszName, TEXT("Temp")) || !lstrcmpi(pszName, TEXT("Users")) || StrStrI(pszName, TEXT("WINNT")) || !lstrcmpi(_szSystemDir, szFolder))
            return S_FALSE;
    }
    else if (!lstrcmpi(pszName, TEXT("Common Files")) || !lstrcmpi(pszName, TEXT("Windows NT")) || !lstrcmpi(pszName, TEXT("Plus!")) || !lstrcmpi(pszName, TEXT("Uninstall Information")))
        return S_FALSE;
    ...
    if (pszName)
    {
        ...
        // The deeper the match folder is down the tree, the better a match it is.
...
// Internet Shortcuts (.URL) and Shell Shortcuts (.LNK) support so it should work generally
...
HRESULT GetShortcutTarget(LPCWSTR pszLinkPath, LPTSTR pszTargetPath, UINT cchTargetPath)
{
    ...
    if (SUCCEEDED(hresT))
    {
        ...
        if (SUCCEEDED(hresT))
        {
            ...
            if (hresT == S_OK)
            {
                hres = bDarwin ? S_FALSE : hresT;// Return S_FALSE for the darwin apps.
            }
...
HRESULT CStartMenuAppFinder::EnterFolder(LPCWSTR pwszFolder, LPTREEWALKERSTATS ptws, WIN32_FIND_DATAW * pwfd)
{
    ...
    if (FindSubWord(pszName, TEXT("Administrative")) || FindSubWord(pszName, TEXT("Accessories")))
    {
        return  S_FALSE;
    }
...
/*
Purpose: Checks if the given shortcut filename closely matches this app's fullname or shortname.
...
HRESULT CStartMenuAppFinder::FoundFile(LPCWSTR pwszFile, LPTREEWALKERSTATS ptws, WIN32_FIND_DATAW * pwfd)
{
    ...
    if (hresT == S_OK)
    {
        if (!PathIsRoot(szTargetFile) && !PathIsUnderWindows(szTargetFile) && !PathIsSetup(szTargetFile, 3) && !PathIsCommonFiles(szTargetFile))
        {
