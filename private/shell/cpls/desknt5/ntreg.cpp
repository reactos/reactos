    ...
    PDISPLAY_REGISTRY_HARDWARE_INFO pInfo)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        cb = 256;
        lRet = RegQueryValueExW(hkVideoRegR, pKeyNames[i], NULL, &dwType, NULL, &cb);
        if (lRet == ERROR_SUCCESS) {
            if (i == 0) {
                ...
                RegQueryValueExW(hkVideoRegR, pKeyNames[i], NULL, &dwType, (PUCHAR) (&mem), &cb);
                ...
                RegQueryValueExW(hkVideoRegR, pKeyNames[i], NULL, &dwType, (LPBYTE) pInfo, &cb);
            }
        }
        else
        {
            ...
            LoadString(hInstance, IDS_UNAVAILABLE, (LPWSTR)pInfo, 256);
        }
