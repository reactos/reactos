...
CDisplaySetup::DoRegistryUpgrade(void)
{
    ...
    dwSize = sizeof(DWORD);
    if (RegQueryValueEx(hKey, SZ_UPGRADE_FAILED_ALLOW_INSTALL, NULL, &dwType, (LPBYTE) &dwFailed, &dwSize) == ERROR_SUCCESS && dwFailed) {
    ...
    if (hdskDefault != NULL) {
        ...
        if (hdsk != NULL) {
            ...
            if (SetThreadDesktop(hdsk)) {
                ...
                Sleep(lpDesktopParam->dwTimeout);// Sleep for some seconds so you have time to look at the screen.
            }
        }
