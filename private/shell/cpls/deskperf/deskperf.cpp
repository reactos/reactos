        ...
        else
        {
            ...
            if (bLocalConsole)
            {
                ...
                if (bSuccess)
                {
                    ...
                    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, gszRegistryPath, 0, KEY_ALL_ACCESS, &hKeyAccelLevel) != ERROR_SUCCESS)
                    {
                        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, gszRegistryPath, 0, KEY_READ, &hKeyAccelLevel) != ERROR_SUCCESS)
                        {
                            hKeyAccelLevel = NULL;
                        }
                        else
                    ...
                    if (hKeyAccelLevel)
                    {
                        ...
                        if (RegQueryValueEx(hKeyAccelLevel, SZ_ACCELLEVEL, NULL,NULL, (LPBYTE) &AccelLevel, &cb) == ERROR_SUCCESS)
                        {
                ...
                if (bSuccess)
                {
                    ...
                    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, gszRegistryPath, 0, KEY_ALL_ACCESS, &hKeyAccelLevel) == ERROR_SUCCESS)
                    {
                        if (AccelLevel == ACCELERATION_FULL)
                        {
                            ...
                            if (RegDeleteValue(hKeyAccelLevel, SZ_ACCELLEVEL) == ERROR_SUCCESS)
                            {
                                bSuccess = TRUE;
                            }
                        }
