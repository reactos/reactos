...
// calculate all of the rectangles based on the given window rect
{
    ...
    RCZ(ELEMENT_ACTIVECAPTION).bottom = RCZ(ELEMENT_ACTIVECAPTION).top + cyCaption + cyBorder;
    ...
    RCZ(ELEMENT_ACTIVESYSBUT1).left = RCZ(ELEMENT_ACTIVESYSBUT1).right - (cyCaption - cxEdge);

    // min/max buttons
    RCZ(ELEMENT_ACTIVESYSBUT2) = RCZ(ELEMENT_ACTIVESYSBUT1);
    RCZ(ELEMENT_ACTIVESYSBUT2).right = RCZ(ELEMENT_ACTIVESYSBUT1).left - cxEdge;
    RCZ(ELEMENT_ACTIVESYSBUT2).left = RCZ(ELEMENT_ACTIVESYSBUT2).right - 2 * (cyCaption - cxEdge);
    ...
    RCZ(ELEMENT_MENUDISABLED).right = RCZ(ELEMENT_MENUSELECTED).left = RCZ(ELEMENT_MENUDISABLED).left + cxDisabled;
...
void NEAR PASCAL LookPrev_Draw(HWND hWnd, HDC hdc)
{
    ...
    hicon = LoadImage(NULL, IDI_APPLICATION, IMAGE_ICON, g_sizes[SIZE_SMCAPTION].CurSize - 2*cxBorder, g_sizes[SIZE_SMCAPTION].CurSize - 2*cyBorder, 0);
    ...
    TextOut(hdc, RCZ(ELEMENT_MSGBOX).left + 3*cxEdge, RCZ(ELEMENT_MSGBOXCAPTION).bottom + cyEdge, g_szMsgBoxText, lstrlen(g_szMsgBoxText));
    if (hfontOld)
...
// ?????? what font should this use ??????
    ...
    DrawText(hdc, g_szButton, -1, &rcT, DT_CENTER | DT_NOPREFIX | DT_SINGLELINE | DT_VCENTER);
