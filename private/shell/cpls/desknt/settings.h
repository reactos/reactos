...
BOOL CALLBACK DisplayDlgProc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
int FmtMessageBox(HWND hwnd, UINT fuStyle, BOOL fSound, DWORD dwTitleID, DWORD dwTextID, ... );
...
DWORD InstallNewDriver(HWND    hwnd, LPCTSTR pszModel, BOOL    bDetect, PBOOL   pbKeepEnabled);
DWORD PreInstallDriver(HWND    hwnd, LPCTSTR pszModel, LPCTSTR pszInf, LPTSTR  DriverName);
DWORD InstallGraphicsDriver(HWND    hwnd, LPCWSTR pszSourceDirectory, LPCWSTR pszModel, LPCWSTR pszInf);
