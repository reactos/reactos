...
void NEAR PASCAL SaveIni(HWND hDlg )
{
    ...
    if(wNumMethods)
    {
        ...
        if(wTemp)
        {
            ...
            wMethod = SendDlgItemMessage(hDlg,IDC_CHOICES,CB_GETITEMDATA, wTemp,0l);
    ...
    for (Counter = 0; Counter < (sizeof(g_TimeoutAssociation) /
        sizeof(TIMEOUT_ASSOCIATION)); Counter++) {
        SystemParametersInfo(g_TimeoutAssociation[Counter].taSetTimeoutAction, (UINT) (g_Timeout[Counter] * 60), NULL, SPIF_UPDATEINIFILE);
        if (Counter != TA_SCREENSAVE) {
            SystemParametersInfo(g_TimeoutAssociation[Counter].taSetActiveAction, IsDlgButtonChecked(hDlg, g_TimeoutAssociation[Counter].taBaseControlID + BCI_SWITCH), NULL, SPIF_UPDATEINIFILE);
        }
    }
    ...
    SendNotifyMessage(HWND_BROADCAST, WM_WININICHANGE, 0, (LONG)g_szWindows);/* Broadcast a WM_WININICHANGE message... */
}
