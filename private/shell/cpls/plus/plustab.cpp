...
GUID g_CLSID_CplExt = { 0x41e300e0, 0x78b6, 0x11ce, { 0x84, 0x9b, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00} };
...
BOOL CALLBACK PropertySheeDlgProc( HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( uMessage )
    {
        case WM_INITDIALOG:
        {
            ...
            // Full Color Icons
            bRet = GetRegValueInt( HKEY_CURRENT_USER, c_szHICKey, c_szHICVal, &iOldHIC);
            ...
            iNewHIC = iOldHIC;
            SendMessage( (HWND)GetDlgItem( hDlg, IDC_ICONHIGHCOLOR ), BM_SETCHECK, (WPARAM)(BOOL)(iOldHIC == 16), 0);
            ...
            SendMessage( (HWND)GetDlgItem( hDlg, IDC_MENUANIMATION ), BM_SETCHECK, (WPARAM)wOldMA, 0);
            ...
            EnableWindow( GetDlgItem( hDlg, IDC_COMBOEFFECT ), (UINT)wOldMA);
            ...
            SendMessage( (HWND)GetDlgItem( hDlg, IDC_FONTSMOOTH ), BM_SETCHECK, (WPARAM)bOldSF, 0);
            bNewSF = bOldSF;
            ...
            SendMessage( (HWND)GetDlgItem( hDlg, IDC_SHOWDRAG ), BM_SETCHECK, (WPARAM)bOldDW, 0);
            bNewDW = bOldDW;

            uOldKI = FALSE;
            SystemParametersInfo( SPI_GETKEYBOARDINDICATORS, 0, (PVOID)&uOldKI, 0 );
            SendMessage( (HWND)GetDlgItem( hDlg, IDC_KEYBOARDINDICATORS ), BM_SETCHECK, (WPARAM)(uOldKI ? BST_UNCHECKED : BST_CHECKED), 0);
            uNewKI = uOldKI;
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
                ...
                case IDC_CHANGEICON:
                {
                    ...
                    ExpandEnvironmentStrings( sIconData[iIndex].szOldFile, szExp, ARRAYSIZE(szExp));
                    ...
                    if ( PickIconDlg( hDlg, (LPTSTR)szTemp, ARRAYSIZE(szTemp), &i) == TRUE)
                    {
                        ...
                        if (g_RunningOnNT)
                        {
                            SHUnicodeToTChar(szTemp, sIconData[iIndex].szNewFile, ARRAYSIZE(sIconData[iIndex].szNewFile));
                        }
                        else
                        {
                            SHAnsiToTChar((LPSTR)szTemp, sIconData[iIndex].szNewFile, ARRAYSIZE(sIconData[iIndex].szNewFile));
                        }
                ...
                case IDC_KEYBOARDINDICATORS:
                    uNewKI = ((SendMessage((HWND)lParam, BM_GETCHECK, 0, 0) == BST_CHECKED)? FALSE : TRUE);
                    bDorked = TRUE;
                    break;
                default:
                    break;
            ...
            switch( ((NMHDR *)lParam)->code )
            {
                ...
                case NM_CLICK:
                    switch (wParam)
                    {
                        case IDC_SHOWME:
                            ...
                            if (DisplayFontSmoothingDetails(&dwSmoothingSetting) && dwNewSFT != dwSmoothingSetting)
                            {
                ...
                case PSN_APPLY: // OK or Apply clicked
                {
                    ...
                    if( iOldHIC != iNewHIC )
                    {
                        ...
                        bRet = SetRegValueInt( HKEY_CURRENT_USER, c_szHICKey, c_szHICVal, iNewHIC);
                        iOldHIC = iNewHIC;
                    ...
                    {
                        bOldDW = bNewDW;
                        SystemParametersInfo( SPI_SETDRAGFULLWINDOWS, bNewDW, 0, SPIF_UPDATEINIFILE);
                    ...
                    if ( bOldSF != bNewSF || dwOldSFT != dwNewSFT)
                    {
                        ...
                        SystemParametersInfo( SPI_SETFONTCLEARTYPE, bST, 0, SPIF_UPDATEINIFILE);
#endif
                        dwOldSFT = dwNewSFT;
                        bOldSF = bNewSF;
                        SystemParametersInfo( SPI_SETFONTSMOOTHING, bNewSF, 0, SPIF_UPDATEINIFILE);
                    }
                    ...
                    for( iX = 0;iX < NUM_ICONS;iX++ )
                    {
                        if( (lstrcmpi( sIconData[iX].szNewFile, sIconData[iX].szOldFile ) != 0) || (sIconData[iX].iNewIndex != sIconData[iX].iOldIndex))
                        {
                            ...
                            wnsprintf( szTemp, ARRAYSIZE(szTemp), TEXT("%s,%d"), sIconData[iX].szNewFile, sIconData[iX].iNewIndex);
                            bRet = IconSetRegValueString( sIconRegKeys[iX].szIconSubKey, sIconRegKeys[iX].szIconValue, (LPTSTR)szTemp);
                    ...
                    if ( uOldKI != uNewKI )
                    {
                        ...
                        if (!uNewKI)
                        {
                            ...
                            SystemParametersInfo(SPI_SETKEYBOARDINDICATORS, 0, (PVOID)uNewKI, SPIF_UPDATEINIFILE);
                        ...
                        else
                        {
                            ...
                            SendMessageTimeout(HWND_BROADCAST, WM_CHANGEUISTATE, MAKEWPARAM(UIS_CLEAR, UISF_HIDEFOCUS | UISF_HIDEACCEL), 0, SMTO_ABORTIFHUNG, 10*1000, &dwResult);
                            SystemParametersInfo(SPI_SETKEYBOARDINDICATORS, 0, (PVOID)uNewKI, SPIF_UPDATEINIFILE);
                        }
                    }
        ...
        case WM_HELP:
        {
            ...
            {
                WinHelp( (HWND)lphi->hItemHandle, (LPTSTR)szHelpFile, HELP_WM_HELP, (DWORD_PTR)((POPUP_HELP_ARRAY FAR *)g_phaHelp));
            }
        }
            break;
        case WM_CONTEXTMENU:
            ...
            else
            {
                ...
                WinHelp( (HWND)wParam, (LPTSTR)szHelpFile, HELP_CONTEXTMENU, (DWORD_PTR)((POPUP_HELP_ARRAY FAR *)g_phaHelp));
            }
            break;
        default:
            return FALSE;
...
HWND CreateListView( HWND hWndParent )
{
    ...
    for( iX = 0; iX < NUM_ICONS; iX++ )
    {
        ...
        bRet = IconGetRegValueString( sIconRegKeys[iX].szIconSubKey, sIconRegKeys[iX].szIconValue, szTemp, MAX_PATH);
    ...
    for( iX = 0; iX < NUM_ICONS; iX++ )
    {
        ...
        bRet = IconGetRegValueString( sIconRegKeys[iX].szTitleSubKey, NULL, (LPTSTR)szTemp, MAX_PATH);
...
HRESULT ExtractPlusColorIcon( LPCTSTR szPath, int nIndex, HICON *phIcon, UINT uSizeLarge, UINT uSizeSmall)
{
    ...
    *phIcon = NULL;
    if (SUCCEEDED(hres = CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER, IID_IShellLink, (void**)&psl)))
    {
...
WPARAM GetAnimations(DWORD *pdwEffect)
{
    ...
    fSmooth = (BOOL)GetRegValueDword( HKEY_CURRENT_USER, (LPTSTR)c_szSSKey, (LPTSTR)c_szSSVal);
