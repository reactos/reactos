...
STDAPI_(BOOL) LaunchInternetControlPanelAtPage(HWND hDlg, UINT nStartPage)
{
    ...
    if (AddInternetPropertySheets(&_AddPropSheetPage, (LPARAM)&psHeader, NULL, NULL) == S_OK)
    {
        // add any extra pages from hooks in the registry
        if( ( hpsxa = SHCreatePropSheetExtArray( HKEY_LOCAL_MACHINE, REGSTR_PATH_INETCPL_PS_EXTENTIONS, MAX_NUM_OPTION_PAGES - NUM_OPTION_PAGES ) ) != NULL )
        {
...
BOOL IsCompatModeProcess()
{
    ...
    {
        // If we are running in compat mode, exit because we don't want users mucking with control panel from here.
