...
Abstract:
    This file contains code to set cache config information from the internet control panel
...
*/
BOOL GetDiskInfo(PTSTR pszPath, PDWORD pdwClusterSize, PDWORDLONG pdlAvail, PDWORDLONG pdlTotal)
{
...
// SHBrowseForFolder should NOT just return a machine name... BUG is shell code.
...
int CALLBACK MoveFolderCallBack(HWND hwnd, UINT uMsg, LPARAM lParam, LPARAM lpData)
{
    if (uMsg==BFFM_SELCHANGED)
    {
        ...
        if (SHGetPathFromIDList((LPCITEMIDLIST)lParam, szNewDest))
        {
            ...
            else
            {
                ...
                BOOL fProblems;
                WideCharToMultiByte(CP_ACP, NULL, szNewDest, -1, szAnsiPath, ARRAYSIZE(szAnsiPath), NULL, &fProblems);
                if (fProblems)
                {
...
// Handles the moving of the Temporary Files (Cache) Folder to another location.
// It checks for the existence of the new folder.
// It warns the user that a REBOOT is necessary before changes are made.
...
BOOL TemporaryInit(HWND hDlg)
{
    ...
    if (GetUrlCacheConfigInfoA(&icci, NULL, CACHE_CONFIG_QUOTA_FC | CACHE_CONFIG_DISK_CACHE_PATHS_FC | CACHE_CONFIG_SYNC_MODE_FC))
    {
...
void TemporaryOnCommand(LPTEMPDLG pTmp, UINT id, UINT nCmd)
{
    switch (id) {
        ...
        case IDC_ADVANCED_DOWNLOADED_CONTROLS:
        {
            ...
            if (SHGetValue(HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings"), TEXT("ActiveXCache"), NULL, szPath, &cb) == ERROR_SUCCESS)
            {
                SHELLEXECUTEINFO ei;
#ifdef UNIX
...
//                     sheet).
INT_PTR CALLBACK TemporaryDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        return TRUE;
    case WM_HELP:                   // F1
        ResWinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, IDS_HELPFILE, HELP_WM_HELP, (DWORD_PTR)(LPSTR)mapIDCsToIDHs);
    break;
