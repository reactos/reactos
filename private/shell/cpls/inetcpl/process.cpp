...
HRESULT CProcessInfo::MakeRoomForInfoArray(int n)
{
    ...
    if (n > _nAlloced)
    {
        ...
        {
            p = (PROCESSINFO *)LocalReAlloc(_pProcInfoArray, sizeof(PROCESSINFO)*_nAlloced, LMEM_MOVEABLE|LMEM_ZEROINIT);
        }
...
HRESULT CProcessInfo::NTInitPsapi()
{
    ...
    if (_hPsapiDLL)
    {
        _lpfnEnumProcesses = (LPFNENUMPROCESSES)GetProcAddress(_hPsapiDLL, "EnumProcesses");
...
HRESULT CProcessInfo::NTFillProcessList(DWORD dwProcessID, int iIndex)
{
    ...
    if (hr == S_OK)
    {
        ...
        _tcsncpy (_pProcInfoArray[iIndex].szExeName, szProcessName, ARRAYSIZE(_pProcInfoArray[iIndex].szExeName));
    }
    return hr;
}
...
// Win95 specific, sets up the things we need to get the process IDs.
{
    ...
    // Obtain a module handle to KERNEL so that we can get the addresses of the 32-bit Toolhelp functions we need.
    hKernel = GetModuleHandle(TEXT("KERNEL32.DLL"));
    if (hKernel)
    {
        _lpfnCreateToolhelp32Snapshot = (CREATESNAPSHOT)GetProcAddress(hKernel, "CreateToolhelp32Snapshot");
        ...
        // must fail because we won't be able to walk one of the lists we need to.
...
// Fills in the array of process info, and also set the count of the items
{
    ...
    if (_lpfnProcess32First(hProcessSnap, &pe32))
    {
        ...
        do // Add PID and associated .EXE file info to list...
        {
            ...
            if (pszFile)
            {
                SHAnsiToUnicode( pszFile, _pProcInfoArray[iIndex].szExeName, ARRAYSIZE(_pProcInfoArray[iIndex].szExeName));
            }
            iIndex++;
