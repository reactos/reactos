...
VOID ShowJavaZonePermissionsDialog (HWND hdlg, LPCUSTOMSETTINGSINFO pcsi)
{
    ...
    hr = CoCreateInstance(CLSID_JavaRuntimeConfiguration,
            NULL,
...
BOOL SecurityCustomSettingsInitDialog(HWND hDlg, LPARAM lParam)
{
    ...
    CoInitialize(0);
    hr = CoCreateInstance(CLSID_CRegTreeOptions, NULL, CLSCTX_INPROC_SERVER, IID_IRegTreeOptions, (LPVOID *)&(pcsi->pTO));
    DWORD cb = SIZEOF(pcsi->fUseHKLM);
    SHGetValue(HKEY_LOCAL_MACHINE, REGSTR_PATH_SECURITY_LOCKOUT, REGSTR_VAL_HKLM_ONLY, NULL, &(pcsi->fUseHKLM), &cb);
