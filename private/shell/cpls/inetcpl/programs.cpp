...
BOOL ProgramsDlgInit( HWND hDlg)
{
    ...
    if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_MAILCLIENTS, 0, NULL, 0, KEY_READ, NULL, &hkey, &dw) == ERROR_SUCCESS)
#else
    pPrg->hwndMail = GetDlgItem(pPrg->hDlg, IDC_EDIT_PROGRAMS_MAIL);
    if (RegCreateKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_MAILCLIENTS, 0, NULL, 0, KEY_READ, NULL, &hkey, &dw) == ERROR_SUCCESS)
#endif
    {
    ...
    if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_NEWSCLIENTS, 0, NULL, 0, KEY_READ, NULL, &hkey, &dw) == ERROR_SUCCESS)
#else
    pPrg->hwndNews = GetDlgItem(pPrg->hDlg, IDC_EDIT_PROGRAMS_NEWS);

    if (RegCreateKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_NEWSCLIENTS, 0, NULL, 0, KEY_READ, NULL, &hkey, &dw) == ERROR_SUCCESS)
#endif
    {
        // populate the combobox
...
void RegCopyKey(HKEY hkeyRoot, const TCHAR *pszDest, HKEY hkeySrc)
{
    ...
    if (RegCreateKeyEx(hkeyRoot, pszDest, NULL, 0, NULL, KEY_READ|KEY_WRITE, NULL, &hkeyDest, &dw) == ERROR_SUCCESS)
    {
        ...
        RegCloseKey(hkeyDest);// close the key
    }   // if RegCreateKey()
} // RegCopyKey()
...
void CopyInfoTo(const TCHAR *pszKeyName, HKEY hkeyClient)
{
    ...
    if (RegOpenKeyEx(hkeyClient, szName, NULL, KEY_READ|KEY_WRITE, &hkey) ==ERROR_SUCCESS)
    {
        ...
        RegCopyKey(HKEY_CLASSES_ROOT, pszKeyName, hkey);// recreate key and copy the protocol info
        RegCloseKey(hkey);// close the key
    }   // if RegOpenKey()
}   // CopyInfoTo()
        ...
        if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, szPath, 0, NULL, 0, KEY_READ|KEY_WRITE, NULL, &hkey, &dw) == ERROR_SUCCESS)
        {
                ...
                i++)                    // get next entry
            {
                ...
                if (RegOpenKeyEx(hkey, szKeyName, 0, KEY_READ, &hkeyClient)==ERROR_SUCCESS)
                {
                    ...
                    RegCloseKey(hkeyClient);// close key
                }   // if RegOpenKey()
            }   // for
