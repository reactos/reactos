...
CAssoc::Save()
{
    ...
    LONG lResult = RegOpenKeyEx(HKEY_CLASSES_ROOT, m_type, 0, KEY_QUERY_VALUE|KEY_WRITE, &hKey1);
    if (lResult != ERROR_SUCCESS)
    {
        lResult = RegCreateKey(HKEY_CLASSES_ROOT, m_type, &hKey1);
    }
    ...
    if (lResult == ERROR_SUCCESS)
    {
        ...
        lResult = RegCreateKey(hKey1, TEXT("defaulticon"), &hKey2);
        if(lResult == ERROR_SUCCESS)
        {
            DWORD dwType = REG_SZ;
        ...
        lResult = RegOpenKeyEx(hKey1, TEXT("shell"), 0, KEY_QUERY_VALUE| KEY_WRITE, &hKey2);
        if(lResult != ERROR_SUCCESS)
        {
            lResult = RegCreateKey(hKey1, TEXT("shell"), &hKey2);
        }

        if(lResult == ERROR_SUCCESS)
        {
            lResult = RegOpenKeyEx(hKey2, TEXT("open"), 0, KEY_QUERY_VALUE| KEY_WRITE, &hKey3);
            if( lResult != ERROR_SUCCESS )
            {
                lResult = RegCreateKey(hKey2, TEXT("open"), &hKey3);
            }

            if( lResult == ERROR_SUCCESS )
            {
                lResult = RegOpenKeyEx(hKey3, TEXT("command"), 0, KEY_QUERY_VALUE| KEY_WRITE, &hKey4);
                if( lResult != ERROR_SUCCESS )
                {
                    lResult = RegCreateKey(hKey3, TEXT("command"), &hKey4);
                }
    ...
    if(mimeKey && m_mime)
    {
        ...
        lResult = RegOpenKeyEx(HKEY_CLASSES_ROOT, mimeKey, 0, KEY_QUERY_VALUE| KEY_WRITE, &hKey1);
        if(lResult != ERROR_SUCCESS)
        {
            lResult = RegCreateKey(HKEY_CLASSES_ROOT, mimeKey, &hKey1);
            if(lResult == ERROR_SUCCESS && m_exts)
            {
                ...
                if(count > 0 )
                {
                    ...
                    RegSetValueEx( hKey1, TEXT("extension"), NULL, REG_SZ, (LPBYTE)firstExt, (lstrlen(firstExt)+1)*sizeof(TCHAR) );
                }
    ...
    if( m_exts )
    {
        ...
        for( int i=0; i<count; i++ )
        {
            ...
            if( ptr && *ptr == TEXT('.') )
            {
                lResult = RegOpenKeyEx(HKEY_CLASSES_ROOT, ptr, 0, KEY_QUERY_VALUE| KEY_WRITE, &hKey1);
                if( lResult != ERROR_SUCCESS )
                {
                    lResult = RegCreateKey(HKEY_CLASSES_ROOT, ptr, &hKey1);
                }
