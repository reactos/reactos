...
VOID InitGeneralDlg(HWND hDlg)
{
    ...
    Status = RegOpenKey(HKEY_LOCAL_MACHINE, c_szMemoryManagement, &hkey);
    if (ERROR_SUCCESS == Status) {
        DWORD paeEnabled;

        Status = RegQueryValueEx(hkey, c_szPhysicalAddressExtension, NULL, NULL, (LPBYTE)&paeEnabled, &cbData);
        if (ERROR_SUCCESS == Status && sizeof(paeEnabled) == cbData && 0 != paeEnabled) {
    ...
    switch( uMsg ) {
        ...
        case WM_INITDIALOG:
        {
            ...
            for( ;; i++ )
            {
                ...
                GetPrivateProfileString( c_szOemSupportSection, line, c_szDefSupportLineText, text, sizeof( text ) - 2, oemfile );
