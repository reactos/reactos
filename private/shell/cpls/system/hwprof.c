...
INT_PTR APIENTRY RenameProfileDlg(HWND    hDlg, UINT    uMessage, WPARAM  wParam, LPARAM  lParam)
{
   PHWPROFILE  pHwProfile;
   ULONG       ulReturn;
   static HIMC himcOrg;
...

{
   ...
   if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, pszRegIDConfigDB, 0, sam, &hKey) == ERROR_SUCCESS) {
       if (pInfo->bHwDetectedPortable) {
           ...
           ulSize = sizeof(DWORD);
           if (RegQueryValueEx(hKey, pszRegIsPortable, NULL, NULL, (LPBYTE)&pInfo->bPortable, &ulSize) != ERROR_SUCCESS) {
      ...
      )
{
   ...
   if(RegOpenKeyEx(
       ...
       if (RegOpenKeyEx(hKey, pszRegKnownDockingStates, 0, KEY_SET_VALUE, &hHwProf) == ERROR_SUCCESS) {
           if (pInfo->ulUndockedProfileNameCount > 0) {
               RegSetValueEx(hHwProf, pszRegUndocked, 0, REG_DWORD, (LPBYTE)&pInfo->ulUndockedProfileNameCount, sizeof(ULONG));
           } else {
               RegDeleteValue(hHwProf, pszRegUndocked);
           }

           if (pInfo->ulDockedProfileNameCount > 0) {
               RegSetValueEx(hHwProf, pszRegDocked, 0, REG_DWORD, (LPBYTE)&pInfo->ulDockedProfileNameCount, sizeof(ULONG));
           } else {
               RegDeleteValue(hHwProf, pszRegDocked);
           }

           if (pInfo->ulUnknownProfileNameCount > 0) {
               RegSetValueEx(hHwProf, pszRegUnknown, 0, REG_DWORD, (LPBYTE)&pInfo->ulUnknownProfileNameCount, sizeof(ULONG));
           } else {
   ...
   )
{
   ...
   if (pProfValues->ulAction & HWP_CREATE) {
      ...
      if ((UuidCreate(&NewGuid) == RPC_S_OK) && (UuidToString(&NewGuid, &UuidString) == RPC_S_OK)) {
          ...
          RegSetValueEx(hKey, pszRegHwProfileGuid, 0, REG_SZ, (LPBYTE)szGuid, (lstrlen(szGuid)+1) * sizeof(TCHAR) );
      }
   } else {
               ...
               if (RegOpenKey(hAliasRoot, szString, &hSrcKey) == ERROR_SUCCESS) {
                   ...
                   if (RegQueryValueEx(
                       ...
                       if (ulSrcProfile == ulAliasProfileNumber) {
                           ...
                           wsprintf(RegStr, TEXT("%s\\%s\\%04u"), pszRegIDConfigDB, szSubKeyName, ulNewAlias);
                           RegCreateKeyEx(HKEY_LOCAL_MACHINE, RegStr, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hDestKey, NULL);


                           // copy this alias entry
                           CopyRegistryNode(hSrcKey, hDestKey);
                           RegSetValueEx(hDestKey, pszRegProfileNumber, 0, REG_DWORD, (LPBYTE)&ulDestProfile, sizeof(ULONG));
...
BOOL UpdateOrderButtonState(HWND  hDlg)
{
   ...
   if (ulIndex == 0) {
      // if focus currently on the button we're about to disable, change focus first or focus will be lost.
    ...
    )
{
    ...
    if (RegQueryValueEx(hKey, pszRegPropertyProviders, NULL, NULL, NULL, &ulSize) == ERROR_SUCCESS) {
        pszProviderList = LocalAlloc(LPTR, ulSize);
        if (pszProviderList != NULL) {
            // read list of providers
            if (RegQueryValueEx(hKey, pszRegPropertyProviders, NULL, NULL, (LPBYTE)pszProviderList, &ulSize) == ERROR_SUCCESS) {
                // Ask each provider to create and register it's property page
                for (pszProvider = pszProviderList; *pszProvider; pszProvider += lstrlen(pszProvider) + 1) {
