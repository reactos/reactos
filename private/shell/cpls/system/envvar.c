...
void SetVar(IN HWND hDlg, IN UINT VarType, IN LPCTSTR szVarName, IN LPCTSTR szVarValue);
void DeleteVar(IN HWND hDlg, IN UINT VarType, IN LPCTSTR szVarName);

// New.../Edit... subdialog functions
INT_PTR APIENTRY EnvVarsEditDlg(IN HWND hDlg, IN UINT uMsg, IN WPARAM wParam, IN LPARAM lParam);

void EVDoEdit(IN HWND hWnd, IN UINT VarType, IN UINT EditType, IN int  iSelection);
BOOL ExpandSystemVar( IN LPCTSTR pszSrc, OUT LPTSTR pszDst, IN DWORD cchDst );
...
BOOL InitEnvVarsDlg(IN HWND hDlg)
/*++
Routine Description:
...
--*/
{
    ...
    SendDlgItemMessage (hDlg, IDC_ENVVAR_SYS_LB_SYSVARS, LVM_INSERTCOLUMN, 0, (LPARAM) &col);
    SendDlgItemMessage (hDlg, IDC_ENVVAR_SYS_LB_USERVARS, LVM_INSERTCOLUMN, 0, (LPARAM) &col);
    ...
    SendDlgItemMessage (hDlg, IDC_ENVVAR_SYS_LB_SYSVARS, LVM_INSERTCOLUMN, 1, (LPARAM) &col);

    SendDlgItemMessage (hDlg, IDC_ENVVAR_SYS_LB_USERVARS, LVM_INSERTCOLUMN, 1, (LPARAM) &col);
...
SysLoop:
    ...
    if (GetUserName(szUserName, &dwSize) && LoadString (hInstance, IDS_USERENVVARS, szBuffer1, 200)) {
...
UserLoop:
    ...
    SendDlgItemMessage (hDlg, IDC_ENVVAR_SYS_LB_USERVARS, LVM_SETITEMSTATE, 0, (LPARAM) &item);
    SendDlgItemMessage (hDlg, IDC_ENVVAR_SYS_LB_SYSVARS, LVM_SETITEMSTATE, 0, (LPARAM) &item);
    ...
    EnableWindow(GetDlgItem(hDlg, IDC_ENVVAR_SYS_NEWSV), bEditSystemVars);
    EnableWindow(GetDlgItem(hDlg, IDC_ENVVAR_SYS_EDITSV), bEditSystemVars);
    EnableWindow(GetDlgItem(hDlg, IDC_ENVVAR_SYS_DELSV), bEditSystemVars);
...
INT_PTR APIENTRY EnvVarsDlgProc(IN HWND hDlg, IN UINT uMsg, IN WPARAM wParam, IN LPARAM lParam)
/*++
Routine Description:
...
void EVDoCommand(IN HWND hDlg, IN HWND hwndCtl, IN int idCtl, IN int iNotify )
/*++
Routine Description:
...
--*/
{
    ...
    switch (idCtl) {
        ...
        case IDC_ENVVAR_SYS_EDITSV:
            EVDoEdit(hDlg, SYSTEM_VAR, EDIT_VAR, GetSelectedItem(GetDlgItem(hDlg, IDC_ENVVAR_SYS_LB_SYSVARS)));
            SetFocus(GetDlgItem(hDlg, IDC_ENVVAR_SYS_LB_SYSVARS));
            break;
        case IDC_ENVVAR_SYS_EDITUV:
            EVDoEdit(hDlg, USER_VAR, EDIT_VAR, GetSelectedItem(GetDlgItem(hDlg, IDC_ENVVAR_SYS_LB_USERVARS)));
...
--*/
{
    ...
    if (n != -1)
    {
        ...
        SendDlgItemMessage (hDlg, i, LVM_SETITEMSTATE, n, (LPARAM) &item);
    }
}
            ...
            *p = TEXT('\0');
            ...
            if (*pszSrc == TEXT('\0')) {
                ...
                *pszDst++ = TEXT('%');
                ...
                pszSrc++;// continue with next char after ending '%'
            }
        }
    } while( ch != TEXT('\0') );
