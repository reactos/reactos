...
SaveOsInfo(HKEY hKey)
{
    ...
    cb = sizeof(DWORD);
    RegSetValueEx(hKey, SZ_UPGRADE_FROM_PLATFORM, 0, REG_DWORD, (PBYTE) &os.dwPlatformId, cb);

    cb = sizeof(DWORD);
    RegSetValueEx(hKey, SZ_UPGRADE_FROM_MAJOR_VERSION, 0, REG_DWORD, (PBYTE) &os.dwMajorVersion, cb);

    cb = sizeof(DWORD);
    RegSetValueEx(hKey, SZ_UPGRADE_FROM_MINOR_VERSION, 0, REG_DWORD, (PBYTE) &os.dwMinorVersion, cb);

    cb = sizeof(DWORD);
    RegSetValueEx(hKey, SZ_UPGRADE_FROM_BUILD_NUMBER, 0, REG_DWORD, (PBYTE) &os.dwBuildNumber, cb);

    cb = lstrlen(os.szCSDVersion);
    RegSetValueEx(hKey, SZ_UPGRADE_FROM_VERSION_DESC, 0, REG_SZ, (PBYTE) os.szCSDVersion, cb);
}
...
SaveLegacyDriver(HKEY hKey)
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, SZ_VIDEOMAP, 0, KEY_READ, &hKeyMap) !=  ERROR_SUCCESS) {
        return;
    }
    ...
    for (cbValue = (sizeof(szValueName) - 1) / sizeof(TCHAR), cbData = sizeof(szData) / sizeof(TCHAR) ;
        ...
        for (pszEnd = szPath + lstrlen(szPath); pszEnd != szPath && *pszEnd != TEXT('\\'); pszEnd--) {
            ; /* nothing */
        }
        ...
        *pszEnd = TEXT('\0');
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szPath, 0, KEY_READ, &hKeyDriver) ==  ERROR_SUCCESS) {
            ...
            if (RegQueryValueEx(hKeyDriver, TEXT("ImagePath"), NULL, NULL, (LPBYTE) szValueName, &cbValue) == ERROR_SUCCESS) {
                ...
                while(pszDriverEnd != pszDriver && *pszDriverEnd != TEXT('.')) {
                    pszDriverEnd--;
                }
                ...
                *pszDriverEnd = UNICODE_NULL;
                ...
                if (pszDriverEnd > pszDriver) {
                    ...
                    RegSetValueEx(hKey, szValueName, 0, REG_SZ, (PBYTE) pszDriverEnd, cbValue);
                }
            }
        ...
        RegSetValueEx(hKey, szValueName, 0, REG_SZ, (PBYTE) pszEnd, cbValue);
    }

    cbValue = sizeof(DWORD);
    RegSetValueEx(hKey, TEXT("NumDrivers"), 0, REG_DWORD, (PBYTE) &num, cbValue);
    RegCloseKey(hKeyMap);
}
    ...
    )
{
    ...
    if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, SZ_UPDATE_SETTINGS, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &disposition) != ERROR_SUCCESS) {
