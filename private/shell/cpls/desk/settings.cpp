...
{
    if ((cfFormat == g_cfDisplayDevice) || (cfFormat == g_cfDisplayName) || (cfFormat == g_cfMonitorDevice) || (cfFormat == g_cfMonitorName))
        return TRUE;
    return FALSE;
}
...
PMONPARAM CDeviceSettings::GetMonitorParams()
{
    ...
    if (pMonParams)
    {
        ...
        _iRate = pMonParams->iCurRate;// Remember current selection
    }
...
void CDeviceSettings::UpdateRefreshRate(PMONPARAM pMonParams)
{
    ...
    if (_iRate != pMonParams->iCurRate) {
        ...
        SetCurFrequency(iFreq);// Set the new frequency
    }
...
BOOL CDeviceSettings::RefreshSettings(BOOL bComplete)
{
    ...
    // in VGA fallback or SafeMode EnumDisplaySettings will fail deal with this case
    ...
    if (pcdev == NULL)
    {
        ...
        iResOk = _pcdmlModes->IndexFromResXY(gUnattenedXResolution, gUnattenedYResolution);
...
//  Writes the new display parameters to the proper place in the registry.
int CDeviceSettings::SaveSettings(DWORD dwSet)
{
    ...
    PCDEVMODE pcdev;
    PDEVMODE  pdevmode;
    pcdev = _pcdmlModes->LookUp(_iResolution, _iColor, _iFrequency);
    ASSERT(pcdev != NULL);
    ...
    iResult = ChangeDisplaySettingsEx(_pszDisplay, pdevmode, NULL, dwSet | (_fPrimary ? CDS_SET_PRIMARY : 0), NULL);
    if (iResult < 0)
    {
        FmtMessageBox(_hwndMain, MB_ICONEXCLAMATION, FALSE, ID_DSP_TXT_CHANGE_SETTINGS, ID_DSP_TXT_CHANGESETTINGS_FAILED);
...
int CDeviceSettings::RestoreSettings()
{
    ...
    pcdev = _pcdmlModes->LookUp(_iOrgResolution, _iOrgColor, _iOrgFrequency);
...
int FmtMessageBox(HWND hwnd, UINT fuStyle, BOOL fSound, DWORD dwTitleID, DWORD dwTextID, ...)
{
    ...
    if (!FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_MAX_WIDTH_MASK,
                       hInstance,
                       dwTextID,
                       0,
                       (LPTSTR)&pszMsg,
    ...
    if (dwTitleID != -1) {
        FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_MAX_WIDTH_MASK | FORMAT_MESSAGE_ARGUMENT_ARRAY,
                      hInstance,
                      dwTitleID,
                      0, 
                      (LPTSTR)&pszTitle,
...
LPTSTR FmtSprint(DWORD id, ...) {
    ...
    if (!FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_MAX_WIDTH_MASK, hInstance, id, 0, (LPTSTR)&pszMsg, 1, &marker)) {
        GetLastError();
        pszMsg = TEXT("...");
    }
    va_end(marker);
