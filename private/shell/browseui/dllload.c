...
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, VariantCopyInd, (VARIANT * pvarDest, VARIANTARG * pvargSrc), (pvarDest, pvargSrc));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, VariantChangeType, (VARIANTARG *pvargDest, VARIANTARG *pvarSrc, unsigned short wFlags, VARTYPE vt), (pvargDest, pvarSrc, wFlags, vt));
...
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, DispGetIDsOfNames, (ITypeInfo*ptinfo, OLECHAR **rgszNames, UINT cNames, DISPID*rgdispid), (ptinfo, rgszNames, cNames, rgdispid));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, CreateErrorInfo, (ICreateErrorInfo **pperrinfo), (pperrinfo));
DELAY_LOAD_SAFEARRAY(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayCreateVector, (VARTYPE vt, long iBound, ULONG cElements), (vt, iBound, cElements) );
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayAccessData, (SAFEARRAY * psa, void HUGEP** ppvData), (psa, ppvData));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayUnaccessData, (SAFEARRAY * psa), (psa) );
DELAY_LOAD_UINT(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayGetElemsize, (SAFEARRAY * psa), (psa) );
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayGetUBound, (SAFEARRAY * psa, UINT nDim, LONG * plUBound), (psa,nDim,plUBound));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayGetElement, (SAFEARRAY * psa, LONG * rgIndices, void * pv), (psa, rgIndices, pv));
DELAY_LOAD_UINT(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayGetDim, (SAFEARRAY * psa), (psa));
DELAY_LOAD_UINT(g_hinstOLEAUT32, OLEAUT32.DLL, SysStringLen, (BSTR bstr), (bstr));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, SafeArrayDestroy, (SAFEARRAY * psa), (psa));
DELAY_LOAD_INT(g_hinstOLEAUT32, OLEAUT32.DLL, DosDateTimeToVariantTime, (USHORT wDosDate, USHORT wDosTime, DOUBLE * pvtime), (wDosDate, wDosTime, pvtime));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32.DLL, VarI4FromStr, (OLECHAR FAR * strIn, LCID lcid, DWORD dwFlags, LONG * plOut), (strIn, lcid, dwFlags, plOut));
...
DELAY_LOAD_WNET(g_hinstMPR, MPR.DLL, WNetGetLastErrorA, ( LPDWORD lpError, LPSTR lpErrorBuf, DWORD  nErrorBufSize, LPSTR lpNameBuf, DWORD nNameBufSize),
       (lpError,   lpErrorBuf,    nErrorBufSize,    lpNameBuf,      nNameBufSize  ));
DELAY_LOAD_WNET(g_hinstMPR, MPR.DLL, WNetDisconnectDialog, ( HWND  hwnd, DWORD dwType), (hwnd, dwType));
DELAY_LOAD_WNET(g_hinstMPR, MPR.DLL, WNetCloseEnum, (HANDLE hEnum), (hEnum));
DELAY_LOAD_WNET(g_hinstMPR, MPR.DLL, WNetEnumResourceA, (HANDLE  hEnum, LPDWORD lpcCount, LPVOID  lpBuffer, LPDWORD lpBufferSize), (hEnum, lpcCount, lpBuffer, lpBufferSize));
...
DELAY_LOAD_WNET(g_hinstMPR, MPR.DLL, WNetOpenEnumA, (DWORD dwScope, DWORD dwType, DWORD dwUsage, LPNETRESOURCEA lpNetResource, LPHANDLE lphEnum),
       (dwScope, dwType, dwUsage, lpNetResource, lphEnum));

DELAY_LOAD_WNET(g_hinstMPR, MPR.DLL, WNetOpenEnumW, (DWORD dwScope, DWORD dwType, DWORD dwUsage, LPNETRESOURCEW lpNetResource, LPHANDLE lphEnum), (dwScope, dwType, dwUsage, lpNetResource, lphEnum));
DELAY_LOAD_WNET(g_hinstMPR, MPR.DLL, WNetGetLastErrorW, (LPDWORD lpError, LPWSTR lpErrorBuf, DWORD nErrorBufSize, LPWSTR lpNameBuf, DWORD nNameBufSize), (lpError, lpErrorBuf, nErrorBufSize, lpNameBuf, nNameBufSize));
...
DELAY_LOAD_SHELL(g_hinstShell32, shell32.dll, BOOL, _WriteCabinetState, 652, (LPCABINETSTATE lpState), (lpState));
DELAY_LOAD_SHELL(g_hinstShell32, shell32.dll, BOOL, __ReadCabinetState, 654, (LPCABINETSTATE lpState, int cLength), (lpState, cLength));
DELAY_LOAD_ORD(g_hinstShell32, shell32.dll, BOOL, __OldReadCabinetState, 651, (LPCABINETSTATE lpState, int cLength), (lpState, cLength));
...
DELAY_LOAD_SHELL(g_hinstShell32, shell32.dll, int, _SHLookupIconIndexA, 7, (LPCSTR pszFile, int iIconIndex, UINT uFlags), (pszFile, iIconIndex, uFlags));

// 8 SHSTDAPI_(int)  SHLookupIconIndexW(LPCWSTR pszFile, int iIconIndex, UINT uFlags);
DELAY_LOAD_SHELL(g_hinstShell32, shell32.dll, int, _SHLookupIconIndexW, 8, (LPCWSTR pszFile, int iIconIndex, UINT uFlags), (pszFile, iIconIndex, uFlags));

// 12 SHSTDAPI SHStartNetConnectionDialogA(HWND hwnd, LPCSTR pszRemoteName, DWORD dwType);
DELAY_LOAD_SHELL(g_hinstShell32, shell32.dll, HRESULT, SHStartNetConnectionDialogA, 12, (HWND hwnd, LPCSTR pszRemoteName, DWORD dwType), (hwnd, pszRemoteName, dwType));

// 14 SHSTDAPI SHStartNetConnectionDialogW(HWND hwnd, LPCWSTR pszRemoteName, DWORD dwType);
DELAY_LOAD_SHELL(g_hinstShell32, shell32.dll, HRESULT, SHStartNetConnectionDialogW, 14, (HWND hwnd, LPCWSTR pszRemoteName, DWORD dwType), (hwnd, pszRemoteName, dwType));
...
DELAY_LOAD_MAP(g_hinstShell32, shell32.DLL, BOOL, DL_ShellExecuteExW, ShellExecuteExW, (LPSHELLEXECUTEINFOW pseiW), (pseiW), FALSE);

#undef SHGetPathFromIDListW
DELAY_LOAD_MAP(g_hinstShell32, shell32.DLL, BOOL, DL_SHGetPathFromIDListW, SHGetPathFromIDListW, (LPCITEMIDLIST pidl, LPWSTR pszPath), (pidl, pszPath), FALSE);

#undef SHBrowseForFolderW
DELAY_LOAD_MAP(g_hinstShell32, shell32.DLL, LPITEMIDLIST, DL_SHBrowseForFolderW, SHBrowseForFolderW, (LPBROWSEINFOW pbiW), (pbiW), NULL);

#undef ExtractIconExW
DELAY_LOAD_MAP(g_hinstShell32, shell32.DLL, UINT, DL_ExtractIconExW, ExtractIconExW, (LPCWSTR lpszFile, int nIconIndex, HICON *phiconLarge, HICON *phiconSmall, UINT nIcons),
              (lpszFile, nIconIndex, phiconLarge, phiconSmall, nIcons), 0);

#undef SHFileOperationW
DELAY_LOAD_MAP(g_hinstShell32, shell32.DLL, int, DL_SHFileOperationW, SHFileOperationW, (LPSHFILEOPSTRUCTW lpfo), (lpfo), ERROR_CALL_NOT_IMPLEMENTED);
...
DELAY_LOAD(g_hinstWININET, WININET.DLL, BOOL, InternetSetOptionA, (HINTERNET hinternet, DWORD dwOptions, LPVOID lpBuffer, DWORD dwBufferLength), (hinternet, dwOptions, lpBuffer, dwBufferLength));
DELAY_LOAD(g_hinstWININET, WININET.DLL, DWORD, InternetConfirmZoneCrossingA, (HWND hWnd, LPSTR szUrlPrev, LPSTR szUrlNew, BOOL bPost), (hWnd, szUrlPrev, szUrlNew, bPost));
DELAY_LOAD(g_hinstWININET, WININET.DLL, DWORD, InternetConfirmZoneCrossingW, (HWND hWnd, LPWSTR pwzUrlPrev, LPWSTR pwzUrlNew, BOOL bPost), (hWnd, pwzUrlPrev, pwzUrlNew, bPost));
...
DELAY_LOAD(g_hinstWININET, WININET.DLL, BOOL, InternetGoOnline, (LPTSTR lpszURL, HWND hwndParent, DWORD    dwReserved), (lpszURL, hwndParent, dwReserved))


#ifdef UNIX
DELAY_LOAD_VOID(g_hinstWININET, WININET.DLL, unixGetWininetCacheLockStatus, (BOOL *pBoolReadOnly, char **ppszLockingHost), (pBoolReadOnly, ppszLockingHost))
DELAY_LOAD_VOID(g_hinstWININET, WININET.DLL, unixCleanupWininetCacheLockFile, (), ())
#endif

DELAY_LOAD(g_hinstWININET, WININET.DLL, BOOL, InternetFortezzaCommand, (DWORD dwCommand, HWND hwnd, DWORD_PTR dwReserved), (dwCommand, hwnd, dwReserved));
DELAY_LOAD(g_hinstWININET, WININET.DLL, BOOL, InternetQueryFortezzaStatus, (DWORD *pdwStatus, DWORD_PTR dwReserved), (pdwStatus, dwReserved));
...
DELAY_LOAD_HRESULT(g_hinstURLMON, URLMON.DLL, CoInternetParseUrl, (LPCWSTR pwzUrl, PARSEACTION ParseAction, DWORD dwFlags, LPWSTR pszResult, DWORD cchResult, DWORD *pcchResult, DWORD dwReserved ),
    (pwzUrl, ParseAction, dwFlags, pszResult, cchResult, pcchResult, dwReserved));

DELAY_LOAD_HRESULT(g_hinstURLMON, URLMON, FaultInIEFeature, (HWND hWnd, uCLSSPEC *pClassSpec, QUERYCONTEXT *pQuery, DWORD dwFlags), (hWnd, pClassSpec, pQuery, dwFlags));
...
DELAY_LOAD_HRESULT(g_hinstMSHTML, MSHTML.DLL, ShowHTMLDialog, (HWND hwndParent, IMoniker *pmk, VARIANT *pvarArgIn, WCHAR* pchOptions, VARIANT *pvArgOut), (hwndParent, pmk, pvarArgIn, pchOptions, pvArgOut));
#ifdef UNIX
DELAY_LOAD_HRESULT(g_hinstMSHTML, MSHTML.DLL, TranslateModelessAccelerator, (MSG* msg, HWND hwnd), (msg, hwnd));
#endif

// --------- SHDOCVW.DLL ---------------
HINSTANCE g_hinstShdocvw = NULL;

DELAY_LOAD_ORD(g_hinstShdocvw, shdocvw.dll, LPNMVIEWFOLDER, _DDECreatePostNotify, 116, (LPNMVIEWFOLDER pnm), (pnm));
DELAY_LOAD_ORD(g_hinstShdocvw, shdocvw.dll, BOOL, _DDEHandleViewFolderNotify, 117, (IShellBrowser* psb, HWND hwnd, LPNMVIEWFOLDER lpnm), (psb, hwnd, lpnm));

// BUGBUG REMOVE approx dec 98
DELAY_LOAD_MAP(g_hinstShdocvw, shdocvw.dll, BOOL, SHDOCVW_DllRegisterWindowClasses, DllRegisterWindowClasses, (const SHDRC * pshdrc), (pshdrc), 0);

// HRESULT ResetWebSettings(HWND hwnd, BOOL *pfHomePageChanged)
DELAY_LOAD_ORD(g_hinstShdocvw, shdocvw.dll, HRESULT, ResetWebSettings, 223, (HWND hwnd, BOOL *pfHomePageChanged), (hwnd, pfHomePageChanged));

// BOOL IsResetWebSettingsRequired(void)
DELAY_LOAD_ORD(g_hinstShdocvw, shdocvw.dll, BOOL, IsResetWebSettingsRequired, 224, (void), ());

// HRESULT PrepareURLForDisplayUTF8W(LPCWSTR pwz, LPWSTR pwzOut, LPDWORD pcbOut, BOOL fUTF8Enabled)
DELAY_LOAD_ORD(g_hinstShdocvw, shdocvw.dll, HRESULT, PrepareURLForDisplayUTF8W, 225, (LPCWSTR pwz, LPWSTR pwzOut, LPDWORD pcbOut, BOOL fUTF8Enabled), (pwz, pwzOut, pcbOut, fUTF8Enabled));
...
DELAY_LOAD(g_hinstVERSION, VERSION, BOOL, VerQueryValueA, (const void *pBlock, LPSTR lpSubBlock, void **ppBuffer, PUINT puLen), (pBlock, lpSubBlock, ppBuffer, puLen))
DELAY_LOAD(g_hinstVERSION, VERSION, BOOL, VerQueryValueW, (const void *pBlock, LPWSTR lpSubBlock, void **ppBuffer, PUINT puLen), (pBlock, lpSubBlock, ppBuffer, puLen))
DELAY_LOAD(g_hinstVERSION, VERSION, BOOL, GetFileVersionInfoA, (LPSTR lpstrFilename, DWORD dwHandle, DWORD dwLen, LPVOID lpData), (lpstrFilename, dwHandle, dwLen, lpData));
DELAY_LOAD(g_hinstVERSION, VERSION, BOOL, GetFileVersionInfoW, (LPWSTR lptstrFilename, DWORD dwHandle, DWORD dwLen, LPVOID lpData), (lptstrFilename, dwHandle, dwLen, lpData));
DELAY_LOAD(g_hinstVERSION, VERSION, DWORD, GetFileVersionInfoSizeA, (LPSTR pszFilename,  LPDWORD lpdwHandle), (pszFilename, lpdwHandle))
DELAY_LOAD(g_hinstVERSION, VERSION, DWORD, GetFileVersionInfoSizeW, (LPTSTR pszFilename,  LPDWORD lpdwHandle), (pszFilename, lpdwHandle))

#ifdef UNIX_FEATURE_ALIAS
HINSTANCE g_hinstInetcpl = NULL;
DELAY_LOAD(g_hinstInetcpl, inetcpl.cpl, DWORD, GetURLForAliasA, (HDPA aliasList, LPSTR alias,  LPSTR szurl, int cch), (aliasList, alias, szurl, cch))
