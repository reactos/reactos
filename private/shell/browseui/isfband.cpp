    ...
    if (_pidl && _psf)
    {
        ...
        if (pidlFull)
        {
            ...
            if (SUCCEEDED(_psf->BindToObject(pidl, NULL, IID_IShellFolder, (void **)&psf)))
            {
                ...
                if (SUCCEEDED(CoCreateInstance(CLSID_TrackShellMenu, NULL, CLSCTX_INPROC_SERVER, IID_ITrackShellMenu, (void**)&ptsm)))
                {
...
HRESULT CLogoBase::AddRefLogoCache( void )
{
    if ( _fWide )
    {
        ...
        if ( !s_lSharedWideLogosRef )
        {
            ...
            HRESULT hr = CoCreateInstance( CLSID_ImageListCache, NULL, CLSCTX_INPROC, IID_IImageCache, (void **) & s_pSharedWideLogoCache );
            if ( FAILED( hr ))
            {
                LeaveCriticalSection( &s_csSharedLogos );
                return hr;
    ...
    else
    {
        ...
        // BUGBUG for now CoCreate one per view
        return CoCreateInstance( CLSID_ImageListCache, NULL, CLSCTX_INPROC, IID_IImageCache, (void **) & _pLogoCache );
    }
}
...
HRESULT CLogoBase::InitLogoView( void )
{
    ...
    if (SUCCEEDED(hr))
    {
        hr = CoCreateInstance(CLSID_ShellTaskScheduler, NULL, CLSCTX_INPROC, IID_IShellTaskScheduler, (void **) &_pTaskScheduler);
        if (FAILED(hr))
        {
            ATOMICRELEASE(_pLogoCache);
        }
...
//     HRESULT hr = CoCreateInstance( CLSID_IntDitherer, NULL, CLSCTX_INPROC_SERVER, IID_IIntDitherer, (void **) & pDither );
//     if ( FAILED( hr ))
//     {
//         return hr;
//     }
...
STDMETHODIMP CImgCtxThumb::InternalResume()
{
    ...
    if ( !m_pImg )
    {
        hr = CoCreateInstance(CLSID_IImgCtx, NULL, CLSCTX_INPROC_SERVER, IID_IImgCtx, (LPVOID*)&m_pImg);
        ASSERT( SUCCEEDED( hr ));
        if (SUCCEEDED(hr))
        {
            ASSERT(m_pImg);
            ...
            if ( hBmp != NULL )
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        hr = CoCreateInstance( CLSID_ThumbnailScaler, NULL, CLSCTX_INPROC_SERVER, IID_IScaleAndSharpenImage2, (LPVOID *) &pScale );
                        if ( SUCCEEDED( hr ))
                        {
                            hr = pScale->ScaleSharpen2((BITMAPINFO *) &dib, lpBits, m_phBmp, &m_rgSize, m_dwRecClrDepth, hpal, 20, m_fOrigSize );
                            pScale->Release();
                        }
                        DeleteObject( hBmp );
                    }
