...
void CMenuBandMetrics::_SetChevronFont(HWND hwnd)
{
    ...
    _hFontChevron = _CalcFont(hwnd, szPath, DEFAULT_CHARSET, CH_MENUARROW, &_cxChevron, &_cyChevron, &_cxChevron, -900, FW_NORMAL);
}
...
IShellTaskScheduler* CMenuBandState::GetScheduler()
{
    if (!_pScheduler)
    {
        CoCreateInstance(CLSID_ShellTaskScheduler, NULL, CLSCTX_INPROC, IID_IShellTaskScheduler, (void **) &_pScheduler);
    }
...
BOOL CMenuBandState::FadeRect(PRECT prc, PFNFADESCREENRECT pfn, LPVOID pvParam)
{
    ...
    if (g_bRunOnNT5 && _ptFader && fFade)
    {
        ...
        if (_ptFader->FadeRect(prc, pfn, pvParam))
        {
            ...
            if (pScheduler)
            {
                fFade = pScheduler->AddTask(_ptFader, TASKID_Fader, ITSAT_DEFAULT_LPARAM, ITSAT_DEFAULT_PRIORITY) == S_OK;
            }
        }
    }
...
HWND CMenuBandState::GetWorkerWindow(HWND hwndParent)
{
    ...
    if (!_hwndWorker)
    {
        ...
        _hwndWorker = SHCreateWorkerWindow(HiddenWndProc, _hwndSubclassed, WS_EX_TOOLWINDOW, WS_POPUP, 0, (void*)_hwndSubclassed);
    }
