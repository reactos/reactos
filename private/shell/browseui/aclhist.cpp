    ...
    *pceltFetched = 0;
    ...
    else if (NULL != _pesu)
    {
        ...
        while (SUCCEEDED(_pesu->Next(1, rsu, &celtFetched)) && celtFetched)
        {
            ...
            ASSERT(NULL == rsu[0].pwcsTitle);// We didn't ask for the title!

            // Ignore if a frame or an error URL
            if (!(rsu[0].dwFlags & STATURLFLAG_ISTOPLEVEL) || IsErrorUrl(rsu[0].pwcsUrl))
            {
            ...
            //    so don't do it!!!  This breaks more things that you could imagine. -BryanSt

            // Unescape the URL (people don't like to type %F1, etc).

            // Unescaping is definitely a problem for ftp, but it should be safe for http and https (stevepro).
            ...
            if (StrChr(rsu[0].pwcsUrl, L'%'))
            {
                ...
                if ((dwScheme == URL_SCHEME_HTTP) || (dwScheme == URL_SCHEME_HTTPS))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        // were written into szBuf is <= the number of characters in rsu[0].pwcsUrl since if anything changes, it is the reduction of
                        // URL escaped sequences into single characters, and the reduction of UTF8 character sequences into single unicode characters.
...
HRESULT CACLHistory::Reset(void)
{
    ...
    // Since Reset() is always called before Next() we will delay opening the History folder until that last moment.
    if (!_puhs)
    {
        hr = CoCreateInstance(CLSID_CUrlHistory, NULL, CLSCTX_INPROC_SERVER, IID_IUrlHistoryStg, (void **)&_puhs);
...
/*
    ...
    WARNING:
        This function will not implement the full functionality of Clone().
    It will not create an enumerator that is pointing to the same location in the list as the original enumerator.
*/
...
void CACLHistory::_AddAlternateDataItem(LPCTSTR pszProtocol, LPCTSTR pszDomain, BOOL fMoveSlashes)
{
    ...
    if (fMoveSlashes)
    {
        ...
        if (ad.cchProtocol > 2 && pszProtocol[ad.cchProtocol - 2] == TEXT('/') && pszProtocol[ad.cchProtocol - 1] == TEXT('/'))
        {
            ad.cchProtocol -= 2;
            ad.cchDomain += 2;
        }
...
void CACLHistory::_CreateAlternateData(void)
{
    ...
    // Add "file://" prefix.  Since "file://foo.txt" doesn't navigate to the same place as "//foo.txt" we have to pass in FALSE to fMoveSlashes.
    ...
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, SZ_REGKEY_URLPrefixesKeyA, 0, KEY_READ|KEY_WRITE, &hkey) == ERROR_SUCCESS)
    {
        ...
        for (int i=0; SHEnumValue(hkey, i, szDomain, &cchDomain, &dwType, (PBYTE)szProtocol, &cbProtocol) == ERROR_SUCCESS; i++)
        {
...
// Given a pszUrl, attempts to create an alternate URL and store it into _pwszAlternate.
...
void CACLHistory::_CreateAlternateItem(LPCTSTR pszUrl)
{
    ...
    // then we strip off the pszProtocol and offer "//ftp.{other stuff}" as the alternate.
    for (int i=0; i<DSA_GetItemCount(_hdsaAlternateData); i++)
    {
        ...
        {
            if ((StrCmpNI(ad.pszProtocol, pszUrl, ad.cchProtocol) == 0) && (StrCmpNI(ad.pszDomain, pszUrl + ad.cchProtocol, ad.cchDomain) == 0))
            {
...
// Given an URL, set _pwszAlternate.  This takes care of all ANSI/UNICODE issues and allocates memory for
