...
STDMETHODIMP CComCatCacheTask::Go()
{
    ...
    HRESULT hr = CoCreateInstance( CLSID_SharedTaskScheduler, NULL, CLSCTX_INPROC_SERVER, IID_IShellTaskScheduler, (LPVOID*)&pScheduler );
    if( SUCCEEDED( hr ) )
    {
...
STDMETHODIMP SHWriteImplementingClassesOfCategory( IN REFCATID refcatid )
{
    ...
    ICatInformation* pci;
    if( SUCCEEDED( (hr = CoCreateInstance( CLSID_StdComponentCategoriesMgr, NULL, CLSCTX_INPROC_SERVER, IID_ICatInformation, (LPVOID*)&pci)) ) )
    {
    ...
    LPCTSTR pszRegValueName /*REGVAL_COMCATEX_IMPLEMENTING/REQUIRING*/ )
{
    ...
    if( SUCCEEDED( (hr = _MakeComCatCacheKey( refcatid, szKey, ARRAYSIZE(szKey) )) ) )
    {
        ...
        if( SUCCEEDED( hr ) )
        {
            ...
            ULONG   cbBuf = 0,
                    dwType,
                    dwRet = RegQueryValueEx( hkeyCache, pszRegValueName, 0L, &dwType, NULL, &cbBuf );
            ...
            if( ERROR_SUCCESS == dwRet )
            {
                ...
                if( NULL != (pBuf = new BYTE[cbBuf]) )
                {
                    dwRet = RegQueryValueEx( hkeyCache, pszRegValueName, 0L, &dwType, pBuf, &cbBuf );
                    hr = HRESULT_FROM_WIN32( dwRet );
                }
                else
                    hr = E_OUTOFMEMORY;
    ...
    IN HDSA hdsa )
{
    ...
    if( SUCCEEDED( (hr = _MakeComCatCacheKey( refcatid, szKey, ARRAYSIZE(szKey) )) ) )
    {
        ...
        dwRet = RegCreateKeyEx( HKEY_CURRENT_USER, szKey, 0L, NULL, 0L, KEY_WRITE, NULL, &hkeyCache, &dwDisposition );
        hr = HRESULT_FROM_WIN32( dwRet );
        if( SUCCEEDED( hr ) )
        {
            ...
            if( SUCCEEDED( (hr = _ComCatCacheFromDSA( hdsa, &pBuf, &cbBuf )) ) )
            {
                ...
                hr = RegSetValueEx( hkeyCache, REGVAL_COMCATEX_IMPLEMENTING, 0L, REG_BINARY, pBuf, cbBuf );
                if( pBuf )
                    delete [] pBuf;
            }
            RegCloseKey( hkeyCache );
    ...
    if( SUCCEEDED( (hr = _MakeComCatCacheKey( refcatid, szKey, ARRAYSIZE(szKey) )) ) )
    {
        ...
        if( SUCCEEDED( hr ) )
        {
            ...
            if( SUCCEEDED( (hr = _ComCatCacheFromDSA( hdsa, &pBuf, &cbBuf )) ) )
            {
                ...
                hr = RegSetValueEx( hkeyCache, REGVAL_COMCATEX_REQUIRING, 0L, REG_BINARY, pBuf, cbBuf );
                if( pBuf )
                    delete [] pBuf;
            }
            RegCloseKey( hkeyCache );
