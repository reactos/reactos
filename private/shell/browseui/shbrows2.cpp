...
#endif
        HRESULT hresT = CoCreateInstance(CLSID_InternetToolbar, NULL, dwServerType, IID_IDockingWindow, (void **) &_GetToolbarItem(ITB_ITBAR)->ptbar);
...
HRESULT CShellBrowser2::OnCreate(LPCREATESTRUCT pcs)
{
    ...
    if (piei->wp.length == SIZEOF(piei->wp))
    {
        ...
        if ((piei->nCmdShow == SW_SHOWNORMAL) || (piei->nCmdShow == SW_SHOWDEFAULT))
    ...
    if (piei->piehs)
    {
        ...
        if (SUCCEEDED(hres))
        {
            hres = CoMarshalInterface(piei->piehs->GetStream(), IID_IUnknown, _pbbd->_pautoWB2, MSHCTX_INPROC, 0, MSHLFLAGS_NORMAL);
        }
    ...
    case FCIDM_SEARCHPAGE:
        {
            ...
            HRESULT hres = CoCreateInstance(CLSID_WebSearchExt, NULL, CLSCTX_INPROC_SERVER, IID_IContextMenu, (void **) &pcm);
            if (SUCCEEDED(hres))
            {
...
void CShellBrowser2::_DisplayFavoriteStatus(LPCITEMIDLIST pidl)
{
    ...
    if (SUCCEEDED(CoCreateInstance(CLSID_InternetShortcut, NULL, CLSCTX_INPROC_SERVER, IID_IUniformResourceLocator, (void **)&pURL)))
    {
        IPersistFile *ppf;
...
BOOL FPidlOnNonPersistentDrive(LPCITEMIDLIST pidl)
{
    ...
    if (WNetOpenEnum(RESOURCE_REMEMBERED, RESOURCETYPE_DISK, RESOURCEUSAGE_CONTAINER | RESOURCEUSAGE_ATTACHED, NULL, &hEnum) == WN_SUCCESS)
    {
        DWORD dwCount=1;
        union
        {
    ...
    case WMC_MARSHALIDISPATCHSLOW:
        {
            ...
            if (SUCCEEDED(hres))
            {
                ...
                hres = CoMarshalInterface(pIStream, IID_IDispatch, _pbbd->_pautoWB2, MSHCTX_NOSHAREDMEM, NULL, MSHLFLAGS_NORMAL);
                if (SUCCEEDED(hres))
                {
...
void CShellBrowser2::_QueryHKCRChanged()
{
    ...
    //  is async regardless.
    PostMessage( GetShellWindow(), DTM_QUERYHKCRCHANGED, QHKCRID_VIEWMENUPOPUP, (LPARAM)NULL) ;
}
