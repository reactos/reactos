    ...
    *ppdtobj = NULL;
    ...
    if((S_OK == hr) && (*szUrl))
    {
       ...
       if(fHitsNet || fIsHTML)
       {
            ...
            HRESULT hr = CoCreateInstance(CLSID_InternetShortcut, NULL, CLSCTX_INPROC_SERVER, IID_IUniformResourceLocator, (void **)ppUrlOut);
            if (SUCCEEDED(hr))
            {
...
HRESULT LoadHandler( const CLSID * pCLSID, LPCWSTR pszBuffer, REFIID riid, void **ppvObj )
{
    ...
    LPPERSISTFILE pFile;
    HRESULT hr = CoCreateInstance( *pCLSID, NULL, CLSCTX_INPROC_SERVER, IID_IPersistFile, (void **) &pFile );
    if ( FAILED( hr ))
    {
        return E_NOINTERFACE;
    }
...
CheckIEMinimalUpdate()
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_MAIN, 0, KEY_READ|KEY_SET_VALUE, &hkeyIE) == ERROR_SUCCESS) {
        dwType = REG_SZ;
        dwSize = MAX_URL_STRING;
        if (RegQueryValueEx(hkeyIE, REGVAL_FIRST_HOME_PAGE, NULL, &dwType, NULL, &dwSize) == ERROR_SUCCESS) {
        ...
        dwType = REG_DWORD;
        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hkeyIE, REGVAL_LASTCHECKEDHI, NULL, &dwType, (unsigned char *)&ftlast.dwHighDateTime, &dwSize)==ERROR_SUCCESS) {
        ...
        if (CompareFileTime(&ftlast, &ftnow) > 0) {
            ...
            RegSetValueEx(hkeyIE,REGVAL_FIRST_HOME_PAGE, NULL, REG_SZ,(LPBYTE)szUpdateUrl, (lstrlen(szUpdateUrl)+1)*sizeof(TCHAR));
            RegSetValueEx(hkeyIE, REGVAL_LASTCHECKEDHI, NULL, REG_DWORD, (unsigned char *)&ftnow.dwHighDateTime, sizeof(DWORD));
        }
