...
#define BIT_ASSIGN(dwBits, dwMasks, dwVals)     (((dwBits) & ~(dwMasks)) | (dwVals))
...
BOOL UEMEncodePidl(IShellFolder *psf, LPITEMIDLIST pidlItem, LPTSTR pszBuf, DWORD cchBuf, int* piIndexStart, int* pcsidl);
...
#define FoldCSIDL(csidl)     ((csidl) == CSIDL_COMMON_PROGRAMS ? CSIDL_PROGRAMS : (csidl))
...
HRESULT CUserAssist::FireEvent(const GUID *pguidGrp, int eCmd, DWORD dwFlags, WPARAM wParam, LPARAM lParam)
{
    ...
    while (ch = *pszDope++) {
        ...
        case 'f':
            ...
            ASSERT((int)(unsigned short)lParam == lParam || ((int)(short)lParam == lParam));
...
//***
// NOTES
//  BUGBUG we get both loggers up front.  however we may only use one of them.
// this could be a perf pblm (and also might cause us to do other semi-weird things, e.g. do GC on the shell when we open a browser).
...
// DESCRIPTION
//  we fake it big time.  our detection of 'currently non-idle' is pretty good, but the the actual *time* we were idle is pretty iffy.
// each time we're called defines a checkpoint.
// any time the new checkpoint differs from the old one, we update our (approx) idle start point.
BOOL UEMGetLastInputInfo(LASTINPUTINFO *plii)
{
    ...
    if (g_dwTime == 0) {
        ...
        *(FARPROC *)&g_pfnGLII = GetProcAddress(GetModuleHandle(TEXT("user32.dll")), "GetLastInputInfo");
    ...
    if (csCursor != g_csCursor || csKeys != g_csKeys || (csCursor == -1 && csKeys == -1)) {
        TraceMsg(DM_UEMTRACE, "bui.glli: !idle cur=0x%x cur'=%x keys=%x keys'=%x gtc(old)=%x", g_csCursor, csCursor, g_csKeys, csKeys, g_dwTime);
        g_dwTime = GetTickCount();
        g_csCursor = csCursor;
        g_csKeys = csKeys;
    }
...
void UEMEnableTimer(UINT uTimeout)
{
    ...
    //  We are sending a function pointer, so make sure the target window is in our address space.

    //  BUGBUG - We need to revalidate that g_hwndTray really is the tray window, because Explorer may have crashed, and then the
    //  window handle got recycled into our process, and so we send a random message to a window that isn't what we think it is.
    // (raymondc)
    HWND hwndTray;
    DWORD dwPid;
...
int UpdateSmartPrefix(char *pszOld, char *pszNew, int fChange)
{
    ...
    // (note that if we rename to the same thing, i==0, so we don't do anything.  this is as it should be).
