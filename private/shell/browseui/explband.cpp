...
{
    _cRef = 1;
    _lEvents =  SHCNE_ASSOCCHANGED | SHCNE_RENAMEFOLDER | SHCNE_RMDIR | SHCNE_MKDIR | SHCNE_UPDATEDIR | SHCNE_UPDATEITEM | SHCNE_ATTRIBUTES | SHCNE_NETSHARE | SHCNE_NETUNSHARE |
                SHCNE_DRIVEREMOVED | SHCNE_MEDIAREMOVED | SHCNE_DRIVEADD | SHCNE_DRIVEADDGUI | SHCNE_MEDIAINSERTED | SHCNE_SERVERDISCONNECT | SHCNE_UPDATEIMAGE;
...
HRESULT CExplorerBand::TranslateAcceleratorIO(LPMSG lpmsg)
{
    ...
    if (hwndFocus == _hwnd)
    {
        if ((lpmsg->message == g_msgMSWheel || lpmsg->message == WM_MOUSEWHEEL) && _FilterMouseWheel(lpmsg->hwnd, lpmsg->message, lpmsg->wParam, lpmsg->lParam))
        {
            return S_OK;
        }
...
HRESULT CExplorerBand::Invoke(DISPID dispidMember,REFIID riid,LCID lcid,WORD wFlags, DISPPARAMS * pdispparams, VARIANT * pvarResult, EXCEPINFO * pexcepinfo,UINT * puArgErr)
{
    HRESULT hr = S_OK;
...
HRESULT CExplorerBand::_ConnectToBrowser(BOOL fConnect)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = ConnectToConnectionPoint(SAFECAST(this, IDispatch*), DIID_DWebBrowserEvents2, fConnect, pcpc, &_dwcpCookie, NULL);
            pcpc->Release();
        }
    }
    ASSERT(SUCCEEDED(hr));
...
HRESULT CExplorerBand::_CreateTree()
{
   ...
   DWORD dwStyle = WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_TABSTOP | TVS_HASBUTTONS  | TVS_EDITLABELS | TVS_HASLINES;
    ...
    // now the tree
    _hwnd = CreateWindowEx(0, WC_TREEVIEW, NULL, dwStyle, 0, 0, 0, 0, _hwndParent, (HMENU)FCIDM_TREE, HINST_THISDLL, NULL);
    if (_hwnd)
    {
...
// *** IDeskBand methods ***
HRESULT CExplorerBand::GetBandInfo(DWORD dwBandID, DWORD fViewMode,  DESKBANDINFO* pdbi)
{
    _dwBandID = dwBandID;
...
{
    if (hwnd == _hwnd ||  _hwnd && (hwnd == (HWND)SendMessage(_hwnd, TVM_GETEDITCONTROL, 0,0)))
        return S_OK;
...
// If the given pt is within an items text or icon area then return that item.
...
#endif
        {
            ...
            if (SUCCEEDED(_GetCtxMenu(_hwnd, pidl, &pcm)))
            {
                ...
                if (hmenu)
                {
                    ...
                    idCmd = TrackPopupMenu(hmenu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTALIGN, ptPopup.x, ptPopup.y, 0, _hwnd, NULL);
                    ...
                    if (idCmd)
                    {
                        ...
                        HRESULT hres = ContextMenu_GetCommandStringVerb(pcm, idCmd-1, szCommandString, ARRAYSIZE(szCommandString));
                        if (SUCCEEDED(hres))
                        {
...
// We're going to delete pidl, but that may be the hti (or a parent of the hti) that we are viewing!  In this case,
...
#ifdef DEBUG
    TraceMsg(DM_TRACE, "ca TR - _TreeTimerProc (msg=%d, timer=%d, selchange=%d)", g_tmStart-g_tmMsg, g_tmReceived-g_tmStart, GetTickCount()-g_tmReceived);
#endif
}
...
#endif
    _nSelChangeTimer = SetTimer(_hwnd, 1, fDelayed ? (GetDoubleClickTime()*3/2) : 1, _TreeTimerProc);
}
...
HTREEITEM CExplorerBand::_TreeFindChildItem(HTREEITEM htiParent, LPCITEMIDLIST pidl)
{
    ...
    if (psf)
    {
        for (hti = TreeView_GetChild(_hwnd, htiParent); hti; hti = TreeView_GetNextSibling(_hwnd, hti))
        {
...
HTREEITEM CExplorerBand::_TreeBuildChild(CExpBandItemWatch &w, LPCITEMIDLIST pidl)
{
    ...
    if (lpn && (pidlChild = ILCloneFirst(pidl)))
    {
        ...
        ASSERT(!_ILIsTerminal(pidl) || (_TreeChildAttribs(lpn, pidlChild, &dwAttribs) && (dwAttribs & SFGAO_FOLDER)));
#endif
...
HTREEITEM CExplorerBand::_TreeBuild(LPCITEMIDLIST pidlFull, BOOL bExpand, BOOL bDontFail)
{
    HTREEITEM hti;
    ASSERT(_hwnd);
    ASSERT(pidlFull);
        ...
        for (; !ILIsEmpty(pidl) && w.StillValid() && hti; pidl = ILGetNext(pidl))
        {
            ...
            if (!_ei.fTimedOut)
            {
                ...
                if (_ILIsTerminal(pidl))        // is last component
                {
                    ...
                    if (!lpnParent || !_TreeChildAttribs(lpnParent, pidl, &dwAttribs) || !(dwAttribs & SFGAO_FOLDER))
                    {
    ...
    if (psfParent)
    {
        ...
        if (pidl)
        {
            ...
            if (pszUnedited == NULL || lstrcmpW(pszUnedited, wszName) != 0)
            {
                ...
                {
                    SendMessage(_hwnd, TVM_EDITLABEL, (WPARAM)ptvdi->item.pszText, (LPARAM)ptvdi->item.hItem);
                }
            }
            ILFree(pidl);
        }
...
LRESULT CExplorerBand::_TreeHandleExpanding(LPNM_TREEVIEW lpnmtv)
{
    ...
    if ((!(lpnmtv->itemNew.state & TVIS_EXPANDEDONCE)) || (lpnmtv->itemNew.state & TVIS_EXPANDPARTIAL))
    {
...
void CExplorerBand::_HandleUpdateDir(LPCITEMIDLIST pidl)
{
    ...
    if (hti)
    {
        ...
        if (fIsDesktop)
        {
            ...
            //  THE REAL FIX - is in SCN (fsnotify.c).  we should update the collapsing code to never identify the common parent as the
            //  desktop.  that way, in the Alias folder case, we would get two UPDATEDIR's. one for the physical folder (possibly collapsing all
            //  the way to MyComputer) and a second for the logical location also limited to the first node off the desktop.
...
//  There is a race condition between us and OneTree, where it's random who gets the change notification first.
//  If we gets it first, then we "invalidate" the item, then TreeView
//  asks us for the info, so we ask OneTree, but OneTree gives us stale info because it hasn't been told about the change yet.

//  So if we invalidate an item due to a change notify, we have to turn around and invalidate the OneTree cache so he won't give
...
void CExplorerBand::_TreeHandleDeleteItem(LPNM_TREEVIEW lpnmtv)
{
    ...
    //  For all pending enumerations, see if they're deleting an item that is active in the enumeration.
    //  If so, then mark the enumeration stale and record enough information to continue
...
// bFlag and bActive are used to prevent stop nodes from being entered without a start node since we get NM_CUSTOMDRAW's when moving throughout the tree.
...
// Handle notification from the tree view.
{
    ...
    case NM_DBLCLK:
        ...
        // Otherwise, we can't prevent double logon dialog problems described below.
        ...
        if (g_fRunningOnNT)
        {
            ...
            switch(lpCD->nmcd.dwDrawStage)
            {
                ...
                case CDDS_ITEMPREPAINT:
                    // Do funky coloring only if the item would otherwise be the normal color.
                    // Otherwise we try coloring blue when the background is already blue and it turns unreadable.
...
BOOL CExplorerBand::_TreeRefreshAll()
{
    ...
    if (htiOld != htiNew)
    {
        ...
        if (pidlCur)
        {
            // if our selection got lost, it could have been because the network enumeration isn't reliable..  try forcing it in
            ...
            {
                TraceMsg(DM_TRACE, "ca TR - _TreeRefreshAll hti,htiNew=%x,%x fExp=%x", htiOld, htiNew, _TreeGetItemState(htiNew) & TVIS_EXPANDEDONCE);
            ...
            //  Handle the selection change synchronously to avoid refreshing invalid right pane.
            if (htiOld != htiNew)
                _TreeRealHandleSelChange();
...
#ifdef SETALLVIS
    // We want to make sure the following UpdateWindow does the job completely
