...
HRESULT CInternetToolbar::GetIDsOfNames(REFIID riid,OLECHAR **rgszNames,UINT cNames, LCID lcid, DISPID * rgdispid)
{
    return(E_NOTIMPL);
}
...
BSTR CInternetToolbar::_GetEditProgID(IHTMLDocument2* pHTMLDocument)
{
    ...
    // First get all document elements.  Note that this is very fast in ie5 because the collection directly accesses the internal tree.
    ...
    if (SUCCEEDED(pHTMLDocument->get_all(&pAllCollection)))
    {
        ...
        // in the head, and since Trident always creates a body tag, we can stop looking when we hit the body.

        // Note, the alternative of using pAllCollection->tags to return the
        // collection of meta tags is likely more expensive because it will walk the whole tree (unless Trident optimizes this).
        long lItemCnt;
        VARIANT vEmpty;
        ...
        for (long lItem = 0; lItem < lItemCnt; lItem++)
        {
            ...
            if (S_OK == pAllCollection->item(vIndex, vEmpty, &pDispItem))
            {
                ...
                if (SUCCEEDED(pDispItem->QueryInterface(IID_IHTMLMetaElement, (void **)&pMetaElement)))
                {
                    BSTR bstrName = NULL;

                    // We have a META element, check its NAME and CONTENT
                    if (SUCCEEDED(pMetaElement->get_name(&bstrName)) && (bstrName != NULL) && (StrCmpIW(bstrName, OLESTR("ProgId")) == 0) && SUCCEEDED(pMetaElement->get_content(&bstrProgID)) && (bstrProgID != NULL))
                    {
                ...
                else if (SUCCEEDED(pDispItem->QueryInterface(IID_IHTMLBodyElement, (void **)&pBodyElement)) )
                {
                ...
                else if (SUCCEEDED(pDispItem->QueryInterface(IID_IHTMLFrameSetElement, (void **)&pFrameSetElement)) )
                {
        ...
        if (GetObject(ii.hbmImage, sizeof(ds), &ds))
        {
            ...
            for (int y=0; y < yHei; ++y)
            {
                for (int x=0; x < xWid; ++x)
                {
                    ...
                    // Using the average of r,g,b can be too dark, and using the max of r,g,b can be too bright.  So, as a simple algorithm we use
                    // the average of the two schemes.  This is cheaper than using true intensity matching.
...
// button to use it.  If the hIcon is -1, the edit button is reset to use it's default glyph.
...
)
{
    ...
    else
    {
        ...
        if (Shell_GetImageLists(NULL, &himlSmall) &&
            ...
            MyImageList_GetBitmaps(himlSmall, iIcon, (cx - cxSmall)/2, (cy - cySmall)/2, cx, cy, &hbmpImage, &hbmpMask))
        {
            if (bMirrored)
            {
                HBITMAP hbmpTemp;
...
void CInternetToolbar::_InitEditButtonStyle()
{
    ...
    // Dochost merges under one of two clsids, so have to check both
    if (SUCCEEDED(_btb._ConvertCmd(&CLSID_InternetButtons, DVIDM_EDITPAGE, NULL, &uiCmd)) || SUCCEEDED(_btb._ConvertCmd(&CLSID_MSOButtons, DVIDM_EDITPAGE, NULL, &uiCmd)))
    {
        ASSERT(uiCmd != -1);
...
// If the edit button is displaying a custon glyph, this function reloads the glyph.
...
void CInternetToolbar::_UpdateEditButton()
{
    ...
    {
        fCheckDocForProgID = SHRegGetBoolUSValue(REGSTR_PATH_MAIN, TEXT("CheckDocumentForProgID"), FALSE, TRUE) ? 1 : 0;
    }
    ...
    if (fCheckDocForProgID)
    {
        ...
        {
            // Check the current document for a META tag specifying the program to use to edit this file.
...
HRESULT CInternetToolbar::Invoke(DISPID dispidMember,REFIID riid,LCID lcid,WORD wFlags, DISPPARAMS * pdispparams, VARIANT * pvarResult, EXCEPINFO * pexcepinfo,UINT * puArgErr)
{
    ASSERT(pdispparams);
    if(!pdispparams)
        return E_INVALIDARG;
    ...
    case DISPID_BEFORENAVIGATE:
    {
        ...
        ASSERT((pdispparams->rgvarg[5].vt == VT_BSTR) && (pdispparams->rgvarg[5].bstrVal != NULL));
    ...
    {
        // Sometimes we get a premature document complete.  We can catch this by checking to see if we have received a DISPID_NAVIGATECOMPLETE2 event.
    ...
    case DISPID_COMMANDSTATECHANGE:
        ...
        if(!pdispparams || (pdispparams->cArgs != 2) || (pdispparams->rgvarg[0].vt != VT_BOOL) || (pdispparams->rgvarg[1].vt != VT_I4))
            return E_INVALIDARG;
...
void CInternetToolbar::_UpdateGroup(const GUID *pguidCmdGroup, int cnt, OLECMD rgcmds[], const GUID* pguidButton, const int buttonsInternal[])
{
    if (!IsEqualGUID(*pguidButton, CLSID_CommonButtons) && !IsEqualGUID(*pguidButton, _btb._guidCurrentButtonGroup))
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        if (rgcmds[i].cmdf & OLECMDF_SUPPORTED)
        {
            ...
            {
                SendMessage(_btb._hwnd, TB_ENABLEBUTTON, idBut, (rgcmds[i].cmdf & OLECMDF_ENABLED) ? TRUE : FALSE);
                SendMessage(_btb._hwnd, TB_CHECKBUTTON, idBut, (rgcmds[i].cmdf & OLECMDF_LATCHED) ? TRUE : FALSE);
            }
        }
    }
}
    ...
    for (int nIndex = 0; nIndex < nButtons; nIndex++)
    {
        ...
        if (pcm)
        {
            ...
            else
            {
                ...
                {
                    SendMessage(_btb._hwnd, TB_ENABLEBUTTON, iCmd, (ocButton.cmdf & OLECMDF_ENABLED) ? TRUE : FALSE);
                    SendMessage(_btb._hwnd, TB_CHECKBUTTON, iCmd, (ocButton.cmdf & OLECMDF_LATCHED) ? TRUE : FALSE);
                }
            }
        }
    }
...
BOOL _UseSmallIcons()
{
    ...
    return SHRegGetBoolUSValue(SZ_REGKEY_SMALLICONS, SZ_REGVALUE_SMALLICONS, FALSE, fDefaultToSmall);
}
...
#define SZ_REGVALUE_MAPNETDRV      TEXT("MapNetDrvBtn")
    ...
    if (GetUIVersion() >= 4)
    {
        ...
        SHRegGetUSValue(SZ_REGKEY_ADVFOLDER, SZ_REGVALUE_MAPNETDRV, NULL, &dwData, &cbData, FALSE, &dwDefault, cbDefault);
    }
    ...
    if (HimlCacheDirty(pimlCache, fSmallIcons))
    {
        ...
        for (i = 0; i < CIMLISTS; i++)
        {
            if ((!pimlCache->arhiml[i]) || (cr3D != pimlCache->cr3D) || fSmallIcons != pimlCache->fSmallIcons)
            {
                ...
                pimlCache->arhiml[i] = ImageList_LoadImage(HINST_THISDLL, MAKEINTRESOURCE(idBmp + i), cx, 0, crMask, IMAGE_BITMAP, uFlags);
...
// removes all buttons marked hidden.  returns the number of buttons left
...
{
    ASSERT(!_fShellView);// We only have policies for web buttons.
    ASSERT(SHRestricted2(REST_SPECIFYDEFAULTBUTTONS, NULL, 0));// Caller should have checked this.
...
void CInternetToolbar::_InitToolbar()
{
    ...
    SendMessage(_btb._hwnd, TB_SETEXTENDEDSTYLE,    0, TBSTYLE_EX_DRAWDDARROWS | TBSTYLE_EX_HIDECLIPPEDBUTTONS);
...
HRESULT CInternetToolbar::_ShowLinks(PBANDSAVE pbs)
{
    ...
    if (!pbid)
    {
        ...
        if (!_fInitialPidlIsWeb || FAILED(CreateFromRegKey(c_szRegKeyCoolbar, TEXT("QuickLinksCLSID"), IID_IDeskBand, (void **)&pdbLinks)))
        {
    ...
    if (!pbid)
    {
        ...
        if (pfcb)
        {
            ...
            if (psm)
            {
                ...
                if (SUCCEEDED(IUnknown_Exec(_pbs2, &CGID_Explorer, SBCMDID_GETCURRENTMENU, 0, NULL, &var)) && var.vt == VT_INT_PTR && var.byref)
                {
                    IDeskBand* pdbMenu;
...
HBITMAP CInternetToolbar::_LoadBackBmp(LPTSTR * ppszBitmap, BMPCACHE * pbmpCache, BOOL fInternet)
{
    ...
    if ((SystemParametersInfoA(SPI_GETHIGHCONTRAST, hc.cbSize, (LPVOID) &hc, FALSE)) && (hc.dwFlags & HCF_HIGHCONTRASTON))
    {
...
HRESULT CInternetToolbar::_CreateBands()
{
    ...
    if (!_hwnd && _ptbsite)
    {
        ...
        if (SUCCEEDED(hres))
        {
            ...
            _hwnd = SHCreateWorkerWindow(SizableWndProc, hwndParent, 0, WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, (HMENU)FCIDM_REBAR, this);
...
LRESULT CALLBACK CInternetToolbar::SizableWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_WININICHANGE:
        if (FALSE == pitbar->_fDestroyed)
        {
            ...
            BOOL fRebuild = (uMsg == WM_SYSCOLORCHANGE) || (dwSection == EICH_KWINEXPLSMICO) || (wParam == SPI_SETNONCLIENTMETRICS);
            if (fRebuild)
            {
...
HRESULT CInternetToolbar::ResizeBorderDW(LPCRECT prcBorder, IUnknown* punkToolbarSite, BOOL fReserved)
{
    ...
    if (_ptbsite)
    {
        ...
        if (_fShow)
        {
            ...
            SetWindowPos(_bs._hwnd, NULL, 0, 0, cx, cy,  SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOMOVE);
            GetWindowRect(_bs._hwnd, &rcRebar);
            rcRequest.top = rcRebar.bottom - rcRebar.top + iExtra;
            SetWindowPos(_hwnd, NULL, prcBorder->left, prcBorder->top, rcRebar.right - rcRebar.left, rcRequest.top, SWP_NOZORDER | SWP_NOACTIVATE);
        }
        ...
        TraceMsg(DM_LAYOUT, "CITB::ResizeBorder calling RequstBS with %d,%d,%d,%d", rcRequest.left, rcRequest.top, rcRequest.right, rcRequest.bottom);
        _ptbsite->RequestBorderSpaceDW(SAFECAST(this, IDockingWindow*), &rcRequest);
        TraceMsg(DM_LAYOUT, "CITB::ResizeBorder calling SetBS with %d,%d,%d,%d", rcRequest.left, rcRequest.top, rcRequest.right, rcRequest.bottom);
...
HRESULT CInternetToolbar::QueryStatus(const GUID *pguidCmdGroup, ULONG cCmds, OLECMD rgCmds[], OLECMDTEXT *pcmdtext)
{
    ...
    if (pguidCmdGroup && IsEqualGUID(CGID_PrivCITCommands, *pguidCmdGroup))
    {
        ...
        for (ULONG i = 0 ; i < cCmds; i++)
        {
            ...
            case CITIDM_EDITPAGE:
                ...
                {
                    TCHAR szBuf[MAX_PATH];
                    if ((pcmdtext->cmdtextf == OLECMDTEXTF_NAME) && _aEditVerb.GetMenuText(szBuf, ARRAYSIZE(szBuf)))
                    {
...
void CInternetToolbar::_CSHSetStatusBar(BOOL fOn)
{
    ...
    v.vt = VT_I4;
    v.lVal = fOn;
    IUnknown_Exec(_ptbsite, &CGID_ShellBrowser, FCIDM_SETSTATUSBAR, 0, &v, NULL);
}
...
BOOL IsBarRefreshable(IDeskBar* pdb)
{
    ...
    if (SUCCEEDED(IUnknown_Exec(pdb, &CGID_DeskBarClient, DBCID_CLSIDOFBAR, 1, NULL, &varClsid)) && (varClsid.vt == VT_BSTR))
    {
        ...
        if ( GUIDFromString(varClsid.bstrVal, &clsidBar) && (IsEqualGUID(clsidBar, GUID_NULL)) )
        {
        ...
        else
        {
            ...
            if ( (SHGetValue(HKEY_CLASSES_ROOT, wszKey, L"DontRefresh", &dwType, &dwValue, &dwcbData) == ERROR_SUCCESS) && (dwValue != 0) )
            {
                fIsRefreshable = FALSE;
            }
        }
...
HRESULT CInternetToolbar::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANTARG *pvarargIn, VARIANTARG *pvarargOut)
{
    ...
    if (!pguidCmdGroup) {
        ...
        if (pvarargOut) {
            ...
            case TBIDM_REFRESH:
            {
                VARIANT v = {0};
                v.vt = VT_I4;
                v.lVal = (GetAsyncKeyState(VK_CONTROL) < 0) ? OLECMDIDF_REFRESH_COMPLETELY|OLECMDIDF_REFRESH_PROMPTIFOFFLINE : OLECMDIDF_REFRESH_NO_CACHE|OLECMDIDF_REFRESH_PROMPTIFOFFLINE;
                _pdie->Refresh2(&v);
                if (_hwndAddressBand)
                {
                    LPBANDITEMDATA pbid = _bs._GetBandItemDataStructByID(CBIDX_ADDRESS);
                ...
                {
                    IDeskBar* pdb;
                    if (SUCCEEDED(psb->FindToolbar(INFOBAR_TBNAME, IID_IDeskBar, (void **)&pdb)) && pdb && IsBarRefreshable(pdb))
                    {
                        IUnknown_Exec(pdb, NULL, OLECMDID_REFRESH, OLECMDIDF_REFRESH_NORMAL|OLECMDIDF_REFRESH_PROMPTIFOFFLINE, NULL, NULL);
                        pdb->Release();
                    }
                    if (SUCCEEDED(psb->FindToolbar(COMMBAR_TBNAME, IID_IDeskBar, (void **)&pdb)) && pdb && IsBarRefreshable(pdb))
                    {
                        IUnknown_Exec(pdb, NULL, OLECMDID_REFRESH, OLECMDIDF_REFRESH_NORMAL|OLECMDIDF_REFRESH_PROMPTIFOFFLINE, NULL, NULL);
                        pdb->Release();
                    }
...
BOOL _GetSearchHKEY(REFGUID guidSearch, HKEY *phkey)
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_SZ_STATIC, 0, KEY_READ, &hkey) == ERROR_SUCCESS)
    {
        ...
        for (iExt=0; !bRet && RegEnumKeyEx(hkey, iExt, szExt, &cchExt, NULL, NULL, NULL, NULL) == ERROR_SUCCESS; cchExt = ARRAYSIZE(szExt), iExt++)
        {
            ...
            if (RegOpenKeyEx(hkey, szExt, 0, KEY_READ, &hkeyExt) == ERROR_SUCCESS)
            {
                ...
                for (i = 0; !bRet && (wnsprintf(szSubKey, ARRAYSIZE(szSubKey), TEXT("%d"), i), RegOpenKey(hkeyExt, szSubKey, &hkeySub) == ERROR_SUCCESS); i++)
                {
                    ...
                    if (SHGetValue(hkeySub, REG_SZ_SEARCH_GUID, NULL, &dwType, (BYTE*)szSearchGuid, &cb) == ERROR_SUCCESS)
                    {
                        ...
                        if (GUIDFromString(szSearchGuid, &guid) && IsEqualGUID(guid, guidSearch))
                        {
                            HKEY hkeyTmp;
...
HRESULT CFolderSearches::EnumSearches(IEnumUrlSearch **ppenum)
{
    ...
    *ppenum = (IEnumUrlSearch *)new CStandardEnum(IID_IEnumUrlSearch, FALSE, _iCount, sizeof(URLSEARCH), _pUrlSearch, CopyEnumElement);
    if (*ppenum)
    {
        _pUrlSearch = NULL;
        _iCount = 0;
...
HRESULT CInternetToolbar::_GetFolderSearches(IFolderSearches **ppfs)
{
    ...
    *ppfs = NULL;
    if (_hdpaFSI)
    {
        ...
        hres = E_OUTOFMEMORY;
        if (cFSIs > 0 && ((pUrlSearch = (LPURLSEARCH)LocalAlloc(LPTR, SIZEOF(URLSEARCH)*cFSIs)) != NULL))
        {
...
{
    if (!IsEqualGUID(_btb._guidCurrentButtonGroup, CLSID_CommonButtons) && _btb._pctCurrentButtonGroup)
    {
...
HRESULT CInternetToolbar::SendToolbarMsg(const GUID* pguidButtonGroup, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT * plRes)
{
    ...
    if (
        ...
        uMsg == TB_GETBUTTONINFOA || uMsg == TB_GETBUTTONINFOW || uMsg == TB_SETBUTTONINFOA || uMsg == TB_SETBUTTONINFOW ||
    ...
    if ((uMsg == TB_ENABLEBUTTON) || (uMsg == TB_HIDEBUTTON) || (uMsg == TB_CHECKBUTTON) || (uMsg == TB_PRESSBUTTON) || (uMsg == TB_MARKBUTTON))
    {
        unsigned int uiTemp;
...
LRESULT TOOLSBANDCLASS::_ToolsCustNotify (LPNMHDR pnmh)
{
    ...
    case TBN_QUERYDELETE:
        return (SendMessage(_hwnd, TB_ISBUTTONHIDDEN, (WPARAM) ptbn->tbButton.idCommand, (LPARAM) 0)) ? FALSE : TRUE;
    ...
    if (fToInternal && _hwnd) {
        ...
        for (int i = 0; i < nCount; i++) {
            ...
            if (pcm) {
                ...
                // loop through the command mapping structures until we find this guid and id
                if (IsEqualGUID(pcm->guidButtonGroup, *pguidButtonGroup) && id == pcm->nCmdID) {
...
LRESULT CInternetToolbar::_AddBitmapFromForeignModule(UINT uiGetMSG, UINT uiSetMSG, UINT uiCount, HINSTANCE hinst, UINT_PTR nID, COLORREF rgbMask)
{
    ...
    if (fOk && (bmp.bmWidth != (LONG)(TB_BMP_CX * uiCount)) || (bmp.bmHeight != (LONG)TB_BMP_CY) )
    {
        ...
        for (UINT n = 0; n < uiCount; n++)
        {
            ...
            StretchBlt(dcMemDest, xDest, yDest, cxOrg, bmp.bmHeight, dcMemSrc, (cxOrg * n), 0, cxCopy, cyCopy, SRCCOPY);
        }
...
#endif
            if (!IsEqualGUID(_cs.clsidVerticalBar, CLSID_SearchBand) && !IsEqualGUID(_cs.clsidVerticalBar, CLSID_FileSearchBand))
            {
...
HRESULT CInternetToolbar::QueryService(REFGUID guidService, REFIID riid, void **ppvObj)
{
...
HRESULT CInternetToolbar::CITBandSite::_Initialize(HWND hwndParent)
{
    _hwnd = CreateWindowEx(WS_EX_TOOLWINDOW, REBARCLASSNAME, NULL,
                           RBS_VARHEIGHT | RBS_BANDBORDERS | RBS_REGISTERDROP | RBS_DBLCLKTOGGLE | WS_VISIBLE | WS_BORDER | WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | CCS_NODIVIDER | CCS_NOPARENTALIGN,
...
HRESULT CInternetToolbar::CITBandSite::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANTARG *pvarargIn, VARIANTARG *pvarargOut)
{
    if (!pguidCmdGroup) {
        /*NOTHING*/
    } else if (IsEqualGUID(CGID_PrivCITCommands, *pguidCmdGroup)) {
...
HRESULT TOOLSBANDCLASS::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANTARG *pvarargIn, VARIANTARG *pvarargOut)
{
     CInternetToolbar* pitbar = IToClass(CInternetToolbar, _btb, this);
    if (!pguidCmdGroup) {
        /*NOTHING*/
    ...
    case TBN_GETOBJECT:
        {
            ...
            if (IsEqualIID(*pnmon->piid, IID_IDropTarget))
            {
                if (pnmh->hwndFrom == _hwnd)
                {
                    ...
                    if (IsEqualGUID(guid, CLSID_CommonButtons) && (uiCmd == TBIDM_HOME || uiCmd == TBIDM_FAVORITES)) {
...
CMDMAPCUSTOMIZE* TOOLSBANDCLASS::_GetCmdMapCustomize(GUID* pguid, UINT nCmdID)
{
    ...
    for (j = 0; j < DSA_GetItemCount(_pcinfo->hdsa); j++) {
        CMDMAPCUSTOMIZE* pcmc = (CMDMAPCUSTOMIZE*)DSA_GetItemPtr(_pcinfo->hdsa, j);
        if (pcmc->cm.nCmdID == nCmdID && IsEqualGUID(*pguid, pcmc->cm.guidButtonGroup)) {
            return pcmc;
        }
    }
...
BOOL_PTR CALLBACK TOOLSBANDCLASS::_BtnAttrDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        {
            if (GET_WM_COMMAND_CMD(wParam, lParam) == CBN_SELENDOK || GET_WM_COMMAND_CMD(wParam, lParam) == CBN_CLOSEUP)
            {
                // what'd they pick?
        ...
        break;
    case WM_CONTEXTMENU:
        SHWinHelpOnDemandWrap((HWND) wParam, c_szHelpFile, HELP_CONTEXTMENU, (DWORD_PTR)(LPTSTR) c_aBtnAttrHelpIDs);
        return TRUE;
    case WM_HELP:
        SHWinHelpOnDemandWrap((HWND) ((LPHELPINFO) lParam)->hItemHandle, c_szHelpFile, HELP_WM_HELP, (DWORD_PTR)(LPTSTR) c_aBtnAttrHelpIDs);
        return TRUE;
    case WM_DESTROY:
        {
...
void CInternetToolbar::CEditVerb::InitDefaultEditor(HKEY hkey)
{
    ...
    HKEY hkeyEdit = hkey;
    if (hkey || ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_DEFAULT_HTML_EDITOR, 0, KEY_READ | KEY_WRITE, &hkeyEdit))
    {
        ...
        else
        {
            ...
            if (SUCCEEDED(AssocCreate(CLSID_QueryAssociations, IID_IQueryAssociations, (LPVOID *)&pqa)))
            {
                ...
                || SUCCEEDED(pqa->GetString(ASSOCF_VERIFY, ASSOCSTR_FRIENDLYAPPNAME, NULL, szBuf, (LPDWORD)MAKEINTRESOURCE(SIZECHARS(szBuf))))))
                {
                    ...
                    // Add this verb to our OpenWithList if it isn't already there.  We add using the following format:
...
void AddNotepadToOpenWithList()
{
    ...
    if (SUCCEEDED(AssocQueryString(ASSOCF_INIT_BYEXENAME | ASSOCF_VERIFY, ASSOCSTR_FRIENDLYAPPNAME, szPath, NULL, szNotepadFriendly, (LPDWORD)MAKEINTRESOURCE(SIZECHARS(szNotepadFriendly)))))
    {
...
// result in rVerb.  The caller is responsible for freeing the string returned.
LPCTSTR CInternetToolbar::CEditVerb::_GetExePath(EDITVERB& rVerb)
{
    ...
    if (NULL == rVerb.pszExe)
    {
        ...
        TCHAR sz[MAX_PATH];
        if (SUCCEEDED(AssocQueryStringByKey(ASSOCF_VERIFY, ASSOCSTR_EXECUTABLE, rVerb.hkeyProgID, rVerb.fUseOpenVerb ? NULL : L"edit", sz, (LPDWORD)MAKEINTRESOURCE(SIZECHARS(sz)))))
...
// Returns TRUE if path of the exe associated with the verb is not found in any of the existing verbs.
...
// command to the appropriate executable. This function returns true if the path contains the name of a known stub.
BOOL CInternetToolbar::CEditVerb::_IsHtmlStub(LPCWSTR pszPath)
{
    BOOL fRet = FALSE;
...
//  Adds a new edit verb.  Returns a pointer to the new verb if it successfully added.
            ...
            for (int i=0; i < ARRAYSIZE(exeToProgID); ++i)
            {
                if (StrStrI(szCommand, exeToProgID[i].pszApp))
                {
                    ...
                    if (SUCCEEDED(AssocQueryKey(0, ASSOCKEY_SHELLEXECCLASS, exeToProgID[i].pszProgID, NULL, &hkeyOffice)))
                    {
                        ...
                        fUseOpenVerb = TRUE;// The office apps always use the open verb
                        fShowIcon = TRUE;// The icon is shown on the button face for office apps
                    }
                    break;
                }
            }
        ...
        else
        {
            ...
            else
            {
                ...
                *pNewVerb = newVerb;
                ...
                // If the description of the executable matches that of the default editor, make it our default edit verb.
                // If we are not checking for the office app, we can assume that this verb was from a progid in an html file and we will also make it our default.
                LPCWSTR pszDefDesc = _GetDefaultEditor();
                LPCWSTR pszNewDesc = _GetDescription(*pNewVerb);

                if (!fCheckForOfficeApp || (pszDefDesc && pszNewDesc && StrCmp(pszDefDesc, pNewVerb->pszDesc) == 0))
                {
...
)
{
    ...
    if (fFileExt)
    {
        ...
        if ((StrCmpI(pszProgID, L".htm") == 0 || StrCmpI(pszProgID, L".html") == 0) && _GetDefaultEditor())
        {
            HKEY hkeyDefault;
...
BOOL CInternetToolbar::CEditVerb::GetToolTip(LPTSTR pszToolTip, UINT cchMax, BOOL fStripAmpersands)
{
    if (_nElements == 0)
    {
        return FALSE;
...
// Gets the name of the app associated with the verb.
{
    ...
    if (NULL == rVerb.pszDesc)
    {
        ...
        TCHAR sz[MAX_PATH];
        if (SUCCEEDED(AssocQueryStringByKey(ASSOCF_VERIFY, ASSOCSTR_FRIENDLYAPPNAME, rVerb.hkeyProgID, rVerb.fUseOpenVerb ? NULL : L"edit", sz, (LPDWORD)MAKEINTRESOURCE(SIZECHARS(sz)))))
        {
...
// Reads the info associated with the progid at the given index.
// This function allows us to do a lazy fetch of the info when requested.
{
    ASSERT(nIndex < _nElements);
    ...
    {
        ASSERT(rVerb.pszExe);
        ASSERT(rVerb.pszDesc);// Note that we fetched the friendly name earlier
        rVerb.iIcon = Shell_GetCachedImageIndex(rVerb.pszExe, 0, 0);// Now get the icon
    }
    else
    {
        rVerb.iIcon = -1;
...
// Clean up MSAAMenuInfo - specifically, release the allocated UNICODE string, if appropriate...
{
...
// Executes the edit verb indicated by nIndex.
{
    ...
    if (PathIsURL(pszURL))
    {
        ...
        else
        {
            ...
            // and pass it as the lpFile parameter to shellexecute.
            // We also pass SEE_MASK_FILEANDURL, so shellexecute can recognize this case.
            int iLength;

            if (FAILED(URLToCacheFile(pszURL, szCacheFileName, ARRAYSIZE(szCacheFileName))))
            {
                // Frontpage express crashes if we pass a null file name, so if the app doesn't prefer the url instead, we bail.
            ...
            StrCpyN(&szCacheFileName[iLength + 1], pszURL, ARRAYSIZE(szCacheFileName) - (iLength + 1));// copy in the URL name
...
//    sei.lpParameters = NULL;
    ...
    // and to sniff the html file for the original document name. Hackers! So let's see if it is registered.
    HKEY hkeyProxy = NULL;
...
// This window procedure intercepts owner-draw menu messages when the edit pop-up menu is displayed.
{
    ...
    switch(uMsg)
    {
        ...
        case WM_MEASUREITEM:
        {
            ...
            if (InRange(idCmd, FCIDM_EDITFIRST, FCIDM_EDITLAST))
            {
                ...
                // We don't want the same accelerator on all items, so remove underlines
                ...
                if (pszFrom)
                {
                    ...
                    // In some locals, the accelerator is identified in brackets at the end of the string, so if we strip ampersands, we strip these too.
