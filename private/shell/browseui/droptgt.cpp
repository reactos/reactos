...
HRESULT CDelegateDropTarget::DragOver(DWORD grfKeyState, POINTL ptl, LPDWORD pdwEffect)
{
        ...
        (itemNew != _itemOver || !_fPrime))
    {
        ...
        if (_pdtCur)
        {
            ...
            //TraceMsg(TF_DRAGDROP, "CDelegateDropTarget::DragOver DragEnter()d id:%d dwEffect:%4x hres:%d", _itemOver, dwEffectOut, hres);
        }
        else
        {
            ...
            //TraceMsg(TF_DRAGDROP, "CDelegateDropTarget::DragOver id:%d had nothing to DragEnter()", _itemOver);
        }
    }
    else
    {
