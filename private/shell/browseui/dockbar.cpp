...
void CDockingBar::_DragEnter(UINT uMsg, int x, int y, RECT* rcFeed)
{
    ...
    TraceMsg(DM_DRAG2, "cwb.de: rcTmp=(%d,%d,%d,%d) (%dx%d) _rcCapture=(%d,%d,%d,%d) (%dx%d)",
             rcTmp.left, rcTmp.top, rcTmp.right, rcTmp.bottom, RECTWIDTH(rcTmp), RECTHEIGHT(rcTmp), _rcCapture.left, _rcCapture.top, _rcCapture.right, _rcCapture.bottom, RECTWIDTH(_rcCapture), RECTHEIGHT(_rcCapture));
#endif
...
void CDockingBar::_DragTrack(UINT uMsg, int x, int y, RECT* rcFeed, int eState)
{
    ...
    TraceMsg(DM_DRAG2,
        ...
        eState, x, y, _PM(rcFeed,left), _PM(rcFeed,right), _PM(rcFeed,bottom), _PM(rcFeed,right), _PX(rcFeed,RECTWIDTH(*rcFeed)), _PX(rcFeed,RECTHEIGHT(*rcFeed)));
#endif
    ...
    case 1:     // WM_MOVE
        TraceMsg(DM_DRAG2, "cwb.dt: %s _xyPend=(%d,%d) xy=(%d,%d)", (_xyPending != MAKELPARAM(x, y)) ? "noop/cancel" : "commit", GET_X_LPARAM(_xyPending), GET_Y_LPARAM(_xyPending), x, y);
        break;
    default: ASSERT(0); break;
    }
}
...
{
#if DM_API
    TraceMsg(DM_DRAG, "cwb.dl: API xy=(%d,%d) fCommit=%d", x, y, fCommit);
#endif
    ...
    TraceMsg(DM_DRAG2, "cwb.dl: %s", EqualRect(&rcTmp, &_rcCapture) ? "noop/cancel" : "commit");
#endif
...
void CDockingBar::_MoveSizeHelper(UINT eModeNew, UINT eSideNew, HMONITOR hMonNew, POINT* ptTrans, RECT* rcFeed, BOOL fCommit, BOOL fMove)
{
    ...
    if (fCommit)
    {
        ...
        BOOL fChangeHide = (_fWantHide && (eSideNew != _uSide || eModeNew != _eMode || hMonNew != _hMon));
    ...
    if (fMove)
    {
        ...
        {
            MoveWindow(_hwnd, rcNew.left, rcNew.top, RECTWIDTH(rcNew), RECTHEIGHT(rcNew), TRUE);
        }
    }
...
void CDockingBar::_TrackSliding(int x, int y, RECT* rcFeed, BOOL fCommit, BOOL fMove)
{
    TraceMsg(DM_DRAG2, "cwb.ts: _TrackSliding(x=%d, y=%d, rcFeed=(%d,%d,%d,%d)(%dx%d), fCommit=%d, fMove=%d)", x, y,
        _PM(rcFeed,left), _PM(rcFeed,top), _PM(rcFeed,right), _PM(rcFeed,bottom), _PX(rcFeed,RECTWIDTH(*rcFeed)), _PX(rcFeed,RECTHEIGHT(*rcFeed)), fCommit, fMove);
