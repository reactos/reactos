...
//  Creates an instance of CITBarDropTarget. ptr is a pointer to the parent CInternetToolbar.
...
STDMETHODIMP CITBarDropTarget::DragEnter(IDataObject *pdtobj, DWORD grfKeyState, POINTL ptl, DWORD *pdwEffect)
{
    ...
    else if (_iTarget == TBIDM_HOME)
    {
        ...
        if (RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_SET_HOMEPAGE_RESTRICTION, 0, KEY_READ, &hkeyRest) == ERROR_SUCCESS)
        {
            if (RegQueryValueEx(hkeyRest, REGVAL_HOMEPAGE_RESTRICTION, NULL, NULL, (LPBYTE)&dwValue, &dwLen) == ERROR_SUCCESS && dwValue)
            {
    ...
    else if (fe.cfFormat = CF_TEXT, NOERROR == pdtobj->QueryGetData (&fe))
    {
        ...
        // We want to eventually pick through the text for an URL, but right now we just leave it unmolested.
    }
...
STDMETHODIMP CITBarDropTarget::Drop(IDataObject *pdtobj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
    ...
    if (_pdrop)
    {
        ...
        if (TBIDM_FAVORITES == _iTarget && SUCCEEDED(SHPidlFromDataObject(pdtobj, &pidl, NULL, 0)))
        {
    ...
    {
        if (TBIDM_HOME == _iTarget && SUCCEEDED(SHPidlFromDataObject(pdtobj, &pidl, NULL, 0)))
        {
        ...
        if (fSafe)
        {
            ...
            if (pdd)
            {
                ...
                if (FAILED(_GetURLData(pdtobj, _iDropType, pdd->szUrl, ARRAYSIZE(pdd->szUrl), pdd->szName)) || !SHCreateThread(ITBarDropThreadProc, pdd, 0, NULL))
                    LocalFree(pdd);
            }
        }
        DragLeave();
