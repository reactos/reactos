...
HRESULT CBandSite::QueryService(REFGUID guidService, REFIID riid, void **ppvObj)
{
    ...
    *ppvObj = NULL; // assume error
    ...
    else if (IsEqualIID(guidService, IID_IBandSite))
    {
        ...
        // CShellLink is a robust way to do this, so let's share one among all the bands.

        // NOTE: This is shared between bands, so if you request it
        // you must complete your use of it within the scope of your function call!
...
CBandSite::CBandSite(IUnknown* punkAgg) : SUPERCLASS(punkAgg)
{
    ...
    ASSERT(!_pbp);
    ASSERT(FALSE == _fCreatedBandProxy);
    SHRegGetUSValue(SZ_REGKEY_GLOBALADMINSETTINGS, SZ_REGVALUE_GLOBALADMINSETTINGS, NULL, (LPVOID)&dwData, &dwSize, FALSE, NULL, 0);
...
BOOL CBandSite::_IsBandDeleteable(DWORD dwBandID)
{
    ...
    if (FAILED(_pbsOuter->QueryBand(dwBandID, NULL, &dwState, NULL, 0)) || (dwState & BSSF_UNDELETEABLE)) {
        return FALSE;
    }
...
{
    ASSERT((NULL == pdwBandID && (UINT)-1 == uBand) || IS_VALID_WRITE_PTR(pdwBandID, DWORD));
...
HRESULT CBandSite::_SetBandStateHelper(DWORD dwBandID, DWORD dwMask, DWORD dwState)
{
    ...
    if (pbid) {
        ...
        // BUGBUG (kkahl): BSSF_UNDELETABLE cannot currently be modified with this interface.
...
HRESULT CBandSite::RemoveBand(DWORD dwBandID)
{
    ...
    if (pbid)
    {
        ...
        // receive cleanup notifications from its control.  *Then* delete the band item.
...
{
    if (dwBandID == -1) {// Close everything
        _CheckNotifyOnAddRemove(dwBandID, CNOAR_CLOSEBAR);
    }
    else {// Close just this band
        LPBANDITEMDATA pbid = _GetBandItemDataStructByID(dwBandID);
        USES_CONVERSION;
...
HRESULT CBandSite::_AddBandByID(IUnknown *punk, DWORD dwID)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        if (pbid)
        {
            ...
            // a NULL handler for the band's hwnd (preventing the band from getting any messages thereafter).
            ATOMICRELEASE(_pwehCache);
            _hwndCache = NULL;
        }
        else
...
int _ExecCallback(LPBANDITEMDATA pbid, void *pv)
{
    ...
    ped->hres = IUnknown_Exec(pbid->pdb, ped->pguidCmdGroup, ped->nCmdID, ped->nCmdexecopt, ped->pvarargIn, ped->pvarargOut);
    return 1;
}
...
HRESULT CBandSite::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANTARG *pvarargIn, VARIANTARG *pvarargOut)
{
    ...
    if (pguidCmdGroup == NULL) {
        ...
        return IUnknown_Exec(_ptbActive, pguidCmdGroup, nCmdID, nCmdexecopt, pvarargIn, pvarargOut);
    }
    ...
    if (SUCCEEDED(hresTmp) && HRESULT_CODE(hresTmp) > 0) {
        ...
        if (HRESULT_CODE(hresTmp) == OCTD_DOWN) {
            // down (singleton)
            hres = IUnknown_Exec(_ptbActive, pguidCmdGroup, nCmdID, nCmdexecopt, pvarargIn, pvarargOut);
        }
        else {
            // down (broadcast)
        ...
        else
        {
            ...
            for (i = _GetBandItemCount() - 1; i >= 0; i--)
            {
                ...
                if (pbid)
                {
                    if (pbid->pweh)
                    {
                        if (hwnd == HWND_BROADCAST || (pbid->pweh->IsWindowOwner(hwnd) == S_OK))
                        {
...
DWORD CBandSite::_GetWindowStyle(DWORD* pdwExStyle)
{
    *pdwExStyle = WS_EX_TOOLWINDOW;
    DWORD dwStyle = RBS_REGISTERDROP | RBS_VARHEIGHT | RBS_BANDBORDERS | WS_VISIBLE | WS_CHILD | WS_CLIPCHILDREN |
            ...
            if (EVAL(pbid))
            {
                ...
                if (IsEqualIID(*pnmon->piid, IID_IDropTarget))
                {
                    ...
                    if (pdt)
                    {
                        ...
                        fNeedReleasePdtBand = TRUE;// we've handed off pdtBand to pdt
                    }
...
HRESULT CBandSite::_OnBSCommand(int idCmd, DWORD idBandActive, LPBANDITEMDATA pbid)
{
    ...
    case BSIDM_IEAK_DISABLE_DDCLOSE:
        ...
        if (idBandActive != (DWORD)-1) {
            ...
            ToggleFlag(dwAdminSettings, dwFlag);// Toggle Setting.
            _SetAdminSettings(idBandActive, dwAdminSettings);// Set Menu Item Check Mark appropriately.
        }
        break;
    default:
        ASSERT(0);
    ...
    if (hmenu)
    {
        ...
        if (hmenuMe) {
            if (pbid) {
                ...
                CheckMenuItem(hmenuMe, BSIDM_SHOWTITLEBAND, pbid->fNoTitle ? MF_BYCOMMAND | MF_UNCHECKED : MF_BYCOMMAND | MF_CHECKED);
        ...
        if (!(_dwStyle & BSIS_NOCONTEXTMENU))
        {
            ...
            if (hmenuMe) {
                ...
                // nash:17821: don't disable when 0 bands (so user can easily get out of toasted mode)
...

        {
            ...
            if (!hwndParent)
                hwndParent = _hwnd;
            idCmd = TrackPopupMenu(hmenu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTALIGN, pt.x, pt.y, 0, hwndParent, NULL);
        }
...
HRESULT CBandSite::Load(IStream *pstm)
{
    ...
    if (hres == S_OK)
    {
        if (!(sfoo.cbSize == SIZEOF(SBandSite) && (sfoo.cbVersion == SBS_VERSION || sfoo.cbVersion == SBS_WOADMIN_VERSION)))
        {
            hres = E_FAIL;
        }
...
HRESULT CBandSite::HasFocusIO()
{
    ...
    //    as S_FALSE and give the focus to the next deserving dude in line.
...
HRESULT CBandSite::_CycleFocusBS(LPMSG lpMsg)
{
    ...
    else
    {
        ...
        // Loop til we find a tabstop and successfully activate it or til we run out of bands.
...
HRESULT CBandSite::Drop(IDataObject *pdtobj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect)
{
    ...
    else
    {
        ...
        else if ((*pdwEffect & (DROPEFFECT_COPY | DROPEFFECT_LINK)) && SUCCEEDED(SHPidlFromDataObject(pdtobj, &pidl, NULL, 0)))
        {
...
HRESULT _MergeBS(IDropTarget *pdtDst, IBandSite *pbsSrc)
{
    ...
    while (SUCCEEDED(pbsSrc->EnumBands(0, &idBand))) {
        ...
        // iBands 0..n.  if that's bogus i'm not sure how we can keep track of where we are.
...
void CBandSite::_BandInfoFromBandItem(REBARBANDINFO *prbbi, LPBANDITEMDATA pbid, BOOL fBSOnly)
{
    ...
    if (_dwStyle & BSIS_NOGRIPPER)
        ...
        // BSIS_AUTOGRIPPER...
        if (!(prbbi->fStyle & RBBS_FIXEDSIZE) && !(_dwMode & DBIF_VIEWMODE_FLOATING))
            prbbi->fStyle |= RBBS_GRIPPERALWAYS;
    }
