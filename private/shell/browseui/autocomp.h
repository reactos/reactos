...
// TODO List:
// 1. Convert AutoComplete to be a Free Threaded object and move it and all it's lists into MultiApartment model.
// 2. Get thread out of ThreadPool API in shlwapi.dll instead of creating thead our selves.
// 3. See if SHWaitForSendMessageThread() will cause a lock if bg thread is in SendMessage().
//    If so, make sure the bg loop's hard loop doesn't call SendMessage() in this case without first looking for QUIT message.
...
public:// Public Interfaces
    ...
    STDMETHODIMP get_accName(VARIANT varChild, BSTR  *pszName);// *** IAccessible ***
