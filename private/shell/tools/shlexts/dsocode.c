...
*  It null-terminates a space delimited string, returns its size and a pointer to the begining of next string
...
/*
* If the highest order bit of psot->dwOffset is set, then the value is a relative offset from the previous field;
*  otherwise, it is the actual field offset from the beginnig of the structure
...
/*
* The field size is calculated by substracting its offset from the next field's offset.
*  If the struct has unions, several "fields" might have the same offset, or a given table entry (i.e., a field) might have an
*  offset value greater than the offset value for the next entry (a union of two structures).
...
*/
{
     ...
     */
    if (pAddress != NULL) {
        Print ("Structure %s %#lx - Size: %#lx\n", pst->pszName, pAddress, pst->dwSize);
    } else {
        Print ("Structure %s - Size: %#lx\n", pst->pszName, pst->dwSize);
    }
