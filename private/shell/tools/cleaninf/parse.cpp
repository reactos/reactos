        ...
        if (0 == cNestedComment)
        {
            ...
            if (_bSkipWhitespace && !bDblQuotes && !bSingleQuotes)
            {
                ...
                else if (IS_NEWLINE(_ch))
                {
                    ...
                    // we should at least replace the newline with a space so tokens don't get appended accidentally.

                    // The javascript engine has a line-length limit.
                    // So don't replace a newline with a space.
                    if (!_bFirstChar)
                        _WriteChar('\n');
