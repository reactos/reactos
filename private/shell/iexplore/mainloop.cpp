...
HWND CreateAppCompatWindow(HINSTANCE hinst)
{
    ...
    hwnd = CreateWindowEx(WS_EX_TOOLWINDOW, c_szClass, TEXT(""), 0, 0x00007FF0, 0x00007FF0, 0, 0, NULL, hmenu, hinst, NULL);
...
void SetCompatModeUserAgentString(void)
{
    ...
    {
        RegSetValueEx( hkey, szcompat, 0, REG_BINARY, (LPBYTE)NULL, 0);
        RegCloseKey(hkey);
    }
}
...
void CheckForSP3RSAOverwrite( void )
{
    ...
    if( GetVersionEx(&osVer) && (osVer.dwPlatformId == VER_PLATFORM_WIN32_NT) && (osVer.dwMajorVersion == 4))
    {
...
int WinMainT(HINSTANCE hinst, HINSTANCE hPrevInstance, LPTSTR lpszCmdLine, int nCmdShow)
{
    ...
    {
        // Integrated browser mode - package up a bunch of data into a COPYDATASTRUCT, and send it to the desktop window via SendMessage(WM_COPYDATA).
        COPYDATASTRUCT cds;
        cds.dwData = nCmdShow;
        ...
        if (pwszBuffer)
        {
            ...
            if (hEventReady)
            {
                // Third piece of data is the event to fire when the browser window closes.
                // This is optional, we only create it (and wait for it) when there are command line parameters.
                HANDLE hEventDead = NULL;

                // The hard part is to figure out when we need the command line and when we don't.
                // For the most part if there is a command line we will assume that we will need it (potentially) we could look for the -nowait flag.
                // But then there are others like NetManage ECCO Pro who do their equiv of ShellExecute who don't pass a command line...
                ...
                if (hEventDead || !*lpszCmdLine || fNowait)
                {
                    ...
                    if (iRet)
                    {
                        ...
                        if (hEventDead)
                        {
                            ...
                            do
                            {
                                // Calling MsgWait... will cause any threads blocked on WaitForInputIdle(IEXPLORE) to resume execution.
                                // This is fine because the browser has already registered its DDE servers by now.
                                dwRet = MsgWaitForMultipleObjects(1, &hEventDead, FALSE, INFINITE, QS_ALLINPUT);
                                if (dwRet == WAIT_OBJECT_0)
                                {
                                    DestroyWindow(hwnd);// Kill our helper window cleanly too.
                                }
...
InThisProcess:
        ...
        if (fEval)
        {
            WarnCompatMode(hinst);// Set "compat" mode user agent
...
// this API is for Office to retrieve our UI language when they are hosted by iexplore.
