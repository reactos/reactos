 ...
 * GACF_INCREASE does not work for ATOK9, because its _DATA is too big and there is no rool to increase Stack.
*/
{
    UINT os[4];
    UINT uLen = 0;
    ...
    if (!lpfnImmGetAppCompatFlag || !((*lpfnImmGetAppCompatFlag)(0) & IMECOMPAT_USEALTSTKFORSHLEXEC))
        return ShellExecute32(hwnd, (LPSTR)lpszOp, (LPSTR)lpszFile, (LPSTR)lpszParams, (LPSTR)lpszDir, wShowCmd);
    ...
    if (lpsep = MAKELP(GlobalAlloc(GPTR, ALT_STACK + uLen + sizeof(SHLEXECPARAM)),0))
    {
        ...
        hRet =  ShellExecute32(lpsep->hwnd, lpsep->lpsz[0], lpsep->lpsz[1], lpsep->lpsz[2], lpsep->lpsz[3], lpsep->wShowCmd);
