    ...
    LPARAM lP)
{
    ...
    if (pPSD->lpfnPagePaintHook)
    {
        ...
        if ((*pPSD->lpfnPagePaintHook)(hWnd,
                                       WM_PSD_PAGESETUPDLG,
                                       MAKELONG(pPI->wPaper, wFlags),
                                       (LPARAM)pPSD) ||
                                       (*pPSD->lpfnPagePaintHook)(hWnd,
                                                                  WM_PSD_FULLPAGERECT,
                                                                  (WPARAM)hDC,
    ...
    SkipMarginRectangle:
        ...
        if (!(pPSD->Flags & PSD_DISABLEPAGEPAINTING))
        {
            psGreekText = LocalAlloc(LPTR,
                                     10 * sizeof(szGreekText));
            for (i = 0; i < 10; i++)
            {
                CopyMemory(&psGreekText[i * (sizeof(szGreekText) / sizeof(szGreekText[0]))],
                           szGreekText,
                           sizeof(szGreekText));
            }
            aRt.left++;
...
//  in the print info structure.

//  NOTE: Modifies pPI->szDefaultPrinter
    ...
    PPRINTINFO pPI)
{
    ...
    if (pPI->szDefaultPrinter[0] != CHAR_NULL)
    {
        return;
    }
    ...
    if (GetProfileString(szTextWindows,
                         szTextDevice,
                         szTextNull,
                         pPI->szDefaultPrinter,
                         MAX_PRINTERNAME))
    {
        lpsz = pPI->szDefaultPrinter;

        while (*lpsz != CHAR_COMMA)
        {
            if (!*lpsz++)
            {
                pPI->szDefaultPrinter[0] = CHAR_NULL;
    ...
    GetDefaultFromRegistry:
        ...
        dwSize = sizeof(pPI->szDefaultPrinter);

        if (RegOpenKeyEx(HKEY_CURRENT_USER, szRegistryPrinter, 0, KEY_READ, &hPrinterKey) == ERROR_SUCCESS)
        {
            RegQueryValueEx(hPrinterKey, szRegistryDefaultValueName, NULL, NULL, (LPBYTE)(pPI->szDefaultPrinter), &dwSize);
    ...
    PPRINTINFO pPI)
{
        ...
        (hCtl = GetDlgItem(hDlg, ID_PRINT_E_COPIES)))
    {
        ...
        SendMessage(hCtl, EM_LIMITTEXT, COPIES_EDIT_SIZE, 0);
        lpEditNumOnlyProc =
            (WNDPROC)SetWindowLong(hCtl,
                                   GWL_WNDPROC,
                                   (DWORD)PrintEditNumberOnlyProc);

        if (GetDlgItem(hDlg, ID_PRINT_G_COPIES))
        {
    ...
    if (!(pPI->pPSD))
    {
        if (hCtl = GetDlgItem(hDlg, ID_PRINT_E_FROM))
        {
            ...
            SendMessage(hCtl, EM_LIMITTEXT, PAGE_EDIT_SIZE, 0);
            lpEditNumOnlyProc =
                (WNDPROC)SetWindowLong(hCtl,
                                       GWL_WNDPROC,
                                       (DWORD)PrintEditNumberOnlyProc);
    ...
    if (pPD->lpfnPrintHook)
    {
        ...
        if (pPI->ApiType == COMDLG_ANSI)
        {
            ...
            ThunkPrintDlgW2A(pPI);
            dwHookRet = (*pPD->lpfnPrintHook)(hDlg,
                                              WM_INITDIALOG,
                                              wParam,
                                              (LONG)pPI->pPDA);
            if (dwHookRet)
    ...
    PPRINTINFO pPI)
{
    ...
    {
        lpStaticProc = (WNDPROC)SetWindowLong(hCtl,
                                              GWL_WNDPROC,
                                              (DWORD)PrintPageSetupPaintProc);
    }
    ...
    if (pPD->lpfnSetupHook)
    {
        ...
        if (pPI->ApiType == COMDLG_ANSI)
        {
            ThunkPrintDlgW2A(pPI);
            TransferPDA2PSD(pPI);
            dwHookRet = (*pPD->lpfnSetupHook)(hDlg,
                                              WM_INITDIALOG,
                                              wParam,
                                              (pPI->pPSD)
                                              ? (LONG)pPI->pPSD
    ...
    BOOL fResetContent)
{
    ...
    if (hCtl = GetDlgItem(hDlg, ID_SETUP_C_SIZE))
    {
        ...
        {
            Count = SendMessage(hCtl, CB_GETCOUNT, 0, 0);
            while (Count != 0)
            {
                Count--;
    ...
    if (GetDlgItem(hDlg, ID_SETUP_R_PORTRAIT))
    {
        PrintSetOrientation(hDlg,
                            ...
                            : ID_SETUP_R_LANDSCAPE,
                            pDM->dmOrientation
                            + ID_SETUP_R_PORTRAIT - DMORIENT_PORTRAIT);
    }
    ...
    UINT Id)
{
        ...
        (pDM = GlobalLock(pPD->hDevMode)))
    {
        ...
        PutInfoInDevMode:
            ...
            sMaxCopies = (WORD)(*WinSpool_DeviceCapabilities)(
                pPI->pCurPrinter->pPrinterName,
    ...
    PPRINTINFO pPI)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (hEdt = GetDlgItem(hDlg, ids[i]))
        {
            ...
            lpEditMarginProc =
                (WNDPROC)SetWindowLong(hEdt,
                                       GWL_WNDPROC,
                                       (DWORD)PrintEditMarginProc);
        }
    }
    ...
    LONG lValue)
{
    ...
    if (hEdt = GetDlgItem(hDlg, Id))
    {
        ...
        //  Attach the measure designator if there's room.

        if (lstrlen(szText) + cchIntlMeasure <= MARGIN_EDIT_SIZE)
        {
            lstrcat(szText, cIntlMeasure);
        }
    ...
    SHORT nQuality)
{
    ...
    if (pPD && pPD->hDevMode && pPD->hDevNames)
    {
        ...
        LPDEVNAMES pDN = GlobalLock(pPD->hDevNames);

        LPTSTR psDriver = (LPTSTR)pDN + pDN->wDriverOffset;
        LPTSTR psDevice = (LPTSTR)pDN + pDN->wDeviceOffset;
        LPTSTR psPort = (LPTSTR)pDN + pDN->wOutputOffset;

        dw = (*WinSpool_DeviceCapabilities)(psDevice,
                                            psPort,
                                            DC_ENUMRESOLUTIONS,
                                            NULL,
                                            NULL);
        ...
        if (pLong = GlobalLock(hPrnQ))
        {
            ...
            for (nStringID = 0, i = (SHORT)(LOWORD(dw) - 1); i >= 0; i--)
            {
                ...
                SendMessage(hCmb, CB_INSERTSTRING, 0, (LONG)(LPTSTR)szBuf);
    ...
    PPRINTINFO pPI)
{
        ...
        (pDM = GlobalLock(hDevMode)))
    {
        ...
        if ((*WinSpool_DocumentProperties)(
            hDlg,
            pPI->hCurPrinter,
            (pPI->pCurPrinter)
            ? pPI->pCurPrinter->pPrinterName
    ...
    PPRINTINFO pPI)
{
    ...
    if (pPI->hCurPrinter)
    {
        (*WinSpool_ClosePrinter)(pPI->hCurPrinter);
        pPI->hCurPrinter = 0;
    }
    ...
    (*WinSpool_OpenPrinter)(pPI->pCurPrinter->pPrinterName,
                            &pPI->hCurPrinter,
                            NULL);
    ...
    cbSize = (*WinSpool_DocumentProperties)(hDlg,
                                            pPI->hCurPrinter,
                                            pPI->pCurPrinter->pPrinterName,
                                            NULL,
                                            NULL,
    ...
    if (cbSize > 0)
    {
        if (pPD->hDevMode)
        {
            pPD->hDevMode = GlobalReAlloc(pPD->hDevMode, cbSize, GHND);
        }
        else
        {
            pPD->hDevMode = GlobalAlloc(GHND, cbSize);
        }
        if (pPD->hDevMode && (pDM = GlobalLock(pPD->hDevMode)))
        {
            (*WinSpool_DocumentProperties)(hDlg,
                                           pPI->hCurPrinter,
                                           pPI->pCurPrinter->pPrinterName,
                                           pDM,
                                           pDM,
    ...
    PPRINTINFO pPI)
{
    ...
    if (!GetDlgItem(hDlg, ID_BOTH_S_STATUS))
    {
        return;
    }
    ...
    if (pPI->pCurPrinter->pLocation && pPI->pCurPrinter->pLocation[0])
    {
        ...
        {
            lstrcat(szText, szSeparator);
            lstrcat(szText, pPI->pCurPrinter->pPortName);
        }
    }
...
//  Sets the current printer to point to the printer with the name
//  lpsPrinterName.



VOID PrintSearchForPrinter(
    PPRINTINFO pPI,
    ...
    PPRINTINFO pPI)
{
    ...
    for (ctr = 0; ctr < pPI->cPrinters; ctr++)
    {
        ...
        iResult = (*WinSpool_ExtDeviceMode)(hDlg,
                                            NULL,
                                            NULL,
                                            szPrinterNameA,
                                            NULL,
        ...
        //  about freeing the current contents of pPrinter[ctr].pDevMode
    ...
    BOOL bEnumPrinters)
{
    ...
    Print_Enumerate:
        ...
        if (!(*WinSpool_EnumPrinters)(PRINTER_ENUM_LOCAL | PRINTER_ENUM_CONNECTIONS,
                                      NULL,
                                      2,
                                      NULL,
                                      0,
        ...
        if (lpsPrinterToSelect)
        {
            PrintSearchForPrinter(pPI, lpsPrinterToSelect);
        }
        ...
        else
        {
            ...
            for (ctr = 0; ctr < pPI->cPrinters; ctr++)
            {
                ...
                if ((*WinSpool_OpenPrinter)(pPI->pCurPrinter->pPrinterName,
                                            &pPI->hCurPrinter,
                                            NULL))
                {
                    break;
    ...
    PPRINTINFO pPI)
{
    ...
    else
    {
        ...
        if ((pPI->ProcessVersion >= 0x40000) ||
            (!GetProfileString(szTextDevices,
                               ...
                               szBuffer,
                               MAX_PATH)) ||
                               (!(pPortName = _tcschr(szBuffer, CHAR_COMMA))) ||
            (!((++pPortName)[0])))
#endif
        {
    ...
    if (pPD->hDevNames)
    {
        pPD->hDevNames = GlobalReAlloc(pPD->hDevNames, cbDevNames, GHND);
    }
    else
    {
    ...
    HANDLE hDevMode)
{
    ...
    cbNeeded = (*WinSpool_DocumentProperties)(hDlg,
                                              hPrinter,
                                              lpsDeviceName,
                                              (PDEVMODE)NULL,
                                              (PDEVMODE)NULL,
    ...
    if (cbNeeded > 0)
    {
        if (hDevMode)
        {
            hDevMode = GlobalReAlloc(hDevMode, cbNeeded, GHND);
        }
        else
        {
        ...
        {
            if ((*WinSpool_DocumentProperties)(hDlg,
                                               hPrinter,
                                               lpsDeviceName,
                                               pDM,
                                               NULL,
...
//  Assumes the PR_PRINTOFILE flag is appropriately set.
    ...
    LPDEVMODE pDM)
{
    ...
    HDC hDC;
    int iHeight;
    PRINTER_INFO_2 *pPrinter = pPI->pCurPrinter;


    pPI->dwRotation = (*WinSpool_DeviceCapabilities)(pPrinter->pPrinterName,
                                                     pPrinter->pPortName,
                                                     DC_ORIENTATION,
                                                     NULL,
                                                     pDM);
    ...
    UINT uiNewId)
{
    ...
    //  The rest of this routine only pertains to the page setup dialog.
    ...
    if (hWndSize = GetDlgItem(hDlg, ID_SETUP_C_SIZE))
    {
        PRINTER_INFO_2* pPrinter = pPI->pCurPrinter;
        DWORD dwNumber;
        LPWORD lpPapers;
        ...
        dwNumber = (*WinSpool_DeviceCapabilities)(pPrinter->pPrinterName,
                                                  pPrinter->pPortName,
                                                  DC_PAPERS,
                                                  NULL,
                                                  pDM);
                                   ...
                                   (sizeof(WORD) + sizeof(POINT)) * 2)))
        {
            ...
            (*WinSpool_DeviceCapabilities)(pPrinter->pPrinterName,
                                           pPrinter->pPortName,
                                           DC_PAPERS,
                                           (LPTSTR)lpPapers,
                                           pDM);
            ...
            nInd = SendMessage(hWndSize, CB_GETCURSEL, 0, 0);
    ...
    LPDEVMODE pDM)
{
    ...
    if (hCtl = GetDlgItem(hDlg, ID_SETUP_I_DUPLEX))
    {
        ...
        {
            ShowWindow(hCtl, SW_HIDE);
            SendMessage(hCtl, STM_SETICON, (LONG)hIconPDuplexNone, 0L);
            ShowWindow(hCtl, SW_SHOW);
        }
    }
}
    ...
    UINT nRad)
{
    ...
    if (hCtl = GetDlgItem(hDlg, ID_SETUP_I_DUPLEX))
    {
        ...
        ShowWindow(hCtl, SW_HIDE);
        SendMessage(hCtl, STM_SETICON, (LONG)hDuplexIcon, 0L);
        ShowWindow(hCtl, SW_SHOW);
    }
}
    ...
    WORD fwCap2)
{
    ...
    cStr1 = (*WinSpool_DeviceCapabilities)(pPrinter->pPrinterName,
                                           pPrinter->pPortName,
                                           fwCap1,
                                           NULL,
                                           pDM);

    cStr2 = (*WinSpool_DeviceCapabilities)(pPrinter->pPrinterName,
                                           pPrinter->pPortName,
                                           fwCap2,
                                           NULL,
                                           pDM);
    ...
    //  Check for error from WinSpool_DeviceCapabilities calls.  If either
    ...
    if (fFill)
    {
        ...
        if (lpsOut1 && lpwOut2)
        {
            cRet1 = (*WinSpool_DeviceCapabilities)(pPrinter->pPrinterName,
                                                   pPrinter->pPortName,
                                                   fwCap1,
                                                   (LPTSTR)lpsOut1,
                                                   pDM);

            cRet2 = (*WinSpool_DeviceCapabilities)(pPrinter->pPrinterName,
                                                   pPrinter->pPortName,
                                                   fwCap2,
                                                   (LPTSTR)lpwOut2,
                                                   pDM);
            ...
            if ((cRet1 == cStr1) && (cRet2 == cStr2))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //  Set the SOURCE to "Print Manager Setting"
                        //  if it exists.
...
VOID PrintOpenPrinter(
    ...
    LPTSTR pPrinterName)
{
    if ((*WinSpool_OpenPrinter)(pPrinterName, &pPI->hCurPrinter, NULL))
    {
...
LPPRINTER_INFO_2 PrintGetPrinterInfo2(
    HANDLE hPrinter)
{
    LPPRINTER_INFO_2 pPrinter = NULL;
    DWORD cbPrinter = 0;
    ...
    if (!(*WinSpool_GetPrinter)(hPrinter, 2, NULL, 0, &cbPrinter) &&
...
VOID FreePrinterArray(
    PPRINTINFO pPI)
{
    LPPRINTER_INFO_2 pPrinters = pPI->pPrinters;
    DWORD dwCount;
    ...
    LPPRINTDLGA pPDA)
{
    ...
    if ((pPDA->hDevMode) && (pDMA = GlobalLock(pPDA->hDevMode)))
    {
        pPDW->hDevMode = GlobalAlloc(GHND,
                                     sizeof(DEVMODE) + pDMA->dmDriverExtra);
    ...
    PPRINTINFO pPI)
{
    ...
    if (pPDW->hDevNames)
    {
        ...
        if (pPDA->hDevNames)
        {
            pPDA->hDevNames = GlobalReAlloc(pPDA->hDevNames, cbLen, GHND);
        }
        else
        {
