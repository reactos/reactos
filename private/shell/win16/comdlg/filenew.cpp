...
#define LPIDL_GetIDList(_pida,n) (LPCITEMIDLIST)(((LPBYTE)(_pida)) + (_pida)->aoffset[n])
...
#define StoreBrowser(_hwnd, _pbrs) SetWindowLong(_hwnd, DWL_USER, (DWORD)_pbrs);
...
int GetControlsBottom(HWND hDlg, HWND hwndExclude);
BOOL CALLBACK OpenDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
HRESULT ICoCreateInstance(REFCLSID rclsid, REFIID riid, LPLPVOID ppv);
...
WORD CD_SendShareMsg(HWND hwnd, LPTSTR szFile, UINT ApiType)
{
#ifdef UNICODE
    if (ApiType == COMDLG_ANSI)
    {
        ...
        WideCharToMultiByte( CP_ACP, 0, szFile, -1, szFileA, MAX_PATH + 1, NULL, NULL );
        return ( (WORD)SendMessage( hwnd, msgSHAREVIOLATIONA, 0, (LONG)(LPSTR)(szFileA) ) );
    }
    else
#endif
    {
        return ( (WORD)SendMessage( hwnd, msgSHAREVIOLATIONW, 0, (LONG)(LPTSTR)(szFile) ) );
    }
}
...
LRESULT CD_SendOKMsg(HWND hwnd, LPOPENFILENAME pOFN, LPOPENFILEINFO pOFI)
{
    LRESULT Result;
...
LRESULT CD_SendLBChangeMsg(HWND hwnd, int Id, short Index, short Code, UINT ApiType)
{
#ifdef UNICODE
    if (ApiType == COMDLG_ANSI)
    {
...
#define CD_SendShareNotify(_hwndTo, _hwndFrom, _szFile, _pofn, _pofi) (WORD)SendOFNotify(_hwndTo, _hwndFrom, CDN_SHAREVIOLATION, _szFile, _pofn, _pofi)
#define CD_SendHelpNotify(_hwndTo, _hwndFrom, _pofn, _pofi) SendOFNotify(_hwndTo, _hwndFrom, CDN_HELP, NULL, _pofn, _pofi)
#define CD_SendOKNotify(_hwndTo, _hwndFrom, _pofn, _pofi) SendOFNotify(_hwndTo, _hwndFrom, CDN_FILEOK, NULL, _pofn, _pofi)
#define CD_SendTypeChangeNotify(_hwndTo, _hwndFrom, _pofn, _pofi) SendOFNotify(_hwndTo, _hwndFrom, CDN_TYPECHANGE, NULL, _pofn, _pofi)
#define CD_SendInitDoneNotify(_hwndTo, _hwndFrom, _pofn, _pofi) SendOFNotify(_hwndTo, _hwndFrom, CDN_INITDONE, NULL, _pofn, _pofi)
#define CD_SendSelChangeNotify(_hwndTo, _hwndFrom, _pofn, _pofi) SendOFNotify(_hwndTo, _hwndFrom, CDN_SELCHANGE, NULL, _pofn, _pofi)
#define CD_SendFolderChangeNotify(_hwndTo, _hwndFrom, _pofn, _pofi) SendOFNotify(_hwndTo, _hwndFrom, CDN_FOLDERCHANGE, NULL, _pofn, _pofi)


LRESULT SendOFNotify(HWND hwndTo, HWND hwndFrom, UINT code, LPTSTR szFile, LPOPENFILENAME pOFN, LPOPENFILEINFO pOFI)
{
    OFNOTIFY ofn;
    ...
    if (pOFI->ApiType == COMDLG_ANSI)
    {
        ...
        //  For apps that side-effect pOFNA stuff and expect it to be preserved through dialog exit, update internal struct after the hook proc is called.
        ThunkOpenFileNameA2W(pOFI);
...
BOOL TEMPSTR::StrCpy(LPCTSTR pszText)
{
    if (!pszText)
    {
        StrSize(0);
...
BOOL TEMPSTR::StrCat(LPCTSTR pszText)
{
    if (!(LPTSTR)*this)
    {
        //  This should 0 init.
...
int _cdecl CDMessageBox(HWND hwndParent, UINT idText, UINT uFlags, ...)
{
...
VOID InvalidFileWarning(HWND hWnd, LPTSTR szFile, int wErrCode)
{
...
void GetControlRect(HWND hwndDlg, UINT idOldCtrl, LPRECT lprc)
{
...
//  WARNING WARNING WARNING:  Some code in the new look depends on hidden controls remaining where they originally were, even when disabled,
//  because they're templates for where to create new controls (the toolbar, or the main list).  Therefore, HideControl() must not MOVE the control
//  being hidden - it may only hide and disable it.  If this needs to change, there must be a separate hiding subroutine used for template controls.
void HideControl(HWND hwndDlg, UINT idControl)
{
...
//  SelectEditText
void SelectEditText(HWND hwndDlg)
{
...
public:
    ...
    MYLISTBOXITEM( MYLISTBOXITEM *pParentItem, IShellFolder *psf, LPCITEMIDLIST pidl, DWORD c, DWORD f );
...
MYLISTBOXITEM::MYLISTBOXITEM(MYLISTBOXITEM *pParentItem, IShellFolder *psf, LPCITEMIDLIST pidl, DWORD c, DWORD f)
{
    cIndent = c;
    dwFlags = f;
...
BOOL ShouldIncludeObject(LPSHELLFOLDER psfParent, LPCITEMIDLIST pidl)
{
    BOOL fInclude = FALSE;
...
BOOL IsContainer(IShellFolder *psf, LPCITEMIDLIST pidl)
{
    DWORD dwAttrs = SFGAO_FOLDER;

    return (SUCCEEDED(psf->GetAttributesOf(1, &pidl, &dwAttrs)) && (dwAttrs & SFGAO_FOLDER));
}


BOOL IsLink(IShellFolder *psf, LPCITEMIDLIST pidl)
{
    DWORD dwAttrs = SFGAO_LINK;

    return (SUCCEEDED(psf->GetAttributesOf(1, &pidl, &dwAttrs)) && (dwAttrs & SFGAO_LINK));
}
...
IShellFolder *MYLISTBOXITEM::GetShellFolder()
{
    if (!psfSub)
    {
        if (FAILED(psfParent->BindToObject( pidlThis, NULL, IID_IShellFolder, (LPLPVOID)&psfSub )))
        {
            psfSub = NULL;
        }
        else
...
public:
    ...
    UINT FindNameInView(LPTSTR pszFile, OKBUTTONFLAGS Flags, LPTSTR pszPathName, int nFileOffset, int nExtOffset, int *pnErrCode, BOOL bTryAsDir = TRUE);
...
CFileOpenBrowser::CFileOpenBrowser(HWND hDlg, BOOL fIsSaveAs)
    : cRef(1), iCurrentLocation(-1), pCurrentLocation(NULL), psv(NULL), hwndDlg(hDlg), hwndView(NULL), psfCurrent(NULL), bSave(fIsSaveAs), iComboIndex(-1), psl(0)
{
    RECT rcToolbar;
    ...
    hwndToolbar = CreateToolbarEx( hwndDlg,
                                   TBSTYLE_TOOLTIPS | WS_CHILD | CCS_NORESIZE | CCS_NODIVIDER, // | WS_CLIPSIBLINGS,
                                   IDC_TOOLBAR,
                                   12,
                                   HINST_COMMCTRL,
    ...
    if (hwndToolbar)
    {
        ::SetWindowPos( hwndToolbar, NULL, rcToolbar.left, rcToolbar.top, rcToolbar.right - rcToolbar.left, rcToolbar.bottom - rcToolbar.top, SWP_NOACTIVATE | SWP_NOZORDER | SWP_SHOWWINDOW );
    }

    Shell_GetImageLists(NULL, &himl);

    //  This setting could change on the fly, but I really don't care about that rare case.
    SHELLSTATE ss;
...
HRESULT STDMETHODCALLTYPE CFileOpenBrowser::QueryInterface(REFIID riid, LPVOID * ppvObj)
{
...
STDMETHODIMP CFileOpenBrowser::BrowseObject(LPCITEMIDLIST pidl, UINT wFlags)
{
...
{
    //  BUGBUG: We should implement this so there is some persistence for the file open dailog.
    return (E_NOTIMPL);
}
...
STDMETHODIMP CFileOpenBrowser::GetControlWindow(UINT id, HWND *lphwnd)
{
    if (id == FCW_TOOLBAR)
    {
        *lphwnd = hwndToolbar;
...
STDMETHODIMP CFileOpenBrowser::SendControlMsg(UINT id, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT *pret)
{
    LRESULT lres = 0;
    if (id == FCW_TOOLBAR)
    {
...
STDMETHODIMP CFileOpenBrowser::QueryActiveShellView(LPSHELLVIEW * ppsv)
{
    if (psv)
    {
        *ppsv = psv;
...
STDMETHODIMP CFileOpenBrowser::OnDefaultCommand(struct IShellView *ppshv)
{
    if (ppshv != psv)
    {
        return (E_INVALIDARG);
...
void CFileOpenBrowser::SetCurrentFilter(LPCTSTR pszFilter, OKBUTTONFLAGS Flags)
{
    LPTSTR lpNext;
    ...
    for (lpNext = szLastFilter; nLeft > 0; )
    {
        ...
        *lpSemiColon = CHAR_NULL;
        ...
        if (lpDot && *(lpDot + 1) && !IsWild(lpDot))
        {
            ...
            if (cTemp != CHAR_NULL)
            {
                hmemcpy( lpSemiColon + 2, lpSemiColon + 1, lstrlen(lpSemiColon + 1) * sizeof(TCHAR) );
            }
...
HWND GetFocusedChild(HWND hwndDlg, HWND hwndFocus)
{
    HWND hwndParent;
...
HRESULT CFileOpenBrowser::SwitchView(IShellFolder *psfNew, LPCITEMIDLIST pidlNew, FOLDERSETTINGS *pfs)
{
    IShellView *psvNew;
    ...
    HRESULT hres = psfNew->CreateViewObject( hwndDlg, IID_IShellView, (LPLPVOID)&psvNew );
    if (SUCCEEDED(hres))
    {
        IShellView *psvOld;
        HWND hwndNew;
        ...
        //  we should leave in SETREDRAW stuff to minimize flicker in case this fails.
        BOOL bLocked = LockWindowUpdate(hwndDlg);

        //  We need to kill the current psv before creating the new one in case
        //  the current one has a background thread going that is trying to call us back (IncludeObject).
        if (psvOld)
        {
        ...
        //  subdialog while creating the view window; drawing will be enabled after the Z-order has been set properly.
        if (hSubDlg)
        {
        ...
        //  psv must be set before creating the view window since we validate it on the IncludeObject callback.
        psv = psvNew;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            if (SUCCEEDED(psvNew->GetItemObject( SVGIO_BACKGROUND, IID_IContextMenu, (LPVOID *)&pcm )))
            {
                bNewFolder = pcm->GetCommandString( (ULONG)CMDSTR_NEWFOLDER, GCS_VALIDATE, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
                if (FAILED(bNewFolder))
                {
                    ...
                    //  Unicode or Ansi.
#ifdef UNICODE
                    bNewFolder = pcm->GetCommandString((ULONG)CMDSTR_NEWFOLDERA, GCS_VALIDATEA, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
#else
                    bNewFolder = pcm->GetCommandString((ULONG)CMDSTR_NEWFOLDERW, GCS_VALIDATEW, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
#endif
                }
                bNewFolder = (bNewFolder == S_OK);

                bViewList = pcm->GetCommandString( (ULONG)CMDSTR_VIEWLIST, GCS_VALIDATE, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
                if (FAILED(bViewList))
                {
                    //  The szTemp parameter is not used in GCS_VALIDATE,
                    //  so there is no need to convert the string to either
                    //  Unicode or Ansi.
#ifdef UNICODE
                    bViewList = pcm->GetCommandString( (ULONG)CMDSTR_VIEWLISTA, GCS_VALIDATEA, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
#else
                    bViewList = pcm->GetCommandString( (ULONG)CMDSTR_VIEWLISTW, GCS_VALIDATEW, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
#endif
                }
                bViewList = (bViewList == S_OK);

                bViewDetails = pcm->GetCommandString( (ULONG)CMDSTR_VIEWDETAILS, GCS_VALIDATE, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
                if (FAILED(bViewDetails))
                {
                    ...
                    //  Unicode or Ansi.
#ifdef UNICODE
                    bViewDetails = pcm->GetCommandString((ULONG)CMDSTR_VIEWDETAILSA, GCS_VALIDATEA, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
#else
                    bViewDetails = pcm->GetCommandString((ULONG)CMDSTR_VIEWDETAILSW, GCS_VALIDATEW, NULL, (LPSTR)szTemp, ARRAYSIZE(szTemp) );
#endif
                }
            ...
            SetWindowPos( hwndNew, GetDlgItem(hwndDlg, lst1), 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE );

            //  Give it the right window ID for WinHelp.
            SetWindowLong(hwndNew, GWL_ID, lst2);

            ::RedrawWindow( hwndView, NULL, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_ALLCHILDREN | RDW_UPDATENOW );
            if (bViewFocus)
            {
                ::SetFocus(hwndView);
            }
...
void JustGetToolTipText(UINT idCommand, LPTOOLTIPTEXT pTtt)
{
    if (!LoadString( ::g_hinst, idCommand + MH_TOOLTIPBASE, pTtt->szText, ARRAYSIZE(pTtt->szText) ))
    {
        *pTtt->lpszText = 0;
    }
}
...
LRESULT CFileOpenBrowser::OnNotify(LPNMHDR pnm)
{
    LRESULT lres = 0;
    ...
    switch (pnm->code)
    {
        ...
        case ( NM_ENDWAIT ) :
        {
            ...
            //  What we really want is for the user to simulate a mouse move/setcursor.
...
void GetViewItemText(IShellFolder *psf, LPCITEMIDLIST pidl, LPTSTR pBuf, UINT cchBuf, BOOL fPath = TRUE)
{
    STRRET sr;

    if (SUCCEEDED(psf->GetDisplayNameOf( pidl, fPath ? SHGDN_INFOLDER | SHGDN_FORPARSING : SHGDN_INFOLDER, &sr )))
    {
        LPTSTR pszName = NULL;
        ...
        switch (sr.uType)
        {
            ...
            case ( STRRET_CSTRA ) :
            {
                ...
                if (pszName)
                {
                    MultiByteToWideChar( CP_ACP, 0, sr.cStr, cchLen, pszName, cchLen );
                }
                else
                {
                    break;
            ...
            case ( STRRET_OFFSETA ) :
            {
                ...
                if (pszName)
                {
                    MultiByteToWideChar( CP_ACP, 0, lpText, cchLen, pszName, cchLen );
                }
                else
                {
                    break;
        ...
        switch (sr.uType)
        {
            case ( STRRET_OLESTR ) :
            {
                ...
                OleStrToStrN( sr.cStr, ARRAYSIZE(sr.cStr), pwszDisplayName, (UINT)(-1) );
...
MYLISTBOXITEM *GetListboxItem(HWND hCtrl, int iItem)
{
    MYLISTBOXITEM *p = (MYLISTBOXITEM *)SendMessage( hCtrl, CB_GETITEMDATA, iItem, NULL );
...
HRESULT _ReleaseStgMedium(LPSTGMEDIUM pmedium)
{
...
void CFileOpenBrowser::SetSaveButton(UINT idSaveButton)
{
    PostMessage(hwndDlg, CDM_SETSAVEBUTTON, idSaveButton, 0);
}


void CFileOpenBrowser::RealSetSaveButton(UINT idSaveButton)
{
    MSG msg;

    if (PeekMessage( &msg, hwndDlg, CDM_SETSAVEBUTTON, CDM_SETSAVEBUTTON, PM_NOREMOVE ))
    {
        //  There is another SETSAVEBUTTON message in the queue, so blow off this one.
        return;
    }
    ...
    if (bSave)
    {
        ...
        //  Load the string if not the "Save" string or there is no app-specified default.
...
void CFileOpenBrowser::SetEditFile(LPTSTR pszFile, BOOL bShowExt, BOOL bSaveNullExt)
{
    ...
    bUseHideExt = (LPTSTR)pszHideExt ? (bSaveNullExt ? TRUE : bHasHiddenExt) : FALSE;
}


LPTSTR FindEOF(LPTSTR pszFiles)
{
    BOOL bQuoted;
    ...
    for ( ; ; pszFiles = CharNext(pszFiles))
    {
        switch (*pszFiles)
        {
            ...
            case ( CHAR_QUOTE ) :
            {
                //  Note we only support '"' at the very beginning and very end of a file name.
                return (pszFiles);
            }
            default :
            {
...
BOOL ConvertToNULLTerm(LPTSTR pchRead)
{
...
BOOL SelFocusEnumCB(CFileOpenBrowser *that, LPCITEMIDLIST pidl, LPARAM lParam)
{
    ...
    if (SUCCEEDED(that->psfCurrent->GetAttributesOf(1, &pidl, &dwAttrs)))
    {
        ...
        else
        {
            if (psf->bSelChange && (dwAttrs & SFGAO_FILESYSTEM))
            {
                ...
                {
                    *szBuf = CHAR_QUOTE;
                    GetViewItemText( that->psfCurrent, pidl, szBuf + 1, ARRAYSIZE(szBuf) - 3 );
                    lstrcat(szBuf, TEXT("\" "));
...
void CFileOpenBrowser::SelFocusChange(BOOL bSelChange)
{
    SELFOCUS sf;
...
BOOL SelRenameCB(CFileOpenBrowser *that, LPCITEMIDLIST pidl, LPARAM lParam)
{
    DWORD dwAttrs = SFGAO_FOLDER;
    ...
    if (SUCCEEDED(that->psfCurrent->GetAttributesOf(1, &pidl, &dwAttrs)))
    {
        if (!(dwAttrs & SFGAO_FOLDER))
        {
            //  If it is not a folder then set the selection to nothing so that whatever is in the edit box will be used.
...
STDMETHODIMP CFileOpenBrowser::OnStateChange(struct IShellView *ppshv, ULONG uChange)
{
    if (ppshv != psv)
    {
        return (E_INVALIDARG);
    ...
    switch (uChange)
    {
        ...
        {
            //  Post one of these messages, since we seem to get a whole bunch of them.
...
STDMETHODIMP CFileOpenBrowser::IncludeObject(struct IShellView *ppshv, LPCITEMIDLIST pidl)
{
    if (ppshv != psv)
    {
        return (E_INVALIDARG);
    ...
    if (*szLastFilter && (dwAttrs == SFGAO_FILESYSTEM))
    {
        ...
        if (!LinkMatchSpec(pidl, szBuf, szLastFilter) && !PathMatchSpec(szBuf, szLastFilter))
        {
            return (S_FALSE);
        }
    }
...
{
    LPCLASSFACTORY pcf;
    HRESULT hres = SHDllGetClassObject( rclsid, IID_IClassFactory, (LPLPVOID)&pcf );
    if (SUCCEEDED(hres))
    {
...
void CFileOpenBrowser::UpdateLevel(HWND hwndLB, int iInsert, MYLISTBOXITEM *pParentItem)
{
    if (!pParentItem)
    {
        return;
    ...
    if (SUCCEEDED(psfParent->EnumObjects(hwndLB, SHCONTF_FOLDERS, &penum)))
    {
        ...
        {
            //  Note: We need to avoid creation of pItem if this is not a file system object (or ancestor) to avoid extra bindings.
            if (ShouldIncludeObject(psfParent, pidl))
            {
                MYLISTBOXITEM *pItem = new MYLISTBOXITEM( pParentItem, psfParent, pidl, cIndent, MLBI_PERMANENT | MLBI_PSFFROMPARENT );
                if (pItem != NULL)
                {
    ...
    for (nDPAIndex = 0; ; ++nDPAIndex)
    {
        ...
        for (pOldItem = GetListboxItem(hwndLB, nLBIndex); pOldItem != NULL; pOldItem = GetListboxItem(hwndLB, ++nLBIndex))
        {
            int nCmp;
            ...
            //  Set this to 1 at the end of the DPA to clear out deleted items at the end.
            nCmp = !pNewItem ? 1 : LBItemCompareProc( pNewItem, pOldItem, (LPARAM)psfParent );
            if (nCmp < 0)
            {
                //  We found the first item greater than the new item, so add it in.
                break;
            }
            else if (nCmp > 0)
            {
                //  Oops! It looks like this item no longer exists, so delete it.
                for ( ; ; )
                {
                ...
                //  We need to continue from the current position, not the next.
                --nLBIndex;
            }
            else
            {
        ...
        GetViewItemText( psfParent, pNewItem->pidlThis, szBuf, ARRAYSIZE(szBuf), FALSE );
...
//  The list consists of pairs of null terminated strings, with an additional null terminating the list.
DWORD InitFilterBox(HWND hDlg, LPCTSTR lpszFilter)
{
    DWORD nIndex = 0;
    UINT nLen;
...
void MoveControls(HWND hDlg, BOOL bBelow, int nStart, int nXMove, int nYMove)
{
    HWND hwnd;
    RECT rcWnd;
...
void ResetDialogHeight(HWND hDlg, HWND hwndExclude, int nCtlsBottom)
{
    ...
    if (nDiffBottom > 0)
    {
        ...
        SetWindowPos( hDlg, NULL, 0, 0, RectWid(rcFull), RectHgt(rcFull) - nDiffBottom, SWP_NOZORDER | SWP_NOMOVE );
    }
}


//  CFileOpenBrowser::CreateHookDialog
BOOL CFileOpenBrowser::CreateHookDialog(int nCtlsBottom)
{
...
#if 0
    //  This is not needed.  WOW apps are not allowed to get the new explorer look.
    UINT uiWOWFlag = 0;
    ...
    hSubDlg = CreateDialogIndirectParamAorW( hinst, (LPDLGTEMPLATE)hTemplate, hwndDlg, lpfnHookProc, (LPARAM)lpOFN, uiWOWFlag );
#endif

    hSubDlg = CreateDialogIndirectParam( hinst, (LPDLGTEMPLATE)hTemplate, hwndDlg, lpfnHookProc, (LPARAM)lpOFN );
    if (!hSubDlg)
    {
        StoreExtendedError(CDERR_DIALOGFAILURE);
        return (FALSE);
    ...
    //  We reset the height of the dialog after creating the hook dialog so the hook can hide controls in its WM_INITDIALOG message.
    ...
    if (hCtlCmn)
    {
        ...
        //  Move the controls in our dialog to make room for the hook's controls above and to the left.
        MoveControls(hwndDlg, FALSE, 0, rcCmn.left, rcCmn.top);

        //  Calculate how far sub dialog controls need to move, and how much more room our dialog needs.
        nXStart = rcCmn.right;
        ...
        if (nXMove < 0)
        {
            //  If the template size is too big, we need more room in the dialog.
            nXRoom -= nXMove;
            nXMove = 0;
        }
        if (nYMove < 0)
        ...
        //  Resize the "template" control so the hook knows the size of our stuff.
        SetWindowPos( hCtlCmn, NULL, 0, 0, rcReal.right - rcReal.left, rcReal.bottom - rcReal.top, SWP_NOMOVE | SWP_NOZORDER );
    }
    else
    {
    ...
    SetWindowPos( hwndDlg, NULL, 0, 0, (rcReal.right - rcReal.left) + nXRoom, (rcReal.bottom - rcReal.top) + nYRoom, SWP_NOZORDER|SWP_NOMOVE );

    //  Note that we are moving this to (0,0) and the bottom of the Z order.
    GetWindowRect(hSubDlg, &rcReal);
    SetWindowPos( hSubDlg, HWND_BOTTOM, 0, 0, (rcReal.right - rcReal.left) + nXMove, (rcReal.bottom - rcReal.top) + nYMove, 0 );
...
void InitSaveAsControls(HWND hDlg)
{
    for (UINT iControl = 0; iControl < ARRAYSIZE(aSaveAsControls); iControl++)
    {
        ...
        LoadString( g_hinst, aSaveAsControls[iControl].idString, szText, ARRAYSIZE(szText) );
...
BOOL InitLocation(HWND hDlg, LPOFNINITINFO poii)
{
    ...
    //  Now that pDlgStruct is stored in the hDlg, it will get freed on the WM_DESTROY.
    IShellFolder *psfRoot;
    if (FAILED(ICoCreateInstance( CLSID_ShellDesktop, IID_IShellFolder, (LPLPVOID)&psfRoot )))
    {
        StoreExtendedError(CDERR_INITIALIZATION);
        return (FALSE);
    }
    ...
    MYLISTBOXITEM *pRootItem = new MYLISTBOXITEM( NULL, psfRoot, pidlRoot, 0, MLBI_PERMANENT );
    SHFree(pidlRoot);
    ...
    //  Now that psfRoot is stored in the pRootItem, it will get freed on the delete.
    ...
    //  Now that pRootItem is stored in the combo, it will get freed on the WM_DESTROY.
#if 0
    ...
    pDlgStruct->uRegister = SHChangeNotifyRegister(
            hDlg,
    ...
    ::lpOKProc = (WNDPROC)::SetWindowLong( ::GetDlgItem(hDlg, IDOK), GWL_WNDPROC, (LONG)OKSubclass );
    ...
    //  the buffer length.  This allows users to type ..\..\.. and move around when the app gives an extremely small buffer.
    ...
    {
        GetCurrentDirectory( ARRAYSIZE(pDlgStruct->szStartDir), pDlgStruct->szStartDir );
    }

    //  Initialize all provided filters.
    if (lpOFN->lpstrCustomFilter && *lpOFN->lpstrCustomFilter)
    {
        SendDlgItemMessage( hDlg, cmb1, CB_INSERTSTRING, 0, (LONG)lpOFN->lpstrCustomFilter );
        SendDlgItemMessage( hDlg, cmb1, CB_SETITEMDATA, 0, (LPARAM)(lpOFN->lpstrCustomFilter + lstrlen(lpOFN->lpstrCustomFilter) + 1) );
        SendDlgItemMessage( hDlg, cmb1, CB_LIMITTEXT, (WPARAM)(lpOFN->nMaxCustFilter), 0L );
    }
    else
    {
    ...
    //  Explicitly set the focus since this is no longer the first item in the dialog template.
    SetFocus(GetDlgItem(hDlg, edt1));

    //  Check out if the filename contains a path.  If so, override whatever
    //  is contained in lpstrInitialDir.  Chop off the path and put up only the filename.
    ...
    //  StringLower(lpInitialText);
        if ( DBL_BSLASH(lpInitialText + 2) && (*(lpInitialText + 1) == CHAR_COLON) )
        {
        ...
        if ( !(lpOFN->Flags & OFN_NOVALIDATE) && (nFileOffset < 0) && (nFileOffset != PARSE_EMPTYSTRING) )
        {
        ...
        if (nFileOffset > 0)
        {
            ...
            //  dir of the file, use the dir of the file and strip the path off the file.
            if (!lpstrInitialDir || !lpstrInitialDir[0] || lstrcmpi(lpstrInitialDir, szScratch) == 0)
            {
    ...
    {
        //  It's better to come up somewhere rather than just tell the app they have a bogus directory set.
    ...
    //  Show the window after creating the ShellView so we do not get a big ugly gray spot.
...
void CleanupDialog(HWND hDlg, BOOL fRet)
{
    ...
    if ( (fRet == TRUE) && pDlgStruct->hSubDlg && ( CD_SendOKNotify(pDlgStruct->hSubDlg, hDlg, lpOFN, pDlgStruct->lpOFI) || CD_SendOKMsg(pDlgStruct->hSubDlg, lpOFN, pDlgStruct->lpOFI) ) )
    {
        //  Give the hook a chance to validate the file name.
        return;
    }

    //  We need to make sure the IShellBrowser is still around during destruction.
...
BOOL GetFullPathEnumCB(CFileOpenBrowser *that, LPCITEMIDLIST pidl, LPARAM lParam)
{
    ...
    if ((SUCCEEDED(that->psfCurrent->GetAttributesOf(1, &pidl, &dwAttrs))) && (dwAttrs & SFGAO_FILESYSTEM))
    {
        LPITEMIDLIST pidlFull;
...
void CFileOpenBrowser::GetFullPath(LPTSTR pszBuf)
{
    *pszBuf = CHAR_NULL;
...
void CFileOpenBrowser::RemoveOldPath(int *piNewSel)
{
    ...
    for (iItem = ComboBox_GetCount(hwndCombo) - 1; iItem >= 0; --iItem)
    {
        ...
        {
            //  We went back a level, so this must be an ancestor of the selected item.
            cIndent = pItem->cIndent;
            continue;
        }
...
int FindLocation(HWND hwndCombo, MYLISTBOXITEM *pFindItem)
{
    int iItem;
...
BOOL CFileOpenBrowser::OnSelChange(int iItem, BOOL bForceUpdate)
{
    HWND hwndCombo = GetDlgItem(hwndDlg, cmb2);
    BOOL bRet = TRUE;
...
TryAgain:
        if (FAILED(SwitchView( pCurrentLocation->GetShellFolder(), pCurrentLocation->pidlFull, &fs )))
        {
            ...
            //  We cannot create the Desktop view.  I think we are in real trouble.  We had better bail out.
            StoreExtendedError(CDERR_DIALOGFAILURE);
            CleanupDialog(hwndDlg, FALSE);
            return (FALSE);
        }

        ::SendMessage( hwndToolbar, TB_SETSTATE, IDC_PARENT, iCurrentLocation ? TBSTATE_ENABLED : 0 );
        ...
        //  We've changed folders; we'd better strip whatever is in the edit box down to the file name.
...
BOOL DblClkEnumCB(CFileOpenBrowser *that, LPCITEMIDLIST pidl, LPARAM lParam)
{
    MYLISTBOXITEM *pLoc = that->pCurrentLocation;
...
//  Process a double-click in the view control, either by choosing the selected non-container object or by opening the selected container.
void CFileOpenBrowser::OnDblClick(BOOL bFromOKButton)
{
    ...
    if (pidlFirst == PIDL_NOTHINGSEL)
    {
        ...
        if (bFromOKButton)
        {
            ...
            //  but nothing was selected.  Let's get the edit text and go from there.
            ProcessEdit();
        }
    }
...
BOOL CFileOpenBrowser::JumpToIDList(LPCITEMIDLIST pidlNew, BOOL bTranslate)
{
    ...
    //  Find the entry in the location dropdown that is the closest parent to the new location.
    HWND hwndCombo = ::GetDlgItem(hwndDlg, cmb2);
    ...
    //  The path provided might have matched an existing item exactly.  In that case, just select the item.
    if (fExact)
    {
        goto FoundIDList;
    }
    ...
    for ( ; ; )
    {
        ...
        MYLISTBOXITEM *pNewItem = new MYLISTBOXITEM( pBestParent, pBestParent->GetShellFolder(), pidlFirst, cIndent, MLBI_PSFFROMPARENT );
        if (pNewItem == NULL)
        {
            break;
        }
        GetViewItemText( pBestParent->psfSub, pidlFirst, szBuf, ARRAYSIZE(szBuf), FALSE );
...
void CFileOpenBrowser::ViewCommand(UINT uIndex)
{
    IContextMenu *pcm;
...
HRESULT CFileOpenBrowser::InitShellLink(void)
{
    ...
    HRESULT hres = ICoCreateInstance( CLSID_ShellLink, IID_IShellLink, (LPLPVOID)&psl );
    if (FAILED(hres))
    {
        return (hres);
    }
...
HRESULT CFileOpenBrowser::ResolveLink(LPCTSTR pszLink, LPTSTR pszFile, UINT cchFile, WIN32_FIND_DATA *pfd)
{
...
BOOL CFileOpenBrowser::LinkMatchSpec(LPCITEMIDLIST pidl, LPCTSTR pszFile, LPCTSTR pszSpec)
{
    ...
    if ((fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) || PathMatchSpec(szFile, pszSpec))
    {
        return (TRUE);
    }
...
void MeasureDriveItems(HWND hwndDlg, MEASUREITEMSTRUCT *lpmi)
{
    HDC hdc;
    HFONT hfontOld;
    int dyDriveItem;
    ...
    hdc = GetDC(NULL);
    hfontOld = (HFONT)SelectObject( hdc, (HFONT)SendMessage( hwndDlg, WM_GETFONT, 0, 0 ) );
...
void CFileOpenBrowser::PaintDriveLine(DRAWITEMSTRUCT *lpdis)
{
    HDC hdc = lpdis->hDC;
    ...
    ::SendDlgItemMessage( hwndDlg, cmb2, CB_GETLBTEXT, lpdis->itemID, (LPARAM)szText );
    ...
    if (lpdis->itemAction != ODA_FOCUS)
    {
        ...
        {
            ExtTextOut( hdc, xString, yString, ETO_OPAQUE, &rc, szText, lstrlen(szText), NULL );
        }

        ImageList_Draw( himl,
                        (lpdis->itemID == (UINT)iCurrentLocation) ? pItem->iSelectedImage : pItem->iImage,
                        hdc,
                        xMiniDrive,
                        rc.top + (rc.bottom - rc.top - MINIDRIVE_HEIGHT) / 2,
                        (pItem->IsShared() ? INDEXTOOVERLAYMASK(IDOI_SHARE) : 0) | ((lpdis->itemState & ODS_SELECTED) ? (ILD_SELECTED | ILD_FOCUS) : ILD_NORMAL) );
    }
...
//  Refresh the view given any change in the user's choice of wildcard filter.
void CFileOpenBrowser::RefreshFilter(HWND hwndFilter)
{
    WAIT_CURSOR w;
    ...
    if (*lpFilter)
    {
        ...
        //  Provide dynamic pszDefExt updating when lpstrDefExt is app initialized.
        if (!bNoInferDefExt && lpOFN->lpstrDefExt)
        {
            //  We are looking for "foo*.ext[;...]".  We will grab ext as the
            //  default extension.  If not of this form, use the default extension passed in.
...
UINT CFileOpenBrowser::GetDirectoryFromLB(LPTSTR pszBuf, int *pichRoot)
{
    if (pCurrentLocation->pidlFull != NULL && SHGetPathFromIDList(pCurrentLocation->pidlFull, pszBuf))
    {
...
BOOL CFileOpenBrowser::EnumItemObjects(UINT uItem, EIOCALLBACK pfnCallBack, LPARAM lParam)
{
    ...
    if (!psv || FAILED(psv->GetItemObject( uItem, IID_IDataObject, (LPVOID *)&pdtobj )))
    {
        goto Error0;
    }
...
BOOL FindNameEnumCB(CFileOpenBrowser *that, LPCITEMIDLIST pidl, LPARAM lParam)
{
    SHFILEINFO sfi;
    ...
    if (!pidl)
    {
        ...
        GetViewItemText( that->psfCurrent, pfns->pidlFound, pfns->pszFile, MAX_PATH );
        if (IsContainer(that->psfCurrent, pfns->pidlFound))
        {
            LPITEMIDLIST pidlFull = ILCombine( that->pCurrentLocation->pidlFull, pfns->pidlFound );
            if (pidlFull)
            {
    ...
    if (!SHGetFileInfo( (LPCTSTR)pidl, 0, &sfi, 0, SHGFI_DISPLAYNAME | SHGFI_PIDL ))
    {
    ...
    that->psv->SelectItem( pfns->pidlFound, SVSI_SELECT | SVSI_DESELECTOTHERS | SVSI_ENSUREVISIBLE | SVSI_FOCUSED );
...
void CDPathQualify(LPCTSTR lpFile, LPTSTR pszPathName)
{
    TCHAR szCurDir[MAX_PATH + 1];
...
//  Returns:   0    success
//             !0   dos error code
int VerifyOpen(LPCTSTR lpFile, LPTSTR pszPathName)
{
    HANDLE hf;

    CDPathQualify(lpFile, pszPathName);

    hf = CreateFile( pszPathName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
...
BOOL CFileOpenBrowser::IsKnownExtension(LPCTSTR pszExtension)
{
    ...
    {
        //  It's a registered extension, so the user is trying to force the type.
        return (TRUE);
    }
...
UINT CFileOpenBrowser::FindNameInView(LPTSTR pszFile, OKBUTTONFLAGS Flags, LPTSTR pszPathName, int nFileOffset, int nExtOffset, int *pnErrCode, BOOL bTryAsDir)
{
    UINT uRet;
    FINDNAMESTRUCT fns =
    {
    ...
    if ((LPTSTR)pszDefExt && ((DWORD)nNewExt + lstrlen(pszDefExt) < lpOFN->nMaxFile))
    {
        bAddExt = TRUE;
        ...
        //  was a directory without the extension, we would have already switched to it.
...
VerifyTheName:
        ...
        //  have not dereferenced any links, since that should have already been done.
        ...
        *pnErrCode = VerifyOpen(pszFile, pszPathName);
        ...
        {
            //  This may be a link to something, so we should try to resolve it.
            if (!(lpOFN->Flags & OFN_NODEREFERENCELINKS) && nLoop > 0)
            {
                ...
                //  so just try to parse the name in the current folder if possible.
...
#else
                    StrToOleStrN( wszDisplayName, ARRAYSIZE(wszDisplayName), pszFile, -1 );
#endif
                    hRes = psfCurrent->ParseDisplayName( NULL, NULL, wszDisplayName, &chEaten, &pidl, &dwAttr );
                }
                ...
                if (SUCCEEDED(hRes))
                {
                    ...
                    if ((dwAttr & SFGAO_LINK) && ResolveLink( pszPathName, szTemp, ARRAYSIZE(szTemp), NULL ) == S_OK)
                    {
        ...
        if (bGetOut || (*pnErrCode != OF_FILENOTFOUND && *pnErrCode != OF_PATHNOTFOUND))
        {
            return (FE_FILEERR);
        }
    ...
    //  We still have not found a match, so try enumerating files we cannot see.
    int nFilterLen;
    ...
    if (nFilterLen + nNewExt + ARRAYSIZE(szDotStar) < ARRAYSIZE(szLastFilter))
    {
        ...
        if ((pCurrentLocation->dwAttrs & SFGAO_FILESYSTEM) && (hf = FindFirstFile( szLastFilter + nFilterLen + 1, &fd)) !=INVALID_HANDLE_VALUE)
        {
            psv->Refresh();
            fns.pidlFound = NULL;
            EnumItemObjects(SVGIO_ALLVIEW, FindNameEnumCB, (LPARAM)&fns);
        ...
        //  enumeration.  ALLVIEW should cause the threads to sync up before returning.
    ...
    if (bAddExt)
    {
        //  Before we fail, check to see if the file typed sans default extension exists.
        *pnErrCode = VerifyOpen(pszFile, pszPathName);
        if (*pnErrCode == 0 || *pnErrCode == OF_SHARINGVIOLATION)
        {
...
BOOL CFileOpenBrowser::SetDirRetry(LPTSTR pszDir, BOOL bNoValidate)
{
...
JumpThere:
    ...
    //  Note that if we are on a net that does not support CD'ing to UNC's directly, this call will connect it to a drive letter.
...
BOOL CFileOpenBrowser::MultiSelectOKButton(LPCTSTR pszFiles, OKBUTTONFLAGS Flags)
{
    ...
    for ( ; *pchRead; pchRead += lstrlen(pchRead) + 1)
    {
        ...
        //  Pass in 0 for the file offset to make sure we do not switch to another folder.
        switch (FindNameInView( szBasicPath, Flags, szPathName, nFileOffset, nExtOffset, &nErrCode, FALSE ))
        {
            case ( FE_OUTOFMEM ) :
            case ( FE_CHANGEDDIR ) :
            {
            ...
            case ( FE_TOOMANY ) :
            {
                CDMessageBox( hwndDlg, iszTooManyFiles, MB_OK | MB_ICONEXCLAMATION, pchRead );
                return (FALSE);
            }
            default :
            {
        ...
        if (nErrCode &&
            ((lpOFN->Flags & OFN_FILEMUSTEXIST) || (nErrCode != OF_FILENOTFOUND)) &&
            ((lpOFN->Flags & OFN_PATHMUSTEXIST) || (nErrCode != OF_PATHNOTFOUND)) &&
            (!(lpOFN->Flags & OFN_SHAREAWARE) || (nErrCode != OF_SHARINGVIOLATION)))
        {
            if ((nErrCode == OF_SHARINGVIOLATION) && hSubDlg)
            {
                int nShareCode = CD_SendShareNotify( hSubDlg, hwndDlg, szPathName, lpOFN, lpOFI );
            ...
            if ((nErrCode == OF_WRITEPROTECTION) || (nErrCode == OF_DISKFULL)        || (nErrCode == OF_ACCESSDENIED))
            {
...
EscapedThroughShare:
        ...
        {
            //  Successfully opened.
            if ((lpOFN->Flags & OFN_NOREADONLYRETURN) && (GetFileAttributes(szPathName) & FILE_ATTRIBUTE_READONLY))
            {
                nErrCode = OF_LAZYREADONLY;
                goto MultiWarning;
            }

            if ((bSave || (lpOFN->Flags & OFN_NOREADONLYRETURN)) && (nErrCode = WriteProtectedDirCheck(szPathName)))
            {
                goto MultiWarning;
            }
        ...
        if (cch > (UINT)lpOFN->nMaxFile)
        {
            //  Buffer is too small, so return the size of the buffer required to hold the string.
            if (lpOFN->nMaxFile >= 3)
            {
#ifdef UNICODE
                lpOFN->lpstrFile[0] = (TCHAR)LOWORD(cch);
...
BOOL CFileOpenBrowser::OKButtonPressed(LPCTSTR pszFile, OKBUTTONFLAGS Flags)
{
    TCHAR szPathName[MAX_PATH];
    TCHAR szBasicPath[MAX_PATH];
    int nErrCode;
...
#else
    if ((!IsDBCSLeadByte(pszFile[0]) && pszFile[1] == CHAR_COLON) || DBL_BSLASH(pszFile))
#endif
    {
    ...
    else
    {
        ...
        if (pszFile[0] == CHAR_BSLASH)
        {
            ...
            lstrcpyn( szBasicPath + nTempOffset, pszFile, ARRAYSIZE(szBasicPath) - nTempOffset - 1 );
        }
        else
        {
    ...
    else if ((nFileOffset != PARSE_DIRECTORYNAME) && (lpOFN->Flags & OFN_NOVALIDATE))
    {
    ...
    else if (nFileOffset == PARSE_DIRECTORYNAME)
    {
        ...
        {
            //  "\\server\share\" and "c:\" keep the trailing backslash, all other paths remove the trailing backslash. Note that
            //  we don't remove the slash if the user typed the path directly
            //  (nTempOffset is 0 in that case).
            if ((nExtOffset != 1) && (szBasicPath[nExtOffset - 2] != CHAR_COLON) && (nExtOffset != nTempOffset + 1))
            {
                szBasicPath[nExtOffset - 1] = CHAR_NULL;
            }
        }
        else if ( (szBasicPath[nExtOffset - 1] == CHAR_DOT) && ( (szBasicPath[nExtOffset - 2] == CHAR_DOT) || ISBACKSLASH(szBasicPath, nExtOffset - 2) ) && IsUNC(szBasicPath) )
        {
...
Warning:
        if (bUNCName)
        {
            ...
            if ((szBasicPath[cch] == CHAR_BSLASH) && (szBasicPath[cch - 1] == CHAR_DOT) && (ISBACKSLASH(szBasicPath, cch - 2)))
            {
        ...
        //  If the disk is not a floppy and they tell me there's no disk in the drive, don't believe them.  Instead, put up the
        //  error message that they should have given us.  (Note that the
        //  error message is checked first since checking the drive type is slower.)

        //  I will assume that if we get error 0 or 1 or removable that we will assume removable.
    ...
    //  We need to parse again in case SetDirRetry changed a UNC path to use a drive letter.
    nFileOffset = ParseFileOld(szBasicPath, &nExtOffset, &nOldExt, FALSE);
    ...
    if ( !bUNCName && nFileOffset && eCode != ECODE_S_OK && (lpOFN->Flags & OFN_PATHMUSTEXIST) )
    {
        ...
        else if (eCode == ECODE_BADDRIVE)
        {
            ...
            //  Since we only need one character for the error message, set szPathName[0] to the drive letter.
    ...
    //  used at the Warning: label to determine if this hack has occurred, and thus it can strip out the ".\" when putting put the error.
    ...
    //  default extension exists.  If the file exists, consider asking permission to overwrite the file.

    //  NOTE: When no extension given, default extension is tried 1st.
    //  FindNameInView calls VerifyOpen before returning.
    szPathName[0] = 0;
    switch (FindNameInView( szBasicPath, Flags, szPathName, nFileOffset, nExtOffset, &nErrCode ))
    {
        case ( FE_OUTOFMEM ) :
        case ( FE_CHANGEDDIR ) :
        {
        ...
        case ( FE_TOOMANY ) :
        {
            CDMessageBox( hwndDlg, iszTooManyFiles, MB_OK | MB_ICONEXCLAMATION, szBasicPath );
            return (FALSE);
        }
        default :
        {
    ...
    switch (nErrCode)
    {
        case ( 0 ) :
        {
            ...
            if ((bSave || (lpOFN->Flags & OFN_NOREADONLYRETURN)) && (nTempOffset = WriteProtectedDirCheck(szPathName)))
            {
...
SharingViolationInquiry:
        ...
        default :
        {
            ...
            //  because there are many extended characters which are invalid which won't be caught by ParseFile.
...
VerifyPath:
            ...
            if (lpOFN->Flags & OFN_PATHMUSTEXIST)
            {
                ...
                {
                    HANDLE hf = CreateFile( szBasicPath, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
                    if (hf != INVALID_HANDLE_VALUE)
                    {
                        CloseHandle(hf);

                        //  This test is here to see if we were able to create it, but couldn't delete it.  If so,
                        //  warn the user that the network admin has given him create-but-no-modify privileges.  As such,
                        //  the file has just been created, but we can't do anything with it, it's of 0 size.
                    ...
                    else
                    {
                        ...
                        //  If it's not write-protection, a full disk, network protection, or the user popping the
                        //  drive door open, assume that the filename is invalid.
    ...
    if (lpOFN->lpstrFileTitle)
    {
        ...
        {
#ifdef DBCS
            EliminateString( szPathName + nFileOffset, lpOFN->nMaxFileTitle - 1 );
#else
...
TryAgain:
    switch (uAction)
    {
        case ( OCDL_TOGGLE ) :
        {
            uAction = SendMessage(hwndDriveList, CB_GETDROPPEDSTATE, 0, 0L) ? OCDL_CLOSE : OCDL_OPEN;
            goto TryAgain;
            break;
        }
        case ( OCDL_OPEN ) :
...
//  Returns the number of characters needed to get the full path, including the NULL.
UINT CFileOpenBrowser::GetFullEditName(LPTSTR pszBuf, UINT cLen, TEMPSTR *pTempStr, BOOL *pbNoDefExt)
{
    UINT cTotalLen;
    HWND hwndEdit;
...
void CFileOpenBrowser::ProcessEdit()
{
    ...
    //  lpstrDefExts here along with whatever else is only needed in OKButtonPressed.
    ...
    if (OKButtonPressed(pszFile, Flags))
    {
        ...
        if (lpOFN->lpstrFile)
        {
            if (!(lpOFN->Flags & OFN_NOVALIDATE))
            {
                if (lpOFN->nMaxFile >= 3)
                {
                    if ((lpOFN->lpstrFile[0] == 0) || (lpOFN->lpstrFile[1] == 0) || (lpOFN->lpstrFile[2] == 0))
                    {
                        bReturn = FALSE;
...
LRESULT CFileOpenBrowser::OnCommandMessage(WPARAM wParam, LPARAM lParam)
{
    ...
    switch (idCmd)
    {
        ...
        case ( cmb1 ) :
        {
            switch (GET_WM_COMMAND_CMD(wParam, lParam))
            {
                ...
                //  the view (very slow) as the user scrolls through the combobox.
                case ( CBN_CLOSEUP ) :
                case ( CBN_SELENDOK ) :
                {
                    //  Did anything change?
                    if (iComboIndex >= 0 && iComboIndex == SendMessage( GET_WM_COMMAND_HWND(wParam, lParam), CB_GETCURSEL, NULL, NULL ))
                    {
                        break;
                    }
                }
...
BOOL CFileOpenBrowser::OnCDMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    LONG lResult;
    LPCITEMIDLIST pidl;
...
CopyAndReturn:
            ...
            //  We'll just fall through to the error case for now, since doing the full combine is not an easy thing.
            TCHAR szFile[MAX_PATH];

            if ( GetFullEditName(szFile, ARRAYSIZE(szFile)) > ARRAYSIZE(szFile) - 5 )
            {
...
//  The OK button is subclassed so we know which control had focus before the user clicked OK.
//  This in turn lets us know whether to process OK based on the current selection in the listview, or the current text in the edit control.
{
    switch (msg)
    {
        case ( WM_SETFOCUS ) :
...
int CFileOpenBrowser::GetNodeFromIDList(LPCITEMIDLIST pidl)
{
    int i;
...
BOOL CFileOpenBrowser::FSChange(LONG lNotification, LPCITEMIDLIST *ppidl)
{
    int iNode = -1;
    ...
    switch (lNotification)
    {
        ...
        {
            //  Rename is special.  We need to invalidate both the pidl and the pidlExtra. so we call ourselves.
    ...
    switch (message)
    {
        ...
        case ( WM_ACTIVATE ) :
            if (wParam == WA_INACTIVE)
            {
                ...
                //  the focus.  This is a process global, so it should not need to be protected.
        ...
        case ( WM_SETCURSOR ) :
            ...
            break;
        case ( WM_HELP ) :
            if ( ((HWND)((LPHELPINFO)lParam)->hItemHandle) != pDlgStruct->hwndToolbar )
            {
                HWND hwndItem = (HWND)((LPHELPINFO)lParam)->hItemHandle;

                //  We assume that the defview has one child window that covers the entire defview window.
                ...
                WinHelp( hwndItem, NULL, HELP_WM_HELP, (DWORD)(LPTSTR)(pDlgStruct && pDlgStruct->bSave ? aFileSaveHelpIDs : aFileOpenHelpIDs) );
            }
            return (TRUE);
        case ( WM_CONTEXTMENU ) :
            if (((HWND)wParam) != pDlgStruct->hwndToolbar)
            {
                WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD)(LPVOID)(pDlgStruct && pDlgStruct->bSave ? aFileSaveHelpIDs : aFileOpenHelpIDs) );
            }
            return (TRUE);
        ...
        case ( CDM_SELCHANGE ) :
            ...
            {
                CD_SendSelChangeNotify( pDlgStruct->hSubDlg, hDlg, pDlgStruct->lpOFN, pDlgStruct->lpOFI );
            }
            break;
        case ( WM_TIMER ) :
...
LRESULT CALLBACK OpenFileHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    //  Check if this message is for the last active OpenDialog in this process.

    //  Note: This is only done for WM_KEY* messages so that we do not slow down this window too much.
    if (IsInRange(lpMsg->message, WM_KEYFIRST, WM_KEYLAST))
    {
        ...
        if (hwndFocus && (pDlgStruct = HwndToBrowser(hwndActiveOpen)) != NULL)
        {
            if (pDlgStruct->psv && hwndFocus == pDlgStruct->hwndView)
            {
                ...
                if (gp_haccOpenView && TranslateAccelerator( hwndActiveOpen, gp_haccOpenView, lpMsg ))
                {
                    return (1);
                }
            }
            ...
            {
                if (gp_haccOpen && TranslateAccelerator( hwndActiveOpen, gp_haccOpen, lpMsg ))
                {
                    return (1);
                }

                //  Note that the view won't be allowed to translate when the focus is not there.
            }
        }
    }
...
BOOL NewGetFileName(LPOPENFILEINFO lpOFI, BOOL bSave)
{
    ...
    int nRet = DialogBoxParam( ::g_hinst, MAKEINTRESOURCE(NEWFILEOPENORD), lpOFN->hwndOwner, OpenDlgProc, (LPARAM)(LPOFNINITINFO)&oii );
    ...
    //  BUGBUG.
    //  There is a race condition here where we free dlls but a thread using this stuff still hasn't terminated so we page fault.
//  FreeImports();
...
static inline void * __cdecl operator new(unsigned int size)
{
    return ((void *)LocalAlloc(LPTR, size));
}


static inline void __cdecl operator delete(void *ptr)
{
    LocalFree(ptr);
}
...
BOOL InitImports(void)
//  Import all the APIs we need from shell32.dll and comctl32.dll, so we don't have hard links to them.
{
    return (TRUE);
}
