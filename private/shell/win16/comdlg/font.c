...
BOOL ThunkChooseFontA2W(PFONTINFO pFI)
{
    ...
    //  !!! hack, should not be based on flag value, since this could happen at any time.
...
VOID ThunkLogFontA2W(LPLOGFONTA lpLFA, LPLOGFONTW lpLFW)
{
    ...
    MultiByteToWideChar( CP_ACP, 0, lpLFA->lfFaceName, -1, lpLFW->lfFaceName, LF_FACESIZE );
}
...
VOID ThunkLogFontW2A(LPLOGFONTW lpLFW, LPLOGFONTA lpLFA)
{
    ...
    if (lpLFW && lpLFA)
    {
        ...
        WideCharToMultiByte( CP_ACP, 0, lpLFW->lfFaceName, -1, lpLFA->lfFaceName, LF_FACESIZE, NULL, &fDefCharUsed );
    }
}
