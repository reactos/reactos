    ...
    UINT mbType)
{
    ...
    switch (wErrCode)
    {
        ...
        {
            FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK, NULL, wErrCode, GetUserDefaultLCID(), szWarning, WARNINGMSGLENGTH, NULL );
            goto DisplayError;
        }
        default :
        {
    ...
    if (!LoadString( g_hinst, isz, (LPTSTR)szCaption, WARNINGMSGLENGTH ))
    {
        wsprintf( szWarning, TEXT("Error occurred, but error resource cannot be loaded.") );
    }
    else
    {
    ...
    HWND hDlg)
{
    ...
    if ((*lpCurThread == 0) && (!hLNDThread))
    {
        if (hLNDEvent && (bNetworkInstalled = IsNetworkInstalled()))
        {
            ...
            {
                hLNDThread = CreateThread(NULL, (DWORD)0, (LPTHREAD_START_ROUTINE)ListNetDrivesHandler, (LPVOID)NULL, (DWORD)NULL, &dwThreadID );
            }
        }
        else
        {
