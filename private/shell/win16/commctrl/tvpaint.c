...
//  Draws a plus or minus sign centered around the given (x,y) location and extending out from that location the given distance (c).
...
void NEAR TV_DrawItem(PTREE pTree, HTREEITEM hItem, HDC hdc, int x, int y, UINT flags)
{
    ...
    iLevel = 0;
    else if ((pTree->style & (TVS_HASLINES | TVS_HASBUTTONS)) && (pTree->style & TVS_LINESATROOT))
    ...
    if (!(flags & TVDI_NOTREE))
    {
        ...
        if (pTree->himlState) {
            ...
            if (iState) {
                ImageList_Draw(pTree->himlState, iState, hdc, x, y + (pTree->cyItem - pTree->cyState), ILD_NORMAL);
                x += pTree->cxState;
            }
        }
        ...
        if (pTree->hImageList) {
            ...
            ImageList_DrawEx(pTree->hImageList, i, hdc, x, y + ((pTree->cyItem - pTree->cyImage) / 2), 0, 0, CLR_DEFAULT, rgb, fStyle | (ti.state & TVIS_OVERLAYMASK));
        }
    }
    ...
    // draw text
    if ((ti.state & TVIS_DROPHILITED) || (!pTree->hDropTarget && ((ti.state & TVIS_SELECTED) && (pTree->fFocus || (pTree->style & TVS_SHOWSELALWAYS))) && !(flags & TVDI_GRAYCTL))) {
    // selected
        ...
        if (pTree->style & TVS_HASLINES)
    {
        ...
        {
            if ((pTree->style & TVS_HASBUTTONS) && (iLevel || pTree->style & TVS_LINESATROOT) && ti.cChildren)
            {
...
HIMAGELIST NEAR TV_CreateDragImage(PTREE pTree, HTREEITEM hItem)
{
    ...
    */
    ...
    ImageList_CopyDitherImage(himl, 0, 0, (pTree->cyItem - pTree->cyImage) / 2, pTree->hImageList, iSrc, hItem->state & TVIS_OVERLAYMASK);
