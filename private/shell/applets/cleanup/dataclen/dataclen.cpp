...
BOOL DllInit(HINSTANCE hInstance);
...
BOOL DllInit (HINSTANCE hInstance)
{
    ...
    //Note:    it is very likely that the commctrl.dll is already loaded and properly initialized.
...
extern "C" int APIENTRY LibMain( HINSTANCE   hInstance, DWORD       fdwReason, LPVOID      lpvReserved)
{
    switch (fdwReason)
    {
...
STDAPI DllRegisterServer()
{
    ...
    // (The inf engine doesn't guarantee DelReg/AddReg order, that's why we explicitly unreg and reg here.)
...
UINT setDllObjectCount(UINT cNew)
{
...
UINT setDllLockCount(ULONG cNew)
{
...
UINT incDllLockCount(void)
{
    return ++g_cDllLocks;
}

UINT decDllLockCount(void)
{
    return --g_cDllLocks;
}
...
*/
STDMETHODIMP CCleanerClassFactory::QueryInterface(REFIID      riid, LPVOID FAR *ppv)
{
...
*/
STDMETHODIMP CCleanerClassFactory::CreateInstance(LPUNKNOWN   pUnkOuter, REFIID      riid, LPVOID *    ppvObj)
{
...
*/
STDMETHODIMP CDataDrivenCleaner::QueryInterface(REFIID      riid, LPVOID FAR *ppv)
{
    *ppv = NULL;
...
*/
STDMETHODIMP CDataDrivenCleaner::Initialize(HKEY    hRegKey, LPCWSTR pszVolume, LPWSTR  *ppszDisplayName, LPWSTR  *ppszDescription, DWORD   *pdwFlags)
{
    DWORD   dwType;
    DWORD   cbData;
    ...
    //This means that cleanmgr.exe will get these values from the registry.
    ...
    *ppszDescription = NULL;
    ...
    //We can't run some cleaners if certain processes are running, this is becasue we might delete the processes temp files right out from under it...bad!.  So we will
    //check if any of the Fail Processes are running and if they are we will not show this cleaner.
    ...
    if ((_dwFlags & DDEVCF_RUNIFOUTOFDISKSPACE) && (!(*pdwFlags & EVCF_OUTOFDISKSPACE)))
    {
        return S_FALSE;
    }
    ...
    *pTemp = NULL;
    ...
    {
        MiDebugMsg((0, TEXT("CDataDrivenCleaner::Initialize -> None of the specified folders are on this volume (%s)"), acsVolume));
...
STDMETHODIMP CDataDrivenCleaner::Purge(DWORDLONG                   dwSpaceToFree, IEmptyVolumeCacheCallBack   *picb)
{
    ...
    if (*_CleanupString)
    {
        ...
        if (!CreateProcess(NULL, _CleanupString, NULL, NULL, FALSE, 0, NULL, NULL, &StartInfo, &ProcInfo))
        {
            MiDebugMsg((0, TEXT("CreateProcess(%s) failed with error %d"), _CleanupString, GetLastError()));
        }
    }
...
*/
BOOL CDataDrivenCleaner::bLastAccessisOK(FILETIME ftFileLastAccess)
{
...
BOOL bFileIsOpen(LPTSTR lpFile, LPFILETIME lpftFileLastAccess)
/*
** Purpose:     This function checks if a file is open by doing a CreateFile with fdwShareMode of 0.
**              If GetLastError() retuns ERROR_SHARING_VIOLATION then this function retuns TRUE because someone has the file open.
**              Otherwise this function retuns false.
...
*/
{
    ...
    if (((hFile = CreateFile(lpFile, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)) == INVALID_HANDLE_VALUE) && (GetLastError() == ERROR_SHARING_VIOLATION))
    {
...
** Mod Log:     Created by Jason Cobb (2/97)
*/
BOOL CDataDrivenCleaner::WalkAllFiles(PTCHAR  lpPath, IEmptyVolumeCacheCallBack   *picb)
{
    BOOL            bRet = TRUE;
    BOOL            bFind = TRUE;
    HANDLE          hFind;
    ...
    //If this is a directory then tack a *.* onto the end of the path and recurse through the rest of the directories
    ...
    if (dwAttributes & FILE_ATTRIBUTE_DIRECTORY)
    {
        ...
        while (hFind != INVALID_HANDLE_VALUE && bFind)
        {
            ...
            if (((!(wd.dwFileAttributes & FILE_ATTRIBUTE_READONLY)) || (_dwFlags & DDEVCF_REMOVEREADONLY)) &&
                ((!(wd.dwFileAttributes & FILE_ATTRIBUTE_SYSTEM)) || (_dwFlags & DDEVCF_REMOVESYSTEM)) &&
                ((!(wd.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN)) || (_dwFlags & DDEVCF_REMOVEHIDDEN)))
            {
                ...
                if ((!(wd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) && (bFileIsOpen(szAddFile, &wd.ftLastAccessTime) == FALSE) && (bLastAccessisOK(wd.ftLastAccessTime)))
                {
        ...
        {
            //This is a directory
            if ((wd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && (lstrcmp(wd.cFileName, TEXT(".")) != 0) && (lstrcmp(wd.cFileName, TEXT("..")) != 0))
            {
                lstrcpy(szAddFile, lpPath);
                if (szAddFile[lstrlen(szAddFile) - 1] != TCHAR('\\'))
                    lstrcat(szAddFile, TEXT("\\"));
...
*/
BOOL CDataDrivenCleaner::WalkForUsedSpace(PTCHAR  lpPath, IEmptyVolumeCacheCallBack   *picb)
{
    BOOL            bRet = TRUE;
    BOOL            bFind = TRUE;
    HANDLE          hFind;
        ...
        for(lpSingleFile = _filelist; *lpSingleFile; lpSingleFile += lstrlen(lpSingleFile) + 1)
        {
            ...
            while (hFind != INVALID_HANDLE_VALUE && bFind)
            {
                ...
                if (((!(wd.dwFileAttributes & FILE_ATTRIBUTE_READONLY)) ||
                    (_dwFlags & DDEVCF_REMOVEREADONLY)) && ((!(wd.dwFileAttributes & FILE_ATTRIBUTE_SYSTEM)) ||
                    (_dwFlags & DDEVCF_REMOVESYSTEM)) && ((!(wd.dwFileAttributes & FILE_ATTRIBUTE_HIDDEN)) ||
                    (_dwFlags & DDEVCF_REMOVEHIDDEN)))
                {
                    ...
                    //This is a file so check if it is open
                    else if ((bFileIsOpen(szAddFile, &wd.ftLastAccessTime) == FALSE) && (bLastAccessisOK(wd.ftLastAccessTime)))
                    {
                        //File is not open so add it to the list
                        dwFileSize.HighPart = wd.nFileSizeHigh;
                        dwFileSize.LowPart = wd.nFileSizeLow;
    ...
    {
        MiDebugMsg((0, TEXT("CDataDrivenCleaner::WalkForUsedSpace -> %s is NOT a directory!"), lpPath));
    }
...
*/
BOOL CDataDrivenCleaner::AddFileToList(PTCHAR  lpFile, ULARGE_INTEGER  filesize, BOOL bDirectory)
{
    BOOL                bRet = TRUE;
    ...
    if (pNew == NULL)
    {
        MiDebugMsg((0, TEXT("CDataDrivenCleaner::AddFileToList -> ERROR HeapAlloc() failed with error %d"), GetLastError()));
        return FALSE;
    }
...
void CDataDrivenCleaner::PurgeFiles(IEmptyVolumeCacheCallBack   *picb, BOOL bDoDirectories)
{
    ...
    while (pCleanFile)
    {
        ...
        if (bDoDirectories && pCleanFile->bDirectory)
        {
            ...
            {
                MiDebugMsg((0, TEXT("Error RemoveDirectory(%s) returned error %d"), pCleanFile->file, GetLastError()));
            }
        }
        ...
        else if (!bDoDirectories && !pCleanFile->bDirectory)
        {
            ...
            {
                MiDebugMsg((0, TEXT("Error DeleteFile(%s) returned error %d"), pCleanFile->file, GetLastError()));
            }
        }
...
*/
void CDataDrivenCleaner::FreeList(PCLEANFILESTRUCT pCleanFile)
{
    if (pCleanFile == NULL)
        return;
...
PTSTR FileNameFromPath(PTSTR pFilePath)
/*
...
BOOL CDataDrivenCleaner::bW98ProcessIsRunning(HKEY    hRegKey)
{
    ...
    *pFailProcess = 0;
    ...
    do
    {
        //Check if this process is one of the processes that should cause this cleaner to fail.
        for (pFailProcess = &(szFailProcess[0]); *pFailProcess; pFailProcess += (lstrlen(pFailProcess) + 1))
        {
            ...
            {
                MiDebugMsg((0, TEXT("Process %s is running so cleaner will not run!"), FileNameFromPath(pe32.szExeFile)));
                bRet = TRUE;
                break;
            }
        }
...
CDataDrivenPropBag::CDataDrivenPropBag(DWORD dw)
{
    ...
    incDllObjectCount();//increment global object count
}
...
CDataDrivenPropBag::~CDataDrivenPropBag()
{
    decDllObjectCount();//Decrement global object count
}


STDMETHODIMP CDataDrivenPropBag::QueryInterface(REFIID      riid, LPVOID FAR *ppv)
/*
** Purpose:    Part of the IUnknown interface
** Parameters:
**    riid  -  interface ID to query on
...
*/
{
    *ppv = NULL;
    ...
    return E_NOINTERFACE;//Error - unsupported interface requested
}
...
STDMETHODIMP CContentIndexCleaner::Initialize(HKEY hRegKey, LPCWSTR pszVolume, LPWSTR *ppszDisplayName, LPWSTR *ppszDescription, DWORD *pdwFlags)
{
