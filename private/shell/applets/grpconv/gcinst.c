...
BOOL CreateSpecialFolderDPA(HDPA* phdpaSF)
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders"), 0, KEY_QUERY_VALUE, &hkSP) != ERROR_SUCCESS)
    {
    ...
    *phdpaSF = DPA_Create(4);
    ...
    do
    {
        ...
        lRet = RegEnumValue(hkSP, dwIndex, szValueName, &cbValueName, NULL, NULL, NULL, &cbData);
        if (lRet == ERROR_SUCCESS)
        {
            ...
            if (RegQueryValueEx(hkSP, szValueName, NULL, NULL, (LPBYTE)pszValueData, &cbData) == ERROR_SUCCESS)
            {
...
BOOL IsShellExplorer()
{
...
#ifdef WINNT
    {
        ...
        if (RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon"), 0L, KEY_QUERY_VALUE, &hKeyWinlogon) == ERROR_SUCCESS)
        {
        ...
        if (!szShell[0])
        {
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon"), 0L, KEY_QUERY_VALUE, &hKeyWinlogon) == ERROR_SUCCESS)
            {
                dwSize = SIZEOF(szShell);
                RegQueryValueEx(hKeyWinlogon, TEXT("shell"), NULL, NULL, (LPBYTE)szShell, &dwSize);
                RegCloseKey(hKeyWinlogon);
...
#else
    {
        ...
        GetPrivateProfileString(TEXT("boot"), TEXT("shell"), TEXT("explorer.exe"), szShell, MAX_PATH, TEXT("system.ini"));
    }
#endif
...
// This parses the grpconv.inf file and creates the appropriate programs folders.
