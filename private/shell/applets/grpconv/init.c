...
void Restrictions_Convert(LPCTSTR szIniFile)
{
        ...
        if (RegCreateKey(HKEY_CURRENT_USER, REGSTR_PATH_POLICIES, &hkeyPolicies) == ERROR_SUCCESS)
        {
                ...
                if (RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Windows NT\\CurrentVersion\\Program Manager\\Restrictions"), 0, KEY_READ, &hkeyPMRestrict) == ERROR_SUCCESS) {
                    cbData = sizeof(dw);
...
void DoConversion(HINSTANCE hinst, LPTSTR lpszCmdLine)
{
    ...
    else if (!lstrcmpi(lpszCmdLine, TEXT("/n")) || !lstrcmpi(lpszCmdLine, TEXT("-n")))
    {
        ...
        Group_EnumNT(Group_EnumCallback, FALSE, FALSE, HKEY_LOCAL_MACHINE, c_szCommonGroups);
        BuildFloppyLinks();
    }
    else if (!lstrcmpi(lpszCmdLine, TEXT("/c")) || !lstrcmpi(lpszCmdLine, TEXT("-c")))
    {
        ...
        hCursor = SetCursor(LoadCursor(NULL, IDC_WAIT));
        g_fDoingCommonGroups = TRUE;
        Group_EnumNT(Group_EnumCallback, TRUE, FALSE, HKEY_LOCAL_MACHINE, c_szCommonGroups);
        SetCursor(hCursor);
    }
    ...
    {
        AppList_Build();// Question and answer stuff.
        WinExec(c_szReporter, SW_NORMAL);// Restart the reporter tool.
    }
...
int WinMainT(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    // we have to LoadLibaray/GetProcAddress ImmDisableIME because this is not exported on win95 gold or NT4.
    ...
    if (hLibImm)
    {
        ...
        {
            if ( (PRIMARYLANGID(LANGIDFROMLCID(lcid)) == LANG_JAPANESE) || (PRIMARYLANGID(LANGIDFROMLCID(lcid)) == LANG_KOREAN)   || (PRIMARYLANGID(LANGIDFROMLCID(lcid)) == LANG_CHINESE) )
            {
                ImmDisableIME(0);
            }
        }
