...
void CPerfPage::DrawCPUGraph(LPDRAWITEMSTRUCT lpdi, UINT iPane)
{
    ...
    else
    {
        ...
        if (g_Options.m_fKernelTimes)
        {
            ...
            MoveToEx(m_hdcGraph, m_rcGraph.right, m_rcGraph.bottom - (dwSum * GraphHeight) / 100, (LPPOINT) NULL);
            ...
            for (int i = 0; i < HIST_SIZE && i * Scale < Width; i++)
            {
                ...
                LineTo(m_hdcGraph, m_rcGraph.right - Scale * i, m_rcGraph.bottom - (dwSum * GraphHeight) / 100);
            }
...
--*/
{
    ...
    for (int ListIndex = 0; ListIndex < g_cProcessors; ListIndex++)
    {
        ...
        CPUKernelTime[ListIndex].QuadPart= ProcessorInfo[ListIndex].KernelTime.QuadPart-ProcessorInfo[ListIndex].IdleTime.QuadPart;
        CPUTotalTime[ListIndex].QuadPart = ProcessorInfo[ListIndex].KernelTime.QuadPart + ProcessorInfo[ListIndex].UserTime.QuadPart;// +
                                           //ProcessorInfo[ListIndex].IdleTime.QuadPart;

        DeltaCPUIdleTime.QuadPart        = CPUIdleTime[ListIndex].QuadPart - PreviousCPUIdleTime[ListIndex].QuadPart;
        DeltaCPUKernelTime.QuadPart      = CPUKernelTime[ListIndex].QuadPart - PreviousCPUKernelTime[ListIndex].QuadPart;
        DeltaCPUTotalTime.QuadPart       = CPUTotalTime[ListIndex].QuadPart - PreviousCPUTotalTime[ListIndex].QuadPart;
        ...
        BYTE ThisCPU =  DeltaCPUTotalTime.QuadPart ? (BYTE) (100 - ((DeltaCPUIdleTime.QuadPart * 100) / DeltaCPUTotalTime.QuadPart)) : 0;

        BYTE * pbHistory = g_pCPUHistory[ListIndex];
        MoveMemory((LPVOID) (pbHistory + 1), (LPVOID) (pbHistory), sizeof(BYTE) * (HIST_SIZE - 1) );
        pbHistory[0] = ThisCPU;

        BYTE ThisKernel = DeltaCPUTotalTime.QuadPart ? (BYTE) (((DeltaCPUKernelTime.QuadPart * 100) / DeltaCPUTotalTime.QuadPart)) : 0;
    ...
    g_CPUUsage = SumTotalTime.QuadPart ? (BYTE) (100 - ((SumIdleTime.QuadPart * 100) / SumTotalTime.QuadPart)) : 0;

    if (fUpdateHistory)
    {
        g_KernelUsage = SumTotalTime.QuadPart ? (BYTE) ((SumKernelTime.QuadPart * 100) / SumTotalTime.QuadPart) : 0;
        ...
        MoveMemory((LPVOID) (g_pMEMHistory + 1), (LPVOID) (g_pMEMHistory), sizeof(BYTE) * (HIST_SIZE - 1) );
