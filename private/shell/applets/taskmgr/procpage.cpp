...
// #endif
    ...
    {
        if ( m_fWowProcess || ! m_fWowProcessTested)
        {
#if !defined (_WIN64)
            if ( ( m_pszImageName != NULL ) && ( ! _wcsicmp(m_pszImageName, TEXT("ntvdm.exe")) ) )
            {
                ...
                if (VDMEnumTaskWOWEx(m_UniqueProcessId, WowTaskCallback, (LPARAM) &WowTaskCallbackParms))
                {
                    if ( ! m_fWowProcess )
                    {
                        m_fWowProcessTested = m_fWowProcess = TRUE;
                    }
                ...
                {
                    // We avoid calling VDMEnumTaskWOWEx if the process has an execution time of more than 10 seconds and has not so far been seen as a WOW process.
    ...
    // If this is a sandboxed version of dllhost.exe, tack on the URL it goes with to the image name
    if (g_Options.m_fShowSandboxNames && 0 == lstrcmpi(m_pszImageName, TEXT("dllhost.exe")))
    {
        HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, m_UniqueProcessId);
        if (NULL != hProcess)
        {
            HANDLE hToken;
            ...
            if (OpenProcessToken(hProcess, TOKEN_READ, &hToken))
            {
                if (IsTokenRestricted(hToken))
                {
                    ...
                    if (NULL != psid)
                    {
                        ...
                        if (LookupAccountSid(NULL, psid, szName, &cchName, szDomain, &cchDomain, &use))
                        {
                            int cch = (lstrlen(m_pszImageName) + lstrlen(szName)) * sizeof(WCHAR) + sizeof(TEXT("  ()"));
                            LPTSTR pszImageName = new TCHAR[cch];

                            if (NULL != pszImageName)
                            {
                                wsprintf(pszImageName, TEXT("%s  (%s)"), m_pszImageName, szName);
                                delete [] m_pszImageName;
...
*/
{
    ...
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\AeDebug"), 0, KEY_READ, &hkDebug))
    {
        TCHAR szDebugger[MAX_PATH * 2];
        DWORD cbString = sizeof(szDebugger);

        if (ERROR_SUCCESS == RegQueryValueEx(hkDebug, TEXT("Debugger"), NULL, NULL, (LPBYTE) szDebugger, &cbString))
        {
    ...
    NTSTATUS Status = NtQuerySystemInformation(SystemBasicInformation, &g_BasicInfo, sizeof(g_BasicInfo), NULL);
    if (!NT_SUCCESS(Status))
    {
        return E_FAIL;
    }
...
retry:
    ...
    if (CommonLargeBuffer == NULL) {
        ...
        CommonLargeBuffer = (BYTE *) VirtualAlloc (NULL, CommonLargeBufferSize, MEM_COMMIT, PAGE_READWRITE);
        if (CommonLargeBuffer == NULL) {
            return 0;
        }
    }
    status = NtQuerySystemInformation(SystemProcessInformation, CommonLargeBuffer, CommonLargeBufferSize, NULL);
    if (status == STATUS_INFO_LENGTH_MISMATCH) {
