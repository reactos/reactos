        ...
        if (hwndDesktop)
        {
            ...
            if (hChangeNotification)
            {
                ...
                if (fFlushNow)
                {
                    ...
                    SendMessage(hwndDesktop, CWM_FSNOTIFY, (WPARAM)hChangeNotification, (LPARAM)dwProcId);
                }
                else
                {
                    TraceMsg(TF_FSNOTIFY, "SHChangeNotifyTransmit Notify 0x%X, %d", lEvent, IsMainShellProcess());
                    SendNotifyMessage(hwndDesktop, CWM_FSNOTIFY, (WPARAM)hChangeNotification, (LPARAM)dwProcId);
                }
            }
            return;
        }
...
ULONG WINAPI NTSHChangeNotifyRegister(HWND hwnd, int fSources, LONG fEvents, UINT wMsg, int cEntries, SHChangeNotifyEntry *pfsne)
{
...
ULONG WINAPI SHChangeNotifyRegisterInternal(HWND hwnd, int fSources, LONG fEvents, UINT wMsg, int cEntries, SHChangeNotifyEntry *pfsne)
{
    int i;
    ...
    if (pshcr)
    {
        switch(pshcr->uCmd)
        {
            case SHCR_CMD_REGISTER:
            {
                ...
                pshcr->ulID = SHChangeNotifyRegisterInternal(LongToHandle(pshcr->hwnd), pshcr->fSources, pshcr->lEvents, pshcr->uMsg, 1, &fsne);
                fResult = TRUE;
                break;
            }
...
STDAPI_(ULONG) SHChangeNotifyRegister(HWND hwnd, int fSources, LONG fEvents, UINT wMsg, int cEntries, SHChangeNotifyEntry *pfsne)
{
    int i;
    HWND hwndDesktop;
    ULONG ulID = 0;
    ...
    for (i = 0; i < cEntries; i++)
    {
        ...
        if (hwndDesktop)
        {
            HANDLE hChangeRegistration = SHChangeRegistration_Create(SHCR_CMD_REGISTER, ulID, hwnd, wMsg, fSources, fEvents, pfsne[i].fRecursive, pfsne[i].pidl, dwProcId);
            if (hChangeRegistration)
            {
                LPSHChangeRegistration pshcr;
                ...
                SendMessage(hwndDesktop, CWM_CHANGEREGISTRATION, (WPARAM)hChangeRegistration, (LPARAM)dwProcId);
        ...
        {
            ulID = SHChangeNotifyRegisterInternal(hwnd, fSources, fEvents, wMsg, 1, &pfsne[i]);
        }
        if (ulID == 0)
        {
            if (hwndProxy)
...
LRESULT CALLBACK HiddenNotifyWindow_WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    switch( iMessage )
    {
        ...
        case WM_NCDESTROY:
            ASSERT(pData != NULL );
            SetWindowLongPtr( hWnd, 0, (LONG_PTR)NULL );// clear it so it won't be in use....
        ...
        case WM_CHANGENOTIFYMSG :
            if ( pData != NULL )
            {
                ...
                pshcnl = SHChangeNotification_Lock( (HANDLE)wParam, (DWORD)lParam, &ppidl, &lEvent );// lock and break the info structure ....
                lRes = SendMessage( pData->hwndParent, pData->wMsg, (WPARAM) ppidl, (LPARAM) lEvent );// pass on to the old style client. ...
                SHChangeNotification_Unlock(pshcnl);// new notifications ......
            }
            break;
