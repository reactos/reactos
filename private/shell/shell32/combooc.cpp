...
CComboBoxExOC::CComboBoxExOC(LPCTSTR pszRegKey, int csidlDefaultPidl)
{
    ...
    // We do this so comctl32.dll won't fail to create the ComboBoxEx control.
...
HWND CComboBoxExOC::_CreateComboBoxEx(IOleClientSite * pClientSite, HWND hWnd, HWND hWndParent, RECT& rcPos, LPCTSTR pszWindowName, DWORD dwStyle, DWORD dwExStyle, UINT_PTR nID)
{
    ...
    if (!hWnd)
    {
        ...
        // This is the wrapper hwnd because ATL doesn't allow the normal winproc to get messages this control doesn't handle.
        ...
        if (EVAL(hWnd))
        {
            ...
            if (EVAL(_hwndComboBox))
            {
                ...
                ::SendMessage(_hwndComboBox, CBEM_SETEXTENDEDSTYLE, CBES_EX_NOEDITIMAGE | CBES_EX_NOSIZELIMIT | CBES_EX_CASESENSITIVE, CBES_EX_NOSIZELIMIT | CBES_EX_CASESENSITIVE);

                // _hwndEdit will be NULL if we are a rooted explorer and we
                // turned off the edit attribute of the ComboBox by
                // setting the CBS_DROPDOWNLIST flag when creating the window at the top of this function.
...
HRESULT CComboBoxExOC::_AddToComboBox(LPCTSTR pszDisplayName, LPVOID pvData, LPCITEMIDLIST pidlIcon, INT_PTR nPos, int iIndent, INT_PTR *pnPosAdded)
{
    HRESULT hr;
    CBXITEM item;
...
/*
    This function is optimizing for perf.
    Since enumerating the drop down includes hitting the disk a bit, we want to optimize for the default case of "My Computer" being selected.
    */
HRESULT CComboBoxExOC::_PopulateTopItem(void)
{
    ...
    if (!fIsSecure || (!szLastSelection[0] || _IsDefaultSelection(szLastSelection)))
    {
        // Yes
        _AddDefaultItem();
    }
    ...
    else if (_RestoreIfUserInputValue(szLastSelection) != S_OK)
    {
        ...
        _Populate();// No, so take the hit now.
