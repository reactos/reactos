...
// This file contains the implementation of CFSIconOverlayManager, a COM object that manages the IShellIconOverlayIdentifiers list.
// It aslo managess the Sytem Image List OverlayIndexes, since we have limited slots, exactly MAX_OVERLAY_IAMGES of them.
// History:
//         5-2-97  by dli
...
// NOTE: The value of OVERLAYINDEX_RESERVED is not the same as the overall size of the s_ReservedOverlays array,
// we need to reserved the overlay slot #3 for the non-existent Read-Only overaly.
// The Read Only overlay was once there in Win95, but got turned off on IE4 however,
// because of the stupidity of the original overlay designs,( we used to assign overlay 1 to share and 2 to link and 3 to readonly,
// and the third parties just copied our scheme,) we have to keep overlay #3 as a ghost.
...
typedef struct _FSIconOverlay{// File system Icon overlay Identifiers
...
HRESULT CFSIconOverlayManager::_InitializeReservedOverlays()
{
    ...
    for (i = 0; i < ARRAYSIZE(s_ReservedOverlays); i++) {
        ...
        // Warning: This is used by non explorer processes on NT only because their image list was initialized with only 4 icons

        HICON hIcon = NULL;
        HICON hSmallIcon = NULL;
        int iIndex = s_ReservedOverlays[i].iShellResvrdImageIndex;

        if (s_ReservedOverlays[i].iImageIndex >= ImageList_GetImageCount(g_himlIcons)) {// if we have an index that is no longer valid...
            if (hkeyIcons) {// check to see if icon is overridden in the registry
                TCHAR val[10];
                TCHAR ach[MAX_PATH];
                DWORD cb = SIZEOF(ach);
                ...
                if (ach[0]) {
                    ...
                    ExtractIcons(ach, iIcon, MAKELONG(g_cxIcon, g_cxSmIcon), MAKELONG(g_cyIcon, g_cySmIcon), hIcons, NULL, 2, g_lrFlags);
                    hIcon = hIcons[0];
                    hSmallIcon = hIcons[1];
                }
            }
            ...
            if (!hIcon) {
                hIcon = (HICON)LoadImage(HINST_THISDLL,
                                         MAKEINTRESOURCE(c_SystemImageListIndexes[iIndex]),
                                         IMAGE_ICON,
                                         g_cxIcon,
                                         g_cyIcon,
                                         g_lrFlags);
                hSmallIcon = (HICON)LoadImage(HINST_THISDLL,
                                              MAKEINTRESOURCE(c_SystemImageListIndexes[iIndex]),
                                              IMAGE_ICON,
                                              g_cxSmIcon,
                                              g_cySmIcon,
...
HRESULT CFSIconOverlayManager::LoadNonloadedOverlayIdentifiers(void)
{
    ...
    EnterCriticalSection(&_cs);
    if (NULL == _hdsaIconOverlays) {// No overlay HDSA yet.  We should never hit this but just in case, this will be valid behavior.
        hres = _InitializeHdsaIconOverlays();
    } else {// Load unloaded identifiers into existing HDSA.        
...
HRESULT CFSIconOverlayManager::_LoadIconOverlayIdentifiers(HDSA hdsaOverlays, BOOL bSkipIfLoaded)
{
    ...
    for (idca = 0; idca < DCA_GetItemCount(hdca); idca++) {
        ...
        if (FAILED(DCA_CreateInstance(hdca, idca, IID_IShellIconOverlayIdentifier, (LPVOID *) &(fsio.psioi))))
            continue;
        ...
        fsio.iOverlayIndex = -1;// Initialize the Overlay Index to -1
        ...
        } else {// Now try to look in the registry for the Overlay Icons
            fsio.iImageIndex = -1;
            const CLSID * pclsid = DCA_GetItem(hdca, idca);
            if (pclsid) {
...
/*int CFSIconOverlayManager::_GetAvailableOverlayIndex(int imyhdsa)
{
    ...
    return ++ib;// Add code to grab indexes here.
}*/


HRESULT CFSIconOverlayManager::QueryInterface(REFIID riid, LPVOID * ppvObj)
{
    ASSERT(ppvObj != NULL);// ppvObj must not be NULL
...
int CFSIconOverlayManager::_GetImageIndex(FSIconOverlay * pfsio)
{
    ...
    if (iImage == -1) {// we couldn't find it from the cache
        ...
        SHDefExtractIcon(pfsio->szIconFile, pfsio->iIconIndex, GIL_FORSHELL, &hIconLarge, &hIconSmall, MAKELONG(g_cxIcon, g_cxSmIcon));
...
HRESULT CFSIconOverlayManager::_SetGetOverlayInfo(FSIconOverlay * pfsio, int iOverlayIndex, int * pIndex, DWORD dwFlags)
{
    ...
    *pIndex = -1;
    ...
    if (dwFlags == SIOM_PRIORITY) {
        *pIndex = pfsio->iPriority;// This must have been initialized in the initialization function
    }
#endif
    ...
    if (pfsio->iImageIndex > 0) {// Only if we have a reasonable image index will we proceed.
        if (dwFlags == SIOM_ICONINDEX) {
            *pIndex = pfsio->iImageIndex;
        } else {
            ASSERT(iOverlayIndex > 0);
            ASSERT(iOverlayIndex <= MAX_OVERLAY_IMAGES);
            if (pfsio->iOverlayIndex == -1) {// Now set the overlay
                ASSERT(g_himlIcons);
            ...
            ASSERT(dwFlags == SIOM_OVERLAYINDEX);// Must be the overlayindex flag
...
STDAPI_(int) SHGetIconOverlayIndexW(LPCWSTR pwszIconPath, int iIconIndex)
{
    ...
    if (pwszIconPath == NULL) {
        ...
        iImage = Shell_GetCachedImageIndex(szIconPath, iIconIndex, 0);// Try to load the image into the shell icon cache
