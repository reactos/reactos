...
VOID Printer_AddPrinterPropPages(LPCTSTR pszPrinterName, LPPROPSHEETHEADER lpsh)
{
    HKEY hkeyBaseProgID;
...
BOOL _GetFileNameFromBrowse(HWND hwnd, LPTSTR szFilePath, UINT cbFilePath, LPCTSTR szWorkingDir, LPCTSTR szDefExt, LPCTSTR szFilters, LPCTSTR szTitle, DWORD dwFlags);
...
typedef struct _PRINTERSTUFF
{
    ...
    BOOL fInitializing;// TRUE during PSN_SETACTIVE to avoid turning on the Apply button...
    BOOL fPortModified;// A random perf flag for IDDC_PRINTTO -- port stuff is slow
    HANDLE hmru;// MRU of ports connected to
...
VOID AddPortToCombobox(HWND hcbPorts, LPTSTR pName, LPTSTR pDescription)
{
    if (CB_ERR == ComboBox_FindString(hcbPorts, 0, pName))
    {
        ...
        if (Str_SetPtr(&lpPortName, pName))
        {
            ...
            {
                lpFree =
                lpPort = ShellConstructMessageString(HINST_THISDLL, MAKEINTRESOURCE(IDS_PRTPROP_PORT_FORMAT), pName, pDescription);
            }

            if (!lpPort)
                lpPort = pName;

            ComboBox_SetItemData(hcbPorts, ComboBox_AddString(hcbPorts, lpPort), lpPortName);
...
#endif
    {
        if(NULL != (lpBuffer=(LPNETRESOURCE)(void*)LocalAlloc(LPTR, 4096)))
        {
            while (TRUE)
            {
                ...
                {
                    if((RESOURCETYPE_PRINT & lpBuffer->dwType) && (RESOURCEUSAGE_CONNECTABLE & lpBuffer->dwUsage) && lpBuffer->lpRemoteName && lpBuffer->lpLocalName)
                    {
                        TCHAR szLocalName[20];
                        WORD wStrLength;

                        // Make sure that the local name terminates with a colon (since this is how it appears from EnumPorts)

                        lstrcpyn(szLocalName,lpBuffer->lpLocalName, ARRAYSIZE(szLocalName));
...
BOOL Printers_EnumPortsCB(LPVOID lpData, HANDLE hPrinter, DWORD dwLevel, LPBYTE pEnum, DWORD dwSize, DWORD *lpdwNeeded, DWORD *lpdwNum)
{
...
BOOL WINAPI CreateUniqueName(LPTSTR lpDest, LPTSTR lpBaseName, WORD  wInstance)
{
    BOOL bSuccess=FALSE;
...
BOOL NewFriendlyName(LPTSTR lpBaseName, LPTSTR lpNewName)
{
...
void DetailDlg_DriverChange(HWND hDlg, LPPRINTERPAGEINFO pppi)
{
    ...
    i = ShellMessageBox(HINST_THISDLL, hDlg, MAKEINTRESOURCE(IDS_PRTPROP_DRIVER_WARN), MAKEINTRESOURCE(IDS_PRINTERS), MB_YESNO|MB_ICONEXCLAMATION);
...
void DetailDlg_NewDriver(HWND hDlg, LPPRINTERPAGEINFO pppi)
{
    ...
    if (ShellMessageBox(HINST_THISDLL, hDlg, MAKEINTRESOURCE(IDS_PRTPROP_DRIVER_WARN), MAKEINTRESOURCE(IDS_PRINTERS), MB_YESNO|MB_ICONEXCLAMATION) != IDYES)
    {
        return;
    }
...
void DetailDlg_SetFields(HWND hDlg, LPPRINTERPAGEINFO pppi)
{
    ...
    SetDlgItemInt(hDlg, IDC_TIMEOUT_NOTSELECTED, pppi->psPrinterStuff->sPrinter.DeviceNotSelectedTimeout/1000, FALSE);
    SetDlgItemInt(hDlg, IDC_TIMEOUT_TRANSRETRY, pppi->psPrinterStuff->sPrinter.TransmissionRetryTimeout/1000, FALSE);
    if (2 == CompareString(LOCALE_SYSTEM_DEFAULT, 0, pppi->psPrinterStuff->sPrinter.pPortName, 3, c_szLPT, 3))
    {
        EnableWindow(GetDlgItem(hDlg, IDC_TIMEOUT_NOTSELECTED), TRUE);
        goto enable_common;
    }
    else if (2 == CompareString(LOCALE_SYSTEM_DEFAULT, 0, pppi->psPrinterStuff->sPrinter.pPortName, 3, TEXT("COM"), 3))
    {
...
BOOL DetailDlg_GetFields(HWND hDlg, LPPRINTERPAGEINFO pppi)
{
    ...
    if (!szPortName[0])
    {
        ...
        ShellMessageBox(HINST_THISDLL, hDlg, MAKEINTRESOURCE(IDS_PRTPROP_PORT_ERROR), MAKEINTRESOURCE(IDS_PRINTERS), MB_OK|MB_ICONINFORMATION);
        return TRUE;
    }
    if (b)
    {
    ...
    pppi->psPrinterStuff->sPrinter.DeviceNotSelectedTimeout = 1000 * GetDlgItemInt(hDlg, IDC_TIMEOUT_NOTSELECTED, &b, FALSE);
    pppi->psPrinterStuff->sPrinter.TransmissionRetryTimeout = 1000 * GetDlgItemInt(hDlg, IDC_TIMEOUT_TRANSRETRY, &b, FALSE);
    return FALSE;
}
...
BOOL CALLBACK Printer_DetailDlgProc(HWND hdlg, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDDC_PRINTTO:
            ...
            if ((GET_WM_COMMAND_CMD(wParam, lParam) == CBN_SELCHANGE) || ((GET_WM_COMMAND_CMD(wParam, lParam) == CBN_KILLFOCUS) && pppi->psPrinterStuff->fPortModified))
            {
                TCHAR szPortName[MAX_PATH];
...
UpdatePrintTo:
        ...
        case IDDB_ADD_PORT:
            ...
            if (DialogBoxParam(HINST_THISDLL, MAKEINTRESOURCE(DLG_ADDPORT), hdlg, Printer_AddPortDlgProc, (LPARAM)GetDlgItem(hdlg, IDDC_PRINTTO)))
            {
        ...
        case IDDB_DEL_PORT:
        {
            ...
            // This may update the contents/selection of the ports combobox
            if (DialogBoxParam(HINST_THISDLL, MAKEINTRESOURCE(DLG_DELPORT), hdlg, Printer_DelPortDlgProc, (LPARAM)&dpd))
            {
                goto UpdatePrintTo; // turns on fApply
            }
            break;
        ...
        case IDDB_SETUP:
            ...
            if (pppi->psPrinterStuff->uFlags & PS_OLDSTYLE)
            {
                ...
                {
                    ShellMessageBox(HINST_THISDLL, hdlg, MAKEINTRESOURCE(IDS_CANTOPENDRIVERPROP), MAKEINTRESOURCE(IDS_PRINTERS), MB_OK|MB_ICONSTOP);
                }
            }
            break;
...
BOOL GeneralDlg_GetFields(HWND hDlg, LPPRINTERPAGEINFO pppi)
{
    ...
    Dlg_CB_GetSelText(hDlg, IDDC_SEPARATOR, pppi->psPrinterStuff->sPrinter.pSepFile, ARRAYSIZE(pppi->psPrinterStuff->sPrinter.pSepFile));
    return FALSE;
}
...
LRESULT GeneralDlg_SaveEverything(HWND hDlg, LPPRINTERPAGEINFO pppi, int fSaveMe)
{
    ...
    if (pPrinter)
    {
        ...
        if (lstrcmp(pPrinter->pPortName, pCopy->pPortName) && pCopy->pPortName[0])
        {
            TCHAR newPrinterPort[ARRAYSIZE(pCopy->pPrinterName) + ARRAYSIZE(pCopy->pPortName)];
            TCHAR oldPrinterPort[ARRAYSIZE(pCopy->pPrinterName) + ARRAYSIZE(pCopy->pPortName)];
            LPTSTR pNewInfo;
            LPTSTR pOldInfo;
            int i;
            ...
            {
                if (CallPrinterCopyHooks(hDlg, PO_PORTCHANGE, 0, pNewInfo, 0, pOldInfo, 0) != IDYES)
                {
...
save_comment:
        ...
        if (lstrcmp(pPrinter->pDriverName, pCopy->pDriverName) && pCopy->pDriverName[0])
        {
...
save_datatype:
        ...
        if (pPrinter5)
        {
            ...
            if (pPrinter5->DeviceNotSelectedTimeout != pCopy->DeviceNotSelectedTimeout || pPrinter5->TransmissionRetryTimeout != pCopy->TransmissionRetryTimeout)
            {
        ...
        if (fPortChange && (fSaveMe == GDSE_SAVE))
        {
            ...
            if (((dwOldAttributes & PRINTER_ATTRIBUTE_NETWORK) != (pppi->psPrinterStuff->sPrinter.Attributes & PRINTER_ATTRIBUTE_NETWORK)) || (!lstrcmp(c_szFileColon, szOldPortName) || !lstrcmp(c_szFileColon, pCopy->pPortName)))
            {
...
handle_error:
        ...
        default:
        {
            ...
            FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwLastError, 0L, buf, ARRAYSIZE(buf), NULL);
            ShellMessageBox(HINST_THISDLL, hDlg, MAKEINTRESOURCE(IDS_PRTPROP_UNKNOWN_ERROR), MAKEINTRESOURCE(IDS_PRINTERS), MB_OK|MB_ICONEXCLAMATION, buf);
            return(PSNRET_INVALID);
            break;
        }
        } // switch (GetLastError())
        ...
        ShellMessageBox(HINST_THISDLL, hDlg, lpError, MAKEINTRESOURCE(IDS_PRINTERS), MB_OK|MB_ICONEXCLAMATION);
        return(PSNRET_INVALID_NOCHANGEPAGE);
    }
