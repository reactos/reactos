...
HRESULT CFTAssocStore::EnumAssocInfo(ASENUM asenumFlags, LPTSTR pszStr, AIINIT aiinitFlags, IEnumAssocInfo** ppEnum)
{
    //for now
...
HRESULT CFTAssocStore::GetComplexAssocInfo(LPTSTR pszStr1, AIINIT aiinitFlags1, LPTSTR pszStr2, AIINIT aiinitFlags2, IAssocInfo** ppAI)
{
    HRESULT hres = E_FAIL;
...
HRESULT CFTAssocStore::CheckAccess()
{
    ...
    if (-1 == _hresAccess)
    {
        ...
        if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CLASSES_ROOT, NULL, 0, KEY_CREATE_SUB_KEY | KEY_READ | KEY_SET_VALUE | KEY_QUERY_VALUE, &hkey))
        {
            _hresAccess = S_OK;
            RegCloseKey(hkey);
        }
    ...
    if (-1 == _hresAccess)
    {
        ...
        while ((S_FALSE == _hresAccess) && (cTry <= 1))
        {
            ...
            if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_CLASSES_ROOT, szGUID, 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hkey, &dwDisp))
            {
