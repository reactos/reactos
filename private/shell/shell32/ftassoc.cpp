...
HRESULT CFTAssocInfo::SetData(AIDWORD aidataFlags, PBYTE pbData, DWORD cbData)
{
        ...
        (AIDATA_PROGIDACTION == aidataFlags))
    {
        ...
        if (ERROR_SUCCESS == lRes)
        {
            ...
            if (ERROR_SUCCESS == lRes)
            {
                lRes = RegCreateKeyEx(hShellKey, _szInitStr2, 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hActionKey, &dwKey);
                RegCloseKey(hShellKey);
            }

            if (ERROR_SUCCESS == lRes)
                lRes = RegCreateKeyEx(hActionKey, TEXT("command"), 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hCommandKey, &dwKey);
            ...
            if (ERROR_SUCCESS == lRes)
            {
                if (pPIDA->fUseDDE)
                {
                    lRes = RegCreateKeyEx(hActionKey, TEXT("ddeexec"), 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hDDEKey, &dwKey);
                    if (ERROR_SUCCESS == lRes)
                    {
                        HKEY hTmpKey = NULL;
                        int iLen = 0;
                        ...
                        {
                            lRes = RegCreateKeyEx(hDDEKey, TEXT("Application"), 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hTmpKey, &dwKey);
                            if (ERROR_SUCCESS == lRes)
                            {
                        ...
                        {
                            lRes = RegCreateKeyEx(hDDEKey, TEXT("IfExec"), 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hTmpKey, &dwKey);
                            if (ERROR_SUCCESS == lRes)
                            {
                        ...
                        if (*(pPIDA->szDDETopic))
                        {
                            lRes = RegCreateKeyEx(hDDEKey, TEXT("Topic"), 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hTmpKey, &dwKey);
                            if (ERROR_SUCCESS == lRes)
                            {
...
HRESULT CFTAssocInfo::Create()
{
    ...
    switch(_aiinitFlags1)
    {
        case AIINIT_EXT:
        {
            ...
            if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_CLASSES_ROOT, _szInitStr1, 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hKey, &dwKey))
            {
                RegCloseKey(hKey);
                hres = S_OK;
            }
...
HRESULT CFTAssocInfo::_SetIconLocation(LPTSTR pszStr)
{
    ...
    if (ERROR_SUCCESS == lRes)
    {
        ...
        if (ERROR_SUCCESS == lRes)
        {
            lRes = RegSetValueEx(hDefaultIconKey, NULL, 0, REG_SZ, (PBYTE)pszStr, lstrlen(pszStr) * sizeof(TCHAR));
