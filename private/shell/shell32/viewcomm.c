...
HKEY SHGetExplorerSubHkey(HKEY hkeyRoot, LPCTSTR szSubKey, BOOL bCreate)
{
    ...
    ASSERT(  lstrlen( szSubKey) + ARRAYSIZE(szRegExplorer) < FULL_KEY_SIZE );
    {
        LONG err;
        DWORD dwDisp;
        err = bCreate ? RegCreateKeyEx(hkeyRoot, szFullKey, 0, NULL, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, NULL, &hkSubKey, &dwDisp) : RegOpenKeyEx  (hkeyRoot, szFullKey, 0, MAXIMUM_ALLOWED, &hkSubKey);
