...
HRESULT CNewMenu::InvokeCommand(LPCMINVOKECOMMANDINFO pici)
{
    ...
    if (PathYetAnotherMakeUniqueName(szPath, szPath, szFileSpec, szFileSpec))
    {
        ...
        {
            SHChangeNotify(fCreateDir ? SHCNE_MKDIR : SHCNE_CREATE, SHCNF_FLUSH, pidlCreatedItem, NULL);
            if (_pShellView2)
            {
                if (!_bMenuBar)
                {
    ...
    if (FAILED(hr) && (HRESULT_CODE(hr) != ERROR_CANCELLED))
        SHSysErrorMessageBox(pici->hwnd, NULL, IDS_CANNOTCREATEFILE, HRESULT_CODE(hr), PathFindFileName(szPath), MB_OK | MB_ICONEXCLAMATION);
...
HRESULT CNewMenu::ConsolidateMenuItems(BOOL bForce)
{
    ...
    //  work.
    if (!bForce &&
        ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_SESSION_SHELLNEW, 0L, KEY_ALL_ACCESS, &hkeyShellNew))
    {
        SHELLNEW_CACHE_STAMP stamp;
        ULONG cbVal = sizeof(stamp);
        if (ERROR_SUCCESS == SHQueryValueEx(hkeyShellNew, REGVAL_SESSION_SHELLNEW_TIMESTAMP, NULL, NULL, (LPBYTE)&stamp, &cbVal) && sizeof(stamp) == cbVal)
        {
        ...
        if (!bUpdate &&
            ERROR_SUCCESS == SHQueryValueEx(hkeyShellNew, REGVAL_SESSION_SHELLNEW_LANG, NULL, NULL, (LPBYTE)&lcid, &cblcid) &&
            sizeof(lcid) == cblcid)
        {
    ...
    if (bUpdate)
    {
        ...
        if (SUCCEEDED((hr = CoCreateInstance(CLSID_SharedTaskScheduler, NULL, CLSCTX_INPROC, IID_IShellTaskScheduler, (void **)&pScheduler))))
        {
...
STDMETHODIMP CNewMenuConsolidator::RunInitRT()
{
    ...
    while (ERROR_SUCCESS == (dwErr = RegCreateKeyEx(HKEY_CURRENT_USER, REGSTR_SESSION_SHELLNEW, 0L, NULL, 0, KEY_ALL_ACCESS, NULL, &hkeyShellNew, &dwDisposition)) && REG_CREATED_NEW_KEY != dwDisposition)
    {
    ...
    if (ERROR_SUCCESS == dwErr)
    {
        ...
        for(i = 0; RegEnumKey(HKEY_CLASSES_ROOT, i, szExt, ARRAYSIZE(szExt)) == ERROR_SUCCESS; i++)
        {
                ...
                && GetClassDisplayName(szClass, szDisplayName, ARRAYSIZE(szDisplayName)))
            {
                ...
                while (GetNewFileInfoForExtension(&noi, NULL, &hkeyIterate, &iIndex))
                {
                    ...
                    RegSetValueEx(hkeyShellNew, noi.szMenuText, NULL, REG_BINARY, (LPBYTE)&noi, sizeof(noi));
                }
            }
        }
        ...
        RegSetValueEx(hkeyShellNew, REGVAL_SESSION_SHELLNEW_TIMESTAMP, NULL, REG_BINARY, (LPBYTE)&stamp, sizeof(stamp));
#ifdef WINNT
        LCID lcid = GetSystemDefaultUILanguage();

        RegSetValueEx(hkeyShellNew, REGVAL_SESSION_SHELLNEW_LANG, NULL, REG_DWORD, (LPBYTE)&lcid, sizeof(lcid));
#endif
    }
...
BOOL CNewMenu::InitMenuPopup(HMENU hmenu)
{
    ...
    HKEY hkeyShellNew;
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_SESSION_SHELLNEW, 0L, KEY_ALL_ACCESS, &hkeyShellNew))
    {
        ...
        for (int i = 0, cnt = 0; ERROR_SUCCESS == RegEnumValue(hkeyShellNew, i, szVal, &cbVal, 0L, &dwType, (LPBYTE)&noi, &cbData); i++)
        {
            if (lstrcmp(szVal, REGVAL_SESSION_SHELLNEW_TIMESTAMP) != 0 && sizeof(noi) == cbData && REG_BINARY == dwType)
            {
                SHFILEINFO sfi;
                _himlSystemImageList = (HIMAGELIST)SHGetFileInfo(noi.szExt, FILE_ATTRIBUTE_NORMAL, &sfi, sizeof(SHFILEINFO), SHGFI_USEFILEATTRIBUTES | SHGFI_SYSICONINDEX | SHGFI_SMALLICON);
