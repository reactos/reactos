...
BOOL ProcessDarwinAd(IShellLinkDataList* psldl, LPCITEMIDLIST pidlFull)
{
    ...
    if (!psldl)
    {
        ...
        if (FAILED(CoCreateInstance(CLSID_ShellLink, NULL, CLSCTX_INPROC, IID_IShellLinkDataList, (void**)&psldl)))
        {
            return FALSE;
        }
...
HRESULT GetFastItemFolder(IShellFolder** ppsf, LPITEMIDLIST* ppidl)
{
    ...
    hres = CoCreateInstance(CLSID_AugmentedShellFolder, NULL, CLSCTX_INPROC, IID_IAugmentedShellFolder, (void **)&pasf);
#endif
    if (SUCCEEDED(hres))
    {
...
HRESULT CStartMenuCallback::InitializeProgramsShellMenu(IShellMenu* psm)
{
    ...
    RegCreateKeyEx(HKEY_CURRENT_USER, STRREG_STARTMENU TEXT("\\Programs"), NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hkeyPrograms, &dwDisp);

#ifdef WINNT
    IAugmentedShellFolder2* psf = NULL;
    hres = CoCreateInstance(CLSID_AugmentedShellFolder2, NULL, CLSCTX_INPROC, IID_IAugmentedShellFolder2, (void **)&psf);
    if (SUCCEEDED(hres))
    {
    ...
    if (pidl)
    {
        ...
        if (SUCCEEDED(SHBindToObject(NULL, IID_IShellFolder, pidl, (void **)&psfFolder)))
        {
            ...
            if (pszRoot)
            {
                ...
                RegCreateKeyEx(HKEY_CURRENT_USER, szPath, NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKey, &dwDisp);
            }
...
HRESULT CStartMenuTask::_CreateNewTask(IShellFolder * psf, LPCITEMIDLIST pidl)
{
    ...
    hres = CoCreateInstance( CLSID_SharedTaskScheduler, NULL, CLSCTX_INPROC, IID_IShellTaskScheduler, (void **) & pScheduler );
    if (SUCCEEDED(hres))
    {
        // create a new task
        IStartMenuTask * psmt;

        if (SUCCEEDED(CoCreateInstance(CLSID_StartMenuTask, NULL, CLSCTX_INPROC, IID_IStartMenuTask, (void **)&psmt)))
        {
            // Initialize the task
