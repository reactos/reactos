...
class CCopyMoveToMenu : public IContextMenu3, public IShellExtInit, public CObjectWithSite
{
public:
    // IUnknown
...
{
    // if they want the default menu only (CMF_DEFAULTONLY) OR this is being called for a shortcut (CMF_VERBSONLY)
...
int BrowseCallback(HWND hwnd, UINT msg, LPARAM lParam, LPARAM lpData)
{
    ...
    case BFFM_VALIDATEFAILEDA:
        ...
        ShellMessageBox(g_hinst, hwnd, MAKEINTRESOURCE(idResource), MAKEINTRESOURCE(IDS_CMTF_COPYORMOVE_DLG_TITLE), MB_OK | MB_ICONERROR, (LPVOID) lParam);
...
HRESULT CCopyMoveToMenu::_DoDragDrop(LPCMINVOKECOMMANDINFO pici, LPCITEMIDLIST pidlFolder)
{
    ...
    // This should always succeed because the caller (SHBrowseForFolder) should have weeded out the non-folders.
    ...
    if (hres != S_OK)
    {
        ...
        ShellMessageBox(g_hinst, pici->hwnd, MAKEINTRESOURCE(IDS_CMTF_ERRORMSG), MAKEINTRESOURCE(IDS_CABINET), MB_OK | MB_ICONEXCLAMATION);
    ...
    if (m_pdtobj)
    {
        ...
        if (pstrm)
        {
            if (S_OK == hres)
            {
                ...
                if (SUCCEEDED(SHGetNameAndFlags(pidlFolder, SHGDN_FORPARSING, szFolder, SIZECHARS(szFolder), NULL)) && !PathIsUNC(szFolder) && !IsRemoteDrive(PathGetDriveNumber(szFolder)))
                {
                    ULARGE_INTEGER uli;

                    // rewind the stream to the beginning so that when we add a new pidl it does not get appended to the first one
...
HRESULT CCopyMoveToMenu::Initialize(LPCITEMIDLIST pidlFolder, IDataObject *pdtobj, HKEY hkeyProgID)
{
    ...
    // See comments above the call to HDXA_AppendMenuItems2 in defcm.cpp!CDefFolderMenu::QueryContextMenu.  Raid #232106
    if (!pidlFolder)
    {
