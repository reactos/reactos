...
HRESULT CFolderOptionsPsx::AddPages(LPFNADDPROPSHEETPAGE AddPage, LPARAM lParam)
{
    if (!m_pgfs)
    {
        HRESULT hres = CoCreateInstance(CLSID_GlobalFolderSettings, NULL, CLSCTX_INPROC_SERVER, IID_IGlobalFolderSettings, (void **)&m_pgfs);
        if (FAILED(hres))
            return hres;
    }
    ...
    if (!m_pbs2)
    {
        IUnknown_QueryService(_punkSite, SID_SShellBrowser, IID_IBrowserService2, (void **)&m_pbs2);
    }

    PROPSHEETPAGE psp;

    /*
     *  We used to do this only if we aren't a rooted Explorer, but TOuzts says to do it always.

     *  The lParam is a pointer back to ourselves so the page can figure out why it was created and so the two pages can talk to each other.
     */
