...
void AdvancedDlgOnNotify(LPADVANCEDPAGE pAdv, LPNMHDR psn)
{
    ...
    case PSN_APPLY:
    {
        ...
        if(pAdv->fDirty) //We will save these only if something changed.
        {
            ...
            SendNotifyMessage(HWND_BROADCAST, WM_WININICHANGE, 0, 0);// Let everybody know about the new settings
            Cabinet_RefreshAll();// Okay now tell all Cabinets to refresh
        }
    ...
    if ( pAdv )
    {
        ...
        case WM_HELP:                   // F1
        {
            ...
            else
            {
                ...
                {
                    WinHelp((HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD_PTR)(LPSTR)aAdvOptsHelpIDs);
                }
            }
            break;
        }
        case WM_CONTEXTMENU:        // right mouse click
        {
            ...
            {
                WinHelp( (HWND) wParam, TEXT(SHELL_HLP), HELP_CONTEXTMENU, (ULONG_PTR)(LPSTR)aAdvOptsHelpIDs);
            }
            break;
        }
        case WM_DESTROY:
