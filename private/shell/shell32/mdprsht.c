...
//drivesx.c
BOOL _DrvGeneralDlgProc(HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam);


int GetDriveIDForVolumeName(IN  PWSTR   Name)
{
    WCHAR   driveName[10];
    UCHAR   driveLetter;
    ...
    for (driveLetter = 'C'; driveLetter <= 'Z'; driveLetter++) {
        ...
        b = GetVolumeNameForVolumeMountPointW(driveName, otherName, MAX_PATH);
        if (!b) {
            continue;
        }
...
BOOL InitMountedDrvPrsht(FILEPROPSHEETPAGE * pfpsp)
{
    ...
    SHGetFileInfo(pfpsp->szPath, 0, &sfi, SIZEOF(sfi), SHGFI_ICON|SHGFI_LARGEICON | SHGFI_DISPLAYNAME| SHGFI_TYPENAME);
    if (sfi.hIcon)
    {
    ...
    if (!bMounted)
    {
        ...
        if (!GetVolumeInformation(pfpsp->szPath, szBuffer, ARRAYSIZE(szBuffer), NULL, NULL, NULL, NULL,0))
        {
            szBuffer[0] = 0;
        }
    }
    else
    {
        ...
        if (!GetVolumeInformation(szVolName, szBuffer, ARRAYSIZE(szBuffer), NULL, NULL, NULL, NULL,0))
        {
                szBuffer[0] = 0;
        }
    }
    ...
    SetInitialFileAttribs(pfpsp, fd.dwFileAttributes, fd.dwFileAttributes);// set the initial attributes
    memcpy(&pfpsp->asCurrent, &pfpsp->asInitial, SIZEOF(pfpsp->asCurrent));// set the current attributes to the same as the initial
    SetDateTimeText(pfpsp->hDlg, IDC_DRV_CREATED, &fd.ftCreationTime);// Set the created  time

    // now setup all the controls on the dialog based on the attribs that we have
        ...
        case WM_COMMAND:
        {
            switch (GET_WM_COMMAND_ID(wParam, lParam))
            {
                ...
                case IDD_READONLY:
                    break;
                case IDC_ADVANCED:
                    DialogBoxParam(HINST_THISDLL, MAKEINTRESOURCE(DLG_FOLDERATTRIBS), hDlg, AdvancedFileAttribsDlgProc, (LPARAM)pfpsp);
                    break;
        ...
        case WM_NOTIFY:
        {
            switch (((NMHDR *)lParam)->code)
            {
                ...
                case PSN_APPLY:
                {
                    ...
                    bRet = DialogBoxParam(HINST_THISDLL, MAKEINTRESOURCE(DLG_ATTRIBS_RECURSIVE), hDlg, RecursivePromptDlgProc, (LPARAM)pfpsp);
                    if (bRet)
                    {
...
HPROPSHEETPAGE AddMountedDrvPage(LPCTSTR pszFile, LPFNADDPROPSHEETPAGE pfnAddPage, LPARAM lParam, BOOL bMounted)
{
    ...
    fpsp.fIsMountedDrive = BOOLIFY(bMounted);//Are we showing this page for mounted volume or for an ordinary volume
    fpsp.fIsDirectory = TRUE;// we set fIsDirectory to always be true, since this is a volume

    lstrcpy(fpsp.szPath , pszFile);

    // if we are not showing this dialog for mounted volume change the title to from "General" to "Contents
    if (!bMounted)
    {
