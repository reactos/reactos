...
UINT _ValidateLocalMem(HLOCAL hMem, LPARAM lParam, LPCTSTR pszText)
{
    ...
    if (hMem)
    {
        ...
        if (uSize)
        {
            ...
            AssertMsg(uByte+CBALLOCEXTRA <= uSize, TEXT("cm ASSERT! Bogus uByte %x (%x, %x) (%s for %x)"),
                      uByte, uByte+CBALLOCEXTRA, uSize, pszText, hMem);
            lParamStored = *(UNALIGNED LPARAM *)HMEM2PTR(hMem, uByte);
            AssertMsg( lParamStored==lParam, TEXT("cm ASSERT! Bad Signiture %x!=%x (%s for %x)"), lParamStored, lParam, pszText, hMem);
        }
        else
        {
            AssertMsg( uSize!=0, TEXT("cm ASSERT! LocalSize is zero (%s for %x)"), pszText, hMem);
        }
    }
