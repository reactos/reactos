...
{
    BOOL bRet = FALSE;
    HANDLE hFile = CreateFile(pszFile, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_NO_BUFFERING | FILE_FLAG_OPEN_NO_RECALL, NULL);
...
STDAPI_(void) SHGetSetSettings(LPSHELLSTATE lpss, DWORD dwMask, BOOL bSet)
{
    ...
    if (bSet) {
        ...
        if ((dwMask & SSF_SORTCOLUMNS) && ((g_pShellState->ss.lParamSort != lpss->lParamSort) || (g_pShellState->ss.iSortDirection != lpss->iSortDirection))) {
    ...
    if (fUpdateShellStateInReg || fSave || fSaveAdvanced) {
        ...
        HKEY hkey = SHGetExplorerHkey(HKEY_CURRENT_USER, TRUE);
        if (hkey) {
            RegSetValueEx(hkey, TEXT("ShellState"), 0L, REG_BINARY, (LPBYTE)g_pShellState, g_pShellState->cbSize);
            RegCloseKey(hkey);
        }
    }
...
STDAPI_(void) CheckWinIniForAssocs(void)
{
    ...
    LocalFree((HLOCAL)pszBuf);// And cleanup our allocation
}
        ...
        if (RegCreateKeyEx(HKEY_CURRENT_USER, EXPLORER_CLSID, 0, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkeyTo, &dwDisp) == ERROR_SUCCESS) {
            ...
            if (RegOpenKeyEx(HKEY_CURRENT_USER, SOFTWARE_CLASSES_CLSID, 0, KEY_ALL_ACCESS, &hkeyFrom) == ERROR_SUCCESS) {
                ...
                if (ERROR_SUCCESS == RegQueryInfoKey(hkeyFrom, NULL, NULL, NULL,
                    ...
                    for (; RegEnumKeyEx(hkeyFrom, --dwIndex, szSubKey, &cchSubKeySize, NULL, szClass, &cchClassSize, NULL) == ERROR_SUCCESS; cchSubKeySize = ARRAYSIZE(szSubKey), cchClassSize = ARRAYSIZE(szClass)) {
                        HKEY hkey;
...
// BUGBUG: this depends on the HDROP being in the TCHAR sense of this code.
// that does not need to be. to fix this use DragQueryFile() to extrac the file names into the list.
