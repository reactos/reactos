...
BOOL APIENTRY IsStringInList(LPWSTR lpS, LPWSTR lpList)
{
   while (*lpList) {
...
BOOL APIENTRY SheShortenPathA(LPSTR pPath, BOOL bShorten)
{
   ...
   WideCharToMultiByte(CP_ACP, 0, pPathW, -1, pPath, MAX_PATH, NULL, NULL);
...
BOOL APIENTRY SheShortenPathW(LPWSTR pPath, BOOL bShorten)
{
   ...
   *pDest = WCHAR_NULL;
   ...
   pNewName = bShorten ? FindData.cAlternateFileName : FindData.cFileName;
...
*/
WORD SearchForFile(LPCWSTR lpDir, LPWSTR lpFile, LPWSTR lpFullPath, DWORD cchFullPath, LPWSTR lpExt)
{
   LPWSTR lpT;
   LPWSTR lpD;
   LPWSTR lpExts;
...
// OUT       ppArgs        Pointer to args in lpCmdLine, _incl_ leading space OPTIONAL

// Return:   DWORD length of path, 0 = fail

// Assumes:  len of executable in lpCmdLine is < MAX_PATH
//           len of exts are < 64
DWORD QualifyAppName(IN LPCWSTR lpCmdLine, OUT LPWSTR lpImage, OPTIONAL OUT LPCWSTR* ppArgs)
{
   LPWSTR lpAppName;
   ...
   while (*lpCmdLine && (*lpCmdLine != WCHAR_SPACE || bAppNameInQuotes)) {
...
BOOL SheConvertPathW(LPWSTR lpCmdLine, LPWSTR lpFile, UINT   cchCmdBuf)
/*++
Routine Description:
   Takes a command line and file and shortens both if the app in the command line is dos/wow.
...
Return Value:
    ...
    There are pathalogoical "lfns" (Single unicode chars) that can actually get longer when they are shortened.
    In this case, we won't AV, but we will truncate the parms!

    // Qualify path assumes that the second parm is a buffer of
    // size atleast MAX_PATH, which is nicely equivalent to MAX_PATH needs cleanup!
--*/
{
    LPWSTR lpszFullPath;
    LONG lBinaryType;
    ...
    lpszFullPath = (LPWSTR) LocalAlloc(LMEM_FIXED, cchCmdBuf*sizeof(*lpCmdLine));
    if (!lpszFullPath)
       return bRetVal;

    lstrcpy(lpszFullPath, lpCmdLine);// We must do the swap here since we need to copy the parms back to lpCmdLine.

    if (QualifyAppName(lpszFullPath, lpCmdLine, &lpArgs)) {
        if (!GetBinaryType(lpCmdLine, &lBinaryType) || lBinaryType == SCS_DOS_BINARY || lBinaryType == SCS_WOW_BINARY) {
        ...
        CheckEscapes(lpCmdLine, cchCmdBuf);// Must readd quotes

        cchNewLen = lstrlen(lpCmdLine);
        StrNCpy(lpCmdLine+cchNewLen, lpArgs, cchCmdBuf-cchNewLen);
    } else {
        // QualifyAppName failed, restore the command line back to the original state.
