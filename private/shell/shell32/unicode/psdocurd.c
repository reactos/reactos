...
INT
SheSetEnvVarW(WCHAR *varname, WCHAR *varvalue)
{
   if (!wcslen(varvalue)) {
      varvalue = NULL; // null to remove from env
   }
...
INT
SheGetPathOffsetW(LPWSTR lpszDir)
{
   // sanity
   if (!lpszDir || !*lpszDir) {
      return(-1);
...
INT SheFullPathW(register WCHAR *fname, DWORD sizpath, WCHAR *buf)
{
   DWORD rc = SUCCESS;         /* prime with good rc */
   DWORD buflen;               /* buffer length      */
   WCHAR *filepart;
   ...
   if (*fname == WCHAR_NULL) {
      ...
      if ((wcslen(fname) == 2) && (*(fname + 1) == WCHAR_COLON)
          // && (!is_dbcsleadchar(*fname))
         ) {
