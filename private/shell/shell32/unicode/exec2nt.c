...
BOOL APIENTRY LocateHookProc(HWND hDlg, UINT uiMessage, WPARAM wParam, LONG lParam)
{
    WCHAR szTemp[40];
    WORD wID;
    ...
    case WM_COMMAND:
        ...
        case ctlLast + 1:
            ...
            if (SendDlgItemMessage(hDlg, lst1, LB_FINDSTRING, (WPARAM) -1, (LONG_PTR) (LPSTR) szTemp) >= 0) {
                wID = IDS_PROGFOUND;
            }
            else {
        ...
        case cmb2:
            ...
            case CBN_SELCHANGE:
                ...
                break;
            case CBN_CLOSEUP:
                PostMessage(hDlg, WM_COMMAND, GET_WM_COMMAND_MPS(cmb2, GetDlgItem(hDlg, cmb2), CBN_SELCHANGE));
                break;
            }
            break;
        case IDOK:
...
BOOL APIENTRY FindExeDlgProcW(HWND hDlg, register UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDD_BROWSE:
        {
            ...
            */
            ...
            ofn.lpstrTitle = NULL;
            ofn.Flags = OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_ENABLETEMPLATE | OFN_SHOWHELP;
            ...
            if (hCommDlg < (HANDLE) 32 && (hCommDlg = LoadLibrary(TEXT("comdlg32.dll"))) < (HANDLE) 32) {
            CommDlgError:
            ...
            if (!(lpfnGetOpenFileName = (LPFNGETOPENFILENAME) GetProcAddress((HINSTANCE) hCommDlg, (LPSTR) szGetOpenFileName)))
                goto CommDlgError;
        ...
        case IDOK:
        {
            ...
            if (lpfind) {
                ...
                switch (*CharPrev(lpfind->lpszPath, lpTemp = lpfind->lpszPath + lstrlen(lpfind->lpszPath))) {
                case WCHAR_BSLASH:
                case WCHAR_COLON:
                    break;
                default:
...
HANDLE APIENTRY FindAssociatedExeW(HWND hwnd, LPWSTR lpCommand, LPWSTR lpName)
// Returns -1 if we found the file (and it was not in an obvious place)
// or an error code which will be returned to the app (see the error returns for ShellExecute)
{
    FINDEXE_PARAMS find;
     ...
     * Check if the file can be opened; if it can, then some DLL could not be found during the WinExec, so return file not found
     */
    if (NULL != (lpTemp = StrRChrW(szPath, NULL, WCHAR_BSLASH)) || NULL != (lpTemp = StrRChrW(szPath, NULL, WCHAR_COLON))) {
        ++lpTemp;
    }
    else {
        lpTemp = szPath;
    ...
    wcscpy(szExe, lpTemp);// store the file name component
    ...
    CheckEscapesW(szExe, MAX_PATH);// add back the quotes, if necessary
    GetProfileString(szPrograms, szExe, szNull, szPath, ARRAYSIZE(szPath));// look in win.ini
    ...
    switch (DialogBoxParam(HINST_THISDLL, MAKEINTRESOURCE(DLG_FINDEXE), hwnd, (DLGPROC) FindExeDlgProcW, (LONG_PTR) (LPFINDEXE_PARAMS) &find)) {
    case IDOK:
        wcscat(szPath, lpSpace);      // add the parameters
        wcscpy(lpCommand, szPath);    // return the new path
        return ((HANDLE) -1);
