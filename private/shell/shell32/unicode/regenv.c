...
*/
{
    ...
    Length = 0;
    Status = RtlExpandEnvironmentStrings_U( pEnv, (PUNICODE_STRING)&Source, (PUNICODE_STRING)&Destination, &Length);
...
/*
* Reads the user-defined environment variables from the user registry and adds them to the environment block at pEnv.
...
*/
{
    ...
    cbData = cbDataBuffer;
    bResult = TRUE;
    while (!RegEnumValue(hkey, dwIndex, lpValueName, &cbValueName, 0, &dwType, lpData, &cbData)) {
        if (cbValueName) {
            // Limit environment variable length
            lpData[MAX_VALUE_LEN-1] = TEXT('\0');

            if (dwType == REG_SZ) {
                // The path variables PATH, LIBPATH and OS2LIBPATH must have their values apppended to the system path.
...
/*
* Reads the system environment variables from the LOCAL_MACHINE registry and adds them to the environment block at pEnv.
...
*/
{
    ...
    // First start by getting the systemroot and systemdrive values and setting it in the new environment.
    ...
    // To generate the environment, this requires two passes.
    // First pass sets all the variables which do not need to be expanded.
    // The second pass expands variables (so it can use the variables from the first pass.
    while (!RegEnumValue(hkey, dwIndex, lpValueName, &cbValueName, 0, &dwType, lpData, &cbData)) {
        if (cbValueName) {
            // Limit environment variable length
            lpData[MAX_VALUE_LEN-1] = TEXT('\0');
    ...
    *lpt = 0;
    ...
    while (NULL != (lpPath = wcsstr (lpValue, TEXT("%")))) {
        if (!_wcsnicmp(lpPath+1, TEXT("PATH%"), 5)) {
            // check if we have an autoexecpath already set, if not just remove the %path%
...
*/
{
    ...
    *lpt = c;
    ...
    if (!*lpt) {
        // if we have a blank path statement in the autoexec file, then we don't want to pass "PATH" as the environment variable because it trashes the system's PATH.
        // Instead we want to change the variable AutoexecPath.
        // This would have be handled below if a value had been assigned to the environment variable.
    ...
    *lpt = c;
    ...
    // If the path includes removable drive, it is assumed that the drive assignment has changed from DOS.
...
BOOL AppendNTPathWithAutoexecPath(void **pEnv, LPTSTR lpPathVariable, LPTSTR lpAutoexecPath)
/*
* Gets the AutoexecPath created in ProcessAutoexec, and appends it to the NT path.
...
STDAPI_(BOOL) GetUserNameAndDomain(LPTSTR *ppszUserName, LPTSTR *ppszUserDomain)
{
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &hToken))
    {
        ...
        if (GetTokenInformation(hToken, TokenUser, NULL, 0, &cbTokenBuffer) ||  GetLastError() == ERROR_INSUFFICIENT_BUFFER)
        {
            ...
            if (pUserToken)
            {
                if (GetTokenInformation(hToken, TokenUser, pUserToken, cbTokenBuffer, &cbTokenBuffer))
                {
                    ...
                    if (LookupAccountSid(NULL, pUserToken->User.Sid, NULL, &cbAccountName, NULL, &cbUserDomain, &SidNameUse) || GetLastError() == ERROR_INSUFFICIENT_BUFFER)
                    {
                        ...
                        if (pszUserName && pszUserDomain && LookupAccountSid(NULL, pUserToken->User.Sid, pszUserName, &cbAccountName, pszUserDomain, &cbUserDomain, &SidNameUse))
                        {
                            if (ppszUserName)
                            {
...
*/
{
    ...
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Windows\\CurrentVersion"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        if (SHQueryValueEx (hKey, TEXT("ProgramFilesDir"), NULL, &dwType, (LPBYTE) szValue, &dwMaxBufferSize) == ERROR_SUCCESS) {
        ...
        dwMaxBufferSize = sizeof(szValue);
        if (SHQueryValueEx (hKey, TEXT("CommonFilesDir"), NULL, &dwType, (LPBYTE) szValue, &dwMaxBufferSize) == ERROR_SUCCESS) {
            SHExpandEnvironmentStrings (szValue, szExpValue, ARRAYSIZE(szExpValue));
...
#ifdef WX86
        dwMaxBufferSize = sizeof(szValue);
        if (SHQueryValueEx (hKey, TEXT("ProgramFilesDir (x86)"), NULL, &dwType, (LPBYTE) szValue, &dwMaxBufferSize) == ERROR_SUCCESS) {
            SHExpandEnvironmentStrings(szValue, szExpValue, ARRAYSIZE(szExpValue));
            SetUserEnvironmentVariable(&pEnv, PROGRAMFILESX86_VARIABLE, szExpValue, TRUE);
        }

        dwMaxBufferSize = sizeof(szValue);
        if (SHQueryValueEx (hKey, TEXT("CommonFilesDir (x86)"), NULL, &dwType, (LPBYTE) szValue, &dwMaxBufferSize) == ERROR_SUCCESS) {
            SHExpandEnvironmentStrings (szValue, szExpValue, ARRAYSIZE(szExpValue));
    ...
    if (RegCreateKeyEx (HKEY_CURRENT_USER, PARSE_AUTOEXEC_KEY, 0, 0, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKey, &dwDisp) == ERROR_SUCCESS) {
        // Query the current value.  If it doesn't exist, then add the entry for next time.
        dwMaxBufferSize = sizeof (TCHAR) * MAX_PARSE_AUTOEXEC_BUFFER;
        if (SHQueryValueEx (hKey, PARSE_AUTOEXEC_ENTRY, NULL, &dwType, (LPBYTE) szParseAutoexec, &dwMaxBufferSize) != ERROR_SUCCESS) {
