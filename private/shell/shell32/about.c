...
#ifdef WINNT
// BUGBUG - BobDay - This is bogus, NT should just store the info in the same place...
...
#endif
{
    ...
    return (int)DialogBoxParam(HINST_THISDLL, (LPTSTR)MAKEINTRESOURCE(DLG_ABOUT), hWnd, AboutDlgProc, (LPARAM)&ap);
}
...
INT  APIENTRY ShellAboutA( HWND hWnd, LPCSTR szApp, LPCSTR szOtherStuff, HICON hIcon)
{
   ...
   if (szApp) {
      ...
      if (!(lpszAppW = (LPWSTR)LocalAlloc(LMEM_FIXED, (cchLen * SIZEOF(WCHAR))))) {
         ...
         MultiByteToWideChar(CP_ACP, 0, (LPSTR)szApp, -1, lpszAppW, cchLen);
      }
   } else {
      lpszAppW = NULL;
   }
   ...
   if (szOtherStuff) {
      ...
      if (!(lpszOtherStuffW = (LPWSTR)LocalAlloc(LMEM_FIXED, (cchLen * SIZEOF(WCHAR))))) {
         if (lpszAppW) {
            LocalFree(lpszAppW);
         }
         return(0);
      } else {
         MultiByteToWideChar(CP_ACP, 0, (LPSTR)szOtherStuff, -1, lpszOtherStuffW, cchLen);
      }
   } else {
...
void _InitAboutDlg(HWND hDlg, LPABOUT_PARAMS lpap)
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_SETUP, 0, KEY_READ, &hkey) == ERROR_SUCCESS)
    {
        ...
        if (NULL != (lpszValue = (LPTSTR)LocalAlloc(LPTR, cb)))
        {
            ...
            if (!err) {
                /*
                 * We have an OEM ID, so hide the product ID controls, and display the text.
                 */
    ...
    case WM_PAINT:
        {
            ...
            if (hdcMem)
            {
                ...
                {
                    hbmAbout = LoadImage(HINST_THISDLL, MAKEINTRESOURCE(fDeep ? IDB_ABOUT_SA256:IDB_ABOUT_SA16), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
                }
                else
                {
                    hbmAbout = LoadImage(HINST_THISDLL, MAKEINTRESOURCE(fDeep ? IDB_ABOUT256:IDB_ABOUT16), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
                }
                ...
                hbmBand = LoadImage(HINST_THISDLL, MAKEINTRESOURCE(fDeep ? IDB_ABOUTBAND256:IDB_ABOUTBAND16), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
                if ( hbmBand )
                {
