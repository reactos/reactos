...
void BldSearchLsts( LPGREPINFO lpgi, LPSTR pStrLst, LPSTR pNotList )
{
    ...
    for ( szNext = pStrLst, i = 0; i < 3; i++ )
    {
        while ( *szNext != '\0' )
        {
            ...
            if ( lpgi->addstr == addexpr || findlist( lpgi, szNext, szNext + iStrLen ) == NULL )
                                             ...
                                             unsigned long ulArg2 ) )
{
    ...
    while ( ReadFile(fh, BufPtr, SectorBased( FILBUFLEN - TailLen), &ByteCount, NULL) && (ByteCount + TailLen != 0))
    {
        if ( ByteCount == 0 )
        ...
        while ( pchChar != NULL && EndBuf > pchChar && (EndBuf - pchChar) >= lpgi->ge.ShortStrLen )
        {
            if ( !(fFlags & FIND_FILE) )
            {
#ifdef DO_CALLBACKS
                ...
                {
                    ulMatchCnt++;
                    if ( !(fFlags & FIND_NOMATCH) && !(fFlags & FIND_COUNT) )
                        iStatus = CB_FindMatch( pchEnd - pchBegin, pchBegin, ulLineNum );
                }
