 ...
 *  in:
 *       lpIconPath      path of thing to extract icon for (may be an exe or something that is associated)
...
UINT WINAPI ExtractIconEx(LPCTSTR szFileName, int nIconIndex, HICON *phiconLarge, HICON *phiconSmall, UINT nIcons)
{
    ...
    if (phiconLarge && phiconSmall && nIcons == 1)
    {
        ...
        result = ExtractIcons(szFileName,
        nIconIndex,
...
UINT WINAPI ExtractIcons(LPCTSTR szFileName, int nIconIndex, int cxIcon, int cyIcon, HICON *phicon, UINT *piconid, UINT nIcons, UINT flags)
{
    ...
    // open the file - First check to see if it is a UNC path.  If it is make sure that we have access to the path...
    ...
    else
    {
        if (SearchPath(NULL, szExpFileName, NULL, ARRAYSIZE(achFileName), achFileName, NULL) == 0)
        {
            ...
            TraceMsg(TF_IMAGE, "ExtractIcon:SearchPath Fail(\"%s\") ==> %d", szExpFileName, result);
            goto error_file;
        }
    }
    ...
    hFile = CreateFile(achFileName, GENERIC_READ|FILE_WRITE_ATTRIBUTES, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
    if (hFile == INVALID_HANDLE_VALUE)
    {
        hFile = CreateFile(achFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
        if (hFile == INVALID_HANDLE_VALUE)
            goto error_file;
    }
    else
...
again:
    hicon = LoadImage(HINST_THISDLL, szFile, IMAGE_ICON, LOWORD(cxIcon), LOWORD(cyIcon), flags);
    if (hicon == NULL)
        return 0;
...
Retry:
    ...
    if (hIcon == NULL)
    {
        ...
        if (!*wszExePath || fAssociated || (*wszExePath && (lstrcmpi(lpIconPath, wszExePath) == 0)))
        {
            LPTSTR lpId;
            WORD wDefIconId;
            HANDLE h;
...
HICON APIENTRY ExtractAssociatedIconExA(HINSTANCE hInst, LPSTR lpIconPath, LPWORD lpiIconIndex, LPWORD lpiIconId)
{
   ...
   if (lpIconPath) {
      ...
      try {
         WideCharToMultiByte(CP_ACP, 0, (LPWSTR)IconPathW, -1, lpIconPath, CCHICONPATHMAXLEN, NULL, &fDefCharUsed);
      } except(EXCEPTION_EXECUTE_HANDLER) {
         hIcon = NULL;
      }
   }
...
HICON APIENTRY ExtractAssociatedIconW(HINSTANCE hInst, LPWSTR lpIconPath, LPWORD lpiIconId)
{
    WORD wIconIndex;
...
#ifndef WINNT
// HACK so that we work with the OSR2 SHELL.DLL thunk scripts. We don't want to change SHELL.DLL when we install on OSR2 so that we can do no reboot.
