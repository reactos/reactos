...
BOOL _QualifyWorkingDir(LPCTSTR pszPath, LPTSTR pszDir, DWORD cchDir)
{
    ...
    //   2) a drive or a root path, or a relative path specified derive the working dir from the qualified path.
    // this is to make sure the working dir for setup programs "A:setup" is set right
...
BOOL ShellExecCmdLine(HWND hwnd, LPCTSTR pszCommand, LPCTSTR pszDir, int nShow, LPCTSTR pszTitle, DWORD dwFlags)
{
    TCHAR szWD[MAX_PATH];
    ...
    // this needs to be here.  app installs rely on the current directory to be the directory with the setup.exe
    if (!UrlIs(szFileName, URLIS_URL) && ((dwFlags & SECL_USEFULLPATHDIR) || !pszDir))
    {
