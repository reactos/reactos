...
HRESULT CFileSysColumnProvider::_LookupOwnerName(LPCTSTR pszFile, VARIANT *pvar)
{
    ...
    if (err == ERROR_SUCCESS)
    {
        ...
        else
        {
            ...
            if (LookupAccountSid(pszServer, psid, szName, &cchName, szDomain, &cchDomain, &snu))
            {
                //  If the domain is the bogus "BUILTIN" or we don't have a domain at all, then just use the name.  Otherwise, use domain\userid.
