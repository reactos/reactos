 ...
 * Global delarations
*/
// We need this global (g_pdsvlvp) for the mouse hook we need to implement the combined view.
// Since we only have one combined view at this point it is sufficient to have a single global, but if we end up with more than one combined view then
// there needs to be some additional code added so the hook(s) can figure out which combined view(s) it is associated with.
...
#else  // This code works well for general painting, like minimizing a window or or surfacing the desktop.
        // However, it doesn't help the drag full windows scenario very much.
        HRGN hrgn, hrgn2;
        RECT rc;
        HWND hwndT = GetSpecialSibling(hwnd);
...
#ifdef CRAZY_CODE
    if ((iMsg == WM_TIMER) && (wParam == MAGIC_TIMER) && ((GetTickCount() - dwLastPaint) > MAGIC_TIMEOUT)) {
        if (hrgnSave) {
            SetWindowLong(hwnd, GWL_EXSTYLE, GetWindowLong(hwnd, GWL_EXSTYLE) & ~WS_EX_TRANSPARENT);
...
// We need to set capture while we are stealing the mouse so that the window manager won't send WM_SETCURSOR messages to the wrong window.
// This function will also, as a side effect, send the WM_SETCURSOR to the listview so that it will set the hand cursor when it needs to.
void StealMouse(DVLVPROP * pdvlvp, BOOL fSteal, UINT msg)
{
    ...
    if (fSteal && (hwndCapture == NULL || hwndCapture == HWNDLISTVIEW)) {
        ...
        // try to send the wm_setcursor message to the wrong window, and we send it here ourself to the listview.
...
#define LPMSG ((LPMSG)lParam)
    ...
    if (g_pdsvlvp) {
        if (LPMSG->message >= WM_MOUSEFIRST && LPMSG->message <= WM_MOUSELAST) {
            ...
            // Unhook our hook when all of the mouse buttons are up and we're not over an item in the listview
            if (GetKeyState(VK_LBUTTON) >= 0 && GetKeyState(VK_RBUTTON) >= 0 && GetKeyState(VK_MBUTTON) >= 0 && iHit == -1) {
        ...
        // If we've just unhooked, or the hover is coming through to the listview and no mouse button is down then clear our ownership of the mouse
#define MK_BUTTON (MK_LBUTTON | MK_RBUTTON | MK_MBUTTON)
        if (!hHookNext ||
            // We need to special case the WM_MOUSEHOVER here so that the listview is able to implement hover select.
            // If we have capture set when the hover select message goes through then it will ignore the message, so clear the capture now.
...
BOOL ShouldStealMouseClick(POINT * ppt, DVLVPROP * pdsvlvp)
{
    ...
    if (SUCCEEDED(pdsvlvp->pdsv->GetItemObject(SVGIO_BACKGROUND, IID_IHTMLDocument2, (LPVOID*) &pihtmldoc2))) {
        // BUGBUG (reinerf)
        // elementFromPoint is returning success here even though pielem is still NULL, so we need to EVAL it until trident fixes this
...
#define PMHS ((MOUSEHOOKSTRUCT *)lParam)
    ...
    if ((nCode == HC_ACTION) && g_pdsvlvp && (PMHS->hwnd != HWNDLISTVIEW) && IsWindowVisible(HWNDLISTVIEW)) {
        ...
        // moving over an area that would hit a listview icon then we need to start hooking mouse events.
        // Install the GetMessage hook so that we can do what we need to do.
        ...
        fStealMouse = (ISMOUSEDOWN(wParam) && ShouldStealMouseClick(&(PMHS->pt), g_pdsvlvp)) || (!GetCapture() && (iHit != -1) && (!hwndHittest || (hwndHittest == HWNDLISTVIEW)));

        if (!g_pdsvlvp->hHookGetMsg) {
            if (hwndHittest && (IsChildOrSelf(hwndParent, hwndHittest) == S_OK)) {
                if (fStealMouse) {
                    // Note:  We have to steal the mouse at this point and use the GetMessage hook to redirect the mouse messages to our listview window.
                    // If we do something different like swallow the message here and then PostMessage faked up events to the listview then all
                    // of the hover select functionality will break because the system won't detect the mouse as being over the listview.
                    StealMouse(g_pdsvlvp, TRUE, (UINT) wParam);
                    g_pdsvlvp->hHookGetMsg = SetWindowsHookEx(WH_GETMESSAGE, CombView_GetMsgHook, NULL, GetCurrentThreadId());
                }
            }
        }
        else {
 ...
 * This is the main entry point where a defview can be turned into a combined view.
 * The effect of a combined view is to layer an extended view under the listview icons (via a regional listview) of a normal defview.

 * Warnings:
 * 1) This is currently only used by the "Active Desktop", it is optimized to only support one instance.  Multiple combined views are not currently supported.
 ...
 * This function is used to optimize the combined view ("Active Desktop") by turning off any animated html elements or embeddings when it is completely obscured.

 * Note that we always honor enabling animations if they aren't already enabled.
 * To make the client code easier though we only disable animations if we know the desktop is obscured.
...
BOOL CombView_EnableAnimations(BOOL fEnable)
{
    ...
    if ((fEnable != fEnabled) && g_pdsvlvp)
    {
        ...
        if (fChangeAnimationState && SUCCEEDED(g_pdsvlvp->pdsv->_psb->QueryInterface(IID_IOleCommandTarget, (LPVOID*) &pct)))
        {
            VARIANT var = { 0 };
 ...
 * This is implemented by the combined view so that we can support various Win95 desktop functionality in a compatible way in the extended view.
 * Some examples include picking off context menu invocations, configuring the host to display the way we want it too, and modifying drag/drop behavior.
*/
HRESULT CSFVSite::ShowContextMenu(DWORD dwID, POINT *ppt, IUnknown *pcmdtReserved, IDispatch *pdispReserved)
{
    ...
    if ((dwID == CONTEXT_MENU_DEFAULT || dwID == CONTEXT_MENU_IMAGE) && pView->_hwndListview) {
        // we used to unselect everything first, but that's bogus because it breaks the app key trying to get a context menu on the currently selected items

        // BOGUS - Trident blows up if we send the message here and the user turns off webview.  Post it for now.
        PostMessage(pView->_hwndListview, WM_CONTEXTMENU, (WPARAM) pView->_hwndListview, MAKELPARAM((short) LOWORD(ppt->x), (short) LOWORD(ppt->y)));
        return S_OK;
    }
    else {
        return S_FALSE;
...
HRESULT CSFVSite::GetHostInfo(DOCHOSTUIINFO *pInfo)
{
    ...
    {
        pInfo->dwFlags = DOCHOSTUIFLAG_DISABLE_HELP_MENU | DOCHOSTUIFLAG_DIALOG | DOCHOSTUIFLAG_DISABLE_SCRIPT_INACTIVE;
    }
...
HRESULT CSFVSite::ShowUI(DWORD dwID, IOleInPlaceActiveObject *pActiveObject, IOleCommandTarget *pCommandTarget, IOleInPlaceFrame *pFrame, IOleInPlaceUIWindow *pDoc)
{
...
HRESULT CSFVSite::EnableModeless(BOOL fEnable)
{
    ...
    // IOleInPlaceActiveObject is called by the frame. We don't care those cases.
    return S_OK;
}
...
{
    // Called from the Trident when the equivalent member of its IOleInPlaceActiveObject is called by the frame.

    // Trap F5 alone and handle the refresh ourselves!
    // Note:This code-path will be hit for desktop only if the active desktop is turned on.

    // And probably if focus is on Trident. It's probably good to pick this off for Web View too.
