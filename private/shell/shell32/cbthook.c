...
BOOL WINAPI RegisterShellHook(HWND hwnd, BOOL fInstall)
{
    ...
    case 4:
        ...
        // it returned NULL
        {
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Windows"), 0, KEY_READ, &hk) != ERROR_SUCCESS)
            ...
            if (RegCreateKeyEx(HKEY_CURRENT_USER, s_szHKEYExplorerVolatile, 0, TEXT(""), REG_OPTION_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hk, &dw) == ERROR_SUCCESS)
            {
                // Do we need to set a value here?
                if (dw != REG_CREATED_NEW_KEY)
                {
                    cbData = sizeof(ft);
                    if ((SHQueryValueEx(hk, TEXT("ShutDownTime"), NULL, &dwType, (LPBYTE)&ft, &cbData) == ERROR_SUCCESS) && (memcmp(&ftLast, &ft, sizeof(FILETIME)) == 0))
                    {
                        RegCloseKey(hk);
