...
DELAY_LOAD_HRESULT(g_hmodOLE32, OLE32, CoMarshalInterface, (LPSTREAM pStm, REFIID riid, LPUNKNOWN pUnk, DWORD dwDestContext, void *pvDestContext, DWORD mshlflags),
...
DELAY_LOAD_HRESULT(g_hmodUrlmon, URLMON, CoInternetCreateSecurityManager, (IServiceProvider *pSP, IInternetSecurityManager **ppSM, DWORD dwReserved), (pSP, ppSM, dwReserved));
DELAY_LOAD_HRESULT(g_hmodUrlmon, URLMON, CreateURLMoniker, (IMoniker* pMkCtx, LPCWSTR pwsURL, IMoniker ** ppimk), (pMkCtx, pwsURL, ppimk));
DELAY_LOAD_HRESULT(g_hmodUrlmon, URLMON, RegisterBindStatusCallback, (LPBC pBC, IBindStatusCallback *pBSCb, IBindStatusCallback**  ppBSCBPrev, DWORD dwReserved), (pBC, pBSCb, ppBSCBPrev, dwReserved));
DELAY_LOAD_HRESULT(g_hmodUrlmon, URLMON, RevokeBindStatusCallback, (LPBC pBC, IBindStatusCallback *pBSCb), (pBC, pBSCb));
DELAY_LOAD_HRESULT(g_hmodUrlmon, URLMON, HlinkNavigateString, (IUnknown *pUnk, LPCWSTR szTarget), (pUnk, szTarget));
...
DELAY_LOAD(g_hmodSECUR32, secur32, BOOLEAN, GetUserNameExW, (EXTENDED_NAME_FORMAT NameFormat, LPWSTR lpNameBuffer, PULONG nSize), (NameFormat, lpNameBuffer, nSize));
...
#ifdef UNICODE
DELAY_LOAD_EXT(g_hmodHHCtrl, hhctrl, OCX, HWND, HtmlHelpW, (HWND hwndCaller, LPCWSTR pwszFile, UINT uCommand, DWORD dwData), (hwndCaller, pwszFile, uCommand, dwData))

#else
DELAY_LOAD_EXT(g_hmodHHCtrl, hhctrl, OCX, HWND, HtmlHelpA, (HWND hwndCaller, LPCSTR pszFile, UINT uCommand, DWORD dwData), (hwndCaller, pszFile, uCommand, dwData))
...
DELAY_LOAD_WNET(g_hmodMPR, MPR, WNetDisconnectDialog, (HWND hwnd, DWORD dwType), (hwnd, dwType));
