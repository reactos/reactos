...
CFileStorage::CFileStorage(LPCTSTR pPath) : _cRef(1)
{
...
// Call the shell file operation code to delete recursively the given directory, don't show any UI.
...
STDMETHODIMP CFileStorage::OpenStream(const OLECHAR* pwcsName, VOID *reserved1, DWORD grfMode, DWORD reserved2, IStream **ppstm)
{
    ...
    *ppstm = NULL;
    ...
    // combine the path strings (our location and the stream, and if they don't specify STGM_CREATE we will fail the call.
...
HRESULT CFileStorage::_OpenCreateStorage(const OLECHAR *pwcsName, DWORD grfMode, IStorage **ppstg, BOOL fCreate)
{
    ...
    *ppstg = NULL;
    ...
    // if the storage doesn't exist then lets create it, then drop into the open storage to do the right thing.
    ...
    {
        // an object exists, we must fail if STGM_FAILIFTHERE is set, or the object that exists is not a directory.

        // if the STGM_CREATE flag is set and the object exists we will delete the existing storage.
    ...
    {
        // the object doesn't exist, and they have not set the STGM_CREATE, nor is this a ::CreateStorage call.
