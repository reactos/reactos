...
// We wait till there arent any more people deleteing before we go try to compact the info file or purge entries and make the bitbucket respect its cbMaxSize.
...
void UndoBBFileDelete(LPCTSTR lpszOriginal, LPCTSTR lpszDelFile)
{
    ...
    // BUGBUG: we need to find out specific information about which file ops were aborted
