...
HRESULT CFTEnumAssocInfo::Init(ASENUM asenumFlags, LPTSTR pszStr, AIINIT aiinitFlags)
{
    HRESULT hres = E_INVALIDARG;

    if (((ASENUM_PROGID & asenumFlags) && !(ASENUM_EXT & asenumFlags)) || (!(ASENUM_PROGID & asenumFlags) && (ASENUM_EXT & asenumFlags)) || (ASENUM_ACTION & asenumFlags) )
    {
        hres = S_OK;
...
HRESULT CFTEnumAssocInfo::_EnumHKCR(ASENUM asenumFlags, LPTSTR pszStr, DWORD* pcchStr)
{
    HRESULT hres = E_FAIL;
    BOOL fNext = TRUE;
    ...
    while (fNext)
    {
        ...
        LONG lRes = RegEnumKeyEx(HKEY_CLASSES_ROOT, _dwIndex, pszStr, &cchStr, NULL, NULL, NULL, NULL);
...
HRESULT CFTEnumAssocInfo::_EnumProgIDActions(LPTSTR pszStr, DWORD* pcchStr)
{
    ...
    {
        LONG lRes = RegEnumKeyEx(hKey, _dwIndex, pszStr, pcchStr, NULL, NULL, NULL, NULL);
        ...
        else
        {
            ...
            if ((ERROR_SUCCESS == SHRegQueryValue(hKey, pszStr, szNiceText, &lNiceText)) && (lNiceText > SIZEOF(TCHAR)))
            {
...
BOOL CFTEnumAssocInfo::_EnumKCRSkip(DWORD asenumFlags, LPTSTR pszExt)
{
    ...
    if (AIINIT_NONE == _aiinitFlags)
    {
        ...
        if ((ASENUM_NOEXPLORERSHELLACTION & asenumFlags) && !fRet)
        {
            ...
            HRESULT hres = AssocCreate(CLSID_QueryAssociations, IID_IQueryAssociations, (LPVOID*)&pQA);
            if (SUCCEEDED(hres))
            {
                ...
                if (SUCCEEDED(hres))
                {
                    ...
                    hres = pQA->GetString(ASSOCF_VERIFY, ASSOCSTR_EXECUTABLE, NULL, szwExec, &cchExec);
        ...
        if ((ASENUM_ASSOC_YES & asenumFlags) && (ASENUM_ASSOC_ALL != (ASENUM_ASSOC_ALL & asenumFlags)) && !fRet)
        {
            IAssocInfo* pAI = NULL;
            HRESULT hres = E_FAIL;
        ...
        if ((ASENUM_ASSOC_NO & asenumFlags) && (ASENUM_ASSOC_ALL != (ASENUM_ASSOC_ALL & asenumFlags)) && !fRet)
        {
            IAssocInfo* pAI = NULL;
            HRESULT hres = E_FAIL;
    ...
    else
    {
        if (AIINIT_PROGID == _aiinitFlags)
        {
            ...
            if (ASENUM_EXT & asenumFlags)
            {
                ...
                LONG lRes = SHGetValue(HKEY_CLASSES_ROOT, pszExt, NULL, &dwType, szProgID, &cbProgID);
                if (ERROR_SUCCESS == lRes)
                {
