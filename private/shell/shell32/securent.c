        ...
        if (pUserSid)
        {
            ...
            if (lStatus == ERROR_SUCCESS)
            {
                ...
                if (lStatus == ERROR_SUCCESS)
                {
                    ...
                    lStatus = SHQueryValueEx(hkeyUser, TEXT("ProfileName"), NULL, &dwType, (LPBYTE) szProfilePath, &dwSize);
                    ...
                    else
                    {
                        ...
                        lStatus = SHQueryValueEx(hkeyUser, TEXT("ProfileImagePath"), NULL, &dwType, (LPBYTE) szProfilePath, &dwSize);
                        if (lStatus == ERROR_SUCCESS && (dwType == REG_SZ || dwType == REG_EXPAND_SZ))
                        {
...
__inline BOOL IsCurrentUserShellSID(PSHELL_USER_SID psusID)
{
    ...
    if ((psusID->dwUserGroupID == 0) && (psusID->dwUserID == 0) && memcmp(&psusID->sidAuthority, &sidNULL, sizeof(SID_IDENTIFIER_AUTHORITY)) == 0)
    {
        return TRUE;
    }
...
// OUT: psd - A pointer to a uninitialized SECURITY_DESCRIPTOR struct to be inited and filled in in by this function
...
#else
    // On Win95 everyone has the same privilage.
    // Someday this might change if we want to special case the supervisor account.
    return FALSE;
#endif
}
