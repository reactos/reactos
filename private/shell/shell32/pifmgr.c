...
*/
{
    ...
    for (pid=aINIData; pid-aINIData < ARRAYSIZE(aINIData); pid++) {
        ...
        if (pid->iFlags & (INIDATA_DECINT | INIDATA_BOOLEAN)) {
            ...
            t = GetPrivateProfileInt(pid->pszSection, pid->pszKey, t, szSystemINI);
            if (pid->iFlags & INIDATA_BOOLEAN) {
                if (t)
                    t = pid->iMask;
                if (pid->iFlags & INIDATA_INVERT)
        ...
        if (pid->iFlags & INIDATA_FIXEDPOINT) {
            ...
            GetPrivateProfileString(pid->pszSection, pid->pszKey, szTemp, szTemp, ARRAYSIZE(szTemp), szSystemINI);
...
*/
{
    ...
    if (ResizePIFData(ppl, SIZEOF(STDPIF)) != -1) {
        ...
        // look like a backward-compatible PIF file -- not that we use or particularly care about any of it
...
HANDLE OpenPIFFile(LPCTSTR pszFile, LPPIFOFSTRUCT pof)
{
    ...
    // CreateFile does not search the path, so do that first, then give CreateFile a fully qualified file name to open...
    ...
    else
    {
        ...
        else
        {
            ...
            GetFullPathName( pszNewFile, ARRAYSIZE(pof->szPathName), pof->szPathName, &lpDummy );
        }
    }
 ...
 *      It expects the PIF search to pick up C:\DELTA\DELTABAT.PIF from the path, even though the WinExec supplied a full path.
*/
...
#endif
        lstrcpyn( &ppl->ofPIF.szPathName[2], pszName, ARRAYSIZE(ppl->ofPIF.szPathName) - 2);
        ppl->hPIF = OpenPIFFile(ppl->ofPIF.szPathName, &ppl->ofPIF);
    }

    // If we were given a path component, then look in that directory.
    // (The fact that we have a backslash or drive letter will suppress the path search.)
  ...
  IgnorePIF:
    ...
    if (!fError && !fExplicitPIF && (hInf != -1)) {
        if (PifMgr_GetProperties(ppl, MAKELP(0,GROUP_PRG), &prg, SIZEOF(prg), GETPROPS_NONE)) {
            ...
            // reinitialize it, it doesn't call AppWiz to silently reconfigure it, etc).
            if (fSearchInf || (prg.flPrgInit & PRGINIT_AMBIGUOUSPIF)) {
                ...
                if (!GetAppsInfData(ppl, &prg, (HINF)IntToPtr( hInf ), lpszApp, fFixedDisk, flOpt)) {
                    ...
                    if (GetSystemDefaultLangID() == 0x0411) {
                        ...
                        PifMgr_SetProperties(ppl, MAKELP(0,GROUP_ENV), &env, SIZEOF(env), SETPROPS_NONE);
                    }
#endif
                }
#ifdef UNICODE
...
int WINAPI PifMgr_GetProperties(HANDLE hProps, LPCSTR lpszGroup, void *lpProps, int cbProps, UINT flOpt)
{
    ...
    // We should never enter PIFMGR with outstanding locks (we also call here from *inside* PIFMGR, but none of those cases should require a lock either)
    ...
    if (!lpszGroup) {
        ...
        if (LOWORD((DWORD_PTR) lpszGroup) == GROUP_ICON) {
            ...
            if ( pnt40 ) {
                ...
                if (pprg) {
                    if ( PifMgr_GetProperties(ppl, MAKELP(0,GROUP_PRG), pprg, SIZEOF(PROPPRG), GETPROPS_NONE) && PifMgr_GetProperties(ppl, MAKELP(0,GROUP_NT40), pnt40, SIZEOF(PROPNT40), GETPROPS_NONE) ) {
...
// #pragma message("Remove this after M8")
    ...
    // We should never leave PIFMGR with outstanding locks (we also call here from *inside* PIFMGR, but none of those cases should require a lock either)
    ASSERTTRUE(!ppl->cLocks);
    return cb;
}
...
void RenameStartupFiles(LPTSTR pszOld, LPTSTR pszNew, int flRename)
{
    ...
    if (flRename & RENAME_CREATEOLD) {
        ...
        if (i == -1) {
            hFile = CreateFile( pszNew, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_HIDDEN, NULL);
                ...
                if (ppl->lpArgs) {      // any associated args?
                    ...
                    if (j) {
...
#ifdef UNICODE
                        {
                            ...
                            WideCharToMultiByte( CP_ACP, 0, ppl->lpArgs, -1, achArgs, ARRAYSIZE(achArgs), NULL, NULL );
...
PPROPLINK ValidPropHandle(HANDLE hProps)
{
    ...
    if (!hProps || (HANDLE)hProps > g_offHighestPropLink || ((PPROPLINK)hProps)->iSig != PROP_SIG) {
        ASSERTFAIL();
        return NULL;
    }
...
BOOL GetPIFData(PPROPLINK ppl, BOOL fLocked)
{
    ...
    if (ppl->hPIF == INVALID_HANDLE_VALUE && !(ppl->flProp & PROP_SKIPPIF)) {
        ...
        ppl->hPIF = CreateFile( pszOpen, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
    }
    if (ppl->hPIF == INVALID_HANDLE_VALUE) {
    ...
    if (dwOff >= SIZEOF(STDPIF)) {
        ...
        if (ResizePIFData(ppl, dwOff) != -1) {
            ...
            ReadFile( ppl->hPIF, ppl->lpPIFData, ppl->cbPIFData, &ppl->cbPIFData, NULL );
            ...
            if (ppl->flProp & PROP_DEFAULTPIF) {
                ...
                PifMgr_WCtoMBPath( ppl->szPathName, ppl->lpPIFData->stdpifdata.startfile, ARRAYSIZE(ppl->lpPIFData->stdpifdata.startfile));
#else
...
BOOL FlushPIFData(PPROPLINK ppl, BOOL fDiscard)
{
    ...
    if (ppl->hPIF != INVALID_HANDLE_VALUE) {
        ...
        WriteFile( ppl->hPIF, (LPCVOID)ppl->lpPIFData, ppl->cbPIFData, &dwDummy, NULL );
...
BOOL AddGroupData(PPROPLINK ppl, LPCSTR lpszGroup, LPCVOID lpGroup, int cbGroup)
{
    ...
    if ((cbOld = ResizePIFData(ppl, cbGroup+SIZEOF(PIFEXTHDR))) != -1) {
        ...
        while ((DWORD_PTR)lpph <= (DWORD_PTR)LPPIF_OFF(cbOld - SIZEOF(PIFEXTHDR)) && (DWORD_PTR)lpph >= (DWORD_PTR)LPPIF_FIELDOFF(stdpifext)) {
...
*/
void *GetGroupData(PPROPLINK ppl, LPCSTR lpszGroup, LPINT lpcbGroup, LPPIFEXTHDR *lplpph)
{
    BOOL fFixMe;
    LPPIFEXTHDR lpph;
