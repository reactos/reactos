...
STDAPI_(BOOL) CommonRestart(DWORD dwExitWinCode)
{
    ...
    Status = SetPrivilegeAttribute(SE_SHUTDOWN_NAME, SE_PRIVILEGE_ENABLED, &OldState);
...
DWORD GetShutdownOptions()
{
    ...
    if (IsSuspendAllowed())
    {
        ...
        if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Power"), 0, KEY_READ, &hKey) == ERROR_SUCCESS)
        {
            dwSize = sizeof(dwAdvSuspend);
            SHQueryValueEx (hKey, TEXT("Shutdown"), NULL, &dwType, (LPBYTE) &dwAdvSuspend, &dwSize);
            RegCloseKey (hKey);
        }
...
//    AllowSetForegroundWindow(ASFW_ANY);
    ...
    case SHTDN_HIBERNATE:
        SuspendComputer ((psdtp->nCmd == SHTDN_HIBERNATE) ? TRUE : FALSE, (GetKeyState(VK_CONTROL) < 0) ? TRUE : FALSE, (psdtp->nCmd == SHTDN_SLEEP2) ? TRUE : FALSE);
        break;
    }
