...
// this is set if pdtnChild has not been traversed (as opposed to NULL which means there are no children
#define DTN_DELAYED ((PDIRTREENODE)-1)


// DIRTREENODE is a node in a linked list/tree cache of the directory structure.
// except for the top level (which is specified by the caller of the api), the order are all files first, then all directories.
...
UINT DTBuild(COPY_STATE* pcs)
{
    ...
    while (!FOQueryAbort(pcs) && *pdth->pFrom)
    {
        ...
        iError = DTPathToDTNode(pdth, pcs, szPath, fRecurse,
            ((PathIsWild(pdth->pFrom) && (pcs->lpfo->fFlags & FOF_FILESONLY)) ? DTF_FILES_ONLY : DTF_FILES_AND_FOLDERS), ppdtn,&fd, NULL, ppdtnConnectedItems, FALSE, 0);
...
UINT DTValidatePathNames(PDIRTREEHEADER pdth, UINT operation, COPY_STATE * pcs)
{
    if (pcs->lpfo->wFunc != FO_DELETE)
    {
        ...
        if (operation == OPER_ENTERDIR)
        {
            ...
            if (!(pcs->fFlags & FOF_RENAMEONCOLLISION) && !StrCmpNI(pdth->szSrcPath, pdth->szDestPath, cchFrom))
            {
                ...
                else if (chNext == TEXT('\\'))
                {
                    ...
                    // same file/folder
                    if ((PathFindFileName(pdth->szDestPath) - pdth->szDestPath - 1) == lstrlen(pdth->szSrcPath))
                    {
                        return OPER_ERROR | DE_DESTSAMETREE;
                    }
                    else
...
UINT DTGoToNextNode(PDIRTREEHEADER pdth, COPY_STATE *pcs)
{
    ...
    else
    {
        ...
        else if (pdth->pdtnCurrent->pdtnNext)
        {
            ...
            else
            {
                ...
                if (!PathAppend(pdth->szSrcPath, pdth->pdtnCurrent->szName) || !PathAppend(pdth->szDestPath, pdth->pdtnCurrent->szName))
                    return OPER_ERROR | DE_INVALIDFILES;
            }
        }
        else
    ...
    if (DTNIsRootNode(pdth->pdtnCurrent))
    {
        ...
        if (!DTDiskCheck(pdth, pcs, pdth->szSrcPath) || !DTDiskCheck(pdth, pcs, pdth->szDestPath))
        {
            return 0;
        }
    }
...
void HideConfirmButtons(HWND hdlg, int idHide)
{
    ...
    if (hwndCancel) {
        ...
        // the windows, because keyboard accelerators for hidden windows are still active!
        if (hwndYesToAll)
...
UINT FileCopy(COPY_STATE *pcs, LPCTSTR pszSource, LPCTSTR pszDest, const WIN32_FIND_DATA *pfd, BOOL fCreateAlways)
{
    ...
    if ( g_bRunOnNT5 && FO_MOVE == pcs->lpfo->wFunc )
    {
        ...
        else
        {
            ...
            {
                pfnMoveFileWithProgress = (PFNMOVEFILEWITHPROGRESS)GetProcAddress( hinstKernel32, "MoveFileWithProgressW" );
                if ( NULL == pfnMoveFileWithProgress )
    ...
    if (pcs)
    {
        ...
        case WM_COMMAND:
            ...
            case PDM_SHUTDOWN:
                ...
                ShellMessageBox(HINST_THISDLL, hDlg, MAKEINTRESOURCE(IDS_CANTSHUTDOWN), NULL, MB_OK | MB_ICONEXCLAMATION | MB_SETFOREGROUND);
                break;
...
int WINAPI SHFileOperation(LPSHFILEOPSTRUCT lpfo)
{
    ...
    hNoDiskWarning = CreateEvent(CreateAllAccessSecurityAttributes(NULL, NULL, NULL), TRUE, FALSE, TEXT("DisableLowDiskWarning"));
...
int WINAPI SHFileOperationA(LPSHFILEOPSTRUCTA lpfo)
{
    ...
    if ((lpfo->fFlags & FOF_SIMPLEPROGRESS) && lpfo->lpszProgressTitle != NULL)
    {
        ...
        MultiByteToWideChar(CP_ACP, 0, lpfo->lpszProgressTitle, uSizeTitle, lpTemp, uSizeTitle);
    }
    else
    {
...
INT_PTR ValidateCreateFileFromClip(HWND hwnd, LPFILEDESCRIPTOR pfdscSrc, TCHAR *szPathDest, PYNLIST pynl)
{
    ...
    // What we do now depends on whether we are processing a directory or a file.
    if (wfdDest.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
    {
    ...
    else
    {
        ...
        // anymore...
        if ((result == IDYES) && (wfdDest.dwFileAttributes & (FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_READONLY)))
        {
