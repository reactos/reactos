...
//      The first is that in 32 bit land the tables are maintined in our shared heap, which makes it shared between all of our apps.
//      The second is that we can assocate a long pointer with each of the items, which in many cases allows us to keep from having to do a secondary lookup from a different table
...
// DESCRIPTION
//  it turns out we have long runs of duplicate lookups (e.g. "Directory" and ".lnk").  a 1-element MRU is a v. cheap way of speeding things up.

// rather than check for the (rare) special case of NULL each time we check our cache, we pt at at this guy.
// then iff we think it's a cache hit, we make sure it's not pting at this special guy.
...
// This function looks up the name in the hash table and optionally does things like add it, or delete it.
LPCTSTR LookupItemInHashTable(PHASHTABLE pht, LPCTSTR pszName, int iOp)
{
    ...
    if (!pht->fCaseSensitive)
    {
        ...
        //  this should be cheaper than upper because in most places in the shell we prettify the names to be mostly lower case.
    ...
    while (*pch)
    {
        ...
        cchName++;//  keep track of the length
    ...
    // note that there's a semi-race on pht->pszHTCache ops, viz. that we LEAVECRITICAL but then return a ptr into our table.
    // however it's 'no worse' than the existing races.
    // so i guess the caller is supposed to avoid a concurrent lookup/delete.
...
{
    ASSERT(FindHashItem(pht, sz) == sz);// string must be from the hash table
...
{
    ASSERT(FindHashItem(pht, sz) == sz);// string must be from the hash table
    return (pht? HIDATAPTR(pht, sz) : NULL);// the default hash table does not have extra data!
}
