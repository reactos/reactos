...
--*/
{
    ...
    if (++g_cRefWx86 == 1)
    {
        ...
        if (g_hmodWx86)
        {
            ...
            if (!g_pfnWx86LoadX86Dll || !g_pfnWx86FreeX86Dll || !g_pfnWx86ThunkProc || !g_pfnWx86Compact)
            {
...
void ConvertCplInfo(LPVOID lpv)
{
    ...
    MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, CplInfoA.szName, ARRAYSIZE(CplInfoA.szName), lpCplInfoW->szName, ARRAYSIZE(lpCplInfoW->szName));
    MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, CplInfoA.szInfo, ARRAYSIZE(CplInfoA.szInfo), lpCplInfoW->szInfo, ARRAYSIZE(lpCplInfoW->szInfo));
    MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, CplInfoA.szHelpFile, ARRAYSIZE(CplInfoA.szHelpFile), lpCplInfoW->szHelpFile, ARRAYSIZE(lpCplInfoW->szHelpFile));
    ...
    WideCharToMultiByte(CP_ACP, 0, CplInfoW.szHelpFile, ARRAYSIZE(CplInfoW.szHelpFile), lpCplInfoA->szHelpFile, ARRAYSIZE(lpCplInfoA->szHelpFile), NULL, NULL);
...
LPTSTR _RestoreTruncatedCplString(HINSTANCE hinst, int id, LPTSTR pszShort, LPTSTR pszBuf, int cchBufMax)
{
...
BOOL _InitializeControl(LPCPLMODULE pcplm, LPCPLITEM pcpli)
{
    ...
    {
        hIconTemp = LoadImage(pcplm->minst.hinst, MAKEINTRESOURCE(cpl.idIcon), IMAGE_ICON, 0, 0, LR_DEFAULTSIZE);
        if (!hIconTemp)
        {
...
BOOL _InitializeCPLModule(LPCPLMODULE pcplm)
{
    ...
    // Initialize the CPL
    if (pcplm->lpfnCPL && CPL_CallEntry(pcplm, NULL, CPL_INIT, 0L, 0L))
    {
        int cControls = (int)CPL_CallEntry(pcplm, NULL, CPL_GETCOUNT, 0L, 0L);
        if (cControls > 0)
        {
            // By passing in the number of applets, we should speed up allocation of this array.
...
LRESULT CPL_CallEntry(LPCPLMODULE pcplm, HWND hwnd, UINT msg, LPARAM lParam1, LPARAM lParam2)
{
        ...
        _except(SetErrorMode(SEM_NOGPFAULTERRORBOX), UnhandledExceptionFilter(GetExceptionInformation()))
    {
        ...
        ShellMessageBox(HINST_THISDLL,
                        NULL,
                        MAKEINTRESOURCE(IDS_CPL_EXCEPTION),
...
int _LoadCPLModule(LPCTSTR pszModule)
{
    ...
    if (!ISVALIDHINST16(minst.hinst)) {
        ...
        if ((UINT_PTR)minst.hinst == 21) {   // Win32 DLL?
            ...
            if ((minst.hinst == NULL) && (GetLastError() == ERROR_BAD_EXE_FORMAT) && (Wx86Enable()))
            {
...
void CPL_StripAmpersand(LPTSTR szBuffer)
{
    ...
    do
    {
        ...
        if (*pIn == TEXT('(') && *(pIn + 1) == TEXT('&') && *(pIn + 2) && *(pIn + 3) == TEXT(')')) {
            pIn += 4;
        }
        ...
        if (*pIn == 0x1e && *++pIn) {
            ...
            pIn = CharNext(pIn);// Skip Alphabet accelerator.

            if (*pIn) {
                if (*pIn == 0x1f && *++pIn) {
                    pIn = CharNext(pIn);// Skip FE accelelator
                }
...
void _InsertModuleName(ControlData *lpData, LPCTSTR szPath, PMODULEINFO pmi)
{
    ...
    if (pmi->pszModule)
    {
        ...
        GetPrivateProfileString(TEXT("don't load"), pmi->pszModuleName, TEXT(""), szTemp, ARRAYSIZE(szTemp), TEXT("control.ini"));
...
//  If this info is up-to-date with current modules (from CPLD_GetModules), then we can enumerate these without loading the CPLs.
...
#endif
        if (ERROR_SUCCESS == SHQueryValueEx(hkey, c_szCPLData, NULL, NULL, NULL, &cbSize))
        {
            lpData->pRegCPLBuffer = LocalAlloc(LPTR, cbSize);
            if (lpData->pRegCPLBuffer)
            {
                if (ERROR_SUCCESS == SHQueryValueEx(hkey, c_szCPLData, NULL, NULL, lpData->pRegCPLBuffer, &cbSize))
                {
                    ...
                    if (lpData->hRegCPLs)
                    {
                        ...
                        for (cbOffset = 0; cbOffset < cbSize; cbOffset += p->cbSize)
                        {
...
// On a typical system, we will successfully cache all the CPLs.  So this function will write out the data only once.
...
void CPLD_AddControlToReg(ControlData *lpData, const MINST * pminst, int nControl)
{
    ...
    // of the file we used to load this module?  (We have the name both in the calling function and in lpData.)
    ...
    if (hFindFile != INVALID_HANDLE_VALUE)
    {
        ...
        if (prcpli)
        {
            ...
            prcpli->cbSize = FIELD_OFFSET(RegCPLInfo, buf) + (prcpli->oInfo + lstrlen(REGCPL_CPLINFO(prcpli)) + 1) * SIZEOF(TCHAR);

            // Force struct size to be DWORD aligned since these are packed
            // together in registry, then read and accessed after reading cache from registry.
            if (prcpli->cbSize & 3)
