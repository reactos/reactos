...
inline CDomainRelativeObjId::operator == (const CDomainRelativeObjId &Other) const
{
...
inline CVolumeId:: operator == (const CVolumeId & Other) const
{
                     ...
                     BOOL *CheckForDfs )
{
    ...
    // If it's not a UNC path, then is it a drive-letter path?
    else if( RtlPrefixString( (PSTRING)&NtDrivePathNamePrefix, (PSTRING)&ServerFileName, TRUE ) && ServerFileName.Buffer[ cchNtDrivePathNamePrefix + 1 ] == L':' ) {
            ...
            ))
        {
            ...
            *CheckForDfs = FALSE;// We know now that it's not a DFS path
        }
        ...
        else if( 0 == wcsncmp( DosDeviceMapping, DfsMappingPrefix, lstrlenW(DfsMappingPrefix) ))
        {
            ...
            NtStatus = NtQueryInformationFile(hFile, &IoStatusBlock, FileNameInfo, cbFileNameInfo, FileNameInformation);
    ...
    WCHAR *pwszComputer )
{
    ...
    if( 0 == GetFullPathName( pwszFile, ARRAYSIZE(wszAbsoluteName), wszAbsoluteName, &pwszFilePart ))
    {
        hr = HRESULT_FROM_WIN32( GetLastError() );
        goto Exit;
    }
    ...
    *pwszFilePart = L'\0';
    ...
    else if( DRIVE_UNKNOWN == DriveType || DRIVE_NO_ROOT_DIR == DriveType )
    {
