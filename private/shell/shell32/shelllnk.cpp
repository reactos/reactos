...
HRESULT GetLogo3SoftwareUpdateInfo( LPCTSTR pszLogo3ID, LPSOFTDISTINFO psdi )
{
    ...
    wsprintf(szBuffer, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\%s"), pszLogo3ID);
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szBuffer, 0, KEY_READ, &hkeyDistInfo) != ERROR_SUCCESS) {
        hr = E_FAIL;
        goto Exit;
    }

    if (RegOpenKeyEx(hkeyDistInfo, TEXT("AvailableVersion"), 0, KEY_READ, &hkeyAvail) != ERROR_SUCCESS) {
        hr = E_FAIL;
        goto Exit;
    }

    dwSize = sizeof(DWORD);
    if (SHQueryValueEx(hkeyAvail, TEXT("Precache"), 0, &dwType, (unsigned char *)&lResult, &dwSize) == ERROR_SUCCESS) {
    ...
    dwSize = MAX_PATH;
    if (SHQueryValueEx(hkeyAvail, TEXT("AdvertisedVersion"), NULL, &dwType, (unsigned char *)szVersionBuf, &dwSize) == ERROR_SUCCESS)
    {
    ...
    dwSize = MAX_PATH;
    if (SHQueryValueEx(hkeyAvail, NULL, NULL, &dwType, (unsigned char *)szVersionBuf, &dwSize) != ERROR_SUCCESS) {
        hr = S_FALSE;
        goto Exit;
    }
