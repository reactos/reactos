...
{
    // This will cause the next call GetIconOverlayManager() call in each process to load any non-loaded icon overlay identifiers.
...
STDMETHODIMP CFSFolder_IconOverlay_GetOverlayIndex(IShellIconOverlay *psio, LPCITEMIDLIST pidl, int * pIndex)
{
    ...
    ASSERT(pIndex);
    if (pIndex)
        hres = (*pIndex == OI_ASYNC) ? E_PENDING : CFSFolder_IconOverlay_GetOverlayInfo(psio, pidl, pIndex, SIOM_OVERLAYINDEX);
...
// global hook in the SHChangeNotify() dispatcher. note we get all change notifies here so be careful!
