...
HRESULT CNetFldr_CreateInstance(LPCITEMIDLIST pidlAbs, 
LPCITEMIDLIST pidlTarget, UINT uDisplayType, LPCIDNETRESOURCE pidnForProvider, LPCTSTR pszResName, REFIID riid, void **ppvOut)
{
        ...
        if (NULL != pNetF)
        {
            ...
            if ( pNetF->_pidl && (!pidlTarget || (pidlTarget && pNetF->_pidlTarget)) )
            {
                if (uDisplayType == RESOURCEDISPLAYTYPE_SERVER)
                {
                    ...
                    CRegFolder_CreateInstance(&riiComputer, (IUnknown*) (IShellFolder*) pNetF, IID_IUnknown, (void**) &pNetF->_punkInner);
                }
...
// handle parsing a net name into a IDLIST structure that represents that NETRESOURCE up to the root.
// we loop using each NETRESOURCE and calling NetResourceGetParent, we assume that if we receive a ERROR_BAD_NET_NAME we are done.
    ...
    do
    {
        ...
        if ( SUCCEEDED(hres) )
        {
            ...
            else
            {
                ...
                // then there is no point as we are already at the top of the chain, calling for root will just fail (or on Win9x cause us to loop forever, eat all the stack and die!).
                if ( (pnr->dwDisplayType == RESOURCEDISPLAYTYPE_NETWORK) || (pnr->dwDisplayType == RESOURCEDISPLAYTYPE_ROOT) || (WNetGetResourceParent(pnr, pnr, &dwbuf) != WN_SUCCESS) )
                {
                    break;
                }
            }
...
LPTSTR CNetFolder::_GetNameForParsing(LPCWSTR pwszName, LPTSTR pszBuffer, INT cchBuffer, LPTSTR *ppszRegItem)
{
    ...
    *ppszRegItem = NULL;
    ...
    // lets walk the name, look for \:: squence to signify the start of a regitem name, and if the number of slashes is > 2 then we should bail
    LPTSTR pszUNC = pszBuffer+2;
    while ( pszUNC && *pszUNC && (cSlashes < 2) )
    {
        if ( (pszUNC[0] == TEXT('\\')) && (pszUNC[1] == TEXT(':')) && (pszUNC[2] == TEXT(':')) )
        {
            *ppszRegItem = pszUNC;
            break;
        }
