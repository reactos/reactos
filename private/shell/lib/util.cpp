...
// Trident will take URLs that don't indicate their source of origin (about:, javascript:, & vbscript:) and will append
// an URL turd and then the source URL.  The turd will indicate
// where the source URL begins and that source URL is needed when the action needs to be Zone Checked.

// This function will remove that URL turd and everything behind it so the URL is presentable for the user.
