    ...
    )
{
    ...
    case FD_MAPPINGDATA:
        if (!FileData->MappedImage) {
            FileData->MappedImage = ShMapFileIntoMemory (FileData->FileName, &FileData->FileHandle, &FileData->MapHandle);
            if (!FileData->MappedImage) {
                return FALSE;
            }
        }
    ...
    case FD_PREVOSDATA:
        if (!FileData->PrevOsKey) {
            status = RegOpenKey (HKEY_LOCAL_MACHINE, S_KEY_PREVOSVERSION, &FileData->PrevOsKey);
...
BOOL DoesPathExist (IN      PCTSTR Path)
{
    BOOL result = FALSE;
    DWORD errMode;
    ...
    __try {
        ...
        while (dataId) {
            if (dataId == VTID_REQFILE) {
                ...
                #else
                    convStr = HeapAlloc (GetProcessHeap (), 0, dataSize);
                    converted = WideCharToMultiByte (CP_ACP, 0, (PWSTR)Blob, -1, convStr, dataSize, NULL, NULL);
                    if (!converted) {
                        HeapFree (GetProcessHeap (), 0, convStr);
                        result = FALSE;
                        __leave;
