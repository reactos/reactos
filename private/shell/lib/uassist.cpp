...
BOOL UEMIsLoaded()
{
    ...
    fRet = GetModuleHandle(TEXT("ole32.dll")) && GetModuleHandle(TEXT("browseui.dll"));
    if (!fRet)
...
extern "C" HRESULT UEMQueryEvent(const GUID *pguidGrp, int eCmd, WPARAM wParam, LPARAM lParam, LPUEMINFO pui)
{
    HRESULT hr = E_FAIL;
    IUserAssist *pua;
...
extern "C" HRESULT UEMSetEvent(const GUID *pguidGrp, int eCmd, WPARAM wParam, LPARAM lParam, LPUEMINFO pui)
{
    HRESULT hr = E_FAIL;
    IUserAssist *pua;
