...
)
{
    ...
    for (pwc = pwsz; (*pwc) && !((*pwc == WC_UNDERSCORE) && IS_DIGITW(pwc[1])); pwc++)
    {
        // do nothing;
    }
...
int GetPointString(LPTSTR buf, HDC hDC, int height)
{
    int pts;
    ...
    if (g_bIsSimplifiedChineseUI)
    {
        ...
        for (ctr = 0; ctr < NUM_ZIHAO; ctr++)
        {
            if ((pts == stZihao[ctr].size) && (abs(ptsfr - stZihao[ctr].sizeFr) <= 3))
            {
                IsZihao = TRUE;
...
BOOL LookUpFontSubs(LPTSTR lpSubFontName, LPTSTR lpRealFontName)
{
    ...
    dwSize = MAX_PATH;
    while (RegEnumValue(hKey, dwIndex, szValueName, &dwValueSize, NULL, &dwType, (LPBYTE)szValueData, &dwSize) == ERROR_SUCCESS)
    {
...
BOOL ThunkChooseFontA2W(PFONTINFO pFI)
{
    BOOL bRet;
...
BOOL ThunkChooseFontW2A(PFONTINFO pFI)
{
    BOOL bRet;
    LPCHOOSEFONTA pCFA = pFI->pCFA;
    LPCHOOSEFONTW pCFW = pFI->pCF;
...
VOID ThunkEnumLogFontExDvA2W(LPENUMLOGFONTEXDVA lpELFDVA, LPENUMLOGFONTEXDVW lpELFDVW)
{
    UINT i;

    ThunkLogFontA2W(&lpELFDVA->elfEnumLogfontEx.elfLogFont, &lpELFDVW->elfEnumLogfontEx.elfLogFont);
...
VOID ThunkEnumLogFontExDvW2A(LPENUMLOGFONTEXDVW lpELFDVW, LPENUMLOGFONTEXDVA lpELFDVA)
{
    BOOL fDefCharUsed;
    UINT i;
...
VOID ThunkLogFontA2W(LPLOGFONTA lpLFA, LPLOGFONTW lpLFW)
{
