...
MustBeDir:
    ...
    if (!bNewStyle)
    {
        if ((bWowApp) && (*(lpstr - 1) == CHAR_DOT) && (*CharNext(lpstr - 2) == CHAR_DOT))
        {
...
VOID AppendExt(LPTSTR lpszPath, LPCTSTR lpExtension, BOOL bWildcard)
{
    WORD wOffset;
    SHORT i;
    ...
    if (lpExtension && *lpExtension)
    {
        ...
        *(lpszPath + wOffset++) = CHAR_DOT;
        ...
        PathRemoveBlanks(szExt);//  Remove leading / trailing blanks in the extension.
        lstrcpy(lpszPath + wOffset, szExt);//  Add the rest.
    }
}
...
{
    return ( DBL_BSLASH(lpszPath) || ((lpszPath[1] == CHAR_COLON) && DBL_BSLASH(lpszPath + 2)) );
}
...
BOOL IsDirectory(LPTSTR pszPath)
{
    ...
    PathRemoveBslash(pszPath);//  Clean up for GetFileAttributes.

    dwAttributes = GetFileAttributes(pszPath);
    return ( (dwAttributes != (DWORD)(-1)) && (dwAttributes & FILE_ATTRIBUTE_DIRECTORY) );
}
...
int WriteProtectedDirCheck(LPTSTR lpszFile)
{
    SHORT nFileOffset;
...
{
    if (!CDLoadString( g_hinst, iszOverwriteQuestion, szCaption, WARNINGMSGLENGTH - 1 ))
    {
        return (FALSE);
    }
    ...
    return (MessageBox( hDlg, szWarning, szCaption, MB_YESNO | MB_DEFBUTTON2 | MB_ICONEXCLAMATION ) == IDYES);
}
...
//  NOTE: For non-Unicode strings only.
BOOL IsBackSlash(LPSTR lpStart, LPSTR lpChar)
{
