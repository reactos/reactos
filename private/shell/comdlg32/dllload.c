...
DELAY_LOAD_WNET(g_hmodMPR, MPR.DLL, WNetEnumResourceW, (HANDLE  hEnum, LPDWORD lpcCount, LPVOID  lpBuffer, LPDWORD lpBufferSize), (hEnum, lpcCount, lpBuffer, lpBufferSize));

DELAY_LOAD_WNET(g_hmodMPR, MPR.DLL, WNetOpenEnumW,
       (DWORD dwScope, DWORD dwType, DWORD dwUsage, LPNETRESOURCEW lpNetResource, LPHANDLE lphEnum), (dwScope, dwType, dwUsage, lpNetResource, lphEnum));
...
DELAY_LOAD_WNET(g_hmodMPR, MPR.DLL, WNetRestoreConnectionW, (HWND hwnd, LPCWSTR psz), (hwnd, psz));
...
DELAY_LOAD_WNET(g_hmodMPR, MPR.DLL, WNetRestoreConnectionA, (HWND hwnd, LPCSTR psz), (hwnd, psz));
#endif
...
DELAY_LOAD_HRESULT(g_hmodOLE32, OLE32.DLL, CoCreateInstance, (REFCLSID rclsid, LPUNKNOWN pUnkOuter, DWORD dwClsContext, REFIID riid, void ** ppv), (rclsid, pUnkOuter, dwClsContext, riid, ppv));
...
DELAY_LOAD_HRESULT(g_hmodOLE32, OLE32.DLL, GetClassFile, (LPCOLESTR szFilename, CLSID FAR* pclsid), (szFilename, pclsid));
DELAY_LOAD_HRESULT(g_hmodOLE32, OLE32.DLL, CreateBindCtx, (DWORD reserved, LPBC FAR* ppbc), (reserved, ppbc));
...
DELAY_LOAD(g_hmodIMM32, IMM32.DLL, LONG, ImmGetCompositionString, (HIMC hIMC, DWORD dwIndex, LPVOID lpBuf, DWORD dwBufLen), (hIMC, dwIndex, lpBuf, dwBufLen))
