    ...
    PPRINTINFOEX pPI)
{
    ...
    {
        g_hHook = SetWindowsHookEx( WH_MSGFILTER, Print_MessageHookProc, 0, GetCurrentThreadId() );
        if (g_hHook)
        {
            bHooked = TRUE;
        }
...
HRESULT Print_ICoCreateInstance(REFCLSID rclsid, REFIID riid, LPCITEMIDLIST pidl, LPVOID *ppv)
{
    ...
    LPARAM lParam)
{
    ...
    if (pidlRoot)
    {
        ...
        uRegister = SHChangeNotifyRegister(
                        ...
                        SHCNE_ATTRIBUTES | SHCNE_UPDATEITEM | SHCNE_CREATE | SHCNE_DELETE | SHCNE_RENAMEITEM,
                        CDM_PRINTNOTIFY,
                        1,
                        &fsne );
    }
...
HRESULT CPrintBrowser::CreatePrintShellView()
{
    ...
    hResult = Print_ICoCreateInstance( CLSID_CPrinters, IID_IShellFolder, pidlRoot, (LPVOID *)&psfRoot );
    if (FAILED(hResult))
    {
        pPI->dwExtendedError = CDERR_INITIALIZATION;
        return (hResult);
...
UINT CPrintBrowser::GetViewMode()
{
    ...
    if (RegOpenKeyEx( HKEY_CURRENT_USER, c_szSettings, 0L, KEY_READ, &hKey ) == ERROR_SUCCESS)
    {
...
VOID CPrintBrowser::SetViewMode()
{
    ...
    {
        //  Open the Printers\Settings registry key and save the information to the ViewMode value entry.
        if (RegOpenKeyEx( HKEY_CURRENT_USER, c_szSettings, 0L, KEY_READ | KEY_WRITE, &hKey ) == ERROR_SUCCESS)
        {
            cbData = sizeof(ViewMode);
...
//  Creates the child window for the application specific area of the General page.
