...
//  thread.  The worker thread sends off a message to each slot in the array that is non-NULL.
...
int FListAll(POPENFILEINFO pOFI, HWND hDlg, LPTSTR szSpec);
...
#ifdef UNICODE
SHORT WINAPI GetFileTitleA(LPCSTR lpszFileA, LPSTR lpszTitleA, WORD cbBuf)
...
#else
SHORT WINAPI GetFileTitleW(LPCWSTR lpszFileW, LPWSTR lpszTitleW, WORD cbBuf)
...
SHORT WINAPI GetFileTitle(LPCTSTR lpCFile, LPTSTR lpTitle, WORD cbBuf)
//  The GetFileTitle function returns the name of the file identified by the lpCFile parameter.
//  This is useful if the file name was received via some method other than GetOpenFileName
...
//            > 0, buffer too small, size needed (including NULL terminator)
{
    ...
    if ((GetProcessVersion(0) >= 0x040000) && !(CDGetAppCompatFlags() & CDACF_FILETITLE))
    {
        ...
        //  If we have a root directory name (eg. c:\), then we need to go to the old implementation so that it will return -1.
        //  SHGetFileInfo will return the display name for the directory (which is the volume name).
        //  This is incompatible with Win95 and previous versions of NT.
        if ((lstrlen(lpszFile) != 3) || (lpszFile[1] != CHAR_COLON) || (!ISBACKSLASH(lpszFile, 2)))
        {
            result = SHGetFileInfo( lpszFile, FILE_ATTRIBUTE_NORMAL, &info, sizeof(info), SHGFI_DISPLAYNAME | SHGFI_USEFILEATTRIBUTES );
...
#ifdef UNICODE
BOOL WINAPI GetOpenFileNameA(LPOPENFILENAMEA pOFNA)
...
#else
BOOL WINAPI GetOpenFileNameW(LPOPENFILENAMEW pOFNW)
...
BOOL WINAPI GetOpenFileName(LPOPENFILENAME pOFN)
...
BOOL WINAPI GetSaveFileName(LPOPENFILENAME pOFN)
//  The GetSaveFileName function creates a system-defined dialog box that enables the user to select a file to save.
...
//            FALSE   if not
{
    ...
    {
        if ( !(pOFN->Flags & OFN_EXPLORER)      || (!pOFN->rgpMonikers || !pOFN->cMonikers))
        {
            StoreExtendedError(CDERR_INITIALIZATION);
            return (FALSE);
        }
    ...
    //  Do not allow the new look if they have hooks, templates, or multi select without the OFN_EXPLORER bit.

    //  Also don't allow the new look if we are in the context of a 16 bit process.
    if ( ((pOFN->Flags & OFN_EXPLORER) || (!(pOFN->Flags & (OFN_ENABLEHOOK | OFN_ENABLETEMPLATE | OFN_ENABLETEMPLATEHANDLE | OFN_ALLOWMULTISELECT)))) && (!IS16BITWOWAPP(pOFN)) )
    {
        BOOL fRet;
#ifdef UNICODE
    ...
    if (fFirstTime)
    {
        //  Create a DC that is compatible with the screen and find the handle of the null bitmap.
    ...
    {
        if (!(hRes = FindResource( pOFN->hInstance, pOFN->lpTemplateName, RT_DIALOG )))
        {
...
#else
        iRet = DialogBoxIndirectParam( g_hinst, (LPDLGTEMPLATE)hDlgTemplate, pOFN->hwndOwner, qfnDlgProc, (DWORD)pOFI );
#endif
...
BOOL FileHookCmd(HANDLE hDlg, WPARAM wParam, LPARAM lParam, POPENFILEINFO pOFI)
...
//  Called by FileOpenDlgProc and FileSaveDlgProc.
{
    switch (GET_WM_COMMAND_ID(wParam, lParam))
    {
        ...
        {
#ifdef UNICODE
            //  Apps that side-effect these messages may not have their internal unicode strings
            //  updated.  They may also forget to gracefully exit the network enum'ing worker thread.
        ...
        case ( cmb2 ) :
        {
            switch (GET_WM_COMMAND_CMD(wParam, lParam))
            {
                ...
                {
                    //  In case an app has a hook, and returns true for processing WM_COMMAND messages,
                    //  we still have to worry about our internal message that came through via WM_COMMAND.
                    FileOpenCmd( hDlg, wParam, lParam, pOFI, FALSE );
                    break;
                }
            }
    ...
    *pszOut = 0;       // prepare to return empty string
    ...
    if (SHGetSpecialFolderPath(NULL, szPersonal, CSIDL_PERSONAL, FALSE))
    {
        ...
        else
        {
            ...
            if (GetCurrentDirectory(ARRAYSIZE(szPath), szPath) && (PathIsTemporary(szPath) || (0 == lstrcmpi(szPath, szPersonal))))
                fUseMyDocs = TRUE;
        }
    }
...
ChangingDir:
                ...
                *pOFI->szPath = 0;
                ...
                {
                    cb = (int) SendDlgItemMessage( hDlg, lst2, LB_GETTEXT, (WPARAM)idirNew, (LPARAM)pOFI->szPath );
    ...
    LPTSTR lpszDisk)
{
    ...
    if (lpszDisk)
    {
        ...
        SendMessage( hCmb,
                     CB_SETCURSEL,
                     (WPARAM)SendMessage( hCmb, CB_FINDSTRING, (WPARAM)-1, (LPARAM)lpszDisk ), 0L );
    }
    else
    {
        ...
        if ((lpCurDlg = (LPCURDLG)TlsGetValue(g_tlsiCurDlg)) && (lpCurDir = lpCurDlg->lpstrCurDir))
        {
            ...
            SendMessage( hCmb,
                         CB_SETCURSEL,
                         (WPARAM)SendMessage( hCmb, CB_FINDSTRING, (WPARAM)-1, (LPARAM)szChangeSel ),
                         0L );
        }
    }
}
    ...
    HWND hDlg)
{
    ...
    if ((lpCurDlg->dwCurDlgNum == 0) && (!hLNDThread))
    {
        if (hLNDEvent && (bNetworkInstalled = IsNetworkInstalled()))
        {
            ...
            {
                hLNDThread = CreateThread(NULL, (DWORD)0, (LPTHREAD_START_ROUTINE)ListNetDrivesHandler, (LPVOID)NULL, (DWORD_PTR)NULL, &dwThreadID );
            }
        }
        else
        {
...
//           OR NULL if pSrcA is NULL.
{
    ...
    //  If the wide buffer is too small (or NULL or invalid), allocate a bigger buffer.
    if (LocalSize((HLOCAL)pDestW) < (size * sizeof(WCHAR)))
    {
        if (hBufW = LocalAlloc(LPTR, (size * sizeof(WCHAR))))
        {
            ...
            {
                LocalFree(hBufW);//  Don't change anything.
            }
        }
    }
    else
    {
        SHAnsiToUnicode(pSrcA,pDestW, cChars);//  Just use the original wide buffer.
    }
...
//  Function to allow NT WOW to keep the ANSI & UNICODE versions of the OPENFILENAME structure in ssync as required by many 16-bit apps.
