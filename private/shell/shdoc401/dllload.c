...
DELAY_LOAD_SHELL(g_hinstShell32, shell32, BOOL, SHSetShellWindowEx, 243, (HWND hwnd, HWND hwndChild), (hwnd, hwndChild));
DELAY_LOAD_SHELL(g_hinstShell32, shell32, int, RealDriveTypeFlags, 525, (int iDrive, BOOL fOKToHitNet), (iDrive, fOKToHitNet));
DELAY_LOAD_SHELL_VOID(g_hinstShell32, shell32, SHChangeNotifyReceive, 643, (LONG lEvent, UINT uFlags, LPCITEMIDLIST pidl, LPCITEMIDLIST pidlExtra), (lEvent, uFlags, pidl, pidlExtra));
DELAY_LOAD_SHELL(g_hinstShell32, shell32, LPSHChangeNotificationLock, SHChangeNotification_Lock, 644,
           (HANDLE hChangeNotification, DWORD dwProcessId, LPITEMIDLIST **pppidl, LONG *plEvent), (hChangeNotification, dwProcessId, pppidl,  plEvent));
DELAY_LOAD_SHELL(g_hinstShell32, shell32, BOOL, SHChangeNotification_Unlock, 645, (LPSHChangeNotificationLock pshcnl), (pshcnl));
DELAY_LOAD_SHELL(g_hinstShell32, shell32, BOOL, SHChangeRegistrationReceive, 646, (HANDLE hChangeNotification, DWORD dwProcId), (hChangeNotification, dwProcId));
DELAY_LOAD_SHELL_VOID(g_hinstShell32, shell32, SHWaitOp_Operate, 648, (HANDLE hWaitOp, DWORD dwProcId), (hWaitOp, dwProcId));
DELAY_LOAD_SHELL(g_hinstShell32, shell32, BOOL, __WriteCabinetState, 652, (LPCABINETSTATE lpState), (lpState));
DELAY_LOAD_SHELL(g_hinstShell32, shell32, BOOL, __ReadCabinetState, 654, (LPCABINETSTATE lpState, int cLength), (lpState, cLength));
...
DELAY_LOAD_SHELL(g_hinstShell32, shell32, BOOL, FileIconInit, 660, ( BOOL fRestoreCache ), ( fRestoreCache ));
...
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32, LoadRegTypeLib, (REFGUID rguid, unsigned short wVerMajor, unsigned short wVerMinor, LCID lcid, ITypeLib **pptlib), (rguid, wVerMajor, wVerMinor, lcid, pptlib));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32, LoadTypeLib, (const WCHAR *szFile, ITypeLib **pptlib), (szFile, pptlib));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32, RegisterTypeLib, (ITypeLib *ptlib, WCHAR *szFullPath, WCHAR *szHelpDir), (ptlib, szFullPath, szHelpDir));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32, SetErrorInfo, (DWORD  dwReserved, IErrorInfo  *perrinfo), (dwReserved, perrinfo));
DELAY_LOAD(g_hinstOLEAUT32, OLEAUT32, BSTR, SysAllocString, (const WCHAR *pch), (pch));
DELAY_LOAD(g_hinstOLEAUT32, OLEAUT32, BSTR, SysAllocStringByteLen, (LPCSTR psz, unsigned int len), (psz, len));
DELAY_LOAD(g_hinstOLEAUT32, OLEAUT32, BSTR, SysAllocStringLen, (const WCHAR *pch, unsigned int i), (pch, i));
DELAY_LOAD_VOID(g_hinstOLEAUT32, OLEAUT32, SysFreeString, (BSTR bs), (bs));
DELAY_LOAD_UINT(g_hinstOLEAUT32, OLEAUT32, SysStringByteLen, (BSTR str), (str));
DELAY_LOAD_UINT(g_hinstOLEAUT32, OLEAUT32, SysStringLen, (BSTR str), (str));
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32, VariantChangeType, (VARIANTARG * pvargDest, VARIANTARG * pvarSrc, USHORT wFlags, VARTYPE vt), (pvargDest, pvarSrc, wFlags, vt));
...
DELAY_LOAD_HRESULT(g_hinstOLEAUT32, OLEAUT32, VarI4FromStr, (OLECHAR FAR * strIn, LCID lcid, DWORD dwFlags, LONG * plOut), (strIn, lcid, dwFlags, plOut));

// --------- CDFVIEW.DLL ---------------

HINSTANCE g_hinstCDFVIEW = NULL;

DELAY_LOAD_HRESULT(g_hinstCDFVIEW, CDFVIEW, ParseDesktopComponent, (HWND hwndOwner, LPWSTR wszURL, COMPONENT *pInfo), (hwndOwner, wszURL, pInfo));
DELAY_LOAD_HRESULT(g_hinstCDFVIEW, CDFVIEW, SubscribeToCDF, (HWND hwndParent, LPCWSTR pwzUrl, DWORD dwCDFTypes), (hwndParent, pwzUrl, dwCDFTypes));
...
DELAY_LOAD_IE_ORD(g_hinstBrowseui, BROWSEUI, HRESULT, SHCreateBandForPidl, 120,
                  (LPCITEMIDLIST pidl, IUnknown** ppunk, BOOL fAllowBrowserBand), (pidl, ppunk, fAllowBrowserBand));
DELAY_LOAD_IE_ORD(g_hinstBrowseui, BROWSEUI, HRESULT, SHPidlFromDataObject, 121,
...
DELAY_LOAD_IE_ORD(g_hinstBrowseui, BROWSEUI, DWORD, IDataObject_GetDeskBandState, 122, (IDataObject *pdtobj), (pdtobj));
