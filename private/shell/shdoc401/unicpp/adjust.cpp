                             ...
                             LPCRECT arectOldMonitors, LPCRECT arectOld, int crectOld)
{
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, (LPCTSTR)lpszDeskcomp, 0, KEY_READ, &hkey) == ERROR_SUCCESS)
    {
        ...
        while (RegEnumKeyEx(hkey, dwSubKeyIndex, szSubKeyName, &dwSubKeyLength, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
        {
            HKEY    hkeyComp;

            if (RegOpenKeyEx(hkey, szSubKeyName, 0, KEY_READ | KEY_WRITE, &hkeyComp) == ERROR_SUCCESS)
            {
                DWORD dwType, cbData;
                COMPPOS cp;
                ...
                cbData = SIZEOF(cp);
                if (RegQueryValueEx(hkeyComp, REG_VAL_COMP_POSITION, NULL, &dwType, (LPBYTE)&cp, &cbData) == ERROR_SUCCESS)
                {

                    // Move and size component if necessary.

                    if (ModifyPosAndSize(&cp, arectNew, crectNew, arectOldMonitors, arectOld, crectOld, &ema))
                    {

                        // Component's position/size has been modified.
                        // So, we need to update the registry.

                        if (RegSetValueEx(hkeyComp, REG_VAL_COMP_POSITION, 0, REG_BINARY, (LPBYTE)&cp, SIZEOF(cp)) == ERROR_SUCCESS)
                        {
                            fRet = TRUE;
                        }
                    }
