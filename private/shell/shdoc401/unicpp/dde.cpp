...
// that might be happening concurently, I don't expect many conversations to happen at the same time, so this can be rather simple
struct _DDECONV;
...
void _CheckForCurrentGroup(PDDECONV pddec)
{
    ...
    if (!pddec->szGroup[0])
    {
        ...
        if (g_pszLastGroupName != NULL) {
            ...
            CABINETSTATE cs;
            if (IsUserAnAdmin() && (ReadCabinetState(&cs, SIZEOF(cs)), cs.fAdminsCreateCommonGroups)) {
...
// REVIEW HACK - Don't just caste, call GetConvInfo() to get this. We can't do this as yet because of a bug in the thunk layer.
#define _GetDDEWindow(hconv)    ((HWND)hconv)
...
LRESULT _HandleDDEExecute(HWND hwnd, HWND hwndForward, UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL fSend)
{
    ATOM aApp, aTopic;
    HANDLE hNew;
    LPTSTR pNew, pOld;
...
// Stupid hacks to get various apps installed (read: ATM).
// These are the people who do a FindWindow for Progman and then do dde to it directly.
...
// Some installers (Wep2) forget to Terminate a conversation so we timeout after not getting any dde-messages for a while.
// If we don't, and you run a Wep2 install a second time we think the installer is already talking via ddeml so we don't reply from the desktop.
// Wep2 then thinks Progman isn't running, does a WinExec of Progman and hangs waiting to talk to it.
// Progman never replies since it is not the shell. Nasty Nasty Nasty.
...
// This is for explorer to call to initialize and uninitialize SHELL DDE services.
