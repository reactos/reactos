...
BOOL GetOldWorkAreas(LPRECT lprc, DWORD* pdwNoOfOldWA)
{
    ...
    *pdwNoOfOldWA = 0;
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, REG_DESKCOMP_OLDWORKAREAS, 0, KEY_READ, &hkey) == ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueEx(hkey, REG_VAL_OLDWORKAREAS_COUNT, NULL, &dwType, (LPBYTE)pdwNoOfOldWA, &cbSize) == ERROR_SUCCESS)// Read in the no of old work areas
        {
...
void SaveOldWorkAreas(LPCRECT lprc, DWORD nOldWA)
{
    ...
    {
        RegSetValueEx(hkey, REG_VAL_OLDWORKAREAS_COUNT, 0, REG_DWORD, (LPBYTE)&nOldWA, SIZEOF(nOldWA));// Write out the no. of old work areas
        RegSetValueEx(hkey, REG_VAL_OLDWORKAREAS_RECTS, 0, REG_BINARY, (LPBYTE)lprc, SIZEOF(*lprc) * nOldWA);// Write out the no work area rectangles
        RegCloseKey(hkey);// Close out the reg key
    }
}
...
void LoadSaveCCKeys(BOOL fLoad)
{
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_INTERNETSETTINGS, 0, NULL, 0, KEY_READ|KEY_WRITE, NULL, &hCUInetKey, &dw) != ERROR_SUCCESS)
    {
        return;
    }

    if (RegCreateKeyEx(HKEY_CURRENT_CONFIG,REGSTR_PATH_INTERNETSETTINGS,0, NULL, 0, KEY_READ|KEY_WRITE, NULL, &hCCInetKey, &dw) != ERROR_SUCCESS)
    {
        goto error1;
    }
    ...
    cb = sizeof (autoDial);
    if (RegQueryValueEx(hSrcKey, REGSTR_VAL_ENABLEAUTODIAL, NULL, NULL, (LPBYTE)&autoDial, &cb) == ERROR_SUCCESS)
    {
        RegSetValueEx(hDestKey, REGSTR_VAL_ENABLEAUTODIAL, 0, REG_BINARY, (LPBYTE)&autoDial, sizeof(DWORD));
    }
    cb = sizeof (proxyEnable);
    if (RegQueryValueEx(hSrcKey, REGSTR_VAL_PROXYENABLE, NULL, NULL, (LPBYTE)&proxyEnable, &cb) == ERROR_SUCCESS)
    {
        RegSetValueEx(hDestKey, REGSTR_VAL_PROXYENABLE, 0, REG_BINARY, (LPBYTE)&proxyEnable, sizeof(DWORD));
    }
...
LRESULT _HandleFSNotify(HANDLE hChange, DWORD dwProcId)
{
    ...
    if (pshcnl)
    {
        SHChangeNotifyReceive(pshcnl->pshcn->lEvent, pshcnl->pshcn->uFlags, pshcnl->pidlMain, pshcnl->pidlExtra);
...
HRESULT CDesktopBrowser::_CreateDeskBarForBand(UINT uEdge, IUnknown *punk, POINTL *pptl, IBandSite **ppbsOut)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        if (SUCCEEDED(CoCreateInstance(CLSID_CDockingBarPropertyBag, NULL, CLSCTX_INPROC_SERVER, IID_IDockingBarPropertyBagInit, (LPVOID*)&ppbi)))
        {
            if ((UINT)uEdge != -1) {
...
void DoTimebomb(HWND hwnd)
{
    ...
    if (st.wYear > 1997 || (st.wYear==1997 && st.wMonth > 11) || (st.wYear==1997 && st.wMonth == 11 && st.wDay > 15))
    {
        TCHAR szTitle[128];
        TCHAR szBeta[512];
...
void StartBackgroundShellTasks(void)
{
    ...
    if (ERROR_SUCCESS == RegOpenKey(g_hklmExplorer, TEXT("SharedTaskScheduler"), &hkey))
    {
        ...
        if (SUCCEEDED(CoCreateInstance(CLSID_SharedTaskScheduler, NULL, CLSCTX_INPROC, IID_IShellTaskScheduler, (LPVOID*) &pScheduler)))
        {
...
STDAPI_(BOOL) SHDesktopMessageLoop(HANDLE hDesktop)
{
    ...
    if (pBrowser)
    {
        ...
        // We will do our own Release() when we don't need the pointer any more.
        pBrowser->AddRef();
