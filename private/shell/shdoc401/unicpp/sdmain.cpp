...
class CShellDispatch : public IShellDispatch, public CObjectSafety, protected CImpIDispatch, public CObjectWithSite
{
...
CShellDispatch::CShellDispatch(void) : m_cRef(1), CImpIDispatch(&LIBID_Shell32, 1, 0, &IID_IShellDispatch)
{
    DllAddRef();
...
HRESULT CShellDispatch::ExecuteFolder(VARIANT vDir, LPCTSTR pszVerb)
{
    ...
    if (sei.lpIDList)
    {
        ...
        // BUGBUG:: Should be invoke idlist but that is failing when explore
...
STDMETHODIMP CShellDispatch::BrowseForFolder(long Hwnd, BSTR Title, long Options, VARIANT RootFolder, Folder **ppsdf)
{
