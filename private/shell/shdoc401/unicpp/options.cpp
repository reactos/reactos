...
BOOL CheckSingleClickDialog(HWND hwndParent)
{
    BOOL fRet = FALSE;
    HKEY hkey;
    if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_CURRENT_USER, TEXT("SOFTWARE\\MICROSOFT\\WINDOWS\\CURRENTVERSION\\EXPLORER\\ADVANCED"), 0, NULL, 0, KEY_ALL_ACCESS, NULL, &hkey, NULL))
    {
        DWORD dwType, dwData;
        DWORD cbData = SIZEOF(dwData);
        if (ERROR_SUCCESS != RegQueryValueEx(hkey, TEXT("CheckedSCDlg"), NULL, &dwType, (LPBYTE)&dwData, &cbData) || !dwData)
        {
            dwData = 1;
...
AddPropSheetCLSID(REFCLSID clsid, PROPSHEETHEADER *ppsh)
{
    ...
    hres = SHCoCreateInstance(NULL, &clsid, NULL, IID_IShellPropSheetExt, (LPVOID *)&psx);
    if (SUCCEEDED(hres)) {
        IShellExtInit *psxi;
...
void DoGlobalFolderOptions(void)
{
    ...
    else
    {
        ...
        hThread = CreateThread(NULL, 0, GlobalFolderOptPropSheetThreadProc, NULL, THREAD_PRIORITY_NORMAL, &dwThread);
        if (hThread != NULL)
        {
            CloseHandle(hThread);
        }
