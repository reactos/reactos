...
BOOL IsActiveDesktopEnabled()
{
    ...
    hres = CoCreateInstance(CLSID_ActiveDesktop, NULL, CLSCTX_INPROC_SERVER, IID_IActiveDesktop, (void**)&pad);
    if(SUCCEEDED(hres))
    {
...
HRESULT CStartMenuCallback::_ExecHmenuItem(HMENU hMenu, UINT uId)
{
    ...
    else
    {
        ...
        case IDM_DESKTOPHTML_CUSTOMIZE:
        case IDM_DESKTOPHTML_UPDATE:
            hThread = CreateThread(NULL, 0, ActiveDesktopThread, (LPVOID)uId, THREAD_PRIORITY_NORMAL, &dwThread);
            if(hThread != NULL)
                CloseHandle(hThread);
            hres = NOERROR;
            break;
...
HRESULT CreateNetworkConnections(IShellMenu* psm, HKEY hKeyRoot)
{
    ...
    HRESULT hres = CoCreateInstance (CLSID_ConnectionFolder, NULL, CLSCTX_INPROC, IID_IShellFolder, (LPVOID *)&psfFolder);
    if (SUCCEEDED(hres))
    {
            ...
            if (SUCCEEDED(hres))
            {
                ...
                if (SUCCEEDED(hres))
                {
                    ...
                    if (hKeyRoot)
                    {
                        ...
                        RegCreateKeyEx(hKeyRoot, TEXT("Network Connections"), NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKey, &dwDisp);
                    }
...
HRESULT CStartMenuCallback::_GetObject(LPSMDATA psmd, REFIID riid, void** ppvOut)
{
    ...
    *ppvOut = NULL;
    ...
    if (IsEqualGUID(riid, IID_IShellMenu))
    {
        ...
        // means the user's order preference isn't shown.  But at least the
        // startmenu still appears!
        RegCreateKeyEx(HKEY_CURRENT_USER, STRREG_STARTMENU, NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKeyRoot, &dwDisp);
        ...
        if (SUCCEEDED(hres))
        {
            ...
            case IDM_PROGRAMS:
                ...
                if (!SHRestricted(REST_NOCOMMONGROUPS))
                {
                    if (SUCCEEDED(hres = SHGetSpecialFolderLocation(NULL, CSIDL_COMMON_PROGRAMS, &pidl2)))
                    {
                        ...
                        hres = CoCreateInstance(CLSID_AugmentedShellFolder, NULL, CLSCTX_INPROC, IID_IAugmentedShellFolder, (LPVOID *)&pasf);
                        if (SUCCEEDED(hres))
                        {
                            ...
                            if (hKeyRoot)
                            {
                                ...
                                // means the user's order preference isn't shown.  But at least the
                                // startmenu still appears!
                                RegCreateKeyEx(hKeyRoot, pszKeyName, NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKey, &dwDisp);
                            }
            ...
            case IDM_FAVORITES:
                ...
                RegCreateKeyEx(HKEY_CURRENT_USER, STRREG_FAVORITES, NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKey, &dwDisp);
    ...
    *ppvOut = NULL;
    ...
    if (psmc)
    {
        ...
        if (SUCCEEDED(hres))
        {
            ...
            RegCreateKeyEx(HKEY_CURRENT_USER, STRREG_STARTMENU, NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hMenuKey, &dwDisp);
...
HRESULT CStartMenuTask::_CreateNewTask(IShellFolder * psf, LPCITEMIDLIST pidl)
{
    ...
    hres = CoCreateInstance( CLSID_SharedTaskScheduler, NULL, CLSCTX_INPROC, IID_IShellTaskScheduler, (LPVOID *) & pScheduler );
    if (SUCCEEDED(hres))
    {
        // create a new task
        IOldStartMenuTask * psmt;

        if (SUCCEEDED(CoCreateInstance(CLSID_StartMenuTask, NULL, CLSCTX_INPROC, IID_IOldStartMenuTask, (LPVOID *)&psmt)))
        {
            // Initialize the task
