...
void GetNextCompPos(COMPPOS *pcp, int iScreenWidth, int iScreenHeight, int iBorder)
{
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, (LPCTSTR)lpszDeskcomp, 0, NULL, 0, KEY_READ | KEY_WRITE, NULL, &hkey, &dw) == ERROR_SUCCESS)
    {
        ...
        RegQueryValueEx(hkey, REG_VAL_GENERAL_CCOMPPOS, NULL, &dw, (LPBYTE)&cComp, &cbSize);
    }
    ...
    if (((dwLayer * iBorder) > (DWORD)(iScreenWidth / (COMPONENT_PER_ROW + 1))) || ((dwLayer * iBorder) > (DWORD)(iScreenHeight / (COMPONENT_PER_COL + 1))))
    {
        cComp = 0;
        dwLayer = 0;
    }
    ...
    if (hkey && (iLeft != pcp->iLeft || iTop != pcp->iTop || dwWidth != pcp->dwWidth || dwHeight != pcp->dwHeight))
    {
        dw = cComp + 1;
        RegSetValueEx(hkey, REG_VAL_GENERAL_CCOMPPOS, 0, REG_DWORD, (LPBYTE)&dw, SIZEOF(dw));
        RegCloseKey(hkey);
    }
}
...
STDAPI_(BOOL) SetDesktopFlags(DWORD dwMask, DWORD dwNewFlags)
{
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, (LPCTSTR)lpszDeskcomp, 0, NULL, 0, KEY_READ | KEY_WRITE, NULL, &hkey, &dwDisposition) == ERROR_SUCCESS)
    {
        DWORD dwFlags;
        DWORD cbSize = SIZEOF(dwFlags);
        DWORD dwType;

        if (RegQueryValueEx(hkey, REG_VAL_COMP_GENFLAGS, NULL, &dwType, (LPBYTE)&dwFlags, &cbSize) != ERROR_SUCCESS)
        {
            dwFlags = 0;
        }
        ...
        if (RegSetValueEx(hkey, REG_VAL_COMP_GENFLAGS, 0, REG_DWORD, (LPBYTE)&dwFlags, sizeof(dwFlags)) == ERROR_SUCCESS)
        {
            fRet = TRUE;
        }
...
BOOL UpdateComponentFlags(LPCTSTR pszCompId, DWORD dwMask, DWORD dwNewFlags)
{
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szRegPath, 0, NULL, 0, KEY_READ | KEY_WRITE, NULL, &hkey, &dwDisposition) == ERROR_SUCCESS)
    {
        ...
        if (RegSetValueEx(hkey, REG_VAL_COMP_FLAGS, 0, REG_DWORD, (LPBYTE)&dwNewFlags, SIZEOF(DWORD)) == ERROR_SUCCESS)
        {
            fRet = TRUE;
        }
...
BOOL GetSavedStateInfo(LPTSTR pszCompId, LPCOMPSTATEINFO    pCompState, BOOL  fRestoredState)
{
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szRegPath, 0, NULL, 0, KEY_READ, NULL, &hkey, &dwDisposition) == ERROR_SUCCESS)
    {
        DWORD   cbSize, dwType;
                            ...
                            BOOL    fSaveRestorePos, DWORD dwCurState)
{
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szRegPath, 0, NULL, 0, KEY_READ | KEY_WRITE, NULL, &hkey, &dwDisposition) == ERROR_SUCCESS)
    {
        ...
        cp.izIndex = izIndex;
        if (RegSetValueEx(hkey, REG_VAL_COMP_POSITION, 0, REG_BINARY, (LPBYTE)&cp, SIZEOF(cp)) == ERROR_SUCCESS)
        {
            fRet = TRUE;
        }
