...
HRESULT THISCLASS::AddUrl(HWND hwnd, LPCWSTR pszSourceW, LPCOMPONENT pcomp, DWORD dwFlags)
{
    ...
    if (fOkay && fPathIsUrl && !fSubscribed)
    {
        ...
        else
        {
            ...
            {
                ShellMessageBox(MLGetHinst(), hwnd, MAKEINTRESOURCE(IDS_COMP_BADSUBSCRIBE), MAKEINTRESOURCE(IDS_COMP_TITLE), MB_OK);
            }
...
HRESULT THISCLASS::AddDesktopItemWithUI(HWND hwnd, LPCOMPONENT pComp, DWORD dwFlags)
{
    ...
    if(hwnd)
    {
        if(fCompExists)
        {
            ...
            {
                ISubscriptionMgr *psm;
                if (SUCCEEDED(hres = CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&psm)))
                {
                    si.cbSize = sizeof(si);
