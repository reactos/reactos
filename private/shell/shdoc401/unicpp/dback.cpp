...
{
    int index;
    LPTSTR pszPath = (LPTSTR) LocalAlloc(LPTR, ((pszDir ? lstrlen(pszDir) : 0) + lstrlen(pszFile) + 2) * sizeof(TCHAR));
    if (pszPath)
    {
        if (pszDir)
        {
...
void THISCLASS::_EnableControls(void)
{
    if (_fAllowChanges)
    {
        ...
        // a picture in centered mode, or when we have no wallpaper at all.
...
int THISCLASS::_GetImageIndex(LPCTSTR pszFile)
{
    ...
    if (pszFile && *pszFile)
    {
        ...
        if (*pszExt == TEXT('.'))
        {
            ...
            // If we fell off the end of the for loop here, this is a file with unknown extension.
            // So, we assume that it is a normal wallpaper and it gets the Bitmap's icon
            iRet = 1;
        }
        else
        {
...
void THISCLASS::_SetNewWallpaper(LPCTSTR pszFile)
{
        ...
        (szFile[1] == TEXT(':')))
    {
        ...
        if ((SHWNetGetConnection(szDrive, szTemp, &cchTemp) == NO_ERROR) && (szTemp[0] == TEXT('\\')) && (szTemp[1] == TEXT('\\')))
        {
    ...
    _EnableControls();// Put all controls in correct enabled state.
    ListView_EnsureVisible(_hwndLV, iSelectionNew, FALSE);// Make sure the selected item is visible.
}
...
void THISCLASS::_OnInitDialog(HWND hwnd)
{
    ...
    HIMAGELIST himl = ImageList_Create(GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), ILC_MASK, ARRAYSIZE(c_rgpszWallpaperExt), ARRAYSIZE(c_rgpszWallpaperExt));
    if (himl)
    {
        SHFILEINFO sfi;

        // Add the 'None' icon.
        HICON hIconNone = (HICON) LoadImage(HINST_THISDLL, MAKEINTRESOURCE(IDI_BACK_NONE), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), 0);
    ...
    _AddAFileToLV(NULL, g_szNone, 0);// Add 'none' option.
    _AddFilesToLV(szBuf, c_rgpszWallpaperExt[0], 1);// Add only the *.BMP files in the windows directory.
    ...
    if (_fAllowHtml)
    {
        ...
        for (i = 0; i < ARRAYSIZE(c_rgpszWallpaperExt); i++)
        {
            ...
            if ((lstrcmpi(c_rgpszWallpaperExt[i], TEXT("HTML")) == 0) || (lstrcmpi(c_rgpszWallpaperExt[i], TEXT("JPEG")) == 0))
                continue;
            _AddFilesToLV(szBuf, c_rgpszWallpaperExt[i], i + 1);
        }
    }

    ListView_SortItems(_hwndLV, _SortBackgrounds, 0);// Sort the standard items.

    WCHAR   wszBuf[MAX_PATH];
    LPTSTR  pszBuf;

    g_pActiveDesk->GetWallpaper(wszBuf, ARRAYSIZE(wszBuf), 0);// Add & select the current setting.
    ...
    // new Plus tab, that check box had been removed. So, we support "Stretch" through our background page drop down combo box.
    int    iEndStyle;
...
// This function checks to see if the currently selected wallpaper is a HTML wallpaper and if so, it makes sure that the active desktop is enabled.
// If it is disabled then it prompts the user asking a question to see if the user wants to enable it.
BOOL EnableADifHtmlWallpaper(HWND hwnd)
{
    ...
    //If the object is NOT dirty, that probably means another property sheet has already did the required work.
    // We don't need to re-do it again!
    ...
    {
        // Currently the active desktop is turned OFF. See, if we have a suitable wallpaper.
        WCHAR wszWallpaper[INTERNET_MAX_URL_LENGTH];
        LPTSTR pszWallpaper;
        ...
        if (!IsNormalWallpaper(pszWallpaper))
        {
            ...
            // Since the active desktop is currently OFF, ask the user if he wants to switch ON the active desktop!
...
void THISCLASS::_OnCommand(WORD wNotifyCode, WORD wID, HWND hwndCtl)
{
    ...
    case IDC_BACK_BROWSE:
        ...
        if (*pszFileName == TEXT('\0'))
        {
            ...
            // GetFileName breaks up the string into a directory and file component,
            // so we append a slash to make sure everything is considered part of the directory.
            lstrcat(pszFileName, TEXT("\\"));
        }
        if (GetFileName(_hwnd, pszFileName, ARRAYSIZE(wszFileName), IDS_BACK_FILETYPES, dwFlags) && ValidateFileName(_hwnd, pszFileName, IDS_BACK_TYPE1))
        {
...
BOOL CALLBACK THISCLASS::_DlgProc(HWND hdlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DISPLAYCHANGE:
        ...
        break;
    case WM_HELP:
        SHWinHelpOnDemandWrap((HWND) ((LPHELPINFO) lParam)->hItemHandle, c_szHelpFile, HELP_WM_HELP, (DWORD) aBackHelpIDs);
        break;
    case WM_CONTEXTMENU:
        SHWinHelpOnDemandWrap((HWND) wParam, c_szHelpFile, HELP_CONTEXTMENU, (DWORD) (LPVOID) aBackHelpIDs);
        break;
    case WM_DESTROY:
    {
