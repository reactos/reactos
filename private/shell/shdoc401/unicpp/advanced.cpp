...
//  This is a registry driven UI that walk through the part of the registry tree and convert it into a tree view list.
//  Note that registry values can be localized, so internation versions
//  have to localize the INF file so that setup will set the registry with the right text.
...
//          code can be added to support any random absolute reg path for the settings.
...
// AdvancedDlgProc
BOOL CALLBACK AdvancedOptionsDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if ( pAdv )
    {
        ...
        case WM_COMMAND:
            ...
            case IDC_ADVO_USECURRENTFOLDER:
                if (ShellMessageBox(MLGetHinst(), hDlg, MAKEINTRESOURCE(IDS_LIKECURRENT_TEXT), MAKEINTRESOURCE(IDS_FOLDERVIEWS), MB_YESNO | MB_ICONINFORMATION) == IDYES)
                {
            ...
            case IDC_ADVO_RESETTOORIGINAL:
                if (ShellMessageBox(MLGetHinst(), hDlg, MAKEINTRESOURCE(IDS_RESETALL_TEXT), MAKEINTRESOURCE(IDS_FOLDERVIEWS), MB_YESNO | MB_ICONINFORMATION) == IDYES)
                {
        ...
        case WM_HELP:// F1
        {
            ...
            {
                SHWinHelpOnDemandWrap( (HWND)((LPHELPINFO)lParam)->hItemHandle, TEXT("update.hlp"), HELP_WM_HELP, (DWORD)(LPSTR)aAdvOptsHelpIDs);
            }
            else
            {
                ...
                {
                    SHWinHelpOnDemandWrap((HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD)(LPSTR)aAdvOptsHelpIDs);
                }
        ...
        case WM_CONTEXTMENU:// right mouse click
        {
            ...
            {
                SHWinHelpOnDemandWrap( (HWND) wParam, TEXT("update.hlp"), HELP_CONTEXTMENU, (DWORD)(LPSTR)aAdvOptsHelpIDs);
            }
            break;
        }
        case WM_DESTROY:
