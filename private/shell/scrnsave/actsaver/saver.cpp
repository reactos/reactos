...
HRESULT CActiveScreenSaver::ReadSSInfo()
{
    ...
    m_pSSInfo->dwFeatureFlags = ReadRegDWORD(   HKEY_CURRENT_USER, g_szRegSubKey, s_szFeatureFlags, DEFAULT_FEATURE_FLAGS);

    // Navigate on Click
    m_pSSInfo->bNavigateOnClick = (ReadRegDWORD(HKEY_CURRENT_USER, g_szRegSubKey, s_szNavOnClick, DEFAULT_NAVIGATE_ON_CLICK) ? VARIANT_TRUE : VARIANT_FALSE);

    // Channel Time
    m_pSSInfo->dwChannelTime = ReadRegDWORD(HKEY_CURRENT_USER, g_szRegSubKey, s_szChannelTime, DEFAULT_CHANNEL_TIME);

    // Restart Time
    m_pSSInfo->dwRestartTime = ReadRegDWORD(HKEY_CURRENT_USER, g_szRegSubKey, s_szRestartTime, DEFAULT_RESTART_TIME_MINUTES);
    // Convert minutes to ms
    m_pSSInfo->dwRestartTime *= (60*1000);

    // Play sounds - 0 Mute 1 = No mute
    m_pSSInfo->bPlaySounds = (ReadRegDWORD( HKEY_CURRENT_USER, g_szRegSubKey, s_szPlaySounds, DEFAULT_PLAY_SOUNDS) ? VARIANT_TRUE : VARIANT_FALSE);
...
void CActiveScreenSaver::WriteSSInfo()
{
    ...
    WriteRegValue(  HKEY_CURRENT_USER, g_szRegSubKey, s_szNavOnClick, REG_DWORD, (BYTE *)&dwVal, SIZEOF(DWORD));

    // Channel Time
    WriteRegValue(  HKEY_CURRENT_USER, g_szRegSubKey, s_szChannelTime, REG_DWORD, (BYTE *)&m_pSSInfo->dwChannelTime, SIZEOF(DWORD));

    // Play sounds - 0 Mute 1 = No mute
    dwVal = ((m_pSSInfo->bPlaySounds == VARIANT_TRUE) ? TRUE : FALSE);
    WriteRegValue(  HKEY_CURRENT_USER, g_szRegSubKey, s_szPlaySounds, REG_DWORD, (BYTE *)&dwVal, SIZEOF(DWORD));
}
...
STDMETHODIMP CActiveScreenSaver::get_Mode(long * plMode)
{
    *plMode = m_lMode;
    return S_OK;
}
...
STDMETHODIMP CActiveScreenSaver::put_Mode(long lMode)
{
    m_lMode = lMode;
    return S_OK;
}
...
STDMETHODIMP CActiveScreenSaver::get_Running(VARIANT_BOOL * pbRunning)
{
...
STDMETHODIMP CActiveScreenSaver::put_Running(VARIANT_BOOL bRunning)
{
    m_bRunning = bRunning;
    return S_OK;
}
...
STDMETHODIMP CActiveScreenSaver::get_Config(IScreenSaverConfig ** ppSSConfig)
{
    return _InternalQueryInterface(IID_IScreenSaverConfig, (void **)ppSSConfig);
}


STDMETHODIMP CActiveScreenSaver::get_CurrentURL(BSTR * pbstrCurrentURL)
{
    if (m_bstrCurrentURL != NULL)
    {
...
STDMETHODIMP CActiveScreenSaver::put_CurrentURL(BSTR bstrCurrentURL)
{
    if (m_bstrCurrentURL != NULL)
    {
        SysFreeString(m_bstrCurrentURL);
...
STDMETHODIMP CActiveScreenSaver::Run(HWND hwndParent)
{
    RECT rectWindow;
    HRESULT hr;
...
STDMETHODIMP CActiveScreenSaver::InterfaceSupportsErrorInfo(REFIID riid)
{
...
STDMETHODIMP CActiveScreenSaver::GetInterfaceSafetyOptions(REFIID  riid, DWORD * pdwSupportedOptions, DWORD * pdwEnabledOptions)
{
...
STDMETHODIMP CActiveScreenSaver::SetInterfaceSafetyOptions(REFIID  riid, DWORD   dwOptionSetMask, DWORD   dwEnabledOptions)
{
    HRESULT hrResult = S_OK;
