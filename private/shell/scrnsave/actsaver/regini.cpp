...
)
{
    ...
    for (;;)
    {
        ...
        if ((retCode = RegOpenKeyEx(hKeyRoot, pszSubKey, 0, KEY_READ, &hKey)) != ERROR_SUCCESS)
        {
            break;
        }

        if ((retCode = RegQueryValueEx( hKey, pszValue, NULL, &dwValueType, pbReturn, pdwReturnSize)) == ERROR_SUCCESS)
        {
...
)
{
    ...
    for (;;)
    {
        ...
        if ((retCode = RegCreateKeyEx(  hKeyRoot, pszSubKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &dwDisposition)) != ERROR_SUCCESS)
        {
            break;
        }

        // Now set the key value first we query to see if
        if ((retCode = RegSetValueEx(   hKey, pszValue, NULL, dwValueType, pValue, dwSize)) == ERROR_SUCCESS)
        {
            bResult = TRUE;
        }
