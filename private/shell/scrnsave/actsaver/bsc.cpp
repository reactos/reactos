...
CBindStatusCallback::CBindStatusCallback() : m_cRef(1)
{
...
ULONG CBindStatusCallback::AddRef()
{
    ...
    TraceMsg(   TF_DEBUGREFCOUNT, "0x%.8X CBindStatusCallback::AddRef, m_cRef = %d", this, m_cRef);
    return m_cRef;
}
...
ULONG CBindStatusCallback::Release()
{
    ...
    TraceMsg(   TF_DEBUGREFCOUNT, "0x%.8X CBindStatusCallback::Release, m_cRef = %d", this, m_cRef);
...
HRESULT CBindStatusCallback::QueryInterface(REFIID riid, LPVOID * ppvObj)
{
    IUnknown * pUnk;
...
#ifdef _DEBUG
        TCHAR szName[128];
        TraceMsg(   TF_DEBUGQI, "0x%.8X CBindStatusCallback::QueryInterface(%s) -- FAILED", this, DebugIIDName(riid, szName));
#endif  // _DEBUG
...
STDMETHODIMP CBindStatusCallback::OnStopBinding(HRESULT hrResult, LPCWSTR szError)
{
...
{
    TraceMsg(   TF_BINDING, "0x%.8X OnDataAvailable(0x%.8X, %d)", this, grfBSCF, dwSize);
    return E_NOTIMPL;
}
...
CTimeoutBSC::CTimeoutBSC(DWORD dwTimeoutMS) : m_dwTimeoutMS(dwTimeoutMS)
{
...
STDMETHODIMP CTimeoutBSC::OnStartBinding(DWORD dwReserved, IBinding *  pBinding)
{
    HRESULT hrResult;
    ...
    for (;;)
    {
        if (FAILED(hrResult = CBindStatusCallback::OnStartBinding(  dwReserved, pBinding)))
        {
            break;
        }

        if ((m_idTimeoutEvent = timeSetEvent(   m_dwTimeoutMS, 500, BSCTimeoutTimerProc, (DWORD_PTR)SAFECAST(this, CTimeoutBSC *), TIME_ONESHOT)) == NULL)
        {
            hrResult = E_FAIL;
            break;
        }
...
STDMETHODIMP CTimeoutBSC::OnProgress(ULONG   ulProgress, ULONG   ulProgressMax, ULONG   ulStatusCode, LPCWSTR szStatusText)
{
    TraceMsg(   TF_BINDING, "0x%.8X OnProgress(ulProgress = %u, ulProgressMax = %d, ulStatusCode = %u)", this, ulProgress, ulProgressMax, ulStatusCode);
...
STDMETHODIMP CTimeoutBSC::OnStopBinding(HRESULT hrResult, LPCWSTR szError)
{
...
CSSNavigateBSC::CSSNavigateBSC(CScreenSaverWindow *    pScreenSaverWnd, DWORD dwTimeoutMS) : CTimeoutBSC(dwTimeoutMS), m_pScreenSaverWnd(pScreenSaverWnd)
{
...
STDMETHODIMP CSSNavigateBSC::OnStopBinding(HRESULT hrResultBind, LPCWSTR szError)
{
...
STDMETHODIMP CSSNavigateBSC::GetBindInfo(DWORD *     grfBINDF, BINDINFO *  pbindinfo)
{
    ...
    *grfBINDF = BINDF_ASYNCHRONOUS | BINDF_NOPROGRESSIVERENDERING | BINDF_SILENTOPERATION | BINDF_NO_UI;
