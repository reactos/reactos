...
{
    WriteSSInfo();// Save the settings and notify everyone that settings have changed.
    SendNotifyMessage(HWND_BROADCAST, WM_WININICHANGE, 0, (LPARAM) "Windows"); // Save the settings and notify everyone that settings have changed.
    return S_OK;
}
...
)
{
    ...
    for (;;)
    {
        ...
        EVAL(SUCCEEDED(CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void **)&pSubscriptionMgr)));
...
)
{
    ...
    if (bstrURL && bstrTitle)
    {
        for (;;)
        {
            if (FAILED(hrResult = CoCreateInstance( CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void **)&pSubscriptionMgr)))
            {
                break;
            }
