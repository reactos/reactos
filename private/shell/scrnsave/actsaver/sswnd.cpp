...
)
{
    ...
    for (;;)
    {
        ...
        if (!g_bPlatformNT)
        {
            ...
            EVAL((m_idReloadTimer = SetTimer(   ID_RESTART_TIMER, dwRestartTime, NULL)) != 0);
        }
...
)
{
    ...
    if  (m_bScrollbarVisible || ((m_pToolbar != NULL) && m_pToolbar->IsWindowVisible()))
    {
        EVAL((m_idControlsTimer = SetTimer( ID_CONTROLS_TIMER, TIME_CONTROLS_TIMEOUT, NULL)) != 0);
    }
...
)
{
    ...
    for (;;)
    {
        ...
        if  (FAILED(CLSIDFromProgID(OLESTR("Shell.Explorer"), &clsidBrowser)) ||
            FAILED(hrResult = CoCreateInstance( clsidBrowser, NULL, CLSCTX_INPROC_SERVER | CLSCTX_LOCAL_SERVER, IID_IUnknown, (void **)&m_pUnkBrowser)))
        {
            TraceMsg(TF_ERROR, "Failed to create Web Browser object.");
            break;
        }

        if (FAILED(hrResult = m_pUnkBrowser->QueryInterface(IID_IWebBrowser2, (void **)&m_pWebBrowser)))
        {
            TraceMsg(TF_ERROR, "Web browser doesn't support IWebBrowser2!");
            break;
        }

        if  (FAILED(hrResult = m_pUnkBrowser->QueryInterface(IID_IOleObject, (void **)&m_pOleObject)) ||
...
HRESULT CScreenSaverWindow::GetHTMLBodyElement(IHTMLBodyElement ** ppHTMLBodyElement)
{
    ...
    for (;;)
    {
        *ppHTMLBodyElement = NULL;
        ...
        if  (FAILED(m_pWebBrowser->get_Document(&pDispatch)) || (pDispatch == NULL))
        {
            hrResult = E_FAIL;
            break;
        }

        if (FAILED(hrResult = pDispatch->QueryInterface( IID_IHTMLDocument2, (void **)&pHTMLDocument)))
        {
            break;
        }

        if  (SUCCEEDED(hrResult = pHTMLDocument->get_body(&pHTMLElement)) && (pHTMLElement != NULL))
        {
            hrResult = pHTMLElement->QueryInterface(IID_IHTMLBodyElement, (void **)ppHTMLBodyElement);
        }
...
void CScreenSaverWindow::ResetChangeTimer(BOOL bExtendedTime)
{
    if (m_idChangeTimer != 0)
    {
        EVAL(KillTimer(m_idChangeTimer));
    ...
    EVAL((m_idChangeTimer = SetTimer(   ID_CHANNELCHANGE_TIMER, (iChannelTime * 1000) + (bExtendedTime ? TIME_NAVIGATE_TIMEOUT : 0), NULL)) != 0);
}
...
)
{
    ...
    for (;;)
    {
        ...
        if  ((pu.nScheme == URL_SCHEME_INVALID) ||
        ...
        if (g_bPlatformNT)
        {
            ...
            HANDLE hTermEvent;
            if ((hTermEvent = OpenEvent(EVENT_MODIFY_STATE, FALSE, s_szSETermEvent)) != NULL)
            {
...
)
{
    ...
    s_hKeyboardHook = SetWindowsHookEx( WH_KEYBOARD, KeyboardHookProc, _pModule->GetModuleInstance(), GetCurrentThreadId());
    s_hMouseHook = SetWindowsHookEx(    WH_MOUSE, MouseHookProc, _pModule->GetModuleInstance(), GetCurrentThreadId());
        ...
        switch (dispIDMember)
        {
            ...
            case DISPID_HTMLDOCUMENTEVENTS_ONCLICK:
            {
                ...
                if (!(dwFeatureFlags & FEATURE_IGNORE_MOUSE_CLICKS))
                {
                    ...
                    EVAL((m_idClickCheckTimer = SetTimer(   ID_CLICKCHECK_TIMER, TIME_CLICKCHECK_TIMEOUT, NULL)) != 0);
                }
