...
)
{
    ...
    if  (
        SUCCEEDED(hrResult = CoCreateInstance( CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr2, (void **)&pSubscriptionMgr2))
        &&
        SUCCEEDED(hrResult = CoCreateInstance( CLSID_ChannelMgr, NULL, CLSCTX_INPROC_SERVER, IID_IChannelMgr, (void **)&pChannelMgr))
        &&
...
)
{
    ...
    if (GetPrivateProfileString(g_szChannel, g_szScreenSaverURL, TEXT(""), szScreenSaverURL, ARRAYSIZE(szScreenSaverURL), szDesktopINI) > 0)
    {
        HRESULT hr = pSubscriptionMgr2->GetItemFromURL(pci->pszURL, &pSubsItem);

        ASSERT((SUCCEEDED(hr) && pSubsItem) || (FAILED(hr) && !pSubsItem));

        VARIANT vFlags = { 0 };

        if (SUCCEEDED(hr))
        {
            if (SUCCEEDED(pSubsItem->ReadProperties(1, &g_pProps[PROP_CHANNEL_FLAGS], &vFlags)) &&
                (V_VT(&vFlags) == VT_I4) &&
...
)
{
    ...
    if (SUCCEEDED(hrResult = CoCreateInstance(  CLSID_CURLFolder, NULL, CLSCTX_INPROC_SERVER, IID_IShellFolder, (void **)psfInternet)))
    {
    ...
    BOOL                fWrite)
{
    ...
    for (;;)
    {
        if (FAILED(hrResult = CoCreateInstance( CLSID_InternetShortcut, NULL, CLSCTX_INPROC_SERVER, IID_IUniformResourceLocator, (void **)&pURL)))
        {
            break;
        }
