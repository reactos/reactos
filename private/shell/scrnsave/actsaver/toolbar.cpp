...
CToolbarWindow::~CToolbarWindow()
{
    if  ((m_hwndProperties != NULL) && IsWindow(m_hwndProperties))
    {
        DestroyWindow(m_hwndProperties);
    }

    if  ((m_hwndClose != NULL) && IsWindow(m_hwndClose))
    {
...
BOOL CToolbarWindow::Create(const RECT &            rect, CScreenSaverWindow *    pParentWnd)
{
    BOOL bResult = FALSE;
    ...
    for (;;)
    {
        ...
        if ((m_hwndClose = CreateWindowEx(  WS_EX_TOOLWINDOW,
                                            TOOLBARCLASSNAME,
                                            NULL,
                                            WS_CHILD | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | TBSTYLE_TRANSPARENT | TBSTYLE_FLAT | TBSTYLE_TOOLTIPS
...
BOOL CToolbarWindow::OnCommand(WPARAM wParam, LPARAM lParam)
{
    switch (wParam)
    {
...
BOOL CToolbarWindow::OnEraseBkgnd(HDC hDC)
{
...
BOOL CToolbarWindow::OnNotify(WPARAM      wParam, LPARAM      lParam, LRESULT *   plResult)
{
...
void CToolbarWindow::ShowToolbar(BOOL bShow)
{
...
void CToolbarWindow::OnShowWindow( BOOL    bShow, int     nStatus)
{
    if  ((m_hwndProperties != NULL) && IsWindow(m_hwndProperties))
    {
