...
void UD_SplatMe(HWND hdlg, BOOL bSet)
{
    ...
    hres = SHCoCreateInstance(NULL, &CLSID_InternetShortcut, NULL, IID_IUniformResourceLocator, (LPVOID *)&purl);
    if (SUCCEEDED(hres))
    {
        ShStr shstr;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            if (SUCCEEDED(hres))
            {
                ...
                if (SUCCEEDED(hres))
                {
                    const PROPSPEC propspec = { PRSPEC_PROPID, PID_INTSITE_FLAGS };
