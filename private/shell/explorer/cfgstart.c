...
void StartMenuSort()
{
    ...
    if (pidl)
    {
        ...
        if (SUCCEEDED(SHGetSpecialFolderLocation(NULL, CSIDL_COMMON_STARTMENU, &pidl2)))
        {
            ...
            hres = CoCreateInstance(&CLSID_AugmentedShellFolder2, NULL, CLSCTX_INPROC, &IID_IAugmentedShellFolder2, (void **)&pasf);
            if (SUCCEEDED(hres))
            {
    ...
    if (psf)
    {
        ...
        if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, STRREG_STARTMENU, 0, KEY_READ | KEY_WRITE, &hkeyRoot))
        {
            LPTSTR pszName;
