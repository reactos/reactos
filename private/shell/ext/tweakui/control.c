...
Control_GetShowState(LPCTSTR ptszName)
{
    ...
    GetPrivateProfileString(c_tszDontLoad, ptszName, c_tszNil, tsz, cA(tsz), c_tszControlIni);
...
Control_InitControls2(HWND hwnd)
{
    ...
    /* BUGBUG -- add string */
    GetPrivateProfileString(TEXT("MMCPL"), NULL, c_tszNil, tszKeys, cA(tszKeys), c_tszControlIni);
    ...
    for (ptsz = tszKeys; ptsz[0]; ptsz += lstrlen(ptsz) + 1) {
        ...
        GetPrivateProfileString(TEXT("MMCPL"), ptsz, c_tszNil, tszName, cA(tszName), c_tszControlIni);
...
void PASCAL Control_InitControls3(HWND hwnd)
{
    WIN32_FIND_DATA wfd;
    HANDLE h;
