...
RegCanModifyKey(HKEY hkRoot, LPCTSTR ptszSubkey)
{
    ...
    if (RegCreateKeyEx(hkRoot, ptszSubkey, 0, c_tszNil, REG_OPTION_NON_VOLATILE, KEY_WRITE, 0, &hk, &dw) == 0) {
        RegCloseKey(hk);
        fRc = 1;
    } else {
        fRc = 0;
...
LONG PASCAL _RegOpenKey(HKEY hk, LPCTSTR ptszSubKey, PHKEY phkResult)
{
...
_RegCreateKey(HKEY hk, LPCTSTR ptszSubKey, PHKEY phkResult)
{
    DWORD dw;
    if (ptszSubKey) {
    return RegCreateKeyEx(hk, ptszSubKey, 0, c_tszNil, REG_OPTION_NON_VOLATILE, MAXIMUM_ALLOWED, 0, phkResult, &dw);
    } else {
