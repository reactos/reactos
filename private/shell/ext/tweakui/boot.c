...
Boot_fLogo(void)
{
    ...
    return !(GetPrivateProfileString(c_tszBoot, c_tszDisplayDrv, c_tszNil, tsz, cA(tsz), c_tszSysIni) && lstrcmpi(tsz, c_tszXgaDrv) == 0);
}
...
Boot_FindMsdosSys(void)
{
    ...
    if (GetDriveTypeA(szRoot) == DRIVE_FIXED) {
        DWORD fl;
        if (GetVolumeInformation(szRoot, 0, 0, 0, 0, &fl, 0, 0) && !(fl & FS_VOL_IS_COMPRESSED)) {
        g_tszMsdosSys[0] = (TCH)szRoot[0];
        if (GetPrivateProfileString(c_tszPaths, c_tszWinDir, 0, tsz, cA(tsz), g_tszMsdosSys)) {
            return;
        }
        }
    }
