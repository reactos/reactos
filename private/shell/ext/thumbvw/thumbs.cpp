...
STDAPI DllRegisterServer(void)
{
    ...
    if ( SUCCEEDED( hr ))
    {
        ...
        {
            DWORD dwDefault = 1;
            lRes = RegSetValueExA( hkExplorer, c_szAutoExtract, NULL, REG_DWORD, (LPBYTE) &dwDefault, sizeof( dwDefault ));
        }
...
STDAPI DllUnregisterServer(void)
{
    ...
    hr = UnregisterHandler( (LPCSTR) c_rgszHtmlExts, ARRAYSIZE( c_rgszHtmlExts ), 7, c_szThumbnailExtract, c_szHtmlCLSID );
    if ( SUCCEEDED( hr ))
    {
        hr = RegisterHTMLExtractor();
    }
    if ( SUCCEEDED( hr ))
    {
        hr = UnregisterHandler( (LPCSTR) c_rgszDocFileExts, ARRAYSIZE( c_rgszDocFileExts ), 5, c_szThumbnailExtract, c_szDocFileCLSID );
    }

    if ( SUCCEEDED( hr ))
    {
        hr = UnregisterHandler( (LPCSTR) c_rgszOffExts, ARRAYSIZE( c_rgszOffExts ), 5, c_szThumbnailExtract, c_szOffCLSID );
        if ( !IsOS( OS_NT5 ))
        {
            hr = RegisterHandler( (LPCSTR) c_rgszOffTifExts, ARRAYSIZE( c_rgszOffTifExts ), 5, c_szThumbnailExtract, c_szOffCLSID );
        }
    }
    if ( SUCCEEDED( hr ))
    {
        hr = UnregisterHandler( (LPCSTR) c_rgszLnkExts, ARRAYSIZE( c_rgszLnkExts ), 5, c_szThumbnailExtract, c_szLnkCLSID );
    }
...
void RegisterApprovedExtension( REFCLSID clsid, UINT idDescr )
{
    ...
    if ( SUCCEEDED( hr ))
    {
        ...
        LONG lres = RegCreateKeyExA( HKEY_LOCAL_MACHINE, REGSTR_APPROVED, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hkey, NULL );
        if ( lres == ERROR_SUCCESS )
        {
            RegSetValueExA( hkey, szCLSID, 0, REG_SZ, (const LPBYTE) szDescr, (lstrlenA( szDescr) + 1) * sizeof( CHAR ));
            RegCloseKey( hkey );
        }
    }
}
...
void UnregisterApprovedExtension( REFCLSID clsid )
{
    ...
    if ( SUCCEEDED( hr ))
    {
        ...
        LONG lres = RegOpenKeyExA( HKEY_LOCAL_MACHINE, REGSTR_APPROVED, 0, KEY_WRITE, &hkey );
        if ( lres == ERROR_SUCCESS )
        {
