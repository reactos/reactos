 ...
 */
LOCAL(void) initial_setup(j_decompress_ptr cinfo)
...
LOCAL(void) per_scan_setup(j_decompress_ptr cinfo)
 ...
 */
LOCAL(void) latch_quant_tables(j_decompress_ptr cinfo)
{
    int ci, qtblno;
    jpeg_component_info *compptr;
    JQUANT_TBL * qtbl;
...
GLOBAL(void) jinit_input_controller(j_decompress_ptr cinfo)
{
    ...
    inputctl = (my_inputctl_ptr) (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_PERMANENT, SIZEOF(my_input_controller));
