    ...
    if ( dwClrDepth == 8 )
    {
        ...
        if ( guidType != CLSID_NULL )
        {
            ...
            if ( CreateSizedDIBSECTION( &rgDithered, dwClrDepth, hpal, NULL, &hbmpDithered, &pDitheredInfo, &pDitheredBits ))
            {
                ...
                HRESULT hr = CoCreateInstance( CLSID_IntDitherer, NULL, CLSCTX_INPROC_SERVER, IID_IIntDitherer, (LPVOID *) &pDither );
                if ( SUCCEEDED( hr ))
                {
...
HRESULT RegisterHandler( LPCSTR pszExts, UINT cExts, UINT cEntrySize, LPCSTR pszIID, LPCSTR pszCLSID )
{
    for ( UINT cKey = 0; cKey < cExts; cKey ++ )
    {
        HKEY hKey1;
        DWORD dwAction;
        LONG lRes = RegCreateKeyExA( HKEY_CLASSES_ROOT, pszExts, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE | KEY_READ, NULL, &hKey1, &dwAction );
