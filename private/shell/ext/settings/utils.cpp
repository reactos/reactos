...
LPTSTR FmtMsgSprintf(DWORD id, ...)
{
    ...
    if(!FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_MAX_WIDTH_MASK, g_hInstance, id, 0, (LPTSTR)&pszMsg, 1, &marker) && NO_ERROR != GetLastError())
    {
...
VOID FmtMsgSprintf(LPTSTR pszDest, UINT cchDest, DWORD id, ...)
{
    ...
    if(!FormatMessage(FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_MAX_WIDTH_MASK, g_hInstance, id, 0, pszDest, cchDest, &marker))
    {
        DebugMsg(DM_ERROR, TEXT("FmtMsgSprintf failed with error 0x%08X"), GetLastError());
