...
BOOL PRIVATE IsIniYes(
    ...
    for (i = 0; i < ARRAYSIZE(s_rgpszTrue); i++)
        {
        if (lstrcmpi(psz, s_rgpszTrue[i]) == 0)
            {
            bResult = TRUE;
            bNotFound = FALSE;
            break;
    ...
    if (bNotFound)
        ...
        for (i = 0; i < ARRAYSIZE(s_rgpszFalse); i++)
            {
            if (lstrcmpi(psz, s_rgpszFalse[i]) == 0)
                {
                bResult = FALSE;
                bNotFound = FALSE;
                break;
...
void PRIVATE ProcessBooleans(void)
    ...
    for (i = 0; i < ARRAYSIZE(s_rgbik); i++)
        {
        DWORD dwcbKeyLen;
        char szRHS[MAXSTRINGLEN];
        BOOLINIKEY * pbik = &(s_rgbik[i]);
        LPCSTR lpcszRHS;

        /* Look for key. */

        dwcbKeyLen = GetPrivateProfileString(pbik->ikh.pszSectionName, pbik->ikh.pszKeyName, "", szRHS, sizeof(szRHS), c_szIniFile);
        if (dwcbKeyLen)
            lpcszRHS = szRHS;
        else
...
void PRIVATE ProcessIntegers(void)
    ...
    for (i = 0; i < ARRAYSIZE(s_rgiik); i++)
        ...
        dwcbKeyLen = GetPrivateProfileString(piik->ikh.pszSectionName, piik->ikh.pszKeyName, "", szRHS, sizeof(szRHS), c_szIniFile);
        if (dwcbKeyLen)
            lpcszRHS = szRHS;
        else
        ...
        *(piik->puStorage) = AnsiToInt(lpcszRHS);
        ...
        DEBUG_MSG(TF_GENERAL, "ProcessIniFile(): %s set to %#04x.", piik->ikh.pszKeyName, *(piik->puStorage));
        }
    }
#endif
