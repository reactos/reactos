...
DWORD GetLeakSetting()
{
    ...
    if (!s_bInit)
    {
        ...
        int val;
        GetPrivateProfileStringA(c_szCcshellIniSecDebug, "LeakDetect", "0", szRHS, ARRAYSIZE(szRHS), c_szCcshellIniFile);
    ...
    else
    {
        ...
        if (VirtualQueryEx(hProcess, (LPVOID) ReturnAddress, &mbi, sizeof(mbi)))
        {
...
#endif
            {
                ...
                cch = GetModuleFileNameA((HINSTANCE) mbi.AllocationBase, achFile, ARRAYSIZE(achFile));
                g_pfnSymLoadModule(hProcess, NULL, ((cch) ? achFile : NULL), NULL, (DWORD) mbi.AllocationBase, 0);// Ignore the return code since we can't do anything with it.
...
int CMallocSpy::GetStackBacktrace(int ifrStart, int cfrTotal, DWORD *pdwEip, LPSTR szSym)
{
    HANDLE        hThread;
    CONTEXT       context;
    STACKFRAME    stkfrm;
