...
UINT GetProfilePath(LPWSTR pszPath, HKEY *phkey, UINT *pcchProfile)
{
    ...
    else
    {
        ...
        szProfileDir [0] = 0;
        err = RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\ProfileReconciliation", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hkeyProfRec, NULL);
...
void MakeFolderRoam(HKEY hkeyProfRec, LPCSTR pszName, LPCWSTR pszPath, UINT cchProfile)
{
    ...
    err = RegCreateKeyExA(hkeyProfRec, pszName, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hSubKey, NULL);
    if (err == ERROR_SUCCESS)
    {
...
HRESULT GetPathFromRegOrDefault(const NT_FOLDER_INFO *npfi, LPWSTR pszPath)
{
    ...
    err = RegCreateKeyExA(pfi->hkRoot, c_szUSF, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hkeyUserShellFolders, NULL);
    if (err == ERROR_SUCCESS)
    {
    ...
    if (pfi)
    {
        ...
        if (SUCCEEDED(hres))
        {
            ...
            if (hres == S_OK)
            {
                ...
                err = RegCreateKeyExA(pfi->hkRoot, c_szSF, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hkeyShellFolders, NULL);
                if (err == ERROR_SUCCESS)
                {
#ifdef UNIX
                    ...
                    */
                    ...
                    if (UnExpandEnvironmentString(pszPath, TEXT("%USERPROFILE%"), szEnvPath, ARRAYSIZE(szEnvPath)))
                    {
