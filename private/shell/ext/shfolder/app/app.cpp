...
{
    HANDLE hToken;
    if (OpenThreadToken(GetCurrentThread(), TOKEN_QUERY, TRUE, &hToken) || OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &hToken))
        return hToken;
...
int  WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pszArgs, INT command)
{
#if 0
    if (argc > 1)
