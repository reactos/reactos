...
Abstract:
    This module contains the application that displays an indicator in the Notification area of the Tray.
--*/
...
BOOL Internat_ValidateRegistryPreload()
{
    ...
    if (RegOpenKeyEx( HKEY_CURRENT_USER, TEXT("Keyboard Layout\\Preload"), 0, KEY_ALL_ACCESS, &hKeyPreload ) != ERROR_SUCCESS)
    {
        return (FALSE);
    }

    //  Open the HKCU\Keyboard Layout\Substitutes key.
    if (RegOpenKeyEx( HKEY_CURRENT_USER, TEXT("Keyboard Layout\\Substitutes"), 0, KEY_ALL_ACCESS, &hKeySubst ) != ERROR_SUCCESS)
    {
...
HICON Internat_GetIconFromFile(HIMAGELIST himIndicators, LPTSTR lpszFileName, UINT uIconIndex)
{
    int cx, cy;
    HICON hicon;
...
//  Internat_CreateIcon
HICON Internat_CreateIcon(HWND hwnd, WORD langID)
{
    HBITMAP hbmColour;
    HBITMAP hbmMono;
    HBITMAP hbmOld;
...
void Internat_CreateOtherIndicatorMenu(HWND hwnd, LPARAM lParam)
{
    HMENU hMenu;
    int cmd;
    TPMPARAMS tpm;
    ...
    if (cmd && (cmd != -1))
    {
        switch (cmd)
        {
            ...
            {
                LoadStringA( hinst, IDS_CPL_INTL, (LPSTR)szTextString, MENUSTRLEN );
            ...
            {
                WinHelp( hwnd, szHelpFile, HELP_CONTEXTPOPUP, IDH_KEYB_INDICATOR_ON_TASKBAR );
                break;
            }
        }
    }
...
int _stdcall Internat_ModuleEntry(void)
{
    ...
    i = Internat_WinMain( GetModuleHandle(NULL), NULL, NULL, si.dwFlags & STARTF_USESHOWWINDOW ? si.wShowWindow : SW_SHOWDEFAULT );
