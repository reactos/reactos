...
//  Function Prototypes.
LRESULT CALLBACK IndicDll_ShellHookProc(int nCode, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK IndicDll_KeyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK IndicDll_CBTProc(int nCode, WPARAM wParam, LPARAM lParam);


//  IndicDll_DllMain
BOOL WINAPI IndicDll_DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpvReserved)
{
    switch (dwReason)
    {
    ...
    BOOL bInternat)
{
    ...
    else
    {
        if (hwnd)
        {
            lpvSHDataHead->hookKbd = SetWindowsHookEx(WH_KEYBOARD, IndicDll_KeyboardHookProc, lpvSHDataHead->hinstDLL, 0);
        }
        else
        {
...
BOOL IndicDll_StartShell()
{
    ...
    {
        lpvSHDataHead->hookShell = SetWindowsHookEx(WH_SHELL, IndicDll_ShellHookProc, lpvSHDataHead->hinstDLL, 0);
        lpvSHDataHead->hookCBT = SetWindowsHookEx(WH_CBT, IndicDll_CBTProc, lpvSHDataHead->hinstDLL, 0);
