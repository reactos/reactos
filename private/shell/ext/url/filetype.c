...
BOOL FTEdit_OnOK(PFILETYPESDIALOGINFO pFTDInfo, HWND hDialog)
{
    ...
    // This may be overkill but for now, have it refresh the windows...
...
BOOL ActionIsEntered(HWND hDialog, BOOL bMBoxFlag)
{
    ...
    if(!GetDlgItemText(hDialog, IDC_FT_CMD_ACTION, szAction, sizeof(szAction)))
    {
        if(bMBoxFlag)
        {
            ...
            ShellMessageBox(hinstCabinet, hDialog, MAKEINTRESOURCE(IDS_FT_MB_NOACTION), MAKEINTRESOURCE(IDS_FT), MB_OK | MB_ICONSTOP);
...
BOOL ActionExeIsValid(HWND hDialog, BOOL bMBoxFlag)
{
    ...
    if(!(*szPath) || (!(PathIsExe(szPath))) || ((!(PathFileExists(szPath))) && (!(PathFindOnPath(szFileName, NULL)))))
    {
        ...
        {
            // Tell user that this exe is invalid
            ShellMessageBox(hinstCabinet, hDialog, MAKEINTRESOURCE(IDS_FT_MB_EXETEXT), MAKEINTRESOURCE(IDS_FT),MB_OK | MB_ICONSTOP);
...
BOOL FT_InitListViewCols(HWND hwndLV)
{
    ...
    SetWindowLong(hwndLV, GWL_EXSTYLE, GetWindowLong(hwndLV, GWL_EXSTYLE) | WS_EX_CLIENTEDGE);

    // Insert one column
    GetClientRect(hwndLV, &rc);
    col.cx = rc.right - GetSystemMetrics(SM_CXVSCROLL) - 2 * GetSystemMetrics(SM_CXEDGE);
...
int FTEdit_InitListView(PFILETYPESDIALOGINFO pFTDInfo)
{
    ...
    iSubKey = 0;
    if(RegOpenKeyEx(pFTDInfo->pFTInfo->hkeyFT, c_szShell, 0, KEY_ENUMERATE_SUB_KEYS | KEY_QUERY_VALUE, &hk) == ERROR_SUCCESS)
    {
        dwClass = sizeof(szClass);
        dwAction = sizeof(szAction);
...
DWORD SetVerbAttributes(HKEY hkeyFT, LPSTR pszVerb, DWORD dwAttributes)
{
    ...
    if((hkeyFT != NULL) && (RegOpenKeyEx(hkeyFT, szVerbKey, 0, KEY_SET_VALUE, &hk) == ERROR_SUCCESS))
    {
        dwSize = sizeof(dwAttributes);
        err = RegSetValueEx(hk, c_szEditFlags, 0, REG_BINARY, (LPBYTE)&dwAttributes, dwSize);
        if (err != ERROR_SUCCESS)
            dwAttributes= 0;
        RegCloseKey(hk);
    }
...
DWORD GetVerbAttributes(HKEY hkeyFT, LPSTR pszVerb)
{
    ...
    if((hkeyFT != NULL) && (RegOpenKeyEx(hkeyFT, szVerbKey, 0, KEY_QUERY_VALUE, &hk) == ERROR_SUCCESS))
    {
        dwSize = sizeof(dwAttributes);
                ...
                if (err != ERROR_SUCCESS || dwType != REG_BINARY || dwSize != sizeof(dwAttributes))
                {
                        dwAttributes = 0;
                }
        RegCloseKey(hk);
