...
/*
   The memory manager implementation in this module uses the process heap.
   The debug implementation of this memory manager keeps track of memory blocks
allocated from the heap using a doubly-linked list of heap element nodes.
  Each node describes one allocated heap element.

   Debug heap elements are allocated with extra space at the beginning and end of the element.
  Prefix and suffix sentinels surround each allocated heap element.
  New heap elements are filled with UNINITIALIZED_BYTE_VALUE.
  Freed heap elements are filled with FREED_BYTE_VALUE.
  The new tails of grown heap elements are filled with UNINITIALIZED_BYTE_VALUE.
*/
...
#ifdef DEBUG
#define PARANOID_ASSERT(exp) if (IS_FLAG_SET(s_dwMemoryManagerModuleFlags, MEMMGR_DFL_PARANOID_HEAP_VALIDATION)) ASSERT(exp);
#else
...
typedef struct _heapelemdesc{/* heap element descriptor structure */
   PCSTR pcszSize;
   PCSTR pcszFile;
   ULONG ulLine;
}HEAPELEMDESC;
...
typedef struct _heap{
   /*
    * This HEAPNODE must be the first HEAP structure field. 
    * Sometimes a PHEAP is used as a PHEAPNODE.
    */
   HEAPNODE hnHead;
...
typedef struct _heapsummary{/* heap summary filled in by AnalyzeHeap() */
...
PRIVATE_CODE BOOL IsValidHeapBlock(PCBYTE pcbyte, DWORD dwcbRequestedSize, DWORD dwcbAllocatedSize)
{
...
{
   /* dwcbPublicSize may be any value. */
   return(IS_VALID_READ_BUFFER_PTR(pcvPublic, VOID, dwcbPublicSize + sizeof(s_chsSuffix)) && EVAL((ULONG_PTR)pcvPublic >= sizeof(s_chsPrefix)));
}
   ...
   *pphn = NULL;
   ...
   if (s_pheap)
   {
      for (phn = s_pheap->hnHead.phnNext; phn; phn = phn->phnNext)
      {
         ...
         PARANOID_ASSERT((IS_VALID_READ_PTR(phn, CHEAPNODE) && phn->pcv == pcvTarget) || IS_VALID_STRUCT_PTR(phn, CHEAPNODE));
...
PRIVATE_CODE BOOL IsValidPublicHeapElement(PVOID pvPublic, DWORD dwcbPublicSize)
{
   PHEAPNODE phnUnused;

   /* dwcbPublicSize may be any value. */
   return(EVAL(IsValidPublicPtr(pvPublic, dwcbPublicSize)) && EVAL(FindHeapElement(GetPrivatePtr(pvPublic), &phnUnused)));
}


PRIVATE_CODE BOOL AddHeapElement(PCVOID pcvNew, DWORD dwcbSize, PCSTR pcszSize, PCSTR pcszFile, ULONG ulLine)
{
...
PRIVATE_CODE void FillNewMemory(PBYTE pbyte, DWORD dwcbRequestedSize, DWORD dwcbAllocatedSize)
{
   /* dwcbRequestedSize may be any value. */
   ...
   CopyMemory(pbyte + sizeof(s_chsPrefix) + dwcbRequestedSize, &s_chsSuffix, sizeof(s_chsSuffix));
...
PRIVATE_CODE void FillReallocatedMemory(PBYTE pbyte, DWORD dwcbOldRequestedSize, DWORD dwcbOldAllocatedSize, DWORD dwcbNewRequestedSize, DWORD dwcbNewAllocatedSize)
{
   ...
   if (IS_FLAG_CLEAR(s_dwMemoryManagerModuleFlags, MEMMGR_DFL_NO_ELEMENT_FILL) && dwcbNewAllocatedSize > sizeof(s_chsPrefix) + dwcbOldRequestedSize + sizeof(s_chsSuffix))
   {
      FillMemory(pbyte + sizeof(s_chsPrefix) + dwcbOldRequestedSize, dwcbNewAllocatedSize - dwcbOldRequestedSize - sizeof(s_chsPrefix), UNINITIALIZED_BYTE_VALUE);
   ...
   CopyMemory(pbyte + sizeof(s_chsPrefix) + dwcbNewRequestedSize, &s_chsSuffix, sizeof(s_chsSuffix));
...
PRIVATE_CODE void AnalyzeHeap(PHEAPSUMMARY phs, DWORD dwFlags)
{
   ...
   if (s_pheap)
   {
      for (pchn = s_pheap->hnHead.phnNext; pchn; pchn = pchn->phnNext)
      {
...
PUBLIC_CODE BOOL InitMemoryManagerModule(void)
{
   ...
   ASSERT((bResult && IS_VALID_STRUCT_PTR(s_pheap, CHEAP)) || (! bResult && EVAL(! s_pheap)));
#endif   /* DEBUG */
...
PUBLIC_CODE COMPARISONRESULT MyMemComp(PCVOID pcv1, PCVOID pcv2, DWORD dwcbSize)
{
   ...
   while (dwcbSize > 0 && ! (nResult = *pcbyte1 - *pcbyte2))
   {
      pcbyte1++;
      pcbyte2++;
      dwcbSize--;
...
PUBLIC_CODE BOOL DebugAllocateMemory(DWORD dwcbSize, PVOID *ppvNew, PCSTR pcszSize, PCSTR pcszFile, ULONG ulLine)
{
   BOOL bResult;
   ...
   ASSERT((bResult && EVAL(IsValidPublicHeapElement(*ppvNew, dwcbRequestedSize))) || (! bResult && EVAL(! *ppvNew)));
   HeapExit();
   return(bResult);
}


PUBLIC_CODE BOOL DebugReallocateMemory(PVOID pvOld, DWORD dwcbNewSize, PVOID *ppvNew)
{
   BOOL bResult;
   ...
   if (dwcbNewSize == dwcbOldSize)
      WARNING_OUT(("ReallocateMemory(): Size of heap element %#lx is already %lu bytes.", GetPublicPtr(pvOld), dwcbNewRequestedSize));

   dwcbOldRequestedSize = GetPublicMemorySize(pvOld);
   bResult = IReallocateMemory(pvOld, dwcbNewSize, ppvNew);
   if (bResult)
   {
      FillReallocatedMemory(*ppvNew, dwcbOldRequestedSize, dwcbOldSize, dwcbNewRequestedSize, IMemorySize(*ppvNew));
      ModifyHeapElement(pvOld, *ppvNew, dwcbNewRequestedSize);
      *ppvNew = GetPublicPtr(*ppvNew);
   }

   ASSERT((bResult && EVAL(IsValidPublicHeapElement(*ppvNew, dwcbNewRequestedSize))) || (! bResult && EVAL(! *ppvNew)));
   HeapExit();
   return(bResult);
}
...
PUBLIC_CODE BOOL SetMemoryManagerModuleIniSwitches(void)
{
   ...
   bResult = SetIniSwitches(s_rgcpcvisMemoryManagerModule, ARRAY_ELEMENTS(s_rgcpcvisMemoryManagerModule));
...
PUBLIC_CODE void SpewHeapSummary(DWORD dwFlags)
{
   ...
   TRACE_OUT(("Heap summary: %lu bytes in %lu used elements.", hs.dwcbUsedSize, hs.ulcUsedElements));
}
