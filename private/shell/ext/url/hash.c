...
// Comments:
//      This file contains functions that are roughly equivelent to the kernel atom function.
//      There are two main differences.  The first
//      is that in 32 bit land the tables are maintined in our shared heap, which makes it shared between all of our apps.
//      The second is that we can assocate a long pointer with each of the items, which in many
//      cases allows us to keep from having to do a secondary lookup from a different table
