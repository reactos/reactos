...
extern "C" void WINAPI OpenURL(HWND hwndParent, HINSTANCE hinst, PSTR pszCmdLine, int nShowCmd)
{
   ...
   TRACE_OUT(("OpenURL(): Trying to open Internet Shortcut %s.", pszCmdLine));
   ...
   if (hr == S_OK)
   {
      ...
      urlici.dwFlags = (IURL_INVOKECOMMAND_FL_ALLOW_UI | IURL_INVOKECOMMAND_FL_USE_DEFAULT_VERB);
      hr = intshcut.InvokeCommand(&urlici);
   }
   else
   {
      if (MyMsgBox(hwndParent, 
...
extern "C" void WINAPI FileProtocolHandler(HWND hwndParent, HINSTANCE hinst, PSTR pszCmdLine, int nShowCmd)
{
   ...
   pu.cbSize = sizeof(pu);
   if (S_OK == ParseURL(pszCmdLine, &pu) && URL_SCHEME_FILE == pu.nScheme)
   {
   ...
   TRACE_OUT(("FileProtocolHandler(): Invoking %s verb on %s.", pcszVerb ? pcszVerb : "open", pszCmdLine));

   hinstExec = ShellExecute(hwndParent, pcszVerb, pszCmdLine, NULL, NULL, nShowCmd);

   if (hinstExec > (HINSTANCE)32)
      TRACE_OUT(("FileProtocolHandler(): ShellExecute() %s verb on %s succeeded.", pcszVerb ? pcszVerb : "open", pszCmdLine));
   else
   {
...
extern "C" void WINAPI MailToProtocolHandler(HWND hwndParent, HINSTANCE hinst, PSTR pszCmdLine, int nShowCmd)
{
   ...
   if (GetProfileString(s_cszMAPISection, s_cszMAPIKey, EMPTY_STRING, szMAPIDLL, sizeof(szMAPIDLL)) > 0)
   {
      HINSTANCE hinstMAPI;

      TRACE_OUT(("MailToProtocolHandler(): MAPI provider DLL is %s.", szMAPIDLL));
      ...
      if (hinstMAPI)
      {
         ...
         MAPISendMailProc = (MAPISENDMAILPROC)GetProcAddress(hinstMAPI, s_cszMAPISendMail);
         if (MAPISendMailProc)
         {
            ...
            if (! lstrnicmp(pszCmdLine, s_cszMailToProtocolPrefix, SIZECHARS(s_cszMailToProtocolPrefix) - 1))
            ...
            TRACE_OUT(("MailToProtocolHandler(): Trying to send mail to %s.", mapito.lpszName));

            ulResult = (*MAPISendMailProc)(NULL, 0, &mapimsg, (MAPI_LOGON_UI | MAPI_DIALOG), 0);
            if (ulResult == SUCCESS_SUCCESS)
               TRACE_OUT(("MailToProtocolHandler(): MAPISendMail() to %s succeeded.", pszCmdLine));
            else
            {
               if (MyMsgBox(hwndParent, MAKEINTRESOURCE(IDS_SHORTCUT_ERROR_TITLE),
...
extern "C" void WINAPI NewsProtocolHandler(HWND hwndParent, HINSTANCE hinst, PSTR pszCmdLine, int nShowCmd)
{
   ...
   if (hinstNews)
   {
      ...
      RealNewsProtocolHandler = (RUNDLL32PROC)GetProcAddress(hinstNews, s_cszNewsProtocolHandler);
      if (RealNewsProtocolHandler)
      {
         TRACE_OUT(("NewsProtocolHandler(): Trying to open %s.", pszCmdLine));
...
extern "C" void WINAPI TelnetProtocolHandler(HWND hwndParent, HINSTANCE hinst, PSTR pszCmdLine, int nShowCmd)
{
  ...
  pszCmdLine = szCmdLine;
   ...
   if (! lstrnicmp(pszCmdLine, s_cszTelnetProtocolPrefix, SIZECHARS(s_cszTelnetProtocolPrefix) - 1))
      pszCmdLine += SIZECHARS(s_cszTelnetProtocolPrefix) - 1;
   else if (! lstrnicmp(pszCmdLine, s_cszRLoginProtocolPrefix, SIZECHARS(s_cszRLoginProtocolPrefix) - 1))
      pszCmdLine += SIZECHARS(s_cszRLoginProtocolPrefix) - 1;
   else if (! lstrnicmp(pszCmdLine, s_cszTN3270ProtocolPrefix, SIZECHARS(s_cszTN3270ProtocolPrefix) - 1))
   ...
   TRACE_OUT(("TelnetProtocolHandler(): Trying telnet to %s.", pszCmdLine));
...
HRESULT UnixReadNews(HWND hWndParent, PCSTR pszCmdLine)
{
    ...
    hr = SHGetValue(IE_USE_OE_NEWS_HKEY, IE_USE_OE_NEWS_KEY, IE_USE_OE_NEWS_VALUE, &dwType, (void*)&dwUseOENews, &dwSize);
