...
HRESULT STDMETHODCALLTYPE InternetShortcut::SetReferent(PCSTR pcszReferent, HWND hwndParent)
{
   HRESULT hr;
   ...
   hr = TranslateURL(pcszReferent, TRANSLATEURL_FL_GUESS_PROTOCOL, &pszTranslatedURL);
   if (SUCCEEDED(hr))
   {
      PCSTR pcszURLToUse;
   ...
   if (hr == S_OK)
      TRACE_OUT(("InternetShortcut::SetReferent(): Set referent %s as URL %s.", pcszReferent, m_pszURL));
   else
   {
      ASSERT(FAILED(hr));
      ...
      TRACE_OUT(("InternetShortcut::SetReferent(): Failed to set referent to %s.", pcszReferent));
   }
...
HRESULT STDMETHODCALLTYPE InternetShortcut::GetReferent(PSTR pszReferent, int ncReferentBufLen)
{
   HRESULT hr;
   ...
   if (m_pszURL)
   {
      if (lstrlen(m_pszURL) < ncReferentBufLen)
      {
         ...
         TRACE_OUT(("InternetShortcut::GetReferent(): Returning referent %s.", pszReferent));
      }
      else
         hr = E_FAIL;
   }
...
HRESULT STDMETHODCALLTYPE InternetShortcut::SetFolder(PCSTR pcszFolder)
{
   ...
   if (pszFolderCopy)
   {
      ...
      TRACE_OUT(("InternetShortcut::SetFolder(): Set folder to %s.", m_pszFolder));
   }
   else
      hr = E_OUTOFMEMORY;
...
HRESULT STDMETHODCALLTYPE InternetShortcut::GetFolder(PSTR pszFolder, int ncFolderBufLen)
{
   HRESULT hr;
   ...
   if (m_pszFolder)
   {
      if (lstrlen(m_pszFolder) < ncFolderBufLen)
      {
         ...
         TRACE_OUT(("InternetShortcut::GetFolder(): Returning folder %s.", pszFolder));
      }
      else
         hr = E_FAIL;
   }
...
HRESULT STDMETHODCALLTYPE InternetShortcut::GetName(PSTR pszName, int ncNameBufLen)
{
   HRESULT hr = E_FAIL;
   ...
   if (MLLoadStringA(IDS_SHORT_NEW_INTERNET_SHORTCUT, rgchShortName, sizeof(rgchShortName)))
   {
      char rgchLongName[MAX_PATH_LEN];

      if (MLLoadStringA(IDS_NEW_INTERNET_SHORTCUT, rgchLongName, sizeof(rgchLongName)))
      {
         ...
         if (pcszFolderToUse)
         {
            ...
            if (lPathYetAnotherMakeUniqueNameA(rgchUniqueName, pcszFolderToUse, rgchShortName, rgchLongName))
            {
               PSTR pszFileName;
               PSTR pszRemoveExt;
   ...
   if (hr == S_OK) {
      TRACE_OUT(("InternetShortcut::GetName(): Returning %s.", pszName));
   }
...
HRESULT STDMETHODCALLTYPE InternetShortcut::GetExtension(PSTR pszExtension, int ncExtensionBufLen)
{
   HRESULT hr;
   ...
   if (lstrlen(g_cszURLExt) < ncExtensionBufLen)
   {
      ...
      TRACE_OUT(("InternetShortcut::GetExtension(): Returning extension %s.", pszExtension));
   }
   else
   {
   ...
   ASSERT(IS_VALID_STRUCT_PTR(this, CInternetShortcut));
   ASSERT((hr == S_OK && IS_VALID_STRING_PTR(pszExtension, STR) && EVAL(lstrlen(pszExtension) < ncExtensionBufLen)) || 
   (hr == E_FAIL && EVAL(! ncExtensionBufLen || ! *pszExtension)));
