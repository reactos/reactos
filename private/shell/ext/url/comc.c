                                   ...
                                   PBYTE pbyteBuf, PDWORD pdwcbBufLen)
{
   ...
   ASSERT(IS_VALID_HANDLE(hkeyParent, KEY));
   ASSERT(! pcszSubKey || IS_VALID_STRING_PTR(pcszSubKey, CSTR));
   ASSERT(! pcszValue || IS_VALID_STRING_PTR(pcszValue, CSTR));
   ASSERT(! pdwValueType || IS_VALID_WRITE_PTR(pdwValueType, DWORD));
   ASSERT(! pbyteBuf || IS_VALID_WRITE_BUFFER_PTR(pbyteBuf, BYTE, *pdwcbBufLen));
   ...
   if (hr == S_OK)
   {
      ...
      if (AllocateMemory(dwcbCmdLineLen * sizeof(*pszCmdLine), &pszCmdLine))
      {
         ...
         if (CreateProcess(NULL, pszCmdLine, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
         {
            ...
            TRACE_OUT(("MyExecute(): CreateProcess() \"%s\" succeeded.", pszCmdLine));
         }
         else
         {
            hr = E_FAIL;

            WARNING_OUT(("MyExecute(): CreateProcess() \"%s\" failed.", pszCmdLine));
         }
