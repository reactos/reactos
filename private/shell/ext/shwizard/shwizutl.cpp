...
HRESULT EnsureFSObjectPresenceWithErrorUI(LPCTSTR pszFile, DWORD dwAccess, DWORD dwAttributes)
{
    ...
    if (FAILED(hres))
    {
        DisplayFolderCustomizationError(IDS_ACCESSERROR);// Report the error.
    }
...
HRESULT EnsureDirResourceObjectPresenceWithErrorUI(LPCTSTR pszDir, LPTSTR pszResourceObjectPath)
{
    ...
    HRESULT  hres = EnsureFSObjectPresenceWithErrorUI(pszResourceObjectPath, GENERIC_WRITE, FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_DIRECTORY);
    if (SUCCEEDED(hres))
    {
...
HRESULT MakeFileLocalCopy(LPCTSTR pszSrcFilePath, LPCTSTR pszFolderPath, LPTSTR pszDestFile)
{
    ...
    if (SUCCEEDED(EnsureDirResourceObjectPresenceWithErrorUI(pszFolderPath, szTemp)))
    {
        ...
        if (PathAppend(szTemp, pszDestFile) && (lstrcmpi(pszSrcFilePath, szTemp) != 0))
        {
...
BOOL IsBackgroundImageSet()
{
    ...
    TCHAR szTemp[30];
    return SHGetIniString(szGUID, TEXT("IconArea_Image"), szTemp, ARRAYSIZE(szTemp), g_szIniFile) && (lstrcmpi(szTemp, szNone) != 0);
}
...
BOOL IsIconTextColorSet()
{
    ...
    return GetPrivateProfileString(szGUID, TEXT("IconArea_Text"), TEXT(""),
        szTemp, ARRAYSIZE(szTemp), g_szIniFile) || GetPrivateProfileString(szGUID, TEXT("IconArea_TextBackground"), TEXT(""), szTemp, ARRAYSIZE(szTemp), g_szIniFile);
}
        ...
        if (g_szKey[0] && RegOpenKeyEx(HKEY_LOCAL_MACHINE, szTemp, 0, KEY_READ, &hkeyTemplate) == ERROR_SUCCESS)
        {
            ...
            if (RegOpenKeyEx(hkeyTemplate, REG_SUPPORTINGFILES, 0, KEY_READ, &hkeySupportingFiles) == ERROR_SUCCESS)
            {
                ...
                if (SUCCEEDED(EnsureDirResourceObjectPresenceWithErrorUI(g_szCurFolder, szTemp2)))
                {
                    ...
                    for (int i = 0; RegEnumValue(hkeySupportingFiles, i, szTemp3, &cchTemp3, NULL, NULL, (LPBYTE) szTemp, &cbTemp) == ERROR_SUCCESS; cchTemp3 = ARRAYSIZE(szTemp3), cbTemp = sizeof(szTemp), i++)
                    {
...
void SetRequiredAttributes()
{
    ...
    PathMakeSystemFolder(g_szCurFolder);// PathMakeSystemFolder() special cases the windir and system dir
}
...
void UnicodeToHTMLEscapeStringAnsi(LPCWSTR pwszSrc, LPSTR pszDest, int cchDest)
{
    while (*pwszSrc && (cchDest > MAX_HTML_ESCAPE_SEQUENCE))
    {
        ...
        // We can optimize the common ansi characters to avoid generating the long escape sequence.
        // This allows us to fit longer paths in the buffer.
        if (ul < 128)
        {
...
// Shows the preview of the HtmlFile lpszFileName in hwnd, which should have been subclassed with ThumbNailSubClassWndProc.
...
void Subclass(HWND hwnd)
{
    if (!g_lpThumbnailWndProc)  // It is not subclassed by anyone else.
    {
        ...
        if (g_lpThumbnailWndProc)
        {
            ...
            SendMessage(hwnd, WM_INITSUBPROC, 0, 0);// Initialize the subclass
        }
    }
}
...
int HasPersistMoniker(LPTSTR pszFileName, int cchFileName, LPTSTR pszPreviewFileName, int cchPreviewFileName)
{
    ...
    if (SUCCEEDED(hr) && pszFileName[0])
    {
        ...
        PathCombine(pszTemp, g_szCurFolder, pszTemp);// handle relative references...
...
BOOL ProdWebViewOn(HWND hwndOwner)
{
    ...
    if (pfn)// Webview is off. Ask if they want to turn it on.
    {
        ...
        if (!ss.fWebView)
        {
            ...
            if (MessageBox(hwndOwner, szMessage, szTitle, MB_OKCANCEL | MB_SETFOREGROUND | MB_ICONQUESTION | MB_DEFBUTTON1) != IDOK)
            {
                fRet = FALSE;
            }
            else
...
HRESULT UpdateComment(LPCTSTR pszHTMLComment)
{
    ...
    {
        TCHAR szTemp[MAX_PATH];
        if (SUCCEEDED(EnsureDirResourceObjectPresenceWithErrorUI(g_szCurFolder, szTemp)) && PathAppend(szTemp, FILE_HTMLINFOTIP))
        {
...
HRESULT GetCurrentComment(LPTSTR pszHTMLComment, int cchHTMLComment)
{
    ...
    if (!pszHTMLComment || (cchHTMLComment <= 0)) {
        ...
        PathCombine(pszHTMLInfoTipFile, g_szCurFolder, pszHTMLInfoTipFile);// handle relative references...

        HANDLE hCommentFile = CreateFile(pszHTMLInfoTipFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
        if (hCommentFile != INVALID_HANDLE_VALUE)
        {
            WCHAR wch;
            DWORD cbRead = 0;

            if (ReadFile(hCommentFile, (LPVOID) &wch, SIZEOF(wch), &cbRead, NULL) && (cbRead == SIZEOF(wch)))
            {
                DWORD cbToRead;
                ...
                if (wch == g_wchUnicodeBOFMarker)
                {
                    ...
                    if (!pwsz) {
                        ...
                        if (ReadFile(hCommentFile, (LPVOID) &wszTemp, (ARRAYSIZE(wszTemp) - 1) * SIZEOF(WCHAR), &cbRead, NULL) && (cbRead == (ARRAYSIZE(wszTemp) - 1) * SIZEOF(WCHAR)))
                        {
                    ...
                    if (!psz) {
                        ...
                        if (ReadFile(hCommentFile, (LPVOID) &szTemp2, (ARRAYSIZE(szTemp2) - 1) * SIZEOF(CHAR), &cbRead, NULL) && (cbRead == (ARRAYSIZE(szTemp2) - 1) * SIZEOF(CHAR)))
                        {
