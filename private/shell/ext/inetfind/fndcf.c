...
typedef struct CFndFactory {
    IClassFactory     cf;/* Supported interfaces */
...
*/
STDMETHODIMP CFndFactory_CreateInstance(PCF pcf, LPUNKNOWN punkOuter, RIID riid, PPV ppvObj)
{
    HRESULT hres;
    ...
    SquirtSqflPtszV(sqfl, TEXT("CFndFactory_CreateInstance() -> %08x [%08x]"), hres, *ppvObj);
    return hres;
}
 ...
 *    creating an object and not releasing it until you want to unlock the server.
*/
STDMETHODIMP CFndFactory_LockServer(PCF pcf, BOOL fLock)
{
