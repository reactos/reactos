...
STDMETHODIMP CMLFLink::GetCharCodePages(WCHAR chSrc, DWORD* pdwCodePages)
{
    ...
    if (SUCCEEDED(hr) && nDoneLen < (int)READWINTELDWORD(pHeader->m_dwBlockSize))
    {
...
HRESULT CMLFLink::CreateNT5FontLinkTable(void)
{
    ...
    // Get number of items
    if ((ERROR_SUCCESS != RegQueryInfoKey(hKey, NULL, NULL, 0, NULL, NULL, NULL, &ulFLinkFonts, NULL, NULL, NULL, NULL) || 0 == ulFLinkFonts) ||
        (ERROR_SUCCESS != RegQueryInfoKey(hKeyFont, NULL, NULL, 0, NULL, NULL, NULL, &ulFonts, NULL, NULL, NULL, NULL) || 0 == ulFonts))
    {
        hr = E_FAIL;
        goto TABLE_DONE;
    }
    ...
    while (ERROR_NO_MORE_ITEMS != RegEnumValueW(hKeyFont, dwIndex++, szFaceName, &dwValue, NULL, &dwType, (LPBYTE)szFontFile, &dwData ))
    {
    ...
    if (FontType == TRUETYPE_FONTTYPE)
    {
        if (g_pfont_table)
        {
            ...
            if (i > (int)g_pfont_table[0].dwCodePages[0] && plfFont->lfFaceName[0] != TEXT('@'))        // GDI font not in current font table?
            {
                ...
                pfont_table = (FONTINFO *) LocalReAlloc(g_pfont_table, sizeof(FONTINFO) * (g_pfont_table[0].dwCodePages[0]+2), LMEM_MOVEABLE | LMEM_ZEROINIT);
                if (NULL != pfont_table)
                {
                    g_pfont_table = pfont_table;
