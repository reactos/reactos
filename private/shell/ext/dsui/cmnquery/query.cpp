...
HRESULT CQueryFrame::GatherForms(VOID)
{
    ...
    for (iForm = 0; iForm < DSA_GetItemCount(_hdsaForms); iForm++) {
        ...
        for (iPage = 0; iPage < DSA_GetItemCount(pQueryForm->hdsaPages); iPage++) {
            ...
            _CallPageProc(pQueryFormPage, CQPM_SETDEFAULTPARAMETERS, (WPARAM)((_pOpenQueryWnd->dwFlags & OQWF_DEFAULTFORM) &&
                                                                              IsEqualCLSID(_pOpenQueryWnd->clsidDefaultForm, pQueryFormPage->clsidForm)), (LPARAM)_pOpenQueryWnd);
        }
...
HRESULT CQueryFrame::SelectForm(REFCLSID clsidForm)
{
    ...
    if ((pQueryForm != _pCurrentForm)) {
        ...
        for (i = 0; i < DSA_GetItemCount(pQueryForm->hdsaPages); i++) {
            ...
            if (pPage->idPageName && LoadString(pPage->hInstance, pPage->idPageName, szBuffer, ARRAYSIZE(szBuffer))) {
...
/   Change the currently active page of a query form to the one specified by the index.
...
/   Given the CLSID for the form return a pointer to its LPQUERYFORM structure, or NULL if not found.
/ In:
