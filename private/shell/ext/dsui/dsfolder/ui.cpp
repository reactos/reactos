...
HRESULT _OverrideProperties(HWND hwndParent, LPDATAOBJECT pDataObject, HKEY* aKeys, INT cKeys, LPCWSTR pPrefix)
{
    ...
    for ( i = 0 ; i < cKeys ; i++ )
    {
        ...
        if ( aKeys[i] )
        {
            ...
            if ( SUCCEEDED(CoCreateInstance(guidProperties, NULL, CLSCTX_INPROC_SERVER, IID_IDsFolderProperties, (LPVOID*)&pDsFolderProperties)) )
            {
...
DWORD WINAPI _ShowObjectPropertiesThread(LPVOID lpParam)
{
    ...
    if ( pDsObjects && (pDsObjects->cItems >= 1) )
    {
        ...
        if ( hres == S_FALSE )
        {
            ...
            hres = FormatMsgResource(&pTitle, GLOBAL_HINSTANCE, (fSelection) ? IDS_LARGESEL:IDS_SINGLESEL, W2T(bstrName));
