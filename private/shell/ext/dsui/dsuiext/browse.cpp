...
CreatePathCracker(IADsPathname **ppPath)
{
    ...
    hres =  CoCreateInstance(CLSID_Pathname, NULL, CLSCTX_INPROC_SERVER, IID_IADsPathname, (LPVOID*)ppPath);
                          ...
                          LPDWORD       pdwAdded)
{
    ...
    for (;;)
    {
        ...
        for (i = 0; i < cFetched; i++)
        {
            ...
            if (SUCCEEDED(V_DISPATCH(&aVariant[i])->QueryInterface(IID_IADs, (LPVOID*)&pDsObject)))
            {
...
// BUGBUG: handle the checked case properly;
        ...
        if ( iResult )
        {
            if ((dsbItem.dwMask & DSBF_STATE) && (dsbItem.dwStateMask & DSBS_HIDDEN))
            {
                ExitGracefully(hres, E_FAIL, "Item is hidden, therefore skipping");
            }

            if ((_bi.dwFlags & DSBI_CHECKBOXES) && (dsbItem.dwMask & DSBF_STATE) && (dsbItem.dwStateMask & DSBS_CHECKED))
            {
...
CBrowseDlg::InitDlg(HWND hDlg)
{
    ...
    if ( _bi.pfnCallback)
    {
        ...
        {
            Trace(TEXT("szFilter after DSBM_GETBROWSEDATA: %s"), _szFilter[0] ? W2T(_szFilter):TEXT("<not specified>"));
...
CBrowseDlg::DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case BFFM_ENABLEOK:
        EnableWindow(GetDlgItem(hDlg, IDOK), (BOOL)wParam);        break;
