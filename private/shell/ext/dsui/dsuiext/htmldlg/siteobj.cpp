...
CSite::Create(LPCTSTR pszUrl)
{
    ...
    // Create Trident
    hr = CoCreateInstance(CLSID_HTMLDocument, NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void **)&m_pObj);
...
#define DOASYNCRONOUS
            ...
            hr = m_pObj->QueryInterface(IID_IPersistStreamInit, (void **)&pPStrmInit);
            if (FAILED(hr))
            {
...
BOOL CSite::ConnectSink()
{
    ...
    for (int i = 0; i < lLength; i++)
    {
        ...
        if (varDisp.pdispVal != NULL)
        {
            ...
            {
                hr = pElem->QueryInterface(IID_IConnectionPointContainer, (void**)&pContainer);
                if (FAILED(hr))
                {
                ...
                hr = pContainer->FindConnectionPoint(DIID_HTMLInputTextEvents, &pConnection);
                if (FAILED(hr))
                {
...
//    if (m_pIOleObject == NULL || m_cCookieArrayElements == 0)
    ...
    hr = m_pIOleObject->QueryInterface(IID_IConnectionPointContainer, (void**)&pContainer);
    ...
    ASSERT(pContainer);
    hr = pContainer->FindConnectionPoint(DIID_HTMLInputTextElementEvents, &pConnection);
    if (FAILED(hr))
    {
