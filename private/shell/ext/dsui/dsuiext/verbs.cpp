...
STDAPI MySHStartNetConnectionDialog(HWND    hwnd, LPCTSTR pszRemoteName,       OPTIONAL DWORD   dwType)
{
    DWORD dwThreadId;
    HANDLE  hThread;
        ...
        for ( i = 0 ; i < ARRAYSIZE(menu_items); i++ )
        {
            ...
            if ( !StrCmpW(pObjectClass0, menu_items[i].pObjectClass ) )
            {
                ...
                if ( iVerb != -1 )
                {
                    ...
                    menu_items[i].pItemCB(MENUCMD_INITITEM, NULL, hMenu, MAKELPARAM(menu_items[i].uID, iVerb+idCmdFirst), &_vi, uFlags);
                }
            }
        }
    }
...
STDMETHODIMP CDsVerbs::InvokeCommand(LPCMINVOKECOMMANDINFO lpcmi)
{
    ...
    hr = menu_items[pMenuItem->iMenuItem].pItemCB(MENUCMD_INVOKE, lpcmi->hwnd, NULL, MAKELPARAM(menu_items[pMenuItem->iMenuItem].uID, 0), &_vi, 0);
    ...
    switch ( uCmd )
    {
        ...
        case MENUCMD_INVOKE:
        {
            ...
            for ( i = 0 ; i < DPA_GetPtrCount(pvi->hdpaSelection); i++ )
            {
                ...
                if ( FAILED(ADsOpenObject(pPath, pvi->pszUserName, pvi->pszPassword, (pvi->dwFlags & DSDSOF_SIMPLEAUTHENTICATE) ? 0:ADS_SECURE_AUTHENTICATION, IID_IADs, (LPVOID*)&pDsObject)) )
                {
...
HRESULT _PrinterVerbCB(UINT uCmd, HWND hWnd, HMENU hMenu, LPARAM uID, LPVERBINFO pvi, UINT uFlags)
{
    ...
    switch ( uCmd )
    {
        ...
        case MENUCMD_INVOKE:
        {
            ...
            hr = LocalAllocStringLen(&pBuffer, lstrlen(PRINT_FMT) + lstrlen(PRINT_SWITCH_OPEN) + lstrlen(PRINT_SWITCH_INSTALL) + lstrlen(pPrinterName) + 1 );
