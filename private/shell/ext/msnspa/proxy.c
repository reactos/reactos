...
SOCKET INTERNAL init_send_socket(SOCKET scfd, LPCSTR pszHost, u_short port, LPCSTR pszErrMsg)
/*
...
void set_sock_opt_int(SOCKET s, int optname, int val)
/*
...
SOCKET INTERNAL create_listen_socket(u_short port)
/*
...
BOOL INTERNAL ProxyPeekCommand(PCONNECTIONSTATE pcxs)
/*
 *      Study the incoming command to see if it is something we have a canned response to.
*/
{
    ...
    if (pcxs->nread > 4 && pcxs->buf[4] == ' ') {
         ...
         */
        if (lstrcmpi(szWord, pproxy->szIgnore1) == 0 || lstrcmpi(szWord, pproxy->szIgnore2) == 0) {
            /*
...
/*
 *      Tiny chunk of memory used to transfer proxy state between the ProxyThread() and the ProxyWorkerThread().
*/
...
DWORD WINAPI ProxyWorkerThread(LPVOID pvRef)
/*
...
*/
{
    ...
    cxs.ssfd = init_send_socket(cxs.scfd, cxs.pproxy->pszHost, cxs.pproxy->serverport, cxs.pproxy->pszError);
