...
int _stdcall ModuleEntry(void)
{
    ...
    i = WinMain(GetModuleHandle(NULL), NULL, (LPSTR)pszCmdLine, si.dwFlags & STARTF_USESHOWWINDOW ? si.wShowWindow : SW_SHOWDEFAULT);
    ExitProcess(i);
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    if (hExistWnd)
    {
        // NOTE: Send an enable message even if the command line parameter is 0 to force us to re-check for all enabled services.
    ...
    else
    {
        ...
        {
            // Wait for up to 30 seconds for the window to be created, send our message every second
            for (i = 0; i < 30; i ++)
            {
                Sleep(1000);
