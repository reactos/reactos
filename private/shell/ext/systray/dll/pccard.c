...
UINT GetDynInfo(DWORD dwDevNode, LPCTSTR lpszValName, LPVOID lpBuffer, UINT cbBuffer)
{
    UINT cbSize = 0;
    HKEY hkDyn = OpenDevnodeDynKey(dwDevNode);

    if (hkDyn) {
        if (RegQueryValueEx(hkDyn, lpszValName, NULL, NULL, lpBuffer, &cbBuffer) == ERROR_SUCCESS) {
            cbSize = cbBuffer;
        }
...
HKEY OpenDevnodeHwKey(DWORD dwDevnode)
{
    ...
    if (RegQueryValueEx(hkDyn, REGSTR_VAL_HARDWARE_KEY,    NULL, NULL, (LPSTR)&(szScratch[cchOffset]),    &cbSize) == ERROR_SUCCESS) {
...
UINT GetHwInfo(DWORD dwDevNode, LPCTSTR lpszValName, LPVOID lpBuffer, UINT cbBuffer)
{
    UINT cbSize = 0;
    HKEY hkHw = OpenDevnodeHwKey(dwDevNode);

    if (hkHw) {
        if (RegQueryValueEx(hkHw, lpszValName, NULL, NULL, lpBuffer, &cbBuffer) == ERROR_SUCCESS) {
            cbSize = cbBuffer;
        }
        RegCloseKey(hkHw);
    }
...
void UpdateSktTypes()
{
    ...
    for (i = 0; i < g_numskts; i++) {
        if (g_aSktState[i] == 0 && g_aDevnodes[i] != 0) {
            if (GetHwInfo(g_aDevnodes[i], g_szClass, szClassName, sizeof(szClassName))) {
...
{
    UINT cbReturned;
    if (DeviceIoControl(g_hPCCARD, PCCARD_IOCTL_GET_DEVNODES, NULL, 0, g_aDevnodes, sizeof(g_aDevnodes), &cbReturned, NULL)) {
...
void UpdateGlobalFlags()
{
    ...
    if (RegOpenKey(HKEY_CURRENT_USER, REGSTR_PATH_SYSTRAY, &hk) ==
        ...
        if (RegQueryValueEx(hk, g_szPCMCIAFlags, NULL, NULL, (LPSTR)(&g_PCMCIAFlags), &cb) != ERROR_SUCCESS) {
            g_PCMCIAFlags = 0;
        }
...
BOOL PCMCIA_Init(HWND hWnd)
{
    if (g_hPCCARD == INVALID_HANDLE_VALUE) {
        g_hPCCARD = CreateFile(TEXT ("\\\\.\\PCCARD"), GENERIC_READ | GENERIC_WRITE,  FILE_SHARE_READ | FILE_SHARE_WRITE,  NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        UpdateGlobalFlags();
    }
...
void PCMCIA_DeviceChange(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
    ...
    case DBT_DEVICEREMOVECOMPLETE:
    {
        ...
        if (RegOpenKey( HKEY_DYN_DATA, szDevNode, &hkDevDyna ) == ERROR_SUCCESS)
        {
        ...
        if (Status & DN_STARTED)
        {
            ...
            if (i != DEVNODE_NOT_IN_LIST) {
                ...
                BOOL fWarnUser = (g_aSktState[i] & (SKTSTATE_SHOULDWARN | SKTSTATE_GOODEJECT)) == SKTSTATE_SHOULDWARN;
                g_aSktState[i] = 0;
                UpdateSocketInfo();
                ...
                if (fWarnUser) {
                    ...
                    if (!(g_PCMCIAFlags & PCMCIA_REGFLAG_NOWARN)) {
                        ...
                        ShellExecute(NULL, szOpen, szRunDLL, szParams, NULL, SW_SHOW);
                    }
                }
            }
        }
        ...
        if (g_hPCCARD != INVALID_HANDLE_VALUE) {
            ...
            for (i = 0; i < g_numskts; i++) {
                if (g_aDevnodes[i] &&
                    ...
                    if (GetDynInfo(g_aDevnodes[i], REGSTR_VAL_CHILD, &dwChild, sizeof(dwChild)) &&
                        GetHwInfo(dwChild, g_szClass, szClassName, sizeof(szClassName)) &&
                        lstrcmpi(g_szDiskDriveClass, szClassName) == 0 &&
                        GetHwInfo(dwChild, REGSTR_VAL_CURDRVLET, szDriveLetters, sizeof(szDriveLetters)) &&
                        lstrlen(szDriveLetters) > 0) {
                        lpszMenuText = LoadDynamicString(IDS_EJECTFMTDISKDRIVE, szDesc, szDriveLetters[0]);
                    }
                    else
#endif
                    {
...
void PCMCIA_Menu(HWND hwnd, UINT uMenuNum, UINT uButton)
{
    ...
    GetCursorPos(&pt);
    iCmd = TrackPopupMenu(PCMCIA_CreateMenu(uMenuNum), uButton | TPM_RETURNCMD | TPM_NONOTIFY, pt.x, pt.y, 0, hwnd, NULL);
    if (iCmd >= PCMCIAMENU_SOCKET) {
        ...
        ShellExecute(NULL, szOpen, szRunDLL, lpszCommand, NULL, SW_SHOW);
