...
IsErrorCheckingEnabled()
{
    ...
    if (ERROR_SUCCESS != RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Usb"), 0, KEY_READ, &hKey)) {
        return TRUE;
    }
    if (ERROR_SUCCESS != RegQueryValueEx(hKey, TEXT("ErrorCheckingEnabled"), 0, &type, (LPBYTE) &ErrorCheckingEnabled, &size)) {
        return TRUE;
    }
