           ...
           )
{
    ...
    SetTimer( hWnd, FAX_STARTUP_TIMER_ID, 20 * 1000, NULL );// create the timer that will delay the startup of the fax code.
    ...
    {
        if (!IsDialogMessage(hWnd, &Msg) && !Fax_MsgProcess(&Msg) && !CSC_MsgProcess(&Msg))
        {
                     ...
                     )
{
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_ALLOCATE_BUFFER, (LPVOID) (DWORD) g_hInstance, StringID, 0, (LPTSTR) (LPTSTR FAR *)&pStr, 0, &Marker);
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ALLOCATE_BUFFER, (LPVOID) (LPTSTR) Buffer, 0, 0, (LPTSTR) (LPTSTR FAR *) &pStr, 0, &Marker);
