    ...
    )
{
    ...
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)HotPlugEjectDevice_Thread, (LPVOID)DeviceInstanceId, 0, &ThreadId);
}
...
void HotPlugContextMenu(HWND hwnd)
{
   POINT pt;
   HMENU ContextMenu;
   UINT MenuIndex;
   ...
   MenuIndex = TrackPopupMenu(ContextMenu, TPM_RIGHTBUTTON | TPM_RETURNCMD | TPM_NONOTIFY, pt.x, pt.y, 0, hwnd, NULL);
   switch (MenuIndex) {
   ...
   )
{
    ...
    if (ServiceEnabled) {
        // If there are now some hot plug devices and the icon is not being shown, then show it.
        if (bAnyHotPlugDevices && !ShowShellIcon) {
            HotPlugShowNotifyIcon(hDlg, TRUE);
        }

        // If there are NOT any hot plug devices and the icon is still being shown, then hide it.
...
/*++
Routine Description:
   To avoid deadlock with CM, a timer is started and the timer message handler does the real work.
Arguments:
