...
/ Globals
*/
const GUID CLSID_MyDocumentsExt =   { 0x450d8fbaL, 0xad25, 0x11d0, { 0x98, 0xa8, 0x08, 0x00, 0x36, 0x1b, 0x11, 0x03}};
...
/*
/   Main entry point.  We are passed reason codes and assored other information when loaded or closed down.
...
void DllSetTraceMask(void)
{
    ...
    if ( ERROR_SUCCESS == RegOpenKey(HKEY_CLASSES_ROOT, c_szCLSIDFormat, &hkey) )
    {
        ...
        if ( ERROR_SUCCESS == RegQueryValueEx( hkey, TEXT("TraceMask"), NULL, NULL, (LPBYTE)&dwTraceMask, &cbTraceMask))
        {
...
HRESULT _CheckPerUserSettings( BOOL fCreatePersonalPath )
{
    ...
    if (!GetPersonalPath( szSpecFolderPath, fCreatePersonalPath ))
    {
        ...
        // (usually) points to wstemp\mydocs.  In this case, we want to bail and try and get out of the way as soon as possible...
...
EXTERN_C STDAPI DllRegisterServer( void )
{
    ...
    cchDisplay = LoadString( g_hInstance, IDS_DISPLAY_NAME, szDisplayName, ARRAYSIZE(szDisplayName));
    if (!cchDisplay)
    {
    ...
    if ( ERROR_SUCCESS != RegSetValueEx( hkey, NULL, 0, REG_SZ, (LPBYTE)szDisplayName, (cchDisplay+1)*sizeof(TCHAR)))
    ...
    *szBuffer = 0;
    ...
    if ( ERROR_SUCCESS != RegSetValueEx( hkey, TEXT("InfoTip"), 0, REG_SZ, (LPBYTE)szBuffer, (cch+1)*sizeof(TCHAR)))
    ...
    // set SortOrderIndex...
    if ( ERROR_SUCCESS != RegSetValueEx( hkey, TEXT("SortOrderIndex"), 0, REG_DWORD, (LPBYTE)&dwSortIndex, sizeof(dwSortIndex) ))
    ...
    if ( ERROR_SUCCESS != RegCreateKey(HKEY_LOCAL_MACHINE, szBuffer, &hkey))
        ExitGracefully( hr, E_FAIL, "couldn't create namespace key" );

    // add our display name...
    if ( ERROR_SUCCESS != RegSetValueEx( hkey, NULL, 0, REG_SZ, (CONST BYTE *)szDisplayName, (cchDisplay+1)*sizeof(TCHAR)))
...
EXTERN_C STDAPI DllInstall( BOOL bInstall, LPCWSTR pszCmdLine )
{
    ...
    if (0 == lstrcmpi( szCmdLine, TEXT("UseReadOnly")))
    {
        ...
        if (ERROR_SUCCESS == RegCreateKey( HKEY_LOCAL_MACHINE, c_szExplorerSettings, &hkey ))
        {
            if (bInstall)
            {
                DWORD dwValue = 1;

                RegSetValueEx( hkey, c_szReadOnlyBit, 0, REG_DWORD, (LPBYTE)&dwValue, sizeof(dwValue));
            }
            else
            {
