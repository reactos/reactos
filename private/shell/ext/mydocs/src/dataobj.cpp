...
void FillInFileDescriptor( LPFILEDESCRIPTOR pfgd, LPTSTR pPath, BOOL bUnicode )
{
    ...
    if( GetFileAttributesEx( pPath, GetFileExInfoStandard, &fad ))
    {
        ...
        pfgd->dwFlags          = FD_ATTRIBUTES | FD_FILESIZE | FD_CREATETIME | FD_ACCESSTIME | FD_WRITESTIME;
    }
    else
    {
        if (bUnicode)
    ...
    {
        WideCharToMultiByte( CP_ACP, 0, pPath, -1, (LPSTR)pfgd->cFileName, ARRAYSIZE(pfgd->cFileName), NULL, NULL);
    }
#else
    if (bUnicode)
    {
        MultiByteToWideChar( CP_ACP, 0, pPath, -1, (LPWSTR)pfgd->cFileName, ARRAYSIZE(pfgd->cFileName));
    }
    else
    {
        lstrcpy( pfgd->cFileName, pPath);
