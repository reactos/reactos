...
try_again:
    ...
    if ( ERROR_SUCCESS == RegOpenKeyEx( hkeyToOpen, pSubKeyToOpen, 0, KEY_READ, &hkey ))
    {
...
BOOL IsPathAlreadyShellFolder( LPTSTR pPath, DWORD dwAttr )
{
    ...
    GetPrivateProfileString( c_szShellInfo, c_szCLSID, c_szMyDocsExt, szBuffer, ARRAYSIZE(szBuffer), szDesktopIni);
    ...
    GetPrivateProfileString( c_szShellInfo, c_szCLSID2, c_szMyDocsExt, szBuffer, ARRAYSIZE(szBuffer), szDesktopIni);
...
BOOL CanChangePersonalPath( void )
{
    ...
    if ( ERROR_SUCCESS == RegOpenKeyEx( HKEY_CURRENT_USER, c_szPolicy, 0, KEY_READ, &hkey ))
    {
        DWORD dwType, dwValue, dwSize = sizeof(DWORD);

        if ( ERROR_SUCCESS == RegQueryValueEx( hkey, c_szDisableChange, NULL, &dwType, (LPBYTE)&dwValue, &dwSize ))
        {
