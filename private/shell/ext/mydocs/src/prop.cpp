...
BOOL GetRawPersonalPath( LPTSTR pPath )
{
    ...
    *pPath = 0;
    if (ERROR_SUCCESS!=RegQueryValueEx( hkey, c_szPersonal, NULL, &dwType, (LPBYTE)pPath, &cbSize))
    {
...
BOOL InitTargetPage( HWND hDlg, LPARAM lParam )
{
    ...
    if (LoadString( g_hInstance, IDS_PROP_INSTRUCTIONS, szFormat, ARRAYSIZE(szFormat)))
    {
    ...
    SendDlgItemMessage( hDlg, IDD_TARGET, EM_SETLIMITTEXT, (WPARAM)(MAX_DIR_PATH), 0L);
    ...
    *szPath = 0;
    if (!GetRawPersonalPath( szPath ))
    {
        ...
        iRes = ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_NO_PATH_TEXT, GetMessageTitle(), MB_OKCANCEL | MB_APPLMODAL | MB_ICONSTOP | MB_TOPMOST);
        if (iRes == IDOK)
        {
    ...
    if (szPath[0])
    {
        ...
        SHGetFileInfo( szExpPath, 0, &sfi, SIZEOF(sfi), SHGFI_ICON|SHGFI_LARGEICON|SHGFI_DISPLAYNAME);
        // icon
        if (sfi.hIcon)
        {
...
void HandleApply( HWND hDlg, LPCUSTINFO lpci )
{
    ...
    if (lpci->bChanged && (lstrcmpi(szPath, lpci->szMyDocs)!=0))
    {
        ...
        // Make sure they haven't changed it to be something on the desktop...
        switch( dwRes )
        {
        case PATH_IS_READONLY:
            // Users generally want to write to My Documents
            ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_READONLY_PATH, GetMessageTitle(), MB_OK | MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST);
        ...
        case PATH_IS_DESKTOP:
            ...
            ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_NODESKTOP_FOLDERS, GetMessageTitle(), MB_OK | MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST);
            lres = PSNRET_INVALID_NOCHANGEPAGE;
            break;
        case PATH_IS_SYSTEM:
        case PATH_IS_WINDOWS:
            ...
            ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_NOWINDIR_FOLDER, GetMessageTitle(), MB_OK | MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST);
            lres = PSNRET_INVALID_NOCHANGEPAGE;
            break;
        case PATH_IS_PROFILE:
            // Looks like someone is trying to set the path
            // to be the profile directory.  Don't allow them to!
            ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_NOPROFILEDIR_FOLDER, GetMessageTitle(), MB_OK | MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST);
            lres = PSNRET_INVALID_NOCHANGEPAGE;
            break;
        ...
        case PATH_IS_GOOD:
            ...
            if (dwAttr & FILE_ATTRIBUTE_DIRECTORY)
            {
                if (lstrcmpi( szText, lpci->szMyDocs )!=0 )
                {
                    if (SUCCEEDED(ChangePersonalPath( szText, lpci->szMyDocs )))
                    {
                        ...
                        if (IDYES == ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_MOVE_DOCUMENTS, (LPTSTR)IDS_MOVE_DOCUMENTS_TITLE, MB_YESNO | MB_ICONQUESTION | MB_APPLMODAL | MB_TOPMOST, lpci->szMyDocs, szText))
                        {
                            // Need to move all the files...
                            if (MyMoveFiles( hDlg, szText, lpci->szMyDocs )) {
                                ShellMessageBox(g_hInstance, hDlg, (LPTSTR) IDS_MOVE_ERROR, (LPTSTR) IDS_MOVE_ERROR_TITLE, MB_OK | MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST, szText, lpci->szMyDocs);
                            }
                        }
            ...
            else if (dwAttr)
            {
                ...
                ShellMessageBox( g_hInstance, hDlg, (LPTSTR)id, (LPTSTR)IDS_INVALID_TITLE, MB_OK | MB_APPLMODAL | MB_ICONERROR | MB_TOPMOST);
                lres = PSNRET_INVALID_NOCHANGEPAGE;
            }
            else
            {
                ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_GENERAL_BADDIR, (LPTSTR)IDS_INVALID_TITLE, MB_OK | MB_APPLMODAL | MB_ICONSTOP | MB_TOPMOST);
                lres = PSNRET_INVALID_NOCHANGEPAGE;
            }
            break;
        default:
            ...
            ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_NOTALLOWED_FOLDERS, GetMessageTitle(), MB_OK | MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST);
            lres = PSNRET_INVALID_NOCHANGEPAGE;
            break;
        }
    }
...
void _FindTarget(HWND hDlg, LPTSTR pPath)
{
    ...
    {
        // It's on the desktop...
        ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_ON_DESKTOP, (LPTSTR)IDS_FIND_TITLE, MB_OK | MB_ICONINFORMATION | MB_APPLMODAL | MB_TOPMOST);
    }
    else
    {
...
BOOL CALLBACK TargetDlgProc( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        *g_szPropTitle = 0;
        ...
        {
            ShellMessageBox( g_hInstance, hDlg, (LPTSTR)IDS_INIT_FAILED_TEXT, GetMessageTitle(), MB_OK | MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST);
            return 0;
        }
        return 1;
    case WM_DESTROY:
    ...
    case WM_HELP:               /* F1 or title-bar help button */
        if ((((LPHELPINFO)lParam)->iCtrlId != IDD_ITEMICON )     && (((LPHELPINFO)lParam)->iCtrlId != IDD_INSTRUCTIONS ) && (((LPHELPINFO)lParam)->iCtrlId != IDC_TARGET_GBOX ))
        {
            WinHelp( (HWND) ((LPHELPINFO) lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD) (LPVOID) &rgdwHelpTarget[0]);
        }
        break;
    case WM_CONTEXTMENU:        /* right mouse click */
        {
            ...
            if (( ctrlid != IDD_ITEMICON )     && ( ctrlid != IDD_INSTRUCTIONS ))
            {
                WinHelp( (HWND) wParam, NULL, HELP_CONTEXTMENU, (DWORD) (LPVOID) &rgdwHelpTarget[0]);
            }
        }
