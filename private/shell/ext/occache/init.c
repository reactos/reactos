...
HRESULT GetControlFolderPath(LPTSTR szCacheDir, DWORD cchBuffer )
{
    ...
    if ((lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_PATH_IE_SETTINGS, 0x0, KEY_READ, &hKeyIntSetting)) == ERROR_SUCCESS)
    {
        ULONG ulSize = ulSizeBuf;
        lResult = RegQueryValueEx(hKeyIntSetting, REG_ACTIVEX_CACHE, NULL, NULL, (unsigned char*)lpszDir, &ulSize);
...
STDAPI SetCacheRegEntries( LPCTSTR szCacheDir )
{
    ...
    lResult = RegOpenKeyEx( HKEY_LOCAL_MACHINE, REG_PATH_IE_SETTINGS, 0x0, KEY_ALL_ACCESS, &hkeyIS );
    if ( lResult == ERROR_SUCCESS)
    {
    ...
    else
    {
        ...
        if ( lResult == ERROR_SUCCESS ) {
            ...
            for ( dwIndex = 0, cbName = sizeof(szName), cbValue = sizeof(szValue);
                  ...
                  dwIndex++, cbName = sizeof(szName), cbValue = sizeof(szValue) )
            {
                lResult = RegEnumValue( hkeyCacheList, dwIndex, szName, &cbName, NULL, NULL, (LPBYTE)szValue, &cbValue );
...
STDAPI_(BOOL) DllMain(HINSTANCE hInst, DWORD dwReason, LPVOID dwReserved)
{
    if (dwReason == DLL_PROCESS_ATTACH)
    {
        ...
        if ( RegOpenKeyEx( HKEY_LOCAL_MACHINE, REG_PATH_IE_SETTINGS, 0x0, KEY_ALL_ACCESS, &hkeyTest ) == ERROR_SUCCESS )
        {
            g_fAllAccess = TRUE;
