...
HRESULT CParseInf::DoParseDU(LPCTSTR szOCXFileName, LPCTSTR szCLSID)
{
    ...
    if (lResult == ERROR_SUCCESS)
    {
        ...
        dw = MAX_PATH;
        lResult = RegQueryValueEx(hKeyDLInfo, REGSTR_VALUE_INF, NULL, NULL, (unsigned char*)szBuffer, &dw);
        if (lResult == ERROR_SUCCESS)
        {
        ...
        pFileName = NULL;
        dw = MAX_PATH;
        lResult = RegQueryValueEx(hKeyDLInfo, REGSTR_VALUE_OSD, NULL, NULL, (unsigned char*)szBuffer, &dw);
        if (lResult == ERROR_SUCCESS)
        {
            pFileName = ReverseStrchr(szBuffer, '\\');
            if (pFileName != NULL)
         ...
         m_pCurPackageNode = m_pCurPackageNode->GetNextPackageNode())
    {
                ...
                !bFoundInOtherDU )
        {
            if ( lstrcmp(szT, m_szCLSID) != 0 ) // skip the current DU
            {
                ...
                if ( lResult == ERROR_SUCCESS )
                {
                    lResult = RegQueryValueEx(hkeyDUCJ, REGSTR_VALUE_INF, NULL, NULL, (unsigned char*)szT, &dw);
                         ...
                         ERROR_SUCCESS)
        {
                                           ...
                                           &ulSize, NULL, NULL, NULL, NULL)) == ERROR_SUCCESS)
            {
                if (!lstrcmpi(szDependency, m_szCLSID))
                {
                    ...
                    lResult = RegQueryValueEx(hkeyCurrent, NULL, NULL, &dwType, (unsigned char *)szDepName, &dwSize);
                    if (lResult != ERROR_SUCCESS || szDepName[0] == '\0') {
                        ...
                        if (lResult != ERROR_SUCCESS) {
                            ...
                            dwSize = MAX_CONTROL_NAME_LEN;
                            lResult = RegQueryValueEx(hkeyCOM, NULL, NULL, &dwType, (unsigned char *)szDepName, &dwSize);
                            if (lResult != ERROR_SUCCESS || szDepName[0] == '\0') {
                                MLLoadString(IDS_OCCACHE_WARNING_DEP_REMOVAL_NAME_UNKNOWN, szDepWarnBuf, MAX_MSGBOX_STRING_LEN);
                            }
                            else {
                                char                 lpszBuf[MAX_MSGBOX_STRING_LEN];
