...
USHORT NLS_STR::InsertParams(const NLS_STR * apnlsParamStrings [])
{
    ...
    */
    ...
    /* Determine total string length required for the expanded string and get out if we can't fulfill the request
     */
    ...
    for (;;)
    {
        ...
        if (wchParam >= '1' && wchParam <= '9')
        {
            ...
            if (iParamIndex < iNumParams) {
                ReplSubStr(*apnlsParamStrings[iParamIndex], istrParamEsc, ++istrCurPos);   // Replace #
                // Skip past entire substituted string
                istrCurPos.SetIB(istrParamEsc.QueryIB() + apnlsParamStrings[iParamIndex]->strlen());
            }
