...
LONG MyRegSaveKey(HKEY hKey, LPCSTR lpszFile, LPSECURITY_ATTRIBUTES lpsa)
{
    ...
    HANDLE hTemp = ::CreateFile(lpszFile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void AddBackslash(NLS_STR& nlsPath)
{
    ...
    if (!nlsPath.strrchr(&istrBackslash, '\\') || *nlsPath.QueryPch(++istrBackslash) != '\0')
        nlsPath += '\\';
}
 ...
 * create attempt fails, it tries to create each element of the path in case any intermediate directories also don't exist.
 */
{
    ...
    for (;;)
    {
         ...
         * not extremely performance-critical, and we can get errors if the directory already exists, etc.
         * With security and other things, the set of benign error codes we'd have to check for could be large.
         */
...
UINT SafeCopy(LPCSTR pszSrc, LPCSTR pszDest, DWORD dwAttrs)
{
    ...
    // servers, won't allow us to rename a read-only file.  So we have to take the attributes off, rename the file, then put back whatever the caller wants.
...
/*
 * PrefixMatch determines whether a given path is equal to or a descendant of a given base path.
 */
{
 ...
 * and recognize when the sync engine wants to replace a file that the user really wants deleted.
 */
{
     ...
     * because the "source" doesn't exist, we recognize that as the source having been deleted and synchronize manually by deleting the dest.

     * prnSrc points to the recnode for the item that's coming from the same
     * side of the transaction that the more recent profile was on;  prnDest points to the recnode for the other side.
     ...
     * dir) is the source and which the destination.  Then we have to figure out which of the two RECNODEs we got matches which directory.
     */
    PRECNODE prnSrc;
    PRECNODE prnDest;
     ...
     * which one is supposed to be definitive and transform the RECITEM and RECNODEs to indicate a copy instead of a merge.
...
BOOL MakePathAbsolute(NLS_STR& nlsDir, LPCSTR lpszBasePath, NLS_STR& nlsOldProfileDir, BOOL fMustBeRelative)
/*
 ...
 * and replaces the keyword with the corresponding real path.  Currently the keyword supported is:
 ...
 */
{
    ...
    if (re.GetError() == ERROR_SUCCESS)
    {
        ...
        if (bufSrcStrings.QueryPtr() != NULL && nlsSrcPath.QueryError() == ERROR_SUCCESS && nlsDestPath.QueryError() == ERROR_SUCCESS && nlsName.QueryError() == ERROR_SUCCESS)
        {
             ...
             * copy engine, because we're going to write this path out to the registry.
             */
...
/*
 GetMaxSubkeyLength just calls RegQueryInfoKey to get the length of the longest named subkey of the given key.
 The return value is the size of buffer needed to hold the longest key name, including the null terminator.
 */
{
    DWORD cchClass = 0;
    DWORD cSubKeys;
    ...
    RegQueryInfoKey(hKey, NULL, &cchClass, NULL, &cSubKeys, &cchMaxSubkey, &cchMaxClass, &cValues, &cchMaxValueName, &cbMaxValueData, &cbSecurityDescriptor, &ftLastWriteTime);
 ...
 * performs all file type reconciliation for the user's profile, excluding the profile itself, of course.

 * nlsOtherProfilePath is the path to the profile which is being cloned, or an empty string if the default profile is being cloned.
 */
{
