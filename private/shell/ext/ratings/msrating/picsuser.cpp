...
PicsCategory *FindInstalledCategory(array<PicsCategory *>&arrpPC, LPCSTR pszName)
{
    ...
    for (UINT i = 0; i < cCategories; i++) {
        ...
        if (!::strcmpf(pszThisName, pszName))
            return arrpPC[i];
        if (!::strncmpf(pszThisName, pszName, strlenf(pszThisName)) && arrpPC[i]->arrpPC.Length() > 0) {
...
UINT UserRatingSystem::ReadFromRegistry(HKEY hkeyProvider)
{
    ...
    do {
        ...
        err = RegEnumValue(hkeyProvider, iValue, szValueName, &cchValue, NULL, NULL, (LPBYTE) &dwValue, &cbData);
...
UINT UserRatingSystem::WriteToRegistry(HKEY hkeyRatings)
{
    ...
    for (pRating = m_pRatingList; pRating != NULL; pRating = pRating->m_pNext)
    {
        err = RegSetValueEx(hkey, pRating->QueryPch(), 0, REG_DWORD, (LPBYTE) &pRating->m_nValue, sizeof(pRating->m_nValue));
        if (err != ERROR_SUCCESS)
            break;
    }
...
UINT PicsUser::ReadFromRegistry(HKEY hkey, char *pszUserName)
{
...
#endif
    {
        ...
        while (((err = RegEnumKey(hkeyRatings, j, szKeyName, sizeof(szKeyName))) == ERROR_SUCCESS) && ((err = RegOpenKey(hkeyRatings, szKeyName, &hKeyProvider)) == ERROR_SUCCESS))
        {
...
UINT PicsUser::WriteToRegistry(HKEY hkey)
{
    ...
    err = RegCreateKeyEx(hkey, nlsUsername.QueryPch(), NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkeyUser, NULL);
    if (err != ERROR_SUCCESS)
        return err;
...
#if 0
    err = RegCreateKeyEx(hkeyUser, szRATINGS, NULL, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkeyRatings, NULL);
    if (err == ERROR_SUCCESS)
#else
    hkeyRatings = hkeyUser;
#endif
...
{
    for (UserRating *pRating = pURS->m_pRatingList; pRating != NULL; pRating = pRating->m_pNext)
    {
    ...
    if (pRating == NULL) {
        ...
        if (pRating != NULL) {
            ...
            if ((pPC->etfLabelled.fIsInit() && pPC->etfLabelled.Get()) || !pPC->etnMin.fIsInit())
                pRating->m_nValue = 0;
            else
...
{
    if (pPRS == NULL || !(pPRS->dwFlags & PRS_ISVALID) || !pPRS->etstrRatingService.fIsInit())
        return;
