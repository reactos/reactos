...
HRESULT CopySubPolicyExpressionToRegistry(PICSRulesPolicyExpression * pPRPolicyExpressionBeingCopied,HKEY hKeyExpression)
{
    ...
    if(pPRPolicyExpressionBeingCopied->m_pPRPolicyExpressionLeft!=NULL)
    {
        ...
        lError=RegCreateKeyEx(hKeyExpression,(char *) &szPICSRULESEXPRESSIONLEFT,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeyExpressionSubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
    ...
    if(pPRPolicyExpressionBeingCopied->m_pPRPolicyExpressionRight!=NULL)
    {
        ...
        lError=RegCreateKeyEx(hKeyExpression,(char *) &szPICSRULESEXPRESSIONRIGHT,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeyExpressionSubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
...
HRESULT WriteSystemToRegistry(HKEY hKey,PICSRulesRatingSystem **ppPRRS)
{
    ...
    if((pPRRSBeingCopied->m_pPRName)!=NULL)
    {
        lError=RegCreateKeyEx(hKey,(char *) &szPICSRULESPRNAME,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeySubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
    ...
    if((pPRRSBeingCopied->m_pPRSource)!=NULL)
    {
        lError=RegCreateKeyEx(hKey,(char *) &szPICSRULESPRSOURCE,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeySubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
    ...
    if(pPRRSBeingCopied->m_arrpPRPolicy.Length()>0)
    {
        ...
        lError=RegCreateKeyEx(hKey,(char *) &szPICSRULESPRPOLICY,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeySubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
        ...
        for(dwSubCounter=0;dwSubCounter<(DWORD) (pPRRSBeingCopied->m_arrpPRPolicy.Length());dwSubCounter++)
        {
            ...
            lError=RegCreateKeyEx(hKeySubKey,szNumber,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeyCopy,NULL);
            if(lError!=ERROR_SUCCESS)
            {
                return(E_FAIL);
            }
            ...
            lError=RegCreateKeyEx(hKeyCopy,(char *) &szPICSRULESPOLICYSUB,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeyExpression,NULL);
            if(lError!=ERROR_SUCCESS)
            {
                return(E_FAIL);
            }
            ...
            if(pPRByURLToCopy!=NULL)
            {
                    ...
                    dwPolicyExpressionSubCounter++)
                {
                    ...
                    lError=RegCreateKeyEx(hKeyExpression,szNumber,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hByURLKey,NULL);
                    if(lError!=ERROR_SUCCESS)
                    {
                        return(E_FAIL);
                    }
    ...
    if(pPRRSBeingCopied->m_arrpPRServiceInfo.Length()>0)
    {
        ...
        lError=RegCreateKeyEx(hKey,(char *) &szPICSRULESSERVICEINFO,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeySubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
    ...
    if(pPRRSBeingCopied->m_arrpPROptExtension.Length()>0)
    {
        ...
        lError=RegCreateKeyEx(hKey,(char *) &szPICSRULESOPTEXTENSION,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeySubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
    ...
    if(pPRRSBeingCopied->m_arrpPRReqExtension.Length()>0)
    {
        ...
        lError=RegCreateKeyEx(hKey,(char *) &szPICSRULESREQEXTENSION,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeySubKey,NULL);
        if(lError!=ERROR_SUCCESS)
        {
            return(E_FAIL);
        }
...
HRESULT WritePICSRulesToRegistry(LPCTSTR lpszUserName,HKEY hkeyUser,DWORD dwSystemToSave,PICSRulesRatingSystem **ppPRRS)
{
    ...
    lError=RegCreateKeyEx(hkeyUser,lpszUserName,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeyWrite,NULL);
    if(lError!=ERROR_SUCCESS)
    {
        return(E_FAIL);
    }
    ...
    lError=RegCreateKeyEx(hKeyWrite,lpszSystemNumber,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeyNumbered,NULL);
    if(lError!=ERROR_SUCCESS)
    {
...
HRESULT PICSRulesReadNameFromRegistry(DWORD dwSystemToRead, LPTSTR *ppszSystemName)
{
    ...
    if(hkeyUser!=NULL)
    {
        ...
        for (iCounter=0;iCounter<gPRSI->arrpPU.Length();++iCounter)
        {
            ...
            lError=RegCreateKeyEx(hKeyWrite,lpszSystem,NULL,NULL, REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS, NULL,&hKeySystem,NULL);
