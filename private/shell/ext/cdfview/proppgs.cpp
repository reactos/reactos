...
CPropertyPages::CPropertyPages(void) : m_cRef(1)
{
...
//    CPropertyPages QI.
STDMETHODIMP CPropertyPages::QueryInterface(REFIID riid, void **ppv)
{
    ASSERT(ppv);
...
// *** Name ***
STDMETHODIMP CPropertyPages::AddPages(LPFNADDPROPSHEETPAGE lpfnAddPage, LPARAM lParam)
{
    HRESULT hr = S_OK;
    PROPSHEETPAGE psp;
...
STDMETHODIMP CPropertyPages::Initialize(LPCITEMIDLIST pidl, LPDATAOBJECT pIDataObject, HKEY hkey)
{
    HRESULT hr;
    ...
    if (SUCCEEDED(hr))
    {
        if (DragQueryFile((HDROP)stgmed.hGlobal, 0, m_szPath, ARRAYSIZE(m_szPath)))
        {
            TCHAR szDesktopINI[MAX_PATH];

            PathCombine(szDesktopINI, m_szPath, c_szDesktopINI);

            GetPrivateProfileString(c_szChannel, c_szCDFURL, TEXT(""), m_szURL, ARRAYSIZE(m_szURL), szDesktopINI);
...
HRESULT CPropertyPages::InitializeSubsMgr2()
{
    ...
    else
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr2, (void**)&m_pSubscriptionMgr2);
            if (SUCCEEDED(hr))
            {
                IShellExtInit* pIShellExtInit;

                hr = m_pSubscriptionMgr2->QueryInterface(IID_IShellExtInit, (void **)&pIShellExtInit);
                if (SUCCEEDED(hr))
                {
...
{
    if (!m_bStartSubscribed && IsDlgButtonChecked(hdlg, IDC_MAKE_OFFLINE) && (NULL != m_pSubscriptionMgr2))
    {
        WCHAR wszURL[INTERNET_MAX_URL_LENGTH];
