...
)
{
    ...
    return GetPrivateProfileString(aIniTable[iv].pszSection, aIniTable[iv].pszKey, TEXT(""), pszOut, cch, szIniFile);
}
...
)
{
    ...
    HRESULT hr = CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&pISubscriptionMgr);
    if (SUCCEEDED(hr))
    {
