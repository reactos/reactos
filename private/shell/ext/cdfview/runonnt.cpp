...
// some internal functions that on Windows 95 the strings are Ansi, whereas the string on NT are unicode
...
//  This thunks a unicode string to ANSI, but if it's an ordinal, then we just leave it alone.
...
STDAPI Priv_SHDefExtractIcon(LPCTSTR pszIconFile, int iIndex, UINT uFlags, HICON *phiconLarge, HICON *phiconSmall, UINT nIconSize)
{
    HRESULT hr;
    ASSERT(uFlags == 0);

    // W95 integrated mode supports SHDefExtractIcon.
    // This supports matching the icon ectracted to the icon color depth.
    // ExtractIcon doesn't.
#ifndef UNIX
    if ((WhichPlatform() == PLATFORM_INTEGRATED))
    {
...
#endif
        {
            ...
            hr = SHDefExtractIconA(szIconFile, iIndex, uFlags, phiconLarge, phiconSmall, nIconSize);
        }
    }
    else
#endif /* !UNIX */
    {
        char szIconFile[MAX_PATH];
        SHUnicodeToAnsi(pszIconFile, szIconFile, ARRAYSIZE(szIconFile));
        hr = ExtractIconExA(szIconFile, iIndex, phiconLarge, phiconSmall, 1) ? S_OK : E_FAIL;
    }
