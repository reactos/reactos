...
)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&m_pSubscriptionMgr);
        }
    }
...
void CChannelMenu::Refresh(HWND hwnd)
{
    ...
    HRESULT hr = CoCreateInstance(CLSID_XMLDocument, NULL, CLSCTX_INPROC_SERVER, IID_IXMLDocument, (void**)&pIXMLDocument);
    if (SUCCEEDED(hr))
    {
        ...
        if (DownloadCdfUI(hwnd, m_bstrURL, pIXMLDocument))
        {
            ...
            SHChangeNotify(SHCNE_UPDATEDIR, SHCNF_PATH | SHCNF_FLUSH, (void*)m_szPath, NULL);
        }
