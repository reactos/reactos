...
//    CChannelMgr QI.
STDMETHODIMP CChannelMgr::QueryInterface (REFIID riid, void **ppv)
{
    ASSERT(ppv);
...
// ** CChannelMgr::AddRef **
STDMETHODIMP_(ULONG) CChannelMgr::AddRef (void)
{
    ASSERT(m_cRef != 0);
...
// ** CChannelMgr::Release **
STDMETHODIMP_(ULONG) CChannelMgr::Release (void)
{
    ASSERT (m_cRef != 0);
...
{
    if (!pChannelInfo || pChannelInfo->cbSize < sizeof(CHANNELSHORTCUTINFO) || pChannelInfo->pszURL == NULL || pChannelInfo->pszTitle == NULL)
    {
        ASSERT(FALSE);
        return E_INVALIDARG;
    }
    ...
    return ::AddChannel(szTitle, szURL, pszLogo, pszWideLogo, pszIcon, pChannelInfo->bIsSoftware ? DOC_SOFTWAREUPDATE : DOC_CHANNEL);
}
...
STDMETHODIMP CChannelMgr::EnumChannels(DWORD dwEnumFlags, LPCWSTR pszURL, IEnumChannels** ppIEnumChannels)
{
...
STDMETHODIMP CChannelMgr::GetChannelFolderPath (LPSTR pszPath, int cch, CHANNELFOLDERLOCATION cflChannel)
{
    ASSERT (pszPath || 0 == cch);
...
STDMETHODIMP CChannelMgr::GetChannelFolder (LPITEMIDLIST* ppidl, CHANNELFOLDERLOCATION cflChannel)
{
    if (ppidl == NULL)
        return E_FAIL;
...
STDMETHODIMP CChannelMgr::PreUpdateChannelImage(LPCSTR pszPath, LPSTR pszHashItem, int* piIndex, UINT* puFlags, int* piImageIndex)
{
#ifdef UNICODE
    TCHAR   tszPath[MAX_PATH];
    ...
    return ::PreUpdateChannelImage(tszPath, tszHashItem, piIndex, puFlags, piImageIndex);
#else
    return ::PreUpdateChannelImage(pszPath, pszHashItem, piIndex, puFlags, piImageIndex);
#endif
}


STDMETHODIMP CChannelMgr::UpdateChannelImage(LPCWSTR pszHashItem, int iIndex, UINT uFlags, int iImageIndex)
{
...
STDMETHODIMP CChannelMgr::ShowChannel(IWebBrowser2 *pIWebBrowser2, LPWSTR pwszURL, HWND hwnd)
{
    HRESULT hr;
...
STDMETHODIMP CChannelMgr::AddAndSubscribe(HWND hwnd, LPCWSTR pwszURL, ISubscriptionMgr *pSubscriptionMgr)
{
    return AddAndSubscribeEx2(hwnd, pwszURL, pSubscriptionMgr, FALSE);
}


STDMETHODIMP CChannelMgr::AddAndSubscribeEx2(HWND hwnd, LPCWSTR pwszURL, ISubscriptionMgr *pSubscriptionMgr, BOOL bAlwaysSubscribe)
{
    HRESULT hr;
    ...
    if (hr == S_OK)
    {
        ...
        if (bAlwaysSubscribe || !bInstalled || (hrPreinstalled == S_OK))
        {
            if (!pSubscriptionMgr)
            {
                hr = CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&pSubscriptionMgr);
            }
            else
            {
            ...
            if (SUCCEEDED(hr))
            {
                hr = pSubscriptionMgr->CreateSubscription(hwnd, pwszURL, wszTitle, dwFlags, SUBSTYPE_CHANNEL, &si);
...
STDMETHODIMP CChannelMgr::DownloadMinCDF(HWND hwnd, LPCWSTR pwszURL, LPWSTR pwszTitle, DWORD cchTitle, SUBSCRIPTIONINFO *pSubInfo, BOOL *pfIsSoftware)
{
    HRESULT hr;
    ...
    *pwszTitle = NULL;
    ...
    hr = CoCreateInstance(CLSID_XMLDocument, NULL, CLSCTX_INPROC_SERVER, IID_IXMLDocument, (void**)&pIXMLDocument);
    if (SUCCEEDED(hr))
    {
        ...
        if (fInformUserOfDownloadProblem)
        {
            ...
            CDFMessageBox(hwnd, IDS_INFO_MUST_CONNECT, IDS_INFO_DLG_TITLE, MB_OK | MB_ICONINFORMATION);
...
// Description:
//     Removes the cache and registry settings that wininet uses to map the given url to a local file.
    ...
    )
{
    ...
    EVAL(WritePrivateProfileString( TEXT(".ShellClassInfo"), TEXT("ConfirmFileOp"),  TEXT("0"), szDesktopIni));

    // Write the URL for this category folders webview
    if (pszURL)
    {
        EVAL(WritePrivateProfileString( TEXT(".ShellClassInfo"), TEXT("URL"),  pszURL, szDesktopIni));
    }

    // Write the Logo for this channel if present
    if (pszLogo)
    {
        EVAL(WritePrivateProfileString( TEXT(".ShellClassInfo"), TEXT("Logo"),  pszLogo, szDesktopIni));
    }

    // Write the WideLogo for this channel if present
    if (pszWideLogo)
    {
        EVAL(WritePrivateProfileString( TEXT(".ShellClassInfo"), TEXT("WideLogo"),  pszWideLogo, szDesktopIni));
    }
    ...
    if (pszIcon)
    {
        ...
        EVAL(WritePrivateProfileString( TEXT(".ShellClassInfo"), TEXT("IconIndex"),  szIconIndex, szDesktopIni));
        EVAL(WritePrivateProfileString( TEXT(".ShellClassInfo"), TEXT("IconFile"),  pszIcon, szDesktopIni));
    }
...
HRESULT Channel_GetBasePath(LPTSTR pszPath, int cch)
{
    ...
    {
#ifndef UNIX
        if (RegQueryValueEx(hKey, TEXT("Favorites"), NULL, NULL, (LPBYTE)pszPath, &dwLen) == ERROR_SUCCESS)
        {
            hr = S_OK;
        }
#else
        ...
        dwLen = MAX_PATH;
        if (RegQueryValueEx(hKey, TEXT("Favorites"), NULL, NULL, (LPBYTE)szUnexpandPath, &dwLen) == ERROR_SUCCESS)
        {
            hr = S_OK;
        }
        DWORD length = ExpandEnvironmentStrings((LPTSTR)szUnexpandPath, (LPTSTR)pszPath, cch);
    ...
    if (SUCCEEDED(Channel_GetBasePath(szFavs, ARRAYSIZE(szFavs))))
    {
        switch (xdt)
        {
            case DOC_CHANNEL:
                ...
                if (ERROR_SUCCESS != SHRegGetUSValue(L"Software\\Microsoft\\Windows\\CurrentVersion", L"ChannelFolderName", NULL, (void*)szChannel, &cbChannel, TRUE, NULL, 0))
                {
        ...
        if (!PathFileExists(pszPath))
        {
            // In IE5+ use the favorites folder if the channels folder does not exist.
            StrCpy(pszPath, szFavs);

            // Create special folder with an icon that lives in cdfview.dll
            /*
            Channel_CreateSpecialFolder(pszPath, NULL, NULL, NULL, g_szModuleName, -IDI_CHANNELFOLDER);
            */
        }
...
HRESULT Channel_GetChannelGuide(LPTSTR pszPath, int cch)
{
    ...
    {
        if (RegQueryValueEx(hKey, c_szRegValueChannelGuide, NULL, NULL, (LPBYTE)pszPath, &dwLen) == ERROR_SUCCESS)
        {
            hr = S_OK;
        }
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (bstrURL && *bstrURL)
                    {
                        ...
                        else
                        {
                            ...
                            if (SUCCEEDED(Channel_GetFolder(szChanDir, DOC_CHANNEL)))
                            {
                                WCHAR wszChanDir[MAX_PATH];

                                if (SHTCharToUnicode(szChanDir, wszChanDir, ARRAYSIZE(wszChanDir)))
                                {
                                    if (PathIsPrefixW(wszChanDir, wszURL))
                                    {
                                        if (SUCCEEDED(ShowChannelPane(pIWebBrowser2)))
                                        {
                                            NavigateChannelPane(pIWebBrowser2, wszURL);
                                        }
                                    }
                                }
                            }
                        ...
                        hr = pIWebBrowser2->Navigate2(&vTargetURL, &vNull, &vNull, &vNull, &vNull);
                    }
...
#ifndef UNIX
            hr = CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgr, (void**)&pISubscriptionMgr);
            if (SUCCEEDED(hr))
            {
...
)
{
    ...
    GetPrivateProfileString(TEXT(".ShellClassInfo"), TEXT("CLSID"), TEXT(""),  szFolderGUID,  GUID_STR_LEN, szPath);
    ...
    if (StrEql(szFolderGUID, szCDFViewGUID))
    {
        ...
        GetPrivateProfileString(TEXT("Channel"), TEXT("CDFURL"), TEXT(""),  szCDFURL,  INTERNET_MAX_URL_LENGTH, szPath);
