...
STDMETHODIMP CPersist::SetClientSite(IOleClientSite *pIOleClientSite)
{
    ...
    if (pIOleClientSite && NULL == m_pIWebBrowser2)
    {
        ...
        HRESULT hr = pIOleClientSite->QueryInterface(IID_IOleWindow, (void**)&pIOleWindow);
        if (SUCCEEDED(hr))
        {
        ...
        hr = pIOleClientSite->QueryInterface(IID_IServiceProvider, (void**)&pIServiceProvider);
        if (SUCCEEDED(hr))
        {
            ASSERT(pIServiceProvider);

            IServiceProvider* pIServiceProvider2;
            hr = pIServiceProvider->QueryService(SID_STopLevelBrowser, IID_IServiceProvider, (void**)&pIServiceProvider2);
            if (SUCCEEDED(hr))
            {
                ASSERT(pIServiceProvider2);
                hr = pIServiceProvider2->QueryService(SID_SWebBrowserApp, IID_IWebBrowser2, (void**)&m_pIWebBrowser2);

                // REVIEW: Determine if the current browser is IE

                // New check if the browser is IE.  IE will fail on a QI of
                // IWebBrowserApp for ITargetEmbedding.  Anyone else hosting the browser OC must support this interface.
                IWebBrowserApp* pIWebBrowserApp;
                hr = m_pIWebBrowser2->QueryInterface(IID_IWebBrowserApp, (void**)&pIWebBrowserApp);
                if (SUCCEEDED(hr))
                {
                    ASSERT(pIWebBrowserApp);
                    ITargetEmbedding* pITargetEmbedding;
                    hr = pIWebBrowserApp->QueryInterface(IID_ITargetEmbedding, (void**)&pITargetEmbedding);
                    if (SUCCEEDED(hr))
                    {
...
STDMETHODIMP CPersist::SetHostNames(LPCOLESTR szContainerApp, LPCOLESTR szContainerObj)
{
    return E_NOTIMPL;
}
...
STDMETHODIMP CPersist::GetMoniker(DWORD dwAssign, DWORD dwWhichMoniker, IMoniker **ppmk)
{
    return E_NOTIMPL;
}

STDMETHODIMP CPersist::InitFromData(IDataObject *pDataObject, BOOL fCreation, DWORD dwReserved)
{
    return E_NOTIMPL;
}
...
STDMETHODIMP CPersist::DoVerb(LONG iVerb, LPMSG lpmsg, IOleClientSite *pActiveSite, LONG lindex, HWND hwndParent, LPCRECT lprcPosRect)
{
    HRESULT hr;
    ...
    if (OLEIVERB_PRIMARY == iVerb)
    {
        ...
        case DOC_SOFTWAREUPDATE:
                ...
                (CountChannels() < SHRestricted2W(REST_MaxChannelCount, NULL, 0))))
            {
                ...
                // In case the SELF tag is different than the URL get the subscribed URL from SubscriptionHelper.
                BSTR bstrSubscribedURL = NULL;
                if (SubscriptionHelper(m_pIXMLDocument, m_hwnd, SUBSTYPE_CHANNEL, SUBSACTION_ADDADDITIONALCOMPONENTS, m_polestrURL, xdt, &bstrSubscribedURL))
                {
                    TCHAR szSubscribedURL[INTERNET_MAX_URL_LENGTH];

                    if (bstrSubscribedURL && SHUnicodeToTChar(bstrSubscribedURL, szSubscribedURL, ARRAYSIZE(szSubscribedURL)))
                    {
                        FILETIME ftLastMod;
                        URLGetLastModTime(szSubscribedURL, &ftLastMod);

                        Cache_AddItem(szSubscribedURL, m_pIXMLDocument, PARSE_NET, ftLastMod, g_dwCacheCount);
                    }
        ...
        case DOC_DESKTOPCOMPONENT:
...
#endif /* UNIX */
            {
                ...
                MLLoadString(IDS_BROWSERONLY_DLG_TEXT,  szText, ARRAYSIZE(szText));
                MLLoadString(IDS_BROWSERONLY_DLG_TITLE, szTitle, ARRAYSIZE(szTitle));
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            if (SUCCEEDED(hr))
                            {
                                ...
                                // If the desktop component url is already in cache, we want to refresh right away - otherwise not
                                if(!(CDFIDL_IsCachedURL(Info.wszSubscribedURL)))
                                {
                                    //It is not in cache, we want to wait until the download is done before refresh. So don't refresh right away
...
HRESULT CPersist::OpenChannel(LPCWSTR pszSubscribedURL)
{
    ASSERT(m_pIXMLDocument);
    ...
    {
        hr = CoCreateInstance(CLSID_InternetExplorer, NULL, CLSCTX_LOCAL_SERVER, IID_IWebBrowser2, (void**)&m_pIWebBrowser2);
        if (FAILED(hr))
        {
            return hr;
        }
    ...
    hr = XML_GetFirstChannelElement(m_pIXMLDocument, &pIXMLElement, &nIndex);
    if (SUCCEEDED(hr))
    {
        ASSERT(pIXMLElement);
        ...
        if (bstrURL)
        {
            ...
            if (*bstrURL != 0)
            {
                HWND hwnd = CreateNavigationWorkerWindow(m_hwnd, m_pIWebBrowser2);
                if (hwnd)
                {
                    LPOLESTR pszPath = Channel_GetChannelPanePath(pszSubscribedURL);
                    if (pszPath)
...
HWND CPersist::CreateNavigationWorkerWindow(HWND hwndParent, IWebBrowser2* pIWebBrowser2)
{
    ...
    hwnd = CreateWindow(TEXT("NavigationWorker"), NULL, WS_CHILD, 0, 0, 0, 0, hwndParent, (HMENU)0xED, g_hinst, NULL);
    if (hwnd) {
...
LRESULT CALLBACK NavigateWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    LRESULT         lRet = 0;
    ...
    case WM_NAVIGATE:
        ...
        if (pIWebBrowser2)
        {
            ...
            pIWebBrowser2->Navigate2(&varURL, &varNull, &varNull, &varNull, &varNull);
        }
