...
//     Retrieves the class factory object for the cdf viewer.
{
    ...
    *ppvObj = NULL;
    ...
    for (int i = 0; i < ARRAYSIZE(aClassFact); i++)
    {
        ...
        {
            CCdfClassFactory *pCdfClassFactory = new CCdfClassFactory(aClassFact[i].pfn);
            if (pCdfClassFactory)
            {
...
//     Self register the cdf viewer.
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        HRESULT hr2 = CoCreateInstance(CLSID_StdComponentCategoriesMgr, NULL, CLSCTX_INPROC_SERVER, IID_ICatRegister, (void**)&pICatRegister);
        if (SUCCEEDED(hr2))
        {
            ...
            pICatRegister->RegisterClassImplCategories(CLSID_CDFVIEW, 1, acatid);
