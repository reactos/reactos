...
CBindStatusCallback::CBindStatusCallback (IXMLDocument* pIXMLDocument, LPCWSTR pszURLW) : m_cRef(1)
{
...
CBindStatusCallback::~CBindStatusCallback (void)
{
    ASSERT(0 == m_cRef);
...
STDMETHODIMP_(ULONG) CBindStatusCallback::AddRef (void)
{
    ASSERT(m_cRef != 0);
...
STDMETHODIMP_(ULONG) CBindStatusCallback::Release (void)
{
    ASSERT (m_cRef != 0);
...
STDMETHODIMP CBindStatusCallback::OnStartBinding(DWORD dwReserved, IBinding* pIBinding)
{
    return S_OK;
}
...
)
{
    ...
    if (m_pPrevIBindStatusCallback)
    {
        hr = m_pPrevIBindStatusCallback->OnProgress(ulProgress, ulProgressMax, ulStatusCode, szStatusText);
    }
    else
    {
        hr = S_OK;
...
STDMETHODIMP CBindStatusCallback::OnObjectAvailable(REFIID riid, IUnknown* pIUnknown)
{
    return S_OK;
}
...
STDMETHODIMP CBindStatusCallback::OnStopBinding(HRESULT hrStatus, LPCWSTR szStatusText)
{
    if (m_pPrevIBindStatusCallback)
...
HRESULT CBindStatusCallback::Init(IBindStatusCallback* pPrevIBindStatusCallback)
{
...
CBindStatusCallback2::CBindStatusCallback2 (HWND hwnd) : m_cRef(1), m_hwnd(hwnd)
{
    DllAddRef();
}


CBindStatusCallback2::~CBindStatusCallback2 (void)
{
    ASSERT(0 == m_cRef);
...
STDMETHODIMP_(ULONG) CBindStatusCallback2::AddRef (void)
{
    ASSERT(m_cRef != 0);
    ASSERT(m_cRef < (ULONG)-1);
...
STDMETHODIMP_(ULONG) CBindStatusCallback2::Release (void)
{
    ASSERT (m_cRef != 0);
...
STDMETHODIMP CBindStatusCallback2::OnStartBinding(DWORD dwReserved, IBinding* pIBinding)
{
    return S_OK;
}
...
{
    PostMessage(m_hwnd, WM_COMMAND, DOWNLOAD_PROGRESS, 0);

    return S_OK;
}


STDMETHODIMP CBindStatusCallback2::OnDataAvailable(DWORD grfBSCF, DWORD dwSize, FORMATETC* pfmtect, STGMEDIUM* pstgmed)
{
    return S_OK;
}
...
STDMETHODIMP CBindStatusCallback2::OnObjectAvailable(REFIID riid, IUnknown* pIUnknown)
{
    return S_OK;
}
...
STDMETHODIMP CBindStatusCallback2::OnStopBinding(HRESULT hrStatus, LPCWSTR szStatusText)
{
    HRESULT hr = hrStatus;

    PostMessage(m_hwnd, WM_COMMAND, DOWNLOAD_COMPLETE, SUCCEEDED(hr) ? TRUE : FALSE);
