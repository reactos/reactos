...
BOOL WINAPI IsPTsrvKnownFileType(LPCTSTR pszPath, OUT OPTIONAL PTSRV_FILETYPE* pType, OUT OPTIONAL PTSRV_FILECLASS* pClass )
{
    BOOL    bRet = FALSE ;
...
public:
    ...
    BOOL operator == ( const tagPROPFILEEXT& other ) const   {
        return  _bAdvanced == other._bAdvanced && 0 == lstrcmpi( _szExt, other._szExt ) ;
    }
    ULONG Hash() const    {
...
STDMETHODIMP GetPropServerClassForFile(IN LPCTSTR pszPath, BOOL bAdvanced, OUT LPCLSID pclsid )
{
    HRESULT hr = E_FAIL;
    LPCTSTR pszExt = NULL;
