...
void CPage0::PersistMode()
{
    ...
    {
        if( RegCreateKeyEx( HKEY_CURRENT_USER, c_szEXTENSION_SETTINGS_REGKEY, 0L, NULL, 0L, KEY_WRITE, NULL, &hKey, &dwDisposition ) == ERROR_SUCCESS )
        {
            ...
            RegSetValueEx( hKey, c_szUIMODE_REGVAL, 0L, REG_DWORD, (LPBYTE)&dwAdvanced, sizeof(dwAdvanced) );
            RegCloseKey( hKey );
        }
    }
}
...
BOOL CPage0::RecallMode()
{
    ...
    if( RegOpenKeyEx( HKEY_CURRENT_USER, c_szEXTENSION_SETTINGS_REGKEY, 0L, KEY_READ, &hKey ) == ERROR_SUCCESS )
    {
        DWORD dwAdvanced = FALSE, cbVal = sizeof(dwAdvanced), dwType, dwErr;

        RegQueryValueEx( hKey, c_szUIMODE_REGVAL, NULL, &dwType, (LPBYTE)&dwAdvanced, &cbVal );
