...
public:
    ...
    ULONG   Hash() const ;//  Hashing methods
    void    Clear() ;//  Release resources and reinitialize.
...
inline BOOL CPropertyUID::operator==( const struct tagPUID& other ) const  {
    return propid == other.propid && memcmp( &fmtid, &other.fmtid, sizeof(fmtid) )==0 ;
}
inline BOOL CPropertyUID::operator!=( const struct tagPUID& other ) const  {
