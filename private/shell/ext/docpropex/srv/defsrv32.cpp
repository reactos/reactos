...
BOOL GetBasicServerForFile( LPCWSTR pwszFile, IBasicPropertyServer** ppbps )
{
    ...
    *ppbps = NULL;
    ...
    {
        if( SUCCEEDED(CoCreateInstance( clsid, NULL, CLSCTX_INPROC_SERVER, IID_IBasicPropertyServer, (void**)ppbps )) )
            return TRUE;
    }
    return FALSE;
}
    ...
    *pppts = NULL;
    ...
    if( pszExt )
    {
        ...
        if( SUCCEEDED( (hr = GetPropServerClassForFile( szPath, TRUE, &clsid )) ) )
        {
            ...
            if( FAILED( (hr = CoCreateInstance( clsid, NULL, CLSCTX_INPROC_SERVER, IID_IAdvancedPropertyServer, (void**)pppts )) ) )
                return hr;
