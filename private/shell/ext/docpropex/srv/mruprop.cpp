...
HRESULT CPropMruStor::PushMruVal( REFFMTID fmtid, PROPID propid, VARTYPE vt, IN const PROPVARIANT& var )
{
    ...
    hr = UpdateIndex() ;//  Persist the index
    return hr ;
}
...
HRESULT CPropMruStor::LoadIndex( PVOID pvDest )
{
    ...
    dwRet = RegQueryValueEx( m_hKey, szINDEXVALNAME, 0L, &dwType, (LPBYTE)pvDest, &cbDest ) ;
...
{
    ASSERT( IsOpen() ) ;
    DWORD dwRet = RegSetValueEx( m_hKey, szINDEXVALNAME, 0L, REG_BINARY, (LPBYTE)m_index, sizeof(m_index) ) ;
...
HRESULT CPropMruStor::Open( REGSAM sam )
{
    ...
    dwRet = RegCreateKeyEx( m_hKeyRoot, m_pszRecSubKey, 0L, NULL, REG_OPTION_NON_VOLATILE, sam, NULL, &hKey, &dwDisp ) ;
    if( S_OK == (hr = HRESULT_FROM_WIN32( dwRet )) )
    {
        Close() ;
        m_hKey = hKey ;
