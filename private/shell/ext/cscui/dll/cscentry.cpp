...
: m_hdpa(NULL), m_hkRoot(NULL)
{
    ...
    RegCreateKeyEx(hkRoot, pszSubkey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ENUMERATE_SUB_KEYS | KEY_CREATE_SUB_KEY, NULL, &m_hkRoot, &dwDisp);
    m_hdpa = DPA_Create(8);
    ReadRegKeys();
    TraceLeave();
}
...
HKEY CSCEntryLog::OpenKeyInternal(LPTSTR pszSubkey, REGSAM samDesired)
{
    ...
    RegCreateKeyEx(m_hkRoot, pszSubkey, 0, NULL, REG_OPTION_NON_VOLATILE, samDesired, NULL, &hkEntry, &dwDisp);
...
CSCEntry* CSCEntryLog::CreateFromKey(LPTSTR pszSubkey)
{
    ...
    if (hkEntry)
    {
        ...
        if (ERROR_SUCCESS != RegQueryValueEx(hkEntry, c_szEntryID, NULL, NULL, (LPBYTE)&guid, &dwSize))
        {
            // Doesn't exist, create a new GUID
            CoCreateGuid(&guid);
            RegSetValueEx(hkEntry, c_szEntryID, 0, REG_BINARY, (LPBYTE)&guid, sizeof(guid));
        }
