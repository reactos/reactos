...
DWORD Security_SetPrivilegeAttrib(LPCTSTR PrivilegeName, DWORD NewPrivilegeAttribute, DWORD *OldPrivilegeAttribute)
{
    LUID             PrivilegeValue;
...
HRESULT GetCurrentUserSid(PSID *ppsid)
{
    HRESULT hr  = NOERROR;
    DWORD dwErr = 0;
    ...
    *ppsid = NULL;
    CWin32Handle hToken;
    if (!OpenThreadToken(GetCurrentThread(), TOKEN_READ, TRUE, hToken.HandlePtr()))
    {
        if (ERROR_NO_TOKEN == GetLastError())
        {
            if (!OpenProcessToken(GetCurrentProcess(), TOKEN_READ, hToken.HandlePtr()))
            {
                dwErr = GetLastError();
    ...
    DWORD cbTokenInfo = 0;
    cbTokenInfo = 0;
    if (!GetTokenInformation(hToken, TokenUser, NULL, cbTokenInfo, &cbTokenInfo))
    {
        dwErr = GetLastError();
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (!GetTokenInformation(hToken, TokenUser, pbTokenInfo, cbTokenInfo, &cbTokenInfo))
        {
            dwErr = GetLastError();
