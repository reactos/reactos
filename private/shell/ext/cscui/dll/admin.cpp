...
void ApplyAdminFolderPolicy(void)
{
    DWORD dwThreadId;
...
BOOL ExistsAPF(HDPA hdpa, LPCTSTR pszPath)
{
...
BOOL ReadAPFFromRegistry(HDPA hdpaFiles)
{
...
void _DoAdminPin(LPCTSTR pszItem)
{
    BOOL bNewItem = FALSE;
    DWORD dwPinCount = 0;
    DWORD dwHintFlags = 0;
    ...
    CSCQueryFileStatus(pszItem, NULL, &dwPinCount, &dwHintFlags);// This may fail, for example if the file is not in the cache
    bNewItem = !(dwHintFlags & FLAG_CSC_HINT_PIN_ADMIN);// Is the admin flag already turned on?
    dwHintFlags |= FLAG_CSC_HINT_COMMAND_ALTER_PIN_COUNT | FLAG_CSC_HINT_PIN_ADMIN;// Turn on the admin flag
    CSCPinFile(pszItem, dwHintFlags, NULL, &dwPinCount, &dwHintFlags);// Pin the item

    // If this item was not previously admin pinned, increment the
    // pin count a second time.  This lets us eliminate items that
    // are no longer in the admin pin list when we decrement pin counts later.
    if (bNewItem)
    {
        CSCPinFile(pszItem, dwHintFlags, NULL, &dwPinCount, &dwHintFlags);
    }
...
void _PinLinkTarget(LPCTSTR pszLink)
{
                          ...
                          LPARAM              /*dwContext*/)
{
    ...
    // items that were admin pinned but are no longer part of the admin pin set.
    if ((eReason == ENUM_REASON_FILE || eReason == ENUM_REASON_FOLDER_BEGIN) && (dwHintFlags & FLAG_CSC_HINT_PIN_ADMIN))
    {
        dwHintFlags = 0;
        ...
        CSCUnpinFile(pszItem, dwHintFlags, &dwStatus, &dwPinCount, &dwHintFlags);
    ...
    else if (ENUM_REASON_FOLDER_END == eReason)
    {
        ...
        // Note that dwPinCount and dwHintFlags are always 0 in the post-order part of the traversal, so fetch them here.
    ...
    if (bDeleteItem && 0 == dwPinCount && 0 == dwHintFlags && !(dwStatus & FLAG_CSCUI_COPY_STATUS_LOCALLY_DIRTY))
    {
        CscDelete(pszItem);
    }
...
DWORD WINAPI _PinAdminFoldersThread(LPVOID)
{
    ...
    // Wait until we either own the "admin pin" mutex OR the "terminate" event is set.
    TraceMsg("Waiting for 'admin-pin' mutex or 'terminate' event...");
    DWORD dwWait = WaitForMultipleObjects(ARRAYSIZE(rghSyncObj), rghSyncObj, FALSE, INFINITE);
    if (1 == (dwWait - WAIT_OBJECT_0))
    {
        ...
        if (NULL != hdpaFiles)
        {
            ...
            for (int i = 0; i < cFiles; i++)
            {
                ...
                if (CSCPROC_RETURN_CONTINUE == dwResult && PathIsUNC(pszItem))
                {
...
HRESULT _ExpandStringInPlace(LPTSTR psz, DWORD cch)
{
...
// Version of RegEnumValue that expands environment variables in all string values.
LONG _RegEnumValueExp(HKEY hKey, DWORD dwIndex, LPTSTR lpValueName, LPDWORD lpcbValueName, LPDWORD lpReserved, LPDWORD lpType, LPBYTE lpData, LPDWORD lpcbData)
{
    ...
    LONG lResult = RegEnumValue(hKey, dwIndex, lpValueName, lpcbValueName, lpReserved, lpType, lpData, lpcbData);
