...
CStateMachine::PingServers()
{
    ...
    if (!m_bNoNet)
    {
        ...
        HANDLE hThread = CreateThread(NULL, 0, _PingServersThread, NULL, 0, &dwThreadID);
        if (hThread)
        {
            CloseHandle(hThread);
        }
    ...
    if (state != m_state)
    {
        ...
        if (!::IsSyncInProgress())
        {
            if (0 == m_idStateChangeTimer)
            {
                ...
                m_stateQueued = STS_INVALID;
                m_idStateChangeTimer = SetTimer(m_hwndNotify, ID_TIMER_STATECHANGE, s_iMinStateChangeInterval, StateChangeTimerProc);
                bResult  = true;
            }
            else
            {
    ...
    if (UF_FLASHICON & uFlags)
    {
        if (0 != sti.iFlashTimeout)
        {
            ...
            if (0 == m_idFlashingTimer)
            {
                ...
                m_idFlashingTimer = SetTimer(m_hwndNotify,  ID_TIMER_FLASHICON,  m_iIconFlashTime, FlashTimerProc);
                if (0 != m_idFlashingTimer)
                {
    ...
    *pszTextBody = TEXT('\0');
    ...
    if (-1 != i)
    {
        ...
        if (STS_OFFLINE == state || STS_DIRTY == state || STS_SERVERBACK == state)
        {
            ...
            FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY, szFmt, 0,0, szBody, ARRAYSIZE(szBody), (va_list *)rgpstr);
        }
        else
        {
    ...
    for (int i = 0; i < ARRAYSIZE(s_rgTooltipInfo); i++)
    {
        ...
        if (tti.state == state && 0 != tti.idTooltip)
        {
            ...
            if (STS_OFFLINE == state || STS_DIRTY == state || STS_SERVERBACK == state)
            {
                ...
                FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY, szFmt, 0,0, szTemp + cchHeader, ARRAYSIZE(szTemp) - cchHeader, (va_list *)rgpstr);
            }
            else
            {
    ...
    )
{
    ...
    if (!config.NoReminders())
    {
        ...
        if (bRestart && 0 < cReminderInterval)
        {
            ...
            m_idReminderTimer = SetTimer(m_hwndNotify, ID_TIMER_REMINDER, cReminderInterval, ReminderTimerProc);
        }
    }
}
            ...
            if (STS_DIRTY == state || STS_OFFLINE == state || STS_SERVERBACK == state)
            {
                ...
                if (NULL != pszServerName && TEXT('\0') != *pszServerName)
                {
                    ...
                    if (NULL != pszTemp)
                    {
                        ...
                        FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY, pszTemp, 0,0, szText, ARRAYSIZE(szText), (va_list *)rgpstr);
                        delete[] pszTemp;
                    }
                }
            }
    ...
    )
{
    ...
    if (ERROR_SUCCESS == lResult)
    {
        ...
        if (ERROR_SUCCESS == lResult)
        {
            ...
            while(ERROR_SUCCESS == SHEnumValue(hkey, iValue, szValue, &cchValue, NULL, NULL, NULL))
            {
...
{
    DWORD dwDisposition;
    return RegCreateKeyEx(HKEY_LOCAL_MACHINE, REGSTR_KEY_OFFLINEFILES, 0, NULL, 0, dwAccess, NULL, phkey, &dwDisposition);
}
