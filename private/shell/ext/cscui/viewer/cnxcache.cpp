...
CnxNameCache::CnxNameCache(void) throw() : m_prgCnx(NULL)
{
    DBGTRACE((TEXT("CnxNameCache::CnxNameCache")));
}


CnxNameCache::~CnxNameCache(void)
{
    DBGTRACE((TEXT("CnxNameCache::~CnxNameCache")));
    delete [] m_prgCnx;
}
...
void CnxNameCache::Refresh(void) throw()
{
    delete [] m_prgCnx;
    m_prgCnx = NULL;
}

void CnxNameCache::Load(void)
{
...
TCHAR CnxNameCache::GetShareDriveLetter(const CString& strShare) const
{
    if (!Loaded())
...
bool CnxNameCache::GetShareFromDriveLetter(TCHAR chDrive, CString *pstrShare) const
{
