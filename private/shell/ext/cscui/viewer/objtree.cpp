    ...
    )
{
    ...
    if (bPin != IsPinned())
    {
        ...
        GetFullPath(&strPath);
        if (rgPinFunc[bPin](strPath, dwHintFlags, &dwStatus, &dwPinCount, &dwHintFlags))
        {
    ...
    )
{
    ...
    CscFindHandle hFind(CSCFindFirstFile(strPath, &fd, &dwStatus, &dwPinCount, &dwHintFlags, &ft));
...
bool CscObjTreeIterator::Next(CscObject **ppObject)
{
    DBGASSERT((NULL != ppObject));
