    ...
    ) const
{
    ...
    dwResult = RegOpenKeyEx(hkeyRoot, (LPCTSTR)m_strSubKey, 0, samDesired, &m_hkey);
    if ((ERROR_FILE_NOT_FOUND == dwResult) && bCreate)
    {
        DWORD dwDisposition;
        dwResult = RegCreateKeyEx(hkeyRoot, (LPCTSTR)m_strSubKey, 0, NULL, 0, samDesired, NULL, &m_hkey, &dwDisposition);
    }
    if (bCloseRootKey)
    {
...
{
    DWORD dwResult = RegSetValueEx(m_hkey, pszValueName, 0, dwValueType, pbData, cbData);
    return HRESULT_FROM_WIN32(dwResult);
}
