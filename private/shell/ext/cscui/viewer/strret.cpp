...
/*  File: strret.cpp
    ...
    Description: Class to handle STRRET objects used in returning strings from the Windows Shell.
        Main function is to properly clean up any dynamic storage.
...
StrRet::StrRet(LPITEMIDLIST pidl, UINT type) : m_pMalloc(NULL)
{
    uType = type;
    pOleStr = NULL;
...
VOID StrRet::FreeOleStr(VOID)
{
    if (NULL != m_pMalloc && STRRET_WSTR == uType && NULL != pOleStr)
    {
...
VOID StrRet::CopyOleStr(LPCWSTR pszOle)
{
    FreeOleStr();
...
LPTSTR StrRet::GetString(LPTSTR pszStr, INT cchStr) const
{
    switch(uType)
    {
        case STRRET_WSTR:
