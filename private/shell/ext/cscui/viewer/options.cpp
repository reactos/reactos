    ...
    )
{
    ...
    if (PURGE_FLAG_NONE != sel.Flags())
    {
        ...
        if (SUCCEEDED(coinit.Result()))
        {
            ...
            if (SUCCEEDED(CoCreateInstance(CLSID_ProgressDialog, NULL, CLSCTX_INPROC_SERVER, IID_IProgressDialog, (void **)ptrpd.getaddr())))
            {
                // Set up the progress dialog using the standard shell "file delete"
                // animation (the one without the recycle bin).  The dialog is modal.
    ...
    )
{
    ...
    try
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = CoCreateInstance(CLSID_OfflineFilesOptions, NULL, CLSCTX_INPROC_SERVER, IID_IShellExtInit, reinterpret_cast<void **>(psei.getaddr()));
            if (SUCCEEDED(hr))
            {
...
STDAPI COfflineFilesOptions_CreateInstance(REFIID riid, void **ppv)
{
    ...
    {
        // Create the property sheet extension to handle the CSC options property pages.
...
// This loads all of the user preference/policy information for the page when the page is first created.
void COfflineFilesPage::CConfigItems::Load(void)
{
