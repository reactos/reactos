...
HRESULT ConvertScheduleItem(CHAR *pszSubsName)
{
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER, szKeyName, 0, KEY_READ, &hkey) == ERROR_SUCCESS)
    {
        DWORD dwType;
        DWORD dwSize;
        ...
        if ((RegQueryValueExA(hkey, pszSubsName, NULL, &dwType, NULL, &dwSize) == ERROR_SUCCESS) && (dwType == REG_BINARY) && (dwSize >= sizeof(NOTFSUBS)))
        {
...
HRESULT ConvertNotfMgrSubscriptions()
{
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER, c_pszRegKeyScheduleItems, 0, KEY_READ, &hkey) == ERROR_SUCCESS)
    {
        int i = 0;
