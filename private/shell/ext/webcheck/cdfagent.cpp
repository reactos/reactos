...
HRESULT CRunDeliveryAgent::StartAgent()
{
    ...
    hr = CoCreateInstance(m_clsidDest, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionAgentControl, (void **)&m_pAgent);
    if (m_pAgent)
    {
...
HRESULT CRunDeliveryAgent::CreateNewItem(ISubscriptionItem **ppItem, REFCLSID rclsidAgent)
{
    ...
    *ppItem = NULL;
    ...
    CoCreateInstance(CLSID_SubscriptionMgr, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionMgrPriv, (void**)&pSubsMgrPriv);
    if (pSubsMgrPriv)
    {
...
HRESULT CChannelAgent::OnDownloadComplete(UINT iID, int iError)
{
    ...
    if (SUCCEEDED(hr) && pStm)
    {
        ...
        CoCreateInstance(CLSID_XMLDocument, NULL, CLSCTX_INPROC, IID_IPersistStreamInit, (void **)&pPersistStm);
...
HRESULT CChannelAgent::OnChildDone(CProcessElement *pChild, HRESULT hr)
{
    ...
    if (SUCCEEDED(GetEndStatus()))
    {
        ...
        HRESULT hrTmp = CoCreateInstance(CLSID_ChannelMgr, NULL, CLSCTX_INPROC_SERVER, IID_IChannelMgrPriv2, (void**)&pChannelMgrPriv2);
        if (SUCCEEDED(hrTmp))
        {
...
STDMETHODIMP CChannelAgent::GetIconLocation(UINT uFlags, LPTSTR szIconFile, UINT cchMax, int * piIndex, UINT * pwFlags)
{
    ...
    if (SUCCEEDED(hr) && pIChannelMgrPriv)
    {
        ...
        hr = CoCreateInstance(CLSID_CDFICONHANDLER, NULL, CLSCTX_INPROC_SERVER, IID_IPersistFile, (void**)&ppf);
