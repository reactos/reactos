...
)
{
    ...
    switch (pTT->TriggerType)
    {
        case TASK_TIME_TRIGGER_DAILY:
        {
            ...
            else
            {
                ...
                wnsprintf(szLineOut, ARRAYSIZE(szLineOut), szFormat, pTT->Type.Daily.DaysInterval);
            }
        ...
        case TASK_TIME_TRIGGER_WEEKLY:
        {
            ...
            if (FAILED(hrResult = GetDaysOfWeekString(  pTT->Type.Weekly.rgfDaysOfTheWeek, szDOW, ARRAYSIZE(szDOW))))
            {
                return hrResult;
            }
            ...
            else
            {
                ...
                if (!FormatMessage( FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY, szFormat, 0, 0, szLineOut, ARRAYSIZE(szLineOut), (va_list*)rgpsz))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }
        ...
        case TASK_TIME_TRIGGER_MONTHLYDATE:
        {
            if (pTT->Type.MonthlyDate.rgfMonths == TASK_ALLMONTHS)
            {
                ...
                if (!MLLoadString(IDS_SUMMARY_EVERY_MONTHLYDATE_FORMAT, szFormat, ARRAYSIZE(szFormat)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }

                wnsprintf(szLineOut, ARRAYSIZE(szLineOut), szFormat, DayFromDaysFlags(pTT->Type.MonthlyDate.rgfDays));
            }
            else
            {
                // Monthly -- on day %d of %s
                if (!MLLoadString(IDS_SUMMARY_MONTHLYDATE_FORMAT, szFormat, ARRAYSIZE(szFormat)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }
                ...
                TCHAR szMonths[128];
                if (FAILED(hrResult = GetMonthsString(  pTT->Type.MonthlyDate.rgfMonths, szMonths, ARRAYSIZE(szMonths))))
                {
                    return hrResult;
                }

                TCHAR szDay[16];
                wnsprintf(szDay, ARRAYSIZE(szDay), "%d", DayFromDaysFlags(pTT->Type.MonthlyDate.rgfDays));
        ...
        case TASK_TIME_TRIGGER_MONTHLYDOW:
        {
            ...
            TCHAR szWeek[32];
            if (!MLLoadString(IDS_MONTHLY_LIST1_FIRST + pTT->Type.MonthlyDOW.wWhichWeek - 1, szWeek, ARRAYSIZE(szWeek)))
            {
                hrResult = HRESULT_FROM_WIN32(GetLastError());
                return hrResult;
            }
            ...
            TCHAR szDOW[128];
            if (FAILED(hrResult = GetDaysOfWeekString(  pTT->Type.MonthlyDOW.rgfDaysOfTheWeek, szDOW, ARRAYSIZE(szDOW))))
            {
                return hrResult;
            }
            ...
            if (pTT->Type.MonthlyDate.rgfMonths == TASK_ALLMONTHS)
            {
                ...
                if (!MLLoadString(IDS_SUMMARY_EVERY_MONTHLYDOW_FORMAT, szFormat, ARRAYSIZE(szFormat)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }
            ...
            {
                TCHAR szMonths[128];
                if (FAILED(hrResult = GetMonthsString(  pTT->Type.MonthlyDOW.rgfMonths, szMonths, ARRAYSIZE(szMonths))))
                {
                    return hrResult;
                }

                // Monthly -- on the %s %s of %s
                if (!MLLoadString(IDS_SUMMARY_MONTHLYDOW_FORMAT, szFormat, ARRAYSIZE(szFormat)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }
    ...
    EVAL(GetTimeFormat( LOCALE_USER_DEFAULT, TIME_NOSECONDS, &st, NULL, szStartTime, ARRAYSIZE(szStartTime)) > 0);

    // Handle the 'Time' settings.
    if (pTT->MinutesInterval == 0)
    {
        // at %s
        if (!MLLoadString(IDS_SUMMARY_ONCE_DAY_FORMAT, szFormat, ARRAYSIZE(szFormat)))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    else
    {
        if ((pTT->MinutesInterval % 60) == 0)   //repeating on the hour
        {
            ...
            {
                // every hour
                if (!MLLoadString(IDS_SUMMARY_REPEAT_EVERY_HOUR, szLineOut, ARRAYSIZE(szLineOut)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }
            ...
            {
                // every %d hours
                if (!MLLoadString(IDS_SUMMARY_REPEAT_HOURLY_FORMAT, szFormat, ARRAYSIZE(szFormat)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }

                wnsprintf(szLineOut, ARRAYSIZE(szLineOut), szFormat, (pTT->MinutesInterval / 60));
            }
        }
        else        //not integral hours, use minutes
        {
            ...
            {
                // every minute
                if (!MLLoadString(IDS_SUMMARY_REPEAT_EVERY_MIN, szLineOut, ARRAYSIZE(szLineOut)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }
            ...
            {
                // every %d minutes
                if (!MLLoadString(IDS_SUMMARY_REPEAT_MINUTELY_FORMAT, szFormat, ARRAYSIZE(szFormat)))
                {
                    hrResult = HRESULT_FROM_WIN32(GetLastError());
                    return hrResult;
                }

                wnsprintf(szLineOut, ARRAYSIZE(szLineOut), szFormat, pTT->MinutesInterval);
            }
        }
        ...
        {
            if (!MLLoadString(IDS_SUMMARY_REPEAT_BETWEEN_FORMAT, szFormat, ARRAYSIZE(szFormat)))
            {
                hrResult = HRESULT_FROM_WIN32(GetLastError());
                return hrResult;
            }

            DWORD dwEndTime = IncTime(  MAKELONG(pTT->wStartHour, pTT->wStartMinute), (pTT->MinutesDuration / 60), (pTT->MinutesDuration % 60));
            ...
            EVAL(GetTimeFormat( LOCALE_USER_DEFAULT, TIME_NOSECONDS, &st, NULL, szEndTime, ARRAYSIZE(szEndTime)) > 0);
...
HRESULT GetDaysOfWeekString(WORD rgfDaysOfTheWeek, LPTSTR  ptszBuf, UINT cchBuf)
{
    ...
    *ptszBuf = TEXT('\0');
    ...
    {
        if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME1, ptszBuf, cchBuf))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    if (rgfDaysOfTheWeek & TASK_TUESDAY)
    {
        ...
        cch = lstrlen(ptszBuf);
        if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME2, ptszBuf + cch, cchBuf - cch))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    if (rgfDaysOfTheWeek & TASK_WEDNESDAY)
    {
        ...
        cch = lstrlen(ptszBuf);
        if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME3, ptszBuf + cch, cchBuf - cch))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    if (rgfDaysOfTheWeek & TASK_THURSDAY)
    {
        ...
        cch = lstrlen(ptszBuf);
        if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME4, ptszBuf + cch, cchBuf - cch))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    if (rgfDaysOfTheWeek & TASK_FRIDAY)
    {
        ...
        cch = lstrlen(ptszBuf);
        if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME5, ptszBuf + cch, cchBuf - cch))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    if (rgfDaysOfTheWeek & TASK_SATURDAY)
    {
        ...
        cch = lstrlen(ptszBuf);
        if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME6, ptszBuf + cch, cchBuf - cch))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    if (rgfDaysOfTheWeek & TASK_SUNDAY)
    {
        ...
        cch = lstrlen(ptszBuf);
        if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME7, ptszBuf + cch, cchBuf - cch))
        {
            hrResult = HRESULT_FROM_WIN32(GetLastError());
            return hrResult;
        }
    ...
    *ptszBuf = TEXT('\0');
    ...
    for (WORD i = 0; i < 12; i++)
    {
        if ((rgfMonths >> i) & 0x1)
        {
            ...
            cch = lstrlen(ptszBuf);
            if (!GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVMONTHNAME1 + i, ptszBuf + cch, cchBuf - cch))
            {
                hr = HRESULT_FROM_WIN32(GetLastError());
                return hr;
            }
...
//xnotfmgr
    ...
    for (;;)
    {
        ...
        if  ((pGroupCookie == NULL) || (pdwFlags == NULL))
        {
            return E_INVALIDARG;
        }
        ...
        {
            EVAL((ssuiDlgInfo.pfnGetRunTimes = (GRTFUNCTION)GetProcAddress( ssuiDlgInfo.hinstURLMON, TEXT("GetRunTimes"))) != NULL);
        }
        else
        ...
        else
        {
            ...
            if (FAILED(hrResult = GetSchedGroupTaskTrigger( ssuiDlgInfo.pGroupCookie, &ssuiDlgInfo.ttTaskTrigger)))
            {
        ...
        ReadRegValue(   HKEY_CURRENT_USER, WEBCHECK_REGKEY, s_szRepeatHrsAreMins, &ssuiDlgInfo.dwRepeatHrsAreMins, SIZEOF(&ssuiDlgInfo.dwRepeatHrsAreMins));
        ...
        // Show the dialog.
        int iResult;
        if ((iResult = DialogBoxParam(  MLGetHinst(), MAKEINTRESOURCE(IDD_CUSTOM_SCHEDULE), hwndParent, ShowScheduleUIDlgProc, (LPARAM)&ssuiDlgInfo)) < 0)
        {
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            SetWindowPos(   hwndDlg,
                            ...
                            0,
                            0,
                            rectCancel.right + (GetSystemMetrics(SM_CXDLGFRAME) * 2) + CX_DIALOG_GUTTER,
                            rectCancel.bottom + (GetSystemMetrics(SM_CYDLGFRAME) * 2) + GetSystemMetrics(SM_CYCAPTION) + CY_DIALOG_GUTTER,
        ...
        case WM_HELP:
        {
            MLWinHelpWrap((HWND)((LPHELPINFO) lParam)->hItemHandle, c_szHelpFile, HELP_WM_HELP, (DWORD)aCustomDlgHelpIDs);
            break;
        }
        case WM_CONTEXTMENU:
        {
            MLWinHelpWrap((HWND)wParam, c_szHelpFile, HELP_CONTEXTMENU, (DWORD)aCustomDlgHelpIDs);
            break;
        }
        case WM_COMMAND:
        {
            ...
            switch (LOWORD(wParam))
            {
                case IDOK:
                {
                    if  (pDlgInfo->bDataChanged && FAILED(ApplyDlgChanges(hwndDlg, pDlgInfo)))
                    {
                        break;
                    }
    ...
    switch (LOWORD(wParam))
    {
        case IDC_CUSTOM_NEWGROUP:
        {
            ...
            if (pDlgInfo->bDataChanged)
            {
                ...
                iReply = SGMessageBox(  hwndDlg, IDS_CUSTOM_INFO_SAVECHANGES, MB_YESNOCANCEL | MB_ICONQUESTION);
        ...
        case IDC_CUSTOM_REMOVEGROUP:
        {
            ...
            if (SGMessageBox(   hwndDlg, IDS_CUSTOM_INFO_REMOVEGROUP, MB_YESNO | MB_ICONWARNING) == IDYES)
            {
                NOTIFICATIONCOOKIE  groupCookie;
                HWND                hwndCombo = GetDlgItem(hwndDlg, IDC_CUSTOM_GROUP_LIST);

                if  (SUCCEEDED(SchedGroupComboBox_GetCurGroup(   hwndCombo, &groupCookie)) && SUCCEEDED(DeleteScheduleGroup(&groupCookie)))
                {
                ...
                {
                    SGMessageBox(   hwndDlg, IDS_CUSTOM_ERROR_REMOVEGROUP, MB_OK | MB_ICONWARNING);
                    break;
                }
            }
...
)
{
    ...
    {
        // Ask user if they want to save changes.
        iReply = SGMessageBox(  hwndDlg, IDS_CUSTOM_INFO_SAVECHANGES, MB_YESNO | MB_ICONQUESTION);
    }
    ...
    switch (iReply)
    {
        ...
        case IDNO:
        {
            ...
            EVAL(SUCCEEDED(SchedGroupComboBox_GetCurGroup(  GetDlgItem(hwndDlg, IDC_CUSTOM_GROUP_LIST), pDlgInfo->pGroupCookie)));
            EVAL(SUCCEEDED(GetSchedGroupTaskTrigger( pDlgInfo->pGroupCookie, &pDlgInfo->ttTaskTrigger)));
            InitDlgInfo(hwndDlg, pDlgInfo);
...
)
{
    ...
    DWORD dwFirstDay = 0;
    EVAL(GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_IFIRSTDAYOFWEEK, (char *)&dwFirstDay, SIZEOF(DWORD)));
    ...
    for (i = dwFirstDay; i < dwFirstDay+7; i++)
    {
        ...
        EVAL(GetLocaleInfo( LOCALE_USER_DEFAULT, iDay, szDayName, ARRAYSIZE(szDayName)));
        SetDlgItemText( hwndDlg, IDC_CUSTOM_WEEKLY_DAY1 + (i - dwFirstDay), szDayName);
    ...
    UpDown_SetRange(GetDlgItem(hwndDlg, IDC_CUSTOM_DAILY_EVERYNDAYS_SPIN), DEFAULT_DAILY_EVERYNDAYS_MIN, DEFAULT_DAILY_EVERYNDAYS_MAX);
    UpDown_SetRange(GetDlgItem(hwndDlg, IDC_CUSTOM_WEEKLY_REPEATWEEKS_SPIN), DEFAULT_WEEKLY_REPEATWEEKS_MIN, DEFAULT_WEEKLY_REPEATWEEKS_MAX);
    UpDown_SetRange(GetDlgItem(hwndDlg, IDC_CUSTOM_MONTHLY_DAYOFMONTH_DAY_SPIN), DEFAULT_MONTHLY_DAY_MIN, DEFAULT_MONTHLY_DAY_MAX);
    UpDown_SetRange(GetDlgItem(hwndDlg, IDC_CUSTOM_MONTHLY_DAYOFMONTH_MONTH_SPIN), DEFAULT_MONTHLY_MONTHS_MIN, DEFAULT_MONTHLY_MONTHS_MAX);
    UpDown_SetRange(GetDlgItem(hwndDlg, IDC_CUSTOM_MONTHLY_PERIODIC_SPIN), DEFAULT_MONTHLY_MONTHS_MIN, DEFAULT_MONTHLY_MONTHS_MAX);
    UpDown_SetRange(GetDlgItem(hwndDlg, IDC_CUSTOM_TIME_REPEATEVERY_SPIN), DEFAULT_REPEATUPDATE_HRS_MIN, DEFAULT_REPEATUPDATE_HRS_MAX);
    ...
    EVAL(GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_STIMEFORMAT, szTimeFormat, ARRAYSIZE(szTimeFormat)));

    TCHAR szTimeSep[MAX_TIMESEP_LEN];
    EVAL(GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_STIME, szTimeSep, ARRAYSIZE(szTimeSep)));
...
)
{
    ...
    ComboBox_SetCurSel( GetDlgItem(hwndDlg, IDC_CUSTOM_MONTHLY_PERIODIC_LIST2), DayIndexFromDaysOfTheWeekFlags(rgfDaysOfTheWeek));
    ...
    HWND hwndGroupEdit = FindWindowEx(  hwndGroupList, NULL, TEXT("Edit"), NULL);
...
)
{
    ...
    CheckDlgButton( hwndDlg, IDC_CUSTOM_MINIMIZENETUSE, ((pTT->wRandomMinutesInterval == 0) ? BST_UNCHECKED : BST_CHECKED));
    ...
    else
    {
        ...
        SetDlgItemInt(  hwndDlg,
                        ...
                        ((pDlgInfo->dwRepeatHrsAreMins) ? pTT->MinutesInterval : (pTT->MinutesInterval / 60)),
                        FALSE);
    ...
    switch (pTT->TriggerType)
    {
        ...
        case TASK_TIME_TRIGGER_WEEKLY:
        {
            ...
            if  ((pTT->Type.Weekly.WeeksInterval == 1) && (pTT->Type.Weekly.rgfDaysOfTheWeek == TASK_WEEKDAYS))
            {
        ...
        case TASK_TIME_TRIGGER_MONTHLYDATE:
        {
            ...
            // Day X
            SetDlgItemInt(  hwndDlg, IDC_CUSTOM_MONTHLY_DAYOFMONTH_DAY_EDIT, DayFromDaysFlags(pTT->Type.MonthlyDate.rgfDays), FALSE);

            // of every Y month(s)
            SetDlgItemMonth(hwndDlg, IDC_CUSTOM_MONTHLY_DAYOFMONTH_MONTH_EDIT, pTT->Type.MonthlyDate.rgfMonths);
            break;
        }
        case TASK_TIME_TRIGGER_MONTHLYDOW:
        {
            ...
            // The X
            ComboBox_SetCurSel( GetDlgItem(hwndDlg, IDC_CUSTOM_MONTHLY_PERIODIC_LIST1), pTT->Type.MonthlyDOW.wWhichWeek-1);

            // Y
            ComboBox_SetCurSel( GetDlgItem(hwndDlg, IDC_CUSTOM_MONTHLY_PERIODIC_LIST2), DayIndexFromDaysOfTheWeekFlags(pTT->Type.MonthlyDOW.rgfDaysOfTheWeek));

            // of every Z month(s)
            SetDlgItemMonth(hwndDlg, IDC_CUSTOM_MONTHLY_PERIODIC_EDIT, pTT->Type.MonthlyDOW.rgfMonths);
            break;
        }
        default:
        {
...
)
{
    ...
    {
        BOOL    bCheck;
        int     iDay = ((i <= LOCALE_SDAYNAME7) ? i : (LOCALE_SDAYNAME1 + (i - LOCALE_SDAYNAME7 - 1)));
        ...
        CheckDlgButton( hwndDlg, IDC_CUSTOM_WEEKLY_DAY1 + (i - dwFirstDay), (bCheck ? BST_CHECKED : BST_UNCHECKED));
    }
}
...
)
{
    ...
    for (DWORD i = dwFirstDay; i <= dwFirstDay+7; i++)
    {
        if (IsDlgButtonChecked( hwndDlg, IDC_CUSTOM_WEEKLY_DAY1 + (i - dwFirstDay)) == BST_CHECKED)
        {
            int iDay = ((i <= LOCALE_SDAYNAME7) ? i : (LOCALE_SDAYNAME1 + (i - LOCALE_SDAYNAME7 - 1)));
...
)
{
    ...
    else
    {
        ...
        if (IsDlgButtonChecked(hwndDlg, IDC_CUSTOM_TIME_REPEATBETWEEN) == BST_CHECKED)
        {
            // Between X
            SYSTEMTIME st = { 0 };
            DateTime_GetSystemtime( GetDlgItem( hwndDlg, IDC_CUSTOM_TIME_REPEATBETWEEN_START_TIME), &st);
            ...
            DateTime_GetSystemtime( GetDlgItem( hwndDlg, IDC_CUSTOM_TIME_REPEATBETWEEN_END_TIME), &st);
        ...
        // Set the interval.
        int iHours = GetDlgItemInt( hwndDlg, IDC_CUSTOM_TIME_REPEATEVERY_EDIT, &bTranslated, FALSE);
    ...
    if (IsDlgButtonChecked(hwndDlg, IDC_CUSTOM_DAILY) == BST_CHECKED)
    {
        ...
        if (IsDlgButtonChecked(hwndDlg, IDC_CUSTOM_DAILY_EVERYNDAYS) == BST_CHECKED)
        {
            ...
            pTT->Type.Daily.DaysInterval = GetDlgItemInt(   hwndDlg, IDC_CUSTOM_DAILY_EVERYNDAYS_EDIT, &bTranslated, FALSE);
        }
        else
        {
            // Daily -- Every Weekday
    ...
    else if (IsDlgButtonChecked(hwndDlg, IDC_CUSTOM_WEEKLY) == BST_CHECKED)
    {
        ...
        pTT->Type.Weekly.WeeksInterval = GetDlgItemInt( hwndDlg, IDC_CUSTOM_WEEKLY_REPEATWEEKS_EDIT, &bTranslated, FALSE);
    ...
    else
    {
        ...
        if (IsDlgButtonChecked(hwndDlg, IDC_CUSTOM_MONTHLY_DAYOFMONTH) == BST_CHECKED)
        {
            ...
            int iDate = GetDlgItemInt(  hwndDlg, IDC_CUSTOM_MONTHLY_DAYOFMONTH_DAY_EDIT, &bTranslated, FALSE);
            ASSERT(iDate > 0);
...
)
{
    ...
    for (;;)
    {
        ...
        {
            SGMessageBox(   hwndDlg, IDS_CUSTOM_ERROR_NAMETOOLONG, MB_OK | MB_ICONEXCLAMATION);
            break;
        }
        ...
        {
            SGMessageBox(   hwndDlg, IDS_CUSTOM_ERROR_NAMEREQUIRED, MB_OK | MB_ICONEXCLAMATION);
            break;
        }

        // Check to see if group name is already in use.
        if  ((pDlgInfo->bScheduleNameChanged || (pDlgInfo->dwFlags & SSUI_CREATENEWSCHEDULE)) && ScheduleGroupExists(szGroupName))
        {
            SGMessageBox(   hwndDlg, IDS_CUSTOM_ERROR_NAMEEXISTS, MB_OK | MB_ICONEXCLAMATION);
            break;
        }
...
)
{
    ...
    if (IsWindowVisible(GetDlgItem(hwndDlg, IDC_CUSTOM_GROUP_EDIT)))
    {
        uResult = GetDlgItemText(   hwndDlg, IDC_CUSTOM_GROUP_EDIT, pszGroupName, cchGroupNameLen);
    }
    else if (IsWindowVisible(GetDlgItem(hwndDlg, IDC_CUSTOM_GROUP_LIST)))
    {
        uResult = GetDlgItemText(   hwndDlg, IDC_CUSTOM_GROUP_LIST, pszGroupName, cchGroupNameLen);
    }
    else
        uResult = 0;
...
)
{
    ...
    for (;;)
    {
        *(pDlgInfo->pGroupCookie) = GUID_NULL;
        ...
        if (FAILED(hrResult = CreateScheduleGroup(  &pDlgInfo->ttTaskTrigger, NULL, &gi, 0, pDlgInfo->pGroupCookie)))
        {
...
{
    return SetDlgItemInt(   hwndDlg, idCtl, MonthCountFromMonthsFlags(rgfMonths), FALSE);
}
...
//xnotfmgr
    ...
    for (;;)
    {
        ...
        ULONG               celtFetched;
        IScheduleGroup *    pScheduleGroup = NULL;
        while   (SUCCEEDED(hrResult = pEnumSchedGroup->Next( 1, &pScheduleGroup, &celtFetched)) && (celtFetched != 0))
        {
            GROUPINFO           info = { 0 };
            ...
            // Get the schedule group attributes.
            if  (SUCCEEDED(pScheduleGroup->GetAttributes(NULL, NULL, &cookie, &info, NULL, NULL)) && (cookie != NOTFCOOKIE_SCHEDULE_GROUP_MANUAL))
            {
                ASSERT(info.cbSize == SIZEOF(GROUPINFO));
                ASSERT(info.pwzGroupname != NULL);
                ...
                EVAL(SUCCEEDED(SchedGroupComboBox_AddGroup( hwndCombo, szGroupName, &cookie)));
            }
            else
