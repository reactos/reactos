...
BOOL CDialMon::RefreshTimeoutSettings(void)
{
    ...
    dwRes = RegCreateKeyEx(HKEY_CURRENT_USER, szKey, 0, TEXT(""), 0, KEY_QUERY_VALUE | KEY_SET_VALUE, NULL, &hKey, &dwDisp);
    if(ERROR_SUCCESS == dwRes)
    {
        ...
        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey,szRegValEnableAutoDisconnect,NULL,NULL, (LPBYTE) &dwData,&dwSize) == ERROR_SUCCESS)
        {
            if(dwData)
            {
                // what's the timeout?
                dwSize = sizeof(DWORD);
                if (RegQueryValueEx(hKey,szRegValDisconnectIdleTime,NULL,NULL, (LPBYTE) &dwData,&dwSize) == ERROR_SUCCESS && dwData)
                {
        ...
        else
        {
            ...
            dwData = 1;
            RegSetValueEx(hKey, szRegValEnableAutoDisconnect, 0, REG_DWORD, (LPBYTE)&dwData, sizeof(DWORD));
            RegSetValueEx(hKey, szRegValExitDisconnect, 0, REG_DWORD, (LPBYTE)&dwData, sizeof(DWORD));
