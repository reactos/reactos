...
HRESULT COfflineObjectItem::Initialize(COfflineFolder *pOOFolder, UINT cidl, LPCITEMIDLIST *ppidl)
{
    ...
    if (_cItems == 1)
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = CoCreateInstance(*(&(pooe->clsidDest)), NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void **) &m_pUIHelper);
            ASSERT(SUCCEEDED(hr));
            ASSERT(m_pUIHelper);
            if (SUCCEEDED(hr))
            {
...
#ifdef UNICODE
        *ppvObj = iid == IID_IExtractIconA ? (LPVOID) (IExtractIconA *)this : (LPVOID) (IExtractIconW *)this;
#else
...
)
{
    ...
    {
        MENUITEMINFO mii = {
            sizeof(MENUITEMINFO), MIIM_ID | MIIM_SUBMENU | MIIM_TYPE, 0, 0, 0, 0, 0, 0, 0, 0, 0 };
    ...
    if ((uFlags & CMF_VERBSONLY) || (uFlags & CMF_DVFILE))
        cItems = MergePopupMenu(&hmenu, POPUP_CONTEXT_VERBSONLY, 0, indexMenu, idCmdFirst - RSVIDM_FIRST, idCmdLast);
    else
    {
        if (_ppooi[0]->ooe.bChannel && SHRestricted2(REST_NoEditingChannels, URL(&(_ppooi[0]->ooe)), 0))
        {
            cItems = MergePopupMenu(&hmenu, POPUP_RESTRICTED_CONTEXT, 0, indexMenu, idCmdFirst - RSVIDM_FIRST, idCmdLast);
        }
        else
        {
            cItems = MergePopupMenu(&hmenu, POPUP_OFFLINE_CONTEXT, 0, indexMenu, idCmdFirst - RSVIDM_FIRST, idCmdLast);
        }
        if (_cItems > 1)
...
STDMETHODIMP COfflineObjectItem::InvokeCommand(LPCMINVOKECOMMANDINFO pici)
{
    UINT i;
...
STDMETHODIMP COfflineObjectItem::GetCommandString(UINT_PTR idCmd, UINT uFlags, UINT *pwReserved, LPSTR pszName, UINT cchMax)
{
    HRESULT hres = E_FAIL;
...
HRESULT COfflineObjectItem::DAdvise(LPFORMATETC pFE, DWORD grfAdv, LPADVISESINK pAdvSink, LPDWORD pdwConnection)
{
...
HRESULT COfflineObjectItem::_CreateFileContents(LPSTGMEDIUM pSTM, LONG lindex)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = SHCoCreateInstance(NULL, &CLSID_InternetShortcut, NULL, IID_IUniformResourceLocator, (void **) &purl);
        if (SUCCEEDED(hr))
        {
