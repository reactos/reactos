                   ...
                   void *pData, DWORD dwBytes, DWORD dwType)
{
    ...
    lResult = RegCreateKeyEx(hkeyRoot, pszKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hkey, &dwStatus);
    if (lResult != ERROR_SUCCESS) {
        return FALSE;
    }
...
BOOL GetSubscriptionFolderPath(LPTSTR pszPath)
{
    ...
    if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_SUBSCRIPTION, 0, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hk, &dwDummy)) {
