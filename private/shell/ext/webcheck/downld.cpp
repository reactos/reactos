...
LRESULT UrlDownloadWndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        TraceMsg(TF_THISMODULE, "Invalid 'this' in UrlDownloadWndProc (0x%08x) - already destroyed?", pThis);
    }
#endif
...
HRESULT InsertHistoricalCodepageIntoBindCtx(LPCWSTR pwszURL, IBindCtx * pbc)
{
    ...
    else
    {
        ...
        if (SUCCEEDED(hr) && propCodepage.lVal != CP_ACP)
        {
            ...
            hr = CoCreateInstance(CLSID_HTMLLoadOptions, NULL, CLSCTX_INPROC_SERVER, IID_IHtmlLoadOptions, (void**)&phlo);
            if (SUCCEEDED(hr) && phlo)
            {
                hr = phlo->SetOption(HTMLLOADOPTION_CODEPAGE, &propCodepage.lVal, sizeof(propCodepage.lVal));
                if (SUCCEEDED(hr))
                {
                    // Insert the option into the bindctx
                    pbc->RegisterObjectParam(L"__HTMLLOADOPTIONS", phlo);
                    TraceMsg(TF_THISMODULE, "InsertHistoricalCodepageIntoBindCtx codepage=%d", propCodepage.lVal);
                }
                phlo->Release();
            }
        }
...
// Loads browser, creates sink and hooks it up to sinks
{
    ...
    if (SUCCEEDED(hr) && SUCCEEDED(m_pParent->GetDownloadNotify(&pNotify)) && pNotify)
    {
        ...
        if (SUCCEEDED(m_pDocument->QueryInterface(IID_IOleCommandTarget, (void **)&pTarget)) && pTarget)
        {
            ...
            if (SUCCEEDED(pTarget->Exec(&CGID_DownloadHost, DWNHCMDID_SETDOWNLOADNOTIFY, 0, &varIn, NULL)))
            {
                fNotifySet=TRUE;
            }
    ...
    if (NULL == m_pCP && SUCCEEDED(hr))
    {
        ...
        hr = m_pDocument->QueryInterface(IID_IConnectionPointContainer, (void **)&pCPCont);
        if (SUCCEEDED(hr))
        {
...
//  HKEY hk;
    ...
    if (!fValidProtocol && (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CLASSES_ROOT, TEXT("PROTOCOLS\\Handler"), 0, KEY_QUERY_VALUE, &hk)))
    {
        // Crack out protocol
