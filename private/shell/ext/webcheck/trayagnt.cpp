...
//xnotfmgr
    ...
    if (SUCCEEDED(hr))
        hr = pMgr->DeliverNotification(pNotf, CLSID_WebCrawlerAgent, DM_DELIVER_DEFAULT_PROCESS | DM_THROTTLE_MODE | DM_NEED_COMPLETIONREPORT,g_pOfflineTraySink,0,0);
...
//xnotfmgr
    ...
    hr = pMgr->CreateNotification(NOTIFICATIONTYPE_TASKS_ABORT, (NOTIFICATIONFLAGS)0, NULL, &pNotCancel, 0);
...
HRESULT GetChannelPath(LPCSTR pszURL, LPTSTR pszPath, int cch, IChannelMgrPriv** ppIChannelMgrPriv)
{
    ASSERT(pszURL);
    ...
    if ((hr == CO_E_NOTINITIALIZED || hr == REGDB_E_IIDNOTREG) && SUCCEEDED(CoInitialize(NULL)))
    {
        bCoinit = TRUE;
        hr = CoCreateInstance(CLSID_ChannelMgr, NULL, CLSCTX_INPROC_SERVER, IID_IChannelMgrPriv, (void**)ppIChannelMgrPriv);
    }
...
HRESULT PreUpdateChannelImage(IChannelMgrPriv* pIChannelMgrPriv, CHANNELIMAGEDATA* pcid)
{
    ASSERT(pcid);
    ASSERT(pIChannelMgrPriv);
...
HRESULT UpdateChannelImage(IChannelMgrPriv* pIChannelMgrPriv, CHANNELIMAGEDATA* pcid)
{
    ASSERT(pcid);
    ASSERT(pIChannelMgrPriv);
...
//xnotfmgr
    ...
    if ((IID_IUnknown == riid) || (IID_INotificationSink == riid))
    {
...
//xnotfmgr
    ...
    else if (notfType == NOTIFICATIONTYPE_END_REPORT)
    {
        ...
        if (g_pTrayUI && SUCCEEDED(ReadBSTR(pNotification,NULL, c_szPropStatusString, &bstrStatus)) && SUCCEEDED(ReadGUID(pNotification, NULL, c_szStartCookie, &cookie)))
        {
        ...
        if (SUCCEEDED(hr) && SUCCEEDED(scEndStatus) && (S_FALSE != scEndStatus))
        {
            ...
            if (SUCCEEDED(hr) && dwRet)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (bChannel)
                    {
                        hr2 = GetChannelPath(strURL, cid.szPath, ARRAYSIZE(cid.szPath), &pIChannelMgrPriv);
                        if (SUCCEEDED(hr2))
...
STDMETHODIMP ReadSingleEntry(PLogEntry pLog, LPCTSTR szSubKey, int index)
{
    ...
    if (prLog && (FALSE == prLog->m_fNewStream)) {
        ...
        if (SUCCEEDED(prLog->Read(&versionNumber, sizeof(int), &cbWritten)) && (sizeof(int) == cbWritten) && (TRAYLOGVERSION == versionNumber))
        {
            hrLog = ERROR_SUCCESS;
        }
        ...
        if (ERROR_SUCCESS == hrLog) {
            hrLog = E_INVALIDARG;
            if (SUCCEEDED(prLog->Read(&clsidAgent, sizeof(CLSID), &cbWritten)) && (sizeof(CLSID) == cbWritten))
            {
                hrLog = ERROR_SUCCESS;
            }
        }
        ...
        if (ERROR_SUCCESS == hrLog) {
            hrLog = E_INVALIDARG;
            if (SUCCEEDED(prLog->Read(&cookie, sizeof(CLSID), &cbWritten)) && (sizeof(CLSID) == cbWritten))
            {
                hrLog = ERROR_SUCCESS;
            }
        }
        ...
        if (ERROR_SUCCESS == hrLog) {
            hrLog = E_INVALIDARG;
            if (SUCCEEDED(prLog->Read(&ftLog, sizeof(FILETIME), &cbWritten)) && (sizeof(FILETIME) == cbWritten))
            {
                hrLog = ERROR_SUCCESS;
            }
        }
...
#error Potential Buffer overflow:
    ...
    for (; index < maxIndex; index ++)  {
        if (SUCCEEDED(WriteSingleEntry(&(m_aLogEntry[index]), szSubKey, cIndex)))
        {
            cIndex ++;
        }
    }
...
#error Potential Buffer overflow:
    ...
    if (!prs)   {
        ...
        if (SUCCEEDED(prs->Read(&maxIndex, sizeof(int), &cbWritten)) && (sizeof(int) == cbWritten) && (maxIndex >= 0))
        {
            FILETIME    ftFirst;
            SYSTEMTIME  stNow;
...
STDMETHODIMP CTrayUI::OpenSubscriptionFolder(void)
{
    ...
    if (ReadRegValue(HKEY_LOCAL_MACHINE, REGSTR_PATH_SUBSCRIPTION, REGSTR_VAL_DIRECTORY, (LPBYTE)szSubPath, dwSize) == FALSE)
    {
