    ...
    if (hr == S_OK)
    {
        ...
        if (DragQueryFile((HDROP)stgmed.hGlobal, 0, szTempPath, ARRAYSIZE(szTempPath)))
        {
            ...
            if ((NULL != pszURL) || (NULL != peInitSrc))
            {
                ...
                {
                    // canonicalize url
                    if (SHGetIniString(pszSection, pszEntry, szTempURL, INTERNET_MAX_URL_LENGTH, szIniPath))
                    {
...
STDMETHODIMP CSubscriptionMgr::Initialize(LPCITEMIDLIST pcidlFolder, IDataObject * pido, HKEY hkeyProgID)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                hr = CoCreateInstance(clsid, NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void**)&m_pUIHelper);
                if (SUCCEEDED(hr))
                {
                    ISubscriptionAgentShellExt *psase;
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        hr = psase->Initialize(&cookie, wszURL, wszName, (clsid == CLSID_ChannelAgent) ? SUBSTYPE_CHANNEL : SUBSTYPE_URL);
                        psase->Release();
                    }
                }
                CoUninitialize();
...
)
{
    ...
    else
    {
        ...
        case SUBSTYPE_CHANNEL:
            hr = SHAddSubscribeFavorite (hwnd, pwszURL, pwszFriendlyName, dwFlags, subsType, pInfo);
            break;
        ...
        case SUBSTYPE_DESKTOPURL:
            ...
            hr = CreateDesktopSubscription (hwnd, pwszURL, pwszFriendlyName, dwFlags, subsType, pInfo);
            break;
...
)
{
    ...
    if ((FAILED(IsSubscribed(pwszURL, &fAlreadySubscribed)) || fAlreadySubscribed) && (!(dwFlags & (CREATESUBS_NOSAVE | CREATESUBS_NOUI))))
    {
        return E_FAIL;
    }
    ...
    if (SUBSTYPE_URL == subType)
    {
        ...
        DWORD dwCount;
        if (SHRestricted2W(REST_NoAddingSubscriptions, pwszURL, 0) || ((dwMaxCount > 0) && SUCCEEDED(CountSubscriptions(subType, &dwCount)) && (dwCount >= dwMaxCount)))
        {
    ...
    if (SUCCEEDED(hr))
    {
        if (!IsFlagSet(dwFlags, CREATESUBS_NOSAVE))
        {
            ...
            if (!IsFlagSet(dwFlags, CREATESUBS_NOUI) || (IsFlagSet(dwFlags, CREATESUBS_FROMFAVORITES) && !IsFlagSet (dwFlags, CREATESUBS_ADDTOFAVORITES)))
            {
                // See if the user wants to be asked about enabling the screen saver.
                DWORD dwValue = BST_UNCHECKED;
                ReadRegValue(   HKEY_CURRENT_USER, WEBCHECK_REGKEY, g_szDontAskScreenSaver, &dwValue, SIZEOF(dwValue));
                if  ((subProps.fChannelFlags & CHANNEL_AGENT_PRECACHE_SCRNSAVER) && (dwValue == BST_UNCHECKED) && !IsADScreenSaverActive())
                {
                    DialogBox( MLGetHinst(), MAKEINTRESOURCE(IDD_SUBSCRIPTION_ENABLECHANNELSAVER), hwnd, EnableScreenSaverDlgProc);
                }
            }
                        ...
                        DWORD dwFlags, SUBSCRIPTIONTYPE subsType, SUBSCRIPTIONINFO *pInfo)
{
    ...
    case IDCANCEL:
        ...
        break;
    default:
        hr = CreateSubscriptionNoSummary (hwnd, pwszURL, pwszFriendlyName, CREATESUBS_NOUI | dwFlags, subsType, pInfo);
        break;
    }
...
{
    // Validate the parameters.
    if (!IS_VALID_SUBSCRIPTIONTYPE(subType) || !pInfo || (pInfo->cbSize != sizeof(SUBSCRIPTIONINFO)))
    {
        ASSERT(FALSE);
        return E_INVALIDARG;
    }
...
STDMETHODIMP CSubscriptionMgr::GetSubscriptionInfo(LPCWSTR pwszURL, SUBSCRIPTIONINFO *pInfo)
{
    HRESULT hr;

    // Validate the parameters.
    if (!pInfo || !pwszURL || (pInfo->cbSize != sizeof(SUBSCRIPTIONINFO)))
    {
        ASSERT(FALSE);
        return E_INVALIDARG;
    }
...
STDMETHODIMP CSubscriptionMgr::UpdateSubscription(LPCWSTR pwszURL)
{
    ASSERT(pwszURL);
...
INT_PTR CALLBACK SummarizeDesktopSubscriptionDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        {   //block to declare vars to update captions
            ...
            {
                if(MLLoadString((pInfo->pSubsInfo->bNeedPassword ? IDS_DESKTOPCHANNEL_SUMMARY_TEXT : IDS_DESKTOPCHANNEL_SUMMARY_NOPW), sz, ARRAYSIZE(sz)))
                {
    ...
    case WM_COMMAND:
        ...
        case IDC_SUBSCRIBE_CUSTOMIZE:
            ...
            if (S_OK == pInfo->pMgr->CreateSubscriptionNoSummary (hDlg, pInfo->pwszUrl, pInfo->pwszName, pInfo->dwFlags | CREATESUBS_NOSAVE, pInfo->subsType, pInfo->pSubsInfo))
            {
