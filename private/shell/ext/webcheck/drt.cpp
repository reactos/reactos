...
STDMETHODIMP CWCRunSynchronous::QueryInterface(REFIID riid, void ** ppv)
{
    *ppv=NULL;
...
STDMETHODIMP_(ULONG) CWCRunSynchronous::AddRef(void)
{
    return ++m_cRef;
}


STDMETHODIMP_(ULONG) CWCRunSynchronous::Release(void)
{
    if( 0L != --m_cRef )
        return m_cRef;
...
HRESULT CWCRunSynchronous::Init()
{
    ...
    if (!m_pManager)
        hr = CoCreateInstance(CLSID_StdNotificationMgr, NULL, CLSCTX_INPROC_SERVER, IID_INotificationMgr, (void**)&m_pManager);
...
INotification *CWCRunSynchronous::CreateNotification()
{
    ...
    if (m_pManager)
    {
        m_pManager->CreateNotification(NOTIFICATIONTYPE_AGENT_START, (NOTIFICATIONFLAGS) 0, NULL, &pNot, 0);
    }
...
HRESULT CWCRunSynchronous::RunObject(INotification *pNot, REFCLSID rclsidDest)
{
    ...
    if (m_pManager)
    {
        ...
        hr = m_pManager->DeliverNotification(pNot, rclsidDest, DM_NEED_COMPLETIONREPORT, (INotificationSink *)this, NULL, 0);
    }
...
HRESULT CWebFile::Open(HANDLE & hFile)
{
    ...
    m_Handle = NULL;
    hTmp = CreateFile(m_Path, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
HRESULT CWebFile::WebCrawl(CWCRunSynchronous *pCourier)
{
    ...
    hr = CoCreateInstance(CLSID_StdNotificationMgr, NULL, CLSCTX_INPROC_SERVER, IID_INotificationMgr, (void **)&pMgr);
    ...
    hr = pMgr->CreateNotification(NOTIFICATIONTYPE_AGENT_START, (NOTIFICATIONFLAGS) 0, NULL, &pNot, 0);
    if (FAILED(hr)) {
        SAFERELEASE(pMgr);
    }
    ASSERT(pNot);
...
HRESULT CheckCache(LPCTSTR szURL, CWCRunSynchronous * prs, UINT cmd)
{
    ...
    char *strBuf = (char *) MapViewOfFile(hFileMapping, FILE_MAP_READ, 0, 0, 0);   //  From offset 0, map whole.
...
HRESULT APIENTRY f3drt(VARIANT *pParam, long *plRetVal)
{
        ...
        if (pParam->vt == VT_BSTR)  {
            ...
            if (PathIsURL(iniURL))  {
                ...
                if (pBreak != NULL) {
                    * pBreak = (TCHAR)0;
                    pBreak ++;
                    lstrcpyn(szSection, pBreak, ARRAYSIZE(szSection));
                }
    ...
    *plRetVal = -2; // assume error
    ...
    ASSERT(SUCCEEDED(hr));
    dwSize = GetPrivateProfileString(szSection, c_szStrURL, c_szDefaultDRTURL, targetURL, INTERNET_MAX_URL_LENGTH, szPath);
    dwSize = GetPrivateProfileString(szSection, c_szStrLoggedURL, c_szDefaultLoggedURL, loggedURL, INTERNET_MAX_URL_LENGTH, szPath);
    dwSize = GetPrivateProfileString(szSection, c_szStrFiles, c_szStrEmpty, filesURL, INTERNET_MAX_URL_LENGTH, szPath);
    CheckCache(filesURL, prs, WF_CMD_DELETE);
    dwSize = GetPrivateProfileString(szSection, c_szStrNoFiles, c_szStrEmpty, nofilesURL, INTERNET_MAX_URL_LENGTH, szPath);
    ...
    dwLevels = GetPrivateProfileInt(
                    ...
                    szPath);
    dwCDF = GetPrivateProfileInt(szSection, c_szCDF, 0, szPath);
    dwFlags = GetPrivateProfileInt(szSection, c_szIntFlags, (dwCDF)?(CHANNEL_AGENT_PRECACHE_ALL):(WEBCRAWL_GET_IMAGES | WEBCRAWL_DONT_MAKE_STICKY), szPath);
    INotification *pNot = NULL;
    HWND hWait;
    ...
    hWait = CreateDialog(MLGetHinst(), MAKEINTRESOURCE(IDD_DRT), GetDesktopWindow(), (DLGPROC)DRT_Dialog);
    ShowWindow(hWait, SW_SHOW);
    SetWindowPos(hWait, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW);
