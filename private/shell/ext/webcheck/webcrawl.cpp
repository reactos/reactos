...
HRESULT GetVersionFromString(const char *szBuf, LPDWORD pdwFileVersionMS, LPDWORD pdwFileVersionLS)
{
...
HRESULT CombineBaseAndRelativeURLs(LPCWSTR szBaseURL, LPCWSTR szRelative1, LPWSTR *szRelative2)
{
    ...
    if (szBaseURL && (IsValidURL(NULL, szBaseURL, 0) == S_OK))
    {
        ...
        if (szRelative1)
        {
            ...
            if ((dwLen2 > 0) && ((*szRelative2)[dwLen2-1] == (unsigned short)L'\\') || ((*szRelative2)[dwLen2-1] == (unsigned short) L'/'))
            {
...
HRESULT CWebCrawler::StartOperation()
{
    ...
    SAFEFREEBSTR(m_bstrBaseURL);
    if (FAILED(ReadBSTR(pItem, c_szPropURL, &m_bstrBaseURL)) || !m_bstrBaseURL || !CUrlDownload::IsValidURL(m_bstrBaseURL))
    {
...
HRESULT CWebCrawler::StartDownload()
{
    ...
    else
    {
        if (!m_llCacheGroupID)
        {
            m_llCacheGroupID = CreateUrlCacheGroup((IsRecurseFlagSet(WEBCRAWL_DONT_MAKE_STICKY) ? 0 : CACHEGROUP_FLAG_NONPURGEABLE), 0);
    ...
    if ((CWCStringList::STRLST_ADDED == m_pPages->AddString(m_bstrBaseURL, m_dwRecurseLevels)) && m_pPages->NumStrings() == 1)
    {
...
HRESULT CWebCrawler::StartNextDownload()
{
    ...
    if (!pslUrls)
    {
        ...
        if (!pslUrls && (m_iCodeBaseStarted < m_pCodeBaseList->NumStrings()))
        {
            ...
            {
                CCodeBaseHold *pcbh = (CCodeBaseHold *) m_pCodeBaseList->GetStringData(m_iCodeBaseStarted);
...
HRESULT CWebCrawler::ActuallyStartDownload(CWCStringList *pslUrls, int iIndex, BOOL fReStart /* = FALSE */)
{
    ...
    TraceMsgA(TF_THISMODULE, "WebCrawler GET_URL (%d of %c%d) Recurse %d : %ws", m_iTotalStarted+1, ((m_lMaxNumUrls>0) ? ' ' : '?'), iMax, pslUrls->GetStringData(iIndex) & DATA_RECURSEMASK, pwszURL);
#endif

    dwBrowseFlags = DLCTL_DOWNLOADONLY | DLCTL_NO_FRAMEDOWNLOAD | DLCTL_NO_SCRIPTS | DLCTL_NO_JAVA | DLCTL_NO_RUNACTIVEXCTLS;
...
HRESULT CWebCrawler::ProcessDependencyLinks(CWCStringList **ppslUrls, int *piStarted)
{
    ...
    {
        if (!m_pPages->FindString(m_pDependencyLinks->GetString(m_iDependencyStarted), m_pDependencyLinks->GetStringLen(m_iDependencyStarted), &iIndex))
        {
...
// Called when a url is finished downloading, it processes the url and kicks off the next download
            ...
            if (!fOperationComplete)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ((iPageIndex==0) && (m_dwRecurseLevels>0) && !IsRecurseFlagSet(WEBCRAWL_LINKS_ELSEWHERE) && !m_bstrHostName)
                        {
...

            {
                ...
                if (pcbh && SUCCEEDED(hrResult[enUsesVer]) && (vaProps[enUsesVer].vt == VT_BSTR))
                {
                    ...
                    if (szVerStr)
                    {
                        ...
                        if (FAILED(GetVersionFromString(szVerStr, &pcbh->dwVersionMS, &pcbh->dwVersionLS)))
                        {
            ...
            GROUPID                     llCacheGroupID)
{
    ...
    if (pszBaseUrl)
    {
        ...
        if (SUCCEEDED(UrlCombine(pszBaseUrl, pszThisUrl, szCombined, &dwSize, 0)))
        {
    ...
    if (!GetUrlCacheEntryInfo(pszThisUrl, lpCacheEntryInfo, &dwBufSize))
    {
        ...
        if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
            ...
            TraceMsgA(llCacheGroupID ? TF_WARNING : TF_THISMODULE, "Failed GetUrlCacheEntryInfo (not in cache) URL=%ws", pszThisUrl);
#endif
        return E_FAIL;
    }
    ...
    if (llCacheGroupID != 0)
    {
        if (!SetUrlCacheEntryGroup(pszThisUrl, INTERNET_CACHE_GROUP_ADD, llCacheGroupID, NULL, 0, NULL))
        {
            switch (GetLastError())
            {
                              ...
                              FILETIME                    ftNewLastModified)
{
    ...
    else if (pvarChange->vt == VT_I4)
    {
        ...
        if ((dwOldCode != dwNewCode) || !dwNewCode)
        {
                        ...
                        IHTMLElementCollection **ppCollection)
{
    ...
    if (pDisp)
    {
        hr = pDisp->QueryInterface(IID_IHTMLElementCollection, (void **)ppCollection);
        pDisp->Release();
    }
