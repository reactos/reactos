...
)
{
    ...
    for (;;)
    {
        ...
        if ((s_hTermEvent = CreateEvent(NULL, FALSE, FALSE, s_szThreadTermEvent)) == NULL)
        {
            ASSERT(FALSE);
            break;
        }
        ...
        // Create the Thread.
        DWORD dwThreadId;
        if ((s_hSEThread = CreateThread(NULL, 1024, SSExecThread, NULL, 0, &dwThreadId)) == NULL)
        {
            ASSERT(FALSE);
            break;
        }
...
BOOL ReloadChannelScreenSaver ()
{
    ...
    EVAL(MLLoadString(IDS_SCREENSAVEREXE, szScreenSaverEXE, ARRAYSIZE(szScreenSaverEXE)) != 0);

    return (ShellExecute(   NULL, NULL, szScreenSaverEXE, NULL, NULL, SW_SHOWNORMAL) > (HINSTANCE)32);
}