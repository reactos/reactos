...
CWebCheck::CWebCheck()
{    
    DllAddRef();// Maintain global object count    
    m_cRef = 1;// Initialize object    
    g_pwc = this;// save our instance
}


CWebCheck::~CWebCheck()
{    
    DllRelease();// Maintain global object count    
    g_pwc = NULL;// no longer available
}
...
STDMETHODIMP CWebCheck::QueryStatus(const GUID *pguidCmdGroup, ULONG cCmds, OLECMD prgCmds[], OLECMDTEXT *pCmdText)
{
...
STDMETHODIMP CWebCheck::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANTARG *pvaIn, VARIANTARG *pvaOut)
{
...
STDMETHODIMP CWebCheck::StartService(BOOL fForceExternals)
{
    ...
    DialmonInit();// create dialmon window
...
STDMETHODIMP CWebCheck::StopService(void)
{
    ...
    DialmonShutdown();// kill dialmon window
...
DWORD WINAPI ExternalsThread(LPVOID lpData)
{
    ...
    // [darrenmi] beta-1 hack: Sens may have a thread sitting in its code at this point so it's not safe to unload sens. 
    // Since we're in the process of shutting down anyway, just leave it alone and let the system unload it.
    ...
    CoUninitialize();// clean up com goo
...
void SetNotificationMgrRestrictions(INotificationProcessMgr0 *pNotfMgrProcess)
{
    ...
    if (!pNotfMgrProcess)
    {
        hr = CoCreateInstance(CLSID_StdNotificationMgr, NULL, CLSCTX_INPROC_SERVER, IID_INotificationProcessMgr0, (void**)&pNotProcess);
...
// OLE bypass code
// Expose a couple of APIs to call start and stop service so loadwc doesn't need to load up OLE at start time.
HRESULT ExtStartService(BOOL    fForceExternals)
{
    HRESULT hr = E_FAIL;
