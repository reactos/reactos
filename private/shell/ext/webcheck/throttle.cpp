        ...
        *ppThrottler = NULL;
        ...
        if (NULL == s_pThrottler)
        {
            ...
            if (NULL != s_pThrottler)
            {
                ...
                if (NULL != pcf)
                {
                    HRESULT hrRegister = CoRegisterClassObject(CLSID_SubscriptionThrottler, pcf, CLSCTX_LOCAL_SERVER, REGCLS_MULTIPLEUSE, &s_pThrottler->m_dwRegister);
...
void CThrottler::RunItem(int queueSlot, CUpdateItem *pUpdateItem)
{
    ...
    else
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                hr = CoCreateInstance(sii.clsidAgent, NULL, CLSCTX_INPROC_SERVER, IID_ISubscriptionAgentControl, (void**)&pUpdateItem->m_pSubsAgentCtl);
                if (SUCCEEDED(hr))
                {
