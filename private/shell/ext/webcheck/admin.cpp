...
DWORD WCRestricted2W(BROWSER_RESTRICTIONS rest, LPCWSTR pwzUrl, DWORD dwReserved)
{
    ...
    SHGetValue(HKEY_CURRENT_USER, TEXT("Software\\Policies\\Microsoft\\Internet Explorer\\Infodelivery\\Restrictions"), TEXT("NoChannelUI"), &dwType, &dw, &dwSize);
                    ...
                    for (i = 0; i < dwChannels; i++)
                    {
                        ...
                        if (bstrTitle)
                        {
                            ...
                            if (dwOffline)
                            {
                                ...
                                if (SUCCEEDED(hr))
                                {
                                    hr = pSubMgr2->CreateSubscription(NULL, bstrURL, bstrTitle, CREATESUBS_NOUI, SUBSTYPE_CHANNEL, NULL);
                                    if (dwSynchronize)
                                    {
                                        BOOL bIsSubscribed;
                                        SUBSCRIPTIONCOOKIE cookie;

                                        if (SUCCEEDED(pSubMgr2->IsSubscribed(bstrURL, &bIsSubscribed)) && bIsSubscribed && SUCCEEDED(ReadCookieFromInetDB(bstrURL, &cookie)))
                                        {
...
HRESULT Channel_GetFolder(LPTSTR pszPath, XMLDOCTYPE xdt )
{
    ...
    if (SUCCEEDED(Channel_GetBasePath(szFavs, ARRAYSIZE(szFavs))))
    {
        ...
        MLLoadString(((xdt == DOC_CHANNEL)? IDS_CHANNEL_FOLDER : IDS_SOFTWAREUPDATE_FOLDER), szChannel, MAX_PATH);
