                ...
                CMemStream(fForceNewStream)
{
    ...
    lRes = RegCreateKeyEx(hBaseKey, pszRegKey, 0, NULL, 0, KEY_QUERY_VALUE | KEY_SET_VALUE, NULL, &m_hKey, &dwDisposition);
...
CRegStream::~CRegStream()
{
    ...
    if (m_pstm && m_fDirty)
    {
        ...
        pData = (char *)GlobalLock(hGlobal);
        if(pData)
            RegSetValueEx(m_hKey, m_pszSubKey, NULL, REG_BINARY, (const BYTE *)pData, dwSize);
        GlobalUnlock(hGlobal);
        m_fDirty = FALSE;
    }
