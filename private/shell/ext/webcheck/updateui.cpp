...
BOOL CUpdateDialog::PersistStateToRegistry (HWND hDlg)
{
    ...
    HKEY key;
    DWORD dwDisposition;
    if (ERROR_SUCCESS != RegCreateKeyEx (HKEY_CURRENT_USER, c_szRegKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &key, &dwDisposition))
        return FALSE;
...
BOOL CUpdateDialog::GetPersistentStateFromRegistry (PROG_PERSIST_STATE& state, int iCharWidth)
{
    ...
    LONG result = RegQueryValueEx (key, c_szProgressWindowSettings, 0, &dwType, (LPBYTE)&state, &dwSize);
