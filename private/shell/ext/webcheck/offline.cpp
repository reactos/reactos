...
BOOL CALLBACK GoOfflinePromptDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
BOOL CALLBACK GoOnlinePromptDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
...
BOOL OnConnectedNotification(void)
{
    BOOL fRet = TRUE;
...
BOOL OnDisconnectedNotification(void)
{
    BOOL fRet = TRUE;
...
{
    // run the dialog
    BOOL fRet = DialogBoxParam(MLGetHinst(),MAKEINTRESOURCE(IDD_GO_OFFLINE_DLG), NULL,GoOfflinePromptDlgProc,(LPARAM) 0);
    return fRet;
}
...
{
    // run the dialog
    BOOL fRet = DialogBoxParam(MLGetHinst(),MAKEINTRESOURCE(IDD_GO_ONLINE_DLG), NULL,GoOnlinePromptDlgProc,(LPARAM) 0);
    return fRet;
}
