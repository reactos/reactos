...
HRESULT CConnectionAgent_CreateInstance(LPUNKNOWN pUnkOuter, IUnknown **ppunk)
{
    ...
    *ppunk = NULL;
    ...
    if (NULL == punk) {
        ...
        hr = RegisterActiveObject(punk, CLSID_ConnectionAgent, ACTIVEOBJECT_STRONG, &pAgent->m_dwRegisterHandle);
        SAFERELEASE(pAgent);
        if (FAILED(hr))
        {
...
BOOL CPing::Init(UINT uTimeoutSec)
{
    // save timeout
    ...
    if(NULL == _hwnd)
        _hwnd = CreateWindow(c_szPingWndClass,
                             NULL, 
                             WS_OVERLAPPED,
                             CW_USEDEFAULT, 
                             CW_USEDEFAULT,
...
LRESULT CPing::WndProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
...
BOOL CPing::EnableAutodial(BOOL fEnable)
{
    ...
    if(g_fIsWinNT) {
        ...
        if(!ReadRegValue(HKEY_CURRENT_USER, szInternetSettings, c_szAutodial, &dwOldState, sizeof(DWORD)))
            dwOldState = 0;
        if(dwOldState) fOldEnable = TRUE;

        // set new state
        if(fEnable) dwNewState = 1;
        WriteRegValue(HKEY_CURRENT_USER, szInternetSettings, c_szAutodial, &dwNewState, sizeof(DWORD), REG_BINARY);
    }
...
BOOL CPing::PingSite(LPTSTR pszHost)
{
    ...
    if(INADDR_NONE == ipAddress) {
        ...
        _hAsync = WSAAsyncGetHostByName(_hwnd, WM_NAME, pszWork, (char *)pGetHostBuff, MAXGETHOSTSTRUCT);
        if(_hAsync) {
...
BOOL PingProxy(UINT uTimeoutSec)
{
    ...
    if(ReadRegValue(HKEY_CURRENT_USER, szInternetSettings, szProxyEnable, &dwValue, sizeof(DWORD))) {
        if(0 == dwValue)
            return FALSE;
    }
...
STDMETHODIMP CConnClient::QueryStatus(const GUID *pguidCmdGroup, ULONG cCmds, OLECMD prgCmds[], OLECMDTEXT *pCmdText)
{
...
STDMETHODIMP CConnClient::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANTARG *pvaIn, VARIANTARG *pvaOut)
{
...
CConnectionAgent::CConnectionAgent()
{
    ...
    m_pMgr = NULL;
    CoCreateInstance(CLSID_StdNotificationMgr, NULL, CLSCTX_INPROC_SERVER, IID_INotificationMgr, (void**)&m_pMgr);
}
...
void CConnectionAgent::Clean(void)
{
...
void CConnectionAgent::Connect(void)
{
    BOOL            fDone = FALSE;
    ...
    if(fContinue)
    {
        ...
        if(!InternetAutodial((INTERNET_AUTODIAL_FORCE_ONLINE | INTERNET_AUTODIAL_FORCE_UNATTENDED | INTERNET_AUTODIAL_FAILIFSECURITYCHECK), 0))
        {
...
void CConnectionAgent::Disconnect(void)
{
...
BOOL CConnectionAgent::IsDialPossible()
{
    BOOL    fPossible = TRUE;
...
BOOL CConnectionAgent::IsDialExisting(void)
{
...
STDMETHODIMP CConnectionAgent::QueryStatus(const GUID *pguidCmdGroup, ULONG cCmds, OLECMD prgCmds[], OLECMDTEXT *pCmdText)
{
    if (IsEqualGUID(*pguidCmdGroup, CGID_ConnCmdGrp))
    {
...
STDMETHODIMP CConnectionAgent::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANTARG *pvaIn, VARIANTARG *pvaOut)
{
    HRESULT     hr;
    CLIENTINFO  *pInfo;
    int         iIndex;
...
BOOL GetLogonInfo(DIALPROPDATA *pData)
{
    RASDIALPARAMS   dp;
    DWORD           dwRes;
