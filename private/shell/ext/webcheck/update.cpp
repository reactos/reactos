...
STDMETHODIMP CUpdateAgent::Init(void)
{
    ...
    if (SUCCEEDED(hr))  {
        ...
        hThread = CreateThread(NULL, 0, DialogThreadProc, (LPVOID)m_pDialog, 0, &m_DialogThreadID);
        if (!hThread)   {
    ...
    if (SUCCEEDED(hr))  {
        ...
        hThread = CreateThread(NULL, 0, UpdateThreadProc, (LPVOID)m_pController, 0, &m_ThreadID);
        if (!hThread)   {
