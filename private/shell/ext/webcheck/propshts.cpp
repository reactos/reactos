...
BOOL ReadAthenaMailSettings(LPSTR lpszEmailAddress, LPSTR lpszSMTPServer)
{
    ...
    hr = CoCreateInstance(CLSID_ImnAccountManager,
        NULL,                       // no aggregation
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (hr == S_OK) {
                ...
                {
                    hr = pAccount->GetPropSz(AP_SMTP_SERVER, lpszSMTPServer, CCHMAX_SERVER_NAME);
                }

                // Get the Users email address for this account
                if (NULL != lpszEmailAddress)
                {
                    hr |= pAccount->GetPropSz(AP_SMTP_EMAIL_ADDRESS, lpszEmailAddress, CCHMAX_EMAIL_ADDRESS);
                }
