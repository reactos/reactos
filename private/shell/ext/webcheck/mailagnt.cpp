...
#define ATHENA_SMTP_SERVER     TEXT("Software\\Microsoft\\Internet Mail and News\\Mail\\SMTP")
#define NETSCAPE_SMTP_SERVER     TEXT("Software\\Netscape\\netscape Navigator\\Services\\SMTP_Server")
...
)
{
    ...
    if (SUCCEEDED(CoCreateInstance(CLSID_CMultiLanguage, NULL, CLSCTX_INPROC_SERVER, IID_IMultiLanguage2, (void**)&pIML2)))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (srcCodePage != dstCodePage)
                {
                    ...
                    DWORD dwMode = 0;
                    hr = pIML2->ConvertString(&dwMode, srcCodePage, dstCodePage, (LPBYTE)lpszSrcString, &uiSrcSize, (LPBYTE)lpszDstString, &uiDstSize);
                }
                else
                {
                               ...
                               // supports it.
{
    ...
    if (lpszNewName)
    {
        ...
        if (S_OK == TranslateCharset(lpszName, lpszNewName, uiSize, lpszCharSet, szTargetEncoding))
        {
    ...
    if (!(fSendHTMLEmail &&
          BuildHTMLMessage(lpszEmailAddress, lpszName, lpszURL, &lpszSMTPMessage, lpszTitle, lpszAbstract, lpszCharSet)))
    {
...
// First try and load a full mapi implementation (exchange or outlook)
    ...
    if (hmodMail != NULL)
    {
        ...
        if (pfnSendMail != NULL && pfnLogon != NULL)
        {
            ...
            else
            {
                ...
                BuildMAPIMessage(lpszName, lpszURL, lpszEmailAddress, &message, &recipient, szSubject, lpszSMTPMessage, szTitle, szAbstract);
    ...
    if (lpszSMTPMessage)
    {
        b = SMTPSendMessage(lpszSMTPServer, lpszEmailAddress, lpszEmailAddress, lpszSMTPMessage);
...
HRESULT SendEmailFromItem(ISubscriptionItem *pItem)
{
    ...
    if (pszURL && pszName)
    {
        ...
        if (!ReadRegValue(HKEY_CURRENT_USER, c_szRegKey, TEXT("EnableHTMLMailNotification"), &fSendHTMLEmail, sizeof(DWORD)))
        {
        ...
        hr = NotifyViaEMail(pszURL, szEmailAddress, szSMTPServer, pszName, pszTitle, pszAbstract, pszCharSet, fSendHTMLEmail);
    }
