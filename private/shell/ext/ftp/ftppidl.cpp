...
/*
    ...
    data structure have been verified by getting the data directly from the server.
*/
...
HRESULT UrlCreateFromPidl(LPCITEMIDLIST pidl, DWORD shgno, LPTSTR pszUrl, DWORD cchSize, DWORD dwFlags, BOOL fHidePassword)
{
    ...
    else
    {
        ...
        if (!EVAL((shgno & SHGDN_FORPARSING) || (shgno & SHGDN_FORADDRESSBAR) || (shgno == SHGDN_NORMAL)))
        {
...
HRESULT CreateFtpPidlFromDisplayPathHelper(LPCWSTR pwzFullPath, 
                                           CWireEncoding * pwe,
                                           ULONG *pcchEaten, 
                                           LPITEMIDLIST * ppidl,
                                           BOOL fIsTypeKnown,
                                           BOOL fIsDir, 
                                           LPITEMIDLIST * ppidlCurrentID,
                                           LPWSTR * ppwzRemaining)
/*
    DESCRIPTION:
...
*/
{
    ...
    *ppidl = 0;
    ...
    EVAL(SUCCEEDED(pwe->UnicodeToWireBytes(NULL,
                                           wzFirstItem,
                                           ((pwe && pwe->IsUTF8Supported()) ? WIREENC_USE_UTF8 : WIREENC_NONE), 
                                           wFirstWireItem, 
                                           ARRAYSIZE(wFirstWireItem))));
...
HRESULT CreateFtpPidlFromFtpWirePathHelper(LPCWIRESTR pwFtpWirePath, 
                                           CWireEncoding * pwe,
                                           ULONG *pcchEaten, 
                                           LPITEMIDLIST * ppidl,
                                           BOOL fIsTypeKnown,
...
HRESULT CreateFtpPidlFromFtpWirePath(LPCWIRESTR pwFtpWirePath, 
                                     CWireEncoding * pwe, 
                                     ULONG *pcchEaten, 
                                     LPITEMIDLIST * ppidl, 
                                     BOOL fIsTypeKnown,
...
/*
    ...
    not E_INVALIDARG.
*/
HRESULT CreateFtpPidlFromUrlEx(LPCTSTR pszUrl, 
                               CWireEncoding * pwe, 
                               ULONG *pcchEaten,
                               LPITEMIDLIST * ppidl,
                               IMalloc * pm,
                               ...
                               BOOL fIsDir)
{
    ...
    *ppidl = 0;
    ...
    if (fResult && (INTERNET_SCHEME_FTP == urlComps.nScheme))
    {
        ...
        TraceMsg(TF_FTPURL_UTILS, 
                 "CreateFtpPidlFromUrl() failed InternetCrackUrl() because pszUrl=%s, fResult=%d, urlComps.nScheme=%d",
                 pszUrl,
                 fResult, 
                 urlComps.nScheme);
...
HRESULT FtpItemID_Alloc(LPFTPIDLIST pfi, LPITEMIDLIST * ppidl)
{
    ...
    cbDataFirst = (WORD)((sizeof(*pfi) -
                          sizeof(pfi->szWireName) -
                          sizeof(pfi->wzDisplayName)) +
                         LENGTH_AFTER_ALIGN(cchSizeOfName + 1, sizeof(DWORD)) -
                         sizeof(DWORD));
...
{
    if (EVAL(!FtpID_IsServerItemID(pidl)) && !FtpItemID_IsFragment(pidl))
    {
...
BOOL FtpItemID_IsParent(LPCITEMIDLIST pidlParent, LPCITEMIDLIST pidlChild)
{
    ...
    if (pidlChild)
    {
        ...
        while (fIsChild && 
               pidl1Iterate && 
               !ILIsEmpty(pidl1Iterate) &&
               pidl2Iterate && 
               !ILIsEmpty(pidl2Iterate) &&
...
LPCITEMIDLIST FtpItemID_FindDifference(LPCITEMIDLIST pidlParent, LPCITEMIDLIST pidlChild)
{
    ...
    if (pidlChild)
    {
        ...
        // Let's see if pidl starts off with
        while (pidl1Iterate && !ILIsEmpty(pidl1Iterate) && pidlDiff && !ILIsEmpty(pidlDiff) && FtpItemID_IsEqual(pidl1Iterate, pidlDiff))
        {
            pidlDiff = _ILNext(pidlDiff);
            pidl1Iterate = _ILNext(pidl1Iterate);
        }
