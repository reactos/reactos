...
HRESULT PurgeDelayedActions(void)
{
    ...
    if (g_hFlushDelayedActionsEvent)
    {
        ...
        {
            // We can't be in a critical section or our background thread can't come alive.
            ASSERTNONCRITICAL;

            TraceMsg(TF_BKGD_THREAD, "PurgeDelayedActions: Waiting for thread to stop.");
            // Now just wait for the thread to finish.
            // Someone may kill the thread so let's make sure we don't keep sleeping if the thread died.
