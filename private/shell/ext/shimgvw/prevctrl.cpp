...
LRESULT CPreview::OnSize(UINT, WPARAM, LPARAM lParam, BOOL&)
{
    ...
    ::SetWindowPos(m_cwndPreview.m_hWnd, NULL, 0, 0, LOWORD(lParam), HIWORD(lParam), SWP_NOZORDER | SWP_NOACTIVATE);
    return 0;
}
...
// This method never gets called. We are safe for any and every thing.
// There should be no possible way that this control could lose, destroy, or expose data.
STDMETHODIMP CPreview::GetInterfaceSafetyOptions(REFIID riid, DWORD *pdwSupportedOptions, DWORD *pdwEnabledOptions)
{
...
STDMETHODIMP CPreview::SetInterfaceSafetyOptions(REFIID riid, DWORD dwSupportedOptions, DWORD dwEnabledOptions)
{
    ...
    case VT_DISPATCH:
        ...
        if (var.punkVal)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                // We now have the folder item for the target, get it's context menu.
                // We look at the context menu to see if the item is printable.
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        for (--cVerbs; cVerbs >= 0; cVerbs--)
                        {
                            ...
                            if (SUCCEEDED(hr))
                            {
                                ...
                                if (SUCCEEDED(hr))
                                {
                                    // BUGBUG: should check against the canonical string "&Print" which will avoid all this load and convert to WCHAR crap
                                    WCHAR szPrint[256];
                ...
                // Now we need to know the path for this item.
                // We can only view items if we can get a path or URL to the target so some namespaces aren't viewable.
                BSTR bstr;
                ...
                // Finally we need to set the printable verb.
                // We do this after setting the path above to avoid the possibility of processing a print when we don't know the path.
                put_printable(bPrintable);

                pfi->Release();// now release the Folder Item pointer
                return hr;
            }
...
STDMETHODIMP CPreview::TranslateAccelerator(LPMSG lpmsg)
{
    ...
    {
        // REVIEW: looks like newer versions of ATL might do this for us so possibly we can replace w/ call to SUPER::TA when we upgrade.
