...
static int _GetOffsetFromID(HMENU hmMain, UINT uID)
{
    ...
    for (index = GetMenuItemCount(hmMain) - 1; index >= 0; index--)
    {
        ...
        if (GetMenuItemInfo(hmMain, (UINT) index, TRUE, &mii) && (mii.wID == FCIDM_MENU_VIEW_SEP_OPTIONS))
        {
...
BOOL CSFView::OnActivate(UINT uState)
{
    ...
    if (uState == SVUIA_ACTIVATE_FOCUS)
    {
        ...
        if ((HMENU) cmMerge)
        {
            ...
            Cab_MergeMenus(hmEdit, GetSubMenu(cmMerge, SUBMENU_EDIT), (UINT) -1, FCIDM_SHVIEWFIRST, FCIDM_SHVIEWLAST, MM_ADDSEPARATOR | MM_SUBMENUSHAVEIDS);
    ...
    {
        CMenuTemp cmMerge(LoadMenu(g_ThisDll.GetInstance(), MAKEINTRESOURCE(MENU_SFV_MAINMERGENF)));
        if ((HMENU) cmMerge)
        {
            Cab_MergeMenus(hmEdit, GetSubMenu(cmMerge, SUBMENUNF_EDIT), (UINT) -1, FCIDM_SHVIEWFIRST, FCIDM_SHVIEWLAST, MM_ADDSEPARATOR | MM_SUBMENUSHAVEIDS);
...
void CSFView::OnCommand(IContextMenu *pcm, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        LPDATAOBJECT pdtobj;
        if (!m_cView.OleInited() || FAILED(m_cView.GetUIObjectFromItem(IID_IDataObject, (LPVOID*) &pdtobj, SVGIO_SELECTION)))
        {
            MessageBeep(0);
            break;
        }
...
void CSFView::InitFileMenu(HMENU hmInit)
{
    ...
    if (pcmSel)
    {
        pcmSel->QueryContextMenu(hmInit, 0, SFV_CONTEXT_FIRST, SFV_CONTEXT_LAST, CMF_DVFILE);
        pcmSel->Release();
    }
...
void CSFView::InitEditMenu(HMENU hmInit)
{
    ...
    UINT uFlags = (m_cView.OleInited() && SUCCEEDED(m_cView.GetAttributesFromItem(&dwAttr, SVGIO_SELECTION)) && (dwAttr & SFGAO_CANCOPY)) ? MF_ENABLED : MF_GRAYED;
...
void CSFView::InitViewMenu(HMENU hmInit)
{
    ...
    CheckMenuRadioItem(hmInit, IDC_VIEW_ICON, IDC_VIEW_DETAILS, iCurViewMenuItem, MF_BYCOMMAND | MF_CHECKED);

    uEnable = (iCurViewMenuItem == IDC_VIEW_LIST + FCIDM_SHVIEWFIRST || iCurViewMenuItem == IDC_VIEW_DETAILS + FCIDM_SHVIEWFIRST) ? (MF_GRAYED | MF_BYCOMMAND) : (MF_ENABLED | MF_BYCOMMAND);
...
void CSFView::GetCommandHelpText(UINT idCmd, LPSTR pszText, UINT cchText, BOOL bToolTip)
{
    *pszText = 0;
    ...
    if (idCmd >= SFV_CONTEXT_FIRST && idCmd <= SFV_CONTEXT_LAST && m_pcmSel)
    {
        ...
        m_pcmSel->GetCommandString(idCmd - SFV_CONTEXT_FIRST, GCS_HELPTEXT, NULL, pszText, cchText);
    }
    ...
    {
        int iid = _FindIt(&c_idTbl[0].idCmd, idCmd - FCIDM_SHVIEWFIRST, sizeof(c_idTbl[0]), ARRAYSIZE(c_idTbl));
        if (iid >= 0)
        {
...
LRESULT CSFView::OnMenuSelect(UINT idCmd, UINT uFlags, HMENU hmenu)
{
    ...
    m_psb->SendControlMsg(FCW_STATUS, SB_SETTEXT, SBT_NOBORDERS | 255, (LPARAM) szHelpText, NULL);
