...
void PRIVATE ProcessBooleans(void)
    ...
    for (i = 0; i < ARRAYSIZE(s_rgbik); i++)
        ...
        dwcbKeyLen = GetPrivateProfileString(pbik->ikh.pszSectionName, pbik->ikh.pszKeyName, TEXT(""), szRHS, ARRAYSIZE(szRHS), c_szIniFile);
        if (dwcbKeyLen)
            lpcszRHS = szRHS;
        else
        ...
        if (IsIniYes(lpcszRHS))
            ...
            if (IsFlagClear(*(pbik->puStorage), pbik->dwFlag))
                DEBUG_MSG(TF_GENERAL, TEXT("ProcessIniFile(): %s set in %s![%s]."), pbik->ikh.pszKeyName, c_szIniFile, pbik->ikh.pszSectionName);
...
void PRIVATE ProcessIntegers(void)
    ...
    for (i = 0; i < ARRAYSIZE(s_rgiik); i++)
        ...
        dwcbKeyLen = GetPrivateProfileString(piik->ikh.pszSectionName, piik->ikh.pszKeyName, TEXT(""), szRHS, ARRAYSIZE(szRHS), c_szIniFile);
        if (dwcbKeyLen)
            lpcszRHS = szRHS;
        else
