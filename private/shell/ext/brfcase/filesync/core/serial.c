...
typedef enum _serialdebugflags
{
    ...
    ALL_SERIAL_DFLAGS = (SERIAL_DFL_BREAK_ON_PROCESS_ATTACH | SERIAL_DFL_BREAK_ON_THREAD_ATTACH)
}
SERIALDEBUGFLAGS;
...
{
    return(IS_VALID_READ_PTR(pcserctrl, CSERIALCONTROL) && (!pcserctrl->AttachProcess ||
        IS_VALID_CODE_PTR(pcserctrl->AttachProcess, AttachProcess)) && (!pcserctrl->DetachProcess ||
        IS_VALID_CODE_PTR(pcserctrl->DetachProcess, DetachProcess)) && (!pcserctrl->AttachThread ||
        IS_VALID_CODE_PTR(pcserctrl->AttachThread, AttachThread)) && (!pcserctrl->DetachThread ||
...
{
    return(IS_VALID_READ_PTR(pcpi, CPROCESSINFO) && IS_VALID_HANDLE(pcpi->hModule, MODULE));
}
...
** Side Effects:  none
*/
PRIVATE_CODE BOOL IsValidPCNONREENTRANTCRITICALSECTION(PCNONREENTRANTCRITICALSECTION pcnrcs)
{
...
PUBLIC_CODE BOOL DetachThread(HMODULE hmod)
{
    ...
    if (bResult)
    {
        TRACE_OUT((TEXT("DetachThread() called for module %#lx, thread ID %#lx."), hmod, GetCurrentThreadId()));
...
** Side Effects:  none
*/
PUBLIC_CODE void ReinitializeNonReentrantCriticalSection(PNONREENTRANTCRITICALSECTION pnrcs)
{
...
** Side Effects:  none
*/
PUBLIC_CODE BOOL EnterNonReentrantCriticalSection(PNONREENTRANTCRITICALSECTION pnrcs)
{
    BOOL bEntered;
    ...
    if (bBlocked)
        WARNING_OUT((TEXT("EnterNonReentrantCriticalSection(): Blocking thread %lx.  Critical section is already owned by thread %#lx."), GetCurrentThreadId(), pnrcs->dwOwnerThread));
#endif
    ...
    if (bEntered)
    {
        ...
        if (bBlocked)
            WARNING_OUT((TEXT("EnterNonReentrantCriticalSection(): Unblocking thread %lx.  Critical section is now owned by this thread."), pnrcs->dwOwnerThread));
#endif
    }
    else
    {
...
** Side Effects:  none
*/
PUBLIC_CODE void LeaveNonReentrantCriticalSection(PNONREENTRANTCRITICALSECTION pnrcs)
{
    ASSERT(IS_VALID_STRUCT_PTR(pnrcs, CNONREENTRANTCRITICALSECTION));
...
** Side Effects:  none
*/
PUBLIC_CODE BOOL NonReentrantCriticalSectionIsOwned(PCNONREENTRANTCRITICALSECTION pcnrcs)
{
