                                              ...
                                              LPTSTR pszValueNameBuf)
{
   ...
   do
   {
      ...
      lResult = RegQueryValueEx(hkeyBriefcases, pszValueNameBuf, NULL, NULL, NULL, NULL);
      switch (lResult)
      {
         case ERROR_SUCCESS:
            /* Used value name.  Continue searching. */
            TRACE_OUT((TEXT("GetUnusedBriefcaseValueName(): Found used briefcase value name %s."), pszValueNameBuf));
            break;
         case ERROR_FILE_NOT_FOUND:
            /* Unused value name.  Stop searching. */
            lResult = ERROR_SUCCESS;
            bFound = TRUE;
            TRACE_OUT((TEXT("GetUnusedBriefcaseValueName(): Found unused briefcase value name %s."), pszValueNameBuf));
            break;
         default:
            WARNING_OUT((TEXT("GetUnusedBriefcaseValueName(): RegQueryValueEx() failed, returning %ld."), lResult));
            break;
      }
   } while (lResult == ERROR_SUCCESS && ! bFound && dwValueNumber++ < DWORD_MAX);

   if (dwValueNumber == DWORD_MAX)
   {
      ASSERT(lResult == ERROR_SUCCESS && ! bFound);
...
PRIVATE_CODE TWINRESULT CreateBriefcaseIterator(PBRFCASEITER *ppbciter)
{
   ...
   lResult = RegOpenKeyEx(HKEY_BRIEFCASE_ROOT, BRIEFCASE_SUBKEY, 0, (KEY_QUERY_VALUE | KEY_SET_VALUE), &hkeyBriefcases);
   if (lResult == ERROR_SUCCESS)
   {
      DWORD dwcBriefcases;

      lResult = RegQueryInfoKey(hkeyBriefcases, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &dwcBriefcases, NULL, NULL, NULL);
      if (lResult == ERROR_SUCCESS)
      {
         if (dwcBriefcases > 0)
         {
...
PRIVATE_CODE TWINRESULT MyAddBriefcaseToSystem(PCLINKINFO pcli)
{
   ...
   lResult = RegCreateKeyEx(HKEY_BRIEFCASE_ROOT, BRIEFCASE_SUBKEY, 0, NULL, REG_OPTION_NON_VOLATILE, (KEY_QUERY_VALUE | KEY_SET_VALUE), NULL, &hkeyBriefcases, &dwDisposition);
   if (lResult == ERROR_SUCCESS)
   {
      TCHAR rgchValueName[MAX_VALUE_NAME_LEN];
      BOOL bFound;
      ...
      lResult = EnumBriefcases(hkeyBriefcases, &CompareLinkInfoProc, pcli, rgchValueName, &bFound);
      if (lResult == ERROR_SUCCESS)
      {
         if (bFound)
         ...
         {
            lResult = GetUnusedBriefcaseValueName(hkeyBriefcases, rgchValueName);
            if (lResult == ERROR_SUCCESS)
            {
               lResult = RegSetValueEx(hkeyBriefcases, rgchValueName, 0, REG_BINARY, (PCBYTE)pcli, pcli->ucbSize);
               if (lResult == ERROR_SUCCESS)
                  TRACE_OUT((TEXT("AddBriefcaseToSystem(): Briefcase database added to registry list as value %s under %s\\%s."), rgchValueName, HKEY_BRIEFCASE_ROOT_STRING, BRIEFCASE_SUBKEY));
            }
         }
      }
                                                ...
                                                PCLINKINFO pcliUpdated)
{
   ...
   if (lResult == ERROR_SUCCESS)
   {
      ...
      lResult = EnumBriefcases(hkeyBriefcases, &CompareLinkInfoProc, pcliOriginal, rgchValueName, &bFound);
      if (lResult == ERROR_SUCCESS)
      {
         if (bFound)
         {
            lResult = RegSetValueEx(hkeyBriefcases, rgchValueName, 0, REG_BINARY, (PCBYTE)pcliUpdated, pcliUpdated->ucbSize);
            if (lResult == ERROR_SUCCESS)
