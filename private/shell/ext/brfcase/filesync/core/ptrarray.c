...
{
   PA_FL_SORTED_ADD        = 0x0001,/* Insert elements in sorted order. */
   ALL_PA_FLAGS            = PA_FL_SORTED_ADD/* flag combinations */
}
PTRARRAYFLAGS;
...
{
   ARRAYINDEX aicPtrsToGrowBy;/* elements to grow array by after it fills up */
   DWORD dwFlags;/* array flags */
   PCVOID *ppcvArray;/* pointer to base of array */
   ARRAYINDEX aicPtrsAllocated;/* index of last element allocated in array */
   ...
   ARRAYINDEX aicPtrsUsed;/* number of elements used in array */
}
PTRARRAY;
...
** Side Effects:  none
*/
PRIVATE_CODE void PtrHeapSift(PPTRARRAY pa, ARRAYINDEX aiFirst, ARRAYINDEX aiLast, COMPARESORTEDPTRSPROC cspp)
{
   ARRAYINDEX ai;
   PCVOID pcvTemp;
   ...
   while (ai <= aiLast)
   {
      if (ai < aiLast && (*cspp)(ARRAY_ELEMENT(pa, ai), ARRAY_ELEMENT(pa, ai + 1)) == CR_FIRST_SMALLER)
         ai++;
...
** Side Effects:  none
*/
PRIVATE_CODE BOOL IsPtrArrayInSortedOrder(PCPTRARRAY pcpa, COMPARESORTEDPTRSPROC cspp)
{
   BOOL bResult = TRUE;
   ...
   if (pcpa->aicPtrsUsed > 1)
   {
      ...
      {
         if ((*cspp)(ARRAY_ELEMENT(pcpa, ai), ARRAY_ELEMENT(pcpa, ai + 1)) == CR_FIRST_LARGER)
         {
            bResult = FALSE;
...
PUBLIC_CODE void DestroyPtrArray(HPTRARRAY hpa)
{
   ...
   FreeMemory((PPTRARRAY)hpa);/* Free PTRARRAY structure. */
}
...
PUBLIC_CODE void DeletePtr(HPTRARRAY hpa, ARRAYINDEX aiDelete)
{
   ...
   ((PPTRARRAY)hpa)->aicPtrsUsed--;/* One less element used. */
}
...
*/
PUBLIC_CODE BOOL SearchSortedArray(HPTRARRAY hpa, COMPARESORTEDPTRSPROC cspp, PCVOID pcvTarget, PARRAYINDEX paiTarget)
{
   BOOL bFound;
...
*/
PUBLIC_CODE BOOL LinearSearchArray(HPTRARRAY hpa, COMPAREUNSORTEDPTRSPROC cupp, PCVOID pcvTarget, PARRAYINDEX paiTarget)
{
   BOOL bFound;
   ARRAYINDEX ai;

   ASSERT(IS_VALID_HANDLE(hpa, PTRARRAY) && (! cupp || IS_VALID_CODE_PTR(cupp, COMPPTRSPROC)) && IS_VALID_WRITE_PTR(paiTarget, ARRAYINDEX));
