...
{
    HPTRARRAY hpa;/* array of pointers to VOLUMEs */
    HSTRINGTABLE hst;/* table of volume root path strings */
    DWORD dwFlags;/* flags from RESOLVELINKINFOINFLAGS */
...
{
    ULONG ulcLock;/* reference count */
    DWORD dwFlags;/* bit mask of flags from VOLUMEFLAGS */
    VOLUMESTATE vs; /* volume state */
    PLINKINFO pli;/* pointer to LinkInfo structure indentifying volume */
    ...
    PVOLUMELIST pvlParent;/* pointer to parent volume list */
}
VOLUME;
...
{
    LONG lcVolumes;/* number of volumes in list */
    UINT ucbMaxLinkInfoLen;/* length of longest LinkInfo structure in volume list in bytes */
}
DBVOLUMELISTHEADER;
...
{
    HVOLUME hvol;/* old handle to volume */
...
*/
PRIVATE_CODE BOOL SearchForVolumeByRootPathCmp(PCVOID pcszFullPath, PCVOID pcvol)
{
    BOOL bDifferent;
...
** Side Effects:  none
*/
PRIVATE_CODE BOOL UnifyVolume(PVOLUMELIST pvl, PLINKINFO pliRoot, PVOLUME *ppvol)
{
    BOOL bResult = FALSE;
