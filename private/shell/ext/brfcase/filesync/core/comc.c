...
** Side Effects:  none
*/
PUBLIC_CODE LONG GetDefaultRegKeyValue(HKEY hkeyParent, LPCTSTR pcszSubKey, LPTSTR pszBuf, PDWORD pdwcbBufLen)
{
   LONG lResult;
   HKEY hkeySubKey;
   ...
   ASSERT(! pszBuf || IS_VALID_WRITE_BUFFER_PTR(pszBuf, STR, *pdwcbBufLen));
   ...
   if (lResult == ERROR_SUCCESS)
   {
      ...
      if (lResult == ERROR_SUCCESS)
      {
         ...
         ASSERT(! pszBuf || (DWORD)(lstrlen(pszBuf) + 1) * sizeof(TCHAR) == *pdwcbBufLen);

         TRACE_OUT((TEXT("GetDefaultRegKeyValue(): Default key value for subkey %s is \"%s\"."), pcszSubKey, pszBuf));
      }
      else
         TRACE_OUT((TEXT("GetDefaultRegKeyValue(): RegQueryValueEx() for subkey %s failed, returning %ld."), pcszSubKey, lResult));

      EVAL(RegCloseKey(hkeySubKey) == ERROR_SUCCESS);
   }
   else
      TRACE_OUT((TEXT("GetDefaultRegKeyValue(): RegOpenKeyEx() for subkey %s failed, returning %ld."), pcszSubKey, lResult));
...
PUBLIC_CODE BOOL IsDrivePath(LPCTSTR pcszFullPath)
{
   ...
   if (lstrlen(pcszFullPath) >= 3 && IsCharAlpha(pcszFullPath[0]) && pcszFullPath[1] == COLON && IS_SLASH(pcszFullPath[2]))
      bResult = TRUE;
   else
      bResult = FALSE;
...
PUBLIC_CODE BOOL IsValidDriveType(UINT uDriveType)
{
   ...
   switch (uDriveType)
   {
      ...
      case DRIVE_RAMDISK:
         ...
         break;
      default:
         ERROR_OUT((TEXT("IsValidDriveType(): Invalid drive type %u."), uDriveType));
         bResult = FALSE;
         break;
   }
