...
// Determine if we're running under Winstone 97 (at least whether we're in the state where Winstone messes up CSIDL_PERSONAL)
...
BOOL IsWinstone97()
{
    ...
    // Winstone97 sets CSIDL_PERSONAL in HKEY_LOCAL_MACHINE to be C:\wstemp\mydocs. if that is there we are running under winstone
...
{
    UnHideMyDocs();// delete the HideMyDocs value
    UpdateSendToFile(FALSE); // Add item to sendto directory
    SHChangeNotifyMyDocs(SHCNE_CREATE);// Tell the shell we're back...
}
    ...
    if (S_OK == SHGetFolderPath(NULL, CSIDL_SENDTO, NULL, SHGFP_TYPE_CURRENT, szSendToDir))
    {
        ...
        if (hFind != INVALID_HANDLE_VALUE)
        {
            do
            {
                ...
                {
                    // The file that we needed to create already exists, just leave it in place instead of deleting it and
                    // then creating it again below (this fixes app compat problems - see NT bug 246932)
                    bDeleteOnly = TRUE;
                }
                else
                {
...
// test pszChild against pszParent to see if pszChild is equal (PATH_IS_EQUAL) or a DIRECT child (PATH_IS_CHILD)
...
// Checks the path to see if it is marked as system or read only and then check desktop.ini for CLSID or CLSID2 entry...
...
BOOL IsPathAlreadyShellFolder(LPCTSTR pPath, DWORD dwAttrib)
{
    ...
    if ((lstrcmpi(szBuffer, TEXT("foo")) != 0) && (lstrcmpi(szBuffer, MYDOCS_CLSID) != 0))
    {
        return TRUE;
    }
...
BOOL PathEndsInDot(LPCTSTR pszPath)
{
    ...
    // will succeed but create a directory named "c:\foo", which isn't what the user asked for.
    // So we use this function to guard against those cases.

    // Note that this simple test also picks off "c:\foo\." -- ok for our purposes.
...
DWORD IsPathGoodMyDocsPath(HWND hwnd, LPCTSTR pPath)
{
    ...
    if (dwAttr == 0xFFFFFFFF)
    {
        ...
        {
            // If the root path doesn't exist, then we're not going to be able to create a path:
            return PATH_IS_ERROR;
        }
        else
        {
    ...
    // Make sure path isn't set as a system or some other kind of folder that already has a CLSID or CLSID2 entry...
...
// Remove our entries from the desktop.ini file in this directory, and then test the desktop.ini to see if it's empty.
// If it is, delete it and remove the system/readonly bit from the directory...
...
void MyDocsUnmakeSystemFolder(LPCTSTR pPath)
{
    ...
    WritePrivateProfileString(c_szShellInfo, TEXT("CLSID2"), NULL, szIniFile);// Remove CLSID2
    WritePrivateProfileString(c_szShellInfo, TEXT("InfoTip"), NULL, szIniFile);// Remove InfoTip
    WritePrivateProfileString(c_szShellInfo, TEXT("IconFile"), NULL, szIniFile);// Remove Icon
...
// Change CSIDL_PERSONAL to the specified path.
// If pszOld is NULL we assume we're creating CSIDL_PERSONAL from setup, and can therefore whack the registry directly.
...
HRESULT ChangePersonalPath(LPCTSTR pszNew, LPCTSTR pszOld, BOOL* pfResetMyPics)
{
    ...
    if (szOldPath[0])
    {
        ...
        pidlOld = ILCreateFromPath(szOldPath);// pidlOld may be NULL if the szOldPath does not exist
        MyDocsUnmakeSystemFolder(szOldPath);// Clean up old attributes and desktop.ini, if possible...
    }
...
void ResetMyPics(LPCTSTR pszOldMyPics)
{
    ...
    // 99/05/24 #341098 vtan: When the call to SetFolderPath() in InstallMyPictures() was removed for #316476 that introduced this bug.
    // Put back the functionality but make it specific
    // to ResetMyPics() which deliberately removes the entry so that the default install case doesn't mess with the USF registry value.
...
void InstallMyPictures()
{
    ...
    if (hr == S_OK)
    {
        ...
        // BUGBUG: Ideally, we could just write "mydocs.dll" or "%SystemRoot%\system32\mydocs.dll", but neither work, so we write the full path here:
        ...
        // NOTE: we need FCS_FORCEWRITE because we didn't used to specify iIconIndex and so "0" was written to the ini file.
        // When we upgrade, this API won't fix the ini file unless we pass FCS_FORCEWRITE
