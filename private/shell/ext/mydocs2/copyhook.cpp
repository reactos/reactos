...
// ICopyHook methods
UINT CMyDocsCopyHook::CopyCallback(HWND hwnd, UINT wFunc, UINT wFlags, LPCTSTR pszSrcFile,  DWORD dwSrcAttribs, LPCTSTR pszDestFile, DWORD dwDestAttribs)
{
    UINT uRes = IDYES;
    ...
    if ((wFunc == FO_COPY) || (wFunc == FO_MOVE))
    {
            ...
            lstrcmpi(pszSrcFile, szPersonal) == 0)
        {
            ...
            if (dwRes == PATH_IS_DESKTOP)
            {
                ...
                if (IsMyDocsHidden())
                {
                    if ( IDYES == ShellMessageBox(g_hInstance, hwnd, (LPTSTR)IDS_NODRAG_DESKTOP_HIDDEN, szPersonal, MB_YESNO | MB_ICONQUESTION, szPersonal, szVerb))
                    {
                        RestoreMyDocsFolder();
                    }
                }
                ...
                {
                    ShellMessageBox(g_hInstance, hwnd, (LPTSTR)IDS_NODRAG_DESKTOP_NOT_HIDDEN, szPersonal, MB_OK | MB_ICONSTOP, szPersonal, szVerb);
                }
            }
        }
    }
