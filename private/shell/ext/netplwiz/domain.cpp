...
// return a BSTR containing the user/domain name to present to a net call.
...
HRESULT _FindComputerInDomain(LPWSTR pszUserName, LPWSTR pszUserDomain, LPWSTR pszSearchDomain, LPWSTR pszPassword, BSTR *pbstrCompDomain)
{
    ...
    hres = ADsOpenObject(szBuffer, szDomainUser, pszPassword, ADS_SECURE_AUTHENTICATION, IID_IDirectorySearch, (void **)&pds);
    ...
    dwres = DsCrackNames(NULL, DS_NAME_FLAG_SYNTACTICAL_ONLY, DS_FQDN_1779_NAME, DS_CANONICAL_NAME, 1, &bstrX500DN, &pdnr);
...
BOOL SetAllowKey(DWORD dwNewValue, DWORD* pdwOldValue)
{
    ...
    if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_USERS, TEXT(".DEFAULT\\Software\\Microsoft\\RAS Logon Phonebook"), NULL, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkey, NULL)) {
        DWORD dwType = 0;
        // Read the previous value if required
        if (NULL != pdwOldValue) {
            DWORD cbSize = sizeof(DWORD);
            if (ERROR_SUCCESS != RegQueryValueEx(hkey, TEXT("AllowLogonPhonebookEdits"), NULL, &dwType, (LPBYTE)pdwOldValue, &cbSize)) {
        ...
        // Set the new value
        if (ERROR_SUCCESS == RegSetValueEx(hkey, TEXT("AllowLogonPhonebookEdits"), NULL, REG_DWORD, (CONST BYTE*) &dwNewValue, sizeof(DWORD))) {
...
void _DoUserInfoNext(HWND hwnd)
{
    ...
    case HRESULT_FROM_WIN32(ERROR_INVALID_DOMAINNAME):
    {
        ...
        ShellMessageBox(GLOBAL_HINSTANCE, hwnd, MAKEINTRESOURCE(IDS_ERR_BADDOMAIN), MAKEINTRESOURCE(IDS_USERINFO), MB_OK | MB_ICONWARNING, g_szDomain);
        break;
    }
    ...
    case HRESULT_FROM_WIN32(ERROR_BAD_USERNAME):
    {
        ...
        if (!fTranslateNameTriedAndFailed) {
            ...
            ShellMessageBox(GLOBAL_HINSTANCE, hwnd, MAKEINTRESOURCE(IDS_ERR_BADPWUSER), MAKEINTRESOURCE(IDS_USERINFO), MB_OK | MB_ICONWARNING);
            break;
        } else {
...
BOOL _IsTCPIPAvailable(void)
{
    ...
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("System\\CurrentControlSet\\Services\\Tcpip\\Linkage"), 0x0, KEY_QUERY_VALUE, &hk)) {
        ...
        if (0 != StrCmpI(szOldShortMachineName, szNewShortMachineName)) {
            ...
            if (fDomainRenameSucceeded) {
                ...
                if (_IsTCPIPAvailable()) {
                    ...
                    fLocalRenameSucceeded = ::SetComputerNameEx(ComputerNamePhysicalDnsHostname, szNewShortMachineName);
                } else {
...
#ifndef DONT_JOIN
    fNameChangeSucceeded = _ChangeMachineName(hwnd, szUser[0] ? szUser : NULL, szPassword[0] ? szPassword : NULL);
...
BOOL _AddUserToGroup(HWND hwnd, LPCTSTR pszLocalGroup, LPCWSTR pszUser, LPCWSTR pszDomain)
{
    ...
    case ERROR_NO_SUCH_MEMBER:
    {
        ...
        DisplayFormatMessage(hwnd, IDS_PERMISSIONS, IDS_ERR_NOSUCHUSER, MB_OK | MB_ICONWARNING, pszUser, pszDomain);
        break;
    }
    default:
    {
