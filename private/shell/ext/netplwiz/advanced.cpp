...
{
    // Create the message for the advanced dialog that informs the user of the purpose of the dialog, etc.
    ...
    Button_SetCheck(GetDlgItem(hwnd, IDC_COMPLETION_RECONNECT_CHECK), (m_pdata->fReconnect ? BST_CHECKED : BST_UNCHECKED));
...
BOOL CAdvancedDialog::OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    case IDOK:
        ...
        GetDlgItemText(hwnd, IDC_USER, m_pdata->szUsername, ARRAYSIZE(m_pdata->szUsername));

        GetDlgItemText(hwnd, IDC_PASSWORD, m_pdata->szPassword, ARRAYSIZE(m_pdata->szPassword));
...
//                    object should be printed
{
    ...
    gui.flUserGroupObjectPickerOtherDomains = UGOP_USERS;
    ...
    if (SUCCEEDED(hr) && hr != S_FALSE && pDsSelectionList)
    {
        ...
        if (NULL != pvarSid && (VT_ARRAY | VT_UI1) == V_VT(pvarSid))
        {
            ...
            if (SUCCEEDED(SafeArrayAccessData(V_ARRAY(pvarSid), &pSid)))
            {
                ...
                BOOL fSuccess = LookupAccountSid(NULL, pSid, szUser, &cchUser, szDomain, &cchDomain, &sUse);
                if (fSuccess)
                {
                    TCHAR szDomainUser[UNLEN + DNLEN + 2];
                    wnsprintf(szDomainUser, ARRAYSIZE(szDomainUser), TEXT("%s\\%s"), szDomain, szUser);
