...
void SetupDCInfo(NETPLACESWIZARDDATA* pdata)
{
    ...
    // We no longer do any DS stuff in the wizard. At least for now. Don't do all this useless work.
        ...
        // Get the name of a domain controller, preferably one that has a GC
    dwError = DsGetDcName(NULL, NULL, NULL, NULL, DS_GC_SERVER_REQUIRED, &pdata->pdcInfo);
    if (dwError == NO_ERROR) {
        ...
        // Try to get a DC name again, this time without preferring a GC
        dwError = DsGetDcName(NULL, NULL, NULL, NULL, 0, &pdata->pdcInfo);
    }
}
...
HPSXA AddExtraCplPages(ADDPROPSHEETDATA* ppsd)
{
    ...
    HPSXA hpsxa = SHCreatePropSheetExtArrayEx(HKEY_LOCAL_MACHINE, REGSTR_USERSANDPASSWORDS_CPL, MAX_EXTRA_CPL_PAGES, NULL);
    if (hpsxa != NULL) {
...
/*
 UsersRunDll
  ...
  pszCmdLine - The name of the "real" user of the machine. This can be different
                than the user context we are running the applet in. 
                We use this username to warn when we need to log off for changes to be effective.
*/
void APIENTRY UsersRunDll(HWND hwndStub, HINSTANCE hAppInstance, LPSTR pszCmdLine, int nCmdShow)
{
    ...
    *szDomainUser = 0;
    ...
    if (!ESI.ShouldExit()) {
        // Create the security check dialog to ensure the logged-on user is a local admin
