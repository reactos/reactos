...
HRESULT _AttemptJoin(HWND hwnd, DWORD dwFlags, LPCWSTR pszDomain, LPCWSTR pszUser, LPCWSTR pszUserDomain, LPCWSTR pszPassword)
{
    ...
    Trace(TEXT("pszDomain: %s, pszUser: %s, pszUserDomain: %s"), pszDomain, pszUser ? pszUser:TEXT("<NULL>"), pszUserDomain ? pszUserDomain:TEXT("<NULL>"));
    ...
    if ( (nas == ERROR_ACCESS_DENIED) )
    {
        ...
        // perhaps an account exists, but we can't delete it so try and remove the account create flag
...
// Handle moving from to a workgroup or domain.
// To do this we are passed a structure containing all the information we need.
HRESULT JoinDomain(HWND hwnd, BOOL fDomain, LPCWSTR pszDomain, CREDINFO* pci)
{
    ...
    Trace(TEXT("fDomain %d, pszDomain: %s, pszUser: %s, pszUserDomain: %s"), fDomain, pszDomain, (pci && pci->pszUser) ? pci->pszUser:TEXT("<NULL>"), (pci && pci->pszDomain) ? pci->pszDomain:TEXT("<NULL>"));

    // lets validate the domain name before we go and use it, therefore avoiding orphaning the computer too badly
    ...
    // now attempt to join the domain, prompt for credentails if the ones specified are not good enough
    ...
    if ( fDomain && ((FAILED(hres) || (!fPassedCredentials))) )
    {
        ...
        do
        {
            ...
            if ( IDCANCEL == DialogBoxParam(GLOBAL_HINSTANCE, MAKEINTRESOURCE(IDD_PSW_JOINCREDENTIALS), hwnd, _CredDlgProc, (LPARAM)pci) )
            {
...
// the clear text password stored in the registry to replace it with a password stored in the LSA secret space.
