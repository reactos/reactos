...
CUserInfo::CUserInfo()
{
    ...
    m_szUsername[0] = m_szDomain[0] = m_szGroups[0] = m_szComment[0] = m_szFullName[0] = m_szPasswordBuffer[0] = TEXT('\0');
...
HRESULT CUserInfo::ChangeLocalGroups(HWND hwndError, GROUPPSEUDONYM grouppseudonym)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (status == NERR_Success)
        {
            // We may now need to get the user's SID.
            // This happens if we are changing local groups for a domain user and we couldn't read their SID since they weren't in the local SAM.
            DWORD cchDomain = ARRAYSIZE(m_szDomain);
            hr = ::AttemptLookupAccountName(szDomainAndUser, &m_psid, m_szDomain, &cchDomain, &m_sUse);
            if (FAILED(hr))
            {
                ::DisplayFormatMessage(hwndError, IDS_USR_APPLET_CAPTION, IDS_USR_CREATE_MISC_ERROR, MB_ICONERROR | MB_OK);
            }
        }
        else
        {
