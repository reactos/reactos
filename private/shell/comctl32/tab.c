...
void FAR PASCAL VertDrawEdge(HDC hdc, LPRECT qrc, UINT edgeType, UINT grfFlags, BOOL fVert);
void FAR PASCAL VertPatBlt(HDC hdc1, int x1, int y1, int w, int h, DWORD rop, BOOL fVert);
...
void Tab_SmoothScrollWindow(PTC ptc, int dx, int dy, LPRECT lprcSrc, LPRECT lprcClip, HRGN hrgn, LPRECT lprcUpdate, UINT fuScroll, UINT uScrollMin)
{
    RECT rcSrc;
    RECT rcClip;
    if (Tab_Bottom(ptc)) {
...
void NEAR PASCAL Tab_ExtTextOut(HDC hdc, int x, int y, UINT uFlags, LPRECT prc, LPTSTR lpsz, UINT cch, CONST INT *pdw, PTC ptc)
{
    RECT rcTemp;
...
BOOL NEAR PASCAL BorrowOne(PTC ptc, int iCurLast, int iPrevLast, int iBorrow)
{
    ...
    if (iBorrow) {
        if (pitem->iRow > 1) {
            ...
            while (iPrevLast-- && Tab_FastGetItemPtr(ptc, iPrevLast)->iRow == (pitem->iRow - 1))
            {
                if (iPrevLast <= 0)
                {
                    // sanity check
...
void NEAR PASCAL FillLastRow(PTC ptc)
{
    ...
    for (iPrevLast = cItems - 2; Tab_FastGetItemPtr(ptc, iPrevLast)->iRow == ptc->iLastRow; iPrevLast--)
    {
        // sanity check
        if (iPrevLast <= 0)
        {
    ...
    while (iPrevLast &&  (hspace = BADNESS(ptc, cItems-1)) && (hspace > ((ptc->cxTabs/8) + BADNESS(ptc, iPrevLast))))
    {
...
BOOL NEAR Tab_OnSetItem(PTC ptc, int iItem, const TC_ITEM FAR* ptci)
{
    ...
    if (mask & TCIF_IMAGE) {
        if (pitem->iImage == -1 || ptci->iImage == -1) {
    ...
    if (mask & TCIF_STATE) {
        ...
        pitem->dwState = (ptci->dwState & ptci->dwStateMask) | (pitem->dwState & ~ptci->dwStateMask);
        ...
        if ((ptci->dwStateMask & TCIS_BUTTONPRESSED) && !(ptci->dwState & TCIS_BUTTONPRESSED)) {
    ...
    else
    {
        ...
        if (ptc->hwndArrows)
        {
            ...
            if (fSizeChanged || !IsWindowVisible(ptc->hwndArrows))
                SetWindowPos(ptc->hwndArrows, NULL, rc.left, rc.top, RECTWIDTH(rc), RECTHEIGHT(rc), SWP_NOACTIVATE | SWP_NOZORDER | SWP_SHOWWINDOW);
