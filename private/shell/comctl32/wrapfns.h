...
STRUCT_ENTRY(GetCurrentDirectory, DWORD, (DWORD nBufferLength, LPWSTR lpBuffer), (nBufferLength, lpBuffer))
...
STRUCT_ENTRY(RegOpenKeyEx, LONG, (HKEY hKey, LPCWSTR lpSubKey, DWORD ulOptions, REGSAM samDesired, PHKEY phkResult), (hKey, lpSubKey, ulOptions, samDesired, phkResult))
...
STRUCT_ENTRY(SendNotifyMessage, BOOL, (HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam), (hWnd, Msg, wParam, lParam))
...
STRUCT_ENTRY(GetTextExtentPoint32, BOOL, (HDC hdc, LPCWSTR pwsz, int cb, LPSIZE pSize), (hdc, pwsz, cb, pSize))
