...
#define HN_BEGINDIALOG        40    // Lens/EditText/garbage detection dialog is about to come up on this hedit/bedit
#define HN_ENDDIALOG          41    // Lens/EditText/garbage detection dialog has just been destroyed
...
void NEAR ListView_LazyCreateWinEvents(LV *plv, int iFrom, int iTo)
{
    ...
    if (iMin <= plv->iMSAAMin && iMax >= plv->iMSAAMin) {
        ListView_LazyCreateObjects(plv, iMin, plv->iMSAAMin);// Notify the low end.
        plv->iMSAAMin = iMin;// Extend the list of things we've notified.
        iMin = plv->iMSAAMax;// Remove it from the things left to be notified.
    }

    //  Now do the same thing to the top end.
    //  (This happens when scrolling forwards.)
    if (iMax >= plv->iMSAAMax && iMin <= plv->iMSAAMax) {
        ListView_LazyCreateObjects(plv, plv->iMSAAMax, iMax);// Notify the top end.
        plv->iMSAAMax = iMax;// Extend the list of things we've notified.
        iMax = plv->iMSAAMin;// Remove it from the things left to be notified.
    }

    //  If there are still things to be notified, then it means that the
    //  incoming range isn't contiguous with the previous range, so throw away the old range and just set it to the current range.
    //  (This happens when you grab the scrollbar and jump to a completely unrelated part of the listview.)
    if (iMin < iMax) {
        plv->iMSAAMin = iMin;
        plv->iMSAAMax = iMax;
...
BOOL NEAR ListView_SendChange(LV* plv, int i, int iSubItem, int code, UINT oldState, UINT newState, UINT changed, LPARAM lParam)
{
    NM_LISTVIEW nm;
...
void NEAR ListView_SendODChangeAndInvalidate(LV* plv, int iFrom, int iTo, UINT oldState, UINT newState)
{
    NM_ODSTATECHANGE nm;
...
BOOL NEAR ListView_GetEmptyText(LV* plv)
{
    ...
    // For each listview control, we will only send this notify once if necessary.
...
int CALLBACK ListView_SortCallback(LPVOID dw1, LPVOID dw2, LPARAM lParam)
{
    ...
    if (!pSortInfo->pfnCompare) {
        ...
        if (pszText1 != LPSTR_TEXTCALLBACK && pszText2 != LPSTR_TEXTCALLBACK )
        {
...
void ListView_RecalcRegion(LV* plv, BOOL fForce, BOOL fRedraw)
{
    ...
    if (!(plv->exStyle & LVS_EX_REGIONAL) || !ListView_RedrawEnabled(plv) || (plv->flags & LVF_INRECALCREGION))
        return;
    ...
    if ((ListView_Count(plv) > 0)) {
        ...
        if (!fForce) {
            for (i = 0; i < ListView_Count(plv); i++) {
                ...
                // the region.
                if ((pitem->pt.x != pitem->ptRgn.x) || (pitem->pt.y != pitem->ptRgn.y) || (!pitem->hrgnIcon) || !EqualRect((CONST RECT *)&pitem->rcTextRgn, (CONST RECT *)&rc))
                    goto changed;
            }
...
changed:;
        ...
        if (!(hdc = CreateCompatibleDC(NULL)) || (!(hbmp = CreateBitmap(cxIcon, cyIcon, 1, 1, NULL)))) {
            goto BailOut;
        }
...
HIMAGELIST CreateCheckBoxImagelist(HIMAGELIST himl, BOOL fTree, BOOL fUseColorKey, BOOL fMirror)
{
    ...
    DrawFrameControl(hdc, &rc, DFC_BUTTON, DFCS_BUTTONCHECK | DFCS_FLAT | (fUseColorKey? 0 : DFCS_TRANSPARENT));
    OffsetRect(&rc, cxImage, 0);
    ...
    DrawFrameControl(hdc, &rc, DFC_BUTTON, DFCS_BUTTONCHECK | DFCS_FLAT | DFCS_CHECKED | (fUseColorKey? 0 : DFCS_TRANSPARENT));
...
DWORD NEAR PASCAL ListView_ExtendedStyleChange(LV* plv, DWORD dwNewStyle, DWORD dwExMask)
{
    ...
    if ((dwOldStyle ^ dwNewStyle) & (LVS_EX_UNDERLINEHOT | LVS_EX_UNDERLINECOLD | LVS_EX_ONECLICKACTIVATE | LVS_EX_TWOCLICKACTIVATE | LVS_EX_SUBITEMIMAGES)) {
        InvalidateRect(plv->ci.hwnd, NULL, TRUE);
    }
    ...
    if ((dwOldStyle ^ dwNewStyle) & LVS_EX_REGIONAL) {
        if (g_fSlowMachine == -1) {
            ...
            SYSTEM_INFO si;
            GetSystemInfo(&si);
            g_fSlowMachine = (BOOL)((si.dwProcessorType == PROCESSOR_INTEL_386) || (si.dwProcessorType == PROCESSOR_INTEL_486));
#else
...
LRESULT NEAR PASCAL ListView_OnHelp(LV* plv, LPHELPINFO lpHelpInfo)
{
    ...
    if ((lpHelpInfo != NULL) && ((plv->ci.style & LVS_TYPEMASK) == LVS_REPORT) && (lpHelpInfo->iCtrlId == LVID_HEADER)) {
...
BOOL ListView_OnSetCursorMsg(LV* plv)
{
    if (plv->exStyle & (LVS_EX_ONECLICKACTIVATE|LVS_EX_TWOCLICKACTIVATE)) {
        if (plv->iHot != -1) {
            if (((plv->exStyle & LVS_EX_ONECLICKACTIVATE && plv->fOneClickOK)) || ListView_OnGetItemState(plv, plv->iHot, LVIS_SELECTED)) {
                if (!plv->hCurHot)
...
BOOL fShouldFirstClickActivate()
{
    ...
    if (!fInited) {
        long cb = 0;
        if (RegQueryValue(HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\NoFirstClickActivate"), NULL, &cb) == ERROR_SUCCESS)
            fActivate = FALSE;
        fInited = TRUE;
    }
...
void ListView_OnMouseMove(LV* plv, int x, int y, UINT uFlags)
{
    if (plv->exStyle & (LVS_EX_TRACKSELECT|LVS_EX_ONECLICKACTIVATE|LVS_EX_TWOCLICKACTIVATE)
        ...
        && (ChildOfActiveWindow(plv->ci.hwnd) || fShouldFirstClickActivate() || ChildOfDesktop(plv->ci.hwnd))) {
        int iItem;
        LV_HITTESTINFO ht;
        NMLISTVIEW nm;
        ...
        if (ht.iSubItem != 0) {
            ...
            if (!ListView_FullRowSelect(plv) && !(plv->ci.style & LVS_OWNERDRAWFIXED)) {
                iItem = -1;
                ht.flags = LVHT_NOWHERE;
            }
        }

        if (ht.flags & LVHT_NOWHERE || ht.flags & LVHT_ONITEMSTATEICON) {
...
void ListView_OnMouseHover(LV* plv, int x, int y, UINT uFlags)
{
    ...
    if (GetCapture() || !ChildOfActiveWindow(plv->ci.hwnd) || EditBoxHasFocus())
...
void NEAR ListView_OnSetWorkAreas(LV* plv, int nWorkAreas, LPRECT prc)
{
    ...
    if (!ListView_IsOwnerData(plv) && plv->nWorkAreas > 0 && ((plv->nWorkAreas  != nOldWorkAreas) || (!EqualRects(&plv->prcWorkAreas[0], &rcOldWorkAreas[0], nOldWorkAreas))))
    {
        int iItem;
        LISTITEM FAR * pitem;

        //  Subtle - ListView_Recompute cleans up all the RECOMPUTE icons, but in order to do that, it needs to have valid work area
        //  rectangles. So the call must happen after the CopyRect but before the loop that checks the icon positions.
        ListView_Recompute(plv);
...
InsertLostItemsArray:
        ...
        if ((plv->ci.style & LVS_AUTOARRANGE) && (ListView_IsSmallView(plv) || ListView_IsIconView(plv)))
            ListView_OnArrange(plv, LVA_DEFAULT);
    }
...
{
    return plv && (item >= 0) && ListView_IsIconView(plv) && (plv->flags & LVF_UNFOLDED) && (plv->iFocus == item);
}


BOOL ListView_IsItemUnfoldedPtr(LV *plv, LISTITEM *pitem)
{
    return plv && pitem && ListView_IsIconView(plv) && (plv->flags & LVF_UNFOLDED) && (pitem->state & LVIS_FOCUSED);
}
...
LRESULT ListView_OnNotify(LV* plv, WPARAM wParam, LPNMHDR pnmh)
{
    ...
    else if (plv->hwndToolTips && (plv->hwndToolTips == pnmh->hwndFrom))
    {
        ...
        case NM_CUSTOMDRAW:
        {
            ...
            if (plv->fPlaceTooltip && (pnm->nmcd.dwDrawStage == CDDS_PREPAINT || pnm->nmcd.dwDrawStage == CDDS_ITEMPREPAINT))
            {
                DWORD dwCustom = 0;
            ...
            if (plv->iTTLastHit != -1)
            {
                if (plv->fPlaceTooltip)
                {
                    ...
                    if (!ListView_IsIconView(plv))
                    {
                        ...
                        SetWindowPos(plv->hwndToolTips, HWND_TOP, rcLabel.left, rcLabel.top, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE | SWP_HIDEWINDOW);
                    }
                    else
                    {
                        SetWindowPos(plv->hwndToolTips, HWND_TOP, rcLabel.left, rcLabel.top, (rcLabel.right - rcLabel.left), (rcLabel.bottom - rcLabel.top), SWP_NOACTIVATE | SWP_HIDEWINDOW);
                    }
...
// Passing focus can cause the window to be destroyed (by the Explorer when renaming).
...
BOOL RectInRect(const RECT* prcOuter, const RECT* prcInner)
{
    ...
    return (prcOuter->left <= prcInner->left) && (prcOuter->top <= prcInner->top) && (prcOuter->right >= prcInner->right) && (prcOuter->bottom >= prcInner->bottom);
#else
    RECT rcDummy;
...
#define MAX_DRAG_RECT_HEIGHT 300
    ...
    pshdi->hbmpDragImage = CreateBitmap( pshdi->sizeDragImage.cx, pshdi->sizeDragImage.cy, GetDeviceCaps(hdcDragImage, PLANES), GetDeviceCaps(hdcDragImage, BITSPIXEL), NULL);
    if (pshdi->hbmpDragImage)
    {
        ...
        return 1;// We're passing back the created HBMP.
    }
...
//  rectangle so that the color change causes a refresh.
    ...
    case LVM_SETITEMTEXTA:
        ...
        return (LRESULT)ListView_OnSetItemTextA(plv, (int)wParam, ((LV_ITEMA *)lParam)->iSubItem, (LPCSTR)((LV_ITEMA FAR *)lParam)->pszText);
    ...
    case LVM_SETITEMSTATE:
        ...
        return (LRESULT)ListView_OnSetItemState(plv, (int)wParam, ((LV_ITEM FAR *)lParam)->state, ((LV_ITEM FAR *)lParam)->stateMask);
    case LVM_SETITEMTEXT:
        if (!lParam)
            return FALSE;
        return (LRESULT)ListView_OnSetItemText(plv, (int)wParam, ((LV_ITEM FAR *)lParam)->iSubItem, (LPCTSTR)((LV_ITEM FAR *)lParam)->pszText);
    ...
    case LVM_GETITEMPOSITION:
        return (LRESULT)ListView_OnGetItemPosition(plv, (int)wParam, (POINT FAR*)lParam);
    case LVM_SETITEMPOSITION:
        return (LRESULT)ListView_OnSetItemPosition(plv, (int)wParam, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
    case LVM_SETITEMPOSITION32:
        if (!lParam)
            return FALSE;
        return (LRESULT)ListView_OnSetItemPosition(plv, (int)wParam, ((LPPOINT)lParam)->x, ((LPPOINT)lParam)->y);
    case LVM_SCROLL:
    {
        int dx = (int)wParam;
        int dy = (int)lParam;
        return (LRESULT) (ListView_ValidateScrollParams(plv, &dx, &dy) && ListView_OnScroll(plv, dx, dy));
    }
    ...
    case LVM_SETCOLUMNWIDTH:
        return (LPARAM)ListView_ISetColumnWidth(plv, (int)wParam, GET_X_LPARAM(lParam), TRUE);
    ...
    case LVM_SETICONSPACING:
    {
        ...
        if (ListView_RedrawEnabled(plv) && ((plv->ci.style & LVS_AUTOARRANGE) && (ListView_IsSmallView(plv) || ListView_IsIconView(plv))))
        {
            // Call off to the arrange function.
            ListView_OnArrange(plv, LVA_DEFAULT);
        }
    ...
    case WM_SIZE:
        if (plv)
        {
            ...
            if ((plv->ulBkImageFlags & LVBKIF_SOURCE_MASK) && (plv->xOffsetPercent || plv->yOffsetPercent)) {
                InvalidateRect(plv->ci.hwnd, NULL, TRUE);
            }
        }
        break;
    ...
    default:
        ...
        if (uMsg == g_msgMSWheel) {
            ...
            if (fScroll) {
                if (g_ucScrollLines > 0 && cDetants != 0 && ((WS_VSCROLL | WS_HSCROLL) & (dwStyle = ListView_GetWindowStyle(plv)))) {
                    ...
                    // The size of a page is at least one line, and leaves one line of overlap
                    ...
                    ListView_ComOnScroll(plv, SB_THUMBTRACK, si.nPos + dPos, sb, cScrollUnitsPerLine, - 1);
                    ListView_UpdateScrollBars(plv);
                    ...
                    ListView_PopBubble(plv);
                    RelayToToolTips(plv->hwndToolTips, plv->ci.hwnd, WM_NCMOUSEMOVE, HTCLIENT, lParam);
                }
                return 1;
            } else if (fDataZoom) {
                LV_HITTESTINFO ht;
                ...
                // If we are rolling forward and we hit an item then navigate into that item (simulate dblclk which will open it).
                // Otherwise just fall through so it isn't handled.
                // In that case if we are being hosted in explorer it will do a backwards history navigation.
                if ((iWheelDelta > 0) && (ListView_OnSubItemHitTest(plv, &ht) >= 0) && (ht.flags & LVHT_ONITEM) && cDetants != 0) {
                    BYTE aKeyState[256];
                    // This is a bit yucky but when ListView_HandleMouse sends the notification to the listview owner we need to make sure that it doesn't think the shift key is down.
                    // Otherwise it may perform some "alternate" action but in this case we always want it to perform the default open action.

                    // Strip the high bit of VK_SHIFT so that the shift key is not down.
...
void NEAR ListView_OnWinIniChange(LV* plv, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((ListView_IsOwnerData( plv ) || (plv->ci.style & LVS_AUTOARRANGE)) && (ListView_IsSmallView(plv) || ListView_IsIconView(plv)))
    {
        // Call off to the arrange function.
        ListView_OnArrange(plv, LVA_DEFAULT);
    }
...
BOOL NEAR ListView_OnCreate(LV* plv, CREATESTRUCT FAR* lpCreateStruct)
{
    ...
    plv->hwndToolTips = CreateWindow(TOOLTIPS_CLASS, NULL, WS_POPUP|TTS_NOPREFIX, 0, 0, 0, 0, NULL, NULL, g_hinst, NULL);
    if ( plv->hwndToolTips )
    {
        TOOLINFO ti;
...
void NEAR ListView_OnNCDestroy(LV* plv)
{
    if ((!(plv->ci.style & LVS_SHAREIMAGELISTS)) || ListView_CheckBoxes(plv)) {
        if (plv->himlState && (plv->himlState != plv->himl) && (plv->himlState != plv->himlSmall))
        {
...
// this creats the brush for drawing the background as well as sets the imagelists background color if needed
...
void ListView_DrawBackground(LV *plv, HDC hdc, RECT *prcClip)
{
    ...
    // Clip the clipping region to the caller's rectangle, and save the final clipping rectangle in rcClip.
    if (prcClip != NULL)
    {
    ...
    // If we have an image to draw, go draw it and exclue it from the clipping region.
    if (plv->pImgCtx && plv->fImgCtxComplete)
    {
        ...
        case LVBKIF_STYLE_TILE:
            ...
            ExcludeClipRect(hdc, prcClip->left, prcClip->top, prcClip->right, prcClip->bottom);
            break;
...
void NEAR ListView_OnCommand(LV* plv, int id, HWND hwndCtl, UINT codeNotify)
{
    if (hwndCtl == plv->hwndEdit)
    {
        ...
        case EN_UPDATE:
#if defined(FE_IME) || !defined(WINNT)
            // We don't want flicker during replacing current selection as we use selection for IME composition.
        ...
        case EN_KILLFOCUS:
            ...
            // edit mode, which traps the user.  Owners need to give the user a way to get out.)

#if 0       // BUGBUG raymondc v6.0

            //  Fix horrible undocumented hanging problem:  LVN_ENDLABELEDIT is sent in response to EN_KILLFOCUS, which is send in response
            //  to WM_KILLFOCUS, and it is undocumented that you cannot display UI during WM_KILLFOCUS when a journal record hook is active,
            //  because the presence of a hook forces serialization of activation, and so when you put up UI, you generate activation changes, which
            //  get stuck because you haven't finished responding to the previous WM_KILLFOCUS message yet.
...
void NEAR ListView_OnWindowPosChanged(LV* plv, const WINDOWPOS FAR* lpwpos)
{
    if (!lpwpos || !(lpwpos->flags & SWP_NOSIZE))
    {
        ...
        if (ListView_IsOwnerData(plv) && (ListView_IsSmallView(plv) || ListView_IsIconView(plv)))
        {
...
void ListView_InvalidateSelectedOrCutOwnerData(LV* plv, ILVRange *plvrangeSel)
{
    ...
    if (plv->clrTextBk == CLR_NONE || (plv->himl && (plv->clrBk != ImageList_GetBkColor(plv->himl)))) {
...
void NEAR ListView_OnSetFocus(LV* plv, HWND hwndOldFocus)
{
    ...
    // due to the way listview call SetFocus on themselves on buttondown, the window can get a strange sequence of focus messages: first
    // set, then kill, and then set again.  since these are not really focus changes, ignore them and only handle "real" cases.

    // But still send out the accessibility notification because USER has already pushed focus back to the listview instead of to the focus item.
...
void NEAR ListView_DeselectAll(LV* plv, int iDontDeselect)
{
    ...
    if (ListView_IsOwnerData(plv)) {
        // if there's only one item selected, and that item is the iDontDeselect then our work is done...
        plv->plvrangeSel->lpVtbl->CountIncluded(plv->plvrangeSel, &plv->nSelected);
...
void NEAR ListView_SelectRangeTo(LV* plv, int iItem, BOOL fResetRest)
{
    ...
    // If we are in report view or list view we simply walk through the indexes to see which items to select or deselect. otherwise it
    // is is based off of the location of the objects being within the rectangle that is defined by
    if (ListView_IsListView(plv) || ListView_IsReportView(plv))
    {
    ...
    {
        // Iconic views first calculate the bounding rectangle of the two items.
        RECT    rcTemp;
        RECT    rcTemp2;
        RECT    rcBounding;
        POINT   pt;
        ...
        // all items are in order and we can search only those between the indexes found using the bouding rect
        if (ListView_IsOwnerData( plv ))
        {
...
int NEAR ListView_SetFocusSel(LV* plv, int iItem, BOOL fSelectAlso, BOOL fDeselectAll, BOOL fToggleSel)
{
   ...
   if (!ListView_IsOwnerData( plv )) {
    ...
    else
    {
        ...
        flags = ((fSelectAlso || plv->ci.style & LVS_SINGLESEL) ? (LVIS_SELECTED | LVIS_FOCUSED) : LVIS_FOCUSED);
...
void NEAR ListView_OnKey(LV* plv, UINT vk, BOOL fDown, int cRepeat, UINT flags)
{
    ...
    case VK_SPACE:
        ...
        if (fCtlDown && fShiftDown) {
            SendMessage(plv->ci.hwnd, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, (SendMessage(plv->ci.hwnd, LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0) ^ LVS_EX_GRIDLINES) | LVS_EX_CHECKBOXES);
        }
#endif
...
void NEAR ListView_OnStyleChanged(LV* plv, UINT gwl, LPSTYLESTRUCT pinfo)
{
    ...
    if (gwl == GWL_STYLE)
    {
        ...
        if (changeFlags & LVS_NOCOLUMNHEADER) {
            if (plv->hwndHdr) {
                SetWindowBits(plv->hwndHdr, GWL_STYLE, HDS_HIDDEN, (plv->ci.style & LVS_NOCOLUMNHEADER) ? HDS_HIDDEN : 0);
                fRedraw = TRUE;
                fShouldScroll = TRUE;
            }
        }
...
void NEAR ScrollDetect(LV* plv, POINT pt, int FAR *pdx, int FAR *pdy)
{
    ...
    *pdx = *pdy = 0;
    ...
    // listview.  we should bound the scroll amount in CanScroll() or ScrollAmount().
    if (*pdx || *pdy) {
...
void NEAR ListView_DragSelect(LV *plv, int x, int y)
{
    ...
    if (plv->exStyle & LVS_EX_REGIONAL) {
        if ((hrgnUpdate = CreateRectRgn(0,0,0,0)) && (hrgnLV = CreateRectRgn(0,0,0,0)) && (LockWindowUpdate(GetParent(hwnd)))) {
...
//          }
            ...
            if (ListView_IsReportView(plv))
            {
                i = (int)((plv->ptlRptOrigin.y + rcUnion.top  - plv->yTop) / plv->cyItem);
                iEnd = (int)((plv->ptlRptOrigin.y + rcUnion.bottom  - plv->yTop) / plv->cyItem) + 1;
            }

            else if (ListView_IsListView(plv))
            {
                i = ((plv->xOrigin + rcUnion.left)/ plv->cxItem) * plv->cItemCol + rcUnion.top / plv->cyItem;
                iEnd = ((plv->xOrigin + rcUnion.right)/ plv->cxItem) * plv->cItemCol + rcUnion.bottom / plv->cyItem + 1;
            }
            else
            {
                if (ListView_IsOwnerData( plv ))
            ...
            for (; i  < iEnd; i++) {
                ...
                if (msg32.wParam & MK_CONTROL) {
                    ...
                    if (bInNew && !(msg32.wParam & (MK_CONTROL | MK_SHIFT))) {
                        int dItem;
                        dItem = (rcTemp2.left - pt.x) * (rcTemp2.left - pt.x) + (rcTemp2.top - pt.y) * (rcTemp2.top - pt.y);
...
{
    if (ListView_FullRowSelect(plv) && ListView_IsReportView(plv) && !(plv->ci.style & LVS_SINGLESEL) && !ListView_OwnerDraw(plv) && plvhti->iSubItem == 0) {
...
void NEAR ListView_HandleMouse(LV* plv, BOOL fDoubleClick, int x, int y, UINT keyFlags, BOOL bMouseWheel)
{
    ...
    fActive = ChildOfActiveWindow(plv->ci.hwnd) || fShouldFirstClickActivate() || ChildOfDesktop(plv->ci.hwnd);
    ...
    if (fDoubleClick)
    {
        ...
        if (click == NM_DBLCLK)
        {
            ...
            if (ht.flags & (LVHT_ONITEMLABEL | LVHT_ONITEMICON))
            {
                ...
                if (!(plv->exStyle & LVS_EX_ONECLICKACTIVATE && plv->fOneClickOK) || (plv->exStyle & LVS_EX_ONECLICKACTIVATE &&  plv->fOneClickOK && (SHIFT_DOWN(keyFlags) || CONTROL_DOWN(keyFlags))))
                {
                    CCSendNotify(&plv->ci, LVN_ITEMACTIVATE, &nm.hdr);
                }
            }
    ...
    if (ht.flags & (LVHT_ONITEMLABEL | LVHT_ONITEMICON))
    {
        ...
        if (!bMouseWheel && CheckForDragBegin(plv->ci.hwnd, x, y))
        {
            ...
            if (ListView_RBeginMarquee(plv, x, y, &ht) && !CCSendNotify(&plv->ci, LVN_MARQUEEBEGIN, &nm.hdr))
            {
        ...
        else if (IsWindow(hwnd))
        {
            ...
            if (!SHIFT_DOWN(keyFlags) && !CONTROL_DOWN(keyFlags) && !RIGHTBUTTON(keyFlags))
            {
                ...
                if ((ht.flags & LVHT_ONITEMLABEL) && bSelected && !(plv->exStyle & (LVS_EX_ONECLICKACTIVATE|LVS_EX_TWOCLICKACTIVATE)))
                {
                    ...
                    if (fHadFocus || (plv->ci.style & LVS_OWNERDRAWFIXED)) {
                ...
                if (!RIGHTBUTTON(keyFlags))
                {
                    ...
                    if ((plv->exStyle & LVS_EX_ONECLICKACTIVATE && plv->fOneClickOK) || bSelected)
                    {
                        if (fActive)
                        {
                            ...
                            if ((bSelected && plv->exStyle & LVS_EX_TWOCLICKACTIVATE) || (bCond && !g_bUseDblClickTimer))
                            {
                                CCSendNotify(&plv->ci, LVN_ITEMACTIVATE, &nm.hdr);
                                if (!IsWindow(hwnd))
                                    return;
    ...
    else if (ht.flags & LVHT_NOWHERE)
    {
        ...
        // If single-select listview, disable marquee selection.
        if (!(plv->ci.style & LVS_SINGLESEL) && CheckForDragBegin(plv->ci.hwnd, x, y) && !CCSendNotify(&plv->ci, LVN_MARQUEEBEGIN, &nm.hdr))
        {
            if (!SHIFT_DOWN(keyFlags) && !CONTROL_DOWN(keyFlags))
                ListView_DeselectAll(plv, -1);
            ListView_DragSelect(plv, x, y);
            ...
            BOOL fDoFirstClickSelection = (fHadFocus || plv->ci.style & LVS_SHOWSELALWAYS || CONTROL_DOWN(keyFlags) || SHIFT_DOWN(keyFlags) || RIGHTBUTTON(keyFlags));
    ...
    // this needs to check the GetKeyState stuff only when we've gone into a modal loop waiting for the rbutton up.
...
BOOL NEAR ListView_OnScroll(LV* plv, int dx, int dy)
{
    ...
    {
        // Note: This function expects that dy is in number of lines
        // and we are working with pixels so do a conversion use some rounding up and down to make it right
        if (dy > 0)
            dy = (dy + plv->cyItem/2) / plv->cyItem;
        else
            dy = (dy - plv->cyItem/2) / plv->cyItem;
...
BOOL NEAR ListView_OnEnsureVisible(LV* plv, int i, BOOL fPartialOK)
{
    ...
    // other callers that do this need to do it outside so that they can know not to call us if there's not wait pending
...
void NEAR ListView_UpdateScrollBars(LV* plv)
{
    ...
    if ((plv->ci.style & LVS_NOSCROLL) || (!(ListView_RedrawEnabled(plv))))
        return;
    ...
    if (fRedraw)
    {
        ...
        if (!(plv->flags & LVF_REDRAW))
        {
            ...
            if (ListView_IsListView(plv) || ListView_IsReportView(plv))
            {
                ...
                {
                    // We may try to resize the column
                    if (!ListView_MaybeResizeListColumns(plv, plv->iFirstChangedNoRedraw, ListView_Count(plv)-1))
...
BOOL NEAR ListView_OnSetItem(LV* plv, const LV_ITEM FAR* plvi)
{
    ...
    if ((plv->ci.style & LVS_SINGLESEL) && (mask & LVIF_STATE) && (stateNew & LVIS_SELECTED)) {
        ...
        stateOld = pitem->state & plvi->stateMask;// Refresh the old state information
    }
    ...
    if (mask & LVIF_STATE)
    {
        ...
        if (change)
        {
            ...
            if (change & LVIS_CUT || plv->clrTextBk == CLR_NONE)
                rdwFlags |= RDW_ERASE;
    ...
    if (mask & LVIF_TEXT)
    {
        // need to do this now because we're changing the text so we need to get the rect of the thing before the text changes
        // but don't redraw the item we are currently painting
        if (plv->iItemDrawing != i)
        {
            ListView_InvalidateItemEx(plv, i, FALSE, RDW_INVALIDATE | RDW_ERASE, LVIF_TEXT);
        }
    ...
    if (maskChanged)
    {
        ...
        if (maskChanged & LVIF_STATE)
        {
            ...
            if (fSelected)
            {
                ...
                {
                    MyNotifyWinEvent((plv->nSelected == 1) ? EVENT_OBJECT_SELECTION : EVENT_OBJECT_SELECTIONADD, plv->ci.hwnd, OBJID_CLIENT, i+1);
                }
                else
                {
...
BOOL NEAR PASCAL ListView_OnSetItemState(LV* plv, int i, UINT data, UINT mask)
{
    ...
    // LVIS_SELECTED and data is zero it implies that we will deselect all items...
    if (ListView_IsOwnerData( plv ))
    {
        UINT uOldData = 0;
            ...
            if (mask & LVIS_CUT)
            {
                ...
                {  // clear selection
                    if (plv->plvrangeCut->lpVtbl->IsEmpty(plv->plvrangeCut) != S_OK) {
                        ...
                        // if nothing was selected, then there's nothing to clear no change.
                        mask &= ~ LVIS_CUT;
                    }
                }
        ...
        else
        {
            ...
            if ((mask & LVIS_SELECTED)) {
                if (((plv->plvrangeSel->lpVtbl->IsSelected(plv->plvrangeSel, i) == S_OK) ? LVIS_SELECTED : 0) ^ (data & LVIS_SELECTED))
                {
                    ...
                    // something actually changed (or else we wouldn't be in this if block
                    uOldData |= (LVIS_SELECTED & (mask ^ data));
                } else {
                    // nothing changed... so make the uOldData be the same for this bit else make this the same as
                    uOldData |= (LVIS_SELECTED & (mask & data));
                }
        ...
        if (i != -1) {
            ...
            // Now iterate over all of the items that match our criteria and set their new value.
...
// Rather than thunking to ListView_OnGetItemText, we let ListView_GetItemA do the work.
...
BOOL WINAPI ListView_OnSetBkImage(LV* plv, LPLVBKIMAGE pbi)
{
    ...
    case LVBKIF_SOURCE_URL:
        ...
        if (pszImage && pszImage[0])
        {
            ...
            if (((hmodOLE = GetModuleHandle(TEXT("OLE32"))) == NULL) || ((pfnCoCreateInstance = (HRESULT(*)(REFCLSID, LPUNKNOWN, DWORD, REFIID, LPVOID *))GetProcAddress(hmodOLE, "CoCreateInstance")) == NULL))
            {
            ...
            hr = pfnCoCreateInstance(&CLSID_IImgCtx, NULL, CLSCTX_INPROC_SERVER, &IID_IImgCtx, (LPVOID *)&plv->pImgCtx);
            if (FAILED(hr))
            {
...
BOOL WINAPI ListView_OnGetBkImageA(LV* plv, LPLVBKIMAGEA pbiA)
{
    ...
    if (!IsBadWritePtr(pbiA, sizeof(*pbiA)))
    {
        ...
        case LVBKIF_SOURCE_URL:
            ...
            {
                ConvertWToAN(plv->ci.uiCodePage, pbiA->pszImage, pbiA->cchImageMax, plv->pszBkImage, -1);
                fRet = TRUE;
            }
            break;
        default:
...
int NEAR ListView_GetCxScrollbar(LV* plv)
{
    ...
    if (((plv->exStyle & LVS_EX_FLATSB) == 0) || !FlatSB_GetScrollProp(plv->ci.hwnd, WSB_PROP_CXVSCROLL, &cx))
    {
        cx = g_cxScrollbar;
    }
...
int NEAR ListView_GetCyScrollbar(LV* plv)
{
    ...
    if (((plv->exStyle & LVS_EX_FLATSB) == 0) || !FlatSB_GetScrollProp(plv->ci.hwnd, WSB_PROP_CYHSCROLL, &cy))
    {
        cy = g_cyScrollbar;
    }
...
DWORD NEAR ListView_GetWindowStyle(LV* plv)
{
    ...
    if (((plv->exStyle & LVS_EX_FLATSB) == 0) || !FlatSB_GetScrollProp(plv->ci.hwnd, WSB_PROP_WINSTYLE, (LPINT)&dwStyle))
    {
...
int ListView_SetScrollInfo(LV *plv, int fnBar, LPSCROLLINFO lpsi, BOOL fRedraw)
{
    ...
    ASSERT((lpsi->fMask & (SIF_PAGE | SIF_RANGE)) == 0 || (lpsi->fMask & (SIF_PAGE | SIF_RANGE)) == (SIF_PAGE | SIF_RANGE));
...
void ListView_LRInvalidateBelow(LV* plv, int i, int fSmoothScroll)
{
    if (ListView_IsListView(plv) || ListView_IsReportView(plv)) {
        ...
        if (!ListView_RedrawEnabled(plv) || (ListView_IsReportView(plv) && (plv->pImgCtx != NULL)))
            fSmoothScroll = FALSE;
        ...
        if ((rcItem.top <= plv->sizeClient.cy) && (rcItem.left <= plv->sizeClient.cx))
        {
            rcItem.bottom = plv->sizeClient.cy;
...
int NEAR ListView_OnInsertItem(LV* plv, const LV_ITEM FAR* plvi)
{
    ...
    if (plv->ci.style & (LVS_SORTASCENDING | LVS_SORTDESCENDING) && !ListView_IsOwnerData( plv ))
    {
    ...
    if ( !ListView_IsOwnerData(plv))
    {
        ...
        UINT uSelMask = plvi->mask & LVIF_STATE ? (plvi->state & (LVIS_FOCUSED | LVIS_SELECTED)) : 0;
        UINT uSel = uSelMask;
...
BOOL NEAR ListView_OnDeleteItem(LV* plv, int iItem)
{
    ...
    if (!ListView_IsOwnerData(plv))
    {
        ...
        // As the Z-order hdpa is a set of indexes we also need to decrement all indexes that exceed the one we are deleting.
...
int NEAR ListView_Arrow(LV* plv, int iStart, UINT vk)
{
    ...
    // For the sorted views we will use some optimizations to make it faster
    iCount = ListView_Count(plv);
    if (ListView_IsReportView(plv) || ListView_IsListView(plv))
    {
        ...
        case VK_NEXT:
            if (ListView_IsReportView(plv))
            {
                ...
                iStart = (int)(((LONG)(plv->sizeClient.cy - (plv->cyItem) - plv->yTop) + plv->ptlRptOrigin.y) / plv->cyItem);

                // If Same item, increment by page size.
                if (iStart <= i)
                    iStart = i + max((plv->sizeClient.cy - plv->yTop)/ plv->cyItem - 1, 1);
    ...
    else
    {
        ...
        if (ListView_IsOwnerData( plv ))
        {
          iStart = max( 0, iStart );
            ...
            if ( vk != VK_LEFT  && vk != VK_RIGHT && vk != VK_UP && vk != VK_DOWN && vk != VK_HOME && vk != VK_END && vk != VK_NEXT && vk != VK_PRIOR )
            {
                return -1;
            }
...
int NEAR ListView_OnGetNextItem(LV* plv, int i, UINT flags)
{
    ...
    if (flags & LVNI_FOCUSED)
    {
        ...
        if (!(flags & (LVNI_ABOVE | LVNI_BELOW | LVNI_TORIGHT | LVNI_TOLEFT)))
        {
            ...
            i = plv->iFocus - 1;// subtract one here -- we increment it below
        }
    }
    ...
    while (TRUE)
    {
        ...
        if (flags & ~(LVNI_ABOVE | LVNI_BELOW | LVNI_TORIGHT | LVNI_TOLEFT))
        {
            ...
            else
            {
                ...
                if (((flags & LVNI_SELECTED) && !(wItemState & LVIS_SELECTED)) || ((flags & LVNI_CUT) && !(wItemState & LVIS_CUT)) || ((flags & LVNI_DROPHILITED) && !(wItemState & LVIS_DROPHILITED))) {
                    if (i != iStart)
                        continue;
                    else {
    ...
    else
    {
        ...
        else // if (flags & (LVFI_STRING | LVFI_SUBSTRING | LVFI_PARTIAL))
        {
            ...
            for (j = cItem; j-- != 0; )
            {
                ...
                if (ListView_CompareString(plv, i, pszFind, (flags & (LVFI_PARTIAL | LVFI_SUBSTRING)), 0) == 0)
                {
                    return i;
                }
            }
...
BOOL NEAR ListView_OnGetItemRect(LV* plv, int i, RECT FAR* prc)
{
    ...
    pRects[prc->left] = prc;
    ListView_GetRects(plv, i, pRects[LVIR_ICON], pRects[LVIR_LABEL], pRects[LVIR_BOUNDS], pRects[LVIR_SELECTBOUNDS]);
    return TRUE;
}
...
void NEAR ListView_GetRects(LV* plv, int iItem, RECT FAR* prcIcon, RECT FAR* prcLabel, RECT FAR* prcBounds, RECT FAR* prcSelectBounds)
{
    ASSERT(plv);
...
void NEAR ListView_GetRectsOwnerData(LV* plv, int iItem, RECT FAR* prcIcon, RECT FAR* prcLabel, RECT FAR* prcBounds, RECT FAR* prcSelectBounds, LISTITEM* pitem)
{
    ASSERT(plv);
    ASSERT(ListView_IsOwnerData(plv));
    ...
    else
    {
      ...
      RECT rcTextBounds;
        ...
        // Don't need to check for folding here, as will have been handled in user data rectangle fetching functions.
...
void NEAR ListView_InvalidateItemEx(LV* plv, int iItem, BOOL fSelectionOnly, UINT fRedraw, UINT maskChanged)
...
//                      columns in invalidation if in details view
{
    ...
    if (ListView_IsOwnerData(plv) && plv->flags & LVF_CUSTOMFONT && !ListView_FullRowSelect(plv)) {
    ...
    if (ListView_RedrawEnabled(plv)) {
        ListView_GetRects(plv, iItem, prcIcon, prcLabel, prcBounds, prcSelectBounds);
...
UINT LV_IsItemOnViewEdge(LV* plv, LISTITEM* pitem)
// this returns BF_* flags to indicate which if any edge the item I is touching or crossing...
{
    RECT rcItem;
    ...
    _ListView_GetRectsFromItem(plv, ListView_IsSmallView(plv), pitem, NULL, NULL, &rcItem, NULL);
...
void LV_AdjustViewRectOnMove(LV* plv, LISTITEM *pitem, int x, int y)
{
    ...
    if (!ListView_IsOwnerData( plv )) {
        if ((plv->rcView.left != RECOMPUTE) && x != RECOMPUTE && y != RECOMPUTE && pitem->cyFoldedLabel != SRECOMPUTE) {
            RECT rcAfter;
            RECT rcView = plv->rcView;
            ...
            if (pitem->pt.x != RECOMPUTE) {
                ...
                // before and after the move, they need to be touching the same edges or not at all
            ...
            _ListView_GetRectsFromItem(plv, ListView_IsSmallView(plv), pitem, NULL, NULL, &rcAfter, NULL);
            // translate from window coordinates to listview coordinate
            ...
            DebugMsg(TF_LISTVIEW, TEXT("Score! (%d %d %d %d) was (%d %d %d %d)"),
                     rcView.left, rcView.top, rcView.right, rcView.bottom, plv->rcView.left, plv->rcView.top, plv->rcView.right, plv->rcView.bottom);
    ...
    DebugMsg(TF_LISTVIEW, TEXT("LV -- AdjustViewRect pitem %d -- (%x, %x)"), pitem, pitem->pt.x, pitem->pt.y);
...
BOOL NEAR ListView_OnSetItemPosition(LV* plv, int i, int x, int y)
{
    ...
    DebugMsg(TF_LISTVIEW, TEXT("LV -- On SetItemPosition %d %d %d %d -- (%x, %x)"),
             plv->rcView.left, plv->rcView.top, plv->rcView.right, plv->rcView.bottom, pitem->pt.x, pitem->pt.y);
...
BOOL NEAR ListView_OnGetItemPosition(LV* plv, int i, POINT FAR* ppt)
{
    ...
    if (ListView_IsListView(plv) || ListView_IsReportView(plv) || ListView_IsOwnerData( plv ))
    {
        RECT rcIcon;
...
void NEAR ListView_Redraw(LV* plv, HDC hdc, RECT FAR* prcClip)
{
    ...
    if (!(plv->ci.dwCustom & CDRF_SKIPDEFAULT)) {
        ...
        // by calculating the index of the first item that may need painting...
        switch (dwType) {
        case LVS_REPORT:
        ...
        for (; i < cItem; i++)
        {
            ...
            if ((dwType == LVS_ICON || dwType == LVS_SMALLICON) && (!ListView_IsOwnerData(plv)))
            {
                LISTITEM FAR *pitem;
                ...
                // by converting the hdpaZorder into a list of indexes instead of pointers.
                // This ovoids the costly convert pointer to index call.
                ...
                // for small icon view we cant clip on the left without getting the text

                // for large icon view we cant clip on the top without getting the text
        ...
        if ((dwType == LVS_ICON || dwType == LVS_SMALLICON) && (ListView_IsOwnerData(plv)) && plv->iFocus != -1) {
        ...
        if (ListView_Count(plv) == 0)
        {
            ...
            if (ListView_GetEmptyText(plv))
            {
                ...
                SHDrawText(hdc, plv->pszEmptyText, &rcClip, LVCFMT_LEFT, flags, plv->cyLabelChar, plv->cxEllipses, plv->clrText, plv->clrBk);
            }
        }
...
BOOL NEAR ListView_DrawItem(PLVDRAWITEM plvdi)
{
    ...
    if (!(plvdi->flags & LVDI_NOWAYFOCUS))
    {
        if (plvdi->plv->flags & LVF_FOCUSED) {
            ...
            if (!plvdi->pitem || (plvdi->plv->stateCallbackMask & (LVIS_SELECTED | LVIS_FOCUSED))) {
                state = (WORD) ListView_OnGetItemState(plvdi->plv, (int) plvdi->nmcd.nmcd.dwItemSpec, LVIS_SELECTED | LVIS_FOCUSED);
            } else {
    ...
    if (plvdi->plv->iHot == (int)plvdi->nmcd.nmcd.dwItemSpec) {
        ...
        if ((plvdi->plv->exStyle & LVS_EX_ONECLICKACTIVATE) || ((plvdi->plv->exStyle & LVS_EX_TWOCLICKACTIVATE) && ListView_OnGetItemState(plvdi->plv, (int) plvdi->nmcd.nmcd.dwItemSpec, LVIS_SELECTED))) {
            if ((plvdi->plv->exStyle & LVS_EX_UNDERLINEHOT) && (plvdi->plv->hFontHot))
    ...
    } else if ((plvdi->plv->exStyle & LVS_EX_ONECLICKACTIVATE) || ((plvdi->plv->exStyle & LVS_EX_TWOCLICKACTIVATE) && ListView_OnGetItemState(plvdi->plv, (int) plvdi->nmcd.nmcd.dwItemSpec, LVIS_SELECTED))) {
...
void WINAPI SHDrawText(HDC hdc, LPCTSTR pszText, RECT FAR* prc, int fmt, UINT flags, int cyChar, int cxEllipses, COLORREF clrText, COLORREF clrTextBk)
// NOTE: this function requires a properly selected font.
{
    ...
    if ((flags & SHDT_ELLIPSES) && ListView_NeedsEllipses(hdc, pszText, &rc, &cchText, cxEllipses))
    {
    ...
    else
    {
        ...
        else if (clrText == CLR_DEFAULT && clrTextBk == CLR_DEFAULT)
        {
            ...
            if ( ( flags & (SHDT_DRAWTEXT | SHDT_DESELECTED) ) == (SHDT_DRAWTEXT | SHDT_DESELECTED) )
            {
                hbrUse = g_hbrWindow;
            }
        }
...
*/
{
    ...
    if (ListView_IsIconView(plv)) {
        ListView_UnfoldRects(plv, iItem, &rcImage, &rcLabel, &rcBounds, &rcSelBounds);
...
// For list view and report view this calculates the actual index for iconic views it alway returns 0
{
    if (ListView_IsReportView(plv))
...
// ListView_OnGetCountPerPage -- Gets the count of items that will fit on a page For list view and report view this calculates the
// count depending on the size of the window and for Iconic views it will always return the count of items in the list view.
{
    if (ListView_IsReportView(plv))
        return (plv->sizeClient.cy - plv->yTop) / plv->cyItem;

    else if (ListView_IsListView(plv))
        return ((plv->sizeClient.cx)/ plv->cxItem) * plv->cItemCol;
    else
...
BOOL NEAR ListView_UnfoldRects(LV* plv, int iItem, RECT * prcIcon, RECT * prcLabel, RECT * prcBounds, RECT * prcSelectBounds)
/*
...
*/
{
    ...
    if ( prcLabel )
    {
        if ( !ListView_IsOwnerData(plv) )
        {
            ...
            if (!EVAL(pitem)) {
                // DavidShi was able to get us into here with an invalid item number during a delete notification.
                // So if the item number is invalid, just return a blank rectangle instead of faulting.
