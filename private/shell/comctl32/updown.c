...
void NEAR PASCAL unachor(PUDSTATE np)
{
    ...
    if ( np->hwndBuddy && (np->ci.style & (UDS_ALIGNLEFT | UDS_ALIGNRIGHT))) {
        ...
        MoveWindow(np->hwndBuddy, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, FALSE);
    }
}
...
void NEAR PASCAL anchor(PUDSTATE np)
{
    ...
    if (bAlignToBuddy)
    {
        if ((np->uClass == CLASS_EDIT) || (GetWindowLong(np->hwndBuddy, GWL_EXSTYLE) & WS_EX_CLIENTEDGE))
        {
    ...
    if (bAlignToBuddy)
    {
        ...
        MoveWindow(np->hwndBuddy, rcBuddy.left, rcBuddy.top, rcBuddy.right - rcBuddy.left, nHeight, TRUE);
    }
...
void NEAR PASCAL setint(PUDSTATE np)
{
    ...
    if (np->hwndBuddy && np->ci.style & UDS_SETBUDDYINT)
    {
        ...
        if (np->uClass == CLASS_LISTBOX)
        {
            ...
            FORWARD_WM_COMMAND(GetParent(np->hwndBuddy), GetDlgCtrlID(np->hwndBuddy),
...
void NEAR PASCAL PaintUpDownControl(PUDSTATE np, HDC hdc)
{
    ...
    {
        UINT bf = BF_TOP | BF_BOTTOM | BF_ADJUST | (np->ci.style & UDS_ALIGNLEFT ? BF_LEFT : 0) | (np->ci.style & UDS_ALIGNRIGHT ? BF_RIGHT : 0);
...
LRESULT CALLBACK UpDownWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (np) {
        if ((uMsg >= WM_MOUSEFIRST) && (uMsg <= WM_MOUSELAST) && (np->ci.style & UDS_HOTTRACK) && !np->fTrackSet) {
            TRACKMOUSEEVENT tme;
...
EndScroll:
            ...
            if (np->uClass == CLASS_EDIT)
                Edit_SetSel(np->hwndBuddy, 0, -1);
                        ...
                        if (np->ci.style & UDS_HORZ)
                            FORWARD_WM_HSCROLL(np->ci.hwndParent, np->ci.hwnd, SB_ENDSCROLL, np->nPos, SendMessage);
                        else
                            FORWARD_WM_VSCROLL(np->ci.hwndParent, np->ci.hwnd, SB_ENDSCROLL, np->nPos, SendMessage);
        }
        break;
    case WM_ENABLE:
    ...
    case WM_WININICHANGE:
        if (np && (!wParam || (wParam == SPI_SETNONCLIENTMETRICS) || (wParam == SPI_SETICONTITLELOGFONT))) {
