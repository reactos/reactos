...
--*/
{
    ...
    ioResReqList = (PIO_RESOURCE_REQUIREMENTS_LIST)ExAllocatePool(PagedPool, sizeof(IO_RESOURCE_REQUIREMENTS_LIST) + count * sizeof(IO_RESOURCE_DESCRIPTOR));
    if (!ioResReqList) {
        return NULL;
    }
...
--*/
{
    ...
    for (alt=0; alt < InCompleteList->AlternativeLists; alt++) {
        ...
        for (cnt = InResourceList->Count; cnt; cnt--) {
            ...
            i -= 1;// take one off for the original which is already accounted for in 'len'
            Pos.InDesc++;// Next descriptor
        }

        InResourceList  = (PIO_RESOURCE_LIST) Pos.InDesc;// Next Resource List
    }
