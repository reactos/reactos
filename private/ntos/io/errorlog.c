...
Routine Description:
    ...
    system process context.  This routine is started when the system is initialized.
Arguments:
...
--*/
{
    ...
    if (!IopErrorLogConnectPort()) {
        // The port could not be connected.  A timer was started that will try again later.
        return;
    }

    // Allocate and zero the port message structure, include space for the name of the device and driver.
    ...
    // Now enter the main loop for this thread.  This thread performs the following operations:
    ...
    //       cleanup, place the packet back onto the head of the queue and return.
    //   3)  After all the packets are sent clear the pending variable and return.
    for (;;) {
        // Loop dequeueing  packets from the queue head and attempt to send each to the port.
        ...
        // place the packet back onto the head of the queue, and start from the top of the loop again.
        ...
        // IO_ERROR_LOG_PACKET. Using the above calculations set the message length.
        ...
        if (errorData->NumberOfStrings != 0) {
            // Start the driver and device strings where the current strings start.
            objectName = (PCHAR)(&errorMessage->EntryData) + errorData->StringOffset;
        } else {
            // Put the driver and device strings at the end of the data.
        ...
        if (driverObject != NULL) {
            ...
            // ie, ntdll, we do this by claiming this event is an application popup.
        ...
        if (driverNameLength != 0) {
            ...
            // Save space for 3 NULLs one for the driver name, one for the device name and one for strings.
            if (driverNameLength > remainingLength - (3 * sizeof(WCHAR))) {
                driverNameLength = remainingLength - (3 * sizeof(WCHAR));
            }

            RtlMoveMemory(objectName, string, driverNameLength);
        }

        // Add a null after the driver name even if there is no driver name.
        *((PWSTR)(objectName + driverNameLength)) = L'\0';
        ...
        RtlMoveMemory(objectName, nameInformation->Name.Buffer, deviceNameLength);

        // Add a null after the device name even if there is no device name.
        *((PWSTR)(objectName + deviceNameLength)) = L'\0';
        ...
        if (errorData->NumberOfStrings) {
            ...
            RtlMoveMemory(objectName, (PCHAR)errorData + errorData->StringOffset, stringLength);
        ...
        if (!NT_SUCCESS(status)) {
            ...
            // Finally, exit the loop since there is no connection to do any work on.
            ...
            // The send worked fine.  Free the packet and the update the allocation count.
            ExInterlockedAddUlong(&IopErrorLogAllocation, (ULONG)(-errorLogEntry->Size), &IopErrorLogAllocationLock);

            // Dereference the object pointers now that the name has been captured.
            if (errorLogEntry->DeviceObject != NULL) {
    ...
    ExFreePool(portMessage);// Finally, free the message buffer and return.
}
...
--*/
{
    ...
    if (ErrorLogPortConnected) {        
        return(TRUE);// The port is connect return.
    }
    ...
    return(FALSE);// The port could not be connected at this time return false.
}
...
Arguments:
    Dpc - Supplies a pointer to the DPC structure.  This structure is freed by this routine.
...
/*++
Routine Description:
    This routine gets the next entry from the head of the error log queue and returns it to the caller.
Arguments:
    None.
Return Value:
    The return value is a pointer to the packet removed, or NULL if there were no packets on the queue.
--*/
{
    KIRQL irql;
    PLIST_ENTRY listEntry;
    ...
    } else {// Remove the next packet from the head of the list.
...
Routine Description:
    ...
    DPC which then queues a worker thread request to run the error log thread routine.
--*/
{
    ...
    if (context == NULL) {
        ...
        // error log packet is currently being queue, it waits with the others.
        IopErrorLogPortPending = FALSE;
        return;
    }
    ...
    interval.QuadPart = -10 * 1000 * 1000 * 30;// Delay for 30 seconds and try for the port again.    
    KeSetTimer(&context->ErrorLogTimer, interval, &context->ErrorLogDpc);// Set the timer to fire a DPC in 30 seconds.
}
...
/*++
Routine Description:
    This routine puts an error packet back at the head of the error log queue since it cannot be processed at the moment.
Arguments:
    ListEntry - Supplies a pointer to the packet to be placed back onto the error log queue.
--*/
{
    KIRQL irql;

    // Simply insert the packet back onto the head of the queue, indicate that
    // the error log port is not connected, queue a request to check again soon, and return.
    ExAcquireSpinLock(&IopErrorLogLock, &irql);
