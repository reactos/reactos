...
--*/
{
    ...
    status = IopCreateRegistryKeyEx( &deviceHandle, rootHandle, &unicodeString, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, NULL);
    ZwClose(rootHandle);
    if (!NT_SUCCESS(status)) {
        if (resourceList) {
            ExFreePool (resourceList);
    ...
    status = IopCreateRegistryKeyEx( &instanceHandle, deviceHandle, &unicodeString, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, &disposition );
    ZwClose(deviceHandle);
    if (NT_SUCCESS(status))  {
        if (disposition == REG_CREATED_NEW_KEY) {
            RtlInitUnicodeString( &unicodeString, L"DeviceDesc" );
            swprintf(buffer, L"%s", L"Device to report Eisa Slot Resources");
            ZwSetValueKey(instanceHandle, &unicodeString, 0, REG_SZ, buffer, (wcslen(buffer) + 1) * sizeof(WCHAR));
            ...
            ZwSetValueKey(instanceHandle, &unicodeString, 0, REG_MULTI_SZ, buffer, (wcslen(buffer) + 2) * sizeof(WCHAR));
        ...
        status = IopCreateRegistryKeyEx( &logConfHandle, instanceHandle, &unicodeString, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, NULL);
    ...
    *ResourceLength = 0;
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            if ((keyValueInformation->DataLength >= sizeof(CM_FULL_RESOURCE_DESCRIPTOR)) && (resourceDescriptor->PartialResourceList.Count > 0) ) {
                LONG eisaInfoLength;
                ...
                if (partialResourceDescriptor->Type == CmResourceTypeDeviceSpecific) {
                    eisaInfo = (PCM_EISA_SLOT_INFORMATION) ((PUCHAR)partialResourceDescriptor + sizeof(CM_PARTIAL_RESOURCE_DESCRIPTOR));
                    ...
                    for (i = 0; i < 0x10 && eisaInfoLength > 0; i++) {
                        ...
                        if (eisaInfo->ReturnCode == EISA_EMPTY_SLOT) {
                            ...
                            eisaInfo = (PCM_EISA_SLOT_INFORMATION) ((PUCHAR)eisaInfo + eisaInfo->NumberFunctions * sizeof(CM_EISA_FUNCTION_INFORMATION) + sizeof(CM_EISA_SLOT_INFORMATION));
                        }
                    }
