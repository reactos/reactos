...
#define UNICODE_REGISTRY_PATH_LENGTH 1024
    ...
    // Set up a string with the pathname to the hardware description portion of the registry.
    registryPathName.Length = 0;
    registryPathName.MaximumLength = UNICODE_REGISTRY_PATH_LENGTH * sizeof(WCHAR);
    registryPathName.Buffer = ExAllocatePoolWithTag( PagedPool, UNICODE_REGISTRY_PATH_LENGTH, 'mNoI' );
...

{
    ...
    if (NT_SUCCESS( status )) {
        // With the keyInformation, allocate a buffer that will be large enough for all the subkeys
        keyBasicInformationSize = keyInformation->MaxNameLen + sizeof(KEY_NODE_INFORMATION);
        keyBasicInformation = ExAllocatePoolWithTag( PagedPool, keyBasicInformationSize, 'mNoI' );
    ...
    for (i = 0; NT_SUCCESS( status ); i++) {
        ...
        if (HighKey) {
            if (wcsncmp( keyBasicInformation->Name, CmTypeString[MultiFunctionAdapter], keyBasicInformation->NameLength / sizeof(WCHAR) )  &&
                wcsncmp( keyBasicInformation->Name, CmTypeString[EisaAdapter], keyBasicInformation->NameLength / sizeof(WCHAR) )  &&
                wcsncmp( keyBasicInformation->Name, CmTypeString[TcAdapter], keyBasicInformation->NameLength / sizeof(WCHAR) )) {
                // All the comparisons returned 1 (which means they all were unsuccessful) so we do not have a bus.
