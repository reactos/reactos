...
/*++
Routine Description:
    This routine is invoked to acquire the cancel spin lock.
    This spin lock must be acquired before setting the address of a cancel routine in an IRP.
Arguments:
...
/*++
Routine Description:
    This routine is invoked to acquire the Volume Parameter Block (VPB) spin lock.
    This spin lock must be acquired before accessing the mount flag, reference count, and device object fields of a VPB.
Arguments:
    Irql - Address of a variable to receive the old IRQL.
--*/
{
...
Routine Description:
    ...
    This is accomplished by calling HalAllocateAdapterChannel which does all of the work.
Arguments:
    AdapterObject - Pointer to the adapter control object to allocate to the driver.
    DeviceObject - Pointer to the driver's device object that represents the device allocating the adapter.
    NumberOfMapRegisters - The number of map registers that are to be allocated from the channel, if any.
...
#else
    return( (*((PDMA_ADAPTER)AdapterObject)->DmaOperations->AllocateAdapterChannel)( (PDMA_ADAPTER)AdapterObject, DeviceObject, NumberOfMapRegisters, ExecutionRoutine, Context) );
...
Routine Description:
    ...
    This is accomplished by placing the device object of the driver that wants to allocate the controller on the controller's queue.
    If the queue is already "busy", then the controller has already been allocated, so the device object is simply placed onto the queue and waits until the controller becomes free.

    Once the controller becomes free (or if it already is), then the driver's execution routine is invoked.
Arguments:
    ControllerObject - Pointer to the controller object to allocate to the driver.
    DeviceObject - Pointer to the driver's device object that represents the device allocating the controller.
    ExecutionRoutine - The address of the driver's execution routine that is invoked once the controller has been allocated.
    Context - An untyped longword context parameter passed to the driver's execution routine.
Notes:
    Note that this routine MUST be invoked at DISPATCH_LEVEL or above.
--*/
{
    ...
    // Initialize the device object's wait context block in case this device must wait before being able to allocate the controller.
    DeviceObject->Queue.Wcb.DeviceRoutine = ExecutionRoutine;
    DeviceObject->Queue.Wcb.DeviceContext = Context;

    // Allocate the controller object for this particular device.
    // If the controller cannot be allocated because it has already been allocated to another device, then return to the caller now;
    // otherwise, continue.
    if (!KeInsertDeviceQueue( &ControllerObject->DeviceWaitQueue, &DeviceObject->Queue.Wcb.WaitQueueEntry )) {
        // The controller was not busy so it has been allocated.
        // Simply invoke the driver's execution routine now.
        action = ExecutionRoutine( DeviceObject, DeviceObject->CurrentIrp, 0, Context );

        // If the driver would like to have the controller deallocated, then deallocate it now.
...
Routine Description:
    ...
    The storage is tagged with a client identification address which is used to retrieve a pointer to the storage.
    The client id must be unique.

    The allocated storage is freed when the driver object is deleted.
Arguments:
    DriverObject - The driver object to which the extension is to be associated.
    ClientIdentificationAddress - Unique identifier used to retrieve the extension.
...
Return Value:
    Returns the status of the operation.  Failure cases are STATUS_INSUFFICIENT_RESOURCES and STATUS_OBJECT_NAME_COLLISION.
--*/
{
    KIRQL irql;
    ...
    *DriverObjectExtension = NULL;
    ...
    if (extension == NULL) {
        // The client id does not exist.  Insert the new extension in the list.
...
/*++
Routine Description:
    This routine allocates and initializes an error log entry buffer and returns a pointer to the data entry portion of the buffer.
Arguments:
    IoObject - Pointer to driver's device object or driver object.
    EntrySize - Size of entry to be allocated, in bytes.  The maximum size is specified by ERROR_LOG_MAXIMUM_SIZE.
Return Value:
    Pointer to the body of the allocated error log entry, or NULL, if there are no free entries in the system.
Note:
    This routine assumes that the caller wants an error log entry within the bounds of the maximum size.
--*/
{
    ...
    deviceObject = IoObject;// Assume for a moment this is a device object.

    // Determine if this is a driver object or device object or if we are allocating a generic error log entry.
    // This is determined from the Type field of the object passed in.
...
Routine Description:
    This routine allocates and initializes an error log entry buffer and returns a pointer to the data entry portion of the buffer.
    It's expected to be called from inside the kernel where there may not be a driver object or a device object.
Arguments:
    EntrySize - Size of entry to be allocated, in bytes.  The maximum size is specified by ERROR_LOG_MAXIMUM_SIZE.
Return Value:
    Pointer to the body of the allocated error log entry, or NULL, if there are no free entries in the system.
...
PVOID IopAllocateErrorLogEntry(IN PDEVICE_OBJECT deviceObject, IN PDRIVER_OBJECT driverObject, IN UCHAR EntrySize)
{
    ...
    EntrySize = (UCHAR) ((EntrySize + sizeof(PVOID) - 1) & ~(sizeof(PVOID) - 1));// Round entry size to a PVOID size boundary.
    size = sizeof(ERROR_LOG_ENTRY) + EntrySize;// Calculate the size of the entry needed.
    ExAcquireSpinLock(&IopErrorLogAllocationLock, &oldIrql);// Make sure that there are not too many outstanding packets.

    try{
        if (IopErrorLogAllocation > IOP_MAXIMUM_LOG_ALLOCATION) {
            return(NULL);// Fail the request.
        }

        IopErrorLogAllocation += size;// Increase the outstanding allocation.
        ...
        // Reference the device object and driver object. So they don't go away before the name gets pulled out.
        ...
        RtlZeroMemory(elEntry, size);// Initialize the fields.
...
Routine Description:
    ...
    The packet will be allocated to contain StackSize stack locations.
    The IRP will also be initialized.
Arguments:
...
Return Value:
    The function value is the address of the allocated/initialized IRP, or NULL if one could not be allocated.
--*/
{
    USHORT allocateSize;
    UCHAR fixedSize;
    ...
    if ((StackSize <= (CCHAR)IopLargeIrpStackLocations) && ((ChargeQuota == FALSE) || (IopLookasideIrpFloat < IopLookasideIrpLimit))) {
        ...
        irp = (PIRP)ExInterlockedPopEntrySList(&lookasideList->L.ListHead, &lookasideList->Lock);
        if (irp == NULL) {
    ...
    if (!irp) {
        ...
        // There are no free packets on the lookaside list, or the packet is too large to be allocated from one of the lists, so it must be
        // allocated from nonpaged pool. If quota is to be charged, charge it against the current process. Otherwise, allocate the pool normally.
        if (ChargeQuota) {
            try {
            ...
            // Attempt to allocate the pool from non-paged pool.
            // If this fails, and the caller's previous mode was kernel then allocate the pool as must succeed.
            irp = ExAllocatePoolWithTag(NonPagedPool, allocateSize, ' prI');
            if (!irp) {
                mustSucceed = IRP_ALLOCATED_MUST_SUCCEED;
                if (KeGetPreviousMode() == KernelMode ) {
                    irp = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, allocateSize, ' prI');
                }
            }
        }
...
Routine Description:
    ...
    If the routine is given a pointer to an Irp, then it will chain the MDL to the IRP in the appropriate way.

    If this routine is not given a pointer to an Irp it is up to the caller to set the MDL address in the IRP that the MDL is being allocated for.
...
--*/
{
    ...
    // If the requested length is greater than 2Gb, then we're not going to be able to map the memory, so fail the request.
    ...
    if (!mdl) {
        ...
        if (!mdl) {
            if (KeGetPreviousMode() == KernelMode) {
                ...
                mdl = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, allocateSize, ' ldM' );
            }
    ...
    // Finally, if an IRP was specified, store the address of the MDL based on whether or not this is a secondary buffer.
    if (Irp) {
        if (!SecondaryBuffer) {
            Irp->MdlAddress = mdl;
        } else {
...
Routine Description:
    ...
    All of the special handling for this request is recognized by setting the IRP_PAGING_IO flag in the IRP flags word.
Arguments:
    FileObject - A pointer to a referenced file object describing which file the write should be performed on.
    MemoryDescriptorList - An MDL which describes the physical pages that the pages should be written to the disk.
        All of the pages have been locked in memory.
        The MDL also describes the length of the write operation.
    StartingOffset - Pointer to the offset in the file from which the write should take place.
    ApcRoutine - The address of a kernel APC routine which should be executed after the write operation has completed.
    ApcContext - A context parameter which should be supplied to the kernel APC routine when it executes.
    IoStatusBlock - A pointer to the I/O status block in which the final status and information should be stored.
    Irp - If specified, allows the caller to squirrel away a pointer to the Irp.
Return Value:
    The function value is the final status of the queue request to the I/O system subcomponents.
--*/
{
    PIRP irp;
    ...
    // Begin by getting a pointer to the device object that the file resides on.
    ...
    //  If specified, let the caller know what Irp is responsible for this transfer.
    //  While this is mainly for debugging purposes, it is absolutely essential to debug certain types of problems, and is very cheap, thus is included in the FREE build as well.
    if (ARGUMENT_PRESENT(Irp)) {
        *Irp = irp;
    }

    // Get a pointer to the first stack location in the packet.
    ...
    // Queue the packet to the appropriate driver based on whether or not there is a VPB associated with the device.
...
/*++
Routine Description:
    This routine "attaches" a device to another device.
    That is, it associates the source device to a target device which enables the I/O system to ensure
    that the target device a) exists, and b) cannot be unloaded until the source device has detached.
    Also, requests bound for the target device are given to the source device first, where applicable.
Arguments:
    SourceDevice - Pointer to device object to be attached to the target.
    TargetDevice - Supplies the name of the target device to which the attach is to occur.
    AttachedDevice - Returns a pointer to the device to which the attach occurred.
        This is the device object that the source driver should use to communicate with the target driver.
Return Value:
...
--*/
{
    ...
    // Attempt to open the target device for attach access.
    // This ensures that the device itself will be opened, with all of the special considerations thereof.
    InitializeObjectAttributes( &objectAttributes, TargetDevice, 0, (HANDLE) NULL, (PSECURITY_DESCRIPTOR) NULL );
    status = ZwOpenFile( &fileHandle, FILE_READ_ATTRIBUTES, &objectAttributes, &ioStatus, 0, FILE_NON_DIRECTORY_FILE | IO_ATTACH_DEVICE_API );
    if (NT_SUCCESS( status )) {
        // The open operation was successful.
        // Dereference the file handle and obtain a pointer to the device object for the handle.
        status = ObReferenceObjectByHandle( fileHandle, 0, IoFileObjectType, KernelMode, (PVOID *) &fileObject, NULL );
        if (NT_SUCCESS( status )) {
            // Get a pointer to the device object for this file, and close the handle.
    ...
    // Set the attached device pointer so that the driver being attached to cannot unload until the detach occurs, and so that attempts to open the device object go through the attached driver.
    // Note that the reference count is not incremented since exclusive drivers can only be opened once and this would count as an open.
    // At that point, both device objects would become useless.
    *AttachedDevice = IoAttachDeviceToDeviceStack( SourceDevice, targetDevice );
    if(!*AttachedDevice) {
        status = STATUS_NO_SUCH_DEVICE;
    }

    // Finally, dereference the file object.
    // This decrements the reference count for the target device so that when the detach occurs the device can go away if necessary.
...
/*++
Routine Description:
    This routine attaches the source device object to the target device object.
Arguments:
    SourceDevice - Specifies the device object that is to be attached to the target device.
    TargetDevice - Specifies the device object to which the attachment is to take place.
Return Value:
...
--*/
{
    ...
    // Get a pointer to the topmost device object in the stack of devices, beginning with the TargetDevice.
...
Return Value:
    If successful, this function returns a pointer to the device object to which the attachment actually occurred.

    If unsuccessful, this function returns NULL.  (This could happen if the device currently at the top of the attachment chain is being unloaded, deleted or initialized.)
--*/
{
    PDEVICE_OBJECT deviceObject;
    ...
    // Retrieve a pointer to the source device object's extension outside of the IopDatabaseLock, since it isn't protected by that.
    sourceExtension = SourceDevice->DeviceObjectExtension;

    // Get a pointer to the topmost device object in the stack of devices, beginning with the TargetDevice, and attach to it.
    ExAcquireFastLock( &IopDatabaseLock, &irql );

    // Tell the Special IRP code the stack has changed. Code that will reexamine the stack takes the database lock, so we can place the call here. This
    ...
    // Make sure that the SourceDevice object isn't already attached to something else, this is now illegal.
    ASSERT( sourceExtension->AttachedTo == NULL );

    // Now attach to the device, provided that it is not being unloaded, deleted or initializing.
    if (deviceObject->Flags & DO_DEVICE_INITIALIZING || deviceObject->DeviceObjectExtension->ExtensionFlags & (DOE_UNLOAD_PENDING | DOE_DELETE_PENDING | DOE_REMOVE_PENDING | DOE_REMOVE_PROCESSED)) {
        // The device currently at the top of the attachment chain is being unloaded, deleted or initialized.
        deviceObject = (PDEVICE_OBJECT) NULL;
    } else {
        // Perform the attachment.  First update the device previously at the top of the attachment chain.
        ...
        sourceExtension->AttachedTo = deviceObject;// Attachment chain is doubly-linked.
    }
    ExReleaseFastLock( &IopDatabaseLock, irql );
...
Routine Description:
    ...
    This routine provides a simple, fast interface to the device driver w/o having to put the knowledge of how to build an IRP into all of the FSDs (and device drivers) in the system.
Arguments:
    ...
    Buffer - Pointer to buffer to get data from or write data into.  This
        parameter is required for read/write, but not for flush or shutdown functions.
    Length - Length of buffer in bytes.  This parameter is required for read/write, but not for flush or shutdown functions.
    StartingOffset - Pointer to the offset on the disk to read/write from/to.
        This parameter is required for read/write, but not for flush or shutdown functions.
    IoStatusBlock - Pointer to the I/O status block for completion status information.
        This parameter is optional since most asynchronous FSD requests will be synchronized by using completion routines, and so the I/O status block will not be written.
Return Value:
...
--*/
{
    ...
    irp->Tail.Overlay.Thread = PsGetCurrentThread();// Set current thread for IoSetHardErrorOrVerifyDevice.
    ...
    if (MajorFunction != IRP_MJ_FLUSH_BUFFERS && MajorFunction != IRP_MJ_SHUTDOWN && MajorFunction != IRP_MJ_PNP && MajorFunction != IRP_MJ_POWER) {
        ...
        // memory based on whether the target device performs direct or buffered I/O operations.
        if (DeviceObject->Flags & DO_BUFFERED_IO) {
            // The target device supports buffered I/O operations.
            // Allocate a system buffer and, if this is a write, fill it in.
            // Otherwise, the copy will be done into the caller's buffer in the completion code.
            // Also note that the system buffer should be deallocated on completion.
            // Also, set the parameters based on whether this is a read or a write operation.
            ...
            // an MDL large enough to map the buffer and lock the pages into memory.
            ...
            // The operation is neither buffered nor direct.  Simply pass the address of the buffer in the packet to the driver.
            irp->UserBuffer = Buffer;
        }

        // Set the parameters according to whether this is a read or a write
        // operation.  Notice that these parameters must be set even if the driver has not specified buffered or direct I/O.
        if (MajorFunction == IRP_MJ_WRITE) {
    ...
    // Finally, set the address of the I/O status block and return a pointer to the IRP.
    irp->UserIosb = IoStatusBlock;
    return irp;
}
...
Arguments:
    IoControlCode - Specifies the device I/O control code that is to be performed by the target device driver.
    DeviceObject - Specifies the target device on which the I/O control function is to be performed.
    InputBuffer - Optional pointer to an input buffer that is to be passed to the device driver.
    InputBufferLength - Length of the InputBuffer in bytes.  If the Input-Buffer parameter is not passed, this parameter must be zero.
    OutputBuffer - Optional pointer to an output buffer that is to be passed to the device driver.
    OutputBufferLength - Length of the OutputBuffer in bytes.  If the OutputBuffer parameter is not passed, this parameter must be zero.
    InternalDeviceIoControl - A BOOLEAN parameter that specifies whether the packet that gets generated should have a major function code
        of IRP_MJ_INTERNAL_DEVICE_CONTROL (the parameter is TRUE), or IRP_MJ_DEVICE_CONTROL (the parameter is FALSE).
    Event - Supplies a pointer to a kernel event that is to be set to the Signaled state when the I/O operation is complete.
        Note that the Event must already be set to the Not-Signaled state.
    IoStatusBlock - Supplies a pointer to an I/O status block that is to be filled in with the final status of the operation once it completes.
Return Value:
    The function value is a pointer to the generated IRP suitable for calling the target device driver.
--*/
{
    PIRP irp;
    PIO_STACK_LOCATION irpSp;
    ...
    // Set the major function code based on the type of device I/O control function the caller has specified.
    ...
    // Copy the caller's parameters to the service-specific portion of the IRP for those parameters that are the same for all four methods.
    irpSp->Parameters.DeviceIoControl.OutputBufferLength = OutputBufferLength;
    irpSp->Parameters.DeviceIoControl.InputBufferLength = InputBufferLength;
    irpSp->Parameters.DeviceIoControl.IoControlCode = IoControlCode;

    // Get the method bits from the I/O control code to determine how the buffers are to be passed to the driver.
    method = IoControlCode & 3;

    // Based on the method that the buffers are being passed, either allocate buffers or build MDLs or do nothing.
    switch ( method ) {
    case 0:
        // For this case, allocate a buffer that is large enough to contain both the input and the output buffers.
        // Copy the input buffer to the allocated buffer and set the appropriate IRP fields.
        if (InputBufferLength != 0 || OutputBufferLength != 0) {
            irp->AssociatedIrp.SystemBuffer = ExAllocatePoolWithTag( NonPagedPoolCacheAligned, InputBufferLength > OutputBufferLength ? InputBufferLength : OutputBufferLength, '  oI' );
            if (irp->AssociatedIrp.SystemBuffer == NULL) {
                IoFreeIrp( irp );
                return (PIRP) NULL;
            }
    ...
    case 1:
    case 2:
        // For these two cases, allocate a buffer that is large enough to contain the input buffer, if any, and copy the information to the allocated buffer.
        // Then build an MDL for either read or write access, depending on the method, for the output buffer.
        // Note that an output buffer must have been specified.
        if (ARGUMENT_PRESENT( InputBuffer )) {
            irp->AssociatedIrp.SystemBuffer = ExAllocatePoolWithTag( NonPagedPoolCacheAligned, InputBufferLength, '  oI' );
            if (irp->AssociatedIrp.SystemBuffer == NULL) {
                IoFreeIrp( irp );
                return (PIRP) NULL;
            }
    ...
    // Finally, set the address of the I/O status block and the address of the kernel event object.
    // Note that I/O completion will not attempt to dereference the event since there is no file object associated with this operation.
    irp->UserIosb = IoStatusBlock;
    irp->UserEvent = Event;

    // Also set the address of the current thread in the packet so the completion code will have a context to execute in.
    // The IRP also needs to be queued to the thread since the caller is going to set the file object pointer.
    irp->Tail.Overlay.Thread = PsGetCurrentThread();
    IopQueueThreadIrp( irp );

    return irp;// Simply return a pointer to the packet.
}
...
/*++
Routine Description:
    This routine maps a portion of a buffer as described by an MDL.
    The portion of the buffer to be mapped is specified via a virtual address and an optional length.
    If the length is not supplied, then the remainder of the buffer is mapped.
Arguments:
...
Notes:
    This routine assumes that the target MDL is large enough to map the desired portion of the buffer.
    If the target is not large enough then an exception will be raised.
    It is also assumed that the remaining length of the buffer to be mapped is non-zero.
--*/
{
    ULONG_PTR baseVa;
    ULONG offset;
    ...
    // Then, determine the length of the buffer to be mapped, if not specified.
    ...
    // Initialize the target MDL header.  Note that the original size of the MDL structure itself is left unchanged.
    ...
    // Set the MdlFlags in the target MDL.
    // Clear all flags but carry across the allocation information, page read and the system mapped info.
    TargetMdl->MdlFlags &= (MDL_ALLOCATED_FIXED_SIZE | MDL_ALLOCATED_MUST_SUCCEED);
    TargetMdl->MdlFlags |= SourceMdl->MdlFlags & (MDL_SOURCE_IS_NONPAGED_POOL | MDL_MAPPED_TO_SYSTEM_VA | MDL_IO_PAGE_READ);
    ...
    // needs to be copied to the target.  Then, copy as many PFNs as are needed.
...
Routine Description:
    ...
    The IRP created by this function causes the I/O system to complete the request by setting the specified event to the Signaled state.
Arguments:
    MajorFunction - Function to be performed;  see previous list.
    DeviceObject - Pointer to device object on which the I/O will be performed.
    Buffer - Pointer to buffer to get data from or write data into.  This parameter is required for read/write, but not for flush or shutdown functions.
    Length - Length of buffer in bytes.  This parameter is required for read/write, but not for flush or shutdown functions.
    StartingOffset - Pointer to the offset on the disk to read/write from/to.
        This parameter is required for read/write, but not for flush or shutdown functions.
    Event - Pointer to a kernel event structure for synchronization.  The event will be set to the Signaled state when the I/O has completed.
...
--*/
{
    ...
    // Notice that because there is no FileObject, the I/O completion code will not attempt to dereference the event.
    irp->UserEvent = Event;

    // There will be a file object associated w/this packet, so it must be queued to the thread.
    IopQueueThreadIrp( irp );
    return irp;
}
...
/*++
Routine Description:
    This routine is invoked to pass an I/O Request Packet (IRP) to another driver at its dispatch routine.
Arguments:
...
{
    // This routine will either jump immediately to IopfCallDriver, or rather IovCallDriver.
...
Routine Description:
    ...
    It acquires the cancel spin lock, sets the cancel flag in the IRP, and then invokes the cancel routine specified by the appropriate field in the IRP, if a routine was specified.
    It is expected that the cancel routine will relaease the cancel spinlock.
    If there is no cancel routine, then the cancel spin lock is released.
Arguments:
    Irp - Supplies a pointer to the IRP to be cancelled.
Return Value:
    The function value is TRUE if the IRP was in a cancelable state (it had a cancel routine), else FALSE is returned.
Notes:
    It is assumed that the caller has taken the necessary action to ensure that the packet cannot be fully completed before invoking this routine.
--*/
{
    ...
    // Obtain the address of the cancel routine, and if one was specified, invoke it.
...
Routine Description:
    ...
    This is accomplished by walking the list of IRPs in the thread IRP list and canceling each one individually.
    No other I/O operations can be started for the thread since this routine has control of the thread itself.
Arguments:
...
--*/
{
    ...
    // Wait for the requests to complete.  Note that waiting may eventually timeout, in which case more work must be done.
    count = 0;
    interval.QuadPart = -10 * 1000 * 100;

    while (!IsListEmpty( &Thread->IrpList )) {
        // Lower the IRQL so that the thread APC can fire which will complete the requests.
        // Delay execution for a time and let the request finish.
        // The delay time is 100ms.
        KeLowerIrql( irql );
        KeDelayExecutionThread( KernelMode, FALSE, &interval );
        if (count++ > 3000) {
            // This I/O request has timed out, as it has not been completed for a full 5 minutes.
            // Attempt to remove the packet's association with this thread.
            // Note that by not resetting the count, the next time through the loop the next packet,
            // if there is one, which has also timed out, will be dealt with, although it will be given another 100ms to complete.
...
/*++
Routine Description:
    This routine is invoked to determine whether or not the granted access to a file allows the access specified by a desired access.
Arguments:
    DesiredAccess - Pointer to a variable containing the access desired to the file.
    GrantedAccess - Access currently granted to the file.
Return Value:
    The final status of the access check is the function value.
    If the accessor has the access to the file, then STATUS_SUCCESS is returned;
...
--*/
{
    ...
    // Determine whether the desired access to the file is allowed, given the current granted access.
...
/*++
Routine Description:
    This routine checks the validity of the specified EA buffer to guarantee that its format is proper, no fields hang over, that it is not recursive, etc.
Arguments:
...
VOID IopCreateVpb (IN PDEVICE_OBJECT DeviceObject)
{
    ...
    Vpb = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, sizeof( VPB ), ' bpV');
...
Arguments:
    ...
    Exclusive - Indicates that the device object should be created with using the exclusive object attribute.
        ...
        drivers.  Drivers attached to the PDO (e.g., the function driver) must enforce any exclusivity rules.
...
Arguments:
    ...
    EventHandle - Supplies a location to return a handle to the event.
Return Value:
    The function value is a pointer to the created/opened event, or NULL if the event could not be created/opened.
--*/
{
    OBJECT_ATTRIBUTES objectAttributes;
    NTSTATUS status;
...
--*/
{
    ...
    if (NT_SUCCESS( Status )) {
        ...
        while (TRUE) {
            Status = ZwQueryDirectoryObject( DirectoryHandle, (PVOID)DirInfo, Size, TRUE, RestartScan, &Context, &ReturnedLength);
            RestartScan = FALSE;
            ...
            if (NT_SUCCESS( Status )) {
                ...
                // temporary object, and then close the handle.  This will cause the object to go away.
...
/*++
Routine Description:
    This routine is invoked to remove a symbolic link from the system. 
    This generally occurs whenever a driver that has assigned a symbolic link needs to exit. 
    It can also be used when a driver no longer needs to redirect a name.
Arguments:
    SymbolicLinkName - Provides the Unicode name string to be deassigned.
Return Values:
    None.
...
/*++
Routine Description:
    This routine detaches the device object which is currently attached to the target device.
Arguments:
...
--*/
{
    ...
    // reference count goes to zero, then the driver may need to be unloaded at this point.
    ExAcquireSpinLock( &IopDatabaseLock, &irql );
...
Routine Description:
    ...
    that no interrupt objects directly connected using the kernel services may be input to this routine.
Arguments:
    InterruptObject - Supplies a pointer to the interrupt object allocated by the IoConnectInterrupt routine.
Return Value:
    None.
--*/
{
    ...
    // Obtain a pointer to the builtin interrupt object in the I/O interrupt structure.
    interruptStructure = CONTAINING_RECORD( InterruptObject, IO_INTERRUPT_STRUCTURE, InterruptObject );

    // The builtin interrupt object is always used, so simply disconnect it.
...
Routine Description:
    ...
    system itself.
Arguments:
    ObjectAttributes - Supplies the attributes to be used for opening the file (e.g., the file's name, etc).
    DesiredAccess - Supplies the type(s) of access that the caller would like to the file.
    OpenOptions - Supplies standard NtOpenFile open options.
    IoStatus - Supplies a pointer to a variable to receive the final status of the operation.
    Buffer - Supplies an output buffer to receive the network attributes for the specified file.
Return Value:
    The final function value indicates whether or not the fast path could be taken successfully.
--*/
{
    HANDLE handle;
    NTSTATUS status;
    ...
    if (openPacket.ParseCheck != OPEN_PACKET_PATTERN) {
        // The parse routine was not invoked properly so the operation did not work at all.
        IoStatus->Status = status;
    } else {
        // The fast path routine was successfully invoked so return the final status of the operation.
    ...
    if (!(Irp->AllocationFlags & IRP_ALLOCATED_FIXED_SIZE) ||
        ...
        if ( ExQueryDepthSList( &lookasideList->L.ListHead ) >= lookasideList->L.Depth ) {
            ...
            if (ExQueryDepthSList( &lookasideList->L.ListHead ) >= lookasideList->L.Depth) {
                ...
                ExInterlockedPushEntrySList( &lookasideList->L.ListHead, (PSINGLE_LIST_ENTRY) Irp, &lookasideList->Lock );
            }
        } else {
            if (Irp->AllocationFlags & IRP_QUOTA_CHARGED) {
                Irp->AllocationFlags ^= IRP_QUOTA_CHARGED;
...
/*++
Routine Description:
    This routine returns the highest level device object associated with the specified device.
...
Arguments:
    None.
Return Value:
    The function value is a pointer to the configuration information structure.
--*/
{
    PAGED_CODE();

    // Simply return a pointer to the built-in configuration information structure.
...
Routine Description:
    ...
    that has been opened to the device that ensures that the device cannot go away.

    To close access to the device, the caller should dereference the file object pointer.
Arguments:
    ObjectName - Name of the device object for which a pointer is to be returned.
    DesiredAccess - Access desired to the target device object.
    FileObject - Supplies the address of a variable to receive a pointer to the file object for the device.
    DeviceObject - Supplies the address of a variable to receive a pointer to the device object for the specified device.
Return Value:
...
/*++
Routine Description:
    This routine places the error log entry specified by the input argument onto the queue of buffers to be written to the error log process's port.
...
--*/
{
    ...
    // Get the address of the error log entry header, acquire the spin lock, insert the entry onto the queue, if there are no pending requests then queue a worker thread request and release the spin lock.
    ...
    KeQuerySystemTime( (PVOID) &entry->TimeStamp );// Set the time that the entry was logged.

    ExAcquireSpinLock( &IopErrorLogLock, &oldIrql );

    InsertTailList( &IopErrorLogListHead, &entry->ListEntry );// Queue the request to the error log queue.
...
/*++
Routine Description:
    This routine provides the caller with the signature and offset of the boot disk and system disk.
    This information is obtained from the loader block.
    The callers have to be boot drivers which have registered for a callback once all disk devices have been started
Arguments:
    BootDiskInformation - Supplies a pointer to the structure allocated by the caller for requested information.
...
Return Value:
    ...
    STATUS_INVALID_PARAMETER - size allocated for boot disk information is insufficient.
--*/
{
    ...
    // If a single bios disk was found if there is only a single entry on the disk signature list.
    ...
    // For each disk, get its drive layout and check to see if the signature is among the list of signatures in the loader block.
    // If yes, check to see if the disk contains the boot or system partitions. If yes, fill up the requested structure.
    for (diskNumber = 0; diskNumber < totalDriverDisksFound; diskNumber++) {
        ...
        irp = IoBuildDeviceIoControlRequest( IOCTL_DISK_GET_DRIVE_GEOMETRY, deviceObject, NULL, 0, &diskGeometry, sizeof(DISK_GEOMETRY), FALSE, &event, &ioStatusBlock );
        if (!irp) {
            ObDereferenceObject( fileObject );
            continue;
        }
        ...
        for (listEntry = arcInformation->DiskSignatures.Flink; listEntry != &arcInformation->DiskSignatures; listEntry = listEntry->Flink) {
            ...
            // Or if there is only a single disk drive from both the bios and driver viewpoints then assign an arc name to that drive.
            if ((singleBiosDiskFound && (totalDriverDisksFound == 1)) || (diskBlock->Signature == driveLayout->Signature && diskBlock->ValidPartitionTable)) {
                ...
                for (partitionNumber = 0; partitionNumber < driveLayout->PartitionCount; partitionNumber++) {
                    ...
                    // Create unicode ARC name for this partition and check to see if this is the boot disk.
                    sprintf( arcNameBuffer, "%spartition(%d)", arcName, partitionNumber+1 );
                    RtlInitAnsiString( &arcNameString, arcNameBuffer );
                    if (RtlEqualString( &arcNameString, &arcBootDeviceString, TRUE )) {
                        BootDiskInformation->BootDeviceSignature = driveLayout->Signature;
                        // Get Partition Information for the offset of the partition within the disk
                        status = IoGetDeviceObjectPointer(&deviceNameUnicodeString, FILE_READ_ATTRIBUTES, &fileObject, &deviceObject );
                        RtlFreeUnicodeString( &deviceNameUnicodeString );
                        if (!NT_SUCCESS( status )) {
                            continue;
                        ...
                        // Create IRP for get drive geometry device control.
                        irp = IoBuildDeviceIoControlRequest(IOCTL_DISK_GET_PARTITION_INFO, deviceObject, NULL, 0, &PartitionInfo, sizeof(PARTITION_INFORMATION), FALSE, &event, &ioStatusBlock );
                        if (!irp) {
                            ObDereferenceObject( fileObject );
                            continue;
                        }
                    ...
                    if (RtlEqualString( &arcNameString, &arcSystemDeviceString, TRUE )) {
                        BootDiskInformation->SystemDeviceSignature = driveLayout->Signature;
                        // Get Partition Information for the offset of the partition within the disk
                        status = IoGetDeviceObjectPointer(&deviceNameUnicodeString, FILE_READ_ATTRIBUTES, &fileObject, &deviceObject );
                        RtlFreeUnicodeString( &deviceNameUnicodeString );
                        if (!NT_SUCCESS( status )) {
    ...
    switch ( DeviceType ) {
        ...
        case FILE_DEVICE_NETWORK: {
            if ((DeviceHasName) && ((DeviceCharacteristics & FILE_FLOPPY_DISKETTE) != 0)) {
                ...
                // on it. This is done by making a copy of the default public ACL and changing the accesses granted to the administrators alias.

                // The logic here is:
                //      - Copy the public default dacl into another buffer
                //      - Find the ACE granting ADMINISTRATORS access
                //      - Change the granted access mask of that ACE to give administrators write access.
                ...
                // If the ACE wasn't found, then the public default ACL has been changed.
                // For this case, this code needs to be updated to match the new public default DACL.
...
/*++
Routine Description:
    This routine returns the session ID for process that originally requested the specified I/O operation.
Arguments:
    Irp - Pointer to the I/O Request Packet.
    pSessionId - Pointer to the session Id which is set upon successful return.
Return Value:
    Returns STATUS_SUCCESS if the session ID was available, otherwise STATUS_UNSUCCESSFUL.
--*/
{
    PEPROCESS Process;
...
/*++
Routine Description:
    This routine is invoked to update the operation count for the current process to indicate that an I/O service other than a read or write has been invoked.
    There is an implicit assumption that this call is always made in the context of the issuing thread.
--*/
{
...
/*++
Routine Description:
    This routine is invoked to update the read operation count for the current process to indicate that the NtReadFile system service has been invoked.
    There is an implicit assumption that this call is always made in the context of the issuing thread.
--*/
{
    if (IoCountOperations == TRUE) {
...
/*++
Routine Description:
    This routine is invoked to update the write operation count for the current process to indicate that the NtWriteFile service other has been invoked.
    There is an implicit assumption that this call is always made in the context of the issuing thread.
--*/
{
    if (IoCountOperations == TRUE) {
...
/*++
Routine Description:
    This routine is invoked to update the transfer count for the current process for an operation other than a read or write system service.
    There is an implicit assumption that this call is always made in the context of the issuing thread.
    Also note that overflow is folded into the thread's process.
Arguments:
...
/*++
Routine Description:
    This routine is invoked to update the read transfer count for the current process.
    There is an implicit assumption that this call is always made in the context of the issuing thread.
    Also note that overflow is folded into the thread's process.
Arguments:
    TransferCount - The count of the number of bytes transferred.
...
/*++
Routine Description:
    This routine is invoked to update the write transfer count for the current process.
...
/*++
Routine Description:
    This routine is invoked by a filter driver to send a close to the next filesystem driver below.
    It's needed as part of the file open process.
    The filter driver forwards the open to the FSD and the FSD returns success.
    The filter driver then examines some stuff and decides that the open has to be failed.
    In this case it has to send a close to the FSD.

    We can safely assume a thread context because it has to be called only in the context of file open.
    If the file object already has a handle then the owner of the handle can then simply close the handle to the file object and we will close the file.

    This code is extracted from IopCloseFile and IopDeleteFile.
    So it is duplication of code but it prevents duplication elsewhere in other FSDs.
Arguments:
    FileObject - Points to the file that needs to be closed.
    DeviceObject - Points to the device object of the filesystem driver below the filter driver.
--*/
{
    PIRP irp;
    PIO_STACK_LOCATION irpSp;
    ...
    ASSERT(!(FileObject->Flags & FO_HANDLE_CREATED));// Cannot call this function if a handle has already been created for this file.
    ...
    KeClearEvent( &FileObject->Event );// Reset the event in the file object.
    ...
    // No function-specific parameters are required for this operation.
    irpSp = IoGetNextIrpStackLocation( irp );
    irpSp->MajorFunction = IRP_MJ_CLEANUP;
    irpSp->FileObject = FileObject;

    IopQueueThreadIrp( irp );// Insert the packet at the head of the IRP list for the thread.
    status = IoCallDriver( DeviceObject, irp );// Invoke the driver at its appropriate dispatch entry with the IRP.
    ...
    // The following code tears down the IRP by hand since it may not be possible for it to be completed (either because this code was invoked as APC_LEVEL in the first place - or because the reference
    // count on the object cannot be incremented due to this routine being invoked by the delete file procedure below).
    // Cleanup IRPs therefore use close semantics (the close operation flag is set in the IRP) so that the I/O complete request routine itself sets the event to the Signaled state.
    KeRaiseIrql( APC_LEVEL, &irql );
    IopDequeueThreadIrp( irp );
    KeLowerIrql( irql );

    IoReuseIrp( irp , STATUS_SUCCESS);// Reuse the IRP for the next operation.

    // Reset the event in the file object.
    KeClearEvent( &FileObject->Event );
    KeClearEvent(&event);

    irpSp = IoGetNextIrpStackLocation( irp );// Get a pointer to the stack location for the first driver.  This is where the function codes and parameters are placed.
    ...
    IopQueueThreadIrp( irp );// Place this packet in the thread's I/O pending queue.

    // Decrement the reference count on the VPB, if necessary.
    // We have to do this BEFORE handing the Irp to the file system because of a trick the file systems play with close, and believe me, you really don't want to know what it is.

    // Since there is not a error path here (close cannot fail), and the file system is the only ome who can actually synchronize with the actual completion of close processing, the file system is the one responsible for Vpb deletion.
    vpb = FileObject->Vpb;
    if (vpb && !(FileObject->Flags & FO_DIRECT_DEVICE_OPEN)) {
        ExInterlockedAddUlong( &vpb->ReferenceCount, 0xffffffff, &IopVpbSpinLock );
        FileObject->Flags |= FO_FILE_OPEN_CANCELLED;
    }

    // Give the device driver the packet.
    // If this request does not work, there is nothing that can be done about it.
    // This is unfortunate because the driver may have had problems that it was about to report about other operations (e.g., write behind failures, etc.) that it can no longer report.
    // The reason is that this routine is really initially invoked by NtClose, which has already closed the caller's handle, and that's what the return status from close indicates:
    // the handle has successfully been closed.
    status = IoCallDriver( DeviceObject, irp );
    if (status == STATUS_PENDING) {
        (VOID) KeWaitForSingleObject( &event,Executive,KernelMode,FALSE,(PLARGE_INTEGER) NULL );
    }

    // Perform any completion operations that need to be performed on the IRP that was used for this request.
    // This is done here as as opposed to in normal completion code because there is a race condition between when this routine executes if it was invoked from a special kernel APC (e.g., some IRP was just completed and
    // dereferenced this file object for the last time), and when the special kernel APC because of this packet's completion executing.

    // This problem is solved by not having to queue a special kernel APC routine for completion of this packet.
    // Rather, it is treated much like a synchronous paging I/O operation, except that the packet is not even freed during I/O completion.
    // This is because the packet is still in this thread's queue, and there is no way to get it out except at APC_LEVEL.
    // Unfortunately, the part of I/O completion that needs to dequeue the packet is running at DISPATCH_LEVEL.

    // Hence, the packet must be removed from the queue (synchronized, of course), and then it must be freed.
    KeRaiseIrql( APC_LEVEL, &irql );
    IopDequeueThreadIrp( irp );
    KeLowerIrql( irql );
    IoFreeIrp( irp );
...
/*++
Routine Description:
    This routine is called from Mm when a page fault has completed.
    It's called on the special occasion when a thread faults a page and then when it's waiting for the inpage to complete, an IopCompleteRequest APC fires and we fault the same page again (say if the user buffer falls on the same page).
    Note the fault during the APC may be referencing the same or a different user virtual address but this is irrelevant - the problem lies in the fact that both virtual address references are to the same physical
    page and thus result in a collided fault in the Mm.

    Mm detects this case (to avoid deadlock) and returns STATUS_FAULT_COLLISION and the I/O manager bails out the APC after marking the Irp with the flag IRP_RETRY_IO_COMPLETION.
    Later on when Mm has decided the fault has progressed far enough to avoid deadlock, it calls back into this routine which calls IopCompleteRequest again.
    The code in IopCompleteRequest is written in a reentrant way so that the retry knows the completion is only partially processed so far.
    We can fault in two places in IopCompleteRequest and in both cases if we call IopCompleteRequest again they will now work.
