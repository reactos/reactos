...
Abstract:
    This module tracks irps and verified drivers when people do stupid things with them.
    ...
    -   "This file is NOT vital to operation of the OS, and could easily be
         disabled while a redesign to compensate for the core change is implemented." - the author
  ...
  Arguments:
    Level         - Level of testing to apply
                      ...
                      3 - Monitors mistakes based on the irp major
                          Surrogate IRPs used, status values rotated, and various other checks.
...
--*/
{
    ...
    switch(Level) {
        ...
        case 7:
            IovpHackFlags = 0;
        case 6:
            newTrackingFlags |= ASSERTFLAG_FORCEPENDING | ASSERTFLAG_DEFERCOMPLETION;
            // Fall through
        case 5:
            newTrackingFlags |= ASSERTFLAG_COMPLETEATDPC;
            // Fall through
        case 4:
            newTrackingFlags |= ASSERTFLAG_SURROGATE | ASSERTFLAG_SMASH_SRBS | ASSERTFLAG_CONSUME_ALWAYS | ASSERTFLAG_ROTATE_STATUS | ASSERTFLAG_SEEDSTACK;
            // Fall through
        case 3:
        ...
        case 2:
            ...
            // Fall through
        case 1:
            newTrackingFlags |= ASSERTFLAG_TRACKIRPS | ASSERTFLAG_MONITOR_ALLOCS;
            // Fall through
        case 0:
            break;
    }
    ...
    if (!(IovpInitFlags & IOVERIFIERINIT_EVERYTHING_TRACKED)) {
        // These options aren't available unless we were marking IRPs since boot.
        newTrackingFlags &=~ (ASSERTFLAG_MONITORMAJORS | ASSERTFLAG_SURROGATE | ASSERTFLAG_SMASH_SRBS | ASSERTFLAG_CONSUME_ALWAYS);
    }
...
--*/
{
    ...
    if (!IovpIrpTrackingEnabled) {
#if 0
        IoVerifierInit(DRIVER_VERIFIER_IO_CHECKING, IOVERIFIERINIT_EVERYTHING_TRACKED | IOVERIFIERINIT_ASYNCHRONOUSINIT | IOVERIFIERINIT_NO_REINIT);
#endif
    }
...
--*/
{
    ...
    if (DeviceObject == NULL) {
        FAIL_CALLER_OF_IOFCALLDRIVER((DCERROR_NULL_DEVOBJ_FORWARDED, DCPARAM_IRP, irp), irpSp);
    }
    ...
    if (iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) {
        ...
        if (irp->CancelRoutine) {
            FAIL_CALLER_OF_IOFCALLDRIVER((DCERROR_CANCELROUTINE_FORWARDED, DCPARAM_IRP, irp), irpSp);
    ...
    if (iovPacket->Flags&TRACKFLAG_QUEUED_INTERNALLY) {
        ...
        FAIL_CALLER_OF_IOFCALLDRIVER((DCERROR_QUEUED_IRP_FORWARDED, DCPARAM_IRP, irp), irpSp);
    }
    ...
    // We also need to see exactly how the IRP was forwarded (down the stack, to another stack, straight to the PDO, etc).
    ...
    // the OS has ever seen the node. So we assume a devobj is a PDO if he has never attached to anyone.
    ...
    if ((irpSp->MajorFunction == IRP_MJ_PNP)&& (irpSp->MinorFunction == IRP_MN_REMOVE_DEVICE)) {
        ...
        if (IovpIsInFdoStack(DeviceObject) && (!(DeviceObject->DeviceObjectExtension->ExtensionFlags&DOE_RAW_FDO))) {
            IofCallDriverStackData->Flags |= CALLFLAG_REMOVING_FDO_STACK_DO ;
        }
    }

    if ((iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) && (iovPacket->AssertFlags&ASSERTFLAG_MONITORMAJORS)) {
    ...
    if ((IovpInitFlags & IOVERIFIERINIT_RANDOMLY_CANCEL_IRPS) && (!(irp->Flags & IRP_PAGING_IO))) {
...
--*/
{
    ...
    if (iovSessionData->AssertFlags&ASSERTFLAG_POLICEIRPS) {
        if (IofCallDriverStackData->Flags&CALLFLAG_IS_REMOVE_IRP) {
            if ((*FinalStatus != STATUS_PENDING) && (iovCurrentStackLocation->ThreadDispatchedTo == PsGetCurrentThread())) {
                ...
                if (mustDetachAndDelete) {
                    ...
                    if (!(DeviceObject->DeviceObjectExtension->ExtensionFlags&DOE_DELETE_PENDING)) {
                        WDM_FAIL_ROUTINE((DCERROR_SHOULDVE_DELETED,
                            DCPARAM_IRP + DCPARAM_ROUTINE + DCPARAM_DEVOBJ,
                            iovSessionData->BestVisibleIrp,
                            DispatchRoutine,
                            DeviceObject));
    ...
    if (IofCallDriverStackData->Flags&CALLFLAG_COMPLETED) {
        ...
        if ((*FinalStatus != IofCallDriverStackData->ExpectedStatus)&& (*FinalStatus != STATUS_PENDING)) {
            if ((iovSessionData->AssertFlags&ASSERTFLAG_POLICEIRPS) && (!(iovSessionData->SessionFlags&SESSIONFLAG_UNWOUND_INCONSISTANT))) {
                // The completion routine and the return value don't match. Hey!
                WDM_FAIL_ROUTINE((DCERROR_INCONSISTANT_STATUS,
            ...
            if (iovSessionData->AssertFlags&ASSERTFLAG_POLICEIRPS) {
                ...
                WDM_FAIL_ROUTINE((DCERROR_UNINITIALIZED_STATUS,
        ...
        if (*FinalStatus != STATUS_PENDING) {
            if ((iovSessionData->AssertFlags&ASSERTFLAG_POLICEIRPS) && (!(iovPacket->Flags&TRACKFLAG_UNWOUND_BADLY))) {
                ...
                WDM_FAIL_ROUTINE((DCERROR_IRP_RETURNED_WITHOUT_COMPLETION,
                    DCPARAM_IRP + DCPARAM_ROUTINE,
                    iovSessionData->BestVisibleIrp,
                    DispatchRoutine));
            }
    ...
    if ((IofCallDriverStackData->Flags&CALLFLAG_OVERRIDE_STATUS)&& (*FinalStatus != STATUS_PENDING)) {
        *FinalStatus = IofCallDriverStackData->NewStatus ;
    }

    if ((iovSessionData->AssertFlags&ASSERTFLAG_FORCEPENDING) && (!(IofCallDriverStackData->Flags&CALLFLAG_IS_REMOVE_IRP))) {
...
VOID FASTCALL IovpCompleteRequest1(
    IN     PIRP               Irp,
...
--*/
{
    ...
    slotIsInUse = IovpAdvanceStackDownwards(iovSessionData->StackData,
         Irp->CurrentLocation,
         irpSp,
...
VOID FASTCALL IovpCompleteRequest2(
    IN     PIRP               Irp,
    IN OUT PIOFCOMPLETEREQUEST_STACKDATA CompletionPacket
    )
/*++
...
--*/
{
    ...
    if (Irp->CurrentLocation <= iovPacket->TopStackLocation) {
        ...
        if (requestFinalized) {
            TRACKIRP_DBGPRINT(("  CR2: Request %lx finalized\n", iovCurrentStackLocation-iovSessionData->StackData), 3) ;
        }
        ...
        // will come from a zero'd slot, and we will do nothing, which is also fine.
        irpSp = IoGetNextIrpStackLocation(Irp) ;
        if ((iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) && (iovPacket->AssertFlags&ASSERTFLAG_MONITORMAJORS)) {
            IovpAssertIrpStackUpward(iovPacket, irpSp, iovCurrentStackLocation, newlyCompleted, requestFinalized);
        }
        ...
        while(!IsListEmpty(&iovCurrentStackLocation->CallStackData)) {
            ...
            if ((iovSessionData->AssertFlags&ASSERTFLAG_ROTATE_STATUS)&& IovpAssertDoAdvanceStatus(irpSp, entranceStatus, &status)) {
                // Purposely munge the returned status for everyone at this
                // layer to flush more bugs. We are specifically trolling for this buggy sequence:
    ...
    if (iovPacket->TopStackLocation == Irp->CurrentLocation) {
        ...
        if (iovPacket->Flags&TRACKFLAG_SURROGATE) {
            ...
            // the pointer count we added to the tracking data here for the same reason.
            irpSp = IoGetNextIrpStackLocation(Irp) ;
            if (iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) {
...
VOID FASTCALL IovpCompleteRequest3(
    IN     PIRP               Irp,
    IN     PVOID              Routine,
    IN OUT PIOFCOMPLETEREQUEST_STACKDATA CompletionPacket
    )
...
--*/
{
    ...
    if (iovSessionData->AssertFlags&ASSERTFLAG_POLICEIRPS) {
        if ((CompletionPacket->LocationsAdvanced <= 0) &&
            ...
            DbgPrint("Verifier Notes: LocationsAdvanced %d\n", CompletionPacket->LocationsAdvanced);
            WDM_FAIL_ROUTINE((DCERROR_COMPLETION_ROUTINE_PAGABLE, DCPARAM_IRP + DCPARAM_ROUTINE, Irp, Routine));
        }
    }
    ...
    if ((!CompletionPacket->IsRemoveIrp)&&
       ((iovSessionData->AssertFlags&ASSERTFLAG_DEFERCOMPLETION)|| (iovSessionData->AssertFlags&ASSERTFLAG_COMPLETEATPASSIVE))) {
...
VOID FASTCALL IovpCompleteRequest4(
    IN     PIRP               Irp,
    ...
    if (iovSessionData->AssertFlags&ASSERTFLAG_FORCEPENDING) {
        ...
        if ((ReturnedStatus != STATUS_MORE_PROCESSING_REQUIRED)&& (iovPacket->pIovSessionData == iovSessionData)) {
            ...
            if (!(irpSp->Control & SL_PENDING_RETURNED )) {
                 WDM_FAIL_ROUTINE((DCERROR_PENDING_BIT_NOT_MIGRATED, DCPARAM_IRP + DCPARAM_ROUTINE, Irp, routine));
...
VOID FASTCALL IovpCompleteRequest5(
    IN     PIRP                          Irp,
    IN OUT PIOFCOMPLETEREQUEST_STACKDATA CompletionPacket)
/*++
  Description:
...
--*/
{
    ...
    if (iovSessionData) {
        ...
        ASSERT((!CompletionPacket->RaisedCount) || (iovSessionData->AssertFlags&ASSERTFLAG_COMPLETEATDPC)) ;
...
VOID FASTCALL IovpCompleteRequestApc(
    IN     PIRP                          Irp,
...
/*++
  ...
  Arguments:
    ...
    BestStackOffset        - A pointer to a last parameter passed on the stack.
                             ...
                             ignored STATUS_PENDING and left the UserIosb on it's stack.
  Return Value:
     None.
--*/
{
    ...
    )
{
    ...
    // at the head of the array to make our logic easier. Therefore we subtract only one.
    ...
    if (isNewSession) {
        // *We* are the original request. None of these fields below should be used.
    ...
    *StackLocationInfo = iovCurrentStackLocation;
    ...
    // Record a pointer in this slot to the requests originating slot as appropriate.
    if (IsNewRequest) {
        ...
        // backfill logic above would need fixing. I think what I have now is correct, not this:
        ...
        //advancedLocationData->RequestsFirstStackLocation = requestOriginalSLD ;
        TRACKIRP_DBGPRINT(("  CD1: %lx is a request for %lx\n", advancedLocationData-StackDataArray, requestOriginalSLD-StackDataArray), 3) ;
    } else {
    ...
    )
{
    ...
    if (!IsNewSession) {
        // We are sitting on current next being one back (-1) from CurrentStackLocation.
        locationsAdvanced = IovPacket->LastLocation-Irp->CurrentLocation ;
    ...
    if ((!IsNewSession) && (IovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS)) {
...
#if 0
            FAIL_CALLER_OF_IOFCALLDRIVER2((DCERROR_NEXTIRPSP_DIRTY, DCPARAM_IRP, Irp), irpSp);
#endif
        }
        ...
        if (locationsAdvanced) {
            // IoCopyCurrentStackLocation copies everything but Completion, Context, and Control
            ...
            isSameStack &= RtlEqualMemory(&irpSp->Parameters, &irpLastSp->Parameters,
                FIELD_OFFSET(IO_STACK_LOCATION, DeviceObject) - FIELD_OFFSET(IO_STACK_LOCATION, Parameters)) ;
            ...
            if (isSameStack) {
                ...
                if ((irpSp->CompletionRoutine == irpLastSp->CompletionRoutine)&&
                    ...
                    FAIL_CALLER_OF_IOFCALLDRIVER2((DCERROR_IRPSP_COPIED, DCPARAM_IRP, Irp), irpSp) ;
                    ...
                    if (!(irpSp->Control&SL_NOTCOPIED)
                        ...
                        if (irpSp->MajorFunction == IRP_MJ_POWER) {
                            ...
                            WDM_CHASTISE_CALLER5((DCERROR_UNNECCESSARY_COPY, DCPARAM_IRP, Irp));
                        } else {
                            WDM_CHASTISE_CALLER3((DCERROR_UNNECCESSARY_COPY, DCPARAM_IRP, Irp));
                        }
                    }

                    IoSetCompletionRoutine(Irp, IovpInternalCompletionTrap, IoGetCurrentIrpStackLocation( Irp ), TRUE, TRUE, TRUE) ;
                }
            }
        } else if (IovPacket->AssertFlags&ASSERTFLAG_CONSUME_ALWAYS) {
            if (ForwardMethod == FORWARDED_TO_NEXT_DO) {
                if (Irp->CurrentLocation<2) {
                    FAIL_CALLER_OF_IOFCALLDRIVER2((DCERROR_INSUFFICIENT_STACK_LOCATIONS, DCPARAM_IRP, Irp), irpSp) ;
                } else {
...
VOID IovpInternalCompleteAfterWait(IN PVOID Context)
{
    ...
    IovpProtectedIrpMakeTouchable(deferralContext->OriginalIrp, &deferralContext->IovRequestPacket->RestoreHandle);
    ...
    status = irpSpNext->CompletionRoutine(deferralContext->DeviceObject, deferralContext->OriginalIrp, irpSpNext->Context) ;
...
--*/
{
    ...
    if (deferralContext->IovRequestPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) {
        if (MmIsSystemAddressLocked(irpSpNext->CompletionRoutine) == FALSE) {
            WDM_FAIL_ROUTINE((DCERROR_COMPLETION_ROUTINE_PAGABLE, DCPARAM_IRP + DCPARAM_ROUTINE, Irp, irpSpNext->CompletionRoutine)) ;
        }
    }
    ...
    switch(trackingFlags&(ASSERTFLAG_DEFERCOMPLETION| ASSERTFLAG_COMPLETEATPASSIVE| ASSERTFLAG_COMPLETEATDPC)) {
        ...
        case ASSERTFLAG_DEFERCOMPLETION:
            deferAction = (KeGetCurrentIrql()==DISPATCH_LEVEL) ? DEFERACTION_QUEUE_DISPATCH_TIMER : DEFERACTION_QUEUE_PASSIVE_TIMER ;
            break;
        default:
    ...
    if (deferAction != DEFERACTION_NORMAL) {
        // Set this flag. If anybody uses this IRP while this flag is on, complain immediately!
        ASSERT(!(trackingFlags&TRACKFLAG_QUEUED_INTERNALLY)) ;
        deferralContext->IovRequestPacket->Flags |= TRACKFLAG_QUEUED_INTERNALLY ;
        deferralContext->DeviceObject = DeviceObject ;

        deferralContext->IovRequestPacket->RestoreHandle = IovpProtectedIrpMakeUntouchable(Irp, FALSE) ;
    } else {
        IovpTrackingDataDereference(deferralContext->IovRequestPacket, IOVREFTYPE_POINTER);
    }
    ...
    switch(deferAction) {
        ...
        case DEFERACTION_QUEUE_WORKITEM:
            ...
            ExInitializeWorkItem((PWORK_QUEUE_ITEM)&deferralContext->WorkQueueItem, IovpInternalCompleteAfterWait, deferralContext);
...
--*/
{
    ...
    iovPrevPacket = CONTAINING_RECORD(iovPacket->SurrogateLink.Blink, IOV_REQUEST_PACKET, SurrogateLink);

    realIrp = iovPrevPacket->TrackedIrp ;
    irpSize = IoSizeOfIrp( Irp->StackCount ) ;

    // Back the IRP stack up so that the original completion routine is called if appropriate
    ...
    // Record final data and make any accesses to the surrogate IRP crash.
    irpSp = IoGetNextIrpStackLocation(Irp) ;
    if (iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) {
        IovpAssertFinalIrpStack(iovPacket, irpSp) ;
    }
...
/*++
  Description:
    ...
    the cancelation was handled internally here (in which case IoCancelIrp should do nothing).

    We need to handle the call internally when we are currently
    dealing with a surrogate. In this case, we make sure the surrogate is cancelled instead.
...
--*/
{
    ...
    *CancelHandled = FALSE ;
    ...
    if (iovPacket->Flags&TRACKFLAG_QUEUED_INTERNALLY) {
        IovpProtectedIrpMakeTouchable(Irp, &iovPacket->RestoreHandle);
    ...
    if (!(iovPacket->Flags&TRACKFLAG_HAS_SURROGATE)) {
        // Cancel of an IRP that doesn't have an active surrogate. Let it proceed normally.
        IovpTrackingDataReleaseLock(iovPacket);
        return;
    }
    ...
    if (iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) {
        if (Irp->CancelRoutine) {
            WDM_FAIL_ROUTINE((DCERROR_CANCELROUTINE_ON_FORWARDED_IRP, DCPARAM_IRP + DCPARAM_ROUTINE, Irp, Irp->CancelRoutine));

            // We will ignore this routine. As we should...
        }
    }

    iovNextPacket = CONTAINING_RECORD(iovPacket->SurrogateLink.Flink, IOV_REQUEST_PACKET, SurrogateLink);
...
/*++
  Description:
    ...
    the free was handled internally here (in which case IoFreeIrp should do nothing).
...
--*/
{
    ...
    if (!IsListEmpty(&Irp->ThreadListEntry)) {
        if (iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) {
            WDM_FAIL_CALLER2((DCERROR_FREE_OF_THREADED_IRP, DCPARAM_IRP, Irp));
        }
    ...
    if (IovpTrackingDataGetCurrentSessionData(iovPacket)) {
        ...
        if ((iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) && (!(iovPacket->Flags&TRACKFLAG_UNWOUND_BADLY))) {
            WDM_FAIL_CALLER2((DCERROR_FREE_OF_INUSE_IRP, DCPARAM_IRP, Irp));
        }
    ...
    // We handled allocation and initialization. There is nothing much more to do.
    *FreeHandled = TRUE ;
}


VOID FASTCALL IovpAllocateIrp1(
    IN     CCHAR             StackSize,
...
/*++
  ...
  Arguments:
    ...
    ChargeQuote            - TRUE if quote should be charged against the current thread.
...
--*/
{
    ...
    *IrpPointer = NULL ;
    ...
    irp = IovpProtectedIrpAllocate(StackSize, ChargeQuota, PsGetCurrentThread()) ;
    if (irp == NULL) {
        return;
    }
...
/*++
  Description:
    This routine is called by IoAllocateIrp and captures information if the IRP was allocated by the OS.
  Arguments:
    Irp                    - Pointer to IRP
  Return Value:
    None.
...
VOID FASTCALL IovpInitializeIrp(
    IN OUT PIRP               Irp,
...
--*/
{
    ...
    if ((iovPacket->AssertFlags&ASSERTFLAG_POLICEIRPS) && (iovPacket->Flags&TRACKFLAG_IO_ALLOCATED)) {
        if (Irp->AllocationFlags&IRP_QUOTA_CHARGED) {
            // Don't let us leak quota now!
            WDM_FAIL_CALLER2((DCERROR_REINIT_OF_ALLOCATED_IRP_WITH_QUOTA, DCPARAM_IRP, Irp));
        } else {
            // In this case we are draining our lookaside lists erroneously.
            // WDM_CHASTISE_CALLER2((DCERROR_REINIT_OF_ALLOCATED_IRP_WITHOUT_QUOTA, DCPARAM_IRP, Irp));
        }
    }
...
VOID IovpReexamineAllStacks(VOID)
{
    ...
    ObEnumerateObjectsByType(IoDeviceObjectType, IovpEnumDevObjCallback, NULL);
}
...
BOOLEAN IovpIsInterestingStack(IN PDEVICE_OBJECT DeviceObject)
{
    ...
    if (DeviceObject->DeviceObjectExtension->ExtensionFlags & DOE_EXAMINED) {
        stackIsInteresting = ((DeviceObject->DeviceObjectExtension->ExtensionFlags & DOE_TRACKED) != 0);
    ...
    } while (deviceAttachedTo && (deviceAttachedTo->DeviceObjectExtension->ExtensionFlags & DOE_EXAMINED));
    
    if (deviceAttachedTo && (deviceAttachedTo->DeviceObjectExtension->ExtensionFlags & DOE_TRACKED)) {
...
--*/
{
    ...
    // upwards and find either NULL or the last device we called.
    ...
    if (DeviceLastCalled == NULL) {
        ...
        result = (DeviceBeingCalled->AttachedDevice) ? STARTED_INSIDE_STACK : STARTED_TOP_OF_STACK ;
    } else if (upperDevobj == NULL) {
        // We were forwarded the Irp beyond our own stack
        result = (lowerObject) ? CHANGED_STACKS_MID_STACK : CHANGED_STACKS_AT_BOTTOM ;
...
VOID FASTCALL IovpAssertNonLegacyDevice(IN PDEVICE_OBJECT DeviceObject, IN ULONG StackFramesToSkip, IN PUCHAR FailureTxt)
{
    ...
    if (pdoDeviceObject) {
        ...
        if (pDevNode&&(!(pDevNode->Flags&DNF_LEGACY_DRIVER))) {
            ...
            ASSERT(0);
/*
            WDM_FAIL_CALLER((FailureTxt), StackFramesToSkip+1, NULL);
*/
        }
...
VOID FASTCALL IovpSeedOnePage(VOID)
{
    ...
    // are trying to catch people who don't initialize NTSTATUS, and it's also a good pointer trap too.
...
/*++
  Description:
    This routine "seeds" the stack so that uninitialized variables are more easily ferreted out.

    ADRIAO BUGBUG 08/17/98 - This is a really neat idea that runs into a memory manager optimization. 
                             While I do find the appropriate guard page, the memory manager
                             throws out the above touched pages on a thread switch and bring in new (and probably zero'd)
                             ones.
  Arguments: None
  Return Value: None
--*/
