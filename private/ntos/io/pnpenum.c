...
/*++
Routine Description:
    This routine queues a work item to enumerate a device. This is for IO internal use only.
Arguments:
    DeviceObject - Supplies a pointer to the device object to be enumerated.
                   if NULL, this is a request to retry resources allocation failed devices.
...
--*/
{
    ...
    if (request) {
        ...
        if (    RequestType == ReenumerateBootDevices || RequestType == ReenumerateRootDevices) {
...
Routine Description:
    ...
    Its main purpose is to start and enumerate the device controlled by the newly loaded drivers.
...
--*/
{
    ...
    for (; ;) {
        ...
        if (!entry) {
            ...
            // This code will get run whenever we remove a device (amongst other situations).  However, if we are trying to start boot devices, we
            // don't want to start other devices.  The startContext should
            // probably be based on some global state that knows what devices should be (and more importantly should not be) started.
            if ((allocateResources && IopResourcesReleased) || assignResources || bootProcess) {
                ...
                do {
                    ...
                    // Process the whole device tree to assign resources to those devices who have been successfully added to their drivers.
        ...
        if (request->DeviceObject == NULL) {
            ...
            if (request->RequestType == ReenumerateBootDevices) {
                ...
                // The device wasn't started when IopResourceRequirementsChanged was called.
            ...
            // We've set the flags, we'll do the actual work once we've processed any other requests in the queue.
            continue;
        }
        ...
        if (request->RequestType == ResourceRequirementsChanged) {
            ...
            // Start the specific device (reenumeration of parent not required in this case) by first registering it then calling kernel-mode new
            // dev routine (note that this routine is normally called via a work item but we're already in a work item now so call it directly.
            ASSERT(deviceNode);
            ...
            if (NT_SUCCESS(status) && !IopDoesDevNodeHaveProblem(deviceNode)) {
                PIDBGMSG( PIDBG_EVENTS, ("IopDeviceActionWorker: START_REQUEST - calling IopNewDevice\n"));
            ...
            while(1) {
                ...
                if (!IopDoesDevNodeHaveProblem(deviceNode) && (deviceNode->Flags & (DNF_ENUMERATED | DNF_STARTED)) == (DNF_ENUMERATED | DNF_STARTED)) {
                    // Enumerate this object
...
Arguments:
    ...
    LoadDriver - Supplies a BOOLEAN value to indicate should a driver be loaded to complete a enumeration.
                 If false, the enumeration will stop once the device's controlling service is not loaded yet.
                 This is mainly for boot device driver initialization.
...
--*/
{
    ...
    status = IopEnumerateDevice( deviceNode->PhysicalDeviceObject, &startContext, AsyncOk);
    if (status != STATUS_PNP_RESTART_ENUMERATION) {
        do {
            startContext.NewDevice = FALSE;
...
/*++
Routine Description:
    This function is used by Pnp manager to start the devices which have been allocated resources and waiting to be started.
Parameters:
...
--*/
{
    ...
    if (DeviceNode->LockCount == 0) {
        KeWaitForSingleObject( &DeviceNode->EnumerationMutex, Executive, KernelMode, FALSE, NULL );
...
--*/
{
    ...
    if ( (DeviceNode->Flags & DNF_NEED_QUERY_IDS) ||   // Reported devices are Started but not enumerated
         ((DeviceNode->Flags & DNF_ADDED) && !(DeviceNode->Flags & DNF_START_PHASE) && (DeviceNode->Flags & DNF_HAS_RESOURCE)) ) {
        ...
        // Acquire enumeration mutex to make sure its children won't change by someone else.
        // Note, the current device node is protected by its parent's Enumeration mutex and it won't disappear either.
        ExAcquireResourceShared(&IopDeviceTreeLock, TRUE);

        if (DeviceNode->LockCount == 0) {
            KeWaitForSingleObject( &DeviceNode->EnumerationMutex, Executive, KernelMode, FALSE, NULL );

            IopForAllChildDeviceNodes(DeviceNode, IopProcessStartDevicesWorker, Context);// Recursively mark all of our children deleted.
            KeSetEvent( &DeviceNode->EnumerationMutex, 0, FALSE );// Release the enumeration mutex of the device node.
        }
...
Arguments:
    DeviceNode - Supplies a pointer to a device node which will be started and enumerated.
    StartContext - Supplies a pointer to START_CONTEXT structure to control how the start should be handled.
Return Value:
    NTSTATUS code.
--*/
{
    ...
    ASSERT((DeviceNode->Flags & DNF_ADDED) && (DeviceNode->Flags & (DNF_HAS_RESOURCE | DNF_NO_RESOURCE_REQUIRED)) && (!(DeviceNode->Flags & DNF_START_PHASE) || (DeviceNode->Flags & DNF_NEED_QUERY_IDS)));
    ...
    if (DeviceNode->Flags & DNF_NEED_QUERY_IDS) {
        ...
        // It should already be started.  We need to enumerate its children and ask the HardwareId and the Compatible ids of the detected device.
        DeviceNode->Flags &= ~DNF_NEED_QUERY_IDS;
        status = IopDeviceObjectToDeviceInstance (deviceObject, &handle, KEY_READ);
        if (NT_SUCCESS(status)) {
            status = IopQueryCompatibleIds(deviceObject, BusQueryHardwareIDs, &hwIds, &hwIdLength);
            if (!NT_SUCCESS(status)) {
                hwIds = NULL;
            }

            status = IopQueryCompatibleIds(deviceObject, BusQueryCompatibleIDs, &compatibleIds, &compatibleIdLength);
            if (!NT_SUCCESS(status)) {
                compatibleIds = NULL;
            }
            ...
            if (hwIds || compatibleIds) {
                ...
                if (hwIds) {
                    if (!IopFixupIds(hwIds, hwIdLength)) {
                        KeBugCheckEx( PNP_DETECTED_FATAL_ERROR, PNP_ERR_BOGUS_ID, (ULONG_PTR)deviceObject, (ULONG_PTR)hwIds, 3);
                    }
                ...
                if (compatibleIds) {
                    if (!IopFixupIds(compatibleIds, compatibleIdLength)) {
                        KeBugCheckEx( PNP_DETECTED_FATAL_ERROR, PNP_ERR_BOGUS_ID, (ULONG_PTR)deviceObject, (ULONG_PTR)compatibleIds, 4);
                    }

                    PiWstrToUnicodeString(&unicodeName, REGSTR_VAL_COMPATIBLEIDS);
                    ZwSetValueKey(handle, &unicodeName, TITLE_INDEX_VALUE, REG_MULTI_SZ, compatibleIds, compatibleIdLength);
...
/*++
Routine Description:
    This function assumes that the specified physical device object is a bus and will enumerate all of the children PDOs on the bus.
Arguments:
    DeviceObject - Supplies a pointer to the physical device object to be enumerated.
    StartContext - supplies a pointer to the START_CONTEXT to control how to add/start new devices.
    AsyncOk - Specifies can QueryDeviceRelation be done at Async way.
--*/
{
    NTSTATUS status;
    ...
    IopAcquireEnumerationLock(deviceNode);// Make sure we don't step on another enumerator's toes
    ...
    if ((deviceNode->Flags & (DNF_STARTED | DNF_REMOVE_PENDING_CLOSES)) != DNF_STARTED) {
        ...
        if (!(deviceNode->Flags & DNF_BEING_ENUMERATED)) {
            // The enumeration lock of the device must be acquired already before performing following instructions.
    ...
    for (i = 0; i < deviceRelations->Count; i++) {
        ...
        if (childDeviceObject->DeviceObjectExtension->ExtensionFlags & DOE_DELETE_PENDING) {
            KeBugCheckEx( PNP_DETECTED_FATAL_ERROR, PNP_ERR_PDO_ENUMERATED_AFTER_DELETION, (ULONG_PTR)childDeviceObject, 0, 0);
        }

        // We've found another physical device, see if there is already a devnode for it.
        ...
        if (childDeviceNode == NULL) {
            ...
            if (childDeviceNode != NULL) {
                // We've found or created a devnode for the PDO that the bus driver just enumerated.
                childDeviceNode->Flags |= DNF_ENUMERATED;
                ...
                // Put this new device node at the head of the parent's list of children.
                IopInsertTreeDeviceNode (deviceNode, childDeviceNode);
            } else {
                // Had a problem creating a devnode.  Pretend we've never seen it.
                KdPrint(("IopEnumerateDevice: Failed to allocate device node space\n"));
                ObDereferenceObject(childDeviceObject);
            }
        } else {
            // The device is alreay enumerated.  Remark it and release the device object reference.
            childDeviceNode->Flags |= DNF_ENUMERATED;

            if (childDeviceNode->DockInfo.DockStatus == DOCK_EJECTIRP_COMPLETED) {
                // A dock that was listed as departing in an eject relation didn't actually leave. Remove it from the profile transition list...
    ...
    // If we get here, the enumeration was successful.  First process any missing devnodes.
    childRemoved = FALSE;

    for (childDeviceNode = deviceNode->Child; childDeviceNode != NULL; childDeviceNode = nextChildDeviceNode) {
        // First, we need to remember the 'next child' because the 'child' will be removed and we won't be able to find the 'next child.'
        ...
        if (!(childDeviceNode->Flags & DNF_ENUMERATED)) {
            if (!(childDeviceNode->Flags & DNF_DEVICE_GONE)) {
                ...
                IopRequestDeviceRemoval( childDeviceNode->PhysicalDeviceObject, CM_PROB_DEVICE_NOT_THERE);
                childRemoved = TRUE;
            }
        }
    }

    // BUGBUG - currently the root enumerator gets confused if we reenumerate it before we process newly reported PDOs.  Since it can't possibly create
    // the scenario we are trying to fix, we won't bother waiting for the removes to complete before processing the new devnodes.
...
NTSTATUS IopProcessNewChildren(IN PDEVICE_NODE     DeviceNode, IN PSTART_CONTEXT   StartContext)
{
    ...
    for (childDeviceNode = DeviceNode->Child; childDeviceNode != NULL; childDeviceNode = childDeviceNode->Sibling) {
        if (childDeviceNode->Flags & DNF_ENUMERATED) {
            ...
            if (OK_TO_ADD_DEVICE(childDeviceNode)) {
                status = IopCallDriverAddDevice(childDeviceNode, StartContext->LoadDriver, &StartContext->AddContext);
                if (NT_SUCCESS(status)) {
...
Routine Description:
    ...
    If LoadDriver is true and the device driver for the device is installed, this routine will load the driver to start enumerating its children.
Arguments:
...
--*/
{
    ...
    if (!IopFixupDeviceId(id)) {
        KeBugCheckEx( PNP_DETECTED_FATAL_ERROR, PNP_ERR_BOGUS_ID, (ULONG_PTR)deviceObject, (ULONG_PTR)id, 1);
    }
    ...
    *wp = UNICODE_NULL;
    ...
    status = IopCreateRegistryKeyEx( &busIdHandle, enumHandle, &unicodeName, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, NULL);
    if (!NT_SUCCESS(status)) {
        ExFreePool(id);
        ZwClose(enumHandle);
        goto exit;
    ...
    status = IopCreateRegistryKeyEx( &deviceIdHandle, busIdHandle, &unicodeName, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, NULL);
    ZwClose(busIdHandle);
    if (!NT_SUCCESS(status)) {
        ExFreePool(id);
        ZwClose(enumHandle);
    ...
    DeviceNode->DockInfo.DockStatus = capabilities.DockDevice ? DOCK_QUIESCENT : DOCK_NOTDOCKDEVICE;// Record, is this a dock?
    RtlZeroMemory(&irpSp, sizeof(IO_STACK_LOCATION));// Initialize the stack location to pass to IopSynchronousCall()
...
RetryDuplicateId:
    ...
    if (!IopFixupDeviceId(uniqueId)) {
        KeBugCheckEx( PNP_DETECTED_FATAL_ERROR, PNP_ERR_BOGUS_ID, (ULONG_PTR)deviceObject, (ULONG_PTR)uniqueId, 2);
    }
    ...
    // Open/create this registry device instance path under HKLM\System\Enum\<Enumerator>\deviceId
    RtlInitUnicodeString(&unicodeName, uniqueId);
    status = IopCreateRegistryKeyEx( &uniqueIdHandle, deviceIdHandle, &unicodeName, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, &disposition);
    if (!NT_SUCCESS(status)) {
        ZwClose(enumHandle);
        ExFreePool(id);
        ZwClose(deviceIdHandle);
    ...
    if (disposition != REG_CREATED_NEW_KEY) {
        ...
        // Retrieve the device node associated with this device instance name (if there is one).  If it's different from the device node we're currently
        ...
        if (dupCheckDeviceObject) {
            // Go ahead and dereference the device object now--we only need the value for comparison.
            ...
            if (dupCheckDeviceObject != deviceObject) {
                ...
                KeBugCheckEx( PNP_DETECTED_FATAL_ERROR, PNP_ERR_DUPLICATE_PDO, (ULONG_PTR)deviceObject, (ULONG_PTR)dupCheckDeviceObject, 0);
        ...
        if (description) {
            ...
            ZwSetValueKey(uniqueIdHandle, &unicodeName, TITLE_INDEX_VALUE, REG_SZ, description, (wcslen(description)+1) * sizeof(WCHAR));
            ExFreePool(description);
            description = NULL;
        }
    }
    ...
    if (location) {
        ...
        ZwSetValueKey(uniqueIdHandle, &unicodeName, TITLE_INDEX_VALUE, REG_SZ, location, (wcslen(location)+1) * sizeof(WCHAR));
        ExFreePool(location);
        location = NULL;
    }

    status = IopDeviceCapabilitiesToRegistry(DeviceNode, &capabilities);// now add the capabilities and UI_NUMBER into registry
    ...
    status = IopCreateRegistryKeyEx( &logConfHandle, uniqueIdHandle, &unicodeName, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, NULL);
    if (!NT_SUCCESS(status)) {
    ...
    if (disposition == REG_CREATED_NEW_KEY) {    // disposition from uniqueId
        ...
        // Set flags to control whether device installation needs to happen later. This means setting the devnode flag to
        // DNF_NOT_CONFIGURED and setting the ConfigFlag ONLY if it's raw.
        if (!IopIsDevNodeProblem(DeviceNode, CM_PROB_NEED_RESTART)) {
            ...
            // the service, if necessary, so that the system can boot far enough to get to the config manager
        ...
        if (!IopIsDevNodeProblem(DeviceNode, CM_PROB_NEED_RESTART)) {
            ...
            tmpStatus = ZwQueryValueKey(uniqueIdHandle, &valueName, KeyValuePartialInformation, keyInfo, sizeof(buffer), &length);
            if (NT_SUCCESS(tmpStatus)) {
                ULONG configFlags = *(PULONG)keyInfo->Data;

                // The ConfigFlags value exists in the registry
                // If DNF_REINSTALL is set.  We mark it as DNF_DELETED such that we will not start the device.  Later when the reinstallation completes, the
        ...
        // if there's no service setup then check to see if this should be processed as a critical device.
    ...
    if (capabilities.HardwareDisabled && !IopIsDevNodeProblem(DeviceNode, CM_PROB_NOT_CONFIGURED) && !IopIsDevNodeProblem(DeviceNode, CM_PROB_NEED_RESTART)) {
    ...
    status = IopCreateRegistryKeyEx( &handle, uniqueIdHandle, &unicodeName, KEY_ALL_ACCESS, REG_OPTION_VOLATILE, NULL);
    if (NT_SUCCESS(status)) {
        // Write DeviceObject reference ...
        PiWstrToUnicodeString(&unicodeName, REGSTR_VALUE_DEVICE_REFERENCE);
        status = ZwSetValueKey( handle, &unicodeName, TITLE_INDEX_VALUE, REG_DWORD, (PULONG_PTR)&deviceObject, sizeof(ULONG_PTR));
        ZwClose(handle);
    }
    ...
    // Query the device's basic config vector. This needs to be done before we check if this is a remote BOOT card.
    ...
    if (logConfHandle) {
        ...
        if (ioResource) {
            ZwSetValueKey(logConfHandle, &unicodeName, TITLE_INDEX_VALUE, REG_RESOURCE_REQUIREMENTS_LIST, ioResource, ioLength);
    ...
    // While we have the hwIds, check if this is the device node for the remote boot net card. If IopLoaderBlock is NULL
    // then we are not initilizing boot drivers so we don't have to check for this.
    if (IoRemoteBootClient && (IopLoaderBlock != NULL)) {
        if (hwIds) {
            isRemoteBootCard = IopIsRemoteBootCard(DeviceNode, (PLOADER_PARAMETER_BLOCK)IopLoaderBlock, hwIds);
        }
        if (!isRemoteBootCard && compatibleIds) {
            isRemoteBootCard = IopIsRemoteBootCard(DeviceNode, (PLOADER_PARAMETER_BLOCK)IopLoaderBlock, compatibleIds);
        }
    }
    ...
    if (hwIds) {
        ...
        PiWstrToUnicodeString(&unicodeName, REGSTR_VAL_HARDWAREID);
        ZwSetValueKey(uniqueIdHandle, &unicodeName, TITLE_INDEX_VALUE, REG_MULTI_SZ, hwIds, hwIdLength);
        ExFreePool(hwIds);
    }
    ...
    if (compatibleIds) {
        ...
        PiWstrToUnicodeString(&unicodeName, REGSTR_VAL_COMPATIBLEIDS);
        ZwSetValueKey(uniqueIdHandle, &unicodeName, TITLE_INDEX_VALUE, REG_MULTI_SZ, compatibleIds, compatibleIdLength);
        ExFreePool(compatibleIds);
    }
    ...
    if (isRemoteBootCard) {
        status = IopSetupRemoteBootCard((PLOADER_PARAMETER_BLOCK)IopLoaderBlock, uniqueIdHandle, &unicodeDeviceInstance);
    ...
    status = IopQueryPnpBusInformation(deviceObject, &busTypeGuid, &DeviceNode->ChildInterfaceType, &DeviceNode->ChildBusNumber);
    if (NT_SUCCESS(status)) {
    ...
    if (processCriticalDevice && !capabilities.HardwareDisabled && !IopIsDevNodeProblem(DeviceNode, CM_PROB_NEED_RESTART)) {
    ...
    ASSERT(!IopDoesDevNodeHaveProblem(DeviceNode) ||
           ...
           IopIsDevNodeProblem(DeviceNode, CM_PROB_NEED_RESTART));
    if (!IopIsDevNodeProblem(DeviceNode, CM_PROB_DISABLED) && !IopIsDevNodeProblem(DeviceNode, CM_PROB_HARDWARE_DISABLED) && !IopIsDevNodeProblem(DeviceNode, CM_PROB_NEED_RESTART)) {
    ...
    if (DeviceNode->BootResources == NULL) {
        status = IopQueryDeviceResources( deviceObject, QUERY_RESOURCE_LIST, &cmResource, &cmLength );
        if (!NT_SUCCESS(status)) {
            cmResource = NULL;
        }
    } else {
        DebugPrint(1, ("PNPENUM: %ws already has BOOT config in IopProcessNewDeviceNode!\n", DeviceNode->InstancePath.Buffer));
    }
    ...
    if (logConfHandle && DeviceNode->BootResources == NULL) {
        ...
        if (cmResource) {
            ...
            status = (*IopReserveResourcesRoutine)(ArbiterRequestPnpEnumerated, deviceObject, cmResource);
            ExAcquireResourceShared(&PpRegistryDeviceResource, TRUE);
    ...
    // Create new value entry under ServiceKeyName\Enum to reflect the newly added made-up device instance node.
    status = IopNotifySetupDeviceArrival( deviceObject, uniqueIdHandle, TRUE);
    configuredBySetup = NT_SUCCESS(status);
    status = PpDeviceRegistration(&unicodeDeviceInstance, TRUE, &DeviceNode->ServiceName);
    if (NT_SUCCESS(status) && (configuredBySetup || isRemoteBootCard) && IopIsDevNodeProblem(DeviceNode, CM_PROB_NOT_CONFIGURED)) {
        IopClearDevNodeProblem(DeviceNode);
    }
...
exit:
    // In case of failure, we don't set the DNF_PROCESSED flags.  So the device will be processed again later.
    ExReleaseResource(&PpRegistryDeviceResource);
    KeLeaveCriticalRegion();
    if (buffer) {
        ExFreePool(buffer);
...
/*++
Routine Description:
    This function checks if the driver for the DeviceNode is present and loads the driver if necessary.
Arguments:
    DeviceNode - Supplies a pointer to the device node to be enumerated.
    LoadDriver - Supplies a BOOLEAN value to indicate should a driver be loaded to complete enumeration.
...
--*/
{
    ...
    // The device node may have been started at this point.
    // This is because some ill-behaved miniport drivers call IopReportedDetectedDevice at DriverEntry for the devices which we already know about.
    ...
    if (DeviceNode->Flags & DNF_LEGACY_DRIVER) {
        // One of the services for this device is a legacy driver.  Don't try to add any filters since we'll just screw up the device stack.
        status = STATUS_SUCCESS;
        goto Cleanup;
    } else if (NT_SUCCESS(status)) {
        // Call was successful so we must have been able to reference the driver object.
        ASSERT(queryContext.DriverLists[DeviceService] != NULL);

        if (queryContext.DriverLists[DeviceService]->NextEntry != NULL) {
            // There's more than one service assigned to this device.  Configuration error
        ...
        // query routine will have set the flags appropriately so we can just bail out.
        goto Cleanup;
    }

    // For each type of filter driver we want to build a list of the driver
    // objects to be loaded.  We'll build all the driver lists if we can and deal with error conditions afterwards.

     // First get all the information we have to out of the instance key and the device node.
     RtlZeroMemory(queryTable, sizeof(queryTable));
     queryTable[0].QueryRoutine = (PRTL_QUERY_REGISTRY_ROUTINE) IopCallDriverAddDeviceQueryRoutine;
    ...
    if (NT_SUCCESS(status)) {
        ...
        // Make sure there's no more than one device service.  Anything else is a configuration error.
        ...
        for (serviceType = 0; serviceType < MaximumAddStage; serviceType++) {
            ...
            if (serviceType == DeviceService) {
                if (deviceRaw&&(queryContext.DriverLists[serviceType]==NULL)) {
                    ...
                    // don't mark a raw PDO as the BOTTOM of the FDO stack as that would be the lower filter in this case.
                    DeviceNode->PhysicalDeviceObject->DeviceObjectExtension->ExtensionFlags |= DOE_DESIGNATED_FDO | DOE_RAW_FDO;
                } else {
                    // Since we are going to see a service, grab a pointer to the current top of the stack. While here, assert there
            ...
            for (listEntry = queryContext.DriverLists[serviceType]; listEntry != NULL; listEntry = listEntry->NextEntry) {
                ...
                status = (addDeviceRoutine)(listEntry->DriverObject, DeviceNode->PhysicalDeviceObject);
                DebugPrint(1, ("IopCallDriverAddDevice:\t\tRoutine returned "
                            "%#08lx\n", status));
                if (NT_SUCCESS(status)) {
#ifndef NO_SPECIAL_IRP
                   if (!deviceObjectHasBeenAttached) {
                       // Mark the first driver loaded by this routine as the bottom of the FDO stack. These must detach on a remove.
                       // Note we can't simply flag the top of the stack, as someone might attach in AddDevice...
                   ...
                   // Also note it is legal for a filter to succeed AddDevice but fail to attach anything to the top of the stack.
                   if (serviceType == DeviceService) {
                       // ADRIAO BUGBUG 10/07/98 - Since we are temporarily letting successful but Noop'd AddDevice's through,
                       // mark the stack appropriately. We will make it look like a RAW FDO
...
Cleanup:
    {
        ...
        // Free the entries in the driver load list & release the references on their driver objects.
...
/*++
Routine Description:
    This routine is called to build a list of driver objects which need to be Added to a physical device object.
    Each time it is called with a service name it will locate a driver object for that device and append it to the proper driver list for the device node.

    In the event a driver object cannot be located or that it cannot be loaded at this time,
    this routine will return an error and will set the flags in the device node in the context appropriately.
Arguments:
...
--*/
{
    ...
    // First check and make sure that the value type is okay.  An invalid type is not a fatal error.
    if (ValueType != REG_SZ) {
    ...
    // Check the service name to see if it should be used directly to reference the driver object.
    // If the string begins with "\Driver", make sure the madeupService flag is set.
    madeupService = TRUE;
    i = 0;
    ...
    if (madeupService) {
        ...
        //      INF is run, but if we don't do it now we'll blow up when we call IopGetDriverLoadType below.
        if (Context->DeviceNode->ServiceName.Length == 0) {
            ...
            if (Context->DeviceNode->ServiceName.Buffer != NULL) {
                RtlCopyMemory( Context->DeviceNode->ServiceName.Buffer, unicodeServiceName.Buffer, unicodeServiceName.MaximumLength );
            } else {
        ...
        // Check in the registry to find the name of the driver object for this device.
        status = IopOpenServiceEnumKeys(&unicodeServiceName, KEY_READ, &serviceKey, NULL, FALSE);
        if (!NT_SUCCESS(status)) {
            // Cannot open the service key for this driver.  This is a fatal error.
        ...
        if (!NT_SUCCESS(status)) {
            ...
            // Can't get the driver name from the service key.  This is a fatal error.
            DebugPrint(1, ("IopCallDriverAddDevice:\t\t\tStatus %#08lx "
                        "getting driver name\n",
                        status));
    ...
    if (driverObject == NULL) {
        ...
        // We couldn't find a driver object.  It's possible the driver isn't loaded & initialized so check to see if we can try to load it now.
        if (madeupService) {
            // The madeup service's driver doesn't seem to exist yet.
            // We will fail the request without setting DNF_ADD_FAILED such that we will try it again later.  (Root Enumerated devices...)
        ...
        if (ServiceType != DeviceService && !PnPBootDriversInitialized) {
            ...
            if (!Context->LoadDriver) {
                // We're not supposed to try and load a driver - most likely our disk drivers aren't initialized yet.
                // We need to stop the add process but we can't mark the devnode as failed or we won't be called again when we can load the drivers.
            ...
            if (groupIndex > Context->AddContext->GroupsToStart) {
                // We're not allowed to initialize this driver until drivers in previous groups are loaded.  Leave the devnode flags untouched
            ...
            if (loadType > Context->AddContext->DriverStartType) {
                if (loadType == SERVICE_DISABLED && !IopDoesDevNodeHaveProblem(Context->DeviceNode)) {
                    IopSetDevNodeProblem(Context->DeviceNode, CM_PROB_DISABLED_SERVICE);
                }

                // The service is either disabled or we are not at the right time to load it.  Don't load it, but make sure we can get
                // called again.  If a service is marked as demand start, we always load it.
...

            {
                ...
                // Check in the registry to find the name of the driver object for this device.
                status = IopOpenServiceEnumKeys(&unicodeServiceName, KEY_READ, &handle, NULL, FALSE);
                if (!NT_SUCCESS(status)) {
                    // Cannot open the service key for this driver.  This is a fatal error.
                    DebugPrint(1, ("IopCallDriverAddDevice:\t\t\tStatus %#08lx "
                                "opening service key\n",
    ...
    // Ignore the return value from the driver load - just try and get a pointer to the driver object for the service.
    driverObject = IopReferenceDriverObjectByName(&unicodeDriverName);
    if (driverObject == NULL) {
        if (PnPBootDriversInitialized) {
            // Apparently the load didn't work out very well.  This is a fatal error.
    ...
    if (IopIsLegacyDriver(driverObject)) {
        // It is.  Since the legacy driver may have already obtained a handle to the device object, we need to assume this device has been added and started.
        ...
        if (ServiceType == DeviceService) {
            ...
            // In this case, the legacy driver will be loaded but will not be part of our pnp driver stack.
            status = STATUS_SUCCESS;
        }
        goto Cleanup;
    }

    // There's a chance the driver detected this PDO during it's driver entry routine.  If it did then just bail out.
...
/*++
Routine Description:
    This routine will issue an irp to the DeviceObject to retrieve the pnp device capabilities.
    Should only be called twice - first from IopProcessNewDeviceNode, and second from IopDeviceNodeCapabilitiesToRegistry, called after device is started.
    If you consider calling this device, see if DeviceNode->CapabilityFlags does what you need instead (accessed via IopDeviceNodeFlagsToCapabilities(...).
Arguments:
...
--*/
{
    ...
    RtlZeroMemory(&irpStack, sizeof(IO_STACK_LOCATION));// Initialize the stack location to pass to IopSynchronousCall()
    ...
    status = IopSynchronousCall(DeviceNode->PhysicalDeviceObject, &irpStack, &dummy);
...
/*++
Routine Description:
    This routine will check whether the device is a "critical" one (see the top of this file for a description of critical).
    If the device is critical then it will be assigned a service based on the contents of IopCriticalDeviceList.
Arguments:
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        // Call IopProcessCriticalDeviceRoutine to enumerate entries in the CriticalDeviceDatabase
        // and compare with HardwareId and CompatibleIds value data from instanceKey.
        RtlInitUnicodeString(&service, NULL);
        RtlInitUnicodeString(&classGuid, NULL);
        RtlInitUnicodeString(&lowerFilters, NULL);
        RtlInitUnicodeString(&upperFilters, NULL);
        status = IopProcessCriticalDeviceRoutine(instanceKey, &foundMatch, &service, &classGuid, &lowerFilters, &upperFilters);
        if (!NT_SUCCESS(status)) {
    ...
    // If we get here then this is a "critical" device and we know the service to setup for it.  Set the service value in the registry.
    if (!foundMatch) {
        ZwClose(instanceKey);
        return FALSE;
    } else {
        UNICODE_STRING serviceValue, classGuidValue, lowerFiltersValue, upperFiltersValue;
        ...
        if (classGuid.Buffer) {
            ...
            status = ZwSetValueKey(instanceKey, &classGuidValue, 0L, REG_SZ, classGuid.Buffer, classGuid.Length + sizeof(UNICODE_NULL));
        }
...
#endif
            status = ZwSetValueKey(instanceKey, &lowerFiltersValue, 0L, REG_MULTI_SZ, lowerFilters.Buffer, lowerFilters.Length); // + sizeof(UNICODE_NULL));
        }
...
#endif
            status = ZwSetValueKey(instanceKey, &upperFiltersValue, 0L, REG_MULTI_SZ, upperFilters.Buffer, upperFilters.Length); // + sizeof(UNICODE_NULL));
        }

        DebugPrint(1, ("IopProcessCriticalDevice: service is %wZ\n", &service));
        status = ZwSetValueKey(instanceKey, &serviceValue, 0L, REG_SZ, service.Buffer, service.Length + sizeof(UNICODE_NULL));

        // If the service was set properly set the CONFIGFLAG_FINISH_INSTALL so
        // we will still get a new hw found popup and go through the class installer.
        if (NT_SUCCESS(status)) {
            UCHAR buffer[sizeof(KEY_VALUE_PARTIAL_INFORMATION) + sizeof(ULONG)];
            UNICODE_STRING valueName;
            PKEY_VALUE_PARTIAL_INFORMATION keyInfo = (PKEY_VALUE_PARTIAL_INFORMATION) buffer;
            ...
            RtlInitUnicodeString(&valueName, REGSTR_VALUE_CONFIG_FLAGS);
            tmpStatus = ZwQueryValueKey(instanceKey, &valueName, KeyValuePartialInformation, keyInfo, sizeof(buffer), &length);
            ...
            ASSERT(!IopDoesDevNodeHaveProblem(DeviceNode) || IopIsDevNodeProblem(DeviceNode, CM_PROB_NOT_CONFIGURED) || IopIsDevNodeProblem(DeviceNode, CM_PROB_FAILED_INSTALL) || IopIsDevNodeProblem(DeviceNode, CM_PROB_REINSTALL));
            IopClearDevNodeProblem(DeviceNode);
        }
        ZwClose(instanceKey);
    }
...
/*++
Routine Description:
    This routine will enumerate all values of the CriticalDeviceDatabase registry key,
    and compare each with entries within the HardwareId and CompatibleIds values of key associated with the current device instance.
Arguments:
    HDevInstance - HANDLE to device instance.
    FoundMatch - receives TRUE if a match was found.
    ServiceName - receives name of service to be assigned to the device instance pointed to by HDevInstance.
Return Value:
    NTSTATUS code
...
#define INITIAL_INFOBUFFER_SIZE sizeof(KEY_VALUE_FULL_INFORMATION) + 8*sizeof(WCHAR) + 255*sizeof(WCHAR)
    ...
    // This key contains hardware id's for so called "critical" devices.
    // These are devices for which, for one reason or another, we cannot wait for config manager to bring them on line.
    // These are primarily devices which are necessary in order to bring the system up into user mode so that config manager can be run (disks, keyboards, video, etc...)
    ...
    // For each CriticalDeviceDatabase value entry compare with all entries of HardwareId and CompatibleIds REG_MULTI_SZ for a match
    enumIndex = 0;
    while (((status = ZwEnumerateKey( hCriticalDeviceKey, enumIndex, KeyBasicInformation, (PVOID) infoBuffer.Buffer, infoBuffer.MaxSize, &resultSize)) != STATUS_NO_MORE_ENTRIES)) {
            ...
            while(ptr != bufferEnd) {
                if (!*ptr) {
                    ...
                    if (RtlEqualUnicodeString(&tmpUnicodeString, &unicodeCriticalEntry, TRUE)) {
                        ...
                        status = IopOpenRegistryKeyEx( &hCriticalEntry, hCriticalDeviceKey, &unicodeCriticalEntry, KEY_READ);
                        if (!NT_SUCCESS(status)) {
                            goto cleanup;
                        }

                        parameters = (PRTL_QUERY_REGISTRY_TABLE) ExAllocatePool(NonPagedPool, sizeof(RTL_QUERY_REGISTRY_TABLE)*(NUM_QUERIES+1));
                        if (!parameters) {
                        ...
                        status = RtlQueryRegistryValues(RTL_REGISTRY_HANDLE | RTL_REGISTRY_OPTIONAL, (PWSTR) hCriticalEntry, parameters, NULL, NULL);
                        ...
                        if (NT_SUCCESS(status)) {
                            ...
                            if (!ServiceName->Buffer || !ServiceName->Length || (ClassGuid->Length && (ClassGuid->Length < 38 * sizeof (WCHAR)))) {
                                goto FreeStrings;
                            }

                            // Caller expects XxxFilters->Buffer == NULL, so make the default case look like that
...
FreeStrings:
        ...
        enumIndex++;// enumerate next key value.
    }
...
/*++
Routine Description:
    This routine returns an index into the global table of bus type guids for guid specified.
    If this guid is not already in the table, then it will be added and the index for the new table entry will be returned.
    This index is later used by IoGetDeviceProperty to retrieve and return the bus type guid back to callers but allows us not to bother storing the entire guid in each devnode (just once per guid in the table).
Arguments:
...
--*/
{
    ...
    if (IopBusTypeGuidList->Count > 0) {
        ...
        size = sizeof(BUS_TYPE_GUID_LIST) + sizeof(GUID) * (IopBusTypeGuidList->Count);
        p = ExAllocatePool(PagedPool, size);
        if (!p) {
            goto Clean0;    // oops, no more room, return unreported
        }

        size = sizeof(BUS_TYPE_GUID_LIST) + sizeof(GUID) * (IopBusTypeGuidList->Count - 1);
    ...
    RtlCopyMemory(&IopBusTypeGuidList->Guid[IopBusTypeGuidList->Count], BusTypeGuid, sizeof(GUID));
...
/*++
Routine Description:
    This routine parses the device instance string and replaces any invalid characters (not allowed in a "device instance") with an underscore character.
...
Arguments:
    DeviceId - specifies a device instance string (or part of one), must be null-terminated.
Return Value:
    None.
--*/
{
...
Routine Description:
    ...
    Properties set/ changed are:
        ...
        * DeviceCharacteristics - the I/O system characteristic flags to be set for the device object
    ...
    to determine whether a VPB should be allocated as well as to set default security if none is specified in the registry.
...
--*/
{
    ...
    if (securityDescriptor == NULL) {
        ...
        if (deviceTypeSpec) {
            ...
            securityDescriptor = IopCreateDefaultDeviceSecurityDescriptor((DEVICE_TYPE)deviceType, characteristics, hasName, buffer, &allocatedAcl, &securityInformation);
        ...
        // See what information is in the captured descriptor so we can build up a securityInformation block to go with it.
...
/*++
Routine Description:
    This function is called after the system has transitioned into a new hardware profile.
    The thread from which it is called may be holding an enumeration lock. Calling this function does two tasks:
    1) If a disabled devnode in the tree should be enabled in this new hardware profile state, it will be started.
    2) If an enabled devnode in the tree should be disabled in this new hardware profile state, it will be (surprise) removed.
    ...
    ADRIAO N.B. 02/19/1999 -
        ...
        if they were in the removal relation lists for the dock. d) is another matter altogether as we need to query-remove/remove devices before starting another.
        NT5's PnP state machine cannot handle this, so for this release we cleanup rather hastily after the profile change.
--*/
{
    ...
    if (workQueueItem) {
        ...
        ExInitializeWorkItem(workQueueItem,IopProcessNewProfileWorker,workQueueItem);
...
/*++
Routine Description:
    This function is called for each devnode after the system has transitioned to a new hardware profile.
Parameters:
    NONE.
--*/
{
...
/*++
Routine Description:
    This function is called for each devnode after the system has transitioned hardware profile states.
Parameters:
    NONE.
Return Value:
    NONE.
--*/
{
    ...
    if (DeviceNode->Flags & DNF_STARTED) {
        // Calling this function will disable the device if it is appropriate to do so.
        if (!IopIsDeviceInstanceEnabled(NULL, &DeviceNode->InstancePath, FALSE)) {
            IopRequestDeviceRemoval(DeviceNode->PhysicalDeviceObject, CM_PROB_DISABLED);
        }
    } else if (IopIsDevNodeProblem(DeviceNode, CM_PROB_DISABLED)) {
        // We might be turning on the device. So we will clear the problem flags iff the device problem was CM_PROB_DISABLED.
        IopClearDevNodeProblem(DeviceNode);

        // Make sure the device stays down iff appropriate.
        if (IopIsDeviceInstanceEnabled(NULL, &DeviceNode->InstancePath, FALSE)) {
            // This device should come back online. Queue up an enumeration at the parent level for him.
