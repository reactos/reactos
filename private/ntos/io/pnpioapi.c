...
// This should never have to grow, since 200 is the maximum length of a device instance name...
...
extern NTSTATUS PiNotifyUserMode(PPNP_DEVICE_EVENT_ENTRY DeviceEvent);
...
NTSTATUS IopDeviceInterfaceKeysFromSymbolicLink(
...
/*++
Routine Description:
    This routine lets drivers query the registry properties associated with the specified device.
Parameters:
    DeviceObject - Supplies the device object whoes registry property is to be returned.  This device object should be the one created by a bus driver.
    ...
    ResultLength - Supplies a pointer to a variable to receive the size of the property data returned.
ReturnValue:
    Status code that indicates whether or not the function was successful.
    If PropertyBuffer is not big enough to hold requested data, STATUS_BUFFER_TOO_SMALL will be returned and ResultLength will be set to the number of bytes actually required.
*/
{
    ...
    *ResultLength = 0;
    ...
    case DevicePropertyBusTypeGuid:
        ...
        if (deviceNode->ChildBusTypeIndex != 0xffff &&  deviceNode->ChildBusTypeIndex < IopBusTypeGuidList->Count) {
            *ResultLength = sizeof(GUID);
            if(*ResultLength <= BufferLength) {
                RtlCopyMemory(PropertyBuffer, &(IopBusTypeGuidList->Guid[deviceNode->ChildBusTypeIndex]), sizeof(GUID));
    ...
    case DevicePropertyEnumeratorName:
        ...
        // HTREE\Root\0, but no one should ever be calling us with that PDO anyway.
        ASSERT (deviceInstanceName);

        // We know we're going to find a separator character (\) in the string, so the fact that unicode strings may not be null-terminated isn't a problem.
    ...
    // Enter critical section and acquire a lock on the registry.
    // Both these mechanisms are required to prevent deadlock in the case where an APC routine calls this routine after the registry resource has been claimed
    // in this case it would wait blocking this thread so the registry would never be released -> deadlock.
    // Critical sectioning the registry manipulatio portion solves this problem
    KeEnterCriticalRegion();
    ExAcquireResourceExclusive(&PpRegistryDeviceResource, TRUE);

    // Based on the PDO specified by caller, find the handle of its device instance registry key.
...
clean1:
    ...
    if (NT_SUCCESS(status)) {
        ...
        *ResultLength = keyValueInformation->DataLength;
        ...
        // Check that the buffer we have been given is big enough and that the value returned is of the correct registry type
        if (keyValueInformation->DataLength <= BufferLength) {
            if (keyValueInformation->Type == valueType) {
                RtlCopyMemory(PropertyBuffer, KEY_VALUE_DATA(keyValueInformation), keyValueInformation->DataLength);
            } else {
               status = STATUS_INVALID_PARAMETER_2;
            }
        } else {
...
/*++
Routine Description:
    This routine returns a handle to an opened registry key that the driver may use to store/retrieve configuration information specific to a particular device instance.
    The driver must call ZwClose to close the handle returned from this api when access is no longer required.
Parameters:
    DeviceObject   - Supples the device object of the physical device instance to open a registry storage key for.  Normally it is a device object created by the hal bus extender.
    DevInstKeyType - Supplies flags specifying which storage key associated with the device instance is to be opened.  May be a combination of the following value:

                     PLUGPLAY_REGKEY_DEVICE - Open a key for storing device specific (driver-independent) information relating to the device instance.
                         The flag may not be specified with PLUGPLAY_REGKEY_DRIVER.

                     PLUGPLAY_REGKEY_DRIVER - Open a key for storing driver-specific information relating to the device instance,  This flag may not be specified with PLUGPLAY_REGKEY_DEVICE.

                     PLUGPLAY_REGKEY_CURRENT_HWPROFILE - If this flag is specified, then a key in the current hardware profile branch will be
                         opened for the specified storage type.  This allows the driver to access configuration information that is hardware profile specific.

    DesiredAccess - Specifies the access mask for the key to be opened.

    DevInstRegKey - Supplies the address of a variable that receives a handle to the opened key for the specified registry storage location.
...
*/
{
    ...
    *DevInstRegKey = NULL;
    ...
    // Select the base path to the CurrentControlSet based on if we are dealing with a hardware profile or not
    ...
    // Both these mechanisms are required to prevent deadlock in the case where an APC routine calls this routine after the registry resource has been claimed
    // in this case it would wait blocking this thread so the registry would never be released -> deadlock.
    // Critical sectioning the registry manipulation portion solves this problem
    KeEnterCriticalRegion();
    ExAcquireResourceExclusive(&PpRegistryDeviceResource, TRUE);
    ...
    if (NT_SUCCESS(status)){
        if (DevInstKeyType == PLUGPLAY_REGKEY_DEVICE) {
            ...
            status = IopCreateRegistryKeyEx( DevInstRegKey,hBasePath,&unicodeRestPath,DesiredAccess,REG_OPTION_NON_VOLATILE,NULL);
        }
    }
...
/*++
Routine Description:
    This API notifies the system that changes have occurred in the device relations of the specified type for the supplied DeviceObject.
    All cached information concerning the relationships must be invalidated, and if needed re-obtained via IRP_MN_QUERY_DEVICE_RELATIONS.

    This routine performs device enumeration synchronously.
    Note, A driver can NOT call this IO api while processing pnp irps AND A driver can NOT call this api from any system thread except the system threads created by the driver itself.
Parameters:
    DeviceObject - the PDEVICE_OBJECT for which the specified relation type information has been invalidated.  This pointer is valid for the duration of the call.
...
*/
{
    ...
    case BusRelations:
        if (PnPInitialized) {
            ...
            if (deviceNode->Flags & DNF_STARTED) {
                ...
                status = IopRequestDeviceAction( DeviceObject,ReenumerateDeviceTree,&completionEvent,NULL );
                if (NT_SUCCESS(status)) {
    ...
    case EjectionRelations:
        // For Ejection relation change, we will ignore it.
        //  We don't keep track the Ejection relation.
        // We will query the Ejection relation only when we are requested to eject a device.
...
*/
{
    ...
    switch (Type) {
    case BusRelations:
        // If the call was made before PnP completes device enumeration we can safely ignore it.  PnP manager will do it without driver's request.
        deviceNode = (PDEVICE_NODE) DeviceObject->DeviceObjectExtension->DeviceNode;
        if (deviceNode) {
            ...
            } else if ((deviceNode->Flags & DNF_STARTED)                       && !(deviceNode->Flags & DNF_ENUMERATION_REQUEST_QUEUED)) {
...
/*++
Routine Description:
    This API notifies that the device eject button has been pressed. This API must be called at IRQL <= DISPATCH_LEVEL.

    This API informs PnP that a device eject has been requested, the device will not necessarily be ejected as a result of this API.
    The device will only be ejected if the drivers associated with it agree to stop and the device is successfully powered down.
    Note that eject in this context refers to device eject, not to media (floppies, cds, tapes) eject.
    For example, eject of a cd-rom disk drive, not ejection of a cd-rom disk.
Arguments:
    DeviceObject - the PDEVICE_OBJECT for the device whose eject button has been pressed.
                   This pointer is valid for the duration of the call, if the API wants to keep a copy of it, it should obtain its own reference to the object (ObReferenceObject).
*/
{
    ASSERT_PDO(DeviceObject);
...
VOID IopRequestDeviceEjectWorker(PVOID Context)
{
    ...
    PpSetTargetDeviceRemove( deviceObject,TRUE,TRUE,TRUE,CM_PROB_DEVICE_NOT_THERE,NULL,NULL,NULL,NULL);
...
/*++
Routine Description:
    PnP device drivers call this API to report any device detected.
    This routine creates a Physical Device object, reference the Physical Device object and returns back to the callers.
    Once the detected device is reported, the Pnp manager considers the device has been fully controlled by the reporting drivers.
    Thus it will not invoke AddDevice entry and send StartDevice irp to the driver.

    The driver needs to report the resources it used to detect this device such that pnp manager can perform duplicates detection on this device.
    The caller must dereference the DeviceObject once it no longer needs it.
Parameters:
    DriverObject - Supplies the driver object of the driver who detected this device.
    ResourceList - Supplies a pointer to the resource list which the driver used to detect the device.
    ResourceRequirements - supplies a pointer to the resource requirements list for the detected device.  This is optional.
    ResourceAssigned - if TRUE, the driver already called IoReportResourceUsage or IoAssignResource to get the ownership of the resources.
                   Otherwise, the PnP manager will call IoReportResourceUsage to allocate the resources for the driver.
    DeviceObject - if NULL, this routine will create a PDO and return it thru this variable.
                   Otherwise, a PDO is already created and this routine will simply use the supplied PDO.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    if (*DeviceObject) {
        ...
        status = IopCreateRegistryKeyEx( &logConfHandle,handle,&unicodeName,KEY_ALL_ACCESS,REG_OPTION_NON_VOLATILE,NULL);
        ZwClose(handle);
        if (NT_SUCCESS(status)) {
            // Write the ResourceList and and ResourceRequirements to the logconf key under device instance key.
            if (ResourceList) {
                RtlInitUnicodeString(&unicodeName, REGSTR_VAL_BOOTCONFIG);
                ZwSetValueKey(logConfHandle,&unicodeName,TITLE_INDEX_VALUE,REG_RESOURCE_LIST,ResourceList,listSize = IopDetermineResourceListSize(ResourceList));
            }
            if (ResourceRequirements) {
                RtlInitUnicodeString(&unicodeName, REGSTR_VALUE_BASIC_CONFIG_VECTOR);
                ZwSetValueKey(logConfHandle,&unicodeName,TITLE_INDEX_VALUE,REG_RESOURCE_REQUIREMENTS_LIST,ResourceRequirements,ResourceRequirements->ListSize);
            }
    ...
    *DeviceObject = NULL;
    ...
    // When a builtin driver calls IoReportDetectedDevice, the ServiceKeyName of the driver object is set to \Driver\DriverName.
    // To create a detected device instance key, we will take only the DriverName.
    if (DriverObject->Flags & DRVO_BUILTIN_DRIVER) {
        ...
        if (NT_SUCCESS(status) && deviceNode) {
            ...
            if ((deviceNode->Flags & DNF_ADDED) ||
                (IopDoesDevNodeHaveProblem(deviceNode) && deviceNode->Problem != CM_PROB_NOT_CONFIGURED && deviceNode->Problem != CM_PROB_REINSTALL && deviceNode->Problem != CM_PROB_FAILED_INSTALL)) {
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (deviceNode) {
            ...
            status = PpCreateLegacyDeviceIds(deviceObject,((DriverObject->Flags & DRVO_BUILTIN_DRIVER) ?&driverName : serviceName),ResourceList);
            if(!NT_SUCCESS(status)) {
                goto exit;
            }

            // Create/Open a registry key for the device instance and write the addr of the device object to registry
            if (DriverObject->Flags & DRVO_BUILTIN_DRIVER) {
            ...
            status = IopCreateRegistryKeyEx( &handle1,enumHandle,&deviceName,KEY_ALL_ACCESS,REG_OPTION_NON_VOLATILE,&disposition);
            if (NT_SUCCESS(status)) {
                deviceName.Buffer[deviceName.Length / sizeof(WCHAR)] = OBJ_NAME_PATH_SEPARATOR;
                deviceName.Length += sizeof(WCHAR);
                if (disposition != REG_CREATED_NEW_KEY) {
                    while (TRUE) {
                        PiUlongToInstanceKeyUnicodeString(&instanceName,buffer + deviceName.Length / sizeof(WCHAR),sizeof(buffer) - deviceName.Length,i);
                        status = IopCreateRegistryKeyEx( &handle,handle1,&instanceName,KEY_ALL_ACCESS,REG_OPTION_NON_VOLATILE,&disposition);
                        if (NT_SUCCESS(status)) {
                            if (disposition == REG_CREATED_NEW_KEY) {
                                ZwClose(handle1);
                                break;
                                ...
                                if (IopIsReportedAlready(handle, serviceName, ResourceList)) {
                                    ...
                                    ZwClose(handle1);
                                    deviceObject = IopDeviceObjectFromDeviceInstance (handle, NULL);  // Add a reference
                                    ZwClose(handle);
                                    ZwClose(enumHandle);
                                    ASSERT(deviceObject);
            ...
            if (NT_SUCCESS(status)) {
                ...
                status = ZwSetValueKey(controlHandle,&unicodeName,TITLE_INDEX_VALUE,REG_DWORD,(PULONG_PTR)&deviceObject,sizeof(ULONG_PTR));
            ...
            // Create Service value name and set it to the calling driver's service key name.
            ...
            ZwSetValueKey(handle,&unicodeName,TITLE_INDEX_VALUE,REG_SZ,p,serviceName->Length + sizeof(UNICODE_NULL));
            if (DriverObject->Flags & DRVO_BUILTIN_DRIVER) {
            ...
            // Register the device for the driver and save the device instance path in device node.
            if (!(DriverObject->Flags & DRVO_BUILTIN_DRIVER)) {
...
checkResource:
    // At this point the *DeviceObject is established.  Check if we need to report resources for the detected device.  If we failed to
    if (ResourceAssigned) {
        //ASSERT(deviceNode->ResourceList == NULL);      // make sure we have not reported resources yet.

        // If the driver specifies it already has acquired the resource.  We will put a flag in the device instance path to not to allocate resources at boot time.
        // The Driver may do detection and report it again.
...
/*++
Routine Description:
    This routine determines if the reported device instance is already reported or not.
Parameters:
...
*/
{
    ...
    // If this registry key is for a device reported during the same boot this is not a duplicate.
    RtlInitUnicodeString(&unicodeName, REGSTR_KEY_CONTROL);
                    ...
                    if (NT_SUCCESS(status)) {
                        ...
                        if (NT_SUCCESS(status)) {
                            if ((keyValueInfo2->Type == REG_RESOURCE_LIST) && (keyValueInfo2->DataLength != 0)) {
                                ...
                                if (ResourceList && cmResource && IopIsDuplicatedDevices(ResourceList, cmResource, NULL, NULL)) {
                                    returnValue = TRUE;
                                }
                            }
                        }
...
/*++
Routine Description:
    Allocates a buffer of Size bytes and initialises the BUFFER_INFO structure so the current position is at the start of the buffer.
Parameters:
    Info - Pointer to a buffer info structure to be used to manage the new buffer
...
/*++
Routine Description:
    Allocates a new buffer of NewSize bytes and associates it with Info, freeing the old buffer.
    It will optionally copy the data stored in the old buffer into the new buffer and update the current position.
Parameters:
    Info - Pointer to a buffer info structure to be used to manage the buffer
    NewSize - The new size of the buffer in bytes
    CopyContents - If TRUE indicates that the contents of the old buffer should be copied to the new buffer
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
...
/*++
Routine Description:
    Copies the data to the end of the buffer, resizing if necessary.  The current position is set to the end of the data just added.
Parameters:
    Info - Pointer to a buffer info structure to be used to manage the buffer
...
*/
{
    ...
    RtlCopyMemory(Info->Current,Data,DataSize);// Copy the data into the buffer
    Info->Current += DataSize;// Advance down the buffer
...
/*++
Routine Description:
    Copies data into the buffer, overwriting what is currently present, resising if necessary.
    The current position is set to the end of the data just added.
Parameters:
    Info - Pointer to a buffer info structure to be used to manage the buffer
    Data - Pointer to the data to be added to the buffer
    DataSize - The size of the data pointed to by Data in bytes
...
*/
{
    ...
    RtlCopyMemory(Info->Buffer,Data,DataSize);// Copy the data into the buffer
    Info->Current += DataSize;// Advance down the buffer
...
/*++
Routine Description:
    This API allows a WDM driver to get a list of paths that represent all devices registered for the specified interface class.
Parameters:

    InterfaceClassGuid - Supplies a pointer to a GUID representing the interface class for whom a list of members is to be retrieved

    DevicePath - Optionally, supplies a pointer to a unicode string containing the enumeration path for a device for whom interfaces of the specified class are
        to be re-trieved.  If this parameter  is not supplied, then all interface devices (regardless of what physical device exposes them) will be returned.

    Flags - Supplies flags that modify the behavior of list retrieval.
        The following flags are presently defined:

        DEVICE_INTERFACE_INCLUDE_NONACTIVE -- If this flag is specified, then all interface devices, whether currently active or not, will be returned
    ...
    SymbolicLinkList - Supplies the address of a character pointer, that on success will contain a multi-sz list of \??\ symbolic link
        names that provide the requested functionality.  The caller is responsible for freeing the memory via ExFreePool.
...
*/
{
    ...
    *SymbolicLinkList = NULL;// Initialise out parameters
    ...
    if (NT_SUCCESS(status) && pDefaultInfo->Type == REG_SZ && pDefaultInfo->DataLength >= sizeof(WCHAR)) {
        ...
        status = IopOpenOrCreateDeviceInterfaceSubKeys(NULL,NULL,&hKey,NULL,hClass,&defaultString,KEY_READ,FALSE);
        if (!NT_SUCCESS(status)) {
            ...
            // If we are just supposed to return live interfaces, then make sure this default interface is linked.
            if (!(Flags & DEVICE_INTERFACE_INCLUDE_NONACTIVE)) {
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    status = ZwQueryValueKey(hControl,&tempString,KeyValuePartialInformation,(PVOID) infoBuffer.Buffer,infoBuffer.MaxSize,&resultSize);
                    ZwClose(hControl);

                    // We don't need to check the buffer was big enough because it starts off that way and doesn't get any smaller!
                    if (NT_SUCCESS(status)
                        ...
                        defaultPresent = *(PULONG)(((PKEY_VALUE_PARTIAL_INFORMATION)(infoBuffer.Buffer))->Data) ? TRUE : FALSE;
                    }
                }
            }
            ...
            if(defaultPresent) {
                ...
                status = IopAppendBuffer(&returnBuffer,defaultString.Buffer,defaultString.Length + sizeof(UNICODE_NULL));
                if (!UserModeFormat) {
                    RtlCopyMemory(returnBuffer.Buffer,KERNEL_SYMLINK_STRING_PREFIX,KERNEL_SYMLINK_STRING_PREFIX_LENGTH);
                }
            } else {
    ...
    while((status = ZwEnumerateKey(hClass,keyIndex,KeyBasicInformation,(PVOID) infoBuffer.Buffer,infoBuffer.MaxSize,&resultSize)) != STATUS_NO_MORE_ENTRIES)
    {
        ...
        // If we're filtering on a particular PDO, then retrieve the owning device instance name for this interface key, and make sure they match.
        PiWstrToUnicodeString(&tempString, REGSTR_VAL_DEVICE_INSTANCE);
        while ((status = ZwQueryValueKey(hKey,&tempString,KeyValuePartialInformation,devnodeNameBuffer.Buffer,devnodeNameBuffer.MaxSize,&resultSize)) == STATUS_BUFFER_TOO_SMALL ) {
        ...
        while((status = ZwEnumerateKey(hKey,instanceKeyIndex,KeyBasicInformation,(PVOID) infoBuffer.Buffer,infoBuffer.MaxSize,&resultSize)) != STATUS_NO_MORE_ENTRIES)
        {
            if (status == STATUS_BUFFER_TOO_SMALL) {
                status = IopResizeBuffer(&infoBuffer, resultSize, FALSE);
                continue;
            ...
            while ((status = ZwQueryValueKey(hInstanceKey,&tempString,KeyValuePartialInformation,symLinkBuffer.Buffer,symLinkBuffer.MaxSize,&resultSize)) == STATUS_BUFFER_TOO_SMALL ) {
            ...
            // If we are only returning interfaces for a particular PDO then check this is from that PDO
            ...
            if (!UserModeFormat) {
                RtlCopyMemory(returnBuffer.Current - (symLinkString.Length + sizeof(UNICODE_NULL)),KERNEL_SYMLINK_STRING_PREFIX,KERNEL_SYMLINK_STRING_PREFIX_LENGTH);
            }
...
clean5:
    ...
    status = IopResizeBuffer(&returnBuffer,(ULONG) (returnBuffer.Current - returnBuffer.Buffer + sizeof(UNICODE_NULL)),TRUE);
    if (NT_SUCCESS(status)) {
...
/*++
Routine Description:
    This API allows a WDM driver to get a list of paths that represent all device interfaces registered for the specified interface class.
Parameters:
    InterfaceClassGuid - Supplies a pointer to a GUID representing the interface class for whom a list of members is to be retrieved

    PhysicalDeviceObject - Optionally, supplies a pointer to the PDO for whom interfaces of the specified class are to be re-trieved.
        If this parameter is not supplied, then all interface devices (regardless of what physical device exposes them) will be returned.

    Flags - Supplies flags that modify the behavior of list retrieval.
        The following flags are presently defined:

        DEVICE_INTERFACE_INCLUDE_NONACTIVE -- If this flag is specified, then all device interfaces, whether currently active or not, will be returned
            (potentially filtered based on the PhysicalDeviceObject, if specified).

    SymbolicLinkList - Supplies the address of a character pointer, that on success will contain a multi-sz list of \DosDevices\ symbolic link names that provide the requested functionality.
        The caller is responsible for freeing the memory via ExFreePool
...
Parameters:
    Buffer - Supplies a pointer to a pointer to the buffer that is being reallocated.
        On sucessful completion it the pointer will be updated to point to the new buffer, on failure it will still point to the old buffer.
    OldSize - The size in bytes of the memory block referenced by Buffer
    NewSize - The desired new size in bytes of the buffer.  This can be larger or smaller than the OldSize
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    ExFreePool(*Buffer);// Free up the old buffer
    *Buffer = newBuffer;// Hand the new buffer back to the caller
    return STATUS_SUCCESS;
}
...
/*++
Routine Description:
    This DDI allows a device class to activate and deactivate an association previously registered using IoRegisterDeviceInterface
Parameters:
    SymbolicLinkName - Supplies a pointer to the symbolic link name which was returned by IoRegisterDeviceInterface when the interface was registered, or as returned by IoGetDeviceInterfaces.
    Enable - If TRUE (non-zero), the interface will be enabled.  If FALSE, it will be disabled.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    if (!NT_SUCCESS(status) && !Enable) {
        status = STATUS_SUCCESS;// If we failed to disable an interface (most likely because the interface keys have already been deleted) report success.
    }
...
/*++
Routine Description:
    This routine will open the registry key where the data associated with a specific device interface can be stored.
Parameters:
    SymbolicLinkName - Supplies a pointer to the symbolic link name which was returned by IoRegisterDeviceInterface when the device class was registered.
    DesiredAccess - Supplies the access privileges to the key the caller wants.
    DeviceInterfaceKey - Supplies a pointer to a handle which on success will contain the handle to the requested registry key.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    status = IopCreateRegistryKeyEx( DeviceInterfaceKey,hKey,&unicodeString,DesiredAccess,REG_OPTION_NON_VOLATILE,NULL);
    ZwClose(hKey);
...
/*++
Routine Description:
    This routine will open the registry key where the data associated with the device pointed to by SymbolicLinkName is stored.
    If the path does not exist it will not be created.
Parameters:
    SymbolicLinkName - Supplies a pointer to the symbolic link name.
    DesiredAccess - Supplies the access privto the function class instance key the caller wants.
    DeviceInterfaceClassKey - Optionally, supplies the address of a variable that receives a handle to the device class key for the interface.
    DeviceInterfaceKey - Optionally, supplies the address of a variable that receives a handle to the device interface (parent) key.
    DeviceInterfaceInstanceKey - Optionally, Supplies the address of a variable that receives a handle to the device interface instance key (i.e., the refstring-specific one).
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    status = IopOpenOrCreateDeviceInterfaceSubKeys(DeviceInterfaceKey,NULL,DeviceInterfaceInstanceKey,NULL,hFunctionClass,SymbolicLinkName,DesiredAccess,FALSE);
...
/*++
Routine Description:
    This device driver interface allows a WDM driver to register a particular interface of its underlying hardware (ie PDO) as a member of a function class.
Parameters:
    PhysicalDeviceObject - Supplies a pointer to the PDO for the P&P device instance associated with the functionality being registered
    InterfaceClassGuid - Supplies a pointer to the GUID representring the functionality to be registered
    ReferenceString - Optionally, supplies an additional context string which is appended to the enumeration path of the device
    SymbolicLinkName - Supplies a pointer to a string which on success will contain the kernel mode path of the symbolic link used to open this device.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
...
/*++
Routine Description:
    This is the worker routine for PnpRegisterDeviceInterface.
    It is also called by the user-mode ConfigMgr (via an NtPlugPlayControl),
    which is why it must take a device instance name instead of a PDO (since the device instance may not currently be 'live'),
    and also why it must optionally return the user-mode form of the interface device name (i.e., "\\?\" instead of "\??\").
Parameters:
    DeviceInstanceName - Supplies the name of the device instance for which a device interface is being registered.
    InterfaceClassGuid - Supplies a pointer to the GUID representring the class of the device interface being registered.
    ReferenceString - Optionally, supplies an additional context string which is appended to the enumeration path of the device
    UserModeFormat - If non-zero, then the symbolic link name returned for the interface device is in user-mode form (i.e., "\\?\").  If zero (FALSE), it is in kernel-mode form (i.e., "\??\").
    SymbolicLinkName - Supplies a pointer to a string which on success will contain either the kernel-mode or user-mode path of the symbolic link used to open this device.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    // Construct both flavors of symbolic link name (go ahead and store the form that the user wants in the SymbolicLinkName parameter they supplied--this saves us from having to copy the appropriate string over to their string later).
    if(UserModeFormat) {
    ...
    // Now open/create the two-level device interface hierarchy underneath this interface class key.
    status = IopOpenOrCreateDeviceInterfaceSubKeys(&hTemp1,&InterfaceDisposition,&hInterfaceInstanceKey,&InterfaceInstanceDisposition,hTemp2,pUserString,KEY_WRITE | DELETE,TRUE);
    ZwClose(hTemp2);
    if(!NT_SUCCESS(status)) {
        goto clean2;
    }
...
clean3:
    if (!NT_SUCCESS(status)) {
        // Since we failed to register the device interface, delete any keys that were newly created in the attempt.
...
/*++
Routine Description:
    This routine removes the interface instance subkey of ReferenceString from the interface for DeviceInstanceName to the given InterfaceClassGuid.
    If the interface instance specified by the Reference String portion of SymbolicLinkName is the only instance of the interface, the interface subkey is removed from the device class key as well.
Parameters:
    SymbolicLinkName - Supplies a pointer to a unicode string which contains the symbolic link name of the device to unregister.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    if ( (!ARGUMENT_PRESENT(SymbolicLinkName)) || (!NT_SUCCESS(IopParseSymbolicLinkName(SymbolicLinkName,NULL,&mungedPathString,&guidString,&refString,&refStringPresent,&guid))) ) {
    ...
    // Set the MaximumLength of the Buffer, and append the REFSTRING_PREFIX_CHAR to it.
    *instanceKeyName.Buffer = REFSTRING_PREFIX_CHAR;
    ...
    // (includes KEY_STRING_PREFIX, mungedPathString, separating '#' char, and the guidString)
    ...
    status = IopDeviceInterfaceKeysFromSymbolicLink(SymbolicLinkName,KEY_ALL_ACCESS,&hInterfaceClassKey,&hInterfaceKey,&hInterfaceInstanceKey);
    if (!NT_SUCCESS(status)) {
        goto clean2;
    }
    ...
    if (NT_SUCCESS(status)) {
        // Check the "linked" value under the "Control" subkey of this interface instance
    ...
    // Ignore any status code returned while attempting to retieve the state of the device.
    // The value of linked will tell us if we need to disable the interface instance first.

    // If no instance "Control" subkey or "linked" value was present (status == STATUS_OBJECT_NAME_NOT_FOUND), this interface instance is not currently enabled -- ok to delete.

    // If the attempt to retrieve these values failed with some other error, any attempt to disable the interface will also likely fail, so we'll just have to delete this instance anyways.
    status = STATUS_SUCCESS;

    if (linked) {
        // Disabled the active interface before unregistering it, ignore any status returned, we'll delete this interface instance key anyways.
    ...
    if ((remainingSubKeys==0) || ((remainingSubKeys==1) && (NT_SUCCESS(status)))) {
        // If the interface key has no subkeys, or the only the remaining subkey is the volatile interface "Control" subkey, then there are no more instances to this interface.
        // We should delete the interface key itself also.
...
/*++
Routine Description:
    This routine checks all device class keys under HKLM\SYSTEM\CCS\Control\DeviceClasses for interfaces for which the DeviceInstance value matches the supplied DeviceInstancePath.
    Instances of such device interfaces are unregistered, and the device interface subkey itself is removed.

    Note that a lock on the registry must have already been acquired, by the caller of this routine.
Parameters:
    DeviceInterfacePath - Supplies a pointer to a unicode string which contains the DeviceInterface name of the device for which interfaces to are to be removed.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    classIndex = 0;
    while((status = ZwEnumerateKey(hDeviceClasses,classIndex,KeyBasicInformation,(PVOID) classInfoBuffer.Buffer,classInfoBuffer.MaxSize,&resultSize)) != STATUS_NO_MORE_ENTRIES) {
        if (status == STATUS_BUFFER_TOO_SMALL) {
        ...
        interfaceIndex = 0;
        while((status = ZwEnumerateKey(hClassGUID,interfaceIndex,KeyBasicInformation,(PVOID) interfaceInfoBuffer.Buffer,interfaceInfoBuffer.MaxSize,&resultSize)) != STATUS_NO_MORE_ENTRIES) {
            if (status == STATUS_BUFFER_TOO_SMALL) {
            ...
            status = IopAllocateUnicodeString(&interfaceString,(USHORT)((PKEY_BASIC_INFORMATION)(interfaceInfoBuffer.Buffer))->NameLength);
            if (!NT_SUCCESS(status)) {
                goto clean1;
            }
            ...
            RtlCopyMemory(interfaceString.Buffer,((PKEY_BASIC_INFORMATION)(interfaceInfoBuffer.Buffer))->Name,interfaceString.Length);
            ...
            if(!NT_SUCCESS(status)) {
                //  Couldn't get the DeviceInstance for this interface -- skip it and move on.
                goto CloseInterfaceKeyAndContinue;
            }

            if((deviceInstanceInfo->Type == REG_SZ) && (deviceInstanceInfo->DataLength != 0)) {
                IopRegistryDataToUnicodeString(&deviceInstanceString,(PWSTR)KEY_VALUE_DATA(deviceInstanceInfo),deviceInstanceInfo->DataLength);
            } else {
            ...
            if (RtlEqualUnicodeString(&deviceInstanceString, DeviceInstancePath, TRUE)) {
                ...
                if (NT_SUCCESS(status)) {
                    ...
                    while(*symLink != UNICODE_NULL) {
                        ...
                        // Unregister this instance of the interface.
                        // Since we are removing the device, ignore any returned status, since there isn't anything we can do about interfaces which fail unregistration.
                ...
                // While IopUnregisterDeviceInterface will itself delete the interface key if no interface instance subkeys remain, if any of the above calls to IopUnregisterDeviceInterface failed to
                // delete an interface instance key, subkeys will remain, and the interface key will not have been deleted.  We'll catch that here.
            ...
            ExFreePool(deviceInstanceInfo);// Free allocated key info structure
...
/*++
Routine Description:
    This API opens or creates a two-level registry hierarchy underneath the specified interface class key for a particular device interface.
    The first level is the (munged) symbolic link name (sans RefString).
    The second level is the refstring, prepended with a '#' sign (if the device interface has no refstring, then this key name is simply '#').
Parameters:
    InterfaceKeyHandle - Optionally, supplies the address of a variable that receives a handle to the interface key (1st level in the hierarchy).
    InterfaceKeyDisposition - Optionally, supplies the address of a variable that receives either REG_CREATED_NEW_KEY or REG_OPENED_EXISTING_KEY indicating whether the interface key was newly-created.
    InterfaceInstanceKeyHandle - Optionally, supplies the address of a variable that receives a handle to the interface instance key (2nd level in the hierarchy).
    InterfaceInstanceDisposition - Optionally, supplies the address of a variable that receives either REG_CREATED_NEW_KEY or REG_OPENED_EXISTING_KEY indicating whether the interface instance key was newly-created.
    InterfaceClassKeyHandle - Supplies a handle to the interface class key under which the device interface keys are to be opened/created.
    DeviceInterfaceName - Supplies the (user-mode or kernel-mode form) device interface name.
...
*/
{
    ...
    if(RefStringPresent) {
        ...
        // Set up refstring to point to a temporary character buffer that will hold the single '#' used for the key name when no refstring is present.
    ...
    if (Create) {
        status = IopCreateRegistryKeyEx( &hTempInterface,InterfaceClassKeyHandle,&TempString,DesiredAccess,REG_OPTION_NON_VOLATILE,&TempInterfaceDisposition);
    } else {
    ...
    // Store a '#' as the first character of the RefString, and then we're ready to open the refstring subkey.
    *RefString.Buffer = REFSTRING_PREFIX_CHAR;
    ...
    if (Create) {
        status = IopCreateRegistryKeyEx( &hTempInterfaceInstance,hTempInterface,&RefString,DesiredAccess,REG_OPTION_NON_VOLATILE,InterfaceInstanceDisposition);
    } else {
...
/*++
Routine Description:
    This API returns a symbolic link name (i.e., device interface) of a particular interface class that 'aliases' the specified device interface.
    Two device interfaces are considered aliases of each other if the following two criteria are met:
        1.  Both interfaces are exposed by the same PDO (devnode).
        2.  Both interfaces share the same RefString.
Parameters:
    SymbolicLinkName - Supplies the name of the device interface whose alias is to be retrieved.
    AliasInterfaceClassGuid - Supplies a pointer to the GUID representing the interface class for which an alias is to be retrieved.
    AliasSymbolicLinkName - Supplies a pointer to a string which, upon success, will contain the name of the device interface in the specified class that aliases the SymbolicLinkName interface.
        (This symbolic link name will be returned in either kernel-mode or user-mode form, depeding upon the form of the SymbolicLinkName path).
        It is the caller's responsibility to free the buffer allocated for this string via ExFreePool().
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    if(pDeviceInstanceInfo->Type == REG_SZ) {
        IopRegistryDataToUnicodeString(&deviceInstanceString,(PWSTR)KEY_VALUE_DATA(pDeviceInstanceInfo),pDeviceInstanceInfo->DataLength);
    } else {
        status = STATUS_INVALID_PARAMETER_1;
        goto clean2;
    }

    // Now parse out the refstring, so that we can construct the name of the interface device's alias.
    // (NOTE: we have not yet verified that the alias actually exists, we're only constructing what its name would be, if it did exist.)

    // Don't bother to check the return code.
    // If this were a bad string, we'd have already failed above when we called IopDeviceInterfaceKeysFromSymbolicLink.
    IopParseSymbolicLinkName(SymbolicLinkName,NULL,NULL,NULL,&refString,&refStringPresent,NULL);

    // Did the caller supply us with a user-mode or kernel-mode format path?
    userModeFormat = (IopConstStringSize(USER_SYMLINK_STRING_PREFIX) == RtlCompareMemory(SymbolicLinkName->Buffer,USER_SYMLINK_STRING_PREFIX,IopConstStringSize(USER_SYMLINK_STRING_PREFIX)));
    if(userModeFormat) {
    ...
    status = IopBuildSymbolicLinkStrings(&deviceInstanceString,&guidString,refStringPresent ? &refString : NULL,pUserString,pKernelString);
    if (!NT_SUCCESS(status)) {
        goto clean2;
    }

    // OK, we now have the symbolic link name of the alias, but we don't yet know whether it actually exists.
    // Check this by attempting to open the associated registry key.
...
/*++
Routine Description:
    This routine will construct various strings used in the registration of function device class associations (IoRegisterDeviceClassAssociation).
    The specific strings are detailed below
Parameters:
    DeviceString - Supplies a pointer to the instance path of the device. It is of the form <Enumerator>\<Device>\<Instance>.
    GuidString - Supplies a pointer to the string representation of the function class guid.
    ReferenceString - Supplies a pointer to the reference string for the given device to exhibit the given function.  This is optional
    UserString - Supplies a pointer to an uninitialised string which on success will contain the string to be assigned to the "SymbolicLink" value under the KeyString.
        It is of the format \\?\<MungedDeviceString>\<GuidString>\<Reference> When no longer required it should be freed using IopFreeAllocatedUnicodeString.
    KernelString - Supplies a pointer to an uninitialised string which on success will contain the kernel mode path of the device and is of the format \??\<MungedDeviceString>\<GuidString>\<Reference>.
...
*/
{
    ...
    // The code is optimised to use the fact that \\.\ and \??\ are the same size - if these prefixes change then we need to change the code.
    ASSERT(IopConstStringSize(KERNEL_SYMLINK_STRING_PREFIX) == IopConstStringSize(USER_SYMLINK_STRING_PREFIX));

    // Calculate the lengths of the strings
    length = IopConstStringSize(KERNEL_SYMLINK_STRING_PREFIX) + DeviceString->Length + IopConstStringSize(REPLACED_SEPERATOR_STRING) + GuidString->Length;
...
/*++
Routine Description:
    This routine will copy a string from InString to OutString replacing any occurence of '\' or '/' with '#' as it goes.
Parameters:
    OutString - Supplies a pointer to a string which has already been initialised to have a buffer large enough to accomodate the string.
        The contents of this string will be over written
...
*/
{
    ...
    // Traverse the in string copying and replacing all occurences of '\' or '/' with '#'
    while (count--) {
...
/*++
Routine Description:
    This routine removes the reference string from a symbolic link name.
    No space is allocated for the out string so no attempt should be made to free the buffer of OutString.
Parameters:
    SymbolicLinkName - Supplies a pointer to a symbolic link name string. Both the prefixed strings are valid.
    GuidReferenceString - Supplies a pointer to an uninitialised string which on success will contain the symbolic link name without the reference string. See the note on storage allocation above.
Return Value:
    Status code that indicates whether or not the function was successful.
Remarks:
    The string returned in OutString is dependant on the buffer of InString and is only valid as long as InString is valid.
*/
{
...
/*++
Routine Description:
    This routine breaks apart a symbolic link name constructed by IopBuildSymbolicLinkNames.
    Both formats of name are valid - user mode \\?\ and kernel mode \??\.
Parameters:
    SymbolicLinkName - Supplies a pointer to the symbolic link name to be analysed.
    PrefixString - Optionally contains a pointer to a string which will contain the prefix of the string.
    MungedPathString - Optionally contains a pointer to a string which will contain the enumeration path of the device with all occurences of '\' replaced with '#'.
    GuidString - Optionally contains a pointer to a string which will contain the device class guid in string format from the string.
    RefString - Optionally contains a pointer to a string which will contain the refstring of the string if one is present, otherwise it is undefined.
    RefStringPresent - Optionally contains a pointer to a boolean value which will be set to true if a refstring is present.
    Guid - Optionally contains a pointer to a guid which will contain the function class guid of the string.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    if (!(RtlCompareMemory(SymbolicLinkName->Buffer, USER_SYMLINK_STRING_PREFIX, IopConstStringSize(USER_SYMLINK_STRING_PREFIX) ) != IopConstStringSize(USER_SYMLINK_STRING_PREFIX)
       || RtlCompareMemory(SymbolicLinkName->Buffer,KERNEL_SYMLINK_STRING_PREFIX, IopConstStringSize(KERNEL_SYMLINK_STRING_PREFIX)) != IopConstStringSize(KERNEL_SYMLINK_STRING_PREFIX))) {
        status = STATUS_INVALID_PARAMETER;
        goto clean0;
    }
    ...
    for (current = 0; current < (SymbolicLinkName->Length / sizeof(WCHAR)) - IopConstStringLength(KERNEL_SYMLINK_STRING_PREFIX); current++, pCurrent++) {
    ...
    if (ARGUMENT_PRESENT(MungedPathString)) {
        MungedPathString->Length = (reference - 1 - GUID_STRING_LENGTH - 1 - IopConstStringLength(KERNEL_SYMLINK_STRING_PREFIX)) * sizeof(WCHAR);
...
/*++
Routine Description:
    This routine allocates a buffer for a unicode string of a given length and initialises the UNICODE_STRING structure appropriately.
    When the string is no longer required it can be freed using IopFreeAllocatedString.
    The buffer also be directly deleted by ExFreePool and so can be handed back to a caller.
Parameters:
    String - Supplies a pointer to an uninitialised unicode string which will be manipulated by the function.
    Length - The number of BYTES long that the string will be.
Return Value:
    Either STATUS_INSUFFICIENT_RESOURCES indicating paged pool is exhausted or STATUS_SUCCESS.
Remarks:
    The buffer allocated will be one character (2 bytes) more than length specified.
    This is to allow for easy null termination of the strings - eg for registry storage.
*/
{
    PAGED_CODE();
...
Parameters:
    ...
    ValueData - Supplies a pointer to the string to be stored in the key.  The data will automatically be null terminated for storage in the registry.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    if ((ValueData->MaximumLength - ValueData->Length) >= sizeof(UNICODE_NULL)) {
        ...
        status = ZwSetValueKey(KeyHandle, ValueName, 0, REG_SZ, (PVOID) ValueData->Buffer, ValueData->Length + sizeof(UNICODE_NULL));
    } else {
        UNICODE_STRING tempString;

        // There is no room so allocate a new buffer and so we need to build a new string with room
        ...
        tempString.Buffer[tempString.Length / sizeof(WCHAR)] = UNICODE_NULL;// Add the null termination
        status = ZwSetValueKey(KeyHandle,ValueName,0,REG_SZ,(PVOID) tempString.Buffer,tempString.Length + sizeof(UNICODE_NULL));// Set the registry value
        IopFreeAllocatedUnicodeString(&tempString);// Free the temporary string
    }
...
/*++
Routine Description:
    This routine unregisters a notification previously registered via IoRegisterPlugPlayNotification.
    A driver cannot be unloaded until it has unregistered all of its notification handles.
Parameters:
    NotificationEntry - This provices the cookie returned by IoRegisterPlugPlayNotification which identifies the registration in question.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    if (PiNotificationInProgress) {
        // Before unregistering the entry, we need to make sure that it's not sitting around in the deferred registration list.
        ...
        // If there is currently no notification in progress, the deferred registration list must be empty.
    ...
    if (!entry->Unregistered || wasDeferred) {
        // Dereference the entry if it is currently registered, or had its registration pending completion of the notification in progress.

        entry->Unregistered = TRUE;// Mark the entry as unregistered so we don't notify on it
        IopDereferenceNotify(entry);// Dereference it thus deleting if no longer required
    }
...
/*++
Routine Description:
    This routine removes notification entries from the deferred registration list, marking them as "registered" so that they can receive notifications.
  --*/
{
    PDEFERRED_REGISTRATION_ENTRY deferredNode;
    PFAST_MUTEX lock;
    ...
    while (!IsListEmpty(&IopDeferredRegistrationList)) {
        ...
        // Dereference the notification entry when removing it from the deferred list, and free the node.
        IopDereferenceNotify((PNOTIFY_ENTRY_HEADER)deferredNode->NotifyEntry);
        ExFreePool(deferredNode);
...
/*++
Routine Description:
    This routine may be used to give notification of 3rd-party target device change events.
    This API will notify every driver that has registered for notification on a file object associated with PhysicalDeviceObject about the event indicated in the NotificationStructure.
Parameters:
    PhysicalDeviceObject - Provides a pointer to the PDO that the change begin reported is associated with.
    NotificationStructure - Provides a pointer to the notification structure to be sent to all parties registered for notifications about changes to PhysicalDeviceObject.
Return Value:
    Status code that indicates whether or not the function was successful.
Note:
    This API may only be used to report non-PnP target device changes.
    In particular, it will fail if it's called with the NotificationStructure->Event field set to GUID_TARGET_DEVICE_QUERY_REMOVE, GUID_TARGET_DEVICE_REMOVE_CANCELLED, or GUID_TARGET_DEVICE_REMOVE_COMPLETE.
*/
{
    NTSTATUS status = STATUS_SUCCESS;
    KEVENT completionEvent;
    ...
    KeInitializeEvent(&completionEvent, NotificationEvent, FALSE);
    status = PpSetCustomTargetEvent( PhysicalDeviceObject,&completionEvent,&completionStatus,NULL,NULL,notifyStruct);
    if (NT_SUCCESS(status))  {
...
*/
{
    ...
    // Since this routine can be called at DPC level we need to queue a work item and process it when the irql drops.
    ...
    if (asyncWorkItem != NULL) {
        ...
        asyncWorkItem->NotificationStructure = (PTARGET_DEVICE_CUSTOM_NOTIFICATION)((PUCHAR)asyncWorkItem + sizeof(ASYNC_TDC_WORK_ITEM));
...
Routine Description:
    ...
    Its main purpose is to invoke IopSynchronousQueryDeviceState and release work item space.
Parameters:
...
*/
{
    ...
    PpSetCustomTargetEvent( asyncWorkItem->DeviceObject,NULL,NULL,asyncWorkItem->Callback,asyncWorkItem->Context,asyncWorkItem->NotificationStructure);
...
/*++
Routine Description:
    This API will cause the PnP manager to send the specified PDO an IRP_MN_QUERY_PNP_DEVICE_STATE IRP.
Parameters:
...
*/
{
    ...
    // If the call was made before PnP completes device enumeration we can safely ignore it.
    // PnP manager will do it without driver's request.
    // If the device was already removed or surprised removed then ignore it as well since this is only valid for started devices.
    ...
    IopQueueDeviceWorkItem( PhysicalDeviceObject, IopInvalidateDeviceStateWorker, NULL);
}
...
*/
{
    ...
    ExQueueWorkItem( &deviceWorkItem->WorkItem, DelayedWorkQueue );// Queue a work item to do the enumeration
    return STATUS_SUCCESS;
}
...
*/
{
    ...
    // If the device was removed or surprised removed while the work item was queued then ignore it.
    deviceNode = (PDEVICE_NODE)deviceObject->DeviceObjectExtension->DeviceNode;
    if ((deviceNode->Flags & (DNF_STARTED | DNF_REMOVE_PENDING_CLOSES)) == DNF_STARTED) {
...
Parameters:
    PhysicalDeviceObject - Provides a pointer to the PDO who's state is to be invalidated.
    StopRequired - Supplies a BOOLEAN value to indicate if the resources reallocation needs to be done after device stopped.
*/
{
    PDEVICE_NODE deviceNode;
    ...
    // If for some reason this device did not start, we need to clear some flags such that it can be started later.
    // In this case, we call IopRequestDeviceEnumeration with NULL device object, so the devices will be handled in non-started case.
    // They will be assigned resources, started and enumerated.
    deviceNode->Flags |= DNF_RESOURCE_REQUIREMENTS_CHANGED;

    IopClearDevNodeProblem(deviceNode);

    // If the device is already started, we call IopRequestDeviceEnumeration with the device object.
    if (deviceNode->Flags & DNF_STARTED) {
...
/*++
Routine Description:
    This routine performs initialization required before any of the notification APIs can be called.
*/
{
    ULONG count;
...
Note:
    The appropriate symchronization lock must be held on the notification list before this routine can be called
*/
{
    PAGED_CODE();
...
/*++
Routine Description:
    This routine decrements the reference count for a notification entry, removing the entry from the list and freeing the associated memory if there are no outstanding reference counts.
Parameters:
    Notify - Supplies a pointer to the notification entry to be referenced
Note:
    The appropriate symchronization lock must be held on the notification list before this routine can be called
*/
{
    ...
    if (Notify->RefCount == 0) {
        // If the refcount is zero then the node should have been deregisterd and is no longer needs to be in the list so remove and free it
        ...
        // If this notification entry is for target device change, dereference the PDO upon which this notification entry was hooked.
...
/*++
Routine Description:
    This routine is used to notify all registered drivers of a hardware profile change.
    If the operation is a HW provile change query then the operation is synchronous and the veto information is propagated.
    All other operations are asynchronous and veto information is not returned.
Parameters:
    EventTypeGuid       - The event that has occured
    NotificationTime    - This is used to tell if we are already in an event when delivering a synchronous notification (ie, querying profile change to eject).
                          It is one of three values:
                              PROFILE_IN_PNPEVENT
...
Return Value:
    Status code that indicates whether or not the function was successful.
Note:
    The contents of the notification structure *including* all pointers is only valid during the callback routine to which it was passed.
    If the data is required after the duration of the callback then it must be physically copied by the callback routine.
*/
{
    ...
    if ((!IopCompareGuid(EventGuid, (LPGUID)&GUID_HWPROFILE_QUERY_CHANGE)) &&
     ...
     // Only the query changes are synchronous, and in that case we must know definitely whether we are nested within a Pnp event or not.
     ASSERT((!IopCompareGuid(EventGuid, (LPGUID)&GUID_HWPROFILE_QUERY_CHANGE))|| (NotificationTime != PROFILE_PERHAPS_IN_PNPEVENT)) ;
     if (!IopCompareGuid(EventGuid, (LPGUID)&GUID_HWPROFILE_QUERY_CHANGE) ) {
     ...
     // Query notifications are synchronous. Determine if we are currently within an event, in which case we must do the notify here instead of queueing it up.
     ...
     // query change notification because we are blocking inside a PnPEvent and thus can't queue/wait on another!
...
/*++
Routine Description:
    This routine is used to deliver the HWProfileNotifications.
    It is called from the worker thread only
...
*/
{
    ...
    while (link != (PLIST_ENTRY)&IopProfileNotifyList) {
        if (!pNotifyList->Unregistered) {
            ...
            // Reference the entry so that no one deletes during the callback and then release the lock
            ...
            // Win32K is a driver that can have multiple copies.
            // If Hydra is running, the Mem. manager will check if the callback exists in "per session" space.
            // If that is the case, it will attach to the console (hence the 3rd param of PULONG containing 0) session and deliver it.
            // If either Hydra is not running, or the callback is outside session space then the callback is called directly.
            ...
            // If the caller returned anything other than success and it was a query hardware profile change, we veto the query and send cancels to all callers that already got the query.
 ...
 Clean0:
    IopReleaseNotifyLock (&IopHwProfileNotifyLock);//UnLock the Profile Notification List
    return status;
}
...
/*++
Routine Description:
    This routine is used to notify all registered drivers of a change to a particular device.
    It does not return until all interested parties have been notified.
Parameters:
    EventGuid - The event that has occured
    DeviceObject - The device object that has changed.  The devnode for this device object contains a list of callback routines that have registered for notification of any changes on this device object.
Return Value:
    Status code that indicates whether or not the function was successful.
Note:
    The contents of the notification structure *including* all pointers is only valid during the callback routine to which it was passed.
    ...
    while (link != &deviceNode->TargetDeviceNotify) {
        ...
        if (!entry->Unregistered) {
            ...
            if (NotificationStructure) {
                ...
                // Dispatch this function via the memory manager.
                // Win32K is a driver that can have multiple copies.
                // If Hydra is running, the Mem.
                // manager will check if the callback exists in "per session" space.
                // If that is the case, it will attach to the console (hence the 3rd param of PULONG containing 0) session and deliver it.
                // If either Hydra is not running, or the callback is outside session space then the callback is called directly.
                status = MmDispatchWin32Callout ((PKWIN32_CALLOUT)callparams.Callout,&IopPnPHydraCallback,&callparams,&Console);
            } else {
            ...
            // If the caller returned anything other than success and it was a query remove, we veto the query remove and send cancels to all callers that already got the query remove.
            if (!NT_SUCCESS(status) && IopCompareGuid(EventGuid, (LPGUID)&GUID_TARGET_DEVICE_QUERY_REMOVE)) {
...
/*++
Routine Description:
    This routine is used to notify all registered drivers of a changes to a particular class of device.
    It does not return until all interested parties have been notified.
Parameters:
    EventTypeGuid - The event that has occured
    ClassGuid - The device class this change has occured in
    SymbolicLinkName - The kernel mode symbolic link name of the interface device that changed
Return Value:
    Status code that indicates whether or not the function was successful.
Note:
    The contents of the notification structure *including* all pointers is only valid during the callback routine to which it was passed.
...
*/
{
    ...
    while (link != &IopDeviceClassNotifyList[hash]) {
        ...
        if ( !entry->Unregistered && IopCompareGuid(&(entry->ClassGuid), ClassGuid) ) {
            ...
            // Callback the entity that registered and ignore the return value as we arn't interested in it
            callparams.Callout=(entry->Callback);
            callparams.NotificationStructure=&notification;
            callparams.Context=entry->Context;
    ...
    IopReleaseNotifyLock(&IopDeviceClassNotifyLock);// Release the lock
    return status;
}
...
/*++
Routine Description:
    IoRegisterPlugPlayNotification provides a mechanism by which WDM drivers may receive notification (via callback) for a variety of Plug&Play events.

Arguments:

    EventCategory - Specifies the event category being registered for.  WDM drivers may currently register for hard-ware profile changes, device class changes
        (instance arrivals and removals), and target device changes (query-removal, cancel-removal, removal-complete, as well as 3rd-party extensible events).

    EventCategoryFlags - Supplies flags that modify the behavior of event registration.
        There is a separate group of flags defined for each event category.  Presently, only the interface device change event category has any flags defined:

            DEVICE_CLASS_NOTIFY_FOR_EXISTING_DEVICES -- Drivers wishing to retrieve a complete list of all interface devices presently available, and keep
                the list up-to-date (i.e., receive notification of interface device arrivals and removals), may specify this flag.
                This will cause the PnP manager to immediately notify the driver about every currently-existing device of the specified interface class.

    EventCategoryData - Used to  'filter' events of the desired category based on the supplied criteria.
        Not all event categories will use this parameter.
        The event categories presently defined use this information as fol-lows:
    ...
    DriverObject - The caller must supply a reference to its driver object (obtained via ObReferenceObject), to prevent the driver from being unloaded while registered for notification.
        The PnP Manager will dereference the driver object when the driver unregisters for notification via IoUnregisterPlugPlayNotification).

    CallbackRoutine - Entry point within the driver that the PnP manager should call whenever an applicable PnP event occurs.
        The entry point must have the following prototype:

            typedef NTSTATUS (*PDRIVER_NOTIFICATION_CALLBACK_ROUTINE) (IN PVOID NotificationStructure,IN PVOID Context);

        where NotificationStructure contains information about the event.
        Each event GUID within an event category may potentially have its own notification structure format, but the buffer must al-ways begin with a PLUGPLAY_NOTIFICATION_HEADER,
        which indicates the size and ver-sion of the structure, as well as the GUID for the event.

        The Context parameter provides the callback with the same context data that the caller passed in during registration.

    Context - Points to the context data passed to the callback upon event notification.

    NotificationEntry - Upon success, receives a handle representing the notification registration.
        This handle may be used to unregister for notification via IoUnregisterPlugPlayNotification.
*/
{
    NTSTATUS status = STATUS_SUCCESS;
#if DBG
    ...
    *NotificationEntry = NULL;
    ...
    // Reference the driver object so it doesn't go away while we still have a pointer outstanding
    ...
    case EventCategoryReserved:
        {
            ...
            // Explicitly NULL out the returned entry as you can *NOT* unregister for setup notifications
            *NotificationEntry = NULL;
    ...
    case EventCategoryHardwareProfileChange:
        {
            ...
            ExAcquireFastMutex(&PiNotificationInProgressLock);
            if (PiNotificationInProgress) {
                // If a notification is in progress, mark the entry as Unregistered until after the current notification is complete.
                PDEFERRED_REGISTRATION_ENTRY deferredNode;
                ...
                // Consider this entry unregistered during the current notification
                entry->Unregistered = TRUE;

                // Reference the entry so that it doesn't go away until it has been removed from the deferred registration list
                IopReferenceNotify((PNOTIFY_ENTRY_HEADER)entry);
    ...
    case EventCategoryTargetDeviceChange:
        {
            ...
            // The PDO associated with the devnode we got back from IopGetRelatedTargetDevice has already been referenced by that routine.
    ...
    case EventCategoryDeviceInterfaceChange:
        {
            ...
            if (EventCategoryFlags & PNPNOTIFY_DEVICE_INTERFACE_INCLUDE_EXISTING_INTERFACES) {
                ...
                // Get the list of all the devices of this function class that are already in the system
...
/*++
Routine Description:
    IopGetRelatedTargetDevice retrieves the device object associated with the specified file object and then sends a query device relations irp to that device object.
    NOTE: The PDO associated with the returned device node has been referenced, and must be dereferenced when no longer needed.
Arguments:
    FileObject - Specifies the file object that is associated with the device object that will receive the query device relations irp.
...
*/
{
    ...
    // Query what the "actual" target device node should be for this file object.
    // Initialize the stack location to pass to IopSynchronousCall() and then send the IRP to the device object that's associated with the file handle.
...
/*++
Routine Description:
    IoGetRelatedTargetDevice retrieves the device object associated with the specified file object and then sends a query device relations irp to that device object.
    NOTE: The PDO associated with the returned device node has been referenced, and must be dereferenced when no longer needed.
Arguments:
    FileObject - Specifies the file object that is associated with the device object that will receive the query device relations irp.
...
/*++
Routine Description:
    This routine is used to notify setup (during text-mode setup) of arrivals of a particular device.
    It does not return until all interested parties have been notified.
Parameters:
    PhysicalDeviceObject - Supplies a pointer to the PDO of the newly arrived device.
    EnumEntryKey - Supplies a handle to the key associated with the devide under the Enum\ branch of the registry.  Can be NULL in which case the key will be opened here.
    InstallDriver - Indicates whether setup should attempt to install a driver for this object.
                    Device objects created through IoReportDetectedDevice() already have a driver but we want to indicate them to setup anyway.
Return Value:
    Status code that indicates whether or not the function was successful.
Note:
    The contents of the notification structure *including* all pointers is only valid during the callback routine to which it was passed.
...
/*++
Routine Description:
    This routine reports whether WDM functionality is available that is greater than or equal to the specified major and minor version.
Parameters:
    MajorVersion - Supplies the WDM major version that is required.
    MinorVersion - Supplies the WDM minor version that is required.
Return Value:
    If WDM support is available at _at least_ the requested level, the return value is TRUE, otherwise it is FALSE.
*/
{
    return ((MajorVersion < WDM_MAJORVERSION) || ((MajorVersion == WDM_MAJORVERSION) && (MinorVersion <= WDM_MINORVERSION)));
}
...
/*++
Routine Description:
    This function returns the appropriate DMA adapter object for the device defined in the device description structure.
    This code is a wrapper which queries the bus interface standard and then calls the returned get DMA adapter function.
    If an adapter object was not retrieved then a legecy function is attempted.
Arguments:
    PhysicalDeviceObject - Optionally, supplies the PDO for the device requesting the DMA adapter.  If not supplied, this routine performs the function of the non-PnP HalGetDmaAdapter routine.
    DeviceDescriptor - Supplies a description of the deivce.
    NumberOfMapRegisters - Returns the maximum number of map registers which may be allocated by the device driver.
Return Value:
    A pointer to the requested adapter object or NULL if an adapter could not be created.
*/
{
    KEVENT event;
    NTSTATUS status;
    ...
    if (PhysicalDeviceObject != NULL) {
        ASSERT_PDO(PhysicalDeviceObject);
        // First off, determine whether or not the caller has requested that we automatically fill in the proper InterfaceType value into the DEVICE_DESCRIPTION structure used in retrieving the DMA adapter object.
        // If so, then retrieve that interface type value into our own copy of the DEVICE_DESCRIPTION buffer.
        if ((DeviceDescription->InterfaceType == InterfaceTypeUndefined) || (DeviceDescription->InterfaceType == PNPBus)) {
            // Make a copy of the caller-supplied device description, so we can modify it to fill in the correct interface type.
            RtlCopyMemory(&privateDeviceDescription, DeviceDescription, sizeof(DEVICE_DESCRIPTION));
            status = IoGetDeviceProperty(PhysicalDeviceObject, DevicePropertyLegacyBusType, sizeof(privateDeviceDescription.InterfaceType), (PVOID)&(privateDeviceDescription.InterfaceType), &resultLength);
            if (!NT_SUCCESS(status)) {
                ASSERT(status == STATUS_OBJECT_NAME_NOT_FOUND);
                // Since the enumerator didn't tell us what interface type to use for this PDO, we'll fall back to our default.
                // This is ISA for machines where the legacy bus is ISA or EISA, and it is MCA for machines whose legacy bus is MicroChannel.
        ...
        // Initialize the status to error in case the ACPI driver decides not to set it correctly.
        ...
        if (NT_SUCCESS( status)) {
            if (busInterface.GetDmaAdapter != NULL) {
                dmaAdapter = busInterface.GetDmaAdapter( busInterface.Context,deviceDescriptionToUse,NumberOfMapRegisters );
            }
        ...
        // The caller didn't specify the PDO, so we'll just use the device description exactly as they specified it (i.e., we can't attempt to make our own determination of what interface type to use).
        deviceDescriptionToUse = DeviceDescription;
    }
...
*/
{
    ...
    status = IopCreateRegistryKeyEx( ParamKeyHandle,deviceKeyHandle,&deviceParamString,DesiredAccess | READ_CONTROL | WRITE_DAC,REG_OPTION_NON_VOLATILE,&disposition);
    ...
    if (disposition == REG_CREATED_NEW_KEY) {
        ...
        // Get the security descriptor from the key so we can add the administrator.
        ...
        if (daclPresent) {
            for (aceIndex = 0; aceIndex < aclSizeInfo.AceCount; aceIndex++) {
                ...
                // We need to skip copying any ACEs which refer to the Administrator to ensure that our full control ACE is the one and only.
                if ((ace->Header.AceType != ACCESS_ALLOWED_ACE_TYPE && ace->Header.AceType != ACCESS_DENIED_ACE_TYPE) || !RtlEqualSid((PSID)&ace->SidStart, SeAliasAdminsSid)) {
        ...
        status = RtlAddAccessAllowedAceEx( newDacl,ACL_REVISION,CONTAINER_INHERIT_ACE,KEY_ALL_ACCESS,SeAliasAdminsSid);
        if (!NT_SUCCESS(status)) {
...
#if 0
NTSTATUS IopSetupDeviceObjectFromDeviceClass(IN PDEVICE_OBJECT PhysicalDeviceObject,IN HANDLE DeviceClassKey)
/*++
Routine Description:
    This routine will migrate device object settings from the device class key in the registry to the physical device object.
    The possible settings are:
        ...
        * DefaultAcl - the default ACL for the device object.  This can be overridden by setting a device specific ACL in the device node in the registry.
    The routine will then use the DeviceType and DeviceCharacteristics specified to determine whether a VPB should be allocated as well as to set default security if none is specified in the registry.
Arguments:
    PhysicalDeviceObject - the PDO we are to configure
    DeviceClassKey - a handle to the this particular device class's information in the Control\DeviceClasses section of the registry.
Return Value:
    status
*/
{
    ...
    // First read from the registry to find the device type and characteristics values.
    RtlInitUnicodeString(&valueName, REGSTR_VAL_DEVICE_TYPE);
    status = IopGetRegistryValue(DeviceClassKey, valueName.Buffer, &info);
    if(NT_SUCCESS(status)) {
    ...
    // Assign the device type to the PDO and OR in the characteristics bits with the ones already set.

    // Make sure no one registered for two incompatible device classes.
    ASSERT((PhysicalDeviceObject->DeviceType == FILE_DEVICE_PNP_WITH_INTERFACE) || (PhysicalDeviceObject->DeviceType == deviceType));
    ...
    if(deviceTypeChanged) {
        status = IopSetSecurityObjectFromRegistry(PhysicalDeviceObject, DeviceClassKey);
        if(!NT_SUCCESS(status)) {
            BOOLEAN hasName;
            ...
            // Security couldn't be set from the registry.  Build the default security descriptor for this object and we'll set that instead.
            hasName = ((PhysicalDeviceObject->Flags & DO_DEVICE_HAS_NAME) == DO_DEVICE_HAS_NAME);
            securityDescriptor = IopCreateDefaultDeviceSecurityDescriptor(deviceType,PhysicalDeviceObject->Characteristics,hasName,buffer,&securityInformation);
        ...
        // If the device is a mass storage type device then we should mark it for VPB allocation later on down the line.
        if((deviceType == FILE_DEVICE_DISK) || (deviceType == FILE_DEVICE_CD_ROM) || (deviceType == FILE_DEVICE_TAPE) || (deviceType == FILE_DEVICE_VIRTUAL_DISK)) {
            IopCreateVpb(PhysicalDeviceObject);
            KeInitializeEvent( &PhysicalDeviceObject->DeviceLock, SynchronizationEvent, TRUE );
...
/*++
Routine Description:
    This routine will read in the security information stored in the specified registry key and assign that security to the specified object.
    If the registry key contains a SecurityDescriptor value it will be validated and assigned to the object if valid.
Arguments:
...
*/
{
    ...
    status = IopGetRegistryValue(Key,valueName.Buffer,&info);
    if(!NT_SUCCESS(status)) {
        return status;
    }

    descriptor = (((PUCHAR) info) + info->DataOffset);
    status = SeCaptureSecurityDescriptor(descriptor,UserMode,PagedPool,FALSE,&capturedDescriptor);
    ...
    try {
        ...
        status = RtlGetSaclSecurityDescriptor(capturedDescriptor,&present,&acl,&tmp);
        if(NT_SUCCESS(status) && (present)) {
            securityInformation |= SACL_SECURITY_INFORMATION;
        }

        status = RtlGetDaclSecurityDescriptor(capturedDescriptor,&present,&acl,&tmp);
        if(NT_SUCCESS(status) && (present)) {
        ...
        SeReleaseSecurityDescriptor(capturedDescriptor,UserMode,FALSE);
    }
...
NTSTATUS PpCreateLegacyDeviceIds(IN PDEVICE_OBJECT DeviceObject,IN PUNICODE_STRING DriverName,IN PCM_RESOURCE_LIST Resources)
{
    ...
    // The compatible ID generated will be DETECTED<InterfaceName>\<Driver Name>
    ...
    // Adjust the buffer to point to the end and generate the second compatible id string.
...
NTSTATUS IoNotifyPowerOperationVetoed(
    ...
    )
{
    ...
    // We have to types of power events, system wide (standby) and device targetted (warm eject). Rather than have two different veto mechanisms,
    // we just retarget the operation against the root device if none is specified (hey, someone's gotta represent the system, right?).
    ...
    return PpSetPowerVetoEvent(VetoedPowerOperation,NULL,NULL,deviceObject,PNP_VetoDevice,&vetoingDeviceNode->InstancePath);
}
...
#define MAX_VETO_NAME_LENGTH    512 //From Revent.c make it common
    ...
    if (Synchronous) {
        ...
        status = PpSetPowerEvent(PwrNotification,&completionEvent,&completionStatus,&vetoType,&vetoName);
        if (NT_SUCCESS(status))  {
            KeWaitForSingleObject( &completionEvent, Executive, KernelMode, FALSE, NULL );
            status = completionStatus;
        ...
        status = PpSetPowerEvent(PwrNotification,NULL,NULL,NULL,NULL);
    }
...
// Release the references to the device object for all the notifications entries of a device object.
// Then fixup the notification node to not point to a physical device object.
// The notification node will be released when IoUnregisterPlugPlayNotification is actually called, but the device object will already be gone.
{
    PTARGET_DEVICE_NOTIFY_ENTRY entry;
    PFAST_MUTEX lock;
...
NTSTATUS IopPnPHydraCallback (PVOID CallbackParams)
// This is the dispatch routine for plug and play notifications on the "far side" of the memory manager.
// If this notification is destined for a routine in "session space" the mmgr will have attached us to the session.
...
/*++
Routine Description:
    This routine appends a veto (driver name or device instance path) to the veto list.
Parameters:
...
*/
{
    ...
    // Compute the length of the (new) veto list.
    // This is the length of the old veto list + the size of the new veto + the size of the terminating '\0'.
    Length = Context->VetoListLength + VetoName->Length + sizeof (WCHAR);

    // Allocate the new veto list.
    Buffer = ExAllocatePool(NonPagedPool, Length);

    // If we succeeded in allocating the new veto list, copy the old veto list to the new list, append the new veto, and finally, append a terminating '\0'.
    // Otherwise, update the status to indicate an error; IopGetLegacyVetoList will free the veto list before it returns.
    if (Buffer != NULL) {
        if (*Context->VetoList != NULL) {
            RtlMoveMemory(Buffer,*Context->VetoList,Context->VetoListLength);
            ExFreePool(*Context->VetoList);
        }

        RtlMoveMemory(&Buffer[Context->VetoListLength / sizeof (WCHAR)],VetoName->Buffer,VetoName->Length);
...
/*++
Routine Description:
    This routine determines whether the specified device node should be added to the veto list, and if so, calls IopAppendLegacyVeto to add it.
Parameters:
    DeviceNode - The device node to be added.
    Context - An IO_GET_LEGACY_VETO_LIST_CONTEXT pointer.
ReturnValue:
    A BOOLEAN value which indicates whether the device node enumeration process should be terminated or not.
*/
{
    PDEVICE_CAPABILITIES DeviceCapabilities;

    // A device node should be added added to the veto list, if it has the NonDynamic capability.
    DeviceCapabilities = IopDeviceNodeFlagsToCapabilities(DeviceNode);
    if (DeviceCapabilities->NonDynamic) {
        // Update the veto type.  If an error occurrs while adding the device node to the veto list, or the caller did not provide a veto list pointer, terminate the enumeration process now.
...
/*++
Routine Description:
    This routine recusively walks the device tree, invoking IopGetLegacyVetoListDevice to add device nodes to the veto list (as appropriate).
Parameters:
    DeviceNode - The device node.
    Context - An IO_GET_LEGACY_VETO_LIST_CONTEXT pointer.
ReturnValue:
    A BOOLEAN value which indicates whether the device tree enumeration process should be terminated or not.
*/
{
    PDEVICE_NODE Child;

    // Determine whether the device node should be added to the veto list and add it.
    // If an operation is unsuccessful or we determine the veto type but the caller doesn't need the veto list, then we terminate our search now.
    if (!IopGetLegacyVetoListDevice(DeviceNode, Context)) {
        return FALSE;
    }

    // Call ourselves recursively to enumerate our children.
    // If while enumerating our children we determine we can terminate the search prematurely, do so.
...
VOID IopGetLegacyVetoListDrivers(IN PIO_GET_LEGACY_VETO_LIST_CONTEXT Context)
{
    ...
    for (;;) {
        // Get info on next object in directory.
        // If the buffer is too small, reallocate it and try again.
        // Otherwise, any failure including STATUS_NO_MORE_ENTRIES breaks us out.
        ...
        // Have name of object.
        // Create object path and use ObReferenceObjectByName() to get DriverObject.
        // This may fail non-fatally if DriverObject has gone away in the interim.
        ...
        driverString.Buffer = ExAllocatePool(PagedPool, driverString.MaximumLength);
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (driverObject->Flags & DRVO_LEGACY_RESOURCES) {
                // Update the veto type.
                // If the caller provided a veto list pointer, add the driver to the veto list.
                // If an error occurs while adding the driver to the veto list, or the caller did not provide a veto list pointer, terminate the driver enumeration now.

                // NOTE: Driver may be loaded but not running, distinction is not made here.
            ...
            // Early out if we have a veto and the caller didn't want a list or we hit some error already
            if (((*Context->VetoType == PNP_VetoLegacyDriver) && (Context->VetoList == NULL)) || !NT_SUCCESS(*Context->Status)) {
                break;
            }
        }
    }
...
/*++
Routine Description:
    This routine is used by PNP and PO to determine whether legacy drivers and devices are installed in the system.
    This routine is conceptually a QUERY_REMOVE_DEVICE and QUERY_POWER-like interface for legacy drivers and devices.
Parameters:
    VetoList - A pointer to a PWSTR. (Optional)  If specified, IoGetLegacyVetoList will allocate a veto list, and return a pointer to the veto list in VetoList.
    VetoType - A pointer to a PNP_VETO_TYPE.  If no legacy drivers or devices are found in the system, VetoType is assigned PNP_VetoTypeUnknown.
        If one or more legacy drivers are installed, VetoType is assigned PNP_VetoLegacyDriver.
        If one or more legacy devices are installed, VetoType is assigned PNP_VetoLegacyDevice.
        VetoType is assigned independent of whether a VetoList is created.
ReturnValue:
    An NTSTATUS value indicating whether the IoGetLegacyVetoList() operation was successful.
*/
{
    NTSTATUS Status;
    ...
    *VetoType = PNP_VetoTypeUnknown;
    ...
    if (PnPInitialized == FALSE) {
        return Status;// Can't touch anything, but nothing is really started either.
    }
    ...
    // This process can: (1) modify the veto list, (2) modify the veto type and/or (3) modify the status.
    IopGetLegacyVetoListDrivers(&Context);

    // If the driver enumeration process was successful and no legacy drivers were detected, enumerate all device nodes.
    // The same context values as above may be modified during device enumeration.
    if (NT_SUCCESS(Status)) {
    ...
    // If the previous operation(s) was/were successful, and the caller provided a veto list pointer and we have constructed a veto list, terminate the veto list with an empty string, i.e. MULTI_SZ.
    if (NT_SUCCESS(Status)) {
        if (*VetoType != PNP_VetoTypeUnknown) {
            if (VetoList != NULL) {
    ...
    // If a previous operation was unsuccessful, free any veto list we may have allocated along the way.
    if (!NT_SUCCESS(Status)) {
...
*/
{
    ...
    for (attachedDevice = DeviceNode->PhysicalDeviceObject; attachedDevice; attachedDevice = attachedDevice->AttachedDevice) {
...
Parameters:
    SymbolicLinkName - Supplies a pointer to the symbolic link name which was returned by IoRegisterDeviceInterface when the interface was registered, or as returned by IoGetDeviceInterfaces.
    Enable - If TRUE (non-zero), the interface will be enabled.  If FALSE, it will be disabled.
    DeferNotStarted - If TRUE then enables will be queued if the PDO isn't started.  It is FALSE when we've started the PDO and are processing the queued enables.
Return Value:
    Status code that indicates whether or not the function was successful.
*/
{
    ...
    status = IopDeviceInterfaceKeysFromSymbolicLink(SymbolicLinkName, KEY_READ | KEY_WRITE, &hInterfaceClassKey, &hInterfaceParentKey, &hInterfaceInstanceKey);
    if (!NT_SUCCESS(status)) {
        goto clean1;
    }
    ...
    PiWstrToUnicodeString(&tempString, REGSTR_KEY_CONTROL);
    status = IopCreateRegistryKeyEx( &hInterfaceParentControl,hInterfaceParentKey,&tempString,KEY_READ,REG_OPTION_VOLATILE,NULL);
    if (!NT_SUCCESS(status)) {
        goto clean1;
    }
    ...
    if(NT_SUCCESS(status)) {
        ...
        PiWstrToUnicodeString(&tempString, REGSTR_KEY_CONTROL);
        status = IopCreateRegistryKeyEx( &hInterfaceInstanceControl,hInterfaceInstanceKey,&tempString,KEY_READ,REG_OPTION_VOLATILE,NULL);
        if(!NT_SUCCESS(status)) {
    ...
    if (pKeyValueInfo->Type == REG_SZ) {
        ...
        if (physicalDeviceObject) {
            // DeferNotStarted is set TRUE if we are being called from IoSetDeviceInterfaceState.
            // It will be set FALSE if we are processing previously queued operations as we are starting the device.
            if (DeferNotStarted) {
                if (physicalDeviceObject->DeviceObjectExtension->ExtensionFlags & DOE_START_PENDING) {
                    ...
                    // The device hasn't been started yet.  We need to queue any enables and remove items from the queue on a disable.
                    deviceNode = (PDEVICE_NODE)physicalDeviceObject->DeviceObjectExtension->DeviceNode;
                    if (Enable) {
                        ...
                        if (pendingSetState != NULL) {
                            ...
                            if (pendingSetState->LinkName.Buffer != NULL) {
                                // Capture the callers info and queue it to the devnode.  Once the device stack is started we will dequeue and process it.
                                pendingSetState->LinkName.MaximumLength = SymbolicLinkName->Length;
                                pendingSetState->LinkName.Length = SymbolicLinkName->Length;
                                RtlCopyMemory( pendingSetState->LinkName.Buffer, SymbolicLinkName->Buffer, SymbolicLinkName->Length);
                                ...
                                // Couldn't allocate a buffer to hold the symbolic link name.
                                ExFreePool(pendingSetState);
                                status = STATUS_INSUFFICIENT_RESOURCES;
                            }
                        } else {
                            // Couldn't allocate the PENDING_SET_INTERFACE_STATE structure.
                            status = STATUS_INSUFFICIENT_RESOURCES;
                        }
                    } else {
                        PLIST_ENTRY entry;
                        // We are disabling an interface.  Since we aren't started yet we should have queued the enable.
                        // Now we go back and find the matching enable and remove it from the queue.
                        for (entry = deviceNode->PendedSetInterfaceState.Flink; entry != &deviceNode->PendedSetInterfaceState; entry = entry->Flink)  {
                            ...
                            if (RtlEqualUnicodeString( &pendingSetState->LinkName, SymbolicLinkName, TRUE)) {
                                // We found it, remove it from the list and free it.
...
#if 0
                        // Debug code to catch the case where we couldn't find the entry to remove.
                        // This could happen if we messed up adding the entry to the list or the driver disabled an interface without first enabling it.
                        // Either way it probably merits some investigation.
                        if (entry == &deviceNode->PendedSetInterfaceState) {
                            PIDBGMSG(PIDBG_ERROR, ("IopProcessSetInterfaceState: Disable couldn't find deferred enable, DeviceNode = 0x%p, SymbolicLink = \"%Z\"\n", deviceNode, SymbolicLinkName));
                        }
    ...
    if (!Enable) {
        // In the case of Disable we want to continue even if there was an error finding the PDO.
        // Prior to adding support for deferring the IoSetDeviceInterfaceState calls, we never looked up the PDO for disables.
        // This will make sure that we continue to behave the same as we used to in the case where we can't find the PDO.
        status = STATUS_SUCCESS;
    }
    ...
    if (Enable) {
        ...
        // (Start out with a reasonably-sized buffer so we hopefully only have to retrieve this once.
        ...
        while (TRUE) {
            ...
            status = IoGetDeviceProperty( physicalDeviceObject,DevicePropertyPhysicalDeviceObjectName,deviceNameBufferLength,deviceNameBuffer,&deviceNameBufferLength);
            if (NT_SUCCESS(status)) {
                break;
            } else {
    ...
    if (status == STATUS_OBJECT_NAME_NOT_FOUND) {
        ...
        if(pKeyValueInfo->Type == REG_DWORD && pKeyValueInfo->DataLength == sizeof(ULONG)) {
            ...
            // The registry is screwed up - assume linked is 0 and the registry will be fixed when we update linked in a few moments
            linked = 0;
        }
    }
    ...
    if (status == STATUS_OBJECT_NAME_NOT_FOUND) {
        ...
        if(pKeyValueInfo->Type == REG_DWORD && pKeyValueInfo->DataLength == sizeof(ULONG)) {
            ...
            // The registry is screwed up - assume refcount is 0 and the registry will be fixed when we update refcount in a few moments
            refcount = 0;
        }
    ...
    if (Enable) {
        if (!linked) {
            ...
            if (refcount > 0) {
                ...
                // According to the reference count, no other device instances currently reference this interface, and therefore no symbolic links should exist, so we should create one.
                refcount = 1;
...
#if 0
            IopSetupDeviceObjectFromDeviceClass(physicalDeviceObject, hInterfaceClassKey);
#endif
        } else {
...
clean0:
    if (!NT_SUCCESS(status) && !Enable) {
        // If we failed to disable an interface (most likely because the interface keys have already been deleted) report success.
        status = STATUS_SUCCESS;
    }
