...
Abstract:
    This module contains the plug-and-play subroutines for the I/O system.
...
Routine Description:
    ...
    key and all the required default value entries. 
    Also a value entry under Service\ServiceKeyName\Enum is created to point to the newly created madeup
...
Parameters:
    ...
    ServiceKeyName - Supplies a pointer to the name of the subkey in the
        ...
        that caused the driver to load. This is the RegistryPath parameter to the DriverEntry routine.

    ReturnedHandle - Supplies a variable to receive the handle of the newly created key.
    KeyName - Supplies a variable to receive the name of the newly created key.
    InstanceNumber - supplies a variable to receive the InstanceNumber value entry created under service\name\enum subkey.
    ResourceOwned - supplies a BOOLEAN variable to indicate if caller owns the registry resource exclusively.
Return Value:
...
--*/
{
    ...
    status = IopOpenRegistryKeyPersist(&handle, enumRootHandle, &unicodeKeyName, KEY_ALL_ACCESS, TRUE, NULL);
    ...
    *InstanceNumber = instance;
    ...
    status = IopOpenRegistryKeyPersist(ReturnedHandle, handle, &unicodeInstanceName, KEY_ALL_ACCESS, TRUE, &disposition);
    ZwClose(handle);
    if (!NT_SUCCESS(status)) {
        RtlFreeUnicodeString(&unicodeKeyName);
        goto local_exit0;
    ...
    *(PWSTR)unicodeBuffer = OBJ_NAME_PATH_SEPARATOR;
    ...
    if (disposition == REG_CREATED_NEW_KEY) {
        ...
        status = IopOpenRegistryKey(&handle, *ReturnedHandle, &unicodeValueName, KEY_ALL_ACCESS, TRUE);
        if (NT_SUCCESS(status)) {
        ...
        ZwSetValueKey(handle, &unicodeValueName, TITLE_INDEX_VALUE, REG_SZ, REGSTR_VALUE_LEGACY_DRIVER, sizeof(REGSTR_VALUE_LEGACY_DRIVER));
        ...
        // name is used.
        status = IopOpenServiceEnumKeys(ServiceKeyName, KEY_READ, &handle, NULL, FALSE);
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                if (keyValueInformation->Type == REG_SZ) {
                    if (keyValueInformation->DataLength > sizeof(UNICODE_NULL)) {
                        IopRegistryDataToUnicodeString(&unicodeString, (PWSTR)KEY_VALUE_DATA(keyValueInformation), keyValueInformation->DataLength);
                    }
                }
            }
    ...
    status = PpDeviceRegistration(KeyName, TRUE, NULL, NULL);
    if (ResourceOwned) {
        KeEnterCriticalRegion();
        ExAcquireResourceShared(&PpRegistryDeviceResource, TRUE);
    }
...
/*++
Routine Description:
    This routine remove a string from a value entry specified by ValueName under an already opened registry handle.  
    Note, this routine will not delete the ValueName entry even it becomes empty after the removal.
Parameters:
    Handle - Supplies the handle to a registry key whose value entry will be modified.
...
--*/
{
    ...
    if (!NT_SUCCESS( status )) {
        return status;
    } else if ((keyValueInformation->Type != REG_MULTI_SZ) || (keyValueInformation->DataLength == 0)) {
        status = (keyValueInformation->Type == REG_MULTI_SZ) ? STATUS_SUCCESS : STATUS_INVALID_PARAMETER;
        ExFreePool(keyValueInformation);
        return status;
    }

    // Scan through the multi_sz string to find the matching string and remove it.
...
Routine Description:
    ...
    and Create is TRUE, a new value entry will be created using the name ValueName.
Parameters:
    Handle - Supplies the handle to a registry key whose value entry will be modified.
    ...
    Create - Supplies a BOOLEAN variable to indicate if the ValueName value entry should be created if it is not present.
Return Value:
    Status code that indicates whether or not the function was successful.
--*/
{
    ...
    if (keyValueInformation) {
        // Note we  need to remove a UNICODE_NULL because the MULTI_SZ has two terminating UNICODE_NULL.
        RtlMoveMemory(p, KEY_VALUE_DATA(keyValueInformation), keyValueInformation->DataLength - sizeof(WCHAR));
    ...
    RtlMoveMemory(p, String->Buffer, String->Length);
    ...
    *p = UNICODE_NULL;
    ...
    status = ZwSetValueKey(Handle, &unicodeValueName, TITLE_INDEX_VALUE, REG_MULTI_SZ, destinationString, size);
...
/*++
Routine Description:
    This routine returns a buffer containing the concatenation of the two specified strings.
    Since String2 is optional, this function may also be used to make a copy of a unicode string.  
    Paged pool space is allocated for the destination string.  
    Caller must release the space once done with it.
Parameters:
    Destination - Supplies a variable to receive the handle of the newly created key.
...
--*/
{
    ...
    if(ARGUMENT_PRESENT(String2)) {
        RtlMoveMemory((PUCHAR)Destination->Buffer + String1->Length, String2->Buffer, String2->Length);
    }
...
/*++
Routine Description:
    This routine first checks if the driver is loadable. 
    If its a PnP driver, it will always be loaded (we trust it to do the right
...
--*/
{
    ...
    if ((Header) && (Header->OptionalHeader.DllCharacteristics & IMAGE_DLLCHARACTERISTICS_WDM_DRIVER)) {
...
/*++
Routine Description:
    This routine reads the Group value of the service key, finds its position in the ServiceOrderList.
    If ServiceHandle is NULL or unrecognized group value, it returns a value with max group order + 1.
    if any.
Parameters:
...
--*/
{
    ...
    status = IopOpenRegistryKey(&handle, NULL, &group, KEY_READ, FALSE);
    if (!NT_SUCCESS( status )) {
...
/*++
Routine Description:
    This routine checks if the Legacy= value of the driver's legacy_xxx key is one.  
    If yes, it deletes the Legacy key.
Parameters:
...
--*/
{
    ...
    status = IopOpenRegistryKey(&handle1, enumHandle, &deviceName, KEY_ALL_ACCESS, FALSE);
    if (NT_SUCCESS(status)) {
        ...
        status = IopOpenRegistryKey(&handle, handle1, &instanceName, KEY_ALL_ACCESS, FALSE);
        if (NT_SUCCESS(status)) {
            legacy = 1;
            ...
            if (legacy != 0) {
                ...
                PiWstrToUnicodeString(&unicodeName, REGSTR_KEY_CONTROL);
                status = IopOpenRegistryKey(&handlex, handle, &unicodeName, KEY_ALL_ACCESS, FALSE);
                if (NT_SUCCESS(status)) {
                    ZwDeleteKey(handlex);
                }
                PiWstrToUnicodeString(&unicodeName, REGSTR_KEY_LOG_CONF);
...
Arguments:
    DeviceObject - supplies a pointer to a device object whose registry values are to be updated.
Return Value:
    status
--*/
{
    ...
    status = ZwSetValueKey(handle, &unicodeName, TITLE_INDEX_VALUE, REG_DWORD, &tmpValue, sizeof(tmpValue));
    ZwClose(handle);
    return status;
}
...
NTSTATUS IopRestartDeviceNode(IN PDEVICE_NODE DeviceNode)
{
    ...
    ASSERT(!(IopDoesDevNodeHaveProblem(DeviceNode) ||
            (DeviceNode->Flags & (DNF_STARTED | DNF_ADDED | DNF_RESOURCE_ASSIGNED | DNF_RESOURCE_REPORTED)) ||
    ...
    ASSERT(!(DeviceNode->Flags &
           ~(DNF_MADEUP | DNF_ENUMERATED | DNF_HAS_BOOT_CONFIG | DNF_BOOT_CONFIG_RESERVED | DNF_NO_RESOURCE_REQUIRED)));
