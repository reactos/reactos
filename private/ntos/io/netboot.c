...
NTSTATUS IopInitCsc(IN PUCHAR CscPath);
NTSTATUS IopResetCsc(IN PUCHAR CscPath);
PWSTR IopBreakPath(IN PWSTR* Path);
NTSTATUS IopMarkRoot(IN PUNICODE_STRING FileName, OUT PHSHADOW RootHandle);
VOID IopSetFlushCSC(IN CSC_CONTROL Command);
...
VOID IopEnableRemoteBootSecurity (IN PLOADER_PARAMETER_BLOCK LoaderBlock);
VOID IopGetHarddiskInfo(OUT PWSTR NetHDCSCPartition);
...
NTSTATUS IopAddRemoteBootValuesToRegistry (PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    if (LoaderBlock->SetupLoaderBlock->ComputerName[0] != 0) {
        ...
        status = RtlDnsHostNameToComputerName(&netbiosNameString, &dnsNameString, TRUE);            // allocate netbiosNameString
        ...
        status = NtSetValueKey(handle, &string, 0, REG_SZ, netbiosNameString.Buffer, netbiosNameString.Length + sizeof(WCHAR));
        NtClose( handle );
        RtlFreeUnicodeString( &netbiosNameString );
        if ( !NT_SUCCESS(status) ) {
    ...
    if ( (p != NULL) && (*(p+1) == 0) ) {
        // NtBootPathName ends with a backslash, so we need to back up to the previous backslash.
        q = p;
        *q = 0;
        p = strrchr( LoaderBlock->NtBootPathName, '\\' );   // find last separator
        *q = '\\';
...
#if defined(REMOTE_BOOT)
    // Store the server path in the shared user data area.
    // Note that we need to add an extra \ at the beginning of this path to make it a UNC name.
...
skiproot:
    ...
    if ( (StartCsc != FLUSH_CSC) && ((LoaderBlock->SetupLoaderBlock->Flags & SETUPBLK_FLAGS_REPIN) != 0) ) {
        StartCsc = FLUSH_CSC;
...
#define ONE_BOOT 2
        ...
        if ( (LoaderBlock->SetupLoaderBlock->Flags & SETUPBLK_FLAGS_DISABLE_CSC) == 0 ) {
            ...
            disabled = ONE_BOOT;
            status = NtSetValueKey(handle, &string, 0, REG_DWORD, &disabled, sizeof(DWORD));
        } else {
            keyValue = (PKEY_VALUE_PARTIAL_INFORMATION)buffer;
            status = NtQueryValueKey(handle, &string, KeyValuePartialInformation, keyValue, sizeof(buffer), &length);
            if (NT_SUCCESS(status)) {
    ...
    // Add registry values for the IP address and subnet mask received from DHCP.
    // These are stored under the Tcpip service key and are read by both Tcpip and Netbt.
    // The adapter name used is the known GUID for the NetbootCard.
    ...
    status = IopWriteIpAddressToRegistry(handle, L"DhcpIPAddress", (PUCHAR)&(LoaderBlock->SetupLoaderBlock->IpAddress));
    if ( !NT_SUCCESS(status)) {
        NtClose(handle);
    ...
    status = IopWriteIpAddressToRegistry(handle, L"DhcpSubnetMask", (PUCHAR)&(LoaderBlock->SetupLoaderBlock->SubnetMask));
    if ( !NT_SUCCESS(status)) {
        NtClose(handle);
    ...
    status = IopWriteIpAddressToRegistry(handle, L"DhcpDefaultGateway", (PUCHAR)&(LoaderBlock->SetupLoaderBlock->DefaultRouter));
    NtClose(handle);
    if ( !NT_SUCCESS(status)) {
    ...
    // Create the service key for the netboot card. We need to have the Type value there or the card won't be initialized.
    status = IopOpenRegistryKey(&handle, NULL, &CmRegistryMachineSystemCurrentControlSetServices, KEY_ALL_ACCESS, FALSE);
    if (!NT_SUCCESS(status)) {
    ...
    status = ZwSetValueKey(serviceHandle, &string, TITLE_INDEX_VALUE, REG_SZ, imagePath, (wcslen(imagePath) + 1) * sizeof(WCHAR));
    if (!NT_SUCCESS(status)) {
...
NTSTATUS IopStartNetworkForRemoteBoot (PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    // Allocate a temporary buffer. It has to be big enough for all the various FSCTLs we send down.
...
#if defined(REMOTE_BOOT)
    NetHDCSCPartition = ExAllocatePoolWithTag(NonPagedPool, (80 * sizeof(WCHAR)) + bufferLength, 'bRoI');
    ...
    // If the setup loader block has a disk secret in it provided by the loader, 
    // pass this down to the redirector (do this before sending the LMR_START, since that uses this information).
#if defined(REMOTE_BOOT)
...
#endif // defined(REMOTE_BOOT)
    {
        ...
        status = NtFsControlFile(RdrHandle, NULL, NULL, NULL, &ioStatusBlock, FSCTL_LMMR_RI_INITIALIZE_SECRET, buffer, sizeof(LMMR_RI_INITIALIZE_SECRET), NULL, 0);
        if ( NT_SUCCESS(status) ) {
    ...
    // Read the computer name and domain name from the registry so we can give them to the datagram receiver.
    // During textmode setup the domain name will not be there, so we won't start the datagram receiver, which is fine.
    ...
    RtlCopyMemory(((PUCHAR)rrp->Parameters.Start.RedirectorName) + computerName.Length, domainName.Buffer, domainName.Length);
    ...
    status = NtFsControlFile(
                ...
                rrp,
                sizeof(LMR_REQUEST_PACKET) + rrp->Parameters.Start.RedirectorNameLength + rrp->Parameters.Start.DomainNameLength,
                &wkstaConfig,
                sizeof(wkstaConfig));
    if ( NT_SUCCESS(status) ) {
        status = ioStatusBlock.Status;
    ...
    if (startDatagramReceiver) {
        ...
        status = NtDeviceIoControlFile(dgHandle, NULL, NULL, NULL, &ioStatusBlock, IOCTL_LMDR_START, drrp, sizeof(LMDR_REQUEST_PACKET), NULL, 0);
        if ( NT_SUCCESS(status) ) {
            status = ioStatusBlock.Status;
        }
        ...
        // Note: In the current redirector implementation, this call just tells the redirector to register for TDI PnP notifications.
        // Starting the datagram receiver also does this, so we only issue this FSCTL if we're not starting the datagram receiver.
        status = NtFsControlFile(RdrHandle, NULL, NULL, NULL, &ioStatusBlock, FSCTL_LMR_BIND_TO_TRANSPORT | 0x80000000, NULL, 0, NULL, 0);
        if ( NT_SUCCESS(status) ) {
            status = ioStatusBlock.Status;
        }
    ...
    ((LoaderBlock->SetupLoaderBlock->Flags & SETUPBLK_FLAGS_FORMAT_NEEDED) == 0)) {
        ...
        // Tell the redirector to initialize remote boot redirection (back to the local disk).
        // Note that we only do this if we're NOT doing textmode setup.
        // During textmode, we let Setup do this so that it can do so AFTER it has reformatted the local disk.
        status = NtFsControlFile(RdrHandle,
        NULL,
        NULL,
        NULL, 
        &ioStatusBlock, 
...
#endif // defined(REMOTE_BOOT)
    {
        // Loop until the redirector is bound to the transport.
        // It may take a while because TDI defers notification of binding to a worker thread.
        // We start with a half a second wait and double it each time, trying five times total.
    ...
    ((LoaderBlock->SetupLoaderBlock->Flags & SETUPBLK_FLAGS_FORMAT_NEEDED) == 0)
        ...
        //  If we are connected and either we are part way through a reset of the csc or the init failed then reset the csc.
        if ( NT_SUCCESS(status) ) {
            ...
            (LoaderBlock->SetupLoaderBlock->NetbootCardDriverName[0] != L'\0')) {
                // if we are connected and we're not repinning all files and we have a new net card driver to pin, 
                // then pin it below after we've created called IopAssignNetworkDriveLetter
                pinNetDriver = TRUE;
            }
        }
...
#if defined(REMOTE_BOOT)
    if (pinNetDriver) {
        //  Pin the new net card driver simply by opening it.
        //  if it fails, it's not fatal, as we'll eventually pin it since the directory it's in is marked as system/inherit.
        HANDLE driverHandle = NULL;
        PWCHAR fullDriverName;

        fullDriverName = (PWCHAR) ExAllocatePoolWithTag(NonPagedPool, 
        sizeof( L"\\SystemRoot\\System32\\Drivers\\" ) + sizeof( LoaderBlock->SetupLoaderBlock->NetbootCardDriverName ),
        'bRoI');
...
VOID IopShutdownCsc()
{
    ...
    if ( RdrHandle != NULL ) {
        ...
        status = NtDeviceIoControlFile(RdrHandle,NULL,NULL,NULL,&ioStatusBlock,IOCTL_SWITCHES,&shadowInfo,0,NULL,0);
#if DBG
        if ( NT_SUCCESS(status) ) {
            status = ioStatusBlock.Status;
        }
...
VOID IopAssignNetworkDriveLetter (PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    // Create the symbolic link of X: to the redirector.
    // We do this after the redirector has loaded, 
    // but before AssignDriveLetters is called the first time in textmode setup (once that has happened, the drive letters will stick).

    // Note that we use X: for the textmode setup phase of a remote installation. But for a true remote boot, we use C:.
    ...
    // If this is a remote boot setup boot, NtBootPathName is of the form \<server>\<share>\setup\<install-directory>\<platform>.
    // We want the root of the X: drive to be the root of the install directory.

    // If this is a normal remote boot, NtBootPathName is of the form \<server>\<share>\images\<machine>\winnt.
    // We want the root of the X: drive to be the root of the machine directory.

    // Thus in either case, we need to remove the last element of the path.
    p = strrchr( LoaderBlock->NtBootPathName, '\\' );   // find last separator
    if ( (p != NULL) && (*(p+1) == 0) ) {
        // NtBootPathName ends with a backslash, so we need to back up to the previous backslash.
        q = p;
...
#if defined(REMOTE_BOOT)
VOID IopEnableRemoteBootSecurity (PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    InitializeObjectAttributes(&objectAttributes,&IpsecString,OBJ_CASE_INSENSITIVE,NULL,NULL);
    status = NtCreateFile(
                &handle,
                GENERIC_READ | GENERIC_WRITE,
                &objectAttributes,
    ...
    // Set the policy. We need two filters, one for outbound and one for inbound.
    ...
    status = NtDeviceIoControlFile(handle,NULL,NULL,NULL,&ioStatusBlock,IOCTL_IPSEC_SET_POLICY,setPolicy,sizeof(policyBuffer),NULL,0);
    if ( !NT_SUCCESS(status) ) {
    ...
    status = NtDeviceIoControlFile(handle,NULL,NULL,NULL,&ioStatusBlock,IOCTL_IPSEC_SET_SPI,&setSpi,sizeof(IPSEC_SET_SPI),&setSpi,sizeof(IPSEC_SET_SPI));
    if ( !NT_SUCCESS(status) ) {
        KdPrint(( "IopEnableRemoteBootSecurity: Unable to IOCTL_IPSEC_SET_SPI: %x\n", status ));
        return;
    }

    // Set up the security association for the outbound connection.
...
NTSTATUS IopStartTcpIpForRemoteBoot (PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    status = NtDeviceIoControlFile(handle,NULL,NULL,NULL,&ioStatusBlock,IOCTL_IP_SET_ADDRESS_DUP,&IpRequest,sizeof(IP_SET_ADDRESS_REQUEST),NULL,0);
    NtClose( handle );
    if ( !NT_SUCCESS(status) ) {
...
/*++
Routine Description:
    This function determines if the card described by the hwIds is the remote boot network card.
    It checks against the hardware ID for the card that is stored in the setup loader block.
    THIS ASSUMES THAT IOREMOTEBOOTCLIENT IS TRUE AND THAT LOADERBLOCK IS VALID.
Arguments:
...
--*/
{
    ...
    setupLoaderBlock = LoaderBlock->SetupLoaderBlock;// setupLoaderBlock will always be non-NULL if we are remote booting, even if we are not in setup.
    curHwId = HwIds;// Scan through the HwIds for a match.
    ...
    while (*curHwId != L'\0') {
        if (wcscmp(curHwId, setupLoaderBlock->NetbootCardHardwareId) == 0) {
            ...
            if ((DeviceNode->ResourceRequirements->BusNumber != BusNumber) || (DeviceNode->ResourceRequirements->SlotNumber != SlotNumber)) {
                KdPrint(("IopIsRemoteBootCard: ignoring non-matching card:\n"));
                KdPrint(("  devnode bus %d, busdevfunc bus %d\n", DeviceNode->ResourceRequirements->BusNumber, BusNumber));
...
Routine Description:
    ...
    We must do this here since the card is needed to boot, we can't wait for the class installer to run.
    THIS ASSUMES THAT IOREMOTEBOOTCLIENT IS TRUE.
Arguments:
    LoaderBlock - Supplies a pointer to the loader parameter block that was created by the OS Loader.
    UniqueIdHandle - A handle to the device's unique node under the Enum key.
...
--*/
{
    ...
    // If we already think we have initialized a remote boot card, then exit (should not really happen once we identify cards using the bus/slot.
    if (IopRemoteBootCardInitialized) {
        return STATUS_SUCCESS;
    }

    // setupLoaderBlock will always be non-NULL if we are remote booting, even if we are not in setup.
    setupLoaderBlock = LoaderBlock->SetupLoaderBlock;

    // Open the current control set.
    status = IopOpenRegistryKey(&currentControlSetHandle,NULL,&CmRegistryMachineSystemCurrentControlSet,KEY_ALL_ACCESS,FALSE);
    if (!NT_SUCCESS(status)) {
        goto cleanup;
    }
    ...
    InitializeObjectAttributes(&objectAttributes,&unicodeName,OBJ_CASE_INSENSITIVE,currentControlSetHandle,(PSECURITY_DESCRIPTOR)NULL);
    status = ZwCreateKey(&remoteBootHandle,KEY_ALL_ACCESS,&objectAttributes,0,(PUNICODE_STRING)NULL,0,&disposition);
    if (!NT_SUCCESS(status)) {
        goto cleanup;
    }

    // Open the key where the netui code stores information about the cards.
    // During textmode setup this will fail because the Control\Network key is not there.
    // After that it should work, although we may need to create the last node in the path.
    ...
    if (NT_SUCCESS(status)) {
        // If the PnpInstanceID of the first netboot card matches the one for this device node, and the NET_CARD_INFO that the loader
        // found is the same as the one we saved, then this is the same card with the same instance ID as before, so we don't need to do anything.
        ...
        status = ZwQueryValueKey(instanceHandle,&unicodeName,KeyValuePartialInformation,keyValue,sizeof(dataBuffer),&length);
        // Check that it matches. We can init the string because we zeroed
        // the dataBuffer before reading the key, so even if the registry value had no NULL at the end that is OK.
        if ((NT_SUCCESS(status)) && (keyValue->Type == REG_SZ)) {
            ...
            if (RtlEqualUnicodeString(UnicodeDeviceInstance, &pnpInstanceId, TRUE)) {
                ...
                RtlZeroMemory(dataBuffer, sizeof(dataBuffer));
                status = ZwQueryValueKey(remoteBootHandle,&unicodeName,KeyValuePartialInformation,keyValue,sizeof(dataBuffer),&length);
                if ((NT_SUCCESS(status)) && 
    ...
    // We come through here if the saved registry data was missing or not correct. Write all the relevant values to the registry.
...
#ifdef REMOTE_BOOT
    // Identify this as the netboot card so the network class installer knows to assign the reserved GUID.
    PiWstrToUnicodeString(&unicodeName, REGSTR_VALUE_CONFIG_FLAGS);
    keyValue = (PKEY_VALUE_PARTIAL_INFORMATION)dataBuffer;
    RtlZeroMemory(dataBuffer, sizeof(dataBuffer));
    status = ZwQueryValueKey(UniqueIdHandle,&unicodeName,KeyValuePartialInformation,keyValue,sizeof(dataBuffer),&length);
    ...
    ZwSetValueKey(UniqueIdHandle,&unicodeName,TITLE_INDEX_VALUE,REG_DWORD,&tmpValue,sizeof(tmpValue));
#endif

    // Open a handle for card parameters. We write RemoteBootCard plus whatever the BINL server told us to write.
    status = IopOpenRegistryKey(&tmpHandle,NULL,&CmRegistryMachineSystemCurrentControlSetControlClass,KEY_ALL_ACCESS,FALSE);
    if (!NT_SUCCESS(status)) {
        goto cleanup;
    }
    ...
    while (TRUE) {
        RtlZeroMemory(dataBuffer, sizeof(dataBuffer));
        status = ZwEnumerateValueKey(parametersHandle,enumerateIndex,KeyValueBasicInformation,keyValueBasic,sizeof(dataBuffer),&length);
        ...
        // We don't delete "NetCfgInstanceID", 
        // it won't change and its presence signifies to the net class installer that this is a replacement not a clean install.
    ...
    // Write a parameter called RemoteBootCard set to TRUE, this is primarily so NDIS can recognize this as such.
    ...
    if (registryList != NULL) {
        ...
        // The registry list is a series of name\0type\0value\0, with a final \0 at the end. It is in ANSI, not UNICODE.

        // All values are stored under parametersHandle. Type is 1 for DWORD and 2 for SZ.
        ...
        while (*registryList != '\0') {
            ...
            if (*registryList == '1') {
                ...
                ZwSetValueKey(parametersHandle,&uString,TITLE_INDEX_VALUE,REG_SZ,uString2.Buffer,uString2.Length + sizeof(WCHAR));
                registryList += (strlen(registryList) + 1);
            } else {
    ...
    PiWstrToUnicodeString(&unicodeName, L"NetCardInfo");
    ZwSetValueKey(remoteBootHandle,&unicodeName,TITLE_INDEX_VALUE,REG_BINARY,setupLoaderBlock->NetbootCardInfo,sizeof(NET_CARD_INFO));

    // Save the hardware ID, driver name, and service name, so the loader can read  those if the server is down on subsequent boots.
...
--*/
{
    ...
    status = ZwDeviceIoControlFile(RdrHandle,NULL,NULL,NULL,&ioStatusBlock,IOCTL_SWITCHES,&shadowInfo,0,NULL,0);
    if ( NT_SUCCESS(status) ) {
        status = ioStatusBlock.Status;
    }
...
/*++
Routine Description:
    This function walks the servers directories creating CSC database entries for each file and directory on the server.
    The entries are marked as SPARSE so that the local copies of all files are discarded.

    There is a potential problem when a file is in the CSC database and is deleted on the server by an admin local to the server.
    Should this happen, go to the workstation with the CSC and delete the file or directory.
Arguments:
    CscPath - supplies the Nt path with no trailing backslash to the csc directory.
Return Value:
    Status of the operation.
...
#if 0
   //  We don't think we want to re-initialize the database since this will delete the information for user pinned files.
    //  If this functionality is required then CSC probably needs to implement it for all CSC machines.
    ...
    status = ZwDeviceIoControlFile(RdrHandle,NULL,NULL,NULL,&ioStatusBlock,IOCTL_DO_SHADOW_MAINTENANCE,&shadowInfo,0,NULL,0);
    if ( NT_SUCCESS(status) ) {
        status = ioStatusBlock.Status;
    }
    ...
    status = ZwQueryValueKey(handle, &string, KeyValuePartialInformation, keyValue, nameBufferLength, &length);
    ZwClose(handle);
    if ( !NT_SUCCESS(status) ) {
...
/*++
Routine Description:
    Break Path by skipping any leading backslashes, skip over filename and then null out the nextbackslash or null.
    NOTE the Path must not end in a backslash.
Arguments:
    Path - Supplies the start of the next token. 
           On return supplies the terminator of the current token or NULL if we return the last element in the Path.
Return Value:
...
NTSTATUS IopMarkRoot(IN PUNICODE_STRING Root,OUT PHSHADOW RootHandle)
/*++
Routine Description:
...
--*/
{
    ...
    //  Process the server component of the name. This is automatically in the database, magically created by CSC.
    ...
    Status = IopAddHintFromInode(hDir,hShadow,&sSI.ulHintPri,&sSI.ulHintFlags,FLAG_CSC_HINT_PIN_SYSTEM);
#if DBG
    ...
    do {
        ...
        if (next) {
            ...
            *next = L'\\';
            ...
            Status = IopAddHintFromInode(hDir,hShadow,&sSI.ulHintPri,&sSI.ulHintFlags,(FLAG_CSC_HINT_PIN_SYSTEM | FLAG_CSC_HINT_PIN_INHERIT_SYSTEM));
#if DBG
            if (Status != STATUS_PENDING) {
                if (DebugReset) KdPrint(( "IopAddHintFromInode: modified hint flags for %ws\n", next ));
            }
...
/*++
Routine Description:
    Accesses the registry key that controls when to walk the server directory, discard all local copies of files and repin.
Arguments:
...
--*/
{
    ...
    if (Command == READ_CSC) {
        keyValue = (PKEY_VALUE_PARTIAL_INFORMATION)buffer;
        status = ZwQueryValueKey(handle, &string, KeyValuePartialInformation, keyValue, sizeof(buffer), &length);
        dword = *((DWORD *)(&keyValue->Data[0]));
        if (NT_SUCCESS(status) && (dword == SET_FLUSH_CSC)) {
            StartCsc = FLUSH_CSC;
        }
    } else {
        status = ZwSetValueKey(handle, &string, 0, REG_DWORD, &dword, sizeof(DWORD));
        if ( !NT_SUCCESS(status) ) {
...
NTSTATUS IopWalkDirectoryHelper(IN PUNICODE_STRING Directory,IN HSHADOW CSCHandle,IN PUCHAR Buffer,IN ULONG BufferSize)
/*++
Routine Description:
    This is a helper function for IopWalkDirectoryTree. Each file found is pinned. Each directory is added to the list for later processing.
Arguments:
    Directory - Supplies the NT Path to the directory to walk.
    CSCHandle - Supplies the handle passed to CSC that describes the Directory when pinning individual files.
...
--*/
{
    ...
    if (!NT_SUCCESS(Status = ZwOpenFile( &FileHandle,FILE_LIST_DIRECTORY | SYNCHRONIZE,&ObjectAttributes,&IoStatus,FILE_SHARE_READ,FILE_DIRECTORY_FILE ))) {
        goto cleanup;
    }
    ...
    TRUE);
        ...
        //  Point to the first record in the buffer, we are guaranteed to have one otherwise IoStatus would have been No More Files
        ...
        while (TRUE) {
            {
                ...
                if (!skip) {
                    ...
                    if (FileInfo->FileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                        //  Allocate an entry in the directory list. We will process the directory at a later time.
                        UNICODE_STRING DirName;
                        USHORT NewNameLength;

                        RtlInitUnicodeString(&DirName, FileInfo->FileName);
                        NewNameLength = (Directory->Length + DirName.Length + 1) * sizeof(WCHAR);
                    ...
                    if (!sSI.hShadow) {
                        ...
                        memcpy(sFind32->cAlternateFileName,FileInfo->ShortName,FileInfo->ShortNameLength );
                        ...
                        Status = IopCopyServerAcl(
                                     ...
                                     512);               // buffer size
#if 0
                        // We may get ACCESS_DENIED errors from dirs that have been created on the server by hand, so don't treat this as fatal.
                        if (!NT_SUCCESS(Status)) {
                            goto cleanup;
                        }
#endif
                    ...
                    Status = IopAddHintFromInode(sSI.hDir,
                    sSI.hShadow,
                    &sSI.ulHintPri,
                    &sSI.ulHintFlags,
                    ...
                    if (FileInfo->FileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                        ...
                        if ((sSI.uStatus & SHADOW_SPARSE) == 0) {
                            Status = IopSetShadowInfoW(sSI.hDir,sSI.hShadow,SHADOW_SPARSE,SHADOW_FLAGS_ASSIGN | SHADOW_FLAGS_TRUNCATE_DATA);
#if DBG
            ...
            //  Check if there is another record, if there isn't then we simply get out of this loop
            if (FileInfo->NextEntryOffset == 0) {
                break;
            }

            //  There is another record so advance FileInfo to the next record
...
NTSTATUS IopWalkDirectoryTree(IN PUNICODE_STRING Directory,IN HSHADOW CSCHandle)
/*++
Arguments:
    Directory - Supplies the NT Path to the directory to walk.
    CSCHandle - Supplies the handle passed to CSC that describes the Directory when pinning individual files.
...
*/
NTSTATUS IopGetShadowExW(HSHADOW hDir,LPWIN32_FIND_DATAW lpFind32,LPSHADOWINFO lpSI)
{
...
*/
NTSTATUS IopAddHintFromInode(HSHADOW hDir,HSHADOW hShadow,ULONG *lpulHintPri,ULONG *lpulHintFlags,ULONG HintFlagsToSet)
{
    IO_STATUS_BLOCK Iosb;
    NTSTATUS status;
    SHADOWINFO      sSI;
...
*/
NTSTATUS IopSetShadowInfoW(HSHADOW hDir,HSHADOW hShadow,ULONG uStatus,ULONG uOp)
{
    IO_STATUS_BLOCK Iosb;
    NTSTATUS status;
    SHADOWINFO      sSI;
...
*/
NTSTATUS IopCreateShadowW(HSHADOW hDir,LPWIN32_FIND_DATAW lpFind32,ULONG uStatus,PHSHADOW lphShadow)
{
    IO_STATUS_BLOCK Iosb;
    NTSTATUS status;
    SHADOWINFO sSI;
...
NTSTATUS IopCopyServerAcl(HANDLE ParentHandle,PWCHAR FileName,HSHADOW ShadowHandle,BOOLEAN IsDirectory,PUCHAR Buffer,ULONG BufferSize)
/*++
Routine Description:
    This routines takes a file/directory on the server and a shadow file handle and copies the security descriptor from the server to the shadow file.
Arguments:
...
--*/
{
    ...
    InitializeObjectAttributes(&ObjectAttributes,&FileNameString,OBJ_CASE_INSENSITIVE,ParentHandle,NULL);
    Status = ZwOpenFile(&ServerFileHandle,READ_CONTROL,&ObjectAttributes,&IoStatusBlock,FILE_SHARE_READ | FILE_SHARE_WRITE,0);
    ...
    Status = ZwQuerySecurityObject(ServerFileHandle,DACL_SECURITY_INFORMATION,Buffer,BufferSize,&LengthNeeded);
    if (!NT_SUCCESS(Status)) {
        if (Status == STATUS_BUFFER_TOO_SMALL) {
            ...
            if (SecurityDescriptor == NULL) {
                ...
                Status = ZwQuerySecurityObject(ServerFileHandle,DACL_SECURITY_INFORMATION,SecurityDescriptor,LengthNeeded,&LengthNeeded);
            }
        }
        if (!NT_SUCCESS(Status)) {
    ...
    Status = IopGetShadowPathW(ShadowHandle,ShadowFileName,&CopyParams);
    if (!NT_SUCCESS(Status)) {
    ...
    InitializeObjectAttributes(&ObjectAttributes,&ShadowFileString,OBJ_CASE_INSENSITIVE,NULL,NULL);
    Status = ZwOpenFile(&ShadowFileHandle,WRITE_DAC,&ObjectAttributes,&IoStatusBlock,FILE_SHARE_READ | FILE_SHARE_WRITE,0);
    if (!NT_SUCCESS(Status) || !NT_SUCCESS(IoStatusBlock.Status)) {
    ...
    Status = ZwSetSecurityObject(ShadowFileHandle,DACL_SECURITY_INFORMATION,SecurityDescriptor);
...
*/
NTSTATUS IopGetShadowPathW(HSHADOW hShadow,PWCHAR lpLocalPath,LPCOPYPARAMSW lpCP)
{
    IO_STATUS_BLOCK Iosb;
    NTSTATUS status;
...
--*/
{
    ...
    // We've got the address table. Loop through it.
    // If we find an exact match for the gateway, then we're adding or deleting a direct route and we're done.
    // Otherwise try to find a match on the subnet mask, and remember the first one we find.
    ...
    for (i = 0, MatchIndex = 0xffff; i < NumReturned; i++) {
        ...
        // The next hop is on the same subnet as this address. If we haven't already found a match, remember this one.
    ...
    // We've found a match. Fill in the route entry, and call the Set API.
...
--*/
{
    ...
    // Thus in either case, we need to isolate the first element of the path.
    ...
    // Submit the IOCTL.
    Status = NtDeviceIoControlFile(Handle,NULL,NULL,NULL,&ioStatusBlock,IOCTL_NETBT_ADD_TO_REMOTE_TABLE,&cacheInfo,sizeof(cacheInfo),Context,sizeof(Context));
    ASSERT( Status != STATUS_PENDING );
    if ( NT_SUCCESS(Status) ) {
        Status = ioStatusBlock.Status;
    }
...
/*++
Routine Description:
    This routine provides the interface to the TDI QueryInformationEx facility of the TCP/IP stack on NT.
    Someday, this facility will be part of TDI.
Arguments:
    TCPHandle     - Open handle to the TCP driver
    ID            - The TDI Object ID to query
    Buffer        - Data buffer to contain the query results
    BufferSize    - Pointer to the size of the results buffer. Filled in with the amount of results data on return.
    Context       - Context value for the query. Should be zeroed for a new query.
                    It will be filled with context information for linked enumeration queries.
Return Value:
    An NTSTATUS value.
--*/
{
...
/*++
Routine Description:
    This routine provides the interface to the TDI SetInformationEx facility of the TCP/IP stack on NT.
    Someday, this facility will be part of TDI.
Arguments:
    TCPHandle     - Open handle to the TCP driver
...
/*++
Routine Description:
    This routine searches the each partition on each hard disk for one which has the active bit set, returning the first one encountered.
Arguments:
...
--*/
{
    ...
    while (TRUE) {
        ...
        while (TRUE) {
            ...
            Status = ZwDeviceIoControlFile(Handle,NULL,NULL,NULL,&IoStatus,IOCTL_DISK_GET_PARTITION_INFO,NULL,0,&PartitionInfo,sizeof(PARTITION_INFORMATION));
            ZwClose(Handle);
            if (!NT_SUCCESS(Status)) {
                break;
            }
...
FoundDisk:
    // Check each partition for the CSC directory, and find largest free space partition in case no CSC directory exists.
    wcscpy(NetHDCSCPartition, NetHDBootPartition);
    PartitionNumber = 0;
    ...
    while (TRUE) {
        ...
        swprintf(NameBuffer,L"%ws\\Partition%d%ws",DiskNameBuffer,PartitionNumber,REMOTE_BOOT_IMIRROR_PATH_W REMOTE_BOOT_CSC_SUBDIR_W);
        RtlInitUnicodeString(&UnicodeString, NameBuffer);
        InitializeObjectAttributes(&ObjectAttributes,&UnicodeString,OBJ_CASE_INSENSITIVE,NULL,NULL);
        Status = ZwOpenFile( &Handle,FILE_GENERIC_READ,&ObjectAttributes,&IoStatus,FILE_SHARE_READ,FILE_DIRECTORY_FILE);
        if (NT_SUCCESS(Status)) {
            ZwClose(Handle);
        ...
        Status = ZwQueryVolumeInformationFile(Handle,&IoStatus,&SizeInfo,sizeof(SizeInfo),FileFsSizeInformation);
        ZwClose(Handle);
        if(NT_SUCCESS(Status)) {
...
VOID IoStartCscForTextmodeSetup (IN BOOLEAN Upgrade)
{
    NTSTATUS status;
    PWSTR NetHDCSCPartition;
    PUCHAR buffer;
    ...
    NetHDCSCPartition = ExAllocatePoolWithTag(NonPagedPool,(80 + MAX_PATH) * sizeof(WCHAR),'bRoI');
    if (NetHDCSCPartition == NULL) {
    ...
    status = ZwFsControlFile(RdrHandle,NULL,NULL,NULL,&ioStatusBlock,FSCTL_LMR_START_RBR,NetHDCSCPartition,wcslen(NetHDCSCPartition) * sizeof(WCHAR),NULL,0);
    if (NT_SUCCESS(status) ) {
        status = ioStatusBlock.Status;
    }
    ...
    // If this is not an upgrade, or if initialization of CSC failed, reset the cache.
