...

{
    ...
    successful = IopConcatenateUnicodeStrings( &unicodeKeyName, &tmpKeyName, ServiceKeyName);
    if (!successful) {
    ...
    status = IopCreateRegistryKeyEx( &handle, enumRootHandle, &unicodeKeyName, KEY_ALL_ACCESS, REG_OPTION_NON_VOLATILE, NULL);
    ZwClose(enumRootHandle);
    if (!NT_SUCCESS(status)) {
        RtlFreeUnicodeString(&unicodeKeyName);
        goto local_exit0;
