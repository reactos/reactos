...
VOID DumpObjectDirs(IN PCH DirName, IN ULONG Level)
{
    ...
    while (TRUE)
    {
        Status = NtQueryDirectoryObject(Handle, (PVOID)DirInfo, sizeof(DirInfoBuffer), TRUE, RestartScan, &Context, &Length);
...
main(int argc, char **argv, char **envp, int DebugFlag)
{
    ...
    if (argc == 1)
    {
        ...
        else
        {
            ...
            Status = RtlCreateUserProcess(&ImageName, NULL, NULL, NULL, TRUE, NULL, NULL, ProcessParameters, &ProcessInformation, NULL);
            if (!NT_SUCCESS(Status))
            {
...
VOID TestParent(VOID)
{
    ...
    Status = NtCreateSection(&SectionHandle, GENERIC_READ, &ObjectAttributes, &SectionSize, PAGE_READONLY, SEC_RESERVE, NULL);
    if (!NT_SUCCESS(Status))
    {
    ...
    InitializeObjectAttributes(&ObjectAttributes, &DirectoryName, OBJ_CASE_INSENSITIVE | OBJ_PERMANENT, NULL, (PSECURITY_DESCRIPTOR)1);
    ...
    Status = NtQueryObject(DirectoryHandle, ObjectBasicInformation, &ObjectBasicInfo, sizeof(ObjectBasicInfo), &ReturnedLength);
    if (!NT_SUCCESS(Status))
    {
    ...
    Status = NtQueryObject(DirectoryHandle, ObjectNameInformation, ObjectNameInfo, sizeof(ObjectInfoBuffer), &ReturnedLength);
    if (!NT_SUCCESS(Status))
    {
    ...
    DbgPrint("    Name = (%ld,%ld) '%Z'\n", ObjectNameInfo->Name.MaximumLength, ObjectNameInfo->Name.Length, &ObjectNameInfo->Name);

    ObjectTypeInfo = (POBJECT_TYPE_INFORMATION)ObjectInfoBuffer;
    Status = NtQueryObject(DirectoryHandle, ObjectTypeInformation, ObjectTypeInfo, sizeof(ObjectInfoBuffer), &ReturnedLength);
    if (!NT_SUCCESS(Status))
    {
    ...
    DbgPrint("    TypeName = (%ld,%ld) '%Z'\n", ObjectTypeInfo->TypeName.MaximumLength, ObjectTypeInfo->TypeName.Length, &ObjectTypeInfo->TypeName);
