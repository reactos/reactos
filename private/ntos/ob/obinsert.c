...
Routine Description:
    ...
    The Object header includes a pointer to a SecurityDescriptor passed in an object creation call.
    This SecurityDescriptor is not assumed to have been captured.
    This routine is responsible for making an appropriate SecurityDescriptor and removing the reference in the object header.
Arguments:
    Object - Supplies a pointer to the new object body
    AccessState - Optionally supplies the access state for the new handle
    DesiredAccess - Optionally supplies the desired access we want for the new handle
    ObjectPointerBias - Supplies a bias to apply for the pointer count for the object
    NewObject - Optionally receives the pointer to the new object that we've created a handle for
...
--*/
{
    ...
    //  Get the address of the object header, the object create information, the object type, and the address of the object name descriptor, if specified.
    ...
    //  If security checks are not required and an object name is not specified, insert an unnamed object, biasing the count by one, dereference the bias, and return to our caller
    PreviousMode = KeGetPreviousMode();
    if (!ObjectType->TypeInfo.SecurityRequired && (ObjectName == NULL)) {
        ObjectHeader->ObjectCreateInfo = NULL;
        *Handle = NULL;
        Status = ObpCreateUnnamedHandle( Object, DesiredAccess, 1 + ObjectPointerBias, ObjectCreateInfo->Attributes, PreviousMode, NewObject, Handle );
    ...
    //  If the caller hasn't specified an access state then dummy up a local one using the requested desired access
    ...
    if (InsertObject == Object) {
        ...
        //       - Named Objects
        //       - Unnamed objects whose object-type information explicitly indicates a security descriptor is required.
        if ((ObjectName != NULL) || ObjectType->TypeInfo.SecurityRequired) {
            ...
            if ((NameInfo != NULL) && (NameInfo->Directory != NULL)) {
                //  This will allocate a block of memory and copy the parent's security descriptor into it, and return the pointer to the block.

                //  Call ObReleaseObjectSecurity to free up this memory.
                ObGetObjectSecurity( NameInfo->Directory, &ParentDescriptor, &SecurityDescriptorAllocated );
            }

            //  Take the captured security descriptor in the AccessState, put it into the proper format, and call the object's security method to assign the new security descriptor to the new object.
        ...
        if (!NT_SUCCESS( Status )) {
            //  The attempt to assign the security descriptor to the object failed.
            if (DirectoryLocked) {
                //  If ObpLookupObjectName already inserted the object into the directory we have to backup this
                ...
                //  If ObpLookupObjectName inserted the object into the directory it added a reference to the object and to its directory object.
                //  We should remove the extra-references
                if (DirObject) {
                    ObDereferenceObject( Object );
            ...
            //  *** The first backout logic used ObpDeleteNameCheck which is wrong because the security descriptor for the object is not initialized.
            //  Actually  ObpDeleteNameCheck had no effect because the object was removed before from the directory
            ObDereferenceObject( Object );
    ...
    //  This call also will unlock the directory lock is necessary on return
