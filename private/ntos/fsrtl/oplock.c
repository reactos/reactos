...
Abstract:
    ...
    The OPLOCK routines provide support to filesystems which implement opporuntistics locks.
    The specific actions needed are based on the current oplocked state of the file (maintained in the OPLOCK
    structure) and the Irp the Io system provides to the file systems.
    Rather than define separate entry points for each oplock operation a single generic entry point is defined.

    The file systems will maintain a variable of type OPLOCK for each open file in the system.
    This variable is initialized when an unopened file is opened.
    It is uninitialized when the last reference to the file is cleared when the Io system calls the file system with a close call.

    The following routines are provided by this package:

      o  FsRtlInitializeOplock - Initialize a new OPLOCK structure.
         There should be one OPLOCK for every opened file.
         Each OPLOCK structure must be initialized before it can be used by the system.

      o  FsRtlUninitializeOplock - Uninitialize an OPLOCK structure.
         This call is used to cleanup any anciallary structures allocated and maintained by the OPLOCK.
         After being uninitialized the OPLOCK must again be initialized before it can be used by the system.
...
//  Define the compatible filter oplock desired access flags.
//  We won't break a filter oplock when these flags are the only flags specified.
...
//  We encode the different bits so we can test without having to enumerate all of the possible states.
...
//  The non-opaque definition of an OPLOCK is a pointer to a privately defined structure.

typedef struct _NONOPAQUE_OPLOCK {
    //  This is the Irp used to successfully request a level I oplock or batch oplock.
    //  It is completed to initiate the Oplock I break procedure.
    PIRP IrpExclusiveOplock;

    PFILE_OBJECT FileObject;//  This is a pointer to the original file object used when granting an Oplock I or batch oplock.
    LIST_ENTRY IrpOplocksII;//  The start of a linked list of Irps used to successfully request a level II oplock.
    LIST_ENTRY WaitingIrps;//  The following links the Irps waiting to be completed on a queue of Irps.
    OPLOCK_STATE OplockState;//  Oplock state.  This indicates the current oplock state.
    PFAST_MUTEX FastMutex;//  This FastMutex is used to control access to this structure.
} NONOPAQUE_OPLOCK, *PNONOPAQUE_OPLOCK;


//  Each Waiting Irp record corresponds to an Irp that is waiting for an
//  oplock break to be acknowledged and is maintained in a queue off of the Oplock's WaitingIrps list.
typedef struct _WAITING_IRP {
    LIST_ENTRY Links;//  The link structures for the list of waiting irps.
    PIRP Irp;//  This is the Irp attached to this structure.

    //  This is the routine to call when we are done with an Irp we held on a waiting queue.
    //  (We originally returned STATUS_PENDING).
    POPLOCK_WAIT_COMPLETE_ROUTINE CompletionRoutine;

    PVOID Context;//  The context field to use when we are done with the Irp.
    PKEVENT Event;//  This points to an event object used when we do not want to give up this thread.

    //  This field contains a copy of the Irp Iosb.Information field.
    //  We copy it here so that we can store the Oplock address in the Irp.
    ULONG Information;
