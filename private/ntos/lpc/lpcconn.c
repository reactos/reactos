...
Routine Description:
    ...
    the client thread, along with any information passed via the ConnectionInformation parameter.  
    The server thread then decides to either accept or reject the connection request.
    ...
    client about the acceptance or rejection via the ConnectionInformation data block.
    ...
    and receive messages to/from the server process using the NtRequestWaitReplyPort service.
    ...
    the section will be returned in the SectionOffset and ViewSize fields.
    ...
    ConnectionInformation data block giving the reason the connection requests was rejected.

    If the PortName does not exist, or the client process does not have
    sufficient access rights then the returned status will indicate that the port was not found.
...
Arguments:
    ...
    ClientView Structure
        ...
        ULONG ViewSize - Specifies a field that will receive the
            ...
            bytes, of the client's view and is rounded up to the next host page size boundary.
        ...
        PVOID ViewRemoteBase - Specifies a field that will receive
            the base address of the client's section in the server's address space. 
    ...
    RequiredServerSid - Optionally specifies the SID that we expect the
        server side of the port to possess.  
        If not specified then we'll connect to any server SID.

    ServerView - An optional pointer to a structure that will receive
        information about the server process' view in the client's address space.  
        The client process can use this information to validate pointers it receives from the server process.
        ...
        PVOID ViewBase - Specifies a field that will receive the base
            address of the server's section in the client's address space.

        ULONG ViewSize - Specifies a field that will receive the
            size, in bytes, of the server's view in the client's address space.  
    ...
    MaxMessageLength - An optional pointer to a variable that will
        ...
        server.  The value of this parameter will not exceed MAX_PORTMSG_LENGTH bytes.
    ...
    ConnectionInformation - An optional pointer to uninterpreted data.
        ...
        back from the server by its call to the NtCompleteConnectPort service.  
    ...
    ConnectionInformationLength - Pointer to the length of the
        ...
        length of the data stored in the ConnectionInformation data block by the server's call to the NtCompleteConnectPort service.  
...
--*/
{
    ...
    //  Get previous processor mode and probe input and output arguments if necessary.
    ...
    } else {//  Otherwise this is a kernel mode operation
        if (ARGUMENT_PRESENT( ClientView )) {
    ...
    //  If the port type object didn't work then try for a waitable port type object
    ...
    if (ARGUMENT_PRESENT( RequiredServerSid )) {
        ...
        //  If the se information token query didn't work then return the error to our caller
        if (!NT_SUCCESS( Status )) {
            ObDereferenceObject( ConnectionPort );
            return Status;
        }
    ...
    //  deleted and it will dereference the connection port pointer stored in the client port object.

    //  Initialize the client port object to zeros and then fill in the fields.
    ...
    if (ARGUMENT_PRESENT( ClientView )) {
        ...
        RtlMoveMemory( &ConnectMsg->ClientView, &CapturedClientView, sizeof( CapturedClientView ));
    ...
    //  The data length is everything after the port message within the lpcp message. 
    //  In other words the connection message and the user supplied information
    ...
    //  If the caller supplied some connection information then copy that into place right now
    if (ARGUMENT_PRESENT( ConnectionInformation )) {
        try {
            RtlMoveMemory( ConnectMsg + 1, ConnectionInformation, ConnectionInfoLength );
        } except( EXCEPTION_EXECUTE_HANDLER ) {
            //  If we fail then cleanup after ourselves and return the error to our caller
    ...
    if (Status == STATUS_SUCCESS) {
        ...
        if (Msg != NULL) {
            ...
            if (ARGUMENT_PRESENT( ConnectionInformation )) {
                try {
                    ...
                    RtlMoveMemory( ConnectionInformation, ConnectMsg + 1, ConnectionInfoLength );
                } except( EXCEPTION_EXECUTE_HANDLER ) {
            ...
            if (ClientPort->ConnectedPort != NULL) {
                ...
                if (NT_SUCCESS( Status )) {
                    ...
                    try {
                        *PortHandle = Handle;
                        ...
                        if (ARGUMENT_PRESENT( ClientView )) {
                            RtlMoveMemory( ClientView, &ConnectMsg->ClientView, sizeof( *ClientView ));
                        }

                        if (ARGUMENT_PRESENT( ServerView )) {
                            RtlMoveMemory( ServerView, &ConnectMsg->ServerView, sizeof( *ServerView ));
                        }
                    } except( EXCEPTION_EXECUTE_HANDLER ) {
                        Status = GetExceptionCode();
                        NtClose( Handle );
                ...
                //  Otherwise we did not get a connect port from the server so the connection was refused
            ...
            LpcpFreeToPortZone( Msg, FALSE );//  Free the reply message back to the port zone
        } else {
            //  We did not get a reply message so the connection must have been refused
            if (SectionToMap != NULL) {
                ObDereferenceObject( SectionToMap );
            }
        ...
        //  queue and free the message back to the connection port's zone.
        SectionToMap = LpcpFreeConMsg( &Msg, &ConnectMsg, CurrentThread );

        //  The wait was not successful, but in the meantime the server could replied, so it signaled the lpc semaphore.
        //  We have to clear the semaphore state right now.
        if (KeReadStateSemaphore( &CurrentThread->LpcReplySemaphore )) {
            KeWaitForSingleObject( &CurrentThread->LpcReplySemaphore, WrExecutive, KernelMode, FALSE, NULL );
        }
        ...
        //  If a client section was specified, then dereference the section object.
        if ( SectionToMap != NULL ) {
            ObDereferenceObject( SectionToMap );
        }

        //  If the connection was rejected or the wait failed, then
        //  dereference the client port object, which will cause it to be deleted.
        ObDereferenceObject( ClientPort );
    }
