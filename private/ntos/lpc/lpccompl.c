...
Routine Description:
    ...
    the message queue of the port being closed and for each message still in the queue, 
...
Arguments:
    ...
    PortContext - An uninterpreted pointer that is stored in the
        server communication port.  This pointer is returned whenever a message is received for this port.
    ...
    ConnectionRequest - A pointer to a structure that describes the connection request being accepted or rejected:
        ...
        ULONG MessageId - A unique value that identifies the connection request being completed.
    ...
    AcceptConnection - Specifies a boolean value which indicates where
        ...
        server communication port handle will be created and connected to the client's communication port handle.  
        A value of FALSE means that the connection request is not accepted.

    ServerView - A pointer to a structure that specifies the section that
        the server process will use to send messages back to the client process connected to this port.
        ...
        PVOID ViewRemoteBase - Specifies a field that will receive
            ...
            address space.  Used to generate pointers that are meaningful to the client.

    ClientView - An optional pointer to a structure that will receive
        information about the client process' view in the server's
        address space.  The server process can use this information to validate pointers it receives from the client process.
        ...
        PVOID ViewBase - Specifies a field that will receive the base
            address of the client port's memory in the server's address space.

        ULONG ViewSize - Specifies a field that will receive the
            size, in bytes, of the client's view in the server's address space.  
            If this field is zero, then client has no view in the server's address space.
...
--*/
{
    ...
    try {
        RtlMoveMemory( ConnectMsg + 1, (PCHAR)(ConnectionRequest + 1), ConnectionInfoLength );
    } except( EXCEPTION_EXECUTE_HANDLER ) {
    ...
    if (AcceptConnection) {
        ...
        if (NT_SUCCESS( Status )) {
            ...
            //  create the handle a rouge caller might close and destroy the port.
        ...
        LpcpPrint(( "Refusing connection from %x.%x\n", Msg->Request.ClientId.UniqueProcess, Msg->Request.ClientId.UniqueThread ));
    }
...
/*++
Routine Description:
    This routine is used to prepare the client thread to receive a reply to its connection request
Arguments:
...
--*/
{
    ...
    //  Remove the thread from the rundown list the connection port as we are sending a reply. 
    //  The operation only needs to take place if the thread isn't exiting and it's in the lpc reply chain for a connection port
