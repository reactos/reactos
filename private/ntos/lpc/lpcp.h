...
//  define LPCP_TRACE_SERVER_THREADS symbol to track the server threads on a LPCP_SERVER_COMMUNICATION port
...
typedef struct _LPC_MUTEX {
    FAST_MUTEX  Lock;    
    PETHREAD    Owner;//  field that holds the thread that owns the lock    
    ULONG       Count;//  Number of locks acquired
