...
Routine Description:
    ...
    working KEVENT that can be used to wait for LPC messages to arrive asynchronously.
Arguments:
    See LpcpCreatePort.
Return Value:
    NTSTATUS - An appropriate status value
...
Arguments:
    ...
    ObjectAttributes - A pointer to a structure that specifies the name of the
        ...
        PUNICODE_STRING ObjectName - An optional pointer to a null terminated
            ...
            port.  This is useful for sending and receiving messages between threads of a single process.
    ...
    MaxMessageLength - Specifies the maximum length of messages sent or
        received on communication ports created from this connection port.  
...
--*/
{
    ...
    //  unconnected communication port that a process can use to communicate between threads.
    ...
    ConnectionPort->MaxMessageLength = LpcpZone.Zone.BlockSize - FIELD_OFFSET( LPCP_MESSAGE, Request );

    ConnectionPort->MaxConnectionInfoLength = ConnectionPort->MaxMessageLength - sizeof( PORT_MESSAGE ) - sizeof( LPCP_CONNECTION_MESSAGE );
