...
/*++
Routine Description:
    This procedure is used by the server thread to temporarily acquire the identifier set of a client thread.

    This service establishes an impersonation token for the calling thread.
    The impersonation token corresponds to the context provided by the port client.  
    The client must currently be waiting for a reply to the specified message.

    This service returns an error status code if the client thread is not waiting for a reply to the message.  
    The security quality of service parameters specified by the client upon connection dictate what use the
...
Arguments:
    ...
    Message - Specifies an address of a message that was received from the
        ...
        thread must be waiting for a reply to the message in order to impersonate the client.

Return Value:
    NTSTATUS - Status code that indicates whether or not the operation was successful.
--*/
{
    ...
    //  It is an error to try this on any port other than a server communication port
    ...
    //  thread pointer.  This is a referenced pointer to keep the thread from evaporating out from under us.
    ...
    if ( ( PortObject->ConnectedPort == NULL ) || ( PortObject->ConnectedPort->Flags & PORT_DELETED )) {
        LpcpReleaseLpcpLock();
        ObDereferenceObject( PortObject );
    ...
    //  specified on this call, if the user gave us a bad message id. 
    //  If not then a bogus message has been specified, so return failure.
    ...
    if ( !IsListEmpty( &ClientThread->LpcReplyChain ) ) {
        ...
        while ((Next != NULL) && (Next != Head)) {
            ...
            if ( ThreadWaitingForReply ==  ClientThread) {
                break;//  If found the Client thread, quit from the loop
            }

            Next = Next->Flink;
        }
        
        if (ThreadWaitingForReply !=  ClientThread) {//  If not found, return an error
            LpcpReleaseLpcpLock();
            ObDereferenceObject( PortObject );
            ObDereferenceObject( ClientThread );
            ObDereferenceObject( ConnectedPort );
    ...
    //  If the client requested dynamic security tracking, then the client security needs to be referenced. 
    //  Otherwise, (static case) it is already in the client's port.
