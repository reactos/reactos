...
Routine Description:
    ...
    does not have its own receive queue and the message is therefore queued to the receive queue of the connection port.
    ...
    whenever message is placed in the receive queue associated with the server communication port.
    ...
    processed and the relevant pages will be mapped into the caller's address space.  
    The service returns an error if there is not enough room in the caller's address space to accomodate the mappings.
...
Arguments:
    ...
    PortContext - Specifies an optional pointer to a variable that is to
        receive the context value associated with the communication port that the message is being received from.  
        This context variable was specified on the call to the NtAcceptConnectPort service.

    ReplyMessage - This optional parameter specifies the address of a reply message to be sent.  
        The ClientId and MessageId fields determine which thread will get the reply.  
        See description of NtReplyPort for how the reply is sent.  
        The reply is sent before blocking for the receive.
...
--*/
{
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            ProbeForWrite(ReceiveMessage, sizeof(*ReceiveMessage), sizeof(ULONG));
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            return GetExceptionCode();
        }
    } else {
        //  Kernel mode threads call with wait mode of user so that their kernel stacks are swappable. 
        //  Main consumer of this is SepRmCommandThread
    ...
    //  Reference the port object by handle and if that doesn't work try a waitable port object type
    ...
    if (ARGUMENT_PRESENT(ReplyMessage)) {
        //  Translate the ClientId from the connection request into a thread pointer. 
        //  This is a referenced pointer to keep the thread from evaporating out from under us.
        Status = PsLookupProcessThreadByCid(&CapturedReplyMessage.ClientId, NULL, &WakeupThread);
        if (!NT_SUCCESS(Status)) {
            ObDereferenceObject(PortObject);
            return Status;
        }
        ...
        //  field of the thread and get the pointer to the message that the thread is waiting for a reply to.
        LpcpAcquireLpcpLock();
        ...
        //  We also fail this request if the caller isn't replying to a request message. 
        //  For example, if the caller is replying to a connection request
        ...
        //  Do this before we actually fiddle with the wakeup threads fields. 
        //  Otherwise we could mess up its state
        try {
            LpcpMoveMessage(&Msg->Request, &CapturedReplyMessage, (ReplyMessage + 1), LPC_REPLY, NULL);
        } except(EXCEPTION_EXECUTE_HANDLER)
        {
            LpcpFreeToPortZone(Msg, TRUE);
            LpcpReleaseLpcpLock();
        ...
        //  test for (CapturedReplyMessage.u2.s2.DataInfoOffset != 0) as a prerequisite for doing the call.
        LpcpFreeDataInfoMessage(PortObject, CapturedReplyMessage.MessageId, CapturedReplyMessage.CallbackId);
        ...
        KeReleaseSemaphore(&WakeupThread->LpcReplySemaphore, 1, 1, FALSE);
        ObDereferenceObject(WakeupThread);
        ...
        //       clause both do a wait it to have the lpcp trace denote the wait without a preceeding reply
    ...
    if (Status == STATUS_SUCCESS) {
        ...
        //  Now make the thread state to be the message we're currently working on
        ...
        try {
            ...
            if ((Msg->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE) == LPC_CONNECTION_REQUEST) {
                ...
                *ReceiveMessage = TempMsg->Request;
                ...
                RtlMoveMemory(ReceiveMessage + 1, ConnectMsg + 1, ConnectionInfoLength);
                if (ARGUMENT_PRESENT(PortContext)) {
                    *PortContext = NULL;
                }
                ...
                LpcpMoveMessage(ReceiveMessage, &Msg->Request, (&Msg->Request) + 1, 0, NULL);
                if (ARGUMENT_PRESENT(PortContext)) {
                    *PortContext = Msg->PortContext;
                }

                //  If message contains DataInfo for access via NtRead/WriteRequestData
                //  then put the message on a list in the communication port and dont free it. 
                //  It will be freed when the server replies to the message.
                if (Msg->Request.u2.s2.DataInfoOffset != 0) {
                    LpcpSaveDataInfoMessage(PortObject, Msg);
                    Msg = NULL;
                }                
            } else {//  Otherwise this is a reply message we just recieved
...
Arguments:
    See NtReplyWaitReceivePort.
    Timeout - Supplies an optional timeout value to use when waiting for a receive.
Return Value:
    See NtReplyWaitReceivePort.
--*/
{
    ...
    Status = LpcpReferencePortObject(PortHandle, 0, PreviousMode, &PortObject);
    if (!NT_SUCCESS(Status)) {
        Status = ObReferenceObjectByHandle(PortHandle, 0, LpcWaitablePortObjectType, PreviousMode, &PortObject, NULL);
        if (!NT_SUCCESS(Status)) {
            return Status;
        }
    }
    ...
    if (ARGUMENT_PRESENT(ReplyMessage)) {
        ...
        //  has been specified, so release the mutex, dereference the thread and return failure.

        //  We also fail this request if the caller isn't replying to a request
        //  message.  For example, if the caller is replying to a connection request
        if ((WakeupThread->LpcReplyMessageId != CapturedReplyMessage.MessageId) ||
            ((WakeupThread->LpcReplyMessage != NULL) &&
            (((PLPCP_MESSAGE)(WakeupThread->LpcReplyMessage))->Request.u2.s2.Type & ~LPC_KERNELMODE_MESSAGE) != LPC_REQUEST)) {
            LpcpPrint(("%s Attempted ReplyWaitReceive to Thread %lx (%s)\n",
        ...
        //  Release the mutex that guards the LpcReplyMessage field after marking message as being replied to.
        Msg->RepliedToThread = WakeupThread;
        WakeupThread->LpcReplyMessageId = 0;
        WakeupThread->LpcReplyMessage = (PVOID)Msg;
        ...
        //       only substantial difference between NtReplyWaitReceivePort and NtReplyWaitReceivePortEx
        Status = KeWaitForSingleObject(ReceivePort->MsgQueue.Semaphore, WrLpcReceive, WaitMode, FALSE, Timeout);

        //  Fall into receive code.  Client thread reference will be returned by the client when it wakes up.
    } else {
        //  The user did not give us a reply message to send off
