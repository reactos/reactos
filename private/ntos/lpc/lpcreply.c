...
Routine Description:
    ...
    The Type field of the message is set to LPC_REPLY by the service.  
    If the MapInfoOffset field of the reply message is non-zero, then the
    ...
    The ClientId and MessageId fields of the ReplyMessage structure are used to identify the thread waiting for this reply.  
    If the target thread is in fact waiting for this reply message, then the reply message is copied
    ...
    PortHandle parameter and the Type field of the message is set to LPC_LOST_REPLY.
...
--*/
{
    ...
    //  and get the pointer to the message that the thread is waiting for a reply to.
    LpcpAcquireLpcpLock();
    ...
    //  Locate and free the messsage from the port. 
    //  This call use to test for (CapturedReplyMessage.u2.s2.DataInfoOffset != 0) as a
...
--*/
{
    ...
    if (Status == STATUS_SUCCESS) {
        ...
        //  event after free the message so that threads waiting to allocate a request message buffer will wake up. 
        //  Finally, release the mutex and return the system service status.
...
Arguments:
    ...
    DataEntryIndex - Supplies the index of the port data entry in the preceeding message that we are reading
    ...
    NumberOfBytesRead - Optionally returns the number of bytes read into the buffer
Return Value:
...
Arguments:
    ...
    DataEntryIndex - Supplies the index of the port data entry in the preceeding message that we are writing
    Buffer - Supplies the location into which the data is to be written
    BufferSize - Supplies the size, in bytes, of the preceeding buffer
    NumberOfBytesRead - Optionally returns the number of bytes written from the buffer
Return Value:
    NTSTATUS - An appropriate status value
--*/
{
    ...
    return LpcpCopyRequestData( TRUE, PortHandle, Message, DataEntryIndex, Buffer, BufferSize, NumberOfBytesWritten );
}
...
Arguments:
    WriteToMessageData - TRUE if the data is to be copied from the user buffer to the message and FALSE otherwise
    ...
    DataEntryIndex - Supplies the index of the port data entry in the preceeding message that we are transfering
    Buffer - Supplies the location into which the data is to be transfered
    BufferSize - Supplies the size, in bytes, of the preceeding buffer
    NumberOfBytesRead - Optionally returns the number of bytes transfered from the buffer
Return Value:
    NTSTATUS - An appropriate status value
--*/
{
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            ProbeForRead( Message, sizeof( *Message ), sizeof( ULONG ));
    ...
    if (ClientThread->LpcReplyMessageId != CapturedMessage.MessageId) {
        ...
        if (Msg != NULL) {
            ...
            //  Make sure the caller isn't asking for an index beyond what's in the message
