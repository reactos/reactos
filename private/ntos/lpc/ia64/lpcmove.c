...
*** or reproduced in whole or in part without explicit written authorization from the company.
*/
...
/*++
Routine Description:
    This function moves an LPC message block and optionally sets the message type and client id to the specified values.
Arguments:
    DstMsg     - Supplies a pointer to the destination message.
    SrcMsg     - Supplies a pointer to the source message.
    SrcMsgData - Supplies a pointer to the source message data to copy to destination.
    MsgType    - If non-zero, then store in type field of the destination message.
    ClientId   - If non-NULL, then points to a ClientId to copy to the destination message.
--*/
{
    ULONG NumberDwords;
