...

{
    ...
    if ((KeI386MachineType & MACHINE_TYPE_PC_9800_COMPATIBLE) == 0) {
        ...
        PartialResourceDescriptor = (PCM_PARTIAL_RESOURCE_DESCRIPTOR) ((PUCHAR)ResourceDescriptor + sizeof(CM_FULL_RESOURCE_DESCRIPTOR) +
    ...
    try {
        Process->VdmObjects = ExAllocatePoolWithTag(NonPagedPool, sizeof(VDM_PROCESS_OBJECTS), ' MDV');
