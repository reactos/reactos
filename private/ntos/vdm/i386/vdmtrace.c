...
VOID VdmTraceEvent(USHORT Type, USHORT wData, USHORT lData, PKTRAP_FRAME TrapFrame)
{
    ...
    PAGED_CODE();
#if 0
    // This code represents a security problem.
    // Since it is only used on special occasions, it won't be built into the standard build.
    // Individuals wishing to use it can build themselves a kernel with it in.
#if 0
    ...
    try {
        ...
        if ((*(ULONG *)(FIXED_NTVDMSTATE_LINEAR) & VDM_TRACE_HISTORY)) {
            ...
            if (VdmTib->TraceInfo.pTraceTable) {
                ...
                if (++VdmTib->TraceInfo.CurrentEntry >= (VdmTib->TraceInfo.NumPages*4096/sizeof(VDM_TRACEENTRY))) {
