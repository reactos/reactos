...
/*++
Routine Description:
    This function gains control after the system has been bootstrapped and before the system has been initialized. Its function is to initialize
    the kernel data structures, initialize the idle thread and process objects, initialize the processor control block, call the executive initialization
    routine, and then return to the system startup routine. This routine is also called to initialize the processor specific structures when a new processor is brought on line.
Arguments:
...
--*/
{
    ...
    KeLoaderBlock = LoaderBlock;// Save the address of the loader parameter block.
    SetMember(Number, KeActiveProcessors);// Set the appropriate member in the active processors set.
    ...
    // Set the maximum address space number to the minimum of all maximum address space numbers passed via the loader block.
    if (Number == 0) {
    ...
    Prcb->Pcr = Pcr;// Set address of PCR in PRCB.
    ...
    KiProcessorBlock[Number] = Prcb;// Set address of processor block.
    Thread->ApcState.Process = Process;// Set address of process object in thread object.
    SetMember( Number, KeActiveProcessors );// Set the appropriate member in the active processors set.
    ...
    // Set global processor architecture, level and revision.  The latter two are the least common denominator on an MP system.
#ifdef _AXP64_
    ...
    // Initialize all interrupt vectors to transfer control to the unexpected interrupt routine.

    // N.B. This interrupt object is never actually "connected" to an interrupt
    //      vector via KeConnectInterrupt. It is initialized and then connected by simply storing the address of the dispatch code in the interrupt vector.
    if (Number == 0) {
        KeFeatureBits = KiGetFeatureBits();
        KxUnexpectedInterrupt.DispatchAddress = KiUnexpectedInterrupt;// Initial the address of the interrupt dispatch routine.
        KeInitializeSpinLock(&KiContextSwapLock);// Initialize the context swap spinlock.
    ...
    KeRaiseIrql(APC_LEVEL, &OldIrql);// Raise IRQL to APC level.
    ...
    if (Number == 0) {
        ...
        KiDmaIoCoherency = DMA_READ_DCACHE_INVALIDATE | DMA_WRITE_DCACHE_SNOOP;// Initialize default DMA coherency value for Alpha.
        KiInitSystem();// Perform architecture independent initialization.
    ...
    KeInitializeThread(Thread, (PVOID)((ULONG_PTR)IdleStack - PAGE_SIZE), (PKSYSTEM_ROUTINE)NULL, (PKSTART_ROUTINE)NULL, (PVOID)NULL, (PCONTEXT)NULL, (PVOID)NULL, Process);
