...
--*/
{
    ...
    // Get the address of the current processor block and move the specified register state from the processor context structure to the specified trap and exception frames
#if !defined(NT_UP)
...
--*/
{
    ...
    // Get the address of the current processor block and move the specified register state from specified trap and exception frames to the current processor context structure.
#if !defined(NT_UP)
    Prcb = KeGetCurrentPrcb();
...
--*/
{
    ...
    RequestSummary = KiIpiProcessRequests();// Process any outstanding interprocessor requests.

    if ((RequestSummary & IPI_FREEZE) != 0) {// If freeze is requested, then freeze target execution.
        KiFreezeTargetExecution(TrapFrame, ExceptionFrame);
    }

    return (RequestSummary & ~IPI_FREEZE) != 0;// Return whether any requests were processed.
}