...
--*/
{
    ...
    if (MappedSession->SequenceNumber != KiMasterSequence) {
        ...
        KiSetRegionRegister((PVOID)MM_SESSION_SPACE_DEFAULT, KiMakeValidRegionRegister(MappedSession->RegionId, PAGE_SHIFT));
    }
...
/*++
 Routine Description:
    Generate a new region id and synchronze the region Ids on all the processors if necessary.
    If the region ids wrap then flush all processor TLB's.
 Arguments:
 ...
 Notes:
    This routine called by KiSwapProcess, KeAttachSessionSpace and KeCreateSessionSpace.
 Environment:
    Kernel mode.
...
--*/
{
    ...
    KiAcquireSpinLock(&KiMasterRidLock);
    if ((ProcessRegion->SequenceNumber == KiMasterSequence) && (SessionRegion->SequenceNumber == KiMasterSequence)) {
        goto not_recycled;
    }
    ...
    if ((RidRecycled == FALSE) && (SessionRegion->SequenceNumber != KiMasterSequence)) {
        if (KiMasterRid + 1 > KiMaximumRid) {
            RidRecycled = TRUE;
        } else {
            KiMasterRid += 1;
    ...
    SessionRegion->RegionId = KiMasterRid;
    SessionRegion->SequenceNumber = KiMasterSequence;
    KiSetRegionRegister((PVOID)MM_SESSION_SPACE_DEFAULT, KiMakeValidRegionRegister(SessionRegion->RegionId, PAGE_SHIFT));
...
/*++
 ...
 Arguments:
    SessionMapInfo - Supplies a session map info to be disabled for sharing.
 Return Value:
    None.
 Environment:
    Kernel mode.
...
/*++
 Routine Description:
    This is the target function for synchronizing the new session region id.
    This routine is called when the session space is removed and all the processors need to be notified.
 Arguments:
    SignalDone - Supplies a pointer to a variable that is cleared when the requested operation has been performed.
...
--*/
{
    ...
    // check to see if the current process is the process that needs to be sync'ed
    if (Process == Thread->ApcState.Process) {
        KiAcquireSpinLock(&KiMasterRidLock);

        // disable the session region.
        KiSetRegionRegister((PVOID)MM_SESSION_SPACE_DEFAULT, KiMakeValidRegionRegister(Process->SessionMapInfo->RegionId, PAGE_SHIFT));
...
/*++
 Routine Description:
    This routine removes the session space and synchronize the all threads on the other processors.
 Arguments:
    DeleteSessionMapInfo - if TRUE, the session map info will be deleted. if FALSE, the session map info will not be deleted.
 Environment:
    Kernel mode.
--*/
{
...
Remarks:
    KiLockDispaterLock or LockQueuedDispatcherLock is not necessary since the process has not run yet.
--*/
{
    KIRQL OldIrql;
