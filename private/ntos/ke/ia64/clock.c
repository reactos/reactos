...
Abstract:
    This module implements the platform specific clock interrupt processing routines in for the kernel.
...
/*++
Routine Description:
    This routine determines if it should attempt to place a timer expiration DPC in the system DPC list and to drive the DPC by initiating a dispatch level interrupt on the current processor.
...
--*/
{
    ...
    if (NextEntry != ListHead) {
        ...
        if (TimeValue <= SharedUserData->InterruptTime) {
            ...
            if (Dpc->Lock == NULL) {
                // Put timer expiration DPC in the system DPC list and initiate a dispatch interrupt on the current processor.
                Prcb->DpcCount += 1;
...
Routine Description:
    ...
    It's function is to update the system time and to check to determine if a timer has expired.

    N.B. This routine is executed on a single processor in a multiprocess system.
    The remainder of the processors in the complex execute the quantum end and runtime update code.
Arguments:
    TrFrame   - Supplies a pointer to a trap frame.
    Increment - The time increment to be used to adjust the time slice for the next tick. The value is supplied in 100ns units.
--*/
{
    long       SaveTickOffset;
    ...
    if ((LONG)KiTickOffset > 0)
    {
        ...
        // Tick has completed, tick count set to maximum increase plus any residue and system time is updated.
        ...
        // Determine if a timer has expired at either the current hand value or the next hand value.
...
Routine Description:
    ...
    It's function is to update the run time of the current thread, udpate the run time for the thread's process, and decrement the current thread's quantum.
Arguments:
    TrFrame - Supplies a pointer to a trap frame.
--*/
{
    ...
    if (TrFrame->PreviousMode != KernelMode) {
        ++Thread->UserTime;
        ExInterlockedIncrementLong(&Process->UserTime, &Lock);// Atomic Update of Process User Time required.
        ++Prcb->UserTime;// Update the time spent in user mode for the current processor.
    } else {
        ...
        ++Prcb->KernelTime;// Update the time spent in kernel mode for the current processor.
    }
    ...
    if (Prcb->DpcQueueDepth != 0 && Prcb->DpcRoutineActive == 0) {
        ...
        KiRequestSoftwareInterrupt(DISPATCH_LEVEL);// Need to request a DPC interrupt.
    ...
    Thread->Quantum -= CLOCK_QUANTUM_DECREMENT;// Decrement current thread quantum and determine if quantum end has occurred.
...
Routine Description:
    This routine is executed on all processors in the processor complex.
    The routine is entered as the result of an interrupt generated by the profile timer.
    Its function is to update the profile information for the currently active profile objects.

    N.B. KeProfileInterrupt is an alternate entry for backwards compatability that sets the source to zero (ProfileTime).
Arguments:
    TrFrame   - Supplies a pointer to a trap frame.
--*/
{
...
Routine Description:
    ...
    N.B. KeProfileInterruptWithSource is not currently fully implemented by any of the architectures.
Arguments:
    TrFrame - Supplies a pointer to a trap frame.
...
--*/
{
    ...
    for (; NextEntry != ListHead; NextEntry = NextEntry->Flink) {
        ...
        BucketPter = (PCHAR)Profile->Buffer + ((((PCHAR)TrFrame->StIIP - (PCHAR)Profile->RangeBase) >> Profile->BucketShift) & 0xFFFFFFFC);
