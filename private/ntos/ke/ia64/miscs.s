...
//    Functions are provided to request a software interrupt, continue thread execution, flush TLBs and write buffers, and perform last chance exception processing.
//
// Author:
...
//    11-Jan-1996  wc    Set up register sp to point to the exception frame on the stack before calling KiSaveExceptionFrame and branching directly to KiExceptionExit.
...

// NTSTATUS NtContinue (IN PCONTEXT ContextRecord, IN BOOLEAN TestAlert)
// Routine Description:
//    This routine is called as a system service to continue execution after an exception has occurred. Its functions is to transfer information from
//    the specified context record into the trap frame that was built when the system service was executed, and then exit the system as if an exception had occurred.
// Arguments:
//    ContextRecord (a0) - Supplies a pointer to a context record.
//    TestAlert (a1) - Supplies a boolean value that specifies whether alert should be tested for the previous processor mode.
//    N.B. Register t0 is assumed to contain the address of a trap frame.
// Return Value:
//    Normally there is no return from this routine. However, if the specified context record is misaligned or is not accessible, then the appropriate status code is returned.
...
// Transfer information from the context record to the exception and trap frames.
...
//
// Check to determine if alert should be tested for the previous processor mode and restore the previous mode in the thread object.
//
// Application that invokes the NtContinue() system service must have flushed all stacked registers to the backing store. 
// Sanitize the bspstore to be equal to bsp; otherwise, some stacked GRs will not be restored from the backing store.
...
//    FirstChance (a2) - Supplies a boolean value that determines whether this is the first (TRUE) or second (FALSE) chance for dispatching the exception.
//    N.B. Register t0 is assumed to contain the address of a trap frame.
// Return Value:
//    Normally there is no return from this routine.
//    However, if the specified context record or exception record is misaligned or is not accessible, then the appropriate status code is returned.
...
// If the raise exception routine returns success, then exit via the exception exit code.  Otherwise, return to the system service dispatcher.
//
// N.B. The exception exit code will restore the exception frame & trap frame and then rfi to user code.
//
// Set up for branch to KiExceptionExit
//
//      s0 = trap frame
...

// VOID KeFillLargeEntryTb (IN HARDWARE_PTE Pte[], IN PVOID Virtual, IN ULONG PageSize)
// Routine Description:
//    This function fills a large translation buffer entry.
//    N.B. It is assumed that the large entry is not in the TB and therefore the TB is not probed.
// Arguments:
//    Pte (a0) - Supplies a pointer to the page table entries that are to be written into the TB.
//    Virtual (a1) - Supplies the virtual address of the entry that is to be filled in the translation buffer.
...

// VOID KeFillFixedEntryTb (IN HARDWARE_PTE Pte[], IN PVOID Virtual, IN ULONG Index)
// Routine Description:
...

// VOID KeFillFixedLargeEntryTb (IN HARDWARE_PTE Pte[], IN PVOID Virtual, IN ULONG PageSize, IN ULONG Index)
// Routine Description:
//    This function fills a fixed translation buffer entry with a large page size.
// Arguments:
//    Pte (a0) - Supplies a pointer to the page table entries that are to be written into the TB.
//    Virtual (a1) - Supplies the virtual address of the entry that is to be filled in the translation buffer.
//    PageSize (a2) - Supplies the size of the large page table entry.
...
//    N.B. It is assumed that the large entry is not in the TB and therefore the TB is not probed.
// Arguments:
//    Pte (a0) - Supplies a page table entry that is to be written into the Inst TB.
//    Virtual (a1) - Supplies the virtual address of the entry that is to be filled in the translation buffer.
...

// Routine Description:
//     This routine saves the thread's current non-volatile NPX state, and sets a new initial floating point state for the caller.
//
//     This is intended for use by kernel-mode code that needs to use the floating point registers. Must be paired with KeRestoreFloatingPointState
// Arguments:
...

// Routine Description:
//     This routine restores the thread's current non-volatile NPX state, to the passed in state.
//
//     This is intended for use by kernel-mode code that needs to use the floating point registers.
//     Must be paired with KeSaveFloatingPointState
// Arguments:
//     a0 - Supplies pointer to KFLOATING_SAVE structure
// Return Value:
//     None.
...
// Routine Description:
//     This routine flush all the dirty registers to the backing store and invalidate them.
// Arguments:
//     None.
// Return Value:
//     None.
...

// PVOID KiGetPhysicalAddress(PVOID Virtual)        
// Routine Description:
...

// VOID KiSetRegionRegister(PVOID Region, ULONGLONG Contents )
// Routine Description:
