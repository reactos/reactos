...
--*/
{
    ...
    try {
        ...
        // N.B. EM requires stacks to be 16-byte aligned, therefore the input length must be rounded up to a 16-byte boundary.
      ...
      if (PsGetCurrentProcess()->DebugPort != NULL) {
        ...
        if (BsFrameSize) {
            ...
            // Copy the dirty stacked registers back into the user backing store
            TearPointOffset = (USHORT) TrapFrame->RsBSPSTORE & 0x1F8;
            RtlCopyMemory((PVOID)(TrapFrame->RsBSPSTORE), (PVOID)(PCR->InitialBStore + TearPointOffset), BsFrameSize);
            ...
            if (TopBound > BottomBound) {
                // user dirty stacked GR span across at least one RNAT boundary; need to deposit the valid RNAT bits from the trap frame into the kernel backing store.
                // Also, the RNAT field in the trap frame has to be updated.
                UserRnats2 = *(PULONGLONG)BottomBound & Mask;
                *(PULONGLONG)BottomBound = UserRnats1 | UserRnats2;
                TrapFrame->RsRNAT = CurrentRNAT;
            } else {
                // user stacked register region does not span across an RNAT boundary; combine the RNAT fields from both the trap frame and the context frame.
