...
--*/
{
    ...
    // Initialize the exception record
    ExceptionRecord = (PEXCEPTION_RECORD)&TrapFrame->ExceptionRecord;
    ExceptionRecord->ExceptionAddress = (PVOID) RtlIa64InsertIPSlotNumber(TrapFrame->StIIP, ((TrapFrame->StISR & ISR_EI_MASK) >> ISR_EI));
    ExceptionRecord->ExceptionFlags = 0;
    ExceptionRecord->ExceptionRecord = (PEXCEPTION_RECORD)NULL;
    ExceptionRecord->NumberParameters = 5;
    ExceptionRecord->ExceptionInformation[0] = 0;
...
Return Value:
    If bug check is called, there is no return from this routine and the system is crashed.
    If an exception occured while attempting to copy the user in-memory argument list, then there is no return from this
    routine, and unwind is called. Otherwise, ExceptionContinueSearch is returned as the function value.
--*/
{
    CONTEXT Context;
    PKTHREAD Thread;
    ...
    if (IS_UNWINDING(ExceptionRecord->ExceptionFlags)) {
        ...
        // If a target unwind is being performed, then continue execution is returned to transfer control to the system service exit code.
        // Otherwise, restore the previous mode if the previous mode is not user and there is no mutex owned by the current thread.
        if (ExceptionRecord->ExceptionFlags & EXCEPTION_TARGET_UNWIND) {
            ...
            if (Thread->PreviousMode == KernelMode) {
                ...
                KeBugCheck(SYSTEM_UNWIND_PREVIOUS_USER);// Previous mode is user, call bug check.
            }
        }
    } else {
        ULONG IsrCode;
        ...
        // If the exception PC is within the in-memory argument copy code of the system service dispatcher, then call unwind to transfer control to the system service exit code.
        // Otherwise, check if the previous mode is user or kernel mode.
        ExceptionAddress = (ULONG_PTR)ExceptionRecord->ExceptionAddress;
        if ((ExceptionAddress < MM_EPC_VA+KiSystemServiceStartOffset) || (ExceptionAddress >= MM_EPC_VA+KiSystemServiceEndOffset))
        {
            if (KeGetCurrentThread()->PreviousMode == UserMode) {
                KeBugCheck(SYSTEM_SERVICE_EXCEPTION);// Previous mode is user, call bug check.
            } else {
                return ExceptionContinueSearch;// Previous mode is kernel, continue to search
            }
        } else {
...
#pragma warning( disable : 4312 ) // disabling warning on PVOID casting for the ExceptionCode
            RtlUnwind2(EstablisherFrame, (PVOID)(MM_EPC_VA+KiSystemServiceExitOffset), NULL, (PVOID)ExceptionRecord->ExceptionCode, &Context);
