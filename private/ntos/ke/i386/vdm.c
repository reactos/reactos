...
Abstract:
    This module contains support routines for the x86 monitor for running Dos applications in V86 mode.
...
Notes:
    In its current implementation, this code is less robust than it needs to be.
    This will be fixed.  Specifically, parameter verification needs to be done. (daveh 7/15/91)
...
/*++
Routine Description:
    This routine gets information about a selector in the ldt, and returns it to the caller.
Arguments:
...
Note:
    ...
    There are a number of issues to clear up with respect to selectors and the kernel, and after they have been cleared up, this code will be moved to its correct place
--*/
{
    ...
    *Flags = 0;
    ...
    Ldt = (PLDT_ENTRY)((Process->LdtDescriptor.BaseLow) | (Process->LdtDescriptor.HighWord.Bytes.BaseMid << 16) | (Process->LdtDescriptor.HighWord.Bytes.BaseHi << 24));
    ...
    // Under normal circumstances, we will only execute the following loop once.
    // If there is a bug in the user mode wow code however, the LDT may change while we execute the following code.
    // We don't want to take the Ldt mutex, because that is expensive.
    do {
        RetryCount++;
        ...
        try {
            if (!Ldt[Selector/sizeof(LDT_ENTRY)].HighWord.Bits.Pres) {
                ...
                *Base = (Ldt[Selector/sizeof(LDT_ENTRY)].BaseLow | (Ldt[Selector/sizeof(LDT_ENTRY)].HighWord.Bytes.BaseMid << 16) | (Ldt[Selector/sizeof(LDT_ENTRY)].HighWord.Bytes.BaseHi << 24));
                *Limit = (Ldt[Selector/sizeof(LDT_ENTRY)].LimitLow | (Ldt[Selector/sizeof(LDT_ENTRY)].HighWord.Bits.LimitHi << 16));
                *Flags = 0;
...
/*++
Routine Description:
    This routine sets up the Event info for an IO event, and causes the event to be reflected to the Monitor.
    It is assumed that interrupts are enabled upon entry, and Irql is at APC level.
Arguments:
...
--*/
{
    ...
    if (Success) {
        ...
        try {
            VdmTib = ((PVDM_PROCESS_OBJECTS)PsGetCurrentProcess()->VdmObjects)->VdmTib;
...
BOOLEAN Ki386VdmDispatchStringIo(
    IN ULONG PortNumber,
    IN ULONG Size,
    IN BOOLEAN Rep,
    IN BOOLEAN Read,
...
/*++
Routine Description:
    This routine sets up the Event info for a string IO event, and causes the event to be reflected to the Monitor.
    It is assumed that interrupts are enabled upon entry, and Irql is at APC level.
Arguments:
    PortNumber -- Supplies the port number the IO was done to
    Size -- Supplies the size of the IO operation.
...
/*++
Routine Description:
     This routine calls the handler for the IO.
     If there is not a handler of the proper size, it will call this function for 2 io's to the next smaller size.
     If the size was a byte, and there was no handler, FALSE is returned.
Arguments:
...
--*/
{
    ...
    case 1:
        if (VdmIoHandler->IoFunctions[FnIndex].UcharIo[PortNumber % 4]) {
            Status = (*(VdmIoHandler->IoFunctions[FnIndex].UcharIo[PortNumber % 4]))(Context, PortNumber, AccessType, (PUCHAR)Data);
    ...
    case 2:
        if (VdmIoHandler->IoFunctions[FnIndex].UshortIo[PortNumber % 2]) {
            Status = (*(VdmIoHandler->IoFunctions[FnIndex].UshortIo[PortNumber % 2]))(Context, PortNumber, AccessType, (PUSHORT)Data);
            if (NT_SUCCESS(Status)) {
                return TRUE;
            }
        } else {
            ...
            Success2 = VdmDispatchIoToHandler(VdmIoHandler, Context, PortNumber + 1, Size / 2, Read, (PULONG)((PUCHAR)Data + 1));
    ...
    case 4:
        if (VdmIoHandler->IoFunctions[FnIndex].UlongIo) {
            ...
            Success1 = VdmDispatchIoToHandler(VdmIoHandler, Context, PortNumber, Size /2, Read, Data);
            Success2 = VdmDispatchIoToHandler(VdmIoHandler, Context, PortNumber + 2, Size / 2, Read, (PULONG)((PUSHORT)Data + 1));
            return (Success1 || Success2);
        }
...
BOOLEAN VdmDispatchUnalignedIoToHandler(
    IN PVDM_IO_HANDLER VdmIoHandler,
    IN ULONG Context,
    IN ULONG PortNumber,
    IN ULONG Size,
...
/*++
Routine Description:
     This routine converts the unaligned IO to the necessary number of aligned IOs to smaller ports.
Arguments:
    VdmIoHandler -- Supplies a pointer to the handler table
    Context -- Supplies 32 bits of data set when the port was trapped
    PortNumber -- Supplies the port number the IO was done to
...
--*/
{
    ...
    if (Size == 4) {
        Success |= VdmDispatchIoToHandler(VdmIoHandler, Context, PortNumber + Offset, 2, Read, (PULONG)((PUCHAR)Data + Offset));
        Offset += 2;
    }

    // If we haven't dispatched the entire port, dispatch the final uchar
    if (Offset != 4) {
        Success |= VdmDispatchIoToHandler(VdmIoHandler, Context, PortNumber + Offset, 1, Read, (PULONG)((PUCHAR)Data + Offset));
    }
...
BOOLEAN VdmDispatchStringIoToHandler(
    IN PVDM_IO_HANDLER VdmIoHandler,
    IN ULONG Context,
    IN ULONG PortNumber,
    IN ULONG Size,
...
Routine Description:
     This routine calls the handler for the IO.
     If there is not a handler of the proper size, or the io is not aligned, it will simulate the io to the normal io handlers.
Arguments:
    VdmIoHandler -- Supplies a pointer to the handler table
    Context -- Supplies 32 bits of data set when the port was trapped
    PortNumber -- Supplies the port number the IO was done to
...
--*/
{
    ...
    switch (Size) {
    case 1:
        Success = VdmCallStringIoHandler(VdmIoHandler, (PVOID)VdmIoHandler->IoFunctions[FnIndex].UcharStringIo[PortNumber % 4], Context, PortNumber, Size, Count, Read, Data);
        break;
    case 2:
        Success = VdmCallStringIoHandler(VdmIoHandler, (PVOID)VdmIoHandler->IoFunctions[FnIndex].UshortStringIo[PortNumber % 2], Context, PortNumber, Size, Count, Read, Data);
        break;
    case 4:
        Success = VdmCallStringIoHandler(VdmIoHandler, (PVOID)VdmIoHandler->IoFunctions[FnIndex].UlongStringIo, Context, PortNumber, Size, Count, Read, Data);
        break;
    }
...
BOOLEAN VdmCallStringIoHandler(
    IN PVDM_IO_HANDLER VdmIoHandler,
...
/*++
Routine Description:
    This routine actually performs the call to string io routine.
    It takes care of buffering the user data in kernel space so that the device driver does not have to.
    If there is not a string io function, or the io is misaligned, it will be simulated as a series of normal io operations
Arguments:
...
--*/
{
    ...
    try {
        while (BytesDone < TotalBytes) {
            ...
            if (StringIoRoutine) {
                // in order to avoid having 3 separate calls, one for each size we simply cast the parameters appropriately for the byte routine.
                Status = (*((PDRIVER_IO_PORT_UCHAR_STRING)StringIoRoutine))(Context, PortNumber, AccessType, VdmStringIoBuffer, NumberIo);
                if (NT_SUCCESS(Status)) {
                    Success |= TRUE;
                }
            } else {
                if (PortNumber % Size) {
                    for (LoopCount = 0; LoopCount < NumberIo; LoopCount++ ) {
                        Success |= VdmDispatchUnalignedIoToHandler(VdmIoHandler, Context, PortNumber, Size, Read, (PULONG)(VdmStringIoBuffer + LoopCount * Size));
                    }
                } else {
                    for (LoopCount = 0; LoopCount < NumberIo; LoopCount++ ) {
                        Success |= VdmDispatchIoToHandler(VdmIoHandler, Context, PortNumber, Size, Read, (PULONG)(VdmStringIoBuffer + LoopCount * Size));
                    }
                }
            }
...
/*++
Routine Description:
    This routine converts the specified segmented address into a linear address, based on processor mode in user mode.
Arguments:
    SegmentedAddress -- Supplies the segmented address to convert.
    LinearAddress -- Supplies a pointer to the destination for the coresponding linear address
Return Value:
...
--*/
{
    ...
    if (!NT_SUCCESS(Status)) {
        // If we have the extensions, set the appropriate bits in cr4
    ...
    // If we have V86 mode int extensions, we don't want to run with IOPL in v86 mode
    if (!KeI386VirtualIntExtensions & V86_VIRTUAL_INT_EXTENSIONS) {
        ...
        if (NT_SUCCESS(Status)) {
            // KeEflagsAndMaskV86 and KeEflagsOrMaskV86 are used in SANITIZE_FLAGS, and the Vdm code to make sure the values in EFlags for v86 mode trap frames are acceptable
            KeI386EFlagsAndMaskV86 = EFLAGS_USER_SANITIZE | EFLAGS_INTERRUPT_MASK;
            KeI386EFlagsOrMaskV86 = EFLAGS_IOPL_MASK;

            // KeVdmIoplAllowed is used by the Vdm code to determine if the virtual interrupt flag is in EFlags, or 40:xx
    ...
    //  Initialize the address of the Vdm communications area based on machine type because of non-AT Japanese PCs.
    //  Note that we only have to change the op-code for PC-98 machines as the default is the PC/AT value.
...
BOOLEAN Ke386VdmInsertQueueApc (
    IN PKAPC             Apc,
    IN PKTHREAD          Thread,
...
/*++
Routine Description:
    This function initializes, and queues a vdm type of APC to the specified thread.
    A Vdm type of APC:
       - OriginalApcEnvironment
       - will only be queued to one thread at a time
       - if UserMode Fires on the next system exit. A UserMode apc should not be queued if the current vdm context is not application mode.
Arguments:
    ...
    NormalRoutine - Supplies an optional pointer to a function that is to be executed at IRQL 0 in the specified processor mode.
        If this parameter is not specified, then the ProcessorMode and NormalContext parameters are ignored.
...
--*/
{
    ...
    // If the apc object not initialized, then initialize it and acquire the target thread APC queue lock.
    if (Apc->Type != ApcObject) {
        ...
        // If the APC is inserted in the corresponding APC queue, and the APC thread is not the same thread as the target thread, then the APC is removed from its current queue,
        // the APC pending state is updated, the APC thread APC queue lock is released, and the target thread APC queue lock is acquired.
        // Otherwise, the APC thread and the target thread are same thread and the APC is already queued to the correct thread.

        // If the APC is not inserted in an APC queue, then release the APC thread APC queue lock and acquire the target thread APC queue lock.
    ...
    KiReleaseSpinLock(&Thread->ApcQueueLock);// Unlock the target thread APC queue.
    ...
    if (Thread->ApcQueueable && KiInsertQueueApc(Apc, Increment)) {
        ...
        // If UserMode:
        //    For vdm a UserMode Apc is only queued by a kernel mode apc which is on the current thread for the target thread.
        //    Force UserApcPending for User mode apcstate, so that the apc will fire when this thread exits the kernel.
        if (ApcMode == UserMode) {
    ...
    // Unlock the dispatcher database, lower IRQL to its previous value, and return whether the APC object was inserted.
    KiUnlockDispatcherDatabase(OldIrql);
    return Inserted;
}
...
/*++
Routine Description:
    Clears a VDM APC object, synchronously with Ke386VdmInsertQueueApc, and is expected to be called by one of the vdm kernel apc routine or the rundown routine.
Arguments:
    Apc - Supplies a pointer to a control object of type APC.
--*/
{
...
#if VDM_IO_TEST
NTSTATUS TestIoByteRoutine(IN ULONG Port, IN UCHAR AccessMode, IN OUT PUCHAR Data)
{
...
NTSTATUS TestIoWordReadRoutine(IN ULONG Port, IN UCHAR AccessMode, IN OUT PUSHORT Data)
{
    if (AccessMode & EMULATOR_READ_ACCESS) {
        *Data = Port - 200;
    }
...
NTSTATUS TestIoWordWriteRoutine(IN ULONG Port, IN UCHAR AccessMode, IN OUT PUSHORT Data)
{
...
NTSTATUS TestIoDwordRoutine(IN ULONG Port, IN USHORT AccessMode, IN OUT PULONG Data)
{
    if (AccessMode & EMULATOR_READ_ACCESS) {
        *Data = Port;
    }
...
NTSTATUS TestIoStringRoutine(IN ULONG Port, IN USHORT AccessMode, IN OUT PSHORT Data, IN ULONG Count)
{
    ULONG i;
...
VOID TestIoHandlerStuff(VOID)
{
    ...
    Entry[3].Routine = TestIoStringRoutine;
     ...
     if (Connect) {
        Status = ZwSetInformationProcess(NtCurrentProcess(), ProcessIoPortHandlers, &IoPortHandler, sizeof(PROCESS_IO_PORT_HANDLER_INFORMATION)) ;
        if (!NT_SUCCESS(Status)) {
            DbgBreakPoint();
        }
        Connect = FALSE;
