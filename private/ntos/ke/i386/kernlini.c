...
Abstract:
    This module contains the code to initialize the kernel data structures and to initialize the idle thread, its process, and the processor control block.
...
// If this processor does XMMI, take advantage of it.  Default is no XMMI.
...
// The following spinlock is for compatiblity with 486 systems that don't have a cmpxchg8b instruction and therefore need to synchronize using a
...
Routine Description:
    ...
    also called to initialize the processor specific structures when a new processor is brought on line.
Arguments:
    Process - Supplies a pointer to a control object of type process for the specified processor.
    Thread - Supplies a pointer to a dispatcher object of type thread for the specified processor.
    IdleStack - Supplies a pointer the base of the real kernel stack for idle thread on the specified processor.
    Prcb - Supplies a pointer to a processor control block for the specified processor.
    Number - Supplies the number of the processor that is being initialized.
...
--*/
{
    ...
    // If the initial processor is being initialized, then initialize the per system data structures.
    if (Number == 0) {
...
#if !defined(NT_UP)
        // During Text Mode setup, it is possible the system is running with an MP kernel and a UP HAL.  On X86 systems,
        // spinlocks are implemented in both the kernel and the HAL with the verisons that alter IRQL in the HAL.   If the
        // HAL is UP, it will not actually acquire/release locks while the MP kernel will which will cause the system to
        // hang (or crash).   As this can only occur during text mode setup, we will detect the situation and disable
        // the kernel only versions of queued spinlocks if the HAL is UP (and the kernel MP).

        // We need to patch 3 routines, two of them are void and the other returns a boolean (must be true (and ZF must be clear) in a UP case).

        // Determine if the HAL us UP by acquiring the dispatcher lock and examining it to see if the HAL actually did anything to it.
        OldIrql = KfAcquireSpinLock(&KiDispatcherLock);
        if (KiDispatcherLock == 0) {
            // KfAcquireSpinLock is in the HAL and it did not change the value of the lock.  This is a UP HAL.
            ...
            *(PUCHAR)(KiReleaseQueuedSpinLock) = RET;
            ...
            // Copy the UP version of KiTryToAcquireQueuedSpinLock over the top of the MP versin.
    ...
    SharedUserData->ProcessorFeatures[PF_XMMI_INSTRUCTIONS_AVAILABLE] = ((KeFeatureBits & KF_FXSR) && (KeFeatureBits & KF_XMMI)) ? TRUE : FALSE;
    ...
    //      6. the specified processor member in the process active processors set.
    KeInitializeThread(Thread, (PVOID)((ULONG)IdleStack), (PKSYSTEM_ROUTINE)NULL, (PKSTART_ROUTINE)NULL, (PVOID)NULL, (PCONTEXT)NULL, (PVOID)NULL, Process);
    ...
    // Initialize the processor block. (Note that some fields have been initialized at KiInitializePcr().
    ...
    // If the initial processor is being initialized, then compute the timer table reciprocal value and reset the PRCB values for the controllable DPC behavior in order to reflect any registry overrides.
    if (Number == 0) {
    ...
    // Set the priority of the specified idle thread to zero, set appropriate member in KiIdleSummary and return to the system start up routine.
    KeRaiseIrql(DISPATCH_LEVEL, &OldIrql);
    KeSetPriorityThread(Thread, (KPRIORITY)0);

    // if a thread has not been selected to run on the current processors, check to see if there are any ready threads; otherwise add this processors to the IdleSummary
...
/*++
Routine Description:
    This function is called to initialize the PCR for a processor.
    It simply stuffs values into the PCR.  (The PCR is not inited statically because the number varies with the number of processors.)
...
--*/
{
...
#if defined (_X86PAE_)
    if (Processor == 0) {
        SharedUserData->ProcessorFeatures[PF_PAE_ENABLED] = TRUE;//  PAE feature must be initialized prior to the first HAL call.
    }
#endif
    ...
    // Init Prcb.Number and ProcessorBlock such that Ipi will work as early as possible.
...
/*++
Routine Description:
    This function is called to initialize the double-fault TSS for a processor.
    It will set the static fields of the TSS to point to the double-fault handler and the appropriate double-fault stack.

    Note that the IOPM for the double-fault TSS grants access to all ports.
    This is so the standard HAL's V86-mode callback to reset the display to text mode will work.
Arguments:
...
--*/
{
    ...
    //      value is used, byte immediate in's and out's will actually go the hardware when executed in V86 mode.
...
Routine Description:
    ...
    It will set the static fields of the TSS.  (ie Those fields that the part reads, and for which NT uses constant values.)

    The dynamic fields (Esp0 and CR3) are set in the context swap code.
Arguments:
...
/*++
Routine Description:
    This function is called to edit the IDT.
    It swaps words of the address and access fields around into the format the part actually needs.
...
/*++
Routine Description:
    (Try to) Determine the manufacturer of this processor based on data returned by the CPUID instruction (if present).
Arguments:
    None.
Return Value:
...
--*/
{
    ...
    // Copy vendor string to Prcb for debugging (ensure it's NULL terminated).
...
/*++
Routine Description:
    Examine the processor specific feature bits to determine the Windows 2000 supported features supported by this processor.
Arguments:
    None.
Return Value:
...
--*/
{
    ...
    // If this processor does not support the CPUID instruction, don't try to use it.
    ...
    if (CpuVendor == CPU_AMD) {
        ...
        if ((ProcessorSignature & 0x0F00) >= 0x0500) {
            ...
            // Less than family 5, don't set GLOBAL PAGE, LARGE PAGE or CMOV.  (greater than family 5 will have the bits set correctly).
            ProcessorFeatures &= ~(0x08 | 0x2000 | 0x8000);

            // We don't know what this processor returns if we probe for extended CPUID support.
    ...
    if (CpuVendor == CPU_INTEL) {
        ...
        if ( ((ProcessorSignature & 0x0FF0) == 0x0610 && (ProcessorSignature & 0x000F) <= 0x9) || ((ProcessorSignature & 0x0FF0) == 0x0630 && (ProcessorSignature & 0x000F) <= 0x4)) {
            NtBits &= ~KF_WORKING_PTE;
        }

        if ( (Prcb->CpuType == 6)  && (Prcb->CpuStep >= 0x0303)  && (ProcessorFeatures & KI_FAST_SYSCALL_SUPPORTED) ) {
    ...
    if (CpuVendor == CPU_CYRIX) {
        // Workaround bug 324467 which is caused by INTR being held high too long during an FP instruction and causing random Trap07 with no exception bits.
        extern BOOLEAN KiIgnoreUnexpectedTrap07;

        KiIgnoreUnexpectedTrap07 = TRUE;

        // Workaround CMPXCHG bug to Cyrix processors where Family = 6, Model = 0, Stepping <= 1.  Note that
    ...
    // N.B. CMPXCHG8B MUST be done in a generic manner or clone processors will not be able to boot if they set this feature bit.
    ...
    // Check extended functions.   First, check for existance, then check extended function 0x80000001 (Extended Processor Features) if present.

    // Note: Intel guarantees that no processor that doesn't support extended CPUID functions will ever return a value with the
    // most significant bit set.   Microsoft asks all CPU vendors to make the same guarantee.
    if (ExtendedCPUIDSupport != FALSE) {
        CPUID(0x80000000, &Junk2, &Junk, &Junk, &Junk);

        // Sanity check the result, assuming there are no more than 256 extended feature functions (should be valid for a little while).
        if ((Junk2 & 0xffffff00) == 0x80000000) {
            // Check extended processor features.  These, by definition, can vary on a processor by processor basis.
            if (Junk2 >= 0x80000001) {
...
#define CPUID_REG_COUNT 4
    ...
    // Obtain Cache size information for those processors on which we know how.
    switch (CpuVendor) {
    case CPU_INTEL:
        CPUID(0, CpuidData, CpuidData+1, CpuidData+2, CpuidData+3);

        // Check this processor supports CPUID function 2 which is the one that returns cache size info.
        if (CpuidData[0] >= 2) {
            // The above returns a series of bytes.    (In EAX, EBX, ECX and EDX).
            // The least significant byte (of EAX) gives the number of times CPUID(2 ...) should be issued to return the complete set of data.
            // The bytes are self describing data.

            // In particular, the bytes describing the L2 cache size will be in the following set (and meaning)
            ...
            // I am extrapolating the above as anything in the range 0x41 thru 0x4f can be computed as

            //   128KB << (descriptor - 0x41)

            // The Intel folks say keep it to a reasonable upper bound, eg 49.

            // N.B. the range 0x80 .. 0x86 indicates the same cache sizes but 8 way associative.

            // Also, the most significant bit of each register indicates whether not the register contains valid information.
            // 0 == Valid, 1 == InValid.
            ...
            do {
                ...
                if (FirstPass) {
                    // Get the iteration count from the first byte of the returned data then replace that byte with 0 (a null descriptor).
                ...
                for (i = 0; i < CPUID_REG_COUNT; i++) {
                    ...
                    if (CpuidReg & 0x80000000) {
                        // Register doesn't contain valid data, skip it.
                        continue;
                    }

                    while (CpuidReg) {
                        // Get LS Byte from this DWORD and remove the byte.
                        ...
                        if (((Descriptor > 0x40) && (Descriptor <= 0x49)) || ((Descriptor > 0x80) && (Descriptor <= 0x89))) {
                            // L2 descriptor.
                            Descriptor &= 0x0f;

                            // Assert the descriptor is in the range we officially know about.   If this hits on
                            // a checked build, check with Intel about the interpretation.
                ...
                // Note: Always run thru all iterations indicated by the first to ensure a subsequent call won't start part way thru.
...
BOOLEAN KiInitMachineDependent (VOID)
{
...
#if !defined(NT_UP)
    // If some processor doesn't have proper MP PTE implementation, then use a synchronous TB shoot down handler
    ...
    // If PAT or MTRR supported but the HAL indicates it shouldn't be used (eg on a Shared Memory Cluster), drop the feature.
    if (KeFeatureBits & (KF_PAT | KF_MTRR)) {
        NTSTATUS Status;
        BOOLEAN  UseFrameBufferCaching;
        ULONG    Size;

        Status = HalQuerySystemInformation(HalFrameBufferCachingInformation, sizeof(UseFrameBufferCaching), &UseFrameBufferCaching, &Size);
    ...
    for (CurrentAffinity=1; ActiveProcessors; CurrentAffinity <<= 1) {
        if (ActiveProcessors & CurrentAffinity) {
            // Switch to that processor, and remove it from the remaining set of processors
            ...
            if (KeFeatureBits & KF_RDTSC) {
                ...
                for (; ;) {
                    ...
                    // Delay the thread a "long" amount and time it with a time source and RDTSC.
                    ...
                    pSamp->MHz = (ULONG) ((pSamp->TSCDelta * pSamp->PerfFreq.QuadPart + 500000L) / (pSamp->PerfDelta * 1000000L));

                    // If last 2 samples matched within a MHz, done
                    if (Index) {
                        if (pSamp->MHz == pSamp[-1].MHz || pSamp->MHz == pSamp[-1].MHz + 1 || pSamp->MHz == pSamp[-1].MHz - 1) {
                                break;
                        }
                    }
...
#if DBG
                        // Temp breakpoint to see where this is failing and why
                        DbgBreakPoint();
#endif
            ...
            // If MTRRs are supported and PAT not supported, initialize MTRRs per processor
            if (!(KeFeatureBits & KF_PAT) && (KeFeatureBits & KF_MTRR)) {
                KiInitializeMTRR ( (BOOLEAN) (ActiveProcessors ? FALSE : TRUE));
            }

            // If the processor is a AMD K6 with MTRR support then perform processor specific initialization.
...
Routine Description:
    ...
    If this is a 386 system without 387s (all processors must be symmetrical) then this function will set the trap 07 vector on all
    processors to point to the address passed in (which should be the entry point of the 80387 emulator in NTDLL, NPXNPHandler).
Arguments:
...
em20:
    ...
    if (!KeI386NpxPresent) {
        ...
        SharedUserData->ProcessorFeatures[PF_FLOATING_POINT_PRECISION_ERRATA] = FALSE;// Errata not present when using emulator
        ...
        for (CurrentAffinity = 1; ActiveProcessors; CurrentAffinity <<= 1) {
            if (ActiveProcessors & CurrentAffinity) {
                ...
                KeSetSystemAffinityThread(CurrentAffinity);// Run this code on each processor.
                KiLockDispatcherDatabase(&OldIrql);// Raise IRQL and lock dispatcher database.
        ...
        // Move any entries from ..\System\FloatingPointProcessor to ..\System\DisabledFloatingPointProcessor.
        ...
        Status = ZwOpenKey( &SystemHandle, KEY_ALL_ACCESS, &ObjectAttributes);// Open system tree
        if (NT_SUCCESS(Status)) {
            InitializeObjectAttributes(&ObjectAttributes, &CmTypeName[FloatingPointProcessor], OBJ_CASE_INSENSITIVE, SystemHandle, NULL);
            Status = ZwOpenKey ( &SourceHandle, KEY_ALL_ACCESS, &ObjectAttributes);// Open FloatingPointProcessor key
            if (NT_SUCCESS(Status)) {// Create DisabledFloatingPointProcessor key
                ...
                Status = ZwCreateKey( &DestHandle, KEY_ALL_ACCESS, &ObjectAttributes, 0, NULL, REG_OPTION_VOLATILE, &disposition);
                if (NT_SUCCESS(Status)) {// Move it
    ...
    KeRevertToUserAffinityThread();// Set affinity back to the original value.
}
...
NTSTATUS KiMoveRegTree(HANDLE  Source, HANDLE  Dest)
{
    ...
    for (; ;) {
        ...
        ZwSetValueKey( Dest, &ValueName, KeyValue->TitleIndex, KeyValue->Type, buffer+KeyValue->DataOffset, KeyValue->DataLength);
    ...
    for (; ;) {
        ...
        // Loop and get first key.  (old first key was delete by the call to KiMoveRegTree).
    }

    return NtDeleteKey (Source);// Remove source node
}
...
/*++
Routine Description:
    This routine is called once on every processor when KiI386PentiumLockErrataPresent is TRUE.

    This routine replaces the local IDT with an IDT that has the first 7 IDT entries on their own page and returns the first page to the caller to be marked as read-only.
    This causes the processor to trap-0e fault when the errata occurs.
    Special code in the trap-0e handler detects the problem and performs the proper fixup.
Arguments:
...
#define IDT_SKIP   (7 * sizeof (KIDTENTRY))
    BasePage = MmAllocateIndependentPages (2*PAGE_SIZE);// Allocate memory for a new copy of the processors IDT

    // The IDT base is such that the first 7 entries are on the first (read-only) page, and the remaining entries are on the second (read-write) page
    NewBase = BasePage + PAGE_SIZE - IDT_SKIP;

    Enable = KiDisableInterrupts();// Disable interrupts on this processor while updating the IDT base
    ...
    RtlMoveMemory ((PVOID) NewBase, (PVOID) IdtDescriptor.Base, IdtDescriptor.Limit + 1);
    ...
    KeGetPcr()->IDT = (PKIDTENTRY) NewBase;// Update the PCR
    KiRestoreInterrupts(Enable);// Restore interrupts
