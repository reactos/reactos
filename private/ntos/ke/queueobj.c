...
Abstract:
    This module implements the kernel queue object.
    Functions are provided to initialize, read, insert, and remove queue objects.
...
// The following assert macro is used to check that an input event is really a kernel event and not something else, like deallocated pool.
...
Arguments:
    ...
    Count - Supplies the target maximum number of threads that should be concurrently active. If this parameter is not specified, then the number of processors is used.
--*/
{
    // Initialize standard dispatcher object header and set initial state of queue object.
    ...
    // Initialize queue listhead, the thread list head, the current number of threads, and the target maximum number of threads.
...
--*/
{
    ...
    return Queue->Header.SignalState;// Return current signal state of Queue object.
}
...
/*++
Routine Description:
    This function inserts the specified entry in the queue object entry list and attempts to satisfy the wait of a single waiter.
    N.B. The wait discipline for Queue object is LIFO.
Arguments:
    Queue - Supplies a pointer to a dispatcher object of type Queue.
    Entry - Supplies a pointer to a list entry that is inserted in the queue object entry list.
Return Value:
...
--*/
{
    ...
    OldState = KiInsertQueue(Queue, Entry, FALSE);// Insert the specified entry in the queue object entry list.
    KiUnlockDispatcherDatabase(OldIrql);// Unlock the dispather database, lower IRQL to the previous level, and return signal state of Queue object.
    return OldState;
}
