...
Abstract:
   This module implements the initialization, open, duplicate and other services of the executive token object.
...
/*++
Routine Description:
    This function returns the type of an instance of a token (TokenPrimary, or TokenImpersonation).
Arguments:
...
/*++
Routine Description:
    This function returns the impersonation level of a token.  The token is assumed to be a TokenImpersonation type token.
Arguments:
...
Routine Description:
    ...
    Communication session layers will need to check this, for some security quality of service modes, to determine whether or not a server's security context needs to be updated to reflect
    changes in the client's security context.

    This routine will also be useful to communications subsystems that need to retrieve client' authentication information from the local security authority in order to perform a remote authentication.
Parameters:
    Token - Points to the token whose information is to be retrieved.
    TokenControl - Points to the buffer to receive the token control information.
--*/
{
    PAGED_CODE();
...
Routine Description:
    ...
    A system token has the following characteristics:
         ...
         - It has protection that provides TOKEN_ALL_ACCESS to the LOCAL_SYSTEM ID.
         - It has a default ACL that grants GENERIC_ALL access to LOCAL_SYSTEM and GENERIC_EXECUTE to WORLD.
Parameters:
    None.
Return Value:
...
//    GroupIds = (PSID_AND_ATTRIBUTES)ExAllocatePool( NonPagedPool, 512 );
    ...
    OwnerGroupAttributes  =    (SE_GROUP_ENABLED_BY_DEFAULT | SE_GROUP_ENABLED            | SE_GROUP_OWNER);
    ...
    Length = (ULONG)sizeof(ACL) + ((ULONG)sizeof(ACCESS_ALLOWED_ACE) - sizeof(ULONG)) + SeLengthSid( SeLocalSystemSid ) ;
    ...
    Status = RtlAddAccessAllowedAce (TokenAcl, ACL_REVISION2, TOKEN_ALL_ACCESS, SeLocalSystemSid);
    ASSERT( NT_SUCCESS(Status) );

    TokenSecurityDescriptor = (PSECURITY_DESCRIPTOR)ExAllocatePoolWithTag(PagedPool, sizeof(SECURITY_DESCRIPTOR), 'dSeS');
    if ( TokenSecurityDescriptor == NULL ) {
        ExFreePool( TokenAcl );
        return NULL ;
    }

    Status = RtlCreateSecurityDescriptor(TokenSecurityDescriptor, SECURITY_DESCRIPTOR_REVISION);
    ASSERT( NT_SUCCESS(Status) );

    Status = RtlSetDaclSecurityDescriptor (TokenSecurityDescriptor, TRUE, TokenAcl, FALSE);
    ASSERT( NT_SUCCESS(Status) );
...
// Debug
    ...
    InitializeObjectAttributes(&TokenObjectAttributes, NULL, 0, NULL, TokenSecurityDescriptor);
    ...
    BufferLength = Length + sizeof(SECURITY_DESCRIPTOR_RELATIVE) + 2 * SeLengthSid(SeAliasAdminsSid);

    Buffer =  (PSECURITY_DESCRIPTOR)ExAllocatePoolWithTag( PagedPool, BufferLength, 'dSeS');
    if ( Buffer ) {
        Status =  RtlAbsoluteToSelfRelativeSD( TokenSecurityDescriptor, Buffer, &BufferLength);
        ASSERT(NT_SUCCESS(Status));
        Status =  ObAssignObjectSecurityDescriptor( Token, Buffer, PagedPool);
        ASSERT(NT_SUCCESS(Status));
    } else {
...
Routine Description:
    ...
    A system token has the following characteristics:
         ...
         - It has protection that provides TOKEN_ALL_ACCESS to the WORLD ID.
         - It has a default ACL that grants GENERIC_ALL access to WORLD.
Parameters:
    None.
Return Value:
    Pointer to a system token.
--*/
{
    ...
    GroupIdsLength = (ULONG)LongAlignSize(SeLengthSid(GroupIds->Sid)) + sizeof(SID_AND_ATTRIBUTES);
    ...
    // give system full reign of terror.  This includes user-mode components
    // running as part of the system.
    Length = (ULONG)sizeof(ACL) + (ULONG)sizeof(ACCESS_ALLOWED_ACE) + SeLengthSid( SeWorldSid ) + 8; // The 8 is just for good measure
    ASSERT( Length < 200 );
    ...
    Status = RtlAddAccessAllowedAce (TokenAcl, ACL_REVISION2, TOKEN_ALL_ACCESS, SeWorldSid);
    ASSERT( NT_SUCCESS(Status) );

    TokenSecurityDescriptor = (PSECURITY_DESCRIPTOR)ExAllocatePoolWithTag(PagedPool, SECURITY_DESCRIPTOR_MIN_LENGTH, 'dSeS');
...
// Debug
    ...
    BufferLength = Length + sizeof(SECURITY_DESCRIPTOR) + 2 * SeLengthSid(SeAliasAdminsSid);
    Buffer =  (PSECURITY_DESCRIPTOR)ExAllocatePoolWithTag( PagedPool, BufferLength, 'dSeS');
    if ( Buffer ) {
        Status =  RtlAbsoluteToSelfRelativeSD( TokenSecurityDescriptor, Buffer, &BufferLength);
        ASSERT(NT_SUCCESS(Status));
