...
/*++
Routine Description:
    Examines the audit queue high and low water mark values and performs a general sanity check on them.
Arguments:
...
/*++
Routine Description:
    This function performs Phase 1 Initialization for the Auditing subcomponent of Security.
    Global variables are initialized within the Nt Executive and Auditing is turned off.
Return Value:
...
/*++
Routine Description:
    Queries the registry for the high and low water mark values for the audit log.
    If they are not found or are unacceptable, returns without modifying the current values, which are statically initialized.
--*/
{
    HANDLE KeyHandle;
    ...
    Status = NtQueryValueKey(KeyHandle, &ValueName, KeyValuePartialInformation, (PVOID)KeyValueInformation, Length, &Length);
    NtClose( KeyHandle );
    if (!NT_SUCCESS( Status )) {
...
--*/
{
    ...
    Status = NtQueryValueKey(KeyHandle, &ValueName, KeyValuePartialInformation, KeyInfo, sizeof(KeyInfo), &ResultLength);
...
/*++
Routine Description:
    Checks to see if there is an entry in the registry telling us to do full privilege auditing (which currently means audit everything we normall audit, plus backup and restore privileges).
Return Value:
    BOOLEAN - TRUE if Auditing has been initialized correctly, else FALSE.
--*/
{
