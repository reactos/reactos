...
NTSTATUS SepAccessCheckAndAuditAlarm (
...
// This routine is to be called to do simple checks of single privileges against the passed token.

// DO NOT CALL THIS TO CHECK FOR SeTcbPrivilege SINCE THAT MUST BE CHECKED AGAINST THE PRIMARY TOKEN ONLY!
...
--*/
{
   ...
   ASSERT(!((DesiredPrivilege.LowPart == SeTcbPrivilege.LowPart) && (DesiredPrivilege.HighPart == SeTcbPrivilege.HighPart)));
...
/*++
Routine Description:
    This routine specifically searches the primary token (rather than the effective token) of the calling process for SeAuditPrivilege.
    In order to do this it must call the underlying worker SepPrivilegeCheck directly, to ensure that the correct token is searched
Arguments:
...
Routine Description:
    ...
    This routine may fail due to lack of memory, in which case, it will return a NULL pointer in the output parameter.
Arguments:
    SourceString - Pointer to a Unicode string to be captured.
    DestString - Returns a pointer to a captured Unicode string.  This will be one contiguous structure, and thus may be freed by a single call to ExFreePool().
--*/
{
    ...
    // Initialize the object name descriptor and capture the specified name string.
    *DestString = NULL;

    Status = STATUS_SUCCESS;
    try {
        // Probe and capture the name string descriptor and probe the name string, if necessary.
        InputString = ProbeAndReadUnicodeString(SourceString);
        ProbeForRead(InputString.Buffer, InputString.Length, sizeof(WCHAR));

        // If the length of the string is not an even multiple of the size of a UNICODE character or cannot be zero terminated, then return an error.
...
Arguments:
    CapturedString - Supplies a pointer to a string previously captured by SepProbeAndCaptureString.
--*/
{
    PAGED_CODE();
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when an attempt is made to perform privileged operations on a protected subsystem object after the object is already opened.
    This routine may result in several messages being generated and sent to Port objects.
    This may result in a significant latency before returning.
    Design of routines that must call this routine must take this potential latency into account.
    This may have an impact on the approach taken for data structure mutex locking, for example.

    This API requires the caller have SeAuditPrivilege privilege.
...
Arguments:
    ...
    ClientToken - A handle to a token object representing the client that requested the operation.
    ...
    Privileges - The set of privileges required for the requested operation.
        Those privileges that were held by the subject are marked using the UsedForAccess flag of the attributes associated with each privilege.
    AccessGranted - Indicates whether the requested access was granted or not.
        A value of TRUE indicates the access was granted.
        A value of FALSE indicates the access was not granted.
--*/
{
    ...
    // If the passed token is an impersonation token, make sure it is at SecurityIdentification or above.
...
//    }
    ...
    try {
        ...
        PrivilegeParameterLength = (ULONG)sizeof(PRIVILEGE_SET) + ((PrivilegeCount - ANYSIZE_ARRAY) * (ULONG)sizeof(LUID_AND_ATTRIBUTES)  );
    ...
    // No need to lock the token, because the only thing we're going to reference in it is the User's Sid, which cannot be changed.

    // SepPrivilegeObjectAuditAlarm will check the global flags to determine if we're supposed to be auditing here.
...
VOID SePrivilegeObjectAuditAlarm(
    IN HANDLE Handle,
...
/*++
Routine Description:
    This routine is used by object methods that perform privileged operations to generate audit and alarm messages related to the use of privileges, or attempts to use privileges.
Arguments:
    Object - Address of the object accessed.  This value will not be used as a pointer (referenced).
    It is necessary only to enter into log messages.

    Handle - Provides the handle value assigned for the open.

    SecurityDescriptor - A pointer to the security descriptor of the object being accessed.

    SubjectSecurityContext - A pointer to the captured security context of the subject attempting to open the object.

    DesiredAccess - The desired access mask.  This mask must have been previously mapped to contain no generic accesses.

    Privileges - Points to a set of privileges required for the access attempt.
    Those privileges that were held by the subject are marked using the UsedForAccess flag of the PRIVILEGE_ATTRIBUTES associated with each privilege.

    AccessGranted - Indicates whether the access was granted or denied.  A value of TRUE indicates the access was allowed.  A value of FALSE indicates the access was denied.

    AccessMode - Indicates the access mode used for the access check. Messages will not be generated by kernel mode accesses.
--*/
{
    BOOLEAN AuditPerformed;
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when an attempt is made to perform privileged system service operations.
    This routine may result in several messages being generated and sent to Port objects.
    This may result in a significant latency before returning.
    Design of routines that must call this routine must take this potential latency into account.
    This may have an impact on the approach taken for data structure mutex locking, for example.
...
Arguments:
    ...
    ServiceName - Supplies a name of the privileged subsystem service.  For example, "RESET RUNTIME LOCAL SECURITY POLICY" might be specified
        by a Local Security Authority service used to update the local security policy database.

    ClientToken - A handle to a token object representing the client that requested the operation.
        This handle must be obtained from a communication session layer, such as from an LPC Port or Local Named Pipe, to prevent possible security policy violations.

    Privileges - Points to a set of privileges required to perform the privileged operation.
        Those privileges that were held by the subject are marked using the UsedForAccess flag of the attributes associated with each privilege.
...
//    }
    ...
    try {
        ...
        // If ExAllocatePool has failed, too bad.  Carry on and do as much of the audit as we can.
        if (CapturedPrivileges != NULL) {
            RtlCopyMemory ( CapturedPrivileges,Privileges,PrivilegeParameterLength );
            CapturedPrivileges->PrivilegeCount = PrivilegeCount;
        }
    ...
    // The AuthenticationId is in the read-only part of the token, so we may reference it without having the token read-locked.
    SepAdtPrivilegedServiceAuditAlarm ( CapturedSubsystemName,CapturedServiceName,Token,SubjectSecurityContext.PrimaryToken,CapturedPrivileges,AccessGranted );
    if (CapturedSubsystemName != NULL) {
        SepFreeCapturedString ( CapturedSubsystemName );
    }
...
/*++
Routine Description:
    This routine is to be called whenever a privileged system service is attempted.
    It should be called immediately after the privilege check regardless of whether or not the test succeeds.
Arguments:
    ServiceName - Supplies the name of the privileged system service.
    SubjectSecurityContext - The subject security context representing the caller of the system service.
    Privileges - Supplies a privilge set containing the privilege(s) required for the access.
...
--*/
{
    ...
    if ( SepAdtAuditThisEvent( AuditCategoryPrivilegeUse, &AccessGranted ) && SepFilterPrivilegeAudits( Privileges )) {
        ...
        SepAdtPrivilegedServiceAuditAlarm (&SeSubsystemName,ServiceName,SubjectSecurityContext->ClientToken,SubjectSecurityContext->PrimaryToken,Privileges,AccessGranted);
    }
}
...
/*++
Routine Description:
    This system service is used to perform both an access validation and generate the corresponding audit and alarm messages.
    This service may only be used by a protected server that chooses to impersonate its client and thereby specifies the client security context implicitly.
Arguments:
    SubsystemName - Supplies a name string identifying the subsystem calling the routine.

    HandleId - A unique value that will be used to represent the client's handle to the object.
        This value is ignored (and may be re-used) if the access is denied.

    ClientToken - Supplies the client token so that the caller does not have to impersonate before making the kernel call.
    ...
    AuditType - Specifies the type of audit to be generated.  Valid values are: AuditEventObjectAccess and AuditEventDirectoryServiceAccess.

    Flags - Flags modifying the execution of the API:

        AUDIT_ALLOW_NO_PRIVILEGE - If the called does not have AuditPrivilege, the call will silently continue to check access and will generate no audit.

    ObjectTypeList - Supplies a list of GUIDs representing the object (and sub-objects) being accessed.  If no list is present, AccessCheckByType behaves identically to AccessCheck.
    ...
    ObjectCreation - A boolean flag indicated whether the access will result in a new object being created if granted.
        A value of TRUE indicates an object will be created, FALSE indicates an existing object will be opened.

    GrantedAccess - Receives a masking indicating which accesses have been granted.

    AccessStatus - Receives an indication of the success or failure of the access check.
        If access is granted, STATUS_SUCCESS is returned.
        If access is denied, a value appropriate for return to the client is returned.
        This will be STATUS_ACCESS_DENIED or, when mandatory access controls are implemented, STATUS_OBJECT_NOT_FOUND.

    GenerateOnClose - Points to a boolean that is set by the audity generation routine and must be passed to NtCloseObjectAuditAlarm when the object handle is closed.

    ReturnResultList - If true, GrantedAccess and AccessStatus are actually arrays of entries ObjectTypeListLength elements long.

Return Value:
    STATUS_SUCCESS - Indicates the call completed successfully.  In this case, ClientStatus receives the result of the access check.
    STATUS_PRIVILEGE_NOT_HELD - Indicates the caller does not have sufficient privilege to use this privileged system service.
--*/
{
    SECURITY_SUBJECT_CONTEXT SubjectSecurityContext;
    ...
    if ( ARGUMENT_PRESENT( ClientToken ) ) {
        ...
        // Save the old token so that it can be recovered before SeReleaseSubjectContext.
        OldToken = SubjectSecurityContext.ClientToken;

        // Set the impersonation token to the one that has been obtained thru ClientToken handle. This must be freed later in Cleanup.
    ...
    // SeCaptureSecurityDescriptor probes the input security descriptor, so we don't have to
    Status = SeCaptureSecurityDescriptor (SecurityDescriptor,PreviousMode,PagedPool,FALSE,&CapturedSecurityDescriptor);
    if (!NT_SUCCESS(Status) ) {
    ...
    if ( RtlpOwnerAddrSecurityDescriptor((PISECURITY_DESCRIPTOR)CapturedSecurityDescriptor) == NULL || RtlpGroupAddrSecurityDescriptor((PISECURITY_DESCRIPTOR)CapturedSecurityDescriptor) == NULL ) {
        Status = STATUS_INVALID_SECURITY_DESCR;
        goto Cleanup;
    }
    ...
    // See if anything (or everything) in the desired access can be satisfied by privileges.
    Status = SePrivilegePolicyCheck(&DesiredAccess,&PreviouslyGrantedAccess,&SubjectSecurityContext,NULL,&PrivilegeSet,PreviousMode);
    ...
    if (!NT_SUCCESS( Status )) {
        ...
        if ( ReturnResultList ) {
            ...
            LocalGrantedAccessPointer = ExAllocatePoolWithTag( PagedPool, (sizeof(ACCESS_MASK)+sizeof(NTSTATUS)) * ObjectTypeListLength, 'aGeS' );
        ...
        // If the user in the token is the owner of the object, we must automatically grant ReadControl and WriteDac access if desired.
        // If the DesiredAccess mask is empty after these bits are turned off, we don't have to do any more access checking (ref section 4, DSA ACL Arch)
    ...
    if ( !AvoidAudit ) {
        ...
        if ( GenerateSuccessAudit || GenerateFailureAudit ) {
            // Save this to a local here, so we don't have to risk accessing user memory and potentially having to exit before the audit
            if ( AccessGranted ) {
                // SAM calls NtCloseObjectAuditAlarm despite the fact that it may not have successfully opened the object, causing a spurious close audit.
                // Since no one should rely on this anyway if their access attempt failed, make sure it's false and SAM will work properly.
            ...
            // We didn't generate an audit due to the SACL.  If privileges were used, we need to audit that.
...
Cleanup:
    if ( TokenSwapped ) {
        ObDereferenceObject( (PVOID)NewToken );// Decrement the reference count for the ClientToken that was passed in.
        SubjectSecurityContext.ClientToken = OldToken;// Reset the value of the token from saved value.
    }
...
/*++
    Routine Description:
    This routine is used to generate audit and alarm messages when an attempt is made to access an existing protected subsystem object or create a new one.
    This routine may result in several messages being generated and sent to Port objects.
    This may result in a significant latency before returning.
    Design of routines that must call this routine must take this potential latency into account.
    This may have an impact on the approach taken for data structure mutex locking, for example.
    ...
    This API requires the caller have SeAuditPrivilege privilege.
    The test for this privilege is always against the primary token of the calling process, not the impersonation token of the thread.
...
Arguments:
    ...
    SecurityDescriptor - An optional pointer to the security descriptor of the object being accessed.
    ...
    Privileges - Optionally points to a set of privileges that were required for the access attempt.
        Those privileges that were held by the subject are marked using the UsedForAccess flag of the attributes associated with each privilege.

    ObjectCreation - A boolean flag indicating whether the access will result in a new object being created if granted.
        A value of TRUE indicates an object will be created, FALSE indicates an existing object will be opened.
...
--*/
{
    ...
    // Check for SeAuditPrivilege.  This must be tested against the caller's primary token.
    SeCaptureSubjectContext ( &SubjectSecurityContext );
    Result = SeCheckAuditPrivilege (&SubjectSecurityContext,PreviousMode);
    if (!Result) {
        ObDereferenceObject( (PVOID)Token );
    ...
    // At this point in time, if there's no security descriptor, there's nothing to do.  Return success.
    ...
    try {
        // Only capture the privileges if we've completed a successful access check.  Otherwise they don't mean anything.
        ...
        // If we run out of memory attempting to capture the strings, the returned pointer will be NULL and we will continue with the audit.
        SepProbeAndCaptureString_U ( SubsystemName,&CapturedSubsystemName );
        SepProbeAndCaptureString_U ( ObjectTypeName, &CapturedObjectTypeName );
        SepProbeAndCaptureString_U ( ObjectName, &CapturedObjectName );
    } except(EXCEPTION_EXECUTE_HANDLER) {
    ...
    if ( SepAdtAuditThisEvent( AuditCategoryObjectAccess, &AccessGranted) ) {
        ...
        if (GenerateAudit || GenerateAlarm) {
            // Take a read lock on the token, because we're going to extract the user's Sid from it.
            LocalGenerateOnClose = TRUE;
    ...
    if ( !(GenerateAudit || GenerateAlarm) ) {
        // We didn't attempt to generate an audit above, so if privileges were used, see if we should generate an audit here.
        if ( ARGUMENT_PRESENT(Privileges) ) {
            if ( SepAdtAuditThisEvent( AuditCategoryPrivilegeUse, &AccessGranted) ) {
                ...
                // If we generate an audit due to use of privilege, don't set generate on close, because then we'll have a close audit without a corresponding open audit.
                LocalGenerateOnClose = FALSE;
            }
        }
    }
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when a handle to a protected subsystem object is deleted.
    This routine may result in several messages being generated and sent to Port objects.
    This may result in a significant latency before returning.
    Design of routines that must call this routine must take this potential latency into account.
    This may have an impact on the approach taken for data structure mutex locking, for example.
...
Arguments:
    SubsystemName - Supplies a name string identifying the subsystem calling the routine.
    HandleId - A unique value representing the client's handle to the object.
    GenerateOnClose - Is a boolean value returned from a corresponding NtAccessCheckAndAuditAlarm() call or NtOpenObjectAuditAlarm() call when the object handle was created.
--*/
{
    BOOLEAN Result;
    SECURITY_SUBJECT_CONTEXT SubjectSecurityContext;
    ...
    SepAdtCloseObjectAuditAlarm ( CapturedSubsystemName,HandleId,NULL,CapturedUserSid,SepTokenAuthenticationId( EffectiveToken( &SubjectSecurityContext )));
    Status = STATUS_SUCCESS;
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when an object in a protected subsystem object is deleted.
    This routine may result in several messages being generated and sent to Port objects.
    This may result in a significant latency before returning.
    Design of routines that must call this routine must take this potential latency into account.
    This may have an impact on the approach taken for data structure mutex locking, for example.
...
--*/
{
    ...
    // This routine will check to see if auditing is enabled
    SepAdtDeleteObjectAuditAlarm ( CapturedSubsystemName,HandleId,NULL,CapturedUserSid,SepTokenAuthenticationId( EffectiveToken( &SubjectSecurityContext )));
    SeReleaseSubjectContext ( &SubjectSecurityContext );
    if ( CapturedSubsystemName != NULL ) {
        SepFreeCapturedString( CapturedSubsystemName );
    }
...
/*++
Routine Description:
    SeOpenObjectAuditAlarm is used by the object manager that open objects to generate any necessary audit or alarm messages.
    The open may be to existing objects or for newly created objects.
    No messages will be generated for Kernel mode accesses.

    This routine is used to generate audit and alarm messages when an attempt is made to open an object.
...
Arguments:
    ...
    ObjectTypeName - Supplies the name of the type of object being accessed.
        This must be the same name provided to the ObCreateObjectType service when the object type was created.

    Object - Address of the object accessed.  This value will not be used as a pointer (referenced).
        It is necessary only to enter into log messages.
        If the open was not successful, then this argument is ignored.
    ...
    AccessState - A pointer to an access state structure containing the subject context, the remaining desired access types, the granted
        access types, and optionally a privilege set to indicate which privileges were used to permit the access.

    ObjectCreated - A boolean flag indicating whether the access resulted in a new object being created.
        A value of TRUE indicates an object was created, FALSE indicates an existing object was opened.

    AccessGranted - Indicates if the access was granted or denied based on the access check or privilege check.

    AccessMode - Indicates the access mode used for the access check.  One of UserMode or KernelMode.
        Messages will not be generated by kernel mode accesses.

    GenerateOnClose - Points to a boolean that is set by the audit generation routine and must be passed to SeCloseObjectAuditAlarm() when the object handle is closed.
--*/
{
    BOOLEAN GenerateAudit = FALSE;
    BOOLEAN GenerateAlarm = FALSE;
    ...
    if (SecurityDescriptor != NULL) {
        ...
        if ( SepAdtAuditThisEvent( AuditCategoryObjectAccess, &AccessGranted )) {
            if ( RequestedAccess & (AccessGranted ? MappedGrantMask : MappedDenyMask)) {
                ...
                SepExamineSacl(RtlpSaclAddrSecurityDescriptor( (PISECURITY_DESCRIPTOR)SecurityDescriptor ),Token,RequestedAccess,AccessGranted,&GenerateAudit,&GenerateAlarm);
            }

            // Only generate an audit on close of we're auditing from SACL settings.
            if (GenerateAudit) {
    ...
    // If we don't generate an audit via the SACL, see if we need to generate one for privilege use.

    // Note that we only audit privileges successfully used to open objects, so we don't care about a failed privilege use here.
    // Therefore, only do this test of access has been granted.
        ...
        if ( SepAdtAuditThisEvent( AuditCategoryPrivilegeUse, &AccessGranted )) {
            if ((AuxData->PrivilegesUsed != NULL) && (AuxData->PrivilegesUsed->PrivilegeCount > 0) ) {
                ...
                if (SepFilterPrivilegeAudits( AuxData->PrivilegesUsed )) {
                    ...
                    // When we finally try to generate this audit, this flag will tell us that we need to audit the fact that we used a privilege, as opposed to audit due to the SACL.
    ...
    // Set up either to generate an audit (if the access check has failed), or save the stuff that we're going to audit later into the AccessState structure.
    if (GenerateAudit || GenerateAlarm) {
        AccessState->GenerateAudit = TRUE;

        // Figure out what we've been passed, and obtain as much missing information as possible.
        ...
        // If the access attempt failed, do the audit here.  If it succeeded, we'll do the audit later, when the handle is allocated.
        if (!AccessGranted) {
            ...
            // Copy all the stuff we're going to need into the AccessState and return.
            if ( ObjectName != NULL ) {
...
/*++
Routine Description:
    SeOpenObjectForDeleteAuditAlarm is used by the object manager that open objects to generate any necessary audit or alarm messages.
    The open may be to existing objects or for newly created objects.
    No messages will be generated for Kernel mode accesses.

    This routine is used to generate audit and alarm messages when an attempt is made to open an object with the intent to delete it.
    Specifically, this is used by file systems when the flag FILE_DELETE_ON_CLOSE is specified.
...
Arguments:
    ObjectTypeName - Supplies the name of the type of object being accessed.  This must be the same name provided to the ObCreateObjectType service when the object type was created.
...
/*++
Routine Description:
    This routine is called to audit directory traverse operations specifically.
    It should be called by parse procedures as they traverse directories as part of their operation.
Arguments:
    OperationID - LUID identifying the operation in progress
    DirectoryObject - Pointer to the directory being traversed.
    SecurityDescriptor - The security descriptor (if any) attached to the directory being traversed.
    SubjectSecurityContext - Security context of the client.
    SubjectContextLocked - Supplies whether the SubjectContext is locked for shared access.
    TraverseAccess - Mask to indicate the traverse access for this object type.
    Privileges - Optional parameter to indicate any privilges that the subject may have used to gain access to the object.
    AccessGranted - Indicates if the access was granted or denied based on the access check or privilege check.
    AccessMode - Indicates the access mode used for the access check.  One of UserMode or KernelMode.  Messages will not be generated by kernel mode accesses.
--*/
{
    PAGED_CODE();
    ...
    if ((SeAuditingState[AuditEventTraverse].AuditOnSuccess && AccessGranted) || SeAuditingState[AuditEventTraverse].AuditOnFailure && !AccessGranted) {
        if ( SecurityDescriptor != NULL ) {
...
Arguments:
    OperationID - Optionally supplies the LUID representing the operation id for this operation.
    DirectoryObject - Provides a pointer to the directory object being examined.
    ComponentName - Provides a pointer to a Unicode string containing the relative name of the object being created.
...
--*/
{
    ...
    if ( SecurityDescriptor != NULL ) {
        if ( SepAdtAuditThisEvent( AuditCategoryObjectAccess, &AccessGranted )) {
            SepExamineSacl(RtlpSaclAddrSecurityDescriptor( (PISECURITY_DESCRIPTOR)SecurityDescriptor ),EffectiveToken( SubjectSecurityContext ),DesiredAccess,AccessGranted,&GenerateAudit,&GenerateAlarm);
            if ( GenerateAudit || GenerateAlarm ) {
...
VOID SeObjectReferenceAuditAlarm(
    IN PLUID OperationID OPTIONAL,
    IN PVOID Object,
    IN PSECURITY_DESCRIPTOR SecurityDescriptor,
    IN PSECURITY_SUBJECT_CONTEXT SubjectSecurityContext,
...
--*/
{
    ...
    if (SecurityDescriptor != NULL) {
        if (SepAdtAuditThisEvent(AuditCategoryDetailedTracking, &AccessGranted)) {
            SepExamineSacl(RtlpSaclAddrSecurityDescriptor((PISECURITY_DESCRIPTOR) SecurityDescriptor), EffectiveToken(SubjectSecurityContext), DesiredAccess, AccessGranted, &GenerateAudit, &GenerateAlarm);
            if (GenerateAudit || GenerateAlarm) {
                SepAdtObjectReferenceAuditAlarm(OperationID, Object, SubjectSecurityContext, DesiredAccess, Privileges, AccessGranted, GenerateAudit, GenerateAlarm);
            }
        }
    }
}
...
Routine Description:
    ...
    It will examine the AuditHandleCreation field in the passed AccessState, which will indicate whether auditing was performed when the object was found or created.

    This routine is necessary because object name decoding and handle allocation occur in widely separate places, preventing us from auditing everything at once.
Arguments:
    AccessState - Supplies a pointer to the AccessState structure representing this access attempt.
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when a handle to an object is deleted.
    ...
    This may have an impact on the approach taken for data structure mutex locking, for example.
Arguments:
    Object - Address of the object being accessed.  This value will not be used as a pointer (referenced).  It is necessary only to enter into log messages.
    Handle - Supplies the handle value assigned to the open.
    GenerateOnClose - Is a boolean value returned from a corresponding SeOpenObjectAuditAlarm() call when the object handle was created.
--*/
{
    SECURITY_SUBJECT_CONTEXT SubjectSecurityContext;
    PSID UserSid;
    ...
    if (GenerateOnClose) {
        SeCaptureSubjectContext ( &SubjectSecurityContext );
        UserSid = SepTokenUserSid( EffectiveToken (&SubjectSecurityContext));
        SepAdtCloseObjectAuditAlarm (&SeSubsystemName,(PVOID)Handle,Object,UserSid,SepTokenAuthenticationId( EffectiveToken (&SubjectSecurityContext)));
        SeReleaseSubjectContext ( &SubjectSecurityContext );
    }
}
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when an object is marked for deletion.
...
--*/
{
    ...
    SeCaptureSubjectContext ( &SubjectSecurityContext );
    UserSid = SepTokenUserSid( EffectiveToken (&SubjectSecurityContext));
    SepAdtDeleteObjectAuditAlarm (&SeSubsystemName,(PVOID)Handle,Object,UserSid,SepTokenAuthenticationId( EffectiveToken (&SubjectSecurityContext)));
    SeReleaseSubjectContext ( &SubjectSecurityContext );
}
...
/*++
Routine Description:
    This routine will examine the passed Sacl and determine what if any action is required based its contents.

    Note that this routine is not aware of any system state, ie, whether or not auditing is currently enabled for either the system or this particular object type.
Arguments:
    Sacl - Supplies a pointer to the Sacl to be examined.
    Token - Supplies the effective token of the caller
    AccessGranted - Supplies whether or not the access attempt was successful.
    GenerateAudit - Returns a boolean indicating whether or not we should generate an audit.
    GenerateAlarm - Returns a boolean indiciating whether or not we should generate an alarm.
Return Value:
...
--*/
{
    ...
    *GenerateAlarm = FALSE;
    ...
    // If we failed an attempt to open an object for ONLY maximumum allowed, then we generate an audit if ANY ACCESS_DENIED audit matching this user's list of sids is found
    ...
    // Iterate through the ACEs on the Sacl until either we reach the end or discover that we have to take all possible actions, in which case it doesn't pay to look any further
             ...
             if ( (((PACE_HEADER)Ace)->AceType == SYSTEM_AUDIT_ACE_TYPE) ) {
                if ( SepSidInToken( (PACCESS_TOKEN)Token, NULL, &((PSYSTEM_AUDIT_ACE)Ace)->SidStart, FALSE ) ) {
                    ...
                    if ( AccessMask & DesiredAccess ) {
                        if (((AceFlags & SUCCESSFUL_ACCESS_ACE_FLAG) && AccessGranted) || ((AceFlags & FAILED_ACCESS_ACE_FLAG) && !AccessGranted)) {
                            *GenerateAudit = TRUE;                        }
...
/*++
Routine Description:
    This routine determines if any children of the object represented by StartIndex have a different degree of success than the StartIndex element.
Arguments:
    ObjectTypeList - The object type list to update.
    ObjectTypeListLength - Number of elements in ObjectTypeList
    AccessStatus - Specifies STATUS_SUCCESS or other error code to be propogated back to the caller
    StartIndex - Index to the target element to update.
    GenerateSuccessAudit - Returns a boolean indicating whether or not we should generate a success audit.
    GenerateFailureAudit - Returns a boolean indicating whether or not we should generate a failure audit.
--*/
{
    ULONG Index;
    BOOLEAN WasSuccess;
    ...
    for ( Index=StartIndex+1; Index<ObjectTypeListLength; Index++ ) {
        // By definition, the children of an object are all those entries immediately following the target.
        // The list of children (or grandchildren) stops as soon as we reach an entry the has the same level as the target (a sibling) or lower than the target (an uncle).
        if ( ObjectTypeList[Index].Level <= ObjectTypeList[StartIndex].Level ) {
            break;
        }

        // If the child has different access than the target, mark the child.
...
/*++
Routine Description:
    Determine if success/failure audit needs to be generated for object at ObjectTypeIndex in ObjectTypeList.
        ...
        if ( (AceFlags & SUCCESSFUL_ACCESS_ACE_FLAG) && NT_SUCCESS(AccessStatus[ObjectTypeIndex]) ) {
            *GenerateSuccessAudit = TRUE;
            if ( ObjectTypeListLength != 0 ) {
                ...
                if ( ReturnResultList ) {
                    SepAuditTypeList( ObjectTypeList,ObjectTypeListLength,AccessStatus,ObjectTypeIndex,GenerateSuccessAudit,GenerateFailureAudit );
                }
            }
...
Arguments:
    ...
    ObjectTypeList - Supplies a list of GUIDs representing the object (and sub-objects) being accessed.
    ObjectTypeListLength - Specifies the number of elements in the ObjectTypeList.
    ReturnResultList - If true, AccessStatus and GrantedAccess is actually an array of entries ObjectTypeListLength elements long.
    AccessStatus - Specifies STATUS_SUCCESS or other error code to be propogated back to the caller
...
#define INVALID_OBJECT_TYPE_LIST_INDEX 0xFFFFFFFF
    ...
    *GenerateFailureAudit = FALSE;
    ...
    // If we failed an attempt to open an object for maximumum allowed, then we generate an audit if ANY ACCESS_DENIED audit matching this user's list of sids is found
    FailedMaximumAllowed = FALSE;
    ...
    // Iterate through the ACEs on the Sacl until either we reach the end or discover that we have to take all possible actions, in which case it doesn't pay to look any further
    for ( i = 0, Ace = FirstAce( Sacl ) ; (i < AceCount) && !((*GenerateSuccessAudit || *GenerateFailureAudit) && ObjectTypeListLength <= 1 ); i++, Ace = NextAce( Ace ) ) {
        AceFlags = ((PACE_HEADER)Ace)->AceFlags;
        if ( !(AceFlags & INHERIT_ONLY_ACE)) {
            ...
            // Handle an object specific audit ACE
                ...
                // If no object type is in the ACE, treat this as a normal audit ACE.
                ...
                // If no object type list was passed, don't generate an audit.
                } else if ( ObjectTypeListLength == 0 ) {
                    // Drop through

                // If an object type is in the ACE, Find it in the LocalTypeList before using the ACE.
                } else {
                    if ( SepSidInToken( Token, NULL, RtlObjectAceSid(Ace), (BOOLEAN)((AceFlags & FAILED_ACCESS_ACE_FLAG) != 0) ) ) {
            ...
            // If the ACE has a matched SID and a matched GUID, handle it.
            if ( Index != INVALID_OBJECT_TYPE_LIST_INDEX ) {
                // ASSERT: we have an ACE to be audited.

                // Index is an index into ObjectTypeList of the entry to mark as the GUID needs auditing.

                // SuccessIndex is an index into AccessStatus to determine if a success or failure audit is to be generated
                SepSetAuditInfoForObjectType(AceFlags,
                                             AccessMask,
                                             DesiredAccess,
                                             ObjectTypeList,
...
/*++
Routine Description:
    This routine will filter out a list of privileges as listed in the SepFilterPrivileges array.
Arguments:
...
Routine Description:
    ...
    This allows the file system to avoid the often considerable setup involved in generating an audit.
Arguments:
    AccessGranted - Supplies whether the access attempt was successful or a failure.
Return Value:
    Boolean - TRUE if events of type AccessGranted are being audited, FALSE otherwise.
--*/
{
