...
/*++
Routine Description:
    This routine modifies the DACL protecting the passed token to allow the current user (described by the PrimaryToken parameter) full access.
    This permits callers of NtOpenThreadToken to call with OpenAsSelf==TRUE and succeed.
...
Arguments:
    ...
    DesiredAccess - Is an access mask indicating which access types
        ...
        with the Discretionary Access Control list of the token to determine whether the accesses will be granted or denied.
...
--*/
{
    ...
    // process's token.  If successful, this will cause the token's reference count to be incremented.
    ...
    //  And decrement the reference count of the token to counter the action performed by PsOpenTokenOfProcess(). 
    //  If the open was successful, the handle will have caused the token's reference count to have been incremented.
...
NTSTATUS SepOpenTokenOfThread(IN HANDLE ThreadHandle,
...
Routine Description:
    ...
    If so, it goes on to increment the reference count of the token object to prevent it from
    going away while the rest of the NtOpenThreadToken() request is processed.
...
Arguments:
    ...
    OpenAsSelf - Is a boolean value indicating whether the access should be made using the calling thread's current security context, which
        may be that of a client if impersonating, or using the caller's process-level security context.  A value of FALSE indicates the
        caller's current context should be used un-modified.  A value of
        TRUE indicates the request should be fulfilled using the process level security context.

        This parameter is necessary to allow a server process to open a client's token when the client specified IDENTIFICATION level
        impersonation.  In this case, the caller would not be able to open the client's token using the client's context (because you
        can't create executive level objects using IDENTIFICATION level impersonation).

    TokenHandle - Receives the handle of the newly opened token.

Return Value:
    STATUS_SUCCESS - Indicates the operation was successful.
    STATUS_NO_TOKEN - Indicates an attempt has been made to open a token associated with a thread that is not currently impersonating a client.
    STATUS_CANT_OPEN_ANONYMOUS - Indicates the client requested anonymous impersonation level.  An anonymous token can not be openned.
--*/
{
    KPROCESSOR_MODE PreviousMode;
    ...
    // We need to create and/or open a token object, so disable impersonation if necessary.
    if (OpenAsSelf) {
         RestoreImpersonationState = PsDisableImpersonation(PsGetCurrentThread(), &DisabledImpersonationState);
    }

    //  If the CopyOnOpen flag is not set, then the token can be
    //  opened directly.  Otherwise, the token must be duplicated, and a handle to the duplicate returned.
    if (CopyOnOpen) {
        ...
        // If we just use the current SubjectContext, we'll get the SID of whoever is calling us, which isn't what we want.
        ...
        if (NT_SUCCESS(Status)) {
            ...
            if (NT_SUCCESS( Status )) {
                if (NewAcl != NULL) {
                    ...
                    // or have security descriptors, but do not have DACLs.  In either case, do nothing.
                ...
                InitializeObjectAttributes(&ObjectAttributes, NULL, 0L, NULL, NewAcl == NULL ? NULL : &SecurityDescriptor);
                ...
                Status = SepDuplicateToken(
                             ...
                             &NewToken);
                if (NT_SUCCESS( Status )) {                    
                    ObReferenceObject(NewToken);// Reference the token so it doesn't go away
        ...
        // we'be be here is if the impersonation is taking place because someone did an NtSetInformationThread and passed in a token.

        // In that case, we absolutely do not want to give the caller guaranteed
        // access, because that would allow anyone who has access to a thread to impersonate any of that thread's clients for any access.
    ...
    //  was successful, the handle will have caused the token's reference count to have been incremented.
