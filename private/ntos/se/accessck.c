...
Abstract:
    This Module implements the access check procedures.
    Both NtAccessCheck and SeAccessCheck check to is if a user (denoted by an input token) can be granted the desired access rights to object protected by a security descriptor and an optional object owner.
    Both procedures use a common local procedure to do the test.
...
Arguments:
    ...
    RequestorMode - Indicates the processor mode by which the access is being requested.
        ...
        if ( ObjectTypeListLength == 0 ) {
            ...
            // Copy the callers structure to the local structure.
                ...
                for ( i=0; i<ObjectTypeListLength; i++ ) {
                    ...
                    ProbeForRead(CapturedObjectType, sizeof(GUID), sizeof(ULONG));
                    ...
                    // Ensure that the level number is consistent with the level number of the previous entry.
                    if ( i == 0 ) {
                        if ( CurrentLevel != 0 ) {
                            Status = STATUS_INVALID_PARAMETER;
                            break;
                    ...
                    Levels[CurrentLevel] = i;// Save this obect as the last object seen at this level.
                }
            }
        } // end_if
...
/*++
Routine Description:
    This routine frees the data associated with a captured ObjectTypeList structure.
Arguments:
...
--*/
{
    ...
    // Get the index to the parent that needs updating and the level of the siblings.
    ...
    //  we're done.
    if ( NewRemaining == ObjectTypeList[ParentIndex].Remaining && NewCurrentGranted == ObjectTypeList[ParentIndex].CurrentGranted && NewCurrentDenied == ObjectTypeList[ParentIndex].CurrentDenied ) {
        return;
    }
...
--*/
{
    ...
    case UpdateCurrentGranted:
        ...
        if ( OldCurrentGranted == ObjectTypeList[StartIndex].CurrentGranted ) {
            ...
            // needs to be added all of the children that haven't been explictly denied.
            AvoidParent = TRUE;
        }
        break;
...
Arguments:
    ...
    PrincipalSelfSid - If the object being access checked is an object which
        represents a principal (e.g., a user object), this parameter should be the SID of the object.
        Any ACE containing the constant PRINCIPAL_SELF_SID is replaced by this SID.
...
--*/
{
    ...
    for (i = 0 ; i < UserAndGroupCount ; i += 1) {
        ...
        if ((((PISID)Sid)->Revision == MatchSid->Revision) && (SidLength == (8 + (4 * (ULONG)MatchSid->SubAuthorityCount)))) {
            if (RtlEqualMemory(Sid, MatchSid, SidLength)) {
                // If this is the first one in the list, then it is the User, and return success immediately.

                // If this is not the first one, then it represents a group, and we must make sure the group is currently enabled before we can say that the group is "in" the token.
                if ((i == 0) || (TokenSid->Attributes & SE_GROUP_ENABLED) || (DenyAce && (TokenSid->Attributes & SE_GROUP_USE_FOR_DENY_ONLY))) {
                    return TRUE;
                } else {
                    return FALSE;
                }
...
Arguments:
    Token - Pointer to the token to be examined
    PrincipalSelfSid - If the object being access checked is an object which represents a principal (e.g., a user object), this parameter should
...
--*/
{
    ...
    for (i = 0; i < UserAndGroupCount ; i += 1) {
        ...
        if ((((PISID)Sid)->Revision == MatchSid->Revision) && (SidLength == (8 + (4 * (ULONG)MatchSid->SubAuthorityCount)))) {
            if (RtlEqualMemory(Sid, MatchSid, SidLength)) {
                // If this is the first one in the list and not deny-only it is not a restricted token then it is the User, and return success immediately.
...
/*++
Routine Description:
    Does an access check for maximum allowed or with a result list.
    If the Restricted flag is set, it is done for a restricted token.
    The sids checked are the restricted sids, not the users and groups.
    The current granted access is stored in the Remaining access and then another access check is run.
Arguments:
            ...
            if ( (((PACE_HEADER)Ace)->AceType == ACCESS_ALLOWED_ACE_TYPE) ) {
                if (SepSidInTokenEx( EToken, PrincipalSelfSid, &((PACCESS_ALLOWED_ACE)Ace)->SidStart, FALSE, Restricted )) {
                    ...
                    if ( LocalTypeListLength == 1 ) {
                        LocalTypeList->CurrentGranted |= (((PACCESS_ALLOWED_ACE)Ace)->Mask & ~LocalTypeList->CurrentDenied);
                    } else {
             ...
             // Handle an object specific Access Allowed ACE
                 ...
                 if ( ObjectTypeInAce == NULL ) {
                     if ( SepSidInTokenEx( EToken, PrincipalSelfSid, RtlObjectAceSid(Ace), FALSE, Restricted ) ) {
                         // Optimize 'normal' case
                         if ( LocalTypeListLength == 1 ) {
                             LocalTypeList->CurrentGranted |= (((PACCESS_ALLOWED_OBJECT_ACE)Ace)->Mask & ~LocalTypeList->CurrentDenied);
                         } else {
                             SepAddAccessTypeList(
                                 LocalTypeList,          // List to modify
                                 LocalTypeListLength,    // Length of list
                 ...
                 // If no object type list was passed, don't grant access to anyone.
                ...
                // If an object type is in the ACE, Find it in the LocalTypeList before using the ACE.
                 ...
                 //  If we're impersonating, EToken is set to the Client, and if we're not, EToken is set to the Primary.
                 //  According to the DSA architecture, if we're asked to evaluate a compound ACE and we're not impersonating, pretend we are impersonating ourselves.
                 //  So we can just use the EToken for the client token, since it's already set to the right thing.
                 if ( SepSidInTokenEx(EToken, PrincipalSelfSid, RtlCompoundAceClientSid( Ace ), FALSE, Restricted) && SepSidInTokenEx(PrimaryToken, NULL, RtlCompoundAceServerSid( Ace ), FALSE, FALSE)) {
                     // Only grant access types from this mask that have
                     // not already been denied
                 ...
                 if ( SepSidInTokenEx( EToken, PrincipalSelfSid, &((PACCESS_DENIED_ACE)Ace)->SidStart, TRUE, Restricted )) {
                     ...
                     // Optimize 'normal' case
                     if ( LocalTypeListLength == 1 ) {
                         LocalTypeList->CurrentDenied |= (((PACCESS_DENIED_ACE)Ace)->Mask & ~LocalTypeList->CurrentGranted);
                     } else {
                         // The zeroeth object type represents the object itself.
                         SepAddAccessTypeList(
                             LocalTypeList,          // List to modify
             ...
             // Handle an object specific Access Denied ACE
                 ...
                 if ( SepSidInTokenEx( EToken, PrincipalSelfSid, RtlObjectAceSid(Ace), TRUE, Restricted ) ) {
                     ...
                     // If there is no object type in the ACE, or if the caller didn't specify an object type list, apply this deny ACE to the entire object.
                     ObjectTypeInAce = RtlObjectAceObjectType(Ace);
                     if ( ObjectTypeInAce == NULL ) {
                         if ( LocalTypeListLength == 1 ) {
...
/*++
Routine Description:
    Does an access check when the caller isn't asking for MAXIMUM_ALLOWED or a type result list.
    If the Restricted flag is set, the sids checked are the restricted sids, not the users and groups.
    The Remaining field is reset to the original remaining value and then another access check is run.
Arguments:
    ...
    for ( i = 0, Ace = FirstAce( Dacl ) ; ( i < AceCount ) && ( LocalTypeList->Remaining != 0 )  ; i++, Ace = NextAce( Ace ) ) {
        if ( !(((PACE_HEADER)Ace)->AceFlags & INHERIT_ONLY_ACE)) {
            ...
            // Handle an object specific Access Allowed ACE
                ...
                // If no object type is in the ACE, treat this as an ACCESS_ALLOWED_ACE.
                ObjectTypeInAce = RtlObjectAceObjectType(Ace);
                if ( ObjectTypeInAce == NULL ) {
                    if ( SepSidInTokenEx( EToken, PrincipalSelfSid, RtlObjectAceSid(Ace), FALSE, Restricted ) ) {
                       // Optimize 'normal' case
            ...
            // Handle a compound Access Allowed ACE
                ...
                // See comment in MAXIMUM_ALLOWED case as to why we can use EToken here for the client.
                if ( SepSidInTokenEx(EToken, PrincipalSelfSid, RtlCompoundAceClientSid( Ace ), FALSE, Restricted) && SepSidInTokenEx(PrimaryToken, NULL, RtlCompoundAceServerSid( Ace ), FALSE, Restricted) ) {
                    // Optimize 'normal' case
                    if ( LocalTypeListLength == 1 ) {
            ...
            // Handle an object specific Access Denied ACE
                ...
                if ( SepSidInTokenEx( EToken, PrincipalSelfSid, RtlObjectAceSid(Ace), TRUE, Restricted ) ) {
                    ...
                    // Otherwise apply the deny ACE to the object specified in the ACE.
                    } else if ( SepObjectInTypeList( ObjectTypeInAce, LocalTypeList, LocalTypeListLength, &Index ) ) {
...
/*++
Routine Description:
    Worker routine for SeAccessCheck and NtAccessCheck.  We actually do the access checking here.

    Whether or not we actually evaluate the DACL is based on the following
    interaction between the SE_DACL_PRESENT bit in the security descriptor and the value of the DACL pointer itself.
...
Arguments:
    ...
    SecurityDescriptor - Pointer to the security descriptor from the object being accessed.
    ...
    TokenLocked - Boolean describing whether or not there is a read lock on the token.
    ...
    ObjectTypeList - Supplies a list of GUIDs representing the object (and
        sub-objects) being accessed.  If no list is present, AccessCheckByType behaves identically to AccessCheck.
    ...
    PreviouslyGrantedAccess - Access mask indicating any access' that have already been granted by higher level routines

    PrivilgedAccessMask - Mask describing access types that may not be granted without a privilege.

    GrantedAccess - Returns an access mask describing all granted access', or NULL.

    Privileges - Optionally supplies a pointer in which will be returned
        any privileges that were used for the access.  If this is null, it will be assumed that privilege checks have been done already.

    AccessStatus - Returns STATUS_SUCCESS or other error code to be propogated back to the caller

    ReturnResultList - If true, GrantedAccess and AccessStatus is actually an array of entries ObjectTypeListLength elements long.

    ReturnSomeAccessGranted - Returns a value of TRUE to indicate that some access' were granted, FALSE otherwise.

    ReturnSomeAccessDenied - Returns a value of FALSE if some of the requested
        access was not granted.  This will alway be an inverse of SomeAccessGranted unless ReturnResultList is TRUE.  In that case,

Return Value:
    A value of TRUE indicates that some access' were granted, FALSE otherwise.
--*/
{
    NTSTATUS Status;
    ...
    if ( Remaining & ACCESS_SYSTEM_SECURITY ) {
        ...
        // into.  Our caller was supposed to have taken care of this in that case.
        ...
        // Success, remove ACCESS_SYSTEM_SECURITY from remaining, add it to PreviouslyGrantedAccess
    ...
    if ( !RtlpAreControlBitsSet((PISECURITY_DESCRIPTOR)SecurityDescriptor, SE_DACL_PRESENT) || (Dacl == NULL)) {
        // Restricted tokens treat a NULL dacl the same as a DACL with no ACEs.
#ifdef SECURE_NULL_DACLS
        if (SeTokenIsRestricted( EToken )) {
            // We know that Remaining != 0 here, because we know it was non-zero coming into this routine,
            // and we've checked it against 0 every time we've cleared a bit.
            ASSERT( Remaining != 0 );

            // There are ungranted accesses.  Since there is nothing in the DACL, they will not be granted.
            // If, however, the only ungranted access at this point is MAXIMUM_ALLOWED, and something has been granted in the PreviouslyGranted mask, return what has been granted.
        ...
        if (DesiredAccess & MAXIMUM_ALLOWED) {
            ...
            PreviouslyGrantedAccess = GenericMapping->GenericAll | (DesiredAccess | PreviouslyGrantedAccess) & ~MAXIMUM_ALLOWED;
        } else {
    ...
    // There is security on this object.  Check to see if he's asking for WRITE_OWNER, and perform the privilege check if so.
    if ( (Remaining & WRITE_OWNER) && ARGUMENT_PRESENT( Privileges ) ) {
        ...
        if (Success) {
            // Success, remove WRITE_OWNER from remaining, add it to PreviouslyGrantedAccess
    ...
    // If the DACL is empty, deny all access immediately.
    ...
    // If the caller wants the MAXIMUM_ALLOWED or the caller wants the results on all objects and subobjects, use a slower algorithm that traverses all the ACEs.
    if ( (DesiredAccess & MAXIMUM_ALLOWED) != 0 || ReturnResultList ) {
        // Do the normal maximum-allowed access check
        SepMaximumAccessCheck(EToken,PrimaryToken,Dacl,PrincipalSelfSid,LocalTypeListLength,LocalTypeList,ObjectTypeListLength,FALSE);

        // If this is a restricted token, do the additional access check
        if (SeTokenIsRestricted( EToken ) ) {
            SepMaximumAccessCheck(EToken,PrimaryToken,Dacl,PrincipalSelfSid,LocalTypeListLength,LocalTypeList,ObjectTypeListLength,TRUE);
        }
        ...
        if ( ReturnResultList ) {
            ...
            for ( ResultListIndex=0; ResultListIndex<LocalTypeListLength; ResultListIndex++ ) {
                // Return the subset of the access granted that the caller expressed interest in.
                GrantedAccess[ResultListIndex] = (LocalTypeList[ResultListIndex].CurrentGranted | PreviouslyGrantedAccess ) & GrantedAccessMask;
                // If absolutely no access was granted, indicate so.
                if ( GrantedAccess[ResultListIndex] == 0 ) {
                    AccessStatus[ResultListIndex] = STATUS_ACCESS_DENIED;
                    SomeAccessDenied = TRUE;
                } else {
                    // If some requested access is still missing, the bottom line is that access is denied.

                    // Note, that ByTypeResultList actually returns the partially granted access mask even though the caller really has no access to the object.
        ...
        // If the caller is only interested in the access to the object itself, just summarize.
        } else {
            // Turn off the MAXIMUM_ALLOWED bit and whatever we found that he was granted.
            // If the user passed in extra bits in addition to MAXIMUM_ALLOWED, make sure that he was granted those access types.  If not, he didn't get what he wanted, so return failure.
    ...
    for ( i = 0, Ace = FirstAce( Dacl ) ; ( i < AceCount ) && ( LocalTypeList->Remaining != 0 )  ; i++, Ace = NextAce( Ace ) ) {
        if ( !(((PACE_HEADER)Ace)->AceFlags & INHERIT_ONLY_ACE)) {
            ...
            // Handle a compound Access Allowed ACE
                ...
                // See comment in MAXIMUM_ALLOWED case as to why we can use EToken here for the client.
                if ( SepSidInToken(EToken, PrincipalSelfSid, RtlCompoundAceClientSid( Ace ), FALSE) && SepSidInToken(PrimaryToken, NULL, RtlCompoundAceServerSid( Ace ), FALSE) ) {
                    // Optimize 'normal' case
                    if ( LocalTypeListLength == 1 ) {
                        LocalTypeList->Remaining &= ~((PCOMPOUND_ACCESS_ALLOWED_ACE)Ace)->Mask;
                    } else {
    ...
    SepNormalAccessCheck(Remaining,EToken,PrimaryToken,Dacl,PrincipalSelfSid,LocalTypeListLength,LocalTypeList,ObjectTypeListLength,FALSE);
    ...
    // If this is a restricted token, do the additional access check
    if (SeTokenIsRestricted( EToken ) ) {
        SepNormalAccessCheck(Remaining,EToken,PrimaryToken,Dacl,PrincipalSelfSid,LocalTypeListLength,LocalTypeList,ObjectTypeListLength,TRUE);
    }
    ...
    // If the caller asked for a list of status', duplicate the status all over.
    if ( ReturnResultList ) {
...
Arguments:
    ...
    PrivilegeSet - A pointer to a buffer that upon return will contain any privileges that were used to perform the access validation.
        If no privileges were used, the buffer will contain a privilege set consisting of zero privileges.
...
Return Value:
    STATUS_SUCCESS - The attempt proceeded normally.  This does not mean access was granted, rather that the parameters were correct.
    STATUS_GENERIC_NOT_MAPPED - The DesiredAccess mask contained an unmapped generic access.
    STATUS_BUFFER_TOO_SMALL - The passed buffer was not large enough to contain the information being returned.
    STATUS_NO_IMPERSONTAION_TOKEN - The passed Token was not an impersonation token.
--*/
{
    PAGED_CODE();
...
Arguments:
    ...
    AccessStatus - Status value that may be returned indicating the reason why access was denied.
         Routines should avoid hardcoding a return value of STATUS_ACCESS_DENIED so that a different value can be returned when mandatory access control is implemented.
...
Arguments:
    ...
    AccessStatus - Status value that may be returned indicating the
         reason why access was denied.  Routines should avoid hardcoding a return value of STATUS_ACCESS_DENIED so that a different value can
         be returned when mandatory access control is implemented.
...
--*/
{
    ...
    // It must be an impersonation token, and at impersonation level of Identification or above.
    ...
    // Compare the DesiredAccess with the privileges in the passed token, and see if we can either satisfy the requested
    // access with a privilege, or bomb out immediately because we don't have a privilege we need.
    Status = SePrivilegePolicyCheck(&DesiredAccess,&PreviouslyGrantedAccess,NULL,(PACCESS_TOKEN)Token,&Privileges,PreviousMode);
    if (!NT_SUCCESS( Status )) {
        try {
            if ( ReturnResultList ) {
                for ( ResultListIndex=0; ResultListIndex<ObjectTypeListLength; ResultListIndex++ ) {
    ...
    // Make sure the passed privileges buffer is large enough for whatever we have to put into it.
    if (Privileges != NULL) {
    ...
    // If there's no security descriptor, then we've been called without all the parameters we need.
    ...
    // If the user in the token is the owner of the object, we must automatically grant ReadControl and WriteDac access
    // if desired.  If the DesiredAccess mask is empty after these bits are turned off, we don't have to do any more access checking (ref section 4, DSA ACL Arch)
...
Routine Description:
    ...
    This routine MAY perform tests for the following privileges:
                SeTakeOwnershipPrivilege
                SeSecurityPrivilege

    depending upon the accesses being requested.
    This routine may also check to see if the subject is the owner of the object (to grant WRITE_DAC access).
Arguments:
    SecurityDescriptor - Supplies the security descriptor protecting the object being accessed
    SubjectSecurityContext - A pointer to the subject's captured security context
    SubjectContextLocked - Supplies a flag indiciating whether or not the user's subject context is locked, so that it does not have to be locked again.
    DesiredAccess - Supplies the access mask that the user is attempting to acquire
    PreviouslyGrantedAccess - Supplies any accesses that the user has already been granted, for example, as a result of holding a privilege.
    Privileges - Supplies a pointer in which will be returned a privilege set indicating any privileges that were used as part of the access validation.
    GenericMapping - Supplies the generic mapping associated with this object type.
    AccessMode - Supplies the access mode to be used in the check
    GrantedAccess - Pointer to a returned access mask indicatating the granted access
    AccessStatus - Status value that may be returned indicating the reason why access was denied.
         Routines should avoid hardcoding a return value of STATUS_ACCESS_DENIED so that a different value can be returned when mandatory access control is implemented.
Return Value:
...
--*/
{
    ...
    // If the object doesn't have a security descriptor (and it's supposed to), return access denied.
    if ( SecurityDescriptor == NULL) {
       *AccessStatus = STATUS_ACCESS_DENIED;
       return( FALSE );
    }

    // If we're impersonating a client, we have to be at impersonation level of SecurityImpersonation or above.
    if ( (SubjectSecurityContext->ClientToken != NULL) && (SubjectSecurityContext->ImpersonationLevel < SecurityImpersonation)) {
    ...
    // If the caller did not lock the subject context for us, lock it here to keep lower level routines from having to lock it.
    ...
    if (DesiredAccess == 0) {
        ...
        *AccessStatus = STATUS_SUCCESS;
        ...
        // If we locked it in this routine, unlock it before we leave.
        if ( !SubjectContextLocked ) {
            SeUnlockSubjectContext( SubjectSecurityContext );
        }
...
Arguments:
    ...
    RelativePath - The volume-relative path of the file being opened.  The full path of the file is the RelativePath appended to the Volume string.
...
/*++
Routine Description:
    This routine implements privilege policy by examining the bits in a DesiredAccess mask and adjusting them based on privilege checks.

    Currently, a request for ACCESS_SYSTEM_SECURITY may only be satisfied by the caller having SeSecurityPrivilege.
    WRITE_OWNER may optionally be satisfied via SeTakeOwnershipPrivilege.
Arguments:
    ...
    PreviouslyGrantedAccess - Supplies an access mask describing any accesses that have already been granted.
        Bits may be set in here as a result of privilge checks.
    SubjectSecurityContext - Optionally provides the subject's security context.
    ExplicitToken - Optionally provides the token to be examined.
    PrivilegeSet - Supplies a pointer to a location in which will be returned a pointer to a privilege set.
    PreviousMode - The previous processor mode.
Return Value:
    STATUS_SUCCESS - Any access requests that could be satisfied via privileges were done.
    STATUS_PRIVILEGE_NOT_HELD - An access type was being requested that requires a privilege, and the current subject did not have the privilege.
--*/
{
    BOOLEAN Success;
    PTOKEN Token;
...
/*++
Routine Description:
    This routine will determine of the Owner of the passed security descriptor is in the passed token.
    If the token is restricted it cannot be the owner.
Arguments:
    Token - The token representing the current user.
    SecurityDescriptor - The security descriptor for the object being accessed.
    TokenLocked - A boolean describing whether the caller has taken a read lock for the token.
Return Value:
...
/*++
Routine Description:
    This routine will examine the DACL of the passed Security Descriptor to see if WORLD has Traverse access.
    If so, no further access checking is necessary.

    Note that the SubjectContext for the client process does not have to be locked to make this call, since it does not examine any data structures in the Token.
Arguments:
    SecurityDescriptor - The Security Descriptor protecting the container object being traversed.
    TraverseAccess - Access mask describing Traverse access for this object type.
    AccessMode - Supplies the access mode to be used in the check
Return Value:
    TRUE - WORLD has Traverse access to this container.  FALSE otherwise.
--*/
{
    PACL Dacl;
    ULONG i;
    ...
    // There is security on this object.  If the DACL is empty, deny all access immediately
    if ((AceCount = Dacl->AceCount) == 0) {
        return( FALSE );
    }

    // There's stuff in the DACL, walk down the list and see if WORLD has been granted TraverseAccess
