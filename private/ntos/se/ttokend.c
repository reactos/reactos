...
// This byte is expected to be different in the DummyLogonHours and NoRestrictionLogonHours.
...
// VOID TST_SUCCESS_ASSERT( IN NTSTATUS S );
...
BOOLEAN TInitialize( VOID );
BOOLEAN EnableSecurityPrivilege( VOID );
VOID DetermineTestsToRun( VOID );
VOID SeeIfSidIsSpecial(IN PSID Sid);
BOOLEAN ServerTestSuite(PHANDLE ServerHandle,PHANDLE DomainHandle,PHANDLE BuiltinDomainHandle,PSID    *DomainSid);
BOOLEAN SecurityTestSuite(HANDLE ServerHandle,HANDLE DomainHandle,ULONG Pass);
BOOLEAN CheckReturnedSD(IN SECURITY_INFORMATION SI,IN PSECURITY_DESCRIPTOR SD,IN BOOLEAN PrintTestSuccess);
BOOLEAN DomainTestSuite(HANDLE DomainHandle);
BOOLEAN GroupTestSuite(HANDLE DomainHandle,ULONG  Pass);
BOOLEAN AliasTestSuite(HANDLE DomainHandle,HANDLE BuiltinDomainHandle,PSID DomainSid,ULONG  Pass);
BOOLEAN UserTestSuite(HANDLE DomainHandle,ULONG Pass);
NTSTATUS SampSetDomainPolicy( VOID );
NTSTATUS SampGetLsaDomainInfo(PPOLICY_ACCOUNT_DOMAIN_INFO *PolicyAccountDomainInfo,PPOLICY_PRIMARY_DOMAIN_INFO *PolicyPrimaryDomainInfo);

// The following are in WRAPPERS.C, but are prototyped here since this test is the only thing that should ever call them.
NTSTATUS SamTestPrivateFunctionsDomain(IN HANDLE DomainHandle);
NTSTATUS SamTestPrivateFunctionsUser(IN HANDLE UserHandle);
...
--*/
{
    ...
    if ( NT_SUCCESS(Status) ) {
        ...
        Status = LsaQueryInformationPolicy( PolicyHandle, PolicyAccountDomainInformation, (PVOID *)PolicyAccountDomainInfo );
#if DBG
        if ( NT_SUCCESS(Status) ) {
        ...
        Status = LsaQueryInformationPolicy( PolicyHandle, PolicyPrimaryDomainInformation, (PVOID *)PolicyPrimaryDomainInfo );
#if DBG
        if ( NT_SUCCESS(Status) ) {
...
--*/
{
    ...
    ASSERT(AccountSidLength == SeLengthSid(DomainSid) + sizeof(ULONG));// Temp sanity check
    ...
    if (AccountSid != NULL) {
        ...
        *RtlSubAuthorityCountSid(AccountSid) = AccountSubAuthorityCount;// Increment the account sid sub-authority count
...
--*/
{
    ...
    if ((iSid->IdentifierAuthority.Value[0] != 0) || (iSid->IdentifierAuthority.Value[1] != 0)){
        ...
        Tmp = (ULONG) iSid->IdentifierAuthority.Value[5] + (ULONG) (iSid->IdentifierAuthority.Value[4] << 8) + (ULONG) (iSid->IdentifierAuthority.Value[3] << 16) + (ULONG) (iSid->IdentifierAuthority.Value[2] << 24);
...
--*/
{
    ...
    SecurityOperatorTest = EnableSecurityPrivilege();// See if we can play with auditing information
    ...
    // See if the ID is one of the special IDs (e.g., local admin, domain account operator, or domain admin)
...
BOOLEAN ServerTestSuite(PHANDLE ServerHandle,PHANDLE DomainHandle,PHANDLE BuiltinDomainHandle,PSID    *DomainSid)
{
    ...
    if (NT_SUCCESS(NtStatus)) {
        ...
        if (NT_SUCCESS(NtStatus)) {
            ...
            if (AdminsAliasTest) {
                DomainAccessMask |= (DOMAIN_EXECUTE | DOMAIN_READ | DOMAIN_READ_OTHER_PARAMETERS | DOMAIN_ADMINISTER_SERVER     | DOMAIN_CREATE_ALIAS);
            }
//            if (SecurityOperatorTest) {
//                DomainAccessMask |= ACCESS_SYSTEM_SECURITY;
//            }
    ...
    if (Pass == 1) {
        ...
        if (SI1 != 0) {
            ...
            if (NT_SUCCESS(NtStatus)) {
                ...
                // However, we want to save this SD for future reference and use.
...
BOOLEAN DomainTestSuite(HANDLE DomainHandle)
{
    ...
    if (NT_SUCCESS(NtStatus)) {
        if (Buffer != NULL) {
            if ( (((DOMAIN_NAME_INFORMATION *)Buffer)->DomainName.MaximumLength > 0) && (((DOMAIN_NAME_INFORMATION *)Buffer)->DomainName.Buffer != NULL) ) {
                printf("Succeeded\n");
            } else {
                printf("Failed\n");
    ...
    if (NT_SUCCESS(NtStatus)) {
        if (Buffer != NULL) {
            if ( (((DOMAIN_OEM_INFORMATION *)Buffer)->OemInformation.MaximumLength >= 0) && (((DOMAIN_OEM_INFORMATION *)Buffer)->OemInformation.Buffer != NULL) ) {
                printf("Succeeded\n");
            } else {
                printf("Failed\n");
                printf("        String body returned and allocated,\n");
    ...
    if (NT_SUCCESS(NtStatus)) {
        if (Buffer != NULL) {
            if ( (((DOMAIN_REPLICATION_INFORMATION *)Buffer)->ReplicaSourceNodeName.MaximumLength >= 0) && (((DOMAIN_REPLICATION_INFORMATION *)Buffer)->ReplicaSourceNodeName.Buffer != NULL) ) {
                printf("Succeeded\n");
            } else {
                printf("Failed\n");
                printf("        String body returned and allocated,\n");
 ...
 *    NtStatus = SamQueryInformationDomain(DomainHandle,DomainStateInformation,&Buffer1);
 *    ASSERT( NT_SUCCESS(NtStatus) );

 *    // Change the field to a new value and write it out.
 *    if ( ((DOMAIN_STATE_INFORMATION *)Buffer1)->DomainServerState == DomainServerEnabled ) {
 *        ((DOMAIN_STATE_INFORMATION *)Buffer1)->DomainServerState = DomainServerDisabled;
 *    } else {
 *        ((DOMAIN_STATE_INFORMATION *)Buffer1)->DomainServerState = DomainServerEnabled;
 *    }

 *    NtStatus = SamSetInformationDomain(DomainHandle,DomainStateInformation,Buffer1);
 *    if ( NT_SUCCESS(NtStatus) ) {
 *        // Now check that the change was really made...
 *        NtStatus = SamQueryInformationDomain(DomainHandle,DomainStateInformation,&Buffer2);
 *        ASSERT(NT_SUCCESS( NtStatus ) );
 *        if (((DOMAIN_STATE_INFORMATION *)Buffer1)->DomainServerState == ((DOMAIN_STATE_INFORMATION *)Buffer2)->DomainServerState    ) {
 *                printf("Succeeded\n");
 *        } else {
 *            printf("Failed\n");
 ...
 *   // Get the current value...
 *   NtStatus = SamQueryInformationDomain(DomainHandle,DomainServerRoleInformation,&Buffer1);
 *   ASSERT( NT_SUCCESS(NtStatus) );

 *   // Change the field to a new value and write it out.
 *   if ( ((DOMAIN_SERVER_ROLE_INFORMATION *)Buffer1)->DomainServerRole == DomainServerRolePrimary ) {
 *       ((DOMAIN_SERVER_ROLE_INFORMATION *)Buffer1)->DomainServerRole = DomainServerRoleBackup;
 *   } else {
 *       ((DOMAIN_SERVER_ROLE_INFORMATION *)Buffer1)->DomainServerRole = DomainServerRolePrimary;
 *   }

 *   NtStatus = SamSetInformationDomain(DomainHandle,DomainServerRoleInformation,Buffer1);
 *   if ( NT_SUCCESS(NtStatus) ) {
 *       // Now check that the change was really made...
 *       NtStatus = SamQueryInformationDomain(DomainHandle,DomainServerRoleInformation,&Buffer2);
 *       ASSERT(NT_SUCCESS( NtStatus ) );
 *       if (((DOMAIN_SERVER_ROLE_INFORMATION *)Buffer1)->DomainServerRole == ((DOMAIN_SERVER_ROLE_INFORMATION *)Buffer2)->DomainServerRole    ) {
 *               printf("Succeeded\n");
 *       } else {
 *           printf("Failed\n");
 *           printf("        Value queried doesn't match value written\n");
...
*/
    ...
    // Set PasswordProperties to COMPLEX so that tests run after this one are a little more interesting.
    ...
    if ( NT_SUCCESS(NtStatus) ) {
        ...
        ASSERT(NT_SUCCESS( NtStatus ) );
        if (((DOMAIN_PASSWORD_INFORMATION *)Buffer1)->MinPasswordLength == ((DOMAIN_PASSWORD_INFORMATION *)Buffer2)->MinPasswordLength    ) {
                printf("Succeeded\n");
        } else {
            printf("Failed\n");
            printf("        Value queried doesn't match value written\n");
    ...
    if ( NT_SUCCESS(NtStatus) ) {
        ...
        ASSERT(NT_SUCCESS( NtStatus ) );
        if (((DOMAIN_LOGOFF_INFORMATION *)Buffer1)->ForceLogoff.LowPart == ((DOMAIN_LOGOFF_INFORMATION *)Buffer2)->ForceLogoff.LowPart    ) {
                printf("Succeeded\n");
        } else {
            printf("Failed\n");
            printf("        Value queried doesn't match value written\n");
    ...
    if ( NT_SUCCESS(NtStatus) ) {
        ...
        ASSERT(NT_SUCCESS( NtStatus ) );
        if ( (((DOMAIN_LOCKOUT_INFORMATION *)Buffer1)->LockoutDuration.LowPart == ((DOMAIN_LOCKOUT_INFORMATION *)Buffer2)->LockoutDuration.LowPart    ) &&
             (((DOMAIN_LOCKOUT_INFORMATION *)Buffer1)->LockoutObservationWindow.LowPart == ((DOMAIN_LOCKOUT_INFORMATION *)Buffer2)->LockoutObservationWindow.LowPart    ) &&
             (((DOMAIN_LOCKOUT_INFORMATION *)Buffer1)->LockoutThreshold == ((DOMAIN_LOCKOUT_INFORMATION *)Buffer2)->LockoutThreshold    ) ) {
                printf("Succeeded\n");
        } else {
            printf("Failed\n");
            printf("        Value queried doesn't match value written\n");
    ...
    if ( NT_SUCCESS(NtStatus) ) {
        ...
        ASSERT(NT_SUCCESS( NtStatus ) );
        if (((DOMAIN_OEM_INFORMATION *)Buffer1)->OemInformation.Length == ((DOMAIN_OEM_INFORMATION *)Buffer2)->OemInformation.Length    ) {
            printf("Succeeded\n");
        } else {
            printf("Failed\n");
            printf("        Value queried doesn't match value written\n");
    ...
    if ( NT_SUCCESS(NtStatus) ) {
        ...
        ASSERT(NT_SUCCESS( NtStatus ) );
        if (((DOMAIN_REPLICATION_INFORMATION *)Buffer1)->ReplicaSourceNodeName.Length == ((DOMAIN_REPLICATION_INFORMATION *)Buffer2)->ReplicaSourceNodeName.Length    ) {
            printf("Succeeded\n");
        } else {
            printf("Failed\n");
            printf("        Value queried doesn't match value written\n");
    ...
    for ( i=0; i<AccountCount; i++) {
        ...
        if (NT_SUCCESS(NtStatus)) {
            if (Buffer != NULL) {
                if ( ((i >= AccountCount -1) && (NtStatus == STATUS_SUCCESS)) || ((i <= AccountCount -1) && (NtStatus == STATUS_MORE_ENTRIES))  ) {
                    if (CountReturned != 1) {
                        printf("Failed\n");
                        printf("        Completion status is 0x%lx\n", NtStatus);
    ...
    if (NtStatus == STATUS_SOME_NOT_MAPPED) {
        ...
        if ((LookedUpRids[0] == SomeRids[0]) && (LookedUpUses[0] == SomeUses[0]) &&
...
BOOLEAN GroupTestSuite(HANDLE DomainHandle, ULONG  Pass)
{
    ...
    if (Pass == 1) {
        // This test suite assumes that lookup and enumeration API funciton properly.
        printf("\n");
        printf("\n");
        ...
        NtStatus = SamOpenGroup(DomainHandle,GROUP_WRITE_ACCOUNT | GROUP_READ_INFORMATION,DOMAIN_GROUP_RID_USERS,&GroupHandle1);
        ASSERT(NT_SUCCESS(NtStatus) );
        Buffer1 = NULL;
        ...
        printf("      Set Admin Comment . . . . . . . . . . . . . . . . . .     ");
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                ASSERT(NT_SUCCESS( NtStatus ) );
                if (
                    !RtlCompareString((PSTRING)&((GROUP_ADM_COMMENT_INFORMATION *)Buffer1)->AdminComment, (PSTRING)&((GROUP_ADM_COMMENT_INFORMATION *)Buffer2)->AdminComment, TRUE)) {
                    printf("Succeeded\n");
                } else {
                    printf("Failed\n");
                    printf("        Value queried doesn't match value written\n");
    ...
    if (Pass == 2) {
        ...
        // Set the user's primary group Id back and remove the user from the group
...
BOOLEAN AliasTestSuite(HANDLE DomainHandle, HANDLE BuiltinDomainHandle, PSID   DomainSid, ULONG  Pass)
{
    ...
    if (Pass == 1) {
        ...
        if ( NT_SUCCESS(NtStatus) ) {
            ...
            ASSERT(NT_SUCCESS( NtStatus ) );
            if (
                !RtlCompareString((PSTRING)&((ALIAS_ADM_COMMENT_INFORMATION *)Buffer1)->AdminComment, (PSTRING)&((ALIAS_ADM_COMMENT_INFORMATION *)Buffer2)->AdminComment, TRUE)) {
                printf("Succeeded\n");
            } else {
                printf("Failed\n");
                printf("        Value queried doesn't match value written\n");
            ...
            if (NT_SUCCESS(NtStatus)) {
                ...
                if (NT_SUCCESS(NtStatus)) {
                    ...
                    if (MemberCount != 2) {
                        ...
                        if (((Members[0] == AliasRid) && (Members[1] == AliasRid2)) || ((Members[0] == AliasRid2) && (Members[1] == AliasRid)) ) {
                            printf("Succeeded\n");
                        } else {
                            printf("Failed\n");
                            printf("Service returned SUCCESS, but combined alias membership for 2 accounts not correct.\n");
...
//        NtStatus = SamOpenGroup(DomainHandle,GROUP_ALL_ACCESS,DOMAIN_GROUP_RID_USERS,&GroupHandle);
//        ASSERT(NT_SUCCESS(NtStatus));
//        NtStatus = SamRemoveMemberFromGroup(GroupHandle,UserRid3);
//        if ( NT_SUCCESS( NtStatus ) ) {
//            printf("Succeeded\n");
//        } else {
//            printf("Failed\n");
...
//        ASSERT(NT_SUCCESS(IgnoreStatus));
        ...
        // Make user2 a member of the ADMIN alias again, so we can test the new function SamRemoveMemberFromForeignDomain().
...
BOOLEAN UserTestSuite(HANDLE DomainHandle,ULONG Pass)
{
    ...
    if (Pass == 1) {
        ...
        NtStatus = SamOpenUser(DomainHandle,USER_READ_PREFERENCES | USER_READ_GENERAL,DOMAIN_USER_RID_ADMIN,&UserHandle1);
        ASSERT(NT_SUCCESS(NtStatus) );
        Buffer = NULL;
        ...
        NtStatus = SamOpenUser(DomainHandle,USER_READ_ACCOUNT | USER_READ_GENERAL | USER_READ_PREFERENCES | USER_READ_LOGON,DOMAIN_USER_RID_ADMIN,&UserHandle1);
        ASSERT(NT_SUCCESS(NtStatus) );
        Buffer = NULL;
        ...
        if (NT_SUCCESS(NtStatus)) {
            if (Buffer != NULL) {
                if ( (((USER_LOGON_INFORMATION *)Buffer)->UserName.MaximumLength > 0)       && (((USER_LOGON_INFORMATION *)Buffer)->UserName.Buffer != NULL)) {
                    ...
                    printf("        last Logon is:    (0x%lx, 0x%lx)\n", (((USER_LOGON_INFORMATION *)Buffer)->LastLogon.HighPart), (((USER_LOGON_INFORMATION *)Buffer)->LastLogon.LowPart)  );
        ...
        NtStatus = SamOpenUser(DomainHandle,USER_READ_GENERAL | USER_READ_PREFERENCES |USER_READ_LOGON   | USER_READ_ACCOUNT,DOMAIN_USER_RID_ADMIN,&UserHandle1);
        ASSERT(NT_SUCCESS(NtStatus) );
        Buffer = NULL;
        ...
        if (NT_SUCCESS(NtStatus)) {
            if (Buffer != NULL) {
                if ( (((USER_ACCOUNT_INFORMATION *)Buffer)->UserName.MaximumLength > 0)       && (((USER_ACCOUNT_INFORMATION *)Buffer)->UserName.Buffer != NULL)) {
                    ...
                    printf("        last Logon is:    (0x%lx, 0x%lx)\n", (((USER_ACCOUNT_INFORMATION *)Buffer)->LastLogon.HighPart), (((USER_ACCOUNT_INFORMATION *)Buffer)->LastLogon.LowPart)  );
                    printf("       last Logoff is:    (0x%lx, 0x%lx)\n", (((USER_ACCOUNT_INFORMATION *)Buffer)->LastLogoff.HighPart), (((USER_ACCOUNT_INFORMATION *)Buffer)->LastLogoff.LowPart)  );
                    printf("      Pwd Last Set is:    (0x%lx, 0x%lx)\n", (((USER_ACCOUNT_INFORMATION *)Buffer)->PasswordLastSet.HighPart), (((USER_ACCOUNT_INFORMATION *)Buffer)->PasswordLastSet.LowPart)  );
...
//        }
        ...
        if ( NtStatus == STATUS_INVALID_INFO_CLASS ) {
            // We're not a trusted client, so we don't expect to be able to do this.
            printf("Succeeded.\n");
        } else {
            printf("Failed\n");
            printf("        Completion status is 0x%lx\n", NtStatus);
...
//            if (Buffer != NULL) {
//                printf("Succeeded\n");
//                printf("        last Logon is:    (0x%lx, 0x%lx)\n", (((USER_INTERNAL2_INFORMATION *)Buffer)->LastLogon.HighPart), (((USER_INTERNAL2_INFORMATION *)Buffer)->LastLogon.LowPart)  );
//                printf("       last Logoff is:    (0x%lx, 0x%lx)\n", (((USER_INTERNAL2_INFORMATION *)Buffer)->LastLogoff.HighPart), (((USER_INTERNAL2_INFORMATION *)Buffer)->LastLogoff.LowPart)  );
...
//        }
        ...
        printf("      Set Preferences Information . . . . . . . . . . . . .     ");
            NtStatus = SamOpenUser(DomainHandle,USER_READ_GENERAL | USER_WRITE_PREFERENCES | USER_READ_PREFERENCES,DOMAIN_USER_RID_ADMIN,&UserHandle1);
            ASSERT(NT_SUCCESS(NtStatus) );
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                ASSERT(NT_SUCCESS( NtStatus ) );
                if (!RtlCompareString((PSTRING)&((USER_PREFERENCES_INFORMATION *)Buffer1)->UserComment, (PSTRING)&((USER_PREFERENCES_INFORMATION *)Buffer2)->UserComment, TRUE) &&
                        (((USER_PREFERENCES_INFORMATION *)Buffer1)->CountryCode == ((USER_PREFERENCES_INFORMATION *)Buffer2)->CountryCode) &&
                        (((USER_PREFERENCES_INFORMATION *)Buffer1)->CodePage == ((USER_PREFERENCES_INFORMATION *)Buffer2)->CodePage)) {
                    printf("Succeeded\n");
        ...
        printf("      Set Logon Hours Information . . . . . . . . . . . . .     ");
            ...
            TST_SUCCESS_ASSERT(NtStatus);
            ASSERT(Buffer1 != NULL);
            ASSERT( ((USER_LOGON_HOURS_INFORMATION *)Buffer1)->LogonHours.LogonHours != NULL);  //Don't support zero length bit masks in this test yet.
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                ASSERT(NT_SUCCESS( NtStatus ) );
                if (((USER_LOGON_HOURS_INFORMATION *)Buffer1)->LogonHours.LogonHours[LOGON_HOURS_DIFFERENT_OFFSET] == ((USER_LOGON_HOURS_INFORMATION *)Buffer2)->LogonHours.LogonHours[LOGON_HOURS_DIFFERENT_OFFSET]) {
                    printf("Succeeded\n");
                } else {
                    printf("Failed\n");
                    printf("        Value queried doesn't match value written\n");
                    ...
                    printf("        Byte 0x%lx of the written bit mask is    0x%lx\n", LOGON_HOURS_DIFFERENT_OFFSET, (ULONG)((USER_LOGON_HOURS_INFORMATION *)Buffer1)->LogonHours.LogonHours[LOGON_HOURS_DIFFERENT_OFFSET]);
        ...
        NtStatus = SamOpenUser(DomainHandle,USER_WRITE_ACCOUNT        |USER_READ_GENERAL     |USER_READ_PREFERENCES |USER_READ_LOGON,DOMAIN_USER_RID_ADMIN,&UserHandle1);
        ASSERT(NT_SUCCESS(NtStatus) );
        ...
        printf("      Set Home  . . . . . . . . . . . . . . . . . . . . . .     ");
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                if (!RtlCompareString((PSTRING)&((USER_HOME_INFORMATION *)Buffer1)->HomeDirectory,(PSTRING)&((USER_HOME_INFORMATION *)Buffer2)->HomeDirectory,TRUE) ) {
                    if (!RtlCompareString((PSTRING)&((USER_HOME_INFORMATION *)Buffer1)->HomeDirectoryDrive,(PSTRING)&((USER_HOME_INFORMATION *)Buffer2)->HomeDirectoryDrive,TRUE)) {
                        printf("Succeeded\n");
                    } else {
                        printf("Failed\n");
        ...
        printf("      Set Script  . . . . . . . . . . . . . . . . . . . . .     ");
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                ASSERT(NT_SUCCESS( NtStatus ) );
                if (!RtlCompareString((PSTRING)&((USER_SCRIPT_INFORMATION *)Buffer1)->ScriptPath,(PSTRING)&((USER_SCRIPT_INFORMATION *)Buffer2)->ScriptPath,TRUE)) {
                    printf("Succeeded\n");
                } else {
                    printf("Failed\n");
                    printf("        Value queried doesn't match value written\n");
        ...
        printf("      Set Profile . . . . . . . . . . . . . . . . . . . . .     ");
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                ASSERT(NT_SUCCESS( NtStatus ) );
                if (!RtlCompareString((PSTRING)&((USER_PROFILE_INFORMATION *)Buffer1)->ProfilePath,(PSTRING)&((USER_PROFILE_INFORMATION *)Buffer2)->ProfilePath,TRUE)) {
                    printf("Succeeded\n");
                } else {
                    printf("Failed\n");
                    printf("        Value queried doesn't match value written\n");
        ...
        printf("      Set Admin Comment . . . . . . . . . . . . . . . . . .     ");
            NtStatus = SamOpenUser(DomainHandle,USER_WRITE_ACCOUNT | USER_READ_GENERAL,DOMAIN_USER_RID_ADMIN,&UserHandle1);
            ASSERT(NT_SUCCESS(NtStatus) );
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                ASSERT(NT_SUCCESS( NtStatus ) );
                if (!RtlCompareString((PSTRING)&((USER_ADMIN_COMMENT_INFORMATION *)Buffer1)->AdminComment,(PSTRING)&((USER_ADMIN_COMMENT_INFORMATION *)Buffer2)->AdminComment,TRUE)) {
                    printf("Succeeded\n");
                } else {
                    printf("Failed\n");
                    printf("        Value queried doesn't match value written\n");
        ...
        printf("BROKEN TEST - NOT TESTED\n");
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                ...
                ASSERT(NT_SUCCESS( NtStatus ) );
                if (!RtlCompareString((PSTRING)&((USER_WORKSTATIONS_INFORMATION *)Buffer1)->WorkStations,(PSTRING)&((USER_WORKSTATIONS_INFORMATION *)Buffer2)->WorkStations,TRUE)) {
                    printf("Succeeded\n");
                } else {
                    printf("Failed\n");
                    printf("        Value queried doesn't match value written\n");
        ...
        printf("      Set Internal1   . . . . . . . . . . . . . . . . . . .     ");
            NtStatus = SamOpenUser(DomainHandle,USER_WRITE_ACCOUNT | USER_READ_LOGON | USER_FORCE_PASSWORD_CHANGE,DOMAIN_USER_RID_ADMIN,&UserHandle1);
            ASSERT(NT_SUCCESS(NtStatus) );

            // We can't get the current values, since this level is only queryable by trusted clients.  So just try setting a couple
...
//            Buffer1 = NULL;
//            NtStatus = SamQueryInformationUser(UserHandle1,UserInternal1Information,&Buffer1);
//            TST_SUCCESS_ASSERT(NtStatus);
//            ASSERT(Buffer1 != NULL);
...
//            NtStatus = RtlCalculateNtOwfPassword((PNT_PASSWORD)&DummyName1,&EncryptedPasswordBuffer);
...
//            NtStatus = RtlCalculateNtOwfPassword((PNT_PASSWORD)&DummyName2,&EncryptedPasswordBuffer2);
...
//            if ( NT_SUCCESS(NtStatus) ) {
//                // Now check that the change was really made...
//                NtStatus = SamQueryInformationUser(UserHandle1,UserInternal1Information,&Buffer2);
//                ASSERT(NT_SUCCESS( NtStatus ) );

//                if ( (!RtlCompareString((PSTRING)&((USER_INTERNAL1_INFORMATION *)Buffer1)->CaseSensitiveUnicode,(PSTRING)&((USER_INTERNAL1_INFORMATION *)Buffer2)->CaseSensitiveUnicode,TRUE)) || (
//                    !RtlCompareString((PSTRING)&((USER_INTERNAL1_INFORMATION *)Buffer1)->CaseInsensitiveDbcs,(PSTRING)&((USER_INTERNAL1_INFORMATION *)Buffer2)->CaseInsensitiveDbcs,TRUE)) ) {
//                    printf("Succeeded\n");
//                } else {
//                    printf("Failed\n");
//                    printf("        Value queried doesn't match value written\n");
...
//            }
        ...
        printf("      Set Internal2   . . . . . . . . . . . . . . . . . . .     ");
            ...
            // We can't get the current values, since this level is only queryable by trusted clients.  We can't set either, but
...
//            Buffer1 = NULL;
//            NtStatus = SamQueryInformationUser(UserHandle1,UserInternal2Information,&Buffer1);
//            TST_SUCCESS_ASSERT(NtStatus);
//            ASSERT(Buffer1 != NULL);
...
//            if ( NT_SUCCESS(NtStatus) ) {
//                // Now check that the change was really made...
//                NtStatus = SamQueryInformationUser(UserHandle1,UserInternal2Information,&Buffer2);
//                ASSERT(NT_SUCCESS( NtStatus ) );
//                if (
//                    (((USER_INTERNAL2_INFORMATION *)Buffer1)->LastLogon.HighPart == ((USER_INTERNAL2_INFORMATION *)Buffer2)->LastLogon.HighPart) &&
//                    (((USER_INTERNAL2_INFORMATION *)Buffer1)->LastLogon.LowPart == ((USER_INTERNAL2_INFORMATION *)Buffer2)->LastLogon.LowPart) &&
//                    (((USER_INTERNAL2_INFORMATION *)Buffer1)->LastLogoff.HighPart == ((USER_INTERNAL2_INFORMATION *)Buffer2)->LastLogoff.HighPart) &&
//                    (((USER_INTERNAL2_INFORMATION *)Buffer1)->LastLogoff.LowPart == ((USER_INTERNAL2_INFORMATION *)Buffer2)->LastLogoff.LowPart) &&
//                    (((USER_INTERNAL2_INFORMATION *)Buffer1)->BadPasswordCount == ((USER_INTERNAL2_INFORMATION *)Buffer2)->BadPasswordCount) &&
//                    (((USER_INTERNAL2_INFORMATION *)Buffer1)->LogonCount == ((USER_INTERNAL2_INFORMATION *)Buffer2)->LogonCount)
//                     ) {
//                    printf("Succeeded\n");
//                } else {
//                    printf("Failed\n");
...
//            }
        ...
        printf("      Set Password  . . . . . . . . . . . . . . . . . . . .     ");
            ...
            if ( NT_SUCCESS(NtStatus) ) {
                // We can't verify that it really worked, so we just have to trust the return code.
                printf("Succeeded\n");
            } else {
                printf("Failed\n");
        ...
        printf("      Set Control . . . . . . . . . . . . . . . . . . . . .     ");
            NtStatus = SamOpenUser(DomainHandle,USER_WRITE_ACCOUNT | USER_READ_ACCOUNT,DOMAIN_USER_RID_ADMIN,&UserHandle1);
            ASSERT(NT_SUCCESS(NtStatus) );
        ...
        printf("BROKEN TEST - NOT TESTED\n");
            ...
            if (NT_SUCCESS(NtStatus)) {
                ...
                TST_SUCCESS_ASSERT(NtStatus);
                ASSERT(Buffer2 != NULL);
                if ( ( ((USER_EXPIRES_INFORMATION *)Buffer1)->AccountExpires.LowPart  == ((USER_EXPIRES_INFORMATION *)Buffer2)->AccountExpires.LowPart )  &&
                     ( ((USER_EXPIRES_INFORMATION *)Buffer1)->AccountExpires.HighPart  == ((USER_EXPIRES_INFORMATION *)Buffer2)->AccountExpires.HighPart ) ) {
                    printf("Succeeded\n");
                    SamFreeMemory( Buffer2 );
        ...
        // The current password is DummyName2.  Using DummyName2 as the old password, change it to DummyName1 and make sure we get STATUS_SUCCESS.
        ...
        // the old password, try to change it to DummyName2 and make sure it doesn't succeed.
        if ( NtStatus == STATUS_SUCCESS ) {
        ...
        // The current password is DummyName1.  Using DummyName1 as the old password, change it to DummyName2 and make sure it works since by default there is no password history.
        if ( NtStatus == STATUS_SUCCESS ) {
        ...
        if ( NT_SUCCESS( NtStatus ) ) {
            ...
            if ( NT_SUCCESS( NtStatus ) ) {
                ...
                if ( NT_SUCCESS( NtStatus ) ) {
                    ...
                    if (
                        // Fields that we didn't touch.
                        // Note that PasswordMustChange changed anyway, since we changed from a null to a non-null password.
                        ( All2->WhichFields != (USER_ALL_READ_GENERAL_MASK    | USER_ALL_READ_PREFERENCES_MASK | USER_ALL_READ_ACCOUNT_MASK     | USER_ALL_READ_LOGON_MASK) ) ||
                        ...
                        (RtlCompareUnicodeString(&(All->HomeDirectoryDrive),&(All2->HomeDirectoryDrive),FALSE) != 0) ||
