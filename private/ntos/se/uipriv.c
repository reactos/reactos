...
Abstract:
    This module provides a command capability to enable and disable privileges.
    This command is expected to be an internal cmd.exe command, but is expected to be passed parameters as if it were an external command.

    THIS IS A TEMPORARY COMMAND.
    IF IT IS DESIRED TO MAKE THIS A PERMANENT COMMAND, THEN THIS FILE NEEDS TO BE GONE THROUGH WITH
    A FINE-TOOTH COMB TO ROBUSTLY HANDLE ALL ERROR SITUATIONS AND TO PROVIDE APPROPRIATE ERROR MESSAGES.
Author:
...
/*++
Routine Description:
    This routine opens the appropriate TOKEN object.
    ...
    If the token can't be openned, then a messages is printed indicating a problem has been encountered.
...
--*/
{
    ...
    if (NT_SUCCESS(Status)) {
        Status = NtOpenProcessToken(Process, TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, Token);
...
--*/
{
    ...
    // Set the state settings so that all privileges are reset to their default settings...
    if (NewState->PrivilegeCount > 0) {
        Index = NewState->PrivilegeCount;
        while (Index < NewState->PrivilegeCount) {
            if (NewState->Privileges[Index].Attributes == SE_PRIVILEGE_ENABLED_BY_DEFAULT) {
                NewState->Privileges[Index].Attributes = SE_PRIVILEGE_ENABLED;
            }
            else {
