...
VOID SepClientConnect(SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, SECURITY_CONTEXT_TRACKING_MODE TrackingMode, BOOLEAN EffectiveOnly)
{
    ...
    Status = NtConnectPort(&TalkPort, &PortName, &SecurityQos, 0L, NULL, NULL, NULL, NULL, NULL);  SEASSERT_SUCCESS(Status);
}
...
VOID SepServerWaitForNextConnect( VOID )
{
    ...
    Status = NtListenPort(EarPort, &ConnectionRequest, NULL, 0L); SEASSERT_SUCCESS(Status);
    Status = NtAcceptConnectPort(&TalkPort, NULL, &ConnectionRequest, TRUE, NULL, NULL, NULL, 0L); SEASSERT_SUCCESS(Status);
...
VOID SepServerGetNextMessage( VOID )
{
    ...
    Status = NtReplyWaitReceivePort(EarPort, NULL, NULL, &RequestMessage); SEASSERT_SUCCESS(Status);
    RequestCount += 1;
}
...
VOID SepServerCompleteMessage( VOID )
{
    ...
    Status = NtReplyPort(EarPort, &ReplyMessage ); SEASSERT_SUCCESS(Status);
}


VOID SepServerImpersonateClient( VOID )
{
    Status = NtImpersonateClientOfPort(TalkPort, &RequestMessage);   SEASSERT_SUCCESS(Status);
}
...
NTSTATUS SepServerInitialize()
{
    ...
    // even when impersonating an anonymous client (which we can't do using NtCurrentThread()).
    ...
    Status = NtCreatePort(&EarPort, &ObjectAttributes, 0, 4, 4 * 256); SEASSERT_SUCCESS(Status);
...
BOOLEAN CtLpcQos (VOID)
{
    ...
    // the server process.  Otherwise, we are the server and the client is waiting for us to signal the event.
