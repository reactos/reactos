...
--*/
{
    ...
    SubjectContext->ClientToken = PsReferenceImpersonationToken(PsGetCurrentThread(), &IgnoreCopyOnOpen, &IgnoreEffectiveOnly, &(SubjectContext->ImpersonationLevel));
...
/*++
    Routine Description:
        This routine returns the authentication ID for the effective token in a subject context
    Parameters:
...
--*/
{
    ...
    Status = SeQueryAuthenticationIdToken(EffectiveToken(SubjectContext), AuthenticationId);
