...
--*/
{
    ...
    if (Component->Class != SystemClass) {
        ...
        ObjectAttributes.Attributes |= OBJ_CASE_INSENSITIVE;
        Status = NtCreateKey(&Handle, KEY_READ | KEY_WRITE, &ObjectAttributes, TITLE_INDEX_VALUE, &(CmClassName[Component->Class]), 0, &Disposition);
        NtClose(OldHandle);
        if (!NT_SUCCESS(Status)) {
            return(Status);
        }
    ...
    Status = NtSetValueKey(
                ...
                REG_BINARY,
                &Component->Flags,
                FIELD_OFFSET(CONFIGURATION_COMPONENT, ConfigurationDataLength) - FIELD_OFFSET(CONFIGURATION_COMPONENT, Flags)
                );
    if (!NT_SUCCESS(Status)) {
        NtClose(Handle);
        return(Status);
    ...
    if (CurrentEntry->ConfigurationData) {
        ...
        if (ConfigurationDataLength > CmpConfigurationAreaSize) {
            ...
            if (NewArea) {
                ...
                RtlMoveMemory((PUCHAR)&CmpConfigurationData->PartialResourceList.Version, CurrentEntry->ConfigurationData, Component->ConfigurationDataLength);
            } else {
