...
/*++
Routine Description:
    Actually execute the command specified in CommandArea, and report its completion.
Arguments:
    Parameter - supplies a pointer to the REGISTRY_COMMAND structure to be executed.
Return Value:
--*/
{
    NTSTATUS Status;
    ...
    switch (CommandArea->Command) {
        ...
        case REG_CMD_FLUSH_KEY:
            CommandArea->Status = CmFlushKey(CommandArea->Hive, CommandArea->Cell);
            break;
        ...
        case REG_CMD_FILE_SET_SIZE:
            CommandArea->Status = CmpDoFileSetSize(CommandArea->Hive, CommandArea->FileType, CommandArea->FileSize);
            break;
        case REG_CMD_HIVE_OPEN:
            ...
            if (((CommandArea->Status == STATUS_ACCESS_DENIED) ||
                ...
                Status = SeImpersonateClientEx(CommandArea->ImpersonationContext, NULL);
        ...
        case REG_CMD_SHUTDOWN:            
            CmpDoFlushAll();// shut down the registry
        ...
        case REG_CMD_RENAME_HIVE:
            ...
            if (CommandArea->OldName != NULL) {
                ...
                Status = ZwQueryObject(Handle, ObjectNameInformation, CommandArea->OldName, CommandArea->NameInfoLength, &CommandArea->NameInfoLength);
                if (!NT_SUCCESS(Status)) {
            ...
            RenameInfo = ExAllocatePool(PagedPool, sizeof(FILE_RENAME_INFORMATION) + CommandArea->NewName->Length);
            if (RenameInfo == NULL) {
            ...
            RtlMoveMemory(RenameInfo->FileName, CommandArea->NewName->Buffer, CommandArea->NewName->Length);
...
--*/
{
    ...
    if (!CmpNoWrite) {
        DueTime.QuadPart = Int32x32To64(LAZY_FLUSH_INTERVAL_IN_SECONDS, - SECOND_MULT);        
        KeSetTimer(&CmpLazyFlushTimer, DueTime, &CmpLazyFlushDpc);// Indicate relative time
    }
}


VOID CmpLazyFlushDpcRoutine(
    IN PKDPC Dpc,
    IN PVOID DeferredContext,
    IN PVOID SystemArgument1,
    IN PVOID SystemArgument2
...
/*++
Routine Description:
    Worker routine called to do a lazy flush.  Called by an executive worker thread in the system process.
Arguments:
    Parameter - not used.
--*/
{
...
Arguments:
    WorkItem - Supplies pointer to the work item. This routine will free the work item.
--*/
{
    NTSTATUS Status;
    ULONG Response;
    ...
    Status = ExRaiseHardError(STATUS_DISK_FULL, 0, 0, NULL, OptionOk, &Response);
}
...
--*/
{
    ...
    if( (!CmpDiskFullWorkerPopupDisplayed) && (CmpCannotWriteConfiguration) && (ExReadyForErrors) && (CmpProfileLoaded) ) {
        ...
        if (WorkItem != NULL) {
            ...
            ExInitializeWorkItem(WorkItem, CmpDiskFullWarningWorker, WorkItem);
