...
Abstract:
    This module contains support for changing the Hardware profile based on the current docking state, either at boot time or by ACPI dock.
...
--*/
{
    ...
    * ProfileList = ExAllocatePool (PagedPool, len);
    ...
    for (i = 0; i < keyInfo.SubKeys; i++) {
        ...
        status = ZwEnumerateKey (profiles, i, KeyBasicInformation, basicInfo, bufferLen - sizeof (UNICODE_NULL), // term 0
                                 &len);
        if (!NT_SUCCESS (status)) {
        ...
        for (j=0; j < (*ProfileList)->CurrentProfileCount; j++) {
            if ((*ProfileList)->Profile[j].PreferenceOrder >=
                ...
                RtlMoveMemory(&(*ProfileList)->Profile[j+1], &(*ProfileList)->Profile[j], sizeof(CM_HARDWARE_PROFILE) * ((*ProfileList)->MaxProfileCount-j-1));
                break;
            }
        }
    ...
    ASSERT (0 < keyInfo.SubKeys);
    * AliasList = ExAllocatePool (PagedPool, sizeof (CM_HARDWARE_PROFILE_LIST) + (sizeof (CM_HARDWARE_PROFILE) * (keyInfo.SubKeys - 1)));
    if (NULL == *AliasList) {
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto Clean;
    }
    ...
    for (i = 0; i < keyInfo.SubKeys; i++) {
        // Get the first key in the list.
        status = ZwEnumerateKey (acpiAlias, i, KeyBasicInformation, basicInfo, bufferLen - sizeof (UNICODE_NULL), // term 0
                                 &len);
        if (!NT_SUCCESS (status)) {
            // This should never happen.
            break;
    ...
    while (aliasNumber < 200) {
        ...
        if (NT_SUCCESS (status)) {
            if (PreventDuplication) {
                ...
                RtlInitUnicodeString (&name, CM_HARDWARE_PROFILE_STR_DOCKING_STATE);
                status = NtQueryValueKey(aliasEntry, &name, KeyValueFullInformation, valueBuffer, valueBufferLength, &len);
                ...
                if (!RtlEqualMemory (NewDockState->SerialNumber, ((PUCHAR) keyInfo + keyInfo->DataOffset), NewDockState->SerialLength)) {
                    // Not a dupe
                    NtClose (aliasEntry);
                    aliasEntry = NULL;
                    continue;
...
Routine Description:
    ...
    Based on the new change calculate the new HW Profile(s) consitent with the new ACPI docking state.
    Pass the list of known profiles to the callers selection routine.
    Set the new current profile.
    Patch up any ACPI alias entries if a new profile for this ACPI state has been used.
Arguments:
    NewDockStateArray - The list of possible Docking States that we might enter.
    Select - Call back to select which profile to enter, given the list of possible profiles.
--*/
{
    NTSTATUS        status = STATUS_SUCCESS;
    ...
    *ProfileChanged = FALSE;
    ...
    status = CmpGetAcpiProfileInformation (IDConfigDB, &ProfileList, &AliasList, nameBuffer, valueBuffer, sizeof (valueBuffer));
    if (!NT_SUCCESS (status)) {
        goto Clean;
    }
    ...
    status = CmpFilterAcpiDockingState (NewDockState, currentDockingState, currentAcpiSN, currentProfileNumber, ProfileList, AliasList);
    if (!NT_SUCCESS (status)) {
        goto Clean;
    }
    ...
    // If the user selected -1 then he is not interested in selecting any of the profiles.
    ...
    if (flags & CM_HP_FLAGS_DUPLICATE) {
        // If there is a duplicate then we need to adjust the pnp bios alias table.

        // This happens if we booted PnP bios detected docked, and then we received a set state for ACPI as docked, then we have the
        // potential for duplicates.  See Comment in CmpFilterAcpiDockingState for details.

        // We need to find any pnp bios alias entries that match the current state and point them to the duplicate entry.
        ASSERT (flags & CM_HP_FLAGS_TRUE_MATCH);
        ASSERT (!(flags & CM_HP_FLAGS_PRISTINE));

        status = CmpMoveBiosAliasTable (IDConfigDB, currentInfo, currentProfileNumber, profileNum, nameBuffer, valueBuffer, sizeof (valueBuffer));
        if (!NT_SUCCESS (status)) {
            goto Clean;
        }
    }
    ...
    if ((flags & CM_HP_FLAGS_PRISTINE) || (profileNum != currentProfileNumber)){
        ...
        *ProfileChanged = TRUE;
        ...
        if (flags & CM_HP_FLAGS_PRISTINE) {
            ...
            ASSERT (!(flags & CM_HP_FLAGS_TRUE_MATCH));
            status = CmpCloneHwProfile (IDConfigDB, parent, HardwareProfile, profileNum, NewDockState->DockingState, &HardwareProfile, &profileNum);
            if (!NT_SUCCESS (status)) {
                HardwareProfile = 0;
                goto Clean;
            }
        ...
        RtlInitUnicodeString (&name, CM_HARDWARE_PROFILE_STR_CURRENT_DOCK_INFO);
        InitializeObjectAttributes (&attributes, &name, OBJ_CASE_INSENSITIVE, IDConfigDB, NULL);
        status = NtCreateKey (&currentInfo, KEY_READ | KEY_WRITE, &attributes, 0, NULL, REG_OPTION_VOLATILE, &disposition);
        if (!NT_SUCCESS (status)) {
            currentInfo = NULL;
            goto Clean;
        }
    ...
    if (!(flags & CM_HP_FLAGS_TRUE_MATCH)) {
        ...
        status = CmpAddAcpiAliasEntry (IDConfigDB, NewDockState, profileNum, nameBuffer, valueBuffer, sizeof (valueBuffer), FALSE); // Don't Prevent Duplication
    }
    ...
    if (profileNum != currentProfileNumber) {
        ...
        status = NtCreateKey(&currentSymLink, KEY_CREATE_LINK, &attributes, 0, NULL, REG_OPTION_OPEN_LINK, &disposition);
...
/*++
Routine Description:
    Given the new state of things and the current state of things, prune the given list of profiles.
--*/
{
    NTSTATUS    status = STATUS_SUCCESS;
    ULONG i = 0;
    ...
    // If the user boots undocked, and then hot docks.  We will generate a profile alias for the pnp reported undocked state [A], and one for the
    // ACPI reported docked state [B}.  If the use subsequently reboots docked, then we will create a third pnp reported docked state [C] profile alias.
    // {C] is really a duplicate of [B}, but we wont know this until such time as the ACPI state for {B] is reported.

    // The same can happen for undocked scenerios.

    // Detection: If the Current Dock State is the same as NewDockingState.DockingState then there is a potential for a duplicate.
    // In order to also have a duplicate we must have an acpi already pointing to a profile different than the current one.
    // This must also be the first ACPI change since we booted, therefore CurrentAcpiSn Should be Zero.
    // In other words there must be at least one true match and none of the true matches can point to the current profile.

    if (AliasList) {
        while (i < AliasList->CurrentAliasCount) {
            alias = &AliasList->Alias[i];

            if (((alias->DockState & mask) != 0) && ((alias->DockState & mask) != (NewDockingState->DockingState & mask))) {
                // This alias claims to be docked or undocked, but does not match the current state.  Therefore skip it.
                ;
            } else if (alias->SerialLength != NewDockingState->SerialLength) {
                // This alias has an incompatible serial number
                ;
            } else if (alias->SerialLength == RtlCompareMemory (NewDockingState->SerialNumber, alias->SerialNumber, alias->SerialLength)) {
                // NB RtlCompareMemory can work with zero length memory addresses.  This is a requirement here.
                ...
                for (j = 0; j < ProfileList->CurrentProfileCount; j++) {
                    ...
                    if ((CurrentDockState == NewDockingState->DockingState) && (NULL == CurrentAcpiSN)) {
                        // The dock state did not change during this acpi event; therefore, we might just have a duplicate on our hands.
                        dupDetect = TRUE;
                    }
                    if (alias->ProfileNumber == CurrentProfileNumber) {
                        ...
                        // Profile.  Therefore, we should chose this one, and ignore the duplicate.
                        currentListed = TRUE;
                    }
                }
            }
    ...
    if ((!dupDetect) && (NULL == CurrentAcpiSN) && (!trueMatch) && (CurrentDockState == NewDockingState->DockingState)) {
        ...
        // (2) the current profile has not yet, on this boot, been marked with an ACPI serial number.
        // (3) There was no Alias match.

        // Therefore we should keep the current profile regardless of it being aliasable.
        keepCurrent = TRUE;
    ...
    while (i < ProfileList->CurrentProfileCount) {
            ...
            if (flags & CM_HP_FLAGS_TRUE_MATCH) {
                if (currentListed) {
                    if (ProfileList->Profile[i].Id == CurrentProfileNumber) {
                        // Let this one live.  This results in no change of profile number.
                        i++;
                        continue;
                    }

                    // Bounce any true matches that do not result in no change of profile.
                    ;
                } else {
                    // We did not find the current one listed so we definately have a duplicate.
        ...
        // discard this profile by (1) shifting remaining profiles in array to fill in the space of this discarded profile and (2) decrementing profile count
...
/*++
Routine Description:
    Search the Alias table for bios entries which match the current docking state, and point from current profile number to new profile number.

    Assumption: If the profile is cloned (therefore created by CmpCloneHwProfile, and we have just moved the bios table to point
    away from this entry, then we *should* be able to safely delete the old hardware profile key.
...
--*/
{
    ...
    for (i = 0; i < keyInfo.SubKeys; i++) {
        ...
        // If this profile is cloned then we created it and can therefore get rid of it.
        RtlInitUnicodeString (&name, CM_HARDWARE_PROFILE_STR_HARDWARE_PROFILES);
        InitializeObjectAttributes (&attributes, &name, OBJ_CASE_INSENSITIVE, IDConfigDB, NULL);
...
Return:
    STATUS_SUCCESS - if the profile has been cloned, in which case the new
        profile key has been opened for read / write privs.  The old profile will be closed.
    <unsuccessful> - for a given error.  NewProfile is invalid and the Old Profile has also been closed.
...
--*/
{
    ...
    *NewProfileNumber = OldProfileNumber;
    ...
    if (STATUS_BUFFER_TOO_SMALL == status) {
        ...
        if (security != NULL) {
            status = NtQuerySecurityObject(OldProfile, DACL_SECURITY_INFORMATION, security, securityLength, &securityLength);
            if (!NT_SUCCESS (status)) {
                CMLOG(CML_BUGCHECK, CMS_INIT) {
    ...
    // Check to make sure the key was created.  If it already exists, something is wrong.
    ...
    // greatest PreferenceOrder value of all the current profiles; assumes current set of PreferenceOrder values is incremental)
    ...
    // At very least, the Pristine and the new profile key we just created, should be there.
    ...
    for (i = 0; i < profileSubKeys; i++) {
        ...
        status = ZwEnumerateKey (hardwareProfiles, i, KeyBasicInformation, valueBuffer, sizeof(valueBuffer) - sizeof (UNICODE_NULL), //term 0
                                 &length);
        if(!NT_SUCCESS(status)) {
            break;
        }
        ...
        RtlInitUnicodeString (&name, CM_HARDWARE_PROFILE_STR_PREFERENCE_ORDER);
        status = NtQueryValueKey(profileEntry, &name, KeyValueFullInformation, valueBuffer, sizeof(valueBuffer), &length);
        ...
        // this PreferenceOrder, OR assign it this valid PreferenceOrder if value is still unassigned.
    ...
    // (If no other profiles were found, (value+=1) == 0, the most preferred profile)
    value += 1;
    ...
    if (NT_SUCCESS(status)) {
        RtlInitUnicodeString (&name, CM_HARDWARE_PROFILE_STR_FRIENDLY_NAME);
        status = NtSetValueKey (IDConfigDBEntry, &name, 0, REG_SZ, friendlyName.Buffer, friendlyName.Length + sizeof(UNICODE_NULL));
    ...
    // (Copied lovingly from CmpCloneControlSet)
    status = ObReferenceObjectByHandle (OldProfile, KEY_READ, CmpKeyObjectType, KernelMode, (PVOID *)(&oldProfileKey), NULL);
    if (!NT_SUCCESS(status)) {
        CMLOG(CML_BUGCHECK, CMS_INIT) {
    ...
    ObReferenceObjectByHandle (*NewProfile, KEY_WRITE, CmpKeyObjectType, KernelMode, (PVOID *)(&newProfileKey), NULL);
    if (!NT_SUCCESS(status)) {
        CMLOG(CML_BUGCHECK, CMS_INIT) {
            KdPrint(("CM: CmpCloneHWProfile: couldn't reference CurrentHandle %08lx\n", status));
        }
    ...
    // are things like "pristine" which we do not wish to have moved to the new tree.
    if (CmpCopyTree(oldProfileKey->KeyControlBlock->KeyHive, oldProfileKey->KeyControlBlock->KeyCell, newProfileKey->KeyControlBlock->KeyHive, newProfileKey->KeyControlBlock->KeyCell)) {
        // Set the max subkey name property for the new target key.
        newProfileKey->KeyControlBlock->KeyNode->MaxNameLen = oldProfileKey->KeyControlBlock->KeyNode->MaxNameLen;
        status = STATUS_SUCCESS;
    } else {
        CMLOG(CML_BUGCHECK, CMS_INIT) {
...
/*++
Routine Description:
    Routine to recursively delete all subkeys under the given key, including the key given.
Arguments:
    hKeyRoot:    Handle to root relative to which the key to be deleted is specified.
    Key:         Root relative path of the key which is to be recursively deleted.
    ThisKeyToo:  Whether after deletion of all subkeys, this key itself is to be deleted.
Return Value:
    Status is returned.
--*/
{
    ...
    // be deleted first.  since deleting the subkey affects the subkey index, we always enumerate on subkeyindex 0
    while(1) {
        ...
        // in TemporaryBuffer, which might get clobbered by recursive calls to this routine.
    ...
    // STATUS_NO_MORE_ENTRIES we failed in deleting some subkey, so we cannot delete this key too
...
Routine Description:
    ...
    If a new profile name based on the DockState cannot be created, an attempt is made to create a default FriendlyName based on NewProfileNumber.
    If successful, a unicode string with the new profile friendlyName is created.
    It is the responsibility of the caller to free this using RtlFreeUnicodeString.  If unsuccesful, no string is returned.
Arguments:
...
Notes:
    The new FriendlyName is generated from the DockState and appropriate counter, and may not necessarily be unique among the existing Hardware Profiles.

    The naming scheme used here (including the localized strings in the kernel message table) should be kept in sync with that provided to the user through
...
--*/
{
    ...
    // If we don't have a handle to IDConfigDB, try to assign a default FriendlyName on the way out.
    if (!IDConfigDB) {
    ...
    // Find the message entry in the kernel's own message table.  KeLoaderBlock is available when we're creating hardware profiles during system
    // initialization only; for profiles created thereafter, use the the first entry of the PsLoadedModuleList to get the image base of the kernel.
    if (KeLoaderBlock) {
        dataTableEntry = CONTAINING_RECORD(KeLoaderBlock->LoadOrderListHead.Flink, LDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
    } else if (PsLoadedModuleList.Flink) {
        dataTableEntry = CONTAINING_RECORD(PsLoadedModuleList.Flink, LDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
    } else {
    ...
    // Check that the size of the label, with any numeric tag that may potentially be added (up to 4 digits, preceded by a space) is not too big.
    ...
    // Retrieve the counter of FriendlyNames we have previously assigned, based on this DockState.
    keyValueInfo = (PKEY_VALUE_FULL_INFORMATION) valueBuffer;
    status = ZwQueryValueKey(hardwareProfiles, &labelName, KeyValueFullInformation, valueBuffer, sizeof(valueBuffer), &length);
    if (NT_SUCCESS(status) && (keyValueInfo->Type == REG_DWORD)) {
 ...
 Exit:
    if (!NT_SUCCESS(status)) {
        ...
        // reason, give the new profile a new (not so) friendly name as a last resort.
