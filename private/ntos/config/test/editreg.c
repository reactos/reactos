...
Abstract:
    This program acts as an interactive shell allowing a user to view and manipulate the configuration registry.
    Also, it has some specific commands for support of the NTFT component of the registry.
...
Notes:
    The commands "disk", "fix", "restore" are commands that know where the configuration information is for the NTFT component of the NT system.
--*/
...
// array.  This way the same help message will print for each of the commands aliases.
PUCHAR   CommandHelp[] = {
    "Displays keys.",
    "Create a new key.",
...
NTSTATUS FtCreateKey(PUCHAR KeyName, PUCHAR KeyClass, ULONG  Index)
{
    ...
    // Create the key.
    status = NtCreateKey(&tempHandle, MAXIMUM_ALLOWED, &objectAttributes, Index, &unicodeClassName, REG_OPTION_NON_VOLATILE, &disposition);
    if (NT_SUCCESS(status)) {
        switch (disposition)
        {
...

{
    ...
    status = NtQueryValueKey(Handle, &unicodeValueName, KeyValueFullInformation, keyValueInformation, WORK_BUFFER_SIZE, &resultLength);
    RtlFreeUnicodeString(&unicodeValueName);
    if (NT_SUCCESS(status)) {
