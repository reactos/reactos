...
--*/
{
    ...
    case IRP_MN_MOUNT_VOLUME:
        // Attempt to mount a volume:  Determine whether or not the volume in question is a FAT volume and, if so, let the I/O system know that it
        // is by returning a special status code so that this driver can get called back to load the FAT file system.
        status = STATUS_UNRECOGNIZED_VOLUME;

        // Attempt to determine whether or not the target volume being mounted is a FAT volume.
        // Note that if an error occurs, and this is a floppy drive, and the error occurred on the actual read from the device,
        // then the FAT file system will actually be loaded to handle the problem since this driver is a place holder and does not need to
        ...
        if (FsRecGetDeviceSectorSize( targetDevice, &bytesPerSector )) {
            ...
            if (FsRecReadBlock( targetDevice, &byteOffset, 512, bytesPerSector, &buffer, &isDeviceFailure ) && IsFatVolume( buffer )) {
...
--*/
{
    ...
    // Begin by unpacking the Bios Parameter Block that is packed in the boot sector so that it can be examined without incurring alignment faults.
    UnpackBiosParameterBlock( &Buffer->PackedBpb, &bios );

    // Assume that the sector represents a FAT boot block and then determine whether or not it really does.
    result = TRUE;
    ...
    // Prior to DOS 3.2 might contains value in both of Sectors and Sectors Large.
