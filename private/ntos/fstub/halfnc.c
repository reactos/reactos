...
PDMA_ADAPTER xHalGetDmaAdapter (IN PVOID Context, IN struct _DEVICE_DESCRIPTION *DeviceDescriptor, OUT PULONG NumberOfMapRegisters)
{
    ...
    AdapterObject = ExAllocatePoolWithTag( NonPagedPool, sizeof( ADAPTER_OBJECT ), ' laH');
    ...
    AdapterObject->RealAdapterObject = HalGetAdapter( DeviceDescriptor, NumberOfMapRegisters );
...
{
    return HalAllocateCommonBuffer( ((PADAPTER_OBJECT) DmaAdapter)->RealAdapterObject, Length, LogicalAddress, CacheEnabled );
}


VOID xHalFreeCommonBuffer (
...
{
    HalFreeCommonBuffer( ((PADAPTER_OBJECT) DmaAdapter)->RealAdapterObject, Length, LogicalAddress, VirtualAddress, CacheEnabled );
}
...
--*/
{
    ...
    while (TransferLength >  0) {
        ...
        while (MdlLength > 0) {
            ...
            Element->Address = IoMapTransfer( WaitBlock->RealAdapterObject, Mdl, *MapRegisterBasePtr, CurrentVa, &Element->Length, WaitBlock->WriteToDevice );
        ...
        if (Mdl->Next == NULL) {
            // There is a few cases where the buffer described by the MDL is less than the transfer length.
            // This occurs when the file system transfering the last page of file and MM defines the MDL to be file size and the file system round the write
            // up to a sector.  This extra should never cross a page bountry. Add this extra to the length of the last element.
    ...
    WaitBlock->DriverExecutionRoutine( DeviceObject, WaitBlock->CurrentIrp, ScatterGather, WaitBlock->DriverContext );
    return( DeallocateObjectKeepRegisters );
}
#endif
...
NTSTATUS xHalAssignSlotResources (
