...

{
    ...
    // clone operation, this allows an easier failure path in the case of insufficient system resources.
    ...
    CloneHeader = ExAllocatePoolWithTag (NonPagedPool, sizeof(MMCLONE_HEADER), '  mM');
    if (CloneHeader == NULL) {
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto ErrorReturn2;
    }

    CloneDescriptor = ExAllocatePoolWithTag (NonPagedPool, sizeof(MMCLONE_DESCRIPTOR), '  mM');
    ...
    while (Vad != (PMMVAD)NULL) {
        ...
        if ((Vad->u.VadFlags.UserPhysicalPages == 0) &&
            ...
            if (NewVad == NULL) {
                // Unable to allocate pool for all the VADs.  Deallocate all VADs and other pool obtained so far.
                *VadList = (PMMVAD)NULL;
                status = STATUS_INSUFFICIENT_RESOURCES;
                goto ErrorReturn4;
            }
