...

{
    ...
    Image = ExAllocatePoolWithTag (NonPagedPool, sizeof(IMAGE_ENTRY_IN_SESSION), 'iHmM');
    if (Image == NULL) {
        UNLOCK_SESSION_SPACE_WS (OldIrql);
...

{
    ...
    if (ImageName) {
        NewName = (PWCHAR) ExAllocatePoolWithTag (PagedPool, ImageName->Length + sizeof(UNICODE_NULL), 'nHmM');
        if (NewName == NULL) {
            ExFreePool (New);
