...
Abstract:
    This module contains the routines which implement the 64-bit Very Large Memory support.
...
#define MM_LARGEST_VLM_RANGE    ((ULONGLONG)MM_HIGHEST_USER_ADDRESS64 - (ULONGLONG)MM_LOWEST_USER_ADDRESS64)
...
ULONG MiDecommitOrDeletePages64 (
...
VOID MiFlushPteList64 (IN PMMPTE_FLUSH_LIST64 PteFlushList, IN ULONG AllProcessors, IN MMPTE FillPte);
VOID MiReturnAvailablePages (IN PFN_NUMBER Amount);
LOGICAL MiCheckPdeForDeletion (IN PMMPTE PointerPde, IN LOGICAL FlushTb);
VOID MiProcessValidPteList64 (IN PMMPTE *ValidPteList, IN ULONG Count, IN LOGICAL FlushTb);
ULONG MiDoesPdeExist64 (IN PMMPTE PointerPde);
LOGICAL MiMakePdeExistAndMakeValid64 (IN PMMPTE PointerPde, IN PEPROCESS TargetProcess, IN LOGICAL PfnLockHeld, IN LOGICAL MakePageTablePage);
HARDWARE_PTE MiFlushTbAndCapture(IN PMMPTE PtePointer, IN HARDWARE_PTE TempPte, IN PMMPFN Pfn1);
NTSTATUS MiMapViewOfVlmDataSection (
...
ULONG MiIsEntireRangeCommitted64 (IN PVOID64 StartingAddress, IN PVOID64 EndingAddress, IN PMMVAD Vad, IN PEPROCESS Process);
VOID MiMakeValidPageNoAccess64 (IN PMMPTE PointePte);
VOID MiMakeNoAccessPageValid64 (IN PMMPTE PointerPte, IN ULONG Protect);
ULONG MiSetProtectionOnTransitionPte (IN PMMPTE PointerPte, IN ULONG ProtectionMask);
ULONG MiQueryAddressState64 (IN PVOID64 Va, IN PMMVAD Vad, IN PEPROCESS TargetProcess, OUT PULONG ReturnedProtect);
    ...
    Protect - Supplies the protection desired for the committed region of pages.
        ...
        Protect Values:
         ...
         PAGE_NOACCESS - No access to the committed region
              of pages is allowed. An attempt to read, write, or execute the committed region
              results in an access violation (i.e. a GP fault).

         PAGE_EXECUTE - Not allowed

         PAGE_READONLY - Read only and execute access to the
              committed region of pages is allowed. An attempt to write the committed region results in an access violation.

         PAGE_READWRITE - Read, write, and execute access to
              the committed region of pages is allowed. If write access to the underlying section is
              allowed, then a single copy of the pages are shared. Otherwise the pages are shared read
              only/copy on write.
...
--*/
{
    ...
    if ((ProtectionMask == MM_NOACCESS) && (AllocationType & MEM_COMMIT)) {
    ...
    if (CapturedBase != NULL) {
        if ((ULONGLONG)CapturedBase + CapturedRegionSize < (ULONGLONG)CapturedBase) {
            return STATUS_INVALID_PARAMETER_4;// the requested region wraps
        }
        ...
        return STATUS_INVALID_PARAMETER_4;// Invalid region size;
    }

    if (CapturedRegionSize == 0) {
        return STATUS_INVALID_PARAMETER_4;// Region size cannot be 0.
    }
    ...
    if ((Process->CloneRoot != NULL) && (Process->ForkWasSuccessful != MM_NO_FORK_ALLOWED)) {
    ...
    if ((CapturedBase == NULL) || (AllocationType & MEM_RESERVE)) {
        ...
        if (CapturedBase == NULL) {
            // No base address was specified.  This MUST be a reserve or reserve and commit.
            CapturedRegionSize = ROUND_TO_PAGES64 (CapturedRegionSize);

            // Establish exception handler as MiFindEmptyAddressRange will raise an exception if it fails.
            try {
                StartingAddress = MiFindEmptyAddressRangeInTree64 (CapturedRegionSize, X64K, (PMMADDRESS_NODE)(Process->CloneRoot));
            } except (EXCEPTION_EXECUTE_HANDLER) {
            ...
            EndingAddress = (PVOID64)(((ULONGLONG)StartingAddress + CapturedRegionSize - 1L) | (PAGE_SIZE - 1L));
        } else {
            // A non-NULL base address was specified. Check to make sure the specified base address to ending address is currently unused.
            EndingAddress = (PVOID64)(((ULONGLONG)CapturedBase + CapturedRegionSize - 1L) | (PAGE_SIZE - 1L));
            ...
            if (MiCheckForConflictingVad64 (StartingAddress, EndingAddress) != (PMMVAD)NULL) {
        ...
        // the pool and charge quota.  Note that the InsertVad routine will also charge quota which could raise an exception.
        try  {
            if ((AllocationType & MEM_COMMIT) && (QuotaCharge + QuotaCharge / PTE_PER_PAGE + 2 >= MM_MAX_COMMIT)) {
                ...
                LargeVad = FALSE;
                Vad = (PMMVAD)ExAllocatePoolWithTag (NonPagedPool, sizeof(MMVAD_SHORT), 'SdaV');
            }
        ...
        if (AllocationType & MEM_COMMIT) {
            ...
            CommitSucceeded = MiCommitPages64 (StartingPte, LastPte, ProtectionMask, Process, &QuotaFree);
            ...
            // the virtual size is saved in a pool allocation whose pointer is in the top level PDE map page.
            CommitSucceeded = MiMakePdeExistAndMakeValid64 (MiGetPdeAddress64 (StartingAddress), Process, FALSE, FALSE);
            if (CommitSucceeded == FALSE) {
                MiRemoveVad64 ((PMMVAD)Vad);
                ExFreePool (Vad);
                Status = STATUS_INSUFFICIENT_RESOURCES;
        ...
        // Update the current virtual size in the process header, the address space lock protects this operation.
        ...
        // Release the address space lock, lower IRQL, detach, and dereference the process object.
        ...
        // Commit previously reserved pages.  Note that these pages could be either private or a section.
        if (AllocationType == MEM_RESET) {
            // Don't do anything for the mem reset case as the pages are all valid.
            Status = STATUS_SUCCESS;
            goto ErrorReturn;
        } else {
            EndingAddress = (PVOID64)(((ULONGLONG)CapturedBase + CapturedRegionSize - 1) | (PAGE_SIZE - 1));
        ...
        // Ensure that the starting and ending addresses are all within the same virtual address descriptor.
        if ((MI_VA_TO_VPN64 (StartingAddress) < FoundVad->StartingVpn) || (MI_VA_TO_VPN64 (EndingAddress) > FoundVad->EndingVpn)) {
        ...
        if (FoundVad->u.VadFlags.PrivateMemory == 1) {
            ...
            // Ensure none of the pages are already committed as described in the virtual address descriptor.
#if 0
            if (AllocationType & MEM_CHECK_COMMIT_STATE) {
                if ( !MiIsEntireRangeDecommitted(StartingAddress, EndingAddress, FoundVad, Process)) {
                    // Previously reserved pages have been committed, or an error occurred, release mutex and return status.
            ...
            // Note, that for private pages, commitment is handled by explicitly updating PTEs to contain Demand Zero entries.
            StartingPte = MiGetPteAddress64 (StartingAddress);
            ASSERT (MiGetPdeAddress64(StartingAddress) == MiGetPteAddress (StartingPte));
            LastPte = MiGetPteAddress64 (EndingAddress);
            ...
            // Charge quota and commitment for the range.  Establish an exception handler as this could raise an exception.
            QuotaFree = 1;
            Status = STATUS_SUCCESS;
            ...
            try {
                ...
                if (Process->JobStatus & PS_JOB_STATUS_REPORT_COMMIT_CHANGES) {
                    ...
                    // job code will be referencing it as though everything has succeeded.
            ...
            StartingPte = MiGetProtoPteAddress (FoundVad, MI_VA_TO_VPN64 (StartingAddress));
            PointerPte = StartingPte;
            LastPte = MiGetProtoPteAddress (FoundVad, MI_VA_TO_VPN64 (EndingAddress));
        ...
        // Previously reserved pages have been committed, or an error occurred, release working set lock, address creation lock, detach, dereference process and return status.
        UNLOCK_ADDRESS_SPACE(Process);
...
NTSTATUS NtFreeVirtualMemory64(
...
Arguments:
   ...
   RegionSize - A pointer to a variable that will receive the actual size in bytes of the freed region of
                pages. The initial value of this argument is rounded up to the next host page size boundary.
...
--*/
{
    ...
    if ((ULONGLONG)MM_HIGHEST_USER_ADDRESS64 - (ULONGLONG)CapturedBase < CapturedRegionSize) {
        // Invalid region size;
    ...
    if (FreeType & MEM_RELEASE) {
        ...
        // The descriptor for the address range is deletable.  Remove the the descriptor.
        ...
        CommitReduction = MiDecommitOrDeletePages64 (StartingPte, EndingPte, Process, DELETE_TYPE_PRIVATE, TRUE);
        UNLOCK_WS (Process);
...
PVOID64 MiFindEmptyAddressRangeInTree64 (IN ULONGLONG SizeOfRange64, IN ULONG_PTR Alignment, IN PMMADDRESS_NODE Root)
/*++
Routine Description:
...
--*/
{
    ...
    // Check to see if a range exists between the lowest address VAD and lowest user address.
    if (Node->StartingVpn > MI_VA_TO_VPN64 (MM_LOWEST_USER_ADDRESS64)) {
        if ( SizeOfRange < (Node->StartingVpn - MI_VA_TO_VPN64 (MM_LOWEST_USER_ADDRESS64))) {
            return (PVOID64)MM_LOWEST_USER_ADDRESS64;
        }
    }
    ...
    for (;;) {
        ...
        if (NextNode != (PMMADDRESS_NODE)NULL) {
            if (SizeOfRange <= ((ULONG_PTR)NextNode->StartingVpn - MI_ROUND_TO_SIZE(1 + Node->EndingVpn, AlignmentVpn))) {
                ...
                if ((ULONG_PTR)NextNode->StartingVpn > MI_ROUND_TO_SIZE(1 + Node->EndingVpn, AlignmentVpn)) {
                    return (PVOID64)MI_ROUND_TO_SIZE((ULONGLONG)(MI_VPN_TO_VA_ENDING64(Node->EndingVpn)), (ULONGLONG)Alignment);
                }
            }
        } else {
            // No more descriptors, check to see if this fits into the remainder of the address space.
            if ((((ULONG_PTR)Node->EndingVpn + MI_VA_TO_VPN(X64K)) < MI_VA_TO_VPN64 (MM_HIGHEST_VAD_ADDRESS64)) &&
                (SizeOfRange64 <= ((ULONGLONG)MM_HIGHEST_VAD_ADDRESS64 - X64K - MI_ROUND_TO_SIZE((ULONGLONG)(MI_VPN_TO_VA64(Node->EndingVpn)), (ULONGLONG)Alignment)))) {
                Start = MI_VPN_TO_VA_ENDING64 (Node->EndingVpn);
                ASSERT (MI_VA_TO_VPN64 (Start) == Node->EndingVpn);
                Start = (PVOID64)MI_ROUND_TO_SIZE((ULONGLONG)Start, (ULONGLONG)Alignment);
                ASSERT (MI_VA_TO_VPN64 (Start) > Node->EndingVpn);
                return Start;
            } else {
...
/*++
Routine Description:
    This function commits by making each page valid in the specified range of PTEs.
    It also changes the protection of any existing valid PTEs in the specified range.
Arguments:
    ...
    PtesCommitted - Supplies a pointer to receive the number of valid PTEs encountered in the range.
Return Value:
    ...
    *PtesCommitted = 0;
    ...
    while (PointerPte <= LastPte) {
        if (MiIsPteOnPdeBoundary(PointerPte)) {
            // Pointing to the next page table page, make a page table page exist and make it valid.
            PointerPde = MiGetPteAddress (PointerPte);
            UsedPde64Handle = MI_GET_USED_PDE64_HANDLE (PointerPde);

            // This call will always succeed since the top level PDE already exists.
            PdeBuilt = MiMakePdeExistAndMakeValid64 (PointerPde, Process, FALSE, TRUE);
            ASSERT (PdeBuilt == TRUE);
        }

        if (PointerPte->u.Long == 0) {
            PageColor = MI_PAGE_COLOR_PTE_PROCESS (PointerPte, &Process->NextPageColor);
            ...
            MI_MAKE_VALID_PTE (TempPte, PageFrameIndex, ProtectionMask, NULL);
            ...
            *PointerPte = TempPte;
            ...
            if (Pfn1->OriginalPte.u.Soft.Protection != ProtectionMask) {
                ...
                MI_MAKE_VALID_PTE (TempPte, PointerPte->u.Hard.PageFrameNumber, ProtectionMask, NULL);
                if (TempPte.u.Hard.Write) {
                    MI_SET_PTE_DIRTY (TempPte);
                }

                // Flush the TB as we have changed the protection of a valid PTE.
                MiFlushTbAndCapture (PointerPte, TempPte.u.Flush, Pfn1);
            }
        }
...
Routine Description:
    ...
    If the page table page exists and is not currently in memory, the working set mutex and, if held, the PFN mutex are released and the
    page table page is faulted into the working set.  The mutexes are required.

    If the PDE exists, the function returns TRUE.

    If the PDE does not exist, a zero filled PTE is created and it
    too is brought into the working set.  In this case the return value is FALSE.
...
Arguments:
    ...
    PfnLockHeld - Supplies the value TRUE if the PFN mutex is held, FALSE otherwise.
    MakePageTablePage - Supplies the value TRUE if the PDE is to be created.
                        FALSE if not - meaning just the top level Base is to be created.
Return Value:
...
redo:
    if (PointerPte->u.Long == 0) {
        ...
        if (VlmInfo == NULL) {
            ...
            VlmInfo = (PMI_PROCESS_VLM_INFO) ExAllocatePoolWithTag (NonPagedPool, sizeof (MI_PROCESS_VLM_INFO), 'lVmM');
            if (VlmInfo) {
                RtlZeroMemory (VlmInfo, sizeof (MI_PROCESS_VLM_INFO));
            }
            ...
            // Note the VAD cannot disappear as the address space lock was held all the way through.
            goto redo;
        }

        PageColor = MI_PAGE_COLOR_PTE_PROCESS (PointerPte, &TargetProcess->NextPageColor);
        if (!PfnLockHeld) {
            LOCK_PFN (OldIrql);
        }
        ...
        MI_MAKE_VALID_PTE (TempPte, PageFrameIndex, MM_READWRITE, NULL);
        MI_SET_PTE_DIRTY (TempPte);
        MI_BARRIER_SYNCHRONIZE (BarrierStamp);
        *PointerPte = TempPte;
...
redo2:
    ...
    PageColor = MI_PAGE_COLOR_PTE_PROCESS (PointerPde, &TargetProcess->NextPageColor);
    if (!PfnLockHeld) {
        LOCK_PFN (OldIrql);
    }

    if (MiEnsureAvailablePageOrWait (TargetProcess, NULL) == TRUE) {
        // A wait state occurred, everything from the PDE down must be rechecked.
        if (!PfnLockHeld) {
            UNLOCK_PFN (OldIrql);
        }
        goto redo2;
...
Arguments:
    ...
    TargetProcess - Supplies a pointer to the current process.
    PfnMutexHeld - Supplies the value TRUE if the PFN mutex is held, FALSE otherwise.
Return Value:
...
Arguments:
    ...
    Type - One of: DELETE_TYPE_PRIVATE for private memory or DELETE_TYPE_SHARED for shared memory.
...
--*/
{
    ...
    // Decommit each page by setting the PTE to be explicitly decommitted.  The PTEs cannot be deleted all at once as
    // this would set the PTEs to zero which would auto-evaluate as committed if referenced by another thread when a page
    ...
    // they exist.  If they don't exist create them by touching a PTE mapped by the PDE.
    ...
    while (PointerPte <= EndingPte) {
        ...
        if (PteContents.u.Long != 0) {
            ...
            if (PteContents.u.Hard.Valid == 1) {
                ...
                // reference count goes to zero, it will be placed on the free list.
                if (Pfn1->u3.e2.ReferenceCount == 0) {
                    MiUnlinkPageFromList (Pfn1);
                    MiReleasePageFileSpace (Pfn1->OriginalPte);
                    MiInsertPageInList (MmPageLocationList[FreePageList], MI_GET_PAGE_FRAME_FROM_TRANSITION_PTE(&PteContents));
                }
            } else {
...
--*/
{
    ...
    do {
        ...
        if (Count < MM_MAXIMUM_FLUSH_COUNT) {
            ...
            PteFlushList.FlushVpn[i] = MiGetVirtualPageNumberMappedByPte64 (ValidPteList[i]) >> PAGE_SHIFT;
        }
...
/*++
Routine Description:
    This routine checks to see if the share count of the specified PDE is 1, and if it is, the PDE is deleted.
Arguments:
...
--*/
{
    ...
    if (VlmInfo->UsedPageTableEntries[PointerPde - LowestPde] == 0) {
        ...
        if (FlushTb == TRUE) {
            KeFlushSingleTb (PointerPte, TRUE, FALSE, (PHARDWARE_PTE)PointerPde, ZeroPte.u.Flush);
        }
        else {
             *PointerPde = ZeroPte;
        }
...
VOID MiDeleteVlmAddressSpace (IN PEPROCESS Process)
/*++
Routine Description:
    This routine flushes the specified range of valid PTEs.
Arguments:
...
BOOLEAN MmIsAddressValid64 (IN PVOID64 VirtualAddress)
/*++
Routine Description:
...
Arguments:
    ...
    SectionOffset - Supplies the offset from the beginning of the section to the view in bytes. This value is
                    rounded down to the next host page size boundary.
    ViewSize - Supplies a pointer to a variable that will receive the actual size in bytes of the view. If the value
               of this argument is zero, then a view of the section will be mapped starting at the specified
               section offset and continuing to the end of the section. Otherwise the initial value of this
               argument specifies the size of the view in bytes and is rounded up to the next host page size
               boundary.
    ...
    Protect - Supplies the protection desired for the region of initially committed pages.
        ...
        Protect Values
         ...
         PAGE_NOACCESS - No access to the committed region of pages is allowed. An attempt to read,
                         write, or execute the committed region results in an access violation (i.e. a GP fault).
         PAGE_READONLY - Read only and execute access to the committed region of pages is allowed. An
                         attempt to write the committed region results in an access violation.
         PAGE_READWRITE - Read, write, and execute access to the region of committed pages is allowed. If
                          write access to the underlying section is allowed, then a single copy of the pages are
                          shared. Otherwise the pages are shared read only/copy on write.
Return Value:
    Returns the status.
--*/
{
    ...
    try {
        if (PreviousMode != KernelMode) {
            ...
            if (ARGUMENT_PRESENT (SectionOffset)) {
                if (PreviousMode != KernelMode) {
                    ProbeForWrite (SectionOffset, sizeof(ULONGLONG), sizeof(ULONGLONG));
                }
            }
        }
    ...
    if ((ARGUMENT_PRESENT (SectionOffset)) && ((CapturedOffset & (X64K - 1)) != 0)) {
    ...
    // Make sure the specified starting and ending addresses are
    // within the user part of the virtual address space.
    if ((CapturedBase != NULL ) && ((CapturedBase > (PVOID64)MM_HIGHEST_USER_ADDRESS64) || (CapturedBase < (PVOID64)MM_LOWEST_USER_ADDRESS64))) {
        // Invalid base address.
        return STATUS_INVALID_PARAMETER_3;
    }
    ...
    // object, the object is not dereferenced as the virtual address descriptor contains a pointer to the section object.
    ...
    if ((CapturedOffset + CapturedViewSize) > (ULONGLONG)Section->SizeOfSection.QuadPart) {
    ...
    if (CapturedViewSize == 0) {
        ...
        TempViewSize = Section->SizeOfSection.QuadPart - CapturedOffset;

        CapturedViewSize = TempViewSize;

        if ((TempViewSize == 0) || (((ULONGLONG)MM_HIGHEST_VAD_ADDRESS64 - (ULONGLONG)CapturedBase) < CapturedViewSize)) {
            // Invalid region size;
            Status = STATUS_INVALID_VIEW_SIZE;
            goto ErrorReturn;
        }
        ...
        if ((CapturedViewSize + CapturedOffset) > (ULONGLONG)Section->SizeOfSection.QuadPart) {
            Status = STATUS_INVALID_VIEW_SIZE;
            goto ErrorReturn;
        }
    }
...
Arguments:
    ...
    ReleasedWsMutex - Supplies FALSE. If the working set mutex is not held when returning this must be set to TRUE
...
//fixfix - make copy on write invalid    CapturedCopyOnWrite = Section->u.Flags.CopyOnWrite;
    ...
    if (*CapturedBase == NULL) {
        ...
        try {
            ...
            StartingAddress = MiFindEmptyAddressRangeInTree64 (*CapturedViewSize, X64K, (PMMADDRESS_NODE)(Process->CloneRoot));
        } except (EXCEPTION_EXECUTE_HANDLER) {
            LOCK_PFN (OldIrql);
            ControlArea->NumberOfMappedViews -= 1;
            ControlArea->NumberOfUserReferences -= 1;
        ...
        EndingAddress = (PVOID64)(((ULONGLONG)StartingAddress + *CapturedViewSize - 1L) | (PAGE_SIZE - 1L));
    } else {
    ...
    // The individual page table pages are materialized when the process faults.
...
--*/
{
    ...
    if (PreviousMode != KernelMode) {
        try {
            ...
            // If an exception occurs during the probe or capture of the initial value, then handle the exception and
            // return the exception code as the status value.
            KeDetachProcess();
            ObDereferenceObject (Process);
            return GetExceptionCode();
    ...
    // Get the address creation mutex to block multiple threads from creating or deleting address space at the same time and
    // get the working set mutex so virtual address descriptors can be inserted and walked.  Raise IRQL to block APCs.

    // Get the working set mutex, no page faults allowed for now until working set mutex released.
    LOCK_WS_AND_ADDRESS_SPACE (Process);
    ...
    CommitReduction = MiDecommitOrDeletePages64 (StartingPte, EndingPte, Process, DELETE_TYPE_SHARED, TRUE);
    ...
    )
{
    ...
    // Check the protection field.  This could raise an exception.
    if ((NewProtect & PAGE_GUARD) || (NewProtect & PAGE_NOCACHE)) {
        return STATUS_INVALID_PAGE_PROTECTION;
    }
    ...
    if (FoundVad->u.VadFlags.PrivateMemory == 0) {
        // For mapped sections, the NO_CACHE and the COPY_ON_WRITE attribute are not allowed.
        if ((NewProtect & PAGE_NOCACHE) || (ProtectionMask & MM_COPY_ON_WRITE_MASK)) {
            // Not allowed.
        ...
        // can have non-committed pages if the alignment is greater than the page size.
        if ((FoundVad->ControlArea->u.Flags.File == 0) || (FoundVad->ControlArea->u.Flags.Image == 1)) {
            PointerProtoPte = MiGetProtoPteAddress (FoundVad, MI_VA_TO_VPN64 (StartingAddress));
            LastProtoPte = MiGetProtoPteAddress (FoundVad, MI_VA_TO_VPN64 (EndingAddress));

            // Release the working set mutex and acquire the section
            // commit mutex.  Check all the prototype PTEs described by the virtual address range to ensure they are committed.
            UNLOCK_WS (Process);
            ExAcquireFastMutex (&MmSectionCommitMutex);
            while (PointerProtoPte <= LastProtoPte) {
                // Check to see if the prototype PTE is committed, if not return an error.
        ...
        while (PointerPte <= LastPte) {
            ...
            if (PteContents.u.Hard.Valid == 1) {
                if (ProtectionMask != MM_NOACCESS) {
                    // Set the protection into both the PTE and the original PTE in the PFN database.
                    Pfn1 = MI_PFN_ELEMENT (PteContents.u.Hard.PageFrameNumber);
        ...
        if ( !MiIsEntireRangeCommitted64 (StartingAddress, EndingAddress, FoundVad, Process)) {
        ...
        // For all the PTEs in the specified address range, set the protection depending on the state of the PTE.
        while (PointerPte <= LastPte) {
            if (MiIsPteOnPdeBoundary(PointerPte)) {
                PointerPde = MiGetPteAddress (PointerPte);
                MiMakePdeExistAndMakeValid(PointerPde, Process, FALSE);
            ...
            if (PteContents.u.Hard.Valid == 1) {
                ...
                if (PteContents.u.Soft.Transition == 1) {
                    // The only reason a PTE would be transition is if it has been made no access.  If the protection is not no access,
                    // make it valid with the right protection.  otherwise, it is already in the desired state, so don't do anything.
                    if (ProtectionMask != MM_NOACCESS) {
...
ErrorFoundNoWs:
    ...
    try {
        ...
        // the protection of the page to change between the initial probe and the final argument update.
        if (PreviousMode != KernelMode) {
            ProbeForWrite (BaseAddress, sizeof(PVOID64), sizeof(PVOID64));
            ProbeForWrite (RegionSize, sizeof(ULONGLONG), sizeof(ULONGLONG));
            ProbeForWriteUlong (OldProtect);
...
ULONG MiIsEntireRangeCommitted64 (
...
/*++
Routine Description:
    This routine examines the range of pages from the starting address up to and including the ending address and returns TRUE if every
...
--*/
{
    ...
    while (PointerPte <= LastPte) {
        if (MiIsPteOnPdeBoundary(PointerPte) || FirstTime) {
            // This is a PDE boundary, check to see if the entire PDE page exists.
            FirstTime = FALSE;
            PointerPde = MiGetPteAddress (PointerPte);
            if (PointerPde->u.Long == 0) {
...
VOID MiMakeValidPageNoAccess64 (IN PMMPTE PointerPte)
{
    PMMPFN Pfn1;
    PMMPTE PointerPde;
    MMPTE PteContents;
    ...
    Vpn = (ULONG_PTR)((ULONGLONG)MiGetVirtualAddressMappedByPte64(PointerPte) >> PAGE_SHIFT);
    ...
    if (Pfn1->u3.e1.PrototypePte == 1) {
        ...
        // Make the PTE transition, but don't decrement the reference count only the share count.
        ASSERT (Pfn1->u3.e2.ReferenceCount != 0);
    ...
    KeFlushSingleTb64 (Vpn, TRUE, FALSE, (PHARDWARE_PTE)PointerPte, TempPte.u.Flush);
    UNLOCK_PFN (OldIrql);
    return;
}
...
VOID MiMakeNoAccessPageValid64 (IN PMMPTE PointerPte, IN ULONG Protect)
{
    ...
    Pfn1 = MI_PFN_ELEMENT (PageFrameIndex);
    MI_MAKE_VALID_PTE (TempPte, PageFrameIndex, Protect, NULL);
    if (TempPte.u.Hard.Write) {
        MI_SET_PTE_DIRTY (TempPte);
    }

    // Note that VLM no access pages are marked as transition pages and are not in any page tables.  Therefore their sharecount is zero.

    // The subtlety is that the reference count on this VLM no access page should never go to 0 because then the page will actually be freed
    // to the standby list - and we'd only want to do that if we were actually getting rid of the page (as opposed to making it no-access).
...
NTSTATUS MiGetVlmInfo (IN PMEMORY_VLM_COUNTERS VlmUserInfo)
/*++
Routine Description:
...
--*/
{
    ...
    VlmUserInfo->VirtualSizeAvailable = MM_HIGHEST_USER_ADDRESS64 - MM_LOWEST_USER_ADDRESS64 + 1;
...
NTSTATUS NtQueryVirtualMemory64(
    IN HANDLE ProcessHandle,
...
/*++
Routine Description:
    This function provides the capability to determine the state, protection, and type of a region of pages within the virtual address
...
Arguments:
    ...
    pBaseAddress - Pointer to the base address of the region of pages to be
        queried. This value is rounded down to the next host-page-address boundary.

        Note this is a 32-bit pointer to a 64-bit pointer. This is
        necessary because all system service arguments are canonicalized to 32 bits.
    ...
    MemoryInformation - A pointer to a buffer that receives the
        specified information.  The format and content of the buffer depend on the specified information class.
        ...
        MemoryBasicInformation - Data type is PMEMORY_BASIC_INFORMATION.
            ...
            ULONG Protect - The protection of the pages within the region.
                ...
                PAGE_EXECUTE - Execute access to the region of pages
                    is allowed. An attempt to read or write within the region results in an access violation.
...
--*/
{
    ...
    // The only supported option is MEMORY_BASIC_INFORMATION_VLM, make sure the user's buffer is large enough for this.
    ...
    if (PreviousMode != KernelMode) {
        ...
        try {
            ...
            ProbeForWrite(MemoryInformation, MemoryInformationLength, sizeof(ULONGLONG));
            if (ARGUMENT_PRESENT(ReturnLength)) {
                ProbeForWriteUlong(ReturnLength);
            }
        } except (EXCEPTION_EXECUTE_HANDLER) {
            // If an exception occurs during the probe or capture of the initial values, then handle the exception and
            // return the exception code as the status value.
            return GetExceptionCode();
        }
    } else {
    ...
    if ((BaseAddress > (PVOID64)MM_HIGHEST_USER_ADDRESS64) || (BaseAddress < (PVOID64)MM_LOWEST_USER_ADDRESS64)) {
    ...
    if (!Found) {
        ...
        // address.  Return the size of the hole starting at the base address.
        if (Vad == NULL) {
            ...
            if (Vad->StartingVpn < BaseVpn) {
                ...
                if (Vad == NULL) {
                    TheRegionSize = (ULONGLONG)MM_HIGHEST_VAD_ADDRESS64 - (ULONGLONG)PAGE_ALIGN64(BaseAddress);
                } else {
                    TheRegionSize = (ULONGLONG)MI_VPN_TO_VA64 (Vad->StartingVpn) - (ULONGLONG)PAGE_ALIGN64(BaseAddress);
                }
            } else {
                TheRegionSize = (ULONGLONG)MI_VPN_TO_VA64 (Vad->StartingVpn) - (ULONGLONG)PAGE_ALIGN64(BaseAddress);
            }
        ...
        if (MemoryInformationClass == MemoryBasicInformation) {
            try {
                ((PMEMORY_BASIC_INFORMATION_VLM)MemoryInformation)->AllocationBase = (PVOID64)MM_LOWEST_USER_ADDRESS64;
                ((PMEMORY_BASIC_INFORMATION_VLM)MemoryInformation)->AllocationProtect = 0;
                ((PMEMORY_BASIC_INFORMATION_VLM)MemoryInformation)->BaseAddressAsUlongLong = (ULONGLONG)PAGE_ALIGN64(BaseAddress);
                ((PMEMORY_BASIC_INFORMATION_VLM)MemoryInformation)->RegionSize = TheRegionSize;
    ...
    while (MI_VA_TO_VPN64 (Va) <= Vad->EndingVpn) {
        NewState = MiQueryAddressState64 (Va, Vad, TargetProcess, &NewProtect);
        if ((NewState != Info.State) || (NewProtect != Info.Protect)) {
            // The state for this address does not match, calculate size and return.
            break;
        }
        Va = (PVOID64)((ULONGLONG)Va + PAGE_SIZE);
    } // end while
...
--*/
{
    ...
    ASSERT ((Vad->StartingVpn <= MI_VA_TO_VPN64 (Va)) && (Vad->EndingVpn >= MI_VA_TO_VPN64 (Va)));

    PteIsZero = TRUE;

    // Note that if the PDE and PTE are not valid, then we must be in a no-access guard page type VAD.
    ...
    if (PteIsZero == TRUE) {
        ...
        State = MEM_RESERVE;
        Protect = 0;
        if ((Vad->u.VadFlags.PrivateMemory == 0) && (Vad->ControlArea != (PCONTROL_AREA)NULL)) {
...
NTSTATUS NtProtectVirtualMemory64(
    IN HANDLE ProcessHandle,
    IN OUT PVOID64 *BaseAddress,
    IN OUT PULONGLONG RegionSize,
    IN ULONG NewProtect,
