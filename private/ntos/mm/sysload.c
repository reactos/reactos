...
Arguments:
    ...
    NamePrefix - Supplies the prefix to use with the image name on load
                 operations.  This is used to load the same image multiple times, by using different prefixes
...
--*/
{
    ...
    return MiLoadSystemImage(ImageFileName, NamePrefix, LoadedBaseName, LoadInSessionSpace, ImageHandle, ImageBaseAddress, FALSE);
}
...
Routine Description:
    This routine reads the image pages from the specified section into the system and returns the address of the DLL's header.
    Very similar to MmLoadSystemImage, except that it also locks down the driver pages.
    This is needed for things like the dump driver because we cannot page it back in after the system has crashed (when we want to write the system dump to the pagefile).
...
Arguments:
    ImageName - Supplies the Unicode name of the image to load.
    ImageFileName - Supplies the full path name (including the image name) of the image to load.
    NamePrefix - Supplies the prefix to use with the image name on load operations.  This is used to load the same image multiple times, by using different prefixes
    LoadedBaseName - If present, supplies the base name to use on the loaded image instead of the base name found on the image name.
    ImageHandle - Returns an opaque pointer to the referenced section object of the image that was loaded.
    ImageBaseAddress - Returns the image base within the system.
Return Value:
...
--*/
{
    ...
    return MiLoadSystemImage(ImageFileName, NamePrefix, LoadedBaseName, FALSE, ImageHandle, ImageBaseAddress, TRUE);
}


NTSTATUS MiLoadSystemImage(
    IN PUNICODE_STRING ImageFileName,
    IN PUNICODE_STRING NamePrefix OPTIONAL,
    IN PUNICODE_STRING LoadedBaseName OPTIONAL,
    IN BOOLEAN LoadInSessionSpace,
    ...
    --*/
{
    ...
    if (NamePrefix) {
        PrefixedImageName.MaximumLength = BaseDirectory.Length + NamePrefix->Length + BaseName.Length;
    ...
    if (NtGlobalFlag & FLG_SHOW_LDR_SNAPS) {
        DbgPrint("MM:SYSLDR Loading %wZ (%wZ) %s\n", &PrefixedImageName, &BaseName, LoadInSessionSpace ? "in session space" : " ");
    }
#endif
        ...
        if (RtlEqualUnicodeString(&PrefixedImageName, &DataTableEntry->FullDllName, TRUE)) {
            if (LoadInSessionSpace == TRUE) {
                if (MI_IS_SESSION_ADDRESS(DataTableEntry->DllBase) == FALSE) {
                    ...
                    // that has already been loaded in system space.  This is not allowed.
                ...
                // unload a DLL on last dereference, but the DLL refused to unload.
                ...
                if (MI_IS_SESSION_ADDRESS(DataTableEntry->DllBase) == TRUE) {
                    ...
                    // that has already been loaded in session space.  This is not allowed.
                    Status = STATUS_CONFLICTING_ADDRESSES;
                    goto return2;
                }
            }
    ...
    if (AlreadyOpen == FALSE) {
        ...
        InitializeObjectAttributes(&ObjectAttributes, ImageFileName, OBJ_CASE_INSENSITIVE, NULL, NULL);
        ...
        if ((Status == STATUS_IMAGE_CHECKSUM_MISMATCH) || (Status == STATUS_IMAGE_MP_UP_MISMATCH) || (Status == STATUS_INVALID_IMAGE_PROTECT)) {
            goto return1;
        }
        ...
        Status = ObReferenceObjectByHandle(SectionHandle, SECTION_MAP_EXECUTE, MmSectionObjectType, KernelMode, (PVOID *)&SectionPointer, (POBJECT_HANDLE_INFORMATION)NULL);
        ...
        if (SectionPointer->Segment->ControlArea->NumberOfSubsections == 1) {
            if ((LoadInSessionSpace == FALSE) && (SectionPointer->Segment->BasedAddress != (PVOID)Session->SystemSpaceViewStart)) {
                ...
                // satisfactory executable, map it directly now to reuse the cache from the MmCheckSystemImage call above.
                Status = ZwCreateSection(&SectionHandle, SectionAccess, (POBJECT_ATTRIBUTES)NULL, (PLARGE_INTEGER)NULL, PAGE_EXECUTE, SEC_COMMIT, FileHandle);
                if (NT_SUCCESS(Status)) {
                    Status = ObReferenceObjectByHandle(SectionHandle, SECTION_MAP_EXECUTE, MmSectionObjectType, KernelMode, (PVOID *)&SectionPointer2, (POBJECT_HANDLE_INFORMATION)NULL);
                    ZwClose(SectionHandle);
                    if (NT_SUCCESS(Status)) {
                        ...
                        // new section, but these cases are under 2% of all the drivers loaded so don't bother.
    ...
    if ((LoadInSessionSpace == FALSE) &&
        ...
        Status = MmMapViewInSystemSpace(SectionPointer, ImageBaseAddress, &ViewSize);
        if ((NT_SUCCESS(Status) && (*ImageBaseAddress == SectionPointer->Segment->BasedAddress))) {
    ...
    Status = MiLoadImageSection(SectionPointer, ImageBaseAddress, ImageFileName, LoadInSessionSpace);
    ...
    if (Status == STATUS_ALREADY_COMMITTED) {
        ...
        // count - just the image count in the session which has already been done.
        ASSERT(MiHydra == TRUE);
    ...
    if (MiFirstDriverLoadEver == TRUE) {
        ...
        // This eliminates the need to check for things like relocated win32ks, Terminal Server systems, etc.
    ...
    // MmUnloadSystemImage will zero the relevant fields in the LDR_DATA_TABLE_ENTRY so that MmUnloadSystemImage will work properly.
    ...
    if (LoadInSessionSpace == FALSE || *ImageBaseAddress != SectionPointer->Segment->BasedAddress) {
...
#if DBG
        // Warn users about session images that cannot be shared because they were linked at a bad address.
        if (LoadInSessionSpace == TRUE && MmSessionSpace->SessionId) {
            DbgPrint("MM: Session %d image %wZ is linked at a nonsharable address (%p)\n", MmSessionSpace->SessionId, ImageFileName, SectionPointer->Segment->BasedAddress);
            DbgPrint("MM: Image %wZ has been moved to address (%p) by the system so it can run,\n", ImageFileName, *ImageBaseAddress);
        ...
        try {
            Status = (NTSTATUS)LdrRelocateImage(*ImageBaseAddress, "SYSLDR", (ULONG)STATUS_SUCCESS, (ULONG)STATUS_CONFLICTING_ADDRESSES, (ULONG)STATUS_INVALID_IMAGE_FORMAT);
...
BindImage:
    try {
        ...
        Status = MiResolveImageReferences(*ImageBaseAddress, &BaseDirectory, NamePrefix, FALSE, &MissingProcedureName, &MissingDriverName, &LoadedImports);
    } except(EXCEPTION_EXECUTE_HANDLER)
    {
        Status = GetExceptionCode();
    ...
    if (AlreadyOpen == FALSE) {
        // Since there was no loaded module list entry for this driver, create one now.
#if DBG
        NextEntry = PsLoadedModuleList.Flink;
        while (NextEntry != &PsLoadedModuleList) {
            DataTableEntry = CONTAINING_RECORD(NextEntry, LDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
            if (RtlEqualUnicodeString(ImageFileName, &DataTableEntry->FullDllName, TRUE)) {
        ...
        DataTableEntry = ExAllocatePoolWithTag(NonPagedPool, sizeof(LDR_DATA_TABLE_ENTRY), 'dLmM');
        if (DataTableEntry == NULL) {
            Status = STATUS_INSUFFICIENT_RESOURCES;
            goto return1;
        }

        DataTableEntry->BaseDllName.Buffer = ExAllocatePoolWithTag(NonPagedPool, BaseName.Length + sizeof(UNICODE_NULL), 'dLmM');
        ...
        RtlMoveMemory(DataTableEntry->BaseDllName.Buffer, BaseName.Buffer, BaseName.Length);
        DataTableEntry->BaseDllName.Buffer[BaseName.Length / sizeof(WCHAR)] = UNICODE_NULL;

        DataTableEntry->FullDllName.Buffer = ExAllocatePoolWithTag(PagedPool, PrefixedImageName.Length + sizeof(UNICODE_NULL), 'TDmM');
        if (DataTableEntry->FullDllName.Buffer == NULL) {// Pool could not be allocated, just set the length to 0.
            ...
            RtlMoveMemory(DataTableEntry->FullDllName.Buffer, PrefixedImageName.Buffer, PrefixedImageName.Length);
        ...
        if (CacheImageSymbols(*ImageBaseAddress)) {//  TEMP TEMP TEMP rip out when debugger converted
            ANSI_STRING AnsiName;
            UNICODE_STRING UnicodeName;

            //  \SystemRoot is 11 characters in length
            if (PrefixedImageName.Length > (11 * sizeof(WCHAR)) && !_wcsnicmp(PrefixedImageName.Buffer, L"\\SystemRoot", 11)) {
...
return1:
    if (!NT_SUCCESS(Status)) {
#if DBG
        // Only way we should fail after we get a valid DataTableEntry is if we're Hydra loading a 2nd instance of a driver.
        if (DataTableEntry) {
        ...
        if (AlreadyOpen == FALSE && SectionPointer != (PVOID)-1) {
            // This is needed for failed win32k.sys loads or any Hydra session's load of the first instance of a driver.
            ObDereferenceObject(SectionPointer);
        }
        ...
        if (IssueUnloadOnFailure == TRUE) {
            if (DataTableEntry == NULL) {
                ...
                // If DataTableEntry is NULL, then we are unloading before one got created.  Once a LDR_DATA_TABLE_ENTRY is created, the
                // load cannot fail, so if exists here, at least one other session contains this image as well.
                ASSERT(MiHydra == TRUE);
                ASSERT(DataTableEntry->LoadCount > 1);
            }
#endif
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        if (Status == STATUS_DRIVER_ORDINAL_NOT_FOUND || Status == STATUS_DRIVER_ENTRYPOINT_NOT_FOUND || Status == STATUS_PROCEDURE_NOT_FOUND) {
            ...
            } else {// Just pass ordinal values as is.                
        ...
        ZwRaiseHardError(Status, NumberOfParameters, UnicodeStringParameterMask, ErrorParameters, OptionOkNoWait, &ErrorResponse);
...
VOID MiEnablePagingOfDriver(IN PVOID ImageHandle)
{
    ...
    while (i > 0) {
        ...
        // Mark as pagable any section which starts with the first 4 characters PAGE or .eda (for the .edata section).
        if ((*(PULONG)FoundSection->Name == 'EGAP') || (*(PULONG)FoundSection->Name == 'ade.')) {
            ...
            if (PointerPte == NULL) {
                ...
                PointerPte = MiGetPteAddress(ROUND_TO_PAGES((PCHAR)Base + FoundSection->VirtualAddress));
            }
            LastPte = MiGetPteAddress((PCHAR)Base + FoundSection->VirtualAddress + (OptionalHeader->SectionAlignment - 1) + FoundSection->SizeOfRawData - PAGE_SIZE);
        } else {
...
/*++
Routine Description:
    Free up the import list and clear the pointer.
    This stops the recursion performed in MiDereferenceImports().
Arguments:
...
--*/
{
    ...
    if (DriverName->Length == 0) {
        // This is an aborted load and the driver name hasn't been filled in yet.  No need to save it.
        return;
    }
    ...
    if (MiUnloadedDrivers == NULL) {
        ...
        MiUnloadedDrivers = (PUNLOADED_DRIVERS)ExAllocatePoolWithTag(NonPagedPool, NumberOfBytes, 'TDmM');
        if (MiUnloadedDrivers == NULL) {
            return;
        }
