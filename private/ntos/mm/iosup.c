...
--*/
{
    ...
    // kernel, make sure the EndVa is in user space AND the byte count does not cause it to wrap.
    ...
    // being modified, we can remove the current page, if it is not resident, and substitute a demand zero page.
    // Note, that after analysis by marking the thread and then noting if a page read was done, this rarely occurs.
    ...
    if (!MI_IS_PHYSICAL_ADDRESS(Va)) {
        ...
        NumberOfPagesToLock = COMPUTE_PAGES_SPANNED (Va, MemoryDescriptorList->ByteCount);
    ...
    if (Va <= MM_HIGHEST_USER_ADDRESS) {
        ...
        while (NextEntry != &CurrentProcess->PhysicalVadList) {
            ...
            if ((PhysicalView->Vad->u.VadFlags.UserPhysicalPages == 0) && (PhysicalView->Vad->u.VadFlags.PhysicalMapping == 0)) {
            ...
            if (StartVa < PhysicalView->StartVa) {
                if ((PCHAR)EndVa - 1 >= PhysicalView->StartVa) {
                    // The range encompasses a physical VAD.  This is not allowed.
                    UNLOCK_PFN2 (OldIrql);
            ...
            if (StartVa <= PhysicalView->EndVa) {
                ...
                if (PhysicalView->Vad->u.VadFlags.UserPhysicalPages == 1) {
                    ...
                    // counts bumped on the pages.  Just don't charge against the working set.
                    NextEntry = NextEntry->Flink;
                    continue;
                }
                ...
                // Don't charge page locking for this transfer as it is all physical, just initialize the MDL. 
                // Note the pages do not have to be physically contiguous, so the frames must be extracted from the PTEs.
...
#endif
            {
                // PDE is not resident, release PFN lock touch the page and make it appear.
...
failure:
            ...
            if (Va <= MM_HIGHEST_USER_ADDRESS) {
                if (Operation != IoReadAccess) {
                    if ((PteContents.u.Long & MM_PTE_WRITE_MASK) == 0) {
                        ...
                        // request has been issued !  Rather than wading through the PFN database entry to see if it
                        // could possibly work out, give the caller an access violation.
        ...
        if (PageFrameIndex > MmHighestPhysicalPage) {
            ...
            if (MI_NONPAGABLE_MEMORY_AVAILABLE() <= 0) {
                // If this page is for paged pool or privileged code/data, then force it in.
...
Routine Description:
    ...
    the MemoryDescriptorList in the specified Process for the AccessMode and Operation.
Arguments:
...
--*/
{
    ...
    if (Tracker == NULL) {
        ...
        // frees and exits don't mistakenly bugcheck when an entry cannot be found.
    ...
    while (NextEntry != &LockedPagesHeader->ListHead) {
        ...
        if (P->Mdl == MemoryDescriptorList) {
            KeBugCheckEx (LOCKED_PAGES_TRACKER_CORRUPTION, 0x1, (ULONG_PTR)P, (ULONG_PTR)MemoryDescriptorList, (ULONG_PTR)MmLockedPagesHead.Count);
        }
        NextEntry = NextEntry->Flink;
    }
    ...
    // Update the systemwide global list.  First make sure it's not already inserted.
    ...
    while (NextEntry != &MmLockedPagesHead.ListHead) {
        ...
        if (P->Mdl == MemoryDescriptorList) {
            KeBugCheckEx (LOCKED_PAGES_TRACKER_CORRUPTION, 0x2, (ULONG_PTR)P, (ULONG_PTR)MemoryDescriptorList, (ULONG_PTR)MmLockedPagesHead.Count);
        }

        NextEntry = NextEntry->Flink;
    }

    InsertHeadList (&MmLockedPagesHead.ListHead, &Tracker->GlobalListEntry);
...
Routine Description:
    ...
    Used specifically by MmProbeAndLockSelectedPages () because it builds an MDL in its local
    stack and then copies the requested pages into the real MDL.  
...
Arguments:
    MemoryDescriptorList - Supplies a pointer to a Memory Descriptor List (MDL). 
                           The MDL must supply the length.
                           The physical page portion of the MDL is updated when the pages are locked in memory.
    SegmentArray - Supplies a pointer to a list of buffer segments to be probed and locked.
    AccessMode - Supplies the access mode in which to probe the arguments.
                 One of KernelMode or UserMode.
    Operation - Supplies the operation type.  One of IoReadAccess, IoWriteAccess or IoModifyAccess.
Return Value:
...
Routine Description:
    ...
    reference counts are incremented as the buffers being in nonpaged pool are always resident.
...
Arguments:
    MemoryDescriptorList - Supplies a pointer to a Memory Descriptor List
                            ...
                            address must be within the non-paged portion of the system space.
Return Value:
    None.
Environment:
    Kernel mode, IRQL of DISPATCH_LEVEL or below.
...
Routine Description:
    ...
    list into the system virtual address space or the user portion of the virtual address space.
Arguments:
...
Arguments:
    ...
    BaseVa - Supplies the base address of the view. 
             If the initial value of this argument is not null, then the view will be allocated starting at the specified virtual
             address rounded down to the next 64kb address boundary. 
             If the initial value of this argument is null, then the operating system will determine where to allocate the view.
Return Value:
    Returns the base address where the pages are mapped.  The base address
    has the same offset as the virtual address in the MDL.
...
/*++
Routine Description:
    This routine unmaps locked pages which were previously mapped via a MmMapLockedPages call.
Arguments:
...
--*/
{
    ...
    if (!ARGUMENT_PRESENT( MemoryDescriptorList )) {
        ...
        if (MdlSize > POOL_BUDDY_MAX) {
            ...
            MemoryDescriptorList = (PMDL)ExAllocatePoolWithTag (NonPagedPoolMustSucceed, MdlSize, 'ldmM');
        }
    }
...
--*/
{
    ...
    Bank = ExAllocatePoolWithTag (NonPagedPool, sizeof (MMBANKED_SECTION) + (NumberOfPtes - 1) * sizeof(MMPTE), '  mM');
    if (Bank == NULL) {
