...
// The maximum amount to try to Probe and Lock is 14 pages, this way it always fits in a 16 page allocation.
#define MAX_LOCK_SIZE ((ULONG)(14 * PAGE_SIZE))

#define MAX_MOVE_SIZE (LONG)0x10000// The maximum to move in a single block is 64k bytes.
#define MINIMUM_ALLOCATION (LONG)128// The minimum to move is a single block is 128 bytes.
#define POOL_MOVE_THRESHOLD 511// Define the pool move threshold value.
...
--*/
{
    ...
    // If the buffer size is not zero, then attempt to read data from the specified process address space into the current process address space.
    BytesCopied = 0;
    Status = STATUS_SUCCESS;
    if (BufferSize != 0) {
        // Reference the target process.
        Status = ObReferenceObjectByHandle(ProcessHandle, PROCESS_VM_READ, PsProcessType, PreviousMode, (PVOID *)&Process, NULL);

        // If the process was successfully referenced, then attempt to read the specified memory either by direct mapping or copying through nonpaged pool.
        if (Status == STATUS_SUCCESS) {
            Status = MmCopyVirtualMemory(Process, BaseAddress, PsGetCurrentProcess(), Buffer, BufferSize, PreviousMode, &BytesCopied);
...
Arguments:
     ProcessHandle - Supplies an open handle to a process object.
     BaseAddress - Supplies the base address to be written to in the specified process.
     Buffer - Supplies the address of a buffer which contains the contents to be written into the specified process address space.
     BufferSize - Supplies the requested number of bytes to write into the specified process.
...
--*/
{
    ...
    // If the buffer size is not zero, then attempt to write data from the current process address space into the target process address space.
    BytesCopied = 0;
    Status = STATUS_SUCCESS;
    if (BufferSize != 0) {
        // Reference the target process.
        ...
        // If the process was successfully referenced, then attempt to write the specified memory either by direct mapping or copying through nonpaged pool.
        if (Status == STATUS_SUCCESS) {
            Status = MmCopyVirtualMemory(PsGetCurrentProcess(), Buffer, Process, BaseAddress, BufferSize, PreviousMode, &BytesCopied);
            ObDereferenceObject(Process);// Dereference the target process.
        }
    }
...
)
{
    ...
    // If the buffer size is greater than the pool move threshold, then attempt to write the memory via direct mapping.
    if (BufferSize > POOL_MOVE_THRESHOLD) {
        Status = MiDoMappedCopy(FromProcess, FromAddress, ToProcess, ToAddress, BufferSize, PreviousMode, NumberOfBytesCopied);

        // If the completion status is not a working quota problem,
        // then finish the service. Otherwise, attempt to write the memory through nonpaged pool.
    ...
    // threshold. Attempt to write the specified memory through nonpaged pool.
    Status = MiDoPoolCopy(FromProcess, FromAddress, ToProcess, ToAddress, BufferSize, PreviousMode, NumberOfBytesCopied);
...
--*/
{
    ...
    *BadVa2 = 0;
    ...
    } else {// Unexpected exception - set the number of bytes copied to zero.
        *BadVa2 = 0xFFFFFFFF;
    }
...
--*/
{
    ...
    // Get the address of the current process object and initialize copy parameters.
    ...
    while (LeftToMove > 0) {
        ...
        try {
            ...
            MmInitializeMdl(Mdl, (PVOID)InVa, AmountToMove);// Initialize MDL for request.
        ...
        {
            // If an exception occurs during the move operation or probe, return the exception code as the status value.
            KeDetachProcess();
            if (MappedAddress != NULL) {
