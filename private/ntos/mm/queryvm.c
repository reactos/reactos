...
Abstract:
    This module contains the routines which implement the NtQueryVirtualMemory service.
...
/*++
Routine Description:
    This function provides the capability to determine the state, protection,
    and type of a region of pages within the virtual address space of the subject process.

    The state of the first page within the region is determined and then subsequent entries in the process address map are scanned from the
    base address upward until either the entire range of pages has been scanned or until a page with a nonmatching set of attributes is
    encountered. The region attributes, the length of the region of pages with matching attributes, and an appropriate status value are returned.

    If the entire region of pages does not have a matching set of attributes, then the returned length parameter value can be used to
    calculate the address and length of the region of pages that was not scanned.
Arguments:
    ...
    MemoryInformation - A pointer to a buffer that receives the specified information.
                        The format and content of the buffer depend on the specified information class.
        ...
        MemoryBasicInformation - Data type is PMEMORY_BASIC_INFORMATION.
            ...
            ULONG Protect - The protection of the pages within the region.
                ...
                PAGE_NOACCESS - No access to the region of pages is allowed.
                    An attempt to read, write, or execute within the region results in an access violation (i.e., a GP fault).

                PAGE_EXECUTE - Execute access to the region of pages is allowed.
                    An attempt to read or write within the region results in an access violation.

                PAGE_READONLY - Read-only and execute access to the region of pages is allowed. An attempt to write within the region results in an access violation.

                PAGE_READWRITE - Read, write, and execute access to the region of pages is allowed.
                    If write access to the underlying section is allowed, then a single copy of the pages are shared.
                    Otherwise, the pages are shared read-only/copy-on-write.

                PAGE_GUARD - Read, write, and execute access to the region of pages is allowed;
                    however, access to the region causes a "guard region entered" condition to be raised in the subject process.

                PAGE_NOCACHE - Disable the placement of committed pages into the data cache.
...
--*/
{
    ...
    if ((BaseAddress >= MM_HIGHEST_VAD_ADDRESS)
        ...
        if ( MemoryInformationClass == MemoryBasicInformation ) {
            try {
                ...
                ((PMEMORY_BASIC_INFORMATION)MemoryInformation)->RegionSize = ((PCHAR)MM_HIGHEST_USER_ADDRESS + 1) - (PCHAR)PAGE_ALIGN(BaseAddress);
    ...
    if (MemoryInformationClass == MemoryWorkingSetInformation) {
        ...
        Status = MiGetWorkingSetInfo (MemoryInformation, MemoryInformationLength, TargetProcess);
        ...
        try {
            if (ARGUMENT_PRESENT(ReturnLength)) {
                *ReturnLength = ((((PMEMORY_WORKING_SET_INFORMATION) MemoryInformation)->NumberOfEntries - 1) * sizeof(ULONG)) + sizeof(MEMORY_WORKING_SET_INFORMATION);
            }
        } except (EXCEPTION_EXECUTE_HANDLER) {
        }
    ...
    LOCK_WS_AND_ADDRESS_SPACE (TargetProcess);// Get working set mutex and block APCs.
    ...
    // Locate the VAD that contains the base address or the VAD which follows the base address.
    ...
    if (!Found) {
        // There is no virtual address allocated at the base address.
        // Return the size of the hole starting at the base address.
        if (Vad == NULL) {
            TheRegionSize = ((PCHAR)MM_HIGHEST_VAD_ADDRESS + 1) - (PCHAR)PAGE_ALIGN(BaseAddress);
        } else {
            if (Vad->StartingVpn < BaseVpn) {
                // We are looking at the Vad which occupies the range just before the desired range.  Get the next Vad.
        ...
        // Establish an exception handler and write the information and returned length.
        if ( MemoryInformationClass == MemoryBasicInformation ) {
            try {
                ((PMEMORY_BASIC_INFORMATION)MemoryInformation)->AllocationBase = NULL;
                ((PMEMORY_BASIC_INFORMATION)MemoryInformation)->AllocationProtect = 0;
                ((PMEMORY_BASIC_INFORMATION)MemoryInformation)->BaseAddress = PAGE_ALIGN(BaseAddress);
                ((PMEMORY_BASIC_INFORMATION)MemoryInformation)->RegionSize = TheRegionSize;
    ...
    while (MI_VA_TO_VPN (Va) <= Vad->EndingVpn) {
        NewState = MiQueryAddressState (Va, Vad, TargetProcess, &NewProtect);
        if ((NewState != Info.State) || (NewProtect != Info.Protect)) {
            break;// The state for this address does not match, calculate size and return.
        }
        Va = (PVOID)((PCHAR)Va + PAGE_SIZE);
    } // end while
    ...
    // A range has been found, release the mutexes, deattach from the target process and return the information.
    ...
    if (TargetProcess->Wow64Process != NULL) {
        ...
        MiQueryRegionFor4kPage(Info.BaseAddress, MI_VPN_TO_VA_ENDING(Vad->EndingVpn), &Info.RegionSize, &Info.State, &Info.Protect, TargetProcess);
    }
    ...
    if (MmDebug & MM_DBG_SHOW_NT_CALLS) {
        if ( !MmWatchProcess ) {
            DbgPrint("queryvm base %lx allocbase %lx protect %lx size %lx\n", Info.BaseAddress, Info.AllocationBase, Info.AllocationProtect, Info.RegionSize);
    ...
    if ( !FilePointer ) {// Try to return the name of the file that is mapped.
        return STATUS_INVALID_ADDRESS;
    } else if ( FilePointer == (PVOID)1 ) {
        return STATUS_FILE_INVALID;
    }

    // We have a referenced pointer to the file. Call ObQueryNameString and get the file name
    Status = ObQueryNameString(FilePointer, MemoryInformation, MemoryInformationLength, ReturnLength);
...
--*/
{
    ...
    if (PteDetected == TRUE) {
        ...
        if (PointerPte->u.Long != 0) {
            ...
            // There is a non-zero PTE at this address, use it to build the information block.
            if (MiIsPteDecommittedPage (PointerPte)) {
                ...
                if (Vad->u.VadFlags.PhysicalMapping == 1) {
                    ...
                    Protect = MiGetPageProtection (PointerPte, TargetProcess);
                    if ((PointerPte->u.Soft.Valid == 0) && (PointerPte->u.Soft.Prototype == 1) && (Vad->u.VadFlags.PrivateMemory == 0) && (Vad->ControlArea != (PCONTROL_AREA)NULL)) {
    ...
    if (PteIsZero) {
        // There is no PDE at this address, the template from the VAD supplies the information unless the VAD is
        // for an image file.  For image files the individual protection is on the prototype PTE.
        ...
        if (Vad->u.VadFlags.PhysicalMapping == 1) {
            ...
            if (CapturedProtoPte.u.Long != 0) {
                ...
                if (Vad->u.VadFlags.ImageMap == 0) {
                    ...
                    // This is an image file, the protection is in the prototype PTE.
                    Protect = MiGetPageProtection (&CapturedProtoPte, TargetProcess);
                }
            }
        } else {
...
NTSTATUS MiGetWorkingSetInfo (IN PMEMORY_WORKING_SET_INFORMATION WorkingSetInfo, IN ULONG Length, IN PEPROCESS Process)
{
    ...
    Mdl = ExAllocatePoolWithTag (NonPagedPool, sizeof(MDL) + sizeof(PFN_NUMBER) + BYTES_TO_PAGES (Length) * sizeof(PFN_NUMBER), '  mM');
    if (Mdl == NULL) {
        return(STATUS_INSUFFICIENT_RESOURCES);
    }

    MmInitializeMdl(Mdl, WorkingSetInfo, Length);// Initialize the MDL for the request.
    ...
    LastEntry = (PMEMORY_WORKING_SET_BLOCK)((PCHAR)Info + (Length & (~(sizeof(ULONG) - 1))));
