...
Abstract:
    This module implements the routines which clients use to register themselves with the Log File Service.
...
--*/
{
    ...
    *Bcb = NULL;
    ...
    CcFlushCache(Lfcb->FileObject->SectionObjectPointer, (PLARGE_INTEGER)&FileOffset, (ULONG)Lfcb->LogPageSize, NULL);
}
...
/*++
Routine Description:
    This routine is called to remove a client record from a client record list in an Lfs restart area.
Arguments:
    ClientArray - Base of client records in restart area.
    ClientRecord - A pointer to the record to add.
    ListHead - A pointer to the beginning of the list.  This points to a USHORT which is the value of the first element in the list.
Return Value:
    None.
--*/
{
