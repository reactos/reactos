...
// values used for the BearerClass field in the Broadband Bearer Capability and UNI 4.0 Broadband Bearer Capability IEs.
...
// values used for the ClippingSusceptability field in the Broadband Bearer Capability and UNI 4.0 Broadband Bearer Capability IEs.
#define CLIP_NOT                0x00    // Not susceptible to clipping
#define CLIP_SUS                0x20    // Susceptible to clipping

// values used for the UserPlaneConnectionConfig field in the Broadband Bearer Capability and UNI 4.0 Broadband Bearer Capability IEs.
...
// Values used for Layer3H310ForwardMultiplexingCapability and Layer3H310BackwardMultiplexingCapability in struct B-LLI ADDON
...
// Values used for the QOSClassForward and QOSClassBackward fields in ATM_QOS_CLASS_IE
...
// Values used for the P-U flag of the Diagnostics field in struct ATM_CAUSE_IE, for certain Cause values
...
// Otherwise we assume "explicit VPCI" (this is the normal setting), and the Vpci value is used.

// Setting the VCI to SAP_FIELD_ABSENT, means it is "switched VP", which is not supported by this CallManager;

// Setting the VCI to SAP_FIELD_ANY, means it is "explicit VPCI, any VCI";

// Setting both the Vpci and the Vci fields to SAP_FIELD_ANY, or setting both the Vpci and the Vci fields to SAP_FIELD_ABSENT,
// means that the ConnectionId is not defined, so it will not be sent to the peer.
// (This allows an empty ConnectionIdentifier IE to hold the response).

// Otherwise we assume "explicit VPCI, explicit VCI" (this is the normal setting), and the Vci value is used.
...
// The Q2931 Call Manager Specific parameters that goes into the CallMgrParameters->CallMgrSpecific.Parameters
...
// Parameters. The following define is used in the ParamType field depending on the signaling type.
...
// The structure passed in the Parameters field of the CO_SPECIFIC_PARAMETERS structure passed in an ADD PVC request for Q.2931
...
// the call mgr can then be optimized not to search these PVCs during make call processing.
...
//  ATM cell layer transfer capacities in bits/sec. This is the throughput available for ATM cells, after allowing for physical framing overhead.
#define ATM_CELL_TRANSFER_CAPACITY_SONET_STS3C            149760000
#define ATM_CELL_TRANSFER_CAPACITY_IBM_25                  25125926

//  User data rate in units of 100 bits/sec.
//  This is returned in response to the OID_GEN_CO_LINK_SPEED query.
//  This is the effective rate of transfer of data available to the ATM layer user, after allowing for the ATM cell header.
#define ATM_USER_DATA_RATE_SONET_155                      1356317
#define ATM_USER_DATA_RATE_IBM_25                           227556

//  The ATM Service Registry MIB Table is used to locate ATM network services.
//  OID_ATM_GET_SERVICE_ADDRESS is used by clients to access this table.
