...
Abstract:
    This is the header file that describes the constants and data structures used by the user mode heap manager,
    exported by ntdll.dll and ntrtl.lib
...
typedef struct _HEAP_ENTRY {
    //  This field gives the size of the current block in allocation granularity units.
    //  (i.e. Size << HEAP_GRANULARITY_SHIFT equals the size in bytes).

    //  Except if this is part of a virtual alloc block then this value is the difference between the commit size in the virtual alloc entry and the what the user asked for.
    USHORT Size;

    // This field gives the size of the previous block in allocation granularity units.
    // (i.e. PreviousSize << HEAP_GRANULARITY_SHIFT equals the size of the previous block in bytes).
    USHORT PreviousSize;

    UCHAR SegmentIndex;// This field contains the index into the segment that controls the memory for this block.
    ...
    // This field contains the number of unused bytes at the end of this block that were not actually allocated.
    // Used to compute exact size requested prior to rounding requested size to allocation granularity.
    // Also used for tail checking purposes.
    UCHAR UnusedBytes;

    UCHAR SmallTagIndex;// Small (8 bit) tag indexes can go here.
...
// This block describes extra information that might be at the end of a busy block.

typedef struct _HEAP_ENTRY_EXTRA {
    union {
        struct {
            // This field is for debugging purposes.
            // It will normally contain a stack back trace index of the allocator for x86 systems.
            USHORT AllocatorBackTraceIndex;

            // This field is currently unused,
            // but is intended for storing any encoded value that will give the that gives the type of object allocated.
            USHORT TagIndex;

            // This field is a 32-bit settable value that a higher level heap package can use.
            // The Win32 heap manager stores handle values in this field.
            ULONG_PTR Settable;
        };
#if defined(_WIN64)
        struct {
...
// This structure is present at the end of a free block if HEAP_ENTRY_EXTRA_PRESENT is set in the Flags field of a HEAP_FREE_ENTRY structure.
// It is used to save the tag index that was associated with the allocated block after it has been freed.
...
// This structure describes a block that lies outside normal heap memory as it was allocated with NtAllocateVirtualMemory and
// has the HEAP_ENTRY_VIRTUAL_ALLOC flag set.
...
typedef struct _HEAP_FREE_ENTRY {
    ...
    // Two fields to encode the location of the bit in FreeListsInUse array in HEAP_SEGMENT for blocks of this size.
    UCHAR Index;
    UCHAR Mask;

    LIST_ENTRY FreeList;// Free blocks use these two words for linking together free blocks of the same size on a doubly linked list.
...
// HEAP_SEGMENT defines the structure used to describe a range of contiguous virtual memory that has been set aside for use by a heap.
...
typedef struct _HEAP {
    ...
    //  The following two fields control the alignment for each new heap entry allocation.
    //  The round is added to each size and the mask is used to align it.
    //  The round value includes the heap entry and any tail checking space
    ULONG AlignRound;
    ULONG AlignMask;
    ...
    //  The following field is used to manage the heap lookaside list.
    //  The pointer is used to locate the lookaside list array.
    //  If it is null then the lookaside list is not active.

    //  The lock count is used to denote if the heap is locked.
    //  A zero value means the heap is not locked.
    //  Each lock operation increments the heap count and each unlock decrements the counter
    PVOID Lookaside;
...
#define HEAP_NEED_EXTRA_FLAGS ((HEAP_TAG_MASK ^ HEAP_SMALL_TAG_MASK)  | HEAP_CAPTURE_STACK_BACKTRACES | HEAP_SETTABLE_USER_VALUE )
...
#error Invalid HEAP_ENTRY_SETTABLE_FLAGS
     ...
     (HEAP_ENTRY_BUSY | HEAP_ENTRY_EXTRA_PRESENT | HEAP_ENTRY_FILL_PATTERN |   \
      HEAP_ENTRY_VIRTUAL_ALLOC |  HEAP_ENTRY_LAST_ENTRY | HEAP_ENTRY_SETTABLE_FLAGS ) \
    )
...
#include <heappage.h>// Page heap external interface.
