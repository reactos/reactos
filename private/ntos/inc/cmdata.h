...
Abstract:
    This module contains data structures used by the configuration manager.
...
// To improve performance without changing the on disk structure, we cache the frequently used registry data to minimize reference on registry data.

// A KCB (Key Control Block) is the core structure for registry cache.
// It uses HashValue for quick cache lookup and contains the most frequently used data in a key node.
...
// Caches for value data will be created during query process, the cached structure is shown as the following picture.
// The structure is almost the same as the registry structure
...
#define KEY_COMP_NAME       0x0020      // The name for this key is stored in a
                                        // compressed form.
#define KEY_PREDEF_HANDLE   0x0040      // There is no real key backing this, return the predefined handle.
...
#define VALUE_COMP_NAME 0x0001              // The name for this value is stored in a compressed form.
...
// When they are cached, List will be pointer to the allocation.
// We distinguish them by marking the lowest bit in the variable to indicate it is a cached allocation.

// Note that the cell index for value list
// is stored in the cached allocation.  It is not used now but may be in further performance optimization.

// When value key and vaule data are cached, there is only one allocation for both.
// Value data is appended that the end of value key.
// DataCacheType indicates whether data is cached and ValueKeySize tells how big is the value key (so we can calculate the address of cached value data)
