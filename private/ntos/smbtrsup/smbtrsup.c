...
Abstract:
    This module contains the code to implement the kernel mode SmbTrace component within the LanMan server and redirector.
    The interface between the kernel mode component and the server/redirector is found in nt\private\inc\smbtrsup.h
    The interface providing user-level access to SmbTrace is found in nt\private\inc\smbtrace.h
...
Revision History:
    ...
    Stephan Mueller (t-stephm)   21-July-1992
        ...
        from various places in the server, ported to the redirector and converted to a kernel DLL.
--*/
...
Routine Description:
    ...
    globals.  On first-ever invocation, it performs truly global initialization.
Arguments:
...
--*/
{
    ...
    if ( ID(InstanceInitialized) == FALSE ) {
        ...
        ID(StateInterlock) = ExAllocatePoolWithTag(NonPagedPool, sizeof(ERESOURCE), 'tbmS');
        if ( ID(StateInterlock) == NULL ) {
            return STATUS_INSUFFICIENT_RESOURCES;
        }
        ExInitializeResource( ID(StateInterlock) );

        ID(HeapInterlock) = ExAllocatePoolWithTag(NonPagedPool, sizeof(ERESOURCE), 'tbmS');
...
/*++
Routine Description:
    This routine cleans up the SmbTrace component-specific instance globals. 
    It should be called by the component when the component is unloaded.
Arguments:
    Component - Context from which we're called: server or redirector
Return Value:
    None
...
Arguments:
    ...
    FileObject - FileObject of the process requesting that SmbTrace be started, used to automatically shut down when the app dies.
    Component - Context from which we're called: server or redirector
...

{
    ...
    if ( !NT_SUCCESS(status) ) {
        TrPrint(("%s!SmbTraceStart: RtlCreateAcl failed: %X\n", ID(ComponentName), status ));
        goto errexit;
    }

    status = RtlAddAccessAllowedAce(AdminAcl, ACL_REVISION2, GENERIC_ALL, SeExports->SeAliasAdminsSid);
    if ( !NT_SUCCESS(status) ) {
        TrPrint(("%s!SmbTraceStart: RtlAddAccessAllowedAce failed: %X\n", ID(ComponentName), status ));
        goto errexit;
    }
    ...
    if ( !NT_SUCCESS(status) ) {
        TrPrint(("%s!SmbTraceStart: RtlCreateSecurityDescriptor failed: %X\n", ID(ComponentName), status ));
        goto errexit;
    }

    status = RtlSetDaclSecurityDescriptor(&securityDescriptor, TRUE, AdminAcl, FALSE);
    if ( !NT_SUCCESS(status) ) {
        TrPrint((
            "%s!SmbTraceStart: "
    ...
    ID(TableHeader) = RtlAllocateHeap(ID(PortMemoryHeap), 0, sizeof( SMBTRACE_TABLE_HEADER ));
    ID(Table) = RtlAllocateHeap(ID(PortMemoryHeap), 0, sizeof( SMBTRACE_TABLE_ENTRY ) * ID(TableSize));
    if ( (ID(TableHeader) == NULL) || (ID(Table) == NULL) ) {
        TrPrint(("%s!SmbTraceStart: Not enough memory!\n", ID(ComponentName) ));
    ...
    InitializeObjectAttributes(&objectAttributes, &eventNameU, OBJ_CASE_INSENSITIVE, NULL, &securityDescriptor);
    ...
    if ( ID(SingleSmbMode) ) {    // this event may not be required.
        ...
        if ( !NT_SUCCESS(status) ) {
            TrPrint(("%s!SmbTraceStart: NtCreateEvent (2nd) failed: %X\n", ID(ComponentName), status ));
            goto errexit;
        }
    ...
    if ( !NT_SUCCESS(status) ) {
        TrPrint(("%s!SmbTraceStart: PsCreateSystemThread failed: %X\n", ID(ComponentName), status ));
        goto errexit;
    }

    // Wait until SmbTraceThreadEntry has finished initializing
    (VOID)KeWaitForSingleObject(&ID(ActiveEvent), UserRequest, KernelMode, FALSE, NULL);
    ...
    ConfigPacketResp->HeaderOffset = (ULONG)((ULONG_PTR)ID(TableHeader) - (ULONG_PTR)ID(PortMemoryBase));
    ConfigPacketResp->TableOffset = (ULONG)((ULONG_PTR)ID(Table) - (ULONG_PTR)ID(PortMemoryBase));
...
// constant only of interest while constructing the particular Acl in SmbTraceStart
#undef ACL_LENGTH
...
--*/
{
    ...
    queueEntry = ExAllocatePoolWithTag( NonPagedPool, sizeof(SMBTRACE_QUEUE_ENTRY), 'tbmS');
    if ( queueEntry == NULL ) {
    ...
    ExInterlockedInsertTailList(&ID(Queue), &queueEntry->ListEntry, &ID(QueueInterlock));
    KeReleaseSemaphore(&ID(QueueSemaphore), SEMAPHORE_INCREMENT, 1, FALSE);

    // ...and wait for the SMB to be eaten, in slow mode.
    if ( ID(SingleSmbMode) ) {
        TrPrint(( "%s!SmbTraceCompleteSrv: Slow mode wait\n", ID(ComponentName) ));
        KeWaitForSingleObject(&WaitEvent, UserRequest, KernelMode, FALSE, NULL);
...

{
    ...
    // allocate buffer for SMB, in non-paged pool or shared heap as appropriate
    if ( AtDpcLevel ) {
...
/*++
Routine Description:
    This routine references the SmbTrace shared memory heap, ensuring it isn't disposed of while caller is using it.
Arguments:
    Component - Context from which we're called: server or redirector
Return Value:
...
/*++
Routine Description:
    If a caller dereferences a heap to 0 from DPC_LEVEL, this routine will be called in a system thread to complete the dereference at task time.
Arguments:
    Component - Context from which we're called: server or redirector
Return Value:
    None
...
/*++
Routine Description:
    This routine dereferences the SmbTrace shared memory heap, disposing of it when the reference count is zero.
Arguments:
    Component - Context from which we're called: server or redirector
Return Value:
    None
...
/*++
Routine Description:
    This routine reverses all the effects of SmbTraceStart. Mostly, it just needs to close certain handles to do this.
Arguments:
    Component - Context from which we're called: server or redirector
Return Value:
    None - always works
...
--*/
{
    ...
    while ( ( listEntry = ExInterlockedRemoveHeadList(&ID(Queue), &ID(QueueInterlock))) != NULL) {
        queueEntry = CONTAINING_RECORD(listEntry, SMBTRACE_QUEUE_ENTRY, ListEntry);
...
Routine Description:
    ...
    continuously until the client SmbTrace dies or another SmbTrace sends an FsCtl to stop the trace.
Arguments:
    Context - pointer to context block containing component from which we're called: server or redirector
Return Value:
    None
--*/
...

{
    ...
    // If we try to wait on too many objects, we need to allocate our own wait blocks.
    ...
    while( Looping )
    {
        ...
        if ( !NT_SUCCESS(status) ) {
            TrPrint(("%s!SmbTraceThreadEntry: KeWaitForMultipleObjectsfailed: %X\n", ID(ComponentName), status ));
        } else {
            TrPrint(("%s!SmbTraceThreadEntry: %lx\n", ID(ComponentName), status ));
        }
        ...
        case STATUS_WAIT_QUEUESEMAPHORE:
            ...
            listEntry = ExInterlockedRemoveHeadList(&ID(Queue), &ID(QueueInterlock));
            if ( listEntry != NULL ) {
                ...
                if ( queueEntry->BufferNonPaged ) {
                    ...
                    buffer = RtlAllocateHeap( ID(PortMemoryHeap), 0, queueEntry->SmbLength );
                    if ( buffer == NULL ) {
                        LOCK_INC_ID(SmbsLost);
                        ExFreePool( queueEntry->Buffer );
                        ExFreePool( queueEntry );
                    ...
                    RtlCopyMemory( buffer, queueEntry->Buffer, queueEntry->SmbLength );
                    ExFreePool( queueEntry->Buffer );
                    ...
                    SmbTraceToClient(buffer, queueEntry->SmbLength, NULL, Component);
                } else {
                    ...
                    SmbTraceToClient(queueEntry->Buffer, queueEntry->SmbLength, queueEntry->SmbAddress, Component);
                    if ( queueEntry->WaitEvent != NULL ) {
                        KeSetEvent( queueEntry->WaitEvent, 0, FALSE );
                    }
                }
                
                ExFreePool( queueEntry );// Now, we must free the queue entry.
            }
            break;
        default:
            break;
    ...
    status = PsTerminateSystemThread( STATUS_SUCCESS );// Kill this thread.

    // Shouldn't get here
    TrPrint(("%s!SmbTraceThreadEntry: PsTerminateSystemThread() failed: %X\n", ID(ComponentName), status ));
} // SmbTraceThreadEntry


// constant only of interest while constructing waitObject arrays in SmbTraceThreadEntry
...
Routine Description:
    ...
    SMB that the client has already consumed. It does not alter table entries, except to record that the memory buffer has been cleared.
    This routinue is not espectally fast, it should not be called often, only when needed.
Arguments:
    Component - Context from which we're called: server or redirector
Return Value:
    NTSTATUS - result of operation.
--*/
{
    ...
    while( tableIndex != ID(TableHeader)->HighestConsumed ) {
        ...
        // Check if this table entry has been used but its memory has not been freed yet. If so, free it.
        if ( tableEntry->BufferOffset != 0L ) {
            buffer = (PVOID)( (ULONG_PTR)tableEntry->BufferOffset + (ULONG_PTR)ID(PortMemoryBase) );
...
/*++
Routine Description:
    Enter an SMB already found in shared memory into the table.  
    Set an event for the client.  If there is no table space, the SMB is not saved.  
    If in slow mode, wait for the client to finish with and then free the memory occupied by the SMB.
Arguments:
    Smb - a pointer to the SMB (which is ALREADY in shared memory).
          Can be NULL, indicating no new SMB is to be added, but the application is to be signalled anyway.
    SmbLength - the length of the SMB.
...
--*/
{
    ...
    if (Smb != NULL) {
        ...
        ID(TableHeader)->NextFree = (tableIndex + 1) % ID(TableSize);// Increment the Next Free counter.
    }
    ...
    if (( ID(SingleSmbMode) ) && ( ID(TraceState) == TraceRunning )) {
        // Wait for the app to acknowledge that the SMB has been processed.

        TrPrint(( "%s!SmbTraceToClient: Waiting for DoneSmbEvent, handle %x, process %x.\n",
                    ID(ComponentName), ID(DoneSmbEvent), PsGetCurrentProcess()));
        status = ZwWaitForSingleObject(ID(DoneSmbEvent), FALSE, NULL);
...
Routine Description:
    ...
    Destination.  Length is present to keep the same interface as RtlCopyMemory.
Arguments:
    Destination - a pointer to previously allocated memory into which the Mdl is to be copied.
...
--*/
{
    ...
    while (Mdl != NULL) {
        RtlCopyMemory(Dest, MmGetSystemAddressForMdl(Mdl), MmGetMdlByteCount(Mdl));
