...
//  An array of these structures is passed to UdfMultipleAsync describing a set of runs to execute in parallel.
typedef struct _IO_RUN {
    //  Disk offset to read from and number of bytes to read. 
    //  These must be a multiple of a sector and the disk offset is also a multiple of sector.
    LONGLONG DiskOffset;
    ULONG DiskByteCount;

    //  Current position in user buffer. 
    //  This is the final destination for this portion of the Io transfer.
    PVOID UserBuffer;
    ...
    //      value of zero indicates that we did do the transfer into the user's buffer directly.

    //  TransferBufferOffset - Offset in this buffer to begin the transfer to the user's buffer.
    ...
    //  the Mdl below.  If the TransferMdl is not the same as the Mdl in the user's Irp then we know we have allocated it.
    PMDL TransferMdl;
...
--*/
{
    ...
    try {
        ...
        do {
            ...
            if ((ThisByteCount < RemainingByteCount) && !FlagOn( IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT )) {
            ...
            if ((RunCount == 1) && !Unaligned && !SparseRuns && FirstPass) {
                UdfSingleAsync( IrpContext, IoRuns[0].DiskOffset, IoRuns[0].DiskByteCount );
            ...
            if (FlagOn( IrpContext->Flags, IRP_CONTEXT_FLAG_WAIT )) {
                ...
                //  necessary.
                if (Unaligned && UdfFinishBuffers( IrpContext, IoRuns, RunCount, FALSE )) {
                    FlushIoBuffers = TRUE;
                }
