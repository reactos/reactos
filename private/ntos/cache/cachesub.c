...
Arguments:
    ...
    ReadOnly - Supplies TRUE if caller will only read the mapped data (i.e., TRUE for CcCopyRead, CcMapData and CcMdlRead and FALSE for everyone else)
    ...
    BeyondLastByte - Returns the File Offset of the first byte beyond the last accessible byte.

Return Value:

    FALSE - if PIN_WAIT was set, and it was impossible to lock all of the data without blocking
    TRUE - if the desired data, is being returned
Raises:
    STATUS_INSUFFICIENT_RESOURCES - If a pool allocation failure occurs.
        This can only occur if Wait was specified as TRUE.  (If Wait is specified as FALSE, and an allocation failure occurs, this routine simply returns FALSE.)
--*/
{
    ...
    //  If there is an end of a page to be zeroed, then free that page now, so it does not cause our data to get zeroed.  If there is an active
    ...
    //  Make sure the calling file system is not asking to map beyond the end of the section, for example, that it did not forget to do CcExtendCacheSection.
    ...
    *BaseAddress = NULL;
    ...
    if (!FlagOn(Flags, PIN_NO_READ)) {
        *BaseAddress = CcGetVirtualAddress( SharedCacheMap, *FileOffset, &Vacb, &ReceivedLength );
        ...
        //  In the PIN_NO_READ case, we simply need to make sure that the sparse structure containing the Bcb listheads is expanded in the region of the file we are interested in.
    ...
    try {
        ...
        //  Search for Bcb describing the largest matching "prefix" byte range, or where to insert it.
        ...
        if (!Found) {
            ...
            if (FlagOn(Flags, PIN_IF_BCB)) {
                //  We need to zap BcbOut since this is a hint to the cleanup code to remove the Bcb if we are returning FALSE.
                BcbOut = NULL;
            ...
            //  At this point we can calculate the ReadOnly flag for the purposes of whether to use the Bcb resource, and we can calculate the ZeroFlags.
            ...
            //  We treat Bcbs as ReadOnly (do not acquire resource) if they are in sections for which we have not disabled modified writing.
            ...
            *BaseAddress = ((PCHAR)*BaseAddress - (FileOffset->LowPart & (PAGE_SIZE - 1)));
            ...
            //  Even if we are readonly, we can still zero pages entirely beyond valid data length.
            ...
            //  We will get into trouble if we try to read more than we can map by one Vacb.  So make sure that our lengths stay within a Vacb.
            ...
            //  our Bcb will have to wait shared to insure that the data is in.
            if (FlagOn(Flags, PIN_WAIT)) {
                ...
                //  Now just acquire the newly-allocated Bcb shared, and release the spin lock.
                if (!ReadOnly) {
                ...
                if (!FlagOn(Flags, PIN_NO_READ)) {
                    ...
                    //  Now we have to reacquire the Bcb List spinlock to load up the mapping if we are the first one, else we collided
                    //  with someone else who loaded the mapping first, and we will just free our mapping.  It is guaranteed that the
            ...
            //  If we cannot wait, then we go immediately see if the data is there (CcMapAndRead), and then only set up the Bcb and release
            //  the spin lock if the data is there.  Note here we call
            //  CcMapAndRead while holding the spin lock, because we know we will not fault and not block before returning.
            else {
                ...
                //  If we are not ReadOnly, we must acquire the newly-allocated resource shared, and then we can free the spin lock.
                if (!ReadOnly) {
                ...
                //  Note that since this call has Wait = FALSE, it cannot get an exception (see procedure header).
        ...
        if (BcbOut->BaseAddress == NULL) {
            //  It is too complicated to attempt to calculate any ZeroFlags in this case, because we have to not only do the tests above, but also
            //  compare to the byte range in the Bcb since we will be passing
            //  those parameters to CcMapAndRead.  Also, the probability of hitting some window where zeroing is of any advantage is quite small.

            //  Set up to just reread the Bcb exactly as the data in it is described.
