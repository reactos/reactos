...
Abstract:
    ...
    the NLS tables are loaded, they convert between ANSI and Unicode by zero-extending.
...
BOOLEAN RtlEqualUnicodeString(IN const PUNICODE_STRING String1,
    ...
    /*++
    Routine Description:
        The RtlEqualUnicodeString function compares two counted unicode strings for equality.
        The CaseInSensitive parameter specifies if case is to be ignored when doing the comparison.
    Arguments:
        String1 - Pointer to the first string.
        String2 - Pointer to the second string.
...
Return Value:
    ...
    STATUS_BUFFER_TOO_SMALL - The destination string length was not big
        enough to allow the source string to be appended.  The Destination string length is not updated.
--*/
{
...
/*++
Routine Description:
    This routine appends the supplied UNICODE string to an existing PUNICODE_STRING.
...
Arguments:
    IN PUNICODE_STRING Destination, - Supplies a pointer to the destination string
...
Routine Description:
    ...
    translation is done with respect to the current system locale information.
Arguments:
...
Arguments:
    SourceCharacter - Supplies a pointer to an ansi character pointer.
        ...
        the next character to be converted.  This is done to allow for dbcs ansi characters.
Return Value:
...
NTSTATUS RtlUpcaseUnicodeToMultiByteN(OUT PCH MultiByteString,
    ...
    Arguments:
        MultiByteString - Returns an ansi string that is equivalent to the
            upper case of the unicode source string.  If the translation can not be done, an error is returned.
    ...
    --*/
{
    ...
    LoopCount = (CharsInUnicodeString < MaxBytesInMultiByteString) ? CharsInUnicodeString : MaxBytesInMultiByteString;
    ...
    --*/
{
    ...
    LoopCount = (MaxCharsInUnicodeString < BytesInMultiByteString) ? MaxCharsInUnicodeString : BytesInMultiByteString;
...
NTSTATUS RtlUnicodeToMultiByteN(OUT PCH MultiByteString,
    ...
    Arguments:
        MultiByteString - Returns an ansi string that is equivalent to the
            unicode source string.  If the translation can not be done, an error is returned.
        MaxBytesInMultiByteString - Supplies the maximum number of bytes to be
            written to MultiByteString.  If this causes MultiByteString to be a
            truncated equivalent of UnicodeString, no error condition results.
