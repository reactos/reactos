...
ARC_STATUS LowGetDriveGeometry(
    IN  PCHAR   DevicePath,
...
ARC_STATUS LowGetPartitionGeometry(
...
/*++
Routine Description:
    This routine computes the drive geometry for the given partition or physical disk.
Arguments:
...
--*/
{
    ...
    *Heads           = 64;
    ...
    // See if there is device specific data describing the geometry of the drive.
    // If there is none, then just use the default SCSI values.
    ...
    if (DiskComponent->ConfigurationDataLength == sizeof(CM_PARTIAL_RESOURCE_LIST) + sizeof(CM_DISK_GEOMETRY_DEVICE_DATA)  ) {
        ...
        if (r == ESUCCESS) {
            ...
            if  ( (DiskConfiguration->Version == 1 && DiskConfiguration->Revision >=3 ) || (DiskConfiguration->Version >  1) ) {
                ...
                if (DiskData->Type == CmResourceTypeDeviceSpecific) {
                    if (DiskData->u.DeviceSpecificData.DataSize == sizeof(CM_DISK_GEOMETRY_DEVICE_DATA)) {
                        DiskGeometry     = (CM_DISK_GEOMETRY_DEVICE_DATA *) &(DiskConfiguration->PartialDescriptors[DiskConfiguration->Count]);
...
//    PrintError("Heads = %08x",*Heads);
    ...
    l.QuadPart = file_info.EndingAddress.QuadPart - file_info.StartingAddress.QuadPart;
...
/*++
Routine Description:
    This routine reads 'NumberOfSectors' sectors starting at sector 'StartingSector' on the volume with ID 'VolumeId'.
Arguments:
...
/*++
Routine Description:
    This routine write 'NumberOfSectors' sectors starting at sector 'StartingSector' on the volume with ID 'VolumeId'.
Arguments:
    VolumeId        - Supplies the ID for the volume.
    SectorSize      - Supplies the number of bytes per sector.
...
Return Value:
    Returns a pointer to a string which contains the ARC pathname for the component.
    NOTE: The string is stored in static storage, and must be copied by the user before another call to this routine.
--*/
{
    ...
    while ((ParentComponent = ArcGetParent(Component)) != NULL) {
        ...
        sprintf(NewSegment, "%s(%d)", MnemonicTable[Component->Type], Component->Key);
...
/*++
Routine Description:
    This routine computes a path from a component.
    The resulting path is allocated on the heap.
Arguments:
...
ARC_STATUS LowTraverseChildren(
...
Routine Description:
    ...
    If the Matching array is provided then it will be filled with pointers to all of the components whose type and class
    match 'ConfigType' and 'ConfigClass'.
    Also, the 'CurrentLength' is incremented by the number of nodes that match.
Arguments:
...
--*/
{
    ...
    for (;;) {
        if ((!ConfigClass || pc->Class == *ConfigClass) && (!ConfigType  || pc->Type  == *ConfigType)) {
            if (MatchingArray) {
        ...
        r = LowTraverseChildren(pc, ConfigClass, ConfigType, MatchingArray, CurrentLength);
        if (r != ESUCCESS) {
            return r;
        }
...
ARC_STATUS LowQueryComponentList(
    IN  CONFIGURATION_CLASS*        ConfigClass OPTIONAL,
    IN  CONFIGURATION_TYPE*             ConfigType OPTIONAL,
...
/*++
Routine Description:
    This routine returns an array of components whose class and type match the ones given.
    (Since each parameter is optional, you can do type-only and class-only searches.)

    The array is allocated on the heap and contains pointers to the actual components (NOT copies).

Arguments:
    ConfigClass     - Supplies the configuation class to search for.
    ConfigType      - Supplies the configuration type to search for.
    ComponentList   - Returns a list of pointers to components whose class and type match 'ConfigClass' and 'ConfigType'.
...
--*/
{
    ...
    *ListLength = 0;
    ...
    if (!(*ComponentList = (PCONFIGURATION_COMPONENT*) AllocateMemory((*ListLength)*sizeof(PCONFIGURATION_COMPONENT)))) {
        return ENOMEM;
    }

    *ListLength = 0;

    return LowTraverseChildren(NULL, ConfigClass, ConfigType, *ComponentList, ListLength);
}


ARC_STATUS LowQueryPathList(
    IN  CONFIGURATION_CLASS*        ConfigClass OPTIONAL,
    IN  CONFIGURATION_TYPE*             ConfigType OPTIONAL,
    OUT PCHAR**                     PathList,
    OUT PULONG                      ListLength
...
/*++
Routine Description:
    This routine returns a list of paths to the components that are of class ConfigClass and of type ConfigType.
Arguments:
    ConfigClass     - Supplies the configuation class to search for.
    ConfigType      - Supplies the configuration type to search for.
...
--*/
{
    ...
    r = LowQueryComponentList(ConfigClass, ConfigType, &component_list, &list_length);
    if (r != ESUCCESS) {
        return r;
    }
...
/*++
Routine Description:
    This routine returns a list of paths to all the devices of interest to FDISK.
Arguments:
...
#define PLENGTH(p)   (                              \
    ...
    DriveInfo = AlReallocateHeap(DriveInfo, sizeof(DRIVE_LAYOUT_INFORMATION) + ((Entry - 1) * sizeof(PARTITION_INFORMATION)));
...
#define SECCNT(l) ((ULONG)(((ULONGLONG)l.QuadPart)/((ULONGLONG)bps)))
    ...
    // Use x86 bootcode as a template so the disk will boot an x86 if it is moved to disk0 on an x86 machine.
    ...
    for(i=0; i<DriveLayout->PartitionCount; i+=ENTRIES_PER_BOOTSECTOR) {
        ...
        for(j=0; j<ENTRIES_PER_BOOTSECTOR; j++) {
            ...
            case SYSID_EXTENDED:
                ...
                SetPartitionTableEntry(PartitionTable+j, p->BootIndicator, SYSID_EXTENDED, SECCNT(p->StartingOffset) - ExtendedPartitionStart, SECCNT(p->PartitionLength), &chs);
                if(mbr) {
                    mbr = FALSE;
            ...
            default:
                ...
                SetPartitionTableEntry(PartitionTable+j, p->BootIndicator, p->PartitionType, SECCNT(p->StartingOffset) - BootSector, SECCNT(p->PartitionLength), &chs);
                break;
            }
