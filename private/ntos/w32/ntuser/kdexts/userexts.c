...
#define FOREACHPPI(ppi)                                              \
    ...
    SAFEWHILE(NextProcess != ProcessHead) {                          \
        pEProcess = GetEProcessData((PEPROCESS)NextProcess, PROCESS_PROCESSHEAD, NULL); \
                                                                     \
        if (GetEProcessData(pEProcess, PROCESS_PROCESSLINK, &List) == NULL) { \
            Print("Unable to read _EPROCESS at %lx\n",pEProcess);    \
            break;                                                   \
        }                                                            \
        NextProcess = List.Flink;                                    \
                                                                     \
        if (GetEProcessData(pEProcess, PROCESS_WIN32PROCESS, &pW32Process) == NULL || pW32Process == NULL) { \
            continue;                                                \
        }                                                            \
                                                                     \
...
BOOL GetProcessName(PEPROCESS pEProcess, LPWSTR lpBuffer)
/*
...
BOOL GetAppName(PETHREAD pEThread, PTHREADINFO pti, LPWSTR lpBuffer, DWORD cbBuffer)
/*
...
#define NQEVENT (sizeof(aszEvents)/sizeof(aszEvents[0]))
    ...
    SAFEWHILE(TRUE) {
        ...
        case QEVENT_ASYNCSENDMSG:
            ...
            Print("%07lx %04lx %08lx %08lx %08lx %08lx %08lx %08lx\n", asm.hwnd, asm.message, asm.wParam, asm.lParam, qmsg.msg.time, qmsg.ExtraInfo, qmsg.dwQEvent, qmsg.pti);
            break;
        case 0:
        default:
            Print("%07lx %04lx %08lx %08lx %08lx %08lx %08lx %08lx\n", qmsg.msg.hwnd, qmsg.msg.message, qmsg.msg.wParam, qmsg.msg.lParam, qmsg.msg.time, qmsg.ExtraInfo, qmsg.dwQEvent, qmsg.pti);
            break;
        }
...
BOOL HtoHE(ULONG_PTR h, HANDLEENTRY *phe, HANDLEENTRY **pphe) // Optional
/*
...
BOOL getHEfromP(
    ...
    PVOID p)
    /*
    * Description: Converts a pointer to a handle and extracts the he and phe info.
...
*/
{
    ...
    if (tryMove(pT, p) && getHEfromP(NULL, &he, p)) {
         ...
         * It was a pointer
         */
        if ((type == -2 || he.bType != TYPE_FREE) && he.bType < TYPE_CTYPES && ((int)type < 0 || he.bType == type)) {
...
*/
{
    ...
    if (wnd.strName.Buffer == NULL) {
        ...
        RtlUnicodeToMultiByteN(achDest, cbText / sizeof(WCHAR), NULL, awch, cbText);
    }
...
BOOL DebugGetClassNameA(LPSTR lpszClassName, char *achDest)
/*
* Description: Placed pcls name into achDest.  No checks for size are made.
...
*/
LPCSTR GetGDIHandleType(HANDLE handle)
{
...
#ifdef KERNEL
VOID DumpAtomTable(PRTL_ATOM_TABLE *ppat, ATOM a)
/*
    ...
    for (iBucket = 0; iBucket < (int)at.NumberOfBuckets; iBucket++) {
        ...
        SAFEWHILE(pate != NULL) {
            ...
            if (a == 0 || a == (ATOM)(ate.HandleIndex | MAXINTATOM)) {
                Print("%hx(%2d) = %ls (%d)%s\n", (ATOM)(ate.HandleIndex | MAXINTATOM), ate.ReferenceCount, pwsz, ate.NameLength, ate.Flags & RTL_ATOM_PINNED ? " pinned" : "");
...
BOOL Iatom(DWORD opts, PVOID param1)
/*
...
#ifndef KERNEL
BOOL GetTargetTEB(PTEB pteb, PTEB *ppteb) // OPTIONAL
/*
...
*/
{
    ...
    Status = NtQueryInformationThread(hCurrentThread, ThreadBasicInformation, &ThreadInformation, sizeof(ThreadInformation), NULL);
...
*/
{
    ...
    if (HighestUserAddress == 0) {
        ...
        if (NT_SUCCESS(NtQuerySystemInformation(SystemBasicInformation, &SystemInformation, sizeof(SystemInformation), NULL))) {
            ...
            // Query failed.  Assume usermode is the low half of the address space.
...
BOOL Idcls(DWORD opts, PVOID param1)
/*
...
*/
{
    ...
    if (opts & OFLAG(v)) {
        ...
        Print("\t lpszClientMenu    @0x%p (%s)\n", localCLS.lpszClientUnicodeMenuName, ach);
...
VOID PrintCurData(PCURSOR pcur, DWORD opts)
{
    CURSOR cur;

    move(cur, pcur);

    if ((opts & OFLAG(x)) && cur.CURSORF_flags & (CURSORF_ACONFRAME | CURSORF_LINKED)) {
    ...
    if (cur.CURSORF_flags & CURSORF_ACON) {
        ...
        if (opts & OFLAG(v)) {
            Print("\nCursor/Icon @%x:\n", pcur);
            Print("  ppiOwner       = %#p(%s)\n", (DWORD_PTR)cur.head.ppi, ProcessName(cur.head.ppi));
            Print("  pcurNext       = %x\n", cur.pcurNext);
            Print("  CURSORF_flags  = %s\n", GetFlags(GF_CURSORF, cur.CURSORF_flags, NULL, TRUE));
            Print("  strName        = %#p\n", (DWORD_PTR)cur.strName.Buffer);
            Print("  atomModName    = %x\n", cur.atomModName);
...
BOOL Idcur(DWORD opts, PVOID param1)
/*
        ...
        FOREACHHANDLEENTRY(phe, he, i)
            if (he.bType == TYPE_CURSOR) {
                ...
                if (tryMove(cur, he.phead)) {
                    if (!(cur.CURSORF_flags & (CURSORF_LINKED | CURSORF_ACONFRAME)) && (!idDesired || (INT_PTR)cur.strName.Buffer == idDesired) && (ppiDesired == NULL || ppiDesired == cur.head.ppi)) {
                        Print("%x:", i);
...
#ifdef KERNEL
BOOL dddexact(DWORD_PTR pOrg, PXSTATE pxs, DWORD opts)
/*
...
#ifdef KERNEL
BOOL dddeconv(DWORD_PTR pOrg, PDDECONV pddeconv, DWORD opts)
/*
...
#ifdef KERNEL
BOOL Idde(DWORD opts, PVOID param1)
/*
...
*/
{
        ...
        case TYPE_WINDOW:
            ...
            for (i = 0; i < propList.iFirstFree; i++) {
                ...
                if (prop.atomKey == (ATOM)MAKEINTATOM(atomDdeTrack)) {
                    ...
                    Print("  ");
                    dddeconv((DWORD_PTR)prop.hData, &ddeconv, opts);
                }
            }
            return TRUE;
        case TYPE_DDECONV:
    ...
    for (i = 0; i < cHandleEntries; i++) {
        ...
        if (he.bType == TYPE_DDECONV && (pObj == FIXKP(he.phead) || pObj == NULL)) {
            ...
            dddeconv((DWORD_PTR)FIXKP(he.phead), (PDDECONV)&ddeconv, opts);
        }
        ...
        if (he.bType == TYPE_DDEXACT && (pObj == NULL || pObj == FIXKP(he.phead))) {
            ...
            dddexact((DWORD_PTR)FIXKP(he.phead), (PXSTATE)&xs, opts);
            Print("\n");
        }
    }
    ...
    SAFEWHILE(pcii != NULL) {
        ...
        if (Instance == 0 || (Instance == (DWORD_PTR)cii.hInstClient)) {
            ...
            SAFEWHILE(ch--) {
                ...
                if (Object && (he.handle != (HANDLE)Object) && Pointer && he.dwData != Pointer) {
                    continue;
                }
...
*/
BOOL Iddesk(DWORD opts, PVOID param1)
{
...
{
    return ('0' <= c && c <= '9') || (base == 16 && (('a' <= c && c <= 'f') || ('A' <= c && c <= 'F')));
}
...
BOOL Idf(DWORD opts, LPSTR pszName)
{
...
#ifdef KERNEL
        (lpExtensionApis->lpWriteProcessMemoryRoutine)((ULONG_PTR)(&psi->wRIPPID), (void *)&wPID, sizeof(wPID), NULL);
#else
        if (!IsRemoteSession()) {
    ...
    if (NT_SUCCESS(status) && !(ulFlags & ~RIPF_VALIDUSERFLAGS)) {
        ...
        (lpExtensionApis->lpWriteProcessMemoryRoutine)((ULONG_PTR)(&psi->wRIPFlags), (void *)&wRipFlags, sizeof(wRipFlags), NULL);
#else
...
BOOL Itag(DWORD opts, LPSTR pszName)
{
    ...
    if (!NT_SUCCESS(status) || tag < 0 || DBGTAG_Max <= tag) {
        ...
        if (NT_SUCCESS(status) && !(dwDBGTAGFlagsNew & ~DBGTAG_VALIDUSERFLAGS)) {
            ...
            (lpExtensionApis->lpWriteProcessMemoryRoutine)((DWORD_PTR)(&psi->adwDBGTAGFlags[tag]), (void *)&dwDBGTAGFlags, sizeof(dwDBGTAGFlags), NULL);
#else
     ...
     * Print the header.
     */
    Print("%-5s%-7s%-*s%-*s\n", "Tag", "Flags", DBGTAG_NAMELENGTH, "Name", DBGTAG_DESCRIPTIONLENGTH, "Description");
    ...
    for (i = iStart; i <= iEnd; i++) {
        ...
        Print("%-5d%-7d%-*s%-*s\n", i, dwDBGTAGFlags & DBGTAG_VALIDUSERFLAGS, DBGTAG_NAMELENGTH, dbgtag.achName, DBGTAG_DESCRIPTIONLENGTH, dbgtag.achDescription);
    }
...
*/
{
    ...
    if (!(opts & OFLAG(r))) {
        ...
        Print(szFormat, pheT, head.h, FIXKP(he.phead), FIXKP(he.pOwner), head.cLockObj, aszTypeNames[(DWORD)he.bType], (DWORD)he.bFlags);
    }
...
* 6/9/1995 SanfordS     made to fit stdexts motif
*/
BOOL Idhk(DWORD opts, PVOID param1)
{
    DWORD dwFlags;
    PTHREADINFO pti;
    THREADINFO ti;
...
#define DHKF_PTI_GIVEN      2
    ...
    if (opts & OFLAG(a)) {
        ...
        SAFEWHILE(pti != NULL) {
            ...
            sprintf(ach, "%lx", pti);
            dhk(hCurrentProcess, hCurrentThread, dwCurPc, dwCurrentPc, ach);
            move(pti, &(pti->ptiNext));
        }
        if (dwFlags & DHKF_GLOBAL_HOOKS) {
            dhk(hCurrentProcess, hCurrentThread, dwCurPc, dwCurrentPc, "g");
        }
        return TRUE;
    }
#endif
    ...
    if (dwFlags & DHKF_PTI_GIVEN || !(dwFlags & DHKF_GLOBAL_HOOKS)) {
        Print("Local hooks on PTHREADINFO @ 0x%p%s:\n", pti, (dwFlags & DHKF_PTI_GIVEN ? "" : " (foreground thread)"));
...
*/
{
    ...
    SAFEWHILE(phk != NULL) {
        move(hk, phk);
        Print("fsModifiers %lx, vk %x - ", hk.fsModifiers, hk.vk);
        Print("%s%s%s%sVK:%x %s",
...
#ifdef KERNEL
BOOL Idhs(DWORD opts, PVOID param1)
/*
...
*/
{
    ...
    if (opts & OFLAG(p) && (ahti[he.bType].bObjectCreateFlags & OCF_PROCESSOWNED)) {
        if (he.pOwner == NULL) {
            continue;
        }
    ...
    else if ((opts & OFLAG(t)) && !(ahti[he.bType].bObjectCreateFlags & OCF_PROCESSOWNED)) {
        if (he.pOwner == NULL) {
            continue;
        }
...
*/
{
    ...
    if (*lpas != 0) {
        ...
        if (cDwordsBack > cDwords) {
            Print("backing up %d DWORDS per struct (b%d): ", cDwordsBack, cDwordsBack);
    ...
    for (i = 0; i < cStructs; i++) {
        ...
        if (!fTestAndCountOnly) {
            ...
            for (j = 0; j < cDwords; j += 4) {
                switch (cDwords - j) {
                case 1:
                    Print("%08lx:  %08lx\n", dw + j * sizeof(DWORD), adw[j + 0]);
                    break;
                case 2:
                    Print("%08lx:  %08lx %08lx\n", dw + j * sizeof(DWORD), adw[j + 0], adw[j + 1]);
                    break;
                case 3:
                    Print("%08lx:  %08lx %08lx %08lx\n", dw + j * sizeof(DWORD), adw[j + 0], adw[j + 1], adw[j + 2]);
                    break;
                default:
                    Print("%08lx:  %08lx %08lx %08lx %08lx\n", dw + j * sizeof(DWORD), adw[j + 0], adw[j + 1], adw[j + 2], adw[j + 3]);
                }
            }
        }
        else if ((i & 0xff) == 0xff) {
...
* 6/9/1995 SanfordS     made to fit stdexts motif
*/
BOOL Idlr(DWORD opts, PVOID param1)
{
    HANDLEENTRY he;
#if DBG
    PLR plrT;
...
void DumpMenu(UINT uIndent, DWORD opts, PMENU pMenu)
{
    ...
    if (!(opts & OFLAG(v))) {
        ...
        Print("%s     fFlags............ %s\n"
              "%s     location.......... (%lu, %lu)\n",
              szIndent, GetFlags(GF_MF, (WORD)localMenu.fFlags, NULL, TRUE), szIndent, localMenu.cxMenu, localMenu.cyMenu);
        Print("%s     spwndNotify....... 0x%p\n"
              "%s     dwContextHelpId... 0x%08lX\n"
              "%s     items............. %lu items in block of %lu\n",
              szIndent, localMenu.spwndNotify, szIndent, localMenu.dwContextHelpId, szIndent, localMenu.cItems, localMenu.cAlloced);
    }
...
#ifdef KERNEL
BOOL Idmq(DWORD opts, PVOID param1)
/*
...
#ifdef KERNEL
BOOL Idwe(DWORD opts, PVOID param1)
/*
* dwe - dump winevents
...
#ifndef KERNEL
BOOL Idped(DWORD opts, PVOID param1)
/*
...
* 6/9/1995 Created SanfordS
*/
BOOL Idpi(DWORD opts, PVOID param1)
{
    PW32PROCESS pW32Process;
    PPROCESSINFO ppi;
    PROCESSINFO pi;
    ...
    if (opts & OFLAG(c)) {
        ...
        param1 = (PVOID)GetCurrentProcessAddress((USHORT)dwProcessor, hCurrentThread, NULL);
        if (param1 == 0) {
    ...
    Print("\tluidSession        0x%08lx:0x%08lx\n", pi.luidSession.HighPart, pi.luidSession.LowPart);
...
* 6/9/1995 SanfordS made to fit stdexts motif
*/
BOOL Idpm(DWORD opts, PVOID param1)
{
...
#ifdef KERNEL
BOOL Idms(DWORD opts, PVOID param1)
/*
* dms - dump pMenuState
...
* 6/9/1995 SanfordS     made to fit stdexts motif
*/
BOOL Idq(DWORD opts, PVOID param1)
{
    char ach[80];
    PQ pq;
    Q q;
    ...
    Print(
        ...
        q.mlInput.pqmsgRead, q.mlInput.pqmsgWriteLast, q.mlInput.cMsgs);

    Print("\tptiSysLock             0x%08lx\n"
          "\tidSysLock              0x%08lx\n"
          "\tidSysPeek              0x%08lx\n",
          q.ptiSysLock, q.idSysLock, q.idSysPeek);

    Print("\tptiMouse               0x%08lx\n"
          "\tptiKeyboard            0x%08lx\n",
          q.ptiMouse, q.ptiKeyboard);

    Print("\tspcurCurrent           0x%08lx\n"
          "\tiCursorLevel           0x%08lx\n",
          q.spcurCurrent, q.iCursorLevel);
    ...
    Print("\tcodeCapture            0x%04lx\n"
          "\tmsgDblClk              0x%04lx\n"
          "\ttimeDblClk             0x%08lx\n",
          q.codeCapture, q.msgDblClk, q.timeDblClk);

    Print("\thwndDblClk             0x%08lx\n", q.hwndDblClk);

    Print("\tptDblClk               { %d, %d }\n", q.ptDblClk.x, q.ptDblClk.y);

    Print("\tQF_flags               0x%08lx %s\n"
          "\tcThreads               0x%08lx\n"
          "\tcLockCount             0x%08lx\n",
          q.QF_flags, GetFlags(GF_QF, q.QF_flags, NULL, FALSE), (DWORD)q.cThreads, (DWORD)q.cLockCount);

    Print("\tmsgJournal             0x%08lx\n"
          "\tExtraInfo              0x%08lx\n",
          q.msgJournal, q.ExtraInfo);
...
*/
{
    ...
    Print("  pxOld            0x%08lx\n", sbt.pxOld);
    Print("  rcTrack          (0x%08lx,0x%08lx,0x%08lx,0x%08lx)\n", sbt.rcTrack.left, sbt.rcTrack.top, sbt.rcTrack.right, sbt.rcTrack.bottom);
...
*/
{
    ...
    if (opts & OFLAG(m)) {
        ...
        for (i = 0; i < SM_CMETRICS; i++) {
            Print("\t\tSM_%-18s = 0x%08lx = %d\n", aSysMet[i].pstrMetric, si.aiSysMet[aSysMet[i].iMetric], si.aiSysMet[aSysMet[i].iMetric]);
        }
    }
    ...
    if (opts & OFLAG(o)) {
        ...
        for (i = 0; i < OBI_COUNT; i++) {
            Print("\tbm[%d]:\t%08x\t%08x\t%08x\t%08x\n", i, si.oembmi[i].x, si.oembmi[i].y, si.oembmi[i].cx, si.oembmi[i].cy);
        }
        Print(
            "\t\tPlanes             = %d\n"
            "\t\tBitsPixel          = %d\n"
...
#ifdef KERNEL
BOOL Idsms(DWORD opts, PVOID param1)
/*
...
#ifdef KERNEL
BOOL DumpThread(DWORD opts, PETHREAD pEThread)
/*
* dt - dump thread
...
*/
{
    ...
    if (ti.pEThread != pEThread || pti == NULL) {
      ...
      * out queue info if in verbose mode (printing out queue info also prints out simple thread info).
      */
        if (!(opts & OFLAG(v))) {
            PWCHAR pwch;
            ...
            Print("  et 0x%08lx t 0x%08lx q 0x%08lx i %2x.%-3lx %ws\n", pEThread, pti, ti.pq, EThread.Cid.UniqueProcess, EThread.Cid.UniqueThread, pwch);
...
#define DT_INDENT "\t"
                ...
                else if ((cti.fsWakeMask == (QS_SMSREPLY | QS_SENDMESSAGE)) || (cti.fsWakeMask == QS_SMSREPLY)) {
...
void DumpProcessThreads(DWORD opts, PEPROCESS pEProcess, ULONG_PTR ThreadToDump)
{
    PW32PROCESS pW32Process;
    PETHREAD pEThread;
    ETHREAD EThread;
    ...
    SAFEWHILE(NextThread != ThreadListHead) {
         ...
         */
        if (ThreadToDump == 0 || (ThreadToDump < UserProbeAddress && ThreadToDump == (ULONG_PTR)EThread.Cid.UniqueThread) || (ThreadToDump >= UserProbeAddress && ThreadToDump == (ULONG_PTR)pEThread)) {
...
BOOL Idt(DWORD opts, PVOID param1)
{
    ...
    if (opts & OFLAG(p)) {
        ...
        if (pti == NULL) {
            ...
            if (!DumpThread(opts, ti.pEThread)) {
                 ...
                 * is whacked out.  Just skip it if we want all threads.
                 */
    ...
    if (opts & OFLAG(c)) {
        ...
        ThreadToDump = (ULONG_PTR)GetCurrentThreadAddress((USHORT)dwProcessor, hCurrentThread);
        if (ThreadToDump == 0) {
...
#ifdef KERNEL
BOOL DumpProcess(DWORD opts, PEPROCESS pEProcess)
/*
* dp - dump process
...
*/
{
    ...
    if (!tryMove(pi, ppi)) {
        Print("sid %2d ep 0x%08lx p 0x???????? f 0x???????? i %2x <unknown name>\n", EProcess.SessionId, pEProcess, EProcess.UniqueProcessId);
        return TRUE;
    }
    ...
    if (pi.Process != pEProcess || ppi == NULL) {
      ...
      */
        if (!(opts & OFLAG(v))) {
            ...
            Print("sid %2d ep 0x%08lx p 0x%08lx f 0x%08lx i %2x %ws\n", EProcess.SessionId, pEProcess, ppi, pi.W32PF_Flags, EProcess.UniqueProcessId, pwch);
        }
        else {
            Idpi(0, ppi);
            Print("\n");
...
BOOL Idp(DWORD opts, PVOID param1)
{
    ...
    if (opts & OFLAG(c)) {
        Print("Current Process:");
        ProcessToDump = (ULONG_PTR)GetCurrentProcessAddress((USHORT)dwProcessor, hCurrentThread, NULL);
        if (ProcessToDump == 0) {
            Print("Unable to get current process pointer.\n");
            return FALSE;
        }
    ...
    SAFEWHILE(NextProcess != ProcessHead) {
         ...
         */
        if (ProcessToDump == 0 || (ProcessToDump < UserProbeAddress && ProcessToDump == (ULONG_PTR)EProcess.UniqueProcessId) || (ProcessToDump >= UserProbeAddress && ProcessToDump == (ULONG_PTR)pEProcess)) {
            if (!DumpProcess(opts, pEProcess) && (ProcessToDump != 0)) {
...
#ifdef KERNEL
BOOL Idtdb(DWORD opts, PVOID param1)
/*
* dtdb - dump TDB
...
*/
{
    ...
    if (fServerProcess) {
         ...
         */
        CsrClientCallServer((PCSR_API_MSG)&m, NULL, CSR_MAKE_API_NUMBER(USERSRV_SERVERDLL_INDEX, UserpActivateDebugger), sizeof(*a));
    }
...
BOOL Idkl(DWORD opts, PVOID param1)
/*
    ...
    SAFEWHILE(NextProcess != ProcessHead) {
        ...
        SAFEWHILE(NextThread != ThreadListHead) {
            ...
            if (ti.pEThread != pEThread || pti == NULL) {
                ...
                nThread++;
                Print("t 0x%08lx i %2x.%-3lx k 0x%08lx  %ws\n", pti, EThread.Cid.UniqueProcess, EThread.Cid.UniqueThread, ti.spklActive, pwch);
                if (opts & OFLAG(v)) {
...
BOOL Iddk(DWORD opts, PVOID param1)
/*
...
BOOL Idii(DWORD opts, PVOID param1)
/*
* dii - dump IMEINFOEX
...
BOOL Idti(DWORD opts, PVOID param1)
/*
* dti - dump THREADINFO
...
BOOL Idtl(DWORD opts, PVOID param1)
/*
* dtl handle|pointer
...
#ifdef KERNEL
BOOL Idtmr(DWORD opts, PVOID param1)
/*
    ...
    if (tryMove(ti, tmr.pti)) {
        ...
        if (tryMove(EThread, ti.pEThread)) {
            if (GetAppName(&EThread, &ti, awch, sizeof(awch))) {
                ...
                Print("  q 0x%08lx i %2x.%-3lx %ws", ti.pq, EThread.Cid.UniqueProcess, EThread.Cid.UniqueThread, pwch);
            }
        }
    }
    Print("\n"
...
*/
{
    ...
    if (aLocalPrevHti[TYPE_WINDOW].lTotalCount) {
        ...
        for (idx = 1; idx < TYPE_CTYPES; idx++) {
            Print("\n%-17s  %9d  %7d  %6d  %d", aszTypeNames[idx], aLocalHti[idx].lTotalCount, aLocalHti[idx].lMaxCount, aLocalHti[idx].lCount, aLocalHti[idx].lSize);
            lTotalAlloc += aLocalHti[idx].lTotalCount;
            lTotalMax += aLocalHti[idx].lMaxCount;
            lTotalCurrent += aLocalHti[idx].lCount;
            lTotalSize += aLocalHti[idx].lSize;
     ...
     */
    if (opts & OFLAG(s)) {
        (lpExtensionApis->lpWriteProcessMemoryRoutine)((ULONG_PTR)(&(pgahti[0])), (void*)(aLocalHti), sizeof(aLocalHti), NULL);
    }
...
*/
{
    ...
    if (opts & OFLAG(t)) {
         ...
         */
        pti = (PTHREADINFO)param1;
        if (!tryMove(pdesk, &pti->rpdesk) || !tryMove(pdi, &pdesk->pDeskInfo) || !tryMove(pwnd, &pdi->spwnd) || !tryMove(pwnd, &pwnd->spwndChild)) {
            return FALSE;
        }
        /*
    ...
    if (!(opts & OFLAG(v))) {
        ...
        if (opts & OFLAG(h)) {
            if (IsWOWProc(wnd.lpfnWndProc)) {
                ...
                Print("wndproc = %04lx:%04lx (WOW) (%s)", HIWORD(dwWOW), LOWORD(dwWOW), TestWF(&wnd, WFANSIPROC) ? "ANSI" : "Unicode");
            }
            else {
                GetSym((LPVOID)wnd.lpfnWndProc, ach, &dwOffset);
                Print("wndproc = 0x%p = \"%s\" (%s)", wnd.lpfnWndProc, ach, TestWF(&wnd, WFANSIPROC) ? "ANSI" : "Unicode");
            }
        }
        else {
            if (IsWOWProc(wnd.lpfnWndProc)) {
                ...
                Print("wndproc = 0x%p (%s)", wnd.lpfnWndProc, TestWF(&wnd, WFANSIPROC) ? "ANSI" : "Unicode");
            }
        }
        ...
        if (IsWOWProc(wnd.lpfnWndProc)) {
            UnMarkWOWProc(wnd.lpfnWndProc, dwWOW);
            Print("\tlpfnWndProc       @%04lx:%04lx (WOW) (%s)\n", HIWORD(dwWOW), LOWORD(dwWOW), TestWF(&wnd, WFANSIPROC) ? "ANSI" : "Unicode");
        }
        else {
            GetSym((LPVOID)wnd.lpfnWndProc, ach, &dwOffset);
            Print("\tlpfnWndProc       @0x%p     (%s) %s\n", wnd.lpfnWndProc, ach, TestWF(&wnd, WFANSIPROC) ? "ANSI" : "Unicode");
        }
        move(cls, FIXKP(wnd.pcls));
        if (cls.atomClassName < 0xC000) {
            sprintf(ach, "0x%04x", cls.atomClassName);
    ...
    if (opts & OFLAG(f)) {
        ...
        for (i = 0; i < ARRAY_SIZE(aWindowFlags); i++) {
            ...
            if (pbyte[HIBYTE(wFlag)] & LOBYTE(wFlag)) {
                Print("\t%-18s\t%lx:%02lx\n", aWindowFlags[i].pszText, (PBYTE)&(pwnd->state) + HIBYTE(wFlag), LOBYTE(wFlag));
            }
        }
    }
    ...
    if (opts & OFLAG(o)) {
        ...
        if (!ppropList) {
            ...
            Print("\tProperty List @0x%08x : %d Properties, %d total entries, %d free entries\n", ppropList, propList.iFirstFree, propList.cEntries, propList.cEntries - propList.iFirstFree);
...
* 6/9/1995 Created SanfordS
*/
BOOL Idwpi(DWORD opts, PVOID param1)
{
...
BOOL DHAVerifyHeap(PWIN32HEAP pHeap, BOOL bVerbose)
{
    ...
    sizeHead = pHeap->dwFlags & WIN32_HEAP_USE_GUARDS ? sizeof(pHeap->szHead) : 0;
...
BOOL Idha(DWORD opts, PVOID pointer)
{
    int ind, counter;
    SIZE_T sizeHead;
    ...
    for (ind = counter = 0; ind < MAX_HEAPS; ind++) {
         ...
         */
        if ((opts & OFLAG(a)) == 0) {
            if ((PVOID)localgWin32Heaps[ind].heap > pointer || (PBYTE)localgWin32Heaps[ind].heap + localgWin32Heaps[ind].heapReserveSize < (PBYTE)pointer) {
                continue;
            }
        }
        ...
        if (localgWin32Heaps[ind].dwFlags & WIN32_HEAP_INUSE) {
            ...
            if (localgWin32Heaps[ind].dwFlags & WIN32_HEAP_USE_GUARDS) {
                ...
                Print("    has string quards szHead %s, szTail %s\n", localgWin32Heaps[0].szHead, localgWin32Heaps[0].szTail);
            }
            else {
                sizeHead = 0;
            ...
            do {
                ...
                if ((PBYTE)pAlloc - sizeHead < (PBYTE)pointer && (PBYTE)pAlloc + sizeof(DbgHeapHead) + Alloc.size + sizeHead >(PBYTE)pointer) {
                    /*
...
* 6/9/1995 SanfordS made to fit stdexts motif
*/
BOOL Idws(DWORD opts, PVOID param1)
{
    ...
    Print("  luidUser           = %0lx.%lx\n", winsta.luidUser.HighPart, winsta.luidUser.LowPart);
    return TRUE;
}


BOOL Iddl(DWORD opts, PVOID param1)
/*
...
*/
{
    ...
    for (i = 0; i < desk.nLogCrt; i++) {
        ...
        Print("%s Tag %6d Extra %8lx\n", (log.type ? "LOCK  " : "UNLOCK"), log.tag, log.extra);
...
BOOL Idcss(DWORD opts, PVOID param1)
/*
...
void PrintStackTrace(PVOID* parrTrace, int    tracesCount)
{
    int       traceInd;
    DWORD_PTR dwOffset;
    CHAR      symbol[160];
...
BOOL Idvs(DWORD opts, PVOID param1)
{
...
BOOL Idfa(DWORD opts, PVOID param1)
{
#if DBG
    DWORD_PTR  dwOffset;
    CHAR       symbol[160];
...
BOOL Idpa(DWORD opts, PVOID param1)
/*
...
*/
{
    ...
    if (opts & OFLAG(c)) {
        ...
        Print("-|-|-|-|-|\n");
        Print(" 0x%08x   0x%08x   0x%08x   0x%08x   0x%08x\n", AllocList.dwCrtMem, AllocList.dwCrtAlloc, AllocList.dwMaxMem, AllocList.dwMaxAlloc, AllocList.pHead);
        return TRUE;
    }
    ...
    if (opts & OFLAG(r)) {
        ...
        for (Ind = 0; Ind < dwFreesToDump; Ind++) {
            ...
            Print("Free pool for p %#p size 0x%x\n", FreeRecord.ExtraData, FreeRecord.size);
...
*/
{
    ...
    if (dw != (DWORD_PTR)param1) {
         ...
         */
        FOREACHHANDLEENTRY(phe, he, i)
            if ((DWORD_PTR)he.phead <= dw && heBest.phead < he.phead && he.bType != TYPE_FREE) {
                heBest = he;
                hBest = i;
            }
        if ((DWORD_PTR)he.phead > dw && heAfter.phead > he.phead && he.bType != TYPE_FREE) {
            heAfter = he;
            hAfter = i;
        }
        NEXTEACHHANDLEENTRY()
         ...
         */
        FOREACHHANDLEENTRY(phe, he, i)
            if ((DWORD_PTR)FIXKP(he.phead) <= dw && heBest.phead < he.phead && he.bType != TYPE_FREE) {
                heBest = he;
                hBest = i;
            }
        if ((DWORD_PTR)FIXKP(he.phead) > dw && heAfter.phead > he.phead && he.bType != TYPE_FREE) {
            heAfter = he;
            hAfter = i;
        }
        NEXTEACHHANDLEENTRY()
...
*/
{
    ...
    Print("Searching range (%#p-%#p) for references to (%#p-%#p)...", pSrc1, pSrc2, pRef1, pRef2);
        ...
        if (!fSuccess) {
            ...
            for (dw = 0; dw < ARRAY_SIZE(buffer); dw++) {
                if (buffer[dw] >= pRef1 && buffer[dw] <= pRef2) {
                    Print("\n[%#p] = %#p ", pSrc1 + dw * sizeof(DWORD_PTR), buffer[dw]);
                }
            }
        }
        if (IsCtrlCHit()) {
...
#if 0
VOID ddGdiDevice(PGDI_DEVICE pGdiDevice)
{
    Print("\t\tGDI_DEVICE\n");
...
BOOL Idy(DWORD opts, PVOID param1)
{
    ...
    for (pMonitor = RebaseSharedPtr(di.pMonitorFirst), i = 1; pMonitor; pMonitor = RebaseSharedPtr(monitor.pMonitorNext), i++) {
...
BOOL Ikbd(DWORD opts, PVOID param1)
{
    PQ pq;
    Q q;
    PBYTE pb, pbr;
...
*/
BOOL Idinp(DWORD opts, PVOID param1)
{
    DWORD    Time;
    NTSTATUS Status;
        ...
        if (param1 && (param1 != pDeviceInfo)) {
            ...
            if (tryMove(DeviceInfo, pDeviceInfo)) {
                ...
                if (DeviceInfo.usActions) {
                    Print(" Pending action %x %s", DeviceInfo.usActions, GetFlags(GF_DIAF, DeviceInfo.usActions, NULL, TRUE));
                }
            ...
            if (bVerbose || (param1 == pDeviceInfo)) {
                ...
                Idso(0, ach);
                gnIndent += 2;
                sprintf(ach, "IO_STATUS_BLOCK 0x%p", (PBYTE)pDeviceInfo + FIELD_OFFSET(DEVICEINFO, iosb));
                Idso(0, ach);
                gnIndent -= 2;
                if (DeviceInfo.type == DEVICE_TYPE_MOUSE) {
                    sprintf(ach, "MOUSE_DEVICE_INFO 0x%p", (PBYTE)pDeviceInfo + FIELD_OFFSET(DEVICEINFO, mouse));
                    Idso(0, ach);

                    gnIndent += 2;
                    sprintf(ach, "MOUSE_ATTRIBUTES 0x%p", (PBYTE)pDeviceInfo + FIELD_OFFSET(DEVICEINFO, mouse) + FIELD_OFFSET(MOUSE_DEVICE_INFO, Attr));
                    Idso(0, ach);
                    gnIndent -= 2;
                }
                else if (DeviceInfo.type == DEVICE_TYPE_KEYBOARD) {
                    sprintf(ach, "KEYBOARD_DEVICE_INFO 0x%p", (PBYTE)pDeviceInfo + FIELD_OFFSET(DEVICEINFO, keyboard));
                    Idso(0, ach);
                    gnIndent += 2;
                    sprintf(ach, "KEYBOARD_ATTRIBUTES 0x%p", (PBYTE)pDeviceInfo + FIELD_OFFSET(DEVICEINFO, keyboard) + FIELD_OFFSET(KEYBOARD_DEVICE_INFO, Attr));
                    Idso(0, ach);
                    gnIndent -= 2;
                }
                else {
                ...
                if (opts & OFLAG(i)) {
                    ...
                    if (DeviceInfo.iosb.Information == 0) {
                        ...
                        Print("\n");
                        gnIndent += 2;
                        sprintf(ach, "KEYBOARD_INPUT_DATA %p *%x", &(pDeviceInfo->keyboard.Data[0]), DeviceInfo.iosb.Information / sizeof(DeviceInfo.keyboard.Data[0]));
                        Idso(0, ach);
                        gnIndent -= 2;
                    }
                }
...
BOOL Ihh(DWORD opts, PVOID param1)
/*
...
LInputContext:
    ...
    if (fShowIMCMinInfo) {
        ...
        Print("  fChgMsg:    %d     uSaveVKey:   %02x %s\n", InputContext.fChgMsg, InputContext.uSavedVKey, GetVKeyName(InputContext.uSavedVKey));
        Print("  StatusWnd:  (0x%x,0x%x)   SoftKbd: (0x%x,0x%x)\n", InputContext.ptStatusWndPos.x, InputContext.ptStatusWndPos.y, InputContext.ptSoftKbdPos.x, InputContext.ptSoftKbdPos.y);
        ...
        // Font
        {
            ...
            Print(fmt, InputContext.lfFont.A.lfFaceName, InputContext.lfFont.A.lfHeight, InputContext.lfFont.A.lfWeight, GetMaskedEnum(EI_CHARSETTYPE, InputContext.lfFont.A.lfCharSet, NULL));
        }
        ...
        if (fDumpInputContext) {
            ...
            if (pCompStr) {
                ...
                Print("  Private: (@%p) off:0x%x len:0x%x\n", (PBYTE)pCompStr + CompStr.dwPrivateOffset, CompStr.dwPrivateOffset, CompStr.dwPrivateSize);
                Print("\n");
            ...
            if (pMsgBuf) {
                ...
                Print(" hMsgBuf: %p (@%p) dwNumMsgBuf=0x%x uMsgCount=0x%x\n", InputContext.hMsgBuf, pMsgBuf, InputContext.dwNumMsgBuf, TransMsgList.uMsgCount);
...
BOOL Iddlgt(DWORD opts, LPVOID param1)
{
    ...
    Style   %08lx   ExStyle %08lx   items 0x%x\n",
              dt.x, dt.y, dt.x + dt.cx, dt.y + dt.cy, dt.cx, dt.cy, dt.style, dt.dwExStyle, dt.cDlgItems);
    }
    else {
        Print("\
    (%d,%d)-(%d,%d) [%d,%d] (dec)  item: 0x%lx\n",
              dt.x, dt.y, dt.x + dt.cx, dt.y + dt.cy, dt.cx, dt.cy, dt.cDlgItems);
        Print("\
...
//        dit.dwExStyle |= WS_EX_NOPARENTNOTIFY;
        ...
        lpdit = (LPDLGITEMTEMPLATE)NextDWordBoundary((LPBYTE)(lpCreateParams + 1) + w);
        Print("\n");
    }
...
BOOL Idimedpi(DWORD opts, PVOID param1)
{
    ...
    while (pImeDpi)
    {
        ...
        if (opts & OFLAG(i)) {
            Print(" ImeInfo: @ %08p   dwPrivateDataSize: %08x\n", &pImeDpi->ImeInfo, ImeDpi.ImeInfo.dwPrivateDataSize);
        }
