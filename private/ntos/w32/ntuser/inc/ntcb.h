...
ULONG_PTR CallHookWithSEH(GENERICHOOKHEADER *pmsg, LPVOID pData, LPDWORD pFlags, ULONG_PTR retval) {
    ...
    try {
        retval = (ULONG_PTR)CALLPROC(pmsg->xpfnProc)(pmsg->nCode, pmsg->wParam, pData, pmsg->xParam);
...
RECVCALL(fnDWORD, FNDWORDMSG)
{
    ...
    retval = (ULONG_PTR)CALLPROC(CALLDATA(xpfnProc))(CALLDATA(pwnd), CALLDATA(msg), CALLDATA(wParam), CALLDATA(lParam), CALLDATA(xParam));
...
SMESSAGECALL(INOUTNCCALCSIZE)
{
    ...
    BEGINSEND(FNINOUTNCCALCSIZE)
        ...
        /*
         * If wParam != 0, lParam points to a NCCALCSIZE_PARAMS structure, otherwise it points to a rectangle.
         */
        if (wParam != 0) {
            ...
            cbCallback = FIELD_OFFSET(FNINOUTNCCALCSIZEMSG, u) + sizeof(RECT);
        }

        /*
         * Don't use the MAKECALL macro so we can select the callback data size
         */
        LOCKPWND();
        LeaveCrit();
        Status = (DWORD)KeUserModeCallback(FI_FNINOUTNCCALCSIZE, mp, cbCallback, &pcbs, &cbCBStatus);
        EnterCrit();
        UNLOCKPWND();
        CHECKRETURN();
        ...
        BEGINCOPYOUT()
            try {
                ...
                if (wParam != 0) {
                    ...
                    *((LPNCCALCSIZE_PARAMS)lParam) = ((POUTNCCALCSIZE)pcbs->pOutput)->params;
...
RECVCALL(fnHkOPTINLPEVENTMSG, FNHKOPTINLPEVENTMSGMSG)
{
    ...
    if (pmsg->wParam) {
        ...
        if (phk != NULL) {
             ...
             * pass on a HC_SKIP
             */
            if ((phk->flags & HF_NEEDHC_SKIP) && (HIWORD(pmsg->nCode) == WH_JOURNALPLAYBACK)) {
                UserAssert(LOWORD(pmsg->nCode) == HC_GETNEXT);
                CALLPROC(pmsg->xpfnProc)(MAKELONG(HC_SKIP, HIWORD(pmsg->nCode)), 0, 0, pmsg->xParam);
            }
...
VOID ClientFontSweep(VOID)
{
    ...
    LeaveCrit();
    KeUserModeCallback(FI_CLIENTFONTSWEEP, NULL, 0, &p, &cb);
    EnterCrit();
}
#endif // SENDSIDE
...
DWORD __ClientFontSweep(PVOID p)
{
...
VOID ClientLoadLocalT1Fonts(VOID)
{
    ...
    LeaveCrit();
    KeUserModeCallback(FI_CLIENTLOADLOCALT1FONTS, NULL, 0, &p, &cb);
    EnterCrit();
    return;
}
#endif // SENDSIDE
...
#ifdef RECVSIDE
DWORD __ClientLoadLocalT1Fonts(PVOID p)
{
    UNREFERENCED_PARAMETER(p);
    vLoadLocalT1Fonts();
    return NtCallbackReturn(NULL, 0, STATUS_SUCCESS);
...
VOID ClientLoadRemoteT1Fonts(VOID)
{
    ...
    LeaveCrit();
    KeUserModeCallback(FI_CLIENTLOADREMOTET1FONTS, NULL, 0, &p, &cb);
    EnterCrit();
    return;
}
#endif // SENDSIDE
...
#ifdef RECVSIDE
DWORD __ClientLoadRemoteT1Fonts(PVOID p)
{
    UNREFERENCED_PARAMETER(p);
    vLoadRemoteT1Fonts();
    return NtCallbackReturn(NULL, 0, STATUS_SUCCESS);
...
NTSTATUS pppUserModeCallback (ULONG uApi, PVOID pIn, ULONG cbIn, PVOID pOut, ULONG cbOut)
{
     ...
     */
    if (cbLocalOut != cbOut) {
        RIPMSG3(RIP_WARNING, "pppUserModeCallback: uAPi: %#lx cbOut: %#lx cbLocalOut: %#lx", uApi, cbOut, cbLocalOut);
...
VOID ClientNoMemoryPopup(VOID)
{
    ...
    LeaveCrit();
    KeUserModeCallback(FI_CLIENTNOMEMORYPOPUP, NULL, 0, &p, &cb);
    EnterCrit();
    return;
}
#endif // SENDSIDE
...
NTSTATUS ClientThreadSetup(VOID)
{
    ...
    LeaveCrit();
    Status = KeUserModeCallback(FI_CLIENTTHREADSETUP, NULL, 0, &p, &cb);
    EnterCrit();
    return Status;
}
#endif // SENDSIDE
...
#ifdef RECVSIDE
DWORD __ClientThreadSetup(PVOID p)
{
    BOOL fSuccess;
    ...
    return NtCallbackReturn(NULL, 0, fSuccess ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL);
}
#endif // RECVSIDE
...
*
* Callback to the client to handle a user APC.
* This is needed to ensure that a thread will exit promptly when terminated.
*
...
VOID ClientDeliverUserApc(VOID)
{
    ...
    LeaveCrit();
    KeUserModeCallback(FI_CLIENTDELIVERUSERAPC, NULL, 0, &p, &cb);
    EnterCrit();
}
#endif // SENDSIDE

#ifdef RECVSIDE
DWORD __ClientDeliverUserApc(PVOID p)
{
    UNREFERENCED_PARAMETER(p);
    return NtCallbackReturn(NULL, 0, STATUS_SUCCESS);
}
...
NTSTATUS xxxUserModeCallback (ULONG uApi, PVOID pIn, ULONG cbIn, PVOID pOut, ULONG cbOut)
{
     ...
     */
    if (cbLocalOut != cbOut) {
        RIPMSG3(RIP_WARNING, "xxxUserModeCallback: uAPi: %#lx cbOut: %#lx cbLocalOut: %#lx", uApi, cbOut, cbLocalOut);
        return STATUS_UNSUCCESSFUL;
    }
