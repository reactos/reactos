...
/*
* Search the window's property list for the specified property.
* pszKey could be a string or an atom.
* If it is a string, convert it to an atom before lookup.
* FindProp will only find internal or external properties depending on the fInternal flag.
...
PPROP _FindProp(PWND pwnd, PCWSTR pszKey, BOOL fInternal)
{
    ...
    /*
     * Now we've got the atom, search the list for a property with the same atom/name.
     * Make sure to only return internal properties if the fInternal flag is set.
     * Do the same for external properties.
     */
...
/*
* Search the window's property list for the specified property and return the hData handle from it.
...
HANDLE _GetProp(PWND pwnd, PCWSTR pszKey, BOOL fInternal)
{
    ...
    /*
     * A quick little optimization for that case where the window has no properties at all.
     */
