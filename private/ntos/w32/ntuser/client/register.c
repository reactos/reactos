...
/*
 * Broadcast-sends the given message to all top-level windows of szClass except to hwndSkip.
*/
...
/*
 * Broadcast-sends a UM_REGISTER or UM_UNREGISTER message to all DDEML16 and DDEML32 listening windows in the system except hwndListen.

 * We post Registration messages to prevent DdeConnectList recursion and send Unregistration messages to avoid invalid source window errors.
*/
VOID RegisterService(BOOL fRegister, GATOM gaApp, HWND hwndListen)
{
     ...
     */
    SendRegisterMessageToClass(gpsi->atomSysClass[ICLS_DDEMLMOTHER], fRegister ? UM_REGISTER : UM_UNREGISTER, gaApp, hwndListen, fRegister);
    /*
     * Send notification to each DDEML16 listening window.
     */
    SendRegisterMessageToClass(gpsi->atomSysClass[ICLS_DDEML16BIT], fRegister ? UM_REGISTER : UM_UNREGISTER, gaApp, hwndListen, fRegister);
}
...
LRESULT ProcessRegistrationMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (pcii != NULL && !((msg == UM_REGISTER) && (pcii->afCmd & CBF_SKIP_REGISTRATIONS)) && !((msg == UM_UNREGISTER) && (pcii->afCmd & CBF_SKIP_UNREGISTRATIONS))) {
        LATOM la, lais;
