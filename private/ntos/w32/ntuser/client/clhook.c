...
/*
* Client side routine for SetWindowsHookEx().
* Needs to remember the library name since hmods aren't global.
* Remembers the hmod as well so that it can be used to calculate pfnFilter in different process contexts.
...
HHOOK SetWindowsHookExAW(int idHook, HOOKPROC lpfn, HINSTANCE hmod, DWORD dwThreadID, DWORD dwFlags)
{
    ...
    /*
     * If we're passing an hmod, we need to grab the file name of the module while we're still on the client since module handles are NOT global.
     */
    if (hmod != NULL) {
...
* ANSI and Unicode wrappers for NtUserSetWindowsHookAW(). Could easily be macros instead, but do we want to expose NtUserSetWindowsHookAW() ?
...
* ANSI and Unicode wrappers for SetWindowsHookExAW().
* Could easily be macros instead, but do we want to expose SetWindowsHookExAW() ?
