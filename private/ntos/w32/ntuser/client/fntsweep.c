    ...
    )
{
    ...
    // get the time of the last sweep of the fonts section of the registry:
        ...
        else
        {
            ...
            Status = NtQueryValueKey(hkeyLastSweep, &UnicodeString, KeyValuePartialInformation, &SweepValueInfo, sizeof(SweepValueInfo), &dwReturnLength);
            if (!NT_SUCCESS(Status))
            {
...
VOID vCleanConvertedTTFs()
{
    ...
    else
    {
        ...
        Status = NtQueryValueKey(hkeyUpgradeType1, &UnicodeString, KeyValuePartialInformation, &UpgradeValueInfo, sizeof(UpgradeValueInfo), &dwReturnLength);
        if (NT_SUCCESS(Status))
        {
