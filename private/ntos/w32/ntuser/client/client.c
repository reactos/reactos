...
{
    return NtUserScrollWindowEx(hwnd, dx, dy, prcScroll, prcClip, NULL, NULL, !IS_PTR(prcScroll) ? SW_FLAG_RC : SW_FLAG_NRC) != ERROR;
}
...
*/
{
    ...
    if (!NT_SUCCESS(status)) {
        if (status == STATUS_OBJECT_TYPE_MISMATCH) {
             ...
             * CreateProcess ORs in a 0x2 (the low 2 bits of handles are not used) so we can identify it more clearly.
             */
             ...
             * CreateProcess ORs in a 0x1 (the low 2 bits of handles are not used) so we can identify and return immidiately.
             */
...
DWORD WINAPI MsgWaitForMultipleObjectsEx(DWORD nCount, CONST HANDLE *pHandles, DWORD dwMilliseconds, DWORD dwWakeMask, DWORD dwFlags)
{
    ...
    /*
     *  WowApps must exit the Wow scheduler otherwise other tasks in this Wow scheduler can't run. The only exception is if the timeout is Zero.
     *  We pass HEVENT_REMOVEME as the handle so we will go into the sleeptask AND return without going to sleep but letting other apps run.
     */
        ...
        PULONG pulReturnLength)
{
    ...
    if ((hwinsta = LoadLibraryA("WINSTA")) != NULL) {
        if ((pfnWinStationQueryInformationW = (PWINSTATIONQUERYINFORMATIONW) GetProcAddress(hwinsta, "WinStationQueryInformationW")) != NULL) {
                /*
                 * Fetch the WinStation's basic information
                 */
                fRet = (*pfnWinStationQueryInformationW)(hServer, ulLogonId, WinStationInformationClass, pWinStationInformation, ulWinStationInformationLength, pulReturnLength);
        }
        FreeLibrary(hwinsta);
    }
            ...
            if (NT_SUCCESS(Status)) {
                ...
                if (IS_IME_KBDLAYOUT(wLayoutId)) {
                    ...
                    if (NT_SUCCESS(NtOpenKey(&hSubKey, KEY_READ, &OA))) {
                        ...
                        Status = NtQueryValueKey(hSubKey, &UnicodeStringIME, KeyValuePartialInformation, &IMEfile, sizeof IMEfile, &cbSize);
                        NtClose(hSubKey);
                        if (NT_SUCCESS(Status)) {
    ...
    LPWSTR lpszName)
{
    ...
    rc = NtCreateKey( &hKeyPreload, STANDARD_RIGHTS_WRITE | KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS | KEY_SET_VALUE | KEY_CREATE_SUB_KEY, &ObjA, 0, NULL, 0, &CreateDisposition );
    if (!NT_SUCCESS( rc ))
    {
    ...
    PUINT pKbdInputLocale)
{
    ...
    if (NT_SUCCESS(NtOpenKey(&hKey, KEY_READ, &OA))) {
        ...
        Status = NtQueryValueKey(hKey, &UnicodeString, KeyValuePartialInformation, &KeyAttributes, sizeof(KeyAttributes), &cbSize);
        if (NT_SUCCESS(Status)) {
#if DBG
            if ((*((PDWORD)KeyAttributes.KeyInfo.Data) & ~KLF_ATTRMASK) != 0) {
                RIPMSG1(RIP_WARNING, "OpenKeyboardLayoutFile - Unexpected attributes %lx", *((PDWORD)KeyAttributes.KeyInfo.Data));
            }
#endif
         ...
         */
        pwszLib = NULL;
        RtlInitUnicodeString(&UnicodeString, L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\Keyboard Layout");
        InitializeObjectAttributes(&OA, &UnicodeString, OBJ_CASE_INSENSITIVE, NULL, NULL);
        if (NT_SUCCESS(NtOpenKey(&hKey, KEY_READ, &OA))) {
