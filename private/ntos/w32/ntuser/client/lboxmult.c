...
*/
{
    ...
    // Since listboxes are always inflated by CXBORDER and CYBORDER, a listbox that was created empty always ends up 2 x 2.
    // Since this isn't big enough to fit the entire client border, we don't mark it as present.
    // Thus the client isn't empty in VER40, although it was in VER31 and before.
    // It is possible to get to this spot without plb->cyChar having been initialized yet if the listbox  is multicolumn && ownerdraw variable.
