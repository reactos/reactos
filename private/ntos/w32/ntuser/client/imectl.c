...
*/
{
    ...
    /*
     * Get the pimeui for the given window now since we will use it a lot in various handlers.
     * This was stored using SetWindowLong(hwnd,0,pimeui) when we initially created the IME control.
     */
...
*/
{
    ...
    if (cchLen > 4 && _wcsicmp(wszHelpFile + cchLen - 4, wszHelpFileExt) == 0) {
        ...
        // If it's not HLP file, try to run hh.exe, HTML based help tool.
        // It should be in %windir%\hh.exe.
        ...
        idProcess = (DWORD)CreateProcessW(NULL, wszCmdLine, NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS, NULL, NULL, &StartupInfo, &ProcessInformation);
        if (idProcess) {
...
LONG ImeSelectHandler(PIMEUI pimeui,UINT   message,WPARAM wParam,LPARAM lParam)
{
    ...
    if ((BOOL)wParam == TRUE) {
        ...
        if (GetIMEShowStatus() && pimeui->fCtrlShowStatus) {
            if (!pimeui->fShowStatus && pimeui->fActivate && IsWindow(pimeui->hwndIMC)) {
                /*
        ...
        if (pimeui->fShowStatus && pimeui->fActivate && IsWindow(pimeui->hwndIMC)) {
            /*
             * This must be sent to an application as an app may want to hook this message to do its own UI.
             */
...
LRESULT ImeSetContextHandler(PIMEUI pimeui, UINT   message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (wParam) {
        ...
        /*
         * When we're receiving context, it is necessary to set the owner to this window.
         * This is for:
    ...
    if (pimeui->fCtrlShowStatus && GetIMEShowStatus()) {
        ...
        if ((BOOL)wParam == TRUE) {
            ...
            if (pwndFocus != NULL && GETPTI(pimeui->spwnd) == GETPTI(pwndFocus)) {
                ...
                else if ((pwndIMC = ValidateHwndNoRip(pimeui->hwndIMC)) != NULL && (pwndPrevIMC = ValidateHwndNoRip(hwndPrevIMC)) != NULL && GetTopLevelWindow(pwndIMC) != GetTopLevelWindow(pwndPrevIMC)) {
                    /*
...
LRESULT ImeNotifyHandler(PIMEUI pimeui, UINT   message, WPARAM wParam, LPARAM lParam)
{
    ...
    case IMN_SETOPENSTATUS:
        ...
        // if this message is sent from ImmSetOpenStatus or ImmSetConversionStatus, we have already notified kernel the change. This is a little bit redundant.
        ...
        if ((pInputContext = fpImmLockIMC(hImc)) != NULL) {
            if ( IsWindow(pimeui->hwndIMC) ) {
                NtUserNotifyIMEStatus( pimeui->hwndIMC, (DWORD)pInputContext->fOpen, pInputContext->fdwConversion );
            }
                ...
                if (pimeui->spwnd) {    // If IME window is still there.
                    ...
                    if (pwndOwner != NULL) {
                        NtUserNotifyIMEStatus(HWq(pwndOwner), (DWORD)pInputContext->fOpen, pInputContext->fdwConversion);
                    }
                }
            }
            fpImmUnlockIMC(hImc);
...
HWND CreateIMEUI(PIMEUI pimeui, HKL    hKL)
{
    ...
    if (iiex.ImeInfo.fdwProperty & IME_PROP_UNICODE) {
         ...
         */
        hwndUI = CreateWindowExW(0L, iiex.wszUIClass, iiex.wszUIClass, WS_POPUP|WS_DISABLED, 0, 0, 0, 0, HWq(pwndIme), 0, wndcls.hInstance, NULL);
    }
    else {
        /*
        ...
        hwndUI = CreateWindowExA(0L, pszClass, pszClass, WS_POPUP|WS_DISABLED, 0, 0, 0, 0, HWq(pwndIme), 0, wndcls.hInstance, NULL);
...
*/
{
    ...
    if (TestWF(pimeui->spwnd, WFINDESTROY) || TestWF(pimeui->spwnd, WFDESTROYED) || TestWF(pwndUI, WFINDESTROY) || TestWF(pwndUI, WFDESTROYED)) {
        return 0L;
    }
...
/*
*  Some IME windows can not share the same input context.
*  This function checks whether the specified hImc can be used (means 'Set activated') by the specified IME window.
