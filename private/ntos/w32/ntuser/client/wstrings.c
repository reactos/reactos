...
/*
* Convert either a single character or an entire string to upper case.
* The two cases are differentiated by checking the high-word of pwsz.
* If it is 0 then we just convert the low-word of pwsz.

* History:
* 06-24-91 GregoryW     Created.  Supports Unicode equivalent of code page 1252 (simple zero extension).
*                       This is for the PDK release only.
*                       After the PDK this routine will be modified to use the NLSAPI.
* 02-11-93 IanJa        Modified to use NLS API.
*/
LPWSTR WINAPI CharUpperW(LPWSTR pwsz)
{
    ...
    if (!IS_PTR(pwsz)) {
        if (!LCMapStringW(LOCALE_USER_DEFAULT, LCMAP_UPPERCASE, (LPWSTR)&pwsz, 1, (LPWSTR)&pwsz, 1)) {
            /*
             * We don't expect LCMapString to fail!
             * The caller is not expecting failure, CharLowerW does not have a failure indicator, so we do nothing.
             */
...
/*
* History:
* 06-24-91 GregoryW     Created.
*                       This version only supports Unicode block 1 (0x0000 - 0x00ff).
*                       All other code points are copied verbatim.
...
DWORD WINAPI CharUpperBuffW(LPWSTR pwsz, DWORD cwch)
{
    ...
    /*
     * LCMapString failed!  The caller is not expecting failure,
     * CharUpperBuffW does not have a failure indicator, so we convert the buffer to upper case as best we can.
     */
    RIPMSG1(RIP_WARNING, "CharUpperBuffW(%ls) failed", pwsz);

    for (i=0; i < cwch; i++) {
        if (IS_UNICODE_BLK1(pwsz[i]) && IsCharLowerA((char)pwsz[i]) && (pwsz[i] != LATIN_SMALL_LETTER_SHARP_S) && (pwsz[i] != LATIN_SMALL_LETTER_Y_DIAERESIS)) {
            pwsz[i] += (WCHAR)('A'-'a');
        }
    }
...
BOOL WINAPI IsCharUpperW(WCHAR wChar)
{
    ...
    /*
     * GetStringTypeW failed!  The caller is not expecting
     * failure, IsCharLowerW does not have a failure indicator, so we determine the case as best we can.
     */
