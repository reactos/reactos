...
PWND xxxSetCapture(PWND pwnd)
{
    ...
    /*
     * Don't allow the app to set capture to a window from another queue.
     */
...
VOID xxxCapture(PTHREADINFO pti, PWND pwnd, UINT code)
{
    ...
    if ((gspwndScreenCapture == NULL) || (code == FULLSCREEN_CAPTURE) || ((pwnd == NULL) && (code == NO_CAP_CLIENT) && (pti->pq != GETPTI(gspwndScreenCapture)->pq))) {
        ...
        /*
         * Free the internal capture if the app (thread) that did the internal capture is freeing the capture.
         */
        if ((code == NO_CAP_CLIENT) && gspwndInternalCapture && (pti == GETPTI(gspwndInternalCapture))) {
        ...
        if ((pq = pti->pq) != NULL) {
             ...
             */
            if ((pdesk->dwDTFlags & DF_TRACKMOUSEEVENT) && (pdesk->htEx == HTCLIENT) && ((pdesk->spwndTrack == pwnd) && (code != CLIENT_CAPTURE) || ((pdesk->spwndTrack == pq->spwndCapture) && (pq->codeCapture != CLIENT_CAPTURE)))) {
                BEGINATOMICCHECK();
                xxxCancelMouseMoveTracking(pdesk->dwDTFlags, pdesk->spwndTrack, pdesk->htEx, DF_TRACKMOUSEEVENT);
                ENDATOMICCHECK();
            }
        ...
        if (FWINABLE()) {
            if (pwndCaptureOld) {
                zzzWindowEvent(EVENT_SYSTEM_CAPTUREEND, pwndCaptureOld, OBJID_WINDOW, INDEXID_CONTAINER, WEF_USEPWNDTHREAD);
            }

            if (pwnd) {
                zzzWindowEvent(EVENT_SYSTEM_CAPTURESTART, pwnd, OBJID_WINDOW, INDEXID_CONTAINER, WEF_USEPWNDTHREAD);
            }
        }
         ...
         * The FNID_DELETED_BIT is set in xxxFreeWindow which means we DON'T want to send the message.
         */
        if (pwndCaptureOld && TestWF(pwndCaptureOld, WFWIN40COMPAT) && !(pwndCaptureOld->fnid & FNID_DELETED_BIT)) {
            TL tlpwnd;

            /*
             * If we are in menu mode and just set capture, don't let them take it from us during this callback.
             */
            ...
            xxxSendMessageCallback(pwndCaptureOld, WM_CAPTURECHANGED, FALSE, (LPARAM)HW(pwnd), NULL, 0, FALSE);
            /* The thread's queue may have changed during the callback, so we need to refresh the local. Bug #377795
             */
            pq = pti->pq;
            UserAssert(pq != NULL);
            ThreadUnlock(&tlpwnd);
