...
*/
{
    ...
    do {
         ...
         */
        if (!fCreator && (IsShellProcess(ppiParent) || ((ppiParent->rpwinsta != NULL) && (ppiParent->rpwinsta->dwWSF_Flags & WSF_NOIO)))) {
            UnlockProcess(pepParent);
            break;
        }
        ...
        if (!fAllowForeground) {
             ...
             */
            if (ppiParent->ptiList != NULL && (ppiParent->ptiList->TIF_flags & TIF_ALLOWFOREGROUNDACTIVATE)) {
                    fAllowForeground = TRUE;
            }
...
*/
{
    ...
    if (retval) {
        if (pcpi->dwFlags & CPI_NEWPROCESSWINDOW) {
            ...
            TAGMSG3(DBGTAG_FOREGROUND, "xxxUserNotifyConsoleApplication %s W32PF %#p-%#p", ((ppiCurrent->W32PF_Flags & W32PF_ALLOWFOREGROUNDACTIVATE) ? "set" : "NOT"), ppiCurrent, PpiFromProcess(Process));
        }
    } else {
...
*/
{
    ...
    if ((dwFlags & 0xb) != 0) {
         ...
         */
        if (pwti == NULL) {
            pwti = (PWOWTHREADINFO)UserAllocPoolWithQuota(sizeof(WOWTHREADINFO), TAG_WOWTHREADINFO);
            if (pwti == NULL) {
                return FALSE;
            }
...
void zzzCalcStartCursorHide(PW32PROCESS pwp, DWORD timeAdd)
{
    ...
    if (pwp != NULL) {
        /*
         * We were passed in a timeout. Recalculate when we timeout and add the pwp to the starting list.
         */
    ...
    for (ppwpT = &gpwpCalcFirst; (pwpT = *ppwpT) != NULL; ) {
         ...
         */
        if (!(pwpT->W32PF_Flags & W32PF_STARTGLASS) || (pwpT->W32PF_Flags & W32PF_FORCEOFFFEEDBACK)) {
...
BOOL SetAppImeCompatFlags(PTHREADINFO pti, PUNICODE_STRING pstrModName, PUNICODE_STRING pstrBaseFileName)
{
    ...
    if (FastGetProfileStringW(NULL, PMAP_IMECOMPAT, pstrModName->Buffer, NULL, szHex, sizeof(szHex))) {
        /*
     ...
     * to get this flags when the keyboard layout is switched. then we get it here, even this flags are not nessesary for non-IME keyboard layouts.
     */
    ZwQueryDefaultLocale(FALSE, &lcid);
    wPrimaryLangID = PRIMARYLANGID(lcid);

    if ((wPrimaryLangID == LANG_KOREAN || wPrimaryLangID == LANG_JAPANESE) && (LOWORD(pti->dwExpWinVer) <= VER31)) {
        /*
...
*/
{
    ...
    try {
        ...
        pchStart = pchEnd = pstrAppName->Buffer + (pstrAppName->Length / sizeof(WCHAR));
...
#undef MAXMODULENAMELEN
    ...
    if (FastGetProfileStringW(NULL, PMAP_COMPAT32, szKey, NULL, szHex, sizeof(szHex))) {
        UNICODE_STRING strHex;
    ...
    cbSize = FastGetProfileValue(NULL, PMAP_COMPAT2, szKey, NULL, (LPBYTE)&Compat2Key, sizeof(Compat2Key));
    /*
...
*/
{
     ...
     */
    if (gptmrMaster != NULL) {
        guAppStartingId = InternalSetTimer(NULL, guAppStartingId, CMSAPPSTARTINGTIMEOUT + CMSHUNGAPPTIMEOUT, CheckAppStarting, TMRF_RIT | TMRF_ONESHOT);
    }
}
...
*/
{
    ...
    ptiCurrent->pstrAppName = UserAllocPoolWithQuota(sizeof(UNICODE_STRING) + pstrModName->Length + sizeof(WCHAR), TAG_TEXT);
    if (ptiCurrent->pstrAppName != NULL) {
        ptiCurrent->pstrAppName->Buffer = (PWCHAR)(ptiCurrent->pstrAppName + 1);
        try {
            RtlCopyMemory(ptiCurrent->pstrAppName->Buffer, pstrModName->Buffer, pstrModName->Length);
    ...
    /*
     * If this task is running in the shared WOW VDM, we handle WaitForInputIdle a little differently than separate WOW VDMs.
     * This is because CreateProcess returns a real process handle when you start a separate WOW VDM, so the "normal" WaitForInputIdle works.
     * For the shared WOW VDM, CreateProcess returns an event handle.
     */
    ptdb->pwti = NULL;
    if (idTask) {
...
#define NORMAL_PRIORITY_TASK 10
    ...
    try {
        if (SetAppImeCompatFlags(ptiCurrent, ptiCurrent->pstrAppName, pstrBaseFileName)) {
            /*
...
*/
{
     ...
     */
    UserAssert((phook == NULL) ^ IsHooked(pti, (WHF_FROM_WH(WH_JOURNALPLAYBACK) | WHF_FROM_WH(WH_JOURNALRECORD))));
...
NTSTATUS xxxCreateThreadInfo(PETHREAD pEThread, BOOL     IsSystemThread)
{
    ...
    if (dwTIFlags & TIF_SYSTEMTHREAD) {
        ptiCurrent->pClientInfo = UserAllocPoolWithQuota(sizeof(CLIENTINFO), TAG_CLIENTTHREADINFO);
     ...
     */
    Status = ZwCreateEvent(&ptiCurrent->hEventQueueClient, EVENT_ALL_ACCESS, NULL, SynchronizationEvent, FALSE);
    if (NT_SUCCESS(Status)) {
        Status = ObReferenceObjectByHandle(ptiCurrent->hEventQueueClient, EVENT_ALL_ACCESS, *ExEventObjectType, UserMode, &ptiCurrent->pEventQueueServer, NULL);
        if (NT_SUCCESS(Status)) {
    ...
    if (ProcessParams) {
        ...
        if (fFirstThread) {
            ...
            if (ProcessParams->WindowFlags & STARTF_USEHOTKEY) {
                ...
                ppi->dwHotkey = ParseReserved(ProcessParams->ShellInfo.Buffer, L"hotkey.");
            }
    ...
    if (!(ptiCurrent->TIF_flags & (TIF_SYSTEMTHREAD | TIF_CSRSSTHREAD)) && grpWinStaList) {
        BOOL bShutDown = FALSE;

        hdesk = xxxResolveDesktop(NtCurrentProcess(), &ProcessParams->DesktopInfo, &hwinsta, (ProcessParams->WindowFlags & STARTF_DESKTOPINHERIT), &bShutDown);
        if (hdesk == NULL) {
            if (bShutDown) {
                /*
                ...
                ExRaiseHardError((NTSTATUS)STATUS_DLL_INIT_FAILED_LOGOFF, ARRAY_SIZE(adwParameters), 0, adwParameters, OptionOkNoWait, &ErrorResponse);
             ...
             */
            Status = ObReferenceObjectByHandle(hdesk, 0, *ExDesktopObjectType, KernelMode, &pdesk, NULL);
            if (!NT_SUCCESS(Status)) {
             ...
             */
            if ((ppi->hdeskStartup == NULL) && (pEProcess->UniqueProcessId != gpidLogon)) {
    ...
    /*
     * Remember dwExpWinVer. This is used to return GetAppVer() (and GetExpWinVer(NULL).
     */
     ...
     * away.  At first we set it to normal priority, then we set the TIF_IDLESCREENSAVER bit so that when it activates it will get lowered in priority.
     */
...
NotAScreenSaver:
    ...
    if (!(ptiCurrent->TIF_flags & (TIF_SYSTEMTHREAD | TIF_CSRSSTHREAD))) {
         ...
         * have been loaded.
         */
        if ((SYSCUR(ARROW) != NULL) && !(ppi->W32PF_Flags & W32PF_CLASSESREGISTERED)) {
            ppi->W32PF_Flags |= W32PF_CLASSESREGISTERED;
     ...
     */
     if (!(ppi->W32PF_Flags & (W32PF_ALLOWFOREGROUNDACTIVATE | W32PF_APPSTARTING))) {
         if (((gptiForeground != NULL) && (ppi == gptiForeground->ppi)) || ((glinp.ptiLastWoken != NULL) && (ppi == glinp.ptiLastWoken->ppi))) {
    ...
    if ((NT_SUCCESS(Status) && fFirstThread) && !(ppi->W32PF_Flags & W32PF_CONSOLEAPPLICATION)) {
        /*
...
CreateThreadInfoFailed:
    RIPMSG2(RIP_WARNING, "xxxCreateThreadInfo: failed: pti %#p pdesk %#p", ptiCurrent, pdesk);
    if (pdesk != NULL) {
    ...
    if (pq->cThreads != 0) {
        ...
        if ((pq->ptiKeyboard == pti) || (pq->ptiMouse == pti)) {
            ...
            if (pq->cThreads != cDying && (pq->ptiKeyboard == NULL || pq->ptiMouse == NULL)) {
                RIPMSG6(RIP_ERROR, "pq %#p pq->cThreads %x cDying %x pti %#p ptiK %#p ptiM %#p", pq, pq->cThreads, cDying, pti, pq->ptiKeyboard, pq->ptiMouse);
            }
#endif
        }
...
{
    return (pti->ppi && pti->ppi->ptiList == pti && pti->ptiSibling == NULL);
}
...
VOID xxxDestroyThreadInfo(VOID)
{
    ...
    while (ptiCurrent->pMenuState != NULL) {
        ...
        if (ptiCurrent == pMenuState->ptiMenuStateOwner) {
             ...
             */
            UserAssert((ppopupmenuRoot->spwndNotify != NULL) && (GETPTI(ppopupmenuRoot->spwndNotify) == ptiCurrent));
...
NTSTATUS InitQEntryLookaside()
{
    ...
    ExInitializePagedLookasideList(QEntryLookaside, NULL, NULL, SESSION_POOL_MASK, sizeof(QMSG), TAG_QMSG, 16);
    ...
    ExInitializePagedLookasideList(QLookaside, NULL, NULL, SESSION_POOL_MASK, sizeof(Q), TAG_Q, 16);
    return STATUS_SUCCESS;
}
...
PQMSG AllocQEntry(PMLIST pml)
{
    ...
    if (pml->cMsgs >= gUserPostMessageLimit) {
        RIPMSG3(RIP_WARNING, "AllocQEntry: # of post messages exceeds the limit(%d) in pti=0x%p, pml=0x%p", gUserPostMessageLimit, W32GetCurrentThread(), pml);
        return NULL;
    }
    ...
    while (pqmsgRead != NULL) {
        ...
        if (pwnd == NULL && pqmsgRead->msg.hwnd != NULL) {
            ...
            if ((pti->pq->idSysLock == (ULONG_PTR)pqmsgRead)
                ...
                RIPMSG2(RIP_VERBOSE, "FindQMsg: Unlocking queue:%#p. Msg:%#lx", pti->pq, pqmsgRead->msg.message);
         ...
         */
        if (bProcessAck && (PtoH(pwndFilter) == pqmsgRead->msg.hwnd) && (pqmsgRead->msg.message == (WM_DDE_ACK | MSGFLAG_DDE_MID_THUNK))) {
            PXSTATE pxs;
        ...
        if (CheckMsgFilter(message, (WM_DDE_FIRST + 1) | MSGFLAG_DDE_MID_THUNK, WM_DDE_LAST | MSGFLAG_DDE_MID_THUNK)) {
...
// We disable all MS badapp code and do it our way
         ...
         */
        if (CheckMsgFilter(lpMsg->message, (WM_DDE_FIRST + 1) | MSGFLAG_DDE_MID_THUNK, WM_DDE_LAST | MSGFLAG_DDE_MID_THUNK)) {
            /*
...
void xxxProcessHungThreadEvent(PWND pwnd)
{
    ...
    if (pwndGhost != NULL) {
        ...
        if (TestWF(pwndGhost, WFMAXIMIZED)) {
             ...
             * hung window.
             */
            xxxSetWindowPos(pwnd, pwndGhost, pwndGhost->rcWindow.left, pwndGhost->rcWindow.top, pwndGhost->rcWindow.right - pwndGhost->rcWindow.left, pwndGhost->rcWindow.bottom - pwndGhost->rcWindow.top, dwFlags);
        }
         ...
         */
        if ((pcpGhost = (PCHECKPOINT)_GetProp(pwndGhost, PROP_CHECKPOINT, PROPF_INTERNAL)) != NULL) {
            if ((pcp = (PCHECKPOINT)_GetProp(pwnd, PROP_CHECKPOINT, PROPF_INTERNAL)) == NULL) {
     ...
     */
    xxxRedrawWindow(pwnd, NULL, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_ALLCHILDREN | RDW_FRAME);
...
VOID xxxProcessEventMessage(PTHREADINFO ptiCurrent, PQMSG pqmsg)
{
    ...
    case QEVENT_ACTIVATE:
    {
        if (pqmsg->msg.lParam == 0) {
             ...
             * queue.
             */
            if ((pqmsg->msg.wParam != 0) && (pq->spwndActive != NULL) && (pq == gpqForeground)) {
                PWND pwndActive;
            ...
            if (gpqForeground == NULL) {
                ...
                if (pwnd != pq->spwndActive) {
                    if (xxxActivateThisWindow(pwnd, (UINT)pqmsg->msg.wParam, (ATW_SETFOCUS | ATW_ASYNC) | ((pqmsg->msg.message & PEM_ACTIVATE_NOZORDER) ? ATW_NOZORDER : 0))) {
                        /*
                    ...
                    xxxSendMessage(pwnd, WM_NCACTIVATE, (DWORD)(fActive), 0);
                    if (fActive) {
                        xxxUpdateTray(pwnd);
                    }
                    ...
                    if (fActive && !(pqmsg->msg.message & PEM_ACTIVATE_NOZORDER))
                        xxxSetWindowPos(pwnd, PWND_TOP, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
                }
            }
    ...
    case QEVENT_CANCELMOUSEMOVETRK: {
        ...
        if (pwnd != NULL) {
            ThreadLockAlwaysWithPti(ptiCurrent, pwnd, &tlpwndT);
            xxxCancelMouseMoveTracking(pqmsg->msg.message, pwnd, (int)pqmsg->msg.wParam, (DWORD)pqmsg->msg.lParam);
            ThreadUnlock(&tlpwndT);
        } else if ((pqmsg->msg.lParam & DF_TOOLTIP) && (pqmsg->msg.message & DF_TOOLTIPSHOWING)) {
            /*
    ...
    case QEVENT_RITACCESSIBILITY:
        if (IsHooked(ptiCurrent, WHF_SHELL)) {
            xxxCallHook((UINT)pqmsg->msg.wParam, (WPARAM)pqmsg->msg.lParam, (LPARAM)0, WH_SHELL);
        }
    ...
    case QEVENT_APPCOMMAND: {
        ...
        if (ptiCurrent != ptiWindowOwner) {
             ...
             */
            PostEventMessage(ptiWindowOwner, ptiWindowOwner->pq, QEVENT_APPCOMMAND, NULL, 0, (WPARAM)0, pqmsg->msg.lParam);
...
DWORD _GetQueueStatus(UINT flags)
{
    ...
    /*
     * Clear out the change bits the app is looking at so it'll know what changed since it's last call to GetQueueStatus().
     */
     ...
     */
    return MAKELONG(fsChangeBits & flags, (ptiCurrent->pcti->fsWakeBits | ptiCurrent->pcti->fsWakeBitsJournal) & flags);
}
...
DWORD xxxMsgWaitForMultipleObjects(DWORD nCount, PVOID *apObjects, MSGWAITCALLBACK pfnNonMsg, PKWAIT_BLOCK WaitBlockArray)
{
    ...
    if (!(ptiCurrent->pcti->fsChangeBits & QS_ALLINPUT)) {
        ...
        if (ptiCurrent == gptiForeground && IsHooked(ptiCurrent, WHF_FOREGROUNDIDLE)) {
...
Again:
        LeaveCrit();
        Status = KeWaitForMultipleObjects(nCount + 1, apObjects, WaitAny, WrUserRequest, UserMode, FALSE, NULL, WaitBlockArray);
        EnterCrit();
        CheckForClientDeath();
        UserAssert(NT_SUCCESS(Status));
...
BOOL xxxSleepThread(UINT fsWakeMask, DWORD Timeout, BOOL fInputIdle)
{
    ...
    while (TRUE) {
        ...
        if (!(ptiCurrent->pcti->fsChangeBits & ptiCurrent->pcti->fsWakeMask)) {
            ...
            if (!(ptiCurrent->TIF_flags & TIF_16BIT))  {
                if (fInputIdle && ptiCurrent == gptiForeground && IsHooked(ptiCurrent, WHF_FOREGROUNDIDLE)) {
                    xxxCallHook(HC_ACTION, 0, 0, WH_FOREGROUNDIDLE);
                }
                ...
                LeaveCrit();
                status = KeWaitForSingleObject(ptiCurrent->pEventQueueServer, WrUserRequest, UserMode, FALSE, pli);
                CheckForClientDeath();
                EnterCrit();
...
VOID SetWakeBit(PTHREADINFO pti, UINT wWakeBit)
{
     ...
     * go away.
     */
    if ((wWakeBit & QS_INPUT) && (pti->ppi->W32PF_Flags & W32PF_IDLESCREENSAVER)) {
        if ((wWakeBit & QS_MOUSEMOVE) && (gpsi->ptCursor.x == gptSSCursor.x) && (gpsi->ptCursor.y == gptSSCursor.y)) {
...
NTSTATUS xxxInitProcessInfo(PW32PROCESS pwp)
{
    ...
    TAGMSG2(DBGTAG_FOREGROUND, "xxxInitProcessInfo %s W32PF %#p", ((ppi->W32PF_Flags & W32PF_ALLOWFOREGROUNDACTIVATE) ? "set" : "NOT"), ppi);
...
BOOL DestroyProcessInfo(PW32PROCESS pwp)
{
    ...
    if ((ppi->W32PF_Flags & W32PF_IOWINSTA) && !(ppi->W32PF_Flags & W32PF_CONSOLEAPPLICATION) && (gspwndLogonNotify != NULL) && !(ppi->rpwinsta->dwWSF_Flags & WSF_OPENLOCK)) {
        PTHREADINFO pti = GETPTI(gspwndLogonNotify);
        PQMSG pqmsg;

        if ((pqmsg = AllocQEntry(&pti->mlPost)) != NULL) {
            StoreQMessage(pqmsg, gspwndLogonNotify, WM_LOGONNOTIFY, LOGON_PLAYEVENTSOUND, USER_SOUND_CLOSE, 0, 0, 0);
    ...
    if (IsShellProcess(ppi)) {
         ...
         */
        if ((gspwndLogonNotify != NULL) && !(ppi->rpwinsta->dwWSF_Flags & WSF_OPENLOCK)) {
            PTHREADINFO pti = GETPTI(gspwndLogonNotify);
            PQMSG pqmsg;

            if ((pqmsg = AllocQEntry(&pti->mlPost)) != NULL) {
                StoreQMessage(pqmsg, gspwndLogonNotify, WM_LOGONNOTIFY, LOGON_RESTARTSHELL, ppi->Process->ExitStatus, 0, 0, 0);
                SetWakeBit(pti, QS_POSTMESSAGE | QS_ALLPOSTMESSAGE);
            }
        }
    }
    ...
    if (fHadThreads) {
...
#if DBG
        {
            ...
            for (pheT = gSharedInfo.aheList; pheT <= pheMax; pheT++) {
                 ...
                 */
                UserAssertMsg0(pheT->bFlags & HANDLEF_DESTROY || !(gahti[pheT->bType].bObjectCreateFlags & OCF_PROCESSOWNED) || (PPROCESSINFO)pheT->pOwner != ppi,
                        "We should have no process objects left for this process!");
            }
        }
#endif
...
DWORD xxxWaitForInputIdle(ULONG_PTR idProcess, DWORD dwMilliseconds, BOOL fSharedWow)
{
    ...
    /*
     * If fSharedWow is set, the client passed in a fake process handle which CreateProcess returns for Win16 apps started in the shared WOW VDM.

     * CreateProcess returns a real process handle when you start a Win16 app in a separate WOW VDM.
     */
    ...
    if (fSharedWow) {  // Waiting for a WOW task to go idle.
        ...
        if (dwResult == STATUS_ABANDONED) {
            dwResult = xxxPollAndWaitForSingleObject(pwti->pIdleEvent, NULL, dwMilliseconds);
        }
        return dwResult;
    }
     ...
     */
    if (ptiCurrent->pEThread->Cid.UniqueProcess == (HANDLE)idProcess && ptiCurrent == ptiCurrent->ppi->ptiMainThread) {
    ...
    if (dwResult == STATUS_ABANDONED) {
        dwResult = xxxPollAndWaitForSingleObject(W32Process->InputIdleEvent, Process, dwMilliseconds);
    }
...
DWORD xxxPollAndWaitForSingleObject(PKEVENT pEvent, PVOID pExecObject, DWORD dwMilliseconds)
{
    ...
    while (TRUE) {
        ...
        Status = KeWaitForMultipleObjects(cEvent,
                                          ...
                                          UserMode,
                                          FALSE,
                                          (dwIntermediateMilliseconds == INFINITE ? NULL : &li),
                                          NULL);
        EnterCrit();
...
DWORD WaitOnPseudoEvent(HANDLE *phE, DWORD dwMilliseconds)
{
    ...
    if (*phE == PSEUDO_EVENT_OFF) {
        if (!NT_SUCCESS(ZwCreateEvent(&hEvent, EVENT_ALL_ACCESS, NULL, NotificationEvent, FALSE))) {
        ...
        Status = ObReferenceObjectByHandle(hEvent, EVENT_ALL_ACCESS, *ExEventObjectType, KernelMode, phE, NULL);
        ZwClose(hEvent);
        if (!NT_SUCCESS(Status))
            return (DWORD)-1;
...
NTSTATUS xxxSetCsrssThreadDesktop(PDESKTOP pdesk, PDESKRESTOREDATA pdrdRestore)
{
    ...
    if (pdesk->dwDTFlags & DF_DESTROYED) {
        RIPMSG1(RIP_WARNING, "xxxSetCsrssThreadDesktop: pdesk 0x%x destroyed", pdesk);
    ...
    if (pdrdRestore->pdeskRestore != NULL) {
        Status = ObReferenceObjectByPointer(pdrdRestore->pdeskRestore, MAXIMUM_ALLOWED, *ExDesktopObjectType, KernelMode);
        if (!NT_SUCCESS(Status)) {
    ...
    Status = ObOpenObjectByPointer(pdesk, 0, NULL, EVENT_ALL_ACCESS, NULL, KernelMode, &(pdrdRestore->hdeskNew));
    if (!NT_SUCCESS(Status)) {
...
ULONG GetTaskName(PTHREADINFO pti, PWSTR Buffer, ULONG BufferLength)
{
    ...
    if (pti->pstrAppName != NULL) {
        ...
        if (BufferLength < sizeof(WCHAR))
            ...
            Status = RtlAnsiStringToUnicodeString(&strAppNameU, &strAppName, FALSE);
            if (NT_SUCCESS(Status))
...
NTSTATUS xxxQueryInformationThread(IN HANDLE hThread, IN USERTHREADINFOCLASS ThreadInfoClass, OUT PVOID ThreadInformation, IN ULONG ThreadInformationLength, OUT PULONG ReturnLength OPTIONAL)
{
    ...
    Status = ObReferenceObjectByHandle(hThread, THREAD_QUERY_INFORMATION, *PsThreadType, UserMode, &Thread, NULL);
    if (!NT_SUCCESS(Status))
        return Status;
    ...
    case UserThreadShutdownInformation:
        ...
        UserAssert(FIELD_OFFSET(USERTHREAD_SHUTDOWN_INFORMATION, drdRestore) == (sizeof(USERTHREAD_SHUTDOWN_INFORMATION) - sizeof(DESKRESTOREDATA)));
        RtlZeroMemory(pShutdown, sizeof(USERTHREAD_SHUTDOWN_INFORMATION) - sizeof(DESKRESTOREDATA));

        /*
         * Return the desktop window handle if the thread
         * has a desktop and the desktop is on a visible windowstation.
         */
        if (pti != NULL && pti->rpdesk != NULL && !(pti->rpdesk->rpwinstaParent->dwWSF_Flags & WSF_NOIO))
         ...
         * app should be shut down, switch to the desktop and assign it to the shutdown worker thread.
         */
        if ((pShutdown->dwFlags & USER_THREAD_GUI) && pShutdown->StatusShutdown == 0) {
            /*
            ...
            if (!(dwClientFlags & WMCS_NORETRY)) {
                if (NT_SUCCESS(Status)) {
                        ...
                        UserAssert(pti == PtiFromThread(Thread));
                        if (!fSwitch) {
                            if ((pti->rpdesk->rpwinstaParent == NULL) || !(pti->rpdesk->rpwinstaParent->dwWSF_Flags & WSF_NOIO)) {
    ...
    case UserThreadHungStatus:
         ...
         */
        if (pti)
            *(PDWORD)ThreadInformation = (DWORD) FHungApp(pti, (DWORD)*(PDWORD)ThreadInformation);
        else
...
NTSTATUS xxxSetInformationThread(IN HANDLE hThread, IN USERTHREADINFOCLASS ThreadInfoClass, IN PVOID ThreadInformation, IN ULONG ThreadInformationLength)
{
    ...
    Status = ObReferenceObjectByHandle(hThread, THREAD_SET_INFORMATION, *PsThreadType, UserMode, &Thread, NULL);
    if (!NT_SUCCESS(Status))
        return Status;
    ...
    case UserThreadUseDesktop:
        ...
        if (hClientThread != NULL) {
            Status = ObReferenceObjectByHandle(hClientThread, THREAD_QUERY_INFORMATION, *PsThreadType, UserMode, &ThreadClient, NULL);
            if (!NT_SUCCESS(Status))
                break;
...
DerefClientThread:
    ...
    case UserThreadUseActiveDesktop:
    {
        ...
        Status = xxxSetCsrssThreadDesktop(grpdeskRitInput, &((PUSERTHREAD_USEDESKTOPINFO)ThreadInformation)->drdRestore);
        break;
    }
    case UserThreadCsrApiPort:
        ...
        if (!NT_SUCCESS(Status)) {
            CsrApiPort = NULL;
            RIPMSG1(RIP_WARNING, "CSR port reference failed, Status=%#lx", Status);
        }
...
*/
BOOL _GetProcessDefaultLayout(OUT DWORD *pdwDefaultLayout)
{
    NTSTATUS Status = STATUS_SUCCESS;
...
* 23-01-98 SamerA         Created.
*/
BOOL _SetProcessDefaultLayout(IN DWORD dwDefaultLayout)
{
    /*
    ...
    {
        RIPERR1(ERROR_INVALID_PARAMETER, RIP_WARNING, "Calling SetProcessDefaultLayout with invalid layout = %lX", dwDefaultLayout);
        return FALSE;
    }
...
NTSTATUS SetInformationProcess(IN HANDLE hProcess, IN USERPROCESSINFOCLASS ProcessInfoClass, IN PVOID ProcessInformation, IN ULONG ProcessInformationLength)
{
    ...
    Status = ObReferenceObjectByHandle(hProcess, PROCESS_SET_INFORMATION, *PsProcessType, UserMode, &Process, NULL);
    if (!NT_SUCCESS(Status)) {
        return Status;
    }
...
NTSTATUS xxxConsoleControl(IN CONSOLECONTROL ConsoleControl, IN PVOID ConsoleInformation, IN ULONG ConsoleInformationLength)
{
    ...
    case ConsoleDesktopConsoleThread:
        ...
        Status = ObReferenceObjectByHandle(pDesktopConsole->hdesk, 0, *ExDesktopObjectType, UserMode, &pdesk, NULL);
        if (!NT_SUCCESS(Status))
            return Status;
        ...
        if (pDesktopConsole->dwThreadId != (DWORD)-1) {
            pdesk->dwConsoleThreadId = pDesktopConsole->dwThreadId;
        }
    ...
    case ConsoleSetVDMCursorBounds:
        UserAssert((ConsoleInformation == NULL) || (ConsoleInformationLength == sizeof(RECT)));
    ...
    case ConsoleWindowStationProcess:
        ...
        UserSetConsoleProcessWindowStation(pConsoleWindowStationInfo->dwProcessId, pConsoleWindowStationInfo->hwinsta);
        break;
#if defined(FE_IME)
    /*
    ...
    case ConsoleRegisterConsoleIME:
        {
            ...
            Status = ObReferenceObjectByHandle(RegConIMEInfo->hdesk, 0, *ExDesktopObjectType, UserMode, &pdesk, NULL);
            if (!NT_SUCCESS(Status))
                return Status;
            ...
            if (pdesk->dwConsoleThreadId)
            {
                ...
                if (RegConIMEInfo->dwAction != REGCONIME_QUERY) {
                    ...
                    {
                        if ( (RegConIMEInfo->dwAction == REGCONIME_REGISTER) && !(ptiConsoleIME->TIF_flags & TIF_DONTATTACHQUEUE) )
                        {
                            /*
                             * Register
                             */
                        ...
                        else if ( (RegConIMEInfo->dwAction == REGCONIME_UNREGISTER) && (ptiConsoleIME->TIF_flags & TIF_DONTATTACHQUEUE) )
                        {
                            /*
                             * Unregister
                             */
    ...
    case ConsoleFullscreenSwitch:
        ...
        xxxbFullscreenSwitch(((PCONSOLE_FULLSCREEN_SWITCH)ConsoleInformation)->bFullscreenSwitch, ((PCONSOLE_FULLSCREEN_SWITCH)ConsoleInformation)->hwnd);
        break;
