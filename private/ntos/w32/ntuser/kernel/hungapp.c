...
/*
* Sets the specified redraw-if-hung flag in the window and adds the window to the list of windows to redraw if hung.
...
{
    /*
     * If the window has no hung redraw bits set and it's a top-level window, add it to the redraw list.
     */
...
/*
* Clears the specified redraw-if-hung flag in the window and if no other redraw-if-hung flags remain, remove the window from list of windows to be redrawn if hung.
* Many windows have WFREDRAW* bits set, but aren't in the list (only those that were top-level were added).
...
{
    /*
     * An app is considered hung if it isn't waiting for input, isn't in startup processing, and hasn't called PeekMessage() within the specified timeout.
     */
    if (((NtGetTickCount() - GET_TIME_LAST_READ(pti)) > dwTimeFromLastRead) && !((pti->pcti->fsWakeMask & QS_INPUT) && (pti->pEThread->Tcb.FreezeCount == 0)) && !(pti->ppi->W32PF_Flags & W32PF_APPSTARTING)) {
        return TRUE;
    }
...
/*
* If the hrgnFullDrag is NULL, redraw the hung window's entire update region, otherwise, only redraw the intersection of the window's update region with the FullDrag region.
...
VOID xxxRedrawHungWindow(PWND pwnd, HRGN hrgnFullDrag)
{
     ...
     */
    if (hrgnFullDrag && hrgnUpdate != HRGN_FULL && IntersectRgn(hrgnUpdate, hrgnUpdate, hrgnFullDrag) == NULLREGION) {
        GreDeleteObject(hrgnUpdate);
        return;
    }
    ...
    hdc = _GetDCEx(pwnd, hrgnUpdate, DCX_USESTYLE | DCX_WINDOW | DCX_INTERSECTRGN | DCX_NODELETERGN | DCX_LOCKWINDOWUPDATE);
    ...
    if (hrgnUpdate != NULL && !TestWF(pwnd, WFCLIPCHILDREN)) {
        ...
        for (pwndT = pwnd->spwndChild; pwndT != NULL; pwndT = pwndT->spwndNext) {
            if (TestWF(pwndT, WFVISIBLE) && (TestWF(pwndT, WFSTARTPAINT) || pwndT->hrgnUpdate == NULL) && IntersectRect(&rcT, &rc, &pwndT->rcWindow)) {
                /*
                 ...
                 */
                BEGINATOMICCHECK();
                xxxInternalInvalidate(pwndT, hrgnUpdate, RDW_INVALIDATE | RDW_FRAME | RDW_ERASE | RDW_ALLCHILDREN);
                ENDATOMICCHECK();
            }
        }
    }
...
* We keep a list of redraw-if-hung windows in a list that remains in a single page to avoid touching the windows themselves each time through this routine.
* Touching the windows causes a bunch of unnecessary paging and in effect keeps all of the pages that contain top-level windows resident at all times; this is very wasteful.
