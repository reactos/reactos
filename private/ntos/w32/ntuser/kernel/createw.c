    ...
    DWORD         dwExpWinVerAndFlags)
{
    ...
    /*
     * For Edit Controls (including those in comboboxes), we must know whether the App used an ANSI or a Unicode CreateWindow call.
     * This is passed in with the private WS_EX_ANSICREATOR dwExStyle bit, 
       but we MUST NOT leave out this bit in the window's dwExStyle! Transfer to the internal window flag WFANSICREATOR immediately.
     */
...
#endif
    /*
     * Ensure that we can create the window.
     * If there is no desktop yet, assume that this will be the root desktop window and allow the creation.
     */
    if (ptiCurrent->hdesk) {
        RETURN_IF_ACCESS_DENIED(ptiCurrent->amdesk, DESKTOP_CREATEWINDOW, NULL);
    }
...
CantFindClassMessageAndFail:
        ...
        if (IS_PTR(cczpstrClass)) {
            try {
                RIPMSG1(RIP_WARNING, "Couldn't find class string %ws", cczpstrClass->Buffer);
            } except (W32ExceptionHandler(FALSE, RIP_WARNING)) {

            }
        } else {
            RIPMSG1(RIP_WARNING, "Couldn't find class atom %lx", cczpstrClass);
        }
#endif
     ...
     */
    pwnd = HMAllocObject(ptiCurrent, pdesk, TYPE_WINDOW, sizeof(WND) + pcls->cbwndExtra);
    if (pwnd == NULL) {
        RIPERR0(ERROR_OUTOFMEMORY, RIP_WARNING, "Out of pool in xxxCreateWindowEx");
        return NULL;
    }
    ...
    if (!IS_PTR(cczpstrClass)) {
        ...
        if (wWFAnsiCreator) {
            ...
            if (IS_PTR(csex.cs.lpszClass)) {
                RtlInitLargeAnsiString((PLARGE_ANSI_STRING)&csex.strClass, (LPSTR)csex.cs.lpszClass, (UINT)-1);
            }
        } else {
...
#error Fix UISTATE bits copying if you moved the UISTATE bits from ExStyle
    ...
    if (IsHooked(ptiCurrent, WHF_CBT)) {
        ...
        if ((BOOL)xxxCallHook(HCBT_CREATEWND, (WPARAM)HWq(pwnd), (LPARAM)&cbt, WH_CBT)) {
            goto MemError;
        } else {
            /*
...
//        if (!TestWF(pwnd, WEFEDGEMASK))
     ...
     */
    if (fStartup) {
        ptiCurrent->ppi->usi.dwFlags &= ~(STARTF_USESIZE | STARTF_USEPOSITION);
    }
...
// LATER mikeke why is xxxGetSystemMenu() returning NULL?
    ...
    if (!fChild) {
        ...
        if ((pwndParent != NULL) &&
            ...
            if (!ValidateOwnerDepth(pwnd, pwndOwner)) {
                RIPERR1(ERROR_INVALID_PARAMETER, RIP_WARNING, "Exceeded nested owner limit for pwnd %#p", pwnd);
                goto MemError;
            }
             ...
             */
            if (atomT != gpsi->atomSysClass[ICLS_IME] && pwnd->spwndOwner != NULL && GETPTI(pwnd->spwndOwner) != ptiCurrent) {
                /*
...
#endif
        if ((pwndParent == NULL) || (pwndParent != pwndParent->head.rpdesk->spwndMessage)) {
    ...
    if (!TestWF(pwnd, WFWIN31COMPAT)) {
         ...
         */
        if ((pcls->style & CS_PARENTDC) && !TestWF(pwndParent, WFCLIPCHILDREN)) {
#if DBG
     ...
     */
    if (TestwndChild(pwnd) && (pwndParent != PWNDDESKTOP(pwnd)) && (ptiCurrent != GETPTI(pwndParent))) {
        /*
    ...
    if (TestCF2(pcls, CFOWNDC) || (TestCF2(pcls, CFCLASSDC) && pcls->pdce == NULL)) {
        if (NULL == CreateCacheDC(pwnd, DCX_OWNDC, NULL)) {
            RIPMSG1(RIP_WARNING, "xxxCreateWindowEx: pwnd %#p failed to create cached DC", pwnd);
            goto MemError;
        }
    }
...
MemError:
        ...
        if (!IS_PTR(cczpstrClass)) {
            RIPMSG2(RIP_WARNING,
                    (pwndParent) ? "xxxCreateWindowEx failed, Class=%#.4x, ID=%d" : "xxxCreateWindowEx failed, Class=%#.4x",
                    PTR_TO_ID(cczpstrClass),
                    (LONG_PTR) pMenu);
        } else {
            RIPMSG2(RIP_WARNING,
                    (pwndParent) ? "xxxCreateWindowEx failed, Class=\"%s\", ID=%d" : "xxxCreateWindowEx failed, Class=\"%s\"",
    ...
    if (TestWF(pwnd, WFTITLESET))
        if (!(csex.strName.Buffer != NULL && csex.strName.Length == 0 && pwnd->strName.Buffer == NULL)) {
    ...
    if (IS_IME_ENABLED() && ptiCurrent->spwndDefaultIme == NULL) {
        ...
        if (WantImeWindow(pwndParent, pwnd)) {
            ...
            Lock(&(ptiCurrent->spwndDefaultIme), xxxCreateDefaultImeWindow(pwnd, atomT, hInstance));
    ...
    if (pwndParent != NULL) {
        if (!fChild && (pwndParent != pwndParent->head.rpdesk->spwndMessage)) {
            ...
            if (!TestWF(pwnd, WEFTOPMOST)) {
                if (pwndZOrder == PWND_TOP || TestWF(pwndZOrder, WEFTOPMOST)) {
...
#if DBG
        if (!IS_PTR(cczpstrClass)) {
            RIPMSG1(RIP_WARNING, "CreateWindow() send of WM_CREATE failed, Class = 0x%x", PTR_TO_ID(cczpstrClass));
        } else {
            RIPMSG1(RIP_WARNING, "CreateWindow() send of WM_CREATE failed, Class = \"%s\"", pcls->lpszAnsiClassName);
        }
#endif
        if (fLockParent)
            ThreadUnlock(&tlpwndParent);
    ...
    if (fChild && !TestWF(pwnd, WEFNOPARENTNOTIFY) && (pwnd->spwndParent != NULL)) {
        ThreadLockAlwaysWithPti(ptiCurrent, pwnd->spwndParent, &tlpwndParentT);
        xxxSendMessage(pwnd->spwndParent, WM_PARENTNOTIFY, MAKELONG(WM_CREATE, PTR_TO_ID(pwnd->spmenu)), (LPARAM)HWq(pwnd));
