...
/*
* Open the windowstation assigned to the service logon session.
* If no windowstation exists, create the windowstation and a default desktop.
...
HWINSTA xxxConnectService(PUNICODE_STRING pstrWinSta, HDESK *phdesk)
{
    ...
    Status = ZwQueryInformationToken(hToken, TokenUser, ptuService, ulLength, &ulLength);
    ZwClose(hToken);
    if (!NT_SUCCESS(Status)) {
     ...
     * Create ACE list.
     */
    paceService = AllocAce(NULL, ACCESS_ALLOWED_ACE_TYPE, 0, WINSTA_CREATEDESKTOP | WINSTA_READATTRIBUTES | WINSTA_ACCESSGLOBALATOMS | WINSTA_EXITWINDOWS | WINSTA_ACCESSCLIPBOARD | STANDARD_RIGHTS_REQUIRED, psid, &ulLength);
    if (paceService == NULL) {
        RIPMSG0(RIP_WARNING, "ConnectService: AllocAce for WindowStation attributes failed");
        goto sd_error;
    }
    pace = AllocAce(paceService, ACCESS_ALLOWED_ACE_TYPE, OBJECT_INHERIT_ACE | INHERIT_ONLY_ACE | NO_PROPAGATE_INHERIT_ACE,
        DESKTOP_READOBJECTS | DESKTOP_WRITEOBJECTS | DESKTOP_ENUMERATE | DESKTOP_CREATEWINDOW | DESKTOP_CREATEMENU | DESKTOP_HOOKCONTROL | STANDARD_RIGHTS_REQUIRED, psid, &ulLength);
    if (pace == NULL) {
        RIPMSG0(RIP_WARNING, "ConnectService: AllocAce for Desktop Attributes failed");
        goto sd_error;
    }
    paceService = pace;
    pace = AllocAce(pace, ACCESS_ALLOWED_ACE_TYPE, 0, WINSTA_ENUMERATE, SeExports->SeAliasAdminsSid, &ulLength);
    if (pace == NULL) {
        RIPMSG0(RIP_WARNING, "ConnectService: AllocAce for admin WinSta enumerate failed");
        goto sd_error;
    }
    paceService = pace;
    pace = AllocAce(pace, ACCESS_ALLOWED_ACE_TYPE, OBJECT_INHERIT_ACE | INHERIT_ONLY_ACE | NO_PROPAGATE_INHERIT_ACE, DESKTOP_READOBJECTS | DESKTOP_WRITEOBJECTS | DESKTOP_ENUMERATE, SeExports->SeAliasAdminsSid, &ulLength);
    if (pace == NULL) {
     ...
     */
    InitializeObjectAttributes(&ObjService, pstrWinSta, OBJ_OPENIF, NULL, psdService);
    hwinsta = xxxCreateWindowStation(&ObjService, KernelMode, MAXIMUM_ALLOWED, NULL, 0, NULL, 0);
    if (hwinsta != NULL) {
        TRACE_INIT(("Service windowstation created\n"));

        /*
         * We have the windowstation, now create the desktop.
         * The security descriptor will be inherited from the windowstation.
         * Save the winsta handle because the access struct may be moved by the desktop creation.
         */
        RtlInitUnicodeString(&strDesktop, TEXT("Default"));
        InitializeObjectAttributes(&ObjService, &strDesktop, OBJ_OPENIF | OBJ_CASE_INSENSITIVE, hwinsta, NULL);
        *phdesk = xxxCreateDesktop(&ObjService, KernelMode, NULL, NULL, 0, MAXIMUM_ALLOWED);
        if (*phdesk == NULL) {
            /*
             * The creation failed, wake the desktop thread, close the windowstation and leave.
             */
