...
void ValidateSmsSendLists(PSMS psms)
{
    ...
    for (psmsT2 = gpsmsList; psmsT2 != NULL; psmsT2 = psmsT2->psmsNext) {
        if (psmsT2->ptiSender != NULL) {
            for (psmsT3 = psmsT2->psmsSendList; psmsT3 != NULL; psmsT3 = psmsT3->psmsSendNext) {
                if (psmsT3 == psmsT2)
                    break;
            }

            UserAssertMsg2(psmsT3 != NULL, "sms %x is not on send list %x\n", psmsT2, psmsT2->psmsSendList);
        }
    }
}
#endif
...
/*
* Allocates a message on a message list. DelSMS deletes a message on a message list.
...
BOOL _ReplyMessage(LRESULT lRet)
{
    ...
    /*
     * Blow off the rest of the call if the SMS came from xxxSendNotifyMessage().
     * Obviously there's no one around to reply to in the case.
     */
     ...
     */
    if (psms->ptiSender && (psms->ptiSender->TIF_flags & TIF_16BIT || ptiCurrent->TIF_flags & TIF_16BIT)) {
...
VOID UserLogError(PCWSTR pwszError, ULONG cbError, NTSTATUS ErrorCode)
{
     ...
     */
    perrLogEntry = (PIO_ERROR_LOG_PACKET)IoAllocateErrorLogEntry(gpWin32kDriverObject, (UCHAR)(cbError + sizeof(IO_ERROR_LOG_PACKET)));
    if (perrLogEntry) {
...
BOOL xxxSendBSMtoDesktop(PWND pwndDesk, UINT message, WPARAM wParam, LPARAM lParam, LPBROADCASTSYSTEMMSGPARAMS pbsmParams)
{
    ...
    if (!(pbsmParams->dwFlags & BSF_POSTMESSAGE)) {
         ...
         */
        if((pbsmParams->dwFlags & BSF_ALLOWSFW) && (GETPDESK(pwndDesk) == grpdeskRitInput) && ((ptiCurrent->TIF_flags & TIF_CSRSSTHREAD) || CanForceForeground(ptiCurrent->ppi))) {
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++)
    {
        ...
        if (pbsmParams->dwFlags & BSF_POSTMESSAGE) {
            ...
            if (xxxSendMessageTimeout(pwnd, message, wParam, lParam, (fNoHang ? SMTO_ABORTIFHUNG : SMTO_NORMAL) | ((pbsmParams->dwFlags & BSF_NOTIMEOUTIFNOTHUNG) ? SMTO_NOTIMEOUTIFNOTHUNG : 0), dwTimeout, &dwResult)) {
...
LONG xxxSendMessageBSM(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam, LPBROADCASTSYSTEMMSGPARAMS pbsmParams)
{
    ...
    if (pbsmParams->dwRecipients & BSM_ALLDESKTOPS) {
        ...
        lRet = xxxSendBSMtoDesktop(pwnd, message, wParam, lParam, pbsmParams);
    }
...
LRESULT xxxSendMessageFF(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam)
{
    ...
    if (xParam != 0L) {
         ...
         */
        return xxxSendMessageTimeout(PWND_BROADCAST, message, wParam, lParam, SMTO_NORMAL, 0, NULL );
    }
}
    ...
    for (pwinsta = grpWinStaList; pwinsta != NULL; )
    {
        ...
        for (pdesk = pwinsta->rpdeskList; pdesk != NULL; )
        {
            ...
            if (pdesk->pDeskInfo->spwnd != NULL) {
                xxxBroadcastMessage(pdesk->pDeskInfo->spwnd, message, wParam, lParam, wCmd1, pbcm);
            }
...
BOOL xxxSendNotifyMessage(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (pwnd == PWND_BROADCAST) {
        ...
        case WM_SPOOLERSTATUS:
            xxxSystemBroadcastMessage(message, wParam, lParam, BMSG_SENDNOTIFYMSG, NULL);
            return 1;
        default:
            break;
        }
...
BOOL xxxSendMessageCallback(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam, SENDASYNCPROC lpResultCallBack, ULONG_PTR dwData, BOOL fClientRequest)
{
    ...
    if (TESTSYNCONLYMESSAGE(message, wParam)) {
        RIPERR1(ERROR_MESSAGE_SYNC_ONLY, RIP_WARNING, "Trying to non-synchronously send a structure msg=%lX", message);
        return FALSE;
    }
    ...
    if (fQueuedNotify || ptiCurrent != GETPTI(pwnd)) {
        ...
        return (BOOL)xxxInterSendMsgEx(pwnd, message, wParam, lParam, NULL, GETPTI(pwnd), pism );
    }
    ...
    if (lpResultCallBack != NULL) {
        ...
        */
        if (fClientRequest) {
             ...
             */
            CallClientProcA(pwnd, message, dwData, lRet, (ULONG_PTR)lpResultCallBack);
        } else {
...
* that means we're called from xxxSendNotifyMessage() and should act accordingly.
...
LRESULT xxxInterSendMsgEx(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam, PTHREADINFO ptiSender, PTHREADINFO ptiReceiver, PINTRSENDMSGEX pism)
{
    ...
    try {
        ...
        case CB_FINDSTRINGEXACT:     // INCBOXSTRING calls fnINSTRING
             ...
             * style. If so, treat lParam as a DWORD.
             */
            if (pwnd && !(pwnd->style & CBS_HASSTRINGS) && (pwnd->style & (CBS_OWNERDRAWFIXED | CBS_OWNERDRAWVARIABLE))) {
                /*
                 * Treat lParam as a dword.
                 */
                break;
...
fnINSTRINGThunk:
        ...
        if (cbCapture && (psms->pvCapture = UserAllocPoolWithQuota(cbCapture, TAG_SMS_CAPTURE)) != NULL) {
            lParamSave = lParam;
    ...
    *ppsms = psms;
    ...
    /*
     * If we're not being called from xxxSendNotifyMessage() or SendMessageCallback(), then sleep while we wait for the reply.
     */
    if (ptiSender == NULL) {
        /*
        ...
        while (!(psms->flags & SMF_REPLY) && !fTimeOut) {
             ...
             * interaction yet.
             */
            if (fTimeOut && pism && (pism->fuSend & SMTO_NOTIMEOUTIFNOTHUNG) && !FHungApp(ptiReceiver, CMSHUNGAPPTIMEOUT)) {
                fTimeOut = FALSE;
            }
        }
        ...
        if (!fTimeOut && cbOutput) {
            ...
            try {
                if(fString == NoString) {
                    RtlCopyMemory((PBYTE)pbOutput, psms->pvCapture, cbOutput);
                } else if(fString == IsAnsiString) {
                    len = strncpycch((LPSTR)pbOutput,(LPCSTR)psms->pvCapture, cbOutput);
                    #if DBG
                    ...
                    len = wcsncpycch((LPWSTR)pbOutput,(LPCWSTR)psms->pvCapture, cbOutput/sizeof(WCHAR));
                    #if DBG
                    len--;
                     if(len != psms->lRet) {
                        RIPMSG0(RIP_WARNING,
...
VOID xxxReceiveMessage(PTHREADINFO ptiReceiver)
{
    ...
    if (psms->flags & SMF_CB_REPLY) {
        ...
        if (psms->flags & SMF_CB_CLIENT) {
             ...
             */
            CallClientProcA(psms->spwnd, psms->message, psms->dwData, psms->lRet, (ULONG_PTR)psms->lpResultCallBack);
        } else {
            psms->lpResultCallBack(HW(psms->spwnd), psms->message, psms->dwData, psms->lRet);
        }
        ...
        /*
         * If this SMS originated from a xxxSendNotifyMessage() or a xxxSendMessageCallback() call, the sender's thread lock is already gone, so we need to re-lock here.
         */
        if (ptiSender == NULL) {
            ThreadLock(psms->spwnd, &tlpwnd);
        }
        ...
        if (psms->message == WM_HOOKMSG) {
            ...
            if (!(psms->flags & (SMF_REPLY | SMF_SENDERDIED | SMF_RECEIVERDIED)) && psms->spwnd != NULL) {
                if (TestWF(psms->spwnd, WFSERVERSIDEPROC)) {
                     ...
                     * how we don't convert the pwnd into an hwnd before calling the proc.
                     */
                    lRet = psms->spwnd->lpfnWndProc(psms->spwnd, psms->message, psms->wParam, psms->lParam);
                    ThreadUnlock(&tlpwndKernel);
                } else {
                    /*
                     * Call the client or xxxDefWindowProc.
                     */
                    XXXSENDMESSAGETOCLIENT(psms->spwnd, psms->message, psms->wParam, psms->lParam, psms, TRUE);
                }

                /*
                 * Call WH_CALLWNDPROCRET if it's installed.
                 */
                if (IsHooked(ptiReceiver, WHF_CALLWNDPROCRET) && !(psms->flags & SMF_SENDERDIED)) {
                    CWPRETSTRUCTEX cwps;
        ...
        SET_OR_CLEAR_FLAG(ptiReceiver->pcti->CTIF_flags, CTIF_INSENDMESSAGE, ptiReceiver->psmsCurrent);
    ...
    if (!(psms->flags & SMF_REPLY)) {
        ...
        if (ptiSender != NULL) {
            ...
            if (ptiSender->TIF_flags & TIF_16BIT || ptiReceiver->TIF_flags & TIF_16BIT) {
                ...
                if (ptiReceiver->TIF_flags & TIF_16BIT && ptiSender->psmsSent == psms)
                  {
                    xxxSleepTask(TRUE, NULL);
                }
            }
...
VOID SendMsgCleanup(PTHREADINFO ptiCurrent)
{
    ...
    for (ppsms = &gpsmsList; *ppsms; )
    {
        ...
        if ((*ppsms)->ptiSender == ptiCurrent || (*ppsms)->ptiCallBackSender == ptiCurrent) {
                ...
                if (!(psms->flags & SMF_REPLY)) {
                    ...
                    if (psms->flags & SMF_CB_REQUEST) {
                        ...
                        if (psms->ptiSender == NULL) {
                             ...
                             * to the sender.
                             */
                            for (ppsms = &(psms->ptiReceiver->psmsReceiveList); *ppsms != NULL; ppsms = &((*ppsms)->psmsReceiveNext)) {
                                if (*ppsms == psms) {
...
VOID ReceiverDied(PSMS psms, PSMS *ppsmsUnlink)
{
    ...
    if (!(ptiReceiver->TIF_flags & TIF_INCLEANUP)) {
         ...
         */
        for (ppsms = &(ptiReceiver->psmsReceiveList); *ppsms != NULL; ppsms = &((*ppsms)->psmsReceiveNext)) {
            if (*ppsms == psms) {
                *ppsms = psms->psmsReceiveNext;
                break;
            }
...
* 01-13-91 DavidPe      Ported.
*/
VOID SenderDied(PSMS psms, PSMS *ppsmsUnlink)
{
    PTHREADINFO ptiSender;
    BOOL fReply = FALSE;
     ...
     */
    if ((psms->flags & SMF_RECEIVERDIED) || (psms->flags & (SMF_REPLY | SMF_RECEIVERFREE)) == SMF_REPLY) {
        UnlinkSendListSms(psms, ppsmsUnlink);
    } else {
        psms->flags |= SMF_RECEIVERFREE;
    }
...
* 01-13-91 DavidPe      Ported.
*/
VOID UnlinkSendListSms(PSMS psms, PSMS *ppsmsUnlink)
{
#if DBG
    PSMS psmsT;
...
/*
* Processes an event message posted by xxxSystemBroadcastMessage by sending a message to the window stored in the event.
...
LONG xxxBroadcastMessage(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam, UINT wCmd, PBROADCASTMSG pbcm)
{
    ...
    if (pwnd == NULL) {
        ...
        case WM_DEVMODECHANGE:
            ...
            if (lParam) {
                ...
                try {
                    if (pstr->bAnsi) {
                        Status = RtlMultiByteToUnicodeN((PWCH)str.Buffer, cbAlloc, &cbAlloc, (PCH)pstr->Buffer, pstr->Length);
                        str.Length = cbAlloc;
                    } else {
            ...
            xxxSystemBroadcastMessage(message, wParam, lParam ? (LPARAM)&str : 0, wCmd, pbcm);
            if (lParam)
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++)
    {
         ...
         */
        if ((message == WM_PALETTEISCHANGING || message == WM_PALETTECHANGED) && !TestWF(pwnd, WFVISIBLE) && !(GETPTI(pwnd)->TIF_flags & TIF_PALETTEAWARE)) {
            continue;
        }
...
CleanupAtom:
        ...
        case BMSG_SENDMSGCALLBACK:
            xxxSendMessageCallback(pwnd, message, wParam, lParam, pbcm->cb.lpResultCallBack, pbcm->cb.dwData, pbcm->cb.bClientRequest);
            break;
        case BMSG_SENDMSGTIMEOUT:
            xxxSendMessageTimeout(pwnd, message, wParam, lParam, pbcm->to.fuFlags, pbcm->to.uTimeout, pbcm->to.lpdwResult);
            break;
        }
