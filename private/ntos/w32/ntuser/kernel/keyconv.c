...
BOOL xxxTranslateMessage(LPMSG pmsg, UINT uiTMFlags)
{
    ...
    case WM_KEYUP:
        ...
        if ((pti->pMenuState != NULL) && (HW(pti->pMenuState->pGlobalPopupMenu->spwndPopupMenu) == pmsg->hwnd)) {
...
*/
        ...
        for (pwch = awch; cChar > 0; cChar--) {
             ...
             */
            _PostMessage(pwnd, wMsgType, (WPARAM)*pwch, lParam | (cChar > 1 ? FAKE_KEYSTROKE : 0));
