...
{
    TAGMSG2(DBGTAG_PNP, "Win32kPnPDriverEntry(DriverObject = %lx, pustrRegistryPath = %#p)", DriverObject, pustrRegistryPath);
...
VOID InitializeMediaChange(HANDLE hMediaRequestEvent)
{
    if (!gbRemoteSession) {
        ...
        ObReferenceObjectByHandle(hMediaRequestEvent, EVENT_ALL_ACCESS, *ExEventObjectType, KernelMode, &gpEventMediaChange, NULL);
...
ULONG GetDeviceChangeInfo()
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        irp = IoBuildDeviceIoControlRequest(IOCTL_MOUNTMGR_NEXT_DRIVE_LETTER,
                                            DeviceObject,
                                            &pContext->DeviceName,
                                            sizeof(MOUNTMGR_DRIVE_LETTER_TARGET) + pContext->DeviceName.DeviceNameLength,
                                            &output,
                                            sizeof(output),
                                            FALSE,
                                            &event,
...
NTSTATUS DeviceCDROMNotify(IN PTARGET_DEVICE_CUSTOM_NOTIFICATION Notification, IN PCDROM_NOTIFY pContext)
{
    ...
    // This is due to opening a handle from within the system process which requires us to do an attach process.
    ...
    if (IsEqualGUID(&classChange->Event, &GUID_DEVICE_INTERFACE_ARRIVAL)) {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            if (pContext) {
                ...
                RtlCopyMemory(pContext->DeviceName.DeviceName,classChange->SymbolicLinkName->Buffer,pContext->DeviceName.DeviceNameLength);
...
PDEVICEINFO CreateDeviceInfo(DWORD DeviceType, PUNICODE_STRING pustrName, BYTE bFlags)
{
    ...
    if (pustrName->Buffer != NULL) {
        ...
        if (pDeviceInfo->ustrName.Buffer == NULL) {
            RIPMSG1(RIP_WARNING, "CreateDeviceInfo: Can't duplicate string %ws", pustrName->Buffer);
            goto CreateFailed;
        }
    ...
    if (pDeviceInfo->pkeHidChangeCompleted == NULL) {
        RIPMSG0(RIP_WARNING, "CreateDeviceInfo: failed to create pkeHidChangeCompleted");
        goto CreateFailed;
    }
    ...
    )
{
    ...
    TAGMSG3(DBGTAG_PNP | RIP_THERESMORE, " Event GUID %lx, %x, %x", classChange->Event.Data1, classChange->Event.Data2, classChange->Event.Data3);
...
BOOL OpenMultiplePortDevice(DWORD DeviceType)
{
     ...
     */
    uiConnectMultiplePorts = FastGetProfileDwordW(NULL, pDevTpl->uiRegistrySection, L"ConnectMultiplePorts", 0);
...
NTSTATUS xxxRegisterForDeviceClassNotifications()
{
    ...
    for (DeviceType = 0; DeviceType <= DEVICE_TYPE_MAX; DeviceType++) {
        if (!OpenMultiplePortDevice(DeviceType)) {
            ...
            if (!NT_SUCCESS(Status)) {
                RIPMSG2(RIP_ERROR, "IoRegisterPlugPlayNotification(%d) failed, status %lx", DeviceType, Status);
            }
        }
    }
...
* Query the device information.  This function is an async function, so be sure any buffers it uses aren't allocated on the stack!

* If this is an asynchronous IOCTL, perhaps we should be waiting on the file handle or on an event for it to succeed?
...
NTSTATUS QueryDeviceInfo(PDEVICEINFO pDeviceInfo)
{
    ...
    if (!NT_SUCCESS(Status)) {
        RIPMSG2(RIP_WARNING, "QueryDeviceInfo(%p): IOCTL failed - Status %lx", pDeviceInfo, Status);
    }
...
BOOL OpenDevice(PDEVICEINFO pDeviceInfo)
{
    ...
    TAGMSG3(DBGTAG_PNP, "OpenDevice(): Opening type %d (%lx %ws)", pDeviceInfo->type, pDeviceInfo->handle, pDeviceInfo->ustrName.Buffer);
    ...
    if (gbRemoteSession) {
        ...
        case DEVICE_TYPE_KEYBOARD:
            ...
            break;
        default:
            RIPMSG2(RIP_ERROR, "Unknown device type %d DeviceInfo %#p", pDeviceInfo->type, pDeviceInfo);
            return FALSE;
        }
    } else {
...
#endif
        Status = ZwCreateFile(&pDeviceInfo->handle, FILE_READ_DATA | SYNCHRONIZE, &ObjectAttributes, &pDeviceInfo->iosb, NULL, 0, FILE_SHARE_WRITE, FILE_OPEN_IF, 0, NULL, 0);
...
VOID CloseDevice(PDEVICEINFO pDeviceInfo)
{
    ...
    TAGMSG3(DBGTAG_PNP, "CloseDevice(): closing type %d (%lx %ws)", pDeviceInfo->type, pDeviceInfo->handle, pDeviceInfo->ustrName.Buffer);
    if (pDeviceInfo->handle) {
        ...
        UserAssertMsg2(NT_SUCCESS(IoStatusBlock.Status), "NtCancelIoFile handle %x failed status %#x", pDeviceInfo->handle, IoStatusBlock.Status);
        Status = ZwClose(pDeviceInfo->handle);
        UserAssertMsg2(NT_SUCCESS(Status), "ZwClose handle %x failed status %#x", pDeviceInfo->handle, Status);
         ...
         */
        UserAssert((pDeviceInfo->iosb.Status == STATUS_CANCELLED) || (pDeviceInfo->ReadStatus == STATUS_INVALID_HANDLE));
    }
}
...
BOOL RegisterForDeviceChangeNotifications(PDEVICEINFO pDeviceInfo)
{
    ...
    Status = ObReferenceObjectByHandle(pDeviceInfo->handle, 0, NULL, KernelMode, (PVOID)&pFileObject, NULL);
    if (NT_SUCCESS(Status)) {
        Status = IoRegisterPlugPlayNotification (
...
BOOL UnregisterForDeviceChangeNotifications(PDEVICEINFO pDeviceInfo)
{
    ...
    TAGMSG3(DBGTAG_PNP, "UnregisterForDeviceChangeNotifications(): type %d (%lx %ws)", pDeviceInfo->type, pDeviceInfo, pDeviceInfo->ustrName.Buffer);
    Status = IoUnregisterPlugPlayNotification(pDeviceInfo->NotificationEntry);
    if (!NT_SUCCESS(Status)) {
        RIPMSG2(RIP_ERROR, "IoUnregisterPlugPlayNotification failed Status = %lx, DEVICEINFO %lx", Status, pDeviceInfo);
        return FALSE;
    }
    ...
    IN PDEVICEINFO pDeviceInfo)  // should the context be a kernel address?
{
    ...
    if (IsEqualGUID(&pNotification->Event, &GUID_TARGET_DEVICE_QUERY_REMOVE)) {
        ...
        TAGMSG4(DBGTAG_PNP | RIP_NONAME, "GUID Unknown: %lx:%lx:%lx:%x...", pNotification->Event.Data1, pNotification->Event.Data2, pNotification->Event.Data3, pNotification->Event.Data4[0]);
...
PDEVICEINFO StartDeviceRead(PDEVICEINFO pDeviceInfo)
{
    ...
    if (pDeviceInfo->bFlags & GDIF_DBGREAD) {
        TAGMSG2(DBGTAG_PNP, "ZwReadFile of Device handle %lx returned status %lx", pDeviceInfo->handle, pDeviceInfo->ReadStatus);
    }
#endif
    ...
    if (!ISTS() && !NT_SUCCESS(pDeviceInfo->ReadStatus))
        RIPMSG2(RIP_WARNING, "StartDeviceRead %#p failed Status %#x", pDeviceInfo, pDeviceInfo->ReadStatus);
    ...
    while (pDeviceInfo) {
        ...
        if (pDeviceInfo->usActions & (GDIAF_ARRIVED | GDIAF_REMOVECANCELLED)) {
            ...
            if (OpenDevice(pDeviceInfo)) {
                ...
                if ((usOriginalActions & GDIAF_ARRIVED) == 0) {
                    RIPMSG2(RIP_WARNING, "Re-Open %#p failed status %x during RemoveCancelled", pDeviceInfo, pDeviceInfo->OpenStatus);
                }
#endif
                pDeviceInfo = FreeDeviceInfo(pDeviceInfo);
                continue;
        ...
        if (pDeviceInfo->usActions & GDIAF_IME_STATUS) {
            ...
            if ((pDeviceInfo->type == DEVICE_TYPE_KEYBOARD) && (pDeviceInfo->handle)) {
                if (FUJITSU_KBD_CONSOLE(pDeviceInfo->keyboard.Attr.KeyboardIdentifier) || (gbRemoteSession && FUJITSU_KBD_REMOTE(gRemoteClientKeyboardType))) {
                    /*
                     * Fill up the KEYBOARD_IME_STATUS structure.
                     */
                    ZwDeviceIoControlFile(pDeviceInfo->handle, NULL, NULL, NULL, &giosbKbdControl, IOCTL_KEYBOARD_SET_IME_STATUS, (PVOID)&gKbdImeStatus, sizeof(gKbdImeStatus), NULL, 0);
                }
            }
        }
...
#endif
            TAGMSG2(DBGTAG_PNP, "Retry Read %#p after %lx ticks", pDeviceInfo, pDeviceInfo->timeStartRead - timeStartReadPrev);
            pDeviceInfoNext = StartDeviceRead(pDeviceInfo);
            if (pDeviceInfoNext) {
                /*
                 * pDeviceInfo wasa freed, move onto the next
        ...
        if (pDeviceInfo->handle) {
            ...
            case DEVICE_TYPE_KEYBOARD:
                UserAssert(PtiCurrentShared() == gptiRit);
                {
                    ...
                    gKbdIoctlLEDSStatus =
#endif
                    Status = ZwDeviceIoControlFile(pDeviceInfo->handle, NULL, NULL, NULL, &giosbKbdControl, IOCTL_KEYBOARD_QUERY_INDICATORS, NULL, 0, (PVOID)&gklpBootTime, sizeof(gklpBootTime));
                    UserAssertMsg2(NT_SUCCESS(Status), "IOCTL_KEYBOARD_QUERY_INDICATORS failed: DeviceInfo %#x, Status %#x", pDeviceInfo, Status);
            ...
            default:
                ...
                RIPMSG2(RIP_ERROR, "pDeviceInfo %#p has strange type %d", pDeviceInfo, pDeviceInfo->type);
                break;
            }
        }
...
VOID RequestDeviceChange(PDEVICEINFO pDeviceInfo, USHORT usAction, BOOL fInDeviceInfoListCrit)
{
    ...
    if (pDeviceInfo->usActions != 0) {
        TAGMSG3(DBGTAG_PNP, "RequestDeviceChange(%#p, %x), but action %x pending", pDeviceInfo, usAction, pDeviceInfo->usActions);
    }
