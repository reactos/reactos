...
void CheckPtiSysPeek(int where, PQ pq, ULONG_PTR newIdSysPeek)
{
    ...
    TAGMSG5(DBGTAG_SysPeek | dwRip, "%d pti %#p sets id %#p to pq %#p ; old id %#p", where, ptiCurrent, newIdSysPeek, pq, pq->idSysPeek);

    if (newIdSysPeek > 1) {
        PQMSG pqmsg = (PQMSG)newIdSysPeek;
        TAGMSG5(DBGTAG_SysPeek | RIP_NONAME, "-> msg %lx hwnd %#p w %#p l %#p pti %#p", pqmsg->msg.message, pqmsg->msg.hwnd,  pqmsg->msg.wParam, pqmsg->msg.lParam,  pqmsg->pti);
    }
}
...
{
    PTHREADINFO ptiCurrent = PtiCurrent();
    TAGMSG5(DBGTAG_SysPeek, "%d pti %#p sets ptiSL %#p to pq %#p ; old ptiSL %#p\n", where, ptiCurrent, ptiSysLock, pq, pq->ptiSysLock);
}
#endif //DEBUGTAGS
...
/*
* This API will block until an input message is received on the current queue.
...
* greater than CSPINBACKGROUND and the process isn't at the background priority, put it there.
* If it is less than this and should be foreground and isn't put it there.
...
* where spinning foreground apps affect the server, the printer spooler, and mstest scenarios.
* On Win3.1, calling PeekMessage() involves making a trip through the scheduler, forcing other apps to run.
* Processes run with priority on NT, where the foreground process gets foreground priority for greater responsiveness.
...
*   however, to avoid potentail deadlocks and maintain NT4 compatibility as much as possible, we fail the call if QS_SENDMESSAGE is set in fsWakeBits but not requested by the caller.
*   The same applies to QS_EVENT which we would always process in NT4.
...
#define DUMPPATHTAKEN()
     ...
     */
    pq = ptiCurrent->pq;
    if (   (ptiCurrent->psmsCurrent != NULL) || (pq->ptiSysLock == ptiCurrent && pq->idSysLock == ptiCurrent->idLast)) {
        CheckSysLock(1, pq, NULL);
        pq->ptiSysLock = NULL;
        PATHTAKEN(2);
    } else if (pq->ptiSysLock && (pq->ptiSysLock->cVisWindows == 0) && (PhkFirstGlobalValid(ptiCurrent, WH_JOURNALPLAYBACK) != NULL)) {
        /*
    ...
    while (TRUE) {
        ...
        if (ptiCurrent->pcti->fsWakeBits & fsWakeMask & QS_SENDMESSAGE) {
            ...
            RIPMSG2(RIP_WARNING, "xxxInternalGetMessage:(1st test) sendmsgs pending. Bits:%#lx Mask:%#lx", ptiCurrent->pcti->fsWakeBits, fsWakeMask);
            goto NoMessages;
        }
         ...
         */
        if (ptiCurrent->pq->ptiSysLock == ptiCurrent && (ptiCurrent->pq->QF_flags & QF_LOCKNOREMOVE)) {
            /*
            ...
            if (fsWakeBits & fsWakeMask & (QS_INPUT | QS_EVENT)) {
                ...
                if (xxxScanSysQueue(ptiCurrent, lpMsg, pwndFilter, msgMin, msgMax, flags, fsWakeBits & fsWakeMask & (QS_INPUT | QS_EVENT))) {
                    PATHTAKEN(0x10);
                    break;
                }
            } else if (fsWakeBits & QS_EVENT) {
                RIPMSG2(RIP_WARNING, "xxxInternalGetMessage:(1st test)events pending. Bits:%#lx Mask:%#lx", fsWakeBits, fsWakeMask);
                goto NoMessages;
            }
        }
        ...
        if (fsWakeBits & fsWakeMask & QS_POSTMESSAGE) {
            if (xxxReadPostMessage(ptiCurrent, lpMsg, pwndFilter, msgMin, msgMax, fRemove)) {
                PATHTAKEN(0x20);
                break;
            }
        }
        ...
        if (fsWakeBits & fsWakeMask & (QS_INPUT | QS_EVENT)) {
            ...
            RIPMSG2(RIP_WARNING, "xxxInternalGetMessage:(2nd test)events pending. Bits:%#lx Mask:%#lx", fsWakeBits, fsWakeMask);
            goto NoMessages;
        }
        ...
        if (ptiCurrent->pcti->fsWakeBits & fsWakeMask & QS_SENDMESSAGE) {
            ...
            RIPMSG2(RIP_WARNING, "xxxInternalGetMessage:(2nd test)sendmsgs pending. Bits:%#lx Mask:%#lx", ptiCurrent->pcti->fsWakeBits, fsWakeMask);
            goto NoMessages;
        }
        ...
        if (!(flags & PM_NOYIELD)) {
            ...
            if (ptiCurrent->pcti->fsWakeBits & fsWakeMask & QS_SENDMESSAGE) {
                ...
                RIPMSG2(RIP_WARNING, "xxxInternalGetMessage:(3rd test) sendmsgs pending. Bits:%#lx Mask:%#lx", ptiCurrent->pcti->fsWakeBits, fsWakeMask);
                goto NoMessages;
            }
...
*/
{
    ...
    if (TESTSYNCONLYMESSAGE(pmsg->message, pmsg->wParam)) {
         ...
         */
        if (!(PtiCurrent()->TIF_flags & TIF_16BIT)) {
            RIPERR1(ERROR_MESSAGE_SYNC_ONLY, RIP_WARNING, "xxxDispatchMessage: Sync only message 0x%lX", pmsg->message);
            return 0;
        }
            ...
            if (pmsg->message == WM_SYSTIMER) {
                ...
                for (ptmr = gptmrFirst; ptmr != NULL; ptmr = ptmr->ptmrNext) {
                    if ((pmsg->lParam == (LPARAM)ptmr->pfn) && (ptmr->flags & TMRF_SYSTEM)) {
                        ptmr->pfn(pwnd, WM_SYSTIMER, (UINT)pmsg->wParam, NtGetTickCount());
                        break;
                    }
                }
                goto Exit;
                ...
                lRet = CallClientProcA(pwnd, WM_TIMER, pmsg->wParam, NtGetTickCount(), pmsg->lParam);
                goto Exit;
            }
        }
    }
    ...
    if (TestWF(pwnd, WFSERVERSIDEPROC)) {
        ...
        fnMessageType = pmsg->message >= WM_USER ? (ULONG_PTR)SfnDWORD : (ULONG_PTR)gapfnScSendMessage[MessageTable[pmsg->message].iFunction];
        ...
        lRet = pwnd->lpfnWndProc(pwnd, pmsg->message, pmsg->wParam, pmsg->lParam);
        goto Exit;
    }
...

    {
        ...
        if (TestWF(pwnd, WFANSIPROC)) {
            ...
            lRet = CallClientProcA(pwnd, pmsg->message, pmsg->wParam, pmsg->lParam, (ULONG_PTR)lpfnWndProc);
        } else {
            lRet = CallClientProcW(pwnd, pmsg->message, pmsg->wParam, pmsg->lParam, (ULONG_PTR)lpfnWndProc);
        }
    }
     ...
     */
    if (pmsg->message == WM_PAINT && RevalidateHwnd(pmsg->hwnd) && TestWF(pwnd, WFPAINTNOTPROCESSED)) {
        //RIPMSG0(RIP_WARNING,
...
* equals the last message in the queue, then coalesce these two messages by simple deleting the last one.
...
*/
{
     ...
     */
    if (!CheckMsgFilter(message, WM_COALESCE_FIRST, WM_COALESCE_LAST) && (message != WM_TIMECHANGE))
        return;
...
/*
* Writes a message to the message queue for pwnd.  If pwnd == -1, the message is broadcasted to all windows.
...
*/
{
    ...
    if (TESTSYNCONLYMESSAGE(message, wParam)) {
        RIPERR1(ERROR_MESSAGE_SYNC_ONLY, RIP_WARNING, "Invalid parameter \"message\" (%ld) to _PostMessage", message);
        return FALSE;
    }
...
BOOL _PostThreadMessage(PTHREADINFO pti, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((pti == NULL) || !(pti->TIF_flags & TIF_GUITHREADINITIALIZED) || (pti->TIF_flags & TIF_INCLEANUP))
    {
     ...
     */
    if (TESTSYNCONLYMESSAGE(message, wParam)) {
        RIPERR1(ERROR_MESSAGE_SYNC_ONLY, RIP_WARNING, "Invalid parameter \"message\" (%ld) to _PostThreadMessage", message);
        return FALSE;
    }
     ...
     */
    if ((pqmsg = AllocQEntry(&pti->mlPost)) == NULL) {
        RIPMSG1(RIP_WARNING, "_PostThreadMessage: Failed to alloc Q entry: Target pti=0x%p", pti);
        return FALSE;
    }
...
VOID PostMove(PQ pq)
{
    ...
    PostInputMessage(pq, NULL, WM_MOUSEMOVE, 0, MAKELONG((SHORT)pt.x, (SHORT)pt.y), gdwMouseMoveTimeStamp, gdwMouseMoveExtraInfo);
#else
    PostInputMessage(pq, NULL, WM_MOUSEMOVE, 0, MAKELONG((SHORT)gpsi->ptCursor.x, (SHORT)gpsi->ptCursor.y), gdwMouseMoveTimeStamp, gdwMouseMoveExtraInfo);
#endif // REDIRECTION
    ...
    for (ind = 0; ind < gnLastMouseMove; ind++) {
        if (pq == gqMouseMove[ind].pq) {
            *ppt = gqMouseMove[ind].pt;
            ...
            RtlMoveMemory(&gqMouseMove[ind], &gqMouseMove[ind + 1], (gnLastMouseMove - ind - 1) * sizeof(QMOUSEMOVE));
            gnLastMouseMove--;
            return;
        }
    }
        ...
        if (gpqCursor != NULL) {
            ...
            if ((pwndOldCursor != NULL) && (PtoHq(pwndOldCursor) != PtoHq(pwnd))) {
                ...
                if (pdesk->dwDTFlags & DF_MOUSEMOVETRK) {
                    ...
                    PostEventMessage(pti, pti->pq, QEVENT_CANCELMOUSEMOVETRK, pdesk->spwndTrack, pdesk->dwDTFlags, pdesk->htEx, DF_MOUSEMOVETRK);
    ...
    ULONG_PTR dwExtraInfo)
{
     ...
     */
    if (pqmsgPrev != NULL && pqmsgPrev->msg.message == message && (message == WM_MOUSEMOVE || message == WM_MOUSEWHEEL)) {
        if (message == WM_MOUSEWHEEL) {
            ...
            if (sWheelDelta == 0) {
                if ((PQMSG)pq->idSysPeek == pqmsgPrev) {
                    RIPMSG0(RIP_VERBOSE, "Coalescing of mouse wheel messages causing "
    ...
    PQMSG pqmsg)
{
    ...
    case WM_XBUTTONUP:
        /*
         * Make sure we wake up the thread with the capture, if there is
         * one. This fixes PC Tools screen capture program, which sets capture and then loops trying to remove messages from the queue.
         */
        if (pq->spwndCapture != NULL && message >= WM_MOUSEFIRST && message <= WM_MOUSELAST)
            ptiT = GETPTI(pq->spwndCapture);
        else
            ptiT = pq->ptiMouse;
    ...
    if (pq->mlInput.cMsgs == 0) {
...
#if DBG
    else if ((!pq->ptiKeyboard || !(pq->ptiKeyboard->pcti->fsWakeBits & QS_KEY)) && (!pq->ptiMouse    || !(pq->ptiMouse->pcti->fsWakeBits & QS_MOUSEBUTTON))) {
        /*
        ...
        for (pqmsgT = pq->mlInput.pqmsgRead; pqmsgT; pqmsgT = pqmsgT->pqmsgNext) {
            if (pqmsgT->msg.message >= WM_KEYFIRST && pqmsgT->msg.message <= WM_KEYLAST) {
                TAGMSG1(DBGTAG_InputWithoutQS, "PostUpdateKeyStateEvent() pushing in front of a keystroke: Q %#p", pq);
            } else if (pqmsgT->msg.message >= WM_LBUTTONDOWN && pqmsgT->msg.message <= WM_XBUTTONDBLCLK) {
                TAGMSG1(DBGTAG_InputWithoutQS, "PostUpdateKeyStateEvent() pushing in front of a mousebutton: Q %#p", pq);
            }
        }
    }
#endif
...
#error "CBKEYSTATERECENTDOWN assumed to be an integral number of DWORDs"
    ...
    /*
     * Make a copy of the async key state buffer, point to it, and add an event to the end of the input queue.
     */
...
BOOL CheckOnTop(PTHREADINFO pti, PWND pwndTop, UINT message)
{
    ...
    switch (message) {
        ...
        case WM_MBUTTONDOWN:
        case WM_XBUTTONDOWN:
            if (   TestWF(pwndTop, WEFTOPMOST) || (_GetWindow(pwndTop, GW_HWNDPREV) != CalcForegroundInsertAfter(pwndTop))) {
                 return xxxSetWindowPos(pwndTop, PWND_TOP, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
             }
             break;
    }
...
PWND GetActiveTrackPwnd(PWND pwnd, Q **ppq)
{
    ...
    if ((pq == gpqForeground) && ((pwndActivate == pq->spwndActive) || IsModelessMenuNotificationWindow(pwndActivate))) {
        return NULL;
    }
...
int xxxActiveWindowTracking(PWND pwnd, UINT uMsg, int iHitTest)
{
     ...
     */
    iRet = (int)xxxSendMessage(pwndActivate, WM_MOUSEACTIVATE, (WPARAM)(HWq(pwndActivate)), MAKELONG((SHORT)iHitTest, uMsg));
    switch (iRet) {
        case MA_ACTIVATE:
        case MA_ACTIVATEANDEAT:
            if (pq == gpqForeground) {
                fSuccess = xxxActivateThisWindow(pwndActivate, 0, (TestUP(ACTIVEWNDTRKZORDER) ? 0 : ATW_NOZORDER));
            } else {
                fSuccess = xxxSetForegroundWindow2(pwndActivate, NULL, SFW_SWITCH | (TestUP(ACTIVEWNDTRKZORDER) ? 0 : SFW_NOZORDER));
            }
...
*/
{
     ...
     */
    else if ((pti->pq->spwndActive != pwnd || pti->pq->QF_flags & QF_EVENTDEACTIVATEREMOVED) && (pwndTop != PWNDDESKTOP(pwndTop))) {
        switch (message) {
        case WM_LBUTTONDOWN:
        case WM_RBUTTONDOWN:
        case WM_MBUTTONDOWN:
        case WM_XBUTTONDOWN:
             ...
             */
            result = (int)xxxSendMessage(pwnd, WM_MOUSEACTIVATE, (WPARAM)(HW(pwndTop)), MAKELONG((SHORT)ht, message));
            switch (result) {
            case 0:
            case MA_ACTIVATE:
            case MA_ACTIVATEANDEAT:
                 ...
                 */
                if ((pwndTop != pti->pq->spwndActive || pti->pq->QF_flags & QF_EVENTDEACTIVATEREMOVED) && !xxxActivateWindow(pwndTop, (UINT)((pti->pq->codeCapture == NO_CAP_CLIENT) ? AW_TRY2 : AW_TRY))) {
                    result = MA_SKIP;
...
ItsActiveJustCheckOnTop:
        ...
        if (TestUP(ACTIVEWINDOWTRACKING)) {
            if (CheckOnTop(pti, pwndTop, message)) {
                 ...
                 *  will need to change
                 */
                 if ((ht == HTCLIENT) && (GETPTI(pwndTop)->TIF_flags & TIF_CSRSSTHREAD) && !(TestWF(pwndTop, WEFTOPMOST))) {
                     RIPMSG2(RIP_WARNING, "xxxMouseActivate: Skipping msg %#lx for pwnd %#p", message, pwndTop);
                     result = MA_SKIP;
                 }
            }
        } /* if (TestUP(ACTIVEWINDOWTRACKING)) */
     ...
     */
    if (pti->pq->spwndCapture == NULL) {
        xxxSendMessage(pwnd, WM_SETCURSOR, (WPARAM)HW(pwnd), MAKELONG((SHORT)ht, message));
    }
...
*/
{
     ...
     */
    InternalSetTimer(pdesk->spwndTrack, IDSYS_MOUSEHOVER, pdesk->dwMouseHoverTime, xxxSystemTimerProc, TMRF_SYSTEM);

    SetRect(&pdesk->rcMouseHover, pt.x - gcxMouseHover / 2, pt.y - gcyMouseHover / 2, pt.x + gcxMouseHover / 2, pt.y + gcyMouseHover / 2);
...
*/
{
     ...
     */
    if ((pwnd != pdesk->spwndTrack) || (!!(lpTME->dwFlags & TME_NONCLIENT) ^ (pdesk->htEx != HTCLIENT))) {
        if ((lpTME->dwFlags & TME_LEAVE) && !(lpTME->dwFlags & TME_CANCEL)) {
            _PostMessage(pwnd, ((lpTME->dwFlags & TME_NONCLIENT) ? WM_NCMOUSELEAVE : WM_MOUSELEAVE), 0, 0);
        }
        return TRUE;
    }
...
*/
{
    if (pqmsg1->msg.hwnd    != pqmsg2->msg.hwnd || pqmsg1->msg.message != pqmsg2->msg.message)
        return FALSE;
    ...
    if (pqmsg1->pti      != pqmsg2->pti || pqmsg1->msg.time != pqmsg2->msg.time)
        return FALSE;
...
*/
{
            ...
            if (!EqualMsg(pqmsgT, pqmsg)) {
                ...
                while (pqmsgS != NULL) {
                    if (EqualMsg(pqmsgS, pqmsg)) {
                        TAGMSG2(DBGTAG_SysPeek | RIP_THERESMORE, "Deleting pqmsg %#p, pti %#p", pqmsgS, pqmsgS->pti);
                        TAGMSG4(DBGTAG_SysPeek | RIP_NONAME, "m %04lx, w %#p, l %#p, t %lx", pqmsgS->msg.message, pqmsgS->msg.hwnd, pqmsgS->msg.lParam, pqmsgS->msg.time);
                        pqmsgT = pqmsgS;
                        break;
                    }
    ...
    case WM_XBUTTONUP:
         ...
         */
    case WM_XBUTTONDOWN:
        UserAssert(GET_XBUTTON_WPARAM(pqmsg->msg.wParam) == XBUTTON1 || GET_XBUTTON_WPARAM(pqmsg->msg.wParam) == XBUTTON2);
    ...
    if ((vk == VK_SHIFT) || (vk == VK_MENU) || (vk == VK_CONTROL)) {
         ...
         */
        vkHanded = (vk - VK_SHIFT) * 2 + VK_LSHIFT + ((pqmsg->msg.lParam & EXTENDED_BIT) ? 1 : 0);
...
#if DBG
void LogPlayback(PWND pwnd, PMSG lpmsg)
/*
* History:
...
*/
{
    ...
    if ((lpmsg->message >= WM_MOUSEFIRST) && (lpmsg->message <= WM_MOUSELAST)) {
        ...
        if (pwnd != pwndM) {
            DbgPrint("*** Mouse input to window \"%ws\" of class \"%s\"\n", pwnd->strName.Length ? pwnd->strName.Buffer : L"", pwnd->pcls->lpszAnsiClassName);
            pwndM = pwnd;
        }
    ...
    DbgPrint("msg = %s, wP = %x, lP = %x\n", lpszMsg, lpmsg->wParam, lpmsg->lParam);
}
#endif  // DBG
...
#define DUMPSUBPATHTAKEN(p, x)
    ...
    UserAssert(IsWinEventNotifyDeferredOK());
    UserAssert((fsReason & ~(QS_EVENT | QS_INPUT)) == 0 && (fsReason & (QS_EVENT | QS_INPUT)) != 0);
...
ReprocessMsg:
             ...
             */
             if (!fOtherApp && (ptiCurrent->pq == gpqCursor)) {
                 ...
                 if ((message != WM_MOUSEMOVE) || fNewpwndTrack || (ptiCurrent->rpdesk->htEx != htEx)) {
                 ...
                 if (!fNewpwndTrack && (ptiCurrent->rpdesk->dwDTFlags & DF_TRACKMOUSEHOVER)) {
                     if ((message != WM_MOUSEMOVE) || !PtInRect(&ptiCurrent->rpdesk->rcMouseHover, ptScreen)) {
            ...
            case WM_MBUTTONDOWN:
            case WM_XBUTTONDOWN:
                if (TestCF(pwnd, CFDBLCLKS) || ptiCurrent->pq->codeCapture == NO_CAP_SYS || IsMenuStarted(ptiCurrent)) {
                    codeMouseDown++;
                    PATHTAKEN(0x10000000);
                    if (qmsg.msg.time <= ptiCurrent->pq->timeDblClk && (!gbClientDoubleClickSupport) && HW(pwnd) == ptiCurrent->pq->hwndDblClk && message == ptiCurrent->pq->msgDblClk &&
                            (message != WM_XBUTTONDOWN || GET_XBUTTON_WPARAM(qmsg.msg.wParam) == ptiCurrent->pq->xbtnDblClk)) {
                        RECT rcDblClk = {
...
NoLayoutSwitch:
        ...
        case WM_CHAR:
            ...
            if (gpImeHotKeyListHeader != NULL && fRemove && !IsMenuStarted(ptiCurrent) && !(ptiCurrent->TIF_flags & TIF_DISABLEIME) && pwnd != NULL) {
                 ...
                 */
                dwImmRet = xxxImmProcessKey( ptiCurrent->pq, pwnd, message, wParamTemp, lParam);
    ...
    if (giScreenSaveTimeOutMs > 0) {
        if (IsTimeFromLastInput((DWORD)(giScreenSaveTimeOutMs))) {
            if (gppiScreenSaver != NULL) {
                 ...
                 */
                if ((gpqForeground != NULL) && (gpqForeground->spwndActive != NULL) && !FHungApp(GETPTI(gpqForeground->spwndActive), CMSHUNGAPPTIMEOUT)) {
                    /*
            ...
            if (pti->pqAttach == pqAttach) {
                ...
                for (pai = gpai; pai != NULL; pai = pai->paiNext) {
                    ...
                    if (pai->pti1 == pti || pai->pti2 == pti) {
                        zzzAddAttachment((pai->pti1 == pti) ? pai->pti2 : pai->pti1, pqAttach, &fChanged);
                    }
                }
                ...
                if (pti->TIF_flags & TIF_16BIT) {
                    ...
                    for (pEntryAttach = pHeadAttach->Flink; pEntryAttach != pHeadAttach; pEntryAttach = pEntryAttach->Flink) {
                        ptiAttach = CONTAINING_RECORD(pEntryAttach, THREADINFO, PtiLink);

                        if (ptiAttach->TIF_flags & TIF_16BIT && ptiAttach->ppi == pti->ppi) {
...
*/
{
    ...
    while (pqmsgS != NULL) {
        ...
        if (ppqmsgD == NULL || ptiSave != ptiT) {
             ...
             * the message.
             */
            if (ptiT->pq->mlInput.pqmsgWriteLast != NULL && pqmsgS->msg.time >= ptiT->pq->mlInput.pqmsgWriteLast->msg.time) {
        ...
        *ppqmsgD = pqmsgT;
        ...
        if (pqmsgT == (PQMSG)(pqRedist->idSysPeek) && (pqRedist != ptiT->pq)) {
            if (ptiT->pq->idSysPeek == 0) {
                ...
                TAGMSG2(DBGTAG_SysPeek, "idSysPeek %#p already set in pq %#p", ptiT->pq->idSysPeek, ptiT->pq);
            }
#endif
            /*
             ...
             * that queue.
             */
            if (ptiT->pq->ptiSysLock == NULL && pqRedist->ptiSysLock != NULL && pqRedist->ptiSysLock->pq == ptiT->pq) {
                ...
                TAGMSG2(DBGTAG_SysPeek, "ptiSysLock %#p already set in pq %#p\n", ptiT->pq->ptiSysLock, ptiT->pq);
            }
#endif
        }
...
*/
{
    ...
    case CANCEL_ACTIVESTATE:
        ...
        QueueNotifyMessage(pwndT, WM_ACTIVATE, MAKELONG(WA_INACTIVE, TestWF(pwndT, WFMINIMIZED)), 0);
        ...
        xxxInternalEnumWindow(GETPTI(pwndT)->rpdesk->pDeskInfo->spwnd->spwndChild, (WNDENUMPROC_PWND)xxxActivateApp, (LPARAM)&aas, BWL_ENUMLIST);
    ...
    case CANCEL_CAPTURESTATE:
         ...
         */
        UserAssert((pti->pMenuState == NULL) || pti->pMenuState->fModelessMenu || pti->pMenuState->fInDoDragDrop);
...
void DBGValidateQueueStates (PDESKTOP pdesk)
{
    ...
    UserAssert((gpqForeground == NULL) || ((gpqForeground->ptiMouse->rpdesk == grpdeskRitInput) && (gpqForeground->cThreads != 0)));
    ...
    for (pEntry = pHead->Flink; pEntry != pHead; pEntry = pEntry->Flink) {
        ...
        /*
         * The queue should have a non-null cThreads excepting when it is QF_INDESTROY
         */
         ...
         * pti's
         */
        UserAssert((pti == pq->ptiMouse) || (fAttached && (pq == pq->ptiMouse->pq)));
        UserAssert(pti->rpdesk == pq->ptiMouse->rpdesk);
        UserAssert((pti == pq->ptiKeyboard) || (fAttached && (pq == pq->ptiKeyboard->pq)));
        UserAssert(pti->rpdesk == pq->ptiKeyboard->rpdesk);
        if (pq->ptiSysLock != NULL) {
            UserAssert((pti == pq->ptiSysLock) || (fAttached && (pq == pq->ptiSysLock->pq)));
        }
        /*
         * pwnd's.
         */
    ...
    UserAssert((gpqForeground == NULL) || (dwInForeground == 0) || (gpqForeground->cThreads == dwInForeground));
}
...
void zzzAttachToQueue(PTHREADINFO pti, PQ   pqAttach, PQ   pqJournal, BOOL fJoiningForeground)
{
     ...
     * Check active state.
     */
    switch (CheckTransferState(pti, pqAttach, FIELD_OFFSET(Q, spwndActive), fJoiningForeground)) {
    case CTS_CANCELOLD:
     ...
     * Check focus state.
     */
    switch (CheckTransferState(pti, pqAttach, FIELD_OFFSET(Q, spwndFocus), fJoiningForeground)) {
    case CTS_CANCELOLD:
    ...
    case CTS_DONOTHING:
         ...
         *  in modal menu mode
         */
        UserAssert((pti->pMenuState == NULL) || ExitMenuLoop(pti->pMenuState, pti->pMenuState->pGlobalPopupMenu) || pti->pMenuState->fModelessMenu || pti->pMenuState->fInDoDragDrop);
     ...
     */
    switch (CheckTransferState(pti, pqAttach, FIELD_OFFSET(Q, spwndActivePrev), fJoiningForeground)) {
    case CTS_TRANSFER:
    ...
    case CTS_CANCELOLD:
         ...
         * to be.
         */
        if (pqAttach->spwndActive && (pqAttach->spwndActivePrev && pti->pq->spwndActivePrev) && (pqAttach->spwndActive->spwndNext == pti->pq->spwndActivePrev)) {
            Lock(&pqAttach->spwndActivePrev, pti->pq->spwndActivePrev);
            Unlock(&pti->pq->spwndActivePrev);
        }
        break;
...
BOOL zzzReattachThreads(BOOL fJournalAttach)
{
    ...
    if (pqJournal != NULL) {
        ...
        if (pqJournal->cThreads == 0) {
             ...
             */
            if ((pqJournal->ptiMouse != NULL) && (pqJournal != pqJournal->ptiMouse->pq)) {
                pqJournal->cThreads++;
                zzzDestroyQueue(pqJournal, pqJournal->ptiMouse);
            }
            if ((pqJournal->ptiKeyboard != NULL) && (pqJournal != pqJournal->ptiKeyboard->pq)) {
                pqJournal->cThreads++;
    ...
    if ((gpqForeground == NULL) && (bHadAnActiveForegroundWindow))  {
        ...
        if (pwndNewForeground != NULL) {
            PostEventMessage(GETPTI(pwndNewForeground), GETPTI(pwndNewForeground)->pq, QEVENT_ACTIVATE, NULL, 0, 0, (LPARAM)HWq(pwndNewForeground));
        }
    }
    zzzSetFMouseMoved();
