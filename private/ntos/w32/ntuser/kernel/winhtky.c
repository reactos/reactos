...
UINT DWP_SetHotKey(PWND  pwnd, DWORD dwKey)
{
     ...
     */
    if (LOBYTE(dwKey) == VK_ESCAPE || LOBYTE(dwKey) == VK_SPACE || LOBYTE(dwKey) == VK_TAB || LOBYTE(dwKey) == VK_PACKET) {
        return (UINT)-1;
    }
    ...
    if ((phk = HotKeyHelper(pwnd)) == NULL) {
        ...
        if (gcHotKey >= gcHotKeyAlloc) {
            if (gcHotKeyAlloc) {
                phk = (PHOTKEYSTRUCT)UserReAllocPool((HANDLE)gpHotKeyList, gcHotKeyAlloc * sizeof(HOTKEYSTRUCT), (gcHotKey + 1) * sizeof(HOTKEYSTRUCT), TAG_HOTKEY);
                if (phk != NULL) {
                    gpHotKeyList = phk;
                ...
                phk = (PHOTKEYSTRUCT)UserAllocPool(sizeof(HOTKEYSTRUCT), TAG_HOTKEY);
                if (phk != NULL) {
                    gpHotKeyList = phk;
                    gcHotKey = 1;
                    gcHotKeyAlloc = 1;
    ...
    if (dwKey == 0) {
        ...
        if (--gcHotKey) {
            ...
            phk = (PHOTKEYSTRUCT)UserReAllocPool((HANDLE)gpHotKeyList, gcHotKeyAlloc * sizeof(HOTKEYSTRUCT), gcHotKey * sizeof(HOTKEYSTRUCT), TAG_HOTKEY);
            if (phk != NULL) {
                gpHotKeyList = phk;
                gcHotKeyAlloc = gcHotKey;
            }
