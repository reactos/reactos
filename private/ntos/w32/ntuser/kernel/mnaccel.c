...
int ItemContainingSubMenu(PMENU pmainMenu, ULONG_PTR wID)
{
    ...
    /*
     * Scan through mainMenu's items (bottom up) until an item is found that either has subMenu or an ancestor of subMenu as it's drop down menu
     */

    /*
     * Make sure this works for new apps that set IDs for popup items that aren't the same as the HMENU_16 value of the submenu.
     * Accelerators for disabled items will get generated otherwise, like in Exchange.
     */
    while (i >= 0)
    {
        ...
        else
        {
            ...
            // Go recurse through this popup and see if we have a match on one of our children.
...
int UT_FindTopLevelMenuIndex(PMENU pMenu, UINT cmd)
{
    ...
    /*
     * We want to search for the item that contains pMenuItemIsOn, unless this is a top-level item without a dropdown, in which case we want to search for cmd.
     */
    return ItemContainingSubMenu(pMenu, pMenuItemIsOn != pMenu ? (ULONG_PTR)pMenuItemIsOn : cmd);
}
        ...
        if ((i = UT_FindTopLevelMenuIndex(pMenu, cmd)) != -1) {
            ...
            if (pItem->spSubMenu != NULL) {
                *phmenuInit = PtoHq(pItem->spSubMenu);
                xxxSendMessage(pwnd, WM_INITMENUPOPUP, (WPARAM)*phmenuInit, (DWORD)i);
                if ((UINT)i >= pMenu->cItems)
                    return 0;
            ...
            /*
             * If the item was removed by the app in response to either of the above messages, pItem will be NULL.
             */
            if (pItem == NULL) {
                rgfItem = 0;
            } else {
...
int xxxTranslateAccelerator(PWND pwnd, LPACCELTABLE pat, LPMSG lpMsg)
{
    ...
    if (PtiCurrent()->spklActive && (PtiCurrent()->spklActive->spkf->pKbdTbl->fLocaleFlags & KLLF_ALTGR) && (_GetKeyState(VK_RMENU) & 0x8000)) {
        /*
    ...
    {
        flags = paccel->fVirt;
        if ( (DWORD)paccel->key != lpMsg->wParam || ((fVirt != 0) != ((flags & FVIRTKEY) != 0))) {
            goto Next;
        }
         ...
         *                   the System Menu.
         */
        if (!(rgfItem & TA_DISABLED) && !(rgfItem && TestWF(pwnd, WFICONIC) && !fSystemMenu)) {
            if (!(rgfItem != 0 && (PtiCurrent()->pq->spwndCapture != NULL || fDisabled))) {
                if (fSystemMenu) {
...
{
    if (CheckMsgFilter(message, WM_SYSKEYDOWN, WM_SYSDEADCHAR) && !(HIWORD(lParam) & SYS_ALTERNATE))
