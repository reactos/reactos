...
void xxxCalcClientRect(PWND pwnd, LPRECT lprc, BOOL fHungRedraw)
{
    ...
    // We need to clear the client border bits also. Otherwise, when the window gets really small, the client border will draw over the menu and caption.
    ...
    CopyRect(&rcTemp, lprc);// Save rect into rcTemp for easy local calculations.
    yTopOld = rcTemp.top;// Save the top so we'll know how tall the caption was
    ...
    if (!TestwndChild(pwnd) && (pMenu = pwnd->spmenu)) {
        ...
        if (!fHungRedraw) {
            ...
            rcTemp.top += xxxMenuBarCompute(pMenu, pwnd, rcTemp.top - yTopOld, cxFrame, rcTemp.right - rcTemp.left);
    ...
    // rects because the client calculations were wrong.  So we make the bottom of the client match up with the top (the bottom of the menu bar).
    fEmptyClient = FALSE;
    ...
    // signed!  So we version-switch this code, since other apps have reported the non-emptiness as an annoying bug.
    ...
    if (TestWF(pwnd, WEFCLIENTEDGE) && (rcTemp.right - rcTemp.left >= (2 * SYSMETFROMPROCESS(CXEDGE))) && (rcTemp.bottom - rcTemp.top >= (2 * SYSMETFROMPROCESS(CYEDGE))) ) {
