...
/*
* This is the server-side stub that calls to the client to call the actual hook function.
...
LRESULT xxxHkCallHook(PHOOK phk, int nCode, WPARAM wParam, LPARAM lParam)
{
     ...
     */
#ifdef REDIRECTION
    UserAssert((PtiCurrent() != gptiRit) || (phk->iHook == WH_MOUSE_LL) || (phk->iHook == WH_KEYBOARD_LL) || (phk->iHook == WH_HITTEST));
#else
    UserAssert((PtiCurrent() != gptiRit) || (phk->iHook == WH_MOUSE_LL) || (phk->iHook == WH_KEYBOARD_LL));
#endif // REDIRECTION
    ...
    case WH_CALLWNDPROCRET:
       if (phk->iHook == WH_CALLWNDPROC) {
        ...
        if (phk->iHook == WH_CALLWNDPROC) {
           nRet = ScSendMessageSMS(PW(pcwp->hwnd), pcwp->message, pcwp->wParam, pcwp->lParam, (ULONG_PTR)pfnHookProc, pfnHk, (phk->flags & HF_ANSI) ? (SCMS_FLAGS_ANSI|SCMS_FLAGS_INONLY) :  SCMS_FLAGS_INONLY, psms);
        } else {
            nRet = ScSendMessageSMS(PW(pcwpret->hwnd), pcwpret->message, pcwpret->wParam, pcwpret->lParam, (ULONG_PTR)pfnHookProc, pfnHk, (phk->flags & HF_ANSI) ? (SCMS_FLAGS_ANSI|SCMS_FLAGS_INONLY) :  SCMS_FLAGS_INONLY, psms);
        }
        /*
    ...
    case WH_CBT:
        ...
        case HCBT_CREATEWND:
             ...
             */
            nRet = fnHkINLPCBTCREATESTRUCT(MAKELONG((WORD)nCode, (WORD)phk->iHook), wParam, (LPCBT_CREATEWND)lParam, pfnHookProc, (phk->flags & HF_ANSI) ? TRUE : FALSE);
            break;
#ifdef REDIRECTION
        case HCBT_GETCURSORPOS:
             ...
             * simple.
             */
            nRet = fnHkINLPPOINT(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPPOINT)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
            break;
#endif // REDIRECTION
        case HCBT_MOVESIZE:
            /*
             ...
             * simple.
             */
            nRet = fnHkINLPRECT(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPRECT)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
            break;
        case HCBT_ACTIVATE:
            /*
             * This hook type points to a CBTACTIVATESTRUCT
             */
            nRet = fnHkINLPCBTACTIVATESTRUCT(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPCBTACTIVATESTRUCT)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
            break;
        default:
            /*
             * The rest of the cbt hooks are all dword parameters.
             */
            nRet = fnHkINDWORD(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook, &phk->flags);
            break;
        }
        break;
    ...
    case WH_GETMESSAGE:
         ...
         */
        nRet = fnHkINLPMSG(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPMSG)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook, (phk->flags & HF_ANSI) ? TRUE : FALSE, &phk->flags);
        break;
...
// MakeClientJournalPlaybackCall:
            nRet = fnHkOPTINLPEVENTMSG(MAKELONG((UINT)nCode, (UINT)phk->iHook), (WPARAM)PtoHq(phk), (LPEVENTMSG)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
#ifdef HOOKBATCH
        }
        ...
        if ((nCode == HC_GETNEXT) && (((LPEVENTMSG)lParam)->message == 0x12341234)) {
             ...
             */
            UserAssert((phk->cEventMessages == 0) || (phk->cEventMessages >= phk->iCurrentEvent));
            ...
            if (phk->aEventCache = LocalAlloc(LPTR, pEventMsg->paramL*sizeof(EVENTMSG))) {
    ...
    case WH_JOURNALRECORD:
        nRet = fnHkOPTINLPEVENTMSG(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPEVENTMSG)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
        break;
    case WH_DEBUG:
        /*
         * This takes an lpDebugHookStruct.
         */
        nRet = fnHkINLPDEBUGHOOKSTRUCT(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPDEBUGHOOKINFO)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
        break;
    case WH_KEYBOARD_LL:
        /*
         * This takes an lpKbdHookStruct.
         */
        nRet = fnHkINLPKBDLLHOOKSTRUCT(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPKBDLLHOOKSTRUCT)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
        break;
    case WH_MOUSE_LL:
        /*
         * This takes an lpMsllHookStruct.
         */
        nRet = fnHkINLPMSLLHOOKSTRUCT(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPMSLLHOOKSTRUCT)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
        break;
    case WH_MOUSE:
        /*
         ...
         */
MouseHook:
        nRet = fnHkINLPMOUSEHOOKSTRUCTEX(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPMOUSEHOOKSTRUCTEX)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook, &phk->flags);
        break;
#ifdef REDIRECTION
    case WH_HITTEST:
        /*
         ...
         */
        nRet = fnHkINLPHTHOOKSTRUCT(MAKELONG((UINT)nCode, (UINT)phk->iHook), wParam, (LPHTHOOKSTRUCT)lParam, (ULONG_PTR)pfnHookProc, ppfnClient->pfnDispatchHook);
        break;
#endif // REDIRECTION
    }
...
/*
* This gets thunked through the message thunks, so it has the format of a c/s message thunk call.
...
LRESULT fnHkINLPCWPEXSTRUCT(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam)
{
    ...
    cwp.lParam = lParam;
    cwp.psmsSender = NULL;
    return xxxCallNextHookEx(HC_ACTION, (pci->CI_flags & CI_INTERTHREAD_HOOK) != 0, (LPARAM)&cwp);
}
