...
BOOL InternalSetProp(PWND pwnd, LPWSTR pszKey, HANDLE hData, DWORD dwFlags)
{
    ...
    if (pszKey == NULL) {
        RIPERR0(ERROR_INVALID_PARAMETER, RIP_WARNING, "Invalid parameter \"pszKey\" (NULL) to InternalSetProp");
        return FALSE;
    }
...
/*
* This is a unique client/server routine - it builds a list of Props and returns it to the client.  Unique since the client doesn't know how big the list is ahead of time.
...
NTSTATUS _BuildPropList(PWND pwnd, PROPSET aPropSet[], UINT cPropMax, PUINT pcPropNeeded)
{
    ...
    {
        /*
         * if we run out of space in shared memory return STATUS_BUFFER_TOO_SMALL
         */
...
/*
* Create a property structure and link it at the head of the specified window's property list.

* History:
* 11-14-90 darrinm      Rewrote from scratch with new data structures and algorithms.
*/
PPROP CreateProp(PWND pwnd)
{
    ...
    if (pwnd->ppropList == NULL) {
        pwnd->ppropList = (PPROPLIST)DesktopAlloc(pwnd->head.rpdesk, sizeof(PROPLIST), DTAG_PROPLIST);
        if (pwnd->ppropList == NULL) {
            return NULL;
        }
        pwnd->ppropList->cEntries = 1;
    } else if (pwnd->ppropList->iFirstFree == pwnd->ppropList->cEntries) {
        ppropList = (PPROPLIST)DesktopAlloc(pwnd->head.rpdesk, sizeof(PROPLIST) + pwnd->ppropList->cEntries * sizeof(PROP), DTAG_PROPLIST);
        if (ppropList == NULL) {
            return NULL;
        }
...
* properties are not destroyed automatically; we assume the application is taking care of that itself (in its WM_DESTROY handler or similar).
...
void DeleteProperties(PWND pwnd)
{
    ...
    for (i = pwnd->ppropList->iFirstFree; i > 0; i--) {
        /*
         * Is this an internal property?  If so, free any data we allocated for it.
         */
    ...
    /*
     * All properties gone, free the property list and clear out the window's property list pointer.
     */
    DesktopFree(pwnd->head.rpdesk, pwnd->ppropList);
    pwnd->ppropList = NULL;
}