...
/*
* Description:
*   Updates the global MonitorFlags variable to reflect the union of all event types being monitored by DDEML applications.
...
* 10-28-97  FritzS      added cbEventData as a passed-in parameter.  This was done because the EVENT_PACKET may be client-side and we capture the count to keep a hostile app from changing the size after data probing.
*/
DWORD xxxCsEvent(PEVENT_PACKET pep, WORD cbEventData)
{
    ...
    {
        // Don't bother with event windows for instances who's flags indicate they're not interrested in the event.
        if (((psiiT->afCmd & pep2->EventType) && !pep2->fSense) || (!(psiiT->afCmd & pep2->EventType) && pep2->fSense)) {
            continue;
        }
        ...
        if (cTargets >= cHwndAllocated) {
            if (ahwndEvent == NULL) {
                cHwndAllocated = 8;
                ahwndEvent = (HWND *)UserAllocPoolWithQuota(sizeof(HWND) * cHwndAllocated, TAG_DDE6);
            } else {
                DWORD dwSize = cHwndAllocated * sizeof(HWND);
                HWND *ahwndEventT = ahwndEvent;

                cHwndAllocated += 8;
                ahwndEvent = (HWND *)UserReAllocPoolWithQuota(ahwndEvent, dwSize, sizeof(HWND) * cHwndAllocated, TAG_DDE7);
                if (ahwndEvent == NULL) {
...
/*
* Description:
*   Window proc for DDEML event windows.  These windows serve to get user into the proper context for callbacks to DDEML applications.
...
LRESULT xxxEventWndProc(PWND pwnd,UINT message,WPARAM wParam,LPARAM lParam)
{
    ...
    psii = HMValidateHandleNoRip((HANDLE)_GetWindowLongPtr(pwnd, GWLP_PSII), TYPE_DDEACCESS);
    if (psii == NULL) {
        goto CallDWP;
    }
...
#define pep ((PEVENT_PACKET)lParam)
        if (((psii->afCmd & pep->EventType) && pep->fSense) || (!(psii->afCmd & pep->EventType) && !pep->fSense)) {
...
/*
* Description:  Called when a hooked DDE message is sent or posted.  flags specifies the applicable MF_ flag.
*   This is called in the server side context of the sender or poster which may or may not be a DDEML process.
...
VOID xxxMessageEvent(PWND pwndTo,UINT message,WPARAM wParam,LPARAM lParam,DWORD flag,PDDEML_MSG_HOOK_DATA pdmhd)
{
    ...
    pep = (PEVENT_PACKET)UserAllocPoolWithQuota(sizeof(EVENT_PACKET) - sizeof(DWORD) + sizeof(MONMSGSTRUCT), TAG_DDE8);
    if (pep == NULL) {
        return;
    }
    pep->EventType = flag;
