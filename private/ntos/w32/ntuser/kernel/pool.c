    ...
    DWORD  dwFlags)
{
    ...
    if (gdwPoolFlags & POOL_CAPTURE_STACK) {
        ph->pTrace = ExAllocatePoolWithTag(SESSION_POOL_MASK | PagedPool, POOL_ALLOC_TRACE_SIZE * sizeof(PVOID), TAG_STACK);
        if (ph->pTrace != NULL) {
            ULONG hash;

            RtlZeroMemory(ph->pTrace, POOL_ALLOC_TRACE_SIZE * sizeof(PVOID));
            GetStackTrace(1, POOL_ALLOC_TRACE_SIZE, ph->pTrace, &hash);
        }
    } else {
        ph->pTrace = NULL;
    }
...
void InitPoolLimitations(void)
{
     ...
     */
    gpAllocFastMutex = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, sizeof(FAST_MUTEX), TAG_DEBUG);
     ...
     */
    RtlInitUnicodeString(&UnicodeString, L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\Session Manager\\SubSystems\\Pool");
...
{
    gpSectionFastMutex = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, sizeof(FAST_MUTEX), TAG_DEBUG);
