...
void xxxSendMenuSelect(PWND pwndNotify, PWND pwndMenu, PMENU pMenu, int idx)
{
    ...
    if ((idx >= 0) && (pMenu->cItems > (UINT)idx)) {
        ...
        flags   = (pItem->fType  & MFT_OLDAPI_MASK) | (pItem->fState & MFS_OLDAPI_MASK);
         ...
         * if the menu was a pop-up the pop-up menu handle was returned instead of the ID.
         * In NT we don't have enough space for 2 handles and flags so if it is a pop-up we return the pop-up index and the main Menu handle.
         */
        if (flags & MF_POPUP)
    ...
    if (FWINABLE() && pwndMenu) {
        xxxWindowEvent(EVENT_OBJECT_FOCUS, pwndMenu, ((pwndMenu != pwndNotify) ? OBJID_CLIENT : ((flags & MF_SYSMENU) ? OBJID_SYSMENU : OBJID_MENU)), idx+1, 0);
    }
}