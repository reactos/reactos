    ...
    WORD wLangId)
{
    ...
    Status = RtlFindMessage((PVOID)hModuleWin, (ULONG_PTR)RT_MESSAGETABLE, wLangId, wID, &MessageEntry);
...
/*
* Kernel-mode version of Sleep() that must have a timeout value and is not alertable.
...
*/
{
    ...
    if (gbRemoteSession) {
        if (gpRemoteBeepDevice == NULL)
            ...
            Status = ObOpenObjectByPointer(gpRemoteBeepDevice, 0, NULL, EVENT_ALL_ACCESS, NULL, KernelMode, &hBeepDevice);
    } else {
    ...
    /*
     * 0,0 is a special case used to turn off a beep.
     * Otherwise validate the dwFreq parameter to be in range.
     */
    if ((dwFreq != 0 || dwDuration != 0) && (dwFreq < (ULONG)0x25 || dwFreq > (ULONG)0x7FFF)) {
        ...
        Status = ZwDeviceIoControlFile(hBeepDevice, NULL, NULL, NULL, &IoStatus, IOCTL_BEEP_SET, &BeepParameters, sizeof(BeepParameters), NULL, 0);
    }
    ...
    /*
     * Beep device is asynchronous, so sleep for duration to allow this beep to complete.
     */
