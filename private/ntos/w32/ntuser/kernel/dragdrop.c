    ...
    PCURSOR pcur)
{
    ...
    {
        while (!(xxxPeekMessage(&msg, NULL, WM_MOUSEFIRST, WM_MOUSELAST, PM_REMOVE) || xxxPeekMessage(&msg, NULL, WM_QUEUESYNC, WM_QUEUESYNC, PM_REMOVE) || xxxPeekMessage(&msg, NULL, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE)))
        {
        ...
        if  ( (pti->pq->spwndCapture != pwndFrom) || (msg.message == WM_KEYDOWN && msg.wParam == VK_ESCAPE) )
        {
            if (pcurT = SYSCUR(NO))
                zzzSetCursor(pcurT);
            break;
         ...
         */
        if (pwndFrom) {
            xxxSendMessage(pwndFrom, WM_DRAGLOOP, (pcurT != SYSCUR(NO)), (LPARAM)lpds);
        }
        ...
        if (pwndDragging != RevalidateHwnd(lpds->hwndSink)) {
            if (pwndDragging != NULL) {
                xxxSendMessage(pwndDragging, WM_DRAGSELECT, FALSE, (LPARAM)(lpds + 1));
            }
    ...
    if (pcurT != SYSCUR(NO)) {
        ...
        if (pwndT != NULL) {
            ...
            result = (DWORD)xxxSendMessage(pwndT, WM_DROPOBJECT, (WPARAM)HW(pwndFrom), (LPARAM)lpds);
...
/*
* Determines where in the window heirarchy the "drop" takes place, and sends a message to the deepest child window first.
* If that window does not respond, we go up the heirarchy (recursively, for the moment) until we either get a window that does respond or the parent doesn't respond.
...
SendQueryDrop:
         ...
         */
        if ((PCURSOR)xxxSendMessageTimeout(pwnd, WM_QUERYDROPOBJECT, fNC, (LPARAM)lpds, SMTO_ABORTIFHUNG, 3*1000, (PLONG_PTR)&pcurT) == FALSE)
            pcurT = (PCURSOR)FALSE;
...
BOOL xxxIsDragging(PWND pwnd, POINT ptScreen, UINT uMsg)
{
    ...
    *(LPPOINT)&rc.right = ptScreen;
    ...
    while (fCheck)
    {
        while ( !(xxxPeekMessage(&msg, NULL, WM_MOUSEFIRST, WM_MOUSELAST,PM_REMOVE) || xxxPeekMessage(&msg, NULL, WM_QUEUESYNC, WM_QUEUESYNC,PM_REMOVE) || xxxPeekMessage(&msg, NULL, WM_KEYFIRST, WM_KEYLAST,PM_REMOVE)) && (pti->pq->spwndCapture == pwnd))
        {
            /*
