...
PBWL BuildHwndList(PWND pwnd, UINT flags, PTHREADINFO pti)
{
    ...
    if ((pbwl = pbwlCache) != NULL) {
         ...
         */
        pbwl = (PBWL)UserAllocPool(sizeof(BWL) + sizeof(PWND) * CHWND_BWLCREATE, TAG_WINDOWLIST);
        if (pbwl == NULL)
            return NULL;
    ...
    *pbwl->phwndNext = (HWND)1;
    ...
    if (flags & BWL_ENUMIMELAST) {
         ...
         */
        pbwl = InternalRebuildHwndListForIMEClass(pbwl, (flags & BWL_REMOVEIMECHILD) == BWL_REMOVEIMECHILD);
    }
#endif
...
BOOL ExpandWindowList(PBWL *ppbwl)
{
     ...
     */
    pbwlT = (PBWL)UserReAllocPool((HANDLE)pbwl, PtrToUlong(phwnd) + sizeof(PWND), PtrToUlong(phwnd) + (BWL_CHWNDMORE + 1) * sizeof(PWND), TAG_WINDOWLIST);
...
#ifdef OWNERLIST
/*
* Builds an hwnd list sorted by owner. Ownees go first. Shutdown uses this for WM_CLOSE messages.
    ...
    for (ppbwl = &gpbwlList; *ppbwl != NULL; ppbwl = &(*ppbwl)->pbwlNext)
    {
        if (*ppbwl == pbwl) {
            *ppbwl = pbwl->pbwlNext;
            ...
            if (pbwlCache == NULL) {
                pbwlCache = pbwl;
            } else if ((pbwl->phwndMax - pbwl->rghwnd) > (pbwlCache->phwndMax - pbwlCache->rghwnd)) {
                pbwlT = pbwlCache;
                pbwlCache = pbwl;
...
PBWL InternalRebuildHwndListForIMEClass(PBWL pbwl, BOOL fRemoveChild)
{
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++)
    {
        ...
        if (pwndIMEOwner = InternalGetIMEOwner(*phwnd, fRemoveChild)) {
            try {
                if (!fRemoveChild ||
                    (pwndIMEOwner->pcls->atomClassName == gpsi->atomSysClass[ICLS_IME] && ((PIMEWND)pwndIMEOwner)->pimeui != NULL && !ProbeAndReadStructure(((PIMEWND)pwndIMEOwner)->pimeui, IMEUI).fChildThreadDef))
                {
                    *phwndIMECur++ = *phwnd;
                }
