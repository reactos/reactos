    ...
    if (pCursorOld != pCursor) {
        ...
        if (fRedraw) {
            if (pcls->cWndReferenceCount > 1) {
                ...
                xxxInternalEnumWindow(pti->rpdesk->pDeskInfo->spwnd->spwndChild, xxxSetClassIconEnum, (LPARAM)pcls, BWL_ENUMLIST);
...
/*
*  Changes the style bits of a window.  Called from SetWindowLong().
*  This sends two messages, a changing and a changed.
*  Upon receipt of a WM_STYLECHANGING message, a window can muck with the style bits for validation purposes.
*  The WM_STYLECHANGED message is simply after the fact.
*/
LONG xxxSetWindowStyle(PWND  pwnd, int   gwl, DWORD styleNew)
{
    ...
    if (gwl == GWL_STYLE) {
         ...
         * fail the call.
         */
        if ((PpiCurrent() != GETPTI(pwnd)->ppi) && (GETFNID(pwnd) == FNID_EDIT) && (sty.styleOld & ES_PASSWORD) && !(sty.styleNew & ES_PASSWORD)) {
         ...
         */
        if ((sty.styleNew & (WS_CLIPCHILDREN | WS_CLIPSIBLINGS)) != (sty.styleOld & (WS_CLIPCHILDREN | WS_CLIPSIBLINGS))) {
            /*
         ...
         */
        if (!TestWF(pwnd, WFCHILD) && TestWF(pwnd, WFMINIMIZED) && !TestWF(pwnd, WFCPRESENT) && !(sty.styleNew & WS_MINIMIZE)) {
...
//            IncDecVisWindows(pwnd, !fWasVisWindow);
        ...
        /*
         * First, see if the app might be setting bits that it really doesn't know about.  If so, replace those bits with the current values.
         */
...
/*
*  Based on style, determines if this is considered to be "visible" by queue foreground styles.
*/
BOOL VisWindow(PWND pwnd, DWORD style)
{
