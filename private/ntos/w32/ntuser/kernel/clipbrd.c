    ...
    PWINDOWSTATION pwinsta)
{
    ...
    if (pwinsta->ptiDrawingClipboard == NULL && pwinsta->spwndClipViewer != NULL) {
        ...
        if (!(PtiCurrent()->TIF_flags & TIF_16BIT)) {
             ...
             */
            xxxSendNotifyMessage(pwinsta->spwndClipViewer, WM_DRAWCLIPBOARD, (WPARAM)HW(pwinsta->spwndClipOwner), 0L);
        } else {
            xxxSendMessage(pwinsta->spwndClipViewer, WM_DRAWCLIPBOARD, (WPARAM)HW(pwinsta->spwndClipOwner), 0L);
        }
