...
{
    if ((pwnd->cbwndExtra < DLGWINDOWEXTRA) || TestWF(pwnd, WFSERVERSIDEPROC) || (PpiCurrent() != GETPTI(pwnd)->ppi)) {
...
/*
*  Returns true if shell window is successfully set.
*  Note that we return FALSE if a shell window already exists.
*  I.E., this works on a first come, first serve basis.

*  We also do NOT allow child windows to be shell windows.
*  Other than that, it's up to the caller to size her window appropriately.

*  The pwndBkGnd is provided for the explorer shell.
*  Since the shellwnd and the window which does the drawing of background wallpapers are different, we need to provide means by which we can draw directly on the background window during hung-app drawing.
*  The pwnd and pwndBkGnd will be identical if called through the SetShellWindow() api.
*/
BOOL xxxSetShellWindow(PWND pwnd, PWND pwndBkGnd)
{
     ...
     *      (3) Not topmost
     */
    if (TestwndChild(pwnd) || (pwnd->spwndOwner != NULL) || TestWF(pwnd, WEFTOPMOST)) {
        RIPMSG0(RIP_WARNING, "xxxSetShellWindow: Invalid type of window");
        return(FALSE);
    }

    /*
     * Chicago has a totally different input model which has special code that checks for Ctrl-Esc and sends it to the shell.
     * We can get the same functionality, without totally re-writing our input model by just automatically installing the Ctrl-Esc as a hotkey for the shell window.
     * The hotkey delivery code has a special case which turns this into a WM_SYSCOMMAND message instead of a WM_HOTKEY message.

     * We don't both checking for failure.  Somebody could already have a Ctrl-Esc handler installed.
     */
    ...
    xxxSetWindowPos(pdeskinfo->spwndShell, PWND_BOTTOM, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
    return(TRUE);
}
...
PSBINFO _InitPwSB(PWND pwnd)
{
    ...
    pwnd->pSBInfo = (PSBINFO)DesktopAlloc(pwnd->head.rpdesk, sizeof(SBINFO), DTAG_SBINFO);
