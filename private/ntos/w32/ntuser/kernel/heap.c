...
NTSTATUS UserCommitDesktopMemory(PVOID pBase, PVOID *ppCommit, PSIZE_T pCommitSize)
{
    ...
    /*
     * If this is a system thread, we have no view of the desktop and must map it in. 
       Fortunately, this does not happen often.

     * We use the Thread variable because IS_SYSTEM_THREAD is a macro that multiply resolves the parameter.
     */
...
FoundIt:
        ...
        {
            RIPMSG3(RIP_ERROR,
                    "UserCommitDesktopMemory failed: pBase %#p, ppCommit %#p, pCommitSize %d",
                    pBase,
                    ppCommit, 
                    *pCommitSize);
        ...
        /*
         * Map the section into the current process and commit the first page of the section.
         */
        dwCommitOffset = (ULONG)((PBYTE)*ppCommit - (PBYTE)pBase);
        Status = CommitReadOnlyMemory(pdesk->hsectionDesktop, pCommitSize, dwCommitOffset, &dCommit);
        ...
        /*
         * 254954: If we didn't find a desktop view then map the desktop view to the current process.
         */
        if (pdv == NULL) {
            /*
             * Find the desktop that owns the section.
...
FoundTheDesktop:
            ...
            try {
                MapDesktop(ObOpenHandle, PsGetCurrentProcess(), pdesk, 0, 1);
                  ...
                  RIPMSG2(RIP_WARNING,
                          "UserCommitDesktopMemory: Could't map pdesk %#p in ppi %#p",
                          pdesk,
                          PpiCurrent());
                return STATUS_NO_MEMORY;
            }
...
NTSTATUS UserCommitSharedMemory(PVOID  pBase, PVOID *ppCommit, PSIZE_T pCommitSize)
{
    ...
    if (Process->Win32Process == NULL || ((PPROCESSINFO)Process->Win32Process)->pClientBase == NULL) {
        ...
        Status = CommitReadOnlyMemory(ghSectionShared, pCommitSize, dwCommitOffset, &dCommit);
        if (NT_SUCCESS(Status)) {
            *ppCommit = (PBYTE) *ppCommit + dCommit;
        }
    } else {
                          ...
                          PRTL_HEAP_COMMIT_ROUTINE pfnCommit)
{
    ...
    liOffset.HighPart = 0;
    Status = MmMapViewOfSection(hSection, 
                                Process,
                                &pUserBase,
                                0,
                                PAGE_SIZE,
