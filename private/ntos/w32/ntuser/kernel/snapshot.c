...
/*
* Effects: Snaps either the desktop hwnd or the active front most window. If any other window is specified, we will snap it but it will be clipped.
*/
BOOL xxxSnapWindow(PWND pwnd)
{
     ...
     */
    if (!NT_SUCCESS(ReferenceWindowStation(PsGetCurrentThread(), NULL, WINSTA_READSCREEN, &pwinsta, TRUE)) ||  pwinsta->dwWSF_Flags & WSF_NOIO) {
        return FALSE;
    }
    ...
    if (TEST_PUSIF(PUSIF_PALETTEDISPLAY)) {
        ...
        lppal = (LPLOGPALETTE)UserAllocPoolWithQuota((LONG)(sizeof(LOGPALETTE) + sizeof(PALETTEENTRY) * palsize), TAG_CLIPBOARD);

        if (lppal != NULL) {
            lppal->palVersion = 0x300;
            lppal->palNumEntries = (WORD)palsize;
            if (GreGetSystemPaletteEntries(hdcScr, 0, palsize, lppal->palPalEntry)) {
