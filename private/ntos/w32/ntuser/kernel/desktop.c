...
PVOID DesktopAlloc(PDESKTOP pdesk, UINT uSize, DWORD tag)
{
    if (pdesk->dwDTFlags & DF_DESTROYED) {
        RIPMSG2(RIP_ERROR, "DesktopAlloc: tag %d pdesk %#p is destroyed", tag, pdesk);
        return NULL;
    }
...
* While waiting for messages, it moves the mouse cursor without entering the USER critical section.
* The RIT does the rest of the mouse input processing.
    ...
    PWINDOWSTATION pwinsta)
{
     ...
     */
    Status = ObReferenceObjectByHandle(hdeskDisconnect, 0, NULL, KernelMode, &gspdeskDisconnect, NULL);
    if (!NT_SUCCESS(Status)) {
    ...
    Status = ObOpenObjectByPointer(gspdeskDisconnect, 0, NULL, EVENT_ALL_ACCESS, NULL, KernelMode, &ghDisconnectDesk);
    if (NT_SUCCESS(Status)) {
        Status = ObOpenObjectByPointer(pwinsta, 0, NULL, EVENT_ALL_ACCESS, NULL, KernelMode, &ghDisconnectWinSta);
    }
...
*/
{
    ...
    if (!NT_SUCCESS(Status)) {
        RIPNTERR0(Status, RIP_WARNING, "Can't map section for desktop heap into system space.");
        goto Error;
    }
     ...
     * Create desktop heap.
     */
    if ((pheap = UserCreateHeap(hsection, 0, pHeapBase, ulHeapSize, UserCommitDesktopMemory)) == NULL) {
    ...
    BOOL           bCreateNew)
{
     ...
     */
    Status = ObOpenObjectByPointer(pdesk, 0, NULL, EVENT_ALL_ACCESS, NULL, KernelMode, &hdesk);
    if (!NT_SUCCESS(Status)) {
        RIPMSG2(RIP_ERROR, "Could not get a handle for pdesk %#p Status %x", pdesk, Status);
        return FALSE;
    }
...
*/
{
     ...
     */
    if (pdesk != NULL && (pdesk->dwDTFlags & (DF_DESKWNDDESTROYED | DF_DYING)) && pdesk != pti->rpdesk) {
        RIPMSG2(RIP_ERROR, "Assigning pti %#p to a dying desktop %#p", pti, pdesk);
        return;
    }
     ...
     */
    if (pti->rpdesk && pti->rpdesk->dwConsoleThreadId == TIDq(pti) && pti->cWindows != 0) {
    ...
    pti->hdesk = hdesk;
    if (hdesk != NULL) {
        if (NT_SUCCESS(ObReferenceObjectByHandle(hdesk, 0, *ExDesktopObjectType, KernelMode, &pdeskRef, &ohi))) {
    ...
    /*
     * If there is no desktop, we need to fake a desktop info structure so that the IsHooked() macro can test a "valid" fsHooks value.
     * Also link the pti to the desktop.
     */
    if (pdesk != NULL) {
    ...
    if (pteb) {
        ...
        if (pdesk && (pdv = GetDesktopView(pti->ppi, pdesk))) {
            pti->pClientInfo->pDeskInfo = (PDESKTOPINFO)((PBYTE)pti->pDeskInfo - pdv->ulClientDelta);
    ...
    /*
     * Allocate thread information visible from client, then copy and free any old info we have lying around.
     */
    if (pdesk != NULL) {
        /*
         * Do not use DesktopAlloc here because the desktop might have DF_DESTROYED set.
         */
        pti->pcti = DesktopAllocAlways(pdesk, sizeof(CLIENTTHREADINFO), DTAG_CLIENTTHREADINFO);
    }
    ...
    if (pdesk == NULL || pti->pcti == NULL) {
        ...
        pti->pClientInfo->pClientThreadInfo = (PCLIENTTHREADINFO)((PBYTE)pti->pcti - pti->pClientInfo->ulClientDelta);
    }
    ...
    /*
     * If journalling is occuring on the new desktop, attach to the journal queue.
     * Assert that the pti and the pdesk point to the same deskinfo if not, we will check the wrong hooks.
     */
...
*/
{
    ...
    if (ptiCurrent->pEThread->ThreadsProcess != gpepCSRSS) {
        ...
        /*
         * If this is the first desktop assigned to the process, make it the startup desktop.
         */
    ...
    /*
     * If the desktop is changing and the thread is sharing a queue, detach the thread.
     * This will ensure that threads sharing queues are all on the same desktop.
     * This will prevent zzzDestroyQueue from getting confused and setting ptiKeyboard and ptiMouse to NULL when a thread detachs.
     */
    if (ptiCurrent->rpdesk != pdesk) {
        if (ptiCurrent->pq->cThreads > 1) {
            ...
            /*
             * This thread doesn't own any windows, still it's attached to qpgForeground and it's the only thread attached to it.
             * Since any threads attached to qpgForeground must be in grpdeskRitInput, we must set qpgForeground to NULL here because this thread is going to another desktop.
             */
...
* ZwDuplicateObject grabs ObpInitKillMutant so we have to leave our critical section.
...
*/
{
    ...
    LeaveCrit();
    Status = ZwDuplicateObject(SourceProcessHandle, SourceHandle, TargetProcessHandle, TargetHandle, DesiredAccess, HandleAttributes, Options);
    EnterCrit();
...
/*
* ObFindHandleForObject grabs ObpInitKillMutant so we have to leave our critical section.
...
*/
{
    ...
    if (pti == NULL) {
        /*
         * If the thread has a console use that desktop.
         * If not, then the thread is either invalid or not a Win32 thread.
         */
        if (hdeskConsole == NULL) {
            RIPERR1(ERROR_INVALID_PARAMETER, RIP_VERBOSE, "xxxGetThreadDesktop: invalid threadId 0x%x", dwThread);
            return NULL;
        }
    ...
    if (hdesk != NULL) {
        /*
         * If the thread belongs to this process, return the handle.
         * Otherwise, enumerate the handle table of this process to find a handle with the same attributes.
         */
        if (ppiThread != PpiCurrent()) {
            ...
            Status = ObReferenceObjectByHandle(hdesk, 0, *ExDesktopObjectType, AccessMode, &pobj, &ohi);
            KeDetachProcess();
            if (!NT_SUCCESS(Status) || !xxxUserFindHandleForObject(PsGetCurrentProcess(), pobj, NULL, &ohi, &hdesk)) {
...
* Returns the first handle to the input desktop found.
...
/*
* Close a reference to a desktop and destroy the desktop if it is no longer referenced.
...
*/
{
     ...
     */
    Status = ObReferenceObjectByHandle(hdesk, 0, *ExDesktopObjectType, AccessMode, &pdesk, NULL);
    if (!NT_SUCCESS(Status)) {
        RIPNTERR0(Status, RIP_VERBOSE, "");
        return FALSE;
    }
    ...
    if (ppi->Process != gpepCSRSS) {
        ...
        for (ptiT = ppi->ptiList; ptiT != NULL; ptiT = ptiT->ptiSibling) {
            if (ptiT->hdesk == hdesk) {
                RIPERR2(ERROR_BUSY, RIP_WARNING, "CloseDesktop: Desktop %#p still in use by thread %#p", pdesk, ptiT);
         ...
         */
         /*
          * Bug 41394. Make sure that hdesk == ppi->hdeskStartup.
          * We might be getting a handle to the desktop object that is different from ppi->hdeskStartup but we still end up setting ppi->hdeskStartup to NULL.
          */
...
/*
* Returns TRUE if the desktop handle allows other accounts to hook this process.
...
*/
{
    ...
    if ((ppi = PpiCurrent()) != NULL) {
        fRet = (Index < ppi->bmHandleFlags.SizeOfBitMap && RtlCheckBit(&ppi->bmHandleFlags, Index));
    }
...
/*
* Sets and clears the ability of a desktop handle to allow other accounts to hook this process.
...
*/
{
    ...
    EnterHandleFlagsCrit();
    fRet = ((gProcessInUse == PsGetCurrentProcess()) && (gHandleInUse == hObject));
    LeaveHandleFlagsCrit();
    return fRet;
}
...
NTSTATUS xxxResolveDesktopForWOW (IN OUT PUNICODE_STRING pstrDesktop)
{
     ...
     * Even if this is client side and we pass KernelMode to the Ob call in _OpenWindowStation this is safe because the TEB is not going to go away during this call.
     * The worst it can happen is to have the buffer in the TEB trashed.
     */
    ...
    if (gbSecureDesktop) {
        ...
        Status = ZwAllocateVirtualMemory(NtCurrentProcess(), &pObjA, 0, &cbObjA, MEM_COMMIT, PAGE_READWRITE);
        ...
        if (NT_SUCCESS(Status)) {
            ...
            try {
                *pstrStatic = strStatic;
                InitializeObjectAttributes( pObjA,pstrStatic,OBJ_CASE_INSENSITIVE,NULL,NULL);
        ...
        InitializeObjectAttributes( &ObjA,&strStatic,OBJ_CASE_INSENSITIVE,NULL,NULL);
    ...
    if (gbSecureDesktop) {
        ...
        try {
            *pstrStatic = strStatic;
            InitializeObjectAttributes( pObjA,pstrStatic,OBJ_CASE_INSENSITIVE,hwinsta,NULL);
        } except (W32ExceptionHandler(FALSE, RIP_WARNING)) {
            Status = GetExceptionCode();
        }

        if (NT_SUCCESS(Status)) {
            hdesk = xxxOpenDesktop(pObjA,UserMode,0,MAXIMUM_ALLOWED,&bShutDown);
        } else {
            hdesk = NULL;
        }
        ...
        InitializeObjectAttributes( &ObjA,&strStatic,OBJ_CASE_INSENSITIVE,hwinsta,NULL);
        hdesk = xxxOpenDesktop(&ObjA,KernelMode,0,MAXIMUM_ALLOWED,&bShutDown);
    }
...
HDESK xxxResolveDesktop(
    ...
    BOOL*           pbShutDown)
/*
* Attempts to return handles to a windowstation and desktop associated with the logon session.
...
*/
{
    ...
    /*
     * If the process already has a windowstation and a startup desktop, return them.
     */
    hwinsta = NULL;
    hwinstaDup = NULL;
    hdesk = NULL;
    ...
    if (ppi != NULL) {
        ...
        if (ppi->hwinsta != NULL && ppi->hdeskStartup != NULL) {
            /*
             * If the target process is the current process, simply return the handles.  Otherwise, open the objects.
             */
            if (Process == PsGetCurrentProcess()) {
                hwinsta = ppi->hwinsta;
                hdesk = ppi->hdeskStartup;
            } else {
                Status = ObOpenObjectByPointer(ppi->rpwinsta, 0, NULL, MAXIMUM_ALLOWED, *ExWindowStationObjectType, 
                ...
                if (!NT_SUCCESS(Status)) {
                    RIPNTERR2(Status,RIP_WARNING,"ResolveDesktop: Could not reference winsta=%#p and/or desk=%#p",ppi->rpwinsta, ppi->rpdeskStartup);
                }
            }

            RIPMSG2(RIP_VERBOSE,"ResolveDesktop: to hwinsta=%#p desktop=%#p",hwinsta, hdesk);
            ObDereferenceObject(Process);
            *phwinsta = hwinsta;
            return hdesk;
        }
    ...
    if (pstrDesktop == NULL || pstrDesktop->Length == 0) {
        ...
        USHORT cch;
        /*
         * The name be of the form windowstation\desktop.
         * Parse the string to separate out the names.
         */
        strWinSta = *pstrDesktop;
        cch = strWinSta.Length / sizeof(WCHAR);
        pszDesktop = strWinSta.Buffer;
        ...
        if (cch == 0) {
            ...
            if (!NT_SUCCESS(Status = _UserTestForWinStaAccess(&strStatic,TRUE))) {
                ...
                RIPMSG2(RIP_WARNING,"ResolveDesktop: Error (0x%X) resolving to WinSta='%ws'",Status, strStatic.Buffer);
                ObDereferenceObject(Process);
                *phwinsta = NULL;
                return NULL;
            }
    ...
    /*
     * If a windowstation has not been assigned to this process yet and there are existing windowstations, attempt an open.
     */
    if (hwinsta == NULL && grpWinStaList) {
        ...
        if (fWinStaDefaulted) {
            ...
            if (!fInteractive) {
                    ...
                    swprintf(awchName, L"Service-0x%x-%x$", luidService.HighPart, luidService.LowPart);
                    RtlInitUnicodeString(&strWinSta, awchName);
            }
        }

        /*
         * If no windowstation name was passed in and a windowstation handle was inherited, assign it.
         */
        if (fWinStaDefaulted) {
            if (xxxUserFindHandleForObject(Process, NULL, *ExWindowStationObjectType, NULL, &hwinsta)) {
                /*
                 * If the handle belongs to another process, dup it into this one
                 */
                if (Process != PsGetCurrentProcess()) {
                    Status = xxxUserDuplicateObject(hProcess,hwinsta,NtCurrentProcess(),&hwinstaDup,0,0,DUPLICATE_SAME_ACCESS);
                    if (!NT_SUCCESS(Status)) {
                        hwinsta = NULL;
                    } else {
                        hwinsta = hwinstaDup;
         ...
         */
        if (hwinsta != NULL) {
            Status = ObReferenceObjectByHandle(hwinsta,0,*ExWindowStationObjectType,KernelMode,&pwinsta,NULL);
            if (NT_SUCCESS(Status)) {
        ...
        /*
         * Only allow service logons at the console.
         * I don't think our win32k exit routines cope with more than one windowstation.
         */
        /*
         * If the open failed and the process is in a non-interactive logon session, attempt to create a windowstation and desktop for that session.
         * Note that the desktop handle will be closed after the desktop has been assigned.
         */
        if (!gbRemoteSession && NT_SUCCESS(Status) && hwinsta == NULL && !fInteractive && fWinStaDefaulted) {
            *phwinsta = xxxConnectService(&strStatic,&hdesk);
            ...
            RIPMSG2(RIP_VERBOSE, "ResolveDesktop: xxxConnectService was called\n"
                    "to hwinsta=%#p desktop=%#p", *phwinsta, hdesk);
            return hdesk;
        }
    }
    ...
    if (hwinsta != NULL) {
        /*
         * Every gui thread needs an associated desktop.
         * We'll use the default to start with and the application can override it if it wants.
         */
        if (hdesk == NULL) {
            /*
             * If no desktop name was passed in and a desktop handle was inherited, assign it.
             */
            if (fDesktopDefaulted) {
                if (xxxUserFindHandleForObject(Process, NULL, *ExDesktopObjectType, NULL, &hdesk)) {
                    /*
                    ...
                    if (Process != PsGetCurrentProcess()) {
                        ...
                        Status = xxxUserDuplicateObject(hProcess,hdesk,NtCurrentProcess(),&hdeskDup,0,0,DUPLICATE_SAME_ACCESS);
                        if (!NT_SUCCESS(Status)) {
                            CloseProtectedHandle(hdesk);
                            hdesk = NULL;
                        } else {
            ...
            if (NT_SUCCESS(Status) && hdesk == NULL) {
                ...
                if (gbSecureDesktop) {
                    ...
                    if (NT_SUCCESS(Status)) {
                        /*
                         * Note -- the string must be in client-space or the address validation in OpenDesktop will fail.
                         */
                        try {
                            *pstrStatic = strStatic;
                            InitializeObjectAttributes( pObjA,pstrStatic,OBJ_CASE_INSENSITIVE,hwinsta,NULL);
                        ...
                        if (NT_SUCCESS(Status)) {
                            hdesk = xxxOpenDesktop(pObjA,UserMode,0,MAXIMUM_ALLOWED,pbShutDown);
                        }
                    }
                } else {
                    InitializeObjectAttributes( &ObjA,&strStatic,OBJ_CASE_INSENSITIVE,hwinsta,NULL);
                    if (fInherit)
                        ObjA.Attributes |= OBJ_INHERIT;
                    hdesk = xxxOpenDesktop(&ObjA,KernelMode,0,MAXIMUM_ALLOWED,pbShutDown);
                }
            }
        }
        if (hdesk == NULL) {
    ...
    *phwinsta = hwinsta;
    ...
    RIPMSG2(RIP_VERBOSE, "ResolveDesktop: to hwinsta=%#p desktop=%#p", *phwinsta, hdesk);
    return hdesk;
}