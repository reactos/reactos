...
NTSTATUS NtUserRemoteConnect(IN PDOCONNECTDATA pDoConnectData, IN ULONG DisplayDriverNameLength, IN PWCHAR DisplayDriverName )
{
    ...
    retval = RemoteConnect(pDoConnectData, DisplayDriverNameLength, DisplayDriverName);
...
NTSTATUS NtUserRemoteRedrawRectangle(IN WORD Left, IN WORD Top, IN WORD Right, IN WORD Bottom )
{
    BEGINRECVCSRSS(NTSTATUS, STATUS_UNSUCCESSFUL);
    if (!ISTS()) {
        errret = STATUS_ACCESS_DENIED;
...
NTSTATUS NtUserRemoteRedrawScreen(VOID)
{
    BEGINRECVCSRSS(NTSTATUS, STATUS_UNSUCCESSFUL);
    if (!ISTS()) {
        errret = STATUS_ACCESS_DENIED;
...
NTSTATUS NtUserRemoteStopScreenUpdates(VOID)
{
    BEGINRECVCSRSS(NTSTATUS, STATUS_UNSUCCESSFUL);
    if (!ISTS()) {
        errret = STATUS_ACCESS_DENIED;
...
NTSTATUS NtUserCtxDisplayIOCtl(IN ULONG  DisplayIOCtlFlags, IN PUCHAR pDisplayIOCtlData, IN ULONG  cbDisplayIOCtlData)
{
    BEGINRECVCSRSS(NTSTATUS, STATUS_UNSUCCESSFUL);
    if (!ISTS()) {
        errret = STATUS_ACCESS_DENIED;
    ...
    retval = CtxDisplayIOCtl(DisplayIOCtlFlags, pDisplayIOCtlData, cbDisplayIOCtlData);
...
UINT NtUserHardErrorControl(IN HARDERRORCONTROL dwCmd, IN HANDLE handle, OUT PDESKRESTOREDATA pdrdRestore OPTIONAL)
{
    ...
    OUT OPTIONAL LPDWORD pnLengthNeeded)
{
    ...
    retval = _GetUserObjectInformation(hObject, nIndex, pvInfo, nLength, &dwLocalLength);
...
DWORD NtUserGetGuiResources(HANDLE hProcess, DWORD dwFlags)
{
    PEPROCESS Process;
    ...
    if (hProcess == NtCurrentProcess()) {
        ...
        NTSTATUS Status;
        Status = ObReferenceObjectByHandle(hProcess, PROCESS_QUERY_INFORMATION, *PsProcessType, UserMode, &Process, NULL);
        if (!NT_SUCCESS(Status)) {
...
HHOOK NtUserSetWindowsHookEx(IN HANDLE hmod, IN PUNICODE_STRING pstrLib OPTIONAL, IN DWORD idThread, IN int nFilterType, IN PROC pfnFilterProc, IN DWORD dwFlags)
{
     ...
     */
    retval = (HHOOK)zzzSetWindowsHookEx(hmod, pstrLib, ptiThread, nFilterType, pfnFilterProc, dwFlags);
    retval = PtoH((PVOID)retval);
    ...
    IN LPARAM lParam OPTIONAL)
{
    ...
    retval = xxxSendNotifyMessage(pwnd, Msg, wParam, lParam );
    if (pwnd != PWND_BROADCAST)
        ThreadUnlock(&tlpwnd);
    ...
    IN ULONG_PTR dwData)
{
    ...
    retval = xxxSendMessageCallback(pwnd, wMsg, wParam, lParam, lpResultCallBack, dwData, TRUE );
    if (pwnd != PWND_BROADCAST)
        ThreadUnlock(&tlpwnd);
...
UINT_PTR NtUserSetTimer(IN HWND hwnd, IN UINT_PTR nIDEvent, IN UINT wElapse, IN TIMERPROC pTimerFunc)
{
    PWND pwnd;
     ...
     */
    if (wElapse < 10) {
        RIPMSG1(RIP_WARNING, "SetTimer: timeout value was %ld set to 10", wElapse);
        wElapse = 10;
    }

    retval = _SetTimer(pwnd, nIDEvent, wElapse, (TIMERPROC_PWND)pTimerFunc);
...
BOOL NtUserUnhookWindowsHookEx(IN HHOOK hhk)
{
    PHOOK phk;
    ...
    retval = zzzUnhookWindowsHookEx(phk);
    ...
    IN DWORD dwFlags)
{
     ...
     */
    ptiCurrent = PtiCurrent();
    if (((dwStyle & (WS_CHILD | WS_POPUP)) != WS_CHILD) && (hmenu != NULL)) {
        ValidateHMENU(pmenu, hmenu);
    ...
    try {
        ...
        if (IS_PTR(pstrClassName)) {
            strClassName = ProbeAndReadLargeString(pstrClassName);
            ProbeForRead(strClassName.Buffer, strClassName.Length, sizeof(WORD));
            pstrClassName = &strClassName;
        }
        if (ARGUMENT_PRESENT(pstrWindowName)) {
            strWindowName = ProbeAndReadLargeString(pstrWindowName);
            ProbeForRead(strWindowName.Buffer, strWindowName.Length, (strWindowName.bAnsi ? sizeof(BYTE) : sizeof(WORD)));
            pstrWindowName = &strWindowName;
        }
#endif
    } except (StubExceptionHandler(TRUE)) {
    ...
    IN DWORD dwRop)
{
    ...
    retval = GreBitBlt(hdc, xDest, yDest, cxDest, cyDest, HDCBITS(), xSrc, ySrc, dwRop, 0);
    ...
    IN BOOL bAnsi)
{
    ...
    if ((msg & ~MSGFLAG_MASK) >= WM_USER) {
        retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, lParam, xParam);
    } else {
...
MESSAGECALL(OPTOUTLPDWORDOPTOUTLPDWORD)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, (WPARAM)&dwwParam, (LPARAM)&dwlParam, xParam);
...
MESSAGECALL(INOUTNEXTMENU)
{
    ...
    // N.B. This function has implicit window translation and thread locking
    //      enabled. These operations are performed in the User server API dispatcher.
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&mnm, xParam);
...
MESSAGECALL(COPYDATA)
{
    ...
    // N.B. This function has implicit window translation and thread locking enabled.
    //      These operations are performed in the User server API dispatcher.
    ...
    /*
     * !!! Data pointed to by cds.lpData must be captured in xxxInterSendMsgEx
     */
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, lParam, xParam);
...
MESSAGECALL(SENTDDEMSG)
{
    ...
    if (xpfnProc == FNID_CALLWINDOWPROC) {
        retval = CALLPROC(xpfnProc)(pwnd, msg | MSGFLAG_DDE_SPECIAL_SEND, wParam, lParam, xParam);
    } else if ((ptiCurrent->TIF_flags & TIF_16BIT) && (ptiCurrent->ptdb) && (ptiCurrent->ptdb->hTaskWow)) {
        /*
...
MESSAGECALL(DDEINIT)
{
     ...
     */
    if ((pddei = (PDDEIMP)_GetProp(pwndFrom, PROP_DDEIMP, TRUE)) == NULL) {
        ...
        pddei->qos = *pqos;
        Status = SeCreateClientSecurity(PsGetCurrentThread(), pqos, FALSE, &pddei->ClientContext);
        if (!NT_SUCCESS(Status)) {
...
MESSAGECALL(INLPCREATESTRUCT)
{
    ...
    if (ARGUMENT_PRESENT(lParam)) {
        try {
            ...
            if (bAnsi) {
                ...
                RtlInitLargeAnsiString((PLARGE_ANSI_STRING)&csex.strName, (LPSTR)csex.cs.lpszName, (UINT)-1);
                ...
                RtlInitLargeUnicodeString((PLARGE_UNICODE_STRING)&csex.strName, csex.cs.lpszName, (UINT)-1);
                if (IS_PTR(csex.cs.lpszClass)) {
                    ProbeForRead(csex.cs.lpszClass, sizeof(WCHAR), CHARALIGN);
                    RtlInitLargeUnicodeString((PLARGE_UNICODE_STRING)&csex.strClass, csex.cs.lpszClass, (UINT)-1);
                }
            }
        } except (StubExceptionHandler(FALSE)) {
            MSGERROR(0);
     ...
     */
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, lParam ? (LPARAM)&csex : 0, xParam);
...
MESSAGECALL(INLPMDICREATESTRUCT)
{
    ...
    try {
        ...
        if (bAnsi) {
            ...
            RtlInitLargeAnsiString((PLARGE_ANSI_STRING)&mdics.strTitle, (LPSTR)mdics.mdics.szTitle, (UINT)-1);
            if (IS_PTR(mdics.mdics.szClass)) {
                ProbeForRead(mdics.mdics.szClass, sizeof(CHAR), sizeof(CHAR));
                RtlInitLargeAnsiString((PLARGE_ANSI_STRING)&mdics.strClass, (LPSTR)mdics.mdics.szClass, (UINT)-1);
            } else {
                /*
                 * mdics.mdics.szClass may be Atom.
                 */
                RtlInitLargeAnsiString((PLARGE_ANSI_STRING)&mdics.strClass, NULL, 0);
            }
        } else {
            ProbeForRead(mdics.mdics.szTitle, sizeof(WCHAR), CHARALIGN);
            RtlInitLargeUnicodeString((PLARGE_UNICODE_STRING)&mdics.strTitle, mdics.mdics.szTitle, (UINT)-1);
            if (IS_PTR(mdics.mdics.szClass)) {
                ProbeForRead(mdics.mdics.szClass, sizeof(WCHAR), CHARALIGN);
                RtlInitLargeUnicodeString((PLARGE_UNICODE_STRING)&mdics.strClass, mdics.mdics.szClass, (UINT)-1);
            } else {
                /*
                 * mdics.mdics.szClass may be Atom.
                 */
                RtlInitLargeUnicodeString((PLARGE_UNICODE_STRING)&mdics.strClass, NULL, 0);
            }
        }
    } except (StubExceptionHandler(FALSE)) {
        MSGERROR(0);
     ...
     */
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&mdics, xParam);
...
MESSAGECALL(INOUTLPSCROLLINFO)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&scrollinfo, xParam);
...
MESSAGECALL(INOUTLPPOINT5)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&pt5, xParam);
...
#error LB_ERR/CB_ERR conflict
    ...
    try {
        if (bAnsi) {
            ...
            RtlInitLargeAnsiString((PLARGE_ANSI_STRING)&str, (LPSTR)lParam, (UINT)-1);
        } else {
            ProbeForRead((LPWSTR)lParam, sizeof(WCHAR), CHARALIGN);
            RtlInitLargeUnicodeString((PLARGE_UNICODE_STRING)&str, (LPWSTR)lParam, (UINT)-1);
        }
    } except (StubExceptionHandler(FALSE)) {
        MSGERROR(0);
    }
     ...
     */
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&str, xParam);
...
MESSAGECALL(INDEVICECHANGE)
{
    ...
    if (fPtr && lParam) {
        ...
        try {
            ...
            RtlCopyMemory(bfr,  (PBYTE)lParam, cbSize);
             ...
             */
                ...
                if (FIELD_OFFSET(DEV_BROADCAST_HANDLE, dbch_data) + pHandleW->dbch_nameoffset + (1+wcslen((LPWSTR)(pHandleW->dbch_data+pHandleW->dbch_nameoffset)))*sizeof(WCHAR) > cbSize) {
                    MSGERRORCLEANUP(0);
                }
                break;
            }
...
MESSAGECALL(INOUTNCCALCSIZE)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, lParamLocal, xParam);
...
MESSAGECALL(INOUTSTYLECHANGE)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&ss, xParam);
...
MESSAGECALL(INOUTLPRECT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&rc, xParam);
...
MESSAGECALL(INLPCOMPAREITEMSTRUCT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&compareitemstruct, xParam);
...
MESSAGECALL(INLPDELETEITEMSTRUCT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&deleteitemstruct, xParam);
...
MESSAGECALL(INLPHLPSTRUCT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)phlp, xParam);
...
MESSAGECALL(INLPHELPINFOSTRUCT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&helpinfo, xParam);
...
MESSAGECALL(INLPDRAWITEMSTRUCT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&drawitemstruct, xParam);
...
MESSAGECALL(INOUTLPMEASUREITEMSTRUCT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&measureitemstruct, xParam);
...
#else
        ProbeForWrite((PVOID)str.Buffer, str.MaximumLength, str.bAnsi ? sizeof(BYTE) : sizeof(WORD));
#endif
    } except (StubExceptionHandler(FALSE)) {
        MSGERROR(0);
    }
...
MESSAGECALL(OUTDWORDINDWORD)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, (WPARAM)&dw, lParam, xParam);
...
MESSAGECALL(INOUTLPWINDOWPOS)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&pos, xParam);
...
MESSAGECALL(INLBOXSTRING)
{
    ...
    if (!(pwnd->style & LBS_HASSTRINGS) && (pwnd->style & (LBS_OWNERDRAWFIXED | LBS_OWNERDRAWVARIABLE))) {
        retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, lParam, xParam);
    } else if (msg == LB_FINDSTRING) {
        retval = NtUserfnINSTRINGNULL(pwnd, msg, wParam, lParam, xParam, xpfnProc, bAnsi);
    } else {
        retval = NtUserfnINSTRING(pwnd, msg, wParam, lParam, xParam, xpfnProc, bAnsi);
    }
...
MESSAGECALL(OUTLBOXSTRING)
{
     ...
     */
    retval = NtUserfnGETDBCSTEXTLENGTHS(pwnd, LB_GETTEXTLEN, wParam, lParam, xParam, xpfnProc, /*IS_DBCS_ENABLED() &&*/ bAnsi);   // HiroYama: LATER
    if (retval == LB_ERR)
        MSGERROR(0);
    retval++;
...
MESSAGECALL(INCBOXSTRING)
{
    ...
    if (!(pwnd->style & CBS_HASSTRINGS) && (pwnd->style & (CBS_OWNERDRAWFIXED | CBS_OWNERDRAWVARIABLE))) {
        retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, lParam, xParam);
...
MESSAGECALL(OUTCBOXSTRING)
{
     ...
     * the string and has passed a pointer to a buffer of adequate size.
     * To do client/server copying of this string, we need to ahead of
     * time the size of this string. We add one character because GETTEXTLEN excludes the null terminator.
     */
    retval = NtUserfnGETDBCSTEXTLENGTHS(pwnd, CB_GETLBTEXTLEN, wParam, lParam, xParam, xpfnProc, /*IS_DBCS_ENABLED() &&*/ bAnsi);   // HiroYama: LATER
    if (retval == CB_ERR)
        MSGERROR(0);
    retval++;
...
MESSAGECALL(KERNELONLY)
{
    ...
    RIPMSG0(RIP_WARNING, "Message sent from client to kernel for a process which has only kernel side\n" );
...
MESSAGECALL(IMECONTROL)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, lData, xParam);
 ...
 * Hook stubs
*/
LRESULT NtUserfnHkINLPCBTCREATESTRUCT(IN UINT msg, IN WPARAM wParam, IN LPCBT_CREATEWND pcbt, IN BOOL bAnsi)
{
    CBT_CREATEWND cbt;
    CREATESTRUCTEX csex;
    ...
    retval = xxxCallNextHookEx(msg, wParam, (LPARAM)&cbt);
...
LRESULT NtUserfnHkINLPRECT(IN DWORD nCode, IN WPARAM wParam, IN LPRECT lParam)
{
    RECT rc;
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&rc);
...
#ifdef REDIRECTION
LRESULT NtUserfnHkINLPPOINT(IN DWORD   nCode, IN WPARAM  wParam, IN LPPOINT lParam)
{
    POINT pt;
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&pt);
...
LRESULT NtUserfnHkINLPMSG(IN int iHook, IN DWORD nCode, IN WPARAM wParam, IN LPMSG lParam)
{
    MSG msg;
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&msg);
...
LRESULT NtUserfnHkINLPDEBUGHOOKSTRUCT(IN DWORD nCode, IN WPARAM wParam, IN LPDEBUGHOOKINFO lParam)
{
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&hookinfo);
...
LRESULT NtUserfnHkOPTINLPEVENTMSG(IN DWORD nCode, IN WPARAM wParam, IN OUT LPEVENTMSGMSG lParam OPTIONAL)
{
    EVENTMSG event;
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)(lParam ? &event : NULL));
...
LRESULT NtUserfnHkINLPMOUSEHOOKSTRUCTEX(IN DWORD nCode, IN WPARAM wParam, IN LPMOUSEHOOKSTRUCTEX lParam)
{
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&mousehook);
...
LRESULT NtUserfnHkINLPKBDLLHOOKSTRUCT(IN DWORD nCode, IN WPARAM wParam, IN LPKBDLLHOOKSTRUCT lParam)
{
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&kbdhook);
...
LRESULT NtUserfnHkINLPMSLLHOOKSTRUCT(IN DWORD nCode, IN WPARAM wParam, IN LPMSLLHOOKSTRUCT lParam)
{
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&msllhook);
...
#ifdef REDIRECTION
LRESULT NtUserfnHkINLPHTHOOKSTRUCT(IN DWORD nCode, IN WPARAM wParam, IN LPHTHOOKSTRUCT lParam)
{
    HTHOOKSTRUCT hthook;
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&hthook);
...
LRESULT NtUserfnHkINLPCBTACTIVATESTRUCT(IN DWORD nCode, IN WPARAM wParam, IN LPCBTACTIVATESTRUCT lParam)
{
    ...
    retval = xxxCallNextHookEx(nCode, wParam, (LPARAM)&cbtactivate);
...
LRESULT NtUserCallNextHookEx(int nCode, WPARAM wParam, LPARAM lParam, BOOL bAnsi)
{
    BEGINRECV(LRESULT, 0);
    ...
    case WH_CBT:
        ...
        case HCBT_CREATEWND:
             ...
             */
            retval =  NtUserfnHkINLPCBTCREATESTRUCT(nCode, wParam, (LPCBT_CREATEWND)lParam, bAnsi);
            break;
#ifdef REDIRECTION
        ...
        case HCBT_ACTIVATE:
             ...
             */
            retval = NtUserfnHkINLPCBTACTIVATESTRUCT(nCode, wParam, (LPCBTACTIVATESTRUCT)lParam);
            break;
        default:
            /*
             * The rest of the cbt hooks are all dword parameters.
             */
            retval = xxxCallNextHookEx(nCode, wParam, lParam);
            break;
        }
        break;
...
#endif // REDIRECTION
    default:
        RIPMSG1(RIP_WARNING, "NtUserCallNextHookEx: Invalid hook type %x", PtiCurrent()->sphkCurrent->iHook);
        MSGERROR(0);
    }
...
HIMC NtUserCreateInputContext(IN ULONG_PTR dwClientImcData)
{
...
BOOL NtUserDestroyInputContext(IN HIMC hImc)
{
    PIMC pImc;
...
AIC_STATUS NtUserAssociateInputContext(IN HWND hwnd, IN HIMC hImc, IN DWORD dwFlag)
{
    PIMC pImc;
...
BOOL NtUserUpdateInputContext(IN HIMC hImc, IN UPDATEINPUTCONTEXTCLASS UpdateType, IN ULONG_PTR UpdateValue)
{
    PIMC pImc;
...
ULONG_PTR NtUserQueryInputContext(IN HIMC hImc, IN INPUTCONTEXTINFOCLASS InputContextInfo)
{
    PTHREADINFO ptiImc;
    PIMC pImc;
    ...
    IN IMEINFOEXCLASS SearchType)
{
    ...
    retval = GetImeInfoEx(_GetProcessWindowStation(NULL), &iiex, SearchType);
...
BOOL NtUserSetImeInfoEx(IN PIMEINFOEX piiex)
{
    IMEINFOEX iiex;
    BEGINRECV(BOOL, FALSE);
    ...
    retval = SetImeInfoEx(_GetProcessWindowStation(NULL), &iiex);
...
BOOL NtUserGetImeHotKey(IN DWORD dwID, OUT PUINT puModifiers, OUT PUINT puVKey, OUT LPHKL phkl)
{
    UINT uModifiers;
    UINT uVKey;
    HKL hkl;
...
BOOL NtUserSetImeHotKey(IN DWORD dwID, IN UINT  uModifiers, IN UINT  uVKey, IN HKL   hkl, IN DWORD dwFlags)
{
    BEGINRECV(BOOL, FALSE);
...
BOOL NtUserSetAppImeLevel(IN HWND  hwnd, IN DWORD dwLevel)
/*
...
DWORD NtUserGetAppImeLevel(IN HWND  hwnd)
/*
...
DWORD NtUserCheckImeHotKey(UINT uVKey, LPARAM lParam)
{
...
BOOL NtUserSetImeOwnerWindow(IN HWND hwndIme, IN HWND hwndFocus)
{
    PWND pwndFocus;
    ...
    if (pwndFocus != NULL) {
        ...
        if (TestCF(pwndFocus, CFIME) || pwndFocus->pcls->atomClassName == gpsi->atomSysClass[ICLS_IME]) {
        ...
        while (pwndT != NULL)
        {
            if (pwndT->pcls->atomClassName == gpsi->atomSysClass[ICLS_IME]) {
                RIPMSG0(RIP_WARNING, "The owner of focus window should not be an IME window!!");
                pwndTopLevel = NULL;
                break;
            }
...
VOID NtUserSetThreadLayoutHandles(IN HKL hklNew, IN HKL hklOld)
{
    PTHREADINFO ptiCurrent;
    PKL         pklNew;
...
VOID NtUserNotifyIMEStatus(IN HWND hwnd, IN DWORD dwOpen, IN DWORD dwConversion)
{
    BEGINRECV_HWNDLOCK_VOID(hwnd);
...
BOOL NtUserDisableThreadIme(IN DWORD dwThreadId)
{
    PTHREADINFO ptiCurrent, pti;
    ...
    IN LPARAM          dwData)
{
    ...
    retval = xxxEnumDisplayMonitors(hdc, lprc, lpfnEnum, dwData, FALSE);
...
*/
{
    ...
    if (dwQueryType == QUERYUSER_HANDLES) {
         ...
         */
        if (dwInSize & (sizeof(DWORD)-1) || dwOutSize != TYPE_CTYPES*dwInSize) {
            MSGERROR(0)
        }
        ...
        _QueryUserHandles(pdwInternalIn, dwInSize/sizeof(DWORD), (DWORD (*)[TYPE_CTYPES])pdwInternalResult);
        retval = TRUE;
...
MESSAGECALL(INOUTMENUGETOBJECT)
{
    ...
    retval = CALLPROC(xpfnProc)(pwnd, msg, wParam, (LPARAM)&mgoi, xParam);
...
*/
BOOL NtUserFlashWindowEx(  // API FlashWindowEx
    IN PFLASHWINFO pfwi)
{
    ...
    if ((pwnd = ValidateHwnd(fwiInternal.hwnd)) == NULL || fwiInternal.cbSize != sizeof(FLASHWINFO) || fwiInternal.dwFlags & ~FLASHW_VALID) {
        RIPMSG0(RIP_WARNING, "NtUserFlashWindowEx: Invalid Parameter");
        MSGERROR(ERROR_INVALID_PARAMETER);
    }
    else {
        ThreadLockAlwaysWithPti(PtiCurrent(), pwnd, &tlpwnd);
        retval = xxxFlashWindow(pwnd, MAKELONG(fwiInternal.dwFlags, fwiInternal.uCount), fwiInternal.dwTimeout);
        ThreadUnlock(&tlpwnd);
    }
    ...
    IN DWORD dwFlags)
{
    ...
    if (dwFlags & ~ULW_VALID) {
        ...
        retval = _UpdateLayeredWindow(pwnd, hdcDst, pptDst, psize, hdcSrc, pptSrc, crKey, pblend, dwFlags);
    }
...
BOOL NtUserSetLayeredWindowAttributes(IN HWND hwnd, IN COLORREF crKey, IN BYTE bAlpha, IN DWORD dwFlags)
{
    PWND pwnd;
...
/*
* Called by NtUserCallTwoParam in GetMonitorInfo to query gre about the HDev name
