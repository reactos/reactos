...
BOOL FAR BitBltSysBmp(HDC hdc, int x, int y, UINT i)
{
    ...
    bRet = GreBitBlt(hdc, x, y, pOem->cx, pOem->cy, HDCBITS(), pOem->x, pOem->y, SRCCOPY, 0);
#ifdef USE_MIRRORING
    /*
    ...
    if (HEBREW_UI_LANGID() && MIRRORED_HDC(hdc)) {
        if ((i >= OBI_HELP) && (i <= OBI_HELP_H)) {
            ...
            bRet = GreBitBlt(hdc, x, y+SYSMET(CXEDGE), pOem->cx-SYSMET(CXEDGE)*2, pOem->cy-SYSMET(CXEDGE)*2, HDCBITS(), pOem->x+SYSMET(CXEDGE), pOem->y+SYSMET(CXEDGE), SRCCOPY|NOMIRRORBITMAP, 0);
        }
    }
#endif
    return bRet;
...
void xxxDrawWindowFrame(PWND pwnd, HDC  hdc, BOOL fHungRedraw, BOOL fActive)
{
     ...
     */
    if (!IsVisible(pwnd) || (TestWF(pwnd, WFNONCPAINT) && !TestWF(pwnd, WFMENUDRAW)) || EqualRect(&pwnd->rcWindow, &pwnd->rcClient)) {
        return;
    }
...
//    if (TRUE) {
    if (pwnd->hrgnUpdate > NULL || GreGetClipBox(hdc, &rcClip, TRUE) != NULLREGION) {
        ...
        cxFrame = cyFrame = cBorders = GetWindowBorders(pwnd->style, pwnd->ExStyle, TRUE, FALSE);
        ...
        if ((TestWF(pwnd, WFBORDERMASK) != 0 || TestWF(pwnd, WEFDLGMODALFRAME)) || TestWF(pwnd, WFSIZEBOX) || TestWF(pwnd, WEFWINDOWEDGE) || TestWF(pwnd, WEFSTATICEDGE) && !TestWF(pwnd, WFNONCPAINT))
        {
            if (fHungRedraw)
                wFlags |= DC_NOSENDMSG;
            if (fActive)
        ...
        rcWindow.top += GetCaptionHeight(pwnd);// Subtract out caption if present.
        ...
        // Since scrolls don't have to use tricks to overlap the window border anymore, we don't have to worry about borders.
...
void xxxRedrawFrame(PWND pwnd)
{
     ...
     */
    xxxSetWindowPos(pwnd, NULL, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE | SWP_DRAWFRAME);
}
...
void xxxRedrawFrameAndHook(PWND pwnd)
{
    ...
    /*
     * We always want to call xxxSetWindowPos, even if invisible or iconic, because we need to make sure the WM_NCCALCSIZE message gets sent.
     */
    xxxSetWindowPos(pwnd, NULL, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE | SWP_DRAWFRAME);
