...
LRESULT SfnINLBOXSTRING(PWND pwnd, UINT msg, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam, PROC xpfn, DWORD dwSCMSFlags, PSMS psms)
{
    ...
    /*
     * See if the control is ownerdraw and does not have the LBS_HASSTRINGS style.  If so, treat lParam as a DWORD.
     */
    ...
    if (!(dw & LBS_HASSTRINGS) && (dw & (LBS_OWNERDRAWFIXED | LBS_OWNERDRAWVARIABLE))) {
        /*
...
/*
* Returns an lbox string - a string that treats lParam as a string pointer or a DWORD depending on LBS_HASSTRINGS and ownerdraw.
...
LRESULT SfnOUTLBOXSTRING(PWND pwnd, UINT msg, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam, PROC xpfn, DWORD dwSCMSFlags, PSMS psms)
{
     ...
     * See if the control is ownerdraw and does not have the LBS_HASSTRINGS style.  If so, treat lParam as a DWORD.
     */
    bNotString =  (!(dw & LBS_HASSTRINGS) && (dw & (LBS_OWNERDRAWFIXED | LBS_OWNERDRAWVARIABLE)));

    /*
     * Make this special call which'll know how to copy this string.
     */
    ThreadLock(pwnd, &tlpwnd);
    dwRet = ClientGetListboxString(pwnd, msg, wParam, (PLARGE_UNICODE_STRING)lParam, xParam, xpfn, dwSCMSFlags, bNotString, psms);
    ThreadUnlock(&tlpwnd);
    return dwRet;
}
...
/*
* Takes a lbox string - a string that treats lParam as a string pointer or a DWORD depending on CBS_HASSTRINGS and ownerdraw.
...
LRESULT SfnINCBOXSTRING(PWND pwnd, UINT msg, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam, PROC xpfn, DWORD dwSCMSFlags, PSMS psms)
{
    ...
    /*
     * See if the control is ownerdraw and does not have the CBS_HASSTRINGS style.  If so, treat lParam as a DWORD.
     */
    if (!RevalidateHwnd(HW(pwnd))) {
        return 0L;
    }
    ...
    if (!(dw & CBS_HASSTRINGS) && (dw & (CBS_OWNERDRAWFIXED | CBS_OWNERDRAWVARIABLE))) {
...
/*
* Returns an lbox string - a string that treats lParam as a string pointer or a DWORD depending on CBS_HASSTRINGS and ownerdraw.
...
LRESULT SfnOUTCBOXSTRING(PWND pwnd, UINT msg, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam, PROC xpfn, DWORD dwSCMSFlags, PSMS psms)
{
    ...
    bNotString = (!(dw & CBS_HASSTRINGS) && (dw & (CBS_OWNERDRAWFIXED | CBS_OWNERDRAWVARIABLE)));
