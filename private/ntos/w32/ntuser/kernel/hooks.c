...
/*
 * This table is used to determine whether a particular hook can be set for the system or a task, and other hook-ID specific things.
*/
...
PHOOK * DbgValidateThisHook (PHOOK phk, int iType, PTHREADINFO ptiHooked)
{
    ...
    if (phk->flags & HF_GLOBAL) {
        ...
        UserAssert((phk->ptiHooked == ptiHooked) || (abHookFlags[iType + 1] & HKF_INTERSENDABLE));
...
/*
* Make sure that the fsHook bit masks are in sync.
* If the bits are out of sync, some hook must have the HF_INCHECKWHF flag (this means the bits are being adjusted right now)
...
void InterQueueMsgCleanup (DWORD dwTimeFromLastRead)
{
    ...
    for (ppsms = &gpsmsList; *ppsms; )
    {
         ...
         */
        if (((*ppsms)->ptiSender != NULL) && ((*ppsms)->ptiReceiver != NULL) && ((*ppsms)->ptiSender->pq != (*ppsms)->ptiReceiver->pq)) {
            /*
            ...
            if (FHungApp ((*ppsms)->ptiReceiver, dwTimeFromLastRead)) {
                ...
                case WM_INPUTLANGCHANGE:
                    RIPMSG3 (RIP_WARNING, "InterQueueMsgCleanup: ptiSender:%#p ptiReceiver:%#p message:%#lx", (*ppsms)->ptiSender, (*ppsms)->ptiReceiver, (*ppsms)->message);
...
* This is the Win32 version of the SetWindowsHook() call.
* It has the same characteristics as far as return values, but only sets 'local' hooks.
* This is because we weren't provided a DLL we can load into other processes.
* Because of this WH_SYSMSGFILTER is no longer a valid hook.
* Apps will either need to call with WH_MSGFILTER or call the new API SetWindowsHookEx().
* Essentially this API is obsolete and everyone should call SetWindowsHookEx().
...
PROC zzzSetWindowsHookAW(int nFilterType, PROC pfnFilterProc, DWORD dwFlags)
{
    ...
    /*
     * If we get an error from zzzSetWindowsHookEx() then we return -1 to be compatible with older version of Windows.
     */
    if (phk == NULL) {
        return (PROC)-1;
    }
    ...
    /*
     * Handle the backwards compatibility return value cases for SetWindowsHook.
     * If this was the first hook in the chain, then return NULL, else return something non-zero.
     * HKF_NZRET is a special case where SetWindowsHook would always return something because there was a default hook installed.
     * Some apps relied on a non-zero return value in those cases.
     */
...
/*
* SetWindowsHookEx() is the updated version of SetWindowsHook().
* It allows applications to set hooks on specific threads or throughout the entire system.
* The function returns a hook handle to the application if successful and NULL if a failure occured.
...
PHOOK zzzSetWindowsHookEx(HANDLE hmod, PUNICODE_STRING pstrLib, PTHREADINFO ptiThread, int nFilterType, PROC pfnFilterProc, DWORD dwFlags)
{
    ...
    if (ptiThread == NULL) {
        ...
        if (ptiThread->rpdesk != ptiCurrent->rpdesk) {
            RIPERR0(ERROR_ACCESS_DENIED, RIP_WARNING, "Access denied to desktop in zzzSetWindowsHookEx - can't hook other desktops");
            return NULL;
        }
        ...
        if (ptiCurrent->ppi != ptiThread->ppi) {
             ...
             * If so return an error.
             * Note that this check is done for global hooks every time the hook is called.
             */
            if ((!RtlEqualLuid(&ptiThread->ppi->luidSession, &ptiCurrent->ppi->luidSession)) && !(ptiThread->TIF_flags & TIF_ALLOWOTHERACCOUNTHOOK)) {
            ...
            if ((ptiThread->TIF_flags & (TIF_CSRSSTHREAD | TIF_SYSTEMTHREAD)) && !(abHookFlags[nFilterType + 1] & HKF_INTERSENDABLE)) {
                /*
                 * Can't hook console or GUI system thread if inter-thread calling isn't implemented for this hook type.
                 */
                 RIPERR1(ERROR_HOOK_TYPE_NOT_ALLOWED, RIP_WARNING, "nFilterType (%ld) not allowed in zzzSetWindowsHookEx", nFilterType);
                 return NULL;
            }
        }
    }
    ...
    if (amDesired != DESKTOP_HOOKCONTROL && (ptiCurrent->rpdesk->rpwinstaParent->dwWSF_Flags & WSF_NOIO)) {
        RIPERR0(ERROR_REQUIRES_INTERACTIVE_WINDOWSTATION, RIP_WARNING, "Journal hooks invalid on a desktop belonging to a non-interactive WindowStation.");
        return NULL;
    }
    ...
    if (abHookFlags[nFilterType + 1] & HKF_JOURNAL) {
         ...
         * If so it's an error.
         * If this code is enabled, use PhkFirstGlobalValid instead of checking phkStart directly
         */
    ...
    /*
     * If a DLL is required for this hook, register the library with the library management routines so we can assure it's loaded into all the processes necessary.
     */
    phkNew->ihmod = -1;
    if (hmod != NULL) {
#if defined(WX86)
        ...
        /*
         * Add a dependency on this module - meaning, increment a count that simply counts the number of hooks set into this module.
         */
        if (phkNew->ihmod >= 0) {
    ...
    /*
     * Depending on whether we're setting a global or local hook, get the start of the appropriate linked-list of HOOKs.
     * Also set the HF_GLOBAL flag if it's a global hook.
     */
    if (ptiThread != NULL) {
        ...
        if (ptiThread->pClientInfo) {
            ...
            /*
             * If the thread being hooked is in another process, attach to that process so that we can access its ClientInfo.
             */
    ...
    /*
     * Initialize the HOOK structure.  Unreferenced parameters are assumed to be initialized to zero by LocalAlloc().
     */
    phkNew->iHook = nFilterType;

    /*
     * Libraries are loaded at different linear addresses in different process contexts.
     * For this reason, we need to convert the filter proc address into an offset while setting the hook, and then convert
     * it back to a real per-process function pointer when calling a hook.
     * Do this by subtracting the 'hmod' (which is a pointer to the linear and contiguous .exe header) from the function index.
     */
    ...
    *pphkStart = phkNew;
     ...
     * AFTER we set the hook - so this synchronization can be cancelled with control-esc.
     */
    if (abHookFlags[nFilterType + 1] & HKF_JOURNAL) {
        /*
    ...
    /*
     * Later 5.0 GerardoB: The old code just to check this but I think it's some left over stuff from server side days.
    .* Let's assert on it for a while Also, I added the assertions in the else's below because I reorganized the code and want to make sure we don't change behavior
     */
    UserAssert(ptiCurrent->pEThread && THREAD_TO_PROCESS(ptiCurrent->pEThread));

    /*
     * Can't allow a process that has set a global hook that works on server-side winprocs to run at background priority! Bump
     * up it's dynamic priority and mark it so it doesn't get reset.
     */
    if ((phkNew->flags & HF_GLOBAL) && (abHookFlags[nFilterType + 1] & HKF_INTERSENDABLE)) {
        ...
        if (abHookFlags[nFilterType + 1] & HKF_JOURNAL) {
             ...
             * This way the first event will always be a mouse move, which will ensure that the cursor is set properly.
             */
            zzzSetFMouseMoved();
            phkNew = ThreadUnlock(&tlphkNew);
            /*
             * If setting a journal playback hook, this process is the input provider.
             * This gives it the right to call SetForegroundWindow
             */
    ...
    /*
     * Return pointer to our internal hook structure so we know which hook to call next in CallNextHookEx().
     */
...
/*
* In the new world DefHookProc() is a bit deceptive since SetWindowsHook() isn't returning the actual address of the next hook to call, but instead a hook handle.
* CallNextHookEx() is a slightly clearer picture of what's going on so apps don't get tempted to try and call the value we return.

* As a side note we don't actually use the hook handle passed in.
* We keep track of which hooks is currently being called on a thread in the Q structure and use that.
* This is because SetWindowsHook() will sometimes
* return NULL to be compatible with the way it used to work, but even though we may be dealing with the last 'local' hook, there may be further 'global' hooks we need to call.
* PhkNext() is smart enough to jump over to the 'global' hook chain if it reaches the end of the 'local' hook chain.
...
/*
* This routine checks to see if any hooks for nFilterType exist, and clear the appropriate WHF_ in the THREADINFO and SERVERINFO.
...
/*
* This is the old version of the Unhook API.
* It does the same thing as zzzUnhookWindowsHookEx(), but takes a filter-type and filter-proc to identify which hook to unhook.
    ...
    for (phk = PhkFirstValid(ptiCurrent, nFilterType); phk != NULL; phk = PhkNextValid(phk))
    {
        ...
        if (PFNHOOK(phk) == pfnFilterProc) {
            ...
            if (GETPTI(phk) != ptiCurrent) {
                RIPERR0(ERROR_ACCESS_DENIED, RIP_WARNING, "Access denied in zzzUnhookWindowsHook: "
...
/*
* Applications call this API to 'unhook' a hook.
* First we check if someone is currently calling this hook.
* If no one is we go ahead and free the HOOK structure now.
* If someone is then we simply clear the filter-proc in the HOOK structure.
* In xxxCallHook2() we check for this and if by that time no one is calling the hook in question we free it there.
...
BOOL _CallMsgFilter(LPMSG pmsg, int nCode)
{
     ...
     */
    if (IsHooked(pti, WHF_SYSMSGFILTER) && xxxCallHook(nCode, 0, (LPARAM)pmsg, WH_SYSMSGFILTER)) {
        return TRUE;
    }
...
* It will check to see if the hook hasn't already been unhooked, and if is it will free it and keep looking until it finds a hook it can call or hits the end of the list.
* We also make sure any needed DLLs are loaded here.
* We also check to see if the HOOK was unhooked inside the call after we return.

* Note: Hooking server-side window procedures (such as the desktop and console windows) can only be done by sending the hook message to the hooking app.
* (This is because we must not load the hookproc DLL into the server process).
* The hook types this can be done with are currently WH_JOURNALRECORD, WH_JOURNALPLAYBACK, WH_KEYBOARD and WH_MOUSE : these are all marked as HKF_INTERSENDABLE.
* In order to prevent a global hooker from locking up the whole system, the hook message is sent with a timeout.
* To ensure minimal performance degradation, the hooker process is set to foreground priority, and prevented from being set back to background priority with the TIF_GLOBALHOOKER bit in hooking thread's pti->flags.
* Hooking emulated DOS apps is prevented with the TIF_DOSEMULATOR bit in the console thread: this is because these apps typically hog the CPU so much that
...
LRESULT xxxCallHook2(PHOOK phkCall, int nCode, WPARAM wParam, LPARAM lParam, LPBOOL lpbAnsiHook)
{
     ...
     */
    if (    ptiCurrent->TIF_flags & (TIF_INCLEANUP | TIF_DISABLEHOOKS) || ((ptiCurrent->rpdesk == NULL) && (phkCall->iHook != WH_MOUSE_LL))) {
    ...
    do {
        *lpbAnsiHook = phkCall->flags & HF_ANSI;
        ...
        if ((phkCall->iHook == WH_SHELL) && (ptiCurrent->TIF_flags & TIF_CSRSSTHREAD)) {
            if ((nCode == HSHELL_LANGUAGE) || (nCode == HSHELL_WINDOWACTIVATED) ||  (nCode == HSHELL_APPCOMMAND)) {
        ...
        fMustIntersend = (GETPTI(phkCall) != ptiCurrent) &&
            (
                 ...
                 * cross bit type.
                 */
                || ((GETPTI(phkCall)->TIF_flags & TIF_WOW64) != (ptiCurrent->TIF_flags & TIF_WOW64))
                 ...
                 */
                || (   ptiCurrent->TIF_flags & (TIF_CSRSSTHREAD | TIF_SYSTEMTHREAD) && GETPTI(phkCall)->ppi != ptiCurrent->ppi)
                /*
                 ...
                 */
                || (   phkCall->flags & HF_GLOBAL && !RtlEqualLuid(&GETPTI(phkCall)->ppi->luidSession, &ptiCurrent->ppi->luidSession) && !(ptiCurrent->TIF_flags & TIF_ALLOWOTHERACCOUNTHOOK) && !RtlEqualLuid(&GETPTI(phkCall)->ppi->luidSession, &luidSystem))

                /*
                 * We must intersend if the hooking thread is running in
                 * another process and is restricted.
                 */
                || (GETPTI(phkCall)->ppi != ptiCurrent->ppi && IsRestricted(GETPTI(phkCall)->pEThread)));
        ...
        if (!fMustIntersend) {
             ...
             */
            if ((phkCall->ihmod != -1) && (TESTHMODLOADED(ptiCurrent, phkCall->ihmod) == 0)) {
                BOOL bWx86KnownDll;
            ...
            /*
             * Time to call the hook! Lock it first so that it doesn't go away while we're using it.
             ...
             */
            if (((bHookFlags & HKF_LOWLEVEL) == 0) && (   (bHookFlags & HKF_JOURNAL) || !(ptiCurrent->TIF_flags & (TIF_CSRSSTHREAD | TIF_SYSTEMTHREAD)))) {
                nRet = xxxInterSendMsgEx(NULL, WM_HOOKMSG, wParam, (LPARAM)&hkmp, ptiCurrent, GETPTI(phkCall), NULL);
            } else {
                /*
                 ...
                 */
                if ((ptiCurrent->TIF_flags & TIF_DOSEMULATOR) || FHungApp(GETPTI(phkCall), CMSHUNGAPPTIMEOUT) || !xxxInterSendMsgEx(NULL, WM_HOOKMSG, wParam, (LPARAM)&hkmp, ptiCurrent, GETPTI(phkCall), &ism)) {
...
BOOL xxxCallMouseHook(UINT message, PMOUSEHOOKSTRUCTEX pmhs, BOOL fRemove)
{
     ...
     * Call the mouse hook.
     */
    if (xxxCallHook2(PhkFirstValid(PtiCurrent(), WH_MOUSE), fRemove ? HC_ACTION : HC_NOREMOVE, (DWORD)message, (LPARAM)pmhs, &bAnsiHook)) {
        return TRUE;
    }
...
void xxxCallJournalRecordHook(PQMSG pqmsg)
{
     ...
     */
    xxxCallHook2(PhkFirstGlobalValid(PtiCurrent(), WH_JOURNALRECORD), HC_ACTION, 0, (LPARAM)&emsg, &bAnsiHook);
     ...
     * If it was a mouse message patch up the mouse point.
     * If it was a WM_KEYxxx message convert the Win 3.1 compatible journal record key back into a half backed WM_KEYxxx format.
     * Only the VK and SC fields where initialized at this point.
...
TryNextEvent:
    ...
    if ((emsg.message >= WM_MOUSEFIRST) && (emsg.message <= WM_MOUSELAST)) {
        ...
        if ((LOWORD(emsg.paramL) == VK_PACKET) && (LOBYTE(emsg.paramH) == 0)) {
            ...
            /*
             * There are old ANSI apps that only fill in the byte for when they generate journal playback so we used to strip everything else off.
             * That however breaks unicode journalling; 22645
...
/*
* Free hook unlinks the HOOK structure from its hook-list and removes any hmod dependencies on this hook.  It also frees the HOOK structure.
...
VOID FreeHook(PHOOK phkFree)
{
    ...
    if (!(phkFree->flags & HF_DESTROYED)) {
        ...
        CheckWHFBits(phkFree->ptiHooked != NULL ? phkFree->ptiHooked : GETPTI(phkFree), phkFree->iHook);
#if DBG
        phkFree->flags &= ~HF_INCHECKWHF;
#endif
    }
...
/*
* Given a filter-type PhkFirstValid() returns the first hook, if any, of the specified type.
...
/*
* During 'exit-list' processing this function is called to free any hooks created on, or set for the current queue.
...
VOID FreeThreadsWindowHooks(VOID)
{
    ...
    for (iHook = WH_MIN ; iHook <= WH_MAX ; ++iHook) {
         ...
         *  ones already marked as destroyed (so don't call PhkFirstValid and PhkNextValid).
         */
        ...
        while (phk != NULL) {
            ...
            /*
             * If this is a local(thread) hook, unlink it and mark it as destroyed so we won't call it anymore.
             *  We want to do this even if not calling FreeHook; also note that
...
VOID zzzRegisterSystemThread (DWORD dwFlags, DWORD dwReserved)
{
    ...
    if (dwFlags & RST_DONTJOURNALATTACH) {
        ...
        /*
         * If we are already journaling, then this queue was already journal attached.
         * We need to unattach and reattach journaling so that we are removed from the journal attached queues.
         */
