...
/*
* Allocates a callback message from pool memory and reserves space for arguments to captured later.
...
PVOID AllocCallbackMessage(DWORD cbBaseMsg, DWORD cPointers, SIZE_T cbCapture, PBYTE pStackBuffer, BOOL fInput)
{
    ...
    if (cbCapture > CALLBACKSTACKLIMIT) {
        ...
        Status = ZwAllocateVirtualMemory(NtCurrentProcess(), &pcb->pvVirtualAddress, 0, &cbCapture, MEM_COMMIT, PAGE_READWRITE);
        if (!NT_SUCCESS(Status)) {
            RIPMSG2(RIP_WARNING, "AllocCallbackMessage: ZwAllocateVirtualMemory failed. Status:%#lx. Size:%#lx", Status, cbCapture);
            UserFreePool(pcb);
            return NULL;
        }
...
NTSTATUS CaptureCallbackData(PCAPTUREBUF pcb, PVOID pData, DWORD cbData, PVOID *ppDest)
{
    ...
    if (pcb->pvVirtualAddress)
        ...
        *ppDest = (PBYTE)(pbBuffer - (PBYTE)pcb);
        ((LPDWORD)((PBYTE)pcb + pcb->offPointers))[pcb->cCapturedPointers++] = (DWORD)((PBYTE)ppDest - (PBYTE)pcb);
    }
...
NTSTATUS CaptureAnsiCallbackData(PCAPTUREBUF pcb, PVOID pData, DWORD cbData, PVOID *ppDest)
{
    ...
    try {
         ...
         */
        if (!NT_SUCCESS(RtlUnicodeToMultiByteN((PCH)pbBuffer, IS_DBCS_ENABLED() ? cbData * DBCS_CHARSIZE : cbData, &nCharsInAnsiString, (PWCH)pData, cbData * sizeof(WCHAR)))) {
#else
        if (!NT_SUCCESS(RtlUnicodeToMultiByteN((PCH)pbBuffer, cbData, &nCharsInAnsiString, (PWCH)pData, cbData * sizeof(WCHAR)))) {
#endif // FE_SB
...
/*
* Converts ANSI to Unicode data and captures the result into a callback structure.
...
NTSTATUS CaptureUnicodeCallbackData(PCAPTUREBUF pcb, PVOID pData, DWORD cbData, PVOID *ppDest)
{
     ...
     */
    try {
        if (!NT_SUCCESS(RtlMultiByteToUnicodeN((PWCH)pbBuffer, cbData, &nCharsInUnicodeString, (PCH)pData, cbData / sizeof(WCHAR)))) {
            return STATUS_UNSUCCESSFUL;
        }
    } except (W32ExceptionHandler(FALSE, RIP_WARNING)) {
        return STATUS_ACCESS_VIOLATION;
...
/*
* Copies a callback output string to the output buffer and performs any necessary ANSI/Unicode translation.
...
* a string longer than that which is null terminated. This is a deficiency in the current implementation.
...
VOID CopyOutputString(PCALLBACKSTATUS pcbs, PLARGE_STRING pstr, UINT cchLimit, BOOL fAnsi)
{
    ...
    ProbeForRead(pcbs->pOutput, pcbs->cbOutput, fAnsi ? sizeof(BYTE) : sizeof(WORD));
    if (!pstr->bAnsi) {
        if (fAnsi) {
            cch = MBToWCS((LPSTR)pcbs->pOutput, (UINT)pcbs->retval, (LPWSTR *)&pstr->Buffer, cchLimit, FALSE);
            if (cch < cchLimit) {
                /*
        ...
        if (fAnsi) {
            ...
            cch = WCSToMB((LPWSTR)pcbs->pOutput, (UINT)pcbs->retval, (LPSTR *)&pstr->Buffer, cchLimit, FALSE);
            if (cch < cchLimit) {
                /*
                 * Add a null terminator and ensure an accurate pstr->Length
                 */
...
*/
DWORD CalcOutputStringSize(PCALLBACKSTATUS pcbs, DWORD cchText, BOOL fAnsiSender, BOOL fAnsiReceiver)
{
    ULONG cch;

    ProbeForRead(pcbs->pOutput, pcbs->cbOutput, fAnsiReceiver ? sizeof(BYTE) : sizeof(WORD));
    if (!fAnsiSender) {
        if (fAnsiReceiver) {
