...
BOOL _DestroyMenu(PMENU pMenu)
{
    ...
    /*
     * Because menus are the only objects on the desktop owned by the process and process cleanup is done after thread cleanup, this may be the last reference to the desktop.
     * We must lock the desktop before unlocking the parent desktop reference and freeing the menu to ensure that the desktop will not be freed until after the menu is freed.
     * Don't use static locks because the pti for this thread will not be valid during process cleanup.
     */
    rpdeskLock = NULL;
