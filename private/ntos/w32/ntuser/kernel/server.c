...
* Server support routines for the CSR stuff.  This basically performs the startup/initialization for USER.
*/
...
LRESULT xxxWrapSendMessageBSM(PWND  pwnd, UINT  message, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam)
{
    ...
    return xxxSendMessageBSM(pwnd, message, wParam, lParam, &bsmParams);
}
...
LRESULT xxxWrapCallWindowProc(PWND  pwnd, UINT  message, WPARAM wParam, LPARAM lParam, ULONG_PTR xParam)
{
    ...
    if (pCPD = HMValidateHandleNoRip((PVOID)xParam, TYPE_CALLPROC)) {
         ...
         */
        lRet = ScSendMessage(pwnd, message, wParam, lParam, xParam, gpsi->apfnClientA.pfnDispatchMessage, (xParam == (ULONG_PTR)gpsi->apfnClientA.pfnEditWndProc) ? SCMS_FLAGS_ANSI : 0);
    }
...
*/
VOID InitWindowMsgTable(PBYTE *ppbyte, PUINT pmax, CONST WORD *pw)
{
    UINT i;
    WORD msg;
    UINT cbTable;
...
VOID InitFunctionTables(VOID)
{
...
#if DBG
    {
         ...
         */
        for (pdw=(PULONG_PTR)&STOCID(FNID_START); (ULONG_PTR)pdw<(ULONG_PTR)(&STOCID(FNID_WNDPROCEND)); pdw++) {
            UserAssert(*pdw);
        }

        for (pdw=(PULONG_PTR)&FNID(FNID_START); (ULONG_PTR)pdw<(ULONG_PTR)(&FNID(FNID_WNDPROCEND)); pdw++) {
            UserAssert(*pdw);
        }
    }
#endif
...
VOID InitGlobalRIPFlags()
{
    ...
    for (idx=0; idx < nCount; idx++) {
        dwFlag = FastGetProfileDwordW(NULL, PMAP_WINDOWSM, aRIPFlags[idx].lpszKey, aRIPFlags[idx].dwDef);
...
NTSTATUS InitCreateSharedSection(VOID)
{
    ...
    Status = Win32CreateSection(&ghSectionShared, SECTION_ALL_ACCESS, (POBJECT_ATTRIBUTES)NULL,  &SectionSize, PAGE_EXECUTE_READWRITE, SEC_RESERVE, (HANDLE)NULL, NULL, TAG_SECTION_SHARED);
    if (!NT_SUCCESS(Status)) {
        RIPMSG1(RIP_WARNING, "MmCreateSection failed in InitCreateSharedSection with Status %x", Status);
        return Status;
    }
    ...
    if (!NT_SUCCESS(Status)) {
        RIPMSG1(RIP_WARNING, "Win32MapViewInSessionSpace failed with Status %x", Status);
     ...
     * Create shared heap.
     */
    if ((gpvSharedAlloc = UserCreateHeap(ghSectionShared, ulHandleTableSize, pHeapBase, ulHeapSize, UserCommitSharedMemory)) == NULL) {
...
BOOL InitCreateUserCrit(VOID)
{
     ...
     */
    gpresUser = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, sizeof(ERESOURCE), TAG_ERESOURCE);
    if (!gpresUser) {
    ...
    /*
     * Initialize a critical section to be used in [Un]QueueMouseEvent to protect the queue of mouse input events that the desktop thread uses to pass input on to the RIT, after having moved the cursor without obtaining gpresUser itself.
     */
    gpresMouseEventQueue = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, sizeof(ERESOURCE), TAG_ERESOURCE);
    ...
    /*
     * Initialize a critical section to protect the list of DEVICEINFO structs kept under gpDeviceInfoList.
     * This is used by the RIT when reading kbd input, the desktop thread when reading mouse input, and the PnP callback routines DeviceClassNotify() and DeviceNotify() when devices come and go.
     */
    gpresDeviceInfoList = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, sizeof(ERESOURCE), TAG_ERESOURCE);
     ...
     */
    gpHandleFlagsMutex = ExAllocatePoolWithTag(NonPagedPoolMustSucceed, sizeof(FAST_MUTEX), TAG_SYSTEM);
...
InitCreateUserCritExit:
    RIPERR0(ERROR_NOT_ENOUGH_MEMORY, RIP_ERROR, "Win32UserInit: InitCreateUserCrit failed");
...
* Create and initialize the user critical sections needed throughout the system.
...
BOOL InitCreateUserSubsystem()
{
     ...
     * user-heap size.
     */
    lpszSubSystem = UserAllocPoolWithQuota(USRINIT_SHAREDSECT_BUFF_SIZE * sizeof(WCHAR), TAG_SYSTEM);
    if (lpszSubSystem == NULL) {
        return FALSE;
    }

    if (FastGetProfileStringW(NULL, PMAP_SUBSYSTEMS, L"Windows", L"SharedSection=,3072", lpszSubSystem, USRINIT_SHAREDSECT_READ_SIZE) == 0) {
        RIPMSG0(RIP_WARNING, "UserInit: Windows subsystem definition not found");
        UserFreePool(lpszSubSystem);
        return FALSE;
    }

    /*
     * Locate the SharedSection portion of the definition and extract the second value.
     */
...
BOOL CreateSetupNameArray()
{
    ...
    dwProgNames = FastGetProfileValue(NULL, PMAP_SETUPPROGRAMNAMES, L"SetupProgramNames",NULL,NULL, 0);
    ...
    FastGetProfileValue(NULL, PMAP_SETUPPROGRAMNAMES, L"SetupProgramNames", NULL, (PBYTE)glpSetupPrograms, dwProgNames);
     ...
     */
    giSetupExe = icnt;
    gpastrSetupExe = UserAllocPoolWithQuota(giSetupExe * sizeof(UNICODE_STRING), TAG_SYSTEM);
...
NTSTATUS InitMapSharedSection(PEPROCESS Process, PUSERCONNECT pUserConnect)
{
     ...
     */
    if (Process->Win32Process == NULL || ((PPROCESSINFO)Process->Win32Process)->pClientBase == NULL) {
        SIZE_T        ViewSize;
        ...
        Status = MmMapViewOfSection(ghSectionShared, Process, &pClientBase, 0, 0, &liOffset, &ViewSize, ViewUnmap, SEC_NO_CHANGE, PAGE_EXECUTE_READ);
        if (NT_SUCCESS(Status)) {
...
int GetCharDimensions(HDC hdc, TEXTMETRIC* lptm, LPINT lpcy)
{
    ...
    if (tm.tmPitchAndFamily & TMPF_FIXED_PITCH) {
         ...
         */
        if (GreGetTextExtentW(hdc, (LPWSTR)wszAvgChars,  (sizeof(wszAvgChars) / sizeof(WCHAR)) - 1, &size, GGTE_WIN3_EXTENT)) {
...
{
    KeBugCheckEx(VIDEO_DRIVER_INIT_FAILURE, 0, 0, 0, USERCURRENTVERSION);
}
...
NTSTATUS UserInitialize(VOID)
{
    ...
    if (ISTS()) {
        if (gbRemoteSession) {
            swprintf(szWindowStationDirectory, L"%ws\\%ld%ws", SESSION_ROOT, gSessionId, WINSTA_DIR);
        } else {
    ...
    if (!NT_SUCCESS(Status)) {
        RIPMSG1(RIP_WARNING, "InitCreateObjectDirectory failed with Status %x", Status);
        goto Exit;
    }
...
BOOL SetupClassAtoms(VOID)
{
     ...
     */
    /*
     * HACK: Controls are registered on the client side so we can't fill in their atomSysClass entry the same way we do for the other classes.
     */
...
NTSTATUS Win32UserInitialize(VOID)
{
    ...
    RIPMSG3(RIP_WARNING, "*** win32k: DBCS:[%d] IME:[%d] MiddleEast:[%d]", IS_DBCS_ENABLED(), IS_IME_ENABLED(), IS_MIDEAST_ENABLED());
#endif
     ...
     */
    if (NT_SUCCESS(RtlCheckRegistryKey(RTL_REGISTRY_ABSOLUTE, L"\\Registry\\Machine\\SOFTWARE\\Microsoft\\SystemCertificates\\Root\\Certificates\\2BD63D28D7BCD0E251195AEB519243C13142EBC3")) ||
        NT_SUCCESS(RtlCheckRegistryKey(RTL_REGISTRY_ABSOLUTE, L"\\Registry\\Machine\\SOFTWARE\\Policies\\Microsoft\\SystemCertificates\\Root\\Certificates\\2BD63D28D7BCD0E251195AEB519243C13142EBC3")))
    {
     ...
     * clients.
     */
    lTemp = FastGetProfileDwordW(NULL, PMAP_WINDOWSM, L"USERProcessHandleQuota", gUserProcessHandleQuota);
     ...
     */
    lTemp = FastGetProfileDwordW(NULL, PMAP_WINDOWSM, L"USERPostMessageLimit", gUserPostMessageLimit);
    ...
    if (!gDrawVersionAlways) {
        gDrawVersionAlways = FastGetProfileDwordW(NULL, PMAP_WINDOWSM, L"DisplayVersion", 0);
    }

    gbSecureDesktop = FastGetProfileDwordW(NULL, PMAP_WINDOWSM, L"SecureDesktop", TRUE);
...
HDC UserGetDesktopDC(ULONG type, BOOL bAltType, BOOL bValidate)
{
    ...
    if (bValidate && type != DCTYPE_INFO && IS_THREAD_RESTRICTED(pti, JOB_OBJECT_UILIMIT_HANDLES)) {
        ...
        if (!ValidateHwnd(PtoH(pti->rpdesk->pDeskInfo->spwnd))) {
            RIPMSG0(RIP_WARNING, "UserGetDesktopDC fails desktop window validation");
            return NULL;
        }
    }
    ...
    if ((type != DCTYPE_DIRECT)  || (hdev != gpDispInfo->hDev) || IS_SYSTEM_THREAD(Thread) || (Thread->ThreadsProcess == gpepCSRSS)) {
        ...
        if (pdesk = PtiCurrent()->rpdesk) {
            hdc = _GetDCEx(pdesk->pDeskInfo->spwnd, NULL, DCX_WINDOW | DCX_CACHE | DCX_CREATEDC);
        } else {
            hdc = NULL;
        }
        LeaveCrit();
...
NTSTATUS NtUserInitialize(IN DWORD dwVersion, IN HANDLE  hPowerRequestEvent, IN HANDLE  hMediaRequestEvent)
{
    ...
    if (dwVersion != USERCURRENTVERSION) {
        KeBugCheckEx(WIN32K_INIT_OR_RIT_FAILURE, 0, 0, dwVersion, USERCURRENTVERSION);
    }
...
NTSTATUS NtUserProcessConnect(IN HANDLE hProcess, IN OUT PVOID pConnectInfo, IN ULONG cbConnectInfo)
{
    ...
    if (ucLocal.dwDispatchCount != gDispatchTableValues) {
        RIPMSG2(RIP_ERROR, "!!!! Client Dispatch info %lX != Server %lX\n", ucLocal.dwDispatchCount, gDispatchTableValues);
    }

    /*
     * Reference the process.
     */
    Status = ObReferenceObjectByHandle(hProcess, PROCESS_VM_OPERATION, *PsProcessType, UserMode, &Process, NULL);
    if (!NT_SUCCESS(Status))
        return Status;
    ...
    if (!NT_SUCCESS(Status)                       &&
        ...
        RIPMSG2(RIP_ERROR, "Failed to map shared data into client %x, status = %x\n", GetCurrentProcessId(), Status);
    }
...
NTSTATUS xxxUserProcessCallout(IN PW32PROCESS Process, IN BOOLEAN Initialize)
{
    ...
    if (Initialize) {
        ...
        if (Status == STATUS_SUCCESS) {
            if (Process->Process && Process->Process->Job && Process->Process->Job->UIRestrictionsClass != 0) {
...
* This function is called by the kernel mode global atom manager to get the address of the current thread's global atom table.

* Pointer to the global atom table for the current thread or NULL if unable to access it.
*/
PVOID UserGlobalAtomTableCallout(VOID)
{
    ...
    /*
     * Now handle the case where this si not a GUI thread/process but it is assigned to a job that has JOB_OBJECT_UILIMIT_GLOBALATOMS restrictions set.
     * There is no easy way to convert this thread to GUI.
     */
    if (pti == NULL && Job != NULL && (Job->UIRestrictionsClass & JOB_OBJECT_UILIMIT_GLOBALATOMS)) {
    ...
    if (NT_SUCCESS(ReferenceWindowStation(Thread, PsGetCurrentProcess()->Win32WindowStation, WINSTA_ACCESSGLOBALATOMS, &pwinsta, TRUE))) {
...
End:
    ...
    if (GlobalAtomTable == NULL) {
        RIPMSG1(RIP_WARNING, "_UserGetGlobalAtomTable: NULL Atom Table for pwinsta=0x%x", pwinsta);
    }
#endif
