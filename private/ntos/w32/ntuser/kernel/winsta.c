...
NTSTATUS xxxInitTerminal(PTERMINAL pTerm)
{
     ...
     */
    Status = ZwCreateEvent(&hEventInputReady, EVENT_ALL_ACCESS, NULL, NotificationEvent, FALSE);
     ...
     */
    Status = ZwCreateEvent(&hEventTermInit, EVENT_ALL_ACCESS, NULL, NotificationEvent, FALSE);
    if (!NT_SUCCESS(Status)) {
        ObDereferenceObject(pTerm->pEventInputReady);
        return Status;
    }
     ...
     */
    Status = CreateSystemThread((PKSTART_ROUTINE)xxxDesktopThread, pTerm, &hThreadDesktop);
    if (!NT_SUCCESS(Status)) {
        EnterCrit();
        ObDereferenceObject(pTerm->pEventInputReady);
...
Wait:
    KeWaitForSingleObject(pEventTermInit, WrUserRequest, KernelMode, FALSE, NULL);
...
NTSTATUS CreateGlobalAtomTable(PVOID* ppAtomTable)
{
    ...
    for (i = 0; i < ARRAY_SIZE(lpszStdFormats); i++) {
        Status = RtlAddAtomToAtomTable(*ppAtomTable, (PWSTR)lpszStdFormats[i], &Atom);
        if (!NT_SUCCESS(Status)) {
            RIPMSG1(RIP_WARNING, "RtlAddAtomToAtomTable failed to add atom %ws", lpszStdFormats[i]);
    ...
    UINT                uKbdInputLocale)
{
    ...
    if (!(pwinsta->dwWSF_Flags & WSF_NOIO)) {
        ...
        if (gbRemoteSession) {
            swprintf(szBaseNamedObjectDirectory, L"\\Sessions\\%ld\\BaseNamedObjects", gSessionId);
            RtlInitUnicodeString(&strName, szBaseNamedObjectDirectory);
        } else {
            RtlInitUnicodeString(&strName, L"\\BaseNamedObjects");
        }

        InitializeObjectAttributes(&obja, &strName, OBJ_CASE_INSENSITIVE, NULL, NULL);
        Status = ZwOpenDirectoryObject(&hRootDir,
                                       DIRECTORY_ALL_ACCESS & ~(DELETE | WRITE_DAC | WRITE_OWNER),
                                       &obja);
        if (NT_SUCCESS(Status)) {
            RtlInitUnicodeString(&strName, L"WinSta0_DesktopSwitch");
            InitializeObjectAttributes(&obja, &strName, OBJ_OPENIF, hRootDir, psd);
            Status = ZwCreateEvent(&hEvent, EVENT_ALL_ACCESS, &obja, NotificationEvent, FALSE);
            ZwClose(hRootDir);

            if (NT_SUCCESS(Status)) {
                Status = ObReferenceObjectByHandle(hEvent, EVENT_ALL_ACCESS, *ExEventObjectType, KernelMode, &gpEventSwitchDesktop, NULL);
                if (NT_SUCCESS(Status)) {
                    /*
                    ...
                    Status = ObOpenObjectByPointer(gpEventSwitchDesktop, 0, NULL, EVENT_ALL_ACCESS, NULL, KernelMode, &ghEventSwitchDesktop);
                    KeDetachProcess();
                }
                ZwClose(hEvent);
            }
     ...
     */
    Status = ObInsertObject(pwinsta, NULL, dwDesiredAccess, 1, &pwinsta, &hwinsta);
    if (Status == STATUS_OBJECT_NAME_EXISTS) {
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            if (Status == STATUS_ACCESS_DENIED) {
                ...
                RIPNTERR1(Status, RIP_ERROR, "Can't create security descriptor! Status = %#lx", Status);
            }
#endif
        } else {
            /*
             ...
             */
            siNew = (OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION | SACL_SECURITY_INFORMATION);
    ...
    BOOL fUseDesktop)
{
     ...
     */
    if (ppi != NULL) {
        if (!fUseDesktop || pti == NULL || pti->rpdesk == NULL || ppi->rpwinsta == pti->rpdesk->rpwinstaParent) {
            /*
             * Use the windowstation assigned to the process.
             */
            pwinsta = ppi->rpwinsta;
            if (pwinsta != NULL) {
                RETURN_IF_ACCESS_DENIED(ppi->amwinsta, amDesiredAccess, STATUS_ACCESS_DENIED);
            }
        }
    ...
    if (pwinsta == NULL) {
        if (hwinsta != NULL) {
            Status = ObReferenceObjectByHandle(hwinsta, amDesiredAccess, *ExWindowStationObjectType, KernelMode, &pwinsta, NULL);
            if (!NT_SUCCESS(Status))
                return Status;
            ObDereferenceObject(pwinsta);
        } else {
...
UINT _SetWindowStationUser(PWINDOWSTATION pwinsta, PLUID pluidUser, PSID ccxpsidUser, DWORD cbsidUser)
{
    ...
    if (GetCurrentProcessId() != gpidLogon) {
        RIPERR0(ERROR_ACCESS_DENIED, RIP_WARNING, "Access denied in _SetWindowStationUser: caller must be in the logon process");
        return FALSE;
    }
    ...
    if (ccxpsidUser != NULL) {
        ...
        if (pwinsta->psidUser == NULL) {
            RIPERR0(ERROR_OUTOFMEMORY, RIP_WARNING, "Memory allocation failed in _SetWindowStationUser");
            return FALSE;
        }
        try {
...
BOOL _LockWorkStation(VOID)
{
    ...
    _PostMessage(gspwndLogonNotify, WM_LOGONNOTIFY, LOGON_LOCKWORKSTATION, 0);

    return TRUE;
}

#ifdef LATER    // HY
BOOL _IsIoDesktop(HDESK hdesk)
{
    BOOL fRet = FALSE;
    NTSTATUS Status;
    PDESKTOP pdesk = NULL;
