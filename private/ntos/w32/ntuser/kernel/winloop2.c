...
//  probably VSlick -- which has two Tray Windows, one is unowned but off the screen....we want the owned one since its on the screen
...
//  extra grilling required for 3.1 and earlier apps before letting 'em in the tray -- trust me, you DON'T want to change this code. -- JEFFBOG 11/10/94
...
//  TRUE if the window passes all the necessary checks -- making it a window that should appear in the tray.
BOOL IsTrayWindow(PWND pwnd)
{
    if ((pwnd==NULL) || !(FDoTray() && (FCallHookTray() || FPostTray(pwnd->head.rpdesk))) || !FTopLevel(pwnd))
        return(FALSE);
...
void xxxSetTrayWindow(PDESKTOP pdesk, PWND pwnd, PMONITOR pMonitor)
{
    ...
    if ( FPostTray(pdesk)) {
        PostShellHookMessages(pMonitor->cFullScreen ? HSHELL_RUDEAPPACTIVATED : HSHELL_WINDOWACTIVATED, (LPARAM) hwnd);
    }

    if ( FCallHookTray() ) {
        xxxCallHook(HSHELL_WINDOWACTIVATED, (WPARAM) hwnd, (pMonitor->cFullScreen ? 1 : 0), WH_SHELL);
    }
}
...
/*
* Adds an app to the fullscreen list and moves the tray if it is the first fullscreen app.
...
BOOL xxxAddFullScreen(PWND pwnd, PMONITOR pMonitor)
{
    ...
    if (!TestWF(pwnd, WFFULLSCREEN) && FCallTray(pdesk))
    {
        ...
        if (pwnd && !TestWF(pwnd, WFCHILD) && pwnd->rcWindow.right == 0 && pwnd->rcWindow.left == 0 && !TestWF(pwnd, WFVISIBLE)) {
            TL tlpwnd;
...
/*
* Adds an app to the fullscreen list and moves the tray if there are no more fullscreen apps.
...
BOOL xxxRemoveFullScreen(PWND pwnd, PMONITOR pMonitor)
{
    ...
    if (TestWF(pwnd, WFFULLSCREEN) && FCallTray(pdesk)) {
        ...
        /*
         * (adams): Remove this assertion temporarily while I work on a fix for the problem.
