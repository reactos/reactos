...
/*
 * This is used in several callback routines to the lpk(s).
 * We can't pass G_TERM(pDispInfo)->hdcGray or HDCBITS() to the client since they are public DCs.
 * We can't just change the owner since we're about to leave the critical section.
 * Some other thread may enter before we return and use hdcGray or HDCBITS().
 * Instead, we create a compatible dc with the same font and bitmap that are currently selected in hdcGray (or HDCBITS()).
 * Pass that to the client lpk.

 * If the function returns successfully , then the dc and bitmap object are guaranteed to be successfully created.

 * History:
 * Dec-16-1997  Samer Arafeh  [samera]
 * Jan-20-1998  Samer Arafeh  [samera] Add support for both hdcGray and HDCBITS()
*/
HDC CreateCompatiblePublicDC(HDC hdcPublic, HBITMAP *pbmPublicDC)
{
    HDC     hdcCompatible = 0;
    ...
    hbmCompatible = GreCreateCompatibleBitmap(hdcPublic, cx, cy);

    // Check whether bitmap couldn't be created or can't be set to OBJECT_OWNER_CURRENT, then fail and do necessary cleanup now!
...
/*
*  Generic state drawing routine.  Does simple drawing into same DC if normal state;  uses offscreen bitmap otherwise.
...
*  Other types are required to draw via the callback function, and are allowed to stick whatever they want in lData and wData.
...
BOOL xxxDrawState(HDC hdcDraw, HBRUSH hbrFore, LPARAM lData, int x, int y, int cx, int cy, UINT uFlags)
{
    ...
    if (uFlags & DSS_MONO) {
        ...
        GreSetTextAlign(hdcT, (oldAlign & ~(TA_RTLREADING |TA_CENTER |TA_RIGHT)) | (GreGetTextAlign(hdcDraw) & (TA_RTLREADING |TA_CENTER |TA_RIGHT)));
        /*
         * Setup font
         */
