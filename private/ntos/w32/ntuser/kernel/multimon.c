...
BOOL xxxEnumDisplayMonitors(HDC hdcPaint, LPRECT lprcPaint, MONITORENUMPROC lpfnEnum, LPARAM lData, BOOL fInternal)
{
    ...
    for (pMonitor = gpDispInfo->pMonitorFirst; pMonitor != NULL; pMonitor = pMonitor->pMonitorNext) {
        ...
        if (hdcPaint) {
            ...
            GreIntersectClipRect(hdcMonitor, rcMonitorPaint.left, rcMonitorPaint.top, rcMonitorPaint.right, rcMonitorPaint.bottom);
        } else {
            hdcMonitor = NULL;
        }
        ...
        if (fInternal) {
            fReturn = (*lpfnEnum) ((HMONITOR) pMonitor, hdcMonitor, &rcMonitorPaint, lData);
        } else {
            fReturn = xxxClientMonitorEnumProc(PtoH(pMonitor), hdcMonitor, &rcMonitorPaint, lData, lpfnEnum);
        }
...
/*
* This function doesn't keep track of the visible monitors count because it assumes that after it was called the count will be recalculated, such as the case during mode changes.
* For a final unlock the monitor will have been removed from the monitor list already, so the count doesn't need to be recalculated.
