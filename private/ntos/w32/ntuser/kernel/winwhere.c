...
PWND _ChildWindowFromPointEx(PWND  pwnd, POINT pt, UINT  uFlags)
{
    ...
    if (PtInRect(&pwnd->rcClient, pt)) {
         ...
         */
        for (pwndChild = pwnd->spwndChild; pwndChild; pwndChild = pwndChild->spwndNext) {
            /*
...
/*
* Call the speed hit test hook to give the opportunity to the hook to fake where the mouse pointer is.
...
HWND xxxWindowHitTest2(PWND  pwnd, POINT pt, int *piPos, DWORD dwHitTestFlags)
{
...
#ifdef REDIRECTION
    /*
     * If this is called when the layered window is actually trying to process the message then let it see the hit test
     */
    ...
    if (!TestWF(pwnd, WFMINIMIZED)) {
        ...
        if (PtInRect((LPRECT)&pwnd->rcClient, pt)) {
            ...
            hwndT = xxxWindowHitTest(pwnd->spwndChild, pt, piPos, dwHitTestFlags);
...
Exit:
    ...
    /*
     * If this is a RTL mirrored window, then the grip is at HTBOTTOMLEFT (in terms of screen coordinates since they are not RTL mirrored).
     */
    if (TestWF(pwnd, WEFLAYOUTRTL)) {
        htGrip = HTBOTTOMLEFT;
    }
    ...
    /*
     * if the click is in the sizebox of the window and this window itself is not sizable, return the window that will be sized by this sizebox
     */
