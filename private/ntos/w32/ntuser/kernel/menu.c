...
*/
{
     ...
     */
    GreBitBlt(hdc, 0, 0, pitem->cxItem, pitem->cyItem, gpDispInfo->hdcScreen, rc.left, rc.top, SRCCOPY, 0);
...
*/
{
     ...
     */
    if ((ptiCurrent->pMenuState == NULL) || (! ptiCurrent->pMenuState->fSetCapture)) {
        return;
    }
...
*/
{
     ...
     */
    if ((pmenu == pmenu->spwndNotify->spmenu) || (pmenu == pmenu->spwndNotify->spmenuSys)) {
        return;
    }
    /*
...
*/
{
     ...
     */
#if DBG
        if ((*pspmenu != NULL) && ((*pspmenu)->spwndNotify != NULL) && ((*pspmenu)->spwndNotify != ppopup->spwndNotify)) {
            RIPMSG3(RIP_WARNING, "LockPopupMenu: Current Menu %#p shared by %#p and %#p", *pspmenu, (*pspmenu)->spwndNotify, ppopup->spwndNotify);
        }
#endif
    ...
    if (pmenu != NULL) {
         ...
         */
#if DBG
        if ((pmenu->spwndNotify != NULL) && (pmenu->spwndNotify != ppopup->spwndNotify) && (pmenu != pmenu->spwndNotify->head.rpdesk->spmenuDialogSys)) {
            RIPMSG3(RIP_WARNING, "LockPopupMenu: New Menu %#p shared by %#p and %#p", pmenu, pmenu->spwndNotify, ppopup->spwndNotify);
        }
#endif
...
*/
{
     ...
     */
#if DBG
    if (((*pspmenu)->spwndNotify != NULL) && (ppopup->spwndNotify != NULL) && (ppopup->spwndNotify != (*pspmenu)->spwndNotify)) {
        RIPMSG3(RIP_WARNING, "UnlockPopupMenu: Menu %#p shared by %#p and %#p", *pspmenu, (*pspmenu)->spwndNotify, ppopup->spwndNotify);
    }
#endif
...
*/
{
    ...
    if (!xxxMNSetTop(ppopup, iScrollTop)) {
         ...
         */
        _SetTimer(ppopup->spwndPopupMenu, uArrow, (fSetTimer ? gpsi->dtScroll : gpsi->dtScroll / 4), NULL);
    }
...
*/
{
     ...
     *   without scrolling
     */
    if ((pMenu->rgItems == 0) || (pMenu->rgItems->cxItem != pMenu->cxMenu) || (pMenu->cyMenu + (2 * SYSMET(CYFIXEDFRAME)) <= cyMax))  {
...
/*
* Switches to the alternate popupmenu. Returns TRUE if we switch else FALSE.
...
*/
{
    ...
    if (!TestWF(ppopupmenu->spwndNotify, WFSYSMENU)) {
        ...
        pMenuState->fIsSysMenu = (ppopupmenu->spwndNotify->spmenuSys == ppopupmenu->spmenu);
    } else {
    ...
    if (FWINABLE()) {
        xxxWindowEvent(EVENT_SYSTEM_MENUEND, ppopupmenu->spwndNotify, (ppopupmenu->fIsSysMenu ? OBJID_MENU : OBJID_SYSMENU), INDEXID_CONTAINER, 0);
        xxxWindowEvent(EVENT_SYSTEM_MENUSTART, ppopupmenu->spwndNotify, (ppopupmenu->fIsSysMenu ? OBJID_SYSMENU : OBJID_MENU), INDEXID_CONTAINER, 0);
    }
...
*/
{
     ...
     */
    if (ppopupmenu->fSendUninit && (ppopupmenu->spwndNotify != NULL)) {
        ThreadLockAlways(ppopupmenu->spwndNotify, &tlpwndT);
        xxxSendMessage(ppopupmenu->spwndNotify, WM_UNINITMENUPOPUP, (WPARAM)PtoH(ppopupmenu->spmenu), MAKELONG(0, (ppopupmenu->fIsSysMenu ? MF_SYSMENU: 0)));
        ThreadUnlock(&tlpwndT);
    }
...
void xxxMNChar(PPOPUPMENU ppopupmenu, PMENUSTATE pMenuState, UINT character)
{
    ...
    item = xxxMNFindChar(pMenu, character, ppopupmenu->posSelectedItem, &matchType);
    ...
    if ((item == MFMWFP_NOITEM) && ppopupmenu->fIsMenuBar && ppopupmenu->spmenuAlternate) {
         ...
         */
        item = xxxMNFindChar(ppopupmenu->spmenuAlternate, character, 0, &matchType);
        if (item != MFMWFP_NOITEM) {
SwitchToAlternate:
    ...
    if (item == MFMWFP_NOITEM) {
        ...
        result = xxxSendMessage(ppopupmenu->spwndNotify, WM_MENUCHAR, MAKELONG((WORD)character, (WORD)flags), (LPARAM)PtoH(ppopupmenu->spmenu));
...
void xxxMNKeyDown(PPOPUPMENU ppopupmenu, PMENUSTATE pMenuState, UINT key)
{
    ...
    case VK_ESCAPE:
         ...
         */
        if (ppopupmenu->fIsMenuBar || ppopupmenu == ppopupmenu->ppopupmenuRoot || TestWF(ppopupmenu->ppopupmenuRoot->spwndNotify, WFMINIMIZED)) {
            xxxMNDismiss(pMenuState);
        } else {
            /*
             * Pop back one level of menus.
             */
            if (ppopupmenu->fHasMenuBar && ppopupmenu->spwndPrevPopup == ppopupmenu->spwndNotify) {
                ...
                xxxSendMessage(ppopupmenu->spwndPrevPopup, MN_CLOSEHIERARCHY, 0, 0);
                ThreadUnlock(&tlpwndT);
            }
        }
        return;
    ...
    case VK_RIGHT:
        ...
        if (!ppopupmenu->fIsMenuBar && (key == VK_RIGHT) && !ppopupmenu->spwndNextPopup) {
            /*
        ...
        item = MNFindItemInColumn(ppopupmenu->spmenu, ppopupmenu->posSelectedItem, (key == VK_LEFT ? -1 : 1), (ppopupmenu->fHasMenuBar && ppopupmenu == ppopupmenu->ppopupmenuRoot));
        if (item == MFMWFP_NOITEM) {
            /*
        ...
        if (!ppopupmenu->fIsMenuBar) {
            ...
            if (item == MFMWFP_NOITEM) {
                if (TestWF(ppopupmenu->spwndNotify, WFSYSMENU)) {
                     ...
                     */
                    pmenuNextMenu = ppopupmenu->fIsSysMenu ? _GetSubMenu(ppopupmenu->spmenu, 0) : ppopupmenu->spmenu;
                    mnm.hmenuIn = PtoH(pmenuNextMenu);
                    ThreadLock(ppopupmenu->spwndNotify, &tlpwndT);
                    dwMDIMenu = xxxSendMessage(ppopupmenu->spwndNotify, WM_NEXTMENU, (WPARAM)keyOrig, (LPARAM)&mnm);
                    ThreadUnlock(&tlpwndT);
                     ...
                     */
                    pmenuUse = (((pwndNextMenu->spmenuSys != NULL) && (_GetSubMenu(pwndNextMenu->spmenuSys, 0) == pmenuNextMenu)) ? pwndNextMenu->spmenuSys : pmenuNextMenu);
                    /*
                    ...
                    if (!TestWF(pwndNextMenu, WFCHILD) && ppopupmenu->spmenu != NULL) {
                        /*
                        ...
                        if (pwndNextMenu->spmenu == ppopupmenu->spmenu) {
                            LockPopupMenu(ppopupmenu, &ppopupmenu->spmenuAlternate, pwndNextMenu->spmenuSys);
                            pMenuState->fIsSysMenu = FALSE;
                        } else {
                            LockPopupMenu(ppopupmenu, &ppopupmenu->spmenuAlternate, pwndNextMenu->spmenu);
                        }
                    }
...
DropHierarchy:
    ...
    case VK_F1: /* Provide context sensitive help. */
        ...
        pItem = ppopupmenu->spmenu->rgItems + ppopupmenu->posSelectedItem;
        ThreadLock(ppopupmenu->spwndNotify, &tlpwndT);
        xxxSendHelpMessage(ppopupmenu->spwndNotify, HELPINFO_MENUITEM, pItem->wID, PtoHq(ppopupmenu->spmenu), GetMenuInheritedContextHelpId(ppopupmenu));
        ThreadUnlock(&tlpwndT);
        break;
        }
    }
...
*/
{
    ...
    if (ppopup->fIsMenuBar) {
         ...
         * in the same place.
         */
        pMonitor = _MonitorFromWindow(ppopup->spwndPopupMenu, MONITOR_DEFAULTTOPRIMARY);
...
PWND xxxMNOpenHierarchy(PPOPUPMENU ppopupmenu, PMENUSTATE pMenuState)
{
    ...
    if (!ppopupmenu->fNoNotify) {
        ...
        xxxSendMessage(ppopupmenu->spwndNotify, WM_INITMENUPOPUP, (WPARAM)hmenuInit, MAKELONG(ppopupmenu->posSelectedItem, (ppopupmenu->fIsSysMenu ? 1: 0)));
        ThreadUnlock(&tlpwndT);
        fSendUninit = TRUE;
    }
    ...
    if (ppopupmenu->fIsMenuBar && (pdesk->spwndMenu != NULL) && (!(pdesk->dwDTFlags & DF_MENUINUSE)) && !TestWF(pdesk->spwndMenu, WFVISIBLE)) {
         ...
         */
        if ((TestWF(pdesk->spwndMenu, WEFTOPMOST) && pMenuState->fModelessMenu) || (!TestWF(pdesk->spwndMenu, WEFTOPMOST) && !pMenuState->fModelessMenu)) {
            ThreadLock(pdesk->spwndMenu, &tlpwndHierarchy);
            xxxSetWindowPos(pdesk->spwndMenu, (pMenuState->fModelessMenu ? PWND_NOTOPMOST: PWND_TOPMOST), 0,0,0,0,
    ...
    if (!ppopupmenu->fNoNotify && (hmenuInit != PtoHq(pItem->spSubMenu))) {
        RIPMSG2(RIP_WARNING, "xxxMNOpenHierarchy: bad app changed submenu from %#p to %#p", hmenuInit, PtoHq(pItem->spSubMenu));
    }
#endif
    LockPopupMenu(ppopupmenuHierarchy, &ppopupmenuHierarchy->spmenu, pItem->spSubMenu);
     ...
     *  direction
     */
    ppopupmenuHierarchy->iDropDir = xxxMNPositionHierarchy(ppopupmenu, pItem, cxPopup, cyPopup, &xLeft, &yTop, &pMonitor);
    ...
    if (ppopupmenu->fIsMenuBar && _GetAsyncKeyState(VK_LBUTTON) & 0x8000) {
         ...
         */
        CopyOffsetRect(&rc, &pwndHierarchy->rcWindow, xLeft - pwndHierarchy->rcWindow.left, yTop  - pwndHierarchy->rcWindow.top);
        ...
        if (IntersectRect(&rc, &rc, &rcParent)) {
             ...
             */
            xrightdrop = ppopupmenu->spwndPopupMenu->rcWindow.left + pItem->xItem + pItem->cxItem + cxPopup;

            if (xrightdrop > pMonitor->rcMonitor.right) {
                xrightdrop = 0;
            }

            xleftdrop = ppopupmenu->spwndPopupMenu->rcWindow.left + pItem->xItem - cxPopup;
            ...
            if (((SYSMET(MENUDROPALIGNMENT) || TestMFT(pItem, MFT_RIGHTORDER))
                ...
                xLeft = ppopupmenu->spwndPopupMenu->rcWindow.left + pItem->xItem - cxPopup;
                    ppopupmenuHierarchy->iDropDir = PAS_LEFT;
            } else if (xrightdrop) {
                xLeft = ppopupmenu->spwndPopupMenu->rcWindow.left + pItem->xItem + pItem->cxItem;
     ...
     */
    UserAssert((ppopupmenu->ppopupmenuRoot->spwndActivePopup == NULL) || TestWF(ppopupmenu->ppopupmenuRoot->spwndActivePopup, WFVISIBLE));
    ...
    xxxSetWindowPos(pwndHierarchy,
                    ...
                    xLeft, yTop, 0, 0,
                    SWP_SHOWWINDOW | SWP_NOSIZE | SWP_NOOWNERZORDER | (pMenuState->fModelessMenu ? 0 : SWP_NOACTIVATE));
    if (FWINABLE()) {
...
Exit:
     ...
     *  failed).
     */
    if (fSendUninit && (ppopupmenu->spwndNotify != NULL)) {
        ThreadLockAlways(ppopupmenu->spwndNotify, &tlpwndT);
        xxxSendMessage(ppopupmenu->spwndNotify, WM_UNINITMENUPOPUP, (WPARAM)hmenuInit, MAKELONG(0, (ppopupmenu->fIsSysMenu ? MF_SYSMENU : 0)));
        ThreadUnlock(&tlpwndT);
    }
...
*/
{
    ...
    if (pwndNext != NULL) {
         ...
         */
        if (pMenuState->fModelessMenu && pMenuState->fInsideMenuLoop && !ppopupmenu->fIsMenuBar) {
        ...
        if (pwndNext == pdesk->spwndMenu) {
            ...
            ppopupmenuReal = (PPOPUPMENU)xxxSendMessage(pwndNext, MN_GETPPOPUPMENU,0, 0L);
#else
    ...
    if (pMenuState->fInsideMenuLoop && (ppopupmenu->posSelectedItem != MFMWFP_NOITEM)) {
        /*
        ...
        if (pwnd) {
            ...
            xxxSendMenuSelect(pwnd, ppopupmenu->spwndPopupMenu, ppopupmenu->spmenu, ppopupmenu->posSelectedItem);
...
BOOL xxxMNDoubleClick(PMENUSTATE pMenuState, PPOPUPMENU ppopup, int idxItem)
{
...
#if DBG
        else if (msg.message == WM_LBUTTONDBLCLK || msg.message == WM_NCLBUTTONDBLCLK)
        {
    ...
    do
    {
        ...
        if (!TestWF(ppopup->ppopupmenuRoot->spwndNotify, WFWIN40COMPAT))
        {
            ...
            xxxSendMenuSelect(ppopup->ppopupmenuRoot->spwndNotify, ppopup->spwndPopupMenu, pMenu, idxItem);
            ThreadUnlock(&tlpopup);
            ThreadUnlock(&tlpwndNotify);
        }
    ...
    if (ppopupmenu->fAboutToHide)
    {
        ...
        if (ppopupPrev->posSelectedItem != ppopupPrev->posDropped)
        {
            ...
            if (ppopupPrev->posSelectedItem != MFMWFP_NOITEM) {
                xxxMNInvertItem(ppopupPrev, ppopupPrev->spmenu, ppopupPrev->posSelectedItem, ppopupPrev->spwndNotify, FALSE);
            }

            ppopupPrev->posSelectedItem = ppopupPrev->posDropped;

            xxxMNInvertItem(ppopupPrev, ppopupPrev->spmenu, ppopupPrev->posDropped, ppopupPrev->spwndNotify, TRUE);
...
DeselectItem:
            xxxMNInvertItem(ppopupmenu, pmenu, ppopupmenu->posSelectedItem, pwndNotify, FALSE);
    }
    ...
    if (itemPos != MFMWFP_NOITEM) {
        ...
        pItem = xxxMNInvertItem(ppopupmenu, pmenu, itemPos, pwndNotify, TRUE);
        ThreadUnlock(&tlpwndNotify);
    ...
    if (ppopupmenu->spwndPrevPopup != NULL) {
        ...
        if (ppopupmenu->fHasMenuBar && (ppopupmenu->spwndPrevPopup ==
            ...
            pp = (PPOPUPMENU)xxxSendMessage(ppopupmenu->spwndPrevPopup, MN_GETPPOPUPMENU, 0, 0L);
...
*/
{
    ...
    if (TestMF(pMenu, MFISPOPUP)) {
         ...
         */
        CopyInflateRect(&rect, &(pwnd->rcWindow), -SYSMET(CXFIXEDFRAME), -SYSMET(CYFIXEDFRAME));
        ...
        /* ScreenToWindow */
#ifdef USE_MIRRORING
        if (TestWF(pwnd, WEFLAYOUTRTL) && ((ptiCurrent->pq->codeCapture == SCREEN_CAPTURE) || (ptiCurrent->pq->codeCapture == NO_CAP_SYS))) {
...
/*
* LONG MenuFindMenuWindowFromPoint(PPOPUPMENU ppopupmenu, PUINT pIndex, POINT screenPt)
...
LONG_PTR xxxMNFindWindowFromPoint(PPOPUPMENU ppopupmenu, PUINT pIndex, POINTS screenPt)
{
    ...
    *pIndex = 0;
    ...
    if (ppopupmenu->spwndNextPopup) {
        ...
        longHit = xxxSendMessage(ppopupmenu->spwndNextPopup, MN_FINDMENUWINDOWFROMPOINT, (WPARAM)&itemHit, MAKELONG(screenPt.x, screenPt.y));
        ThreadUnlock(&tlpwndT);
...
*/
            ...
            OffsetRect(&rect, pwnd->rcWindow.left + cBorders*SYSMET(CXBORDER), pwnd->rcWindow.top + cBorders*SYSMET(CYBORDER));
#ifdef USE_MIRRORING
...
/*
*void MenuCancelMenus(PPOPUPMENU ppopupmenu, UINT cmd, BOOL fSend)
...
void xxxMNCancel(PMENUSTATE pMenuState, UINT uMsg, UINT cmd, LPARAM lParam)
{
    ...
    if (FWINABLE()) {
        xxxWindowEvent(EVENT_SYSTEM_MENUEND, pwndT, (fIsSysMenu ? OBJID_SYSMENU : (fIsMenuBar ? OBJID_MENU : OBJID_WINDOW)), INDEXID_CONTAINER, 0);
    }
     ...
     */
        xxxSendMessage(pwndT, WM_EXITMENULOOP, ((fTrackFlagsSet && !fIsSysMenu)? 1 : 0), 0);
    }
...
void xxxMNMouseMove(PPOPUPMENU ppopup, PMENUSTATE pMenuState, POINTS ptScreen)
{
    ...
    if (pMenuState->mnFocus == KEYBDHOLD) {
         ...
         */
        if ((cmdHitArea == MFMWFP_OFFMENU) || ((cmdHitArea == MFMWFP_NOITEM) && TestWF(ppopup->spwndNotify, WFMINIMIZED))) {
            return;
        }
    ...
    if (cmdHitArea == MFMWFP_NOITEM) {
         ...
         */
        ppopup = ((PMENUWND)pwnd)->ppopupmenu;
        if (pMenuState->fModelessMenu && !pMenuState->fInDoDragDrop && !ppopup->fTrackMouseEvent) {
            TRACKMOUSEEVENT tme;
...
void xxxMNButtonUp(PPOPUPMENU ppopup, PMENUSTATE pMenuState, UINT posItemHit, LPARAM lParam)
{
     ...
     */
    if (!(pItem->fType & MFT_SEPARATOR) && !(pItem->fState & MFS_GRAYED) && (pItem->spSubMenu == NULL)) {
        xxxMNDismissWithNotify(pMenuState, ppopup->spmenu, pItem, ppopup->posSelectedItem, lParam);
        return;
    }
...
UINT MNSetTimerToOpenHierarchy(PPOPUPMENU ppopup)
{
    ...
    if (ppopup->fShowTimer || (ppopup->fHierarchyDropped && (ppopup->posSelectedItem == ppopup->posDropped))) {
        /*
...
BOOL xxxCallHandleMenuMessages(PMENUSTATE pMenuState, PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((message >= WM_MOUSEFIRST) && (message <= WM_MOUSELAST)) {
        msg.lParam = MAKELONG(GET_X_LPARAM(lParam) + pwnd->rcClient.left, GET_Y_LPARAM(lParam) + pwnd->rcClient.top);
    } else {
        msg.lParam = lParam;
    }
     ...
     */
    if (fHandled && pMenuState->fModelessMenu && ExitMenuLoop (pMenuState, pMenuState->pGlobalPopupMenu)) {
...
LRESULT xxxMenuWindowProc(PWND pwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((pMenuState == NULL) || (pmenu == NULL)) {
         ...
         */
        fIsRecursedMenu = ((ppopupmenu->ppopupmenuRoot != NULL) && IsRecursedMenuState(pMenuState, ppopupmenu));
        if (fIsRecursedMenu) {
            while (IsRecursedMenuState(pMenuState, ppopupmenu) && (pMenuState->pmnsPrev != NULL)) {
        ...
        if (pMenuState->fModelessMenu && !pMenuState->fInCallHandleMenuMessages) {
             ...
             */
            if (fIsRecursedMenu) {
                if (((message >= WM_MOUSEFIRST) && (message <= WM_MOUSELAST)) || ((message >= WM_KEYFIRST) && (message <= WM_KEYLAST)) || ((message >= WM_NCMOUSEFIRST) && (message <= WM_NCMOUSELAST))) {
                    goto CallDWP;
                }
            } else {
    ...
    case WM_ERASEBKGND:
        if (pmenu->hbrBack != NULL) {
            MNEraseBackground ((HDC) wParam, pmenu, 0, 0, pwnd->rcClient.right - pwnd->rcClient.left, pwnd->rcClient.bottom - pwnd->rcClient.top);
            return TRUE;
        } else {
            goto CallDWP;
        }
    ...
    case WM_PRINT:
        ...
        if ((lParam & PRF_NONCLIENT) && (pmenu->dwArrowsOn != MSA_OFF)) {
            ...
            GreSetWindowOrg((HDC)wParam, ptOrg.x - MNXBORDER, ptOrg.y - MNYBORDER - gcyMenuScrollArrow, NULL);
    ...
    case WM_WINDOWPOSCHANGING:
        ...
        if (!TestEffectUP(MENUANIMATION) || !(ppopupmenu->iDropDir & PAS_OUT) || (GetAppCompatFlags2(VER40) & GACF2_ANIMATIONOFF)) {
NoAnimation:
        ...
        if (TestALPHA(MENUFADE)) {
            if ((hdcAni = CreateFade(pwnd, NULL, CMS_MENUFADE, FADE_SHOW | FADE_MENU)) == NULL) {
                goto NoAnimation;
            }
        } else {
            if (!MNCreateAnimationBitmap(pMenuState, pMenuState->cxAni, pMenuState->cyAni)) {
                goto NoAnimation;
            }
    ...
    case WM_NCPAINT:
        if (ppopupmenu->iDropDir & PAS_OUT) {
             ...
             */
            if (pmenu->dwArrowsOn != MSA_OFF) {
                HDC hdc = _GetDCEx(pwnd, (HRGN)wParam, DCX_USESTYLE | DCX_WINDOW | DCX_INTERSECTRGN | DCX_NODELETERGN | DCX_LOCKWINDOWUPDATE);
    ...
    case MN_SIZEWINDOW:
        {
            ...
            if (wParam != 0) {
                ...
                if (TestWF(pwnd, WFVISIBLE)) {
                    lPos = FindBestPos(pwnd->rcWindow.left, pwnd->rcWindow.top, cx, cy, NULL, 0, ppopupmenu, pMonitor);
                    x = GET_X_LPARAM(lPos);
                    y = GET_Y_LPARAM(lPos);
                } else {
                    dwFlags |= SWP_NOMOVE;
    ...
    case MN_SELECTITEM:
        ...
        if (pItem != NULL) {
            return((LONG)(DWORD)(WORD)(pItem->fState | ((pItem->spSubMenu != NULL) ? MF_POPUP : 0)));
        }
...
EndMenu:
     ...
     case MN_DODRAGDROP:
         ...
         */
        if (pMenuState->fDragging && (ppopupmenu->spwndNotify != NULL) && IsMFMWFPWindow(pMenuState->uButtonDownHitArea)) {
            /*
            ...
            lRet = xxxSendMessage(ppopupmenu->spwndNotify, WM_MENUDRAG, pMenuState->uButtonDownIndex, (LPARAM)PtoH(pmenu));
    ...
    case WM_MOUSELEAVE:
        ...
        break;
    case WM_ACTIVATEAPP:
        if (pMenuState->fModelessMenu && (pwnd == pMenuState->pGlobalPopupMenu->spwndActivePopup)) {
            /*
            ...
            if (ppopupmenu->spwndNotify != NULL) {
                ThreadLockAlways(ppopupmenu->spwndNotify, &tlpwndNotify);
                xxxDWP_DoNCActivate(ppopupmenu->spwndNotify, ((wParam && !pMenuState->fActiveNoForeground) ? NCA_ACTIVE : NCA_FORCEFRAMEOFF), HRGN_FULL);
                ThreadUnlock(&tlpwndNotify);
            }
        }
        break;
     case WM_ACTIVATE:
         if (pMenuState->fModelessMenu) {
              ...
              */
             if ((LOWORD(wParam) == WA_INACTIVE) && !pMenuState->fInCallHandleMenuMessages && !pMenuState->pGlobalPopupMenu->fInCancel) {
                 lParam = (LPARAM)RevalidateHwnd((HWND)lParam);
                 if ((lParam != 0) && ((GETFNID((PWND)lParam) != FNID_MENU) || IsRecursedMenuState(pMenuState, ((PMENUWND)lParam)->ppopupmenu))) {
                     /*
                      * If we're just coming to the foreground, then
                      *  activate the popup later and stay up.
                      */
                     if (pMenuState->fActiveNoForeground && (gpqForeground == PtiCurrent()->pq)) {
       ...
       if (LOWORD(wParam)) {
           ...
           xxxActivateWindow(pwnd, AW_SKIP2);
             ...
             */
            if ((gpqForegroundPrev != NULL) && !FBadWindow(gpqForegroundPrev->spwndActivePrev) && !ISAMENU(gpqForegroundPrev->spwndActivePrev)) {
                ...
                do {
                    ...
                    if (pwnd && !FBadWindow(pwnd->spwndLastActive) && !ISAMENU(pwnd->spwndLastActive)) {
...
#define pmenuNext (((PMENUWND)ppopupmenu->spwndNextPopup)->ppopupmenu->spmenu)
               ...
               */
              xxxMNPositionHierarchy(ppopupmenu, pItem, pmenuNext->cxMenu + (2 * SYSMET(CXFIXEDFRAME)), pmenuNext->cyMenu + (2 * SYSMET(CXFIXEDFRAME)), &x, &y, &pMonitorDummy);

              /*
               * Move it
               */
              ThreadLockAlways(ppopupmenu->spwndNextPopup, &tlpwndNotify);
              xxxSetWindowPos(ppopupmenu->spwndNextPopup, NULL, x, y, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOSENDCHANGING);
              ThreadUnlock(&tlpwndNotify);
#undef pmenuNext
           } /* if (pItem != NULL) */
       } /* if (ppopupmenu->spwndNextPopup != NULL) */
