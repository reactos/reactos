...
/*
* Returns true if current process is the shell process and this window is the first non-shell/user one we find in zorder.
* If so, we consider him to be the "palette foreground".
*/
BOOL IsTopmostRealApp(PWND pwnd)
{
    PTHREADINFO  ptiCurrent = PtiCurrent();
    PDESKTOPINFO pdeskinfo = pwnd->head.rpdesk->pDeskInfo;
    if ((pdeskinfo->spwndShell == NULL) || (GETPTI(pdeskinfo->spwndShell)->pq != gpqForeground)) {
        return FALSE;
    }

    return (pwnd == NextTopWindow(ptiCurrent, NULL, NULL, NTW_IGNORETOOLWINDOW));
}


/*
* Selects palette into DC.  This is a wrapper to gdi where we can perform checks to see if it's a foreground dc.
*/
HPALETTE _SelectPalette(HDC hdc, HPALETTE hpal, BOOL fForceBackground)
{
     ...
     */
    if (!fForceBackground && TEST_PUSIF(PUSIF_PALETTEDISPLAY) && (hpal != GreGetStockObject(DEFAULT_PALETTE))) {
...
#if 0
            if (pwndActive && (pwndTop != pwnd->head.rpdesk->pDeskInfo->spwndShell) && ((pwndActive == pwnd) || _IsChild(pwndActive, pwnd) || IsTopmostRealApp(pwnd)) && !TestWF(pwnd, WEFTOOLWINDOW)) {
                fBackgroundPalette = FALSE;
            }
#else
            if ((pwndTop != pwndTop->head.rpdesk->pDeskInfo->spwnd) && (pwndTop != pwndTop->head.rpdesk->pDeskInfo->spwndShell) && (pwndActive != NULL) && ((pwndActive == pwnd) ||
                _IsChild(pwndActive, pwnd) || IsTopmostRealApp(pwnd)) && !TestWF(pwnd, WEFTOOLWINDOW))
            {
                fBackgroundPalette = FALSE;
            }
#endif
...
/*
* Realizes palette to the DC.  This is a wrapper to gdi so that we can check for changes prior to sending notifications.
*/
int xxxRealizePalette(HDC hdc)
{
    ...
    if (HIWORD(dwNumChanged) && IsDCCurrentPalette(hdc)) {
        ...
        /*
         * if there is no associated window, don't send the palette change messages since this is a memory hdc.
         */
        if (pwnd != NULL) {
            /*
    ...
    /*
     * Walk through the SPB list (the saved bitmaps under windows with the CS_SAVEBITS style) discarding all bitmaps
     */
...
/*
* RealizePalette passes FALSE for fForceDesktopso so that it does not cause a loop in case RealizePalette was called by the desktop window.
* In such a case we don't want to call RealizeDesktop.
* In all other cases we do want to go to RealizeDesktop to give the desktop a chance to reailze its palette or possibly just repaint.
