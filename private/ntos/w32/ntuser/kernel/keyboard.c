...
SHORT _GetKeyState(int vk)
{
    ...
    if ((UINT)vk >= CVKKEYSTATE) {
        RIPERR1(ERROR_INVALID_PARAMETER, RIP_WARNING, "Invalid parameter \"vk\" (%ld) to _GetKeyState", vk);
        return 0;
    }
...
#ifdef LATER
// note - anything that accesses the pq structure is a bad idea since it can be changed between any two instructions.
#endif
...
/*
* This function is similar to GetKeyState except it returns what could be considered the 'hardware' keystate or what state the key is in at the
* moment the function is called, rather than based on what key events the application has processed.
* Also, rather than returning the toggle bit, it has a bit telling whether the key was pressed since the last call to GetAsyncKeyState().
...
SHORT _GetAsyncKeyState(int vk)
{
    ...
    if ((UINT)vk >= CVKKEYSTATE) {
        RIPERR1(ERROR_INVALID_PARAMETER, RIP_WARNING, "Invalid parameter \"vk\" (%ld) to _GetAsyncKeyState", vk);
        return 0;
    }
    ...
    /*
     * Don't return the toggle bit since it's a new bit and might cause compatibility problems.
     */
    return sKeyState;
}

/*
* This function allows the app to set the current keystate.
* This is mainly useful for setting the toggle bit, particularly for the keys associated with the LEDs on the physical keyboard.
...
VOID RegisterPerUserKeyboardIndicators(PUNICODE_STRING pProfileUserName)
{
    ...
    FastWriteProfileStringW(pProfileUserName, PMAP_KEYBOARD, wszInitialKeyboardIndicators, wszInitKbdInd);
}
...
VOID UpdatePerUserKeyboardIndicators(PUNICODE_STRING pProfileUserName)
{
     ...
     * Initial Keyboard state (Num-Lock only)
     */
    dw = FastGetProfileIntW(pProfileUserName, PMAP_KEYBOARD, wszInitialKeyboardIndicators, 2);
    dw &= 0x80000002;
...
/*
* Based on a VK and a make/break flag, this function will update the async keystate table.
...
void UpdateAsyncKeyState(PQ pqOwner, UINT wVK, BOOL fBreak)
{
    ...
    /*
     * A key has changed state. Update all queues not receiving this input so they know that this key has changed state.
     * This lets us know which keys to update in the thread specific key state table to keep it in sync with the user.
     * Walking down the thread list may mean that an individual queue may by updated more than once, but it is cheaper than maintaining a list of queues on the desktop.
     */
    ...
    for (pEntry = pHead->Flink; pEntry != pHead; pEntry = pEntry->Flink) {
        ...
        /*
         * Don't update the queue this message is going to - it'll be in sync because it is receiving this message.
         */
        pqT = pti->pq;
        if (pqT == pqOwner)
            continue;
        ...
        /*
         * Set the "recent down" bit. In this case this doesn't really mean "recent down", it means "recent change" (since the last time we synced this queue), either up or down.
         * This tells us which keys went down since the last time this thread synced with key state.
         * Set the "update key state" flag so we know that later we need to sync with these keys.
         */
