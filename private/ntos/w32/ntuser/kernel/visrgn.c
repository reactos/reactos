...
/*
* Sets a region to a rectangle, creating it and making it public if it is not already there.
...
/*
*   This routine is used to resize the vis-rgn memory buffer if the count is exceeded.

* History:
* 22-Oct-1994 ChrisWil  Created
* 27-Feb-1997 adams     Removed call to UserReallocPool, since the pool allocator doesn't support realloc.
*/
BOOL ResizeVisExcludeMemory(VOID)
{
    ...
    apwndNew = (PWND)UserAllocPool(crcNew * sizeof(PWND), TAG_VISRGN);
    if (!apwndNew)
        return FALSE;
...
BOOL ExcludeWindowRects(PWND pwnd, PWND pwndStop, LPRECT lprcIntersect)
{
...
#if DBG
    if (pwnd != NULL && pwndStop != NULL && pwnd->spwndParent != pwndStop->spwndParent) {
    ...
    while ((pwnd != NULL) && (pwnd != pwndStop)) {
        UserAssert(pwnd);
        prc = &pwnd->rcWindow;
        if (TestWF(pwnd, WFVISIBLE) && !FLayeredOrRedirected(pwnd) && (TestWF(pwnd, WEFTRANSPARENT) == 0) && CheckIntersectRect(lprcIntersect, prc) && !EmptyRect(prc)) {
...
BOOL CalcWindowVisRgn(PWND  pwnd, HRGN  *phrgn, DWORD flags)
{
    ...
    /*
     * First get the initial window rectangle which will be used for the basis of exclusion calculations.
     */
    ...
    /*
     * The parent can be NULL in the case when pwnd == pwndRoot. In other cases we should figure why the hell the parent is NULL.
     */
    if (pwndParent == NULL) {
#if DBG
        if (pwnd != pwndRoot) {
    ...
    if (gcrcVisExclude > 0) {
        ...
        if (gcrcVisExclude <= CEXCLUDERECTSMAX) {
            ...
            arcExclude = (PRECT)UserAllocPoolWithQuota(sizeof(RECT) * gcrcVisExclude, TAG_VISRGN);
            if (!arcExclude)
                goto NullRegion;
        }
        ...
        if (ircVisExclude != 0) {
            GreSubtractRgnRectList(*phrgn, &rcWindow, arcExclude, ircVisExclude);
        } else {
...
BOOL CalcVisRgn(HRGN  *phrgn, PWND  pwndOrg, PWND  pwndClip, DWORD flags)
{
    ...
    if (grpdeskRitInput != NULL) {
        UserAssert(pdesk->rpwinstaParent == grpdeskRitInput->rpwinstaParent || !IsVisible(pwndOrg));
    }
#endif // DBG
     ...
     */
    if ((gspwndLockUpdate != NULL) && !(flags & DCX_LOCKWINDOWUPDATE) &&  _IsDescendant(gspwndLockUpdate, pwndOrg)) {
        goto EmptyRgn;
    }
...
int CalcWindowRgn(PWND pwnd, HRGN hrgn, BOOL fClient)
{
    ...
    /*
     * If the window has a region, then intersect the rectangle region with that. If this is low on memory, it'll propagate ERROR back.
     */
    if (pwnd->hrgnClip != NULL) {
