...
DWORD xxxCsDdeInitialize(PHANDLE phInst,HWND *phwndEvent,LPDWORD pMonitorFlags,DWORD afCmd,PVOID pcii)
{
    ...
    psii = (PSVR_INSTANCE_INFO)HMAllocObject(PtiCurrent(), NULL, TYPE_DDEACCESS, sizeof(SVR_INSTANCE_INFO));
    if (psii == NULL) {
        return DMLERR_SYS_ERROR;
    }

    /*
     * We have to tell CreateWindow that window is not created for the same module has the app, (CW_FLAGS_DIFFHMOD), so CreateWindow doesn't assign a hotkey to this window.
     * Other window are done in the client-server thunk
     */
    Lock(&(psii->spwndEvent), xxxCreateWindowEx(0, (PLARGE_STRING)gpsi->atomSysClass[ICLS_DDEMLEVENT], NULL, WS_POPUP | WS_CHILD, 0, 0, 0, 0,
            (PWND)NULL, (PMENU)NULL, hModuleWin, NULL, CW_FLAGS_DIFFHMOD | VER31));
