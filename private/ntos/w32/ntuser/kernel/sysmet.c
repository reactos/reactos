...
BOOL APIENTRY _SwapMouseButton(BOOL fSwapButtons)
{
     ...
     */
    RETURN_IF_ACCESS_DENIED(ppiCurrent->amwinsta, WINSTA_READATTRIBUTES | WINSTA_WRITEATTRIBUTES, FALSE);
    if (!(ppiCurrent->W32PF_Flags & W32PF_IOWINSTA)) {
        RIPERR0(ERROR_REQUIRES_INTERACTIVE_WINDOWSTATION, RIP_WARNING, "SwapMouseButton invalid on a non-interactive WindowStation.");
        return FALSE;
    }
    ...
    /*
     * Give xxxButtonEvent a hint that a mouse button event may have to be left/right swapped to correspond with our current async key state.
...
/*
* Changes the value of a system color, and updates the brush.  Tries to recover in case of an error.
*/
...
BOOL APIENTRY xxxSetSysColors(PUNICODE_STRING pProfileUserName, int cicol, PUINT picolor, COLORREF *prgb, UINT uOptions)
{
    ...
    if (uOptions & SSCF_SETMAGICCOLORS) {
        ...
        for(i = 0; i < cicol; i++) {
            icol = picolor[i];
            rgb = prgb[i];
            if (    icol == COLOR_3DFACE || icol == COLOR_3DSHADOW || icol == COLOR_3DHILIGHT || icol == COLOR_DESKTOP) {
    ...
    for (i = 0; i < cicol; i++) {
        ...
        if ((uOptions & SSCF_SETMAGICCOLORS) && (icol == COLOR_3DFACE || icol == COLOR_3DSHADOW || icol == COLOR_3DHIGHLIGHT || icol == COLOR_DESKTOP)) {
            continue;
        }
    ...
    if (uOptions & SSCF_NOTIFY) {
        ...
        /*
         * Just redraw the entire screen.  Trying to just draw the parts that were changed isn't worth it, since Control Panel always resets every color anyway.

         * Anyway, it could get messy, sending apps NCPAINT messages without accumulating update regions too.
         */
        xxxRedrawScreen();
    }
