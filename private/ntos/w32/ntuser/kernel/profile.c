...
void InitPreviousUserString(void) {
    ...
    /*
     * Speed hack, check if luid of this process == system or previous to save work.
     */
...
*/
{
    ...
    Status = RtlAppendUnicodeToString(&UnicodeString, (PWSTR)(&aFastRegRoot[aFastRegMap[idSection].idRoot][1]));
    UserAssert(NT_SUCCESS(Status));

    Status = RtlAppendUnicodeToString(&UnicodeString, (PWSTR)(aFastRegMap[idSection].szSection));
    UserAssert(NT_SUCCESS(Status));
...
TryAgain:
    ...
    if (dwPolicyFlags & POLICY_MACHINE) {
        ...
        RtlAppendUnicodeToString(&UnicodeString, L"\\Registry\\Machine\\");
        RtlAppendUnicodeToString(&UnicodeString, L"Software\\Policies\\Microsoft\\Windows\\");
    } else {
        if (aFastRegRoot[aFastRegMap[idSection].idRoot][0] == L'M') {
            RtlAppendUnicodeToString(&UnicodeString, L"\\Registry\\Machine\\");
        } else {
            if (!pMapName) {
                InitPreviousUserString();
                RtlAppendUnicodeStringToString(&UnicodeString, &PreviousUserString);
            } else {
                RtlAppendUnicodeStringToString(&UnicodeString, pMapName);
            }
        }
     ...
     * Open the key for kernel mode access
     */
    InitializeObjectAttributes(&OA, &UnicodeString, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    Status = ZwOpenKey(&hKey, amRequest, &OA);
    ...
    if (
        ...
        if (NT_SUCCESS(Status)) {
            PVOID pKey;
            NTSTATUS Status2;
            Status2 = ObReferenceObjectByHandle(hKey, amRequest, NULL, KernelMode, &pKey, NULL);
...
TryAgain:
     ...
     * Try to open a key.
     */
    if ((hKey = OpenCacheKeyEx(pProfileUserName, PMAP_DESKTOP, KEY_READ, &dwPolicyFlags)) == NULL) {
        return FALSE;
    }
    ...
    Status = ZwQueryValueKey(hKey, &UnicodeString, KeyValueBasicInformation, &KeyInfo, sizeof(KeyInfo), &cbSize);
    ZwClose(hKey);
    if (!NT_ERROR(Status)) {
        return TRUE;
...
*/
{
    ...
    hKey = OpenCacheKeyEx(pProfileUserName, PMAP_DESKTOP, KEY_READ, &dwPolicyFlags);
    if (hKey) {
        ZwQueryKey(hKey, KeyValueBasicInformation, &KeyInfo, sizeof(KeyInfo), &cbSize);
        ZwClose(hKey);
...
*/
{
    ...
    if ((hKey = OpenCacheKeyEx(pProfileUserName, idSection, KEY_READ, NULL)) == NULL) {
...
Override:
    RtlInitUnicodeString(&UnicodeString, lpKeyName);
    Status = ZwQueryValueKey(hKey, &UnicodeString, KeyValuePartialInformation, (PKEY_VALUE_PARTIAL_INFORMATION)Buf, sizeof(Buf), &cbSize);
    dwRet = dwDefault;
    if (NT_SUCCESS(Status)) {
    ...
    *lpReturnedString = NULL;
    ...
    while (TRUE) {
        ...
        if (lpKeys == NULL) {
            ...
            lpTmp = lpKeys;
            lpKeys = UserReAllocPoolWithQuota(lpTmp, dwPoolSize, cchSize + sizeof(WCHAR), TAG_PROFILE);
...
TryAgain:
    if ((hKey = OpenCacheKeyEx(pProfileUserName, idSection, KEY_READ, &dwPolicyFlags)) == NULL) {
#if DBG
        if (PsGetCurrentProcess()->UniqueProcessId != gpidLogon) {
            RIPMSG1(RIP_WARNING | RIP_NONAME, "%ws", lpKeyName);
        }
...
Override:
    cbSize = (cchBuf * sizeof(WCHAR)) + FIELD_OFFSET(KEY_VALUE_PARTIAL_INFORMATION, Data);
...
DefExit:
    ...
    /*
     * wcscopy copies terminating zero, but the length returned by wcslen does not, so add 1 to be consistent with success return [bodind]
     */
    if (lpDefault != NULL) {
...
*/
{
    ...
    if (!FastGetProfileStringW(pProfileUserName, idSection, lpKeyName, NULL, ValueBuf, sizeof(ValueBuf) / sizeof(WCHAR))) {
        return nDefault;
    }
...
*/
{
    ...
    if ((hKey = OpenCacheKeyEx(pProfileUserName, idSection, KEY_WRITE, NULL)) == NULL) {
        RIPMSG1(RIP_WARNING | RIP_NONAME, "%ws", lpKeyName);
        return FALSE;
    }

    RtlInitUnicodeString(&UnicodeString, lpKeyName);
    Status = ZwSetValueKey(hKey, &UnicodeString, 0, REG_SZ, (PVOID)lpString, (wcslen(lpString) + 1) * sizeof(WCHAR));
    ZwClose(hKey);
    ...
    )
{
    ...
    UserAssert(idSection <= PMAP_LAST);
    ServerLoadString(hModuleWin, idKey, szKey, ARRAY_SIZE(szKey));
    return FastGetProfileStringW(pProfileUserName, idSection, szKey, lpDefault, lpReturnedString, cch);
}
...
#if DBG
    if (!NT_SUCCESS(Status)) {
        RIPMSG3 (RIP_WARNING, "FastWriteProfileValue: ZwSetValueKey Failed. Status:%#lx idSection:%#lx KeyName:%s", Status, idSection, UnicodeString.Buffer);
    }
#endif
...
Override:
    ...
    if (Status != STATUS_OBJECT_NAME_NOT_FOUND) {
        RIPMSG3 (RIP_WARNING, "FastGetProfileValue: ZwQueryValueKey Failed. Status:%#lx idSection:%#lx KeyName:%s", Status, idSection, UnicodeString.Buffer);
    }
#endif
...
BOOL FastGetProfileIntsW(PUNICODE_STRING pProfileUserName OPTIONAL, PPROFINTINFO ppii)
{
    ...
    while (ppii->idSection != 0) {
        ...
        *ppii->puResult = FastGetProfileIntW(pProfileUserName, ppii->idSection, szKey, ppii->nDefault);
        ppii++;
    }
