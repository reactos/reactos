...
NTSTATUS OpenEffectiveToken(PHANDLE phToken)
{
    ...
    if (Status == STATUS_NO_TOKEN) {
         ...
         */
        Status = ZwOpenProcessToken(NtCurrentProcess(), TOKEN_QUERY, phToken);
    }
...
NTSTATUS GetProcessLuid(PETHREAD Thread, PLUID LuidProcess)
{
    ...
    UserToken = PsReferenceImpersonationToken(Thread, &fCopyOnOpen, &fEffectiveOnly, &ImpersonationLevel);// Check for a thread token first
    if (UserToken == NULL) {// No thread token, go to the process
    ...
    ObDereferenceObject(UserToken);// We're finished with the token
    return Status;
}
    ...
    PHANDLE         phThread)
{
    ...
    Status = PsCreateSystemThread(phThread, THREAD_ALL_ACCESS, &Obja, hProcess, NULL, lpThreadAddress, pvContext);
    ZwClose(hProcess);
    return Status;
}
