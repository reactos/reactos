...
PMENU xxxLoadSysDesktopMenu (PMENU * ppmenu, UINT uMenuId)
{
    ...
    if (*ppmenu != NULL) {
        ...
        RIPMSG1(RIP_WARNING, "LoadSysDesktopMenu: Menu loaded during callback. ppmenu:%#p", ppmenu);
        _DestroyMenu(pmenu);
        return *ppmenu;
    }
...
* These functions lock/unlock a pmenu into a desktop structure (spmenuSys or spmenuDialogSys) and mark/clear it as such.
...
* Note that this assumes that there is only one submenu. If more are added, these functions have to be fixed accordingly.
...
/*
* Returns a handle to the system menu of the given window. NULL if the window doesn't have a system menu.
*/
...
BOOL IsSmallerThanScreen(PWND pwnd)
{
    ...
    if ((pwnd->rcWindow.right - pwnd->rcWindow.left < dxMax) || (pwnd->rcWindow.bottom - pwnd->rcWindow.top < dyMax)) {
        return TRUE;
    }
...
//            (UINT)((_GetSysModalWindow() == NULL) || hTaskLockInput ? 0: MFS_GRAYED);
         ...
         */
        fFramedDialogBox = (((TestWF(pwnd, WFBORDERMASK) == (BYTE)LOBYTE(WFDLGFRAME)) || (TestWF(pwnd, WEFDLGMODALFRAME))) && !TestWF(pwnd, WFSIZEBOX | WFMINBOX | WFMAXBOX));
        if (!fFramedDialogBox) {
...
PMENU xxxGetSystemMenu(PWND pwnd, BOOL fRevert)
{
    ...
    if (fRevert) {
        ...
        if (((pmenu == NULL) || TestMF(pmenu, MFSYSMENU))
            ...
            if ((pGlobalPopupMenu != NULL) && !pGlobalPopupMenu->fIsTrackPopup && (pGlobalPopupMenu->spwndPopupMenu == pwnd)) {
...
DWORD xxxEnableMenuItem(PMENU pMenu, UINT wIDEnableItem, UINT wEnable)
{
    ...
    dres = MenuItemState(pMenu, wIDEnableItem, wEnable, MFS_GRAYED, &pRealMenu);
...
/*
*  Sets the default item in the menu, by command or by position based on the fByPosition flag.
*  We unset all the other items as the default, then set the given one.

*  The return value is TRUE if the given item was set as default, FALSE if not.
*/
...
//  Tries to find a close item in the first level of menu items.  Looks for SC_CLOSE, then a couple other IDs.
//  We'd rather not do lstrstri's for "Close", which is slow.
void _SetCloseDefault(PMENU pSubMenu)
{
...
//  Attempts to find first child visible child window in the zorder that has a system menu or is maxed.
//  We can't check for an exact system menu match because several apps make their own copy of the sys menu.
