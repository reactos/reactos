...
/*
* Checks to see if the current queue has a caret. If pwnd != NULL, check to see if the caret is for pwnd.
...
BOOL UT_CaretSet(PWND pwnd)
{
    ...
    if (pq->caret.spwnd == NULL) {
        RIPERR0(ERROR_ACCESS_DENIED, RIP_VERBOSE, "Access denied in UT_CaretSet to current queue's caret");
        return FALSE;
    }

    /*
     * If the current task does not own the caret, then return FALSE but we let 32 bit multithreaded apps set the caret position from a second thread for compatibility to our NT 3.1 BETAs
     */
    if (pq->caret.tid != TIDq(ptiCurrent)) {
        PTHREADINFO ptiCursorOwner;

        ptiCursorOwner = PtiFromThreadId(pq->caret.tid);
        if ((ptiCurrent->TIF_flags & TIF_16BIT) || ptiCursorOwner == NULL || (ptiCurrent->ppi != ptiCursorOwner->ppi))  {
            RIPERR0(ERROR_ACCESS_DENIED, RIP_VERBOSE, "Access denied in UT_CaretSet");
            return FALSE;
        }
    }
...
void UT_InvertCaret()
{
    ...
    if ((pq->caret.hBitmap) > (HBITMAP)1) {
        ...
        GreBitBlt(hdc, pq->caret.x, pq->caret.y, pq->caret.cx, pq->caret.cy, ghdcMem, 0, 0, SRCINVERT, 0);
        GreSelectBitmap(ghdcMem, hbmSave);
    } else {
        POLYPATBLT PolyData;

        /*
         * The caret is a pattern (gray or white).  PATINVERT it onto the screen.
         * Remember to unrealize the gray object so it aligns to the window correctly.

         * Remove call to UnrealizeObject.  GDI handles this on NT for brushes.
        ...
        //GrePatBlt(hdc, pq->caret.x, pq->caret.y, pq->caret.cx, pq->caret.cy, PATINVERT);
...
BOOL xxxCreateCaret(PWND pwnd, HBITMAP hBitmap, int cx, int cy)
{
    ...
    pq->caret.hTimer = _SetSystemTimer(pwnd, IDSYS_CARET, gpsi->dtCaretBlink, CaretBlinkProc);

    UserAssert(pwnd == pq->caret.spwnd);
    zzzEndDeferWinEventNotify();
    if (FWINABLE()) {
        /*
         * It's best to force this routine to be an xxx routine: that way we can force pwnd to be locked and force notifications from within this routine and all of the callers are happy with this.
         */
...
/*
* This routine gets called by DispatchMessage when it gets the WM_SYSTIMER message - it blinks the caret.
...
BOOL zzzSetCaretPos(int x, int y)
{
     ...
     */
    _KillSystemTimer(pq->caret.spwnd, IDSYS_CARET);
    pq->caret.hTimer = _SetSystemTimer(pq->caret.spwnd, IDSYS_CARET, gpsi->dtCaretBlink, CaretBlinkProc);
    pq->caret.fOn = TRUE;
