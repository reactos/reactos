...
void xxxPSMTextOut(HDC hdc, int xLeft, int yTop, LPWSTR lpsz, int cch, DWORD dwFlags)
{
    ...
    /*
     * Adjust for proportional font when setting the length of the underline and height of text.
     */
    GreGetTextExtentW(hdc, (LPWSTR)(pchOut + LOWORD(result)), 1, &size, GGTE_WIN3_EXTENT);
    textsize = size.cx;

    /*
     * Find the width of the underline character.
     * Just subtract out the overhang divided by two so that we look better with italic fonts.
     * This is not going to effect embolded fonts since their overhang is 1.
     */
