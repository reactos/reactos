...
// Insert/remove the specified IME hotkey into/from the IME hotkey list (gpImeHotKeyListHeader).
BOOL SetImeHotKey(DWORD dwHotKeyID, UINT uModifiers, UINT uVKey, HKL hKL, DWORD dwAction )
{
    ...
    case ISHK_ADD:
        ...
        ph = FindImeHotKeyByKeyWithLang(gpImeHotKeyListHeader, uModifiers & MOD_MODIFY_KEYS, uModifiers & MOD_BOTH_SIDES, uVKey, GetHotKeyLangID(dwHotKeyID));
        if ( ph != NULL ) {
            if ( ph->hk.dwHotKeyID != dwHotKeyID ) {
                RIPERR0( ERROR_HOTKEY_ALREADY_REGISTERED, RIP_WARNING, "There is an IME hotkey that has the same vkey/modifiers/Lang Id");
                return FALSE;
            }
            // So far we found a hotkey that has the same vkey and same ID.
            // But because modifiers may be slightly different, so go ahead and change it.
        } else {
            // the specified vkey/modifiers combination cound not be found in the hotkey list. The caller may want to change the key assignment of an existing hotkey or add a new hotkey.
            ph = FindImeHotKeyByID( gpImeHotKeyListHeader, dwHotKeyID );
        }
        ...
        // adding a new hotkey
            ...
            if ( ph == NULL ) {
                RIPERR0( ERROR_OUTOFMEMORY, RIP_WARNING, "Memory allocation failed in SetImeHotKey");
                return FALSE;
            }
...
int GetLangIdMatchLevel(HKL hkl, LANGID langId)
{
    if (langId == LANG_NEUTRAL) {
        // If langId is LANG_NEUTRAL, the hot key does not depend on the current HKL. Make it perfect match always.
...
//      else NULL - failure
// Finds the best matching of IME hot keys considering the current input locale.
    ...
    UINT uVKey)                     // the input key
{
    ...
    for (ph = pHead; ph != NULL; ph = ph->pNext)
    {
        if (ph->hk.uVKey == uVKey) {
            ...
            if ((ph->hk.uModifiers & MOD_BOTH_SIDES) == uRL || (ph->hk.uModifiers & MOD_BOTH_SIDES) & uRL) {
                fDoCheck = TRUE;
            }
    ...
    )
{
    ...
    if ( fKeyUp ) {
        ...
        // we previously saved, there is no chance that this is a hotkey.
        if ( uVKeySaved != uVKey ) {
            uVKeySaved = 0;
            return NULL;
        }
        ...
        // the hotkey list because there is a chance that the hotkey list is modified between the key make and break.
    }

    // Current specification doesn't allow us to use a complex hotkey such as LSHIFT+RMENU+SPACE
