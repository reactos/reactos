...
BOOL BltIcon(HDC hdc, int x, int y, int cx, int cy, HDC hdcSrc, PCURSOR pcur, BOOL fMask, LONG rop)
{
     ...
     * in the DC.
     */
    GreStretchBlt(hdc, x, y, cx, cy, hdcSrc, 0, (fMask || pcur->hbmColor ? 0 : pcur->cy / 2), pcur->cx, pcur->cy / 2, rop, (COLORREF)-1);
...
BOOL _DrawIconEx(HDC hdc, int x, int y, PCURSOR pcur, int cx, int cy, UINT istepIfAniCur, HBRUSH hbr, UINT diFlags)
{
    ...
    if (hbr) {
        ...
        if (diFlags & DI_MASK) {
            BltIcon(hdc, x, y, cx, cy, ghdcMem, pcur, TRUE, ((diFlags & DI_IMAGE) ? SRCAND : SRCCOPY));
        }

        if (diFlags & DI_IMAGE) {
            BltIcon(hdc, x, y, cx, cy, ghdcMem, pcur, FALSE, ((diFlags & DI_MASK) ? SRCINVERT : SRCCOPY));
        }
