...
int GetHmodTableIndex(PUNICODE_STRING pstrLibName)
{
    ...
    if (i == catomSysTableEntries) {
        ...
        if (i == catomSysTableEntries) {
            if (i == CLIBS) {
                UserDeleteAtom(atom);
                RIPERR0(ERROR_NOT_ENOUGH_MEMORY, RIP_WARNING, "Memory allocation failed in GetHmodTableIndex");
                return -1;
            }
...
/*
* This function merely increments the dependency count of a given hmod atom table index.
...
/*
* This function removes a system dependency on a given index into the hmod atom table.
* If all dependencies on the hmod have been removed (the Depends count  reaches zero) then the QS_SYSEXPUNGE bit is set in all message queues so the eventually each process will do a free module on it.
...
{
    UserAssert(iatom >= 0);
    if (iatom < catomSysTableEntries && --acatomSysDepends[iatom] == 0) {
        if (acatomSysUse[iatom]) {

            /*
             * Cause each thread to check for expunged dlls the next time they awake.
             */
            gcSysExpunge++;
...
/*
* This function attempts to load the hmodule specified by iatom into the system hmod table.
* Updates the per-process bitmap accordingly.
...
HANDLE xxxLoadHmodIndex(int iatom, BOOL bWx86KnownDll)
{
    ...
    UserAssert((!gptiRit || gptiRit->ppi != PtiCurrent()->ppi) && "Shouldn't load global hooks on system process - gptiRit->ppi is the system process");
...
/*
* This function is called when a thread wakes up and finds its QS_SYSEXPUNGE wakebit set.
...
VOID xxxDoSysExpunge(PTHREADINFO pti)
{
    ...
    for (i = 0; i < catomSysTableEntries; i++) {
        if ((acatomSysDepends[i] == 0) && (aatomSysLoaded[i] != 0) && TESTHMODLOADED(pti, i)) {
