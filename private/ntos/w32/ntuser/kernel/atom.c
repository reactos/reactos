...
ATOM UserAddAtom(LPCWSTR ccxlpAtom, BOOL bPin)
{
     ...
     */
    atom = 0;
    Status = RtlAddAtomToAtomTable( UserAtomTableHandle, (PWSTR)ccxlpAtom, &atom);
...
ATOM UserFindAtom(LPCWSTR ccxlpAtom)
{
     ...
     */
    atom = 0;
    Status = RtlLookupAtomInAtomTable( UserAtomTableHandle, (PWSTR)ccxlpAtom, &atom);
    if (!NT_SUCCESS(Status)) {
...
UINT UserGetAtomName(ATOM atom, LPWSTR ccxlpch, int cchMax)
{
    ...
    Status = RtlQueryAtomInAtomTable( UserAtomTableHandle, atom, NULL, NULL, ccxlpch, &AtomNameLength);
    if (!NT_SUCCESS(Status)) {
