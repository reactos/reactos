    ...
    if (!(dwFlags & EWX_NONOTIFY)) {
        ...
        if (dwFlags & EWX_SHUTDOWN) {
            ...
            if (gspwndLogonNotify != NULL) {
                _PostMessage(gspwndLogonNotify, WM_LOGONNOTIFY, dwllParam, (LONG)dwStatus);
                fNotified = TRUE;
            }
        } else {
            if (gspwndLogonNotify != NULL && (RtlEqualLuid(&pwinsta->luidUser, pluidCaller) || RtlEqualLuid(&luidSystem, pluidCaller))) {
                _PostMessage(gspwndLogonNotify, WM_LOGONNOTIFY, dwllParam, (LONG)dwStatus);
                fNotified = TRUE;
            }
        }
...
NTSTATUS InitiateShutdown(PETHREAD Thread, PULONG lpdwFlags)
{
    ...
    if (gdwThreadEndSession != 0) {
        ...
        /*
         * If the current shutdown in another sid and is not being done by winlogon, override it.
         */
        if (!RtlEqualLuid(&luidCaller, &gpwinstaLogoff->luidEndSession) && (gpidEndSession != gpidLogon)) {
            return STATUS_RETRY;
        }
         ...
         */
        if (dwNew && HandleToUlong(PsGetCurrentThread()->Cid.UniqueThread) != gdwThreadEndSession) {
            /*
             * Only one windowstation can be logged off at a time.
             */
            if (!(dwFlags & EWX_SHUTDOWN) && pwinsta != gpwinstaLogoff) {
    ...
    *lpdwFlags = dwFlags;
     ...
     * logoff
     */
    if (dwFlags & (EWX_WINLOGON_OLD_SHUTDOWN | EWX_WINLOGON_OLD_REBOOT | EWX_SHUTDOWN | EWX_REBOOT)) {
...
NTSTATUS EndShutdown(PETHREAD Thread, NTSTATUS StatusShutdown)
{
    ...
    if (!NT_SUCCESS(StatusShutdown)) {
         ...
         * again.
         */
        pwinsta->dwWSF_Flags = (pwinsta->dwWSF_Flags & ~WSF_OPENLOCK) | gdwLocks;
     ...
     * and continue.
     */
    if (((pwinsta->luidUser.LowPart != 0) || (pwinsta->luidUser.HighPart != 0)) && RtlEqualLuid(&pwinsta->luidUser, &luidCaller)) {
...
/*
* This routine simply records the WOW callback address for notification of "hung" wow apps.
...
BOOL xxxRegisterUserHungAppHandlers(PFNW32ET pfnW32EndTask, HANDLE hEventWowExec)
{
    ...
    if (bRetVal) {//  if sucess then intialize the pwpi, ppi structs else free allocated memory
