...
*/
{
    ...
    Status = ObReferenceObjectByHandle(hwinsta,amDesired,*ExWindowStationObjectType,AccessMode,ppwinsta,NULL);
    if (!NT_SUCCESS(Status)) {
        RIPNTERR1(Status, RIP_VERBOSE, "ValidateHwinsta failed for %#p", hwinsta);

    } else if ((*ppwinsta)->dwSessionId != gSessionId) {
        RIPNTERR3(STATUS_INVALID_HANDLE, RIP_WARNING, "SessionId %d. Wrong session id %d for pwinsta %#p", gSessionId, (*ppwinsta)->dwSessionId, *ppwinsta);
...
*/
{
    ...
    Status = ObReferenceObjectByHandle(hdesk,amDesired,*ExDesktopObjectType,AccessMode,ppdesk,NULL);
    if (NT_SUCCESS(Status)) {
        if ((*ppdesk)->dwSessionId != gSessionId) {
            RIPNTERR3(STATUS_INVALID_HANDLE, RIP_WARNING, "SessionId %d. Wrong session id %d for pdesk %#p", gSessionId, (*ppdesk)->dwSessionId, *ppdesk);
            goto Error;
        }
        ...
        if ((*ppdesk)->dwDTFlags & (DF_DESTROYED | DF_DESKWNDDESTROYED | DF_DYING)) {
            RIPNTERR1(STATUS_INVALID_HANDLE, RIP_ERROR, "ValidateHdesk: destroyed desktop %#p", *ppdesk);
Error:
...
*/
{
    ...
    if (pmenuRet != NULL && ((pti->rpdesk != NULL &&  // hack so console initialization works.
...
*/
{
    StartValidateHandleMacro(hwnd)
        ...
        /*
         * Now make sure the app is passing the right handle type for this api.
         * If the handle is TYPE_FREE, this'll catch it.
         */
        if (phe->bType == TYPE_WINDOW) {
            ...
            if (pwndRet != NULL) {
                if (phe->bFlags & HANDLEF_DESTROY) {
                    RIPERR2(ERROR_INVALID_WINDOW_HANDLE, RIP_WARNING,"ValidateHwnd, hwnd %#p, pwnd %#p already destroyed\n", hwnd, pwndRet);
                    return NULL;
                }
                if (GETPTI(pwndRet) == pti ||
                    ...
                    if (IS_THREAD_RESTRICTED(pti, JOB_OBJECT_UILIMIT_HANDLES)) {
                        ...
                        if (!IsHandleEntrySecure(hwnd, phe)) {
                            RIPERR1(ERROR_INVALID_WINDOW_HANDLE,RIP_WARNING,"ValidateHwnd: Invalid hwnd (%#p) for restricted process\n",hwnd);
                            pwndRet = NULL;
                        }
                    }
                    return pwndRet;
    ...
    RIPERR1(ERROR_INVALID_WINDOW_HANDLE, RIP_WARNING, "ValidateHwnd: Invalid hwnd (%#p)",hwnd);
    return NULL;
}
    ...
    )
{
    ...
    b = ReadProcessMemory(hCurrentProcess, (LPVOID) IndexAddress, &InitialIndex, sizeof(InitialIndex), NULL);
    if (!b) {
        return;
    }
    ...
    b = ReadProcessMemory(hCurrentProcess, (LPVOID) ArrayAddress, Array, sizeof(UserSrvArray), NULL);
    if (!b) {
...
{
    DumpArray(hCurrentProcess,hCurrentThread,dwCurrentPc,lpExtensionApis,lpArgumentString,&UserSrvIndex,(LPDWORD)&UserSrvArray[0]);
}
...
{
    UserAssert(gpresUser != NULL);
    UserAssert( (ExIsResourceAcquiredExclusiveLite(gpresUser) == TRUE) || (ExIsResourceAcquiredSharedLite(gpresUser) == TRUE));
}
...
{
    UserAssert(gpresUser != NULL);
    return( (ExIsResourceAcquiredExclusiveLite(gpresUser) == TRUE) || (ExIsResourceAcquiredSharedLite(gpresUser) == TRUE));
    return(TRUE);
}
...
void CheckDevLockOut()
{
     ...
     * hDev is initialized later in InitVideo, after the critical section has been released at least once; so we better check it too.
     */
...
NTSTATUS _KeUserModeCallback (IN ULONG ApiNumber, IN PVOID InputBuffer, IN ULONG InputLength, OUT PVOID *OutputBuffer, OUT PULONG OutputLength)
{
    ...
    /*
     * Added this so we can detect an erroneous user mode callback with a checked win32k on top of a free system.
     */
    ASSERT(KeGetPreviousMode() == UserMode);

    return KeUserModeCallback( ApiNumber, InputBuffer, InputLength, OutputBuffer, OutputLength);
}
#endif