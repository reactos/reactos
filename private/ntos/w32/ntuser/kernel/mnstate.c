...
*/
{
    ...
    if (pmenusys == NULL) {
        RIPERR0(ERROR_INVALID_HANDLE, RIP_WARNING, "Invalid menu handle pmenusys (NULL) to MNPositionSysMenu");
        return;
    }
    ...
    rc.top = rc.left = 0;/*Setup the SysMenu hit rectangle.*/
    ...
    if (!TestMF(pmenusys, MF_POPUP) && (pmenusys->cItems > 0)) {
        ...
        UserAssert(FALSE);// BOGUS -- MF_POPUP should never be set on a MENU -- only a MENU ITEM
}
...
*/
{
    ...
    while (ppmDestroyed->ppmDelayedFree != NULL) {
        ...
        if (ppmDestroyed->ppmDelayedFree->fDestroyed) {
            ...
            /*
             * fUnlock is TRUE if the root popup is being destroyed; if so, reset fDelayedFree and unlink it
             */
            if (fUnlock) {
                /*
                 * This means that the root popup is going away before some of the hierarchical popups have been destroyed.
                 * This can happen if someone destroys one of the menu windows breaking the spwndNextPopup chain.
                 */
            ...
            } else {/*Not fDestroyed so move to the next one.*/
...
VOID MNFreePopup(PPOPUPMENU ppopupmenu)
{
     ...
     *  then the reference to the popup must be cleared at this point since we're going to free this popup
     */
    ...
    /*
     * if spwndNextPopup is not NULL, we're breaking the chain and spwndNext won't get closed.
     *  I won't remove the unlock since it has always been there.
     */
...
/*
* spwndNotify might have been created by a thread other than the one the menu mode is running on.
*  If this is the case, this function NULLs out pMenuState for the thread that owns spwndNotify.
* It returns TRUE if the menu state owner doesn't own the notification window (multiple threads involved)
...
/*
* This funtion must be called to clean up pMenuState after getting out of menu mode.
*  It must be called by the same thread that initialized pMenuState either manually or by calling xxxMNStartMenuState.
...
*/
{
    ...
    if (pMenuState->pGlobalPopupMenu != NULL) {
        if (fFreePopup) {
            ...
            /*
             * This means that we're ending the menustate but the popup menu window is still around.
             *  This can happen when called from xxxDestroyThreadInfo.
             */
    ...
    ptiCurrent->pMenuState = pMenuState->pmnsPrev; /*Restore the previous state, if any*/
    ...
    if (ptiCurrent->pMenuState != NULL) {
        if (ptiCurrent->pMenuState->fModelessMenu) {
            xxxActivateThisWindow(ptiCurrent->pMenuState->pGlobalPopupMenu->spwndActivePopup, 0, 0);
        } else {
...
#if DBG
    /*If this thread is not in menu mode anymore, it must not be using the desktop menu*/
...
*/
{
     ...
     * This is done here because in case of failure, MNEndMenuState will find ptiCurrent->pMenuState properly.
     */
    if (fAllocate) {
        RtlZeroMemory((PBYTE)pMenuState + sizeof(MENUSTATE) - sizeof(MENUANIDC), sizeof(MENUANIDC));
...
/*
* This function is called when the menu bar is about to be activated (the app's main menu).
*  It makes sure that the threads involved are not in menu mode already, finds the owner/notification window,
*  initializes pMenuState and sends the WM_ENTERMENULOOP message.
...
*/
{
    ...
    /*
     * If pwnd is not owned by ptiCurrent, the _PostMessage call below might send us in a loop
     */
...
hasmenu:
    ...
    /*
     * If the owner/notification window was created by another thread, make sure that it's not in menu mode already
     * This can happen if PtiCurrent() is attached to other threads, one of which created pwnd.
     */
    ...
    /*
     * If the notification window is owned by another thread, then the menu loop wouldn't get any keyboard or mouse
    ...
    if (pwndT->spmenu)
        ...
        // keys are really not important. however lets take the next best thing just to be safe
     ...
     * procedure will only be called for menu bar menus not TrackPopupMenu menus.
     */
...
* Note that this function calls back many times so we might be forced out of menu mode at any time.
*  We don't want to check this after each callback so we lock what we need and go on. Be careful.
...
*/
{
    ...
    pMenuState->fButtonDown = pMenuState->fButtonAlwaysDown = ((_GetKeyState(VK_LBUTTON) & 0x8000) != 0);

    xxxMNSetCapture(ppopupmenu);

    xxxSendMessage(pwndMenu, WM_SETCURSOR, (WPARAM)HWq(pwndMenu), MAKELONG(MSGF_MENU, 0));
    ...
    /*
     * Bogus!  We don't always know that this is the system menu.
     * We will frequently pass on an OBJID_MENU even when you hit Alt+Space.
...
PMENU xxxGetInitMenuParam(PWND pwndMenu, BOOL * lpfSystem)
//  GetInitMenuParam()
//  Gets the HMENU sent as the wParam of WM_INITMENU, and for menu bars, is the actual menu to be interacted with.
{
    ...
    if (TestWF(pwndMenu, WFMINIMIZED) || TestwndChild(pwndMenu) || (pwndMenu->spmenu == NULL) || !pwndMenu->spmenu->cItems) {
        if (lpfSystem != NULL)
            *lpfSystem = TRUE;
