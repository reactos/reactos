...
/*
* This queues a show window event in another thread's queue. Used mainly from within taskmgr, so that taskmgr doesn't hang waiting on hung apps.
...
BOOL xxxShowWindow(PWND pwnd, DWORD cmdShowAnimate)
{
     ...
     */
    if ((pti->ppi->usi.dwFlags & STARTF_USESHOWWINDOW) && !TestwndChild(pwnd) && (TestWF(pwnd, WFBORDERMASK) == (BYTE)LOBYTE(WFCAPTION)) && (pwnd->spwndOwner == NULL)) {
        bFirstMain = TRUE;
     ...
     * real SW_* command from the application's STARTUPINFO structure (STARTUPINFO is passed to CreateProcess() when this application was launched).
     */
     ...
     */
    if (bFirstMain) {
        pti->ppi->usi.dwFlags &= ~(STARTF_USESHOWWINDOW | STARTF_USESIZE | STARTF_USEPOSITION);
    }
     ...
     */
    if ((GetFullScreen(pwnd) != WINDOWED) && (pwnd->head.rpdesk == grpdeskRitInput)) {
        if ((cmdShow == SW_SHOWNORMAL) || (cmdShow == SW_RESTORE) || (cmdShow == SW_MAXIMIZE) || (cmdShow == SW_SHOWMAXIMIZED)) {
            ...
            if (gpqForeground != NULL && gpqForeground->spwndActive == pwnd) {
    ...
    if (TestWF(pwnd, WFSENDSIZEMOVE)) {
        ...
        xxxSendMessage(pwnd, WM_MOVE, 0, (pwnd->spwndParent == PWNDDESKTOP(pwnd)) ? MAKELONG(pwnd->rcClient.left, pwnd->rcClient.top) : MAKELONG(pwnd->rcClient.left - pwnd->spwndParent->rcClient.left, pwnd->rcClient.top - pwnd->spwndParent->rcClient. top));
    }
...
void xxxShowOwnedWindows(PWND pwndOwner, UINT cmdShow, HRGN hrgnHung)
{
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++)
    {
        ...
        if ((pwndTopOwner = pwnd->spwndOwner) != NULL) {
             ...
             * is invalid.
             */
            while (!TestwndHI(pwndTopOwner) && pwndTopOwner != pwndOwner && pwndTopOwner->spwndOwner != NULL)
        ...
        if (cmdZoom != 0) {
             ...
             */
            if (   cmdShow == SW_OTHERUNZOOM && pwndTopOwner != NULL && TestWF(pwndTopOwner, WFMINIMIZED) && !TestWF(pwndTopOwner, WFDISABLED))
                continue;
        } else {
            /*
         ...
         */
        if ((fShow && TestWF(pwnd, WFHIDDENPOPUP)) || (!fShow && TestWF(pwnd, WFVISIBLE))) {
            /*
             * For hung minimization, just set the HIDDENPOPUP bit, clear
             * the visible bit and add the window rect to the region to be repainted.
             */
            if (hrgnHung != NULL) {
...
/*
* This routine is accessable to the user.  It will either show or hide all popup windows owned by the window handle specified.
* If fShow if TRUE, all hidden popups will be shown.
* If it is FALSE, all visible popups will be hidden.
...
{
    CheckLock(pwndOwner);
    xxxShowOwnedWindows(pwndOwner, (UINT)(fShow ? SW_PARENTOPENING : SW_PARENTCLOSING), NULL);
    return TRUE;
}