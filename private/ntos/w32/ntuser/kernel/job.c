...
NTSTATUS UserJobCallout(PKWIN32_JOBCALLOUT_PARAMETERS Parm)
{
    ...
    case PsW32JobCalloutAddProcess:
        /*
         * 'Data' parameter is a pointer to W32PROCESS. So this callout happens only for GUI processes.
         */
        ...
        TAGMSG3(DBGTAG_Job, "UserJobCallout: AddProcess Job 0x%x W32Job 0x%x Process 0x%x", Job, pW32Job, (ULONG_PTR)Data);
    ...
    case PsW32JobCalloutTerminate:
        TAGMSG2(DBGTAG_Job, "UserJobCallout: Terminate Job 0x%x W32Job 0x%x", Job, pW32Job);
        if (pW32Job) {
            JobCalloutTerminate(pW32Job);
        }
        break;
    default:
        TAGMSG2(DBGTAG_Job, "UserJobCallout: Invalid callout 0x%x Job 0x%x", CalloutType, Job);
...
PW32JOB CreateW32Job(PEJOB Job)
{
    ...
    if (pW32Job->pAtomTable == NULL) {
        RIPMSG1(RIP_ERROR, "CreateW32Job: fail to create the atom table for job 0x%x", pW32Job);
    ...
    gpJobsList = pW32Job;
    pW32Job->Job = Job;
    TAGMSG2(DBGTAG_Job, "CreateW32Job: pW32Job 0x%x created for EJOB 0x%x", pW32Job, Job);
    return pW32Job;
}


/*
* Walks the processinfo list in userk to update all the processes assigned to this job .
...
BOOL RemoveProcessFromJob(PPROCESSINFO ppi)
{
    ...
    TAGMSG2(DBGTAG_Job, "RemoveProcessFromJob: ppi 0x%x pW32Job 0x%x", ppi, pW32Job);
    ...
    for (ip = 0; ip < pW32Job->uProcessCount; ip++) {
        ...
        if (ppi == pW32Job->ppiTable[ip]) {
            ppi->pW32Job = NULL;
            RtlMoveMemory(pW32Job->ppiTable + ip, pW32Job->ppiTable + ip + 1, (pW32Job->uProcessCount - ip - 1) * sizeof(PPROCESSINFO));
            ...
            TAGMSG2(DBGTAG_Job, "RemoveProcessFromJob: ppi 0x%x removed from pW32Job 0x%x", ppi, pW32Job);
            return TRUE;
        }
    }

    TAGMSG2(DBGTAG_Job, "RemoveProcessFromJob: ppi 0x%x not found in pW32Job 0x%x", ppi, pW32Job);
    UserAssert(0);
    return FALSE;
}
...
void SetProcessFlags(PW32JOB pW32Job, PPROCESSINFO ppi)
{
    ...
    TAGMSG3(DBGTAG_Job, "SetProcessFlags: pW32Job 0x%x ppi 0x%x restrictions 0x%x", pW32Job, ppi, pW32Job->restrictions);
...
* 30-Jul-1997 CLupu   Created.
*/
BOOL JobCalloutAddProcess(PW32JOB pW32Job, PPROCESSINFO ppi)
{
    ...
    if (!(ppi->W32PF_Flags & W32PF_PROCESSCONNECTED)) {
        TAGMSG2(DBGTAG_Job, "JobCalloutAddProcess: pW32Job 0x%x ppi 0x%x not yet initialized", pW32Job, ppi);
        return FALSE;
    }

    TAGMSG2(DBGTAG_Job, "JobCalloutAddProcess: pW32Job 0x%x ppi 0x%x", pW32Job, ppi);
    ...
    if (pW32Job->uProcessCount == pW32Job->uMaxProcesses) {
        ...
        if (pW32Job->uMaxProcesses == 0) {
            ...
            UserAssert(pW32Job->ppiTable != NULL);
            ppiTable = UserReAllocPool(pW32Job->ppiTable, pW32Job->uMaxProcesses * sizeof(PPROCESSINFO), (pW32Job->uMaxProcesses + JP_DELTA) * sizeof(PPROCESSINFO), TAG_W32JOBEXTRA);
        }
...
* 30-Jul-1997 CLupu   Created.
*/
BOOL JobCalloutTerminate(PW32JOB pW32Job)
{
    CheckCritIn();
