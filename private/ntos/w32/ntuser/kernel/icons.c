...
/*
* Function to arrange all icons for a particular window.  Does this by Returns 0 if no icons or the height of one icon row if there are any icons.
...
UINT xxxArrangeIconicWindows(PWND pwnd)
{
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++) {
        if (((pwndTest = RevalidateHwnd(*phwnd)) == NULL) || !TestWF(pwndTest , WFVISIBLE) || pwndTest == pwndSwitch || (pcp = (CHECKPOINT *)_GetProp(pwndTest, PROP_CHECKPOINT, PROPF_INTERNAL)) == NULL) {
            *phwnd = NULL;
            continue;
        }
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++) {
        ...
        for (phwndSort = pbwl->rghwnd; phwndSort < phwnd; phwndSort++) {
            if (*phwndSort == NULL || (pwndSort = RevalidateHwnd(*phwndSort)) == NULL)
                continue;

            pcpSort = (CHECKPOINT*)_GetProp(pwndSort, PROP_CHECKPOINT, PROPF_INTERNAL);
            ...
            switch (SYSMET(ARRANGE) & ~ARW_HIDE) {
                case ARW_BOTTOMLEFT | ARW_RIGHT:
                    ...
                    if (((ptSort.y == ptSrc.y) && (ptSort.x > ptSrc.x)) || (ptSort.y < ptSrc.y))
                        fBreak = TRUE;
                    break;
                case ARW_BOTTOMLEFT | ARW_UP:
                    // Lower left, moving vertically
                    if (((ptSort.x == ptSrc.x) && (ptSort.y < ptSrc.y)) || (ptSort.x > ptSrc.x))
                        fBreak = TRUE;
                    break;
                case ARW_BOTTOMRIGHT | ARW_LEFT:
...
JustParkEm:
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++)
    {
        ...
        pcp = (CHECKPOINT *)_GetProp(pwndTest, PROP_CHECKPOINT,  PROPF_INTERNAL);
        if (pcp != NULL) {
    ...
    for (phwnd = pbwl->rghwnd; *phwnd != (HWND)1; phwnd++)
    {
        ...
        psmwp = _DeferWindowPos(psmwp, pwndTest, NULL, pcp->ptMin.x, pcp->ptMin.y, SYSMET(CXMINIMIZED), SYSMET(CYMINIMIZED), SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOCOPYBITS);
