...
/*
* Menu windows involved in drag drop are registered as targets. This function is called from the client side IDropTarget functions so the menu code can update the selection given the mouse position
...
void xxxMNUpdateDraggingInfo (PMENUSTATE pMenuState, ULONG_PTR uHitArea, UINT uIndex)
{
    ...
    /*
     * Remember current dragging area so we can detected when crossing item/gap boundries.
     */
     ...
     *  ON the item and a drop BEFORE/AFTER the item; there is no actual space between items so we define a virtual gap
     */
    pMenuState->uDraggingFlags = 0;
    if (pItem != NULL) {
        /*
        ...
        if ((y < (int)pItem->yItem)
            ...
            RIPMSG4(RIP_ERROR, "xxxMNUpdateDraggingInfo: y Point not in selected item. "
                               "pwnd:%#lx ppopup:%#lx Index:%#lx pItem:%#lx", pMenuState->uDraggingHitArea, ppopup, pMenuState->uDraggingIndex, pItem);
        }
#endif
    ...
    if (!fCross) {
        ...
        switch (iIndexDelta) {
            ...
            case 1:
                 ...
                 */
                fCross = !((pMenuState->uDraggingFlags == MNGOF_TOPGAP) && (uLastFlags == MNGOF_BOTTOMGAP));
                break;
            case -1:
                /*
                 * We've moved to the previous item
                 */
                fCross = !((pMenuState->uDraggingFlags == MNGOF_BOTTOMGAP) && (uLastFlags == MNGOF_TOPGAP));
                break;
            default:
                /*
    ...
    if (fCross) {
        ...
        xxxMNSetGapState(pMenuState->uDraggingHitArea, pMenuState->uDraggingIndex, pMenuState->uDraggingFlags, TRUE);
    }
