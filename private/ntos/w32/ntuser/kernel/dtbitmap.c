...
*/
{
    ...
    RTL_QUERY_REGISTRY_TABLE BaseServerRegistryConfigurationTable[] = {
        {NULL, RTL_QUERY_REGISTRY_DIRECT, L"CurrentBuildNumber", &UserBuildString, REG_NONE, NULL, 0},
        {NULL, RTL_QUERY_REGISTRY_DIRECT, L"CurrentType",        &UserTypeString,  REG_NONE, NULL, 0},
     ...
     */
      ...
      */
    swprintf(wszProductName, wszPID, ((USER_SHARED_DATA->NtProductType == NtProductWinNt) ? wszPro : wszSrv));
    ...
    if (Verbose) {
        ...
        swprintf(wszT, UserCSDString.Length == 0 ? Title1 : Title2, UserBuildString.Buffer, UserCSDString.Buffer, USER_SHARED_DATA->NtSystemRoot);
    }
    else {
        ...
        swprintf(wszT, UserCSDString.Length == 0 ? Title1 : Title2, UserBuildString.Buffer, UserCSDString.Buffer, s);
    }
}
...
*/
{
     ...
     * The global name is an at most 8 character name with no extension. 
     * It is "winnt" for workstation or "lanmannt" for server or server upgrade. 
     * It is followed by 256 it is for 256 color devices.
     */
...
*/
{
    ...
    if ((lpszFile == NULL) || (lpszFile == SETWALLPAPER_DEFAULT) || (lpszFile == SETWALLPAPER_METRICS)) {
         ...
         */
        if (!FastGetProfileStringFromIDW(pProfileUserName, PMAP_DESKTOP, STR_DTBITMAP, wszNone, lpszBitmap, GDWPN_BITSIZE)) {
...
*/
{
    ...
    if (hbmNew = GreCreateCompatibleBitmap(hdc, bm.bmWidth, bm.bmHeight)) {
         ...
         * halftoning.  Since otherwise, the colors won't translate correctly.
         */
         ...
         */
        GreStretchBlt(ghdcMem2, 0, 0, bm.bmWidth, bm.bmHeight, ghdcMem, 0, 0, bm.bmWidth, bm.bmHeight, SRCCOPY, 0);
...
HPALETTE CreatePaletteFromBitmap(HBITMAP hbm)
{
    ...
    ppal->palNumEntries = (WORD)GreGetDIBColorTable(ghdcMem, 0, MAXPAL, (LPRGBQUAD)ppal->palPalEntry);
    GreSelectBitmap(ghdcMem, hbmT);

    /*
     * Create a halftone-palette if their are no entries.  Otherwise,
     * swap the RGB values to be palentry-compatible and create us a palette.
     */
...
*/
{
     ...
     * be called as part of ExpandBitmap, before srcWallpaper is set.
     */
        ...
        if (hbmT = GreSelectBitmap(ghdcMem, ghbmWallpaper)) {
            for (y = yO; y < lprc->bottom; y += bm.bmHeight) {
                for (x = xO; x < lprc->right; x += bm.bmWidth) {
                    GreBitBlt(hdc, x, y, bm.bmWidth, bm.bmHeight, ghdcMem, 0, 0, SRCCOPY, 0);
                }
            }
...
*/
{
    ...
    if (GetWallpaperCenterRect(&rc, &pt, lprcMonitor)) {
        /*
         * This used to call TileWallpaper, but this really slowed up the system for small dimension bitmaps.
         * We really only need to blt it once for centered bitmaps.
         */
        if (hbmT = GreSelectBitmap(ghdcMem, ghbmWallpaper)) {
            GreBitBlt(hdc, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, ghdcMem, pt.x, pt.y, SRCCOPY, 0);
            GreSelectBitmap(ghdcMem, hbmT);
        }
        else {
            f = FALSE;
...
* If the (fPaint) is false, then we only to palette realization and no drawing.
...
/*
* Expand this bitmap to fit the screen.  This is used for tiled images only.
...
BOOL xxxLoadDesktopWallpaper(LPWSTR lpszFile)
{
    ...
    if (gpDispInfo->cMonitors == 1 && gpsi->BitCount <= bm.bmPlanes * bm.bmBitsPixel) {
...
SDW_Exit:
     ...
     */
    if ((pwndShell != NULL) && ((hbmOld && !ghbmWallpaper) || (!hbmOld && ghbmWallpaper))) {
        ThreadLockAlways(pwndShell, &tl);
        xxxSendNotifyMessage(pwndShell, WM_SHELLNOTIFY, SHELLNOTIFY_WALLPAPERCHANGED, (LPARAM)ghbmWallpaper);
        ThreadUnlock(&tl);
    }
    ...
    PMONITOR    pMonitor)
{
    ...
    GreGetTextExtentW(hdc, pwszT, wcslen(pwszT), &size, GGTE_WIN3_EXTENT);

    x = pMonitor->rcWork.right - size.cx - 5;
    y = pMonitor->rcWork.bottom - ((gDrawVersionAlways ? 3 : 2) * size.cy) - 5;
    ...
    GreExtTextOutW(hdc, rcText1.right, rcText1.bottom, 0, &rcText1, wszProductName, wcslen(wszProductName), (LPINT)NULL);
    ...
    GreExtTextOutW(hdc, rcText1.right, rcText1.bottom, 0, &rcText1, wszProductBuild, wcslen(wszProductBuild), (LPINT)NULL);
    ...
    if (gDrawVersionAlways) {
        ...
        GreExtTextOutW(hdc, rcText1.right, rcText1.bottom, 0, &rcText1, USER_SHARED_DATA->NtSystemRoot, wcslen(USER_SHARED_DATA->NtSystemRoot), (LPINT)NULL);
    }
    ...
    LPARAM          dwData)
{
    ...
    if (**((PULONG *)&InitSafeBootMode)) {
        ...
        if (!gbDesktopLocked) {
             ...
             */
            if (ghbmWallpaper) {
                f = xxxDrawWallpaper(pwnd, hdc, pMonitor, lprcMonitorClip);
            }
            else {
    ...
    if (**((PULONG *)&InitSafeBootMode) || gDrawVersionAlways || gdwCanPaintDesktop) {
...
void xxxInvalidateDesktopOnPaletteChange(PWND pwnd)
{
    ...
    if (!pwndShell) {
        ...
        xxxRedrawWindow(pwndShell, NULL, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_ALLCHILDREN);
     ...
     */
    if (fRedrawDesktop) {
        xxxRedrawWindow(pwnd, &rc, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_ALLCHILDREN);
    }
}
