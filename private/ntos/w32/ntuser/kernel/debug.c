...
* Force an exception on the active application's context so it will break into the debugger.
...
*/
{
    ...
    if (fsModifiers & MOD_CONTROL) {
        ...
        /*
         * Bail out if the process is not being debugged or the process id is invalid.
         */
    ...
    if (CsrApiPort != NULL) {
        ...
        m.ApiNumber = CSR_MAKE_API_NUMBER( USERSRV_SERVERDLL_INDEX, UserpActivateDebugger);
        LeaveCrit();
    ...
    /*
     * Don't eat this event unless we are breaking into CSR! Since we have choosen an arbitrary hot key like F12 for the debug key, we need to pass on the key to the application, or apps that want this key would never see it.
     * If we had an api for installing a debug hot key (export or MOD_DEBUG flag to RegisterHotKey()), then it would be ok to eat because the user selected the hot key.
     * But it is not ok to eat it as long as we've picked an arbitrary hot key. scottlu.
     */
