...
artsquickndirtybugfix:
    ...
    UserAssert(FALSE);// We should never get here!
}
...
UINT MNFindItemInColumn(PMENU pMenu, UINT idxB, int dir, BOOL fRoot)
{
    ...
    while (cItems-- > 0 && (idxB = MNFindNextValidItem(pMenu, idxB, dir, 0)) != idxE)
    {
        pItem = &pMenu->rgItems[idxB];
...
* Translates Virtual cursor key movements into pseudo-ascii values.  Maps a character to an item number.
*/
UINT xxxMNFindChar(PMENU pMenu, UINT ch, int idxC, LPINT lpr)       /* Put match type here */
{
    ...
    do {
        ...
        if (pItem->lpstr != NULL) {
            if (pItem->cch != 0) {
                ...
                if (((rT = (UINT)xxxClientFindMnemChar(&strMnem, (WCHAR)ch, TRUE, TRUE)) == 0x0080) && (idxF == MFMWFP_NOITEM))
                    idxF = idxC;
            }
        }
        if (idxC == idxPrev) {
