    ...
    IN PUNICODE_STRING RegistryPath)
{
     ...
     * any threads.
     */
    gpW32FastMutex = ExAllocatePoolWithTag(NonPagedPool, sizeof(FAST_MUTEX), TAG_SYSTEM);
    if (gpW32FastMutex == NULL) {
     ...
     */
    countTable = (PULONG)ExAllocatePoolWithTag(NonPagedPool, W32pServiceLimit * sizeof(ULONG), 'llac');
    if (countTable == NULL ) {
        Status = STATUS_NO_MEMORY;
        goto Failure;
    }
...
#define CCLASSES 8
    ...
    CONST static struct {
        ...
        { TRUE, TRUE, FNID_DESKTOP, CS_DBLCLKS, (WNDPROC)xxxDesktopWndProc, sizeof(DESKWND) - sizeof(WND), TRUE, (HBRUSH)(COLOR_BACKGROUND + 1), DESKTOPCLASS},
        ...
        { TRUE, FALSE, FNID_TOOLTIP,
            ...
            NULL,
            TOOLTIPCLASS},
        { TRUE, TRUE, FNID_ICONTITLE, 0, (WNDPROC)xxxDefWindowProc, 0, TRUE, NULL, ICONTITLECLASS},
        { FALSE, FALSE, 0, 0, (WNDPROC)xxxEventWndProc, sizeof(PSVR_INSTANCE_INFO), FALSE, NULL, L"DDEMLEvent"},
#ifdef HUNGAPP_GHOSTING
        { TRUE, TRUE, 0, 0, (WNDPROC)xxxGhostWndProc, 0, TRUE, NULL, L"Ghost"},
#endif // HUNGAPP_GHOSTING
        { TRUE, TRUE, 0, 0, (WNDPROC)xxxDefWindowProc, 4, TRUE, NULL, L"Message"}
    };
    ...
    for (i = 0; i < CCLASSES; i++)
    {
        ...
        if (InternalRegisterClassEx(&wndcls, rc[i].fnid, CSF_SERVERSIDEPROC | CSF_WIN40COMPAT) == NULL) {
            RIPMSG0(RIP_WARNING, "LW_RegisterWindows: InternalRegisterClassEx failed");
            return FALSE;
        }

        if (fSystem && rc[i].fGlobalClass) {
            if (InternalRegisterClassEx(&wndcls, rc[i].fnid, CSF_SERVERSIDEPROC | CSF_SYSTEMCLASS | CSF_WIN40COMPAT) == NULL) {
                RIPMSG0(RIP_WARNING, "LW_RegisterWindows: InternalRegisterClassEx failed");
                return FALSE;
            }
        }
...
* This routine gets called by the client to tell the kernel where its important functions can be located.
