...
/*
* Replace a system (aka 'public') cursor with a user provided one.  The new cursor is pulled from a file (.CUR, .ICO, or .ANI) specified in WIN.INI.
...
#define pacon ((PACON)pcur)
    ...
    /*
     * All ssytem images being replaced should have ordinal names and reference the USER module and be unowned.
     */
     ...
     * incorrectly - read the comments in user.h and wingdi.w around tagCURSOR, tagACON, and CURSINFO.
     */
    ...
    pcur->CURSORF_flags = (pcur->CURSORF_flags    & ~CURSORF_ACON) | (pcurSys->CURSORF_flags &  CURSORF_ACON);
    pcurSys->CURSORF_flags = (pcurSys->CURSORF_flags & ~CURSORF_ACON) | CURSORF_flags;

    /*
     * If we swapped acons into pcur, then we need to change the ownerhsip to make sure they can get destroyed
     */
...
{
    /*
     * If this is only a single cursor (not an ACON) just return it and a frame count of 1.
     */
    if (!(pcur->CURSORF_flags & CURSORF_ACON)) {
        *pccur = 1;
        *pjifRate = 0;
...
/*
* Free all the individual cursors that make up the frames of an animated icon.

* WARNING: DestroyAniIcon assumes that all fields that an ACON shares with a cursor will be freed by some cursor code (probably the cursor function that calls this one).
...
/*
* Links unlinked cursor into the apropriate icon cache IFF its the type of cursor that needs to be in the cache.

* Note that changing ownership if cursor objects needs to keep this cache linking in mind.
* The unlink routine in DestroyEmptyCursorObject() will handle public cursor objects made local but that is all.
...
VOID LinkCursor(PCURSOR pcur)
{
    ...
    /*
     * We don't cache acon frames because they all belong to the root acon object.

     * We don't cache process owned objects that are not LRSHARED either.
     */
...
* pcurData are not captured.  If a fault occurs in this routine, all allocated memory will be freed when the cursors are destroyed.

* Critical side effect:  If this function fails, the bitmaps must NOT have been made public.  (See CreateIconIndirect()).
...
#define pacon ((PACON)pcur)
    ...
    if (pcurData->CURSORF_flags & CURSORF_ACON) {
        ...
        RtlCopyMemory(&pacon->cpcur, &pcurData->cpcur, sizeof(ACON) - FIELD_OFFSET(ACON, cpcur));
    } else {
        RtlCopyMemory(&pcur->CI_COPYSTART, &pcurData->CI_COPYSTART, sizeof(CURSOR) - FIELD_OFFSET(CURSOR, CI_COPYSTART));
    }
