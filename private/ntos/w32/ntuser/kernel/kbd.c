...
BOOL xxxNumpadCursor(PKE pKe)
{
    ...
    if (bLastNumpadCursor) {
        if (bLastNumpadCursor == (BYTE)(pKe->usFlaggedVk)) {
            ...
            if (pKe->usFlaggedVk & KBDBREAK) {
                /*
                 * Numpad Cursor key came back up. Send this key now, and make sure that the Shift key will then appear to go back down again.
                 */
                xxxKeyEvent(pKe->usFlaggedVk, pKe->bScanCode, pKe->dwTime, 0, FALSE);
                bLastNumpadCursor = 0;
                pKe->usFlaggedVk = VkFakedShiftUp;
                pKe->bScanCode = VscFakedShiftUp;
            }
            /*
             * Going down: this key is repeating, so just pass it on unaltered and keep the KEProc active
             */
            return TRUE;
        } else {
            /*
             * It is a different key.  Fake the Shift key back down again, and continue (it may be another Numpad Cursor key)
             */
            xxxKeyEvent(VkFakedShiftUp, (WORD)(VscFakedShiftUp | SCANCODE_SIMULATED), pKe->dwTime, 0, FALSE);
            bLastNumpadCursor = 0;
        }
    }
    ...
    if (pKe->usFlaggedVk & KBDNUMPAD) {
         ...
         * Here, if NumLock is set, we change the virtual keycodes to numeric VK_NUMPAD codes, so the keys will be translated as numbers etc.
         * But if a shift key is down, we handle these as cursor keys, but we need to make sure that these are seen as UNSHIFTED
         */
        ...
        if (IsSAS((BYTE)(pKe->usFlaggedVk), &fsModifiers)) {
            ...
            if (TestRawKeyDown(VK_SHIFT)) {
                /*
                 * key is down (bit(s) set in BIOS key state), so we are going to keep this as a cursor key.
                 * To do this, we need to make sure that Windows' state vector entry for VK_SHIFT is OFF even though a shift key is actually down.
                 */
                ...
                xxxKeyEvent((USHORT)(VkFakedShiftUp | KBDBREAK), (WORD)(VscFakedShiftUp | SCANCODE_SIMULATED), pKe->dwTime, 0, FALSE);
                return TRUE;
            }

            /*
             * NumLock ON but Shift key up: Alter the Virtual Key event, but not for injected virtual keys.
             */
...
* This function will leave the critical section only if low level hooks are installed ant the call to xxxKeyEvent is made
*/
