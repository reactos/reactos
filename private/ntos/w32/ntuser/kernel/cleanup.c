...
VOID PseudoDestroyClassWindows(PWND pwndParent, PCLS pcls)
{
    ...
    for (pwnd = pwndParent->spwndChild; pwnd != NULL; pwnd = pwnd->spwndNext) {
        ...
        if (pwnd->pcls == pcls && pti == GETPTI(pwnd)) {
             ...
             */
            RIPMSG1(RIP_WARNING, "USER: Wow Window not destroyed: %lX", pwnd);
...
VOID _WOWModuleUnload(HANDLE hModule)
{
    ...
    for (pheT = gSharedInfo.aheList; pheT <= pheMax; pheT++) {
        ...
        PWND pwnd;
        if ((pheT->bType == TYPE_WINDOW) && (ptiTest->TIF_flags & TIF_16BIT) && (ptiTest->ppi == ppi)) {
            pwnd = (PWND) pheT->phead;
            if (!TestWF(pwnd, WFSERVERSIDEPROC) && IsWOWProc(pwnd->lpfnWndProc) && (pwnd->hMod16 == (WORD)(ULONG_PTR)hModule)) {
                pwnd->lpfnWndProc = (WNDPROC_PWND)gpsi->apfnClientA.pfnDefWindowProc;
            }
        }
    }
    ...
    /*
     * Destroy private classes identified by hInstance that are not referenced by any windows.
...
VOID _WOWCleanup(HANDLE hInstance, DWORD hTaskWow)
{
    ...
    if (hInstance != NULL) {
        ...
        for (pheT = gSharedInfo.aheList; pheT <= pheMax; pheT++) {
            PTHREADINFO ptiTest = (PTHREADINFO)pheT->pOwner;
            if ((pheT->bType == TYPE_WINDOW) && (ptiTest->TIF_flags & TIF_16BIT) && (ptiTest->ptdb) && (ptiTest->ptdb->hTaskWow == hTaskWow) && (ptiTest->ppi == ppi)) {
                pwnd = (PWND) pheT->phead;
                if (!TestWF(pwnd, WFSERVERSIDEPROC)) {
                    pwnd->lpfnWndProc = (WNDPROC_PWND)gpsi->apfnClientA.pfnDefWindowProc;
                }
    ...
    for (i = 0; i < 2; ++i) {
        while (*ppcls != NULL) {
            if ((*ppcls)->hTaskWow == hTaskWow && ((*ppcls)->CSF_flags & CSF_WOWDEFERDESTROY)) {
                if ((*ppcls)->cWndReferenceCount == 0) {
                    DestroyClass(ppcls);
                } else {
    ...
    for (pheT = gSharedInfo.aheList; pheT <= pheMax; pheT++) {
         ...
         */
        if (    !(gahti[pheT->bType].bObjectCreateFlags & OCF_PROCESSOWNED) || (PPROCESSINFO)pheT->pOwner != ppi || (((PPROCOBJHEAD)pheT->phead)->hTaskWow != hTaskWow) ||
                (pheT->bType == TYPE_CALLPROC)  /* Do not destroy CALLPROCDATA objects.
                                                      * These should only get nuked when the process goes away or when the class is nuked.
                                                      */
                    )
        {
            continue;
