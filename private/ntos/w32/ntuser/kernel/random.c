...
BOOL xxxUpdateWindows(PWND pwnd, HRGN hrgn)
{
    ...
    /*
     * This function needs to return a value, since it is called through NtUserCallHwndParam.
     */
    return TRUE;
}

/*
* States allowed to be set/cleared by Set/ClearWindowState. If you're allowing a new flag here, you must make sure it won't cause an AV in the kernel if someone sets it maliciously.
*/
...
{
    if ((pwndFilter == NULL) || (pwndFilter == pwnd) || ((pwndFilter == (PWND)1) && (pwnd == NULL))) {
        return TRUE;
    }
...
#if DBG
        if (hbrush != 0)
            RIPMSG2(RIP_WARNING, "Invalid HBRUSH from WM_CTLCOLOR*** msg %lX brush %lX", message, hbrush);
#endif
...
HBRUSH xxxGetControlBrush(PWND pwnd, HDC hdc, UINT message)
{
    ...
    if ((pwndSend = (TestwndPopup(pwnd) ? pwnd->spwndOwner : pwnd->spwndParent)) == NULL)
        pwndSend = pwnd;
...
UINT xxxHardErrorControl(DWORD dwCmd, HANDLE handle, PDESKRESTOREDATA pdrdRestore)
{
    ...
    case HardErrorInDefDesktop:
         ...
         */
        if (ISTS()) {
            if ((grpdeskRitInput == NULL) || ((grpdeskRitInput == gspdeskDisconnect) && (gspdeskShouldBeForeground == NULL)) || ((grpdeskRitInput == gspdeskDisconnect) && (gspdeskShouldBeForeground == gspdeskDisconnect))) {
                return HEC_ERROR;
            }
        }
        ...
        if (pstrName && (!_wcsicmp(TEXT("Winlogon"), pstrName->Buffer) || !_wcsicmp(TEXT("Disconnect"), pstrName->Buffer) || !_wcsicmp(TEXT("Screen-saver"), pstrName->Buffer))) {
    ...
    case HardErrorAttachNoQueue:
        ...
        if (handle != NULL) {
            Status = ObReferenceObjectByHandle(handle, THREAD_QUERY_INFORMATION, *PsThreadType, UserMode, &Thread, NULL);
            if (NT_SUCCESS(Status)) {
