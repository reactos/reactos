...
void GetSKT1TextMetric(LPTEXTMETRIC   lptm)
{
    ...
    if (GetTextExtentPoint32(hDC, &wcDbcs, 1, &szDbcs) && lptm->tmMaxCharWidth < szDbcs.cx) {
        TAGMSG2(DBGTAG_IMM, "GetSKT1TextMetric: tmMaxCharWidth(%d) is smaller than real width(%d).", lptm->tmMaxCharWidth, szDbcs.cx);
...
#undef INIT_WIDTH
    ...
    xStartPos += (lpSKT1Ctxt->nButtonWidth[SKT1_ALT_TYPE] + SKT1_XIN) + (lpSKT1Ctxt->nButtonWidth[SKT1_ESC_TYPE] + SKT1_XIN);
    // ctrl 2
...
void SKT1DrawLabel(HDC hDC, LPSKT1CTXT lpSKT1Ctxt, LPCWSTR lpszLabel)
{
    ...
    for (i = 0; i < SKT1_LETTER_KEY_NUM; i++) {
        BitBlt(hDC, lpSKT1Ctxt->ptButtonPos[i].x, lpSKT1Ctxt->ptButtonPos[i].y, SKT1_LABEL_BMP_X, SKT1_LABEL_BMP_Y, hMemDC, i * SKT1_LABEL_BMP_X, 0, SRCCOPY);
    }
...
void InitSKT1Bitmap(HWND hSKWnd, LPSKT1CTXT lpSKT1Ctxt)
{
    ...
    lpSKT1Ctxt->hSKBitmap = CreateCompatibleBitmap(hDC, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top);
    ...
    Rectangle(hMemDC, rcClient.left, rcClient.top, rcClient.right + 1, rcClient.bottom + 1);
...
void DestroyT1Window(HWND hSKWnd)
{
    ...
    if (lpSKT1Ctxt->ptSkOffset.x != SKT1_NOT_DRAG && lpSKT1Ctxt->ptSkOffset.y != SKT1_NOT_DRAG) {
    ...
    UINT       uKeyIndex)
{
    ...
    if (nWidth) {
        ...
        PatBlt(hDC, lpSKT1Ctxt->ptButtonPos[uKeyIndex].x - 1, lpSKT1Ctxt->ptButtonPos[uKeyIndex].y - 1, nWidth + 2, nHeight + 2, DSTINVERT);
    }
...
BOOL SKT1SetCursor(HWND hSKWnd, LPARAM lParam)
{
    ...
    if (lpSKT1Ctxt->uKeyIndex < SKT1_TOTAL_INDEX) {
        ...
        keybd_event((BYTE)uVirtKey, (BYTE)guScanCode[uVirtKey], (DWORD)KEYEVENTF_KEYUP, 0);
    ...
    if (uKeyIndex < SKT1_TOTAL_INDEX) {
        ...
        if (SKT1IsValidButton(uKeyIndex, lpSKT1Ctxt)) {
            ...
            keybd_event((BYTE)uVirtKey, (BYTE)guScanCode[uVirtKey], 0, 0);
...
LRESULT SetSKT1Data(HWND          hSKWnd, LPSOFTKBDDATA lpSoftKbdData)
{
    ...
    for (i = 0; i < SKT1_LETTER_KEY_NUM; i++) {
        ...
        rcOpaque.left = lpSKT1Ctxt->ptButtonPos[i].x + SKT1_LABEL_BMP_X - SKT1_XOVERLAP;
        rcOpaque.top = lpSKT1Ctxt->ptButtonPos[i].y + SKT1_LABEL_BMP_Y;
        rcOpaque.right = rcOpaque.left + lpSKT1Ctxt->nButtonWidth[SKT1_LETTER_TYPE] - SKT1_LABEL_BMP_X + SKT1_XOVERLAP;
        rcOpaque.bottom = rcOpaque.top + lpSKT1Ctxt->nButtonHeight[0] - SKT1_LABEL_BMP_Y;

        ExtTextOut(hMemDC, rcOpaque.left, rcOpaque.top, ETO_OPAQUE, &rcOpaque, (LPWSTR)&lpSKT1Ctxt->wCodeTable[i], cbCount, NULL);
    }
