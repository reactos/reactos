 ...
 * Only the API's that WIN32 implements as opposed to ICASRV are entered here.
 * The rest are NULL so that the same WinStation API numbers may be used by ICASRV and WIN32.
 * If this table is changed, the table below must be modified too, as well as the API dispatch table in the ICASRV.
*/

/*
 * If you  ever change the index of the function pointers in this array make sure API_W32WINSTATIONTERMINATE has the right index for W32WinStationTerminate
*/
...
*/
{
    ...
    Status = RtlCreateUserThread(NtCurrentProcess(), NULL, TRUE, 0L, 0L, 0L, TerminalServerRequestThread, NULL, &ThreadHandle, &ClientId);
    ...
    Status = NtSetInformationThread(ThreadHandle, ThreadBasePriority, &Priority, sizeof(Priority));
    if (!NT_SUCCESS(Status)) {
...
NTSTATUS TerminalServerRequestThread(IN PVOID ThreadParameter)
{
     ...
     *  Connect to user
     */
    hevtTermSrvInit = CreateEvent(NULL, TRUE, FALSE, L"Global\\TermSrvReadyEvent");
    ...
    // The session manager creates and resume a new session and returns to Terminal server the session id of the new session.
    // There is a race condition where CSR can resume and call into terminal server before terminal server can store the session id in its internal structure.
    // To prevent this CSR will wait here on a named event which will be set by Terminal server once it gets the sessionid for the newly created session
    if (NtCurrentPeb()->SessionId != 0) {
        CsrStartHandle = CreateEvent(NULL, TRUE, FALSE, L"CsrStartEvent");
        if (!CsrStartHandle) {
            RIPMSG1(RIP_WARNING, "TerminalServerRequestThread: Failed to create 'CsrStartEvent' with Status 0x%x", Status);
            return Status;
        } else {
    ...
    for (;;) {
        ...
        Status = NtRequestWaitReplyPort(ghportLPC, (PPORT_MESSAGE)&ApiMsg, (PPORT_MESSAGE)&ApiMsg);
        if (!NT_SUCCESS(Status)) {
        ...
        if (ApiMsg.ApiNumber >= SMWinStationMaxApiNumber ) {
            ...
            if (pDispatch->pWin32ApiProc) {
                ...
                /*
                 * For all the win32k callouts with the exception of terminate set this thread to the current desktop
                 */
...
#if DBG
VOID W32WinStationDumpReconnectInfo(WINSTATIONDORECONNECTMSG *pDoReconnect, BOOLEAN bReconnect)
{
    PSTR pCallerName;
...
NTSTATUS W32WinStationDoConnect(PWINSTATION_APIMSG pMsg)
{
    ...
    /*
     * Populate the sessions \DosDevices from the current consoles settings
     */
    ...
    memcpy(G_WinStationName, m->WinStationName, min(sizeof(G_WinStationName), sizeof(m->WinStationName)));
    ...
    memcpy(DoConnectData.WinStationName, G_WinStationName, min(sizeof(G_WinStationName), sizeof(DoConnectData.WinStationName)));
    ...
    Status = NtUserRemoteConnect(&DoConnectData, sizeof(DisplayDriverName), DisplayDriverName);
    if (!NT_SUCCESS(Status)) {
        DBGHYD(("NtUserRemoteConnect failed with Status %lx\n", Status));
        goto Exit;
    }

    Status = RtlCreateUserThread( NtCurrentProcess(), NULL, TRUE, 0L, 0L, 0L, Win32CommandChannelThread, NULL, &ThreadHandle, &ClientId );
...
NTSTATUS W32WinStationDoReconnect(PWINSTATION_APIMSG pMsg)
{
    ...
    memcpy(G_WinStationName, m->WinStationName, min(sizeof(G_WinStationName), sizeof(m->WinStationName)));
    memcpy(DoReconnectData.WinStationName, G_WinStationName, min(sizeof(G_WinStationName), sizeof(DoReconnectData.WinStationName)));
     ...
     * Give the information to the WIN32 driver.
     */
    Status = (NTSTATUS)NtUserCallOneParam((ULONG_PTR)&DoReconnectData, SFI_XXXREMOTERECONNECT);
    if (!NT_SUCCESS(Status)) {
...
extern HANDLE WinStationIcaApiPort;
    ...
    if (gdwHardErrorThreadId != 0) {
        ...
        /*
         * Poll (!!?) for hard error thread completion.  The thread does not exit.
         */
    ...
    hevtShutDown = OpenEvent(EVENT_ALL_ACCESS, FALSE, L"EventShutDownCSRSS");
    if (hevtShutDown == NULL) {
        /*
         * This case is for cached sessions where RIT and Destiop thread have not been created.
         */
    ...
    hevtRitExited = CreateEvent(NULL, FALSE, FALSE, L"EventRitExited");
    UserAssert(hevtRitExited != NULL);
    hevtRitStuck = CreateEvent(NULL, FALSE, FALSE, L"EventRitStuck");
    UserAssert(hevtRitStuck != NULL);

    /*
     * RIT is created. Signal this event that starts the cleanup in win32k
     */
    ...
    while (1) {
        ...
        case WAIT_OBJECT_0 + 1:
            /*
             * The RIT is stucked because there are still GUI threads assigned to desktops.
             * One reason for this is that winlogon died w/o calling ExitWindowsEx.
             */
...
NTSTATUS W32WinStationThinwireStats(PWINSTATION_APIMSG pMsg)
{
    ...
    Status = (NTSTATUS)NtUserCallOneParam((ULONG_PTR)&m->Stats, SFI_REMOTETHINWIRESTATS);
    if (!NT_SUCCESS(Status)) {
...
NTSTATUS W32WinStationShadowStart(PWINSTATION_APIMSG pMsg)
{
    ...
    Status = (NTSTATUS)NtUserCallTwoParam((ULONG_PTR)m->pThinwireData, (ULONG_PTR)m->ThinwireDataLength, SFI_REMOTESHADOWSTART);
    if (!NT_SUCCESS(Status)) {
...
NTSTATUS W32WinStationShadowCleanup(PWINSTATION_APIMSG pMsg)
{
    ...
    Status = (NTSTATUS)NtUserCallTwoParam((ULONG_PTR)m->pThinwireData, (ULONG_PTR)m->ThinwireDataLength, SFI_REMOTESHADOWCLEANUP);
    if (!NT_SUCCESS(Status)) {
...
NTSTATUS CleanupSessionObjectDirectories(VOID)
{
    ...
    Status = NtOpenDirectoryObject(&DosDevicesDirectory,DIRECTORY_ALL_ACCESS,&Attributes);
    if (!NT_SUCCESS(Status)) {
...
Restart:
    ...
    while (TRUE) {
        Status = NtQueryDirectoryObject( DosDevicesDirectory, (PVOID)DirInfo, sizeof(DirInfoBuffer), TRUE, RestartScan, &Context, &ReturnedLength);
        ...
        if (!wcscmp(DirInfo->TypeName.Buffer, L"SymbolicLink")) {
            ...
            InitializeObjectAttributes( &Attributes, &DirInfo->Name, OBJ_CASE_INSENSITIVE, DosDevicesDirectory, NULL);
            Status = NtOpenSymbolicLinkObject( &LinkHandle, SYMBOLIC_LINK_ALL_ACCESS, &Attributes);
            if (NT_SUCCESS(Status)) {
