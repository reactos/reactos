...
*/
{
    ...
    rc = BroadcastSystemMessage( pmsg->dwFlags, &pmsg->dwRecipients, pmsg->uiMessage, pmsg->wParam, tmpLPARAM );
...
NTSTATUS RemoteDoSendWindowMessage(PWINSTATION_APIMSG pMsg)
{
    ...
    rc = (LONG)SendMessage( pmsg->hWnd, pmsg->Msg, pmsg->wParam, tmpLPARAM  );
    status = STATUS_SUCCESS;
    pmsg->Response = rc;
    return status ;
}