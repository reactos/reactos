...
DWORD main(int argc, char *argv[])
{
    ...
    Palette = LocalAlloc(LMEM_FIXED, sizeof(LOGPALETTE) + 256 * sizeof(PALETTEENTRY));
...
#if 0
call HANDLE APIENTRY CreateConsoleScreenBuffer(
...
#define CONSOLE_GRAPHICS_BUFFER  2
#endif
    hgo = CreateConsoleScreenBuffer(MAXIMUM_ALLOWED, 0, NULL, CONSOLE_GRAPHICS_BUFFER, &cgbi);
    if (hgo == NULL) {
        DbgPrint("CreateConsoleScreenBuffer failed\n");
        return FALSE;
    }
    Success = SetConsolePalette(hgo, hPalette, SYSPAL_NOSTATIC);
    if (!Success) {
     ...
     */
    for (i = 0; i < 16; i++) {
        memcpy(&Palette->palPalEntry[i], MyPalette, 64);
    }
    ...
    Success = SetConsolePalette(hgo, hPalette, SYSPAL_STATIC);
    if (!Success) {
