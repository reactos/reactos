...
/*++
    This routine initializes ctrl handling.
    It is called by AllocConsole and the dll initialization code.
--*/
{
...
/*++
Routine Description:
    This thread is created when ctrl-c or ctrl-break is entered, or when close is selected.
...
--*/
{
    ...
    // If this bit is set, it means we don't want to cause this process to exit itself if it is a logoff or shutdown event.
    fNoExit = 0x80000000 & EventNumber;
    EventNumber &= ~0x80000000;

    // the ctrl_close event is set when the user selects the window close option from the system menu, or EndTask, or Settings-Terminate.
    ...
    case CTRL_BREAK_EVENT:
        // If the process is being debugged, give the debugger a shot.
        // If the debugger handles the exception, then go back and wait.
        ...
        try {
            ...
            LockDll();
            try {
                if (EventNumber != CTRL_C_EVENT || NtCurrentPeb()->ProcessParameters->ConsoleFlags != IGNORE_CTRL_C) {
    ...
    try {
        if (EventNumber != CTRL_C_EVENT || NtCurrentPeb()->ProcessParameters->ConsoleFlags != IGNORE_CTRL_C) {
            for (i=HandlerListLength;i>0;i--) {
                // Don't call the last handler (the default one which calls ExitProcess() if this process isn't supposed to exit (system process are not supposed to exit because of shutdown or logoff event notification).
                if ((i-1) == 0 && fNoExit) {
                    if (EventNumber == CTRL_LOGOFF_EVENT || EventNumber == CTRL_SHUTDOWN_EVENT) {
                        break;
                    }
                }
...
--*/
{
    ...
    CsrClientCallServer( (PCSR_API_MSG)&m, NULL, CSR_MAKE_API_NUMBER( CONSRV_SERVERDLL_INDEX, ConsolepNotifyLastClose), sizeof( *a ));
}
...
--*/
{
    ...
    // NULL handler routine is not stored in table. It is used to temporarily inhibit ^C event handling
    if (!HandlerRoutine) {
    ...
    if (HandlerListLength == AllocatedHandlerListLength) {
        // grow list
        NewHandlerList = (PHANDLER_ROUTINE *) RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(PHANDLER_ROUTINE) * (HandlerListLength + LIST_INCREMENT));
        if (!NewHandlerList) {
        ...
        if (HandlerList != SingleHandler) {
            RtlFreeHeap(RtlProcessHeap(), 0, HandlerList);// free old list
        }
...
--*/
{
    ...
    // NULL handler routine is not stored in table. It is used to temporarily inhibit ^C event handling.
    // Removing this handler allows normal processing to occur
    if (!HandlerRoutine) {
