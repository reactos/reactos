...
#define DUMP_COUNT 4
    ...
    {
        FsVgaPrint((1, "FSVGA-FSVGAInitialize: Couldn't allocate pool for registry path\n"));
    ...
    if (!deviceNameSuffix.Buffer) {
        FsVgaPrint((1, "FSVGA-FsVgaInitialize: Couldn't allocate string for device object suffix\n"));
    ...
    RtlInitUnicodeString(&fullFsVgaName, NULL);
    fullFsVgaName.MaximumLength = sizeof(L"\\Device\\") + baseFsVgaName.Length + deviceNameSuffix.MaximumLength;
    fullFsVgaName.Buffer = ExAllocatePool(PagedPool, fullFsVgaName.MaximumLength);
    if (!fullFsVgaName.Buffer) {
        FsVgaPrint((1, "FSVGA-FsVgaInitialize: Couldn't allocate string for Full Screen Video device object name\n"));
    ...
    RtlZeroMemory(fullFsVgaName.Buffer, fullFsVgaName.MaximumLength);
    RtlAppendUnicodeToString(&fullFsVgaName, L"\\Device\\");
    RtlAppendUnicodeToString(&fullFsVgaName, baseFsVgaName.Buffer);
    ...
    *deviceExtension = tmpDeviceExtension;
    ...
    resourceDeviceClass.Buffer = ExAllocatePool(PagedPool, resourceDeviceClass.MaximumLength);
    if (!resourceDeviceClass.Buffer) {
        FsVgaPrint((1, "FSVGA-FsVgaInitialize: Couldn't allocate string for resource device class name\n"));
...
#define PARAMETER_MAX 256
    ...
    else
    {
        ...
        {
            FsVgaPrint((1, "FSVGA-FsVgaServiceParameters: Couldn't allocate string for path to parameters for %ws\n", path));
            status = STATUS_UNSUCCESSFUL;
        }
    }
    ...
    RtlInitUnicodeString(&defaultFsVgaName, DD_FULLSCREEN_PORT_BASE_NAME_U);
    RtlCopyUnicodeString(FsVgaDeviceName, &defaultFsVgaName);

    FsVgaPrint((1, "FsVga-FsVgaServiceParameters: Full Screen Video port base name = %ws\n", FsVgaDeviceName->Buffer));
    FsVgaPrint((1, "FsVga-FsVgaServiceParameters: Emulation Mode = %d\n", configuration->EmulationMode));
    FsVgaPrint((1, "FsVga-FsVgaServiceParameters: Hardware Cursor = %d\n", configuration->HardwareCursor));
    FsVgaPrint((1, "FsVga-FsVgaServiceParameters: Hardware Scroll = %d\n", configuration->HardwareScroll));
