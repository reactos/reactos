...
/*
    Converts SourceLength Unicode characters from Source into not more than TargetLength Codepage characters at Target.
...
*/
{
    ...
    Status = RtlUnicodeStringToAnsiString(&TargetAns, &SourceUni, FALSE);
    ...
    while (Length--)
    {
        if (InputBuffer->CharInfo.Attributes & COMMON_LVB_LEADING_BYTE)
        {
            if (Length >= 2)    // Safe DBCS in buffer ?
            {
                ...
                NumBytes = ConvertOutputToOem(&InputBuffer->CharInfo.Char.UnicodeChar, 1, &AsciiDbcs[0], NumBytes);
        ...
        {
            ConvertOutputToOem(&InputBuffer->CharInfo.Char.UnicodeChar, 1, &OutputBuffer->CharInfo.Char.AsciiChar, 1);
            OutputBuffer->CharInfo.Attributes = InputBuffer->CharInfo.Attributes;
            OutputBuffer++;
            InputBuffer++;
        }
