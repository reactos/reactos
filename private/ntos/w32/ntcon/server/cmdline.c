...
retry_clean:
    ...
    memcpy(gaKeyDef, pKeyDefBuf->table, sizeof gaKeyDef);// Copy the entity
}
...
CONST ExtKeySubst* ParseEditKeyInfo(IN OUT PKEY_EVENT_RECORD pKeyEvent)
{
    ...
    if (!gExtendedEditKey || (pKeyEvent->dwControlKeyState & (CTRL_PRESSED | ALT_PRESSED)) == 0 || pKeyEvent->wVirtualKeyCode < 'A' || pKeyEvent->wVirtualKeyCode > 'Z') {
        return NULL;
    }
    ...
    )
{
    ...
    if (UnicodeExe) {
        ...
        AliasList->ExeLength = (USHORT)ConvertInputToUnicode(Console->CP, ExeName, ExeLength, AliasList->ExeName, ExeLength);
    ...
    )
{
    ...
    String2.Buffer = Str2;
    return RtlCompareUnicodeString(&String1, &String2, bCaseInsensitive);
}
...
/*++
    This routine searches for the specified exe alias list.
    It returns a pointer to the exe list if found, NULL if not found.
--*/
{
    PEXE_ALIAS_LIST AliasList;
    ...
    if (UnicodeExe) {
        ...
        if (UnicodeExeName == NULL)
            return NULL;
        ExeLength = (USHORT)ConvertInputToUnicode(Console->CP, ExeName, ExeLength, UnicodeExeName, ExeLength);
        ExeLength *= 2;
    }
    ...
    while (ListNext != ListHead) {
        ...
        if (AliasList->ExeLength == ExeLength && !my_wcsncmpi(AliasList->ExeName,UnicodeExeName,ExeLength)) {
            if (!UnicodeExe) {
...
/*++
    This routine searches for the specified alias.
    If it finds one, it moves it to the head of the list and returns a pointer to the alias.
...
--*/
{
    ...
    while (ListNext != ListHead) {
        ...
        if (Alias->SourceLength == AliasLength && !my_wcsncmpi(Alias->Source,AliasName,AliasLength)) {
...
--*/
{
    ...
    if (a->Unicode) {
        ...
        a->SourceLength = (USHORT)ConvertInputToUnicode(Console->CP, a->Source, a->SourceLength, Source, a->SourceLength);
        a->SourceLength *= 2;
        a->TargetLength = (USHORT)ConvertInputToUnicode(Console->CP, a->Target, a->TargetLength, Target, a->TargetLength);
        a->TargetLength *= 2;
    }
...
--*/
{
    ...
    if (!a->Unicode) {
        if (NT_SUCCESS(Status)) {
            a->TargetLength = (USHORT)ConvertToOem(Console->CP, Target, a->TargetLength / sizeof(WCHAR), a->Target, CHAR_COUNT(a->TargetLength));
        }
        ConsoleHeapFree(Source);
        ConsoleHeapFree(Target);
    }
...
VOID ClearAliases(IN PCONSOLE_INFORMATION Console)
{
    ...
    ExeAliasList = FindExe(Console, L"cmd.exe", 14, TRUE);
...
DWORD SrvGetConsoleAliases(IN OUT PCSR_API_MSG m, IN OUT PCSR_REPLY_STATUS ReplyStatus)
{
    ...
    if (!CsrValidateMessageBuffer(m, &a->AliasesBuffer, a->AliasesBufferLength, sizeof(BYTE)) || !CsrValidateMessageBuffer(m, &a->Exe, a->ExeLength, sizeof(BYTE))) {
        UnlockConsole(Console);
        return STATUS_INVALID_PARAMETER;
    }
        ...
        while (ListNext != ListHead) {
            ...
            if (a->Unicode) {
                ...
                if ((a->AliasesBufferLength + ((Alias->SourceLength + Alias->TargetLength)/sizeof(WCHAR)) + (2*sizeof(CHAR)))
                    ...
                    SourceLength = (USHORT)ConvertToOem(Console->CP, Alias->Source, Alias->SourceLength / sizeof(WCHAR), AliasesBufferPtrA, CHAR_COUNT(Alias->SourceLength));
                    AliasesBufferPtrA+=SourceLength;
                    *AliasesBufferPtrA++ = '=';
                    TargetLength = (USHORT)ConvertToOem(Console->CP, Alias->Target, Alias->TargetLength / sizeof(WCHAR), AliasesBufferPtrA, CHAR_COUNT(Alias->TargetLength));
    ...
    while (ListNext != ListHead) {
        ...
        if (a->Unicode) {
            ...
            if ((a->AliasExesBufferLength + (AliasList->ExeLength/sizeof(WCHAR)) + (1*sizeof(CHAR)))
                ...
                USHORT Length;
                Length = (USHORT)ConvertToOem(Console->CP, AliasList->ExeName, AliasList->ExeLength / sizeof(WCHAR), AliasExesBufferPtrA, CHAR_COUNT(AliasList->ExeLength));
...
Arguments:
    ...
    TargetLength - on input, contains size of TargetBuffer.  On output, contains length of alias stored in TargetBuffer.
    SourceIsCommandLine - if true, source buffer is a command line, where the first blank separate token is to be check for an alias, and if
        it matches, replaced with the value of the alias.  if false, then the source string is a null terminated alias name.
...
--*/
{
    ...
    // find first blank
    for (Tmp=Source,SourceUpToFirstBlank=0; *Tmp!=(WCHAR)' ' && SourceUpToFirstBlank<(USHORT)(SourceLength/sizeof(WCHAR)); Tmp++,SourceUpToFirstBlank++) ;
...
DWORD SrvExpungeConsoleCommandHistory(IN OUT PCSR_API_MSG m, IN OUT PCSR_REPLY_STATUS ReplyStatus)
{
    ...
    EmptyCommandHistory(FindExeCommandHistory(Console, a->Exe, a->ExeLength, a->UnicodeExe));
    UnlockConsole(Console);
...
DWORD SrvSetConsoleNumberOfCommands(IN OUT PCSR_API_MSG m, IN OUT PCSR_REPLY_STATUS ReplyStatus)
{
    ...
    ReallocCommandHistory(Console, FindExeCommandHistory(Console, a->Exe, a->ExeLength, a->UnicodeExe), a->NumCommands);
    UnlockConsole(Console);
    return STATUS_SUCCESS;
    UNREFERENCED_PARAMETER(ReplyStatus);
}
...
DWORD SrvGetConsoleCommandHistoryLength(IN OUT PCSR_API_MSG m, IN OUT PCSR_REPLY_STATUS ReplyStatus)
{
    ...
    CommandHistory=FindExeCommandHistory(Console, a->Exe, a->ExeLength, a->UnicodeExe);
    if (CommandHistory) {
...
DWORD SrvGetConsoleCommandHistory(IN OUT PCSR_API_MSG m, IN OUT PCSR_REPLY_STATUS ReplyStatus)
{
    ...
    if (!CsrValidateMessageBuffer(m, &a->CommandBuffer, a->CommandBufferLength, sizeof(BYTE)) || !CsrValidateMessageBuffer(m, &a->Exe, a->ExeLength, sizeof(BYTE))) {
        UnlockConsole(Console);
        return STATUS_INVALID_PARAMETER;
    }
...
PCOMMAND_HISTORY ReallocCommandHistory(IN PCONSOLE_INFORMATION Console, IN PCOMMAND_HISTORY CurrentCommandHistory, IN DWORD NumCommands)
{
    ...
    if (CurrentCommandHistory == NULL || CurrentCommandHistory->MaximumNumberOfCommands==(SHORT)NumCommands) {
...
PCOMMAND_HISTORY FindExeCommandHistory(IN PCONSOLE_INFORMATION Console, IN PVOID AppName, IN DWORD AppNameLength, IN BOOLEAN Unicode)
{
    ...
    if (!Unicode) {
        ...
        AppNameLength = ConvertInputToUnicode(Console->CP, AppName, AppNameLength, AppNamePtr, AppNameLength);
        AppNameLength *= 2;
    } else {
        AppNamePtr = AppName;
    }
    ...
    while (ListNext != ListHead) {
        ...
        if (History->Flags & CLE_ALLOCATED && !my_wcsncmpi(History->AppName,AppNamePtr,(USHORT)AppNameLength)) {
            if (!Unicode) {
...
--*/
{
    ...
    // if there isn't a free buffer for the app name and the maximum number of command history buffers hasn't been allocated, allocate a new one.
...
NTSTATUS EndPopup(IN PSCREEN_INFORMATION ScreenInfo, IN PCOMMAND_HISTORY CommandHistory)
{
    ...
    if ((ScreenInfo->Flags & CONSOLE_OEMFONT_DISPLAY) && !(ScreenInfo->Console->FullScreenFlags & CONSOLE_FULLSCREEN)) {
...
#if !defined(FE_SB)
            TranslateOutputToAnsiUnicode(ScreenInfo->Console, Popup->OldContents, Size);
#else
            TranslateOutputToAnsiUnicode(ScreenInfo->Console, Popup->OldContents, Size, NULL);
#endif
        }
    } else {
        /*
        ...
        if (Popup->Flags & CLEPF_FALSE_UNICODE) {
#if !defined(FE_SB)
            TranslateOutputToOemUnicode(ScreenInfo->Console, Popup->OldContents, Size);
#else
            TranslateOutputToOemUnicode(ScreenInfo->Console, Popup->OldContents, Size, FALSE);
#endif
        }
    }
...
VOID DeleteCommandLine(IN OUT PCOOKED_READ_DATA CookedReadData, IN BOOL UpdateFields)
{
    ...
    if (CONSOLE_IS_DBCS_OUTPUTCP (CookedReadData->ScreenInfo->Console) &&
        ...
        CookedReadData->BackupLimit, CharsToWrite,
    ...
    SetCursorPosition(CookedReadData->ScreenInfo, CookedReadData->OriginalCursorPosition, TRUE);
}
...
VOID RedrawCommandLine(IN OUT PCOOKED_READ_DATA CookedReadData)
{
    ...
    if (CookedReadData->Echo) {
        ...
        CursorPosition.X += (SHORT)RetrieveTotalNumberOfSpaces(CookedReadData->OriginalCursorPosition.X, CookedReadData->BackupLimit, CookedReadData->CurrentPosition, CookedReadData->ScreenInfo->Console);
...
--*/
{
    ...
    Status = RetrieveNthCommand(CookedReadData->CommandHistory, Index, CookedReadData->BackupLimit, CookedReadData->BufferSize, &CookedReadData->BytesRead);
    ASSERT(NT_SUCCESS(Status));
...
{
    if (!(KeyEvent->dwControlKeyState & (RIGHT_ALT_PRESSED | LEFT_ALT_PRESSED | RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED))) {
...
BOOL IsCommandLineEditingKey(IN PKEY_EVENT_RECORD KeyEvent)
{
    ...
    if ((KeyEvent->dwControlKeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED))) {
        switch (KeyEvent->wVirtualKeyCode) {
        case VK_END:
        case VK_HOME:
        case VK_LEFT:
    ...
    if ((KeyEvent->dwControlKeyState & (RIGHT_ALT_PRESSED | LEFT_ALT_PRESSED))) {
        switch (KeyEvent->wVirtualKeyCode) {
        case VK_F7:
        case VK_F10:
            return TRUE;
    ...
    while (TRUE) {
        ...
        if (CommandLinePopupKeys) {
            ...
            case VK_F9:
                ...
                if (CookedReadData->CommandHistory && CookedReadData->ScreenInfo->ScreenBufferSize.X >= MINIMUM_COMMAND_PROMPT_SIZE+2) {  // 2 is for border
                    PopupSize.X = COMMAND_NUMBER_PROMPT_LENGTH+COMMAND_NUMBER_LENGTH;
                    PopupSize.Y = 1;
                    Status = BeginPopup(CookedReadData->ScreenInfo, CookedReadData->CommandHistory, PopupSize);
                    if (NT_SUCCESS(Status)) {
                        // CommandNumberPopup does EndPopup call
                        return CommandNumberPopup(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
                    }
                }
                }
                break;
            ...
            case VK_UP:
                UpdateCommandListPopup(-1, &Popup->CurrentCommand, CommandHistory, Popup, CookedReadData->ScreenInfo, 0);
                break;
            case VK_DOWN:
                UpdateCommandListPopup(1, &Popup->CurrentCommand, CommandHistory, Popup, CookedReadData->ScreenInfo, 0);
                break;
            case VK_END:
                 ...
                 */
                UpdateCommandListPopup((SHORT)(CommandHistory->NumberOfCommands), &Popup->CurrentCommand, CommandHistory, Popup, CookedReadData->ScreenInfo, 0);
                break;
            case VK_HOME:
                /*
                ...
                break;
            case VK_PRIOR:
                UpdateCommandListPopup((SHORT)-POPUP_SIZE_Y(Popup), &Popup->CurrentCommand, CommandHistory, Popup, CookedReadData->ScreenInfo, 0);
                break;
            case VK_NEXT:
                UpdateCommandListPopup(POPUP_SIZE_Y(Popup), &Popup->CurrentCommand, CommandHistory, Popup, CookedReadData->ScreenInfo, 0);
                break;
            case VK_LEFT:
            case VK_RIGHT:
            ...
            ProcessCookedReadInput(CookedReadData, UNICODE_CARRIAGERETURN, 0, &Status);
            // complete read
            ...
            if (!a->Unicode) {
                ...
                a->NumBytes = (ULONG)ConvertToOem(CookedReadData->Console->CP, CookedReadData->UserBuffer, a->NumBytes / sizeof(WCHAR), TransBuffer, CHAR_COUNT(a->NumBytes));
            ...
            Index = FindMatchingCommand(CookedReadData->CommandHistory, &Char, 1 * sizeof(WCHAR), Popup->CurrentCommand, FMCFL_JUST_LOOKING);
            if (Index != -1) {
                UpdateCommandListPopup((SHORT)(Index - Popup->CurrentCommand), &Popup->CurrentCommand, CommandHistory, Popup, CookedReadData->ScreenInfo, UCLP_WRAP);
            }
        }
    }
}
...
/*++
    This routine handles the delete from cursor to char char popup.
    It returns when we're out of input or the user has entered a char.
    Return Value:
        CONSOLE_STATUS_WAIT - we ran out of input, so a wait block was created
        CONSOLE_STATUS_READ_COMPLETE - user hit return
--*/
{
    ...
    while (TRUE) {
        ...
        for (i=CookedReadData->CurrentPosition+1; i<(int)(CookedReadData->BytesRead/sizeof(WCHAR)); i++) {
        ...
        if (i!=(int)(CookedReadData->BytesRead/sizeof(WCHAR)+1)) {
                ...
                // delete chars
                RtlCopyMemory(&CookedReadData->BackupLimit[CookedReadData->CurrentPosition], &CookedReadData->BackupLimit[i], CookedReadData->BytesRead-(i*sizeof(WCHAR)));
                ...
                Status = SetCursorPosition(CookedReadData->ScreenInfo, CursorPosition, TRUE);
                ASSERT(NT_SUCCESS(Status));
        }
    ...
    while (TRUE) {
        ...
        if (LastCommand) {
            ...
            if (i<(int)(LastCommand->CommandLength/sizeof(WCHAR)) && (USHORT)(LastCommand->CommandLength/sizeof(WCHAR)) > (USHORT)CookedReadData->CurrentPosition) {
                ...
                ASSERT(j>0);
                RtlCopyMemory(CookedReadData->BufPtr, &LastCommand->Command[CookedReadData->CurrentPosition], j*sizeof(WCHAR));
...
/*++
    This routine handles the delete char popup.
    It returns when we're out of input or the user has entered a char.
    Return Value:
        CONSOLE_STATUS_WAIT - we ran out of input, so a wait block was created
        CONSOLE_STATUS_READ_COMPLETE - user hit return
...
/*++
    This routine handles the command list popup.
    It puts up the popup, then calls ProcessCommandListInput to get and process input.
    Return Value:
        CONSOLE_STATUS_WAIT - we ran out of input, so a wait block was created
...
--*/
{
    ...
    return ProcessCommandListInput(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
}
    ...
    )
{
    ...
    for (i=0;i<POPUP_SIZE_Y(Popup);i++) {
        FillOutput(ScreenInfo, Popup->Attributes, WriteCoord, CONSOLE_ATTRIBUTE, &lStringLength);
    ...
    WriteOutputString(ScreenInfo, Prompt, WriteCoord, CONSOLE_REAL_UNICODE, &lStringLength, NULL);
}
...
/*++
    This routine handles the "delete up to this char" popup.
    It puts up the popup, then calls ProcessCopyFromCharInput to get and process input.
    Return Value:
        CONSOLE_STATUS_WAIT - we ran out of input, so a wait block was created
        STATUS_SUCCESS - read was fully completed (user hit return)
--*/
{
    ...
    return ProcessCopyFromCharInput(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
}
...
/*++
    This routine handles the "delete up to this char" popup.
    It puts up the popup, then calls ProcessCopyToCharInput to get and process input.
    Return Value:
        CONSOLE_STATUS_WAIT - we ran out of input, so a wait block was created
        STATUS_SUCCESS - read was fully completed (user hit return)
--*/
{
    ...
    return ProcessCopyToCharInput(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
}
...
/*++
    This routine handles the "enter command number" popup.
    It puts up the popup, then calls ProcessCommandNumberInput to get and process input.
    Return Value:
        CONSOLE_STATUS_WAIT - we ran out of input, so a wait block was created
        STATUS_SUCCESS - read was fully completed (user hit return)
--*/
{
    ...
    SetCursorPosition(CookedReadData->ScreenInfo, CursorPosition, TRUE);
    Popup->NumberRead=0;
    Popup->PopupInputRoutine = (PCLE_POPUP_INPUT_ROUTINE) ProcessCommandNumberInput;
    return ProcessCommandNumberInput(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
}
...
--*/
{
    ...
    if (Char == VK_F7 && !(KeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED | RIGHT_ALT_PRESSED | LEFT_ALT_PRESSED))) {
        COORD PopupSize;

        if (CookedReadData->CommandHistory && CookedReadData->CommandHistory->NumberOfCommands) {
            PopupSize.X = 40;
            PopupSize.Y = 10;
            Status = BeginPopup(CookedReadData->ScreenInfo, CookedReadData->CommandHistory, PopupSize);
            if (NT_SUCCESS(Status)) {
                // CommandListPopup does EndPopup call
                return CommandListPopup(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
            }
        }
    } else {
        switch (Char) {
            ...
            case VK_F5:
                ...
                if (Char==VK_UP && !AtFirstCommand(CookedReadData->CommandHistory) || Char==VK_DOWN && !AtLastCommand(CookedReadData->CommandHistory)) {
                    DeleteCommandLine(CookedReadData, TRUE);
                    Status = RetrieveCommand(CookedReadData->CommandHistory, Char, CookedReadData->BackupLimit, CookedReadData->BufferSize, &CookedReadData->BytesRead);
                    ASSERT(CookedReadData->BackupLimit == CookedReadData->BufPtr);
                    if (CookedReadData->Echo) {
                        Status = WriteCharsFromInput(CookedReadData->ScreenInfo,
                                CookedReadData->BackupLimit,
            ...
            case VK_HOME:
                if (KeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED)) {
                    ...
                    CookedReadData->BytesRead -= CookedReadData->CurrentPosition*sizeof(WCHAR);
                    CookedReadData->CurrentPosition = 0;
                    RtlCopyMemory(CookedReadData->BackupLimit, CookedReadData->BufPtr, CookedReadData->BytesRead);
                    CookedReadData->BufPtr = CookedReadData->BackupLimit;
                    if (CookedReadData->Echo) {
                        Status = WriteCharsFromInput(CookedReadData->ScreenInfo,
                                            CookedReadData->BackupLimit,
                ...
                if (KeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED)) {
                    ...
                    if (CookedReadData->BufPtr != CookedReadData->BackupLimit) {
                        ...
                        CurrentPosition.X = (SHORT)(CurrentPosition.X +
                            RetrieveTotalNumberOfSpaces(CookedReadData->OriginalCursorPosition.X, CookedReadData->BackupLimit, CookedReadData->CurrentPosition, CookedReadData->ScreenInfo->Console));
                        if (CheckBisectStringW(CookedReadData->ScreenInfo,
                                               CookedReadData->ScreenInfo->Console->CP,
                                               CookedReadData->BackupLimit,
...
#else
                        CurrentPosition.X = (SHORT)(CurrentPosition.X + RetrieveTotalNumberOfSpaces(CookedReadData->OriginalCursorPosition.X, CookedReadData->BackupLimit, CookedReadData->CurrentPosition));
#endif
                        UpdateCursorPosition = TRUE;
                    }
                } else {
...
#else
                        CurrentPosition.X = (SHORT)(CurrentPosition.X - RetrieveNumberOfSpaces(CookedReadData->OriginalCursorPosition.X, CookedReadData->BackupLimit, CookedReadData->CurrentPosition));
#endif
                        UpdateCursorPosition = TRUE;
                    }
                }
            ...
            case VK_RIGHT:
            case VK_F1:
                // we don't need to check for end of buffer here because we've already done it.
                if (KeyState & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED)) {
                    if (Char != VK_F1) {
                        PWCHAR NextWord;
                        SHORT i;
                        if (CookedReadData->CurrentPosition < (CookedReadData->BytesRead/sizeof(WCHAR))) {
                            ...
                            for (i=(SHORT)(CookedReadData->CurrentPosition); i<(SHORT)((CookedReadData->BytesRead-1)/sizeof(WCHAR)); i++) {
                                if (IS_WORD_DELIM(*NextWord)) {
                                    i++;
                                    NextWord++;
                                    while ((i<(SHORT)((CookedReadData->BytesRead-1)/sizeof(WCHAR))) && IS_WORD_DELIM(*NextWord)) {
                                        i++;
                                        NextWord++;
                                    }
                                    break;
                ...
                break;
            case VK_F2:
                // copy the previous command to the current command, up to but not including the character specified by the user.  the user
                ...
                if (CookedReadData->CommandHistory) {
                    ...
                    if (NT_SUCCESS(Status)) {
                        ...
                        return CopyToCharPopup(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
                    }
                }
                break;
            case VK_F3:
                ...
                break;
            case VK_F4:
                // copy the previous command to the current command, from the letter specified by the user.   the user
                // is prompted via popup to enter a character.
                if (CookedReadData->CommandHistory) {
                    COORD PopupSize;
                    ...
                    if (NT_SUCCESS(Status)) {
                        ...
                        return CopyFromCharPopup(CookedReadData, WaitReplyMessage, WaitingThread, WaitRoutine);
                    }
                }
                break;
            case VK_F6:
            ...
            case VK_F8:
                if (CookedReadData->CommandHistory) {
                    ...
                    i = FindMatchingCommand(CookedReadData->CommandHistory, CookedReadData->BackupLimit, CookedReadData->CurrentPosition*sizeof(WCHAR), CookedReadData->CommandHistory->LastDisplayed, 0);
                    if (i!=-1) {
                        ...
                        DeleteCommandLine(CookedReadData, TRUE);
                        Status = RetrieveNthCommand(CookedReadData->CommandHistory, i, CookedReadData->BackupLimit, CookedReadData->BufferSize, &CookedReadData->BytesRead);
                        ASSERT(CookedReadData->BackupLimit == CookedReadData->BufPtr);
                        if (CookedReadData->Echo) {
                            Status = WriteCharsFromInput(CookedReadData->ScreenInfo,
                                    CookedReadData->BackupLimit,
            ...
            case VK_INSERT:
                ...
                SetCursorMode(CookedReadData->ScreenInfo, (BOOLEAN)(CookedReadData->InsertMode != CookedReadData->Console->InsertMode));
                break;
        case VK_DELETE:
...
del_repeat:
                    ...
                    if (CONSOLE_IS_DBCS_ENABLED() && CONSOLE_IS_DBCS_CP(CookedReadData->Console)) {
                        ...
                        if (CookedReadData->Echo) {
                            Status = AdjustCursorPosition(CookedReadData->ScreenInfo, CurrentPosition, TRUE, NULL);
                            ASSERT(NT_SUCCESS(Status));
                        }
                    }
                    else {
                    ...
                    if ((KeyState & CTRL_PRESSED) && !AT_EOL(CookedReadData) && fStartFromDelim ^ !IS_WORD_DELIM(*CookedReadData->BufPtr)) {
...
/*++
    this routine finds the most recent command that starts with the letters already in the current command.
    it returns the array index (no mod needed).
--*/
{
    SHORT i;
...
VOID DrawCommandListBorder(IN PCLE_POPUP Popup, IN PSCREEN_INFORMATION ScreenInfo)
{
    ...
    FillOutput(ScreenInfo, Popup->Attributes, WriteCoord, CONSOLE_ATTRIBUTE, &Length);
    ...
    )
{
    ...
    // inverted attributes
    Attributes = (WORD)(((Popup->Attributes << 4) & 0xf0) | ((Popup->Attributes >> 4) & 0x0f));
...
VOID DrawCommandListPopup(IN PCLE_POPUP Popup, IN SHORT CurrentCommand, IN PCOMMAND_HISTORY CommandHistory, IN PSCREEN_INFORMATION ScreenInfo)
{
    ...
    for (i=max((SHORT)(Popup->BottomIndex-POPUP_SIZE_Y(Popup)+1),0);i<=Popup->BottomIndex;i++) {
        ...
        WriteCoord.X = (SHORT)(Popup->Region.Left+1);
        WriteOutputString(ScreenInfo, CommandNumberPtr, WriteCoord, CONSOLE_ASCII, &CommandNumberLength, NULL);
...
#if defined(FE_SB)
        {
            ...
            while (lTmpStringLength--) {
                if (IsConsoleFullWidth(ScreenInfo->Console->hDC, ScreenInfo->Console->OutputCP,*lpStr++)) {
                    lPopupLength -= 2;
                }
                else {
                    lPopupLength--;
...
#if defined(FE_SB)
        {
            ...
            WriteOutputString(ScreenInfo, TransBuffer, WriteCoord, CONSOLE_REAL_UNICODE, &lStringLength, NULL);
            ConsoleHeapFree(TransBuffer);
        }
#else
        ...
        if ((ScreenInfo->Flags & CONSOLE_OEMFONT_DISPLAY) && !(ScreenInfo->Console->FullScreenFlags & CONSOLE_FULLSCREEN)) {
            FalseUnicodeToRealUnicode(CommandHistory->Commands[COMMAND_NUM_TO_INDEX(i,CommandHistory)]->Command, lStringLength, ScreenInfo->Console->OutputCP);
        }
#endif
...
NTSTATUS AddCommand(IN PCOMMAND_HISTORY CommandHistory, IN PWCHAR Command, IN USHORT Length, IN BOOL HistoryNoDup)
{
    ...
    if (CommandHistory->NumberOfCommands == 0 ||
        ...
        if (HistoryNoDup) {
            SHORT i;
            i = FindMatchingCommand(CommandHistory, Command, Length, CommandHistory->LastDisplayed, FMCFL_EXACT_MATCH);
            if (i != -1) {
        ...
        if (pCmdReuse) {
            ...
            *ppCmd = (PCOMMAND)ConsoleHeapAlloc( MAKE_TAG(HISTORY_TAG), Length - sizeof(WCHAR) + sizeof(COMMAND));
            if (*ppCmd == NULL) {
...
NTSTATUS RetrieveCommand(IN PCOMMAND_HISTORY CommandHistory, IN WORD VirtualKeyCode, IN PWCHAR Buffer, IN ULONG BufferSize, OUT PULONG CommandSize)
{
    ...
    if (CommandHistory->NumberOfCommands == 1) {
        ...
        // been retrieved, return the current command.  otherwise, return the previous command.
        if (CommandHistory->Flags & CLE_RESET) {
            CommandHistory->Flags &= ~CLE_RESET;
        } else {
...
#if defined(FE_SB)
        a->TitleLength = (USHORT)ConvertToOem(OEMCP, Console->Title, Console->TitleLength / sizeof(WCHAR), a->Title, a->TitleLength);
#else
        a->TitleLength = (USHORT)ConvertToOem(Console->CP, Console->Title, Console->TitleLength / sizeof(WCHAR), a->Title, a->TitleLength);
#endif
    }
    UnlockConsole(Console);
...
#if defined(FE_SB)
        Console->TitleLength = (USHORT)ConvertInputToUnicode(OEMCP, a->Title, a->TitleLength, NewTitle, a->TitleLength);
#else
        Console->TitleLength = (USHORT)ConvertInputToUnicode(Console->CP, a->Title, a->TitleLength, NewTitle, a->TitleLength);
#endif
    ...
    )
{
     ...
     */
    if (hResInfo = FindResourceEx(hModule, RT_STRING, (LPTSTR)((LONG)(((USHORT)wID >> 4) + 1)), wLangId)) {
        /*
         * Load that segment.
         */
