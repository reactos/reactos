...
Notes:
 ...
 necessarily the top row. ScreenInfo->BufferInfo.TextInfo.FirstRow contains the index of the top row.
 That means scrolling (if scrolling entire screen)
 merely involves changing the FirstRow index, filling in the last row, and updating the screen.
--*/
...
// if a window is created when this value is zero, the Face Names must be reenumerated because no WM_FONTCHANGE message was processed if there's no window.
...
#if defined(FE_SB)
BOOL SB_PolyTextOutCandidate(IN PSCREEN_INFORMATION ScreenInfo, IN PSMALL_RECT Region);
VOID SB_ConsolePolyTextOut(IN PSCREEN_INFORMATION ScreenInfo, IN PSMALL_RECT Region);
#endif
...
VOID InitializeSystemMetrics( VOID )
{
    ...
    AdjustWindowRectEx(&WindowSize, CONSOLE_WINDOW_FLAGS, FALSE, CONSOLE_WINDOW_EX_FLAGS);
...
/*++
    ...
    based on the data from STARTUPINFO and the defaults in win.ini, then calls CreateScreenBuffer.
--*/
{
    ...
    // Grab font
#if defined(FE_SB)
    FontIndexWant = FindCreateFont(ConsoleInfo->uFontFamily, ConsoleInfo->FaceName, ConsoleInfo->dwFontSize, ConsoleInfo->uFontWeight, ConsoleInfo->uCodePage);
#else
    FontIndexWant = FindCreateFont(ConsoleInfo->uFontFamily, ConsoleInfo->FaceName, ConsoleInfo->dwFontSize, ConsoleInfo->uFontWeight);
#endif
...
/*++
Routine Description:
    This routine allocates and initializes the data associated with a screen buffer.  It also creates a window.
Arguments:
...
--*/
{
    ...
    if ((ScreenInfo->Flags = Flags) & CONSOLE_TEXTMODE_BUFFER) {
...
#endif
        for (i=0,TextRowPtr=ScreenInfo->BufferInfo.TextInfo.TextRows; i<dwScreenBufferSize.Y; i++,TextRowPtr+=dwScreenBufferSize.X)
        {
        ...
        Status = CreateConsoleBitmap(GraphicsBufferInfo, ScreenInfo, lpBitmap, hMutex);
        if (!NT_SUCCESS(Status)) {
            ConsoleHeapFree(ScreenInfo);
            return Status;
        }
...
NTSTATUS CreateWindowsWindow(IN PCONSOLE_INFORMATION Console)
{
    ...
    if (NT_SUCCESS(NtQueryInformationThread(Console->ClientThreadHandle, ThreadBasicInformation, &ThreadInfo, sizeof(ThreadInfo), NULL))) {
    ...
    if (Console->iIconId) {
        ...
        PostMessage(HWND_BROADCAST, ProgmanHandleMessage, (WPARAM)Console->hWnd, 1);
    }
    ...
    SetScreenColors(ScreenInfo, ScreenInfo->Attributes, ScreenInfo->PopupAttributes, FALSE);
    if (Console->Flags & CONSOLE_NO_WINDOW) {
        ...
        if (Console->wShowWindow != SW_SHOWNOACTIVATE && Console->wShowWindow != SW_SHOWMINNOACTIVE && Console->wShowWindow != SW_HIDE) {
            if (!NT_SUCCESS(ConsoleSetActiveWindow(Console))) {
                return STATUS_INVALID_HANDLE;
            }
    ...
    if (!(Console->Flags & CONSOLE_IS_ICONIC) && (Console->FullScreenFlags == 0) ) {
...
Arguments:
    ...
    for example, if the attribute string was { 5, BLUE } and the requested index was 3, CountOfAttr would be 2.
--*/
{
    SHORT i;
...
Routine Description:
    ...
    for example, if the source string was { 4, BLUE }, the merge string was { 2, RED }, and the StartIndex and EndIndex were 1 and 2,
    respectively, the target string would be { 1, BLUE, 2, RED, 1, BLUE } and the target length would be 3.
...
--*/
{
    ...
    if (SrcAttr != SrcEnd || i!=(SHORT)(EndIndex+1)) {
        // see if we've gone past the right attribute.  if so, back up and copy the attribute and the correct length.
        TargetAttr--;
...
--*/
{
    ...
    if (XSize == TargetSize.X) {
        ...
        if (TargetRect->Top != 0) {
            TargetPtr = (PCHAR_INFO) ((PBYTE)Target + SCREEN_BUFFER_POINTER(TargetRect->Left, TargetRect->Top, TargetSize.X, sizeof(CHAR_INFO)));
        }
        WholeTarget = TRUE;
    }
    ...
    for (i=0;i<YSize;i++) {
        if (!WholeTarget) {
            TargetPtr = (PCHAR_INFO)
                ((PBYTE)Target + SCREEN_BUFFER_POINTER(TargetRect->Left, TargetRect->Top+i, TargetSize.X, sizeof(CHAR_INFO)));
        }

        // copy the chars and attrs from their respective arrays
        Row = &ScreenInfo->BufferInfo.TextInfo.Rows[RowIndex];
        Char = &Row->CharRow.Chars[SourcePoint.X];
        FindAttrIndex(Row->AttrRow.Attrs, SourcePoint.X, &Attr, &CountOfAttr);
        k=0;
#if defined(FE_SB)
...
--*/
{
    ...
    ReadRectFromScreenBuffer(ScreenInformation, SourcePoint, Buffer, TargetSize, &Target);
    return STATUS_SUCCESS;
}
...
--*/
{
    ...
    if (WriteRegion->Left >= ScreenInformation->ScreenBufferSize.X || WriteRegion->Top  >= ScreenInformation->ScreenBufferSize.Y) {
        return STATUS_SUCCESS;
    }
    ...
    WriteRectToScreenBuffer((PBYTE)Buffer, SourceSize, &SourceRect, ScreenInformation, TargetPoint, 0xFFFFFFFF);
    return STATUS_SUCCESS;
}
...
--*/
{
    ...
    X=ReadCoord.X;
    Y=ReadCoord.Y;
    if (X>=ScreenInfo->ScreenBufferSize.X || X<0 || Y>=ScreenInfo->ScreenBufferSize.Y || Y<0) {
        *NumRecords = 0;
        return STATUS_SUCCESS;
    }
...
#endif
    if ((StringType == CONSOLE_ASCII) || (StringType == CONSOLE_REAL_UNICODE) || (StringType == CONSOLE_FALSE_UNICODE)) {
        ...
        while (NumRead < *NumRecords) {
            ...
            if (CONSOLE_IS_DBCS_OUTPUTCP(Console))
                AttrP = &Row->CharRow.KAttrs[X];
#endif
            FindAttrIndex(Row->AttrRow.Attrs, X, &Attr, &CountOfAttr);
            k=0;
    ...
    if (StringType == CONSOLE_ASCII) {
        ...
        ConsoleHeapFree(TransBuffer);
    } else if (StringType == CONSOLE_REAL_UNICODE && (ScreenInfo->Flags & CONSOLE_OEMFONT_DISPLAY) && !(ScreenInfo->Console->FullScreenFlags & CONSOLE_FULLSCREEN)) {
        /*
         * Buffer contains false Unicode (UnicodeOem) only in Windowed
         * RasterFont mode, so in this case, convert it to real Unicode.
         */
        FalseUnicodeToRealUnicode(Buffer, NumRead, ScreenInfo->Console->OutputCP);
    }
...
VOID UpdateScrollBars(IN PSCREEN_INFORMATION ScreenInfo)
{
    ...
    PostMessage(ScreenInfo->Console->hWnd, CM_UPDATE_SCROLL_BARS, (WPARAM)ScreenInfo, 0);
}
...
VOID ScreenBufferSizeChange(IN PSCREEN_INFORMATION ScreenInfo, IN COORD NewSize)
{
    ...
    WriteInputBuffer(ScreenInfo->Console, &ScreenInfo->Console->InputBuffer, &InputEvent, 1);
}
...
--*/
{
...
#endif
    LimitX = (NewScreenSize.X < ScreenInfo->ScreenBufferSize.X) ? NewScreenSize.X : ScreenInfo->ScreenBufferSize.X;
    LimitY = (NewScreenSize.Y < ScreenInfo->ScreenBufferSize.Y) ? NewScreenSize.Y : ScreenInfo->ScreenBufferSize.Y;
    TopRow = 0;
    ...
    if (NewScreenSize.Y != ScreenInfo->ScreenBufferSize.Y) {
        ...
        if (TopRowIndex!=0 && NumToCopy != NewScreenSize.Y) {
            ...
            RtlCopyMemory(&Temp[NumToCopy], ScreenInfo->BufferInfo.TextInfo.Rows, NumToCopy2*sizeof(ROW));
        }
        for (i=0;i<LimitY;i++) {
        ...
        // free the extra rows.  if the new screen buffer has more rows than the existing one, allocate new rows.
        if (NewScreenSize.Y < ScreenInfo->ScreenBufferSize.Y) {
    ...
    // Realloc each row.  any horizontal growth results in the last attribute in a row getting extended.
#if defined(FE_SB)
    TextRowPtrA=NewDbcsScreenBuffer.KAttrRows;
#endif
    for (i=0,TextRowPtr=TextRows;i<LimitY;i++,TextRowPtr+=NewScreenSize.X)
    {
        RtlCopyMemory(TextRowPtr, ScreenInfo->BufferInfo.TextInfo.Rows[i].CharRow.Chars, LimitX*sizeof(WCHAR));
#if defined(FE_SB)
        if (TextRowPtrA) {
            RtlCopyMemory(TextRowPtrA, ScreenInfo->BufferInfo.TextInfo.Rows[i].CharRow.KAttrs, LimitX*sizeof(CHAR));
        }
#endif
        ...
        if (TextRowPtrA) {
            if (NewScreenSize.X > ScreenInfo->ScreenBufferSize.X)
                RtlZeroMemory(TextRowPtrA+ScreenInfo->ScreenBufferSize.X, NewScreenSize.X-ScreenInfo->ScreenBufferSize.X);
    ...
    if (NewScreenSize.X != ScreenInfo->ScreenBufferSize.X) {
        for (i=0;i<LimitY;i++) {
            ...
            if (NewScreenSize.X > ScreenInfo->ScreenBufferSize.X) {
                FindAttrIndex(ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs, (SHORT)(ScreenInfo->ScreenBufferSize.X-1), &IndexedAttr, &CountOfAttr);
                ASSERT (IndexedAttr <= &ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs[ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Length-1]);
                IndexedAttr->Length += NewScreenSize.X - ScreenInfo->ScreenBufferSize.X;
            }
            else {
                FindAttrIndex(ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs, (SHORT)(NewScreenSize.X-1), &IndexedAttr, &CountOfAttr);
                ...
                if (ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Length != 1)  {
                    ...
                    if (ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Length != 1) {
                        ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs = (PATTR_PAIR)ConsoleHeapReAlloc(
                            MAKE_TAG( SCREEN_TAG ),
                            ScreenInfo->BufferInfo.TextInfo.Rows[i].AttrRow.Attrs,
...
#endif
    if (CursorPosition.X != ScreenInfo->BufferInfo.TextInfo.CursorPosition.X || CursorPosition.Y != ScreenInfo->BufferInfo.TextInfo.CursorPosition.Y) {
        SetCursorPosition(ScreenInfo, CursorPosition, FALSE);
    }
    ...
    WindowMaximizedX = (CONSOLE_WINDOW_SIZE_X(ScreenInfo) == ScreenInfo->ScreenBufferSize.X);
    WindowMaximizedY = (CONSOLE_WINDOW_SIZE_Y(ScreenInfo) == ScreenInfo->ScreenBufferSize.Y);

#if defined(FE_IME)
    if (CONSOLE_IS_IME_ENABLED()) {
        if (!NT_SUCCESS(ConsoleImeMessagePump(ScreenInfo->Console, CONIME_NOTIFY_SCREENBUFFERSIZE, (WPARAM)ScreenInfo->Console->ConsoleHandle, (LPARAM)MAKELPARAM(NewScreenSize.X, NewScreenSize.Y)))) {
            return STATUS_INVALID_HANDLE;
        }
    }

    if ( (! ScreenInfo->ConvScreenInfo) && (CONSOLE_IS_DBCS_OUTPUTCP(ScreenInfo->Console)))
    {
        if (!NT_SUCCESS(ConsoleImeResizeModeSystemScreenBuffer(ScreenInfo->Console,NewScreenSize)) || !NT_SUCCESS(ConsoleImeResizeCompStrScreenBuffer(ScreenInfo->Console,NewScreenSize))) {
            /*
...
#endif // FE_IME
    if (ScreenInfo->WindowMaximizedX != WindowMaximizedX || ScreenInfo->WindowMaximizedY != WindowMaximizedY) {
...
NTSTATUS InitializeScrollBuffer( VOID )
{
    ...
    Status = AllocateScrollBuffer(DefaultRegInfo.ScreenBufferSize.X * DefaultRegInfo.ScreenBufferSize.Y * sizeof(CHAR_INFO));
    if (!NT_SUCCESS(Status)) {
        goto error;
    }
...
VOID UpdateComplexRegion(IN PSCREEN_INFORMATION ScreenInfo, IN COORD FontSize)
{
    ...
    for(i=0;i<(int)pRgnData->rdh.nCount;i++,pRect++) {
         ...
         * the textout call.
         */
        UpdateRegion.Left = (SHORT)((pRect->left/FontSize.X)+ ScreenInfo->Window.Left);
        UpdateRegion.Right = (SHORT)(((pRect->right-1)/FontSize.X)+ ScreenInfo->Window.Left);
        UpdateRegion.Top = (SHORT)((pRect->top/FontSize.Y)+ ScreenInfo->Window.Top);
        UpdateRegion.Bottom = (SHORT)(((pRect->bottom-1)/FontSize.Y)+ ScreenInfo->Window.Top);
        /*
...
VOID ScrollScreen(IN PSCREEN_INFORMATION ScreenInfo, IN PSMALL_RECT ScrollRect, IN PSMALL_RECT MergeRect, IN COORD TargetPoint)
{
    ...
    if (ScreenInfo->Console->FullScreenFlags == 0 && !(ScreenInfo->Console->Flags & (CONSOLE_IS_ICONIC | CONSOLE_NO_WINDOW))) {
#if defined(FE_SB)
        ...
        if (Success) {
            ...
            if (CONSOLE_IS_DBCS_OUTPUTCP(ScreenInfo->Console)) {
                ...
                if (UpdateRect.Left && UpdateRect.Right+1 < ScreenInfo->ScreenBufferSize.X && UpdateRect.Right-UpdateRect.Left <= 2) {
        ...
        if (CONSOLE_IS_DBCS_OUTPUTCP(ScreenInfo->Console)) {
            if (! ScreenInfo->ConvScreenInfo) {
                if (ScreenInfo->Console->CurrentScreenBuffer == ScreenInfo) {
                    ScrollHW(ScreenInfo, ScrollRect, MergeRect, TargetPoint);
                }
            }
...
{
    if (PrevRow->AttrRow.Length != 1 || Row->AttrRow.Length != 1 || PrevRow->AttrRow.Attrs->Attr != Row->AttrRow.Attrs->Attr) {
...
--*/
{
    ...
    new = (RowIndex + ScreenInfo->Window.Bottom + ScrollValue) % ScreenInfo->ScreenBufferSize.Y;
    old = (RowIndex + ScreenInfo->Window.Bottom) % ScreenInfo->ScreenBufferSize.Y;
    for (i = WINDOW_SIZE_Y(&ScreenInfo->Window) - 1; i >= 0; i--) {
        CopyRow(&ScreenInfo->BufferInfo.TextInfo.Rows[new], &ScreenInfo->BufferInfo.TextInfo.Rows[old]);
        if (--new < 0)
    ...
    if (UpdateRowIndex) {
        ScreenInfo->BufferInfo.TextInfo.FirstRow = (SHORT)((RowIndex + ScrollValue) % ScreenInfo->ScreenBufferSize.Y);
    }
...
--*/
{
    ...
    if (ACTIVE_SCREEN_BUFFER(ScreenInfo) && Console->FullScreenFlags == 0 && !(Console->Flags & (CONSOLE_IS_ICONIC | CONSOLE_NO_WINDOW))) {
        ...
        if (UsePolyTextOut) {
            ...
            LockScrollBuffer();
            ASSERT (ScreenInfo->BufferInfo.TextInfo.UpdatingScreen>0);
            Success = (int)ScrollDC(Console->hDC, 0, -FontSize.Y, &Rect, NULL, ghrgnScroll, NULL);
            if (Success && ScreenInfo->Window.Top!=ScreenInfo->Window.Bottom) {
#if defined(FE_SB)
                if (CONSOLE_IS_DBCS_OUTPUTCP(Console) &&
                    ScreenInfo->Attributes & (COMMON_LVB_GRID_HORIZONTAL + COMMON_LVB_GRID_LVERTICAL + COMMON_LVB_GRID_RVERTICAL + COMMON_LVB_REVERSE_VIDEO  + COMMON_LVB_UNDERSCORE     )){
                ...
                switch (GetRgnBox(ghrgnScroll, &BoundingBox)) {
                case SIMPLEREGION:
                    if (BoundingBox.left == 0 && BoundingBox.right == ScreenWidth && BoundingBox.top == ScrollHeight && BoundingBox.bottom == ScreenHeight) {
        ...
        if (CONSOLE_IS_DBCS_OUTPUTCP(Console) ) {
            if (! ScreenInfo->ConvScreenInfo)  {
                if (ScreenInfo->Console->CurrentScreenBuffer == ScreenInfo) {
                    ScrollHW(ScreenInfo, &ScrollRect, NULL, TargetPoint);
                }
            }
            else if (ScreenInfo->Console->CurrentScreenBuffer->Flags & CONSOLE_TEXTMODE_BUFFER) {
                ScrollHW(ScreenInfo, &ScrollRect, NULL, TargetPoint);
...
Routine Description:
    ...
    fills in ScrollRectangle with Fill.  The scroll region is copied to a third buffer, the scroll region is filled, then the
...
--*/
{
    ...
    if (ScrollRectangle->Bottom < ScrollRectangle->Top || ScrollRectangle->Right < ScrollRectangle->Left) {
        return STATUS_SUCCESS;
    }
    ...
    if (!(TargetRectangle.Bottom < TargetRectangle.Top || TargetRectangle.Right < TargetRectangle.Left)) {
        ...
        // if we can, don't use intermediate scroll region buffer.  do this by figuring out fill rectangle.  NOTE: this code will only work
        // if CopyRectangle copies from low memory to high memory (otherwise we would overwrite the scroll region before reading it).
        if (ScrollRectangle2.Right == TargetRectangle.Right && ScrollRectangle2.Left == TargetRectangle.Left && ScrollRectangle2.Top > TargetRectangle.Top && ScrollRectangle2.Top < TargetRectangle.Bottom) {
            SMALL_RECT FillRect;
            ...
            if (ScrollRectangle2.Right == (SHORT)(ScreenInfo->ScreenBufferSize.X-1) && ScrollRectangle2.Left == 0 &&
                ScrollRectangle2.Bottom == (SHORT)(ScreenInfo->ScreenBufferSize.Y-1) && ScrollRectangle2.Top == 1 ) {
                ...
                LastRowIndex = -1;
                CopyRectangle(ScreenInfo, &ScrollRectangle2, TargetPoint);
            }
            ...
            FillRectangle(Fill, ScreenInfo, &FillRect);
            ...
            if (!(Console->Flags & CONSOLE_IS_ICONIC) || Console->FullScreenFlags & CONSOLE_FULLSCREEN_HARDWARE) {
                ScrollScreen(ScreenInfo, &ScrollRectangle2, &FillRect, TargetPoint);
            }
        }

        // if no overlap, don't need intermediate copy
        else if (ScrollRectangle3.Right < TargetRectangle.Left || ScrollRectangle3.Left > TargetRectangle.Right || ScrollRectangle3.Top > TargetRectangle.Bottom || ScrollRectangle3.Bottom < TargetRectangle.Top) {
            TargetPoint.X = TargetRectangle.Left;
            TargetPoint.Y = TargetRectangle.Top;
            CopyRectangle(ScreenInfo, &ScrollRectangle2, TargetPoint);
            FillRectangle(Fill, ScreenInfo, &ScrollRectangle3);
        ...
        // for the case where the source and target rectangles overlap, we copy the source rectangle, fill it, then copy it to the target.
            ...
            ReadRectFromScreenBuffer(ScreenInfo, SourcePoint, ScrollBuffer, Size, &TargetRect);
            ...
            TargetPoint.X = TargetRectangle.Left;
            TargetPoint.Y = TargetRectangle.Top;
            WriteRectToScreenBuffer((PBYTE)ScrollBuffer, Size, &SourceRectangle, ScreenInfo, TargetPoint, 0xFFFFFFFF);
            UnlockScrollBuffer();

            // update to screen, if we're not iconic.  we're marked as iconic if we're fullscreen, so check for fullscreen.
            if (!(Console->Flags & CONSOLE_IS_ICONIC) || Console->FullScreenFlags & CONSOLE_FULLSCREEN_HARDWARE) {
        ...
        // update to screen, if we're not iconic.  we're marked as iconic if we're fullscreen, so check for fullscreen.
        if (ACTIVE_SCREEN_BUFFER(ScreenInfo) && !(Console->Flags & CONSOLE_IS_ICONIC) || Console->FullScreenFlags & CONSOLE_FULLSCREEN_HARDWARE) {
...
--*/
{
    ...
    if (!Absolute) {
        ...
        if (WindowOrigin.X == ScreenInfo->Window.Left && WindowOrigin.Y == ScreenInfo->Window.Top) {
            return STATUS_SUCCESS;
        }
    ...
    if (NewWindow.Left < 0 || NewWindow.Top < 0 || NewWindow.Right < 0 || NewWindow.Bottom < 0 || NewWindow.Right >= ScreenInfo->ScreenBufferSize.X || NewWindow.Bottom >= ScreenInfo->ScreenBufferSize.Y) {
        return STATUS_INVALID_PARAMETER;
    }
...
#if defined(FE_SB)
        if (CONSOLE_IS_DBCS_OUTPUTCP(Console) && !(Console->ConsoleIme.ScrollFlag & HIDE_FOR_SCROLL)) {
            ConsoleImeBottomLineUse(ScreenInfo,0);
        }
#endif
        if (   ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER && UsePolyTextOut && NewWindow.Left == ScreenInfo->Window.Left) {
            ScrollEntireScreen(ScreenInfo, (SHORT)(NewWindow.Top - ScreenInfo->Window.Top), FALSE);
            ...
            ScrollRect.top = 0;
#if defined(FE_SB)
            if (CONSOLE_IS_DBCS_OUTPUTCP(Console) && Console->InputBuffer.ImeMode.Open )
            {
...
#if defined(FE_SB)
            if (CONSOLE_IS_DBCS_OUTPUTCP(Console) && ScrollRect.bottom == 0) {
                UpdateRegion.Left   = 0;
                UpdateRegion.Top    = 0;
...
Routine Description:
    ...
    window size change.  it does not call the user component to update the screen.
Arguments:
    ScreenInformation - the new screen buffer.
...
VOID SetWindowSize(IN PSCREEN_INFORMATION ScreenInfo)
{
    ...
    PostMessage(ScreenInfo->Console->hWnd, CM_SET_WINDOW_SIZE, (WPARAM)ScreenInfo, 0x47474747);
}
...
#if defined(FE_IME)
        if ( (ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) && (CONSOLE_IS_DBCS_OUTPUTCP(Console)))
        {
    ...
    if (ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) {
        ...
        if (Console->FullScreenFlags == 0) {
            ...
            if (!(Console->Flags & CONSOLE_VDM_REGISTERED)) {
                if ( (!(OldScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER)) || (OldScreenInfo->BufferInfo.TextInfo.ModeIndex!=ScreenInfo->BufferInfo.TextInfo.ModeIndex)) {
                ...
                SetCursorInformationHW(ScreenInfo, ScreenInfo->BufferInfo.TextInfo.CursorSize, ScreenInfo->BufferInfo.TextInfo.CursorVisible);
                SetCursorPositionHW(ScreenInfo, ScreenInfo->BufferInfo.TextInfo.CursorPosition);
            }
        }
#endif
    }
    ...
    if (Console->FullScreenFlags == 0) {
        ...
        if (!(Console->Flags & CONSOLE_IS_ICONIC) && Console->FullScreenFlags == 0) {
            if (ScreenInfo->hPalette != NULL || OldScreenInfo->hPalette != NULL) {
                ...
                SelectPalette(Console->hDC, hPalette, FALSE);
...
VOID DestroyWindowsWindow(IN PCONSOLE_INFORMATION Console)
{
    ...
    if (CONSOLE_IS_IME_ENABLED()) {
        ...
        ConsoleImeMessagePump(Console, CONIME_DESTROY, (WPARAM)Console->ConsoleHandle, (LPARAM)NULL);
    }
    // end FE_IME
    // end FE_SB
    ...
    // must keep this thread handle around until after the destroywindow call so that impersonation will work.
    CloseHandle(Console->ClientThreadHandle);

    // once the sendmessage returns, there will be no more input to the console so we don't need to lock it.
...
// Plan of bottom line reservation for console IME.
    ...
    NewOrigin.Y = (WORD)(max(0,min((SHORT)NewOrigin.Y, (SHORT)ScreenInfo->ScreenBufferSize.Y-(SHORT)CONSOLE_WINDOW_SIZE_Y(ScreenInfo))));
    SetWindowOrigin(ScreenInfo, (BOOLEAN)TRUE, NewOrigin);
}
...
VOID HorizontalScroll(IN PSCREEN_INFORMATION ScreenInfo, IN WORD ScrollCommand, IN WORD AbsoluteChange)
{
    ...
    NewOrigin.X = (WORD)(max(0,min((SHORT)NewOrigin.X, (SHORT)ScreenInfo->ScreenBufferSize.X-(SHORT)CONSOLE_WINDOW_SIZE_X(ScreenInfo))));
    SetWindowOrigin(ScreenInfo, (BOOLEAN)TRUE, NewOrigin);
}
...
LRESULT APIENTRY ConsoleWindowProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (Console != NULL) {
        ...
        CSR_SERVER_QUERYCLIENTTHREAD()->ThreadHandle = Console->ClientThreadHandle;

        // If the console is terminating, don't bother processing messages other than CM_DESTROY_WINDOW.
        ...
        if (Console == NULL || ScreenInfo == NULL) {
            ...
            if ((HWND)wParam != hWnd && Console->bIconInit) {
                ...
                if (!(Console->Flags & CONSOLE_TERMINATING)) {
                    ...
                    SendMessage(Console->hWndProgMan, WM_DDE_INITIATE, (WPARAM)hWnd, MAKELONG(App,Topic));
                    ...
                    if (NT_SUCCESS(Status)) {
                        ...
                        PostMessage(Console->hWndProgMan, WM_DDE_REQUEST, (WPARAM)hWnd, MAKELONG(CF_TEXT,aItem));
                    }
                }
            }
        } else {
...
#endif // FE_IME
#ifdef THERESES_DEBUG
DbgPrint("  WindowRect is now %d %d %d %d\n",Console->WindowRect.left, Console->WindowRect.top, Console->WindowRect.right, Console->WindowRect.bottom);
#endif
                    ...
                    // Console is going iconic. Trim working set of all processes in the console
            ...
            case WM_DDE_DATA:
                ...
                hIcon = CreateIconFromResourceEx(&lpIconData->iResource, 0, TRUE, 0x30000, 0, 0, LR_DEFAULTSIZE);
                if (hIcon) {
                    if (Console->hIcon != NULL && Console->hIcon != ghDefaultIcon) {
                        DestroyIcon(Console->hIcon);
                    }
                ...
                if (lpDDEData->fAckReq) {
                    PostMessage(Console->hWndProgMan, WM_DDE_ACK, (WPARAM)hWnd, ReuseDDElParam(lParam, WM_DDE_DATA, WM_DDE_ACK, 0x8000, atomTemp));
                }
                ...
                PostMessage(Console->hWndProgMan, WM_DDE_TERMINATE, (WPARAM)hWnd, 0);
                if (Console->Flags & CONSOLE_IS_ICONIC) {
            ...
            case WM_INPUTLANGCHANGEREQUEST:
                if (CONSOLE_IS_IME_ENABLED()) {
                    ...
                    if (!NT_SUCCESS(ConsoleImeMessagePumpWorker(Console, ConimeMessage, (WPARAM)Console->ConsoleHandle, (LPARAM)lParam, &lResult)) || !lResult) {
                        break;
                    }
                }
#ifdef LATER
            ...
            case WM_INPUTLANGCHANGE:
                ...
                // FE_IME
                if (CONSOLE_IS_IME_ENABLED()) {
                    if (!NT_SUCCESS(ConsoleImeMessagePump(Console, CONIME_INPUTLANGCHANGE, (WPARAM)Console->ConsoleHandle, (LPARAM)Console->hklActive))) {
                        break;
                    }
                    else{
            ...
            case WM_SETFOCUS:
                ...
                if (CONSOLE_IS_IME_ENABLED()) {
                    ...
                    if (!NT_SUCCESS(ConsoleImeMessagePump(Console, CONIME_SETFOCUS, (WPARAM)Console->ConsoleHandle, (LPARAM)Console->hklActive))) {
                        break;
                    }
            ...
            case WM_KILLFOCUS:
                ...
                if (CONSOLE_IS_IME_ENABLED()) {
                    ...
                    if (!NT_SUCCESS(ConsoleImeMessagePump(Console, CONIME_KILLFOCUS, (WPARAM)Console->ConsoleHandle, (LPARAM)NULL))) {
                        break;
                    }
                }
                // end FE_IME
            ...
            case WM_PAINT:
                ...
                if (Console->Flags & CONSOLE_IS_ICONIC || Console->FullScreenFlags == CONSOLE_FULLSCREEN) {
                    RECT rc;
                    UINT cxIcon, cyIcon;
                ...
                break;
            case WM_CLOSE:
                if (!(Console->Flags & CONSOLE_NO_WINDOW) || !(Console->Flags & CONSOLE_WOW_REGISTERED)) {
                    HandleCtrlEvent(Console,CTRL_CLOSE_EVENT);
                }
                break;
            case WM_ERASEBKGND:
                // ICONIC bit is not set if we're fullscreen and don't have the hardware
                if (Console->Flags & CONSOLE_IS_ICONIC || Console->FullScreenFlags == CONSOLE_FULLSCREEN) {
                ...
                if (TRUE || (Console->FullScreenFlags == 0)) {
                    ...
                    if (fMinimized) {
                        if (!(Console->Flags & CONSOLE_IS_ICONIC)) {
                            ...
                            // is painted.
                            if (ScreenInfo->hPalette != NULL && Console->FullScreenFlags == 0) {
                                SelectPalette(Console->hDC, Console->hSysPalette, FALSE);
                        ...
                        if (Console->Flags & CONSOLE_IS_ICONIC) {
                            ...
                            // if the palette is something other than default,
                            // select the default palette in.  otherwise, the
                            // screen will repaint twice each time the icon is painted.
                            if (ScreenInfo->hPalette != NULL && Console->FullScreenFlags == 0) {
                                SelectPalette(Console->hDC, ScreenInfo->hPalette, FALSE);
                                SetActivePalette(ScreenInfo);
                            }
                        }
                    }
                    if (!ScreenInfo->ResizingWindow && (WindowPos->cx || WindowPos->cy) && !fMinimized) {
            ...
            case WM_CONTEXTMENU:
                if (DefWindowProc(hWnd, WM_NCHITTEST, 0, lParam) == HTCLIENT) {
                    TrackPopupMenuEx(Console->hHeirMenu, TPM_RIGHTBUTTON, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), hWnd, NULL);
                } else {
                    goto CallDefWin;
                }
                break;
            case WM_NCLBUTTONDOWN:
                ...
                switch (wParam & 0x00FF) {
                    case HTCAPTION:
                        ...
                        Console = NULL;
                        SetActiveWindow(hWnd);
                        SendMessage(hWnd, WM_SYSCOMMAND, SC_MOVE | wParam, lParam);
                        break;
                    default:
                        goto CallDefWin;
                }
...
// Sep.16.1995 Support Console IME
            ...
            case WM_COMMAND:
                ...
                // FALL THRU
            case WM_SYSCOMMAND:
                if (wParam >= ScreenInfo->CommandIdLow && wParam <= ScreenInfo->CommandIdHigh) {
                    ...
                    if (wParam == SC_RESTORE) {
                        MSG RestoreMsg;
                        SetCapture(hWnd);
                        while (GetCapture() != NULL && (GetKeyState(VK_LBUTTON) & KEY_PRESSED)) {
                            PeekMessage(&RestoreMsg, hWnd, WM_MOUSEFIRST, WM_MOUSELAST, PM_REMOVE);
                        }
                        ReleaseCapture();
                    }
            ...
            case WM_TIMER:
                ...
                if (wParam == SCROLL_WAIT_TIMER)
                {
                    ...
                    if ((ScreenInfo->Console->ConsoleIme.ScrollFlag & (HIDE_FOR_SCROLL)) && (ScreenInfo->Console->ConsoleIme.ScrollWaitCountDown > 0)) {
            ...
            case WM_MOUSEWHEEL:
                ...
                if (abs(ScreenInfo->WheelDelta) >= WHEEL_DELTA && gucWheelScrollLines > 0) {
                    COORD   NewOrigin;
                    SHORT   dy;
                    ...
                    if (!(wParam & MK_SHIFT))
                        dy = (int) min((UINT) CONSOLE_WINDOW_SIZE_Y(ScreenInfo) - 1, gucWheelScrollLines);
                    else
                    ...
                    if (NewOrigin.Y < 0) {
                        NewOrigin.Y = 0;
                    } else if (NewOrigin.Y + CONSOLE_WINDOW_SIZE_Y(ScreenInfo) > ScreenInfo->ScreenBufferSize.Y) {
                        NewOrigin.Y = ScreenInfo->ScreenBufferSize.Y - CONSOLE_WINDOW_SIZE_Y(ScreenInfo);
                    }
                ...
                if (Console->FullScreenFlags == 0) {
                    if (ScreenInfo->hPalette != NULL) {
                        ...
                        if (ScreenInfo->Flags & CONSOLE_GRAPHICS_BUFFER) {
                            WriteRegionToScreenBitMap(ScreenInfo, &ScreenInfo->Window);
                        }
                    } else {
                        SetScreenColors(ScreenInfo, ScreenInfo->Attributes, ScreenInfo->PopupAttributes, TRUE);
                    }
                }
                break;
#if defined(_X86_)
            case WM_FULLSCREEN:
                // This message is sent by the system to tell console that the fullscreen state of a window has changed.
                // In some cases, this message will be sent in response to a call from console to change to fullscreen (Atl-Enter)
                // or may also come directly from the system (switch of focus from a windowed app to a fullscreen app).
            ...
            case CM_SET_WINDOW_SIZE:
                if (lParam == 0x47474747) {
                    Status = InternalSetWindowSize(Console, (PSCREEN_INFORMATION)wParam, &ScreenInfo->Window);
                }
                break;
            case CM_BEEP:
                if (lParam == 0x47474747) {
            ...
            case CM_CONSOLE_MSG:
                ...
                case CM_GET_NLSMODE:
                    ...
                    else if (lParam < 10)
                    {
                         ...
                         */
                        Status = QueueConsoleMessage(Console, CM_GET_NLSMODE, wParam, lParam+1);
                        if (!NT_SUCCESS(Status)) {
                            if (wParam) {
                                NtSetEvent((HANDLE)wParam, NULL);
                                NtClose((HANDLE)wParam);
...
Routine Description:
    ...
    shelldll API DragQueryFile. We didn't use DragQueryFile () because we don't want to load Shell32.dll in CSR
Arguments:
...
Routine Description:
    ...
    It initially calls ConsoleDragQueryFile() to calculate the number of files dropped and then ConsoleDragQueryFile() is called to retrieve the filename.
...
{
    if (CONSOLE_IS_DBCS_OUTPUTCP(Console)) {
        DbcsScreenBuffer->TransBufferCharacter = (PWCHAR)ConsoleHeapAlloc(MAKE_TAG( SCREEN_DBCS_TAG ), (dwScreenBufferSize.X*dwScreenBufferSize.Y*sizeof(WCHAR))+sizeof(WCHAR));
        if (DbcsScreenBuffer->TransBufferCharacter == NULL) {
            return FALSE;
        }

        DbcsScreenBuffer->TransBufferAttribute = (PBYTE)ConsoleHeapAlloc(MAKE_TAG( SCREEN_DBCS_TAG ), (dwScreenBufferSize.X*dwScreenBufferSize.Y*sizeof(BYTE))+sizeof(BYTE));
        if (DbcsScreenBuffer->TransBufferAttribute == NULL) {
            ConsoleHeapFree(DbcsScreenBuffer->TransBufferCharacter);
            return FALSE;
        }

        DbcsScreenBuffer->TransWriteConsole = (PWCHAR)ConsoleHeapAlloc(MAKE_TAG( SCREEN_DBCS_TAG ), (dwScreenBufferSize.X*dwScreenBufferSize.Y*sizeof(WCHAR))+sizeof(WCHAR));
        ...
        DbcsScreenBuffer->KAttrRows = (PBYTE)ConsoleHeapAlloc(MAKE_TAG( SCREEN_DBCS_TAG ), dwScreenBufferSize.X*dwScreenBufferSize.Y*sizeof(BYTE));
...
BOOL ReCreateDbcsScreenBufferWorker(IN PCONSOLE_INFORMATION Console,IN PSCREEN_INFORMATION ScreenInfo)
{
    ...
    if (!CreateDbcsScreenBuffer(Console,dwScreenBufferSize,&NewDbcsScreenBuffer)) {
        return FALSE;
    }
