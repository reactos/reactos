...
HANDLE FindActiveScreenBufferHandle(IN PCONSOLE_PER_PROCESS_DATA ProcessData, IN PCONSOLE_INFORMATION Console)
{
    ...
    for (i=0;i<ProcessData->HandleTableSize;i++) {
        Status = DereferenceIoHandleNoCheck(ProcessData, (HANDLE) i, &ActiveScreenHandleData);
        if (NT_SUCCESS(Status) && Console->CurrentScreenBuffer == ActiveScreenHandleData->Buffer.ScreenBuffer) {
...
--*/
{
    ...
    try {
        ...
        if (a->HandleType == CONSOLE_INPUT_HANDLE) {
            Status = AllocateIoHandle(ProcessData, a->HandleType, &Handle);
            if (!NT_SUCCESS(Status)) {
                leave;
            }
            ...
            Status = ConsoleAddShare(a->DesiredAccess,a->ShareMode,&HandleData->Buffer.InputBuffer->ShareAccess,HandleData);
            if (!NT_SUCCESS(Status)) {
            ...
            Status = ConsoleAddShare(a->DesiredAccess,a->ShareMode,&HandleData->Buffer.ScreenBuffer->ShareAccess,HandleData);
            if (!NT_SUCCESS(Status)) {
        ...
        if (!NT_SUCCESS(Status) && Handle != (HANDLE)-1) {
            FreeIoHandle(ProcessData,Handle);
        }
...
/*++
Routine Description:
    This routine is used in stream input.
    It gets input and filters it for unicode characters.
Arguments:
    ...
    while (TRUE) {
        ...
        if (Event.EventType == KEY_EVENT) {
            ...
            if (Event.Event.KeyEvent.uChar.UnicodeChar != 0 && !fCommandLineEditKey) {
                ...
                else if (Event.Event.KeyEvent.bKeyDown && Event.Event.KeyEvent.wVirtualKeyCode != VK_ESCAPE && Event.Event.KeyEvent.uChar.UnicodeChar != 0x0a) {
                    *Char = Event.Event.KeyEvent.uChar.UnicodeChar;
                    return STATUS_SUCCESS;
                }
            }
            ...
            if (Event.Event.KeyEvent.bKeyDown) {
                SHORT sTmp;
                if (ARGUMENT_PRESENT(CommandLineEditingKeys) && fCommandLineEditKey) {
                    *CommandLineEditingKeys = TRUE;
                    *Char = (WCHAR) Event.Event.KeyEvent.wVirtualKeyCode;
                    return STATUS_SUCCESS;
                }
                else if (ARGUMENT_PRESENT(CommandLinePopupKeys) && fCommandLineEditKey) {
...
--*/
{
    ...
    Status = DereferenceIoHandleNoCheck(RawReadData->ProcessData, RawReadData->HandleIndex, &HandleData);
    ASSERT (NT_SUCCESS(Status));

    // see if this routine was called by CloseInputHandle.
    // if it was, see if this wait block corresponds to the dying handle.
    // if it doesn't, just return.
    if (SatisfyParameter1 != NULL && SatisfyParameter1 != HandleData) {
        return FALSE;
    }
    ...
    try {
        ...
        // via CsrNotifyWaitBlock.   if so, just decrement the ReadCount and return.
        if (WaitFlags & CSR_PROCESS_TERMINATING) {
            Status = STATUS_THREAD_IS_TERMINATING;
            leave;
        }

        // We must see if we were woken up because the handle is being closed.
        // if so, we decrement the read count.
        // if it goes to zero, we wake up the close thread.
        // otherwise, we wake up any other thread waiting for data.
        ...
        // if we get to here, this routine was called either by the input thread or a write routine.
        // both of these callers grab the current console lock.
...
#ifdef FE_SB
        IsConsoleFullWidth(Console->hDC, Console->CP,*lpBuffer) ? NumBytes+=2 : NumBytes++;
#endif
        lpBuffer++;
        ...
        if (Status != CONSOLE_STATUS_WAIT) {
            if (!a->Unicode) {
                ...
                if (CONSOLE_IS_DBCS_CP(Console))
                {
                    a->NumBytes = TranslateUnicodeToOem(Console, lpBuffer, a->NumBytes / sizeof (WCHAR), TransBuffer, NumBytes, &HandleData->Buffer.InputBuffer->ReadConInpDbcsLeadByte);
                }
                else
#endif
                a->NumBytes = ConvertToOem(RawReadData->Console->CP, lpBuffer, a->NumBytes / sizeof (WCHAR), TransBuffer, a->NumBytes / sizeof (WCHAR));
...
--*/
{
    ...
    *Status = STATUS_SUCCESS;
    if (CookedReadData->BytesRead >= (CookedReadData->BufferSize-(2*sizeof(WCHAR))) && Char != UNICODE_CARRIAGERETURN && Char != UNICODE_BACKSPACE) {
        return FALSE;
    }

    if (CookedReadData->CtrlWakeupMask != 0 && Char < L' ' && (CookedReadData->CtrlWakeupMask & (1 << Char))) {
    ...
    if (AT_EOL(CookedReadData)) {
        ...
        if (Char != UNICODE_BACKSPACE || CookedReadData->BufPtr != CookedReadData->BackupLimit) {
...
eol_repeat:
            ...
            if (Char == UNICODE_BACKSPACE && CookedReadData->Processed) {
                ...
                *CookedReadData->BufPtr=(WCHAR)' ';
                ...
                if (gExtendedEditKey && wchOrig == EXTKEY_ERASE_PREV_WORD && CookedReadData->BufPtr != CookedReadData->BackupLimit && fStartFromDelim ^ !IS_WORD_DELIM(CookedReadData->BufPtr[-1])) {
                    goto eol_repeat;
                }
            }
            else {
...
bs_repeat:
                ...
                CookedReadData->BufPtr-=1;
                CookedReadData->CurrentPosition-=1;
                RtlCopyMemory(CookedReadData->BufPtr, CookedReadData->BufPtr+1, CookedReadData->BytesRead - (CookedReadData->CurrentPosition * sizeof(WCHAR)));
#if defined(FE_SB)
                {
                    PWCHAR buf = (PWCHAR)((PBYTE)CookedReadData->BackupLimit + CookedReadData->BytesRead    );
                    *buf = (WCHAR)' ';
                }
#endif
                NumSpaces = 0;
            ...
            if (Char == UNICODE_CARRIAGERETURN) {
                ...
                *CookedReadData->BufPtr = Char;
                ...
                if (INSERT_MODE(CookedReadData)) {
                    memmove(CookedReadData->BufPtr+1, CookedReadData->BufPtr, CookedReadData->BytesRead - (CookedReadData->CurrentPosition * sizeof(WCHAR)));
                    CookedReadData->BytesRead += sizeof(WCHAR);
                }
                *CookedReadData->BufPtr = Char;
                CookedReadData->BufPtr+=1;
        ...
        if (CookedReadData->Echo && CallWrite) {
            ...
            *Status = WriteCharsFromInput(CookedReadData->ScreenInfo,
                                ...
                                CookedReadData->OriginalCursorPosition.X,
                                (Char != UNICODE_CARRIAGERETURN) ? WC_DESTRUCTIVE_BACKSPACE | WC_ECHO : WC_DESTRUCTIVE_BACKSPACE | WC_KEEP_CURSOR_VISIBLE | WC_ECHO,
                                &ScrollY
                                );
            if (!NT_SUCCESS(*Status)) {
                RIPMSG1(RIP_WARNING, "WriteCharsFromInput failed %x", *Status);
            ...
            if (Char != UNICODE_CARRIAGERETURN) {
                ...
                *Status = AdjustCursorPosition(CookedReadData->ScreenInfo, CursorPosition, TRUE, NULL);
                ASSERT(NT_SUCCESS(*Status));
                if (!NT_SUCCESS(*Status)) {
                    CookedReadData->BytesRead = 0;
                    return TRUE;
...
NTSTATUS CookedRead(IN PCOOKED_READ_DATA CookedReadData, IN PCSR_API_MSG WaitReplyMessage, IN PCSR_THREAD WaitingThread, IN BOOLEAN WaitRoutine)
{
    ...
    Status = DereferenceIoHandleNoCheck(CookedReadData->ProcessData, CookedReadData->HandleIndex, &HandleData);
    ASSERT (NT_SUCCESS(Status));
    a = (PCONSOLE_READCONSOLE_MSG)&WaitReplyMessage->u.ApiMessageData;
    while (CookedReadData->BytesRead < CookedReadData->BufferSize) {
        ...
        // we should probably set these up in GetChars, but we set them up here because the debugger is multi-threaded and calls read before outputting the prompt.
        ...
        if (CommandLineEditingKeys) {
            ...
            if (!NT_SUCCESS(Status)) {
                if (Status == CONSOLE_STATUS_WAIT_NO_BLOCK) {
                    ...
                    if (!WaitRoutine) {
                        ...
                        WaitForMoreToRead(CookedReadData->InputInfo,WaitReplyMessage,CookedReadWaitRoutine,CookedReadData,sizeof(*CookedReadData),FALSE);
                    }
                } else {
                    CookedReadData->BytesRead = 0;
                }
            ...
            if (ProcessCookedReadInput(CookedReadData,Char,KeyState,&Status)) {
    ...
    // data.  also, close the temporary output handle that was opened to echo the characters read.
    if (Status != CONSOLE_STATUS_WAIT) {
        DWORD LineCount=1;
        if (CookedReadData->Echo) {
            BOOLEAN FoundCR;
            ...
            CloseOutputHandle(CONSOLE_FROMTHREADPERPROCESSDATA(WaitingThread), CookedReadData->Console, &CookedReadData->TempHandle, NULL, FALSE);
        }
        WaitReplyMessage->ReturnValue = Status;

        // at this point, a->NumBytes contains the number of bytes in
        // the UNICODE string read.  UserBufferSize contains the converted size of the app's buffer.
            ...
            if (LineCount > 1) {
                ...
                if (!a->Unicode && CONSOLE_IS_DBCS_CP(CookedReadData->Console)) {
                    ...
                    for (Tmp=CookedReadData->BackupLimit;
                         *Tmp!=UNICODE_LINEFEED && CookedReadData->UserBufferSize/sizeof(WCHAR) > NumBytes;
                         (IsConsoleFullWidth(CookedReadData->Console->hDC, CookedReadData->Console->CP,*Tmp) ? NumBytes+=2 : NumBytes++),Tmp++) ;
                }
#endif
                ...
                if (!a->Unicode && CONSOLE_IS_DBCS_CP(CookedReadData->Console)) {
                    ...
                    for (Tmp=CookedReadData->BackupLimit;
                         ...
                         (IsConsoleFullWidth(CookedReadData->Console->hDC, CookedReadData->Console->CP,*Tmp) ? NumBytes+=2 : NumBytes++),Tmp++,NumToWrite-=sizeof(WCHAR)) ;
                }
#endif
...
Arguments:
    ...
    SatisfyParameter1 - if this routine was called (indirectly) by CloseInputHandle, this argument contains a HandleData pointer of the dying handle.  otherwise, it contains NULL.
    SatisfyParameter2 - if this routine is called because a ctrl-c or ctrl-break was seen, this argument contains CONSOLE_CTRL_SEEN. otherwise it contains NULL.
    WaitFlags - Flags indicating status of wait.
--*/
{
    NTSTATUS Status;
    ...
    // thread or a write routine.  both of these callers grab the current console lock.
    ...
    if (CookedReadData->CommandHistory) {
        ...
        if (!CLE_NO_POPUPS(CookedReadData->CommandHistory)) {
            ...
            Status = (Popup->PopupInputRoutine)(CookedReadData, WaitReplyMessage, WaitingThread, TRUE);
    ...
    if (a->CaptureBufferSize <= BUFFER_SIZE && CookedReadData->BytesRead == 0) {
        CookedReadData->UserBuffer = a->Buffer;
    }
    Status = CookedRead(CookedReadData,WaitReplyMessage,WaitingThread,TRUE);
    if (Status != CONSOLE_STATUS_WAIT) {
        return TRUE;
    } else {
        return FALSE;
...
--*/
{
    ...
    *NumBytes = 0;
    ...
    if (HandleData->InputReadData->InputHandleFlags & HANDLE_INPUT_PENDING) {
        // if we have leftover input, copy as much fits into the user's buffer and return.
        // we may have multi line input, if a macro has been defined that contains the $T character.
        if (HandleData->InputReadData->InputHandleFlags & HANDLE_MULTI_LINE_INPUT) {
            ...
            if (!Unicode && CONSOLE_IS_DBCS_CP(Console)) {
                ...
                if (HandleData->InputReadData->BytesAvailable == 0 || BufferSize == 0) {
                    ...
                    *NumBytes = 1;
                    ...
                    for (NumToWrite=0,Tmp=HandleData->InputReadData->CurrentBufPtr,NumToBytes=0;
                         ...
                         (IsConsoleFullWidth(Console->hDC, Console->CP,*Tmp) ? NumToBytes+=2 : NumToBytes++),Tmp++,NumToWrite+=sizeof(WCHAR)) ;
                }
            }
#endif
...
#endif
            NumToWrite = (BufferSize < HandleData->InputReadData->BytesAvailable) ? BufferSize : HandleData->InputReadData->BytesAvailable;
        }
        ...
        if (!Unicode) {
            ...
            if (CONSOLE_IS_DBCS_CP(Console))
            {
                NumToWrite = TranslateUnicodeToOem(Console,lpBuffer,NumToWrite / sizeof (WCHAR),TransBuffer,NumToBytes,&HandleData->Buffer.InputBuffer->ReadConInpDbcsLeadByte);
            }
            else
#endif
            NumToWrite = ConvertToOem(Console->CP,lpBuffer,NumToWrite / sizeof (WCHAR),TransBuffer,NumToWrite / sizeof (WCHAR));
    ...
    // we need to create a temporary handle to the current screen buffer if echo is on.
    if ((InputInfo->InputMode & (ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT)) == (ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT)) {
        HANDLE ActiveScreenHandle;
        ...
        if (ActiveScreenHandle != INVALID_HANDLE_VALUE) {
            ...
            if (TempHandle.Buffer.ScreenBuffer != NULL) {
                Status = ConsoleAddShare(GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, &TempHandle.Buffer.ScreenBuffer->ShareAccess, &TempHandle);
                if (NT_SUCCESS(Status)) {
                    Echo = TRUE;
    ...
    if (InputInfo->InputMode & ENABLE_LINE_INPUT) {
        ...
        // since we may wait inside this loop, store all important variables in the read data structure.
        // if we do wait, a read data structure will be allocated from the heap and its pointer will be stored in the wait block.
        // the CookedReadData will be copied into the structure.
        // the data is freed when the read is completed.
        COOKED_READ_DATA CookedReadData;
        ULONG i;
        PWCHAR TempBuffer;
        ULONG TempBufferSize;

        // to emulate OS/2 KbdStringIn, we read into our own big buffer (256 bytes) until the user types enter.
        // then return as many chars as will fit in the user's buffer.
        ...
        if (TempBuffer==NULL) {
            if (Echo) {
                CloseOutputHandle(ProcessData,Console,&TempHandle,NULL,FALSE);
            }
            return STATUS_NO_MEMORY;
        }

        // initialize the user's buffer to spaces.
        // this is done so that moving in the buffer via cursor doesn't do strange things.
        ...
        Status = CookedRead(&CookedReadData,Message,CSR_SERVER_QUERYCLIENTTHREAD(),FALSE);
#ifdef FE_SB
        if (Status != CONSOLE_STATUS_WAIT) {
    ...
    // character (raw) mode
        ...
        // read at least one character in.
        // after one character has been read, get any more available characters and return.
        // the first call to GetChar may wait.
        // if we do wait, a read data structure will be allocated from the heap and its pointer will be stored in the wait block.
        // the RawReadData will be copied into the structure.
        // the data is freed when the read is completed.
        ...
        if (*NumBytes < BufferSize) {
            ...
            *NumBytes += sizeof(WCHAR);
            ...
            if (!Unicode) {
                ...
                if (CONSOLE_IS_DBCS_CP(Console))
                {
                    *NumBytes = TranslateUnicodeToOem(Console,lpBuffer,NumToWrite / sizeof (WCHAR),TransBuffer,*NumBytes,&HandleData->Buffer.InputBuffer->ReadConInpDbcsLeadByte);
                }
                else
#endif
                *NumBytes = ConvertToOem(Console->CP,lpBuffer,*NumBytes / sizeof (WCHAR),TransBuffer,*NumBytes / sizeof (WCHAR));
...
--*/
{
    ...
    ProcessData = CONSOLE_PERPROCESSDATA();
    Status = DereferenceIoHandle(ProcessData,a->InputHandle,CONSOLE_INPUT_HANDLE,GENERIC_READ,&HandleData);
    if (!NT_SUCCESS(Status)) {
        a->NumBytes = 0;
    } else {
        if (a->CaptureBufferSize <= BUFFER_SIZE) {
...
VOID MakeCursorVisible(IN PSCREEN_INFORMATION ScreenInfo,IN COORD CursorPosition)
{
    ...
    if (WindowOrigin.X != 0 || WindowOrigin.Y != 0) {
        Status = SetWindowOrigin(ScreenInfo,FALSE,WindowOrigin);
        if (!NT_SUCCESS(Status)) {
            return;
        }
    }
...
/*++
Routine Description:
    This routine determines whether the text string contains characters that require special processing.
    If it doesn't, unicode characters.
    The string is also copied to the input buffer, if the output mode is line mode.
Arguments:
...
--*/
{
    ...
    while (NumChars) {
        if (*StrPtr < UNICODE_SPACE) {
            Tmp = (PDWORD)StrPtr;
            if (NumChars == 2 && *Tmp == WRITE_UNICODE_CRLF) {
                return WRITE_CR_LF;
            } else if (NumChars == 1 && *StrPtr == (WCHAR)'\r') {
                return WRITE_CR;
            } else {
...
VOID UnblockWriteConsole(IN PCONSOLE_INFORMATION Console, IN DWORD Reason)
{
    ...
    if ((Console->Flags & (CONSOLE_SUSPENDED | CONSOLE_SELECTING | CONSOLE_SCROLLBAR_TRACKING)) == 0) {
        ...
        if (CsrNotifyWait(&Console->OutputQueue, TRUE, NULL, NULL)) {
            ...
            ASSERT ((Console->WaitQueue == NULL) || (Console->WaitQueue == &Console->OutputQueue));
...
--*/
{
    ...
    Status = DereferenceIoHandle(CONSOLE_PERPROCESSDATA(),a->OutputHandle,CONSOLE_OUTPUT_HANDLE,GENERIC_WRITE,&HandleData);
    if (!NT_SUCCESS(Status)) {
        UnlockConsole(Console);
        return Status;
    }
    ...
    )
{
    ...
    Status = DereferenceConsoleHandle(a->ConsoleHandle,&Console);
    UnlockConsoleHandleTable();
    ASSERT (NT_SUCCESS(Status));

    // if we get to here, this routine was called by the input thread, which grabs the current console lock.

    // this routine should be called by a thread owning the same lock on the same console as we're reading from.
    ASSERT (ConsoleLocked(Console));

    // if we're unicode, the string may still be in the message buffer.
    // since the message was reallocated and copied when the wait was created, we need to fix up a->TransBuffer here.
...
--*/
{
    ...
    ProcessData = CONSOLE_PERPROCESSDATA();
    Status = DereferenceIoHandleNoCheck(ProcessData,HANDLE_TO_INDEX(a->SourceHandle),&SourceHandleData);
    if (!NT_SUCCESS(Status)) {
        goto exit;
    }
    ...
    Status = AllocateIoHandle(ProcessData,SourceHandleData->HandleType,&a->TargetHandle);
    if (!NT_SUCCESS(Status)) {
        goto exit;
    }

    // it's possible that AllocateIoHandle realloced the handle table, so deference SourceHandle again.
    Status = DereferenceIoHandleNoCheck(ProcessData,HANDLE_TO_INDEX(a->SourceHandle),&SourceHandleData);
    ASSERT (NT_SUCCESS(Status));
    if (!NT_SUCCESS(Status)) {
        goto exit;
    }
    Status = DereferenceIoHandleNoCheck(ProcessData,a->TargetHandle,&TargetHandleData);
    ASSERT (NT_SUCCESS(Status));
    if (!NT_SUCCESS(Status)) {
        FreeIoHandle(ProcessData,a->TargetHandle);
        goto exit;
    }
    if (SourceHandleData->HandleType & CONSOLE_INPUT_HANDLE) {
        // grab input lock
        if (!InitializeInputHandle(TargetHandleData,SourceHandleData->Buffer.InputBuffer)) {
            FreeIoHandle(ProcessData,a->TargetHandle);
            Status = STATUS_NO_MEMORY;
            goto exit;
        }
    ...
    Status = ConsoleDupShare(a->DesiredAccess,SourceHandleData->ShareAccess,ShareAccess,TargetHandleData);
    if (!NT_SUCCESS(Status)) {
        FreeIoHandle(ProcessData,a->TargetHandle);
        if (SourceHandleData->HandleType & CONSOLE_INPUT_HANDLE) {
...
--*/
{
    ...
    Status = DereferenceIoHandleNoCheck(CONSOLE_PERPROCESSDATA(),HANDLE_TO_INDEX(a->Handle),&HandleData);
    if (NT_SUCCESS(Status)) {
        a->Flags = 0;
...
/*++
Routine Description:
    This routine closes an input handle.
    It decrements the input buffer's reference count.
    If it goes to zero, the buffer is reinitialized.
...
--*/
{
    ...
    if (HandleData->InputReadData->ReadCount != 0) {
        ...
        WaitSatisfied |= CsrNotifyWait(&HandleData->Buffer.InputBuffer->ReadWaitQueue,TRUE, (PVOID) HandleData, NULL);
        LockReadCount(HandleData);
    }
    if (WaitSatisfied) {
        // #334370 under stress, WaitQueue may already hold the satisfied waits
        ASSERT ((Console->WaitQueue == NULL) || (Console->WaitQueue == &HandleData->Buffer.InputBuffer->ReadWaitQueue));
    ...
    if (HandleData->Buffer.InputBuffer->RefCount == 0) {
        ...
        ConsoleRemoveShare(HandleData->Access, HandleData->ShareAccess, &HandleData->Buffer.InputBuffer->ShareAccess);
    }
    return FreeIoHandle(ProcessData,Handle);
}


NTSTATUS CloseOutputHandle(
    IN PCONSOLE_PER_PROCESS_DATA ProcessData,
    IN PCONSOLE_INFORMATION Console,
    IN PHANDLE_DATA HandleData,
    IN HANDLE Handle,
...
/*++
Routine Description:
    This routine closes an output handle.
    It decrements the screen buffer's reference count.
    If it goes to zero, the buffer is freed.
    Otherwise, the handle is removed from sharing.
Arguments:
    ProcessData - Pointer to per process data.
    ...
    HandleData - Pointer to handle data structure.
    Handle - Handle to close.
    FreeHandle - if TRUE, free handle.  used by ReadChars in echo mode and by process cleanup.
Note:
    The console lock must be held when calling this routine.
--*/
{
    ...
    if (HandleData->Buffer.ScreenBuffer->RefCount == 0) {
        ...
        if (HandleData->Buffer.ScreenBuffer == Console->CurrentScreenBuffer && Console->ScreenBuffers != Console->CurrentScreenBuffer) {
        ...
        Status = ConsoleRemoveShare(HandleData->Access, HandleData->ShareAccess, &HandleData->Buffer.ScreenBuffer->ShareAccess);
    }
...
--*/
{
    ...
    ProcessData = CONSOLE_PERPROCESSDATA();
    Status = DereferenceIoHandleNoCheck(ProcessData, HANDLE_TO_INDEX(a->Handle), &HandleData);
    if (NT_SUCCESS(Status)) {
...
/*++
Routine Description:
    This routine converts chars from their true unicode representation to the Unicode representation (UnicodeAnsi) that will generate the correct glyph given an OEM font and an ANSI translation by GDI.
...
Arguments:
    ...
    lpBuffer - Pointer to buffer to copy string to.  assumed to be at least as long as lpString.  This pointer is updated to point to the next position in the buffer.
    lpString - Pointer to string to write.
    NumBytes - On input, number of bytes to write.  On output, number of bytes written.
...
Note:
    This routine does not process tabs and backspace properly.
    That code will be implemented as part of the line editing services.
--*/
{
    DWORD Length,i;
    ...
    if (!(ScreenInfo->Flags & CONSOLE_OEMFONT_DISPLAY) || (ScreenInfo->Console->FullScreenFlags & CONSOLE_FULLSCREEN)) {
        goto SimpleWrite;
    }
...
SimpleWrite:
    return WriteChars(ScreenInfo,lpBufferBackupLimit,lpBuffer,lpString,NumBytes,NumSpaces,OriginalXPosition,dwFlags,ScrollY);
}
