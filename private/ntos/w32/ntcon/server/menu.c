...
VOID InitSystemMenu(IN PCONSOLE_INFORMATION Console)
{
    ...
    if (!AppendMenu(Console->hMenu, MF_POPUP | MF_STRING, (ULONG_PTR)Console->hHeirMenu,
               //"Edit"
               ItemString)) {
...
--*/
{
    ...
    if (Console->Flags & CONSOLE_IS_ICONIC) {
        ...
        if ((Console->CurrentScreenBuffer->WindowMaximizedX && Console->CurrentScreenBuffer->WindowMaximizedY) || Console->Flags & CONSOLE_SELECTING) {
            EnableMenuItem(hHeirMenu,cmScroll,MF_GRAYED);
        } else {
...
/*++
    Displays the properties dialog and updates the window state, if necessary.
--*/
{
    ...
    pStateInfo = NULL;
    ulViewSize = 0;
    Status = NtMapViewOfSection(hSection, NtCurrentProcess(), &pStateInfo, 0, 0, NULL, &ulViewSize, ViewUnmap, 0, PAGE_READWRITE);
    if (!NT_SUCCESS(Status)) {
...
// it will be incorrect. Jun-26-1996
        ...
        if (CONSOLE_IS_DBCS_ENABLED() && TM_IS_TT_FONT(SCR_FAMILY(ScreenInfo)))
        {
            if ((SCR_FONTNUMBER(ScreenInfo) >= 0 ) && (SCR_FONTNUMBER(ScreenInfo) < NumberOfFonts)) {
    ...
    RtlCopyMemory(pStateInfo->ColorTable, Console->ColorTable, sizeof(Console->ColorTable));
...
PropCallback:
    ...
    hThread = InternalCreateCallbackThread(ProcessHandleRecord->ProcessHandle, (ULONG_PTR)MyPropRoutine, (ULONG_PTR)hClientSection);
    if (!hThread) {
...
--*/
{
    ...
    pStateInfo = NULL;
    ulViewSize = 0;
    Status = NtMapViewOfSection(hSection, NtCurrentProcess(), &pStateInfo, 0, 0, NULL, &ulViewSize, ViewUnmap, 0, PAGE_READONLY);
    if (!NT_SUCCESS(Status)) {
        KdPrint(("CONSRV: error %x mapping view of file\n", Status));
        NtClose(hSection);
        return;
