...
BOOL LoadLink( LPWSTR pszLinkName, CShellLink * this )
{
    ...
    if (this->sld.dwFlags & (SLDF_HAS_LINK_INFO))
    {
        ...
        if (cbSize >= sizeof(cbSize))
        {
            ...
            fResult &= (SetFilePointer(hFile,cbSize,NULL,FILE_CURRENT)!=0xFFFFFFFF);
        }
    }
...
*/
    ...
    RtlZeroMemory( &mld, sizeof(mld) );// Zero out structure on the stack
    ...
    for( lpExtraData = (LPNT_CONSOLE_PROPS)mld.pExtraData; lpExtraData && lpExtraData->cbSize; (LPBYTE)lpExtraData += dwSize)
    {
        ...
        if (dwSize)
        {
            ...
            {
                RtlCopyMemory( &((LPLNKPROPNTCONSOLE)lpvBuffer)->console_props, lpExtraData, sizeof( NT_CONSOLE_PROPS ));
            ...
            if (lpExtraData->dwSignature == NT_FE_CONSOLE_PROPS_SIG)
            {
                ...
                RtlCopyMemory( &((LPLNKPROPNTCONSOLE)lpvBuffer)->fe_console_props, lpFEExtraData, sizeof( NT_FE_CONSOLE_PROPS ));
            }
#endif
        }
    }
