...
#if defined(i386)
NTSTATUS ImeWmFullScreen(IN BOOL Foreground, IN PCONSOLE_INFORMATION Console, IN PSCREEN_INFORMATION ScreenInfo)
{
    ...
    {
        if (!NT_SUCCESS(ConsoleImeMessagePump(Console, CONIME_KILLFOCUS, (WPARAM)Console->ConsoleHandle, (LPARAM)Console->hklActive))) {
            return STATUS_INVALID_HANDLE;
        }
    }
...
--*/
{
    ...
    if (!NT_SUCCESS(ConsoleImeMessagePump(Console, CONIME_SET_NLSMODE, (WPARAM)Console->ConsoleHandle, (LPARAM)fdwConversion))) {
        return STATUS_INVALID_HANDLE;
    }
...
NTSTATUS SetImeCodePage(IN PCONSOLE_INFORMATION Console)
{
    ...
    if (CONSOLE_IS_IME_ENABLED()) {
        if (!NT_SUCCESS(ConsoleImeMessagePump(Console, CONIME_NOTIFY_CODEPAGE, (WPARAM)Console->ConsoleHandle, (LPARAM)MAKELPARAM(FALSE, CodePage)))) {
            return STATUS_INVALID_HANDLE;
        }
    }
...
NTSTATUS SetImeOutputCodePage(IN PCONSOLE_INFORMATION Console, IN PSCREEN_INFORMATION ScreenInfo, IN DWORD PrevCodePage)
{
    DWORD CodePage = Console->OutputCP;

    // Output code page
    if ((ScreenInfo->Flags & CONSOLE_TEXTMODE_BUFFER) && (IsAvailableFarEastCodePage(CodePage) || IsAvailableFarEastCodePage(PrevCodePage)))
    {
...
#ifdef i386
    if ( (Console->FullScreenFlags & CONSOLE_FULLSCREEN_HARDWARE) && !(ScreenInfo->Flags & CONSOLE_GRAPHICS_BUFFER))
    {
        SetROMFontCodePage(CodePage, ScreenInfo->BufferInfo.TextInfo.ModeIndex);
        SetCursorInformationHW(ScreenInfo, ScreenInfo->BufferInfo.TextInfo.CursorSize, ScreenInfo->BufferInfo.TextInfo.CursorVisible);
        WriteRegionToScreenHW(ScreenInfo, &ScreenInfo->Window);
    }
#endif

    if (CONSOLE_IS_IME_ENABLED()) {
        if (!NT_SUCCESS(ConsoleImeMessagePump(Console, CONIME_NOTIFY_CODEPAGE, (WPARAM)Console->ConsoleHandle, (LPARAM)MAKELPARAM(TRUE, CodePage)))) {
            return STATUS_INVALID_HANDLE;
        }
    }
    ...
    )
{
    ...
    if (!NT_SUCCESS( Status )) {
        ...
        for( dwIndex = 0; ; dwIndex++) {
            ...
            pTTFontList = ConsoleHeapAlloc(MAKE_TAG( SCREEN_DBCS_TAG ), sizeof(TTFONTLIST));
            if (pTTFontList == NULL) {
                break;
            }
    ...
    if (!NT_SUCCESS( Status )) {
         ...
         * InitialPalette
         */
        Status = MyRegQueryValueEx(hkRegistry, MACHINE_REGISTRY_INITIAL_PALETTE, sizeof( Buffer ), Buffer, &Length);
        if (NT_SUCCESS( Status ) && Length > sizeof(DWORD)) {
            ...
            {
                Palette = ConsoleHeapAlloc(MAKE_TAG( BUFFER_TAG ), Length);
                if (Palette != NULL)
                {
         ...
         * ColorBuffer
         */
        Status = MyRegQueryValueEx(hkRegistry, MACHINE_REGISTRY_COLOR_BUFFER, sizeof( Buffer ), Buffer, &Length);
        if (NT_SUCCESS( Status ) && Length > sizeof(DWORD)) {
            ...
            {
                Color = ConsoleHeapAlloc(MAKE_TAG( BUFFER_TAG ), Length);
                if (Color != NULL)
                {
         ...
         */
        Status = MyRegQueryValueEx(hkRegistry, MACHINE_REGISTRY_COLOR_BUFFER_NO_TRANSLATE, sizeof( Buffer ), Buffer, &Length);
        if (NT_SUCCESS( Status ) && Length > sizeof(DWORD)) {
            DWORD ColorBufferLength = ((LPDWORD)Buffer)[0];
            PUCHAR Color;
         ...
         * ModeFontPairs
         */
        Status = MyRegQueryValueEx(hkRegistry, MACHINE_REGISTRY_MODE_FONT_PAIRS, sizeof( Buffer ), Buffer, &Length);
        if (NT_SUCCESS( Status ) && Length > sizeof(DWORD)) {
            ...
            {
                ModeFont = ConsoleHeapAlloc(MAKE_TAG( BUFFER_TAG ), Length);
                if (ModeFont != NULL)
                {
                    Length -= sizeof(DWORD);
         ...
         */
        {
            ...
            Status = MyRegOpenKey(hkRegistry, MACHINE_REGISTRY_FS_CODEPAGE, &hkRegCP);
            if (!NT_SUCCESS( Status )) {
                ...
                for( dwIndex = 0; ; dwIndex++) {
                    Status = MyRegEnumValue(hkRegCP, dwIndex, sizeof(awchValue), (LPWSTR)&awchValue, sizeof(awchData),  (PBYTE)&awchData);
                    if (!NT_SUCCESS( Status )) {
                        break;
                    }

                    pFsCodePage = ConsoleHeapAlloc(MAKE_TAG( BUFFER_TAG ), sizeof(FS_CODEPAGE));
                    if (pFsCodePage == NULL) {
                        break;
                    }
...
NTSTATUS InitConsoleIMEStuff(HDESK hDesktop, DWORD dwConsoleThreadId, PCONSOLE_INFORMATION Console)
{
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    if (RegConIMEInfo.dwThreadId == 0)
    {
         ...
         */
        hThread = InternalCreateCallbackThread(CONSOLE_CLIENTPROCESSHANDLE(), (ULONG_PTR)ConsoleIMERoutine, (ULONG_PTR)0);
        if (hThread == NULL) {
        ...
        else
        {
             ...
             */
            Status = QueueThreadMessage(dwConsoleThreadId, CM_WAIT_CONIME_PROCESS, (WPARAM)hDesktop, (LPARAM)hThread);
            if (!NT_SUCCESS(Status)) {
    ...
    Status = QueueThreadMessage(dwConsoleThreadId, CM_CONIME_CREATE, (WPARAM)Console->ConsoleHandle, (LPARAM)First);
    if (!NT_SUCCESS(Status)) {
...
NTSTATUS WaitConsoleIMEStuff(HDESK hDesktop, HANDLE hThread)
{
    NTSTATUS Status = STATUS_SUCCESS;
    CONSOLE_REGISTER_CONSOLEIME RegConIMEInfo;
    ...
    )
{
    ...
    else
    {
        ...
        if ( (dwAction == REGCONIME_UNREGISTER) || (dwAction == REGCONIME_TERMINATE)     )
        {
            ...
            Status = NtUserConsoleControl(ConsoleRegisterConsoleIME, &RegConIMEInfo, sizeof(RegConIMEInfo));
            if (NT_SUCCESS(Status)) {
                Status = QueueThreadMessage(RegConIMEInfo.dwConsoleInputThreadId, CM_SET_CONSOLEIME_WINDOW, (WPARAM)NULL, 0);
                if (!NT_SUCCESS(Status)) {
                    RIPMSG1(RIP_WARNING, "ConSrvRegisterConsoleIME: QueueThreadMessage failed (%08x)\n", Status);
                    Status = STATUS_UNSUCCESSFUL;
                    goto ErrorExit;
...
VOID RemoveConsoleIME(PCSR_PROCESS Process, DWORD dwConsoleIMEThreadId)
{
    NTSTATUS Status;
    CONSOLE_REGISTER_CONSOLEIME RegConIMEInfo;
    PCONSOLE_PER_PROCESS_DATA ProcessData;
    ...
    if (dwConsoleIMEThreadId == RegConIMEInfo.dwThreadId)
    {
         ...
         */
        Status = ConSrvRegisterConsoleIME(Process, ProcessData->hDesk, ProcessData->hWinSta, NULL, dwConsoleIMEThreadId, REGCONIME_TERMINATE, NULL);
    }
    RtlLeaveCriticalSection(&ConIMEInitWindowsLock);
}
...
#if defined(DBG) && defined(DBG_KATTR)
VOID BeginKAttrCheck(IN PSCREEN_INFORMATION ScreenInfo)
{
    SHORT RowIndex;
    PROW Row;
    SHORT i;
