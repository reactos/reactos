    ...
    try {
        switch (Message)
        {
            ...
            case WM_INPUTLANGCHANGEREQUEST:
                // Console IME never receive this message for this window is hidden and doesn't have focus.

                // However, Hot key of IME_CHOTKEY_IME_NONIME_TOGGLE/IME_THOTKEY_IME_NONIME_TOGGLE are send this message by ImmSimulateHotKey API.

                // If nothing processing by this message, then DefWindowProc calls ActivateKeyboardLayout on kernel side.
                // And, ActivateKeyboardLayout send WM_INPUTLANGCHANGE message to focus window on this message queue.
                // It window is console window procedure.
                // Console window procedure can do send CONIME_INPUTLANGCHANGE message to console IME window.
            ...
            case WM_COMMAND: // message: command from application menu
                // Message packing of wparam and lparam have changed for Win32, so use the GET_WM_COMMAND macro to unpack the commnad
            ...
            case WM_IME_SETCONTEXT:
                ...
                // When the application want to handle the IME at the timing of focus changing, the application should use WM_GETFOCUS or WM_KILLFOCUS.
            ...
            case WM_ENABLE:{
                ...
                if (!wParam) {
                    ...
                    if (FocusedConsole != NULL && FocusedConsole->hConsole != NULL) {
                    ...
                    for ( i = 1; i < NumberOfConsoleTable; i ++){
                        ...
                        {
                            if ((FocusedConsole->hConsole != NULL)&& (!FocusedConsole->Enable)&& (!IsWindowEnabled(FocusedConsole->hWndCon))){
            ...
            case WM_SETFOCUS:
                CreateCaret( hWnd,
                             NULL,
                             IsUnicodeFullWidth( ConvertLine[xPos] ) ? CaretWidth*2 : CaretWidth,
                             (UINT)cyMetrics );
            ...
            case WM_QUERYENDSESSION:
                ...
                {
                    int i;
                    i = MessageBox(hWnd,TEXT("Could you approve exit session?"), TEXT("Console IME"), MB_ICONSTOP | MB_YESNO);
    ...
    )
{
    ...
    do {
        for (; i < NumberOfConsoleTable; i++) {
            ...
            if ((FocusedConsole->hConsole != NULL) && (FocusedConsole->LateRemove)&& (FocusedConsole->Enable)) {
...
PCONSOLE_TABLE SearchConsole(HANDLE hConsole)
{
    ...
    // this will happen after restart conime when conime dead by bogus ime's AV or other problem
    ...
    for (i = 1; i < NumberOfConsoleTable; i++) {
        ...
        if (FocusedConsole != NULL)
        {
            if ((FocusedConsole->hConsole == hConsole)&& (!FocusedConsole->LateRemove)) {
                UnlockConsoleTable();
...
BOOL ConImeInputLangchangeRequest(HWND hWnd, HANDLE hConsole, HKL hkl, int Direction)
{
    ...
    if ( !IS_IME_KBDLAYOUT(hkl) || ( HKL_TO_LANGID(hkl) == RequiredLID)) {
        return TRUE;
    }
    ...
    for (i = 0; i < nLayouts; i++) {
        ...
        if ((( HandleToUlong(lphkl[StartPos]) & 0xf0000000) == 0x00000000) || (( HandleToUlong(lphkl[StartPos]) & 0x0000ffff) == RequiredLID)) {
            PostMessage( ConTbl->hWndCon, CM_CONIME_KL_ACTIVATE, HandleToUlong(lphkl[StartPos]), 0);
            LocalFree(lphkl);
            return FALSE;
        }
    }
    ...
    if (IS_IME_KBDLAYOUT(hkl)) {
        for (counter=0; counter < ConTbl->hklListMax ; counter++)
        {
            if (ConTbl->lphklList[counter].hkl == hkl)
            {
                ...
                ImeUIOpenStatusWindow(hWnd) ;
                if (ImeUIMakeInfoString(ConTbl, lpModeInfo))
                {
...
*/
{
    ...
    fNoTimeout = SendMessageTimeout(hWndConsoleIME, WM_COPYDATA, wParam, lParam, SMTO_ABORTIFHUNG | SMTO_NORMAL, CONIME_SENDMSG_TIMEOUT, &lResult);
    if (fNoTimeout)
    {
        return TRUE;
    }
