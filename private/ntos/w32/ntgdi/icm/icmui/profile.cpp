/**
  ...
  This implements the class we use to encapsulate everything we will ever care to know about a profile, including the classes we need to support
  ...
  12-13-96  A-RobKj Modified for faster operation (more lazy evaluation, and common DLL-wide database for installation checks)
                    Also moved CDeviceList derived classes to the header, so
                    I can use them other places, as well...
  01-07-97  KjelgaardR@acm.org  Fixed CProfileArray::Empty- wasn't setting Next object pointer to NULL after deleting said object (Fixed GP fault).
  01-08-97  KjelgaardR@acm.org  Modified printer enumeration routine to only enumerate color models (uses Global utility function).
...
void    CPrinterList::Enumerate() {
    ...
    EnumPrinters(PRINTER_ENUM_LOCAL, NULL, 4, NULL, 0, &dwcNeeded, &dwcReturned);
    ...
    while   (pBuff && !EnumPrinters(PRINTER_ENUM_LOCAL, NULL, 4, pBuff, dwcNeeded, &dwcNeeded, &dwcReturned) && GetLastError() == ERROR_MORE_DATA) {
        delete pBuff;
        pBuff = new BYTE[dwcNeeded];
    }
    ...
    while   (pBuff && !EnumPrinters(PRINTER_ENUM_CONNECTIONS, NULL, 4, pBuff, dwcNeeded, &dwcNeeded, &dwcReturned) && GetLastError() == ERROR_MORE_DATA) {
        delete pBuff;
        pBuff = new BYTE[dwcNeeded];
    }
    ...
    DWORD   dwcNeeded, dwcReturned;
    EnumPrinters(PRINTER_ENUM_LOCAL, NULL, 5, NULL, 0, &dwcNeeded, &dwcReturned);
    ...
    while   (pBuff && !EnumPrinters(PRINTER_ENUM_LOCAL, NULL, 5, pBuff, dwcNeeded, &dwcNeeded, &dwcReturned) && GetLastError() == ERROR_MORE_DATA) {
        delete pBuff;
        pBuff = new BYTE[dwcNeeded];
    }
...
void    CScannerList::Enumerate()
{
    ...
    for (i=0; i<dwItemsReturned; i++, pDevInfo++)
    {
        ...
        if (WideCharToMultiByte(CP_ACP, 0, pDevInfo->szDeviceInternalName, -1, szName, dwLen, NULL, &bUsedDefaultChar) && ! bUsedDefaultChar)
        {
        ...
        if (WideCharToMultiByte(CP_ACP, 0, pDevInfo->pszLocalName, -1, szName, dwLen, NULL, &bUsedDefaultChar) && ! bUsedDefaultChar)
        {
...
//  The following static functions fills the appropriate array using the profiles that match the search criteria goven.
...
void    CProfile::Enumerate(ENUMTYPE& et, CStringArray& csaList)
{
    ...
    if  (!dwBuffer) {
        _RPTF2(_CRT_WARN, "CProfile::Enumerate(String)- empty list- dwBuffer %d Error %d\n", dwBuffer, GetLastError());
        return;
    }
    ...
    if (pbBuffer) {
        if  (EnumColorProfiles(NULL, &et, pbBuffer, &dwBuffer, &dwcProfiles)) {
            for (PTSTR pstrMe = pstrBuffer; dwcProfiles--; pstrMe += 1 + lstrlen(pstrMe)) {
...
void    CProfile::Enumerate(ENUMTYPE& et, CProfileArray& cpaList)
{
    ...
    if  (!dwBuffer) {
        _RPTF2(_CRT_WARN, "CProfile::Enumerate(Profile)- empty list- dwBuffer %d Error %d\n", dwBuffer, GetLastError());
        return;
    }
...
//  This retrieves the color directory name.  Since it is a const, we whouldn't be calling it too often...
...
{
    //  Enumerate the existing profiles, so we can see if this one's been installed, already.
...
BOOL    CProfile::Install()
{
    if  (!InstallColorProfile(NULL, m_csName)) {
        CGlobals::ReportEx(InstFailedWithName,NULL,FALSE, MB_OK|MB_ICONEXCLAMATION,1,m_csName.NameAndExtension());
        return (FALSE);
    } else {
        m_bIsInstalled = TRUE;
...
void    CProfile::Uninstall(BOOL bDelete)
{
    ...
    if  (!UninstallColorProfile(NULL, m_csName.NameAndExtension(), bDelete)) {
        CGlobals::ReportEx(UninstFailedWithName,NULL,FALSE, MB_OK|MB_ICONEXCLAMATION,1,m_csName.NameAndExtension());
    } else {
...
void    CProfile::Associate(LPCTSTR lpstrDevice)
{
    ...
    if  (bInstalled) {
        if  (!AssociateColorProfileWithDevice(NULL, m_csName.NameAndExtension(),
            lpstrDevice)) {
            CGlobals::ReportEx(AssocFailedWithName,NULL,FALSE,1, MB_OK|MB_ICONEXCLAMATION,m_csName.NameAndExtension());
        } else
...
void    CProfile::Dissociate(LPCTSTR lpstrDevice) {
    if  (!DisassociateColorProfileFromDevice(NULL, m_csName.NameAndExtension(),
        lpstrDevice)) {
        CGlobals::ReportEx(DisassocFailedWithName,NULL,FALSE,1, MB_OK|MB_ICONEXCLAMATION,m_csName.NameAndExtension());
    } else
...
{
    return  u < ChunkSize() ? m_aStore[u] : m_pcpaNext -> operator[](u - ChunkSize());
}
