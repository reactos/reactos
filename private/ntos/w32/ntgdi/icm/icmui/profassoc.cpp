...
//  Class Constructor
CProfileAssociationPage::CProfileAssociationPage(HINSTANCE hiWhere, LPCTSTR lpstrTarget)
{
    m_pcpTarget = NULL;
...
BOOL CProfileAssociationPage::OnInit()
{
    ...
    if (m_pcpTarget) {
        SetDlgItemText(m_hwnd, ProfileFilename, m_pcpTarget->GetName());// Set profile filename

        // Update ICON to show installed/non-installed status.
        HICON hIcon = LoadIcon(CGlobals::Instance(), MAKEINTRESOURCE(m_pcpTarget->IsInstalled() ? DefaultIcon : UninstalledIcon));
        if (hIcon) {
        ...
        ConstructAssociations();// Build tentitive association list.
        UpdateDeviceListBox();// And then, fill up device listbox
        SetFocus(GetDlgItem(m_hwnd,AddButton));// And set focus on AddButton.
...
//  tentative associations- this starts out as the true list from the profile object.
...
VOID    CProfileAssociationPage::ConstructAssociations()
{
    ...
    {
        for (unsigned uDelete = 0; uDelete < m_cuaDelete.Count(); uDelete++) {
...
BOOL    CProfileAssociationPage::OnCommand(WORD wNotifyCode, WORD wid, HWND hwndControl)
{
    ...
    case RemoveButton:
        if (wNotifyCode == BN_CLICKED) {
            LRESULT i = SendDlgItemMessage(m_hwnd, DeviceListControl, LB_GETCURSEL, 0, 0);
            if (i == -1)
                return  TRUE;

            unsigned uItem = (unsigned) SendDlgItemMessage(m_hwnd, DeviceListControl, LB_GETITEMDATA, i, 0);
            Dissociate(uItem);
            UpdateDeviceListBox();
            EnableApplyButton();
            SettingChanged(TRUE);
    ...
    case DeviceListControl:
        if (wNotifyCode == LBN_SELCHANGE) {
            EnableWindow(GetDlgItem(m_hwnd, RemoveButton), -1 != SendDlgItemMessage(m_hwnd, DeviceListControl, LB_GETCURSEL, 0, 0));
            return  TRUE;
        }
        break;
    }
...
BOOL    CProfileAssociationPage::OnNotify(int idCtrl, LPNMHDR pnmh)
{
    switch  (pnmh -> code)
    {
        ...
        {
            if (SettingChanged()) {
                DisableApplyButton();//  Disable apply button, because current request are going to be "Applied".
                SettingChanged(FALSE);//  We are going to update changed.
...
BOOL    CProfileAssociationPage::OnHelp(LPHELPINFO pHelp)
{
    if (pHelp->iContextType == HELPINFO_WINDOW) {
        WinHelp((HWND) pHelp->hItemHandle, WINDOWS_HELP_FILE, HELP_WM_HELP, (ULONG_PTR) (LPSTR) ProfileAssociationUIHelpIds);
    }
...
BOOL CProfileAssociationPage::OnContextMenu(HWND hwnd)
{
    ...
    WinHelp(hwnd, WINDOWS_HELP_FILE, HELP_CONTEXTMENU, (ULONG_PTR) (LPSTR) ProfileAssociationUIHelpIds);
    return (TRUE);
}
...
CAddDeviceDialog::CAddDeviceDialog(CProfileAssociationPage *pcpas, HWND hwndParent) : CDialog(pcpas->Instance(), AddDeviceDialog, hwndParent)
{
    m_pcpasBoss = pcpas;
    m_bCanceled = TRUE;
    DoModal();
...
BOOL    CAddDeviceDialog::OnInit()
{
    ...
    for (unsigned uDevice = 0; uDevice < pcpThis->DeviceCount(); uDevice++)
    {
        ...
        LRESULT idItem = SendMessage(m_hwndList, LB_ADDSTRING, (WPARAM)0, (LPARAM) pcpThis->DisplayName(uDevice));
    ...
    EnableWindow(m_hwndButton, -1 != SendMessage(m_hwndList, LB_GETCURSEL, 0, 0));
    return  TRUE;
}
...
BOOL    CAddDeviceDialog::OnCommand(WORD wNotification, WORD wid, HWND hwndControl)
{
    ...
    case    BN_CLICKED:
        if (wid == AddDeviceButton) {
            ...
            unsigned uItem = (unsigned) SendMessage(m_hwndList, LB_GETITEMDATA, i, 0);
...
{
    if (pHelp->iContextType == HELPINFO_WINDOW) {
        WinHelp((HWND) pHelp->hItemHandle, WINDOWS_HELP_FILE, HELP_WM_HELP, (ULONG_PTR) (LPSTR) AddDeviceUIHelpIds);
    }
...
BOOL    CAddDeviceDialog::OnContextMenu(HWND hwnd)
{
    ...
    WinHelp(hwnd, WINDOWS_HELP_FILE, HELP_CONTEXTMENU, (ULONG_PTR) (LPSTR) AddDeviceUIHelpIds);
    return (TRUE);
}