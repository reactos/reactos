...
HRESULT CICMUserInterface::AddPrinterTab(LPFNADDPROPSHEETPAGE lpfnAddPage, LPARAM lParam)
{
    ...
    FORMATETC       fmte = { (CLIPFORMAT)RegisterClipboardFormat(_TEXT("Shell IDList Array")), (DVTARGETDEVICE FAR *) NULL, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
    STGMEDIUM       stgm;
    HRESULT         hres = m_lpdoTarget ? m_lpdoTarget -> GetData(&fmte, &stgm) : 0;
    ...
    //  Create the property sheet- it will get deleted if it is not in use when the shell tries to unload the extension
...
PSTR GetFilenameFromPath(PSTR pPathName)
{
    ...
    // Go to the end of the pathname, and start going backwards till you reach the beginning or a backslash
    pPathName += dwLen;
