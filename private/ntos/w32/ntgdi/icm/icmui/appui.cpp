/**
  ...
  This file implements the Application UI.
  This consists of two (ANSI/UNICODE) functions that allow an application to specify profiles to be used, and so forth.
...
BOOL    CColorMatchDialog::GoodParms(PCOLORMATCHSETUP pcms) {
    ...
    if  (pcms -> dwFlags & CMS_SETRENDERINTENT && pcms -> dwRenderIntent > INTENT_ABSOLUTE_COLORIMETRIC)
         Fail(ERROR_INVALID_PARAMETER);

    if  (pcms -> dwFlags & CMS_SETPROOFINTENT && pcms -> dwFlags & CMS_ENABLEPROOFING && pcms -> dwProofingIntent > INTENT_ABSOLUTE_COLORIMETRIC)
         Fail(ERROR_INVALID_PARAMETER);
...
void    CColorMatchDialog::CompleteInitialization() {
    ...
    if  (m_csaPrinter.Map(m_csPrinterProfile) == m_csaPrinter.Count())
    {
        _RPTF2(_CRT_WARN, "Printer Profile %s isn't associated with "
            "the monitor (%s)", (LPCTSTR) m_csPrinterProfile, (LPCTSTR) m_csPrinter);
    ...
    if  (m_csaMonitor.Map(m_csMonitorProfile) == m_csaMonitor.Count())
    {
        _RPTF2(_CRT_WARN, "Monitor Profile %s isn't associated with "
            "the monitor (%s)", (LPCTSTR) m_csMonitorProfile, (LPCTSTR) m_csMonitor);
    ...
    {
        _RPTF1(_CRT_WARN, "Target Profile %s isn't installed", (LPCTSTR) m_csTargetProfile);
        if (m_csaPrinter.Count())
        {
...
void CColorMatchDialog::FillStructure(COLORMATCHSETUPA *pcms) {
    if  (m_bEnableICM) {
        ...
        //  Don't fail if there is no monitor or printer profile.  Set them to an empty string and succeed, instead.
        //  We can always do this, because 0 counts and empty pointers have already been screened out.
        if  (m_csMonitorProfile.IsEmpty())
            pcms -> pMonitorProfile[0] = '\0';
        else {
            lstrcpynA(pcms -> pMonitorProfile, m_csMonitorProfile, pcms -> ccMonitorProfile);
        ...
        if  (m_bEnableProofing) {
            pcms -> dwFlags |= CMS_ENABLEPROOFING | CMS_SETTARGETPROFILE;
            pcms -> dwProofingIntent = m_dwProofIntent;
            lstrcpynA(pcms -> pTargetProfile, m_csTargetProfile, pcms -> ccTargetProfile);
...
void CColorMatchDialog::FillStructure(COLORMATCHSETUPW *pcms)
{
    if  (m_bEnableICM) {
        ...
        if  (m_csMonitorProfile.IsEmpty())
            ...
            lstrcpynW(pcms -> pMonitorProfile, m_csMonitorProfile, pcms -> ccMonitorProfile);
        ...
        if  (m_csPrinterProfile.IsEmpty() || !m_bColorPrinter)
            ...
            lstrcpynW(pcms -> pPrinterProfile, m_csPrinterProfile, pcms -> ccPrinterProfile);
        ...
        if  (m_bEnableProofing) {
            pcms -> dwFlags |= CMS_ENABLEPROOFING | CMS_SETTARGETPROFILE | CMS_SETPROOFINTENT;
            pcms -> dwProofingIntent = m_dwProofIntent;
            lstrcpynW(pcms -> pTargetProfile, m_csTargetProfile, pcms -> ccTargetProfile);
...
BOOL    CColorMatchDialog::OnInit()
{
    ...
    CheckDlgButton(m_hwnd, EnableBasic, m_bEnableProofing ? BST_UNCHECKED : BST_CHECKED);
    CheckDlgButton(m_hwnd, EnableProofing, m_bEnableProofing ? BST_CHECKED : BST_UNCHECKED);
    ...
    for (int i = INTENT_PERCEPTUAL; i <= INTENT_ABSOLUTE_COLORIMETRIC; i++) {
        ...
        SendMessage(m_hwndRenderIntent, CB_ADDSTRING, 0, (LPARAM) (LPCTSTR) csWork);
        SendMessage(m_hwndTargetIntent, CB_ADDSTRING, 0, (LPARAM) (LPCTSTR) csWork);
    }
    ...
    for (unsigned u = 0; u < m_csaTarget.Count(); u++) {
        if (m_bUseProfileDescription) {
            id = SendMessage(m_hwndTargetList, CB_ADDSTRING, 0, (LPARAM)((LPTSTR) m_csaTargetDesc[u]));
        } else {
            id = SendMessage(m_hwndTargetList, CB_ADDSTRING, 0, m_csaTarget[u].NameOnly());
        }
    ...
    for (u = 0; u < m_csaMonitor.Count(); u++) {
        if (m_bUseProfileDescription) {
            id = SendMessage(m_hwndMonitorList, CB_ADDSTRING, 0, (LPARAM)((LPTSTR) m_csaMonitorDesc[u]));
        } else {
            id = SendMessage(m_hwndMonitorList, CB_ADDSTRING, 0, m_csaMonitor[u].NameOnly());
        }
    ...
    for (u = 0; u < m_csaPrinter.Count(); u++) {
        if (m_bUseProfileDescription) {
            id = SendMessage(m_hwndPrinterList, CB_ADDSTRING, 0, (LPARAM)((LPTSTR) m_csaPrinterDesc[u]));
        } else {
            id = SendMessage(m_hwndPrinterList, CB_ADDSTRING, 0, m_csaPrinter[u].NameOnly());
        }
    ...
    if  (m_dpApplyCallback == NULL) {
        ...
        MoveWindow(GetDlgItem(m_hwnd, IDCANCEL), ptApply.x,ptApply.y, rcApply.right - rcApply.left, rcApply.bottom - rcApply.top,TRUE);
        MoveWindow(GetDlgItem(m_hwnd, IDOK), ptCancel.x,ptCancel.y, rcCancel.right - rcCancel.left, rcCancel.bottom - rcCancel.top,TRUE);
    }
            ...
            switch  (wid) {
                ...
                case    EnableProofing:
                    if (m_bEnableProofing == FALSE)
                    {
                        ...
                        //  Copy the original rendering intent to the proofing intent, and set original to Absolute Colorimetric
                    ...
                    return  TRUE;
                case ApplyButton: {
                    EnableApplyButton(FALSE);// Disable apply button
            ...
            switch  (wid) {
                case    RenderingIntent:
                    if (m_dwRenderIntent != idItem)
                    {
                        ...
                        // If proofing is disabled, proof intent follows render intent
                        if (! m_bEnableProofing)
                        {
...
BOOL CColorMatchDialog::OnHelp(LPHELPINFO pHelp)
{
    if (pHelp->iContextType == HELPINFO_WINDOW) {
        WinHelp((HWND) pHelp->hItemHandle, WINDOWS_HELP_FILE, HELP_WM_HELP, (ULONG_PTR) (LPSTR) ApplicationUIHelpIds);
    }
