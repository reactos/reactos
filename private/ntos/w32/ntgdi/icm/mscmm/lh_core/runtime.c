...
OSErr Convert565To8 (Ptr dataPtr5, Ptr dataPtr8, SINT32 startLine, SINT32 height, SINT32 width, SINT32 rowBytes5);
OSErr Convert8To565 (Ptr dataPtr8, Ptr dataPtr5, SINT32 startLine, SINT32 height, SINT32 width, SINT32 rowBytes5);
OSErr Convert565To8 (Ptr dataPtr5, Ptr dataPtr8, SINT32 startLine, SINT32 height, SINT32 width, SINT32 rowBytes5)
/*
...
OSErr Convert8To1 (Ptr dataPtr8, Ptr dataPtr1, SINT32 startLine, SINT32 height, SINT32 width, SINT32 rowBytes1)
/*
...
CMError LHCheckColorsPrivate (CMMModelPtr modelingData, CMColor* myColors, UINT32 count, UINT8 *result)
{
    ...
    if (modelingData->hasNamedColorProf == NamedColorProfileOnly || modelingData->hasNamedColorProf == NamedColorProfileAtEnd)
    {
        err = unimpErr;
        goto CleanupAndExit;
    }
...
CMError LHMatchBitMapPrivate (CMMModelPtr modelingData, const CMBitmap * inBitMap, CMBitmapCallBackUPP progressProc, void * refCon, CMBitmap * outBitMap)
{
    ...
    else if (modelingData->hasNamedColorProf != NamedColorProfileOnly)
    {
        ...
        while (info.processedLinesIn < bitMapIn.height)
        {
            ...
            {
                err = ConvertIndexToLabBitmap (modelingData, bitMapIn.image, (Ptr)info.tempInBuffer, info.processedLinesIn, inLineCount, bitMapIn.width,
                                               bitMapIn.rowBytes, calcParam.cmInputBytesPerLine, calcParam.cmInputPixelOffset * 8);
                if (err)
                    goto CleanupAndExit;
            }
            ...
            if (modelingData->hasNamedColorProf == NamedColorProfileAtEnd)
            {
                ...
                err = ConvertLabToIndexBitmap (modelingData, aBuffer, info.processedLinesIn, calcParam.cmLineCount, bitMapOut.width, bitMapOut.rowBytes);
                if (err)
                    goto CleanupAndExit;
            }
...
CMError CMConvNameToIndexProfile (CMProfileRef aProf, pcCMColorName Ptr2Name, unsigned long *Arr2Indices, unsigned long count)
{
    ...
    err = CMGetPartialProfileElement (aProf, icSigNamedColor2Tag, 0, &byteCount, 0);
    if (err)
        goto CleanupAndExit;
    ...
    if (err)
        goto CleanupAndExit;
    err = CMGetPartialProfileElement (aProf, icSigNamedColor2Tag, 0, &byteCount, (Ptr)aName);
    if (err)
    {
        goto CleanupAndExit;
    }
...
CMError CMConvIndexToName (icNamedColor2Type *Data, unsigned long *Arr2Indices, pCMColorName Ptr2Name, unsigned long count)
{
    ...
    for (i = 0; i < (UINT32)count; i++)
    {
        ...
        colorPtr = tagTbl + (index - 1)*elemSz;
        strcpy (Ptr2Name[i], colorPtr);/* put in all the info */
    }
...
CMError CMGetNamedProfileInfoProfile (CMProfileRef aProf, pCMNamedProfileInfo Info)
{
    ...
    tagTbl = (Ptr)aName + 8;/* = CMNamedColor2Type_header(84) + firstName(32) + PCSSize(3*2) */
