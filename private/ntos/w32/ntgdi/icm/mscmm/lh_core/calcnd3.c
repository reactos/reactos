...
#if FARBR_FILES
    WriteLuts("DoNDim", 1, adr_bereich_elut, aElutWordSize, ein_lut, NDIM_IN_DIM, NDIM_OUT_DIM, aXlutAdrSize, aXlutWordSize, (LH_UINT16 *) Xlut, adr_bereich_alut, aAlutWordSize, (LH_UINT16 *) aus_lut);
#endif
        ...
        while (LineCount)
        {
            ...
            while (i)
            {
...

                {
                    for (jj = 0; jj < NDIM_IN_DIM; ++jj)
                    {
                        if (*((LH_UINT16 *) inputData[jj]) ^ ein_Cache[jj])
                            break;
                    }
                }
