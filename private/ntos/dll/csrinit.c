...
Abstract:
    This module contains the initialization code for the Client-Server (CS) Client DLL.
...
Routine Description:
    ...
    This function gets control when the application links to this DLL are snapped.
Arguments:
    Context - Supplies an optional context buffer that will be restore after all DLL initialization has been completed.
              If this parameter is NULL then this is a dynamic snap of this module.
              Otherwise this is a static snap prior to the user process gaining control.
Return Value:
    Status value.
--*/
{
    ...
    CsrDllHandle = DllHandle;// Remember our DLL handle in a global variable.

    return( TRUE );
}


NTSTATUS CsrOneTimeInitialize( VOID )
{
    NTSTATUS Status;
...
/*++
Routine Description:
    This function is called by the client side DLL to connect with its server side DLL.

Arguments:

    ObjectDirectory - Points to a null terminate string that is the same
        as the value of the ObjectDirectory= argument passed to the CSRSS program.

    ServerDllIndex - Index of the server DLL that is being connected to.
        It should match one of the ServerDll= arguments passed to the CSRSS program.

    CallbackInformation - An optional pointer to a structure that contains a pointer to the client callback function dispatch table.
    ...
    ConnectionInformation - An optional pointer to uninterpreted data.
        ...
        protocol identification information to the server to allow the server to determine if it can satisify the client before accepting the connection.
        Upon return to the client, the ConnectionInformation data block contains any information passed back from the server DLL by its call to the CsrCompleteConnection call.
    ...
    CalledFromServer - On output, TRUE if the dll has been called from a server process.
...
--*/
{
    ...
    if (ARGUMENT_PRESENT( ConnectionInformation ) && (!ARGUMENT_PRESENT( ConnectionInformationLength ) || *ConnectionInformationLength == 0)) {
    ...
    if (ARGUMENT_PRESENT( CallbackInformation )) {
        ...
        CsrLoadedClientDll[ ServerDllIndex ]->ApiNumberBase = CallbackInformation->ApiNumberBase;
        CsrLoadedClientDll[ ServerDllIndex ]->MaxApiNumber = CallbackInformation->MaxApiNumber;
        CsrLoadedClientDll[ ServerDllIndex ]->CallbackDispatchTable = CallbackInformation->CallbackDispatchTable;
    }

    // if we are being called by a server process, skip lpc port initialization and call to server connect routine and just initialize heap.
    // the dll initialization routine will do any necessary initialization.
    // this stuff only needs to be done for the first connect.
    ...
    if ( ARGUMENT_PRESENT(ConnectionInformation) ) {
        ...
        if (ARGUMENT_PRESENT( ConnectionInformation )) {
            ...
            CsrAllocateMessagePointer( CaptureBuffer, a->ConnectionInformationLength, (PVOID *)&a->ConnectionInformation);
        ...
        Status = CsrClientCallServer( &m, CaptureBuffer, CSR_MAKE_API_NUMBER( CSRSRV_SERVERDLL_INDEX, CsrpClientConnect), sizeof( *a ));
        if (CaptureBuffer != NULL) {
            if (ARGUMENT_PRESENT( ConnectionInformation )) {
...
BOOLEAN xProtectHandle(HANDLE hObject)
{
    NTSTATUS Status;
...
NTSTATUS CsrpConnectToServer(IN PWSTR ObjectDirectory)
{
    ...
    // Create the port name string by combining the passed in object directory name with the port name.
    ...
    // Set up the security quality of service parameters to use over the port.
    // Use the most efficient (least overhead) - which is dynamic rather than static tracking.
    DynamicQos.ImpersonationLevel = SecurityImpersonation;
    DynamicQos.ContextTrackingMode = SECURITY_DYNAMIC_TRACKING;
    DynamicQos.EffectiveOnly = TRUE;

    // Create a section to contain the Port Memory.
    // Port Memory is private memory that is shared between the client and server processes.
    // This allows data that is too large to fit into an API request message to be passed to the server.
    ...
    // This includes a description of the Port Memory section so that the LPC connection logic can make the section visible to both the client and server processes.
    // Also pass information the server needs in the connection information structure.
    ...
    Status = NtSecureConnectPort( &CsrPortHandle, &CsrPortName, &DynamicQos, &ClientView, SystemSid, &ServerView, (PULONG)&MaxMessageLength, (PVOID)&ConnectionInformation, (PULONG)&ConnectionInformationLength);
    ...
    IF_CSR_DEBUG( LPC ) {
        DbgPrint( "CSRDLL: ClientView: Base=%p  RemoteBase=%p  Delta: %lX  Size=%lX\n", ClientView.ViewBase, ClientView.ViewRemoteBase, CsrPortMemoryRemoteDelta, (ULONG)ClientView.ViewSize);
    }

    // Create a sparse heap in the share memory section.  Initially commit just one page.
    ...
    CsrPortBaseTag = RtlCreateTagHeap( CsrPortHeap, 0,
                                       L"CSRPORT!",
                                       L"!CSRPORT\0"
                                       L"CAPTURE\0");
    return( STATUS_SUCCESS );
