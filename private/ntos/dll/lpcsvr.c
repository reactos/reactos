...
//  History:    2-06-98   RichardW   Created
{
    ...
    case LPC_CLIENT_DIED:
        ...
        break;
    default:
        // An unexpected message came through.  Normal LPC servers don't handle the other types of messages.  Drop it.
        break;
    }
...
VOID RtlpLpcServerCallback(PVOID Parameter, BOOLEAN TimedOut)
{
    ...
    while (1) {
        ...
        if (NT_SUCCESS(Status)) {            
            if (Status == STATUS_TIMEOUT) {// If we timed out, nobody was waiting for us:
                // Set up a general wait that will call back to this function when ready.
                RtlpLpcLockServer(Server);
                if ((Server->Flags & LPCSVR_SHUTDOWN_PENDING) == 0) {
                    Status = RtlRegisterWait(&Server->WaitHandle, Server->Port, RtlpLpcServerCallback, Server, 0xFFFFFFFF, WT_EXECUTEONLYONCE);
                }
                RtlpLpcUnlockServer(Server);
                break;
            }
                            ...
                            PVOID * LpcServer)
{
    ...
    *LpcServer = NULL;
    ...
    RtlInitializeCriticalSectionAndSpinCount(&Server->Lock, 1000);
...
NTSTATUS RtlShutdownLpcServer(PVOID LpcServer)
{
    ...
    // If there are receives still pending, we have to sync with those threads. 
    // To do so, we will tag the shutdown flag, and then wait the timeout amount.
    if (Server->ReceiveThreads != 0) {
        ...
        if (Status == STATUS_TIMEOUT) {
            // Hmm, the LPC server thread is hung somewhere, press on
        }
    ...
    // The server object is locked, and there are no receives pending. 
    // Or, the receives appear hung.  Skim through the context list, calling the server code. 
    // The disconnect message is NULL, indicating that this is a server initiated shutdown.
    ...
    return(STATUS_SUCCESS);// Clean up server objects
}
