...
/*++
Routine Description:
    This routine makes a connection between the caller and the DbgUi port in the Dbg subsystem.  
    In addition to returning a handle to a port object, a handle to a state change semaphore is returned.  
    This semaphore is used in DbgUiWaitStateChange APIs.
Arguments:
    None.
Return Value:
    TBD.
...
Routine Description:
    ...
    state change to occur in one of it's application threads. 
...
Return Value:
    ...
    STATUS_INVALID_CID - An invalid ClientId was specified for the
        AppClientId, or the specified Application was not waiting for a continue.
...
--*/
{
    ...
    st = NtRequestWaitReplyPort(DbgUiApiPort, (PPORT_MESSAGE) &ApiMsg, (PPORT_MESSAGE) &ApiMsg);
    if ( NT_SUCCESS(st) ) {
