...
NTSTATUS LdrpLoadWx86Dll(IN PCONTEXT Context);

NTSTATUS LdrpInitWx86(IN PWX86TIB Wx86Tib, IN PCONTEXT Context, IN BOOLEAN NewThread);
...
NTSTATUS Wx86IdentifyPlugin(IN PVOID ViewBase, IN PUNICODE_STRING FullDllName);
NTSTATUS Wx86ThunkPluginExport(IN PVOID DllBase, IN PCHAR ExportName, IN ULONG Ordinal, IN PVOID ExportAddress, OUT PVOID *ExportThunk);
...
#define NATIVE_BYTES_TO_PAGES(Size)  ((ULONG)((ULONG_PTR)(Size) >> NATIVE_PAGE_SHIFT) + (((ULONG)(Size) & (NATIVE_PAGE_SIZE - 1)) != 0))
#else
...
NTSTATUS NTAPI LdrpGetProcedureAddress(
    IN PVOID DllHandle,
...
#if defined(_X86_)
BOOLEAN LdrpCallInitRoutine(IN PDLL_INIT_ROUTINE InitRoutine, IN PVOID DllHandle, IN ULONG Reason, IN PCONTEXT Context OPTIONAL);
#else
