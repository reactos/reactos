...
#if defined(_ALPHA_)
VOID LdrpSetGp(IN ULONG_PTR GpValue);
#endif
...
/*++
Routine Description:
    This function is called as a User-Mode APC routine as the first user-mode code executed by a new thread.
    It's function is to initialize loader context, perform module initialization callouts...
Arguments:
...
*/
{
    ...
    if (!Peb->Ldr)
    {
        ...
        // Set GP register
        LdrpGpValue = (ULONG_PTR)RtlImageDirectoryEntryToData(Peb->ImageBaseAddress, TRUE, IMAGE_DIRECTORY_ENTRY_GLOBALPTR, &temp);
        if (Context != NULL) {
...
//#endif
        {
            ...
            //  Hack for NT4 SP4.
            //  So we don't overload another GlobalFlag bit that we have to be "compatible" with for NT5, look for another value named "DisableHeapLookaside".
            LdrQueryImageFileExecutionOptions( &UnicodeImageName, L"DisableHeapLookaside", REG_DWORD, &RtlpDisableHeapLookaside, sizeof( RtlpDisableHeapLookaside ), NULL);

            st = LdrQueryImageFileExecutionOptions( &UnicodeImageName, L"GlobalFlag", REG_DWORD, &Peb->NtGlobalFlag, sizeof( Peb->NtGlobalFlag ), NULL);
            if (!NT_SUCCESS( st )) {
        ...
        if ( Peb->NtGlobalFlag & FLG_HEAP_PAGE_ALLOCS ) {
            // We will enable page heap (RtlpDebugPageHeap) only after all other initializations for page heap are finished.

            // If page heap is enabled we need to disable any flag that might force creation of debug heaps for normal NT heaps.
            // This is due to a dependency between page heap and NT heap where the page heap within PageHeapCreate tries to create a normal NT heap to accomodate some of the allocations.
            // If we do not disable these flags we will get an infinite recursion between RtlpDebugPageHeapCreate and RtlCreateHeap.
            ...
            // Read page heap per process global flags.
            // If we fail to read a value, the default ones are kept.
            LdrQueryImageFileExecutionOptions(&UnicodeImageName, L"PageHeapFlags", REG_DWORD, &RtlpDphGlobalFlags, sizeof(RtlpDphGlobalFlags), NULL);

            // Read several page heap parameters.
            LdrQueryImageFileExecutionOptions(&UnicodeImageName, L"PageHeapSizeRangeStart", REG_DWORD, &RtlpDphSizeRangeStart, sizeof(RtlpDphSizeRangeStart), NULL);
            LdrQueryImageFileExecutionOptions(&UnicodeImageName, L"PageHeapSizeRangeEnd", REG_DWORD, &RtlpDphSizeRangeEnd, sizeof(RtlpDphSizeRangeEnd), NULL);
            LdrQueryImageFileExecutionOptions(&UnicodeImageName, L"PageHeapRandomProbability", REG_DWORD, &RtlpDphRandomProbability, sizeof(RtlpDphRandomProbability), NULL);

            // The two values below should be read as PVOIDs so that this works on 64-bit architetures.
            // However since this feature relies on good stack traces and since we can get reliable stack traces only on X86 architectures we will leave it as it is.
            LdrQueryImageFileExecutionOptions(&UnicodeImageName, L"PageHeapDllRangeStart", REG_DWORD, &RtlpDphDllRangeStart,  sizeof(RtlpDphDllRangeStart), NULL);
            LdrQueryImageFileExecutionOptions(&UnicodeImageName, L"PageHeapDllRangeEnd", REG_DWORD, &RtlpDphDllRangeEnd, sizeof(RtlpDphDllRangeEnd), NULL);
            LdrQueryImageFileExecutionOptions(&UnicodeImageName, L"PageHeapTargetDlls", REG_SZ, &RtlpDphTargetDlls, 512, NULL);

            //  Turn on BOOLEAN RtlpDebugPageHeap to indicate that new heaps should be created with debug page heap manager when possible.
    ...
    if (Teb->DeallocationStack == NULL) {
        st = NtQueryVirtualMemory(NtCurrentProcess(), Teb->NtTib.StackLimit, MemoryBasicInformation, (PVOID)&MemInfo, sizeof(MemInfo), NULL);
        if ( !NT_SUCCESS(st) ) {
    ...
    try {
        if (!Peb->Ldr) {
            ...
            try {
                InitStatus = LdrpInitializeProcess( Context, SystemArgument1, &UnicodeImageName);
...
NTSTATUS LdrpForkProcess( VOID )
{
    ...
    RtlpInitDeferedCriticalSection();// Initialize the critical section package.
...
*/
{
    ...
    {
        // The process is running in WOW64.  Sort out the optional header format and reformat the image if its page size is smaller than the native page size.
        ...
        if (NtHeader32->FileHeader.Machine == IMAGE_FILE_MACHINE_I386 &&
            ...
            !NT_SUCCESS(st = LdrpWx86FormatVirtualImage(NtHeader32, NtCurrentPeb()->ImageBaseAddress
    ...
    if (NtHeader->FileHeader.Machine == IMAGE_FILE_MACHINE_I386 &&
        ...
        !NT_SUCCESS(st = LdrpWx86FormatVirtualImage((PIMAGE_NT_HEADERS32)NtHeader, (PVOID)NtHeader->OptionalHeader.ImageBase)))
    {
        return st;
    }
#endif
    ...
    if (UseWOW64) {
        ...
        // The 32-bit loader in ntdll32 will look at the config data and do the right thing.
        ImageConfigData = NULL;
    } else
#endif
    {
        ImageConfigData = RtlImageDirectoryEntryToData( Peb->ImageBaseAddress, TRUE, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &i);
    }
    ...
    if (ImageConfigData != NULL && i == sizeof( *ImageConfigData )) {
        ...
        if (ImageConfigData->CriticalSectionDefaultTimeout != 0) {
            RtlpTimeout.QuadPart = Int32x32To64( (LONG)ImageConfigData->CriticalSectionDefaultTimeout, -10000);// Convert from milliseconds to NT time scale (100ns)
        }
...
//    }
    ...
    // This field is non-zero if the image file that was used to create this process contained a non-zero value in its image header.
    // If so, then set the affinity mask for the process using this value.
    // It could also be non-zero if the parent process created us suspended and poked our PEB with a non-zero value before resuming.
    if (Peb->ImageProcessAffinityMask) {
        st = NtSetInformationProcess( NtCurrentProcess(), ProcessAffinityMask, &Peb->ImageProcessAffinityMask, sizeof( Peb->ImageProcessAffinityMask ));
        if (NT_SUCCESS( st )) {
    ...
    RtlInitializeBitMap (&TlsBitMap, &Peb->TlsBitmapBits[0], TLS_MINIMUM_AVAILABLE);
    RtlInitializeBitMap (&TlsExpansionBitMap, &Peb->TlsExpansionBitmapBits[0], TLS_EXPANSION_SLOTS);
    InsertTailList(&RtlCriticalSectionList, &LoaderLock.DebugInfo->ProcessLocksList);
    LoaderLock.DebugInfo->CriticalSection = &LoaderLock;
    LoaderLockInitialized = TRUE;
    ...
    if (Peb->NtGlobalFlag & FLG_USER_STACK_TRACE_DB) {
        ...
        st = NtAllocateVirtualMemory( NtCurrentProcess(), (PVOID *)&BaseAddress, 0, &ReserveSize, MEM_RESERVE, PAGE_READWRITE);
        if ( NT_SUCCESS( st ) ) {
...
#if defined(_WIN64)
    if (UseWOW64) {
        // Create a heap using all defaults.  The 32-bit process heap will be created later by ntdll32 using the parameters from the exe.
    ...
    st = LdrQueryImageFileExecutionOptions( UnicodeImageName, L"DebugProcessHeapOnly", REG_DWORD, &DebugProcessHeapOnly, sizeof( DebugProcessHeapOnly ), NULL);
    if (NT_SUCCESS( st )) {
    ...
    if (ProcessParameters) {
        // If the process was created with process parameters, than extract:
        //      - Library Search Path
        ...
        if (CurDir.Buffer == NULL || CurDir.Buffer[ 0 ] == UNICODE_NULL || CurDir.Length == 0) {
            ...
            RtlMoveMemory( CurDir.Buffer, USER_SHARED_DATA->NtSystemRoot, 3 * sizeof( WCHAR ));
            CurDir.Buffer[ 3 ] = UNICODE_NULL;
        }
    }

    // Make sure the module data base is initialized before we take any exceptions.
    ...
    // Allocate the first data table entry for the image.
    // Since we have already mapped this one, we need to do the allocation by hand.
    // Its characteristics identify it as not a Dll, but it is linked into the table so that pc correlation searching doesn't have to be special cased.
    ...
    if (pp) {
        ...
        LdrDataTableEntry->BaseDllName.Buffer = RtlAllocateHeap(Peb->ProcessHeap, MAKE_TAG( LDR_TAG ), LdrDataTableEntry->BaseDllName.MaximumLength);
        RtlMoveMemory(LdrDataTableEntry->BaseDllName.Buffer, pp, LdrDataTableEntry->BaseDllName.MaximumLength);
    }  else {
    ...
    if (ShowSnaps) {
        ...
        DbgPrint( "     Image Path: %wZ (%wZ)\n", &LdrDataTableEntry->FullDllName, &LdrDataTableEntry->BaseDllName);
        DbgPrint( "     Current Directory: %wZ\n", &CurDir );
        DbgPrint( "     Search Path: %wZ\n", &LdrpDefaultPath );
    }

    // The process references the system DLL, so map this one next.
    // Since we have already mapped this one, we need to do the allocation by hand.
    // Since every application will be statically linked to the system Dll, we'll keep the LoadCount initialized to 0.
    ...
    LdrDataTableEntry->FullDllName.Buffer = (RtlAllocateStringRoutine)( SystemDllPath.Length + sizeof( UNICODE_NULL ) );
    ASSERT(LdrDataTableEntry->FullDllName.Buffer != NULL);
    RtlMoveMemory( LdrDataTableEntry->FullDllName.Buffer, SystemDllPath.Buffer, SystemDllPath.Length);
    ...
    LdrDataTableEntry->BaseDllName.Buffer = (PWSTR) ((PCHAR)(LdrDataTableEntry->FullDllName.Buffer) + LdrDataTableEntry->FullDllName.Length - LdrDataTableEntry->BaseDllName.Length);
    LdrpInsertMemoryTableEntry(LdrDataTableEntry);
    ...
    if (UseWOW64) {
        ...
        st = LdrGetProcedureAddress(Wow64Handle, &ProcName, 0, (PVOID *)&Wow64LdrpInitialize);
        if (!NT_SUCCESS(st)) {
            if (ShowSnaps) {
        ...
        st = LdrGetProcedureAddress(Wow64Handle, &ProcName, 0, (PVOID *)&Wow64PrepareForException);
        if (!NT_SUCCESS(st)) {
            if (ShowSnaps) {
        ...
        RtlLeaveCriticalSection(&LoaderLock);// Release the loaderlock now - this thread doesn't need it any more.
        (*Wow64LdrpInitialize)(Context);// Call wow64 to load and run 32-bit ntdll.dll.
        // This never returns.  It will destroy the process.
    }
#endif
    ...
    if ((PVOID)NtHeader->OptionalHeader.ImageBase != NtCurrentPeb()->ImageBaseAddress ) {
        // The executable is not at its original address.  It must be relocated now.
        PVOID ViewBase;
        NTSTATUS status;
        ...
        status = (NTSTATUS)LdrRelocateImage(ViewBase, "LDR", (ULONG)STATUS_SUCCESS, (ULONG)STATUS_CONFLICTING_ADDRESSES, (ULONG)STATUS_INVALID_IMAGE_FORMAT);
        if (!NT_SUCCESS(status)) {
            return status;
        }
...
#if defined (_ALPHA_)
    AlphaFindArchitectureFixups(NtHeader, NtCurrentPeb()->ImageBaseAddress, TRUE); // Find and apply Alpha architecture fixups for this image
#endif

    LdrpReferenceLoadedDll(LdrpImageEntry);

    // Lock the loaded DLLs to prevent dlls that back link to the exe to cause problems when they are unloaded.
    {
    ...
    LdrpLdrDatabaseIsSetup = TRUE;// All static DLLs are now pinned in place. No init routines have been run yet
    ...
    LdrQueryApplicationCompatibilityGoo(UnicodeImageName);// Get all application goo here (hacks, flags, etc.)
...
*/
{
    ...
    try {
        // check to see if the heap is locked.
        // If so, do not do ANY dll processing since it is very likely that a dll will need to do heap operations, but that the heap is not in good shape.
        // ExitProcess called in a very active app can leave threads terminated in the middle of the heap code or in other very bad places.
        // Checking the heap lock is a good indication that the process was very active when it called ExitProcess
        if ( RtlpHeapIsLocked( RtlProcessHeap() )) {
            ...
            // Go in reverse order initialization order and build the unload list
            Next = Peb->Ldr->InInitializationOrderModuleList.Blink;
            while ( Next != &Peb->Ldr->InInitializationOrderModuleList) {
                LdrDataTableEntry = (PLDR_DATA_TABLE_ENTRY) (CONTAINING_RECORD(Next,LDR_DATA_TABLE_ENTRY,InInitializationOrderLinks));
                Next = Next->Blink;

                // Walk through the entire list looking for entries. For each entry, that has an init routine, call it.
...
#if defined (WX86)
                            if (!Wx86ProcessInit || LdrpRunWx86DllEntryPoint(InitRoutine, NULL, LdrDataTableEntry->DllBase, DLL_PROCESS_DETACH, (PVOID)1) ==  STATUS_IMAGE_MACHINE_TYPE_MISMATCH)
#endif
                            {
            ...
            if ( LdrpImageHasTls ) {
                LdrpCallTlsInitializers(NtCurrentPeb()->ImageBaseAddress,DLL_PROCESS_DETACH);// If the image has tls than call its initializers
            }
        }
    } finally {
        RtlLeaveCriticalSection(&LoaderLock);
...
Routine Description:
    ...
    It's purpose is to call all of the processes DLLs to notify them that the thread is detaching.
*/
{
    PPEB Peb;
    PLDR_DATA_TABLE_ENTRY LdrDataTableEntry;
    ...
    try {
        ...
        while ( Next != &Peb->Ldr->InInitializationOrderModuleList)
        {
            ...
            // Walk through the entire list looking for entries.
            // For each entry, that has an init routine, call it.
            if (Peb->ImageBaseAddress != LdrDataTableEntry->DllBase) {
...
#if defined (WX86)
                            if (!Wx86ProcessInit || LdrpRunWx86DllEntryPoint(InitRoutine, NULL, LdrDataTableEntry->DllBase, DLL_THREAD_DETACH, NULL) ==  STATUS_IMAGE_MACHINE_TYPE_MISMATCH)
#endif
                            {
        ...
        if ( LdrpImageHasTls ) {
            LdrpCallTlsInitializers(NtCurrentPeb()->ImageBaseAddress,DLL_THREAD_DETACH);// If the image has tls than call its initializers
        }
        LdrpFreeTls();
    } finally {
        RtlLeaveCriticalSection(&LoaderLock);
...
*/
{
...
#if defined (WX86)
                        if (!Wx86ProcessInit || LdrpRunWx86DllEntryPoint(InitRoutine, NULL, LdrDataTableEntry->DllBase, DLL_THREAD_ATTACH, NULL) ==  STATUS_IMAGE_MACHINE_TYPE_MISMATCH)
#endif
                        {
                            if ( !LdrpShutdownInProgress ) {
    ...
    if ( LdrpImageHasTls && !LdrpShutdownInProgress ) {
        LdrpCallTlsInitializers(NtCurrentPeb()->ImageBaseAddress,DLL_THREAD_ATTACH);// If the image has tls than call its initializers
    }
}
    ...
    )
{
    ...
    RtlAppendUnicodeToString( &KeyPath, L"\\Registry\\Machine\\Software\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\");
    ...
    Status = NtQueryValueKey( KeyHandle, &UnicodeString, KeyValuePartialInformation, KeyValueInformation, sizeof( KeyValueBuffer ), &ResultLength);
    if (Status == STATUS_BUFFER_OVERFLOW) {
        // This function can be called before the process heap gets created therefore we need to protect against this case.
        // The majority of the code will not hit this code path because they read just strings containing hex numbers and for this the size of KeyValueBuffer is more than sufficient.
        if (RtlProcessHeap()) {
            AllocLength = sizeof( *KeyValueInformation ) + KeyValueInformation->DataLength;
            KeyValueInformation = (PKEY_VALUE_PARTIAL_INFORMATION) RtlAllocateHeap( RtlProcessHeap(), MAKE_TAG( TEMP_TAG ), AllocLength);
            if (KeyValueInformation == NULL) {
                Status = STATUS_NO_MEMORY;
            } else {
                bNeedToFree = TRUE;
                Status = NtQueryValueKey( KeyHandle, &UnicodeString, KeyValuePartialInformation, KeyValueInformation, AllocLength, &ResultLength);
            }
        } else {
            Status = STATUS_NO_MEMORY;
        }
...
NTSTATUS LdrpInitializeTls(VOID)
{
    ...
    // Walk through the loaded modules an look for TLS.
    // If we find TLS, lock in the module and add to the TLS chain.
    Head = &NtCurrentPeb()->Ldr->InLoadOrderModuleList;
    Next = Head->Flink;
    while ( Next != Head )
    {
        Entry = CONTAINING_RECORD(Next, LDR_DATA_TABLE_ENTRY, InLoadOrderLinks);
        Next = Next->Flink;
        TlsImage = (PIMAGE_TLS_DIRECTORY)RtlImageDirectoryEntryToData(Entry->DllBase, TRUE, IMAGE_DIRECTORY_ENTRY_TLS, &TlsSize);
        ...
        if ( TlsImage ) {
            ...
            Entry->LoadCount = (USHORT)0xffff;// Since this DLL has TLS, lock it in
            Entry->TlsIndex = (USHORT)0xffff;// Mark this as having thread local storage
    ...
    // We now have walked through all static DLLs and know all DLLs that reference thread local storage.
    // Now we just have to allocate the thread local storage for the current thread and for all subsequent threads
...
NTSTATUS LdrpAllocateTls(VOID)
{
    ...
    if ( LdrpNumberOfTlsEntries )
    {
        ...
        while ( Next != Head )
        {
            ...
            Next = Next->Flink;
            TlsVector[TlsEntry->Tls.Characteristics] = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG( TLS_TAG ), TlsEntry->Tls.EndAddressOfRawData - TlsEntry->Tls.StartAddressOfRawData);
            ...
            if (ShowSnaps) {
                DbgPrint("LDR: TlsVector %x Index %d = %x copied from %x to %x\n",
                    TlsVector, TlsEntry->Tls.Characteristics, &TlsVector[TlsEntry->Tls.Characteristics], TlsEntry->Tls.StartAddressOfRawData, TlsVector[TlsEntry->Tls.Characteristics]);
            }

            RtlCopyMemory(TlsVector[TlsEntry->Tls.Characteristics], (PVOID)TlsEntry->Tls.StartAddressOfRawData, TlsEntry->Tls.EndAddressOfRawData - TlsEntry->Tls.StartAddressOfRawData);
...
#if defined (WX86)
                    if (!Wx86ProcessInit || LdrpRunWx86DllEntryPoint((PDLL_INIT_ROUTINE)InitRoutine, NULL, DllBase, Reason, NULL) ==  STATUS_IMAGE_MACHINE_TYPE_MISMATCH)
#endif
                    {
...
*/
{
    ...
    st = LdrQueryImageFileExecutionOptions( UnicodeImageName, L"ApplicationGoo", REG_BINARY, &LocalAppCompatGoo, sizeof(APP_COMPAT_GOO), &ResultSize);
    ...
    if (st == STATUS_BUFFER_OVERFLOW) {
        ...
        st = LdrQueryImageFileExecutionOptions( UnicodeImageName, L"ApplicationGoo", REG_BINARY, AppCompatGoo, ResultSize, &ResultSize);
        if (!NT_SUCCESS( st )) {
            RtlFreeHeap(Peb->ProcessHeap, 0, AppCompatGoo);
            return;
        }

        // Got a hit on this key, however we don't know fer sure that its an exact match.
        // There could be multiple App Compat entries within this Goo.
        // So we get the version resource information out of the Image hdr (if avail) and later we compare it against all of the entries found within the Goo hoping for a match.
...
//        NtQueryDefaultUILanguage(&LangId);
        ...
        // Now that we either have (or have not) the version resource info, bounce down each app compat entry looking for a match.
        // If there wasn't any version resource info in the image hdr, its going to be an automatic match to an entry that also doesn't have anything for its version resource info.
        // Obviously there can be only one of these "empty" entries within the Goo (as the first one will always be matched first.
        st = STATUS_SUCCESS;
        ...
        while (TotalGooLength) {
            try {
                ...
                // The ResourceInfo (if avail) is directly behind the AppCompatEntry
                ...
                // In this case, we don't have any version resource info in the image header, so set OutputCompareLength to zero.
                // If InputCompareLength was set to zero (above) due to the AppCompatEntry also having no version resource info, then the test will succeed (below) and we've found our match.
            ...
            // We're a match!!!  Now we have to create the final "Post" app compat structure that will be used by everyone to follow.
            // This guy hangs off the Peb and it doesn't have the resource info still lying around in there.
            ...
            // Copy what was beyond the resource info to near the top starting at the Application compat flags.
            RtlMoveMemory(&AppCompatInfo->CompatibilityFlags, (PUCHAR) ResourceInfo + AppCompatEntry->dwResourceInfoSize, AppCompatInfo->dwTotalSize - FIELD_OFFSET(APP_COMPAT_INFO, CompatibilityFlags));

            // Now that we've created the "Post" app compat info struct to be used by everyone, we need to check if version lying for this app is requested.  If so, we need to stuff the Peb right now.
            if (AppCompatInfo->CompatibilityFlags.QuadPart & KACF_VERSIONLIE) {
                ...
                // The variable length information itself comes at the end of the normal struct and could be of any aribitrary length
                AppVariableInfo++;
                ...
                Peb->CSDVersion.Buffer = RtlAllocateHeap(Peb->ProcessHeap, HEAP_ZERO_MEMORY, Peb->CSDVersion.MaximumLength);
    ...
    if ( FALSE == fNewCSDVersionBuffer )
    {
        ...
        // One of the possible error codes is BUFFER_TOO_SMALL - this indicates a string that's wacko - they should not be larger than the size we define/expect In this case, we'll ignore that string
        if ( STATUS_SUCCESS == st )
        {
            ...
            if ( len )
            {
                ...
                // We do a movemem here rather than a string copy because current comments in RtlQueryEnvironmentVariable() indicate that in an edge case, we might not have a trailing NULL - berniem 7/7/99
...
Routine Description:
    ...
    The caller specifies what type its looking for and this function chews thru all the variable length structs to find it.
    If it does it returns the pointer and TRUE, else FALSE.
Arguments:
...
*/
{
    ...
    if (Peb->AppCompatInfo) {
        ...
        // The first variable structure (if there is one) will start immediately after the fixed stuff
        ...
        while (CurOffset < TotalSize)
        {
            ...
            CurOffset += (ULONG)(pCurrentEntry->dwVariableInfoSize);// Let's go look at the next blob
        }
    }
