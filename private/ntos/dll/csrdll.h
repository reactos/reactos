...
// reserved for the Client heap.  This heap is used to store all data structures maintained by the Client DLL.
#define CSR_HEAP_MEMORY_SIZE (64*1024)


// CSR_PORT_MEMORY_SIZE defines how much address space should be
// reserved for passing data to the Server.  The memory is visible to both the client and server processes.
...
// This points to the server routine that dispatches APIs, if the dll is being called by a server process.
NTSTATUS (*CsrServerApiRoutine)(PCSR_API_MSG,PCSR_API_MSG);

// The CsrNtSysInfo global variable contains NT specific constants of
// interest, such as page size, allocation granularity, etc.  It is filled in once during process initialization.
...
// The CsrDebugFlag is non-zero if the Client Application was invoked with the Debug option.
ULONG CsrDebugFlag;

// The CsrHeap global variable describes a single heap used by the Client
// DLL for process wide storage management.  Process private data maintained by the Client DLL is allocated out of this heap.
PVOID CsrHeap;


// The connection to the Server is described by the CsrPortHandle global
// variable.  The connection is established when the CsrConnectToServer function is called.
...
// is visible to both the Windows Client process and the Server process.
PVOID CsrPortHeap;
...
HANDLE CsrDllHandle;// The CsrDllHandle global variable contains the DLL handle for the WINDLL client stubs executable.
HANDLE CsrObjectDirectory;// The CsrObjectDirecotory global variable contains the handle to the object directory that is the name of the server.

// The CsrCallbackInfo structure has the information passed to
// CsrClientConnectToServer if it was present, otherwise it is initialized to all zeros.
