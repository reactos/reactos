...
#define IsNativeMachineType(m)   ((m) == IMAGE_FILE_MACHINE_ALPHA)
#else
#define IsNativeMachineType(m)   ((m) >= USER_SHARED_DATA->ImageNumberLow && (m) <= USER_SHARED_DATA->ImageNumberHigh)
#endif
...
BOOLEAN DllHasExports(PVOID DllBase)
{
    ULONG ExportSize;
    PIMAGE_EXPORT_DIRECTORY ExportDir;

    ExportDir = RtlImageDirectoryEntryToData(DllBase, TRUE, IMAGE_DIRECTORY_ENTRY_EXPORT, &ExportSize);

    return ExportDir && ExportSize && (ExportDir->NumberOfFunctions || ExportDir->NumberOfNames);
}
...
--*/
{
    ...
    // if we are not checking Machine Type, return based on FullName matching.
    ...
    if ((LdrEntry->Flags & LDRP_WX86_IGNORE_MACHINETYPE) || (LdrEntry->Flags & LDRP_WX86_PLUGIN)) {
    ...
    if (ImporterX86) {
        ...
        // dlls. All risc Wx86 Thunk dlls are marked as Wx86 Thunk dlls in the ntheader.
        if (FullNameMatches) {
            return (NtHeaders->OptionalHeader.DllCharacteristics & IMAGE_DLLCHARACTERISTICS_X86_THUNK) != 0;
        }

        // The full name doesn't match, we can still allow matches
        // for loads which were redirected from system32 to wx86 system dir (See LdrpWx86MapDll).
        else {
...
--*/
{
    ...
    // formulate the name for each path component, and see if it exists.
    ...
    do {
        ...
        //  if we got a path component, append the basename and return if it exists.
        if (FileName->Length) {
            if (*(pwch - 1) != L'\\') {
                *pwch = L'\\';
                FileName->Length += sizeof(WCHAR);
...
--*/
{
    ...
    // a path component may contain relative references, it may exceed MAX_PATH.
    ...
    FreeUnicode.Buffer = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TEMP_TAG), FreeUnicode.MaximumLength);
    ...
    *pSection = NULL;
    ...
    RetryWx86SystemDir:
        ...
        if (ShowSnaps) {
            DbgPrint("LDR: Loading (%s) %wZ\n", StaticLink ? "STATIC" : "DYNAMIC", &FullNameUnicode);
        }

        st = LdrpCreateDllSection(&NameUnicode, NULL, pLocalDllName, DllCharacteristics, pSection);
        ...
        st = NtQuerySection(*pSection, SectionImageInformation, &SectionInfo, sizeof(SectionInfo), NULL);
        if (!NT_SUCCESS(st)) {
            break;
        }
        ...
        if (IsNativeMachineType(SectionInfo.Machine)) {
            if (!Wx86KnownDll || (SectionInfo.DllCharacteristics & IMAGE_DLLCHARACTERISTICS_X86_THUNK)) {
                break;
            }
        ...
        if (!MismatchEncountered) {
            ...
            FullName.Buffer = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TEMP_TAG), FullName.MaximumLength);
            if (!FullName.Buffer) {
                st = STATUS_NO_MEMORY;
                break;
            }
...
// Cleanup the temporary allocated buffers.
    ...
    if (NT_SUCCESS(st)) {
        ...
        *pViewSize = 0;
        ...
        st = NtMapViewOfSection(*pSection, NtCurrentProcess(), &ViewBase, 0L, 0L, NULL, pViewSize, ViewShare, 0L, PAGE_READWRITE);
    ...
    if (NT_SUCCESS(st)) {
        ...
        if (st == STATUS_IMAGE_MACHINE_TYPE_MISMATCH) {
            st = NtHeaders->OptionalHeader.ImageBase == (ULONG_PTR)ViewBase ? STATUS_SUCCESS : STATUS_IMAGE_NOT_AT_BASE;
        }
        ...
        *pEntry = Entry = LdrpAllocateDataTableEntry(ViewBase);
        ...
        RtlCopyMemory(Unicode->Buffer, FullNameUnicode.Buffer, Unicode->MaximumLength);
...
Arguments:
    ...
    DllName      - Name of Dll
    Wx86KnownDll - if true, Importer is x86.
    FullDllName  - buffer to receive full path name, assumes STATIC_UNICODE_BUFFER_LENGTH
Return Value:
...
--*/
{
    ...
    do {
        ...
        if (Length && Length < FullDllName->MaximumLength) {
            ...
            PathPart = *FullDllName;
            PathPart.Length = (USHORT)((ULONG_PTR)BaseDllName.Buffer - (ULONG_PTR)FullDllName->Buffer - sizeof(WCHAR));
...
VOID LdrpWx86DllProcessDetach(IN PLDR_DATA_TABLE_ENTRY LdrDataTableEntry)
/*++
Routine Description:
...
--*/
{
    ...
    if (NtHeader && NtHeader->FileHeader.Machine == IMAGE_FILE_MACHINE_I386) {
        InitStatus = (Wx86DllEntryPoint)(InitRoutine, DllBase, Reason, Context);
        if (pInitStatus) {
...
NTSTATUS LoaderWx86Unload(VOID)
/*++
Routine Description:
...
NTSTATUS LdrpLoadWx86Dll(PCONTEXT Context)
/*++
Routine Description:
   Loads in the i386 emulator (wx86.dll) and performs process initialization for wx86 specific ldr code.
Arguments:
...
--*/
{
    ...
    Wx86SystemDir.Buffer = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(LDR_TAG), Wx86SystemDir.MaximumLength);
    if (!Wx86SystemDir.Buffer) {
        st = STATUS_NO_MEMORY;
        goto LWx86DllError;
    }
    ...
    // Load Wx86.dll, wintdll.dll. This must be done before the app binary is snapped to ensure wx86.dll is ready for emulation.
    if (!DllHandle) {
    ...
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86KnownDllName);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }

    RtlInitAnsiString(&ProcName, "Wx86KnownNativeDll");
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86KnownNativeDll);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }

    RtlInitAnsiString(&ProcName, "Wx86KnownRedistDll");
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86KnownRedistDll);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }

    RtlInitAnsiString(&ProcName, "RunWx86DllEntryPoint");
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86DllEntryPoint);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }

    RtlInitAnsiString(&ProcName, "Wx86ThreadStartRoutine");
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86ThreadStartRoutine);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }

    RtlInitAnsiString(&ProcName, "Wx86ProcessStartRoutine");
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86ProcessStartRoutine);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }

    RtlInitAnsiString(&ProcName, "Wx86DllMapNotify");
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86DllMapNotify);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }

    RtlInitAnsiString(&ProcName, "Wx86ProcessInit");
    st = LdrGetProcedureAddress(DllHandle, &ProcName, 0, (PVOID *)&Wx86ProcessInit);
    if (!NT_SUCCESS(st)) {
        goto LWx86DllError;
    }
...
LWx86DllError:
    if (!NT_SUCCESS(st)) {
        LoaderWx86Unload();// If the load failed make sure we clean-up.
    }
...
NTSTATUS LdrpInitWx86(PWX86TIB Wx86Tib, PCONTEXT Context, BOOLEAN NewThread)
/*++
Routine Description:
...
NTSTATUS Wx86SetRelocatedSharedProtection(IN PVOID Base, IN BOOLEAN Reset)
/*++
Routine Description:
    This function loops thru the images sections/objects, setting
    all relocated shared sections/objects marked r/o to r/w. It also resets the original section/object protections.
Arguments:
    Base - Base of image.
...
NTSTATUS FixupBlockList(IN PUCHAR ImageBase);
VOID FixupSectionHeader(IN PUCHAR ImageBase);


NTSTATUS LdrpWx86FormatVirtualImage(IN PIMAGE_NT_HEADERS32 NtHeaders, IN PVOID DllBase)
{
    ...
    // Second pass is from the end of the image and work backwards since src and dst overlap
    Section = --LastSection;
...
ULONG LdrpWx86RelocatedFixupDiff(IN PUCHAR ImageBase, IN ULONG  Offset)
{
    PIMAGE_SECTION_HEADER SectionHeader;
    ULONG i;
    ULONG NumberOfSharedDataPages;
...
BOOLEAN LdrpWx86DllHasRelocatedSharedSection(IN PUCHAR ImageBase)
{
    PIMAGE_SECTION_HEADER SectionHeader;
    ULONG i;
    PIMAGE_NT_HEADERS32 NtHeaders = (PIMAGE_NT_HEADERS32)RtlImageNtHeader(ImageBase);
    ...
    IN ULONG_PTR SectionEndVA)
{
    ...
    while (SizeOfBlock--)
    {
        ...
        case IMAGE_REL_BASED_MIPS_JMPADDR:
            ...
            break;
        case IMAGE_REL_BASED_ABSOLUTE:
            break;// Absolute - no fixup required.
        case IMAGE_REL_BASED_SECTION:
            break;// Section Relative reloc.  Ignore for now.
        case IMAGE_REL_BASED_REL32:
            break;// Relative intrasection. Ignore for now.
        case IMAGE_REL_BASED_HIGH3ADJ:
            ...
            break;
        default:
            return (PIMAGE_BASE_RELOCATION)NULL;// Illegal - illegal relocation type.
        }
...
NTSTATUS Wx86IdentifyPlugin(IN PVOID DllBase, IN PUNICODE_STRING FullDllName)
/*++
Routine Description:
    Determine which (if any) plugin provider dlls support this plugin.

    All registered plugin provider dlls are loaded sequentially and given an opportunity to examine the the plugin dll.
    Provider dlls that support the plugin are stored in a WX86PLUGIN object and linked into Wx86PluginList.

    Note: This routine may be invoked before Wx86 has been loaded so the global values initialized by LdrpLoadWx86 cannot be used.
...
--*/
{
    ...
    st = NtQueryKey(hProviderKey, KeyFullInformation, KeyFullInfo, sizeof(DataBuffer), &Length);
    if (NT_ERROR(st)) {
        goto Wx86IdentifyDone;
    }
    ...
    do
    {
        ...
        if (st == STATUS_SUCCESS) {
            ...
            *(PWCHAR)(&ValPartInfo->Data[ValPartInfo->DataLength]) = UNICODE_NULL;
            ...
            // Get the name of an export this provider requires (if any).
            // If the plugin dll doesn't have this export then it's not possible for this provider to support it.
            // This is a quick check which excludes most false tests for plug providers.
            ...
            if (NT_SUCCESS(st)) {
                ...
                if (NT_SUCCESS(st) && IdentifyPlugin) {
                    try {
                        CanThunk = IdentifyPlugin(DllBase, FullDllName->Buffer, MachineType == IMAGE_FILE_MACHINE_I386);
    ...
    // Return success if we found at least on plugin provider.
    // Allocate a WX86PLUGIN list entry to keep track of which provders were loaded for this plugin
...
NTSTATUS Wx86ThunkPluginExport(
    IN PVOID DllBase,
    IN PCHAR ExportName,
    IN ULONG Ordinal,
...
/*++
Routine Description:
    This procedure is called during GetProcAddress() processing when the request if for a cross-architecure procedure address in an
    image which was loaded as a plugin dll (LDRP_WX86_PLUGIN flag).

    The providers associated with this plugin dll are called in order to provide a thunk for the specified plugin export.
    The thunk attempt stops when a provider successfully thunks the export.
...
--*/
{
    ...
    for (Index = 0; Index < Wx86Plugin->Count; Index++) {
        st = LdrGetProcedureAddress(Wx86Plugin->Provider[Index], &ProcName, 0, (PVOID *)&ThunkExport);
        if (NT_SUCCESS(st) && ThunkExport != NULL) {
            try {
                Thunked = ThunkExport(DllBase, ExportName, Ordinal, ExportAddress, ExportThunk, MachineType == IMAGE_FILE_MACHINE_I386);
            } except(EXCEPTION_EXECUTE_HANDLER) {
                Thunked = FALSE;
            }
            ...
            if (Thunked) {
                if (ShowSnaps) {
                    DbgPrint("LDRWx86: thunk export for %08X Ord=%04X Addr=%08X Name=%s Thunk=%08X\n",
                             DllBase, Ordinal, ExportAddress, ExportName ? ExportName : "<noname>", *ExportThunk);
                    break;
                }
            }
        }
...
BOOLEAN Wx86UnloadProviders(IN PVOID DllBase)
/*++
Routine Description:
