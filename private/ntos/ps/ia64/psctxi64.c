...
--*/
{
    ...
    // Get the address of the context frame and compute the address of the system entry trap frame.
    ContextInfo = CONTAINING_RECORD(Apc, GETSETCONTEXT, Apc);

    TrFrame1 = (PKTRAP_FRAME)(((ULONG_PTR)PsGetCurrentThread()->Tcb.InitialStack - KTHREAD_STATE_SAVEAREA_LENGTH - KTRAP_FRAME_LENGTH));


    // Capture the current thread context and set the initial control PC value.
    RtlCaptureContext(&ContextRecord);
    ControlPc = ContextRecord.BrRp;

    // Initialize context pointers for the nonvolatile integer and floating registers.
    ...
    do {
        ...
        // unwind to the caller of the current routine to obtain the address where control left the caller.
        if (FunctionEntry != NULL) {
            ControlPc = RtlVirtualUnwind(ImageBase, ControlPc, FunctionEntry, &ContextRecord, &InFunction, &EstablisherFrame, &ContextPointers);
        } else {
    ...
    if (*SystemArgument1 != 0) {
        PspSetContext(TrFrame1, &ContextPointers, &ContextInfo->Context, ContextInfo->Mode);// Set Context from proper Context mode
    } else {
        KiFlushUserRseState(TrFrame1);
        PspGetContext(TrFrame1, &ContextPointers, &ContextInfo->Context);// Get Context from proper Context mode
    }
