...
Abstract:
    This module implements process and thread object termination and deletion.
...
/*++
Routine Description:
    This routine implements the thread reaper. The reaper is responsible for processing terminated threads. This includes:
...
--*/
{
    ...
    // Lock the dispatcher data and continually remove entries from the reaper list until no more entries exist.

    // N.B. The dispatcher database lock is used to synchronize access to the reaper list. This is done to avoid a race condition with the thread termination code.
    KiLockDispatcherDatabase(&OldIrql);
    NextEntry = PsReaperListHead.Flink;
    while (NextEntry != &PsReaperListHead)
    {
        // Remove the next thread from the reaper list, get the address of the executive thread object, acquire the context swap lock, and then release the both the context swap dispatcher database locks.

        // N.B. The context swap lock is acquired and immediately released.
        //    This is necessary to ensure that the respective thread has completely passed through the context switch code before it is terminated.
        ...
        // Get the address of the executive process object, release the process' CreateDelete lock and then dereference the process object.
        ...
        // Lock the dispatcher database and get the address of the next entry in the list.
        KiLockDispatcherDatabase(&OldIrql);
        NextEntry = PsReaperListHead.Flink;
    }
...
--*/
{
    ...
    if ( Thread == PsGetCurrentThread() ) {
        ...
        KeInitializeApc(ExitApc, &Thread->Tcb, OriginalApcEnvironment, PsExitSpecialApc, NULL, PspExitNormalApc, KernelMode, ULongToPtr(ExitStatus) // Sundown: ExitStatus is zero-extended.
            );
...
--*/
{
    ...
    st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_TERMINATE, PsProcessType, KeGetPreviousMode(), (PVOID *)&Process, NULL);
    if ( !NT_SUCCESS(st) ) {
        return(st);
    }

    // If the exit status is DBG_TERMINATE_PROCESS, then fail the terminate if the process is being debugged.
    // This is for shutdown so that we can kill debuggers and not debuggees reliably
    ...
    // the following allows NtTerminateProcess to fail properly if called while the exiting process is blocked holding the createdeletelock.
    // This can happen during debugger/server lpc transactions that occur in pspexitthread
    ...
    if ( Process == PsGetCurrentProcess() && ProcessHandleSpecified ) {
        ...
        // early set of process exit time will eliminate the deadlocks that occur when csrss tries to create a remote thread in a
        // process that is in the process of exiting, and that has an unhandled exit call outstanding to its debugger
    ...
    // that occur when csrss tries to create a remote thread in a process that is in the process of exiting, and that has an unhandled exit call outstanding to its debugger
...
--*/
{
    ...
    if ( !ARGUMENT_PRESENT(ThreadHandle) ) {
        if ( (PsGetCurrentProcess()->ThreadListHead.Flink == PsGetCurrentProcess()->ThreadListHead.Blink) &&
             (PsGetCurrentProcess()->ThreadListHead.Flink == &PsGetCurrentThread()->ThreadListEntry))
        {
    ...
    st = ObReferenceObjectByHandle(ThreadHandle, THREAD_TERMINATE, PsThreadType, KeGetPreviousMode(), (PVOID *)&Thread, NULL);
    if ( !NT_SUCCESS(st) ) {
        return(st);
    }
...
/*++
Routine Description:
    This function causes the current thread, which must be a system thread, to terminate.
Arguments:
    ExitStatus - Supplies the exit status associated with the thread.
--*/
{
...
VOID PspExitNormalApc(IN PVOID NormalContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2)
{
    ...
    if ( IS_SYSTEM_THREAD(Thread) ) {
        ...
        KeInitializeApc(ExitApc, &Thread->Tcb, OriginalApcEnvironment, PsExitSpecialApc, NULL, PspExitNormalApc, UserMode, NormalContext);
...
--*/
{
    ...
    PoRundownThread(Thread);// Clear any execution state associated with the thread

    // Account for a miss where we try to freeze a thread and bump the freeze count, but in the freeze APC we bail from the APC due to the pending exit APC.
    // This results in an active thread running with a biased freeze count and no pending APC.
    // When this sort of thread tries to grab the process lock, it ends of spinning since it things a freeze is pending and an APC should occur.
    ...
    if (PspCreateThreadNotifyRoutineCount != 0) {
        ...
        for (i = 0; i < PSP_MAX_CREATE_THREAD_NOTIFY; i++) {
            if (PspCreateThreadNotifyRoutine[i] != NULL) {
                (*PspCreateThreadNotifyRoutine[i])(Process->UniqueProcessId, Thread->Cid.UniqueThread, FALSE);
            }
        }
    }
    ...
    if (!IsListEmpty(&Thread->TerminationPortList)) {
        ...
        // If there are no ports to send notifications to, but there is an exception port, then we have to send a client died message through the exception port.
        // This will allow a server a chance to get notification if an app/thread dies before it even starts

        // We only send the exception if the thread creation really worked.
        // DeadThread is set when an NtCreateThread returns an error, but the thread will actually execute this path.
        ...
        if ((ExitStatus == STATUS_THREAD_IS_TERMINATING && Thread->DeadThread) || !Thread->DeadThread) {
            CdMsg.PortMsg.u1.s1.DataLength = sizeof(LARGE_INTEGER);
            CdMsg.PortMsg.u1.s1.TotalLength = sizeof(LPC_CLIENT_DIED_MSG);
            CdMsg.PortMsg.u2.s2.Type = LPC_CLIENT_DIED;
            CdMsg.PortMsg.u2.s2.DataInfoOffset = 0;
    ...
    // Delete the thread's TEB.  If the address of the TEB is in user space, then this is a real user mode TEB.
    // If the address is in system space, then this is a special system thread TEB allocated from paged or nonpaged pool.
    if (Thread->Tcb.Teb) {
        if (IS_SYSTEM_THREAD(Thread)) {
            ExFreePool(Thread->Tcb.Teb);
        } else {
            // The thread is a user-mode thread. Look to see if the thread owns the loader lock (and any other key peb-based critical sections.
            // If so, do our best to release the locks.

            // Since the LoaderLock used to be a mutant, releasing the lock like this is very similar to mutant abandonment and the loader never did anything with abandoned status anyway
            try {
                PTEB Teb;
                PPEB Peb;
                ...
                if (Cs) {
                    ...
                    // if the thread exited while waiting on the loader lock clean it up.
                    // There is still a potential race here since we can not safely know what happens to a thread after it interlocked increments the lock count but before it sets the waiting on loader lock flag.
                    // On the release side, it it safe since we mark ownership of the lock before clearing the flag.
                    // This triggers the first part of this test.
                    // The only thing out of whack is the recursion count, but this is also safe since in this state, recursion count is 0.
                    else if ( Teb->WaitingOnLoaderLock ) {
                        // This code isn't right. We need to bump down our lock count increment.

                        // A few cases to consider:

                        // Another thread releases the lock signals the event.
                        // We take the wait and then die before setting our ID.
                        // I doubt very much that this can happen because right after we come out of the wait, we set the owner Id (meaning that we would go through the other part of the if).
                        // Bottom line is that we should just decrement our lock count and get out of the way.
                        // There is no need to set the event.
                        // In the RAS stress failure, I saw us setting the event just because the lock count was >= 0.
                        // The lock was already held by another thread so setting the event let yet another thread also own the lock.
                        // Last one to release would get a not owner critical section failure
                ...
                if (Process->Wow64Process) {
                    ...
                    if (Cs32) {
                        ...
                        if (Cs32->OwningThread == PtrToUlong(Thread->Cid.UniqueThread)) {
                            // x86 uses a 1 based recursion count, so the IA64 kernel needs to do the same, since the critsect is really implemented by IA32 usermode.
            ...
            if (Process->Wow64Process) {
                ...
                try {
                    ...
                    BaseAddress = *(PVOID *) WOW64_TEB32_POINTER_ADDRESS(((PTEB) Thread->Tcb.Teb));// Get the TEB32 pointer

                    // Free it.  ZwFreeVirtualMemory will probe the address for us, ensuring that it is in usermode memory.
    ...
    // Let LPC component deal with message stack in Thread->LpcReplyMessage but do it after the client ID becomes invalid.
    ...
    ASSERT(Thread->Tcb.KernelApcDisable == 0);
    if (LastThread) {
        // can not hold the process lock while running down the object table since this can lead to a delete routine, which will need the job lock, BUT our lock ordering is always joblock -> processlock
        ObKillProcess(TRUE, Process);
        if (Process->Job) {
            // Now we can fold the process accounting into the job. Don't need to wait for the delete routine.
    ...
    (VOID) KeDisableApcQueuingThread(&Thread->Tcb);// Rundown pending APCs
    ...
    if ( FirstEntry ) {
        ...
        do {
            ...
            // If the APC has a rundown routine then call it. Otherwise deallocate the APC
    ...
    // There should never be any kernel mode APCs found this far into thread termination.
    // Since we go to PASSIVE_LEVEL upon entering exit.
    FirstEntry = KeFlushQueueApc(&Thread->Tcb,KernelMode);
    if ( FirstEntry ) {
        KeBugCheckEx(KERNEL_APC_PENDING_DURING_EXIT, (ULONG_PTR)FirstEntry, (ULONG_PTR)Thread->Tcb.KernelApcDisable, (ULONG_PTR)KeGetCurrentIrql(), 0);
    }
...
VOID PspExitProcess(IN BOOLEAN TrimAddressSpace, IN PEPROCESS Process)
{
    ...
    if (!Process->ExitProcessCalled && PspCreateProcessNotifyRoutineCount != 0) {
        ...
        for (i=0; i<PSP_MAX_CREATE_PROCESS_NOTIFY; i++) {
            if (PspCreateProcessNotifyRoutine[i] != NULL) {
                (*PspCreateProcessNotifyRoutine[i])( Process->InheritedFromUniqueProcessId, Process->UniqueProcessId, FALSE);
            }
        }
    }
    ...
    // If the process is on the active list, remove it now. Must be done before ObKill due to code in ex\sysinfo that references the process through the handle table
    if ( Process->ActiveProcessLinks.Flink != NULL && Process->ActiveProcessLinks.Blink != NULL ) {
    ...
    if ( TrimAddressSpace ) {
        // If the current process has previously set the timer resolution, then reset it.
             ...
             && !(Process->JobStatus & PS_JOB_STATUS_EXIT_PROCESS_REPORTED))
        {
            ...
            PS_SET_CLEAR_BITS(&Process->JobStatus, PS_JOB_STATUS_EXIT_PROCESS_REPORTED, PS_JOB_STATUS_LAST_REPORT_MEMORY);

            ExAcquireFastMutex(&Process->Job->MemoryLimitsLock);
            if (Process->Job->CompletionPort != NULL) {
                IoSetIoCompletion(Process->Job->CompletionPort, Process->Job->CompletionKey, (PVOID) Process->UniqueProcessId, STATUS_SUCCESS, ExitMessageId, FALSE);
            }
...
VOID PspProcessDelete(IN PVOID Object)
{
    ...
    AddressSpace = (ULONG) ((PHARDWARE_PTE) (&(Process->Pcb.DirectoryTableBase[0])))->PageFrameNumber;
...
/*++
Routine Description:
    This API allows a thread to register a port to be notified upon thread termination.
Arguments:
    PortHandle - Supplies an open handle to a port object that will be sent a termination message when the thread terminates.
Return Value:
    TBD
--*/
{
    ...
    st = ObReferenceObjectByHandle (PortHandle, 0, LpcPortObjectType, KeGetPreviousMode(), (PVOID *)&Port, NULL);
    if ( !NT_SUCCESS(st) ) {
        return st;
    }
...
--*/
{
    ...
    return PsGetCurrentProcess()->ExitTime;// Simply return the exit time for this process.
}
...
/*++
Routine Description:
    This routine returns TRUE if the specified thread is in the process of terminating.
Arguments:
