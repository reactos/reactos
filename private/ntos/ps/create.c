...
extern PVOID MmWorkingSetList;// This should really be in MM.H, but it cant be there yet.
...
// The following two globals are present to make it easier to change working set sizes when debugging.
ULONG PsMinimumWorkingSet = 20;
ULONG PsMaximumWorkingSet = 45;

BOOLEAN PsImageNotifyEnabled;

FAST_MUTEX PspProcessLockMutex;// Define the local storage for the process lock fast mutex.
...
Arguments:
    ...
    ProcessHandle - Supplies a handle to the process that the thread is being created within.
    ...
    CreateSuspended - Supplies a value that controls whether or not a thread is created in a suspended state.
Return Value:
    TBD
--*/
{
    ...
    if (KeGetPreviousMode() != KernelMode) {
        ...
        try {
            ...
            if (CapturedInitialTeb.OldInitialTeb.OldStackBase == NULL && CapturedInitialTeb.OldInitialTeb.OldStackLimit == NULL) {
        ...
        if (InitialTeb->OldInitialTeb.OldStackBase == NULL && InitialTeb->OldInitialTeb.OldStackLimit == NULL) {
            CapturedInitialTeb = *InitialTeb;
        } else {
            CapturedInitialTeb.OldInitialTeb = InitialTeb->OldInitialTeb;
        }
    ...
    st = PspCreateThread(ThreadHandle, DesiredAccess, ObjectAttributes, ProcessHandle, NULL, ClientId, ThreadContext, &CapturedInitialTeb, CreateSuspended, NULL, NULL);
    return st;
}
...
Arguments:
    ...
    DesiredAccess - Supplies the desired access modes to the new thread.
    ObjectAttributes - Supplies the object attributes of the new thread.
    ProcessHandle - Supplies a handle to the process that the thread is being created within.  If this parameter is not specified, then the initial system process is used.
    ClientId - Returns the CLIENT_ID of the new thread.
...
--*/
{
    ...
    st = PspCreateThread (ThreadHandle, DesiredAccess, ObjectAttributes, SystemProcess, ProcessPointer, ClientId, NULL, NULL, FALSE, StartRoutine, StartContext);
    return st;
}
...
Routine Description:
    ...
    It implements the foundation for NtCreateThread and for PsCreateSystemThread.
Arguments:
    ThreadHandle - Returns the handle for the new thread.
    DesiredAccess - Supplies the desired access modes to the new thread.
    ObjectAttributes - Supplies the object attributes of the new thread.
    ProcessHandle - Supplies a handle to the process that the thread is being created within.
    ClientId - Returns the CLIENT_ID of the new thread.
    ThreadContext - Supplies a pointer to a context frame that represents the initial user-mode context for a user-mode thread.
                    The absence of this parameter indicates that a system thread is being created.
    InitialTeb - Supplies the contents of certain fields for the new threads TEB.
                 This parameter is only examined if both a trap and exception frame were specified.
    CreateSuspended - Supplies a value that controls whether or not a user-mode thread is created in a suspended state.
    StartRoutine - Supplies the address of the system thread start routine.
    StartContext - Supplies context for a system thread start routine.
Return Value:
    TBD
--*/
{
    ...
    if (ProcessHandle) {
        ...
        // This accounts for the pointer given to the kernel that remains in effect until the thread terminates (and becomes signaled)
        st = ObReferenceObjectByHandle(ProcessHandle, PROCESS_CREATE_THREAD, PsProcessType, PreviousMode, (PVOID *) &Process, NULL);
    } else {
        if (StartRoutine) {
    ...
    // If the previous mode is user and the target process is the system process, then the operation cannot be performed.
    if ((PreviousMode != KernelMode) && (Process == PsInitialSystemProcess)) {
        ObDereferenceObject(Process);
        return STATUS_INVALID_HANDLE;
    }

    st = ObCreateObject(PreviousMode, PsThreadType, ObjectAttributes, PreviousMode, NULL, (ULONG) sizeof(ETHREAD), 0, 0, (PVOID *)&Thread);
    if ( !NT_SUCCESS( st ) ) {
        ObDereferenceObject(Process);
        return st;
    }
    ...
    Thread->ReadClusterSize = MmReadClusterSize;// Initialize Mm
    ...
    if (ARGUMENT_PRESENT(ThreadContext)) {
        ...
        try {
            ...
            (VOID) KeInitializeThread(&Thread->Tcb, KernelStack, PspUserThreadStartup, (PKSTART_ROUTINE) NULL, (PVOID) CONTEXT_TO_PROGRAM_COUNTER(ThreadContext), ThreadContext, Teb, &Process->Pcb);
        } except(EXCEPTION_EXECUTE_HANDLER) {
            if (Teb) {
        ...
        KeInitializeThread(&Thread->Tcb, KernelStack, PspSystemThreadStartup, StartRoutine, StartContext, NULL, NULL, &Process->Pcb);
    }

    InsertTailList(&Process->ThreadListHead,&Thread->ThreadListEntry);

    if (NeedToFixProcessId) {
        // Now that the thread is really there and will rundown through exit, open up the process id
        ...
        if (PspCreateProcessNotifyRoutineCount != 0) {
            ...
            for (i = 0; i < PSP_MAX_CREATE_PROCESS_NOTIFY; i++) {
                if (PspCreateProcessNotifyRoutine[i] != NULL) {
                    (*PspCreateProcessNotifyRoutine[i])(Process->InheritedFromUniqueProcessId, Process->UniqueProcessId, TRUE);
                }
            }
        }
    }

    // If the process has a job with a completion port, AND if the process is really considered to be in the Job, AND the process has not reported, report in

    // This should really be done in add process to job, but can't in this path because the process's ID isn't assigned until this point in time
    if (Process->Job && Process->Job->CompletionPort && !(Process->JobStatus & PS_JOB_STATUS_NOT_REALLY_ACTIVE) && !(Process->JobStatus & PS_JOB_STATUS_NEW_PROCESS_REPORTED))
    {
        ...
        if (Process->Job->CompletionPort != NULL) {
            IoSetIoCompletion(Process->Job->CompletionPort, Process->Job->CompletionKey, (PVOID) Process->UniqueProcessId, STATUS_SUCCESS, JOB_OBJECT_MSG_NEW_PROCESS, FALSE);
        }
    ...
    if (PspCreateThreadNotifyRoutineCount != 0) {
        ...
        for (i = 0; i < PSP_MAX_CREATE_THREAD_NOTIFY; i++) {
            if (PspCreateThreadNotifyRoutine[i] != NULL) {
                (*PspCreateThreadNotifyRoutine[i])(Thread->Cid.UniqueProcess, Thread->Cid.UniqueThread, TRUE);
            }
        }
    }
    ...
    // Failures that occur after this point cause the thread to go through PspExitThread

    // Reference count of thread is biased once for itself, and once to account for its Cid Handle
    // Note:
    //      if this fails we should do punt so we only have one cleanup path that really goes through PspExitThread ?
    //      Remember to free spin locks, cid...
    ObReferenceObject(Thread);
    ObReferenceObject(Thread);
    ObReferenceObject(Thread);
    st = ObInsertObject(Thread, NULL, DesiredAccess, 0, (PVOID *)NULL, &LocalThreadHandle);
    if ( !NT_SUCCESS(st) ) {
        // The insert failed. Terminate the thread.

        // This trick us used so that Dbgk doesn't report events for dead threads
    ...
    if (!Thread->DeadThread) {
        st = ObGetObjectSecurity(Thread, &SecurityDescriptor, &MemoryAllocated);
        if (!NT_SUCCESS(st)) {
            // This trick us used so that Dbgk doesn't report events for dead threads
            Thread->DeadThread = TRUE;
            Thread->HasTerminated = TRUE;
...
--*/
{
    ...
    if (ARGUMENT_PRESENT(SectionHandle) ) {
        // Use Object manager tag bits to indicate that breakaway is desired
        if ((UINT_PTR) SectionHandle & 1) {
            BreakAwayRequested = TRUE;
        }

        st = ObReferenceObjectByHandle(SectionHandle, SECTION_MAP_EXECUTE, MmSectionObjectType, PreviousMode, (PVOID *) &SectionToMap, NULL);
        if (!NT_SUCCESS(st)) {
            if (Parent) {
    ...
    if (ARGUMENT_PRESENT(DebugPort)) {
        st = ObReferenceObjectByHandle(DebugPort, 0, LpcPortObjectType, KeGetPreviousMode(), (PVOID *) &DebugPortObject, NULL);
        if (!NT_SUCCESS(st)) {
            if (Parent) {
                ObDereferenceObject(Parent);
            }
    ...
    if (ARGUMENT_PRESENT(ExceptionPort)) {
        st = ObReferenceObjectByHandle(ExceptionPort, 0, LpcPortObjectType, KeGetPreviousMode(), (PVOID *) &ExceptionPortObject, NULL);
        if (!NT_SUCCESS(st)) {
            if (Parent) {
                ObDereferenceObject(Parent);
            }
    ...
    st = ObCreateObject(KeGetPreviousMode(), PsProcessType, ObjectAttributes, KeGetPreviousMode(), NULL, (ULONG) sizeof(EPROCESS), 0, 0, (PVOID *) &Process);
    if (!NT_SUCCESS(st)) {
        if (Parent) {
            ObDereferenceObject(Parent);
        }
    ...
    // The process object is created set to NULL.
    // Errors That occur after this step cause the process delete routine to be entered.

    // Teardown actions that occur in the process delete routine do not need to be performed inline.
    ...
    KeInitializeProcess(&Process->Pcb, BasePriority, Affinity, &DirectoryTableBase[0], (BOOLEAN)(Process->DefaultHardErrorProcessing & PROCESS_HARDERROR_ALIGNMENT_BIT));
    Process->Pcb.ThreadQuantum = PspForegroundQuantum[0];
    Process->PriorityClass = PROCESS_PRIORITY_CLASS_NORMAL;
    if (Parent) {
        // this used to happen in basesrv\srvtask.c
        if (Parent->PriorityClass == PROCESS_PRIORITY_CLASS_IDLE || Parent->PriorityClass == PROCESS_PRIORITY_CLASS_BELOW_NORMAL) {
            Process->PriorityClass = Parent->PriorityClass;
        }

        // if address space creation worked, then when going through delete, we will attach.
        // Of course, attaching means that the kprocess must be initialized, so we delay the object stuff till here.
    ...
    //      2 - System Process. Address space is a virgin address space that only maps system space.
    //          Process is same as PspInitialSystemProcess.
    //      3 - User Process (Cloned Address Space). Address space is cloned from the specified process.
    //      4 - User Process (New Image Address Space). Address space is initialized so that it maps the specified section.
    if ( SectionToMap ) {
        // User Process (New Image Address Space). Don't specify Process to clone, just SectionToMap.
        st = MmInitializeProcessAddressSpace(Process, NULL, SectionToMap, &AuditName);
        ObDereferenceObject(SectionToMap);
        ObInitProcess2(Process);
        if ( NT_SUCCESS(st) ) {
            // In order to support relocating executables, the proper status (STATUS_IMAGE_NOT_AT_BASE) must be returned, so save it here.
            savedst = st;
    ...
    if ( Parent ) {
        if ( Parent != PsInitialSystemProcess ) {
            ...
            // User Process ( Cloned Address Space ).  Don't specify section to map, just Process to clone.
            st = MmInitializeProcessAddressSpace(Process, Parent, NULL, NULL);
            CreatePeb = TRUE;
        } else {
            // System Process.  Don't specify Process to clone or section to map
            st = MmInitializeProcessAddressSpace(Process, NULL, NULL, NULL);
        }
    }

insert_process:
    // If MmInitializeProcessAddressSpace was NOT successful, then dereference and exit.
    if ( !NT_SUCCESS(st) ) {
        if (Parent) {
            ObDereferenceObject(Parent);
        }
    ...
    // Reference count of process is not biased here. Each thread in the process bias the reference count when they are created.
    st = ObInsertObject(Process, NULL, DesiredAccess, 0, (PVOID *)NULL, &LocalProcessHandle);
    if ( !NT_SUCCESS(st) ) {
        if (Parent) {
            ObDereferenceObject(Parent);
        }
    ...
    // See if the parent has a job. If so reference the job and add the process in.
    ...
    if (Parent && CreatePeb ) {
        // For processes created w/ a section, a new "virgin" PEB is created. Otherwise, for forked processes, uses inherited PEB with an updated mutant.
        ...
        if ( SectionToMap ) {
            ...
            ZwWriteVirtualMemory(LocalProcessHandle, Process->Peb, &InitialPeb, sizeof(INITIAL_PEB), NULL);
        }

        // The new process should have a handle to its section. The section is either from the specified section, or the section of its parent.
        if ( ARGUMENT_PRESENT(SectionHandle) ) {
            DuplicateStatus = ZwDuplicateObject(NtCurrentProcess(), SectionHandle, LocalProcessHandle, &NewSection, 0L, 0L, DUPLICATE_SAME_ACCESS);
        } else {
            DuplicateStatus = ZwDuplicateObject(ParentProcess, Parent->SectionHandle, LocalProcessHandle, &NewSection, 0L, 0L, DUPLICATE_SAME_ACCESS);
        }
...
rewait:
                ...
                // Only place waitforever is used in conjunction with UserMode is exit where all we want to do with user-mode is allow our stack to swap, not service an exitapc
                if (Status == STATUS_USER_APC && WaitMode == UserMode && LocalLockMode == PsLockWaitForever) {
                    WaitMode = KernelMode;
                    goto rewait;
                }

                // Reacquire the process lock fast mutex and continue the loop if timeout has occured.
                ExAcquireFastMutexUnsafe(&PspProcessLockMutex);

                // If the specified process has exited, then set the completion status and exit the loop.
                // This test needs to be repeated so we catch the non-timed out wait case where the process terminated and the lock was released during the wait period
                if (Process->ExitTime.QuadPart != 0 && LocalLockMode != PsLockWaitForever) {
                    if (Status == STATUS_SUCCESS) {
                        // We came out of the wait due to someone setting the event.
                        // Since we are now going to bail, we need to set the event to pass ownership on to someone else.
                        WaitSuccess = TRUE;
                    }
                    Status = STATUS_PROCESS_IS_TERMINATING;
                    break;
            ...
            // If the completion status is success, then the lock has been granted and the owner is set.
            // Otherwise, a user APC is pending or the process has terminated and the lock request should be dropped.
            if (Status != STATUS_SUCCESS) {
                Status = STATUS_PROCESS_IS_TERMINATING;
                Process->LockCount += 1;
                if ( Process->LockCount == 1 ) {
                    // No one else is involved with the lock. We were granted owner ship. We need to make sure the event is NOT signalled

                    // Is doesn't matter if we bail in this case due to a successful wait, timeout. A lock count of 1 is the initial state of a free lock.
                    KeClearEvent(&Process->LockEvent);
                } else {
                    // A LockCount !=1 means that others are involved in the lock. We requested ownership of the lock by decrementing lockcount.

                    // We were granted ownership IF our wait was satisfied. Since we are bailing, we need to pass this ownership along to the next waiter.

                    // If we were not granted ownership (wait timed out, or we bailed before the wait) then we don't have to do anything more than increment the lock count
                    if ( WaitSuccess ) {
        ...
        // If the lock mode is not wait forever and the process has already terminated, then set the completion status to process terminating.
        // Otherwise, decrement the lock count, set the lock owner, and set the completion status to success.
        if ((LocalLockMode != PsLockWaitForever) && ((Process->ExitTime.QuadPart != 0 || KeReadStateProcess(&Process->Pcb) != FALSE))) {
            Status = STATUS_PROCESS_IS_TERMINATING;
        } else {
            Process->LockCount -= 1;
            Process->LockOwner = KeGetCurrentThread();
    ...
    // Release the process lock fast mutex and return the completion status.
    ExReleaseFastMutexUnsafe(&PspProcessLockMutex);

    if (Status != STATUS_SUCCESS) {
        KeLeaveCriticalRegion();
    } else {
        // We don't want to be "frozen" with the process lock held, so now that we own the lock, 
        // check to see if we have a pending freeze count, and if we do and dropping the lock will help, then drop it
        if ((LockMode != PsLockIAmExiting) && (Thread->Tcb.FreezeCount != 0) && (Thread->Tcb.KernelApcDisable == (ULONG) -1) ) {
            PsUnlockProcess(Process);
            goto retry;
        }
    }
