...
Arguments:
    CreateDeleteLockHeld - Supplies a flag that specifies whether or not the caller is holding the process create delete lock.
        If the caller holds the lock, than this function will not aquire the lock before suspending the process.
--*/
{
    PEPROCESS Process;
    ...
    // Freeze the execution of all threads in the current process, but the calling thread.
...
--*/
{
    ...
    // Thaw the execution of all threads in the current process, but the calling thread.
    if (!CreateDeleteLockHeld) {
        PsLockProcess(Process, KernelMode, PsLockWaitForever);
    }
...
/*++
Routine Description:
    This function Opens a handle to the file associated with the processes section.
    The file is opened such that it can be dupped all the way to the UI where the UI can either map the file or read the file to get the debug info.
Arguments:
...
--*/
{
    ...
    InitializeObjectAttributes(&Obja, &UnicodeFileName, OBJ_CASE_INSENSITIVE, NULL, NULL);
    Status = ZwOpenFile(
        &Handle,
...
/*++
Routine Description:
    This function is called when a new thread begins to execute.
    If the thread has an associated DebugPort, then a message is sent thru the port.
...
--*/
{
    ...
    // To determine if this should be turned into a create process, all threads in the process are suspended.
    // The process list is then examined to see if the curent thread is the only thread on the list.
    // If so, this becomes a create process message

    PsLockProcess(Process, KernelMode, PsLockWaitForever);

    // If we are doing a debug attach, then the create process has
    // already occured. If this is the case, then the process has accumulated some time, so set reported to true
    ...
    if (Process->CreateProcessReported == FALSE) {
        ...
        CreateProcessArgs->FileHandle = DbgkpSectionHandleToFileHandle(Process->SectionHandle);
        ...
        InitializeObjectAttributes(&Obja, &PsNtDllPathName, OBJ_CASE_INSENSITIVE, NULL, NULL);
        Status = ZwOpenFile(
...
/*++
Routine Description:
    This function is called when a new thread terminates.
    At this point, the thread will no longer execute in user-mode.
    No other exit processing has occured.
...
Routine Description:
    ...
    The address space of the process is still intact, but no threads exist in the process.
Arguments:
...
--*/
{
    ...
    // since this call is done while holding the process lock, and lock duration is controlled by debugger
...
/*++
Routine Description:
    This function is called when the current process successfully maps a view of an image section.
    If the process has an associated debug port, then a load dll message is sent.
Arguments:
...
/*++
Routine Description:
    This function is called when the current process successfully un maps a view of an image section.
    If the process has an associated debug port, then an "unmap view of section" message is sent.
Arguments:
