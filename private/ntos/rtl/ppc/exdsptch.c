...
/*++
Routine Description:
    This function attempts to dispatch an exception to a frame based handler by searching backwards through the stack based call frames.
    ...
    As each frame is encountered, the PC where control left the corresponding function is determined and used to lookup exception handler information
    in the runtime function table built by the linker. If the respective routine has an exception handler, then the handler is called. If the
    handler does not handle the exception, then the prologue of the routine is executed backwards to "unwind" the effect of the prologue and then
...
--*/
{
    ...
    if (ImageBase != NULL) {
        ...
        // If a function table is located, then search the function table for a function table entry for the specified PC.
        if (FunctionTable != NULL) {
            ...
            while (High >= Low) {
                ...
                if (ControlPc < FunctionEntry->BeginAddress) {
                    ...
                    if (((FunctionEntry->PrologEndAddress < FunctionEntry->BeginAddress) || (FunctionEntry->PrologEndAddress > FunctionEntry->EndAddress)) && (FunctionEntry->PrologEndAddress & 3) == 0) {
...
Arguments:
    ...
    FunctionEntry - Supplies the address of the function table entry for the specified function or NULL if the function is a leaf function.
    ContextRecord - Supplies the address of a context record.
    InFunction - Supplies a pointer to a variable that receives whether the control PC is within the current function.
    ...
    LowStackLimit, HighStackLimit - Range of valid values for the stack pointer.  This indicates whether it is valid to examine NextPc.
Return Value:
