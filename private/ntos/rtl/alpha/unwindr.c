...
--*/
{
    ...
    LastPc = RtlVirtualUnwind(ControlPc, FunctionEntry, &ContextRecord1, &InFunction, &EstablisherFrame, NULL);
    ControlPc = LastPc;
    ...
    if (RtlDebugFlags & RTL_DBG_UNWIND_DETAIL) {
        ...
        FunctionEntry = RtlLookupFunctionEntry(ControlPc);// Lookup the function table entry using the point at which control left the procedure.
        ...
        if (FunctionEntry != NULL) {
            ...
            if ((EstablisherFrame.Real < LowLimit) ||
                ...
                if ((ARGUMENT_PRESENT(TargetRealFrame) != FALSE) && ((ULONG_PTR)TargetRealFrame < EstablisherFrame.Real)) {
    ...
    if (RtlDebugFlags & RTL_DBG_UNWIND_DETAIL) {
        DbgPrint("RtlUnwindRfp: RtlpExecuteHandlerForUnwind returned Disposition = %lx\n", Disposition);
                    ...
                    switch (Disposition) {
                        ...
                        // Continue the search for a handler or continue execution.
                    ...
                    case ExceptionCollidedUnwind :
                        ...
                        RtlMoveMemory(&ContextRecord1, DispatcherContext.ContextRecord, sizeof(CONTEXT));
        ...
        ControlPc = LastPc;
    } while ((EstablisherFrame.Real < HighLimit) && (EstablisherFrame.Real != (ULONG_PTR)TargetRealFrame));
...

{
    ...
    if (RtlDebugFlags & RTL_DBG_UNWIND_DETAIL) {
        ...
        if (FunctionEntry != NULL) {
            ...
            // If the virtual frame pointer is not within the specified stack limits, the virtual frame pointer is unaligned, or the target
            // frame is below the virtual frame and an exit unwind is not being
            // performed, then raise the exception STATUS_BAD_STACK. Otherwise, check to determine if the current routine has an exception handler.
            if ((EstablisherFrame.Virtual < LowLimit) || (EstablisherFrame.Virtual > HighLimit) || ((ULONG_PTR)TargetFrame < EstablisherFrame.Virtual) || ((EstablisherFrame.Virtual & 0xF) != 0)) {
#if DBG
                DbgPrint("\n****** Warning - bad stack or target frame (unwind).\n");
                DbgPrint("  EstablisherFrame Virtual = %p, Real = %p\n", EstablisherFrame.Virtual, EstablisherFrame.Real);
                DbgPrint("  TargetFrame = %p\n", TargetFrame);
                if ((ARGUMENT_PRESENT(TargetFrame) != FALSE) && ((ULONG_PTR)TargetFrame < EstablisherFrame.Virtual)) {
