...
Abstract:
    ...
    The interfaces exported from this module are portable, but they must be re-implemented for each architecture.
...
--*/
{
    ...
    // InitialPc is the module entry point which is a function pointer in IA64.
    // StIIP and IntGp are initiailized with actual IP and GP from the plabel in LdrInitializeThunk after the loader runs.
    ...
    // ie, pass the initial parameter to the RSE by saving it at the bottom of the backing store.
...
/*++
Routine Description:
    This function calls a procedure in another thread/process, using NtGetContext and NtSetContext.
    Parameters are passed to the target procedure via its stack.
Arguments:
...
--*/
{
    ...
    // N.B. Align the context record address, stack pointer, and allocate stack scratch area.
    ...
    if (Count) {
        Status = NtWriteVirtualMemory(Process, (PVOID)Context.RsBSPSTORE, ArgumentsCopy, Count * sizeof(ULONGLONG), NULL);
        if (NT_SUCCESS(Status) == FALSE) {
            if (AlreadySuspended == FALSE) {
                NtResumeThread(Thread, NULL);
            }
    ...
    Context.RsRSC = (RSC_MODE_LY<<RSC_MODE) | (RSC_BE_LITTLE<<RSC_BE) | (0x3<<RSC_PL);// zero out loadrs, adjust RseStack

    //  Setup argument numbers
    Context.StIFS = (ULONGLONG)Count;       // # of arguments/size of frame
    Context.RsBSP = Context.RsBSPSTORE;
    Context.IntSp = (ULONG_PTR)NewSp;// Set the address of the target code into IIP, the new target stack into sp, setup ap, and reload context to make it happen.
    Context.StIIP = (ULONG_PTR)CallSite;// We expect NtSetContext to get the real IP and GP for us
    SANITIZE_FSR(Context.StFPSR, UserMode);// sanitize the floating pointer status register
...
static VOID RtlSetIaToEmDebugReg(IN ULONG Dr7_RW, IN ULONG Dr7_Len, IN ULONG Addr, IN OUT ULONGLONG *DbrAddr1, IN OUT ULONGLONG *DbrAddr2, IN OUT ULONGLONG *IbrAddr1, IN OUT ULONGLONG *IbrAddr2)
/*++
Routine Description:
    This function constructs the EM mode Debug Register Context given iA mode debug Context as input.
Arguments:
    ...
    N.B. Currently EM Debug Registers are only stored on a thread switch.
         Activating the EM debug context generated by this routine requires the EM debug registers to be loaded in the context of the target thread.
--*/
{
    ULONGLONG  mask;
...
Routine Description:
    This function constructs the EM mode Debug Register Context given iA mode debug Context as input.
    The EM debug control bits in the PSR are set based on the iA mode context.
Arguments:
...
Return Value:
    ...
    N.B. Currently EM Debug Registers are only stored on a thread switch. Activating the
         EM debug context generated by this routine requires the EM debug registers to be loaded in the context of the target thread.
--*/
{
    ULONG Dr7 = ContextX86->Dr7;

    RtlSetIaToEmDebugReg(DR7_DB0_RW(Dr7), DR7_DB0_LEN(Dr7), DR_ADDR_L0(ContextX86->Dr0), &ContextEM->DbD0, &ContextEM->DbD1, &ContextEM->DbI0, &ContextEM->DbI1);
    RtlSetIaToEmDebugReg(DR7_DB1_RW(Dr7), DR7_DB1_LEN(Dr7), DR_ADDR_L1(ContextX86->Dr1), &ContextEM->DbD2, &ContextEM->DbD3, &ContextEM->DbI2, &ContextEM->DbI3);
    RtlSetIaToEmDebugReg(DR7_DB2_RW(Dr7), DR7_DB2_LEN(Dr7), DR_ADDR_L2(ContextX86->Dr2), &ContextEM->DbD4, &ContextEM->DbD5, &ContextEM->DbI4, &ContextEM->DbI5);
    RtlSetIaToEmDebugReg(DR7_DB3_RW(Dr7), DR7_DB3_LEN(Dr7), DR_ADDR_L3(ContextX86->Dr3), &ContextEM->DbD6, &ContextEM->DbD7, &ContextEM->DbI6, &ContextEM->DbI7);
...
/*++
Routine Description:
    This function constructs the iA mode Debug Register Context given EM mode debug Context as input.
Arguments:
...
Return Value:
    ...
    N.B. Assumes that the thread's virtual iA mode Debug Registers reflects the current debug context.
         The kernel must keep the field DebugIaDr6 and DebugIaDr7 up to date.

    TBD How do we set DR6? From Status, what status? Probably should be set by exception handler.
--*/
{
    // Check to see if Debugging is enabled or disabled.
    ...
    if (!(DR7_L0(ContextX86->Dr7) || DR7_L1(ContextX86->Dr7) || DR7_L2(ContextX86->Dr7) || DR7_L3(ContextX86->Dr7)))
        return(FALSE);
    else
        return(TRUE);
}
...
VOID RtlEmToX86Context(IN PCONTEXT ContextEm, IN OUT PCONTEXT86 ContextX86)
{
    ...
    // TBD: How to transfer the remaining floating point states from EM context record to the floating save area in iA context record.
...
VOID RtlX86ToEmContext(IN PCONTEXT86 ContextX86, IN OUT PCONTEXT ContextEm)
{
    ...
    RtlIaFpToEmFpContext((PVOID)&ContextX86->FloatSave, &ContextEm->FltT2, &FpWorkArea);
