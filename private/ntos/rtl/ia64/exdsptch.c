...
/*++
Routine Description:
    This function returns the address of the first exception registration record for the current context.
Arguments:
    None.
Return Value:
...
/*++
Routine Description:
    This function removes the specified exception registration record (and thus the relevant handler) from the exception traversal chain.
Arguments:
...
/*++
Routine Description:
    This function searches the currently active function tables for an entry that corresponds to the specified PC value.
Arguments:
    ControlPc - Supplies the virtual address of an instruction bundle within the specified function.
    ImageBase - Returns the base address of the module to which the function belongs.
...
Return Value:
    ...
    Otherwise, the address of the function table entry that corresponds to the specified PC is returned.
--*/
{
    ...
    *ImageBase = (ULONG_PTR)RtlPcToFileHeader((PVOID)ControlPc, (PVOID *)ImageBase);// Search for the image that includes the specified swizzled PC value.

    // If an image is found that includes the specified PC, then locate the function table for the image.
    if ((PVOID)*ImageBase != NULL) {
        *TargetGp = (ULONG_PTR)(RtlImageDirectoryEntryToData((PVOID)*ImageBase, TRUE, IMAGE_DIRECTORY_ENTRY_GLOBALPTR, &Size));
        FunctionTable = (PRUNTIME_FUNCTION)RtlImageDirectoryEntryToData((PVOID)*ImageBase, TRUE, IMAGE_DIRECTORY_ENTRY_EXCEPTION, &SizeOfExceptionTable);

        // If a function table is located, then search the table for a function table entry for the specified PC.
...
--*/
{
    ...
    // there is a problem with the argument list itself. Raise another exception specifying the status value returned.
...

{
    ...
    RtlUnwind2(Frame, TargetIp, ExceptionRecord, ReturnValue, &ContextRecord);// Call real unwind routine specifying a context record as an extra argument.
}
...

{
    ...
    do {
        ...
        if (FunctionEntry != NULL) {
            NextPc = RtlpVirtualUnwind(ImageBase, ControlPc, FunctionEntry, ContextRecord, &InFunction, &EstablisherFrame, NULL);
        ...
        // handler.
            if (CHECK_MSTACK_FRAME(EstablisherFrame, TargetFrame)) {
                ...
                // unwind to the caller of the current routine and update the context record.
                if (!IS_SAME_FRAME(EstablisherFrame, TargetFrame)) {
                    NextPc = RtlVirtualUnwind(ImageBase, ControlPc, FunctionEntry, ContextRecord, &InFunction, &EstablisherFrame, NULL);
                }
            }
        ...
        ControlPc = NextPc;
    } while (((EstablisherFrame.MemoryStackFp < HighStackLimit) || (EstablisherFrame.BackingStoreFp > LowBStoreLimit)) && !(IS_SAME_FRAME(EstablisherFrame, TargetFrame)));
    ...
    // performed or the target of the unwind did not exist and the debugger and subsystem are given a second chance to handle the unwind.
    if (IS_SAME_FRAME(EstablisherFrame, TargetFrame)) {
...

{
    ...
    do {
        FunctionEntry = RtlLookupFunctionEntry(ControlPc, &ImageBase, &TargetGp);// Lookup the function table entry using the point at which control left the procedure.

        // If there is a function table entry for the routine, then
        // virtually unwind to the caller of the current routine to
        // obtain the virtual frame pointer of the establisher and check if there is an exception handler for the frame.
        if (FunctionEntry != NULL) {
            NextPc = RtlVirtualUnwind(ImageBase, ControlPc, FunctionEntry, &ContextRecordEm, &InFunction, &EstablisherFrame, NULL);

            // If either one or both of the two virtual frame pointers are not within the specified stack limits or unaligned,
            // then set the stack invalid flag in the exception record and return exception not handled.  Otherwise, check if the
            ...
            if (CHECK_MSTACK_FRAME(EstablisherFrame, NullFrame)) {
                ...
                // is written in assembler.  This is required because up level addressing of this routine information is required when a nested exception is encountered.
                ...
                do {
                    ...
                    if (NtGlobalFlag & FLG_ENABLE_EXCEPTION_LOGGING) {
                        Index = RtlpLogExceptionHandler(ExceptionRecord, ContextRecord, (ULONG)ControlPc, FunctionEntry, sizeof(RUNTIME_FUNCTION));
                    }
                    ...
                    // If the current scan is within a nested context and the frame just examined is the end of the nested region,
                    // then clear the nested context frame and the nested exception flag in the exception flags.
                    ...
                    // The disposition is hitting a frame processed by a previous unwind.

                    // Set the target of the current dispatch to the context record of the previous unwind
                    case ExceptionCollidedUnwind:
                        ControlPc = DispatcherContext.ControlPc;
                        NextPc = ControlPc;
                        EstablisherFrame = DispatcherContext.EstablisherFrame;
                        FunctionEntry = DispatcherContext.FunctionEntry;
                        ImageBase = DispatcherContext.ImageBase;
                        RtlMoveMemory(&ContextRecordEm, DispatcherContext.ContextRecord, sizeof(CONTEXT));
                        ExceptionFlags |= EXCEPTION_COLLIDED_UNWIND;
                        TargetGp = (ULONG_PTR)(RtlImageDirectoryEntryToData((PVOID)ImageBase, TRUE, IMAGE_DIRECTORY_ENTRY_GLOBALPTR, &Size));
                        break;
