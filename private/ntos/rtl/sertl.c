...
Routine Description:
    ...
    The purpose of this run-encoding is to change the character values to appear somewhat random and typically not printable.
    This is useful for transforming passwords that you don't want to be easily distinguishable by visually scanning a paging file or memory dump.
Arguments:
    Seed - Points to a seed value to use in the encoding.  If the pointed to value is zero, then this routine will assign a value.
    String - The string to encode.  This string may be decode by passing it and the seed value to RtlRunDecodeUnicodeString().
Return Value:
    None - Nothing can really go wrong unless the caller passes bogus parameters.  In this case, the caller can catch the access violation.
--*/
{
    LARGE_INTEGER Time;
    PUCHAR        LocalSeed;
    ...
    if ((*Seed) == 0) {
        ...
        // Occasionally, this byte could be zero.
        // That would cause the string to become un-decodable, since 0 is the magic value that causes us to re-gen the seed.
        // This loop makes sure that we never end up with a zero byte (unless time is zero, as well).
    ...
    // The funny constant just keeps the first byte from propagating into the second byte in the next step.
    // Without a funny constant this would happen for many languages (which typically have every other byte zero.
    if (S->Length >= 1) {
    ...
    for (i=1; i<S->Length; i++) {
        //  There are export issues that cause us to want to keep this algorithm simple.
        //  Please don't change it without checking with JimK first.  Thanks.

        // In order to be compatible with zero terminated unicode strings,
        //  this algorithm is designed to not produce a wide character of zero as long a the seed is not zero.

        // Simple running XOR with the previous byte and the seed value.
...
/*++
Routine Description:
    This function performs the inverse of the function performed by RtlRunEncodeUnicodeString().
    Please see RtlRunEncodeUnicodeString() for details.
Arguments:
...
--*/
{
    ...
    for (i=S->Length; i>1; i--) {
        // a simple running XOR with the previous byte and the seed value.
...
/*++
Routine Description:
    This function scrubs the passed string by over-writing all characters in the string.
    The entire string (i.e., MaximumLength) is erased, not just the current length.
Argumen ts:
    String - The string to be erased.
Return Value:
    None - Nothing can really go wrong unless the caller passes bogus parameters.
    In this case, the caller can catch the access violation.
--*/
{
    RTL_PAGED_CODE();
...
Arguments:
    Privilege - The lower 32-bits of the privilege ID to be enabled or disabled.  The upper 32-bits is assumed to be zero.
    Enable - A boolean indicating whether the privilege is to be enabled or disabled.
        TRUE indicates the privilege is to be enabled.
        FALSE indicates the privilege is to be disabled.
    Client - A boolean indicating whether the privilege should be adjusted in a client token or the process's own token.
        TRUE indicates the client's token should be used (and an error returned if there is no client token).
        FALSE indicates the process's token should be used.
    WasEnabled - points to a boolean to receive an indication of whether the privilege was previously enabled or disabled.
        TRUE indicates the privilege was previously enabled.
        FALSE indicates the privilege was previoulsy disabled.
        This value is useful for returning the privilege to its original state after using it.
Return Value:
...
--*/
{
    ...
    HANDLE Token;
    LUID LuidPrivilege;
    PTOKEN_PRIVILEGES NewPrivileges, OldPrivileges;
    ULONG Length;
    UCHAR Buffer1[sizeof(TOKEN_PRIVILEGES)+ ((1-ANYSIZE_ARRAY)*sizeof(LUID_AND_ATTRIBUTES))],
    ...
    // Map the success code NOT_ALL_ASSIGNED to an appropriate error since we're only trying to adjust the one privilege.
    if (Status == STATUS_NOT_ALL_ASSIGNED) {
        Status = STATUS_PRIVILEGE_NOT_HELD;
    }

    if (NT_SUCCESS(Status)) {
        // If there are no privileges in the previous state, there were no changes made.
        // The previous state of the privilege is whatever we tried to change it to.
...
--*/
{
    ...
    RTL_PAGED_CODE();
    // Make sure revision is SID_REVISION and sub authority count is not greater than maximum number of allowed sub-authorities.
    try {
...
Return Value:
    BOOLEAN - TRUE if the value of Sid1 is equal to Sid2, and FALSE otherwise.
--*/
{
   ULONG SidLength;
   ...
   if ( ((SID *)Sid1)->Revision == ((SID *)Sid2)->Revision ) {
       // Check the SubAuthorityCount first, because it's fast and can help us exit faster.
...
Arguments:
    Sid1, Sid2 - Supply pointers to the two SID values to compare.
        The SID structures are assumed to be valid.
Return Value:
    BOOLEAN - TRUE if the prefix value of Sid1 is equal to Sid2, and FALSE otherwise.
--*/
{
    LONG Index;
    ...
    // Either the Revision, SubAuthorityCount, or IdentifierAuthority values weren't equal.
    return FALSE;
}
...
Routine Description:
    ...
    A sid allocated with this routine must be freed using RtlFreeSid().
...
Arguments:
    ...
    SubAuthorityCount - The number of sub-authorities to place in the SID.
        This also identifies how many of the SubAuthorityN parameters have meaningful values.
        This must contain a value from 0 through 8.
    SubAuthority0-7 - Provides the corresponding sub-authority value to place in the SID.
        For example, a SubAuthorityCount value of 3 indicates that SubAuthority0, SubAuthority1, and SubAuthority0 have meaningful values and the rest are to be ignored.
...
Arguments:
    ...
    IdentifierAuthority - Pointer to the Identifier Authority value to set in the SID.
    SubAuthorityCount - The number of sub-authorities that will be placed in the SID (a separate action).
--*/
{
    PISID ISid;
...
/*++
Routine Description:
    This function is used to free a SID previously allocated using RtlAllocateAndInitializeSid().
    THIS ROUTINE IS CURRENTLY NOT CALLABLE FROM KERNEL MODE.
Arguments:
...
--*/
{
    ...
    ISid = (PISID)Sid;//  Typecast to the opaque SID
...
Arguments:
    ...
    SubAuthority - An index indicating which sub-authority is being specified.
        This value is not compared against the number of sub-authorities in the SID for validity.
--*/
{
    RTL_PAGED_CODE();
...
Arguments:
    Sid - Points to the SID whose length is to be returned.  The SID's structure is assumed to be valid.
Return Value:
...
Arguments:
    DestinationSidLength - Indicates the length, in bytes, of the destination SID buffer.
    DestinationSid - Pointer to a buffer to receive a copy of the source Sid value.
    SourceSid - Supplies the Sid value to be copied.
Return Value:
    STATUS_SUCCESS - Indicates the SID was successfully copied.
    STATUS_BUFFER_TOO_SMALL - Indicates the target buffer wasn't large enough to receive a copy of the SID.
--*/
{
    ...
    RtlMoveMemory( DestinationSid, SourceSid, SidLength );// Buffer is large enough
    return STATUS_SUCCESS;
}
...
/*++
Routine Description:
    This routine copies the value of the source SID_AND_ATTRIBUTES array to the target.
    The actual SID values are placed according to a separate parameter.
    This allows multiple arrays to be merged using this service to copy each.
Arguments:
    ArrayLength - Number of elements in the source array to copy.
    Source - Pointer to the source array.
    TargetSidBufferSize - Indicates the length, in bytes, of the buffer to receive the actual SID values.
        If this value is less than the actual amount needed, then STATUS_BUFFER_TOO_SMALL is returned.
    TargetArrayElement - Indicates where the array elements are to be copied to (but not the SID values themselves).
    TargetSid - Indicates where the target SID values s are to be copied.  This is assumed to be ULONG aligned.
        Each SID value will be copied into this buffer.
        Each SID will be ULONG aligned.
    NextTargetSid - On completion, will be set to point to the ULONG aligned address following the last SID copied.
    RemainingTargetSidBufferSize - On completion, receives an indicatation of how much of the SID buffer is still unused.
Return Value:
    STATUS_SUCCESS - The call completed successfully.
    STATUS_BUFFER_TOO_SMALL - Indicates the buffer to receive the SID values wasn't large enough.
--*/
{
    ULONG Index = 0;
...
/*++
Routine Description:
    This function returns the maximum length of the string needed to represent the SID supplied.
    The actual string may be shorter, but this is intended to be a quick calculation.
Arguments:
    Sid - Supplies the SID that is to be converted to unicode.
    StringLength - Receives the max length required in bytes.
Return Value:
    SUCCESS - The conversion was successful
    STATUS_INVALID_SID - The sid provided does not have a valid structure, or has too many sub-authorities (more than SID_MAX_SUB_AUTHORITIES).
--*/
{
    ULONG   i ;
    ...
    // if the SID's IA value has 5 or 6 significant bytes, the representation will be in hex, with a 0x preceding.
    // Otherwise it will be in decimal, with at most 10 characters.
    ...
    // for each sub authority, it is a max of 10 chars (for a ulong), plus the - separator
...
Routine Description:
    ...
    The resulting string will take one of two forms.
    If the IdentifierAuthority value is not greater than 2^32, then the SID will be in the form:
...
Arguments:
    UnicodeString - Returns a unicode string that is equivalent to the SID.
...
Return Value:
    SUCCESS - The conversion was successful
    STATUS_INVALID_SID - The sid provided does not have a valid structure, or has too many sub-authorities (more than SID_MAX_SUB_AUTHORITIES).
    STATUS_NO_MEMORY - There was not sufficient memory to allocate the target string.
        This is returned only if AllocateDestinationString is specified as TRUE.
    STATUS_BUFFER_OVERFLOW - This is returned only if AllocateDestinationString is specified as FALSE.
--*/
{
    NTSTATUS Status;
    WCHAR UniBuffer[ 256 ];
...
Routine Description:
    ...
    This routine is here for backwards compatibility only. New code should use the macro.
Arguments:
    Luid1, Luid2 - Supply pointers to the two LUID values to compare.
Return Value:
    BOOLEAN - TRUE if the value of Luid1 is equal to Luid2, and FALSE otherwise.
--*/
{
...
/*++
Routine Description:
    This routine copies the value of the source LUID to the destination LUID.
Arguments:
    DestinationLuid - Receives a copy of the source Luid value.
    SourceLuid - Supplies the Luid value to be copied.  This LUID is assumed to be structurally valid.
--*/
{
    RTL_PAGED_CODE();
...
/*++
Routine Description:
    This routine copies the value of the source LUID_AND_ATTRIBUTES array to the target.
Arguments:
    ArrayLength - Number of elements in the source array to copy.
...
Routine Description:
    ...
    After the procedure call the security descriptor is initialized with no system ACL, no discretionary ACL, no owner, no primary group and all control flags set to false (null).
Arguments:
    SecurityDescriptor - Supplies the security descriptor to initialize.
    Revision - Provides the revision level to assign to the security descriptor.  This should be one (1) for this release.
Return Value:
    STATUS_SUCCESS - Indicates the call completed successfully.
    STATUS_UNKNOWN_REVISION - Indicates the revision level provided is not supported by this routine.
--*/
{
    RTL_PAGED_CODE();
...
Return Value:
    ...
    STATUS_UNKNOWN_REVISION - Indicates the revision level provided is not supported by this routine.
Note:
    Warning, this code assume the caller allocated a relative security descriptor rather than a relative one.
    Absolute is larger on systems with 64-bit pointers.
--*/
{
    RTL_PAGED_CODE();
...
/*++
Routine Description:
    This routine returns the length, in bytes, necessary to capture a structurally valid SECURITY_DESCRIPTOR.
    The length includes the length of all associated data structures (like SIDs and ACLs).
    The length also takes into account the alignment requirements of each component.
    The minimum length of a security descriptor (one which has no associated SIDs or ACLs) is SECURITY_DESCRIPTOR_MIN_LENGTH.
Arguments:
    SecurityDescriptor - Points to the SECURITY_DESCRIPTOR whose length is to be returned.  The SECURITY_DESCRIPTOR's structure is assumed to be valid.
Return Value:
...
--*/
{
    ...
    sum = ISecurityDescriptor->Control & SE_SELF_RELATIVE ? sizeof(SECURITY_DESCRIPTOR_RELATIVE) : sizeof(SECURITY_DESCRIPTOR);
...
/*++
Routine Description:
    This routine returns the length, in bytes, in use in a structurally valid SECURITY_DESCRIPTOR.

    This is the number of bytes necessary to capture the security descriptor, which may be less the the current actual length of the security descriptor
    (RtlLengthSecurityDescriptor() is used to retrieve the actual length).

    Notice that the used length and actual length may differ if either the SACL or DACL include padding bytes.
...
Arguments:
    SecurityDescriptor - Points to the SECURITY_DESCRIPTOR whose used length is to be returned.
        The SECURITY_DESCRIPTOR's structure is assumed to be valid.
Return Value:
...
--*/
{
    ...
    Temp = RtlpDaclAddrSecurityDescriptor(ISecurityDescriptor);
    if ( Temp != NULL ) {
        RtlQueryInformationAcl( Temp,(PVOID)&AclSize,sizeof(AclSize),AclSizeInformation );
...
NTSTATUS RtlSetAttributesSecurityDescriptor(IN PSECURITY_DESCRIPTOR SecurityDescriptor,IN SECURITY_DESCRIPTOR_CONTROL Control,OUT PULONG Revision)
{
    ...
    // Always return the revision value - even if this isn't a valid security descriptor
...
Return Value:
    STATUS_SUCCESS - Indicates the call completed successfully.
    STATUS_UNKNOWN_REVISION - Indicates the revision of the security descriptor is not known to the routine.
        It may be a newer revision than the routine knows about.
--*/
{
    RTL_PAGED_CODE();
...
Routine Description:
    ...
    For instance,SetSecurityDescriptorControl( &SecDesc,SE_DACL_PROTECTED,SE_DACL_PROTECTED );
    marks the DACL on the security descriptor as protected. And SetSecurityDescriptorControl( &SecDesc,SE_DACL_PROTECTED,0 );
    marks the DACL as not protected.
Arguments:
    pSecurityDescriptor - Supplies the security descriptor.
    ControlBitsOfInterest - A mask of the control bits being changed, set, or reset by this call.
        The mask is the logical OR of one or more of the following flags:
            SE_DACL_UNTRUSTED
            SE_SERVER_SECURITY
            SE_DACL_AUTO_INHERIT_REQ
            SE_SACL_AUTO_INHERIT_REQ
...
Arguments:
    SecurityDescriptor - Supplies the security descriptor to be which the discretionary ACL is to be added.
    DaclPresent - If FALSE, indicates the DaclPresent flag in the security descriptor should be set to FALSE.
        In this case, the remaining optional parameters are ignored.
        Otherwise, the DaclPresent control flag in the security descriptor is set to TRUE and the remaining optional parameters are not ignored.
    Dacl - Supplies the discretionary ACL for the security descriptor.
        If this optional parameter is not passed, then a null ACL is assigned to the security descriptor.
    ...
    DaclDefaulted - When set, indicates the discretionary ACL was picked up from some default mechanism (rather than explicitly specified by a user).
        This value is set in the DaclDefaulted control flag in the security descriptor.
...
Arguments:
    SecurityDescriptor - Supplies the security descriptor.
    DaclPresent - If TRUE, indicates that the security descriptor does contain a discretionary ACL.
    ...
    Dacl - This value is returned only if the value returned for the DaclPresent flag is TRUE.
    ...
    DaclDefaulted - This value is returned only if the value returned for the DaclPresent flag is TRUE.
...
--*/
{
    ...
    *DaclPresent = RtlpAreControlBitsSet( ISecurityDescriptor, SE_DACL_PRESENT );
    if (*DaclPresent) {
        *Dacl = RtlpDaclAddrSecurityDescriptor(ISecurityDescriptor);// Assign the ACL address.
        *DaclDefaulted = RtlpAreControlBitsSet( ISecurityDescriptor, SE_DACL_DEFAULTED );// Assign DaclDefaulted flag.
    }
...
/*++
Routine Description:
    This procedure sets the system ACL information of an absolute security descriptor.
    If there is already a system ACL present in the security descriptor, it is superseded.
Arguments:
    SecurityDescriptor - Supplies the security descriptor to be which the system ACL is to be added.
    SaclPresent - If FALSE, indicates the SaclPresent flag in the security descriptor should be set to FALSE.
        In this case, the remaining optional parameters are ignored.
        Otherwise, the SaclPresent control flag in the security descriptor is set to TRUE and the remaining optional parameters are not ignored.
    Sacl - Supplies the system ACL for the security descriptor.  If this optional parameter is not passed, then a null ACL is assigned to the security descriptor.
        The ACL is referenced by, not copied into, by the security descriptor.
    SaclDefaulted - When set, indicates the system ACL was picked up from some default mechanism (rather than explicitly specified by a user).
        This value is set in the SaclDefaulted control flag in the security descriptor.
        If this optional parameter is not passed, then the SaclDefaulted flag will be cleared.
Return Value:
    STATUS_SUCCESS - Indicates the call completed successfully.
    STATUS_UNKNOWN_REVISION - Indicates the revision of the security descriptor is not known to the routine.
        It may be a newer revision than the routine knows about.
...
Arguments:
    SecurityDescriptor - Supplies the security descriptor.
    SaclPresent - If TRUE, indicates that the security descriptor does contain a system ACL.
        In this case, the remaining OUT parameters will receive valid values.
        Otherwise, the security descriptor does not contain a system ACL and the remaining OUT parameters will not receive valid values.
    Sacl - This value is returned only if the value returned for the SaclPresent flag is TRUE.
        In this case, the Sacl parameter receives the address of the security descriptor's system ACL.
        If this value is returned as null, then the security descriptor has a null system ACL.
    SaclDefaulted - This value is returned only if the value returned for the SaclPresent flag is TRUE.
        In this case, the SaclDefaulted parameter receives the value of the security descriptor's SaclDefaulted control flag.
Return Value:
    STATUS_SUCCESS - Indicates the call completed successfully.
    STATUS_UNKNOWN_REVISION - Indicates the revision of the security descriptor is not known to the routine.
        It may be a newer revision than the routine knows about.
--*/
{
    ...
    *SaclPresent = RtlpAreControlBitsSet( ISecurityDescriptor, SE_SACL_PRESENT );
    if (*SaclPresent) {
        *Sacl = RtlpSaclAddrSecurityDescriptor(ISecurityDescriptor);// Assign the ACL address.
        *SaclDefaulted = RtlpAreControlBitsSet( ISecurityDescriptor, SE_SACL_DEFAULTED );// Assign SaclDefaulted flag.
    }
...
/*++
Routine Description:
    This procedure sets the owner information of an absolute security descriptor.
    If there is already an owner present in the security descriptor, it is superseded.
Arguments:
    SecurityDescriptor - Supplies the security descriptor in which the owner is to be set.
        If the security descriptor already includes an owner, it will be superseded by the new owner.
    Owner - Supplies the owner SID for the security descriptor.  If this optional parameter is not passed, then the owner is cleared (indicating the security descriptor has no owner).
        The SID is referenced by, not copied into, the security descriptor.
    OwnerDefaulted - When set, indicates the owner was picked up from some default mechanism (rather than explicitly specified by a user).
        This value is set in the OwnerDefaulted control flag in the security descriptor.
        If this optional parameter is not passed, then the SaclDefaulted flag will be cleared.
Return Value:
    STATUS_SUCCESS - Indicates the call completed successfully.
    STATUS_UNKNOWN_REVISION - Indicates the revision of the security descriptor is not known to the routine.
...
Arguments:
    SecurityDescriptor - Supplies the security descriptor.
    Owner - Receives a pointer to the owner SID.  If the security descriptor does not currently contain an owner, then this value will be returned as null.
    ...
    OwnerDefaulted - This value is returned only if the value returned for the Owner parameter is not null.
        In this case, the OwnerDefaulted parameter receives the value of the security descriptor's OwnerDefaulted control flag.
Return Value:
    STATUS_SUCCESS - Indicates the call completed successfully.
    STATUS_UNKNOWN_REVISION - Indicates the revision of the security descriptor is not known to the routine.  It may be a newer revision than the routine knows about.
--*/
{
    // Typecast to the opaque SECURITY_DESCRIPTOR structure.
    SECURITY_DESCRIPTOR *ISecurityDescriptor = SecurityDescriptor;
    ...
    *Owner = RtlpOwnerAddrSecurityDescriptor(ISecurityDescriptor);// Return the Owner field value.
    *OwnerDefaulted = RtlpAreControlBitsSet( ISecurityDescriptor, SE_OWNER_DEFAULTED );// Return the OwnerDefaulted flag value.
    return STATUS_SUCCESS;
}
...
Arguments:
    ...
    SecurityDescriptor - Supplies the security descriptor in which the primary group is to be set.
        If the security descriptor already includes a primary group, it will be superseded by the new group.

    Group - Supplies the primary group SID for the security descriptor.
        If this optional parameter is not passed, then the primary group is cleared (indicating the security descriptor has no primary group).
        The SID is referenced by, not copied into, the security descriptor.

    GroupDefaulted - When set, indicates the owner was picked up from some default mechanism (rather than explicitly specified by a user).
        This value is set in the OwnerDefaulted control flag in the security descriptor.
        If this optional parameter is not passed, then the SaclDefaulted flag will be cleared.
...
Arguments:
    SecurityDescriptor - Supplies the security descriptor.
    Group - Receives a pointer to the primary group SID.  If the security descriptor does not currently contain a primary group, then this value will be returned as null.
        In this case, the remaining OUT parameters are not given valid return values.
        Otherwise, this parameter points to an SID and the remaining OUT parameters are provided valid return values.
    GroupDefaulted - This value is returned only if the value returned for the Group parameter is not null.
...
Routine Description:
    ...
    It is used by the Object Management component when dereferencing a handle.
Arguments:
        GrantedAccess - Specifies the granted access mask.
        DesiredAccess - Specifies the desired access mask.
Return Value:
    BOOLEAN - TRUE if the GrantedAccess mask has all the bits set that the DesiredAccess mask has set.
        That is, TRUE is returned if all of the desired accesses have been granted.
--*/
{
    RTL_PAGED_CODE();
...
Routine Description:
    ...
    It is used by components other than the the Object Management component for checking access mask subsets.
Arguments:
        GrantedAccess - Specifies the granted access mask.
        DesiredAccess - Specifies the desired access mask.
Return Value:
    BOOLEAN - TRUE if the GrantedAccess mask contains any of the bits specified in the DesiredAccess mask.
        That is, if any of the desired accesses have been granted, TRUE is returned.
--*/
{
    RTL_PAGED_CODE();
...
/*++
Routine Description:
    This routine maps all generic accesses in the provided access mask to specific and standard accesses according to the provided GenericMapping.
Arguments:
...
Routine Description:
    ...
    This may be useful for enabling a privilege for a single thread rather than for the entire process; or changing the default DACL for a single thread.
...
Arguments:
    ...
    ServerObject - Boolean indicating whether or not this is a server object, meaning it is protected by a primary-client combination.
...
--*/
{
    ...
    // If it's not a server object, check the owner against the contents of the client token.
    // If it is a server object, the owner must be valid in the primary token.
    if (!ServerObject) {
        TokenToUse = Token;
    } else {
    ...
    *ReturnStatus = NtQueryInformationToken(TokenToUse,TokenGroups,GroupIds,ReturnLength,&ReturnLength);
    ...
    //  Walk through the list of group IDs looking for a match to the specified SID.  If one is found, make sure it may be assigned as an owner.
...
/*++
Routine Description:
    This is a private routine that maps Access Masks of an ACL so that they are applicable to the object type the ACL is being applied to.
    Only known DSA ACEs are mapped.  Unknown ACE types are ignored.
    Only access types in the GenericAll mapping for the target object type will be non-zero upon return.
Arguments:
...
Arguments:
    ...
    WillGenerateInheritAce - Specifies if the caller intends to generate an inheritable ACE the corresponds to OldAce.
        If TRUE, this routine will try to not map the effective ACE (increasing the likelyhood that EffectiveAceMapped will return FALSE),
    ...
    AcePosition - On entry and exit, specifies location of the next available ACE position in NewAcl.
        A NULL ACE position means there is no room at all in NewAcl.
    NewAceLength - Returns the length (in bytes) needed in NewAcl to copy the specified ACE.
        This might be zero to indicate that the ACE need not be copied at all.
    NewAcl - Provides a pointer to the ACL into which the ACE is to be inherited.
    ...
    EffectiveAceMapped - Return TRUE if the SID, guid, or access mask of Old Ace was modifed when copying the ACE.
    AclOverflowed - Returns TRUE if NewAcl wasn't long enough to contain NewAceLength.
Return Value:
    TRUE - No problem was detected.
    FALSE - Indicates something went wrong preventing the ACE from being compied.
        This generally represents a bugcheck situation when returned from this call.
--*/
{
    ...
    *AclOverflowed = FALSE;
    ...
    if ( IsMSAceType(OldAce) ) {
        ...
        if (IsKnownAceType( OldAce ) ) {
            ...
            ServerSidToCopy = RtlCompoundAceServerSid( OldAce );// Compound ACEs have two SIDs (Map one now).
        ...
        // Handle Object ACEs
            ...
            if ( ARGUMENT_PRESENT(ObjectAceInherited) && InheritedObjectType != NULL ) {
                // If the object type doesn't match the inherited object type, don't inherit the ACE.
                if ( NewObjectType == NULL || !RtlEqualMemory( InheritedObjectType,NewObjectType,sizeof(GUID) ) ) {
                    LengthRequired = 0;

                // If the object type matches the inherited object type, Inherit an ACE with no inherited object type.
                } else {
                    *ObjectAceInherited = TRUE;// Tell the caller we inherited an object type specific ACE.
                    ...
                    if ( !WillGenerateInheritAce ) {
                        *EffectiveAceMapped = TRUE;
                        ...
                        // If an object type GUID is present, simply delete the inherited object type GUID.
                        ...
                        // If an object type GUID is not present, convert the ACE to non-object type specific.
                        } else {
                            AceHeaderToCopyLength = AceHeaderToCopyLength - sizeof(GUID) + sizeof(KNOWN_ACE) - sizeof(KNOWN_OBJECT_ACE);
        ...
        if ( LengthRequired != 0 ) {
            // If after mapping the access mask, the access mask is empty, then drop the ACE.

            // This is incompatible with NT 4.0 which simply mapped and left undefined access bits set.
            ...
            if (LocalMask == 0) {
                ...
                if ( *AcePosition == NULL || LengthRequired > (ULONG)NewAcl->AclSize - ((PUCHAR)(*AcePosition) - (PUCHAR)NewAcl) ) {
                    *AclOverflowed = TRUE;
                } else {
                    PUCHAR Target;
                    ...
                    ((PKNOWN_ACE)*AcePosition)->Mask = LocalMask;// Put the mapped access mask in the new ACE
                }
            }
        }
    } else {
        ...
        if ( LengthRequired > (ULONG)NewAcl->AclSize - ((PUCHAR)*AcePosition - (PUCHAR)NewAcl) ) {
            *AclOverflowed = TRUE;
            ...
            RtlCopyMemory(*AcePosition,OldAce,LengthRequired );// Not a known ACE type, copy ACE as is
         }
    }

    // If the ACE was actually kept, clear all the inherit flags and update the ACE count of the ACL.
    ...
    // it with a ULONG.  It must fit into a USHORT.  See if it does.
...
/*++
Routine Description:
    Copy ACEs from of an ACL and perform generic mapping.
    Only ACEs specified by 'AceFilter' are copied.
Arguments:
    ...
    MapSids - TRUE if the SID in the ACE is to be mapped to the corresponding actual SID.
    ClientOwnerSid - Specifies the owner Sid to use
    ClientGroupSid - Specifies the new Group Sid to use
...
Return Value:
    ...
    STATUS_BUFFER_TOO_SMALL - The ACL specified by NewAcl is too small for the copied ACEs.  The required size is returned in NewAceLength.
--*/
{
    NTSTATUS Status;
    ULONG i;
    ...
    for (i = 0, OldAce = FirstAce(Acl); i < Acl->AceCount; i += 1, OldAce = NextAce(OldAce)) {
        // If the ACE wasn't inherited, copy it.
        ...
        if ( CopyAce ) {
            // If SIDs are to be mapped, do so (and potentially create up to two ACEs).
            if ( MapSids ) {
                ...
                NewAceSize = 0;
                GenerateInheritAce = (((PACE_HEADER)OldAce)->AceFlags & (OBJECT_INHERIT_ACE|CONTAINER_INHERIT_ACE)) != 0;

                // If the orginal ACE is an effective ACE, create an effective ACE.
                ...
                // If the original ACE is inheritable, create an inheritable ACE.
                ...
                if ( GenerateInheritAce ) {
                    ...
                    if ( EffectiveAceSize != 0 && !EffectiveAceMapped ) {
                       ...
                       if ( !AclOverflowed ) {
                            ((PACE_HEADER)AcePosition)->AceFlags |= ((PACE_HEADER)OldAce)->AceFlags & (VALID_INHERIT_FLAGS);
                            ((PACE_HEADER)AcePosition)->AceFlags &= ~AceFlagsToReset;
                       }
                ...
                if ( AcePosition == NULL || NewAceSize > (ULONG)NewAcl->AclSize - ((PUCHAR)AcePosition - (PUCHAR)NewAcl) ) {
                    ...
                    // Is it really right to map the generic bits on an ACE that's both effective and inheritable.
                    // Shouldn't this be split into two ACEs in that case?  Or just skip the mapping?
    ...
    // We have the length of the new ACE, but we've calculated it with a ULONG.
    // It must fit into a USHORT.  See if it does.
...
Arguments:
    ...
    ChildAcl - Supplies the acl associated with the object.  This is either the current acl on the object or the acl being assigned to the object.
    ChildGenericControl - Specifies the control flags from the SecurityDescriptor describing the ChildAcl:
        SEP_ACL_PRESENT: Specifies that the child ACL is explictly supplied by the caller.
        SEP_ACL_DEFAULTED: Specifies that the child ACL was supplied by some defaulting mechanism.
        SEP_ACL_PROTECTED: Specifies that the child ACL is protected and should not inherit any ACE from the DirectoryACL
    IsDirectoryObject - Specifies if the new acl is for a directory.
    AutoInherit - Specifies if the inheritance is an "automatic inheritance".
        As such, the non-inherited ACEs from the ChildAcl will be preserved and the inherited ACEs from the DirectoryAcl will be marked as such.
    DefaultDescriptorForObject - If set, the CreatorDescriptor is the default descriptor for ObjectType.
        As such, the CreatorDescriptor will be ignored if any ObjectType specific ACEs are inherited from the parent.
        If not such ACEs are inherited, the CreatorDescriptor is handled as though this flag were not specified.
    ...
    NewAclExplicitlyAssigned - Returns true to indicate that some portion of "NewAcl" was derived from an the explicit ChildAcl
...
Return Value:
    ...
    STATUS_NO_INHERITANCE - An inheritable ACL was not successfully generated.
        This is a warning completion status.  The caller should use the default ACL.
    ...
    STATUS_BUFFER_TOO_SMALL - The ACL specified by NewAcl is too small for the inheritance ACEs.  The required size is returned in AclBufferSize.
--*/
{
    NTSTATUS Status;
    ...
    *NewGenericControl = AutoInherit ? SEP_ACL_AUTO_INHERITED : 0;
    ...
    if ( (ChildGenericControl & SEP_ACL_DEFAULTED) == 0 ) {
        ...
        if ( (ChildGenericControl & (SEP_ACL_PRESENT|SEP_ACL_PROTECTED)) != 0 ) {
            if ( ChildAcl != NULL ) {
                ...
                // Since we're explicitly using the ACL specified by the caller, we never want to return a NULL ACL.
                //  Rather, if we have an ACL with no ACEs, we'll return exactly that.
                //  For a DACL, that results in a DACL that grants no access rather than a DACL that grants all access.
                NullAclOk = FALSE;

                // If the caller doesn't understand auto inheritance, simply preserve the specified ACL 100% intact.
                if ( !AutoInherit ) {
                    AceTypeToCopy = CopyAllAces;
                    AceFlagsToReset = 0;      // Don't turn off any ACE Flags
                    MapSids = FALSE;          // For backward compatibility

                // If the child is protected, keep all of the ACEs turning off the INHERITED ACE flags.
                } else if ( ChildGenericControl & SEP_ACL_PROTECTED ) {
                    AceTypeToCopy = CopyAllAces;
                    AceFlagsToReset = INHERITED_ACE; // Turn off all INHERITED_ACE flags
                    MapSids = TRUE;

                // If the child is not protected, just copy the non-inherited ACEs.
                ...
                Status = RtlpCopyAces(ChildAcl,GenericMapping,AceTypeToCopy,AceFlagsToReset,MapSids,OwnerSid,GroupSid,ServerOwnerSid,ServerGroupSid,&ChildNewAclSize,(PACL)AclBuffer );
                ...
                // If this ACL might be ignored later, remember the current state of the ACL.
            ...
            // If the ACL isn't protected, don't allow NULL ACL semantics.
            //  (those semantics are ambiguous for auto inheritance)
            } else if ( AutoInherit && !IsSacl && (ChildGenericControl & (SEP_ACL_PRESENT|SEP_ACL_PROTECTED)) == SEP_ACL_PRESENT ) {
    ...
    if ( (!AutoInherit && (ChildGenericControl & SEP_ACL_PRESENT) == 0 || (ChildGenericControl & SEP_ACL_DEFAULTED) != 0) || (AutoInherit && !AclProtected) ) {
        //  If there is no directory ACL, don't inherit from it.
        if ( DirectoryAcl != NULL ) {
            // If the DirectoryAcl is used, the revision of the Directory ACL is picked up.
            ...
            // If the default descriptor for the object should be ditched, because object specific ACEs were inherited from the directory, ditch them now.
            if ( DefaultDescriptorForObject && ChildNewAclSize != 0 && ObjectAceInherited && !AclOverflowed ) {
                ...
                RtlMoveMemory( FirstAce( AclBuffer ), ChildAcePosition, (ULONG)(((PUCHAR)DirectoryAcePosition) - (PUCHAR)ChildAcePosition) );

                // Adjust the ACE count to remove the deleted ACEs
                ((PACL)AclBuffer)->AceCount -= ChildAceCount;

                // Save the number of bytes of the Child ACL that were actually used.
                UsedChildNewAclSize = 0;
            }
        }
    }

    // If this routine didn't build the ACL, tell the caller.
    if ( DirectoryNewAclSize + UsedChildNewAclSize == 0) {
        // If the ACL was not explicitly assigned, tell the caller to default the ACL.
    ...
    // BUGBUG: The caller has to allocate a buffer large enough for ChildNewAclSize rather than UsedChildNewAclSize.  Due to the nature of my algorithm above.
...
Return Value:
    ...
    STATUS_BAD_INHERITANCE_ACL - Indicates the acl built was not a valid ACL.
        This can becaused by a number of things.  One of the more probable causes is the replacement of a CreatorId with an SID that didn't fit into the ACE or ACL.
    STATUS_UNKNOWN_REVISION - Indicates the source ACL is a revision that is unknown to this routine.
--*/
{
...
/*++
Routine Description:
    This is a private routine that checks if the input ace is inheritable and produces 0, 1, or 2 inherited aces in the given buffer.
Arguments:
    OldAce - Supplies the ace being inherited
    ...
    GenericMapping - Specifies the generic mapping to use
    NewObjectType - Type of object being inherited to.  If not specified, the object has no object type.
    NewAceLength - Receives the length (number of bytes) needed to allow for the inheritance of the specified ACE.  This might be zero.
    NewAcl - Provides a pointer to the ACL into which the ACE is to be inherited.
    NewAceExtraLength - Receives a length (number of bytes) temporarily used in the ACL for the inheritance ACE.  This might be zero
    ObjectAceInherited - Returns true if one or more object ACEs were inherited based on NewObjectType
Return Value:
    STATUS_SUCCESS - The ACE was inherited successfully.
    STATUS_BAD_INHERITANCE_ACL - Indicates something went wrong preventing the ACE from being inherited.  This generally represents a bugcheck situation when returned from this call.
