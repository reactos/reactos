...
Arguments:
    ControlPc - Supplies the address of an instruction within the specified function.
Return Value:
    If there is no entry in the function table for the specified PC, then
    NULL is returned. Otherwise, the address of the function table entry that corresponds to the specified PC is returned.
--*/
{
    PRUNTIME_FUNCTION FunctionEntry;
    ...
    ImageBase = RtlPcToFileHeader((PVOID)ControlPc, &ImageBase);// Search for the image that includes the specified PC value.

    // If an image is found that includes the specified PC, then locate the function table for the image.
    if (ImageBase != NULL) {
        FunctionTable = (PRUNTIME_FUNCTION)RtlImageDirectoryEntryToData(ImageBase, TRUE, IMAGE_DIRECTORY_ENTRY_EXCEPTION, &SizeOfExceptionTable);
        ...
        if (FunctionTable != NULL) {
            ...
            while (High >= Low) {
                ...
                // is greater than of equal to the beginning address and less than the ending address of the function table entry, then
                // return the address of the function table entry. Otherwise, continue the search.
                ...
                if (ControlPc < FunctionEntry->BeginAddress) {
                    ...
                    // ending address is the address of a function table entry that accurately describes the ending prologue address.
                    if ((FunctionEntry->PrologEndAddress < FunctionEntry->BeginAddress) || (FunctionEntry->PrologEndAddress > FunctionEntry->EndAddress)) {
    ...
    return NULL;// A function table entry for the specified PC was not found.
}
...
--*/
{
    ...
    // routine, set the fault instruction address to that of the caller, and call the raise exception system service.
...
Routine Description:
    ...
    The machine state at the time of the call to unwind is captured in a context record
    ...
    scan through the procedure call frames is then performed to find the target of the unwind operation.

    As each frame is encounter, the PC where control left the corresponding
    function is determined and used to lookup exception handler information
    in the runtime function table built by the linker. If the respective routine has an exception handler, then the handler is called.
...
--*/
{
    ...
    RtlUnwind2(TargetFrame, TargetIp, ExceptionRecord, ReturnValue, &ContextRecord);// Call real unwind routine specifying a context record as an extra argument.
}
...
--*/
{
    ...
    do {
        ...
        if (FunctionEntry != NULL) {
            NextPc = RtlpVirtualUnwind(ControlPc, FunctionEntry, ContextRecord, &InFunction, &EstablisherFrame, NULL);
            ...
            // check to determine if the current routine has an exception handler.
            if ((EstablisherFrame < LowLimit) || (EstablisherFrame > HighLimit) ||
