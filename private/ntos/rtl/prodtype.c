...
BOOLEAN RtlGetNtProductType(OUT PNT_PRODUCT_TYPE    NtProductType)
/*++
Routine Description:
    Returns the product type of the current system.
Arguments:
    NtProductType - Returns the product type.  Either NtProductWinNt or NtProductLanManNt.
Return Value:
...
--*/
{
    ...
    // if we are in gui setup mode, product type is read from the registry since gui setup mode is the only time product type can be changed.
    ...
    *NtProductType = NtProductWinNt;
    ...
    if (NT_SUCCESS( Status )) {
        ...
        if (KeyValueInformation == NULL) {
            ...
            Status = NtQueryValueKey( KeyHandle, &ValueName, KeyValueFullInformation, KeyValueInformation, KeyValueInfoLength, &ResultLength);
        }
    } else {
        KeyHandle = NULL;
    }
