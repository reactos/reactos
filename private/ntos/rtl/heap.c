...
//  If any of these flags are set, the fast allocator punts to the slow do-everything allocator.
...
*/
{
    ...
    if (RtlpDebugPageHeap && (HeapBase == NULL) && (Lock == NULL)) {
        ...
        PageHeap = RtlpDebugPageHeapCreate(Flags, HeapBase, ReserveSize, CommitSize, Lock, Parameters);
        if (PageHeap != NULL) {
            return PageHeap;
        }

        // A `-1' value signals a recursive call from page heap manager.
        // We set this to null and continue creating a normal heap.
        // This small hack is required so that we minimize the dependencies between the normal and the page heap manager.
    ...
    //  The maximum heap block size is really 0x7f000 which is 0x80000 minus a page.
    // Maximum block size is 0xfe00 and granularity shift is 3.
    MaximumHeapBlockSize = HEAP_MAXIMUM_BLOCK_SIZE << HEAP_GRANULARITY_SHIFT;

    Status = STATUS_SUCCESS;//  Assume we're going to be successful until we're shown otherwise

    //  This part of the routine builds up local variable containing all the parameters used to initialize the heap.
    RtlZeroMemory(&TempParameters, sizeof(TempParameters));//  First thing we do is zero it out.
    ...
    Parameters = &TempParameters;//  Set the parameter block to the local copy
    ...
    if (!NT_SUCCESS(ZwQuerySystemInformation(SystemBasicInformation, &SystemInformation, sizeof(SystemInformation), NULL))) {
        return NULL;
    }
    ...
    //  Set the virtual memory threshold to be non zero and not more than the maximum heap block size of 0x7f000.
    //  If the user specified one that is too large we automatically and silently drop it down.
    ...
    //  The default commit size is one page and the default reserve size is 64 pages.
    //  **** this doesn't check that commit size if specified is less than reserved size if specified
    ...
    //  Compute the size of the heap which will be the heap struct itself and if we are to seralize with out own lock then add room for the lock.
    //  If the user did not supply the lock then set the lock variable to -1.
    ...
    if (ARGUMENT_PRESENT(HeapBase)) {
        ...
        if (Parameters->CommitRoutine != NULL) {
            //  The caller specified a commit routine so he caller also needs to have given us certain parameters and make sure the heap is not growable.
         ...
         //  So query the base to get its size
            ...
            CommittedBase = MemoryInformation.BaseAddress;//  Set our commit base to the start of the range
            if (MemoryInformation.State == MEM_COMMIT) {//  If the memory is commmitted then we can zero out a page worth
                RtlZeroMemory(CommittedBase, PAGE_SIZE);

                //  Set the commit size and uncommited base according to the start of the vm
        ...
        //  This user gave us a base and we've just taken care of the committed bookkeeping.
        ...
        SegmentFlags = 0;//  Indicate that this segment is not user supplied
    ...
    if (CommittedBase == UnCommittedBase) {
        ...
        UnCommittedBase = (PVOID)((PCHAR)UnCommittedBase + CommitSize);//  The new uncommitted base is not adjusted above what we just committed
    }
    ...
    //  Calculate the end of the heap header and make room for 8 uncommitted range structures.
    //  Once we have the room for them then chain them together and null terminate the chain
    ...
    *pp = NULL;
    ...
    //  Check if tagging is enabled in global flags.  This check is always true in a debug build.

    //  If tagging is enabled then make room for 129 pseudo tag heap entry.
    //  Which is one more than the number of free lists.
    //  Also point the heap header to this array of pseudo tags entries.
    if (IS_HEAP_TAGGING_ENABLED()) {
        ...
        //  **** this advancement of the next heap address doesn't seem right given that a pseudo heap entry is 12 ulongs in length and not a single byte
    ...
    //  If the sizeof the heap header is larger than the native page size, you have a problem.
    //  Further, if the CommitSize passed in was smaller than the SizeOfHeapHeader, you may not even make it this far before death...
    ...
    InitializeListHead(&Heap->VirtualAllocdBlocks);//  Make it so that there a no big block allocations

    //  Initialize the cricital section that controls access to the free list.
    //  If the lock variable is -1 then the caller did not supply a lock so we need to make room for one and initialize it.
    ...
    Heap->VirtualMemoryThreshold = (ULONG)(ROUND_UP_TO_POWER2(Parameters->VirtualMemoryThreshold, HEAP_GRANULARITY) >> HEAP_GRANULARITY_SHIFT);
    Heap->CommitRoutine = Parameters->CommitRoutine;

    //  We either align the heap at 16 or 8 byte boundaries.
    //  The AlignRound and AlignMask are used to bring allocation sizes up to the next boundary.
    //  The align round includes the heap header and the optional check tail size
    ...
    //  Initialize the heap lookaide lists.
    //  This is only for the user mode heap and the heap contains a pointer to the lookaside list array.
    //  The array is sized the same as the dedicated free list.
    //  First we allocate space for the lookaside list and then we initialize each lookaside list.

    //  But the caller asked for no serialize or asked for non growable heap then we won't enable the lookaside lists.
    Heap->Lookaside = NULL;
    Heap->LookasideLockCount = 0;
    if ((!(Flags & HEAP_NO_SERIALIZE)) && ((Flags & HEAP_GROWABLE)) && (!(RtlpDisableHeapLookaside))) {
        ULONG i;

        Heap->Lookaside = RtlAllocateHeap(Heap, Flags, sizeof(HEAP_LOOKASIDE) * HEAP_MAXIMUM_FREELISTS);
    ...
    return (PVOID)Heap;//  And return the fully initialized heap to our caller
}
...
Routine Description:
    ...
    It tears down an existing heap structure.
Arguments:
    HeapHandle - Supplies a pointer to the heap being destroyed
Return Value:
    PVOID - Returns null if the heap was destroyed completely and a pointer back to the heap if for some reason the heap could not be destroyed.
*/
{
    ...
    return NULL;//  And we return to our caller
}
...
/*++
Routine Description:
    This routine allocates a memory of the specified size from the specified heap.
Arguments:
...
*/
{
    ...
    Flags |= Heap->ForceFlags;//  Take the callers flags and add in the flags that we must forcibly set in the heap

    //  Check for special features that force us to call the slow, do-everything version.
    //  We do everything slow for any of the following flags.
    ...
    //  Note that if the request is for 0 bytes, we still allocate memory, because we add in an extra 1 byte to protect ourselves from idiots.
    ...
    //  This will take care of the condition where a walk or lock heap can cause us to check for a non null pointer and then have it become null when we read it again.
    //  If it is non null to start with then even if the user walks or locks the heap via another thread the pointer to still valid here so we can still try and do a lookaside list pop.
    ...
    try {
        if (!(Flags & HEAP_NO_SERIALIZE)) {//  Check if we need to serialize our access to the heap
            RtlAcquireLockRoutine(Heap->LockVariable);//  Lock the free list.
            LockAcquired = TRUE;
        }

        //  If the allocation index is less than the maximum free list size
        //  then we can use the index to check the free list otherwise we have to either pull the entry off of the [0] index list or allocate memory directly for this request.
        if (AllocationIndex < HEAP_MAXIMUM_FREELISTS) {
            ...
            if (!IsListEmpty(FreeListHead)) {
                ...
                Heap->TotalFreeSize -= AllocationIndex;//  Adjust the total number of bytes free in the heap

                //  Mark the block as busy and and set the number of bytes unused and tag index.
                //  Also if it is the last entry then keep that flag.
                BusyBlock = (PHEAP_ENTRY)FreeBlock;
                BusyBlock->Flags = HEAP_ENTRY_BUSY | (FreeFlags & HEAP_ENTRY_LAST_ENTRY);
                BusyBlock->UnusedBytes = (UCHAR)(AllocationSize - Size);
                BusyBlock->SmallTagIndex = 0;
            } else {//  The free list that matches our request is empty
    ...
    SplitFreeBlock:
                ...
                if (FreeSize != 0) {
                    //  But first we won't ever bother doing a split that only gives us 8 bytes back.
                    ...
                    if (FreeSize == 1) {
                        ...
                        if (FreeFlags & HEAP_ENTRY_LAST_ENTRY) {
                            ...
                            if (SplitBlock2->Flags & HEAP_ENTRY_BUSY) {
                                ...
                                RtlpFastRemoveFreeBlock(Heap, SplitBlock2);//  Removing the second block from its free list
                                ...
                                if (FreeSize <= HEAP_MAXIMUM_BLOCK_SIZE) {
                                    ...
                                    //  Again check if the new following block exists and if so then updsate is previous size
                                    ...
                                    RtlpFastInsertFreeBlockDirect(Heap, SplitBlock, (USHORT)FreeSize);
                                    Heap->TotalFreeSize += FreeSize;
                                } else {//  The new free block is pretty large so we need to call a private routine to do the insert
                                    RtlpInsertFreeBlock(Heap, SplitBlock, FreeSize);
                                }
                            }
                        }

                        FreeFlags = 0;//  Now that free flags made it back into a free block we can zero out what we saved.

                        if (SplitBlock->Flags & HEAP_ENTRY_LAST_ENTRY) {//  If splitblock now last, update LastEntryInSegment
                            PHEAP_SEGMENT Segment;
                ...
                if (FreeFlags & HEAP_ENTRY_LAST_ENTRY) {//  If there are no following entries then mark the new block as such
            ...
            if (Flags & HEAP_ZERO_MEMORY) {//  If the flags indicate that we should zero memory then do it now
                RtlZeroMemory(ReturnValue, Size);
            }

            leave;//  And return the allocated block to our caller
    ...
    LookInNonDedicatedList:
            ...
            if (FreeListHead != Next) {
                ...
                if (FreeBlock->Size >= AllocationIndex) {
                    //  Here we are sure there is at least a block here larger than the requested size. Start searching from the first block
                    Next = FreeListHead->Flink;
                    while (FreeListHead != Next) {
                        FreeBlock = CONTAINING_RECORD(Next, HEAP_FREE_ENTRY, FreeList);
                        if (FreeBlock->Size >= AllocationIndex) {
                            //  We've found something that we can use so now remove it from the free list and go to where we treat spliting a free block.
            ...
            if (FreeBlock != NULL) {//  And provided we got something we'll treat it just like the previous split free block cases
                RtlpFastRemoveNonDedicatedFreeBlock(Heap, FreeBlock);
                goto SplitFreeBlock;
            }

            Status = STATUS_NO_MEMORY;//  We weren't able to extend the heap so we must be out of memory
...
*/
{
    ...
    //  Round up the requested size to the allocation granularity.
    //  Note that if the request is for zero bytes we will still allocate memory,

    //      Allocation size will be either 16, 24, 32, ...
    //      Allocation index will be 2, 3, 4, ...
    AllocationSize = ((Size ? Size : 1) + Heap->AlignRound) & Heap->AlignMask;

    //  Generate the flags needed for this heap entry.
    //  Mark it busy and add any user settable bits.
    //  Also if the input flag indicates any entry extra fields and we have a tag to use then make room for the extra fields in the heap entry
    ...
    try {
        ...
        //  Do all the actual heap work under the protection of a try-except clause to protect us from corruption
        try {
            //  If the allocation index is less than the maximum free list size
            //  then we can use the index to check the free list otherwise we have to either pull the entry off of the [0] index list or allocate memory directly for this request.
            if (AllocationIndex < HEAP_MAXIMUM_FREELISTS) {
                ...
                if ( !IsListEmpty( FreeListHead ))  {
                    ...
                    //  We know that there are 128 free lists managed by a 4 ulong bitmap.
                    //  The next big if-else-if statement will decide which ulong we tackle
    ...
    SplitFreeBlock:
                ...
                //  If the flags indicate that we should zero memory then remember how much to zero.
                //  We'll do the zeroing later
                if (Flags & HEAP_ZERO_MEMORY) {
                    ZeroSize = Size;
    ...
    LookInNonDedicatedList:
            ...
            } else {//  Otherwise we have an error condition
                Status = STATUS_BUFFER_TOO_SMALL;
            }
    ...
    return ReturnValue;//  And return to our caller
}
...
*/
{
    ...
    //  We can do everything in this routine. 
    //  So now backup to get a pointer to the start of the block
    BusyBlock = (PHEAP_ENTRY)BaseAddress - 1;

    //  Protect ourselves from idiots by refusing to free blocks that do not have the busy bit set.
    ...
    //  If there is a lookaside list and the block is not a big allocation and the index is for a dedicated list then free the block to the lookaside list.
    //  We'll actually capture the lookaside pointer from the heap and only use the captured pointer.
    //  This will take care of the condition where a walk or lock heap can cause us to check for a non null pointer and then have it become null when we read it again.
    //  If it is non null to start with then even if the user walks or locks the heap via another thread the pointer to still valid here so we can still try and do a lookaside list push
#ifndef NTOS_KERNEL_RUNTIME
    {
        PHEAP_LOOKASIDE Lookaside = (PHEAP_LOOKASIDE)Heap->Lookaside;
    ...
    try {
        ...
        if (!(BusyBlock->Flags & HEAP_ENTRY_VIRTUAL_ALLOC)) {
            //  This block is not a big allocation so we need to to get its size, and coalesce the blocks note that the user mode heap does this conditionally on a heap flag.
            ...
            //  the total free in the heap is under the decomit threshold then we'll put this into a free list
                ...
                //  and update the heaps free space counter
                if (FreeSize <= (ULONG)HEAP_MAXIMUM_BLOCK_SIZE) {
                    ...
                    //  The block is too big to go on a free list in its entirety but we don't want to decommit anything so
                    //  simply call a worker routine to hack up the block into pieces that will fit on the free lists.
                    RtlpInsertFreeBlock(Heap, (PHEAP_FREE_ENTRY)BusyBlock, FreeSize);
                }
            } else {//  Otherwise the block is to big for any lists and we should decommit the block
    ...
    return ReturnValue;//  The block was freed successfully so return success to our caller
}
...
Routine Description:
    This routine returns a previously allocated block back to its heap.
    It is the slower version of Rtl Free Heap and does more checking and tagging control.
Arguments:
    HeapHandle - Supplies a pointer to the owning heap structure
    Flags - Specifies the set of flags to use in the deallocation
    BaseAddress - Supplies a pointer to the block being freed
...
*/
{
    ...
    Result = FALSE;//  Until we figure out otherwise we'll assume that this call will fail
    ...
    try {
        ...
        try {
            ...
            //  Note that this test is just opposite from the test used in Rtl Free Heap
            if ((BusyBlock->Flags & HEAP_ENTRY_BUSY) && (((ULONG_PTR)BaseAddress & 0x7) == 0) && (BusyBlock->SegmentIndex < HEAP_MAXIMUM_SEGMENTS)) {
                ...
                if (BusyBlock->Flags & HEAP_ENTRY_VIRTUAL_ALLOC) {
                    ...
                    //  This block is not a big allocation so we need to to get its size,
                    ...
                    if ((FreeSize < Heap->DeCommitFreeBlockThreshold) || ((Heap->TotalFreeSize + FreeSize) < Heap->DeCommitTotalFreeThreshold)) {
                        //  Check if the block can fit on one of the dedicated free lists
                        if (FreeSize <= (ULONG)HEAP_MAXIMUM_BLOCK_SIZE) {
                            ...
                            //  If there is a following entry then make sure the sizes agree
                            if (!(BusyBlock->Flags & HEAP_ENTRY_LAST_ENTRY)) {
                                HEAPASSERT((BusyBlock + FreeSize)->PreviousSize == (USHORT)FreeSize);
                            }
...
Routine Description:
    ...
    The size only includes the number of bytes the original caller used to allocate the block and not any unused bytes at the end of the block.
Arguments:
    HeapHandle - Supplies a pointer to the heap that owns the block being queried
    Flags - Supplies a set of flags used to allocate the block
    BaseAddress - Supplies the address of the block being queried
Return Value:
    SIZE_T - returns the size, in bytes, of the queried block, or -1 if the block is not in use.
*/
{
    PHEAP Heap = (PHEAP)HeapHandle;
    PHEAP_ENTRY BusyBlock;
    ...
    //  No lock is required since nothing is modified and nothing outside the busy block is read.
    //  Backup to get a pointer to the heap entry
    BusyBlock = (PHEAP_ENTRY)BaseAddress - 1;
    ...
    return BusySize;//  And return to our caller
}
...
Arguments:
    ...
    Flags - Supplies a set of heap flags to compliment those already set in the heap
Return Value:
        ...
        try {
            ...
            for (SegmentIndex=0; SegmentIndex<HEAP_MAXIMUM_SEGMENTS; SegmentIndex++) {
                ...
                while (CurrentBlock < Segment->LastValidEntry) {
                    ...
                    //  If the following entry is uncommited then we need to skip over it.
                    //  This code strongly implies that the uncommitted range list is in perfect sync with the blocks in the segement
                    if (CurrentBlock->Flags & HEAP_ENTRY_LAST_ENTRY) {
                        CurrentBlock += CurrentBlock->Size;

                        //  Check if the we've reached the end of the segment and should just break out of the while loop
...
Routine Description:
    ...
    This routine works by essentially doing a pop of the stack of unused uncommitted range structures located off the heap structure.
    If the stack is empty then we'll create some more before doing the pop.
Arguments:
    Segment - Supplies the heap segment being modified
Return Value:
    PHEAP_UNCOMMITTED_RANGE - returns a pointer to the newly created uncommitted range structure
*/
{
    NTSTATUS Status;
    ...
    pp = &Segment->Heap->UnusedUnCommittedRanges;//  Get a pointer to the unused uncommitted range structures for the specified heap

    //  If the list is null then we need to allocate some more to put on the list
    if (*pp == NULL) {
        UCRSegment = Segment->Heap->UCRSegments;//  Get the next uncommitted range segment from the heap

        //  If there are no more uncommitted range segments or the segemtns commited and reserved sizes are equal (meaning
        //  it's all used up) then we need to allocate another uncommitted range segment
        if ((UCRSegment == NULL) || (UCRSegment->CommittedSize == UCRSegment->ReservedSize)) {
            //  We'll reserve 16 pages of memory and commit at this time one page of it.
            ReserveSize = PAGE_SIZE * 16;
            UCRSegment = NULL;

            Status = ZwAllocateVirtualMemory(NtCurrentProcess(), &UCRSegment, 0, &ReserveSize, MEM_RESERVE, PAGE_READWRITE);
            if (!NT_SUCCESS(Status)) {
                return NULL;
            }

            CommitSize = PAGE_SIZE;
            Status = ZwAllocateVirtualMemory(NtCurrentProcess(), &UCRSegment, 0, &CommitSize, MEM_COMMIT, PAGE_READWRITE);
            if (!NT_SUCCESS(Status)) {
                ZwFreeVirtualMemory(NtCurrentProcess(), &UCRSegment, &ReserveSize, MEM_RELEASE);
                return NULL;
            }
            ...
            FirstEntry = (PCHAR)(UCRSegment + 1);//  Point to the first free spot in the segment
        } else {
            //  We have an existing UCR segment with available space
            //  So now try and commit another PAGE_SIZE bytes.
            //  When we are done FirstEntry will point to the newly committed space
            CommitSize = PAGE_SIZE;
            FirstEntry = (PCHAR)UCRSegment + UCRSegment->CommittedSize;
            Status = ZwAllocateVirtualMemory(NtCurrentProcess(), &FirstEntry, 0, &CommitSize, MEM_COMMIT, PAGE_READWRITE);
            if (!NT_SUCCESS(Status)) {
                return NULL;
            }

            UCRSegment->CommittedSize += CommitSize; //  And update the amount committed in the segment
        }

        //  At this point UCR segment exists and First Entry points to the start of the available committed space.
        //  We'll make Last Entry point to the end of the committed space
        ...
        *pp = NULL;//  Null terminate the list
        pp = &Segment->Heap->UnusedUnCommittedRanges;//  And have Pp point the new top of the list
    }

    //  At this point the Pp points to a non empty list of unused uncommitted range structures.
    //  So we pop the list and return the top to our caller
    UnCommittedRange = *pp;
    *pp = UnCommittedRange->Next;
...
Arguments:
    Segment - Supplies any segment in the heap being modified.  Most likely but not necessarily the segment containing the uncommitted range structure
    UnCommittedRange - Supplies a pointer to the uncommitted range structure being decommissioned.
*/
{
    RTL_PAGED_CODE();

    //  This routine simply does a "push" of the uncommitted range structure onto the heap's stack of unused uncommitted ranges
    UnCommittedRange->Next = Segment->Heap->UnusedUnCommittedRanges;
    Segment->Heap->UnusedUnCommittedRanges = UnCommittedRange;

    //  For safety sake we'll also zero out the fields in the decommissioned structure
...
Routine Description:
    ...
    When done the information will hang off the segments uncommitted ranges list.
Arguments:
...
*/
{
    ...
    //  While we haven't reached the end of the list we'll zoom through trying to find a fit
    while (UnCommittedRange = *pp) {
        //  If address we want is less than what we're pointing at then we've found where this new entry goes
        if (UnCommittedRange->Address > Address) {
            //  If the new block matches right up to the existing block then we can simply backup the existing block and add to its size
            if ((Address + Size) == UnCommittedRange->Address) {
                UnCommittedRange->Address = Address;
                UnCommittedRange->Size += Size;

                //  Check if we need to update our notion of what the largest uncommitted range is
            ...
            //  So now fall out to where the insertion takes place.
            break;
        } else if ((UnCommittedRange->Address + UnCommittedRange->Size) == Address) {//  Otherwise if this existing block stops right where the new block starts then we get to modify this entry.
        ...
        } else {//  Otherwise we'll continue search down the list
            pp = &UnCommittedRange->Next;
        }
    }

    //  If we reach this point that means we've either fallen off the end of the list, or the list is empty, or we've located the spot where a new uncommitted range structure belongs.
...
Routine Description:
    ...
    It commits the range and returns a heap entry for the range.
Arguments:
    Heap - Supplies the heap being maniuplated
    Segment - Supplies the segment being searched
    Size - Supplies the size of what we need to look for, on return it contains the size of what we're just found and committed.
    AddressWanted - Optionally gives an address where we would like the pages based.  If supplied the entry must start at this address
Return Value:
    PHEAP_FREE_ENTRY - Returns a pointer to the newly committed range that satisfies the given size requirement,
                       or NULL if we could not find something large enough and/or based at the address wanted.
*/
{
    NTSTATUS Status;
    ...
    //  What the outer loop does is cycle through the uncommited ranges stored in in the specified segment
    ...
    while (UnCommittedRange = *pp)
    {
        ...
        if ((UnCommittedRange->Size >= *Size) && (!ARGUMENT_PRESENT(AddressWanted) || (UnCommittedRange->Address == (ULONG_PTR)AddressWanted))) {
            ...
            //  We want last entry to point to the last real entry before this newly committed spot.
                ...
                (ULONG_PTR)(Segment->LastEntryInSegment + Segment->LastEntryInSegment->Size) == UnCommittedRange->Address)
            {
                ...
                //  Now we zoom through the entries until we find the one marked last
                while (!(LastEntry->Flags & HEAP_ENTRY_LAST_ENTRY)) {
                    ...
                    if (((PCHAR)LastEntry >= (PCHAR)Segment->LastValidEntry) || (LastEntry->Size == 0)) {
                        //  Check for the situation where the last entry in the segment isn't marked as a last entry but does put us right where the have a new committed range
            ...
            //  Turn off the last bit on this entry because what's following is no longer uncommitted
            ...
            //  Now if the size is zero then we've committed everything that there was in the range.
            //  Otherwise make sure the first entry of what we've just committed knows that an uncommitted range follows.
            if (UnCommittedRange->Size == 0) {
                //  This uncommitted range is about to vanish.
                //  Base on if the range is the last one in the segemnt then we know how to mark the committed range as being last or not.
            ...
            //  Now if we adjusted the largest uncommitted range to zero then we need to go back and find the largest uncommitted range
            //  To do that we simply zoom down the uncommitted range list remembering the largest one
            ...
            return (PHEAP_FREE_ENTRY)FirstEntry;//  And return the heap entry to our caller
        } else {
            //  Otherwise the current uncommited range is too small or doesn't have the right address so go to the next uncommitted range entry
            PreviousUnCommittedRange = UnCommittedRange;
            pp = &UnCommittedRange->Next;
        }
    }

    //  At this point we did not find an uncommitted range entry that satisfied our requirements either because of size and/or address.
    //  So return null to tell the user we didn't find anything.
    return NULL;
}
...
*/
{
    ...
    //  At this point we know there is enough memory committed to handle the segment header and one heap entry

    //  Now compute the number of uncommited pages and the number of committed pages
    ...
    //  In the non kernel x86 case see if we need to capture the callers stack backtrace
    if (NtGlobalFlag & FLG_USER_STACK_TRACE_DB) {
...
Routine Description:
    ...
    After the call it is as if the segment never existed
Arguments:
...
*/
{
    ...
    //  We actually only have work to do if the segment is not user allocated.
    //  If the segement is user allocated then we'll assume knows how to get rid of the memory
...
*/
{
    ...
    //  At this point we weren't able to get the memory from an existing heap segment so now check if we found an unused segment index and if we're alowed to grow the heap.
    ...
    return NULL;//  Either the heap cannot grow or we out of resources of some type so we're going to return null
}
...
Arguments:
    ...
    FreeSize - Supplies the size, in bytes, of the free block.  On return it contains the size, in bytes, of the of the newly coalesced free block
    RemoveFromFreeList - Indicates if the input free block is already on a free list and needs to be removed to before coalescing
Return Value:
...
*/
{
    ...
    if ((FreeBlock1 != FreeBlock) && !(FreeBlock1->Flags & HEAP_ENTRY_BUSY) && ((*FreeSize + FreeBlock1->Size) <= HEAP_MAXIMUM_BLOCK_SIZE)) {
        ...
        *FreeSize += FreeBlock1->Size;
        ...
        //  Check if we need to update the previous size of the next entry
    ...
    return FreeBlock;//  And return the free block to our caller
}
...
Routine Description:
    ...
    This is usually called because the block is beyond the decommit threshold
Arguments:
    Heap - Supplies a pointer to the heap being manipulated
...
*/
{
    ...
    //  The leading free block is used to identify whatever space is needed to round up the callers specified address to a page address.
    //  If the caller already gave us a page aligned address then the free block address is identical to what the caller supplied.
    ...
    //  Otherwise if we have a preceding block and the leading free size is zero then identify the preceding block as the leading busy block
    ...
    //  The trailing busy block identifies the block immediately after the one we are trying to decommit provided what we are decommitting ends right
    //  on a page boundary otherwise the trailing busy block stays null and the trailing free block value is used.
    ...
    //  Now adjust the trailing free block to compensate for the trailing free size we just computed.
    TrailingFreeBlock = (PHEAP_FREE_ENTRY)((PHEAP_ENTRY)TrailingFreeBlock - TrailingFreeSize);

    //  Right now DeCommit size is really a pointer.
    //  If it points at is beyond the decommit address then make the size really be just the byte count to decommit.
    //  Otherwise the decommit size is zero.
    ...
    if (DeCommitSize != 0) {
        ...
        if (NT_SUCCESS( Status )) {
            ...
            //  If we have a leading free block then mark its proper state update the heap, and put it on the free list
            ...
            //  If there is a trailing free block then sets its state, update the heap, and insert it on a free list
...
Routine Description:
    ...
    If necessary this routine will divide up the free block to sizes that fit on the free list
Arguments:
...
*/
{
    ...
    while (FreeSize != 0) {
        ...
        if (FreeSize > (ULONG)HEAP_MAXIMUM_BLOCK_SIZE) {
            ...
            FreeBlock->Flags = 0;//  Guarantee that Last entry does not get set in this block.
        } else {
            Size = (USHORT)FreeSize;

            FreeBlock->Flags = Flags;//  This could propagate the last entry flag
        }
        ...
        //  Check if we're done with the free block based on the segment information, otherwise go back up and check size
        //  Note that is means that we can get called with a very large size and still work.
        if ((PHEAP_ENTRY)FreeBlock >= Segment->LastValidEntry) {
            return;
        }
    }

    //  If the block we're freeing did not think it was the last entry then tell the next block our real size.
...
Routine Description:
    ...
    The caller must have already checked that the entry extry field is present
Arguments:
...
Return Value:
    SIZE_T - Returns the size, in bytes, that was allocated to the big block
*/
{
    PHEAP_VIRTUAL_ALLOC_ENTRY VirtualAllocBlock;
...
Routine Description:
    ...
    It does this by checking for a tail fill pattern
Arguments:
...
*/
{
    ...
    //  Compute a pointer to the tail of the input block.
    //  This would be the space right after the user allocated portion
