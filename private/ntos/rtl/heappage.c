...
Abstract:
    Implementation of NT RtlHeap family of APIs for debugging applications with heap usage bugs.
    Each allocation returned to the calling app is placed at the end of a virtual page such that the following virtual page is protected (ie, NO_ACCESS).
    So, when the errant app attempts to reference or modify memory beyond the allocated portion of a heap block, an access violation is immediately caused.
    This facilitates debugging the app because the access violation occurs at the exact point in the app where the heap corruption or abuse would occur.
    Note that significantly more memory (pagefile) is required to run an app using this heap implementation as opposed to the retail heap manager.
...
//  Remainder of entire file is wrapped with #ifdef DEBUG_PAGE_HEAP so that it will compile away to nothing if DEBUG_PAGE_HEAP is not defined in heappage.h
...
// value (system wide or per process one) and checking if the FLG_HEAP_PAGE_ALLOCS is set.
BOOLEAN RtlpDebugPageHeap;

// Internal version used to figure out what are people running in various VBLs.
PCHAR RtlpDphVersion = "03/14/2000";


// Page heaps list manipulation.

// We maintain a list of all page heaps in the process to support APIs like GetProcessHeaps.
// The list is also useful for debug extensions that need to iterate the heaps.
// The list is protected by RtlpDphHeapListCriticalSection lock.
...
// The initial value is so that by default we use page heap only with normal allocations.
// This way if system wide global flag for page heap is set the machine will still boot.
// After that we can enable page heap with "sudden death" for specific processes.
// The most useful flags for this case would be:

//    PAGE_HEAP_ENABLE_PAGE_HEAP       |
//    PAGE_HEAP_COLLECT_STACK_TRACES   ;

// If no flags specified the default is page heap light with stack trace collection.
...
// Process wide trace database and the maximum size it can grow to.
...
// This header information prefixes both the normal and page heap blocks.
...
// RtlpDphTargetDllsLoadCallBack is called in ntdll\ldrapi.c (LdrpLoadDll) whenever a new dll is loaded in the process space.
    ...
    if (Flags & HEAP_NO_SERIALIZE) {
        if (!RtlTryEnterCriticalSection(HeapRoot->HeapCritSect)) {
            if (HeapRoot->nRemoteLockAcquired == 0) {
                ...
                //  bug since multithreaded access to heap was attempted with the HEAP_NO_SERIALIZE flag specified.
...
BOOLEAN RtlpDebugPageHeapRobustProtectVM(IN PVOID   VirtualBase, IN SIZE_T  VirtualSize, IN ULONG   NewAccess, IN BOOLEAN Recursion)
{
    ...
    if (!Recursion) {
        ...
        //  one adjancent blocks allocated by separate calls to ZwAllocateVirtualMemory.
        //  It also seems fails occasionally
...
INLINE BOOLEAN RtlpDebugPageHeapReleaseVM(IN PVOID pVirtual)
{
    ...
    return NT_SUCCESS(ZwFreeVirtualMemory(NtCurrentProcess(), &pVirtual, &nSize, MEM_RELEASE));
}
...
INLINE BOOLEAN RtlpDebugPageHeapCommitVM(IN PVOID pVirtual, IN SIZE_T nSize)
{
    ...
    if (Failed) {
        ...
        for (pCurrent -= PAGE_SIZE; pCurrent >= pStart && pCurrent < pEnd; pCurrent -= PAGE_SIZE) {
            CommitSize = PAGE_SIZE;
            Status = ZwFreeVirtualMemory(NtCurrentProcess(), &pCurrent, &CommitSize, MEM_DECOMMIT);
            if (!NT_SUCCESS(Status)) {
                // There is now valid reason known to me for a correct free operation failure.
                // So, in this case we make a little bit of fuss about it.
...
INLINE BOOLEAN RtlpDebugPageHeapDecommitVM(IN PVOID pVirtual, IN SIZE_T nSize)
{
    ...
    for (pCurrent = pStart; pCurrent < pEnd; pCurrent += PAGE_SIZE) {
        ...
        Status = ZwFreeVirtualMemory(NtCurrentProcess(), &pCurrent, &DecommitSize, MEM_DECOMMIT);
        if (!NT_SUCCESS(Status)) {
...
VOID RtlpDebugPageHeapRemoveFromAvailableList(
    IN PDPH_HEAP_ROOT  pHeap,
...
VOID RtlpDebugPageHeapCoalesceNodeIntoAvailable(IN PDPH_HEAP_ROOT pHeap, IN PDPH_HEAP_BLOCK pNode)
{
    ...
    if (pNext) {
        if ((pVirtual + nVirtual) == pNext->pVirtualBlock) {
            //  pNode and pNext are adjacent, so simply add size of pNext entry to pNode entry and remove pNext entry from the list.
...
)
{
    ...
    // If we use uncommitted ranges it is really important to call FindAvailableMemory only with page aligned sizes.
    ...
    if ((pAvail == NULL) && (bGrowVirtual)) {
        ...
        //  enough memory (large enough block) to satisfy request, so we need to allocate more VM.
...
PDPH_HEAP_BLOCK RtlpDebugPageHeapAllocateNode(IN PDPH_HEAP_ROOT pHeap)
{
    ...
    if (pHeap->pUnusedNodeListHead == NULL) {
        ...
        //  is still empty before we try to use or allocate more memory.
        ...
        if ((pHeap->pUnusedNodeListHead == NULL) && (pNode == NULL)) {
            ...
            //  we can find at least a page on the available list.
        ...
        if (pHeap->pUnusedNodeListHead == NULL) {
            if (pNode == NULL) {
                //  Insufficient memory on Available list.  Try allocating a new virtual block.
                nRequest = POOL_SIZE;
                nVirtual = RESERVE_SIZE;
                pVirtual = RtlpDebugPageHeapAllocateVM(nVirtual);
                if (pVirtual == NULL) {
                    //  Unable to allocate full RESERVE_SIZE block,
                    //  so reduce request to single VM unit (64K) and try again.
            ...
            if (pNode == NULL) {
                ...
                if (pNode->nVirtualBlockSize > nRequest) {
                    ...
                    //  Used up entire available block -- return node to unused list.
                    RtlpDebugPageHeapReturnNodeToUnusedList(pHeap, pNode);
                }
            }
        }
...
VOID RtlpDebugPageHeapProtectStructures(IN PDPH_HEAP_ROOT pHeap)
{
    ...
    //  Assume CritSect is owned so we're the only thread twiddling the protection.
    ...
    if (--pHeap->nUnProtectionReferenceCount == 0) {
        ...
        while (pNode != NULL) {
            RtlpDebugPageHeapProtectVM(pNode->pVirtualBlock, pNode->nVirtualBlockSize, PAGE_READONLY);
            pNode = pNode->pNextAlloc;
        }
    }
}
...
VOID RtlpDebugPageHeapUnProtectStructures(IN PDPH_HEAP_ROOT pHeap)
{
    ...
    if (pHeap->nUnProtectionReferenceCount == 0) {
        ...
        while (pNode != NULL) {
            RtlpDebugPageHeapProtectVM(pNode->pVirtualBlock, pNode->nVirtualBlockSize, PAGE_READWRITE);
            pNode = pNode->pNextAlloc;
        }
    }
...
)
{
    ...
    // a very loose dependency between the normal and page heap manager.
    ...
    //  Parameters as we always have a growable heap with our own thresholds, etc.
    ...
    //  variable, which must always be READWRITE.  Beyond that, the remainder of the virtual allocation is placed on the available list.
    ...
    //  which we must set to include our HEAP_FLAG_PAGE_ALLOCS flag, and then we'll make the whole page read-only.
    ...
    // If the PAGE_HEAP_UNALIGNED_ALLOCATIONS bit is set in ExtraFlags we will set the HEAP_NO_ALIGNMENT flag in the HeapFlags.
    // This last bit controls if allocations will be aligned or not.
    // The reason we do this transfer is that ExtraFlags can be set from the registry whereas the normal HeapFlags cannot.
    ...
    // from where should we make allocations if random decision flag is on.
    {
    ...
    HeapRoot->NormalHeap = RtlCreateHeap(Flags, HeapBase, ReserveSize, CommitSize, Lock, (PRTL_HEAP_PARAMETERS)-1);

    RtlInitializeCriticalSection(HeapRoot->HeapCritSect);// Initialize heap lock.

    //  On the page that contains our DPH_HEAP structure, use
    //  the remaining memory beyond the DPH_HEAP structure as pool for allocating heap nodes.
    ...
    //  Make AvailableList entry containing remainder of initial VM and add to (create) the AvailableList.
    Node = RtlpDebugPageHeapAllocateNode(HeapRoot);
    DEBUG_ASSERT(Node != NULL);
    ...
    HeapRoot->CreateStackTrace = RtlpDphLogStackTrace(1);// Get heap creation stack trace.
    ...
    // the global delayed free queue for normal blocks and the trace database.
    if (!RtlpDphHeapListHasBeenInitialized) {
        ...
        RtlpDphTargetDllsLogicInitialize();// Initialize the target dlls logic
    }
...
PVOID RtlpDebugPageHeapAllocate(IN PVOID  HeapHandle, IN ULONG  Flags, IN SIZE_T Size)
{
    ...
    //  after we've acquired the critsect so we can still catch serialization problems.
    ...
    //  added to the Available list.  If that fails, the function will finally give up and return NULL.
    ...
    // page at the end. BAckward overruns flag disables smart memory usage flag.
    if ((HeapRoot->ExtraFlags & PAGE_HEAP_CATCH_BACKWARD_OVERRUNS)) {
        // nothing
    }
    else {
    ...
    if (pAvailNode->nVirtualBlockSize > nBytesAllocate) {
        //  Adjust pVirtualBlock and nVirtualBlock size of existing node in avail list.
        //  The node will still be in correct address space order on the avail list.
        //  This saves having to remove and then re-add node to avail list.
        //  Note since we're changing sizes directly, we need to adjust the avail and busy list counters manually.

        //  Note: since we're leaving at least one page on the available list, we are guaranteed that AllocateNode will not fail.
    ...
    //  storage, we'll skip the bottom 3 entries, leaving RtlAllocateHeap as the first recorded entry.
    ...
    //  that don't specify HEAP_ZERO_MEMORY, we fill the whole user block with DPH_PAGE_BLOCK_INFIX.

    if ((Flags & HEAP_ZERO_MEMORY)) {
        // SilviuC: The call below can be saved if we have a way
        // to figure out if the memory for the block was freshly
        // virtual allocated (this is already zeroed). This has an impact for large allocations.
...
BOOLEAN RtlpDebugPageHeapFree(IN PVOID HeapHandle, IN ULONG Flags, IN PVOID Address)
{
    ...
    if (Address == NULL) {
        ...
        return TRUE;// For C++ apps that delete NULL this is valid.
    }
    ...
    if (Node == NULL) {
        ...
        Success = RtlpDphNormalHeapFree(HeapRoot, Flags, Address);
        goto EXIT;
    }
    ...
    //  RtlpDebugPageHeapFree gets called from RtlDebugFreeHeap, which gets called from RtlFreeHeapSlowly, which gets called from
    //  RtlFreeHeap.  To keep from wasting lots of stack trace storage,
    //  we'll skip the bottom 3 entries, leaving RtlFreeHeap as the first recorded entry.
...
PVOID RtlpDebugPageHeapReAllocate(IN PVOID  HeapHandle, IN ULONG  Flags, IN PVOID  Address, IN SIZE_T Size)
{
    ...
    //  Validate requested size so we don't overflow while rounding up size computations.
    //  We do this after we've acquired the critsect so we can still catch serialization problems.
    ...
    if (OldNode == NULL) {
        ...
        NewAddress = RtlpDphNormalHeapReAllocate(HeapRoot, Flags, Address, Size);
        goto EXIT;
    }
    ...
    //  list pointers will change, possibly leaving our acquired Prev pointer invalid.
    ...
    if (NewAddress) {
        ...
        if (!ReallocInNormalHeap) {
            ...
            NewNode->UserFlags = (Flags & HEAP_SETTABLE_USER_FLAGS) ? (Flags & HEAP_SETTABLE_USER_FLAGS) : OldNode->UserFlags;
        }
        ...
        if (OriginalAllocationInPageHeap) {
            ...
            // leaving RtlReAllocateHeap as the first recorded entry in the freed stack trace.

            // Note. For realloc we need to do the accounting for free in the
            // trace block. The accounting for alloc is done in the real alloc operation which always happens for page heap reallocs.
    ...
    else {//  Failed to allocate a new block.  Return old block to busy list.
        if (OriginalAllocationInPageHeap) {
...
PVOID RtlpDebugPageHeapDestroy(IN PVOID HeapHandle)
{
    ...
    NormalHeap = HeapRoot->NormalHeap;// Save normal heap pointer for later.
    Node = HeapRoot->pBusyAllocationListHead;//  Walk all busy allocations and check for tail fill corruption
    ...
    //  Each time we alloc new VM, we make that the head of the of the VM list, like a LIFO structure.
    //  I think we're ok because no VM list node should be on a subsequently alloc'd
    //  VM -- only a VM list entry might be on its own memory (as is the case for the root node).
    //  We read pNode->pNextAlloc before releasing the VM in case pNode existed on that VM.
    //  I think this is safe -- as long as the VM list is LIFO and we don't do any list reorganization.
    ...
    // normal heap just about to be destroyed. Note that this is not a bug. The application freed the blocks correctly but we delayed the free operation.
    RtlpDphFreeDelayedBlocksFromHeap(HeapRoot, NormalHeap);

    // Destroy normal heap. Note that this will not make a recursive
    // call into this function because this is not a page heap and code in NT heap manager will detect this.
    ...
    DbgPrint("Page heap: process 0x%X destroyed heap @ %p (%p)\n", NtCurrentTeb()->ClientId.UniqueProcess, HeapRoot, NormalHeap);
...
SIZE_T RtlpDebugPageHeapSize(IN PVOID HeapHandle, IN ULONG Flags, IN PVOID Address)
{
    ...
    if (Node == NULL) {
        ...
        Size = RtlpDphNormalHeapSize(HeapRoot, Flags, Address);
        goto EXIT;
    }
    else {
...
ULONG RtlpDebugPageHeapGetProcessHeaps(ULONG NumberOfHeaps, PVOID *ProcessHeaps)
{
    ...
    //  Although we'd expect GetProcessHeaps never to be called before at least the very first heap creation,
    //  we should still be safe and initialize the critical section if necessary.
    if (!RtlpDphHeapListHasBeenInitialized) {
        RtlpDphHeapListHasBeenInitialized = TRUE;
        RtlInitializeCriticalSection(&RtlpDphHeapListCriticalSection);
    }
    ...
    if (RtlpDphHeapListCount <= NumberOfHeaps) {
        for (HeapRoot = RtlpDphHeapListHead, Count = 0; HeapRoot != NULL; HeapRoot = HeapRoot->pNextHeapRoot, Count += 1) {
        ...
        //  Return number of entries necessary for subsequent call to succeed.
...
ULONG RtlpDebugPageHeapCompact(IN PVOID HeapHandle, IN ULONG Flags)
{
    ...
    //  in case this was called with HEAP_NO_SERIALIZE while another thread is in the heap code.
...
RtlpDphShouldAllocateInPageHeap(PDPH_HEAP_ROOT HeapRoot, SIZE_T Size)
{
    ...
    if (!(HeapRoot->ExtraFlags & PAGE_HEAP_ENABLE_PAGE_HEAP)) {// If page heap is not enabled => normal heap.
        return FALSE;
    }
    else if ((HeapRoot->ExtraFlags & PAGE_HEAP_USE_SIZE_RANGE)) {// If in size range => page heap
    ...
    else if ((HeapRoot->ExtraFlags & PAGE_HEAP_USE_DLL_RANGE)) {// If in dll range => page heap
        PVOID StackTrace[32];
        ULONG Count;
        ULONG Index;
        ULONG Hash;
    ...
    else if ((HeapRoot->ExtraFlags & PAGE_HEAP_USE_RANDOM_DECISION)) {// If randomly decided => page heap
    ...
    else if ((HeapRoot->ExtraFlags & PAGE_HEAP_USE_DLL_NAMES)) {// If call not generated from one of the target dlls => normal heap
    ...
    else {// For all other cases we will allocate in the page heap.
        return TRUE;
    }
}
