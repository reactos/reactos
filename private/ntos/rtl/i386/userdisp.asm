...
; Abstract:
;    The module contains procedures to do user mode dispatching ("trampolining") of user apcs and user exceptions.
...
; VOID KiUserApcDispatcher (IN PKNORMAL_ROUTINE NormalRoutine, IN PVOID NormalContext, IN PVOID SystemArgument1, IN PVOID SystemArgument2, IN CONTEXT ContinueContext)
; Routine Description:
;    This routine is entered on return from kernel mode to deliver an APC in user mode.
;    The context frame for this routine was built when the APC interrupt was processed and contains the entire machine state of the current thread.
;    The specified APC routine is called and then the machine state is restored and execution is continued.
; Arguments:
;    NormalRoutine - Supplies that address of the function that is to be called.
;    NormalContext] - Supplies the normal context parameter that was specified when the APC was initialized.
;    SystemArgument1 - Supplies the first argument that was provied by the executive when the APC was queued.
;    SystemArgument2 - Supplies the second argument that was provided by the executive when the APC was queued.
...
; Routine Description:
;    This routine is entered on a callout from kernel mode to execute a user mode callback function.
;    All arguments for this function have been placed on the stack.
; Arguments:
;    ApiNumber - Supplies the API number of the callback function that is executed.
...
; Routine Description:
;    This routine is entered on return from kernel mode to dispatch a user mode exception.
;    If a frame based handler handles the exception, then the execution is continued.
...
.FPO (0, 2, 0, 0, 0, 0)
        ...
        stdCall   _RtlDispatchException, <ebx, ecx>; attempt to dispatch the exception

; If the return status is TRUE, then the exception was handled and execution
; should be continued with the NtContinue service in case the context was changed.
; If the return statusn is FALSE, then the exception was not handled
...
; Common code for nonsuccessful completion of the continue or raiseexception services.
; Use the return status as the exception code, set noncontinuable
; exception and attempt to raise another exception. Note the stack grows and eventually this loop will end.
...
; NTSTATUS KiUserExceptionDispatcher (IN PVOID ReturnAddress
;    IN NTSTATUS ExceptionCode)
; Routine Description:
;    This routine is entered on return from kernel mode to raise a user mode exception.
;    NOTE:  This procedure is not called, but rather dispatched to.
