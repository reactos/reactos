...
//      dispatch code to do the right thing with the service, and call RtlRaiseException (NOT ExRaiseException!!!).
//  Arguments:
...
// A label after the above _asm  is necessary so BBT can treat DebugService as  "KnownDataRange".  
// Otherwise, the two  'int' instructions could get broken up by BBT's optimizer.
