...
Abstract:
    This module implements the dispatching of exception and the unwinding of procedure call frames.
    ...
    while (RegistrationPointer != EXCEPTION_CHAIN_END) {
        ...
        if ( ((ULONG)RegistrationPointer < LowLimit) || (HighAddress > HighLimit) || (((ULONG)RegistrationPointer & 0x3) != 0) ) {
            ...
            if (((TestAddress & 0x3) == 0) && KeGetCurrentIrql() >= DISPATCH_LEVEL) {
                ...
                if ((Prcb->DpcRoutineActive) && (HighAddress <= DpcStack) && (TestAddress >= DpcStack - KERNEL_STACK_SIZE)) {
                    // This error occured on the DPC stack, switch stack limits to the DPC stack and restart the loop.
                    HighLimit = DpcStack;
        ...
        if (NtGlobalFlag & FLG_ENABLE_EXCEPTION_LOGGING) {
            Index = RtlpLogExceptionHandler(ExceptionRecord, ContextRecord, 0, (PULONG)RegistrationPointer, 4 * sizeof(ULONG));
                    // can't use sizeof(EXCEPTION_REGISTRATION_RECORD
                    // because we need the 2 dwords above it.
        }
#endif

        Disposition = RtlpExecuteHandlerForException(ExceptionRecord, (PVOID)RegistrationPointer, ContextRecord, (PVOID)&DispatcherContext, (PEXCEPTION_ROUTINE)RegistrationPointer->Handler);
        ...
        // the nested context frame and the nested exception in the exception flags.
        ...
        case ExceptionContinueExecution :
            ...
            // The disposition is to continue the search. Get next frame address and continue the search.
...
Routine Description:
    ...
    N.B.    The captured context passed to unwinding handlers will not be a  completely accurate context set for the 386.
            ...
            The segement and control registers (ebp, esp) will have correct values for the flat 32 bit environment.
...
--*/
{
    ...
    // If the target frame of the unwind is specified, then set EXCEPTION_UNWINDING flag in the exception flags.
    // Otherwise set both EXCEPTION_EXIT_UNWIND and EXCEPTION_UNWINDING flags in the exception flags.
    ...
    while (RegistrationPointer != EXCEPTION_CHAIN_END) {
        ...
        // If the call frame is not within the specified stack limits or the call frame is unaligned, then raise the exception STATUS_BAD_STACK.
        ...
        if ( ((ULONG)RegistrationPointer < LowLimit) || (HighAddress > HighLimit) || (((ULONG)RegistrationPointer & 0x3) != 0) ) {
            ...
            // The handler must be executed by calling another routine
            // that is written in assembler. This is required because up level addressing of the handler information is required when a collided unwind is encountered.
            Disposition = RtlpExecuteHandlerForUnwind(ExceptionRecord, (PVOID)RegistrationPointer, ContextRecord, (PVOID)&DispatcherContext, RegistrationPointer->Handler);
            ...
            case ExceptionContinueSearch :
                break;
                // The disposition is colided unwind. Maximize the target of the unwind and change the context record pointer.
            case ExceptionCollidedUnwind :
                // Pick up the registration pointer that was active at the time of the unwind, and simply continue.
                RegistrationPointer = DispatcherContext.RegistrationPointer;
                break;

                // All other disposition values are invalid. Raise invalid disposition exception.
            default :
            ...
            RtlpUnlinkHandler(PriorPointer);// Unlink the unwind handler, since it's been called.

            // If chain goes in wrong direction or loops, raise an exception.
        }
    }
    ...
    if (TargetFrame == EXCEPTION_CHAIN_END) {
        ...
        //  Do a normal continue, since we've effectively found the "target" the caller wanted.
        ZwContinue(ContextRecord, FALSE);
    } else {
        //  Either (1) a real exit unwind was performed, or (2) the specified TargetFrame is not present in the exception handler list.
        //  In either case, give debugger and subsystem a chance to see the unwind.
