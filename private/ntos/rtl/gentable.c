...
static TABLE_SEARCH_RESULT FindNodeOrParent(IN PRTL_GENERIC_TABLE Table,
                                            IN PVOID Buffer,
        ...
        while (TRUE) {
            ...
            if (Result == GenericLessThan) {
                if (Child = RtlLeftChild(NodeToExamine)) {
                    ...
                    // parameter to point to what would be its parent and return which child it would be.
...
Arguments:
    ...
    Restart - Flag that if true we should start with the least
              ...
              a pointer to the user data for the root and make the real successor to the root the new root.
Return Value:
...
--*/
{
    if (RtlIsGenericTableEmpty(Table)) {
        ...
        if (Restart) {
            ...
            for (NodeToReturn = Table->TableRoot;
            ...
            // making it the root of the tree).  Note that we need to take care when there are no more elements.
...
--*/
{
    ...
    if (CurrentLocation > NormalizedI) {
        ...
        // go to is more than half way between the listhead and where we currently are.
        if (NormalizedI > (CurrentLocation/2)) {
            ...
            for (BackwardDistance = CurrentLocation - NormalizedI;
                BackwardDistance;
                BackwardDistance--) {
