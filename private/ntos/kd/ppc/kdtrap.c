...
Return Value:
    A value of TRUE is returned if the exception is handled.
...
re_enter_debugger:
    ...
    if ((ExceptionRecord->ExceptionCode == STATUS_BREAKPOINT) && ((ExceptionRecord->ExceptionInformation[0] & BREAKPOINT_CODE_MASK) >= DEBUG_PRINT_BREAKPOINT)) {
        ...
        case DEBUG_LOAD_SYMBOLS_BREAKPOINT:
            ...
            if (KdDebuggerNotPresent == FALSE) {
                KdpReportLoadSymbolsStateChange((PSTRING)ContextRecord->Gpr3, (PKD_SYMBOLS_INFO) ContextRecord->Gpr4, UnloadSymbols, &Prcb->ProcessorState.ContextFrame);
            }
            ...
            // If the kernel debugger did not update the IAR, then increment past the breakpoint instruction.
    ...
    // check to see if the user of the remote debugger requested memory to be paged in
    if (KdpPageInAddress) {
        if (KeGetCurrentIrql() <= APC_LEVEL) {
            // if the IQRL is below DPC level then cause the page fault to occur and then re-enter
            // the debugger.  this whole process is transparent to the user.
            KdpPageInData( (PUCHAR)KdpPageInAddress );
            KdpPageInAddress = 0;
            KdpControlCPending = FALSE;
            goto re_enter_debugger;
        } else {
            // we cannot take a page fault here so a worker item is queued to take the
            // page fault.  after the worker item takes the page fault it sets the contol-c flag so that
            // the user re-enters the debugger just as if control-c was pressed.
...
--*/
{
    ...
    if (ContextRecord->Dr6 !=0) {      // Debug Register Breakpoint
        if ((PreviousMode == KernelMode) || (KeGetCurrentThread()->DebugActive == 0)) { // No DR set for thread
            return TRUE;
