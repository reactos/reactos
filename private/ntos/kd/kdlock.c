...
/*++
Routine Description:
    This procedure raises IRQL to high_level, seizes the Debug port spinlock, and checks to see if a breakin packet is pending.
    ...
    if (KdDebuggerEnabled != FALSE) {
        ...
        if (KdpControlCPending != FALSE) {
            ...
            if (KiTryToAcquireSpinLock(&KdpDebuggerLock) != FALSE) {
                ...
                if ((Status == CP_GET_SUCCESS) && (Input == BREAKIN_PACKET_BYTE)) {
                    BreakIn = TRUE;
                    KdpControlCPressed = TRUE;
                }
...
/*++
Routine Description:
    This procedure same as KdPollBreakIn, but assumes the caller already holds the port lock.
