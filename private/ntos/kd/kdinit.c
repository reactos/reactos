...
Arguments:
    LoaderBlock - Supplies a pointer to the LOADER_PARAMETER_BLOCK passed in from the OS Loader.
    StopInDebugger - Supplies a boolean value that determines whether a debug message and breakpoint are generated.
Return Value:
    None.
--*/
{
    ...
    // from BugCheck code. For this case the debugger is always enabled to report the bugcheck if possible.
    if (LoaderBlock != NULL) {
        KdpNtosImageBase =  CONTAINING_RECORD((LoaderBlock->LoadOrderListHead.Flink), LDR_DATA_TABLE_ENTRY, InLoadOrderLinks)->DllBase;
        ...
        // Most fields are already initialized, some fields will not be filled in until later.
        ...
        if (LoaderBlock->LoadOptions != NULL) {
            ...
            // If any of the port option, baud option, or debug is specified, then enable the debugger unless it is explictly disabled.
            Initialize = TRUE;
            ...
            if ((PortOption == NULL) && (BaudOption == NULL)) {
                ...
                if (PortOption) {
                    ...
                    if (PortOption) {
                        KdDebugParameters.CommunicationPort = atol(PortOption + 3);
                    }
                }
    ...
    // Initialize ID for NEXT packet to send and Expect ID of next incoming packet.
...
/*++
Routine Description:
    This routine is called by a component or driver to register a debugger data block.
    The data block is made accessible to the kernel debugger, thus providing a reliable method of exposing random data to debugger extensions.
Arguments:
    Tag - Supplies a unique 4 byte tag which is used to identify the data block.
    DataHeader - Supplies the address of the debugger data block header.
            The OwnerTag field must contain a unique value, and the Size field must contain the size of the data block, including the
            header.  If this block is already present, or there is already a block with the same value for OwnerTag, this one
            will not be inserted.  If Size is incorrect, this code will not notice, but the usermode side of the debugger might not function correctly.
...
/*++
Routine Description:
    This routine is called to deregister a data block previously registered with KdRegisterDebuggerDataBlock.
    If the block is found in the list, it is removed.
Arguments:
    DataHeader - Supplies the address of the data block which is to be removed from the list.
--*/
{
    KIRQL OldIrql;
    PLIST_ENTRY List;
...
VOID KdLogDbgPrint(IN PSTRING String)
{
    ...
    if (KdPrintCircularBuffer) {
        ...
        if (KdPrintWritePointer + Length <= KdPrintCircularBuffer+KDPRINTBUFFERSIZE) {
            ...
            LengthCopied = KdpMoveMemory(KdPrintWritePointer, String->Buffer, First);
            if (LengthCopied == First) {
                LengthCopied += KdpMoveMemory(KdPrintCircularBuffer, String->Buffer + First, Length - First);
            }
            if (LengthCopied > First) {
