    ...
    )
{
    ...
    rc = NtCreateKey(RootKey, &String1, 0, NULL, GENERIC_READ|GENERIC_WRITE, &Handle1);
    if (!NT_SUCCESS(rc)) {
    ...
    rc = NtQueryValueKey(Handle1, &Type, ValueBuffer, &ValueLength, &Time);
    if (!NT_SUCCESS(rc)) {
    ...
    return;
    } else if (RtlCompareMemory(ValueBuffer, Value1, ValueLength) != ValueLength) {
    ...
    rc = NtCreateKey(Handle1, &String1, 0, NULL, GENERIC_READ|GENERIC_WRITE, &Handle2);
    if (!NT_SUCCESS(rc)) {
    ...
    DbgPrint("Part7\n");
    ValueLength = MAX_VALUE;
    rc = NtQueryValueKey(Handle2, &Type, ValueBuffer, &ValueLength, &Time);
    if (!NT_SUCCESS(rc)) {
    ...
    return;
    } else if (RtlCompareMemory(ValueBuffer, Value2, ValueLength) != ValueLength) {
