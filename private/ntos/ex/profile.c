...
Routine Description:
    ...
    stops profiling, returns locked buffers and pages, dereferences the specified process and returns.
Arguments:
...
Arguments:
    ...
    RangeSize - Supplies the size of the range to profile in the
                ...
                such that RangeBase <= address < RangeBase+RangeSize will generate a profile hit.
    BucketSize - Supplies the LOG base 2 of the size of the profiling bucket.  Thus, BucketSize = 2 yields four-byte
                 ...
                 cannot be smaller than a ULONG.  The acceptable range of this value is 2 to 30 inclusive.
    Buffer - Supplies an array of ULONGs.  Each ULONG is a hit counter,
             which records the number of hits of the corresponding bucket.
...
--*/
{
    ...
    if (!ARGUMENT_PRESENT(Process)) {
        ...
        Status = ObReferenceObjectByHandle(Process, PROCESS_QUERY_INFORMATION, PsProcessType, PreviousMode, (PVOID *)&ProcessAddress, NULL);
    ...
    // If the profile object was successfully allocated, initialize the profile object.
    if (NT_SUCCESS(Status)) {
...
/*++
Routine Description:
    The NtStartProfile routine starts the collecting data for the specified profile object.  This involved allocating nonpaged
...
Routine Description:
    ...
    Performance frequency is the frequency of the performance counter in Hertz, i.e., counts/second.  Note that this value is implementation
    dependent.  If the implementation does not have hardware to support performance timing, the value returned is 0.
...
--*/
{
    ...
    if (PreviousMode != KernelMode) {
        // Probe accessibility of user's buffer.
        try {
            ProbeForWrite(PerformanceCounter, sizeof(LARGE_INTEGER), sizeof(ULONG));
            if (ARGUMENT_PRESENT(PerformanceFrequency)) {
                ProbeForWrite(PerformanceFrequency, sizeof(LARGE_INTEGER), sizeof(ULONG));
            }
        } except(EXCEPTION_EXECUTE_HANDLER) {
            // If an exception occurs during the probe or capture
            // of the initial values, then handle the exception and
    ...
    try {
        *PerformanceCounter = KeQueryPerformanceCounter((PLARGE_INTEGER)&KernelPerformanceFrequency);
        if (ARGUMENT_PRESENT(PerformanceFrequency)) {
