...
--*/
{
    ...
    if (PoolType == NonPagedPool || PoolType == NonPagedPoolMustSucceed) {
        ...
        KeWaitForSingleObject(PoolVector[PoolType]->LockAddress, PoolAllocation, KernelMode, FALSE, NULL);
        return Irql;
    }
}
...
--*/
{
    if (PoolType == NonPagedPool || (PoolType) == NonPagedPoolMustSucceed) {
        ...
        KeReleaseSpinLock((PKSPIN_LOCK)PoolVector[PoolType]->LockAddress, LockHandle);
    } else {
...
Routine Description:
    ...
    type.  Once initialized, the pool may be used for allocation and deallocation.
...
Routine Description:
    ...
    This function is used to access both the page-aligned pools, and the binary buddy (less than a page) pools.

    If the number of bytes specifies a size that is too large to be satisfied by the appropriate binary buddy pool, then the page-aligned
    pool allocator is used.  The allocated block will be page-aligned and a page-sized multiple.
    ...
    The binary buddy allocator calculates the smallest block size that is a power of two and that can be used to satisfy the request.
    If there are no blocks available of this size, then a block of the next larger block size is allocated and split in half.
...
Arguments:
    ...
    PoolType - Supplies the type of pool to allocate.  If the pool type
        is one of the "MustSucceed" pool types, then this call will always succeed and return a pointer to allocated pool.
        Otherwise, if the system can not allocate the requested amount of memory a NULL is returned.
...
--*/
{
    ...
    if (NumberOfBytes > POOL_BUDDY_MAX) {
        ...
        Entry = (PPOOL_HEADER)MiAllocatePoolPages(BasePoolTypeTable[PoolType], NumberOfBytes);
        if (!Entry && MustSucceedPoolTable[PoolType]) {
            Entry = (PPOOL_HEADER)MiAllocatePoolPages(PoolType, NumberOfBytes);
        }
    ...
    *PointerPte = TempPte;
    ...
    Entry = (PVOID)(((ULONG)Entry + (PAGE_SIZE - (NumberOfBytes))) & 0xfffffff8L);
    return Entry;
...
#ifdef TRACE_ALLOC
    {
        ...
        KeWaitForSingleObject(&TracePoolLock, PoolAllocation, KernelMode, FALSE, NULL);
        ...
        (VOID)KeReleaseSemaphore(&TracePoolLock, 0L, 1L, FALSE);
        KeLowerIrql(xIrql);
    }
#endif // TRACE_ALLOC
        ...
        *(BadFood + 1) = 0xBAADF00D;
        ...
        if (Index != (POOL_LIST_HEADS - 1)) {
            ...
            *(BadFood + 1) = 0xBAADF00D;
            ...
            InsertTailList(&PoolListHead->ListHead, (PLIST_ENTRY)(((PCH)Buddy + POOL_OVERHEAD)));
            return (PVOID)Entry;
        } else {

            // Need to call page allocator for a page to add to the pool

            Entry = (PPOOL_HEADER)MiAllocatePoolPages(BasePoolTypeTable[PoolDesc->PoolType], PAGE_SIZE);
            if (!Entry) {
                if (MustSucceedPoolTable[PoolDesc->PoolType]) {
                    Entry = (PPOOL_HEADER)MiAllocatePoolPages(PoolDesc->PoolType, PAGE_SIZE);
                    ASSERT(Entry);
                } else {
                    return NULL;
                }
