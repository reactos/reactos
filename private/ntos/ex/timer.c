...
--*/
{
    ...
    // It is possible for the timer not to be in the current thread's active timer list since the APC could have been delivered, and then another thread could have set the timer again with another APC.
...
--*/
{
    ...
    // If there is still an APC associated with the timer, then insert the APC in target thread's APC queue. It is possible that the timer does not have an associated APC.
...
--*/
{
    ...
    // If the time object type descriptor was successfully created, then return a value of TRUE. Otherwise return a value of FALSE.
...
--*/
{
    ...
    while (NextEntry != &ExThread->ActiveTimerListHead) {
        ...
        // If the timer is still in the current thread's active timer list, then cancel the timer, remove the timer's DPC from the DPC
        // queue, remove the timer's APC from the APC queue, remove the timer from the thread's active timer list, and set the associate APC flag FALSE.
