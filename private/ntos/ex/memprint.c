...
Abstract:
    ...
    DbgPrint text is stored in a large circular buffer, and optionally written to a file and/or the debug console.
    Output to file is buffered to allow high performance by the file system.
...
// The maximum message size is the largest message that can be written by a single call to MemPrint.
#define MEM_PRINT_MAX_MESSAGE_SIZE 256

// These macros aid in determining the size of a subbuffer and the subbuffer corresponding to an index into the circular buffer.
...
// The definition of the header put before each message if the MEM_PRINT_FLAG_HEADER bit of MemPrintFlags is turned on.
...
// MemPrintCurrentSubbuffer stores the index of the subbuffer currently being used to hold data.
// It has a range between 0 and MemPrintSubbufferCount-1.
CLONG MemPrintCurrentSubbuffer = 0;

// The MemPrintSubbufferWriting array is used to indicate when a subbuffer is being written to disk.
// While this occurs, new data cannot be written to the subbuffer.
BOOLEAN MemPrintSubbufferWriting[MEM_PRINT_MAX_SUBBUFFER_COUNT];

// The MemPrintSubbufferFullEvent array is used to communicate between threads calling MemPrintMemory and the thread that writes the log file.
// When a subbuffer is full and ready to be written to disk, the corresponding event in this array is signaled, which causes the write thread to wake up and perform the write.
...
Routine Description:
    ...
    It should be called before the first call to MemPrint to set up the various structures used and to start the log file write thread.
--*/
{
    CLONG i;
    NTSTATUS status;
    ...
    // Allocate memory for the circular buffer that will receive the text and data.
    // If we can't do it, try again with a buffer half as large.
    // If that fails, quit trying.
    ...
    // Allocate the spin lock that protects access to the various pointers and the circular buffer.
    // This ensures integrity of the buffer.
    KeInitializeSpinLock( &MemPrintSpinLock );

    // Make sure that the subbuffer count is in range.  (We assume that the number is a power of 2.)
    ...
    // Initialize the array of BOOLEANs that determines which subbuffers are being written to disk and therefore cannot be used to store new DbgPrint data.

    // Initialize the array of events that indicates that a subbuffer is ready to be written to disk.
    ...
    // Start the thread that writes subbuffers from the large circular buffer to disk.
    status = PsCreateSystemThread(&threadHandle, PROCESS_ALL_ACCESS, NULL, NtCurrentProcess(), NULL, MemPrintWriteThread, NULL);
...
/*++
Routine Description:
    This routine is called in place of DbgPrint to process in-memory printing.
Arguments:
...
--*/
{
    ...
    // If memory DbgPrint has not been initialized, simply print to the console.
    if ( !MemPrintInitialized ) {
        DbgPrint( "%s", tempBuffer );
        return;
    }

    // Acquire the spin lock that synchronizes access to the pointers and circular buffer.
    KeAcquireSpinLock( &MemPrintSpinLock, &oldIrql );

    // Make sure that the request will fit.
    // xx_sprintf will just dump all it gets, so assume the message is maximum size, and, if the request would go into the next subbuffer and it is writing, fail the request.
    nextSubbuffer = GET_MEM_PRINT_SUBBUFFER( MemPrintIndex + MEM_PRINT_MAX_MESSAGE_SIZE );
    if (  nextSubbuffer != MemPrintCurrentSubbuffer ) {
        // The request will go to a new subbuffer.
        // Check if we should wrap around to the first subbuffer (i.e. start of circular buffer).
        ...
        // If we went to subbuffer 0 and it is available to receive data, set up the "end of last subbuffer" conditions and reset the index into the circular buffer.
        // By setting a special type value in the message header that precedes the garbage at the end of the last subbuffer,
        // an interpreter program can know to skip over the garbage by using the size in the header.
        // This is done instead of writing only good data so that we can write just full sectors to disk, thereby enhancing write performance.
        if ( nextSubbuffer == 0 ) {
            // Set up the message header.
            // This always gets done at the end of the circular buffer, regardless of the flags bit.
            ...
            // Reset the index to start at the beginning of the circular buffer.
            MemPrintIndex = 0;
        }
    }

    // Store a pointer to the location that will contain the message header.
    messageHeader = (PMEM_PRINT_MESSAGE_HEADER)&MemPrintBuffer[MemPrintIndex];
    if ( MemPrintFlags & MEM_PRINT_FLAG_HEADER ) {
        MemPrintIndex += sizeof(MEM_PRINT_MESSAGE_HEADER);
    }

    // Dump the formatted string to the subbuffer.  xx_sprintf is a special version of sprintf that takes a variable argument list.
    ...
    // If it was too large, there's a potential problem with writing off the end of the circular buffer.
    // Print the offending message to the console and breakpoint.
    ...
    if ( nextSubbuffer != MemPrintCurrentSubbuffer ) {
        ...
        // We did step into a new subbuffer, so set the boolean to indicate that the old subbuffer is writing to disk, thereby
        // preventing it from being overwritten until the write is complete.
        MemPrintSubbufferWriting[MemPrintCurrentSubbuffer] = TRUE;

        // Set the event that will wake up the thread writing subbuffers to disk.
...
/*++
Routine Description:
    This routine causes the current subbuffer to be written to disk, regardless of how full it is.
    The unwritten part of the subbuffer is zeroed before writing.
--*/
{
    KIRQL oldIrql;
    PMEM_PRINT_MESSAGE_HEADER messageHeader;
    ...
    // Increment the current subbuffer so that it corresponds with the buffer index.
    MemPrintCurrentSubbuffer++;

    KeReleaseSpinLock( &MemPrintSpinLock, oldIrql );

    // Delay so that the memory print write thread wakes up and performs the write to disk.

    // !!! This is obviously not a perfect solution--the write thread may never wake up, so this could complete before the flush is really done.
...
/*++
Routine Description:
    The log file write thread executes this routine.
    It sets up the log file for writing, then waits for subbuffers to fill, writing them to disk when they do.
    When the log file fills, new space for it is allocated on disk to prevent the file system from having to do it.
Arguments:
    Dummy - Ignored.
--*/
{
    ...
    // Initialize the string containing the file name and the object attributes structure that will describe the log file to open.
    ...
    // Set the allocation size of the log file to be three times the size of the circular buffer.  When it fills up, we'll extend it.
    ...
    // !!! The loop here is to help avoid a system initialization timing problem, and should be removed when the problem is fixed.
    while ( TRUE ) {
        status = NtCreateFile(
                     &fileHandle,
                     FILE_WRITE_DATA,
    ...
    status = NtSetInformationThread(NtCurrentThread(), ThreadPriority, &threadPriorityLevel, sizeof(threadPriorityLevel));
    if ( !NT_SUCCESS(status) ) {
        DbgPrint( "Unable to set error log thread priority: %X\n", status );
    }

    // Loop waiting for one of the subbuffer full events to be signaled.
    // When one is signaled, wake up and write the subbuffer to the log file.
    while ( TRUE ) {
        waitStatus = KeWaitForMultipleObjects((CCHAR)MemPrintSubbufferCount, waitObjects, WaitAny, Executive, KernelMode, TRUE, NULL, waitBlockArray);
        ...
        // Check the DbgPrint flags to see if we really want to write this.
        ...
        // The APC routine will handle checking the return status from the write and resetting the MemPrintSubbufferWriting boolean.
        status = NtWriteFile(
                     fileHandle,
                     NULL,
        ...
        // Extend the file if we have reached the end of what we have thus far allocated for the file.
        // This increases performance by extending the file here rather than in the file system, which would have to extend it each time a write past end of file comes in.
        if ( totalBytesWritten.QuadPart >= fileAllocation.QuadPart ) {
            ...
            status = NtSetInformationFile(fileHandle, &localIoStatusBlock, &fileAllocation, sizeof(fileAllocation), FileAllocationInformation);
            if ( !NT_SUCCESS(status) ) {
...
Routine Description:
    ...
    It also sets the appropriate MemPrintSubbufferWriting location to FALSE so that the subbuffer can be reused.
Arguments:
...
--*/
{
    ...
    // Acquire the spin lock that protects memory print global variables and set the subbuffer writing boolean to FALSE so that other threads can write to the subbuffer if necessary.
    KeAcquireSpinLock( &MemPrintSpinLock, &oldIrql );
