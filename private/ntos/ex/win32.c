...
/*++
Routine Description:
    This function creates the Win32 object type descriptors at system initialization and stores the address of the object type descriptor in local static storage.
Return Value:
    A value of TRUE is returned if the Win32 object type descriptors are successfully created. Otherwise a value of FALSE is returned.
--*/
{
    ...
    Status = ObCreateObjectType(&TypeName, &ObjectTypeInitializer, (PSECURITY_DESCRIPTOR)NULL, &ExWindowStationObjectType);

    // If the windowstation object type descriptor was not successfully created, then return a value of FALSE.
    ...
    // If the desktop object type descriptor was successfully created, then return a value of TRUE. Otherwise return a value of FALSE.
