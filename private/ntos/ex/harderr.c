    ...
    )
{
    ...
    if ( ErrorPort ) {
        ...
        Status = LpcRequestWaitReplyPort(ErrorPort, (PPORT_MESSAGE) m, (PPORT_MESSAGE) m);
        if ( NT_SUCCESS(Status) ) {
            switch ( m->Response ) {
    ...
    )
{
    ...
    if (PreviousMode != KernelMode) {
            ...
            if ( ARGUMENT_PRESENT(Parameters) ) {
                ...
                if ( UnicodeStringParameterMask ) {
                    for(Counter=0;Counter < NumberOfParameters;Counter++){
                        ...
                        if ( UnicodeStringParameterMask & (1<<Counter) ) {
                            ProbeForRead((PVOID)CapturedParameters[Counter], sizeof(UNICODE_STRING), sizeof(ULONG_PTR));
                            RtlMoveMemory(&CapturedString, (PVOID)CapturedParameters[Counter], sizeof(UNICODE_STRING));
        ...
        Status = ExRaiseHardError(ErrorStatus, NumberOfParameters, UnicodeStringParameterMask, CapturedParameters, ValidResponseOptions, &LocalResponse);
        }
