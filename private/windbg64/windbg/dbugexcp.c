...
Abstract:
    This file contains the code for dealing with the Debug Exceptions dialog box.
...
// Local functions
void SetFields(HWND hDlg, int idx);
static void FillListbox(HWND hDlg, EXCEPTION_LIST *eList);
static void UpdateExcptInListbox(HWND hDlg, EXCEPTION_LIST *eItem);
static int IndexFromExceptionCode(EXCEPTION_LIST * eList, DWORD dwExcpt);
static EXCEPTION_LIST * ExceptionItemFromIndex(EXCEPTION_LIST * eList, int idx);
static EXCEPTION_LIST * DupExcpt(EXCEPTION_LIST *eItem);
static void RemoveExcpt(EXCEPTION_LIST ** peList, EXCEPTION_LIST  * eItem);
static void DeleteExcpt(EXCEPTION_LIST *eItem);
static void DeleteList(EXCEPTION_LIST *eList);
INT_PTR CALLBACK DbgExcpt__ButtonSubProcADD (HWND    hWnd, UINT    message, WPARAM  wParam, LPARAM  lParam);
    ...
    LPARAM lParam)
{
    ...
    switch (message) {
      case WM_INITDIALOG:
        ...
        // when it is disabled, we want the OK button to become the default.
        DbgExcpt__currDlg = hDlg;
        DbgExcpt__origWindowProcADD = (WNDPROC) SetWindowLongPtr (GetDlgItem (hDlg, ID_DBUGEXC_ADD), GWLP_WNDPROC, (LONG_PTR) DbgExcpt__ButtonSubProcADD);
        ...
        for (i = 0; i < NUMEFD; i++) {
            SendDlgItemMessage(hDlg, ID_DBUGEXC_ACTION, CB_ADDSTRING, 0, (LPARAM)EfdMap[i].lpName);
        }

        SendDlgItemMessage(hDlg, ID_DBUGEXC_EXCLIST, LB_SETTABSTOPS, NUMTABS, (LPARAM)wTabs);
        ...
        return TRUE;
      case WM_HELP:
          WinHelp((HWND)((LPHELPINFO) lParam)->hItemHandle, "windbg.hlp", HELP_WM_HELP, (ULONG_PTR) HelpArray );
          return TRUE;
      case WM_CONTEXTMENU:
          WinHelp ((HWND) wParam, "windbg.hlp", HELP_CONTEXTMENU, (ULONG_PTR) HelpArray );
          return TRUE;
      case WM_COMMAND:
        switch (LOWORD(wParam)) {
          ...
          case ID_DBUGEXC_EXCNAME:
            ...
            if (HIWORD(wParam) == EN_CHANGE && (idx = (int) SendDlgItemMessage(hDlg, ID_DBUGEXC_EXCLIST, LB_GETCURSEL, 0, 0L)) >= 0) {
                eItem = ExceptionItemFromIndex(pDisplayedList, idx);
                SendMessage((HWND)lParam, WM_GETTEXT, sizeof(szText), (LPARAM)(LPSTR)szText);
                switch(LOWORD(wParam)) {
                  case ID_DBUGEXC_CMD:
                    ppstr = &eItem->lpCmd;
                    break;
          ...
          case ID_DBUGEXC_EXCNUM:
            if (HIWORD(wParam) == EN_CHANGE) {
                ...
                GetDlgItemText(hDlg, ID_DBUGEXC_EXCNUM, (LPSTR)szStr, MAX_CMDLINE_TXT);
                err = (*szStr == 0);
                if (!err) {
                    dwExceptionCode = CPGetNbr(szStr, 16, TRUE, &CxfIp, (LPSTR)szError, &err, g_contWorkspace_WkSp.m_bMasmEval);
                }
                if (!err) {
                    idx = IndexFromExceptionCode( pDisplayedList, dwExceptionCode);
                }
                if (err || idx < 0) {
                    // clear list selection
                    SendDlgItemMessage(hDlg, ID_DBUGEXC_EXCLIST, LB_SETCURSEL, (WPARAM)-1, 0);
                    EnableWindow(GetDlgItem(hDlg, ID_DBUGEXC_DELETE), FALSE);
                    if (err) {
                        EnableWindow(GetDlgItem(hDlg,ID_DBUGEXC_ADD), FALSE);
                    } else {
                    ...
                    SendDlgItemMessage(hDlg, ID_DBUGEXC_EXCLIST, LB_SETCURSEL, idx, 0);
                    if (!FSettingUp) {
          ...
          case ID_DBUGEXC_ACTION:
            if (HIWORD(wParam) == CBN_SELCHANGE && !FSettingUp) {
                SendMessage( (HWND)lParam, CB_GETLBTEXT, SendMessage((HWND)lParam, CB_GETCURSEL, 0, 0L ), (LPARAM)(LPSTR)szStr );
                DAssert(*szStr);
          ...
          case ID_DBUGEXC_EXCLIST:
            switch (HIWORD(wParam)) {
              ...
              case LBN_SELCHANGE:
                ...
                idx = (int) SendDlgItemMessage(hDlg, ID_DBUGEXC_EXCLIST, LB_GETCURSEL, 0, 0);
                if (idx < 0) {
                    EnableWindow(GetDlgItem(hDlg, ID_DBUGEXC_DELETE), FALSE);
                } else {
                    EnableWindow(GetDlgItem(hDlg, ID_DBUGEXC_DELETE), TRUE);
          ...
          case ID_DBUGEXC_ADD:
            ...
            dwExceptionCode = CPGetNbr(szStr, radix, TRUE, &CxfIp, (LPSTR)szError, &err, g_contWorkspace_WkSp.m_bMasmEval);
            ...
            GetDlgItemText(hDlg,ID_DBUGEXC_EXCNAME, (LPSTR)szText, MAX_CMDLINE_TXT);
          ...
          case ID_DBUGEXC_DELETE:
            ...
            // We want standard handling of the enable/disable situation after the Delete happens.  We get it by notifying the dialog that there
            // has been a change in the exception number...
            PostMessage (hDlg, WM_COMMAND, MAKEWPARAM(ID_DBUGEXC_EXCNUM,EN_CHANGE), MAKELPARAM(0,0));
            break;
          case IDOK:
            ...
            if (LppdCur) {
                for (eItem = pOriginalList; eItem; eItem = eItem->next) {
                    // If the exception is no longer in the list, set OSDebug to the default behaviour.
                    if (IndexFromExceptionCode(pDisplayedList,eItem->dwExceptionCode) < 0) {
                        ...
                        OSDGetExceptionState(LppdCur->hpid, NULL, &exd);

                        exd.efd = efd;
                        exd.exc = exfSpecified;
                        OSDSetExceptionState(LppdCur->hpid, NULL, &exd);
                    }
                }
                ...
                for (eItem = pDisplayedList; eItem; eItem = eItem->next) {
                    ...
                    if (eItem->lpName) {
                        _tcsncpy(exd.rgchDescription, eItem->lpName, EXCEPTION_STRING_SIZE);
                    }
                    OSDSetExceptionState(LppdCur->hpid, NULL, &exd);
                }
            }
...
void SetFields(HWND hDlg, int idx)
{
    ...
    if ( idx == LB_ERR ) {
        ...
        SendDlgItemMessage(hDlg, ID_DBUGEXC_ACTION, CB_SELECTSTRING, 0, (LPARAM)(LPSTR)EfdMap[i].lpName);
...
BOOL GetDefaultExceptionList (VOID)
{
    EMFUNC  lpfnEm = NULL;
    EXCEPTION_DESCRIPTION exd;
