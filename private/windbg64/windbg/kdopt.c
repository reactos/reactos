    ...
    )
{
    ...
    i = sizeof(rgch);
    if (RegQueryInfoKey( hkey, rgch, &i, NULL, &i, &i, &i, &NumComPorts, &i, &i, &i, &ft ) != ERROR_SUCCESS) {
        RegCloseKey( hkey );
        return 0;
    }
    ...
    if (NumComPorts) {
        ...
        while( TRUE ) {
            ...
            if (RegEnumValue( hkey, i, szValueName, &dwSz1, NULL, &dwType, (PUCHAR) szValueData, &dwSz2 ) != ERROR_SUCCESS) {
                break;
            }

            sprintf( rgch, "\\\\.\\%s", szValueData );
            hCommDev = CreateFile(rgch, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
            PortNum = strtoul(&szValueData[3], NULL, 0);
            if ((hCommDev != INVALID_HANDLE_VALUE) || (hCommDev == INVALID_HANDLE_VALUE && g_contWorkspace_WkSp.m_bKernelDebugger && DebuggeeActive() && PortNum == g_contKernelDbgPreferences_WkSp.m_dwPort)) {
