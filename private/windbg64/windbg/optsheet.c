...
Abstract:
    This module contains the code for Options property sheet dialog.
...
void Init(HWND hwndOwner, HINSTANCE hinst, LPPROPSHEETHEADER lppsh, PROPSHEETPAGE apsp[], const int nNumPropPages)
/*++
Routine Description:
...
INT_PTR DisplayOptionsPropSheet(HWND hwndOwner, HINSTANCE hinst, int nStartPage)
/*++
Routine Description:
...
#define ZFREE(P) ((P)?(free(P),((P)=NULL)):NULL)
    ...
    case WM_DESTROY:
        ...
        break;
      case WM_HELP:
          WinHelp((HWND)((LPHELPINFO) lParam)->hItemHandle, "windbg.hlp", HELP_WM_HELP, (DWORD_PTR)(LPVOID) HelpArray );
          return TRUE;
      case WM_CONTEXTMENU:
          WinHelp ((HWND) wParam, "windbg.hlp", HELP_CONTEXTMENU, (DWORD_PTR)(LPVOID) HelpArray );
          return TRUE;
    case WM_COMMAND:
        {
    ...
    case WM_INITDIALOG:
        { // begin Prog & Arguments code block
            ...
            CheckDlgButton(hwndDlg, ID_ENV_SRCHPATH, g_contGlobalPreferences_WkSp.m_bSrchSysPathForExe ? BST_CHECKED : BST_UNCHECKED);
            PostMessage(hwndDlg, WM_COMMAND, MAKEWPARAM(ID_ENV_SRCHPATH, BN_CLICKED), (LPARAM) GetDlgItem(hwndDlg, ID_ENV_SRCHPATH));
        ...
        case PSN_APPLY:
            {
                ...
                g_contGlobalPreferences_WkSp.m_bSrchSysPathForExe = (IsDlgButtonChecked(hwndDlg, ID_ENV_SRCHPATH) == BST_CHECKED);
                nCnt= (int) SendDlgItemMessage(hwndDlg, IDC_EDIT_ARGUMENTS, WM_GETTEXTLENGTH, 0, 0);
...
void Workspace_EnableLoggingOptions(HWND hwndDlg, BOOL bEnable)
{
...
void Workspace_ApplyChanges(HWND hwndDlg)
{
    BOOL bChecked = FALSE;
    char sz[_MAX_PATH];
    ...
    if (0 == g_contWorkspace_WkSp.m_pszLogFileName || strcmp(sz, g_contWorkspace_WkSp.m_pszLogFileName)) {
...
INT_PTR CALLBACK DlgProc_Workspace(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        // Figure what option to enable
        {
            ...
            CheckRadioButton(hwndDlg, IDC_RADIO_NEVER_SAVE_WORKSPACE, IDC_RADIO_AUTO_SAVE_WORKSPACE, nId);
        }

        CheckDlgButton(hwndDlg,ID_LFOPT_APPEND, g_contWorkspace_WkSp.m_bLfOptAppend ? BST_CHECKED : BST_UNCHECKED);
        CheckDlgButton(hwndDlg,ID_LFOPT_AUTO, g_contWorkspace_WkSp.m_bLfOptAuto ? BST_CHECKED : BST_UNCHECKED);

        // Don't just leave it blank, put something in there.
        SetDlgItemText(hwndDlg, ID_LFOPT_FNAME, (g_contWorkspace_WkSp.m_pszLogFileName && *g_contWorkspace_WkSp.m_pszLogFileName) ? g_contWorkspace_WkSp.m_pszLogFileName : DEFAULT_CMD_WINDOW_LOGFILENAME);
        return FALSE;
      case WM_HELP:
          WinHelp((HWND)((LPHELPINFO) lParam)->hItemHandle, "windbg.hlp", HELP_WM_HELP, (DWORD_PTR)(LPVOID) HelpArray );
          return TRUE;
                ...
                case BN_CLICKED:
                    {
                        ...
                        sprintf(sz, ".%s %s\r\n", DotTable[nIdx].lpName, g_contWorkspace_WkSp.m_pszLogFileName);
