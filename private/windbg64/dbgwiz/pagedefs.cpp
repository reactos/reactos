    ...
    case WM_INITDIALOG:
        {
            ...
            // Figure out if it's a workstation, server, enterprise server
            {
                ...
                Assert(ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Control\\ProductOptions", 0, KEY_READ, &hkey));
                Assert(ERROR_SUCCESS == RegQueryValueEx(hkey, "ProductType", NULL, &dwType, (PUCHAR) sz, &dwSize));
                ...
                if (!_stricmp(sz, "WinNT")) {
                    ...
                    if (ERROR_SUCCESS == RegQueryValueEx(hkey, "ProductSuite", NULL, &dwType, (PUCHAR) sz, &dwSize)) {
            ...
            if (enumKERNELMODE == g_CfgData.m_DebugMode) {
                ...
                PSTR pszSerial = WKSP_DynaLoadStringWithArgs(g_hInst, IDS_SERIAL_CONNECTION, BaudRateBitFlagToText(g_CfgData.m_dwBaudRate));
                pszConnection = WKSP_DynaLoadStringWithArgs(g_hInst, IDS_KERNEL_SUMMARY, pszSerial);
                free(pszSerial);
            } else {
                if (g_CfgData.m_bSerial) {
                    PSTR pszSerial = WKSP_DynaLoadStringWithArgs(g_hInst, IDS_SERIAL_CONNECTION, BaudRateBitFlagToText(g_CfgData.m_dwBaudRate));
                    pszSerial = WKSP_DynaLoadStringWithArgs(g_hInst, IDS_REMOTE_USER_SUMMARY, pszSerial);
                    free(pszSerial);
                } else {
                    Assert(g_CfgData.m_pszCompName);
                    Assert(g_CfgData.m_pszPipeName);

                    PSTR pszNetwork = WKSP_DynaLoadStringWithArgs(g_hInst, IDS_NETWORK_CONNECTION, g_CfgData.m_pszCompName, g_CfgData.m_pszPipeName);
                    pszConnection = WKSP_DynaLoadStringWithArgs(g_hInst, IDS_REMOTE_USER_SUMMARY, pszNetwork);
                    free(pszNetwork);
                }
            }
            ...
            // Get Main text
            pszMainText = WKSP_DynaLoadStringWithArgs(g_hInst, m_nMainTextStrId, pszConnection, pszOs);
            Assert(pszMainText);
