...
PAGEID MULTI_OPT_PAGE_DEF<m_dwNumOptions>::GetNextPage(BOOL bCalcNextPage)
{
    int nOptIdx;
...
MULTI_OPT_PAGE_DEF(
    ...
    : PAGE_DEF(nPageId, nDlgRsrcId, nMoreInfo_PageId, nHeaderTitleStrId, nHeaderSubTitleStrId)
{
...
template<const DWORD m_dwNumOptions>
int MULTI_OPT_PAGE_DEF<m_dwNumOptions>::
DlgProc(
    IN HWND     hDlg,
    IN UINT     uMsg,
    IN WPARAM   wParam,
...
)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        if (FINISH_PAGEID == m_nPageId &&
            ((enumKERNELMODE == g_CfgData.m_DebugMode && !g_CfgData.m_bRunningOnHost) || NULL == g_CfgData.m_pszDeskTopShortcut_FullPath)) {
    ...
    case UM_DLG_PAGE_SETACTIVE:
    {
        ...
        if (HOTFIX_SYM_LOC_SYMCPY_PAGEID == m_nPageId) {
            if (g_CfgData.m_pszHotFixes) {
                pszMainText = WKSP_DynaLoadStringWithArgs(g_hInst, m_nMainTextStrId, g_CfgData.m_pszHotFixes);
            }
            else {
        ...
        case PSN_WIZFINISH:
        {
            ...
            if (IsDlgButtonChecked(hDlg, m_rgnOpts_CtrlId[0]) == BST_CHECKED) {
                if (enumKERNELMODE == g_CfgData.m_DebugMode && !g_CfgData.m_bRunningOnHost) {
                    ...
                    if (PtrToUlong(ShellExecute(hDlg, NULL, pszShortcutPath, NULL, NULL, SW_SHOWNORMAL)) <= 32) {
...
GetNextPage(BOOL bCalcNextPage)
{
    int nOptIdx;
...
)
{
    ...
    if (!GetDiskInfo(m_pszPath, pszVolumeLabel, dwSerialNumber, uDriveType)) {
        ...
        int nRes = WKSP_CustMsgBox(MB_YESNO | MB_ICONINFORMATION | MB_TASKMODAL, NULL, IDS_ERROR_GETTING_DRIVE_INFO, pszPath, pszErr);
...
CALC_NEXT_PAGE:
    ...
    case SP_SYM_LOC_SYMCPY_PAGEID:
        ...
        if (!g_CfgData.AreWeUsingACfgFile() &&
            ...
            if (OS_SYM_LOC_SYMCPY_PAGEID == m_nPageId && g_CfgData.m_dwServicePack) {
                return SP_SYM_LOC_SYMCPY_PAGEID;
            }
...
)
{
    ...
    {
        if (!m_pszPath) {
            if (m_nPageId == SAVE_INI_FILE_PAGEID || m_nPageId == SPECIFY_INI_FILE_PAGEID) {
            ...
            else if (m_nPageId == SP_SYM_LOC_SYMCPY_PAGEID || m_nPageId == HOTFIX_SYM_LOC_SYMCPY_PAGEID) {
    ...
    case WM_NOTIFY:
    {
        ...
        case PSN_WIZBACK:
        {
            ...
            if (PSN_WIZNEXT == pnmh->code) {
                if (!m_bDisplayingMoreInfo && IsDlgButtonChecked(hDlg, m_rgnOpts_CtrlId[0]) == BST_CHECKED && !*sz) {
                    // Stay here
    ...
    if (WM_NOTIFY == uMsg && PSN_WIZNEXT == ((LPNMHDR)lParam)->code && -1 != GetWindowLongPtr(hDlg, DWLP_MSGRESULT) && m_pszPath) {
        // Conditional jumps, and the setting of flags
        switch (m_nPageId) {
        case OS_SYM_LOC_SYMCPY_PAGEID:
