...
void SpawnMunger()
{
    ...
    sprintf(szCmdLine, "%s /d /q /a %s /b %s", pszAppName, g_CfgData.m_pszCommPortName, BaudRateBitFlagToText(g_CfgData.m_dwBaudRate));
    Assert(strlen(szCmdLine) < sizeof(szCmdLine));

    if (WinExec(szCmdLine, SW_SHOWNORMAL) <= 31) {
        WKSP_MsgBox(NULL, IDS_ERROR_COULDNT_EXEC_MUNGER, g_CfgData.m_pszCommPortName, BaudRateBitFlagToText(g_CfgData.m_dwBaudRate));
    }
}
...
*/
{
    ...
    n = GetPrivateProfileString (szBootLdr, NULL, NULL, szTemp, sizeof(szTemp), szBootIni);
    if (n != 0) {
        pszBoot = szBootLdr;
    } else {
        n = GetPrivateProfileString (szFlexBoot, NULL, NULL, szTemp, sizeof(szTemp), szBootIni);
        if (n != 0) {
            pszBoot = szFlexBoot;
        } else {
            pszBoot = szMultiBoot;
    ...
    GetPrivateProfileString (pszBoot, szDefault, NULL, szTemp, sizeof(szTemp), szBootIni);
    ...
    )
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Hardware\\DeviceMap\\SerialComm", 0, KEY_ALL_ACCESS, //KEY_READ,
        &hkey ) != ERROR_SUCCESS ) {
        return;
    }

    WKSP_RegKeyValueInfo(hkey, NULL, &dwNumPortsPresent, NULL, &dwMaxValueNameLen);
