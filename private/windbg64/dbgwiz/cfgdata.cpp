...
CFGDATA()
{
...

    {
        ...
        if (hkey) {
            ...
            Assert(ERROR_SUCCESS == RegQueryValueEx(hkey, pszRegValue, NULL, &dwType, (PUCHAR) m_szInstallSrcPath, &dwSize));
    ...
    if (VER_PLATFORM_WIN32_NT == m_osi.dwPlatformId) {
        ...
        Assert(ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_READ, &hkey));
        Assert(ERROR_SUCCESS == RegQueryValueEx(hkey, "CurrentType", NULL, &dwType, (PUCHAR) sz, &dwSize));
        Assert(ERROR_SUCCESS == RegCloseKey(hkey));
