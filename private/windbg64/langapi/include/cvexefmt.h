...
//  Pcode support.
//  This subsection contains debug information generated by the MPC utility used to process Pcode executables.
//  Currently it contains a mapping table from segment index (zero based) to frame paragraph.
//  MPC converts segmented exe's to non-segmented exe's for DOS support.
//  To avoid backpatching all CV info, this table is provided for the mapping.
//  Additional info may be provided in the future for profiler support.
...
//  The following structures and constants describe the format of the CodeView Debug OMF for linkers that emit executables with the NB02 signature.
//  Current utilities with the exception of cvpack and cvdump will not accept or emit executables with the NB02 signature.
//  Cvdump will dump an unpacked executable with the NB02 signature.
//  Cvpack will read an executable with the NB02 signature but the packed executable will be written with the table format, contents and signature of NB08.
...
// OMFSegMap - This table contains the mapping between the logical segment indices used in the symbol table and the physical segments where the program is loaded
