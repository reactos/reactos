XOSD EMFunc (EMF  emf, HPID hpid, HTID htid, DWORD64 wValue, DWORD64 lValue);
XOSD DebugPacket (DBC dbc, HPID hpid, HTID htid, DWORD wValue, LPBYTE lpb);
...
void UnRegisterEmi(LPPRC lpprc, HMDI hmdi);
...
RestoreRegs(HPID hpid, HTID htid, HIND hmem);
XOSD SaveRegs(HPID hpid, HTID htid, LPHIND lphmem);
...
ULONG64 SwGetModuleBase(LPVOID  lpvhpid, ULONG64   ReturnAddress);
...
MPT MPTFromHthd(HTHD hthd);
MPT MPTFromHprc(HPRC hprc);
PCPU_POINTERS PointersFromMPT(MPT mpt);
PCPU_POINTERS XXPointersFromHprc(HPRC hprc);
PCPU_POINTERS PointersFromHpid(HPID hpid);
