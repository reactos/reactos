...
LPVOID PpcSwFunctionTableAccess(LPVOID lpvhpid, DWORD   AddrBase);
XOSD PpcGetAddr(HPID   hpid, HTID   htid, ADR    adr, LPADDR lpaddr);
XOSD PpcSetAddr(HPID   hpid, HTID   htid, ADR    adr, LPADDR lpaddr);
LPVOID PpcDoGetReg(LPVOID lpregs1, DWORD ireg, LPVOID lpvRegValue);
XOSD PpcGetRegValue(HPID hpid, HTID htid, DWORD ireg, LPVOID lpvRegValue);
LPVOID PpcDoSetReg(LPVOID   lpregs1, DWORD    ireg, LPVOID   lpvRegValue);
XOSD PpcSetRegValue(HPID hpid, HTID htid, DWORD ireg, LPVOID lpvRegValue);
XOSD PpcDoGetFrame(HPID hpid, HTID uhtid, DWORD wValue, DWORD lValue);
XOSD PpcDoGetFrameEH(HPID hpid, HTID htid, LPEXHDLR *lpexhdlr, LPDWORD cAddrsAllocated);
XOSD PpcUpdateChild(HPID hpid, HTID htid, DMF dmfCommand);
void PpcCopyFrameRegs(LPTHD lpthd, LPBPR lpbpr);
void PpcAdjustForProlog(HPID hpid, HTID htid, PADDR origAddr, CANSTEP *CanStep);
XOSD PpcDoGetFunctionInfo(HPID hpid, LPGFI lpgfi);
...
--*/
{
    ...
    case adrPC:
        if (!fVhtid) {
            AddrInit(lpaddr, 0, 0, (UOFFSET)((PCONTEXT)(lpthd->regs))->Iar, lpthd->fFlat, lpthd->fOff32, FALSE, lpthd->fReal);
        } else {
            AddrInit(lpaddr, 0, 0, (UOFFSET)lpthd->StackFrame.AddrPC.Offset, lpthd->fFlat, lpthd->fOff32, FALSE, lpthd->fReal);
        }
        SetEmi(hpid, lpaddr);
        break;
    case adrBase:
        if (!fVhtid) {
            AddrInit(lpaddr, 0, 0, (UOFFSET)((PCONTEXT)(lpthd->regs))->Gpr1, lpthd->fFlat, lpthd->fOff32, FALSE, lpthd->fReal);
        } else {
            AddrInit(lpaddr, 0, 0, (UOFFSET)lpthd->StackFrame.AddrFrame.Offset, lpthd->fFlat, lpthd->fOff32, FALSE, lpthd->fReal);
        }
        SetEmi(hpid, lpaddr);
        break;
    case adrData:
    {
        ...
        AddrInit(lpaddr, 0, 0, offset, lpthd->fFlat, lpthd->fOff32, FALSE, lpthd->fReal);
        SetEmi(hpid, lpaddr);
    }
    break;
    case adrTlsBase:
        ...
        if (hemi != emiAddr(lpthd->addrTls)) {
            ...
            SendRequestX(dmfQueryTlsBase, hpid, htid, sizeof(lpmdi->lpBaseOfDll), &lpmdi->lpBaseOfDll);
    ...
    case adrStack:
        if (!fVhtid) {
            ...
            AddrInit(lpaddr, 0, 0, (UOFFSET)lpthd->StackFrame.AddrStack.Offset, lpthd->fFlat, lpthd->fOff32, FALSE, lpthd->fReal);
        }
        SetEmi(hpid, lpaddr);
        break;
    default:
...
XOSD PpcSetAddrFromCSIP(HTHD hthd)
{
    ADDR addr = {0};
    LPTHD lpthd;
...
/*++
Routine Description:
    This routine is used to extract the value of a single register from the debuggee.
Arguments:
...
Return Value:
    return-value - lpvRegValue + size of register on sucess and NULL on failure
--*/
{
