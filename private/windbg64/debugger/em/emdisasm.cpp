    ...
    )
{
    ...
    xosd = GetRegValue(hpid, HtidFromHthd(hthd), CvRegFromSimpleReg(MPTFromHthd(hthd), regInstr), &retVal);
    ...
    )
{
    ...
    // We will get regrel calls on registers that the compiler does not use as a frame register.
...
Arguments:
    ppchOut - Supplies a pointer to a pointer to the position of the next entry in the string.
        Returns the new position after this string is added.
    pichCur - Supplies a pointer to the current index in the string.  Returns the new index.
    pcchMax - Supplies a pointer to the number of characters available in the string.  Returns the new value of same.
    string - Supplies the string to add to the list.
    PackSpaces - Supplies flag telling it to remove spaces from the string.
Return Value:
    Index to beginning of string (original ichCur).  Returns -1 if the string could not be added.
--*/
{
    int r = -1;
        ...
        if (dop & dopEA) {
            ...
            if (lpsdi->cbEA0) {
                if (!Sdis.cbMemref) {
                    ...
                    GetAddrOff(addr) = (sizeof(Sdis.dwEA0)==8) ? Sdis.dwEA0 : SE32To64(Sdis.dwEA0);
                    xosd = ReadBuffer (hpid, htid, &addr, Sdis.cbMemref, rgb, &cb);
                    if (xosd == xosdNone) {
...
/*++
Routine Description:
    This will find the instruction which ends nearest to the supplied address without consuming the supplied address.

    On machines with fixed instruction size and alignment, this is trivially accomplished with arithmetic.

    On machines with variable instruction size, this is done by disassembling instructions until a closest fit is found.

    This implementation will not consume the supplied address, but will accept either a) the first (longest) match which consumes the byte before the address, or b) the match which ends closest to the address.
...
--*/
{
    ...
    if (GetAddrOff(*lpgpis->lpaddr) == 0) {
        ...
        *(lpgpis->lpuoffset) = GetAddrOff(*(lpgpis->lpaddr)) & 3;
        SetAddrOff((lpgpis->lpaddr), GetAddrOff(*(lpgpis->lpaddr)) - *(lpgpis->lpuoffset));
        ...
        while (cbTry > 0) {
            ...
            while (1) {
                int Bytes = SimplyDisassemble(pbCurr, cbCurr, (DWORD)currOffset, arch, &Sdis, EmCchAddr, EmCchFixup, EmCchRegrel, EmQwGetreg, (PVOID)hthd);
                if (Bytes < 0) {
