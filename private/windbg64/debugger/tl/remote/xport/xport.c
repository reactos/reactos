...
Abstract:
    This module contains the code for the named pipe transport layer which explicitly deals with the machanics of doing named pipes.
...
BOOL TlpWaitForThreadsToExit(void);
LPTLIS TlGetInfo(VOID);
...
BOOL DllVersionMatch(HANDLE hMod, LPSTR pType)
{
    ...
    if (!pVerProc) {
        ...
        pavs = (*pVerProc)();
        if ((pType[0] != pavs->rgchType[0] || pType[1] != pavs->rgchType[1]) || (Avs.rlvt != pavs->rlvt) || (Avs.iRmj != pavs->iRmj)) {
            return(FALSE);
        }
    }
...
XOSD LoadDM(HPID hpid, LPLOADDMSTRUCT lds)
/*++
Routine Description:
...
XOSD LoadRemoteDM(HPID hpid, LPLOADDMSTRUCT lds)
{
    DWORD cb;
    XOSD xosd;
    LPSTR p;
    ...
    cb = ibMaxDM;
    xosd = SendRequest(mtypeLoadDMReply, PnlblkRq, PnlblkRq->cchMessage + sizeof(NLBLK), lpbDM, &cb, 60);
    if (xosd == xosdNone) {
        xosd = *(XOSD*)lpbDM;
    }
...
XOSD EXPENTRY TLFunc(TLF   wCommand, HPID  hpid, DWORD64 wParam, DWORD64 lParam)
/*++
Routine Description:
...
Return Value:
    XOSD error code.  xosdNone means that no errors occured.  Other error codes are defined in osdebug\include\od.h.
--*/
{
    int        cb;
    LPSTR      p;
    ...
    case tlfDebugPacket:
        ...
        if (!FConnected) {
            ...
            pMpckt->packetCount = (((short)wParam + MAX_INTERNAL_PACKET - 1) / MAX_INTERNAL_PACKET);
            while (wParam > MAX_INTERNAL_PACKET) {
                memcpy(pMpckt->rgchData, (LPB)lParam, MAX_INTERNAL_PACKET);
                xosd = SendData(Pnlblk, sizeof(NLBLK) + sizeof(MPACKET) + MAX_INTERNAL_PACKET);
                if (xosd != xosdNone) {
                    return xosdUnknown;
                }
    ...
    case tlfReply:
        ...
        if (!FConnected) {
            ...
            while (wParam > MAX_INTERNAL_PACKET) {
                memcpy(pMpckt->rgchData, (LPB)lParam, MAX_INTERNAL_PACKET);
                xosd = SendData(PnlblkRp, sizeof(NLBLK) + sizeof(MPACKET) + MAX_INTERNAL_PACKET);
                if (xosd != xosdNone) {
                    return xosdUnknown;
                }
                wParam -= MAX_INTERNAL_PACKET;
    ...
    case tlfRequest:
        ...
        if (!FConnected) {
            ...
            if (wParam > MAX_INTERNAL_PACKET) {
                ...
                while (wParam > MAX_INTERNAL_PACKET) {
                    memcpy(pMpckt->rgchData, (LPB)lParam, MAX_INTERNAL_PACKET);
                    xosd = SendData(PnlblkRq, sizeof(NLBLK) + sizeof(MPACKET) + MAX_INTERNAL_PACKET);
                    if (xosd != xosdNone) {
                        return xosdUnknown;
                    }
                    wParam -= MAX_INTERNAL_PACKET;
                ...
                memcpy(pMpckt->rgchData, (LPB)lParam, (size_t)wParam);
                xosd = SendRequest(mtypeReply, PnlblkRq, sizeof(NLBLK) + sizeof(MPACKET) + (int)wParam, lpbDM, &cb, TLFuncSendReplyTimeout);
            } else {
                ...
                xosd = SendRequest(mtypeReply, PnlblkRq, (int)wParam + sizeof(NLBLK), lpbDM, &cb, TLFuncSendReplyTimeout);
            }
        }
        break;
    case tlfGetVersion:
        DEBUG_OUT("TlFunc:  tlfGetVersion\n");
        {
            ...
            xosd = SendRequest(mtypeVersionReply, Pnlblk, sizeof(NLBLK), (void *)lParam, &cb, 5);
            if (xosd == xosdNone) {
            } else {
    ...
    case tlfSendVersion:
        ...
        if (!FConnected) {
            ...
            if (HDm) {
                DBGVERSIONPROC verproc = (DBGVERSIONPROC)GetProcAddress(HDm, DBGVERSIONPROCNAME);
                dmavs = *verproc();
            } else {
                dmavs = Avs;
            }
