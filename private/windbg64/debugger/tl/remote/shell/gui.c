    ...
    )
{
    ...
    case WM_COMMAND:
        ...
        case IDM_OPTIONS_DEBUG_DLLS:
            ...
            //  of a new TL.
            if (g_RM_Windbgrm_WkSp.m_pszSelectedTL && pszTlName && strcmp(pszTlName, g_RM_Windbgrm_WkSp.m_pszSelectedTL)) {
                char szWarning[MAX_MSG_TXT];

                Dbg(LoadString(g_hInst, IDS_Sys_Warning, szWarning, sizeof(szWarning)));
                WKSP_MsgBox(szWarning, IDS_Sys_Default_TL_Overriden, pszTlName, g_RM_Windbgrm_WkSp.m_pszSelectedTL);
            }

            DialogBox( g_hInst, MAKEINTRESOURCE(IDD_DLG_WINDBGRM_TRANSPORTLAYER), hwnd, TransportLayersDlgProc);
        ...
        case WM_SYSCOMMAND:
            if (wParam == IDM_STATUS) {
                ...
                MessageBox( hwnd, s, "WinDbgRm Connection Status", MB_OK | MB_ICONINFORMATION);
            }
            break;
        case WM_DESTROY:
            ExitProcess( 0 );
...
void EnableTransportChange(BOOL Enable)
{
    HMENU hMenu = GetSystemMenu( HWndFrame, FALSE );
    EnableMenuItem(hMenu, IDM_OPTIONS_DEBUG_DLLS, Enable ? MF_ENABLED : MF_DISABLED);
}