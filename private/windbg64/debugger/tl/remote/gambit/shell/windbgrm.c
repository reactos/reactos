...
Abstract:
    This file implements the WinDbg remote debugger that doesn't use any USER32.DLL or csrss functionality one initialized.
...
DWORD MessagePumpThread(LPVOID lpvArg)
{
    MSG       msg;
    WNDCLASS  WndClass = {0};
    HMENU     hMenu;
...
BOOL LoadTransport(LPTRANSPORT_LAYER lpTl)
/*++
Routine Description:
    Loads the named pipes transport layer and does the necessary initialization of the TL.
Arguments:
    None.
Return Value:
    None.
...
void ShutdownAndUnload(LPTRANSPORT_LAYER lpTl)
{
