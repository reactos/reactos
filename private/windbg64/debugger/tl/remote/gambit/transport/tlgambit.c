...
BOOL TlDisconnectTransport(VOID)
{
    FConnected = FALSE;
    return TRUE;
}
...
BOOL TlFlushTransport(VOID)
{
    return FlushFileBuffers( TlComPort );
}


DWORD ReaderThread(LPVOID     lpvArg)
/*++
Routine Description:
    This is the main function for the reader thread in this transport layer.
    Its sole purpose is to pull things from the transport queue as fast as possible and place them into an internal queue.
    This will prevent us from getting piled up in the network queue to fast.
Arguments:
...
#endif
        if ((pnlblk->mtypeBlk == mtypeVersionReply) || (pnlblk->mtypeBlk == mtypeReply) || (pnlblk->mtypeBlk == mtypeLoadDMReply)) {
...
XOSD EXPENTRY TLSetup(LPTLSS lptlss)
{
    // struct _TLSS {
    // DWORD fLoad;
    // DWORD fInteractive;
