...
char *  RgSzTypes[] = {"FirstAsync", "Async", "FirstReply", "Reply", "Disconnect", "VersionRequest", "VersionReply"};
...
void TlSetRemoteStatus(MPT     mpt)
{
    Tlis.mptRemote = mpt;
    FDMSide = TRUE;
}
...
void printel( void )
{
    ...
    for (i=0; i<ei; i++) {
        DebugPrint( "%d\t%d\t%x\t%d\t%08x\t%08x\t%x\n", el[i].ty, el[i].ec, el[i].cb, el[i].ln, el[i].p, el[i].ob, el[i].td);
    }
}
...
int FAR PASCAL CopyString(LPSTR * lplps, LPSTR lpT, char  chEscape, BOOL  fQuote);


BOOL CreateStuff(VOID)
{
    int i;
...
BOOL StartWorkerThreads(BOOL fStartControlReader)
{
    DWORD       id;
...
VOID DestroyStuff(VOID)
{
    ...
    //  If there is a reader thread -- then wait for it to be terminated and close the handle
    if (HReadThread) {
        TerminateThread( HReadThread, 0 );
    ...
    //  If there is a control thread -- then wait for it to be termianted and close the handle
    if (HControlReadThread) {
        TerminateThread( HControlReadThread, 0 );
...
XOSD TlCreateTransport(LPSTR szName)
/*++
Routine Description:
...
--*/
{
    ...
    _snprintf(RgchPipeName, sizeof(RgchPipeName), PIPE_NAME_FORMAT, ".", szName ? szName : DEFAULT_PIPE);
...
--*/
{
    ...
    if (!FPipeConnected) {
        ...
        status = WaitForSingleObject( OverlappedPipe.hEvent, MAX_CONNECT_WAIT * 1000 );
        switch ( status ) {
            case WAIT_OBJECT_0:
                goto connected;
            case WAIT_TIMEOUT:
            ...
            default:
                ec = GetLastError();
                DPRINT(("PLPIPE: ConnectNamedPipe failed, Status %u, ec=%u\n", status, ec));
...
XOSD TlCreateClient(LPSTR szName)
{
    ...
    if ( *SzRemoteHostName == 0 ) {
        _snprintf( RgchPipeName, sizeof(RgchPipeName), PIPE_NAME_FORMAT, DEFAULT_SERVER, DEFAULT_PIPE );
    } else {
        _snprintf(RgchPipeName, sizeof(RgchPipeName), PIPE_NAME_FORMAT, SzRemoteHostName, SzRemotePipeName);
    }
...
BOOL TlWriteTransport(PUCHAR  pch, DWORD   cch)
{
...
DWORD ReadFromPipe(PUCHAR  pch, DWORD   cch)
{
    DWORD       dwBytesRead;
    DWORD       ec;
...
Routine Description:
    ...
    Its sole purpose is to pull things from the named pipe queue as fast as possible and place them into an internal queue.
...
XOSD TlDoTlSetup(LPTLSS lptlss)
{
    PTSTR lpsz;
