...
DWORD MessageBoxThread(LPMESSAGEBOX_INFO lpMbi)
{
...
VOID TlControlInitialization(VOID)
/*++
Routine Description:
...
XOSD TlCreateControlPipe(LPSTR  szName)
/*++
Routine Description:
...
--*/
{
    ...
    for (i=0; i<MAX_PIPES; i++) {
        ...
        ci[i].hPipe = CreateNamedPipe( ci[i].szName,
...
XOSD TlCreateClientControlPipe(LPSTR  lpHostName, LPSTR  lpPipeName)
/*++
Routine Description:
    This function creates the pipe which will be connected to windbgrm (server).
Arguments:
...
--*/
{
    ...
    sprintf( ControlClient(szName), "\\\\%s\\pipe\\%scontrol", lpHostName, lpPipeName);
    ...
    while ((ControlClient(hPipe) == INVALID_HANDLE_VALUE) && (TlUtilTime() < timeOut)) {
        WaitNamedPipe( ControlClient(szName), 10000 );// wait for the server to make a pipe available...
        ...
        ControlClient(hPipe) = CreateFile( ControlClient(szName),
                             ...
                             NULL );
        if (ControlClient(hPipe) == INVALID_HANDLE_VALUE && GetLastError() == ERROR_BAD_NETPATH) {
    ...
    if (cp->Response) {
        ...
        // the server did not give us permission to open a debugger connection
        ControlClient(fConnected) = FALSE;
        CloseHandle( ControlClient(hPipe) );
...
XOSD TlConnectControlPipe(VOID)
{
    DWORD               ec;
    DWORD               status;
    LPSTR               NewClientId;
    ...
    ControlConnect(fConnected) = ConnectNamedPipe( ControlConnect(hPipe), &ControlConnect(olConnect) );
    ...
    if (!ControlConnect(fConnected)) {
        ...
        status = WaitForSingleObject( ControlConnect(olConnect.hEvent), MAX_CONNECT_WAIT * 1000 );
        switch ( status ) {
            case WAIT_OBJECT_0:
                goto connected;
            case WAIT_TIMEOUT:
    ...
    )
{
    ...
    if (WriteFile(ci[ciIdx].hPipe, pch, cch, &dwBytesWritten, &ci[ciIdx].olWrite )) {
    ...
    )
{
    ...
    if (ReadFile(ci[ciIdx].hPipe, pch, cch, &dwBytesRead, &ci[ciIdx].olRead)) {
        return dwBytesRead;// Read was successful and finished return packet size and exit.
    }
...
VOID ControlPipeFailure(VOID)
{
    TlDisconnectControl( CiClient );
}


BOOL TlDisconnectControl(DWORD   ciIdx)
{
    BOOL    Ok = TRUE;
    DWORD   Error;
...
DWORD ControlReaderThread(LPVOID lpv)
{
    int                 cb;
    int                 response;
    BYTE                buf[512];
