...
Abstract:
    This implements the local transport layer for OSDebug versions 2 and 4 on Win32.
...
void ClearReply(DWORD n)
{
    Flag[n] = FALSE;
}


int TestReply(DWORD n)
{
    return Flag[n];
}


void SetReply(DWORD n)
{
    Flag[n] = TRUE;
}
...
BOOL DllVersionMatch(HANDLE hMod, LPSTR pType)
{
    ...
    if (!pVerProc) {
        ...
        if ((pType[0] != pavs->rgchType[0] || pType[1] != pavs->rgchType[1]) || (Avs.iApiVer != pavs->iApiVer) || (Avs.iApiSubVer != pavs->iApiSubVer)) {
            return(FALSE);
        }
    }
...
XOSD WINAPI TLFunc (TLF wCommand, HPID hpid, DWORD64 wParam, DWORD64 lParam)
/*++
Routine Description:
    This function contains the dispatch loop for commands comming into the transport layer.
    The address of this procedure is exported to users of the DLL.
Arguments:
...
Return Value:
    XOSD error code.  xosdNone means that no errors occured.
    Other error codes are defined in osdebug\include\od.h.
--*/
{
    XOSD xosd = xosdNone;
...
XOSD WINAPI DMTLFunc (TLF wCommand, HPID hpid, DWORD64 wParam, DWORD64 lParam)
{
    XOSD xosd = xosdNone;
...
int WINAPI DllMain(HANDLE hModule, DWORD dwReason, DWORD dwReserved)
{
        switch(dwReason) {
