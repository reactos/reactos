...
retry:
    ...
    switch (ImageType) {
        ...
        case ROM_IMAGE:
            ...
            SecHdr = (PIMAGE_SECTION_HEADER) MHAlloc(ul);// Note: SecHdr is free'd by LoadOmfForReal.
            lpexg->machine = pfile->Machine;// Capture the architecture of the image file
            ...
            if (ImageType == PE_IMAGE) {
                    ...
                    (pfile->Machine == IMAGE_FILE_MACHINE_IA64))
                {
                    for (ul=0; ul < cObjs; ul++) {
                        if (_tcscmp((char *) SecHdr[ul].Name, ".pdata") == 0) {
                            LoadPdata(lpexg, hfile, dllLoadAddress, ImageBase, SecHdr[ul].PointerToRawData, SecHdr[ul].SizeOfRawData, FALSE);
                ...
                // If the debug info has been stripped, close this handle and look for the .dbg file...
                ...
                for (ul=0; ul < cObjs; ul++) {
                    if ((SecHdr[ul].VirtualAddress <= pehdr.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].VirtualAddress) &&
                        (pehdr.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].VirtualAddress < SecHdr[ul].VirtualAddress + SecHdr[ul].SizeOfRawData))
                    {
                        ...
                        // creating a new MISC debug entry and relocating the directory to the DEBUG section...
                        ...
                        cDebugDir = pehdr.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].Size / sizeof(IMAGE_DEBUG_DIRECTORY);
                        break;
                    }
                }
            } else {
                ...
                for (ul=0; ul < cObjs; ul++) {
                    if (!_tcscmp((char *)SecHdr[ul].Name, ".rdata")) {
                        ...
                        // The linker stores an empty directory entry for ROM images to terminate the list.
            ...
            if (ul == cObjs) {
                // We didn't find any CV info.  Try converting what we did find.
                if (pfConvertSymbolsForImage) {
            ...
            for (ul=0; ul < cDebugDir; ul++) {
                ...
                if (dbgDir.Type == IMAGE_DEBUG_TYPE_OMAP_FROM_SRC || dbgDir.Type == IMAGE_DEBUG_TYPE_OMAP_TO_SRC) {
        ...
        case NTDBG_IMAGE:
            ...
            lpexg->machine = sepHdr.Machine; // Capture the architecture of the image file
            SYSeek(hfile, sepHdr.ExportedNamesSize, SEEK_CUR);// Skip over the exported names.
            ZeroMemory(&cvDbgDir, sizeof(cvDbgDir));// Look for the interesting debug data.
            ...
            for (ul=0; ul < (sepHdr.DebugDirectorySize/sizeof(dbgDir)); ul++) {
                ...
                switch (dbgDir.Type) {
                    ...
                    case IMAGE_DEBUG_TYPE_EXCEPTION :
                        ...
                        // that we've already processed the pdata from the real image...
                        LoadPdata(lpexg, hfile, dllLoadAddress, ImageBase, dbgDir.PointerToRawData, dbgDir.SizeOfData, TRUE);
    ...
    // O.K.  Everything's loaded.  If we're looking at a pdb file, load it and get out.
    if ((*(LONG UNALIGNED *)(Signature.Signature)) == sigNB10) {
        sheRet = LoadPdb(lpexg, &pdbInfo, ImageBase, !SYIgnoreAllSymbolErrors());
    } else {
        // No PDB.
        // If the symbols weren't synthesized, allocate a buffer and copy them in...
        if (!lpexg->fSymConverted) {
            HANDLE hMap;
            HANDLE hFileMap;

            hFileMap = CreateFile(lpexg->lszDebug, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
            if (hFileMap != INVALID_HANDLE_VALUE) {
                hMap = CreateFileMapping(hFileMap, NULL, PAGE_WRITECOPY, 0, 0, NULL);
                if (hMap != NULL) {
                    ...
                    lpexg->pvSymMappedBase = MapViewOfFile(hMap, FILE_MAP_COPY, 0, dwAllocStart, cbData + dwAllocDiff);
            ...
            if (lpexg->lpbData == NULL) {
                ...
                if ((SYSeek (hfile, lfaBase, SEEK_SET) != lfaBase) || (SYReadFar (hfile, lpexg->lpbData, cbData) != cbData))
                {
