...
#define CW_BUFFER 1024 // 1K ought to do it
    ...
    OLECHAR FAR * ToW(char FAR* lpszA)
    {
        ...
        cb = strlen(lpszA) + 1;
        Assert(cb <= CW_BUFFER);
        SideAssert(MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, lpszA, cb, szBufW, cb) != 0);
        return szBufW;
    }
    ...
    OLECHAR FAR * ToNewW(char FAR* lpszA)
    {
        ...
        SideAssert(MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, lpszA, cb, szW, cb) != 0);
        return szW;
    }

    char FAR * ToA(OLECHAR FAR* lpszW)
    {
        SideAssert(WideCharToMultiByte(CP_ACP, 0, lpszW, -1, (char FAR *)szBufW, CW_BUFFER, NULL, NULL) != 0);
        return (CHAR *) szBufW;
    }
#else //FV_UNICODE_OLE
...
#define ToA(lpszW) lpszW
    ...
    VOID FAR OutputTyplib (CHAR * szTypeLibFile)
    {
...
#ifndef MAC
        // pre-pend ".\" to filename if unqualified name, to work-around OLE path searching bug.
        if (!XStrChr(szTypeLibFile, '\\') && !XStrChr(szTypeLibFile, '/') && *(szTypeLibFile + 1) != ':')
        {   // if unqualified name
        ...
        CHECKRESULT(lpdtlib->SetLcid((typlib.attr.fAttr & fLCID) ? typlib.attr.lLcid : 0));
    ...
    VOID NEAR OutputElems(ICreateTypeInfo FAR* lpdtinfo, LPELEM  pElemList, TYPEKIND    tkind)
    {
        LPELEM  pElem;
        HRESULT res;
    ...
    VOID NEAR UpdateHRefType(ICreateTypeInfo FAR* lpdtinfo, LPTYPE lpTypeBase)
    {
            ...
            // VT_CARRAY
        {
            ...
            lpTypeBase = lpTypePublic(lpTypeBase);// get * to real base type, ignoring non-public typedef's
        }
        ...
        if (lpTypeBase->tdesc.vt == VT_USERDEFINED)
        {
            ...
            CHECKRESULT(lpdtinfo->AddRefTypeInfo(lpTypeBase->lptinfo, &hreftype));// get reference to given typeinfo
    ...
    VOID NEAR OutputAlias(ICreateTypeInfo FAR* lpdtinfo, LPTYPE lpTypeAlias)
    {
        ...
        UpdateHRefType(lpdtinfo, lpTypeAlias);// update the tdesc.hreftype field of the base type if it's a user-defined type.
        CHECKRESULT(lpdtinfo->SetTypeDescAlias(&(lpTypePublic(lpTypeAlias)->tdesc)));// Define the alias:
    }
    ...
    VOID NEAR LoadElemDesc(ICreateTypeInfo FAR* lpdtinfo, ELEMDESC FAR* lpElemDesc, LPELEM pElem)
    {
        ...
        // update the tdesc.hreftype field of the element type if it's a user-defined type.
    ...
    VOID FAR LoadExtTypeLib(LPIMPORTLIB lpImpLib)
    {
        ...
        CHECKRESULT(lptlib->GetTypeComp(&(lpImpLib->lptcomp)));// get * to ITypeComp interface
        CHECKRESULT(lptlib->GetLibAttr(&(lpImpLib->lptlibattr)));// get library attributes
    }
    ...
    VOID FAR CleanupImportedTypeLibs()
    {
        ...
        if (typlib.pEntry) {
            ...
            for (;;)
            {
                ...
                if (pEntry->type.lptinfo != NULL && (pEntry->type.tentrykind & tFORWARD) == 0)
    ...
    // find type defined in an external type library, and add it to the type table if found.
    // lpszLibName is NULL if we're to look in ALL external type libraries.
    LPTYPE FAR FindExtType(LPSTR lpszLibName, LPSTR lpszTypeName)
    {
        LPIMPORTLIB lpImpLib;
        HRESULT res;
        ULONG   lHashVal;
        ...
        if (typlib.pImpLib != NULL)   // if any imported type libraries
        {
            ...
            for (;;)
            {
                ...
                {
                    SETITEMCUR(lpImpLib->lpszFileName);
                    lHashVal = LHashValOfNameSysA(lpImpLib->lptlibattr->syskind, lpImpLib->lptlibattr->lcid, lpszTypeName);
