...
VOID NEAR ParseNewElem(LPELEM FAR * lplpElem, DWORD validElemAttr, DWORD validElemAttr2, LPTYPE lpType);
...
VOID NEAR InitNewEntry (LPENTRY FAR * lplpEntryLast)
{
    LPENTRY lpEntry;
...
#define DISPID_COLLECT (-8)
                ...
                break;
                  default:
                    if ((long)pAttr->lId >= -999 && (long)pAttr->lId <= -500) {
                  break;
                }
        ...
        if ((pAttr->fAttr & (fREQUESTEDIT | fDISPLAYBIND | fDEFAULTBIND)) && !(pAttr->fAttr & fBINDABLE))
        ParseErrorTokLast(PERR_INV_ATTR_COMBO);
    } // if
}
...
LPTYPE NEAR InitIntrinsic(CHAR * szName, VARTYPE vt)
{
    ...
    typlib.pEntry->type.intr.fUnsigned = (vt == VT_UI1 || vt == VT_UINT || vt == VT_UI2 || vt == VT_UI4);
...
#pragma warning(default:4127)
    ...
    if (tentrykind == tSTRUCT || tentrykind == tUNION)
        {    // if 'struct'/'union' keyword specified, then match tags
            if ((lpType->tentrykind & ~tFORWARD) == tentrykind && lpType->structenum.szTag && !FCmpCaseIns(lpType->structenum.szTag, lpszName))
...
// parse an element name (property, struct elem, function parm, etc, handling array references)
VOID NEAR ParseElemName (LPELEM lpElem,WORD fAllow)
{
    ARRAYDESC FAR * lpAD;
    WORD  cDims;
...
VOID NEAR EnsureNoDupElem (LPELEM lpElemLast)
{
    ...
    for (lpElem = (LPELEM)ListFirst(lpElemLast); lpElem != lpElemLast; lpElem = lpElem->pNext)
        ...
        {   // names match
            ...
            if ( (fIdLast != ((lpElem->attr.fAttr & fID) != 0)) || (fIdLast && idElemLast != lpElem->attr.lId) )
            ParseErrorTokLast(PERR_DUP_DEF);
...
VOID NEAR ParseAlias (LPENTRY lpEntry)
{
    ...
    // parse base type
    lpEntry->type.td.ptypeAlias = ParseKnownType(&lpEntry->attr, fAllowInter | fAllowMODULE);
...
VOID NEAR ParseStructEnumUnion (LPENTRY lpEntry, TENTRYKIND tentrykind)
{
    ...
    // ensure attributes valid in this context
    CheckAttrTokLast(&lpEntry->attr, VALID_STRUCT_ENUM_UNION_ATTR, VALID_STRUCT_ENUM_UNION_ATTR2);
...
VOID NEAR ParseNewEnumElem (LPELEM FAR * lplpElem,long *  pEnumVal)
{
    ...
    ParseOptAttr(&lpElem->attr, cVAR);            // parse attributes, if any
    // ensure attributes valid in this context
    CheckAttrTokLast(&lpElem->attr, VALID_ENUM_ELEM_ATTR, VALID_ENUM_ELEM_ATTR2);
...
VOID NEAR ParseConstant (LPELEM lpElem)
{
    LPTYPE lpType;
    long  constVal;
    WORD  vt;
    ...
    // ensure attributes valid in this context
    CheckAttrTokLast(&lpElem->attr, VALID_MODULE_CONST_ATTR, VALID_MODULE_CONST_ATTR2);
...
GotI4:
    ...
    if (cbszConstVal) {
        SideAssert (MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,lpszConstVal,cbszConstVal,bstrVal,cbszConstVal) != 0);
    }
#else //WIN32
...
)
{
    ...
    if ((lpFunc->func.attr.fAttr & fBINDABLE) && !(lpFunc->func.attr.fAttr & fPropBits))
    ParseErrorTokLast(PERR_INV_ATTR_COMBO);

    // ensure attributes valid in this context
    CheckAttr(&lpFunc->func.attr, fValidFuncAttrs[funcType], fValidFuncAttrs2[funcType]);

    // parse function return type
    lpFunc->func.elemType = ParseKnownType(&lpFunc->func.attr, fAllowSAFEARRAY);

    lpTypeRetVal = lpFunc->func.elemType;    // store in case no retval parm

    // if PROPGET specified, return type cannot be VOID
    if ((lpFunc->func.attr.fAttr & fPROPGET) && IsType(lpTypeRetVal, VT_VOID))
...
#pragma warning(default:4127)
        {  // parse a function parameter
            ...
            // parse & validate parm attributes, if any
             ...
             // ensure attributes valid in this context
            CheckAttr(&attr, fValidParmAttrs[funcType], fValidParmAttrs2[funcType]);
            ...
            ParseError(PERR_INV_LCID_USE);
                    ...
                    if (IDLFlagsCur & fLCID && lpFunc->func.attr.fAttr & (fPROPPUT | fPROPPUTREF))
            ...
            if (IDLFlagsCur) {
                ...
                // 'retval' only legal if it's a pointer type this is ensured by the check above that 'out' parms must be pointers

                // But it can't be an alias type, either (due to a typelib.dll limitation (bug?) in the munging code in gdtinfo.cxx -- VBA2 bug #3716).
                // That code must have a pure pointer type.  We're too close to Daytona ship to fix it in typelib, so we impose the restriction here.
...
)
{
    ...
    case VT_PTR:
        ...
        wCompat = GetTypeCompatibility(lpType->ref.ptypeBase, fRecurse);
            ...
            if (lpType->ref.ptypeBase->tdesc.vt != VT_USERDEFINED && (cInd == 1 && (!fPARAM || fARRAY) || cInd == 2 && fPARAM) || (cInd == 2 && (!fPARAM || fARRAY))) {
              wCompat &= ~COMPAT_OA;
            }
    ...
    case VT_SAFEARRAY:
        ...
        return GetTypeCompatibility(lpType->ref.ptypeBase, (WORD)(fRecurse & ~VT_BYREF | VT_ARRAY));
...
VOID NEAR ParseInterface ()
{
    ...
    while (tok.id != RW_RCURLY)
    {
        ...
        {    // a function
            ...
            ParseFunction(lpEntry->inter.funcList, ((lpEntry->attr.fAttr2 & f2OACompatBits) ? FUNC_OAINTERFACE : FUNC_INTERFACE));
        }
    }
...
VOID NEAR ParseDispinterface ()
{
    ...
    {
        ParseNewElem(&elemList, VALID_DISPINTER_PROP_ATTR, VALID_DISPINTER_PROP_ATTR2, NULL);
        ConsumeTok(RW_SEMI, 0);    // ends with a semicolon
...
#define BIT_SOURCE    2
    ...
    ParseOptAttr(&attr, cIMPLTYPE);        // parse attributes, if any
        ...
        // ensure attributes valid in this context
        CheckAttrTokLast(&attr, VALID_COCLASS_INTER_ATTR, VALID_COCLASS_INTER_ATTR2);
...
VOID NEAR GotAttr2 (LPATTR pAttr,DWORD attrbit2)
{
...
// Also ensures that the uuid defined by this entry (if specified) isn't specified by somebody else.
VOID NEAR EnsureNoDupEntries(LPENTRY lpEntryLast)
{
    LPENTRY lpEntry;
    LPSTR   szNameLast;
    ...
    default:
        ...
        if (lpEntryLast->attr.fAttr & fUUID)    // if UUID specified
        {
            ...
            if ((typlib.attr.fAttr & fUUID) && !_fmemcmp(lpUuidLast, typlib.attr.lpUuid, sizeof(GUID)))
    ...
    for (lpEntry = (LPENTRY) ListFirst(typlib.pEntry); lpEntry != lpEntryLast; lpEntry = (LPENTRY) (lpEntry->type.pNext))
    {
        if (lpEntry->type.tentrykind & tIMPORTED) {
            // The following check ensures that you can't have a type definition of the same name as an unqualified reference to a type in another library.
            // While things will work, it's a bit confusing if two id's in the same file refer to to different types just because one comes before the definition of a type.
            // Qualifed references to types in other type libraries don't present any ambiguity.
            ...
            case tUNION:
                ...
                if (szTagLast && lpEntry->type.structenum.szTag && !_fstrcmp(szTagLast, lpEntry->type.structenum.szTag))
            ...
            default:
                ...
                if (lpUuidLast && (lpEntry->attr.fAttr & fUUID) && !_fmemcmp(lpUuidLast, lpEntry->attr.lpUuid, sizeof(GUID)))
                    ParseErrorTokLast(PERR_DUP_UUID);
            }
        }
    }
...
INT FAR FCmpCaseIns (LPSTR str1, LPSTR str2)
{
    return !(CompareStringA(g_lcidCompare, NORM_IGNOREWIDTH | NORM_IGNOREKANATYPE | NORM_IGNORECASE, str1, -1, str2, -1) == 2);
}
...
VOID FAR ListInsert (LPVOID ppList, WORD cbElem)
{
...
BOOL NEAR VerifyLcid(LCID lcid)
{
    ...
    // Call the nlsapi function to compare string.
    // If the compare succeeds then the LCID is valid or else the passed in lcid is invalid.
    // This is because the only reason the comparision will fail is f the lcid is invalid.

    char rgTest[] = "Test\0";

    fValidLcid = (BOOL) (CompareStringA(lcid, NORM_IGNORECASE | NORM_IGNORENONSPACE, rgTest, -1, rgTest, -1)== 2);
    ...
    if (GetLocaleInfoA(lcid, LOCALE_NOUSEROVERRIDE | LOCALE_IDEFAULTANSICODEPAGE, szCodePage, sizeof(szCodePage)) != 0) {
...
VOID NEAR FindIDispatch()
{
    ...
    if (lpTypeIDispatch == NULL) {
        // if not found here, then search all external type libraries for this type definition.
