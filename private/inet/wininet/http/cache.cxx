...
Abstract:
    ...
    Contains HTTP cache-related functions. Cache functions pulled out of read.cxx and send.cxx
...
PRIVATE BOOL FExcludedMimeType(IN LPSTR lpszMimeType, IN DWORD dwMimeTypeSize);
PRIVATE VOID FilterHeaders(IN LPSTR lpszHeaderInfo, OUT LPDWORD lpdwHeaderLen);
...
BOOL HTTP_REQUEST_HANDLE_OBJECT::FCanWriteToCache(VOID)
/*++
Routine Description:
    Determines if we can write this file to the cache
--*/
{
    DEBUG_ENTER((DBG_HTTP, Bool, "HTTP_REQUEST_HANDLE_OBJECT::FCanWriteToCache", NULL));
    ...
    if (GetOpenFlags() & INTERNET_FLAG_SECURE)
    {
        ...
        index = 0;
        while ( FastQueryResponseHeader(HTTP_QUERY_PRAGMA, (LPVOID *) &lpszBuf, &length, index) == ERROR_SUCCESS )
        {
            if (length == NO_CACHE_LEN && strnicmp(NO_CACHE_SZ, lpszBuf, NO_CACHE_LEN) == 0)
            {
                fNoCache = TRUE;
                break;
            }
    ...
    if (IsResponseHttp1_1())
    {
        ...
        while (FastQueryResponseHeader(HTTP_QUERY_CACHE_CONTROL, (LPVOID *) &lpszBuf, &length, index) == ERROR_SUCCESS)
        {
        ...
        index = 0;
        if (FastQueryResponseHeader(HTTP_QUERY_VARY, (LPVOID *) &lpszBuf, &length, index) == ERROR_SUCCESS
            && !(length == USER_AGENT_LEN
               && !strnicmp (lpszBuf, USER_AGENT_SZ, length)) )
        {
            // content-encoding?
            if (FastQueryResponseHeader(HTTP_QUERY_CONTENT_ENCODING, (LPVOID *) &lpszBuf, &length, index) == ERROR_SUCCESS )
            {
                fContentEnc = TRUE;
            }
            fVary = TRUE;
    ...
    if ((GetStatusCode() == HTTP_STATUS_OK) || (GetStatusCode() == 0))
    {
        if (FastQueryResponseHeader(HTTP_QUERY_CONTENT_TYPE, (LPVOID *) &lpszBuf, &length, 0) == ERROR_SUCCESS)
        {
            ...
            {
                DEBUG_PRINT(CACHE, INFO, ("%s Mime Excluded from caching\n", lpszBuf));
                goto check_need_file;
            }
        }
    ...
    else
    {
        ...
        if ((schemeType == INTERNET_SCHEME_FTP) || (schemeType == INTERNET_SCHEME_GOPHER))
        {
            ok = TRUE;
            goto quit;
        }
...
check_need_file:
    ...
    if (GetCacheFlags() & INTERNET_FLAG_NEED_FILE || GetCacheFlags () & INTERNET_FLAG_HYPERLINK)
    {
...
BOOL HTTP_REQUEST_HANDLE_OBJECT::FAddIfModifiedSinceHeader(IN LPCACHE_ENTRY_INFO lpCEI)
/*++
Routine Description:
