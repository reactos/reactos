  ...
  int nCmdShow)          // show state of window);int main(int argc, char** argv)
{
    ...
    lRet = RegCreateKeyEx  (HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\SharedDLLs", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkRef, &dwDisposition);
...
void CRefCount::ValueSet(char *sz_RegSubkey, char *sz_RegValue)
{
    ...
    lRet = RegCreateKeyEx(HKEY_LOCAL_MACHINE, sz_RegSubkey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkRef, &dwDisposition);
    _ASSERT(ERROR_SUCCESS == lRet);
    if (ERROR_SUCCESS != lRet)
    {
        return;
...
//  _ASSERT(FALSE);
   ...
   int cchRenameLine = wsprintfA(szRenameLine,
      ...
      hfilemap = CreateFileMapping(hfile, NULL, PAGE_READWRITE, 0, dwFileSize + cchRenameLine + cchRenameSec, NULL);
      if (hfilemap != NULL) {
