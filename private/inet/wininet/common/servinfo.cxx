...
--*/
{
    ...
    if (hostName.HaveString()) {
        ...
        if (lpServerInfo == NULL) {
            lpServerInfo = new CServerInfo(lpszHostNameLower,dwServiceType);
            if (lpServerInfo != NULL) {
                bCreated = TRUE;
            } else {
    ...
    // name then do it now, outside of the global server info list lock. This operation may take some time
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,Pointer,"FindServerInfo","%q",lpszHostName));
...
Routine Description:
    ...
    Returned info is referenced, so caller must call ReleaseServerInfo() when done
Return Value:
    CServerInfo *
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,Pointer,"FindNearestServer",NULL));
    CServerInfo * lpServerInfo;
...
/*++
Routine Description:
    Release a CServerInfo by dereferencing it.
    If the reference count goes to zero, the CServerInfo will be destroyed
Arguments:
    lpServerInfo    - pointer to CServerInfo to release
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"ReleaseServerInfo","%#x [%q]",lpServerInfo,lpServerInfo->GetHostName()));
...
/*++
Routine Description:
    Throw out any CServerInfo entries that have expired or any KEEP_ALIVE entries (for any CServerInfo) that have expired
Arguments:
    bForce  - TRUE if we forcibly remove entries which have not yet expired but which have a reference count of 1, else FALSE to remove only entries that have expired
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"PurgeServerInfoList","%B",bForce));
...
/*++
Routine Description:
    Throw out any KEEP_ALIVE entries from any CServerInfo that have expired or which have failed authentication or which are unused, depending on dwForce
Arguments:
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,
                 ...
                 "PurgeKeepAlives",
                 "%s [%d]",
                 (dwForce == PKA_NO_FORCE) ? "NO_FORCE" : (dwForce == PKA_NOW) ? "NOW" : (dwForce == PKA_AUTH_FAILED) ? "AUTH_FAILED" : "?",
                 dwForce
                 ));
...
//    If any ping succeeds, this function succeeds. If all pings fail, then this function fails

//    Assumes:    1. global ping object has been instantiated

//Arguments:
//    lpbUnreachable  - TRUE if one or more servers were unreachable (the network seems to be alive, just that we couldn't reach one or more servers.
//                      Useful to indicate that an address we are in the process of resolving is bad
...
//                    There are no known server addresses (there may be items in the list, but the addresses are not yet resolved)

//                  ERROR_INTERNET_PING_FAILED (internal)
//                    We have (resolved) addresses, but couldn't successfully ping any. We believe we have connectivity

//                  ERROR_INTERNET_NO_PING_SUPPORT (internal)
//                    We couldn't ping any addresses because ping support is not loaded, or globally disabled
//--*/
//{
//    DEBUG_ENTER((DBG_SOCKETS,Dword,"PingServerInfoList","%#x",lpbUnreachable));
//    DWORD error;
...
//        err = DestinationAddressFromAddressList(lpServerInfo->GetAddressList(),0,(LPBYTE)&dwIpAddress,&addressLength);
//        INET_ASSERT(IS_VALID_NON_LOOPBACK_IP_ADDRESS(dwIpAddress));
//        if ((err == ERROR_SUCCESS) && IS_VALID_NON_LOOPBACK_IP_ADDRESS(dwIpAddress)) {
...
//                // although the server is unreachable, we still have connectivity (the ping would have failed completely otherwise)
//                error = ERROR_SUCCESS;
...
//*++
//Routine Description:
//    Populates the server info database from the registry. This allows us to avoid server capability negotiation each time we start IE/Wininet
//Return Value:
//    DWORD
//        Success - ERROR_SUCCESS
//        Failure -
...
CServerInfo::CServerInfo(IN LPSTR lpszHostName,IN DWORD dwService,IN DWORD dwMaxConnections)
/*++
Routine Description:
...
Arguments:
    ...
    dwMaxConnections    - maximum number of simultaneous connections to this server
Return Value:
    None.
--*/
{
    ...
    // only initialize the keep-alive and connection limit lists if we are creating the server info entry for a HTTP server (or CERN proxy)
    ...
    //if (IsHTTP()) {
        ...
        // the maximum number of connections per server is initialized to the default (registry) value unless overridden by the caller
...
//dprintf("** %s: limit = %d\n", GetHostName(), m_ConnectionLimit);
    ...
    // add to the global list. We are assuming here that the caller has already checked for dupes
...
--*/
{
    DEBUG_ENTER((DBG_OBJECTS,None,"CServerInfo::~CServerInfo","{%q}",GetHostName()));
...
VOID CServerInfo::Reference(VOID)
/*++
Routine Description:
    Increments the reference count for the CServerInfo
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"CServerInfo::Reference","{%q}",GetHostName()));
...
//dprintf("CServerInfo %s - %d\n", GetHostName(), m_ReferenceCount);
    ...
    DEBUG_PRINT(SESSION,INFO,("Reference count = %d\n",ReferenceCount()));
    DEBUG_LEAVE(0);
}


BOOL CServerInfo::Dereference(VOID)
/*++
Routine Description:
    Dereferences the SESSION_INFO. If the reference count goes to zero then this entry is deleted.
    If the reference count goes to 1 then the expiry timer is started
Return Value:
    BOOL
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"CServerInfo::Dereference","{%q}",GetHostName()));

    CHECK_SERVER_INFO();
    INET_ASSERT(m_ReferenceCount > 0);

    // we need to grab the list - we may be removing this entry or updating the reference count and expiry fields which must be done atomically
    LockSerializedList(&GlobalServerInfoList);
...
/*++
Routine Description:
    If the Version information match up, copies the proxy information and links this server object to the appopriate proxy server object

    Assumes that this is called on successful use of the proxy object.
Arguments:
    None.
Return Value:
    DWORD
...
--*/
{
    ...
    if ( m_ProxyLink )
    {
        if ( IsProxyScriptCached() && HostScheme == m_HostScheme && HostPort == m_HostPort && fUseProxy )
        {
        ...
        // unlink, because we have a new entry to save, and the previous entry is bad
...
CServerInfo * CServerInfo::GetCachedProxyServerInfo(IN INTERNET_SCHEME HostScheme,IN INTERNET_PORT HostPort,OUT BOOL *pfCachedEntry)
/*++
Routine Description:
   Retrieves a cached server object, that indicates a probable proxy to use
   On Success, the return has an additional increment on its ref count, assumition that caller derefs
Arguments:
    None.
Return Value:
    CServerInfo *
...
--*/
{
    ...
    *pfCachedEntry = FALSE;
    if ( IsProxyScriptCached() )
    {
        ...
        if ( GlobalProxyVersionCount == m_dwProxyVersion && HostScheme == m_HostScheme && HostPort == m_HostPort)
        {
            *pfCachedEntry = TRUE;
...
BOOL CServerInfo::CopyCachedProxyInfoToProxyMsg(IN OUT AUTO_PROXY_ASYNC_MSG *pQueryForProxyInfo)
/*++
Routine Description:
...
--*/
{
    ...
    pQueryForProxyInfo->_lpszProxyHostName = m_HostName.StringAddress() ? NewString(m_HostName.StringAddress()) : NULL;
    ...
    if ( pQueryForProxyInfo->_lpszProxyHostName != NULL ) {
        ...
        pQueryForProxyInfo->_dwProxyHostNameLength = strlen((pQueryForProxyInfo)->_lpszProxyHostName);
...
//DWORD CServerInfo::ResolveHostName(IN BOOL bForce)
//*++
//Routine Description:
//    Resolves the host name for a CServerInfo. If we already have a resolved name for this info then it is freed.
//    There can be only one thread updating or using the host name info
//Arguments:
...
//--*/
//{
//    DEBUG_ENTER((DBG_SESSION,Dword,"CServerInfo::ResolveHostName","{%q} %B",GetHostName(),bForce));
//    DWORD error = ERROR_SUCCESS;

//    if (bForce || IsAddressListEmpty(&m_AddressList)) {
//        EnterCriticalSection(&m_AddressListCritSec);

//        // BUGBUG - ideally, we want to test TTL here if bForce is TRUE to stop multiple simultaneous forced requests re-resolving the same info
//        if (bForce || IsAddressListEmpty(&m_AddressList)) {
//            // FreeAddressList() checks for an empty list
//            FreeAddressList(&m_AddressList);

//            // resolve the name & generate an address list.
//            // Since we don't know the port address we want to talk to right now, we will generate a
//            // list containing a default port number (0) which we must override when we really connect to the server
...
VOID CServerInfo::UpdateConnectTime(IN DWORD dwConnectTime)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"CServerInfo::UpdateConnectTime","{%q} %d",GetHostName(),dwConnectTime));
...
//dprintf("%s: connect time = %d, ave = %d\n", GetHostName(), dwConnectTime, connectTime);
    ...
    DEBUG_PRINT(SESSION,INFO,("average connect time = %d mSec\n",connectTime));
    InterlockedExchange((LPLONG)&m_ConnectTime, connectTime);
    DEBUG_LEAVE(0);
}


VOID CServerInfo::UpdateRTT(IN DWORD dwRTT)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"CServerInfo::UpdateRTT","{%q} %d",GetHostName(),dwRTT));
    DWORD RTT = m_RTT;
...
//dprintf("%s: RTT = %d, ave = %d\n", GetHostName(), dwRTT, RTT);
    ...
    DEBUG_PRINT(SESSION,INFO,("average round trip time = %d mSec\n",RTT));
    InterlockedExchange((LPLONG)&m_RTT, RTT);
    DEBUG_LEAVE(0);
}


VOID CServerInfo::UpdateDownloadRate(IN DWORD dwBytesPerSecond)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"CServerInfo::UpdateDownloadRate","{%q} %d",GetHostName(),dwBytesPerSecond));
    ...
    DEBUG_PRINT(SESSION,INFO,("average download rate = %d bytes/second\n",downloadRate));
    InterlockedExchange((LPLONG)&m_DownloadRate, downloadRate);
    DEBUG_LEAVE(0);
}


VOID CServerInfo::UpdateUploadRate(IN DWORD dwBytesPerSecond)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SESSION,None,"CServerInfo::UpdateUploadRate","{%q} %d",GetHostName(),dwBytesPerSecond));
    ...
    DEBUG_PRINT(SESSION,INFO,("average upload rate = %d bytes/second\n",uploadRate));
    InterlockedExchange((LPLONG)&m_UploadRate, uploadRate);
    DEBUG_LEAVE(0);
}


//DWORD CServerInfo::GetConnection(IN DWORD dwSocketFlags,IN INTERNET_PORT nPort,IN DWORD dwTimeout,OUT ICSocket * * lplpSocket)
//*++
//Routine Description:
//    Combines connection limiting and keep-alive list management.
//    If keep-alive connection requested and one is available, it is given out.
//    If we can create a connection, the connection count is increased (actually decremented) and an OK-to-continue indication returned to the caller.
//    If all connections are currently in use, we wait for one to become available.
//    If this is a sync operation, we block waiting for the waiter event.
//    If this is an async operation, we return ERROR_IO_PENDING and pick up the request on a worker thread when it becomes unblocked or times out proper
//Arguments:
...
//    nPort           - required port
//    dwTimeout       - number of milliseconds we are willing to wait for connection to become available
//    lplpSocket      - returned pointer to ICSocket. ONLY used if the request is for a keep-alive socket and we had one available
//Return Value:
//    DWORD
//        Success - ERROR_SUCCESS
//                    Depending on *lplpSocket, we either returned the socket to use, or its okay to create a new connection
...
//--*/
//{
//    DEBUG_ENTER((DBG_SESSION,Dword,"CServerInfo::GetConnection","{%q} %#x, %d, %d, %#x",GetHostName(),dwSocketFlags,nPort,dwTimeout,lplpSocket));
//    *lplpSocket = NULL;
//    DWORD error = DoFsm(new CFsm_GetConnection(dwSocketFlags,nPort,dwTimeout,lplpSocket,this));
//    DEBUG_LEAVE(error);
//    return error;
//}


DWORD CFsm_GetConnection::RunSM(IN CFsm * Fsm)
/*++
Routine Description:
...
//dprintf("%#x: %s FSM %#x state %s\n", GetCurrentThreadId(), Fsm->MapType(), Fsm, Fsm->MapState());
    DEBUG_ENTER((DBG_SESSION,Dword,"CFsm_GetConnection::RunSM","%#x",Fsm));
    ...
    case FSM_STATE_ERROR:
        INET_ASSERT((Fsm->GetError() == ERROR_INTERNET_TIMEOUT) || (Fsm->GetError() == ERROR_INTERNET_OPERATION_CANCELLED));
...
#ifdef NEW_CONNECTION_SCHEME
//DWORD CServerInfo::GetConnection_Fsm(IN CFsm_GetConnection * Fsm)
//*++
//Routine Description:
//    Tries to get a connection of requested type for caller. If no connection is available then one of the following happens:
//        * If there are available keep-alive connections of a different type then one is closed and the caller allowed to create a new connection
//        * If this is an async request, the FSM is blocked and the thread returns to the pool if a worker, or back to the app if an app thread
//        * If this is a sync request, we wait on an event for a conneciton to be made available, or the connect timeout to elapse
//        * In the situation where we are being run out of connections (number of active connections >= connection limit AND no activity has taken place
//          on any connection in the last dwLimitTimeout mSec (in FSM)) then we allow a new connection to be created
//Arguments:
...
//    DEBUG_PRINT(SESSION,INFO,("FSM %#x state %s elapsed %d\n",Fsm,Fsm->MapState(),Fsm->GetElapsedTime()));
//    DPRINTF("%#x: FSM %#x state %s elapsed %d\n",GetCurrentThreadId(),Fsm,Fsm->MapState(),Fsm->GetElapsedTime());
...
//    // FSM_STATE_ERROR processing. Typically a timeout has occurred. We now want to force a new connection instead of timing out
//    if (error != ERROR_SUCCESS) {
//        RemoveWaiter((DWORD)Fsm);
//    }

//    // timeout error is OK - we just try again until we get cancelled out or hit the retry limit.
//    // Any other error causes failure.
//    // We should not be getting ERROR_SUCCESS (test put here for defensive purposes only)
//    if ((state == FSM_STATE_ERROR) && (error != ERROR_INTERNET_TIMEOUT) && (error != ERROR_SUCCESS)) {
//        DPRINTF("%#x: FSM %#x FSM_STATE_ERROR, error = %d\n",GetCurrentThreadId(),Fsm,error);
//        DEBUG_PRINT(SESSION,ERROR,("FSM %#x FSM_STATE_ERROR, error = %s (%d)\n",Fsm,InternetMapError(error),error));
//        INET_ASSERT(error != ERROR_SUCCESS);
//        goto quit;
//    }
...
//    bAsyncRequest = lpThreadInfo->IsAsyncWorkerThread || ((INTERNET_HANDLE_OBJECT *)lpThreadInfo->hObjectMapped)->IsAsyncHandle();

//    // before we do anything, check if we've been run out of connections.
//    // If we don't do this check here and we are out, then we'll wait unnecessarily (assuming connections are not returned)
...
//        DEBUG_PRINT(SESSION,ERROR,("out of connections on first attempt: %d+%d/%d\n",AvailableConnections(),KeepAliveConnections(),ConnectionLimit()));
//        error = ERROR_SUCCESS;
//        pSocket = NULL;
//        goto quit;
//    }
...
//        // maintain requester order - if there are already waiters then queue this request, else try to satisfy the requester.
//        // HOWEVER, only check for existing requesters the FIRST time through.
//        // If we're here with FSM_STATE_CONTINUE then we've been unblocked and we can ignore any waiters that came after us
//        if ((state == FSM_STATE_CONTINUE) || !bKeepAliveWaiters) {
//            DEBUG_PRINT(SESSION,INFO,("continuing or no current waiters for K-A connections\n"));
//            while (pSocket = FindKeepAliveConnection(fsm.m_dwSocketFlags, fsm.m_nPort)) {
//                if (pSocket->IsReset()) {
//                    DPRINTF("%#x: ** socket %#x is closed already\n",GetCurrentThreadId(),pSocket->GetSocket());
//                    DEBUG_PRINT(SESSION,INFO,("K-A connection %#x [%#x] is closed\n",pSocket,pSocket->GetSocket()));
...
//                } else {
//                    DPRINTF("%#x: ** matched %#x, %#x\n",GetCurrentThreadId(),pSocket->GetSocket(),pSocket->GetFlags());
//                    break;
//                }
//            }
//            if (pSocket == NULL) {
//                DEBUG_PRINT(SESSION,INFO,("no available K-A connections\n"));

//                /*
//                // if all connections are in use as keep-alive connections then since we're here, we want a keep-alive connection that doesn't
//                // match the currently available keep-alive connections. Terminate the oldest keep-alive connection (at the head of the queue)
...
//            }
//        } else {
//            DEBUG_PRINT(SESSION,INFO,("%d waiters for K-A connection to %q\n",ElementsOnSerializedList(&m_KeepAliveList),GetHostName()));
//        }
//    }

//    // if we found a matching keep-alive connection or we are not limiting connections then we're done
...
//    // no keep-alive connections matched, or there are already waiters for keep-alive connections
//    INET_ASSERT((AvailableConnections() >= 0) && (AvailableConnections() <= ConnectionLimit()));
//    if (AvailableConnections() > 0) {
//        // can create a connection
//        DEBUG_PRINT(SESSION,INFO,("%s: OK to create new connection: %d/%d\n",GetHostName(),AvailableConnections(),ConnectionLimit()));
//        DPRINTF("%#x: ** %s: OK to create connection: %d/%d\n",GetCurrentThreadId(),GetHostName(),AvailableConnections(),ConnectionLimit());
//        INET_ASSERT(error == ERROR_SUCCESS);
...
//    } else {
//        // if there are keep-alive connections but no keep-alive waiters then either we don't want a keep-alive connection, or the ones
//        // available don't match our requirements.
//        // If we need a connection of a different type - e.g. SSL when all we have is non-SSL then close a connection & generate a new one.
//        // If we need a non-keep-alive connection then its okay to return a current keep-alive connection, the understanding being that the
//        // caller will not add Connection: Keep-Alive header (HTTP 1.0) or will add Connection: Close header (HTTP 1.1)
...
//        // If this situation continues, eventually B will time-out, whereas it could have had the connection taken by D. Request D is younger and
...
//                    DEBUG_PRINT(SESSION,INFO,("different socket types requested: %#x, %#x\n",fsm.m_dwSocketFlags,pSocket->GetFlags()));
//                    DPRINTF("%#x: ** closing socket %#x: %#x vs. %#x\n",GetCurrentThreadId(),pSocket->GetSocket(),pSocket->GetFlags(),fsm.m_dwSocketFlags);
//                    pSocket->SetLinger(FALSE, 0);
//                    pSocket->Shutdown(2);
...
//        // about to wait for a connection. If it looks as though we're being run out of connections, then create a new one
...
//        DPRINTF("%#x: blocking %s FSM %#x state %s %d/%d elapsed: %d mSec\n",GetCurrentThreadId(),Fsm->MapType(),Fsm,Fsm->MapState(),AvailableConnections(),ConnectionLimit(),Fsm->GetElapsedTime());

//        // we have to wait for a connection to become available.
//        // If we are an async request then we queue this FSM & return the thread to the pool or, if app thread, return pending indication to the app.
//        // If this is a sync request (in an app thread) then we block on an event waiting for a connection to become available
//        HANDLE hEvent = NULL;
//        if (!bAsyncRequest) {
...
//#if INET_DEBUG
//        for (pWaiter = (CConnectionWaiter *)HeadOfSerializedList(&m_Waiters); pWaiter != (CConnectionWaiter *)SlSelf(&m_Waiters); pWaiter = (CConnectionWaiter *)pWaiter->Next()) {
...
//                                        !bAsyncRequest,
//                                        (fsm.m_dwSocketFlags & SF_KEEP_ALIVE) ? TRUE : FALSE,
//                                        bAsyncRequest ? (DWORD)Fsm : lpThreadInfo->ThreadId,
//                                        hEvent);
//        if (pWaiter == NULL) {
//            error = ERROR_NOT_ENOUGH_MEMORY;
//            goto exit;
//        }
//        if (bAsyncRequest) {
//            // ensure that when the FSM is unblocked normally, the new state is STATE_CONTINUE
//            DWORD dwWaitTime = min(fsm.m_dwLimitTimeout, fsm.m_dwTimeout);

//            // make wait time elastic: we don't want to time out after the connect timeout value elapses.
//            // We keep expanding the wait time for a connection until we get one, or the request is cancelled
...
//            DPRINTF("%#x: %s FSM %#x %s waiting %d mSec\n",GetCurrentThreadId(),Fsm->MapType(),Fsm,Fsm->MapState(),fsm.m_dwTimeout);

//            //DWORD dwWaitTime = fsm.m_dwTimeout - fsm.GetElapsedTime();
//            DWORD dwWaitTime = fsm.m_dwTimeout;

//            if ((int)dwWaitTime <= 0) {
//                DEBUG_PRINT(SESSION,ERROR,("SYNC wait timed out (%d mSec)\n",fsm.m_dwTimeout));
//                error = ERROR_INTERNET_TIMEOUT;
//            } else {
//                DEBUG_PRINT(SESSION,INFO,("waiting %d mSec for SYNC event %#x\n",dwWaitTime,hEvent));

//                // we'd better not be doing a sync wait if we are in the context of an app thread making an async request
//                INET_ASSERT(!lpThreadInfo->IsAsyncWorkerThread && !((INTERNET_HANDLE_OBJECT *)lpThreadInfo->hObjectMapped)->IsAsyncHandle());
...
//                do {
//                    DPRINTF("%#x: sync wait %d mSec\n",GetCurrentThreadId(),dwDeltaWaitTime);
...
//                        }
//                    }
//                } while (((GetTickCount() - dwTimeStarted) < dwWaitTime) && (error == STATUS_TIMEOUT));
...
//                }
//                */
//                error = RunOutOfConnections() ? WAIT_OBJECT_0 : ERROR_INTERNET_TIMEOUT;
//            //} else {
//            //    bUnlockList = FALSE;
//            }
...
//exit:
//    // if we are returning a (keep-alive) socket that has a different blocking mode from that requested, change it
//    if (pSocket != NULL) {
//        if ((pSocket->GetFlags() & SF_NON_BLOCKING) ^ (fsm.m_dwSocketFlags & SF_NON_BLOCKING)) {
//            DEBUG_PRINT(SESSION,INFO,("different blocking modes requested: %#x, %#x\n",fsm.m_dwSocketFlags,pSocket->GetFlags()));
...
//    If we will break the connection limit then we assume we created an additional connection when we were being starved, and we discard this one
//Arguments:
...
//    // quite often (at least with catapult proxy based on IIS) the server may drop the connection even though it indicated it would keep it open.
//    // This typically happens on 304 (frequent) and 302 (less so) responses.
//    // If we determine the server has dropped the connection then throw it away and allow the app to create a new one
//    if (lpSocket != NULL) {
//        if (TotalAvailableConnections() >= ConnectionLimit()) {
//            DPRINTF("%#x: !!! too many K-A connections %d+%d/%d - discarding %#x\n",GetCurrentThreadId(),AvailableConnections(),KeepAliveConnections(),ConnectionLimit(),lpSocket->GetSocket());

//            // trying to return keep-alive socket would overflow limit (if we are not already over).
//            // If we have less than the max. keep-alive connections, we will keep this one else close it
//            if (KeepAliveConnections() >= ConnectionLimit()) {
//                INET_ASSERT(KeepAliveConnections() == ConnectionLimit());
//                // BUGBUG - discarding k-a connection: it should be the oldest (if oldest not this one)
...
//            lpSocket = NULL;
//        } else {
//            // if we are returning a keep-alive socket, put it in non-blocking mode if not already.
//            // Typically, Internet Explorer uses non-blocking sockets.
//            // In the infrequent cases where we want a blocking socket
...
//    }
//    if (lpSocket != NULL) {
//        DPRINTF("%#x: releasing K-A %#x (%d/%d)\n",GetCurrentThreadId(),lpSocket ? lpSocket->GetSocket() : 0,AvailableConnections(),ConnectionLimit());
...
//        // The reason we do this is that we must free a waiter if there are any but the waiter corresponding to pWaiter may have been concurrently timed out and cannot be unblocked by us
//        do {
...
//                        INET_ASSERT(n == 1);
//                        bFreed = TRUE;
//                        DPRINTF("%#x: unblocked waiting FSM %#x\n",GetCurrentThreadId(),pWaiter->Id());
//                        DEBUG_PRINT(SESSION,INFO,("unblocked waiting FSM %#x\n",pWaiter->Id()));
//                    } else {
//                        // should never be < 0
//                        INET_ASSERT(n == 0);
//                        DPRINTF("%#x: ** waiting FSM %#x NOT UNBLOCKED\n",GetCurrentThreadId(),pWaiter->Id());
//                        DEBUG_PRINT(SESSION,ERROR,("waiting FSM %#x not unblocked\n",pWaiter->Id()));
//                    }
//                }
//                delete pWaiter;
//            } else {
...
/*++
Routine Description:
    Tries to get a connection of requested type for caller.
    If no connection is available then one of the following happens:
        * If there are available keep-alive connections of a different type then one is closed and the caller allowed to create a new connection
        * If this is an async request, the FSM is blocked and the thread returns to the pool if a worker, or back to the app if an app thread
        * If this is a sync request, we wait on an event for a connection to be made available, or the connect timeout to elapse
Arguments:
    Fsm - get connection FSM
Return Value:
    DWORD
...
--*/
{
    ...
    INET_ASSERT(lpThreadInfo != NULL);
    INET_ASSERT(lpThreadInfo->hObjectMapped != NULL);
    INET_ASSERT(((HANDLE_OBJECT *)lpThreadInfo->hObjectMapped)->GetHandleType() == TypeHttpRequestHandle);
...
try_again:
    ...
    // use m_Waiters to serialize access. N.B. - we will acquire m_KeepAliveList from within m_Waiters
    m_Waiters.Acquire();
    if (IsNewLimit()) {
    ...
    if (fsm.m_dwSocketFlags & SF_KEEP_ALIVE) {
        ...
        if ((fsm.GetState() == FSM_STATE_CONTINUE) || !bKeepAliveWaiters) {
            DEBUG_PRINT(SESSION,INFO,("no current waiters for K-A connections\n"));
            while (pSocket = FindKeepAliveConnection(fsm.m_dwSocketFlags, fsm.m_nPort)) {
                if (pSocket->IsReset() || pSocket->HasExpired()) {
                    DPRINTF("%#x: %#x: ** socket %#x is closed already\n",GetCurrentThreadId(),Fsm,pSocket->GetSocket());
                    DEBUG_PRINT(SESSION,
                                INFO,
...
//dprintf("GetConnection: destroying reset socket %#x\n", pSocket->GetSocket());
                    ...
                    DPRINTF("%#x: %#x: ** matched %#x, %#x\n",GetCurrentThreadId(),Fsm,pSocket->GetSocket(),pSocket->GetFlags());
                    break;
                }
            }
            if (pSocket == NULL) {
                ...
                // the oldest keep-alive connection (at the head of the queue) and generate a new connection
    ...
    if (m_ConnectionsAvailable > 0) {
        // can create a connection
        DEBUG_PRINT(SESSION,INFO,("OK to create new connection\n"));
        DPRINTF("%#x: %#x: ** %s OK to create connection %d/%d\n",GetCurrentThreadId(),Fsm,GetHostName(),m_ConnectionsAvailable,m_ConnectionLimit);
        --m_ConnectionsAvailable;
    } else if (fsm.GetElapsedTime() > fsm.m_dwTimeout) {
        error = ERROR_INTERNET_TIMEOUT;
    } else {
        // if there are keep-alive connections but no keep-alive waiters then either we don't want a keep-alive connection, or the ones available don't match our requirements.
        // If we need a connection of a different type - e.g.
        // SSL when all we have is non-SSL then close a connection & generate a new one.
        // If we need a non-keep-alive connection then its okay to return a current keep-alive connection, the understanding being that the
        // caller will not add Connection: Keep-Alive header (HTTP 1.0) or will add Connection: Close header (HTTP 1.1)
        ...
        // If this situation continues, eventually B will time-out, whereas it could have had the connection taken by D.
        // Request D is younger and therefore can afford to wait while B continues with the connection
        BOOL fHaveConnection = FALSE;
...
#define SOCK_FLAGS  (SF_ENCRYPT | SF_DECRYPT | SF_SECURE)
                DWORD dwSocketTypeFlags = pSocket->GetFlags() & SOCK_FLAGS;
                DWORD dwRequestTypeFlags = fsm.m_dwSocketFlags & SOCK_FLAGS;
                if ((dwSocketTypeFlags ^ dwRequestTypeFlags) || (fsm.m_nPort != pSocket->GetPort())) {
                    DEBUG_PRINT(SESSION,
                                INFO,
...
//dprintf("GetConnection: destroying different type socket %#x\n", pSocket->GetSocket());
        ...
        DPRINTF("%#x: %#x: blocking %s FSM %#x state %s %d/%d\n",GetCurrentThreadId(),Fsm,Fsm->MapType(),Fsm,Fsm->MapState(),m_ConnectionsAvailable,m_ConnectionLimit);
        ...
        pWaiter = new CConnectionWaiter(&m_Waiters,
                                        !bAsyncRequest,
                                        (fsm.m_dwSocketFlags & SF_KEEP_ALIVE) ? TRUE : FALSE,
                                        bAsyncRequest ? (DWORD_PTR)Fsm : lpThreadInfo->ThreadId,
                                        hEvent,

                                        // priority in request handle object controls relative position in list of waiters
                                        ((HTTP_REQUEST_HANDLE_OBJECT *) lpThreadInfo->hObjectMapped)->GetPriority()
                                        );
        ...
        if (bAsyncRequest) {
            // ensure that when the FSM is unblocked normally, the new state is STATE_CONTINUE
            Fsm->SetState(FSM_STATE_CONTINUE);
            error = BlockWorkItem(Fsm,(DWORD_PTR)pWaiter,fsm.m_dwTimeout);
            if (error == ERROR_SUCCESS) {
                error = ERROR_IO_PENDING;
            }
        } else {
            ...
            DPRINTF("%#x: %#x: %s FSM %#x %s waiting %d msec\n",GetCurrentThreadId(),Fsm,Fsm->MapType(),Fsm,Fsm->MapState(),fsm.m_dwTimeout);
            DWORD dwWaitTime = fsm.m_dwTimeout - fsm.GetElapsedTime();
            if ((int)dwWaitTime <= 0) {
                DEBUG_PRINT(SESSION,ERROR,("SYNC wait timed out (%d mSec)\n",fsm.m_dwTimeout));
                error = ERROR_INTERNET_TIMEOUT;
...
exit:
    ...
    if (pSocket != NULL) {
        if ((pSocket->GetFlags() & SF_NON_BLOCKING) ^ (fsm.m_dwSocketFlags & SF_NON_BLOCKING)) {
            DEBUG_PRINT(SESSION,INFO,("different blocking modes requested: %#x, %#x\n",fsm.m_dwSocketFlags,pSocket->GetFlags()));
            DPRINTF("%#x: %#x: ** changing socket %#x to %sBLOCKING\n",GetCurrentThreadId(),Fsm,pSocket->GetSocket(),fsm.m_dwSocketFlags & SF_NON_BLOCKING ? "NON-" : "");
...
//dprintf("ReleaseConnection: destroying already closed socket %#x\n", lpSocket->GetSocket());
            ...
            // Typically, Internet Explorer uses non-blocking sockets.
            // In the infrequent cases where we want a blocking socket
            // - mainly when doing java downloads - we will convert the socket to blocking mode when we get it from the pool
            if (!lpSocket->IsNonBlocking()) {
                DPRINTF("%#x: ** WARNING: releasing BLOCKING k-a socket %#x\n",GetCurrentThreadId(),lpSocket->GetSocket());
                if (!(GlobalRunningNovellClient32 && !GlobalNonBlockingClient32)) {
                    lpSocket->SetNonBlockingMode(TRUE);
...
ICSocket * CServerInfo::FindKeepAliveConnection(IN DWORD dwSocketFlags, IN INTERNET_PORT nPort)
/*++
Routine Description:
...
--*/
{
    ...
    // don't check whether socket is non-blocking - we only really want to match on secure/non-secure.
    // Possible flags to check on are:
    ...
    for (pEntry = HeadOfSerializedList(&m_KeepAliveList); pEntry != (PLIST_ENTRY)SlSelf(&m_KeepAliveList); pEntry = pEntry->Flink) {
        ...
        // We make sure the socket we request is the correct socket, Match() is a bit confusing and needs a bit of explaining,
        //  Match IS NOT AN EXACT MATCH, it mearly checks to make sure that the requesting flags (dwSocketFlags) are found in the socket flags.
        //  So this can lead to a secure socket being returned on a non-secure open request, now realistically this doesn't happen because of the port number.
        //  But in the case of tunnelling this may be an issue, so we add an additional check to make sure that we only
        //  get a tunneled socket to a proxy if we specifically request one.

        if (pSocket->Match(dwSocketFlags) && (pSocket->GetPort() == nPort) &&  pSocket->MatchTunnelSemantics(dwSocketFlags)) {
            ...
            DPRINTF("%#x: ** %s keep-alive connection %#x (%d/%d)\n",GetCurrentThreadId(),GetHostName(),pSocket->GetSocket(),AvailableConnections(),ConnectionLimit());
            break;
        }
    }
...
#ifdef NEW_CONNECTION_SCHEME
//BOOL CServerInfo::RunOutOfConnections(VOID)
//*++
//Routine Description:
//    Determines whether we have been run out of connections.
//    Criteria for being out of connections is: no available connections or keep-alive connections
...
//--*/
//{
//    DEBUG_ENTER((DBG_SESSION, Bool, "CServerInfo::RunOutOfConnections", NULL));
//    BOOL bOut = FALSE;

//    if ((TotalAvailableConnections() == 0) && (GetLastActiveTime() != 0) && !ConnectionActivity()) {
//        DPRINTF("%#x: >>>>>>>> Run out of connections. Last activity %d mSec ago. Create new\n", GetCurrentThreadId(), (GetLastActiveTime() == 0) ? -1  : (GetTickCount() - GetLastActiveTime()));
//        INET_ASSERT(!UnlimitedConnections());
//        DEBUG_PRINT(SESSION,WARNING,("out of connections! Last activity %d mSec ago. Creating new\n",(GetLastActiveTime() == 0) ? -1 : (GetTickCount() - GetLastActiveTime())));
//        bOut = TRUE;
//    } else if (TotalAvailableConnections() == 0) {
//        DPRINTF("%#x: no connections, last activity %d mSec ago\n",GetCurrentThreadId(),(GetLastActiveTime() == 0) ? -1 : (GetTickCount() - GetLastActiveTime()));
//        DEBUG_PRINT(SESSION,INFO,("no connections, last activity %d mSec ago\n",(GetLastActiveTime() == 0) ? -1 : (GetTickCount() - GetLastActiveTime())));
//    }
