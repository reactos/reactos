    ...
    if (lpDllInfo->hModule == NULL) {
        ...
        if (hDll != NULL) {
            ...
            for (DWORD i = 0; i < lpDllInfo->dwNumberOfEntryPoints; ++i) {
                FARPROC proc = GetProcAddress(hDll, lpDllInfo->lpEntryPoints[i].lpszProcedureName);
        ...
        DEBUG_PRINT(UTIL, INFO, ("info for %q already loaded\n", lpDllInfo->lpszDllName));
...

{
       ...
       pSecInfo->pCertificate->pCertInfo )
    {
        ...
        CopyMemory((PVOID) &pCertificate->ftStart, (PVOID) &pCertInfo->NotBefore, sizeof(FILETIME));
        CopyMemory((PVOID) &pCertificate->ftExpiry, (PVOID) &pCertInfo->NotAfter, sizeof(FILETIME));
    }
    ...
    /*if(pSecInfo->dwProtocol)
    {
        ...
        if(LoadString(GlobalDllHandle, dwProtocolID, lpszProtocol, sizeof(lpszProtocol)/sizeof(lpszProtocol[0])))
        {
...
quit:
    ...
    if ( error != ERROR_SUCCESS && fCanAlloc)
    {
