...
Abstract:
    ...
    in this module take care of indirecting sockets calls, hence _I_ in front of the function name
...
GLOBAL int (PASCAL FAR * _I_WSAGetLastError)(void) = NULL;
GLOBAL void (PASCAL FAR * _I_WSASetLastError)(int iError) = NULL;
...
#if !defined(unix)
PRIVATE SOCKETS_FUNCTION SocketsFunctions[] = {
    1,      (FARPROC*)&_I_accept,
    2,      (FARPROC*)&_I_bind,
...
--*/
{
    // initialize the critical section that protects against multiple threads trying to initialize Winsock
...
--*/
{
    ...
    // ensure no 2 threads are trying to modify the loaded state of winsock at the same time
    ...
    if (hWinsock == NULL) {
        ...
        if (hWinsock != NULL) {
            ...
            for (int i = 0; i < ARRAY_ELEMENTS(SocketsFunctions); ++i) {
                ...
                farProc = GetProcAddress(hWinsock,(LPCSTR)SocketsFunctions[i].FunctionOrdinal);
                if (farProc == NULL) {
                    failed = TRUE;
                    break;
                }
            ...
            if (!failed) {
                // although we need a WSADATA for WSAStartup(), it is an expendible structure (not required for any other sockets calls)
                WSADATA wsaData;
                ...
                if (error == ERROR_SUCCESS) {
                    ...
                    if (strnistr(wsaData.szDescription, "novell", stringLen) && strnistr(wsaData.szDescription, "wsock32", stringLen)) {
        ...
        // if we failed to find an entry point or WSAStartup() returned an error then unload the library
        if (failed) {
            // important: there should be no API calls between determining the failure and coming here to get the error code

            // if error == ERROR_SUCCESS then we have to get the last error, else it is the error returned by WSAStartup()
            if (error == ERROR_SUCCESS) {
                error = GetLastError();
        ...
        // just increment the number of times we have called LoadWinsock() without a corresponding call to UnloadWinsock();
        ++WinsockLoadCount;
    }
    ...
    if (hWinsock != NULL) {
        ...
        if (WinsockLoadCount == 0) {
            ...
            if (_I_WSACleanup != NULL) {
                // need to terminate async support too - it is reliant on Winsock
            ...
            // if there have been multiple virtual loads, then just reduce the load count
            --WinsockLoadCount;
        }
    }
...
--*/
{
    ...
    ENDEXCEPT
    return (serr == SOCKET_ERROR) ? MapInternetError(_I_WSAGetLastError()) : ERROR_SUCCESS;
}
...
VOID SetupSocketsTracing(VOID)
{
    ...
    if ((pRtlCaptureStackBackTrace = (RTL_CAPTURE_STACK_BACK_TRACE) GetProcAddress(NtDllHandle, "RtlCaptureStackBackTrace")) == NULL) {
...
VOID AddSockEntry(SOCKET S)
{
    ...
    for( i = 0; i < MAX_SOCK_ENTRIES; i++ ) {
        if( GlobalSockEntry[i].Socket == 0 ) {
            ...
            GlobalSockEntry[i].Socket = S;// found a free entry.

            // get caller stack.

#if i386
            Hash = 0;
            GlobalSockEntry[i].StackTraceLength = pRtlCaptureStackBackTrace(2,MAX_STACK_TRACE,GlobalSockEntry[i].StackTrace,&Hash );
#else // i386
    ...
    // we have reached a high handle limit, which is unusal, needs to be debugged.
    INET_ASSERT( FALSE );
    UNLOCK_DEBUG_SOCK();
}


VOID RemoveSockEntry(SOCKET S)
{
    DWORD i;
...
{
    DEBUG_PRINT(SOCKETS,INFO,("GlobalSocketsCount = %d\n",GlobalSocketsCount));
