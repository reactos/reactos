...
VOID CAddressList::FreeList(VOID)
/*++
Routine Description:
    Free address list
--*/
...
DWORD CAddressList::SetList(IN DWORD dwIpAddress)
/*++
Routine Description:
...
DWORD CAddressList::SetList(IN LPHOSTENT lpHostent)
/*++
Routine Description:
...
BOOL CAddressList::GetNextAddress(
...
Routine Description:
    ...
    address, use that. We make a copy of the address to use in the caller's data space
Arguments:
    lpdwResolutionId    - used to determine whether the address list has been resolved between calls
...
--*/
{
    ...
    if (m_BadAddressCount < m_AddressCount) {
        if (*lpdwIndex != (DWORD)-1) {
            ...
            INET_ASSERT((m_CurrentAddress >= 0) && (m_CurrentAddress < m_AddressCount));
        ...
        DEBUG_PRINT(SOCKETS, INFO, ("exhausted %d addresses\n", m_BadAddressCount));
        bOk = FALSE;
    }
    ...
    *lpdwResolutionId = m_ResolutionId;
    ...
    DEBUG_PRINT(SOCKETS, INFO, ("ResolutionId = %d, Index = %d\n", m_ResolutionId, m_CurrentAddress));
...
VOID CAddressList::InvalidateAddress(IN DWORD dwResolutionId, IN DWORD dwAddressIndex)
/*++
Routine Description:
    We failed to create a connection. Invalidate the address so other requests will try another address
Arguments:
...
--*/
{
    ...
    // only do this if the list is the same age as when the caller last tried an address

    if (dwResolutionId == m_ResolutionId) {
        INET_ASSERT(((INT)dwAddressIndex >= 0) && ((INT)dwAddressIndex < m_AddressCount));
...
DWORD CAddressList::ResolveHost(IN LPSTR lpszHostName, IN OUT LPDWORD lpdwResolutionId, IN DWORD dwFlags)
/*++
Routine Description:
...

{
    ...
    error = DoFsm(new CFsm_ResolveHost(lpszHostName, lpdwResolutionId, dwFlags, this));
...
DWORD CFsm_ResolveHost::RunSM(IN CFsm * Fsm)
{
    DEBUG_ENTER((DBG_SESSION, Dword, "CFsm_ResolveHost::RunSM", "%#x", Fsm));
...
DWORD CAddressList::ResolveHost_Fsm(IN CFsm_ResolveHost * Fsm)
{
    DEBUG_ENTER((DBG_SOCKETS,
                 Dword,
    ...
    if (!lpThreadInfo->IsAsyncWorkerThread && pHandle->IsAsyncHandle() && (fsm.GetAppContext() != INTERNET_NO_CALLBACK)) {
        DEBUG_PRINT(SOCKETS, INFO, ("async request on app thread - jumping to hyper-drive\n"));
    ...
    if (dwFlags & SF_INDICATE) {
        InternetIndicateStatusString(INTERNET_STATUS_RESOLVING_NAME, lpszHostName);
    }

    // figure out if we're being asked to resolve a name or an address. If
    // inet_addr() succeeds then we were given a string representation of an address
    ...
    // now try to find the name or address in the cache. If it's not in the cache then resolve it
...
quit:
    if ((error == ERROR_SUCCESS) && (dwFlags & SF_INDICATE)) {
        ...
        InternetIndicateStatusAddress(INTERNET_STATUS_NAME_RESOLVED, RemoteSockaddr(), RemoteSockaddrLength());
    }
    *lpdwResolutionId = m_ResolutionId;
...
PRIVATE DWORD CAddressList::IPAddressToAddressList(IN DWORD ipAddr)
/*++
Routine Description:
...
--*/
{
    ...
    // address family (AF_INET), zero port value and the IP address presented in the arguments
...
PRIVATE DWORD CAddressList::HostentToAddressList(IN LPHOSTENT lpHostent)
/*++
Routine Description:
...
--*/
{
    ...
    LPRESOLVED_ADDRESS addresses = (LPRESOLVED_ADDRESS)ALLOCATE_MEMORY(LMEM_FIXED, nAddrs * (sizeof(RESOLVED_ADDRESS)
