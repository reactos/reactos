...
// private prototypes
PRIVATE DWORD __cdecl SendCommand(IN ICSocket * s, IN LPCSTR lpszFormat, IN ...);
PRIVATE DWORD I_SendCommand(IN ICSocket * s, IN LPCSTR lpszFormat, va_list arglist);
PRIVATE DWORD I_AttemptDataNegotiation(
...
PRIVATE DWORD I_NegotiateDataConnection(
    IN LPFTP_SESSION_INFO lpSessionInfo,
    IN DWORD dwFlags,
    IN OUT FTP_RESPONSE_CODE *prcResponse,
    IN LPCSTR lpszCommandFormat,
...
DWORD Command(IN LPFTP_SESSION_INFO lpSessionInfo,
...
Arguments:
    ...
    dwFlags             - controlling how to transfer data between the client
                          and server data connection, and how to set up data connection
...
DWORD I_Command(IN LPFTP_SESSION_INFO lpSessionInfo,
    IN BOOL fExpectResponse,
    IN DWORD dwFlags,
    IN OUT FTP_RESPONSE_CODE *prcResponse,
    IN LPCSTR lpszCommandFormat,
...
--*/
{
    ...
    // if we need a data connection then send the connection set-up commands and issue the command
    if (fExpectResponse) {
        error = I_AttemptDataNegotiation(lpSessionInfo, dwFlags, prcResponse, lpszCommandFormat, arglist);
        if (error == ERROR_SUCCESS) {
            // check the server response for failure
            if ((prcResponse->Major != FTP_RESPONSE_PRELIMINARY) && (prcResponse->Major != FTP_RESPONSE_COMPLETE)) {
        ...
        error = I_SendCommand(lpSessionInfo->socketControl, lpszCommandFormat, arglist);
        if (error == ERROR_SUCCESS) {
...
DWORD NegotiateDataConnection(IN LPFTP_SESSION_INFO lpSessionInfo,
    IN DWORD dwFlags,
    IN OUT FTP_RESPONSE_CODE *prcResponse,
    IN LPCSTR lpszCommandFormat,
    IN ...)
...
DWORD GetReply(IN LPFTP_SESSION_INFO lpSessionInfo, OUT FTP_RESPONSE_CODE *prcResponse)
/*++
Routine Description:
...
--*/
{
    ...
    error = ReceiveFtpResponse(lpSessionInfo->socketControl, (LPVOID *)&pchReplyBuffer, &cchBufferLength, TRUE, prcResponse);
    if (error == ERROR_SUCCESS) {
        INET_ASSERT(pchReplyBuffer != NULL);

        // append this response text to that already stored in this thread's data object
...
Routine Description:
    ...
    expect to get EOF(connection) because we are usually reading the control connection
Arguments:
    ...
    while (TRUE) {
        ...
        if (fEndOfLineCheck) {
            ...
            for (DWORD bytesChecked = 0; bytesChecked < bytesReceived; ++bytesChecked, ++idxPosInLine) {
                // ISSUE: fAtEOL && fLastLineDigitsSeen ==> protocol error, since there is data after the response line
        ...
        if (fEndOfLineCheck && fLastLineDigitsSeen && fAtEOL) {
            ...
            // tear the numeric reply up by digit, placing it into our nicer reply structure
...
DWORD AbortTransfer(IN LPFTP_SESSION_INFO lpSessionInfo)
/*++
Routine Description:
...
PRIVATE DWORD __cdecl SendCommand(IN ICSocket * Socket, IN LPCSTR lpszFormat, IN ...)
/*++
Routine Description:
...
PRIVATE DWORD I_SendCommand(IN ICSocket * Socket, IN LPCSTR lpszFormat, va_list arglist)
/*++
Routine Description:
...
PRIVATE DWORD I_NegotiateDataConnection(IN LPFTP_SESSION_INFO lpSessionInfo,
    IN DWORD dwFlags,
    IN OUT FTP_RESPONSE_CODE *prcResponse,
    IN LPCSTR lpszCommandFormat,
    IN va_list arglist,
...
//Command(lpSessionInfo, FALSE, FTP_TRANSFER_TYPE_UNKNOWN, prcResponse, "MODE B"
////        "MODE S");
    ...
    error = Command(lpSessionInfo,
                    ...
                    prcResponse,
                    "TYPE %s",
                    ((dwFlags & FTP_TRANSFER_TYPE_MASK) == FTP_TRANSFER_TYPE_ASCII) ? "A" : "I");
    if (error != ERROR_SUCCESS) {
        goto quit;
    }
    if (prcResponse->Major != FTP_RESPONSE_COMPLETE) {
    ...
    if (!bPassiveMode) {
        ...
        // which we then connect to. This is the inverse of the non-PASV connect case above
            ...
            if (pch != NULL) {
                ...
                if (isdigit(*pch)) {
                    ...
                    if ((i == ARRAY_ELEMENTS(octets)) && (octets[i - 1] <= 255)) {
                        ...
                        remoteSockaddr.sin_port = _I_htons((USHORT)((octets[4] << 8) | (USHORT)octets[5]));
                        ...
                        error = socketData->DirectConnect((PSOCKADDR)&remoteSockaddr);
                    } else {
    ...
    if(StrCmpNI(lpszCommandFormat, "RETR", 4) == 0) {   // If we're downloading a file
        error = I_SendCommand(socketControl, "SIZE %s", arglist);
        if (error != ERROR_SUCCESS) {
            goto error_exit;
        }
    ...
    error = I_SendCommand(socketControl, lpszCommandFormat, arglist);
    if (error != ERROR_SUCCESS) {
        goto error_exit;
    }
    ...
    // then perform an accept() on our listening socket in order to establish the connection
    if (!bPassiveMode) {
        ...
        error = socketListener->SelectAccept(*socketData, // BUGBUG - call GetTimeoutValue()
    ...
    socketData->SetTimeout(SEND_TIMEOUT, (int)GetTimeoutValue(INTERNET_OPTION_DATA_SEND_TIMEOUT));
    socketData->SetTimeout(RECEIVE_TIMEOUT, (int)GetTimeoutValue(INTERNET_OPTION_DATA_RECEIVE_TIMEOUT));
