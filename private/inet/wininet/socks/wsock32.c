...
Abstract:
    Contains Socks V4 support, written by Hummingbird corporation.
    Licensed from Hummingbird for ulimited use by Microsoft.  Ported to WININET code base.
...
struct Sockets * FindSocket(int s)
    /*++
    Routine Description:
...
DWORD WINAPI closesocket(int s)
/*++
Routine Description:
...
DWORD WINAPI connect(int s, const struct sockaddr_in FAR * name, int namelen)
/*++
Routine Description:
    Connect to remote host via SOCKS proxy.
    Modified from original.
    If we are here then we are going specifically via a known SOCKS proxy.
    There is now only one Hosts object, containing a single SOCKD socks proxy address and user name
Arguments:
    s       - socket to connect
...
--*/
{
    ...
    // from this point, we cannot touch pHost or pSocket until we take the mutex again
    LEAVE_MUTEX();
...
DWORD WINAPI getpeername(int s, struct sockaddr_in * name, int *namelen)
/*++
Routine Description:
...
DWORD WINAPI setsockopt(int s, int level, int optname, const char FAR * optval, int optlen)
/*++
Routine Description:
    If SO_SET_SOCKS_FIREWALL, create SOCKS information if it is new or changed from current, else pass on the request to wsock32!setsockopt()
Arguments:
...
--*/
{
    ...
    if (level != SO_SET_SOCKS_FIREWALL) {
        ...
        if (!pHost) {
            ...
            if (pHost = (struct Hosts *)LocalAlloc(LPTR, sizeof(struct Hosts) + userlen)) {
...
BOOL WINAPI DllMain(IN HINSTANCE hInstance, IN DWORD reason, IN LPVOID Reserved)
{
    ...
    // Load an alternate Winsock DLL based on a registry value, in the event that a customer wants to load a different wsock32.
    if (ERROR_SUCCESS == (lResult = RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Internet Settings"), 0, KEY_QUERY_VALUE, &hKey))) {
        if (ERROR_SUCCESS == (lResult = RegQueryValueEx(hKey, TEXT("AlternateWinsock"), NULL, &dwRegType, (LPBYTE)szRegBuf, &dwRegBufSize)) && dwRegType == REG_SZ) // only allow type REG_SZ
        {
    ...
    if (hModule < (HMODULE)HINSTANCE_ERROR) {
        MessageBox(NULL, "Unable to find old WSOCK32.DLL named \"WSOCK32.DLL\".", "Microsoft/Hummingbird SOCKS Shim", MB_OK);
        LoadCount = 0;
        return 0;
    }
