...
PRIVATE DWORD WinNtWsControl(DWORD dwProtocol,
    DWORD dwRequest,
...
PRIVATE DWORD OpenTcpipDriverHandle(VOID);
PRIVATE VOID CloseTcpipDriverHandle(VOID);
PRIVATE DWORD GetEntityList(OUT TDIEntityID * * lplpEntities);

// private debug prototypes
PRIVATE LPSTR InternetMapEntity(IN INT EntityId);
PRIVATE LPSTR InternetMapInterface(IN DWORD InterfaceType);
PRIVATE LPSTR InternetMapInterfaceOnNT5(IN DWORD InterfaceType);
...
PRIVATE DWORD (PASCAL FAR * _I_WsControl)(DWORD, DWORD, LPVOID, LPDWORD, LPVOID, LPDWORD) = NULL;
...
PRIVATE DWORD (PASCAL FAR * _I_DhcpRequestOptions)(LPWSTR, BYTE  *, DWORD, BYTE **, DWORD *, BYTE **, DWORD *) = NULL;
...
BOOL CIpAddress::GetAddress(OUT LPBYTE lpbAddress, IN OUT LPDWORD lpdwAddressLength)
/*++
Routine Description:
...
BOOL CIpAddressList::IsContextInList(IN DWORD dwContext)
{
...
CIpAddress * CIpAddressList::Find(IN DWORD dwIpAddress, IN DWORD dwIpMask)
/*++
Routine Description:
...
VOID CIpAddressList::Add(IN CIpAddress * pAddress)
/*++
Routine Description:
...
BOOL CIpAddressList::Add(IN DWORD dwIpAddress, IN DWORD dwIpMask, IN DWORD dwContext)
/*++
Routine Description:
    Adds an IP address entry to the list
Arguments:
...
BOOL CIpAddressList::GetAddress(IN OUT LPDWORD lpdwIndex, OUT LPBYTE lpbAddress, IN OUT LPDWORD lpdwAddressLength)
/*++
Routine Description:
...
PRIVATE BOOL CIpAddressList::ThrowOutUnfoundEntries(VOID)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Bool, "CIpAddressList::ThrowOutUnfoundEntries", NULL));
...
CAdapterInterface::~CAdapterInterface(VOID)
/*++
Routine Description:
...
BOOL CAdapterInterface::DhcpDoInformNT5(IN OUT LPSTR lpszAutoProxyUrl, IN DWORD dwAutoProxyUrlLength)
/*++
Routine Description:
...
--*/
{
    ...
    if ( error == ERROR_SUCCESS )
    {
        ...
        for (LPBYTE pbOptionId = pbReturnOptions, pbOptionValue = pbOptionList;
            (pbOptionId < (pbReturnOptions + dwReturnOptionSize) && pbOptionValue < (pbOptionList + dwOptionListSize));
             pbOptionId++, pbOptionValue += (dwNextInc + 1))
        {
            dwNextInc = *pbOptionValue;
            pbOptionValue++; // advance past the size/length byte

            if (*pbOptionId == OPTION_WPAD_URL && dwAutoProxyUrlLength > dwNextInc && lpszAutoProxyUrl)
            {
...
BOOL CAdapterInterface::CopyAdapterInfoToDhcpContext(PDHCP_CONTEXT pDhcpContext)
{
...
/*++
Routine Description:
    CIpConfig constructor - initializes the object & loads the requird DLLs if not already loaded
Arguments:
    None.
Return Value:
    None.
--*/
{
    DEBUG_ENTER((DBG_OBJECTS, None, "CIpConfig::CIpConfig", NULL));
...
/*++
Routine Description:
    CIpConfig destructor - destroys this object and unloads (or reduces the reference count on) the DLLs
--*/
{
    DEBUG_ENTER((DBG_OBJECTS, None, "CIpConfig::~CIpConfig", NULL));
...
BOOL CIpConfig::GetRouterAddress(
...
--*/
{
    ...
    //    found = ((CAdapterInterface *)m_List.Flink)->m_RouterList.GetAddress(lpdwIndex, lpbAddress, lpdwAddressLength);
    //} else {
        found = FALSE;
    //}
...
--*/
{
    ...
    if (!m_DnsList.IsEmpty()) {
        found = m_DnsList.GetAddress(lpdwIndex, lpbAddress, lpdwAddressLength);
    } else {
        found = FALSE;
    }
...
BOOL CIpConfig::IsKnownIpAddress(
    IN LPBYTE lpbInterfaceAddress OPTIONAL,
    IN DWORD dwInterfaceAddressLength,
...
BOOL CIpConfig::Refresh(VOID)
/*++
Routine Description:
    Refreshes the interface information - re-reads the interfaces and IP addresses
Arguments:
    None.
Return Value:
    BOOL
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Bool, "CIpConfig::Refresh", NULL));
...
PRIVATE BOOL CIpConfig::GetAdapterList(OUT LPBOOL lpbChanged)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Bool, "CIpConfig::GetAdapterList", "%#x", lpbChanged));
    ...
    if (numberOfEntities == 0) {
        ...
        DEBUG_PRINT(UTIL, ERROR, ("GetAdapterList: failed to get entity list\n"));
        goto quit;
    }
...
//        if ((status != TDI_SUCCESS) || (outputLen != sizeof(isMib))) {
        if (status != TDI_SUCCESS) {
            ...
            DEBUG_PRINT(UTIL, ERROR, ("WsControl(ENTITY_TYPE_ID): status = %d, outputLen = %d\n", status, outputLen));
            goto error_exit;
        }
        if (isMib != IF_MIB) {
            // entity doesn't support MIB requests - try another
            DEBUG_PRINT(UTIL, WARNING, ("Entity %#x, Instance #%d doesn't support MIB (%#x)\n", id.toi_entity.tei_entity, id.toi_entity.tei_instance, isMib));
            continue;
        }
        ...
        status = WsControl(IPPROTO_TCP, WSCNTL_TCPIP_QUERY_INFO, (LPVOID)&req, &inputLen, (LPVOID)&info, &outputLen);
        if (status != TDI_SUCCESS) {
            // unexpected results - bail out
            DEBUG_PRINT(UTIL, ERROR, ("WsControl(IF_MIB_STATS_ID) returns %d\n", status));
            goto error_exit;
        }

        // we only want physical adapters
        if (!IS_INTERESTING_ADAPTER(pIfEntry)) {
            DEBUG_PRINT(UTIL, INFO, ("ignoring adapter #%d [%s]\n", pIfEntry->if_index, InternetMapInterface(pIfEntry->if_type)));
            continue;
        }

        // got this adapter info ok. Find or create an interface object and fill in what we can
        ...
        pInterface = FindOrCreateInterface(pIfEntry->if_index,
                                           ...
                                           (DWORD) len);
        if (pInterface == NULL) {
            DEBUG_PRINT(UTIL, ERROR, ("failed to allocate memory for CAdapterInterface\n"));
            goto error_exit;
        }
    }
...
//        if ((status != TDI_SUCCESS) || (outputLen != sizeof(type))) {
        ...
        if (type != CL_NL_IP) {
            ...
            DEBUG_PRINT(UTIL, INFO, ("CL_NL_ENTITY #%d not CL_NL_IP - skipping\n", pEntity->tei_instance));
            continue;
        }
        ...
        if ((status != TDI_SUCCESS) || (outputLen != sizeof(info))) {
            // unexpected results - bail out
            DEBUG_PRINT(UTIL, ERROR, ("WsControl(IP_MIB_STATS_ID): status = %d, outputLen = %d\n", status, outputLen));
            goto error_exit;
        }
        ...
        if (info.ipsi_numaddr != 0) {
            ...
            if (buffer == NULL) {
                // unexpected results - bail out
                DEBUG_PRINT(UTIL, ERROR, ("failed to allocate %d bytes\n", outputLen));
                goto error_exit;
            }
            ...
            if (status != TDI_SUCCESS) {
                // unexpected results - bail out
                DEBUG_PRINT(UTIL, ERROR, ("WsControl(IP_MIB_ADDRTABLE_ENTRY_ID): status = %d, outputLen = %d\n", status, outputLen));
                goto error_exit;
            }

            // now loop through this list of IP addresses, applying them to the correct adapter

            numberOfAddresses = min((UINT)(outputLen / sizeof(IPAddrEntry)), (UINT)info.ipsi_numaddr);
            DEBUG_PRINT(UTIL, INFO, ("%d IP addresses\n", numberOfAddresses));
            ...
            for (j = 0; j < numberOfAddresses; ++j, ++pAddr) {
                ...
                if (pInterface != NULL) {
                    ...
                    pIpAddress = pInterface->m_IpList.Find(pAddr->iae_addr, pAddr->iae_mask);
                    if (pIpAddress == NULL) {
                        pInterface->m_IpList.Add(pAddr->iae_addr, pAddr->iae_mask, pAddr->iae_context);
...
//        );
        ...
        if (info.ipsi_numroutes != 0) {
            ...
            // Warning: platform specifics; Win95 structure size is different than NT 4.0

            // BUGBUG - this will probably have to be checked for version # on Memphis

            int structLength = (GlobalPlatformType == PLATFORM_TYPE_WINNT) ? sizeof(IPRouteEntry) : sizeof(IPRouteEntry95);
            ...
            for (j = 0; j < 4; ++j) {
                ...
                routeTable = (IPRouteEntry*)ResizeBuffer(routeTable, outputLen, FALSE);
                if (routeTable == NULL) {
                    goto error_exit;
                }

                status = WsControl(IPPROTO_TCP, WSCNTL_TCPIP_QUERY_INFO, (LPVOID)&req, &inputLen, (LPVOID)routeTable, &outputLen);
                if (status != TDI_SUCCESS) {
                    // unexpected results - bail out
                    DEBUG_PRINT(UTIL, ERROR, ("WsControl(IP_MIB_RTTABLE_ENTRY_ID): status = %d, outputLen = %d\n", status, outputLen));
                    goto error_exit;
                }
...
//        );
    ...
    // Even if we don't get any DNS servers, we deem that this function has succeeded

    char dnsBuffer[1024];   // arbitrary (how many DNS entries?)
    UINT error;

    error = SockGetSingleValue(CONFIG_NAME_SERVER, (LPBYTE)dnsBuffer, sizeof(dnsBuffer));
    if (error == ERROR_SUCCESS) {
        //m_DnsList.Clear();
...
PRIVATE BOOL CIpConfig::GetAdapterListOnNT5(OUT LPBOOL lpbChanged)
/*++
Routine Description:
    Builds a list of interfaces corresponding to physical and logical adapters using the new NT 5 and Win98 APIs.
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Bool, "CIpConfig::GetAdapterListOnNT5", "%#x", lpbChanged));
    ...
    {
        DEBUG_PRINT(UTIL, ERROR, ("GetAdapterListOnNT5: IPHLPAPI dll could not be found with correct entry point\n"));
        goto quit;
    }
    ...
    if ( dwError != ERROR_SUCCESS )
    {
        ...
        DEBUG_PRINT(UTIL, ERROR, ("GetAdapterListOnNT5: failed to get adapters list\n"));
        goto quit;
    }

    // first off, mark all the current interfaces (if any), including current IP addresses, as not found
    SetNotFound();
    ...
    for (pAdapterInfo = AdapterInfo; pAdapterInfo; pAdapterInfo = pAdapterInfo->Next)
    {
        ...
        if (!IS_INTERESTING_ADAPTER_NT5(pAdapterInfo)) {
            DEBUG_PRINT(UTIL, INFO, ("ignoring adapter #%u [%s]\n", pAdapterInfo->Index, InternetMapInterfaceOnNT5(pAdapterInfo->Type)));
            continue;
        }
        ...
        if ( pAdapterInfo->CurrentIpAddress->IpAddress.String && pAdapterInfo->CurrentIpAddress->IpMask.String )
        {
            ...
            if ( dwAddress   != INADDR_NONE && dwMask      != INADDR_NONE  )
            {
                DEBUG_PRINT(UTIL,
                            INFO,
                            ("IP address %d.%d.%d.%d, index %d, context %d\n",
                ...
                CIpAddress * pIpAddress;
                pIpAddress = pInterface->m_IpList.Find(dwAddress, dwMask);
                if (pIpAddress == NULL) {
                    pInterface->m_IpList.Add(dwAddress, dwMask, dwContext);
        ...
        if ( pAdapterInfo->DhcpEnabled )
        {
            ...
            for ( pDhcpServer = &pAdapterInfo->DhcpServer; pDhcpServer; pDhcpServer = pDhcpServer->Next )
            {
                ...
                if ( dwAddress   != INADDR_NONE )
                {
                    ...
                    pIpAddress = pInterface->m_DhcpList.Find(dwAddress, dwMask);
                    if (pIpAddress == NULL)
                    {
                        pInterface->m_DhcpList.Add(dwAddress, dwMask, dwContext);
...
BOOL CIpConfig::DoInformsOnEachInterface(IN OUT LPSTR lpszAutoProxyUrl, IN     DWORD dwAutoProxyUrlLength)
{
    for (CAdapterInterface * pEntry = (CAdapterInterface *)m_List.Flink;
         pEntry != (CAdapterInterface *)&m_List.Flink;
         pEntry =  (CAdapterInterface *)pEntry->m_List.Flink)
    {
        if ( pEntry->IsDhcp() )
        {
            ...
            if ( GlobalPlatformVersion5 )
            {
                fSuccess = pEntry->DhcpDoInformNT5(lpszAutoProxyUrl, dwAutoProxyUrlLength);
            }
            else
            {
 ...
 *  matching the IPInterfaceContext value with the adapter which owns the IP address with that context value
 *
 *  ENTRY   nothing
 *
 *  EXIT    nothing
 ...
 **/
{
    ...
    if (GlobalPlatformType == PLATFORM_TYPE_WINNT)
    {
        if ( ServicesKey == NULL )
        {
            err = REGOPENKEY(HKEY_LOCAL_MACHINE, SERVICES_KEY_NAME, &ServicesKey);
        }
        ...
        if (err == ERROR_SUCCESS && (boundAdapterNames = GetBoundAdapterList(TcpipLinkageKey)))
        {
            ...
            // section with the context values read from the stack for the IP addresses
            for (i = 0; boundAdapterNames[i]; ++i) {
                ...
                if (!ReadRegistryDword(key, "IPInterfaceContext", &context)) {
    ...
    {
        // Win95: search through the list of adapters, gather DHCP server names for each.
        for (CAdapterInterface * pEntry = (CAdapterInterface *)m_List.Flink;
             pEntry != (CAdapterInterface *)&m_List.Flink;
             pEntry =  (CAdapterInterface *)pEntry->m_List.Flink)
        {
...
PRIVATE DWORD CIpConfig::LoadEntryPoints(VOID)
/*++
Routine Description:
    Loads NTDLL.DLL entry points if Windows NT else (if Windows 95) loads WsControl from WSOCK32.DLL
Arguments:
    None.
Return Value:
    DWORD
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Dword, "CIpConfig::LoadEntryPoints", NULL));

    DWORD error = ERROR_SUCCESS;

    if (m_Loaded == TRI_STATE_UNKNOWN) {
        error = LoadDllEntryPoints((GlobalPlatformType == PLATFORM_TYPE_WINNT) ? &NtDllInfo : &WsControlInfo, 0);
...
PRIVATE DWORD CIpConfig::UnloadEntryPoints(VOID)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Dword, "CIpConfig::UnloadEntryPoints", NULL));

    DWORD error = ERROR_SUCCESS;

    if (m_Loaded == TRI_STATE_TRUE) {
        error = UnloadDllEntryPoints((GlobalPlatformType == PLATFORM_TYPE_WINNT) ? &NtDllInfo : &WsControlInfo, FALSE);
        if (error == ERROR_SUCCESS) {
            m_Loaded = TRI_STATE_UNKNOWN;
        }
    }
...
PRIVATE CAdapterInterface * CIpConfig::FindOrCreateInterface(
    IN DWORD dwIndex,
    IN DWORD dwType,
    IN DWORD dwSpeed,
    IN LPSTR lpszDescription,
...
PRIVATE CAdapterInterface * CIpConfig::FindInterface(IN DWORD dwIndex)
/*++
Routine Description:
    Returns a pointer to the CAdapterInterface object corresponding to dwIndex
Arguments:
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Pointer, "CIpConfig::FindInterface", "%d", dwIndex));

    CAdapterInterface * pInterface = NULL;

    for (PLIST_ENTRY pEntry = m_List.Flink; pEntry != (PLIST_ENTRY)&m_List; pEntry = pEntry->Flink) {
...
PRIVATE BOOL CIpConfig::ThrowOutUnfoundEntries(VOID)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Bool, "CIpConfig::ThrowOutUnfoundEntries",NULL));
    ...
    while (pEntry != (PLIST_ENTRY)&m_List) {
        ...
        if (!pInterface->IsFound()) {
            DEBUG_PRINT(UTIL, WARNING, ("adapter index %d (%q) not located in list\n", pInterface->m_dwIndex, pInterface->m_lpszDescription));
...
DWORD WsControl(IN DWORD dwProtocol,
    IN DWORD dwRequest,
...
--*/
{
    ...
    if (GlobalPlatformType == PLATFORM_TYPE_WINNT) {
        error = WinNtWsControl(dwProtocol, dwRequest, lpInputBuffer, lpdwInputBufferLength, lpOutputBuffer, lpdwOutputBufferLength);
    } else {
        error = _I_WsControl(dwProtocol, dwRequest, lpInputBuffer, lpdwInputBufferLength, lpOutputBuffer, lpdwOutputBufferLength);
    }
...
/*++
Routine Description:
    Handles WsControl() functionality on NT platform. Assumes NTDLL.DLL has already been loaded
Arguments:
    dwProtocol              - unused
    dwRequest               - unused
...
/*++
Routine Description:
    Allocates a buffer for, and retrieves, the list of entities supported by the TCP/IP device driver
Arguments:
    lplpEntities    - pointer to allocated returned list of entities. Caller must free
Return Value:
    UINT    - number of entities returned
--*/
{
    DEBUG_ENTER((DBG_SOCKETS, Int, "GetEntityList", "%#x", lplpEntities));
    ...
    // change between calls. Stops us getting stuck in infinite loop if something bad happens with outputLen
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        status = WsControl(IPPROTO_TCP, WSCNTL_TCPIP_QUERY_INFO, (LPVOID)&req, &inputLen, (LPVOID)pEntity, &outputLen);

        // TDI_SUCCESS is returned if all data is not returned: driver
        // communicates all/partial data via outputLen

        if (status == TDI_SUCCESS) {
            DEBUG_PRINT(UTIL, INFO, ("GENERIC_ENTITY required length = %d\n", outputLen));
    ...
    DEBUG_PRINT(UTIL, INFO, ("%d entities returned in %#x\n", (outputLen / sizeof(TDIEntityID)), pEntity));
