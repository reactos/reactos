...
PRIVATE UINT CheckRegistryForParameter(IN UINT ParameterId, OUT LPBYTE Data, IN UINT DataLength);
PRIVATE UINT GetDnsServerListFromDhcp(LPBYTE Data, UINT DataLength);
PRIVATE UINT GetDomainNameFromDhcp(LPBYTE Data, UINT DataLength);
PRIVATE LPDHCP_QUERYINFO GetDhcpHardwareInfo(VOID);
PRIVATE DWORD_PTR OpenDhcpVxdHandle(VOID);
PRIVATE WORD DhcpVxdRequest(IN DWORD_PTR Handle, IN WORD Request, IN WORD BufferLength, OUT LPVOID Buffer);
PRIVATE DWORD_PTR OsOpenVxdHandle(CHAR * VxdName, WORD   VxdId);
PRIVATE VOID OsCloseVxdHandle(DWORD_PTR VxdHandle);
PRIVATE INT OsSubmitVxdRequest(DWORD_PTR   VxdHandle, INT         OpCode, LPVOID      Param, INT         ParamLength);
...
BOOL ReadRegistryString(HKEY Key, LPSTR ParameterName, LPSTR String, LPDWORD Length)
{
    ...
    *String = '\0';
    ...
    if (err == ERROR_SUCCESS) {
        ...
        DEBUG_PRINT(SOCKETS, INFO, ("ReadRegistryString(%s): err=%d\n", ParameterName, err ));
        return 0;
    }
}


UINT SockGetSingleValue(IN UINT ParameterId, OUT LPBYTE Data, IN UINT DataLength)
/*++
Routine Description:
...
--*/
{
    ...
    if (error != ERROR_SUCCESS) {
        if (ParameterId == CONFIG_HOSTNAME) {
            ...
            // computer name, which is the default host name, if none is specified elsewhere
    ...
    IF_DEBUG(REGISTRY) {
        if (error != ERROR_SUCCESS) {
            DLL_PRINT(("SockGetSingleValue(%s) returns %d\r", MAP_PARAMETER_ID(ParameterId), error));
        } else {
            DLL_PRINT(("SockGetSingleValue(%s) returns \"%s\"\n", MAP_PARAMETER_ID(ParameterId), Data));
        }
    }
...
Routine Description:
    ...
    ASSUMES 1. Data is big enough to hold the default value (single byte for strings, dword for dwords)
Arguments:
    ParameterId - identifier of parameter to retrieve
    Data        - pointer to untyped storage space for parameter
    DataLength  - length of data returned (in bytes)
...
--*/
{
    ...
    if (error == ERROR_SUCCESS) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if ((error != ERROR_SUCCESS) || (length == 0) || ((length == 1) && (Data[0] == '\0'))) {
                if (tryDhcp) {
                    p = dhcpBuffer;
                    tryDhcp = FALSE;
                    continue;
 ...
 **/
{
    ...
    if ( GlobalPlatformType == PLATFORM_TYPE_WINNT )
    {
            ...
            OpenAdapterKey(KEY_TCP, paiInterface->GetAdapterName(), &key))
        {
            ...
            ReadRegistryDword(key, "EnableDHCP", &fDhcpEnabled);

            if ( fDhcpEnabled )
            {
                addressLength = sizeof(dhcpServerAddress);
                if (ReadRegistryString(key, "DhcpServer", dhcpServerAddress, &addressLength))
                {
            ...
            //ReadRegistryDword(key, "LeaseObtainedTime", &AdapterInfo->LeaseObtained);
            //ReadRegistryDword(key, "LeaseTerminatesTime", &AdapterInfo->LeaseExpires);
    ...
    else
    {
        if (pDhcpInfoPtr = GetDhcpHardwareInfo()) {
            ...
            DEBUG_PRINT(SOCKETS, INFO, ("GetDhcpServerFromDhcp: DhcpInfoPtr is 0\n"));
        }
    }
    ...
    if (pInfo = GetDhcpHardwareInfo()) {
        ...
        while (n-- && DataLength) {
            ...
            for (i = 0; i < pNicInfo->DNSServersLen; i += 4) {
                ...
                // inet_ntoa will validate the address: if it returns NULL, the address is no good, so we skip it
...
/*++
Routine Description:
    Attempts to retrieve the domain from DHCP. In this case, the DHCP server gave out the domain name we are supposed to use
Arguments:
    Data        - pointer to place to return the list
    DataLength  - length of Data
Return Value:
    ...
    if (pInfo) {
        ...
        while (n--) {
            ...
            if (dnLen && (DataLength >= dnLen)) {
                ...
                memcpy(Data, (LPBYTE)pInfo + pNicInfo->OffsetDomainName, (size_t)dnLen);
...
PRIVATE LPDHCP_QUERYINFO GetDhcpHardwareInfo(VOID)
{
    ...
    if (handle) {
        ...
        result = DhcpVxdRequest(handle, DHCP_QUERY_INFO, sizeof(sizeRequired), &sizeRequired);
        ...
        if (result == ERROR_BUFFER_OVERFLOW) {
            ...
            if (info) {
                result = DhcpVxdRequest(handle, DHCP_QUERY_INFO, (WORD)sizeRequired, info);
...
{
    return (WORD) OsSubmitVxdRequest(Handle, (INT)Request, (LPVOID)Buffer, (INT)BufferLength);
}
    ...
    )
{
    ...
    IF_DEBUG( VXD_IO )
    {
        DLL_PRINT(( "OsSubmitVxdRequest: opcode %04X, param %08lX, length %d\n", OpCode, Param, ParamLength ));
    }

    if( !DeviceIoControl( (HANDLE)VxdHandle, OpCode, Param, ParamLength, Param, ParamLength, &BytesRead, NULL ) )
    {
        Result = GetLastError();
    }
