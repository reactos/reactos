...
Abstract:
    This is the overall generic wrappers and entry code to the auto-proxy, auto-detection code, that sends DHCP informs,
...
DWORD WINAPI GetProxyDetectType(VOID)
{
    ...
    if (! UseCached )
    {
        InternetReadRegistryDword("AutoProxyDetectType", (LPDWORD)&(_adType));
...
BOOL WINAPI DetectAutoProxyUrl(IN OUT LPSTR lpszAutoProxyUrl, IN DWORD dwAutoProxyUrlLength, IN DWORD dwDetectFlags)
{
    BOOL fRet = FALSE;
    DWORD error;

    DEBUG_ENTER((DBG_SOCKETS, Bool, "DetectAutoProxyUrl", "%x, %u", lpszAutoProxyUrl, dwAutoProxyUrlLength));
