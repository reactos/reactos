...
DWORD CCookieLoader::GetHKLMCookiesDirectory(CHAR *szCookiesDirectory)
{
    ...
    if (dwError = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szCookiesDirRegKey, NULL, KEY_ALL_ACCESS, &hKey) != ERROR_SUCCESS)
        goto exit;

    if (dwError = RegQueryValueEx(hKey, CACHE_DIRECTORY_VALUE, NULL, &dwKeyType, (LPBYTE) szCookiesDirectory, &cbKeyLen) != ERROR_SUCCESS)
        goto exit;
...
DWORD CCookieLoader::GetHKCUCookiesDirectory(CHAR *szCookiesDirectory)
{
    ...
    if (dwError = RegOpenKeyEx(HKEY_CURRENT_USER, szCookiesDirRegKey, NULL, KEY_ALL_ACCESS, &hKey) != ERROR_SUCCESS)
        goto exit;
