...
PUBLIC void AuthFlush (void)
{
                    ...
                    REG_SZ) ) // Type expected
    {
        ...
        static LPSTR rszDll[] = {"MSNSSPC.DLL", "MSNSSPS.DLL", "MSAPSSPC.DLL", "MSAPSSPS.DLL"};
...
PRIVATE void AddAuthorizationHeader(HTTP_REQUEST_HANDLE_OBJECT *pRequest, AUTHCTX* pAuthCtx)
{
    if (!pAuthCtx)
        return;
    ...
    *pBuf++ = ' ';
    ...
    if (dwPlugInError == ERROR_SUCCESS && pAuthCtx->GetState() != AUTHCTX::STATE_ERROR)
    {
        ...
        *pBuf = 0;
        ...
        wHttpAddRequestHeaders (pRequest, szHeader, cbBuf, HTTP_ADDREQ_FLAG_ADD | HTTP_ADDREQ_FLAG_REPLACE);
    }
}
...
PUBLIC DWORD AuthOnRequest (IN HINTERNET hRequestMapped)
{
    ...
    if  (pRequest->IsRequestUsingProxy() && !pRequest->IsTalkingToSecureServerViaProxy())
    {
        if (pAuthCtx && pAuthCtx->_fIsProxy)
        {
            ...
            // GetUserAndPass.
            if (pRequest->GetUserAndPass(IS_PROXY, &lpszUser, &lpszPass) || (!GlobalIsProcessExplorer && pRequest->GetUserAndPass(IS_SERVER, &lpszUser, &lpszPass)))
            {
...
)
{
    ...
    LPSTR lpszHost;
    lpszHost = fIsProxy? GetProxyName(pRequest) : pRequest->GetServerName();
