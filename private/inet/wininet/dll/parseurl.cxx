...
BOOL ScanSchemes(LPTSTR pszToCheck, DWORD ccStr, PDWORD pwResult)
{
    ...
    {
        if ((UrlSchemeList[i].SchemeLength == ccStr) && (strnicmp(UrlSchemeList[i].SchemeName, pszToCheck, ccStr)==0))
        {
            *pwResult = i;
            return TRUE;
        }
...
/*++
Routine Description:
    URL parser for generic File URLs. Support function for InternetOpenUrl() and ParseUrl().
    This is a macro function that just cracks the URL and calls Win32 File APIs to do the work
Arguments:
...
DWORD CFsm_ParseUrlForHttp::ScanProxyUrl(IN CFsm_ParseUrlForHttp * Fsm)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_UTIL, Dword, "ScanProxyUrl", "%#x", Fsm));
