...
{
    if (lpszConnection1 == NULL && lpszConnection2 == NULL) {
        return TRUE;
    }

    if (lpszConnection1 && lpszConnection2 && stricmp(lpszConnection1, lpszConnection2) == 0) {
        return TRUE;
    }
...
BOOL EnableAutodiscoverForDialup(VOID)
{
    ...
    if (UNKNOWN_AUTODETECT == dwDialUpAutodetect) {
        ...
        if (ERROR_SUCCESS != SHGetValue(HKEY_CURRENT_USER, INTERNET_POLICY_KEY, szDialupAutodetect, &dwType, &dwDialUpAutodetect, &dwSize)) {
            // not set
...
)
{
    ...
    lRes = REGCREATEKEYEX(hBaseKey, pszSubKey, 0, "", 0, regsam, NULL, &_hkey, &dwDisposition);
...

{
    ...
    ReturnValue = RegOpenKeyExW(HKEY_USERS, String, 0, samDesired, &hKeyClient);
...

{
    ...
    if (ERROR_SUCCESS == REGOPENKEYEX(FindBaseProxyKey(), pszKey, NULL, KEY_READ, &hKey)) {
        // read enable
        dwSize = sizeof(DWORD);
        if (ERROR_SUCCESS != RegQueryValueEx(hKey, szRegValProxyEnabled, NULL, NULL, (LPBYTE)&dwValue, &dwSize)) {
            dwValue = 0;
        }
        if (dwValue) {
        ...
        if (pProxy->lpszProxy) {
            if (ERROR_SUCCESS != RegQueryValueEx(hKey, REGSTR_VAL_PROXYSERVER, NULL, NULL, (LPBYTE)pProxy->lpszProxy, &dwSize) || (dwSize == 0) || (*pProxy->lpszProxy == '\0')) {
        ...
        if (pProxy->lpszProxyBypass) {
            if (ERROR_SUCCESS != RegQueryValueEx(hKey, REGSTR_VAL_PROXYOVERRIDE, NULL, NULL, (LPBYTE)pProxy->lpszProxyBypass, &dwSize) || (dwSize == 0) || (*pProxy->lpszProxyBypass == '\0')) {
        ...
        if (pProxy->lpszAutoconfigUrl) {
            if (ERROR_SUCCESS != RegQueryValueEx(hKey, szLegacyAutoConfigURL, NULL, NULL, (LPBYTE)pProxy->lpszAutoconfigUrl, &dwSize) || (dwSize == 0) || (*pProxy->lpszAutoconfigUrl == '\0')) {
                // clear out
...

{
    ...
    if (ERROR_SUCCESS == REGCREATEKEYEX(FindBaseProxyKey(), pszKey, 0, "", 0, KEY_WRITE, NULL, &hKey, &dwDisposition)) {
        fSuccess = TRUE;
        ...
        if (FALSE == fOverwrite) {
            ...
            if (ERROR_SUCCESS == RegQueryValueEx(hKey, szRegValProxyEnabled, NULL, NULL, (LPBYTE)&dwEnable, &dwSize)) {
                REGCLOSEKEY(hKey);
    ...
    SHSetValue(HKEY_CURRENT_CONFIG, pszKey, szRegValProxyEnabled, REG_DWORD, &dwValue, sizeof(DWORD));
    DEBUG_LEAVE(fSuccess);
    return fSuccess;
}
