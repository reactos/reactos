...
void dump_proxy_info(HINTERNET hInternet, LPINTERNET_PROXY_INFO ProxyInfo) {
    printf("INTERNET_PROXY_INFO for handle %#x:\n"
           ...
           "\n",
           hInternet,
           (ProxyInfo->dwAccessType == INTERNET_OPEN_TYPE_PRECONFIG) ? "PRECONFIG" :
           (ProxyInfo->dwAccessType == INTERNET_OPEN_TYPE_DIRECT) ? "DIRECT" :
           (ProxyInfo->dwAccessType == INTERNET_OPEN_TYPE_PROXY) ? "PROXY" : "?",
