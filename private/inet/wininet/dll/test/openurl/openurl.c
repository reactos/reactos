...
void _CRTAPI1 main(int argc, char** argv) {
    ...
    InternetHandle = InternetOpen("OpenUrl", accessMethod, proxyServer, NULL, AsyncMode ? INTERNET_FLAG_ASYNC : 0);
    ...
    )
{
    ...
    if (!InternetQueryOption(hInternet, INTERNET_OPTION_HANDLE_TYPE, (LPVOID)&handleType, &size)) {
...
void open_urls(LPSTR* purls, int nurls, LPSTR headers, DWORD flags, DWORD context) {
    ...
    while (nurls--) {
        ...
        handle = InternetOpenUrl(InternetHandle, *purls, headers, headers ? -1 : 0, flags, context);
...
void get_request_flags(HINTERNET hInternet) {
    ...
    if (InternetQueryOption(hInternet, INTERNET_OPTION_REQUEST_FLAGS, &dwFlags, &len)) {
        char buf[256];
        char * p = buf;
