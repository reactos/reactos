...
void _CRTAPI1 main(int argc, char** argv) {
    ...
    // open gateway
    InternetHandle = InternetOpen("cernprox", CERN_PROXY_INTERNET_ACCESS, proxy, proxyPort, 0);
...
VOID my_callback(DWORD Context, DWORD Status, LPVOID Info, DWORD Length)
{
    char* type$;
...
void open_urls(LPSTR* purls, int nurls)
{
    ...
    while (nurls--) {
        printf("\nopening URL \"%s\"\n\n", *purls);
        handle = InternetOpenUrl(InternetHandle, *purls, NULL, 0, 0, 0);
        if (handle == NULL) {
...
void get_url_data(HINTERNET handle)
{
    ...
    if (InternetQueryOption(handle, INTERNET_OPTION_HANDLE_TYPE, (LPVOID)&handleType, &handleTypeLen)) {
        if (handleType != INTERNET_HANDLE_TYPE_HTTP_REQUEST) {
            printf("error: get_url_data: handle type %d returned, should be %d\n", handleType, INTERNET_HANDLE_TYPE_HTTP_REQUEST);
        }
...
void gopher_find(HINTERNET handle)
{
    ...
    while (InternetFindNextFile(handle, (LPVOID)&data))
    {
        ...
        if ((p->LastModificationTime.dwLowDateTime != 0) && (p->LastModificationTime.dwHighDateTime != 0)) {
            FileTimeToSystemTime(&p->LastModificationTime, &systemTime);
            sprintf(timeBuf, "%02d-%02d-%02d", systemTime.wMonth, systemTime.wDay, systemTime.wYear % 100);
