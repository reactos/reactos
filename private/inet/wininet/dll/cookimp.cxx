...
BOOL FindNetscapeCookieFile( IN DWORD dwNSVer, OUT LPTSTR szFilename, /* in-out */ LPDWORD lpnBufSize)
{
    ...
    if( dwNSVer < NS_NAVI4)
    {
        ...
        if( RegQueryValueEx( hCookieKey, NS3_COOKIE_REG_KEY, NULL,  &dwType, (LPBYTE)szFilename, &cbBufSize) != ERROR_SUCCESS || dwType != REG_SZ)
        {
    ...
    OUT LPTSTR* pFilenameSubstring)
{
    ...
    if( RegQueryValueEx( hHtmlEntry, NULL, NULL, &dwType, (BYTE*)szCommandKeyPath, &cBufSize) != ERROR_SUCCESS || dwType != REG_SZ )
    {
        goto doneGetExecuteableFromExtension;
    }
    ...
    cBufSize = MAX_PATH * sizeof(TCHAR);
    if( RegQueryValueEx( hOpenCmd, "", NULL, &dwType, (BYTE*)szOpenCommand, &cBufSize) != ERROR_SUCCESS || dwType != REG_SZ)
    {
        goto doneGetExecuteableFromExtension;
    }
...
BOOL SetNetscapeImportVersion( IN DWORD dwNSVersion)
{
    ...
    if( REGCREATEKEYEX( HKEY_LOCAL_MACHINE, NS_COOKIE_IMPORT_KEY, 0, TEXT("REG_SZ"), REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hNSVersionKey, &dwDisposition) != ERROR_SUCCESS)
    {
...
BOOL GetNetscapeImportVersion( OUT DWORD* pNSVersion)
{
    ...
    if( RegQueryValueEx( hNSVersionKey, NS_COOKIE_IMPORT_VERSION, 0, &dwType, (BYTE*)pNSVersion, &dwBufSize) != ERROR_SUCCESS || dwType != REG_DWORD)
    {
        goto doneGetNetscapeImportVersion;
    }
    ...
    DWORD fDontImport;
    if( (RegQueryValueEx( hNSVersionKey, NS_COOKIE_IMPORT_FLAG, 0, &dwType, (BYTE*)&fDontImport, &(dwBufSize = sizeof(fDontImport))) == ERROR_SUCCESS) && dwType == REG_DWORD && fDontImport != FALSE)
    {
        goto doneGetNetscapeImportVersion;
    }
