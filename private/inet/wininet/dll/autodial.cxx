...

{
    ...
    if(PLATFORM_TYPE_WIN95 == GlobalPlatformType) {
        ...
        lRes = REGOPENKEYEX(HKEY_LOCAL_MACHINE, REGSTR_PATH_RNACOMPONENT, NULL, KEY_READ, &hkey);
        if(ERROR_SUCCESS == lRes) {
            cb = sizeof(szSmall);
            //  REGSTR_VAL_RNAINSTALLED is defined with TEXT() macro so
            //  if wininet is ever compiled unicode this will be a compile
            //  error.
            lRes = RegQueryValueExA(hkey, REGSTR_VAL_RNAINSTALLED, NULL, NULL, (LPBYTE)szSmall, &cb);
            if(ERROR_SUCCESS == lRes) {
...

{
    ...
    if(PLATFORM_TYPE_WIN95 == GlobalPlatformType) {
        ...
        if (osvi.dwMajorVersion < 5) {
            ...
            if(ERROR_SUCCESS == REGOPENKEYEX(HKEY_CURRENT_USER, szNTRasPhonebookKey, 0, KEY_READ, &hkey))
            {
                *szPhonebook = 0;
                cb = sizeof(ULONG);
                if(ERROR_SUCCESS == RegQueryValueEx(hkey, szPhonebookMode, NULL, NULL, (LPBYTE)&ulMode, &cb) && 0 == ulMode)
                {
                    ...
                    if(uLen)
                    {
                        ...
                        hFile = CreateFile(szPhonebook, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
--*/
{
    ...
    InitAutodialModule(TRUE);// Make sure we're all initialized
    ...
    if(FALSE == fDontProcessHook && IsAutodialEnabled(NULL, NULL))
    {
        ...
        if(pszLocalHostname)
        {
            ...
            if (SHGetValue(HKEY_LOCAL_MACHINE,szRegPathTCP, szRegValHostName,&dwValType,pszLocalHostname,&dwSize) == ERROR_SUCCESS)
            {
                if(0 == lstrcmpi(pszLocalHostname, pszHostName))
                    fDial = FALSE;
            }
            ...
            if (fDial && SHGetValue(HKEY_LOCAL_MACHINE, szRegPathComputerName, szRegValComputerName,&dwValType,pszLocalHostname,&dwSize) == ERROR_SUCCESS)
            {
                if(0 == lstrcmpi(pszLocalHostname, pszHostName))
                    fDial = FALSE;
            }
    ...
    // Didn't recognize name.  Dial.  Also call if autodial isn't enabled to handle no connection present case.
