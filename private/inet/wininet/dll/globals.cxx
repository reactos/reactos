...
// BUGBUG - all these need to be per-process. They are intended for IE
    ...
    if ( ! GlobalDisableNewCookieDlg )
    {
        ...
        if (error == ERROR_SUCCESS) {
            error = ReadRegistryDword(clientKey, "IEWarn", &dwIECookiePalEnabled);
            if (clientKey) {
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\InternetExpressLane", 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS) {
        REGCLOSEKEY(key);
        OpenIeMainKey();
...
PRIVATE VOID OpenIeMainKey(VOID)
{
    if (RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Microsoft\\Internet Explorer\\Main", 0, KEY_QUERY_VALUE, &IeMainKey) != ERROR_SUCCESS) {
        IeMainKey = NULL;
    }
}
