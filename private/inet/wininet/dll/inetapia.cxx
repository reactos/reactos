...

{
    ...
    if (lpUrlComponents->lpszHostName != NULL) {
        if (lpUrlComponents->dwHostNameLength > hostNameLength) {
            memcpy((LPVOID)lpUrlComponents->lpszHostName, (LPVOID)hostName, hostNameLength);
...
INTERNETAPI BOOL WINAPI InternetUnlockRequestFile(IN HANDLE hLockHandle)
    ...
    //else
    {
        ...
        if (dwFlags & FLAG_ICC_FORCE_CONNECTION) {
            ...
            if (buff[0] && lpServerInfo) {
                ...
                hInternet = InternetOpen("Internal", 0, NULL, NULL, 0);
                if (!hInternet) {
                    dwError = GetLastError();
                    goto Cleanup;
                }

                hConnect = InternetConnect(hInternet, buff, ServerPort, NULL, NULL, ustScheme, 0, 0);
                if (!hConnect) {
                    dwError = GetLastError();
                    goto Cleanup;
                }
            ...
            dwError = (/*vSocketsDatabase.GetSocketCount() ||*/ lpServerInfo) ? ERROR_SUCCESS : ERROR_NOT_CONNECTED;
        }
    }
