...
class PROXY_STATE {
private:
    LPSTR _lpszAutoProxyList;// _lpszAutoProxyList - List, or Proxy that we are tracking.
    DWORD _dwcbAutoProxyList;// _dwcbAutoProxyList - size of _lpszAutoProxyList.
    LPSTR _lpszOffset;// _lpszOffset - Current Offset into _lpszAutoProxyList
    LPSTR _lpszLastProxyUsed;// _lpszLastProxyUsed - Tracks the last proxy that was returned.
    INTERNET_PORT _LastProxyUsedPort;// _LastProxyUsedPort - Last used Internet Port on the Proxy
    DWORD _Error;// _Error - Error code in constructor.
...
public:
    ...
    )
    {
        ...
        if (lpszAutoProxyList && dwcbAutoProxyList > 0)
        {
    ...
    ~PROXY_STATE()
    {
        DEBUG_ENTER((DBG_OBJECTS, None, "~PROXY_STATE", NULL));
    ...
    BOOL IsAnotherProxyAvail(VOID) const
    {
        return _fIsAnotherProxyAvail;
    }

    LPSTR GetLastProxyUsed(INTERNET_PORT *pProxyPort)
    {
...
public:  //BUGBUG, I'm being lazy, need to make methods to guard access
    ...
    BOOL IsProxyEnumeration(VOID) const {
        if (_pProxyState && !IsDontWantProxyStrings() && !_pProxyState->IsEmpty())
        {
            return TRUE;
        }
    ...
    BOOL IsAlloced(VOID) const {
        return (_pmaAllocMode == MSG_ALLOC_HEAP || _pmaAllocMode == MSG_ALLOC_HEAP_MSG_OBJ_OWNS) ? TRUE : FALSE;
    }
    ...
    DWORD GetNextProxy(IN  BAD_PROXY_LIST & BadProxyList)
    {
        BOOL fSuccess;
