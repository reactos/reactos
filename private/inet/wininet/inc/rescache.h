...
typedef struct {
    ...
    LPSTR HostName;// HostName - original name resolved to Hostent
    HOSTENT_CACHE_ENTRY_STATE State;// State - unused, in-use, or delete
    LONG ReferenceCount;// ReferenceCount - only change State when zero


    // Hostent - the fixed data portion of a HOSTENT structure. The variable part overflows the end of this structure
    HOSTENT Hostent;
