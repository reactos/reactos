...
// build. Alternative is to use a private (wininet) heap (which we do in the debug version if required)
#if !defined(USE_PRIVATE_HEAP_IN_RETAIL)
#define USE_PRIVATE_HEAP_IN_RETAIL  0
#endif

// prototypes
VOID InternetDebugMemInitialize(VOID);
VOID InternetDebugMemTerminate(IN BOOL bReport);
HLOCAL InternetDebugAllocMem(IN UINT Flags, IN UINT Size, IN LPSTR File, IN DWORD Line);
HLOCAL InternetDebugFreeMem(IN HLOCAL hLocal, IN LPSTR File, IN DWORD Line);
HLOCAL InternetDebugReAllocMem(IN HLOCAL hLocal, IN UINT Size, IN UINT Flags, IN LPSTR File, IN DWORD Line);
SIZE_T InternetDebugSizeMem(IN HLOCAL hLocal, IN LPSTR File, IN DWORD Line);
BOOL InternetDebugCheckMemFreed(IN BOOL bReport);
BOOL InternetDebugMemReport(IN BOOL bTerminateSymbols, IN BOOL bCloseFile);
...
#define INITIALIZE_DEBUG_MEMORY()     InternetDebugMemInitialize()
#define TERMINATE_DEBUG_MEMORY(bReport)     InternetDebugMemTerminate(bReport)
...
#define ALLOCATOR(Flags, Size)    IEUnixLocalAlloc(Flags, Size)
#define DEALLOCATOR(hLocal)    IEUnixLocalFree(hLocal)
#define REALLOCATOR(hLocal, Size, Flags)    IEUnixLocalReAlloc(hLocal, Size, Flags)
#define MEMORYSIZER(hLocal)     IEUnixLocalSize(hLocal)
#endif /* unix */
...
#define ALLOCATE_ZERO_MEMORY(Size)     ALLOCATE_MEMORY(LPTR, (Size))
#define ALLOCATE_FIXED_MEMORY(Size)     ALLOCATE_MEMORY(LMEM_FIXED, (Size))
#define ALLOCATE_MEMORY(Flags, Size)     ALLOCATOR((UINT)(Flags), (UINT)(Size))
...
#define REALLOCATE_MEMORY(hLocal, Size, Flags)     REALLOCATOR((HLOCAL)(hLocal), (UINT)(Size), (UINT)(Flags))
