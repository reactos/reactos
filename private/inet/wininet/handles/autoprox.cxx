...
// private templates
PRIVATE BOOL MatchFileExtensionWithUrl(IN LPCSTR lpszFileExtensionList, IN LPCSTR lpszUrl);
...
BOOL CALLBACK InternetInitializeAutoProxyDll(DWORD dwReserved)
/*++
Routine Description:
...
AUTO_PROXY_ASYNC_MSG::~AUTO_PROXY_ASYNC_MSG(VOID)
{
    DEBUG_ENTER((DBG_OBJECTS, None, "~AUTO_PROXY_ASYNC_MSG", NULL));

    if ( IsAlloced() )
    {
        DEBUG_PRINT(OBJECTS, INFO, ("Freeing Allocated MSG ptr=%x\n", this));

        if ( _lpszUrl )
        {
            //DEBUG_PRINT(OBJECTS, INFO, ("Url ptr=%x, %q\n", _lpszUrl, _lpszUrl));
