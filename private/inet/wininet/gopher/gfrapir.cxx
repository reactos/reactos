...
Abstract:
    ...
    the abstract0 version (no RPC) then the A forms of the functions simply call the w functions
...
DWORD wGopherReadFile(IN HINTERNET hFile, OUT LPBYTE lpBuffer, IN DWORD dwBufferLength, OUT LPDWORD lpdwBytesReturned)
/*++
Routine Description:
...
--*/
{
    DEBUG_ENTER((DBG_GOPHER, Dword, "wGopherReadFile", "%#x, %#x, %d, %#x", hFile, lpBuffer, dwBufferLength, lpdwBytesReturned));
...
--*/
{
    DEBUG_ENTER((DBG_GOPHER, Dword, "wGopherQueryDataAvailable", "%#x, %#x", hFile, lpdwNumberOfBytesAvailable));
    ...
    if ((error == ERROR_SUCCESS) && (bytesAvailable == 0)) {
        ...
        if ((error == ERROR_SUCCESS) && (viewType == ViewTypeFind) && (bytesAvailable != 0)) {
            bytesAvailable = sizeof(GOPHER_FIND_DATA);
        }
    } else {
        error = ERROR_SUCCESS;
