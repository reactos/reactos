...
PRIVATE DWORD GopherLocatorToFindData(IN LPCSTR Locator, IN DWORD Length, OUT LPGOPHER_FIND_DATA FindData);
PRIVATE DWORD ExtractLine(IN LPVIEW_INFO ViewInfo, OUT LPBYTE LineBuffer, IN OUT LPDWORD LineBufferLength, IN OUT LPDWORD DataOffset);
PRIVATE DWORD ExtractDisplayString(IN LPCSTR Locator, IN OUT LPSTR* StringPointer, IN DWORD BufferLength);
PRIVATE BOOL SkipLeading(IN OUT LPSTR* String, IN OUT LPDWORD Length);
PRIVATE DWORD ExtractAttributeName(OUT LPSTR AttributeName, IN OUT LPDWORD AttributeNameLength, IN OUT LPSTR* LinePtr, IN OUT LPDWORD LineLength);
PRIVATE DWORD CharacterCount(IN OUT LPSTR* LinePtr, IN OUT LPDWORD LineLength, IN LPSTR TerminationSet);
PRIVATE DWORD CountCharactersToEol(IN OUT LPSTR* LinePtr, IN OUT LPDWORD LineLength);
PRIVATE VOID CopyString(IN OUT LPSTR* String, IN LPSTR Source, IN DWORD Length);


// functions


BOOL IsValidLocator(IN LPCSTR Locator, IN DWORD MaximumLength)
/*++
Routine Description:
    ...
    Port or Gopher+Stuff, since these will be sorted out by sockets functions or the gopher protocol
Arguments:
        ...
        if ((locatorLength != 0) && (locatorLength <= MaximumLength)) {
            ...
            if ((*Locator == '\t') && (locatorLength != 0)) {
                ...
                if ((*Locator == '\t') && (locatorLength != 0)) {
                    ...
                    // that comprise HostName, or the length. We used to require a non-zero length
...
BOOL IsGopherPlus(IN LPCSTR Locator)
/*++
Routine Description:
...
/*++
Routine Description:
    Given a locator, break it into its constituent parts. The Locator argument is NOT modified
...
Arguments:
    ...
    ExtraStuff              - optional returned extra (gopher+) data from end of locator.
                              ...
                              should be taken since this argument aliases Locator (or part thereof)
Return Value:
    TRUE    - locator cracked ok
    FALSE   - problem encountered cracking locator, probably substring breaks buffer limit
--*/
{
    LPSTR pTab;
    DWORD len;
...
DWORD GopherCharToType(IN CHAR GopherChar)
/*++
Routine Description:
...
Return Value:
    DWORD
        mapped gopher type or GOPHER_TYPE_UNKNOWN if we don't recognise the character
--*/
{
    // these are the types currently specified in RFC 1436 (plus a few that aren't)
...
CHAR GopherTypeToChar(IN DWORD GopherType)
/*++
Routine Description:
...
DWORD GetDirEntry(IN LPVIEW_INFO ViewInfo, OUT LPGOPHER_FIND_DATA FindData)
/*++
Routine Description:
...
--*/
{
    ...
    linePtr = lineBuf;
    error = ExtractLine(ViewInfo, (LPBYTE)linePtr, &lineLen, &ViewInfo->ViewOffset);
    ...
    if ((error == ERROR_SUCCESS) && (ViewInfo->Flags & VI_GOPHER_PLUS)) {
        ...
        tokenLength = IsGopherPlusToken(GOPHER_PLUS_INFO_TOKEN, GOPHER_PLUS_INFO_TOKEN_LENGTH, linePtr, lineLen);
        if (tokenLength != 0) {
            linePtr += tokenLength;
            lineLen -= tokenLength;
        }
    ...
    // if no error occurred, convert the locator just retrieved into the GOPHER_FIND_DATA structure
    if (error == ERROR_SUCCESS) {
        ...
        // then we must get the date and size information from the +ADMIN section Mod-Date line and +VIEWS section resp.
        ...
        if ((error == ERROR_SUCCESS) && (ViewInfo->Flags & VI_GOPHER_PLUS)) {
            ...
            do {
                ...
                lineLen = sizeof(lineBuf);
                error = ExtractLine(ViewInfo, (LPBYTE)linePtr, &lineLen, &dataOffset);
                if (error == ERROR_SUCCESS) {
                    // again, convert lineLen to the number of characters extracted
                    lineLen = sizeof(lineBuf) - (lineLen + 1);
                    ...
                    if (searchType == SEARCH_TYPE_VIEW) {
                        ...
                        ok = ExtractView(&linePtr, contentType, &contentTypeLength, language, &languageLength, &FindData->SizeLow);

                        INET_ASSERT(ok);

                        // we have the first view line. We aren't interested in the rest
                        ...
                        case GOPHER_ATTRIBUTE_ID_MOD_DATE:
                            ...
                            if (argPtr != NULL) {
                                ExtractDateAndTime(&argPtr, &FindData->LastModificationTime);
                            }
                            break;
...
/*++
Routine Description:
    Special-case version of CharacterCount - knows that the termination set comprised <CR>, <LF>
Arguments:
