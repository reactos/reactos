 ...
 * Berkeley Software License Agreement specifies the terms and conditions for redistribution.
*/
...
/*
 * Error return codes from gethostbyname() and gethostbyaddr() (when using the resolver).
 * Note that these errors are retrieved via WSAGetLastError() and must therefore follow the rules for avoiding clashes with error numbers from
 ...
 * to extract the error code from the lParam in the response to a WSAGetXByY().
*/
#define WSAGETASYNCERROR(lParam)            HIWORD(lParam)
/*
 * WSAGETSELECTEVENT is intended for use by the Windows Sockets application
 * to extract the event code from the lParam in the response to a WSAAsyncSelect().
*/
#define WSAGETSELECTEVENT(lParam)           LOWORD(lParam)
/*
 * WSAGETSELECTERROR is intended for use by the Windows Sockets application
 * to extract the error code from the lParam in the response to a WSAAsyncSelect().
*/
