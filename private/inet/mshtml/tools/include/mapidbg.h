...
/*
 * Debugging Macros
 *      IFDBG(x)        Results in the expression x if DEBUG is defined, or to nothing if DEBUG is not defined
 *      IFNDBG(x)       Results in the expression x if DEBUG is not defined, or to nothing if DEBUG is defined
 *      Unreferenced(a) Causes a to be referenced so that the compiler doesn't issue warnings about unused local variables
 *                      which exist but are reserved for future use (eg ulFlags in many cases)
*/
...
/*
 *   Assert Macros --
 *      Assert(a)       Displays a message indicating the file and line number of this Assert() if a == 0. 
 *                      OK'ing an assert traps into the debugger.
 *      AssertSz(a,sz)  Works like an Assert(), but displays the string sz along with the file and line number.
 *      Side asserts    A side assert works like an Assert(), but evaluates 'a' even when asserts are not enabled.
 *      NF asserts      A NF (Non-Fatal) assert works like an Assert(), but continues instead of trapping into the debugger when OK'ed.
*/
 ...
 *                          takes exactly the same arguments as the Windows wsprintf() function.
 *      DebugTraceResult    Shorthand for error tracing with an 
 *                          HRESULT. Arguments are the name of the function (not quoted) and the HRESULT.
 *      DebugTraceSc        Shorthand for error tracing with an
 *                          SCODE. Arguments are the name of the function (not quoted) and the SCODE.
 *      DebugTraceArg       Shorthand for invalid parameter tracing. Arguments are the name of the
 *                          function (not quoted) and a quoted string describing the bad parameter.
*/
 ...
 *      SzDecodeUlPropTag       Returns the string name of a property tag
 *      UlPropTagFromSz         Given a property tag's name, returns its value
 *      ScodeFromSz             Given an SCODE's name, returns its value
 *      DBGMEM_Encapsulate      Given an IMalloc interface, adds heap-
 *                              checking functionality and returns a wrapped interface
 *      DBGMEM_Shutdown         Undoes DBGMEM_Encapsulate, and prints out information on any allocations made
 *                              since the interface was encapsulated that have not yet been released.
 *      DBGMEM_CheckMem         Checks all memory allocated on the heap, and optionally reports leaked blocks.
 *      DBGMEM_NoLeakDetect     Prevents a block from appearing on the leak
 *                              report.  Pass NULL for pv to inhibit leak reports at all from this heap.
*/
...
/*
 *  SCODE maps -
 *      ScCheckSc       Given an SCODE and method name, verifies that the SCODE can legally be returned from 
 *                      thet method. Prints out a debug string if it cannot.
 *      HrCheckHr       As ScCheckSc, for functions that return HRESULT.
*/
