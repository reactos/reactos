...
extern "C" {
    ...
    /*
        ...
        +---------------+---------------+---------------+-------+-------+
         ...
         The Microsoft Local Data Representation (for all platforms which are of interest currently is edefined below:
    */
...
#define cbNDRContext 20         /* size of context on WIRE */
    ...
    RPC_BINDING_HANDLE RPC_ENTRY NDRCContextBinding(IN NDR_CCONTEXT CContext);
    void RPC_ENTRY NDRCContextMarshall(IN  NDR_CCONTEXT CContext, OUT void __RPC_FAR *pBuff);
    void RPC_ENTRY NDRCContextUnmarshall(
    ...
    void RPC_ENTRY NDRSContextMarshall(IN  NDR_SCONTEXT CContext, OUT void __RPC_FAR *pBuff, IN  NDR_RUNDOWN userRunDownIn);
    NDR_SCONTEXT RPC_ENTRY NDRSContextUnmarshall(IN  void __RPC_FAR *pBuff, IN  unsigned long DataRepresentation);
    void RPC_ENTRY RpcSsDestroyClientContext(IN void __RPC_FAR * __RPC_FAR * ContextHandle);
...
#define MIDL_memset(s,c,n) \
    ...
    */
    ...
    void RPC_ENTRY NDRcopy(IN void __RPC_FAR *pTarget, IN void __RPC_FAR *pSource, IN unsigned int size);
    size_t RPC_ENTRY MIDL_wchar_strlen(IN wchar_t __RPC_FAR *   s);
    void RPC_ENTRY MIDL_wchar_strcpy(OUT void __RPC_FAR *     t, IN wchar_t __RPC_FAR *   s);
    void RPC_ENTRY char_from_ndr(IN OUT PRPC_MESSAGE SourceMessage, OUT unsigned char __RPC_FAR * Target);
    void RPC_ENTRY
        char_array_from_ndr(
    ...
    void RPC_ENTRY float_from_ndr(
        IN OUT PRPC_MESSAGE  SourceMessage,
        OUT void __RPC_FAR * Target
    );

    void RPC_ENTRY float_array_from_ndr(
        IN OUT PRPC_MESSAGE SourceMessage,
        IN unsigned long    LowerIndex,
        IN unsigned long    UpperIndex,
        OUT void __RPC_FAR * Target
    ...
    void RPC_ENTRY hyper_from_ndr(IN OUT PRPC_MESSAGE  source, OUT    hyper __RPC_FAR * target);
...
#include <pshpack4.h>
    ...
    typedef struct _MIDL_STUB_MESSAGE
    {
        ...
        unsigned long               BufferLength;/* Set by the buffer sizing routines. */       
        unsigned long               MemorySize; /* Set by the memory sizing routines. */        
        unsigned char __RPC_FAR *   Memory;/* Pointer to user memory. */
...
#define NDR_SHAREABLE static
    ...
    typedef unsigned long (__RPC_FAR __RPC_USER * USER_MARSHAL_SIZING_ROUTINE)(unsigned long __RPC_FAR *, unsigned long, void __RPC_FAR *);
    ...
    typedef void (__RPC_FAR __RPC_USER * USER_MARSHAL_FREEING_ROUTINE)(unsigned long __RPC_FAR *, void __RPC_FAR *);
...
#pragma warning( default:4200 )
    ...
    */
    ...
    /*
     * Stores the translation for the conversion from a full pointer into it's corresponding ref id.
    */
    ...
    */
    typedef struct _FULL_PTR_XLAT_TABLES
    {
         ...
         * translation whenever we insert a refid-to-pointer translation, and vica versa for the client.
         */
        XLAT_SIDE               XlatSide;
...
#define USER_MARSHAL_FC_DOUBLE      12
    ...
    unsigned char __RPC_FAR * RPC_ENTRY NdrUserMarshalSimpleTypeConvert(unsigned long * pFlags, unsigned char * pBuffer, unsigned char   FormatChar);
    ...
    */
    ...
    void RPC_ENTRY NdrIsAppDoneWithPipes(NDR_PIPE_DESC  * pPipeDesc);
    void RPC_ENTRY NdrPipesDone(PMIDL_STUB_MESSAGE pStubMsg);
    ...
    */
    ...
    typedef void __RPC_FAR * __RPC_API RPC_CLIENT_ALLOC(IN size_t Size);
    typedef void __RPC_API RPC_CLIENT_FREE(IN void __RPC_FAR * Ptr);

    /*++
         RpcSs* package
    */
    void __RPC_FAR * RPC_ENTRY RpcSsAllocate(IN size_t Size);
    void RPC_ENTRY RpcSsDisableAllocate(void);
    void RPC_ENTRY RpcSsEnableAllocate(void);
    void RPC_ENTRY RpcSsFree(IN void __RPC_FAR * NodeToFree);
    RPC_SS_THREAD_HANDLE RPC_ENTRY RpcSsGetThreadHandle(void);
    void RPC_ENTRY RpcSsSetClientAllocFree(IN RPC_CLIENT_ALLOC __RPC_FAR * ClientAlloc, IN RPC_CLIENT_FREE __RPC_FAR * ClientFree);
    ...
    /*++
         RpcSm* package
    */
    void __RPC_FAR * RPC_ENTRY RpcSmAllocate(IN  size_t          Size, OUT RPC_STATUS __RPC_FAR *    pStatus);
    RPC_STATUS RPC_ENTRY RpcSmClientFree(IN  void __RPC_FAR * pNodeToFree);
    ...
    */
    ...
    int RPC_ENTRY NdrFullPointerQueryPointer(
