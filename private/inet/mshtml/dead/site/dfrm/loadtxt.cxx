        ...
        while (!file.eof() && file.good() && SUCCEEDED(hr))
        {
            ...
            if (file.good())
            {
                ...
                case FormType_Layout:
                    ...
                    hr = pCurrentRoot->InsertDOFromCLSID(CLSID_CDataFrame, &rcl, 0, (CSite **)&pCurrentDataFrame);
                    if (hr)
                        goto Cleanup;
                    ...
                    SetPropertyFromText(pDisp, _T("Name"), chUnicode, TRUE);
                ...
                case FormType_Control:
                    ...
                    {
                        RECTL rcl = { 0, 0, 0, 0 };
                        hr = pCurrentRoot->InsertDOFromCLSID(CLSID_CMdcText, &rcl, 0, (CSite **)&pControl);
                        if (!hr)
                        {
                ...
                case FormType_Property:
                    ...
                    {
                        hr = SetPropertyFromText(pDisp, chUnicode, chPropValue);
                    }
                    else
                    {
                        IControlElement * pControlI;
                ...
                case FormType_EndLayoutSection:
                    ...
                    pCurrentRoot->Move(rclDetail.left, rclDetail.top, rclDetail.Width(), rclDetail.Height());
...
HRESULT CLoadText::SetHeaderFooterRelations(void)
{
    HRESULT hr = S_OK;
...
Enum_FormType CLoadText::ParseName(Enum_FormType eft, char *pchName)
{
    ...
    {
        Assert(FALSE);
        _pDoc->ShowMessage(NULL, MB_ERRORFLAGS, 0, IDS_SYNTAX_ERROR, GetFileName(), GetLineNumber(), "section NAME is missing");
        *pchName = '\0';
        eft = FormType_Error;
    }
    else
...
HRESULT CLoadText::SetPropertyFromText(IDispatch * pDisp, TCHAR * pchPropName, TCHAR *pchPropValue, BOOL fNewTypeLib)
{
    HRESULT hr = E_FAIL;
