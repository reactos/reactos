...
class CImpIRowset : public CBase, public IRowsetExactScroll, public IRowsetInfo,
    public IAccessor, public IColumnsInfo, public IRowsetChange, public IRowsetNewRowAfter,
#ifdef NEVER
    public IRowsetFind,
#endif
...
public:
    ...
    // IRowsetInfo members
    STDMETHODIMP            GetProperties(const ULONG, const DBPROPIDSET[], ULONG *, DBPROPSET  **);
    ...
    STDMETHODIMP            Compare(HCHAPTER hChapter, ULONG cbBookmark1, const BYTE *pBookmark1, ULONG cbBookmark2, const BYTE *pBookmark2, 
                                    DBCOMPARE *pdwComparison);
    STDMETHODIMP            GetRowsAt(HWATCHREGION hRegion, HCHAPTER hChapter, ULONG cbBookmark, const BYTE *pBookmark,
                                      LONG  lRowsOffset, LONG  cRows, ULONG *pcRowsObtained, HROW **pahRows);
    ...
    STDMETHODIMP            CanConvert(DBTYPE wFromType, DBTYPE wToType, DBCONVERTFLAGS dwConvertFlags);
...
#endif
    HRESULT     FireFieldEvent(HROW hRow, ULONG cColumns, ULONG aColumns[], DBREASON eReason, DBEVENTPHASE ePhase);
    HRESULT     FireRowEvent(ULONG cRows, const HROW rghRows[], DBREASON eReason, DBEVENTPHASE ePhase);
...
private:
    ...
    //  to the actual rows being deleted will behave as if the rows have already been deleted though.
    ...
    public:
        ...
        public:
            ...
            CDeleteInProgressNode *_pDipn;  // allows for re-entrancy, although
                                            //    we only detect it and fail certain operations.

            CDeleteInProgressNode(ULONG index, ULONG cRows, CDeleteInProgress &rDip)
        ...
        CDeleteInProgressNode *_pDipn;  // head of linked-list of nested delete
                                        //  notfication info; most recently entered notification is at head of list.
        ...
        HRESULT HRowNumberToIndex(ULONG ulRow, BOOL fDeleted, ULONG &rindex, BOOL fAdjustBackward = FALSE);
    };
...
#   define DBREASON_MAX     ((DWORD) DBREASON_POPULATION_STOPPED) // for now
    ...
    HRESULT DataToWSTR(DATA_DIRECTION dDirection, ULONG uRow, ULONG uCol, XferInfo & xferData);
    void    DataCoerce_STR(ULONG uRow, ULONG uCol, BOOL fBSTR, XferInfo & xferData);
    HRESULT DataCoerce(DATA_DIRECTION dDirection, ULONG uRow, ULONG uCol, XferInfo & xferData);

    static HRESULT WhineAboutChapters(HCHAPTER hChapter);
    HRESULT Bookmark2HRowNumber(ULONG cbBookmark, const BYTE *pBookmark, ULONG &rulRow);
    HRESULT GenerateHRowsFromHRowNumber(ULONG ulFirstRow, LONG lRowsOffset, LONG cRows, ULONG *pcRowsObtained, HROW **pahRows);
    ...
    HRESULT HROW2Index(HROW hRow, ULONG &rulIndex, ULONG *pulIndexOld = NULL, BOOL *pfDeleting = NULL);
    static ULONG   ColToDBColIndex(ULONG ulColId);
    HRESULT ReleaseRowsQuiet(ULONG cRows, const HROW ahRow[], ULONG aRefCounts[] = NULL, DBROWSTATUS aRowStatus[] = NULL);
    static void LogErrors(DBROWSTATUS aRowStatus[], ULONG iFirstRow, ULONG cErrorRows, DBROWSTATUS dbrs);
...
inline HRESULT CImpIRowset::WhineAboutChapters(HCHAPTER hChapter)
{
...
//              VT_EMPTY and then zeros the entire VARIANT, this routine simply sets the type to VT_EMPTY.
//  Arguments:  pVar        VARIANT or VARIANTARG structure to initialize
//  Returns:    None
inline void CImpIRowset::FastVariantInit(VARIANTARG *pVar)
{
    Assert(pVar);
...
//              structure.  With current Nile spec, bookmarks are at at the beginning,and this routine does nothing.
//  Arguments:  ulColId     Column ID to return index into DBColumnInfo.
//  Returns:    None
inline ULONG CImpIRowset::ColToDBColIndex(ULONG ulColId)
{
    return ulColId;
}
...
inline IUnknown * CImpIRowset::getpIUnknown() const
{
