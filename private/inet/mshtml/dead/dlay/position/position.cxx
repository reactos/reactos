...
IMPLEMENT_FORMS_SUBOBJECT_IUNKNOWN(CRowPosition::MyRowsetNotify, CRowPosition, _RowsetNotify);

// Gee, why doesn't the macro above give us one of these??
STDMETHODIMP CRowPosition::MyRowsetNotify::QueryInterface(REFIID riid, LPVOID *ppv)
{
    Assert(ppv);
...
STDMETHODIMP CRowPosition::PrivateQueryInterface(REFIID riid, LPVOID * ppv)
{
...
//              describe the number of connection points our container can handle and the connection points.
//  Arguments:  None
//  Returns:    CLASSDESC
CBase::CLASSDESC * CRowPosition::GetClassDesc() const
{
    return &s_classdesc;
}
...
//  Returns:    None
HRESULT CRowPosition::Create(IUnknown *punkOuter, IRowset *pRowset, IRowPosition **ppRowPosition)
{
    HRESULT hr;
...
void CRowPosition::ReleaseResources()
...
HRESULT CRowPosition::Init(IRowset *pRowset)
{
    HRESULT hr;
...
//  Returns:    S_OK                if everything is fine,
STDMETHODIMP CRowPosition::GetRowset(REFIID riid, LPUNKNOWN *ppIUnknown)
{
    HRESULT hr;
...
//  Returns:    S_OK                if everything is fine,
STDMETHODIMP CRowPosition::SetRowPosition(HROW hRow)
{
    ...
    if (_hRow != hRow)                  // optimize no-op case
    {
        ...
        // First, OKTODO..
        hr = FireRowPositionChange(DBREASON_ROWSET_FETCHPOSITIONCHANGE, DBEVENTPHASE_OKTODO, 0, &iProgress);
        if (hr != S_OK)
            goto Cancel;

        // Then ABOUTTODO
        hr = FireRowPositionChange(DBREASON_ROWSET_FETCHPOSITIONCHANGE, DBEVENTPHASE_ABOUTTODO, 0, &iDummy);
        if (hr != S_OK)
            goto Cancel;
        ...
        FireRowPositionChange(DBREASON_ROWSET_FETCHPOSITIONCHANGE, DBEVENTPHASE_SYNCHAFTER, 1, &iDummy);

        iDummy = 0;
        // Do the DIDEVENT.  This cannot be cancelled or fail!
        FireRowPositionChange(DBREASON_ROWSET_FETCHPOSITIONCHANGE, DBEVENTPHASE_DIDEVENT, 1, &iDummy);
    }
    return hr;

Cancel:
    // If Change was cancelled, we fire a FAILEDTODO event.
    hr = FireRowPositionChange(DBREASON_ROWSET_FETCHPOSITIONCHANGE, DBEVENTPHASE_FAILEDTODO, 1, &iProgress);
...
//  Returns:    S_OK            if everything is fine,
STDMETHODIMP CRowPosition::GetRowPosition(HROW *phRow)
{
    if (_hRow)
...
//  Returns:    S_OK            if everything is fine,
HRESULT CRowPosition::CreateBookmarkAccessor()
{
    ...
    if (_pAccessor)
    {
        ...
        hr = _pAccessor->CreateAccessor(DBACCESSOR_ROWDATA, 1, &dbind, 0, &_hAccessorBookmark, NULL /*rgStatus*/);
        if (FAILED(hr))
        {
...
void CRowPosition::ClearBookmark()
{
    if (_Bookmark.ptr)
    {
...
//  Arguments: uses member variables.
void CRowPosition::ReleaseBookmarkAccessor()
{
    ULONG ulDontCare;
...
//                               (caller doesn't have to comply)
HRESULT CRowPosition::FireRowPositionChange(DBREASON eReason, DBEVENTPHASE ePhase, BOOL fCantDeny, int *piProgress)
{
    ...
    if (_pRPCSink)
    {
        while ((pRPC = (IRowPositionChange*)GetNextSink(_pRPCSink, &pdwCookie)) != NULL)
        {
...
STDMETHODIMP CRowPosition::MyRowsetNotify::OnFieldChange(IRowset *pRowset,
                                            HROW hRow,
                                            ULONG cColumns,
                                            ULONG aColumns[],
                                            DBREASON eReason,
                                          ...
                                          BOOL fCantDeny)
{
    ...
    for (i = 0; i != cRows; i++)
    {
        ...
        case DBEVENTPHASE_OKTODO:
            ...
            thr = pCRP->FireRowPositionChange(eReason, ePhase, fCantDeny, &iDummy);
        ...
        case DBEVENTPHASE_ABOUTTODO:
            ...
            else
            {
                ...
                IGNORE_HR(pCRP->_pRowset->GetData(pCRP->_hRow, pCRP->_hAccessorBookmark, &pCRP->_Bookmark));
            }
            break;
...
//              one of these because if we're still connected, the row position can't go away.
//  Arguments:  hRow            new hRow position
//  Returns:    S_OK                if everything is fine,
STDMETHODIMP CRowPosition::MyRowsetNotify::OnRowsetChange(IRowset *pRowset, DBREASON eReason, DBEVENTPHASE ePhase, BOOL fCantDeny)
{
    ...
    // Make sure to decrement refcount of our _hRow so rowset cleans up nicely.
    if (pCRP->_hRow != NULL)
    {
        IGNORE_HR(pCRP->_pRowset->ReleaseRows(1, &pCRP->_hRow, NULL, NULL, NULL));
        pCRP->_hRow = NULL;
    }
    return S_OK;
#endif
