...
/* Prototypes */
void GetDateFromForms3Version (unsigned int    EncodedDate, PSYSTEMTIME        d);
void GetDateFromSystem (PSYSTEMTIME        d);
    ...
    */
{
    ...
    (*d).wYear = 1996 + (EncodedYearMonth + 8) / 12;/* Year and month are found from 1st 2 digits of 4-digit code */
    ...
    (*d).wDay = EncodedDate % 100;/* Day = last 2 digits of 4-digit code */
}
...
{
    /*
    GetLocalTime() is a Win32 API function in winbase.h.
    It takes the place of _dos_getdate() or getdate() in typical DOS programming.
    */
    GetLocalTime(d);
}