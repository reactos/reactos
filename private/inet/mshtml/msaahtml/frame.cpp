...
{    
    m_pIUnknown = pIUnknown;// Assign controlling IUnknown.    
    m_pAOContainer = pAOContainer;// Assign AO container (parent) pointer.
}
...
{    
    return m_pIUnknown->AddRef();// delegate ref counting to owner
}
...
{ 
    return m_pIUnknown->Release();// delegate ref counting to owner
}
...
//              Constructor
//      PARAMETERS:
//              pAOParent       [IN]    Pointer to the parent accessible object in the AOM tree
//              nTOMIndex       [IN]    Index of the element from the TOM document.all collection.
//              hWnd            [IN]    Pointer to the window of the trident object that this object corresponds to.
//      RETURNS:
//              None.
...
//              pHTMLDocIUnk    [IN]    Pointer to IUnkown cast of IHTMLDocument2 pointer.
//      RETURNS:
...
HRESULT CFrameAO::Init(IUnknown * pTOMObjIUnk)
{
    ...
    // to be referenced as a FRAME element when we are looking for a match.
...
//      DESCRIPTION:
//              Standard QI implementation : the CFrameAO object only implements IUnknown.
//      PARAMETERS:
...
HRESULT CFrameAO::GetAccName(long lChild, BSTR * pbstrName)
{
    ...
    *pbstrName = NULL;
    ...
    // in order to get a fully functioning object.
