...
//    RETURNS:
//        None.
CButtonAO::CButtonAO(CTridentAO * pAOParent, CDocumentAO * pDocAO, UINT nTOMIndex, UINT nChildID, HWND hWnd) : CControlAO(pAOParent, pDocAO, nTOMIndex, nChildID, hWnd)
{
    // assign the delegating IUnknown to CButtonAO :
    // this member will be overridden in derived class
    // constructors so that the delegating IUnknown will always be at the derived class level.
    m_pIUnknown = (IUnknown *)this;

    m_lRole = ROLE_SYSTEM_PUSHBUTTON;// set the role parameter to be used in the default CAccElement implementation.
    m_lAOMType = AOMITEM_BUTTON;// set the item type so that it can be accessed via base class pointer.


    // for IE3 compatibility, buttons must determine
    // their accName and accDescription each time they are queried.
...
HRESULT CButtonAO::Init(IUnknown* pTOMObjIUnk)
{
    ...
    if (hr == S_OK)
    {
        ...
        // we want to treat the button as an atomic unit (even if the html would suggest that the button
        // should have children) because we'll fail when
        // trying to resolve the text ranges of elements scoped by the button due to a Trident bug
        m_bResolvedState = TRUE;

#ifdef _MSAA_EVENTS
        HRESULT    hrEventInit;

        // initialize event handling interface : establish Advise.
        hrEventInit = INIT_EVENT_HANDLER(ImplIHTMLButtonElementEvents, m_pIUnknown, m_hWnd, m_nChildID, pTOMObjIUnk)

#ifdef _DEBUG
            //    If we are in debug mode, assert any event handler initialization errors.

            //    (In release mode, just ignore.  This will allow
            //    the object to be created, it just won't support events.)
...
//    DESCRIPTION:
//        Standard QI implementation : the CButtonAO object only implements IUnknown.
//    PARAMETERS:
...
STDMETHODIMP CButtonAO::QueryInterface(REFIID riid, void** ppv)
{
    ...
    *ppv = NULL;
    ...
    {
        //    This is the event interface for the CButtonAO class.
    ...
    // if ppv has not been set, then we don't support the requested interface.
    if (!(*ppv))
    {
...
HRESULT    CButtonAO::GetAccState(long lChild, long* plState)
{
    ...
    *plState = 0;
    ...
    {
        //    If the Trident button is an INPUT TYPE=SUBMIT, then this is a default button.
...
//    PARAMETERS:
//        pbDisabled    set to TRUE is Trident button is disabled, FALSE if enabled
//    RETURNS:
...
HRESULT CButtonAO::getDescriptionString(BSTR* pbstrDescStr)
{
    ...
    *pbstrDescStr = NULL;
    ...
    //    If the Trident is an INPUT button, the face text is the INPUT's VALUE.
    //    Otherwise, the Trident is a BUTTON button and the face text is the BUTTON's INNERTEXT.
    if (m_bIsInputButtonType)
    {
        CComQIPtr<IHTMLInputButtonElement, &IID_IHTMLInputButtonElement> pIHTMLInputButtonElement(m_pTOMObjIUnk);
