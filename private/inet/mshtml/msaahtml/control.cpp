...
//     CControlAO implements common functionality that the CButtonAE, the CEditFieldAE,
//     and the COptionButtonAE (base for both CCheckboxAE and CRadioButtonAE) classes all share.
...
//PARAMETERS:
//    pAOParent  pointer to the parent accessible object in the AOM tree
//    nTOMIndex  index of the element from the TOM document.all collection.
//    nChildID   unique Child ID
//    hWnd       pointer to the window of the trident object that this object corresponds to.
{
    //Controls, except buttons, can determine their accName and accDescription once and then cache those BSTRs.
...
//    PARAMETERS:
//        lChild              Child ID
//        pbstrDescription    pointer to the BSTR to return the description string in
//    RETURNS:
//        HRESULT :    S_OK | DISP_E_MEMBERNOTFOUND
{
    HRESULT hr = S_OK;
...
//    PARAMETERS:
//        lChild                       child ID
//        pbstrKeyboardShortcut        pointer to BSTR to return keyboard shortcut string in
//    RETURNS:
//        HRESULT :    S_OK | DISP_E_MEMBERNOTFOUND
//    NOTES:
//        All Trident controls support the IHTMLControlElement interface.
{
    ...
    if (m_bstrKbdShortcut) {
        *pbstrKeyboardShortcut = SysAllocString(m_bstrKbdShortcut);
        ...
        //    Get the shortcut via IHTMLControlElement accessKey property
...
//        HRESULT :    S_OK | E_FAIL | DISP_E_MEMBERNOTFOUND | E_NOINTERFACE
{
    ...
    *plState = 0;//    Initialize the out parameter.

    //    Determine if the control is enabled.  If not, set state to STATE_SYSTEM_UNAVAILABLE.

    //    To determine whether or not the control is enabled, call the virtual CControlAO method
    //    isControlDisabled().  This method should be
    //    overridden by derived class for handling of particular controls.
    ...
    //    This call returns either S_OK or DISP_E_MEMBERNOTFOUND.
    //     If the latter is returned, reset the state to zero.
    ...
    if (hr == S_OK && uTOMID > 0) {
        ...
        //    GetFocusedTOMElementIndex() returned something other than S_OK (or an invalid source index)
        //    meaning that the source index of the focused TEO could not be obtained.
        //    So, just assume that there is no active element, which means that our CControlAO can neither be FOCUSABLE nor FOCUSED.
        hr = S_OK;
    }
...
//    RETURNS:
//        HRESULT :    S_OK, E_OUTOFMEMORY, DISP_E_MEMBERNOTFOUND, or other standard COM error as returned by Trident.
{
    HRESULT hr;
...
//    PARAMETERS:
//        pbDisabled    set to TRUE is Trident button is disabled, FALSE if it is enabled
//    RETURNS:
...
//        HRESULT
//    NOTES:
//        This method overrides the method defined in the base class, CTridentAO.

//        For IE3 compatibility, the accName of controls is the traditional value: for buttons, its the "face" text and for
//        check boxes, radio buttons, edit fields its the static text associated with the LABEL tag.  The TITLE attribute is
//        used as the accDescription, or as the accName when the traditional value is not set.  So, although the method name
//        is confusing, the BSTR set in getDescriptionString() is used as the accName and the BSTR set in
//        getTitleFromIHTMLElement() is used as the accDescription.

//        For IE3 compatibility, the accName property of the CButtonAO must not be cached.  Given the current architecture and the
//        interdependence of the button's accName and accDescription, this means two things.  First, the accDescription property
//        is not cached.  Second, the lack of caching is achieved by never the CButtonAO class setting the member boolean
//        m_bCacheNameAndDescription to FALSE in its ctor.
{
    HRESULT hr = E_FAIL;
    BSTR    bstrTmp = NULL;
    
    m_bNameAndDescriptionResolved = FALSE;//Initialize affected member variables.
    ...
    //    If the traditional control name is defined, use it for the control's accName.
    ...
    if (bstrTmp) {
        ...
        //    The accName property has been cached, now let's try to get the accDescription.

        //    If the control has a TITLE attribute, use that as its description.
        bstrTmp = NULL;
        ...
        //    The traditional control name is not defined, so use its TITLE, if it exists, for the accName.
        //    Note that this means that the control's description will be empty.
        hr = getTitleFromIHTMLElement(&bstrTmp);
        if (hr == S_OK) {
            if (bstrTmp) {
                m_bstrName = SysAllocString(bstrTmp);
...
//        pbstrDescStr    [out]    pointer to the BSTR to hold the INNERTEXT
{
    ...
    *pbstrDescStr = NULL;//Initialize the out parameter.
    ...
    //    If S_FALSE was returned, there is no LABEL associated with this control.
    else if (hr == S_FALSE)
        hr = S_OK;
...
//    DESCRIPTION:
//        Get the IHTMLElement pointer to the LABEL tag associated with the AE's Trident.
//    PARAMETERS:
//        ppIHTMLElementLABEL        pointer to a pointer to an IHTMLElement
//    RETURNS:
//        S_OK if the AE has an associated LABEL tag
//        S_FALSE if the AE has no associated LABEL tag valid COM error otherwise
{
    HRESULT    hr = S_OK;
    
    *ppIHTMLElementLABEL = NULL;//Initialize the out parameter.
    ...
    //    Obtain the IHTMLElement* for the parent of the AE's Trident.
    ...
    //    If it is, it should support the IHTMLLabelElement interface.  QI for this iface.
    ...
    if (pIHTMLLabelElement) {
        ...
        //    so the AE's parent is a LABEL.  Set the out parameter appropriately.
...
//    PARAMETERS:
//        pIHTMLElement            pointer to an IHTMLElement (this is the TEO associated with the CTridentAE)
//        pbstrID                    pointer to a BSTR
//        ppIHTMLElementLABEL        pointer to a pointer to an IHTMLElement
//    RETURNS:
//        S_OK if there is a LABEL whose HTMLFOR equals pcstrID
//        S_FALSE if there's no such LABEL and but no errors valid COM error otherwise
{
    HRESULT hr = S_OK;
    ...
    *ppIHTMLElementLABEL = NULL;
    ...
    //    Traverse the collection of LABELs looking for one whose HTMLFOR property is the same as the ID property of the control.
    ...
    while (nCurLabel < nCollectionLen  &&  hr == S_OK && !bLabelFound) {
        ...
        //    is the same as ID property of the control, we've found our LABEL.
        BSTR    bstrHTMLFor;
        ...
        if (bstrHTMLFor) {
            if (!_wcsicmp(*pbstrID, bstrHTMLFor)) {                
                bLabelFound = TRUE;//    This is the LABEL we are looking for!
...
//        lpcwstrTagName        pointer to a wide string that holds the name of the tag for the desired collection
//        ppTagsCollection    pointer to the IHTMLElementCollection pointer that will point to the desired collection
//    RETURNS:
//        HRESULT                S_OK or a COM error code.
//    NOTES:
//        It is assumed that the IHTMLElement* pointer (parameter 1)
//        will be valid for the duration of this method and that it is not this method's responsibility to release the interface.

//        This method sets the out parameter to point to the equivalent
//        of the script document.all.tags(<tag_name>) where tag_name is specified by the BSTR in parameter.
//        The IHTMLElement* in parameter is used to obtain the IHTMLDocument2* which starts things off.
{
    HRESULT    hr;
    ...
    //    Get the all collection (IHTMLElementCollection) pointer.
    ...
    //    Get the collection (IHTMLElementCollection) of requested tags.
    VARIANT varTagName;
