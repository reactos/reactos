...
//      Notes:  The CWindowAO class implements the main proxy object for MSAAHTML.
//              As a main proxy object, it exposes and implements
...
//              the AOM tree and handles all AOM heirarchy navigation and information retrieval.
...
//                  remove themselves from the proxy manager's window list.
{
    ...
    DetachChildren();// detach our own children
    ReleaseTridentInterfaces();// release all cached Trident objects
    m_pProxyMgr->RemoveAOMWindow(this);// since we are detached, remove us from the proxy managers list.
    Release();
}
...
//      AccessibleObjectFromWindow() defined in API.CPP.
{
    ...
    *ppTridentDoc = NULL;
    ...
    if (IsWindow(m_hWnd))
    {
        ...
        //  but Trident returns a failure code while processing the message, return the specified error code.
        ...
        else if (ref)
        {
            ...
            // Now we get the IID_IHTMLWindow2 pointer from the document interface.
