...
CJPEGFilter::CJPEGFilter() : m_pStream(NULL), m_iScanLine(0), m_iFirstStaleScanLine(0), m_pSource(NULL), m_pbScanLines(NULL), m_bRGB8Allowed(FALSE)
{
...
CJPEGSource::CJPEGSource(CJPEGFilter* pFilter) : m_pFilter(pFilter), m_pbBuffer(NULL)
{
...
boolean CJPEGSource::FillInputBuffer(j_decompress_ptr pInfo)
{
    ...
    hResult = pSource->m_pFilter->m_pStream->Read(pSource->m_pbBuffer, IMGFILT_BUF_SIZE, &nBytesRead);
    if (FAILED(hResult)) {
...
void CJPEGSource::SkipInputData(j_decompress_ptr pInfo, long nBytes)
{
     ...
     *  Could use fseek() except it doesn't work on pipes.
...
HRESULT CJPEGFilter::FireGetSurfaceEvent()
{
    ...
    hResult = m_pEventSink->GetSurface(m_nWidth, m_nHeight, bfid, 1, IMGDECODE_HINT_TOPDOWN | IMGDECODE_HINT_FULLWIDTH, &pSurface);
    if (FAILED(hResult)) {
        return hResult;
    }

    if (m_dwEvents & IMGDECODE_EVENT_USEDDRAW) {
        pSurface->QueryInterface(IID_IDirectDrawSurface, (void **) &m_pDDrawSurface);
    }

    if (m_pDDrawSurface == NULL)
    {
        hResult = pSurface->QueryInterface(IID_IBitmapSurface, (void **) &m_pBitmapSurface);
        if (FAILED(hResult)) {
            return(hResult);
        }
    }

    // If this is a grayscale JPEG set the color table to a gray palette, if the surface is an indexed surface as well.
    //   If the caller only allows 24bpp surfaces, we will setup the JPEG decoder to do the work for us.
    ...
    if (bfid == BFID_RGB_8)
    {
        ...
        else
        {
            ...
            hResult = m_pBitmapSurface->QueryInterface(IID_IRGBColorTable, (void**) &pColorTable);
            if (FAILED(hResult)) {
                return (hResult);
            }
...
STDMETHODIMP CJPEGFilter::Initialize(IImageDecodeEventSink* pEventSink)
{
    ...
    } __except ((GetExceptionCode() == JPEG_EXCEPTION) ? EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH) {
        return(E_FAIL);
    }
...
STDMETHODIMP CJPEGFilter::Process(IStream* pStream)
{
    ...
    // We have to do this every time.  We don't AddRef, since we don't hold onto the stream.
    m_pStream = pStream;
    ...
    __try
    {
        ...
        while (m_iScanLine < m_nHeight)
        {
            ...
            hResult = LockBits(&rect, SURFACE_LOCK_EXCLUSIVE, &pBits, &nPitch);
            if (FAILED(hResult)) {
                return(hResult);
            }
            ...
            for (iScanLine = 0; iScanLine < nScanLinesRead; iScanLine++)
            {
                ...
                if (m_nBytesPerPixel == 3)
                {
                    if (m_jpegInfo.out_color_space == JCS_RGB)
                    {
                        ...
                        // Grayscale image for client that only allows 24bpp surfaces.
                        for (iPixel = 0; iPixel < m_nWidth; iPixel++)
                        {
                    ...
                    memcpy(pbDestPixel, pbSrcPixel, m_nWidth);// Indexed grayscale image
                }
            }
