   ...
   REFGUID bfid, BOOL bAnimation, IBitmapSurfaceFactory* pFactory )
{
   ...
   m_pBits = LPBYTE( VirtualAlloc( NULL, m_nBufferSize, MEM_RESERVE, PAGE_READWRITE ) );
   if( m_pBits == NULL )
   {
   ...
   void** ppBits, LONG* pnPitch )
{
   ...
   if( pBounds == NULL )
   {
      if( (m_pFirstCommittedByte > m_pBits) || (m_pLastCommittedByte < (m_pBits+m_nBufferSize) ) )
      {
         pBits = LPBYTE( VirtualAlloc( m_pBits, m_nBufferSize, MEM_COMMIT, PAGE_READWRITE ) );
         if( pBits == NULL )
         {
            m_csLock.Unlock();
            InterlockedDecrement( &m_nLockCount );
   ...
   {
      pFirstCommittedByte = m_pBits+(pBounds->top*m_nPitch)+ ((pBounds->left*m_nBitsPerPixel)/8);
      pLastCommittedByte = m_pBits+((pBounds->bottom-1)*m_nPitch)+ ((pBounds->right*m_nBitsPerPixel)/8);
      if( (pFirstCommittedByte < m_pFirstCommittedByte) || (pLastCommittedByte > m_pLastCommittedByte) )
      {
         pBits = LPBYTE( VirtualAlloc( pFirstCommittedByte, pLastCommittedByte-pFirstCommittedByte, MEM_COMMIT, PAGE_READWRITE ) );
         if( pBits == NULL )
         {
            m_csLock.Unlock();
            InterlockedDecrement( &m_nLockCount );
         ...
         m_pFirstCommittedByte = min( pFirstCommittedByte, m_pFirstCommittedByte );
         m_pLastCommittedByte = max( pLastCommittedByte, m_pLastCommittedByte );
      }
