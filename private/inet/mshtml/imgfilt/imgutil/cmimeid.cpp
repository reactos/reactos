   ...
   IStream** ppOutStream )
{
   ...
   {
      hResult = CoCreateInstance( CLSID_CoSniffStream, NULL, CLSCTX_INPROC_SERVER, IID_ISniffStream, (void**)&pSniffStream );
      if( FAILED( hResult ) )
      {
         return( hResult );
      }
...
HRESULT CMIMEType::InitFromKey( HKEY hKey, LPCTSTR pszName, ULONG* pnMaxBytes )
{
   ...
   nResult = RegQueryValueEx( hBitsKey, _T( "0" ), NULL, &dwValueType, NULL, &nBytes );
   ...
   nResult = RegQueryValueEx( hBitsKey, _T( "0" ), NULL, &dwValueType, pData, &nBytes );
   if( nResult != ERROR_SUCCESS )
   {
      return( E_FAIL );
   }
