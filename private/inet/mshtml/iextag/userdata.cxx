...
CPersistUserData::initXMLCache(BOOL fReset/*=false*/)
{
    ...
    if (!fReset)
    {
        ...
        hr = CoCreateInstance(CLSID_DOMDocument, 0, CLSCTX_INPROC_SERVER, IID_IXMLDOMDocument, (void **)&_pInnerXMLDoc);
        if (hr)
            goto Cleanup;
...
//           userdata object determines if the scheme (i.e. protocol) it uses is supported.
...
CPersistUserData::GetDirPath(LPOLESTR *ppDirPath)
{
    ...
    if ((hr = pdispBrowseDoc->QueryInterface(IID_IHTMLDocument2, (void**)&pBrowseDoc)) != S_OK)
        goto cleanup;
