...
CPersistDataPeer::BuildNewScriptBlock(CBufferedStr * pstrBuffer, ULONG *puFlags)
{
    ...
    hr = (pdispBrowseDoc->QueryInterface(IID_IHTMLDocument2, (void**)&pBrowseDoc));
    if (hr)
        goto Cleanup;
...
HRESULT CPersistDataPeer::SaveHandler_ScriptTag()
{
    HRESULT          hr = S_OK;
    CVariant         cvarText;
    BSTR             bstrLanguage = NULL;
    ...
    // get the new script block, and the language that it originated from
    hr = BuildNewScriptBlock(&cbsNewScript, &uFlags);
    if (hr)
        goto Cleanup;

    // Language can only be set in design mode,  as a result we can't change the script block since we don't know the
    //  syntax for an arbitrary script engine.  so, only save script if this is javascript or vbscript.
