...
STDMETHODIMP CHomePage::IsAuthorized(BSTR bstrURL)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr)) {
            if (SUCCEEDED(hr = CoCreateInstance(CLSID_InternetSecurityManager, NULL, CLSCTX_INPROC_SERVER, IID_IInternetSecurityManager, (void **)&pISM))) {
...
STDMETHODIMP CHomePage::SetUserHomePage(LPCSTR szURL)
{
    ...
    {
        if (((lResult = RegSetValueExA (hKey, REGSTRA_VAL_STARTPAGE, 0, REG_SZ, (LPCBYTE)szURL, lstrlenA(szURL)+1))) != ERROR_SUCCESS) {
