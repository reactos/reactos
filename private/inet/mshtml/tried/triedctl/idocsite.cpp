...
/*
    ...
    CSite controls the destruction of the object through delete, not reference counting
    */
 ...
 * Purpose:
 *  Instructs the container to activate the object in this site as a document object.
...
STDMETHODIMP CImpIOleDocumentSite::ActivateMe(IOleDocumentView *pView)
{
    ...
    /*
     * If we're passed a NULL view pointer, then try to get one from the document object (the object within us).
     */
    if (NULL == pView)
    {
        if (FAILED(m_pSite->GetObjectUnknown()->QueryInterface(IID_IOleDocument, (void **) &pDoc)))
        {
            return E_FAIL;
        }

        if (FAILED(pDoc->CreateView(m_pSite->GetIPSite(), NULL, 0, &pView)))
        {
            pDoc->Release();
            return E_OUTOFMEMORY;
        }

        pDoc->Release();// Release doc pointer since CreateView is a good com method that addrefs
    }
    else
    {
        pView->SetInPlaceSite(m_pSite->GetIPSite());//Make sure that the view has our client site
        pView->AddRef();//We're holding onto the pointer, so AddRef it.
    }

    // Activation steps, now that we have a view:
    m_pSite->SetDocView(pView);

    pView->UIActivate(TRUE);//This sets up toolbars and menus first

    //Set the window size sensitive to new toolbars
    m_pSite->GetFrame()->GetControl()->GetClientRect(&rc);
    pView->SetRect(&rc);

    pView->Show(TRUE);//Makes it all active
    return S_OK;
}