...
// Load the glyphs from the DLL and install them in Trident's table. No return value.
void CTriEditDocument::SetUpGlyphTable(BOOL fDetails)
{
    ...
    m_pCmdTgtTrident->Exec(&CMDSETID_Forms3, IDM_EMPTYGLYPHTABLE, OLECMDEXECOPT_DONTPROMPTUSER, NULL, NULL);

    V_VT(&var) = VT_BSTR;
    V_BSTR(&var) = SysAllocString(szGlyphTable);
    m_pCmdTgtTrident->Exec(&CMDSETID_Forms3, IDM_ADDTOGLYPHTABLE, OLECMDEXECOPT_DONTPROMPTUSER, &var, NULL);
    VariantInit(&var);
}
                ...
                case IDM_TRIED_NUDGE_ELEMENT:
                    {
                        ...
                        if (SUCCEEDED(GetElement()) && m_pihtmlElement && SUCCEEDED(Is2DElement(m_pihtmlElement, &f2d)) && f2d)
                        {
                            BOOL fLock = FALSE;
                ...
                case IDM_TRIED_UNORDERLIST:
                    {
                        ...
                        if (pCmd->cmdID == IDM_TRIED_SHOWBORDERS || pCmd->cmdID == IDM_TRIED_SHOWDETAILS)
                        {
...
STDMETHODIMP CTriEditDocument::Exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdExecOpt, VARIANTARG *pvaIn, VARIANTARG *pvaOut)
{
    if (pguidCmdGroup && IsEqualGUID((const GUID&)*pguidCmdGroup, GUID_TriEditCommandGroup) && m_pUnkTrident)
    {
        ...
        switch(nCmdID)
        {
            case IDM_TRIED_IS_1D_ELEMENT:   //[out,VT_BOOL]
                if (pvaOut && m_pihtmlElement && SUCCEEDED(VariantChangeType(pvaOut, pvaOut, 0, VT_BOOL)))
                {
           ...
           case IDM_TRIED_IS_2D_ELEMENT:   //[out,VT_BOOL]
            ...
            case IDM_TRIED_LOCK_ELEMENT:
                if (m_pihtmlElement)
                {
                    BOOL f2d=FALSE;
                    BOOL fLocked=TRUE;
                    if (SUCCEEDED(Is2DElement(m_pihtmlElement, &f2d)) && f2d && SUCCEEDED(IsLocked(m_pihtmlElement, &fLocked)))
                    {
            ...
            case IDM_TRIED_UNORDERLIST:
                {
                    ...
                    {
                        if (nCmdID == IDM_TRIED_ACTIVATEACTIVEXCONTROLS || nCmdID == IDM_TRIED_ACTIVATEAPPLETS || nCmdID == IDM_TRIED_ACTIVATEDTCS)
                        {
...
HRESULT CTriEditDocument::NudgeElement(IHTMLElement* pihtmlElement, LPPOINT pptNudge)
{
    ...
    if (pihtmlElement)
    {
        if (SUCCEEDED(pihtmlElement->get_style(&pihtmlStyle)) && pihtmlStyle && SUCCEEDED(pihtmlStyle->get_pixelTop(&y)) && SUCCEEDED(pihtmlStyle->get_pixelLeft(&x)))
        {
            if (x == 0 || y == 0)
            {
                ...
                if (SUCCEEDED(pihtmlElement->get_offsetParent(&pihtmlElementParent)) && pihtmlElementParent)
                {
...
STDMETHODIMP CTriEditDocument::GetDocument(IHTMLDocument2** ppihtmlDocument)
{
    ...
    if (ppihtmlDocument)
    {
        if (m_pUnkTrident)
        {
            return m_pUnkTrident->QueryInterface(IID_IHTMLDocument2, (LPVOID*)ppihtmlDocument);
        }
...
// Return the all collection of the HTML document (under *ppihtmlCollection), or E_FAIL.
