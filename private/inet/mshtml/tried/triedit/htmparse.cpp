...
#ifdef NEEDED
inline int indexPrevtokTagStart(int index, TOKSTRUCT *pTokArray)
{
    while ((index >= 0) && (pTokArray[index].token.tokClass != tokTag) && (pTokArray[index].token.tok != TokTag_START))
    {
        index--;
    }
    return(index);
...
inline int indexPrevTokElem(int index, TOKSTRUCT *pTokArray)
{
    while ((index >= 0) && (pTokArray[index].token.tokClass != tokElem))
    {
        index--;
    }
    return(index);
...
BOOL FIsWhiteSpaceToken(WCHAR *pwOld, int indexStart, int indexEnd)
{
    ...
    {
        if (pwOld[index] != ' ' && pwOld[index] != '\t' && pwOld[index] != '\r' && pwOld[index] != '\n')
        {
            fWhiteSpace = FALSE;
            break;
        }
...
BOOL FIsAbsURL(LPOLESTR pstr)
{
    ...
    if (0 == _wcsnicmp(szHttp[0], pstr, wcslen(szHttp[0])) || 0 == _wcsnicmp(szFile[0], pstr, wcslen(szFile[0])))
    {
        fRet = TRUE;
        goto LRet;
    }
...
BOOL FURLNeedSpecialHandling(TOKSTRUCT *pTokArray, int iArray, LPWSTR pwOld, int cMaxToken, int *pichURL, int *pcchURL)
{
    int index = iArray + 1;
    int iHref = -1;
    int iURL = -1;
    ...
    while (index < cMaxToken && pTokArray[index].token.tok != TokTag_CLOSE && pTokArray[index].token.tokClass != tokTag)
    {
        if (iHref == -1 &&
        ...
        if (iHref != -1 && pTokArray[index].token.tok == 0 && (pTokArray[index].token.tokClass == tokString
            || pTokArray[index].token.tokClass == tokValue))
        {
            iURL = index;
            break;
        }
    ...
    if (iURL != -1)//its set properly
    {
        ...
        *pichURL = (fQuote) ? pTokArray[iURL].token.ibTokMin + 1 : pTokArray[iURL].token.ibTokMin;
        // special case - if the quoted value happens to be a serverside script,
        // we can ignore it here and decalre that we don't need to do any special processing.
        if (((pTokArray[iURL].token.ibTokMac - pTokArray[iURL].token.ibTokMin) == 1) || (cchURL < 0))
        {
            *pcchURL = 0;
            goto LRet;
        }
        *pcchURL = cchURL;
        ...
        ASSERT(pszURL != NULL);
        memcpy((BYTE *)pszURL, (BYTE *)&pwOld[pTokArray[iURL].token.ibTokMin + ((fQuote) ? 1 : 0)], (cchURL)*sizeof(WCHAR));
        pszURL[cchURL] = '\0';
...
static LPOLESTR SkipServer(LPOLESTR pstr)
...
//  Assumes that protocol-less URLs are "http:". Must specify "file:" explicitly to play with file URLs.
{
    pstr = wcschr(pstr, L'/');
    if (pstr == NULL)
        return NULL;
...
static LPOLESTR SkipFile(LPOLESTR pstr)
{
    LPOLESTR    pstrT;
    ...
    if (pstrT == pstr && IsDirSep(pstr[0]) && IsDirSep(pstr[1]))
    {
...
static LPOLESTR FindLastSlash(LPOLESTR pstr)
{
    LPOLESTR    pstrSlash;// '/'
    LPOLESTR    pstrWhack;// '\'
...
HRESULT UtilConvertToRelativeURL(
    ...
    BSTR *   pbstrRelativeURL)
{
    ...
    {
        // Check the protocol against the two we understand. If it is some other thing, we punt.
    ...
    // If both the URL and the folder had absolute paths, we need to ensure that the servers are the same.
...
STDMETHODIMP CTriEditParse::NextToken(LPCWSTR pszText, UINT cbText, UINT* pcbCur, DWORD*  pLXS, TXTB* pToken)
{
    ASSERT(pszText != NULL);
    ASSERT(pcbCur != NULL);
    ASSERT(pLXS != NULL);
    ...
    *pcbCur = GetToken(pszText, cbText, *pcbCur, pLXS, *pToken);
    ...
    cch = WideCharToMultiByte(CP_ACP, 0, pszText, cbText, NULL, 0, NULL, NULL);
...
void CTriEditParse::InitSublanguages()
{
    ...
    {
        TCHAR strHTML2[2048];
        ::LoadString(_Module.GetModuleInstance(), IDS_RFC1866, strHTML2, sizeof(strHTML2));
...
// Reallocs are expensive, so when we Realloc, should we add some more pad so that we wont have to call Realloc very often?
HRESULT ReallocBuffer(HGLOBAL *phg, DWORD cbNew, UINT uFlags)
{
    HRESULT hr = S_OK;
...
HRESULT ReallocIfNeeded(HGLOBAL *phg, WCHAR **ppwNew, UINT cbNeed, UINT uFlags)
{
    HRESULT hr = S_OK;
...
void CTriEditParse::fnRestoreSSS(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                 TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok ft,
                                 INT *pcSSSOut, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    ...
    while (cSSSOut > 0)
    {
        ...
        // Here's the deal - we have to ignore all SSS that appear as values inside client scripts or insize objects/dtcs
        // so, we need to skip this TokElem_SCRIPT tag if we found '</' before TokElem_SCRIPT
        if (pTokArray[iArray].token.tok == TokElem_SCRIPT && pTokArray[iArray - 1].token.tok != TokTag_START)
        {
        ...
        // we don't really need to look for this till ptep->m_cMaxToken, but this will cover boundary cases
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichScrStart - ichBeginCopy)*sizeof(WCHAR));
        ...
        if (fSpecialSSS)
        {
            ...
            memmove((BYTE *)(&pwNew[ichSSSEnd - ichSSSStart - 3]), (BYTE *)pwNew, (ichNewCur)*sizeof(WCHAR));
        ...
        else
        {
            ...
            ichSp = ichNewCur - 1;
            while ((ichSp >= 0) && (pwNew[ichSp] == ' ' || pwNew[ichSp] == '\r' || pwNew[ichSp] == '\n' || pwNew[ichSp] == '\t'))
            {
                ichSp--;
            }
            ...
            {
                memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichScrWspBegin], (ichSSSStart - ichScrWspBegin)*sizeof(WCHAR));
            ...
            {
                memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichSSSEnd], (ichScrWspEnd - ichSSSEnd)*sizeof(WCHAR));
            ...
            while (iArray < (int)ptep->m_cMaxToken)
            {
                ...
                {
                    if (pwOld[ich] != ' ' && pwOld[ich] != '\t' && pwOld[ich] != '\r' && pwOld[ich] != '\n')
                    {
                        fNonWspToken = TRUE;
                        break;
                    }
...
void CTriEditParse::fnSaveSSS(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray,
                              UINT *piArrayStart, FilterTok ft, INT *pcSSSIn, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    // Server Side Script case
    // This occur inside <%  %>. We assume simple SSS
    // add <SCRIPT LANGUAGE=SERVERASP> & </SCRIPT> around it tag used for saving the SSS.
    ...
    while (cSSSIn > 0)
    {
        ...
        while (index >= 0)
        {
            if ((pTokArray[index].token.tok == TokElem_XMP || pTokArray[index].token.tok == TokElem_PLAINTEXT) &&
                pTokArray[index].token.tokClass == tokElem && pTokArray[index].iNextprev > iArray)
            {
                iArray++;
                goto LRet;
            }
        ...
        if (pTokArray[i].iNextprev != -1)
        {
            ...
            // this will be a wierd case where the iNextprev is incorrectly pointing to another token but lets handle that case.
        ...
        cbNeed = wcslen(rgSSSTags[0]) + wcslen(rgSSSTags[0]) + wcslen(rgSSSTags[2]) + (ichSSSEnd - ichSSSStart) + cbMin;
        ...
        // ASSUMPTION - The big assumption we are making is that IE5 doesn't change anything inside the client sctipt.
        // So far we have seen that. In the worst case, if they start mucking with the contents of client script,
        // we will loose the spacing, but there will be NO DATA LOSS.

        // Based on this assumption, we simply save the pre-post script spacing as is and expect to restore it on the way out.
        ...
        ichSp = ichSSSStart - 1;
        while ((ichSp >= 0) && (pwOld[ichSp] == ' ' || pwOld[ichSp] == '\r' || pwOld[ichSp] == '\n' || pwOld[ichSp] == '\t'))
        {
            ichSp--;
        }
        ...
        ichSp = ichSSSEnd;
        while ((ichSp < pTokArray[ptep->m_cMaxToken - 1].token.ibTokMac - 1) && (pwOld[ichSp] == ' '
        ...
        // ichBeginCopy is a position in pwOld and ichNewCur is a position in pwNew
        if ((int)(ichSSSStart - ichBeginCopy) >= 0)
        {
            // copy till begining of the <%
            memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichSSSStart - ichBeginCopy)*sizeof(WCHAR));
            ichNewCur += ichSSSStart - ichBeginCopy;
            ichBeginCopy = ichSSSEnd; // set it for next script

            // copy the converted SSS
            memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(pSSS), cchCurSSS*sizeof(WCHAR));
            ichNewCur += cchCurSSS;
        }
...
void CTriEditParse::fnRestoreDTC(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray,
                                 UINT *piArrayStart, FilterTok ft, INT *piObj, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    ...
    // look for the /OBJECT (i.e. look for OBJECT and look for previous /) and once we get those two next to each other,
    // wait for upcoming toktag_CLOSE which will end that Object remember ibTokMac at that position.
    // This is the OBJECT range.
    // First, insert the startspan text
    // Then generate and insert the endspan text (note that we may have to extend our buffer becausethe generated text mey not fit.
    ...
    {
        if (pTokArray[i].token.tok == TokTag_BANG && pTokArray[i].token.tokClass == tokTag)
        {
            ...
            if ((pwOld[pTokArray[i + 1].token.ibTokMin] == '-')
                ...
                && (0 == _wcsnicmp(rgCommentRT[0],
                &pwOld[pTokArray[i + 1].token.ibTokMac - 2 - wcslen(rgCommentRT[0])],
                wcslen(rgCommentRT[0])))
                )
            {
    ...
    *piObj += 1; // get it ready for the next control
    ...
    // Trident has a bug that if the object was nested inside <scripts> tags, it returns S_OK with pDispControl as NULL. (See VID BUG 11303)
    ...
    if (hr == S_OK) // Found the control!
    {
        ...
        i = indexObjStart;
        // actually, this has to exist before TokElem_PARAM, but this takes care of boundary cases
        while (i < (int)indexObjEnd)
        {
        ...
        if (!(dwFlags & dwFilterDTCsWithoutMetaTags))
        {
            ...
            // NOTE - we want to make sure that the classId value is inside quotes, if there is one for this <OBJECT> tag,

            // we actually don't need to go this far, but thats the indexObjEnd is the only index know
            for (i = indexObjStart; i < (INT)indexObjEnd; i++)
            {
                if (pTokArray[i].token.tok == TokAttrib_CLASSID && pTokArray[i].token.tokClass == tokAttr)
                {
                    indexClsId = i;
                }
                if (pwOld[pTokArray[i].token.ibTokMin] == '=' && pTokArray[i].token.tokClass == tokOp && indexTokOp == -1)
                {
                    indexTokOp = i;
                }
            }
            // following are simply error cases, we won't run into them unless we have incomplete HTML
        ...
        if ((hr = pActiveDesigner->SaveRuntimeState(IID_IPersistTextStream, IID_IStream, pStm)) == S_OK)
        {
            ...
            // before we put stuff from hg into pDTC, lets make sure that its big enough
        ...
        // we are insert/replacing (cchObjEnd-cchObjStart) wchars by cchCurDTC wchars, so realloc pwNew first
        ...
        // cchObjStart/End are actually ich's ichBeginCopy is a position in pwOld and ichNewCur is a position in pwNew

        // copy till begining of the <OBJECT>
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (cchObjStart - ichBeginCopy)*sizeof(WCHAR));
            ...
            if (S_OK == pPersistPropBag->GetClassID(&clsid))
            {
                if (IsEqualCLSID(clsid, CLSID_PageTr))
                {
                    if (ptep->m_cchPTDTC != 0)
                    {
                        ...
                        memcpy((BYTE *)(pwNew + ptep->m_ichPTDTC), (BYTE *)(pDTC), cchCurDTC*sizeof(WCHAR));
                    ...
                    else // this is the case where the PTDTC didn't exist before going to Trident
                    {
                        ...
                        if (ptep->m_ichBeginHeadTagIn > 0) // we had HEAD tag in Source view
                        {
                            ...
                            ichNewCur += cchCurDTC;
                            memcpy((BYTE *)(pwNew + ichInsertPTDTC), (BYTE *)(pDTC), cchCurDTC*sizeof(WCHAR));
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(pDTC), cchCurDTC*sizeof(WCHAR));
        ichNewCur += cchCurDTC;
    ...
    else // this object was not a DTC
    {
        ...
        for (i = indexObjStart; i < (INT)indexObjEnd; i++)
        {
            if (pTokArray[i].token.tok == TokElem_PARAM && pTokArray[i].token.tokClass == tokElem)
            {
                fFoundParam = TRUE;
                iParam = i;
                break;
        ...
        // copy till end of <OBJECT...> tag and set ichBeginCopy to be after the commented <PARAM> tags calculate ichObjStartEnd
        iObjTagEnd = indexObjStart;
        while (iObjTagEnd < indexObjEnd)
        {
            if (pTokArray[iObjTagEnd].token.tok == TokTag_CLOSE && pTokArray[iObjTagEnd].token.tokClass == tokTag)
                break;
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichObjStartEnd - ichBeginCopy)*sizeof(WCHAR));
        ichNewCur += (ichObjStartEnd - ichBeginCopy);
        ichBeginCopy = ichObjStartEnd;
        iArray = iObjTagEnd + 1;

        // generally, we don't expect Trident to move the comment from where it was put but if it does, be prepared.
        ...
        // if rgComment[0] matches and rgComment[1] does not, Trident may have mucked with the comment contents.
        // This invalidates our original assumption.
        ...
        while (i < (INT)indexObjEnd)
        {
            ...
            if ((pwOld[pTokArray[i].token.ibTokMac - 1] == '-') &&
                ...
                (0 == _wcsnicmp(rgComment[0], &pwOld[pTokArray[i].token.ibTokMac - (wcslen(rgComment[0]) + 2)], wcslen(rgComment[0]))))
            {
                iCommentEnd = i + 1;
                iComment = i;
        ...
        if (fFoundParam)
        {
            ...
            {
                cbNeed = (ichNewCur + (pTokArray[iCommentEnd].token.ibTokMac - pTokArray[iObjTagEnd].token.ibTokMin) +
                          (iCommentStart - iObjTagEnd) * 3/*for eol,tab*/ + (pTokArray[iObjTagEnd].token.ibTokMac - ichBeginCopy))
                ...
                for (i = iObjTagEnd + 1; i < iCommentStart; i++)
                {
                    ...
                    if (pTokArray[i].token.tok == TokTag_START && pTokArray[i].token.tokClass == tokTag)
                    {
                        pwNew[ichNewCur] = '\r';
                        ichNewCur++;
                        pwNew[ichNewCur] = '\n';
                ...
                // from here, look for extra spaces/tabs/eols that trident has accumulated at the end of the PARAM tags and remove them.
                for (i = iCommentEnd + 1; i <= (int)indexObjEnd; i++)
                {
                        ...
                        pTokArray[i].token.ibTokMac - pTokArray[i].token.ibTokMin == 1))
                    {
                        ...
                        {
                            if (pwOld[iChar] != ' ' && pwOld[iChar] != '\r' && pwOld[iChar] != '\n' && pwOld[iChar] != '\t')
                            {
        ...
        else
        {
            if (iCommentStart != -1 && iCommentEnd != -1 && iComment != -1)
            {
                ...
                // remove cchComment2 chars from the end of pwOld[pTokArray[i+1].token.ibTokMac and copy the rest into pwNew
                ...
                cbNeed = (ichNewCur + ichCommentStart - ichBeginCopy + pTokArray[iComment].token.ibTokMac -
                          pTokArray[iComment].token.ibTokMin)*sizeof(WCHAR)+cbBufPadding;
                if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
                    goto LRet;
                // copy till begining of the comment
                memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)(pwOld + ichBeginCopy), (ichCommentStart - ichBeginCopy)*sizeof(WCHAR));
...
void CTriEditParse::fnSaveDTC(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray,
                              UINT *piArrayStart, FilterTok ft, INT *pcDTC, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    // DTC case -
    ...
    // once we get a ENDSPAN tagID, wait for upcoming toktag_CLOSE which will end DTC remember ibTokMac at that position.
    // This is the DTC range.
    ...
    while (cDTC > 0)
    {
        ...
        if (pTokArray[iStartSpan].iNextprev != -1 /* validate */ && pTokArray[pTokArray[iStartSpan].iNextprev].token.tok == ft.tokEnd2)
        {
        ...
        {
            if (pTokArray[indexRTMac].token.tok == TokTag_BANG && pTokArray[indexRTMac].token.tokClass == tokTag)
            {
                break;
            }
            indexRTMac--;
        ...
        if (indexObjectEnd > indexObjectStart) // </OBJECT> found
        {
            ...
            i = indexObjectStart;
            // generally, the previous tag should be the one we want, but this covers the boundary cases
            ...
            i = indexObjectEnd;
            // generally, the next tag should be the one we want, but this covers the boundary cases
        ...
        if ((int)(cchDTCStart - ichBeginCopy) >= 0)
        {
            ...
            if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
                goto LSkipCopy;
            memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (cchDTCStart - ichBeginCopy)*sizeof(WCHAR));
        ...
        if (i < (int)indexObjectEnd - 1) // found TokAttrib_CLASSID
        {
            ...
            // Is there any other way to skip "clsid:" string that appears before the clsid?
            ichClsid = pTokArray[i + 2].token.ibTokMin + strlen("clsid:");
            // This is a HACK to fix DaVinci's bug, where they can't handle non-quoted classId
        ...
        if (ptep->m_fInHdrIn)
        {
            if ((S_OK == StringFromCLSID(CLSID_PageTr, &szClsid)) && (0 == _wcsnicmp(szClsid + 1/* for {*/, &pwOld[ichClsid], sizeof(CLSID))))
            {
                ...
                if (ptep->m_hgPTDTC != NULL)
                {
                    ...
                    memcpy((BYTE *)(ptep->m_pPTDTC), (BYTE *)(&pwOld[cchObjectStart]), (cchObjectEnd - cchObjectStart)*sizeof(WCHAR));
        ...
        cbNeed = (ichNewCur + (cchObjectEnd - cchObjectStart) +
                  (pTokArray[indexRTMac].token.ibTokMin - cchObjectEnd) +
                  wcslen(rgCommentRT[0]) + wcslen(rgCommentRT[1]))*sizeof(WCHAR)+cbBufPadding;
        if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
            goto LSkipCopy;
        // STEP 1 - copy till the begining of </OBJECT>
        ASSERT((int)(ichObjectEndBegin - cchObjectStart) >= 0);
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[cchObjectStart]), (ichObjectEndBegin - cchObjectStart)*sizeof(WCHAR));
        ichNewCur += ichObjectEndBegin - cchObjectStart;

        // STEP 2 - Insert the runtime text as a comment
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(rgCommentRT[0]), wcslen(rgCommentRT[0])*sizeof(WCHAR));
        ichNewCur += wcslen(rgCommentRT[0]);

        // we need to loop thr indexRTStart & indexRTMac and copy token by token and modify TokTag_BANG on the way
        fFirstDash = TRUE;
        while (indexRTStart < indexRTMac)
        {
            // (4/14/98)
            // VID-BUG 17453 Fotm Manager DTC puts in 0x0d (\r) as an end of line instead of putting 0x0d 0xa (\r\n) as an end of line.
            ...
            // 1. Handle this would be in our tokenizer that treats "0x0d" as an end of line as well.
            //    But at this time, its not a safe change to do.
            // 2. In the below if condition, add the fact that we may have "0x0d" followed
            //    by "-->" for end of metadata comment.
            if (fFirstDash && ((0 == _wcsnicmp(rgCommentRT[2], &pwOld[pTokArray[indexRTStart].token.ibTokMin], wcslen(rgCommentRT[2])))
                || ((0 == _wcsnicmp(rgCommentRT[2],
                &pwOld[pTokArray[indexRTStart].token.ibTokMin + 1],
                wcslen(rgCommentRT[2]))) && (pwOld[pTokArray[indexRTStart].token.ibTokMin] == 0x0d)))
                )
            {
                indexRTStart++;
                fFirstDash = FALSE;
            ...
            if (pTokArray[indexRTStart].token.tok == TokTag_BANG && pTokArray[indexRTStart].token.tokClass == tokTag)
            {
                pwNew[ichNewCur - 2] = '?';
            }
            ...
            if (pTokArray[indexRTStart].token.tok == TokElem_METADATA && pTokArray[indexRTStart].token.tokClass == tokElem)
            {
                pwNew[ichNewCur - 1] = '?';
            }
            indexRTStart++;
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(rgCommentRT[1]), wcslen(rgCommentRT[1])*sizeof(WCHAR));
        ichNewCur += wcslen(rgCommentRT[1]);

        // STEP 3 - copy the rest of the object, i.e. the </OBJECT> tag
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichObjectEndBegin]), (cchObjectEnd - ichObjectEndBegin)*sizeof(WCHAR));
...
void CTriEditParse::fnSaveHtmlTag(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                  TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok ft, INT* /*pcHtml*/,
                                  UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    ...
    memmove((BYTE *)(&pwNew[cchHtml]), (BYTE *)pwNew, ichNewCur*sizeof(WCHAR));
    ichNewCur += cchHtml;
...
void CTriEditParse::fnRestoreHtmlTag(CTriEditParse* /*ptep*/, LPWSTR /*pwOld*/, LPWSTR* /*ppwNew*/, UINT* /*pcchNew*/, HGLOBAL* /*phgNew*/,
                                     TOKSTRUCT* /*pTokArray*/, UINT* /*piArrayStart*/, FilterTok /*ft*/,
...
void CTriEditParse::fnSaveNBSP(CTriEditParse* /*ptep*/, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                               TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok /*ft*/, INT* /*pcHtml*/,
                               UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    UINT ichNewCur = *pichNewCur;
    ...
    // see if pwOld[pTokArray->token.ibtokMin] matches with "&nbsp", and convert it to lower case
    ...
    if (0 == _wcsnicmp(szNBSP[0], &pwOld[pTokArray[iArray].token.ibTokMin], wcslen(szNBSP[0])))
    {
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichNbspStart - ichBeginCopy)*sizeof(WCHAR));
        ichNewCur += (ichNbspStart - ichBeginCopy);
        ichBeginCopy = ichNbspEnd; // make it ready for next copy

        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(szNBSPlower[0]), (wcslen(szNBSPlower[0]))*sizeof(WCHAR));
...
void CTriEditParse::fnRestoreNBSP(CTriEditParse* /*ptep*/, LPWSTR /*pwOld*/, LPWSTR* /*ppwNew*/, UINT* /*pcchNew*/, HGLOBAL* /*phgNew*/,
                                  TOKSTRUCT* /*pTokArray*/, UINT* /*piArrayStart*/, FilterTok /*ft*/,
                                  INT* /*pcHtml*/, UINT* /*pichNewCur*/, UINT* /*pichBeginCopy*/, DWORD /*dwFlags*/)
{
...
BOOL FIsSpecialTag(TOKSTRUCT *pTokArray, int iTag, WCHAR* /*pwOld*/)
{
    BOOL fRet = FALSE;

    if ((pTokArray[iTag].token.tokClass == tokSpace || pTokArray[iTag].token.tokClass == tokComment) &&
        pTokArray[iTag].token.tok == 0 && iTag > 0 &&
        (pTokArray[iTag - 1].token.tok == TokTag_START || pTokArray[iTag - 1].token.tok == TokTag_PI || (pTokArray[iTag - 1].token.tok == TokTag_BANG &&
        ...
        && pTokArray[iTag - 1].token.tokClass == tokTag)
    {
        ...
        if (pStr != NULL)
        {
            memcpy((BYTE *)pStr, (BYTE *)&pwOld[pTokArray[iTag].token.ibTokMin], cch*sizeof(WCHAR));
            pStr[cch] = '\0';
            pFound = wcschr(pStr, ':');
            if (pFound)
                fRet = TRUE;
...
void GetTagRange(TOKSTRUCT *pTokArray, int iArrayLast, int *piTag, int *pichTokTagClose, BOOL fMatch)
{
    int index = *piTag;
    int iTokTagClose = -1;
    ...
    {
        if (pTokArray[index].token.tokClass == tokTag && pTokArray[index].token.tok == TokTag_CLOSE)
        {
            iTokTagClose = index;
            break;
        }
                              ...
                              INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    ...
    // copy from pwOld
    memcpy((BYTE *)pHdr, (BYTE *)&cchBeforeBody, sizeof(INT));
    memcpy((BYTE *)(pHdr)+sizeof(INT), (BYTE *)pwOld, cchBeforeBody*sizeof(WCHAR));
    ...
    if (cchBeforeBody > 0)
    {
        ...
        // if we have a unicode stream, we should preserve 0xff,0xfe that occurs at the beginning of the file
        ichNewCur = 0;
        ...
        {
            if (pTokArray[iTag].token.tokClass == tokAttr && pTokArray[iTag].token.tok == TokAttrib_STARTSPAN)
            {
                ...
                )
            {
                ...
                fMatch = FALSE;
                ichTokTagClose = -1;
                if (pTokArray[iTag].token.tok == TokElem_TITLE || pTokArray[iTag].token.tok == TokElem_STYLE || pTokArray[iTag].token.tok == TokElem_OBJECT)
                    fMatch = TRUE;
                ...
                if (ichTokTagClose != -1)
                {
                    ...
                    pwNew[ichNewCur++] = '<';
                    if (pTokArray[iTagSav - 1].token.tok == TokTag_END && pTokArray[iTagSav - 1].token.tokClass == tokTag)
                    {
                        pwNew[ichNewCur++] = '/';
                    }
                    else if (pTokArray[iTagSav - 1].token.tok == TokTag_PI && pTokArray[iTagSav - 1].token.tokClass == tokTag)
                    {
                        pwNew[ichNewCur++] = '?';
                    }
                    else if (pTokArray[iTagSav - 1].token.tok == TokTag_BANG && pTokArray[iTagSav - 1].token.tokClass == tokTag)
                    {
                        pwNew[ichNewCur++] = '!';
                    }
                    memcpy((BYTE *)&pwNew[ichNewCur],
        ...
        while (index >= 0)
        {
            if (pTokArray[index].token.tok == TokTag_START && pTokArray[index].token.tokClass == tokTag)
            {
                break;
            }
            index--;
        ...
        // now go forward till we get the '>' of <BODY>, we don't have to go this far, but this covers boundary cases
        index = iArray;
        while (index < (int)ptep->m_cMaxToken)
        {
            if (pTokArray[index].token.tok == TokTag_CLOSE && pTokArray[index].token.tokClass == tokTag)
            {
                break;
            }
            index++;
        ...
        if (dwFlags & dwPreserveSourceCode)
        {
            ...
            memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)(szIndex), wcslen(szIndex)*sizeof(WCHAR));
    ...
    if (ptep->m_pPTDTC != NULL) // we had saved PageTransitionDTC in a temporary
    {
        ...
        if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
            goto LRet;
        memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)ptep->m_pPTDTC, ptep->m_cchPTDTCObj*sizeof(WCHAR));
...
void CTriEditParse::fnRestoreHdr(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                 TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok ft,
                                 INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    UINT ichNewCur = *pichNewCur;
    ...
    // tag before this text comming out of Trident without looking forward and recognizing that a <BODY> tag already exists. Ideally, Trident should move teh
    ...
    while (i < ptep->m_cMaxToken)
    {
        if ((pTokArray[i].token.tok == ft.tokBegin2) /*TokElem_BODY*/ && (pTokArray[i - 1].token.tok == TokTag_START))
        {
            iFound = i;
            break;
        }
    ...
    if (cchBeforeBody > 0)
    {
        // ichBeginCopy is a position in pwOld and ichNewCur is a position in pwNew
        // copy from ichBeginCopy to begining of &nbsp
        memcpy((BYTE *)(pwNew), (BYTE *)(pHdr)+sizeof(INT), cchBeforeBody*sizeof(WCHAR));
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBodyStart]), (ichBodyEnd - ichBodyStart)*sizeof(WCHAR));
    ...
    else // if we didn't save anything, it means that we had no pre-BODY stuff in the doc (bug 15393)
    {
        ...
        else
        {
            ...
            ichNewCur += wcslen(rgPreBody[0]);
#endif //NEEDED
            // Note that we had not saved any thing before going to design view because there was no <BODY> tag.
            // we should now copy from current pwOld[ichBeginCopy] till
            ...
            {
                memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichBeginCopy], (pTokArray[iArray - 1].token.ibTokMin - ichBeginCopy)*sizeof(WCHAR));
...
LErrorRet:
    GlobalUnlock(ptep->m_hgDocRestore);// unlock
    *pcchNew = ichNewCur;
    *ppwNew = pwNew;
    *pichNewCur = ichNewCur;
    *pichBeginCopy = ichBeginCopy;
                              ...
                              INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    ...
    pFtr = (WCHAR *)GlobalLock(ptep->m_hgDocRestore); // lock
    ASSERT(pFtr != NULL);
    ...
    // copy from pwOld
    memcpy((BYTE *)pFtr, (BYTE *)&cchAfterBody, sizeof(INT));
    memcpy((BYTE *)(pFtr)+sizeof(INT), (BYTE *)(pwOld + pTokArray[iArray].token.ibTokMac), cchAfterBody*sizeof(WCHAR));
    pFtr += cchAfterBody + sizeof(INT) / sizeof(WCHAR);

    memcpy((BYTE *)pFtr, (BYTE *)&cchPreEndBody, sizeof(INT));
    memcpy((BYTE *)(pFtr)+sizeof(INT), (BYTE *)&(pwOld[ichStart]), cchPreEndBody*sizeof(WCHAR));
    ...
    memcpy((BYTE *)&(pwNew[ichNewCur]), (BYTE *)&(pwOld[ichBeginCopy]), (pTokArray[iArray].token.ibTokMac - ichBeginCopy)*sizeof(WCHAR));
                                 ...
                                 INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    ...
    // HACK to fix a TRIDENT misbehaviour
    // If we had any text before <BODY> tag going into Trident, it will add 2nd <BODY>
    // tag before this text comming out of Trident without looking forward and recognizing that a <BODY> tag already exists.
    // Ideally, Trident should move teh
    // <BODY> tag at appropriate place rather than inserting a 2nd one.
    // Lets assume that Trident will insert only one extra <\BODY> tag.
    i = iArray + 1; // we know iArray is the 1st <\BODY> tag
    iFound = iArray;
    while (i < ptep->m_cMaxToken)
    {
        if ((pTokArray[i].token.tok == ft.tokBegin2) /*TokElem_BODY*/ && (pTokArray[i - 1].token.tok == TokTag_END))
        {
            iFound = i;
            break;
        }
    ...
    if (cchAfterBody > 0)
    {
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichBodyEnd - ichBeginCopy)*sizeof(WCHAR));
        ...
        // we know that the following condition will be met most of the times, but just to cover incomplete HTML cases...
        if ((pTokArray[iArray].token.tok == ft.tokBegin2) /*TokElem_BODY*/ && (pTokArray[iArray - 1].token.tok == TokTag_END))
        {
            ichInsEOL = ichNewCur - (pTokArray[iArray].token.ibTokMac - pTokArray[iArray - 1].token.ibTokMin);
        }

        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)pFtr, (cchAfterBody)*sizeof(WCHAR));
        ...
        if (ichInsEOL != -1)
        {
            ...
            if (cchPreEndBody > 0)
            {
                ...
                // contents before </BODY> and after any previous text/tokens, then we shouldn't do the following memcpy()
                while (ichT >= 0 /* validation */
                       && (pwNew[ichT] == ' ' || pwNew[ichT] == '\n' || pwNew[ichT] == '\r' || pwNew[ichT] == '\t'))
                {
                    ichT--;
                    cchSubStr++;
                }
                ...
                if (pw == NULL) // means that the substring wasn't found
                {
                    ...
                    memmove((BYTE *)(&pwNew[ichInsEOL + cchPreEndBody]), (BYTE *)(&pwNew[ichInsEOL]), (ichNewCur - ichInsEOL)*sizeof(WCHAR));
                    memcpy((BYTE *)(&pwNew[ichInsEOL]), (BYTE *)(pFtr), (cchPreEndBody)*sizeof(WCHAR));
    ...
    else
    {
        ...
        if (dwFlags & dwFilterSourceCode)
        {
            ...
            {
                if (pTokArray[index].token.tok == TokTag_END && pTokArray[index].token.tokClass == tokTag)
                {
                    break;
                }
                index--;
            ...
            if (index >= 0)
            {
                ...
                if (ichBodyStart > (int)ichBeginCopy)
                {
                    ...
                    if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
                        goto LErrorRet;
                    memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichBodyStart - ichBeginCopy)*sizeof(WCHAR));
                ...
                else if (ichBodyEnd > (int)ichBeginCopy)
                {
                    ...
                    if (index < (int)ptep->m_cMaxToken)
                    {
                        ...
                        if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
                            goto LErrorRet;
                        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichBodyTagEnd - ichBeginCopy)*sizeof(WCHAR));
                        ichNewCur += (ichBodyTagEnd - ichBeginCopy);
                        ichBeginCopy = ichBodyTagEnd; // setting this is redundant, but it makes the code readable.
                    }
                }

                // add a null at the end to keep the code in ssync with the if (cchAfterBody > 0) case
                pwNew[ichNewCur++] = '\0';
    ...
    if (ptep->m_cchPTDTC != 0)
    {
        ...
        memset((BYTE *)&pwNew[ptep->m_ichPTDTC], 0, ptep->m_cchPTDTC*sizeof(WCHAR));
                                 ...
                                 INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    ...
    // If we find '<% %>' blocks inside, put a comment with a special tag around it, else simply copy that object as is and exit
    UINT ichNewCur = *pichNewCur;
    UINT ichBeginCopy = *pichBeginCopy;
    UINT iArray = *piArrayStart;
    LPWSTR pwNew = *ppwNew;
    ...
    if ((iArray - 1 >= 0) && pTokArray[iArray - 1].token.tok == TokTag_END && pTokArray[iArray - 1].token.tokClass == tokTag)
    {
        iArray++;
        goto LRet;
    }
    ...
    {
        if (pTokArray[i].token.tok == TokTag_SSSOPEN && pTokArray[i].token.tokClass == tokSSS)
        {
            fSSSFound = TRUE;
            break;
        }
    ...
    if (fSSSFound) // this object can't be displayed in Trident, so convert it
    {
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichObjectStart - ichBeginCopy)*sizeof(WCHAR));
        ...
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichObjectStart]), (ichObjectEnd - ichObjectStart)*sizeof(WCHAR));
    ...
    else
    {
        ...
        while (iParamStart < iObjectEnd)
        {
            ...
            //  iClsId = iParamStart;
            if (pTokArray[iParamStart].token.tok == TokElem_PARAM && pTokArray[iParamStart].token.tokClass == tokElem)
                break;
            iParamStart++;
        }
        ...
        {
            if (pTokArray[iParamStart].token.tok == TokTag_START && pTokArray[iParamStart].token.tokClass == tokTag)
                break;
            iParamStart--;
        }
        ...
        {
            if (pTokArray[iParamEnd].token.tok == TokElem_PARAM && pTokArray[iParamEnd].token.tokClass == tokElem)
                break;
            iParamEnd--;
        }
        while (iParamEnd < iObjectEnd) // generally this will the previous token, but cover all cases
        {
            if (pTokArray[iParamEnd].token.tok == TokTag_CLOSE && pTokArray[iParamEnd].token.tokClass == tokTag)
                break;
            iParamEnd++;
        }
        ...
        while (index <= iParamEnd)
        {
            ...
            // get PARAM
            while ((pTokArray[index].token.tok != TokElem_PARAM || pTokArray[index].token.tokClass != tokElem) && (index <= iParamEnd))
                index++;
            if (index > iParamEnd)
                goto LDoneCopy;
            // get '<' before the PARAM
            while ((pTokArray[index].token.tok != TokTag_START || pTokArray[index].token.tokClass != tokTag) && (index >= iPrev))
                index--;
            if (index < iPrev)
                goto LDoneCopy;
            iStart = index;
            ...
            // get matching '>'
            while ((pTokArray[index].token.tok != TokTag_CLOSE || pTokArray[index].token.tokClass != tokTag) && (index <= iParamEnd))
                index++;
...
void CTriEditParse::fnRestoreObject(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                    TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok /*ft*/,
                                    INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    ...
    if (!fSimpleComment)
    {
        ...
        // remove cchComment2 chars from the end of pwOld[pTokArray[iArray+1].token.ibTokMac and copy the rest into pwNew
        ...
        cbNeed = (ichNewCur + (ichCommentStart - ichBeginCopy) +
                  (pTokArray[iArray + 1].token.ibTokMac - pTokArray[iArray + 1].token.ibTokMin))*sizeof(WCHAR)+cbBufPadding;
        if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
            goto LRet;
        ...
        memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)&(pwOld[ichObjectStart]),
    ...
    else
    {
        ...
        ichNewCur--;
        while ((ichNewCur >= 0) && (pwNew[ichNewCur] == ' ' || pwNew[ichNewCur] == '\t' || pwNew[ichNewCur] == '\r' || pwNew[ichNewCur] == '\n'))
        {
            ichNewCur--;
        }
        ...
        while (ichspBegin < ichspEnd)
        {
            switch (pwOld[ichspBegin])
            {
                ...
                case chCommentEOL:
                    ...
                    break;
                case ',':
                    while (pwOld[ichCopy] == ' ' || pwOld[ichCopy] == '\t' || pwOld[ichCopy] == '\r' || pwOld[ichCopy] == '\n')
                    {
...
void CTriEditParse::fnSaveSpace(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok ft,
                                INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    UINT ichNewCur = *pichNewCur;
    ...
    if ((iArray + 1 < (INT)ptep->m_cMaxToken) /* validation */
        && (pTokArray[iArray + 1].token.tok == TokElem_A || pTokArray[iArray + 1].token.tok == TokElem_IMG || pTokArray[iArray + 1].token.tok == TokElem_LINK)
        && (pTokArray[iArray + 1].token.tokClass == tokElem)
    ...
    {
        if ((pTokArray[iArrayPrevTag].token.tokClass == tokTag && pTokArray[iArrayPrevTag].token.tok == TokTag_CLOSE)
            || (pTokArray[iArrayPrevTag].token.tokClass == tokSSS && pTokArray[iArrayPrevTag].token.tok == TokTag_SSSCLOSE
            )/* VID6 - bug 22787 */
            )
        {
            break;
    ...
    if (iArrayPrevTag < 0) // handle error case
    {
        ...
        {
            if (pwOld[ichBeginPrev] != ' ' && pwOld[ichBeginPrev] != '\r' && pwOld[ichBeginPrev] != '\n' && pwOld[ichBeginPrev] != '\t')
                break;
            ichBeginPrev--;
        }
        goto LGotEndNext;
...
LGotEndNext:
    ...
    {
        if (pwOld[ichEndNext] != ' ' && pwOld[ichEndNext] != '\r' && pwOld[ichEndNext] != '\n' && pwOld[ichEndNext] != '\t')
            break;
        ichEndNext++;
    }
    ...
    if (iArrayMatch != -1) // match was set while tokenizing
    {
        ...
        {
            if (pTokArray[iArrayMatch].token.tokClass == tokTag && (pTokArray[iArrayMatch].token.tok == TokTag_START
        ...
        if (iArrayMatch > iArray) // did find '</' or '<' after the current tag
        {
            ...
            while (iArrayMatch >= iArray) // iArray is TokTag_CLOSE of the current tag (i.e. '>')
            {
                if ((pTokArray[iArrayMatch].token.tokClass == tokTag && pTokArray[iArrayMatch].token.tok == TokTag_CLOSE)
                    || (pTokArray[iArrayMatch].token.tokClass == tokSSS && pTokArray[iArrayMatch].token.tok == TokTag_SSSCLOSE
                    )/* VID6 - bug 22787 */
                    )
                    break;
                iArrayMatch--;
...
LSkipMatchCalc:
    ...
    {
        memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)(pwOld + ichBeginCopy), (ichBegin - ichBeginCopy)*sizeof(WCHAR));
    ...
    {
        memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)(rgSpaceTags[2]), (wcslen(rgSpaceTags[2]))*sizeof(WCHAR));
    ...
    memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)(pwOld + ichBegin), (ichEnd - ichBegin)*sizeof(WCHAR));
    ichNewCur += (ichEnd - ichBegin);

    iArray = iArraySav + 1;// restore iArray
...
void CTriEditParse::fnRestoreSpace(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray,
                                   UINT *piArrayStart, FilterTok ft, INT *pcHtml, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    UINT ichNewCur = *pichNewCur;
    UINT ichBeginCopy = *pichBeginCopy;
    INT iArray = (INT)*piArrayStart;
    ...
    if (pTokArray[iArray].token.tok == ft.tokBegin2 && pTokArray[iArray].token.tokClass == tokTag)
    {
        ASSERT(ft.tokBegin2 == TokTag_END);
        fnRestoreSpaceEnd(ptep, pwOld, ppwNew, pcchNew, phgNew, pTokArray, piArrayStart, ft, pcHtml, pichNewCur, pichBeginCopy, dwFlags);
        goto LRetOnly;
    }
    ...
    {
        if (pTokArray[itoktagStart].token.tok == ft.tokBegin && pTokArray[itoktagStart].token.tokClass == tokTag)
        {
            break;
        }
        itoktagStart--;
    ...
    while (iArray < (int)ptep->m_cMaxToken)
    {
        if ((iArrayValue == -1) && ((pTokArray[iArray].token.tokClass == tokValue) || (pTokArray[iArray].token.tokClass == tokString)))
            iArrayValue = iArray;
        else if (pTokArray[iArray].token.tok == TokTag_CLOSE && pTokArray[iArray].token.tokClass == tokTag)
        {
    ...
    // NOTE - token before iArraySav'th one should be tokSpace with lenght 1 and with a value of chSpace (unless Trident has modified it).
    // If thats TRUE, we should skip that too, because we added it when we put in DESIGNTIMESPx.
    ...
    if ((iArraySav - 1 > 0) /* validation */
        && (((pTokArray[iArraySav - 1].token.ibTokMac - pTokArray[iArraySav - 1].token.ibTokMin == 1) && (pwOld[pTokArray[iArraySav - 1].token.ibTokMin] == ' '))
    ...
    if (ichBegin - ichBeginCopy >= 0)
    {
        ...
        {
            memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)(pwOld + ichBeginCopy), (pTokArray[itoktagStart].token.ibTokMac - ichBeginCopy)*sizeof(WCHAR));
    ...
    if (pwOld[pTokArray[iArrayValue].token.ibTokMin] == '"' && pwOld[pTokArray[iArrayValue].token.ibTokMac - 1] == '"')
    {
        memcpy((BYTE *)szIndex,
            ...
            if (index == 0) // special case BUG 8741
            {
                ...
                if (ichtoktagStart >= 0)
                {
                    ...
                    if (cws > 0)
                    {
                        ...
                        memmove((BYTE*)&pwNew[ichtoktagStart + 1], (BYTE*)&pwNew[ichtoktagStart + 1 + cws], (ichNewCur - ichtagStart - 1)*sizeof(WCHAR));
                        ichNewCur -= cws;
                    }
                }
            }
...
void CTriEditParse::fnRestoreSpaceEnd(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                      TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok ft,
                                      INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    UINT ichNewCur = *pichNewCur;
    ...
    i = iArrayMatch;
    cchtag = wcslen(rgSpaceTags[0]);
    while (i < iArraySav /* boundary case */ && (pTokArray[i].token.tokClass != tokTag || pTokArray[i].token.tok != TokTag_CLOSE))
    {
    ...
    while (i < iArraySav)
    {
        if ((iArrayValue == -1) && (pTokArray[i].token.tokClass == tokValue || pTokArray[i].token.tokClass == tokString))
            iArrayValue = i;
        else if (pTokArray[i].token.tok == TokTag_CLOSE && pTokArray[i].token.tokClass == tokTag)
        {
    ...
    memcpy((BYTE *)(pwNew + ichNewCur), (BYTE *)(pwOld + ichBeginCopy), (pTokArray[iArraySav].token.ibTokMin - ichBeginCopy)*sizeof(WCHAR));
    ...
    if (cchwspInfo == 2) // we didn't save any spacing info
    {
        ...
        // FRestorespacing gets called from other places too so this is late in the game to that kind of change.
...
LRestoreCaseAndRet:
    ...
    // we should combine this memcpy with the above ones, but I want to keep the code separate
...
void CTriEditParse::fnSaveTbody(CTriEditParse* /*ptep*/, LPWSTR /*pwOld*/, LPWSTR* /*ppwNew*/, UINT* /*pcchNew*/, HGLOBAL* /*phgNew*/,
                                TOKSTRUCT* /*pTokArray*/, UINT *piArrayStart, FilterTok /*ft*/,
                                INT* /*pcHtml*/, UINT* /*pichNewCur*/, UINT* /*hBeginCopy*/, DWORD /*dwFlags*/)
{
    UINT iArray = *piArrayStart;
...
void CTriEditParse::fnRestoreTbody(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                   TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok /*ft*/,
                                   INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    ...
    {
        if ((pTokArray[iArray].token.tok == TokTag_START) && (pTokArray[iArray].token.tokClass == tokTag))
        {
            fBeginTBody = TRUE;
            ichTBodyStart = pTokArray[iArray].token.ibTokMin;
            break;
        }
        else if ((pTokArray[iArray].token.tok == TokTag_END) && (pTokArray[iArray].token.tokClass == tokTag))
        {
    ...
    if (fBeginTBody)
    {
        ...
        else
        {
            ...
            if (ptep->m_iTBodyMax + 1 >= ptep->m_iMaxTBody)
            {
                ...
                hrRet = ReallocBuffer(&ptep->m_hgTBodyStack, (ptep->m_iMaxTBody + cTBodyInit)*sizeof(UINT), GMEM_MOVEABLE | GMEM_ZEROINIT);
    ...
    {
        memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichTBodyStart - ichBeginCopy)*sizeof(WCHAR));
...
void CTriEditParse::fnSaveApplet(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray,
                                 UINT *piArrayStart, FilterTok /*ft*/, INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    UINT ichNewCur = *pichNewCur;
    UINT ichBeginCopy = *pichBeginCopy;
    ...
    else // save the URL as an attribute value of DESIGNTIMEURL
    {
        ...
        cbNeed = (ichNewCur + pTokArray[index].token.ibTokMin - ichBeginCopy + wcslen(rgDspURL[0]) + cchURL +
                  3/*eq,quotes*/)*sizeof(WCHAR)+cbBufPadding;
        if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
        {
            iArray = iArraySav + 1;
            goto LRet;
        ...
        if (cchURL != 0)
        {
            ...
            memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)rgDspURL[0], wcslen(rgDspURL[0])*sizeof(WCHAR));
            ichNewCur += wcslen(rgDspURL[0]);

            pwNew[ichNewCur++] = '"';
            memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichURL], cchURL*sizeof(WCHAR));
            ichNewCur += cchURL;
            pwNew[ichNewCur++] = '"';
        }
...
LStep2:
    ...
    memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichAppletEnd - ichBeginCopy)*sizeof(WCHAR));
...
void CTriEditParse::fnRestoreApplet(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                    TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok /*ft*/,
                                    INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    UINT ichNewCur = *pichNewCur;
    ...
    for (i = iArraySav; i < indexAppletTagClose; i++)
    {
        ...
        else if (pTokArray[i].token.tok == TokAttrib_CODEBASE && pTokArray[i].token.tokClass == tokAttr)
        {
            indexCB = i;
        }
    }
    ...
    memcpy((BYTE *)(&pwNew[ichNewCur]), (BYTE *)(&pwOld[ichBeginCopy]), (ichAppletStart - ichBeginCopy)*sizeof(WCHAR));
    ...
    if (indexDSU != -1)
    {
        ...
        {
            if (pTokArray[i].token.tok == 0 && (pTokArray[i].token.tokClass == tokValue || pTokArray[i].token.tokClass == tokString))
            {
                indexDSUEnd = i;
                break;
            }
    ...
    while (i <= indexAppletTagClose)
    {
        if ((indexDSU != -1) && (i >= indexDSU && i <= indexDSUEnd))
        {
            i++; // don't copy this token
        }
        else if ((indexDSP != -1) && (i >= indexDSP && i <= indexDSPEnd))
        {
            i++; // don't copy this token
        }
        else if (pTokArray[i].token.tok == TokAttrib_CODEBASE && pTokArray[i].token.tokClass == tokAttr && !fCodeBaseFound)
        {
                 ...
                 )
        {
            ...
            ichURL = (pwOld[pTokArray[i].token.ibTokMin] == '"') ? pTokArray[i].token.ibTokMin + 1 : pTokArray[i].token.ibTokMin;
            ichURLEnd = (pwOld[pTokArray[i].token.ibTokMac - 1] == '"') ? pTokArray[i].token.ibTokMac - 1 : pTokArray[i].token.ibTokMac;
            if (FIsAbsURL((LPOLESTR)&pwOld[ichURL]))
            {
                ...
                ichDSURL = (pwOld[pTokArray[indexDSUEnd].token.ibTokMin] == '"') ? pTokArray[indexDSUEnd].token.ibTokMin + 1 : pTokArray[indexDSUEnd].token.ibTokMin;
                ...
                {
                    pwNew[ichNewCur++] = '"';
                    memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichDSURL], (ichDSURLEnd - ichDSURL)*sizeof(WCHAR));
    ...
    if ((indexDSP != -1) && (dwFlags & dwPreserveSourceCode))
...
void CTriEditParse::RestoreSpacingSpecial(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR *ppwNew, HGLOBAL* /*phgNew*/,
                                          TOKSTRUCT *pTokArray, UINT iArray, UINT *pichNewCur)
{
    ...
    while (itoktagStart >= 0)
    {
        if (pTokArray[itoktagStart].token.tok == TokTag_START && pTokArray[itoktagStart].token.tokClass == tokTag)
        {
            break;
        }
        itoktagStart--;
    ...
    if (iArrayValue == -1 || iArray >= (int)ptep->m_cMaxToken) // didn't find tokValue after DESIGNTIMESPx
    {
        ...
        // SOLUTION 2
        // look for DESIGNTIMESP from pwOld[pTokArray[itokTagStart].token.ibTokMac - 1]
        // to pwOld[pTokArray[iArray].token.ibTokMac - 1] and overwrite all of those strings with spaces.
        // We could NULL those and do the blts, but why bother when the html isn't valid!
...
void CTriEditParse::SaveSpacingSpecial(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR *ppwNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray, INT iArray, UINT *pichNewCur)
{
    UINT ichNewCur = *pichNewCur;
    LPWSTR pwNew = *ppwNew;
    ...
    while (iArray >= 0)
    {
        if (pTokArray[iArray].token.tokClass == tokTag && pTokArray[iArray].token.tok == TokTag_START)
        {
            break;
        }
        iArray--;
    ...
    if (iArrayPrevTag < 0) // handle error case
    {
        ...
        while (ichBeginPrev >= 0)
        {
            if (pwOld[ichBeginPrev] != ' '
                && pwOld[ichBeginPrev] != '\r' && pwOld[ichBeginPrev] != '\n' && pwOld[ichBeginPrev] != '\t')
                break;
            ichBeginPrev--;
        }
        goto LGotEndNext;
...
void CTriEditParse::fnSaveAImgLink(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                   TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok /*ft*/,
                                   INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    UINT ichNewCur = *pichNewCur;
    ...
    ASSERT(pTokArray[iArray].token.tok == TokElem_A || pTokArray[iArray].token.tok == TokElem_IMG || pTokArray[iArray].token.tok == TokElem_LINK);
    ASSERT(pTokArray[iArray].token.tokClass == tokElem);
    ...
    else // save the URL as an attribute value of DESIGNTIMEURL
    {
        ...
        if ((int)(pTokArray[index].token.ibTokMin - ichBeginCopy) > 0)
        {
            memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichBeginCopy], (pTokArray[index].token.ibTokMin - ichBeginCopy)*sizeof(WCHAR));
            ichNewCur += (pTokArray[index].token.ibTokMin - ichBeginCopy);
        }
...
void CTriEditParse::fnRestoreAImgLink(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                      TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok /*ft*/,
                                      INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy, DWORD dwFlags)
{
    UINT ichNewCur = *pichNewCur;
    ...
    {
        if ((pTokArray[indexStart].token.tok == TokTag_START) && (pTokArray[indexStart].token.tokClass == tokTag))
        {
            break;
        }
        indexStart--;
    ...
    {
        if ((pTokArray[indexEnd].token.tok == TokTag_CLOSE) /* > */ && (pTokArray[indexEnd].token.tokClass == tokTag))
        {
            break;
        }
        indexEnd++;
    ...
    // Here is the deal - If we found DESIGNTIMESP, it means that this A/Img/Link existed while in source view.
    // And in that case, we shouldn't find DESIGNTIMEREF. With the
    ...
    // and its value was relative. This can coexist with DESIGNTIMESP, but not with DESIGNTIMEREF.
    ...
    if (indexDSR != -1)
    {
        ...
        if ((int)(pTokArray[indexStart].token.ibTokMin - ichBeginCopy) > 0)
        {
            ...
            memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichBeginCopy], (pTokArray[indexStart].token.ibTokMin - ichBeginCopy)*sizeof(WCHAR));
        ...
        while (i <= indexEnd)
        {
            if (i == indexDSR)
                i++; // don't copy this token
            else if ((pTokArray[i].token.tok == TokAttrib_HREF || pTokArray[i].token.tok == TokAttrib_SRC)
                     && pTokArray[i].token.tokClass == tokAttr && !fHrefSrcFound)
            {
                fHrefSrcFound = TRUE;
                memcpy((BYTE *)&pwNew[ichNewCur],
                       (BYTE *)&pwOld[pTokArray[i].token.ibTokMin],
            ...
            else if (pTokArray[i].token.tok == 0 && pTokArray[i].token.tokClass == tokString && fHrefSrcFound)
            {
                HRESULT hr;
                int cchURL;
                WCHAR *pszURL;
                ...
                cchURL = (fQuote) ? pTokArray[i].token.ibTokMac - pTokArray[i].token.ibTokMin - 2 : pTokArray[i].token.ibTokMac - pTokArray[i].token.ibTokMin;
                pszURL = new WCHAR[cchURL + 1];
                ...
                if (ptep->m_bstrBaseURL != NULL) // get the relative URL
                {
                    ...
                    memcpy((BYTE *)pszURL, (BYTE *)&pwOld[pTokArray[i].token.ibTokMin + ((fQuote) ? 1 : 0)], (cchURL)*sizeof(WCHAR));
                    pszURL[cchURL] = '\0';
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        {
                            pwNew[ichNewCur++] = '"';
                            memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)pszRelativeURL, wcslen(pszRelativeURL)*sizeof(WCHAR));
    ...
    else // DESIGNTIMEURL case
    {
        ...
        while (i <= indexEnd)
        {
            if ((i >= indexDSU && i <= indexDSUEnd) || (i >= indexDSP && i <= indexDSPEnd))
                i++; // don't copy this token
            else if ((pTokArray[i].token.tok == TokAttrib_HREF || pTokArray[i].token.tok == TokAttrib_SRC)
                     && pTokArray[i].token.tokClass == tokAttr && !fHrefSrcFound)
            {
...
void CTriEditParse::fnSaveComment(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew,
                                  TOKSTRUCT *pTokArray, UINT *piArrayStart, FilterTok /*ft*/, INT* /*pcHtml*/,
                                  UINT *pichNewCur, UINT *pichBeginCopy, DWORD /*dwFlags*/)
{
    UINT ichNewCur = *pichNewCur;
    ...
    // REMOVE METADATA from here, we don't need it because we are checking for end of comment too.
    ...
    iCommentEnd = iArray + 2;
    ASSERT(iCommentEnd < (INT)ptep->m_cMaxToken);
    if (pTokArray[iCommentEnd].token.tok != TokTag_CLOSE && pTokArray[iCommentEnd].token.tokClass != tokTag)
    {
    ...
    cbNeed = (ichNewCur + 2 * cchComment + wcslen(rgComment[0]) + wcslen(rgComment[1]) +
              (pTokArray[iCommentStart].token.ibTokMac - ichBeginCopy + 2))*sizeof(WCHAR)+cbBufPadding;
    if (S_OK != ReallocIfNeeded(phgNew, &pwNew, cbNeed, GMEM_MOVEABLE | GMEM_ZEROINIT))
        goto LRet;

    // write till '<!--' part of the comment
    memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichBeginCopy], (pTokArray[iCommentStart].token.ibTokMac - ichBeginCopy)*sizeof(WCHAR));
    ...
    while ((ichSp >= 0) && (pwOld[ichSp] == ' ' || pwOld[ichSp] == '\t' || pwOld[ichSp] == '\r' || pwOld[ichSp] == '\n'))
    {
        ichSp--;
    }
...
void CTriEditParse::fnRestoreComment(CTriEditParse* /*ptep*/, LPWSTR /*pwOld*/, LPWSTR* /*ppwNew*/, UINT* /*pcchNew*/, HGLOBAL* /*phgNew*/,
                                     TOKSTRUCT* /*pTokArray*/, UINT* /*piArrayStart*/, FilterTok /*ft*/,
                                     INT* /*pcHtml*/, UINT* /*pichNewCur*/, UINT* /*pichBeginCopy*/, DWORD /*dwFlags*/)
{
...
void CTriEditParse::fnSaveTextArea(CTriEditParse *ptep, LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray,
                                   UINT *piArrayStart, FilterTok /*ft*/, INT* /*pcHtml*/, UINT *pichNewCur, UINT *pichBeginCopy,
                                   DWORD /*dwFlags*/)
{
    UINT ichNewCur = *pichNewCur;
    UINT ichBeginCopy = *pichBeginCopy;
    ...
    // look for TEXTAREA block and simply copy it into pwNew.
    // Thereby avoiding the space preservation & stuff.
    ...
    // NOTE that we don't even need to get get the '<' before the textarea here because we are not doing anything special with them.
    // We simply are going to copy everything inside the textarea to pwNew.
    // So, we start copying from ichBeginCopy and copy till end of the textarea block.
    ...
    {
        if ((pTokArray[iTextAreaEnd].token.tok == TokTag_CLOSE) /* > */ && (pTokArray[iTextAreaEnd].token.tokClass == tokTag))
        {
            break;
        }
        iTextAreaEnd++;
    ...
    // but at this point in the game the assert is of no use, because no one is using debug builds (6/10/98)
    if ((int)(pTokArray[iTextAreaEnd].token.ibTokMac - ichBeginCopy) > 0)
    {
        ...
        memcpy((BYTE *)&pwNew[ichNewCur], (BYTE *)&pwOld[ichBeginCopy], (pTokArray[iTextAreaEnd].token.ibTokMac - ichBeginCopy)*sizeof(WCHAR));
...
void CTriEditParse::fnRestoreTextArea(CTriEditParse* /*ptep*/, LPWSTR /*pwOld*/, LPWSTR* /*ppwNew*/, UINT* /*pcchNew*/,
                                      HGLOBAL* /*phgNew*/, TOKSTRUCT* /*pTokArray*/, UINT *piArrayStart, FilterTok /*ft*/,
                                      INT* /*pcHtml*/, UINT* /*pichNewCur*/, UINT* /*pichBeginCopy*/, DWORD /*dwFlags*/)
{
    UINT iArray = *piArrayStart;
    ...
    // NOTE that we never put in designtimesp's in this block, so we souldn't have to look for them here.
    ASSERT(pTokArray[iArray].token.tok == TokElem_TEXTAREA);
    ASSERT(pTokArray[iArray].token.tokClass == tokElem);
...
void CTriEditParse::FilterHtml(LPWSTR pwOld, LPWSTR* ppwNew, UINT *pcchNew, HGLOBAL *phgNew, TOKSTRUCT *pTokArray,
                               FilterMode mode, DWORD dwFlags)
{
        ...
        while (cItems > 0)
        {
            ...
            while (iArray < m_cMaxToken)
            {
                         ...
                         )
                {
                    ...
                    // If this view changes, we need to add some other designtime attribute along with spacing attributes.
                    // This will be somewhat big change than
                    // simply adding an attribute because then we need to change the code to start going backwards in the token array in the main loop.
                    iItem = 1;
                    index = cRuleMid + 8;
                    cItems++; //  to compensate for cItems-- after the pfn() call
                    break;
    ...
    LIncorrectcItems:
        ...
        {
            hr = ReallocBuffer(phgNew, (ichNewCur + pTokArray[m_cMaxToken - 1].token.ibTokMac - ichBeginCopy)*sizeof(WCHAR), GMEM_MOVEABLE | GMEM_ZEROINIT);
            if (hr == E_OUTOFMEMORY)
                goto LCopyAndRet;
            ASSERT(*phgNew != NULL);
            *ppwNew = (WCHAR *)GlobalLock(*phgNew);
        }
        memcpy((BYTE *)(*ppwNew + ichNewCur), (BYTE *)(pwOld + ichBeginCopy), (pTokArray[m_cMaxToken - 1].token.ibTokMac - ichBeginCopy)*sizeof(WCHAR));
    ...
    LCopyAndRet:
        memcpy((BYTE *)*ppwNew, (BYTE *)pwOld, (pTokArray[m_cMaxToken - 1].token.ibTokMac)*sizeof(WCHAR));
        *pcchNew = pTokArray[m_cMaxToken - 1].token.ibTokMac;
    }
}


int CTriEditParse::ValidateTag(LPWSTR pszText)
{
    int len = 0;
...
INT CTriEditParse::GetTagID(LPWSTR pszText, TXTB token)
{
...
void CTriEditParse::PreProcessToken(TOKSTRUCT *pTokArray, INT *pitokCur, LPWSTR /*pszText*/,
                                    UINT /*cbCur*/, TXTB token, DWORD lxs, INT tagID, FilterMode mode)
{
    ...
    if (mode == modeInput)
    {
        if (pTokT->token.tok == TokTag_SSSOPEN && pTokT->token.tokClass == tokSSS && ((lxs & inSCRIPT) || (lxs & inAttribute)))
        {
            pTokT->token.tok = TokTag_SSSOPEN_TRIEDIT;
        }
        if (pTokT->token.tok == TokTag_SSSCLOSE && pTokT->token.tokClass == tokSSS && ((lxs & inSCRIPT) || (lxs & inAttribute)))
        {
...
void CTriEditParse::PostProcessToken(OLECHAR* /*pwOld*/, OLECHAR* /*pwNew*/, UINT* /*pcbNew*/,
                                     UINT /*cbCur*/, UINT /*cbCurSav*/, TXTB token, FilterMode mode, DWORD lxs, DWORD dwFlags)
{
    ...
    if (mode == modeInput)
    {
        ...
        if (token.tokClass == tokEntity && dwFlags != dwFilterNone)
        {
            m_cNbsp++;
        }
        if ((token.tok == TokElem_OBJECT) && (token.tokClass == tokElem) && (lxs & inEndTag) && (dwFlags != dwFilterNone))
        {
            m_cObjIn++;
        }
        if (token.tok == TokElem_APPLET && token.tokClass == tokElem && (lxs & inEndTag) && (dwFlags != dwFilterNone))
        {
            m_cAppletIn++;
        }
    }
    else if (mode == modeOutput)
    {
        ...
        if (token.tok == TokElem_SCRIPT && token.tokClass == tokElem && (lxs & inEndTag) && (dwFlags & dwFilterServerSideScripts))
        {
            m_cSSSOut++;
        }
        if (token.tok == TokTag_BANG && token.tokClass == tokTag)
        {
            m_cComment++;
        }
        if (token.tok == TokElem_APPLET && token.tokClass == tokElem && (lxs & inEndTag) && (dwFlags != dwFilterNone))
...
HRESULT CTriEditParse::ProcessToken(DWORD &lxs, TXTB &tok, LPWSTR pszText, UINT /*cbCur*/, TOKSTACK *pTokStack,
                                    INT *pitokTop, TOKSTRUCT *pTokArray, INT iArrayPos, INT tagID)
{
    TXTB token = tok;
...
LSpecialCase:
    if (m_fEndTagFound) // end tag was found previously, means pop from the stack
    {
        ...
        // if we get an end tag, in ideal case, the top of the stack should match with what we got
        if (tagID == pTokT->tagID)
        {
            if (tagID == -1) // special case, match token.tok & token.tokClass
            {
                if ((pTokT->token.tok == TokTag_SSSCLOSE) /* faked token for <% */ && (pTokT->token.tokClass == tokSSS))
                {
...
HRESULT CTriEditParse::hrTokenizeAndParse(HGLOBAL hOld, HGLOBAL *phNew, IStream *pStmNew, DWORD dwFlags, FilterMode mode, int cbSizeIn, UINT *pcbSizeOut,
                                          IUnknown *pUnkTrident, HGLOBAL *phgTokArray, UINT *pcMaxToken, HGLOBAL *phgDocRestore, BSTR bstrBaseURL, DWORD dwSpecialize)
{
    ...
    //initilize fBeginTokSelect (used by special case code that detects server side scripts inside a SELECT block)
    ...
    *phgTokArray = GlobalAlloc(GMEM_MOVEABLE | GMEM_ZEROINIT, MIN_TOK*sizeof(TOKSTRUCT)); // stack
    ...
    hgOld = GlobalAlloc(GMEM_ZEROINIT, (dwFlags & dwFilterMultiByteStream) ? (cbOld + 1/*for NULL*/)*sizeof(WCHAR) : (cbOld + 2/*for NULL*/));
    if (hgOld == NULL)
    {
        hrRet = E_OUTOFMEMORY;
        goto LOOM;
    ...
    *(pwOld + cchwOld) = '\0';
    ...
    while (cbCur < cchwOld)
    {
        ...
        if ((mode == modeInput) && (token.tokClass == tokElem) && (lxs & inTag)
        ...
        if ((token.tokClass == tokElem) && ((token.tok == TokElem_FRAME) || (token.tok == TokElem_FRAMESET)))
        {
            // skip tokenizing. we can't let this go to Trident
            memcpy((BYTE *)pwNew, (BYTE *)pwOld, cchwOld*sizeof(WCHAR));
            cchNew = cchwOld;
        ...
        if ((token.tok == TokTag_SSSOPEN) && (token.tokClass == tokSSS) && (lxs & inAttribute) && !(lxs & inString)
        ...
        if ((fBeginTokSelect) && (token.tok == TokTag_SSSOPEN || token.tok == TokElem_SCRIPT))
        {
            // skip tokenizing. we can't let this go to Trident
            memcpy((BYTE *)pwNew, (BYTE *)pwOld, cchwOld*sizeof(WCHAR));
            cchNew = cchwOld;
        ...
        if ((fBeginTokTextarea) && (token.tok == TokTag_SSSOPEN && token.tokClass == tokSSS))
        {
            // skip tokenizing. we can't let this go to Trident
            memcpy((BYTE *)pwNew, (BYTE *)pwOld, cchwOld*sizeof(WCHAR));
            cchNew = cchwOld;
        ...
        if ((fBeginTokLabel) && (token.tok == TokTag_SSSOPEN && token.tokClass == tokSSS))
        {
            // skip tokenizing. we can't let this go to Trident
            memcpy((BYTE *)pwNew, (BYTE *)pwOld, cchwOld*sizeof(WCHAR));
            cchNew = cchwOld;
        ...
        if ((fBeginTokListing) && (token.tok == TokTag_SSSOPEN && token.tokClass == tokSSS))
        {
            // skip tokenizing. we can't let this go to Trident
            memcpy((BYTE *)pwNew, (BYTE *)pwOld, cchwOld*sizeof(WCHAR));
            cchNew = cchwOld;
        ...
        if ((token.tok == TokElem_TITLE) && (token.tokClass == tokElem))
        {
            if (mode == modeInput)
            {
                m_fHasTitleIn = TRUE;
        ...
        if ((token.tok == TokElem_BODY) && (token.tokClass == tokElem) && (m_ichBeginBodyTagIn == -1))
        {
            if (PARSE_SPECIAL_HEAD_ONLY & dwSpecialize)
                break;
            if (mode == modeInput)
        ...
        if ((token.tok == TokElem_HEAD) && (token.tokClass == tokElem) && (m_ichBeginHeadTagIn == -1) && (mode == modeInput))
        {
            m_ichBeginHeadTagIn = token.ibTokMin;
        }
        if ((token.tok == TokElem_SELECT) && (token.tokClass == tokElem) && (mode == modeInput) && !(lxs & inSCRIPT))
        {
            if ((pTokArray[itokCur - 1].token.tok == TokTag_START) && (pTokArray[itokCur - 1].token.tokClass == tokTag))
                fBeginTokSelect = TRUE;
            else if ((pTokArray[itokCur - 1].token.tok == TokTag_END) && (pTokArray[itokCur - 1].token.tokClass == tokTag))
                fBeginTokSelect = FALSE;
        }
        if ((token.tok == TokElem_TEXTAREA) && (token.tokClass == tokElem) && (mode == modeInput))
        {
            if ((pTokArray[itokCur - 1].token.tok == TokTag_START) && (pTokArray[itokCur - 1].token.tokClass == tokTag))
        ...
        if ((token.tok == TokElem_LABEL) && (token.tokClass == tokElem) && (mode == modeInput))
        {
            if ((pTokArray[itokCur - 1].token.tok == TokTag_START) && (pTokArray[itokCur - 1].token.tokClass == tokTag))
                fBeginTokLabel = TRUE;
            else if ((pTokArray[itokCur - 1].token.tok == TokTag_END) && (pTokArray[itokCur - 1].token.tokClass == tokTag))
        ...
        if ((token.tok == TokElem_LISTING) && (token.tokClass == tokElem) && (mode == modeInput))
        {
            if ((pTokArray[itokCur - 1].token.tok == TokTag_START) && (pTokArray[itokCur - 1].token.tokClass == tokTag))
        ...
        if ((token.tok == TokAttrib_STARTSPAN) && (token.tokClass == tokAttr) && (mode == modeInput))
            fInDTC = TRUE;
        if ((token.tok == TokElem_OBJECT) && (token.tokClass == tokElem) && (lxs & inEndTag) && (fInDTC) && (mode == modeInput))
            fInDTCOutput = TRUE;
        if ((token.tok == TokAttrib_ENDSPAN) && (token.tokClass == tokAttr) && (mode == modeInput))
        {
            fInDTCOutput = FALSE;
            fInDTC = FALSE;
        }
    ...
    *pcMaxToken = m_cMaxToken = itokCur;
    ...
    if (dwFlags & dwFilterDTCs || dwFlags & dwFilterDTCsWithoutMetaTags || dwFlags & dwFilterServerSideScripts || dwFlags & dwPreserveSourceCode)
    {
        ASSERT(!(dwFlags & dwFilterNone));
        ...
        ASSERT(pTokArray != NULL);
        FilterHtml(pwOld, &pwNew, &cchNew, &hgNew, pTokArray, mode, dwFlags);
...
LOOM:
    ...
    if ((m_hgspInfo != NULL) && (dwFlags & dwPreserveSourceCode))
    {
        if (mode == modeInput && phgDocRestore)
        {
            ...
            if (GlobalSize(*phgDocRestore) < SAFE_PTR_DIFF_TO_INT(pHdr - pHdrSav)*sizeof(WCHAR)+
                SAFE_PTR_DIFF_TO_INT(m_pspInfoCur - m_pspInfo)*sizeof(WORD)+sizeof(int))
            {
                INT cdwSize = SAFE_PTR_DIFF_TO_INT(pHdr - pHdrSav);

                ASSERT(cdwSize >= 0); // validation
                hrRet = ReallocBuffer(phgDocRestore,
                                      SAFE_INT64_TO_DWORD(pHdr - pHdrSav)*sizeof(WCHAR) +
                                      SAFE_INT64_TO_DWORD(m_pspInfoCur - m_pspInfo)*sizeof(WORD) +
                                      sizeof(int),
            ...
            *(int*)pHdr = SAFE_PTR_DIFF_TO_INT(m_pspInfoCur - m_pspInfo);
            ...
            memcpy((BYTE *)pHdr, (BYTE *)m_pspInfo, SAFE_PTR_DIFF_TO_INT(m_pspInfoCur - m_pspInfo)*sizeof(WORD));
...
void CTriEditParse::SetSPInfoState(WORD inState, WORD *pdwState, WORD *pdwStatePrev, BOOL *pfSave)
{
    *pfSave = TRUE;
...
HRESULT CTriEditParse::hrMarkOrdering(WCHAR *pwOld, TOKSTRUCT *pTokArray, INT iArrayStart, int iArrayEnd, UINT cbCur, INT *pichStartOR)
{
    HRESULT hr = S_OK;
    WORD *pspInfoSize;
    WORD cAttr = 0;
        ...
        if (pTokArray[iArrayStart].token.tokClass == tokAttr)
        {
            ...
            while (iArrayQuote < iArrayEnd) // handle the case of white space before the quotes
            {
                if (pTokArray[iArrayQuote].token.tokClass == tokAttr) // gone too far, found next attr
                    break;
                if ((pTokArray[iArrayQuote].token.tokClass == tokValue || pTokArray[iArrayQuote].token.tokClass == tokString)
                    && (pwOld[pTokArray[iArrayQuote].token.ibTokMin] == '"' || pwOld[pTokArray[iArrayQuote].token.ibTokMin] == '\''))
                    break;
            ...
            if (iArrayEq != -1
                ...
                && (pTokArray[iArrayQuote].token.tokClass == tokValue || pTokArray[iArrayQuote].token.tokClass == tokString)
...
BOOL CTriEditParse::FRestoreOrder(WCHAR *pwNew, WCHAR *pwOld, WORD *pspInfoOrder, UINT *pichNewCur, INT /*cwOrderInfo*/,
                                  TOKSTRUCT *pTokArray, INT iArrayStart, INT iArrayEnd,
                                  INT iArrayDSPStart, INT iArrayDSPEnd, INT cchNewCopy, HGLOBAL *phgNew)
{
    ...
    for (i = iStart; i < iEnd; i++)
    {
        if (*(pTokListSav + i - iArrayStart) != TRUE) // if not already copied
        {
            ...
            {
                memcpy((BYTE *)pNewAttr, (BYTE *)(pwOld + pTokArray[i].token.ibTokMin), (1)*sizeof(WCHAR));
                pNewAttr++;
            }
            else
            {
            ...
            if (*(pTokListSav + i - iArrayStart) != TRUE) // we didn't copy this token
            {
                ...
                else
                {
                    if (pTokArray[i].token.tokClass == tokAttr)
                    {
                        ...
                        memcpy((BYTE *)pNewAttr, (BYTE *)pspInfoOrder, (cchAttr)*sizeof(WCHAR));
                    }
                    else if ((isQuote == 1)
    ...
    memcpy((BYTE *)(pwNew + *pichNewCur - cchNewCopy), (BYTE *)(pNewAttrSav), cchNew*sizeof(WCHAR));
    *pichNewCur += (cchNew - cchNewCopy);
    ...
    if (/*(cchNew > cchNewCopy)&&*/ (pwNew[*pichNewCur - 1] == '>' && pwNew[*pichNewCur - 2] == ' '))
    {
...
HRESULT CTriEditParse::hrMarkSpacing(WCHAR *pwOld, UINT cbCur, INT *pichStartSP)
{
    HRESULT hrRet = S_OK;
    UINT i;
    ...
    else // reallocate if needed
    {
        ...
        if (GlobalSize(m_hgspInfo) < SAFE_PTR_DIFF_TO_INT(m_pspInfoCur - m_pspInfo)*sizeof(WORD) +
            (cbCur - *pichStartSP) * 2 * sizeof(WORD)+cbBufPadding)
        {
    ...
    *m_pspInfoCur++ = SAFE_INT_DIFF_TO_WORD(cbCur - *pichStartSP);
    ...
    // part of this info is already in pTokArray.
    // We should use it to reduce the time in this function
...
BOOL CTriEditParse::FRestoreSpacing(LPWSTR pwNew, LPWSTR /*pwOld*/, UINT *pichNewCur,
                                    INT *pcchwspInfo, INT cchRange, INT ichtoktagStart, BOOL fLookback, INT index)
{
    BOOL fRet = TRUE;
    ...
    while (cspInfopair > 0)//(pspInfoCur >= m_pspInfoOut)
    {
        ...
        switch (dwState)
        {
            case inChar:
                ...
                if (index == 0 || index == 3)
                {
                    ...
                    if (countws > 0)
                    {
                        if (ichtoktagStart - countws >= 0)
                        {
                            memcpy((BYTE*)&pwNew[ichtoktagStart - countws + 1],
            ...
            case inTagClose:
            case inTagEq:
                // make sure that we have atleast count number of spaces at pwNew[ichtoktagStart-count]
                if (pwNew[ichtoktagStart] == '=' /* && index == 1*/)
                    fInValue = TRUE;
                else
                    fInValue = FALSE;
                while ((pwNew[ichtoktagStart] == '<' || pwNew[ichtoktagStart] == '>' || pwNew[ichtoktagStart] == '=') && count > 0)
                {
                    count--;
                    ichtoktagStart += cchIncDec;
                    cchRange--;
            ...
            case inSpace:
                ...
                break;
            case inEOL:
                // make sure that we have atleast count number of EOLs at pwNew[ichtoktagStart-count]
                ...
                if (count == 0) //we matched exact EOLs, we may have more EOLs in pwNew
                {
                    ...
                    else if (!fLookback)
                    {
                        ...
                        {
                            while (pwNew[ichtoktagStart] == '\r' || pwNew[ichtoktagStart] == '\n')
                            {
                                countT++;
                                ichtoktagStart += cchIncDec;
                            }
    ...
    if (cspInfopair == 0 && pwNew[ichNewCur - 1] == '>' && ichNewCur > ichtoktagStart && !fLookback && index == 1)
    {
        INT countT = 0;
        ...
        while (pwNew[ichtoktagStart + countT] == ' ' || pwNew[ichtoktagStart + countT] == '\r' ||
        ...
        // Here is how they get there - If we had spaces between the parameter and the '=' and its value, those spacves are removed by Trident.
        // We then go in and add those spaces at the end rather than at proper place because
        // we don't break up the text. e.g. "width = 23" --> "width=23".
        // Now, because we don't break that text, we end up inserting these spaces at the end. Lets remove them.
    }
    else if (cspInfopair == 0 && fLookback && (index == 0 || index == 3)) /* VID6 - bug 18207 */
    {
        INT countT = 0;
