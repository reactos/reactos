...
CHTMLDlg::Create(HTMLDLGINFO * pdlginfo, TCHAR * pchOptions)
{
    ...
    // create the doc
    hr = THR(CoCreateInstance(CLSID_HTMLDocument, NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void **) &_pUnkObj));
    if (hr)
        goto Cleanup;
...
HRESULT CHTMLDlg::InitValues()
{
    ...
    while (S_OK == (hr = pEnumVar->Next(1, &var, NULL)))
    {
        ...
        hr = THR_NOTRACE(V_DISPATCH(&var)->QueryInterface(IID_IHTMLElement, (void **)&pHTMLElement));
        if (hr)
            goto LoopCleanup;
...
HRESULT CHTMLDlg::ConnectElement(IHTMLElement * pHTMLElement, BSTR bstrID)
{
    HRESULT             hr = S_OK;
    DISPID              dispid;
    ...
    hr = THR(_aryDispObjs[0]->GetIDsOfNames(IID_NULL, &bstrID, 1, _lcid, &dispid));
    if (hr)
        goto Cleanup;
    ...
    hr = THR(::ConnectSink(pHTMLElement, IID_IPropertyNotifySink, pExtender, &pExtender->_dwCookie));
    if (hr)
        goto Error;
...
HRESULT CHTMLDlg::GetDocumentTitle(BSTR *pbstr)
...
//  Synopsis:   Per IDispatch
STDMETHODIMP CHTMLDlg::GetIDsOfNames(REFIID, LPOLESTR *  rgszNames, UINT, LCID, DISPID *    rgdispid)
{
...
HRESULT CHTMLDlg::GetDispID(BSTR bstrName, DWORD grfdex, DISPID *pid)
{
    RRETURN(THR_NOTRACE(super::GetInternalDispID(bstrName, pid, grfdex, NULL, NULL)));
}
