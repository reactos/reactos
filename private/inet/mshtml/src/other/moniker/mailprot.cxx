...
HRESULT CMailtoProtocol::LoadMailProvider(HMODULE *pHMod)
{
    ...
    *pHMod = NULL;
    ...
    if (lSuccess != ERROR_SUCCESS || cb <= sizeof(TCHAR))
    {
        fReadFromHKCU = FALSE;
        cb = MAX_PATH;
        lSuccess = RegQueryValue(HKEY_LOCAL_MACHINE, TEXT("Software\\Clients\\Mail"), szMAPIDLL, (LONG *)&cb);
    }
    ...
    lSuccess = RegOpenKeyEx(fReadFromHKCU ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE, aBuffer, NULL, KEY_QUERY_VALUE, &hkey);
    if (lSuccess != ERROR_SUCCESS)
        goto Cleanup;
