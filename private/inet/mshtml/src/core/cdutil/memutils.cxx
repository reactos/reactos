...
//              06-Jul-95   PaulG   Macintosh compiles now use Global instead of Heap memory functions
...
//              Note that MemAlloc/MemRealloc can allocate more than the requested number of bytes.
//              Therefore the size returned from this function is possibly greater than the size passed to MemAlloc/Realloc.
...
//              If the requested size is zero, this function returns a valid pointer.  The returned
//              pointer is guaranteed to be suitably aligned for storage of any object type.
...
//  Returns:    Pointer to the allocated block, or NULL on error.
{
    ...
    // The small-block heap will lose its mind if this ever happens, so we protect against the possibility.
...
//              If the requested size is zero, this function returns a valid pointer.
//              The returned pointer is guaranteed to be suitably aligned for storage of any object type.
//  Arguments:  [cb] - Number of bytes to allocate.
//  Returns:    Pointer to the allocated block, or NULL on error.
{
...
//  Synopsis:   Free a block of memory allocated with MemAlloc, MemAllocFree or MemRealloc.
//  Arguments:  [pv] - Pointer to block to free.  A value of zero is is ignored.
{
...
//              block of memory, or free a block of memory depending on the arguments.

//              If cb is zero, this function always frees the block of memory and *ppv is set to zero.
//              If cb is not zero and *ppv is zero, then this function allocates cb bytes.
//              If cb is not zero and *ppv is non-zero, then this function changes the size of the block, possibly by moving it.

//              On error, *ppv is left unchanged.
//              The block contents remains unchanged up to the smaller of the new and old sizes.
//              The contents of the block beyond the old size is undefined.
//              The returned pointer is guaranteed to be suitably aligned for storage of any object type.

//              The signature of this function is different than thy typical
//              realloc-like function to avoid the following common error:
//                  pv = realloc(pv, cb);
//              If realloc fails, then null is returned and the pointer to the original block of memory is leaked.

//  Arguments:  [cb] - Requested size in bytes. A value of zero always frees the block.
        ...
        *ppv = _MemAlloc(cb);
...
#endif
        {
            ...
            if ((pmap = __sbh_find_block(pv, &preg, &ppage)) != NULL)
            {
                ...
                pvNew = NULL;
                /*
                 * If the new size falls below __sbh_threshold, try to carry out the reallocation within the small-block heap.
                 */
                if (cbr < __sbh_threshold) {
                 ...
                 * performed in the small-block heap, try to allocate a new block with HeapAlloc.
                 */
...
void _MemSetHeader(void * pv, ULONG cb, PERFMETERTAG mt)
{
#ifdef PERFMETER
    if (!IsVMemEnabled())
    {
...
void * _MtMemAlloc(PERFMETERTAG mt, ULONG cb)
{
...
void * _MtMemAllocClear(PERFMETERTAG mt, ULONG cb)
{
    if (MtSimulateOutOfMemory(mt, -1))
    {
        return NULL;
...
HRESULT _MtMemRealloc(PERFMETERTAG mt, void ** ppv, ULONG cb)
{
    if (cb == 0)
    {
        _MtMemFree(*ppv);
...
HRESULT _MtMemAllocString(PERFMETERTAG mt, ULONG cch, const TCHAR *pchSrc, TCHAR **ppchDest)
{
    TCHAR *pch;
...
HRESULT _MtMemReplaceString(PERFMETERTAG mt, const TCHAR *pchSrc, TCHAR **ppchDest)
{
    HRESULT hr;
    TCHAR *pch;
...
void _MtMemSetName(void * pv, char * szFmt, ...)
{
    if (pv && !IsVMemEnabled())
    {
        char szBuf[1024];
...
void * _MgMemAllocClear(ULONG cb)
{
    _MgMemValidate();
...
HRESULT _MgMemRealloc(void ** ppv, ULONG cb)
{
    _MgMemValidate();
...
HRESULT _MgMemAllocString(ULONG cch, const TCHAR *pchSrc, TCHAR **ppchDest)
{
    TCHAR *pch;
    size_t cb = cch * sizeof(TCHAR);
...
HRESULT _MgMemReplaceString(const TCHAR *pchSrc, TCHAR **ppchDest)
{
    HRESULT hr;
    TCHAR *pch;
