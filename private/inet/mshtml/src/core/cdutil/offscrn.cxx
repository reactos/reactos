...
HRESULT InitSurface()
{
    ...
    g_hrDirectDraw = THR(CoCreateInstance(CLSID_DirectDrawFactory, NULL, CLSCTX_INPROC_SERVER, IID_IDirectDrawFactory, (void **)&pDDFactory));
    if (SUCCEEDED(g_hrDirectDraw))
    {
        ...
        g_hrDirectDraw = THR(pDDFactory->CreateDirectDraw(NULL, GetDesktopWindow(), DDSCL_NORMAL, 0, NULL, &g_pDirectDraw));
        SetErrorMode( dwMode );

        ReleaseInterface((IUnknown*)pDDFactory);// release the factory
    }
#ifdef PRODUCT_PROF_FERG
    // for PROFILING perposes only
    ::ResumeCAP();
