...
static HRESULT ResetTimer(void * pvObject, UINT idTimer, UINT uTimeout)
{
    ...
    // than 10, NT spews to the debugger.  Work around this problem by rounding up to 10.
...
#ifndef WIN16
        if (((g_dwPlatformID == VER_PLATFORM_WIN32_NT) || (g_dwPlatformID == VER_PLATFORM_WIN32_UNIX)) && !pts->gwnd.hhookGWMouse)
        {
            pts->gwnd.hhookGWMouse = SetWindowsHookEx(WH_MOUSE, GWMouseProc, (HINSTANCE) NULL, GetCurrentThreadId());
        }
#endif //!WIN16
    }
...
HRESULT InitGlobalWindow(THREADSTATE *   pts)
{
    ...
    {
        hr = THR(RegisterWindowClass(_T("Hidden"), GlobalWndProc, 0, NULL, NULL, &s_atomGlobalWndClass));
        if (hr)
            goto Error;
    }
#endif // _MAC
...
#else
    pts->gwnd.hwndGlobalWindow = TW32(NULL, CreateWindow(pszBuf, NULL, WS_POPUP, 0, 0, 0, 0, NULL, NULL, g_hInstCore, NULL));
    if (pts->gwnd.hwndGlobalWindow == NULL)
    {
        hr = GetLastWin32Error();
        goto Error;
