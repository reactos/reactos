...
void GetChildWindowRect(HWND hwndChild, LPRECT prc)
//  Synopsis:   Gets the rectangle of the child window in its parent window coordinates
...
//  Synopsis:   Calls UpdateWindow for a window, then recursively calls UpdateWindow its children.
//  Arguments:  [hWnd]      Window to update, along with its children
{
    ...
    {
        UpdateWindow(hWnd);
        RedrawWindow(hWnd, (GDIRECT *)NULL, NULL, RDW_UPDATENOW | RDW_ALLCHILDREN | RDW_INTERNALPAINT);
    }
}
...
BOOL InClientArea(POINTL pt, HWND hWnd)
...
//  Synopsis:   Returns TRUE if the passed HWND is contained within a window that has WS_POPUP set
//  Arguments:  [hwnd] - Window handle to check
...
//  Synopsis:   Return the HWND of the MDI child which contains the passed HWND
...
HWND GetOwningMDIChild(HWND    hwnd)
{
    ...
    while (hwndMDIChild && !(::GetWindowLong(hwndMDIChild, GWL_EXSTYLE) & WS_EX_MDICHILD))
    {
...
//  Synopsis:   Return the outer most owner/parent HWND of the given window HWND
