...
BEGIN_TEAROFF_TABLE(CBase, IDispatchEx)
    ...
    TEAROFF_METHOD(CBase, GetMemberProperties, getmemberproperties, (DISPID id, DWORD grfdexFetch, DWORD *pgrfdex))
    TEAROFF_METHOD(CBase, GetMemberName, getmembername, (DISPID id, BSTR *pbstrName))
    TEAROFF_METHOD(CBase, GetNextDispID, getnextdispid, (DWORD grfdex, DISPID id, DISPID *pid))
...
HRESULT CFunctionPointer::PrivateQueryInterface(REFIID iid, void **ppv)
{
    *ppv = NULL;
    ...
    if (cArgs + cNamedArgs)
    {
        ...
        if (pOrgDispParams->cArgs)
        {
            if (cArgs >= pOrgDispParams->cArgs)
            {
                ...
                {
                    memcpy(&rgvarg[iStartIndex], pOrgDispParams->rgvarg, sizeof(VARIANTARG) * pOrgDispParams->cArgs);
                }
            }
            else
            {
        ...
        if (cNamedArgs)
        {
            ...
            if (pOrgDispParams->cNamedArgs)
            {
                if (cNamedArgs >= pOrgDispParams->cNamedArgs)
                {
                    ...
                    {
                        memcpy(&rgdispidNamedArgs[iStartIndex], pOrgDispParams->rgdispidNamedArgs, sizeof(VARIANTARG) * pOrgDispParams->cNamedArgs);
                    }
                }
                else
                {
...
STDMETHODIMP CBaseLockCF::LockServer (BOOL fLock)
{
    HRESULT hr;
...
//  Synopsis:   Per IDispatch
STDMETHODIMP CBase::GetIDsOfNames(REFIID riid, LPOLESTR *  rgszNames, UINT, LCID, DISPID *    rgdispid)
{
    HRESULT         hr;
    IDispatchEx    *pDispEx;
                                    ...
                                    IUnknown *pUnkContext)
{
    ...
    {
ExpandoInvoke:
        hr = THR(InvokeAA (dispidMember, CAttrValue::AA_Expando, lcid, wFlags, pdispparams, pvarResult, pexcepinfo, pSrvProvider));
    }
    else
    {
        ...
        else
        {
            ...
            if (IsTagEnabled(tagInvokeTrace))
            {
                ...
                OutputDebugString((pDesc->a.pstrExposedName) ? pDesc->a.pstrExposedName : pDesc->a.pstrName);
...
STDAPI MatchExactGetIDsOfNames(ITypeInfo *pTI, REFIID riid, LPOLESTR *rgszNames, UINT cNames, LCID lcid, DISPID *rgdispid, BOOL fCaseSensitive)
{
    HRESULT         hr = S_OK;
    CTypeInfoNav    tin;
