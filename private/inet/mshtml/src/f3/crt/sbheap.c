...
)
{
     ...
     */
    if ((ppage = VirtualAlloc(NULL, _PAGESIZE_ * _PAGES_PER_REGION, MEM_RESERVE, PAGE_READWRITE)) != NULL)
    {
        /*
         * Commit the first _PAGES_PER_COMMITMENT of the new region.
         */
        if (VirtualAlloc(ppage, _PAGESIZE_ * _PAGES_PER_COMMITMENT, MEM_COMMIT, PAGE_READWRITE) != NULL)
        {
            /*
            ...
            if (pregnew == &__small_block_heap) {
                if (__small_block_heap.p_next_region == NULL)
                    __small_block_heap.p_next_region = &__small_block_heap;
...
)
{
    ...
    do
    {
            ...
            (preg->first_uncommitted_index != _NO_INDEX))
        {
             ...
             * Commit the pages.
             */
            if (VirtualAlloc((void *)(preg->p_pages + preg->first_uncommitted_index),
                (i - preg->first_uncommitted_index) * _PAGESIZE_, MEM_COMMIT,
                             PAGE_READWRITE) ==
