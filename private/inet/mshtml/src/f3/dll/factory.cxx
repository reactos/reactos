...
LocalGetClassObject(REFCLSID clsid, REFIID iid, LPVOID FAR* ppv)
{
    ...
    // co-create shdocvw
    hr = THR(CoCreateInstance(CLSID_DocHostUIHandler, NULL, CLSCTX_INPROC_SERVER, IID_IOleCommandTarget, (void**)&pCommandTarget));
    if (hr)
    ...
    hr = pCommandTarget->Exec(&CGID_DocHostCommandHandler, SHDVID_CLSIDTOMONIKER, 0, &varIn, &varOut);
    fIsPropPageClsid = !hr;
    if (hr)
        goto PropPageCleanup;
