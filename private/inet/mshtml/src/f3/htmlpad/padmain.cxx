...
Initialize()
{
    ...
    g_hwndMain = CreateWindow(SZ_APPLICATION_NAME TEXT(" Main"), NULL, WS_OVERLAPPEDWINDOW, 0, 0, 0, 0, NULL, NULL, g_hInstCore, NULL);
    if (!g_hwndMain)
    {
        hr = E_FAIL;
        goto Cleanup;
...
CheckError(HWND hwnd, HRESULT hr)
{
    ...
    if (!FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, hr, LANG_SYSTEM_DEFAULT, achBuf, ARRAY_SIZE(achBuf), NULL))
    {
        _tcscpy(achBuf, TEXT("Unknown error."));
    }

    MessageBox(hwnd, achBuf, NULL, MB_APPLMODAL | MB_ICONERROR | MB_OK);
    return hr;
}
