...
static HRESULT CreateDoc(IUnknown **ppUnk)
{
...
#if !defined(WINCE)
    if (    !achPath[0] &&  !achStartDir[0] &&  GetEnvironmentVariable(_T("PAD_DEFAULTFILEDIR"), achStartDir, MAX_PATH))
    {
...
CPadDoc::Open(REFCLSID clsid, TCHAR *pchPath)
{
    ...
    hr = THR(CoCreateInstance(clsid, NULL, _fNetscape ? CLSCTX_LOCAL_SERVER : CLSCTX_INPROC_SERVER | CLSCTX_INPROC_HANDLER, IID_IUnknown, (void **)&pUnk));
    if (hr)
        goto Cleanup;
...
CPadDoc::Open(IStream * pStream)
{
    ...
    hr = THR(CoCreateInstance(CLSID_HTMLDocument, NULL, CLSCTX_INPROC_SERVER | CLSCTX_INPROC_HANDLER, IID_IUnknown, (void **)&pUnk));
    if (hr)
        goto Cleanup;
