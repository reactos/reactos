...
HRESULT CPadScriptSite::Init(TCHAR *pchType)
{
    HRESULT hr;
    ...
    hr = THR(CoCreateInstance(pchType && fJscript ? CLSID_JSCRIPT : CLSID_VBS, NULL, CLSCTX_INPROC_SERVER, IID_IActiveScript, (void **)&_pScript));
    if (hr)
        goto Cleanup;
    ...
    if (fJscript)
    {
        ...
        hr = THR(pParse->AddScriptlet(NULL, _T("PadLoad()"), _T("Pad"), NULL, _T("Load"), _T("\""), 0, 0, 0, &bstrName, NULL));
...
HRESULT CPadScriptSite::GetClassInfo(ITypeInfo **ppTypeInfo)
{
    HRESULT hr;
...
CPadScriptSite::OnScriptError(IActiveScriptError *pse)
{
    ...
    {
        achDescription[0] = 0;
        FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, ei.scode, LANG_SYSTEM_DEFAULT, achDescription, ARRAY_SIZE(achDescription), NULL);
