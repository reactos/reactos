...
//  File:       rootctx.cxx
//  Contents:   CHtmRootParseCtx adds text and nodes to the tree on behalf of the parser.
...
HRESULT CreateHtmRootParseCtx(CHtmParseCtx **pphpx, CMarkup *pMarkup)
{
...
CHtmParseCtx * CHtmRootParseCtx::GetHpxEmbed()
{
    return this;
}

BOOL CHtmRootParseCtx::SetGapToFrontier(CTreePosGap * ptpg)
{
...
HRESULT CHtmRootParseCtx::Prepare()
{
    HRESULT hr = S_OK;
...
HRESULT CHtmRootParseCtx::Commit()
{
    HRESULT         hr;
...
HRESULT CHtmRootParseCtx::Finish()
{
    HRESULT hr = S_OK;
...
HRESULT CHtmRootParseCtx::InsertLPointer(CTreePos * * pptp, CTreeNode * pNodeCur)
{
    RRETURN(InsertPointer(pptp, pNodeCur, TRUE));
}

HRESULT CHtmRootParseCtx::InsertRPointer(CTreePos * * pptp, CTreeNode * pNodeCur)
{
    RRETURN(InsertPointer(pptp, pNodeCur, TRUE));
}

HRESULT CHtmRootParseCtx::InsertPointer(CTreePos * * pptp, CTreeNode * pNodeCur, BOOL fRightGravity)
{
    HRESULT hr = S_OK;
...
HRESULT CHtmRootParseCtx::InsertTextFrag(TCHAR * pch, ULONG cch, CTreeNode * pNodeCur)
{
    HRESULT     hr = S_OK;
...
//  3. Call hack code
HRESULT CHtmRootParseCtx::BeginElement(CTreeNode **ppNodeNew, CElement *pel, CTreeNode *pNodeCur, BOOL fEmpty)
{
    HRESULT         hr = S_OK;
    CTreeNode *     pNode;
...
HRESULT CHtmRootParseCtx::EndElement(CTreeNode **ppNodeNew, CTreeNode *pNodeCur, CTreeNode *pNodeEnd)
{
    HRESULT hr = S_OK;
    ...
    // because the element is ending
    {
        ...
        if (pElementEnd->WantTextChangeNotifications() || pElementEnd->WantEndParseNotification())
        {
        ...
        *ppNodeNew = pNodeEnd->Parent();
        ...
        if (*ppNodeNew)
        {
            ...
            if (_ptpChainCurr == _ptpChainTail)
            {
                ...
                Assert(ptpContent->IsEndNode() && ptpContent->Branch() == pNodeEnd);
            }
            ...
            else
            {
                ...
                Assert(_ptpChainCurr &&   _ptpChainCurr->IsEndNode() && _ptpChainCurr->Branch() == pNodeEnd);
...
HRESULT CHtmRootParseCtx::OverlappedEndElement(CTreeNode **ppNodeNew, CTreeNode* pNodeCur, CTreeNode *pNodeEnd, BOOL fFlushNotification)
{
    HRESULT         hr = S_OK;
    ...
    // besides end edges.  If we encounter pointers here, we will move them.
...
HRESULT CHtmRootParseCtx::AddText(CTreeNode *pNode, TCHAR *pch, ULONG cch, BOOL fAscii)
{
    HRESULT         hr = S_OK;
    ...
    if (_cchNodeBefore)
    {
        ...
        Verify(CTxtPtr(_pMarkup, cpInsert).InsertRepeatingChar(_cchNodeBefore, WCH_NODE) == _cchNodeBefore);
        _cchNodeBefore = 0;
    }
    ...
    if (cch > 0)
    {
        Verify(ULONG(CTxtPtr(_pMarkup, _cpChainCurr).InsertRange(cch, pch)) == cch);
    }
...
HRESULT CHtmRootParseCtx::HookBeginElement(CTreeNode * pNode)
{
    return S_OK;
}
...
HRESULT CHtmRootParseCtx::HookEndElement(CTreeNode * pNode, CTreeNode * pNodeCur)
{
...
BOOL CHtmTopParseCtx::QueryTextlike(ELEMENT_TAG etag, CHtmTag *pht)
{
