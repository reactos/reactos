    ...
    BOOL fDisableNoShowElement)
{
    ...
    *pfReturn = FALSE;
    ...
    hr = THR(_pSecurityMgr->ProcessUrlAction(pchURL, dwAction, (BYTE *)&dwPolicy, sizeof(DWORD), pbArg, cbArg, dwPuaf, 0));
    if (hr == S_FALSE)
        hr = S_OK;
...
CDoc::GetSecurityID(BYTE *pbSID, DWORD *pcb, LPCTSTR pchURL)
{
    ...
    // Unescape the URL.
    hr = THR(CoInternetParseUrl(pchURL, PARSE_ENCODE, 0, achURL, ARRAY_SIZE(achURL), &dwSize, 0));
    if (hr)
        goto Cleanup;

    hr = THR(_pSecurityMgr->GetSecurityId(achURL, pbSID, pcb, (DWORD_PTR)(TCHAR *)_cstrSetDomain));
    if (hr)
        goto Cleanup;
...
CDoc::UpdateSecurityID()
{
    ...
    if (_pOmWindow)
    {
        ...
        if ((TLS(windowInfo.paryWindowTbl)->FindProxy(_pOmWindow->_pWindow, abSID, cbSID, _fTrustedDoc, NULL)))
        {
            ...
            THR(TLS(windowInfo.paryWindowTbl)->AddTuple(_pOmWindow->_pWindow, abSID, cbSID, _fTrustedDoc, _pOmWindow));
        }
    }
}
...
//  Synopsis:   Pass out the zone of the current doc depending on variant passed in.
    ...
    DWORD dwReserved)
{
    ...
    if (guidKey == GUID_CUSTOM_CONFIRMOBJECTSAFETY)
    {
        ...
        if (OK(THR_NOTRACE(pconfirm->pUnk->QueryInterface(IID_IActiveScript, (void **)&pScript))) && pScript)
        {
        ...
        if (!fSafe)
        {
            fSafe = IsSafeTo(safety, *piid, pconfirm->clsid, pconfirm->pUnk, this);
        }
...
CSecurityMgrSite::GetWindow(HWND *phwnd)
{
    ...
    else
    {
        ...
        if (hr)
        {
            if (Doc()->_pClientSite && S_OK == Doc()->_pClientSite->QueryInterface(IID_IOleWindow, (void **)&pOleWindow))
            {
