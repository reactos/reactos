...
CBase * CreateJSProtocol(IUnknown *pUnkOuter)
    ...
    DWORD       dwReserved)
{
    ...
    {
        hr = THR_NOTRACE(super::ParseUrl(pwzUrl, ParseAction, dwFlags, pwzResult, cchResult, pcchResult, dwReserved));
    }
...
HRESULT CJSProtocol::ParseAndBind()
{
    HRESULT         hr = S_OK;
    ...
    Assert(URL_SCHEME_VBSCRIPT == uProt || URL_SCHEME_JAVASCRIPT == uProt);
    ...
    *(pchSourceUrlTmp - --i) = 0;
    ...
    hr = THR(QueryService(CLSID_HTMLDocument, CLSID_HTMLDocument, (void **)&pDoc));
    if (hr)
    {
        // We didn't succeed, so try the old method of querying
        // up to shdocvw.
        hr = THR(QueryService(IID_ITargetFrame2, IID_ITargetFrame2, (void **)&pTF2));
        if (hr)
            goto Cleanup;
        ...
        if (pOleContainer)
        {
            ...
            hr = THR(pOleContainer->QueryInterface(CLSID_HTMLDocument, (void **)&pDoc));
            if (hr)
                goto Cleanup;
        }
    }
    ...
    else
    {
        ...
        hr = THR(Format(FMT_OUT_ALLOC, &pchOutput, 0, WRITTEN_SCRIPT, (LPCSTR)cstrProtocol, pchScript));
#else
        hr = THR(Format(FMT_OUT_ALLOC, &pchOutput, 0, WRITTEN_SCRIPT, (LPTSTR)cstrProtocol, pchScript));
#endif
        if (hr)
            goto Cleanup;
    }
    ...
    if (pchOutput)
    {
        cb = WideCharToMultiByte(_bindinfo.dwCodePage, 0, pchOutput, -1, NULL, 0, NULL, NULL);
        ...
        WideCharToMultiByte(_bindinfo.dwCodePage, 0, pchOutput, -1, (char *)pbOutput, cb, NULL, NULL);
