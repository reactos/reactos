...
WrapSpecialUrl(TCHAR *pchURL, CStr *pcstrExpandedUrl, CStr &cstrDocUrl, BOOL fNonPrivate, BOOL fIgnoreUrlScheme)
{
    ...
    if (IsSpecialUrl(pchURL) || fIgnoreUrlScheme)
    {
        ...
        // If this is javascript:, vbscript: or about:, append the url of this document so that on the other side we can
        ...
        while ((uPreviousLen != cstrSafeUrl.Length()) && _tcschr(cstrSafeUrl, _T('%')))
        {
            ...
            for (;;)
            {
                ...
                {
                    // Encode the URL
                    hr = THR(CoInternetParseUrl(cstrSafeUrl, PARSE_ENCODE, 0, achUrl, ARRAY_SIZE(achUrl), &dwSize, 0));
...
*/
    ...
    else
    {
        ...
        if (nResult == 16)// if iexplore is already running...
        {
            // find the window for IExplore and send a registered to open the selected URL
            ...
            if (hBrowser)
            {
                ...
                SendMessage(hBrowser, WM_BROWSER_OPENURL, 0, (LPARAM)szURL);
            }
        }
#else
        DbgMemoryTrackDisable(TRUE); // (dbau) hlsntm leaks one string
        grfHLNF &= ~HLNF_OPENINNEWWINDOW;   // Implied by HlinkFrameNavigateNHL
        hr = THR(HlinkFrameNavigateNHL(grfHLNF, pBindCtx, pDwnBindInfo, fOpenWithFrameName ? pchTarget:NULL, cstrExpandedUrl, *pcstrLocation));
