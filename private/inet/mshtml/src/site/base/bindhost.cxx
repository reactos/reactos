...
HRESULT CDoc::GetBaseTarget(TCHAR **ppchTarget, CElement *pElementContext)
//              Get the base URL for the document.
//              If supplied with an element, gets the base TARGET in effect at that element, based on the position of <BASE> tags.
{
    ...
    if (pElementContext && pElementContext->GetFirstBranch())
    {
        ...
        for (pNode = pElementContext->GetFirstBranch()->Parent(); pNode; pNode = pNode->Parent())
        {
...
HRESULT CDoc::GetBaseUrl(TCHAR * *  ppchHref, CElement * pElementContext, BOOL *     pfDefault, TCHAR *    pszAlternateDocUrl)
//              Get the base URL for the document.
//              If supplied with an element, gets the base URL in effect at that element, based on the position of <BASE> tags.
//              Note that this is a pointer to an internal string, it can't be modified.
//              If you need to modify it, make a copy first.
{
    ...
    if (pElementContext != EXPANDIGNOREBASETAGS && _fHasBaseTag)
    {
        ...
        // probably want to search in a specific tree! Maybe we always need a context element!
        if (pchHref == NULL)
        {
    ...
    TCHAR *    pszAlternateDocUrl)
{
    ...
    hr = THR(CoInternetCombineUrl(pchBaseUrl, pchRel, dwFlags, pchUrlOut, pdlUrlLen, &cchBuf, 0));
    if (hr)
        goto Cleanup;

    if (!fDefault
        &&  S_OK == THR(CoInternetQueryInfo(pszAlternateDocUrl ? pszAlternateDocUrl : _cstrUrl,
                                            QUERY_RECOMBINE, 
                                            0, 
                                            &fCombine,
                                            sizeof(BOOL),
        ...
        && fCombine)
    {
        ...
        hr = THR(CoInternetCombineUrl(pszAlternateDocUrl ? pszAlternateDocUrl : _cstrUrl, pchUrlOut, dwFlags, achBuf2, pdlUrlLen, &cchBuf2, 0));
        if (hr)
            goto Cleanup;
...
BOOL CDoc::ValidateSecureUrl(TCHAR *pchUrl, BOOL fReprompt, BOOL fSilent, BOOL fUnsecureSource)
//  Synopsis:   Given an absolute URL, returns TRUE if it is okay (in terms of SSL security) to begin download of the URL.
//              May prompt the user (once per top-level frame).
//              If fReprompt is TRUE (default FALSE), the user is reprompted even if s/he previously answered NO.
//              The user is never reprompted if s/he previously answered YES. (Once unsecure, always unsecure.)
{
    BOOL bResult = TRUE;
    DWORD error;
    ...
    GetRootSslState(&sslSecurity, &sslPrompt);// grab the current security state
    ...
    if (sslPrompt == SSL_PROMPT_QUERY || (sslPrompt == SSL_PROMPT_DENY && fReprompt))
    {
        ...
        else
        {
            ...
            // in case state has changed during message box, grab the current security state again
...
void CDoc::GetRootSslState(SSL_SECURITY_STATE *psss, SSL_PROMPT_STATE *psps)
{
    CDoc *pDoc = GetRootDoc();
...
void CDoc::SetRootSslState(SSL_SECURITY_STATE sss, SSL_PROMPT_STATE sps, BOOL fInit)
{
    CDoc *pDoc = GetRootDoc();
    ...
    if (pDoc->_sslSecurity != sss || fInit)
    {
        ...
        if (pDoc->_pClientSite)
        {
            ...
            CTExec(pDoc->_pClientSite, &CGID_ShellDocView, SHDVID_SETSECURELOCK, 0, &varIn, 0);
        }
    }
}


BOOL CDoc::AllowFrameUnsecureRedirect()
{
    SSL_SECURITY_STATE sslSecurity;
    SSL_PROMPT_STATE   sslPrompt;
...
void CDoc::OnHtmDownloadSecFlags(DWORD dwFlags)
{
    if (IsRootDoc())
    {
        SSL_SECURITY_STATE sslSecurity;
...
void CDoc::OnSubDownloadSecFlags(const TCHAR *pchUrl, DWORD dwFlags)
{
    SSL_SECURITY_STATE sslSecurity;
    ...
    // so fixup "min" by detecting the case where the smaller one was SSL_SECURITY_SECURE and taking the other one
...
void CDoc::OnIgnoreFrameSslSecurity()
//  Synopsis:   BUGBUG: when a non-SSL frame load has been suppressed by the user, we need to touch subframe's host with a CTExec
//              SDHVID_SETSECURELOCK in LoadFromInfo so that the shell doesn't ask urlmon/wininet for its idea of the frame's security.
{
    SSL_SECURITY_STATE sslSecurity;
    SSL_PROMPT_STATE   sslPrompt;
    ...
    if (_pClientSite)
    {
        ...
        CTExec(_pClientSite, &CGID_ShellDocView, SHDVID_SETSECURELOCK, 0, &varIn, 0);
    }
}
...
TCHAR * GetFriendlyUrl(const TCHAR *pchUrl, const TCHAR *pchBaseUrl, BOOL fShowFriendlyUrl, BOOL fPreface, LONG x, LONG y)
{
    URL_COMPONENTS uc;
    ...
    if (InternetCrackUrl(achUrl, _tcslen(achUrl), 0, &uc))
    {
        ...
        if (fMail)
        {
            if (fShowFriendlyUrl)
            {
                Format(0,
                       achFriendlyUrl,
                       ARRAY_SIZE(achFriendlyUrl),
                       MAKEINTRESOURCE(IDS_FRIENDLYURL_SENDSMAILTO),
        ...
        else if (fShowFriendlyUrl)
        {
            ...
            if (fFriendlyString)
            {
                ...
                if (fShowHostName && pchShortName && *(pchShortName + 1))
                {
                    ...
                    Format(0, achFriendlyUrl, ARRAY_SIZE(achFriendlyUrl), MAKEINTRESOURCE(IDS_FRIENDLYURL_AT), achShortName, achHostName);
                }
            ...
            if (fPreface && !fMail)
                Format(FMT_OUT_ALLOC, &pchFriendlyUrl, 0, MAKEINTRESOURCE(IDS_FRIENDLYURL_SHORTCUTTO), achFriendlyUrl, achSuffix);
            else
                Format(FMT_OUT_ALLOC, &pchFriendlyUrl, 0, _T("<0s> <1s>"), achFriendlyUrl, achSuffix);
...
HRESULT ExpandUrlWithBaseUrl(LPCTSTR pchBaseUrl, LPCTSTR pchRel, TCHAR ** ppchUrl)
{
    ...
    *ppchUrl = NULL;
    ...
    else
    {
        ...
        if (S_OK == CoInternetQueryInfo(pchBaseUrl, QUERY_RECOMBINE, 0, &fCombine, sizeof(BOOL), &dwSize, 0) && fCombine)
        {
            TCHAR achBuf2[pdlUrlLen];
            DWORD cchBuf2;
