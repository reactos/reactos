...
TCHAR * ProtocolFriendlyName(TCHAR * szURL)
{
    ...
    if (ProtocolTable[i].szName)
    {
        ...
        LONG lResult = RegOpenKeyEx(HKEY_CLASSES_ROOT, ProtocolTable[i].szRegKey, 0, KEY_QUERY_VALUE, &hkeyProtocol);
        if (lResult != ERROR_SUCCESS)
            goto Cleanup;

        lResult = RegQueryValue(hkeyProtocol, NULL, szBuf, (long *) &dwLen);
