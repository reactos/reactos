...
HRESULT CPropertyPageSite::Init(CPropertyDialog *pDialog, int iLevel, CLSID *pclsid)
{
    HRESULT         hr;
    ...
    hr = THR_NOTRACE(LocalGetClassObject(*pclsid, IID_IClassFactory, (void **)&pCF));
    if (!hr)
    {
        hr = THR(pCF->CreateInstance(NULL, IID_IPropertyPage, (void **)&_pPage));
    }
    else
    {
        hr = THR(CoCreateInstance(*pclsid, NULL, CLSCTX_INPROC_SERVER, IID_IPropertyPage, (void **)&_pPage));
    }
    if (hr)
        goto Cleanup;
...
CDoc::ShowPropertyDialog(int cElements, CElement ** apElement)
{
...

    {
        ...
        if (_pBackupHostUIHandler)
        {
            ...
            hr = pBackupHostUICommandHandler->Exec(&CGID_DocHostCommandHandler, OLECMDID_PROPERTIES, 0, &varIn, NULL);
        }
    }
