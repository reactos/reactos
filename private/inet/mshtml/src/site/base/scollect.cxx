...
CScriptCollection::IsSafeToRunScripts(CLSID *pClsid, IUnknown *pUnk)
{
    ...
    hr = THR(Doc()->ProcessURLAction(URLACTION_SCRIPT_OVERRIDE_SAFETY, &fSafe));
    if (hr || fSafe)
        goto Cleanup;

    // WINCEREVIEW - ignore script safety checking !!!!!!!!!!!!!!!!
#ifndef WINCE
    fSafe = ::IsSafeTo(SAFETY_SCRIPTENGINE, IID_IActiveScriptParse, *pClsid, pUnk, Doc());
    if (fSafe)
        goto Cleanup;

    fSafe = ::IsSafeTo(SAFETY_SCRIPTENGINE, IID_IActiveScript, *pClsid, pUnk, Doc());
#else // !WINCE
    ...
    TCHAR **            ppchCleanCode)
{
    ...
    if (pchCode)
    {
        ...
        if (pchColon)
        {
            *pchColon = 0;// we assume here that we can modify string at pchColon temporarily
    ...
    CScriptHolder **    ppHolder)
{
    ...
    // Perf optimization for Win98 and Win95 to not hit the registry with CLSIDFromPROGID.

    // BUGBUG: ***TLL*** better solution is to remember
    // the language name and clsid in the script holder as a cache and check in the holder first.
    if ((*pchLanguage == _T('j') || *pchLanguage == _T('J')) && (0 == StrCmpIC(pchLanguage, _T("jscript")) || 0 == StrCmpIC(pchLanguage, _T("javascript"))))
    {
        clsid = CLSID_JScript;
    }
    else if ((*pchLanguage == _T('v') || *pchLanguage == _T('V')) && (0 == StrCmpIC(pchLanguage, _T("vbs")) || 0 == StrCmpIC(pchLanguage, _T("vbscript"))))
    {
...
InitScriptDebugging()
{
   TraceTag((tagScriptCollection, "InitScriptDebugging"));
    ...
    hr = THR_NOTRACE(CoCreateInstance(CLSID_ProcessDebugManager, NULL, CLSCTX_ALL, IID_IProcessDebugManager, (void **)&g_pPDM));
    if (hr)
        goto Cleanup;
