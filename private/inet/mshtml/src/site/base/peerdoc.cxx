...
GetUrlFromDefaultBehaviorName(LPCTSTR pchName, LPTSTR pchUrl, UINT cchUrl)
{
    ...
    lRet = RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\Microsoft\\Internet Explorer\\Default Behaviors"), 0, KEY_QUERY_VALUE, &hKey);
    ...
    UINT                        cchUrlDownload)
{
    ...
    *ppFactory = NULL;
    ...
    if (S_OK == hr2)
    {
        ...
        hr = THR(CoCreateInstance(classpec.tagged_union.clsid, NULL, CLSCTX_INPROC_SERVER | CLSCTX_INPROC_HANDLER, IID_IElementBehaviorFactory, (void **)ppFactory));
        Assert (*ppFactory);
    }
    else
    {
    ...
    IElementBehavior **     ppPeer)
{
    ...
    *ppPeer = NULL;
    ...
    if (!_fDefaultPeerFactoryEnsured)
    {
        ...
        hr = THR(CoCreateInstance(CLSID_DEFAULTPEERFACTORY, NULL, CLSCTX_INPROC_SERVER | CLSCTX_INPROC_HANDLER, IID_IElementBehaviorFactory, (void **)&_pDefaultPeerFactory));
        if (hr)
