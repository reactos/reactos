        ...
        CLSID *pclsid)
{
    ...
    // BUGBUG (alexz) - we need some generic protocol allowing us to ask script engine about it's case-sensitivity.
    if (IsEqualGUID(_clsid, CLSID_JScript))
    {
        _fCaseSensitive = TRUE;
    }

    // If script engine supports fast sinking (uses ITridentEventSink) then tell the script engine we support it.
    TurnOnFastSinking();
...
CScriptHolder::GetLCID(LCID *plcid)
{
    ...
    // Return the user locale ID, this make VBS locale sensitive and means that VBS pages can not be written to be locale neutral (a page cannot
    // be written to run on different locales).  Only JScript can do this.
    // However, VBS can be written for Intra-net style apps where functions like string comparisons, UCase, LCase, etc. are used.  Also, some
    // pages could be written in both VBS and JS.  Where JS has the locale neutral things like floating point numbers, etc.
