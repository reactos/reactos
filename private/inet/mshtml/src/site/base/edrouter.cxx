                             ...
                             CDoc *          pDoc )
{
    ...
    if (_pHostCmdTarget)
    {
        ...
        if (idm == IDM_UNKNOWN)
            hr = THR_NOTRACE( _pHostCmdTarget->Exec(pguidCmdGroup, nCmdID, nCmdexecopt, pvarargIn, pvarargOut));
        else
            hr = THR_NOTRACE( _pHostCmdTarget->Exec(&CGID_MSHTML, idm, nCmdexecopt, pvarargIn, pvarargOut));
    ...
    hr = THR_NOTRACE( _pInternalCmdTarget->Exec(&CGID_MSHTML, idm, nCmdexecopt, pvarargIn, pvarargOut));
                                    ...
                                    CDoc       * pDoc )
{
    ...
    if (_pHostCmdTarget)
    {
        ...
        if (idm == IDM_UNKNOWN)
            hr = _pHostCmdTarget->QueryStatus(pguidCmdGroup, cCmds, &newCommand , pcmdtext );
        else
            hr = _pHostCmdTarget->QueryStatus(&CGID_MSHTML, cCmds, & newCommand, pcmdtext );
    ...
    if( _pInternalCmdTarget)
    {
            ...
            && newCommand.cmdID == IDM_JUSTIFYLEFT)
        {
            ...
            hr = THR_NOTRACE( _pInternalCmdTarget->QueryStatus(&CGID_MSHTML, cCmds, & newCommand, pcmdtext ));
