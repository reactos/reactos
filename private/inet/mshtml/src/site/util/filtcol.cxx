...
HRESULT CCSSFilterSite::PrivateQueryInterface(REFIID iid, LPVOID *ppv)
{
    HRESULT hr = S_OK;
...
//  Synopsis:   Returns the IHTMLElement pointer of the Element this site belongs to.
//              This causes a hard AddRef on CElement. Clients are *not* to cache this pointer.
HRESULT CCSSFilterSite::GetElement(IHTMLElement **ppElement)
{
    RRETURN(_pElem->QueryInterface(IID_IHTMLElement, (void **)ppElement));
}


//  Member:     CCSSFilterSite::FireOnFilterEvent, public
//  Synopsis:   Fires an event for the extension object
HRESULT CCSSFilterSite::FireOnFilterChangeEvent()
{
    HRESULT     hr = S_OK;
    ...
    if (pLayout && !pLayout->ElementOwner()->GetAAdisabled())
    {
        ...
        hr = _pElem->FireEvent(DISPID_EVMETH_ONFILTER, DISPID_EVPROP_ONFILTER, NULL, (BYTE *)VTS_NONE);
    }
...
//  Synopsis:   Get service from host. this delegates to the documents implementation.
HRESULT CCSSFilterSite::QueryService(REFGUID guidService, REFIID iid, void ** ppv)
{
...
//  SYNOPSIS : internal helper methid this fills the propbag and sets the contents of the CCSSFrameSite
HRESULT CCSSFilterSite::Connect(LPTSTR pNameValues)
{
...
//  SYNOPSIS : internal helper methid this fills the propbag and sets the contents ofteh CCSSFrameSite
HRESULT CCSSFilterSite::ParseFilterNameValuePair(LPTSTR pchNameValue, CPropertyBag **ppPropBag)
{
    HRESULT hr = S_OK;
    TCHAR chQuote = 0;
    ...
    while (*pchNameValue)
    {
        ...
        if (pchNameEnd != pchNameStart)
        {
            ...
            *pchNameEnd = _T('\0');
            ...
            {
                *pchNameEnd = cTemp;
                hr = THR((*ppPropBag)->AddProp(pchNameStart, pchNameEnd - pchNameStart, pchValueStart, pchValueEnd - pchValueStart));
            }
            if (hr)
                goto Cleanup;
        }
...
HRESULT CCSSFilterSite::InvokeEx(DISPID dispidMember,
                         LCID lcid,
                         WORD wFlags,
...
STDMETHODIMP CCSSFilterSite::CreateMoniker(LPOLESTR szName, IBindCtx * pbc, IMoniker ** ppmk, DWORD dwReserved)
{
...
STDMETHODIMP CCSSFilterSite::MonikerBindToStorage(IMoniker * pmk, IBindCtx * pbc, IBindStatusCallback * pbsc, REFIID riid, void ** ppvObj)
{
    RRETURN1(MonikerBind(_pElem->Doc(), pmk, pbc, pbsc, riid, ppvObj, FALSE, 0), S_ASYNCHRONOUS);
}


//  Member:     CCSSFilterSite::MonikerBindToObject, IBindHost
//  Synopsis:   Calls BindToObject on the moniker
STDMETHODIMP CCSSFilterSite::MonikerBindToObject(IMoniker * pmk, IBindCtx * pbc, IBindStatusCallback * pbsc, REFIID riid, void ** ppvObj)
{
    RRETURN1(MonikerBind(_pElem->Doc(), pmk, pbc, pbsc, riid, ppvObj, TRUE, 0), S_ASYNCHRONOUS);
}
...
HRESULT CFilterArray::ParseFilterProperty(LPCTSTR pcszFilter, CTreeNode * pNodeOwner)
{
    HRESULT hr = S_OK;
    LPTSTR pszCopy;
    LPTSTR pszString;
...
CFilterArray::CFilterArray(CElement * pOwner, LPCTSTR pText) : _pElemOwner(pOwner)
{
...
HRESULT CFilterArray::OnAmbientPropertyChange(DISPID dspID)
{
    long  lSize = _aryFilters.Size();
    long  i;
...
HRESULT CFilterArray::OnCommand(COnCommandExecParams * pParm)
{
    long  lSize = _aryFilters.Size();
    long  i;
    ...
    for (i = 0; i < lSize; i++)
    {
        if (_aryFilters[i]->GetExtension())
        {
            CTExec(_aryFilters[i]->GetExtension(), pParm->pguidCmdGroup, pParm->nCmdID, pParm->nCmdexecopt, pParm->pvarargIn, pParm->pvarargOut);
        }
    }
    return S_OK;
}
...
HRESULT CFilterArray::PrivateQueryInterface(REFIID iid, void **ppv)
{
    *ppv = NULL;
    switch (iid.Data1)
    {
    ...
    default:
        {
            ...
            if (pclassdesc && pclassdesc->_piidDispinterface && (iid == *pclassdesc->_piidDispinterface))
            {
...
HRESULT CFilterArray::get_length(long * pLength)
{
    HRESULT hr = S_OK;
...
HRESULT CFilterArray::item(VARIANT * pvarIndex, VARIANT * pvarRet)
{
    HRESULT   hr = S_OK;
    long      lIndex;
    CVariant  varArg;
...
HRESULT CFilterArray::GetItem(long lIndex, VARIANT *pvar)
{
    HRESULT hr = S_OK;
...
HRESULT CFilterArray::get__newEnum(IUnknown ** ppEnum)
{
    HRESULT hr = S_OK;
    ...
    *ppEnum = NULL;
    ...
    hr = THR(_aryFilters.EnumVARIANT(VT_BSTR, (IEnumVARIANT**)ppEnum, FALSE, FALSE));
...
//  Sysnopsis : this helper function will return the index of the filter that has the name. other wise it returns -1.
long CFilterArray::FindByName(LPCTSTR pszTargetName, BOOL fCaseSensitive /* ==true */)
{
    EnsureCollection();
...
//      internal array. it returns the index where the filter was added, or -1 if there was an error
long CFilterArray::AddFilterSite(CCSSFilterSite * pFilter)
{
    long lIdx = -1;
    long lSize = _aryFilters.Size();
...
//  Sysnopsis : this helper function will remove a filter record from the internal array
HRESULT CFilterArray::RemoveFilterSite(long lIndex)
{
    HRESULT hr = S_OK;
...
void CFilterArray::EnsureCollection(void)
{
    Assert(_pElemOwner);
...
LPCTSTR CFilterArray::GetName(long lIdx)
{
