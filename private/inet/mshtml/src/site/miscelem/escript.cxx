...
HRESULT CScriptElement::CreateElement(CHtmTag *pht, CDoc *pDoc, CElement **ppElement)
{
    HRESULT         hr = S_OK;
...
HRESULT CScriptElement::PrivateQueryInterface(REFIID iid, void ** ppv)
{
    *ppv = NULL;
...
HRESULT CScriptElement::Save(CStreamWriteBuff * pStreamWriteBuff, BOOL fEnd)
{
    HRESULT hr = S_OK;

    // Do not write out the script when saving in plaintext mode, or when
    // writing for the rtf converter.
    if (!pStreamWriteBuff->TestFlag(WBF_SAVE_PLAINTEXT) && !pStreamWriteBuff->TestFlag(WBF_FOR_RTF_CONV))
    {
        DWORD   dwOld;
...
HRESULT CScriptElement::CommitCode()
{
    ...
    if (IsTagEnabled(tagHtmSrcTest))
    {
        ...
        hrT = pDoc->GetHtmSourceText(_ulScriptOffset, ARRAY_SIZE(achSrc) - 1, achSrc, &cch);
        if (hrT == S_OK)
        {
            cch = min(cch, (ULONG)_tcslen(STRVAL(pchSource)));
            achSrc[cch] = 0;

            TraceTag((tagHtmSrcTest, "Expect \"%.64ls\", (ln=%ld pos=%ld) \"%.64ls\"", pchSource, _ulScriptLine, _ulScriptOffset, achSrc));
        }
    }
#endif
...
#define MAX_SCRIPT_INCLUDES 0 // feature disabled - see CLIENT_SIDE_INCLUDES
    ...
    // a scriptlet.  Otherwise, just parse it in as script text as long as we're not an event tag.
...
HRESULT CScriptElement::EnsureScriptDownloadLeft()
{
    HRESULT     hr = S_OK;
...
HRESULT CScriptElement::EnsureSourceObjectRevoked()
{
    HRESULT     hr = S_OK;
    CDoc *      pDoc = Doc();
...
HRESULT TokenizeEventDescription(TCHAR * pchEvent, TCHAR ** ppchEventName, TCHAR ** ppchFormalParams)
{
    HRESULT     hr = S_OK;
    TCHAR *     pchStart;
    TCHAR *     pchEnd;
...
HRESULT CScriptElement::CommitFunctionPointersCode(CBase *pelTarget, BOOL fHookup, BOOL fMarkupDestroy /*=FALSE*/)
{
    HRESULT                 hr = S_OK;
    TCHAR *                 pchEvent;
    TCHAR *                 pchFor;
    ...
    if (pchEvent && *pchEvent && pchFor && *pchFor && pchSource && *pchSource)
    {
        ...
        for (iObject = 0; iObject < cObjects; iObject++)
        {
            ...
            if (pObject->BaseDesc()->_dwFlags & CElement::ELEMENTDESC_OLESITE)
            {
                ...
                if (pTIEvent)
                {
                    hr = THR_NOTRACE(pTIEvent->GetIDsOfNames(&_pchEventName, 1, &dispid));
                }
            }
            ...
            if (hr)
            {
                ...
                if (hr || IsExpandoDispid(dispid))
                {
                    if (!pNonElementPropHost)
                    {
                        ...
                        {
                            hr = THR_NOTRACE(pElement->GetPeerHolder()->GetEventDispidMulti(_pchEventName, &dispid));
                        }
                    }
                    else
                        hr = E_FAIL;
...
void CScriptElement::OnDwnChan(CDwnChan * pDwnChan)
{
    HRESULT         hr;
    CDoc *          pDoc = Doc();
    ULONG           ulState;
...
void CScriptElement::Notify(CNotification *pNF)
{
    HRESULT     hr = S_OK;
    TCHAR      *pchEvent;
    TCHAR      *pchFor;
...
HRESULT CScriptElement::DownLoadScript()
{
    HRESULT         hr = S_OK;
    CDoc *          pDoc = Doc();
...
HRESULT CScriptElement::Execute()
{
    HRESULT         hr = S_OK;
    HRESULT         hr2;
    ...
    if (VB_FALSE == vbDefer || !ParserWillExecute())
    {
        ...
        // the paste tree to object model before it's been spliced into the main tree.
...
//  Returns:    void
void CScriptElement::SetReadyStateScript(long readyStateScript)
{
    long readyState;
...
HRESULT CScriptElement::get_readyState(BSTR * p)
{
    HRESULT hr = S_OK;
...
HRESULT CScriptElement::get_readyState(VARIANT * pVarRes)
{
    HRESULT hr = S_OK;
