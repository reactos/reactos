...
HRESULT CFormElement::CreateElement(CHtmTag *pht, CDoc *pDoc, CElement **ppElement)
{
...
HRESULT CFormElement::Init2(CInit2Context * pContext)
{
    HRESULT hr = S_OK;
    CDoc *  pDoc = Doc();
...
HRESULT CFormElement::PrivateQueryInterface(REFIID iid, void ** ppv)
{
    *ppv = NULL;
    switch (iid.Data1)
    {
...
HRESULT CFormElement::get_elements(IDispatch ** ppElemCol)
{
    ...
    *ppElemCol = NULL;
    ...
    hr = THR_NOTRACE(QueryInterface(IID_IDispatch, (void**) ppElemCol));
    if (hr)
        goto Cleanup;
...
HRESULT CFormElement::EnsureCollectionCache()
{
    ...
    {
        _pCollectionCache = new CCollectionCache(this, GetDocPtr(), ENSURE_METHOD(CFormElement, EnsureCollections, ensurecollections));
        if (!_pCollectionCache)
        {
            hr = E_OUTOFMEMORY;
            goto Cleanup;
        ...
        _pCollectionCache->SetDISPIDRange(FORM_ELEMENT_COLLECTION, DISPID_FORM_ELEMENT_GN_MIN, DISPID_FORM_ELEMENT_GN_MAX);
        ...
        // Set the DISPID range for the second collection, so we can distinguish between the two in Invoke
        _pCollectionCache->SetDISPIDRange(FORM_NAMED_IMG_COLLECTION, DISPID_FORM_NAMED_IMG_GN_MIN, DISPID_FORM_NAMED_IMG_GN_MAX);
    }
...
HRESULT CFormElement::EnsureCollections(long lIndex, long * plCollectionVersion)
{
    ...
    for (++l; l < lSize; ++l)
    {
        ...
        case ETAG_IMG:
            ...
            {
                hr = THR(_pCollectionCache->SetIntoAry(FORM_NAMED_IMG_COLLECTION, pElemCandidate));
                if (hr)
                    goto Error;
            }
            break;
        case ETAG_INPUT:
            ...
            if (DYNCAST(CInput, pNodeCandidate->Element())->GetAAtype() == htmlInputImage)
            {
                if (fAddToSubmit)
                {
                    hr = THR(_pCollectionCache->SetIntoAry(FORM_SUBMIT_COLLECTION, pElemCandidate));
                }
                break;
            }
            hr = S_OK;
        ...
        case ETAG_EMBED:
            if (fAddToElements)
            {
                hr = THR(_pCollectionCache->SetIntoAry(FORM_ELEMENT_COLLECTION, pElemCandidate));
                if (hr)
                    goto Error;
            }
            if (fAddToSubmit)
...
//              format: n=v[&n=v[...]]\0
HRESULT CFormElement::CallGetSubmitInfo(
    ...
    CODEPAGE   cp /*= NULL*/)
{
    ...
    *ppchAction = GetAAaction();
    ...
    if (pSubmitData->_encType == htmlEncodingText && !*pfSendAsPost)
    {
        ...
        if (URL_SCHEME_HTTP == uProt || URL_SCHEME_HTTPS == uProt)
        {
            *pfSendAsPost = TRUE;
        }
    }
    ...
    for (i = 0; i < cElements; i++)
    {
        ...
        hr = THR(_pCollectionCache->GetIntoAry(FORM_SUBMIT_COLLECTION, i, &pElement));
        if (hr)
            goto Cleanup;
        ...
        case ETAG_TEXTAREA:
            ...
            {
                (*pnFieldsChanged)++;
                *pnMultiLines += DYNCAST(CRichtext, pElement)->Layout()->GetDisplay()->LineCount() - 1;
            }
            break;
        }
    }
...
//              Then constructs the submit string and uses the
//              hyperlink apis to submit
HRESULT CFormElement::DoSubmit(CElement *pSubmitSite, BOOL fFireEvent)
{
    HRESULT     hr = S_OK;
    ...
    */
    ...
    if (SubmitData._fCharsetNotDefault)
    {
        ...
        hr = THR(CallGetSubmitInfo(pSubmitSite, &pInputImg, &SubmitData, &nMultiLines, &nFieldsChanged, &pchAction, &fSendAsPost, fUseUtf8, cp));
        if (hr)
            goto Cleanup;
    }
...
BOOL CFormElement::Utf8InAcceptCharset()
{
    BOOL    fRetVal = FALSE;
...
//              enumerates all contained sites
//              and calls the reset API on the site
HRESULT CFormElement::DoReset(BOOL fFireEvent)
{
    ...
    while (c--)
    {
        ...
        hr = THR(_pCollectionCache->GetIntoAry(FORM_ELEMENT_COLLECTION, c, &pElem));
...
HRESULT CFormElement::ContextThunk_InvokeEx(
    DISPID          dispidMember,
    LCID            lcid,
    WORD            wFlags,
    ...
    IServiceProvider *pSrvProvider)
{
    ...
    if (dispidMember >= DISPID_COLLECTION_TI_MIN && dispidMember <= DISPID_COLLECTION_TI_MAX)
    {
...
HRESULT CFormElement::GetDispID(BSTR bstrName, DWORD grfdex, DISPID *pid)
{
    ...
    if (IsInMarkup())
    {
        ...
        hr = THR_NOTRACE(_pCollectionCache->GetDispID(FORM_ELEMENT_COLLECTION, bstrName, grfdex, pid));
        ...
        if (hr == DISP_E_UNKNOWNNAME)
        {
            ...
            if (hr)
                goto Cleanup;
            hr = THR_NOTRACE(_pCollectionCache->GetDispID(FORM_NAMED_IMG_COLLECTION, bstrName, grfdex, pid));
        }
    }
...
HRESULT CFormElement::GetNextDispID(DWORD grfdex, DISPID id, DISPID *prgid)
{
    HRESULT     hr;
...
HRESULT CFormElement::GetMemberName(DISPID id, BSTR *pbstrName)
{
    HRESULT     hr;
...
HRESULT CFormElement::GetMultiTypeInfoCount(ULONG *pc)
{
    ...
    IID*        piidSource)
{
    ...
    if (dwFlags & MULTICLASSINFO_GETTYPEINFO)
    {
        ...
        {
            hr = THR(Doc()->BuildObjectTypeInfo(_pCollectionCache, lIndex, dispidMin, dispidMax, ppTypeInfo, ppTypeInfoCoClass));
            if (hr)
                goto Cleanup;
        }
...
Dosuper:
    hr = THR(super::GetInfoOfIndex(iTI, dwFlags, ppTICoClass, pdwTIFlags, pcdispidReserved, piidPrimary, piidSource));
    if (hr)
        goto Cleanup;
...
HRESULT CFormElement::FOnlyTextbox(CInput * pTextbox, BOOL * pfOnly)
{
    HRESULT     hr = S_OK;
    long        i;
    CElement *  pElem;
    ...
    *pfOnly = TRUE;
    ...
    for (i = _pCollectionCache->SizeAry(FORM_ELEMENT_COLLECTION) - 1; i >= 0 && *pfOnly; i--)
    {
        hr = THR(_pCollectionCache->GetIntoAry(FORM_ELEMENT_COLLECTION, i, &pElem));
        if (hr)
            goto Cleanup;
...
HRESULT CFormElement::FormTraverseGroup(
