                    ...
                    CAdaptingProvider *pProvider)
{
    ...
    if (!_fCompleteFired)
    {
                                             ...
                                             (void **)&pAsynchStatus))
        {
            ...
            pAsynchStatus->GetStatus(_pProvider->GetChapter(), DBASYNCHOP_OPEN, &ulProgress, &ulProgressMax, &ulStatusCode, NULL);
...
CAdaptingProvider::AdaptRowsetOnSTD(IRowset** ppRowset, OLEDBSimpleProvider *pOSP)
{
    ...
    hr = CoCreateInstance(clsidPW, NULL, CLSCTX_ALL, IID_IDBInitialize, (void **)&pIDBInitialize);
    if (hr)
        goto Cleanup;
...
CAdaptingProvider::AdaptCursorOnRowset(ICursor** ppCursor, IRowset *pRowset)
{
    ...
    // create an adapter
    hr = CoCreateInstance(CLSID_CCursorFromRowset, NULL, CLSCTX_INPROC, IID_ICursorFromRowset, (void **)&pIcfr);
    if (hr)
        goto Cleanup;
...
CAdaptingProvider::AdaptCursorOnRowPosition(ICursor** ppCursor, IRowPosition *pRowPos)
{
    ...
    // create an adapter
    hr = CoCreateInstance(CLSID_CCursorFromRowset, NULL, CLSCTX_INPROC, IID_ICursorFromRowPosition, (void **)&pIcfr);
    if (hr)
        goto Cleanup;
                  ...
                  CAdaptingProvider *pProvider)
{
    ...
    // create an adapter
    hr = CoCreateInstance(CLSID_CADORecordset, NULL, CLSCTX_INPROC, IID_IADORecordset15, (void **)&pADO);
    if (hr)
        goto Cleanup;
...
CAdaptingProvider::AdaptRowPositionOnRowset(IRowPosition** ppRowPos, IRowset *pRowset)
{
    ...
    // create an adapter
    hr = CoCreateInstance(CLSID_CRowPosition, NULL, CLSCTX_INPROC, IID_IRowPosition, (void **)&pRowPos);
    if (hr)
    {
#ifndef UNIX
...
//    IRowsetFromCursor *pIrfc = 0;
    ...
    // create an adapter
    hr = CoCreateInstance(CLSID_CRowsetFromCursor, NULL, CLSCTX_INPROC, IID_IRowsetFromCursor, (void **)&pIrfc);
    if (hr)
        goto Cleanup;
...
void CDataSourceProvider::Detach()
// Member:      Detach (CDataSourceProvider, public)
...
HRESULT CDataSourceProvider::AdviseDataProviderEvents(CDataSourceBinder *pdsb)
...
HRESULT CDataSourceProvider::UnadviseDataProviderEvents(CDataSourceBinder *pdsb)
...
HRESULT CDataSourceProvider::ReplaceProvider(CDataSourceProvider *pdspNewProvider)
...
HRESULT CDataSourceProvider::LoadBoundElementCollection(CCollectionCache *pCollectionCache, long lIndex)
