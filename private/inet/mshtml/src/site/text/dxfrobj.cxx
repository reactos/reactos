...
void RegisterFETCs()
{
    ...
    if (!g_rgFETC[iHTML].cfFormat)
        g_rgFETC[iHTML].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA("HTML Format");

    if (!g_rgFETC[iRtfFETC].cfFormat)
        g_rgFETC[iRtfFETC].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA("Rich Text Format");

    if (!g_rgFETC[iRtfAsTextFETC].cfFormat)
        g_rgFETC[iRtfAsTextFETC].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA("RTF As Text");

    if (!g_rgFETC[iFileDescA].cfFormat)
        g_rgFETC[iFileDescA].cfFormat = (CLIPFORMAT)RegisterClipboardFormat(CFSTR_FILEDESCRIPTORA);

    if (!g_rgFETC[iFileDescW].cfFormat)
        g_rgFETC[iFileDescW].cfFormat = (CLIPFORMAT)RegisterClipboardFormat(CFSTR_FILEDESCRIPTORW);

    if (!g_rgFETC[iFileContents].cfFormat)
        g_rgFETC[iFileContents].cfFormat = (CLIPFORMAT)RegisterClipboardFormat(CFSTR_FILECONTENTS);

    if (!g_rgFETC[iUniformResourceLocator].cfFormat)
        g_rgFETC[iUniformResourceLocator].cfFormat = (CLIPFORMAT)RegisterClipboardFormat(CFSTR_SHELLURL);

    //    if(!g_rgFETC[iRichEdit].cfFormat)
    //        g_rgFETC[iRichEdit].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA("RICHEDIT");

    //    if(!g_rgFETC[iObtDesc].cfFormat)
    //        g_rgFETC[iObtDesc].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA(CF_OBJECTDESCRIPTOR);

    //    if(!g_rgFETC[iEmbObj].cfFormat)
    //        g_rgFETC[iEmbObj].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA(CF_EMBEDDEDOBJECT);

    //    if(!g_rgFETC[iEmbSrc].cfFormat)
    //        g_rgFETC[iEmbSrc].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA(CF_EMBEDSOURCE);

    //    if(!g_rgFETC[iLnkSrc].cfFormat)
    //        g_rgFETC[iLnkSrc].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA(CF_LINKSOURCE);

    //    if(!g_rgFETC[iRtfNoObjs].cfFormat)
    //        g_rgFETC[iRtfNoObjs].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA("Rich Text Format Without Objects");

    //    if(!g_rgFETC[iTxtObj].cfFormat)
    //        g_rgFETC[iTxtObj].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA("RichEdit Text and Objects");

    //    if(!g_rgFETC[iFilename].cfFormat)
    //        g_rgFETC[iFilename].cfFormat = (CLIPFORMAT)RegisterClipboardFormatA(CF_FILENAME);
}
...
HRESULT CTextXBag::Create(CMarkup *             pMarkup,
                  BOOL                  fSupportsHTML,
                         ...
                         int            iFETCIndex)
{
    ...
    {
        hr = THR(GetHTMLText(&hText, pSegmentList, pMarkup, dwSaveHtmlFlags, cp, dwStmWrBuffFlags));
        if (hr)
            goto Error;
    }
                          ...
                          ISegmentList * pSegmentList)
{
    ...
    RRETURN(SetTextHelper(pMarkup, pSegmentList, RSF_SELECTION, CP_UCS_2, dwFlags, &_hUnicodeText, iUnicodeFETC));
}
#endif // !WIN16
