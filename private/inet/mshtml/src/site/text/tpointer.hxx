...
public:
    ...
    // Get the embedded treepos.  Be careful, pointers are not always embedded.
    ...
    // GetNormalizedReference returns a ptp/ich pair which is immediately after REAL content.
    // Not real content is pointers and empty text runs.
...
private:
    ...
    // The _pMarkup member points the markup I'm positioned in, when I'm positioned.
    // If it is NULL, then I'm not positioned in any markup, and thus the members which indicate where are unused.
    CMarkup * _pMarkup;

    // Each markup has a list of markup pointers which are in the markup, but do not have a pointer pos.
    ...
    // gravity and cling, even when we are embedded, when the pointer pos has redundant indicators of this.
    // They are redundant because we want gravity and cling with out CMarkupPointer.

    // The _fEmbedded member is used when _pMarkup is non-NULL to indicate
    // if our position is indicated by an embedded pointer pos or a reference to a non-pointer pos / offset pair.
    ...
    // The _cpCache member records the cp this pointer is currently at.
    // If the contents version of the markup matches that stored here, then _cpCache is up to date.
...
inline BOOL CMarkupPointer::CpIsCached ( ) const
{
    return Markup() ? _verCp == Markup()->GetMarkupContentsVersion() : FALSE;
}

inline long CMarkupPointer::GetCp ( )
{
    if (!IsPositioned())
        return -1;
