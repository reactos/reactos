...
void TxDivideInsertion(DWORD cch, DWORD ichBlock, DWORD cchAfter, DWORD *pcchFirst, DWORD *pcchLast);
...
NODE_CLASS ClassifyNodePos( CTreePos *ptp, BOOL * pfBegin )
{
    ...
    if( ptp->IsEdgeScope() )
    {
        ...
        if( pFL && pFL->IsElementBlockInContext( pElement ) )
        {
            ...
            else if(    etag == ETAG_TD || etag == ETAG_TH || etag == ETAG_TC )
            {
                ncClass = NODECLASS_SITEBREAK;
            }
            else
 ...
 */
static NODE_CLASS Classify( CTxtPtr * ptp, BOOL * pfBegin )
{
...
{
    return    _T(' ')  == ch || _T('\t') == ch || WCH_NBSP == ch;
}
...
BOOL IsWhiteSpace(CTxtPtr *ptp)
{
    ...
    return (    ch == L' ' || InRange( ch, TAB, CR ) || (ch | 1) == PS || (    ch == WCH_NODE && Classify( ptp, NULL ) ) );
}
 ...
 */
CTxtPtr::CTxtPtr (const CTxtPtr &tp)
{
 ...
 */
DWORD CTxtPtr::BindToCp ( DWORD cp )
{
    if(_prgRun)
    {
...
HRESULT CTxtPtr::MoveToBookmark( BSTR bstrBookmark, CTxtPtr *pTxtPtrEnd )
{
    ...
    // Look for bookends
    SetCp( bm.left.cp );
    fFoundIt =    FindBookend( this, bm.left, FALSE ) || FindBookend( this, bm.left, TRUE );

    if( !bm.chDegenerate )
    {
        pTxtPtrEnd->SetCp( bm.right.cp );
        fFoundIt = fFoundIt && (    FindBookend( pTxtPtrEnd, bm.right, FALSE ) || FindBookend( pTxtPtrEnd, bm.right, TRUE ) );
    }
    else
    {
...
static void CountNasties (CTxtPtr * pTxtPtr, long cp, NastyCharsCounts & nasties )
{
    long cchLeft;
