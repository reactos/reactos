    ...
    CONST INT *lpDx)            //@parm Ptr to array of intercharacter spacing values
{
    ...
    while (lpwchT < lpwchEnd)
    {
        ...
        if (*lpwchT > 0x007F)
        {
            if ((cch = lpwchT - lpwchStart) > 0)
            {
                ...
                fRet = ::ExtTextOutW( hdc, xp, yp, fuOptions, lprect, lpwchStart, cch, (int *) lpdxpCur);
                if (!fRet)
                    return fRet;
            ...
            fRet = ::ExtTextOutW(hdc, xp, yp, fuOptions, lprect, lpwchStart, 1, (int *)lpdxpCur);
            if (!fRet)
                return fRet;
    ...
    {
        fRet = ::ExtTextOutW( hdc, xp, yp, fuOptions, lprect, lpwchStart, cch, (int *) (lpDx ? lpDx + (lpwchStart - lpwchString) : NULL) );
    }
    ...
    for (;pch < pchStop; pch++)
    {
        ...
        else
        {
            if (pchStart)
            {
                ...
                const int cb = WideCharToMultiByte( uiCodePage, dwFlags, pchStart, pch - pchStart, pb, pbStop - pb, NULL, NULL );
                pb += cb;
                pchStart = NULL;
            }
        ...
        while (cch > 0)
        {
            ...
            else
            {
                ...
                if (!fWhiteSpaceSkip)
                {
                    ...
                    Assert(   ptTemp.x == xCur || (   VER_PLATFORM_WIN32_WINDOWS == g_dwPlatformID && xCur >= 0x8000));
