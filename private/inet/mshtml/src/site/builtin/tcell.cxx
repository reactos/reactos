...
#endif // NO_PROPERTY_PAGE
        s_acpi,                         // _pcpi
        ELEMENTDESC_NEVERSCROLL | ELEMENTDESC_TEXTSITE | ELEMENTDESC_NOOFFSETCTX | ELEMENTDESC_NOBKGRDRECALC | ELEMENTDESC_TABLECELL, // _dwFlags
...
HRESULT CTableCell::CreateElement(CHtmTag *pht, CDoc *pDoc, CElement **ppElement)
{
    Assert(ppElement);
...
HRESULT CTableCell::OnPropertyChange(DISPID dispid, DWORD dwFlags)
{
    HRESULT hr;
...
HRESULT CTableCell::Save(CStreamWriteBuff *pStmWrBuff, BOOL fEnd)
{
    HRESULT hr;
...
//  Returns:    rows
int CTableCell::RowSpanHelper(int cRowSpan)
{
#if DBG==1
...
//  Returns:    HRESULT
HRESULT CTableCell::ApplyCellFormat(CFormatInfo *pCFI)
{
    ...
    if (!fComputeFFOnly)
    {
        if (Tag() == ETAG_CAPTION || Tag() == ETAG_TC)
        {
            ...
            if (pCFI->_ppf->_bTableVAlignment == htmlCaptionVAlignTop || pCFI->_ppf->_bTableVAlignment == htmlCaptionAlignTop)
            {
                pCFI->PrepareParaFormat();
            ...
            else if (pCFI->_ppf->_bTableVAlignment == htmlCaptionVAlignBottom || pCFI->_ppf->_bTableVAlignment == htmlCaptionAlignBottom)
            {
                pCFI->PrepareParaFormat();
...
BOOL CTableCell::CheckSameFormat(
...
)
{
    ...
    if (pNodeCell1->_iPF >= 0 &&
        (_pAA == NULL && pCell1->_pAA == NULL || (_pAA != NULL && pCell1->_pAA != NULL && _pAA->Compare(pCell1->_pAA))) &&
        ...
        (pCol && pCol1 && ((pNodeCol->_iPF >= 0 && pCol == pCol1) ||
          (pNodeCol1->_iPF >= 0 && pNodeCol->_iPF == pNodeCol1->_iPF && pNodeCol->_iCF == pNodeCol1->_iCF && pNodeCol->_iFF == pNodeCol1->_iFF)))))
    {
        return TRUE;
    }
    else
...
HRESULT CTableCell::ApplyFormatUptoTable(CFormatInfo * pCFI)
{
...
HRESULT CTableCell::ComputeFormats(CFormatInfo * pCFI, CTreeNode * pNodeTarget)
{
    ...
    if (pNodeInherit->_iCF == -1 || pNodeInherit->_iFF == -1)
    {
    ...
    else if (!pTableLayout->_fCollapse)
    {
        ...
        if (uvPadding.GetPixelValue() || uvSpacing.GetPixelValue() || uvBorder.GetPixelValue())
        {
            pCFI->PrepareFancyFormat();
            ...
            pCFI->_ff()._cuvPaddingLeft = pCFI->_ff()._cuvPaddingRight = uv;
            ...
            if (valign == htmlCaptionVAlignNotSet)
            {
                ...
                valign = (valign == htmlCaptionAlignBottom ? htmlCaptionVAlignBottom : htmlCaptionVAlignTop);
            }
    ...
    if (etag != ETAG_CAPTION && etag != ETAG_TC)
    {
        ...
        if (!fComputeFFOnly)
        {
            ...
            if (pCFI->_bBlockAlign == htmlBlockAlignNotSet && etag == ETAG_TH)
            {
