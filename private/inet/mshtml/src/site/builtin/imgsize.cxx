...
//  Synopsis:   Open the image size cache file.  Create and initialize it if required.
...
//    MessageBox(NULL, "Need to implement InitImageSizeCache", "BUGWIN16", MB_OK);
    ...
    else
    {
        ...
        __try
        {
            if (fInitialize || pCacheFile->dwMagic != MAGIC || pCacheFile->dwVersion != VERSION)
            {
...
void SetCachedImageSize(LPCTSTR pchURL, SIZE size)
{
    ...
    // The following chunk of code can confuse GetCachedImageSize because the cache memory is shared by multiple processes.
    // We don't bother with the expense of a mutex because we can tolerate fetching a bad image size.
