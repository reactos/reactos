...
HRESULT COptionElement::CreateElement(CHtmTag *pht, CDoc *pDoc, CElement **ppElementResult)
{
...
void COptionElement::Notify(CNotification *pNF)
{
...
HRESULT COptionElement::Save(CStreamWriteBuff * pStreamWrBuff, BOOL fEnd)
{
...
HRESULT COptionElement::Init2(CInit2Context * pContext)
{
    HRESULT hr;
...
HRESULT COptionElement::get_index(long * plIndex)
{
    if (!plIndex)
...
HRESULT COptionElement::put_index(long lIndex)
{
    return S_OK;
}
...
HRESULT COptionElement::get_form(IHTMLFormElement **ppDispForm)
{
    HRESULT          hr = S_OK;
    CFormElement   * pForm;
...
//  Synopsis:   Gets the index of the option object
HRESULT COptionElement::get_text(BSTR * pbstrText)
{
    HRESULT hr = S_OK;
    ...
    *pbstrText = NULL;
    ...
    if (_fInCollection && NULL != (pSelect = GetParentSelect()) && pSelect->_hwnd)
    {
        CStr cstrText;
        long cchText;
        long lIndex;
        ...
        cchText = pSelect->SendSelectMessage(CSelectElement::Select_GetText, lIndex, (LPARAM)(LPTSTR)cstrText);
        if (cchText == LB_ERR)
            goto Win32Error;
...
HRESULT COptionElement::put_text(BSTR bstrText)
{
    HRESULT hr;
...
//  Synopsis:   Gets the index of the option object
HRESULT COptionElement::put_defaultSelected(VARIANT_BOOL f)
{
...
//  Synopsis:   Gets the index of the option object
HRESULT COptionElement::get_defaultSelected(VARIANT_BOOL * pf)
{
    if (!pf)
...
HRESULT COptionElement::GetSelectedHelper(long * pf)
{
    HRESULT hr = S_OK;
...
//  Synopsis:   Gets the selected state of the option object
HRESULT COptionElement::SetSelectedHelper(long f)
{
    HRESULT hr = S_OK;
    CSelectElement *pSelect;
    long lIndex;
...
void COptionElement::GetDisplayColors(COLORREF * pcrFore, COLORREF * pcrBack, BOOL fListbox)
{
    CColorValue ccv;
    CSelectElement * pSelect;
    ...
    if ((NULL != (pSelect = GetParentSelect())) && pSelect->GetAAdisabled())
    {
...
CStr * COptionElement::GetDisplayText(CStr * pcstrBuf)
{
    BYTE bTextTransform = GetFirstBranch()->GetCascadedtextTransform();

    Assert(pcstrBuf);

    if ((bTextTransform == styleTextTransformNotSet) || (bTextTransform == styleTextTransformNone))
    {
        return &_cstrText;
    }
    else
    {
        TransformText(*pcstrBuf, _cstrText, _cstrText.Length(), bTextTransform);
        return pcstrBuf;
    }
}
...
//  Synopsis : Do any work that Options need when a property is changed
HRESULT COptionElement::OnPropertyChange(DISPID dispid, DWORD dwFlags)
{
    HRESULT hr = S_OK;
    CSelectElement *pSelect;
    ...
    if (NULL != (pSelect = GetParentSelect()))
    {
        ...
        if (dispid == DISPID_COptionElement_text || dispid == DISPID_UNKNOWN)
        {
            pSelect->DeferUpdateWidth();
        }
...
long COptionElement::MeasureLine(CCalcInfo * pci)
{
    CStr cstrBuffer;
...
HRESULT COptionElement::CacheText(void)
{
    HRESULT hr;
...
HRESULT STDMETHODCALLTYPE COptionElementFactory::create(VARIANT varText,
                                                        VARIANT varvalue,
