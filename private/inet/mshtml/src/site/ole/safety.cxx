...
GetCategoryManager(ICatInformation **ppCat)
{
    ...
    if (!TLS(pCatInfo))
    {
        ...
        hr = THR(CoCreateInstance(CLSID_StdComponentCategoriesMgr, NULL, CLSCTX_INPROC_SERVER, IID_ICatInformation, (void **) &pCatInfo));
...
COleSite::IsSafeToInitialize(REFIID riid)
{
    ...
    if (!fSafe)
    {
        ...
        if (pici)
        {
            fSafe = IsSafeTo(SAFETY_INIT, riid, pici->clsid, _pUnkCtrl, pDoc);
        }
    }
    ...
    CDoc *          pDoc)       // if NULL, we don't confirm with user
{
    ...
    if (posafe)
    {
        ...
        if (sOperation == SAFETY_SCRIPTENGINE)
        {
            ...
            hr = THR(posafe->GetInterfaceSafetyOptions(riid, &dwMask, &dwEnabled));
...
EnsureSafeForScripting:
    ...
    {
        IGNORE_HR(pDoc->ProcessURLAction(URLACTION_SCRIPT_SAFE_ACTIVEX, &fSafe));
    }
    else
    {
        fSafe = TRUE;
...
Confirm:
    if (pDoc) {
        ...
        {
            IGNORE_HR(pDoc->ProcessURLAction(URLACTION_ACTIVEX_CONFIRM_NOOBJECTSAFETY, &fSafe));
        }
    }
    goto Cleanup;
