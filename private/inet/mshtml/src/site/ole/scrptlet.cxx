...
CBase * STDMETHODCALLTYPE CreateScriptlet(IUnknown *pUnkOuter)
...
CScriptlet::CScriptlet(IUnknown *pUnkOuter) : _aryDR(Mt(CScriptlet_aryDR_pv))
{
...
HRESULT CScriptlet::DoVerb(LONG iVerb, LPMSG lpmsg, IOleClientSite* pActiveSite, LONG lindex, HWND hwndParent, LPCRECT lprcPosRect)
{
    ...
    if (iVerb == OLEIVERB_INPLACEACTIVATE) {
        ...
        // called with an lprcPosRect, but we don't care, at least you will have the default size.
        if (!fExtentSet) {
...
HRESULT CScriptlet::PrivateQueryInterface(REFIID iid, void **ppv)
{
    *ppv = NULL;
...
STDMETHODIMP CScriptlet::GetDispID(BSTR bstrName, DWORD grfdex, DISPID *pid)
{
    HRESULT                 hr;
    long                    lIndex;
    DispidRecord            dr;
    ...
    // Check to see if the Names are implemented directly by the Scriptlet. If so, intercept them immediately.
    ...
    *pid = DISPID_UNKNOWN;
    ...
    if (!_pDescription) {
        ...
        if (dr.dispid_get == DISPID_UNKNOWN && dr.dispid_put == DISPID_UNKNOWN) {
...
STDMETHODIMP CScriptlet::InvokeEx(DISPID dispid, LCID lcid, WORD wFlags, DISPPARAMS* pdispparams, VARIANT* pvarResult, EXCEPINFO* pexcepinfo, IServiceProvider *pSrvProvider)
{
    HRESULT         hr;
    DispidRecord *  pDR;
    ...
    if (dispid < _dispidCur) {
        ...
        if (wFlags & DISPATCH_PROPERTYGET) {
            ...
            if (dispid == DISPID_UNKNOWN)
                ...
                // public_put_foo, as it thinks it really belongs to foo (which is how it was called from script)
...
BOOL CScriptlet::PassThruDISPID(DISPID dispid)
{
    switch (dispid) {
...
STDMETHODIMP CScriptlet::get_Scrollbar(VARIANT_BOOL *pfShow)
{
    *pfShow = _vbScrollbar;
    return S_OK;
}
...
STDMETHODIMP CScriptlet::put_Scrollbar(VARIANT_BOOL fShow)
{
...
STDMETHODIMP CScriptlet::get_embed(VARIANT_BOOL * pfEmbedded)
{
...
STDMETHODIMP CScriptlet::put_embed(VARIANT_BOOL fEmbed)
{
    if (!InDesignMode())
        return DISP_E_MEMBERNOTFOUND;
...
STDMETHODIMP CScriptlet::get_readyState(long * pReadyState)
{
...
STDMETHODIMP CScriptlet::get_event(IDispatch **ppEvent)
{
    HRESULT          hr;
...
STDMETHODIMP CScriptlet::AboutBox()
{
    HRESULT hr;
    HWND    hwnd;
    RECT    rcPos;
    ...
    if ((unsigned)fNewVisible != _fIsVisible) {
        ...
        if (V_VT(&_varOnVisChange) == VT_DISPATCH) {
            ...
            if (SUCCEEDED(V_DISPATCH(&_varOnVisChange)->QueryInterface(
                ...
                hr = pDispEx->InvokeEx(DISPID_VALUE, LOCALE_USER_DEFAULT, DISPATCH_METHOD, &dp, NULL, NULL, NULL);
            } else {
                hr = V_DISPATCH(&_varOnVisChange)->Invoke(DISPID_VALUE, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_METHOD, &dp, NULL, NULL, NULL);
            }
        }
    }
...
HRESULT Property_get(IDispatch * pDisp, DISPID dispid, VARIANT * pvar)
{
    ...
    return pDisp->Invoke(dispid, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_METHOD | DISPATCH_PROPERTYGET, &dispparams, pvar, NULL, NULL);
}
...
STDMETHODIMP CScriptlet::Load(LPPROPERTYBAG pPropBag, LPERRORLOG pErrorLog)
{
    ...
    if (!_fHardWiredURL) {
        ...
        if (hr == E_INVALIDARG) {
            // No URL property was saved in the property bag. We force the update of the scrollbar and go home.
...
HRESULT CSortedAtomTable::Insert(LPCTSTR pch, LONG lInsertAt, LONG *plIndex)
{
    HRESULT hr = S_OK;
    WORD    wIndex;
    long    lIndex;
