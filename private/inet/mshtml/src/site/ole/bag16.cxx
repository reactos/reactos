...
HRESULT CObjectElement::SaveWin16AppletProps(IPropertyBag* pBag)
{
    ...
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, &aJavaKey[0], 0 , NULL , &hKey))
    {
        ...
        if (ERROR_SUCCESS != RegQueryValueEx(hKey, "ClassPath" , NULL, NULL , (LPBYTE)&aClassPath[0], &dwData))
        {
            goto FAILED;
        }

        dwData = sizeof(aDefaultOptions);
        if (ERROR_SUCCESS != RegQueryValueEx(hKey, "DefaultOptions", NULL, NULL, (LPBYTE)&aDefaultOptions[0], &dwData))
        {
    ...
    if (aProxyString[0])
    {
        ...
        {
            if (GetProxyString((LPSTR)aProtocol[i] , aProxyString, aProxyBuffer, sizeof(aProxyBuffer)))
            {
                var.vt = VT_BSTR;
