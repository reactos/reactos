...
STDMETHODIMP CProgressBindStatusCallback::QueryService(REFGUID guidService, REFIID iid, void ** ppv)
{
    ...
    *ppv = NULL;
    ...
    if (_pBSCChain)
    {
        hr = _pBSCChain->QueryInterface(IID_IServiceProvider, (void **)&pServiceProvider);
        if (S_OK == hr)
        {
...
STDMETHODIMP CProgressBindStatusCallback::OnStartBinding(DWORD dwReserved, IBinding *pBinding)
{
    ReplaceInterface(&_pBinding, pBinding);

    IGNORE_HR(pBinding->SetPriority(THREAD_PRIORITY_BELOW_NORMAL));

    RRETURN(_pBSCChain ? THR(_pBSCChain->OnStartBinding(dwReserved, pBinding)) : S_OK);
}
