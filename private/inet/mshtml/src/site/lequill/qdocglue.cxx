...
CQDocGlue::Init()
{
    ...
    HKEY hkey = NULL;
    if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"software\\debug\\quill", 0, KEY_READ, &hkey))
    {
        long value = 0;
        DWORD type, size;
        size = sizeof(long);

        if (ERROR_SUCCESS == RegQueryValueEx(hkey, L"Layout", 0, &type, (LPBYTE) &value, &size))
        {
            if (value)
...
BOOL CQDocGlue::FExternalLayout()
{
