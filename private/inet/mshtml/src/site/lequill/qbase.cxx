...
MtDefine(CBaseFireEventAry_pv, Locals, "CBase::FireEvent stack array")
MtDefine(CBaseFireTypeAry_pv, Locals, "CBase::FireType stack array")
MtDefine(CBaseFirePropertyNotify_pv, Locals, "CBase::FirePropertyNotify stack array")
...
BEGIN_TEAROFF_TABLE(CBase, IDispatchEx)
    ...
    TEAROFF_METHOD(CBase, GetIDsOfNames, getidsofnames, (REFIID riid, LPOLESTR *prgpsz, UINT cpsz, LCID lcid, DISPID *prgid))
    ...
    TEAROFF_METHOD(CBase, GetDispID, getdispid, (BSTR bstrName, DWORD grfdex, DISPID *pid))
        ...
        if (pvarRes)
        {
            ...
            if (!hr)
            {
                Format(0, &pchFuncName, 255, _T("\nfunction <0s>() {\n    [native code]\n}\n"),
                       pDesc->pstrExposedName ? pDesc->pstrExposedName : pDesc->pstrName);
                if (!hr)
                {
    ...
    IServiceProvider *pSrvProvider)
{
    ...
    IUnknown *      pUnk = NULL;
    CDispParams     dispParams(pdispparams->cArgs, pdispparams->cNamedArgs + 1);
                                    ...
                                    IUnknown *pUnkContext)
{
    ...
    if (wFlags & DISPATCH_PROPERTYGET)
    {
        ...
        case DISPID_UNKNOWN:
            // Special case handling if we ever have a condition were something can't be found but the invoke caller expect us
            // to return data (pvarResult) then instead of returning the error DISP_EMEMBERNOTFOUND we'll return DISPID_UNKNOWN with
            // a hr result of S_OK (inn GetIDsOfNamesEx) when the Invoke is called and if we ever get a DISPID_UNKNOWN we'll simply
            // return VT_NULL.  This will JavaScript to compare the result to null and VBScript to detect isnull()
            if (pvarResult)
            {
                VariantClear(pvarResult);
                pvarResult->vt = VT_NULL;
    ...
    else
    {
        ...
        else
        {
                ...
                (pDesc->b.dwPPFlags & PROPPARAM_INVOKEGet))
            {
                ...
                // in the propdesc is for the set adding +4 gets us to the get
                // property function.
            ...
            {
                // Check if method and args are at least minimum #. 
                // Note, I longer check the maximum count, JavaScript (Navigator)
                         ...
                         IDispatchEx  * pDispEx /*= NULL*/)
{
    ...
    if (pDisp || pDispEx)
    {
        ...
        else if (pDisp)
        {
            ...
            if (!hr)
            {
                hr = THR_NOTRACE(MatchExactGetIDsOfNames(pTI, IID_NULL, &bstrName, 1, 0, pid, !!(grfdex & fdexNameCaseSensitive)));
            }
            ...
            if (hr)
            {
                ...
                hr = THR_NOTRACE(pDisp->GetIDsOfNames(IID_NULL, &bstrName, 1, 0, pid));
            }
...
HRESULT CBase::GetTheDocument(IHTMLDocument2 **ppDoc)
{
    HRESULT         hr;
    IDispatchEx    *pDex = NULL;
    BSTR            bstr = NULL;
    ...
    if (hr)
    {
        ...
        hr = pDex->InvokeEx(dispid, g_lcidUserDefault, DISPATCH_PROPERTYGET, &dp, &retVariant, NULL, NULL);
        if (hr)
            goto Cleanup;
    ...
    ITypeInfo *pTIEventSrc/* = NULL*/)
{
    ...
    CStackPtrAry<BOOL, 2>           arySinkType(Mt(CBaseFireTypeAry_pv));
    long                            i;
    ...
    if (dispidProp != DISPID_UNKNOWN)   // Skip property events
    {
        ...
        IGNORE_HR(FireAttachEvents(dispidProp, pdispparams, pvarResult, this, pexcepinfo, puArgErr));
    }
    ...
    for (;;)
    {
        ...
        if (pAV)
        {
            if (OK(FetchObject(pAV, VT_UNKNOWN, (void **)&pDisp)) && pDisp)
            {
                ...
                hr = THR(arySinkType.Append(pAV->IsTridentSink()));
                if (hr)
                    goto Cleanup;
    ...
    for (i = 0; i < arySinks.Size(); i++)
    {
        ...
        if (arySinkType[i])
        {
            ...
            hr = FindPropDescFromDispID(dispidProp, (PROPERTYDESC **)&pDesc, NULL, NULL);
            if (!hr)
            {
                LPCOLESTR pEventString = (LPCOLESTR)(pDesc->pstrExposedName ? pDesc->pstrExposedName
                                                                  : pDesc->pstrName);
                IGNORE_HR(pTriSink->FireEvent(pEventString, pdispparams, &Var, pexcepinfo));
            }
            else if (pTIEventSrc)
            {
                BSTR    eventName;
    ...
    if (dispidProp != DISPID_UNKNOWN)   // Skip property events
    {
        ...
        for (;;)
        {
            ...
            if (!pDoc)
            {
                ...
                Verify(!pDocAccessObject->GetTheDocument(&pDoc));

                hr = THR(pDoc->get_parentWindow(&pWindow));
                if (hr)
                    continue;

                hr = THR(pWindow->get_event((IHTMLEventObj **)&V_DISPATCH(&varEventObj)));
                if (!hr)
    ...
    va_list         valParms)
{
    ...
    hr = THR(FireAttachEvents(dispidProp, &dp, pvRes, pDocAccessObject));
    if (hr)
        goto Cleanup;
    ...
    va_list         valParms)
{
    ...
    hr = THR(FireEvent(dispidEvent, dispidProp, pvRes, &dp, &ei, &uArgErr));
    if (hr)
        goto Cleanup;
...
HRESULT
CBase::FireCancelableEvent(DISPID dispidMethod, DISPID dispidProp,
                 VARIANT_BOOL *pfRetVal, BYTE * pbTypes, ...)
{
    va_list     valParms;
    CVariant    Var;
    HRESULT     hr;
    ...
    hr = THR(FireEventV(dispidMethod, dispidProp, &Var, pbTypes, valParms));
    va_end(valParms);
...
void
CBase::FindAAIndexAndDelete (DISPID dispID, CAttrValue::AATYPE aaType)
{
    AAINDEX aaidx;
...
HRESULT
CBase::GetVariantAt(AAINDEX aaIdx, VARIANT *pVar)
{
    HRESULT             hr = S_OK;
...
CBase::removeAttribute(BSTR strPropertyName, LONG lFlags, VARIANT_BOOL *pfSuccess)
{
...
#if TREE_SYNC
    (void)PrivateQueryInterface(CLSID_CElement,(void**)&pEl);

    if (pEl != NULL)
    {
        if (S_OK != THR(pEl->Doc()->_pqdocGlue->_SyncLogger.SetAttributeHandler(this,strPropertyName,NULL/*remove*/,lFlags,&fDoIt,&pElementSyncRoot)))
        {
            goto Cleanup;
        }
    }
    ...
    if (fDoIt)
    {
        if (S_OK != THR(pEl->Doc()->_pqdocGlue->_SyncLogger.SyncLoggerSend(pElementSyncRoot)))
        {
            goto Cleanup;
        }
    }
    ...
    if (pLogRecord != NULL)
    {
        if (S_OK != THR(pEl->Doc()->_pqdocGlue->_SyncLogger.CloseLog()))
        {
            goto Cleanup;
        }
...
BOOL
CBase::removeAttributeDispid(DISPID dispid)
{
    AAINDEX aaIx;
    BOOL fExpando;
    CVariant varNULL(VT_NULL);
    ...
    if ( !fExpando )
    {
        ...
        if (THR(OnPropertyChange(dispid, pVtDesc->pPropDesc->GetdwFlags())))
            return FALSE;
    }
...
CBase::setAttribute(BSTR strPropertyName, VARIANT varPropertyValue, LONG lFlags)
{
    ...
    if (pEl != NULL)
    {
        hr = THR(pEl->Doc()->_pqdocGlue->_SyncLogger.SetAttributeHandler(this,strPropertyName,&varPropertyValue,lFlags,&fDoIt,&pElementSyncRoot));
        if (S_OK != hr)
        {
            goto Cleanup;
        }
...
#endif // TREE_SYNC
#if TREE_SYNC // hack: before doc modificiation
    if (fDoIt)
    {
        hr = THR(pEl->Doc()->_pqdocGlue->_SyncLogger.SyncLoggerSend(pElementSyncRoot));
        if (S_OK != hr)
        {
            goto Cleanup;
        }
...
#if TREE_SYNC && 0 // hack: disabled
    if (SUCCEEDED(hr) && (pLogRecord != NULL))
    {
        hr = THR(pEl->Doc()->_pqdocGlue->_SyncLogger.CloseLog());
        if (S_OK != hr)
        {
            goto Cleanup;
        }
...
CBase::FindPropDescFromDispID(DISPID dispidMember, PROPERTYDESC **ppDesc, WORD *pwEntry, WORD *pwIIDIndex)
{
    ...
    *ppDesc = NULL;
    ...
    if (!pVTblArray)
    {
        ...
        // Everyone had better have a vtable array.
        Assert(pVTblArray != NULL);
...
CBase::GetDisplayString(DISPID dispID, BSTR *pBstr)
{
    ...
    *pBstr = NULL;
    ...
    if(!pEnumDesc || !pEnumDesc->cEnums)
    {
        // No enum description, return the value as string
        hr = THR(GetIntoBSTRAt(idx, &*pBstr));
        goto Cleanup;
    }
