...
public:
    ...
    static BOOL ShouldStartTracker( CSelectionManager* pManager, SelectionMessage* pMessage, ELEMENT_TAG_ID eTag, IHTMLElement* pIElement, IHTMLElement** ppIEditThisElement = NULL )  ;
    virtual HRESULT HandleMessage( SelectionMessage *pMessage,  DWORD* pdwFollowUpAction,  TRACKER_NOTIFY * peTrackerNotify ) = 0 ;
    virtual HRESULT Notify(TRACKER_NOTIFY inNotify,  SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify  ) = 0 ;
    virtual HRESULT Position( IMarkupPointer* pStart, IMarkupPointer* pEnd, BOOL fNotAtBOL = TRUE, BOOL fAtLogicalBOL = FALSE) = 0;
    virtual HRESULT GetLocation(POINT *pPoint);
    virtual VOID OnEditFocusChanged();

    HRESULT static GetTagIdFromMessage( SelectionMessage* pMessage, ELEMENT_TAG_ID* peTag , IHTMLDocument2 * pIDoc = NULL, IHTMLViewServices * pVS = NULL , IMarkupServices* pMarkup = NULL );
    HRESULT static GetElementAndTagIdFromMessage(SelectionMessage* pMessage, IHTMLElement** ppElement,  ELEMENT_TAG_ID* peTag , IHTMLDocument2 * pIDoc , IHTMLViewServices * pVS = NULL , IMarkupServices * pMark = NULL  );
    HRESULT static GetElementFromMessage( CSelectionManager* pManager, SelectionMessage* peMessage, IHTMLElement** ppElement );
...
class CCaretTracker : public CEditTracker
{
    ...
    public:
        ...
        static HRESULT SetCaretVisible(IHTMLDocument2*         pIDoc, BOOL                    fVisible );
        static CARET_MOVE_UNIT GetMoveDirectionFromMessage(SelectionMessage*       pMessage, BOOL                    fRightToLeft);
...
class CSelectTracker: public CEditTracker
{
    public:
        ...
        HRESULT GetCaretStartPoint( CARET_MOVE_UNIT inCaretMove, IMarkupPointer* pCopyStart );
        BOOL IsListeningForMouseDown();
        HRESULT DoTimerDrag( );
    ...
    private:
        ...
        HRESULT MoveWord( IMarkupPointer* pPointer, MOVEUNIT_ACTION  muAction );
        VOID StartTimer();
        HRESULT ConstrainSelection( BOOL fConstrainStart = FALSE, POINT* pptGlobal = NULL  );
        HRESULT AdjustSelection(BOOL * pfAdjustedSel);
        HRESULT ScanForEnterBlock( Direction iDirection, IMarkupPointer* pPointer, BOOL* pfFoundEnterBlock, DWORD *pdwBreakCondition = NULL );
        HRESULT ScanForLastExitBlock( Direction iDirection, IMarkupPointer* pPointer, DWORD* pdwBreakCondition = NULL );
        HRESULT ScanForLastEnterBlock( Direction iDirection, IMarkupPointer* pPointer, DWORD* pdwBreakCondition  = NULL );
        HRESULT ScanForLastEnterOrExitBlock( Direction iDirection, IMarkupPointer* pPointer, DWORD dwTerminateCondtion , DWORD* pdwBreakCondition = NULL );
        BOOL IsValidForAdjust( ELEMENT_TAG_ID eTag );
        HRESULT HandleMessagePrivate( SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify );
        HRESULT HandleChar( SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify );
        HRESULT HandleKeyUp( SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify );
        HRESULT HandleKeyDown( SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify );
        HRESULT OnTimerTick(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify   );
        ...
        BOOL IsAdjacentToBlock( IMarkupPointer* pPointer, Direction iDirection, DWORD *pdwBreakCondition = NULL );
...
class CControlTracker : public CEditTracker
{
    public:
        ...
        BOOL HasAdorner();
        HRESULT HandleMessage( SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify );
        HRESULT HandleMouseDown( SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify );
        HRESULT HandleMouseUp(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify );
        HRESULT Notify(TRACKER_NOTIFY inNotify, SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify  );
        HRESULT Position( IMarkupPointer* pStart, IMarkupPointer* pEnd, BOOL            fNotAtBOL = TRUE, BOOL            fAtLogicalBOL = FALSE);
        static BOOL ShouldStartTracker( CSelectionManager* pManager, SelectionMessage* pMessage, ELEMENT_TAG_ID eTag, IHTMLElement* pIElement, IHTMLElement** ppIEditThisElement = NULL ) ;
        BOOL IsActive();
        IHTMLElement* GetControlElement() {  return _pIControlElement; }
        BOOL IsSameElementAsControl(         IHTMLElement* pIElement );
...
// Inlines
inline BOOL CSelectTracker::GetMadeSelection()
{
    return _fMadeSelection;
}

inline IMarkupPointer* CSelectTracker::GetStartSelection()
{
    return _pStartPointer;
}

inline IMarkupPointer* CSelectTracker::GetEndSelection()
{
    return _pEndPointer;
}

inline BOOL CControlTracker::IsActive()
{
    return( ! _fPassive );
}

inline BOOL CControlTracker::IsPassive()
{
    return( _fPassive );
}
...
inline HRESULT CSelectTracker::MoveWord( IMarkupPointer* pPointer, MOVEUNIT_ACTION  muAction)
{
    HRESULT hr = THR( GetViewServices()->MoveWord( pPointer, muAction, _pManager->GetStartEditContext(), _pManager->GetEndEditContext() ));
