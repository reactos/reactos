...
HRESULT CEditTracker::GetTagIdFromMessage(SelectionMessage* pMessage, ELEMENT_TAG_ID* peTag , IHTMLDocument2 * pIDoc, IHTMLViewServices * pVS, IMarkupServices * pMark )
{
    HRESULT hr = S_OK;
    IHTMLViewServices* pViewServices = NULL;
    IMarkupServices* pMarkup = NULL;
...
{
    return ( ( eTag == TAGID_TD ) || ( eTag == TAGID_TR ) || ( eTag == TAGID_TBODY ) || ( eTag == TAGID_TFOOT ) || ( eTag == TAGID_TH ) || ( eTag == TAGID_THEAD ) ||
       ( eTag == TAGID_CAPTION ) || ( eTag == TAGID_TC )  || ( eTag == TAGID_COL ) || ( eTag == TAGID_COLGROUP ));
}
...
HRESULT CSelectTracker::HandleMessage(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify )
{
    ...
    switch( pMessage->message )
    {
        case WM_RBUTTONUP:
        {
            ...
            {
                IGNORE_HR( GetViewServices()->MoveMarkupPointerToMessage( _pStartPointer, pMessage, & _fNotAtBOL , & _fAtLogicalBOL , & fRightOfCp , & fValidTree, FALSE, GetEditableElement(), NULL, FALSE  ));
...
HRESULT CSelectTracker::Notify(TRACKER_NOTIFY inNotify, SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify  )
{
    HRESULT hr = S_OK;
    DWORD followUpAction = FOLLOW_UP_ACTION_None;
...
BOOL CSelectTracker::ShouldStartTracker(CSelectionManager* pManager, SelectionMessage* pMessage, ELEMENT_TAG_ID eTag, IHTMLElement* pIElement, IHTMLElement** ppIEditThisElement /*= NULL */)
{
    ...
    switch ( eTag )
    {
        ...
        case TAGID_INPUT:
        {
            ...
            {
                if (!_tcsicmp( bstrType, TEXT("image")) || !_tcsicmp(bstrType, TEXT("radio")) || !_tcsicmp(bstrType, TEXT("checkbox")))
                {
                    fShouldStart = FALSE;
                }
                else if ( !_tcsicmp( bstrType, TEXT("button")) && !pManager->IsContextEditable() )
                {
...
HRESULT CSelectTracker::HandleChar(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify )
{
    HRESULT         hr = S_OK ;
...
HRESULT CSelectTracker::HandleKeyUp(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify)
{
    HRESULT hr = S_FALSE;
    DWORD followUpAction = FOLLOW_UP_ACTION_None;
...
HRESULT CSelectTracker::HandleKeyDown(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify)
{
    HRESULT hr = S_FALSE;
    DWORD followUpAction = FOLLOW_UP_ACTION_None;
...
#ifndef NO_IME
            case VK_KANJI:
                if (   949 == GetKeyboardCodePage() && _pManager->IsContextEditable() && EndPointsInSameFlowLayout())
                {
                    BOOL fEndAfterStart;

                    IFC( _pEndPointer->IsRightOf( _pStartPointer, &fEndAfterStart ) );
                    _pManager->StartHangeulToHanja(peTrackerNotify, fEndAfterStart ? _pStartPointer : _pEndPointer);
                }
                hr = S_OK;
                break;
#endif // !NO_IME
...
HRESULT CSelectTracker::HandleImeStartComposition(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify )
{
...
HRESULT CSelectTracker::GetCaretStartPoint(CARET_MOVE_UNIT inCaretMove, IMarkupPointer* pCopyStart )
{
    HRESULT     hr;
    int         iWherePointer;
    Direction   dir;
    ...
    IFC( OldCompare( _pStartPointer, _pEndPointer, & iWherePointer ));
    if ( (iWherePointer == RIGHT && dir == LEFT) || (iWherePointer == LEFT && dir == RIGHT) )
    {
...
HRESULT CSelectTracker::MovePointersToTrueStartAndEnd(IMarkupPointer* pTrueStart, IMarkupPointer* pTrueEnd, BOOL *pfSwap , BOOL *pfEqual /*= NULL*/ )
{
    HRESULT hr = S_OK;
    int iWherePointer = SAME;
    BOOL fSwap = FALSE;
...
// Ignore BR's !
BOOL CSelectTracker::IsBetweenBlocks( IMarkupPointer* pPointer )
{
    HRESULT hr;
    BOOL fBetween = FALSE;
    ...
    {
        IFC( scanPointer.MoveToPointer( pPointer ));
        IFC( scanPointer.Scan( RIGHT , BREAK_CONDITION_BlockScan, & dwBreakCondition ));
...
BOOL CSelectTracker::IsAtEdgeOfTable( Direction iDirection, IMarkupPointer* pPointer )
{
    HRESULT hr;
    BOOL fAtEdge = FALSE;
    DWORD dwBreakCondition = 0;
    ...
    CEditPointer scanPointer( GetEditor());
    IFC( scanPointer.MoveToPointer( pPointer ));
    IFC( scanPointer.Scan( iDirection, BREAK_CONDITION_BlockScan - BREAK_CONDITION_Block , & dwBreakCondition, & pIElement));

    if ( scanPointer.CheckFlag( dwBreakCondition, BREAK_CONDITION_ExitSite ) )
    {
...
HRESULT CSelectTracker::MoveSelection(  CARET_MOVE_UNIT inCaretMove )
{
    HRESULT             hr = S_OK;
    LONG                curMouseX;
    HTMLPtrDispInfoRec  pt;
    ...
    if (( fBetweenBlocks && inCaretMove == CARET_MOVE_RIGHT ) ||
        (( inCaretMove == CARET_MOVE_RIGHT || inCaretMove == CARET_MOVE_LEFT ) && IsAtEdgeOfTable( Reverse( GetPointerDirection(inCaretMove )) , _pEndPointer )))
    {
...
HRESULT CSelectTracker::OnTimerTick(SelectionMessage *pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackerNotify )
{
    HRESULT                 hr = S_OK;
...
HRESULT CSelectTracker::BeginSelection(SelectionMessage* pMessage, DWORD* pdwFollowUpAction, TRACKER_NOTIFY * peTrackNotify )
{
    HRESULT hr = S_OK;
    DWORD followUp = 0;
    BOOL fTestNotAtBOL;
    ...
    IFC( GetViewServices()->MoveMarkupPointerToMessage( _pStartPointer, pMessage, & _fNotAtBOL , & _fAtLogicalBOL , & fRightOfCp , & fValidTree, FALSE, GetEditableElement(), NULL, FALSE )); // we don't test EOL
    ...
    // BUG BUG - this is a bit expensive - but here I want to hit test EOL and previously I didn't.

    // Why do we want to hit test EOL here ? Otherwise we'll get bogus changes in direction.
    IFC( GetViewServices()->MoveMarkupPointerToMessage( _pTestPointer, pMessage, & fTestNotAtBOL , & fTestAtLogicalBOL , & fRightOfCp , & fValidTree, FALSE, GetEditableElement(), NULL, TRUE )); // we do test EOL
...
HRESULT CSelectTracker::ConstrainSelection( BOOL fMoveStart /*=fALSE*/, POINT* pptGlobal /*=NULL*/ )
{
    HRESULT hr = S_OK;
    ...
    if ( !pptGlobal || !_pManager->IsEditContextPositioned() )
    {
        IFC( ConstrainPointer( _pEndPointer, fDirection ));
    }
    else if ( ! _pManager->IsInEditContext( _pEndPointer ))
    {
        // We do this for positioned elements only. Why ? Because where the pointer is in the stream may not necessarily match where it "looks like" it should be.

        // Most of the time we don't get here. MoveMarkupPointer should have adjusted us perfectly already.
        IFC( GetViewServices()->GetAdjacencyForPoint(_pManager->GetEditableElement(), *pptGlobal, & eAdj ));
...
HRESULT CSelectTracker::MoveEndToPointer( IMarkupPointer* pPointer )
{
    HRESULT hr = S_OK;
    ...
    if ( IsTagEnabled( tagSelectionValidate))
    {
        ...
        if ( endCp != 0 && oldEndCp != -1 && ( endCp - startCp == 0 ) && ( oldEndCp - startCp != 0 ) )
        {
...
HRESULT CSelectTracker::DoSelection(SelectionMessage* pMessage, BOOL fAdjustSelection /*=FALSE*/)
{
    HRESULT hr = S_OK;
    Assert( _pEndPointer );
    BOOL fRightOfCp;
    ...
    if (  fAdjustSelection || ( pMessage->pt.x != _curMouseX) || ( pMessage->pt.y != _curMouseY ) || ( pMessage->message == WM_TIMER ) )  // ignore spurious mouse moves.
    {
        hr = GetViewServices()->MoveMarkupPointerToMessage( _pTestPointer, pMessage, & fNotAtBOL , & fAtLogicalBOL, & fRightOfCp , & fValidTree, FALSE  , GetEditableElement(), & fValidLayout, TRUE );
#if DBG ==1
       testCp = GetCp( _pTestPointer );
       if ( testCp == gDebugTestPointerCp || ( testCp >= gDebugTestPointerMinCp && testCp <= gDebugTestPointerMaxCp ) )
       {
        ...
        if ( fValidTree || !fAdjustedSel )
        {
            if ( !fValidLayout || !fInEdit )
            {
                ...
                GetTagIdFromMessage( pMessage, & eTag, _pManager->GetDoc(), GetViewServices(), GetMarkupServices() );
                ...
                else if ( _pManager->IsContextEditable() || IsJumpOverAtBrowse( eTag ) ) // we only jump over at edit time or you can jump over this thing.
                {
        ...
        if ( fAdjustSelection || fAdjustedSel )
        {
            ...
            if ( fDidAdjust || fAdjustedSel )
            {
               ...
               if ( endCp == gDebugEndPointerCp || ( ( newSelSize == 0 )  && (  oldSelSize != 0  ) ) )
               {
                    if ( IsTagEnabled(tagSelectionDumpOnCp))
                    {
                        DumpTree( _pEndPointer );
...
HRESULT CSelectTracker::ScrollMessageIntoView(SelectionMessage* pMessage )
{
    HRESULT hr = S_OK;
...
HRESULT CSelectTracker::ScrollPointerIntoView(IMarkupPointer* pScrollPointer, BOOL fNotAtBOL, SelectionMessage* pMessage )
{
    HRESULT hr = S_OK;
    ...
    if ( pMessage && pMessage->message == WM_TIMER && pMessage->fEmptySpace && IsInWindow( pMessage->pt , TRUE ) && IsPointInEditContextContent( pMessage->pt ) )
    {
    ...
    if ( ! IsInValidPlaceForScroll( pScrollPointer ))
    {
        ...
        else
        {
            ...
            IFC( scanPointer.Scan( GetMoveDirection() ? LEFT : RIGHT  , BREAK_CONDITION_OMIT_PHRASE - BREAK_CONDITION_Block - BREAK_CONDITION_Site, & dwBreak ));
            ...
            {
                IFC( scanPointer.MoveToPointer( pScrollPointer ));
                IFC( scanPointer.Scan( GetMoveDirection() ? RIGHT : LEFT  , BREAK_CONDITION_OMIT_PHRASE - BREAK_CONDITION_Block - BREAK_CONDITION_Site, & dwBreak ));
                if ( scanPointer.CheckFlag( dwBreak, BREAK_CONDITION_Text ))
                {
                    fFoundGoodPlace =  IsInValidPlaceForScroll( pScrollPointer );
                }
            ...
            if ( fFoundGoodPlace )
            {
                hrScroll = THR( GetViewServices()->ScrollPointerIntoView( scanPointer, fNotAtBOL, POINTER_SCROLLPIN_Minimal ));
            }
        }
    }
    else
    {
        hrScroll = THR ( GetViewServices()->ScrollPointerIntoView( pScrollPointer , fNotAtBOL, POINTER_SCROLLPIN_Minimal ));
    }
...
BOOL CSelectTracker::IsPointInEditContextContent(POINT ptGlobal)
{
    HRESULT hr;
    RECT contentRect;
    hr = THR( GetViewServices()->GetContentRect( _pManager->GetEditableElement(), COORD_SYSTEM_GLOBAL, & contentRect  ));
    if ( ! FAILED( hr ))
    {
...
BOOL CSelectTracker::IsInValidPlaceForScroll(IMarkupPointer* pScrollPointer)
{
    HRESULT hr = S_OK;
...
// Synopsis: Check if end is to the left/right of the swap pointer. If it is flip start and end and flip the swap direction.
...
HRESULT CSelectTracker::CheckSwap()
{
    HRESULT hr = S_OK;
    BOOL fOkSwap;
...
// Synopsis: Do the word selection behavior. Select on word boundaries when in Word Selection mode.
HRESULT CSelectTracker::DoWordSelection(SelectionMessage* pMessage , BOOL* pfAdjustedSel, BOOL fRightOfCp, BOOL fFurtherInStory )
{
    HRESULT hr = S_OK;
    int iWherePointer = SAME;
    ...
    if ( _fInWordSel )
    {
        ...
        if ( _fInWordSel ) // if we're still in word sel
        {
            ...
            fNeedsAdjust = ( fFurtherInStory ) ? ( iWherePointer == RIGHT ) : ( iWherePointer == LEFT ) ;
            if ( fNeedsAdjust )
            {
                ...
                if ( ! fAlreadyAtWord )
                {
                    ...
                    IFC( MoveWord( _pWordPointer, GetMoveDirection() ? MOVEUNIT_NEXTWORDBEGIN : MOVEUNIT_PREVWORDBEGIN   ));
    ...
    else
    {
        ...
        if ( _fReversed && ( fFurtherInStory == ENSURE_BOOL( _fReversedDirection )) )
        {
    ...
    //                    DumpTree( _pEndPointer );
                    ...
                    if ( ! ENSURE_BOOL(_fStartAdjusted) )
                    {
                        ...
                        // we don't make the adjustment if the context to the left/right of start is not text !
                        if ( fFurtherInStory )
                        {
                        ...
                        if ( eContext == CONTEXT_TYPE_Text )
                        {
                            ...
                            {
                                IFC( MoveWord( _pStartPointer, fFurtherInStory ? MOVEUNIT_PREVWORDBEGIN  : MOVEUNIT_NEXTWORDBEGIN ));
...
HRESULT CSelectTracker::SetWordPointer(
                                ...
                                BOOL* pfWordPointerSet /*= NULL*/ )
{
    ...
    if ( !fAtWord )
    {
        IFC( MoveWord( pPointerToSet, fFurtherInStory ? MOVEUNIT_NEXTWORDBEGIN : MOVEUNIT_PREVWORDBEGIN ));
        IFC( ConstrainPointer( pPointerToSet, fFurtherInStory ));

        // Compensate for moving past the end of a line.

        // We look for this by seeing if the Test is to the left of a block (for the fFurtherInStory case) and the word is to the right of a block.
        fBlockBetween = IsAdjacentToBlock( pPointerToSet, fFurtherInStory ? LEFT: RIGHT ) && IsAdjacentToBlock( _pTestPointer, fFurtherInStory ? RIGHT : LEFT );
        if ( fBlockBetween )
        {
...
HRESULT CSelectTracker::GetStartSelectionForSpringLoader( IMarkupPointer* pPointerStart)
{
    HRESULT hr = S_OK;
    ...
    {
        IFC( MovePointerToText(GetMarkupServices(), GetViewServices(), pPointerStart, RIGHT, pBoundary, &fHitText, FALSE ) );
    }
...
BOOL CSelectTracker::IsAdjacentToBlock(IMarkupPointer* pPointer, Direction iDirection, DWORD* pdwBreakCondition)
{
...
HRESULT CControlTracker::UnSelect()
{
    HRESULT hr = S_OK;
    IHTMLDocument2* pIDoc = _pManager->GetDoc();
    IHTMLViewServices * pViewServices = NULL;
...
HRESULT CControlTracker::CommitMove(SelectionMessage* peMessage, TRACKER_NOTIFY * peTrackerNotify )
{
    HRESULT hr = S_OK;
    ...
    if ( fGoAhead )
    {
        switch ( eAction )
        {
            case DRAG_ACTION_2D_POSITION :
            {
                ...
                _pGrabAdorner->EndMove(peMessage->pt  , & newLocation, ! fHostDraw , &NewRect);
                ...
                IGNORE_HR( GetViewServices()->TransformPoint( & point, COORD_SYSTEM_GLOBAL, COORD_SYSTEM_PARENT, _pIDragElement ) );
...
//           BUGBUG - we assume the drag ends in trident. We also assume that it's valid to do a cut & paste here.
//           We don't do a 'real' OLE-Drag-Drop, so any assumptiions underlying pieces may have made about this may break.
HRESULT CControlTracker::DoPasteInFlow( SelectionMessage* peMessage, TRACKER_NOTIFY *peTrackerNotify )
{
    ...
    IFC( GetViewServices()->MoveMarkupPointerToMessage( pPointer, peMessage, & fNotAtBOL, & fAtLogicalBOL, & fRightOfCp , & fValidTree, TRUE, GetEditableElement(), NULL )) ;
