...
HRESULT CMshtmlEd::Initialize( IUnknown * pContext )
{
    _pContext = pContext;
    return(S_OK);
}
...
STDMETHODIMP_(ULONG) CMshtmlEd::AddRef( void )
{
    return( ++_cRef );
}


STDMETHODIMP_(ULONG) CMshtmlEd::Release( void )
{
    --_cRef;
...
STDMETHODIMP CMshtmlEd::QueryInterface(
    REFIID              iid,
    LPVOID *            ppv )
{
    if (!ppv)
...
BOOL CMshtmlEd::IsDialogCommand(DWORD nCmdexecopt, DWORD nCmdID, VARIANT *pvarargIn)
{
    BOOL bResult = FALSE;
...
STDMETHODIMP CMshtmlEd::Exec( const GUID *       pguidCmdGroup,
                       DWORD        nCmdID,
                       DWORD        nCmdexecopt,
...
HRESULT CMshtmlEd::ControlRangeExec(
    DWORD                    cmdId,
    DWORD                    nCmdexecopt,
    VARIANTARG *             pvarargIn,
    VARIANTARG *             pvarargOut)
...
HRESULT CMshtmlEd::ControlRangeQueryStatus(
    UINT          cCmds,
    OLECMD        rgCmds[],
    OLECMDTEXT *  pcmdtext)
{
