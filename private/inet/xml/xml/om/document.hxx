 ...
 * and then save it back out again. The document can be loaded by specifying a URL or an input stream.
 * <P>
 ...
 * A helper method <code>getRoot</code> is provided as a short cut to finding the root element.
...
class DECLSPEC_UUID("B5B359F0-53DC-11d1-8C6A-80BAB4000000")
    Document : public CSafeControl, public PersistStream, public PersistMoniker, public OleCommandTarget
{
...
public: Mutex * getMutexNonReentrant(DWORD dwTID);
        ...
        // So you can mark the document non-re-entrant for this thread for write operations.
public: DWORD registerNonReentrant();
public: void removeNonReentrant(DWORD = 0);

        // You can also makr the document read only so that it cannot be modified through the DOM at all (e.g. schema documents).
...
public: void leaveDOMLoadLock(HRESULT hr);
        ...
        /**
         * Construct a new empty document and use the default element factory.
         */
public: Document();
protected: ~Document();
...
public: virtual Document * clone(bool deep);
        ...
        void save(String * Url, String * Encoding);
...
public: Node* parseXMLDecl(const WCHAR* attrs);
         ...
         * with the XML element.  Set/getVersion and set/getEncoding are available for those purposes.
         */
...
public: NameDef * getDocType();
         ...
         * @return the URL specified in the &lt;!DOCTYPE&gt; tag or null if an internal DTD was specified.
         */
public: String * getDTDURL();

        /**
         * Retrieves the URL.
         * @return the last URL sent to the load() method  or null if an input stream was used.
         */
...
public: virtual void queryStatus(const GUID *pguidCmdGroup, ULONG cCmds, OLECMD prgCmds[], OLECMDTEXT *pCmdText);
public: virtual void exec(const GUID *pguidCmdGroup, DWORD nCmdID, DWORD nCmdexecopt, VARIANT *pvaIn, VARIANT *pvaOut);
...
public: virtual void NotifyListener(XMLNotifyReason, XMLNotifyPhase, Node *pNode, Node *pNodeParent, Node *pNodeBefore = NULL)
{ }
