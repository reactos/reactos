                                           ...
                                           bool fProcessNamespaces)
{
    ...
    TRY
    {
        ...
        if (!fIgnoreDTD) {
            ...
            // But we still need the ValidationFactory even when the DTD is empty so we can generate errors for undefined entities.
            if (doc->getValidateOnParse() && !_pDTD->isSchema()) {
                // Then we will start with default node factory,
                // switch to the DTD node factory if we find a DOCTYPE decl and then switch to validation node factory.
    ...
    /* [in] */ XML_NODEFACTORY_EVENT iEvt)
{
    ...
    TRY
    {
        ...
        case XMLNF_ENDENTITY:
            ...
            if (hr == S_OK) {
                // If we have a DTD that requires validation & validation is turned on, then switch to validation factory.
                ...
                } else {// switch back to default factory.
                    _pFactory = _pFactoryDefault;
                }
            } else if (hr == S_FALSE) {
                // S_FALSE is a sneaky way for DTDNodeFactory to tell
                // the NamespaceNodeFactory that we are now parsing entities.
                hr = S_OK;
            }
            goto CleanUp;
            break;
    ...
    /* [in] */ XML_NODE_INFO __RPC_FAR *__RPC_FAR *apNodeInfo)
{
    ...
    TRY
    {
        ...
        // We could eventually remember this in a flag on the Node so we can persist the attribute value quotes correctly.
...
createurn:
            ...
            case XML_XMLDECL:
atomize:
                pNodeInfo->pReserved = _pMgr->createNameDef(pNodeInfo->pwcText,
                                                            pNodeInfo->ulLen,
                                                            pNodeInfo->ulNsPrefixLen,
                                                            fDefaultNS,
                                                            pAtomURN,
            ...
            case XML_DOCTYPE:        // <!DOCTYPE
                if (!_pDTD->isSchema()) {
                    if (!_fIgnoreDTD) {
                        // Then we will start with DTD node factory and then switch to validation node factory.
        ...
        // stack to use this node as the context so this namespace scope gets popped on EndChildren.
        ...
        if (cNumRecs > 1) {
            ...
            if (pNodeInfo->dwType == XML_PI) {
                ...
                // need to make sure we do the BeginChildren call for the nodedatanodefactory.
        ...
        if (fLoadedSchema && _pDoc->hasPendingChildDocs()) {
            // The child document will wake up this document when it's done.
                                                  ...
                                                  bool& fLoadedSchema)
{
    ...
    while (i > 0) {
        ...
        if (XML_ATTRIBUTE == pInfo->dwType && XML_NS == pInfo->dwSubType && 0 != pInfo->ulLen) {
            ...
            {
                // BUGBUG - need to resolve the url to a full path so that the URN is the same no matter how
                // we get there.  But this is very difficult to do because of HTTP redirection.
...
void NameSpaceNodeFactory::LoadSchema(IXMLNodeSource* pNodeSource, Atom* pURN)
{
    if (!_pDTD->hasSchema(pURN)) {
        ...
        // if this schema is already pending for a parent document, then move it to this document so we can process it now !
...
HRESULT NameSpaceNodeFactory::HandlePending()
{
    HRESULT hr = S_OK;
    void* pTag;
    Node* attr;
