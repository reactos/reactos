...
HRESULT STDMETHODCALLTYPE CXMLConnectionPt::GetConnectionInterface(IID *pIID)
{
    if (pIID) {
        pIID = (IID *)&_EventIID;
        return S_OK;
...
HRESULT STDMETHODCALLTYPE CXMLConnectionPt::GetConnectionPointContainer(IConnectionPointContainer **ppCPC)
{
    HRESULT hr;
    if (ppCPC) {
...
HRESULT STDMETHODCALLTYPE CXMLConnectionPt::Advise(IUnknown *pUnkSink, DWORD *pdwCookie)
{
    HRESULT hr;
...
#endif
            *_ppRootNode = pNode;// Just insert the node at the beginning of the list
        } else {
            hr = E_OUTOFMEMORY;// Couldn't create the new node
        }
    } else {
        hr = E_POINTER;// Passed in one or more bad pointers
    }
...
HRESULT STDMETHODCALLTYPE CXMLConnectionPt::Unadvise(DWORD dwCookie)
{
    HRESULT hr = S_OK;
...
HRESULT STDMETHODCALLTYPE CXMLConnectionPt::EnumConnections(IEnumConnections **ppEnum)
{
    HRESULT hr = S_OK;
...
CXMLConnectionPtContainer::CXMLConnectionPtContainer(REFIID iid, IUnknown *pUnk, PCPNODE *ppNodeList, ULONG_PTR *plSpinLock) : _EventIID(iid)
{
...
CXMLConnectionPt* CXMLConnectionPtContainer::CreateConnectionPoint(REFIID riid, CPTYPE cpt)
{
    CXMLConnectionPt *pXCPT;

    pXCPT = new_ne CXMLConnectionPt(riid, _punkOuter, _ppNodeList, _plSpinLock, cpt);
...
HRESULT STDMETHODCALLTYPE CXMLConnectionPtContainer::QueryInterface(REFIID riid, void **ppvObject)
{
...
HRESULT STDMETHODCALLTYPE CXMLConnectionPtContainer::EnumConnectionPoints(IEnumConnectionPoints  **ppEnum)
{
    HRESULT hr = S_OK;
...
HRESULT STDMETHODCALLTYPE CXMLConnectionPtContainer::FindConnectionPoint(REFIID riid, IConnectionPoint **ppCP)
{
    HRESULT hr = S_OK;
    ...
    // meant to handle that interface, we pass the hint through correctly, rather than assuming CP_Dispatch
...
CXMLEnumConnectionPt::CXMLEnumConnectionPt(REFIID iid, CXMLConnectionPtContainer *pCPC)
{
    Assert(!ISNULLIID(iid) && "NULL IID for events is not valid");
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnectionPt::Next(ULONG cConnections, LPCONNECTIONPOINT *ppCP, ULONG *pcFetched)
{
    HRESULT hr = S_OK;
    int i;
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnectionPt::Skip(ULONG cConnections)
{
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnectionPt::Reset()
{
    _iIndex = 0;
    return S_OK;
}
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnectionPt::Clone(IEnumConnectionPoints **ppEnum)
{
...
CXMLEnumConnections::CXMLEnumConnections(REFIID iid, PCPNODE pRootNode, IUnknown *pUnk, ULONG_PTR *plSpinLock) : _EventIID(iid)
{
    Assert(!ISNULLIID(iid) && "NULL IID for events is not valid");
...
// Make a copy of the list of connections so that we can safely iterate through it at any time. 
// Validity of connections is obviously not guaranteed
HRESULT CXMLEnumConnections::CopyConnectionList(PCPNODE pRootNode)
{
    HRESULT hr = S_OK;
    ...
    while (pSrcNode) {
        ...
        if (pTempNode) {
            ...
            *ppLast = pTempNode;
            ...
            // Yes, setting it to be NULL, only to over-write on the next iteration but it's safe and not perf critical
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnections::Next(ULONG cConnections, LPCONNECTDATA rgcd, ULONG *pcFetched)
{
    // I have to use returns here because otherwise, in case of error, the
    // mutex would have to be grabbed (before the two if blocks below, and then immediately released
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnections::Skip(ULONG cConnections)
{
    int i = cConnections;
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnections::Reset()
{
    _pCurrentConnection = _pRootNode;
    return S_OK;
}
...
HRESULT STDMETHODCALLTYPE CXMLEnumConnections::Clone(IEnumConnections **ppEnum)
{
...
UINT CrackVarArgList(
    ...
    /*
        ...
        will produce an array of VARIANTARGS in reverse order (which is what is needed for IDispatch)
    */
{
    VARTYPE vt;
    UINT cArgs = 0;
    ...
    while (TRUE) {
        ...
        case VT_INT:
            ...
            // argument list, the compiler actually converts the float to a double and pushes the double onto the stack.
        ...
        case VT_BOOL:
            ...
            break;
        case VT_BSTR:
            if ((pva->bstrVal = va_arg(args, LPOLESTR)) && ((pva->bstrVal = SysAllocString(pva->bstrVal)) == NULL))
                return E_OUTOFMEMORY;
            break;
        case VT_DISPATCH:
...
/*
    This function below is used to call functions in script the Trident way.
    Basically, when a function name is passed in to a function, you are given an IDispatch pointer.
    You invoke against it with dispid == DISPID_THIS, and voila !
*/
{
    DISPPARAMS dp;
