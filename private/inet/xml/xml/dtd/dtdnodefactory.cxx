...
HRESULT STDMETHODCALLTYPE DTDNodeFactory::NotifyEvent(
    ...
    /* [in] */ XML_NODE_INFO* __RPC_FAR * __RPC_FAR apNodeInfo)
{
    if (_fParsingEntity && _pEntity)
    {
        ...
        String* msg = Resources::FormatMessage(XML_PARSING_ENTITY, _pEntity->getName()->toString(), String::emptyString(), null);
        if (bstr != null)
        {
    ...
    /* [in] */ XML_NODE_INFO* __RPC_FAR pNodeInfo)
{
    ...
    case eDTD:
        if (dwType == XML_DTDATTRIBUTE)
        {
            ...
            if (_fBuildDefNode) {
                _fBuildDefNode = false;
                _endChildren(_pFactory, pSource, (Object*)(_pAttDef->_pDefNode), XML_ATTRIBUTE, FALSE, NULL);
                _pAttDef = null;
    ...
    case eDocType:
        ...
        doctype:
            ...
            if (_pDTDHref != null && _pDocument->getResolveExternals())
            {
                ...
                // come AFTER the internal subset so that things in the internal subset take precidence.
                const WCHAR* baseURL;
                WCHAR* buffer = NULL;
    ...
    /* [in] */ XML_NODE_INFO* __RPC_FAR * __RPC_FAR apNodeInfo)
{
    ...
    case eProlog:
        ...
        // and check the XML version number, and check to make sure we don't get undefined entity references.
    ...
    case eDocType:
        ...
        else if (pNodeInfo->dwType == XML_PCDATA)
        {
            ...
            else // need to check PublicID
            {
                ...
                {
                    Exception::throwE(XML_PUBLICID_INVALID, XML_PUBLICID_INVALID, String::newString(pNodeInfo->pwcText, 0, pNodeInfo->ulLen), null);
                }
            }
        }
        goto createnode;
    ...
    case eDTD:
        ...
        if (pNodeInfo->dwType == XML_ATTRIBUTE || pNodeInfo->dwType == XML_DTDATTRIBUTE) // need to remember when we're inside an attribute or not.
        {
            _fInAttribute = true;
        ...
        if (pNodeInfo->dwType == XML_PEREF) {
            ...
            hr = (this->*_pfn)(pSource, pNodeParent, pNodeInfo->dwType, pNodeInfo->dwSubType, text, name, &(pNodeInfo->pNode));
            if (SUCCEEDED(hr))
            {
                if (_fBuildDefNode)
                {
...
HRESULT DTDNodeFactory::HandlePERef(IXMLNodeSource* pSource, Name* name)
{
    HRESULT hr = S_OK;
    XMLParser* pParser = NULL;
    ...
    /* [out] */ PVOID* ppNodeChild)
{
    ...
    case XML_ENTITYDECL:
        ...
        if (!pe)
        {
            ...
            _createDTDNode(_pFactory, pSource, _pDocTypeNode, XML_ENTITYDECL, true, false, NULL, 0, 0, namedef, (PVOID*)ppNodeChild);
    ...
    case XML_NOTATION:
        ...
        if (_pNotation != null)
        {
            Exception::throwE(XML_NOTATION_DEFINED, XML_NOTATION_DEFINED, namedef->toString(), null);
        }
        _pNotation = new Notation(name);
        _pDTD->addNotation(_pNotation);

        _createDTDNode(_pFactory, pSource, _pDocTypeNode, XML_NOTATION, false, false, NULL, 0, 0, namedef, ppNodeChild);
    ...
    /* [out] */ PVOID* ppNodeChild)
{
    ...
    case XML_NAME:
        if (_dwAttrType == XML_NDATA)
        {
            ...
            {
                _pDTD->addForwardRef(_pDeclNamedef, name, pSource->GetLineNumber(), pSource->GetLinePosition(), false, DTD::REF_NOTATION);
            }
    ...
    /* [out] */ PVOID* ppNodeChild)
{
    ...
    if (_fBuildDefNode && _pAttDef->_pDefNode == NULL)
    {
        ...
        {
            _createDTDNode(_pFactory, pSource, NULL, XML_ELEMENT, false, false, NULL, 0, 0, _pElementDecl->getNameDef(), &pNode);
            ...
            _endChildren(_pFactory, pSource, (Object*)(_pElementDecl->_pNode), XML_ELEMENT, TRUE, NULL);
        }
        // Create a node to hold the default value nodes
        _createDTDNode(_pFactory, pSource, _pElementDecl->_pNode, XML_ATTRIBUTE, true, false, NULL, 0, 0, _pDeclNamedef, &pNode);
...
HRESULT DTDNodeFactory::_parseEntities(IXMLNodeSource* pSource)
{
    HRESULT hr = S_OK;
...
void DTDNodeFactory::checkEntityRefLoop()
{
...
HRESULT DTDNodeFactory::_parseEntity(IXMLNodeSource* pSource)
{
    HRESULT hr = S_OK;
    bool external = false;
    ATCHAR* atext;
    ...
    if (_fParsingEntity)
    {
        ...
        if (FAILED(hr))
        {
            ...
            String* msg = AddError(hr, Resources::FormatMessage(XML_PARSING_ENTITY, _pEntity->getName()->toString(), msgError, null));
...
String* DTDNodeFactory::AddError(HRESULT hr, String* error)
{
    String* msg = null;
    TRY
    {
...
void _createDTDNode(IXMLNodeFactory* pFactory,
...
void DTDNodeFactory::FixNamesInDTD()
{
    ElementDecl * pDecl;
    ...
    while (pDecl = (ElementDecl*)pElementDecls->nextElement())
    {
            ...
            for (; pAttdefs->hasMoreElements();)
            {
                ...
                if ((Atom*)XMLNames::atomURNXMLNS == pAttDef->getName()->getNameSpace())
                {
                    ...
                    if (pAtomPrefix && ((Atom*)XMLNames::atomEmpty != pAtomPrefix))
                    {
                        ...
                        {
                            Exception::throwE(XML_XMLNS_RESERVED, XML_XMLNS_RESERVED, pAttDef->getName()->toString(), null);
                        }
                    } else {
                        pAtomPrefix = null;
                    }

                    _pNamespaceMgr->pushScope(pAtomPrefix, null, pSrcURN, pDecl);
                }
            }
        }
        ...
        pTempElementDecls->put(pDecl->getName(), pDecl);// Add elementdecl to a temp hashtable
    }
        ...
        for (int i = pSymbols->size() - 1; i >= 0; i--)
        {
            ...
            if (pName != XMLNames::name(NAME_PCDATA))
            {
                ...
                if (pDecl1) {
                    ...
                    Exception::throwE(XML_ELEMENT_UNDEFINED, XML_ELEMENT_UNDEFINED, pName->toString(), null);
                }
            }
        }
    }
...
Error:
    Exception::throwE(XML_XMLNS_UNDEFINED, XML_XMLNS_UNDEFINED, pAtomPrefix->toString(), null);
...
HRESULT DTDNodeFactory::FinishAttDefs()
{
    ...
    while (pDecl = (ElementDecl*)pElementDecls->nextElement())
    {
        ...
        if (pAttdefs)
        {
            for (; pAttdefs->hasMoreElements();)
            {
                ...
                // Now set and validate the value of the default node according to the pAttDef itself.
                // This is done at the end of the DTD so that we can support forward references to entities in the default value.
