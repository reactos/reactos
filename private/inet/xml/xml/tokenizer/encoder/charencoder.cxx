...
int CharEncoder::getCharsetInfo(const TCHAR * charset, CODEPAGE * pcodepage, UINT * mCharSize)
{
    ...
    for (int i = LENGTH(charsetInfo) - 1; i >= 0; i--) {
        if (StrCmpI(charset, charsetInfo[i].charset) == 0) {
            // test whether we can handle it locally or not BUGBUG(HACK) the index number may change if we change charsetInfo
...
HRESULT CharEncoder::getWideCharFromMultiByteInfo(Encoding * encoding,
                                                  CODEPAGE * pcodepage,
...
HRESULT CharEncoder::getWideCharToMultiByteInfo(Encoding * encoding,
                                                CODEPAGE * pcodepage,
...
HRESULT CharEncoder::wideCharFromUtf8(DWORD* pdwMode, CODEPAGE codepage, BYTE* bytebuffer, UINT * cb, TCHAR * buffer, UINT * cch)
/*
 * Scans rawbuffer and translates UTF8 characters into UNICODE characters
 */
{
#if 0
    // Just for the record - I tried this and measured it and it's twice as slow as our hand-crafted code.
    ...
    // and third bytes are easy to identify - they always start with the bit pattern 0x10xxxxxx.
    ...
    while (remaining > 0 && count < max) {
        ...
        case 4:
            ...
            if (valid) {
                ...
                *buffer++ = (USHORT)((ucs4 - 0x10000) / 0x400 + 0xd800);
                ...
                c = (USHORT)((ucs4 - 0x10000) % 0x400 + 0xdc00);// second ucs2 char
            }
            break;
        ...
        // the line and character where the error occurred and couldn't produce a detailed error message.
...
Cleanup:
    // tell caller that there are bytes remaining in the buffer to be processed next time around when we have more data.
    *cb -= remaining;
    *cch = count;
    return S_OK;
}
...
HRESULT CharEncoder::wideCharFromUcs2Bigendian(DWORD* pdwMode, CODEPAGE codepage, BYTE* bytebuffer, UINT * cb, TCHAR * buffer, UINT * cch)
/*
...
HRESULT CharEncoder::wideCharFromUcs2Littleendian(DWORD* pdwMode, CODEPAGE codepage, BYTE* bytebuffer, UINT * cb, TCHAR * buffer, UINT * cch)
/*
...
HRESULT CharEncoder::wideCharFromUcs4Bigendian(DWORD* pdwMode, CODEPAGE codepage, BYTE* bytebuffer, UINT * cb, TCHAR * buffer, UINT * cch)
/*
...
HRESULT CharEncoder::wideCharFromUcs4Littleendian(DWORD* pdwMode, CODEPAGE codepage, BYTE* bytebuffer, UINT * cb, TCHAR * buffer, UINT * cch)
/*
...
HRESULT CharEncoder::wideCharFromMultiByteWin32(DWORD* pdwMode, CODEPAGE codepage, BYTE* bytebuffer, UINT * cb, TCHAR * buffer, UINT * cch)
/*
 * Scans bytebuffer and translates characters of charSet identified by
 * <code> codepage </code> into UNICODE characters, using Win32 function MultiByteToWideChar() for encoding
 */
{
    HRESULT hr = S_OK;

    *cch = ::MultiByteToWideChar(codepage, MB_PRECOMPOSED, (char*)bytebuffer, *cb, buffer, *cch);
    if (*cch == 0)
        hr = GetLastError();
...
HRESULT CharEncoder::wideCharFromMultiByteMlang(DWORD* pdwMode, CODEPAGE codepage, BYTE* bytebuffer, UINT * cb, TCHAR * buffer, UINT * cch)
/*
 * Scans bytebuffer and translates multibyte characters into UNICODE characters, using Mlang for encoding
 */
{
    HRESULT hr;
    checkhr2(_EnsureMultiLanguage());
    checkhr2(pMultiLanguage->ConvertStringToUnicode(pdwMode, codepage, (char*)bytebuffer, cb, buffer, cch));
    return S_OK;
}


HRESULT CharEncoder::wideCharToUcs2Bigendian(DWORD* pdwMode, CODEPAGE codepage, TCHAR * buffer, UINT *cch, BYTE* bytebuffer, UINT * cb)
/*
 ...
 */
{
    ...
    // Currently we just throw away the high WORD - but I don't know how else to do it, since UCS2 is 2-byte unicode by definition.
    for (UINT i = num; i > 0; i--) {
        *bytebuffer++ = (*buffer) >> 8;
...
HRESULT CharEncoder::wideCharToUcs2Littleendian(DWORD* pdwMode, CODEPAGE codepage, TCHAR * buffer, UINT *cch, BYTE* bytebuffer, UINT * cb)
/*
...
HRESULT CharEncoder::wideCharToUcs4Bigendian(DWORD* pdwMode, CODEPAGE codepage, TCHAR * buffer, UINT *cch, BYTE* bytebuffer, UINT * cb)
/*
...
HRESULT CharEncoder::wideCharToUcs4Littleendian(DWORD* pdwMode, CODEPAGE codepage, TCHAR * buffer, UINT *cch, BYTE* bytebuffer, UINT * cb)
/*
...
HRESULT CharEncoder::wideCharToUtf8(DWORD* pdwMode, CODEPAGE codepage, TCHAR * buffer, UINT *cch, BYTE* bytebuffer, UINT * cb)
/*
...
HRESULT CharEncoder::wideCharToMultiByteWin32(DWORD* pdwMode, CODEPAGE codepage, TCHAR * buffer, UINT *cch, BYTE* bytebuffer, UINT * cb)
/*
...
HRESULT CharEncoder::wideCharToMultiByteMlang(DWORD* pdwMode, CODEPAGE codepage, TCHAR * buffer, UINT *cch, BYTE* bytebuffer, UINT * cb)
/*
 ...
 */
{
    HRESULT hr;
    checkhr2(_EnsureMultiLanguage());
    checkhr2(pMultiLanguage->ConvertStringFromUnicode(pdwMode, codepage, buffer, cch, (char*)bytebuffer, cb));
    return S_OK;
}
