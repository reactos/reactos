...
private:
    ...
    CXMLScriptEngine(HRESULT *phr);

    // Private functions
    IHTMLElement *GetMyScriptElement();
    HRESULT AttachToXMLParser(IXMLDOMDocument **ppDoc);
    BOOL IsSecure(IHTMLElement *pElem);
    HRESULT GetSrcAttrib(IHTMLElement *pElem);
    HRESULT RaiseScriptError(ULONG   iLineNumber, LONG    iCharNumber, LPCWSTR pcwszDescription);
