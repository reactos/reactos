...
Viewer::~Viewer()
{
    ...
    // BUGBUG right now for some reason the shell hangs if we run xsl script and the download thread is around.
    // Our DllCanUnloadNow never gets called
...
Viewer::init(void)
{
    ...
    hr = CoCreateInstance(CLSID_HTMLDocument, reinterpret_cast<IUnknown*>(this), CLSCTX_INPROC_SERVER, IID_IUnknown, (void**)&_pTrident);
    CHECKHR(hr);

    hr = CoCreateInstance(CLSID_DOMDocument, NULL, CLSCTX_INPROC_SERVER, IID_IXMLDOMDocument, (void**)&_pDocument);
    CHECKHR(hr);
