        ...
        )
{
         ...
         */
        if ( (ppage = (__sbh_page_t *)VirtualAlloc( NULL, _PAGESIZE_ * _PAGES_PER_REGION, MEM_RESERVE, PAGE_READWRITE )) != NULL )
        {
            /*
            ...
            if ( VirtualAlloc( ppage, _PAGESIZE_ * _PAGES_PER_COMMITMENT, MEM_COMMIT, PAGE_READWRITE ) != NULL )
            {
                ...
                if ( pregnew == pregroot ) {
                    if ( pregroot->p_next_region == NULL )
                        pregroot->p_next_region = pregroot;
                    if ( pregroot->p_prev_region == NULL )
                        pregroot->p_prev_region = pregroot;
                }
                else {
        ...
        )
{
        ...
        do
        {
                 ...
                 (preg->p_first_uncommitted != NULL) )
            {
                 ...
                 * committed)
                 */
                for ( i = 0, pregmap2 = pregmap ; (pregmap2->free_paras_in_page == _UNCOMMITTED_PAGE) && (i < _PAGES_PER_COMMITMENT) ; pregmap2++, i++ ) ;

                /*
                 * Commit the pages.
                 */
                if ( VirtualAlloc( (void *)ppage, i * _PAGESIZE_, MEM_COMMIT, PAGE_READWRITE ) == ppage)
                {
#ifdef PRFDATA
