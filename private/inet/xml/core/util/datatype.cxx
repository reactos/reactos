...
static int _fastcmp(const TCHAR * pwszA, const TCHAR * pwchB, int maxlen)
{
    TCHAR chA, chB;
...
} s_adtparse[DT_USER_DEFINED] =
{
    ...
    { VT_DATE,          // dateTime.iso8601.tz
        ...
        0 },
    { VT_CY,            // fixed.14.4
        NUMPRS_LEADING_WHITE | NUMPRS_TRAILING_WHITE | NUMPRS_LEADING_PLUS | NUMPRS_LEADING_MINUS | NUMPRS_DECIMAL | NUMPRS_THOUSANDS,
        VTBIT_CY },
    { VT_R8,            // float
        NUMPRS_LEADING_WHITE | NUMPRS_TRAILING_WHITE | NUMPRS_LEADING_PLUS | NUMPRS_LEADING_MINUS | NUMPRS_DECIMAL | NUMPRS_EXPONENT,
        VTBIT_R8 },
    ...
    { VT_R8,            // float.IEEE.754.64
        ...
        VTBIT_R8 },
    { VT_I1,            // i1
        NUMPRS_LEADING_WHITE | NUMPRS_TRAILING_WHITE | NUMPRS_LEADING_PLUS | NUMPRS_LEADING_MINUS, 
        VTBIT_I1 },
    { VT_I2,            // i2
        NUMPRS_LEADING_WHITE | NUMPRS_TRAILING_WHITE | NUMPRS_LEADING_PLUS | NUMPRS_LEADING_MINUS,
        VTBIT_I2 },
 ...
 */
HRESULT ParseDatatype(const WCHAR * pS, int cch, DataType datatype, VARIANT * pVar, const WCHAR ** ppwcNext)
{
    HRESULT hr = S_OK;
    UUID uuid;
    ...
    if ((DT_NONE <= datatype && datatype <= DT_AV_NAMEDEF) || DT_URI == datatype)
    {
        if (ppwcNext)
            *ppwcNext = pS + cch;
    ...
    else
    {
        ...
        case DT_UUID: // uuid, Hexadecimal digits representing octets, optional embedded hyphens which should be ignored. VT_BSTR GUID
            hr = ParseUuid(pS, cch, &uuid, &pNext);
            if (hr)
                goto Cleanup;
...
HRESULT UnparseDatatype(String ** ppReturn, VARIANT * pVar, DataType dt)
{
    HRESULT hr = S_OK;
    VARIANT var;
    String * pString = null;
...
static const WCHAR * ParseDecimal(const WCHAR * pS, int cch, WORD * pw)
{
    WCHAR c;
    WORD w = 0;
...
HRESULT ParseBinHex(const WCHAR * pS, long lLen, BYTE * abData, int * pcbSize, const WCHAR ** ppwcNext)
{
    WCHAR wc;
    bool fLow = false;
    const WCHAR * pwc = pS;
...
static void swapBytes(BYTE * pb1, BYTE * pb2)
{
    BYTE t = *pb1;
    *pb1 = *pb2;
    *pb2 = t;
...
HRESULT ParseNumber(const WCHAR * pS, int c, DWORD dwParse, DWORD dwStore, VARIANT * pVar, const WCHAR ** ppwcNext)
{
    HRESULT hr;
    NUMPARSE numparse;
    ...
    if (dwParse & NUMPRS_CURRENCY && // only happens for fixed.14.4
        ((numparse.cDig + numparse.nPwr10 > 14) || (numparse.nPwr10 < -4)))
    {
    ...
    hr = VarNumFromParseNum(&numparse, abDigits, dwStore, pVar);
    if (hr)
        goto Cleanup;
...
HRESULT ParseNumeric(const WCHAR * pS, int cch, DataType datatype, VARIANT * pVar, const WCHAR ** ppwcNext)
{
    HRESULT hr;
    DWORD dwParse;
    DWORD dwVtBits;
