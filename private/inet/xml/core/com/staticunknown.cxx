...
HRESULT CreateMultiLanguage(IMultiLanguage ** ppUnk)
{
    ...
    if (*ppUnk == NULL)
    {
        hr = CoCreateInstance(CLSID_CMultiLanguage, NULL, CLSCTX_INPROC_SERVER, IID_IMultiLanguage2, (void**)ppUnk);
        if (hr)
        {
            hr = CoCreateInstance(CLSID_CMultiLanguage, NULL, CLSCTX_INPROC_SERVER, IID_IMultiLanguage, (void**)ppUnk);
        }
...
HRESULT CreateSecurityManager(IInternetSecurityManager ** ppUnk)
{
    ...
    if (*ppUnk == NULL)
    {
        hr = CoCreateInstance(CLSID_InternetSecurityManager, NULL, CLSCTX_INPROC_SERVER, IID_IInternetSecurityManager, (void **)ppUnk);
        if (!hr)
        {
...
HRESULT CreateCatalogInformation(ICatInformation ** ppUnk)
{
    ...
    if (*ppUnk == NULL)
    {
        hr = CoCreateInstance(CLSID_StdComponentCategoriesMgr, NULL, CLSCTX_INPROC_SERVER, IID_ICatInformation, (void **)ppUnk);
        if (!hr)
        {
            // Must register all static foreign IUnknowns (See base\StaticUnknown.hxx).
            hr = ::RegisterStaticUnknown((IUnknown **)ppUnk);
