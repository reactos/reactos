...
: _riid(riid)
{
    ...
    hr = CoCreateInstance(CLSID_StdGlobalInterfaceTable, 0, CLSCTX_INPROC_SERVER, IID_IGlobalInterfaceTable, (void**)&_pGIT);
    if (FAILED(hr))
        Exception::throwE(hr);
#endif
}
...
HRESULT __gitpointer::_setPointer(IUnknown* ptr)
{
    ...
    if (ptr != NULL)
    {
        ...
        if (!hr)
        {
            hr = CoMarshalInterface(pStm, *_riid, ptr, MSHCTX_INPROC, NULL, MSHLFLAGS_TABLESTRONG);
            if (!hr)
            {
