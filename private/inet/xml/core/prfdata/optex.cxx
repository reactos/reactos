...
BOOL COptex::CommonConstructor(PVOID pszName, BOOL fUnicode,
   ...
   if (m_hevt != NULL) {
      ...
      m_hfm = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(*m_pSharedInfo), sz);
      Assert(m_hfm != NULL);
      if (m_hfm != NULL) {
         m_pSharedInfo = (PSHAREDINFO) MapViewOfFile(m_hfm, FILE_MAP_WRITE, 0, 0, 0);
...
void COptex::Enter() {
   ...
   if (InterlockedIncrement(&m_pSharedInfo->m_lLockCount) == 1) {
      ...
      if (m_pSharedInfo->m_dwThreadId == dwThreadId) {
         m_pSharedInfo->m_lRecurseCount++;// Optex is owned by this thread, own it again
      } else {
         ...
         INTERLOCKEDEXCHANGE_PTR(&m_pSharedInfo->m_dwThreadId, dwThreadId); // We own it now
...
BOOL COptex::TryEnter() {
   ...
   do {
      fThisThreadOwnsTheOptex = (0 == (DWORD_PTR)InterlockedCompareExchangePointer((PVOID*) &m_pSharedInfo->m_lLockCount, (PVOID) 1, (PVOID) 0));

      if (fThisThreadOwnsTheOptex) {
         // We now own the Optex
         INTERLOCKEDEXCHANGE_PTR(&m_pSharedInfo->m_dwThreadId, dwThreadId); // We own it
         m_pSharedInfo->m_lRecurseCount = 1;       // We own it once
      } else {
   ...
   return(fThisThreadOwnsTheOptex);// Return whether or not this thread owns the Optex
}
...
void COptex::Leave() {
   ...
   if (--m_pSharedInfo->m_lRecurseCount > 0) {
      InterlockedDecrement(&m_pSharedInfo->m_lLockCount);// We still own the Optex
   } else {// We don't own the Optex
      INTERLOCKEDEXCHANGE_PTR(&m_pSharedInfo->m_dwThreadId, 0);
      if (InterlockedDecrement(&m_pSharedInfo->m_lLockCount) > 0) {
         SetEvent(m_hevt);// Other threads are waiting, wake one of them
      }
   }
}
