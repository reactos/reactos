...
void CPrfData::InstallPrfData(LPCWSTR pszDllPathname)
{
   ...
   WCHAR szSubkey[100];
   wsprintfW(szSubkey, L"SYSTEM\\CurrentControlSet\\Services\\%s\\Performance", m_pszAppName);
...
DWORD CPrfData::ActivatePrfData() {
   ...
   m_hfm = CreateFileMappingW(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, cbBytesNeededForAllObjs, m_pszAppName);
   ...
   for (ObjOrd = 0; ObjOrd < m_nNumObjects; ObjOrd++) {
      ...
      pm.pPOT->DefinitionLength     = sizeof(PERF_OBJECT_TYPE) + sizeof(PERF_COUNTER_DEFINITION) * (INT)PtrToInt(pPrfObj->NumCounters);
