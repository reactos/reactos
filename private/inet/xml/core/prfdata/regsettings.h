...
public:  // Opening/closing registry subkey functions
   ...
   LONG OpenSubkey(BOOL fReadOnly, HKEY hkeyRoot, LPCTSTR pszSubkey) {
      ...
      LONG lErr;
      if (fReadOnly) {
         lErr = ::RegOpenKeyEx(hkeyRoot, pszSubkey, 0, KEY_QUERY_VALUE, &m_hkeySubkey);
      } else {
         DWORD dwDisposition;
         lErr = RegCreateKeyEx(hkeyRoot, pszSubkey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_QUERY_VALUE | KEY_SET_VALUE, NULL, &m_hkeySubkey, &dwDisposition);
      }
      return(lErr);
   }
