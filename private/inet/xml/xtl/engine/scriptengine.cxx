...
ScriptEngine * ScriptEngine::newScriptEngine(Processor * xtl, REFCLSID clsid)
{
...
ScriptEngine::ScriptEngine(Processor * xtl, REFCLSID clsid)
{
    ...
    checkhr(CoCreateInstance(clsid, NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void **)&_unk));
...
void ScriptEngine::addScriptText(String * str)
{
    if (!_scriptText)
    {
...
void ScriptEngine::parseScriptText()
{
    VARIANT var;
    HRESULT hr;

    _as->addNamedItem(String::newString(_T("Context")), (SCRIPTITEM_GLOBALMEMBERS | SCRIPTITEM_ISPERSISTENT | SCRIPTITEM_ISSOURCE));
    ...
    if (_scriptText)
    {
        ...
        _xtl->markReadOnly();
        hr = _asp->parseScriptText(_scriptText->toString(), null, null, null, 0, 1, SCRIPTTEXT_ISVISIBLE, &var);
        VariantClear(&var);
        if (hr)
        {
            throwError(hr);
...
void ScriptEngine::parseScriptText(Processor * xtl, String * text, VARTYPE vt, VARIANT * pvar)
{
    HRESULT hr;
    ...
    if (text)
    {
        ...
        hr = _asp->parseScriptText(text, null, null, null, 0, 1, SCRIPTTEXT_ISEXPRESSION, pvar);
        if (hr)
        {
            throwError(hr);
        }
...
void ScriptEngine::throwError(HRESULT hr)
{
