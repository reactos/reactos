    ...
    DWORD_PTR dwReserved)
{
    if (_fSecurityContext)
    {
        ...
        return g_pSecurityMgr->GetSecurityId((LPCWSTR)_bstrURL, pbSecurityId, pcbSecurityId, dwReserved);
    }
    else
    {
...
SecurityManager::CheckSafety(SAFETYOPERATION sOperation, REFIID riid, CLSID clsid, IUnknown *pUnk)
{
    ...
    if (SUCCEEDED(hr) && pOSafe)
    {
        ...
        if (sOperation == SAFETY_SCRIPTENGINE)
        {
            ...
            hr = pOSafe->GetInterfaceSafetyOptions(riid, &dwMask, &dwEnabled);
...
HRESULT CompatFlagsFromClsid(REFCLSID clsid, DWORD *pdwCompat)
{
    ...
    *pdwCompat = 0;
    ...
    // find the table root
    hr = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szTableRootKey, 0, KEY_READ, &hkeyRoot);
    ...
    hr = RegOpenKeyEx(hkeyRoot, pszClsid, 0, KEY_READ, &hkeyClsid);
    ...
    hr = RegQueryValueEx(hkeyClsid, szCompatFlags, NULL, &dwType, (LPBYTE)&rkValue, &dwSize);
