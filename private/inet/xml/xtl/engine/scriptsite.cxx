...
static bool DoDebugXSLScript()
{
    ...
    if (fCheckRegistry)
    {
        ...
        dwResult = RegOpenKeyEx(HKEY_CURRENT_USER, s_RegKeyMSXML, 0, KEY_QUERY_VALUE, &hKey);
        if (ERROR_SUCCESS == dwResult)
        {
            dwResult = RegQueryValueEx(hKey, s_XSLScriptDebug, NULL, &dwType, (LPBYTE) &dwValue, &dwSize);
            RegCloseKey(hKey);
        }
