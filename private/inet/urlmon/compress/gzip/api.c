...
HRESULT WINAPI ResetCompression(PVOID void_context)
{
    ...
    // BUGBUG This forces a realloc of the particular compressor we are using each time we reset, but if we don't do this then we are stuck with one
    // compressor (fast,std,optimal) forever until we destroy the context.
    // Should create a workaround for this problem.  Luckily, IIS creates a new context all the time, and doesn't call reset (so says davidtr).
