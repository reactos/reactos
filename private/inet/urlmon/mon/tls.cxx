...
HRESULT RemoveTSFromList(DWORD tid)
{
    ...
    while( pts )
    {
        if( pts->_dwTID == tid )
        {
            if( ptsPrev == NULL )
            {
                g_pHeadURLMONTSList = pts->_pNext;// this is the head of the list
            }
            else
            {
            ...
            DbgLog2(tagUrlDllErr, NULL, ">>> tid: %lx -> delete hwnd:%p", tid, pts->_hwndNotify);
...
HRESULT CleanupTSOnProcessDetach()
{
    ...
    while( pts )
    {
        ...
        {   // destroy the window (the owner thread can do so)
            DestroyWindow(pts->_hwndNotify);
            DbgLog2(tagUrlDllErr, NULL, ">>> tid: %lx -> DestroyWindow :%p", pts->_dwTID, pts->_hwndNotify);
        }
        else
        {
            ...
            SetWindowLongPtr(pts->_hwndNotify, GWLP_WNDPROC, (LONG_PTR)DefWindowProc); // set wndproc to user32's

            // post message
            PostMessage(pts->_hwndNotify, WM_CLOSE, 0, 0);
            DbgLog2(tagUrlDllErr, NULL, ">>> tid: %lx -> PostMessage WM_CLOSE :%p", pts->_dwTID, pts->_hwndNotify);
        }

        ptsToFree = pts;// save this pts since we are to free it
        pts = pts->_pNext;// walk down the list
    ...
    g_pHeadURLMONTSList = NULL;// mark list empty
    return NOERROR;
}
...
HRESULT CUrlMkTls::TLSAllocData(void)
{
    ...
    _pData = (SUrlMkTlsData *) HeapAlloc(g_hHeap, HEAP_SERIALIZE, sizeof(SUrlMkTlsData));
    if (_pData)
    {
        // This avoids having to set most fields to NULL, 0, etc and
        // is needed cause on debug builds memory is not guaranteed to be zeroed.
        memset(_pData, 0, sizeof(SUrlMkTlsData));

        _pData->dwFlags = URLMKTLS_LOCALTID;// fill in the non-zero values

        if (TlsSetValue(gTlsIndex, _pData))// store the data ptr in TLS
        {
            return S_OK;
        }
...
//  Synopsis:   Called to perform cleanup on all this threads data structures, and to call CoUninitialize() if needed.
...
void DoThreadCleanup(BOOL bInThreadDetach)
{
    ...
    if (pTls != NULL)
    {
        ...
        // Because of the DLL unload rules in NT we need to be careful what we do in clean up.
        // We notify the routines with special behavior here.
        ...
        if (pTls->pCTransMgr != NULL)
        {
            ...
            // AND we're not really in ThreadDetach, then we have other references on the Transaction Manager.
            // Put back our reference and leave.
            if (pTls->pCTransMgr->Release() && bInThreadDetach == FALSE)
            {
        ...
        TlsSetValue(gTlsIndex, NULL);// reset the index so we dont find this data again.
        ...
        {
            DbgLog1(tagUrlDllErr, NULL, ">>> tld: %lx -> hwnd == NULL ", tid );
        }
    ...
    // else
    // there is no TLS for this thread, so there can't be anything to cleanup.
...
//  Notes:      other one time initialization occurs in ctors for global objects
//  WARNING:    if we are called because of FreeLibrary, then we should do as much cleanup as we can.
//              If we are called because of process termination, we should not do any cleanup, as other threads in
//              this process will have already been killed, potentially while holding locks around resources.
STDAPI_(BOOL) TlsDllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpvReserved )
{
    ...
    if (dwReason == DLL_THREAD_ATTACH || dwReason == DLL_THREAD_DETACH)
        PerfDbgLog1(tagUrlDll, NULL, "+TlsDllMain %s", dwReason == DLL_THREAD_ATTACH ? "DLL_THREAD_ATTACH" : "DLL_THREAD_DETACH");
    #endif

    switch (dwReason)
    {
    case DLL_THREAD_ATTACH:// new thread is starting
        {
            HRESULT hr;
            CUrlMkTls tls(hr);
            if (FAILED(hr))
        ...
        break;
    case DLL_THREAD_DETACH:// Thread is exiting, clean up resources associated with threads.
        DoThreadCleanup(TRUE);
        break;
    case DLL_PROCESS_ATTACH:// Initial setup. Get a thread local storage index for use by OLE
        g_hInst = hInstance;
    ...
    case DLL_PROCESS_DETACH:
        ...
        //if (NULL == lpvReserved)
        {
            ...
            // DLL_PROCESS_DETACH is called when we unload. The thread that is currently calling has not done thread specific cleanup yet.
            DoThreadCleanup(TRUE);
