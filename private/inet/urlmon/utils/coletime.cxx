...
const COleDateTime& COleDateTime::operator=(const time_t& timeSrc)
{
    ...
    if (ptm != NULL)
    {
        m_status = OleDateFromTm((WORD)ptm->tm_year + 1900,
            (WORD)(ptm->tm_mon + 1), (WORD)ptm->tm_mday, (WORD)ptm->tm_hour, (WORD)ptm->tm_min, (WORD)ptm->tm_sec, m_dt) ? valid : invalid;
    }
    else
    {
...
const COleDateTime& COleDateTime::operator=(const FILETIME& filetimeSrc)
{
    ...
    else
    {
        ...
        if (GetStatus() == valid)
        {
            m_status = OleDateFromTm(systime.wYear, systime.wMonth, systime.wDay, systime.wHour, systime.wMinute, systime.wSecond, m_dt) ? valid : invalid;
        }
    }
...
BOOL COleDateTime::SetDateTime(int nYear, int nMonth, int nDay, int nHour, int nMin, int nSec)
{
...
BOOL COleDateTime::ParseDateTime(LPCTSTR lpszDate, DWORD dwFlags, LCID lcid)
{
    ...
    SCODE sc;
    if (FAILED(sc = VarDateFromStr((LPOLESTR)T2COLE(strDate), lcid, dwFlags, &m_dt)))
    {
