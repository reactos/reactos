    ...
    if(SUCCEEDED(GetAttributeA(pSoftDist, DU_ATTRIB_STYLE, szStyle, MAX_PATH))){
        ...
        if (m_Style == STYLE_UNKNOWN) {
            ...
            if (m_sdMSInstall == NULL) {
                ...
                if (SUCCEEDED(CLSIDFromProgID(wszStyle, &clsidOther))) {
                    ...
                    if (FAILED(hr = CoCreateInstance(clsidOther, NULL, CLSCTX_INPROC_SERVER, IID_ISoftDistExt, (void **)&m_sdMSInstall)))
                        goto Exit;
        ...
        if (m_Style == STYLE_MSINSTALL) {
            if (m_sdMSInstall == NULL) {
                ...
                if (FAILED(hr = CoCreateInstance(clsidMSInstall, NULL, CLSCTX_INPROC_SERVER, IID_ISoftDistExt, (void **)&m_sdMSInstall)))
                    goto Exit;
            }
        }
...
CSoftDist::Logo3Advertise(BOOL bFullAdvt)
{
    ...
    if (bFullAdvt || IsAbstractDifferent(hkeyAvailVersion, m_szAbstract)) {
        ...
        if ((lResult == ERROR_SUCCESS) && m_szHREF) {
            lResult = ::RegSetValueEx(hkeyAvailVersion, szHREF, NULL, REG_SZ, (unsigned char *)m_szHREF, lstrlen(m_szHREF) + 1);
        }

        // save Abstract
        if ((lResult == ERROR_SUCCESS) && m_szAbstract) {
            lResult = RegSetValueEx(hkeyAvailVersion, REGVAL_ABSTRACT_AVAILABLE, NULL, REG_SZ, (unsigned char *)m_szAbstract, lstrlen(m_szAbstract) + 1);
        }

        // save Title
        if ((lResult == ERROR_SUCCESS) && m_szTitle) {
            lResult = ::RegSetValueEx(hkeyAvailVersion, szTitle, NULL, REG_SZ, (unsigned char *)m_szTitle, lstrlen(m_szTitle) + 1);
        }
...
CSoftDist::ICDAdvertise(BOOL bFullAdvt)
{
    ...
    while (pos != NULL) {
        ...
        if ((lResult = ::RegSetValueEx(hkeyDownloadInfo, szTmpCodeBase, NULL, REG_SZ, (unsigned char *)pszCodeBase, strlen(pszCodeBase))) != ERROR_SUCCESS) {
    ...
    // save HREF
    if ( (lResult == ERROR_SUCCESS) && m_szHREF)
        lResult = ::RegSetValueEx(hkeyVersion, szHREF, NULL, REG_SZ, (unsigned char *)m_szHREF, lstrlen(m_szHREF)+1);
    // save Abstract
    if ( (lResult == ERROR_SUCCESS) && m_szAbstract)
        lResult = ::RegSetValueEx(hkeyVersion, REGVAL_ABSTRACT_AVAILABLE, NULL, REG_SZ, (unsigned char *)m_szAbstract, lstrlen(m_szAbstract)+1);
...
HRESULT CSoftDist::ActSetupAdvertise(BOOL bFullAdvt)
{
    ...
    lstrcat(szKey, "\\");
    lstrcat(szKey, pszDist);
    if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, szKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_READ|KEY_SET_VALUE, NULL, &hKey, NULL) == ERROR_SUCCESS)
    {
        DWORD dwAdStateInit = SOFTDIST_ADSTATE_NONE;

        // Note Version
        wsprintf(szKey, "%d,%d,%d,%d", (m_dwVersionMS & 0xffff0000)>>16, (m_dwVersionMS & 0xffff), (m_dwVersionLS & 0xffff0000)>>16, (m_dwVersionLS & 0xffff));
        lResult = ::RegSetValueEx(hKey, REGVAL_VERSION_AVAILABLE, NULL, REG_SZ, (unsigned char *)szKey, lstrlen(szKey)+1);
        ...
        if (bFullAdvt || IsAbstractDifferent(hKey, m_szAbstract)) {
            ...
            if( (lResult == ERROR_SUCCESS) && m_szTitle)
                lResult=RegSetValueEx(hKey,REGVAL_TITLE_AVAILABLE, NULL, REG_SZ, (unsigned char *)m_szTitle, lstrlen(m_szTitle)+1);

            if( (lResult == ERROR_SUCCESS) && m_szAbstract)
                lResult=RegSetValueEx(hKey,REGVAL_ABSTRACT_AVAILABLE, NULL, REG_SZ, (unsigned char *)m_szAbstract, lstrlen(m_szAbstract)+1);
    ...
    /*
                ...
                if (lstrcmpi(pszDist, DISTUNIT_NAME_IE4) == 0) {
                    ...
                    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_IE_MAIN, 0, KEY_SET_VALUE, &hKeyIE) == ERROR_SUCCESS) {
                        ...
                        RegSetValueEx(hKeyIE,REGVAL_FIRST_HOME_PAGE, NULL, REG_SZ, (unsigned char *)m_szHREF, lstrlen(m_szHREF)+1);
                        RegCloseKey(hKeyIE);
                    }
                }
    */
    ...
    if ( SUCCEEDED(hr) )
    {
        if (dwStyle == STYLE_MSICD)
        {
            ...
            if (RegCreateKey(HKEY_LOCAL_MACHINE, szTmp, &hkeyAdvertisedVersion) == ERROR_SUCCESS)
            {
                ...
                lResult = ::RegSetValueEx(hkeyAdvertisedVersion, NULL, NULL, REG_SZ, (unsigned char *)szTmp, lstrlen(szTmp)+1);
                if ( lResult == ERROR_SUCCESS )
                {
                    lResult = ::RegSetValueEx(hkeyAdvertisedVersion, REGVAL_ADSTATE, NULL, REG_DWORD, (unsigned char *)&dwAdState, sizeof(DWORD) );
                    if ( lResult != ERROR_SUCCESS )
        ...
        else if (dwStyle == STYLE_LOGO3)
        {
            ...
            if (RegCreateKey(HKEY_LOCAL_MACHINE, szTmp, &hkeyAvailableVersion) == ERROR_SUCCESS)
            {
                ...
                wsprintf(szTmp, "%d,%d,%d,%d",
                        ...
                        (dwAdvertisedVersionLS & 0xffff0000)>>16,
                        (dwAdvertisedVersionLS & 0xffff));
                lResult = ::RegSetValueEx(hkeyAvailableVersion, REGSTR_LOGO3_ADVERTISED_VERSION, NULL, REG_SZ, (unsigned char *)szTmp, lstrlen(szTmp)+1);
                if ( lResult == ERROR_SUCCESS )
                {
                    lResult = ::RegSetValueEx(hkeyAvailableVersion, REGVAL_ADSTATE, NULL, REG_DWORD, (unsigned char *)&dwAdState, sizeof(DWORD) );
                    if ( lResult != ERROR_SUCCESS )
                        hr = HRESULT_FROM_WIN32( lResult );
                }
                else
        ...
        else
        {
            ...
            if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, szKey, 0, KEY_WRITE , &hKey ) == ERROR_SUCCESS)
            {
                ...
                wsprintf(szKey, "%d,%d,%d,%d",
                        ...
                        (dwAdvertisedVersionLS & 0xffff0000)>>16,
                        (dwAdvertisedVersionLS & 0xffff));
                lResult = ::RegSetValueEx(hKey, REGVAL_VERSION_ADVERTISED, NULL, REG_SZ, (unsigned char *)szKey, lstrlen(szKey)+1);
                if ( lResult == ERROR_SUCCESS )
                {
                    lResult = ::RegSetValueEx(hKey, REGVAL_ADSTATE, NULL, REG_DWORD, (unsigned char *)&dwAdState, sizeof(DWORD) );
                    if ( lResult != ERROR_SUCCESS )
                        hr = HRESULT_FROM_WIN32( lResult );
                }
                else
