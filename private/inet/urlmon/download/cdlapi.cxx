...
LONG RegisterNewActiveXCacheFolder(LPCTSTR lpszNewPath, DWORD dwPathLen)
{
    ...
    lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_IE_SETTINGS, 0, KEY_ALL_ACCESS, &hkeyIntSetting);
    if (lResult != ERROR_SUCCESS)
        goto Exit;

    // read ActiveXCache
    lResult = RegQueryValueEx(hkeyIntSetting, g_szActiveXCache, NULL, NULL, (LPBYTE)szOldPath, &dwLen);
    if (lResult != ERROR_SUCCESS)
    {
        fOldFound = TRUE;
        szOldPath[0] = '\0';
...
LONG SwitchToNewCachePath(LPCSTR lpszNewPath, DWORD dwPathLen)
{
    ...
    lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szKey, 0, KEY_ALL_ACCESS, &hkey);
    for (; lResult == ERROR_SUCCESS; dwLen = MAX_PATH)
    {
        lResult = RegEnumValue(hkey, dwIndex++, szKey, &dwLen, NULL, NULL, (LPBYTE)szPath, &dwLen);
...
SetCoInstall()
{
    ...
    if (!g_bCheckedForCoInstall && !g_pfnCoInstall)
    {
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_IE_SETTINGS, 0, KEY_READ, &hKeyIESettings) == ERROR_SUCCESS) {
            if (RegQueryValueEx(hKeyIESettings, REGVAL_USE_COINSTALL, NULL, NULL, NULL, 0) != ERROR_SUCCESS) {
...
//    static const char szActiveXCache[] = "ActiveXCache";
    ...
    if (!g_fHaveCacheDir) {
        ...
        lResult = ::RegOpenKeyEx( HKEY_LOCAL_MACHINE, REGSTR_PATH_IE_SETTINGS, 0, KEY_READ, &hKeyIESettings );
        if (lResult != ERROR_SUCCESS) {
            ...
            !PathFileExists( g_szOCXCacheDir ) )
        {
            ...
            if ( lResult != ERROR_SUCCESS ) {
                ...
                if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, REGSTR_PATH_IE_SETTINGS,0, KEY_ALL_ACCESS, &hkeyWriteIESettings ) == ERROR_SUCCESS) {
                    RegSetValueEx(hkeyWriteIESettings,g_szActiveXCache,0,REG_SZ, (LPBYTE)g_szOCXCacheDir, lstrlen(g_szOCXCacheDir) + 1);
   ...
   GetActiveXSafetyProvider(IActiveXSafetyProvider **ppProvider)
{
    ...
    hr = CoCreateInstance(CLSID_IActiveXSafetyProvider, NULL, CLSCTX_INPROC_SERVER, IID_IActiveXSafetyProvider, (void **)ppProvider);
    return hr;
}
    ...
    IUnknown **ppUnk)                   // pass back pUnk for synchronous case
{
    ...
    if (szDistUnit || !bNullClsid ) {
        ...
        if (pcdd->dwFlags & CD_FLAGS_EXACT_VERSION) {
            ...
            if (hrAny != S_OK) {
                ...
                if (SUCCEEDED(RegOpenKeyEx(HKEY_LOCAL_MACHINE, REGSTR_PATH_IE_SETTINGS, 0, KEY_READ, &hKeyIESettings))) {
                    if (SUCCEEDED(RegQueryValueEx(hKeyIESettings, g_szActiveXCache, NULL, &dwType, (unsigned char *)g_szOCXCacheDir, &Size))) {
    ...
    if ((flags & CD_FLAGS_NEED_CLASSFACTORY) &&
        ...
        // We don't care about the version. Check if someone registered themselves with COM via CoRegisterClassObject, and if it succeeds, we don't need to download.
        hrResult = CoGetClassObject(((pPluginFileName) ? (inclsid) : (myclsid)), dwClsContext, pvReserved, riid, (void**)&punk);
...
LPCTSTR GetZeroImpactRootDir()
{
    ...
    else
    {
        ...
        if(lResult == ERROR_SUCCESS)
        {
            lResult = ::RegQueryValueEx(hKeyIESettings, g_szActiveXCache, NULL, &dwType, (BYTE *)g_szZeroImpactRootDir, &Size);
            if(lResult == ERROR_SUCCESS)
