...
HRESULT SetComponentDeclined(LPCWSTR pwszDistUnit, LPSTR pszSecId)
{
...
HRESULT CCodeDownload::DelayRegisterOCX(LPCSTR pszSrcFileName, FILEXTN extn)
{
    HRESULT hr = S_OK;
...
HRESULT CCodeDownload::UpdateZeroImpactCache()
{
...
HRESULT CCodeDownload::HandleUnSafeAbort()
{
    HRESULT hr = S_FALSE;
    ...
    if(WaitingForEXE()) { // Did the setup start a self-registering EXE?
        ...
        if (pCodeInstall) {
            ...
            hr = pCodeInstall->OnCodeInstallProblem(CIP_EXE_SELF_REGISTERATION_TIMEOUT, NULL, szBuf, 0);
        } else {
        ...
        // if hr == S_FALSE/E_ABORT ignore this EXE and try to proceed with rest of installation
...
HRESULT CCodeDownload::SelfRegEXETimeout()
{
    HRESULT hr = S_OK;
...
HRESULT CCodeDownload::SetManifest(FILEXTN extn, LPCSTR szManifest)
{
    HRESULT hr = S_OK;
...
BOOL CCodeDownload::VersionFromManifest(LPSTR szVersionInManifest)
{
...
HRESULT CCodeDownload::ProcessJavaManifest(IXMLElement *pJava, const char *szOSD, char *szOSDBaseName, CDownload *pdl)
{
    ...
    while( (GetNextChildTag(pJava, DU_TAG_PACKAGE, &pPackage, nLastPackage)) == S_OK) {
        ...
        // check if package of right version is already locally installed if so go to next package
        ...
        if (hr == S_OK) {
            ...
            // for a NOSETUP CJavaSetup it doesn't matter which pdl it gets added on to.
...
HRESULT CCodeDownload::ProcessNativeCode(CDownload *pdl, IXMLElement *pNativeCode)
{
    ...
    while (GetNextChildTag(pNativeCode, DU_TAG_CODE, &pCode, nLast2) == S_OK) {
        ...
        if(! IsZeroImpact())
        {
            ...
            if (m_bExactVersion) {
                hrExact = IsControlLocallyInstalled(szNativeName, (LPCLSID)&clsid, NULL, dwVersionMS, dwVersionLS, &lci, GetDestDirHint(), TRUE);
            }
        ...
        while (GetNextChildTag(pCode, DU_TAG_CONFIG, &pConfig, nLast3) == S_OK) {
            ...
            if (hr == S_OK) {
                ...
                if ((!pcbh || !pcbh->bHREF) && extn == FILEXTN_INF) {
                    ...
                    hr = StartDownload(szNativeName, pdl, szCodeBase, dest, lci.lpDestDir, dwRegisterServer, dwCopyFlags, pcbhList);
                    bDestroyPCBHList = FALSE;
                }
...
HRESULT CCodeDownload::AddDistUnitList(LPWSTR szDistUnit)
{
    HRESULT hr = E_FAIL;
...
#define NUM_VARS            1
    ...
    // if it doesn't exist we'll get just the 2 charact Lang abbreviation and try again, failing that we default to English
...
HRESULT CCodeDownload::GetInfCodeLocation(LPCSTR lpCurCode, LPSTR szURL)
{
    ...
    GetPrivateProfileString(lpCurCode, szPreferredArch, szDefault, szURL, INTERNET_MAX_URL_LENGTH, m_szInf);
    if (szURL[0] != '\0' && lstrcmpi(szURL, szIGNORE) != 0) {
        ...
        GetPrivateProfileString(lpCurCode, szAlternateArch, szDefault, szURL, INTERNET_MAX_URL_LENGTH, m_szInf);
        if (szURL[0]) {
...
#else
    GetPrivateProfileString(lpCurCode, g_szPlatform, szDefault, szURL, INTERNET_MAX_URL_LENGTH, m_szInf);
#endif

    if (szURL[0] == '\0') {
        GetPrivateProfileString(lpCurCode, szLoc, szDefault, szURL, INTERNET_MAX_URL_LENGTH, m_szInf);
    } else {
...
)
{
    ...
    if (GetPrivateProfileString(lpCurCode, szRegisterServerOverride, szDefault, szBuf, MAX_PATH, m_szInf)) {
        *pdwRegisterServer = CST_FLAG_REGISTERSERVER_OVERRIDE;
        if ((szBuf[0] == 'y') || (szBuf[0] == 'Y') || (szBuf[0] == '1') || (lstrcmpi(szBuf, "true") == 0)) {
    ...
    *pbDestDir=GetPrivateProfileInt(lpCurCode, szForceDestDir, 0, m_szInf);// get ForceDestDir flag
    ...
    hr = ConvertANSItoCLSID(szBuf, *plpClsid);// Get CLSID from string
...
HRESULT CCodeDownload::StartDownload(LPSTR szCurCode, CDownload *pdl, LPSTR szURL, DESTINATION_DIR dest, LPSTR szDestDir, DWORD dwRegisterServer, DWORD dwCopyFlags, CList<CCodeBaseHold *, CCodeBaseHold *> *pcbhList)
{
    ...
    // CAB
    if ((pdl->GetExtn() == FILEXTN_CAB) && (lstrcmpi(szTHISCAB, szURL) == 0)) {
        pdl->AddSetupToExistingCAB(szCurCode, szDestDir, dest, dwRegisterServer, dwCopyFlags);
        goto Exit;
    }
...
VOID CCodeDownload::ProcessInf(CDownload *pdl)
{
    ...
    if (m_bExactVersion) {
        hrExact = IsControlLocallyInstalled(szCurCode, lpclsid, NULL, dwFileVersionMS, dwFileVersionLS, &lci, GetDestDirHint(), TRUE);
    }

    hrAny = IsControlLocallyInstalled(szCurCode, lpclsid, NULL, dwFileVersionMS, dwFileVersionLS, &lci, GetDestDirHint(), FALSE);
    if (m_bExactVersion && hrExact == S_FALSE && hrAny == S_OK) {
        // Newer version exists on the machine.
        // Check if we are going to install outside of DPF and disallow if we are going to downgrade.
        BOOL bIsDPFComponent = FALSE;
        CHAR szOCXCacheDirSFN[MAX_PATH];
        CHAR szFNameSFN[MAX_PATH];
...
HRESULT CCodeDownload::QueueModuleUsage(LPCSTR szFileName, LONG muFlags)
{
    HRESULT hr = S_OK;
...
HRESULT CCodeDownload::ProcessHookSection(LPCSTR lpCurHook, CDownload *pdl)
{
    ...
    szCmdLine[0] = '\0';
    GetPrivateProfileString(lpCurHook, szCMDLINE, szDefault, szCmdLine, MAX_PATH, m_szInf);
...
Complete_failed:
    ...
    // in some post binding processing such as verifytrust cab extraction etc and so we need to post a DoSetup() packet with UserCancelled flag set.
...
HRESULT CCodeDownload::ResolveCacheDirNameConflicts()
{
    HRESULT hr = S_OK;
    char szDir[MAX_PATH];
...
HRESULT CCodeDownload::SetupZeroImpactDir()
{
...
HRESULT CCodeDownload::UpdateJavaList(HKEY hkeyContains)
{
    ...
    for (i=0; i < m_pDownloads.GetCount(); i++) {
        ...
        for (int j=0; j < iNumJava; j++) {
            ...
            if (szPkg)
                WideCharToMultiByte(CP_ACP, 0, szPkg, -1, szPkgA, MAX_PATH, NULL, NULL);
...
HRESULT CCodeDownload::UpdateFileList(HKEY hkeyContains)
{
    ...
    for (i=0; i < iNumFiles; i++) {
        ...
        // this just works, and converting to the ANSI code page should not be done. See IE5 RAID #34606 for more details.
        if (g_bRunOnWin95) {
    ...
    if (!VersionFromManifest(szVersionBuf)) {
        if (!BitsInCache()) {
            if (!plci->IsPresent()) {
                ...
                m_debuglog->DebugOut(DEB_CODEDL, FALSE, ID_CDLDBG_DL_UPDATE_DU_NO_VERS, plci->szExistingFileName);
                plci->dwLocFVLS = 1;
            }
...
VOID CCodeDownload::CompleteAll(HRESULT hr, LPCWSTR szError)
{
...
BOOL CCodeDownload::GenerateErrStrings(HRESULT hr, char **ppszErrMsg, WCHAR **ppwszError)
{
    ...
    if (!dwFMResult) {
        ...
        // FormatMessage().
        if (!dwFMResult) {
            dwFMResult = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, 0, hr, 0, (LPTSTR)&szBuf, 0, NULL);
        }

        if (!dwFMResult) {
            dwFMResult = FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE, g_hInst, hr, 0, (LPTSTR)&szBuf, 0, NULL);
        }
    ...
    // ppszErrMsg now holds saved error message or whatever came back from FormatMessage(). Construct a complete error message.
...
STDMETHODIMP_(ULONG) CCodeDownload::AddRef()
{
    return m_cRef++;
}


STDMETHODIMP_(ULONG) CCodeDownload::Release()
...
HRESULT CCodeDownload::AcquireSetupCookie()
{
    HRESULT hr = S_OK;
    CUrlMkTls tls(hr); // hr passed by reference!
...
HRESULT CCodeDownload::RelinquishSetupCookie()
{
    HRESULT hr = S_OK;
    CUrlMkTls tls(hr); // hr passed by reference!
