...
Win32Open(char *pszFile, int oflag, int pmode )
{
    ...
    //BUGBUG: seterrormode to no crit errors and then catch sharing violations and access denied

    // Call Win32
    FileHandle = CreateFile(pszFile, fAccess, FILE_SHARE_READ, NULL, fCreate, FILE_ATTRIBUTE_NORMAL, INVALID_HANDLE_VALUE);
    if (FileHandle == INVALID_HANDLE_VALUE && SetFileAttributes(pszFile, FILE_ATTRIBUTE_NORMAL))
        FileHandle = CreateFile(pszFile, fAccess, FILE_SHARE_READ, NULL, fCreate, FILE_ATTRIBUTE_NORMAL, INVALID_HANDLE_VALUE);
    return( FileHandle );
}
