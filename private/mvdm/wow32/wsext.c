...
DWORD WWS32CallBackHandler (VOID);
BOOL WWS32DefaultBlockingHook (VOID);
...
ULONG FASTCALL WWS32WSAAsyncSelect(PVDMFRAME pFrame)
{
    ...
    if ( s32 == INVALID_SOCKET ) {
        ...
        ul = GETWORD16( (*wsockapis[WOW_WSAASYNCSELECT].lpfn)(s32, (HWND)HWND32(parg16->hWnd), (parg16->wMsg << 16) | WWS32_MESSAGE_ASYNC_SELECT, parg16->lEvent));
    }
...
ULONG FASTCALL WWS32WSAStartup(PVDMFRAME pFrame)
{
    ...
    RtlCopyMemory( wsaData16->szDescription, description, strlen(description) + 1 );

    systemStatus = "Running.";
    RtlCopyMemory( wsaData16->szSystemStatus, systemStatus, strlen(systemStatus) + 1 );
...
VOID WWS32TaskCleanup(VOID)
{
    ...
    for ( listEntry = listHead.Flink; listEntry != &listHead; listEntry = listEntry->Flink ) {
        ...
        err = (*wsockapis[WOW_SETSOCKOPT].lpfn)(socketInfo->SocketHandle32, SOL_SOCKET, SO_LINGER, (char *)&lingerInfo, sizeof(lingerInfo));
        //ASSERT( err == NO_ERROR );
...
{
    ASSERT( WWS32PostDispatchTable[WWS32_MESSAGE_ASYNC_SELECT] == WWS32PostAsyncSelect );
    ASSERT( WWS32PostDispatchTable[WWS32_MESSAGE_ASYNC_GETHOST] == WWS32PostAsyncGetHost );
    ASSERT( WWS32PostDispatchTable[WWS32_MESSAGE_ASYNC_GETPROTO] == WWS32PostAsyncGetProto );
    ASSERT( WWS32PostDispatchTable[WWS32_MESSAGE_ASYNC_GETSERV] == WWS32PostAsyncGetServ );
    ...
    return WWS32PostDispatchTable[Msg & 0xFFFF](hWnd, Msg, wParam, lParam);
...
BOOL WWS32PostAsyncSelect (HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    ...
    return PostMessage(hWnd, Msg >> 16, h16, lParam);
} // WWS32PostAsyncSelect
