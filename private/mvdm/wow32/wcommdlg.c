...
--*/
{
    ...
    if(pOFN16 && pOFN32) {
        ...
        if(bUpperStrings && pOFN32->lpstrFile) {
            // Note we have to upcase the pOFN32 here because some apps (notably QC/Win) do case-sensitive compares on the extension.
            // In Win3.1, the upcasing happens as a side-effect of the OpenFile call.
        ...
        if(bUpperStrings && (pOFN32->lpstrFileTitle)) {
            // Not sure if we really need to upcase this or not, but I figure somewhere there is an app that depends on this being uppercased like Win3.1
            CharUpperBuff(pOFN32->lpstrFileTitle, strlen(pOFN32->lpstrFileTitle));
        }
...
VOID FASTCALL WOWTellWOWThehDlg(HWND hDlg)
{
    if(CURRENTPTD()->CommDlgTd) {
        if(GetCommdlgTd(hDlg) == NULL) {
            WOW32WARNMSGF(FALSE, ("WOW::WOWTellWOWThehDlg: No unassigned hDlgs\n"));
        }
    }
}
