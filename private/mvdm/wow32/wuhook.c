    ...
    */
    ...
    if (!(ul = (ULONG)W32IsDuplicateHook(iHook, Proc16, ptd->htask16))) {
        if (W32GetThunkHookProc(iHook, Proc16, &HkData)) {
            ...
            ul = (ULONG)SetWindowsHookEx(iHook, (HOOKPROC)HkData.Proc32, (HINSTANCE)HkData.hMod, ThreadId);
...
ULONG FASTCALL WU32SetWindowsHookEx(PVDMFRAME pFrame)
{
    ...
    if (W32GetThunkHookProc(iHook, Proc16, &HkData)) {
        ul = (ULONG)SetWindowsHookEx(iHook, (HOOKPROC)HkData.Proc32, (HINSTANCE)HkData.hMod, (DWORD)THREADID32(parg16->f4));
        HkData.hHook = (HANDLE)ul;
        if (ul == (ULONG)NULL) {
            HkData.InUse = FALSE;
        } else {
