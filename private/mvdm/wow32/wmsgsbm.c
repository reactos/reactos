...
BOOL FASTCALL ThunkSBMMsg16(LPMSGPARAMEX lpmpex)
{
    ...
    if (wMsg < (SBM_ENABLE_ARROWS - SBM_SETPOS + 1)) {
        switch(lpmpex->uMsg = wMsg + SBM_SETPOS) {
            ...
            // they contain no pointers, handles, or rearranged message parameters, so consequently they are not documented in great detail here:
            ...
            case SBM_GETRANGE:
                ...
                // Win32:
                //   SendMessage(hwnd, SBM_GETRANGE, (WPARAM) &posMin, (LPARAM) &posMax);
            ...
            case SBM_SETRANGE:
                ...
                // Win32:
                //   SendMessage(hwnd, fRedraw ? SBM_SETRANGE : SBM_SETRANGEREDRAW, posMin, posMax);
...
VOID FASTCALL UnThunkSBMMsg16(LPMSGPARAMEX lpmpex)
{
    ...
    case SBM_GETRANGE:
        if (lpmpex->uParam && lpmpex->lParam) {
            lpmpex->lReturn = MAKELONG(*(UINT *)lpmpex->uParam, *(UINT *)lpmpex->lParam);
        }
        break;
    }
}