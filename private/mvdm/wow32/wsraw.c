...
ULONG FASTCALL WWS32accept(PVDMFRAME pFrame)
{
    ...
    if(!WSThunkAddrBufAndLen(&fastSockaddr, vpSockAddr16, vpwAddrLen16, &addressLength, &pAddressLength, &realSockaddr)) {
        goto exit;
    }
    ...
    WSUnThunkAddrBufAndLen((ULONG)news32, vpwAddrLen16, vpSockAddr16, addressLength, &fastSockaddr, realSockaddr);

    // If the call succeeded, alias the 32-bit socket handle we just obtained into a 16-bit handle.
...
ULONG FASTCALL WWS32bind(PVDMFRAME pFrame)
{
    ...
    if(!WSThunkAddrBuf(addressLength,vpSockAddr16,&fastSockaddr, &realSockaddr)) {
        goto exit;
    }

    ul = GETWORD16( (*wsockapis[WOW_BIND].lpfn)(s32, realSockaddr, addressLength));
...
ULONG FASTCALL WWS32connect(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_CONNECT].lpfn)(s32, realSockaddr, addressLength));
...
ULONG FASTCALL WWS32getpeername(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_GETPEERNAME].lpfn)(s32, realSockaddr, pAddressLength));
    ...
    // Un-Thunk the 16-bit Address name and length buffers
    WSUnThunkAddrBufAndLen(ul,vpwAddrLen16,vpSockAddr16,addressLength,&fastSockaddr,realSockaddr);
...
ULONG FASTCALL WWS32getsockopt(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_GETSOCKOPT].lpfn)(s32,parg16->Level,SocketOption16To32( parg16->OptionName ),(char *)optionValue32,(int *)&optionLength32));
...
ULONG FASTCALL WWS32inet_addr(PVDMFRAME pFrame)
{
    ...
    // If the thread is version 1.0 of Windows Sockets, play special stack games to return a struct in_addr.
...
ULONG FASTCALL WWS32recv(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_RECV].lpfn)(s32, buffer, BufferLength, parg16->Flags));
...
ULONG FASTCALL WWS32recvfrom(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_RECVFROM].lpfn)(s32,buffer,BufferLength,parg16->Flags,realSockaddr,pAddressLength));
...
ULONG FASTCALL WWS32send(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_SEND].lpfn)(s32, buffer, BufferLength, parg16->Flags));
...
ULONG FASTCALL WWS32sendto(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_SENDTO].lpfn)(s32,buffer,BufferLength,parg16->Flags,realSockaddr,addressLength));
...
ULONG FASTCALL WWS32setsockopt(PVDMFRAME pFrame)
{
    ...
    ul = GETWORD16( (*wsockapis[WOW_SETSOCKOPT].lpfn)(s32,parg16->Level,SocketOption16To32( parg16->OptionName ),optionValue32,optionLength32));
...
ULONG FASTCALL WWS32socket(PVDMFRAME pFrame)
{
    ...
    s32 = (*wsockapis[WOW_SOCKET].lpfn)(INT32(parg16->AddressFamily),INT32(parg16->Type),INT32(parg16->Protocol));
...
PWINSOCK_SOCKET_INFO FindSocketInfo16(IN SOCKET h32, IN HAND16 h16)
{
    ...
    for ( listEntry = WWS32SocketHandleListHead.Flink; listEntry != &WWS32SocketHandleListHead; listEntry = listEntry->Flink ) {
        socketInfo = CONTAINING_RECORD(listEntry,WINSOCK_SOCKET_INFO,GlobalSocketListEntry);
        if ( socketInfo->SocketHandle32 == h32 || socketInfo->SocketHandle16 == h16 ) {
            return socketInfo;
        }
    }
...
HAND16 AllocateUnique16BitHandle(VOID)
{
    ...
    // This function assumes it is called with the WWS32CriticalSection lock held!
    ...
    if( ( WWS32SocketHandleCounter > 255 ) && IsListEmpty( &WWS32SocketHandleListHead ) ) {
        WWS32SocketHandleCounter = 1;
        WWS32SocketHandleCounterWrapped = FALSE;
    }

    // If the socket handle counter has not wrapped around, then we can quickly return a unique handle.
    ...
    // If we made it this far, then there were no unique handles available. Bad news.
    return 0;
} // AllocateUnique16BitHandle


HAND16 GetWinsock16(IN INT h32, IN INT iClass)
{
    PWINSOCK_SOCKET_INFO socketInfo;
    HAND16 h16;
...
DWORD WSGetWinsock32 (IN  HAND16 h16, OUT PULONG pul)
{
    DWORD  s32;
...
VOID WSUnThunkAddrBuf(IN PSOCKADDR  fastSockaddr, IN PSOCKADDR  realSockaddr)
{
        if( (realSockaddr) && (realSockaddr != fastSockaddr) ) {
            free_w(realSockaddr);
        }
