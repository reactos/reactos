...
ULONG FASTCALL WMM32midiOutGetDevCaps(PVDMFRAME pFrame)
{
    ...
    */
    if ( UINT32( parg16->f3 ) == 0 ) {
        ...
        ul = GETWORD16((*mmAPI)( INT32(parg16->f1), &midioutcaps, sizeof(MIDIOUTCAPS) ));
        /*
...
ULONG FASTCALL WMM32midiOutGetErrorText(PVDMFRAME pFrame)
{
    ...
    */
    ...
    if ( pszText != NULL ) {
        ul = GETWORD16((*mmAPI)( UINT32(parg16->f1), pszText, UINT32(parg16->f3) ));
...
ULONG FASTCALL WMM32midiOutOpen(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutOpen( %x, %x, %x, %x, %x )", DWORD32( parg16->f1 ), INT32  ( parg16->f2 ), DWORD32( parg16->f3 ), DWORD32( parg16->f4 ), DWORD32( parg16->f5 ) ));
    ...
    if ( lpHMidiOut ) {
        ...
        */
        if ( pInstanceData = malloc_w(sizeof(INSTANCEDATA)) ) {
            dprintf2(( "WM32midiOutOpen: Allocated instance buffer at %8X", pInstanceData ));
            ...
            ul = GETWORD16((*mmAPI)( &Hand32, uDevID, (DWORD)W32CommonDeviceOpen, (DWORD)pInstanceData, CALLBACK_FUNCTION ));
        }
        else {
            ul = MMSYSERR_NOMEM;
        }
        ...
        */
        if ( ul == MMSYSERR_NOERROR ) {
                ...
                FLUSHVDMPTR( DWORD32(parg16->f1), sizeof(HMIDIOUT16), lpHMidiOut );
        }
...
ULONG FASTCALL WMM32midiOutPrepareHeader(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutPrepareHeader( %x %x %x)", WORD32( parg16->f1 ), DWORD32( parg16->f2 ), WORD32( parg16->f3 ) ));

    GETMIDIHDR16(parg16->f2, &midihdr);

    ul = GETWORD16((*mmAPI)( HMIDIOUT32(parg16->f1), &midihdr, WORD32(parg16->f3) ) );
    /*
...
ULONG FASTCALL WMM32midiOutUnprepareHeader(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutUnprepareHeader( %x %x %x)", WORD32( parg16->f1 ), DWORD32( parg16->f2 ), WORD32( parg16->f3 ) ));
...
ULONG FASTCALL WMM32midiOutShortMsg(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutShortMsg( %x, %x )", WORD32( parg16->f1 ), DWORD32( parg16->f2 ) ));
...
ULONG FASTCALL WMM32midiOutLongMsg(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutLongMsg( %x, %x, %x )", WORD32( parg16->f1 ), DWORD32( parg16->f2 ), UINT32( parg16->f3 ) ));
    ...
    */
    if ( UINT32(parg16->f3) < sizeof(MIDIHDR16) || HIWORD( DWORD32(parg16->f2) ) == 0 ) {
        ul = MMSYSERR_INVALPARAM;
    }
    else {
        if ( pMidihdr32 = malloc_w(sizeof(MIDIHDR32)) ) {
            ...
            */
            ...
            ul = GETWORD16( (*mmAPI)( HMIDIOUT32(parg16->f1), lpwhdr, UINT32(parg16->f3) ) );
            /*
...
ULONG FASTCALL WMM32midiOutGetVolume(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutGetVolume( %x, %x )", INT32( parg16->f1 ), DWORD32( parg16->f2 ) ));
...
ULONG FASTCALL WMM32midiOutSetVolume(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutSetVolume( %x, %x )", WORD32( parg16->f1 ), DWORD32( parg16->f2 ) ));
...
ULONG FASTCALL WMM32midiOutCachePatches(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutCachePatches( %x, %x, %x, %x )", WORD32( parg16->f1 ), UINT32( parg16->f2 ), DWORD32( parg16->f3 ), UINT32( parg16->f4 ) ));
    ...
    */
    ...
    if ( lppa1 ) {
        ul = GETWORD16((*mmAPI)( HMIDIOUT32(parg16->f1), UINT32(parg16->f2), lppa1, UINT32(parg16->f4) ));
        FREEMISCPTR( lppa1 );
    }
    else  {
...
ULONG FASTCALL WMM32midiOutCacheDrumPatches(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutCacheDrumPatches( %x, %x, %x, %x )", WORD32( parg16->f1 ), UINT32( parg16->f2 ), DWORD32( parg16->f3 ), UINT32( parg16->f4 ) ));

    GETMISCPTR( DWORD32( parg16->f3 ), lpka1 );
    if ( lpka1 ) {
        ul = GETWORD16((*mmAPI)( HMIDIOUT32(parg16->f1), UINT32(parg16->f2), lpka1, UINT32(parg16->f4) ) );
        FREEMISCPTR(lpka1);
    }
    else  {
...
ULONG FASTCALL WMM32midiOutMessage(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiOutMessage( %x, %x, %x, %x )", WORD32( parg16->f1 ),   UINT32( parg16->f2 ), DWORD32( parg16->f3 ), DWORD32( parg16->f4 ) ));

    if ( (UINT32(parg16->f2) >= DRV_BUFFER_LOW) && (UINT32(parg16->f2) <= DRV_BUFFER_HIGH) ) {
        LPDWORD     lpdwParam1;
        GETMISCPTR(parg16->f3, lpdwParam1);

        ul = GETDWORD16((*mmAPI)( HMIDIOUT32(parg16->f1), UINT32(parg16->f2), (DWORD)lpdwParam1, DWORD32(parg16->f4)));
        FREEMISCPTR(lpdwParam1);
    } else {
        ul = GETDWORD16((*mmAPI)( HMIDIOUT32(parg16->f1), MAKELONG( WORD32(parg16->f2), 0xFFFF ), DWORD32(parg16->f3), DWORD32(parg16->f4) ));
    }
...
ULONG FASTCALL WMM32midiInGetDevCaps(PVDMFRAME pFrame)
{
    ...
    trace_midi(( "midiInGetDevCaps( %x, %x, %x )", INT32( parg16->f1 ), DWORD32( parg16->f2 ), UINT32( parg16->f3 ) ));
    if ( UINT32( parg16->f3 ) == 0 ) {
        ul = MMSYSERR_NOERROR;
    }
    else {
        ul = GETWORD16((*mmAPI)(INT32(parg16->f1), &midiincaps1, sizeof(MIDIINCAPS) ) );
        /*
        ** This must now thunk the ENTIRE structure, then copy parg16->f3
