 ...
 *                         message numbers from name text, replaced "EMPTY!0x02A7" with NULL and changed lookup code to display message number when NULL.
--*/
#include "precomp.h"
#pragma hdrstop
...
// Note: If any of the message thunks is changed from a 'validthunk' to WM32NoThunking  make sure that 16->32 messages  are not affected (see thunkwmmsg16/unthunkwmmsg16 in msg16.c).
//       If a 16->32 thunk does exist for the message in question you must use W32Thunk16To32 instead of WM32NoThunking.
//                                                     - nanduri


// The message names present in the checked build of this table are used by GetWMMsgName, which is called in lots of places on checked builds.
