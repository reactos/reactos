...
#define EUDC_RANGE_KEY    (LPSTR) "SYSTEM\\CurrentControlSet\\Control\\Nls\\CodePage\\EUDCCodeRange"
...
ULONG FASTCALL WWM32MiscGetEUDCLeadByteRange(PVDMFRAME pFrame)
{
    ...
    // In Win32, We support multiple EUDC Range, but for Win16, we just return only first EUDC range to keep backward compatibility...
    ulRet = RegOpenKeyExA(HKEY_LOCAL_MACHINE, EUDC_RANGE_KEY, (DWORD) 0, KEY_QUERY_VALUE, &hKey);
    ...
    RtlIntegerToChar(GetACP(),10,sizeof(achACP),achACP);// Convert ACP to string..

    ulRet = RegQueryValueExA(hKey, achACP, (LPDWORD)NULL, (LPDWORD)&dwType, (LPBYTE)achRange, &dwRangeSize);
    if (ulRet != ERROR_SUCCESS) {
        #if DBG
    ...
    usEUDCRange = ((unsigned short)chEUDCEnd << 8) | ((unsigned short)chEUDCStart     );// Setup return value.
