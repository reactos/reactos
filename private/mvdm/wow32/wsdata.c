...
DWORD BytesInHostent32 (PHOSTENT Hostent32);
DWORD CopyHostent32To16 (PHOSTENT16 Hostent16, VPHOSTENT16 VHostent16, int BufferLength, PHOSTENT Hostent32);
DWORD BytesInProtoent32 (PPROTOENT Protoent32);
DWORD CopyProtoent32To16 (PPROTOENT16 Protoent16, VPPROTOENT16 VProtoent16, int BufferLength, PPROTOENT Protoent32);
DWORD BytesInServent32 (PSERVENT Servent32);


DWORD CopyServent32To16 (PSERVENT16 Servent16, VPSERVENT16 VServent16, int BufferLength, PSERVENT Servent32);
/*++
...
NOTE:
  The VDM frame is automatically set up, with all the function parameters available via parg16->f<number>.
  ...
  ptr in WOW32 needs to be refreshed to reflect the NEW selector-to-flat_memory mapping.
--*/
ULONG FASTCALL WWS32gethostbyaddr(PVDMFRAME pFrame)
{
    ...
    hostent32 = (PHOSTENT) (*wsockapis[WOW_GETHOSTBYADDR].lpfn)((CHAR *)&addr32, parg16->Length, parg16->Type);
    ...
    if ( hostent32 != NULL ) {
        ...
        bytesRequired = CopyHostent32To16(hostent16, WWS32vHostent, MAXGETHOSTSTRUCT, hostent32);
...
ULONG FASTCALL WWS32WSAAsyncGetHostByAddr(PVDMFRAME pFrame)
{
    ...
    // Enter a critical section to synchronize access to the context block and their global list.
    ...
    if ( context->AsyncTaskHandle32 != 0 ) {
        ...
        InsertTailList(&WWS32AsyncContextBlockListHead, &context->ContextBlockListEntry);
    }
