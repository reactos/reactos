...
ULONG FASTCALL WS32RegCreateKey(PVDMFRAME pFrame)
{
    ...
    if (!hkey) {
       ...
       ul = RegCreateKey (HKEY_CLASSES_ROOT, psz1, &hkResult);
       if ((psz1) && (psz1 != psz)) {
            free_w (psz1);
        }
    }
    else {
       ul = RegCreateKey (hkey, psz, &hkResult);
    }
...

{
    ...
    Status = RegOpenKeyEx(hKey, lpszSubKey, 0, KEY_ENUMERATE_SUB_KEYS | KEY_QUERY_VALUE, &Key);
...
/*
 * This is an equivalent of a nasty win'95 style hack that prevents us from launching things from winexec that are lfn-based
 * Unfortunate as it is -- this alone can't save us in all the cases -- it
 * just allows for a fix to ole-based method (another method should be employed to fix winexec)
