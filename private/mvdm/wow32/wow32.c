...
BOOLEAN W32DllInitialize(IN PVOID DllHandle, IN ULONG Reason, IN PCONTEXT Context OPTIONAL)
/*++
...
--*/
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        if ((hWOWHeap = HeapCreate (0, INITIAL_WOW_HEAP_SIZE, GROW_HEAP_AS_NEEDED)) == NULL)
            return FALSE;
        ...
        // initialize the thunk table offsets.  do it here so the debug process gets them.
        ...
        if (!LoadCriticalStringResources()) {
            MessageBox(NULL, "The Win16 subsystem could not load critical string resources from wow32.dll, terminating.", "Win16 subsystem load failure", MB_ICONEXCLAMATION | MB_OK);
            return FALSE;
        }
        ...
        if (IsTerminalServer()) {
            ...
            if (dllHandle) {
                gpfnTermsrvCORIniFile = (PTERMSRVCORINIFILE) GetProcAddress(dllHandle, "TermsrvCORIniFile");
...
--*/
{
    ...
    // Allocate too much memory for strings (maximum possible) at first, reallocate to the real size when we're done loading strings.
...
BOOL W32EWExecData(DWORD fnid, LPSTR lpData, DWORD cb)
{
    ...
    switch (fnid) {
        case EWEXEC_SET:
            bRet = WriteProfileString(WOWSZ_EWEXECVALUE, WOWSZ_EWEXECVALUE, lpData);
            break;
        case EWEXEC_DEL:
            bRet = WriteProfileString(WOWSZ_EWEXECVALUE, NULL, NULL);
            break;
        case EWEXEC_QUERY:
            if (bRet = GetProfileString(WOWSZ_EWEXECVALUE, WOWSZ_EWEXECVALUE, "", abT, sizeof(abT))) {
                strcpy(lpData, abT);
            }
...
BOOL W32Init(VOID)
{
    ...
    if (RegOpenKeyEx ( HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Control\\WOW", 0, KEY_QUERY_VALUE, &WowKey) != 0){
    ...
    if ( ! RegQueryValueEx(WowKey, "SharedWowTimeout", NULL, &dwType, (LPBYTE) &dwSharedWowTimeout, &cb) && REG_DWORD == dwType) {
    ...
    if (RegQueryValueEx(WowKey, "ThunkNLS", NULL, &dwType, (LPBYTE) &fThunkStrRtns, &cb) || dwType != REG_DWORD) {
        // Didn't find the registry value or it's the wrong type,
        // so we use the default behavior which is to thunk outside the
        // US.
        fThunkStrRtns = GetSystemDefaultLCID() != MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), SORT_DEFAULT);
    } else {
        // We did find a ThunkNLS value in the registry, warn on debug builds to save testers and developers who turn it on for one bug but forget to turn it back off.
#ifdef DEBUG
    ...
    if (RegQueryValueEx (WowKey, "wowcmdline", NULL, &dwType, (LPBYTE)&WOWCmdLine, &WOWCmdLineSize) != 0){
        RegCloseKey (WowKey);
    ...
    if (ExpandEnvironmentStrings ((LPCSTR)WOWCmdLine, (LPSTR)pWOWCmdLine, WOWCmdLineSize) > WOWCmdLineSize) {
        LOGDEBUG(0,("    W32INIT ERROR: Registry Buffer too small\n"));
        return FALSE;
    }
...
INT W32FilterException(INT ExceptionCode, PEXCEPTION_POINTERS ExceptionInformation)
...

{
    ...
    if ((ExceptionCode != EXCEPTION_WOW32_ASSERTION) && IsW32WorkerException()) {
...
*/
{
    ...
    wsprintf(szErrorMessage, pszErrorFormatString, szTask, pszModuleFilePart, pexi->ExceptionRecord->ExceptionAddress, dwException);
    ...
    if (AeAutoDebug) {
        ...
        if (wDebugButton == SEB_CANCEL) {
            wsprintf(szDialogText, "%s\n%s\n%s\n%s\n", szErrorMessage, CRITSTR(ChooseClose), CRITSTR(ChooseCancel),
                     (dwException == EXCEPTION_DATATYPE_MISALIGNMENT) ? CRITSTR(ChooseIgnoreAlignment) : CRITSTR(ChooseIgnore));
        } else {
            wsprintf(szDialogText, "%s\n%s\n%s\n", szErrorMessage, CRITSTR(ChooseClose),
                (dwException == EXCEPTION_DATATYPE_MISALIGNMENT) ? CRITSTR(ChooseIgnoreAlignment) : CRITSTR(ChooseIgnore));
        }
    ...
    // If IGNORE is chosen and it's an EXCEPTION_DATATYPE_MISALIGNMENT, turn on software emulation of misaligned access and restart the faulting instruction.
    // Otherwise,  just fail the API and continue.
    if (dwButtonPushed == 3) {
...
/*++
Routine Description:
    This routine checks to see if there's a debugger attached to WOW.
    If not, it attempts to spawn one with a command to attach to WOW.
    If the system was booted with /DEBUG in boot.ini (kernel debugger enabled), we'll run "ntsd -d" otherwise we'll run "ntsd".
--*/
{
    ...
    // Create an event for NTSD to signal once it has fully connected and is ready for the exception.  We force the handle to be inherited.
    sa.nLength = sizeof(sa);
    sa.lpSecurityDescriptor = NULL;
    sa.bInheritHandle = TRUE;
    ...
    wsprintf(szCmdLine, "ntsd %s -p %lu -e %lu -x -g -G", fKernelDebuggerEnabled ? "-d" : "", GetCurrentProcessId(), hEvent);
...
--*/
{
    ...
    if (!fDebuggerAttached) {
        // Query our ProcessDebugPort, if it is nonzero we have a debugger attached.
    ...
    // If we have a debugger attached share that information with the 16-bit kernel.
...
--*/
{
    ...
    if (vp = GetPModeVDMPointerMacro(Address, Count)) {
        ...
        if (SelectorLimit && (Address & 0xFFFF) + Count > SelectorLimit[Address >> 19] + 1)
        {
        ...
        if (vp != Sim32GetVDMPointer(Address, Count, TRUE)) {
            LOGDEBUG(LOG_ALWAYS, ("GetPModeVDMPointer: GetPModeVDMPointerMacro(%x) returns %x, Sim32 returns %x!\n", Address, vp, Sim32GetVDMPointer(Address, Count, TRUE)));
...
VOID logprintf(PSZ pszFmt, ...)
{
    ...
    if(fLog > 1) {
        if(fLog == 2) {
            if((hfLog = CreateFile(szLogFile, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL)) != INVALID_HANDLE_VALUE) {
                fLog = 1;
            } else {
                hfLog = NULL;
                fLog  = 0;
...
BOOL checkloging(register PVDMFRAME pFrame)
{
    ...
    if (iLogLevel < 20 ) {
        if((iFun == FUN_WOWOUTPUTDEBUGSTRING) || ((iFun < pto->user) && (iFun >= FUN_WOWINITTASK)))
...
// malloc_w_or_die should not be used by API or message thunks or worker routines called by API or message thunks.
...
int WOW32_strncmp(const char* str1, const char* str2, size_t      n)
{
    if (gbDBCSEnable) {
        ...
        retval = CompareStringA( GetThreadLocale(), LOCALE_USE_CP_ACP, str1, n, str2, n );
        if (retval == 0) {
...
int WOW32_strnicmp(const char* str1, const char* str2, size_t      n)
{
    if (gbDBCSEnable) {
        ...
        retval = CompareStringA( GetThreadLocale(), LOCALE_USE_CP_ACP | NORM_IGNORECASE, str1, n, str2, n );
        if (retval == 0) {
            // The caller is not expecting failure.  Try the system
            // default locale id.
