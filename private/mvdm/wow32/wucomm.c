...
ULONG FASTCALL WU32EnableCommNotification(PVDMFRAME pFrame)
{
    ...
    if ((VALIDCOM(idComDev)) && (pWOWPort = PortTab[idComDev].pWOWPort)) {
        ...
        else {// else set up the notification mechanisms
    ...
    else {// else there is no notification for LPT in Win3.1
        ul = (ULONG)FALSE;
    }
...
ULONG FASTCALL WU32FlushComm(PVDMFRAME pFrame)
{
    ...
    if (pWOWPort = GETPWOWPTR(idComDev)) {
        ...
        else {// else just return current error code for LPTx
            ul = pWOWPort->dwErrCode;
        }
    }
    ...
    if (VALIDCOM(idComDev)) {
        if(pWOWPort = PortTab[idComDev].pWOWPort) {
            if(pDEB16 = pWOWPort->lpComDEB16) {
                ul = (ULONG)pDEB16->EvtWord;// in Win3.1 the app gets current event word (NOT the EvtMask!!)
    ...
    if (pWOWPort = GETPWOWPTR(idComDev)) {
        ...
        if(pdcb16) {
            ...
            // else get DCB for LPT's
                ...
                RtlCopyMemory((PVOID)pdcb16, (PVOID)pWOWPort->pdcb16, sizeof(DCB16));
                ul = 0; // Win3.1 returns 0 if success
            }
...
ULONG FASTCALL WU32OpenComm(PVDMFRAME pFrame)
{
    ...
    if( !fIsLPTPort ) {
        ...
        if((WORD32(parg16->f2) == 0) && (WORD32(parg16->f3) == 0)) {
            ret = IE_MEMORY;
            goto ErrorExit1;
        }
        ...
        // tail pointers would be equal, which we use to indicate an *empty* buffer.
    ...
    // we need to set up a default DCB for LPT's
        ...
        RtlZeroMemory((PVOID)pdcb16, sizeof(DCB16));// initialize everything to 0
        pdcb16->Id = LOBYTE(LOWORD(idComDev));// save the idComDev only in the DCB
    }
    ...
    dwDEBAddr &= 0xFFFF0000;// Isolate the segment value
    lpComDEB16 = (PCOMDEB16) GetRModeVDMPointer(dwDEBAddr);// save flat pointer to DEB for use in Modem interrupt thread
    InitDEB16(lpComDEB16, iTab, WORD32(parg16->f2), WORD32(parg16->f3));// init the DEB
    RtlZeroMemory((PVOID)pWOWPort, sizeof(WOWPORT));// init the support struct
            ...
            if ((pwp = GETPWOWPTR(UINT32(parg16->f1))) && psz2) {
                if (!WriteFile(pwp->h32, psz2, parg16->f3, &cbWritten, &pwp->olWrite)) {
                    if (ERROR_IO_PENDING == GetLastError() ) {
                        ...
                        if (GetOverlappedResult(pwp->h32, &pwp->olWrite, &cbWritten, TRUE)) {
                            i = cbWritten;
                            goto WriteSuccess;
                        }
                    }
...
BOOL GetPortName(LPSTR pszMode, LPSTR pszPort)
{
    ...
    len = strlen(pszMode);
    if((len >= 3) && (len < 80)) {
        GetPortStringToken(pszMode, szTemp);// Get the first token from the mode string.
...
BOOL WOWStartModemIntThread(PWOWPORT pWOWPort)
{
    ...
    if((hMiThread = CreateThread(NULL, 8192, (LPTHREAD_START_ROUTINE)WOWModemIntThread, (PWOWPORT)pWOWPort, 0, (LPDWORD)&dwUnused))  == NULL) {
        goto ErrorExit1;
    }
...
VOID WOWModemIntThread(PWOWPORT pWOWPort)
{
    ...
    ol.Offset       = 0;
    ol.OffsetHigh   = 0;
    ol.hEvent       = CreateEvent(NULL, TRUE, FALSE, (LPTSTR)PortTab[iTab].szPort);
