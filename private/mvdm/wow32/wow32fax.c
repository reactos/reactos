...
LPSTR *GetSupportedFaxDrivers(UINT *uCount)
{
    ...
    *uCount = 0;
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows NT\\CurrentVersion\\WOW\\WowFax\\SupportedFaxDrivers", 0, KEY_READ, &hKey ) != ERROR_SUCCESS) {
        goto GSFD_error;
    }
...
BOOL DoAddPrinterStuff(LPVOID pPrinterStuff, INT iCode)
{
    ...
    if (hWaitObjects = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)AddPrinterThread, &WowAddPrinter, 0, &dwUnused)) {
        // Pump messages while we wait for AddPrinterThread to finish.
        for (;;) {
            dwEvent = MsgWaitForMultipleObjects(1, &hWaitObjects, FALSE, INFINITE, QS_ALLEVENTS | QS_SENDMESSAGE);
...
BOOL InstallWowFaxPrinter(PSZ szSection, PSZ szKey, PSZ szString)
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows NT\\CurrentVersion\\WOW\\WowFax", 0, KEY_WRITE, &hKey ) == ERROR_SUCCESS) {
