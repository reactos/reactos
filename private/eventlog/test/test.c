...
NTSTATUS WriteLogEntry (HANDLE LogHandle, ULONG EventID)
{
...
#define SIZE_TOKEN_BUFFER 512
    ...
    if (!InvalidUser) {
        ...
        if (!GetTokenInformation(hToken, TokenUser, pTokenUser, SIZE_TOKEN_BUFFER, &SizeRequired)) {
    ...
    // Allocate space for the unicode strings in the array, and copy over the strings from Strings[] to that array.
...
VOID DisplayEventRecords( PVOID Buffer, ULONG  BufSize, PULONG NumRecords)
{
    ...
    while (Offset < BufSize && Count < *NumRecords) {
        ...
        printf("UserSidOffset: 0x%lx DataLength: 0x%lx DataOffset: 0x%lx Category: 0x%lx\n", pLogRecord->UserSidOffset, pLogRecord->DataLength, pLogRecord->DataOffset, pLogRecord->EventCategory);
        ...
        // If verbose mode, display binary data (up to 256 bytes) BUGBUG - this code will hit an alignment fault on mips.
        if (Verbose) {
            PULONG pData;
            PULONG pEnd;
            ...
            printf("Data: \n\n");
            for (pData = (PULONG)((PBYTE) pLogRecord + pLogRecord->DataOffset); pData < pEnd; (PBYTE) pData += 32) {
...
NTSTATUS ReadFromLog ( HANDLE LogHandle, PVOID  Buffer, PULONG pBytesRead, ULONG  ReadFlag, ULONG  Record)
{
    NTSTATUS    Status;
...
NTSTATUS TestReadEventLog (ULONG Count,ULONG ReadFlag,ULONG Record)
{
    ...
    if (ReadingBackupFile) {
        ...
        Status = ElfOpenBackupEventLogW (&ServerName,&ModuleNameU,&LogHandle);
    }
    else {
        printf("ElfOpenEventLog - ");
        Status = ElfOpenEventLogW (&ServerName,&ModuleNameU,&LogHandle);
    }
...
NTSTATUS TestReportEvent (ULONG Count)
{
    NTSTATUS    Status, IStatus;
    HANDLE      LogHandle;
    UNICODE_STRING  ModuleNameU;
    ...
    Status = ElfRegisterEventSourceW (&ServerName,&ModuleNameU,&LogHandle);
    if (!NT_SUCCESS(Status)) {
         printf("Error - 0x%lx\n", Status);
    } else {
        printf("SUCCESS\n");
...
NTSTATUS TestElfClearLogFile(VOID)
{
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        RtlInitUnicodeString( &BackupU, L"\\SystemRoot\\System32\\Config\\view.evt" );
...
retry:
        ...
        if (Status == STATUS_OBJECT_NAME_COLLISION) {
            if (DontRetry) {
                ...
                InitializeObjectAttributes(&ObjectAttributes,&BackupU,OBJ_CASE_INSENSITIVE,NULL,NULL);
                Status = NtOpenFile(&ClearHandle,GENERIC_READ | DELETE | SYNCHRONIZE,&ObjectAttributes,&IoStatusBlock,FILE_SHARE_DELETE,FILE_SYNCHRONOUS_IO_NONALERT);
                Status = NtSetInformationFile(ClearHandle,&IoStatusBlock,&DeleteInfo,sizeof(DeleteInfo),FileDispositionInformation);
                if (NT_SUCCESS (Status) ) {
...
JustClear:
        ...
        Status = ElfClearEventLogFileW (LogHandle,NULL);
        if (!NT_SUCCESS(Status)) {
            printf ("Error - 0x%lx\n", Status);
        } else {
            printf ("SUCCESS\n");
...
NTSTATUS TestElfBackupLogFile(VOID)
{
    NTSTATUS    Status, IStatus;
    HANDLE      LogHandle;
    UNICODE_STRING  BackupU, ModuleNameU;
    ...
    if (!NT_SUCCESS(Status)) {
        ...
        // Backup the log file
        printf("Calling ElfBackupEventLogFile backing up to %s ",pBackupFileName);
        RtlInitAnsiString( &AnsiString, pBackupFileName);
        RtlAnsiStringToUnicodeString(&BackupU, &AnsiString, TRUE);
        BackupU.MaximumLength = BackupU.Length + sizeof(WCHAR);
        Status = ElfBackupEventLogFileW (LogHandle,&BackupU);
        if (!NT_SUCCESS(Status)) {
            printf ("Error - 0x%lx\n", Status);
        } else {
            printf ("SUCCESS\n");
...
VOID TestLPCWrite(DWORD NumberOfRecords, DWORD MillisecondsToDelay)
{
    HANDLE PortHandle;
    ...
    Status = NtConnectPort(& PortHandle,& PortName,& Qos,NULL,NULL,NULL,NULL,NULL);
    if (!NT_SUCCESS(Status)) {
       printf("Connect to the LPC port failed with RC %X\n", Status);
       return;
    }

    // Allocate the memory for the Message to send to the LPC port.
    // It will contain a PORT_MESSAGE followed by an IO_ERROR_LOG_MESSAGE followed by Drivername and Devicename UNICODE strings
    DataLength =  PORT_MAXIMUM_MESSAGE_LENGTH - (sizeof(IO_ERROR_LOG_MESSAGE) + DRIVER_NAME_LENGTH  * sizeof(WCHAR) + DEVICE_NAME_LENGTH * sizeof(WCHAR) + STRING_LENGTH * sizeof(WCHAR));
    ...
    pIoErrorLogMessage = (PIO_ERROR_LOG_MESSAGE) ((LPBYTE) RequestMessage + sizeof(PORT_MESSAGE));
    ...
    pIoErrorLogMessage->DriverNameOffset = sizeof(IO_ERROR_LOG_MESSAGE) + DataLength - sizeof(DWORD);
    ...
    pIoErrorLogMessage->EntryData.StringOffset = sizeof(IO_ERROR_LOG_MESSAGE) - sizeof(DWORD) + DataLength + DRIVER_NAME_LENGTH * sizeof(WCHAR);
    ...
    pIoErrorLogMessage->EntryData.DeviceOffset = RtlConvertUlongToLargeInteger(1);
    ...
    // Copy the strings
    pDestinationString = (LPWSTR) ((LPBYTE) pIoErrorLogMessage + sizeof(IO_ERROR_LOG_MESSAGE) - sizeof(DWORD) + pIoErrorLogMessage->EntryData.DumpDataSize);
    ...
    while (NumberOfRecords) {
        ...
        while(NumberOfRecords--) {
            ...
            Status = NtRequestWaitReplyPort(PortHandle, RequestMessage, & ReplyMessage);
            if (!NT_SUCCESS(Status)) {
...
VOID TestChangeNotify(VOID)
{
    ...
    Status = NtCreateEvent(&Event,SYNCHRONIZE | EVENT_QUERY_STATE | EVENT_MODIFY_STATE,&obja,SynchronizationEvent,FALSE);
    ...
    Status = ElfReadEventLogW (LogHandle,EVENTLOG_SEEK_READ | EVENTLOG_FORWARDS_READ,OldestRecord,Buffer,READ_BUFFER_SIZE,&BytesRead,&MinBytesNeeded);

    // This one should hit end of file
    Status = ElfReadEventLogW (LogHandle,EVENTLOG_SEQUENTIAL_READ | EVENTLOG_FORWARDS_READ,0,Buffer,READ_BUFFER_SIZE,&BytesRead,&MinBytesNeeded);
    if (Status != STATUS_END_OF_FILE) {
...
VOID TestLogFull(VOID)
{
    ...
    for (i = 0; i < sizeof(szLogNames) / sizeof(LPWSTR); i++) {
        ...
        if (fIsSecLog) {
            if (!NT_SUCCESS(RtlAdjustPrivilege(SE_SECURITY_PRIVILEGE,TRUE,FALSE,&fPrevious))) {
...
VOID __cdecl main(IN SHORT argc, IN PSZ argv [])
{
           ...
           while (*++pch) {
               switch (*pch) {
                  ...
                  case 's':
                     ...
                     RtlAnsiStringToUnicodeString(&ServerName, &AnsiString,TRUE);
                     break;
                  case 't':
