...
BOOL SendAdminAlert (ULONG   MessageID, ULONG   NumStrings, UNICODE_STRING  *pStrings)
/*++
Routine Description:
...
Return Value:
    NONE
Note:
    This routine dynamically links to netapi32.dll to avoid a build dependency.
--*/
{
    typedef DWORD (*ALERT_FNC_PTR)(IN LPTSTR AlertEventName, IN LPVOID VariableInfo, IN DWORD VariableInfoSize, IN LPTSTR ServiceName);
    ...
    // Dynamically link to netapi.dll.  If it's not there just return.  Return TRUE so we won't try to send this alert again.
    ...
    // Get the address of the service's main entry point.  This entry point has a well-known name.
    AlertFunction = (ALERT_FNC_PTR)GetProcAddress(dllHandle, "NetAlertRaiseEx");
    ...
    NetStatus = (*AlertFunction)(ALERT_ADMIN_EVENT, AlertBuffer, DataSize, EVENTLOG_SVC_NAMEW);
    if (NetStatus != NERR_Success) {
        ElfDbgPrint(("[ELF] NetAlertRaiseEx failed with %d\n", NetStatus));
        ElfDbgPrint(("[ELF] Attempted to raise alert %d\n", MessageID));

        return(FALSE);// Probably just not started yet, try again later
    }
