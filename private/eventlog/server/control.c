...
VOID ElfControlResponse(DWORD   opCode)
{
    DWORD   state;
...
Routine Description:
    ...
    This state information is protected as a critical section such that only one thread can modify or read it at a time.
Return Value:
