...
DWORD_PTR NTGetPID(HANDLE hProcess)
{
    HMODULE hModule;
...
// the block bounded by the #ifdef _X86_ statement contains the code for getting the PID from an HPROCESS when running under Win9X
...
#define IHTEISGLOBAL(ihte)         (((ihte) >> (32 - 8 - IHTETOHANDLESHIFT)) == (((DWORD)GLOBALHANDLEMASK) >> 24))
#define IS_WIN32_PREDEFINED_HANDLE(hnd)         ((hnd == HANDLE_CURRENT_PROCESS)||(hnd == HANDLE_CURRENT_THREAD)||(hnd == HANDLE_INVALID))
...
--*/
{
    ...
    if (Handle == HANDLE_CURRENT_PROCESS) {
        ...
        // of the kernel process's handle table...we don't currently deal with this type of handle
...
DWORD GetProcessModules(HANDLE hProcess, PINTERNAL_GET_MODULE    InternalGetModule, PVOID Context)
{
#ifdef _X86_
    OSVERSIONINFO VerInfo;
    ...
    )
{
    ...
    if (pModule32First(hSnapshot, &ModuleEntry))
    {
        do
        {
            if (!InternalGetModule(hProcess, ModuleEntry.szModule, (DWORD) ModuleEntry.modBaseAddr, ModuleEntry.modBaseSize, Context))
            {
                break;
            }
    ...
    )
{
    ...
    Status = RtlQueryProcessDebugInformation((HANDLE)ProcessId, RTL_QUERY_PROCESS_MODULES, Buffer);
    ...
    for (i=0; i<Buffer->Modules->NumberOfModules; i++) {
        ...
        if (!InternalGetModule(hProcess, (LPSTR) &Module->FullPathName[Module->OffsetToFileName], (DWORD64)Module->ImageBase, (DWORD)Module->ImageSize, Context))
        {
            break;
        }
    }
...
VOID FreeModuleEntry(PMODULE_ENTRY ModuleEntry)
{
    ...
    )
{
    ...
    if (ProcessEntry->pCallbackFunction32) {
        ...
        Dups = 0;
        for (i = 0; i < mi->numsyms; i++) {
            if ((mi->symbolTable[i].NameLength == sym->NameLength) && (strcmp( mi->symbolTable[i].Name, sym->Name ) == 0)) {
    ...
    __try {
        if (ProcessEntry->pCallbackFunction32) {
            ProcessEntry->pCallbackFunction32(ProcessEntry->hProcess, CBA_DUPLICATE_SYMBOL, (PVOID) DupSym32, (PVOID) ProcessEntry->CallbackUserContext);
            ...
            ProcessEntry->pCallbackFunction64(ProcessEntry->hProcess, CBA_DUPLICATE_SYMBOL, (ULONG64) &DupSym64, ProcessEntry->CallbackUserContext);
...
IMGHLP_RVA_FUNCTION_DATA * SearchRvaFunctionTable(IMGHLP_RVA_FUNCTION_DATA *FunctionTable, LONG High, LONG Low, DWORD dwPC)
{
    LONG    Middle;
...
PIMAGE_FUNCTION_ENTRY LookupFunctionEntryAxp32 (PMODULE_ENTRY mi, DWORD         ControlPc)
{
...
PIMAGE_FUNCTION_ENTRY64 LookupFunctionEntryAxp64 (PMODULE_ENTRY mi, DWORD64       ControlPc)
{
    IMAGE_ALPHA64_RUNTIME_FUNCTION_ENTRY *FunctionEntry;
...
PIMAGE_ALPHA64_RUNTIME_FUNCTION_ENTRY GetAlphaFunctionEntryFromDebugInfo (PPROCESS_ENTRY  ProcessEntry, DWORD64         ControlPc)
{
    PMODULE_ENTRY mi;
...
PIMAGE_IA64_RUNTIME_FUNCTION_ENTRY LookupFunctionEntryIA64 (PMODULE_ENTRY mi, DWORD         ControlPc)
{
    IMGHLP_RVA_FUNCTION_DATA   *FunctionTable;
    IMGHLP_RVA_FUNCTION_DATA   *FunctionEntry;
    ...
    )
{
    ...
    if (ProcessEntry->pCallbackFunction32) {
        ...
        __try {
            Status = ProcessEntry->pCallbackFunction32(ProcessEntry->hProcess, CallbackType, (PVOID)&idsl32, (PVOID)ProcessEntry->CallbackUserContext);
    ...
    if (ProcessEntry->pCallbackFunction64) {
        ...
        __try {
            Status = ProcessEntry->pCallbackFunction64(ProcessEntry->hProcess, CallbackType, (ULONG64)(ULONG_PTR)idsl64, ProcessEntry->CallbackUserContext);
        } __except (EXCEPTION_EXECUTE_HANDLER) {
...
VOID SympSendDebugString(PPROCESS_ENTRY ProcessEntry, LPSTR          String)
{
    DWORD Status;
    __try {
        if (!ProcessEntry)
        ...
        if (ProcessEntry->pCallbackFunction32) {
            Status = ProcessEntry->pCallbackFunction32(ProcessEntry->hProcess, CBA_DEBUG_INFO, (PVOID)String, (PVOID)ProcessEntry->CallbackUserContext);
        } else
        if (ProcessEntry->pCallbackFunction64) {
            Status = ProcessEntry->pCallbackFunction64(ProcessEntry->hProcess, CBA_DEBUG_INFO, (ULONG64)String, ProcessEntry->CallbackUserContext);
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
...
int WINAPIV SympDprintf(PPROCESS_ENTRY ProcessEntry, LPSTR Format, ...)
{
...
int WINAPIV SympEprintf(PPROCESS_ENTRY ProcessEntry, LPSTR Format, ...)
{
...
BOOL CompleteDeferredSymbolLoad(IN  HANDLE          hProcess, IN  PMODULE_ENTRY   mi)
{
    ...
    DoSymbolCallback(ProcessEntry, CBA_DEFERRED_SYMBOL_LOAD_START, mi, &idsl, CallbackFileName);
    ImageName = mi->ImageName;
    for (; ;) {
        pIDD = ImgHlpFindDebugInfo(hProcess, mi->hFile, ImageName, ProcessEntry->SymbolSearchPath, mi->BaseOfDll, 0);
        if (pIDD) {
            break;
        }

        DPRINTF(ProcessEntry, "ImgHlpFindDebugInfo(%p, %s, %s, %I64x, 0) failed\n", mi->hFile, ImageName, ProcessEntry->SymbolSearchPath, mi->BaseOfDll);
        if (!DoSymbolCallback(ProcessEntry, CBA_DEFERRED_SYMBOL_LOAD_FAILURE, mi, &idsl, CallbackFileName) || !idsl.Reparse) {
            mi->SymType = SymNone;
    ...
    if (pIDD->cFpo) {
        ...
        mi->pFpoData = VirtualAlloc(NULL, sizeof(FPO_DATA) * pIDD->cFpo, MEM_COMMIT, PAGE_READWRITE);
        if (mi->pFpoData) {
            mi->dwEntries = pIDD->cFpo;
            CopyMemory(mi->pFpoData, pIDD->pFpo, sizeof(FPO_DATA) * mi->dwEntries);
            VirtualProtect(mi->pFpoData, sizeof(FPO_DATA) * mi->dwEntries, PAGE_READONLY, &i);
        }
    }

    mi->NumSections = pIDD->cCurrentSections;
    if (pIDD->fCurrentSectionsMapped) {
        mi->SectionHdrs = (PIMAGE_SECTION_HEADER) MemAlloc(sizeof(IMAGE_SECTION_HEADER) * mi->NumSections);
        if (mi->SectionHdrs) {
            CopyMemory(mi->SectionHdrs, pIDD->pCurrentSections, sizeof(IMAGE_SECTION_HEADER) * mi->NumSections);
        }
    } else {
    ...
    if (pIDD->pOriginalSections) {
        ...
        mi->OriginalSectionHdrs = (PIMAGE_SECTION_HEADER) MemAlloc(sizeof(IMAGE_SECTION_HEADER) * mi->NumSections);
        if (mi->OriginalSectionHdrs) {
            CopyMemory(mi->OriginalSectionHdrs, pIDD->pCurrentSections, sizeof(IMAGE_SECTION_HEADER) * mi->NumSections);
        }
    }
    ...
    if (pIDD->pPdb) {
        ...
        if (pIDD->pMappedCv) {
            SymbolsLoaded = LoadCodeViewSymbols(hProcess, mi, pIDD);
    ...
    DoSymbolCallback(ProcessEntry, CBA_DEFERRED_SYMBOL_LOAD_COMPLETE, mi, &idsl, CallbackFileName);
    return TRUE;
}
    ...
    )
{
    ...
    if (mi) {
        ...
        if ( (mi->Flags & MIF_DEFERRED_LOAD) && CompleteDeferredSymbolLoad( hProcess, mi )) {
            return mi->BaseOfDll;
        } else {
            return 0;
        }
...
PMODULE_ENTRY FindModule(HANDLE hProcess, PPROCESS_ENTRY ProcessEntry, LPSTR ModuleName, BOOL LoadSymbols)
{
    PLIST_ENTRY Next;
    PMODULE_ENTRY mi;
    ...
    if (Next) {
        while ((PVOID)Next != (PVOID)&ProcessEntry->ModuleList) {
            ...
            if ((_stricmp( mi->ModuleName, ModuleName ) == 0) || (mi->AliasName[0] && _stricmp( mi->AliasName, ModuleName ) == 0))
            {
...
BOOL GetSizeofPDBSym(PMODULE_ENTRY   mi
    IN     HANDLE               hProcess,
...
Return Value:
    ...
    NULL pointer        - The symbol was not found.  Call GetLastError to discover the cause of the failure.
--*/
{
    PPROCESS_ENTRY      ProcessEntry;
    PMODULE_ENTRY       mi;
...
PSYMBOL_ENTRY GetSymFromAddr(DWORD64         dwAddr, PDWORD64        pqwDisplacement, PMODULE_ENTRY   mi)
{
    PSYMBOL_ENTRY           sym;
    DWORD                   i;
    LONG                    High;
    ...
    if (mi->SymType == SymPdb) {
        ...
        srcBlkAddr = ConvertOmapToSrc( mi, dwAddr, &Bias, (SymOptions & SYMOPT_OMAP_FIND_NEAREST) != 0);
        if (srcBlkAddr == 0) {
        ...
        // locate the section that the address resides in
        {
            ...
            for (i=0, sh=mi->OriginalSectionHdrs; i < mi->OriginalNumSections; i++, sh++) {
                if (srcIPAddr >= (mi->BaseOfDll + sh->VirtualAddress) && srcIPAddr <  (mi->BaseOfDll + sh->VirtualAddress + sh->Misc.VirtualSize))
                {
        ...
        dataSym = (DATASYM32*)GSINearestSym(mi->gsi, (USHORT)(i+1), (ULONG)(srcIPAddr - mi->BaseOfDll - SectionBase), &Displacement32);
        if (pqwDisplacement) {
        ...
        // setup the name
        if (SymOptions & SYMOPT_UNDNAME) {
            SymUnDNameInternal( mi->TmpSym.Name, TMP_SYM_LEN, DataSymNameStart(dataSym), DataSymNameLength(dataSym));
        } else {
...
PMODULE_ENTRY GetModuleForPC(PPROCESS_ENTRY  ProcessEntry, DWORD64         dwPcAddr, BOOL            ExactMatch)
{
    ...
    while ((PVOID)Next != (PVOID)&ProcessEntry->ModuleList) {
        ...
        if ((dwPcAddr == ModuleEntry->BaseOfDll && ModuleEntry->DllSize == 0) || ((dwPcAddr >= ModuleEntry->BaseOfDll) && (dwPcAddr  < ModuleEntry->BaseOfDll + ModuleEntry->DllSize))) {
               return ModuleEntry;
        }
    }
...
PSYMBOL_ENTRY GetSymFromAddrAllContexts(DWORD64         dwAddr, PDWORD64        pqwDisplacement, PPROCESS_ENTRY  ProcessEntry)
{
...
DWORD ComputeHash(LPSTR   lpbName, ULONG   cb)
{
...
PSYMBOL_ENTRY AllocSym(PMODULE_ENTRY   mi, DWORD64         addr, LPSTR           name)
{
    PSYMBOL_ENTRY       sym;
    ULONG               Length;
...
int __cdecl SymbolTableAddressCompare(const void *e1, const void *e2)
{
...
int __cdecl SymbolTableNameCompare(const void *e1, const void *e2)
{
    PSYMBOL_ENTRY    sym1 = (PSYMBOL_ENTRY) e1;
    PSYMBOL_ENTRY    sym2 = (PSYMBOL_ENTRY) e2;
...
VOID CompleteSymbolTable(PMODULE_ENTRY   mi)
{
    PSYMBOL_ENTRY       sym;
    PSYMBOL_ENTRY       symH;
    ULONG               Hash;
    ...
    qsort(mi->symbolTable, mi->numsyms, sizeof(SYMBOL_ENTRY), SymbolTableNameCompare);

    // mark duplicate names
    seq = 0;
    for (i=0; i<mi->numsyms; i++) {
        dups = 0;
        while ((mi->symbolTable[i+dups].NameLength == mi->symbolTable[i+dups+1].NameLength) && (strcmp( mi->symbolTable[i+dups].Name, mi->symbolTable[i+dups+1].Name ) == 0)) {
    ...
    qsort(mi->symbolTable, mi->numsyms, sizeof(SYMBOL_ENTRY), SymbolTableAddressCompare);
...
BOOL CreateSymbolTable(PMODULE_ENTRY   mi, DWORD           SymbolCount, SYM_TYPE        SymType, DWORD           NameSize)
{
    ...
    mi->symbolTable = (PSYMBOL_ENTRY) MemAlloc((sizeof(SYMBOL_ENTRY) * (SymbolCount + OMAP_SYM_EXTRA)) + NameSize + (SymbolCount * CPP_EXTRA));
    if (!mi->symbolTable) {
        return FALSE;
    }
...
PIMAGE_SECTION_HEADER FindSection(PIMAGE_SECTION_HEADER   sh, ULONG                   NumSections, ULONG                   Address)
{
    ULONG i;
    for (i=0; i<NumSections; i++) {
        if (Address >= sh[i].VirtualAddress && Address <  (sh[i].VirtualAddress + sh[i].Misc.VirtualSize)) {
                    return &sh[i];
        }
    }
    ...
    )
{
    ...
    if (!hp) {
        status = (DWORD_PTR)memcpy((PCHAR)buf, (PCHAR)base + sh->PointerToRawData + (address - sh->VirtualAddress), size);
    } else {
    ...
    )
{
    ...
    rc = ReadSectionInfo(hp, base, pIDD, expdir->AddressOfNames, (PCHAR)names, expdir->NumberOfNames * sizeof(ULONG));
    if (!rc)
        goto cleanup;

    rc = ReadSectionInfo(hp, base, pIDD, expdir->AddressOfFunctions, (PCHAR)addrs, expdir->NumberOfFunctions * sizeof(ULONG));
    if (!rc)
        goto cleanup;

    rc = ReadSectionInfo(hp, base, pIDD, expdir->AddressOfNameOrdinals, (PCHAR)ordinals, expdir->NumberOfNames * sizeof(USHORT));
    if (!rc)
        goto cleanup;
    ...
    for (i=0; i<expdir->NumberOfNames; i++) {
        *name = 0;
        ReadSectionInfo(hp, base, pIDD, names[i], name, sizeof(name));
        if (!*name) {
            continue;
        }
        if (SymOptions & SYMOPT_UNDNAME) {
    ...
    )
{
    ...
    LineNumbers = (PIMAGE_LINENUMBER)(PCHAR)pCoffHeader + pCoffHeader->LvaToFirstLinenumber;
    ...
    for (i= 0; i < numberOfSymbols; i++) {
        ...
        if (Symbol->StorageClass == IMAGE_SYM_CLASS_EXTERNAL && Symbol->SectionNumber > 0) {
            ...
            if (szSymName[0] == '?' && szSymName[1] == '?' && szSymName[2] == '_' && szSymName[3] == 'C'    ) {
                // ignore strings
    ...
    )
{
    ...
    if ((*(DWORD *)(omfSig->Signature) != '80BN') && (*(DWORD *)(omfSig->Signature) != '90BN') && (*(DWORD *)(omfSig->Signature) != '11BN')) {
        ...
        if (omfDirEntry->SubSection == sstGlobalPub) {
            ...
            for (j=sizeof(OMFSymHash); j<=omfSymHash->cbSymbol; ) {
                ...
                if (DataSymSeg(dataSym) && (DataSymSeg(dataSym) <= mi->OriginalNumSections))
                {
                    ...
                    if (SymbolName[0] == '?' && SymbolName[1] == '?' && SymbolName[2] == '_' && SymbolName[3] == 'C' )
                    {
                        // ignore strings
                    } else if (SymOptions & SYMOPT_UNDNAME) {
    ...
    for (i=0; i<omfDirHdr->cDir; i++,omfDirEntry++) {
        ...
        else if (omfDirEntry->SubSection == sstSrcModule && (SymOptions & SYMOPT_LOAD_LINES)) {
            AddLinesForOmfSourceModule(mi, (PCHAR)(pIDD->pMappedCv)+omfDirEntry->lfo, (OMFSourceModule *) ((PCHAR)(pIDD->pMappedCv)+omfDirEntry->lfo), NULL);
        }
    }
...
VOID ProcessOmapForModule(PMODULE_ENTRY      mi, PIMGHLP_DEBUG_DATA pIDD)
{
    PSYMBOL_ENTRY       sym;
    PSYMBOL_ENTRY       symN;
    DWORD               i;
    ...
    if (pIDD->pOmapTo) {
        if (pIDD->fOmapToMapped) {
            ...
            if (mi->pOmapTo) {
                CopyMemory(mi->pOmapTo, pIDD->pOmapTo, pIDD->cOmapTo * sizeof(OMAP));
            }
        } else {
    ...
    if (pIDD->pOmapFrom) {
        if (pIDD->fOmapFromMapped) {
            ...
            if (mi->pOmapFrom) {
                CopyMemory(mi->pOmapFrom, pIDD->pOmapFrom, pIDD->cOmapFrom * sizeof(OMAP));
            }
        } else {
    ...
    if (!mi->pOmapFrom || !mi->symbolTable || ((mi->SymType != SymCoff) && (mi->SymType != SymCv)))
    {
        return;
    }
...
BOOL InternalGetModule(HANDLE  hProcess, LPSTR   ModuleName, DWORD64 ImageBase, DWORD   ImageSize, PVOID   Context)
{
    InternalLoadModule(hProcess, ModuleName, NULL, ImageBase, ImageSize, NULL);
    return TRUE;
}
...
LPSTR SymUnDNameInternal(LPSTR UnDecName, DWORD UnDecNameLength, LPSTR DecName, DWORD DecNameLength)
{
    LPSTR p;
    ULONG Suffix;
    ULONG i;
    ...
    if ((DecName[0] == '?') || (DecName[0] == '.' && DecName[1] == '.' && DecName[2] == '?')) {
        __try {
            ...
            if(UnDecorateSymbolName( TmpDecName, UnDecName+Suffix, UnDecNameLength-Suffix, UNDNAME_NAME_ONLY ) == 0 ) {
                strncat( UnDecName, DecName, min(DecNameLength,UnDecNameLength) );
            }
...
PIMAGEHLP_SYMBOL symcpy32(PIMAGEHLP_SYMBOL  External, PSYMBOL_ENTRY       Internal)
{
...
PIMAGEHLP_SYMBOL64 symcpy64(PIMAGEHLP_SYMBOL64  External, PSYMBOL_ENTRY       Internal)
{
...
BOOL SympConvertSymbol64To32(PIMAGEHLP_SYMBOL64 Symbol64, PIMAGEHLP_SYMBOL Symbol32)
{
...
BOOL SympConvertSymbol32To64(PIMAGEHLP_SYMBOL Symbol32, PIMAGEHLP_SYMBOL64 Symbol64)
{
...
BOOL SympConvertLine64To32(PIMAGEHLP_LINE64 Line64, PIMAGEHLP_LINE Line32)
{
...
BOOL SympConvertLine32To64(PIMAGEHLP_LINE Line32, PIMAGEHLP_LINE64 Line64)
{
...
DWORD ReadInProcMemory(HANDLE    hProcess, DWORD64   addr, PVOID     buf, DWORD     bytes, DWORD    *bytesread)
{
    DWORD                    rc;
    PPROCESS_ENTRY           pe;
    ...
    __try {
        ...
        if (pe->pCallbackFunction32) {
            rc = pe->pCallbackFunction32(pe->hProcess, CBA_READ_MEMORY, (PVOID)&rm, (PVOID)pe->CallbackUserContext);

        } else if (pe->pCallbackFunction64) {
            rc = pe->pCallbackFunction64(pe->hProcess, CBA_READ_MEMORY, (ULONG64)&rm, pe->CallbackUserContext);
        }
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        rc = FALSE;
    }
...
__inline BOOL miReadInProcMemory(PMODULE_ENTRY mi, ULONG_PTR     addr, PVOID         buf, DWORD         bytes, DWORD        *bytesread)
{
    ...
    if (pc = MemAlloc(OPTIONALHEADER(DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].Size))) {
        ...
        if (status && cb == OPTIONALHEADER(DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].Size)) {
            switch (ImageFileHdr->Machine) {
                case IMAGE_FILE_MACHINE_ALPHA:
                    feCount = OPTIONALHEADER(DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].Size) / sizeof(IMAGE_ALPHA_RUNTIME_FUNCTION_ENTRY);
                    break;
                case IMAGE_FILE_MACHINE_ALPHA64:
                    feCount = OPTIONALHEADER(DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].Size) / sizeof(IMAGE_ALPHA64_RUNTIME_FUNCTION_ENTRY);
                    break;
                case IMAGE_FILE_MACHINE_IA64:
                    feCount = OPTIONALHEADER(DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].Size) / sizeof(IMAGE_IA64_RUNTIME_FUNCTION_ENTRY);
                    break;
            }
        }
    }
