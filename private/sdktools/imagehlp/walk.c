...
LPVOID FunctionTableAccessRoutineLocal(HANDLE  hProcess, DWORD64 AddrBase);
DWORD64 GetModuleBaseRoutineLocal(HANDLE  hProcess, DWORD64 ReturnAddress);
DWORD64 TranslateAddressRoutineLocal(HANDLE    hProcess, HANDLE    hThread, LPADDRESS64 lpaddr);
...
{
    return ImagepUserReadMemory32(hProcess, (DWORD)qwBaseAddress, lpBuffer, nSize, lpNumberOfBytesRead);
}


LPVOID ImagepFunctionTableAccessThunk(HANDLE  hProcess, DWORD64 AddrBase)
{
    return ImagepUserFunctionTableAccess32(hProcess, (DWORD)AddrBase);
}


PIMAGE_ALPHA64_RUNTIME_FUNCTION_ENTRY AlphaFunctionTableAccessThunk(HANDLE  hProcess, DWORD64 AddrBase)
{
    PIMAGE_ALPHA_RUNTIME_FUNCTION_ENTRY FunctionEntry32;

    FunctionEntry32 = (PIMAGE_ALPHA_RUNTIME_FUNCTION_ENTRY)ImagepUserFunctionTableAccess32(hProcess, (DWORD)AddrBase);
    if (FunctionEntry32) {
...
DWORD64 ImagepGetModuleBaseThunk(HANDLE  hProcess, DWORD64 ReturnAddress)
{
    return (ULONG64)(LONG64)(LONG)ImagepUserGetModuleBase32(hProcess, (DWORD)ReturnAddress);
}


DWORD64 ImagepTranslateAddressThunk(HANDLE    hProcess, HANDLE    hThread, LPADDRESS64 lpaddr)
{
    return 0;
}


void StackFrame32To64(LPSTACKFRAME StackFrame32, LPSTACKFRAME64 StackFrame64)
{
...
void StackFrame64To32(LPSTACKFRAME64 StackFrame64, LPSTACKFRAME StackFrame32)
{
    ...
    )
{
    ...
    switch (MachineType) {
        case IMAGE_FILE_MACHINE_I386:
            rval = WalkX86( hProcess, hThread, &StackFrame, ReadMemory, FunctionTableAccess, GetModuleBase, TranslateAddress);
            break;
        case IMAGE_FILE_MACHINE_ALPHA:
            rval = WalkAlpha( hProcess, &StackFrame, (PCONTEXT) ContextRecord, ReadMemory, GetModuleBase, FunctionTableAccess, FALSE);
            break;
    ...
    )
{
    ...
    if (UseSym) {
        ...
        // Always call syminitialize.  It's a nop if process is already loaded.
        if (!SymInitialize( hProcess, NULL, FALSE )) {
            return FALSE;
        }
    }
    ...
    switch (MachineType) {
        case IMAGE_FILE_MACHINE_I386:
            ...
            break;
        case IMAGE_FILE_MACHINE_IA64:
            rval = WalkIa64( hProcess, StackFrame, (PCONTEXT) ContextRecord, ReadMemory, FunctionTableAccess, GetModuleBase);
            break;
        case IMAGE_FILE_MACHINE_ALPHA:
            rval = WalkAlpha( hProcess, StackFrame, (PCONTEXT) ContextRecord, ReadMemory, GetModuleBase, FunctionTableAccess, FALSE);
            break;
        case IMAGE_FILE_MACHINE_ALPHA64:
            rval = WalkAlpha( hProcess, StackFrame, (PCONTEXT) ContextRecord, ReadMemory, GetModuleBase, FunctionTableAccess, TRUE);
            break;
        default:
            rval = FALSE;
            break;
...
BOOL ReadMemoryRoutineLocal(HANDLE hProcess, DWORD64 qwBaseAddress, LPVOID lpBuffer, DWORD nSize, LPDWORD lpNumberOfBytesRead)
{
    return ReadProcessMemory( hProcess, (LPVOID)(ULONG_PTR)qwBaseAddress, lpBuffer, nSize, lpNumberOfBytesRead );
}
...
DWORD64 TranslateAddressRoutineLocal(HANDLE    hProcess, HANDLE    hThread, LPADDRESS64 paddr)
{
    return 0;
}