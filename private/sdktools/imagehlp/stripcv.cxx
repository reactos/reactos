...
extern "C" BOOL IMAGEAPI CopyPdb(CHAR const * szSrcPdb, CHAR const * szDestPdb, BOOL StripPrivate);

BOOL StripCv(PSZ szImage);
...
BOOL StripCv(PSZ szImage)
{
    ...
    if (*(USHORT *)ImageBase == IMAGE_SEPARATE_DEBUG_SIGNATURE) {
        ...
        DebugDirectories = ImageDirectoryEntryToData(ImageBase, FALSE, IMAGE_DIRECTORY_ENTRY_DEBUG, &DebugDirectorySize );
    ...
    for (i=0; i < NumberOfDebugDirectories; i++) {
        ...
        if (DebugDirectory->Type == IMAGE_DEBUG_TYPE_FIXUP) {
            ...
            RtlZeroMemory(((PUCHAR)ImageBase + DebugDirectory->PointerToRawData), DebugDirectory->SizeOfData);
