...
BOOL MapIt(HANDLE hFile, PLOADED_IMAGE LoadedImage, BOOL   ReadOnly)
{
    HANDLE hMappedFile;

    hMappedFile = CreateFileMapping(hFile, NULL, ReadOnly ? PAGE_READONLY : PAGE_READWRITE, 0, 0, NULL);
    if ( !hMappedFile ) {
        return FALSE;
    }
    ...
    if (!LoadedImage->MappedAddress || !CalculateImagePtrs(LoadedImage)) {
        return(FALSE);
    }
    ...
    )
{
    if (pLi->hFile == INVALID_HANDLE_VALUE) {
        ...
        hMappedFile = CreateFileMapping(pLi->hFile, NULL, PAGE_READWRITE, 0, pLi->SizeOfImage, NULL);
        if ( !hMappedFile ) {
            return FALSE;
        }
...
VOID UnMapIt(PLOADED_IMAGE pLi)
{
    ...
    if (pLi->hFile == INVALID_HANDLE_VALUE) {
        ...
        CheckSumMappedFile( pLi->MappedAddress, pLi->SizeOfImage, &HeaderSum, &CheckSum);
...
BOOL GetImageConfigInformation(PLOADED_IMAGE LoadedImage, PIMAGE_LOAD_CONFIG_DIRECTORY ImageConfigInformation)
{
    ...
    ImageConfigData = (PIMAGE_LOAD_CONFIG_DIRECTORY) ImageDirectoryEntryToData( LoadedImage->MappedAddress, FALSE, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG, &i);
...
PLOADED_IMAGE ImageLoad(LPSTR DllName, LPSTR DllPath)
{
