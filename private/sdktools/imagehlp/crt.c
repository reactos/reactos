...
char * __cdecl ctime(const time_t *timp)
{
    ...
    sprintf( &buf[3], " %02d %02d:%02d:%02d %04d", TimeFields.Day, TimeFields.Hour, TimeFields.Minute, TimeFields.Second, TimeFields.Year );

    return buf;
}


time_t __cdecl time(time_t *timp)
{
    time_t         tm;
    LARGE_INTEGER  MyTime;
