...
BOOL VerifyImageImports(char *ImageName);
...
void __cdecl main(int argc, char *argv[])
{
    ...
    if (argc < 2 || (((argv[1][0] == '/') || (argv[1][0] == '-')) && (argv[1][1] == '?'))) {
        ...
        while (argc > 1) {
            argc--;
            i++;
            printf("Image: \"%s\" should %s marked as Beta\n", argv[i], VerifyImageImports(argv[i]) ? "be" : "not be");
        }
    }
}
...
BOOL VerifyImageImports(char *ImageName)
{
    ...
    if (MapAndLoad(ImageName, ".", &Image, TRUE, TRUE)) {
        ...
        if (NtHeaders != NULL) {
            Imports = (PIMAGE_IMPORT_DESCRIPTOR)ImageDirectoryEntryToData((PVOID)Image.MappedAddress, FALSE, IMAGE_DIRECTORY_ENTRY_IMPORT, &ImportSize);
            for(;Imports;Imports++) {
                if (!Imports->Name) {
                    break;
                }

                ImportModule = (LPSTR) ImageRvaToVa( (PIMAGE_NT_HEADERS) Image.FileHeader, Image.MappedAddress, (ULONG) Imports->Name, &Image.LastRvaSection);
                if (ImportModule) {
                    ThunkNames = (PIMAGE_THUNK_DATA) ImageRvaToVa((PIMAGE_NT_HEADERS)Image.FileHeader,
                    Image.MappedAddress,
                    (ULONG) Imports->OriginalFirstThunk,
                    &Image.LastRvaSection);
                    if (!ThunkNames || ThunkNames->u1.Function == 0) {                        
                        continue;// Skip this one if no thunks or first thunk is the terminating null thunk
                    }
                    ...
                    for(i=0;i<NumberOfThunks;i++) {
                        if (!(BOOL)IMAGE_SNAP_BY_ORDINAL(tname->u1.Ordinal)) {
                            ImportName = (PIMAGE_IMPORT_BY_NAME) ImageRvaToVa((PIMAGE_NT_HEADERS)Image.FileHeader,
                            Image.MappedAddress,
                            (ULONG)(ULONG_PTR) tname->u1.AddressOfData,
                            &Image.LastRvaSection);
                            if (bsearch(ImportName->Name, rgszBetaAPIs, sizeof(rgszBetaAPIs) / sizeof(char *), sizeof(char *), CompareNames)) {
                                printf("%s: Imports \"%s\" from \"%s\"\n", ImageName, ImportName->Name, ImportModule);
                                rc = TRUE;
                            }
                        }
