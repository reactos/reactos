...
Abstract:
    This module implements a function for computing the checksum of an image file. It will also compute the checksum of other files as well.
...
Arguments:
    ...
    HeaderSum - Suppllies a pointer to a variable that receives the checksum from the image file, or zero if the file is not an image file.
...
--*/
{
    ...
    *HeaderSum = 0;
    ...
    // in the optional header from the computed checksum before adding the file length, and set the value of the header checksum.
    __try {
...
/*++
Routine Description:
    This functions maps the specified file and computes the checksum of the file.
Arguments:
    Filename - Supplies a pointer to the name of the file whose checksum is computed.
    HeaderSum - Supplies a pointer to a variable that receives the checksum from the image file, or zero if the file is not an image file.
    CheckSum - Supplies a pointer to the variable that receive the computed checksum.
Return Value:
...
--*/
{
    ...
    //  of this function.
    if (WideCharToMultiByte(CP_ACP, 0, Filename, -1, FileNameA, MAX_PATH, NULL, NULL ) ) {
    ...
    //  Create a file mapping, map a view of the file into memory, and close the file mapping handle.
...
BOOL TouchFileTimes(HANDLE FileHandle, LPSYSTEMTIME lpSystemTime)
{
