...
NET_API_STATUS NET_API_FUNCTION NetRegisterDomainNameChangeNotification(PHANDLE NotificationEventHandle)
/*++
Routine Description:
    This function is used to register a notification for a domain name change.
    The waitable event that is returned gets signalled when ever the flat or dns domain name is changed.
Arguments:
    NotificationHandle - Where the handle to the created notification event is returned.
Return Value:
...
--*/
{
    ...
    if ( EventHandle == NULL ) {
        ...
        Status = LsaRegisterPolicyChangeNotification( PolicyNotifyDnsDomainInformation, EventHandle );
        if ( !NT_SUCCESS( Status ) ) {// If the function was successful, return the event handle.  Otherwise, close the event
...
NET_API_STATUS NET_API_FUNCTION NetUnregisterDomainNameChangeNotification(HANDLE NotificationEventHandle)
/*++
Routine Description:
    This function is used to unregister a previously registered notification for a domain name change.
...
--*/
{
    ...
    Status = LsaUnregisterPolicyChangeNotification( PolicyNotifyDnsDomainInformation, NotificationEventHandle );
    Err = NetpNtStatusToApiStatus( Status );
    if ( Err == NERR_Success ) {// If the unregister was successful, close the event handle
