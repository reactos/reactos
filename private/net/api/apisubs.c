...
{
#if 0
    NetpKdPrint((PREFIX_NETAPI "Initializing, Reason=" FORMAT_DWORD ", thread ID " FORMAT_NET_THREAD_ID ".\n", (DWORD) Reason, NetpCurrentThread() ));
#endif
    ...
    if (Reason == DLL_PROCESS_ATTACH) {
        ...
        {
            NetpKdPrint((PREFIX_NETAPI "DisableThreadLibraryCalls failed: " FORMAT_API_STATUS "\n", GetLastError()));
        }

        NetbiosInitialize(DllHandle);// Initialize Netbios
        NetpInitRpcBindCache();// Initialize RPC Bind Cache
