...
/*++
Routine Description:
    This routine raises an alert to notify the Alerter service by writing to the Alerter service mailslot.
Arguments:
    AlertType - Supplies the name of the alert event which could be one of the three the Alerter service supports: ADMIN, USER, or PRINTING.
        The ALERT_xxx_EVENT equates are used to provide these strings.
    Buffer - Supplies the data to be written to the alert mailslot. This must begin with a STD_ALERT structure.
...
--*/
{
    ...
    ApiStatus = NetpAlertStructureInfo((LPWSTR)AlertType, & MaxTotalSize, & RequiredFixedSize, & MaxVariableSize );
...
NET_API_STATUS NET_API_FUNCTION NetAlertRaiseEx(IN LPCWSTR AlertType, IN LPVOID  VariableInfo, IN DWORD VariableInfoSize, IN LPCWSTR ServiceName)
/*++
Routine Description:
    This routine raises an alert to notify the Alerter service by writing to the Alerter service mailslot.
Arguments:
    AlertType - Supplies the name of the alert event which could be one of the three the Alerter service supports: ADMIN, USER, or PRINTING.
        The ALERT_xxx_EVENT equates are used to provide these strings.
    VariableInfo - Supplies the variable length portion of the alert notification.
    VariableInfoSize - Supplies the size in number of bytes of the variable portion of the notification.
...
#define TEMP_VARIABLE_SIZE (512-sizeof(STD_ALERT))       // BUGBUG: arbitrary?
    ...
    (VOID) memcpy(ALERT_OTHER_INFO(Alert), VariableInfo, VariableInfoSize);// Copy variable portion to end of our buffer.
    Alert->alrt_timestamp = (DWORD) time_now();// Store current time in seconds since 1970.
    (VOID) STRCPY(Alert->alrt_eventname, AlertType);// Put alert event name into AlertMailslotBuffer
    (VOID) STRCPY(Alert->alrt_servicename, ServiceName);// Put service name into AlertMailslotBuffer
