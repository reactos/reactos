...
#define INCREMENT_BUFFER_SIZE   (4096*2)
{
    ...
    */
    while ((ntstatus = NtQuerySystemInformation(SystemProcessInformation, pProcessBuffer, ProcessBufSize, &dwReturnedBufferSize)) == STATUS_INFO_LENGTH_MISMATCH)
    {
        /* expand buffer & retry */
...
static BOOL AddHrSWRunRow(PSYSTEM_PROCESS_INFORMATION ProcessInfo   /* --> Next process to process */
)
...
#define ANSI_PNAME_LEN 256
{
    ...
    */
    if (status == 2 && type == 4)
    {  /* If it is a runnable application . . . */
...
#define ANSI_PARAM_LEN (MAX_PATH * 2)
    ...
    static CHAR            pbuf[ANSI_PARAM_LEN + 1];    /* Buffer for "parameters"      */
    ...
    *params_str = NULL;
    ...
    if (ReadProcessMemory(hProcess, BasicInfo.PebBaseAddress, &Peb, sizeof(PEB), NULL))
    {
        ...
        {
            // get cmdline
            if (ReadProcessMemory(hProcess, ProcessParameters.CommandLine.Buffer, szT, sizeof(szT), &dwbytesret))
            {
