...
TrapInit( IN OUT HANDLE *hPollForTrapEvent )
{
    ...
    *hPollForTrapEvent = NULL ;
    ...
    // the Extendible Agent to call this Extension Agent's SnmpExtensionTrap routine to collect any traps.
...
BOOL            waitable;       /* Status from SetWaitable()     */
    ...
    *hPollForTrapEvent = CreateWaitableTimer(NULL,      // Security
    ...
    /*
    | Set a negative due time to mean "relative": We want it to go off in 30 seconds.
    | Ticks are 100 ns or 1/10th of a millionth of a second.
    |
    */
