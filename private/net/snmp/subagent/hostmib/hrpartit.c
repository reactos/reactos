 ...
 *  Abstract:
 *    This module contains the code for dealing with the get, set, and instance name routines for the HrPartitionEntry.
...
|
| This is a unique sub-table within the hrDevice table in that it is doubly indexed.
|
 ...
 *    instance                   address of instance name as ordered native data type(s)
 ...
 * mibtget.c v0.10
 ...
 |                *-hrDevice
*/
UINT GetHrPartitionIndex(OUT Integer *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
 ...
 * mibtget.c v0.10
 ...
 |                *-hrDevice
*/
UINT GetHrPartitionLabel(OUT InternationalDisplayString *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
    ULONG           disk_index;     /* As fetched from instance structure */
    ULONG           part_index;     /* As fetched from instance structure */
    CACHEROW        *disk_row;      /* Row entry fetched from Disk cache  */
 ...
 * mibtget.c v0.10
 ...
 |                *-hrDevice
*/
UINT GetHrPartitionID(OUT OctetString *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
    ULONG           disk_index;     /* As fetched from instance structure */
    ULONG           part_index;     /* As fetched from instance structure */
    CACHEROW        *disk_row;      /* Row entry fetched from Disk cache  */
 ...
 * mibtget.c v0.10
 ...
 |                *-hrDevice
*/
UINT GetHrPartitionSize(OUT KBytes *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
    ULONG           disk_index;     /* As fetched from instance structure */
    ULONG           part_index;     /* As fetched from instance structure */
    CACHEROW        *disk_row;      /* Row entry fetched from Disk cache  */
 ...
 * mibtget.c v0.10
 ...
 |                *-hrDevice
*/
UINT GetHrPartitionFSIndex(OUT Integer *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
    ULONG           disk_index;     /* As fetched from instance structure */
    ULONG           part_index;     /* As fetched from instance structure */
    CACHEROW        *disk_row;      /* Row entry fetched from Disk cache  */
...
/*
 *     This routine is used to verify that the specified instance is valid.

 *  Arguments:
 *     FullOid                 Address for the full oid - group, variable, and instance information
...
*/
UINT HrPartitionEntryFindInstance(IN ObjectIdentifier *FullOid, IN OUT ObjectIdentifier *instance)
{
    ...
    //  However, if there is any context that needs to be set, it can be done here.
    ...
    // Instance is 2 arcs for this table:
        ...
        // The only valid instance for a non-table are instance 0.
        // If this is a non-table, the following code validates the instances.
        // If this is a table, developer modification is necessary below.
    ...
    */
    if (dev_row->attrib_list[HIDDEN_CTX].attrib_type != CA_CACHE || (part_cache = dev_row->attrib_list[HIDDEN_CTX].u.cache) == NULL) {
        return SNMP_ERRORSTATUS_NOSUCHNAME;
    }
    ...
    */
    ...
    {
        // the both instances are valid.  Create the instance portion of the OID to be returned from this call.
 ...
 *     SNMP_ERRORSTATUS_NOSUCHNAME  Invalid instance
*/
UINT HrPartitionEntryFindNextInstance(IN ObjectIdentifier *FullOid, IN OUT ObjectIdentifier *instance)
{
    ...
    if (FullOid->idLength <= HRPARTITIONENTRY_VAR_INDEX) {
        ...
        */
        if (dev_row != NULL) {
            ...
            if (dev_row->attrib_list[HRDS_REMOVABLE].u.unumber_value == FALSE) {
                /*
                ...
                */
                if ((hrDevice_row->attrib_list[HIDDEN_CTX].attrib_type != CA_CACHE) || (hrDevice_row->attrib_list[HIDDEN_CTX].u.cache == NULL)) {
                    return SNMP_ERRORSTATUS_GENERR;
                }
    ...
    */
    ...
    while (1) {         /* "General Roll" */
        ...
        */
        if ((part_row = FindNextTableRow(part_tmp_instance, part_cache)) != NULL) {
            /*
...
/* RollToNextFixedDisk - Helper Routine for HrPartitionEntryFindNextInstance */
/* RollToNextFixedDisk - Helper Routine for HrPartitionEntryFindNextInstance */
static UINT RollToNextFixedDisk(
/* Index */
UINT       *dev_tmp_instance,     /* ->Device Table Instance Arc    ("1st") */
UINT       *part_tmp_instance,    /* ->Partition Table Instance Arc ("2nd") */
CACHEROW  **dev_row,              /* ->> Entry in hrDevice Table            */
...
|
|       Note that "*dev_tmp_instance" is also implicitly the index into the hrDiskStorage table as well.
|
|       "dev_row" - is a pointer to the pointer to the hrDiskStorage row currently selected by "*dev_tmp_instance".
|       It is the case that "*dev_row" might be null, indicating that no row has been selected yet.
|
    ...
    while (1) {   /* Forever . . . */
        ...
        */
        ...
        if (*dev_row == NULL) {
            ...
            if ((*dev_row = FindNextTableRow(*dev_tmp_instance, &hrDiskStorage_cache)) == NULL) {
                return SNMP_ERRORSTATUS_NOSUCHNAME;
            }
        }
        else {
        ...
        */
        ...
        |
        | if (no hrdevice entry was found) */
        if ((hrDevice_row = FindTableRow(*dev_tmp_instance, &hrDevice_cache)) == NULL) {
            /*
 ...
 *     is that object identifier that is returned from the Find Instance or Find Next Instance routines.
 *     It is NOT the full object identifier that contains the group and variable object ids as well.
 *     The native representation is an argc/argv-like structure that contains the ordered variables that define the instance.
 *     This is specified by the MIB's INDEX clause.
 *     See RFC 1212 for information about the INDEX clause.

 *  Arguments:
 *     oid_spec                Address of the object id instance specification
 *     native_spec             Address to return the ordered native instance specification

 *  Return Codes:
 *     SUCCESS                 Conversion complete successfully
 *     FAILURE                 Unable to convert object id into native format
*/
UINT HrPartitionEntryConvertInstance(IN ObjectIdentifier *oid_spec, IN OUT InstanceName *native_spec)
{
...
*/
void HrPartitionEntryFreeInstance(IN OUT InstanceName *instance)
{
