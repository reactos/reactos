...
UINT GetHrSWInstalledIndex(OUT Integer *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
...
UINT GetHrSWInstalledName(OUT InternationalDisplayString *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
    ULONG           index;          /* As fetched from instance structure */
    CACHEROW        *row;           /* Row entry fetched from cache       */
...
UINT GetHrSWInstalledID(OUT ProductID *outvalue, IN Access_Credential *access, IN InstanceName *instance)
{
    /*
...
#define SUBKEY_LEN 64   // Long enough for short key-name of software
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,    // hkey - currently open
        ...
        for (index = 0; enum_status != ERROR_NO_MORE_ITEMS; index += 1) {/* Enumerate the keys using the Uninstall subkey */
            ...
            if (RegOpenKeyEx(subkey,                // hkey - currently open
                ...
                add_status = AddSWInstalled(sw_key,   // Key to obtain DisplayName
