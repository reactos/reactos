...
while ( strlen(pDrvStrings) > 0 ) {
    ...
    */
    if (GetDiskFreeSpace(pDrvStrings,           // drive
        ...
        /* Failure */
        row->attrib_list[HRST_ALLOC].u.unumber_value = 0;/* =========== hrStorageAllocationUnits ==========*/
        row->attrib_list[HRST_SIZE].u.unumber_value = 0;/* =========== hrStorageSize ==========*/
        row->attrib_list[HRST_USED].u.unumber_value = 0; /* =========== hrStorageUsed ==========*/
        }
...
row->attrib_list[HRST_ALLOC].attrib_type = CA_NUMBER;
row->attrib_list[HRST_ALLOC].u.unumber_value = sys_info.dwAllocationGranularity;
...
row->attrib_list[HRST_SIZE].attrib_type = CA_NUMBER;
row->attrib_list[HRST_SIZE].u.unumber_value = (DWORD)(mem_status.dwTotalPageFile / sys_info.dwAllocationGranularity);


/* =========== hrStorageUsed ==========*/
row->attrib_list[HRST_USED].attrib_type = CA_NUMBER;
row->attrib_list[HRST_USED].attrib_type = (int)((mem_status.dwTotalPageFile - mem_status.dwAvailPageFile) / sys_info.dwAllocationGranularity);
    ...
    while (dwRead > 0) {
        ...
        */
        if (   pevlr->EventID == EVENTLOG_START_ID
            ...
            && strcmp( ((LPSTR) ((LPBYTE) pevlr + sizeof(EVENTLOGRECORD))), EVENTLOG_START_SRC) == 0) {
            keep_scanning = FALSE;
            break;
            }
...
return (alloc_failures);/* Give 'em the count */
}
