         ...
         OUT tPrimaryModuleParms &Params)  // allocated output buffer, ready to receive the values for the requested parameters
{
    ...
    retCode = RegOpenKeyEx(hkLogFile, tchPrimModule, 0, KEY_READ, &hkPrimaryModule);
    ...
    if (Params.dwParams & PMP_PARAMMSGFILE) // "ParameterMessageFile" is requested
    {
        ...
        retCode = RegQueryValueEx(hkPrimaryModule, EXTENSION_PARM_MODULE, 0, &dwType, (LPBYTE) tszParamMsgFileName, &dwParamMsgFileLen);
        if (retCode == ERROR_SUCCESS)
        {
            TCHAR tszExpandedFileName[MAX_PATH+1];

            if (ExpandEnvironmentStrings(tszParamMsgFileName, tszExpandedFileName, MAX_PATH+1) > 0)
            {
