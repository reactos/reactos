...
extern "C" {
...

    {
        ...
        if (hkRegResult == NULL)
        {
            if ((status = RegOpenKeyEx(HKEY_LOCAL_MACHINE, EXTENSION_PARM, 0, KEY_READ | KEY_SET_VALUE, &hkRegResult)) != ERROR_SUCCESS)                    // open registry information
            {
        ...
        if (!fRegNotify)
        {
            ...
            if ((hRegChanged = CreateEvent((LPSECURITY_ATTRIBUTES) NULL, FALSE, FALSE, (LPTSTR) NULL)) == NULL)
            {
                lastError = GetLastError(); // save error status
        ...
        while ((status = RegEnumValue(hkRegResult, iValue, parmName, &nameSize, 0, &dwType, (LPBYTE) &parm, &parmSize)) != ERROR_NO_MORE_ITEMS)
        ...
        if ((status = RegOpenKeyEx(HKEY_LOCAL_MACHINE, EVENTLOG_BASE, 0, (KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS), &hkResult)) != ERROR_SUCCESS)                     // open for log names
        {
...
extern "C" {
...

    {
        ...
        case DLL_PROCESS_ATTACH:
            ...
            if ((hStopAll = CreateEvent((LPSECURITY_ATTRIBUTES) NULL, FALSE, FALSE, (LPTSTR) NULL)) == NULL)
            {
                lastError = GetLastError(); // save error status
...
extern "C" {
...

    {
        ...
        if ((hEventNotify = CreateEvent((LPSECURITY_ATTRIBUTES) NULL, FALSE, FALSE, (LPTSTR) NULL)) == NULL)
        {
            lastError = GetLastError(); // save error status
    ...
    if (Result) {
        ...
        if (Result) {
            ...
            if (Result) {
                ...
                Result = LookupPrivilegeValue(NULL, "SeSecurityPrivilege", &Luid);
                if (Result) {
