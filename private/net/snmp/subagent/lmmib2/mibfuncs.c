         ...
         switch ( LMData )
            ...
            case MIB_LM_COMVERSIONMAJ:
                ...
                if ((NULL == cache_table[C_NETWKSTAGETINFO].bufptr) ||
                    (curr_time > (cache_table[C_NETWKSTAGETINFO].acquisition_time + cache_expire[C_NETWKSTAGETINFO])))
              {  // it has expired!
                ...
                if (lmCode == 0) {  // valid so return it, otherwise error NULL
                    ...
                    if (NULL == (stream = SnmpUtilMemAlloc(strlen(temp)))) {
                    ...
                    if (cache_it) {
                        ...
                        cache_table[C_NETWKSTAGETINFO].acquisition_time = curr_time;
            ...
            case MIB_LM_COMTYPE:
                if ((NULL == cache_table[C_NETSERVERGETINFO].bufptr) ||
                    (curr_time > (cache_table[C_NETSERVERGETINFO].acquisition_time + cache_expire[C_NETSERVERGETINFO])))
                {  // it has expired!
               ...
               NetServerGetInfo( NULL,                  // local server
                ...
                if (lmCode == 0) {  // valid so return it, otherwise error NULL
                    ...
                    if (NULL == (stream = SnmpUtilMemAlloc(4 * sizeof(BYTE)))) {
                        SnmpUtilMemFree(retval);
                        retval = NULL;
                        goto Exit;
                    }
                    *(DWORD*)stream = server_info_two->sv102_type & 0x000000FF;
                    ...
                    if (cache_it) {
                        // now save it in the cache
                        cache_table[C_NETSERVERGETINFO].acquisition_time = curr_time;
            ...
            case MIB_LM_COMSTATNUMNETIOS:
              ...
              {  // it has expired!
               ...
               if(lmCode == 0)  {  // valid so return it, otherwise error NULL
                       ...
                       retval->d.intval = (wrk_stats_zero->SmbsReceived).LowPart + (wrk_stats_zero->SmbsTransmitted).LowPart;

                       if(cache_it) {
                       // now save it in the cache
                                cache_table[C_NETSTATISTICSGET_WORKST].acquisition_time = curr_time ;
            ...
            case MIB_LM_COMSTATFINETIOS:
              if((NULL == cache_table[C_NETSTATISTICSGET_WORKST].bufptr) ||
                 (curr_time >
                        (cache_table[C_NETSTATISTICSGET_WORKST].acquisition_time + cache_expire[C_NETSTATISTICSGET_WORKST]              ) ) )
              {  // it has expired!
