...
SNMPAPI MIB_HashInit()
{
    ...
    for ( I=0;I < MIB_num_variables;I++ )
    {
        ...
        if ( MIB_HashTable[HashRes] == NULL )
        {
            ...
            ht_ptr = MIB_HashTable[HashRes];// Make copy of position in hash table to save MIB entry
        }
        else
        {
            // Find end of bucket
            ...
            ht_ptr = ht_ptr->Next;// Make copy of position in hash table to save MIB entry
        }
          ...
          )
{
    ...
    HashPos = MIB_Hash( Oid );// Hash OID to find position in Hash Table
