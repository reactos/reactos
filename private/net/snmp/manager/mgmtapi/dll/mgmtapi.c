...
--*/
{
    ...
    EnterCriticalSection(&g_GlobalLock);// obtain exclusive access to request id
    dwRequestId = g_dwRequestId++;// obtain copy of request id
    LeaveCriticalSection(&g_GlobalLock);// obtain exclusive access to request id
...
--*/
{
    ...
    WSNMP_ASSERT(pSMS != NULL);// validate session ptr
    ...
    if ((pVarBind != NULL) && (pVarBind->name.ids != NULL) && (pVarBind->name.idLength != 0)) {
        ...
        if (pSMS->nPduType == SNMP_PDU_SET) {
            tmpValue.syntax = (smiINT32)(BYTE)pVarBind->value.asnType;// syntax values are equivalent
            ...
            case ASN_COUNTER64:
                ...
                tmpValue.value.hNumber.lopart = pVarBind->value.asnValue.counter64.LowPart;
                tmpValue.value.hNumber.hipart = pVarBind->value.asnValue.counter64.HighPart;
                break;
            case ASN_OPAQUE:
            case ASN_IPADDRESS:
            ...
            case ASN_BITS:
                ...
                tmpValue.value.string.len = pVarBind->value.asnValue.string.length;
                tmpValue.value.string.ptr = pVarBind->value.asnValue.string.stream;
                break;
            case ASN_OBJECTIDENTIFIER:

                // transfer object id
                tmpValue.value.oid.len = pVarBind->value.asnValue.object.idLength;
                tmpValue.value.oid.ptr = pVarBind->value.asnValue.object.ids;
                break;
            case ASN_NULL:
        ...
        if (fOk) {
            ...
            status = SnmpSetVb(
                        ...
                        0, // index
                        &tmpOID,
                        (pSMS->nPduType == SNMP_PDU_SET) ? &tmpValue : NULL
                        );
...
--*/
{
    ...
    if (WSNMP_SUCCEEDED(pSMS->hVbl)) {
        fOk = TRUE;// re-init
        pVarBind = pSMS->pVarBindList->list;// initialize varbind pointer
        cVarBind = pSMS->pVarBindList->len;// initialize varbind count
        ...
        if (!fOk) {
            status = SnmpFreeVbl(pSMS->hVbl);// release varbind list handle
...
--*/
{
    ...
    if (pSMS->hVbl != (HSNMP_VBL)NULL) {
        status = SnmpFreeVbl(pSMS->hVbl);// actually release vbl handle
        ...
        pSMS->hVbl = (HSNMP_VBL)NULL;// re-initialize handle
    }
...
--*/
{
    ...
    if (pSMS->hPdu != (HSNMP_PDU)NULL) {
        FreeVbl(pSMS);// free vbl
        status = SnmpFreePdu(pSMS->hPdu);// actually release pdu handle
        ...
        pSMS->hPdu = (HSNMP_PDU)NULL;// re-initialize handle
    }
