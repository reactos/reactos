...
// Original Microsoft code modified by ACE*COMM for use with WSNMP32.DLL and other trap receiving clients, per contract.

// Bob Natale, ACE*COMM (bnatale@acecomm.com)
// For NT v5 Beta, v970228
// Additional enhancements planned.

// This version of SNMPTRAP has no dependencies on either MGMTAPI.DLL or WSNMP32.DLL.

// WinSNMP clients use the SnmpRegister() function.

// Other clients will need to match the following values and structures:
...
//     two threads...one for IP and one for IPX, to comply with WinSock v2's restrictions against multi-protocol select().
...
VOID WINAPI svcMainFunction (IN DWORD dwNumServicesArgs, IN LPSTR *lpServiceArgVectors);
...
// trap notification pipes.shared by this thread and the pipe thread
{
...
while (TRUE)
   ...
   if (pRecvTrap == NULL || pRecvTrap->TrapBufSz < ulTrapSize)
   {
       if (pRecvTrap != NULL)
       {
   ...
   len = recvfrom (fd,pRecvTrap->TrapBuf,pRecvTrap->TrapBufSz,0, &(pRecvTrap->Addr),&(pRecvTrap->AddrLen));
   if (len == SOCKET_ERROR)
      continue;
   EnterCriticalSection (&cs_PIPELIST);
   ...
   if (!ll_empt(pSvrPipeListHead))
      ...
      while (item = ll_next(item, pSvrPipeListHead))
         {
         if (!WriteFile(((svrPipeListEntry *)item)->hPipe,(LPBYTE)pRecvTrap,len,&written,NULL))
            {
...
PACL AllocGenericACL()
{
    ...
    if ( !AllocateAndInitializeSid( &Authority,2,SECURITY_BUILTIN_DOMAIN_RID,DOMAIN_ALIAS_RID_ADMINS,0, 0, 0, 0, 0, 0,&pSidAdmins ) ||
         !AllocateAndInitializeSid( &Authority,2,SECURITY_BUILTIN_DOMAIN_RID,DOMAIN_ALIAS_RID_USERS,0, 0, 0, 0, 0, 0,&pSidUsers ))
    {
        return NULL;
    }
    ...
    if (pAcl != NULL)
    {
        if (!InitializeAcl( pAcl, dwAclLength, ACL_REVISION) ||
            !AddAccessAllowedAce ( pAcl,ACL_REVISION,GENERIC_READ | GENERIC_WRITE,pSidAdmins ) ||
            !AddAccessAllowedAce ( pAcl, ACL_REVISION,(GENERIC_READ | (FILE_GENERIC_WRITE & ~FILE_CREATE_PIPE_INSTANCE)),pSidUsers ))
        {
            GlobalFree(pAcl);
            pAcl = NULL;
        }
...
{
    // This thread creates a named pipe instance and blocks waiting for a client connection.
    // When client connects, the pipe handle is added to the list of trap notification pipes.
    ...
    if ((pAcl = AllocGenericACL()) == NULL || !SetSecurityDescriptorDacl(&S_Desc, TRUE, pAcl, FALSE))
    {
        FreeGenericACL(pAcl);
        return (0);
    }
    ...
    while (TRUE)
    {
        ...
        hPipe = CreateNamedPipe(SNMPMGRTRAPPIPE, PIPE_ACCESS_DUPLEX, (PIPE_WAIT | PIPE_READMODE_MESSAGE | PIPE_TYPE_MESSAGE), PIPE_UNLIMITED_INSTANCES, nOutBufLen, nInBufLen, 0, &S_Attrib);
        if (hPipe == INVALID_HANDLE_VALUE)
        {
            break;
        }
        else if (!ConnectNamedPipe(hPipe, NULL) && (GetLastError() != ERROR_PIPE_CONNECTED))
        {
            break;
        }
        else if (!(item = (svrPipeListEntry *) GlobalAlloc(GPTR, sizeof(svrPipeListEntry))))
        {
            break;;
        }
        else
