...
void MsgNotify (smiUINT32 msgType, smiLPBYTE msgAddr, smiUINT32 msgLen, smiUINT32 nAgent, LPSAS host)
{
    ...
    case NP_TRAP:
        if (TrapDescr.Used && // Discard traps if no registrations
            (pduType == SNMP_PDU_INFORM || pduType == SNMP_PDU_TRAP || pduType == SNMP_PDU_V1TRAP))
        {
                ...
                (pMsg->dllReqId == reqID))   // Must match up!
            {
                ...
                { // window/message session notification mode
                    if (IsWindow(pSession->hWnd))
                    {
                        ...
                        PostMessage(pSession->hWnd, pSession->wMsg, 0, pMsg->appReqId);
                    }
                    else
                        FreeMsg(nMsg);
                }
...
THR_TYPE WINAPI thrNotify (LPVOID cbSessionSlot)
{
    ...
    do
    {
        ...
        if (pSession->thrCount != 0xFFFFFFFF && pSession->thrCount != 0)
            pSession->thrCount-- ;
        ...
        EnterCriticalSection (&cs_MSG);
        for (nUsed=0, nMsg=0; nUsed<MsgDescr.Used && nMsg<MsgDescr.Allocated; nMsg++)
        {
            pMsg = snmpGetTableEntry(&MsgDescr, nMsg);
            if (pMsg->Session == hSession && (pMsg->Status == NP_RCVD || pMsg->Status == NP_EXPIRED))
            {
        ...
        for (nMsg = 0, nFound = 0; nFound < MsgDescr.Used && nMsg < MsgDescr.Allocated; nMsg++)
        {
            ...
            if (pMsg->nRetransmitMode)
            {
                if (pMsg->Tries)
                {
                    ...
                    // Send the data
                    sendto(tSock, pMsg->Addr, pMsg->Size, 0, (LPSOCKADDR) &(pMsg->Host), sizeof(SAS));
                ...
                else
                {
                    ...
                    { // windows/message session notification mode
                        if (IsWindow(pSession->hWnd))
                        {
                            PostMessage(pSession->hWnd, pSession->wMsg, SNMPAPI_TL_TIMEOUT, pMsg->appReqId);
                        }
                    }
...
#define TRAPBUFSIZE 4096
    ...
    if (TaskData.sEnv.dwPlatformId == VER_PLATFORM_WIN32_NT)
    {
        ...
        while (TRUE)
        {
            if (ReadFile(TaskData.trapPipe, (LPBYTE) &recvTrap, sizeof(SNMP_TRAP) - sizeof(recvTrap.TrapBuf), &iBytes, NULL) || GetLastError() != ERROR_MORE_DATA)
                break;

            // Find the message buffer address...
            rMsgPtr = GlobalAlloc(GPTR, 2 * recvTrap.TrapBufSz);

            if (rMsgPtr == NULL || !ReadFile(TaskData.trapPipe, (LPBYTE) rMsgPtr, 2 * recvTrap.TrapBufSz, &iBytes, NULL)) // No space error or wrong data on pipe...drop the message...
            {
                GetLastError();
                continue;
            }
...
#define MAX_PORTS 10
        ...
        if (TaskData.trapWnd == NULL)
        {
            ...
            if (CreateProcess(NULL, NP_WSX95_EXE, NULL, NULL, FALSE, 0, NULL, NULL, &su, &pi))
            {
...
BOOL MapV1TrapV2(LPPDUS pdu)
{
    ...
    { // SNMP_TRAP_ENTERPRISE
        VbTrap->value.value.oid.ptr[pdu->v1Trap->enterprise.len + 1] = pdu->v1Trap->specific_trap;
    }
...
#define MAXSLOTS 10 // maximum active trap receivers
    ...
    for (nTrap = 0; nTrap < TrapDescr.Allocated; nTrap++)
    {
        ...
        {                            // Nope, specific test
            ...
            lError = SnmpOidCompare(&(pTrap->notification), &pdu->VBL_addr->next_var->value.value.oid, pTrap->notification.len, &nCmp);
        ...
        if (pTrap->agentEntity)         // Specific agent?
        {
            ...
            if (host->ipx.sa_family == AF_IPX)
                nResult = memcmp(&host->ipx.sa_netnum, &(pEntity->addr.ipx.sa_netnum), AF_IPX_ADDR_SIZE);
            else // AF_IPX
                nResult = memcmp(&host->inet.sin_addr, &(pEntity->addr.inet.sin_addr), AF_INET_ADDR_SIZE);
            if (nResult)
                continue;                           // not equal...
        }
        if (pTrap->Context)             // Specific context?
        {
            ...
            if (memcmp(community->ptr, pCtxt->commStr, (size_t) community->len))
    ...
    while (nFound)
    {
        ...
        CopyMemory(&(pMsg->Host), host, sizeof(SAS));
        if (!(BuildMessage(1, community, pdu, pdu->appReqId, &(pMsg->Addr), &(pMsg->Size))))
        {
    ...
    // The next while loop actually sends the one or more trap messages to the application(s)...
    ...
    { // Now actually send the message(s)
        ...
        { // window/message session notification mode
            ...
            {
                pMsg->Status = NP_READY;
                PostMessage(pSession->hWnd, pSession->wMsg, 0, 0L);
            }
            else
                FreeMsg(nMsg);
        }
...
LPPDUS MapV2TrapV1(HSNMP_PDU hPdu)
{
    ...
    //   1.  If any of the varBinds in the varBindList has an SNMPv2 syntax of Counter64, then such varBinds are implicitly considered to
    //       be not in view, and so they are removed from the varBindList to be sent with the SNMPv1 trap.
    ...
    //   2.  The 3 special varBinds in the varBindList of an SNMPv2 trap (sysUpTime.0 (TimeTicks), snmpTrapOID.0 (OBJECT IDENTIFIER) and
    //       optionally snmpTrapEnterprise.0 (OBJECT IDENTIFIER)) are removed from the varBindList to be sent with the SNMPv1 trap.
