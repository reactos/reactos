...
switch (TaskData.nTranslateMode)
   ...
   if (!GetPrivateProfileString ("Entities", entityString, "", profileBuf, sizeof(profileBuf), "NP_WSNMP.INI"))
      {
...
if (pCtxt->commLen = contextString->len)
   ...
   switch (TaskData.nTranslateMode)
      {
      case SNMPAPI_TRANSLATED:
      if (!GetPrivateProfileString ("Contexts", contextString->ptr, "", profileBuf, sizeof(profileBuf), "NP_WSNMP.INI"))
         {
