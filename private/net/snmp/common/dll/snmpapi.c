...
--*/
{
    ...
    if (g_hNtDll != NULL) {
        ...
        g_pfnRtlGetNtProductType = (PFNRTLGETNTPRODUCTTYPE)GetProcAddress(g_hNtDll, "RtlGetNtProductType");

        // load api to help determine system uptime
        g_pfnNtQuerySystemInformation = (PFNNTQUERYSYSTEMINFORMATION)GetProcAddress(g_hNtDll, "NtQuerySystemInformation");

        // load api to help determine system uptime
        g_pfnRtlExtendedLargeIntegerDivide = (PFNRTLEXTENDEDLARGEINTEGERDIVIDE)GetProcAddress(g_hNtDll, "RtlExtendedLargeIntegerDivide");

        // validate each function pointer
        fOk = ((g_pfnRtlGetNtProductType != NULL) && (g_pfnNtQuerySystemInformation != NULL) && (g_pfnRtlExtendedLargeIntegerDivide != NULL));
    }
