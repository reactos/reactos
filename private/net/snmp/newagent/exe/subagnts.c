...
BOOL AddSubagentByKey(LPTSTR pKey)
/*++
Routine Description:
    Adds subagent to list.
Arguments:
...
--*/
{
    ...
    lStatus = RegOpenKeyEx(HKEY_LOCAL_MACHINE, pKey, 0, KEY_READ, &hKey);// open registry subkey
    if (lStatus == ERROR_SUCCESS) {// validate return code
        ...
        while (lStatus == ERROR_SUCCESS) {
            lStatus = RegEnumValueA(hKey, dwIndex, szName, &dwNameSize, NULL, &dwValueType, szValue, &dwValueSize);// read next value
            if (lStatus == ERROR_SUCCESS) {// validate return code
                if (!_stricmp(szName, REG_VALUE_SUBAGENT_PATH)) {// check to see if value is pathname
                    DWORD dwRequired;
                    CHAR szExpanded[MAX_PATH];

                    // expand environment strings in path
                    dwRequired = ExpandEnvironmentStringsA(szValue, szExpanded, sizeof(szExpanded));
...

{
    ...
    SNMPDBG((SNMP_LOG_TRACE, "SNMP: SVC: loading subagents.\n"));

    lStatus = RegOpenKeyEx(HKEY_LOCAL_MACHINE, pszKey, 0, KEY_READ, &hKey);// open registry subkey
    if (lStatus == ERROR_SUCCESS) {// validate return code
        dwIndex = 0;// initialize
        ...
        while (lStatus == ERROR_SUCCESS) {// loop until error or end of list
            // read next value
