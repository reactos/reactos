...
--*/
{
    ...
    pLE = g_Subagents.Flink;// point to first subagent

    // process each subagent
    while (pLE != &g_Subagents) {
        pNLE = CONTAINING_RECORD(pLE, SUBAGENT_LIST_ENTRY, Link);// retreive pointer to subagent list entry from link
        if (pNLE->hSubagentTrapEvent != NULL) {// check for subagent trap event
            nWaitObjects++;// increment
        }

        pLE = pLE->Flink;// next entry
    }

    ppNLEs = AgentMemAlloc(nWaitObjects * sizeof(PSUBAGENT_LIST_ENTRY));// attempt to allocate array of subagent pointers
    if (ppNLEs != NULL) {// validate pointers
        pWaitObjects = AgentMemAlloc(nWaitObjects * sizeof(HANDLE));// attempt to allocate array of event handles
        if (pWaitObjects != NULL) {// validate pointer
            fOk = TRUE;// success
        } else {
            SNMPDBG((SNMP_LOG_ERROR,"SNMP: SVC: could not allocate handle array.\n"));
            AgentMemFree(ppNLEs);// release array
            ppNLEs = NULL;// re-init
        }
    } else {
        SNMPDBG((SNMP_LOG_ERROR,"SNMP: SVC: could not allocate subagent pointers.\n"));
    }

    if (fOk) {
        DWORD dwIndex = 0;// initialize
        ...
        while ((pLE != &g_Subagents) && (dwIndex < nWaitObjects - 1)) {
            pNLE = CONTAINING_RECORD(pLE, SUBAGENT_LIST_ENTRY, Link);// retreive pointer to subagent list entry from link
            if (pNLE->hSubagentTrapEvent != NULL) {// check for subagent trap event
                pWaitObjects[dwIndex] = pNLE->hSubagentTrapEvent;// copy subagent trap event handle
                ppNLEs[dwIndex] = pNLE;// copy subagent pointer
                dwIndex++;// next
            }
        ...
        pWaitObjects[dwIndex++] = g_hRegistryEvent;// copy registry update event into second last entry
        pWaitObjects[dwIndex++] = g_hTerminationEvent;// copy termination event into last entry

        if (dwIndex != nWaitObjects) {// validate number of items
            SNMPDBG((SNMP_LOG_WARNING,"SNMP: SVC: updating number of events from %d to %d.\n",nWaitObjects,dwIndex));
            nWaitObjects = dwIndex;// use latest number
        }
    }
...
*/
{
    ...
    Pdu.nType = SNMP_PDU_V1TRAP;// note this is in older format

    // validate pointer
    if (pVbl != NULL) {
        Pdu.Vbl = *pVbl;// copy varbinds
    } else {
        // initialize
        Pdu.Vbl.len = 0;
        Pdu.Vbl.list = NULL;
    ...
    return GenerateTrap(&Pdu);// send trap to managers
}
    ...
    if (LoadWaitObjects(&nWaitObjects, &pWaitObjects, &ppNLEs)) {
        ...
        for (;;) {
            ...
            if (dwIndex == (WAIT_OBJECT_0 + nWaitObjects - 1)) {
                SNMPDBG((SNMP_LOG_TRACE,"SNMP: SVC: shutting down trap thread.\n"));

                break; // bail...

            // check for registry update event next
            } else if (dwIndex == (WAIT_OBJECT_0 + nWaitObjects - 2)) {
                SNMPDBG((SNMP_LOG_TRACE,"SNMP: SVC: recalling LoadWaitObjects.\n"));
            ...
            // check for subagent trap notification event
                ...
                if (pfnSnmpExtensionTrap != NULL) {
                    __try {
                        ...
                        while ((*pfnSnmpExtensionTrap)(&EnterpriseOid,&nGenericTrap,&nSpecificTrap,&nTimeStamp,&Vbl)) {
                            // send extension trap
                            GenerateExtensionTrap(&EnterpriseOid,nGenericTrap,nSpecificTrap,nTimeStamp,&Vbl);
                            SnmpUtilVarBindListFree(&Vbl);
                        }
                    } __except (EXCEPTION_EXECUTE_HANDLER) {
                        SNMPDBG((SNMP_LOG_ERROR,"SNMP: SVC: exception 0x%08lx polling %s.\n",GetExceptionCode(),ppNLEs[dwIndex]->pPathname));
                    }
                }
            }
        }

        UnloadWaitObjects(pWaitObjects, ppNLEs);// release memory for wait objects
    }
...
*/
{
    ...
    pLE1 = g_TrapDestinations.Flink;// obtain first trap destination
    ...
    while (pLE1 != &g_TrapDestinations) {
        ...
        pLE2 = pTLE->Managers.Flink;// obtain first manager

        // process each receiving manager
        while (pLE2 != &pTLE->Managers) {
            pMLE = CONTAINING_RECORD(pLE2, MANAGER_LIST_ENTRY, Link);// retrieve pointer to next manager

            UpdateMLE(pMLE);// refresh addr

            // don't send traps to addresses that are DEAD or NULL
            if (pMLE->dwAge == MGRADDR_DEAD || !IsValidSockAddr(&pMLE->SockAddr))
            {
                pLE2 = pLE2->Flink;
                continue;
            }

            pLE3 = g_OutgoingTransports.Flink;// obtain first outgoing transport

            // process each outgoing transport
            while (pLE3 != &g_OutgoingTransports) {
                pNLE = CONTAINING_RECORD(pLE3, NETWORK_LIST_ENTRY, Link);// retrieve pointer to outgoing transport structure
                pNLE->Buffer.len = NLEBUFLEN;// initialize buffer length
                ...
                if (pNLE->SockAddr.sa_family == AF_INET)
                {
                    ...
                    pSockAddrIn = (struct sockaddr_in * )&pNLE->SockAddr;// obtain pointer to protocol specific structure
                    
                    dwIPAddr = pSockAddrIn->sin_addr.s_addr;// copy agent address into temp buffer
                ...
                // build message
                if (BuildMessage(SNMP_VERSION_1,&CommunityOctets,pPdu,pNLE->Buffer.buf,&pNLE->Buffer.len)) {
                    // synchronous send
                    dwStatus = WSASendTo(pNLE->Socket,&pNLE->Buffer,1,&pNLE->dwBytesTransferred,pNLE->dwFlags,&pMLE->SockAddr,pMLE->SockAddrLen,NULL,NULL);

                    mgmtCTick(CsnmpOutPkts);// register outgoing packet into the management structure
                    mgmtCTick(CsnmpOutTraps);// retister outgoing trap into the management structure

                    // validate return code
                    if (dwStatus == SOCKET_ERROR) {
                        SNMPDBG((SNMP_LOG_ERROR,"SNMP: SVC: error code %d on sending trap to %s.\n",WSAGetLastError(),pTLE->pCommunity));
                    }
                }

                pLE3 = pLE3->Flink;// next entry
            }

            pLE2 = pLE2->Flink;// next entry
        }
