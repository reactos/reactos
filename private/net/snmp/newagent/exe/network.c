...
--*/
{
    ...
    if (pNLE != NULL) {
        ...
        if (pNLE->Buffer.buf != NULL) {
            pNLE->Socket = INVALID_SOCKET;// initialize socket to socket
            pNLE->Buffer.len = NLEBUFLEN;// initialize buffer length
            InitializeListHead(&pNLE->Queries);// initialize subagent query list
            InitializeListHead(&pNLE->Bindings);// initialize variable bindings list
            fOk = TRUE;// success
        } else {
            SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: could not allocate network io buffer.\n"));
            FreeNLE(pNLE);// release
            pNLE = NULL;// re-init
        }
    } else {
        SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: could not allocate network entry.\n"));
    }
...
--*/
{
    ...
    if (pNLE != NULL) {
        ...
        if (pNLE->Socket != INVALID_SOCKET) {
            closesocket(pNLE->Socket);// release socket
        }

        UnloadPdu(pNLE);// release pdu
        UnloadQueries(pNLE);// release query list
        UnloadVarBinds(pNLE);// release bindings list
        AgentMemFree(pNLE->Buffer.buf);// release network buffer
        AgentMemFree(pNLE);// release memory
    }
...
--*/
{
    ...
    if (AllocNLE(&pNLE)) {
        ...
        pSockAddr->sin_port = (pServEnt != NULL) ? (SHORT)pServEnt->s_port : htons(DEFAULT_SNMP_PORT_UDP);
        ...
        if (pNLE->Socket != INVALID_SOCKET) {
            ...
            if (nStatus != SOCKET_ERROR) {
                ...
                InsertTailList(&g_IncomingTransports, &pNLE->Link);// insert transport into list of incoming
                fUdpOk = TRUE;// success
            } else {
                SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: error %d binding to udp port %d.\n", WSAGetLastError(), ntohs(pSockAddr->sin_port)));
            }
        } else {
            SNMPDBG((SNMP_LOG_WARNING, "SNMP: SVC: error %d creating udp socket.\n", WSAGetLastError()));
        }
    ...
    if (AllocNLE(&pNLE)) {
        ...
        pNLE->SockAddrLen = sizeof(struct sockaddr_ipx);// initialize sockaddr structure size
        pSockAddr = (struct sockaddr_ipx *)&pNLE->SockAddr;// obtain pointer to sockaddr structure
        ...
        if (pNLE->Socket != INVALID_SOCKET) {
            ...
            if (nStatus != SOCKET_ERROR) {
                ...
                SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: error %d binding to ipx port %d.\n", WSAGetLastError(), ntohs(pSockAddr->sa_socket)));
            }
        } else {
            SNMPDBG((SNMP_LOG_WARNING, "SNMP: SVC: error %d creating ipx socket.\n", WSAGetLastError()));
        }
    ...
    return (fUdpOk || fIpxOk);// need one transport min
}
...
BOOL UnloadTransport(PNETWORK_LIST_ENTRY pNLE)
{
    ...
    RemoveEntryList(&(pNLE->Link));// remove the entry from the list
    closesocket(pNLE->Socket);// close the socket
    FreeNLE(pNLE);// release the memory
    return TRUE;
}
...
--*/
{
    ...
    while (!IsListEmpty(&g_IncomingTransports)) {
        pLE = RemoveHeadList(&g_IncomingTransports);// extract next entry from head
        pNLE = CONTAINING_RECORD(pLE, NETWORK_LIST_ENTRY, Link);// retrieve pointer to mib region structure
        FreeNLE(pNLE); // release
    }
...
--*/
{
    ...
    if (AllocNLE(&pNLE)) {
        ...
        if (pNLE->Socket != INVALID_SOCKET) {
            ...
            InsertTailList(&g_OutgoingTransports, &pNLE->Link);// insert transport into list of incoming
            fUdpOk = TRUE;// success
        } else {
            SNMPDBG((SNMP_LOG_WARNING, "SNMP: SVC: error %d creating udp socket.\n", WSAGetLastError()));
            FreeNLE(pNLE);// release
...
--*/
{
    ...
    while (!IsListEmpty(&g_OutgoingTransports)) {
        pLE = RemoveHeadList(&g_OutgoingTransports);// extract next entry from head
        pNLE = CONTAINING_RECORD(pLE, NETWORK_LIST_ENTRY, Link);// retrieve pointer to mib region structure
        FreeNLE(pNLE);// release
    }
...
--*/
{
    SnmpUtilOctetsFree(&pNLE->Community);// release community string
    SnmpUtilVarBindListFree(&pNLE->Pdu.Vbl);// release varbinds in pdu
    return TRUE;
}