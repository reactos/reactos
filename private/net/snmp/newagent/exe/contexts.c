...
--*/
{
    ...
    lStatus = RegQueryValueExW(hKey, wszCommunity, 0, &dwDataType, NULL, &dwDataSize);
    ...
    if (lStatus != ERROR_SUCCESS || dwDataType != REG_DWORD)
    {
        ...
        // all communities that are converted to new format at this point, are converted to READ-CREATE permissions to ensure same functionality as the permisionless communities.
    ...
    lStatus = RegDeleteValueW(hKey, wszBogus);// delete the old format value
    return lStatus;
}
#endif
...
--*/
{
    ...
    if (lStatus == ERROR_SUCCESS) {
        ...
        for (dwIndex = 0; lStatus == ERROR_SUCCESS; dwIndex++)
        {
            ...
            lStatus = RegEnumValueW(hKey, dwIndex, wszName, &dwNameSize, NULL, &dwDataType, (LPBYTE)wszData, &dwDataSize);// read next value
                ...
                if (dwDataType != REG_DWORD)
                {
                    ...
                    if (dwDataType == REG_SZ)
                    {
                        if (bFirstCall)
                        {
                            ...
                            {
                                SNMPDBG((SNMP_LOG_WARNING, "SNMP: SVC: updated community registration\n"));
                        ...
                        {
                            SNMPDBG((SNMP_LOG_WARNING, "SNMP: SVC: old format community to be considered with full rights"));
