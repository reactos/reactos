...
BOOL AddTrapDestination(HKEY   hKey, LPWSTR pwCommunity)
/*++
Routine Description:
...
--*/
{
    ...
    lStatus = RegOpenKeyExW(hKey, pwCommunity, 0, KEY_READ, &hSubKey);

    // validate return code
    if (lStatus == ERROR_SUCCESS &&
        SnmpUtilUnicodeToUTF8(&pCommunity, pwCommunity, TRUE) == 0) {
        // attempt to locate in list
        if (FindTrapDestination(&pTLE, pCommunity)) {
            SNMPDBG((SNMP_LOG_TRACE, "SNMP: SVC: updating trap destinations for %s.\n", pCommunity));
            LoadManagers(hSubKey, &pTLE->Managers);// load associated managers
            fOk = TRUE;// success
        } else {
            // allocate trap destination structure
            if (AllocTLE(&pTLE, pCommunity)) {
                SNMPDBG((SNMP_LOG_TRACE, "SNMP: SVC: adding trap destinations for %s.\n", pCommunity));

                // load associated managers
                if (LoadManagers(hSubKey, &pTLE->Managers)) {
                    InsertTailList(&g_TrapDestinations, &pTLE->Link);// insert into valid communities list
                    fOk = TRUE;// success
                }

                if (!fOk) {// cleanup
                    FreeTLE(pTLE);// release
                }
            }
        }

        RegCloseKey(hSubKey);// release subkey
...

{
    ...
    // validate return code
    if (lStatus == ERROR_SUCCESS) {
        dwIndex = 0;// initialize

        // loop until error or end of list
        while (lStatus == ERROR_SUCCESS) {
            lStatus = RegEnumKeyW(hKey, dwIndex, wszName, sizeof(wszName));// read next value
            if (lStatus == ERROR_SUCCESS) {// validate return code
    ...
    if (!fOk) {
        SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: error %d processing TrapDestinations subkey.\n", lStatus));
