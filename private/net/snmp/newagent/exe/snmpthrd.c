...
--*/
{
    ...
    if (pSockAddr->sa_family == AF_INET) {// determine family
        struct sockaddr_in * pSockAddrIn;
        
        pSockAddrIn = (struct sockaddr_in * )pSockAddr;// obtain pointer to protocol specific structure        
        return inet_ntoa(pSockAddrIn->sin_addr);// forward to winsock conversion function
    } else if (pSockAddr->sa_family == AF_IPX) {
        struct sockaddr_ipx * pSockAddrIpx;
        
        pSockAddrIpx = (struct sockaddr_ipx * )pSockAddr;// obtain pointer to protocol specific structure
...
--*/
{
    ...
    if (fAccessOk && FindValidCommunity(&pCLE, &unicodeCommunity))
    {
        ...
        if (pNLE->Pdu.nType == SNMP_PDU_SET) {            
            fAccessOk = (pCLE->dwAccess >= SNMP_ACCESS_READ_WRITE);// check flags for write privileges
        } else {            
            fAccessOk = (pCLE->dwAccess >= SNMP_ACCESS_READ_ONLY);// check flags for read privileges
        }
    ...
    if (!fAccessOk && snmpMgmtBase.AsnIntegerPool[IsnmpEnableAuthenTraps].asnValue.number) {        
        GenerateAuthenticationTrap();// send authentication trap
    }
    ...
    if (ParseMessage(
        ...
        if (ValidateContext(pNLE)) {
            ...
            if ((*RefErrStatus(&pNLE->Pdu) != SNMP_ERRORSTATUS_NOERROR) ||
                ...
                pNLE->Pdu.nType = SNMP_PDU_RESPONSE;// reset pdu type to response
...
--*/
{
    ...
    SNMPDBG((SNMP_LOG_TRACE, "SNMP: SVC:  transaction %d begin \n", pNLE->nTransactionId));

    // validate status
    if (dwStatus == NOERROR) {        
        mgmtCTick(CsnmpInPkts);// register incoming packet into the management structure
        ...
        if (ValidateManager(pNLE)) {// check manager address            
            if (ProcessSnmpMessage(pNLE)) {// process snmp message                
                dwStatus = WSASendTo(pNLE->Socket,// synchronous send
                              &pNLE->Buffer,
                              1,
                              &pNLE->dwBytesTransferred,
                              pNLE->dwFlags,
                ...
                mgmtCTick(CsnmpOutGetResponses);// register outgoing Response PDU
                ...
                if (dwStatus != SOCKET_ERROR) {
                    ...
                    SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: error %d sending response.\n", WSAGetLastError()));
                }
            }
        }
    } else {
        SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: error %d receiving snmp request.\n", dwStatus));
    }
...
--*/
{
    ...
    SNMPDBG((SNMP_LOG_TRACE, "SNMP: SVC: Loading Registry Parameters.\n"));
    
    GenerateColdStartTrap();// fire cold start trap
    ...
    for (;;)
    {        
        pLE = g_IncomingTransports.Flink;// obtain pointer to first transport
        ...
        while (pLE != &g_IncomingTransports)
        {
            ...
            {                
                pNLE->dwStatus = WSA_IO_PENDING;// reset completion status                
                pNLE->SockAddrLenUsed = pNLE->SockAddrLen;// intialize address structure size                
                pNLE->Buffer.len = NLEBUFLEN;// initialize buffer length                
                pNLE->dwFlags = 0;// re-initialize
                ...
                if (dwStatus == SOCKET_ERROR)// handle network failures
                {                    
                    dwStatus = WSAGetLastError();// retrieve last error
                    ...
                    if (dwStatus != WSA_IO_PENDING)
                    {
                        ...
                        else
                        {
                            ...
                            pLE = pLE->Flink;// first step next with the pointer                            
                            UnloadTransport(pNLE);// delete this transport from the incoming transports list                            
                            continue;// go on further
                        }
                    }
                }
            }
        ...
        if (dwStatus == WAIT_OBJECT_0) {// validate return code
            SNMPDBG((SNMP_LOG_TRACE, "SNMP: SVC: exiting pdu processing thread.\n"));            
            return NOERROR;// success
        } else if (dwStatus != WAIT_IO_COMPLETION) {            
            dwStatus = GetLastError();// retrieve error
            SNMPDBG((SNMP_LOG_ERROR, "SNMP: SVC: error %d waiting for request.\n", dwStatus));            
            return dwStatus;// failure
        }
    }
}
