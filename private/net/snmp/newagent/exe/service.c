...
--*/
{
    ...
    if (!g_CmdLineArguments.fBypassCtrlDispatcher) {
        g_SnmpSvcHandle = RegisterServiceCtrlHandler(SNMP_SERVICE, ProcessControllerRequests);// register snmp with service controller
        if (g_SnmpSvcHandle == 0) { // validate handle
            g_SnmpSvcStatus.dwWin32ExitCode = GetLastError();// save error code in service status structure
            SNMPDBG((SNMP_LOG_ERROR,  "SNMP: SVC: error 0x%08lx registering service.\n", g_SnmpSvcStatus.dwWin32ExitCode));
            return; // bail...
        }
    }

    UpdateController(SERVICE_START_PENDING, SNMP_WAIT_HINT);// report status to service controller

    if (StartupAgent()) {// startup agent
        UpdateController(SERVICE_RUNNING, NO_WAIT_HINT);// report status to service controller
