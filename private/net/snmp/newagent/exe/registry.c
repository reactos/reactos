...
--*/
{
    ...
    {
        RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_KEY_SNMP_PARAMETERS, 0, KEY_READ, &g_hDefaultKey);
    }
    ...
    if (g_hDefaultRegNotifier &&
        ...
        RegNotifyChangeKeyValue(g_hDefaultKey, TRUE, REG_NOTIFY_CHANGE_NAME | REG_NOTIFY_CHANGE_LAST_SET, g_hDefaultRegNotifier, TRUE) == ERROR_SUCCESS)
    {
        SNMPDBG((SNMP_LOG_TRACE, "SNMP: REG: Default reg notifier initialized successfully.\n"));
        nEvents++;
    }
    ...
    {
        RegOpenKeyEx(HKEY_LOCAL_MACHINE, REG_POLICY_ROOT, 0, KEY_READ, &g_hPolicyKey);
    }
    ...
    if (g_hPolicyRegNotifier &&
        ...
        RegNotifyChangeKeyValue(g_hPolicyKey, TRUE, REG_NOTIFY_CHANGE_NAME | REG_NOTIFY_CHANGE_LAST_SET, g_hPolicyRegNotifier, TRUE) == ERROR_SUCCESS)
    {
        SNMPDBG((SNMP_LOG_TRACE, "SNMP: REG: Policy reg notifier initialized successfully.\n"));
        nEvents++;
    }
...
--*/
{
    UnloadRegistryNotifiers();// unload the registry notifiers as the first thing to do
    UnloadPermittedManagers();// unload managers
    UnloadTrapDestinations();// unload trap destinations
    UnloadValidCommunities();// unload communities
    UnloadSubagents();// unload subagents
    UnloadSupportedRegions();// unload mib regions
    return TRUE;
}
