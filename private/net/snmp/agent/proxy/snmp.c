...
SERVICE_STATUS status = {SERVICE_WIN32, SERVICE_STOPPED, SERVICE_ACCEPT_STOP, NO_ERROR, 0, 0, 0};
...
static VOID serviceHandlerFunction(IN DWORD dwControl)
{
    SNMPDBG((SNMP_LOG_TRACE, "SNMP: SVC: serviceHandlerFunction entered, dwControl=%d.\n", dwControl));

    // is it a LogLevel change control?
    if (SNMP_SERVICE_LOGLEVEL_BASE + SNMP_SERVICE_LOGLEVEL_MIN <= dwControl && dwControl <= SNMP_SERVICE_LOGLEVEL_BASE + SNMP_SERVICE_LOGLEVEL_MAX)
    {
        INT nLogLevel;
    ...
    else if (SNMP_SERVICE_LOGTYPE_BASE + SNMP_SERVICE_LOGTYPE_MIN <= dwControl && dwControl <= SNMP_SERVICE_LOGTYPE_BASE + SNMP_SERVICE_LOGTYPE_MAX)
    {
        INT nLogType;
...
static VOID serviceMainFunction(IN DWORD dwNumServicesArgs, IN LPSTR *lpServiceArgVectors)
{
    ...
    if (!noservice) {
        ...
        if ((hService = RegisterServiceCtrlHandler("SNMP", serviceHandlerFunction)) == 0)
        {
...
static BOOL breakHandler(IN ULONG ulType)
{
    ...
    int nCmdShow)
{
    ...
    if ( ( lpCmdLine[0]=='-') || ( lpCmdLine[0]=='/') )
    {
        ...
        if ( _strnicmp ( &lpCmdLine[1], szCloseStr, strlen(szCloseStr) ) ==0 )
            ...
            fKillParent = TRUE;
        else if (( _strnicmp ( &lpCmdLine[1], szHelpStr, strlen(szHelpStr) )==0 )|| ( _strnicmp ( &lpCmdLine[1], szQuestStr, strlen(szQuestStr) )==0 ))
        {   // they have asked for help...
            MessageBox (GetTopWindow(GetDesktopWindow()), szHelpText1, szHelpText2, MB_OK );
            return 0;
        }
    }

    if ((hPrevInstance) || (SnmpAgentEvent == NULL))
    {
        ...
        if (!fKillParent) // if we are not killing the parent, only one agent at a time...
        {
            ...
            MessageBox (hParentWin, szHelpText3, szHelpText2, MB_OK );
            return 1;
        }
    ...
    if ((hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) serviceMainFunction, NULL, 0, &threadId)) == 0)
    {
