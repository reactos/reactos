...
BOOL CreateAgentWnd()
{
    ...
    if (hwnd != NULL) {
        g_hwndAgent = hwnd;// transfer        
        fOk = TRUE;// success
    }
    else {
        SNMPDBG((SNMP_LOG_ERROR, "SNMP: INIT: CreateWindow returned %d.\n", GetLastError()));
        g_hwndAgent = NULL;// transfer        
        fOk = FALSE;// failure
    }
...
BOOL DestroyAgentWnd()
{
    BOOL fOk;

    // validate handle
    if (g_hwndAgent != NULL) {
        fOk = DestroyWindow(g_hwndAgent);// destroy notification window
        if (!fOk) {
            SNMPDBG((SNMP_LOG_ERROR, "SNMP: INIT: DestroyWindow returned %d.\n", GetLastError()));
        }
    }
...
VOID agentCommThread(VOID *threadParam)
{
    ...
    RegisterAgentWndClass();// register window class    
    CreateAgentWnd();// create window
...
cleanup:    
    DestroyAgentWnd();// destroy window    
    UnregisterAgentWndClass();// register window class
} // end agentCommThread()
