...
void GetCurrentSelections(void)
{
    ...
    if ((lSel = SendMessage(ghwndList1, LB_GETCURSEL, 0, 0)) != LB_ERR) {
        ...
        gpSelectedSocket = (PMYSOCKET)SendMessage(ghwndList1, LB_GETITEMDATA, (WPARAM)lSel, 0);
    } else {
    ...
    if (lSelCount == 0 || lSelCount == LB_ERR) {
        ...
        SendMessage(ghwndList3, LB_GETSELITEMS, 1, (LPARAM)&giSelectedEventIndex);
        ghSelectedEvent = (WSAEVENT)SendMessage(ghwndList3, LB_GETITEMDATA, giSelectedEventIndex, 0);
    }
}


BOOL LetUserMungeParams(PFUNC_PARAM_HEADER pParamsHeader)
{
    if (!bShowParams) {
        return TRUE;
    }
    ...
    return (DialogBoxParam(ghInst,
        (LPCSTR)MAKEINTRESOURCE(IDD_DIALOG2),
                           ...
                           (LPARAM)pParamsHeader) ? TRUE : FALSE);
}
...
#define TIMER_TIMEOUT 2000
    ...
    case WM_INITDIALOG:
    {
        ...
        for (i = 0; aFuncNames[i]; i++) {
            SendMessage(ghwndList2, LB_INSERTSTRING, (WPARAM)-1, (LPARAM)aFuncNames[i]);
        }
                ...
                // Read in defaults from ini file
        {
            ...
            for (i = 0; aDefVals[i].lpszEntry; i++) {
                GetProfileString(gszSockEye, aDefVals[i].lpszEntry, aDefVals[i].lpszDefValue, buf, 15);
            ...
            for (j = i; aDefVals[i].lpszEntry; i++) {
                ...
                SetDlgItemText(hwnd, IDC_BUTTON7 + (i - j), (LPCSTR)aDefVals[i].lp);
            }

            if (dwInitialized == 0) {
                // If here assume this is first time user had started app, so post a msg that will automatically bring up the hlp dlg
...

        {
            ...
            hFont = CreateFont(13, 5, 0, 0, 400, 0, 0, 0, 0, 1, 2, 1, 34, "MS Sans Serif");
            ghFixedFont = hFont2 = CreateFont(13, 8, 0, 0, 400, 0, 0, 0, 0, 1, 2, 1, 49, "Courier");

            for (i = 0; i < 12; i++) {
                SendDlgItemMessage(hwnd, IDC_BUTTON1 + i, WM_SETFONT, (WPARAM)hFont, 0);
            }
        ...
        GetProfileString(gszSockEye, "ControlRatios", "20, 20, 100", buf, 63);
        sscanf(buf, "%ld,%ld,%ld", &cxList2, &cxList1, &cxWnd);
        GetProfileString(gszSockEye, "Position", "max", buf, 63);
    ...
    case WM_COMMAND:
    {
...
#else
                    SendMessage(ghwndEdit, EM_SETSEL, (WPARAM)1, (LPARAM)MAKELONG(0xfffd, 0xfffe));
#endif
                }
            }
            break;
        ...
        case IDM_PARAMS:
            ...
            if (bShowParams) {
                CheckMenuItem(hMenu, IDM_PARAMS, MF_BYCOMMAND | MF_CHECKED);
                CheckDlgButton(hwnd, IDM_PARAMS, 1);
            } else {
                CheckMenuItem(hMenu, IDM_PARAMS, MF_BYCOMMAND | MF_UNCHECKED);
        ...
        case IDC_BUTTON12:
        {
            ...
            if (aUserButtonFuncs[i] >= MiscBegin) {
                // Hot button func id is bogus, so bring up hot button init dlg
        ...
        case IDC_ENTER:
        {
            if (GetFocus() != ghwndEdit) {
                GetCurrentSelections();
                FuncDriver((FUNC_INDEX)SendMessage(ghwndList2, LB_GETCURSEL, 0, 0));
            } else {
        ...
        case IDM_DEFAULTVALUES:
        {
            ...
            FUNC_PARAM_HEADER paramsHeader = {13, DefValues, params, NULL};
            BOOL bShowParamsSave = bShowParams;
        ...
        case IDM_LOGFILE:
        {
            if (hLogFile) {
                ...
                if ((hLogFile = fopen(szFile, "at")) == (FILE *)NULL) {
                    ...
                    fprintf(hLogFile, "\nLog opened: %s\n", asctime(newtime));
                    CheckMenuItem(hMenu, IDM_LOGFILE, MF_BYCOMMAND | MF_CHECKED);
                }
            }
            break;
        }
        ...
        case IDM_ABOUT:
        {
            DialogBoxParam(ghInst, (LPCSTR)MAKEINTRESOURCE(IDD_DIALOG4), (HWND)hwnd, (DLGPROC)AboutDlgProc, 0);
            break;
        }
        } // switch
    ...
    case WM_ASYNCREQUESTCOMPLETED:
    {
        ...
        if ((pAsyncReqInfo = DequeueAsyncRequestInfo((HANDLE)wParam))) {
            if (WSAGETASYNCERROR(lParam) == 0) {
                ShowStr("Request x%x (%s) completed successfully",
                ...
                pszError = (aWSAErrors[i].lpszVal ? aWSAErrors[i].lpszVal : gszUnknownError);

                ShowStr("Request x%x (%s) completed unsuccessfully: %s (%d)",
                        pAsyncReqInfo->hRequest,
                        aFuncNames[pAsyncReqInfo->FuncIndex],
                        pszError,
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if ((y > icyButton && (x < (cxList2 + icyBorder) || x >(cxList2 + icyBorder + cxList1))) || lCaptureFlags) {
        ...
        if (lCaptureFlags == 1) {
            ...
            x = (x > (cxList1 + cxList2 - cxVScroll) ? (cxList1 + cxList2 - cxVScroll) : x);
            x = (x < cxVScroll ? cxVScroll : x);

            cxList2New = (int)(cxList2 + x - xCapture);

            SetWindowPos(ghwndList2,
    ...
    case WM_LBUTTONDOWN:
    {
        if ((int)((short)HIWORD(lParam)) > icyButton) {
            ...
            if (xCapture > (cxList2 + icyBorder) && xCapture < (cxList2 + icyBorder + cxList1)) {
                break;
            }
    ...
    case WM_CLOSE:
    {
        ...
        // Save defaults in ini file
        {
            ...
            for (; aDefVals[i].lpszEntry; i++) {
                WriteProfileString(gszSockEye, aDefVals[i].lpszEntry, (LPCSTR)aDefVals[i].dwValue);
            }
            ...
            if (!IsIconic(hwnd)) {
                ...
                WriteProfileString(gszSockEye, "Position", (LPCSTR)buf);
                sprintf(buf, "%ld,%ld,%ld", cxList2, cxList1, cxWnd);
                WriteProfileString(gszSockEye, "ControlRatios", (LPCSTR)buf);
            }
        }
...
void FAR ShowStr(LPCSTR format, ...)
{
    ...
    SendMessage(ghwndEdit, EM_SETSEL, (WPARAM)0xfffffffd, (LPARAM)0xfffffffe);
#else
    SendMessage(ghwndEdit, EM_SETSEL, (WPARAM)0, (LPARAM)MAKELONG(0xfffd, 0xfffe));
#endif

    SendMessage(ghwndEdit, EM_REPLACESEL, 0, (LPARAM)buf);

#ifdef WIN32
    SendMessage(ghwndEdit, EM_SCROLLCARET, 0, 0);// Scroll to end of text
#endif
...
INT_PTR CALLBACK ParamsDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // Stick all the param names in the listbox, & for each PT_DWORD param save it's default value
        for (i = 0; i < pDlgInstData->pParamsHeader->dwNumParams; i++) {
            ...
            if (pDlgInstData->pParamsHeader->aParams[i].dwType == PT_DWORD) {
                pDlgInstData->pParamsHeader->aParams[i].u.dwDefValue = (ULONG_PTR)pDlgInstData->pParamsHeader->aParams[i].dwValue;
            }
        }
...
// help        }
        ...
        // child edit ctrl & send it the LIMITTEXT msg.)
        {
            ...
            SendMessage(hwndChild,
                        EM_LIMITTEXT,
                        (WPARAM)(gbWideStringParams ? (MAX_STRING_PARAM_SIZE / 2 - 1) : MAX_STRING_PARAM_SIZE - 1),
                        0);
        }
    ...
    case WM_COMMAND:
    {
        ...
        case IDOK:
            ...
            if (lLastSel != -1) {
                ...
                case PT_STRING:
                {
                    ...
                    {
                        pParamsHeader->aParams[lLastSel].dwValue = (ULONG_PTR)-1;
                ...
                case PT_PTRNOEDIT:
                {
                    if (!sscanf(buf, "%08lx", &pParamsHeader->aParams[lLastSel].dwValue)) {
                        // Default to 0
                        pParamsHeader->aParams[lLastSel].dwValue = 0;
                    }
...
#endif
            {
                ...
                if (lLastSel != -1) {
                    ...
                    case PT_STRING:
                    {
                        ...
                        {
                            pParamsHeader->aParams[lLastSel].dwValue = (DWORD)0xffffffff;
                ...
                case PT_STRING:
                {
                    ...
                    for (i = 0; i < 3; i++) {
                        SendDlgItemMessage(hwnd, IDC_COMBO1, CB_INSERTSTRING, (WPARAM)-1, (LPARAM)aszOptions[i]);
                    }
                ...
                case PT_FLAGS:
                {
                    ...
                    for (i = 0; pLookup[i].dwVal != 0xffffffff; i++) {
                        SendDlgItemMessage(hwnd, IDC_LIST2, LB_INSERTSTRING, (WPARAM)-1, (LPARAM)pLookup[i].lpszVal);
                        if (pParamsHeader->aParams[lSel].dwValue & pLookup[i].dwVal) {
                            SendDlgItemMessage(hwnd, IDC_LIST2, LB_SETSEL, (WPARAM)TRUE, (LPARAM)MAKELPARAM((WORD)i, 0));
                        }
                    }
                ...
                case PT_ORDINAL:
                {
                    ...
                    for (i = 0; pLookup[i].dwVal != 0xffffffff; i++) {
                        SendMessage(hwndList2, LB_INSERTSTRING, (WPARAM)-1, (LPARAM)pLookup[i].lpszVal);
                        if (pParamsHeader->aParams[lSel].dwValue == pLookup[i].dwVal) {
                            SendMessage(hwndList2, LB_SETSEL, (WPARAM)TRUE, (LPARAM)MAKELPARAM((WORD)i, 0));
                        }
                    }
                        ...
                        if (lSelCount == 1) {
                            ...
                            if (sscanf(buf, "%lx", &dwValue)) {
                                if (pLookup[ai[0]].dwVal == dwValue) {
                                    SendDlgItemMessage(hwnd, IDC_LIST2, LB_SETSEL, 0, (LPARAM)ai[0]);
                                    dwValue = pLookup[ai[1]].dwVal;
                                } else {
                                    SendDlgItemMessage(hwnd, IDC_LIST2, LB_SETSEL, 0, (LPARAM)ai[1]);
                                    dwValue = pLookup[ai[0]].dwVal;
                                }
                            } else {
        ...
        case IDC_COMBO1:
            ...
            case CBN_SELCHANGE:
            {
                LRESULT lSel = SendDlgItemMessage(hwnd, IDC_COMBO1, CB_GETCURSEL, 0, 0);
                ...
                case PT_STRING:
                    if (lSel == 1) {
                        strncpy(lpszComboText, pParamsHeader->aParams[lLastSel].u.buf, MAX_STRING_PARAM_SIZE);
                ...
                {
                    if (lSel == 1) {
                        LPWSAPROTOCOL_INFOA  pInfo = (gbWideStringParams ? (LPWSAPROTOCOL_INFOA)&gWSAProtocolInfoW : &gWSAProtocolInfoA);
                        ...
                        FUNC_PARAM_HEADER paramsHeader = {31, WSAProtoInfo, params, NULL};

                        // Set "ghwndModal" so this Params dlg will be modal to the existing Params dlg
                        ghwndModal = hwnd;
                        ...
                        if (LetUserMungeParams(&paramsHeader)) {
                            // Save the new params
                            pInfo->dwServiceFlags1 = (DWORD)params[0].dwValue;
                            pInfo->dwServiceFlags2 = (DWORD)params[1].dwValue;
                            pInfo->dwServiceFlags3 = (DWORD)params[2].dwValue;
                            pInfo->dwServiceFlags4 = (DWORD)params[3].dwValue;
                            pInfo->dwProviderFlags = (DWORD)params[4].dwValue;
                            pInfo->ProviderId.Data1 = (DWORD)params[5].dwValue;
                            pInfo->ProviderId.Data2 = (unsigned short)LOWORD(params[6].dwValue);
                            pInfo->ProviderId.Data3 = (unsigned short)LOWORD(params[7].dwValue);
                            *((LPDWORD)pInfo->ProviderId.Data4) = (DWORD)params[8].dwValue;
                            *((LPDWORD)&pInfo->ProviderId.Data4[4]) = (DWORD)params[9].dwValue;
                            pInfo->dwCatalogEntryId = (DWORD)params[10].dwValue;
                            pInfo->ProtocolChain.ChainLen = (DWORD)params[11].dwValue;
                            pInfo->ProtocolChain.ChainEntries[0] = (DWORD)params[12].dwValue;
                            pInfo->ProtocolChain.ChainEntries[1] = (DWORD)params[13].dwValue;
                            pInfo->ProtocolChain.ChainEntries[2] = (DWORD)params[14].dwValue;
                            pInfo->ProtocolChain.ChainEntries[3] = (DWORD)params[15].dwValue;
                            pInfo->ProtocolChain.ChainEntries[4] = (DWORD)params[16].dwValue;
                            pInfo->ProtocolChain.ChainEntries[5] = (DWORD)params[17].dwValue;
                            pInfo->ProtocolChain.ChainEntries[6] = (DWORD)params[18].dwValue;
                            pInfo->iVersion = (int)params[19].dwValue;
                            pInfo->iAddressFamily = (int)params[20].dwValue;
                            pInfo->iMaxSockAddr = (int)params[21].dwValue;
                            pInfo->iMinSockAddr = (int)params[22].dwValue;
                            pInfo->iSocketType = (int)params[23].dwValue;
                            pInfo->iProtocol = (int)params[24].dwValue;
                            pInfo->iProtocolMaxOffset = (int)params[25].dwValue;
                            pInfo->iNetworkByteOrder = (int)params[26].dwValue;
                            pInfo->iSecurityScheme = (int)params[27].dwValue;
                            pInfo->dwMessageSize = (DWORD)params[28].dwValue;
                            pInfo->dwProviderReserved = (DWORD)params[29].dwValue;
                        ...
                        ghwndModal = ghwndMain;// Now reset ghwndModal to what it was initially
                ...
                {
                    if (lSel == 1) {
                        LPQOS   pQOS = (LPQOS)pParamsHeader->aParams[lLastSel].u.ptr;
                        FUNC_PARAM params[] =
                        {
                        ...
                        FUNC_PARAM_HEADER paramsHeader = {18, ws_QOS, params, NULL};
                        ...
                        if (LetUserMungeParams(&paramsHeader)) {
                            // Save the params
                            for (i = 0; i < 17; i++) {
                                *(((LPDWORD)pQOS) + i) = (DWORD)params[i].dwValue;
                            }
                        }
                        ...
                        pParamsHeader->aParams[lLastSel].dwValue = (ULONG_PTR)pQOS;
                    }
            ...
            case CBN_EDITCHANGE:
            {
                // If user entered text in the edit field then copy the text to our buffer
...
INT_PTR CALLBACK UserButtonsDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        if (lParam) {
            ...
            // The dlg was invoked because the user chose a menuitem, so allow chgs on all buttons
    ...
    case WM_COMMAND:
        ...
        case IDOK:
        {
            ...
            if (lFuncSel == MiscBegin) {
                ...
                GetDlgItemText(hwnd, IDC_EDIT1, (LPSTR)&aUserButtonsText[iButtonIndex], MAX_USER_BUTTON_TEXT_SIZE - 1);
                aUserButtonsText[iButtonIndex][MAX_USER_BUTTON_TEXT_SIZE - 1] = 0;
            }

            SetDlgItemText(ghwndMain, IDC_BUTTON7 + iButtonIndex, (LPSTR)&aUserButtonsText[iButtonIndex]);
...
int CALLBACK ConditionProc(LPWSABUF  lpCallerId,
...
void CALLBACK CompletionProc(DWORD dwError,
                             DWORD cbTransferred,
...
DWORD PASCAL ShowError(FUNC_INDEX  funcIndex, int iResult)
{
    ...
    for (i = 0; (dwError != aWSAErrors[i].dwVal && aWSAErrors[i].lpszVal); i++);
...
void PASCAL ShowUnknownError(FUNC_INDEX  funcIndex, int iResult)
{
    ShowStr("%s returned unrecognized error (%d)", aFuncNames[funcIndex], iResult);
}
...
void FAR PASCAL FuncDriver(FUNC_INDEX funcIndex)
{
    ...
    ZeroMemory(pBigBuf, dwBigBufSize);// Zero the global buf so the user doesn't see extraneous stuff
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((s = accept((SOCKET)params[0].dwValue,
                ...
                if (params[1].dwValue) {
                    ...
                    ShowStr("  ->sa_family=%d, %s",
                            sa.sa_family,
                            GetStringFromOrdinalValue((DWORD)sa.sa_family, aAddressFamilies));
                    ShowStr("  ->sa_data=");
            ...
            if ((i = closesocket((SOCKET)params[0].dwValue)) == 0) {
                ...
                if ((SOCKET)params[0].dwValue != gpSelectedSocket->Sock) {
                    ...
                    for (i = 0; i < iNumItems; i++) {
                        ps = (PMYSOCKET)SendMessage(ghwndList1, LB_GETITEMDATA, i, 0);
                    ...
                    if (i == iNumItems) {
                        ShowStr("Strange, couldn't find socket=x%x in list", params[0].dwValue);
                        break;
                    }
                }

                SendMessage(ghwndList1, LB_DELETESTRING, glCurrentSelection, 0);
    ...
    case ws_getsockopt:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = getsockopt((SOCKET)params[0].dwValue,
                ...
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  Level/OptionName=%d/%d, %s/%s",
                        params[1].dwValue,
                        params[2].dwValue,
    ...
    case ws_htons:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            ShowStr("  networkShort=x%x", (DWORD)htons((u_short)LOWORD(params[0].dwValue)));
        }
    ...
    case ws_listen:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = listen((SOCKET)params[0].dwValue, (int)params[1].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
    ...
    case ws_ntohs:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            ShowStr("  hostShort=x%x", (DWORD)ntohs((u_short)LOWORD(params[0].dwValue)));
        }
    ...
    case ws_shutdown:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = shutdown((SOCKET)params[0].dwValue, (int)params[1].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
                ...
                if (params[1].dwValue  && params[2].dwValue  && *((LPINT)params[2].dwValue)) {
                    ...
                    ShowStr("  lpAddr->AddressFamily=%d, %s",
                        (DWORD)pSockAddr->sa_family,
                            GetStringFromOrdinalValue((DWORD)pSockAddr->sa_family, aAddressFamilies));

                    ShowStr("lpAddr->sa_data=");
                    ShowBytes(*((LPINT)params[2].dwValue) - sizeof(u_short), pSockAddr->sa_data, 1);
                }
            } else {
                ShowError(funcIndex, 0);
            }
    ...
    case ws_WSAAddressToStringW:
    {
        ...
        LPWSAPROTOCOL_INFOA pProtoInfo = (funcIndex == ws_WSAAddressToStringA ? &gWSAProtocolInfoA : (LPWSAPROTOCOL_INFOA)&gWSAProtocolInfoW);
        FUNC_PARAM params[] =
        {
        ...
        FUNC_PARAM_HEADER paramsHeader = {7, funcIndex, params, NULL};
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if (i == 0) {
                ...
                ShowBytes((funcIndex == ws_WSAAddressToStringA ? dwLength : dwLength * sizeof(WCHAR)),
    ...
    case ws_WSAAsyncGetHostByAddr:
    {
        ...
        if ((pAsyncReqInfo = MyAlloc(sizeof(ASYNC_REQUEST_INFO) + dwBigBufSize))) {
    ...
    case ws_WSAAsyncSelect:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = WSAAsyncSelect((SOCKET)params[0].dwValue,
                (HWND)params[1].dwValue,
                                    (unsigned int)params[2].dwValue,
                                    (long)params[3].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
    ...
    case ws_WSACleanup:
    {
        FUNC_PARAM_HEADER paramsHeader = {0, funcIndex, NULL, NULL};
            ...
            if (WSACleanup() == 0) {
                ...
                if (giCurrNumStartups == 0) {
                    ...
                    for (i = 0; i < iNumItems; i++) {
                        ...
                        pSocket = (PMYSOCKET)SendMessage(ghwndList1, LB_GETITEMDATA, 0, 0);
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (WSACloseEvent((WSAEVENT)params[0].dwValue) == TRUE) {
                ...
                if ((WSAEVENT)params[0].dwValue != ghSelectedEvent) {
                    // The user overrode the selected widget, so make sure we don't delete the wrong string from the list box
                    int         iNumItems;
                    WSAEVENT    hEvent;

                    iNumItems = (int)SendMessage(ghwndList3, LB_GETCOUNT, 0, 0);
                    for (i = 0; i < iNumItems; i++) {
                        hEvent = (WSAEVENT)SendMessage(ghwndList3, LB_GETITEMDATA, i, 0);
                    ...
                    if (i == iNumItems) {
                        ShowStr("Strange, couldn't find that hEvent=x%x in list", params[0].dwValue);
                        break;
                    }
                }

                SendMessage(ghwndList3, LB_DELETESTRING, (WPARAM)giSelectedEventIndex, 0);
            } else {
                ShowError(funcIndex, 0);
            }
        }
    ...
    case ws_WSAConnect:
    {
        ...
        LPBYTE      pCallerBuf = pBuf + dwBigBufSize, pCalleeBuf = pBuf + 2 * dwBigBufSize;
        LPQOS       pSQOS = (LPQOS)(pBuf + 3 * dwBigBufSize), pGQOS = (LPQOS)(pBuf + 4 * dwBigBufSize + sizeof(QOS));
        ...
        FUNC_PARAM_HEADER paramsHeader = {13, funcIndex, params, NULL};
    ...
    case ws_WSACreateEvent:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((wsaEvent = WSACreateEvent()) != WSA_INVALID_EVENT) {
                ...
                index = SendMessage(ghwndList3, LB_ADDSTRING, 0, (LPARAM)buf);
                SendMessage(ghwndList3, LB_SETITEMDATA, (WPARAM)index, (LPARAM)wsaEvent);
            } else {
                ShowError(funcIndex, 0);
            }
        }
    ...
    {
        LPWSAPROTOCOL_INFOA pProtoInfo = (gbWideStringParams ? &gWSAProtocolInfoA : (LPWSAPROTOCOL_INFOA)&gWSAProtocolInfoW);
        FUNC_PARAM params[] =
        {
            { gszSocket,        PT_DWORD,       (ULONG_PTR)gpSelectedSocket->Sock, NULL },
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (gbWideStringParams) {
                i = WSADuplicateSocketW((SOCKET)params[0].dwValue,
                    (DWORD)params[1].dwValue,
                                        (LPWSAPROTOCOL_INFOW)params[2].dwValue);
            } else {
                i = WSADuplicateSocketA((SOCKET)params[0].dwValue,
                    (DWORD)params[1].dwValue,
            ...
            if (i == 0) {
                ...
                ShowProtoInfo((LPWSAPROTOCOL_INFOA)params[2].dwValue, 0xffffffff, !gbWideStringParams);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
                ShowUnknownError(funcIndex, i);
    ...
    case ws_WSAEnumNameSpaceProvidersW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (funcIndex == ws_WSAEnumNameSpaceProvidersA) {
                i = WSAEnumNameSpaceProvidersA((LPDWORD)params[0].dwValue, (LPWSANAMESPACE_INFOA)params[1].dwValue);
            } else {
                i = WSAEnumNameSpaceProvidersW((LPDWORD)params[0].dwValue, (LPWSANAMESPACE_INFOW)params[1].dwValue);
            }

            if (i != SOCKET_ERROR) {
                LPWSANAMESPACE_INFOA    pInfo = (LPWSANAMESPACE_INFOA)params[1].dwValue;

                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  *lpdwBufferLength=x%x", *((LPDWORD)params[0].dwValue));
                ...
                for (j = 0; j < i; j++, pInfo++) {
                    ...
                    ShowStr("    %x %x %x %02x%02x%02x%02x%02x%02x%02x%02x",
                    ...
                    ShowStr("%sdwNameSpace=%d, %s",
                            szNSInfoN,
                            pInfo->dwNameSpace,
                            GetStringFromOrdinalValue(pInfo->dwNameSpace, aNameSpaces));
                    ShowStr("%sfActive=%s", szNSInfoN, (pInfo->fActive ? "TRUE" : "FALSE"));
                    ShowStr("%sdwVersion=x%x", szNSInfoN, pInfo->dwVersion);
                    ShowStr((funcIndex == ws_WSAEnumNameSpaceProvidersA ? "%slpszIdentifier=%s" : "%slpszIdentifier=%ws"),
                            szNSInfoN,
    ...
    case ws_WSAEnumNetworkEvents:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = WSAEnumNetworkEvents((SOCKET)params[0].dwValue,
                (WSAEVENT)params[1].dwValue,
                                          (LPWSANETWORKEVENTS)params[2].dwValue)) == 0) {
                LPWSANETWORKEVENTS  pNetEvents = (LPWSANETWORKEVENTS)params[2].dwValue;

                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  lpNetworkEvents=x%x", pNetEvents);
                ShowStr("    ->lNetworkEvents=x%x", pNetEvents->lNetworkEvents);
                ShowStr("    ->iErrorCode[READ]=x%x", pNetEvents->iErrorCode[FD_READ_BIT]);
                ShowStr("    ->iErrorCode[WRITE]=x%x", pNetEvents->iErrorCode[FD_WRITE_BIT]);
                ShowStr("    ->iErrorCode[OOB]=x%x", pNetEvents->iErrorCode[FD_OOB_BIT]);
                ShowStr("    ->iErrorCode[ACCEPT]=x%x", pNetEvents->iErrorCode[FD_ACCEPT_BIT]);
                ShowStr("    ->iErrorCode[CONNECT]=x%x", pNetEvents->iErrorCode[FD_CONNECT_BIT]);
                ShowStr("    ->iErrorCode[CLOSE]=x%x", pNetEvents->iErrorCode[FD_CLOSE_BIT]);
                ShowStr("    ->iErrorCode[QOS]=x%x", pNetEvents->iErrorCode[FD_QOS_BIT]);
                ShowStr("    ->iErrorCode[GROUP_QOS]=x%x", pNetEvents->iErrorCode[FD_GROUP_QOS_BIT]);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
                ShowUnknownError(funcIndex, i);
    ...
    case ws_WSAEnumProtocolsW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (funcIndex == ws_WSAEnumProtocolsA) {
                i = WSAEnumProtocolsA((LPINT)params[0].dwValue,
                    (LPWSAPROTOCOL_INFOA)params[1].dwValue,
                                      (LPDWORD)params[2].dwValue);
            } else {
                i = WSAEnumProtocolsW((LPINT)params[0].dwValue,
                    (LPWSAPROTOCOL_INFOW)params[1].dwValue,
                                      (LPDWORD)params[2].dwValue);
            }

            if (i != SOCKET_ERROR) {
                LPWSAPROTOCOL_INFOA pInfo = (LPWSAPROTOCOL_INFOA)params[1].dwValue;
                ...
                for (j = 0; j < i; j++) {
                    if (funcIndex == ws_WSAEnumProtocolsA) {
                        ...
                        pInfo = (LPWSAPROTOCOL_INFOA)(((LPBYTE)pInfo) + sizeof(WSAPROTOCOL_INFOW));
                    }
                }
    ...
    case ws_WSAEventSelect:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = WSAEventSelect((SOCKET)params[0].dwValue,
                (WSAEVENT)params[1].dwValue,
                                    (long)params[2].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
    ...
    case ws_WSAGetLastError:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
            ShowStr("  LastError=%d, %s", i, GetStringFromOrdinalValue(i, aWSAErrors));
        }
    ...
    case ws_WSAGetOverlappedResult:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (WSAGetOverlappedResult((SOCKET)params[0].dwValue,
                (LPWSAOVERLAPPED)params[1].dwValue,
                                       (LPDWORD)params[2].dwValue,
                                       (BOOL)params[3].dwValue,
                                       (LPDWORD)params[4].dwValue) == TRUE) {
                PMYOVERLAPPED pOverlapped = (PMYOVERLAPPED)params[1].dwValue;

                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  Overlapped %s completed", aFuncNames[pOverlapped->FuncIndex]);
                ShowStr("  *lpcbTransfer=x%x", *((LPDWORD)params[2].dwValue));
                ShowFlags(*((LPDWORD)params[4].dwValue), "  *lpdwFlags=", aWSASendAndRecvFlags);

                switch (pOverlapped->FuncIndex) {
                case ws_WSAIoctl:
                    if (*((LPDWORD)params[2].dwValue)) {
                        ShowStr("  lpvOUTBuffer=x%x", (pOverlapped + 1));
                        ShowBytes(*((LPDWORD)params[2].dwValue), (pOverlapped + 1), 2);
                    }
    ...
    case ws_WSAGetQOSByName:
    {
        ...
        LPWSABUF    pQOSName = (LPWSABUF)MyAlloc(dwBigBufSize + sizeof(WSABUF));
        FUNC_PARAM params[] =
        {
            { gszSocket,    PT_DWORD,       (ULONG_PTR)gpSelectedSocket->Sock, NULL },
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if (WSAGetQOSByName((SOCKET)params[0].dwValue, (LPWSABUF)params[1].dwValue, (LPQOS)params[4].dwValue) == TRUE) {
                ...
                ShowStr("    ->SendingFlowspec.TokenRate=x%x", pQOS->SendingFlowspec.TokenRate);
                ShowStr("    ->SendingFlowspec.TokenBucketSize=x%x", pQOS->SendingFlowspec.TokenBucketSize);
                ShowStr("    ->SendingFlowspec.PeakBandwidth=x%x", pQOS->SendingFlowspec.PeakBandwidth);
                ShowStr("    ->SendingFlowspec.Latency=x%x", pQOS->SendingFlowspec.Latency);
                ShowStr("    ->SendingFlowspec.DelayVariation=x%x", pQOS->SendingFlowspec.DelayVariation);
                ShowStr("    ->SendingFlowspec.ServiceType=%s (x%x)",
                        GetStringFromOrdinalValue((DWORD)pQOS->SendingFlowspec.ServiceType, aQOSServiceTypes),
                        (DWORD)pQOS->SendingFlowspec.ServiceType);
                ShowStr("    ->SendingFlowspec.MaxSduSize=x%x", pQOS->SendingFlowspec.MaxSduSize);
                ShowStr("    ->SendingFlowspec.MinimumPolicedSize=x%x", pQOS->SendingFlowspec.MinimumPolicedSize);
                ShowStr("    ->ReceivingFlowspec.TokenRate=x%x", pQOS->ReceivingFlowspec.TokenRate);
                ShowStr("    ->ReceivingFlowspec.TokenBucketSize=x%x", pQOS->ReceivingFlowspec.TokenBucketSize);
                ShowStr("    ->ReceivingFlowspec.PeakBandwidth=x%x", pQOS->ReceivingFlowspec.PeakBandwidth);
                ShowStr("    ->ReceivingFlowspec.Latency=x%x", pQOS->ReceivingFlowspec.Latency);
                ShowStr("    ->ReceivingFlowspec.DelayVariation=x%x", pQOS->ReceivingFlowspec.DelayVariation);
                ShowStr("    ->ReceivingFlowspec.ServiceType=%s (x%x)",
                        GetStringFromOrdinalValue((DWORD)pQOS->ReceivingFlowspec.ServiceType, aQOSServiceTypes),
                        (DWORD)pQOS->SendingFlowspec.ServiceType);
                ShowStr("    ->ReceivingFlowspec.MaxSduSize=x%x", pQOS->ReceivingFlowspec.MaxSduSize);
                ShowStr("    ->ReceivingFlowspec.MinimumPolicedSize=x%x", pQOS->ReceivingFlowspec.MinimumPolicedSize);
                ShowStr("    ->ProviderSpecific.len=x%x", pQOS->ProviderSpecific.len);

                if (pQOS->ProviderSpecific.len) {
                    ShowStr("    ->ProviderSpecific.buf=x%x", pQOS->ProviderSpecific.buf);
                    ShowBytes(pQOS->ProviderSpecific.len, pQOS->ProviderSpecific.buf, 3);
                }
            } else {
                ShowError(funcIndex, 0);
            }
    ...
    case ws_WSAGetServiceClassInfoW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (funcIndex == ws_WSAGetServiceClassInfoA) {
                i = WSAGetServiceClassInfoA((LPGUID)params[0].dwValue,
                    (LPGUID)params[1].dwValue,
                                            (LPDWORD)params[2].dwValue,
                                            (LPWSASERVICECLASSINFOA)params[3].dwValue);
            } else {
                i = WSAGetServiceClassInfoW((LPGUID)params[0].dwValue,
                    (LPGUID)params[1].dwValue,
                                            (LPDWORD)params[2].dwValue,
            ...
            if (i == 0) {
                ...
                LPWSASERVICECLASSINFOA  pInfo = (LPWSASERVICECLASSINFOA)params[3].dwValue;

                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  lpServiceClassInfo=x%x", pInfo);
                ShowStr("    ->lpServiceClassId=%x %x %x %02x%02x%02x%02x%02x%02x%02x%02x",
                ...
                ShowStr((funcIndex == ws_WSAGetServiceClassInfoA ?
                ...
                for (i = 0; i < (int)pInfo->dwCount; i++, pClassInfo++) {
                    ...
                    ShowStr((funcIndex == ws_WSAGetServiceClassInfoA ? "%s.lpszName=%s" : "%s.lpszName=%ws"),
                            szClassInfoN,
                            pClassInfo->lpszName);
                    ShowStr("%sdwNameSpace=%d, %s",
                            szClassInfoN,
                            pClassInfo->dwNameSpace,
                            GetStringFromOrdinalValue(pClassInfo->dwNameSpace, aNameSpaces));
                    ShowStr("%sdwValueType=%d", szClassInfoN, pClassInfo->dwValueType); // BUGBUG supposed to be flags?
                    ShowStr("%sdwValueSize=%d", szClassInfoN, pClassInfo->dwValueSize);
                    ShowStr("%slpValue=x%x", szClassInfoN, pClassInfo->lpValue);
                    ShowBytes(pClassInfo->dwValueSize, pClassInfo->lpValue, 3);
                }
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
    ...
    case ws_WSAGetServiceClassNameByClassIdW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (funcIndex == ws_WSAGetServiceClassNameByClassIdA) {
                i = WSAGetServiceClassNameByClassIdA((LPGUID)params[0].dwValue,
                    (LPSTR)params[1].dwValue,
                                                     (LPDWORD)params[2].dwValue);
            } else {
                i = WSAGetServiceClassNameByClassIdW((LPGUID)params[0].dwValue,
    ...
    case ws_WSAIoctl:
    {
        ...
        FUNC_PARAM_HEADER paramsHeader = {10, funcIndex, params, NULL};
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((i = WSAIoctl(
                ...
                (LPWSAOVERLAPPED_COMPLETION_ROUTINE)params[9].dwValue)) == 0) {
    ...
    case ws_WSAJoinLeaf:
    {
        ...
        FUNC_PARAM_HEADER paramsHeader = {14, funcIndex, params, NULL};
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((i = (int)WSAJoinLeaf((SOCKET)params[0].dwValue,
                (LPSOCKADDR)params[1].dwValue,
                                      (int)params[4].dwValue,
                                      (LPWSABUF)params[5].dwValue,
                                      (LPWSABUF)params[8].dwValue,
    ...
    case ws_WSALookupServiceNextW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (funcIndex == ws_WSALookupServiceNextA) {
                i = WSALookupServiceNextA((HANDLE)params[0].dwValue,
                    (DWORD)params[1].dwValue,
                                          (LPDWORD)params[2].dwValue,
                                          (LPWSAQUERYSETA)params[3].dwValue);
            } else {
                i = WSALookupServiceNextW((HANDLE)params[0].dwValue,
                    (DWORD)params[1].dwValue, (LPDWORD)params[2].dwValue,
    ...
    case ws_WSARecv:
    {
        PMYOVERLAPPED   pOverlapped = MyAlloc(sizeof(MYOVERLAPPED) + 2 * sizeof(WSABUF) + 2 * dwBigBufSize);
        LPWSABUF        lpBuffers = (LPWSABUF)(pOverlapped + 1);
        LPBYTE          pBuf0 = (LPBYTE)(lpBuffers + 2), pBuf1 = pBuf0 + dwBigBufSize;
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((i = WSARecv((SOCKET)params[0].dwValue,
                (LPWSABUF)params[1].dwValue,
                             ...
                             (LPWSAOVERLAPPED_COMPLETION_ROUTINE)params[12].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  *lpdwNumBytesRecvd=x%x", *((LPDWORD)params[7].dwValue));
                ShowFlags(*((LPDWORD)params[8].dwValue), "  *lpdwFlags", aWSASendFlags);
            } else if (i == SOCKET_ERROR) {
                if (ShowError(funcIndex, 0) == WSA_IO_PENDING) {
    ...
    case ws_WSARecvDisconnect:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = WSARecvDisconnect((SOCKET)params[0].dwValue, (LPWSABUF)params[1].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
    ...
    case ws_WSARecvFrom:
    {
        ...
        FUNC_PARAM_HEADER paramsHeader = {17, funcIndex, params, NULL};
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((i = WSASendTo(
                ...
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  *lpdwNumBytesSent=x%x", *((LPDWORD)params[7].dwValue));
                ShowFlags(*((LPDWORD)params[8].dwValue), "  *lpdwNumBytesSent=x%x", aWSAFlags);
            } else if (i == SOCKET_ERROR) {
                if (ShowError(funcIndex, 0) == WSA_IO_PENDING) {
    ...
    case ws_WSASend:
    {
        ...
        FUNC_PARAM_HEADER paramsHeader = {12, funcIndex, params, NULL};
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((i = WSASend((SOCKET)params[0].dwValue,
                (LPWSABUF)params[1].dwValue,
                             (DWORD)params[6].dwValue,
                             (LPDWORD)params[7].dwValue,
                             (DWORD)params[8].dwValue,
                             (LPWSAOVERLAPPED)params[9].dwValue,
                             (LPWSAOVERLAPPED_COMPLETION_ROUTINE)params[11].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  *lpdwNumBytesSent=x%x", *((LPDWORD)params[7].dwValue));
            } else if (i == SOCKET_ERROR) {
                if (ShowError(funcIndex, 0) == WSA_IO_PENDING) {
    ...
    case ws_WSASendDisconnect:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = WSASendDisconnect((SOCKET)params[0].dwValue, (LPWSABUF)params[1].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
            } else if (i == SOCKET_ERROR) {
                ShowError(funcIndex, 0);
            } else {
    ...
    case ws_WSASendTo:
    {
        ...
        FUNC_PARAM_HEADER paramsHeader = {16, funcIndex, params, NULL};
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if ((i = WSASendTo((SOCKET)params[0].dwValue,
                (LPWSABUF)params[1].dwValue,
                               (DWORD)params[6].dwValue,
                               (LPDWORD)params[7].dwValue,
                               (DWORD)params[8].dwValue,
                               ...
                               (LPWSAOVERLAPPED_COMPLETION_ROUTINE)params[15].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  *lpdwNumBytesSent=x%x", *((LPDWORD)params[7].dwValue));
            } else if (i == SOCKET_ERROR) {
                if (ShowError(funcIndex, 0) == WSA_IO_PENDING) {
    ...
    case ws_WSASocketW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            ...
            if (gbWideStringParams) {
                pSocket->Sock = WSASocketW((int)params[0].dwValue,
                    (int)params[1].dwValue,
                                           (int)params[2].dwValue,
                ...
                pSocket->Sock = WSASocketA((int)params[0].dwValue,
                    (int)params[1].dwValue,
                                           (int)params[2].dwValue,
            ...
            if (pSocket->Sock != INVALID_SOCKET) {
                ...
                wsprintf(buf, "WSASocket=x%x (%s %s%s)",
                         pSocket->Sock,
                         GetStringFromOrdinalValue(pSocket->dwAddressFamily, aAddressFamilies),
                         GetStringFromOrdinalValue(pSocket->dwSocketType, aSocketTypes),
                         (pSocket->dwFlags & WSA_FLAG_OVERLAPPED ? " OVERLAPPED" : "")
                );
    ...
    case ws_WSAStartup:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = WSAStartup(LOWORD(params[0].dwValue), (LPWSADATA)params[1].dwValue)) == 0) {
                char szButtonText[32];
    ...
    case ws_WSAStringToAddressW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if (funcIndex == ws_WSAStringToAddressA) {
                i = WSAStringToAddressA((LPSTR)params[0].dwValue,
                    (INT)params[1].dwValue,
                                        (LPWSAPROTOCOL_INFOA)params[2].dwValue,
                                        (LPSOCKADDR)params[3].dwValue,
                                        (LPINT)params[4].dwValue);
            } else {
                i = WSAStringToAddressW((LPWSTR)params[0].dwValue,
                    (INT)params[1].dwValue,
                                        (LPWSAPROTOCOL_INFOW)params[2].dwValue,
                                        (LPSOCKADDR)params[3].dwValue,
                                        (LPINT)params[4].dwValue);
            ...
            if (i == 0) {
                ...
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
                ShowStr("  *lpAddressLength=x%x", *((LPINT)params[4].dwValue));
    ...
    case ws_WSCEnumProtocols:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            i = WSCEnumProtocols((LPINT)params[0].dwValue,
                (LPWSAPROTOCOL_INFOW)params[1].dwValue,
                                 (LPDWORD)params[2].dwValue,
                                 (LPINT)params[3].dwValue);
            if (i != SOCKET_ERROR) {
                LPWSAPROTOCOL_INFOW pInfo = (LPWSAPROTOCOL_INFOW)params[1].dwValue;
    ...
    case ws_WSCGetProviderPath:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            if ((i = WSCGetProviderPath((LPGUID)params[0].dwValue,
                (LPWSTR)params[1].dwValue,
                                        (LPINT)params[2].dwValue,
                                        (LPINT)params[3].dwValue)) == 0) {
                ShowStr(gszXxxSUCCESS, aFuncNames[funcIndex]);
    ...
    case ws_EnumProtocolsW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            i = (funcIndex == ws_EnumProtocolsA ?
                 EnumProtocolsA((LPINT)params[0].dwValue, (LPVOID)params[1].dwValue, (LPDWORD)params[2].dwValue) :
                 EnumProtocolsW((LPINT)params[0].dwValue, (LPVOID)params[1].dwValue, (LPDWORD)params[2].dwValue));
            ...
            if (i >= 0) {
                ...
                for (j = 0, pInfo = (PROTOCOL_INFO *)params[1].dwValue; j < i; j++, pInfo++) {
                    ...
                    ShowStr("%siAddressFamily=%d, %s",
                            szProtoInfoN,
                            (DWORD)pInfo->iAddressFamily,
                            GetStringFromOrdinalValue((DWORD)pInfo->iAddressFamily, aAddressFamilies));
                    ShowStr("%siMaxSockAddr=%d", szProtoInfoN, (DWORD)pInfo->iMaxSockAddr);
                    ShowStr("%siMinSockAddr=%d", szProtoInfoN, (DWORD)pInfo->iMinSockAddr);
                    ShowStr("%siSocketType=%d, %s",
                            szProtoInfoN,
                            (DWORD)pInfo->iSocketType,
                            GetStringFromOrdinalValue((DWORD)pInfo->iSocketType, aSocketTypes));
                    ShowStr("%siProtocol=%d (x%x)", // BUGBUG ought to be flags???
                            szProtoInfoN,
                            (DWORD)pInfo->iProtocol,
                            (DWORD)pInfo->iProtocol);
                    ShowStr("%sdwMessageSize=x%x", szProtoInfoN, (DWORD)pInfo->dwMessageSize);
                    ShowStr("%slpProtocol=x%x", szProtoInfoN, pInfo->lpProtocol);

                    if (funcIndex == ws_EnumProtocolsA) {
                        ShowStr("%s*lpProtocol=%s", szProtoInfoN, pInfo->lpProtocol);
    ...
    case ws_GetAddressByNameW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            i = (funcIndex == ws_GetAddressByNameA ?
                 ...
                 GetAddressByNameA((DWORD)params[0].dwValue,
                 (LPGUID)params[1].dwValue,
                 ...
                 GetAddressByNameW((DWORD)params[0].dwValue,
                 (LPGUID)params[1].dwValue,
            ...
            if (i >= 0) {
                ...
                for (j = 0, pInfo = (CSADDR_INFO *)params[6].dwValue; j < i; j++, pInfo++) {
                    ...
                    ShowBytes(pInfo->LocalAddr.iSockaddrLength, pInfo->LocalAddr.lpSockaddr, 2);
                    ShowStr("%sLocalAddr.iSockaddrLen=%d (x%x)",
                            szAddrInfoN,
                            pInfo->LocalAddr.iSockaddrLength,
                            pInfo->LocalAddr.iSockaddrLength);
                    ShowStr("%sRemoteAddr.lpSockaddr=x%x", szAddrInfoN, pInfo->RemoteAddr.lpSockaddr);
                    ShowBytes(pInfo->RemoteAddr.iSockaddrLength, pInfo->RemoteAddr.lpSockaddr, 2);
                    ShowStr("%sRemoteAddr.iSockaddrLen=%d (x%x)",
                            szAddrInfoN,
    ...
    case ws_GetNameByTypeW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            i = (funcIndex == ws_GetNameByTypeA ?
                 GetNameByTypeA((LPGUID)params[0].dwValue, (LPSTR)params[1].dwValue, (DWORD)params[2].dwValue) :
                 GetNameByTypeW((LPGUID)params[0].dwValue, (LPWSTR)params[1].dwValue, (DWORD)params[2].dwValue));
    ...
    case ws_GetServiceW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            i = (funcIndex == ws_GetServiceA ?
                 GetServiceA((DWORD)params[0].dwValue,
                 (LPGUID)params[1].dwValue,
                             (LPSTR)params[2].dwValue,
                             (DWORD)params[3].dwValue,
                             (LPVOID)params[4].dwValue,
                 ...
                 GetServiceW((DWORD)params[0].dwValue,
                 (LPGUID)params[1].dwValue,
                             (LPWSTR)params[2].dwValue,
                             (DWORD)params[3].dwValue,
                             (LPVOID)params[4].dwValue,
            ...
            if (i >= 0) {
                ...
                for (j = 0, pInfo = (SERVICE_INFO *)params[4].dwValue; j < i; j++, pInfo++) {
                    ...
                    ShowStr((funcIndex == ws_GetServiceA ? "%slpServiceName=%s" : "%slpServiceName=%ws"), szSvcInfoN, pInfo->lpServiceName);
                    ShowStr((funcIndex == ws_GetServiceA ? "%slpComment=%s" : "%slpComment=%ws"), szSvcInfoN, pInfo->lpServiceName);
                    ShowStr((funcIndex == ws_GetServiceA ? "%slpLocale=%s" : "%slpLocale=%ws"), szSvcInfoN, pInfo->lpLocale);
                    ShowStr("%sdwDisplayHint=%d, %s",
                            szSvcInfoN,
                            (DWORD)pInfo->dwDisplayHint,
                            GetStringFromOrdinalValue((DWORD)pInfo->dwDisplayHint, aResDisplayTypes));
                    ShowStr("%sdwVersion=x%x", szSvcInfoN, pInfo->dwVersion);
                    ShowStr("%sdwTime=x%x", szSvcInfoN, pInfo->dwTime);
                    ShowStr((funcIndex == ws_GetServiceA ? "%slpMachineName=%s" : "%slpMachineName=%ws"),
                            szSvcInfoN,
                            pInfo->lpMachineName);
                    ShowStr("%slpServiceAddress.dwAddressCount=%d (x%x)",
                            szSvcInfoN,
                            pInfo->lpServiceAddress->dwAddressCount,
                            pInfo->lpServiceAddress->dwAddressCount);

                    for (k = 0, pAddr = pInfo->lpServiceAddress->Addresses;
                         k < (int)pInfo->lpServiceAddress->dwAddressCount;
                         k++, pAddr++) {
                    }

                    ShowStr("%sServiceSpecificInfo.cbSize=%d (x%x)",
                            szSvcInfoN,
                            pInfo->ServiceSpecificInfo.cbSize,
                            pInfo->ServiceSpecificInfo.cbSize);
                    ShowBytes(pInfo->ServiceSpecificInfo.cbSize, pInfo->ServiceSpecificInfo.pBlobData, 2);
                }
    ...
    case ws_GetTypeByNameW:
    {
        ...
        if (LetUserMungeParams(&paramsHeader)) {
            i = (funcIndex == ws_GetTypeByNameA ?
                 GetTypeByNameA((LPSTR)params[0].dwValue, (LPGUID)params[1].dwValue) :
                 GetTypeByNameW((LPWSTR)params[0].dwValue, (LPGUID)params[1].dwValue));
...
void PASCAL QueueAsyncRequestInfo(PASYNC_REQUEST_INFO pAsyncReqInfo)
{
...
PASYNC_REQUEST_INFO PASCAL DequeueAsyncRequestInfo(HANDLE  hRequest)
{
    PASYNC_REQUEST_INFO  pAsyncReqInfo;

    for (pAsyncReqInfo = gpAsyncReqInfoList;
...
{
    // In order to maximize speed, minimize flash, & have the latest info in the edit control scrolled into view we shrink the window down and hide it.
    // Later, when all the text has been inserted in the edit control, we show the window (since window must be visible in order to
    // scroll caret into view), then tell it to scroll the caret (at this point the window is still 1x1 so the painting
    // overhead is 0), and finally restore the control to it's
    // full size. In doing so we have zero flash and only 1 real complete paint. Also put up the hourglass for warm fuzzies.
    ...
    if (bBegin) {
        ...
        SetWindowPos(ghwndEdit,
            (HWND)NULL,
                     ...
                     1,
                     1,
                     SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOREDRAW | SWP_NOZORDER | SWP_HIDEWINDOW);
    } else {
        iNumBegins--;
...
void PASCAL ShowBytes(DWORD dwSize, LPVOID  lp, DWORD dwNumTabs)
{
    char    tabBuf[17] = "";
    char    buf[80];
    ...
    if (dwMod4) {
        DWORD dwNumUnalignedBytes = 4 - dwMod4,
            dwNumBytesToShow = (dwNumUnalignedBytes > dwSize ? dwSize : dwNumUnalignedBytes);
    ...
    if (dwSize >= (4 * sizeof(DWORD))) {
        ...
        for (i = 0; i < (dwNumDWORDs - (dwNumDWORDs % 4)); i += 4) {
            sprintf(buf, "%s%08lx %08lx %08lx %08lx  ", tabBuf, *lpdw, *(lpdw + 1), *(lpdw + 2), *(lpdw + 3));
...
LPSTR PASCAL GetStringFromOrdinalValue(DWORD   dwValue, PLOOKUP pLookup)
{
    DWORD i;
...
VOID PASCAL ShowGUID(char *pszProlog, GUID *pGuid)
{
    ...
    // Dump the GUID in a form that can be copied & pasted into other API params
...
VOID PASCAL ShowProtoInfo(LPWSAPROTOCOL_INFOA pInfo, DWORD dwIndex, BOOL bAscii)
{
    ...
    ShowStr("%sdwServiceFlags2=x%x", szProtoInfoN, pInfo->dwServiceFlags2);
    ShowStr("%sdwServiceFlags3=x%x", szProtoInfoN, pInfo->dwServiceFlags3);
    ShowStr("%sdwServiceFlags4=x%x", szProtoInfoN, pInfo->dwServiceFlags4);
    ...
    ShowStr("%sdwCatalogEntryId=x%x", szProtoInfoN, pInfo->dwCatalogEntryId);
    ShowStr("%sProtocolChain.ChainLen=x%x", szProtoInfoN, (DWORD)pInfo->ProtocolChain.ChainLen);
    ShowStr("%sProtocolChain.ChainEntries=", szProtoInfoN);
    ShowBytes(sizeof(DWORD) * MAX_PROTOCOL_CHAIN, pInfo->ProtocolChain.ChainEntries, 2);
    ShowStr("%siVersion=x%x", szProtoInfoN, (DWORD)pInfo->iVersion);
    ShowStr("%siAddressFamily=%d, %s",
            szProtoInfoN,
            (DWORD)pInfo->iAddressFamily,
            GetStringFromOrdinalValue((DWORD)pInfo->iAddressFamily, aAddressFamilies));
    ...
    ShowStr("%siProtocolMaxOffset=%d", szProtoInfoN, (DWORD)pInfo->iProtocolMaxOffset);
    ShowStr("%siNetworkByteOrder=%d, %s",
            szProtoInfoN,
            (DWORD)pInfo->iNetworkByteOrder,
            GetStringFromOrdinalValue((DWORD)pInfo->iNetworkByteOrder, aNetworkByteOrders));
    ShowStr("%siSecurityScheme=x%x", szProtoInfoN, (DWORD)pInfo->iSecurityScheme);
    ShowStr("%sdwMessageSize=x%x", szProtoInfoN, (DWORD)pInfo->dwMessageSize);
    ShowStr("%sdwProviderReserved=x%x", szProtoInfoN, (DWORD)pInfo->dwMessageSize);

    if (bAscii) {
        ShowStr("%sszProtocol=%s", szProtoInfoN, pInfo->szProtocol);
