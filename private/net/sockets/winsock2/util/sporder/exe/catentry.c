...
typedef struct {
    ...
    // then  this  type  definition  will  have  to  be changed, since this structure must be strictly flat.
} PACKED_CAT_ITEM;

PACKED_CAT_ITEM garPackCat[MAX_ENTRIES];
DWORD garcbData[MAX_ENTRIES];

// When we first enumerate and read the child registry keys, store all of those names for later use.
...
int gNumRows = 0;//  keep track of the total number of entries in the list box;


int CatReadRegistry (HWND hwnd)
/*++
  Called once when the dialog first comes up.  Read the registry and fill the listbox with all of the entries.
--*/
{
    ...
    //  index there, and it will stay glued to the string even as the user reorders them (unless we are building a debug).
    ...
    r = RegOpenKeyEx (HKEY_LOCAL_MACHINE, pszWS2RegKey, 0, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &hKey);
    if (r != ERROR_SUCCESS) {
        CatCheckRegErrCode (hwnd, r, TEXT("RegOpenKeyEx"));
        return FALSE;
    }

    // The initial open succeeded, now enumerate registry keys until we don't get any more back
    for (iIndex = 0; ;iIndex++) {
        ASSERT ((iIndex <MAX_ENTRIES), TEXT("Exceeded max number of protocol entries"));

        dwSize = MAX_STR;
        szOutput[0]=0;
        r=RegEnumKeyEx (hKey, iIndex, szOutput, &dwSize, NULL, NULL, NULL, &ftDummy);
        ...
        r = RegOpenKeyEx (HKEY_LOCAL_MACHINE, szInput, 0, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &hSubKey);
        if (r != ERROR_SUCCESS) {
            CatCheckRegErrCode (hwnd, r, TEXT("RegOpenKeyEx"));
            return FALSE;
        }
        ...
        // hack garPackCat
        dwType = REG_BINARY;
        r=RegQueryValueEx (hSubKey, WS2_SZ_KEYNAME, NULL, &dwType, (LPVOID) &garPackCat[iIndex], &dwBytes);
...
--*/
{
    ...
    switch (r) {
        ...
        default: {
            FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM, NULL, r, GetUserDefaultLangID(), szBuffer, sizeof (szBuffer), 0);
            lstrcat (szBuffer, TEXT("\n"));
            lstrcat (szBuffer, lpstr);
            MessageBox (hwnd, szBuffer, szTitle, MB_ICONSTOP | MB_OK);
            return FALSE;
