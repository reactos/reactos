...
Abstract:
    Main module for sporder.dll...  the 32-Bit Windows functions that are used to change the order or WinSock2 transport service providers and name space providers.
--*/
...
void _cdecl MyDbgPrint(PSTR Format, ...)
{
    va_list arglist;
...
--*/
{
    ...
    r = RegOpenKeyEx (HKEY_LOCAL_MACHINE, pszBaseKey, 0, KEY_QUERY_VALUE, &hKey);
    if (r != ERROR_SUCCESS)
    {
    ...
    r = RegQueryValueEx (hKey, pszCurrentProtocolCatalog, NULL, &dwType, (LPVOID) pszBuffer, &dwBytes);
    RegCloseKey (hKey);
    if (r != ERROR_SUCCESS)
    {
    ...
    r = RegOpenKeyEx (HKEY_LOCAL_MACHINE, pszFinalKey, 0, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &hKey);
    if (r != ERROR_SUCCESS)
    {
    ...
    for (iIndex = 0; ;iIndex++)
    {
        ...
        r = RegOpenKeyEx (HKEY_LOCAL_MACHINE, szFinalPlusSubKey, 0, KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS, &hSubKey);
        if (r != ERROR_SUCCESS)
        {
    ...
    for (i = 0; i < dwNumberOfEntries ;i++)
    {
        if (dwMapping[i] != i)
        {
            r = RegOpenKeyEx (HKEY_LOCAL_MACHINE, &pszKeyNames[i][0], 0, KEY_SET_VALUE, &hKey);
            if (r != ERROR_SUCCESS)
            {
            ...
            r = RegSetValueEx (hKey, WS2_SZ_KEYNAME, 0, REG_BINARY, (LPVOID) &garPackCat[dwMapping[i]], garcbData[i]);
            if (r != ERROR_SUCCESS)
            {
...
int WSPAPI WSCEnumNameSpaceProviders (IN OUT LPDWORD lpdwBufferLength, IN LPWSANAMESPACE_INFOW lpnspBuffer, LPINT lpErrno)
{
