...
Abstract:
    This module contains support for the Name Space Provider for TCPIP, which includes four actual NSPs:
...
LPHOSTENT GetHostentFromName(IN LPSOCK_THREAD pThread, IN LPSTR Name);
...
VOID CopyToAliasBuffer(IN PSTR Alias, IN OUT LPTSTR AliasBuffer, IN OUT LPDWORD lpdwAliasBufferLength, IN OUT LPDWORD AliasBytesUsed);
INT GuidToPort(IN LPGUID lpServiceType, IN LPINT lpiProtocols, OUT LPWORD Port, OUT LPBOOL IsTcp);
INT HostentToCsaddr(
...
VOID ReadPriorities(VOID);
INT ServiceCsaddr(IN WORD Port, IN BOOL IsTcp, IN OUT LPVOID lpCsaddrBuffer, IN OUT LPDWORD lpdwBufferLength);
INT APIENTRY TcpipLocalGetAddressByName(
...
DWORD APIENTRY TcpipLocalSetService(
...
INT APIENTRY TcpipHostsGetAddressByName(
    IN     LPGUID          lpServiceType,
    IN     LPTSTR          lpServiceName,
    IN     LPINT           lpiProtocols,
    IN     DWORD           dwResolution,
...
INT APIENTRY TcpipDnsGetAddressByName(
    IN     LPGUID          lpServiceType,
    IN     LPTSTR          lpServiceName,
    IN     LPINT           lpiProtocols,
    IN     DWORD           dwResolution,
...
INT APIENTRY TcpipNetbtGetAddressByName(
    IN     LPGUID          lpServiceType,
    IN     LPTSTR          lpServiceName,
    IN     LPINT           lpiProtocols,
    IN     DWORD           dwResolution,
...
)
{
    ...
    // We support four name spaces in this DLL, each of which exports a single NSP API.
    bufferLengthRequired = 4 * (sizeof(NS_ROUTINE) + (4 * sizeof(LPFN_NSPAPI)));

    // If the caller specified too small a buffer, fail and tell them how large a buffer is required.
    ...
    // Tell the caller the version of the NSP API interface which we support.
    *lpdwVersion = 1;

    // Calculate where in the buffer we'll put the NS_ROUTINE structures for each of our name spaces.
    nsRoutineLocal = nsrBuffer;
    nsRoutineHosts = nsrBuffer + 1;
    nsRoutineDns = nsrBuffer + 2;
    nsRoutineNetbt = nsrBuffer + 3;

    // Calculate where in the buffer each of the function pointer arrays will go.
    ...
    nsRoutineLocal->alpfnFunctions[NSPAPI_GET_ADDRESS_BY_NAME] = (LPFN_NSPAPI)TcpipLocalGetAddressByName;
    nsRoutineLocal->alpfnFunctions[NSPAPI_GET_SERVICE] = NULL;
    nsRoutineLocal->alpfnFunctions[NSPAPI_SET_SERVICE] = (LPFN_NSPAPI)TcpipLocalSetService;
    ...
    nsRoutineHosts->alpfnFunctions[NSPAPI_GET_ADDRESS_BY_NAME] = (LPFN_NSPAPI)TcpipHostsGetAddressByName;
    ...
    nsRoutineDns->alpfnFunctions[NSPAPI_GET_ADDRESS_BY_NAME] = (LPFN_NSPAPI)TcpipDnsGetAddressByName;
    ...
    nsRoutineNetbt->alpfnFunctions[NSPAPI_GET_ADDRESS_BY_NAME] = (LPFN_NSPAPI)TcpipNetbtGetAddressByName;
...
VOID ReadPriorities(VOID)
{
    ...
    error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TCPIP_SERVICE_PROVIDER_KEY, 0, KEY_READ, &tcpipKey);
    if (error != NO_ERROR) {
        return;
    }
    ...
    RegQueryValueEx(tcpipKey, TEXT("LocalPriority"), NULL, &type, (PVOID)&LocalPriority, &entryLength);
    RegQueryValueEx(tcpipKey, TEXT("HostsPriority"), NULL, &type, (PVOID)&HostsPriority, &entryLength);
    RegQueryValueEx(tcpipKey, TEXT("DnsPriority"), NULL, &type, (PVOID)&DnsPriority, &entryLength);
    RegQueryValueEx(tcpipKey, TEXT("NetbtPriority"), NULL, &type, (PVOID)&NetbtPriority, &entryLength);

#if defined(HOSTENT_CACHING_ENABLED)
    RegQueryValueEx(tcpipKey, TEXT("MaxHostentCacheSize"), NULL, &type, (PVOID)&MaxHostentCacheSize, &entryLength);
#endif
...
)
{
    ...
    // Find the port number for this service.  We do this before other resolutions to save time in case this fails.
    error = GuidToPort(lpServiceType, lpiProtocols, &port, &isTcp);
    if (error != NO_ERROR) {
        return 0;
    }

    // If this is a service resolution, we don't actually want to do a lookup.  Just fill in the appropriate bind address immediately.
    ...
    // that we don't look in the cache if RES_FIND_MULTIPLE is set since we'll be going to all the providers anyway and the info returned
    // here will be extraneous and could cause problems with lpdwBufferLength if the app makes multiple calls for the same data.
#if defined(HOSTENT_CACHING_ENABLED)
    if ((dwResolution & RES_FIND_MULTIPLE) == 0 && CurrentHostentCacheSize > 0) {
        ...
        if (NT_SUCCESS(status)) {
            // We hold the hostent cache lock until we're completely done with any returned hostent.  This prevents another
            // thread from screwing with the list or the returned hostent while we're processing.
            ...
            if (hostEntry != NULL) {
                error = HostentToCsaddr(hostEntry, port, isTcp, lpCsaddrBuffer, lpdwBufferLength, lpAliasBuffer, lpdwAliasBufferLength, &count);
    ...
    // If the service name is present and equal to "localhost" or "loopback", return 127.0.0.1.
    if (lpServiceName != NULL && (_tcscmp(lpServiceName, TEXT("localhost")) == 0 || _tcscmp(lpServiceName, TEXT("loopback")) == 0)) {
    ...
    // If the service name is NULL and the caller is requesting info on DNS addresses, return information on the DNS server IP addresses that we know about.
    ...
    // Check whether the service name is the same as the local host name.  Note that we consider lpServiceName == NULL to be the local host.
    ...
    if (lpServiceName == NULL || _tcscmp(lpServiceName, compareHostName) == 0) {
#if defined(UNICODE)
    ...
    // If we're not inside gethostbyname(), check if the service name is really an IP address.
    // We intentionally disable this for gethostbyname() calls because we don't want to break apps which depend on gethostname() not resolving IP addresses.
    // However, this is a useful feature for general RNR applications.
    if ((dwResolution & RES_GETHOSTBYNAME) == 0) {
        ...
        if (inAddr.s_addr != INADDR_NONE) {
            ...
            error = HostentToCsaddr(&hostEntryStruct, port, isTcp, lpCsaddrBuffer, lpdwBufferLength, lpAliasBuffer, lpdwAliasBufferLength, &count);
#if defined(UNICODE)
            RtlFreeAnsiString(&ansiHostName);
#endif
            if (error != NO_ERROR) {
...
)
{
    ...
    // If this is a service resolution, don't do anything here.
    // Only the TCP/IP local NSP will handle RES_SERVICE requests.
    // This prevents getting back multiple TCP/IP addresses to bind to, one from each TCP/IP NSP.
    if ((dwResolution & RES_SERVICE) != 0) {
        return 0;
    }

    // Find the port number for this service.
    // We do this before other resolutions to save time in case this fails.
    error = GuidToPort(lpServiceType, lpiProtocols, &port, &isTcp);
    if (error != NO_ERROR) {
        return 0;
    }
...
)
{
    ...
    // If we're not supposed to do WINS name resolution for this thread, stop.
...
)
{
    ...
    SERVICE_TYPE_INFO *pSvcTypeInfo = (SERVICE_TYPE_INFO *)lpServiceInfo->ServiceSpecificInfo.pBlobData;
    ...
    // We need to take action only on SERVICE_ADD_TYPE.  The rest are meaningless for the static name spaces we support.
...
#if defined(UNICODE)
    if (fUnicodeBlob) {
        pszSvcTypeName = (LPWSTR)(((LPBYTE)pSvcTypeInfo) + pSvcTypeInfo->dwTypeNameOffset);
    } else {
        ANSI_STRING ansiStr;

        RtlInitAnsiString(&ansiStr, (LPSTR)(((LPBYTE)pSvcTypeInfo) + pSvcTypeInfo->dwTypeNameOffset));
...
#else
    pszSvcTypeName = (LPSTR)(((LPBYTE)pSvcTypeInfo) + pSvcTypeInfo->dwTypeNameOffset);
#endif

    // If the service type name is an empty string, return error.
    if ((pSvcTypeInfo->dwTypeNameOffset == 0) || (pszSvcTypeName == NULL) || (*pszSvcTypeName == 0)) {
        err = ERROR_INVALID_PARAMETER;
        goto CleanExit;
    }

    // The following keys should have already been created
    err = REG_OPEN_KEY_EX(HKEY_LOCAL_MACHINE, NSP_SERVICE_KEY_NAME, 0, KEY_READ | KEY_WRITE, &hkey);
    if (err) {
        goto CleanExit;
    }

    err = REG_OPEN_KEY_EX(hkey, pszSvcTypeName, 0, KEY_READ | KEY_WRITE, &hkeyType);
    if (err) {
        goto CleanExit;
    }

    // Loop through all values in the specific and add them one by one to the registry if it belongs to our name space
    for (i = 0, pVal = pSvcTypeInfo->Values; i < pSvcTypeInfo->dwValueCount; i++, pVal++) {
        // Skip over values not aimed at one of the TCPIP name spaces.
        if (pVal->dwNameSpace != NS_TCPIP_LOCAL && pVal->dwNameSpace != NS_TCPIP_HOSTS && pVal->dwNameSpace != NS_DNS && pVal->dwNameSpace != NS_NETBT) {
            continue;
        }
...
INT GuidToPort(IN LPGUID lpServiceType, IN LPINT   lpiProtocols, OUT LPWORD Port, OUT LPBOOL IsTcp)
{
    ...
    // Determine whether they want TCP, UDP, or both.
    // If there was no protocols list passed in, they want both.
    // Otherwise, they must specify on or both manually.
    isTcp = FALSE;
    isUdp = FALSE;
    ...
    // If this is the special hostname GUID, we don't need to find a port.
    ...
    // Convert the GUID into a service name.
    // We'll first check whether port information is in the registry.
    // An application/service writes this information to the registry by using SetService() with SERVICE_ADD_TYPE.
    ...
    err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, keyNameBuffer, 0, KEY_READ, &serviceKey);
    if (err != NO_ERROR) {
        return -1;
    }

    // Attempt to read the "TcpPort" or "UdpPort" values from this key.
    // Note that we sort of ignore errors here; we act based on whether the regPort local variable changes from 0xFFFFFFFF.

    // Also, we don't have any support for a single GUID which supports both TCP and UDP.  This currently requires separate GUIDs.
    regPort = 0xFFFFFFFF;
    guidIsTcp = TRUE;
    bytesRequired = sizeof(regPort);

    if (isTcp) {
        RegQueryValueEx(serviceKey, TEXT("TcpPort"), NULL, &type, (PVOID)&regPort, &bytesRequired);
    }

    if (regPort == 0xFFFFFFFF && isUdp) {
        RegQueryValueEx(serviceKey, TEXT("UdpPort"), NULL, &type, (PVOID)&regPort, &bytesRequired);
        guidIsTcp = FALSE;
    }
...
)
{
    ...
    *lpdwBufferLength = requiredBufferLength;
    ...
    // Copy the aliases to the alias buffer.
    // The "official" name from the hostent goes as the first alias, then the actual aliases.
    if (ARGUMENT_PRESENT(lpAliasBuffer) && ARGUMENT_PRESENT(lpdwAliasBufferLength)) {
        ...
        if (HostEntry->h_name != NULL) {
            CopyToAliasBuffer(HostEntry->h_name, lpAliasBuffer, lpdwAliasBufferLength, &aliasBytesUsed);
        }

        if (HostEntry->h_aliases != NULL) {
            for (i = 0; HostEntry->h_aliases[i] != NULL; i++) {
                CopyToAliasBuffer(HostEntry->h_aliases[i], lpAliasBuffer, lpdwAliasBufferLength, &aliasBytesUsed);
            }
        }
...
VOID CopyToAliasBuffer(IN PSTR Alias, IN OUT LPTSTR AliasBuffer, IN OUT LPDWORD lpdwAliasBufferLength, IN OUT LPDWORD AliasBytesUsed)
{
    ...
    unicodeString.MaximumLength = (USHORT)(*lpdwAliasBufferLength - *AliasBytesUsed);
#endif
...
PHOSTENT QueryHostentCache(IN LPSTR Name OPTIONAL, IN DWORD IpAddress OPTIONAL)
{
    ...
    return NULL;// We didn't find a match in the hostent cache.
} // QueryHostentCache
...
INT ServiceCsaddr(IN WORD Port, IN BOOL IsTcp, IN OUT LPVOID lpCsaddrBuffer, IN OUT LPDWORD lpdwBufferLength)
{
    ...
    *lpdwBufferLength = requiredBufferLength;
    ...
    // Set the local sockaddr's port to the port for the service, and leave the remote's port zero.
    // All IP addresses remain INADDR_ANY (0).
    csaddrInfo = lpCsaddrBuffer;
    RtlZeroMemory(csaddrInfo, sizeof(*csaddrInfo));
    ...
    return 1;// Return a count of one entry.
} // ServiceCsaddr
...
INT WSHEnumProtocols(IN LPINT lpiProtocols, IN LPSTR lpTransportKeyName, IN OUT LPVOID lpProtocolBuffer, IN OUT LPDWORD lpdwBufferLength)
{
    DWORD bytesRequired;
    ...
    // Make sure that the caller has specified a sufficiently large buffer.
    bytesRequired = (sizeof(PROTOCOL_INFO11) * 2) + ((lstrlen(TCP_NAME) + 1) * sizeof(TCHAR)) + ((lstrlen(UDP_NAME) + 1) * sizeof(TCHAR));
    ...
    if (useTcp) {
        ...
        tcpProtocolInfo->dwServiceFlags = XP_GUARANTEED_DELIVERY | XP_GUARANTEED_ORDER | XP_GRACEFUL_CLOSE | XP_EXPEDITED_DATA | XP_FRAGMENTATION;
        ...
        tcpProtocolInfo->lpProtocol = (LPSTR)((PBYTE)lpProtocolBuffer + *lpdwBufferLength - ((lstrlen(TCP_NAME) + 1) * sizeof(TCHAR)));
        lstrcpy(tcpProtocolInfo->lpProtocol, TCP_NAME);

        udpProtocolInfo = tcpProtocolInfo + 1;
        udpProtocolInfo->lpProtocol = (LPSTR)((PBYTE)tcpProtocolInfo->lpProtocol - ((lstrlen(UDP_NAME) + 1) * sizeof(TCHAR)));
    } else {
        udpProtocolInfo = lpProtocolBuffer;
        udpProtocolInfo->lpProtocol = (LPSTR)((PBYTE)lpProtocolBuffer + *lpdwBufferLength - ((lstrlen(UDP_NAME) + 1) * sizeof(TCHAR)));
    }

    // Fill in UDP info, if requested.
    if (useUdp) {
        udpProtocolInfo->dwServiceFlags = XP_CONNECTIONLESS | XP_MESSAGE_ORIENTED | XP_SUPPORTS_BROADCAST | XP_SUPPORTS_MULTICAST | XP_FRAGMENTATION;
