...
void LogInit()
{
    ...
    if ( fp = fopen( "ws2.log", "r+" ) )
    {
        ...
        fprintf( fp, "****************************************************\n" );
        fprintf( fp, "\tTime: %d:%02d %s\n\tDate: %s %d, 19%d\n", newtime->tm_hour, newtime->tm_min, am_pm, month[newtime->tm_mon], newtime->tm_mday, newtime->tm_year );
        fprintf( fp, "****************************************************\n\n" );
        fclose( fp );
    }
}
...
void CDECL LogIt( char * lpszFormat, ... )
{
    ...
    {   // if we can't open file, do nothing
        fprintf( fp, "%d:%02d %s - %s", newtime->tm_hour, newtime->tm_min, am_pm, szLogStr );
        // KdPrint(( szLogStr ));
        LineCount++;
        fclose( fp );
    }
