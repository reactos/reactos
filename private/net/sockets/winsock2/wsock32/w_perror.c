...
Abstract:
    This module implements the WSA_perror() operation used by the p_error function.
    This is a temporary workaround for beta.
...
int WSA_perror(char *yourmsg, int lerrno)
{
    ...
    if (!(msglen = FormatMessageW(FORMAT_MESSAGE_FROM_HMODULE, (LPVOID)SockModuleHandle, usMsgNum, 0L, perrW, MAX_MSGTABLE, NULL))) {
        return(0);
    }
