...
Abstract:
    This header files contains the common routines used for Name Space support.
...
INT GetProviderList (OUT PTSTR *ProviderList, OUT PDWORD ProviderCount);
BOOL GuidEqual (IN LPGUID Guid1, IN LPGUID Guid2);
INT InitializeNsp (VOID);
VOID InsertNameSpace (IN PNAME_SPACE_INFO NameSpace);
BOOL IsValidNameSpace (IN DWORD dwNameSpace, IN PNAME_SPACE_INFO NameSpace);
INT LoadNspDll (IN PTSTR ProviderName);
INT ReadDefaultExclusions (VOID);
