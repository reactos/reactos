...
Abstract:
    This module contains support for the Name Space Provider APIs GetService() & SetService().
    This is a wrapper that uses the newer enumeration APIs to emulate these 1.1 functions
...
VOID FreeAllocatedClassInfo(BOOL fUnicodeCaller, DWORD dwNumberOf, LPWSANSCLASSINFO lpcli);
// Utility functions

// Do the registry add or delete for a SetService call

INT SSRegistrySettings(
    IN DWORD   dwOperation,
    IN LPTSTR  lpServiceTypeName,
    IN LPGUID  lpServiceType
)
{
    ...
    err = RegCreateKeyEx(HKEY_LOCAL_MACHINE, NSP_SERVICE_KEY_NAME, 0, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKey, &dwDisposition);
    if (err == ERROR_SUCCESS) {
        if (dwOperation == SERVICE_ADD_TYPE) {
            // if adding a type, create the key if need & set a <GUID> value to have a { XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX } representation of the GUID.
            ...
            err = RegCreateKeyEx(hKey, lpServiceTypeName, 0, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hKeyService, &dwDisposition);
            if (err == ERROR_SUCCESS) {
                ...
                if (err == NO_ERROR) {
                    ...
                    if (lpGuidString = ALLOCATE_HEAP((_tcslen(lpTmpString) + 3) * sizeof(TCHAR))) {
                        _tcscpy(lpGuidString, TEXT("{"));
                        _tcscat(lpGuidString, lpTmpString);
                        _tcscat(lpGuidString, TEXT("}"));

                        // write it out
                        err = RegSetValueEx(hKeyService, TEXT("GUID"), 0, REG_SZ, (LPBYTE)lpGuidString, (_tcslen(lpGuidString) + 1) * sizeof(TCHAR));
                ...
                err = RegDeleteKey(hKey, lpServiceTypeName);
            }
        }
...
CSADDR_INFO * SrvAddrsToCsAddrs(PSERVICE_ADDRESSES psa, PDWORD  pdwNumberOfCsAddrs)
{
    ...
    *pdwNumberOfCsAddrs = psa->dwAddressCount;
    ...
    if (pcsa) {
        ...
        // BUGBUG. Don't know what to put in the iProtocol field of the CSADDR_INFO since there is no universal value for the "default" protocol.
        ...
        for (dwX = 0; dwX < psa->dwAddressCount; dwX++) {
            ...
            psocka->lpSockaddr->sa_family = (USHORT)psa->Addresses[dwX].dwAddressType;
            pcsa1++;
        }
    }
...
// And the reverse: convert a CSADDR_INFO structure into a SERVICE_ADDRESSES
PSERVICE_ADDRESSES CsAddrsToSrvAddrs(LPCSADDR_INFO pcsadr, DWORD  dwNumberOfCsAddr)
{
    DWORD  dwTotalSize;
    DWORD  dwX;
    ...
    dwTotalSize = sizeof(SERVICE_ADDRESSES) + (sizeof(SERVICE_ADDRESS) * (dwNumberOfCsAddr - 1));

    psas = (PSERVICE_ADDRESSES)ALLOCATE_HEAP(dwTotalSize);
    if (psas) {
        // The buffer is allocated with the SERVICE_ADDRESSES structure first, followed by the requisite number of SERVICE_ADDRESS structures.
...
// Convert a SERVICE_TYPE_VALUE into a CLASSINFO structure. This is straight-forward requiring only switching around a few values.
// returns a pointer to the structure allocated using LocalAlloc.
LPWSANSCLASSINFO ServiceTypeToClassInfo(BOOL   fUnicodeCaller, DWORD  dwNumberOf, PSERVICE_TYPE_VALUE_ABS pstv)
{
...
#ifdef UNICODE
            if (!fUnicodeCaller) {
                if (AllocateUnicodeString((PCHAR)pstv->lpValueName, &lpci1->lpszName) != NO_ERROR) {
                    FreeAllocatedClassInfo(fUnicodeCaller, dwSaveNumber, lpci);
                    lpci = 0;
                    break;
                }
            } else
...
VOID FreeAllocatedClassInfo(BOOL fUnicodeCaller, DWORD dwNumberOf, LPWSANSCLASSINFO lpci)
{
...
--*/
{
    ...
    // check parameters. majority are checked in the worker, but we need access the structure below to map to Unicode, so we check it here.
    if (!lpServiceInfo) {
    ...
    lpUnicodeServiceInfo = (LPSERVICE_INFOW)ALLOCATE_HEAP(sizeof(SERVICE_INFOW));
    if (!lpUnicodeServiceInfo) {
        goto ExitPoint;
    }

    // allocate the necessary memory for the service name string
    if (err = AllocateUnicodeString(lpServiceInfo->lpServiceName, &lpUnicodeServiceName)) {
        goto ExitPoint;
    }

    // allocate the necessary memory for the service comment string
    if (err = AllocateUnicodeString(lpServiceInfo->lpComment, &lpUnicodeComment)) {
        goto ExitPoint;
    }

    // allocate the necessary memory for the locale string
    if (err = AllocateUnicodeString(lpServiceInfo->lpLocale, &lpUnicodeLocale)) {
        goto ExitPoint;
    }

    // allocate the necessary memory for the machine name string
    if (err = AllocateUnicodeString(lpServiceInfo->lpMachineName, &lpUnicodeMachineName)) {
        goto ExitPoint;
    }
    ...
    lpUnicodeServiceInfo->ServiceSpecificInfo.cbSize = lpServiceInfo->ServiceSpecificInfo.cbSize;
    lpUnicodeServiceInfo->ServiceSpecificInfo.pBlobData = lpServiceInfo->ServiceSpecificInfo.pBlobData;
...
ExitPoint:
    ...
    // if this routine hit an error, set it as last error. else rely on the worker routine to set it.
    if (err) {
        SetLastError(err);
        res = -1;
    }
...
/*
Routine Description:
    Unicode version of SetService. This function is called to set information about a network service.
Arguments:
    dwNameSpace         - Name space affected by the operation. NS_DEFAULT is used to mean all default name spaces.
    dwOperation         - Operation to perform, like SERVICE_REGISTER, SERVICE_DEREGISTER.
    dwFlags             - flags for the opertation, like SERVICE_FLAG_DEFER
    dwServiceInfo       - pointer to SERVICE_INFO structure containing the information to set.
...
--*/
{
    ...
    // call the worker routine
    res = SetServiceWorker(dwNameSpace, dwOperation, dwFlags, lpServiceInfo, lpServiceAsyncInfo,
#if defined(UNICODE)
...
Routine Description:
    ...
    If dwNameSpace is NS_DEFAULT,may return >1 NS_SERVICE_INFO structures, one for each name space.
Arguments:
    See GetServiceW.
Return Value:
    See GetServiceW.
--*/
{
    ...
    // allocate the necessary memory for the service name string
    if (err = AllocateUnicodeString(lpServiceName, &lpUnicodeServiceName)) {
        res = -1;
        goto ExitPoint;
    }
...
Routine Description:
    ...
    If dwNameSpace is NS_DEFAULT,may return >1 NS_SERVICE_INFO structures, one for each name space.
Arguments:
    dwNameSpace         - name spaces to query. if NS_DEFAULT, then all default name spaces.
    ...
    lpBuffer            - return buffer to be filled with NS_SERVICE_INFO structures
    lpdwBufferSize      - on input, size of buffer. used to return required size if buffer too small.
    lpServiceAsyncInfo  - currently must be NULL.
Return Value:
    number of structures returned if successful. zero if no matching structure returned. -1 if an error occured.
--*/
{
    INT res;
...
Arguments:
   As SetServiceW, except that fUnicodeCaller has been added which tells us if the call originated as Unicode or Ansi.
   We need distinguish because of the BLOB data which we cannot interpret.
Return Value:
   Zero if no error. SOCKET_ERROR otherwise. Caller may issue WSAGetLastError() to get more detailed error info.
--*/
{
    DWORD err = NO_ERROR;
    ...
    if (!ARGUMENT_PRESENT(lpServiceInfo) || !ARGUMENT_PRESENT(lpdwStatusFlags)) {
        SetLastError(ERROR_INVALID_PARAMETER);
        return -1;
    }
    ...
    *lpdwStatusFlags = 0;
    ...
    // the only operation where we take explicit action is ADD_TYPE and DELETE_TYPE. everything else gets done by providers.
    if ((dwOperation == SERVICE_ADD_TYPE) || (dwOperation == SERVICE_DELETE_TYPE)) {
        ...
        if ((TmpSvcInfo.ServiceSpecificInfo.cbSize == 0) || (TmpSvcInfo.ServiceSpecificInfo.pBlobData == NULL)) {
            SetLastError(ERROR_INVALID_PARAMETER);
            return -1;
        }
        ...
        lpSvcTypeInfoAbs = (LPSERVICE_TYPE_INFO_ABS)TmpSvcInfo.ServiceSpecificInfo.pBlobData;
#if defined(UNICODE)
        if (fUnicodeCaller) {
#endif
            ...
            err = AllocateUnicodeString(((LPSERVICE_TYPE_INFO_ABSA)lpSvcTypeInfoAbs)->lpTypeName, &lpServiceTypeName);
            if (err != NO_ERROR) {
                SetLastError(err);
                return -1;
            }
        ...
        // We convert this to be self relative before passing on to providers.
        err = PackBlobWorker(lpSvcTypeInfoAbs->lpTypeName,
        ...
        if (err == NO_ERROR) {
            TmpSvcInfo.ServiceSpecificInfo.pBlobData = lpTmpBlobData;
            TmpSvcInfo.ServiceSpecificInfo.cbSize = dwTmpBlobDataSize;
        }
    ...
    // Loop through our name spaces, performing the operation requested. This loop only goes thru more than one iteration if
    ...
    for (listEntry = NameSpaceListHead.Flink; listEntry != &NameSpaceListHead; listEntry = listEntry->Flink) {
        ...
        lpNameSpace = CONTAINING_RECORD(listEntry, NAME_SPACE_INFO, NameSpaceListEntry);
        ...
        error = lpNameSpace->SetServiceProc(dwOperation, dwFlags, fUnicodeCaller, &TmpSvcInfo);
        if (error != NO_ERROR) {
...
Arguments:
   As GetServiceW, except that fUnicodeCaller has been added which tells us if the call originated as Unicode or Ansi.
   We need distinguish because of the BLOB data which we cannot interpret.
Return Value:
   Number of NS_SERVICE_INFO structures obtained if no error.
   SOCKET_ERROR otherwise. Caller may issue WSAGetLastError() to get more detailed error info.
--*/
{
    DWORD          err = NO_ERROR;
    INT            cEntries = 0;
    ...
    if (!ARGUMENT_PRESENT(lpGuid) || !ARGUMENT_PRESENT(lpdwBufferSize) || !ARGUMENT_PRESENT(lpServiceName)) {
        SetLastError(ERROR_INVALID_PARAMETER);
        return -1;
    }
    ...
    // allocate tmp buffer. make the tmp buffer at least as big as user's buffer and no less than 4K.
    ...
    lpEnd = (LPBYTE)ROUND_DOWN_POINTER((LPBYTE)lpBuffer + *lpdwBufferSize, ALIGN_LPDWORD);
    ...
    for (listEntry = NameSpaceListHead.Flink; listEntry != &NameSpaceListHead; listEntry = listEntry->Flink) {
        ...
        error = lpNameSpace->GetServiceProc(lpGuid, (LPWSTR)lpServiceName, dwProperties, fUnicodeCaller, lpServiceInfo, &dwTmpBufferSize);
        if (error != NO_ERROR) {
            SetLastError(error);
        } else {
            err = PackBuffer(lpNameSpace->NameSpace, lpGuid, lpServiceInfo, fUnicodeCaller, lpServiceName, &lpStart, &lpEnd, &dwBytesTooFew);
            if (err == NO_ERROR) {
    ...
    // if the buffer is not big enough, set the number of bytes required and return -1.
    if (dwBytesTooFew) {
    ...
    // no matching provider was called. set the error since no-one else would have done so.
...
--*/
{
    ...
    *lpdwStatusFlags = 0;
    ...
    // Note, this call goes to the wsock32 routine first, so the version must be the 1.1 WINSOCK. Pretty hokey, but that's the way it is.
    ...
    case SERVICE_DELETE_TYPE:
    {
        ...
        if (dwOperation == SERVICE_ADD_TYPE) {
            ...
            lpcli = ServiceTypeToClassInfo(fUnicodeCaller, lpSvcTypeInfoAbs->dwValueCount, lpSvcTypeInfoAbs->Values);
            if (lpcli) {
                ...
                FreeAllocatedClassInfo(fUnicodeCaller, lpSvcTypeInfoAbs->dwValueCount, lpcli);
                if (err != NO_ERROR) {
                    err = GetLastError();
                }
            }
    ...
    // Not ADD_TYPE or DELETE_TYPE. Do the others by issuing a WSASetService call.
    ...
    if (dwOperation == SERVICE_REGISTER) {
        // registration includes addresses that must be coerced from SERVICE_ADDRESSES to CSADDR_INFO.
        pcsadr = SrvAddrsToCsAddrs(lpServiceInfo->lpServiceAddress, &wsaq.dwNumberOfCsAddrs);
    } else {
        pcsadr = 0;
    }
    ...
    // N.B. dwFlags ignored.
    err = WSASetService(&wsaq, dwType, 0);
    if (pcsadr) {
        FREE_HEAP(pcsadr);
    }
...
--*/
{
    ...
    while (err == NO_ERROR) {
        // The lookup  is primed. Fetch actual matches. For each match, construct a SERVICE_INFO and use the 1.1 Packing routines
        ...
        // if it worked, pack it in. Since the allocated buffer is quite large, it is unlikely it overflowed
        if (err == NO_ERROR) {
            // got some. Build a ServiceInfo structure so that the 1.1 Packing routines work
            fGotAtLeastOne = TRUE;

            si.lpServiceAddress = CsAddrsToSrvAddrs(pwsaq->lpcsaBuffer, pwsaq->dwNumberOfCsAddrs);
            ...
            err = PackBuffer(pwsaq->dwNameSpace, lpGuid, &si, fUnicodeCaller, lpServiceName, &lpStart, &lpEnd, &dwBytesTooFew);
...
/*
Routine Description:
   Pack a buffer with fixed size structure at front & variable size data at end.
Arguments:
   ...
   lppEnd           - just past end of buffer. this pointer is moved back before variable data is packed.
   lpdwBytesTooFew  - count of bytes too few. this is always incremented as it may already contain value from earlier calls.
Return Value:
...
--*/
{
    ...
    *lppStart = lpStartVar;
    ...
    // ditto for each of the strings. the PackString function does not return errors. it merely increments BytesTooFew as need.
    PackString(lpServiceInfo->lpComment, lpStartVar, &lpEnd, &lpTmpComment, lpdwBytesTooFew, !fUnicodeCaller);   // convert to ansi if caller not unicode
    PackString(lpServiceInfo->lpLocale, lpStartVar, &lpEnd, &lpTmpLocale, lpdwBytesTooFew, !fUnicodeCaller);   // convert to ansi if caller not unicode
    PackString(lpServiceInfo->lpMachineName, lpStartVar, &lpEnd, &lpTmpMachinename, lpdwBytesTooFew, !fUnicodeCaller);   // convert to ansi if caller not unicode
    PackString(lpServiceName, lpStartVar, &lpEnd, &lpTmpName, lpdwBytesTooFew, !fUnicodeCaller);   // convert to ansi if caller not unicode

    // figure out whether we have enough room for the service address
    if (lpServiceInfo->lpServiceAddress) {
        DWORD AddressCount = lpServiceInfo->lpServiceAddress->dwAddressCount;
        LPSERVICE_ADDRESS lpAddress = &(lpServiceInfo->lpServiceAddress->Addresses[0]);
        LPSERVICE_ADDRESS lpTmpAddress;
        LPSERVICE_ADDRESSES lpTmpServiceAddress;

        // make sure we have enough space for the structure & the variable array at the end
        Length = sizeof(SERVICE_ADDRESSES) + ((AddressCount ? (AddressCount - 1) : 0) * sizeof(SERVICE_ADDRESS));
        if ((lpStartVar + Length) > lpEnd) {
            // not enough space.
            *lpdwBytesTooFew += (DWORD)(ULONG_PTR)(((LPBYTE)lpStartVar + Length) - ((LPBYTE)lpEnd));
            lpEnd = lpStartVar;
        } else {
            lpEnd -= Length;
        ...
        while (AddressCount--) {
            DWORD AddressLength = ROUND_UP_COUNT(lpAddress->dwAddressLength, ALIGN_DWORD);
            DWORD PrincipalLength = ROUND_UP_COUNT(lpAddress->dwPrincipalLength, ALIGN_DWORD);
            if ((lpStartVar + AddressLength + PrincipalLength) > lpEnd) {
                // not enough space.
                *lpdwBytesTooFew += (DWORD)(ULONG_PTR)(((LPBYTE)lpStartVar + AddressLength + PrincipalLength) - (LPBYTE)lpEnd);
                lpEnd = lpStartVar;
            } else {
                ...
                lpEnd -= AddressLength;
                RtlCopyMemory(lpEnd, (LPBYTE)lpAddress->lpAddress, lpAddress->dwAddressLength);
                ...
                if (lpAddress->lpPrincipal) {
                    RtlCopyMemory(lpEnd, (LPBYTE)lpAddress->lpPrincipal, lpAddress->dwPrincipalLength);
        ...
        lpServiceInfoNew->lpServiceAddress = *lpdwBytesTooFew ? NULL : lpTmpServiceAddress;
    }

    // figure out whether we have enough room for the Blob
    if (Length = ROUND_UP_COUNT(lpServiceInfo->ServiceSpecificInfo.cbSize, ALIGN_DWORD)) {
        if ((lpStartVar + Length) > lpEnd) {
            // not enough space.
            *lpdwBytesTooFew += (DWORD)(ULONG_PTR)(((LPBYTE)lpStartVar + Length) - ((LPBYTE)lpEnd));
            lpEnd = lpStartVar;
            ...
            lpEnd -= Length;
            RtlCopyMemory(lpEnd, lpServiceInfo->ServiceSpecificInfo.pBlobData, Length);
            lpServiceInfoNew->ServiceSpecificInfo.cbSize = lpServiceInfo->ServiceSpecificInfo.cbSize;
...
Arguments:
    ...
    lpStartVar       - start of buffer, lppEnd cannot be backed up beyond this.
    lppEnd           - points just past end of buffer. back up before using. if not enough room (ie. gone < lpStartVar) then set to lpStartVar.
    lppDest          - if enough room, on exit set to *lppEnd. If not, set to NULL.
    lpdwBytesTooFew  - count of bytes too few. this is always incremented as it may already contain value from earlier calls.
    ...
    if ((lpStartVar + Length) > *lppEnd) {
        ...
        *lppDest = *lppEnd;
        ...
        if (fConvertToAnsi) {
            ...
            NtStatus = RtlUnicodeStringToAnsiString(&AnsiString, &UnicodeString, FALSE); // dont alloc
...
/*
Routine Description:
    Takes the data describing a service and packs it into a blob.
    It uses an intermdiate structure, SERVICE_TYPE_VALUES_ABS that is easier to construct than the blob-based structure.
    This is called from the external APIs and works with either Unicode or ANSI strings.
Author:
...
--*/
{
    ...
    // N.B. dwType is the type of string. It's also the log(2) of the size that we use as the left-shift value.
    // The index we compute must match the order of the pointers in apfnCopy and apfnLength above.

    // compute size of fixed areas
    Length = pfnLength(lpTypeName);
    Length = ROUND_UP_COUNT((Length + 1) << dwType, ALIGN_DWORD);
    dwSize = Length + sizeof(SERVICE_TYPE_INFO) + (sizeof(SERVICE_TYPE_VALUE) * (dwValueCount - 1));

    // Now we have to compute the size needed by the type names and values.
    for (dwIndex = 0, pv = Values; dwIndex < dwValueCount; dwIndex++, pv++) {
        DWORD dwSS;

        dwSS = pfnLength(pv->lpValueName);
        dwSize += ROUND_UP_COUNT(pv->dwValueSize, ALIGN_DWORD) + ROUND_UP_COUNT((dwSS + 1) << dwType, ALIGN_DWORD);
    }

    // got the total needed size.
    *lpdBufferSize = dwSize;    // assume we succeed

    pBuffer = (PBYTE)ALLOCATE_HEAP(dwSize);
    if (!pBuffer) {
        return(GetLastError());
    }
    ...
    *lppBuffer = pBuffer;
    ...
    psi = (PSERVICE_TYPE_INFO)pBuffer;// The SERVICE_TYPE_INFO starts at the beginning.
    pv1 = (PSERVICE_TYPE_VALUE)&psi->Values[0];// Find the first SERVICE_TYPE_VALUE
    ptr = (PBYTE)(pv1 + dwValueCount);// now the string space. It is after all of the structures

    // fill in the SERVICE_TYPE_INFO
    psi->dwTypeNameOffset = (DWORD)(ULONG_PTR)(ptr - pBuffer);
    psi->dwValueCount = dwValueCount;

    pfnCopy(ptr, lpTypeName);// copy the service name into the string space.

    ptr += Length;

    // now do the service type values
    for (dwIndex = 0, pv = Values; dwIndex < dwValueCount; dwIndex++, pv++, pv1++) {
        ...
        // We already computed this, it's too bad we couldn't have saved it.
        // On the assumption there aren't very many of these to do, this wasteful immplementation is probably a good trade-off, however.
        // The only way to save them, 
        // and to make it extensible, would have been to allocate a buffer of the proper size and use that as an arrary of Length values.
        // This would have been worth it if we believe that callers had enough values to marshall to make it worthwhile.
