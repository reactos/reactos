...
BOOLEAN SockEnterApi(IN BOOLEAN MustBeStarted, IN BOOLEAN BlockingIllegal, IN BOOLEAN GetXByYCall)
{
...
VOID WsAssert(IN PVOID FailedAssertion, IN PVOID FileName, IN ULONG LineNumber)
{
    BOOL ok;
    CHAR choice[16];
    DWORD bytes;
    ...
    IF_DEBUG(CONSOLE)
    {
        ...
        do {
            ...
            bytes = sizeof(choice);
            ok = ReadFile(GetStdHandle(STD_INPUT_HANDLE), &choice, bytes, &bytes, NULL);
            if (ok) {
...
VOID WsPrintf(char *Format, ...)
{
    ...
    IF_DEBUG(CONSOLE)
    {
        ...
        ret = WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), (LPVOID)OutputBuffer, length, &length, NULL);
        if (!ret) {
    ...
    IF_DEBUG(FILE)
    {
        if (DebugFileHandle == INVALID_HANDLE_VALUE) {
            DebugFileHandle = CreateFile(DebugFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, 0, NULL);
        }
        ...
        if (DebugFileHandle == INVALID_HANDLE_VALUE) {
            ...
            length = strlen(OutputBuffer);
            ret = WriteFile(DebugFileHandle, (LPVOID)OutputBuffer, length, &length, NULL);
            if (!ret) {
...
VOID WsEnterApiCall(IN PCHAR RoutineName, IN PVOID Arg1, IN PVOID Arg2, IN PVOID Arg3, IN PVOID Arg4)
{
    ULONG i;
    ...
    // duplicated in SockEnterApi(), but we need it here to access SockIndentLevel below.
    if (GET_THREAD_DATA() == NULL) {
        if (SockProcessTerminating || !SockThreadInitialize()) {
            return;
        }
    }
...
PCHAR WsGetErrorString(IN INT Error)
{
    INT i;
...
VOID WsExitApiCall(IN PCHAR RoutineName, IN INT ReturnValue, IN BOOLEAN Failed)
{
    ULONG i;
...
VOID SockInitializeDebugData(VOID)
{
...
PVOID SockAllocateHeap(IN ULONG NumberOfBytes, PCHAR FileName, ULONG LineNumber)
{
#ifdef CHICAGO
    ...
    header = RtlAllocateHeap(SockPrivateHeap, 0, NumberOfBytes + sizeof(*header) + sizeof(*tail));
    if (header == NULL) {
...
VOID SockFreeHeap(IN PVOID Pointer)
{
#ifdef CHICAGO
    LocalFree(Pointer);
#else
...
VOID SockCheckHeap(VOID)
{
    ...
    for (listEntry = SockHeapListHead.Flink; listEntry != &SockHeapListHead; listEntry = listEntry->Flink) {
        if (listEntry == NULL) {
...
VOID SockAcquireGlobalLockExclusive(VOID)
{
    WS_ASSERT(!SockProcessTerminating);
...
VOID SockReleaseGlobalLock(VOID)
{
    WS_ASSERT(!SockProcessTerminating);
...
BOOL SockThreadInitialize(VOID)
{
