...
PWINSOCK_CONTEXT_BLOCK SockAllocateContextBlock (VOID);
BOOLEAN SockCheckAndInitAsyncThread (VOID);
VOID SockFreeContextBlock (IN PWINSOCK_CONTEXT_BLOCK ContextBlock);
VOID SockQueueRequestToAsyncThread(IN PWINSOCK_CONTEXT_BLOCK ContextBlock);
VOID SockProcessAsyncGetHost (
    IN DWORD TaskHandle,
    IN DWORD OpCode,
    IN HWND hWnd,
...
BOOL SockBlockingHookHelper(DWORD Context);
...
int dn_expand(IN  unsigned char *msg, IN  unsigned char *eomorig, IN  unsigned char *comp_dn, OUT unsigned char *exp_dn, IN  int length);
...
struct hostent * localhostent_W (void);
...
ULONG SockNbtResolveName (IN  PCHAR    Name, OUT WORD *   pwIpCount OPTIONAL, OUT PULONG * pIpArray  OPTIONAL);
PHOSTENT QueryHostentCache (IN LPSTR Name OPTIONAL, IN DWORD IpAddress OPTIONAL);

// DNS Caching Resolver Service helper routine
struct hostent * QueryDnsCache(IN  LPSTR  pszName, IN  WORD wType, IN  DWORD Options, IN  PVOID * pMsg OPTIONAL);
struct hostent * QueryDnsCache_W(IN  LPWSTR pszName, IN  WORD wType, IN  DWORD Options, IN  PVOID * pMsg OPTIONAL);
struct hostent * QueryNbtWins(IN  LPSTR pszOemName, IN  LPSTR pszAnsiName);
