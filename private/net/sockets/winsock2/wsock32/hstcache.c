...
VOID CacheHostent (IN PHOSTENT HostEntry, IN INT Ttl);
...
VOID CacheHostent (IN PHOSTENT HostEntry, IN INT Ttl)
{
    ...
    CopyHostentToBuffer((char FAR *)&cacheEntry->HostEntry, bytesRequired - sizeof(*cacheEntry), HostEntry);

    // Acquire the global lock exclusively to protect our lists and counts, and test whether we're at the limit of the caching we'll do.
    SockAcquireGlobalLockExclusive( );

    if ( CurrentHostentCacheSize < MaxHostentCacheSize ) {
        CurrentHostentCacheSize++;
    } else {
        // We're at our limit for cached entries.  Remove the oldest entry from the cache.
        oldestCacheEntry = NULL;

        for ( listEntry = HostentCacheListHead.Flink; listEntry != &HostentCacheListHead; listEntry = listEntry->Flink ) {
            testCacheEntry = CONTAINING_RECORD(listEntry, HOSTENT_CACHE_ENTRY, HostentListEntry);
            if ( oldestCacheEntry == NULL || QUADPART(testCacheEntry->LastAccessTime) < QUADPART(oldestCacheEntry->LastAccessTime) ) {
...
PHOSTENT QueryHostentCache (IN LPSTR Name OPTIONAL, IN DWORD IpAddress OPTIONAL)
{
    ...
    // *** It is assumed that this routine is called while the caller holds the appropriate global cache lock!

    // First get the current system time.  We'll use this to reset the LastAccessTime if we find a hit.
    ...
    // Walk the host entry cache.  As soon as we find a match, quit walking the list.
    for ( listEntry = HostentCacheListHead.Flink; listEntry != &HostentCacheListHead; ) {
        testCacheEntry = CONTAINING_RECORD(listEntry, HOSTENT_CACHE_ENTRY, HostentListEntry );
        ...
        // First check for a matching name.  A match on either the primary name or any of the aliases results in a hit.
        ...
        // Now check for a match against any of the IP addresses in the hostent.
        if ( IpAddress != 0 ) {
