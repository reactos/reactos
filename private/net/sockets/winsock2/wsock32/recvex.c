...
/*++
Routine Description:
    This is an extended API to allow better Windows Sockets support over message-based transports.
    It is identical to recv() except that the ReceiveFlags parameter is an IN-OUT parameter that sets MSG_PARTIAL is the TDI provider returns STATUS_RECEIVE_PARTIAL, STATUS_RECEIVE_PARTIAL_EXPEDITED or STATUS_BUFFER_OVERFLOW.
Arguments:
...
--*/
{
    ...
    *ReceiveFlags = 0;
    ...
    result = WSARecv(Handle,&wsaBuf,1,&bytesRead,ReceiveFlags,NULL,NULL);
    if( result == 0 ) {
        // Success.
        ...
        // Partial message received. The provider will return a negative bytesRead length. Not anymore.
        result = (int)bytesRead;
