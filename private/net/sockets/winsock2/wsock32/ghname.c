...
Abstract:
    This module implements routines to set and retrieve the host's TCP/IP network name.
...
int _pgethostname(OUT char *name, IN int namelen);
...
int _pgethostname(OUT char *name, IN int namelen)
{
    ...
    // Do not return the fully qualified name--returning this much info breaks some poorly written applications.
    ...
    if (!NT_SUCCESS(status) || (strlen(temp) == 0)) {
        status = SockGetSingleValue(myKey, "DhcpDomain", temp, &myType, HOSTDB_SIZE);
    }
...
// BUGBUG - this function may only be performed by the sys manager.
//          Must add security. Actually, this function will probably go away since the NCAP will take care of it.