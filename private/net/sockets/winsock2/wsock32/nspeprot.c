...
Abstract:
    This module contains support for the Name Space Provider API EnumProtocols().
...
INT SockLoadTransportList (OUT PTSTR *TransportList);

#if defined(CHICAGO)
#include <wsahelp.h>

PTSTR KludgeMultiSz(HKEY hkey, LPDWORD lpdwLength);
#endif


INT APIENTRY EnumProtocols(
    ...
    )
{
    ...
    for ( currentTransport = transportList; *currentTransport != UNICODE_NULL; currentTransport += _tcslen( currentTransport ) + 1 ) {
        ...
        error = RegOpenKeyEx(HKEY_LOCAL_MACHINE,winsockKeyName,0,KEY_READ,&winsockKey);
        ...
        error = RegQueryValueEx(winsockKey,TEXT("HelperDllName"),NULL,&type,(PVOID)helperDllName,&entryLength);
        if ( error != NO_ERROR ) {
            FREE_HEAP( transportList );
            FREE_HEAP( helperDllName );
            FREE_HEAP( helperDllExpandedName );
        ...
        helperDllExpandedNameLength = ExpandEnvironmentStrings(helperDllName,helperDllExpandedName,DOS_MAX_PATH_LENGTH);
        FREE_HEAP( helperDllName );
        ...
        // Get the addresses of the WSHEnumProtocols entry point for the helper DLL.  If the helper DLL does not export this routine, just skip it.
        enumProtocols = (PWSH_ENUM_PROTOCOLS)GetProcAddress( dllHandle, "WSHEnumProtocols" );
        if ( enumProtocols == NULL ) {
            FreeLibrary( dllHandle );
            continue;
        }

        // Calculate how many bytes are still available in the user buffer.
        ...
        // Call the helper DLL so that it fills in information about the transport protocols which it supports.
        count = enumProtocols(lpiProtocols,currentTransport,lpProtocolBuffer,&bytesLeft);
        ...
        // If the helper DLL returned any entries then fix up pointers and counts.
        if ( count > 0 ) {
            totalCount += count;
            lpProtocolBuffer = (PBYTE)lpProtocolBuffer + (count * sizeof(PROTOCOL_INFO));
        }
    }
...
INT SockLoadTransportList (OUT PTSTR *TransportList)
{
    ...
    // Open the key that stores the list of transports that support winsock.
    error = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Services\\Winsock\\Parameters"),0,KEY_READ,&winsockKey);
    if ( error != NO_ERROR ) {
        return error;
    }
    ...
    *TransportList = KludgeMultiSz(winsockKey, &transportListLength);
    ...
    // Determine the size of the mapping.  We need this so that we can allocate enough memory to hold it.
    transportListLength = 0;
    error = RegQueryValueEx(winsockKey,TEXT("Transports"),NULL,&type,NULL,&transportListLength);
    ...
    *TransportList = ALLOCATE_HEAP( transportListLength );
    ...
    error = RegQueryValueEx(winsockKey,TEXT("Transports"),NULL,&type,(PVOID)*TransportList,&transportListLength);
    if ( error != NO_ERROR ) {
        RegCloseKey( winsockKey );
        FREE_HEAP( *TransportList );
        return error;
...
#endif  // !CHICAGO
    // It worked!  The caller is responsible for freeing the memory allocated to hold the list.
    RegCloseKey( winsockKey );
