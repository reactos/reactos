...
BOOL SockInitialize (IN PVOID DllHandle, IN ULONG Reason, IN PVOID Context OPTIONAL)
{
    ...
    // On a thread detach, set up the context param so that all necessary deallocations will occur.
    ...
    case DLL_PROCESS_ATTACH:
...
#if DBG
        // If there is a file in the current directory called "wsdebug" open it and read the first line to set the debugging flags.
        {
            HANDLE handle;

            handle = CreateFile("WsDebug", GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
        ...
        // here rather than statically because it otherwise won't be thunked correctly.
        SockPostRoutine = PostMessage;

        // ** lock acquisition order: it is legal to acquire SocketLock while holding an individual socket lock, but not the other way around!
...
#if !defined(USE_TEB_FIELD)
        // Allocate space in TLS so that we can convert global variables to thread variables.
        SockTlsSlot = TlsAlloc( );
        if ( SockTlsSlot == 0xFFFFFFFF ) {
            WS_PRINT(( "SockInitialize: TlsAlloc failed: %ld\n", GetLastError( ) ));
            // Don't delete critical sections here, a call to SockInitialize will be made where these are freed then.
            return FALSE;
        }
#endif  // !USE_TEB_FIELD

        // Create private WinSock heap on MP machines.  UP machines just use the process heap.
        ...
        if ( SockPrivateHeap == NULL ) {
            // This is either a UP box, or RtlCreateHeap() failed.  In either case, just use the process heap.
    ...
    case DLL_PROCESS_DETACH:
        ...
        // Only clean up resources if we're being called because of a FreeLibrary().
        // If this is because of process termination, do not clean up, as the system will do it for us.
        // Also, if we get called at process termination, it is likely that a thread was terminated while it held a winsock lock, which would cause a deadlock if we then tried to grab the lock.
        if ( Context == NULL )
        {
            GetHostCleanup();
    ...
    case DLL_THREAD_DETACH:
        ...
        // If the TLS information for this thread has been initialized, free the thread data buffer.
        if ( Context == NULL && GET_THREAD_DATA() != NULL )
        {
            if ( SockBigBufSize > 0 )
            {
