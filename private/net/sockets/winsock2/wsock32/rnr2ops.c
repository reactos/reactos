...
// If a single NS_DNS is desired to do all of these, don't define a provider for the others and the NS_DNS provider will consider NS_ALL to include the others.
// If, however, other providers exist, then the code will the explicitly correct thing.
// All that is needed is to fill in the provider IDs for NbtProviderId and LclProviderId.
...
INT WINAPI NSPSetService(IN  LPGUID lpProviderId,
                         IN  LPWSASERVICECLASSINFOW  lpServiceClassInfo,
...
LPSTR GetAnsiNameRnR(IN  PWCHAR Name, IN  LPSTR  Domain, OUT PBOOL  pfConversionFailed);
LPSTR GetOemNameRnR(IN  PWCHAR Name, OUT PBOOL  pfConversionFailed);
...
// The provider Ids. Each of the providers implemented in this DLL is defined below. These match the GUIDs that setup puts in the registry.
...
// the following three functions are used to determine which provider action this should honor.
// It allows there to be but one provider to handle all of the actions or distinct providers.
BOOL DoDnsProvider(PDNS_RNR_CONTEXT pdrc)
{
    WS2LOG_F1("rnr2ops.c - DoDnsProvider");

    if (!MaskOfGuids || GuidEqual(&pdrc->gdProviderId, &DNSProviderId)) {
        return(TRUE);
    }
...
BOOL DoNbtProvider(PDNS_RNR_CONTEXT pdrc, PCHAR pszName)
{
    WS2LOG_F1("rnr2ops.c - DoNbtProvider");

    if (!pdrc->DnsRR && (!MaskOfGuids || GuidEqual(&pdrc->gdProviderId, &NbtProviderId)) && (pdrc->dwNameSpace == NS_ALL)) {
...
BOOL DoLclProvider(PDNS_RNR_CONTEXT pdrc)
{
    WS2LOG_F1("rnr2ops.c - DoLclProvider");

    if (!pdrc->DnsRR && DoDnsProvider(pdrc)) {
        return(TRUE);
    }
...
WORD GetDnsQueryTypeFromGuid(IN  LPGUID gdType)
{
    if (GuidEqual(gdType, &HostnameGuid) || GuidEqual(gdType, &InetHostName))
        return DNS_TYPE_A;
...
WORD GetDnsQueryTypeFromGuid(IN      LPGUID          pGuid)
/*
Routine Description:
...
--*/
{
    ...
    //  note, since defaulting to type A, there's no need to specifically check for standard GUIDs for Hostname lookup

    //  if ( GuidEqual( pGuid, &HostnameGuid ) || GuidEqual( pGuid, &InetHostName ) )
    //  {
    ...
    //  with NT5 GUIDs are available for querying specific DNS types (see svcguid.h);  catch those GUIDs here;
    //  note that encoding of these GUIDs is SVCID_TCP with port==53 and RR set for query type
    ...
    //  two reasons here
    //      1) glenn has no error return checking in any of the calls to this function and we don't want to go to the wire with type==0
    //      2) apparently some apps do call WSALookupServiceNext with a GUID for some service (eg. FTP), instead of having the app call with a name resolution GUID;
    //         these services seemed to be able to get away with this in NT4, which means we somehow hit the wire with type==A query
    return(wtype);
}
...
Routine Description:
   ...
   pfHaveGlobalLock -- BOOL indicating whether the name was found in the cache and therefore the lock is held
...
--*/
{
    ...
    if (DoDnsProvider(pdrc)) {
        ...
        if (dwControlFlags & LUP_FLUSHCACHE)
            hostEntry = QueryDnsCache(pszName, wType, DNS_QUERY_BYPASS_CACHE, ppMsg);
        else
            hostEntry = QueryDnsCache(pszName, wType, (ppMsg != 0) ? DNS_QUERY_BYPASS_CACHE : 0, ppMsg);
        ...
        if (!hostEntry && pdrc->blAnswer.pBlobData && (pdrc->dwControlFlags & LUP_RETURN_BLOB)) {
            // getanswer failed, but the caller wants the raw answer. So only
            // return that, but also conjure a valid hostEntry for later on
...
)
{
    ...
    if (lpszString && lpType && (GuidEqual(lpType, &HostnameGuid) || GuidEqual(lpType, &InetHostName))) {
        PCHAR   servname;
        PCHAR   protocolname;
        WCHAR   wszTemp[1000];
        INT     port = 0;
...
DWORD PackCsAddr(IN PHOSTENT HostEntry,
                 IN DWORD UdpPort,
                 IN DWORD TcpPort,
                 IN OUT LPVOID lpCsaddrBuffer,
                 IN OUT PLONG lplBufferLength,
                 ...
                 BOOL fReversi)
{
    ...
    if (fATMAddr) {
        requiredBufferLength = count * (sizeof(CSADDR_INFO) + 2 * sizeof(SOCKADDR_ATM));
    } else if (fIPv6Addr) {
        requiredBufferLength = count * (sizeof(CSADDR_INFO) + 2 * sizeof(SOCKADDR_IN6));
    } else {
    ...
    *lpdwBytesTaken = requiredBufferLength;
    ...
    while (nAddresses--) {
        ...
        for (i = 0; i < count; i++) {
            ...
            // all zeros except for the family so that it is a "wildcard" address for binding.
...
LPSTR GetAnsiNameRnR(IN  PWCHAR Name, IN  LPSTR  Domain, OUT PBOOL  pfConversionFailed)
{
    ...
    if (!WideCharToMultiByte(CP_ACP, 0, Name, -1, pszBuffer, dwLen, 0, pfConversionFailed)) {
        FREE_HEAP(pszBuffer);
        return NULL;
    }
...
LPSTR GetOemNameRnR(IN  PWCHAR Name, OUT PBOOL  pfConversionFailed)
{
    ...
    if (!WideCharToMultiByte(CP_OEMCP, 0, pszTemp, -1, pszBuffer, dwLen, 0, pfConversionFailed)) {
        FREE_HEAP(pszBuffer);
        FREE_HEAP(pszTemp);
        return NULL;
    }
...
--*/
{
    ...
    if (dwPort == NOPORTDEFINED) {
        // this was taken out because there was no time to test it for NT4
#if REGISTRY_WORKS
...
--*/
{
    ...
    if ((dwControlFlags & ~ALL_LUP_FLAGS) || (dwControlFlags & (LUP_CONTAINERS | LUP_NOCONTAINERS) == (LUP_CONTAINERS | LUP_NOCONTAINERS))) {
        SetLastError(WSA_INVALID_PARAMETER);
        return(SOCKET_ERROR);
    }

    // Check that no context is specified.
    if ((lpqsRestrictions->lpszContext && (lpqsRestrictions->lpszContext[0] != 0) && wcscmp(&lpqsRestrictions->lpszContext[0], L"\\")) ||
    ...
    // If this is a lookup of the local name allow the instance name to be NULL.
    // If it is a reverse lookup, mark it as such.
    ...
    if (!pwszServiceName || !pwszServiceName[0]) {
        ...
        if (DnsNameCompare_W(pwszServiceName, L"localhost") || DnsNameCompare_W(pwszServiceName, L"loopback")) {
            dwLocalFlags |= LOCALLOOK | LOOPLOOK;
        } else if (DnsNameCompare_W(pwszServiceName, szLocalComputerName) || DnsNameCompare_W(pwszServiceName, pszFullName)) {
            dwLocalFlags |= LOCALLOOK;
        }
    }
    ...
    // Complete computation of protocols.
    // We might have information in the query string that helps us, and if so, we need to fetch the servent for that specification.
    nProt &= GetServerAndProtocolsFromString(lpqsRestrictions->lpszQueryString, lpqsRestrictions->lpServiceClassId, &sent);
    if (sent) {
        if (nProt & UDP_BIT) {
    ...
    *lphLookup = (HANDLE)pdrc;
    ...
    if (IS_SVCID_TCP(lpqsRestrictions->lpServiceClassId) || IS_SVCID_UDP(lpqsRestrictions->lpServiceClassId)) {
        pdrc->DnsRR = RR_FROM_SVCID(lpqsRestrictions->lpServiceClassId);

        // BUGBUG. Should a query value of T_A be ignored? Note that specifying T_A will cause all of the local optimizations
        // to be bypassed. Hence, it seems as if it is a good idea to ingnore it. What is the harm?
        if (pdrc->DnsRR == T_A) {
            pdrc->DnsRR = 0;
        }
    }

    // The following is no longer needed since the dwControlFlag option LUP_FLUSHCACHE is handled in the LookupServiceNext routine.
...
)
{
    ...
    // GBUGBUG - WinSock2 UNICODE support in this module was poorly implemented in earlier versions of NT.
    // Calling GetAnsiNameRnR and doing all name resolution as ANSI and using hostent structures does not allow for easy support of UNICODE.
    // This problem was discovered late in NT5.

    // The work around code change is to specifically plumb a call to DNSQuery_W and return the found record information from DNS.
    // If the query was for some other type of look up such as LOOPLOOK, LOCALLOOK, LUP_RES_SERVICE, IANALOOK, REVERSELOOK, we will
    // fail the LookupServiceNext call since we can't do anything with the pAnsiName since it's translation from UNICODE is wrong.
    if (fUnicodeQuery) {
        if ((pdrc->fFlags & IANALOOK) || (pdrc->dwControlFlags & LUP_RES_SERVICE) || (pdrc->fFlags & LOOPLOOK) || (pdrc->fFlags & REVERSELOOK)) {
            FREE_HEAP(pAnsiName);
            pAnsiName = NULL;

            // None of these types of queries are supported for service query names that are UNICODE that don't map
            // to ANSI because the particular foreign language code page is not installed
            err = WSA_E_NO_MORE;
            goto Done;
        }
    ...
    // After all these years, it turns out that the NBT driver is expecting OEM strings for the name lookup calls.
    ...
    if (pdrc->fFlags & IANALOOK) {
        ...
        nProt = GetServerAndProtocolsFromString(pdrc->wcName, &HostnameGuid, &sent);
        ...
        if (pdrc->dwControlFlags & LUP_RES_SERVICE) {
            ...
            if (pdrc->fFlags & REVERSELOOK) {
                ...
                if (dwIp == INADDR_ANY) {
                    ...
                    if (!IsLocalQuery(hostEntry, dwIp, NULL)) {
                        hostEntry = _pgethostbyaddr((PCHAR)&dwIp, 4, AF_INET, pdrc->dwControlFlags, g_NbtFirst, &fUnicodeQuery);
                    }
                }
                ...
                if (fUnicodeQuery) {
                    ...
                    if (wType != DNS_TYPE_A &&
                        ...
                        // The IPv6 guys still want raw wire bytes instead of the parsed hostent.
                        wType != DNS_TYPE_PTR) {
                        ppMsg = &pMsg;
                    }

                    // If this is a query for a name that could not be converted to the ANSI code page (cleanly),
                    // because the specific code page is not installed on the current system.
                    // Do a pure UNICODE DNS query with the origical wide char string . . .

                    // Perform query through DNS Caching Resolver Service
                    if (pdrc->dwControlFlags & LUP_FLUSHCACHE) {
                        hostEntry = QueryDnsCache_W(pdrc->wcName, wType, DNS_QUERY_BYPASS_CACHE, ppMsg);
                    } else {
                        hostEntry = QueryDnsCache_W(pdrc->wcName, wType, (ppMsg != 0) ? DNS_QUERY_BYPASS_CACHE : 0, ppMsg);
                    }
                ...
                if (!hostEntry && !fUnicodeQuery) {
                    ...
                    // local didn't find it. See if NBT can find it Depending on g_NbtFirst, try Nbt first or DNS first.

                    do {
                        if ((dwNbtFirst & DONBTFIRST) && pOemName) {
                            TryNbt(pdrc, &hostEntry, pOemName, pAnsiName);
                        } else {
                            TryDns(pdrc, &hostEntry, pAnsiName, pdrc->dwControlFlags, &fHaveGlobalLock);
                        }
                    } while (!hostEntry && ((dwNbtFirst ^= DONBTFIRST) != (g_NbtFirst & DONBTFIRST)));
                }
            } else {
                ...
                if (pdrc->fFlags & LOOPLOOK) {
                    ...
                    // this code will return the machine's DNS addresses when the lookup is on a NULL name and the port is the DNS port.
                    if (!*pAnsiName && ((pdrc->dwUdpPort == DNS_PORT) || (pdrc->dwTcpPort == DNS_PORT))) {
                        // special DNS address-list hack
                        hostEntry = dnshostent();
                    } else {
                        if ((GuidEqual(&pdrc->gdType, &InetHostName) && !(pdrc->dwControlFlags & LUP_RETURN_BLOB)) || GuidEqual(&pdrc->gdType, &HostnameGuid)) {
                            fUnicodeQuery = TRUE;
                            hostEntry = myhostent_W();

                            if (hostEntry && fAllowGetHostNameHack) {
                                if (GuidEqual(&pdrc->gdType, &HostnameGuid) || (GuidEqual(&pdrc->gdType, &InetHostName) && !*pdrc->wcName)) {
                                    LPWSTR pszDot;
                                    // this is a gethostname call.
                                    // So return the machine name part only.
                                    // Do this by copying the machine-name prefix to a local.
                                    // It really doesn't matter since just below the hostEntry is copied and saved in the lookup context so we don't care about what we got back from myhostent.
                                    pszDot = wcschr((LPWSTR)hostEntry->h_name, L'.');
                                    if (pszDot) {
                                        DWORD dwDot = (DWORD)(pszDot - (LPWSTR)hostEntry->h_name);

                                        memcpy(szComputerName, hostEntry->h_name, dwDot * sizeof(WCHAR));
                                        szComputerName[dwDot] = 0;
                                        hostEntry->h_name = (LPSTR)szComputerName;
                                    }

                                    // Don't return any aliases with a HOSTNAME query.
                            ...
                            if (hostEntry && fAllowGetHostNameHack) {
                                if (GuidEqual(&pdrc->gdType, &HostnameGuid) || (GuidEqual(&pdrc->gdType, &InetHostName) && !*pdrc->wcName)) {
                                    ...
                                    if (pszDot) {
                                        DWORD dwDot = (DWORD)(pszDot - hostEntry->h_name);

                                        memcpy((LPSTR)szComputerName, hostEntry->h_name, dwDot * sizeof(char));
            ...
            if (hostEntry) {
                ...
                pdrc->phent = hostEntry = CopyHostEntry(hostEntry, 0, 0, &lSizeOf, FALSE, fUnicodeQuery);
    ...
    if ((pdrc->dwControlFlags & LUP_RETURN_BLOB) && !fUnicodeQuery) {
        if (GuidEqual(&pdrc->gdType, &InetHostName) || GuidEqual(&pdrc->gdType, &AddressGuid) || GuidEqual(&pdrc->gdType, &AtmaGuid)) {
            ...
            if (lFree >= 0) {
                ...
                memcpy(pBuff, pdrc->blAnswer.pBlobData, pdrc->blAnswer.cbSize);
            } else {
                err = WSAEFAULT;
            }
        }
...
INT WINAPI NSPStartup(IN     LPGUID        lpProviderId, IN OUT LPNSP_ROUTINE lpsnpRoutines)
{
    ...
    if (!szLocalComputerName[0]) {
        ...
        if (!myent) {
        Error:
            // if we can't get the local computer name or the local host entry, something is very amiss and we should refuse to load.
            if (!GetLastError()) {
        ...
        *pszDest = 0;
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TCPKEY, 0, KEY_QUERY_VALUE, &Key) == NO_ERROR) {
            DWORD dwSize = sizeof(g_NbtFirst);
            DWORD dwType;

            RegQueryValueEx(Key, LOOKUPORDER, 0, &dwType, (PCHAR)&g_NbtFirst, &dwSize);
            RegCloseKey(Key);
        }
    }
    ...
    RtlCopyMemory(lpsnpRoutines, &nsrVector, dwSize);
...
// lSizeOf -- if pbAllocated in non-NULL, the size of the memory at that address
// plTake -- a pointer to a LONG where the amount of memory needed to make a copy of phent is returned.
...
)
{
    ...
    dwSize = sizeof(struct hostent) + GetAliasSize(phent->h_aliases, &dwAlCount, fUnicode) + GetAddrSize(phent, &dwAddCount) + dwNameLen + 1;

    if (!(pb = pbAllocated)) {
        pb = (PBYTE)AllocLocal(dwSize);
    } else {
        // align it first.
        // This is done to insure that if this space is within another buffer, as it will be when returning a BLOB through a LoookupServiceNext,
        // that the buffer is left aligned to hold a string.
...
struct servent * CopyServEntry(struct servent * sent, PBYTE pbAllocated, LONG lSizeOf, PLONG plTaken, BOOL fOffsets)
{
    ...
    dwSize = sizeof(struct servent) + GetAliasSize(sent->s_aliases, &dwAlCount, FALSE) + dwNameSize + strlen(sent->s_proto) + 1;
...
// The following is not used, but is here. It was implemented for NT 4 but too late to make it into the release.
...
)
{
    ...
    if (err == NO_ERROR) {
        ...
        err = RegOpenKeyEx(serviceTypesKey, lpServiceName, 0, KEY_READ, &serviceKey);
        ...
        if (err == NO_ERROR) {
            ...
            err = RegQueryValueEx(serviceKey, TEXT("GUID"), NULL, &type, (PVOID)guidString, &length);
...
)
{
    ...
    if (err == NO_ERROR) {
        ...
        while ((err = RegEnumKeyEx(serviceTypesKey, keyIndex, serviceName, &nameLength, NULL, NULL, NULL, &lastWriteTime) == NO_ERROR)) {
            // Get the Guid for this service type. This is pretty lazy but it makes it modular
            ...
            if (err == NO_ERROR) {
                ...
                if (GuidEqual(lpServiceType, &guid)) {
                    ...
                    // We have a match.  See if we can find the desired port
                    err = RegOpenKeyEx(serviceTypesKey, serviceName, 0, KEY_READ, &key1);
                    RegCloseKey(serviceTypesKey);
                    if (err == NO_ERROR) {
                        LPTSTR pwszValue;
                        DWORD type;
                        ...
                        err = RegQueryValueEx(key1, pwszValue, NULL, &type, (PVOID)&dwPort, &length);
                        RegCloseKey(key1);
                        if (err == NO_ERROR) {
                            return dwPort;
                        }
...
INT RnR2AddServiceType(IN  PWSASERVICECLASSINFOW psci)
{
    ...
    if (!err) {
        ...
        // write it
        err = RegSetValueEx(hKeyService, TEXT("GUID"), 0, REG_SZ, (LPBYTE)wszUuid, 39 * sizeof(TCHAR));
        if (!err) {
            ...
            while (dwCount--) {
                if (pci->dwNameSpace == NS_DNS) {
                    // it's ours
                    err = RegSetValueEx(hKeyService, pci->lpszName, 0, pci->dwValueType, (LPBYTE)pci->lpValue, pci->dwValueSize);
                    if (err) {
                        break;
                    }
                }
