...
Abstract:
    This module contains support for the Name Space Provider API GetAddressByName().
...
INT ConvertProtocols(IN LPINT lpProtocols, OUT PDWORD pdwNumberOfAddresses, OUT LPAFPROTOCOLS * lpafpProtocols)
{
    /*++
    Routine Description:
        Given a pointer to a 1.1 list of protocol restrictions, create a 2.0 corresponding list.
        If successful, allocate memory to hold the list.
...
)
{
    ...
    count = GetAddressByNameW(dwNameSpace,
                              lpServiceType,
                              unicodeServiceName,
                              lpiProtocols,
                              dwResolution,
    ...
    if (count > 0 && ARGUMENT_PRESENT(lpAliasBuffer)) {
        ...
        // Put in the final double zero-terminator and set up the alias buffer length.
        *a = '\0';
...
)
{
    ...
    // If the caller is asking for an asynchronous request, fail since we do not yet support async.
...
#if OLDXBYY
    // If our NSP internal structures have not yet been initialized, initialize them now.
    if (!NspInitialized) {
        INT error;
        IF_DEBUG(RESOLVER)
        {
...
)
{
    ...
    // Determine whether we should find a single address or many.
    // If the latter we'll loop through all name spaces; if the former, we'll search in only a single name space.
    ...
    // Loop through our name spaces attempting to resolve the service name into addresses.
    for (listEntry = NameSpaceListHead.Flink; listEntry != &NameSpaceListHead; listEntry = listEntry->Flink) {
        nameSpace = CONTAINING_RECORD(listEntry, NAME_SPACE_INFO, NameSpaceListEntry);

        // Determine whether we should make use of this name space provider.
        ...
        do {
            ...
            // If we're in the fast priority range, we will always serialize resolution attempts.
            if (currentPriority <= NS_MAX_FAST_PRIORITY) {
                break;
            }

            // Find a pointer to the next name space.  If we're at the end of the list, stop.
            if (listEntry->Flink == &NameSpaceListHead) {
                break;
            }

            nextNameSpace = CONTAINING_RECORD(listEntry->Flink, NAME_SPACE_INFO, NameSpaceListEntry);

            // If this name space has a different priority than the current name space, stop.
            if (nextNameSpace->Priority != currentPriority) {
                break;
            }

            // Loop around again looking for more name spaces at the same priority level.
            nameSpace = nextNameSpace;
            listEntry = listEntry->Flink;
        } while (nameSpaceCount < MAX_PARALLEL_REQUESTS);

        // If there is only a single name space provider we need to examine, just call it serially.
        if (nameSpaceCount == 1) {
            // Allocate a buffer to pass to the provider to fill in.
            // We do this rather than pass the user buffer so that we can control where in the buffer we place the information.
            ...
            // If anything was found, copy the information to the user buffer.
            if (count > 0) {
                CopyCsaddrToUserBuffer(buffer, count, lpCsaddrBuffer, *lpdwBufferLength, totalCount, &addressPointer, &bytesUsed);
                totalCount += count;
            ...
            // There are multiple name space providers at the same priority level.
            // We need to spin threads so that we can call them in parallel.
            IF_DEBUG(RESOLVER)
            {
            ...
            for (i = 0; i < nameSpaceCount; i++) {
                ...
                if (ARGUMENT_PRESENT(lpAliasBuffer) && ARGUMENT_PRESENT(lpdwAliasBufferLength)) {
                    ...
                    requests[i].lpAliasBuffer = ALLOCATE_HEAP(*lpdwAliasBufferLength);
                ...
                // Create an event which we'll signal if we want the thread to stop working on the request.
                ...
                // Fill in other elements of the request structure which we communicate to the resolution thread.
                requests[i].Count = 0;
                ...
                // Create the actual thread which will execute the request.
                // The thread is created in a suspended state so that we can easily kill it if an error occurs in starting other threads.
                requests[i].Thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)DoParallelResolution, &requests[i], CREATE_SUSPENDED, &requests[i].ThreadId);
            ...
            // If an error occurred, set all the cancellation events, wait for the threads to complete, and fail the request.
            if (error != NO_ERROR) {
                IF_DEBUG(RESOLVER)
                {
                ...
                // Clean up all the resources we allocates and kill the threads.
                for (i = 0; i < nameSpaceCount; i++) {
                    FREE_HEAP(requests[i].Buffer);
            ...
            // If we're only trying to find a single entry, wait for one thread to complete.
            // If we're finding multiple entries, wait for all the threads to complete.
            threadWaitCount = nameSpaceCount;
            do {
                // Wait for a thread to complete (or all to complete if we're finding multiple entries).
                IF_DEBUG(RESOLVER)
                {
                    DLL_PRINT(("DoNameResolution: waiting for Mult Objs.\n"));
                }

                completed = WaitForMultipleObjects(threadWaitCount, threads, findMultiple, INFINITE);
                ...
                // Find the index of the thread that completed.
                // We must do this because we muck with the thread handle array when threads complete but do not find an answer.
                for (i = 0; i < nameSpaceCount; i++) {
                ...
                // If we're finding a single entry and the thread which completed did not find anything, we will need to continue to wait.
                if (!findMultiple && requests[index].Count <= 0) {
                    // Readjust the thread handle array and decrement the thread wait count.
                    threadWaitCount--;
                    ...
                    // The thread which completed was successful in finding an answer.
                    // Stop waiting for a successful thread.
                    break;
                }
            } while (!findMultiple && threadWaitCount > 0);

            // If this was a single entry search, set the cancellation events for the other threads and wait for them to exit.
            if (!findMultiple) {
                for (i = 0; i < nameSpaceCount; i++) {
            ...
            // Copy over information for each provider which was able to retrieve some addresses.
            aliasBytesUsed = 0;
            ...
            for (i = 0; i < nameSpaceCount; i++) {
                if (requests[i].Count > 0) {
                    CopyCsaddrToUserBuffer(requests[i].Buffer, requests[i].Count, lpCsaddrBuffer, *lpdwBufferLength, totalCount, &addressPointer, &bytesUsed);
                    totalCount += requests[i].Count;

                    // Also copy over any alias information from the thread buffer to the user's alias buffer.
                    if (ARGUMENT_PRESENT(lpAliasBuffer) && ARGUMENT_PRESENT(lpdwAliasBufferLength)) {
                        for (w = requests[i].lpAliasBuffer; *w != L'\0'; w += length / sizeof(TCHAR)) {
                            ...
                            _tcscat(lpAliasBuffer + (aliasBytesUsed / sizeof(TCHAR)), w);
            ...
            // Put the final zero-terminator on the alias buffer and tell the user the count of alias buffer bytes required.
            if (ARGUMENT_PRESENT(lpAliasBuffer) && ARGUMENT_PRESENT(lpdwAliasBufferLength)) {
...
DWORD DoParallelResolution(LPVOID lpThreadParameter)
{
    ...
    IF_DEBUG(RESOLVER)
    {
        ...
        DLL_PRINT(("request: Count=%d, lpServiceName=%s, lpiProtocols@ %x.\n", request->Count, request->lpServiceName, request->lpiProtocols));
    }
#else         // CHICAGO
...
)
{
    ...
    if (!lpdwAliasBufferLength || !lpAliasBuffer || !(dwAliasBytesLeft = *lpdwAliasBufferLength)) {
        lpAliasBuffer = 0;
        dwAliasBytesLeft = 0;
    }
    ...
    if (lpiProtocols) {
        error = ConvertProtocols(lpiProtocols, &dwProtocols, &pp);
        if (error) {
            SetLastError(error);
            return(-1);
        }
    ...
    if (!pwsaq) {
        ...
        dwFlags = LUP_RETURN_NAME | LUP_RETURN_ADDR | (dwResolution & RES_SERVICE ? LUP_RES_SERVICE : 0);

        // ready to start this thing going. We ask for the name back to handle aliases.
#ifdef CHICAGO
...
#endif
                    hRnR, 0, &dwTempSize, pwsaq);
                if (error == NO_ERROR) {
                    // got something. See if it is from the same provider.
                    // If so, this is an alias, otherwise it's a unique entry.
                    if ((dwLastNameSpace == NS_ALL) || (dwLastNameSpace == pwsaq->dwNameSpace) || (dwResolution & RES_FIND_MULTIPLE)) {
                        // use it. Either copy all of the information or just the name. Since we always copy the name, do that first.
                        if (lpAliasBuffer) {
                            DWORD dwLen = _tcslen(pwsaq->lpszServiceInstanceName);
                            if ((dwLen + 2) < dwAliasBytesLeft) {
                                _tcscpy(lpAliasBuffer, pwsaq->lpszServiceInstanceName);
                    ...
                    if ((err1 == NO_DATA) || (err1 == WSA_E_NO_MORE)) {
                        error = NO_ERROR;
                    }
                    break;
                }
    ...
    // Return the count of addresses found or a -1, depending on whether this succeeded
    if (error != NO_ERROR) {
        if (error != -1) {
            SetLastError(error);
        }
...
)
{
    ...
    for (i = 0; i < AddressCount; i++, csaddrInfo += 1, userCsaddrInfo += 1) {
        ...
        *TotalBytesRequired += sizeof(*userCsaddrInfo) + csaddrInfo->LocalAddr.iSockaddrLength + 8 + csaddrInfo->RemoteAddr.iSockaddrLength + 8;

        // If the user buffer is full, continue calculating the number of bytes required.
        if ((ULONG_PTR)*BufferTailPointer < (ULONG_PTR)(userCsaddrInfo + 1) + csaddrInfo->LocalAddr.iSockaddrLength + 8 + csaddrInfo->RemoteAddr.iSockaddrLength + 8) {
            continue;
        }
        ...
        *BufferTailPointer = (PVOID)((ULONG_PTR)*BufferTailPointer - sockaddrLength);
        userCsaddrInfo->LocalAddr.lpSockaddr = *BufferTailPointer;

        memcpy(*BufferTailPointer, csaddrInfo->LocalAddr.lpSockaddr, csaddrInfo->LocalAddr.iSockaddrLength);

        sockaddrLength = (csaddrInfo->RemoteAddr.iSockaddrLength + 7) & ~7;
        *BufferTailPointer = (PVOID)((ULONG_PTR)*BufferTailPointer - sockaddrLength);
        userCsaddrInfo->RemoteAddr.lpSockaddr = *BufferTailPointer;

        memcpy(*BufferTailPointer, csaddrInfo->RemoteAddr.lpSockaddr, csaddrInfo->RemoteAddr.iSockaddrLength);
    }
...
INT APIENTRY __GetAddressByNameA(
    IN     DWORD                dwNameSpace,
    IN     LPGUID               lpServiceType,
    IN     LPSTR                lpServiceName,
    IN     LPINT                lpiProtocols,
...
)
{
    ...
    // Call a subroutine to actually perform the name resolution.
    count = __DoNameResolution(dwNameSpace, lpServiceType, lpServiceName, lpiProtocols, dwResolution, lpCsaddrBuffer, lpdwBufferLength, lpAliasBuffer, lpdwAliasBufferLength);
            ...
            do {
                ...
                if (error == NO_ERROR) {
                    ...
                    if ((dwLastNameSpace == NS_ALL) || (dwLastNameSpace == pwsaq->dwNameSpace) || (dwResolution & RES_FIND_MULTIPLE)) {
                        // use it. Either copy all of the information or just the name. Since we always copy the name, do that first.
                        if (lpAliasBuffer) {
                            DWORD dwLen = strlen(pwsaq->lpszServiceInstanceName);
                            if ((dwLen + 2) < dwAliasBytesLeft) {
                                strcpy(lpAliasBuffer, pwsaq->lpszServiceInstanceName);
                                lpAliasBuffer += dwLen + 1;
                                dwAliasBytesLeft -= dwLen + 1;
                            }
                        }
                        if (dwLastNameSpace != pwsaq->dwNameSpace) {
                            // Need to copy the addresses as well
                            if (pwsaq->dwNumberOfCsAddrs) {
                                CopyCsaddrToUserBuffer(pwsaq->lpcsaBuffer, pwsaq->dwNumberOfCsAddrs, lpCsaddrBuffer, *lpdwBufferLength, totalCount, &addressPointer, &bytesUsed);
                                totalCount += pwsaq->dwNumberOfCsAddrs;
                            }
