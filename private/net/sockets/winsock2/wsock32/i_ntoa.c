...
Abstract:
    This module implements a routine to convert a numerical IP address into a dotted-decimal character string Internet address.
...
char * PASCAL inet_ntoa(IN  struct in_addr  in)
/*++
Routine Description:
    This function takes an Internet address structure specified by the in parameter.
    It returns an ASCII string representing the address in ".'' notation as "a.b.c.d".
    Note that the string returned by inet_ntoa() resides in memory which is allocated by the Windows Sockets implementation.
    The application should not make any assumptions about the way in which the memory is allocated.
    The data is guaranteed to be valid until the next Windows Sockets API call within the same thread, but no longer.
Arguments:
...
Return Value:
    ...
    The data should be copied before another Windows Sockets call is made.
--*/
{
    PUCHAR p;
    PUCHAR buffer;
    ...
    // A number of applications apparently depend on calling inet_ntoa() without first calling WSAStartup().
    // Because of this, we must perform our own explicit thread initialization check here.
    ...
    // Note that for values less than 100 we will do one or two extra assignments, but we save a test/jump with this algorithm.
