...
static char * any (IN char *cp, IN char *match)
{
    register char *mp, c;
...
// internal form of above
struct servent * _pgetservebyport(IN const int port, IN const char *proto)
{
    register struct servent *p;
...
DWORD CopyServentToBuffer (IN char FAR *Buffer, IN int BufferLength, IN PSERVENT Servent)
{
    ...
    for ( i = 0; i < aliasCount; i++ ) {
        ...
        RtlCopyMemory(currentLocation, Servent->s_aliases[i], strlen( Servent->s_aliases[i] ) + 1);
        currentLocation = Buffer + bytesFilled;
    }
