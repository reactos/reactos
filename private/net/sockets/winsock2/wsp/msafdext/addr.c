...
--*/
{
    ...
    if ((sscanf( args, "%s", expr )!=1) || (address = GetExpression (expr))==0 ) {
        dprintf (("use: addr address\n"));
    } else {
        if( !ReadMemory(address, &addr, sizeof(addr), &result) ) {
            dprintf (("addr: cannot read SOCKADDR @ %08lx\n", address));
            return;
        }

        DumpSockaddr("", &addr, address);
    }
}   // addr
