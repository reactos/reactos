...
Abstract:
    This module contains support TransmitFile() extended WinSock API.
...
BOOL TransmitFile (IN SOCKET hSocket, 
                   IN HANDLE hFile,
...
Arguments:
    ...
    lpOverlapped - specify this parameter when the socket handle is
        opened as overlapped to achieve overlapped I/O.  Note that by default sockets are opened as overlapped.

        Also use this parameter to specify an offset within the file at which to start the file data transfer.  The file pointer is
        ignored by this API, so if lpOverlapped is NULL the transmission always starts at offset 0 in the file.

        When lpOverlapped is non-NULL, TransmitFile() may return ERROR_IO_PENDING to allow the calling function to continue
        processing while the operation completes.  The event (or hSocket 
        if hEvent is NULL) will be set to the signalled state upon completion of the request.

    lpTransmitBuffers - an optional pointer to a TRANSMIT_FILE_BUFFERS structure which contains pointers to data to send before the
        file (the head buffer) and after the file (the tail buffer).  If
        only the file is to be sent, this parameter may be specified as NULL.
    ...
    dwFlags - any combination of the following:
        ...
        TF_REUSE_SOCKET - prepare the socket handle to be reused.  When
            ...
            passed to the SuperAccept API.  Only valid if TF_DISCONNECT is also specified.
        ...
        TF_WRITE_BEHIND - complete the TransmitFile request immediately,
            ...
            necessarily acknowledged by the remote end.  Do not use this setting with the other two settings.
...
--*/
{
    ...
    __try {
        ...
        if ( ARGUMENT_PRESENT( lpOverlapped ) ) {
            ...
            // allocates a large enough system buffer for AFD to use as a scratch buffer.
            status = NtDeviceIoControlFile((HANDLE)hSocket,
                                           lpOverlapped->hEvent,
                                           NULL,
            ...
            if ( NT_SUCCESS(status) && status != STATUS_PENDING) {
                ...
                return SanTransmitFile(hSocket, hFile, nNumberOfBytesToWrite, nNumberOfBytesPerSend, lpOverlapped, lpTransmitBuffers, dwFlags);
            ...
            if ( NT_SUCCESS(status) ) {
                ...
                // STATUS_INVALID_PARAMETER_12 is indication from AFD that this socket is connected thru SAN
                return SanTransmitFile(hSocket, hFile, nNumberOfBytesToWrite, nNumberOfBytesPerSend, lpOverlapped, lpTransmitBuffers, dwFlags);
#endif // _AFD_SAN_SWITCH_
            } else {
                SetLastError( SockNtStatusToSocketError(status) );
