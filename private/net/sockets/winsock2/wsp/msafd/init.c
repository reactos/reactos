...
BOOL SockInitialize(IN PVOID DllHandle, IN ULONG Reason, IN PVOID Context OPTIONAL)
{
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        // open it and read the first line to set the debugging flags.
        {
            ...
            handle = CreateFile("MsafdDebug", GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
        ...
        // Initialize the lists of sockets and helper DLLs, and the resource protecting them
    ...
    case DLL_PROCESS_DETACH:
        ...
        SockProcessTerminating = TRUE;// We will fail all futher API calls
        ...
        if (Context == NULL) {
            ...
            if (tlsData != NULL) {
                if (tlsData->PendingAPCCount) {
                    InterlockedExchangeAdd(&SockProcessPendingAPCCount, -(tlsData->PendingAPCCount));
                }
