...
Abstract:
    This module contains support for the select( ) and WSASelectWindow WinSock APIs.
...
// FD_SET uses the FD_SETSIZE macro, so define it to a huge value here so that apps can pass a very large number of sockets to select(), which uses the FD_SET macro.
// Have to do this after precompiled include since it will be ignored otherwise
#undef FD_SETSIZE
...
BOOL SockCheckAndInitAsyncSelectHelper(VOID);
VOID SockProcessAsyncSelect(PSOCKET_INFORMATION Socket, PPOLL_CONTEXT_BLOCK Context);


int WSPAPI WSPSelect(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, const struct timeval *timeout, LPINT lpErrno)
/*++
Routine Description:
    ...
    Upon return, the structure is updated to reflect the subset of these sockets which meet the specified condition,
    and select() returns the number of sockets meeting the conditions.
    ...
    If the socket is currently listen()ing, it will be marked as readable if an incoming connection request has been received,
    so that an accept() is guaranteed to complete without blocking.
    For other sockets, readability means that queued data is available for reading,
    so that a recv() or recvfrom() is guaranteed to complete without blocking.
    The presence of out-of-band data will be checked if the socket option SO_OOBINLINE has been enabled (see setsockopt()).

    The parameter writefds identifies those sockets which are to be checked for writeability.
    If a socket is connect()ing (non-blocking), writeability means that the connection establishment is complete.
    For other sockets, writeability means that a send() or sendto() will complete without blocking.
    ...
    For a SOCK_STREAM, the breaking of the connection by the peer or due to KEEPALIVE failure will be indicated as an exception.
    ...
    Four macros are defined in the header file winsock.h for manipulating the descriptor sets.
    The variable FD_SETSIZE determines the maximum number of descriptors in a set.
    (The default value of FD_SETSIZE is 64, which may be modified by #defining FD_SETSIZE to another value before #including winsock.h.)
    Internally, an fd_set is represented as an array of SOCKETs; the last valid entry is followed by an element set to INVALID_SOCKET.
    The macros are:
    ...
    The parameter timeout controls how long the select() may take to complete.
    If timeout is a null pointer, select() will block indefinitely until at least one descriptor meets the specified criteria.
    Otherwise, timeout points to a struct timeval which specifies the maximum time that select() should wait before returning.
    If the timeval is initialized to {0, 0}, select() will return immediately; this is used to "poll" the state of the selected sockets.
...
Return Value:
    select() returns the total number of descriptors which are ready and contained in the fd_set structures, or 0 if the time limit expired.
--*/
{
    NTSTATUS status;
    ...
    __try {
        // Determine how many handles we're going to check so that we can allocate a buffer large enough to hold information about all of them.
        handleCount = HANDLES_IN_SET(readfds) + HANDLES_IN_SET(writefds) + HANDLES_IN_SET(exceptfds);
        ...
        pollBufferSize = sizeof(AFD_POLL_INFO) + handleCount * sizeof(AFD_POLL_HANDLE_INFO);
        ...
        for (i = 0; readfds != NULL && i < (readfds->fd_count & 0xFFFF); i++) {
            // If the connection is disconnected, either abortively or orderly, 
            // then it is considered possible to read immediately on the socket, so include these events in addition to receive.
            pollHandleInfo->Handle = (HANDLE)readfds->fd_array[i];
            pollHandleInfo->PollEvents = AFD_POLL_RECEIVE | AFD_POLL_DISCONNECT | AFD_POLL_ABORT;
            pollHandleInfo++;
        }
        ...
        for (i = 0; exceptfds != NULL && i < (exceptfds->fd_count & 0xFFFF); i++) {
            ...
            pollHandleInfo->PollEvents = AFD_POLL_RECEIVE_EXPEDITED | AFD_POLL_CONNECT_FAIL;
            pollHandleInfo++;
        }

        // If a timeout was specified, convert it to NT format.  Since it is a relative time, it must be negative.
        if (timeout != NULL) {
            LARGE_INTEGER microseconds;

            pollInfo->Timeout = RtlEnlargedIntegerMultiply(timeout->tv_sec, -10 * 1000 * 1000);
            microseconds = RtlEnlargedIntegerMultiply(timeout->tv_usec, -10);
            pollInfo->Timeout.QuadPart = pollInfo->Timeout.QuadPart + microseconds.QuadPart;
        } else {
            // No timeout was specified, just set the timeout value to the largest possible value, in effect using an infinite timeout.
    ...
    // Send the IOCTL to AFD.  AFD will complete the request as soon as one or more of the specified handles is ready for the specified operation.

    // Just use the first handle as the handle for the request.  Any handle is fine; we just need a handle to AFD so that it gets to the driver.

    // Note that the same buffer is used for both input and output.
    // Since IOCTL_AFD_POLL is a method 0 (buffered) IOCTL, this shouldn't cause problems.
    ...
    if (status == STATUS_PENDING) {
        SockWaitForSingleObject(tlsData->EventHandle,
            (SOCKET)pollInfo->Handles[0].Handle,
                                pollInfo->Timeout.QuadPart == 0 ? SOCK_NEVER_CALL_BLOCKING_HOOK : SOCK_ALWAYS_CALL_BLOCKING_HOOK,
                                SOCK_NO_TIMEOUT);
    ...
    __try {
        // Use the information provided by the driver to set up the fd_set structures to return to the caller.  First zero out the structures.
        if (readfds != NULL) {
            FD_ZERO(readfds);
        }
        ...
        // Walk the poll buffer returned by AFD, setting up the fd_set structures as we go.
        pollHandleInfo = pollInfo->Handles;
        for (i = 0; i < pollInfo->NumberOfHandles; i++) {
            ...
            if ((pollHandleInfo->PollEvents & AFD_POLL_LOCAL_CLOSE) != 0) {
                // If the app does a closesocket() on a handle that has a select() outstanding on it, 
                // this event may get set by AFD even though we didn't request notification of it.
...
exit:
    IF_DEBUG(SELECT)
    {
        if (err != NO_ERROR) {
            ...
            WS_PRINT(("WSPSelect succeeded, %ld readfds, %ld writefds, "
                      "%ld exceptfds\n", HANDLES_IN_SET(readfds), HANDLES_IN_SET(writefds), HANDLES_IN_SET(exceptfds)));
        }
    }
    ...
    WS_ASSERT((ULONG)handlesReady == (ULONG)(HANDLES_IN_SET(readfds) + HANDLES_IN_SET(writefds) + HANDLES_IN_SET(exceptfds)));
    WS_EXIT("WSPSelect", handlesReady, FALSE);
    return handlesReady;
}   // WSPSelect


int WSPAPI WSPAsyncSelect(SOCKET Handle, HWND hWnd, unsigned int wMsg, long lEvent, LPINT lpErrno)
/*++
Routine Description:
    This function is used to request that the Windows Sockets DLL should send a message to the window hWnd whenever it detects any of the network events specified by the lEvent parameter.
    ...
    Although the WSAAsyncSelect() takes effect immediately, it is possible that messages may be waiting in the application's message queue.
    The application must therefore be prepared to receive network event messages even after cancellation.

    This function automatically sets socket s to non-blocking mode.

    When one of the nominated network events occurs on the specified socket s, the application's window hWnd receives message wMsg.
    ...
    The error code be any error as defined in winsock.h.
...
Return Value:
    The return value is 0 if the application's declaration of interest in the network event set was successful.
...
--*/
{
    ...
    // !!! should we also make sure that the bits make sense for the state of the socket, i.e. don't allow FD_ACCEPT on a connected socket?
    ...
    nonBlocking = TRUE;
    err = SockSetInformation(socket, AFD_NONBLOCKING_MODE, &nonBlocking, NULL, NULL);
    if (err != NO_ERROR) {
        goto errorexit;
    }
    ...
    if (socket->EventSelectlNetworkEvents) {
        err = SockEventSelectHelper(socket, NULL, 0);
        if (err != NO_ERROR) {
            goto errorexit;
        }
    }
    ...
    // Acquire the lock that protects this socket. 
    // We hold this lock throughout this routine to synchronize against other callers performing operations on the socket we're using.
    SockAcquireSocketLockExclusive(socket);

    // Store the specified window handle, message, and event mask in the socket information structure.
    socket->AsyncSelecthWnd = hWnd;
    socket->AsyncSelectwMsg = wMsg;
    socket->AsyncSelectlEvent = lEvent;
    socket->DisabledAsyncSelectEvents = 0;

    // If the socket is not connected and is not a datagram socket, disable FD_WRITE events.
    // We'll reenable them when the socket becomes connected.
    // Disabling them here prevents a race condition between FD_WRITE events and FD_CONNECT events, 
    // and we don't want FD_WRITE events to be posted before FD_CONNECT.
    if (!SockIsSocketConnected(socket) && !IS_DGRAM_SOCK(socket)) {
        socket->DisabledAsyncSelectEvents |= FD_WRITE;
    }

    // Bump the async select serial number on this socket.
    // This field allows us to stop sending a particular message as soon as WSPAsyncSelect is called to change the settings on the socket.
    // When an AFD poll completes for an async select, if the serial number has changed then no message is sent.
    socket->AsyncSelectSerialNumber++;

    // If there are no events that we can poll on, just return.
    // When a reenabling function or WSPAsyncSelect is called a new request will be queued to the async thread, and a new poll will be initiated.
    if ((socket->AsyncSelectlEvent & (~socket->DisabledAsyncSelectEvents)) == 0) {
        ...
        IF_DEBUG(ASYNC_SELECT)
        {
            WS_PRINT(("WSPAsyncSelect: no events to poll on, "
                      "socket = %lx, lEvent = %lx, disabled = %lx\n", socket, socket->AsyncSelectlEvent, socket->DisabledAsyncSelectEvents));
        }
        WS_EXIT("WSPAsyncSelect", NO_ERROR, FALSE);
        return NO_ERROR;
    }

    // Initialize unchanging part of the context Store referenced pointer to socket info structure, 
    // so we can safely access it when poll completes even if socket is closed.
    ...
    // Some of the applications tend to exit the thread in which async select was started killing the poll IRP.
    // Because we are not that much concerned with performance in async select, 
    // we'd rather take a hit here than trying to pick up and repost the IRP in our own thread when it gets cancelled.
    status = NtSetIoCompletion(SockAsyncQueuePort, (PVOID)&SockProcessQueuedAsyncSelect, context, 0, 0);
    if (!NT_SUCCESS(status)) {
        SockDereferenceAsyncThread();
        err = SockNtStatusToSocketError(status);
        goto errorexit;
...
VOID SockProcessAsyncSelect(PSOCKET_INFORMATION Socket, PPOLL_CONTEXT_BLOCK Context)
{
    ...
    // Initialize the context and poll buffer.
    // Use an infinite timeout and just the one socket handle.
    // Also, this should be a unique poll, so if another unique poll is outstanding on this socket it will be canceleld.
    ...
    eventsToPoll = Socket->AsyncSelectlEvent & (~Socket->DisabledAsyncSelectEvents);
    WS_ASSERT(eventsToPoll != 0);

    // Set up the events to poll on.
    // Note that we don't put down AFD_POLL_CONNECT or AFD_POLL_CONNECT_FAIL requests even if FD_CONNECT is specified-- this event is handled manually by the connect() routine because of it's inherently different semantics.
    ...
    if ((eventsToPoll & FD_CLOSE) != 0) {
        Context->PollInfo.Handles[0].PollEvents |= AFD_POLL_DISCONNECT | AFD_POLL_ABORT | AFD_POLL_LOCAL_CLOSE;
    }
    ...
    if (NT_ERROR(status)) {
        // If the request failed, call the completion routine since the system didn't complete it.
...
VOID SockProcessQueuedAsyncSelect(IN PVOID ApcContext, IN PIO_STATUS_BLOCK IoStatusBlock)
{
    PPOLL_CONTEXT_BLOCK context = ApcContext;
    PSOCKET_INFORMATION socket = context->SocketInfo;

    // Acquire the socket's lock to keep it's state constant while we do the message posts.
    SockAcquireSocketLockExclusive(socket);
    ...
    // If the serial number on the socket has changed since this request was initialized,
    // or if the original socket was closed, just bag this notification.
    // Another poll request should be on the way if the socket is still open.
    if (context->AsyncSelectSerialNumber != socket->AsyncSelectSerialNumber) {
        IF_DEBUG(ASYNC_SELECT)
        {
            WS_PRINT(("SockProcessQueuedAsyncSelect: tossing req on handle %lx, "
                      " ASN %ld req ASN %ld\n", socket->HContext.Handle, socket->AsyncSelectSerialNumber, context->AsyncSelectSerialNumber));
        }
    ...
    // Start another poll.
    SockProcessAsyncSelect(socket, context);
    SockReleaseSocketLock(socket);
    return;
...
VOID SockAsyncSelectCompletion(IN PVOID ApcContext, IN PIO_STATUS_BLOCK IoStatusBlock)
{
    PPOLL_CONTEXT_BLOCK context = ApcContext;
    PSOCKET_INFORMATION socket = context->SocketInfo;
    BOOL posted = FALSE;
    ...
    if (IoStatusBlock->Status == STATUS_CANCELLED || socket->State == SocketStateClosing) {
        IF_DEBUG(ASYNC_SELECT)
        {
            WS_PRINT(("AsyncSelectCompletionApc: tossing request on handle %lx"
                      "--%s.\n", socket->HContext.Handle, (socket->State == SocketStateClosing) ? "closed" : "cancelled"));
        }
    ...
    // If the serial number on the socket has changed since this request was initialized, or if the original socket was closed, just bag this notification.
    // Another poll request should be on the way if the socket is still open.
    if (context->AsyncSelectSerialNumber != socket->AsyncSelectSerialNumber) {
        IF_DEBUG(ASYNC_SELECT)
        {
    ...
    if (!NT_SUCCESS(IoStatusBlock->Status)) {
        ...
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
    ...
    // We should never get AFD_POLL_CONNECT or AFD_POLL_CONNECT_FAIL events, since we never ask for them.  FD_CONNECT is handled manually by the connect() routine.
    ...
    if (IS_EVENT_ENABLED(FD_READ, socket) && (pollEvents & AFD_POLL_RECEIVE) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        // Disable this event.  It will be reenabled when the app does a recv().
        socket->DisabledAsyncSelectEvents |= FD_READ;
    }

    if (IS_EVENT_ENABLED(FD_OOB, socket) && (pollEvents & AFD_POLL_RECEIVE_EXPEDITED) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        // Disable this event.  It will be reenabled when the app does a recv( MSG_OOB ).

        // !!! need synchronization?
        socket->DisabledAsyncSelectEvents |= FD_OOB;
    }

    if (IS_EVENT_ENABLED(FD_WRITE, socket) && (pollEvents & AFD_POLL_SEND) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        // Disable this event.  It will be reenabled when the app does a send().
        socket->DisabledAsyncSelectEvents |= FD_WRITE;
    }

    if (IS_EVENT_ENABLED(FD_ACCEPT, socket) && (pollEvents & AFD_POLL_ACCEPT) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        // Disable this event.  It will be reenabled when the app does an accept().
        socket->DisabledAsyncSelectEvents |= FD_ACCEPT;
    }

    if (IS_EVENT_ENABLED(FD_CLOSE, socket) &&
        ((pollEvents & AFD_POLL_DISCONNECT) != 0 || (pollEvents & AFD_POLL_ABORT) != 0 || (pollEvents & AFD_POLL_LOCAL_CLOSE) != 0)) {
    ...
    if (IS_EVENT_ENABLED(FD_QOS, socket) && (pollEvents & AFD_POLL_QOS) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        // Disable this event.  It will be reenabled when the app does an WSAIoctl (SIO_GET_QOS).
        socket->DisabledAsyncSelectEvents |= FD_QOS;
    }

    if (IS_EVENT_ENABLED(FD_GROUP_QOS, socket) && (pollEvents & AFD_POLL_GROUP_QOS) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        // Disable this event.  It will be reenabled when the app does an WSAIoctl (SIO_GET_GROUP_QOS).
        socket->DisabledAsyncSelectEvents |= FD_GROUP_QOS;
    }

#ifdef _PNP_POWER_
    if (IS_EVENT_ENABLED(FD_ROUTING_INTERFACE_CHANGE, socket) && (pollEvents & AFD_POLL_ROUTING_IF_CHANGE) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        IF_DEBUG(POST)
        {
            WS_PRINT(("POSTED wMsg %lx hWnd %lx socket %lx event %s err %ld\n",
                      ...
                      socket->AsyncSelecthWnd,
                      socket->Handle,
                      "FD_ROUTING_INTERFACE_CHANGE", 0));
        }

        // Don't need to disable this event.  It will not occur until application does WSAIoctl (SIO_ROUTING_INTERFACE_CHANGE).

        // socket->DisabledAsyncSelectEvents |= FD_ROUTING_INTERFACE_CHANGE;
    }

    if (IS_EVENT_ENABLED(FD_ADDRESS_LIST_CHANGE, socket) && (pollEvents & AFD_POLL_ADDRESS_LIST_CHANGE) != 0) {
        posted = (SockUpcallTable->lpWPUPostMessage)(socket->AsyncSelecthWnd,
                                                     socket->AsyncSelectwMsg,
                                                     (WPARAM)socket->Handle,
        ...
        IF_DEBUG(POST)
        {
            WS_PRINT(("POSTED wMsg %lx hWnd %lx socket %lx event %s err %ld\n",
                      ...
                      socket->AsyncSelecthWnd,
                      socket->Handle,
                      "FD_ADDRESS_LIST_CHANGE", 0));
        }

        // Don't need to disable this event.  It will not occur until application does WSAIoctl (SIO_ADDRESS_LIST_CHANGE).
...
VOID SockReenableAsyncSelectEvent(IN PSOCKET_INFORMATION Socket, IN ULONG Event)
{
    ...
    // Check whether the specified event is in the list of disabled async select events for the socket.
    ...
    // *** Note that it is assumed that the socket lock is held on entry to this routine!
    ...
    // We need to bump the number, so that if there is outstaning async select on the socket it will go away
...
BOOL SockCheckAndInitAsyncSelectHelper(VOID)
{
    ...
    // Check again, in case another thread has already initialized the async helper handle.
    if (SockAsyncSelectHelperHandle != NULL) {
    ...
    InitializeObjectAttributes(&objectAttributes, &afdName, OBJ_INHERIT | OBJ_CASE_INSENSITIVE, NULL, NULL);
    ...
    status = NtSetInformationFile(Handle, &ioStatusBlock, &completionInfo, sizeof(completionInfo), FileCompletionInformation);
    if (!NT_SUCCESS(status)) {
    ...
    // Protect our handle from being closed by random NtClose call by some messed up dll or application
    HandleInfo.ProtectFromClose = TRUE;
    HandleInfo.Inherit = FALSE;
    status = NtSetInformationObject(Handle, ObjectHandleFlagInformation, &HandleInfo, sizeof(HandleInfo));
    // This shouldn't fail
    ...
    // We'll use this information in the send and receive routines to know if and when to attempt to reenable async select events.
...
/*++
Routine Description:
    This routine is used by the FD_ISSET macro; applications should not call it directly.
    It determines whether a socket handle is included in an fd_set structure.
Arguments:
