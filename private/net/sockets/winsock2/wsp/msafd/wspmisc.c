...
Routine Description:
    This function cancels any outstanding blocking operation for this task.  It is normally used in two situations:
        (1) An application is processing a message which has been
          received while a blocking call is in progress.  In this case, WSAIsBlocking() will be true.
        (2) A blocking call is in progress, and Windows Sockets has
          called back to the application's "blocking hook" function (as established by WSASetBlockingHook()).

    In each case, the original blocking call will terminate as soon as possible with the error WSAEINTR.  (In (1), the termination will not
    take place until Windows message scheduling has caused control to revert to the blocking routine in Windows Sockets.  In (2), the
    blocking call will be terminated as soon as the blocking hook function completes.)

    In the case of a blocking connect() operation, the Windows Sockets implementation will terminate the blocking call as soon as possible,
    but it may not be possible for the socket resources to be released until the connection has completed (and then been reset) or timed
    out.  This is likely to be noticeable only if the application immediately tries to open a new socket (if no sockets are
...
Return Value:
    ...
    was successfully canceled.  Otherwise the value SOCKET_ERROR is returned, and a specific error number is availalbe in lpErrno.
--*/
{
    ...
    // This is now checked by ws2_32.dll, no point in duplicating this check here
    ...
    // Note that because we disable the blocking hook callback below, the IO should not have already been cancelled.
    WS_ASSERT(!tlsData->IoCancelled);
    WS_ASSERT(tlsData->SocketHandle != INVALID_SOCKET);

    // Cancel all the IO initiated in this thread for the socket handle we're blocking on.
...
int WSPAPI WSPDuplicateSocket(SOCKET Handle, DWORD ProcessId, LPWSAPROTOCOL_INFOW lpProtocolInfo, LPINT lpErrno)
{
    PWINSOCK_TLS_DATA    tlsData;
    int err;
    ...
    err = SockBuildProtocolInfoForSocket(socket, lpProtocolInfo);
    if (err != NO_ERROR) {
        CloseHandle(processHandle);
    ...
    // Success!
    __try {
        // This cast is safe as handle is guaranteed to take only first 32 bits.
...
BOOL WSPAPI WSPGetOverlappedResult(SOCKET Handle, LPWSAOVERLAPPED lpOverlapped, LPDWORD lpcbTransfer, BOOL fWait, LPDWORD lpdwFlags, LPINT lpErrno)
{
    PWINSOCK_TLS_DATA    tlsData;
    int err;
    ...
    __try {
        ...
        if (GetOverlappedResult((HANDLE)Handle, lpOverlapped, lpcbTransfer, fWait)) {
...
BOOL WSPAPI WSPGetQOSByName(SOCKET Handle, LPWSABUF lpQOSName, LPQOS lpQOS, LPINT lpErrno)
{
    PWINSOCK_TLS_DATA    tlsData;
    int err;
    DWORD bytesReturned;
    ...
    if (WSPIoctl(Handle, SIO_GET_QOS, NULL, 0, lpQOS, sizeof(*lpQOS), &bytesReturned, NULL, NULL, NULL, &err) == SOCKET_ERROR) {
        WS_ASSERT(err != NO_ERROR);
        goto exit;
    }
