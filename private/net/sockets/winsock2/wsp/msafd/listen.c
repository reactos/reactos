...
int WSPAPI WSPListen(IN SOCKET Handle, IN int Backlog, OUT LPINT lpErrno)
/*++
Routine Description:
    To accept connections, a socket is first created with socket(), a backlog for incoming connections is specified with listen(), and
    then the connections are accepted with accept().
    listen() applies only to sockets that support connections, i.e.  those of type SOCK_STREAM.
    The socket s is put into "passive'' mode where incoming connections are acknowledged and queued pending acceptance by the process.
    ...
    with the queue full, the client will receive an error with an indication of WSAECONNREFUSED.
    ...
    If descriptors become available, a later call to listen() or accept() will re-fill the queue to the current or most
    recent "backlog'', if possible, and resume listening for incoming connections.
...
Return Value:
    If no error occurs, listen() returns 0.
...
--*/
{
    ...
    // We hold this lock throughout this routine to synchronize against other callers
    // performing operations on the socket we begin the listening process.
    ...
    // address but not already connected (except if this is a C_Root socket. 
    // The only valid states for this call is bound and listening.
    // Note that if the socket is already listening, we just no-op this call.
    // Some unixes refill the connection backlog on a subsequent listen, but we don't do that.
    if (socket->Listening) {
        err = NO_ERROR;
        goto exit;
    ...
    // If it is out of range, just set it to the closest in-range value--this duplicates BSD 4.3 behavior.
    // Note that NT Workstation
    // is tuned to have a lower backlog limit in order to conserve resources on that product type.
...
// Tell AFD to start listening on this endpoint.
    ...
    retrywithsan :
                 ...
                 // Notify the helper DLL that the socket is now listening for connections.
                 err = SockNotifyHelperDll(socket, WSH_NOTIFY_LISTEN);
                 if (err != NO_ERROR) {
                     goto exit;
                 }

                 socket->Listening = TRUE;// Indicate the listening state of this socket.
