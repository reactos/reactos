...
int WSPAPI WSPShutdown(IN SOCKET Handle, IN int HowTo, OUT LPINT lpErrno)
/*++
Routine Description:
    This routine is used on all types of sockets to disable reception, transmission, or both.

    If how is SD_RECEIVE, subsequent receives on the socket will be disallowed. 
    ...
    sockets, a FIN will be sent. Setting how to SD_BOTH disables both sends and receives as described above.
    ...
    provider is not required to support the use of WSPConnect() on such a socket.
...
Return Value:
    If no error occurs, WSPShutdown() returns 0. Otherwise, a value of SOCKET_ERROR is returned, and a specific error code is available in lpErrno.
--*/
{
    NTSTATUS status;
    ...
    // If this is not a datagram socket, then it must be connected in order for WSPShutdown() to be a legal operation.
    if (!IS_DGRAM_SOCK(socket) && !SockIsSocketConnected(socket)) {
        err = WSAENOTCONN;
        goto exit;
    }

    // Translate the How parameter into the AFD disconnect information structure.
    switch (HowTo) {
    case SD_RECEIVE:
    ...
    case SD_SEND:
        ...
        break;
    case SD_BOTH:
        disconnectInfo.DisconnectMode = AFD_PARTIAL_DISCONNECT_RECEIVE | AFD_PARTIAL_DISCONNECT_SEND;
        socket->ReceiveShutdown = TRUE;
        socket->SendShutdown = TRUE;
    ...
    // This routine should complete immediately, not when the remote client acknowledges the disconnect.
...
int WSPAPI WSPRecvDisconnect(SOCKET Handle, LPWSABUF lpInboundDisconnectData, LPINT lpErrno)
/*++
Routine Description:
...
--*/
{
    ...
    // If this is not a datagram socket, then it must be connected in order for WSPRecvDisconnect() to be a legal operation.
    if (!IS_DGRAM_SOCK(socket) && !SockIsSocketConnected(socket)) {
        err = WSAENOTCONN;
        goto exit;
    }
    ...
    __try {
        if (lpInboundDisconnectData != NULL && lpInboundDisconnectData->buf != NULL && lpInboundDisconnectData->len > 0) {
            INT bufferLength;

            // Retrieve the disconnect data from AFD.
            bufferLength = (INT)lpInboundDisconnectData->len;

            err = SockGetConnectData(socket,
...
int WSPAPI WSPSendDisconnect(SOCKET Handle, LPWSABUF lpOutboundDisconnectData, LPINT lpErrno)
/*++
Routine Description:
...
--*/
{
    ...
    // If this is not a datagram socket, then it must be connected in order for WSPSendDisconnect() to be a legal operation.
    if (!IS_DGRAM_SOCK(socket) && !SockIsSocketConnected(socket)) {
        err = WSAENOTCONN;
        goto exit;
    }
    ...
    __try {
        if (lpOutboundDisconnectData != NULL && lpOutboundDisconnectData->buf != NULL && lpOutboundDisconnectData->len > 0) {
            INT bufferLength;

            // Set the disconnect data on the socket.
            bufferLength = (INT)lpOutboundDisconnectData->len;
            err = SockSetConnectData(socket,
