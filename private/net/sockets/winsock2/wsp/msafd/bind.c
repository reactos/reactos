...
int WSPAPI WSPBind(IN SOCKET Handle, IN const struct sockaddr *SocketAddress, IN int SocketAddressLength, OUT LPINT lpErrno)
/*++
Routine Description:
    This routine is used on an unconnected datagram or stream socket, before subsequent connect()s or listen()s.  When a socket is created
    with socket(), it exists in a name space (address family), but it has no name assigned.  bind() establishes the local association
    (host address/port number) of the socket by assigning a local name to an unnamed socket.

    If an application does not care what address is assigned to it, it may specify an Internet address and port equal to 0.  If this is the
    case, the Windows Sockets implementation will assign a unique address to the application.  The application may use getsockname()
    ...
    For SOCK_DGRAM and SOCK_STREAM, the name consists of
    three parts: a host address, the protocol number (set implicitly to UDP or TCP, respectively),
...
--*/
{
    ...
    IF_DEBUG(BIND)
    {
        WS_PRINT(("WSPBind() on socket %lx addr %lx addrlen %ld\n"
                  "    req", Handle, SocketAddress, SocketAddressLength));
    ...
    // Find a pointer to the socket structure corresponding to the passed-in handle.
    ...
    // throughout this routine to synchronize against other callers performing operations on the socket we're binding.
    SockAcquireSocketLockExclusive(socket);

    // If the socket has been initialized at all then this is not a legal request.
    ...
    // just truncate it.  Otherwise, the TDI address buffer we allocate may not be large enough.
    if (SocketAddressLength > socket->HelperDll->MaxSockaddrLength) {
        SocketAddressLength = socket->HelperDll->MaxSockaddrLength;
    }

    // Make sure that the address structure passed in is legitimate, and determine the type of address we're binding to.
    err = socket->HelperDll->WSHGetSockaddrType((PSOCKADDR)SocketAddress, SocketAddressLength, &sockaddrInfo);
    if (err != NO_ERROR) {
        goto exit;
    }
    ...
    // in the reserved field so that UDP knows that we really want to bind to the zero address (0.0.0.0).
    if (socket->DontUseWildcard && sockaddrInfo.AddressInfo == SockaddrAddressInfoWildcard) {
        *(UNALIGNED ULONG *)(SocketAddress->sa_data + 6) = 0x12345678;
    }

    // !!! test for reserved port?

    // Allocate enough space to hold the TDI address structure we'll pass to AFD and get back in return

    // This is used for input
    bindInfoLength = FIELD_OFFSET(AFD_BIND_INFO, Address) + socket->HelperDll->MaxTdiAddressLength;

    // This is used for output
    tdiAddressInfoLength = FIELD_OFFSET(TDI_ADDRESS_INFO, Address) + socket->HelperDll->MaxTdiAddressLength;

    if (tdiAddressInfoLength <= sizeof(localBuffer) && bindInfoLength <= sizeof(localBuffer)) {
        tdiAddressInfo = (PVOID)localBuffer;
        bindInfo = (PVOID)localBuffer;
    } else {
        tdiAddressInfo = ALLOCATE_HEAP(max(tdiAddressInfoLength, bindInfoLength));
    ...
    // Convert the address from the sockaddr structure to the appropriate TDI structure.
    err = SockBuildTdiAddress(&bindInfo->Address, (PSOCKADDR)SocketAddress, SocketAddressLength);
    if (err != NO_ERROR) {
        goto exit;
    }

    // Call AFD to perform the actual bind operation.  AFD will open a
    // TDI address object through the proper TDI provider for this socket.
    ...
    if (status == STATUS_PENDING) {
        ...
        SockWaitForSingleObject(tlsData->EventHandle, Handle, SOCK_NEVER_CALL_BLOCKING_HOOK, SOCK_NO_TIMEOUT);
        SockAcquireSocketLockExclusive(socket);
    ...
    err = SockBuildSockaddr(socket->LocalAddress, &SocketAddressLength, &tdiAddressInfo->Address);
    if (err != NO_ERROR) {
        goto exit;
    }

#if DBG
    if ((socket->ExclusiveAddressUse || (sockaddrInfo.EndpointInfo != SockaddrEndpointInfoWildcard && !socket->ReuseAddresses)) &&
        (tdiAddressInfo->ActivityCount > 1)) {
        WS_PRINT(("WSPBind: Asked for unique address but tdi address info"
                  " activity count is %ld for ", tdiAddressInfo->ActivityCount));
...
exit:
    IF_DEBUG(BIND)
    {
        if (err != NO_ERROR) {
            ...
            WS_PRINT(("    WSPBind on socket %lx (%lx), granted", Handle, socket));
            WsPrintSockaddr(socket->LocalAddress, &socket->LocalAddressLength);
        }
    }

    // Perform cleanup--dereference the socket if it was referenced, free allocated resources.
    if (socket != NULL) {
        SockReleaseSocketLock(socket);
