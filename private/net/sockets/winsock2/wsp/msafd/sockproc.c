...
PSOCKET_INFORMATION SockpImportHandle(IN SOCKET Handle);
...
INT SockBuildSockaddr(OUT PSOCKADDR Sockaddr, OUT PINT SockaddrLength, IN PTRANSPORT_ADDRESS TdiAddress)
{
    __try {
        WS_ASSERT(sizeof(TdiAddress->Address[0].AddressType) == sizeof(Sockaddr->sa_family));
        ...
        *SockaddrLength = TdiAddress->Address[0].AddressLength + sizeof(Sockaddr->sa_family);
...
INT SockBuildTdiAddress(OUT PTRANSPORT_ADDRESS TdiAddress, IN PSOCKADDR Sockaddr, IN INT SockaddrLength)
{
    __try {
        WS_ASSERT(sizeof(TdiAddress->Address[0].AddressType) == sizeof(Sockaddr->sa_family));
        WS_ASSERT(FIELD_OFFSET(TA_ADDRESS, AddressLength) == 0);
        ...
        TdiAddress->Address[0].AddressLength = (USHORT)(SockaddrLength - sizeof(Sockaddr->sa_family));
...
VOID SockDestroySocket(IN PSOCKET_INFORMATION Socket)
/*++
Routine Description:
...
#if DBG
    {
        LPWSHANDLE_CONTEXT  ctx = WahReferenceContextByHandle(SockContextTable, Socket->HContext.Handle);
        if (ctx != NULL) {
            PSOCKET_INFORMATION aSocket = CONTAINING_RECORD(ctx, SOCKET_INFORMATION, HContext);
...
#ifdef _AFD_SAN_SWITCH_
    {
        ...
        if (switchSocket && (switchSocket->RefSocket == Socket)) {
#if DBG
            {
                ...
                // We don't decrement NumberReceivesPosted upon out-of-order recv, so take that into account
            ...
            // Synchronize deletion of tracker with CheckForHungLargeSends.
            // Must do before removing socket from SockSanOpen/ClosingList
            if (switchSocket->Tracker && InterlockedIncrement(&switchSocket->Tracker->TrackerDeletionCount) == 2) {
                FREE_HEAP(switchSocket->Tracker);
            }

            // Remove from SockSanClosingList. Even if mltiple listening sockets, only one is on the SockSanClosingList
            EnterCriticalSection(&SockSanListCritSec);
            RemoveEntryList(&switchSocket->SockListEntry);
            // Delete SockSanListCritSec if this is last SAN socket and WSPCleanup has already been called
            if (IsListEmpty(&SockSanOpenList) && IsListEmpty(&SockSanClosingList) && SockSanDeleteListCritSec) {
                deleteCS = TRUE;
            }
            LeaveCriticalSection(&SockSanListCritSec);
            ...
            // Delete each one. Note: listening sockets do not have flow control initialized,
            // so there's no need to call any of the other cleanup calls above.
            while (switchSocket) {
...
PSOCKET_INFORMATION SockFindAndReferenceSocket(IN SOCKET Handle, IN BOOLEAN AttemptImport)
/*++
Routine Description:
    Looks up a socket in the global socket table, and references it if found.
Arguments:
    Handle - NT system handle of the socket to locate.
    AttemptImport - if the socket isn't currently valid in this process, 
                    this parameter specifies whether we should attempt to import the handle into this process.
Return Value:
    PSOCKET_INFORMATION - a referenced pointer to a socket structure, or NULL if none was found that matched the specified handle.
--*/
{
...
PSOCKET_INFORMATION SockpImportHandle(IN SOCKET Handle)
{
    ...
    // Take the import lock, so no other thread can do the same operation while we are at it.
    ...
    // If this succeeds, then it is most likely true that the handle is valid for this process.
    ...
    // Now allocate memory to hold the socket context and get the actual context for the socket.
    ...
    // We have obtained the necessary context for the socket.  The context information is structured as follows:
    ...
    error = SockGetTdiName(&addressFamily,
                           &socketType,
                           &protocol,
                           0,
                           0,
    ...
    newSocketLength = (ULONG)(ALIGN_8(sizeof(*newSocket)) + (ALIGN_8(helperDll->MaxSockaddrLength) * 2));
    ...
    IF_DEBUG(SOCKET)
    {
        WS_PRINT(("Retreived socket info from AFD, handle %lx of type %s\n",
                  newSocket->Handle,
                  (newSocket->SocketType == SOCK_DGRAM ? "SOCK_DGRAM" : (newSocket->SocketType == SOCK_STREAM ? "SOCK_STREAM" : "SOCK_RAW"))));
    }
    // Initialize various fields in the socket information structure.

    // Note that the reference count is initialized to 2 to account for base reference and reference requested by the caller
    ...
    newSocket->RemoteAddress = (PVOID)ALIGN_8((PUCHAR)newSocket->LocalAddress + helperDll->MaxSockaddrLength);
    ...
    RtlCopyMemory(newSocket->LocalAddress, contextPtr, helperDll->MaxSockaddrLength);
    contextPtr += helperDll->MaxSockaddrLength;
    RtlCopyMemory(newSocket->RemoteAddress, contextPtr, helperDll->MaxSockaddrLength);
    contextPtr += helperDll->MaxSockaddrLength;
    ...
    error = newSocket->HelperDll->WSHSetSocketInformation(newSocket->HelperDllContext,
                                                          newSocket->Handle,
    ...
    if (ctx == NULL) {
        ...
        SockDereferenceSocket(CONTAINING_RECORD(ctx, SOCKET_INFORMATION, HContext));
        ctx = &newSocket->HContext;
    }

    // If the socket has AsyncSelect events set up, set them up for this process.
    // BUGBUG
    ...
    if (newSocket->AsyncSelectlEvent) {
        ...
        result = WSPAsyncSelect(newSocket->Handle, newSocket->AsyncSelecthWnd, newSocket->AsyncSelectwMsg, newSocket->AsyncSelectlEvent, &error);
...
INT SockSetHandleContext(IN PSOCKET_INFORMATION Socket)
{
    NTSTATUS status;
    PVOID context;
    PCHAR contextPtr;
    ...
    // Copy over information to the context buffer.  The context buffer has the following format:
    ...
    *(PULONG)contextPtr = helperDllContextLength;
    ...
    error = Socket->HelperDll->WSHGetSocketInformation(Socket->HelperDllContext,
                                                       Socket->Handle,
                                                       Socket->TdiAddressHandle,
                                                       Socket->TdiConnectionHandle,
                                                       SOL_INTERNAL,
    ...
    status = NtDeviceIoControlFile(
        ...
        0);
    if (status == STATUS_PENDING) {
        SockWaitForSingleObject(tlsData->EventHandle, Socket->Handle, SOCK_NEVER_CALL_BLOCKING_HOOK, SOCK_NO_TIMEOUT);
        status = ioStatusBlock.Status;
    }
...
BOOL SockWaitForSingleObject(IN HANDLE Handle, IN SOCKET SocketHandle, IN DWORD BlockingHookUsage, IN DWORD TimeoutUsage)
/*++
Routine Description:
    Does an alertable wait on the specified handle.  If the wait completes due to an alert, it rewaits.
Arguments:
    Handle - NT system handle to wait on.
    SocketHandle - the socket handle on which we're performing the IO we're waiting for.  This is necessary to support WSACancelBlockingCall().
    BlockingHookUsage - indicates whether to call the thread's blocking hook.  Possible values are:
            SOCK_ALWAYS_CALL_BLOCKING_HOOK - blocking hook is always called if blocking is necessary.
            SOCK_CONDITIONALLY_CALL_BLOCKING_HOOK - blocking hook is called if the socket is blocking (i.e. not a nonblocking socket).
            SOCK_NEVER_CALL_BLOCKING_HOOK - blocking hook is never called.
    TimeoutUsage - determines whether to wait infinitely or for a timeout.  Possible values are:
            SOCK_NO_TIMEOUT - wait forever for the handle to be signalled.
            SOCK_SEND_TIMEOUT - use the socket's send timeout value as a timeout.
            SOCK_RECEIVE_TIMEOUT - use the socket's receive timeout value as a timeout.
Return Value:
    BOOL - TRUE if the object was signalled within the appropriate timeout, and FALSE if the timeout occurred first.
--*/
{
    NTSTATUS status;
    ...
    // First wait for the object for a little while.
    // This handles the usual case where the object is already signalled or is signalled shortly into the wait.
    // We'll only go through the longer, more complex path if we're going to have to wait longer.
    ...
    // If we need to extract information from the socket, get a pointer to the socket information structure.
    ...
    // Determine whether we need to call the blocking hook while we're waiting.
    switch (BlockingHookUsage) {
    case SOCK_ALWAYS_CALL_BLOCKING_HOOK:

        // We'll assume (for now) that we'll need to call the blocking hook.
        // If we later determine that there is no blocking hook installed, then we obviously cannot call it...
    ...
    case SOCK_CONDITIONALLY_CALL_BLOCKING_HOOK:
        ...
        // (Later we'll determine if there is really a blocking hook installed.)
    ...
    // Determine if there's really a blocking hook installed.
    // If the upcall fails, we'll just press on regardless.
    if (callBlockingHook == TRUE) {
        ...
        result = (SockUpcallTable->lpWPUQueryBlockingCallback)(socket->CatalogEntryId, &blockingCallback, &blockingContext, &error);
        if (result == SOCKET_ERROR) {
            WS_PRINT(("SockWaitForSingleObject: WPUQueryBlockingCallback failed %d\n", error));
        }
    ...
    // Dereference the socket if we got a pointer to the socket information structure.
    if (socket != NULL) {
    ...
    // Calculate the end time we'll use when waiting on the handle.
    // The end time is the time at which we must quit waiting on the handle and must instead return from this function.
    if (useTimeout) {
        // The end time if the current time plus the timeout.  Query the current time.
        status = NtQuerySystemTime(&currentTime);
        WS_ASSERT(NT_SUCCESS(status));
        endTime.QuadPart = currentTime.QuadPart + timeout.QuadPart;
    } else {
        // We need an infinite timeout.  Set the end time to the largest possible time in NT format.
        endTime.LowPart = 0xFFFFFFFF;
        endTime.HighPart = 0x7FFFFFFF;
    }

    // If we're going to be calling a blocking hook, set up a minimal timeout since we have to call the blocking hook instead of idly waiting.
    // If we won't be calling the blocking hook, then we'll wait until the end time.
    if (callBlockingHook) {
    ...
    // Boolean so that we can tell whether the IO has been cancelled, and remember the socket handle on which we're doing the IO.
    tlsData->IoCancelled = FALSE;
    tlsData->SocketHandle = SocketHandle;

    do {
        // If necessary, call the blocking hook function until it returns FALSE.
        // This gives the routine the oppurtunity to process all the available messages before we complete the wait.
        if (callBlockingHook) {
        ...
        // If the operation was cancelled, reset the timeout to infinite and wait for the cancellation.
        // We don't want to call the blocking hook after the IO is cancelled.
        if (tlsData->IoCancelled) {
            timeout.LowPart = 0xFFFFFFFF;
            timeout.HighPart = 0x7FFFFFFF;
        } else {
            // Determine whether we have exceeded the end time.
            // If we have exceeded the end time then we must not wait any longer.
            status = NtQuerySystemTime(&currentTime);
            WS_ASSERT(NT_SUCCESS(status));
    ...
    } while (status == STATUS_USER_APC || status == STATUS_ALERTED || status == STATUS_TIMEOUT);

    tlsData->SocketHandle = INVALID_SOCKET;// Reset thread variables.

    // Return TRUE if the wait's return code was success; otherwise, we had to timeout the wait so return FALSE.
    if (status == STATUS_SUCCESS) {
        return TRUE;
    } else {
        return FALSE;
...
BOOL SockDefaultBlockingHook(VOID)
{
    MSG msg;
    ...
    return retrievedMessage;// If we got a message, indicate that we want to be called again.
} // SockDefaultBlockingHook


BOOLEAN SockIsSocketConnected(IN PSOCKET_INFORMATION Socket)
{
    ...
    // If there is a connect in progress, 
    // check if it has already completed but has not been processed by async thread and attempt to process it here
    while ((Socket->AsyncConnectContext != NULL) && (Socket->AsyncConnectContext->IoStatusBlock.Status != STATUS_PENDING)) {
        SockReleaseSocketLock(Socket);
        status = NtRemoveIoCompletion(SockAsyncQueuePort, (PVOID *)&proc, &context, &ioStatus, (PLARGE_INTEGER)&Wait0);
        WS_ASSERT(NT_SUCCESS(status));
        if (status == STATUS_SUCCESS) {
            if (proc != ASYNC_TERMINATION_PROC) {
                SockHandleAsyncIndication(proc, context, &ioStatus);
            } else {
                status = NtSetIoCompletion(SockAsyncQueuePort, ASYNC_TERMINATION_PROC, (PVOID)-1, 0, 0);
                WS_ASSERT(NT_SUCCESS(status));
    ...
    if (Socket->State == SocketStateConnected) {// Check whether the socket is already connected.
        return TRUE;
    }
...
INT SockGetTdiHandles(IN PSOCKET_INFORMATION Socket)
{
    ...
    // ** Because this routine can be called at APC level from ConnectCompletionApc(), IOCTL_AFD_QUERY_HANDLES must never pend.
...
INT SockGetInformation(IN PSOCKET_INFORMATION Socket,
                       ...
                       IN OUT PLARGE_INTEGER LargeInteger OPTIONAL)
{
    ...
    if (ARGUMENT_PRESENT(AdditionalInputInfo) && (AdditionalInputInfoLength > 0)) {
        ...
        // If there is additional input information, copy it to the input buffer.
    ...
    afdInfo->InformationType = InformationType;// Set up the AFD information block.   
    status = NtDeviceIoControlFile( // Set the blocking mode to AFD.
                                   Socket->HContext.Handle,
                                   tlsData->EventHandle,
                                   NULL,                      // APC Routine
                                   NULL,                      // APC Context
                                   ...
                                   afdInfo,
                                   sizeof(*afdInfo));
    if (status == STATUS_PENDING) {// Wait for the operation to complete.
        SockWaitForSingleObject(tlsData->EventHandle, Socket->Handle, SOCK_NEVER_CALL_BLOCKING_HOOK, SOCK_NO_TIMEOUT);
        status = ioStatusBlock.Status;
    }
...
INT SockSetInformation(IN PSOCKET_INFORMATION Socket,
                       IN ULONG InformationType,
...
int SockEnterApiSlow(OUT PWINSOCK_TLS_DATA    *tlsData)
{
...
VOID WsPrintSockaddr(IN PSOCKADDR Sockaddr, IN PINT SockaddrLength)
{
#if DBG
    if (Sockaddr == NULL) {
...
#include <wsnetbs.h>
        ...
        DbgPrint("), port 0x%lx, type 0x%lx\n", sockaddrNb->snb_name[NETBIOS_NAME_LENGTH - 1], sockaddrNb->snb_type);
        return;
    }
    default:
    {
...
VOID WsEnterApiCall(IN PCHAR RoutineName, IN PVOID Arg1, IN PVOID Arg2, IN PVOID Arg3, IN PVOID Arg4)
{
    ...
    // If this thread has not been initialized, do it now.
    // This is duplicated in SockEnterApi(), but we need it here to access SockIndentLevel below.
    if (tlsData == NULL) {
...
VOID WsExitApiCall(IN PCHAR RoutineName, IN LONG_PTR ReturnValue, IN BOOLEAN Failed, IN INT error)
{
    ULONG i;
    PWINSOCK_TLS_DATA    tlsData;
    tlsData = GET_THREAD_DATA();

    if (SockProcessTerminating || tlsData == NULL) {
        SetLastError(error);
        return;
    }
...
LONG SockExceptionFilter(LPEXCEPTION_POINTERS ExceptionPointers, LPSTR SourceFile, LONG LineNumber)
{
    ...
    __try {
        // Exceptions should never be thrown in a properly functioning system, so this is bad.
        // To ensure that someone will see this, forcibly enable debugger output if none of the output bits are enabled.
        if ((WsDebug & (WINSOCK_DEBUG_CONSOLE | WINSOCK_DEBUG_FILE | WINSOCK_DEBUG_DEBUGGER)) == 0) {
        ...
        WS_PRINT(("SockExceptionFilter: exception %08lx @ %08lx, caught in %s:%d\n",
...
VOID WINAPI SockIoCompletion(PVOID ApcContext, PIO_STATUS_BLOCK IoStatusBlock, DWORD Reserved)
/*++
Routine Description:
    This procedure is called to complete WSARecv, WSARecvFrom, WSASend, WSASendTo, and WSAIoctl asynchronous I/O operations.
    Its primary function is to extract the appropriate information from the passed IoStatusBlock and call the user's completion routine.

    The users completion routine is called as:

        Routine Description:

            When an outstanding I/O completes with a callback, this function is called.
            This function is only called while the thread is in an alertable wait (SleepEx, WaitForSingleObjectEx, or WaitForMultipleObjectsEx with the bAlertable flag set to TRUE).
            Returning from this function allows another pendiong I/O completion callback to be processed.
            If this is the case, this callback is entered before the termination of the thread's wait with a return code of WAIT_IO_COMPLETION.

            Note that each time your completion routine is called, the system uses some of your stack.
            If you code your completion logic to do additional ReadFileEx's and WriteFileEx's within your completion routine,
            AND you do alertable waits in your completion routine, you may grow your stack without ever trimming it back.

        Arguments:

            dwErrorCode - Supplies the I/O completion status for the related I/O.
                A value of 0 indicates that the I/O was successful.
                Note that end of file is indicated by a non-zero dwErrorCode value of ERROR_HANDLE_EOF.

            dwNumberOfBytesTransfered - Supplies the number of bytes transfered during the associated I/O.
                If an error occured, a value of 0 is supplied.

            lpOverlapped - Supplies the address of the WSAOVERLAPPED structure used to initiate the associated I/O.
                The hEvent field of this structure is not used by the system and may be used by the application to provide additional I/O context.
                Once a completion routine is called, the system will not use the WSAOVERLAPPED structure.
                The completion routine is free to deallocate the overlapped structure.

Arguments:
    ApcContext - Supplies the users completion routine. The format of this routine is an LPWSAOVERLAPPED_COMPLETION_ROUTINE.
    IoStatusBlock - Supplies the address of the IoStatusBlock that contains the I/O completion status.
                    The IoStatusBlock is contained within the WSAOVERLAPPED structure.
...
--*/
{
    ...
    __try {
        if (NT_ERROR(IoStatusBlock->Status)) {
            ...
            // Set up the ReceiveFlags output parameter based on the type of receive.
    ...
    (CompletionRoutine)(dwErrorCode, dwNumberOfBytesTransfered, lpOverlapped, dwFlags);
...
BOOL SockThreadInitialize(VOID)
{
    ...
    status = NtCreateEvent(&threadEvent, EVENT_ALL_ACCESS, NULL, NotificationEvent, FALSE);
    if (!NT_SUCCESS(status)) {
    ...
    if (!SET_THREAD_DATA(tlsData)) {
        ...
        // If TlsSetValue failed, we are in big trouble anyway since this can only occur if someone hajacked and freed our TLS slot.
        // In that case we leak all the thread slots.  One more won't make a difference.
        return FALSE;
    }
...
INT SockIsAddressConsistentWithConstrainedGroup(IN PSOCKET_INFORMATION Socket,
                                                IN GROUP Group,
                                                IN PSOCKADDR SocketAddress,
                                                IN INT SocketAddressLength)
    /*++
    Routine Description:
        Searches all open sockets, validating that the specified address is consistent with all sockets associated to the specified constrained group identifier.
    Arguments:
        Socket - An open socket handle. Used just to get use "into" AFD where the real work is done.
    ...
    --*/
{
    ...
    // Allocate enough space to hold the TDI address structure we'll pass to AFD.
    // Note that is the address is small enough, we just use an automatic in order to improve performance.
    validateInfo = (PAFD_VALIDATE_GROUP_INFO)validateInfoBuffer;
    validateInfoLength = sizeof(AFD_VALIDATE_GROUP_INFO) - sizeof(TRANSPORT_ADDRESS) + Socket->HelperDll->MaxTdiAddressLength;
    ...
    err = SockBuildTdiAddress(&validateInfo->RemoteAddress, SocketAddress, SocketAddressLength);
    if (err != NO_ERROR)
        return err;

    validateInfo->GroupID = (LONG)Group;// Let AFD do the dirty work.
    status = NtDeviceIoControlFile(
        Socket->HContext.Handle,
        tlsData->EventHandle,
        NULL,                  // APC Routine
...
VOID SockCancelIo(IN SOCKET Socket)
/*++
Routine Description:
...
--*/
{
    ...
    status = NtCancelIoFile((HANDLE)Socket, &ioStatusBlock);
    WS_ASSERT(status != STATUS_PENDING);
    if (!NT_SUCCESS(status) || !NT_SUCCESS(ioStatusBlock.Status)) {
        WS_PRINT(("SockCancelIo: NtCancelIoFile() failed, %08lx:%08lx\n", status, ioStatusBlock.Status));
    }
}   // SockCancelIo


INT SockpGetProtocolInfoArray(VOID)
{
    DWORD               sz;
    INT                 err, res;
...
INT SockBuildProtocolInfoForSocket(IN PSOCKET_INFORMATION Socket, OUT LPWSAPROTOCOL_INFOW ProtocolInfo)
{
    INT     i, err;
...
// WriterWaitEvent is used to unwait the writer by the last reader who owned the lock.
NTSTATUS SockInitializeRwLockAndSpinCount(PSOCK_RW_LOCK   Lock, ULONG           SpinCount)
/*++
Routine Description:
...
Return Value:
    Status of the operation (event allocation or undelying critical section initialization status).
--*/
{
    NTSTATUS    status;

    if (SpinCount & 0x80000000) {
        status = NtCreateEvent(&Lock->WriterWaitEvent, EVENT_ALL_ACCESS, NULL, SynchronizationEvent, FALSE);
        if (!NT_SUCCESS(status)) {
            return status;
        }
    }
...
#endif
#if DEBUG_LOCKS
        Lock->DebugInfo = RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(SOCK_RW_LOCK_DEBUG_INFO)*MAX_RW_LOCK_DEBUG);
        Lock->DebugInfoIdx = -1;
#endif
...
NTSTATUS SockDeleteRwLock(PSOCK_RW_LOCK   Lock)
/*++
Routine Description:
...
VOID SockAcquireRwLockShared(PSOCK_RW_LOCK   Lock)
/*++
Routine Description:
    Acquire socket RW lock in shared mode.
    Allows for recursive acquire if lock is already owned shared or exclusive by the current thread.
Arguments:
    Lock    - pointer to initialized SOCK_RW_LOCK structure.
--*/
{
    ...
    do {
        ...
        if (count < 0) {
            // Writer is active, wait for it by acquiring critical section.
            // This will also prevent us from starvation if another writer comes after us.
            EnterCriticalSection(&Lock->Lock);
            lockAcquired = TRUE;

            // Re-read the counter, it is likely that it has changed while we were waiting for critical section
            count = Lock->ReaderCount;

            if (count < 0) {
                // This is a recursive acquire while holding writer lock in the same thread.
                WS_ASSERT(count < -1);
                WS_ASSERT(Lock->WriterId == NtCurrentTeb()->ClientId.UniqueThread);
                newCount = count - 1;
            } else {// Normal acquire, writer is gone.
                newCount = count + 1;
            }
        } else {// No active writers, get it.
            newCount = count + 1;
        }

        // Change reader count.
        newCount = InterlockedCompareExchange((LPLONG)&Lock->ReaderCount, newCount, count);
        if (lockAcquired) {// Release critical section if we acquired it
    ...
    if (Lock->DebugInfo != NULL) {
        ...
        RtlGetCallersAddress(&Lock->DebugInfo[idx].Caller, &Lock->DebugInfo[idx].CallersCaller);
    }
#endif
} // SockAcquireRwLockShared


VOID SockReleaseRwLockShared(PSOCK_RW_LOCK   Lock)
/*++
Routine Description:
    Release shared ownership of the socket RW lock.
Arguments:
    Lock    - pointer to initialized SOCK_RW_LOCK structure previously acquired in shared mode
--*/
{
    while (1) {
        ...
        if (count > 0) {// No writers active, decrement the count
            newCount = count - 1;
        } else {
            WS_ASSERT(count != 0);
            newCount = count + 1;// Writer is active, increment count.
        }

        if (InterlockedCompareExchange((LPLONG)&Lock->ReaderCount, newCount, count) == count) {
#if DEBUG_LOCKS
            if (Lock->DebugInfo != NULL) {
                INT idx = InterlockedIncrement(&Lock->DebugInfoIdx) % MAX_RW_LOCK_DEBUG;
...
VOID SockpWaitForReaders(PSOCK_RW_LOCK   Lock)
/*++
Routine Description:
...
Return Value:
    None.
    This routine raises an exception if it cannot allocate event or wait on event fails.
NOTE:
...
--*/
{
    ...
    // We must be dealing with stuburn reader or priority inversion problem.
    if (Lock->WriterWaitEvent == NULL) {
    ...
    // Some readers are still in, tell them we need a signal and wait.
...
VOID SockAcquireRwLockExclusive(PSOCK_RW_LOCK   Lock)
/*++
Routine Description:
    Acquire socket RW lock exclusively.
    Allows for recursive acquire if lock is already owned exclusively by the current thread.
Arguments:
    Lock    - pointer to initialized SOCK_RW_LOCK structure.
Return Value:
    None.
...
--*/
{
    ...
    EnterCriticalSection(&Lock->Lock);// Guarantee mutual exclusion
    ...
    if (Lock->ReaderCount >= 0) {
        ...
        do {
            ...
            // Readers expect the counter to be negative when writer is waiting,
            // hence -1 to cover the case of 0 readers and -1 to have the room to decrement before reaching 0.
            newCount = -count - 2;
        } while (InterlockedCompareExchange((LPLONG)&Lock->ReaderCount, newCount, count) != count);
        ...
        if (newCount != -2) {
            ...
            while (Lock->ReaderCount != -2) {
                WS_ASSERT(Lock->ReaderCount <= -2);
                if (spinCount > 0) {// Spin if we have set spin count.
                    spinCount--;
                } else {// Some readers are still in, use the slow path.
...
VOID SockReleaseRwLockExclusive(PSOCK_RW_LOCK   Lock)
/*++
Routine Description:
...
--*/
{
    ...
    LeaveCriticalSection(&Lock->Lock); // Release the mutual exclusion (critical section maintains its own recursion count).
