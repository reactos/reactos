...
typedef struct _SOCK_SHARED_INFO
{
    ...
    // We need these when creating accept()ed sockets.
    DWORD CreationFlags;
    ...
    // Last error set on this socket, used to report the status of non-blocking connects.
    INT LastError;
...
// These structures are stored in the generic table SocketTable.
// Different fields of this structure are filled in during different APIs.
typedef struct _SOCKET_INFORMATION
{
    union
    {
        // Caveat. The order of fields in the the structures of union should match!!!
...
// Set the following to 1 to enable critical section debugging, even on retail builds (SET USER_C_FLAGS=/DDEBUG_LOCKS=1).
