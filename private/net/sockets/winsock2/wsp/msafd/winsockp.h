...
#define MAX_FAST_AFD_POLL           (sizeof(AFD_POLL_INFO) + 3*sizeof(AFD_POLL_HANDLE_INFO))
...
VOID WsExitApiCall(IN PCHAR RoutineName, IN LONG_PTR ReturnCode, IN BOOLEAN Failed, IN INT error);
...
// Note that the per-thread data is always allocated from the process heap, never from any private heap created for this DLL.
// This prevents a nasty problem where an app loads the winsock DLLs (creating the private
...
#define ALIGN_8(count)             (ALIGN_UP( (ULONG_PTR)(count), 8 ))

// All available multipoint flags.
#define ALL_MULTIPOINT_FLAGS ( WSA_FLAG_MULTIPOINT_C_ROOT | WSA_FLAG_MULTIPOINT_C_LEAF | WSA_FLAG_MULTIPOINT_D_ROOT | WSA_FLAG_MULTIPOINT_D_LEAF )
