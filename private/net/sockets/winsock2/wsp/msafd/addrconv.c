...
PWINSOCK_HELPER_DLL_INFO SockFindHelperDllForAddressFamily(INT AddressFamily);
BOOL SockIsAddressFamilySupported(IN PWINSOCK_MAPPING Mapping, IN INT AddressFamily);
...
int WSPAPI WSPAddressToString(LPSOCKADDR lpsaAddress,
    ...
    Routine Description:
        ...
        readable string representation of the address. This is used mainly for display purposes.
    Arguments:
        ...
        lpdwAddressStringLength - The length of the AddressString buffer. Returns the length of the string actually copied into the buffer.
        lpErrno - A pointer to the error code.
    Return Value:
        If no error occurs, WSPAddressToString() returns 0.
    ...
    --*/
{
    ...
    if (helperDll == NULL || helperDll->WSHAddressToString == NULL) {
    ...
    *lpErrno = helperDll->WSHAddressToString(lpsaAddress, dwAddressLength, lpProtocolInfo, lpszAddressString, lpdwAddressStringLength);
...
int WSPAPI WSPStringToAddress(LPWSTR AddressString,
    ...
    /*++
    Routine Description:
        This routine converts a human-readable string to a socket address structure (SOCKADDR) suitable for pass to Windows Sockets routines which
        take such a structure. Any missing components of the address will be
        defaulted to a reasonable value if possible. For example, a missing port number will be defaulted to zero.
    Arguments:
        AddressString - Points to the zero-terminated human-readable string to convert.
        AddressFamily - The address family to which the string belongs, or AF_UNSPEC if it is unknown.
    ...
    --*/
{
    ...
    // Find a helper DLL supporting this address family.
    helperDll = SockFindHelperDllForAddressFamily(AddressFamily);
    if (helperDll == NULL || helperDll->WSHStringToAddress == NULL) {
        *lpErrno = WSA_INVALID_PARAMETER;
        return SOCKET_ERROR;
    }
    ...
    *lpErrno = helperDll->WSHStringToAddress(AddressString, AddressFamily, lpProtocolInfo, lpAddress, lpAddressLength);
    return (*lpErrno == 0) ? NO_ERROR : SOCKET_ERROR;
}   // WSPStringToAddress
...
walkList:
    for (listEntry = SockHelperDllListHead.Flink; listEntry != &SockHelperDllListHead; listEntry = listEntry->Flink) {
        ...
        if (SockIsAddressFamilySupported(helperDll->Mapping, AddressFamily)) {
            // Found a match.
            if (resourceShared) {
    ...
    // If we've not already done so, try again with exclusive access to the helper dll list (since another thread may have loaded
    // the helper dll we want after we've released the resource), then if we still can't find a match drop thru to code below
    if (resourceShared) {
        SockReleaseRwLockShared(&SocketGlobalLock);
    ...
    // specified address family.  First get the REG_MULTI_SZ that contains the list of transports that have winsock support.
    ...
    for (currentTransport = transportList; *currentTransport != UNICODE_NULL; currentTransport += wcslen(currentTransport) + 1) {
        ...
        if (SockIsAddressFamilySupported(mapping, AddressFamily)) {
            // The address family is supported.  Load the helper DLL for the transport.
