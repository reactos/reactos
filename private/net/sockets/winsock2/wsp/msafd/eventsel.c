...
Abstract:
    This module contains support for the WSAEventSelect() and WSAEnumNetworkEvents() WinSock APIs.
...
int WSPAPI WSPEventSelect(SOCKET Handle, WSAEVENT hEventObject, long lNetworkEvents, LPINT lpErrno)
/*++
Routine Description:
    Does that event select thang.   CKMBUGBUG
Arguments:
    Handle - A descriptor identifying the socket for which event notification is required.
    hEventObject - A handle identifying the event object which should be signalled when a network event occurs.
...
--*/
{
    ...
    err = SockEventSelectHelper(socket, hEventObject, lNetworkEvents);
...
int SockEventSelectHelper(PSOCKET_INFORMATION Socket, WSAEVENT hEventObject, long lNetworkEvents)
{
    ...
    // throughout this routine to synchronize against other callers performing operations on the socket we're using.
    SockAcquireSocketLockExclusive(Socket);
    ...
    // Send the IOCTL to AFD.  AFD will reference the event object and store the poll events in its internal endpoint structure.
...
int WSPAPI WSPEnumNetworkEvents(SOCKET Handle, WSAEVENT hEventObject, LPWSANETWORKEVENTS lpNetworkEvents, LPINT lpErrno)
/*++
Routine Description:
...
Arguments:
    ...
    lpNetworkEvents - Points to a WSANETWORKEVENTS structure which records an occurred network event and the associated error code.
Return Value:
    The return value is 0 if the application's declaration of interest in the network event set was successful.
    Otherwise the value SOCKET_ERROR is returned, and a specific error number may be retrieved by calling WSAGetLastError().
--*/
{
    ...
    // information on network events that have occurred.  AFD will then reset the event object (if specified).
    status = NtDeviceIoControlFile(
        (HANDLE)Handle,
        tlsData->EventHandle,
        NULL,                      // APC Routine
    ...
    __try {        
        lpNetworkEvents->lNetworkEvents = 0;// Zero the WSANETWORKEVENTS structure.       
        pollMapping = PollEventMapping; // Interpret the results.

        for (i = 0; i < NUM_POLL_MAPPINGS; i++) {
            if (eventInfo.PollEvents & (1 << pollMapping->PollEventBit)) {
                lpNetworkEvents->lNetworkEvents |= (1 << pollMapping->NetworkEventBit);
                eventStatus = eventInfo.EventStatus[pollMapping->PollEventBit];
                if (!NT_SUCCESS(eventStatus)) {
                    lpNetworkEvents->iErrorCode[pollMapping->NetworkEventBit] = SockNtStatusToSocketError(eventStatus);
                } else {
        ...
        if (eventInfo.PollEvents & AFD_POLL_CONNECT) {
            ...
            if (!NT_SUCCESS(eventStatus)) {
                lpNetworkEvents->iErrorCode[FD_CONNECT_BIT] = SockNtStatusToSocketError(eventStatus);
            } else {
        ...
        if (eventInfo.PollEvents & AFD_POLL_ABORT) {
            ...
            if (!NT_SUCCESS(eventStatus)) {
                lpNetworkEvents->iErrorCode[FD_CLOSE_BIT] = SockNtStatusToSocketError(eventStatus);
            } else {
