...
Abstract:
    This module contains code for the WinSock asynchronous processing thread.
    It is necessary to have this as a separate thread because of postprocessing required for WSAAsyncSelect and WSAConnect on non-blocking sockets.
    We cannot use application threads as we are not guaranteed that such threads wait alertably (so we can run APCs in them)

Author:
    David Treadwell (davidtr)    25-May-1992

Revision History:
    Vadim Eydelman  (vadime)    Summer 1998.  Rewrote to use completion port instead of APCs to remove one context switch
--*/
...
DWORD SockAsyncThread(IN PVOID Param);


BOOLEAN SockCheckAndReferenceAsyncThread(VOID)
/*
    Initializes async thread if not already running and references it to make sure it won't go away until IO is completed
*/
{
    NTSTATUS status;
    HANDLE threadHandle;
    ...
    do {
        // If thread has already been initialized, just increment the reference count, otherwise start it.
        count = (LONG volatile)SockAsyncThreadReferenceCount;
        WS_ASSERT(count >= 0);
        if ((count > 0) && (InterlockedCompareExchange(&SockAsyncThreadReferenceCount, (count + 1), count) == count)) {
            return TRUE;
        }
    } while (count > 0);
    ...
    if (SockAsyncQueuePort == NULL) {
        ...
        // This can happen if we are cleaned up but not unloaded before this call is made.
        ...
        status = NtSetInformationObject(SockAsyncQueuePort, ObjectHandleFlagInformation, &HandleInfo, sizeof(HandleInfo));
    ...
    // go away unexpectedly. We'll remove this reference when we shut down the async thread.
    instance = NULL;

    if (GetModuleFileName(SockModuleHandle, moduleFileName, sizeof(moduleFileName) / sizeof(moduleFileName[0]))) {
        instance = LoadLibrary(moduleFileName);
    }

    if (instance == NULL) {
        SockReleaseRwLockExclusive(&SocketGlobalLock);
        WS_PRINT(("SockCheckAndInitAsyncThread: LoadLibrary failed: %ld\n", GetLastError()));
        return FALSE;
    }
    ...
    threadHandle = CreateThread(NULL, 0, SockAsyncThread, (PVOID)tlsData, 0, &threadId);
    ...
    // Thread creation succeeded, add reference to make sure that the thread is running while it is referenced.
...
DWORD SockAsyncThread(IN PVOID Param)
{
    ...
    BOOL                    res;
    static const LONGLONG   IdleWaitTimeout = -(((LONGLONG)MAX_ASYNC_THREAD_IDLE_TIME) * 10000000i64);
    ...
    // Setup per-thread data. We must do this "manually" because this thread doesn't go through the normal SockEnterApi() routine.
    ...
    do {
        status = NtRemoveIoCompletion(SockAsyncQueuePort, (PVOID *)&proc, &context, &ioStatus, (PLARGE_INTEGER)&IdleWaitTimeout);
        WS_ASSERT(NT_SUCCESS(status));
        if (status == STATUS_SUCCESS) {
            if (proc != ASYNC_TERMINATION_PROC) {
                SockHandleAsyncIndication(proc, context, &ioStatus);
            } else {
                // We were asked to terminate, treat this like a timeout to avoid additional wait
                status = STATUS_TIMEOUT;
                SockDereferenceAsyncThread();
            }
        } else {
            if (SockAsyncThreadReferenceCount > 1 && NT_ERROR(status)) {
    ...
    // Continue while we have not hit a timeout or have more pending items.
    while (((status != STATUS_TIMEOUT) && (SockWspStartupCount > 0)) || InterlockedCompareExchange(&SockAsyncThreadReferenceCount, 0, 1) != 1);
    ...
    if (SockWspStartupCount > 0) {
        ...
        status = NtQueryIoCompletion(SockAsyncQueuePort, IoCompletionBasicInformation, &iocInfo, sizeof(iocInfo), NULL);
        WS_ASSERT(NT_SUCCESS(status));
...
VOID SockHandleAsyncIndication(PASYNC_COMPLETION_PROC  Proc, PVOID Context, PIO_STATUS_BLOCK IoStatus)
{
#ifdef _AFD_SAN_SWITCH_
    if ((Proc == SockAsyncConnectCompletion)
        ...
        if (Proc == SockSanCompletion) {
            if (Context == &SockSanAddressNotifyContext) {                
                SockSanProviderAddressUpdate();// PnP notification
            } else {// accept() completion from AFD                
                SockSanCompletion(Context, IoStatus);
            }
        } else {// ReadFile/WriteFile indirection            
            SockSanRedirectRequest((PVOID)Proc, Context, IoStatus);
        }
    }
#else
    WS_ASSERT((Proc == SockAsyncConnectCompletion) || (Proc == SockAsyncSelectCompletion) || (Proc == SockProcessQueuedAsyncSelect));
    (*Proc) (Context, IoStatus);
#endif
}
...
VOID SockTerminateAsyncThread(VOID)
{
    ...
    // Close helper endpoints first so that completion indication for all outstanding selects/connects complete first.
    if (SockAsyncSelectHelperHandle != NULL) {
        Handle = SockAsyncSelectHelperHandle;
        SockAsyncSelectHelperHandle = NULL;
        // UnProtect our handle from being closed by random NtClose call by some messed up dll or application
    ...
    if (SockAsyncQueuePort != NULL) {
        LONG    count;
        do {
            // If thread has already been initialized, then increment reference count and post work item to terminate the thread.
            count = (LONG volatile)SockAsyncThreadReferenceCount;
            WS_ASSERT(count >= 0);
            if ((count > 0) && (InterlockedCompareExchange(&SockAsyncThreadReferenceCount, (count + 1), count) == count)) {
