...
// GETSOCKNAME_HACK turns on a hack where instead of returning INADDR_ANY for AF_INET sockets, it returns one of ths host's actual IP addresses.

#define GETSOCKNAME_HACK 0

#if GETSOCKNAME_HACK
IN_ADDR GetHostIpAddress(VOID);
#endif


int WSPAPI WSPGetPeerName(IN SOCKET Handle, OUT struct sockaddr * SocketAddress, OUT int *SocketAddressLength, OUT LPINT lpErrno)
/*++
Routine Description:
    getpeername() retrieves the name of the peer connected to the socket s and stores it in the struct sockaddr identified by name.
...
--*/
{
    ...
    SockAcquireSocketLockExclusive(socket);// Acquire the lock that protects the socket.

    // If the socket is not in a connected state then this is not a legal request.
    ...
    __try {
        ...
        RtlCopyMemory(SocketAddress, socket->RemoteAddress, socket->RemoteAddressLength);// Copy over the remote socket's name into the output buffer.
...
exit:
    IF_DEBUG(GETNAME)
    {
        if (err != NO_ERROR) {
            ...
            WS_PRINT(("WSPGetPeerName on socket %lx (%lx) addr", Handle, socket));
...
int WSPAPI WSPGetSockName(IN SOCKET Handle, OUT struct sockaddr *SocketAddress, OUT int *SocketAddressLength, OUT LPINT lpErrno)
/*++
Routine Description:
    getsockname() retrieves the current name for the specified socket descriptor in name.
    ...
    this call provides the only means by which you can determine the local association which has been set by the system.
...
--*/
{
    ...
    UCHAR tdiAddressInfoBuffer[MAX_FAST_TDI_ADDRESS + FIELD_OFFSET(TDI_ADDRESS_INFO, Address)];
    ...
    tdiAddressInfo = NULL;// Initialize locals so we know how to clean up on exit.
    ...
    // If the socket has not been bound, then this is not a valid call.
    ...
    if (SockSanEnabled && socket->SanSocket && socket->SanSocket->IsConnected == CONNECTED) {
        __try {
            if (*SocketAddressLength < (int)socket->LocalAddressLength) {
                ...
                RtlMoveMemory(SocketAddress, socket->LocalAddress, socket->LocalAddressLength);
...
tryTCP:
    ...
    tdiAddressInfoLength = socket->HelperDll->MaxTdiAddressLength + FIELD_OFFSET(TDI_ADDRESS_INFO, Address);
    ...
    // It is possible on some transports to partially specify an address, in which case the transport will assign an address for use.
    // This IOCTL obtains the real address for the endpoint.
    // Note that we can't just use the address stored in the socket, as this can change due to a connect() or other operation.
    ...
    // Convert the actual address that was bound to this socket to the sockaddr format in order to store it.
    SockBuildSockaddr(socket->LocalAddress, &socket->LocalAddressLength, &tdiAddressInfo->Address);
    __try {
        // Make sure that the specified address buffer is large enough.
        ...
        RtlCopyMemory(SocketAddress, socket->LocalAddress, socket->LocalAddressLength);// Copy over the socket's address into the output buffer.
...
exit:
    IF_DEBUG(GETNAME)
    {
        if (err != NO_ERROR) {
            ...
            WS_PRINT(("WSPGetSockName on socket %lx (%lx) addr", Handle, socket));
...
IN_ADDR GetHostIpAddress(VOID)
{
    ...
    error = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Services\\TcpIp\\Linkage", 0, KEY_READ, &tcpipKey);
    if (error != NO_ERROR) {
        IF_DEBUG(GETNAME)
        {
    ...
    error = RegQueryValueExW(tcpipKey, L"Bind", NULL, &type, NULL, (LPDWORD)&bindListLength);
    ...
    error = RegQueryValueExW(tcpipKey, L"Bind", NULL, &type, (PVOID)bindList, &bindListLength);
    if (error != NO_ERROR) {
        IF_DEBUG(GETNAME)
        {
    ...
    error = RegOpenKeyExW(HKEY_LOCAL_MACHINE, adapterRegKeyName, 0, KEY_READ, &adapterKey);
    if (error != NO_ERROR) {
        IF_DEBUG(GETNAME)
        {
    ...
    error = RegQueryValueExA(adapterKey, "IPAddress", NULL, &type, (PVOID)ipAddress, &ipAddressLength);
    if (error != NO_ERROR) {
        IF_DEBUG(HELPER_DLL)
        {
