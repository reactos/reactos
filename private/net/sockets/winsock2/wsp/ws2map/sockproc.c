...
Abstract:
    This module contains socket management code for the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
// Private prototypes.
INT WINAPI SockBlockingHook(VOID);
...
/*++
Routine Description:
    This routine maps an "API visible" (i.e. WinSock 2) handle to the corresponding SOCKET_INFORMATION structure.
Arguments:
    Socket - The WinSock 2 socket handle to map.
Return Value:
    PSOCKET_INFORMATION - A pointer to the sockets SOCKET_INFORMATION structure if successful, NULL otherwise.
--*/
{
    INT result;
    INT err;
    ...
    try {
        ...
        result = SockUpcallTable.lpWPUQuerySocketHandleContext(Socket,(PULONG_PTR)&socketInfo,&err);
...
/*++
Routine Description:
    This routine maps a "downlevel" (i.e. WinSock 1) handle to the corresponding SOCKET_INFORMATION structure.
Arguments:
...
--*/
{
    ...
    SockAcquireGlobalLock();
    try {
        // We cannot use the WS2_32.DLL upcalls to map the WinSock 1 handle, so we'll need to scan the global linked list.
        for( listEntry = SockGlobalSocketListHead.Flink ; listEntry != &SockGlobalSocketListHead ; listEntry = listEntry->Flink ) {
            socketScan = CONTAINING_RECORD(listEntry,SOCKET_INFORMATION,SocketListEntry);
...
/*++
Routine Description:
    Dereferences the specified socket. If the reference count drops to zero, removes the socket from the global list and frees its resources.
Arguments:
    ...
    try {
        ...
        if( SocketInfo->ReferenceCount == 0 ) {
            ...
            if( SocketInfo->DeferSocket != INVALID_SOCKET ) {
                WSPCloseSocket(SocketInfo->DeferSocket, &err);
            }
...
Return Value:
    PSOCKET_INFORMATION - Pointer to the newly created socket if successful, NULL if not. If NULL, then an error code may be found in *Error.
--*/
{
    PSOCKET_INFORMATION socketInfo;
    SOCKET ws2Handle;
    ...
    // Grab the global lock. We must hold this until the socket is created and put on the global list.
    SockAcquireGlobalLock();

    try {
        // Create a new socket structure.
        socketInfo = SOCK_ALLOCATE_HEAP(sizeof(*socketInfo) + MaxSockaddrLength);
        if( socketInfo == NULL ) {
            err = WSAENOBUFS;
            leave;
        }
        ...
        // Initialize it.
        ZeroMemory(socketInfo, sizeof(*socketInfo) + MaxSockaddrLength);
        ...
        socketInfo->ShutdownEvent = CreateEvent(NULL,FALSE,FALSE,NULL);
        if( socketInfo->ShutdownEvent == NULL ) {
            err = WSAENOBUFS;
            leave;
        }
        ...
        ws2Handle = SockUpcallTable.lpWPUCreateSocketHandle(CatalogEntryId,(ULONG_PTR)socketInfo,&err);
        ...
        InsertHeadList(&SockGlobalSocketListHead,&socketInfo->SocketListEntry);
        SOCK_ASSERT( err == NO_ERROR );
    } except( SOCK_EXCEPTION_FILTER() ) {
    ...
    try {
        if( err == NO_ERROR ) {
            IF_DEBUG(SOCKET) {
                SOCK_PRINT(("Created socket %lx:%lx (%08lx) from hooker %08lx\n",ws2Handle,WS1Handle,socketInfo,HookerInfo));
            }
        } else {
            if( ws2Handle != INVALID_SOCKET ) {
                INT dummy;
                SockUpcallTable.lpWPUCloseSocketHandle(ws2Handle,&dummy);
            }
...
/*++
Routine Description:
    Prepares a hooker for possible blocking hook use. This routine is called before any hooker entrypoint that may invoke the blocking hook.
Arguments:
...
--*/
{
    ...
    result = SockUpcallTable.lpWPUQueryBlockingCallback(SocketInfo->CatalogEntryId,&tlsData->BlockingCallback,&tlsData->BlockingContext,&err);
    if( result == SOCKET_ERROR ) {
        SOCK_PRINT(("SockPrepareForBlockingHook: cannot query blocking callback: %d\n",err));
    ...
    // If there's been a change in state (meaning we have not previously set a blocking hook for this thread and now we need one, OR we have previously set a blocking hook for this thread and now we don't need one) the send the update request to the hooker.
    if( tlsData->BlockingHookInstalled && tlsData->BlockingCallback == NULL ) {
        ...
        if( result == SOCKET_ERROR ) {
            ...
            SOCK_ASSERT( err != NO_ERROR );
            SockPostApiCallout();
            SOCK_PRINT(("SockPrepareForBlockingCallback: cannot unhook blocking hook: %d\n",err));
        } else {
            SockPostApiCallout();
    ...
    if( !tlsData->BlockingHookInstalled && tlsData->BlockingCallback != NULL ) {
        ...
        result2 = SocketInfo->Hooker->WSASetBlockingHook((FARPROC)SockBlockingHook);
        if( result2 == NULL ) {
            err = SocketInfo->Hooker->WSAGetLastError();
            SOCK_ASSERT( err != NO_ERROR );
            SockPostApiCallout();
            SOCK_PRINT(("SockPrepareForBlockingCallback: cannot set blocking hook: %d\n",err));
        } else {
            SockPostApiCallout();
...
/*++
Routine Description:
    Prepares the current thread for a callout to a hooker entrypoint.
    This routine is called before every hooker entrypoint.
    Currently, the only thing done here is to set the per-thread reentrancy flag.
--*/
{
    PSOCK_TLS_DATA tlsData;
...
/*++
Routine Description:
    Undoes any preparation performed in SockPreApiCallout().
    This routine is called after every hooker entrypoint.
    Currently, the only thing done here is to clear the per-thread reentrancy flag.
--*/
{
    PSOCK_TLS_DATA tlsData;
...
/*++
Routine Description:
    Blocking hook routine set in the hooker.
    If the current thread really has blocking hook installed, then we set the hooker's blocking hook to point to this routine.
    This allows us to make the appropriate upcall to the WS2_32.DLL for blocking hook processing.
Return Value:
    INT - TRUE if there are more messages to process, FALSE otherwise. (We always return FALSE.)
--*/
{
    INT result;
    PSOCK_TLS_DATA tlsData;
...
/*++
Routine Description:
    Builds an appropriate WSAPROTOCOL_INFOW structure for the given socket.
Arguments:
