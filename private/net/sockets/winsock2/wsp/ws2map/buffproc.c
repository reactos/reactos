...
Abstract:
    This module contains buffer managment routines for the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
Return Value:
    DWORD - The total byte length of the buffers described by the buffer array.
--*/
{
    DWORD totalLength;
...
--*/
{
    ...
    while( BufferCount > 0 && FlatBufferLength > 0 ) {
        ...
        CopyMemory(destination, BufferArray->buf, bytesToCopy);
...
/*++
Routine Description:
    Copies the data in a scatter/gather buffer array to a flat (linear) buffer.
Arguments:
...
--*/
{
    ...
    while( BufferCount > 0 && FlatBufferLength > 0 ) {
        bytesToCopy = min( BufferArray->len, FlatBufferLength );
        CopyMemory(BufferArray->buf, source, bytesToCopy);
        source += bytesToCopy;
        FlatBufferLength -= bytesToCopy;
        BufferArray++;
        BufferCount--;
