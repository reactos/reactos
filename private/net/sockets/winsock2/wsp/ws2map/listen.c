...
Abstract:
    This module contains listen routines for the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
Routine Description:
    ...
    the connections are accepted with WSPAccept. 
    WSPListen() applies only to sockets that are connection-oriented (e.g.,  SOCK_STREAM). 
    ...
    queue full, the client will receive an error with an indication of WSAECONNREFUSED.

    WSPListen() should continue to function rationally when there are no available descriptors. 
    It should accept connections until the queue is emptied. 
    If descriptors become available, a later call to WSPListen() or
    ...
    A WinSock SPI client may call WSPListen() more than once on the same socket. 
    This has the effect of updating the current backlog for the listening socket. 
    Should there be more pending connections than the new backlog value, the excess pending connections will be reset and dropped.
...
--*/
{
    ...
    if( socketInfo == NULL || socketInfo->State == SocketStateClosing ) {
        IF_DEBUG(LISTEN) {
            SOCK_PRINT(("WSPListen failed on %s handle: %lx\n", socketInfo == NULL ? "unknown" : "closed", s));
        }
    ...
    SockPreApiCallout();// Let the hooker do its thang.

    result = socketInfo->Hooker->listen(socketInfo->WS1Handle, backlog);
