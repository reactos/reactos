...
// Private prototypes.
DWORD WINAPI SockWorkerThreadStarter(LPVOID lpThreadParameter);
DWORD WINAPI SockHookerThreadStarter(LPVOID lpThreadParameter);
BOOL SockAddDllReference(VOID);
...
--*/
{
    ...
    // Create the thread.
    threadHandle = CreateThread(lpThreadAttributes, dwStackSize, &SockWorkerThreadStarter, context, dwCreationFlags, lpThreadId);
    if( threadHandle == NULL && context != NULL ) {
        DWORD err;

        // CreateThread() failed. Destroy the context before returning, but be careful to preserve LastError.
        err = GetLastError();
...
Routine Description:
    ...
    context, call the original starting point, then exit via the special FreeLibraryAndExitThread() API.
...
Routine Description:
    ...
    point for the thread. We unpack the necessary data from the passed-in
    context, mark the current thread (if it's a hooker thread), then call the original starting point.
Arguments:
    lpThreadParameter - Actually points to a SOCK_THREAD_STARTUP_CONTEXT
        structure that contains the original thread entrypoint and parameter.
Return Value:
...
--*/
{
    ...
    return routine( param );// Let the original routine do its thang.
}   // SockHookerThreadStarter
...
Routine Description:
    ...
    detached from the process's address space while we have worker threads active.
Arguments:
    None.
Return Value:
