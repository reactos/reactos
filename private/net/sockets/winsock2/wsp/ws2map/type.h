...
Abstract:
    This module contains global type definitions for the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
typedef struct _SOCK_TLS_DATA {
    BOOL ReentrancyFlag;// Reentrancy flag. Keeps us from getting reentered on the same thread.
    BOOL IoCancelled;// Set if the current IO request has been cancelled.
...
typedef struct _SOCKET_INFORMATION {
    LIST_ENTRY SocketListEntry;// Linked-list linkage.
    LONG ReferenceCount;// Reference count.
    SOCKET WS2Handle;// The 2.x handle.
    SOCKET WS1Handle;// The 1.1 handle.
    SOCK_STATE State;// Socket state.
    ...
    // is called on this socket so that any worker threads can cleanup and exit cleanly.
    ...
    // creating this socket. We need these when creating accept()ed sockets.
...
typedef struct _HOOKER_INFORMATION {
    LIST_ENTRY HookerListEntry;// Linked-list linkage.
    LONG ReferenceCount;// Reference count.

    // This boolean gets set to TRUE only after the hooker is fully initialized (meaning that the target DLL's WSAStartup() has been called).
    // This is to prevent a nasty recursion when DLLs such as RWS insist on creating sockets from *within* their WSAStartup() routines...

    BOOL Initialized;

    HINSTANCE DllHandle;// Handle to the hooker's DLL.
    GUID ProviderId;// The provider GUID generated for this hooker.
    ...
    // after loading the hooker to ensure the hooker's internal state is fully initialized.
    SOCKET AnchorSocket;

    // The maximum datagram size, retrieved from the WSADATA structure returned from WSAStartup().
