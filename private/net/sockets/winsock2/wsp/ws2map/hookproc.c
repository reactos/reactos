...
Abstract:
    This module contains hooker management code for the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
PHOOKER_INFORMATION SockFindAndReferenceHooker(IN LPGUID ProviderId)
/*++
Routine Description:
    This routine maps the given provider GUID to the corresponding HOOKER_INFORMATION structure.
    If the specified hooker has already been loaded (and therefore has an entry in the global list) its structure pointer is simply returned.
    Otherwise, an attempt is made to load the necessary information from the registry.
Arguments:
    ProviderId - A pointer to the GUID identifying a hooker.
Return Value:
    PHOOKER_INFORMATION - Pointer to the hooker structure if successful, NULL otherwise.
--*/
{
    ...
    SOCK_ASSERT( ProviderId != NULL );// Sanity check.

    // Setup locals so we know how to cleanup on exit.
    str = NULL;
    dllHandle = NULL;

    SockAcquireGlobalLock();// Scan the in-memory hooker list.

    try {
        for( listEntry = SockHookerListHead.Flink ; listEntry != &SockHookerListHead ; listEntry = listEntry->Flink ) {
            hookerInfo = CONTAINING_RECORD(listEntry,HOOKER_INFORMATION,HookerListEntry);
        ...
        hookerInfo = NULL;// Prepare for early exit.

        // Open the registry key if not already open.
        if( SockHookerRegistryKey == NULL ) {
            err = RegOpenKeyEx(HKEY_LOCAL_MACHINE,SOCK_HOOKER_GUID_MAPPER_KEY,0,KEY_READ,&SockHookerRegistryKey);
            if( err != NO_ERROR ) {
                SOCK_PRINT(("SockFindAndReferenceHooker: cannot open %s, error %ld\n",SOCK_HOOKER_GUID_MAPPER_KEY,err));
                leave;
            }
        }
        ...
        if( status != RPC_S_OK ) {
            SOCK_PRINT(("SockFindAndReferenceHooker: cannot map GUID to string, error %d\n",status));
            leave;
        }

        // Try to load the DLL path from the registry.
        length = sizeof(dllPath);
        err = RegQueryValueEx(SockHookerRegistryKey,str,NULL,&type,(LPBYTE)dllPath,&length);
        if( err != NO_ERROR ) {
            SOCK_PRINT(("SockFindAndReferenceHooker: cannot query %s, error %ld\n",str,err));
            leave;
        }

        // Expand any embedded environment strings.
        length = ExpandEnvironmentStrings(dllPath,expandedDllPath,sizeof(expandedDllPath));
        if( length == 0 ) {
            err = GetLastError();
            SOCK_PRINT(("SockFindAndReferenceHooker: cannot expand %s, error %ld\n",dllPath,err));
            leave;
        }

        dllHandle = LoadLibrary( expandedDllPath );// Load the DLL.
        if( dllHandle == NULL ) {
            err = GetLastError();
            SOCK_PRINT(("SockFindAndReferenceHooker: cannot load %s, error %ld\n",expandedDllPath,err));
            leave;
        }

        hookerInfo = SOCK_ALLOCATE_HEAP( sizeof(*hookerInfo) );// Allocate a new hooker.
        if( hookerInfo == NULL ) {
            leave;
        }

        ZeroMemory(hookerInfo,sizeof(*hookerInfo));
        ...
        if( hookerInfo->accept == NULL ||
            ...
            SOCK_PRINT(("SockFindAndReferenceHooker: cannot find entrypoints in %s\n",expandedDllPath));
            SOCK_FREE_HEAP( hookerInfo );
            hookerInfo = NULL;
            leave;
        }

        // Go ahead and put it on the global list, but with the "Initialized" flag set to FALSE.
        // We don't set this to TRUE until after we've completed the hooker's initialization.
        hookerInfo->ProviderId = *ProviderId;
        SOCK_ASSERT( !hookerInfo->Initialized );
        InsertHeadList(&SockHookerListHead,&hookerInfo->HookerListEntry);

        // Initialize it.

        // Note that the hooker starts out with a reference count of two;
        // one for the "active" reference, and one for the reference added by this routine.
        SockPreApiCallout();
        result = hookerInfo->WSAStartup(MAKEWORD(1,1),&wsaData);
        if( result != NO_ERROR ) {
            SOCK_PRINT(("SockFindAndReferenceHooker: %s:WSAStartup() failed, error %d\n",expandedDllPath,result));
        ...
        hookerInfo->MaxUdpDatagramSize = (INT)wsaData.iMaxUdpDg;// Save the max UDP datagram size for future reference.

        // Create and bind the anchor socket.
        hookerInfo->AnchorSocket = hookerInfo->socket(AF_INET,SOCK_STREAM,IPPROTO_TCP);
        if( hookerInfo->AnchorSocket != INVALID_SOCKET ) {
            SOCKADDR_IN addr;

            ZeroMemory(&addr,sizeof(addr));
            addr.sin_family = AF_INET;
            hookerInfo->bind(hookerInfo->AnchorSocket,(SOCKADDR *)&addr,sizeof(addr));
        }
        ...
        // Success!
        IF_DEBUG(HOOKER) {
            SOCK_PRINT(("SockFindAndReferenceHooker: loaded %s [%s] @ %08lx\n",str,dllPath,hookerInfo));
        }
...
VOID SockReferenceHooker(IN PHOOKER_INFORMATION HookerInfo)
/*++
Routine Description:
    This routine references the given HOOKER_INFORMATION structure.
Arguments:
    HookerInfo - A pointer to the HOOKER_INFORMATION structure to reference.
--*/
{
    SOCK_ASSERT( HookerInfo != NULL );// Sanity check.
    SockAcquireGlobalLock();// Reference the hooker, protected by a lock.
...
VOID SockDereferenceHooker(IN PHOOKER_INFORMATION HookerInfo)
/*++
Routine Description:
    This routine dereferences the given HOOKER_INFORMATION structure.
    If the reference count drops to zero, all resources associated with the structure are released.
Arguments:
    HookerInfo - A pointer to the HOOKER_INFORMATION structure to dereference.
--*/
{
    INT result;
    INT err;
    ...
    SOCK_ASSERT( HookerInfo != NULL );// Sanity check.
    SockAcquireGlobalLock();// Dereference the hooker, protected by a lock.
    ...
    try {
        ...
        if( HookerInfo->ReferenceCount == 0 ) {
            // Shut 'er down, Scotty.
            if( HookerInfo->AnchorSocket != INVALID_SOCKET ) {
                result = HookerInfo->closesocket(HookerInfo->AnchorSocket);
                if( result == SOCKET_ERROR ) {
                    err = HookerInfo->WSAGetLastError();
                    SOCK_PRINT(("SockDereferenceHooker: closesocket() failed, error %d\n",err));
                    // Press on regardless...
                }
            }
            ...
            if( result == SOCKET_ERROR ) {
                err = HookerInfo->WSAGetLastError();
                SOCK_PRINT(("SockDereferenceHooker: WSACleanup() failed, error %d\n",err));
                // Press on regardless...
            }

            // Remove the hooker from the global list, then free the hooker's resources.
            RemoveEntryList( &HookerInfo->HookerListEntry );
...
VOID SockFreeAllHookers(VOID)
/*++
Routine Description:
    This routine frees all allocated HOOKER_INFORMATION structures and closes the hooker registry key.
--*/
{
    PLIST_ENTRY listEntry;
    PHOOKER_INFORMATION hookerInfo;

    SockAcquireGlobalLock();// Scan the in-memory hooker list and free 'em.

    try {
        for( listEntry = SockHookerListHead.Flink ; listEntry != &SockHookerListHead ; ) {
            hookerInfo = CONTAINING_RECORD(listEntry,HOOKER_INFORMATION,HookerListEntry);
            SockDereferenceHooker( hookerInfo );// Dereference the hooker.
        }
