...
Abstract:
    This module contains the DNS RNR support routines for the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
struct hostent * CopyHostEntry(struct hostent * phent, PBYTE pbAllocated, LONG  lSizeOf, PLONG plTaken, BOOL  fOffsets);
PDNS_RNR_CONTEXT RnR2GetContext(HANDLE hContext);
PDNS_RNR_CONTEXT RnR2MakeContext(IN HANDLE hContext, IN DWORD  dwExtra);
BOOL IsLocalComputerName(IN LPGUID ProviderId, IN LPSTR AnsiName);
VOID RnR2Cleanup(VOID);
LPSTR GetAnsiNameRnR(IN PWCHAR Name, IN LPSTR  Domain);
DWORD FetchPortFromClassInfo(IN DWORD dwType, IN    LPGUID lpType, IN LPWSASERVICECLASSINFOW lpServiceClassInfo);
VOID RnR2ReleaseContext(IN PDNS_RNR_CONTEXT pdrc);
DWORD PackCsAddr(IN PHOSTENT HostEntry, IN DWORD UdpPort, IN DWORD TcpPort, IN OUT LPVOID lpCsaddrBuffer, IN OUT PLONG lplBufferLength, OUT LPDWORD lpdwBytesTaken, IN OUT LPDWORD Count, BOOL fReversi);
DWORD AllocateUnicodeString(IN LPSTR lpAnsi, IN OUT PWCHAR *lppUnicode);
DWORD GetServerAndProtocolsFromString(PHOOKER_INFORMATION HookerInfo, PWCHAR lpszString, LPGUID lpType, struct servent ** pServEnt);
INT WINAPI NSPLookupServiceBegin(IN  LPGUID lpProviderId, IN  LPWSAQUERYSETW lpqsRestrictions, IN  LPWSASERVICECLASSINFOW lpServiceClassInfo, IN  DWORD dwControlFlags, OUT LPHANDLE lphLookup);
INT WINAPI NSPLookupServiceNext(IN     HANDLE hLookup, IN     DWORD dwControlFlags, IN OUT LPDWORD lpdwBufferLength, OUT    LPWSAQUERYSETW lpqsResults);
INT WINAPI NSPUnInstallNameSpace(IN LPGUID lpProviderId);
INT WINAPI NSPCleanup(IN LPGUID lpProviderId);
INT WINAPI NSPLookupServiceEnd(IN HANDLE hLookup);
INT WINAPI NSPSetService(IN  LPGUID lpProviderId, IN  LPWSASERVICECLASSINFOW lpServiceClassInfo, IN LPWSAQUERYSETW lpqsRegInfo, IN WSAESETSERVICEOP essOperation, IN DWORD dwControlFlags);
INT WINAPI NSPInstallServiceClass(IN  LPGUID lpProviderId, IN LPWSASERVICECLASSINFOW lpServiceClassInfo);
INT WINAPI NSPRemoveServiceClass(IN  LPGUID lpProviderId, IN LPGUID lpServiceCallId);
INT WINAPI NSPGetServiceClassInfo(IN  LPGUID lpProviderId, IN OUT LPDWORD lpdwBufSize, IN OUT LPWSASERVICECLASSINFOW lpServiceClassInfo);
...
DWORD GetServerAndProtocolsFromString(PHOOKER_INFORMATION HookerInfo, PWCHAR lpszString, LPGUID lpType, struct servent ** pServEnt)
{
    ...
    if (lpszString && lpType && (GUIDS_ARE_EQUAL(lpType, &HostnameGuid) || GUIDS_ARE_EQUAL(lpType, &InetHostName))) {
        ...
        // the string is  of the form service/protocol.
        // If there is no protocol, just look up the name and take the first entry.
        ...
        for (pszTemp = servname; *pszTemp && isdigit(*pszTemp); pszTemp++);
...
)
{
    ...
    // Make sure that the buffer is large enough to hold all the entries which will be necessary.
    ...
    *lpdwBytesTaken = requiredBufferLength;
    ...
    while (nAddresses--) {
        ...
        for (i = 0; i < count; i++, csaddrInfo++, sockaddrIn++) {
            // First fill in the local address.
            // It should remain basically all zeros except for the family so that it is a "wildcard" address for binding.
            ...
            if (fReversi) {
                ...
                csaddrInfo->RemoteAddr.lpSockaddr = csaddrInfo->LocalAddr.lpSockaddr;
...
LPSTR GetAnsiNameRnR(IN PWCHAR Name, IN LPSTR  Domain)
{
    ...
    if (!WideCharToMultiByte(CP_ACP, 0, Name, -1, pszBuffer, dwLen, 0, 0)) {
...
PDNS_RNR_CONTEXT RnR2MakeContext(IN HANDLE hHandle, IN DWORD  dwExtra)
/*++
Routine Description:
...
--*/
{
    ...
    pdrc = (PDNS_RNR_CONTEXT)AllocLocal(sizeof(DNS_RNR_CONTEXT) + dwExtra);
    if (pdrc) {
        pdrc->lInUse = 2;
...
PDNS_RNR_CONTEXT RnR2GetContext(IN  HANDLE Handle)
{
    /*++
    Routine Description:
        This routine checks the existing DNS contexts to see if we have one for this call.
    Arguments:
...
DWORD TryFetchClass(IN   LPWSASERVICECLASSINFOW lpServiceClassInfo, IN   PWCHAR     pwszMatch)
{
    /*++
    Routine Description:
       Helper routine to rummage through the Class Info entries looking for the desired one.
       If none is found, return NOPORTDEFINED.
       If one is found, return the value as a port number.
    --*/
    if (lpServiceClassInfo) {
        ...
        for (; dwNumClassInfos; dwNumClassInfos--, lpClassInfoBuf++) {
            if (!_wcsicmp(pwszMatch, lpClassInfoBuf->lpszName) && (lpClassInfoBuf->dwValueType == REG_DWORD) && (lpClassInfoBuf->dwValueSize >= sizeof(WORD))) {
...
--*/
{
    ...
    case UDP_PORT:
        if (IS_SVCID_UDP(lpType)) {
            ...
            dwPort = TryFetchClass(lpServiceClassInfo, SERVICE_TYPE_VALUE_UDPPORTW);
        }
        break;
    case TCP_PORT:
        if (IS_SVCID_TCP(lpType)) {
            ...
            dwPort = TryFetchClass(lpServiceClassInfo, SERVICE_TYPE_VALUE_TCPPORTW);
        }
...
INT WINAPI NSPLookupServiceBegin(
...
--*/
{
    ...
    if ((lpqsRestrictions->lpszContext && (lpqsRestrictions->lpszContext[0] != 0) && wcscmp(&lpqsRestrictions->lpszContext[0], L"\\")) || (dwControlFlags & LUP_CONTAINERS)) {
    ...
    hookerInfo = SockFindAndReferenceHooker(lpProviderId);
    if (hookerInfo == NULL) {
        err = WSA_INVALID_PARAMETER;
        SOCK_EXIT("NSPLookupServiceBegin", SOCKET_ERROR, &err);
        SetLastError(err);
    ...
    // If this is a lookup of the local name allow the instance name to be NULL. If it is a reverse lookup, mark it as such.
    // If it is a host lookup, check for one of the several special names that require returning specific local information.
    ...
    // Compute whether this is some sort of name lookup. Do this here since we've two places below that need to test for it.
    ...
    if (!pwszServiceName || !pwszServiceName[0]) {
        ...
        if (fNameLook) {
            ...
            if (!MultiByteToWideChar(CP_ACP, 0, pszAddr, -1, pwszServiceName, 1000)) {
        ...
        // it's some kind of name lookup. So, let's see if it is a special name
        PCHAR pszAnsiName = GetAnsiNameRnR(pwszServiceName, 0);

        if (!_stricmp(pszAnsiName, "localhost") || !_stricmp(pszAnsiName, "loopback")) {
    ...
    if (lpqsRestrictions->lpafpProtocols) {
        ...
        for (i = 0; dwNum--;) {
            if ((lpqsRestrictions->lpafpProtocols[i].iAddressFamily == AF_INET) || (lpqsRestrictions->lpafpProtocols[i].iAddressFamily == AF_UNSPEC)) {
    ...
    // Complete computation of protocols. We might have information in the query string that helps us, and if so, we need to fetch the servent for that specification.
    nProt &= GetServerAndProtocolsFromString(hookerInfo, lpqsRestrictions->lpszQueryString, lpqsRestrictions->lpServiceClassId, &sent);
    if (sent) {
        if (nProt & UDP_BIT) {
        ...
        if (nProt & UDP_BIT) {
            dwUdpPort = FetchPortFromClassInfo(UDP_PORT, lpqsRestrictions->lpServiceClassId, lpServiceClassInfo);
        } else {
            dwUdpPort = NOPORTDEFINED;
        }
        if (nProt & TCP_BIT) {
            dwTcpPort = FetchPortFromClassInfo(TCP_PORT, lpqsRestrictions->lpServiceClassId, lpServiceClassInfo);
        } else {
            dwTcpPort = NOPORTDEFINED;
        }
    }

    // if no protocol info so far, then this has to be a host name lookup of some sort or it is an error
    ...
    // one final check. If the name does not contain a dotted part, add the home domain to it.
    if (!(dwLocalFlags & (IANALOOK | LOCALLOOK)) && !wcschr(pwszServiceName, L'.')) {
    ...
    dwHostLen = (wcslen(pwszServiceName) + 1) * sizeof(WCHAR);
...
INT WINAPI NSPLookupServiceNext(
    IN     HANDLE          hLookup,
    IN     DWORD           dwControlFlags,
...
)
{
    /*++
    Routine Description:
        The continuation of LookupBegin.
        This tries to lookup the service based on the parameters in the context
    --*/
    INT err;
    PDNS_RNR_CONTEXT pdrc;
    ...
    if (pdrc->fFlags & IANALOOK) {
        ...
        // This is a lookup of an IANA service name and the caller wants the type returned.
        // The RNR conventions are that these names look like 21/ftp, that being something our bind code can't handle.
        // So, check for such a name and ignore the prefix port  for the lookup.
        // If the service cannot be found, then use that port and assume tcp.
        BOOL IsTcp;
        DWORD nProt;

        nProt = GetServerAndProtocolsFromString(pdrc->HookerInfo, pdrc->wcName, &HostnameGuid, &sent);
        if (!sent) {
        ...
        if (pdrc->dwControlFlags & LUP_RES_SERVICE) {
            ...
            // It's some form of host lookup and we don't yet have a saved hostent for it. See if we can get one
            if (pdrc->fFlags & REVERSELOOK) {
                DWORD dwIp = inet_addr(pAnsiName);

                SockPreApiCallout();
                hostEntry = pdrc->HookerInfo->gethostbyaddr((PCHAR)&dwIp, 4, AF_INET);
                if (hostEntry == NULL) {
                ...
                // if in the 2:1 mapper, just defer the call to someone else.
                SockPreApiCallout();
                hostEntry = pdrc->HookerInfo->gethostbyname(pAnsiName);
                if (hostEntry == NULL) {
                    err = pdrc->HookerInfo->WSAGetLastError();
                    SOCK_ASSERT(err != NO_ERROR);
                    SockPostApiCallout();
                ...
                // the caller is looking for information about the local machine.
                // So return either the localhostent information, the WINS form of the DNS information, or the real DNS information.
                // This is important to get right in order for apps to work properly
                INT result;
                CHAR szLocalHostName[256];   // allow plenty of room

                // in the 2:1 mapper we have to first get our local host name and then ask for the hostent structure.
                // It's really the "right" way to do this, but we take a  short-cut when we own the provider.
                // Note there's no way to know how much space to allocate, so if 1000 bytes is insufficient, it simply fails, but I suspect many other things fail we well.
                SockPreApiCallout();

                result = pdrc->HookerInfo->gethostname(szLocalHostName, sizeof(szLocalHostName));
            ...
            // copy the host entry so we can keep a local copy of it
            pdrc->phent = hostEntry = CopyHostEntry(hostEntry, 0, 0, &lSizeOf, FALSE);
        ...
        if (!lInstance) {
            ...
            if ((pdrc->dwControlFlags & LUP_RETURN_ALIASES) && hostEntry->h_aliases) {
                LONG x;
    ...
    if (pdrc->dwControlFlags & LUP_RETURN_ADDR) {
        ...
        err = PackCsAddr(hostEntry, pdrc->dwUdpPort, pdrc->dwTcpPort, (PVOID)pBuff, &lFree, &dwTaken, &lpqsResults->dwNumberOfCsAddrs, (pdrc->fFlags & REVERSELOOK) == 1);
        if (err == NO_ERROR) {
            pBuff += dwTaken;
        }
    }
    ...
    if (pdrc->dwControlFlags & LUP_RETURN_BLOB) {
        if (GUIDS_ARE_EQUAL(&pdrc->gdType, &InetHostName) || GUIDS_ARE_EQUAL(&pdrc->gdType, &AddressGuid)) {
            ...
            lFree -= sizeof(BLOB);
            pBuff += sizeof(BLOB);
            if (CopyHostEntry(hostEntry, pBuff, lFree, &lTaken, TRUE)) {
            ...
            lFree -= lTaken;
        } else if ((pdrc->fFlags & IANALOOK) && sent) {
            ...
            lFree -= sizeof(BLOB);
            pBuff += sizeof(BLOB);
            if (CopyServEntry(sent, pBuff, lFree, &lTaken, TRUE)) {
                lpqsResults->lpBlob->pBlobData = pBuff;
                lpqsResults->lpBlob->cbSize = lTaken;
                pBuff += lTaken;
            } else {
    ...
    if (pdrc->dwControlFlags & LUP_RETURN_NAME) {
        ...
        if (lFree < 0) {
            ...
            CopyMemory(pBuff, pszString, dwLen);
...
Done:
    ...
    if (err != NO_ERROR) {
        SetLastError(err);
        if (err == WSAEFAULT) {
            // If this is the error, lFree should be the  value -(extra bytes needed)
...
INT WINAPI NSPInstallServiceClass(
    IN  LPGUID               lpProviderId,
    IN LPWSASERVICECLASSINFOW lpServiceClassInfo
)
{
...
INT WINAPI NSPRemoveServiceClass(
    IN  LPGUID               lpProviderId,
...
INT WINAPI NSPGetServiceClassInfo(
    IN  LPGUID               lpProviderId,
...
INT WINAPI NSPStartup(IN LPGUID lpProviderId, IN OUT LPNSP_ROUTINE lpsnpRoutines)
{
    ...
    CopyMemory(lpsnpRoutines, &nsrVector, dwSize);
    return NO_ERROR;
}


// Two helper routines to assist CopyHostEntry.
// These compute the  space needed to  hold the alias and address structures respectively.
...
DWORD GetAliasSize(PCHAR * pal, PDWORD pdwCount)
{
    DWORD dwSize;
...
// Copy a hostent structure into a freshly allocate block of memory or into a provided block of size lSizeoOf.
...
// the copied hostent is placed in a contiguous chunk of memory so that if pbAllocated in non-NULL the first *plTaken bytes will have been used to hold the structure.

// Return:
//   0 -- insufficient memory to do the copy
//   != 0 -- the address of the new hostent
struct hostent * CopyHostEntry(
    struct hostent * phent,
    PBYTE pbAllocated,
    LONG lSizeOf,
    PLONG plTaken,
...
)
{
    ...
    dwSize = sizeof(struct hostent) + GetAliasSize(phent->h_aliases, &dwAlCount) + GetAddrSize(phent, &dwAddCount) + strlen(phent->h_name) + 1;
    if (!(pb = pbAllocated)) {
        pb = (PBYTE)AllocLocal(dwSize);
    } else {
        // align it first.
        // This is done to insure that if this space is within another buffer, as it will be when returning a BLOB through a LookupServiceNext, that the buffer is left aligned to hold a string.
        // Note it is not left aligned to hold a structure on the assumption that any structures are packed ahead of this.
        // If it is ever necessary to guarantee address alignment, replace the ALIGN_WORD with ALIGN_DWORD.
...
struct servent * CopyServEntry(struct servent * sent, PBYTE pbAllocated, LONG lSizeOf, PLONG plTaken, BOOL fOffsets)
{
    ...
    dwSize = sizeof(struct servent) + GetAliasSize(sent->s_aliases, &dwAlCount) + dwNameSize + strlen(sent->s_proto) + 1;
    if (!(pb = pbAllocated)) {
        pb = (PBYTE)AllocLocal(dwSize);
    } else {
        // align it first.
    ...
    *plTaken = (LONG)dwSize;
    ...
    if (pb) {
        ...
        // The layout in the string space is the aliases first, then the name, then the protocol string
...
BOOL IsLocalComputerName(IN LPGUID ProviderId, IN LPSTR AnsiName)
{
    ...
    result = hookerInfo->gethostname(name, sizeof(name));
    if (result == SOCKET_ERROR) {
        SockPostApiCallout();
        goto exit;
    }
