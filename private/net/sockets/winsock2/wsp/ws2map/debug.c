...
Abstract:
    ...
    This module contains debug-specific functions used by the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
// The (PCHAR) casts in the following macro force the compiler to assume only BYTE alignment.
...
LPSTR SockGetErrorString(IN INT Error);
...
VOID SockEnterApiCall(IN PCHAR RoutineName, IN PVOID Arg1, IN PVOID Arg2, IN PVOID Arg3, IN PVOID Arg4)
/*++
Routine Description:
...
*/
{
    ...
    IF_DEBUG(ENTER) {
        SOCK_PRINT(("---> %s() args 0x%lx 0x%lx 0x%lx 0x%lx\n",RoutineName,Arg1,Arg2,Arg3,Arg4));
    }
}   // SockEnterApiCall


VOID SockExitApiCall(IN PCHAR RoutineName,IN LONG_PTR ReturnCode,IN LPINT ErrorCode)
/*++
Routine Description:
    Displays the SPI entrypoint name and the completion status. This routine is called just before every SPI entrypoint returns.
Arguments:
    RoutineName - The name of the SPI entrypoint.
    ReturnCode - The return code for the entrypoint.
    ErrorCode - Points to an error code if this API is failing, NULL if the API is successful.
*/
{
    ...
    IF_DEBUG(EXIT) {
        if( ErrorCode != NULL ) {
            ...
            SOCK_PRINT(("<--- %s() FAILED--error %ld (0x%lx) == %s\n",RoutineName,*ErrorCode,*ErrorCode,errorString));
        } else {
            SOCK_PRINT(("<--- %s() returning %ld (0x%lx)\n",RoutineName,ReturnCode,ReturnCode));
        }
    }
}   // SockExitApiCall


VOID SockPrintf(char *Format, ...)
/*++
Routine Description:
...
*/
{
    ...
    IF_DEBUG(FILE) {
        if ( SockDebugFileHandle == INVALID_HANDLE_VALUE ) {
            SockDebugFileHandle = CreateFile(SockDebugFileName,GENERIC_READ | GENERIC_WRITE,FILE_SHARE_READ,NULL,CREATE_ALWAYS,0,NULL);
        }

        if ( SockDebugFileHandle == INVALID_HANDLE_VALUE ) {
            OutputDebugString("SockPrintf: Failed to open winsock debug log file\n");
        } else {
            length = strlen( OutputBuffer );
            ret = WriteFile(SockDebugFileHandle,(LPVOID )OutputBuffer,length,&length,NULL);
            if ( !ret ) {
                OutputDebugString("SockPrintf: file WriteFile failed\n");
            }
        }
    }
}   // SockPrintf
...
VOID SockAssert(IN PVOID FailedAssertion,IN PVOID FileName,IN ULONG LineNumber)
/*++
Routine Description:
    Assertion routine invoked when the parameter to SOCK_ASSERT or SOCK_REQUIRE evaluates to FALSE.
Arguments:
    FailedAssertion - The text of the failed assertion. This is the "stringized" version of the SOCK_ASSERT/SOCK_REQUIRE parameter.
...
/*++
Routine Description:
    A debug heap allocator. Keeps various statistics, and maintains all heap blocks on a global list.
Arguments:
...
Return Value:
    PVOID - Pointer to the newly allocated block if successful, NULL otherwise.
*/
{
    PSOCK_HEAP_HEADER header;
    ...
    header = HeapAlloc(SockHeap,0,NumberOfBytes + sizeof(*header) + sizeof(*tail));
    if ( header == NULL ) {
    ...
    SockCopyMemory(tail,&localTail,sizeof(localTail));
...
*/
{
    ...
    SockCopyMemory(&localTail,tail,sizeof(localTail));
    ...
    HeapFree(SockHeap,0,(LPVOID)header);
...
*/
{
    ...
    for ( listEntry = SockHeapListHead.Flink; listEntry != &SockHeapListHead; listEntry = listEntry->Flink ) {
        if ( listEntry == NULL ) {
            SockPrintf("listEntry == NULL, lastListEntry == %lx\n", lastListEntry);
            DebugBreak();
        }
        ...
        if ( header->HeapCode1 != WINSOCK_HEAP_CODE_1 ) {
            SockPrintf("SockCheckHeap, fail 1, header %lx tail %lx\n",header,tail);
            DebugBreak();
        }

        if ( header->HeapCode2 != WINSOCK_HEAP_CODE_2 ) {
            SockPrintf("SockCheckHeap, fail 2, header %lx tail %lx\n",header,tail);
            DebugBreak();
        }

        if ( localTail.HeapCode3 != WINSOCK_HEAP_CODE_3 ) {
            SockPrintf("SockCheckHeap, fail 3, header %lx tail %lx\n",header,tail);
            DebugBreak();
        }

        if ( localTail.HeapCode4 != WINSOCK_HEAP_CODE_4 ) {
            SockPrintf("SockCheckHeap, fail 4, header %lx tail %lx\n",header,tail);
            DebugBreak();
        }

        if ( localTail.HeapCode5 != WINSOCK_HEAP_CODE_5 ) {
            SockPrintf("SockCheckHeap, fail 5, header %lx tail %lx\n",header,tail);
            DebugBreak();
        }

        if ( localTail.Header != header ) {
            SockPrintf("SockCheckHeap, fail 6, header %lx tail %lx\n",header,tail);
            DebugBreak();
        }
...
LONG SockExceptionFilter(LPEXCEPTION_POINTERS ExceptionPointers,LPSTR SourceFile,LONG LineNumber)
/*++
Routine Description:
    An exception filter. This routine is invoked within the except() clause of a try/except block, giving it the opportunity to print useful debugging information about the exception.
Arguments:
    ExceptionPointers - Pointer to an EXCEPTION_POINTERS structure containing information about the exception.
    SourceFile - The name of the file that caught the exception.
    LineNumber - The line number where the exception was caught.
Return Value:
    LONG - One of the EXCEPTION_* values indicating the manner in which the exception should be handled. This routine always returns EXCEPTION_EXECUTE_HANDLER.
*/
{
    LPSTR fileName;
    ...
    try {
        // Exceptions should never be thrown in a properly functioning system, so this is bad. To ensure that someone will see this, forcibly enable debugger output.
        ...
        SOCK_PRINT(("SockExceptionFilter: exception %08lx @ %08lx, caught in %s:%d\n", ExceptionPointers->ExceptionRecord->ExceptionCode, ExceptionPointers->ExceptionRecord->ExceptionAddress, fileName, LineNumber));
