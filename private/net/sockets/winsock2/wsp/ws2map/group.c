...
Abstract:
    This module contains group management routines used by the Winsock 2 to Winsock 1.1 Mapper Service Provider.
...
--*/
{
    ...
    SockGroupDataMapping = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(*SockGroupData), SOCK_GROUP_MAPPING_NAME);
    if( SockGroupDataMapping == NULL ) {
        goto complete;
    }

    // Map the data into our process.
    SockGroupData = MapViewOfFile(SockGroupDataMapping, FILE_MAP_WRITE, 0, 0, 0);
    if( SockGroupData == NULL ) {
        goto complete;
    }

    // Create the protective mutex.
    SockGroupDataMutex = CreateMutex(NULL, TRUE, SOCK_GROUP_MUTEX_NAME);
...
--*/
{
    ...
    result = WaitForSingleObject(SockGroupDataMutex, INFINITE);
    if( result == WAIT_FAILED ) {
        SOCK_PRINT(("SockAcquireGroupLock: WaitForSingleObject failed, error %d\n", GetLastError()));
    }
}   // SockAcquireGroupLock
...
--*/
{
    ...
    if( !result ) {
        SOCK_PRINT(("SockReleaseGroupLock: ReleaseMutex failed, error %d\n", GetLastError()));
    }
}   // SockReleaseGroupLock
...
/*++
Routine Description:
    This routine does all of the nasty stuff necessary for enforce constrained groups.
Arguments:
...
--*/
{
    ...
    //       list of just those sockets that belong to constrained groups.
    return TRUE;
}   // SockValidateConstrainedGroup
