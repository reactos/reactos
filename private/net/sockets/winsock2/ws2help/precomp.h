...
#define ENTER_WS2HELP_API()                                         (Ws2helpInitialized ? 0 : Ws2helpInitialize())

VOID NewCtxInit (VOID);

#ifdef NT351
#define ALLOC_MEM(cb)       RtlAllocateHeap(RtlProcessHeap(), 0, (cb))
#define REALLOC_MEM(p,cb)   RtlReAllocateHeap(RtlProcessHeap(), 0, (p), (cb))
#define FREE_MEM(p)         RtlFreeHeap(RtlProcessHeap(), 0, (p))
#else   // !NT351
#include <ws2ifsl.h>
#define ALLOC_MEM(cb)       (LPVOID)GlobalAlloc(GPTR, (cb))
#define REALLOC_MEM(p,cb)   (LPVOID)GlobalReAlloc((HGLOBAL)(p), (cb), (GMEM_MOVEABLE | GMEM_ZEROINIT))
#define FREE_MEM(p)         (VOID)GlobalFree((HGLOBAL)(p))
#endif  // NT351
