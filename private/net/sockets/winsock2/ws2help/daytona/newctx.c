...
Abstract:
    This module implements functions for creating and manipulating context tables.
    Context tables are used in WinSock 2.0 for associating 32-bit context values with socket handles.
...
// Min & max allowable values for number of handle lookup tables (must be some number == 2**N for optimal performance)
...
{
    31, 61, 127, 257, 521, 1031, 2053, 4099, 8191, 16381, 32749, 65537, 131071, 261983, 0xFFFFFFFF  // Indicates end of the table, next number must be computed
                // on the fly.
};
...
#define TABLE_FROM_HANDLE(_h,_tbls)             (&(_tbls)->Tables[((((ULONG_PTR)_h) >> 2) & (_tbls)->HandleToIndexMask)])

// Hash function (no need to shift as we use prime numbers)
#define HASH_BUCKET_FROM_HANDLE(_h,_hash)             ((_hash)->Buckets[(((ULONG_PTR)_h) % (_hash)->NumBuckets)])


// RW_LOCK macros

//VOID AcquireTableReaderLock (IN  LPCTX_LOOKUP_TABLE     tbl, OUT LPLONG                 pctr);
/*++
...
#else  //_RW_LOCK_
#define AcquireTableReaderLock(tbl,pctr)                                        (EnterCriticalSection(&tbl->WriterLock),hash=tbl->HashTable)
#endif //_RW_LOCK_


//VOID ReleaseTableReaderLock (IN  LPCTX_LOOKUP_TABLE     tbl, OUT LPLONG                 pctr);
/*++
...
#else  //_RW_LOCK_
#define ReleaseTableReaderLock(tbl,pctr)                                    LeaveCriticalSection (&(tbl)->WriterLock)
#endif //_RW_LOCK_


//VOID AcquireTableWriterLock (IN  LPCTX_LOOKUP_TABLE  tbl);
/*++
...
//VOID ReleaseTableWriterLock (IN  LPCTX_LOOKUP_TABLE  tbl);
/*++
...
//VOID WaitForAllReaders (IN  LPSOCK_LOOKUP_TABLE    tbl);
/*++
...
VOID DoWaitForReaders (LPCTX_LOOKUP_TABLE  Table, PVOLCTR             Ctr)
{
    ...
    if (*Ctr>0) {
        ...
        if (ghWriterEvent==NULL) {
            ...
            if (hEvent!=NULL) {
                ...
                if (InterlockedCompareExchangePointer ((PVOID *)&ghWriterEvent, hEvent, NULL)!=NULL) {
...
// Function called during DLL initialization to peek up system parameters and initialize globals
VOID NewCtxInit (VOID)
{
    ...
    // This number will vary depending on whether the platform is NT Server or not.
    // Allow configuration of this value via registry, and make sure that the # of buckets is reasonable given the
    // platform, and that it is some value == 2**N so our handle->lock mapping scheme is optimal.

    // Also retrieve the spin count for use in calling InitializeCriticalSectionAndSpinCount()

    numLookupTables = ( productType==NtProductWinNt ? MIN_HANDLE_BUCKETS_WKS : MIN_HANDLE_BUCKETS_SRV );

    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("System\\CurrentControlSet\\Services\\Winsock2\\Parameters"), 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS) {
        dwDataSize = sizeof (numLookupTables);
        RegQueryValueEx(hKey,TEXT("Ws2_32NumHandleBuckets"),0,&dwDataType,(LPBYTE) &numLookupTables,&dwDataSize);

        if (sysInfo.dwNumberOfProcessors>1) {
            // Spinning only makes sense on multiprocessor machines
            dwDataSize = sizeof (gdwSpinCount);
            RegQueryValueEx(hKey,TEXT("Ws2_32SpinCount"),0,&dwDataType,(LPBYTE) &gdwSpinCount,&dwDataSize);
        }
    ...
    for(dwBitMask = MAX_HANDLE_BUCKETS_SRV;(dwBitMask & numLookupTables) == 0;dwBitMask >>= 1);
...
WS2HELPAPI DWORD WINAPI WahCreateHandleContextTable(OUT LPCONTEXT_TABLE FAR * Table)
/*++
Routine Description:
...
--*/
{
    ...
    *Table = ALLOC_MEM (FIELD_OFFSET (struct _CONTEXT_TABLE, Tables[gHandleToIndexMask+1]));
    if ( *Table == NULL ) {
...
#endif //_RW_LOCK_
        __try {
            InitializeCriticalSectionAndSpinCount (&(*Table)->Tables[i].WriterLock, gdwSpinCount);
        }
...
WS2HELPAPI DWORD WINAPI WahDestroyHandleContextTable(LPCONTEXT_TABLE Table)
/*++
Routine Description:
...
WS2HELPAPI LPWSHANDLE_CONTEXT WINAPI WahReferenceContextByHandle(LPCONTEXT_TABLE Table, HANDLE          Handle)
/*++
Routine Description:
...
Return Value:
    REFERENCED context for the handle if found, NULL if it does not exist
--*/
{
...
//        return return_code;
    ...
    if ((hash!=NULL) && ((ctx=HASH_BUCKET_FROM_HANDLE(Handle, hash))!=NULL) && (ctx->Handle==Handle)) {
...
WS2HELPAPI LPWSHANDLE_CONTEXT WINAPI WahInsertHandleContext(LPCONTEXT_TABLE     Table, LPWSHANDLE_CONTEXT  HContext)
/*++
Routine Description:
    Inserts context for the handle in the table.
Arguments:
    Table   -   Supplies pointer to the table into which to insert
    HContext -  Supplies handle context to insert which contains handle value and internally\externally used reference count
Return Value:
    ...
    other context - HContext replaced another context which is returned to the caller
--*/
{
    INT                 return_code;
...
//        return return_code;
    ...
    do {
        ...
        AcquireTableReaderLock(table,state);
        hash = table->HashTable;
        // First make sure we have already initialized hash table and it is not being expanded at the moment
        if (!table->ExpansionInProgress && (hash!=NULL)) {
            ...
            if (InterlockedCompareExchangePointer ((PVOID *)pBucket, HContext, NULL)==NULL) {
                ...
                // Another context for the same handle exists or collision in hash value, need to go the long way with the exclusive lock
                // held (we will either need to replace the context or expand the table, in both cases we need to make sure that no-one
                // can be still looking at the table or the context after we return.
            }
        }
        else {
        ...
        TryAgain:
            ...
            if (SockPrimes[i]!=0xFFFFFFFF) {
                ...
                // Reached the end of precomputed primes, simply double the size of the table (we are getting real big now, any mapping should do).
                newNumHashBuckets *= 2;
            }

            newHashTable = (LPCTX_HASH_TABLE) ALLOC_MEM(FIELD_OFFSET (CTX_HASH_TABLE,Buckets[newNumHashBuckets]));
            if (newHashTable!=NULL) {
                newHashTable->NumBuckets = newNumHashBuckets;
                ZeroMemory(newHashTable->Buckets, newNumHashBuckets * sizeof (newHashTable->Buckets[0]));

                // Well, first insert the new object, that's why we are there in the first place.
                HASH_BUCKET_FROM_HANDLE(HContext->Handle, newHashTable) = HContext;
                if (hash!=NULL) {
                    // The previous table wasn't empty, we need to move all the entries.
                    // Note that we have already freezed all the table modifications above.
                    for (i=0 ; i<hash->NumBuckets; i++) {
                        if (hash->Buckets[i] != NULL) {
                            pBucket = &HASH_BUCKET_FROM_HANDLE(hash->Buckets[i]->Handle, newHashTable);
                            if (*pBucket == NULL) {
                    ...
                    // Table was successfully moved, safe to destroy the old one except we need to wait while no-one is accessing it.
...
WS2HELPAPI DWORD WINAPI WahRemoveHandleContext(LPCONTEXT_TABLE     Table, LPWSHANDLE_CONTEXT  HContext)
/*++
Routine Description:
...
Arguments:
    ...
    HContext -  Supplies handle context to insert which contains handle value and internally\externally used reference count
Return Value:
    NO_ERROR - success, ERROR_INVALID_PARAMETER context did not exist in the table
--*/
{
...
//        return return_code;
    ...
    pBucket = &HASH_BUCKET_FROM_HANDLE(HContext->Handle,hash);
    if ((hash!=NULL)
            // Use interlocked operation to make sure we won't remove another context.
            && (InterlockedCompareExchangePointer ((PVOID *)pBucket, NULL, HContext)==HContext)) {
...
WS2HELPAPI BOOL WINAPI WahEnumerateHandleContexts(LPCONTEXT_TABLE         Table, LPFN_CONTEXT_ENUMERATOR Enumerator, LPVOID                  EnumCtx)
/*++
Routine Description:
    Calls specified enumeration procedure for all contexts in the table untill enumeration function returns FALSE.
    While enumeration is performed the table is completely locked for any modifications (read access is still allowed).
     It is OK to call table modification procedures from inside the enumeration function (modification lock allows for recursion).
Arguments:
