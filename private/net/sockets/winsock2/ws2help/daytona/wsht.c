...
VOID WINAPI WsCompletion (DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, LPOVERLAPPED lpOverlapped, DWORD Flags);
...
int _cdecl main (int argc, CHAR **argv) {
    ...
    if (Server) {
        ...
        if (CreateIoCompletionPort (hAPort, hPort, (DWORD)hAPort, 0) == INVALID_HANDLE_VALUE) {
        ...
        if (CreateIoCompletionPort (hANoPort, hPort, (DWORD)hANoPort, 0) == INVALID_HANDLE_VALUE) {
            printf ("CreateIoCompletionPort (associate) failed, err: %ld.\n", GetLastError ());
            return -1;
        }
        ...
        wsprintfW (name, L"\\\\%hs\\pipe\\Winsock2\\CatalogChangeListener-%hs-%hs", argv[2], argv[3], argv[4]);
        hPipe =  CreateFileW (name, GENERIC_WRITE, FILE_SHARE_READ, (LPSECURITY_ATTRIBUTES) NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, (HANDLE) NULL);
...
VOID WINAPI WsCompletion (DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, LPOVERLAPPED lpOverlapped, DWORD dwFlags) {
    if (dwErrorCode!=0) {
