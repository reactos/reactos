...
int _cdecl main (int argc, CHAR **argv)
{
    ...
    gHandleContexts = (LPWSHANDLE_CONTEXT)GlobalAlloc (GPTR, sizeof (WSHANDLE_CONTEXT)*gNumHandles);
    ...
    for (i=0; i<numThreads; i++) {
        Threads[i] = CreateThread (NULL, 0, StressThread, (PVOID)i, 0, &id);
        if (Threads[i]==NULL) {
