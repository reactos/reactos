...
Abstract:
    This module implements the socket handle helper functions for the WinSock 2.0 helper library.
...
/* Private Prototypes */
VOID DoSocketRequest(PVOID Context1, PVOID Context2, PVOID Context3);
NTSTATUS DoSocketCancel(PVOID Context1, PVOID Context2, PVOID Context3);
DWORD WINAPI ApcThread(PVOID param);
VOID CALLBACK ExitThreadApc(ULONG_PTR   param);
void CALLBACK WinsockApc(IN DWORD dwError, IN DWORD cbTransferred, IN LPWSAOVERLAPPED lpOverlapped, IN DWORD dwFlags);
NTSTATUS WsErrorToNtStatus(IN DWORD Error);

/* Private Globals */
BOOL                    Ws2helpInitialized = FALSE;
CRITICAL_SECTION        StartupSyncronization;

/* Our module handle: we keep a reference to it to make sure that in is not unloaded while our thread is executing */
...
DWORD WINAPI WahOpenHandleHelper(OUT LPHANDLE HelperHandle)
/*++
Routine Description:
...
--*/
{
    ...
    if (hCtx != NULL) {
        ...
        hCtx->ThreadHdl = CreateThread(NULL, 0, ApcThread, hCtx, CREATE_SUSPENDED, &apcThreadId);
...
--*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (DeviceIoControl(
            ...
            &count,                             // BytesReturned
            NULL)) {                              // Overlapped
            WshPrint(DBG_SOCKET, ("WS2HELP-%lx WahCreateSocketHandle: Handle %p\n", PID, *s));
            error = NO_ERROR;
        } else {
            error = GetLastError();
            NtClose((HANDLE)*s);
...
DWORD WINAPI WahCloseSocketHandle(IN HANDLE           HelperHandle, IN SOCKET           s)
/*++
Routine Description:
...
--*/
{
    ...
    if ((HelperHandle == NULL) || (s == 0) || (s == INVALID_SOCKET))
        return ERROR_INVALID_PARAMETER;

    WshPrint(DBG_SOCKET, ("WS2HELP-%lx WahCloseSocketHandle: Handle %p\n", PID, s));
...
Arguments:
    ...
    cbTransferred- number of bytes transferred to/from user buffers as the result of the operation being completed
Return Value:
    DWORD - NO_ERROR if successful, a Win32 error code if not.
--*/
{
    ...
    if ((HelperHandle == NULL) || (lpOverlapped == NULL) || (s == INVALID_SOCKET) || (s == 0))
        return ERROR_INVALID_PARAMETER;
...
DWORD WINAPI WahEnableNonIFSHandleSupport(VOID)
/*++
Routine Description:
    This function installs and starts Winsock2 Installable File System Layer driver to provide socket handles for Non-IFS handle transport service providers.
Return Value:
    DWORD - NO_ERROR if successful, a Win32 error code if not.
--*/
{
    ...
    if (rc == 0) {
        ...
        WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahEnableNonIFSHandleSupport:"
    ...
    hSCManager = OpenSCManager(NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_CREATE_SERVICE);
    if (hSCManager == NULL) {
        rc = GetLastError();
        WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahEnableNonIFSHandleSupport: Could not open SC, err: %ld\n", PID, rc));
        return rc;
    }
    ...
    if (hWS2IFSL == NULL) {
        ...
        if (rc != ERROR_SERVICE_EXISTS) {
            ...
            WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahEnableNonIFSHandleSupport: Could not create service, err: %ld\n", PID, rc));
        ...
        hWS2IFSL = OpenService(hSCManager, WS2IFSL_SERVICE_NAME, SERVICE_ALL_ACCESS);
        if (hWS2IFSL == NULL) {
            // Could not open, bail out
            rc = GetLastError();
            WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahEnableNonIFSHandleSupport: Could not open service, err: %ld\n", PID, rc));
            CloseServiceHandle(hSCManager);
            return rc;
        }
        ...
        if (!ChangeServiceConfig(hWS2IFSL,
            ...
            rc = GetLastError();
            WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahEnableNonIFSHandleSupport: Could not enable service, err: %ld\n", PID, rc));
            CloseServiceHandle(hSCManager);
            return rc;
        }
    }
    ...
    WshPrint(DBG_SERVICE, ("WS2HELP-%lx WahEnableNonIFSHandleSupport: Created and started service.n", PID));
...
DWORD WINAPI WahDisableNonIFSHandleSupport(VOID)
/*++
Routine Description:
    This function deinstalls Winsock2 Installable File System Layer driver to provide socket handles for Non-IFS handle transport service providers.
Arguments:
    None
Return Value:
    DWORD - NO_ERROR if successful, a Win32 error code if not.
--*/
{
    ...
    if (hSCManager == NULL) {
        rc = GetLastError();
        WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahDisableNonIFSHandleSupport: Could not open SC, err: %ld\n", PID, rc));
        return rc;
    }
    ...
    if (hWS2IFSL == NULL) {
        rc = GetLastError();
        WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahDisableNonIFSHandleSupport: Could open service, err: %ld\n", PID, rc));
        CloseServiceHandle(hSCManager);
        return rc;
    }
    ...
    // Deleting service is dangerous because this will require a reboot before it can be installed again and we are working towards no-reboot system

    // Stopping is even worse, because it will go into uncontrollable (STOP_PENDING) state until all handles to it are closed so we won't be able to start it until reboot if some service is holding its handles.
    if (ChangeServiceConfig(hWS2IFSL,
        ...
        rc = 0;
        WshPrint(DBG_SERVICE, ("WS2HELP-%lx WahDisableNonIFSHandleSupport: Disabled service.\n", PID));
    } else {
        rc = GetLastError();
        WshPrint(DBG_SERVICE | DBG_FAILURES, ("WS2HELP-%lx WahDisableNonIFSHandleSupport: Could not disable service, err: %ld\n", PID, GetLastError()));
    }
...
/*++
Routine Description:
    This is a thread which is used by the driver to execute IO system requests
Arguments:
...
--*/
{
    ...
    // Increment our module reference count so it does not go away while this thread is running
    rc = GetModuleFileName(LibraryHdl, ModuleName, sizeof(ModuleName) / sizeof(ModuleName[0]));
    ASSERT(rc > 0);

    hCtx->LibraryHdl = LoadLibrary(ModuleName);
    ASSERT(hCtx->LibraryHdl != NULL);

    WshPrint(DBG_APC_THREAD, ("WS2HELP-%lx ApcThread: Initialization completed\n", PID));
    ...
    while (TRUE) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            NtDelayExecution(FALSE, &Timeout2);// Sleep for 3 seconds
        }
    }

    // We should never get here, the thread terminates from the ExitApc
    ASSERT(FALSE);
    return 0;
}
...
Arguments:
    ...
    lpOverlapped    - overlapped structure associated with the request (it is actually our extended structure: OVERLAPPED_CTX_
    dwFlags         - flags associated with the request (ignored)
--*/
{
    POVERLAPPED_CTX     ctx = CONTAINING_RECORD(lpOverlapped, OVERLAPPED_CTX, ovlp);
    IO_STATUS_BLOCK     ioStatus;
    DWORD               ignore;
...
VOID DoSocketRequest(PVOID   PvCtx, PVOID   PvRequestId, PVOID   PvBufferLength)
/*++
Routine Description:
...
--*/
{
    ...
    WshPrint(DBG_CANCEL, ("WS2HELP-%lx CancelHook: %s\n", PID, res ? "succeded" : "failed"));
    return FALSE;
}


NTSTATUS DoSocketCancel(PVOID   PvCtx, PVOID   PvRequestId, PVOID   PvDllContext)
/*++
Routine Description:
    Hack to attempt to cancel request in progress.
    This works with MSAFD, but may (actully will) not work with any other provider which is likely to implement select differently.
    Our hope here that by cancelling select we will also cancel any other outstanding requests on a socket handle.
Return Value:
    None
--*/
{
    ...
    if ((rc = (*pWSAStartup) (wVersionRequested, &wsaData)) == NO_ERROR) {
        ...
        if (oldHook != NULL) {
            ...
            WshPrint(DBG_CANCEL, ("WS2HELP-%lx CancelApc: Done on socket id %ld (res %ld)\n", PID, PtrToUlong(PvRequestId), res));
            pWSASetBlockingHook(oldHook);
        } else {
            rc = pWSAGetLastError();
            WshPrint(DBG_CANCEL | DBG_FAILURES, ("WS2HELP-%lx DoSocketCancel: Could not install blocking hook, err - %ld\n", PID, rc));
        }
        (*pWSACleanup)();
    } else {
        WshPrint(DBG_CANCEL | DBG_FAILURES, ("WS2HELP-%lx DoSocketCancel: Could not get version 1.1, rc - %ld\n", PID, rc));
    }
...
NTSTATUS WsErrorToNtStatus(DWORD   dwError)
/*++
Routine Description:
...
#define MAPWSERROR(line,Error,Status)   Status
    ...
    else if ((dwError >= WSABASEERR) && (dwError < WSABASEERR + sizeof(WSAEMap) / sizeof(WSAEMap[0])))
...
VOID ReadDbgInfo(VOID)
{
    ...
    if ((rc = RegOpenKeyExA(HKEY_LOCAL_MACHINE, WS2IFSL_DEBUG_KEY, 0, KEY_QUERY_VALUE, &hDebugKey)) == NO_ERROR) {
        sz = sizeof(DbgLevel);
        if (RegQueryValueEx(hDebugKey, TEXT("DbgLevel"), NULL, NULL, (LPBYTE)&level, &sz) == 0)
            DbgLevel = level;

        sz = sizeof(DbgLevel);
        if (RegQueryValueEx(hDebugKey, pProcessFileName, NULL, NULL, (LPBYTE)&level, &sz) == 0)
            DbgLevel = level;
