...
LRESULT APIENTRY  DTEditWndProc(IN HWND   WindowHandle, IN UINT   Message, IN WPARAM WParam, IN LPARAM LParam);
...
BOOL WINAPI  DllMain(HINSTANCE InstanceHandle, DWORD     dwReason, LPVOID    lpvReserved)
/*++
  DllMain()
    ...
    if (!DllInitialized) {
        ...
        if (hUser32!=NULL) {
            ...
            if ((OutputStyle == WINDOW_ONLY) || (OutputStyle == FILE_AND_WINDOW)) {
                ...
                wsprintf(Buffer, "Process ID: 0x%X   Thread ID: 0x%X\r\n", InitDataPtr->PID, InitDataPtr->TID);
                DTTextOut(DebugWindow, LogFileHandle, Buffer, OutputStyle);

                // Setting this event allows {Pre|Post}ApiNotify to proceed.  This event isn't really needed in this case
...
/*++
  Function Description:
      Builds a string for output and passes it, along with information about the call, to a handler function.
  ...
  Arguments:
      ...
      case of a short-circuit (see how the return value from PreApiNotify works for more information on short-circuiting the API function).

      LibraryName -- a string pointing to the name of the library that called us.
  ...
  Return Value:
      ...
      in other words, returning non-zero here forces the API function to return immediately before any other actions take place.
...
--*/
{
    ...
    Index = wsprintf(Buffer, "Function call: %d   ", ThreadCstack->CGetCounter());
    ...
    if ((NotificationCode < MAX_DTCODE) && HdlFuncTable[NotificationCode]) {
        ...
        ReturnValue = (*HdlFunc)(vl, ReturnCode, LibraryName, Buffer, Index, TEXT_LEN, TRUE);
    } else {
...
/*++
  ...
  Arguments:
      ...
      ...    -- variable number argument list.  These are pointers to the actual parameters of the API functions.
  Return Value:
...
--*/
{
    ...
    if ((NotificationCode < MAX_DTCODE) && HdlFuncTable[NotificationCode]) {
        HdlFunc = HdlFuncTable[NotificationCode];
        (*HdlFunc)(vl, ReturnCode, LibraryName, Buffer, Index, TEXT_LEN, FALSE);
    } else {
...
--*/
{
    ...
    case WM_CREATE:
        ...
        SendMessage(DebugWindow, WM_SETFONT, (WPARAM)FixedFontHandle, MAKELPARAM(TRUE, 0));
        return(0);
    case WM_COMMAND:
        if (LOWORD(WParam) == EC_CHILD) {
            ...
            case EN_MAXTEXT:
                ...
                LineIndex = (DWORD)SendMessage(DebugWindow, EM_LINEFROMCHAR, (WPARAM)CharIndex1, 0);
                CharIndex2 = (DWORD)SendMessage(DebugWindow, EM_LINEINDEX, (WPARAM)LineIndex, 0);
                SendMessage(DebugWindow, EM_SETSEL, 0, CharIndex2);
                SendMessage(DebugWindow, EM_REPLACESEL, 0, (LPARAM)NullString);

                // send this text to the window only...
                OldOutputStyle = OutputStyle;
                OutputStyle = WINDOW_ONLY;
                DTTextOut(DebugWindow, LogFileHandle, "-Buffer Overflow...Resetting-\r\n", OutputStyle);
...
--*/
{
    ...
    case WM_CHAR:
        ...
        // number!
        if (WParam == 3) {
            return (CallWindowProc(EditWndProc, WindowHandle, Message, WParam, LParam));
        } // else flows through
...
/*
  ...
  Return Value:
      Returns the wParam of the quit message that forced us out of the message loop.
--*/
{
...
/*++
  ...
  Arguments:
      ...
      Style -- specifies whether the output should go to the window, the file, or both.
  Return Value:
...
BOOL GetFile(IN  HWND   OwnerWindow, OUT LPSTR  FileName, IN  DWORD  FileNameSize)
/*
  Function Description:
      Uses the predefined "Save As" dialog box style to retrieve a file name from the user.
      The file name the user selects is stored in LogFileName.
  Arguments:
  ...
  Return Value:
      Returns whatever GetSaveFileName returns; see documentation for that function.
--*/
{
