/*++
  ...
  Abstract:
    Implements Deque structures.
    These are double linked lists  that can place and remove data from the beginning and end.
    This file also contains an iterator for Deques.
    Important to notice is that these are non-intrusive deques.
    Meaning the use of these classes does not need to insert pointers into there classes to use these.
...
/*++
  ...
  Return Value:
      None.
*/
template<class T> void NIDequeIter_c<T>::RemoveData(NILNode_c<T> *cptr, T &ret_data)
{
    NILNode_c<T>  *bptr    = NULL,
                  *nptr    = NULL,
                  *fptr    = NULL;
...
/*++
  ...
  Function Description:
      Returns the First data in the linked list.
      This primes the current pointer so that next time the Next method is used to get  more linked list data.
  Arguments:
