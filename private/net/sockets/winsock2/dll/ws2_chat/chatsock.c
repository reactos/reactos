...
DWORD IOThreadFunc(IN LPVOID ParamPtr);
BOOL HandleSocketEvent(IN OUT PCONNDATA ConnData);
BOOL HandleOutputEvent(IN OUT PCONNDATA);
BOOL HandleOtherEvent(IN     DWORD     WaitStatus,IN OUT PCONNDATA ConnData);
int HandleEvents(IN PCONNDATA          ConnData,IN LPWSANETWORKEVENTS NetworkEvents);
BOOL FillLocalAddress(IN LPVOID          SockAddr);
int DoRecv(IN PCONNDATA ConnData);
int DoOverlappedCallbackSend(IN POUTPUT_REQUEST OutReq,IN PCONNDATA ConnData);
int DoOverlappedEventSend(IN POUTPUT_REQUEST OutReq,IN PCONNDATA ConnData);
int DoSend(IN POUTPUT_REQUEST OutReq,IN PCONNDATA ConnData);
...
LPWSAPROTOCOL_INFO GetProtoFromSocket(IN SOCKET Socket);
BOOL GetMaxMsgSize(IN OUT PCONNDATA ConnData);
...
--*/
{
    ...
    if (Error) {
        MessageBox(GlobalFrameWindow, "Could not find high enough version of WinSock", "Error", MB_OK | MB_ICONSTOP | MB_SETFOREGROUND);
        ReturnValue = FALSE;
    } else {
        ...
        if (LOBYTE(WsaData.wVersion) != VERSION_MAJOR || HIBYTE(WsaData.wVersion) != VERSION_MINOR) {
            MessageBox(GlobalFrameWindow, "Could not find the correct version of WinSock", "Error",  MB_OK | MB_ICONSTOP | MB_SETFOREGROUND);
            WSACleanup();
            ReturnValue = FALSE;
        }
    }
...
/*++
Routine Description:
    Finds out about all transport protocols installed on the local machine and saves the information into global variables.
Implementation:
    This function uses WSAEnumProtocols to find out about all installed protocols on the local machine.
    It stores this information in two variables which are global to this file;
    InstalledProtocols is a pointer to a buffer of WSAPROTOCOL_INFO structs, while NumProtocols is the number of protocols in that buffer.
    This function is the only function in the file allowed to touch these variables.
Return Value:
    TRUE - Successfully initialized the protocol buffer.
    FALSE - Some kind of problem arose.  The user is informed of the error.
--*/
{
    ...
    if ((NumProtocols != SOCKET_ERROR) && (WSAGetLastError() != WSAENOBUFS)) {
        // We're in trouble!!
        MessageBox(GlobalFrameWindow, "WSAEnumProtocols is broken.", "Error", MB_OK | MB_ICONSTOP | MB_SETFOREGROUND);
        goto Fail;
    }
    ...
    if (InstalledProtocols == NULL) {
        MessageBox(GlobalFrameWindow, "malloc failed.", "Error", MB_OK | MB_ICONSTOP | MB_SETFOREGROUND);
        goto Fail;
    }
    NumProtocols = WSAEnumProtocols(NULL, (LPVOID)InstalledProtocols, &BufferSize);
    if (NumProtocols == SOCKET_ERROR) {
        // uh-oh
        wsprintf(MsgText, "WSAEnumProtocols failed.  Error Code: %d", WSAGetLastError());
        MessageBox(GlobalFrameWindow, MsgText, "Error", MB_OK | MB_ICONSTOP | MB_SETFOREGROUND);
        goto Fail;
    }
...
--*/
{
    ...
    // before actually accepting the connection).
    {
        ...
        ConnData->Socket = WSAAccept(Socket, &address, &address_len, NULL, (DWORD)NULL);
//         ConnData->Socket = WSAAccept(Socket, NULL, NULL, AcceptCondFunc, (DWORD)ConnData);
    }
    ...
    if (ConnData->Socket == INVALID_SOCKET) {
        ...
        if (Error != WSAECONNREFUSED) {
            ...
            MessageBox(ConnectionWindow, MsgText, "Error", MB_OK | MB_ICONSTOP | MB_SETFOREGROUND);
            goto Fail;
        } else {
    ...
    WSAEventSelect(ConnData->Socket, ConnData->SocketEventObject, FD_READ | FD_WRITE | FD_CLOSE);
    ...
    ConnData->IOThreadHandle =
      CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)IOThreadFunc, ConnData, 0, &ThreadId);
    if (ConnData->IOThreadHandle == NULL) {
        ChatSysError("CreateThread()", "HandleAcceptMessage()", TRUE);
    }
...
--*/
{
    ...
    // Start the I/O thread, and save the thread handle.
    ConnData->IOThreadHandle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)IOThreadFunc, ConnData, 0, &ThreadId);
    if (ConnData->IOThreadHandle == NULL) {
...
--*/
{
    ...
    if ((ConnData->ProtocolInfo->dwMessageSize == 0) ||
        ...
        if (ConnData->MaxMsgSize == 0x1) {
            ...
            Error = getsockopt(ConnData->Socket, SOL_SOCKET, SO_MAX_MSG_SIZE, (char *)&ConnData->MaxMsgSize, &DwordLen);
            if (Error) {
                ReturnValue = FALSE;
            }
        } // if (ConnData->MaxMsgSize == 0x1)
        else {
            // The message size is stored in the protocol information structure.  Use it.
...
BOOL IsSendable(char Char)
/*++
Routine Description:
    Determines whether a certain character value is ok to send over the socket to the far end.
    Eliminates most non-printable characters except for newline and backspace.
Arguments:
