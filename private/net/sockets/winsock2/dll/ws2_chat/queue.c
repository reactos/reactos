...
/*++
Routine Description:
    Creates a new QUEUE, initializes it, and returns it. Returns NULL if memory cannot be allocated in the heap.
Return Value:
...
/*++
Routine Description:
    This function removes any items left on the queue, frees them, and then frees the queue itself.
Arguments:
...
/*++
Routine Description:
    This function enqueues one item into Queue, if there is room for it.
Arguments:
...
Return Value:
    ...
    FALSE -- There were too many items already in the queue; one item must be removed in order to insert another.
--*/
{
    ...
    if(((Queue->Tail + 1) % MAX_QUEUE_SIZE == Queue->Head) && (Queue->Head != Q_NULL)) {
        ReturnValue = FALSE;
    } else {
...
/*++
Routine Description:
    This function inserts one item at the HEAD of the queue.
    Of course, this violates queue semantics...but this can be useful at times.
Arguments:
    Queue -- The queue into which to insert Object.
    Object -- A void pointer to the object being inserted.
Return Value:
    TRUE -- The object was successfully inserted at the head of the queue.
    FALSE -- There were too many items already in the queue; one item must be removed in order to insert another.
--*/
{
    BOOL ReturnValue; // holds the return value
    ...
    // If the queue is full, set the return value to FALSE and do nothing; if not, update the indices appropriately and set the return value to TRUE.
...
/*++
Routine Description:
    This function tells the caller if the queue in question contains any items.
Arguments:
