...
Abstract:
    This module contains special debug versions of the new and delete C++ operators.
...
PVOID SockAllocateHeap(IN ULONG NumberOfBytes)
{
    PSOCK_HEAP_HEADER header;
    SOCK_HEAP_TAIL UNALIGNED *tail;
    ...
    RtlAcquireResourceExclusive(&SocketHeapLock, TRUE);
    header = (PSOCK_HEAP_HEADER)RtlAllocateHeap(RtlProcessHeap(), 0, NumberOfBytes + sizeof(*header) + sizeof(*tail));
    if( header == NULL ) {
        RtlReleaseResource( &SocketHeapLock );
...
#if i386
    {
        ...
        RtlCaptureStackBackTrace(2, MAX_STACK_BACKTRACE, (PVOID *)&header->Stack, &hash);
    }
#endif  // i386
...
VOID SockFreeHeap(IN PVOID Pointer)
{
    ...
    RtlReleaseResource( &SocketHeapLock );
    RtlFreeHeap(RtlProcessHeap(), 0, (PVOID)header);
    if( SockDoubleHeapCheck ) {
        SockCheckHeap();
    }
} // SockFreeHeap
