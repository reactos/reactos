...
#pragma warning(disable: 4057)// 'type' differs in indirection to slightly different base types from 'other type'
...
/*++
  Routine Description:
  This routine outputs a debug messages.
  Debug messages are routed to a file or a debug window depnding on the value of a global variable defined in this module
    ...
    if(lstrcmp(g_CurrentMessage, g_PreviousMessage))
    {
        if (iTraceDestination == TRACE_TO_FILE)
        {
            ...
            if (LogOpened)
            {
                if (RepeatCount > 0)
                {
                    wsprintf(g_PreviousMessage, "Last Message Repeated < %d > times\n", RepeatCount);
                    WriteFile(OutputFile, g_PreviousMessage, lstrlen(g_PreviousMessage), &BytesWritten, NULL);
                } //if

                WriteFile(OutputFile, g_CurrentMessage, lstrlen(g_CurrentMessage), &BytesWritten, NULL); // Write the current message to the trace file
                FlushFileBuffers( TraceFile );// Flush debug output to file
        ...
        if( iTraceDestination == TRACE_TO_AUX)
        {
            ...
            OutputDebugString(g_CurrentMessage); // Send message to AUX device
        }

        lstrcpy(g_PreviousMessage, g_CurrentMessage);// Store off this message
    }
    else
    {
        RepeatCount++;
...
LONG Ws2ExceptionFilter(LPEXCEPTION_POINTERS ExceptionPointers, LPSTR SourceFile, LONG LineNumber)
{
    ...
    __try {
        // Exceptions should never be thrown in a properly functioning system, so this is bad. To ensure that someone will see this, print to the debugger directly
