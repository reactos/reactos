...
Abstract:
    This header defines the "DPROCESS" class.
    The DPROCESS class defines state variables  and operations for DPROCESS objects within the WinSock 2 DLL.
    A DPROCESS  object  represents  all  of the information known about a process using the Windows Sockets API.
...
class DPROCESS
{
  ...
  // Static (global-scope) member functions
    ...
    static PDPROCESS GetCurrentDProcess(VOID);
    static INT DProcessClassInitialize(VOID);
  ...
  // Normal member functions
    ...
    INT DThreadAttach(IN PDTHREAD NewThread);
    INT DThreadDetach(IN PDTHREAD  OldThread);
    BOOL  DSocketDetach (IN LPWSHANDLE_CONTEXT   Context);
    PDCATALOG GetProtocolCatalog();
    PNSCATALOG GetNamespaceCatalog();
    INT GetAsyncHelperDeviceID(OUT LPHANDLE HelperHandle);
    INT GetHandleHelperDeviceID(OUT LPHANDLE HelperHandle);
    INT GetNotificationHelperDeviceID(OUT LPHANDLE HelperHandle);
    VOID IncrementRefCount();
    DWORD DecrementRefCount();
...
// Give debugger extension access to all fields
  ...
  private:
    ...
    INT OpenAsyncHelperDevice(OUT LPHANDLE HelperHandle);
    INT OpenHandleHelperDevice(OUT LPHANDLE HelperHandle);
    INT OpenNotificationHelperDevice(OUT LPHANDLE HelperHandle);
...
inline PDPROCESS DPROCESS::GetCurrentDProcess()
/*++
Routine Description:
    Retrieves  a reference to the current DPROCESS object.  Note that this is a "static" function with global scope instead of object-instance scope.
Arguments:
    None
Return Value:
    Returns pointer to current DPROCESS object or NULL if process has not been initialized yet
*/
{
...
inline VOID DPROCESS::IncrementRefCount(VOID)
/*++
Routine Description:
...
inline DWORD DPROCESS::DecrementRefCount(VOID)
/*++
Routine Description:
...
inline BYTE DPROCESS::GetMajorVersion()
/*++
Routine Description:
    This function returns the major WinSock version number negotiated at WSAStartup() time.
Arguments:
    None.
Return Value:
...
inline BYTE DPROCESS::GetMinorVersion()
/*++
Routine Description:
    This function returns the minor WinSock version number negotiated at WSAStartup() time.
Arguments:
    None.
Return Value:
...
inline WORD DPROCESS::GetVersion()
/*++
Routine Description:
    This function returns the WinSock version number negotiated at WSAStartup() time.
Arguments:
    None.
Return Value:
...
inline VOID DPROCESS::LockDThreadList()
/*++
  Routine Description:
...
inline VOID DPROCESS::UnLockDThreadList()
/*++
  Routine Description:
...
inline INT DPROCESS::GetAsyncHelperDeviceID(OUT LPHANDLE HelperHandle)
/*++
Routine Description:
...
inline INT DPROCESS::GetHandleHelperDeviceID(OUT LPHANDLE HelperHandle)
/*++
Routine Description:
...
inline INT DPROCESS::GetNotificationHelperDeviceID(OUT LPHANDLE HelperHandle)
/*++
Routine Description:
    Retrieves  the  opened  Async  Helper  device  ID  required  for processing callbacks  in  the  overlapped  I/O  model.
    The operation opens the Async Helper device if necessary.
