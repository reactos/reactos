...
Abstract:
    This  module  is  a  standalone  collection  of  linked-list definition and manipulation  macros  originally  defined  within  the  internal Windows NT development.
...
Notes:
    ...
    As  of 07-08-95, this header file contains usage comments.
    If desired, the usage  comments  can  be  removed.
    However,  since  this file is only for internal  use, there is no strong reason to remove the comments.
    Since the comments  make  it easier to understand and use these macros they should be preserved as far as possible.
...
Routine Description:
    ...
    The offset is from the beginning of the containing structure.

    Note  that  since  this macro uses compile-time type knowledge, there is no equivalent C procedure for this macro.
Arguments:
    type  - Supplies the type name of the containing structure.
    field - Supplies  the  field  name  of  the  field  whose  offset  is to be computed.
Return Value:
    Returns  the byte offset of the named field within a structure of the named type.
*/
...
Routine Description:
    Initializes  a  PLIST_ENTRY  structure to be the head of an initially empty linked list.
Arguments:
...
Arguments:
    ListHead - Supplies  a  reference  to  the  head  of  the linked list to be examined.
Return Value:
...
/*++
Routine Description:
    Removes  an  item  from a linked list.  Attempting to remove the head of an empty list is a no-op.
Arguments:
    Entry - Supplies  a reference to the linkage structure embedded in a linked list item structure.
Return Value:
    None
*/
...
Arguments:
    ListHead - Supplies  a  reference  to  the  head  of  the linked list to be operated upon.
    Entry    - Supplies  a  reference  to the linkage structure embedded in the linked list item to be added to the linked list.
Return Value:
    None
*/
...
// {
//     foo = CONTAINING_RECORD( listEntry, FOO, FooListEntry );
