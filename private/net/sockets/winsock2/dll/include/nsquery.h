...
Abstract:
    This  module  gives  the class definition for the NSQUERY object type.
    The NSQUERY    object   holds   all   the   state   information   regarding   a WSALookup{Begin/Next/End}   series   of  operations.
    It  supplies  member functions that implement the API-level operations in terms of the SPI-level operations.
...
class NSQUERY
{
  ...
  BOOL IsValid();
    ...
    INT WINAPI LookupServiceEnd();
    BOOL WINAPI IsDeletable();
    BOOL RemoveProvider(PNSPROVIDER  pNamespaceProvider);
    BOOL AddProvider(PNSPROVIDER  pNamespaceProvider);

  private:
    PNSPROVIDERSTATE NextProvider(PNSPROVIDERSTATE Provider);
    ...
    // This  keeps  track  of  the  sequence  number  of  the current first provider  in  the  provider  list.
    // When  a  LookupNext  encounters WSA_E_NOMORE, this number is compared against the number that was at the  start  of  the operation.
    //  The provider list is updated only if these  two  numbers  are  equal.  This covers the case where several
    ...
    BOOL m_restartable;
    // TRUE if no results have been returned for this query; FALSE otherwise.
#endif // RASAUTODIAL
