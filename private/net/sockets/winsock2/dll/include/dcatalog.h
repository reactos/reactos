...
Abstract:
    This  module  contains  the  interface  to  the  catalog  of  protocol_info structures and their associated providers for the winsock2 DLL.
...
public:
    ...
    INT InitializeFromRegistry(IN  HKEY    ParentKey, IN  HANDLE  CatalogChangeEvent OPTIONAL);
    INT RefreshFromRegistry (IN  HANDLE  CatalogChangeEvent OPTIONAL);
    INT WriteToRegistry();

    ~DCATALOG();

    VOID EnumerateCatalogItems(IN CATALOGITERATION  Iteration, IN PVOID             PassBack);
    INT GetCountedCatalogItemFromCatalogEntryId(IN  DWORD                     CatalogEntryId, OUT PPROTO_CATALOG_ITEM FAR * CatalogItem);
    INT GetCountedCatalogItemFromAddressFamily(IN  INT af, OUT PPROTO_CATALOG_ITEM FAR * CatalogItem);
    INT GetCountedCatalogItemFromAttributes(IN  INT   af, IN  INT   type, IN  INT   protocol, IN  DWORD StartAfterId OPTIONAL, OUT PPROTO_CATALOG_ITEM FAR * CatalogItem);
    DWORD AllocateCatalogEntryId();
    VOID AppendCatalogItem(IN  PPROTO_CATALOG_ITEM  CatalogItem);
    VOID RemoveCatalogItem(IN  PPROTO_CATALOG_ITEM  CatalogItem);
    INT FindIFSProviderForSocket(SOCKET Socket);
...
private:
    ...
    VOID UpdateProtocolList (PLIST_ENTRY new_list);
    INT LoadProvider(IN PPROTO_CATALOG_ITEM CatalogEntry);
    PDPROVIDER FindAnotherProviderInstance(IN LPGUID ProviderId);
...
inline VOID DCATALOG::AcquireCatalogLock(VOID)
{
    EnterCriticalSection( &m_catalog_lock );
}


inline VOID DCATALOG::ReleaseCatalogLock(VOID)
{
