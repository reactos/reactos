...
// Opcodes for processing by the winsock asynchronous processing thread.
...
BOOL SockAsyncGlobalInitialize(VOID);
VOID SockAsyncGlobalTerminate(VOID);
BOOL SockCheckAndInitAsyncThread(VOID);
VOID SockTerminateAsyncThread(VOID);


// Work queue functions.

PWINSOCK_CONTEXT_BLOCK SockAllocateContextBlock(DWORD AdditionalSpace);
...
INT SockCancelAsyncRequest(IN HANDLE TaskHandle);
