...
Abstract:
    This header describes the interface to the WinSock 2 debug/trace DLL.
    Please see the design spec for more information.
...
typedef BOOL (WINAPIV * LPFNWSANOTIFY)(IN  INT    NotificationCode, OUT LPVOID ReturnCode, IN  LPSTR  LibraryName, ...);

// Function prototypes for Pre/PostApiNotify

BOOL WINAPIV WSAPreApiNotify(IN  INT    NotificationCode, OUT LPVOID ReturnCode, IN  LPSTR  LibraryName, ...);
BOOL WINAPIV WSAPostApiNotify(IN  INT    NotificationCode, OUT LPVOID ReturnCode, IN  LPSTR  LibraryName, ...);

// Pointer to an exception notification function.
typedef VOID (WINAPI * LPFNWSAEXCEPTIONNOTIFY)(IN LPEXCEPTION_POINTERS ExceptionPointers);

// Function prototype for exception notify.
VOID WINAPI WSAExceptionNotify(IN LPEXCEPTION_POINTERS ExceptionPointers);
...
// Note  that  a  "MAX_DTCODE"  definition  should  be updated if new codes are added.
