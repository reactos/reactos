...
Abstract:
    ...
    This  class  defines the interface to the entries that can be installed and retrieved in the namespace provider catalog.
...
public:
    ...
    PNSPROVIDER     GetProvider();
    LPGUID     GetProviderId();
    DWORD     GetNamespaceId();
    LONG     GetAddressFamily();
    LPWSTR     GetLibraryPath();
    VOID     SetVersion(IN DWORD Version);
    DWORD GetVersion();
    BOOL GetEnabledState();
    BOOL StoresServiceClassInfo();
    LPWSTR GetProviderDisplayString();
    INT WriteToRegistry(IN  HKEY  ParentKey, IN  INT   SequenceNum);
    VOID Enable(IN BOOLEAN EnableValue);
    VOID Reference ();
    VOID Dereference ();
...
inline VOID NSCATALOGENTRY::Reference () {
...
inline VOID NSCATALOGENTRY::Dereference () {
    assert (m_reference_count>0);
...
inline PNSPROVIDER NSCATALOGENTRY::GetProvider()
/*++
Routine Description:
    This  procedure  retrieves  a reference to the NSPROVIDER associated with a catalog  entry.
Arguments:
    None
Return Value:
    Returns  the  current  provider  reference,  or  NULL if provider is not loaded yet
*/
{
    return(m_provider);
}  // GetProvider
...
inline LPGUID NSCATALOGENTRY::GetProviderId()
/*++
Routine Description:
...
inline LONG NSCATALOGENTRY::GetAddressFamily()
/*++
Routine Description:
...
inline DWORD NSCATALOGENTRY::GetNamespaceId()
/*++
Routine Description:
...
inline DWORD NSCATALOGENTRY::GetVersion()
/*++
Routine Description:
...
inline LPWSTR NSCATALOGENTRY::GetLibraryPath()
/*++
Routine Description:
...
inline BOOL NSCATALOGENTRY::GetEnabledState()
/*++
Routine Description:
...
inline LPWSTR NSCATALOGENTRY::GetProviderDisplayString()
/*++
Routine Description:
...
inline BOOL NSCATALOGENTRY::StoresServiceClassInfo()
/*++
Routine Description:
