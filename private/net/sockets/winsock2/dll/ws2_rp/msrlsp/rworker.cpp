...
--*/
{
    ...
    DEBUGF( DBG_TRACE, ("Initializing worker thread \n"));

    // Create the event we will use to communicat with the worker thread.
    m_wakeup_event = CreateEvent(NULL, FALSE, FALSE, NULL);
    if ( m_wakeup_event){
        // Add the wakeup event to the event array that the worker thread will be waiting on
    ...
    if (NO_ERROR == ReturnCode){
        ...
        m_thread_handle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)OuterThreadProc, this, 0, &ThreadId);
        if (m_thread_handle){
            ReturnCode = NO_ERROR;
        } //if
    } //if
