...
static const GUID CLSID_RestrictedProcess = {0x570da105, 0x3c30, 0x11d1, 0x8b, 0xf1, 0x00, 0x00, 0xf8, 0x75, 0x40, 0x35};

#if FALSE
static const GUID IID_IRestrictedProcess = {0x3ae0b7e0, 0x3c19, 0x11d1, 0x8b, 0xf1, 0x00, 0x00, 0xf8, 0x75, 0x40, 0x35};
#endif
...
BOOL RP_InitOle()
{
    ...
    g_pOle->CoInitialize = (PF_COINITIALIZE)GetProcAddress(g_hOleLib, "CoInitialize");
    g_pOle->CoCreateInstance = (PF_COCREATEINSTANCE)GetProcAddress(g_hOleLib, "CoCreateInstance");
                          ...
                          INT                *pError)
{
    ...
    Socket = g_UpCallTable.lpWPUCreateSocketHandle(dwCatalogEntryId, dwContext, pError);
    ...
    if (!Socket) {
        ...
        *pError = GetLastError();
        ...
        Socket = g_UpCallTable.lpWPUModifyIFSHandle(dwCatalogEntryId, Socket, pError);
    }
    ...
    hr = pIRestrictedProcess->RP_WahOpenHandleHelper(dwPid, (DWORD*)&hHelperHandle, &dwStatus);
    if (FAILED(hr) || (*pError != NO_ERROR)) {
        *pError = WSASYSCALLFAILURE;
        return SOCKET_ERROR;
    }

    hr = pIRestrictedProcess->RP_WahCreateSocketHandle(dwPid, (DWORD)hHelperHandle, (DWORD*)&Socket, &dwStatus);
    if (FAILED(hr) || (*pError != NO_ERROR)) {
...
INT CloseSocketHandle(RSOCKET *pRSocket, INT     *pError)
{
    INT  iStatus;

#if defined(LOCAL_HANDLES)
    iStatus = g_UpCallTable.lpWPUCloseSocketHandle(pRSocket->GetContext(), pError);
#elif defined(LOCAL_EVENT_HANDLES)
...
//  INVALID_SOCKET is returned, and a specific error code is available in *pError.

//  NOTE: For a restricted process, WSPAccept() always fails.

//  s              - A descriptor identiying a socket which is listening for connections after a WSPListen().
...
//                   as a condition function parameter. This parameter is not interpreted by the service provider.
//  pError         - A pointer to the error code.                        
{
    ...
    if (pRSocket) {
        ...
        NewProviderSocket = Provider->WSPAccept(ProviderSocket, addr, addrlen, lpfnCondition, dwCallbackData, pError);
        if (INVALID_SOCKET != NewProviderSocket) {
            // Create a new socket object and initialize it.
            pNewRSocket = new RSOCKET;
            if (pNewRSocket) {
                ReturnValue = CreateSocketHandle(0, pRSocket->GetCatalogEntryId(), (ULONG_PTR)pNewRSocket, pError);
                DEBUGF(DBG_TRACE, ("Accept Returning Socket %X\n", ReturnValue));

                if (INVALID_SOCKET != ReturnValue) {
                    pNewRSocket->Initialize(Provider, NewProviderSocket, pRSocket->GetCatalogEntryId(), ReturnValue, 0); // pIRestrictedProcess
    ...
    *pError = WSAEACCES;
    ...
    POSTAPINOTIFY((DTCODE_WSPAccept, &ReturnValue, g_szLibraryName, &s, &addr, &addrlen, &lpfnCondition, &dwCallbackData, &pError));
                              ...
                              OUT    LPINT pError)
{
    ...
    ReturnValue = gProviderCatalog->FindNextProviderInChain(lpProtocolInfo, &ThisProviderCatalogEntryId, &NextProviderCatalogEntryId);
    if (ReturnValue == NO_ERROR) {
        // Get the provider for the catlog entry
        ReturnValue = gProviderCatalog->GetCatalogItemFromCatalogEntryId(NextProviderCatalogEntryId, &CatalogItem);
        if (ReturnValue == NO_ERROR) {
            // Get the provider for the catlog entry
            ReturnValue = gProviderCatalog->GetCatalogItemFromCatalogEntryId(NextProviderCatalogEntryId, &CatalogItem);
            if (ReturnValue == NO_ERROR) {
...
INT WSPAPI WSPAsyncSelect(IN SOCKET s, IN HWND hWnd, IN unsigned int wMsg, IN long lEvent, OUT INT FAR *pError)
/*++
Routine Description:
    Request  Windows  message-based  event notification of network events for a socket.
Arguments:
    s       - A  descriptor identiying a socket for which event notification is required.
    ...
    lEvent  - bitmask  which specifies a combination of network events in which the WinSock client is interested.
    pError - A pointer to the error code.
Return Value:
...
--*/
{
    ...
    if (PREAPINOTIFY((DTCODE_WSPAsyncSelect, &ReturnValue, g_szLibraryName, &s, &hWnd, &wMsg, &lEvent, &pError))) {
        return ReturnValue;
    }
    ...
    pRSocket = RSOCKET::FindListDSocket(s);
    if (pRSocket) {
        *pError = pRSocket->RegisterAsyncOperation(hWnd, wMsg, lEvent);
        if (NO_ERROR == *pError) {
    ...
    POSTAPINOTIFY((DTCODE_WSPAsyncSelect, &ReturnValue, g_szLibraryName, &s, &hWnd, &wMsg, &lEvent, &pError));

    return ReturnValue;
}


INT WSPAPI WSPBind(IN SOCKET s, IN const struct sockaddr FAR *name, IN INT namelen, OUT INT FAR *pError)
/*++
Routine Description:
...
--*/
{
    ...
    if (PREAPINOTIFY((DTCODE_WSPBind, &ReturnValue, g_szLibraryName, &s, &name, &namelen, &pError))) {
        return(ReturnValue);
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPBind(ProviderSocket, name, namelen, pError);

        POSTAPINOTIFY((DTCODE_WSPBind, &ReturnValue, g_szLibraryName, &s, &name, &namelen, &pError));
    }
...
INT WSPAPI WSPCancelBlockingCall(OUT INT FAR *pError)
/*++
Routine Description:
...
--*/
{
   ...
   //      if (PREAPINOTIFY(( DTCODE_WSPCancelBlockingCall, &ReturnValue, g_szLibraryName, &pError)) ) {
   //          return(ReturnValue);
   //      }

   //      ReturnValue = gProcTable.lpWSPCancelBlockingCall(pError);

   //      POSTAPINOTIFY(( DTCODE_WSPCancelBlockingCall, &ReturnValue, g_szLibraryName, &pError))
   //      return(ReturnValue);
       // **
       // Note:
       ...
       // in thread local storage so we can know what provider to send this call to.
    return(WSAEINVAL);
}
...
INT WSPAPI WSPCleanup(OUT INT *pError)
{
    ...
    if (PREAPINOTIFY((DTCODE_WSPCleanup, &ReturnValue, g_szLibraryName, &pError))) {
        return(ReturnValue);
    }
    ...
    if (g_StartupCount > 0) {
        ...
        if (g_StartupCount == 0) {
            ...
            delete(gProviderCatalog);// Kill the ProviderCatalog            
            delete(gWorkerThread);// Terminate worker thread:            
            delete(gBufferManager);// Kill the buffer manager:
    ...
    POSTAPINOTIFY((DTCODE_WSPCleanup, &ReturnValue, g_szLibraryName, &pError));
...
//  value of SOCKET_ERROR is returned, and the specific error code is returned in *pError.
INT WSPAPI WSPCloseSocket(IN  SOCKET s, OUT INT   *pError)
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    RPROVIDER   *pProvider;
    ...
    if (PREAPINOTIFY((DTCODE_WSPCloseSocket, &ReturnValue, g_szLibraryName, &s, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        DEBUGF(DBG_TRACE, ("Closing socket %X\n", s));
    ...
    POSTAPINOTIFY((DTCODE_WSPCloseSocket, &ReturnValue, g_szLibraryName, &s, &pError));
...
//  namelen      - The length of the name.
//  lpCallerData - A  pointer to the user data that is to be transferred to the peer during connection established.
//  lpCalleeData - A pointer to a buffer into which may be copied any user data received from the peer during connection establishment.
...
INT WSPAPI WSPDuplicateSocket(IN SOCKET s, IN DWORD dwProcessID, IN LPWSAPROTOCOL_INFOW lpProtocolInfo, OUT INT FAR *pError)
/*++
Routine Description:
...
Arguments:
    ...
    lpProtocolInfo - A  pointer  to  a  buffer  allocated by the client that is large enough to contain a WSAPROTOCOL_INFOA struct.
...
Return Value:
    ...
    value of SOCKET_ERROR is returned, and a specific error number is available in pError.
--*/
{
    INT ReturnValue = SOCKET_ERROR;

    //      if (PREAPINOTIFY(( DTCODE_WSPDuplicateSocket, &ReturnValue, g_szLibraryName, &s, &dwProcessID, &lpProtocolInfo, &pError)) ) {
    //          return(ReturnValue);
    //      }

    //      ReturnValue = gProcTable.lpWSPDuplicateSocket(s, dwProcessID, lpProtocolInfo, pError);
    //      POSTAPINOTIFY(( DTCODE_WSPDuplicateSocket, &ReturnValue, g_szLibraryName, &s, &dwProcessID, &lpProtocolInfo, &pError));

    return(ReturnValue);
}


INT WSPAPI WSPEnumNetworkEvents(IN SOCKET s, OUT WSAEVENT hEventObject, OUT LPWSANETWORKEVENTS lpNetworkEvents, OUT INT FAR *pError)
/*++
Routine Description:
...
Return Value:
    ...
    Otherwise  the  value SOCKET_ERROR is returned, and a specific error number is available in pError.
--*/
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    ...
    if (PREAPINOTIFY((DTCODE_WSPEnumNetworkEvents, &ReturnValue, g_szLibraryName, &s, &hEventObject, &lpNetworkEvents, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPEnumNetworkEvents(ProviderSocket, hEventObject, lpNetworkEvents, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPEnumNetworkEvents, &ReturnValue, g_szLibraryName, &s, &hEventObject, &lpNetworkEvents, &pError));

    return ReturnValue;
}


INT WSPAPI WSPEventSelect(IN SOCKET s, IN OUT WSAEVENT hEventObject, IN long lNetworkEvents, OUT INT FAR *pError)
/*++
Routine Description:
    Specify  an  event object to be associated with the supplied set of network events.
Arguments:
    s              - A descriptor identifying the socket.
    hEventObject   - A  handle  identifying  the  event object to be associated with the supplied set of network events.
    lNetworkEvents - A  bitmask  which  specifies  the  combination  of network events in which the WinSock client has interest.
    pError        - A pointer to the error code.
Return Value:
    ...
    SOCKET_ERROR is returned, and a specific error number is available in pError
--*/
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    ...
    if (PREAPINOTIFY((DTCODE_WSPEventSelect, &ReturnValue, g_szLibraryName, &s, &hEventObject, &lNetworkEvents, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPEventSelect(ProviderSocket, hEventObject, lNetworkEvents, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPEventSelect, &ReturnValue, g_szLibraryName, &s, &hEventObject, &lNetworkEvents, &pError));

    return ReturnValue;
}


INT WSPAPI WSPGetOverlappedResult(IN SOCKET s,
...
--*/
{
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPGetOverlappedResult(ProviderSocket, lpOverlapped, lpcbTransfer, fWait, lpdwFlags, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }
...
INT WSPAPI WSPGetPeerName(IN SOCKET s, OUT struct sockaddr FAR *name, OUT INT FAR *namelen, OUT INT FAR *pError)
/*++
Routine Description:
...
Arguments:
    ...
    namelen - A  pointer  to  an integer which, on input, indicates the size of
              the  structure  pointed  to  by name, and on output indicates the size of the returned name.
    pError - A pointer to the error code.
Return Value:
    If  no  error occurs, WSPGetPeerName() returns NO_ERROR.  Otherwise, a
    value  of  SOCKET_ERROR is returned, and a specific error code is available in pError
--*/
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    ...
    if (PREAPINOTIFY((DTCODE_WSPGetPeerName, &ReturnValue, g_szLibraryName, &s, &name, &namelen, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPGetPeerName(ProviderSocket, name, namelen, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPGetPeerName, &ReturnValue, g_szLibraryName, &s, &name, &namelen, &pError));

    return ReturnValue;
}


INT WSPAPI WSPGetQOSByName(IN SOCKET s, IN LPWSABUF lpQOSName, IN LPQOS lpQOS, OUT INT FAR *pError)
/*++
Routine Description:
...
Return Value:
    ...
    the  return  value  is  FALSE, and  a  specific error code is available in pError.
--*/
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    ...
    if (PREAPINOTIFY((DTCODE_WSPGetQOSByName, &ReturnValue, g_szLibraryName, &s, &lpQOSName, &lpQOS, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPGetQOSByName(ProviderSocket, lpQOSName, lpQOS, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPGetQOSByName, &ReturnValue, g_szLibraryName, &s, &lpQOSName, &lpQOS, &pError));

    return(ReturnValue);
}


INT WSPAPI WSPGetSockName(IN SOCKET s, OUT struct sockaddr FAR *name, OUT INT FAR *namelen, OUT INT FAR *pError)
/*++
Routine Description:
...
--*/
{
    ...
    if (PREAPINOTIFY((DTCODE_WSPGetSockName, &ReturnValue, g_szLibraryName, &s, &name, &namelen, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPGetSockName(ProviderSocket, name, namelen, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPGetSockName, &ReturnValue, g_szLibraryName, &s, &name, &namelen, &pError));
...
INT WSPAPI WSPGetSockOpt(IN SOCKET s,
    ...
    //  SOCKET_ERROR is returned, and a specific error code is returned in pError.

    //  s       - A descriptor identifying a socket.
    //  level   - The level at  which the option is defined; the supported levels include SOL_SOCKET (See annex for more protocol-specific levels.)
    ...
    //  pError  - A pointer to the error code.
{
    ...
    if (pRSocket) {
        ...
        if (pIRestrictedProcess) {
            ...
            hr = pIRestrictedProcess->RP_GetSockOpt(level, optname, (UCHAR*)optval, optlen, pError);
            if (FAILED(hr)) {
                *pError = WSAEACCES;
                ReturnValue = SOCKET_ERROR;
            } else if (*pError) {
            ...
            ReturnValue = pProvider->WSPGetSockOpt(ProviderSocket, level, optname, optval, optlen, pError);
        }
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    ...
    POSTAPINOTIFY((DTCODE_WSPGetSockOpt, &ReturnValue, g_szLibraryName, &s, &level, &optname, &optval, &optlen, &pError));
...
Arguments:
    ...
    lpThreadId          - A  pointer to a thread ID structure to be used by the provider
    pError             - A pointer to the error code.
Return Value:
    ...
    Otherwise, a value of SOCKET_ERROR is returned, and  a  specific  error code is available in pError.
    The error code  WSA_IO_PENDING  indicates  that  an  overlapped  operation  has  been
    successfully  initiated  and  that  conpletion will be indicated at a later time.
...
/*++
Routine Description:
    Join  a  leaf  node  into  a multipoint session, exchange connect data, and specify needed quality of service based on the supplied flow specs.
Arguments:
...
--*/
{
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPJoinLeaf(ProviderSocket, name, namelen, lpCallerData, lpCalleeData, lpSQOS, lpGQOS, dwFlags, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }
...
INT WSPAPI WSPListen(IN SOCKET s, IN INT backlog, OUT INT FAR *pError)
//  Sets a socket to listen for incoming connections.
//  If no error occurs, WSPListen() returns 0.
//  Otherwise, a value of SOCKET_ERROR is returned, and a specific error code is returned in pError.
...
//  pError  - A pointer to the return error code.
{
    ...
    if (PREAPINOTIFY((DTCODE_WSPListen, &ReturnValue, g_szLibraryName, &s, &backlog, &pError))) {
        return(ReturnValue);
    }
    ...
    if (pRSocket) {
        pProvider = pRSocket->GetDProvider();
        ProviderSocket = pRSocket->GetSocketHandle();
        ReturnValue = pProvider->WSPListen(ProviderSocket, backlog, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }
    ...
    *pError = WSAEACCES;
    ...
    POSTAPINOTIFY((DTCODE_WSPListen, &ReturnValue, g_szLibraryName, &s, &backlog, &pError));
...
Arguments:
    ...
    pError              - A pointer to the error code.
Return Value:
    If  no  error  occurs  and the receive operation has completed immediately, WSPRecv() returns the number of bytes received.
    If the connection has been closed, it  returns  0.
    Note that in this case the completion routine, if specified,  will   have  already  been  queued.
    Otherwise, a  value  of SOCKET_ERROR  is  returned, and  a  specific  error  code  is available in pError.
    The  error  code WSA_IO_PENDING indicates that the overlapped an
    ...
    overlapped  operations  was  initiated  and  no  completion indication will occur.
--*/
{
    INT              ReturnValue = NO_ERROR;
    RSOCKET         *pRSocket;
    ...
    if (pRSocket) {
        ...
        ReturnValue = gBufferManager->AllocBuffer(lpBuffers, dwBufferCount, &InternalBuffers, &InternalBufferCount);
        if (ReturnValue == NO_ERROR) {
...
INT WSPAPI WSPRecvDisconnect(IN SOCKET s, IN LPWSABUF lpInboundDisconnectData, OUT INT FAR *pError)
/*++
Routine Description:
...
Return Value:
    ...
    a value of SOCKET_ERROR is returned, and a specific error code is available in pError.
--*/
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    ...
    if (PREAPINOTIFY((DTCODE_WSPRecvDisconnect, &ReturnValue, g_szLibraryName, &s, &lpInboundDisconnectData, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPRecvDisconnect(ProviderSocket, lpInboundDisconnectData, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPRecvDisconnect, &ReturnValue, g_szLibraryName, &s, &lpInboundDisconnectData, &pError));
...
Arguments:
    ...
    lpFrom               - An  optional pointer to a buffer which will hold the
                           source address upon the completion of the overlapped operation.
    lpFromlen            - A  pointer  to the size of the from buffer, required only if lpFrom is specified.
    lpOverlapped         - A pointer to a WSAOVERLAPPED structure.
    CompletionRoutine    - A  pointer to the completion routine called when the receive operation has been completed.
    lpThreadId           - A pointer to a thread ID structure to be used by the provider in a subsequent call to WPUQueueApc().
    pError              - A pointer to the error code.
Return Value:
                                     ...
                                     OUT LPINT    Errno)
{
    ...
    if (UserSet && (UserSet->fd_count > 0)) {
        ...
        SocketMap->Associations = (PSOCKETASSOCIATION)new BYTE[(sizeof(SOCKETASSOCIATION) * UserSet->fd_count)];
        if (SocketMap->Associations) {
            for (Index = 0; Index < UserSet->fd_count; Index++) {
                ...
                SocketMap->Associations[Index].ProviderSocket = pRSocket->GetSocketHandle();

                SocketMap->Associations[Index].UserSocket = UserSet->fd_array[Index];
                ProviderSet->fd_array[Index] = SocketMap->Associations[Index].ProviderSocket;
...
Return Value:
    ...
    SOCKET_ERROR  if an error occurred.  If the return value is SOCKET_ERROR, a specific error code is available in pError.
--*/
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    ...
    if (FoundSocket) {
        ...
        if (pRSocket) {
            ...
            TransferUserFdSetToProviderFdSet(readfds, &InternalReadfds, &ReadMap, pError);
            TransferUserFdSetToProviderFdSet(writefds, &InternalWritefds, &WriteMap, pError);
            TransferUserFdSetToProviderFdSet(exceptfds, &InternalExceptfds, &ExceptMap, pError);
            if (*pError == NO_ERROR) {
                ReturnValue = pProvider->WSPSelect(nfds, &InternalReadfds, &InternalWritefds, &InternalExceptfds, timeout, pError);
                TransferProviderFdSetToUserFdSet(readfds, &InternalReadfds, &ReadMap, pError);
                TransferProviderFdSetToUserFdSet(writefds, &InternalWritefds, &WriteMap, pError);
                TransferProviderFdSetToUserFdSet(exceptfds, &InternalExceptfds, &ExceptMap, pError);
    ...
    POSTAPINOTIFY((DTCODE_WSPSelect, &ReturnValue, g_szLibraryName, &nfds, &readfds, &writefds, &exceptfds, &timeout, &pError));
                   ...
                   OUT INT FAR *pError)
{
    ...
    if (ReturnValue != SOCKET_ERROR) {
        ...
        if (NO_ERROR == ReturnValue) {
            ...
            ReturnValue = gBufferManager->CopyBuffer(lpBuffers, dwBufferCount, InternalBuffers, InternalBufferCount, &InternalBytesTransfered);
        }
...
INT WSPAPI WSPSendDisconnect(IN SOCKET s, IN LPWSABUF lpOutboundDisconnectData, OUT INT FAR *pError)
/*++
Routine Description:
...
Return Value:
    ...
    SOCKET_ERROR  is  returned, and  a  specific  error  code  is available in pError.
--*/
{
    INT          ReturnValue = NO_ERROR;
    RSOCKET     *pRSocket;
    ...
    if (PREAPINOTIFY((DTCODE_WSPSendDisconnect, &ReturnValue, g_szLibraryName, &s, &lpOutboundDisconnectData, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPSendDisconnect(ProviderSocket, lpOutboundDisconnectData, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPSendDisconnect, &ReturnValue, g_szLibraryName, &s, &lpOutboundDisconnectData, &pError));
...
Return Value:
    ...
    SOCKET_ERROR  is  returned, and  a  specific  error  code  is available in
    pError.   The  error  code  WSA_IO_PENDING  indicates  that the overlapped
    operation  has  been  successfully  initiated  and  that completion will be indicated  at  a  later  time.
    Any  other  error  code  indicates that no overlapped operation was initiated and no completion indication will occur.
--*/
{
    INT          ReturnValue = NO_ERROR;
    ...
    if ((optname != SO_KEEPALIVE)
        ...
        *pError = WSAENOPROTOOPT;
        ...
        if (pIRestrictedProcess) {
            // Note that in this case, the Provider Socket is still
            // remote, in the helper process.
            hr = pIRestrictedProcess->RP_SetSockOpt(level, optname, (UCHAR*)optval, optlen, pError);
            if (FAILED(hr)) {
                *pError = WSAEACCES;
                ReturnValue = SOCKET_ERROR;
            } else if (*pError) {
            ...
            ReturnValue = pProvider->WSPSetSockOpt(ProviderSocket, level, optname, optval, optlen, pError);
        }
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
...
INT WSPAPI WSPShutdown(IN SOCKET s, IN INT how, OUT INT FAR *pError)
//  WSPShutdown()
...
//  s       - A descriptor identifying a socket.
//  how     - A flag that describes what types of operation will no longer be allowed (SD_SEND, SD_RECEIVE or SD_BOTH).
//  pError  - A pointer to the error code.
{
    INT ReturnValue;
    PRSOCKET     pRSocket;
    ...
    if (PREAPINOTIFY((DTCODE_WSPShutdown, &ReturnValue, g_szLibraryName, &s, &how, &pError))) {
        return ReturnValue;
    }
    ...
    if (pRSocket) {
        ...
        ReturnValue = pProvider->WSPShutdown(ProviderSocket, how, pError);
    } else {
        ReturnValue = SOCKET_ERROR;
        *pError = WSAENOTSOCK;
    }

    POSTAPINOTIFY((DTCODE_WSPShutdown, &ReturnValue, g_szLibraryName, &s, &how, &pError));
    ...
    //  pError        - Returns the error code
{
    ...
    if (RP_IsRestrictedProcess()) {
        ...
        // process.
        hr = g_pOle->CoCreateInstance(CLSID_RestrictedProcess, NULL, CLSCTX_LOCAL_SERVER, IID_IRestrictedProcess, (VOID**)&pIRestrictedProcess);
        if (FAILED(hr)) {
            *pError = WSAEACCES;
            return INVALID_SOCKET;
        }
    ...
    // Get the catlog entry for the next provider in the chain
    ReturnValue = gProviderCatalog->FindNextProviderInChain(pProtocolInfo, &ThisProviderCatalogEntryId, &NextProviderCatalogEntryId);
    if (NO_ERROR == ReturnValue) {
        // Get the provider for the catlog entry
        ReturnValue = gProviderCatalog->GetCatalogItemFromCatalogEntryId(NextProviderCatalogEntryId, &CatalogItem);
        if (NO_ERROR == ReturnValue) {
            ...
            if (!RP_IsRestrictedProcess()) {
                ...
                ReturnValue = pProvider->WSPSocket(af, type, protocol, CatalogItem->GetProtocolInfo(), g, dwFlags, pError);
            ...
            if (pRSocket) {
                ReturnValue = CreateSocketHandle(pIRestrictedProcess, ThisProviderCatalogEntryId, (ULONG_PTR)pRSocket, pError);

                DEBUGF(DBG_TRACE, ("Socket Returning Socket %X\n", ReturnValue));
...
INT WSPAPI WSPStringToAddress(IN     LPWSTR AddressString,
    ...
    //  lpProtocolInfo - (optional) the WSAPROTOCOL_INFO struct for a particular provider. May be NULL.
    //  Address        - Buffer which is filled with a single SOCKADDR structure.
    //  lpAddressLength - The length of the Address buffer.  Returns the size of the resultant SOCKADDR structure.
{
    INT                 ReturnValue;
    DWORD               NextProviderCatalogEntryId;
    DWORD               ThisProviderCatalogEntryId;
    ...
    if (NO_ERROR == ReturnValue) {
        ...
        if (NO_ERROR == ReturnValue) {
            ...
            if (NO_ERROR == ReturnValue) {
                ...
                ReturnValue = Provider->WSPStringToAddress(AddressString, AddressFamily, lpProtocolInfo, lpAddress, lpAddressLength, pError);
            }
        }
    }
...
int WSPAPI WSPStartup(IN  WORD wVersion, OUT LPWSPDATA lpWSPData,
                      IN  LPWSAPROTOCOL_INFOW lpProtocolInfo,
                      IN  WSPUPCALLTABLE UpcallTable,
                      OUT LPWSPPROC_TABLE lpProcTable)
    //  Initialize LSP. On success return NO_ERROR. Otherwise return a WSA error code.
    ...
    //  lpProcTable - A pointer to the table of SPI function pointers.
{
    ...
    if (g_StartupCount == 0) {
        ...
        g_UpCallTable = UpcallTable;// Save the WinSock2 upcall table

        RSOCKET::InitListDSocket();// Initialize the socket list
        ...
        if ((NO_ERROR == ReturnCode) && (RP_IsRestrictedProcess())) {
