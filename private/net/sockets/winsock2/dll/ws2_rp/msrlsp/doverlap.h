/*++
     ...
     infringement of any proprietary rights, and including the warranties of merchantability and fitness for a particular purpose.
...
class DOVERLAPPEDSTRUCTMGR
{
  public:
    ...
    PINTERNALOVERLAPPEDSTRUCT AllocateOverlappedStruct();
    VOID FreeOverlappedStruct(LPWSAOVERLAPPED   pOverlappedStruct);
    PAPCCONTEXT AllocateApcContext();
    VOID FreeApcContext(PAPCCONTEXT Context);
    PINTERNALOVERLAPPEDSTRUCT GetInternalStuctPointer(LPWSAOVERLAPPED OverlappedStruct);

  private:
    PINTERNALOVERLAPPEDSTRUCT PopOverlappedStruct();
    VOID PushOverlappedStruct(LPWSAOVERLAPPED OverlappedStruct);
...
inline PINTERNALOVERLAPPEDSTRUCT DOVERLAPPEDSTRUCTMGR::GetInternalStuctPointer(LPWSAOVERLAPPED OverlappedStruct)
/*++
Routine Description:
    Calculates the address of and internal overlapped structure from the address of the Overlapped structure passed to the underlaying provider.
Arguments:
    OverlappedStruct - A pointer to an WSAOVERLAPPED structure that is part of an internal overlapped structure.
Return Value:
    The address of the internal overlapped structure containing
    OverlappedStruct.
*/
{
    return( (PINTERNALOVERLAPPEDSTRUCT) ((ULONG_PTR) OverlappedStruct - offsetof(INTERNALOVERLAPPEDSTRUCT, InternalOverlappedStruct)));
}
