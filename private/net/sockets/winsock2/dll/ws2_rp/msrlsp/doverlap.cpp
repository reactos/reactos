...
/*++
Routine Description:
    DOVERLAPPEDSTRUCTMGR  object  constructor.
    Creates and returns a DOVERLAPPEDSTRUCTMGR object.
    Note that  the  DOVERLAPPEDSTRUCTMGR object has not been fully initialized.  The "Initialize" member function must be the first member function called on the new DOVERLAPPEDSTRUCTMGR object.
Arguments:
    None
Return Value:
    None
...
INT DOVERLAPPEDSTRUCTMGR::Initialize()
/*++
Routine Description:
    The initialization routine for a DOVERLAPPEDSTRUCTMGR  object.
    This procedure completes the initialzation of the object.
...
Arguments:
    None
Return Value:
    The  function returns NO_ERROR if successful.
    Otherwise it returns an appropriate WinSock error code if the initialization cannot be completed.
--*/
{
    INT   ReturnCode;
    ULONG BlockSize;
    ...
    BlockSize = sizeof(INTERNALOVERLAPPEDSTRUCT) * OUTSTANDINGOVERLAPPEDSTRUCTS;
    ...
    if (m_overlapped_struct_block){
        ...
        while (BytesAvailable > StructSize){
            PushEntryList(&m_overlapped_free_list, (PSINGLE_LIST_ENTRY) CurrentBlock);
    ...
    if (NO_ERROR == ReturnCode){
        ...
        if (m_context_struct_block){
            ...
            while (BytesAvailable > StructSize){
                PushEntryList(&m_context_free_list, (PSINGLE_LIST_ENTRY) CurrentBlock);
                BytesAvailable -= StructSize;
                CurrentBlock += StructSize;
            } //while
            ReturnCode = NO_ERROR;
...
/*++
Routine Description:
    DOVERLAPPEDSTRUCTMGR object destructor.
    This procedure has the responsibility to perform any required shutdown operations for the DOVERLAPPEDSTRUCTMGR object before the object memory is deallocated.
Arguments:
    None
Return Value:
    None
...
PINTERNALOVERLAPPEDSTRUCT DOVERLAPPEDSTRUCTMGR::AllocateOverlappedStruct()
/*++
Routine Description:
    Allocates an interanl overlapped structure from the pool of available structures.
Arguments:
    None
Return Value:
...
VOID DOVERLAPPEDSTRUCTMGR::FreeOverlappedStruct(LPWSAOVERLAPPED   OverlappedStruct)
/*++
Routine Description:
    Frees an interanl overlapped structure to the pool of available structures.
Arguments:
    OverlappedStruct - A pointer to the InternalOverlappedStruct member of an interanl overlapped struct.
Return Value:
    NONE
--*/
{
...
PINTERNALOVERLAPPEDSTRUCT DOVERLAPPEDSTRUCTMGR::PopOverlappedStruct()
/*++
Routine Description:
    Pops a internal overlapped structure off of the free list and initializes the structure.
Arguments:
    NONE
Return Value:
...
--*/
{
    ...
    ReturnValue = (PINTERNALOVERLAPPEDSTRUCT)PopEntryList(&m_overlapped_free_list);
    if (ReturnValue){
        // Init the structure
        ZeroMemory(ReturnValue, sizeof(INTERNALOVERLAPPEDSTRUCT));
        ReturnValue->Signature = STRUCTSIGNATURE;
    } //if
...
VOID DOVERLAPPEDSTRUCTMGR::PushOverlappedStruct(LPWSAOVERLAPPED OverlappedStruct)
/*++
Routine Description:
    Pushes an internal overlapped structure onto the free list.
Arguments:
    A pointer to the InteranlOverlappedStruct member of an internal overlapped struct.
Return Value:
    NONE
--*/
{
    ...
    InternalOverlappedStruct = GetInternalStuctPointer(OverlappedStruct);
    assert(STRUCTSIGNATURE == InternalOverlappedStruct->Signature);
    PushEntryList(&m_overlapped_free_list, (PSINGLE_LIST_ENTRY)InternalOverlappedStruct);
    LeaveCriticalSection(&m_overlapped_free_list_lock);
}


PAPCCONTEXT DOVERLAPPEDSTRUCTMGR::AllocateApcContext()
/*++
Routine Description:
...
--*/
{
    ...
    if (ReturnValue){
        ZeroMemory(ReturnValue, sizeof(APCCONTEXT));
    } //if
...
VOID DOVERLAPPEDSTRUCTMGR::FreeApcContext(PAPCCONTEXT Context)
/*++
Routine Description:
...
{
    EnterCriticalSection(&m_overlapped_free_list_lock);
    PushEntryList(&m_overlapped_free_list, (PSINGLE_LIST_ENTRY)Context);
    LeaveCriticalSection(&m_overlapped_free_list_lock);
}