...
class RPROVIDER {
  ...
  public:
    ...
    INT WSPListen(IN SOCKET s, IN INT backlog, OUT INT FAR *lpErrno);
...
inline INT
RPROVIDER::WSPCancelBlockingCall(OUT INT FAR *lpErrno)
/*++
Routine Description:
...

{
    ...
    ReturnValue = m_proctable->lpWSPCleanup(
        lpErrno);
...
inline INT
RPROVIDER::WSPEnumNetworkEvents(
    IN SOCKET s,
    OUT WSAEVENT hEventObject,
    OUT LPWSANETWORKEVENTS lpNetworkEvents,
    OUT INT FAR *lpErrno
...
inline INT
RPROVIDER::WSPGetOverlappedResult(
    IN SOCKET s,
    IN LPWSAOVERLAPPED lpOverlapped,
    IN LPDWORD lpcbTransfer,
    IN BOOL fWait,
...
inline INT RPROVIDER::WSPListen(IN SOCKET s, IN INT backlog, OUT INT FAR *lpErrno)
/*++
Routine Description:
...
Arguments:
    ...
    backlog - The  maximum length to which the queue of pending connections may grow.   
              If  this  value  is  SOMAXCONN, then the service provider should set the backlog to a maximum "reasonable" value.
    lpErrno - A pointer to the error code.
Return Value:
...
*/
{
    ...
    if (PREAPINOTIFY(( DTCODE_WSPListen, &ReturnValue, m_lib_name, &s, &backlog, &lpErrno)) ) {
        return(ReturnValue);
    }

    ReturnValue = m_proctable->lpWSPListen(s, backlog, lpErrno);

    POSTAPINOTIFY(( DTCODE_WSPListen, &ReturnValue, m_lib_name, &s, &backlog, &lpErrno));
...
*/
{
    ...
    assert(m_proctable->lpWSPSocket);
