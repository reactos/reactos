...
CRestrictedProcess::CRestrictedProcess( LPUNKNOWN    pUnkOuter, PFNDESTROYED pfnDestroy )
{
    m_cRef = 0;
...
STDMETHODIMP CImpIRestrictedProcess::RP_WahCreateSocketHandle( IN  DWORD  dwTargetPid, IN  DWORD  dwHelperHandle, OUT DWORD *pdwSocket, OUT DWORD *pdwStatus )
{
    BOOL      fInherit;
    DWORD     dwSourcePid;
    DWORD     dwAccess;
...
//  Note that the newly created socket isn't retruned to the restricted process until the completion of the connect().
STDMETHODIMP CImpIRestrictedProcess::RP_WSPSocket(
    IN long   af,
    IN long   type,
    IN long   protocol,
    ...
    IN OUT long *pError )
{
    ...
    *pError = 0;
    ...
    m_Socket = WSASocketW( af, type, protocol, (WSAPROTOCOL_INFOW*)pProtocolInfo, (GROUP)g, dwFlags );
...
STDMETHODIMP CImpIRestrictedProcess::RP_GetSockOpt( IN int iLevel, IN int iOptname, IN unsigned char *pOptval, OUT int *piOptlen, OUT int *piError )
{
    int iRet = getsockopt(m_Socket, iLevel, iOptname, (char *) pOptval, piOptlen);
    if (iRet == SOCKET_ERROR)
    {
        *piError = WSAGetLastError();
    }
...
STDMETHODIMP CImpIRestrictedProcess::RP_HelperWSALookupServiceBeginW(
    ...
    OUT int   *piErrno )
{
    ...
    *piErrno = WSAEOPNOTSUPP;
    ...
    if ((0 != memcmp(pRestrictions->lpServiceClassId, &HostnameGuid, sizeof(GUID))) &&
        (0 != memcmp(pRestrictions->lpServiceClassId, &AddressGuid, sizeof(GUID))))
    {
    ...
    *piRet = WSALookupServiceBeginW((WSAQUERYSETW *) pRestrictions, dwControlFlags, (HANDLE*) pdwLookupHandle);
...
#define TO_OFFSET(base,offset,typ)         ((typ)((char*)(base) - (DWORD_PTR)(offset)))
...
#   define  MAX_IP_HOST_NAME  256
    ...
    if (!m_pHostEnt)
    {
        ...
        m_pHostEnt = gethostbyaddr( pAddr, lAddrLen, AF_INET );
    }
    ...
    if ((m_pHostEnt) && (lAddrLen == m_pHostEnt->h_length))
    {
        ...
        if (pListAddr)
        {
            ...
            if (MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, szHostName, 1+strlen(szHostName), wsHostName, MAX_IP_HOST_NAME))
            {
...
STDMETHODIMP CImpIRestrictedProcess::IsValidAddr( char  *pAddr,
    long   lAddrLen )
{
    HRESULT hr;
    BOOL    fIsValid;
    ...
    // If not, then the address the client specified (pAddr) is not a valid address to work with.
...
//  Utility function used by UnpackHostEnt() to convert a list of offsets into pointers.
VOID CImpIRestrictedProcess::UnpackList( PCHAR ** List, PCHAR Base )
{
    if(*List)
    {
        PCHAR * Addr;
...
VOID CImpIRestrictedProcess::UnpackHostEnt( HOSTENT *pHost )
{
    PCHAR pch;
...
//  The pBlob contains a HOSTENT (but with the pointers converted to offsets).
//  Make a copy of it into a local cache (m_pHostEnt) and unpack it into a true HOSTENT.
STDMETHODIMP CImpIRestrictedProcess::CacheHostEnt( BLOB *pBlob )
{
    if (m_pHostEnt)
    {
        delete m_pHostEnt;
...
HOSTENT * CImpIRestrictedProcess::CopyHostEnt( HOSTENT *pHost )
{
    int      i;
    DWORD    dwSize = 0;
    DWORD    dwNameSize = 0;
