...
/*++
Routine Description:
    The enumeration procedure for LookupBegin.
    Inspects each catalog item to see if it matches the selection criteria the query, if so adds the provider associated with the item to the list of providers involved in the query.
Arguments:
    PassBack - A context value passed to EunerateCatalogItems. It is really a pointer to a NSPENUMERATIONCONTEXT struct.
        ...
        CatalogEntry->GetEnabledState())
    {
        ...
        if (Provider==NULL) {
            ...
            if (ErrorCode!=ERROR_SUCCESS) {
                return TRUE;// no error if the provider won't load.
            }
            Provider = CatalogEntry->GetProvider();
