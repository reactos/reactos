...
--*/
{
    DEBUGF(DBG_TRACE, ("NSCATALOGENTRY constructor\n"));
...
INT NSCATALOGENTRY::InitializeFromRegistry(IN  HKEY  ParentKey, IN  INT   SequenceNum)
/*
Routine Description:
...
Arguments:
    ParentKey   - Supplies  an open registry key for the registry entry that is
                  the  parent  of  the registry item defining the catalog item, i.e., the catalog entry list key.
...
Routine Description:
    ...
    caller,  so  the  caller  is  free  to  deallocate the passed structures on return.
Arguments:
...
--*/
{
    ...
    wcscpy(m_LibraryPath, LibraryPath);
...
Routine Description:
    ...
    responsibility  to unload and/or destroy any dprovider associated with this catalog item if appropriate.
Arguments:
    None
Return Value:
    None
...
VOID NSCATALOGENTRY::SetProvider(IN  PNSPROVIDER  Provider)
/*
Routine Description:
...
INT NSCATALOGENTRY::WriteToRegistry(IN  HKEY  ParentKey, IN  INT   SequenceNum)
/*
Routine Description:
    ...
    that  the  catalog  portion of the registry is locked against competing I/O attempts.
Arguments:
...
VOID NSCATALOGENTRY::Enable(IN BOOLEAN EnableValue)
/*
Routine Description:
...
INT NSCATALOGENTRY::IoRegistry(IN  HKEY  EntryKey, IN  BOOL  IsRead)
/*
Routine Description:
...
--*/
{
    ...
    if (IsRead) {
        ...
        if (lresult != ERROR_SUCCESS) {
            DEBUGF(DBG_ERR, ("querying length of ProviderName entry\n"));
            return WSANO_RECOVERY;
        }
        if (m_providerDisplayString != NULL) {
            DEBUGF(DBG_ERR, ("should never be re-reading a catalog entry\n"));
            return WSANO_RECOVERY;
        }
        if (val >= MAX_PATH) {
            DEBUGF(DBG_ERR, ("cannot handle provider names > MAX_PATH\n"));
            return WSANO_RECOVERY;
        }
