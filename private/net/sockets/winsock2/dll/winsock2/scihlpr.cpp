...
Abstract:
    ...
    translating WSASERVICECLASSINFO structs from the ansi to unicode and uncode to ansi
...
DWORD CalculateBufferSize(BOOL IsAnsi, LPVOID Buffer)
{
    ...
    )
{
    ...
    __try {
        ...
        for (Index=0;Index < Source->dwCount ;Index++ ){
            ...
            CopyMemory(Target->lpClassInfos[Index].lpValue, Source->lpClassInfos[Index].lpValue, Source->lpClassInfos[Index].dwValueSize);
    ...
    )
{
    ...
    __try {
        ...
        // Toplevel structure
        FreeSpace = (LPBYTE)Target;
        CopyMemory(Target, Source, sizeof(WSASERVICECLASSINFOA));
        FreeSpace += sizeof(WSASERVICECLASSINFOA);

        // The array of WSANSCLASSINFO's
        Target->lpClassInfos = (LPWSANSCLASSINFOA)FreeSpace;
        CopyMemory(Target->lpClassInfos, Source->lpClassInfos, (sizeof(WSANSCLASSINFOA) * Source->dwCount));
