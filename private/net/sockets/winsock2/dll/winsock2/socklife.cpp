/*++
    ...
    This listing is supplied under the terms of a license agreement with Intel Corporation and may not be used, copied, nor disclosed except in
...
Arguments:
    af - An address family specification.
         The only format currently supported is PF_INET, which is the ARPA Internet address format.
...
Returns:
    A socket descriptor referencing the new socket.
    Otherwise, a value of INVALID_SOCKET is returned and the error code is stored with SetErrorCode.
--*/
{
    PDPROCESS Process;
    PDTHREAD  Thread;
    ...
    return(WSASocketW(af, type, protocol, NULL,      // lpProtocolInfo
                      0,         // g
                      dwFlags));
}
...
/*++
Routine Description:
    Create  a  socket  which is bound to a specific transport service provider, optionally create and/or join a socket group.
Arguments:
    af             - An   address   family   specification.
                     The  only  format currently supported is PF_INET, which is the ARPA Internet address format.
    type           - A type specification for the new socket.
    ...
    lpProtocolInfo - A pointer to a WSAPROTOCOL_INFOW struct that defines the characteristics  of  the  socket  to  be created.
                     If this parameter  is  not  NULL,  the first three parameters (af, type, protocol) are ignored.
...
--*/
{
    ...
    ErrorCode = PROLOG(&Process, &CurrentThread);
    if (ErrorCode != ERROR_SUCCESS) {
        SetLastError(ErrorCode);
        return(INVALID_SOCKET);
    }

    ReturnValue = INVALID_SOCKET;//Set Default return code
    Catalog = Process->GetProtocolCatalog();// Find a provider that can support the user request
    ...
    if (lpProtocolInfo) {
        ...
        ErrorCode = Catalog->GetCountedCatalogItemFromCatalogEntryId(dwCatalogId, &CatalogEntry);
    } else {
        dwCatalogId = 0;

    RestartCatalogLookupHack:
        ErrorCode = Catalog->GetCountedCatalogItemFromAttributes(af, type, protocol, dwCatalogId, &CatalogEntry);
        // If we failed to find a provider, try to reload the catalog from the registry and retry the lookup.
        // This handles the case (first noticed in CAIRO SETUP) where WS2_32.DLL is loaded and WSAStartup() is called *before* CAIRO SETUP has had the opportunity to install the necessary providers.
        // Later, CAIRO SETUP needs to create sockets.

        // Do not need this anymore, we now support dynamic notifications of protocol installation/removal and check for changes every time we access the catalog.
    } // else choosing from attributes

    if (ERROR_SUCCESS == ErrorCode) {
        Provider = CatalogEntry->GetProvider();
        if (lpProtocolInfo) {// Must  be  sure  we  use  the  client's lpProtocolInfo if one was supplied, to support the WSADuplicateSocket model.
        ...
        // Hack-O-Rama.
        // Temporary set the open type of the thread depending on the overlapped flag so that we can create appropriate socket handle for the layered service provider.
        // However, if layered service provide caches the handles we create for it, not much we can do.
        {
            INT oldOpenType = CurrentThread->GetOpenType();
            CurrentThread->SetOpenType((dwFlags & WSA_FLAG_OVERLAPPED) ? 0 : SO_SYNCHRONOUS_NONALERT);
            ReturnValue = Provider->WSPSocket(af, type, protocol, ProtocolInfo, g, dwFlags, &ErrorCode);// Now we have a provider that can support the user request lets ask get a socket
            CurrentThread->SetOpenType(oldOpenType);// Reset saved open type
        }

        // Hack-O-Rama. If WSPSocket() failed with the distinguished error code WSAEINPROGRESS *and* this was not a request for a specific provider (i.e. lpProtocolInfo == NULL) then
        // restart the catalog lookup starting at the current item (the current catalog id).

        // Snap the catalog id and dereference the catalog entry
        dwCatalogId = ProtocolInfo->dwCatalogEntryId;
        CatalogEntry->Dereference();

        if (ReturnValue == INVALID_SOCKET && ErrorCode == WSAEINPROGRESS && lpProtocolInfo == NULL) {
    ...
    SetLastError(ErrorCode);// There was an error, set this threads lasterror
    return(INVALID_SOCKET);
}
...
SOCKET WSAAPI WSASocketA(IN int af,
    ...
    Arguments:
        ...
        type           - A type specification for the new socket.
        protocol       - A  particular protocol to be used with the socket, or 0 if the caller does not wish to specify a protocol.
        lpProtocolInfo - A pointer to a WSAPROTOCOL_INFOA struct that defines the characteristics  of  the  socket  to  be created.
                         If this parameter  is  not  NULL,  the first three parameters (af, type, protocol) are ignored.
         g             - The identifier of the socket group.
         dwFlags       - The socket attribute specification.
    Returns:
    ...
    --*/
{
    ...
    if (lpProtocolInfo != NULL) {
        error = MapAnsiProtocolInfoToUnicode(lpProtocolInfo, &ProtocolInfoW);
    ...
    return WSASocketW(af, type, protocol, lpProtocolInfo ? &ProtocolInfoW : NULL, g, dwFlags);// Call through to the UNICODE version.
}


SOCKET WSAAPI accept(IN SOCKET s, OUT struct sockaddr FAR *addr, OUT int FAR *addrlen)
/*++
Routine Description:
...
Arguments:
    ...
    addr - An optional pointer to a buffer which receives the address of the
           ...
           format of the addr argument is determined by the address family established when the socket was  created.
...
SOCKET WSAAPI WSAAccept(IN SOCKET s,
    ...
    Routine Description:
         ...
         condition function, and optionally create and/or join a socket group.
    Arguments:
        s - A descriptor identifying a socket which is listening for connections after a listen().
        addr - An optional pointer to a buffer which receives the address of the
               connecting entity, as known to the communications layer. The exact
    ...
    Returns:
        A socket descriptor for the newly accepted socket on success, otherwise INVALID_SOCKET.
    --*/
{
    SOCKET             ReturnValue;
    INT                ErrorCode;
...
SOCKET WSPAPI WPUCreateSocketHandle(IN DWORD dwCatalogEntryId, IN DWORD_PTR lpContext, OUT LPINT lpErrno)
/*++
Routine Description:
            ...
            if (ERROR_SUCCESS == ErrorCode) {
                ...
                if (ErrorCode == ERROR_SUCCESS) {
                    ...
                    if (ErrorCode == ERROR_SUCCESS) {
                        ...
                        if (Socket) {
                            // Init the new socket
                            Socket->Initialize(Process, CatalogEntry);
    ...
    if (Process != NULL) {
        ...
        if (Socket != NULL) {
            if (!Socket->IsProviderSocket()) {
                ...
                DEBUGF(DBG_ERR, ("Foreign socket handle handed in by service provider for closure\n"));
                *lpErrno = WSAEINVAL;
                ReturnValue = SOCKET_ERROR;
            }
            Socket->DropDSocketReference();
            ...
            DEBUGF(DBG_ERR, ("Bad socket handle handed in by service provider for closure\n"));
            *lpErrno = WSAENOTSOCK;
            ReturnValue = SOCKET_ERROR;
        }
    } else {
    ...
    if (Socket != NULL) {
        if (!Socket->IsProviderSocket()) {
            *lpContext = Socket->GetContext();
            ...
            DEBUGF(DBG_ERR, ("Foreign socket handle handed in by service provider for query\n"));
            *lpErrno = WSAEINVAL;
        }
        Socket->DropDSocketReference();
    } else {
        DEBUGF(DBG_ERR, ("Bad socket handle handed in by service provider for query\n"));
        *lpErrno = ErrorCode;
    }
    return(ReturnCode);
}
...
WPUModifyIFSHandle(
    ...
    Arguments:
        ...
        ProposedHandle   - An  Installable File System(IFS) handle allocated by the provider.
        lpErrno          - A pointer to the error code.
    Returns:
...
Arguments:
    ...
    dwFlags      - Flags  to  indicate the socket acting as a sender, receiver, or both.
Returns:
...
--*/
{
    ...
    if (ErrorCode == ERROR_SUCCESS) {
        ...
        if (Socket != NULL) {
            ...
            Provider = Socket->GetDProvider();
            ReturnValue = Provider->WSPCloseSocket(s, &ErrorCode);
            if ((ReturnValue == ERROR_SUCCESS) && Socket->IsProviderSocket()) {
                ...
                // used only by the layered provider above and never returned via socket/WSASocket call. 
                // closesocket is not called for such socket and we never get to execute the code below.
                // which leads to socket object leak.
            ...
            // The close failed. Restore the context table entry if necessary.
