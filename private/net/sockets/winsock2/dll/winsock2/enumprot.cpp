/*++
    ...
    Intel Corporation and may not be used, copied, nor disclosed except in accordance with the terms of that agreeement.

Module Name:
    enumprot.cpp

Abstract:
    This module contains the WSAEnumProtocol entrypoint for the winsock API.
...
// TRUE  and  the  actual  number  of bytes needed for the path is written into CharsNeeded.
// The  path is copied into the buffer if there is enough space, otherwise the buffer is not used.
typedef struct
{
    GUID   ProviderId;
    LPWSTR PathBuffer;
...
// the enumeration procedure ProtocolIterationProc() and the functions it calls to do its work.
typedef struct
{
    LPINT               Protocols;
...
static VOID CopyProtocolInfo(IN LPWSAPROTOCOL_INFOW ProtocolInfo, IN PPROTOCOL_ENUMERATION_CONTEXT Context)
/*++
Routine Description:
    Copies a protocol info struct onto the end of a user buffer is enough room exists in the user buffer.
Arguments:
    ProtocolInfo - A Pointer to a WSAPROTOCOL_INFOW struct to be copied.
    Context - A Pointer to an enumeration context structure. This structure contians all the infomation about the user buffer.
--*/
{
    if (OK_TO_COPY_PROTOCOL(Context)) {
        CopyMemory(NEXT_BUFFER_LOCATION(Context), ProtocolInfo, sizeof(WSAPROTOCOL_INFOW));
...
static BOOL IsProtocolInSet(IN LPINT Set, IN LPWSAPROTOCOL_INFOW ProtocolInfo)
/*++
Routine Description:
    This function returns whether the protocol described by ProtocolInfo is a member of the set of protocols pointed to by Set.
Arguments:
...
--*/
{
    ...
    if (Set) {
        ...
        while (ProtocolID != END_PROTOCOL_LIST_MARKER) {
            if ((ProtocolID >= ProtocolInfo->iProtocol) && (ProtocolID <= (ProtocolInfo->iProtocol + ProtocolInfo->iProtocolMaxOffset))) {
                ReturnCode = TRUE;
                break;
            } //if
            SetIndex++;
...
static BOOL ProtocolIterationProcAPI(IN OUT PVOID IterationContext, IN PPROTO_CATALOG_ITEM  CatalogEntry)
/*++
Routine Description:
    This  function  is the enumeration procedure passed to the protocol catalog enumeration  function.
    It  adds  the  protocol  to  the list if (1) it is allowed  by the protocol list restriction set, (2) if it is not hidden, and (3) it is not a non-chained layer.
    "Adding to the list" only has effect if there is room in the buffer.
...
static BOOL ProtocolIterationProcSPI(IN OUT PVOID IterationContext, IN PPROTO_CATALOG_ITEM  CatalogEntry)
/*++
Routine Description:
...
--*/
{
    ...
    Catalog->EnumerateCatalogItems(ProtocolIterationProcAPI, &EnumerationContext);
...
PDCATALOG OpenInitializedCatalog()
{
    INT ReturnCode;
    ...
    if (protocol_catalog) {
        ...
        DEBUGF(DBG_ERR, ("Failed to allocate dcatalog object\n"));
    } //if
...
--*/
{
    ...
    Catalog->EnumerateCatalogItems(ProtocolIterationProcSPI, &EnumerationContext);
    ReturnCode = EnumerationContext.ProtocolCount;
    if (EnumerationContext.ErrorCode==ERROR_SUCCESS) {
        __try {
            // Start the iteration through the catalog. All the real work is
...
static BOOL PathIterationProc(IN OUT PVOID IterationContext, IN PPROTO_CATALOG_ITEM  CatalogEntry)
/*++
Routine Description:
    This function is the enumeration procedure passed to the protocol catalog enumeration function.
Arguments:
...
--*/
{
    ...
    if( ProtocolInfo->ProviderId == Context->ProviderId ) {
        ...
        {
            __try {
                result = MultiByteToWideChar(CP_ACP, 0, Path, -1, Context->PathBuffer, Context->PathBufferLength);
            }
            __except (WS2_EXCEPTION_FILTER()) {
                Context->ErrorCode = WSAEFAULT;
            }
...
Arguments:
    ...
    lpszProviderDllPath - A pointer to a buffer to hold the path to the provider DLL
    ProviderDLLPathLen - On input the size of lpszProviderDllPath, on output the number of bytes used.
    lpErrno - A pointer to the error value of the function.
Returns:
    WSCGetProviderPath() returns 0. Otherwise, it returns SOCKET_ERROR, and a specific error code is available in lpErrno.
--*/
{
    ...
    // Start the iteration through the catalog. All the real work is
    // done in ProtocolIterationProc
    Catalog->EnumerateCatalogItems(PathIterationProc, &EnumerationContext);
...
int WSPAPI WSCGetProviderPath(IN LPGUID lpProviderId, OUT WCHAR FAR * lpszProviderDllPath, IN OUT LPINT ProviderDLLPathLen, OUT LPINT lpErrno )
{
    return WPUGetProviderPath(lpProviderId, lpszProviderDllPath, ProviderDLLPathLen, lpErrno);
}
...
Arguments:
    ...
    lpProtocolBuffer - A buffer which is filled with WSAPROTOCOL_INFOA
                       structures.  See below for a detailed description of the contents of the WSAPROTOCOL_INFOA structure.

    lpdwBufferLength - On input, the count of bytes in the lpProtocolBuffer
                       buffer passed to WSAEnumProtocols().  On output, the minimum buffer size that can be passed to
                       WSAEnumProtocols() to retrieve all the requested information.  This routine has no ability to enumerate
                       over multiple calls; the passed-in buffer must be large enough to hold all entries in order for the routine to
                       succeed.  This reduces the complexity of the API and should not pose a problem because the number of protocols loaded on a machine is typically small.

Returns:
    The number of protocols to be reported on. Otherwise a value of SOCKET_ERROR is returned and a specific  stored with SetLastError().
--*/
{
    ...
    __try {
        if( *lpdwBufferLength > 0 && lpProtocolBuffer!=NULL) {
            // Since all of the structures are of fixed size (no embedded pointers to variable-sized data) we can calculate the required
            // size of the UNICODE buffer by dividing the buffer size by the size of the ANSI structure, then multiplying by the size of the UNICODE structure.
            ...
            ProtocolInfoW = new WSAPROTOCOL_INFOW[NumProtocolEntries];// Try to allocate the UNICODE buffer.
        ...
        result = WSAEnumProtocolsW(lpiProtocols, ProtocolInfoW, &ProtocolInfoWLength);// Call through to the UNICODE version.
        *lpdwBufferLength = ( ProtocolInfoWLength / sizeof(WSAPROTOCOL_INFOW) ) * sizeof(WSAPROTOCOL_INFOA);// Map the size back to ANSI.
    }
    __except (WS2_EXCEPTION_FILTER()) {
        result = SOCKET_ERROR;
        SetLastError (WSAEFAULT);
    ...
    if( result == SOCKET_ERROR ) {
        // Could not store the data, probably because the supplied buffer was too small.
    ...
    for( i = 0 ; i < result ; i++ ) {
        error = MapUnicodeProtocolInfoToAnsi(ProtocolInfoW + i, lpProtocolBuffer + i);
...
BOOL CompareMemory(LPBYTE  p1, LPBYTE  p2, DWORD   dwSize)
/*++
Routine Description:
        ...
        if (ErrorCode==ERROR_SUCCESS) {
            __try {
                if (*lpNotificationHandle == NULL) {
                    ErrorCode = WahCreateNotificationHandle(HelperHandle, lpNotificationHandle);
                } else {
                    ErrorCode = WahWaitForNotification(HelperHandle, *lpNotificationHandle, lpOverlapped, lpCompletionRoutine);
                }
            } __except (WS2_EXCEPTION_FILTER()) {
                ErrorCode = WSAEFAULT;
            }
