...
DWORD WINAPI SockAsyncThread(IN PSOCK_ASYNC_THREAD_PARAMS pThreadParams);
BOOL WINAPI SockAsyncThreadBlockingHook(VOID);
...
DWORD CopyHostentToBuffer(IN char FAR * Buffer, IN int BufferLength, IN PHOSTENT Hostent);
DWORD CopyServentToBuffer(IN char FAR *Buffer, IN int BufferLength, IN PSERVENT Servent);
DWORD CopyProtoentToBuffer(IN char FAR *Buffer, IN int BufferLength, IN PPROTOENT Protoent);
DWORD BytesInHostent(PHOSTENT Hostent);
DWORD BytesInServent(PSERVENT Servent);
DWORD BytesInProtoent(IN PPROTOENT Protoent);
...
BOOL SockAsyncGlobalInitialize()
{
...
BOOL SockCheckAndInitAsyncThread(VOID)
{
    ...
    // Check again, in case another thread has already initialized the async thread.
    if (!SockAsyncThreadInitialized) {
        ...
        TRY_START(guard_lock)
        {
            ...
            if (pThreadParams == NULL) {
                DEBUGF(DBG_ERR, ("Allocating async thread parameter block.\n"));
                TRY_THROW(guard_lock);
            }

            SockAsyncQueueHead = &pThreadParams->SockAsyncQueueHead;
            InitializeListHead(SockAsyncQueueHead);
            SockAsyncQueueEvent = pThreadParams->SockAsyncQueueEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
            if (SockAsyncQueueEvent == NULL) {
                DEBUGF(DBG_ERR, ("Creating async queue event.\n"));
                TRY_THROW(guard_lock);
            }
            ...
            if ((GetModuleFileName(gDllHandle, u.ModuleName, sizeof(u.ModuleName)) == 0) ||
                ((SockAsyncModuleHandle = LoadLibrary(u.ModuleName)) == NULL)) {
                DEBUGF(DBG_ERR, ("Referencing ws2_32.dll.\n"));
                TRY_THROW(guard_lock);
            }

            // Add an artificial reference to the startup count so that we
            // won't clean up this dll while the SockAsyncThread is still processing a request
            if (WSAStartup(0x202, &u.WSAData) != 0) {
                DEBUGF(DBG_ERR, ("Starting up ws2_32.dll.\n"));
                TRY_THROW(guard_lock);
            }
            startup_done = TRUE;

            // Create the async thread itself.
            threadHandle = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SockAsyncThread, pThreadParams, 0, &threadId);
            if (threadHandle == NULL) {
                DEBUGF(DBG_ERR, ("Creating async thread.\n"));
                TRY_THROW(guard_lock);
            }
            // Close the thread handle, indicate a successful result, and jump down to the right cleanup step
...
PWINSOCK_CONTEXT_BLOCK SockAllocateContextBlock(DWORD AdditionalSpace)
{
    ...
    contextBlock->TaskHandle = (HANDLE)InterlockedIncrement(&SockAsyncTaskHandleCounter);

    assert(contextBlock->TaskHandle != NULL);

    return contextBlock;// Return the task handle we allocated.
...
VOID SockQueueRequestToAsyncThread(IN PWINSOCK_CONTEXT_BLOCK ContextBlock)
{
    ...
    SockAcquireGlobalLock();// Acquire the lock that protects the async queue list.

    // Insert the context block at the end of the queue.
    InsertTailList(SockAsyncQueueHead, &ContextBlock->AsyncThreadQueueListEntry);

    // Set the queue event so that the async thread wakes up to service this request.
    result = SetEvent(SockAsyncQueueEvent);
    assert(result);
    
    SockReleaseGlobalLock();// Release the resource and return.
...
INT SockCancelAsyncRequest(IN HANDLE TaskHandle)
{
    ...
    // If the async thread has not been initialized, then this must be an invalid context handle.
    if (!SockAsyncThreadInitialized) {
        return WSAEINVAL;
    }
    ...
    // will cancel the request, and the handler routine will not post the message for completion of the request.
    ...
    // Attempt to find the task handle in the queue of context blocks to the async thread.
    for (entry = SockAsyncQueueHead->Flink; entry != SockAsyncQueueHead; entry = entry->Flink) {
        contextBlock = CONTAINING_RECORD(entry, WINSOCK_CONTEXT_BLOCK, AsyncThreadQueueListEntry);
...
DWORD WINAPI SockAsyncThread(IN PSOCK_ASYNC_THREAD_PARAMS    pThreadParams)
{
    ...
    // Set up our blocking hook routine. 
    // We'll use it to handle cancelling async requests.
    previousHook = WSASetBlockingHook((FARPROC)SockAsyncThreadBlockingHook);

    // Loop forever dispatching actions.
    while (TRUE) {
        // Wait for the async queue event to indicate that there is something on the queue.
        WaitForSingleObject(sockAsyncQueueEvent, INFINITE);

        SockAcquireGlobalLock();// Acquire the lock that protects the async queue.
        ...
        while (!IsListEmpty(sockAsyncQueueHead)) {
            ...
            contextBlock = CONTAINING_RECORD(listEntry, WINSOCK_CONTEXT_BLOCK, AsyncThreadQueueListEntry);
            ...
            case WS_OPCODE_GET_HOST_BY_NAME:
                SockProcessAsyncGetHost(contextBlock->TaskHandle,
                                        contextBlock->OpCode,
            ...
            case WS_OPCODE_GET_PROTO_BY_NAME:
                SockProcessAsyncGetProto(contextBlock->TaskHandle,
                                         contextBlock->OpCode,
            ...
            case WS_OPCODE_GET_SERV_BY_NAME:
                SockProcessAsyncGetServ(contextBlock->TaskHandle,
                                        contextBlock->OpCode,
            ...
            case WS_OPCODE_TERMINATE:
                ...
                FreeLibraryAndExitThread(SockAsyncModuleHandle, 0);
            ...
            // Free the context block, reacquire the list lock, and continue.
            SockFreeContextBlock(contextBlock);
            SockAcquireGlobalLock();
        }

        SockReleaseGlobalLock();// Release the list lock and redo the wait.
    }
} // SockAsyncThread
...
BOOL WINAPI SockAsyncThreadBlockingHook(VOID)
{
    // If the current async request is being cancelled, blow away the current blocking call.
...
)
{
    ...
    assert(OpCode == WS_OPCODE_GET_HOST_BY_ADDR || OpCode == WS_OPCODE_GET_HOST_BY_NAME);
    ...
    // Hold the lock that protects the async thread context block queue
    // while we do this.  This prevents a race between this thread and any thread invoking WSACancelAsyncRequest().
    SockAcquireGlobalLock();
    ...
    if (returnHost != NULL) {
        requiredBufferLength = CopyHostentToBuffer(Buffer, BufferLength, returnHost);
    ...
    SockReleaseGlobalLock();// Release the global lock.
    ...
    // Post a message to the application indication that the data it requested is available.
    assert(sizeof(TaskHandle) == sizeof(HANDLE));

    sockPostRoutine = GET_SOCK_POST_ROUTINE();

    // !!! Need a mechanism to repost if the post failed!
    if (!sockPostRoutine || !sockPostRoutine(hWnd, wMsg, (WPARAM)TaskHandle, lParam)) {
    ...
    IN int BufferLength)
{
    ...
    assert(OpCode == WS_OPCODE_GET_PROTO_BY_NAME || OpCode == WS_OPCODE_GET_PROTO_BY_NUMBER);
    ...
    if (returnProto != NULL) {
        requiredBufferLength = CopyProtoentToBuffer(Buffer, BufferLength, returnProto);
        if (requiredBufferLength > (DWORD)BufferLength) {
            error = WSAENOBUFS;
        } else {
            error = NO_ERROR;
    ...
    // Build lParam for the message we'll post to the application.
    // The high 16 bits are the error code, the low 16 bits are the minimum buffer size required for the operation.
    lParam = WSAMAKEASYNCREPLY(requiredBufferLength, error);
...
)
{
    ...
    assert(OpCode == WS_OPCODE_GET_SERV_BY_NAME || OpCode == WS_OPCODE_GET_SERV_BY_PORT);

    // Get the necessary information.
    if (OpCode == WS_OPCODE_GET_SERV_BY_NAME) {
        returnServ = getservbyname(Filter, Protocol);
    } else {
        returnServ = getservbyport((int)(LONG_PTR)Filter, Protocol);
    }
    ...
    if (returnServ != NULL) {
        requiredBufferLength = CopyServentToBuffer(Buffer, BufferLength, returnServ);
        if (requiredBufferLength > (DWORD)BufferLength) {
            error = WSAENOBUFS;
        } else {
            error = NO_ERROR;
...
DWORD CopyHostentToBuffer(char FAR *Buffer, int BufferLength, PHOSTENT Hostent)
{
    ...
    for (i = 0; i < addressCount; i++) {
        ...
        CopyMemory(currentLocation, Hostent->h_addr_list[i], Hostent->h_length);
        currentLocation = Buffer + bytesFilled;
    }
    ...
    for (i = 0; i < aliasCount; i++) {
        ...
        CopyMemory(currentLocation, Hostent->h_aliases[i], strlen(Hostent->h_aliases[i]) + 1);
...
DWORD CopyServentToBuffer(IN char FAR *Buffer, IN int BufferLength, IN PSERVENT Servent)
{
    ...
    // Count the service's aliases and set up an array to hold pointers to them.
    for (aliasCount = 0; Servent->s_aliases[aliasCount] != NULL; aliasCount++);
    ...
    for (i = 0; i < aliasCount; i++) {
        ...
        CopyMemory(currentLocation, Servent->s_aliases[i], strlen(Servent->s_aliases[i]) + 1);
        currentLocation = Buffer + bytesFilled;
    }
...
DWORD CopyProtoentToBuffer(IN char FAR *Buffer, IN int BufferLength, IN PPROTOENT Protoent)
{
    ...
    // Count the protocol's aliases and set up an array to hold pointers to them.
    for (aliasCount = 0; Protoent->p_aliases[aliasCount] != NULL; aliasCount++);
    ...
    for (i = 0; i < aliasCount; i++) {
        ...
        CopyMemory(currentLocation, Protoent->p_aliases[i], strlen(Protoent->p_aliases[i]) + 1);
        currentLocation = Buffer + bytesFilled;
    }
...
DWORD BytesInServent(IN PSERVENT Servent)
{
    DWORD total;
    int i;
