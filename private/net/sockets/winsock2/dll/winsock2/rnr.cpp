...
Abstract:
    This module contains the implementation of the Registration and Name Resolution API for the WinSock2 API

    This module contains the following functions.
    For functions whose function signature contains sting arguments both and ASCII and Wide charater version of the function are supplied
...
// BONUSSIZE is a hack that is used  to bias the computed size when WSALookupServiceNextA gets a WSAEFAULT from the WSALookupServiceNextW call.
// It is the "maximum" padding that we might need.
// Note this assumes all values returned and a limit of 3 addresses.
// There is no way to know exactly what the makeup of the returned data will be, so this is a "best guess".
// The right fix is to redo the code to pack the result optimally, so no padding is needed.

#define BONUSSIZE (3 + 3 + 3 + (3 * 3))


BOOL CalculateBufferSize(PVOID PassBack, PNSCATALOGENTRY CatalogEntry)
/*++
Routine Description:
    This fuction calculates the size of buffer required to return the NAMESPACE_INFO structs for a call to WSAEnumNameSpaces().
    This function is a callback function used as an argument to the name space catalog enumeration funtion
Arguments:
    PassBack - A context value passed thru the catalog enumeration function.
               This passback value is really a pointer to a NSCATALOG_ENUMERATION_CONTEXT.
    CatalogEntry - A pointer to the current name space catalog entry to be inspected.
Return Value:
   TRUE, Signalling the catalog enumeration function should continue the enumeration.
--*/
{
    ...
    *(Context->RequiredBufferSize) += sizeof(WSANAMESPACE_INFO);// Add the fixed length of the WSANAMESPACE_INFO struct
    *(Context->RequiredBufferSize) += sizeof(GUID);// Add room for the GUID

    // Add room for the display string
    EntryDisplayString = CatalogEntry->GetProviderDisplayString();
    *(Context->RequiredBufferSize) += ((wcslen(EntryDisplayString) + 1) * sizeof(WCHAR));
    return(TRUE); // Continue the enumeration
}


BOOL CopyFixedPortionNameSpaceInfo(PVOID PassBack, PNSCATALOGENTRY CatalogEntry)
/*++
Routine Description:
    This Funtion copies the fixed size elements of a NSCATALOGENTRY object into a user buffer for return from a call to WSAEnumNameSpaces().
    It also increments the number of fixed size elements copied so far.
Arguments:
    PassBack - A context value passed thru the catalog enumeration function.
               This passback value is really a pointer to a NSCATALOG_ENUMERATION_CONTEXT.
    CatalogEntry - A pointer to the current name space catalog entry to be inspected.
...
BOOL CopyVariablePortionNameSpaceInfo(PVOID PassBack, PNSCATALOGENTRY CatalogEntry)
/*++
Routine Description:
...
Arguments:
    PassBack - A context value passed thru the catalog enumeration
               function. This passback value is really a pointer to a NSCATALOG_ENUMERATION_CONTEXT.
    CatalogEntry - A pointer to the current name space catalog entry to be inspected.
Return Value:
  TRUE, Signalling the catalog enumeration function should continue the enumeration.
--*/
{
    ...
    __try {
        if (Context->Ansi) {
            ...
            Context->BufferFreePtr += lstrlen((LPSTR)CurrentNSInfo->lpszIdentifier) + 1;
        } //if
        else {
            memcpy(CurrentNSInfo->lpszIdentifier, DisplayString, StringLength);
...
Arguments:
    Catalog - A pointer to a NSCATALOG object containing the requested information.
    Ansi - A boolean value marking whether the user requested the ansi or unicode version of the WSANAMESPACE_INFO struct should be returned.
    BufferLength - The size of the user buffer in bytes.
    Buffer - A pointer to the user buffer.
    ErrorCode - A pointer to a DWORD to contain the error return from this function.
Return Value:
    ...
    to  reflect  the  size  of  buffer  required  to  hold  all  the  requested information.
--*/
{
    INT        ReturnCode;
    ...
    *ErrorCode = WSAEFAULT;
    ...
    Catalog->EnumerateCatalogItems(CalculateBufferSize, &Context);
    ...
    //Copy over the fixed part of the WSANAMESPACE_INFO struct(s) into the user buffer
    Catalog->EnumerateCatalogItems(CopyFixedPortionNameSpaceInfo, &Context);
    if (Context.ErrorCode == ERROR_SUCCESS) {
        //Copy over the variable part of the WSANAMESPACE_INFO struct(s) into the user buffer
        Catalog->EnumerateCatalogItems(CopyVariablePortionNameSpaceInfo, &Context);
        if (Context.ErrorCode == ERROR_SUCCESS) {
...
INT WSAAPI WSAEnumNameSpaceProvidersA(IN OUT LPDWORD lpdwBufferLength, IN OUT LPWSANAMESPACE_INFOA lpnspBuffer)
/*++
Routine Description:
    Retrieve information about available name spaces.
Arguments:
    lpdwBufferLength - on input, the number of bytes contained in the buffer pointed to by lpnspBuffer.
                       On output (if the API fails, and the error is  WSAEFAULT), the minimum number of bytes to pass for the lpnspBuffer to retrieve all the requested information.
                       The passed-in buffer must be sufficient to hold all of the name space information.
    lpnspBuffer - A buffer which is filled with WSANAMESPACE_INFO structures described below.
                  The returned structures are located consecutively at the head of the buffer.
                  Variable sized information referenced by pointers in the structures point to locations within the buffer located between the end of the fixed sized structures and the end of the buffer.
                  The number of structures filled in is the return value of WSAEnumNameSpaceProviders().
Return Value:
    WSAEnumNameSpaceProviders() returns the number of WSANAMESPACE_INFO structures copied into lpnspBuffer.
...
INT WSAAPI WSAEnumNameSpaceProvidersW(IN OUT LPDWORD              lpdwBufferLength, IN OUT LPWSANAMESPACE_INFOW lpnspBuffer)
/*++
Routine Description:
    Retrieve information about available name spaces.
Arguments:
    ...
    lpnspBuffer - A buffer which is filled with WSANAMESPACE_INFO structures
                  ...
                  of structures filled in is the return value of WSAEnumNameSpaceProviders().
Return Value:
...
/*++
Routine Description:
    WSALookupServiceBegin() is used to initiate a client query that is constrained by the information contained within a WSAQUERYSET structure.
...
Arguments:
    ...
    lphLookup - A pointer Handle to be used when calling WSALookupServiceNext in order to start retrieving the results set.
Returns:
...
--*/
{
    ...
    // The Winsock spec says that these are ignored fields, clear them so that they don't cause a problem in the Ansi to Unicode copy routines - NT bug #91655
    ...
    UniCodeBuffer = NULL;
    UniCodeBufferSize = 0;
    ReturnCode = MapAnsiQuerySetToUnicode(lpqsRestrictions, &UniCodeBufferSize, UniCodeBuffer);
    if (WSAEFAULT == ReturnCode) {
        ...
        if (UniCodeBuffer) {
            ...
            if (ERROR_SUCCESS == ReturnCode) {
                ReturnCode = WSALookupServiceBeginW(UniCodeBuffer, dwControlFlags, lphLookup);
            } //if
            else {
...
INT WSAAPI WSALookupServiceBeginW(IN  LPWSAQUERYSETW lpqsRestrictions, IN  DWORD          dwControlFlags, OUT LPHANDLE       lphLookup)
/*++
Routine Description:
    ...
    used by subsequent calls to WSALookupServiceNext() to get the actual results.
Arguments:
    lpqsRestrictions - contains the search criteria.
    dwControlFlags - controls the depth of the search.
    lphLookup - A pointer Handle to be used when calling WSALookupServiceNext in order to start retrieving the results set.
...
--*/
{
    ...
    if (IsBadWritePtr(lphLookup, sizeof(*lphLookup)) || IsBadReadPtr(lpqsRestrictions, sizeof(*lpqsRestrictions))) {
        SetLastError(WSAEFAULT);
        return SOCKET_ERROR;
    }
    ...
    if (Query) {
        Query->Initialize();
        ReturnValue = Query->LookupServiceBegin(lpqsRestrictions, dwControlFlags, Process->GetNamespaceCatalog());
...
Routine Description:
    ...
    returns WSA_E_NOMORE, indicating that all of the WSAQUERYSET have been returned.
Arguments:
    ...
    lpdwBufferLength - on input, the number of bytes contained in the buffer
                       ...
                       minimum number of bytes to pass for the lpqsResults to retrieve the record.
...
--*/
{
    ...
    //  is guaranteed to hold the data that could be held in the user's buffer.
    ...
    ReturnCode = WSALookupServiceNextW(hLookup, dwControlFlags, &UniCodeBufferLength, UniCodeBuffer);

    // if the call did not supply a buffer, the user does have a buffer,
    // and it the call failed, do it again. This should never happen, and if it does things are very odd, but account for it nonetheless.
    if (!UniCodeBuffer && (*lpdwBufferLength >= sizeof(WSAQUERYSET)) && (ReturnCode == SOCKET_ERROR)) {
        ErrorCode = GetLastError();
        if (WSAEFAULT == ErrorCode) {
            // delete old buffer, if any, and get a new buffer of the proper size.
            delete  (PBYTE)UniCodeBuffer;

            UniCodeBuffer = (LPWSAQUERYSETW) new BYTE[UniCodeBufferLength];

            // if a buffer is allocated, call the provider again. Else,
            // return the EFAULT and the buffer size to the caller to handle it.
            if (UniCodeBuffer) {
                ReturnCode = WSALookupServiceNextW(hLookup, dwControlFlags, &UniCodeBufferLength, UniCodeBuffer);
            }
        }
    ...
    // Either it worked, in which case UniCodeBuffer contains the results, or it didn't work for one of the above branches.
    if (ERROR_SUCCESS == ReturnCode) {
        ReturnCode = MapUnicodeQuerySetToAnsi(UniCodeBuffer, lpdwBufferLength, lpqsResults);
        if (ERROR_SUCCESS != ReturnCode) {
            SetLastError(ReturnCode);
            ReturnCode = SOCKET_ERROR;
        } //if
...
--*/
{
    ...
    // Verify that pointers are valid
    if (IsBadReadPtr(lpdwBufferLength, sizeof(*lpdwBufferLength)) || (*lpdwBufferLength != 0 && IsBadWritePtr(lpqsResults, *lpdwBufferLength))) {
        SetLastError(WSAEFAULT);
        return SOCKET_ERROR;
    }
    ...
    ReturnValue = Query->LookupServiceNext(dwControlFlags, lpdwBufferLength, lpqsResults);
    if (SOCKET_ERROR == ReturnValue) {
...
Routine Description:
    ...
    effect as a cancel, and will cause the WSALookupServiceNext() call to return immediately.
Arguments:
...
Routine Description:
    ...
    name space provider, all providers associated with a specific name space, or all providers across all name spaces.
Arguments:
    ...
    essOperation - an enumeration whose values include:
        ...
        FLUSH used to initiate the registration requests that have previously occurred.

    dwControlFlags - The meaning of dwControlFlags is dependent on the value of essOperation as follows:
...
--*/
{
    ...
    ErrorCode = MapAnsiQuerySetToUnicode(lpqsRegInfo, &UniCodeBufferSize, UniCodeBuffer);
    if (WSAEFAULT == ErrorCode) {
        UniCodeBuffer = (LPWSAQUERYSETW) new BYTE[UniCodeBufferSize];
        if (UniCodeBuffer) {
            ErrorCode = MapAnsiQuerySetToUnicode(lpqsRegInfo, &UniCodeBufferSize, UniCodeBuffer);
            if (ERROR_SUCCESS == ErrorCode) {
                ReturnCode = WSASetServiceW(UniCodeBuffer, essOperation, dwControlFlags);
            } //if
            delete UniCodeBuffer;
        } //if
    } //if
...
public:
    ...
    INT Initialize(PNSCATALOGENTRY  CatalogEntry);
    PNSPROVIDER GetProvider(IN  PNSCATALOG    Catalog);
    ~NSCATALOGENTRYSTATE();
...
inline NSCATALOGENTRYSTATE::NSCATALOGENTRYSTATE()
/*++
Routine Description:
    Constructor for the NSCATALOGENTRYSTATE object.  The first member function called after this must be Initialize.
Arguments:
    None
Return Value:
...
inline INT NSCATALOGENTRYSTATE::Initialize(PNSCATALOGENTRY  CatalogEntry)
/*++
Routine Description:
    This  procedure  performs  all initialization for the NSCATALOGENTRYSTATE
    object.  This function  must  be  invoked  after the constructor, before any other member function is invoked.
Arguments:
    CatalogEntry - A pointer to a namespace catalog entry object.
Return Value:
    If  the  function  is  successful,  it  returns ERROR_SUCCESS, otherwise it returns an appropriate WinSock 2 error code.
--*/
{
...
PNSPROVIDER NSCATALOGENTRYSTATE::GetProvider(IN PNSCATALOG Catalog)
/*++
Routine Description:
...
BOOL MatchProviders(IN PVOID                PassBack, IN PNSCATALOGENTRY      CatalogEntry)
/*++
Routine Description:
    ...
    item to see if it meets the selection criteria contained in the context value passed back from EnumerateCatalogItems().
Arguments:
...
--*/
{
    ...
    if (Context->UseGuid) {
        ...
        if (CatalogEntry->GetEnabledState() && ((Context->NameSpaceId == CatalogEntry->GetNamespaceId()) || Context->NameSpaceId == NS_ALL)) {
            UseThisProvider = TRUE;
        }
    }
    ...
    if (UseThisProvider) {
        ...
        if (EntryState != NULL) {
            ...
            InsertTailList(&Context->EntryList, &EntryState->m_context_linkage);
        } else {
...
--*/
{
    ...
    Catalog->EnumerateCatalogItems(MatchProviders, &Context);
    ...
    if (Context.ErrorCode == ERROR_SUCCESS) {
        ...
        while (!IsListEmpty(&Context.EntryList)) {
            ...
            EntryState = CONTAINING_RECORD(ListItem, NSCATALOGENTRYSTATE, m_context_linkage);
...
Routine Description:
    ...
    space specific information that is common to all instances of the service, such as the SAP ID or object ID.
Arguments:
    lpServiceClasslnfo - contains service class to name space specific type
                         mapping information.  Multiple mappings can be handled at one time.
Returns:
    The return value is 0 if the operation was successful.  Otherwise the value SOCKET_ERROR is returned.
--*/
{
    ...
    ErrorCode = MapAnsiServiceClassInfoToUnicode(lpServiceClassInfo, &WideServiceClassInfoSize, WideServiceClassInfo);
    if (WSAEFAULT == ErrorCode) {
        WideServiceClassInfo = (LPWSASERVICECLASSINFOW)new BYTE[WideServiceClassInfoSize];
        if (WideServiceClassInfo) {
            ErrorCode = MapAnsiServiceClassInfoToUnicode(lpServiceClassInfo, &WideServiceClassInfoSize, WideServiceClassInfo);
            if (ERROR_SUCCESS == ErrorCode) {
                ReturnCode = WSAInstallServiceClassW(WideServiceClassInfo);
            } //if
...
--*/
{
    ...
    if (Context.ErrorCode == ERROR_SUCCESS) {
        ...
        while (!IsListEmpty(&Context.EntryList)) {
            ...
            Provider = EntryState->GetProvider(Catalog);
            if (Provider != NULL) {
                INT Error = Provider->NSPInstallServiceClass(lpServiceClassInfo);
                if (!Error) {
                    ErrorCode = ERROR_SUCCESS;
                } else {
                    if (ErrorCode) {
...
Routine Description:
    ...
    schema.
Arguments:
    lpServiceClassId - Pointer to the service class GUID that you wish to remove.
Returns:
    The return value is 0 if the operation was successful.  Otherwise the value SOCKET_ERROR is returned.
--*/
{
...
Arguments:
    ...
    lpdwBufferLength - on input length of buffer returned by
                       lpszServiceClassName. On output, the length of the service name copied into lpszServiceClassName.
Returns:
    The return value is 0 if the operation was successful.  Otherwise the value SOCKET_ERROR is returned.
--*/
{
    ...
    if (Context.ErrorCode == ERROR_SUCCESS) {
        ...
        while (!IsListEmpty(&Context.EntryList)) {
            ...
            if (Provider != NULL) {
                ...
                ErrorCode = Provider->NSPGetServiceClassInfo(&BufferSize, &Buffer);
                if (ErrorCode == ERROR_SUCCESS) {
                    // this is impossible. The provider has made an error, so concoct an error for it.
                ...
                if (WSAEFAULT == ErrorCode) {
                    ...
                    // buffer was to small big suprise :-() so get a new buffer and go get the answer.
                    pBuffer = (LPWSASERVICECLASSINFOW) new BYTE[BufferSize];
                    if (pBuffer != NULL) {
                        pBuffer->lpServiceClassId = lpServiceClassId;
                        ErrorCode = Provider->NSPGetServiceClassInfo(&BufferSize, pBuffer);
                        if (ErrorCode == ERROR_SUCCESS && pBuffer->lpszServiceClassName) {
                            DWORD StringLen = ((wcslen(pBuffer->lpszServiceClassName) + 1) * sizeof(WCHAR));
...
Routine Description:
    This API will return the name of the service associated with the given
    type.  This name is the generic service name, like FTP, or SNA, and not the name of a specific instance of that service.
Arguments:
    lpServiceClassId - pointer to the GUID for the service class.
    lpszServiceClassName - service name.
    lpdwBufferLength - on input length of buffer returned by
                ...
                if (WSAEFAULT == ErrorCode) {
                    ...
                    if (pBuffer != NULL) {
                        ...
                        if (ErrorCode == ERROR_SUCCESS) {
                            ...
                            __try {
                                if (*lpdwBufferLength >= StringLen) {
                                    wcscpy(lpszServiceClassName, pBuffer->lpszServiceClassName);
                                } //if
                                else {
                                    ErrorCode = WSAEFAULT;
                                } //else
...
Routine Description:
    ...
    (schema) pertaining to a specified service class from a specified name space provider.
Arguments:
    ...
    lpdwBufferLength - on input, the number of bytes contained in the buffer
                       ...
                       the minimum number of bytes to pass for the lpServiceClassInfo to retrieve the record.
...
--*/
{
    ...
    if (WideServiceClassInfo) {
        ReturnCode = WSAGetServiceClassInfoW(lpProviderId, lpServiceClassId, lpdwBufSize, WideServiceClassInfo);
        if (ERROR_SUCCESS == ReturnCode) {
            MapUnicodeServiceClassInfoToAnsi(WideServiceClassInfo, lpdwBufSize, lpServiceClassInfo);
        } //if
        else {
            ErrorCode = GetLastError();
        } //else
...
--*/
{
    ...
    Catalog = Process->GetNamespaceCatalog();
    ErrorCode = Catalog->GetCountedCatalogItemFromProviderId(lpProviderId, &CatalogEntry);
    if (ERROR_SUCCESS == ErrorCode) {
        if (CatalogEntry->GetEnabledState()) {
            ...
            if (Provider->NSPGetServiceClassInfo(lpdwBufSize, lpServiceClassInfo) != ERROR_SUCCESS) {
                ErrorCode = GetLastError();
                if (ErrorCode == ERROR_SUCCESS)
                    ErrorCode = WSANO_DATA;
            }
...
Routine Description:
    ...
    particular provider, it should supply the corresponding WSAPROTOCOL_INFO struct in the lpProtocolInfo parameter.
Arguments:
    lpsaAddress - points to a SOCKADDR structure to translate into a string.
    dwAddressLength - the length of the Address SOCKADDR.
    lpProtocolInfo - (optional) the WSAPROTOCOL_INFO struct for a particular provider.
    ...
    lpdwAddressStringLength - on input, the length of the AddressString buffer.
                              On output, returns the length of  the string actually copied into the buffer.
Return Value:
    The return value is 0 if the operation was successful.  Otherwise the value SOCKET_ERROR is returned
--*/
{
    ...
    if (lpProtocolInfo) {
        ...
        ErrorCode = Catalog->GetCountedCatalogItemFromCatalogEntryId(dwCatalogEntryId, &CatalogEntry);
    } //if
    else {
        int family;
        __try {
        ...
        ErrorCode = Catalog->GetCountedCatalogItemFromAddressFamily(family, &CatalogEntry);
    }
...
Routine Description:
    ...
    a particular provider, it should supply the corresponding WSAPROTOCOL_INFOW struct in the lpProtocolInfo parameter.
Arguments:
    ...
    lpProtocolInfo - (optional) the WSAPROTOCOL_INFOW struct for a particular provider.
...
--*/
{
    ...
    if (lpProtocolInfo) {
        ...
        ErrorCode = Catalog->GetCountedCatalogItemFromAddressFamily(AddressFamily, &CatalogEntry);
    }
    ...
    if (ERROR_SUCCESS == ErrorCode) {
        ...
        assert(ProtocolInfo != NULL);
        Provider = CatalogEntry->GetProvider();
        ReturnValue = Provider->WSPStringToAddress(AddressString, AddressFamily, ProtocolInfo, lpAddress, lpAddressLength, &ErrorCode);
        CatalogEntry->Dereference();
        if (ReturnValue == ERROR_SUCCESS)
            return ERROR_SUCCESS;
    } //if
...
Routine Description:
    ...
    structure (SOCKADDR) suitable for pass to Windows Sockets routines which
    take such a structure.  If the caller wishes the translation to be done by
    a particular provider, it should supply the corresponding WSAPROTOCOL_INFOA struct in the lpProtocolInfo parameter.
Arguments:
    AddressString - points to the zero-terminated human-readable string to convert.
    AddressFamily - the address family to which the string belongs.
    lpProtocolInfo - (optional) the WSAPROTOCOL_INFOA struct for a particular provider.
    Address - a buffer which is filled with a single SOCKADDR structure.
    lpAddressLength - The length of the Address buffer.  Returns the size of the resultant SOCKADDR structure.
Return Value:
    The return value is 0 if the operation was successful.  Otherwise the value SOCKET_ERROR is returned.
--*/
{
    ...
    if (ERROR_SUCCESS == ErrorCode) {
        ...
        ReturnValue = Provider->WSPStringToAddress(LocalString, AddressFamily, ProtocolInfo, lpAddress, lpAddressLength, &ErrorCode);
        CatalogEntry->Dereference();
        delete(LocalString);
        if (ReturnValue == ERROR_SUCCESS)
            return ERROR_SUCCESS;
...
PNSCATALOG OpenInitializedNameSpaceCatalog()
{
    ...
    TRY_START(mem_guard)
    {
        ...
        ReturnCode = ns_catalog->InitializeFromRegistry(RegistryKey, NULL);
        if (ERROR_SUCCESS != ReturnCode) {
