...
Revision History:
    ...
    21-Aug-1995 dirk@mink.intel.com
        Clean up from code review. Moved single line functions to inlines in header file. Added debug traceing code. Rewrote destructor. Removed ProviderID from the class.
...
inline INT DPROVIDER::WSPCleanup(OUT INT FAR *lpErrno)
/*++
Routine Description:
...
--*/
{
    ...
    if (m_library_handle) {
        ...
        lpWSPCleanup = (LPWSPCLEANUP)InterlockedExchangePointer ((PVOID *)&m_proctable.lpWSPCleanup, NULL);
        if (lpWSPCleanup!=NULL) {
            DEBUGF( DBG_TRACE, ("Calling WSPCleanup for provider %X\n", this));
            if (!PREAPINOTIFY(( DTCODE_WSPCleanup, &ReturnValue, m_lib_name, &lpErrno)) ) {
                ReturnValue = lpWSPCleanup(lpErrno);
                POSTAPINOTIFY(( DTCODE_WSPCleanup, &ReturnValue, m_lib_name, &lpErrno));
            }
        }
    }
...
--*/
{
    if (m_library_handle)
    {
        ...
        WSPCleanup (&ErrorCode);// Cleanup service provider DLL if not already done so
        FreeLibrary(m_library_handle);// Free the service provider DLL
    ...
    DEBUGF( DBG_TRACE, ("Destroying provider %X\n", this));
}


INT DPROVIDER::Initialize(IN LPSTR lpszLibFile, IN LPWSAPROTOCOL_INFOW lpProtocolInfo)
/*++
Routine Description:
    Initializes the DPROVIDER object.

Arguments:
    lpszLibFile - A  Null  terminating  string  that  points to the .DLL of the service to load.
    lpProtocolInfo - A pointer to a WSAPROTOCOL_INFOW struct to hand to the provider startup routine.
Return Value:
    If no error occurs, Initialize() returns ERROR_SUCEES.
...
--*/
{
    ...
    DEBUGF( DBG_TRACE, ("Initializing provider %X\n", this));
    ...
    TRY_START(guard_memalloc) {
        ...
        // Fill  the  upcall  table  we  will hand to the service provider with the address  of  our  upcall entry points.
        // We will also pre-fill this thing with  zeros,  mainly  to  help  with  consistency  checks  if we add new functions to the upcall table in the future.
        ZeroMemory(
        ...
        if (!( ExpandEnvironmentStrings(lpszLibFile, LibraryPath, MAX_PATH))){
            DEBUGF(DBG_ERR, ("Expansion of environment variables failed\n"));
        ...
        if (m_lib_name == NULL) {
            DEBUGF(DBG_ERR, ("Allocation of m_lib_name failed\n"));
        ...
        // First load the DLL for the service provider. Then get two functions that init the service provider structures.
        m_library_handle = LoadLibrary(LibraryPath);
        if(!m_library_handle){
            DEBUGF(DBG_ERR, ("Failed to load DLL %s\n",LibraryPath));
            error_code = WSAEPROVIDERFAILEDINIT;
            TRY_THROW(guard_memalloc);
        }
        WSPStartupFunc = (LPWSPSTARTUP)GetProcAddress(m_library_handle, "WSPStartup");
        if(!(WSPStartupFunc)){
        ...
        // Set exception handler around this call since we hold critical section (catalog lock).
        __try {
#if !defined(DEBUG_TRACING)
            error_code = (*WSPStartupFunc)(wVersionRequested, & WSPData, lpProtocolInfo, UpCallTable, &m_proctable);
#else
            { // declaration block
                LPWSPDATA  pWSPData = & WSPData;
                BOOL       bypassing_call;

                bypassing_call = PREAPINOTIFY((DTCODE_WSPStartup, & error_code, LibraryPath, & wVersionRequested, & pWSPData, & lpProtocolInfo, & UpCallTable, & m_proctable));
                if (! bypassing_call) {
                    error_code = (*WSPStartupFunc)(wVersionRequested, & WSPData, lpProtocolInfo, UpCallTable, &m_proctable);
                    POSTAPINOTIFY((DTCODE_WSPStartup, & error_code, LibraryPath, & wVersionRequested, & pWSPData, & lpProtocolInfo, & UpCallTable, & m_proctable));
                }
            } // declaration block
#endif // !defined(DEBUG_TRACING)
        ...
        if( !m_proctable.lpWSPAccept              ||
            ...
            DEBUGF(DBG_ERR, ("Service provider %s returned an invalid procedure table\n", lpszLibFile));
            error_code = WSAEINVALIDPROCTABLE;
            TRY_THROW(guard_memalloc);
        }

        // Confirm that the WinSock service provider supports 2.2.
        // If it supports a version greater then 2.2 it will still return 2.2 since this is the version  we requested.
        if( WSPData.wVersion != WINSOCK_HIGH_SPI_VERSION ) {
            WSPCleanup (&error_code);
            DEBUGF( DBG_ERR, ("Service Provider %s does not support version 2.2\n", lpszLibFile));
