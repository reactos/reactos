...
INT CheckForHookersOrChainers();

static CRITICAL_SECTION  Startup_Synchro;
...
/*++
Routine Description:
    This procedure creates the Startup/Cleanup synchronization mechanism.
    This must  be  called  once  before  the  WSAStartup  procedure  may  be called.
    Typically, this is called from the DLL_PROCESS_ATTACH branch of DllMain, as the  only  reliable  way to guarantee that it gets called before any thread calls WSAStartup.
--*/
{
    DEBUGF(DBG_TRACE, ("Initializing Startup/Cleanup critical section\n"));
    InitializeCriticalSection(& Startup_Synchro);
}  // CreateStartupSynchronization


VOID DestroyStartupSynchronization()
/*++
Routine Description:
    ...
    Typically, this is called from the DLL_PROCESS_DETACH branch of DllMain, as the  only  reliable  way  to guarantee that it gets called after any thread calls WSACleanup.
--*/
{
    DEBUGF(DBG_TRACE, ("Deleting Startup/Cleanup critical section\n"));
    DeleteCriticalSection(& Startup_Synchro);
}  // DestroyStartupSynchronization


int WSAAPI WSAStartup(IN WORD wVersionRequired, OUT LPWSADATA lpWSAData)
/*++
Routine Description:
    Winsock  DLL initialization routine.  A Process must successfully call this routine before calling any other winsock API function.
Arguments:
    wVersionRequested - The  highest version of WinSock support that the caller can  use.
...
--*/
{
    ...
    __try {
        ...
        (void) lstrcpy(lpWSAData->szDescription, "WinSock 2.0");
    #if defined(TRACING) && defined(BUILD_TAG_STRING)
        (void) lstrcat(lpWSAData->szDescription, " Alpha BUILD_TAG=");
        (void) lstrcat(lpWSAData->szDescription, BUILD_TAG_STRING);
    #endif  // TRACING && BUILD_TAG_STRING

        //TODO: Think up a good value for "system status"
        (void) lstrcpy(lpWSAData->szSystemStatus, "Running");

        // The following line is commented-out due to annoying and totally nasty alignment problems in WINSOCK[2].H. The exact location of
        // the lpVendorInfo field of the WSAData structure is dependent on the structure alignment used when compiling the source. Since we
        // cannot change the structure alignment of existing apps, the best way to handle this mess is to just not set this value. This turns
        // out to not be too bad a solution, as neither the WinNT nor the Win95 WinSock implementations set this value, and nobody appears to pay
    ...
    if (ReturnCode==ERROR_SUCCESS) {
        ...
        if (CheckForHookersOrChainers() == ERROR_SUCCESS) {
            ...
            LeaveCriticalSection(& Startup_Synchro);
        }
        else {
            ReturnCode = WSASYSNOTREADY;
        }
...
--*/
{
    ...
    ErrorCode = PROLOG(&CurrentProcess, &CurrentThread);
...
{
    EnterCriticalSection(& Startup_Synchro);
    SockPostRoutine = (LPFN_POSTMESSAGE)PostRoutine;// Save the routine locally.
    LeaveCriticalSection(& Startup_Synchro);
...
int WSAAPI DTHOOK_WSAConnect (SOCKET s,const struct sockaddr FAR *name,int namelen,LPWSABUF lpCallerData,LPWSABUF lpCalleeData,LPQOS lpSQOS,LPQOS lpGQOS);
...
SOCKET WSAAPI DTHOOK_WSAJoinLeaf (SOCKET s,const struct sockaddr FAR * name,int namelen,LPWSABUF lpCallerData,LPWSABUF lpCalleeData,LPQOS lpSQOS,LPQOS lpGQOS,DWORD dwFlags);
...
struct protoent FAR * WSAAPI DTHOOK_getprotobyname(const char FAR * name);
struct protoent FAR * WSAAPI DTHOOK_getprotobynumber(int number);
struct servent FAR * WSAAPI DTHOOK_getservbyname(const char FAR * name,const char FAR * proto);
struct servent FAR * WSAAPI DTHOOK_getservbyport(int port,const char FAR * proto);
...
HANDLE WSAAPI DTHOOK_WSAAsyncGetProtoByName(HWND hWnd,u_int wMsg,const char FAR * name,char FAR * buf,int buflen);
HANDLE WSAAPI DTHOOK_WSAAsyncGetProtoByNumber(HWND hWnd,u_int wMsg,int number,char FAR * buf,int buflen);
HANDLE WSAAPI DTHOOK_WSAAsyncGetServByName(HWND hWnd,u_int wMsg,const char FAR * name,const char FAR * proto,char FAR * buf,int buflen);
HANDLE WSAAPI DTHOOK_WSAAsyncGetServByPort(HWND hWnd,u_int wMsg,int port,const char FAR * proto,char FAR * buf,int buflen);
int WSAAPI DTHOOK_WSACancelAsyncRequest(HANDLE hAsyncTaskHandle);
...
int WSPAPI DTHOOK_WSCDeinstallProvider(LPGUID lpProviderId,LPINT lpErrno);
int WSPAPI DTHOOK_WSCInstallProvider(LPGUID lpProviderId, const WCHAR FAR * lpszProviderDllPath, const LPWSAPROTOCOL_INFOW lpProtocolInfoList, DWORD dwNumberOfEntries, LPINT lpErrno);
int WSPAPI
...
int WSPAPI WSCWriteNameSpaceOrder (IN LPGUID lpProviderId,IN DWORD dwNumberOfEntries);
...
--*/
{
    ...
    {
        if (!(pfnXxx = (LPVOID) GetProcAddress (gDllHandle, aszFuncNames[i])) || pfnXxx != apfns[i])
        {
