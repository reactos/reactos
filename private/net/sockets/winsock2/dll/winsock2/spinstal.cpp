...
Abstract:
    This module contains the entry points for service provider installation and deinstallation.
...
BOOL GuidMatcher(IN PVOID PassBack, IN PPROTO_CATALOG_ITEM  CatalogEntry)
/*++
Routine Description:
    This  procedure determines if ProviderId of a passed CatalogEntry matches a  target  provider GUID.
    If so, it sets a flag to indicate that the match was  found  and  returns  FALSE to terminate the enumeration.
...
Arguments:
    ...
    CatalogEntry - Supplies  to  the client a reference to a PROTO_CATALOG_ITEM structure with values for this item of the enumeration.
Return Value:
    If a match is found, the function returns FALSE to terminate the iteration, otherwise it returns TRUE to continue the iteration.
--*/
{
...
/*++
Routine Description:
    This   procedure   installs   the   specified   provider  into  the  system configuration  database.
    After  this  call,  a  WinSock  2  DLL  instance initialized  via  a  first call to WSAStartup will return the new protocols from the WSAEnumProtocols function.

    It  is the caller's responsibility to perform required file installation or service provider specific configuration.
Arguments:
    lpProviderId        - Supplies a GUID giving the locally unique identifier for the newly installed provider.
    lpszProviderDllPath - Supplies a reference to a fully qualified path to the providers  DLL image.
                          This path may contain embedded environment  strings  (such  as  %SystemRoot%).
                          Such environment strings are expanded whenever the WinSock 2  DLL  needs  to  load  the provider DLL.
                          After any embedded   environment   strings  are  expanded,  the WinSock  2  DLL  passes the resulting string into the LoadLibrary() API to load the provider into memory.
    lpProtocolInfoList  - Supplies a reference to an array of WSAPROTOCOL_INFOW structures.
                          Each     structure     defines     a protocol/address_family/socket_type  supported by the provider.
    dwNumberOfEntries   - Supplies    the    number    of    entries   in   the lpProtocolInfoList array.
    lpErrno             - Returns the error code.
Return Value:
    If no error occurs, WSCInstallProvider() returns ERROR_SUCCESS.
    Otherwise, it  returns  SOCKET_ERROR, and a specific error code is returned in the int referenced by lpErrno.
...
--*/
{
    ...
    if (registry_root == NULL) {
        DEBUGF(DBG_ERR, ("Opening registry root\n"));
        * lpErrno = WSANO_RECOVERY;
        return SOCKET_ERROR;
    }

    // Check the current protocol catalog key. If it doesn't match the expected value, blow away the old key and update the stored value.

    ValidateCurrentCatalogName(registry_root, WINSOCK_CURRENT_PROTOCOL_CATALOG_NAME, DCATALOG::GetCurrentCatalogName());
...
Routine Description:
    ...
    If one is found, it clears a flag to indicate that NonIFS handle support should not be removed and returns FALSE to terminate the enumeration.
    Otherwise it returns TRUE and the enumeration continues.
Arguments:
    PassBack     - Supplies a pointer to bool that servers as the flag to tell if NonIFS provider is found
    CatalogEntry - Supplies  to  the client a reference to a PROTO_CATALOG_ITEM structure with values for this item of the enumeration.
                   The pointer  is  not guaranteed to be valid after this procedure returns, so the client should copy data if required.
Return Value:
    If a match is found, the function returns FALSE to terminate the iteration, otherwise it returns TRUE.
--*/
{
    PBOOL  context;
...
/*++
Routine Description:
    This procedure removes the specified provider from the system configuration database.
    After  this  call,  a  WinSock 2 DLL instance initialized via a first  call  to  WSAStartup  will no longer return the specified provider's protocols from the WSAEnumProtocols function.

    Any  additional  file  removal  or  service provider specific configuration information  removal  needed  to completely de-install the service provider must be performed by the caller.
Arguments:
    lpProviderId - Supplies  the  locally  unique identifier of the provider to deinstall.   This  must  be  a  value previously passed to WSCInstallProvider().
    lpErrno      - Returns the error code.
Return Value:
    ...
    TRY_START(guard_memalloc) {
        ...
        do {
            ...
            if (context.CatalogItem!=NULL) {
                ...
                if (!DeinstallNonIFS) {
                    DeinstallNonIFS = !(context.CatalogItem->GetProtocolInfo()->dwServiceFlags1 & XP1_IFS_HANDLES);
                }
...
Arguments:
    PassBack     - Supplies  a reference to a PROVIDER_SNAP_CONTEXT structure.
                   Returns an array of items in the order specified by catalog id array.
    CatalogEntry - Supplies  to  the client a reference to a PROTO_CATALOG_ITEM structure with values for this item of the enumeration.  The
                   pointer  is  not guaranteed to be valid after this procedure returns, so the client should copy data if required.
Return Value:
    If an item is found that could not be matched to catalog id in the array, the function returns FALSE to terminate the iteration, otherwise it returns TRUE.
--*/
{
...
/*++
Routine Description:
    Reorder existing WinSock2 service providers.
    The order of the service providers determines their priority in being selected for use.
    The sporder.exe tool will show you the installed provider and their ordering, Alternately, WSCEnumProtocols(), in conjunction with this function, will allow you to write your own tool.
Arguments:
    lpwdCatalogEntryId  [in]
      An array of CatalogEntryId elements as found in the WSAPROTOCOL_INFO
      structure.  The order of the CatalogEntryId elements is the new priority ordering for the service providers.
...
Return Value:
    ...
    WSAEFAULT       - CatalogEnryId array is not fully contained within process address space.
...
Comments:
    Here are scenarios in which the WSCWriteProviderOrder function may fail:
      ...
      The dwNumberOfEntries is not equal to the number of registered service providers.

      The lpwdCatalogEntryId contains an invalid catalog ID.

      The lpwdCatalogEntryId does not contain all valid catalog IDs exactly 1 time.

      The routine is not able to access the registry for some reason (e.g. inadequate user persmissions)
...
--*/
{
    ...
    if (items==NULL) {
        DEBUGF(DBG_ERR, ("Allocating items array\n"));
    ...
    TRY_START(guard_memalloc) {
        ...
        for (i=0; i<dwNumberOfEntries; i++) {
            if (context.Items[i]!=NULL) {
                ...
                DEBUGF (DBG_ERR, ("Checking item array against catalog, item: %ld(CatId:%ld).\n", i,lpwdCatalogEntryId[i]));
