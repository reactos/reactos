...
// Private contants.
#define DATABASE_PATH_REGISTRY_KEY             "System\\CurrentControlSet\\Services\\Tcpip\\Parameters"
#define DATABASE_PATH_REGISTRY_VALUE "DataBasePath"
#define PROTOCOL_DATABASE_FILENAME "protocol"


// Private prototypes.
FILE * GetProtoOpenNetworkDatabase(CHAR * Name);
CHAR * GetProtoPatternMatch(CHAR * Scan, CHAR * Match);
PPROTOENT GetProtoGetNextEnt(FILE * DbFile, PGETPROTO_INFO ProtoInfo);
...
Returns:
    If  no  error  occurs, getprotobynumber() returns a pointer to the protoent structure  described  above.
    Otherwise  it  returns  a NULL pointer and a specific error r code is stored with SetErrorCode().
--*/
{
    PDPROCESS Process;
    PDTHREAD Thread;
    ...
    // Scan it.
    while( TRUE ) {
        pent = GetProtoGetNextEnt(dbFile, protoInfo);
        if( pent == NULL ) {
            break;
        }
...
Returns:
    If  no  error  occurs,  getprotobyname()  returns a pointer to the protoent structure  described  above.
...
Arguments:
    Name - The name of the database to open (i.e. "services" or "protocol").
Return Value:
...
--*/
{
    ...
    if( version.dwPlatformId == VER_PLATFORM_WIN32_NT ) {
        // We're running under NT, so try to get the path from the registry.
        err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, DATABASE_PATH_REGISTRY_KEY, 0, KEY_READ, &key);
        if( err == NO_ERROR ) {
            length = sizeof(unexpanded);
            err = RegQueryValueEx(key, DATABASE_PATH_REGISTRY_VALUE, NULL, &type, (LPBYTE)unexpanded, &length);
            RegCloseKey( key );
        }

        if( err == NO_ERROR ) {
            if( ExpandEnvironmentStrings(unexpanded, path, sizeof(path)) == 0 ) {
    ...
    // Ensure the path has a trailing backslash, then tack on any suffix needed, then tack on the filename.
    ...
    return fopen( path, "rt" );// Open the file, return the result.
...
Return Value:
    CHAR * - Pointer to the first occurrance in Scan if successful, NULL if not.
--*/
{
    CHAR ch;
...
PPROTOENT GetProtoGetNextEnt(FILE * DbFile, PGETPROTO_INFO ProtoInfo)
{
    ...
    while( TRUE ) {
        ...
        ptr = fgets(ProtoInfo->TextLine, MAX_PROTO_TEXT_LINE, DbFile);
        if( ptr == NULL ) {
            break;
        }
