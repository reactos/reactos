...
--*/
{
    ...
    TRY_START(guard_open) {
        ...
        if (key_disposition == REG_CREATED_NEW_KEY) {
            ...
            DEBUGF(DBG_TRACE, ("Creating empty ns catalog in registry\n"));
...
INT NSCATALOG::LoadProvider(IN PNSCATALOGENTRY CatalogEntry)
/*
Routine Description:
    Load   the   provider  described  by  CatalogEntry and set it into catalog entry
Arguments:
    CatalogEntry - Supplies  a reference to a name sapce catalog entry describing the provider to load.
Return Value:
