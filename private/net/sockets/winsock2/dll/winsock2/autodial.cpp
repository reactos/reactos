...
/*++
Routine Description:
    Attempt an Autodial connection given the parameters to an unsuccessful call to connect().
Arguments:
...
--*/
{
    ...
    // TRUE if a new connection was made, and FALSE otherwise.
...
/*++
Routine Description:
    Attempt an autodial connection given the parameters to an unsuccessful to WSALookupServiceNext().
Arguments:
    lpqsRestrictions: a pointer to the WSAQUERYSETW structure used in the call to WSALookupServiceBegin().
Return Value:
    TRUE if the connection was made successfully,
    FALSE otherwise.
--*/
{
    ...
    // It will return TRUE if a new connection was made, and FALSE otherwise.
...
/*++
Routine Description:
    Give Autodial information about successful name resolutions via gethostbyname().
    This is useful for resolvers like DNS where it is impossible to get complete alias information about aliases via a low-level address.

    Ultimately, this should be called from WSLookupServiceNext(), but it's impossible at that level to determine what format the results are in.
Arguments:
...
--*/
{
    ...
    (*lpfnWSNoteSuccessfulHostentLookupG)(name, ipaddr);// Call the Autodial DLL.
