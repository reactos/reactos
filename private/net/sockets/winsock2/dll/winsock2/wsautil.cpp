...
Abstract:
    This  module  contains utility functions for the winsock DLL implementation that did not seem to fit into the other module.
...
// Global pointer to the appropriate prolog function. This either points to Prolog_v1 for WinSock 1.1 apps or Prolog_v2 for WinSock 2.x apps.
...
Routine Description:
     ...
     This function ensures that the process has called WSAStartup.
Arguments:
    Process   - Pointer to the DPROCESS object for the process calling the winsock API.
    Thread    - Pointer to the DTHREAD object for the calling thread
Returns:
    This function returns ERROR_SUCCESS if successful, otherwise the specific WinSock error code
--*/
{
    INT ErrorCode;
...
Routine Description:
     This routine is the standard WinSock 1.1 prolog function used at all the winsock API entrypoints.
     This function ensures that the process has called WSAStartup and that the current thread in the process does not have a WinSock call outstanding.
Arguments:
    Process   - Pointer to the DPROCESS object for the process calling the winsock API.
    Thread    - Pointer to the DTHREAD object for the calling thread
Returns:
...
Routine Description:
    ...
    In theory, this should be totally unnecessary, but at least one popular DLL (MFC 4.x) calls WSACleanup() in its process detach handler, which may occur *after* our DLL is already detached. Grr...
Arguments:
    Process - Unused.
    Thread - Unused.
Returns:
...
Arguments:
    EntryKey  - Supplies  the open entry key under which the new named value is to be written.
    EntryName - Supplies the name of the value to be written.
    Data      - Supplies  a  reference  to the location where the entry data is found,  or to a WSABUF describing the data location in the case of REG_BINARY data.
    TypeFlag  - Supplies  an identifier for the type of the data to be written.
                Supported   types  are  REG_BINARY,  REG_DWORD,  REG_EXPAND_SZ, REG_SZ.
                Types   not   supported   are  REG_DWORD_BIG_ENDIAN, REG_DWORD_LITTLE_ENDIAN,   REG_LINK,   REG_MULTI_SZ,  REG_NONE, REG_RESOURCE_LIST.
                Note  that  depending on the architecture, one   of   the   "big_endian"   or   "little_endian"  forms  of REG_DWORD_x_ENDIAN  is implicitly allowed, since it is equal to REG_DWORD.
Return Value:
...
Routine Description:
    ...
    The value  may  be any type whose length can be determined from its type (e.g., scalar  types,  zero-terminated  strings).
    The function checks the type of the newly read value to make sure it matches the expected type.

Arguments:

    EntryKey  - Supplies  the  open entry key from which the new named value is to be read.
    EntryName - Supplies the name of the value to be read.
    Data      - Supplies  a  reference  to the location where the entry data is placed.
                Returns  the registry entry value.
                In the case where the  TypeFlag  is  REG_BINARY,  this is a reference to a WSABUF describing the target data buffer.
                The "len" field returns the length read (or required) from the registry.
...
Implementation note:
    There was no need identified for the REG_MULTI_SZ case, so support for this case  was  omitted  since  it was more difficult to derive the data length.
    There  is  no  reason  in  principle why this case cannot be added if it is really needed.

    The  validity  checks  in this routine have been written as a linear series instead  of  in the "conditional-tunnelling" nested-if form.
    The series of tests  is  long  enough that the nested-if form is far too complex to read.
    This  procedure  should  not  be sensitive to execution speed, so the extra tests and branches in the linear series form should not be a problem.
--*/
{
    ...
    if (ok_so_far) {
        ...
        if (result != ERROR_SUCCESS) {
            ...
            if (result == ERROR_MORE_DATA) {
                DEBUGF(DBG_WARN, ("Data buffer too small\n"));
            } // if ERROR_MORE_DATA
            ok_so_far = FALSE;
        } // if result != ERROR_SUCCESS
    } // if ok_so_far
    ...
    if (ok_so_far) {
        if (type_read != TypeFlag) {
            DEBUGF(DBG_ERR, ("Type read (%lu) different from expected (%lu)\n", type_read, TypeFlag));
            ok_so_far = FALSE;
        } // if type_read != TypeFlag
    } // if ok_so_far
...
/*
Routine Description:
    The RegDeleteKeyRecursive function deletes the specified key and all of its subkeys, recursively.
Arguments:
...
--*/
{
    ...
    DEBUGF(DBG_TRACE, ("RegDeleteSubkeys (%lu)\n", (ULONG_PTR)hkey));
...
/*
Routine Description:
    This  procedure opens the root of the WinSock2 portion of the registry.
    It takes  care  of  creating  and initializing the root if necessary.
    It also takes  care  of  comparing versions of the WinSock2 portion of the registry and updating the registry version if required.

    It   is   the  caller's  responsibility  to  call  CloseWinSockRegistryRoot eventually with the returned key.

Arguments:
    None

Return Value:
    The  function  returns the opened registry key if successful.  If it is not successful, it returns NULL.

Implementation Notes:
    The first version of this function has no previous versions of the registry to  be  compatible  with,  so it does not have to take care of updating any
    out-of-date  registry  information.   If  and  when  the  WinSock  spec  or implementation  is  updated  in a way that changes the registry information
...
/*
Routine Description:
    This  procedure  closes  the open registry key representing the root of the WinSock  portion  of the registry.
    The function checks for and handles any errors that might occur.
Arguments:
    RootKey - Supplies  the  open  registry  key  representing  the root of the WinSock portion of the registry.
Return Value:
    None
--*/
{
    ...
    DEBUGF(DBG_TRACE, ("Closing registry root\n"));
...
/*
Routine Description:
    This procedure maps a UNICODE WSAPROTOCOL_INFOW structure to the corresponding ANSI WSAPROTOCOL_INFOA structure.
    All scalar fields are copied over "as is" and any embedded strings are mapped.
Arguments:
...
--*/
{
    ...
    __try {
        ...
        // that the szProtocol[] character array is the last field of the WSAPROTOCOL_INFO structure.
...
/*
Routine Description:
    This procedure maps an ANSI WSAPROTOCOL_INFOA structure to the corresponding UNICODE WSAPROTOCOL_INFOW structure.
    All scalar fields are copied over "as is" and any embedded strings are mapped.
Arguments:
    AnsiProtocolInfo - Points to the source WSAPROTOCOL_INFOA structure.
    UnicodeProtocolInfo - Points to the destination WSAPROTOCOL_INFOW structure.
Return Value:
    INT - ERROR_SUCCESS if successful, a Win32 status code otherwise.
--*/
{
    ...
    __try {
        ...
        CopyMemory(UnicodeProtocolInfo, AnsiProtocolInfo, sizeof(*AnsiProtocolInfo) - sizeof(AnsiProtocolInfo->szProtocol));
...
/*
Routine Description:
    This routine checks for consistency between the protocol or namespace catalog as stored in the registry and the catalog format expected by
    the current version of this DLL. There's no great magic here; this code assumes that the person updating the registry format will change
    the catalog to use a different catalog name (such as Protocol_Catalog9, Protocol_Catalog10, etc.). This assumption means we can validate the
    registry format by validating the *name* of the registry key used for this catalog.
    ...
    The following steps are performed:
        ...
        2.  If it doesn't exist, cool. Just create the new value. This
            typically means we're updating a pre-release system that did not support this mechanism.
        3.  If it does, and its value matches 'ExpectedName', fine.
        4.  If it does, and its value doesn't match, then the catalog format has been updated, so blow away the old catalog, then write the updated value into the registry.

    Since this routine is called at setup/upgrade time, it should only
    fail if something truly horrible happens. In other words, it should be very 'fault tolerant'.

Arguments:

    RootKey - An open key to the WinSock configuration registry tree.

    ValueName - The name of the registry value that contains the name of the current catalog.
        This will typically be a value such as "Current_Protocol_Catalog" or "Current_NameSpace_Catalog".

    ExpectedName - The expected value stored in the 'ValueName' registry value.
        This will typically be a value such as "Protocol_Catalog9" or "NameSpace_Catalog5".
...
Routine Description:
    ...
    This ensures that only one application at a time can modify registry catalog.
Arguments:
    CatalogKey  -   Supplies catalog key to lock

    ExpectedSerialNum -  Supplies catalog serial number that caller expects to see in the registry.
                    It validates that catalog has not changed since it was last read by the client
    AccessKey   -   Returns handle to the registry key that is used for synchronization (to be passed back in ReleaseExclusiveCatalogAccess)
Return Value:
    If  the  function  is  successful, it returns ERROR_SUCCESS.  Otherwise, it
    returns an appropriate WinSock error code:
        WSATRY_AGAIN    - catalog serial number in the registry does not match the one supplied
        WSAEACCESS      - caller does not have write access to the catalog portion of the registry
...
Arguments:
    CatalogKey  -   Supplies catalog key to lock
    CurrentSerialNum -  Supplies catalog serial number which was in effect when catalog was locked.
    AccessKey   -   Supplise handle to the registry key that was used for synchronization.
Return Value:
    None
--*/
{
...
Routine Description:
    ...
    that are accessing the catalog at the time of the call and establishes event notification mechanism for any registry catalog modification afterwards
Arguments:
    CatalogKey  -   Supplies catalog key to synchronize with
    ChangeEvent -   Supplies event to signal when registry catalog is changed.
    CurrentSerialNumber - Returns current catalog serial number
Return Value:
    If  the  function  is  successful, it returns ERROR_SUCCESS.  Otherwise, it returns an appropriate WinSock error code:
--*/
{
    LONG    lresult;
    INT     return_value;
