...
/*++
Routine Description:
    Coustructor for a NSPROVIDER object. Initializes object member variables to default values.
Arguments:
    None
Return Value:
    None
...
inline INT WSAAPI NSPROVIDER::NSPCleanup(VOID)
/*++
Routine Description:
...
--*/
{
    ...
    if (m_library_handle) {
        ...
        lpNSPCleanup = (LPNSPCLEANUP)InterlockedExchangePointer ((PVOID *)&m_proctable.NSPCleanup, NULL);
        if (lpNSPCleanup!=NULL) {
            DEBUGF( DBG_TRACE,("Calling NSPCleanup for provider %X\n", this));
...
/*++
Routine Description:
    Destructor for NSPROVIDER object.  Frees resoures used by the object and set the member variables to the uninitialized state.
Arguments:
    None
Return Value:
    None
...
INT NSPROVIDER::Initialize(IN LPWSTR lpszLibFile, IN LPGUID  lpProviderId)
/*++
Routine Description:
    This routine initializes an NSPROVIDER object.
Arguments:
    lpszLibFile - A string containing the path to the DLL for the name space provider to be associated with this object.
    lpProviderId - A pointer to a GUID containing the provider ID for the namespace provider.
Return Value:
    ERROR_SUCCESS if the provider was successfully initialized else an apropriate winsock error code.
--*/
{
    ...
    if( PathLength == 0 ) {
        DEBUGF(DBG_ERR, ("Cannot map library path from UNICODE to ANSI\n"));
        return WSASYSCALLFAILURE;
    }

    // Expand the library name to pickup environment/registry variables
    ExpandedPathLen = ExpandEnvironmentStringsA(AnsiPath, ExpandedAnsiPath, MAX_PATH);
    ...
    TRY_START(guard_memalloc) {
...
#endif
        // Load the provider DLL, Call the provider startup routine and validate that the provider filled out all the NSP_ROUTINE function pointers.
        ...
        NSPStartupFunc = (LPNSPSTARTUP)GetProcAddress(m_library_handle, "NSPStartup");
        ...
        __try {
            ReturnCode = (*NSPStartupFunc)(lpProviderId, &m_proctable);
        }
