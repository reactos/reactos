...
Abstract:
    ...
    This  module takes care of forwarding the "getxbyy" family of operations to the  correct getxbyy provider.
    The preferred getxbyy provider is found via a  DLL  path  stored  in the registry.
    If this registry entry is found, an attempt  is  made  to  load the DLL and retrieve entry points from it.
    Any failures  cause  entry  points to be taken from the WSOCK32.DLL instead.
    A getxbyy provider does not have to export all of the getxbyy functions since any it does not export are taken from WSOCK32.DLL.

    The  getxbyy  functions  handled  by  this module that can be replaced by a getxyy provider are the following:
    ...
    The   actual  entry  points  supplied  by  the  getxbyy  provider  (or  the WSOCK32.DLL) are prefixed with a special string.
    This prefix is defined by the manifest constant GETXBYYPREFIX.
...
// This is the initial buffer size passed by getxbyy functions to WSALookupServiceNext.
// If it is insuffucient for the query, the amount specified by the provider is allocated and call is repeated.
// The initial buffer is allocated from stack, so we try to keep it
// relatively small, but still for performance reasons we want to be able to satisfy most of the calls with just this amount
...
// Utility to turn a list of offsets into a list of addresses. Used to convert structures returned as BLOBs.
VOID FixList(PCHAR ** List, PCHAR Base)
{
    if (*List) {
        PCHAR * Addr;
...
// Routine to convert a hostent returned in a BLOB to one with usable pointers. The structure is converted in-place.
VOID UnpackHostEnt(struct hostent * hostent)
{
    PCHAR pch;
...
// Routine to unpack a servent returned in a BLOB to one with usable pointers. The structure is converted in-place
VOID UnpackServEnt(struct servent * servent)
{
    PCHAR pch;
...
struct hostent FAR * WSAAPI gethostbyaddr(IN const char FAR * addr, IN int len, IN int type)
    ...
    Returns:
        ...
        Otherwise  it  returns  a NULL pointer and a specific error code is stored with SetErrorCode().
    --*/
{
    CHAR qbuf[100];
    struct hostent *ph;
    ...
    // BUGBUG. Only handles current inet address forms. Need to fix it to handle any address length, or do we?
    (void)sprintf(qbuf, "%u.%u.%u.%u", ((unsigned)addr[0] & 0xff), ((unsigned)addr[1] & 0xff), ((unsigned)addr[2] & 0xff), ((unsigned)addr[3] & 0xff));

    pBlob = getxyDataEnt(&pResults, RNR_BUFFER_SIZE, qbuf, &AddressGuid, 0);
    if (pBlob) {
...
struct hostent FAR * WSAAPI gethostbyname(IN const char FAR * name)
    ...
    Returns:
        ...
        Otherwise  it  returns  a NULL pointer and a specific errorr code is stored with SetErrorCode().
    --*/
{
    struct hostent * hent;
    LPBLOB pBlob;
    ...
    CHAR  szLocalName[200];   // for storing the local name.
                              // This is simply a big number assumed to be large enough.
                              // This is used only when the caller chooses not to provide a name. Very lazy.
    PCHAR pszName;
    ...
    // A NULL input name means look for the local name. So, get it.
    if (!name || !*name) {
    ...
    pBlob = getxyDataEnt(&pResults, RNR_BUFFER_SIZE, pszName, &HostAddrByNameGuid, 0);
    if (!pBlob && (!name || !*name)) {
        pBlob = getxyDataEnt(&pResults, RNR_BUFFER_SIZE, NULL, &HostAddrByNameGuid, 0);
    }
    ...
    // This is important with DNS, since reverse lookups do not provide complete information about name aliases.
    if (hent)
...
Returns:
    ...
    The  error  code  is  stored  with SetErrorCode().
--*/
{
    PCHAR lpName = NULL;
    ...
    ValueLength = GetEnvironmentVariableA("_CLUSTER_NETWORK_NAME_", name, namelen);
    if (ValueLength != 0) {
    ...
    (void)getxyDataEnt(&pResults, RNR_BUFFER_SIZE, NULL, &HostAddrByNameGuid, &lpName);
    if (lpName) {
...
struct servent FAR * WSAAPI getservbyport(IN int port, IN const char FAR * proto)
    ...
    Arguments:
        ...
        proto - An  optional  pointer  to  a  protocol  name.   If  this  is  NULL,
                getservbyport()  returns the first service entry for which the port matches  the  s_port.
                Otherwise  getservbyport() matches both the port and the proto.
    Returns:
        If  no  error  occurs,  getservbyport()  returns  a  pointer to the servent structure  described  above.
        Otherwise  it  returns  a NULL pointer and a specific error code is stored with SetErrorCode().
    --*/
{
    PCHAR pszTemp;
...
struct servent FAR * WSAAPI getservbyname(IN const char FAR * name, IN const char FAR * proto)
    ...
    Arguments:
         ...
         proto - An  optional  pointer to a null terminated protocol name.  If this
                 pointer  is  NULL, getservbyname() returns the first service entry for  which  the  name  matches the s_name or one of the s_aliases.
    ...
    Returns:
         ...
         structure  described  above.   Otherwise  it  returns a NULL pointer and a specific error code is stored with SetErrorCode().
    --*/
{
    PCHAR pszTemp;
    struct servent * sent;
...
)
{
    ...
    err = WSALookupServiceBeginA(pwsaq, LUP_RETURN_BLOB | LUP_RETURN_NAME, &hRnR);
    if (err == NO_ERROR) {
        // If the original buffer is small to contain the results will allocate new one and retry the call.
    Retry:

        // The query was accepted, so execute it via the Next call.
        err = WSALookupServiceNextA(hRnR, 0, &dwLength, pwsaq);
        // if NO_ERROR was returned and a BLOB is present, this worked, just return the requested information.
        // Otherwise, invent an error or capture the transmitted one.
        if (err == NO_ERROR) {
...
Routine Description:
    ...
    This function is an asynchronous version of gethostbyname(), and is used to retrieve host name and address information corresponding to a hostname.
    The Windows Sockets implementation initiates the operation and returns to the caller immediately, passing back an
    asynchronous task handle which the application may use to identify the operation.
    When the operation is completed, the results (if any) are copied into the buffer provided by the caller and a message is sent to the application's window.
...
Routine Description:
    ...
    asynchronous operation which was initiated by one of the WSAAsyncGetXByY() functions such as WSAAsyncGetHostByName().
    The operation to be canceled is identified by the hAsyncTaskHandle parameter, which should be set to the asynchronous task handle as returned by the initiating function.
Arguments:
