...
Revision History:
    21-Aug-1995 dirk@mink.intel.com
       ...
       CRITICAL_SECTION's from pointers to being embedded in the dprocess object.
--*/
...
/*++
Routine Description:
    DPROCESS  object constructor.  Creates and returns a DPROCESS object.
    Note that  the DPROCESS object has not been fully initialized.
    The "Initialize" member  function  must  be  the  first  member  function  called on the new DPROCESS object.

    In  the Win32 environment, only one DPROCESS object may be in existence for a  process.
    It  is  the  caller's  responsibility  to  ensure  that  this restriction is met.
Return Value:
    Returns a pointer to the new DPROCESS object or NULL if a memory allocation failed.
--*/
{
...
INT DPROCESS::Initialize()
/*++
Routine Description:
    Completes  the  initialization  of  the DPROCESS object.
    This must be the first  member  function  called  for  the DPROCESS object.
    This procedure should be called only once for the object.
Return Value:
  The  function returns 0 if successful.  Otherwise it returns an appropriate WinSock error code if the initialization cannot be completed.
--*/
{
    ...
    TRY_START(mem_guard){
        ...
        if (!RegistryKey) {
            DEBUGF(DBG_ERR, ("Opening Winsock Registry Root\n"));
        ...
        if (m_proto_catalog_change_event==NULL) {
            ...
            DEBUGF(DBG_ERR, ("Creating protocol catalog change event, err:%ld\n", ReturnCode));
            TRY_THROW(mem_guard);
        }
        ...
        if (!m_protocol_catalog) {
            DEBUGF(DBG_ERR, ("Allocating dcatalog object\n"));
            ReturnCode = WSA_NOT_ENOUGH_MEMORY;
            TRY_THROW(mem_guard);
        } //if

        ReturnCode = m_protocol_catalog->InitializeFromRegistry(RegistryKey, m_proto_catalog_change_event);
        if (ERROR_SUCCESS != ReturnCode) {
            DEBUGF(DBG_ERR, ("Initializing protocol catalog from registry\n"));
            TRY_THROW(mem_guard);
        } //if
        ...
        if (m_ns_catalog_change_event==NULL) {
            ReturnCode = GetLastError ();
            DEBUGF(DBG_ERR, ("Creating ns catalog change event, err:%ld\n", ReturnCode));
            TRY_THROW(mem_guard);
        }
        ...
        if (ERROR_SUCCESS != ReturnCode) {
            DEBUGF(DBG_ERR, ("Initializing name space catalog from registry\n"));
            TRY_THROW(mem_guard);
        } //if
...
BOOL DeleteSockets(LPVOID              EnumCtx, LPWSHANDLE_CONTEXT  HContext) {
    return ((PDPROCESS)EnumCtx)->DSocketDetach (HContext);
}


BOOL DPROCESS::DSocketDetach (IN LPWSHANDLE_CONTEXT   HContext)
{
    ...
    if (Provider!=NULL) {
        ...
        __try {
            ...
            if (ReturnValue!=NO_ERROR) {
                DEBUGF (DBG_WARN, ("Calling provider %ls cleanup function, ret: %ld, err: %ld.\n", CatalogEntry->GetProtocolInfo()->szProtocol, ReturnValue, ErrorCode));
            }
        }
        __except (WS2_EXCEPTION_FILTER ()) {
    ...
    )
{
    ...
    if (Provider!=NULL) {
        ...
        __try {
            ...
            if (ReturnValue!=NO_ERROR) {
                DEBUGF (DBG_WARN, ("Calling provider %ls cleanup function, ret: %ld, err: %ld.\n", CatalogEntry->GetProviderDisplayString (), ReturnValue, GetLastError ()));
            }
        }
        __except (WS2_EXCEPTION_FILTER ()) {
...
--*/
{
    ...
    {
        WahEnumerateHandleContexts (DSOCKET::sm_context_table, DeleteSockets, this);
    }
    ...
    while (!IsListEmpty(&m_thread_list)) {
        Thread = CONTAINING_RECORD(m_thread_list.Flink, DTHREAD, m_dprocess_linkage);
    ...
    DeleteCriticalSection( &m_thread_list_lock );// Clean up critical sections
} //~DPROCESS


INT DPROCESS::DThreadAttach(IN PDTHREAD NewThread)
/*++
Routine Description:
    Adds  a  DTHREAD  reference  into  the  list  of  threads belonging to this process.
    The  operation  takes  care of locking and unlocking the list as necessary.
Arguments:
    NewThread - Supplies  the  reference to the DTHREAD object to be added into the list
Return Value:
    The  function  returns 0 if successful, otherwise it returns an appropriate WinSock error code.
--*/
{
...
INT DPROCESS::DThreadDetach(IN PDTHREAD  OldThread)
/*++
Routine Description:
    Removes  a  DTHREAD  reference  from  the list of threads belonging to this process.
    The  operation  takes  care of locking and unlocking the list as necessary.
Arguments:
    OldThread - Supplies the reference to the DTHREAD object to be removed from the list.
Return Value:
    The  function  returns 0 if successful, otherwise it returns an appropriate WinSock error code.
--*/
{
...
INT DPROCESS::DProcessClassInitialize(IN VOID)
/*++
Routine Description:
    Performs  global  initialization for the DPROCESS class.
    In particular, it creates  the  global  DPROCESS  object  and  stores  it  in a static member variable.
Arguments:
    None
Return Value:
    The  function  returns ERROR_SUCCESS if successful, otherwise it returns an appropriate WinSock error code.
--*/
{
    ...
    if (sm_current_dprocess) {
        ...
        if (ReturnCode != ERROR_SUCCESS) {
            DEBUGF( DBG_ERR, ("Failed to Initialize dprocess object\n"));
            delete(sm_current_dprocess);
            sm_current_dprocess = NULL;
        }
    } //if
    else {
        DEBUGF( DBG_ERR, ("Failed to allocate dprocess object\n"));
        ReturnCode = WSA_NOT_ENOUGH_MEMORY;
    } //else
...
INT DPROCESS::OpenAsyncHelperDevice(OUT LPHANDLE HelperHandle)
/*++
Routine Description:
    Retrieves  the  opened  Async  Helper  device  ID  required  for processing callbacks  in  the  overlapped  I/O  model.
    The operation opens the Async Helper device if necessary.
Arguments:
    HelperHandle - Returns the requested Async Helper device ID.
Return Value:
    The  function  returns ERROR_SUCESS if successful, otherwise it returns an appropriate WinSock error code.
--*/
{
    INT ReturnCode;
...
INT DPROCESS::OpenHandleHelperDevice(OUT LPHANDLE HelperHandle)
/*++
Routine Description:
    Retrieves  the  opened  Handle  Helper  device  ID  required  for allocation of socket handles for non-IFS providers.
    The operation opens the Handle Helper device if necessary.
Arguments:
...
--*/
{
    ...
    LockDThreadList ();// Protect from double opens
...
INT DPROCESS::OpenNotificationHelperDevice(OUT LPHANDLE HelperHandle)
/*++
Routine Description:
...
VOID DPROCESS::SetVersion( WORD Version )
/*++
Routine Description:
...
--*/
{
    ...
    if( newMajor < GetMajorVersion() || ( newMajor == GetMajorVersion() && newMinor < GetMinorVersion() ) ) {
        m_version = Version;
    }
} // SetVersion


PDCATALOG DPROCESS::GetProtocolCatalog()
/*++
Routine Description:
...
PNSCATALOG DPROCESS::GetNamespaceCatalog()
/*++
Routine Description:
