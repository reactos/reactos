...
BOOL DCATALOG::OpenCatalog(IN  HKEY   ParentKey)
/*
Routine Description:
    This  procedure  opens the catalog portion of the registry.
    If the catalog is  not  yet  present,  it  also  initializes  new  first-level  values and first-level  subkeys  for  the  catalog.
    It is assumed that the catalog is locked against competing registry I/O attempts.
Arguments:
    ParentKey  - Supplies  the open registry key representing the parent key of the catalog.
Return Value:
...
--*/
{
    ...
    TRY_START(guard_open) {
        ...
        if (key_disposition == REG_CREATED_NEW_KEY) {
            ...
            DEBUGF(DBG_TRACE, ("Creating empty catalog in registry.\n"));
...
INT DCATALOG::InitializeFromRegistry(IN  HKEY    ParentKey, IN  HANDLE  ChangeEvent OPTIONAL)
/*
Routine Description:
    This  procedure takes care of initializing a newly-created protocol catalog from  the  registry.
    If the registry does not currently contain a protocol catalog,  an  empty catalog is created and the registry is initialized with the new empty catalog.

Arguments:
    ParentKey - Supplies  an  open registry key under which the catalog is read or  created  as  a  subkey.
    The  key may be closed after this procedure returns.
Return Value:
...
INT DCATALOG::RefreshFromRegistry(IN  HANDLE  ChangeEvent OPTIONAL)
/*
Routine Description:
        ...
        TRY_START(guard_open) {
            ...
            for (seq_num = 1; seq_num <= num_entries; seq_num++) {
                ...
                if (item == NULL) {
                    ...
                    DEBUGF(DBG_ERR, ("Allocating new proto catalog item\n"));
                    TRY_THROW(guard_open);
                }
...
INT DCATALOG::WriteToRegistry()
/*
Routine Description:
    This procedure writes the "entries" and "numentries" portion of the catalog out  to  the  registry.
Return Value:
...
Routine Description:
    ...
    The operation takes care of creating, initializing, and setting a DPROVIDER object  for the retrieved catalog item if necessary.
Arguments:
    CatalogEntryId  - Supplies The ID of a catalog entry to be searched for.
    CatalogItem     - Returns a reference to the catalog item with the matching catalog entry ID if it is found, otherwise returns NULL.
Return Value:
  The  function  returns  ERROR_SUCESS  if  successful, otherwise it returns an appropriate WinSock error code.
--*/
{
...
Routine Description:
    ...
    The procedure selects the first transport provider able to support the stipulated address family, socket type, and protocol values.
    If "protocol" is not specifieid (i.e., equal to zero).
    the default for the specified socket type is used.
    However, the address family may be given as AF_UNSPEC  (unspecified),  in  which  case the "protocol" parameter must be specified.
...
Arguments:
    ...
    protocol    - Supplies  an  address  family  specific  identification  of a protocol  to  be  used with a socket, or 0 if the caller does not wish to specify a protocol.
    StartAfterId - Optionally (non 0) supplies the catalog id of the item after which to begin enumeration.
    CatalogItem - Returns  a reference to the catalog item that was found to be a suitable match or NULL if no suitable match was found.
Return Value:
    The  function  returns ERROR_SUCCESS if successful, otherwise it returns an appropriate WinSock error code.
Implementation Notes:
    For  each  protocol  item  to  test,  match  first  type, then family, then protocol.
    Keep  track of the "strongest" match found.
    If there was not a complete  match,  the  strength of the strongest match determines the error code returned.
--*/
{
...
#define LARGER_OF(a,b) (((a) > (b)) ? (a) : (b))
    ...
    * CatalogItem = NULL;// Prepare for early error returns
    ...
    if (af == 0) {
        ...
        DEBUGF(DBG_WARN, ("Use of AF_UNSPEC is discouraged\n"));
...
#define TYPE_WILDCARD_VALUE 0
            // Can  this  entry  support  the  requested  socket  type?   Or is the wildcard type specified?
            if ((ProtoInfo->iSocketType == type) || (type == TYPE_WILDCARD_VALUE)) {
                match_strength = LARGER_OF(match_strength, MATCHED_TYPE);

#define FAMILY_WILDCARD_VALUE AF_UNSPEC
                // Can it support the requested address family?  Or is the wildcard family specified?
                if ((ProtoInfo->iAddressFamily == af) || (af == FAMILY_WILDCARD_VALUE)) {
                    match_strength = LARGER_OF(match_strength, MATCHED_TYPE_FAMILY);

#define PROTO_IN_RANGE(proto,lo,hi) (((proto) >= (lo)) && ((proto) <= (hi)))
#define IS_BIT_SET(test_val,bitmask) (((test_val) & (bitmask)) == (bitmask))
                    // Is  the  requested  protcol  in  range?  Or is the requested protocol zero and entry supports protocol zero?
                    {  // declare block
                        int range_lo = ProtoInfo->iProtocol;
                        int range_hi = range_lo + ProtoInfo->iProtocolMaxOffset;
                        if (PROTO_IN_RANGE(protocol, range_lo, range_hi) || ((protocol == 0) && IS_BIT_SET(ProtoInfo->dwProviderFlags, PFL_MATCHES_PROTOCOL_ZERO))) {
                            match_strength = LARGER_OF(match_strength, MATCHED_TYPE_FAMILY_PROTOCOL);
    ...
    if (ReturnCode == ERROR_SUCCESS) {
        if (CatalogEntry->GetProvider() == NULL) {
            ReturnCode = LoadProvider(CatalogEntry);
            if (ReturnCode != ERROR_SUCCESS) {
                DEBUGF(DBG_ERR, ("Error (%lu) loading chosen provider\n", ReturnCode));
            } // else
        }  // if provider is NULL
...
INT DCATALOG::FindIFSProviderForSocket(SOCKET Socket)
/*
Routine Description:
    This procedure searches the installed providers that support IFS handles for one that recognizes the given socket.
    If one is found, then the necessary internal infrastructure is established for supporting the socket.
Arguments:
    Socket - The socket.
Return Value:
    If  the  function  is  successful,  it  returns ERROR_SUCCESS, otherwise it returns an appropriate WinSock error code.
...
VOID DCATALOG::AppendCatalogItem(IN  PPROTO_CATALOG_ITEM  CatalogItem)
/*
Routine Description:
...
VOID DCATALOG::RemoveCatalogItem(IN  PPROTO_CATALOG_ITEM  CatalogItem)
/*
Routine Description:
...
LPSTR DCATALOG::GetCurrentCatalogName()
{
    return CATALOG_NAME;
} // GetCurrentCatalogName


INT DCATALOG::LoadProvider(IN PPROTO_CATALOG_ITEM CatalogEntry)
/*
Routine Description:
    Load   the   provider  described  by  CatalogEntry and set it into catalog entry
Arguments:
    CatalogEntry - Supplies  a reference to a protocol catalog entry describing the provider to load.
Return Value:
    The  function  returns ERROR_SUCCESS if successful, otherwise it returns an appropriate WinSock error code.
--*/
{
...
PDPROVIDER DCATALOG::FindAnotherProviderInstance(IN LPGUID ProviderId)
/*
Routine Description:
    Check  all catalog enteries for a provider with the pointer to the provider object for the provider.
Arguments:
