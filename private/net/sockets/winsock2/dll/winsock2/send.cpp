...
Returns:
    The  total  number  of  bytes  sent.  Otherwise, a value of SOCKET_ERROR is returned, and the error code is stored with SetLastError().
--*/
{
    PDSOCKET           Socket;
    INT                ErrorCode;
...
Arguments:
    ...
    lpBuffers           - A  pointer  to  an  array of WSABUF structures.
                          Each WSABUF  structure  contains a pointer to a buffer and the length of the buffer.
...
Returns:
    If  no  error  occurs  and  the  send  operation has completed immediately, WSASend() returns 0.
    ...
    If the MSG_INTERRUPT  flag  is set, the meaning of the return value is changed.
    A value  of  zero  indicates  success  and is interpreted as described above.
    Otherwise,  the  return  value  will directly contain the appropriate error code.
    Note that this is applicable only to Win16 environments and only for protocols that have the XP1_INTERRUPT bit set in the WSAPROTOCOL_INFO struct.
--*/
{
    PDSOCKET           Socket;
    INT                ErrorCode;
    ...
    if (ErrorCode==ERROR_SUCCESS)
    {
        ...
        if(Socket != NULL){
            ...
            Provider = Socket->GetDProvider();
            ReturnValue = Provider->WSPSend(s, lpBuffers, dwBufferCount, lpNumberOfBytesSent, dwFlags, lpOverlapped, lpCompletionRoutine, ThreadId, &ErrorCode);
            Socket->DropDSocketReference();
            if (ReturnValue==ERROR_SUCCESS)
                return ReturnValue;
            assert (ErrorCode!=NO_ERROR);
...
int WSAAPI WSASendDisconnect(IN SOCKET s, OUT LPWSABUF lpOutboundDisconnectData)
/*++
Routine Description:
...
Returns:
    ERROR_SUCCESS  on  success  else  SOCKET_ERROR.   The  error  code is  stored  with SetLastError().
--*/
{
    INT                ErrorCode;
    PDPROCESS          Process;
    ...
    if (ErrorCode==ERROR_SUCCESS) {
        ...
        if(Socket != NULL){
            ...
            Provider = Socket->GetDProvider();
            ReturnValue = Provider->WSPSendDisconnect(s, lpOutboundDisconnectData, &ErrorCode);
            Socket->DropDSocketReference();
            if (ReturnValue==ERROR_SUCCESS)
...
Arguments:
    s                   - A descriptor identifying a connected socket which was created      using      WSASocket()     with     flag WSA_FLAG_OVERLAPPED.
    lpBuffers           - A  pointer  to  an  array of WSABUF structures.  Each WSABUF  structure  contains a pointer to a buffer and thee length of the buffer.
    dwBufferCount       - The  number  of  WSABUF  structures  in the lpBuffers array.
    lpNumberOfBytesSent - A pointer to the number of bytes sent by this call if the I/O operation completes immediately.
    dwFlags             - Specifies the way in which the call is made.
    ...
    lpCompletionRoutine - A  pointer  to the completion routine called when the send   operation  has  been  completed  (ignored  for non-overlapped sockets).
Returns:
    If the function completes successfully, it returns ERROR_SUCCESS, otherwise it returns SOCKET_ERROR.
--*/
{
    PDSOCKET           Socket;
    INT                ErrorCode;
    ...
    if (ErrorCode==ERROR_SUCCESS)
    {
        ...
        if(Socket != NULL){
            ...
            Provider = Socket->GetDProvider();
            ReturnValue = Provider->WSPSendTo(s, lpBuffers, dwBufferCount, lpNumberOfBytesSent, dwFlags, lpTo, iTolen, lpOverlapped, lpCompletionRoutine, ThreadId, &ErrorCode);
            Socket->DropDSocketReference();
            if (ReturnValue==ERROR_SUCCESS)
                return ReturnValue;
            assert (ErrorCode!=NO_ERROR);
