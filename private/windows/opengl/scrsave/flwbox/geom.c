...
void DrawWithVArrays (GEOMETRY *geom)
{
    ...
    for (side = 0; side < geom->nsides; side++) {
        geom->sides[side].dBuf = (GLuint *) LocalAlloc (LMEM_FIXED, sizeof (GLuint) * 3 * MAXPTS * 2);
        k = 0;
...
void DrawGeom(GEOMETRY *geom)
{
    ...
    if (!(bOgl11 && !bCheckerOn)) {
                ...
                for (idc = 0; idc < geom->sides[side].strip_size[strip];
                    ...
                    if (config.color_pick == ID_COL_CHECKER) {
                        if (config.triangle_colors) {
                            glMaterialfv(config.two_sided, GL_DIFFUSE, checker_cols[side][(idc+1)/2+strip & 1]);
                        }
                        else {
                            glMaterialfv(config.two_sided, GL_DIFFUSE, checker_cols[side][idc/2+strip & 1]);
                        }
                    }
        ...
        for (side = 0; side < geom->nsides; side++) {
            ...
            glDrawElements (GL_TRIANGLES, geom->sides[side].num_eles, GL_UNSIGNED_INT, &(geom->sides[side].dBuf[0]));
...
void ComputeAveragedNormals(GEOMETRY *geom)
{
    ...
    for (side = 0; side < geom->nsides; side++)
    {
        ...
        for (strip = 0; strip < geom->sides[side].nstrips; strip++)
        {
            ...
            assert(idx1 >= 0 && idx1 < geom->total_pts && idx2 >= 0 && idx2 < geom->total_pts);
