...
//mf: since pass bXXX params why not do same with ulSurfStyle, fTesselFact, ulTexQual
STATE::STATE(BOOL bFlexMode, BOOL bMultiPipes)
{
    resetStatus = RESET_STARTUP_BIT;// various state values

    // Put initial hglrc in drawThreads[0]
    // This RC is also used for dlists and texture objects that are shared by other RC's
    ...
    // will be drawn, NORMAL_STATE will be initialized so that dlists are built

    // Again, since have either NORMAL or FLEX, set maxPipesPerFrame, maxDrawThreads
    if (bMultiPipes)
    ...
    drawScheme = FRAME_SCHEME_RANDOM;// default draw scheme
}
...
void STATE::CalcTexRepFactors()
{
    ...
    // Figure out repetition factor of texture, based on bitmap size and screen size.

    // We arbitrarily decide to repeat textures that are smaller than 1/8th of screen width or height.
    for (int i = 0; i < nTextures; i++) {
    ...
    if (pNState) {
        //put smallest rep factors in texRep[0]; (mf:this is ok for now, as flex pipes and normal pipes don't coexist)
...
* buffer mode.  We set resetStatus accordingly to clear things up on next draw.
\**/
void STATE::Repaint(LPRECT pRect, void *data)
{
...
*   - set new window size for VIEW object, and set resetStatus for validation at draw time
\**/
...
* Called on FrameReset resulting from change in viewing paramters (e.g. from a Resize event).
\**/
void STATE::ResetView()
{
    IPOINT3D numNodes;

    view.CalcNodeArraySize(&numNodes);// Have VIEW calculate the node array size based on view params
    nodes->Resize(&numNodes);// Resize the node array
...
void STATE::FrameReset()
{
    ...
    nodes->Reset();// Reset the node states to empty
    ...
    for (i = 0; i < nDrawThreads; i++, pThread++) {
        ...
        if (!pThread->HasRC()) {
            ...
            GLInit(); // Do GL Init for this new RC
            view.SetGLView();// Set viewing params
            wglShareLists(shareRC, pThread->GetRC());// Give this rc access to any dlists
        }
        else
            pThread->MakeRCCurrent();
        ...
        glRotatef(view.yRot, 0.0f, 1.0f, 0.0f);// Rotate Scene
    ...
    resetStatus = 0;// clear reset status
...
\**/
static int PickRandomTexture(int iThread, int nTextures)
{
    if (nTextures == 0)
        return 0;
...
/**\
* Clear the screen.  Depending on resetStatus, use normal clear or fancy transitional clear.
\**/
void STATE::Clear()
{
    ...
    if (resetStatus & RESET_RESIZE_BIT) {
        ...
        glClear(GL_COLOR_BUFFER_BIT); // do a fast one-shot clear
    }
}
...
*   stuck.  Once number of drawing threads reaches 0, we start a new frame
\**/
void STATE::Draw(void *data)
{
    ...
    DrawValidate();// Validate the draw state
    ...
    for (int i = 0; i < nDrawThreads; i++, pThread++) {
        if (pThread->pPipe->IsStuck()) {
            if (++nPipesDrawn > maxPipesPerFrame) {
                ...
                if ((drawScheme == FRAME_SCHEME_CHASE) && (pThread->pPipe == pLeadPipe))
                    bChooseNewLead = TRUE;
...
/*
* Starts up pipe of the approppriate type.  If can't find an empty node for the pipe to start on, returns FALSE;
*/
{
    MakeRCCurrent();
