...
void CEudcList::SetInitEUDCCodeRange(int     nIndex)
{
    m_Index = nIndex;
...
/*    Set coderange of EUDC        */
void CEudcList::SetEUDCCodeRange(int     nIndex)
{
    m_Index = nIndex;
    EUDC_SView = CountryInfo.sRange[m_Index] & 0xfff0;
    EUDC_EView =(CountryInfo.eRange[m_Index] & 0xfff0) -(0x10*(NUM_LINE-1));
...
WORD CEudcList::CorrectEUDCCode(WORD     Code, BOOL     UporDown)
{
COUNTRYINFO    Info;
    WORD    wCode;
    BYTE     LByte, HByte;
    ...
    for( int i = 0; i < Info.nTralByte; i++){
        if( LByte >= (Info.sTralByte[i] & 0xf0) && LByte <= (Info.eTralByte[i] & 0xf0)){
                goto RET;
...
#ifdef BUILD_ON_WINNT
int CEudcList::GetBarPosition(WORD     Code)
{
COUNTRYINFO    Info;
    ...
    for (i = 0; i < Info.nTralByte; i++)
        if( LOBYTE( Code) >= (Info.sTralByte[i] & 0xf0) && LOBYTE( Code) <= (Info.eTralByte[i] & 0xff)){
...
void CEudcList::CalcCharSize()
{
    ...
    CharSize.cy = ( EditListRect.Height() - (NUM_LINE-1)*LINEWIDTH - 2) / NUM_LINE;
    CharSize.cx = ( EditListRect.Width() - FixSize.cx - 2 - (NUM_CHAR*LINEWIDTH)) / NUM_CHAR;
    ...
    for( int i = 0; i < NUM_LINE; i++){
        ...
        for( int j = 0; j < NUM_CHAR; j++){
            ...
            rcEditChar[i][j].right  = rcEditChar[i][j].left + CharSize.cx;
            rcEditChar[i][j].bottom = rcEditChar[i][j].top + CharSize.cy;
        }
    }
#ifdef BUILD_ON_WINNT
    for( int j = 0; j < NUM_CHAR; j++){
...
//    Check character code
    ...
    ExtTextOut( dc.GetSafeHdc(), rcEditCode[i].left, rcEditCode[i].top  + yOffset, ETO_OPAQUE, &rcEditCode[i], (LPCTSTR)CodeArray, 4, NULL);
        ...
        for( j = 0; j < NUM_CHAR; j++,Code++){
            ...
            for( k = 0; k < CountryInfo.nTralByte; k++){
                if( sOffset >= CountryInfo.sTralByte[k] && sOffset <= CountryInfo.eTralByte[k]){
                        flg = TRUE;
                }
            }
            if((Code < CountryInfo.sRange[cRange]) ||(Code > CountryInfo.eRange[cRange]))
                flg = FALSE;
        ...
        CodeArray[2] = (BYTE)'\0';
            ...
            if(( rcEditChar[i][j].PtInRect( LButtonPt) || SelectCode == Code) && wLength != 0){
                TCHAR    CodeNum[10];
      ...
      if (CountryInfo.bUnicodeMode)
            ...
            if(bHasGlyph == TRUE)
            {
                if (CountryInfo.bUnicodeMode)
                {
                    ...
                    ExtTextOutA(dc.GetSafeHdc(),rcEditChar[i][j].left + xOffset, rcEditChar[i][j].top + yOffset, ETO_OPAQUE, &rcEditChar[i][j], (LPCSTR)ViewTmp, wLength, NULL);
#endif
                }
                else
                {
                    ExtTextOutA(dc.GetSafeHdc(), rcEditChar[i][j].left + xOffset, rcEditChar[i][j].top + yOffset, ETO_OPAQUE, &rcEditChar[i][j], (LPCSTR)CodeArray, wLength, NULL);
                }
            }
...
CScrollBar* pScrollBar)
{
    ...
    if( abs( MoveScr) < (CharSize.cy + LINEWIDTH)*NUM_LINE && abs( MoveScr) > 0 && !ThumbTrkFlag){
        CRect    ScrllRect;
        CRect    ClintRect;
    ...
    for( i = 0; i < NUM_LINE; i++){
        for( j = 0; j < NUM_CHAR; j++){
            if( rcEditChar[i][j].PtInRect( LButtonPt)){
                test.SetRect( rcEditChar[i][j].left - 2, rcEditChar[i][j].top - 2, rcEditChar[i][j].right + 2, rcEditChar[i][j].bottom + 2);
...
void CEudcList::SearchSelectPosition()
{
    CRect    test;
    int    sViewPt, sCodePt;
unsigned int    i, j;
...
//fix for 261529
  ...
  if( !ViewFont.CreateFontIndirect( &LogFont))
    ...
    if (CountryInfo.bUnicodeMode)
    {
        ...
        ExtTextOutA(dc.GetSafeHdc(), xOffset, yOffset, ETO_OPAQUE, &ViewRect, (LPCSTR)ViewTmp, 2, NULL);
#endif
...
CColumnHeading::OnPaint()
{
    ...
    for( j = 0; j < NUM_CHAR; j++){
        ...
        GetTextExtentPoint32(dc.GetSafeHdc(), ColumnHead, 1, &cSize);
        ...
        dc.ExtTextOut( rcColumnHead[j].left + xOffset, rcColumnHead[j].top, ETO_OPAQUE, &rcColumnHead[j], ColumnHead, 1, NULL);
    }
}
