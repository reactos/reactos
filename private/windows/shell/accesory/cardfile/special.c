...
void DoPasteSpecial(void)
{
    ...
    Result= DialogBox(hIndexInstance, (LPTSTR)  MAKEINTRESOURCE(DLGPASTESPECIAL), hIndexWnd, (DLGPROC) PasteSpecialDlgProc);
    if (Result == -1)
        return; /* probably error msg , out of mem */
    else if (Result == 0)
        return;   /* dlg cancelled */

    PicPaste( &CurCard, fPaste, (WORD) ((ClipFormat == vcfOwnerLink) ? 0 : ClipFormat) );
}


int FAR PASCAL PasteSpecialDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    int i;
...
NOEXPORT BOOL GetClipObjectInfo(LPTSTR szClass, LPTSTR szItem)
{
    ...
    if (!(hData = GetClipboardData(vcfOwnerLink)) && !(hData = GetClipboardData(vcfLink)))
    {
        CloseClipboard();
        return FALSE;
    }
    ...
    iDataSize = GlobalSize(hData);// convert to unicode if need be
    ...
    {
        MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, (LPCSTR) lpData, iDataSize, szShortClass, CharSizeOf(szShortClass));
    }
