...
BOOL fnLinksDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDD_UPDATE:        /* Update the Link */
            ...
            if (SendMessage(hIndexWnd, WM_COMMAND, wParam, 0L) && LOWORD(wParam) == IDD_FREEZE)
            {
        ...
        case IDD_AUTO:
        case IDD_MANUAL:
            if ((IsDlgButtonChecked(hDlg, IDD_AUTO) && LOWORD(wParam) == IDD_AUTO) || (IsDlgButtonChecked(hDlg, IDD_MANUAL) && LOWORD(wParam) == IDD_MANUAL) || fRecursive)
                break;

            fRecursive = TRUE;

            switch (OleSetLinkUpdateOptions(CurCard.lpObject, wParam == IDD_AUTO ? oleupdate_always : oleupdate_oncall))
            {
