...
NOEXPORT int GetKeyCount(HKEY hKeyVerb);
NOEXPORT BOOL FixEditObject(HMENU hEditMenu);
...
void OleMenuItemFix(HMENU hMenu, int Mode)
{
    ...
    if (Mode == I_TEXT)
    {
        ...
        mfCutCopy = (HIWORD(lSelection) == LOWORD(lSelection)) ? MF_GRAYED : MF_ENABLED;
    ...
    else
    {
        ...
        if (fOLE)
        {
            if (OleQueryCreateFromClip(szPStdFile, olerender_draw, 0) == OLE_OK || OleQueryCreateFromClip(szPStatic, olerender_draw, 0) == OLE_OK)
                mfPaste = MF_ENABLED;
...
NOEXPORT BOOL FixEditObject(HMENU hEditMenu)
{
    ...
    if (!CurCard.lpObject || CurCard.otObject == STATIC)
        return FALSE;
    if (OLE_OK != OleGetData(CurCard.lpObject, (OLECLIPFORMAT)(CurCard.otObject == LINK ? vcfLink : vcfOwnerLink), &hData))
    {
    ...
    if (lpstrData = GlobalLock(hData))
    {
        ...
        MultiByteToWideChar( CP_ACP, MB_PRECOMPOSED, (LPCSTR) lpstrData, lstrlen(lpstrData), szShortClass, CharSizeOf(szShortClass) );
...
*/
        ...
        InsertMenu(hEditMenu, MENUPOS_EDITOBJECT, MF_BYPOSITION | MF_POPUP, (UINT)hPopupNew, szMenuStr);
    }
    if (hKeyVerb)
