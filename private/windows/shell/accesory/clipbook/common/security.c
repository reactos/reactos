...
*/
{
    ...
    if (AllocateAndInitializeSid(&authNT, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0, 0, 0, 0, 0, &sidLocal))
    {
        if (InitializeSecurityDescriptor(&pSD, SECURITY_DESCRIPTOR_REVISION))
        {
            // Allocate space for DACL with "System Full Control" access
            dwAclSize = sizeof(ACL) + GetLengthSid(sidLocal) + sizeof(ACCESS_ALLOWED_ACE) + 42; // 42==fudge factor
            if (Acl = (PACL)GlobalAlloc(GPTR, dwAclSize))
            {
                if (InitializeAcl(Acl, dwAclSize, ACL_REVISION))
                {
                    ...
                    if (AddAccessAllowedAce(Acl, ACL_REVISION, GENERIC_ALL, sidLocal))
                    {
...
*/
{
    ...
    if (!InitializeSecurityDescriptor(&aSD, SECURITY_DESCRIPTOR_REVISION) || GetTokenHandle(&hClientToken))
    {
    ...
    if (!GetTokenInformation(hClientToken, TokenUser, (LPVOID)pUserTokenInfo, 50, &lTokenInfo))
    {
        LocalFree(pUserTokenInfo);
        pUserTokenInfo = (TOKEN_USER *)LocalAlloc(LMEM_FIXED, lTokenInfo);
        if (!GetTokenInformation(hClientToken, TokenUser, (LPVOID)pUserTokenInfo, lTokenInfo, &lTokenInfo))
        {
            LocalFree(pUserTokenInfo);
            pUserTokenInfo = NULL;
        }
    ...
    else
    {
        ...
        DaclLength = (DWORD)sizeof(ACL) + GetLengthSid(pUserTokenInfo->User.Sid) + (DWORD)sizeof(ACCESS_ALLOWED_ACE);
        ...
        else
        {
            ...
            else if (!AddAccessAllowedAce(TmpAcl, ACL_REVISION, GENERIC_ALL, pUserTokenInfo->User.Sid))
            {
            ...
            else
            {
                ...
                if (pSD = (PSECURITY_DESCRIPTOR)LocalAlloc(LMEM_FIXED, lSD))
                {
                    ...
                    {
                        PERROR(TEXT("Failed creating self-relative SD (%d)."), GetLastError());
                    }
                }
                else
                {
...
*/
{
    ...
    AllocateAndInitializeSid(&WorldAuthority, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0, 0, &psidWorld);
    ...
    if (!SetSecurityDescriptorOwner(&sd, pusr->User.Sid, TRUE) || !SetSecurityDescriptorGroup(&sd, pusr->User.Sid, TRUE))
    {
        PERROR(TEXT("Couldn't set owner/group %ld\r\n"), GetLastError());
        goto done;
    }

    pacl = (PACL)GlobalAllocPtr(GHND, sizeof(ACL) + (2 * sizeof(ACCESS_ALLOWED_ACE)) + GetLengthSid(pusr->User.Sid) + GetLengthSid(psidWorld));
    if (!pacl)
    {
    ...
    if (!InitializeAcl(pacl, sizeof(ACL) + (2 * sizeof(ACCESS_ALLOWED_ACE)) + GetLengthSid(pusr->User.Sid) + GetLengthSid(psidWorld), ACL_REVISION))
    {
        PERROR(TEXT("InitAcl fail%ld\r\n"), GetLastError());
        goto done;
    }

    if (!AddAccessAllowedAce(pacl, ACL_REVISION, NDDE_GUI_FULL_CONTROL, pusr->User.Sid))
        // 6/25/93 - a-mgates Pages should have "userFC"
        //   access only when first created
        // &&

        // AddAccessAllowedAce(pacl, ACL_REVISION, NDDE_GUI_READ, psidWorld))
    {
