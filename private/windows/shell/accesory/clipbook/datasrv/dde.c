...
LONG MakeTheDamnKey(PHKEY phkey, REGSAM regsam)
{
    DWORD dwIck;

    if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_LOCAL_MACHINE, szSection, 0, szRegClass, REG_OPTION_NON_VOLATILE, regsam, NULL, phkey, &dwIck))
    {
        return ERROR_SUCCESS;
    }
    else
    {
        if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, szSection, 0, regsam, phkey))
        {
            return ERROR_SUCCESS;
        }
        else
...
// Returns:
//    TRUE on success, FALSE on failure.
BOOL AddShare(LPTSTR pszName, WORD flags)
{
    ...
    else if (AddRecord(pszName, rgtchFName, flags))
    {
        ...
        if (ERROR_SUCCESS == MakeTheDamnKey(&hkeyClp, KEY_SET_VALUE))
        {
            RegSetValueEx(hkeyClp, pszName, 0, REG_SZ, rgtchFName, lstrlen(rgtchFName));
            RegCloseKey(hkeyClp);
    ...
    for (pshrinfo = SIHead; pshrinfo; pshrinfo = pshrinfo->Next)
    {
            ...
            ) == 0)
        {
                ...
                if (NDDE_NO_ERROR == ret)
                {
                        ...
                        &fDacl, &Acl, &fDefault))
                    {
                        ...
                        for (i = 0; GetAce(Acl, i, &pace); i++)
                        {
                            ...
                            // mask types
                            if (ACCESS_ALLOWED_ACE_TYPE == pace->Header.AceType || ACCESS_DENIED_ACE_TYPE == pace->Header.AceType)
                            {
                                ...
                                if ((pace->Mask & NDDE_GUI_FULL_CONTROL) == NDDE_GUI_FULL_CONTROL)
                                {
                                    PINFO(TEXT("A"));
                        ...
                        ret = SetFileSecurityW(pshrinfo->szFileName, DACL_SECURITY_INFORMATION, pSDShare);
            ...
            else
            {
                ...
                if (ERROR_SUCCESS == MakeTheDamnKey(&hkeyClp, KEY_SET_VALUE))
                {
                    PINFO(TEXT("Making registry key %ls from %ls, %d\r\n"), pshrinfo->szName, pshrinfo->szFileName, lstrlenW(pshrinfo->szFileName));
                    RegSetValueExW(hkeyClp, pshrinfo->szName, 0, REG_SZ, (LPBYTE) pshrinfo->szFileName, lstrlenW(pshrinfo->szFileName) * sizeof(WCHAR) + sizeof(WCHAR));
                    RegCloseKey(hkeyClp);
                    DdePostAdvise(idInst, hszSysTopic, hszTopicList);
                    return TRUE;
                }
                ...
                {
                    PERROR(TEXT("Could not make registry key to record %s"), pshrinfo->szName);
                }
            }
        }
    }
...
BOOL GetRandShareFileName(LPTSTR buf)
{
    TCHAR szWinDir[144];
    BOOL IsUnique = FALSE;
    WORD rand;
