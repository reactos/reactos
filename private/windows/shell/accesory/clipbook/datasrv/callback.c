...
DWORD   dwIck;
    ...
    dwR = RegCreateKeyEx (HKEY_LOCAL_MACHINE, szSection, 0, szRegClass, REG_OPTION_NON_VOLATILE, regsam, NULL, phkey, &dwIck);
...
int lstrncmp(LPTSTR s1, LPTSTR s2, WORD count )
{
unsigned i;
register TCHAR tch1;
register TCHAR tch2;
...
static VOID    AddXactErr(HCONV   hConv)
/*
...
static VOID DelXactErr(HCONV   hConv)
/*
...
DWORD   GetXactErrType(HCONV   hConv)
/*
...
BOOL    CheckPageName (LPTSTR  ComputerName, LPTSTR  Name)
/*
...
unsigned iKeys = 0;
    ...
    do  {
        ...
        dwR = RegEnumValue (hkeyClp, iValue, rgtchPageName, &dwNameSize, NULL, &dwType, (LPBYTE)rgtchPageFile, &dwPageSize);
        if (dwR == ERROR_SUCCESS)
            {
            ...
            // this does not work in 548
                ...
                AddRecord (rgtchPageName, rgtchPageFile, (SHR_CHAR == rgtchPageName[0]) ? SIF_SHARED : 0);
            }
