...
void HexDumpBytes(char        *pv, unsigned    cb)
{
    ...
    if (pacl)
    {
        if (GetAclInformation(pacl, &aclsi, sizeof(aclsi), AclSizeInformation))
        {
            for (i = 0; i < aclsi.AceCount; i++)
            {
                ...
                PINFO(TEXT("Type(%x) Flags(%x) Access(%lx)\r\nSID:"), (int)pace->Header.AceType, (int)pace->Header.AceFlags, pace->Mask);
...
void PrintSD(PSECURITY_DESCRIPTOR    pSD)
{
    DWORD   dwRev;
    WORD    wSDC;
    BOOL    fDefault, fAcl;
    ...
    {
        PINFO(TEXT("SD - Length: [%ld] Control: [%x] [%lx]\r\nGroup:"), GetSecurityDescriptorLength(pSD), wSDC, dwRev);
    }
    else
    {
    ...
    if (GetSecurityDescriptorGroup(pSD, &sid, &fDefault) && sid && IsValidSid(sid))
    {
        PrintSid(sid);
    ...
    if (GetSecurityDescriptorOwner(pSD, &sid, &fDefault) && sid && IsValidSid(sid))
    {
        PrintSid(sid);
