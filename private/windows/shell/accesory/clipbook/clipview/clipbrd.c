...
int WINAPI WinMain(HINSTANCE   hInstance, HINSTANCE   hPrevInstance, LPSTR lpCmdLine, int  nCmdShow)
{
    ...
    if (OpenFile(lpCmdLine, &of, OF_EXIST) != HFILE_ERROR)
    {
        if (ClearClipboard(hwndApp))
        {
            ...
            MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, lpCmdLine, -1, ptch, lstrlenA(lpCmdLine) + 1);
            lstrcat(lstrcpy(szBuf, SZCMD_OPEN), ptch);
#else
            lstrcat(lstrcpy(szBuf, SZCMD_OPEN), lpCmdLine);
#endif

            MySyncXact((LPBYTE) szBuf, lstrlen(szBuf) + 1, GETMDIINFO(hwndLocal)->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, SHORT_SYNC_TIMEOUT, NULL);
    ...
    {
        if (!TranslateMDISysAccel(hwndMDIClient, &msg) && (hAccel ? !TranslateAccelerator(hwndApp, hAccel, &msg) : 1))
        {
...
static void SetupForFloatingProfile ()
{
    ...
    NDdeShareAdd(szComputerName, 2, NULL, (LPBYTE) &ShareInfo, sizeof(ShareInfo));
    NDdeSetTrustedShare(szComputerName, szClpBookShare, NDDE_TRUST_SHARE_START | NDDE_TRUST_SHARE_INIT);
}
...
BOOL InitInstance(HINSTANCE   hInstance, int nCmdShow)
{
    ...
    if (PRIMARYLANGID(LANGIDFROMLCID(lcid)) == LANG_JAPANESE || PRIMARYLANGID(LANGIDFROMLCID(lcid)) == LANG_CHINESE || PRIMARYLANGID(LANGIDFROMLCID(lcid)) == LANG_KOREAN)
    {
    ...
    // which is called when DDEML is in a modal loop during synchronous transaction processing.
    lpMsgFilterProc = (HOOKPROC) MyMsgFilterProc;
    hMsgFilterHook = SetWindowsHookEx(WH_MSGFILTER, lpMsgFilterProc, hInst, GetCurrentThreadId());
    ...
    if (ERROR_SUCCESS != RegCreateKeyEx(HKEY_CURRENT_USER, szRoot, 0L, NULL, REG_OPTION_NON_VOLATILE, KEY_QUERY_VALUE | KEY_SET_VALUE | KEY_ENUMERATE_SUB_KEYS, NULL, &hkeyRoot, &dwKeyStatus))
    {
    ...
    else
    {
        ...
        if (ERROR_SUCCESS != RegQueryValueEx(hkeyRoot, szStatusbar, NULL, NULL, (LPBYTE) &fStatus, &iSize))
        {
            fStatus = TRUE;
        }

        if (ERROR_SUCCESS != RegQueryValueEx(hkeyRoot, szToolbar, NULL, NULL, (LPBYTE) &fToolBar, &iSize))
        {
            fToolBar = TRUE;
        }

        if (ERROR_SUCCESS != RegQueryValueEx(hkeyRoot, szShPref, NULL, NULL, (LPBYTE) &fSharePreference, &iSize))
        {
            fSharePreference = FALSE;
        }

#if DEBUG
        if (ERROR_SUCCESS != RegQueryValueEx(hkeyRoot, szDebug, NULL, NULL, (LPBYTE) &DebugLevel, &iSize))
        {
            DebugLevel = 2;
        }
#endif
...
VOID StripCharRange (TCHAR   *s, char    lower, char    upper)
{
    TCHAR *p = s, *q = s;
...
VOID StripAcceleratorKey (TCHAR   *s)
{
TCHAR *p = s, *q = s;
...
void OnDrawClipboard(HWND    hwnd)
{
    ...
    else
    {
        ...
        {
            wOldFormat = GetBestFormat(hwndClpbrd, GETMDIINFO(hwndClpbrd)->CurSelFormat);
...
LRESULT CALLBACK FrameWndProc(HWND    hwnd, UINT    msg, WPARAM  wParam, LPARAM  lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        // a flat DDEML data handle - not the regular DDE interpretation of a handle that carries those format IDs
        ...
        hwndMDIClient = CreateWindow("mdiclient", NULL, WS_BORDER | WS_CHILD | WS_CLIPCHILDREN | MDIS_ALLCHILDSTYLES | WS_HSCROLL | WS_VSCROLL, 0, 0, 0, 0, hwnd, (HMENU) 0xCAC, hInst, (LPVOID) &ccs);
    ...
    case WM_MENUSELECT:
        ...
        else
        {
            ...
            if (wParam >= 4100 && wParam <= 4200)
            {
                ...
                wsprintf(szBuf, szActivateFmt, *szBuf2 == ' ' ? (LPSTR) (szBuf2 + 1) : (LPSTR) szBuf2);
    ...
    case WM_DESTROY:
        ...
        if (hkeyRoot != NULL)
        {
            RegSetValueEx(hkeyRoot, szStatusbar, 0L, REG_DWORD, (LPBYTE) &fStatus, sizeof(fStatus));
            RegSetValueEx(hkeyRoot, szToolbar, 0L, REG_DWORD, (LPBYTE) &fToolBar, sizeof(fToolBar));
            RegSetValueEx(hkeyRoot, szShPref, 0L, REG_DWORD, (LPBYTE) &fSharePreference, sizeof(fSharePreference));
        }
    ...
    case WM_RENDERFORMAT:
    {
        ...
        hListData = MySyncXact(NULL, 0L, GETMDIINFO(hwndClpOwner)->hClpConv, hszFormatList, CF_TEXT, XTYP_REQUEST, SHORT_SYNC_TIMEOUT, NULL);
        if (!hListData && !fAppShuttingDown)
        {
            uiErr = DdeGetLastError(idInst);
            PERROR(TEXT("WM_RENDERFORM: REQUEST for formatlist failed: %x\n\r"), uiErr);
            MessageBoxID(hInst, hwnd, IDS_DATAUNAVAIL, IDS_APPNAME, MB_OK | MB_ICONEXCLAMATION);
            break;
        }
        ...
        for (q = strtokA(lpszList, "\t"); q; q = strtokA(NULL, "\t"))
        {
            ...
            if (wParam == RealFmt || msg == WM_RENDERALLFORMATS)
            {
                ...
                hFmtData = MySyncXact(NULL, 0L, GETMDIINFO(hwndClpOwner)->hClpConv, hszFmt, MyGetFormat(q, GETFORMAT_LIE), XTYP_REQUEST, LONG_SYNC_TIMEOUT, NULL);
                if (hFmtData)
                {
        ...
        if (wParam == CF_BITMAP && !hFmtData)
        {
            ...
            hFmtData = MySyncXact(NULL, 0L,  GETMDIINFO(hwndClpOwner)->hClpConv, hszFmt, MyGetFormat(szName, GETFORMAT_LIE), XTYP_REQUEST, LONG_SYNC_TIMEOUT, NULL);
            if (hFmtData)
...
LRESULT CALLBACK ChildWndProc(HWND    hwnd, UINT    msg, WPARAM  wParam, LPARAM  lParam)
{
    ...
    case WM_SETFOCUS:
        ...
        if (pActiveMDI->DisplayMode != DSP_PAGE && IsWindow(pActiveMDI->hWndListbox))
        {
    ...
    case WM_LBUTTONDBLCLK:
        ...
        if (GETMDIINFO(hwnd)->DisplayMode == DSP_PAGE && !(GETMDIINFO(hwnd)->flags & F_CLPBRD))
        {
    ...
    case WM_QUERYNEWPALETTE:
        ...
        if (VOpenClipboard(GETMDIINFO(hwnd)->pVClpbrd, hwnd))
        {
            ...
            {
                hdc = GetDC(hwnd);
                hOldPal = SelectPalette(hdc, hCurrentPal, (msg == WM_QUERYNEWPALETTE) ? FALSE : TRUE);
        ...
        break;
    case WM_MENUSELECT:
        MenuHelp((WORD) msg, wParam, lParam, GetMenu(hwndApp), hInst, hwndStatus, nIDs);
        break;
    case WM_CREATE:
        ...
        pMDI->hWndListbox = CreateWindow(TEXT("listbox"), szNull, WS_CHILD | LBS_STANDARD | LBS_NOINTEGRALHEIGHT | LBS_LISTVIEW, 0, 0, 100,
            100,   // non zero size to work around
            // possible USER bug which results
            // in divide by zero
            hwnd, (HMENU) ID_LISTBOX, hInst, 0L);

        // create the scroll bars
        pMDI->hwndVscroll = CreateWindowW(L"scrollbar", L"", WS_CHILD | SBS_VERT, 0, 0, 0, 0, hwnd, (HMENU) ID_VSCROLL, hInst, 0L);

        pMDI->hwndHscroll = CreateWindowW(L"scrollbar", L"", WS_CHILD | SBS_HORZ, 0, 0, 0, 0, hwnd, (HMENU) ID_VSCROLL, hInst, 0L);

        // create the stupid corner size box
        pMDI->hwndSizeBox = CreateWindowW(L"scrollbar", L"", WS_CHILD | SBS_SIZEBOX, 0, 0, 0, 0, hwnd, (HMENU) ID_SIZEBOX, hInst, 0L);

        // create the page fwd/bkwd buttons
        pMDI->hwndPgUp = CreateWindowW(L"button", L"", WS_CHILD | BS_OWNERDRAW, 0, 0, 0, 0, hwnd, (HMENU) ID_PAGEUP, hInst, 0L);

        pMDI->hwndPgDown = CreateWindowW(L"button", L"", WS_CHILD | BS_OWNERDRAW,  0, 0, 0, 0, hwnd, (HMENU) ID_PAGEDOWN, hInst, 0L);
    ...
    case WM_COMPAREITEM:
        ...
        tmp = lstrcmpi(&((LPLISTENTRY) ((LPCOMPAREITEMSTRUCT) lParam)->itemData1)->name[1], &((LPLISTENTRY) ((LPCOMPAREITEMSTRUCT) lParam)->itemData2)->name[1]);
        if (tmp < 0)
        {
            return -1;
        }
...
/*
 *  PURPOSE    : Send the given message with the given parameters to all of the MDI child windows.
*/
