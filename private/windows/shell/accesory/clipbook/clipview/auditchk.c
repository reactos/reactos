...
// Purpose: Tests, enables, or disables the Security privilege, which allows auditing to take place.
...
BOOL AuditPrivilege(int fAudit)
{
    HANDLE              hToken;
    LUID                SecurityValue;
    ...
    {
        /*
         * Enable the SE_SECURITY_NAME privilege or disable all privileges, depending on the fEnable flag.
         */
        if (LookupPrivilegeValue((LPSTR) NULL, SE_SECURITY_NAME, &SecurityValue))
        {
            ...
            {
                AdjustTokenPrivileges(hToken, TRUE, NULL, 0L, (PTOKEN_PRIVILEGES) NULL, (PDWORD) NULL);
                if (ERROR_SUCCESS == GetLastError() && AUDIT_PRIVILEGE_OFF == fAudit)
                {
                    fOK = TRUE;
                }
            }
