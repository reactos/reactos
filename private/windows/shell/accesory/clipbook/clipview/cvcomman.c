...
LRESULT OnIDMDelete(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    if (MessageBox(hwndApp, szBuf, szDelete, MB_ICONEXCLAMATION | MB_OKCANCEL) != IDCANCEL)
    {
        ...
        if (MySyncXact(szBuf, lstrlen(szBuf) + 1, pActiveMDI->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, SHORT_SYNC_TIMEOUT, NULL))
        {
...
*/
LRESULT OnIDMKeep(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, BOOL bNewPage)
{
    PMDIINFO        pMDI;
    int             tmp;
    DWORD           ret;
    ...
    if (bNewPage)
    {
        ...
        ret = (DWORD) DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_KEEPASDLG), hwnd, KeepAsDlgProc, (LPARAM) &KeepAs);
    ...
    if (!bNewPage || KeepAs.bAlreadyExist)
    {
        ...
        if (!bNewPage)
            ...
            tmp = (int) SendMessage(pMDI->hWndListbox, LB_FINDSTRING, (WPARAM) -1, (LPARAM) (LPCSTR) KeepAs.ShareName);

        if (LB_ERR == tmp)
            goto done;

        SendMessage(pMDI->hWndListbox, LB_GETTEXT, tmp, (LPARAM) &lpLE);
    ...
    if (KeepAs.bAlreadyExist && KeepAs.bAlreadyShared)
    {
        DWORD   dwAddItem = 0;
        DWORD   dwTotal;
        ret = NDdeShareGetInfo(rgtchCName, KeepAs.ShareName, 2, (PUCHAR) lpDdeInfo, 2048 * sizeof(TCHAR), &dwTotal, (PUSHORT) &dwAddItem);
    }
    else
    {
    ...
    *lpEnd = SHR_CHAR;
    ...
    if (OpenClipboard(hwnd))
    {
        ...
        if ((hData = VGetClipboardData(NULL, cf_link)) && (lpData = GlobalLock(hData)))
        {
        ...
        if ((hData = VGetClipboardData(NULL, cf_objectlink)) && (lpData = GlobalLock(hData)))
        {
    ...
    *lpEnd++ = TEXT('\0');
    ...
    if (!KeepAs.bAlreadyExist)
    {
        ...
        ret = NDdeSetTrustedShare(rgtchCName, lpDdeInfo->lpszShareName, NDDE_TRUST_SHARE_INIT);
        if (ret != NDDE_NO_ERROR)
            NDdeMessageBox(hInst, hwnd, ret, IDS_APPNAME, MB_OK | MB_ICONSTOP);
    }
    else
    {
        ret = NDdeShareSetInfo(rgtchCName, lpDdeInfo->lpszShareName, 2, (LPBYTE) lpDdeInfo, sizeof(NDDESHAREINFO), 0);
    ...
    // NOTE must force all formats rendered to prevent deadlock on the clipboard.
    ForceRenderAll(hwnd, NULL);

    lstrcat(lstrcpy(szBuf, SZCMD_PASTE), KeepAs.ShareName);
    AssertConnection(hwndLocal);
    if (!MySyncXact((LPBYTE) szBuf, lstrlen(szBuf) + 1, pMDI->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, LONG_SYNC_TIMEOUT, NULL))
    {
        ...
        if (!KeepAs.bAlreadyExist)
        {
            ...
            MySyncXact(szBuf, lstrlen(szBuf) + 1, pMDI->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, SHORT_SYNC_TIMEOUT, NULL);

            // and we'll delete the rest.
            NDdeSetTrustedShare(rgtchCName, lpDdeInfo->lpszShareName, NDDE_TRUST_SHARE_DEL);
            NDdeShareDel(rgtchCName, lpDdeInfo->lpszShareName, 0);
            goto done;
        }
    }
    ...
    // update the list box in all cases, the function is smart enough to figure out which item has changed and update only it.
...
*/
LRESULT OnIDMCopy(HWND    hwnd, UINT    msg, WPARAM  wParam, LPARAM  lParam)
{
    LPLISTENTRY lpLE;
    PMDIINFO    pMDIc;
    PDATAREQ    pDataReq;
    ...
    if (SendMessage(pMDIc->hWndListbox, LB_GETTEXT, tmp, (LPARAM) (LPCSTR) &lpLE) == LB_ERR)
    {
    ...
    pMDIc->hClpConv = DdeConnect(idInst, pMDIc->hszConvPartner, pMDIc->hszClpTopic, NULL);
    if (!pMDIc->hClpConv)
    {
        PERROR(TEXT("DdeConnect to (%s) failed %d\n\r"), (LPSTR) (lpLE->name), DdeGetLastError(idInst));
        MessageBoxID(hInst, hwndActiveChild, IDS_DATAUNAVAIL, IDS_APPNAME, MB_OK | MB_ICONEXCLAMATION);
        goto done;
    }
    ...
    if (!DdeClientTransaction(NULL, 0L, pMDIc->hClpConv, hszFormatList, CF_TEXT, XTYP_REQUEST, (DWORD) TIMEOUT_ASYNC, NULL))
        DdeMessageBox(hInst, hwndApp, DdeGetLastError(idInst), IDS_APPNAME, MB_OK | MB_ICONEXCLAMATION);
...
static void CreateLocalWindow(void)
{
    ...
    pMDI->hszConvPartner = pMDI->hszConvPartnerNP = hszDataSrv;
    pMDI->hExeConv = InitSysConv(hwndLocal, pMDI->hszConvPartner, hszSystem, TRUE);
    if (!pMDI->hExeConv)
        goto error;
...
void UnsharePage (void)
{
    ...
    wAddlItems = 0;
    ret = NDdeShareGetInfo(NULL, lpLE->name, 2, (LPBYTE) lpDdeI, 2048 * sizeof(TCHAR), &dwRet, &wAddlItems);
    if (NDDE_ACCESS_DENIED == ret)
    {
    ...
    else
    {
        ...
        // BUGBUG: TEXT('|') should == BAR_CHAR. If not, this needs to be adjusted.
        while (*lpOog++ != TEXT('|'));
        ...
        *lpOog = UNSHR_CHAR;
        ...
        ret = NDdeShareSetInfo(NULL, lpLE->name, 2, (LPBYTE) lpDdeI, 2048 * sizeof(TCHAR), 0);
        if (NDDE_NO_ERROR == ret)
        {
#if 0
            ...
            pSD = atch;
            if (NDDE_NO_ERROR == NDdeGetShareSecurity(NULL, lpLE->name, DACL_SECURITY_INFORMATION, pSD, 2048 * sizeof(TCHAR), &dwSize))
            {
            ...
            if (MySyncXact((LPBYTE) szBuf, lstrlen(szBuf) + 1, GETMDIINFO(hwndLocal)->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, SHORT_SYNC_TIMEOUT, NULL))
            {
 ...
 *  dwItem is the item number to unshare.  If == LB_ERR then the current selected item will be unshared.
...
LRESULT OnIdmUnshare (DWORD dwItem)
{
    ...
    SendMessage(pActiveMDI->hWndListbox, LB_GETTEXT, dwItem, (LPARAM) (LPCSTR) &lpLE);
    ...
    *lpOog = UNSHR_CHAR;
    ...
    // Have to get trusted share settings before we modify the share, 'cuz they'll be invalid.
    NDdeGetTrustedShare(NULL, lpDdeI->lpszShareName, adwTrust, adwTrust + 1, adwTrust + 2);

    DumpDdeInfo(lpDdeI, NULL);
    ret = NDdeShareSetInfo(NULL, lpDdeI->lpszShareName, 2, (LPBYTE) lpDdeI, 2048 * sizeof(TCHAR), 0);
    if (NDDE_ACCESS_DENIED == ret)
    {
        MessageBoxID(hInst, hwndApp, IDS_PRIVILEGEERROR, IDS_APPNAME, MB_OK | MB_ICONHAND);
        return 0L;
    ...
    {
        NDdeMessageBox(hInst, hwndApp, ret, IDS_APPNAME, MB_OK | MB_ICONHAND);
    ...
    // Setting trusted share info needs to be the last operation we do on the share.
...
LRESULT ClipBookCommand (HWND   hwnd, UINT   msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_PERMISSIONS:
    {
        ...
        // Permissions may have changed.  Get old data, they need to be updated.
    ...
    case ID_PAGEDOWN:
    {
        ...
        iLstbox = (int) SendMessage(pMDIc->hWndListbox, LB_GETCURSEL, 0, 0L);
        if (iLstbox == LB_ERR)
            break;
        ...
        if ((int) iLstbox == (int) SendMessage(pMDIc->hWndListbox, LB_GETCOUNT, 0, 0L) - 1 && wParam == (WPARAM) ID_PAGEDOWN)
        {
            MessageBeep(0);
            break;
        }
    ...
    case IDM_PREVIEWS:
    {
        ...
        pActiveMDI->DisplayMode = (wParam == IDM_PREVIEWS) ? DSP_PREV : DSP_LIST;// set new display mode so listbox will get created right
        hwndtmp = pActiveMDI->hWndListbox; // save handle to old listbox
        ShowWindow(hwndtmp, SW_HIDE);// hide the old listbox - will soon destroy
        pActiveMDI->hWndListbox = CreateNewListBox(hwndActiveChild, (pActiveMDI->DisplayMode == DSP_PREV) ? LBS_PREVIEW : LBS_LISTVIEW);// make new listbox and save handle in extra window data

        // loop, extracting items from one box and into other
        while (SendMessage(hwndtmp, LB_GETTEXT, 0, (LPARAM) (LPCSTR) &lpLE) != LB_ERR)
        {
            lpLE->fDelete = FALSE;// mark this item not to be deleted in WM_DELETEITEM
            SendMessage(hwndtmp, LB_DELETESTRING, 0, 0L);// remove from listbox
            lpLE->fDelete = TRUE;// reset fDelete flag
            SendMessage(pActiveMDI->hWndListbox, LB_ADDSTRING, 0, (LPARAM) (LPCSTR) lpLE);// add to new listbox
        }
    ...
    case IDM_PASTE_PAGE:
    case IDM_KEEP:
        bRet = (BOOL) OnIDMKeep(hwnd, msg, wParam, lParam, IDM_KEEP == LOWORD(wParam) /* a new page? */);
        return bRet;
        break;
    case IDM_SAVEAS:
    {
        ...
        if (CountClipboardFormats())
        {
            ...
            if (GetSaveFileName(&ofn) && szFile[0])
            {
                ...
                wsprintfA(szBuf, "%s%s", (ofn.nFilterIndex == 1) ? (LPSTR) SZCMD_SAVEAS : (LPSTR) SZCMD_SAVEASOLD, (LPSTR) szFile);
    ...
    case IDM_OPEN:
    {
        ...
        if (GetOpenFileName(&ofn) && szFile[0])
        {
            ...
            if (ClearClipboard(hwnd))
            {
                ...
                // these two lines replaces the follow if() else so we can open files directly without going to the server.
...
*/
        ...
        *szConvPartner = '\0';
        ...
        if (hMod = LoadLibraryW(L"NTLANMAN.DLL"))
        {
            if (lpfn = (LPFNSYSFOCUS) GetProcAddress(hMod, "I_SystemFocusDialog"))
            {
                ...
                fFoundLMDlg = TRUE;
                (*lpfn)(hwnd,
                    FOCUSDLG_BROWSE_LOGON_DOMAIN | FOCUSDLG_BROWSE_WKSTA_DOMAIN | FOCUSDLG_BROWSE_OTHER_DOMAINS | FOCUSDLG_BROWSE_TRUSTING_DOMAINS | FOCUSDLG_BROWSE_WORKGROUP_DOMAINS | FOCUSDLG_SERVERS_ONLY,
                    rgwch,
                ...
                {
#ifndef UNICODE
                    WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK | WC_DISCARDNS, rgwch, -1, szConvPartner, MAX_COMPUTERNAME_LENGTH + 2, NULL, &bOK);
#else
        ...
        if (!fFoundLMDlg)
        {
            bOK = (BOOL) DialogBox(hInst, MAKEINTRESOURCE(IDD_CONNECT), hwnd, ConnectDlgProc);
        }
        ...
        {
            MessageBoxID(hInst, hwnd, IDS_NOCONNECTION, IDS_APPNAME, MB_OK | MB_ICONHAND);
        }
        UpdateWindow(hwnd);
        break;
    }
    ...
    case IDM_ABOUT:
    {
        ...
        if (hMod = LoadLibrary(TEXT("SHELL32")))
        {
            ...
            {
                (*lpfn)(hwnd, szAppName, szNull, LoadIcon(hInst, MAKEINTRESOURCE(IDFRAMEICON)));
            }
            FreeLibrary(hMod);
        }
        else
...
BOOL SetListboxEntryToPageWindow(HWND hwndc, PMDIINFO pMDIc, int lbindex)
{
    ...
    if (LB_ERR == SendMessage(pMDIc->hWndListbox, LB_GETTEXT, lbindex, (LPARAM) (LPCSTR) &lpLE) || !lpLE || !(pDataReq = CreateNewDataReq()))
    {
        PERROR(TEXT("error from CreateNewDataReq\n\r"));
        goto done;
    }
    ...
    if (!DdeClientTransaction(NULL, 0L, hConv, hszFormatList, CF_TEXT, XTYP_REQUEST, (DWORD) TIMEOUT_ASYNC, NULL))
        DdeMessageBox(hInst, pDataReq->hwndMDI, DdeGetLastError(idInst), IDS_APPNAME, MB_OK | MB_ICONEXCLAMATION);
