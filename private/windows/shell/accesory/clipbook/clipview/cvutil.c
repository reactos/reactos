    ...
    if (LockApp (TRUE, szEstablishingConn))
        ...
        if (!hConv)
            ...
            if ( pDataReq = CreateNewDataReq() )
                ...
                if (!MySyncXact ( NULL, 0L, hConv, hszTopics, CF_TEXT, XTYP_ADVSTART, LONG_SYNC_TIMEOUT, NULL ))
                    {
...
*/
void    RequestXactError(HCONV   hConv)
{
HDDEDATA    hDDE;
BOOL        fAlreadyLocked;
UINT        uiErr;
...
DWORD   dwSize = 20;
    ...
    dwR = FormatMessage (FORMAT_MESSAGE_ALLOCATE_BUFFER| FORMAT_MESSAGE_FROM_SYSTEM,
                         NULL,
                         dwErr,
                         0,
