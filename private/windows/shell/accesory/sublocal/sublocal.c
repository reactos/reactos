...
BOOL EnablePrivilege(IN PTSTR PrivilegeName)
{
    HANDLE Token;
    BOOL b;
...
RESULTCODE DoNewUserDef(VOID)
{
    ...
    // Form the full pathname of the userdef hive and a temporary filename.
    ...
    // Note that GetTempFileName creates the temp file and leaves it there so we have to use MoveFileEx to replace it.
...
RESULTCODE PatchTurkishDosKeybIDs(VOID)
{
    DWORD   dwSize, dwType;
    int     rc;
    HANDLE  hkey;
    ...
    if (!RegOpenKeyEx(HKEY_LOCAL_MACHINE, szKey, 0L, KEY_WRITE, (PHKEY) &hkey)) {
        ...
        // rc = RegQueryValueEx (hkey, TEXT("0000041F"), NULL, &dwType, (LPBYTE)szX0000041F, &dwSize);
        rc = RegSetValueEx(hkey, TEXT("0000041F"), 0L, REG_SZ, (LPBYTE) TEXT("179"), (lstrlen(TEXT("179")) + 1)*sizeof(TCHAR));
        if (rc != 0) {
            RegCloseKey(hkey);
            printf("Error = %d\n", GetLastError());
        ...
        // rc = RegQueryValueEx (hkey, TEXT("0001041F"), NULL, &dwType, (LPBYTE)szX0001041F, &dwSize);
        rc = RegSetValueEx(hkey, TEXT("0001041F"), 0L, REG_SZ, (LPBYTE) TEXT("440"), (lstrlen(TEXT("440")) + 1)*sizeof(TCHAR));
        RegCloseKey(hkey);
        if (rc != 0) {
            printf("Error = %d\n", GetLastError());
            return ResultTurkishKeyboardFailure;
...
int tmain(IN int   argc, IN PTSTR argv [])
{
    RESULTCODE result;
    ...
    if (!lstrcmpi(argv[1] + 1, TEXT("winini"))) {
        result = (argc == 5) ? (WriteProfileString(argv[2], argv[3], argv[4]) ? ResultSuccess : ResultWinIniFailure) : ResultInvalidArgs;
