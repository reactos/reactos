...
INT_PTR CALLBACK AbortDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        SetDlgItemText(hwnd, ID_FILENAME, fUntitled ? szUntitled : PFileInPath(szFileName));
        SetFocus(hwnd);
        return(TRUE);
    case WM_INITMENU:
...
HDC GetPrinterDCviaDialog(VOID)
{
    ...
    pdTemp.Flags = PD_NOPAGENUMS | PD_RETURNDC | PD_NOCURRENTPAGE | PD_NOSELECTION | 0;
...
INT NpPrintGivenDC(HDC hPrintDC)
{
...
#ifdef DBGPRINT
    {
        ...
        wsprintf(dbuf, TEXT("Print pOffx %d  pOffy %d\n"), GetDeviceCaps(hPrintDC, PHYSICALOFFSETX), GetDeviceCaps(hPrintDC, PHYSICALOFFSETY));
        ODS(dbuf);
        ...
        ODS(dbuf);
        wsprintf(dbuf, TEXT("PaperSize  x: %d  y: %d\n"), g_PageSetupDlg.ptPaperSize.x, g_PageSetupDlg.ptPaperSize.y);
        ODS(dbuf);
    ...
    */
    ...
    hAbortDlgWnd = CreateDialog(hInstanceNP, (LPTSTR)MAKEINTRESOURCE(IDD_ABORTPRINT), hwndNP, AbortDlgProc);
    if (!hAbortDlgWnd) {
        goto ErrorExit;
    }
    ...
    while (!fAbort && LineNum < wpNumLines) {
        ...
        do                          // till lpLine == pLineEOL
        {
            ...
            } else                          // first character not a tab
            {
                ...
                GetTextExtentExPoint(hPrintDC, lpLine, pNextLine - lpLine, nPixelsLeft, &guess, NULL, &Size);
                if (guess) {
    ...
    // the header and footer area.
    {
        ...
        while (!fAbort && (iTextLeft > 0)) {
            ...
            iSta = DrawTextEx(hPrintDC, pStartText, iTextLeft, &rect, dwDTFormat, &dtParm);
            PrintHeaderFooter(hPrintDC, FOOTER);
            if (EndPage(hPrintDC) <= 0) {
                iErr = GetLastError();
                goto ExitWithThisError;
...
VOID TranslateString(TCHAR * src)
{
    ...
    while (*src)   /* look at all of source */
    {
        ...
        if (*src == TEXT('&'))   /* is it the escape char? */
        {
            ...
            if (*src == letters[0] || *src == letters[1]) {                      /* &f file name (no path) */
                if (!fUntitled) {
