...
VOID PASCAL SetPageSetupDefaults( VOID )
{
    ...
    g_PageSetupDlg.Flags= PSD_MARGINS  | PSD_ENABLEPAGESETUPHOOK | PSD_ENABLEPAGESETUPTEMPLATE;
...
UINT_PTR APIENTRY NpSaveDialogHookProc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
    INT id;
    POINT pt;
    ...
    switch( msg )
    {
        ...
        case WM_HELP:
            ...
            WinHelp(((LPHELPINFO) lParam)-> hItemHandle,szHelpFile,HELP_WM_HELP,(ULONG_PTR) s_SaveAsHelpIDs);
            return TRUE;
        case WM_CONTEXTMENU:
            ...
            WinHelp( (HWND)   wParam,szHelpFile,HELP_CONTEXTMENU,(ULONG_PTR) s_SaveAsHelpIDs);
            return TRUE;
    }
...
INT NPCommand(HWND     hwnd,WPARAM   wParam,LPARAM   lParam )
{
    HWND     hwndFocus;
    LONG     lSel;
    ...
    switch (LOWORD(wParam))
    {
        ...
        case M_GOTO:
            {
                ...
                Result= (INT)DialogBox( hInstanceNP, MAKEINTRESOURCE(IDD_GOTODIALOG), hwndNP, GotoDlgProc );

                // move cursor only if ok pressed and line number ok
                if( Result == 0 )
                {
                    UINT CharIndex;
                    CharIndex= (UINT) SendMessage( hwndEdit,EM_LINEINDEX,lGotoLine-1,0 );
                    if( CharIndex != (UINT) -1 )
                    {
        ...
        case M_WW:
            ...
            {
                MessageBox(hwndNP, szNoWW, szNN, MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION);
            }
            break;
        case ID_EDIT:
            break;
        ...
        case M_PAGESETUP:
            ...
            else
            {
                ...
                if( rc == PDERR_PRINTERNOTFOUND || rc == PDERR_DNDMMISMATCH    || rc == PDERR_DEFAULTDIFFERENT )
                  {
                      FreeGlobal();
        ...
        case M_SETFONT:
        {
            ...
            cf.Flags = CF_INITTOLOGFONTSTRUCT | CF_SCREENFONTS         |  CF_NOVERTFONTS         | 0;
...
*/
        ...
        case WM_ACTIVATEAPP:
             ...
             */
                ...
                {
                   SendMessage( hwndEdit, EM_SETSEL, dwCurrentSelectionStart, dwCurrentSelectionEnd );
                   SendMessage( hwndEdit, EM_SCROLLCARET, 0, 0 );
                }
            }
            else
            {
                SendMessage( hwndEdit, EM_GETSEL, (WPARAM) &dwCurrentSelectionStart, (LPARAM) &dwCurrentSelectionEnd);
            ...
            break;
        case WM_ACTIVATE:
            if ((LOWORD(wParam) == WA_ACTIVE       || LOWORD(wParam) == WA_CLICKACTIVE) && !IsIconic(hwndNP))
               {
        ...
        case WM_COMMAND:
            if ((HWND)(lParam) == hwndEdit && (HIWORD(wParam) == EN_ERRSPACE || HIWORD(wParam) == EN_MAXTEXT))
            {
 ...
 *                 10 July 1991    ClarkC
*/
    if ((FARPROC) lpfnRegisterPenApp = GetProcAddress((HINSTANCE)(INT_PTR)(GetSystemMetrics(SM_PENWINDOWS)), "RegisterPenApp"))
    ...
    while (GetMessage((LPMSG)&msg, (HWND)NULL, 0, 0))
    {
        ...
        if (msg.message == WM_INPUTLANGCHANGEREQUEST) {
            // WM_INPUTLANGCHANGE will be *sent* to WndProc, so there's no chance to catch WM_INPUTLANGCHANGE from the frame window.
...
INT_PTR CALLBACK GotoDlgProc(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_HELP:
            WinHelp(((LPHELPINFO) lParam)-> hItemHandle, szHelpFile, HELP_WM_HELP, (ULONG_PTR) (LPVOID) s_GotoHelpIDs);
            break;
        case WM_CONTEXTMENU:
            WinHelp((HWND) wParam, szHelpFile, HELP_CONTEXTMENU, (ULONG_PTR) (LPVOID) s_GotoHelpIDs);
            break;
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
