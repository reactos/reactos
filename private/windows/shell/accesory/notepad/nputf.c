...
INT IsInputTextUnicode  (LPSTR lpstrInputStream, INT iLen)
{
    ...
    if (bUnicode && ((iResult & IS_TEXT_UNICODE_STATISTICS)    != 0 ) && ((iResult & (~IS_TEXT_UNICODE_STATISTICS)) == 0 )    )
    {
        CPINFO cpiInfo;
        CHAR* pch= (CHAR*)lpstrInputStream;
        INT  cb;

        // If the result depends only upon statistics, check to see if there is a possibility of DBCS.
