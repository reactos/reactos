...
int CFormatBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    if (!m_comboFontName.Create(WS_TABSTOP | WS_VISIBLE | WS_TABSTOP | WS_VSCROLL | CBS_DROPDOWN | CBS_SORT | CBS_AUTOHSCROLL | CBS_HASSTRINGS |
...
void CFontComboBox::EnumFontFamiliesEx(CDC& dc, BYTE nCharSet)
{
    ...
    if (dc.m_hDC != NULL)
    {
        ...
        {
            ::EnumFontFamilies(dc.m_hDC, NULL, (FONTENUMPROC)EnumFamPrinterCallBack, (LPARAM)this);
        }
    }
    else
    {
        ...
        {
            ::EnumFontFamilies(hDC, NULL, (FONTENUMPROC)EnumFamScreenCallBack, (LPARAM)this);
        }
    }
...
void CFontComboBox::AddFont(ENUMLOGFONT* pelf, DWORD dwType, LPCTSTR lpszScript)
{
    ...
    CFontDesc* pDesc = new CFontDesc(lf.lfFaceName, lpszScript, lf.lfCharSet, lf.lfPitchAndFamily, dwType);
    m_arrayFontDesc.Add(pDesc);
}


BOOL CALLBACK AFX_EXPORT CFontComboBox::EnumFamScreenCallBack(ENUMLOGFONT* pelf, NEWTEXTMETRICEX* /*lpntm*/, int FontType, LPVOID pThis)
{
    // don't put in non-printer raster fonts
...
BOOL CALLBACK AFX_EXPORT CFontComboBox::EnumFamPrinterCallBack(ENUMLOGFONT* pelf, NEWTEXTMETRICEX* /*lpntm*/, int FontType, LPVOID pThis)
{
...
BOOL CALLBACK AFX_EXPORT CFontComboBox::EnumFamScreenCallBackEx(ENUMLOGFONTEX* pelf, NEWTEXTMETRICEX* /*lpntm*/, int FontType, LPVOID pThis)
{
    // don't put in non-printer raster fonts
    if (FontType & RASTER_FONTTYPE)
        return 1;
...
BOOL CALLBACK AFX_EXPORT CFontComboBox::EnumFamPrinterCallBackEx(ENUMLOGFONTEX* pelf, NEWTEXTMETRICEX* /*lpntm*/, int FontType, LPVOID pThis)
{
    DWORD dwData = PRINTER_FONT;
    if (FontType & TRUETYPE_FONTTYPE)
        dwData |= TT_FONT;
...
void CFontComboBox::MatchFont(LPCTSTR lpszName, BYTE nCharSet)
{
    ...
    if (nFirstIndex != CB_ERR)
    {
        ...
        do
        {
            ...
            if ((nCharSet == DEFAULT_CHARSET || pDesc->m_nCharSet == nCharSet) && lstrcmp(lpszName, pDesc->m_strName) == 0)
            {
                //got a match
                if (GetCurSel() != nIndex)
                    SetCurSel(nIndex);
...
BOOL CALLBACK AFX_EXPORT CSizeComboBox::EnumSizeCallBack(LOGFONT FAR* /*lplf*/, LPNEWTEXTMETRIC lpntm, int FontType, LPVOID lpv)
{
