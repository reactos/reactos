...
// at least MAX_PATH because the contents of the array will be replaced with the filesystem path in ScanForConverters.

#define CONVERTER_PATH(x) TEXT("Software\\Microsoft\\Shared Tools\\")   TEXT("Text Converters\\Import\\") TEXT(x)
...
int GetDocTypeFromName(LPCTSTR pszPathName, CFileException& fe, bool defaultToText)
{
    CFile file;
...
#define TRACE_ERROR(error, api, string)                                     \
        {if (ERROR_SUCCESS != error)                                        \
                TRACE(TEXT("Wordpad: error 0x%08x from %s looking for")       \
                                    TEXT("\r\n\t%s\r\n"),error, api, string); \
        }
#else // !_DEBUG
#define TRACE_ERROR(error, api, string)
    ...
    for (int i = 0; i < NUM_DOC_TYPES; i++)
    {
        ...
        {
            error = RegOpenKey(HKEY_LOCAL_MACHINE, pszConverterKey, &keyConverter);
            TRACE_ERROR(error, TEXT("RegOpenKey"), pszConverterKey);
            if (ERROR_SUCCESS == error)
            {
                error = RegQueryValueEx(keyConverter, TEXT("Path"), NULL, NULL, (LPBYTE) szConverterPath, &cbConverterPath);
                TRACE_ERROR(error, TEXT("RegQueryValueEx"), TEXT("Path"));
                RegCloseKey(keyConverter);
                if (ERROR_SUCCESS == error)
                {
                    if (0 == ExpandEnvironmentStrings(szConverterPath, szExpandedPath, MAX_PATH))
                    {
                        error = GetLastError();
                        TRACE_ERROR(error, TEXT("ExpandEnvironmentStrings"), szConverterPath);
                    }
                }

                if (ERROR_SUCCESS == error)
                {
                    // The FILE_ATTRIBUTE_DIRECTORY bit will also be set if an error occurs - like file not found.
                    error = GetFileAttributes(szExpandedPath) & FILE_ATTRIBUTE_DIRECTORY;
                    TRACE_ERROR(error, TEXT("GetFileAttribytes"), szExpandedPath);
                }
            }
...
CString GetFileTypes(BOOL bOpen)
{
    ...
    for (int i = 0; i < NUM_DOC_TYPES; i++)
    {
        if (bOpen && doctypes[i].bRead && !doctypes[i].bDup && !(RD_WORD97 == doctypes[i].nID))
        {
