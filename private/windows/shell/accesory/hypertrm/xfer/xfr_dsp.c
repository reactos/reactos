...
*/
void xferMsgProgress(HSESSION hSession, long stime, long ttime, long cps, long file_so_far, long total_so_far)
{
    XD_TYPE *pX;
 ...
 *    Nothing.
*/
void xferMsgNewfile(HSESSION hSession, int filen, BYTE *theirname, TCHAR *ourname)
{
    XD_TYPE *pX;
...
void xferMsgMessage(HSESSION hSession, BYTE *pszMsg)
{
    ...
    if (pX)
    {
        ...
        if (IsWindow(pX->hwndXfrDisplay))
        {
            PostMessage(pX->hwndXfrDisplay, WM_DLG_TO_DISPLAY, XFR_UPDATE_DLG, 0);
        }
        xfer_idle(hSession, XFER_IDLE_DISPLAY);
    }
}
