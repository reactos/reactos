...
* Everybody keeps changing the time standard to whatever they feel might be a little bit better for them.
* So far I have found 3 different standards in Microsoft functions.
* This does not even count the fact that HyperP uses its own format for time.

* Henceforth, all time values that are passed around in the program will be based on the old UCT format of the number of seconds since Jan 1, 1970.
...
/*
* DESCRIPTION:
*    This function is called by the transfer routines to set the date/time of a file.
...
*/
{
    ...
    hFile = CreateFile(pszName, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
    ...
    if (pstT)
    {
        ...
        wDOSDate = ((pstT->tm_year - 80) << 9) | (pstT->tm_mon << 5) | pstT->tm_mday;
        DbgOutStr("Date %d %d %d 0x%x\r\n", pstT->tm_year, pstT->tm_mon, pstT->tm_mday, wDOSDate, 0);

        wDOSTime = ((pstT->tm_hour - 1) << 11) | (pstT->tm_min << 5) | (pstT->tm_sec / 2);
        ...
        SetFileTime(hFile, &stFileTime, &stFileTime, &stFileTime);/* Set the time */
    }
...
* DESCRIPTION:
*    This function is called by the transfer routines to get the date/time of a file.
...
/*
* DESCRIPTION:
*    This function converts the "new" internal MICROSOFT time format (based at 1900) to the "old" format (based at 1970).
*/
{
