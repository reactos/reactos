...
/*
 * DESCRIPTION:
 *    Prepares an XMODEM packet for transmission by filling it with data and initializing other fields such as
*/
int load_pckt(ST_MDMX *pX, struct s_mdmx_pckt *p, unsigned pcktnum, int kpckt, int chktype)
{
    BYTE checksum;
    unsigned int crc;
    BYTE *cp;
    ...
    else
    {
        ...
        *(cp + 1) = 0;
        ...
        crc = calc_crc(pX, (unsigned) 0, p->bdata, (kpckt ? LARGE_PACKET : SMALL_PACKET) + 2);
