...
int krec_data(ST_KRM *kc)
    ...
    case 'D':                    /* data packet        */
        ...
        if (rtype == 'D')
            ...
            ksend_packet(kc, 'Y', kc->kr.uabort_code ? 1 : 0, kc->ksequence, &kc->kr.resp_pckt);
            kr_progress(kc, 0);
            if (kunload_packet(kc, kplen, packet) == ERROR)
                {    /* storage file error */
                kc->kr.lasterr = KE_FATAL;
                kc->abort_code = KA_DISK_ERROR;
                LoadString(glblQueryDllHinst(), IDS_TM_KRM_CANT_WRITE, kc->xtra_err, sizeof(kc->xtra_err) / sizeof(TCHAR));
                ...
                ksend_packet(kc, 'E', StrCharGetByteCount(kc->kr.resp_pckt.pdata), (kc->ksequence + 1) % 64, &kc->kr.resp_pckt);
                return(KREC_ABORT);
                }
            }
        else if (rtype == 'A')
            ...
            ksend_packet(kc, 'Y', StrCharGetByteCount(kc->kr.resp_pckt.pdata), kc->ksequence, &kc->kr.resp_pckt);
            }
        kc->kr.oldtries = kc->tries;
        kc->tries = 0;
        kc->ksequence = (kc->ksequence + 1) % 64;
