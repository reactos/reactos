...
*/
int resLoadDataBlock(const HINSTANCE hInst, const int id, const void **ppData, int *pSize)
{
    HGLOBAL hG;
    HRSRC hR;
    LPVOID pV;
...
*/
int resFreeDataBlock(const HSESSION hSession, const void *pData)
{
    return 0;
}
...
/*
* DESCRIPTION:
*    This function is used to get around a problem that exists in loading strings into the common file dialogs.
*   The file name masks are two strings that are NULL separated.  This internal NULL is not treated with any
...
*/
int resLoadFileMask(HINSTANCE hInst, UINT uId, int nCount, LPTSTR pszBuffer, int nSize)
{
    int i;
    LPTSTR pszEnd;
    LPTSTR pszPtr;
