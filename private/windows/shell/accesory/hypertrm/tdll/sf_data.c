...
*/
int sfdGetDataBlock(const HSESSION hSession, const int nId, const void **ppData)
{
    SF_DATA *pSFD;
    SF_HANDLE hSF;
    int sRet;
...
/*
 * DESCRIPTION:
 *    This function is called to save a "standard" data block to the session file.  A "standard" data block is any data structure with an int as the
 *    first element that contains the size of the block.
 ...
 *    ZERO if everything is OK, otherwise an error code.
*/
int sfdPutDataBlock(const HSESSION hSession, const int nId, const void *pData)
{
    SF_DATA *pSFD;
    SF_HANDLE hSF;
    int sRet;
