 ...
 *    is made or a call to ComSendPush is made while the transmitter is not busy.
 ...
 *                COM_INVALID_HANDLE if invalid com handle
*/
int ComSendSetStatusFunction(const HCOM pstCom, STATUSFUNCT pfNewStatusFunct, STATUSFUNCT *ppfOldStatusFunct)
{
...
*/
int ComSendDefaultStatusFunction(int iReason, unsigned afHsStatus, long lDelay)
{
...
static int ComSendCheck(const HCOM pstCom, const int fDataWaiting)
{
    ...
    if (ComSndBufrBusy(pstCom) != COM_OK)
    {
        ...
        if (afXmitStatus != 0)
        {
            switch ((*(pstCom->pfUserFunction))(fDataWaiting ? COMSEND_DATA_WAITING : COMSEND_NORMAL, afXmitStatus, lHandshakeDelay))
            {
            case COMSEND_OK:
                break;
            case COMSEND_GIVEUP:
            ...
            case COMSEND_FORCE_CONTINUATION:
                if (bittest(afXmitStatus, COMSB_WAIT_XON))
                    ComSendXon(pstCom);
                else if (bittest(afXmitStatus, (COMSB_WAIT_CTS | COMSB_WAIT_DSR | COMSB_WAIT_DCD)))
                {
    ...
    else
    {
        if (pstCom->nSendCount > 0)
        {
            ...
            pstCom->puchSendBufr = pstCom->puchSendPut = ((pstCom->puchSendBufr == pstCom->puchSendBufr1) ? pstCom->puchSendBufr2 : pstCom->puchSendBufr1);
            pstCom->nSendCount = 0;
        }
