...
int InitializeCaptureFileHandle(const HSESSION hSession, HCAPTUREFILE hCapt)
{
    ...
    LoadString(glblQueryDllHinst(), IDS_CPF_CAP_FILE, pszStr, (int)(sizeof(acBuffer) - (pszStr - acBuffer) / sizeof(TCHAR)));
...
int LoadCaptureFileHandle(HCAPTUREFILE hCapt)
{
    ...
    if (lSize)
    {
        sfGetSessionItem(sessQuerySysFileHdl(pOld->hSession), SFID_CPF_MODE, &lSize, &pOld->nDefaultCaptureMode);
    ...
    lSize = 0;
    sfGetSessionItem(sessQuerySysFileHdl(pOld->hSession), SFID_CPF_FILE, &lSize, NULL);
    if (lSize)
    {
        lSize = sizeof(int);
        sfGetSessionItem(sessQuerySysFileHdl(pOld->hSession), SFID_CPF_FILE, &lSize, &pOld->nDefaultFileMode);
    ...
    lSize = 0;
    sfGetSessionItem(sessQuerySysFileHdl(pOld->hSession), SFID_CPF_FILENAME, &lSize, NULL);
    if (lSize)
    {
        LPTSTR pszName;
        ...
        sfGetSessionItem(sessQuerySysFileHdl(pOld->hSession), SFID_CPF_FILENAME, &lSize, pszName);
