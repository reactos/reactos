 ...
 *  Description:
 *      Declares the functions used for manipulating the Windows 95 system registry.
...
long regOpenKey(HKEY hKey, LPCTSTR pszSubKey, REGSAM samAccess, HKEY* phOpenKey)
    ...
    // Instead of calling RegOpenKeyEx, call RegCreateKeyEx which will return an open in key, but will also create a key that does not exist.
    return RegCreateKeyEx(hKey, pszSubKey, 0, 0, REG_OPTION_NON_VOLATILE, samAccess, NULL, phOpenKey, &dwDisposition);
    }
 ...
 *                on exit this will be the size of the data read.
...
*/
 int regQueryValue(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValue, LPBYTE pData, LPDWORD pdwDataSize)
    {
    ...
    if ( lResult == ERROR_SUCCESS )
        ...
        lResult = RegQueryValueEx(hSubKey, pszValue, NULL, NULL, pData, pdwDataSize);
        }
...
*/
int regSetStringValue(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValue, LPCTSTR pszData)
    {
    long  lResult = ERROR_SUCCESS;
    DWORD dwSize = 0;
    HKEY  hSubKey = 0;
    ...
    if ( lResult == ERROR_SUCCESS )
        {
        dwSize = StrCharGetByteCount(pszData) + sizeof(TCHAR); // The size of the string must include the null terminator.
        lResult = RegSetValueEx(hSubKey, pszValue, 0, REG_SZ, pszData, dwSize);// Set value of that subkey.
        }
 ...
 * AUTHOR:  C. Baumgartner, 11/27/96
*/
int regSetDwordValue(HKEY hKey, LPCTSTR pszSubKey, LPCTSTR pszValue, DWORD dwData)
    {
    long  lResult = ERROR_SUCCESS;
    HKEY  hSubKey = 0;
    ...
    if ( lResult == ERROR_SUCCESS )
        ...
        lResult = RegSetValueEx(hSubKey, pszValue, 0, REG_DWORD, (LPBYTE)&dwData, sizeof(dwData));
        }
