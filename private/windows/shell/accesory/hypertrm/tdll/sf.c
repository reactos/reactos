...
int sfOpenSessionFile(const SF_HANDLE sF, const TCHAR *pszName)
    ...
    if (pszName && (StrCharGetStrLength(pszName) > 0))
        ...
        hFile = CreateFile(pszName, GENERIC_READ, 0, 0, OPEN_EXISTING, 0, 0);
                ...
                if (nOrderOK)
                    ...
                    if (pD->uItemCount >= pD->uItemsAlloc)
                        ...
                        if (pD->hIndex == (pstDSII)0)
                            ...
                            {
                            pD->uItemsAlloc *= 2;
                            pD->hIndex = (pstDSII)realloc(pD->hIndex, (size_t)pD->uItemsAlloc * sizeof(stDSII));
                            }
                        }
                    ...
                    sfPutSessionItem(uNdx + 1, (unsigned int)sID, (unsigned long)dwSZ, acBuffer);
                    }
                }
