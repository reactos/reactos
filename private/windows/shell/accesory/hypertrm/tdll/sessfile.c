...
/*
 * DESCRIPTION:
 *    This function gets called whenever the user wants to read in the data from a session file. 
 *    If there is a currently opened session file, it is open, otherwise we prompt for one.
 * PARAMETERS:
...
*/
{
    ...
    // the terminal load its fonts before loading the emulator so there is no conflict. mrw,3/2/95
    if (bRet)
    {
    ...
    if (bRet)
    {
        ...
        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_SOUND, &lSize, &hhSess->fSound);
    }
    ...
    if (bRet)
    {
        ...
        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_EXIT, &lSize, &hhSess->fExit);
    }
    ...
    if (bRet)
    {
        ...
        sfGetSessionItem(hhSess->hSysFile, SFID_TLBR_VISIBLE, &lSize, &fBool);
    ...
    if (bRet)
    {
        ...
        sfGetSessionItem(hhSess->hSysFile, SFID_STBR_VISIBLE, &lSize, &fBool);
    ...
    if (bRet)
    {
        ...
        // Initialize...
        TCHAR_Fill(hhSess->achSessName, TEXT('\0'), sizeof(hhSess->achSessName) / sizeof(TCHAR));
        sfGetSessionFileName(hhSess->hSysFile, sizeof(hhSess->achSessName) / sizeof(TCHAR), hhSess->achSessName);

        // Hold on to just the session name, no path, no extension.  It is usefull to keep it around.
        ...
        //        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_NAME, &lSize, hhSess->achSessName);
    ...
    if (bRet)
    {
        ...
        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_LEFT, &lSize, &hhSess->rcSess.left);
        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_TOP, &lSize, &hhSess->rcSess.top);
        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_RIGHT, &lSize, &hhSess->rcSess.right);
        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_BOTTOM, &lSize, &hhSess->rcSess.bottom);
    }
    ...
    if (bRet)
    {
        ...
        sfGetSessionItem(hhSess->hSysFile, SFID_SESS_SHOWCMD, &lSize, &hhSess->iShowCmd);
    }
    ...
    {
        PostMessage(hhSess->hwndSess, WM_COMMAND, MAKEWPARAM(IDM_CONTEXT_SNAP, 0), 0);
    }
...
/*
 * DESCRIPTION:
 *    This function is called to call all the functions that save things in the session file. 
 *    If you have stuff to write into the session file, it should get called from here. 
 *    This function also makes sure that the user has a chance to specify the name of the session file if there is not one currently.
 * PARAMETERS:
 *    hSession -- the session handle
*/
{
...
*/
{
    ...
    //    sfPutSessionItem(hhSess->hSysFile, SFID_SESS_NAME, (StrCharGetByteCount(hhSess->achSessName) + 1) * sizeof(TCHAR), hhSess->achSessName);
    ...
    // mrw:4/21/95
    if (stWP.showCmd == SW_SHOWMINIMIZED || stWP.showCmd == SW_MINIMIZE || stWP.showCmd == SW_SHOWMINNOACTIVE)
    {
...
/*
 * DESCRIPTION:
 *    This function is called to take the stuff that is in the backscroll and on the screen and save it away in the session file.
...
*/
{
    ...
    // won't be any terminal window and CopyTextFromTerminal() will fault. - mrw
    ...
    if (pszData != (ECHAR *)0)
    {
         ...
         * trailing blank lines there are don't get put into the saved text.
         */
        pszPtr = pszData;
        pszEnd = pszPtr;
        ...
        nRet = sfPutSessionItem(sessQuerySysFileHdl(hSession), SFID_BKSC_TEXT, dwSize, pszData);
        free(pszData);
        pszData = NULL;
    }
...
*/
{
    ...
    if (lSize > 0)
    {
        ...
        sfGetSessionItem(sessQuerySysFileHdl(hSession), SFID_BKSC_TEXT, &lSize, pszData);

        // JYF 29-Mar-1999 changed pszPtr <= pszData+lSize
        //  so we don't go beyond the end of the buffer.

        pszPtr = pszData;
        while ((*pszPtr != ETEXT('\0')) && ((DWORD_PTR)pszPtr < ((DWORD_PTR)pszData + lSize)))
        {
            pszEnd = pszPtr;
            while (((DWORD_PTR)pszEnd < ((DWORD_PTR)pszData + lSize)) && (*pszEnd != ETEXT('\0')) && (*pszEnd != ETEXT('\r')))
                pszEnd = pszEnd++;
...
*/
{
    ...
    for (pszStr = hhSess->achSessCmdLn; *pszStr != TEXT('\0'); pszStr = StrCharNext(pszStr))
    {
        /*
        ...
        if (fTestOpenOldTrmFile(hhSess, acName) != 0)
        {
            ...
            if (sfOpenSessionFile(hhSess->hSysFile, acName) < SF_OK)
            {
                ...
                if (hhSess->iCmdLnDial == CMDLN_DIAL_DIAL)
                {
                    ...
                    TCHAR acFormat[64];
                    TCHAR ach[FNAME_LEN];
                    LoadString(glblQueryDllHinst(), IDS_ER_BAD_SESSION, acFormat, sizeof(acFormat) / sizeof(TCHAR));
                    wsprintf(ach, acFormat, acName);

                    TimedMessageBox(sessQueryHwnd(hSession), ach, TEXT(""), MB_OK | MB_ICONEXCLAMATION, sessQueryTimeout(hSession));
...
/*
 * DESCRIPTION:
 *    Tests if its an old trm file. If so, it opens it and reads the data out.
 * ARGUMENTS:
...
*/
{
    ...
    {
        /* Old .TRM files case */
        hFile = CreateFile(achName, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
