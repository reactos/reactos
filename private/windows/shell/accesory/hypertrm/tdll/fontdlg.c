...
void DisplayFontDialog(const HSESSION hSession, BOOL fPrinterFont)
{
    ...
    if (!fPrinterFont)
    {
        SendMessage(sessQueryHwndTerminal(hSession), WM_TERM_GETLOGFONT, 0, (LPARAM) &lf);
        chf.hDC = GetDC(hwnd);
        chf.Flags = CF_SCREENFONTS | CF_FIXEDPITCHONLY | CF_NOVERTFONTS | CF_INITTOLOGFONTSTRUCT;
        lfOld = lf;
        fRet = ChooseFont(&chf);
    ...
    else
    {
        ...
        chf.hDC = hhPrint->hDC;
        chf.Flags = CF_EFFECTS | CF_PRINTERFONTS | CF_NOVERTFONTS | CF_INITTOLOGFONTSTRUCT;
        lfOld = lf;
        fRet = ChooseFont(&chf);
    }
    ...
    if (fRet && memcmp(&lf, &lfOld, sizeof(LOGFONT)) != 0)
    {
        ...
        {
            // save the dialog returned log font in the print handle and also save the selected point size.
            // This is done since the font point size returned by the dialog is is not correct when used for printing.
            // However the dialog settings must be saved for the next time the dialog is displayed.
            // The correct font is calculated based on the save point size and face name before printing by the printCreatePointFont function.
