...
void glblSetHelpFileName(void)
    ...
    if ( RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\HyperTrm.exe"), 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
        {
        memset(szHelpFileName, 0, dwSize);
        if (RegQueryValueEx(hKey, TEXT("HelpFileName"), 0, 0, szHelpFileName, &dwSize) != ERROR_SUCCESS)
            {
            LoadString(glblQueryDllHinst(), IDS_GNRL_HELPFILE, szHelpFileName, dwSize / sizeof(TCHAR));
            }
