...
*/
{
    ...
    while (!bittest(pstCLoop->afControl, CLOOP_TERMINATE))
    {
        ...
        dwRet = WaitForMultipleObjects(DIM(pstCLoop->ahEvents), pstCLoop->ahEvents, FALSE, INFINITE);
        ...
        if (!pstCLoop->afRcvBlocked)
        {
            ...
            for (nRcvCount = 10; nRcvCount--; )
            {
...
#if defined(CHARACTER_TRANSLATION)
                    LeaveCriticalSection(&pstCLoop->csect);
                    hTrans->pfnIn(hTrans->pDllHandle, chData, &nCount, sizeof(chBuffer), chBuffer);
                    EnterCriticalSection(&pstCLoop->csect);
#else
                    chBuffer[0] = chData;
                    nCount = 1;
                        ...
                        if (fValidChar)
                        {
                            ...
                            if (pstCLoop->fRmtChain)
                            {
                                ...
                                while (pstCLoop->pstRmtChainNext)
                                {
                                    ...
                                    LeaveCriticalSection(&pstCLoop->csect);
                                    mc = (*pfChainFunc)(chData, pstCurrent->pvUserData);
                                    EnterCriticalSection(&pstCLoop->csect);
                                    if (mc == CLOOP_DISCARD)
                                        break;
                                }
                            ...
                            if (pstCLoop->afRcvBlocked)
                            {
                                 ...
                                 * characters will "slip thru" before this loop terminates.
                ...
                {
                    CLoopRcvControl((HCLOOP)pstCLoop, CLOOP_SUSPEND, CLOOP_RB_NODATA);
        ...
        if (!pstCLoop->afSndBlocked)
        {
            ...
            else if (CLoopGetNextOutput(pstCLoop, &keyOut))
            {
                ...
                if (keyOut == TEXT('\t') && pstCLoop->stWorkSettings.fExpandTabsOut && pstCLoop->stWorkSettings.nTabSizeOut)
                {
                    //* int   i;
                    //* POINT pt;

                    //* mEmuGetCursorPos(mGetEmulatorHdl(pstCLoop->hSession), &pt);
                ...
                if (keyOut == TEXT('\r'))
                {
                    ...
                    {
                        CLoopSndControl((HCLOOP)pstCLoop, CLOOP_SUSPEND, CLOOP_SB_LINEWAIT);
                    }
                    ...
                    if (pstCLoop->stWorkSettings.nLineDelay)
                    {
                        if (TimerCreate(
                            ...
                            (void *)pstCLoop) == TIMER_OK)
                            CLoopSndControl((HCLOOP)pstCLoop, CLOOP_SUSPEND, CLOOP_SB_DELAY);
                    }
                }
                else
                {
...
*/
{
    ...
    if (pstCLoop->stWorkSettings.fLineWait && chIn == pstCLoop->stWorkSettings.chWaitChar)
...
*/
{
    ...
    (*hTrans->pfnOut)(hTrans->pDllHandle, chOut, &nCount, sizeof(chBuffer), chBuffer);
    ...
    for (nIndx = 0; nIndx < nCount; nIndx += 1)
    {
        ...
        if (pstCLoop->stWorkSettings.fLocalEcho && !pstCLoop->fSuppressDsp)
        {
            if (pstCLoop->fDoMBCS)
            {
