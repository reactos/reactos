...
int CLoopActivate(const HCLOOP hCLoop)
{
    ...
    if (fSuccess)
    {
        ...
        pstCLoop->hEngineThread = CreateThread((LPSECURITY_ATTRIBUTES) 0, 4096, (LPTHREAD_START_ROUTINE) CLoop, (LPVOID) pstCLoop, 0, &dwThreadId);
...
void CLoopDeactivate(const HCLOOP hCLoop)
{
    ...
    if (pstCLoop)
    {
        ...
        if (pstCLoop->hEngineThread)
        {
            ...
            if (WaitForSingleObject(pstCLoop->hEngineThread, 10000) == WAIT_OBJECT_0)
            {
