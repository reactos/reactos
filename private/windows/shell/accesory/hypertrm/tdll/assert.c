...
void DoAssertDebug(TCHAR *file, int line)
    ...
    retval = MessageBox((HWND)0, buffer, TEXT("Assert"), MB_ICONQUESTION | MB_YESNOCANCEL | MB_SETFOREGROUND);
    switch (retval)
        {
        case IDYES:
            return;
...
void DoShowLastError(const TCHAR *file, const int line)
    ...
    retval = MessageBox((HWND)0, ach, TEXT("GetLastError"), MB_ICONQUESTION | MB_YESNOCANCEL | MB_SETFOREGROUND);
    switch (retval)
        {
        case IDYES:
            return;
