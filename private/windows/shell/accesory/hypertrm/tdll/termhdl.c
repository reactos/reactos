...
static int APIENTRY EnumFontCallback(LPLOGFONT lplf, LPTEXTMETRIC lptm, DWORD dwType, LPVOID lpData);
...
*/
{
    ...
    if (plf == 0)
    {
        ...
        if (LoadString(glblQueryDllHinst(), IDS_TERM_DEF_VGA_SIZE, ach, sizeof(ach)))
        {
            nSize1 = atoi(ach);
        }
        else
        ...
        if (LoadString(glblQueryDllHinst(), IDS_TERM_DEF_NONVGA_SIZE, ach, sizeof(ach)))
        {
            nSize2 = atoi(ach);
        }
        ...
        if (LoadString(glblQueryDllHinst(), IDS_TERM_DEF_FONT, ach, sizeof(ach)))
        {
            strncpy(lf.lfFaceName, ach, sizeof(lf.lfFaceName));
            lf.lfFaceName[sizeof(lf.lfFaceName) - 1] = TEXT('\0');
        }

        if (LoadString(glblQueryDllHinst(), IDS_TERM_DEF_CHARSET, ach, sizeof(ach)))
        {
            nCharSet = atoi(ach);
    ...
    // We need to know if the font is italic because it changes the way we draw. 
    // The italic fonts are regular fonts sheared. 
    // The shear causes the character to draw into the next text box.
...
HFONT termMakeFont(const HHTERM hhTerm, const BOOL fUnderline, const BOOL fHigh, const BOOL fWide, const BOOL fSymbol)
/*
 * DESCRIPTION:
 *    When user selects a font, we only create the standard non-underlined font. 
 *    If the paint routines encounter an attribute that needs a
...
*/
{
    ...
    // This number should be big enough so that a maximized window of backscroll text can be displayed.

    hhTerm->iMaxPhysicalBkRows = hhTerm->iPhysicalBkRows = min(5000, (GetSystemMetrics(SM_CYFULLSCREEN) / hhTerm->yChar) + 1);

    if ((hhTerm->fplpstrBkTxt = malloc((unsigned int)hhTerm->iMaxPhysicalBkRows * sizeof(ECHAR *))) == 0)
    {
        return FALSE;
    }
    ...
    {
        if ((hhTerm->fplpstrBkTxt[i] = malloc(MAX_EMUCOLS * sizeof(ECHAR))) == 0)
        {
            termFreeBkBuf(hhTerm);
            return FALSE;
        }
 ...
 *    The goal is to find the pallete color closest to the given colorref value.   
 *    Borrowing from the 10th grade algrebra we know that:
