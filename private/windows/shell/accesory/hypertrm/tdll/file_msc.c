                     ...
                     const size_t cb)
{
    ...
    int SetFileSize(const TCHAR *pszName, unsigned long ulFileSize)
    {
        ...
        hFile = CreateFile(pszName, GENERIC_WRITE, FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
    ...
    int  ValidateFileName(LPSTR pszName)
    {
        ...
        {
            hfile = CreateFile(pszName, GENERIC_READ, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
        }
        else
        {
            hfile = CreateFile(pszName, GENERIC_READ, FILE_SHARE_READ, NULL, CREATE_NEW, FILE_FLAG_DELETE_ON_CLOSE, NULL);
        }
