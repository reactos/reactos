...
void emuAutoInit(const HHEMU hhEmu)
{
    ...
    // Allocate space for and initialize data that is used only by the Auto ANSI emulator.
    ...
    // The Auto Detect emulator is ANSI based.
    // So, do the same stuff we do when initializing the ANSI emulator.
    emuKeyTableLoad(hhEmu, AnsiKeyTable, sizeof(AnsiKeyTable) / sizeof(KEYTBLSTORAGE), &hhEmu->stEmuKeyTbl);
    emuKeyTableLoad(hhEmu, IBMPCKeyTable, sizeof(IBMPCKeyTable) / sizeof(KEYTBLSTORAGE), &hhEmu->stEmuKeyTbl2);
