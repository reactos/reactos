 ...
 *     The cursor can not move past the end of the scrolling region.
...
/*
 *     Moves the cursor backwards (to the left) the specified number of characters, but stops at the 1st character in the current line.
...
/*
 * DESCRIPTION: Moves the cursor down the specified number of lines, but stops at the bottom of the scrolling region.
 *                The column is constant.
 *                If below the scrolling region, it stops at the bottom of the screen.
...
*/
{
    ...
    if (row > hhEmu->bottom_margin && (hhEmu->emu_currow <= hhEmu->bottom_margin || hhEmu->emu_currow > hhEmu->emu_maxrow))
        row = hhEmu->bottom_margin;
...
/*
 *     Moves the cursor forward the specified number of characters, but stops at the last character in the current line.
 ...
 *     The row & column numbering start at 1.
 *     If origin mode is on, the positioning is relative to the home of the scrolling region.
...
/*
 * DESCRIPTION: Moves the cursor up the specified number of lines, but stops at the top of the scrolling region.
 *                The column is constant.
 *                If above the scrolling region, it stops at the top of the screen.
...
/*
 *    Deletes the specified number of characters starting at the current cursor position and moving right.
 *    It stops at the end of the current line.
...
*/
{
    ...
    // Range check.  Have we been asked to delete more characters than are displayed?  If so, change the number.
    iChars = min(hhEmu->num_param[hhEmu->num_param_cnt], (hhEmu->emu_aiEnd[hhEmu->emu_imgrow] - hhEmu->emu_curcol) + 1);
    if (iChars < 1)
        iChars = 1;
    ...
    if ((hhEmu->emu_curcol == hhEmu->emu_maxcol) && ((hhEmu->stUserSettings.nEmuId == EMU_VT100) || (hhEmu->stUserSettings.nEmuId == EMU_VT100J)))
    {
    ...
    iR = hhEmu->emu_aiEnd[hhEmu->emu_imgrow] - hhEmu->emu_curcol - (iChars - 1);

    // Move the text image if there are remaining characters to display.
    // Replace iChar characters at end of line with spaces.
#if 0
    if (iR) {
        memmove(hhEmu->emu_apText[hhEmu->emu_imgrow + hhEmu->emu_curcol], &(hhEmu->emu_apText[hhEmu->emu_imgrow + hhEmu->emu_curcol][iChars]), ((size_t) iR * sizeof(ECHAR)));
    }
#endif
    ...
    ECHAR_Fill(&hhEmu->emu_apText[hhEmu->emu_imgrow][hhEmu->emu_aiEnd[hhEmu->emu_imgrow] - iChars + 1], EMU_BLANK_CHAR, (unsigned int) iChars);

    // Move the attributes.  Clear iChar attributes at end of line.
    if (iR) {
        pstAttr = hhEmu->emu_apAttr[hhEmu->emu_imgrow] + hhEmu->emu_curcol;
        memmove(pstAttr, (pstAttr + iChars), (size_t) (sizeof(STATTR)*(unsigned) iR));
    }

    pstAttr = hhEmu->emu_apAttr[hhEmu->emu_imgrow];
    for (i = (hhEmu->emu_aiEnd[hhEmu->emu_imgrow] - iChars) + 1; i <= hhEmu->emu_aiEnd[hhEmu->emu_imgrow]; i++)
        pstAttr[i] = hhEmu->emu_clearattr;

    // Note that emu_aiEnd[emu_imgrow] is used before we reset that value.
    // Remember there may have been some characters and attributes removed from the end of the line.
    // We need to tell the update stuff to go that far over in the line.
    updateChar(sessQueryUpdateHdl(hhEmu->hSession), hhEmu->emu_currow, hhEmu->emu_curcol, hhEmu->emu_aiEnd[hhEmu->emu_imgrow]);

    // Reset emu_aiEnd.  Note that it is expected and intended that the result of the following calculation may be (-1).
...
/*
 *     Deletes the specified number of lines starting at the current cursor line and moving down.
 *     It stops at the bottom of the scrolling region.
...
/*
 *     Erases some or all of the virtual screen image and corresponding real screen.
...
*/
{
    ...
    case 0xF2:
        ...
        if ((hhEmu->stUserSettings.nEmuId == EMU_ANSI) || (hhEmu->stUserSettings.nEmuId == EMU_ANSIW) || (hhEmu->stUserSettings.nEmuId == EMU_AUTO))
        {
...
/*
 *     Erases some or all of the current virtual screen line and corresponding real screen line.
...
/*
 *     Inserts the specified number of spaces starting at the current cursor position.
...
/*
 *     Inserts the specified number of lines starting at the current cursor row.
...
/*
 *     Moves cursor down 1 line and scrolls 1 line if necessary. IND stands for index.
...
/*
 *     Saves ANSI style parameters and selectors. The selectors are saved as hex and numeric parameters as decimals.
...
*/
{
    ...
    hhEmu->selector[hhEmu->selector_cnt] = (int) ((unsigned) hhEmu->selector[hhEmu->selector_cnt] << 4) + ccode - ETEXT('0');
    hhEmu->num_param[hhEmu->num_param_cnt] = 10 * hhEmu->num_param[hhEmu->num_param_cnt] + (ccode - ETEXT('0'));
}
...
/*
 *     Sets next numeric parameter and selector to 0 to indicate end of escape sequence.
...
/*
 *     Moves cursor up 1 line and scrolls 1 line if necessary. RI stands for reverse index.

 * ARGUMENTS:
 *     none
*/
{
    /* If at the scrolling region's top margin, scroll down 1, but if above the top margin, and below to top edge of the screen,
     * move the cursor up. If above the top margin and at the top edge of the screen, do nothing.
     */
 ...
 * NOTES:This function contains Auto Detect code.
*/
{
    STATTR stAttr;
    int fAnsi, sel, i;

    fAnsi = ((hhEmu->stUserSettings.nEmuId == EMU_ANSI) || (hhEmu->stUserSettings.nEmuId == EMU_ANSIW) || (hhEmu->stUserSettings.nEmuId == EMU_VIEW) || (hhEmu->stUserSettings.nEmuId == EMU_AUTO)) ? TRUE : FALSE;
    ...
    for (i = 0; i <= hhEmu->selector_cnt; i++)
    {
        ...
        case 0: /* all attributes off */
            ...
            {
                // don't mess with color attributes in AutoDetect mode for attribute reset case - mrw, 10/17/94
                stAttr.undrln = 0;
                stAttr.hilite = 0;
                stAttr.bklite = 0;
                stAttr.blink = 0;
            ...
            else if (fAnsi)
            {
                ...
                // Reset to the user-defined colors.
                // I'm afraid this may re-introduce the intensity bug referred to below.
                // rde 9 Jun 98
...
#else
                // mrw:2/21/96 - Changed from 15 to 7 to fix bug with intensity.
        ...
        case 7: /* reverse video */
            // Reverse video should be reverse video for a cell reguardless of the current screen mode.
            // Changing the following line removed a bug where blocks of text would not get set to reverse video when the
        ...
        if (fAnsi && hhEmu->attrState[CS_STATE].revvid)
            hhEmu->attrState[CSCLEAR_STATE].revvid = hhEmu->attrState[CS_STATE].revvid;
