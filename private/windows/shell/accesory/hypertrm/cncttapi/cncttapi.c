...
int WINAPI cnctdrvDisconnect(const HHDRIVER hhDriver, const unsigned int uFlags)
    ...
    if (uFlags & CNCT_DIALNOW)
        ...
        if (hhDriver->fRedialOnBusy && hhDriver->iRedialCnt > 0)
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\HyperTerminal\\TimeToRedial", 0, KEY_READ, &hKey) == ERROR_SUCCESS)
                {
                dwSize = sizeof(ab);
            ...
            {
            PostMessage(sessQueryHwnd(hhDriver->hSession), WM_CNCT_DIALNOW, uFlags, 0);
            }
...
SHUTDOWN:
        ...
        for (i=0 ;; ++i)
        {
            if (lineInitialize(&hhDriver->hLineApp, glblQueryDllHinst(), lineCallbackFunc, g_achApp, &hhDriver->dwLineCnt) != 0)
            {
                if (i > 10)
                {
                    assert(0);
...
static int DoNewModemWizard(HWND hWnd, int iTimeout)
    ...
    if(mscAskWizardQuestionAgain())
        ...
        if (TimedMessageBox(hWnd, ach, 0, MB_YESNO | MB_ICONQUESTION, iTimeout) == IDYES)
            ...
            if (CreateProcess(0, "control.exe modem.cpl,,Add", 0, 0, 0, 0, 0, 0, &stSI, &stPI) == FALSE)
                {
                #if defined(_DEBUG)
                    {
                    char ach[100];
