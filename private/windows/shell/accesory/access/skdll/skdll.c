...
static BOOL SkeySetRegistryValues(HKEY hkey, LPSERIALKEYS psk)
{
    ...
    if (fOk)
    {
        lErr = RegSetValueEx(           // Write Active Port
                ...
                (NULL == psk->lpszActivePort) ? 0 : (lstrlen(psk->lpszActivePort) + 1) * sizeof(*psk->lpszActivePort));
        ...
        BOOL fWinIni)
{
    ...
    fOk = ((uAction ==  SK_SPI_INITUSER) || (NULL != psk && (0 != psk->cbSize)));
    ...
    if (fOk)
    {
        ...
        case SPI_GETSERIALKEYS:
            ...
            if (fOk && (psk->dwFlags & SERKF_SERIALKEYSON) && !IsSerialKeysRunning() && IsServiceStartAllowed())
            {
                fOk = SerialKeysInstall();
            }
