...
VOID ServiceMain(DWORD dwArgc, LPTSTR *ppszArgv)
{
    ...
    s_sshStatusHandle = RegisterServiceCtrlHandler(SERVICENAME, (LPHANDLER_FUNCTION) ServiceCtrl);
...
void InstallUserInit()
{
    ...
    lErr = RegOpenKeyEx(HKEY_LOCAL_MACHINE, WINLOGONPATH, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, &hkey);
    if (ERROR_SUCCESS == lErr)
    {
        lErr = RegQueryValueEx(hkey, USERINIT, 0, &dwType, (LPBYTE)szUserinit, &cbData);
        if (ERROR_SUCCESS == lErr)
        {
            ...
            if (NULL == _tcsstr(szUserinit, SKEYSUSERINITCMD) && lstrlen(szUserinit) + lstrlen(SKEYSUSERINITCMD) + 2 < ARRAY_SIZE(szUserinit))
            {
                lstrcat(szUserinit, USERINITCMDSEP);
                lstrcat(szUserinit, SKEYSUSERINITCMD);

                RegSetValueEx(hkey, USERINIT, 0, REG_SZ, (CONST LPBYTE)szUserinit, (lstrlen(szUserinit) + 1) * sizeof(*szUserinit));
            }
        }
        RegCloseKey(hkey);
    }
