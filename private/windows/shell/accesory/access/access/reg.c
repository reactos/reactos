...
{
    DWORD dwResult;
    dwResult = RegSetValueEx(hkey, lpszEntry, 0, REG_SZ, lpszValue, strlen(lpszValue) + sizeof(TCHAR));
...
DWORD CopyKey(HKEY hkeySrc, HKEY hkeyDst, LPSTR szKey)
{
    ...
    for (nKey = 0; ERROR_SUCCESS == (iStatus = RegEnumKey(hkeyOld, nKey, szBuffer, sizeof(szBuffer))); nKey++)
    {
