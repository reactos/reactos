...
BOOL CreateQuickResKey( )
{
        ...
        if(  (RegReturn = RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_SOFTWARE, 0, KEY_CREATE_SUB_KEY, &hKeyOpen))           == ERROR_SUCCESS )
        {
            // Create my quickres key
            if (RegReturn=RegCreateKeyEx(hKeyOpen, QUICKRES_KEY, 0, 0, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hKeyCreate, &Disposition)       == ERROR_SUCCESS)
            {
                bRet = TRUE;
            }
...
BOOL ReadRegistryValue( LPTSTR ValueName, PDWORD KeyType, PVOID Value, PDWORD RegKeySize )
{
    ...
    if( (RegReturn=RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_QUICKRES, 0, KEY_QUERY_VALUE, &hKeyOpen))           == ERROR_SUCCESS )
    {

        // Try to get the value


        if ( (RegReturn=RegQueryValueEx(hKeyOpen, ValueName, NULL, KeyType, (LPBYTE)Value, RegKeySize))      == ERROR_SUCCESS )
        {
            ret = TRUE;
        }
...
VOID SetRegistryValue(LPTSTR ValueName, UINT ValueType, PVOID Value, UINT size)
{
    ...
    if( (RegReturn=RegOpenKeyEx(HKEY_CURRENT_USER, REGSTR_QUICKRES, 0, KEY_WRITE, &hKeyOpen))           == ERROR_SUCCESS )
    {
        // Set the value under that key
        RegSetValueEx( hKeyOpen, ValueName, 0, ValueType, (LPBYTE)Value, size );
        RegCloseKey(hKeyOpen);
    }
}
#endif  // SAVEFLAGS
