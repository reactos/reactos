        ...
        case WM_COMMAND:
        {
            switch (LOWORD(wParam))
            {
                ...
                case MENU_PROPERTIES:
                    DoProperties();// Start control panel applet
                    break;
                case MENU_ABOUT:
                    MsgBox(IDS_ABOUT, 0, MB_OK );// Show a generic about box
                    break;
                case MENU_OPTIONS:
                ...
                default:
                {
                    ...
                    if( OffsetPdev >= 0 && OffsetPdev < pMonitors[iDisplay].iModes )
                    {
                        ...
                        if ( CompareDevmodes( &pMonitors[iDisplay].pModes[OffsetPdev], pMonitors[iDisplay].pCurrentdm) )
                        {
...
//  Generic messagebox function that can print a value into a format string
int MsgBox(int id, UINT value, UINT flags)
{
    ...
    //  Ignore tray clicks while msgbox is up, and Show at least an OK button.
    Waiting = TRUE;
    if (flags == 0)
    {
    ...
    if (value)
    {
        ...
        {
            if (msgboxtext = LocalAlloc ( LPTR, sizeof(TCHAR)* (lstrlen(msgboxfmt)+INT_FORMAT_TO_5_DIGITS+1)))
            {
