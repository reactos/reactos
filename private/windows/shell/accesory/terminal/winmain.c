...
VOID FAR mainProcess()
{
   ...
   commThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)checkCommEvent, (LONG)dwTimerRes, CREATE_SUSPENDED | STANDARD_RIGHTS_REQUIRED, (LPDWORD) &dwThreadID);
   if (commThread)
   {
...
DWORD APIENTRY idleProcess(VOID)
{
    ...
    if(!gbXferActive)
    {
        ...
        {
            gbXferActive = TRUE;
            hThread = CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE)xSndThread, (LPVOID)GetCurrentThreadId(), TRUE, &dwId);
        }
        else if(xferFlag == XFRBRCV)
        {
            gbXferActive = TRUE;
            hThread = CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE)xRcvThread, (LPVOID)GetCurrentThreadId(), TRUE, &dwId);
        }
        else
        {
            if (activTerm)
