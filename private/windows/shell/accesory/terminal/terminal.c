...
int APIENTRY WinMain(HANDLE hInstance, HANDLE hPrevInstance, LPSTR lpszCmdLine, INT cmdShow)
{
   ...
   if (lpfnRegisterPenApp = (VOID *)GetProcAddress((HANDLE)GetSystemMetrics(SM_PENWINDOWS), "RegisterPenApp"))
...
BOOL createWindows(INT   cmdShow)
{
   ...
   if(!(hdbXferCtrls = CreateDialog(hInst, getResId(IDDBXFERCTRLS), hTermWnd, (WNDPROC)lpdbmyControls)))
   {
    return(FALSE);
   }
...
if (!RegOpenKeyEx (HKEY_LOCAL_MACHINE, szRegSerialMap, 0L, KEY_READ, &hkey))
     {
         dwBufz = sizeof(szSerial);
         dwSize = sizeof(szCom);
         nEntries = i = 0;

         while (!RegEnumValue (hkey, i++, szSerial, &dwBufz, NULL, &dwType, szCom, &dwSize))
         {
         if (dwType != REG_SZ)
             continue;
