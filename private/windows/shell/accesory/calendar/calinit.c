...
BOOL APIENTRY   ProcessShellOptions(LPSTR lpszCmdLine);
    ...
    INT    cmdShow)
{
    ...
    *vszCustFilterSpec = '\0';
    ...
    if (hPrevInstance == (HANDLE) NULL)
    {
        ...
        WndClass.lpszClassName = (LPSTR)"CalWndMain", WndClass.hInstance = hInstance;
        WndClass.style = CS_VREDRAW | CS_HREDRAW | CS_DBLCLKS | CS_BYTEALIGNCLIENT;
    ...
    /* Calculate the window heights.  All heights are client rectangle
       ...
       borders.
       */
    vcyWnd2A = max(vcyExtLead + vcyLineToLine, vcyHScrollBar + 2 * vcyBorder);
    ...
    /* Note - the assumption is that numeric digits will not have
       ...
       */
    /* changed from 6 to 9   */
    vcyWnd2BBot = 9 * (6 * vcyBorder + max(vcyBorder - vcyDescent, 0) + vcyFont) + vcyBorder;
    ...
    vcyWnd1 = vcyBorder + (vycoNotesBox = vcyWnd2A + vcyWnd2B) + vcyBorder + vcyExtLead + cyWnd2C + vcyBorder;
    ...
    vcxWnd1 = max(
        /* width of header */
        (cchLongDateMax + cchTimeMax + 4) * vcxFont + 2 * vcxHScrollBar + 3 * vcxBorder,
    ...
    vycoQdMax = (vycoQdFirst = 2 + (cyUseable % vcyLineToLine) / 2) + vcln * vcyLineToLine;
    ...
    if (!(vhwnd1 = CreateWindow((LPSTR) "CalWndSub", (LPSTR) NULL, WS_VISIBLE | WS_CHILD | WS_BORDER, XcoWnd1(), YcoWnd1(), vcxWnd1, vcyWnd1, vhwnd0, NULL, hInstance, NULL)))
    {
    ...
    if (!(vhwnd2A = CreateWindow((LPSTR) "CalWndSub", (LPSTR) NULL, WS_VISIBLE | WS_CHILD, 0, 0, vcxWnd2A, vcyWnd2A, vhwnd1, NULL, hInstance, NULL)))
    {
        DestroyWindow(vhwnd0);
        return FALSE;
    }
    ...
    if (!(vhScrollWnd = CreateWindow((LPSTR) "scrollbar", (LPSTR) NULL, WS_VISIBLE | WS_CHILD | SBS_HORZ, vxcoLeftArrowFirst, 0, (vcxHScrollBar + vcxBorder) << 1,
        vcyWnd2A, vhwnd2A, (HMENU) IDHORZSCROLL, hInstance, NULL)))
    {
        DestroyWindow(vhwnd0);
        return FALSE;
    }
    ...
    /* Create window CalWnd2B.  Note - use width of vcxWnd1 - vcxBorder
       ...
       */
    if (!(vhwnd2B = CreateWindow((LPSTR) "CalWndSub", (LPSTR) NULL, WS_VISIBLE | WS_CHILD | WS_VSCROLL | WS_HSCROLL, 0, vcyWnd2A, vcxWnd1 - vcxBorder, vcyWnd2B, vhwnd1, NULL, hInstance, NULL)))
    {
        DestroyWindow(vhwnd0);
        return FALSE;
    }
    ...
    if (!(vhwnd2C = CreateWindow((LPSTR) "Edit", (LPSTR) NULL, WS_VISIBLE | WS_CHILD | ES_MULTILINE, vxcoWnd2C = (vcxWnd2A - cxWnd2C) / 2,
        vycoWnd2C = vcyWnd1 - 2 * vcyBorder - cyWnd2C, cxWnd2C, cyWnd2C, vhwnd1, (HMENU) IDECNOTES, hInstance, NULL)))
    {
        DestroyWindow(vhwnd0);
        return FALSE;
    }
    ...
    if (!(vhwnd3 = CreateWindow((LPSTR) "Edit", (LPSTR) NULL, WS_CHILD | ES_AUTOHSCROLL, 0, 0, CW_USEDEFAULT, CW_USEDEFAULT, vhwnd2B, (HMENU) IDECQD, hInstance, NULL)))
    {
        DestroyWindow(vhwnd0);
        return FALSE;
    }
    ...
    {
        AlertBox(vrgsz[IDS_NOTIMER], (CHAR *) NULL, MB_SYSTEMMODAL | MB_OK | MB_ICONHAND);
        DestroyWindow(vhwnd0);
        return FALSE;
    }
#endif /* #ifdef DOTIMER */
    ...
    else
    {
        ...
        hFile[IDFILEORIGINAL] = MOpenFile(lpszCmdLine, (OFSTRUCT FAR *)&OFStruct[IDFILEORIGINAL], OF_READ);
        LoadCal();
    }
...
BOOL APIENTRY   ProcessShellOptions(LPSTR lpszCmdLine)
{
    ...
    hFile[IDFILEORIGINAL] = MOpenFile(szFileName, &OFStruct[IDFILEORIGINAL], OF_READ);
