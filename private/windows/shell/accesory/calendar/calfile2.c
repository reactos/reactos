     ...
     DD   *pddMax)                 /* Pointer beyond locked tdd. */
{
    ...
    if (idFileSource == IDFILEORIGINAL && !vfOriginalFile || idFileSource == IDFILECHANGE && !vfChangeFile)
        return (TRUE);
    ...
    for (pddCur = pddFirst; pddCur < pddMax; pddCur++)
    {
        /* If this date has not already been transferred, and it's not
           ...
           */
        if (pddCur->dlSave == DLNOCHANGE && (dl = pddCur->dl) < DLSPECIALLOW && ((dl & DLFCHANGEFILEMASK) && idFileSource == IDFILECHANGE
                                   ...
                                   */
{
    ...
    /* Delete the file being overwritten.  Note that this is the
       ...
       */
    if ((FileHandle = MOpenFile((LPSTR) szFileSpec, (OFSTRUCT FAR *)&OFStruct[IDFILEORIGINAL], OF_PROMPT | OF_CANCEL | OF_READWRITE))
        != -1)
    {
        M_lclose(FileHandle);
    ...
    vfDirty = FALSE;/* Everything is clean now. */
    SetTitle(szFileSpec);/* Set the new title. */
    HourGlassOff();/* The waiting is over. */
    return (TRUE);
...
error1:   /* Error attempting to create the new file. */
    ...
    AlertBox(vszSaveFailed, (CHAR *) NULL, MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION);
...
/*
Reconnect - reconnect DRs with their DDs, and make sure the selected date is in memory.
*/
{
    register WORD idr;
    register DR *pdr;
    ...
    {
        pdr = PdrLock(idr);/* Get a pointer to the DR. */
...
BOOL APIENTRY FFlushDr ()
{
    ...
    for (idr = 0, fOk = TRUE; idr < CDR && fOk; idr++)
    {
        ...
        if (pdr->dt != DTNIL)
        {
            ...
            TddUnlock();/* Unlock the tdd. */
        }

        DrUnlock(idr);/* Unlock the DR. */
    }
...
/**** FReopenFile - */
BOOL APIENTRY FReopenFile(INT  idFile, WORD wFlags)
{
    hFile[idFile] = MOpenFile((LPSTR) vrgsz[IDS_GIVEMEFIRST + idFile], (OFSTRUCT FAR *)&OFStruct[idFile], wFlags);
...
VOID APIENTRY LoadCal ()
{
   ...
   not attempt to save in the end */
    ...
    if ((FileHandle = MOpenFile(szNewFileSpec, (OFSTRUCT FAR *)&OFStruct[IDFILEORIGINAL], OF_READWRITE | BLOCKWRITE)) == -1)
    {
        FileHandle = MOpenFile(szNewFileSpec, (OFSTRUCT FAR *)&OFStruct[IDFILEORIGINAL], OF_READ);
    ...
    if (!(fOk = M_llseek(FileHandle, (LONG) 0, 0) != -1 && _lread(FileHandle, bkBuf, CBBK) == CBBK))
        goto error1;
    ...
    for (i = 6; i < CBMAGIC; i++)
    {
        if (bkBuf[i] != vrgbMagic[i])
        {
            ...
            AlertBox(vszNotCalFile, (CHAR *) NULL, MB_SYSTEMMODAL | MB_OK | MB_ICONEXCLAMATION);
            goto error0;
        }
    }
...
error2:   /* Error occurred while attempting to read the tdd or the data for today.
             */
...
error0:   /* Not a calendar file, or couldn't make tdd big enough.
             The error message has already been displayed so the only thing we must do is close the file.
             It's OK for other cases (where the file has already been closed) to come through
