...
*/
{
    ...
    if (fScale)
    {
        SetStretchBltMode(hMemDCdst, bm.bmPlanes * bm.bmBitsPixel == 1 ? BLACKONWHITE : COLORONCOLOR);
        StretchBlt(hMemDCdst,0,0,dx,dy, hMemDCsrc,0,0,bm.bmWidth,bm.bmHeight, SRCCOPY);
    }
    else
    {
...
HPALETTE PUBLIC CopyPalette(HPALETTE hPal)
{
   ...
   pPal = (LOGPALETTE *) LocalAlloc(LPTR, sizeof(LOGPALETTE) + wPalSize * sizeof(PALETTEENTRY));
   if (!pPal)
       goto cleanup;
...
BOOL SnapshotBitmap(HDC hDC, RECT rcDraw)
{
   ...
   MSetBitmapDimension(ghBitmapSnapshot, (int)((long)myBM.bmWidth *254/GetDeviceCaps(hMemDC, LOGPIXELSX)), (int)((long)myBM.bmHeight*254/GetDeviceCaps(hMemDC, LOGPIXELSY)));
...
BOOL PUBLIC DumpBitmapToClipboard(HDC hDC, UINT msg, RECT Rect)
{
   ...
   MSetBitmapDimension(hCopy, (int)((long)myBM.bmWidth *254/GetDeviceCaps(hMemDC, LOGPIXELSX)), (int)((long)myBM.bmHeight*254/GetDeviceCaps(hMemDC, LOGPIXELSY)));
...
void PUBLIC DlgCheckOkEnable(HWND hwnd, int idEdit, UINT message)
{
   ...
   {
       EnableWindow(GetDlgItem(hwnd, IDOK), (BOOL) (SendMessage(GetDlgItem(hwnd, idEdit), WM_GETTEXTLENGTH, 0, 0L)));
   }
}
