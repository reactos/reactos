...
LONG APIENTRY DrawOval(HDC dstDC, LPRECT lprBounds, WPARAM wParam)
{
    ...
    if (GetROP2(dstDC) == R2_COPYPEN)
    {
        ...
        BitBlt(dstDC, rcTemp.left - imageView.left, rcTemp.top - imageView.top, rcTemp.right - rcTemp.left, rcTemp.bottom - rcTemp.top, hdcWork, rcTemp.left, rcTemp.top, SRCCOPY);
...
void OvalDP(HWND hWnd, UINT message, WPARAM wParam, LONG lParam)
{
    ...
    if (message == WM_LBUTTONDOWN)
    {
        ...
        if (TrackTool(hWnd, DrawOval, &rcReturn, &wParam, NULL) != WM_RBUTTONDOWN)
        {
            ...
            if (theSize > 0)
            {
                pen = CreatePen(PS_INSIDEFRAME, theSize, rgbColor[(theTool == OVALFRAMEtool) ? theForeg : theBackg]);
            }
            else
            {
