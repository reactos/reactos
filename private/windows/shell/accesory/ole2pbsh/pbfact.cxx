...
extern "C" BOOL CreatePBClassFactory(HINSTANCE hinst,BOOL fEmbedded)
{
    ...
    if(PBFactory::Create(hinst))
    {
        ...
        if(fEmbedded)
        {
            ...
            HRESULT hr = CoRegisterClassObject(CLSID_PBRUSH32, (LPUNKNOWN)(LPCLASSFACTORY)gpPBFactory, CLSCTX_LOCAL_SERVER, REGCLS_SINGLEUSE, &gdwRegisterClass);
            if(OK(hr))
            {
                CoLockObjectExternal((LPUNKNOWN)(LPCLASSFACTORY)gpPBFactory, TRUE, TRUE);
                fRet = TRUE;
            }
#if DBG
            else
            {
                ...
                wsprintf(achBuffer, L"PBrush: CoRegisterClassObject (%lx)\r\n", (long)hr);
                DOUT(achBuffer);
            }
#endif
        }
