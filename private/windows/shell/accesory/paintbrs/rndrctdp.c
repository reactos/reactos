...
LONG APIENTRY DrawRndRct(HDC dstDC, LPRECT lprBounds, WPARAM wParam)
{
    ...
    if (GetROP2(dstDC) == R2_COPYPEN) {
        ...
        RoundRect(hdcWork, rcTemp.left, rcTemp.top, rcTemp.right, rcTemp.bottom, xRoundMax, yRoundMax);
        ...
        RoundRect(dstDC, rcTemp.left, rcTemp.top, rcTemp.right, rcTemp.bottom, xRoundMax, yRoundMax);
...
void RndRctDP(HWND hWnd, UINT message, WPARAM wParam, LONG lParam)
{
    ...
    if (message == WM_LBUTTONDOWN) {
        ...
        if (TrackTool(hWnd, DrawRndRct, &rcReturn, &wParam, NULL) != WM_RBUTTONDOWN) {
            ...
            if (theSize > 0)
                pen = CreatePen(PS_INSIDEFRAME, theSize, rgbColor[(theTool == RNDRECTFRAMEtool) ? theForeg : theBackg]);
            else
