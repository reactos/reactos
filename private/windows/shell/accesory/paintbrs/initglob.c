...
#ifdef JAPAN
extern BOOL bWritable; // This flag identifies whether the Font is possible to write vertically or not.
extern BOOL bVertical; // This flag identifies whether user chose the Vertical-Writing option or not.
#endif
...
/*
Function to determine if the command line passed to PBRUSH contains the print file command, i.e. "pbrush /p <filename>".
*/
...
{
    if (lpLogPen->lopnStyle == PS_SOLID && !(lpLogPen->lopnColor & 0xff000000L))
    {
        if (cntr < MAXcolors)
...
static void NEAR PASCAL FindBestColors(HDC hDC, HANDLE hInstApp)
{
    ...
    for (i = 0; i < MAXcolors; ++i)
    {
        ...
        for (j = 0; j < cntr; ++j)
        {
            ...
            dCol = devColors[j];
            rCol = defltColor[i];
            nDiff = abs(SBYTE(dCol, 0) - SBYTE(rCol, 0)) + abs(SBYTE(dCol, 1) - SBYTE(rCol, 1)) + abs(SBYTE(dCol, 2) - SBYTE(rCol, 2));
            if (maxDiff > nDiff)
            {
                maxDiff = nDiff;
                indexi = i;
 ...
 * Used to get around the OLE limitations on the insignia 286 interpreter, where DDE can not copy data >= 1 Meg to a 16 bit app.
*/
BOOL OnRISC(void)
{
    SYSTEM_INFO si;
...
WndInitGlob(HINSTANCE hInst, LPTSTR lpCmdLine, int cmdShow)
{
    ...
    /* Let Commdlg retrieve default printer so we can modify printer settings with hDevMode.
     */
    ...
    {   /* Are we in real mode today? */
        /* OLE:  Initialize the Server, Document, Item Virtual Tables */
        /* NOTE:  This must be done before the main window is created */
        InitVTbls(hInst);

        fOLE = InitServer(hInst);/* OLE server initialization call */
    }

    InitNewDialogs(hInst);/* Initialize the new dialogs */
    ...
    pbrushWnd[PARENTid] = CreateWindow(
        pbrushWndClass[PARENTid],
        pgmTitle,
     ...
     */
#ifndef WIN32
    if (!(lpMouseDlg = MakeProcInstance(MouseDlg, hInst)) || !(lpColorDlg = MakeProcInstance(ColorDlg, hInst)) || !(lpNullWP = MakeProcInstance(NullWP, hInst)))
        goto NoMemory;
#endif
...
#ifdef PENWIN
    if (lpfnRegisterPenApp = GetProcAddress((HANDLE) GetSystemMetrics(SM_PENWINDOWS), "RegisterPenApp")) (*lpfnRegisterPenApp)(1, TRUE);
    {
    ...
    /* 09-Jan-1993 - Jonma - change this to SM_C<n>FULLSCREEN so that we can paste in full-screen bitmaps */
#ifdef OLDCODE
    ...
    if (parentDC = GetDisplayDC(pbrushWnd[PARENTid]))
    {
        ...
        horzDotsMM = (int) ((GetDeviceCaps(parentDC, HORZRES) * 100L) / GetDeviceCaps(parentDC, HORZSIZE));
        vertDotsMM = (int) ((GetDeviceCaps(parentDC, VERTRES) * 100L) / GetDeviceCaps(parentDC, VERTSIZE));
    ...
    if (fOLE && (fServer = (Contains(lpCmdLine, szEmbedding) || Contains(lpCmdLine, szEmbedding2))))
    {
        DB_OUT("ig: fInv=TRUE ");
        fInvisible = TRUE;
    }
    ...
    for (i = 1; i < MAXwnds; ++i)
    {
        pbrushWnd[i] = CreateWindow(
            pbrushWndClass[i],
            (LPTSTR) NULL,
            WS_CHILD | WS_CLIPSIBLINGS | ((TOOLid == i) ? 0L : WS_BORDER) | ((PAINTid == i) ? (WS_VSCROLL | WS_HSCROLL) : 0L),
            pbrushRct[i].left,
            pbrushRct[i].top,
     ...
     * It fixes the problem where we can't send more than 1 meg across the 16 bit OLE (KRNL286.EXE).
     * If we ever get a 386 capable NTVDM, then the code inside the following 'if' should be removed.
     */
    if (OnRISC() && fInvisible && (nNewImageHeight * (((nNewImageWidth * cPlanes * cbitsPixel) + 31) & ~31) / 8) > MAX_286_OLE_BM_SIZE)
    {
        /*
         * The default picture size is too big to be mashalled across the 16 bit OLE layer.
         * Scale it down till it is less than one meg.
         */
        int xNew, yNew;
        int cPixels = MAX_286_OLE_BM_SIZE * 8 / (cPlanes * cbitsPixel);

        xNew = (int) sqrt((double) (cPixels * nNewImageWidth / nNewImageHeight));
        yNew = MAX_286_OLE_BM_SIZE / (((cPlanes * cbitsPixel * xNew + 31) & ~31) / 8);
    ...
    SendMessage(pbrushWnd[PARENTid], WM_SIZE, SIZENORMAL, MAKELONG(rWind.right, rWind.bottom));
...
void SetupFileVars(LPTSTR szPath)
{
    LPTSTR lpStr, pPtr;
    int cch;
    ...
    if (*szPath)
    {
        ...
        for (lpStr = szPath + lstrlen(szPath); lpStr > szPath && !IsPathSep(*lpStr); lpStr = CharPrev(szPath, lpStr))
            /* do nothing */;
        ...
        pPtr = pathPtr = filePath;/* save the path */
        ...
        *pPtr = TEXT('\0');
        ...
        if (pPtr != pathPtr)
        {
            ...
            if ((*pPtr != TEXT('\\') && *pPtr != TEXT('/')) || (pPtr > pathPtr && pPtr[-1] == TEXT(':')))
            {
                pPtr++;
            }
        }
        ...
        *tempName = *tempPath = TEXT('\0');/* load the file */
    }
}
