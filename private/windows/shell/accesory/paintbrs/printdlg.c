...
static BOOL PRIVATE ValidateMargins(NUM nmLeft, NUM nmTop, NUM nmRight, NUM nmBottom)
{
   NUM nmWidth, nmHeight;

   if(NLTI(nmLeft, 0) || NLTI(nmTop, 0) || NLTI(nmRight, 0) || NLTI(nmBottom, 0)) {
   ...
   if(NLEN(nmWidth, NAddN(nmLeft, nmRight)) || NLEN(nmHeight, NAddN(nmTop, nmBottom))) {
...
void PUBLIC ComputePrintRect(NPRECT npImageRect, NPRECT npPrintRect, NPRECT npHeaderRect, NPRECT npFooterRect)
{
...
//   RECT headerRect, footerRect;
   ...
   case WM_COMMAND:
      ...
      case IDOK:
         ...
         if(!printDC || Escape(printDC, GETSCALINGFACTOR, 0, NULL, (LPVOID)&sFactor) <= 0)
...
/* Actually, I want to round up in the next line, but +1 will do */
         nSizeMin = (int)max(max(hResPrt*xLogImage/(xLogPrint*0x7fffL) + 1, vResPrt*yLogImage/(yLogPrint*0x7fffL) + 1), 1);
         nSizeMax = (int)min(min(0x7fffL*xLogImage/(imageWid*xLogPrint), 0x7fffL*yLogImage/(imageHgt*yLogPrint)), 10000);
...
/**      DrawProc = NULL;  **/  /* signal that IDPARTIAL wasn't selected */
      ...
      case IDWHOLE:
      case IDPARTIAL:
         CheckRadioButton(hDlg, IDWHOLE, IDPARTIAL, amount = GET_WM_COMMAND_ID(wParam,lParam));
         break;
      }
      break;
   case WM_INITDIALOG:
...
BOOL FAR PASCAL PageSetDlg(HWND hDlg, UINT message, WPARAM wParam, LONG lParam)
{
   ...
   case WM_COMMAND:
      ...
      case IDOK:
         ...
         /* get margins */
         if(!GetDlgItemNum(hDlg, IDLEFT  , &tnmLeft) || !GetDlgItemNum(hDlg, IDTOP    , &tnmTop) || !GetDlgItemNum(hDlg, IDRIGHT  , &tnmRight) || !GetDlgItemNum(hDlg, IDBOTTOM , &tnmBottom)) {
            PbrushOkError(IDSInvalidNumb, MB_ICONHAND);
            break;
         }
