...
#include "pbserver.h"           /* OLE server functions and variables */
#ifdef DBCS_IME
#include <winnls.h>
#endif
...
long FAR PASCAL ParentWP(HWND hWnd, UINT message, WPARAM wParam, LONG lParam)
{
   ...
   case WM_INITMENU:
      SendMessage(pbrushWnd[PAINTid], WM_HIDECURSOR, 0, 0l);
      hMenu = GetMenu(hWnd);
      EnableMenuItem(hMenu, EDITpaste,
            (!inMagnify &&
      ...
      CheckMenuItem(hMenu, MISCOmitPictureFormat,
                    (MF_BYCOMMAND | (fOmitPictureFormat ?  MF_CHECKED : MF_UNCHECKED)));
      if(bPrtCreateErr) {
         bPrtCreateErr = FALSE;
         GetPrintParms(NULL);
      }
      EnableMenuItem(hMenu, FILEprint, bPrtCreateErr? MF_GRAYED : MF_ENABLED);

//      if (!fOLE) {
//          EnableMenuItem(hMenu, EDITcutpict, MF_GRAYED);
//          EnableMenuItem(hMenu, EDITcopypict, MF_GRAYED);
//      }
       break;

   case WM_COMMAND:
      if(pbrushWnd[PARENTid] && (GET_WM_COMMAND_ID(wParam,lParam)!=STYLEitalic
                                      || processCntrlI))
   ...
   case WM_SIZE:
      ...
      if(wParam != SIZEICONIC) {
         ...
         if(pbrushWnd[PAINTid] && IsWindow(pbrushWnd[PAINTid])) {
            CalcWnds(NOCHANGEWINDOW, NOCHANGEWINDOW,
                  NOCHANGEWINDOW, NOCHANGEWINDOW);
            for(i=1; i<MAXwnds; ++i)
               MoveWindow(pbrushWnd[i],pbrushRct[i].left,
                     pbrushRct[i].top,
      ...
      break;
        ...
        case WM_QUERYENDSESSION:
            ...
            if(message == WM_CLOSE)
            {
                if (fOLE)
                {
                    /* OLE:  Send document change message.  Only one document! */
                    /* If user has chosen not to update, return NULL data from
                     * ItemGetData(). fSendData takes care of this. */
                    if (answer == IDNO)
                        fSendData = FALSE;
                    DeleteDoc(vpdoc);
                    fSendData = TRUE;
                ...
                /* If OLE is active the window is destroyed at SrvrRelease() */
                if (!fOLE)
                    DestroyWindow(hWnd);
            }
            else
   ...
   case WM_ACTIVATEAPP:
      ...
      if(pbrushWnd[PAINTid]) {
         if(wParam == 0) {
            ...
            InvalidateRect(pbrushWnd[PAINTid], (LPRECT )NULL, FALSE);
...
MovePaintCurs:
   ...
   case PBM_INITFONTPRINT:
      oldcsr = SetCursor(LoadCursor(NULL, IDC_WAIT));

      if (message != PBM_INITFONTPRINT) {
...
BOOL FAR SaveAsNeeded(void) {
    ...
    if (imageFlag) {
        switch (SimpleMessage((WORD)(vfIsLink ? IDSSaveTo : IDS_MAYBEUPDATE),
            ...
            case IDYES:
                /* For linked objects save the file
                 * For embedded object, just return IDYES which will cause
                 * a OLE_CHANGE from DeleteDoc() */
                if (vfIsLink)
                {
