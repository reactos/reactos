...
VOID APIENTRY PaintMonth (HDC hDC)
     ...
     INT  *pxcoCur;
     INT  *pycoCur;
     TCHAR rgchDayAbbrevs[4];
     TCHAR rgch[CCHDATEDISP];
     DOSDATE dd;
     ...
     cch = GetMonthYear(&dd, rgch);

     xcoText = (vcxWnd2B - cch * vcxFont) / 2;
     ycoText = 2;

     TextOut(hDC, xcoText, ycoText, rgch, cch);
     ...
     for (cDay = iDayStart; cDay < iDayEnd; cDay++ )
         ...
         TextOut (hDC, xcoText, ycoText, rgchDayAbbrevs, cchT);
         }
     ...
     for (cWeek = iWeekStart ; cWeek < iWeekEnd ; cWeek++,pb+=iDayStart)
          ...
          for (cDay = iDayStart; cDay < iDayEnd ; cDay++, pb++)
               ...
               if  (*pb !=0)
                    ...
                    TextOut (hDC, xcoText, ycoText, pch, cch);
                    }
               }
          }
...
VOID APIENTRY ShowToday (
     ...
     INT  dx)
     {
     TextOut (hDC, xcoBox + 1, ycoText, TEXT(">"), 1);
     TextOut (hDC, xcoBox + dx - vcxFont - 2, ycoText, TEXT("<"), 1);
     }
...
DT   APIENTRY FAR DtFromPd3 (D3 *pd3)
     ...
     dt = (WORD) pd3->wYear * 365;

     /* Add in the days for the preceding leap years. */
     if (pd3->wYear != 0)
          dt += 1 + (WORD) (pd3->wYear - 1) / 4;
...
VOID APIENTRY MonthMode ()
     ...
     if (vfDayMode)
          ...
          */
          ...
          SetScrollPos (vhwnd2B, SB_VERT, (WORD) vd3Sel.wYear * 12 + vd3Sel.wMonth,
            TRUE);
