...
VOID APIENTRY CalTimer (BOOL fAdjust)
    ...
    if (tmNew != vftCur.tm || d3New.wMonth != vd3Cur.wMonth || d3New.wDay != vd3Cur.wDay || d3New.wYear != vd3Cur.wYear)
         {
         ...
         if (!IsIconic(vhwnd0))
           ...
           if ((dtNew = DtFromPd3 (&d3New)) != vftCur.dt)
                ...
                if (!vfDayMode)
                     ...
                     */
                     if ((vd3Cur.wMonth == vd3Sel.wMonth && vd3Cur.wYear == vd3Sel.wYear) || (d3New.wMonth == vd3Sel.wMonth && d3New.wYear == vd3Sel.wYear))
                           {
