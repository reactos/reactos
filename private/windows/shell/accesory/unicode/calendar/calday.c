          ...
          if (tmFirstOld != TMNOON)
...
#ifdef DBCS
               {
                   ...
                   if (!vfHour24 && !Time.iTime && fTimePrefix)
                   {
                      ...
                      TextOut( hDC, vxcoApptTime, vycoQdFirst,
                           vszBlankString, cchTimeMaxDBCS+2 );
                      wLen = GetTimePrefixLength( hDC, sz );
                      TextOut (hDC, vxcoApptTime+wLen, vycoQdFirst, sz, 6);
                   }
                   else
                       TextOut( hDC, vxcoApptTime + sizeTime.cx, vycoQdFirst,
                                vszBlankString, cchTimeMaxDBCS+2 );
#else
                   TextOut (hDC, vxcoApptTime + sizeTime.cx, vycoQdFirst,
                            vszBlankString, cchTimeMaxDBCS+2);
               ...
               TextOut (hDC, vxcoApptTime + size.cx + 19, vycoQdFirst,
                                 TEXT("              "),12);
#endif
               ReleaseDC (vhwnd2B, hDC);
               }
