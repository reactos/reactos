...
WORD GetTimePrefixLength(HDC, LPTSTR);
#endif
...
VOID APIENTRY DayPaint(HDC  hDC)
{
    ...
    for (ln = 0; ln < vcln; ln++)
    {
        ...
        /* Display am or pm only for the first appointment in the window
           ...
           */
#if defined (JAPAN)||defined(KOREA) // added 29 May 1992  by Hiraisi JeeP, 10/13/92
        if (!vfHour24 && !Time.iTime && fTimePrefix)
        {
            if (ln == 0 || tm == TMNOON)
                TextOut(hDC, vxcoApptTime, ycoText, sz, cch);
            else
            {
                WORD wLen;

                cch = 6;
                wLen = GetTimePrefixLength(hDC, sz);
                TextOut(hDC, vxcoApptTime + wLen, ycoText, sz, cch);
            }
        }
        else
        {
        ...
        if (vtld[ln].otqr != OTQRNIL)
            pchQd = (TCHAR *) (pbTqr + vtld[ln].otqr + CBQRHEAD);

        DrawText(hDC, (LPTSTR) pchQd, -1, (LPRECT) &rectQd,
        ...
        if (ln == vlnCur)
        {
            ...
            //  added 01 Jun. 1992  by Hiraisi
            /*
             * The reason is as follows.
             *     The length of time prefix may be changed
             *   when "calendar" is active.
             */
            MoveWindow(vhwnd3, vxcoQdFirst, YcoFromLn(ln), vxcoQdMax - vxcoQdFirst, vcyFont, FALSE);
#endif
        }
    }
    DrUnlockCur();
...
{
    BltByte((BYTE *) vtld, (BYTE *) (vtld + cld), (WORD) ((vcln - cld) * sizeof(LD)));
}
...
{
    BltByte((BYTE *) (vtld + cld), (BYTE *) vtld, (WORD) ((vcln - cld) * sizeof(LD)));
}


/**** FGetNextLd */

BOOL APIENTRY FGetNextLd(TM   tm, LD   *pld)
{
    TM   tmFromTqr;
    DR   *pdr;
...
BOOL APIENTRY FGetPrevLd(TM   tm, LD   *pld)
{
    TM   tmFromTqr;
    TM   tmInterval;
...
BOOL APIENTRY FScrollDay(INT  code, WORD posNew)
{
    wID = code;
...
VOID APIENTRY ScrollDownDay(INT  ctmNew, BOOL fScrollBar, BOOL fSizing)
{
    register INT ctm;
    register INT ln;
    LD   ldTemp;
...
#ifdef DBCS
            {
                extern int cchTimeMaxDBCS;   // Initialized in 'CalInit()'
                SIZE       sizeTime;

                GetTextExtentPoint(hDC, sz, 5, &sizeTime);
#if defined (JAPAN)||defined(KOREA) //  added 29 May 1992  by Hiraisi, JeeP 10/13/92
                if (!vfHour24 && !Time.iTime && fTimePrefix)
                {
                    WORD wLen;

                    TextOut(hDC, vxcoApptTime, vycoQdFirst, vszBlankString, cchTimeMaxDBCS + 2);
                    wLen = GetTimePrefixLength(hDC, sz);
                    TextOut(hDC, vxcoApptTime + wLen, vycoQdFirst, sz, 6);
                }
                else
                    TextOut(hDC, vxcoApptTime + sizeTime.cx, vycoQdFirst, vszBlankString, cchTimeMaxDBCS + 2);
#else
                TextOut(hDC, vxcoApptTime + sizeTime.cx, vycoQdFirst, vszBlankString, cchTimeMaxDBCS + 2);
#endif
            }
#else
            //MGetTextExtent(hDC, sz1, 5, &iHeight, &iWidth); /* width of time string
            GetTextExtentPoint(hDC, sz, 5, &size); /* width of time string // lhb tracks
                                                            + the blank following it */
            //- KLUDGE: TextOut (hDC, vxcoApptTime + lpSize->cx, vycoQdFirst,
            //- KLUDGE:          vszBlankString,cchTimeMax+3);
            //- For some reason, the above code no longer blanks out the
            //- correct area.  It puts a space in the center of the time.
            TextOut(hDC, vxcoApptTime + size.cx + 19, vycoQdFirst, TEXT("              "), 12);
#endif
            ReleaseDC(vhwnd2B, hDC);
        }
...
VOID APIENTRY ScrollUpDay(INT  ctmNew, BOOL fScrollBar)
{
    register INT ctm;
    register INT ln;
    LD   ldTemp;
    ...
    if (ctm != 0)
    {
        ...
        if (!vfHour24)
        {
            ...
            TextOut(hDC, vxcoApptTime, vycoQdFirst, sz, cch);
#endif
            ReleaseDC(vhwnd2B, hDC);
        }
...
#if defined (JAPAN)||defined(KOREA) //  added 29 May 1992  by Hiraisi
/*
 *  This function gets length of TimePrefix,
 * and increases point of TimePrefix buffer.
*/
WORD GetTimePrefixLength(HDC hDC, LPTSTR sz)
{
    INT   iLen;
    LPTSTR sz2;
    SIZE  size;
    ...
    sz2 = sz;
    for (iLen = 0; *sz2 != TEXT(' '); sz2++, iLen++)
        ;

    GetTextExtentPoint(hDC, sz, iLen, &size);
    lstrcpy(sz, sz2);
