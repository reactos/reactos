...
WORD id;
{
    ...
    if ((h = FindResource (hinstTimeDate, (LPTSTR) MAKEINTRESOURCE(IDS_DATESTRINGS >> 4), RT_STRING)) && (h = LoadResource(hinstTimeDate, h)))
    {
        GlobalLock(h);
    }
}
...
VOID FAR APIENTRY InitTimeDate (HANDLE hInstance)
{
  ...
  extern TCHAR    szDec[5];
    ...
    lcid = GetUserDefaultLCID ();/* get current locale */
    ...
    GetLocaleInfoW (lcid, LOCALE_SSHORTDATE, (LPWSTR) Date.szShortFmt, MAX_SHORTFMT);/* Get short date format */
    GetLocaleInfoW (lcid, LOCALE_SLONGDATE, (LPWSTR) Date.szLongFmt, CCHDATEDISP);/* Get long date format */
    ...
    /* this is sum of separator strings, plus max of months, plus max of weekdays, plus 2 digits, plus 4 digits, plus zero terminator */
    cchLongDateMax = 2 + 4;
    ...
    GetLocaleInfoW(lcid, LOCALE_SDECIMAL, szDec, CharSizeOf(szDec));/* Get the intl decimal character for use in Page Setup Box. */
}