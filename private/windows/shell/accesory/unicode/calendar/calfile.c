     ...
     */
     if (!(vfChangeFile = FCreateTempFile (IDFILECHANGE, 0)))
          ...
          /* Post error saying edits will not be recorded. */
                  OutputDebugString (TEXT("Message Box Is Broken\n"));
...
BOOL APIENTRY FCreateTempFile (
     ...
     INT  iDrive)        /* 0 means let GetTempFileName decide where to put
                          ...
                          */
{
    //TCHAR  szFileSpec [CCHFILESPECMAX];
    TCHAR  szTempPath [CCHFILESPECMAX];
    HANDLE FileHandle;
         ...
         */
        GetTempPath ( CCHFILESPECMAX, szTempPath) ;
        if (GetTempFileName (szTempPath, TEXT("CAL"), 0, vszOFSFileSpec[idFile]) == 0)
        {
                return (FALSE);
        }

        if ((FileHandle = CreateFile(vszOFSFileSpec[idFile],
     ...
     if (!vfChangeFile)
          ...
          */
          AlertBox (vszNoChangeFile,
           (TCHAR *)NULL, MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION);
          *pdl = DLNOCHANGE;
          return (TRUE);
          }
     ...
     */
     ...
     if (FWriteDrToFile (TRUE, IDFILECHANGE, pdr))
          ...
          *pdl = dlNew;
          ...
          AlertBox (vszErrorWritingChanges, (TCHAR *)NULL,
     ...
     if (!fOpenClose || FReopenFile (idFile, OF_PROMPT | OF_CANCEL | OF_REOPEN | OF_READWRITE))
          ...
          */
          ...
          fOk = M_llseek (FileHandle, (LONG)(CBBK*(*pobkEOD)), 0) != -1
                       && FWriteFile (FileHandle, (BYTE *)pdr, (WORD)cbTransfer);
...
BOOL APIENTRY FReadDrFromFile (
     ...
     */
     ...
     if (M_llseek (FileHandle, (LONG)(CBBK * obk), 0) == -1
              || M_lread (FileHandle, (LPTSTR)pdr, CBDRHEAD) != CBDRHEAD)
     {
     ...
     fOk = (WORD)M_lread (FileHandle, (LPTSTR)((BYTE *)pdr + CBDRHEAD), cbData) == cbData;
