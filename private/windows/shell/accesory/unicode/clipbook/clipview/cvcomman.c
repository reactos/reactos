...
LRESULT OnIDMDelete(HWND hwnd,UINT msg,WPARAM wparam,LPARAM lparam)
{
    int tmp;
    LPLISTENTRY lpLE;
    LISTENTRY   LE;
    ...
    else
    {
        ...
        if (tmp != LB_ERR)
        {
            ...
            if (MessageBox(hwndApp, szBuf, szDelete, MB_ICONEXCLAMATION | MB_OKCANCEL) != IDCANCEL)
            {
                ...
                // Perform an execute to the server to let it know that we're not sharing anymore.
                wsprintf(szBuf, TEXT("%s%s"), SZCMD_DELETE, lpLE->name);
                if (MySyncXact((LPBYTE) szBuf, ByteCountOf(lstrlen(szBuf) + 1), pActiveMDI->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, SHORT_SYNC_TIMEOUT, NULL))
                {
                    if (pActiveMDI->DisplayMode == DSP_PAGE)
                    {
                        PINFO(TEXT("forcing back to list mode\n\r"));
                        SendMessage(hwndApp, WM_COMMAND, pActiveMDI->OldDisplayMode == DSP_PREV ? IDM_PREVIEWS : IDM_LISTVIEW, 0L);
                    }
...
// Purpose: Create and activate a window showing the contents of the clipboard.
...
static void CreateLocalWindow(void)
{
    ...
    pActiveMDI->hszConvPartner = pActiveMDI->hszConvPartnerNP = hszDataSrv;
    pActiveMDI->hExeConv = InitSysConv(hwndLocal, pActiveMDI->hszConvPartner, hszSystem, TRUE);
    if (pActiveMDI->hExeConv)
    {
        ...
        else
        {
            if (!IsIconic(hwndApp))
            {
                ...
                MoveWindow(hwndLocal, MDIrect.left, MDIrect.top, MDIrect.right - MDIrect.left, (MDIrect.bottom - MDIrect.top) - GetSystemMetrics(SM_CYICONSPACING), FALSE);
            }
            else
            {
        ...
        if (NULL != hkeyRoot)
        {
            ...
            if (ERROR_SUCCESS != RegQueryValueEx(hkeyRoot, (LPTSTR) szDefView, NULL, NULL, (LPBYTE) &dwDefView, &dwSize));
            {
    ...
    {
#if DEBUG
        MessageBox(hwndApp, TEXT("No Local Server"), TEXT("ClipBook Initialization"), MB_OK | MB_ICONEXCLAMATION);
#endif
...
void UnsharePage(void)
{
    ...
    if (tmp != LB_ERR)
    {
        ...
        if (lpDdeI = LocalAlloc(LPTR, ByteCountOf(2048)))
        {
            ...
            wAddlItems = 0;
            ret = NDdeShareGetInfo(NULL, lpLE->name, 2, (LPBYTE) lpDdeI, ByteCountOf(2048), &dwRet, &wAddlItems);
            if (NDDE_NO_ERROR == ret)
            {
                ...
                *lpOog = UNSHR_CHAR;
                ...
                ret = NDdeShareSetInfo(NULL, lpLE->name, 2, (LPBYTE) lpDdeI, ByteCountOf(2048), 0);
                if (NDDE_NO_ERROR == ret)
                {
#if 0
                    ...
                    pSD = atch;
                    if (NDDE_NO_ERROR == NDdeGetShareSecurity(NULL, lpLE->name, DACL_SECURITY_INFORMATION, pSD, ByteCountOf(2048), &dwSize))
                    {
                        if (GetSecurityDescriptorDacl(pSD, &fDacl, &pacl, &fDefault))
                        {
                            if (fDefault || !fDacl)
                            {
                                ...
                                {
                                    NDdeSetShareSecurity(NULL, lpLE->name, DACL_SECURITY_INFORMATION, pSD);
                                    LocalFree(pSD);
                                }
                            }
                        }
                    ...
                    if (MySyncXact((LPBYTE) szBuf, ByteCountOf(lstrlen(szBuf) + 1), GETMDIINFO(hwndLocal)->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, SHORT_SYNC_TIMEOUT, NULL))
                    {
                    ...
                    {
                        MessageBoxID(hInst, hwndApp, IDS_INTERNALERR, IDS_APPNAME, MB_OK | MB_ICONSTOP);
...
// Purpose: Set the currently selected page in the active MDI window to 'unshared'.

// Parameters: None.

// Returns: 0L always, function handles its own errors.
LRESULT OnIdmUnshare(void)
{
    DWORD          adwTrust[3];
    int            tmp;
    WORD           wAddlItems;
        ...
        if (lpDdeI = LocalAlloc(LPTR, ByteCountOf(2048)))
        {
            ...
            if (NDDE_NO_ERROR == ret)
            {
                ...
                *lpOog = UNSHR_CHAR;
                ...
                NDdeGetTrustedShare(NULL, lpDdeI->lpszShareName, adwTrust, adwTrust + 1, adwTrust + 2);

                DumpDdeInfo(lpDdeI, NULL);
                ret = NDdeShareSetInfo(NULL, lpDdeI->lpszShareName, 2, (LPBYTE) lpDdeI, ByteCountOf(2048), 0);
                if (NDDE_NO_ERROR == ret)
                {
#if 0
                    PSECURITY_DESCRIPTOR pSD;
                    ...
                    ret = NDdeGetShareSecurity(NULL, lpDdeI->lpszShareName, DACL_SECURITY_INFORMATION, pSD, 30, &dwSize);
                    ...
                    if (NDDE_NO_ERROR == ret)
                    {
                            ...
                            &pacl, &fDefault))
                        {
                            ...
                            if (fDefault || !fDacl)
                            {
                                if (pSD = CurrentUserOnlySD())
                                {
                                    NDdeSetShareSecurity(NULL, lpDdeI->lpszShareName, DACL_SECURITY_INFORMATION, pSD);
                                }
                                else
                                {
...
// Returns: 0L, or DefWindowProc() if wParam isn't a WM_COMMAND id I know about.
LRESULT ClipBookCommand(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    int tmp;
    FARPROC lpProc;
    UINT wNewFormat;
    ...
    case IDM_PERMISSIONS:
        ...
        break;
    case IDC_TOOLBAR:
        MenuHelp(WM_COMMAND, wParam, lParam, GetMenu(hwnd), hInst, hwndStatus, nIDs);
        break;
    case IDM_EXIT:
    ...
    case IDM_TILEVERT:
    case IDM_TILEHORZ:
        SendMessage(hwndMDIClient, WM_MDITILE, wParam == IDM_TILEHORZ ? MDITILE_HORIZONTAL : MDITILE_VERTICAL, 0L);
        break;
    case IDM_CASCADE:
    ...
    case IDM_COPY:
    {
        ...
        // make a copy to ensure that the global is not changed from under us in case proc is reentered
        pMDIc = GETMDIINFO(hwndActiveChild);
        tmp = (int) SendMessage(pMDIc->hWndListbox, LB_GETCURSEL, 0, 0L);
        if (tmp == LB_ERR)
            break;

        if (SendMessage(pMDIc->hWndListbox, LB_GETTEXT, tmp, (LPARAM) (LPTSTR) &lpLE) == LB_ERR)
        {
        ...
        if (!pMDIc->hszClpTopic)
        {
            MessageBoxID(hInst, hwndActiveChild, IDS_DATAUNAVAIL, IDS_APPNAME, MB_OK | MB_ICONEXCLAMATION);
            break;
        }
        ...
        hConv = DdeConnect(idInst, pMDIc->hszConvPartner, pMDIc->hszClpTopic, NULL);
        if (!hConv)
        {
            PERROR(TEXT("DdeConnect to (%s) failed %d\n\r"), (LPTSTR) (lpLE->name), DdeGetLastError(idInst));
            MessageBoxID(hInst, hwndActiveChild, IDS_DATAUNAVAIL, IDS_APPNAME, MB_OK | MB_ICONEXCLAMATION);
            break;
        }
        ...
        DdeClientTransaction(NULL, 0L, hConv, hszFormatList, CF_TEXT, XTYP_REQUEST, (DWORD) TIMEOUT_ASYNC, NULL);
    ...
    case IDM_PREVIEWS:
    {
        ...
        // NOP?
        if (pActiveMDI->DisplayMode == DSP_PREV && wParam == IDM_PREVIEWS || pActiveMDI->DisplayMode == DSP_LIST && wParam == IDM_LISTVIEW)
            break;
        ...
        while (SendMessage(hwndtmp, LB_GETTEXT, 0, (LPARAM) (LPTSTR) &lpLE) != LB_ERR)
        {
            ...
            // add to new listbox
            SendMessage(pActiveMDI->hWndListbox, LB_ADDSTRING, 0, (DWORD) (LPCSTR) lpLE);
        }
        ...
        if (NULL != hkeyRoot)
        {
            ...
            dwValue = pActiveMDI->DisplayMode == DSP_LIST ? IDM_LISTVIEW : pActiveMDI->DisplayMode == DSP_PREV ? IDM_PREVIEWS : IDM_PAGEVIEW;
            RegSetValueEx(hkeyRoot, (LPTSTR) szBuf, 0L, REG_DWORD, (LPBYTE) &dwValue, sizeof(DWORD));
        }
    ...
    case IDM_SAVEAS:
    {
        ...
        if (CountClipboardFormats())
        {
            ...
            {
                if (lpfnSOF = (LPFNGOF) GetProcAddress(hMod, "GetSaveFileNameW"))
                {
                    if ((*lpfnSOF)(&ofn) && szFile[0])
                    {
                        ...
                        wsprintf(szBuf, TEXT("%s%s"), (ofn.nFilterIndex == 1) ? SZCMD_SAVEAS : SZCMD_SAVEASOLD, szFile);
                        if (!MySyncXact((LPBYTE) szBuf, ByteCountOf(lstrlen(szBuf) + 1), GETMDIINFO(hwndLocal)->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, LONG_SYNC_TIMEOUT, NULL))
                        {
    ...
    case IDM_OPEN:
    {
        ...
        if (hMod = LoadLibrary(TEXT("COMDLG32.DLL")))
        {
            if (lpfnGOF = (LPFNGOF) GetProcAddress(hMod, "GetOpenFileNameW"))
            {
                if ((*lpfnGOF)(&ofn) && szFile[0])
                {
                    ...
                    if (ClearClipboard(hwnd))
                    {
                        ...
                        MySyncXact((LPBYTE) szBuf, ByteCountOf(lstrlen(szBuf) + 1), GETMDIINFO(hwndLocal)->hExeConv, 0L, CF_TEXT, XTYP_EXECUTE, SHORT_SYNC_TIMEOUT, NULL);
                        InitializeMenu(GetMenu(hwnd));
                    }
    ...
    case IDM_CONNECT:
    {
        ...
        *szConvPartner = TEXT('\0');
        ...
        if (hMod = LoadLibraryW(L"NTLANMAN.DLL"))
        {
            ...
            {
                fFoundLMDlg = TRUE;
                (*lpfn)(hwnd, FOCUSDLG_SERVERS_ONLY, rgwch, MAX_COMPUTERNAME_LENGTH, &bOK, szHelpFile, IDH_CONNECT);
                if (IDOK == bOK) {
        ...
        // If we didn't find the fancy LanMan dialog, we still can get by with our own cheesy version-- 'course, ours comes up faster, too.
        if (!fFoundLMDlg)
        {
            bOK = DialogBox(hInst, MAKEINTRESOURCE(IDD_CONNECT), hwnd, ConnectDlgProc);
        }
...
#if DEBUG
    {
        ...
        RegQueryValueEx(hkeyRoot, szDebug, NULL, NULL, (LPBYTE) &DebugLevel, &cbDBL);
    }
#endif
    if (pActiveMDI->flags & F_CLPBRD)
        break;
    ...
    case IDM_ABOUT:
    {
        ...
        if (hMod = LoadLibrary(TEXT("SHELL32")))
        {
            ...
            {
                (*lpfn)(hwnd, szAppName, szNull, LoadIcon(hInst, (LPTSTR) MAKEINTRESOURCE(IDFRAMEICON)));
            }
            FreeLibrary(hMod);
        }
        else
...
BOOL SetListboxEntryToPageWindow(HWND hwndc, PMDIINFO pMDIc, int lbindex)
{
    HCONV hConv;
    LPLISTENTRY lpLE;
    PVCLPBRD pVClp;
    ...
    if (LB_ERR != SendMessage(pMDIc->hWndListbox, LB_GETTEXT, lbindex, (LPARAM) (LPTSTR) &lpLE) && lpLE && (pDataReq = CreateNewDataReq()))
    {
        // make new clipboard
