...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpAnsiCmdLine, int nCmdShow)
{
    MSG msg;
    ...
    // if we were started with the name of a file on the command line, attempt to load the .clp file via an open dde execute
    ...
    CloseHandle(hmutexClp); // Clear the mutex
...
static BOOL  InitApplication(HINSTANCE hInstance)
{
    ...
    if (0 == RegisterClass(&wc))// Register the frame window
    {
        return FALSE;
    }
    ...
    wc.style = CS_DBLCLKS;
    if (0 == RegisterClass(&wc))// Register the MDI child class
        return FALSE;
    ...
    wc.style = 0;
    if (0 == RegisterClass(&wc))// register dummy window for SetCapture target
        return FALSE;
}
...
static BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    hAccel = LoadAccelerators(hInstance, (LPTSTR) MAKEINTRESOURCE(IDACCELERATORS));
    if (NULL == hAccel)
    {
    ...
    hicLock = LoadIcon(hInst, (LPTSTR) MAKEINTRESOURCE(IDLOCKICON));// Load Lock icon
    ...
    /* a-mgates 9/24/92                                                    */
    hFilterRsc = FindResourceEx(hInst, (LPTSTR) MAKEINTRESOURCE(ENVSTR), (LPTSTR) MAKEINTRESOURCE(IDS_FILTERTEXT), MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL));
    hFilterText = LoadResource(hInst, hFilterRsc);
    szFilter = LockResource(hFilterText);

    fOwnerDisplay = FALSE;// initialize variables in clipdsp.c
    ...
    // We set this hook up so that we can catch the MSGF_DDEMGR message which is called when DDEML is in a modal loop during synchronous transaction processing.
    ...
    else
    {
        ...
        if (ERROR_SUCCESS != RegQueryValueEx(hkeyRoot, szEnableShr, NULL, NULL, (LPBYTE) &fShareEnabled, &iSize))
        {
            fShareEnabled = TRUE;
        }
    }
    ...
    // Create main window
    if (!(hwndApp = CreateWindow(szClipBookClass, szAppName, WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL)))
    {
    ...
    if (!(hwndDummy = CreateWindow(szDummy, szNull, WS_CHILD | WS_DISABLED ^ WS_VISIBLE, 0, 0, 0, 0, hwndApp, NULL, hInstance, NULL)))
        return FALSE;

    // Make clipboard window -- needs to happen BEFORE we SetClipboardViewer,
    // 'cause hwndApp will get a WM_DRAWCLIPBOARD and there won't be any friggin' windows.
    SendMessage(hwndApp, WM_COMMAND, IDM_CLPWND, 0L);

    hwndNextViewer = SetClipboardViewer(hwndApp);// Attach us to the clipboard viewer chain

    // hconv=

    SendMessage(hwndApp, WM_COMMAND, IDM_LOCAL, 0L);// create initial local window.
    UpdateWindow(hwndApp);// force paint before restoring other connections so we don't have to wait too long. Should we do this earlier?
...
void OnDrawClipboard(HWND hwnd)
{
    ...
    // until the next unlock - if we did this now we could cause other apps to break...

    InitializeMenu(GetMenu(hwnd));// clipboard may have been empty and now isn't
    ...
    else
    {
        ...
        if (IsWindow(hwndClpbrd))
        {
            ...
            else
            {
                ...
                {
                    ChangeCharDimensions(hwndClpbrd, wOldFormat, wNewFormat);// Change the character dimensions based on the format.
                    InitOwnerScrollInfo();// Initialize the owner display scroll info, because the contents have changed.

                    // Force a total repaint. fOwnerDisplay gets updated during a total repaint.
                    InvalidateRect(hwndClpbrd, NULL, TRUE);
                    ResetScrollInfo(hwndClpbrd);

                    UpdateWindow(hwndClpbrd);// force update here BEFORE sending on WM_DRAWCLIPBOARD
                }
            }
        }
    }
    ...
    else if (pMDI->DisplayMode == DSP_PAGE)
    {
        ...
        if (!fAppShuttingDown)
        {
            if (VOpenClipboard(pMDI->pVClpbrd, hwnd))
            {
                ...
                if (hpal)
                {
                    SelectPalette(ps.hdc, hpalT, FALSE);// We don't want to put the DEFAULT palette in the foreground.
                }
...
LRESULT CALLBACK FrameWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        // note that CF_BITMAP, CF_METAFILEPICT, CF_PALETTE are re-registered in a private format because data for these
        ...
        hwndMDIClient = CreateWindow(TEXT("mdiclient"), NULL, WS_BORDER | WS_CHILD | WS_CLIPCHILDREN | MDIS_ALLCHILDSTYLES | WS_HSCROLL | WS_VSCROLL, 0, 0, 0, 0, hwnd, (HMENU) 0xCAC, hInst, (LPVOID) &ccs);
    ...
    case WM_QUERYNEWPALETTE:
        ...
        // If mapping is unchanged, other documents could still change, so give them a change to realize.
        if (!tmp)
    ...
    case WM_MENUSELECT:
        ...
        if (HIWORD(wParam) & MF_POPUP) {
            ...
            if (wParam >= 4100 && wParam <= 4200)
            {
                ...
                wsprintf(szBuf, szActivateFmt, *szBuf2 == TEXT(' ') ? szBuf2 + 1 : szBuf2);
                SendMessage(hwndStatus, SB_SETTEXT, SBT_NOBORDERS | 255, (LPARAM) szBuf);
    ...
    case WM_CLOSE:
        ...
        // so we don't end up yielding in WM_RENDERALLFORMATS and get into trouble.
    ...
    case WM_CLOSE_REALLY:
        ...
        // queue when we yield getting the clipboard data in ForceRenderAll and destroying the app prematurely
    ...
    case WM_DESTROY:
        ...
        ChangeClipboardChain(hwnd, hwndNextViewer);// Take us out of the viewer chain
    ...
    case WM_RENDERALLFORMATS:
        // WM_DESTROY follows close on the heels of this message, and we will process it and die while another copy of FrameWndProc is in sync DDEML transaction...

        // Note that we now attempt to render all formats in WM_DESTROY
        // before doing the PostQuitMessage so we should not have to respond to this message.
        break;
...
LRESULT CALLBACK ChildWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        pMDI->hWndListbox = CreateWindow(TEXT("listbox"), szNull, WS_CHILD | LBS_STANDARD | LBS_NOINTEGRALHEIGHT | LBS_LISTVIEW, 0, 0, 100, 100,   // non zero size to work around
            // possible USER bug which results in divide by zero
    ...
    case WM_COMMAND:
        ...
        break;
    case WM_SYSCOMMAND:
        // The Close menuitem on the system menus of the clipboard and local clipbook windows should be greyed, so we shouldn't get that message.
        switch (wParam)
        {
        case SC_CLOSE:
    ...
    case WM_DESTROY:
        ...
        GlobalFree((HGLOBAL) pMDI);// free up the MDI info struct
        break;
    default:
        return (DefMDIChildProc(hwnd, msg, wParam, lParam));
    }
