...
BOOL IconDialog(LPIC lpic)
{
    ...
    if (PkgPickIconDlg(ghwndPane[APPEARANCE], szIconPath, sizeof(szIconPath)/sizeof(char), &iDlgIcon))
    {
...
BOOL ChangeCmdLine(LPCML lpcml)
{
    ...
    if (DialogBoxAfterBlock(MAKEINTRESOURCE(DTCHANGECMDTEXT), ghwndPane[CONTENT], fnChangeCmdText) != IDOK)
        return FALSE;
...
VOID ChangeLabel(LPIC lpic)
{
    ...
    if (DialogBoxAfterBlock(MAKEINTRESOURCE(DTCHANGETEXT), ghwndPane[iPane], fnChangeText) && lstrcmp(lpic->szIconText, szIconText))
    {
...
INT_PTR CALLBACK fnChangeCmdText(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        PostMessage(hDlg, WM_NEXTDLGCTL, (WPARAM) GetDlgItem(hDlg, IDD_COMMAND), 1L);
        break;
    case WM_COMMAND:
...
INT_PTR CALLBACK fnProperties(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        // Redrawing the string, get the update options and the button state for IDD_AUTO/IDD_MANUAL.
        ...
        // Retrieve the server name (try it from Undo if the object has been frozen)
        if (Error(OleGetData(lpObject, gcfLink, &hData)) || !hData)
        {
            ...
            if (gptyUndo[iPane] == PICTURE && (Error(OleGetData(((LPPICT) lpobjFocusUndo)->lpObject, gcfLink, &hData)) || !hData))
            {
                ErrorMessage(E_GET_FROM_CLIPBOARD_FAILED);
                return TRUE;
            }
        ...
        lstrcat(szFull, szType);// Append the type of link
        SendMessage(hwndLB, LB_INSERTSTRING, (WPARAM) -1, (LPARAM) szFull);// Draw the link in the list box
        ...
        EnableWindow(GetDlgItem(hDlg, IDD_CHANGE), ((otFocus != OT_STATIC) && fChangeLink));
    ...
    case WM_COMMAND:
        ...
        SendMessage(ghwndPane[GetTopWindow(ghwndFrame) == ghwndPane[CONTENT]], WM_COMMAND, wParam, 0L);
...
INT_PTR CALLBACK fnChangeText(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        PostMessage(hDlg, WM_NEXTDLGCTL, (WPARAM)GetDlgItem(hDlg, IDD_ICONTEXT), 1L);
        break;
    case WM_COMMAND:
        switch (LOWORD(wParam))
        {
