...
Module Name:
    UCE.c
Abstract:
    This module contains the main routines for the Universal Character Explorer, an new interface for selecting special characters.
    ...
    switch (message)
    {
        case ( WM_CREATE ) :
        {
            ...
            if ((hdcScrn = GetWindowDC(hWnd)) != NULL)
            {
                ...
                {
                    SetTextColor( sycm.hdcMag, GetSysColor(COLOR_WINDOWTEXT) );
                    SetBkColor( sycm.hdcMag, GetSysColor(COLOR_WINDOW) );
                    SetBkMode(sycm.hdcMag, OPAQUE);
                    if ((sycm.hbmMag = CreateCompatibleBitmap( hdcScrn, sycm.dxpMag, sycm.dypMag * 2 )) == NULL)
                    {
                        ;
            ...
            MoveWindow( hWndScroll, rect.right+1, rect.top+1, sycm.dxpBox, sycm.dypCM, FALSE );
            break;
        }
        case ( WM_DESTROY ) :
        {
        ...
        case ( WM_SETFOCUS )  :
        {
            WCHAR wcBuf[8];
            UINT  cp;
            if(GetLocaleInfo(LOWORD(GetKeyboardLayout(0)), LOCALE_IDEFAULTANSICODEPAGE, wcBuf, 8))
            {
                cp = _wtol(wcBuf);
                if(gKBD_CP != cp)
                {
        ...
        case ( WM_LBUTTONDOWN ) :
        {
            ...
            if ((sycm.fMagnify)&&(chMouseSymbol == sycm.chCurr))
            {
                ...
                fPendingDrag = TRUE;// Go into dragging mode
                SetTimer(hWnd, 1, nDragDelay, NULL);// Start timer
                SetCapture(hWnd);
            }
            else
            {
...
int DoDragScroll(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    iyDistance = (yMousePosn < 0) ? -yMousePosn : yMousePosn - rcClient.bottom;
    ...
    if (iScrollTimer < 1)
        iScrollTimer = 1;
    SendMessage(hWnd, WM_KEYDOWN, (WPARAM)dwScrollDir, (LPARAM)0L);// Scroll
    SetTimer(hWnd, ID_SCROLLTIMER, iScrollTimer, NULL);// Set timer for next scroll
    return 0;
}
    ...
    LPARAM lParam)
{
    switch (message)
    {
      ...
      {
        if (gDisplayFontChgMsg)
          SendMessage(GetDlgItem(hWnd, IDC_CHECKNOMSG), BM_SETCHECK,(WPARAM)BST_UNCHECKED,(LPARAM)0L);
        else
          SendMessage(GetDlgItem(hWnd, IDC_CHECKNOMSG), BM_SETCHECK,(WPARAM)BST_CHECKED,(LPARAM)0L);
        return TRUE;
      }
      case ( WM_COMMAND ) :
      {
        switch (LOWORD(wParam))
        {
          ...
          case (IDC_CHECKNOMSG):
            LRESULT iCheckState;
            iCheckState = SendMessage(GetDlgItem(hWnd, IDC_CHECKNOMSG), BM_GETCHECK,(WPARAM)0L,(LPARAM)0L);
