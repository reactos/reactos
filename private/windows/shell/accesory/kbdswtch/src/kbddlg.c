...
BOOL InitKbdConfDlg( HWND hDlg )
{
    ...
    if (bFirstTime)
    {
        ...
        if (ERROR_SUCCESS != RegOpenKeyEx (HKEY_CURRENT_USER, szRegNameKeyboard, 0L, KEY_QUERY_VALUE | KEY_SET_VALUE | KEY_CREATE_SUB_KEY, &hkeyKeyboard)) {
    ...
    pszErr = TEXT("Selection");
        if (ERROR_SUCCESS != RegCreateKeyEx (hkeyKeyboard, pszErr, 0L, NULL, REG_OPTION_NON_VOLATILE, KEY_QUERY_VALUE | KEY_SET_VALUE, NULL, &hkeySelection, &dwKeyStatus)) {
            goto ErrorKey;
    }
        if (dwKeyStatus == REG_CREATED_NEW_KEY) {
        if (ERROR_SUCCESS != RegOpenKeyEx (hkeyKeyboard, pszErr, 0L, KEY_QUERY_VALUE | KEY_SET_VALUE, &hkeySelection)) {
ErrorKey:
...
Error:
    ...
    RegSetValueEx (hkeyKeyboard, TEXT("Active"), 0, REG_SZ, (LPBYTE)szPrimaryKbd, lstrlen(szPrimaryKbd) * sizeof(TCHAR));
    RegSetValueEx (hkeySelection, szRegNamePrimary, 0, REG_SZ, (LPBYTE)szPrimaryKbd, lstrlen(szPrimaryKbd) * sizeof(TCHAR));
    RegSetValueEx (hkeySelection, szRegNameAlternate, 0, REG_SZ, (LPBYTE)szAlternateKbd, lstrlen(szAlternateKbd) * sizeof(TCHAR));
    wsprintf (szBuf, TEXT("%d\0"), fHotKeyCombo);
    RegSetValueEx (hkeySelection, szRegNameHotKey, 0, REG_SZ, (LPBYTE)szBuf, lstrlen(szBuf) * sizeof(TCHAR));
    wsprintf (szBuf, TEXT("%d\0"), fOnTop);
    RegSetValueEx (hkeySelection, szOnTop, 0, REG_SZ, (LPBYTE)szBuf, lstrlen(szBuf) * sizeof(TCHAR));
    return (TRUE);
}
