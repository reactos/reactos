    ...
    if (fSet)
    {
        if (!hmodHook)
        {
            if (!(hmodHook = LoadLibrary (TEXT("kbddll.dll"))))
            {
                ...
                MessageBox (NULL, szErrorBuf, szCaption, MB_TASKMODAL | MB_OK | MB_ICONEXCLAMATION);
                return (FALSE);
            }
        }
        ...
        if (!hhkKeyboard)
        {
            if (!(hhkKeyboard = SetWindowsHookEx (WH_KEYBOARD, (HOOKPROC)GetProcAddress (hmodHook, "KbdGetMsgProc"), hmodHook, 0)))
            {
                return (FALSE);
            }
        }
