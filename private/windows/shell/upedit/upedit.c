...
void InitGroupLists(HWND hwnd)
{
    ...
    else if (RegOpenKeyEx(hkeyCurrentUser, szProgramGroups, 0, KEY_READ | KEY_WRITE, &hkeyProgramGroups)) {

        // Try openning the key for the ANSI groups.

        if (RegOpenKeyEx(hkeyCurrentUser, TEXT("Program Groups"), 0, KEY_READ | KEY_WRITE, &hkeyProgramGroups))
...
BOOL InitializeUPESettings(HWND hwnd, BOOL bInitOrgSettings)
{
    ...
    else if (!RegCreateKeyEx(hkeyCurrentUser, szProgramManager, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyProgramManager, NULL)) {
        RegCreateKeyEx(hkeyProgramManager, szSettings, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyPMSettings, NULL);
        RegCreateKeyEx(hkeyProgramManager, szRestrict, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyPMRestrict, NULL);
    }
    ...
    if (hkeyPMSettings) {
        ...
        if (RegQueryValueEx(hkeyPMSettings, szStartup, 0, 0, (LPBYTE)szStartupGroup, &cbData) != ERROR_SUCCESS) {
     ...
     */
    if (SendMessage(hwndStartup, CB_SELECTSTRING, (WPARAM)-1, (LPARAM)(LPTSTR)szStartupGroup) == CB_ERR) {
     ...
     * the settings.
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szFileManager, 0, szFileManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyFileManager, NULL)) {
        if (!RegCreateKeyEx(hkeyFileManager, szSettings, 0, szFileManager, 0, KEY_READ, 0, &hkeyFMSettings, NULL)){
     ...
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szWinlogon, 0, NULL, 0, KEY_READ | KEY_WRITE, 0, &hkeyWinlogon, NULL)) {
        cbData = sizeof(bSyncLogonScript);
        RegQueryValueEx(hkeyWinlogon, szSyncLogonScript, 0, 0, (LPBYTE)&bSyncLogonScript, &cbData);
     ...
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szPrintManager, 0, szPrintManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyPrintManager, NULL)) {
        cbData = sizeof(bNetwork);
...
BOOL SaveUPESettingsToRegistry()
{
    ...
    if (hkeyPMRestrict) {
        RegSetValueEx(hkeyPMRestrict, szEditLevel, 0, REG_DWORD, (LPBYTE)&dwEditLevel, sizeof(dwEditLevel));
        RegSetValueEx(hkeyPMRestrict, szNoRun, 0, REG_DWORD, (LPBYTE)&bNoRun, sizeof(bNoRun));
        RegSetValueEx(hkeyPMRestrict, szNoSave, 0, REG_DWORD, (LPBYTE)&bNoSave, sizeof(bNoSave));
        RegSetValueEx(hkeyPMRestrict, szShowCommonGrps, 0, REG_DWORD, (LPBYTE)&bShowCommonGrps, sizeof(bShowCommonGrps));
    }
    ...
    if ((i = (INT)SendDlgItemMessage(hwnd, IDD_STARTUP, CB_GETCURSEL, 0, 0)) >= 0) {
        ...
        if (lstrcmpi(szNone, szGroupName)) {
            if (hkeyPMSettings) {
                RegSetValueEx( hkeyPMSettings, szStartup, 0, REG_SZ, (LPBYTE)szGroupName, (lstrlen(szGroupName) + 1) * sizeof(TCHAR) );
            }
        }
        else {
     ...
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szFileManager, 0, szFileManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyFileManager, NULL)) {
        if (!RegCreateKeyEx(hkeyFileManager, szSettings, 0, szFileManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyFMSettings, NULL)) {
            RegSetValueEx(hkeyFMSettings, szNetwork, 0, REG_DWORD, (LPBYTE)&bNetwork, sizeof(bNetwork));
            RegCloseKey(hkeyFMSettings);
        }
        RegCloseKey(hkeyFileManager);
    }
     ...
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szPrintManager, 0, szPrintManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyPrintManager, NULL)) {
        RegSetValueEx(hkeyPrintManager, szNetwork, 0, REG_DWORD, (LPBYTE)&bNetwork, sizeof(bNetwork));
        RegCloseKey(hkeyPrintManager);
    }
     ...
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szWinlogon, 0, NULL, 0, KEY_READ | KEY_WRITE, 0, &hkeyWinlogon, NULL)) {
        RegSetValueEx(hkeyWinlogon, szSyncLogonScript, 0, REG_DWORD, (LPBYTE)&bSyncLogonScript, sizeof(bSyncLogonScript));
        RegCloseKey(hkeyWinlogon);
    }
    return(TRUE);
}
...
BOOL PASCAL InitDialog(HWND hwnd)
{
     ...
     * the settings.
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szProgramManager, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyProgramManagerCurrent, NULL)) {
        RegCreateKeyEx(hkeyProgramManagerCurrent, szSettings, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyPMSettingsCurrent, NULL);
        RegCreateKeyEx(hkeyProgramManagerCurrent, szRestrict, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyPMRestrictCurrent, NULL);
    }

    RegOpenKeyEx(hkeyCurrentUser, szProgramGroups, 0, KEY_READ | KEY_WRITE, &hkeyProgramGroupsCurrent);
    if (!hkeyProgramGroupsCurrent) {

        // Try the ANSI groups

        RegOpenKeyEx(hkeyCurrentUser, TEXT("Program Groups"), 0, KEY_READ | KEY_WRITE, &hkeyProgramGroupsCurrent);
    }
...
VOID ResetCurrentUserSettings(HWND hwnd)
{
     ...
     */
    if (hkeyPMRestrict) {
        RegSetValueEx(hkeyPMRestrict, szEditLevel, 0, REG_DWORD, (LPBYTE)&dwEditLevelOrg, sizeof(dwEditLevelOrg));
        RegSetValueEx(hkeyPMRestrict, szNoRun, 0, REG_DWORD, (LPBYTE)&bNoRunOrg, sizeof(bNoRunOrg));
        RegSetValueEx(hkeyPMRestrict, szNoSave, 0, REG_DWORD, (LPBYTE)&bNoSaveOrg, sizeof(bNoSaveOrg));
        RegSetValueEx(hkeyPMRestrict, szShowCommonGrps, 0, REG_DWORD, (LPBYTE)&bShowCommonGrpsOrg, sizeof(bShowCommonGrpsOrg));
    }
    ...
    if (hkeyPMSettings) {
        if (lpStartupGroupOrg) {
            RegSetValueEx(hkeyPMSettings, szStartup, 0, REG_SZ, (LPBYTE)lpStartupGroupOrg, (lstrlen(lpStartupGroupOrg)+1) * sizeof(TCHAR));
    ...
    if (!RegCreateKeyEx(hkeyCurrentUser, szFileManager, 0, szFileManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyFileManager, NULL)) {
        if (!RegCreateKeyEx(hkeyFileManager, szSettings, 0, szFileManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyFMSettings, NULL)) {
            RegSetValueEx(hkeyFMSettings, szNetwork, 0, REG_DWORD, (LPBYTE)&bFileManNetworkOrg, sizeof(bFileManNetworkOrg));
            RegCloseKey(hkeyFMSettings);
        }
        RegCloseKey(hkeyFileManager);
    }
     ...
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szPrintManager, 0, szPrintManager, 0, KEY_READ | KEY_WRITE, 0, &hkeyPrintManager, NULL)) {
        RegSetValueEx(hkeyPrintManager, szNetwork, 0, REG_DWORD, (LPBYTE)&bPrintNetworkOrg, sizeof(bPrintNetworkOrg));
        RegCloseKey(hkeyPrintManager);
    }
     ...
     */
    if (!RegCreateKeyEx(hkeyCurrentUser, szWinlogon, 0, NULL, 0, KEY_READ | KEY_WRITE, 0, &hkeyWinlogon, NULL)) {
        RegSetValueEx(hkeyWinlogon, szSyncLogonScript, 0, REG_DWORD, (LPBYTE)&bSyncLogonScriptOrg, sizeof(bSyncLogonScriptOrg));
        RegCloseKey(hkeyWinlogon);
    }
