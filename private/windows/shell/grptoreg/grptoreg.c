...
LoadFail:
    ...
    if (bNewOrder) {
        ...
        RegSetValueEx(hkeyPMSettings, szOrder, 0, REG_SZ, szGroupsOrder, lstrlen(szGroupsOrder)+1);
    }
...
VOID PASCAL LoadAllGroups()
{
    ...
    for (GetGroupList(pT); *pT; pT += (lstrlen(pT) + 1)) {
        if (!GetPrivateProfileString(szGroups, pT, szNULL, szPath, sizeof(szPath), szINIFile)) {
            continue;
        }
...
BOOL SaveGroupInRegistry(PGROUP pGroup, BOOL bCommonGrp, BOOL bOverwriteGroup)
{
    ...
    while(1) {
        if (err = RegCreateKeyEx(hkeyGroups, pGroup->lpKey, 0, 0, 0, DELETE | KEY_READ | KEY_WRITE, pSecAttr, &hkey, &dwDisposition)) {
            goto Exit1;
        }
...
BOOL SaveSettingsToRegistry()
{
    ...
    if (hkeyPMSettings) {
        err = RegSetValueEx(hkeyPMSettings, szWindow, 0, REG_SZ, szPMWindowSetting, lstrlen(szPMWindowSetting)+1);
        err |= RegSetValueEx(hkeyPMSettings, szOrder, 0, REG_SZ, szGroupsOrder, lstrlen(szGroupsOrder)+1);
        if (*szStartupGroup)
            err |= RegSetValueEx(hkeyPMSettings, szStartup, 0, REG_SZ, szStartupGroup, lstrlen(szStartupGroup)+1);
        err |= RegSetValueEx(hkeyPMSettings, szMinOnRun, 0, REG_DWORD, (LPBYTE)&bMinOnRun, sizeof(bMinOnRun));
        err |= RegSetValueEx(hkeyPMSettings, szAutoArrange, 0, REG_DWORD, (LPBYTE)&bAutoArrange, sizeof(bAutoArrange));
        err |= RegSetValueEx(hkeyPMSettings, szSaveSettings, 0, REG_DWORD, (LPBYTE)&bSaveSettings, sizeof(bSaveSettings));
    }

    if (hkeyPMRestrict) {
        err = RegSetValueEx(hkeyPMRestrict, szNoRun, 0, REG_DWORD, (LPBYTE)&fNoRun, sizeof(fNoRun));
        err |= RegSetValueEx(hkeyPMRestrict, szNoClose, 0, REG_DWORD, (LPBYTE)&fNoClose, sizeof(fNoClose));
        err |= RegSetValueEx(hkeyPMRestrict, szNoSave, 0, REG_DWORD, (LPBYTE)&fNoSave, sizeof(fNoSave));
        err |= RegSetValueEx(hkeyPMRestrict, szNoFileMenu, 0, REG_DWORD, (LPBYTE)&fNoFileMenu, sizeof(fNoFileMenu));
        err |= RegSetValueEx(hkeyPMRestrict, szEditLevel, 0, REG_DWORD, (LPBYTE)&dwEditLevel, sizeof(dwEditLevel));
    }
...
BOOL AccessToPersonalGroups()
{
     ...
     */
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szProgramManager, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, pPGrpSecAttr, &hkeyProgramManager, NULL)) {
        return(FALSE);
    }

    RegCreateKeyEx(hkeyProgramManager, szSettings, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, pPGrpSecAttr, &hkeyPMSettings, NULL);
    RegCreateKeyEx(hkeyProgramManager, szRestrict, 0, szProgramManager, 0, KEY_READ, pPGrpSecAttr, &hkeyPMRestrict, NULL);
    RegCreateKeyEx(hkeyProgramManager, szGroups, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, pPGrpSecAttr, &hkeyPMGroups, NULL);
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szProgramGroups, 0, szProgramGroups, 0, KEY_READ | KEY_WRITE, pPGrpSecAttr, &hkeyProgramGroups, NULL) ){
        return(FALSE);
    }
    return(TRUE);
}
...
BOOL AccessToCommonGroups()
{
    ...
    if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, szCommonGroups, 0, szProgramGroups, 0, KEY_READ | KEY_WRITE | DELETE, pCGrpSecAttr, &hkeyCommonGroups, NULL) ){
        return(FALSE);
    }
