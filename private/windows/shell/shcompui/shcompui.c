...
//    in the NT Explorer user interface.  There are two interfaces to the compression features.

//    The first interface is a shell context menu extension that adds the options "Compress" and "Uncompress" to the context/file menu of
//    an object that has registered the extension.  This interface uses the standard shell extension protocol of QueryContextMenu,
...
static DWORD FormatStringWithArgs(LPCTSTR pszFormat, LPTSTR pszBuffer, DWORD nSize, ...)
{
   ...
   va_start(args, nSize);
   dwCharCount = FormatMessage(FORMAT_MESSAGE_FROM_STRING, pszFormat, 0, 0, pszBuffer, nSize, &args);
   va_end(args);
