...
//  This file contains the implementation of the CDSSecurityInfo object, which provides the ISecurityInformation interface for invoking the ACL Editor.
...
public:
    ...
    STDMETHODIMP GetAccessRights(const GUID* pguidObjectType, DWORD dwFlags, PSI_ACCESS *ppAccess, ULONG *pcAccesses, ULONG *piDefaultAccess);
    STDMETHODIMP MapGeneric(const GUID *pguidObjectType, UCHAR *pAceFlags, ACCESS_MASK *pmask);
...
GetDsDcAddress(BSTR *pbstrDcAddress)
{
    ...
    if (hNetApi32) {
        ...
        if (pfnDsGetDcName && pfnNetApiFree) {
            ...
            DWORD dwErr = (*pfnDsGetDcName)(NULL, NULL, NULL, NULL, DS_DIRECTORY_SERVICE_REQUIRED | DS_IP_REQUIRED, &pDCI);
...
CDSSecurityInfo::Init2(LPCWSTR pszUserName, LPCWSTR pszPassword)
{
    ...
    hr = ADsOpenObject(m_strObjectPath, (LPWSTR)pszUserName, (LPWSTR)pszPassword, ADS_SECURE_AUTHENTICATION | ADS_FAST_BIND, IID_IDirectoryObject, (LPVOID*)&m_pDsObject);
    ...
    m_pDsObject->GetObjectAttributes(&pszTemp, 1, &pAttributeInfo, &dwAttributesReturned);
    if (pAttributeInfo) {
    ...
    if (pObjectInfo) {
        ...
        if ((m_dwInitFlags & DSSI_IS_ROOT)
            || (m_strObjectClass && CSTR_EQUAL == CompareString(LOCALE_SYSTEM_DEFAULT, NORM_IGNORECASE, m_strObjectClass, ARRAYSIZE(c_szDomainClass) - 1, c_szDomainClass, ARRAYSIZE(c_szDomainClass) - 1))) {
        ...
        if (CSTR_EQUAL == CompareString(LOCALE_SYSTEM_DEFAULT, NORM_IGNORECASE, pszTemp, nClassLen, pObjectInfo->pszClassName, nClassLen)) {
            *pszTemp = L'\0';
        }
...
CDSSecurityInfo::Init3()
{
    ...
    hr = Schema_BindToObject(m_strSchemaRootPath, m_strObjectClass, IID_IADsClass, (LPVOID*)&pDsClass);
                                       ...
                                       LPARAM lpContext)
{
    ...
    hr = pThis->m_pDsObject->SetObjectAttributes(&attributeInfo, 1, &dwAttributesModified);
...
CDSSecurityInfo::CheckObjectAccess()
{
    ...
    m_pDsObject->GetObjectAttributes(&pProp, 1, &pSDRightsInfo, &dwAttributesReturned);
    if (pSDRightsInfo) {
                              ...
                              LPARAM lpContext)
{
    ...
    *ppSI = NULL;
    ...
    hr = pSI->Init(pwszObjectPath, pwszObjectClass, pwszServer, pwszUserName, pwszPassword, dwFlags, pfnReadSD, pfnWriteSD, lpContext);
    if (FAILED(hr)) {
        DoRelease(pSI);
    }
                     ...
                     LPARAM lpContext)
{
    ...
    *phPage = NULL;
    ...
    hr = DSCreateISecurityInfoObject(pwszObjectPath, pwszObjectClass, dwFlags, &pSI, pfnReadSD, pfnWriteSD, lpContext);
    if (SUCCEEDED(hr)) {
               ...
               LPARAM lpContext)
{
    ...
    if (pwszCaption && *pwszCaption) {
        ...
        hr = DSCreateSecurityPage(pwszObjectPath, pwszObjectClass, dwFlags, &hPage, pfnReadSD, pfnWriteSD, lpContext);
        if (SUCCEEDED(hr)) {
