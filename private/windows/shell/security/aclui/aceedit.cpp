...
CACEPage::OnApply(HWND hDlg, BOOL /*bClose*/)
{
    ...
    if ((m_siObjectInfo.dwFlags & SI_CONTAINER) != 0)
    {
        ...
        if (iSel != CB_ERR)
        {
            psiInheritType = (PSI_INHERIT_TYPE)SendMessage(hInheritType, CB_GETITEMDATA, iSel, 0);
        }
...
BOOL CACEPage::DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
    case WM_INITDIALOG:
    ...
    case WM_NOTIFY:
        ...
        case CLN_GETCOLUMNDESC:
        {
            ...
            GetDlgItemText(hDlg, IDC_ACEE_ALLOW - 1 + pnmc->iSubItem, pnmc->pszText, pnmc->cchTextMax);
        }
        break;
        case PSN_APPLY:
            SetWindowLongPtr(hDlg, DWLP_MSGRESULT, OnApply(hDlg, (BOOL)(((LPPSHNOTIFY)lParam)->lParam)));
            break;
        }
        break;
    case WM_HELP:
        if (IsWindowEnabled(hDlg))
        {
            ...
            WinHelp((HWND)((LPHELPINFO)lParam)->hItemHandle, c_szAcluiHelpFile, HELP_WM_HELP, (DWORD_PTR)pdwHelpIDs);
        }
        break;
    case WM_CONTEXTMENU:
        if (IsWindowEnabled(hDlg))
        {
            ...
            WinHelp(hwnd, c_szAcluiHelpFile, HELP_CONTEXTMENU, (DWORD_PTR)pdwHelpIDs);
        }
        break;
    default:
        return FALSE;
              ...
              HDPA *phEntries)
{
    ...
    pPage = new CACEPage(psi, siType, pAce, bReadOnly, dwFlags, pdwResult, phEntries);
    if (pPage)
    {
             ...
             UINT nStartPage)
{
    ...
    if (phEntries)
    {
        hPage[cPages] = CreateACEPage(psi, siType, pAce, bReadOnly, SI_ACCESS_SPECIFIC, pdwResult, phEntries);
        if (hPage[cPages])
            cPages++;
    }
    ...
    {
        hPage[cPages] = CreateACEPage(psi, siType, pAce, bReadOnly, SI_ACCESS_PROPERTY, pdwResult, phPropertyEntries);
        if (hPage[cPages])
            cPages++;
    }
