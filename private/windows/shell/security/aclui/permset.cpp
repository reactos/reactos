...
BOOL CPermissionSet::AddAce(LPCGUID pguid, ACCESS_MASK mask, DWORD dwFlags)
{
...
BOOL CPermissionSet::AddPermission(PPERMISSION pPerm)
{
    ...
    else
    {
        ...
        while (cItems > 0)
        {
            ...
            {
                // The new permission implies the existing permission, so the existing one can be removed.
                DSA_DeleteItem(m_hPermList, cItems);
                // Keep looking.  Maybe we can remove some more entries before adding the new one.
            }
            else if (dwMergeStatus == MERGE_OK_2)
            {
                // The existing permission implies the new permission, so there is nothing to do here.
...
BOOL CPermissionSet::AddAdvancedAce(PACE_HEADER pAce)
{
...
UINT CPermissionSet::GetPermCount(BOOL fIncludeAdvAces) const
{
    ULONG cAces = 0;
...
ULONG CPermissionSet::GetAclLength(ULONG cbSid) const
{
    // Return an estimate of the buffer size needed to hold the requested ACEs. The size of the ACL header is NOT INCLUDED.
    ULONG nAclLength = 0;
    ULONG cAces;
...
BOOL CPermissionSet::AppendToAcl(PACL pAcl,
    ...
    DWORD dwFlags) const
{
    ...
    while (cAces > 0)
    {
        ...
        psidT = &((PKNOWN_ACE) pAce)->SidStart;// Get the normal SID location
        ...
        if (bObjectAce)
        {
            ...
            pGuid = RtlObjectAceObjectType(pAce);// Get the object type guid location
            TraceAssert(pGuid);// We just set the flag for this, so it can't be NULL
            ...
            *pGuid = pPerm->guid;// Copy the object type guid
            psidT = RtlObjectAceSid(pAce);// Get new SID location
    ...
    if ((dwFlags & PS_OBJECT) && m_hAdvPermList != NULL)
    {
        ...
        while (cAces > 0)
        {
            ...
            CopyMemory(*ppAcePos, pAce, pAce->AceSize);// Copy the ACE
...
void CPermissionSet::ConvertInheritedAces(CPermissionSet &permInherited)
{
    UINT cItems;
...
void CPermissionSet::RemovePermission(PPERMISSION pPerm)
{
