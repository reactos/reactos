...
/*
    ...
    HISTORY:
        JeffreyS    08-Oct-1996     Created
**/
int CALLBACK _LocalFreeCB(LPVOID pVoid, LPVOID /*pData*/)
{
    if (pVoid)
        LocalFree(pVoid);
...
LSA_HANDLE GetLSAConnection(LPCTSTR pszServer, DWORD dwAccessDesired)
{
    ...
    if (pszServer && *pszServer && RtlCreateUnicodeString(&uszServer, pszServer))
    {
                ...
                PHANDLE phThread)
{
    ...
    if (pdata)
    {
        ...
        hThread = CreateThread(NULL, 0, _LookupSidsAsyncProc, pdata, NULL, &dwThreadId);
        if (hThread != NULL)
        {
            if (phThread)
                *phThread = hThread;
    ...
    __try
    {
        ...
        if (Status == 0)
        {
            ...
            if ((Status == 0) && (SubStatus == 0))
            {
                ...
                if (LookupAccountNameW(pszServer, pResp->CallerName, psid, &cbSid, szDomain, &cchDomain, &sidType))
                {
