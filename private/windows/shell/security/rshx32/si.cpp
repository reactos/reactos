...
BOOL WINAPI SetSecurityDescriptorControl(PSECURITY_DESCRIPTOR psd, SECURITY_DESCRIPTOR_CONTROL wControlMask, SECURITY_DESCRIPTOR_CONTROL wControlBits)
{
    DWORD dwErr = NOERROR;
...
CSecurityInformation::CSecurityInformation(SE_OBJECT_TYPE seType) : m_cRef(1), m_seType(seType), m_hwndOwner(NULL)
{
...
STDMETHODIMP CSecurityInformation::Initialize(HDPA   hItemList, DWORD  dwFlags, LPTSTR pszServer, LPTSTR pszObject)
{
...
STDMETHODIMP CSecurityInformation::QueryInterface(REFIID riid, LPVOID FAR* ppv)
{
...
STDMETHODIMP CSecurityInformation::GetObjectInformation(PSI_OBJECT_INFO pObjectInfo)
{
    TraceEnter(TRACE_SI, "CSecurityInformation::GetObjectInformation");
    TraceAssert(pObjectInfo != NULL && !IsBadWritePtr(pObjectInfo, sizeof(*pObjectInfo)));
...
STDMETHODIMP CSecurityInformation::GetSecurity(SECURITY_INFORMATION si, PSECURITY_DESCRIPTOR *ppSD, BOOL fDefault)
{
    HRESULT hr = S_OK;
    LPTSTR pszItem;
...
STDMETHODIMP CSecurityInformation::SetSecurity(SECURITY_INFORMATION si, PSECURITY_DESCRIPTOR pSD)
{
    HRESULT hr;
...
STDMETHODIMP CSecurityInformation::ReadObjectSecurity(LPCTSTR pszObject, SECURITY_INFORMATION si, PSECURITY_DESCRIPTOR *ppSD)
{
    DWORD dwErr;
...
#if(_WIN32_WINNT >= 0x0500)
    dwErr = GetNamedSecurityInfo((LPTSTR)pszObject, m_seType, si, NULL, NULL, NULL, NULL, ppSD);
    ...
    if (!dwErr)
    {
        dwErr = ConvertAccessToSecurityDescriptor(pAccessList, pAuditList, pOwner, pGroup, ppSD);
        if (pAccessList)
...
STDMETHODIMP CSecurityInformation::WriteObjectSecurity(LPCTSTR pszObject, SECURITY_INFORMATION si, PSECURITY_DESCRIPTOR pSD)
{
    DWORD dwErr;
    ...
    dwErr = SetNamedSecurityInfo((LPTSTR)pszObject, m_seType, si, psidOwner, psidGroup, pDacl, pSacl);
#else   // _WIN32_WINNT < 0x0500
    PACTRL_ACCESS pAccessList = NULL;
    PACTRL_AUDIT pAuditList = NULL;
    LPTSTR pOwner = NULL;
    ...
    if (!dwErr)
    {
        dwErr = SetNamedSecurityInfoEx(pszObject, m_seType, si, NULL, pAccessList, pAuditList, pOwner, pGroup, NULL);
        if (pAccessList)
            LocalFree(pAccessList);
        if (pAuditList)
            LocalFree(pAuditList);
