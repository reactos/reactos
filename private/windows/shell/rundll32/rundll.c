...
EndSwitches:
        ...
        // At this point we're just past the terminated dll path.
        // We then skip spaces and commas, which should take us to the start of the entry point (lpFunction)
        while ( *lpEnd && ((*lpEnd == TEXT(' ')) || (*lpEnd == TEXT(','))))
            lpEnd++;
        if (!*lpEnd)
            return FALSE;
        ...
        // the function name and move the end pointer, because that's where the arguments to the function live.
        ...
        // Note that we try to get a module handle first, so we don't need to pass full file names around
        ...
        // If the load fails try it thru wx86, since it might be an x86 on risc binary
        if (g_hModule==NULL) {
        ...
        if (g_hModule==NULL)
        {
            ...
            BOOL fSuccess = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, szSysErrMsg, ARRAYSIZE(szSysErrMsg), NULL);
            if (fSuccess)
            {
...
int PASCAL WinMainT (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpszCmdLine, int nCmdShow)
{
        ...
        SetErrorMode(SEM_NOOPENFILEERRORBOX | SEM_FAILCRITICALERRORS);// turn off critical error bullshit
