...
void FatalError(LPSTR Message, ULONG_PTR MessageParameter1, ULONG_PTR MessageParameter2)
{
    if (Message != NULL) {
...
void Usage(LPSTR Message, ULONG MessageParameter)
{
...
PWSTR GetErrorMessage(DWORD MessageId)
{
    PWSTR Message, s;
...
PWSTR GetArgAsUnicode(LPSTR s)
{
    ULONG n;
    PWSTR ps;

    n = strlen(s);
    ps = HeapAlloc(GetProcessHeap(), 0, (n + 1) * sizeof(WCHAR));
    if (ps == NULL) {
        FatalError("Out of memory", 0, 0);
    }

    if (MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, s, n, ps, n) != (LONG) n
        ) {
        FatalError("Unable to convert parameter '%s' to Unicode (%u)", (ULONG_PTR) s, GetLastError());
    }
...
HRESULT ProcessLinkFile(PWSTR FileName);
void ProcessLinkFilesInDirectoryTree(PWSTR Directory);
    ...
    while (--argc) {
        ...
        if (*s == '-' || *s == '/') {
            while (*++s) {
                ...
                case 'r':
                    if (--argc) {
                        GetFullPathName(GetArgAsUnicode(*++argv), MAX_PATH, RootOfSearch, &FileName);
                    }
                    else {
            ...
            if (Error != NO_ERROR) {
                FatalError("Failed to load from file '%s' (%ws)", (ULONG_PTR) s, (ULONG_PTR) GetErrorMessage(Error));
            }
        }
    }
...
HRESULT ProcessLinkFile(PWSTR FileName)
{
    ...
    rc = CoCreateInstance(&CLSID_ShellLink, NULL, CLSCTX_INPROC, &IID_IShellLink, &psl);
    if (!SUCCEEDED(rc)) {
        FatalError("Unable to create ShellLink instance (%ws)", (ULONG_PTR) GetErrorMessage(rc), 0);
    }

    rc = (psl->lpVtbl->QueryInterface)(psl, &IID_IPersistFile, &ppf);
    if (!SUCCEEDED(rc)) {
        FatalError("Unable to get ShellLink PersistFile interface (%ws)", (ULONG_PTR) GetErrorMessage(rc), 0);
