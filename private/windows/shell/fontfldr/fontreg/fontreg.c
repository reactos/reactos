...
void DorkOnRegistry( PSZ pszDescription, PSZ pszRegFile )
 {
  ...
  lrc = RegSetValueEx( hKeyFonts, pszDescription, 0, REG_SZ, pszRegFile, lstrlen(pszRegFile) );
  if( lrc != ERROR_SUCCESS )
   {
...
int GetSection( LPSTR lpFile, LPSTR lpSection, LPHANDLE hSection)
{
    ...
    do
    {
        ...
        if( lpFile )
            nCount = GetPrivateProfileString(lpSection, NULL, "", pszSect, nSize, lpFile);
        else
...
int APIENTRY WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
 {
  ...
  lrc = RegCreateKeyEx( HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Fonts", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hKeyFonts, &dwDisposition );
  if( lrc == ERROR_SUCCESS )
    {
     ProcessIt();
