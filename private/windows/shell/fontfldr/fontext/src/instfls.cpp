...
{
    if( GetPrivateProfileString( szDisks, lpDisk, szNull, lpName, PATHMAX, szSetupInfPath ) )
      return( TRUE );

    return( GetPrivateProfileString( szOEMDisks, lpDisk, szNull, lpName, PATHMAX, szSetupInfPath ) != 0 );
}
        ...
        default:
            if( iMessage == s_wBrowseDoneMsg )
            {
                ...
                if( ( fh = LZOpenFile( szDrv, &of, OF_READ ) ) == -1 )
                {
                    ...
                    return( TRUE );//  return TRUE so commdlg does not exit
                }
...
VOID NEAR PASCAL FormatAddFilePrompt( LPTSTR szStr2 )
{
          ...
          && (pszEnd = StrChr( ++pszStart, TEXT( '"' ) ) ) )
    {
        *pszEnd = TEXT( '\0' );
        ...
        FormatMessage( FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY, szString, 0, 0, szStr2, 256, (va_list *) args);
    }
    else
    {
        ...
        case IDD_BROWSE:
            {
                ...
                *szPath = TEXT( '\0' );
                ...
                OpenFileName.Flags = OFN_HIDEREADONLY | OFN_ENABLEHOOK | OFN_ENABLETEMPLATE | /* OFN_SHOWHELP | */ OFN_NOCHANGEDIR;
                ...
                {
                    wsprintf( szPath, TEXT( "Commdlg error = 0x%04x" ), temp = LOWORD( CommDlgExtendedError( ) ) );

                    if( temp )
                        MessageBox( hDlg, szPath, TEXT( "Control" ), MB_SETFOREGROUND|MB_OK|MB_ICONINFORMATION );
                }
#endif
...
DWORD FAR PASCAL InstallFiles( HWND hwnd, LPTSTR FAR *pszFiles, int nCount, INSTALL_PROC lpfnNewFile, WORD wIFFlags )
{
    SHFILEOPSTRUCT fop;
    ...
    for( i = 0; i < nCount; i++)
    {
        ...
        if( !nDisk )
        {
            ...
            if( !GetInstDiskName( (LPTSTR) &nDisk, szDirOfSrc ) || !(pszEnd = StrChr( szDirOfSrc, TEXT( ',' ) ) ) )
                goto backout;
        ...
        *szTmpFile = 0;
        ...
        if( ( iSHFileOpResult = SHFileOperation( &fop ) ) || fop.fAnyOperationsAborted )
        {
            ...
            if( fop.fAnyOperationsAborted || ( iSHFileOpResult == 0x75 /* DE_OPCANCELLED */) )
            {
                dwInstalledCount = (DWORD)-1;
            }
            goto backout;
