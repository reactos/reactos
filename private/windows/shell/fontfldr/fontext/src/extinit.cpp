...
STDMETHODIMP CShellExtInit::QueryInterface( REFIID riid, LPVOID FAR* ppvObj )
{
    *ppvObj = NULL;
    ...
    DEBUGMSG( ( DM_NOEOL | DM_TRACE1, TEXT( "FONTEXT: CShellExtInit::QueryInterface called for: " ) ) );
...
{
    DEBUGMSG( (DM_TRACE1, TEXT( "FONTEXT: CShellExtInit::AddRef called: %d->%d references" ), m_cRef, m_cRef + 1 ) );
    return( ++m_cRef );
}
...
{
    DEBUGMSG( (DM_TRACE1, TEXT( "FONTEXT: CShellExtInit::Release called: %d->%d references" ), m_cRef, m_cRef - 1 ) );
...
STDMETHODIMP CShellExtInit::Initialize( LPCITEMIDLIST pidlFolder, LPDATAOBJECT lpdobj, HKEY hkeyProgID )
{
    if( m_poData )
        m_poData->Release( );
...
STDMETHODIMP CShellExtInit::QueryContextMenu( HMENU hmenu, UINT indexMenu, UINT idCmdFirst, UINT idCmdLast, UINT uFlags )
{
...
STDMETHODIMP CShellExtInit::GetCommandString( UINT_PTR idCmd, UINT    uFlags, UINT   *pwReserved, LPSTR   pszName, UINT    cchMax )
{
    HRESULT  hr = ResultFromScode( E_INVALIDARG );
    UINT  nID;
...
// WM_INITDIALOG message. In this sample code, we simply fill the listbox with the list of fully qualified paths to the file and directories.
void FSPage_InitDialog( HWND hDlg, LPPROPSHEETPAGE psp )
{
    ...
    if( SUCCEEDED( hres ) )
    {
        ...
        if( bIsTrueType( &fdi ) || bIsNewExe( &fdi ) )
        {
            ...
            wsprintf( szAll, TEXT( "%s\r\n\r\n%s\r\n\r\n%s" ), fdi.lpszVersion, fdi.lpszTrademark, fdi.lpszCopyright );
...
STDMETHODIMP CShellExtInit::AddPages( LPFNADDPROPSHEETPAGE lpfnAddPage, LPARAM lParam )
{
...
STDMETHODIMP CShellExtInit::ReplacePage( UINT uPageID, LPFNADDPROPSHEETPAGE lpfnReplaceWith, LPARAM lParam )
{
    return NOERROR;
}
...
VOID InstallDataObject( LPDATAOBJECT pdobj, DWORD dwEffect, HWND hWnd, CFontView * poView)
{
    // Get an HDROP, if possible.
    FORMATETC fmte = {
                    CF_HDROP,
    ...
    if( SUCCEEDED( hres ) )
    {
        ...
        for( UINT i = 0; i < cnt; )
        {
            ...
            switch( CPDropInstall( poView->GetViewWindow(), szFile, dwEffect, lpszNameMap, (int) (cnt - ++i) ) )
            {
            case CPDI_SUCCESS:
                bAdded = TRUE;
                break;
