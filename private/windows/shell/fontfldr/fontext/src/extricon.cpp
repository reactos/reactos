    ...
    )
{
    ...
    if (INVALID_HANDLE_VALUE != m_hFile)
    {
        if ((m_hFileMapping = CreateFileMapping(m_hFile, NULL, PAGE_READONLY, 0, 0, NULL)) != NULL)
        {
            m_pbBase = (LPBYTE)MapViewOfFile(m_hFileMapping, FILE_MAP_READ, 0, 0, 0);
            if (NULL == m_pbBase)
            {
                hr = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_NULL, GetLastError());
                DEBUGMSG((DM_ERROR, TEXT("FONTEXT: Error 0x%08X mapping view of OTF file %s"), hr, pszFile));
            }
        }
        else
        {
            hr = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_NULL, GetLastError());
            DEBUGMSG((DM_ERROR, TEXT("FONTEXT: Error 0x%08X creating mapping for OTF file %s"), hr, pszFile));
        }
    }
    else
    {
        hr = MAKE_HRESULT(SEVERITY_ERROR, FACILITY_NULL, GetLastError());
        DEBUGMSG((DM_ERROR, TEXT("FONTEXT: Error 0x%08X opening OTF file %s"), hr, pszFile));
    }
    return hr;
}
