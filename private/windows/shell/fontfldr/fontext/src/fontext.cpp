...
HKEY OpenRegistryKey( TCHAR* szRegPath, TCHAR** ppValueName )
{
    ...
    for( i = 0; szCopy[ i ]; i++)
    {
        if( szCopy[ i ] == TEXT( '\\' ) )
        {
            ...
            *ppValueName = &szRegPath[ i+1 ];
            ...
            if( !szCopy[ i+1 ] )
            {
                DEBUGMSG( (DM_ERROR, TEXT( "REGEXT:  Value path name ended in a \\:%s" ), szRegPath ) );
                return( NULL );
            }
        }
    }
    ...
    for( i = 0; &szRegPath[ i+1 ] != *ppValueName; i++ )
    {
        if( !szCopy[ i ] )
        {
            ...
            if( ERROR_SUCCESS != RegCreateKeyEx( hKeyPrev, &szCopy[ i+1 ], NULL, TEXT( "" ), NULL, KEY_ALL_ACCESS, NULL, &hKey, &dwDisp ) )
            {
