...
SkipToSibling:
        ...
        if (g_FindFlags & (FIND_VALUES | FIND_DATA)) {
            ...
            hRootKey = KeyTree_BuildKeyPath(g_RegEditData.hKeyTreeWnd, TVItem.hItem, KeyName, BKP_TOSUBKEY);
            if(hRootKey && RegOpenKeyEx(hRootKey,KeyName,0,KEY_QUERY_VALUE,&hKey) == ERROR_SUCCESS) {
                ...
                if ((g_FindFlags & (FIND_VALUES | FIND_DATA | FIND_EXACT)) == (FIND_VALUES | FIND_EXACT)) {
                    if (RegQueryValueEx(hKey, s_FindSpecification, NULL, NULL, NULL, NULL) == ERROR_SUCCESS) {
                ...
                //  matching entry!
                    ...
                    while (TRUE) {
                        ...
                        if (RegEnumValue(hKey, EnumIndex++, ValueName, &cbValueName, NULL, &Type, lpValueData, &cbValueData) != ERROR_SUCCESS)
                            break;

                        if (((g_FindFlags & FIND_VALUES) && FindCompare(ValueName)) || ((g_FindFlags & FIND_DATA) && IsRegStringType(Type) && FindCompare((PTSTR)g_ValueDataBuffer))) {
...
SelectTreeItem:
    ...
    if (!fFoundMatch)
        ...
        if (BestValueName[0] == 0)
            ...
            LVItem.iItem = ListView_FindItem(g_RegEditData.hValueListWnd, -1, &LVFindInfo);
        }
