...
INT_PTR PASCAL RegDisconnectDlgProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam);
INT_PTR PASCAL RegDisconnect_OnInitDialog(HWND hWnd);
VOID PASCAL RegDisconnect_OnCommandOk(HWND hWnd);
...
VOID PASCAL RegDisconnect_OnCommandOk(HWND hWnd)
{
    ...
    //  Walk through each selected item in the ListView and disconnect the computer.
    LVItem.mask = LVIF_PARAM;
    LVItem.iItem = -1;
    LVItem.iSubItem = 0;
    hRemoteListWnd = GetDlgItem(hWnd, IDC_REMOTELIST);
    while ((LVItem.iItem = ListView_GetNextItem(hRemoteListWnd, LVItem.iItem, LVNI_SELECTED)) != -1) {
