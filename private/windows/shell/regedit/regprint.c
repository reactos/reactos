...
VOID PASCAL PrintBranch(HKEY hKey, LPTSTR lpFullKeyName);
VOID PASCAL PrintLiteral(LPCTSTR lpLiteral);
VOID PASCAL PrintBinary(CONST BYTE FAR* lpBuffer, DWORD cbBytes);
BOOL PASCAL PrintChar(TCHAR Char);
...
static HRESULT CPrintCallback_QueryInterface(IPrintDialogCallback *ppcb, REFIID riid, void **ppv)
{
...
static ULONG CPrintCallback_AddRef(IPrintDialogCallback *ppcb)
{
    CPrintCallback *this = (CPrintCallback*)ppcb;
    return 1;
}
...
static ULONG CPrintCallback_Release(IPrintDialogCallback *ppcb)
{
    CPrintCallback *this = (CPrintCallback*)ppcb;
    return 1;
}
...
static HRESULT CPrintCallback_InitDone(IPrintDialogCallback *ppcb)
{
    return S_OK;
}


static HRESULT CPrintCallback_SelectionChange(IPrintDialogCallback *ppcb)
{
    return S_OK;
}


static HRESULT CPrintCallback_HandleMessage(IPrintDialogCallback *ppcb, HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT *pResult)
{
...
/*
*  DESCRIPTION:
*     Handles the selection of the "Print" option by the user for the RegEdit dialog box.
*  PARAMETERS:
...
*/
{
    ...
    if ((s_PrintIo.hRegPrintAbortWnd = CreateDialog(g_hInstance, MAKEINTRESOURCE(IDD_REGPRINTABORT), hWnd, RegPrintAbortDlgProc)) == NULL)
...
error_ShowDialog:
    if (s_PrintIo.ErrorStringID != 0)
        InternalMessageBox(g_hInstance, hWnd, MAKEINTRESOURCE(s_PrintIo.ErrorStringID), MAKEINTRESOURCE(IDS_REGEDIT), MB_ICONERROR | MB_OK);
}
...
VOID PASCAL PrintBranch(HKEY hKey, LPTSTR lpFullKeyName)
{
    ...
    while (s_PrintIo.fContinueJob) {
        ...
        if (RegEnumValue(hKey, EnumIndex++, g_ValueNameBuffer, &cbValueName, NULL, &Type, g_ValueDataBuffer, &cbValueData) != ERROR_SUCCESS)
            break;
    ...
    *lpSubKeyName = 0;
    ...
    while (s_PrintIo.fContinueJob) {
        if (RegEnumKey(hKey, EnumIndex++, lpSubKeyName, MAXKEYNAME) != ERROR_SUCCESS)
            break;
