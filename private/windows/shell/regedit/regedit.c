...
*/
{
    ...
    WndClassEx.hIconSm = LoadImage(g_hInstance, MAKEINTRESOURCE(IDI_REGEDIT), IMAGE_ICON, 16, 16, 0);
...
/*
*  DESCRIPTION:
*     Check the registry for a data structure that contains the last positions of our various interface components.
*/
{
    BOOL fSuccess;
    HKEY hKey;
    ...
    if (RegOpenKey(HKEY_CURRENT_USER, g_RegEditAppletKey, &hKey) == ERROR_SUCCESS) {
        //  Sort of a hack, but while we're here, pull the last find flags from the registry as well.
        cbValueData = sizeof(DWORD);

        RegQueryValueEx(hKey, (LPTSTR) g_RegEditFindFlagsValue, NULL, &Type, (LPBYTE) &g_FindFlags, &cbValueData);
        cbValueData = sizeof(REGEDITVIEW);
        if (RegQueryValueEx(hKey, (LPTSTR) g_RegEditViewValue, NULL, &Type, (LPBYTE) lpRegEditView, &cbValueData) == ERROR_SUCCESS && Type == REG_BINARY && cbValueData == sizeof(REGEDITVIEW))
            fSuccess = TRUE;
    ...
    //  they couldn't see them.  Without this validation, the only way to fix it is to run our application... ugh.
    if (fSuccess) {
    ...
    //  there was some sort of registry error), so pick some good(?) defaults for the various interface components.
    else {
        lpRegEditView-> Flags = REV_STATUSBARVISIBLE;

        //  Figure out how many pixels there are in two logical inches.  We use this
        //  to set the initial size of the TreeView pane (this is what the Cabinet does) and of the Name column of the ListView pane.
        hDC = GetDC(NULL);
...
*/
{
    switch (Message) {
        ...
        //  text hasn't changed until after we return from the end notification, so we post this dummy message to tell ourselves to do it later.
        case REM_UPDATESTATUSBAR:
            RegEdit_UpdateStatusBar();
            break;

        //  We must watch for this message to know that when we're in
        //  WM_INITMENUPOPUP, we're really looking at the main menu and not a context menu.
        case WM_INITMENU:
            g_RegEditData.fMainMenuInited = (g_RegEditData.hMainMenu == (HMENU) wParam);
            break;
        case WM_ACTIVATE:
...
VOID PASCAL RegEdit_ExpandKeyPath(LPTSTR lpExpandPath)
/*
...
*/
{
    ...
    //  Load several strings that will be using very often to display the keys and values.
    if ((g_RegEditData.pDefaultValue = LoadDynamicString(IDS_DEFAULTVALUE)) == NULL)
        return FALSE;

    if ((g_RegEditData.pValueNotSet = LoadDynamicString(IDS_VALUENOTSET)) == NULL)
        return FALSE;

    if ((g_RegEditData.pEmptyBinary = LoadDynamicString(IDS_EMPTYBINARY)) == NULL)
        return FALSE;
    ...
    if ((g_RegEditData.pNewKeyTemplate = LoadDynamicString(IDS_NEWKEYNAMETEMPLATE)) == NULL)
        return FALSE;

    if ((g_RegEditData.pNewValueTemplate = LoadDynamicString(IDS_NEWVALUENAMETEMPLATE)) == NULL)
        return FALSE;

    //  Create the left pane, a TreeView control that displays the keys of the registry.
    if ((g_RegEditData.hKeyTreeWnd = CreateWindowEx(WS_EX_CLIENTEDGE, WC_TREEVIEW, NULL, WS_CHILD | WS_VISIBLE | WS_TABSTOP | TVS_HASBUTTONS | TVS_DISABLEDRAGDROP | TVS_LINESATROOT | TVS_HASLINES | TVS_EDITLABELS,
        0, 0, 0, 0, hWnd, (HMENU) IDC_KEYTREE, g_hInstance, NULL)) == NULL)
        return FALSE;

    //  Create the right pane, a ListView control that displays the values of the currently selected key of the sibling TreeView control.
    if ((g_RegEditData.hValueListWnd = CreateWindowEx(WS_EX_CLIENTEDGE, WC_LISTVIEW, NULL,
    WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_TABSTOP | LVS_REPORT | LVS_ALIGNLEFT | LVS_EDITLABELS | LVS_SHAREIMAGELISTS | LVS_NOSORTHEADER, 0, 0, 0, 0, hWnd,
    ...
    //  because we need only one pane that's only used when scrolling through the menus.
    if ((g_RegEditData.hStatusBarWnd = CreateStatusWindow(WS_CHILD | SBARS_SIZEGRIP | CCS_NOHILITE, NULL, hWnd, IDC_STATUSBAR)) == NULL)
        return FALSE;

    g_RegEditData.StatusBarShowCommand = lpRegEditView-> Flags & REV_STATUSBARVISIBLE ? SW_SHOW : SW_HIDE;
    ...
    TVInsertStruct.item.mask = TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM | TVIF_CHILDREN;
    ...
    TVInsertStruct.hParent = TreeView_InsertItem(g_RegEditData.hKeyTreeWnd, &TVInsertStruct);
    ...
    //  in the tree.  Note we still keep the string around in case we can connect to the key remotely.
    ...
    TreeView_Expand(g_RegEditData.hKeyTreeWnd, TVInsertStruct.hParent, TVE_EXPAND);
    ...
    if (!g_RegEditData.fHaveNetwork) {
        hPopupMenu = GetSubMenu(g_RegEditData.hMainMenu, IDM_REGEDIT_REGISTRY_POPUP);
    ...
    if (RegOpenKey(HKEY_CURRENT_USER, g_RegEditAppletKey, &hKey) == ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueEx(hKey, (LPTSTR) g_RegEditLastKeyValue, NULL, &Type, (LPBYTE) lpKeyName, &cbValueData) == ERROR_SUCCESS && Type == REG_SZ && cbValueData > 0)
        {
...
VOID PASCAL RegEdit_OnDestroy(HWND hWnd)
{
    ...
    //  Write out a new RegEditView record to the registry for our next (hopeful?) activation.
...
INT_PTR PASCAL RegAddFavoriteDlgProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (Message) {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage(hWnd, IDC_FAVORITENAME, EM_SETLIMITTEXT, MAXKEYNAMEPATH, 0);
            ...
            switch (GET_WM_COMMAND_ID(wParam, lParam)) {
                case IDC_FAVORITENAME:
                    if (GET_WM_COMMAND_CMD(wParam, lParam) == EN_CHANGE)
                        EnableWindow(GetDlgItem(hWnd, IDOK), SendMessage(GET_WM_COMMAND_HWND(wParam, lParam), WM_GETTEXTLENGTH, 0, 0) != 0);
                    break;
                case IDOK:
                    GetDlgItemText(hWnd, IDC_FAVORITENAME, lpFavoriteName, MAXKEYNAMEPATH);
                    //  FALL THROUGH
                case IDCANCEL:
...
VOID PASCAL RegEdit_OnAddToFavorites(HWND hWnd)
/*
*  DESCRIPTION:
...
*/
{
    ...
    {
        KeyTree_BuildKeyPath(g_RegEditData.hKeyTreeWnd, TreeView_GetSelection(g_RegEditData.hKeyTreeWnd), lpKeyName, BKP_TOCOMPUTER);
        ...
        while (TRUE)
        {
            ...
            if (DialogBoxParam(g_hInstance, MAKEINTRESOURCE(IDD_ADDFAVORITE), hWnd, RegAddFavoriteDlgProc, (LPARAM) lpFavoriteName) != IDOK)
            {
                RegCloseKey(hKey);
                return;
            }
            ...
            if (*lpFavoriteName)
            {
                ...
                InternalMessageBox(g_hInstance, hWnd, MAKEINTRESOURCE(IDS_FAVORITEEXISTS), MAKEINTRESOURCE(IDS_FAVORITEERROR), MB_ICONERROR | MB_OK);
            }
        }
...
INT_PTR PASCAL RegRemoveFavoriteDlgProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    TCHAR KeyName[MAXKEYNAMEPATH*2];
    LPTSTR lpKeyName = KeyName;
...
VOID PASCAL RegEdit_OnRemoveFavorite(HWND hWnd)
/*
*  DESCRIPTION:
...
*/
{
    DialogBoxParam(g_hInstance, MAKEINTRESOURCE(IDD_REMFAVORITE), hWnd, RegRemoveFavoriteDlgProc, (LPARAM) NULL);
}


VOID PASCAL RegEdit_OnSelectFavorite(HWND hWnd, int DlgItem)
/*
*  DESCRIPTION:
*     Handles the selection of Favorites
...
*/
{
    ...
    if (RegOpenKey(HKEY_CURRENT_USER, g_RegEditFavoritesKey, &hKey) == ERROR_SUCCESS)
    {
        ...
        if (RegEnumValue(hKey, DlgItem - ID_ADDTOFAVORITES - 1, lpValueName, &cbValueName, NULL, &dwType, (LPBYTE) lpKeyName, &cbValueData) == ERROR_SUCCESS && dwType == REG_SZ && cbValueData > 0)
        {
            RegEdit_ExpandKeyPath(lpKeyName);
        }
...
VOID PASCAL RegEdit_OnFavorites(HWND hWnd, UINT uItem)
/*
*  DESCRIPTION:
*     Handles the selection of Favorites
...
/*
*  DESCRIPTION:
*     Handles the selection of a menu item by the user, notification messages from a child control, or translated accelerated keystrokes for the RegEdit window.
...
*/
{
    ...
    //  Check to see if this menu command should be handled by the main window's command handler.
    ...
    switch (DlgItem) {
        ...
        case ID_FINDNEXT:
            ...
            break;
        case ID_NEWKEY:
            RegEdit_OnNewKey(hWnd, TreeView_GetSelection(g_RegEditData.hKeyTreeWnd));
            break;
        ...
        //  Show or hide the status bar.  In either case, we'll need to resize the KeyTree and ValueList panes.
        case ID_STATUSBAR:
            g_RegEditData.StatusBarShowCommand = (g_RegEditData.StatusBarShowCommand == SW_HIDE) ? SW_SHOW : SW_HIDE;
            ShowWindow(g_RegEditData.hStatusBarWnd, g_RegEditData.StatusBarShowCommand);
            RegEdit_ResizeWindow(hWnd, RESIZEFROM_UNKNOWN);
            break;
        case ID_SPLIT:
        ...
        case ID_ABOUT:
            ...
            ShellAbout(hWnd, pAppName, g_NullString, LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_REGEDIT)));
            DeleteDynamicString(pAppName);
            break;

        //  Cycle the focus to the next pane when the user presses "tab".  The
        //  assumption is made that there are only two panes, so the tab direction doesn't really matter.
        case ID_CYCLEFOCUS:
            SetFocus(((g_RegEditData.hFocusWnd == g_RegEditData.hKeyTreeWnd) ? g_RegEditData.hValueListWnd : g_RegEditData.hKeyTreeWnd));
            break;
        case ID_HELPTOPICS:
        ...
        case IDC_KEYTREE:
            switch (lpNMHdr-> code) {
                case TVN_ITEMEXPANDING:
                    return RegEdit_OnKeyTreeItemExpanding(hWnd, (LPNM_TREEVIEW) lpNMHdr);
                case TVN_SELCHANGED:
                    RegEdit_OnKeyTreeSelChanged(hWnd, (LPNM_TREEVIEW) lpNMHdr);
                    break;
                case TVN_BEGINLABELEDIT:
                    return RegEdit_OnKeyTreeBeginLabelEdit(hWnd, (TV_DISPINFO FAR*) lpNMHdr);
                case TVN_ENDLABELEDIT:
                    return RegEdit_OnKeyTreeEndLabelEdit(hWnd, (TV_DISPINFO FAR*) lpNMHdr);
                case NM_SETFOCUS:
                    g_RegEditData.hFocusWnd = g_RegEditData.hKeyTreeWnd;
                    break;
            }
        ...
        case IDC_VALUELIST:
            switch (lpNMHdr-> code) {
                case LVN_BEGINLABELEDIT:
                    return RegEdit_OnValueListBeginLabelEdit(hWnd, (LV_DISPINFO FAR*) lpNMHdr);
                case LVN_ENDLABELEDIT:
                    return RegEdit_OnValueListEndLabelEdit(hWnd, (LV_DISPINFO FAR*) lpNMHdr);
                case NM_RETURN:
                case NM_DBLCLK:
    ...
    switch (MenuPosition) {
        case IDM_REGEDIT_REGISTRY_POPUP:
            if (g_RegEditData.fHaveNetwork) {
                ...
                hKeyTreeWnd = g_RegEditData.hKeyTreeWnd;
                EnableFlags = (TreeView_GetNextSibling(hKeyTreeWnd, TreeView_GetRoot(hKeyTreeWnd)) != NULL) ? MF_BYCOMMAND | MF_ENABLED : MF_BYCOMMAND | MF_GRAYED;
                EnableMenuItem(hPopupMenu, ID_DISCONNECT, EnableFlags);
            }
            break;
        case IDM_REGEDIT_EDIT_POPUP:
            if (g_RegEditData.hFocusWnd == g_RegEditData.hKeyTreeWnd) {
                //  Don't show items that are specific only to the ValueList context.
                ...
                //  Disable "Copy Key Name" for top-level items such as "My Computer" or a remote registry connection.
                EnableMenuItem(hPopupMenu, ID_COPYKEYNAME, (TreeView_GetParent(g_RegEditData.hKeyTreeWnd, hSelectedTreeItem) != NULL) ? (MF_BYCOMMAND | MF_ENABLED) : (MF_BYCOMMAND | MF_GRAYED));
                NewPopupPosition = IDM_EDIT_WHENKEY_NEW_POPUP;
            }
            else {
                //  Show menu items that are specific only to the ValueList context.
                if (GetMenuItemID(hPopupMenu, 0) != ID_MODIFY) {
                    InsertMenu(hPopupMenu, 0, MF_BYPOSITION | MF_SEPARATOR, 0, NULL);
                    InsertMenu(hPopupMenu, 0, MF_BYPOSITION | MF_STRING, ID_MODIFY, g_RegEditData.pModify);
                    SetMenuDefaultItem(hPopupMenu, 0, MF_BYPOSITION);
                }

                RegEdit_SetValueListEditMenuItems(hPopupMenu, ListView_GetNextItem(g_RegEditData.hValueListWnd, -1, LVNI_SELECTED));
                NewPopupPosition = IDM_EDIT_WHENVALUE_NEW_POPUP;
            }

            RegEdit_SetNewObjectEditMenuItems(GetSubMenu(hPopupMenu, NewPopupPosition));
            break;
        case IDM_REGEDIT_VIEW_POPUP:
            CheckMenuItem(hPopupMenu, ID_STATUSBAR, MF_BYCOMMAND | ((g_RegEditData.StatusBarShowCommand == SW_HIDE) ? MF_UNCHECKED : MF_CHECKED));
            break;
        case IDM_REGEDIT_FAVS_POPUP:
            ...
            EnableMenuItem(hPopupMenu, ID_ADDTOFAVORITES, (hSelectedTreeItem == TreeView_GetRoot(g_RegEditData.hKeyTreeWnd)) ? (MF_BYCOMMAND | MF_ENABLED) : (MF_BYCOMMAND | MF_GRAYED));
            break;
    }
}
...
*/
{
    ...
    //  API by sending it a normal menu item id.  This makes it easier to display context sensitive help for the popups, too.
...
*/
{
    ...
    while (GetMessage(&Msg, NULL, 0, 0)) {
        if (Msg.message == WM_KEYDOWN || Msg.message == WM_SYSKEYDOWN || (Msg.message >= WM_MOUSEFIRST && Msg.message <= WM_MOUSELAST)) {
            if (Msg.message == WM_LBUTTONUP || Msg.message == WM_LBUTTONDOWN || Msg.message == WM_RBUTTONDOWN)
                break;
...
*/
{
    ...
    MessagePos.x = g_RegEditData.xPaneSplit + GetSystemMetrics(SM_CXSIZEFRAME) / 2;
...
/*
*  DESCRIPTION:
*     Called whenever the size of the RegEdit window has changed or the size of its child controls should be adjusted.
...
*/
{
    ...
    //  resize comes from a change in the splitter position to avoid some flicker.
...
BOOL PASCAL RegEdit_SetImageLists(HWND hWnd)
{
    ...
    if ((hImageList = ImageList_Create(cxSmIcon, cySmIcon, uFlags, IDI_LASTIMAGE - IDI_FIRSTIMAGE + 1, 1)) == NULL)
        return FALSE;

    //  Initialize the image list with all of the icons that we'll be using
    //  throughout the Registry Editor (at least this window!).  Once set, send its handle to all interested child windows.
...
/*
*  DESCRIPTION:
*     Queries the system for any desired system colors and sets window attributes as necessary.
*/
{
    ...
    //  Optimize the drawing of images by informing interested parties of the background color.
    //  This lets ImageLists avoid extra BitBlts (biggie) and ListViews do some minor stuff.
...
/*
*  DESCRIPTION:
*     Simple logic to show or hide the wait cursor.  Assumes that we won't be called by multiple layers, so no wait cursor count is maintained.
*  PARAMETERS:
