...
*/
{
    ...
    hRootKey = KeyTree_BuildKeyPath(g_RegEditData.hKeyTreeWnd, hTreeItem, KeyName, BKP_TOSUBKEY);
    if(RegOpenKeyEx(hRootKey,KeyName,0,KEY_CREATE_SUB_KEY,&hKey) != ERROR_SUCCESS) {
        //  Get the text of the selected tree item so that we can display a more meaningful error message.
    ...
    if (fNewKeyIsOnlyChild) {
        ...
        hNewTreeItem = KeyTree_InsertItem(g_RegEditData.hKeyTreeWnd, hTreeItem, TVI_LAST, KeyName, FALSE, 0);
    }
...
error_CloseKey:
    ...
    //  BUGBUG:  For any errors that may crop up, we may need to turn off the child flag.
error_ShowDialog:
    InternalMessageBox(g_hInstance, hWnd, MAKEINTRESOURCE(ErrorStringID), MAKEINTRESOURCE(IDS_NEWKEYERRORTITLE), MB_ICONERROR | MB_OK, (LPTSTR) KeyName);
}
...
*     we call to the real worker routine, RegEdit_KeyTreeSelChanged, or delay the call for several milliseconds.
...
*/
{
    ...
    //  as the user scrolls through the tree.  (This behavior is directly taken from the Explorer.)
    switch (g_RegEditData.SelChangeTimerState) {
        ...
        TimerDelay = 1;
#else
        TimerDelay = (lpNMTreeView != NULL && lpNMTreeView-> action == TVC_BYMOUSE) ? (1) : (GetDoubleClickTime() * 3 / 2);
#endif
        SetTimer(hWnd, SELCHANGE_TIMER_ID, TimerDelay, NULL);
            g_RegEditData.SelChangeTimerState = SCTS_TIMERSET;
            break;

        //  We want to punt the first selection change notification that comes through.
...
/*
*  DESCRIPTION:
*     Called several milliseconds after a keyboard operation has selected a new item in the KeyTreeWnd.
*     Act as if a new selection has just been made in the KeyTreeWnd.
*  PARAMETERS:
*     hWnd, handle of RegEdit window.
*/
{
...
/*
*  DESCRIPTION:
*     Called after a new item has been selected in the KeyTreeWnd.
*     Opens a registry key to the new branch and notifies the ValueListWnd to update itself.
...
*/
{
    ...
    if (g_RegEditData.SelChangeTimerState != SCTS_INITIALIZING) {
        //  Draw an animation that shows the "expansion" of the newly selected tree item to the ListView.
    ...
    //  "My Computer" or a network computer name.  Right now, nothing is displayed in the ListView, so just empty it and return.
    if (TreeView_GetParent(hKeyTreeWnd, hSelectedTreeItem) != NULL) {
        //  Build a registry path to the selected tree item and open a registry key.
...
*/
{
    //  B#7933:  We don't want the user to hurt themselves by making it too easy to rename keys and values.
    //  Only allow renames via the menus.
...
*/
{
    ...
    //  Attempt to open the key to be renamed.  This may or may not be the same key that is already open.
    hRootKey = KeyTree_BuildKeyPath(hKeyTreeWnd, lpTVDispInfo-> item.hItem, SourceKeyName, BKP_TOSUBKEY);
    ...
    //  and replace the old key name with the new.  Make sure that this key doesn't exceed our internal buffers.
    ...
    *lpEndOfParentKey = 0;
    ...
    //  Check to see if we're renaming the currently selected key.  If so, toss our cached key handle and change to our source key.
    if (TreeView_GetSelection(hKeyTreeWnd) == lpTVDispInfo-> item.hItem) {
        ...
        //  won't be updated until we return TRUE from this message.  So we must post a message to tell ourselves to do the update later on.
...
*/
{
    ...
    //  control will let you activate the context menu of one tree item while another one is really the selected tree item.
    if (hTreeItem == NULL)
    ...
    switch (MenuCommand) {
        ...
        case ID_NEWBINARYVALUE:
            if (hTreeItem != TreeView_GetSelection(hKeyTreeWnd)) {
                //  Force the selection to occur now, so that we're dealing with the right open key.
...
VOID PASCAL RegEdit_OnKeyTreeContextMenu(HWND hWnd, BOOL fByAccelerator)
{
    ...
    //  context menu.  Following the Cabinet's convention, this menu is placed at (0,0) of the KeyTree client area.
    if (fByAccelerator) {
        MessagePoint.x = 0;
        MessagePoint.y = 0;
    ...
    if (TVItem.state & TVIS_EXPANDED)
        ModifyMenu(hContextPopupMenu, ID_TOGGLE, MF_BYCOMMAND | MF_STRING, ID_TOGGLE, g_RegEditData.pCollapse);

    if (MenuID == IDM_COMPUTER_CONTEXT) {
        if (g_RegEditData.fHaveNetwork) {
            if (TreeView_GetPrevSibling(hKeyTreeWnd, TVItem.hItem) == NULL)
                EnableMenuItem(hContextPopupMenu, ID_DISCONNECT, MF_GRAYED | MF_BYCOMMAND);
        }
        else {
...
VOID PASCAL RegEdit_OnKeyTreeDelete(HWND hWnd, HTREEITEM hTreeItem)
{
    ...
    if (InternalMessageBox(g_hInstance, hWnd, MAKEINTRESOURCE(IDS_CONFIRMDELKEYTEXT), MAKEINTRESOURCE(IDS_CONFIRMDELKEYTITLE), MB_ICONWARNING | MB_YESNO) != IDYES)
        return;
    ...
    if (RegDeleteKeyRecursive(hRootKey, KeyName) == ERROR_SUCCESS) {
        ...
        //  See if the key that we just deleted was the last child of its parent.  If so, remove the expand/collapse button.
        ...
        //  Make sure we can see the selected tree item now since it may be currently off-screen.
...
VOID PASCAL RegEdit_OnKeyTreeRefresh(HWND hWnd)
{
    ...
    while (TRUE) {
        ...
        hRootKey = KeyTree_BuildKeyPath(hKeyTreeWnd, CurrentTVItem.hItem, KeyName, BKP_TOSUBKEY);
        if (CurrentTVItem.state & TVIS_EXPANDED) {
            //  If this isn't a top-level label (and it won't be if hRootKey is
            //  not NULL), then compare the actual contents of the registry against what we're showing.
            if(hRootKey && RegOpenKeyEx(hRootKey,KeyName,0,KEY_ENUMERATE_SUB_KEYS,&hEnumKey) == ERROR_SUCCESS) {
                ...
                //  following algorithm to work correctly, we must now sort these keys.
                TreeView_SortChildren(hKeyTreeWnd, CurrentTVItem.hItem, FALSE);

                //  Build a sorted dynamic array of strings that represent the keys actually in the registry at this time.
                ...
                //  Does this key have subkeys anymore?  If not, then we need to reset it's child flag and remove all of it's children.
                if (Index == 0) {
                ...
                //  with the keys that our key tree lists.  Add and remove elements from the tree as appropriate.
                lpDPAKeyName = DPA_FastGetPtr(hDPA, --Index);

                EnumTVItem.hItem = TreeView_GetChild(hKeyTreeWnd, CurrentTVItem.hItem);
                if (EnumTVItem.hItem)
                ...
                while (Index >= 0 && EnumTVItem.hItem != NULL) {
                    ...
                    if (CompareResult == 0) {
                        EnumTVItem.hItem = TreeView_GetNextSibling(hKeyTreeWnd, EnumTVItem.hItem);
                        if (EnumTVItem.hItem)
                            TreeView_GetItem(hKeyTreeWnd, &EnumTVItem);
...
GetNextDPAPointer:
                        ...
                        hTempTreeItem = TreeView_GetNextSibling(hKeyTreeWnd, EnumTVItem.hItem);
            ...
            //  If this isn't a top-level label (and it won't be if hRootKey is not NULL), then re-check if this key has any children.
            if (hRootKey != NULL) {
                TreeView_Expand(hKeyTreeWnd, CurrentTVItem.hItem, TVE_COLLAPSE | TVE_COLLAPSERESET);
...
SetCurrentTreeItem:
            ...
            //  walk to the siblings of this tree item.  And if no siblings exist, we walk back to the parent and check again for siblings.
            while (TRUE) {
                ...
                if ((CurrentTVItem.hItem = TreeView_GetParent(hKeyTreeWnd, CurrentTVItem.hItem)) == NULL) {
                    //  We've now walked over all of the tree items, so do any cleanup here and exit.
                    DPA_Destroy(hDPA);
                    SetWindowRedraw(hKeyTreeWnd, TRUE);

                    //  The selection may have changed as a result of having the focus on an nonexistent key.
...
/*
*  DESCRIPTION:
*     Callback comparision routine for refresh's DPA_Sort call.  Simply returns the result of lstrcmpi.
...
/*
*  DESCRIPTION:
*     Show the full registry path in the status bar, for lack of anything better to do with it.
*/
{
    HWND hKeyTreeWnd;
    ...
    hKeyTreeWnd = g_RegEditData.hKeyTreeWnd;
    KeyTree_BuildKeyPath(hKeyTreeWnd, TreeView_GetSelection(hKeyTreeWnd), KeyName, BKP_TOCOMPUTER);
...
VOID PASCAL RegEdit_OnCopyKeyName(HWND hWnd, HTREEITEM hTreeItem)
{
    ...
    KeyTree_BuildKeyPath(g_RegEditData.hKeyTreeWnd, hTreeItem, KeyName, BKP_TOSYMBOLICROOT);
...
*/
{
    ...
    //  Computer" or a network computer name.  It's children are already filled in and are always valid.
    ...
    //  used to specify TVI_SORT, but on NT, expanding a key with several subkeys (e.g., HKEY_CLASSES_ROOT) would take several seconds!
...
*/
{
    ...
    if(RegOpenKeyEx(hKey,lpKeyName,0,KEY_QUERY_VALUE|KEY_ENUMERATE_SUB_KEYS, &hCheckChildrenKey) == ERROR_SUCCESS) {
        if (RegQueryInfoKey(hCheckChildrenKey, NULL, NULL, NULL, &cSubKeys, NULL, NULL, NULL, NULL, NULL, NULL, NULL) == ERROR_SUCCESS && cSubKeys > 0)
            fHasKids = TRUE;
...
*/
{
    ...
    if (hTreeItem != NULL) {
        //  Check if this tree item has any reference data indicating that it is a predefined root.
        //  Predefined roots cannot be renamed or deleted.
        TVItem.hItem = hTreeItem;
        TVItem.mask = TVIF_PARAM;
        TreeView_GetItem(hWnd, &TVItem);
        if ((HKEY) TVItem.lParam == NULL) {
            //  Check that this isn't a top-level item such as "My Computer" or a remote registry connection.
