...
// in Win 4.0 regedit was brain damaged (See comments in ImportRegFile for details)
...
VOID NEAR PASCAL ImportNewerRegFile(VOID);
VOID NEAR PASCAL ParseHeader(LPHKEY lphKey);
VOID NEAR PASCAL ParseValue(HKEY hKey, LPCTSTR lpszValueName);
VOID NEAR PASCAL ParseValuename(HKEY hKey);
BOOL NEAR PASCAL ParseString(LPTSTR lpString, LPDWORD cbStringData);
BOOL NEAR PASCAL ParseHexSequence(LPBYTE lpHexData, LPDWORD lpcbHexData);
BOOL NEAR PASCAL ParseHexDword(LPDWORD lpDword);
BOOL NEAR PASCAL ParseHexByte(LPBYTE lpByte);
BOOL NEAR PASCAL ParseHexDigit(LPBYTE lpDigit);
BOOL NEAR PASCAL ParseEndOfLine(VOID);
VOID NEAR PASCAL SkipWhitespace(VOID);
VOID NEAR PASCAL SkipPastEndOfLine(VOID);
BOOL NEAR PASCAL GetChar(PTCHAR lpChar);
VOID NEAR PASCAL UngetChar(VOID);
BOOL NEAR PASCAL MatchChar(TCHAR CharToMatch);
BOOL NEAR PASCAL IsWhitespace(TCHAR Char);
BOOL NEAR PASCAL IsNewLine(TCHAR Char);
VOID NEAR PASCAL PutBranch(HKEY hKey, LPTSTR lpKeyName);
VOID NEAR PASCAL PutLiteral(LPCTSTR lpString);
VOID NEAR PASCAL PutString(LPCTSTR lpString);
VOID NEAR PASCAL PutBinary(CONST BYTE FAR* lpBuffer, DWORD Type, DWORD cbBytes);
VOID NEAR PASCAL PutDword(DWORD Dword, BOOL fLeadingZeroes);
VOID NEAR PASCAL PutChar(TCHAR Char);
    ...
    )
{
    ...
    // file, we have to write all of the data to the registry using RegSetValueExA, because it was read from the registry using RegQueryValueExA.
    ...
    if ((g_ImportFileVersion < 0x0500) && ((REG_EXPAND_SZ == Type) || (REG_MULTI_SZ == Type))) {
        ...
        WideCharToMultiByte(CP_THREAD_ACP, 0, lpszValueName, -1, AnsiValueName, MAXVALUENAME_LENGTH, NULL, NULL);
        if (RegSetValueExA(hKey, AnsiValueName, 0, Type, g_ValueDataBuffer, cbData) != ERROR_SUCCESS)
            g_FileErrorStringID = IDS_IMPFILEERRREGSET;
    }
    else {
#endif // UNICODE
        if (RegSetValueEx(hKey, lpszValueName, 0, Type, g_ValueDataBuffer, cbData) != ERROR_SUCCESS)
            g_FileErrorStringID = IDS_IMPFILEERRREGSET;
#ifdef UNICODE
    }
#endif // UNICODE
...
BOOL NEAR PASCAL ParseString(LPTSTR lpString, LPDWORD lpcbStringData)
{
    TCHAR Char;
