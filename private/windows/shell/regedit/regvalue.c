...
//  Maximum number of bytes that will be shown in the ListView.
//  If the user wants to see more, then they can use the edit dialogs.
#define SIZE_DATATEXT                   196

//  Allow room in a SIZE_DATATEXT buffer for one null and possibly the ellipsis.
#define MAXIMUM_STRINGDATATEXT          192
const TCHAR s_StringDataFormatSpec[] = TEXT("%.192s");

//  Allow room for multiple three character pairs, one null, and possibly the ellipsis.
...
*/
{
    ...
    if ((ListIndex = ListView_InsertItem(g_RegEditData.hValueListWnd, &LVItem)) != -1) {
        ValueList_SetItemDataText(g_RegEditData.hValueListWnd, ListIndex, g_ValueDataBuffer, cbValueData, Type);
...
*/
{
    //  B#7933:  We don't want the user to hurt themselves by making it too easy to rename keys and values.  Only allow renames via the menus.
    //  We don't get any information on the source of this editing action, so we must maintain a flag that tells us whether or not this is "good".
    if (!g_RegEditData.fAllowLabelEdits)
        return TRUE;

    //  All other labels are fair game.  We need to disable our keyboard accelerators so that the edit control can "see" them.
    g_fDisableAccelerators = TRUE;
    return FALSE;
}


BOOL PASCAL RegEdit_OnValueListEndLabelEdit(HWND hWnd, LV_DISPINFO FAR* lpLVDispInfo)
{
    HWND hValueListWnd;
    DWORD cbValueData;
    DWORD Ignore;
    ...
    //  We can reenable our keyboard accelerators now that the edit control no longer needs to "see" them.
...
/*
*  DESCRIPTION:
*     Handles the selection of a menu item by the user intended for the ValueList child window.
*  PARAMETERS:
*     hWnd, handle of RegEdit window.
...
VOID PASCAL RegEdit_OnValueListContextMenu(HWND hWnd, BOOL fByAccelerator)
{
    ...
    //  If fByAcclerator is TRUE, then the user hit Shift-F10 to bring up the context menu.
    //  Following the Cabinet's convention, this menu is placed at (0,0) of the ListView client area.
    if (fByAccelerator) {
        MessagePoint.x = 0;
        MessagePoint.y = 0;
...
/*
*  DESCRIPTION:
*     Shared routine between the main menu and the context menu to setup the edit menu items.
*  PARAMETERS:
...
*/
{
    ...
    //  The rename option is also only enabled when a single item is selected and that item cannot be the default item.
    //  EnableFlags is already disabled if the SelectedCount is not one from above.
...
VOID PASCAL RegEdit_OnValueListModify(HWND hWnd)
{
    HWND hValueListWnd;
    UINT SelectedCount;
    int ListIndex;
    ...
    //  Verify that we only have one item selected for editing and "notify" the user if this is not the case.
    //  Don't beep if the user is just double-clicking on the background.
    ...
    // RegQueryValueEx on an empty string value is supposed to return the size of the terminating NULL.
    // But the W version returns 1 instead of 2.
    // So we don't have a properly terminated Unicode string and we can read past it into garbage.
    ...
    if (RegError != ERROR_SUCCESS) {
        ...
        //  If the default value didn't exist, then Type isn't necessarily valid.
        //  Here, we make sure it's a string which by definition must be true.
        if (ListIndex == 0) {
            Type = REG_SZ;
            ((TCHAR*)g_ValueDataBuffer)[0] = 0;
        }
    ...
    //  This is kinda funky-- if this is the default value, then we zapped the first byte of "(Default Value") up above to do the RegQueryValueEx.
...
VOID PASCAL RegEdit_OnValueListRefresh(HWND hWnd)
{
    HWND hValueListWnd;
    LV_ITEM LVItem;
    DWORD cbValueName;
    ...
    if (g_RegEditData.hCurrentSelectionKey != NULL) {
        ...
        while (TRUE) {
            ...
            ValueList_SetItemDataText(hValueListWnd, ListIndex, g_ValueDataBuffer, cbValueData, Type);
        }
...
*/
{
    ...
    //  When pValueData is NULL, then that's a special indicator to us that this is the default value and it's value is undefined.
    if (pValueData == NULL)
...
*/
{
    ...
    //  We have to set the focus to the ListView or else ListView_EditLabel will return FALSE.
    //  While we're at it, clear the selected state of all the items to eliminate some flicker when we move the focus back to this pane.
