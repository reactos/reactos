...
INT_PTR ThanksDlgFunc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    ReturnValue = DialogBox(hInst, MAKEINTRESOURCE(IDD_THANK_YOU), hWnd, ThanksDlgProc);
    if (ReturnValue == -1)
    {
...
INT_PTR CALLBACK ThanksDlgProc(HWND hDlg, unsigned message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
...
BOOL ThanksDlgDefault(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case TXT_WEBSITE:
            EndDialog(hDlg, IDOK);
            LoadString(hInst, IDS_WEDSITE, &str[0], 256);//load our web URL
