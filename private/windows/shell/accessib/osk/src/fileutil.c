...
//   HKEY hkGlobal;
   ...
   sa.lpSecurityDescriptor = &sdPermissions;
    ...
    lResult= RegCreateKeyEx(HKEY_CURRENT_USER, &pathbuff[0], 0, TEXT("Application Per-User Data"), REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, &sa, &hkPerUser, &dwDisposition);
   ...
   // say by loss of electrical power.
    ...
    if((lResult != ERROR_SUCCESS) || (dwStepping < CURRENT_STEPPING))
    {
        ...
        RegSetValueEx(hkPerUser, pathbuff, 0, REG_BINARY, g_DefaultSettings, sizeof(g_DefaultSettings));
        cbData= sizeof(KBPREFINFO);
        lResult=RegQueryValueEx(hkPerUser, &pathbuff[0], NULL, &dwType, (LPBYTE)kbPref, &cbData);
        ...
        RegSetValueEx(hkPerUser, pathbuff, 0, REG_DWORD, (LPBYTE)&dwStepping, sizeof(DWORD));
    }
...
//      MessageBox(0, TEXT("Running as administrator"), TEXT("Admin"), MB_OK);
   ...
   // say by loss of electrical power.
    ...
    lstrcpy(pathbuff, TEXT("Setting"));
    lResult= RegSetValueEx(hkPerUser, &pathbuff[0], 0, REG_BINARY, (LPBYTE) kbPref, sizeof(KBPREFINFO));
