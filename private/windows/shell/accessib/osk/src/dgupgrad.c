...
INT_PTR UpgradeDlgFunc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    ReturnValue = DialogBox(hInst,  MAKEINTRESOURCE(DLG_UPGRADE), NULL, (DLGPROC)UpgradeDlgProc);
    if (ReturnValue==-1)
    {
...
INT_PTR CALLBACK UpgradeDlgProc(HWND hDlg, unsigned message, WPARAM wParam, LPARAM lParam)
{
    TCHAR str[256]=TEXT("");
    ...
    case WM_PAINT:
        {
            ...
            hBitMap = (HBITMAP) LoadImage(hInst, MAKEINTRESOURCE(IDB_BITMAP4), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);
