...
BOOL CMagnifyDlg::OnInitDialog()
{
    ...
    m_wndStationary.SetHidden(FALSE);// HACK: Make it work with utility Manager: Change post beta3
    SetTimer(idEvent, 10, NULL);// Show the warning message after 100 ms...
...
void CMagnifyDlg::OnEndSession( BOOL bEnding )
{
    ...
    if(bEnding)
    {
        ...
        // Save in RunOnce key
        RegCreateKeyEx(HKEY_CURRENT_USER, m_szMagSubkey,0, NULL, REG_OPTION_NON_VOLATILE , KEY_QUERY_VALUE | KEY_SET_VALUE, NULL, &hSubkey, &dwPosition );
