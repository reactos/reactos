    ...
    void ApplyOriginal()
    {
        ...
        if (ERROR_SUCCESS == RegCreateKeyEx(HKEY_CURRENT_USER, HC_KEY, 0, __TEXT(""), REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hkey, &dwDisposition))
        {
            len = (lstrlen(m_schemeCurrent.m_HIGHCONTRAST.lpszDefaultScheme) + 1) * sizeof(TCHAR);
             RegSetValueEx(hkey, HIGHCONTRAST_SCHEME, 0, REG_SZ, (PBYTE) m_schemeCurrent.m_HIGHCONTRAST.lpszDefaultScheme, len);
             RegSetValueEx(hkey, VOLATILE_SCHEME, 0, REG_SZ, (PBYTE) m_schemeCurrent.m_HIGHCONTRAST.lpszDefaultScheme, len);
             RegCloseKey(hkey);
             hkey = NULL;
        }
#endif
