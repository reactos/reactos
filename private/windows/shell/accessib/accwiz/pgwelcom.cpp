...
CWelcomePg::CWelcomePg(LPPROPSHEETPAGE ppsp) : WizardPage(ppsp, IDS_TEXTSIZETITLE, IDS_TEXTSIZESUBTITLE)
{
...
LRESULT CWelcomePg::OnDrawItem(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    // For each button, Check the state, And if the button is selected, means that it has current focus, So Re-paint the previously hilighted and the current selected buttons....
