...
static HKEY hkeyShutdown = NULL;
    ...
    case WM_INITDIALOG:
        ...
        if (RegCreateKeyEx(HKEY_CURRENT_USER, SHUTDOWN_SETTING_KEY, 0, 0, 0, KEY_READ | KEY_WRITE | DELETE, NULL, &hkeyShutdown, &dwDisposition) == ERROR_SUCCESS) {
...
static HKEY hkeyShutdown = NULL;
    ...
    case WM_COMMAND:
        ...
        case IDOK2:
            ...
            // Restore the shutdown privilege state
            if (!WasEnabled) {
                Status = RtlAdjustPrivilege(SE_SHUTDOWN_PRIVILEGE, (BOOLEAN)WasEnabled, (BOOLEAN)FALSE, &WasEnabled);
            }
        case IDCANCEL:
        case IDCANCEL2:
            if (hkeyShutdown) {
