...
Abstract:
    ...
    keep in the user window manager, then sets active focus to the selected dialog box element(ie active window).
--*/
...
VOID DoEndTask(HWND hwnd )
{
   ...
   /*
    * We don't want to let someone do an EndTask on progman or EndTask of a EndTask dialog (which would make progman die)
    */
   ...
   if (!EndTask(hwnd, FALSE, FALSE)) {
       ...
       */
        ...
        */
        ...
        if( MessageBox( NULL, szMsgBoxText, szTempField, MB_SETFOREGROUND | MB_SYSTEMMODAL | MB_YESNO ) == IDYES) {
        ...
        case IDD_CASCADE:
            {
                ...
                if (wParam == IDD_CASCADE) {
                     ...
                     */
                    TileChildWindows(hwndDesktop, ((GetKeyState(VK_SHIFT) & 0x8000) ? MDITILE_HORIZONTAL : MDITILE_VERTICAL));
                }
                break;
            }
...
//              INT  idButton - ID of button
{
    ...
    SendMessage( GetDlgItem(hwndDlg, idButton), BM_SETSTYLE, MAKEWPARAM( BS_DEFPUSHBUTTON, 0 ), MAKELPARAM( TRUE, 0 ));
}
