...
DWORD CAddSheet::Add(LPTSTR UserName, LPTSTR DnName, PVOID UserCert, PSID UserSid /*= NULL */, DWORD Flag /*= USERINFILE*/, PVOID Context /*= NULL*/)
{
    return m_Users.Add(UserName, DnName, UserCert, UserSid, Flag, Context );
}


DWORD CAddSheet::Remove(LPCTSTR UserName, LPCTSTR UserCertName )
{
    return m_Users.Remove(UserName, UserCertName);
}


PVOID CAddSheet::StartEnum()
{
    return m_Users.StartEnum();
}


PVOID CAddSheet::GetNextUser(PVOID Token, CString &UserName, CString &CertName)
{
    return m_Users.GetNextUser(Token, UserName, CertName);
}


void CAddSheet::ClearUserList(void)
{
    m_Users.Clear();
}


DWORD CAddSheet::AddNewUsers(void)
{
