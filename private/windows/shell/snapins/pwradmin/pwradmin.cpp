...
STDAPI DllRegisterServer(void)
{
    ...
    RegSetValueEx (hKey, TEXT("NameString"), 0, REG_SZ, (LPBYTE)szExtensionName, (lstrlen(szExtensionName) + 1) * sizeof(TCHAR));
    RegCloseKey (hKey);
    ...
    RegSetValueEx (hKey, szExtensionKey, 0, REG_SZ, (LPBYTE)szExtensionName, (lstrlen(szExtensionName) + 1) * sizeof(TCHAR));
    RegCloseKey (hKey);
    ...
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\MMC\\NodeTypes\\{A6B4EEC1-B681-11D0-9484-080036B11A03}\\Extensions\\NameSpace"), 0, KEY_WRITE, &hKey) == ERROR_SUCCESS) {
    ...
    RegSetValueEx (hKey, NULL, 0, REG_SZ, (LPBYTE)szNodeTypeName, (lstrlen(szNodeTypeName) + 1) * sizeof(TCHAR));
    RegCloseKey (hKey);
