    ...
    )
{
    ...
    Rc = RegCreateKeyEx(hGptKey, TEXT(POWER_CFG_KEY), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &PowerCfgKey, &dwDisposition);
    if (Rc == ERROR_SUCCESS) {
        Rc = RegSetValueEx(PowerCfgKey, ValueName, 0, REG_DWORD, (CONST BYTE *)&dwValue, sizeof(ULONG));
    ...
    )
{
    ...
    if (ERROR_SUCCESS == RegOpenKeyEx(hGptKey, TEXT(POWER_CFG_KEY), 0, KEY_QUERY_VALUE, &PowerCfgKey)) {
        dwSize = sizeof(ULONG);
        if (ERROR_SUCCESS == RegQueryValueEx(PowerCfgKey, ValueName, NULL, &dwType, (LPBYTE)pValue, &dwSize)) {
            bSuccess = TRUE;
        }
