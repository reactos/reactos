...
HRESULT EXPORT CTVCtrl::XDMTVOCX::QueryInterface(REFIID iid, void ** ppvObj)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HTREEITEM EXPORT CTVCtrl::XDMTVOCX::InsertItem(LPTV_INSERTSTRUCT pis)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::DeleteItem(HTREEITEM hitem)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HIMAGELIST EXPORT CTVCtrl::XDMTVOCX::SetImageList(INT iImage, HIMAGELIST himl)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::SetItem(TV_ITEM* pitem)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::Expand(UINT Flags, HTREEITEM hitem)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::SelectItem(UINT Flags, HTREEITEM hitem)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::SetStyle(DWORD dwStyle)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::GetItem(TV_ITEM* pti)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HTREEITEM EXPORT CTVCtrl::XDMTVOCX::GetNextItem(UINT Flags, HTREEITEM htiRef)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::SelectItem(HTREEITEM hti)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::Connect(IComponent* pIComponent, MMC_COOKIE  cookie)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
HRESULT EXPORT CTVCtrl::XDMTVOCX::SetActiveConnection(MMC_COOKIE cookie)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
BOOL EXPORT CTVCtrl::XDMTVOCX::EnsureVisible(HTREEITEM hitem)
{
    METHOD_PROLOGUE(CTVCtrl, DMTVOCX)
...
const IID BASED_CODE IID_DTV = { 0xcd6c7866, 0x5864, 0x11d0, { 0xab, 0xf0, 0, 0x20, 0xaf, 0x6b, 0xb, 0x7a } };
const IID BASED_CODE IID_DTVEvents = { 0xcd6c7867, 0x5864, 0x11d0, { 0xab, 0xf0, 0, 0x20, 0xaf, 0x6b, 0xb, 0x7a } };
...
void CTVCtrl::OnDraw(CDC* pdc, const CRect& rcBounds, const CRect& rcInvalid)
{
...
/// Tree View functions
HRESULT CTVCtrl::Connect(IComponent* pIComponent, MMC_COOKIE cookie)
{
    HRESULT hr = S_OK;
    ...
    if (0 == m_nConnections)
    {
        ...
        hr = m_pIComponent->QueryInterface(IID_ISnapinCallback, reinterpret_cast<void**>(&m_pISnapinCallback));
    }
...
HRESULT CTVCtrl::SetActiveConnection(MMC_COOKIE cookie)
{
...
MMC_COOKIE CTVCtrl::GetActiveConnection()
{
    return m_ActiveCookie;
}


HTREEITEM CTVCtrl::InsertItem(LPTV_INSERTSTRUCT pis)
{
    return (HTREEITEM)SendMessage(TVM_INSERTITEM, 0, (LPARAM)pis);
}


HRESULT CTVCtrl::DeleteItem(HTREEITEM hitem)
{
...
HIMAGELIST CTVCtrl::SetImageList(INT iImage, HIMAGELIST hmil)
{
    return (HIMAGELIST)SendMessage(TVM_SETIMAGELIST, (WPARAM)iImage, (LPARAM)hmil);
}


HRESULT CTVCtrl::SetItem(TV_ITEM* pitem)
{
...
HRESULT CTVCtrl::Expand(UINT Flags, HTREEITEM hitem)
{
...
HRESULT CTVCtrl::SelectItem(UINT Flags, HTREEITEM hitem)
{
...
HRESULT CTVCtrl::SetStyle(DWORD dwStyle)
{
...
HRESULT CTVCtrl::GetItem(TV_ITEM* pti)
{
...
HTREEITEM CTVCtrl::GetNextItem(UINT Flags, HTREEITEM htiRef)
{
    return (HTREEITEM) SendMessage(TVM_GETNEXTITEM, (WPARAM)Flags, (LPARAM)htiRef);
}


HRESULT CTVCtrl::SelectItem(HTREEITEM hti)
{
...
HTREEITEM CTVCtrl::HitTest(LONG x, LONG y, UINT* pFlags)
{
    POINT pt;
    pt.x = x;
    pt.y = y;
...
HRESULT CTVCtrl::SetRedraw(BOOL Redraw)
{
    if (Redraw)
        Invalidate();
    return S_OK;
...
BOOL CTVCtrl::EnsureVisible(HTREEITEM hitem)
{
...
LRESULT CTVCtrl::OnOcmNotify(WPARAM wParam, LPARAM lParam)
{
    ...
    switch (((NMHDR*)lParam)->code)
    {
        ...
        case NM_CLICK:
        case NM_DBLCLK:
            NotifyCode = DoMouseNotification(((NMHDR*)lParam)->code, &cookie, &arg, &param);
            break;
        case TVN_KEYDOWN:
    ...
    if (TV_NOTIFY_CODE_UNKNOWN != NotifyCode && m_pISnapinCallback)
    {
        ...
        if (S_FALSE == hr)
        {
            ...
            else if (TV_NOTIFY_CODE_KEYDOWN == NotifyCode && (VK_F10 == param && GetKeyState(VK_SHIFT) < 0) || (VK_APPS == param))
            {
                RECT rect;
...
TV_NOTIFY_CODE CTVCtrl::DoMouseNotification(UINT Code, MMC_COOKIE* pcookie, LPARAM* parg, LPARAM* pparam)
{
    DWORD MsgPos;
    POINT point;
...
void CTVCtrl::OnContextMenu(CWnd* pWnd, CPoint point)
{
    ...
    if (hti)
    {
        ...
        if (SUCCEEDED(GetItem(&TI)))
        {
            m_pISnapinCallback->tvNotify(*this, (MMC_COOKIE)TI.lParam, TV_NOTIFY_CODE_CONTEXTMENU, (LPARAM)hti, (LPARAM)&point );
        }
    }
}
...
BOOL CTVCtrl::PreTranslateMessage(MSG* pMsg)
{
    ...
    if (WM_KEYDOWN == pMsg->message && (VK_DELETE == pMsg->wParam || VK_RETURN == pMsg->wParam))
    {
        OnKeyDown((UINT)pMsg->wParam, LOWORD(pMsg->lParam), HIWORD(pMsg->lParam));
        return TRUE;
    }

    else if (WM_SYSKEYDOWN == pMsg->message && VK_F10 == pMsg->wParam && GetKeyState(VK_SHIFT) < 0)
    {
