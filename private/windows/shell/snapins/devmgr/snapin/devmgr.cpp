...
BOOL DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
    ...
    case DLL_PROCESS_ATTACH:        
        DisableThreadLibraryCalls(hInstance);// we do not need thread attach/detach calls        
        InitCommonControls();// do must be done
        ...
        InitGlobals(hInstance);// initiailze our global stuff
        break;
...
BOOL InitGlobals(HINSTANCE hInstance)
{
    ...
    try
    {        
        g_strDevMgr.LoadString(hInstance, IDS_NAME_DEVMGR);//preload strings
