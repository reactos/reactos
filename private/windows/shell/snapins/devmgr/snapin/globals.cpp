...
{
    // according to wsprintf specification, the max buffer size is 1024
    TCHAR Buffer[1024];
    va_list arglist;
    ...
    )
{
    ...
    FormatMessage(FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM, NULL, Error, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), szMsg, ARRAYLEN(szMsg), NULL);
...
BOOL AddToolTips(HWND hDlg, UINT id, LPCTSTR pszText, HWND *phwnd)
{
    if (*phwnd == NULL)
    {
