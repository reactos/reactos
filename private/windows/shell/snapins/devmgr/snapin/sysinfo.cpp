...
// disk drive root template name. Used to retreive the disk's media information or geometry
const TCHAR* const DRIVE_ROOT = TEXT("\\\\.\\?:");
const int DRIVE_LETTER_IN_DRIVE_ROOT = 4;

// disk drive root directory template name. Used to retreive the disk's total and free space
const TCHAR* const DRIVE_ROOT_DIR = TEXT("?:\\");
const int DRIVE_LETTER_IN_DRIVE_ROOT_DIR = 0;


// Registry various subkey and value names used to retreive system information
...
CSystemInfo::CSystemInfo(CMachine* pMachine)
{
    // assuming the machine is a local machine and initialize the registry root key as well.
...
BOOL CSystemInfo::GetDiskInfo(int  Drive, DISK_INFO& DiskInfo)
{
    ...
    if (DRIVE_UNKNOWN == DriveType || DRIVE_REMOTE == DriveType || DRIVE_NO_ROOT_DIR == DriveType)
    {
        return FALSE;
    }
...
DWORD CSystemInfo::WindowsVersion(TCHAR* Buffer, DWORD  BufferSize)
{
    ...
    if (m_fLocalMachine)
    {
        ...
        {
            wsprintf(Temp, TEXT(" %d.%d"), OsVerInfo.dwMajorVersion, OsVerInfo.dwMinorVersion);
...
#endif
    {
        ...
        if (regWindowsNT.Open(m_hKeyMachine, REG_PATH_WINDOWS_NT, KEY_READ))
        {
            DWORD Type, Size;
            Size = sizeof(Temp);
            if (regWindowsNT.GetValue(REG_VALUE_CURRENTVERSION, &Type, (PBYTE)Temp, &Size))
            {
                lstrcat(FinalText, Temp);
            }
            Size = sizeof(Temp);
            if (regWindowsNT.GetValue(REG_VALUE_CSDVERSION, &Type, (PBYTE)Temp, &Size) && Size)
            {
                lstrcat(FinalText, TEXT(" "));
                lstrcat(FinalText, Temp);
            }
            Size = sizeof(Temp);
            if (regWindowsNT.GetValue(REG_VALUE_CURRENTBUILDNUMBER, &Type, (PBYTE)Temp, &Size) && Size)
            {
                DWORD FinalLen = lstrlen(FinalText);
...
{
    // validate parameters
    if (!SubkeyName || !ValueName || _T('\0') == *SubkeyName || _T('\0') == *SubkeyName || (!Buffer && BufferSize))
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return 0;
    }
...
DWORD CSystemInfo::SystemBiosName(TCHAR* Buffer, DWORD BufferSize)
{
    SetLastError(ERROR_SUCCESS);
    return 0;
}
...
DWORD CSystemInfo::SystemBiosDate(TCHAR* Buffer, DWORD BufferSize)
{
    return InfoFromRegistry(REG_PATH_HARDWARE_SYSTEM, REG_VALUE_SYSTEMBIOSDATE, Buffer, BufferSize);
}
...
DWORD CSystemInfo::SystemBiosVersion(TCHAR* Buffer, DWORD BufferSize)
{
    return InfoFromRegistry(REG_PATH_HARDWARE_SYSTEM, REG_VALUE_SYSTEMBIOSVERSION, Buffer, BufferSize);
}
...
DWORD CSystemInfo::MachineType(TCHAR* Buffer, DWORD BufferSize)
{
    return InfoFromRegistry(REG_PATH_HARDWARE_SYSTEM, REG_VALUE_MACHINETYPE, Buffer, BufferSize);
}
...
DWORD CSystemInfo::RegisteredOwner(TCHAR* Buffer, DWORD  BufferSize)
{
    return InfoFromRegistry(REG_PATH_WINDOWS_NT, REG_VALUE_REGISTERED_OWNER, Buffer, BufferSize);
}
...
DWORD CSystemInfo::RegisteredOrganization(TCHAR* Buffer, DWORD  BufferSize)
{
...
// OUTPUT:
//  Number of processor.
DWORD CSystemInfo::NumberOfProcessors()
{
...
DWORD CSystemInfo::ProcessorVendor(TCHAR* Buffer, DWORD BufferSize)
{
...
//  have the same type, therefore, this function does not take
//  processor number as a parameter.
DWORD CSystemInfo::ProcessorType(TCHAR* Buffer, DWORD BufferSize)
{
    return ProcessorInfo(REG_VALUE_CPU_TYPE, Buffer, BufferSize);
}


DWORD CSystemInfo::ProcessorInfo(LPCTSTR ValueName, TCHAR* Buffer, DWORD  BufferSize)
{
    ...
    if (regCPU.Open(m_hKeyMachine, REG_PATH_CPU, KEY_READ))
    {
        ...
        while (CPUInfoSize <= sizeof(TCHAR) && regCPU.EnumerateSubkey(CPUIndex, CPUKey, &Size))
        {
...
DWORD CSystemInfo::ProcessorType(TCHAR* Buffer, DWORD BufferSize)
{
    ...
    case PROCESSOR_ARCHITECTURE_INTEL:
        ...
        if (3 == SysInfo.wProcessorLevel || 4 == SysInfo.wProcessorLevel)
        {

            int StringId = (3 == SysInfo.wProcessorLevel) ? IDS_CPU_INTEL_386 : IDS_CPU_INTEL_486;
            LoadString(g_hInstance, StringId, CPUType, ARRAYLEN(CPUType));

            if (SysInfo.wProcessorRevision & 0xFF00 == 0xFF00)
            {
                LoadString(g_hInstance, IDS_CPU_REVISION_MODEL_STEPPING, Format, ARRAYLEN(Format));
                wsprintf(Revision, Format,
                    ((SysInfo.wProcessorRevision & 0x00F0) > 4) - 10, SysInfo.wProcessorRevision & 0x000F);
            }
            else
            {
                LoadString(g_hInstance, IDS_CPU_REVISION_STEPPING, Format, ARRAYLEN(Format));
                wsprintf(Revision, Format,
                    (SysInfo.wProcessorRevision >> 8) + _T('A'), SysInfo.wProcessorRevision & 0x00FF);
            }
        }
        else if (5 == SysInfo.wProcessorLevel)
        {
            ...
            LoadString(g_hInstance, IDS_CPU_REVISION_MODEL_STEPPING, Format, ARRAYLEN(Format));
            wsprintf(Revision, Format, SysInfo.wProcessorRevision >> 8, SysInfo.wProcessorRevision & 0x00FF);
        }
    ...
    case PROCESSOR_ARCHITECTURE_ALPHA:
        ...
        LoadString(g_hInstance, IDS_CPU_REVISION_ALPHA, Format, ARRAYLEN(Format));
        wsprintf(Revision, Format, (SysInfo.wProcessorRevision >> 8) + _T('A'), SysInfo.wProcessorRevision & 0x00FF);
        lstrcat(CPUType, Revision);
    default:
        CPUType[0] = _T('\0');
    }
...
// OUTPUT:
//  Total Memory in KB
void CSystemInfo::TotalPhysicalMemory(ULARGE_INTEGER& Size)
{
    if (m_fLocalMachine)
    {
        SYSTEM_BASIC_INFORMATION SysBasicInfo;
        NTSTATUS Status;
        Status = NtQuerySystemInformation(SystemBasicInformation, (PVOID)&SysBasicInfo, sizeof(SysBasicInfo), NULL);
        if (NT_SUCCESS(Status))
        {
            Size.QuadPart = Int32x32To64(SysBasicInfo.PageSize, SysBasicInfo.NumberOfPhysicalPages);
        }
        else
        {
