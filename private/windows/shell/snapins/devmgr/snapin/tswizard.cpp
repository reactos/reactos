...
const CMPROBLEM_INFO ProblemInfo[NUM_CM_PROB] =
{
    // no problem
    {
        TRUE,        FIX_COMMAND_TROUBLESHOOTER,        IDS_INST_WORKING_TROUBLESHOOTER,        1,        IDS_FIXIT_TROUBLESHOOTER
    },
    // CM_PROB_NOT_CONFIGURED
    {
        TRUE,        FIX_COMMAND_REINSTALL,        IDS_INST_REINSTALL,        1,        IDS_FIXIT_REINSTALL
    },
    // CM_PROB_DEVLOADER_FAILED
    {
        TRUE,        FIX_COMMAND_TROUBLESHOOTER,        IDS_INST_TROUBLESHOOTER,        1,        IDS_FIXIT_TROUBLESHOOTER
    },
    ...
    {
        TRUE,        FIX_COMMAND_RESTARTCOMPUTER,        IDS_INST_RESTARTCOMPUTER,        1,        IDS_FIXIT_RESTARTCOMPUTER
    },
    ...
    // CM_PROB_DISABLED
    {
        TRUE,        FIX_COMMAND_ENABLEDEVICE,        IDS_INST_ENABLEDEVICE,        1,        IDS_FIXIT_ENABLEDEVICE
    },
...
CProblemAgent::CProblemAgent(CDevice* pDevice, ULONG Problem, BOOL SeparateProcess)
{
    m_pDevice = pDevice;
    m_Problem = Problem;
...
DWORD CProblemAgent::InstructionText(LPTSTR Buffer, DWORD  BufferSize)
{
...
DWORD CProblemAgent::FixitText(LPTSTR Buffer, DWORD BufferSize)
{
...
BOOL CProblemAgent::FixIt(HWND hwndOwner)
/*++
    Lanuches a troubleshooter based on the m_FixCommand.
Arguments:
    hwndOwner - Parent window handle
Return Value:
    TRUE if launching the troubleshooter changed the device in some way so that the UI on the general tab needs to be refreshed.
    FALSE if launching the troubleshooter did not change the device in any way.
--*/
{
    BOOL Result;
    ...
    m_pDevice->m_pMachine->DiSetClassInstallParams(*m_pDevice, (PSP_CLASSINSTALL_HEADER)&tsp, sizeof(tsp));
    ...
    if (m_pDevice->m_pMachine->DiCallClassInstaller(DIF_TROUBLESHOOTER, *m_pDevice)) {
        ...
        m_pDevice->m_pMachine->DiGetClassInstallParams(*m_pDevice, (PSP_CLASSINSTALL_HEADER)&tsp, sizeof(tsp), &RequiredSize);
    }
...
BOOL CProblemAgent::UpgradeDriver(HWND hwndOwner, CDevice* pDevice)
{
    return CDeviceDriverPage::UpdateDriver(pDevice, hwndOwner, FALSE, NULL);
}


BOOL CProblemAgent::Reinstall(HWND hwndOwner, CDevice* pDevice)
{
    return UpgradeDriver(hwndOwner, pDevice);
}


BOOL CProblemAgent::EnableDevice(HWND hwndOwner, CDevice* pDevice)
{
...
BOOL CProblemAgent::RestartComputer(HWND hwndOwner, CDevice* pDevice)
{
    HWND hwndFocus;
...
BOOL ParseTroubleShooter(LPCTSTR TSString, LPTSTR ChmFile, LPTSTR HtmlTroubleShooter)
{
    // verify parameters
...
BOOL CProblemAgent::GetTroubleShooter(CDevice* pDevice, LPTSTR ChmFile, LPTSTR HtmlTroubleShooter)
{
    BOOL Result = FALSE;
    ...
    try {
        ...
        hKey = pDevice->m_pMachine->DiOpenDevRegKey(*pDevice, DICS_FLAG_GLOBAL, 0, DIREG_DRV, KEY_READ);
        if (INVALID_HANDLE_VALUE != hKey)
        {
            ...
            if (regDrv.GetValue(TroubleShooterKey, &regType, (PBYTE) TroubleShooterValue, &Len))
            {
        ...
        if (!Result) {
            ...
            if (INVALID_HANDLE_VALUE != hKey)
            {
                ...
                if (regClass.GetValue(TroubleShooterKey, &regType, (PBYTE) TroubleShooterValue, &Len))
                {
                    if (ParseTroubleShooter(TroubleShooterValue, ChmFile, HtmlTroubleShooter)) {
                        Result = TRUE;
                    }
        ...
        // If we still don't have a TroubleShooter then try the default TroubleShooters key.
        if (!Result) {
            CSafeRegistry regDevMgr;
            CSafeRegistry regTroubleShooters;

            if (regDevMgr.Open(HKEY_LOCAL_MACHINE, REG_PATH_DEVICE_MANAGER) && regTroubleShooters.Open(regDevMgr, REG_STR_TROUBLESHOOTERS)) {
                DWORD regType;
                DWORD Len = sizeof(TroubleShooterValue);

                // get the TroubleShooter value from the default TroubleShooters key
                if (regTroubleShooters.GetValue(TroubleShooterKey, &regType, (PBYTE) TroubleShooterValue, &Len))
                {
                    if (ParseTroubleShooter(TroubleShooterValue, ChmFile, HtmlTroubleShooter)) {
                        Result = TRUE;
                    }
...
void CProblemAgent::LaunchHtlmTroubleShooter(HWND hwndOwner, LPTSTR ChmFile, LPTSTR HtmlTroubleShooter)
{
...
BOOL CProblemAgent::StartTroubleShooter(HWND hwndOwner, CDevice* pDevice, LPTSTR ChmFile, LPTSTR HtmlTroubleShooter)
{
    if ((ERROR_DI_DO_DEFAULT == GetLastError()) && ((ChmFile[0] != TEXT('\0')) && (HtmlTroubleShooter[0] != TEXT('\0')))) {
    ...
    // Return FALSE since launching the troubleshooter does not change the device in any way.
    return FALSE;
}


CWizard98::CWizard98(HWND hwndParent, UINT MaxPages)
{
    m_MaxPages = 0;
