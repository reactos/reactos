...
//the constant is the size we use to allocate GUID list from within stack when we have to build a GUID list.
...
BOOL CDevInfoList::DiGetExtensionPropSheetPage(
...
BOOL CDevInfoList::InstallDevInst(HWND hwndParent, LPCTSTR DeviceId, BOOL    UpdateDriver, DWORD* pReboot)
{
    BOOL Result = FALSE;
...
DWORD CDevInfoList::DiGetFlags(PSP_DEVINFO_DATA DevData)
{
...
DWORD CDevInfoList::DiGetExFlags(PSP_DEVINFO_DATA DevData)
{
    SP_DEVINSTALL_PARAMS dip;
    dip.cbSize = sizeof(dip);
    if (DiGetDeviceInstallParams(DevData, &dip))
...
BOOL CDevInfoList::DiTurnOnDiFlags(PSP_DEVINFO_DATA DevData, DWORD FlagsMask)
{
    SP_DEVINSTALL_PARAMS dip;
    dip.cbSize = sizeof(dip);
    if (DiGetDeviceInstallParams(DevData, &dip))
...
BOOL CDevInfoList::DiTurnOffDiFlags(PSP_DEVINFO_DATA DevData, DWORD FlagsMask)
{
    SP_DEVINSTALL_PARAMS dip;
    dip.cbSize = sizeof(dip);
    if (DiGetDeviceInstallParams(DevData, &dip))
...
BOOL CDevInfoList::DiTurnOnDiExFlags(PSP_DEVINFO_DATA DevData, DWORD FlagsMask)
{
    SP_DEVINSTALL_PARAMS dip;
    dip.cbSize = sizeof(dip);
    if (DiGetDeviceInstallParams(DevData, &dip))
...
BOOL CDevInfoList::DiTurnOffDiExFlags(PSP_DEVINFO_DATA DevData, DWORD FlagsMask)
{
    SP_DEVINSTALL_PARAMS dip;
    dip.cbSize = sizeof(dip);
    if (DiGetDeviceInstallParams(DevData, &dip))
...
CMachine::~CMachine()
{
    ...
    while (!m_listPropertySheets.IsEmpty() && (iSecondsCount++ < 10)) {
        ...
        // Sleep for .5 seconds and then try again.  This will give the property pages time to finish up their work.
        Sleep(500);
    }
