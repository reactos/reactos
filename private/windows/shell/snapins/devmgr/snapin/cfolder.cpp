...
HRESULT CScopeItem::GetDisplayInfo(LPSCOPEDATAITEM pScopeDataItem)
{
    if (!pScopeDataItem)
        return E_INVALIDARG;
...
BOOL CScopeItem::EnumerateChildren(int Index, CScopeItem** ppScopeItem)
{
...
CCookie* CScopeItem::FindSelectedCookieData(CResultView** ppResultView)
{
    CFolder* pFolder;
    ...
    while (NULL != pos)
    {
        ...
        {
            if (!pFolder->m_bSelect && (pResultView = pFolder->GetCurResultView()) != NULL)
            {
                if (ppResultView)
                {
...
CFolder* CScopeItem::CreateFolder(CComponent* pComponent)
{
    ASSERT(pComponent);
...
HRESULT CScopeItem::AddMenuItems(LPCONTEXTMENUCALLBACK pCallback, long* pInsertionAllowed)
{
...
HRESULT CScopeItem::MenuCommand(long lCommandId)
{
    CCookie* pSelectedCookie;
    CResultView* pResultView;
...
HRESULT CScopeItem::QueryPagesFor()
{
...
HRESULT CScopeItem::CreatePropertyPages(LPPROPERTYSHEETCALLBACK lpProvider, LONG_PTR handle)
{
    CCookie* pSelectedCookie;
    CResultView* pResultView;
...
CFolder::CFolder(CScopeItem* pScopeItem, CComponent* pComponent)
{
...
HRESULT CFolder::Compare(MMC_COOKIE cookieA, MMC_COOKIE cookieB, int  nCol, int* pnResult)
{
    ASSERT(pnResult);
...
HRESULT CFolder::GetDisplayInfo(LPRESULTDATAITEM pResultDataItem)
{
    if (!pResultDataItem)
        return E_POINTER;
...
BOOL CFolder::SetDescBarText(LPCTSTR DescText)
{
...
HRESULT CFolder::AddMenuItems(CCookie* pCookie, LPCONTEXTMENUCALLBACK pCallback, long* pInsertionAllowed)
{
    ASSERT(pCookie);
    ...
    if (NULL == pCookie->GetResultItem()) {
        ...
        if (*pInsertionAllowed & CCM_INSERTIONALLOWED_VIEW)
        {
            ...
            for (int i = 0; i < TOTAL_VIEWS; i++)
            {
                ...
                hr = AddMenuItem(pCallback, ViewDevicesMenuItems[i].idName, ViewDevicesMenuItems[i].idStatusBar, ViewDevicesMenuItems[i].lCommandId, CCM_INSERTIONPOINTID_PRIMARY_VIEW, Flags, 0);
                if (FAILED(hr))
                    break;
            }
            ...
            if (SUCCEEDED(hr))
            {
                hr = AddMenuItem(pCallback, 0, 0, 0, CCM_INSERTIONPOINTID_PRIMARY_VIEW, MF_ENABLED, CCM_SPECIAL_SEPARATOR);
                if (SUCCEEDED(hr))
                {
            ...
            if (SUCCEEDED(hr))
            {
                hr = AddMenuItem(pCallback, 0, 0, 0, CCM_INSERTIONPOINTID_PRIMARY_VIEW, MF_ENABLED, CCM_SPECIAL_SEPARATOR);
                if (SUCCEEDED(hr))
                    hr = AddMenuItem(pCallback, IDS_PRINT, IDS_MENU_STATUS_PRINT, IDM_PRINT, CCM_INSERTIONPOINTID_PRIMARY_VIEW, MF_ENABLED, 0);
            }
        }
    }
    else
...
HRESULT CFolder::MenuCommand(CCookie* pCookie, long lCommandId)
{
...
HRESULT CFolder::QueryPagesFor(CCookie* pCookie)
{
    // we do not have property pages for scope item
    if (NULL == pCookie->GetResultItem()) {
        ASSERT(m_pScopeItem == pCookie->GetScopeItem());
...
HRESULT CFolder::CreatePropertyPages(CCookie* pCookie, LPPROPERTYSHEETCALLBACK lpProvider, LONG_PTR handle)
{
    if (NULL == pCookie->GetResultItem()) {
        ASSERT(m_pScopeItem == pCookie->GetScopeItem());
        return S_OK;
...
BOOL CFolder::SelectView(VIEWTYPE ViewType, BOOL     fShowHiddenDevices)
{
    CResultView* pNewView;

    if (m_CurViewType == ViewType && m_ShowHiddenDevices == fShowHiddenDevices && m_pCurView)
    {
        return TRUE;
    }
...
HRESULT CFolder::OnShow(BOOL fShow)
{
    if (fShow && !m_pMachine)
    {
        ASSERT(m_pComponent);
            ...
            if (m_FirstTimeOnShow && m_Show)
            {
                ...
                if (!m_pMachine->IsLocal())
                {
                    ...
                    if (VerifyMachineName(m_pMachine->GetRemoteMachineFullName()) != CR_SUCCESS)
                    {
                        ...
                        m_pComponent->m_pConsole->MessageBox(WarningMsg, (LPCTSTR)g_strDevMgr, MB_ICONEXCLAMATION | MB_OK, &ReturnValue);
...
HRESULT CFolder::OnRestoreView(BOOL* pfHandled)
{
    ASSERT(pfHandled);
...
HRESULT CFolder::GetResultViewType(LPOLESTR* ppViewType, long*     pViewOptions)
{
    ASSERT(pViewOptions);
...
HRESULT CFolder::Reset()
{
    // delete all views so that we will create new ones when OnShow is called.
    if (m_pViewTreeByType)
    {
        delete m_pViewTreeByType;
        m_pViewTreeByType = NULL;
...
HRESULT CFolder::MachinePropertyChanged(CMachine* pMachine)
{
    if (m_pCurView)
...
HRESULT CFolder::GetPersistData(PBYTE pBuffer, int BufferSize)
{
...
HRESULT CFolder::SetPersistData(PBYTE pData, int Size)
{
    if (!pData)
        return E_POINTER;
    ...
    {
        if (VIEW_DEVICESBYTYPE == pStates->CurViewType || VIEW_DEVICESBYCONNECTION == pStates->CurViewType)
...
HRESULT CFolder::tvNotify(HWND hwndTV, CCookie* pCookie, TV_NOTIFY_CODE Code, LPARAM arg, LPARAM param)
{
    if (m_pCurView)
...
HRESULT CFolder::OnOcxNotify(MMC_NOTIFY_TYPE event, LPARAM arg, LPARAM param)
{
    if (m_pCurView)
...
HRESULT CResultView::OnShow(BOOL fShow)
{
    if (!fShow)
        return S_OK;
    ...
    if (NULL == m_pIDMTVOCX)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (!strStartupCommand.IsEmpty() && !strStartupDeviceId.IsEmpty() && !strStartupCommand.CompareNoCase(DEVMGR_COMMAND_PROPERTY))
            {
...
inline LPCTSTR CResultView::GetStartupDeviceId()
{
    return m_pFolder->m_pComponent->GetStartupDeviceId();
}


inline LPCTSTR CResultView::GetStartupCommand()
{
    return m_pFolder->m_pComponent->GetStartupCommand();
}


HRESULT CResultView::MachinePropertyChanged(CMachine* pMachine)
...
void CResultView::SaveTreeStates(CCookie* pCookieStart)
...
void CResultView::DestroySavedTreeStates()
{
...
void CResultView::RestoreSavedTreeState(CCookie* pCookie)
// This function restores the expanded and selected state for the cookie.

// INPUT:
//      pCookie -- cookie to restore states for
{
    // If the cookie was expanded before, mark it so that DisplayTree will expand it.
    if (!m_listExpandedItems.IsEmpty())
    {
...
BOOL CResultView::DisplayTree()
{
    ...
    if (m_pCookieComputer) {
        ...
        // if we have a pre-selected item, use it. Otherwise, use computer as the selected node.
        HTREEITEM hSelectedItem = (m_pSelectedCookie && m_pSelectedCookie->m_lParam) ? (HTREEITEM)m_pSelectedCookie->m_lParam : (HTREEITEM)m_pCookieComputer->m_lParam;
        SetSelectOk(TRUE);
...
// OUTPUT:
//      none.
BOOL CResultView::DisplaySubtree(HTREEITEM htiParent, CCookie* pCookie, BOOL* pReportProblem)
{
    TV_INSERTSTRUCT ti;
    ...
    while (pCookie)
    {
        ...
        if (COOKIE_TYPE_RESULTITEM_DEVICE == pCookie->GetType())
        {
            ...
            if (!fShowHiddenDevices && pDevice->IsHidden()) {
                ...
                // Continue on with the next device. This will skip all of the display code below.
        ...
        else if (COOKIE_TYPE_RESULTITEM_CLASS == pCookie->GetType())
        {
            ...
            // If we don't have any devices to show for this class, or this is a NoDisplayClass and we are not showing hidden devices,
            // then just get our next sibling and continue without showing this class.
                ...
                COOKIE_TYPE_RESULTITEM_RESOURCE_IRQ == pCookie->GetType())
        {
            ...
            // If this is a FORCED CONFIG resource then overlay the forced config icon
        ...
        if (NULL != hti)
        {
            ...
            if (pCookieChild)
            {
                if (bResource && htiParent && GetDescriptionStringID() == IDS_STATUS_RESOURCES_BYTYPE)
                {
                    // This is a child of a resource being viewed by type,
                    // so the tree needs to be flattened.  This is done by using the same parent.
                    DisplaySubtree(htiParent, pCookieChild, &ChildProblem);
                }
                else
                {
...
HRESULT CResultView::GetResultViewType(LPOLESTR* ppViewType, long*     pViewOptions)
{
...
HRESULT CResultView::AddMenuItems(
    CCookie* pCookie,
        ...
        switch (pCookie->GetType())
        {
            case COOKIE_TYPE_RESULTITEM_DEVICE:
                if(m_pMachine->IsLocal() && g_HasLoadDriverNamePrivilege)
                {
                    ...
                    // Only show the Enable/Disable menu item if the device can be disabled.
                    if (pDevice->IsDisableable())
                    {
                        if (pDevice->IsStateDisabled())
                        {
                            hr = AddMenuItem(pCallback, IDS_ENABLE, 0, IDM_ENABLE, CCM_INSERTIONPOINTID_PRIMARY_TOP, MF_ENABLED, 0);
                        }
                        else
                        {
                            hr = AddMenuItem(pCallback, IDS_DISABLE, 0, IDM_DISABLE, CCM_INSERTIONPOINTID_PRIMARY_TOP, MF_ENABLED, 0);
                        }
                    }

                    // Only show the uninstall menu item if the device can be uninstalled.
                    if (SUCCEEDED(hr) && pDevice->IsUninstallable())
                    {
                        hr = AddMenuItem(pCallback, IDS_REMOVE, 0, IDM_REMOVE, CCM_INSERTIONPOINTID_PRIMARY_TOP, MF_ENABLED, 0);
                    }
                }
            ...
            case COOKIE_TYPE_RESULTITEM_CLASS:
                if (SUCCEEDED(hr))
                {
                    hr = AddMenuItem(pCallback, 0, 0, 0, CCM_INSERTIONPOINTID_PRIMARY_TOP, MF_ENABLED, CCM_SPECIAL_SEPARATOR);
                }
                if (SUCCEEDED(hr))
                {
                    hr = AddMenuItem(pCallback, IDS_REFRESH, IDS_MENU_STATUS_SCAN_CHANGES, IDM_REFRESH, CCM_INSERTIONPOINTID_PRIMARY_TOP, MF_ENABLED, 0);
                }
                if (fContextMenu)
                {
                    if (SUCCEEDED(hr))
                    ...
                    if (SUCCEEDED(hr))
                    {
                        hr = AddMenuItem(pCallback, IDS_PROPERTIES, 0, IDM_PROPERTIES, CCM_INSERTIONPOINTID_PRIMARY_TOP, MF_DEFAULT, CCM_SPECIAL_DEFAULT_ITEM);
                    }
                }
                ...
                if (SUCCEEDED(hr))
                    hr = AddMenuItem(pCallback, IDS_TROUBLESHOOTING, 0, IDM_TROUBLESHOOTING, CCM_INSERTIONPOINTID_PRIMARY_TOP, TshootFlags, TshootSpecialFlags);
#endif
                break;
...
//                  S_XXX error code.
HRESULT CResultView::MenuCommand(CCookie* pCookie, long     lCommandId)
{
    HRESULT hr = S_OK;
    ...
    switch (lCommandId)
    {
        ...
        case IDM_REFRESH:
            ...
            if (m_pMachine->GetActivePropSheetCount())
            {
                ...
                int ReturnValue;
                m_pFolder->m_pComponent->m_pConsole->MessageBox(szText, m_pMachine->GetMachineDisplayName(), MB_ICONEXCLAMATION | MB_OK, &ReturnValue);
            }
            else
            {
...
HRESULT CResultView::QueryPagesFor(CCookie* pCookie)
{
    ASSERT(pCookie);
...
//                  S_XXX error code.
HRESULT CResultView::CreatePropertyPages(CCookie* pCookie, LPPROPERTYSHEETCALLBACK lpProvider, LONG_PTR    handle)
{
// Design issue:
            ...
            if (ppsd->Create(g_hInstance, m_hwndTV, MAX_PROP_PAGES, handle))
            {
                ...
                if (pClassDevInfo && pClassDevInfo->DiGetClassDevPropertySheet(NULL, &ppsd->m_psh, MAX_PROP_PAGES, DIGCDP_FLAG_ADVANCED))
                {
                    ...
                    if (pClassDevInfo->DiGetFlags(NULL) & DI_GENERALPAGE_ADDED)
                    {
                        ...
                        int ReturnValue;
                        m_pFolder->m_pComponent->m_pConsole->MessageBox(szText, pClass->GetDisplayName(), MB_ICONEXCLAMATION | MB_OK, &ReturnValue);
        ...
        case COOKIE_TYPE_RESULTITEM_DEVICE:
            ...
            if (ppsd->Create(g_hInstance, m_hwndTV, MAX_PROP_PAGES, handle))
            {
                ...
                if (DiFlags & DI_GENERALPAGE_ADDED)
                {
                    ...
                    int ReturnValue;
                    m_pFolder->m_pComponent->m_pConsole->MessageBox(szText, pDevice->GetDisplayName(), MB_ICONEXCLAMATION | MB_OK, &ReturnValue);
                        // fall through to create our general page.
                }
                CDeviceGeneralPage* pGenPage = new CDeviceGeneralPage;
                GenPagePtr.Attach(pGenPage);
                ...
                {
                    m_pMachine->DiGetExtensionPropSheetPage(*pDevice, AddPropPageCallback, SPPSR_SELECT_DEVICE_RESOURCES, (LPARAM)ppsd);
                }
...
HRESULT CResultView::tvNotify(HWND hwndTV, CCookie* pCookie, TV_NOTIFY_CODE Code, LPARAM arg, LPARAM param)
{
    HRESULT hr;
    if (m_hwndTV != hwndTV)
        return S_FALSE;
...
HRESULT CResultView::UpdateConsoleVerbs(CCookie* pCookie)
{
...
HRESULT CResultView::OnOcxNotify(MMC_NOTIFY_TYPE event, LPARAM arg, LPARAM param)
{
    HRESULT hr = S_OK;
    TV_ITEM TI;
...
//              S_XXXX other error.
HRESULT CResultView::DoProperties(HWND hwndTV, CCookie* pCookie)
{
    HRESULT hr;
    ...
    if (FAILED(pComponent->QueryInterface(IID_IExtendPropertySheet, (void**) &pExtendSheet)) ||
        ...
        FAILED(m_pFolder->m_pComponent->m_pConsole->QueryInterface(IID_IPropertySheetProvider, (void**) &pSheetProvider)) ||
...
//                 screen coordinate.
HRESULT CResultView::DoContextMenu(HWND hwndTV, CCookie* pCookie, POINT* pPoint)
{
    HRESULT hr = S_FALSE;
    ...
    // I am sure that there is more of these bugs lurking around in the code and this needs to be addressed by a better overall change after NT 5.0.
    if (pCookie) {
        CDevice *pDevice;
        CResource *pResource;
        CClass *pClass;
    ...
    if (FAILED(pComponent->QueryDataObject((MMC_COOKIE)pCookie, CCT_RESULT, &pDataObject)) ||
        FAILED(m_pFolder->m_pComponent->m_pConsole->QueryInterface(IID_IContextMenuProvider, (void**)&pMenuProvider)))
    {
        hr = S_FALSE;
        goto clean0;
    }
    ...
    if (SUCCEEDED(pMenuProvider->AddItem(&MenuItem)) && SUCCEEDED(pMenuProvider->AddPrimaryExtensionItems(pComponent, pDataObject)) &&
...
#ifdef PSS_TROUBLESHOOTING
HRESULT CResultView::DoTroubleshooting(CCookie* pCookie)
{
...
#ifdef DISPLAY_STATUS_BAR
HRESULT CResultView::UpdateViewDescText(LPCTSTR NewText)
{
    if (NewText)
...
HRESULT CResultView::RemoveDevice(CDevice* pDevice)
{
    ...
    // the property sheet to the foreground if the user agree to do so.
    HWND hwndPropSheet;
    ...
    if (hwndPropSheet)
    {
        ...
        MsgBoxResult = m_pFolder->m_pComponent->MessageBox(szText, pDevice->GetDisplayName(), MB_ICONEXCLAMATION | MB_OKCANCEL);
        if (IDOK == MsgBoxResult)
            SetForegroundWindow(hwndPropSheet);

        // Can not wait for the property sheet because it is running in a separate thread.
        return S_OK;
    }
    ...
    // device change notification. When the removal is done, we will re-enable the refresh.
    m_pMachine->EnableRefresh(FALSE);
...
HRESULT CViewDeviceTree::OnShow(BOOL fShow)
{
    if (!fShow)
        return S_OK;
...
BOOL CViewDeviceTree::CreateDeviceTree()
{
...
#if DBG
void CViewDeviceTree::DumpDeviceTree()
{
    ASSERT(m_pCookieComputer);
    DumpDeviceSubtree(TEXT("|--"), m_pCookieComputer);
}

void CViewDeviceTree::DumpDeviceSubtree(LPCTSTR Insert, CCookie* pCookie)
{
    String str;
    if (Insert)
        str = Insert;
...
BOOL CViewTreeByType::CreateDeviceTree()
{
    ...
    if (m_pMachine->GetFirstClass(&pClass, ContextClass))
    {
        do
        {
            // We do not display a class if it does not have any devices in it.
            if (pClass->GetNumberOfDevices(TRUE))
            {
                ...
                if (pClass->GetFirstDevice(&pDevice, ContextDevice))
                {
                    do
                    {
                        ...
                        if (!strStartupDeviceId.IsEmpty() && !strStartupDeviceId.CompareNoCase(pDevice->GetDeviceID()))
                        {
...
BOOL CViewTreeByConnection::CreateDeviceTree()
{
    ...
    {
        // Locate the end of the CookieComputer Sibling list to add the phantom devices to.
...
// This function may throw CMemoryException
BOOL CViewTreeByConnection::CreateSubtree(CCookie* pCookieParent, CCookie* pCookieSibling, CDevice* pDeviceStart)
{
    CCookie* pCookie;
    ...
    while (pDeviceStart)
    {
        ...
        if (!strStartupDeviceId.IsEmpty() && !strStartupDeviceId.CompareNoCase(pDeviceStart->GetDeviceID()))
        {
            m_pSelectedCookie = pCookie;
        }
...
// This function may throw CMemoryException
HRESULT CViewResourceTree::OnShow(BOOL fShow)
{
    if (!fShow)
        return S_OK;
...
// This function may throw CMemoryException
void CViewResourceTree::CreateResourceTree()
{
...
//OUTPUT:
//  None
BOOL CViewResourceTree::InsertCookieToTree(CCookie* pCookie, CCookie* pCookieRoot, BOOL     ForcedInsert)
{
    CResource* pResRef;
    ...
    while (pCookieRoot)
    {
        ...
        if (COOKIE_TYPE_RESULTITEM_RESOURCE_IO == pCookie->GetType() || COOKIE_TYPE_RESULTITEM_RESOURCE_MEMORY == pCookie->GetType())
        {
...
BOOL CBusPropPageProvider::EnumPages(CDevice* pDevice, CPropSheetData* ppsd)
{
    ASSERT(!m_hDll);
    ...
    if (pDevice->m_pMachine->CmGetBusGuidString(pDevice->GetDevNode(), strBusGuid))
    {
        ...
        if (regDevMgr.Open(HKEY_LOCAL_MACHINE, REG_PATH_DEVICE_MANAGER) && regBusTypes.Open(regDevMgr, REG_STR_BUS_TYPES) && regBus.Open(regBusTypes, strBusGuid))
        {
            ...
            if (regBus.GetValue(REGSTR_VAL_ENUMPROPPAGES_32, strEnumPropPage))
            {
                ...
                if (LoadEnumPropPage32(strEnumPropPage, &m_hDll, (FARPROC*)&PropPageProvider))
                {
                    ...
                    if (PropPageProvider(&PropPageRequest,(LPFNADDPROPSHEETPAGE)AddPropPageCallback, (LPARAM)ppsd))
                        return TRUE;
                }
            }
        }
