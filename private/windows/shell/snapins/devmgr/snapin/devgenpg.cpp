...
HPROPSHEETPAGE CDeviceGeneralPage::Create(CDevice* pDevice)
{
    ASSERT(pDevice);
...
BOOL CDeviceGeneralPage::OnInitDialog(LPPROPSHEETPAGE ppsp)
{
...
BOOL CDeviceGeneralPage::OnApply()
{
    try
    {
        ...
        if ((-1 != m_SelectedDeviceUsage) && (m_SelectedDeviceUsage != m_CurrentDeviceUsage)) {
...
void CDeviceGeneralPage::UpdateHwProfileStates()
{
    ...
    m_pDevice->m_pMachine->DiSetClassInstallParams(*m_pDevice, (PSP_CLASSINSTALL_HEADER)&pcp, sizeof(pcp));

    m_pDevice->m_pMachine->DiCallClassInstaller(DIF_PROPERTYCHANGE, *m_pDevice);
    Canceled = (ERROR_CANCELLED == GetLastError());

    if (Canceled && ((m_pDevice->m_pMachine->DiGetFlags(*m_pDevice)) & DI_NEEDREBOOT) && m_pDevice->IsPCMCIA())
    {
    ...
    // class installer has not objection of our enabling/disabling, do real enabling/disabling.
    if (!Canceled)
    {
        CHwProfile* phwpf;
        ...
        if (m_pHwProfileList->GetCurrentHwProfile(&phwpf))
        {
            if (DEVICE_ENABLE == m_SelectedDeviceUsage)
            {
                // we are enabling the device, do a specific enabling then a globally enabling.
...
UINT CDeviceGeneralPage::DestroyCallback()
{
    // the property sheet is going away, consolidate the changes on the device.
...
void CDeviceGeneralPage::UpdateControls(LPARAM lParam)
{
    if (lParam)
    ...
    try
    {
        ...
        if (hIconNew)
        {
            ...
            hIconOld = (HICON)SendDlgItemMessage(m_hDlg, IDC_DEVGEN_ICON, STM_SETICON, (WPARAM)hIconNew, 0);
            if (hIconOld)
                DestroyIcon(hIconOld);
        }

        // get the device location information
        if (::GetLocationInformation(m_pDevice->GetDevNode(), TempString, ARRAYLEN(TempString), m_pDevice->m_pMachine->GetHMachine()) != CR_SUCCESS)
        {
            // if the devnode does not have location information, use the word "unknown"
        ...
        if (m_pDevice->GetStatus(&Status, &Problem) || m_pDevice->IsPhantom())
        {
            ...
            // if the device is not started and no problem is assigned to it fake the problem number to be failed start.
            if (!(Status & DN_STARTED) && !Problem && m_pDevice->IsRAW())
            {
                Problem = CM_PROB_FAILED_START;
            }

            // if a device has a private problem then give it special text telling the user to contact the manufacturer of this driver.
            if (Status & DN_PRIVATE_PROBLEM)
            {
                str.LoadString(g_hInstance, IDS_SPECIALPROB_PRIVATEPROB);
            }

            // if a device is not started and still does not have a problem then give it special problem text saying that this device does not have a driver.
            ...
            {
                UINT len = GetDeviceProblemText((HMACHINE)(*m_pDevice->m_pMachine), m_pDevice->GetDevNode(), Problem, TempString, ARRAYLEN(TempString));
                if (len)
                {
                    ...
                    else
                    {
                        ...
                        GetDeviceProblemText((HMACHINE)(*m_pDevice->m_pMachine), m_pDevice->GetDevNode(), Problem, TextPtr, len + 1);
                        str = (LPTSTR)TextPtr;
                    }
                }
                else
        ...
        if (m_pDevice->NoChangeUsage() || !m_pDevice->IsDisableable() || (CM_PROB_HARDWARE_DISABLED == Problem))
        {
...
BOOL CDeviceGeneralPage::OnQuerySiblings(WPARAM wParam, LPARAM lParam)
{
...
BOOL CDeviceGeneralPage::OnCommand(WPARAM wParam, LPARAM lParam)
{
    if (BN_CLICKED == HIWORD(wParam) && IDC_DEVGEN_TROUBLESHOOTING == LOWORD(wParam)) {
        BOOL fChanged = FALSE;
...
BOOL CDeviceGeneralPage::OnHelp(LPHELPINFO pHelpInfo)
{
    WinHelp((HWND)pHelpInfo->hItemHandle, DEVMGR_HELP_FILE_NAME, HELP_WM_HELP, (ULONG_PTR)g_a103HelpIDs);
    return FALSE;
}


BOOL CDeviceGeneralPage::OnContextMenu(HWND hWnd, WORD xPos, WORD yPos)
{
    WinHelp(hWnd, DEVMGR_HELP_FILE_NAME, HELP_CONTEXTMENU, (ULONG_PTR)g_a103HelpIDs);
    return FALSE;
}