...
DiskQuotaUser::DiskQuotaUser(FSObject *pFSObject) : m_cRef(0),
...
DiskQuotaUser::~DiskQuotaUser(VOID)
{
    DBGTRACE((DM_USER, DL_HIGH, TEXT("DiskQuotaUser::~DiskQuotaUser")));
    DBGPRINT((DM_USER, DL_HIGH, TEXT("\tthis = 0x%08X"), this));
...
STDMETHODIMP DiskQuotaUser::QueryInterface(REFIID riid,LPVOID *ppvOut)
{
    ...
    try
    {
        *ppvOut = NULL;
        ...
        if (IID_IUnknown == riid || IID_IDiskQuotaUser == riid)
        {
...
/*  Function: DiskQuotaUser::AddRef
    ...
    05/22/96    Initial creation.                                    BrianAu
*/
STDMETHODIMP_(ULONG) DiskQuotaUser::AddRef(VOID)
{
    DBGTRACE((DM_USER, DL_LOW, TEXT("DiskQuotaUser::AddRef")));
    DBGPRINT((DM_USER, DL_LOW, TEXT("\t0x%08X  %d -> %d"), this, m_cRef, m_cRef + 1));
...
/*  Function: DiskQuotaUser::Release
    ...
    05/22/96    Initial creation.                                    BrianAu
*/
STDMETHODIMP_(ULONG) DiskQuotaUser::Release(VOID)
{
...
/*  Function: DiskQuotaUser::Initialize
    ...
    09/05/96    Added exception handling.                            BrianAu
*/
HRESULT DiskQuotaUser::Initialize(PFILE_QUOTA_INFORMATION pfqi)
{
...
/*  Function: DiskQuotaUser::Destroy
    ...
    09/05/96    Added domain name string.                            BrianAu
*/
VOID DiskQuotaUser::Destroy(VOID)
{
...
*/
VOID DiskQuotaUser::DestroyContainerNameCache(VOID)
{
...
STDMETHODIMP DiskQuotaUser::GetID(ULONG *pulID)
{
    *pulID = m_ulUniqueId;
    return NOERROR;
}
...
*/
STDMETHODIMP DiskQuotaUser::GetAccountStatus(LPDWORD pdwAccountStatus)
{
    ...
    )
{
    ...
    else
    {
        if (NULL != pszContainerBuffer)
        {
            ...
            {
                GetCachedContainerName(m_iContainerName, pszContainerBuffer, cchContainerBuffer);
            }
            else
        ...
        {
            lstrcpyn(pszLogonNameBuffer, (NULL != m_pszLogonName) ? m_pszLogonName : TEXT(""), cchLogonNameBuffer);
        }

        if (NULL != pszDisplayNameBuffer)
        {
            lstrcpyn(pszDisplayNameBuffer, (NULL != m_pszDisplayName) ? m_pszDisplayName : TEXT(""), cchDisplayNameBuffer);
        }
        DiskQuotaUser::ReleaseLock();
    }
