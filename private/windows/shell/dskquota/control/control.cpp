    ...
    else
    {
        ...
        try
        {
            ...
            if (bAskDomainController)
            {
                ...
                SID_NAME_USE eUse;
                DWORD cbSid = sizeof(Sid);
                if (m_NTDS.LookupAccountByName(NULL, pszLogonName, &strContainerName, &strDisplayName, &Sid[0], &cbSid, &eUse))
                {
                    // Add it to the cache for later use.
                    if (NULL != pSidCache)
                    {
                        pSidCache->Add(&Sid[0], strContainerName, pszLogonName, strDisplayName);
                    }
    ...
    )
{
    ...
    HRESULT hr = ntds.LookupAccountByName(NULL, reinterpret_cast<LPCWSTR>(LogonName), NULL, NULL, sid, &cbSid, &eSidUse);
    if (SUCCEEDED(hr))
    {
        LPTSTR pszSid = NULL;
