...
STDMETHODIMP SidNameResolver::QueryInterface(REFIID riid, LPVOID *ppvOut)
{
...
STDMETHODIMP_(ULONG) SidNameResolver::AddRef(VOID)
{
    DBGTRACE((DM_RESOLVER, DL_LOW, TEXT("SidNameResolver::AddRef")));
    DBGPRINT((DM_RESOLVER, DL_LOW, TEXT("\t0x%08X %d -> %d"), this, m_cRef, m_cRef + 1));
...
STDMETHODIMP_(ULONG) SidNameResolver::Release(VOID)
{
...
STDMETHODIMP SidNameResolver::Initialize(VOID)
{
    ...
    DBGPRINT((DM_RESOLVER, DL_MID, TEXT("Resolver thread. hThread = 0x%08X, idThread = %d"), m_hResolverThread, m_dwResolverThreadId));
...
STDMETHODIMP SidNameResolver::Shutdown(BOOL bWait)
{
...
HRESULT SidNameResolver::GetUserSid(PDISKQUOTA_USER pUser, PSID *ppSid)
{
    HRESULT hResult   = NO_ERROR;
    DWORD cbSid = 0;
...
HRESULT SidNameResolver::AddUserToResolverQueue(PDISKQUOTA_USER pUser)
{
...
HRESULT SidNameResolver::RemoveUserFromResolverQueue(PDISKQUOTA_USER *ppUser)
{
    ...
    *ppUser = NULL;
    ...
    if (!m_UserQueue.IsEmpty() && m_UserQueue.Remove(*ppUser))
    {
        hResult = NO_ERROR;
    }
    else
...
STDMETHODIMP SidNameResolver::PromoteUserToQueueHead(PDISKQUOTA_USER pUser)
{
...
*/
HRESULT SidNameResolver::ResolveSidToName(PDISKQUOTA_USER pUser)
{
    ...
    if (SUCCEEDED(hResult))
    {
        ...
        hResult = m_rQuotaController.m_NTDS.LookupAccountBySid(NULL, (PSID)(ptrUserSid.get()), &strContainer, &strLogonName, &strDisplayName, &SidNameUse);
        if (SUCCEEDED(hResult))
        {
            switch (SidNameUse)
            {
            ...
            default:
                ...
                if (NULL != g_pSidCache) // Cache creation may have failed.
                {
                    ...
                    // Indicate failure only with a debug msg.
                    // If cache addition fails, we'll still work OK, just slower.
                    // This can throw OutOfMemory.
                    HRESULT hr = g_pSidCache->Add((PSID) (ptrUserSid.get()), strContainer, strLogonName, strDisplayName);
                    if (FAILED(hr))
                    {
                ...
                hResult = static_cast<DiskQuotaUser *>(pUser)->SetName(strContainer, strLogonName, strDisplayName);
                if (SUCCEEDED(hResult))
                {
                ...
                {
                    DBGERROR((TEXT("SIDNAME - SetName failed in ResolveSidToName. hResult = 0x%08X"), hResult));
                }
    ...
    {
        DBGERROR((TEXT("SIDNAME - GetUserSid failed in ResolveSidToName, hResult = 0x%08X"), hResult));
    }
...
HRESULT SidNameResolver::FindCachedUserName(PDISKQUOTA_USER pUser)
{
        ...
        if (SUCCEEDED(hResult))
        {
            ...
            try
            {
                ...
                if (NULL != g_pSidCache)
                {
                    ...
                    hResult = g_pSidCache->Lookup(pUserSid, &pszContainer, &pszLogonName, &pszDisplayName);
                    if (SUCCEEDED(hResult))
                    {
                        // Name was cached.  Set it in the user object and return NO_ERROR.
                        hResult = static_cast<DiskQuotaUser *>(pUser)->SetName(pszContainer, pszLogonName, pszDisplayName);
                        if (SUCCEEDED(hResult))
                        {
                            static_cast<DiskQuotaUser *>(pUser)->SetAccountStatus(DISKQUOTA_USER_ACCOUNT_RESOLVED);
                        }
...
STDMETHODIMP SidNameResolver::FindUserName(PDISKQUOTA_USER pUser)
{
    ...
    {
        DBGPRINT((DM_RESOLVER, DL_MID, TEXT("RESOLVER - User 0x%08X not cached.  Resolving..."), pUser));
...
STDMETHODIMP SidNameResolver::FindUserNameAsync(PDISKQUOTA_USER pUser)
{
    ...
    if (ERROR_FILE_NOT_FOUND == HRESULT_CODE(hResult))
    {
        if (0 != m_dwResolverThreadId)
        {
            DBGPRINT((DM_RESOLVER, DL_MID, TEXT("RESOLVER - User 0x%08X not cached.  Submitting to Resolver."), pUser));
        ...
        {
            DBGERROR((TEXT("RESOLVER - Thread not active.  Can't resolve user 0x%08X async."), pUser));
            hResult = E_FAIL;
        }
    }
    else
...
*/
HRESULT SidNameResolver::ClearInputQueue(void)
{
...
/*  Function: SidNameResolver::ThreadOnQueueNotEmpty
    ...
    06/18/96    Initial creation.                                    BrianAu
*/
HRESULT SidNameResolver::ThreadOnQueueNotEmpty(void)
{
...
DWORD SidNameResolver::ThreadProc(DWORD dwParam)
{
        ...
        while(!bExitThread)
        {
            ...
            try
            {
                ...
                while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE) && !bExitThread)
                {
                    ...
                    {
                        DBGPRINT((DM_RESOLVER, DL_MID, TEXT("RESOLVER: Thread %d dispatching msg %d"), GetCurrentThreadId(), msg.message));
                        TranslateMessage(&msg);
                        DispatchMessage(&msg);
                    }
                    else
                    {
                        // Rcvd WM_QUIT.  Clear the resolver's input queue and exit the msg loop.
                        DBGPRINT((DM_RESOLVER, DL_MID, TEXT("RESOLVER: Thread %d received WM_QUIT"), GetCurrentThreadId()));
                ...
                if (!bExitThread)
                {
                    ...
                    {
                        // Tell the thread creation function that it can now return.
                        // The thread is ready to accept messages.
                    ...
                    DBGPRINT((DM_RESOLVER, DL_MID, TEXT("RESOLVER - Thread %d waiting for messages..."), GetCurrentThreadId()));
                    dwWaitResult = MsgWaitForMultipleObjects(1, &(pThis->m_hsemQueueNotEmpty), FALSE, INFINITE, QS_ALLINPUT);
                    switch(dwWaitResult)
                    {
                        case WAIT_OBJECT_0:
                        ...
                        case WAIT_OBJECT_0 + 1:
                            ...
                            DBGPRINT((DM_RESOLVER, DL_MID, TEXT("RESOLVER -  Thread %d rcvd message(s)."),  GetCurrentThreadId()));
                            break;
                        case WAIT_FAILED:
                        case WAIT_TIMEOUT:
                        default:
                            // Something bad happened.
                            DBGPRINT((DM_RESOLVER, DL_MID, TEXT("RESOLVER - Thread %d wait failed."), GetCurrentThreadId()));
    ...
    {
        DBGERROR((TEXT("RESOLVER - OleInitialize failed for thread %d."),  GetCurrentThreadId()));
    }
...
HRESULT SidNameResolver::CreateResolverThread(PHANDLE phThread, LPDWORD pdwThreadId)
{
