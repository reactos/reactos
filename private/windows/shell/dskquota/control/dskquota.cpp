...
STDAPI DllCanUnloadNow(VOID)
{
    ...
    DBGPRINT((DM_COM, DL_HIGH, TEXT("\tRefCnt = %d  LockCnt = %d"), g_cRefThisDll, g_cLockThisDll));

#ifdef DEBUG_PROCESS_DETACH
    // This code will force the DLL to be unloaded so that we can
    // test OnProcessDetach().  Otherwise, OLE will wait 10 minutes after DllCanUnloadNow returns S_OK to call FreeLibrary.
    // Note however that the process will AV when OLE finally gets around to freeing the library.  Therefore, we don't leave
    // this code active in free or check builds; only private debug builds.  Must explicitly define DEBUG_PROCESS_DETACH.
...
/*
    Description: Handles all tasks associated with a process attaching to the DLL.
...
/*
    Description: Handles all tasks associated with a process detaching from the DLL.
    Returns:
        NOERROR    - Success.
    Revision History:
    Date        Description                                          Programmer
...
*/
{
    ...
    ICAP_STOP_ALL;// Stop IceCAP profiling.
    return hr;
}
...
/*
    ...
    Arguments:
        ...
        fdwReason - Reason DllMain is being called.  Can be at Process attach/detach or Thread attach/detach.
