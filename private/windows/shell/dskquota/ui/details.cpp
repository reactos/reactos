...
// This structure is used to pass the DetailsView object's "this" pointer in WM_CREATE.
...
*/
DetailsView::DetailsView(VOID) : m_cRef(0),
...
*/
BOOL DetailsView::Initialize(const CVolumeID& idVolume)
{
    BOOL bResult   = FALSE;
    HANDLE hThread = NULL;
    ...
    try
    {
        ...
        {
            CloseHandle(hThread);            
            bResult = TRUE;// Everything succeeded.
        }
    }
...
*/
STDMETHODIMP_(ULONG) DetailsView::AddRef(VOID)
{
    ...
    )
{
    ...
    try
    {
        if (SUCCEEDED(OleInitialize(NULL)))
        {
            ...
            hResult = CoCreateInstance(CLSID_DiskQuotaControl, NULL, CLSCTX_INPROC_SERVER, IID_IDiskQuotaControl, (LPVOID *)&(pThis->m_pQuotaControl));
            if (SUCCEEDED(hResult))
            {
                hResult = pThis->m_pQuotaControl->Initialize(pThis->m_idVolume.ForParsing(), TRUE); // Read-write.
                if (SUCCEEDED(hResult))
                {
...
*/
HRESULT DetailsView::CreateListViewToolTip(VOID)
{
    HRESULT hResult = E_FAIL;

    m_hwndListViewToolTip = CreateWindowEx(0, TOOLTIPS_CLASS, (LPTSTR)NULL, TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, m_hwndListView, (HMENU)NULL, g_hInstDll, NULL);
    if (NULL != m_hwndListViewToolTip)
    {
        ...
        SendMessage(m_hwndListViewToolTip, TTM_SETDELAYTIME, TTDT_INITIAL, (LPARAM)500);
        ...
        if (SendMessage(m_hwndListViewToolTip, TTM_ADDTOOL, 0, (LPARAM)&ti))
        {
            hResult = NO_ERROR;
        }
    }
...
/*  Function: DetailsView::CreateStatusBar
    ...
    08/20/96    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::CreateStatusBar(VOID)
{
    HRESULT hResult = NO_ERROR;

    DBGASSERT((NULL != m_hwndMain));
    m_hwndStatusBar = CreateWindow(STATUSCLASSNAME, TEXT(""), WS_VISIBLE | WS_CHILD | WS_BORDER | SBS_SIZEGRIP, 0, 0, 0, 0, m_hwndMain, (HMENU)NULL, g_hInstDll, NULL);
    if (NULL != m_hwndStatusBar)
    {
        ...
        CheckMenuItem(GetMenu(m_hwndMain), IDM_VIEW_STATUSBAR, MF_BYCOMMAND | (m_lvsi.fStatusBar ? MF_CHECKED : MF_UNCHECKED));
    }
    else
    {
        hResult = E_FAIL;
...
*/
HRESULT DetailsView::CreateToolBar(VOID)
{
    HRESULT hResult = NO_ERROR;
    ...
    if (NULL != m_hwndToolBar)
    {
        ...
        m_hwndToolbarCombo = CreateWindowEx(0,
                                            TEXT("COMBOBOX"),
                                            TEXT(""),
                                            WS_CHILD | WS_BORDER | CBS_HASSTRINGS | CBS_DROPDOWN | CBS_AUTOHSCROLL,
                                            0, 0,
                                            CX_TOOLBAR_COMBO,
                                            CY_TOOLBAR_COMBO,
                                            m_hwndToolBar,
        ...
        if (NULL != m_hwndToolbarCombo)
        {
            ...
            if (SUCCEEDED(keyPref.Open(KEY_READ)))
            {
                ...
                if (SUCCEEDED(keyPref.GetValue(REGSTR_VAL_FINDMRU, &rgstrMRU)))
                {
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        SendMessage(m_hwndToolbarCombo, CB_ADDSTRING, 0, (LPARAM)(rgstrMRU[i].Cstr()));
                    }
                }
            }
        }
        ...
        CheckMenuItem(GetMenu(m_hwndMain), IDM_VIEW_TOOLBAR, MF_BYCOMMAND | (m_lvsi.fToolBar ? MF_CHECKED : MF_UNCHECKED));
...
/*  Function: DetailsView::LoadObjects
    ...
    08/20/96    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::LoadObjects(VOID)
{
    HRESULT hResult = NO_ERROR;
    ...
    if (SUCCEEDED(hResult))
    {
        ...
        // The faster it is, the less time the user must wait for the listview to be populated with user objects.
        try
        {
...
VOID DetailsView::DataObject::Renderer::Stream::Write(LPCWSTR pszTextW)
{
    Write((LPBYTE)pszTextW, lstrlenW(pszTextW) * sizeof(WCHAR));
}


VOID DetailsView::DataObject::Renderer::Stream::Write(LPCSTR pszTextA)
{
    Write((LPBYTE)pszTextA, lstrlenA(pszTextA) * sizeof(CHAR));
}


VOID DetailsView::DataObject::Renderer::Stream::Write(BYTE bData)
{
    Write((LPBYTE)&bData, sizeof(bData));
}


VOID DetailsView::DataObject::Renderer::Stream::Write(WCHAR chDataW)
{
    Write((LPBYTE)&chDataW, sizeof(chDataW));
}


VOID DetailsView::DataObject::Renderer::Stream::Write(CHAR chDataA)
{
    Write((LPBYTE)&chDataA, sizeof(chDataA));
}


VOID DetailsView::DataObject::Renderer::Stream::Write(DWORD dwData)
{
    Write((LPBYTE)&dwData, sizeof(dwData));
}


VOID DetailsView::DataObject::Renderer::Stream::Write(double dblData)
{
    Write((LPBYTE)&dblData, sizeof(dblData));
}


// The following section of code contains the different implementations of the virtual rendering functions that make each type of rendering object
// unique.  Since they're pretty self-explanatory, I haven't commented each function.  It should be obvious as to what they do.
// I have separated each rendering-type section with a banner comment for readability.  [brianau]


// CF_UNICODETEXT
VOID DetailsView::DataObject::Renderer_UNICODETEXT::AddTitle(LPCTSTR pszTitle)
{
...
VOID DetailsView::DataObject::Renderer_UNICODETEXT::AddRowColData(INT iRow, INT idCol)
{
    WCHAR szText[MAX_PATH];
    LV_REPORT_ITEM item;
...
// CF_TEXT
VOID DetailsView::DataObject::Renderer_TEXT::AddTitle(LPCWSTR pszTitleW)
{
...
VOID DetailsView::DataObject::Renderer_TEXT::AddHeader(LPCWSTR pszHeaderW)
{
...
VOID DetailsView::DataObject::Renderer_TEXT::AddRowColData(INT iRow, INT idCol)
{
...
LPSTR DetailsView::DataObject::Renderer_RTF::DoubleBackslashes(LPSTR pszSrc)
{
...
VOID DetailsView::DataObject::Renderer_RTF::Begin(INT cRows, INT cCols)
{
    m_cCols = cCols;
...
VOID DetailsView::DataObject::Renderer_RTF::AddTitle(LPCWSTR pszTitleW)
{
...
VOID DetailsView::DataObject::Renderer_RTF::BeginHeaders(VOID)
{
...
VOID DetailsView::DataObject::Renderer_RTF::AddCellDefs(VOID)
{
    char szText[80];
    INT cxTwips = 0;
...
VOID DetailsView::DataObject::Renderer_RTF::BeginHeaderOrRow(VOID)
{
    m_Stm.Write("\\trowd \\pard \\intbl ");
}


VOID DetailsView::DataObject::Renderer_RTF::AddHeader(LPCWSTR pszHeaderW)
{
    array_autoptr<CHAR> ptrHeaderA(DataObject::WideToAnsi(pszHeaderW));
...
VOID DetailsView::DataObject::Renderer_RTF::AddRowColData(INT iRow, INT idCol)
{
    WCHAR szTextW[MAX_PATH];
    LV_REPORT_ITEM item;
    item.fType      = LVRI_TEXT;  // Want text data.
...
VOID DetailsView::DataObject::Renderer_Export::Render(IStream *pStm)
{
    HRESULT hResult = NO_ERROR;
    INT cRows = m_DV.GetReportRowCount();
    INT iRow = -1;
...
VOID DetailsView::DataObject::Renderer_Export::Begin(INT cRows, INT cCols)
{
    // The stream header contains a GUID as a unique identifier followed by a version number.
...
VOID DetailsView::DataObject::Renderer_Export::AddBinaryRecord(INT iRow)
{
...
VOID DetailsView::DataObject::Renderer_FileGroupDescriptor::Begin(INT cRows, INT cCols)
{
    ...
    // case, it contains backslashes and a colon; both invalid buried in a filename.
    TCHAR szLabel[MAX_VOL_LABEL] = { TEXT('\0') };
    DWORD dwSerialNumber = 0;
    GetVolumeInformation(m_DV.GetVolumeID().ForParsing(), szLabel, ARRAYSIZE(szLabel), &dwSerialNumber, NULL, NULL, NULL, 0);

    CString strFileName;
    if (TEXT('\0') != szLabel[0])
    {
        // Volume has a label.
        strFileName.Format(g_hInstDll, IDS_EXPORT_STREAM_FILENAME_TEMPLATE, szLabel);
    }
    else
    {
        // No volume label.
        strFileName.Format(g_hInstDll, IDS_EXPORT_STREAM_FILENAME_TEMPLATE_VOLSN, HIWORD(dwSerialNumber), LOWORD(dwSerialNumber));
    }
...
*/
VOID LVSelection::Add(PDISKQUOTA_USER pUser, INT iItem)
{
    DBGASSERT((NULL != pUser));
...
/*  Function: LVSelection::Retrieve
    ...
    09/10/96    Initial creation.                                    BrianAu
*/
BOOL LVSelection::Retrieve(INT i, PDISKQUOTA_USER *ppUser, INT *piItem)
{
    ListEntry entry;
...
*/
ColumnMap::ColumnMap(UINT cMapSize) : m_pMap(NULL), m_cMapSize(cMapSize)
{
...
/*  Function: ColumnMap::SubItemToId
    ...
    10/09/96    Initial creation.                                    BrianAu
*/
INT ColumnMap::SubItemToId(INT iSubItem) const
{
...
/*  Function: ColumnMap::IdToSubItem
    ...
    10/09/96    Initial creation.                                    BrianAu
*/
INT ColumnMap::IdToSubItem(INT iColId) const
{
...
*/
VOID ColumnMap::RemoveId(INT iSubItem)
{
    DBGASSERT((iSubItem >= 0 && iSubItem < (INT)m_cMapSize));
...
/*  Function: ColumnMap::InsertId
    ...
    11/30/96    Fixed off-by-one error.                              BrianAu
*/
VOID ColumnMap::InsertId(INT iSubItem, INT iColId)
{
    DBGASSERT((iSubItem >= 0 && iSubItem < (INT)m_cMapSize));
...
/*  Function: DetailsView::Finder::Finder
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
DetailsView::Finder::Finder(DetailsView& DetailsView, INT cMaxMru) : m_DetailsView(DetailsView), m_hwndToolbarCombo(NULL), m_cMaxComboEntries(cMaxMru), m_pfnOldToolbarComboWndProc(NULL)
{
    // Nothing more to do.
}
...
/*  Function: DetailsView::Finder::ConnectToolbarCombo
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
VOID DetailsView::Finder::ConnectToolbarCombo(HWND hwndToolbarCombo)
{
    ...
    for (HWND hwndChild =  GetTopWindow(m_hwndToolbarCombo); hwndChild      != NULL; hwndChild      =  GetNextWindow(hwndChild, GW_HWNDNEXT))
    {
    ...
    if (NULL != hwndComboEdit)
    {
        ...
        m_pfnOldToolbarComboWndProc = (WNDPROC)GetWindowLongPtr(hwndComboEdit, GWLP_WNDPROC);
        SetWindowLongPtr(hwndComboEdit, GWLP_WNDPROC, (INT_PTR)DetailsView::Finder::ToolbarComboSubClassWndProc);
    }
}
...
/*  Function: DetailsView::Finder::InvokeFindDialog
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
VOID DetailsView::Finder::InvokeFindDialog(HWND hwndParent)
{
    DialogBoxParam(g_hInstDll, MAKEINTRESOURCE(IDD_FINDUSER), hwndParent, (DLGPROC)DetailsView::Finder::DlgProc, (LPARAM)this);
}
...
/*  Function: DetailsView::Finder::DlgProc
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
INT_PTR CALLBACK DetailsView::Finder::DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            SetWindowPos(hwndCombo, NULL, 0, 0, rcCombo.right - rcCombo.left, CY_TOOLBAR_COMBO, SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOACTIVATE);
...
/*  Function: DetailsView::Finder::FillDialogCombo
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
VOID DetailsView::Finder::FillDialogCombo(HWND hwndComboSrc, HWND hwndComboDest)
{
...
/*  Function: DetailsView::Finder::ToolbarComboSubclassWndProc
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
LRESULT CALLBACK DetailsView::Finder::ToolbarComboSubClassWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    return CallWindowProc(pThis->m_pfnOldToolbarComboWndProc, hwnd, uMsg, wParam, lParam);
}
...
/*  Function: DetailsView::Finder::AddNameToCombo
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
VOID DetailsView::Finder::AddNameToCombo(HWND hwndCombo, LPCTSTR pszName, INT cMaxEntries)
{
    if (NULL != pszName && TEXT('\0') != *pszName)
    {
        // See if the item already exists in the list.
        INT iItemToDelete = (INT)SendMessage(hwndCombo, CB_FINDSTRING, (WPARAM)-1, (LPARAM)pszName);
...
/*  Function: DetailsView::Finder::UserNameEntered
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
BOOL DetailsView::Finder::UserNameEntered(HWND hwndCombo)
{
    ...
    if (0 < SendMessage(hwndCombo, WM_GETTEXT, (WPARAM)ARRAYSIZE(szName), (LPARAM)szName))
    {
        ...
        else
        {
            ...
            DiskQuotaMsgBox(hwndCombo, (LPCTSTR)strMsg, IDS_TITLE_DISK_QUOTA, MB_ICONEXCLAMATION);
        }
    }
...
DetailsView::Importer::Importer(DetailsView& DV) : m_DV(DV),
        m_bUserCancelled(FALSE),
        m_bPromptOnReplace(TRUE),
        m_dlgProgress(IDD_PROGRESS, IDC_PROGRESS_BAR, IDC_TXT_PROGRESS_DESCRIPTION, IDC_TXT_PROGRESS_FILENAME),
...
DetailsView::Importer::~Importer(VOID)
{
    Destroy();
}


VOID DetailsView::Importer::Destroy(VOID)
{
...
/*  Function: DetailsView::Importer::Import [IDataObject *]
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::Importer::Import(IDataObject *pIDataObject)
{
    HRESULT hResult = NO_ERROR;
    FORMATETC fmt;
    CStgMedium medium;
...
/*  Function: DetailsView::Importer::Import [FORMATETC&, STGMEDIUM&]
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::Importer::Import(const FORMATETC& fmt, const STGMEDIUM& medium)
{
    HRESULT hResult   = E_FAIL;
    ...
    {
        // Medium type is an HGLOBAL but our import functions need a stream.
...
/*  Function: DetailsView::Importer::Import [LPCTSTR]
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::Importer::Import(LPCTSTR pszFilePath)
{
    HRESULT hResult     = NO_ERROR;
    ...
    else
    {
        ...
        // Contents will be validated during the import process.
        hResult = StgOpenStorage(pszFilePath, NULL, STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, NULL, 0, &pStg);
        if (SUCCEEDED(hResult))
        {
            // Open the import stream.
            IStream *pIStream;
            hResult = pStg->OpenStream(DetailsView::DataObject::SZ_EXPORT_STREAM_NAME, NULL, STGM_DIRECT | STGM_READ | STGM_SHARE_EXCLUSIVE, 0, &pIStream);
            if (SUCCEEDED(hResult))
            {
    ...
    if (FAILED(hResult))
    {
        ...
        switch(hResult)
        {
            case STG_E_FILENOTFOUND:
                ...
                iMsg = bStreamFailure ? IDS_IMPORT_STREAM_INVALID_STREAM : IDS_IMPORT_STREAM_FILENOTFOUND;
                break;
            case STG_E_ACCESSDENIED:
        ...
        DiskQuotaMsgBox(GetTopmostWindow(), iMsg, IDS_TITLE_DISK_QUOTA, MB_ICONERROR | MB_OK);
    }
...
/*  Function: DetailsView::Importer::Import [HDROP]
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::Importer::Import(HDROP hDrop)
{
    HRESULT hResult = NO_ERROR;
...
/*  Function: DetailsView::Importer::Import [Source&]
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::Importer::Import(Source& source)
{
    ULONG cbRead;
    HRESULT hResult = E_FAIL;
    ...
    try
    {
        ...
        if (guidStreamSignature == GUID_NtDiskQuotaStream)
        {
            ...
            else
            {
                ...
                DiskQuotaMsgBox(GetTopmostWindow(), IDS_IMPORT_STREAM_INVALID_STREAM, IDS_TITLE_DISK_QUOTA, MB_ICONERROR | MB_OK);
            }
        }
        else
        {
    ...
    catch(CFileException& fe)
    {
        ...
        DiskQuotaMsgBox(GetTopmostWindow(), IDS_IMPORT_STREAM_READ_ERROR, IDS_TITLE_DISK_QUOTA, MB_ICONERROR | MB_OK);
...
/*  Function: DetailsView::Importer::Import [LPBYTE, LONGLONG, LONGLONG]
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
HRESULT DetailsView::Importer::Import(LPBYTE pbSid, LONGLONG llQuotaThreshold, LONGLONG llQuotaLimit)
{
    INT iResult     = 0;
    HRESULT hResult = NO_ERROR;
    PDISKQUOTA_USER pIUser = NULL;
    ...
    hResult = m_DV.m_pQuotaControl->AddUserSid(pbSid, DISKQUOTA_USERNAME_RESOLVE_SYNC, &pIUser);
    if (SUCCEEDED(hResult))
    {
        ...
        if (!bAddNewUser)
        {
            ...
            if (m_bPromptOnReplace)
            {
                ...
                if (-1 != iItem)
                {
                    ...
                    DBGASSERT((NULL != pIUser));
                    pIUser->GetName(NULL, 0, szLogonName,   ARRAYSIZE(szLogonName), szDisplayName, ARRAYSIZE(szDisplayName));
                }

                CString strTitle(g_hInstDll, IDS_TITLE_DISK_QUOTA);
                CString strMsg(g_hInstDll, IDS_IMPORT_REPLACE_RECORD, szDisplayName, szLogonName);

                // Ask the user if they want to replace the record's
                // quota settings.
                YesNoToAllDialog ynToAllDlg(IDD_YNTOALL);
                INT_PTR iResponse = ynToAllDlg.CreateAndRun(g_hInstDll, GetTopmostWindow(), strTitle, strMsg);
...
/*  Function: DetailsView::Importer::GetTopmostWindow
    ...
    05/20/97    Initial creation.                                    BrianAu
*/
HWND DetailsView::Importer::GetTopmostWindow(VOID)
{
...
// Source --
DetailsView::Importer::Source::Source(IStream *pStm) : m_pTheSource(NULL)
{
...
DetailsView::Importer::Source::Source(LPBYTE pb, ULONG cbMax) : m_pTheSource(NULL)
{
...
DetailsView::Importer::Source::~Source(VOID)
{
...
HRESULT DetailsView::Importer::Source::Read(LPVOID pvOut, ULONG cb, ULONG *pcbRead)
{
...
// StreamSource ---
DetailsView::Importer::StreamSource::StreamSource(IStream *pStm) : m_pStm(pStm)
{
...
DetailsView::Importer::StreamSource::~StreamSource(VOID)
{
...
HRESULT DetailsView::Importer::StreamSource::Read(LPVOID pvOut, ULONG cb, ULONG *pcbRead)
{
    HRESULT hr = E_FAIL;
    if (NULL != m_pStm)
    {
...
// MemorySource ---
DetailsView::Importer::MemorySource::MemorySource(LPBYTE pb, ULONG cbMax) : m_pb(pb), m_cbMax(cbMax)
{

}


HRESULT DetailsView::Importer::MemorySource::Read(LPVOID pvOut, ULONG cb, ULONG *pcbRead)
{
    HRESULT hr = E_FAIL;
