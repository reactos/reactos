...
*/
BOOL SidToString(PSID pSid, LPTSTR pszSid, LPDWORD pcchBuffer)
{
    ...
    psia = GetSidIdentifierAuthority(pSid);// obtain SidIdentifierAuthority
    dwSubAuthorities = *GetSidSubAuthorityCount(pSid);// obtain sidsubauthority count
    ...
    {
        cchSid += wsprintf(pszSid + lstrlen(pszSid), TEXT("0x%02hx%02hx%02hx%02hx%02hx%02hx"), (USHORT)psia->Value[0], (USHORT)psia->Value[1], (USHORT)psia->Value[2], (USHORT)psia->Value[3], (USHORT)psia->Value[4], (USHORT)psia->Value[5]);
    }
    else
    {
        cchSid += wsprintf(pszSid + lstrlen(pszSid), TEXT("%lu"),
                    (ULONG)(psia->Value[5]      )   +
    ...
    {
        cchSid += wsprintf(pszSid + cchSid, TEXT("-%lu"), *GetSidSubAuthority(pSid, dwCounter) );
    }
    ...
    if (SUCCEEDED(hResult))
    {
        ...
        *pcbSidList = cbBuffer;
        ...
        for (UINT i = 0; i < cpSids; i++)
        {
            ...
            cbRecord = sizeof(pfgqi->NextEntryOffset) + sizeof(pfgqi->SidLength) + cbSid;
...
{
    MessageBox(NULL, TEXT("This feature has not been implemented."), TEXT("Under Construction"), MB_ICONWARNING | MB_OK);
}
...
INT DiskQuotaMsgBox(HWND hWndParent, LPCTSTR pszText, LPCTSTR pszTitle, UINT uType)
{
...
// on the desktop.
VOID CenterPopupWindow(HWND hwnd, HWND hwndParent)
{
    RECT rcScreen;
...
void GetDialogItemText(HWND hwnd, UINT idCtl, CString *pstrText)
{
...
BOOL UserIsAdministrator(PDISKQUOTA_USER pUser)
{
...
HRESULT CallRegInstall(HINSTANCE hInstance, LPSTR pszSection)
{
    HRESULT hr = E_FAIL;
