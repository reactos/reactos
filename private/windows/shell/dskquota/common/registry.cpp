...
RegKey::RegKey(void) : m_hkeyRoot(NULL), m_hkey(NULL), m_hChangeEvent(NULL), m_bWatchSubtree(false), m_dwChangeFilter(0)
{
    DBGTRACE((DM_REG, DL_MID, TEXT("RegKey::RegKey [default]")));
}


RegKey::RegKey(HKEY hkeyRoot, LPCTSTR pszSubKey) : m_hkeyRoot(hkeyRoot), m_hkey(NULL), m_strSubKey(pszSubKey), m_hChangeEvent(NULL), m_bWatchSubtree(false), m_dwChangeFilter(0)
{
    ...
    ) const
{
    ...
    DBGPRINT((DM_REG, DL_HIGH, TEXT("\thkeyRoot = 0x%08X, SubKey = \"%s\""), m_hkeyRoot, m_strSubKey.Cstr()));
    ...
    {
        dwResult = RegOpenKeyEx(m_hkeyRoot, (LPCTSTR)m_strSubKey, 0, samDesired, &m_hkey);
    }
...
// This is the basic form of GetValue.  All other forms of GetValue() call into this one.
HRESULT RegKey::GetValue(LPCTSTR pszValueName, DWORD dwTypeExpected, LPBYTE pbData, int cbData) const
{
    DWORD dwType;
    DWORD dwResult = RegQueryValueEx(m_hkey, pszValueName, 0, &dwType, pbData, (LPDWORD)&cbData);
...
HRESULT RegKey::GetValue(LPCTSTR pszValueName, DWORD *pdwDataOut) const
{
    return GetValue(pszValueName, REG_DWORD, (LPBYTE)pdwDataOut, sizeof(DWORD));
}

// Get a byte buffer value (REG_BINARY).
HRESULT RegKey::GetValue(LPCTSTR pszValueName, LPBYTE pbDataOut, int cbDataOut) const
{
...
HRESULT RegKey::GetValue(LPCTSTR pszValueName, CString *pstrDataOut) const
{
    HRESULT hr = E_FAIL;
    int cch = GetValueBufferSize(pszValueName) / sizeof(TCHAR);
    if (NULL != pstrDataOut && 0 < cch)
    {
        hr = GetValue(pszValueName, REG_SZ, (LPBYTE)pstrDataOut->GetBuffer(cch), pstrDataOut->SizeBytes());
...
HRESULT RegKey::GetValue(LPCTSTR pszValueName, CArray<CString> *prgstrOut) const
{
    HRESULT hr = E_FAIL;
...
int RegKey::GetValueBufferSize(LPCTSTR pszValueName) const
{
    ...
    int cbData = 0;
    DWORD dwDummy;
    DWORD dwResult = RegQueryValueEx(m_hkey, pszValueName, 0, &dwType, (LPBYTE)&dwDummy, (LPDWORD)&cbData);
...
// This is the basic form of SetValue.  All other forms of SetValue() call into this one.
...
HRESULT RegKey::SetValue(LPCTSTR pszValueName, const LPBYTE pbData, int cbData)
{
...
HRESULT RegKey::SetValue(LPCTSTR pszValueName, LPCTSTR pszData)
{
...
HRESULT RegKey::SetValue(LPCTSTR pszValueName, const CArray<CString>& rgstrSrc)
{
...
LPTSTR RegKey::CreateDoubleNulTermList(const CArray<CString>& rgstrSrc) const
{
...
DWORD RegKey::NotifyWaitThreadProc(LPVOID pvParam)
{
    ...
    while(NULL != pThis->m_hkey)
    {
        ...
        LONG lResult = RegNotifyChangeKeyValue(pThis->m_hkey, pThis->m_bWatchSubtree, pThis->m_dwChangeFilter, pThis->m_hChangeEvent, true);
...
HRESULT RegKey::WatchForChange(DWORD dwChangeFilter, bool bWatchSubtree)
{
    ...
    DWORD dwThreadId = 0;
    HANDLE hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)NotifyWaitThreadProc, this, 0, &dwThreadId);
...
#if DBG
//                        DEBUG ONLY
RegKeyChg::RegKeyChg(HKEY hkeyRoot, LPCTSTR pszSubKey) : RegKey(hkeyRoot, pszSubKey)
{
...
RegKeyChg::~RegKeyChg(void)
{
    DBGTRACE((DM_REG, DL_HIGH, TEXT("RegKeyChg::~RegKeyChg")));
}


void RegKeyChg::OnChange(HKEY hkey)
{
