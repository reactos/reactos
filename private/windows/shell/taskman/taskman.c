    ...
    CHAR *envp[])
{
    ...
    if (!RegisterHotKey(ghwndDialog, 1, MOD_CONTROL, VK_ESCAPE) || !RegisterTasklist(ghwndDialog)) {
        goto exit;
    }
...
OKToExec()
{
     ...
     */
    if (!RegCreateKeyEx(HKEY_CURRENT_USER, szProgramManager, 0, szProgramManager, 0, KEY_READ | KEY_WRITE, NULL, &hkeyProgramManager, NULL)) {
