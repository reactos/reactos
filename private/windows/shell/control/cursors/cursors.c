...
void InitRegistry(void) {
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, szRegPath, 0, KEY_READ, &hkeyCPl) != ERROR_SUCCESS) {
        /*
...
DWORD CSRGetProfileString(LPCTSTR lpszSection, LPCTSTR lpszKey,
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, szRegPath, 0, KEY_READ, &hkeyCPl) == ERROR_SUCCESS) {
        if (lpszKey != NULL) {
            ...
            if (RegQueryValueEx (hkeyCPl, (LPTSTR)lpszKey, NULL, &dwType, (LPBYTE)lpszReturnBuffer, &cchReturnBuffer) == ERROR_SUCCESS && dwType == REG_SZ ) {
                fRet = TRUE;
            }
        } else {
            ...
            if (RegQueryInfoKey(hkeyCPl, szClass, &cch, NULL, &cSubKeys, &cchMaxSK, &cchMaxClass, &cValues, &cchMaxVal, &i, &dwType, &ft) == ERROR_SUCCESS) {
                fRet = TRUE;

                p = lpszReturnBuffer;
                for ( i = 0; i < cValues; i++ ) {
                    cch = cchReturnBuffer;
                    if (RegEnumValue(hkeyCPl, i, p, &cch, NULL, &dwType, NULL, NULL) == ERROR_SUCCESS && dwType == REG_SZ) {
     ...
     */
    if (fRet == FALSE) {
        fRet = GetProfileString(lpszSection, lpszKey, lpszDefault, lpszReturnBuffer, cchReturnBuffer);
    }
...
BOOL CSRWriteProfileString(LPCTSTR lpszSection, LPCTSTR lpszKey, LPCTSTR lpszString) {
    HKEY hkeyCPl;
    BOOL fRet = FALSE;
    TCHAR szRegPath[MAX_PATH];
    DWORD dw;
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szRegPath, 0, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hkeyCPl, &dw) == ERROR_SUCCESS) {
        if (lpszString != NULL) {
            fRet = (ERROR_SUCCESS == RegSetValueEx(hkeyCPl, lpszKey, 0, REG_SZ, (LPBYTE)lpszString, (lstrlen(lpszString) + 1) * sizeof(TCHAR)));
        } else {
