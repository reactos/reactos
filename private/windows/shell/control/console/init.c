 ...
 *  Description:  Initialization code for Console control panel applet
...
LONG CPlApplet( HWND hwnd, WORD wMsg, LPARAM lParam1, LPARAM lParam2)
{
    ...
    case CPL_NEWINQUIRE:
        ...
        if (!LoadString(ghInstance, IDS_NAME, lpCPlInfo->szName, NELEM(lpCPlInfo->szName))) {
            lpCPlInfo->szName[0] = TEXT('\0');
        }

        if (!LoadString(ghInstance, IDS_INFO, lpCPlInfo->szInfo, NELEM(lpCPlInfo->szInfo))) {
