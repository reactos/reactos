...
void AspectPoint(RECT* rectPreview, POINT* pt);
LONG AspectScale(LONG n1, LONG n2, LONG m);
...
--*/
{
    ...
    /*
     * Get the window rectangle, making sure it's at least twice the size of the non-client area.
     */
...
--*/
{
    ...
    /*
     * Get the dimensions of the preview "window" and scale it to the preview "screen"
     */
...
VOID PreviewPaint(PAINTSTRUCT* pPS, HWND hWnd)
/*++
    Paints the font preview.  This is called inside the paint message handler for the preview window
--*/
{
    RECT rectWin;
    RECT rectPreview;
    ...
    hBitmap = CreateCompatibleBitmap(pPS->hdc, rectPreview.right, rectPreview.bottom);
