...
INT_PTR WINAPI SaveQueryDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    HFONT hFont;
    HWND  hChild;
    HWND  hCreator;
...
UINT ConsolePropSheetProc(HWND hDlg, UINT wMsg, LPARAM lParam)
{
    DWORD dwExStyle;
...
VOID EndDlgPage(HWND hDlg)
{
    ...
    {
        Result = DialogBoxParam(ghInstance, MAKEINTRESOURCE(DID_SAVE_QUERY_LINK), GetParent(hDlg), SaveQueryDlgProc, (LPARAM)hDlg);
        SetDlgMsgResult(hDlg, PSN_APPLY, Result);
    }
    else
    {
        Result = DialogBoxParam(ghInstance, MAKEINTRESOURCE(DID_SAVE_QUERY), GetParent(hDlg), SaveQueryDlgProc, (LPARAM)hDlg);
        SetDlgMsgResult(hDlg, PSN_APPLY, Result);
    }
}
    ...
    case WM_PAINT:
        ...
        if ((hbr = CreateSolidBrush(rgbBrush)) != NULL) {
            ...
            if (ColorArray[Index] == (BYTE)(ColorId - IDD_COLOR_1)) {
                ...
                UpdateItem(hDlg, IDD_COLOR_RED, GetRValue(AttrToRGB(ColorArray[Index])));
                UpdateItem(hDlg, IDD_COLOR_GREEN, GetGValue(AttrToRGB(ColorArray[Index])));
                UpdateItem(hDlg, IDD_COLOR_BLUE, GetBValue(AttrToRGB(ColorArray[Index])));
...
--*/
{
    ...
    switch (wMsg) {
    case WM_INITDIALOG:
        ColorArray[IDD_COLOR_SCREEN_TEXT - IDD_COLOR_SCREEN_TEXT] = LOBYTE(gpStateInfo->ScreenAttributes) & 0x0F;
        ColorArray[IDD_COLOR_SCREEN_BKGND - IDD_COLOR_SCREEN_TEXT] = LOBYTE(gpStateInfo->ScreenAttributes >> 4);
        ColorArray[IDD_COLOR_POPUP_TEXT - IDD_COLOR_SCREEN_TEXT] = LOBYTE(gpStateInfo->PopupAttributes) & 0x0F;
        ColorArray[IDD_COLOR_POPUP_BKGND - IDD_COLOR_SCREEN_TEXT] = LOBYTE(gpStateInfo->PopupAttributes >> 4);
        ...
        SendDlgItemMessage(hDlg, IDD_COLOR_REDSCROLL, UDM_SETPOS, 0, MAKELONG(GetRValue(AttrToRGB(ColorArray[Index])), 0));
        SendDlgItemMessage(hDlg, IDD_COLOR_GREENSCROLL, UDM_SETRANGE, 0, MAKELONG(255, 0));
        SendDlgItemMessage(hDlg, IDD_COLOR_GREENSCROLL, UDM_SETPOS, 0, MAKELONG(GetGValue(AttrToRGB(ColorArray[Index])), 0));
        SendDlgItemMessage(hDlg, IDD_COLOR_BLUESCROLL, UDM_SETRANGE, 0, MAKELONG(255, 0));
        SendDlgItemMessage(hDlg, IDD_COLOR_BLUESCROLL, UDM_SETPOS, 0, MAKELONG(GetBValue(AttrToRGB(ColorArray[Index])), 0));
        return TRUE;
    case WM_COMMAND:
        Item = LOWORD(wParam);
        switch (Item) {
        ...
        case IDD_COLOR_BLUE:
            ...
            case EN_KILLFOCUS:
                ...
                if (bOK) {
                    ...
                    UpdateStateInfo(hDlg, ColorArray[Index] + IDD_COLOR_1, RGB(Red, Green, Blue));
                }
...
int GetStateInfo(HWND hDlg, UINT Item, BOOL *bOK)
{
    int Value = 0;
...
BOOL UpdateStateInfo(HWND hDlg, UINT Item, int Value)
{
    ...
    case IDD_AUTO_POSITION:
        ...
        break;
    case IDD_COLOR_SCREEN_TEXT:
        gpStateInfo->ScreenAttributes = (gpStateInfo->ScreenAttributes & 0xF0) | (Value & 0x0F);
        break;
    case IDD_COLOR_SCREEN_BKGND:
        gpStateInfo->ScreenAttributes = (gpStateInfo->ScreenAttributes & 0x0F) | (Value << 4);
        break;
    case IDD_COLOR_POPUP_TEXT:
        gpStateInfo->PopupAttributes = (gpStateInfo->PopupAttributes & 0xF0) | (Value & 0x0F);
        break;
    case IDD_COLOR_POPUP_BKGND:
        gpStateInfo->PopupAttributes = (gpStateInfo->PopupAttributes & 0x0F) | (Value << 4);
        break;
    case IDD_COLOR_1:
    case IDD_COLOR_2:
    case IDD_COLOR_3:
...
VOID UpdateWarningMessage(HWND hDlg, BOOL fLoadString)
{
    ...
    Value = (gpStateInfo->ScreenBufferSize.X * gpStateInfo->ScreenBufferSize.Y) / 0x080000;
    if (Value) {
...
INT_PTR ConsolePropertySheet(IN HWND hWnd)
/*++
...
WriteStateValues(PCONSOLE_STATE_INFO pStateInfo)
{
     ...
     */
    hMap = CreateFileMapping((HANDLE)-1, NULL, PAGE_READWRITE, 0, pStateInfo->Length, NULL);
    if (!hMap) {
