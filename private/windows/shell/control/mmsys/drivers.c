...
#define TreeView_GetGrandParent(_htree,_hti)         TreeView_GetParent(_htree,TreeView_GetParent(_htree,_hti))
#endif
...
#define nDriverKEYWORDS ((int)(sizeof(aDriverKeyword) / sizeof(aDriverKeyword[0])))
...
#define nKeywordDESCS   ((int)(sizeof(aKeywordDesc) / sizeof(aKeywordDesc[0])))
...
aDriverRoot[] = // (order will define order in display)
   {
      { dcINVALID, TRUE,  IDI_MMICON, IDS_MM_HEADER, 0, 0 },
      { dcWAVE,    TRUE,  IDI_WAVE,   IDS_WAVE_HEADER, IDS_ENABLEAUDIO, IDS_DISABLEAUDIO },
      { dcMIDI,    TRUE,  IDI_MIDI,   IDS_MIDI_HEADER, IDS_ENABLEMIDI, IDS_DISABLEMIDI },
      { dcMIXER,   TRUE,  IDI_MIXER,  IDS_MIXER_HEADER, IDS_ENABLEMIXER, IDS_DISABLEMIXER },
      { dcAUX,     TRUE,  IDI_AUX,    IDS_AUX_HEADER, IDS_ENABLEAUX, IDS_DISABLEAUX },
      { dcMCI,     TRUE,  IDI_MCI,    IDS_MCI_HEADER, IDS_ENABLEMCI, IDS_DISABLEMCI },
      { dcVCODEC,  TRUE,  IDI_ICM,    IDS_ICM_HEADER, IDS_ENABLEICM, IDS_DISABLEICM },
      { dcACODEC,  TRUE,  IDI_ACM,    IDS_ACM_HEADER, IDS_ENABLEACM, IDS_DISABLEACM },
      { dcVIDCAP,  TRUE,  IDI_VIDEO,  IDS_VIDCAP_HEADER, IDS_ENABLECAP, IDS_DISABLECAP },
      { dcJOY,     TRUE, IDI_JOYSTICK,IDS_JOYSTICK_HEADER, IDS_ENABLEJOY, IDS_DISABLEJOY },
      { dcOTHER,   FALSE, IDI_MMICON, IDS_OTHER_HEADER, IDS_ENABLEJOY, IDS_DISABLEJOY },
   };
...
#define nDriversToSKIP ((int)( sizeof(aDriversToSKIP)                                / sizeof(aDriversToSKIP[0]) ))
...
// We want to run "control joy.cpl" when the joystick devices are highlight and the user clicks Add/Remove/Properties buttons.
...
#undef REALLOC
#endif
#define REALLOC(_pData,_cOld,_cNew,_cDelta)         ReallocFn (sizeof(*_pData), (void **)&_pData, &_cOld, _cNew, _cDelta)
...
BOOL ReallocFn (mysize_t cbElement, LPVOID *ppOld, mysize_t *pcOld, mysize_t cNew, mysize_t cDelta)
{
   LPVOID     pNew;
   mysize_t   cbOld, cbNew;

            // First check if we actually need to reallocate or not.
            // It's possible that {ppOld} was already allocated with enough space.
...
*/
 ...
 LPTSTR GetProfile(LPTSTR pstrAppName, LPTSTR pstrKeyName, LPTSTR pstrIniFile, LPTSTR pstrRet, int cbSize)
 {
     TCHAR szNULL[2];

     szNULL[0] = TEXT('\0');
     GetPrivateProfileString(pstrAppName, (pstrKeyName==NULL) ? NULL : (LPTSTR)pstrKeyName, szNULL, pstrRet, cbSize/sizeof(TCHAR), pstrIniFile);
     return(pstrRet);
 }
...
BOOL AddIDriverToArray (PIDRIVER pIDriver)
{
    ...
    for (ii = 0; ii < cInstalledDrivers; ++ii)
    {
        ...
        {
            if (!lstrcmpi (aInstalledDrivers[ ii ].pIDriver->szFile, pIDriver->szFile))
            {
                return FALSE;
            }
        }
...
BOOL FillTreeFromWinMM (HWND hTree)
{
   ...
   for (iDevice = 0; iDevice < cDevices; ++iDevice)
      {
      if (mixerMessage ((HMIXER)iDevice,DRV_QUERYFILENAME,(DWORD_PTR)szDriver,(DWORD_PTR)cchRESOURCE) == MMSYSERR_NOERROR)
         {
   ...
   for (iDevice = 0; iDevice < cDevices; ++iDevice)
      {
      if (auxOutMessage (iDevice,DRV_QUERYFILENAME,(DWORD_PTR)szDriver,(DWORD_PTR)cchRESOURCE) == MMSYSERR_NOERROR)
         {
   ...
   if ((mmr == MMSYSERR_NOERROR) && (pCodecs != NULL))
      ...
      for (iiDr = 0; iiDr < cInstalledDrivers; ++iiDr)
         ...
         if (GuessDriverClass (aInstalledDrivers[ iiDr ].pIDriver) == dcACODEC)
            ...
            hDriver = OpenDriver (aInstalledDrivers[iiDr].pIDriver->wszAlias,aInstalledDrivers[iiDr].pIDriver->wszSection,0L);
            if (hDriver != NULL)
               {
      ...
      for (iiDr = 0; iiDr < cInstalledDrivers; ++iiDr)
         ...
         if ((aInstalledDrivers[ iiDr ].pIDriver->szAlias[0] == TEXT('\0')) || (GuessDriverClass (aInstalledDrivers[iiDr].pIDriver) == dcACODEC))
            {
            for (ii = 0; ii < cCodecs; ++ii)
               ...
               if ( (pCodecs[ ii ].wMid == LOWORD( aInstalledDrivers[ iiDr ].pIDriver->lp )) && (pCodecs[ ii ].wPid == HIWORD( aInstalledDrivers[ iiDr ].pIDriver->lp )) )
                  {
...
BOOL FillTreeFromMCI (HWND hTree)
{
   ...
   if (mciSendCommand (MCI_ALL_DEVICE_ID,MCI_SYSINFO,MCI_SYSINFO_QUANTITY,(DWORD_PTR)&mciSysInfo) == 0)
      {
      DWORD    iDevice;
      DWORD    cDevices;
      ...
      for (iDevice = 0; iDevice < cDevices; ++iDevice)
         ...
         if (mciSendCommand (MCI_ALL_DEVICE_ID,MCI_SYSINFO,MCI_SYSINFO_NAME,(DWORD_PTR)&mciSysInfo) != 0)
            {
            continue;
            }
         ...
         for (ii = 0; ii < cInstalledDrivers; ++ii)
            ...
            if (!lstrcmpi (aInstalledDrivers[ ii ].pIDriver->szAlias, szAlias))
               ...
               if (rc == MCIERR_MUST_USE_SHAREABLE)
                  {
                  rc = mciSendCommand (0, MCI_OPEN,MCI_OPEN_TYPE | MCI_OPEN_SHAREABLE,(DWORD)&mciOpen);
                  }
               if (rc == 0)
                  {
                  ...
                  if (mciSendCommand (mciOpen.wDeviceID,MCI_INFO,MCI_INFO_PRODUCT,(DWORD)&mciInfo) == 0)
                     {
...
BOOL CALLBACK FillTreeFromMSACMQueryCallback (HACMDRIVERID hadid,DWORD dwUser,DWORD fdwSupport)
{
   short       ii;
   AudioCodec *pac;
...
void FillTreeFromRemainingBySection (HWND hTree,long iiDriver,LPCTSTR pszSection,DriverClass dcSection)
{
   HKEY hk;
   UINT ii;
   ...
   for (ii = 0; ; ++ii)
      ...
      dw1 = cchRESOURCE;
      dw3 = cchRESOURCE;
      if (RegEnumValue (hk, ii,  szLHS, &dw1,0, &dw2, (LPBYTE)szRHS, &dw3) != ERROR_SUCCESS)
         {
         break;
         }
...
HWND MakeThisCPLLookLikeTheOldCPL (HWND hWndCPL)
{
   ...
   for (hWndOldCPL = GetWindow (hWndCPL, GW_HWNDFIRST);hWndOldCPL != NULL;hWndOldCPL = GetWindow (hWndOldCPL, GW_HWNDNEXT))
      {
      ...
      if ( (!lstrcmpi (szTitle, szTitleTest)) && (hWndCPL != hWndOldCPL) )
         ...
         SetWindowPos (hWndCPL, hWndOldCPL, rOld.left, rOld.top,0, 0, SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER);
         SetWindowPos (hWndOldCPL, hWndCPL, 0, 0, 0, 0,SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
         }
      }
...
HWND MakeThisDialogLookLikeTheOldDialog (HWND hDlg)
{
   ...
   for (hWndOldDlg = GetWindow (hDlg, GW_HWNDFIRST);hWndOldDlg != NULL;hWndOldDlg = GetWindow (hWndOldDlg, GW_HWNDNEXT))
      {
      TCHAR  szTitleTest[ cchRESOURCE ];
...
#define msecMAXWAIT   5000
   ...
   for (tickStart = GetTickCount();GetTickCount() - tickStart < msecMAXWAIT;)
      {
      MSG msg;

      for (hWnd = GetWindow (hWndMyDlg, GW_HWNDFIRST);hWnd != NULL;hWnd = GetWindow (hWnd, GW_HWNDNEXT))
         {
         TCHAR  szTitleTest[ cchRESOURCE ];
    ...
    switch ( uMsg )
    {
        ...
        if (szDriverWhichNeedsSettings[0] != TEXT('\0'))
        {
            ...
            */
            ...
            if ((!IniFileReadAllowed) || (!IniFileWriteAllowed))
            {
                ...
                MessageBox(hDlg, szCantAdd, szError, MB_OK | MB_ICONEXCLAMATION | MB_TASKMODAL);
            }
        ...
        case WM_COMMAND:
            ...
            switch ( LOWORD(wParam ))
            {
                ...
                break;
                    ...
                    if (TreeView_GetParent (hAdvDlgTree, htiCur) && TreeView_GetGrandParent (hAdvDlgTree, htiCur) && (GuessDriverClassFromTreeItem (TreeView_GetGrandParent (hAdvDlgTree, htiCur)) == dcMIDI))
                       {
                       TV_ITEM tvi;
                       PINSTRUM pin;
                       ...
                       if ((Status = PostRemove (pid, TRUE)) != DRVCNF_CANCEL)
                          ...
                          switch (dc)
                             ...
                             case dcMIDI:
                                   break;
                             case dcACODEC:
                                   acmDeleteCodec (LOWORD(pid->lp),HIWORD(pid->lp));
                                   break;
                             default:
        ...
        case ID_INIT:
            if (szDriverWhichNeedsSettings[0] != TEXT('\0'))
               ...
               if ((hti = FindTreeItemByDriverName (szDriverWhichNeedsSettings)) != 0)
               {
               TreeView_Expand (hAdvDlgTree,TreeView_GetParent(hAdvDlgTree,hti),TVE_EXPAND);
      ...
      case WM_NOTIFY:
            ...
            switch (lpnm->code)
               ...
               case NM_DBLCLK:
                     ...
                     if (lpnm->idFrom == (UINT)IDC_ADV_TREE)
                        ...
                        if ( (tvht.flags & TVHT_ONITEM) && (TreeView_GetChild (hTree, htiCur) == NULL) && (IsWindowEnabled (GetDlgItem(hDlg,ID_ADV_PROP))) )
                           {
                           FORWARD_WM_COMMAND(hDlg,ID_ADV_PROP,0,0,PostMessage);
                           }
                        }
        ...
        case WM_CONTEXTMENU:
            ...
            {
                WinHelp((HWND)wParam, gszWindowsHlp, HELP_CONTEXTMENU,(UINT_PTR)(LPTSTR)aAdvDlgHelpIds);
            }
            break;
        case WM_HELP:
            WinHelp(((LPHELPINFO)lParam)->hItemHandle, gszWindowsHlp,HELP_WM_HELP, (UINT_PTR)(LPTSTR)aAdvDlgHelpIds);
            break;
        case WM_DESTROY:
...
INT_PTR AvailableDriversDlg(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch ( uMsg )
    {
        ...
        case WM_COMMAND:
                        ...
                        if (!iIndex)
                        {
                            ...
                            */
                            ...
                            if (iFound == 1)
                            {
                                    ...
                                    SendDlgItemMessage(hWnd, LB_AVAILABLE, LB_RESETCONTENT, 0, 0L);
                                    PostMessage(hWnd, WM_INITDIALOG, 0, 0L);
                            }
                            bFindOEM = FALSE;
                        }
            ...
            break;
        case WM_CONTEXTMENU:
            WinHelp((HWND)wParam, gszWindowsHlp, HELP_CONTEXTMENU,(UINT_PTR)(LPTSTR)aAvailDlgHelpIds);
            break;
        case WM_HELP:
            WinHelp(((LPHELPINFO)lParam)->hItemHandle, gszWindowsHlp, HELP_WM_HELP, (UINT_PTR)(LPTSTR)aAvailDlgHelpIds);
            break;
        case WM_DESTROY:
...
BOOL DriversDllInitialize( IN PVOID hInstance, IN DWORD ulReason, IN PCONTEXT pctx OPTIONAL)
{
...
void DeleteCPLCache(void)
{
    ...
    if (ERROR_SUCCESS == RegOpenKey(HKEY_CURRENT_USER,TEXT("Control Panel\\Cache\\multimed.cpl"),&hKeyCache)) {
        for ( ; ; ) {
            TCHAR Name[50];

            if (ERROR_SUCCESS == RegEnumKey(hKeyCache,0,Name,sizeof(Name)/sizeof(TCHAR))) {
                HKEY hSubKey;
        ...
        case WM_INITDIALOG:
              ...
              */
              ...
              if (iRestartMessage)
              {
                ...
                {
                  PostMessage (hDlg, WM_NEXTDLGCTL, (WPARAM)GetDlgItem(hDlg,IDOK), (LPARAM)TRUE);
                }
              }
              return TRUE;
        case WM_KEYUP:
  ...
  switch (nMsg)
  {
      case WM_INITDIALOG:
      {
          ...
          if (!bFoundDrivers)
          {
                ...
                bFindOEM = TRUE;
                bBadOemSetup = TRUE;
                while ((iFound = DialogBox(myInstance, MAKEINTRESOURCE(DLG_INSERTDISK), hMesgBoxParent, AddDriversDlg)) == 2);
                bFindOEM = FALSE;
                if (iFound == 1)
                {
                        SendDlgItemMessage(hDlg, LB_AVAILABLE, LB_RESETCONTENT, 0, 0L);
...
DoHelp:
 ...
 /*
 *  Restart the system.
    If this fails we put up a message box
*/
{
     DWORD Error;
     BOOLEAN WasEnabled;
         ...
         */
         if (Error != NO_ERROR) {
            ...
            LoadString(myInstance,
                       Error == ERROR_PRIVILEGE_NOT_HELD  || Error == ERROR_NOT_ALL_ASSIGNED  || Error == ERROR_ACCESS_DENIED ? IDS_CANNOT_RESTART_PRIVILEGE : IDS_CANNOT_RESTART_UNKNOWN,
                       szCantRestart,
 ...
 * Note that the listed PIDRIVER should already have been added to the aInstalledDrivers array (via AddIDriverToArray()) before calling this routine.
*/
 ...
 * If there's a "Multimedia Devices" tree item under which the other roots are collected, this will return that item.
 * Otherwise, it returns TVI_ROOT.
*/
HTREEITEM AdvDlgFindTopLevel (void)
{
   short  idr;
...
BOOL InitAdvDlgTree (HWND hTree)
{
   ...
   if ((hImageList = ImageList_Create (cxIcon, cyIcon, TRUE, nDriverROOTS, 1)) == NULL)
      return FALSE;

   for (idr = 0; idr < nDriverROOTS; idr++)
      {
      HICON hi = LoadImage (myInstance, MAKEINTRESOURCE( aDriverRoot[idr].idIcon ), IMAGE_ICON, cxIcon, cyIcon, LR_DEFAULTCOLOR);
...
void lsplitpath (LPTSTR pszSource, LPTSTR pszDrive, LPTSTR pszPath, LPTSTR pszName, LPTSTR pszExt)
{
   ...
   if ((scManager = OpenSCManager (NULL, NULL, GENERIC_READ)) != NULL)
      ...
      if ((scDriver = OpenService (scManager, szName, GENERIC_READ)) != NULL)
         ...
         if (QueryServiceConfig (scDriver, &qsc, sizeof(qsc), &cbReq))
            ...
            {
            if (QueryServiceConfig (scDriver,(QUERY_SERVICE_CONFIG *)pqsc,cbReq, &cbReq))
               {
               if ( ((QUERY_SERVICE_CONFIG *)pqsc)->dwStartType != SERVICE_DISABLED)
                  {
                  SetFlag (dwStatus, dwStatusSvcENABLED);
                  }
               }
...
void ShowDeviceProperties (HWND hPar, HTREEITEM hti)
{
   ...
   if (TreeView_GetParent (hAdvDlgTree, hti) && TreeView_GetGrandParent (hAdvDlgTree, hti) && (GuessDriverClassFromTreeItem (TreeView_GetGrandParent (hAdvDlgTree, hti)) == dcMIDI))
      {
      if (InstrumentToResource (&ir, hti))
         {
         ShowMidiPropSheet (NULL,ir.szFriendlyName,hPar,MIDI_INSTRUMENT_PROP,ir.szFriendlyName,hti,(LPARAM)&ir,(LPARAM)hAdvDlgTree);
         FreeIResource (&ir);
         }
      return;
      }
   ...
   if (pid == NULL) // Just want class properties?
      ...
      if (dc == dcMIDI) // MIDI class properties?
         ...
         ShowMidiPropSheet (NULL,szTitle,hPar,MIDI_CLASS_PROP,szTitle,hti,(LPARAM)&cn,(LPARAM)hAdvDlgTree);
         }
...
INT_PTR CALLBACK DevPropDlg (HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    IsFlagClear(pIResource->fStatus,
        ...
        case WM_DROPFILES:
            break;
        case WM_CONTEXTMENU:
            WinHelp ((HWND) wParam, NULL, HELP_CONTEXTMENU, (UINT_PTR) (LPTSTR) aDevPropHelpIds);
            return TRUE;
        case WM_HELP:
        {
            LPHELPINFO lphi = (LPVOID) lParam;
            WinHelp (lphi->hItemHandle, NULL, HELP_WM_HELP, (UINT_PTR) (LPTSTR) aDevPropHelpIds);
            return TRUE;
        }
        case WM_COMMAND:
...
BOOL PASCAL DoDevPropCommand(HWND hDlg, int id, HWND hwndCtl, UINT codeNotify)
{
    ...
    case ID_APPLY:
        ...
        {
            if ( (IsFlagSet (fDevStatus, dwStatusMAPPABLE)) != (IsFlagSet (pIResource->fStatus, dwStatusMAPPABLE)) )
            {
                SetMappable (pIResource, (BOOL)IsFlagSet (fDevStatus, dwStatusMAPPABLE));
            }

            if ( (IsFlagSet (fDevStatus, dwStatusHASSERVICE)) && (IsFlagSet (fDevStatus, dwStatusSvcENABLED) != IsFlagSet (pIResource->fStatus, dwStatusSvcENABLED)) )
            {
#if 0 // TODO: Multiportmidi
                if ( (pIResource->iClassID == MIDI_ID) && (IsFlagSet(fDevStatus, DEV_MULTIPORTMIDI)) )
                {
                    EnableMultiPortMIDI (pIResource, IsFlagSet (fDevStatus, dwStatusSvcENABLED));
                }
                else
#endif
                {
                    EnableDriverService (pIResource, IsFlagSet (fDevStatus, dwStatusSvcENABLED));
                }
            }
...
#else // FIX_BUG_15451
    {
        ...
        else
        {
            ...
            if ((SendDriverMessage(hDriver,DRV_CONFIGURE,(LONG)hDlg,(LONG)(LPDRVCONFIGINFO)&DrvConfigInfo) == DRVCNF_RESTART))
            {
               iRestartMessage= 0;
               DialogBox(myInstance, MAKEINTRESOURCE(DLG_RESTART), hDlg, RestartDlg);
            }
   ...
   if ((scManager = OpenSCManager (NULL, NULL, SC_MANAGER_ALL_ACCESS)) != NULL)
      ...
      if ((scDriver = OpenService (scManager, szName, SERVICE_ALL_ACCESS)) != 0)
         ...
         ChangeServiceConfig (scDriver,
                              SERVICE_NO_CHANGE,
                              (fEnable) ? SERVICE_SYSTEM_START : SERVICE_DISABLED,
                              SERVICE_NO_CHANGE,
                              NULL,
                              NULL,
                              NULL,
...
BOOL SetMappable (PIRESOURCE pIResource, BOOL fMappable)
{
    ...
    if (RegSetValueEx (hKey,REGSTR_VALUE_MAPPABLE,(DWORD)0,REG_DWORD,(void *)&dwMappable,sizeof(dwMappable)) != ERROR_SUCCESS)
    {
        RegCloseKey (hKey);
        return FALSE;
    }
...
BOOL GetMappable (PIDRIVER pIDriver)
{
    ...
    if (RegQueryValueEx (hKey,REGSTR_VALUE_MAPPABLE,NULL,&dwType,(void *)&dwMappable,&dwSize) != ERROR_SUCCESS)
    {
        RegCloseKey (hKey);
        return TRUE;
    }
            ...
            if (RegOpenKey (HKEY_LOCAL_MACHINE, szKey, &hkParams) == 0)
            {
                ...
                for (iSubKey = 0; iSubKey < cSubKeys; ++iSubKey)
                {
                    ...
                    if (RegOpenKey (hkParams, szSubKey, &hk) == ERROR_SUCCESS)
                    {
                        ...
                        if (RegQueryValueEx (hk,cszRegValueLOADTYPE,NULL,&dwType,(void *)&dwLoadType,&dwSize) == 0)
                        {
                            if (dwLoadType == 1)
                            {
                                dwLoadType = 0;
                ...
                if (fFixedLoadType)
                {
                    ...
                    {
                        ChangeServiceConfig (scDriver,SERVICE_NO_CHANGE,SERVICE_SYSTEM_START,SERVICE_NO_CHANGE,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
                    }
                }
            }
        }
...
void ConfigureDriver (HWND hDlg, LPTSTR pszName)
{
    ...
    else
    {
        ...
        BOOL fHadService = IsFlagSet (dwStatus, dwStatusSvcSTARTED) && IsFlagSet (dwStatus, dwStatusHASSERVICE);
        DRVCONFIGINFO   DrvConfigInfo;
        InitDrvConfigInfo(&DrvConfigInfo, pid);
        if ((SendDriverMessage(hDriver,DRV_CONFIGURE,(LONG_PTR)hDlg,(LONG_PTR)(LPDRVCONFIGINFO)&DrvConfigInfo) == DRVCNF_RESTART))
        {
            iRestartMessage = 0;
            fRestartDialog = TRUE;
        }
        ...
        if (fHadService)
        {
            ...
            if (!IsFlagSet (dwStatus, dwStatusSvcSTARTED) || !IsFlagSet (dwStatus, dwStatusHASSERVICE))
            {
...
BOOL fDeviceHasMixers (LPTSTR pszName)
{
    ...
    for (ii = 0; ; ++ii)
    {
        ...
        if ( (GuessDriverClassFromAlias (szLHS) == dcMIXER) && (!FileNameCmp (pszName, szRHS)) )
        {
            rc = TRUE;
            break;
        }
