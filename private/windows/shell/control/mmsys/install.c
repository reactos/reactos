...
BOOL InstallDrivers(HWND hWnd, HWND hWndAvail, LPTSTR pstrKey)
{
    ...
    for (n = 1; infParseField(szTypes, n, szType); n++)
    {
        ...
        if (GetValidAlias(szType, IDTemplate.szSection) == FALSE)
        {
            ...
            LoadString(myInstance, IDS_CONFIGURE_DRIVER, szApp, sizeof(szApp)/sizeof(TCHAR));
            LoadString(myInstance, IDS_TOO_MANY_DRIVERS, szMessage, sizeof(szMessage)/sizeof(TCHAR));
            if (NULL != (pstrMessage = (LPTSTR)LocalAlloc(LPTR, sizeof(szMessage) + (lstrlen(szType)*sizeof(TCHAR)))))
            {
                wsprintf(pstrMessage, szMessage, (LPTSTR)szType);
                MessageBox(hWndAvail, pstrMessage, szApp, MB_OK | MB_ICONEXCLAMATION|MB_TASKMODAL);
        ...
        if ( (pIDriver = (PIDRIVER)LocalAlloc(LPTR, sizeof(IDRIVER))) != NULL)
        {
            ...
            if ( n > 1)
            {
                if (wcslen(szParams) != 0 && !pIDriver->KernelDriver)
                {
                     ...
                     */
                    WriteProfileString(pIDriver->szFile, pIDriver->szAlias, szParams);
                }

                WritePrivateProfileString(pIDriver->szSection, pIDriver->szAlias, pIDriver->szFile, szSysIni);
            }
            else
            {
                /*
                ...
                if (!SelectInstalled(hWndAvail, pIDriver, szParams, INVALID_HANDLE_VALUE, NULL))
                {
                     ...
                     */
                    WritePrivateProfileString(pIDriver->szSection, pIDriver->szAlias, NULL, szSysIni);
                    WriteProfileString(pIDriver->szFile, pIDriver->szAlias, NULL);
                 ...
                 */
                WritePrivateProfileString(szUserDrivers, pIDriver->szAlias, pIDriver->szFile, szControlIni);
                 ...
                 */
                WritePrivateProfileString(szRelatedDesc, pIDriver->szAlias, pIDriver->szRemove, szControlIni);

                /*
                 * Cache the description string in control.ini in the drivers description section.
                ...
                if (bInstallBootLine)
                {
                    ...
                    GetPrivateProfileString(szBoot, szDrivers, szTemp, szTemp, sizeof(szTemp) / sizeof(TCHAR), szSysIni);
                    wcscat(szTemp, TEXT(" "));
...
BOOL SelectInstalledKernelDriver(PIDRIVER pIDriver, LPTSTR pszParams)
{
    ...
    /*
     *  The services controller will create the registry node to which we can add the device parameters value
     */
    ...
    /*
     *  Try to write the parameters to the registry if there are any
     */
    if (wcslen(pszParams))
    {
        HKEY ParmsKey;
        ...
        Success = RegCreateKey(HKEY_LOCAL_MACHINE, RegPath, &ParmsKey) == ERROR_SUCCESS &&
                  RegSetValue(ParmsKey, TEXT(""), REG_SZ, pszParams, wcslen(pszParams)*sizeof(TCHAR)) == ERROR_SUCCESS &&
    ...
    if (Success)
    {
        ...
        Success = StartService(ServiceHandle, 0, NULL) && ChangeServiceConfig(ServiceHandle, SERVICE_NO_CHANGE, SERVICE_SYSTEM_START, SERVICE_NO_CHANGE, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        if (!Success)
        {
            TCHAR szMesg[MAXSTR];
...
BOOL SelectInstalled(HWND hwnd, PIDRIVER pIDriver, LPTSTR pszParams, HDEVINFO DeviceInfoSet, PSP_DEVINFO_DATA DeviceInfoData)
{
    ...
    // Open device reg key
    hkDrv = SetupDiOpenDevRegKey(DeviceInfoSet, DeviceInfoData, DICS_FLAG_GLOBAL, 0, DIREG_DRV, KEY_ALL_ACCESS);
    if (!hkDrv)
    {
        return GetLastError();
    }
    ...
    if (!SetupDiGetDeviceRegistryProperty(DeviceInfoSet, DeviceInfoData, SPDRP_CONFIGFLAGS, NULL, (PBYTE)&ConfigFlags, sizeof(ConfigFlags), NULL))
    {
        ConfigFlags = 0;
    }
    ...
    SetupDiSetDeviceRegistryProperty(DeviceInfoSet, DeviceInfoData, SPDRP_CONFIGFLAGS, (PBYTE)&ConfigFlags, sizeof(ConfigFlags));

    // See if this is a PnP device by trying to send it a PnP install message.
    // Call driver with DRV_PNPINSTALL
    DrvMsgResult = SendDriverMessage(hDriver, DRV_PNPINSTALL, (LONG_PTR)DeviceInfoSet, (LONG_PTR)DeviceInfoData);
    ...
    default:
        ...
        DrvMsgResult = SendDriverMessage(hDriver, DRV_INSTALL, 0L, (LONG_PTR)(LPDRVCONFIGINFO)&DrvConfigInfo);
        ...
        case DRVCNF_OK:
            ...
            pIDriver->fQueryable = (int)SendDriverMessage(hDriver, DRV_QUERYCONFIGURE, 0L, 0L);
            ...
            if (pIDriver->fQueryable)
            {
                ...
                switch (SendDriverMessage(hDriver, DRV_CONFIGURE, (LONG_PTR)hwnd, (LONG_PTR)(LPDRVCONFIGINFO)&DrvConfigInfo))
                {
                case DRVCNF_RESTART:
                    DeviceInstallParams.Flags |= DI_NEEDREBOOT;
                    bRestart = TRUE;
...
BOOL GetValidAlias(LPTSTR pstrType, LPTSTR pstrSection)
{
    ...
    for (AppendVal=0; AppendVal<=9; AppendVal++)
    {
        ...
        CharsFound = GetPrivateProfileString( pstrSection, pstrType, TEXT(""), keystr, sizeof(keystr) / sizeof(TCHAR), szSysIni);
        if (!CharsFound)
        {
            return TRUE;
        }
...
void CheckIniDrivers(LPTSTR szFile, LPTSTR szSection)
{
    ...
    GetPrivateProfileString(szSection, NULL, NULL, allkeystr, sizeof(allkeystr) / sizeof(TCHAR), szSysIni);
    keystr = allkeystr;
    while (wcslen(keystr) > 0)
    {
        GetPrivateProfileString(szSection, keystr, NULL, szRemovefile, sizeof(szRemovefile) / sizeof(TCHAR), szSysIni);
...
--*/
{
    ...
    if ((hKey = SetupDiOpenDevRegKey(DeviceInfoSet, DeviceInfoData, DICS_FLAG_GLOBAL, 0, DIREG_DRV, KEY_ALL_ACCESS)) == INVALID_HANDLE_VALUE)
    {
        return GetLastError();
    }
    ...
    if ((Err = RegQueryValueEx(hDriversKey, gszSubClassesValue, NULL, &RegDataType, (PBYTE)szTypes, &cbRegDataSize)) != ERROR_SUCCESS)
    {
        goto clean0;
    }
    ...
    for (i = 1; ((Err == NO_ERROR) && infParseField(szTypes, i, szType)); i++)
    {
        ...
        for (RegKeyIndex = 0; ((Err == NO_ERROR) && (RegEnumKey(hKey, RegKeyIndex, CharBuffer, sizeof(CharBuffer)/sizeof(TCHAR)) == ERROR_SUCCESS)); RegKeyIndex++)
        {
            ...
            if ((RegQueryValueEx(hTypeInstanceKey, gszDescriptionValue, NULL, &RegDataType, (LPBYTE)pIDriver->szDesc, &cbRegDataSize) != ERROR_SUCCESS) || (RegDataType != REG_SZ) || !cbRegDataSize)
            {
                LocalFree((HANDLE)pIDriver);
                goto CloseInstanceAndContinue;
            }

            wcsncpy(pIDriver->szSection, wcsstr(pIDriver->szDesc, TEXT("MCI")) ? szMCI : szDrivers, sizeof(pIDriver->szSection) / sizeof(TCHAR));
            cbRegDataSize = sizeof(pIDriver->szFile);
            if ((RegQueryValueEx(hTypeInstanceKey, gszDriverValue, NULL, &RegDataType, (LPBYTE)pIDriver->szFile, &cbRegDataSize) != ERROR_SUCCESS) || (RegDataType != REG_SZ) || !cbRegDataSize)
            {
                LocalFree((HANDLE)pIDriver);
                goto CloseInstanceAndContinue;
            }
            ...
            RegSetValueEx(hTypeInstanceKey, gszAliasValue, 0, REG_SZ, (PBYTE)(pIDriver->szAlias), (wcslen(pIDriver->szAlias)*sizeof(TCHAR)) + sizeof(TCHAR));
...
void InsertNewIDriverNodeInList(IN OUT PIDRIVER *IDriverList, IN     PIDRIVER  NewIDriverNode)
/*
Routine Description:
    This routine inserts a new IDriver node into the specified linked list of IDriver
    nodes.  The list is sorted by driver filename, and this node will be placed after any existing nodes having this same driver filename.
Arguments:
    IDriverList - Supplies the address of the variable that points to the head of the
        linked list.  If the new node is inserted at the head of the list, this variable will be updated upon return to reflect the new head of the list.
    NewIDriverNode - Supplies the address of the new driver node to be inserted into the list.
Return Value:
    None.
--*/
{
    ...
    for (CurNode = *IDriverList, PrevNode = NULL; CurNode; PrevNode = CurNode, CurNode = CurNode->related)
    {
...
void DestroyIDriverNodeList(IN PIDRIVER IDriverList, IN BOOL     CleanRegistryValues, IN BOOL     NotifyDriverOfCleanUp)
/*
Routine Description:
    This routine frees all memory associated with the nodes in the specified IDriver
    linked list.  It also optionally cleans up any modifications that were previously made as a result of an attempted install.
Arguments:
    IDriverList - Points to the head of the linked list of IDriver nodes.
    CleanRegistryValues - If TRUE, then the multimedia registry values previously created (e.g., Drivers32 aliases) will be deleted.
    NotifyDriverOfCleanUp - If TRUE, then the driver will be notified of its removal.
        This only applies to non-kernel (i.e., installable) drivers, and this flag is ignored if CleanRegistryValues is FALSE.
Return Value:
    None.
--*/
{
    ...
    while (IDriverList)
    {
        ...
        if (CleanRegistryValues)
        {
            ...
            WritePrivateProfileString(IDriverList->szSection, IDriverList->szAlias, NULL, szSysIni);
...
BOOL WINAPI SetRunOnceSchemeInit (void)
{
    ...
    if (ERROR_SUCCESS != RegCreateKeyEx (HKEY_LOCAL_MACHINE, aszRunOnce, 0, NULL, 0, KEY_ALL_ACCESS, NULL, &hKey, NULL))
    {
        return FALSE;
    }

    cbSize = (lstrlen(aszCommand)+1) * sizeof(TCHAR);
    if (ERROR_SUCCESS != RegSetValueEx (hKey, aszName, 0, REG_SZ, (LPBYTE)(LPVOID)aszCommand, cbSize))
    {
        RegCloseKey (hKey);
        return FALSE;
    }
...
Return Value:
    If the driver node supports Windows NT, the return value is TRUE, otherwise it is FALSE.
--*/
{
    ...
    if (!SetupDiGetDriverInfoDetail(DeviceInfoSet, DeviceInfoData, DriverInfoData, &DriverInfoDetailData, sizeof(DriverInfoDetailData), NULL) && ((Err = GetLastError()) != ERROR_INSUFFICIENT_BUFFER))
    {
        return FALSE;
    }
    ...
    SetupDiGetActualSectionToInstall(hInf, DriverInfoDetailData.SectionName, ActualSectionName, sizeof(ActualSectionName) / sizeof(TCHAR), &ActualSectionNameLen, NULL);

    // Generate the service install section name, and see if it exists.
    CopyMemory(&(ActualSectionName[ActualSectionNameLen - 1]), szServiceInstallSuffix, sizeof(szServiceInstallSuffix));
...
BOOL FilterOutNonNTInfs(IN HDEVINFO         DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL, DWORD DriverType)
{
    DWORD MemberIndex;
...
DWORD Media_SelectBestCompatDrv(IN HDEVINFO         DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
...
DWORD Media_AllowInstall(IN HDEVINFO         DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
    DWORD Err;
    SP_DRVINFO_DATA DriverInfoData;
...
DWORD Media_InstallDevice(IN HDEVINFO         DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
    ...
    if (!SetupDiInstallDevice(DeviceInfoSet, DeviceInfoData))
    {
        ...
        if (Err == ERROR_NO_ASSOCIATED_SERVICE)
        {
            ...
            SetupDiSetDeviceRegistryProperty(DeviceInfoSet, DeviceInfoData, SPDRP_DEVICEDESC, (PBYTE)DriverInfoData.Description, (lstrlen(DriverInfoData.Description) + 1) * sizeof(TCHAR));
        }
    ...
    // applicable for this device.  We must now traverse this tree, and create entries in Drivers32 for each function alias.
    ...
    if ((Err = InstallDriversForPnPDevice(hWnd, DeviceInfoSet, DeviceInfoData)) != NO_ERROR)
    {
        ...
        // Delete the Driver= entry from the Dev Reg Key and delete the DrvRegKey.
