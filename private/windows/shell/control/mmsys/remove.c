...
BOOL RemoveService(LPTSTR szFile)
{
    ...
    ServiceHandle = OpenService(SCManagerHandle, ServiceName, SERVICE_ALL_ACCESS);
...
LONG_PTR PostRemove(PIDRIVER pIDriver, BOOL bLookAtRelated)
{
     ...
     */
    for (pstr = pIDriver->szFile; *pstr && (*pstr != COMMA) && (*pstr != SPACE); pstr++)
        ;
    *pstr = TEXT('\0');
    ...
    if (bLookAtRelated) {
        ...
        if (GetPrivateProfileString(szRelatedDesc, pIDriver->szAlias, allkeystr, allkeystr, sizeof(allkeystr) / sizeof(TCHAR), szControlIni)) {
            int  i;
            TCHAR szTemp[MAXSTR];
    ...
    GetPrivateProfileString(pIDriver->szSection, NULL, NULL, allkeystr, sizeof(allkeystr) / sizeof(TCHAR), szSysIni);
    keystr = allkeystr;
...
BOOL SetValidAlias(LPTSTR pstrType, LPTSTR pstrSection)
{
    ...
    GetPrivateProfileString(pstrSection, NULL, NULL, allkeystr, sizeof(allkeystr) / sizeof(TCHAR), szSysIni);
    keystr = allkeystr;
    while (*keystr != TEXT('\0')) {
        // Compare the root of the aliases
        if (!_wcsnicmp(keystr, pstrType, typelen) &&
            ((keystr[typelen] <= TEXT('9') && keystr[typelen] > TEXT('0')) || keystr[typelen] == TEXT('\0'))) {
    ...
    if (bfound) {
        ...
        if (bExchange) {
            ...
            GetPrivateProfileString(pstrSection, szExKey, NULL, szExFile, sizeof(szExFile) / sizeof(TCHAR), szSysIni);
...
BOOL FindBootDriver(TCHAR *szKey)
{
    ...
    GetPrivateProfileString(TEXT("boot"), TEXT("drivers"), szDriverline, szDriverline, MAX_PATH, szSysIni);
    ...
    if (ptr) {
        if ((((ptr != szDriverline) && (*(ptr - 1) == TEXT(' '))) ||
            ...
            while (*endkey)
                *ptr++ = *endkey++;
            *ptr = NULL;
            WritePrivateProfileString(TEXT("boot"), TEXT("drivers"), szDriverline, szSysIni);
            return(TRUE);
        }
    }
    return(FALSE);
...
BOOL RemoveDriver(IN HDEVINFO DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
    ...
    hkDevReg = SetupDiOpenDevRegKey(DeviceInfoSet, DeviceInfoData, DICS_FLAG_GLOBAL, 0, DIREG_DRV, KEY_ALL_ACCESS);
    ...
    for (pszClass = mystrtok(szSubClasses, NULL, &strtok_State); pszClass; pszClass = mystrtok(NULL, NULL, &strtok_State)) {
        ...
        for (idxR3DriverName = 0;
            ...
            if (!RegQueryValueEx(hkR3DriverName, TEXT("Alias"), NULL, NULL, (LPBYTE)szAlias, &cbLen)) {
                RegDeleteValue(hkDrivers32, szAlias);// Delete the corresponding entry in Drivers32
            }


            RegCloseKey(hkR3DriverName);// Close the Driver Name key
        }

        RegCloseKey(hkClass);// Close the class key
    }
    ...
    for (pszClass = mystrtok(szSubClasses, NULL, &strtok_State); pszClass; pszClass = mystrtok(NULL, NULL, &strtok_State)) {
        ...
        for (idxR3DriverName = 0;
            ...
            if (!RegQueryValueEx(hkR3DriverName, TEXT("Driver"), NULL, NULL, (LPBYTE)szDriver, &cbLen)) {
                ...
                SHRegDeleteKey(HKEY_LOCAL_MACHINE, szR3Path);// Delete the key
            }

            RegCloseKey(hkR3DriverName);// Close the Driver Name key
        }

        RegCloseKey(hkClass);// Close the class key in the devnode
        ...
        RegCloseKey(hkClass);// Close the class key in MediaResources
    }
...
BOOL RemoveDriverInfo(IN HDEVINFO         DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
    ...
    SHRegDeleteKey(hkDevReg, TEXT("Drivers"));// Blow away the Drivers subtree
...
BOOL RemoveDeviceInfo(IN HDEVINFO DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
...
*/
DWORD Media_RemoveDevice(IN HDEVINFO DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
#if 0
    ...
    RemoveMediaResources(DeviceInfoSet, DeviceInfoData);// Clean out MediaResources section of registry    
    RemoveDriverInfo(DeviceInfoSet, DeviceInfoData);// Clean out driver's classguid\instance section of registry    
    RemoveDeviceInfo(DeviceInfoSet, DeviceInfoData);// Clean out device's enum section of registry

    return ERROR_DI_DO_DEFAULT;
}

#if 0   // Unused at present
BOOL AddDrivers32(IN HDEVINFO         DeviceInfoSet, IN PSP_DEVINFO_DATA DeviceInfoData OPTIONAL)
{
    BOOL bRet = FALSE;          // Return value
    TCHAR szDriverKey[MAX_PATH]; // Value of Driver's key in Enum branch
    TCHAR *pszDrvInst;           // Driver's instance, e.g. "0000", "0001", etc.
    ...
    // Open the Drivers32 section of the registry
    if (RegOpenKey(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Drivers32"), &hkDrivers32)) {
        goto RemoveDrivers32_exit;
    }

    // If we're adding a driver, need to open key to drivers.desc also
    if (RegOpenKey(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\drivers.desc"), &hkDriversDesc)) {
        goto RemoveDrivers32_exit;
    }
