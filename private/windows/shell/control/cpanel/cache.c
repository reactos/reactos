...
void ClearCacheValid()
{
    ...
    if (RegOpenKeyEx (HKEY_CURRENT_USER, pszRegCache, 0L, KEY_ALL_ACCESS, &hkeyRCache) == ERROR_SUCCESS)
    {
        j = 0;
        if ((RegSetValueEx (hkeyRCache, pszCacheValid, 0L, REG_DWORD, (LPBYTE) &j, sizeof(DWORD))) != ERROR_SUCCESS)
        {
            RIPREG();
        }
...
void SetCachedValue (LPTSTR pszCacheEntry, DWORD dwValue)
{
    ...
    if (RegOpenKeyEx (HKEY_CURRENT_USER, pszRegCache, 0L, KEY_ALL_ACCESS, &hkeyRCache) == ERROR_SUCCESS)
    {
        if ((RegSetValueEx (hkeyRCache, pszCacheEntry, 0L, REG_DWORD, (LPBYTE) &dwValue, sizeof(DWORD))) != ERROR_SUCCESS)
        {
            RIPREG();
        }
...
CreateKeyAgain:
        ...
        else
        {
            ...
            if ((RegSetValueEx  (hkeyCache, pszModulePath, 0L, REG_SZ, (LPBYTE) pCPlMod->szPathname, ByteCountOf(lstrlen(pCPlMod->szPathname)+1)))
                ||  // Set "Low File Time" value
                (RegSetValueEx (hkeyCache, pszFileTimeLow, 0L, REG_DWORD, (LPBYTE) &bhfiMod.ftLastWriteTime.dwLowDateTime, sizeof(DWORD)))
                ||  // Set "High File Time" value
                (RegSetValueEx (hkeyCache, pszFileTimeHigh, 0L, REG_DWORD, (LPBYTE) &bhfiMod.ftLastWriteTime.dwHighDateTime, sizeof(DWORD)))
                ||  // Set "File Size" value
                (RegSetValueEx (hkeyCache, pszFileSize, 0L, REG_DWORD, (LPBYTE) &bhfiMod.nFileSizeLow, sizeof(DWORD)))
                || // Set "Number Applets" value
                (RegSetValueEx (hkeyCache, pszNumApplets, 0L, REG_DWORD, (LPBYTE) &pCPlMod->numApplets, sizeof(DWORD))) != ERROR_SUCCESS)
            {
                RIPREG();
                goto RegistryError;
            }
...
//         USER would automatically choose the best icon for this display.
            //++++ NEED iICON value from ICON Handle (pCPlApp->hIcon)
                    ...
                    //  Icon
                    if ((RegSetValueEx (hkey, pszAppletIcon, 0L, REG_BINARY, (LPBYTE) lpIconRes, dwRes)) != ERROR_SUCCESS)
                    {
                        RIPREG();
                        goto RegistryError;
                    }
                    ...
                    {
                        //  IconX
                        if ((RegSetValueEx (hkey, pszAppletIconX, 0L, REG_DWORD, (LPBYTE) &iconinfo.xHotspot, sizeof(DWORD))) != ERROR_SUCCESS)
                        {
                            RIPREG();
                            goto RegistryError;
                        }
                        //  IconY
                        if ((RegSetValueEx (hkey, pszAppletIconY, 0L, REG_DWORD, (LPBYTE) &iconinfo.yHotspot, sizeof(DWORD))) != ERROR_SUCCESS)
                        {
                            RIPREG();
                            goto RegistryError;
                        }
                        ...
                        if (iconinfo.hbmMask)
                        {
                            if (dwBits = GetObject (iconinfo.hbmMask, sizeof(BITMAP), (LPTSTR) &bmInfo))
                            {
                                if ((RegSetValueEx (hkey, pszAppletInfoM, 0L, REG_BINARY, (LPBYTE) &bmInfo, dwBits)) != ERROR_SUCCESS)
                                {
                                    RIPREG();
                                    goto RegistryError;
                                }
                            ...
                            dwBits = GetBitmapBits (iconinfo.hbmMask, BMPSIZE, (LPVOID) lpBits);
                            if ((RegSetValueEx (hkey, pszAppletIconM, 0L, REG_BINARY, (LPBYTE) lpBits, dwBits)) != ERROR_SUCCESS)
                            {
                                RIPREG();
                                goto RegistryError;
                            }
                        ...
                        {
                            if (dwBits = GetObject (iconinfo.hbmColor, sizeof(BITMAP), (LPTSTR) &bmInfo))
                            {
                                if ((RegSetValueEx (hkey, pszAppletInfoC, 0L, REG_BINARY, (LPBYTE) &bmInfo, dwBits)) != ERROR_SUCCESS)
                                {
                                    RIPREG();
                                    goto RegistryError;
                                }
                            ...
                            dwBits = GetBitmapBits (iconinfo.hbmColor, BMPSIZE, (LPVOID) lpBits);
                            if ((RegSetValueEx (hkey, pszAppletIconC, 0L, REG_BINARY, (LPBYTE) lpBits, dwBits)) != ERROR_SUCCESS)
                            {
                                RIPREG();
                                goto RegistryError;
                            }
                            ...
                            //  Name
                    if ((RegSetValueEx (hkey, pszAppletName, 0L, REG_SZ, (LPBYTE) pCPlApp->pszName, ByteCountOf(lstrlen (pCPlApp->pszName)+1)))
                        ||  //  Full Name includes '&' char
                        (RegSetValueEx (hkey, pszAppletFull, 0L, REG_SZ, (LPBYTE) pCPlApp->pszFullName, ByteCountOf(lstrlen (pCPlApp->pszFullName)+1)))
                        ||  //  Info
                        (RegSetValueEx (hkey, pszAppletInfo, 0L, REG_SZ, (LPBYTE) pCPlApp->pszInfo, ByteCountOf(lstrlen (pCPlApp->pszInfo)+1)))
                        ||  //  Help context
                        (RegSetValueEx (hkey, pszAppletCntx, 0L, REG_DWORD, (LPBYTE) &pCPlApp->dwContext, sizeof(DWORD)))
                        ||  //  lData value
                        (RegSetValueEx (hkey, pszAppletData, 0L, REG_DWORD, (LPBYTE) &pCPlApp->lData, sizeof(DWORD))) != ERROR_SUCCESS)
                    {
                        RIPREG();
                        goto RegistryError;
                    }
                    ...
                    {
                        if ((RegSetValueEx (hkey, pszAppletHelp, 0L, REG_SZ, (LPBYTE) pCPlApp->pszHelpFile, ByteCountOf(lstrlen (pCPlApp->pszHelpFile)+1))) != ERROR_SUCCESS)
                        {
                            RIPREG();
                            goto RegistryError;
                        }
...
TryCreateKeyAgain:
            ...
            else
            {
                ...
                //  the default set by CheckFindModules().
                        ...
                        // Set "Module Path" value
                if ((RegSetValueEx  (hkeyCache, pszModulePath, 0L, REG_SZ, (LPBYTE) cfCPlFiles[i].szPathname, ByteCountOf(lstrlen(cfCPlFiles[i].szPathname)+1)))
                    ||  // Set "Low File Time" value
                    (RegSetValueEx (hkeyCache, pszFileTimeLow, 0L, REG_DWORD, (LPBYTE) &cfCPlFiles[i].ftModule.dwLowDateTime, sizeof(DWORD)))
                    ||  // Set "High File Time" value
                    (RegSetValueEx (hkeyCache, pszFileTimeHigh, 0L, REG_DWORD, (LPBYTE) &cfCPlFiles[i].ftModule.dwHighDateTime, sizeof(DWORD)))
                    ||  // Set "File Size" value
                    (RegSetValueEx (hkeyCache, pszFileSize, 0L, REG_DWORD, (LPBYTE) &cfCPlFiles[i].dwSize, sizeof(DWORD)))
                    || // Set "Number Applets" value
                    (RegSetValueEx (hkeyCache, pszNumApplets, 0L, REG_DWORD, (LPBYTE) &cfCPlFiles[i].numApplets, sizeof(DWORD))) != ERROR_SUCCESS)
                {
                    RIPREG();
                    goto RegistryError;
                }
...
CommonExit:
    ...
    SetErrorMode (uError);//  Restore prior error mode
