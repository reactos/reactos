...
typedef INT (APIENTRY *SHELL_ABOUT)(HWND hWnd, LPTSTR szApp, LPTSTR szOtherStuff, HICON hIcon);
...
void ConvertCplInfoA (LPNEWCPLINFO lpCPlInfoW)
{
   ...
   MultiByteToWideChar (CP_ACP, MB_PRECOMPOSED, CplInfoA.szName, 32, (LPWSTR) lpCPlInfoW->szName, 32);
   MultiByteToWideChar (CP_ACP, MB_PRECOMPOSED, CplInfoA.szInfo, 64, (LPWSTR) lpCPlInfoW->szInfo, 64);
   MultiByteToWideChar (CP_ACP, MB_PRECOMPOSED, CplInfoA.szHelpFile, 128, (LPWSTR) lpCPlInfoW->szHelpFile, 128);
}
...
void SetAppItems(PCPLMODULE pCPlMod, LPCPLINFO pCPlInfo, LPNEWCPLINFO pNewCPlInfo, PCPLAPPLET pCPlApp, int iIndex)
{
    ...
    if (pCPlApp->pszName = (LPTSTR) LocalAlloc (LPTR, ByteCountOf(numChars+1)))
    {
        ...
        // after stripping & junk
         ...
         GetPrivateProfileString(TEXT("don't load"), pCPlApp->pszName, szNULL, szTemp, 10, aszControlIniPath);
...
//              SendMessage (hCPlLB, LB_SETCURSEL, 0, 0L);
    ...
    if (bCache)
    {
        ...
        hThread = CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE) AbsoluteValidation, NULL, 0, &tid);
        CloseHandle(hThread);
    }
    else
    {
...
BOOL GetAppletName (DWORD dwProcessId, LPTSTR lpszAppletName, DWORD dwAddress, BOOL fUnicode)
{
    ...
    if (hProcess)
    {
        ...
        if (fUnicode)
        {
            while (ReadProcessMemory (hProcess, (LPVOID) dwAddress, szReadBufferW, 2, &dwBytesRead))
            {
        ...
        {
            while (ReadProcessMemory (hProcess, (LPVOID) dwAddress, szReadBufferA, 1, &dwBytesRead))
            {
...
ConvertAppletName:
                        //  Convert to Unicode
                        MultiByteToWideChar (CP_ACP, MB_PRECOMPOSED, szNameBufferA, -1, lpszAppletName, MAX_APPLET_NAME_LEN);
                        break;
                    }
                    else
                        dwAddress++;
