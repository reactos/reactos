...
static  void PASCAL NEAR KeyDown(HWND    hwnd, UINT    uKey)
{
    DWORD   dCoordinates;
...
static  void PASCAL NEAR KeyUp(HWND    hwnd, UINT    uKey)
{
    if (fKeyDown && ((uKey == VK_DOWN) || (uKey == VK_UP))) {
        fKeyDown = FALSE;
...
static  void PASCAL NEAR LButtonDown(HWND    hwnd, int     iCoord)
{
    if (!fRealButton) {
        fButton = TRUE;
        fRealButton = TRUE;
...
static  void PASCAL NEAR MouseMove(HWND    hwnd, POINT   pt)
{
...
static  void PASCAL NEAR Paint(HWND    hwnd)
{
    PAINTSTRUCT     ps;
...
BOOL    PASCAL FAR RegisterArrowClass(HINSTANCE       hInstance)
{
    WNDCLASS        wcArrow;
...
void    PASCAL FAR UnregisterArrowClass(HINSTANCE       hInstance)
{
