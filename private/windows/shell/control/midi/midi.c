...
#ifdef STUPID
/*
 * ComboBox String Lookup, written because we were stupid and let an intern design this applet.
*/
int FAR PASCAL ComboLookup(HWND hCombo, LPSTR szLookup)
{
    int iEntries;
...
static  BOOL FAR PASCAL FGetMapFile(HWND    hwnd)
{
        char    aszBuf[256];
        DWORD   dwVersion;
        MMAPERR mmaperr;
        ...
        LPARAM  lParam2)
{
        ...
        case CPL_NEWINQUIRE :
                ...
                lpCPlInfo->hIcon = LoadIcon(hLibInst, MAKEINTRESOURCE(applets[iApplet].idIcon));
 ...
 * exists in szCurrent.
*/
static  BOOL NEAR PASCAL FConfirmDeleteMap(HWND    hwnd)
{
        ...
        int     idCtrl)
{
        ...
        for (; uCount--; ) {
                ...
                if ((hDescription = (HGLOBAL)(DWORD)SendDlgItemMessage(hdlg, idCtrl, CB_GETITEMDATA, (WPARAM)uCount, (LPARAM)0)) != NULL)
...
static  BOOL PASCAL FEditMap(HWND    hwnd)
{
        HWND    hTmpWnd;
        MBDATA  mbData;
        DWORD   dwRet;
...
!!*/
                ...
                mmaperr = mapEnumerate( MMAP_SETUP, EnumFunc, MMENUM_INTOCOMBO, GetDlgItem( hdlg, ID_MAINSETUPCOMBO), NULL);
                if ( mmaperr != MMAPERR_SUCCESS )
                        goto exit00;
                // set up the auto-radiobuttons
                CheckRadioButton(hdlg, ID_MAINFIRSTRADIO, ID_MAINLASTRADIO, ID_MAINSETUP);
...
#if defined(WIN16)
                SendMessage(hdlg, WM_COMMAND, (WPARAM)ID_MAINCOMBO, MAKELPARAM(hCombo, CBN_SELCHANGE));
#else
        ...
        case WM_COMMAND:
            {   WORD wNotifCode;
                ...
                case IDOK:
                        ...
                        if ((mmaperr = MmaperrInvalidPortCheck(szCurSetup)) == MMAPERR_INVALIDPORT) {
                                if (!InvalidPortMsgBox(hdlg))
                                        break;
                        } else if (mmaperr != MMAPERR_SUCCESS)
                                goto exit00;
                        if (lstrcmpi(szCurSetup, aszInitialSetup))
                                {
                                        mmaperr = mapSetCurrentSetup(szCurSetup);
                                        if (mmaperr != MMAPERR_SUCCESS)
                                                goto exit00;
                                }
                        ...
                        if (uDeleted & MMAP_SETUP)
                                mapEnumerate( iMap = MMAP_SETUP, EnumFunc, MMENUM_DELETE, GetDlgItem(hdlg,ID_MAINSETUPCOMBO), NULL);
                        if (uDeleted & MMAP_PATCH)
                                mapEnumerate( iMap = MMAP_PATCH, EnumFunc, MMENUM_DELETE, GetDlgItem(hdlg,ID_MAINPATCHCOMBO), NULL);
                        if (uDeleted & MMAP_KEY)
                                mapEnumerate( iMap = MMAP_KEY, EnumFunc,, MMENUM_DELETE, GetDlgItem (hdlg,ID_MAINKEYCOMBO), NULL);
#endif
                ...
                case ID_MAINDELETE:
                        ...
                        uIdx = (UINT)SendMessage(hCombo, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
                        // get the handle to selections description
                        if ((hDesc = (HGLOBAL)(DWORD)SendMessage(hCombo, CB_GETITEMDATA, (WPARAM)uIdx, (LPARAM)0)) != NULL)
                                GlobalFree(hDesc);
                ...
                case ID_MAINNEW:
                        ...
                        fNew = TRUE;
                        if (!DialogBox(hLibInst, MAKEINTRESOURCE(ID_PROPBOX), hdlg, (DLGPROC)PropBox)) {
                                fNew = FALSE;
                                break;
                        }
                        ...
                        if (!FEditMap(hdlg)) {
                                fNew = FALSE;
                                GetWindowText(hCombo, (LPSTR)szCurrent, MMAP_MAXNAME);
                                GetDlgItemText(hdlg, ID_MAINDESC, szCurDesc, MMAP_MAXDESC);
                                break;
                        }
                        ...
                        if (uIdx == CB_ERR)
                                uIdx = (UINT)SendMessage(hCombo, CB_ADDSTRING, (WPARAM)0, (LPARAM)(LPCSTR)szCurrent);
                        ...
                        if ((hDesc = GlobalAlloc(GHND, (DWORD)(sizeof(char) * (lstrlen (szCurDesc) + 1)))) != NULL) {
                        ...
                        SendMessage(hCombo, CB_SETITEMDATA, (WPARAM)uIdx, (LPARAM)(DWORD)(UINT)hDesc);
                ...
                case ID_MAINCOMBO:
                        ...
                        if (LOWORD(wParam) == ID_MAINSETUPCOMBO)
                                SendMessage(hCombo, CB_GETLBTEXT, (WPARAM)uIdx, (LPARAM) (LPSTR) szCurSetup);
                        hDesc = (HGLOBAL)(DWORD)SendMessage(hCombo, CB_GETITEMDATA, (WPARAM)uIdx, (LPARAM)0);
                        lpDesc = GlobalLock(hDesc);
                        lstrcpy(szCurDesc, lpDesc);
                        GlobalUnlock(hDesc);
                        SetDlgItemText (hdlg, ID_MAINDESC, szCurDesc);
                        SendMessage(hCombo, CB_GETLBTEXT, (WPARAM)uIdx, (LPARAM)(LPSTR)szCurrent);
                        break;
                case ID_MAINDESC:
                        ...
                        if (wNotifCode == EN_CHANGE)
                                ...
                                GetDlgItemText (hdlg, ID_MAINDESC, szTmpDesc, MMAP_MAXDESC);
...
DoHelp:
                        WinHelp(hWnd, szMidiHlp, HELP_CONTEXT, IDH_CHILD_MIDI);
                        return TRUE;
                }
                else
                        return FALSE;
...
BOOL    FAR PASCAL InvalidPortMsgBox (HWND    hwnd)
{
        int     iRet;
        char    szBuf[256];

        LoadString(hLibInst, IDS_INVALIDPORT, szBuf, sizeof(szBuf));
        iRet = MessageBox (hwnd, szBuf, szMidiCtl, MB_ICONSTOP | MB_YESNO);
        return iRet == IDYES;
} /* InvalidPortMsgBox */

VOID FAR PASCAL Modify(BOOL    fSet)
{
        fModified = fSet;
        if (fModified)
                fChanged = TRUE;
        ...
        )
{
        ...
        if (uCase == MMENUM_INTOCOMBO) {
                ...
                SendMessage( hCombo, CB_SETITEMDATA, (WPARAM)uIdx, (LPARAM)(DWORD)(UINT)hMem);
        }
        return TRUE;
 } /* EnumFunc */
...
*/
VOID FAR PASCAL SetWindowCaption(VOID)
{
        char    szCaption[80],
                szName[MM_MAXRESLEN];
