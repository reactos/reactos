    ...
    if (nLongData && (fForce || FILTERSTATUS(status) != FILTERSTATUS(LastStatus) || uMessageLength + nLongData > sizeof(LongBuffer) ||     LastBroadcast != fBroadcast))
    {
        ...
        if (LastBroadcast) {
            ...
            for (wDevice = 0; (wDevice < 16) && (mapdevs[wDevice].wDeviceID != (WORD)(-1)); wDevice++) {
                ...
                if (MMSYSERR_NOERROR == midiOutLongMsg(mapdevs[wDevice].hMidi, pmidihdrMapper, sizeof(MIDIHDR))) {
                    while (!modHeaderDone())
                        Sleep(1);
                }
            }
            ...
            pmidihdrMapper->dwFlags &= ~MHDR_DONE;
            if (MMSYSERR_NOERROR ==    midiOutLongMsg(chnldevs[FILTERSTATUS(LastStatus)].hMidi, pmidihdrMapper, sizeof(MIDIHDR))) {
                while (!modHeaderDone())
                    Sleep(1);
            }
        }
