        ...
        UINT    uSize)
{
        ...
        */
        lRet = RegOpenKeyEx( HKEY_LOCAL_MACHINE, RegEntry, 0L, KEY_QUERY_VALUE, &hKey );
        if (lRet == ERROR_SUCCESS) {
            DWORD  dwType, dwLen;

            dwLen = uSize;
            lRet = RegQueryValueEx( hKey, RegCurrent, 0L, &dwType, (LPBYTE)lpBuf, &dwLen );
            RegCloseKey( hKey );
...
//      1.      None anymore.
STATIC  MMAPERR NEAR PASCAL MmaperrReadTable(UINT    APIn)
{
        MMHEADER        mmHeader;
        LPMMTABLE       lpmmTable;
        LPMMTABLEENTRY  lpmmEntry;
...
exit02:                         GlobalUnlock(*lpTable);
                        ...
                        if ( _lread(iFile, (LPSTR)&lpmmTable->mmHeader,sizeof(MMTABLEHEADER)) != sizeof(MMTABLEHEADER)) {
                                mmaperr = MMAPERR_READ;
                                goto exit02;
                        }
                        uTableSize = lpmmTable->mmHeader.wEntrys * sizeof(MMTABLEENTRY);
                        lpmmTable->hEntrys = GlobalAlloc( GHND, (DWORD)uTableSize);
                        if (lpmmTable->hEntrys == NULL) {
                                mmaperr = MMAPERR_MEMORY;
                                goto exit02;
                        }
                        lpmmEntry = (LPMMTABLEENTRY)GlobalLock(lpmmTable->hEntrys);
                        if ( _lread(iFile, (LPSTR)lpmmEntry, uTableSize) != uTableSize) {
                                mmaperr = MMAPERR_READ;
...
STATIC  VOID NEAR PASCAL VFreeTable(UINT    APIn)
...
exit01:         return mmaperr;
        ...
        if (_llseek( iFile, (LONG) ( *lpoTable + sizeof(MMTABLEHEADER) + uIdx * sizeof(MMTABLEENTRY)), 0) == -1L) {
                mmaperr = MMAPERR_WRITE;
                goto exit00;                            // 00, not 01.
        }
        if ( _lwrite(iFile, (LPSTR)lpmmEntry, sizeof(MMTABLEENTRY))!= sizeof(MMTABLEENTRY)) {
                mmaperr = MMAPERR_WRITE;
                goto exit00;                            // 00, not 01.
        }
        mmaperr = MmaperrFileAccess(MAP_FCLOSE, 0);
...
STATIC  MMAPERR NEAR PASCAL MmaperrWriteTabHeader(UINT uFlag, LPMMTABLEHEADER lpmmHeader)
...
STATIC  MMAPERR NEAR PASCAL MmaperrGarbage(UINT    uBytes)
