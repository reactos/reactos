...
**   Description:
**        This is a module of the T1 to TT font converter.
**        The module contains functions that manages the "printer binary file" file format (Adobe Type 1 for MS-Windows).
...
**   Please note that this function does not check if it succeedes it reading a byte or not.
**   It is up to the calling module to manage the  error checkes by using the FileError() function when appropriate.
*/
short FASTCALL PFBGetByte(struct t1file *pfb)
{
   ...
   if (pfb->curr>=pfb->size) {
      ...
      pfb->type = ((type[0]==0x80 && (type[1]==0x01 || type[1]==0x02)) ? ascii : encoded);
   }
