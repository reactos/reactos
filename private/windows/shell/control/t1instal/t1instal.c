...
char *GetCompany(char *buf)
{
    ...
    while (token && !done) {
        ...
        if (!_strnicmp((char*) token, "(c)", strlen("(c)")) || !_strnicmp((char*) token, "copyright", strlen("copyright")) || token[0] == COPYSIGN)
            continue;

        /* Skip the years. */
        company = token;
        if (isdigit(token[0])) {
            while (isdigit(*company) || isspace(*company) || ispunct(*company) || (*company) == '-')
                company++;
    ...
    const char *notice)
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, SUBKEY_TYPE1COPYRIGHTS, 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS) {
...
static BOOL ReadStringFromOffset(struct ioFile *file, const DWORD dwOffset, char *pszString, int cLen, BOOL bStrip)
{
    BOOL result = TRUE;
    DWORD offset;
    ...
    void *arg)
{
    ...
    try
    {
        bRet = ConvertTypefaceAInternal(type1, metrics, truetype, Progress, arg);
    }
...
short STDCALL FindPfb(char *pszPFM, char *achPFB);
...
**           FVS_BAD_VERSION
*/
short STDCALL CheckPfmA(char  *pszPFM, DWORD  cjDesc, char  *pszDesc, DWORD  cjPFB, char  *pszPFB)
{
    struct ioFile *file;
...
*/
BOOL WINAPI DllMain(PVOID hmod, ULONG ulReason, PCONTEXT pctx OPTIONAL)
{
...
*/
short STDCALL CheckCopyrightAInternal(const char *szPFB, const DWORD wSize, char *szVendor)
{
    struct T1Arg t1Arg;
    struct TTArg ttArg;
...
short STDCALL CheckCopyrightA(const char *szPFB, const DWORD wSize, char *szVendor)
{
    short iRet;
    ...
    )
{
    ...
    *pbCreatedPFM = FALSE; // initialization is essential.
    ...
    if (!_strcmpi(&pszKeyFile[cjKey - 5], ".PFM"))
    {
        ...
        return CheckPfmA(pszKeyFile, cjDesc, pszDesc, cjPFB, pszPFB);
    }
    else if (!_strcmpi(&pszKeyFile[cjKey - 5], ".INF"))
    {
        return CheckInfA(pszKeyFile, cjDesc, pszDesc, cjPFM, pszPFM, cjPFB, pszPFB, pbCreatedPFM, pszFontPath);
    }
    else
    {
    ...
    )
{
    ...
    try
    {
        status = CheckType1AInternal(pszKeyFile, cjDesc, pszDesc, cjPFM, pszPFM, cjPFB, pszPFB, pbCreatedPFM, pszFontPath);
    }
    except(EXCEPTION_EXECUTE_HANDLER)
    {
...
{
    short status = CheckType1WithStatusA(pszKeyFile, cjDesc, pszDesc, cjPFM, pszPFM, cjPFB, pszPFB, pbCreatedPFM, pszFontPath);
