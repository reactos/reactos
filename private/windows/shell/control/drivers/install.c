        ...
        */
        ...
        if (GetValidAlias(hWndI, szType, IDTemplate.szSection) == FALSE)
        {
            ...
            */
            ...
            LoadString(myInstance, IDS_CONFIGURE_DRIVER, szApp, sizeof(szApp));
            LoadString(myInstance, IDS_TOO_MANY_DRIVERS, szMessage, sizeof(szMessage));
            if (NULL != (pstrMessage = (PSTR)LocalAlloc(LPTR, sizeof(szMessage) + lstrlen(szType))))
            {
        ...
        if ( (pIDriver = (PIDRIVER)LocalAlloc(LPTR, sizeof(IDRIVER))) != NULL)
        {
            ...
            if ( n > 1) {
                ...
                */
                iIndex = (int)SendMessage(hWndI, LB_ADDSTRING, 0, (LONG)pIDriver->szDesc);
                if (iIndex >= LB_OKAY) {
                   /*
                   ...
                   /*
                    *  Call the driver to see if it can be configured and configure it if it can be
                    */
                    ...
                    */
                    ...
                    if (bInstallBootLine) {
                        ...
                        GetPrivateProfileString(szBoot, szDrivers, szTemp, szTemp, sizeof(szTemp), szSysIni);
                        strcat(szTemp, " ");
            ...
            if (ServiceHandle != NULL) {
                ...
                */
                if (strlen(pszParams)) {
                    ...
                    Success = RegCreateKey(HKEY_LOCAL_MACHINE, RegPath, &ParmsKey) == ERROR_SUCCESS && RegSetValue(ParmsKey, "", REG_SZ, pszParams, strlen(pszParams)) == ERROR_SUCCESS && RegCloseKey(ParmsKey) == ERROR_SUCCESS;
                } else {
                    Success = TRUE;
                }
        ...
        */
        ...
        if (hDriver)
        {
            ...
            */
            ...
            switch (SendDriverMessage(hDriver, DRV_INSTALL, 0L, (LONG)(LPDRVCONFIGINFO)&DrvConfigInfo))
            {
            case DRVCNF_RESTART:
                bRestart = TRUE;
                break;
            ...
            */
            if (pIDriver->fQueryable) {
                switch (SendDriverMessage(hDriver, DRV_CONFIGURE, (LONG)hwnd, (LONG)(LPDRVCONFIGINFO)&DrvConfigInfo)) {
                case DRVCNF_RESTART:
                    bRestart = TRUE;
                    break;
                case DRVCNF_CANCEL:
...
BOOL GetValidAlias(HWND hwnd, PSTR pstrType, PSTR pstrSection)
{
    ...
    GetPrivateProfileString(pstrSection, NULL, NULL, allkeystr, sizeof(allkeystr), szSysIni);
    keystr = allkeystr;
    ...
    */
    while (*keystr != '\0')
    {
       if (!_strnicmp(keystr, pstrType, typelen) && ((keystr[typelen] > '0' && keystr[typelen] <= '9') || keystr[typelen] == TEXT('\0') ))
       {
          found = TRUE;
...
*/
{
    ...
    GetPrivateProfileString(szSection, NULL, NULL, allkeystr, sizeof(allkeystr), szSysIni);
    keystr = allkeystr;
    while (strlen(keystr) > 0)
    {
         GetPrivateProfileString(szSection, keystr, NULL, szRemovefile, sizeof(szRemovefile), szSysIni);
