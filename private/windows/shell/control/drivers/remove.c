     ...
     */
     ...
     if (ServiceHandle != NULL) {
         ...
         */
         ...
         if (Error == ERROR_FILE_NOT_FOUND || Error == ERROR_PATH_NOT_FOUND || Error == ERROR_SERVICE_DOES_NOT_EXIST) {
              Status = TRUE;
         }
     }
...
LONG PostRemove(HWND hWnd, PIDRIVER pIDriver, BOOL bLookAtRelated, int iIndexMain)
/*
...
*/
{
    ...
    */
    ...
    if (bLookAtRelated)
    {
        ...
        if (GetPrivateProfileString(szRelatedDesc, pIDriver->szAlias, allkeystr, allkeystr, sizeof(allkeystr), szControlIni))
        {
            int i, iIndex;
            BOOL bFound;
            PIDRIVER pIDriver;
            ...
            for (i = 1; infParseField(allkeystr, i, szTemp); i++)
            {
                ...
                while (iIndex-- > 0 && !bFound)
                    if ((int) (pIDriver = (PIDRIVER) SendMessage(hWnd, LB_GETITEMDATA, iIndex, 0L)) != LB_ERR) {
                        ...
                        {
                            if (PostRemove(hWnd, pIDriver, FALSE, iIndex) == DRVCNF_RESTART) {
    ...
    GetPrivateProfileString(pIDriver->szSection, NULL, NULL, allkeystr, sizeof(allkeystr), szSysIni);
    keystr = allkeystr;
 ...
 BOOL SetValidAlias(PSTR pstrType, PSTR pstrSection)
 {
    ...
    if (len == typelen)
        ...
        if (bExchange)
        {
           ...
           GetPrivateProfileString(pstrSection, szExKey, NULL, szExFile, sizeof(szExFile), szSysIni);
 ...
 *  tell if a DRV_INSTALL message has been sent to the driver yet OR if it is the last instance of the driver so we can send a DRV_REMOVE message.
*/
 BOOL IsOnlyInstance( HWND hwndLB, PIDRIVER pIDriver )
 {
     ...
     */
     ...
     if ((iEntries = (int)SendMessage(hwndLB, LB_GETCOUNT, 0, 0L)) == LB_ERR)
         ...
         while (iEntries-- > 0)
         {
             pIDTest = (PIDRIVER)SendMessage(hwndLB, LB_GETITEMDATA, iEntries, 0L);
             if (pIDTest == NULL)
                 continue;
...
*/
{
    ...
    GetPrivateProfileString("boot", "drivers", szDriverline, szDriverline, MAX_PATH, szSysIni);
    ptr = strstri(szDriverline, szKey);
    if (ptr)
    {
        if ((((ptr != szDriverline) && (*(ptr - 1) == ' ')) || (ptr == szDriverline)) && (*(ptr + wKeyLen) == ' ' || *(ptr + wKeyLen) == NULL))
        {
            endkey = ptr + wKeyLen;
            while (*endkey)
                *ptr++ = *endkey++;
            ...
            *ptr = NULL;
            WritePrivateProfileString("boot", "drivers", szDriverline, szSysIni);
            return(TRUE);
        }
    }
