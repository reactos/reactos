...
UINT FileCopy (LPSTR szSource, LPSTR szDir, FPFNCOPY fpfnCopy, UINT fCopy)
{
   ...
   */
   ...
   for (nDisk = 1; err == ERROR_SUCCESS && (cntFiles > 0) && (nDisk <= MAX_DISKS + 'Z' - 'A' + 1); nDisk++)
   {
    ...
    */
    ...
    for(iAttemptCount = 0, wVerFlags = 0 ;
        ...
        */
        ...
        if (!*bWindowsDir  && err != FC_ERROR_LOADED_DRIVER && err != ERROR_DISK_FULL)
        {
...
{
    if ((iDisk >= 'A' && iDisk <= 'Z') || (iDisk >= 'a' && iDisk <= 'z'))
    {
