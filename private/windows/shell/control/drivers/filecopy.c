         ...
         case COPY_QUERYCOPY:
             ...
             */
             ...
             if (OpenFile(szFullPath, &ofs, OF_EXIST|OF_SHARE_DENY_NONE) >= 0)
             {
                 ...
                 *  current or new after we have queried the user once
                 */
                 ...
                 if (bQueryExist)
                 {
                     ...
                     LoadString(myInstance, IDS_DRIVER_EXISTS, szDriverExists, sizeof(szDriverExists));
                     wsprintf(szErrMsg, szDriverExists, FileName(szFile));

                    /*
                     *  Ask the user whether to copy or not ?
                     */
                     DriverCopy = DialogBox(myInstance, MAKEINTRESOURCE(DLG_EXISTS), hMesgBoxParent, wsExistDlg);
                 }
