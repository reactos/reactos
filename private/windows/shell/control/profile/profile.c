...
BOOL InitializeDialog (HWND hDlg)
{
    ...
    lResult = RegCreateKeyEx (HKEY_CURRENT_USER, szProfileRegInfo, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDisp);
...
BOOL SaveSettings (HWND hDlg)
{
    ...
    RegSetValueEx (hKey, szDontSaveList, 0, REG_SZ, (LPBYTE) lpDoNotSaveList, sizeof (TCHAR) * lstrlen (lpDoNotSaveList) + 1);

    // Write the Save list to the registry
    RegSetValueEx (hKey, szSaveList, 0, REG_SZ, (LPBYTE) lpSaveList, sizeof (TCHAR) * lstrlen (lpSaveList) + 1);
    ...
    RegSetValueEx (hKey, szSaveOnUnlisted, 0, REG_SZ, (LPBYTE) szTempBuffer, sizeof (TCHAR) * lstrlen (szTempBuffer) + 1);
