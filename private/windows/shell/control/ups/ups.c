...
void ShowError (HWND hDlg, DWORD dwError)
{
    TCHAR   szErrorMessage[LONGBZ];
    if (!FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError, 0, szErrorMessage, LONGBZ, NULL))
    {
                  ...
                  SC_HANDLE        hups)
{
    ...
    while ((pss->dwCurrentState != dwFinalState) && (i++ < max_tries))
    {
...
BOOL UPSDlg (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        CheckRadioButton(hDlg, IDD_UPS_PFSIGNALHIGH, IDD_UPS_PFSIGNALLOW, ulOptions & UPS_POWERFAIL_LOW ? IDD_UPS_PFSIGNALLOW : IDD_UPS_PFSIGNALHIGH);
        CheckRadioButton(hDlg, IDD_UPS_LOWBATTERYHIGH, IDD_UPS_LOWBATTERYLOW, ulOptions & UPS_LOWBATTERY_LOW ? IDD_UPS_LOWBATTERYLOW : IDD_UPS_LOWBATTERYHIGH);
        CheckRadioButton(hDlg, IDD_UPS_TURNOFFHIGH, IDD_UPS_TURNOFFLOW, ulOptions & UPS_TURNOFF_LOW ? IDD_UPS_TURNOFFLOW : IDD_UPS_TURNOFFHIGH);
        ...
        SendMessage(GetDlgItem(hDlg, IDD_UPS_FILENAME), EM_LIMITTEXT, PATHMAX-1, 0L);
        SendMessage(GetDlgItem(hDlg, IDD_UPS_BLEDIT), EM_LIMITTEXT, 3, 0L);
        SendMessage(GetDlgItem(hDlg, IDD_UPS_RPMEDIT), EM_LIMITTEXT, 3, 0L);
        SendMessage(GetDlgItem(hDlg, IDD_UPS_FWEDIT), EM_LIMITTEXT, 3, 0L);
        SendMessage(GetDlgItem(hDlg, IDD_UPS_WIEDIT), EM_LIMITTEXT, 3, 0L);

        // Change the window proc of the edit fields.
        lpDefWndProc = (WNDPROC)GetWindowLongPtr(GetDlgItem(hDlg, IDD_UPS_BLEDIT), GWLP_WNDPROC);
        SetWindowLongPtr (GetDlgItem(hDlg, IDD_UPS_BLEDIT), GWLP_WNDPROC, (LONG_PTR)NewEditWndProc);
        SetWindowLongPtr (GetDlgItem(hDlg, IDD_UPS_RPMEDIT), GWLP_WNDPROC, (LONG_PTR)NewEditWndProc);
        SetWindowLongPtr (GetDlgItem(hDlg, IDD_UPS_FWEDIT), GWLP_WNDPROC, (LONG_PTR)NewEditWndProc);
        SetWindowLongPtr (GetDlgItem(hDlg, IDD_UPS_WIEDIT), GWLP_WNDPROC, (LONG_PTR)NewEditWndProc);
        if (!(ulOptions & UPS_INSTALLED))
        {
    ...
    case WM_VSCROLL:
        ...
        default:
            ...
            num = ArrowVScrollProc (LOWORD(wParam), (short)num, (LPARROWVSCROLL) (avs + i));
            _itoa (num, szNum, 10);
        ...
        case IDOK:
            ...
            if (fUpsSelected && (ulOptions & UPS_POWERFAILSIGNAL))
            {
                ...
                if ((ulWarningInterval < (ULONG) avs[3].bottom) | (ulWarningInterval > (ULONG) avs[3].top))
                {
                    ...
                    SendMessage(GetDlgItem (hDlg, IDD_UPS_WIEDIT), EM_SETSEL, 0, -1L);
                    break;
                }
            }

            if (fUpsSelected && (ulOptions & UPS_POWERFAILSIGNAL) && !(ulOptions & UPS_LOWBATTERYSIGNAL))
            {
                ...
                if ((ulBatteryLife <  (ULONG) avs[0].bottom) | (ulBatteryLife > (ULONG) avs[0].top))
                {
                    ...
                    SendMessage(GetDlgItem (hDlg, IDD_UPS_BLEDIT), EM_SETSEL, 0, -1L);
                    break;
                }
                ...
                if ((ulRechargePerMinute < (ULONG) avs[1].bottom) | (ulRechargePerMinute > (ULONG) avs[1].top))
                {
                    ...
                    SendMessage(GetDlgItem (hDlg, IDD_UPS_RPMEDIT), EM_SETSEL, 0, -1L);
                    break;
                }
                ...
                if (ulFirstWarning > ulBatteryLife * 60)
                {
                    ...
                    SetFocus (GetDlgItem (hDlg, IDD_UPS_FWEDIT));
                    SendMessage(GetDlgItem (hDlg, IDD_UPS_FWEDIT), EM_SETSEL, 0, -1L);
                    break;
                }
            ...
            if (fUpsSelected && (ulOptions & UPS_COMMANDFILE))
            {
                ...
                if (!IsFilename (szFilename))
                {
                    ...
                    SendMessage(GetDlgItem (hDlg, IDD_UPS_FILENAME), EM_SETSEL, 0, -1L);
                    break;
                }
                ...
                else
                {
                    ...
                    if (hFile == INVALID_HANDLE_VALUE)
                    {
                        ...
                        if (!FormatMessage (FORMAT_MESSAGE_FROM_STRING|FORMAT_MESSAGE_ARGUMENT_ARRAY, szTemp, 0, 0, szErrorMessage, LONGBZ, (va_list *)&(pszAug)))
                        {
            ...
            RegSetValueEx(vhKey, szOptions, 0, REG_DWORD, (LPSTR)&ulOptions, sizeof(ulOptions));
            RegSetValueEx(vhKey, szBatteryLife, 0, REG_DWORD, (LPSTR)&ulBatteryLife, sizeof(ulBatteryLife));
            RegSetValueEx(vhKey, szRechargePerMinute, 0, REG_DWORD, (LPSTR)&ulRechargePerMinute, sizeof(ulRechargePerMinute));
            RegSetValueEx(vhKey, szFirstWarning, 0, REG_DWORD, (LPSTR)&ulFirstWarning, sizeof(ulFirstWarning));
            RegSetValueEx(vhKey, szWarningInterval, 0, REG_DWORD, (LPSTR)&ulWarningInterval, sizeof(ulWarningInterval));
            ...
            else if ((ss.dwCurrentState == SERVICE_RUNNING) || (ss.dwCurrentState == SERVICE_PAUSED))
            {
                RetVal = MyMessageBox (hDlg, fUpsSelected? UPS_RESTART_MSG: UPS_STOP_MSG, CPCAPTION, MB_YESNOCANCEL|MB_ICONINFORMATION);
                if (RetVal == IDCANCEL)
                {
                    break;
                }
                ...
                if (RetVal == IDYES)
                {
                    ...
                    {
                        if (MyMessageBox (hDlg, UPS_STOPFAIL_MSG, CPCAPTION, MB_OKCANCEL|MB_ICONSTOP) == IDCANCEL)
                        {
                            break;
                        }
                        else
                    ...
                    if (fUpsSelected)
                    {
                        ...
                        if (!FinishCheck (SERVICE_RUNNING, &ss, hups))
                        {
                            if (MyMessageBox (hDlg, UPS_STARTFAIL_MSG, CPCAPTION, MB_OKCANCEL|MB_ICONSTOP) == IDOK)
                            {
                                break;
                            }
                        }
            ...
            if (!ChangeServiceConfig (hups, SERVICE_NO_CHANGE, dwStart, SERVICE_ERROR_NORMAL, NULL, NULL, NULL, NULL, NULL, NULL, NULL))
            {
                dwError = GetLastError();
                return (ErrorOut (hDlg, dwError, hups, hsc, vhKey));
            }
