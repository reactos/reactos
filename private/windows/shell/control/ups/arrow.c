...
VOID ArrowTimerProc(HWND hWnd, UINT wMsg, UINT nID, DWORD dwTime)
{
    ...
    if (StateTest(dwSpinnerState, SPINNERSTATE_CLICKED))
    {
        ...
        SendMessage(hParent, WM_VSCROLL, MAKELONG(wScroll, GetWindowLong(hWnd, GWL_ID)), (LPARAM)hWnd);
    }
...
*/
{
    ...
    case WM_CANCELMODE:
        ...
        if (bArrowTimed)
        {
            SendMessage(hParent, WM_VSCROLL, MAKELONG(SB_ENDSCROLL, GetWindowLong(hArrow, GWL_ID)), (LPARAM)hArrow);
    ...
    case WM_LBUTTONDOWN:
        ...
        else
        {
            ...
            bArrowTimed = SetTimer(hArrow, GetWindowLong(hArrow, GWL_ID), 200, (TIMERPROC)ArrowTimerProc) != 0;
        }

        SendMessage(hParent, WM_VSCROLL, MAKELONG(wScroll, GetWindowLong(hArrow, GWL_ID)), (LONG_PTR)hArrow);
        break;
    case WM_MOUSEMOVE:
...
LONG SpinnerPaint(HWND hWnd, DWORD dwSpinnerState)
/*
 *  Handles all WM_PAINT messages for the control and paints the control for the current state, whether it be clicked or disabled.
