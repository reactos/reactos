...
void Draw3DButtonRect(HDC hDC, HPEN hPenHigh, HPEN hPenShadow, int x1, int y1, int x2, int y2, BOOL fClicked);
...
BOOL OddArrowWindow(HWND hArrowWnd)
{
    ...
    if (!(bResize = (rResize.right - rResize.left) % 2))
    {
        ...
        MoveWindow(hArrowWnd, rResize.left, rResize.top, (rResize.right - rResize.left), (rResize.bottom - rResize.top), FALSE);
    }
    return(bResize);
#endif  //  OLD_CODE
}
...
VOID ArrowTimerProc(HWND hWnd, UINT wMsg, UINT nID, DWORD dwTime)
{
    ...
    {
        wScroll = (StateTest(dwSpinnerState, SPINNERSTATE_DOWNCLICK)) ? SB_LINEDOWN : SB_LINEUP;
        if (bRight == WM_RBUTTONDOWN)
            wScroll += SB_PAGEUP - SB_LINEUP;

        SendMessage(hParent, WM_VSCROLL, MAKELONG(wScroll, GetWindowLong(hWnd, GWL_ID)), (LONG)hWnd);
    }
...
/*
 * Description:
 *  Calculates the rectangle of the clicked region based on the state flags SPINNERSTATE_UPCLICK and SPINNERSTATE_DOWNCLICK.
 * Parameter:
 ...
 *  State information about the control is maintained ALL drawing is handled during WM_PAINT message processing.
*/
{
    WORD    wScroll;
    POINT   pt;
    ...
    case WM_ENABLE:
        ...
        //  change-state-and-repaint strategy since we let the painting code take care of the visuals.
    ...
    case WM_CANCELMODE:
        ...
        if (bArrowTimed)
        {
            SendMessage(hParent, WM_VSCROLL, MAKELONG(SB_ENDSCROLL, GetWindowLong(hArrow, GWL_ID)), (LONG)hArrow);
    ...
    case WM_LBUTTONDOWN:
        ...
        else
        {
            ...
            bArrowTimed = SetTimer(hArrow, GetWindowLong(hArrow, GWL_ID), 200, (TIMERPROC)ArrowTimerProc);
        }

        SendMessage(hParent, WM_VSCROLL, MAKELONG(wScroll, GetWindowLong(hArrow, GWL_ID)), (LONG)hArrow);
        break;
    case WM_MOUSEMOVE:
        ...
        //  the SPINNERSTATE_MOUSEOUT flag and repaint so the button visually comes up.
    ...
    case WM_RBUTTONUP:
        ...
        if ((UINT)(bRight - WM_LBUTTONDOWN + WM_LBUTTONUP) == message)
        {
            ...
            if (StateTest(dwSpinnerState, SPINNERSTATE_CLICKED) && StateTest(dwSpinnerState, ~SPINNERSTATE_MOUSEOUT))
            {
 ...
 *  the control for the current state, whether it be clicked or disabled.
...
*/
{
    ...
    Draw3DButtonRect(hDC, rgHPen[SPINNERCOLOR_HIGHLIGHT], rgHPen[SPINNERCOLOR_SHADOW], 0, 0, cx - 1, cy2, (BOOL)xAdd1);

    Draw3DButtonRect(hDC, rgHPen[SPINNERCOLOR_HIGHLIGHT], rgHPen[SPINNERCOLOR_SHADOW], 0, cy2, cx - 1, cy - 1, (BOOL)xAdd2);
 ...
 *  is assumed to be bounded by a one pixel black border, so everything is bumped in by one.
