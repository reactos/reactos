    ...
    LPARAM  lParam)
{
    ...
    switch (uMessage)
    {
       case WM_INITDIALOG:
          ...
          RegRes = RegOpenKeyEx (HKEY_LOCAL_MACHINE, TEXT("SYSTEM\\CurrentControlSet\\Control\\PriorityControl"), 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &Hkey);
          if (RegRes == ERROR_SUCCESS)
          {
              Length = sizeof(Value);
              RegRes = RegQueryValueEx (Hkey, TEXT("Win32PrioritySeparation"), NULL, &Type, (LPBYTE) &Value, &Length);
              if (RegRes == ERROR_SUCCESS)
              {
                  switch (Value)
                  {
          ...
          switch (LOWORD(wParam))
          {
             case IDB_OK:
                ...
                if (Hkey)
                {
                    Type = REG_DWORD;
                    Length = sizeof(Value);
                    RegSetValueEx (Hkey, TEXT("Win32PrioritySeparation"), 0, REG_DWORD, (LPBYTE)&Value, Length);
                    RegCloseKey (Hkey);
                    Hkey = NULL;
                }
