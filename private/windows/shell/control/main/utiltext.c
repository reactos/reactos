...
int GetSection(LPTSTR lpFile, LPTSTR lpSection, LPHANDLE hSection, LPINT pSize)
{
    ...
    do
    {
        ...
        if (!lpFile)
            ...
            nCount = GetPrivateProfileString (lpSection, NULL, szNull, pszSect, nSize/sizeof(TCHAR), lpFile);
...
HANDLE FindRHSIni (LPTSTR pFile, LPTSTR pSection, LPTSTR pRHS)
{
    ...
    /* Must get the RHS for every LHS.  If it matches, copy LHS into buffer. */
        GetPrivateProfileString (pSection, pszSect, szNull, szRoot, CharSizeOf(szRoot), pFile);
...
int Copy (HWND hParent, TCHAR *szSrcFile, TCHAR *szDestFile)
{
    ...
    if (!RootPath (szSrcFile, szSrcPath) || !RootPath (szDestFile, szDestPath))
        ...
        if ((fhW = LZOpenFile (szDestPath, &ofstruct, OF_CREATE | OF_WRITE )) != -1)
        {
            ...
            if (cch >= 0)
            {
              HANDLE  fhR, fhW;
                ...
                GetFileTime (fhR, &CreationTime, &LastAccessTime, &LastWriteTime);
                SetFileTime (fhW, &CreationTime, &LastAccessTime, &LastWriteTime);
