    ...
    case WM_COMMAND:
        ...
        case IDOK:
                ...
                _tcsnicmp (szStartString, szBaseIO, CharSizeOf(szBaseIO)-1))
            {
                ...
                if (RegSetValueEx (hkeySerial, szRegPortAddress, 0L, REG_DWORD, (LPBYTE) &dwBaseIO, dwSize))
                    goto RegistryError;
                ...
                //  Set FIFO value
                dwSize = sizeof(DWORD);
                if (RegSetValueEx (hkeySerial, szFIFO, 0L, REG_DWORD, (LPBYTE) &dwFIFO, dwSize))
                    goto RegistryError;
                ...
                if (RegSetValueEx (hkeySerial, szDosDev, 0L, REG_SZ, (LPBYTE) szComPort, ByteCountOf(lstrlen(szComPort)+1)))
                {
                ...
                if (RegCreateKey (HKEY_LOCAL_MACHINE, szRegSerialMap, &hkey) == ERROR_SUCCESS)
                {
                    if (RegSetValueEx (hkey, szSerialKey, 0L, REG_SZ, (LPBYTE) szComPort, ByteCountOf(lstrlen (szComPort)+1)) != ERROR_SUCCESS)
                    {
                        RegCloseKey (hkey);
                        goto ComCreateError;
                    }
...
ComCreateError:
                ...
                dwSize   = sizeof(DWORD);
                if (RegSetValueEx (hkeySerial, szRegPortIRQ, 0L, REG_DWORD, (LPBYTE) &dwBaseIRQ, dwSize))
                {
RegistryError:
                    //  ERROR - Cannot set Registry value
                    MyMessageBox (hDlg, MYPORT+18, INITS+1, MB_OK | MB_ICONINFORMATION);
