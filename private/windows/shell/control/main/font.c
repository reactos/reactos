...
//  attempting to open an undecorated filename.  Under Windows NT, this is the "%systemroot%\system" directory (e.g. "d:\nt\system").
...
//  Sets the current directory to the "Shared" or "Fonts" directory before
//  attempting to Delete an undecorated filename.  Under Windows NT, this is the "%systemroot%\system" directory (e.g. "d:\nt\system").
...
BOOL APIENTRY TrueTypeDlg (HANDLE hDlg, UINT message, DWORD wParam, LONG lParam)
{
    ...
    case WM_COMMAND:
      ...
      case IDOK:
            ...
            wsprintf (szLHS,TEXT("%d"), bTrueTypeOnly = IsDlgButtonChecked (hDlg, FONT_TRUETYPEONLY));
            bTTOnlyChange = bTrueTypeOnly != (BOOL)GetProfileInt(szTrueType, szOnly,0);
            ...
            {
                DialogBoxParam(hModule,(LPTSTR) MAKEINTRESOURCE (DLG_RESTART),hDlg, (DLGPROC) RestartDlg, MAKELONG(IDS_TRUETYPECHANGE,0));
            }
#endif  // ALLOW_NO_TT
    ...
    case 1:
        ...
        if (iFontType == IF_TYPE1)
        {
            if (GetT1Install (hDlg, szLHS, szFile, szPfxFile))
            {
                ...
                //  to do a PassedInspection on a file we never did an InspectFontFile on.
...
VOID CtrlDeleteFont (HWND hDlg)
{
    ...
    for (i = 0, bYesAll = bDeleteFont = FALSE; i < nNumFonts; ++i)
    {
        ...
        if (!bYesAll)
        {
            ...
            switch (DoDialogBoxParam (DLG_REMOVEFONT, hDlg, (DLGPROC)RemoveFontDlg, IDH_DLG_REMOVEFONT, (DWORD)szLHS))
            {
            case IDNO:
...
BOOL APIENTRY FontDlg (HWND hDlg, UINT message, DWORD wParam, LONG lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        if (WideCharToMultiByte (CP_ACP, 0, szSharedDir, -1, szFontsDirA, PATHMAX, NULL, NULL) == FALSE)
        {
            //  Report error adding font resource
            MyMessageBox (hDlg, MYFONT+49, MYFONT+48, MB_OK | MB_ICONEXCLAMATION, (LPTSTR)szSharedDir);
            return (FALSE);
        }
        ...
        for (hWndTmp = GetWindow (hDlg, GW_CHILD); hWndTmp; hWndTmp = GetWindow (hWndTmp, GW_HWNDNEXT))
...
#define lpdis ((LPDIS)lParam)
        ...
        // Load sample string for each character set.
            {
                ...
                case ANSI_CHARSET :
                case SYMBOL_CHARSET :
                    LoadString(hModule, ANSI_FONTSAMPLE, szTest, CharSizeOf(szTest));
                    break;
                case SHIFTJIS_CHARSET :
                    LoadString(hModule, SJ_FONTSAMPLE, szTest, CharSizeOf(szTest));
                    break;
                case HANGEUL_CHARSET :
                    LoadString(hModule, HANGEUL_FONTSAMPLE, szTest, CharSizeOf(szTest));
                    break;
                case OEM_CHARSET :
                    LoadString(hModule, OEM_FONTSAMPLE, szTest, CharSizeOf(szTest));
                    break;
                default :
                    LoadString(hModule, ANSI_FONTSAMPLE, szTest, CharSizeOf(szTest));
                    break;
            ...
            ExtTextOut(hdc, lpdis->rcItem.left, lpdis->rcItem.top, ETO_OPAQUE, &(lpdis->rcItem), szTest, lstrlen(szTest), NULL);
    ...
    case WM_COMMAND:
        ...
        case ADDFONT:
            ...
            break;
        case FONT_TRUETYPE:
            DoDialogBoxParam (DLG_TRUETYPE, hDlg, (DLGPROC) TrueTypeDlg, IDH_DLG_TRUETYPE, 0L);
            break;
        case PUSH_OK:
        case PUSH_CANCEL:
            PassedInspection(hLogFonts, szOldFileName);
...
//  resource.  It also allows the user to have the font file deleted from the disk.
BOOL APIENTRY RemoveFontDlg (HWND hDlg, UINT nMsg, DWORD wParam, LONG lParam)
{
    TCHAR szFormat[120];
    TCHAR szTemp[120];
...
//  The following function verifies that the user wishes to delete a font
//  resource and the companion PostScript font files.  It also allows the user to have the font files deleted from the disk.
...
//  manage directories and controls for finding fonts.  The HOOK Proc for this routine is located in font3.c.
void DoAddFont(HWND hDlg)
{
    TCHAR szFilter[80];
    ...
    *szFileTemp = TEXT('\0');
    ...
    OpenFileName.Flags = OFN_HIDEREADONLY | OFN_ENABLEHOOK | OFN_SHOWHELP | OFN_ENABLETEMPLATE;
