...
static BOOL ParsePageFileDesc(LPTSTR *ppszDesc, INT *pnDrive, INT *pnMinFileSize, INT *pnMaxFileSize, LPTSTR *ppstr)
{
    LPTSTR psz;
    LPTSTR pszName = NULL;
    int cFields;
    ...
    )
{
    ...
    if (nBootPF != 0) {
        ...
        if (nBootPF == -1) {
            ...
            if ((nBootPF * ONE_MEG - ms.dwTotalPhys) < si.dwPageSize) {
                nBootPF += 1;
            }
        }
    }
        ...
        case IDOK: {
            ...
            if (cMegBootPF != 0) {
                ...
                if (cMegBootPF == -1) {
                    ...
                    // If the slop at the end is less than one page, then make it bigger.
                    GetSystemInfo(&si);
                    if ((cMegBootPF * ONE_MEG - ms.dwTotalPhys) < si.dwPageSize) {
                        cMegBootPF += 1;
                    }
                }
