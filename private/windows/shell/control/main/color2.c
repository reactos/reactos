...
void EraseCrossHair(HDC hDC)
{
    ...
    topy = ((DWORD)rRainbow.top > nSatPos - distancey) ? (DWORD)rRainbow.top : nSatPos - distancey;
    bottomy = ((DWORD)rRainbow.bottom < nSatPos + distancey) ? (DWORD)rRainbow.bottom : nSatPos + distancey;
    leftx = ((DWORD)rRainbow.left > nHuePos - distancex) ? (DWORD)rRainbow.left : nHuePos - distancex;
    rightx = ((DWORD)rRainbow.right < nHuePos + distancex) ? (DWORD)rRainbow.right : nHuePos + distancex;

    hOldBitmap = SelectObject(hDCBits, hRainbowBitmap);
    BitBlt(hDC, leftx, topy, rightx - leftx + 1, bottomy - topy + 1, hDCBits, leftx - (DWORD)rRainbow.left, topy - (DWORD)rRainbow.top, SRCCOPY);
...
void CrossHairPaint(HDC hDC, DWORD x, DWORD y)
{
    ...
    bottomy = ((DWORD)rRainbow.bottom < y + 2 * distancey) ? (DWORD)rRainbow.bottom : y + 2 * distancey;
    leftx = ((DWORD)rRainbow.left > x - 2 * distancex) ? (DWORD)rRainbow.left : x - 2 * distancex;
    rightx = ((DWORD)rRainbow.right < x + 2 * distancex) ? (DWORD)rRainbow.right : x + 2 * distancex;
    topy2 = ((DWORD)rRainbow.top > y - distancey) ? (DWORD)rRainbow.top : y - distancey;
    bottomy2 = ((DWORD)rRainbow.bottom < y + distancey) ? (DWORD)rRainbow.bottom : y + distancey;
...
BOOL APIENTRY RainbowDlg(HWND hWnd, UINT message, DWORD wParam, LONG lParam)
{
    ...
    case WM_LBUTTONDOWN:
        ...
        else if (PtInRect(&rLumPaint, Point) || PtInRect(&rLumScroll, Point))
        {
            SetupRainbowCapture(hWnd);
            bUpdateExample = TRUE;
    ...
    case WM_KEYDOWN:
        ...
        case VK_UP:
            if (currentSat < RANGE - 1)
            {
                ...
                if (currentSat > (WORD)(2 * (bHighLum ? RANGE - currentLum : currentLum)))
                {
                    if (bHighLum)
                    {
                        currentLum--;
...
void RainbowPaint(HDC hDC, LPRECT lpPaintRect)
{
    ...
    if (IntersectRect(&Rect, lpPaintRect, &rLumPaint))
    {
        ...
        for (Lum = LUMINC; Lum < RANGE; Lum += LUMINC)
        {
            ...
            Rect.top = (short)(((rLumPaint.bottom + LUMINC / 2) * (DWORD)RANGE - (Lum + LUMINC) * nLumHeight) / RANGE);
        ...
        if (!EqualRect(lpPaintRect, &rLumPaint))
        {
            ...
            Rectangle(hDC, rLumPaint.left - 1, rLumPaint.top - 1, rLumPaint.right + 1, rLumPaint.bottom + 1);
