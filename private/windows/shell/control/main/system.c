...
void SetGenLBWidth (HWND hwndLB, LPTSTR szBuffer, LPDWORD pdwWidth, HANDLE hfontNew, DWORD cxExtra);
...
BOOL GetRGSZEnvVar(CPEnvBuf * pEnvBuf, PCHAR pszName)
{
    ...
    cb = fpGetNVRAMvar (rgtmp, (USHORT)1, rgchOut, (USHORT) CP_MAX_ENV);
    ...
    case WM_INITDIALOG:
...
#ifdef JAPAN
        hfont = CreateFont (GetHeightFromPointsString (sz10), 0, 0, 0, 0, 0, 0, 0, SHIFTJIS_CHARSET, 0, 0, 0, 0, szSystemFont);
        hfontBold = CreateFont (GetHeightFromPointsString (sz10), 0, 0, 0, 700, 0, 0, 0, SHIFTJIS_CHARSET, 0, 0, 0, 0, szSystemFont);
#else
        hfont = CreateFont (GetHeightFromPointsString (sz8), 0, 0, 0, 400, 0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, szFont);
        hfontBold = CreateFont (GetHeightFromPointsString (sz8), 0, 0, 0, 700, 0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, szFont);
#endif

        SendDlgItemMessage (hDlg, IDD_SYS_OS, WM_SETFONT, (WPARAM) hfont, MAKELPARAM(TRUE, 0));
        SendDlgItemMessage (hDlg, IDD_SYS_LB_USERVARS, WM_SETFONT, (WPARAM) hfont, MAKELPARAM(TRUE, 0));
        SendDlgItemMessage (hDlg, IDD_SYS_LB_SYSVARS, WM_SETFONT, (WPARAM) hfont, MAKELPARAM(TRUE, 0));
        SendDlgItemMessage (hDlg, IDD_SYS_VAR, WM_SETFONT, (WPARAM) hfont, MAKELPARAM(TRUE, 0));
        SendDlgItemMessage (hDlg, IDD_SYS_VALUE, WM_SETFONT, (WPARAM) hfont, MAKELPARAM(TRUE, 0));
        ...
        if (hmodSetupDll = LoadLibrary(TEXT("setupdll")))
        {
            if (fpGetNVRAMvar = (GETNVRAMPROC)GetProcAddress(hmodSetupDll, "GetNVRAMVar"))
            {
                ...
                {
                    for (iTemp = 0; iTemp < CPEBOSLoadIdentifier.cEntries; iTemp++)
                        n = SendMessageA (hwndTemp, CB_ADDSTRING, 0, (LPARAM)CPEBOSLoadIdentifier.pszVars[iTemp]);
                   ...
                   if (GetRGSZEnvVar (&cpebAutoLoad, "AUTOLOAD"))
                   {
                      if (!lstrcmpiA (cpebAutoLoad.pszVars[0], "yes"))
                      {
                         ...
                         if (GetRGSZEnvVar (&cpebTimeout, "COUNTDOWN"))
                            SetDlgItemInt (hDlg, IDD_SYS_SECONDS, atoi (cpebTimeout.pszVars[0]), FALSE);
                      }
                   }
...
//        szBootIni[0] = GetBootDrive();
        ...
        n = GetPrivateProfileString (szBootLdr, NULL, NULL, szTemp2, CharSizeOf(szTemp2), szBootIni);
        if (n != 0)
            pszBoot = szBootLdr;
        else
        {
            n = GetPrivateProfileString (szFlexBoot, NULL, NULL, szTemp2, CharSizeOf(szTemp2), szBootIni);
            if (n != 0)
                pszBoot = szFlexBoot;
            else
            {
                n = GetPrivateProfileString (szMultiBoot, NULL, NULL, szTemp2, CharSizeOf(szTemp2), szBootIni);
                if (n != 0)
                {
                    pszBoot = szMultiBoot;
                }
                ...
                {
                    //  This final case is here because I want to DEFAULT to "[boot loader]" as the section name to use in
                    //  the event that we do not find any section in the boot.ini file.
                    pszBoot = szBootLdr;
                }
            }
        }
        ...
        szTemp2[0] = TEXT('\0');
        GetPrivateProfileString (pszBoot, szDefault, NULL, szTemp2, CharSizeOf(szTemp2), szBootIni);

        //  Display all choices under [operating system] in boot.ini file in combobox for selection
        hwndTemp = GetDlgItem (hDlg, IDD_SYS_OS);
        selection = -1;
...
ContinueSystemInit:
        ...
        if (hkeyEnv)
        {
                                 ...
                                 &dwValz))  // Size of ValueData buffer
            {
                ...
                //  Clip length of returned Environment variable string to MAX_VALUE_LEN-1, as necessary.
        ...
        n = LoadString (hModule, SYSTEM+1, szTemp, CharSizeOf(szTemp));
        ...
        else
        {
            ...
            if (MyMessageBox (hDlg, SYSTEM+8, INITS+1, MB_OKCANCEL | MB_ICONEXCLAMATION) == IDCANCEL)
            {
        ...
        case IDD_SYS_VMEM:
            {
                ...
                dwSave = dwContext;
                dwContext = IDH_DLG_VIRTUALMEM;

                //  Virtual memory dialog box changes registry values on IDOK

                dw = DialogBox (hModule, (LPTSTR) MAKEINTRESOURCE(DLG_VIRTUALMEM), hDlg, (DLGPROC) VirtualMemDlg);
                if (dw != RET_NO_CHANGE)
                {
        ...
        case IDD_SYS_COREDUMP:
            {
                ...
                dwSave = dwContext;
                dwContext = IDH_DLG_COREDUMP;

                //  Recovery dialog box changes registry values on IDOK

                dw = DialogBox (hModule, (LPTSTR) MAKEINTRESOURCE(DLG_COREDUMP), hDlg, (DLGPROC) CoreDumpDlg);
                if (dw != RET_NO_CHANGE)
                {
                     SetDlgItemText(hDlg, IDCANCEL, pszClose);
                     dwRestartSystem |= dw;
        ...
        case IDD_SYS_TASKING:
          {
            ...
            dwSave = dwContext;
            dwContext = IDH_DLG_TASKING;

            //  Tasking memory dialog box changes registry values on IDOK

            i = DialogBox (hModule, (LPTSTR) MAKEINTRESOURCE(DLG_TASKING), hDlg, (DLGPROC) TaskingDlg);
            if (i)
                SetDlgItemText (hDlg, IDCANCEL, pszClose);
            dwContext = dwSave;
            break;
...
ChangeVariables:
            ...
            {
                //  Clear the selection from the other listbox so the user doens't have to figure out which one he is editing
                SendDlgItemMessage (hDlg, (LOWORD(wParam) == IDD_SYS_LB_USERVARS) ? IDD_SYS_LB_SYSVARS : IDD_SYS_LB_USERVARS, LB_SETCURSEL, (WPARAM) -1, 0);
            }

            selection = SendMessage (hwndTemp, LB_GETCURSEL, 0, 0L);
            if (selection != LB_ERR)
            {
                penvar = (ENVARS *)SendMessage (hwndTemp, LB_GETITEMDATA, selection, 0L);
            ...
            else
            {
                ...
                SetDlgItemText (hDlg, IDD_SYS_VAR, szNull);
                SetDlgItemText (hDlg, IDD_SYS_VALUE, szNull);
        ...
        case IDD_SYS_DELUV:
            ...
            hwndTemp = GetDlgItem (hDlg, bUserVars ? IDD_SYS_LB_USERVARS : IDD_SYS_LB_SYSVARS);
...
BootIniWriteError:
                            bOK = FALSE;
                            MyMessageBox (hDlg, SYSTEM+19, INITS+1, MB_OK | MB_ICONINFORMATION);
                        }
                ...
                if (bOK)
                {
                    ...
                    if (selection != nOriginalSelection)
                    {
                        ...
                        SendMessage (hwndTemp, CB_GETLBTEXT, selection, (LONG) (LPTSTR) szTemp);
                        ...
                        if (n != LB_ERR)
                        {
                            ...
                            {
                                //  Skip the User's selection since we got it above.
                                if (iTemp == selection)
                                    continue;

                                pszTemp = (LPTSTR) SendMessage (hwndTemp,CB_GETITEMDATA,iTemp, 0L);
                                SendMessage (hwndTemp, CB_GETLBTEXT, iTemp, (LONG) (LPTSTR) szTemp);
            ...
            if (RegOpenKeyEx (HKEY_CURRENT_USER, szUserEnv, 0, KEY_READ | KEY_WRITE, &hkeyEnv) == ERROR_SUCCESS)
            {
                ...
                if ((n = SendMessage (hwndTemp, LB_GETCOUNT, 0, 0L)) != LB_ERR)
                {
                    for (i = 0; i < n; i++)
                    {
                        penvar = (ENVARS *) SendMessage (hwndTemp,LB_GETITEMDATA,i, 0L);
                        if (RegSetValueEx (hkeyEnv,penvar->szValueName,0L,penvar->dwType,(LPBYTE) penvar->szValue,ByteCountOf(lstrlen (penvar->szValue)+1)))
                        {
                            //  Report error trying to set registry values
                            if (MyMessageBox (hDlg, SYSTEM+9, INITS+1, MB_OKCANCEL | MB_ICONEXCLAMATION) == IDCANCEL)
                                break;
                        }
                    }
                }
            ...
            else
            {
                ...
                if (MyMessageBox (hDlg, SYSTEM+21, INITS+1, MB_OKCANCEL | MB_ICONEXCLAMATION) == IDCANCEL)
                    goto SystemFreeAndExit;
            }
...
SystemFreeAndExit:
            ...
            if (dwRestartSystem & ~RET_CHANGE_NO_REBOOT)
            {
                ...
                DialogBoxParam(hModule, (LPTSTR) MAKEINTRESOURCE(DLG_RESTART), hDlg, (DLGPROC)RestartDlg, dwMsg);
            }
            EndDialog (hDlg, 0L);
            HourGlass (FALSE);
            break;
...
int FindVar (HWND hwndLB, LPTSTR szVar)
{
    ...
    //  Try for a quick out, check szVar against itemdata->szValueName of current selection
    ...
    //  Only check those items which at least start with the same chars and start search at beginning of listbox
    control = sel = -1;
...
void SetLBWidth (HWND hwndLB, LPTSTR szBuffer, DWORD dwListBox)
{
    SetGenLBWidth (hwndLB, szBuffer, (dwListBox == LB_SYSVAR) ? &dwSysWidth : &dwUserWidth, hfont, ARBITRARY_USER_LB_EXTENT_ADDITION);
}
...
NOTE:  This routine is no longer used.  It has been superseeded by
       some routines from SETUP that do a better job of trying to
       find the system partition on X86 machines.  See syspart.c.



//  GetBootDrive()

//  Purpose:    Determines which drive letter Windows NT was
//              boot from by looking at the defined DOS devices

//  Parameters:  void


//  Return:     (TCHAR) Drive letter if successful
//                      0 if an error occurs

//  Comments:    This function can be compiled for Unicode or ANSI



TCHAR GetBootDrive (void)
{
    LPTSTR   lpDevices, lpDeviceName, lpArcName;
    LPTSTR   lpDev, lpArc;
    TCHAR    chResult = TEXT('c');
    TCHAR    szArcName[] = TEXT("\\Device\\Harddisk0\\Partition1");

    // Allocate space for all of the DOS Devices
    if (!(lpDevices = LocalAlloc (LPTR, MAX_DEVICES * sizeof(TCHAR))))
    {
       return (chResult);
    }
    ...
    // Allocate space for the arc name of the drive letters
    if (!(lpArcName = LocalAlloc (LPTR, MAX_ARC_NAME * sizeof (TCHAR))))
    {
       goto Exit_Devices;
    }
    ...
    // Query for all of the DOS Devices.  The buffer will contain a list
    // of names each separated by a NULL with a double NULL at the end.
    if (!QueryDosDevice (NULL, lpDevices, MAX_DEVICES))
    {
       goto Exit_ArcName;
    }

    // Setup the inital pointer
    lpDeviceName = lpDevices;

    // Loop through all the strings looking for \Device\Harddisk0\Partition1
    while (*lpDeviceName)
    {
        // Look at the second character of the string to see if it is a colon.  If so, then we are working with a
        // drive letter and we need to call QueryDosDevice on it.
        if (lpDeviceName[1] == TEXT(':'))
        {
           if (!QueryDosDevice (lpDeviceName, lpArcName, MAX_ARC_NAME))
           {
              goto Exit_ArcName;
           }

           // If the device name found above matches with
           // \Device\Harddisk0\Partition1 we have found the
           // boot drive.  Save the drive letter and exit.
           if (!lstrcmpi (lpArcName, szArcName))
           {
              chResult = lpDeviceName[0];
              goto Exit_ArcName;
           }
        }

        // Find terminating NULL and place pointer on the next character after it.
        while (*lpDeviceName++);
    }
...
BOOL SetNewEnvVariables (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{

    //  Delete all old USER environment variables
    ...
    if (RegOpenKeyEx (HKEY_CURRENT_USER, szUserEnv, 0, KEY_READ | KEY_WRITE, &hkeyEnv) == ERROR_SUCCESS)
    {
        ...
        //  Delete all values of type REG_SZ under key
...
//  Update all of the strings in the Environment Variables listbox by
//  re-expanding them.  This will let the User see the effect of any
//  changes immediately.


BOOL UpdateEnvStrings (HWND hDlg, PVOID pEnvBlock, DWORD dwListBox)
{
    TCHAR   szTemp2[PATHMAX];
    TCHAR  *bBuffer;
    LPTSTR  pszString;
    ...
    bUser = (dwListBox == IDD_SYS_LB_USERVARS);
    ...
    //  Create our own environment from what User has set currently
    pEnvBlock = CreateEnvBlock (hDlg);

    //  Determine which Listbox is active (SYSTEM or USER vars)
    hwndTemp = GetDlgItem (hDlg, dwListBox);

    //  Get count of items in listbox
    if ((n = SendMessage (hwndTemp, LB_GETCOUNT, 0, 0L)) != LB_ERR)
    {
        //  Allocate an array for ItemData ptrs

        iEnv = n * sizeof(penvar);
        penvar = (ENVARS *) AllocMem (iEnv);

        for (i = 0; i < n; i++)
        {
            penvar[i] = (ENVARS *) SendMessage (hwndTemp, LB_GETITEMDATA, i, 0L);
        }

        //  Clear everything from listbox...
        SendMessage (hwndTemp, WM_SETREDRAW, FALSE, 0L);
        SendMessage (hwndTemp, LB_RESETCONTENT, FALSE, 0L);

        //  And update it with new, expanded environment strings

        for (i = 0; i < n; i++)
        {
            FreeStr (penvar[i]->szExpValue);

//+++ TODO: Call RtlExpandEnvironmentString (pEnvBlock, penvar[i]->Value, pszString, BUFZ);

            ExpandEnvironmentStrings (penvar[i]->Value, pszString, BUFZ);
            penvar[i]->szExpValue  = AllocStr (pszString);

            wsprintf (bBuffer, TEXT("%s = %s"), penvar[i]->szValueName, pszString);
            k = SendMessage (hwndTemp, LB_ADDSTRING, 0, (LPARAM)bBuffer);
            SendMessage (hwndTemp, LB_SETITEMDATA, k, (LPARAM)penvar[i]);

            SetLBWidth (hwndTemp, bBuffer, bUser ? LB_USERVAR : LB_SYSVAR);
        }

        SendMessage (hwndTemp, WM_SETREDRAW, TRUE, 0L);
...
PVOID CreateEnvBlock (HWND hDlg)
{
    TCHAR  *bBuffer;
    HWND    hwndTemp;
    int     i;
    int     iMemEnv;
    int     iMemUser, iMemSys;
    int     iCountSys, iCountUser;

    ENVARS *psysenv = NULL;
    ENVARS *pusrenv = NULL;
    PVOID   pTemp, pEnvBlock = NULL;

    bBuffer = (TCHAR *) AllocMem (ByteCountOf(BUFZ));


    //  See how much space I need for the environment string block


    //  System env variables - get count of items in listbox

    hwndTemp = GetDlgItem (hDlg, IDD_SYS_LB_SYSVARS);

    if ((iCountSys = SendMessage (hwndTemp, LB_GETCOUNT, 0, 0L)) != LB_ERR)
    {
        //  Allocate an array for ItemData ptrs

        iMemSys = iCountSys * sizeof(psysenv);
        psysenv = (ENVARS *) AllocMem (iMemSys);

        for (i = 0; i < iCountSys; i++)
        {
            psysenv[i] = (ENVARS *) SendMessage (hwndTemp, LB_GETITEMDATA, i, 0L);
        }

        //  Calculate memory needed for environment

        //  Allowance for Double-NULL termination of environment block
        iMemEnv = sizeof(TCHAR);

        for (i = 0; i < iCountSys; i++)
        {
            //  Size of Variable name plus " = " plus terminating NULL
            iMemEnv += lstrlen (psysenv[i]->szValueName) + 4 * sizeof(TCHAR);

            //  Size of Value plus terminating NULL
            iMemEnv += lstrlen (psysenv[i]->szValue) + 1 * sizeof(TCHAR);
        }
    }

    //  User env variables - get count of items in listbox

    hwndTemp = GetDlgItem (hDlg, IDD_SYS_LB_USERVARS);

    if ((iCountUser = SendMessage (hwndTemp, LB_GETCOUNT, 0, 0L)) != LB_ERR)
    {
        //  Allocate an array for ItemData ptrs

        iMemUsr = iCountUser * sizeof(pusrenv);
        pusrenv = (ENVARS *) AllocMem (iMemSys);

        for (i = 0; i < iCountUser; i++)
        {
            pusrenv[i] = (ENVARS *) SendMessage (hwndTemp, LB_GETITEMDATA, i, 0L);
        }
        ...
        {
            //  Size of Variable name plus " = " plus terminating NULL
            iMemEnv += lstrlen (pusrenv[i]->szValueName) + 4 * sizeof(TCHAR);

            //  Size of Value plus terminating NULL
            iMemEnv += lstrlen (pusrenv[i]->szValue) + 1 * sizeof(TCHAR);
        }
    }
    ...
    //  Get some memory - NOTE: Caller frees this buffer
    pEnvBlock = pTemp = LocalAlloc (LPTR, iMemEnv);

    for (i = 0; i < iCountSys; i++)
    {
        wsprintf (bBuffer, TEXT("%s = %s"), psysenv[i]->szValueName, psysenv[i]->szValue);
        lstrcpy ((LPTSTR)pTemp, bBuffer)
        pTemp = (lstrlen (pTemp) + 1) * sizeof(TCHAR);
    }

    for (i = 0; i < iCountUser; i++)
    {
        wsprintf (bBuffer, TEXT("%s = %s"), pusrenv[i]->szValueName, pusrenv[i]->szValue);
        lstrcpy ((LPTSTR)pTemp, bBuffer)
        pTemp = (lstrlen (pTemp) + 1) * sizeof(TCHAR);
    }

    //  Double-NULL terminate Environment block
    *pTemp = '\0';

    if (psysenv)
        FreeMem (psysenv, iMemSys);

    if (pusrenv)
        FreeMem (pusrenv, iMemUser);
