...
void DateTimeInit(HWND   hDlg, WORD   nBaseID, WORD   nSepID, TCHAR *pszSep, int    nMaxDigitWidth, BOOL   bDate);
...
/* Determine the widest digit (safety against variable pitch fonts) */
{
    ...
    for (nMaxNumWidth = nNumWidth[0]; pNumWidth < (int *) (nNumWidth + 10); pNumWidth++)
    {
...
BOOL  bDate;
{
    ...
    if (bDate)
    {
        i = GetLocaleValue (0, LOCALE_IDATE, szTemp, CharSizeOf(szTemp), TEXT("0"));
    }
    else
    {
        if (!(bAMPM = (BOOL) GetLocaleValue (0, LOCALE_ITIME, szTemp, CharSizeOf(szTemp), TEXT("0"))))
        {
            bPrefix = GetLocaleValue (0, LOCALE_ITIMEMARKPOSN, szTemp, CharSizeOf(szTemp), TEXT("0"));
            GetLocaleValue (0, LOCALE_S1159, sz1159, CharSizeOf(sz1159), IntlDef.s1159);
            GetLocaleValue (0, LOCALE_S2359, sz2359, CharSizeOf(sz2359), IntlDef.s2359);
        }
        i = 0;
    }
    ...
    //  Find the correct window for proper order for date controls and for window positioning
    switch (nBaseID)
    {
        case DATETIME_HOUR:
    ...
    if (bDate)
    {
        SetWindowPos (hOrder[0], hwndTemp, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOREDRAW);
        SetWindowPos (hOrder[2], hOrder[0], 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOREDRAW);
        SetWindowPos (hOrder[4], hOrder[2], 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOREDRAW);
    }
...
// and the user must explicitly set the desired Date and Time.
            ...
            bLZero[HOUR] = GetLocaleValue (0, LOCALE_ITLZERO, szTemp, CharSizeOf(szTemp), TEXT("0"));
            GetLocaleValue (0, LOCALE_SSHORTDATE, szShortDate, CharSizeOf(szShortDate), IntlDef.sShortDate);
            ReadShortDate (szShortDate, bLZero + MONTH, bLZero + DAY, bLZero + YEAR);

            GetLocaleValue (0, LOCALE_SDATE, szNum, CharSizeOf(szNum), IntlDef.sDateSep);
            DateTimeInit (hDlg, DATETIME_MONTH, DATETIME_DSEP1, szNum, nMaxDigitWidth, TRUE);
#ifdef LATER
            //  Setup timezone date fields
            DateTimeInit (hDlg, IDD_TZ_SD_MONTH, IDD_TZ_SD_SEP1, szNum, nMaxDigitWidth, TRUE);
            DateTimeInit (hDlg, IDD_TZ_ED_MONTH, IDD_TZ_ED_SEP1, szNum, nMaxDigitWidth, TRUE);
#endif   // LATER

            GetLocaleValue (0, LOCALE_STIME, szNum, CharSizeOf(szNum), IntlDef.sTime);
            ...
            switch (LOWORD(wParam))
            {
                ...
                default:
                    if (nDelta == DATETIME_AMPM)
                    {
                        ...
                        hWndAMPM = GetFocus ();
                        SendMessage (hWndAMPM, LB_SETCURSEL, 1 - (WORD)SendMessage (hWndAMPM, LB_GETCURSEL, 0, 0L), 0L);
                         return (TRUE);
                     }
                     else
                     {
                        ...
                        wDateTime[i] = ArrowVScrollProc (LOWORD(wParam), (short)nNewNum, (LPARROWVSCROLL) (avs + i));
        ...
        case WM_TIMER:
            if (wTimerOn && (wParam == SECOND))
            {
                ...
                for (i = 0; i < 6; i++)
                {
                    if ((wDateTime[i] != wPrevDateTime[i]) && (GetFocus () != GetDlgItem (hDlg, DATETIME_HOUR + i)))
                    {
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case DATETIME_AMPM:
                {
                    ...
                    {
                        case LBN_SETFOCUS:
                            SendMessage ((HWND) lParam, LB_SETCURSEL, (WORD)SendMessage ((HWND) lParam, LB_GETTOPINDEX, 0, 0L), 0L);
                            break;
                        case LBN_KILLFOCUS:
                            SendMessage ((HWND)lParam, LB_SETCURSEL, (WPARAM)(LONG)-1, 0L);
                            bPM = (BOOL)SendMessage ((HWND)lParam, LB_GETTOPINDEX, 0, 0L);
                ...
                case IDD_TZ_DAYLIGHT:
                {
                    ...
                    if (iTimeZone != CB_ERR)
                    {
                        ptzi = (PAPPLET_TIME_ZONE_INFORMATION) SendMessage (hwndCB, CB_GETITEMDATA, (WPARAM)iTimeZone, 0L);
...
KillTheTimer:
                    ...
                    {
                        ptzi = (PAPPLET_TIME_ZONE_INFORMATION) SendMessage (hwndCB, CB_GETITEMDATA, (WPARAM)j, 0L);
                        if (ptzi)
...
//  This function initializes everything to do with the Timezones
{
    ...
    if ((CurrentTzi.StandardDate.wMonth == 0) || (CurrentTzi.DaylightDate.wMonth == 0))
    {
        CheckDlgButton (hDlg, IDD_TZ_DAYLIGHT, 0);
...
QuickOut:
    ...
    ptziOriginal = (PAPPLET_TIME_ZONE_INFORMATION) SendMessage (hwndCB, CB_GETITEMDATA, (WPARAM)iOriginalTimeZone, 0L);
...
//  Retrieve Time Zone information from registry
{
    ...
    while (RegEnumKey (hkey, i++, szTimeZone, CharSizeOf(szTimeZone)) != ERROR_NO_MORE_ITEMS)
    {
        hkeySub = NULL;
                ...
                == ERROR_SUCCESS)
        {
            ...
            if ((RegQueryValueEx (hkeySub, pszTZDisplayName, 0L, &dwType, (LPBYTE) ptzi->szDisplayName, &dwSize)) != ERROR_SUCCESS)
            {
                goto TryNextSubKey;
            }
            ...
            if ((RegQueryValueEx (hkeySub, pszTZDaylightName, 0L, &dwType, (LPBYTE) ptzi->szDaylightName, &dwSize)) != ERROR_SUCCESS)
            {
            ...
            if ((RegQueryValueEx (hkeySub, pszTZI, 0L, &dwType, (LPBYTE) &ptzi->Bias, &dwSize)) != ERROR_SUCCESS)
            {
                goto TryNextSubKey;
            }
            ...
            if (hDlg)
            {
                if ((j = (int)SendDlgItemMessage (hDlg, IDD_TZ_TIMEZONES, CB_ADDSTRING, (WPARAM) 0, (LPARAM) ptzi->szDisplayName)) >= 0)
                {
                    SendDlgItemMessage (hDlg, IDD_TZ_TIMEZONES, CB_SETITEMDATA, j, (LPARAM) ptzi);
                }
            }
            else
            {
...
// Then halftone the WORLD bitmap into it
    ...
    if (hdcTZmap = CreateCompatibleDC (hdc))
    {
        ...
        if (hbmTZmap = CreateCompatibleBitmap (hdc, rc.right, rc.bottom))
        {
            ...
            StretchDIBits (hdcTZmap, 0, 0, rc.right, rc.bottom, 0, 0, lpBitmapInfo->biWidth, lpBitmapInfo->biHeight, lpBits, lpBitmapInfo, DIB_RGB_COLORS, SRCCOPY);
        }
        else
            return FALSE;
    }
...
//  regions using stretchblt and halftoning into the dialog frame area from the memory bitmap.
{
    RECT        rect;
...
// Then halftone the WORLD bitmap into it
    ...
    StretchBlt (hdcTZmap, 0, 0, rc.right, rc.bottom, hdcMem,   0, 0, iWidth,   iHeight, SRCCOPY);
...
LONG PaintWorld (HWND hwndWorld, HDC hdcWorld)
{
    ...
    StretchBlt (hdcWorld, 0, 0, rect.right, rect.bottom, hdcMem, 0, 0, iWidth, iHeight, SRCCOPY);
