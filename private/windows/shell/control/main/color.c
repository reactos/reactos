        ...
        case COLOR_REMOVE:
            ...
            *pszScan = TEXT('\0');
            ...
            else if (RemoveMsgBox (hWnd, szNew, REMOVEMSG_COLOR))
            {
                ...
                GetPrivateProfileString (szCurrent, szColorSchemes, szNull, szTemp, CharSizeOf(szTemp),  szCtlIni);
                if (!lstrcmpi (szNew, szTemp))
                    WritePrivateProfileString (szCurrent, szColorSchemes, szNull, szCtlIni);
                ...
                if ((temp = (short)SendDlgItemMessage (hWnd, COLOR_SCHEMES, CB_DELETESTRING, (LONG)id, 0L))
                     == CB_ERR)
                    id = 0;
...
// in the Schemes combobox.
    ...
    {                   // there's a RHS here
        i = lstrlen (szTemp);
        GetPrivateProfileString (szColorSchemes, szTemp, szNull, (szTemp + i + 1), CharSizeOf(szTemp) - i - 1, szCtlIni);
        szTemp[i] = TEXT('=');
...
DWORD FillFromControlIni (HWND  hWnd, LPTSTR pszSection)
{
    ...
    do
    {
        nCount = GetPrivateProfileString (pszSection, NULL, szNull, pszItem, nSize, szCtlIni);
        if (nCount >= nSize)
        {
            LocalUnlock (hLocal);
    ...
    while (*pszItem)
    {
        ...
        GetPrivateProfileString (pszSection, pszItem, szNull, (szItem + nSize + 1), CharSizeOf(szItem) - nSize - 1, szCtlIni);
