...
InstallKeybd:
    ...
    if (nID == INTL_LANGUAGE)
    {
        ...
        {
            RegSetValueEx (hkey, szRegLocale, 0L, REG_SZ, (LPBYTE)pszOption, ByteCountOf(lstrlen(pszOption) + 1));
            RegCloseKey (hkey);
        }
    }
    ...
    LPTSTR pszOption)
{
    ...
    if (!RegOpenKeyEx (HKEY_LOCAL_MACHINE, pszRegKey, 0L, KEY_READ, &hkeyOptions))
    {
        ...
        while (!RegEnumValue (hkeyOptions, dwIndex++, szValueName, &dwBufz, NULL, NULL, (LPBYTE) szValue, &dwBufv))
        {
