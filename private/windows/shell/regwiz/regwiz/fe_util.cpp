...
//  if so it pFeType as FE country and gives the corrosponding screen type in pFeScrType
DWORD MapCountryLcidWithFETable(DWORD dwCurCountry, FeCountriesIndex *pFeType, FeScreenType      *pFeScrType)
{
    DWORD dwReturn;
    int iIndex;
    dwReturn = 0;
...
//    This fucntion gets the current LCID of the system uisng GetSystemDefaultLCID().
FeCountriesIndex IsFarEastCountry(HINSTANCE hIns)
{
    ...
    if (gWhatFECountry == kNotInitialised) {
        ...
        MapCountryLcidWithFETable(lcRet, &gWhatFECountry, &gWhichFEScreenTye);
    }
    else {
        ;
    }
