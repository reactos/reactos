    ...
    )
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            SendDlgItemMessage(hDlg, IDC_USERNAME, WM_GETTEXT, 255, (LPARAM) szInfo);
            _tcscpy(sPrInf.m_User, szInfo);

            SendDlgItemMessage(hDlg, IDC_PASSWORD, WM_GETTEXT, 255, (LPARAM) szInfo);
...
DWORD_PTR GetProxyAuthenticationInfo(HINSTANCE hIns, TCHAR *czProxy, TCHAR *czUserName, TCHAR *czPswd)
{
    DWORD_PTR dwRet;
    ...
    dwRet = DialogBoxParam(hIns, MAKEINTRESOURCE(IDD_PROXY_LOG), NULL, DisplayProxyAuthentication, (LPARAM) hIns);
    if (dwRet == -1) {
