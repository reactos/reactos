...
BOOL Ping(LPSTR szIPAddress)
{
    ...
    if (pIpe)
    {
        ...
        dwStatus = pIcmpSendEcho(hIP, *dwIPAddr, szBuffer, 64, NULL, pIpe, sizeof(IP_ECHO_REPLY) + 64, PING_TOUT);
        if (dwStatus)
        {
            bRet = TRUE;
        }
...
//  This function returns Calls the gethostbyname and returns 0 if Successful and 1 if failure the return
int ResolveHostByThread(LPSTR pHost)
{
    int   iRet = 0;
...
DWORD GetHostByAddrThread(void *vp)
{
    ...
    gphostent = gethostbyaddr(szAddr, 4, PF_INET);
    if (!gphostent)
    {
        iError = WSAGetLastError();
        if (iError)
...
//  This function returns Calls the gethostbyaddr and returns 0 if Successful and 1 if failure the return
int ResolveHostByAddrThread(LPSTR pHost)
{
    int   iRet = 0;
    DWORD  dwThreadExitCode;
    ...
    HANDLE hParent = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) GetHostByAddrThread, (void *) pHost, dwCreationFlags, &ThreadId);
    DWORD dwRet = WaitForSingleObject(hParent, GET_HOST_TOUT);
    switch (dwRet)
    {
        case WAIT_ABANDONED:
