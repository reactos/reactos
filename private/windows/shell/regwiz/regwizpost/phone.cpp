...
//  This function expands and substitutes Environment variable in input string
...
TCHAR * DUNFileProcess::GetDunFileRoot()
{
    ...
    dwInfoSize = 256;
    regStatus = RegOpenKeyEx(HKEY_LOCAL_MACHINE, GetFromResource(IDS_ICW_DIRECTORY_ENTRY), 0, KEY_READ, &hKey);
    if (regStatus == ERROR_SUCCESS) {
        // Get The Path
        dwInfoSize = MAX_PATH;
        RegQueryValueEx(hKey, GetFromResource(IDS_ICW_DIRECTORY_ENTRY_VALUE), NULL, 0, (LPBYTE)czTemp, &dwInfoSize);
...
TCHAR * DUNFileProcess::GetFromResource(int iResId)
{
    ...
    LoadString(m_hInstance, iResId, m_szDunEntry[iCount], MAX_DUN_ENTRYNAME);
...
DWORD DUNFileProcess::CreateRasEntry(RASENTRY *pRasEntry, LPTSTR szUserName, LPTSTR szPassword)
...
//          0 failure
{
    ...
    pRasEntry->dwfOptions = RASEO_UseCountryAndAreaCodes | RASEO_ModemLights;
    ...
    // Area Code
    DWORD  dwAreaCode = GetPrivateProfileInt(szSection, GetFromResource(IDS_DUN_PHONE_AREACODE), 0, m_szDUNFile);
    _tprintf(pRasEntry->szAreaCode, "%d", dwAreaCode);

    // Country Code
    pRasEntry->dwCountryCode = GetPrivateProfileInt(szSection, GetFromResource(IDS_DUN_PHONE_COUNTRY), 1, m_szDUNFile);
    ...
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_PHONE_DIALAS), GetFromResource(IDS_DUN_VALUE_YES), szValue, sizeof(szValue), m_szDUNFile);
    ...
    // SW Compression
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_SERVER_SW_COMPRESS), GetFromResource(IDS_DUN_VALUE_YES), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
        pRasEntry->dwfOptions |= RASEO_SwCompression;
    }

    //Negociate TCP
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_SERVER_TCPIP), GetFromResource(IDS_DUN_VALUE_YES), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
        pRasEntry->dwfNetProtocols = RASNP_Ip;
    }

    // Disable LCP
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_SERVER_LCP), GetFromResource(IDS_DUN_VALUE_YES), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
    ...
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_TCP_IPADDRESS), GetFromResource(IDS_DUN_VALUE_NO), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
        pRasEntry->dwfOptions |= RASEO_SpecificIpAddr;
    }

    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_TCP_SERVERADDRESS), GetFromResource(IDS_DUN_VALUE_NO), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
        pRasEntry->dwfOptions |= RASEO_SpecificNameServers;
    }

    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_TCP_HEADERCOMPRESSION), GetFromResource(IDS_DUN_VALUE_NO), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
        pRasEntry->dwfOptions |= RASEO_IpHeaderCompression;
    }

    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_TCP_GATEWAY), GetFromResource(IDS_DUN_VALUE_NO), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
    ...
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_TCP_SERVERADDRESS), GetFromResource(IDS_DUN_VALUE_NO), szValue, sizeof(szValue), m_szDUNFile);
    if (!_tcsicmp(szValue, GetFromResource(IDS_DUN_VALUE_YES))) {
        dwRead = GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_TCP_DNS), _T("0"), szValue, sizeof(szValue), m_szDUNFile);
        if (dwRead > 2) {
            FillIpAddress(szValue, &pRasEntry->ipaddrDns);
        }

        //  Get The Secondary Address
        dwRead = GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_TCP_DNS_ALT), _T("0"), szValue, sizeof(szValue), m_szDUNFile);
        if (dwRead > 2) {
    ...
    // User Section
    _tcscpy(szSection, GetFromResource(IDS_DUN_USER_SECTION));
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_USER_NAME), _T(""), szUserName, MAX_PATH, m_szDUNFile);
    GetPrivateProfileString(szSection, GetFromResource(IDS_DUN_USER_PASSWORD), _T(""), szPassword, MAX_PATH, m_szDUNFile);
...
int GetDeaultCfgAreaCode(HINSTANCE hInstance, DWORD *CountryCode, DWORD *AreaCode)
{
    ...
    *AreaCode = 1; // Default Area Code
    ...
    if (lStatus == ERROR_SUCCESS) {//  Get Index
...
DWORD  GetRASEntries(HINSTANCE hInstance, RASENTRY *pRasEntry, LPTSTR szUserName, LPTSTR szPassword)
//  returns 0 if Error
//          1 if Succesful
{
    HKEY    hKey;
    ...
    //  Presently this string is not passed as the Phone bokk is still not a UNICODE version    CXG 6-0497
    ...
    DUNFileProcess    dfp(hInstance, ConvertToUnicode(m_rgAccessEntry->szDataCenter));
...
DWORD ConfigureDUN(HWND hWnd, HINSTANCE hInstance, RASENTRY *pRasEntry, TCHAR *szUserName, TCHAR *szPassword, int iModemIndex)
{
    DWORD dwEntrySize;
    DWORD dwRet;

    // Prepare RASENTRY for Dial-up
    if (!GetRASEntries(hInstance, pRasEntry, szUserName, szPassword)) {
#ifdef _LOG_IN_FILE
    ...
    regStatus = RegOpenKeyEx(HKEY_CURRENT_USER, "RemoteAccess\\Addresses", 0, KEY_READ, &hRegKey);
    if (regStatus == ERROR_SUCCESS) {
        //  Key Exists  so search if REG WIZ exists
        dwInfoSize = 512;
        regStatus = RegQueryValueEx(hRegKey, RAS_ENTRY_NAME, NULL, 0, (unsigned char*)szParam, &dwInfoSize);
        if (regStatus == ERROR_SUCCESS) {
            RegDeleteValue(hRegKey, RAS_ENTRY_NAME);// The Key Exits So delete the Entry
#ifdef _LOG_IN_FILE
            RW_DEBUG << "\nRegistry Entry found  " << RAS_ENTRY_NAME << "  and Deleting the Entry " << flush;
#endif
        }
    ...
    dwRet = ATK_RasSetEntryProperties(NULL, RAS_ENTRY_NAME, pRasEntry, dwEntrySize, NULL, 0);
    if (dwRet == ERROR_CANNOT_OPEN_PHONEBOOK) {
#ifdef _LOG_IN_FILE
        RW_DEBUG << "\nFun  ConfigureDUN : ERROR_CANNOT_OPEN_PHONEBOOK  " << dwRet << flush;
#endif
        return FALSE;
    } else if (dwRet == ERROR_BUFFER_INVALID) {
#ifdef _LOG_IN_FILE
