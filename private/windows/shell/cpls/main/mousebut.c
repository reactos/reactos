...
Abstract:
    This module contains the routines for the Mouse Buttons Property Sheet page.
--*/
...
//  GetIntFromSubKey
//  hKey is the handle to the subkey (already pointing to the proper location.
...
//  UpdateRegistry
//  This updates a given value of any data type at a given location in the registry.
    ...
    DWORD dwDataSize)
{
    ...
    {
        RegSetValueEx(hk, (LPTSTR)lpszValueName, 0, dwDataType, (LPBYTE)lpvData, dwDataSize);
#ifdef DEBUG
...
void ShellClick_UpdateUI(HWND hDlg, PMOUSEBUTSTR pMstr)
{
    HICON hicon = pMstr->bShellSingleClick ? pMstr->hIconSglClick : pMstr->hIconDblClick;

    SendMessage(GetDlgItem(hDlg, MOUSE_CLICKICON), STM_SETICON, (WPARAM)hicon, 0L);
}
