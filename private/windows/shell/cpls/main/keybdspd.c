...
Abstract:
    This module contains the main routines for the Keyboard applet's Speed property page.
--*/
    ...
    LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case ( WM_HELP ) :             // F1
        {
            WinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD_PTR)(LPTSTR)aKbdHelpIds );
            break;
        }
        case ( WM_CONTEXTMENU ) :      // right mouse click
        {
            WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD_PTR)(LPTSTR)aKbdHelpIds );
            break;
        }
        case ( WM_NOTIFY ) :
        {
            switch (((NMHDR *)lParam)->code)
            {
                case ( PSN_APPLY ) :
                {
                    ...
                    if (RegCreateKey( HKEY_CURRENT_USER, c_szUserDesktopKey, &hk ) == ERROR_SUCCESS)
                    {
                        TCHAR buf[16];
                    ...
                    SetDelayAndSpeed( hDlg, -1, -1, SPIF_UPDATEINIFILE | SPIF_SENDWININICHANGE );
                    GetSpeedGlobals();
                    HourGlass(FALSE);
                    break;
                }
                case ( PSN_RESET ) :
                {
                    ...
                    if (bKbNeedsReset)
                    {
                        ...
                        SetDelayAndSpeed( hDlg, uOriginalDelay, uOriginalSpeed, 0 );
                    }
                    break;
                }
            }
