...
static const TCHAR c_szInstalledLocales[] = TEXT("System\\CurrentControlSet\\Control\\Nls\\Locale");
static const TCHAR c_szLanguageGroups[] = TEXT("System\\CurrentControlSet\\Control\\Nls\\Language Groups");
static const TCHAR c_szMUILanguages[] = TEXT("System\\CurrentControlSet\\Control\\Nls\\MUILanguages");
static const TCHAR c_szCPanelIntl[] = TEXT("Control Panel\\International");
static const TCHAR c_szMUIPolicyKeyPath[] = TEXT("Software\\Policies\\Microsoft\\Control Panel\\Desktop");
static const TCHAR c_szMUIValue[] = TEXT("MultiUILanguageId");
static const TCHAR c_szIntlRun[] = TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\IntlRun");
static const TCHAR c_szSysocmgr[] = TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\IntlRun.OC");
...
//  Function Prototypes.
BOOL Region_LoadLanguageGroups(HWND hDlg, LPREGDLGDATA pDlgData);
BOOL Region_SetupLanguageGroups(HWND hDlg, LPBOOL pbReboot, LPBOOL pbUserCancel);
VOID Region_RebootTheSystem();
BOOL Region_InstallKeyboardLayout(HWND hDlg, LCID Locale, LPTSTR pszLocale);
int Region_ShowMsg(HWND hDlg, UINT iMsg, UINT iTitle, UINT iType, LPTSTR pString);
BOOL Region_InstallUserLocale(LCID Locale);
BOOL Region_InstallDefaultLayout(LCID Locale, LPTSTR pszLocale);
BOOL Region_InstallAllLayouts(HINF hIntlInf, LCID Locale, LPTSTR pszLocale);
BOOL Region_SetPreloadLocale(LCID Locale, HINF hIntlInf, DWORD dwLayout);
BOOL CALLBACK Region_EnumUILanguagesProc(PWSTR pwszUILanguage, LONG_PTR lParam);
void Region_RunRegApps(LPCTSTR pszRegKey);
...
void Region_EnumLocales(HWND hDlg, HWND hLocale, BOOL EnumSystemLocales)
{
    LPLANGUAGEGROUP pLG;
    ...
    //  Display only the locales for the groups that are either already installed or the groups the user wants to be installed.
    pLG = pLanguageGroups;
    while (pLG)
    {
        //  If the language group is originally installed and not marked for removal OR is marked to be installed, then add the locales for this language group to the System and User combo boxes.
        if ((pLG->wStatus & ML_INSTALL) || ((pLG->wStatus & ML_ORIG_INSTALLED) && !(pLG->wStatus & ML_REMOVE)))
        {
            for (ctr = 0; ctr < pLG->NumLocales; ctr++)
            {
                ...
                {
                    //  If we've already displayed Spanish (Spain), then don't display it again.
                    if (!fSpanish)
                    {
                        ...
                        if (LoadString(hInstance, IDS_SPANISH_NAME, szBuf, SIZE_300))
                        {
                            ...
                            ComboBox_SetItemData( hLocale, dwIndex, LCID_SPANISH_INTL );
                            fSpanish = TRUE;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        iRet = GetLocaleInfo( Locale, LOCALE_IDEFAULTANSICODEPAGE | LOCALE_NOUSEROVERRIDE | LOCALE_RETURN_NUMBER, (PTSTR) &dwLocaleACP, sizeof(dwLocaleACP) / sizeof(TCHAR) );
                        if (iRet)
                        {
                            iRet = dwLocaleACP;
                        }
...
void Region_UpdateSortingCombo(HWND hDlg, LCID NewLocale)
{
    ...
    if (NewLocale == 0)
    {
        ...
        if ((dwIndex == CB_ERR) || ((NewLocale = (LCID)ComboBox_GetItemData(hUserLocale, dwIndex)) == CB_ERR))
        {
            return;
        }
    }
    ...
    if (LangID == LANG_SPANISH_INTL)
    {
        ...
        if (GetLocaleInfo((LCID)LangID, LOCALE_SSORTNAME, szBuf, SIZE_128))
        {
            ...
            //  locale id.
            if ((NewLocale == LCID_SPANISH_TRADITIONAL) || (UserLocaleID == LCID_SPANISH_TRADITIONAL))
            {
                ComboBox_SetCurSel(hSorting, dwIndex);
            }
        }
    ...
    for (ctr = 0; ctr < g_NumAltSorts; ctr++)
    {
        ...
        if ((LANGIDFROMLCID(LocaleID) == LangID) && (GetLocaleInfo(LocaleID, LOCALE_SSORTNAME, szBuf, SIZE_128)))
        {
            //  Add the new sorting option to the sorting combo box.
            dwIndex = ComboBox_AddString(hSorting, szBuf);
            ComboBox_SetItemData(hSorting, dwIndex, LocaleID);

            //  Set this as the current selection if it's the current locale id.
    ...
    {
        if ((IsWindowEnabled(hSorting) == FALSE) || (IsWindowVisible(hSorting) == FALSE))
        {
    ...
    {
        if ((IsWindowEnabled(hSorting) == TRUE) || (IsWindowVisible(hSorting) == TRUE))
        {
...
//  The function assumes this is an MUI system.
{
    ...
    if (RegOpenKey(HKEY_CURRENT_USER, c_szMUIPolicyKeyPath, &hKey) == ERROR_SUCCESS)
    {
        if ((RegQueryValueEx(hKey, c_szMUIValue, NULL, &dwType, &buf[0], &dwResultLen) == ERROR_SUCCESS) && (dwType == REG_SZ) && (dwResultLen > 2))
        {
            bRet = TRUE;
        }
        RegCloseKey(hKey);
...
void Region_UpdateUILanguageCombo(HWND hDlg)
{
    ...
    if (!(DefaultUILang = GetUserDefaultUILanguage()) || (RegOpenKeyEx( HKEY_LOCAL_MACHINE, c_szMUILanguages, 0, KEY_READ, &hKey ) != ERROR_SUCCESS))
    {
    ...
    szData[0] = TEXT('\0');
    rc = RegEnumValue( hKey, dwIndex, szValue, &cchValue, NULL, NULL, (LPBYTE)szData, &cbData );
    while (rc == ERROR_SUCCESS)
    {
        //  If the UI language contains data, then it is installed.
        if ((szData[0] != 0) && (UILang = TransNum(szValue)) && (GetLocaleInfo(UILang, LOCALE_SNATIVELANGNAME, szData, MAX_PATH)))
        {
    ...
    if (ComboBox_GetCount(hUILang) < 1)
    {
        ...
        //  combo box.
        if ((GetLocaleInfo(DefaultUILang, LOCALE_SNATIVELANGNAME, szData, MAX_PATH)) && (ComboBox_AddString(hUILang, szData) == 0))
        {
    ...
    if (ComboBox_GetCount(hUILang) > 1)
    {
        if ((IsWindowEnabled(hUILang) == FALSE) || (IsWindowVisible(hUILang) == FALSE))
        {
    ...
    {
        if ((IsWindowEnabled(hUILang) == TRUE) || (IsWindowVisible(hUILang) == TRUE))
        {
            EnableWindow(hUILangText, FALSE);
            EnableWindow(hUILang, FALSE);
            ShowWindow(hUILangText, SW_HIDE);
...
void Region_ClearValues(HWND hDlg)
...
void Region_SetValues(HWND hDlg, LPREGDLGDATA pDlgData, BOOL fInit)
//  Initialize all of the controls in the region property sheet page.
{
    ...
    if (fInit)
    {
        ...
        if ((UserLocaleID == LCID_SPANISH_TRADITIONAL) || (UserLocaleID == LCID_SPANISH_INTL))
        {
    ...
    else
    {
        ...
        ComboBox_GetLBText( hUserLocale, ComboBox_GetCurSel(hUserLocale), szUserBuf );
        ComboBox_GetLBText( hSorting, ComboBox_GetCurSel(hSorting), szSorting );
        ComboBox_GetLBText( hUILang, ComboBox_GetCurSel(hUILang), szUILang );

        if (pDlgData)
        {
            ComboBox_GetLBText( hUserLocale, pDlgData->dwCurUserLocale, szDefaultUserBuf );
            ComboBox_GetLBText( hUserLocale, pDlgData->dwLastUserLocale, szLastUserBuf );
        }
    }

    Region_ClearValues(hDlg);//  Reset the combo boxes.
    Region_EnumLocales(hDlg, hUserLocale, FALSE);//  Get the list of locales and fill in the user locale combo box.
    ...
    if (ComboBox_SetCurSel( hSorting, ComboBox_FindStringExact( hSorting, -1, szSorting ) ) == CB_ERR)
    {
        ComboBox_SetCurSel(hSorting, dwIndex);
    }
    ...
    if (ComboBox_SetCurSel( hUILang, ComboBox_FindStringExact( hUILang, -1, szUILang ) ) == CB_ERR)
    {
        ComboBox_SetCurSel(hUILang, dwIndex);
    }
...
BOOL Region_ChangeUILangForAllUsers(LANGID UILanguageId)
{
    HKEY hKey;
    TCHAR szData[MAX_PATH];
    ...
    if ((rc = RegOpenKeyEx( HKEY_USERS, TEXT(".DEFAULT\\Control Panel\\Desktop"), 0L, KEY_READ | KEY_WRITE, &hKey )) == ERROR_SUCCESS)
    {
        rc = RegSetValueEx( hKey, c_szMUIValue, 0L, REG_SZ, (LPBYTE)szData, (lstrlen(szData) + 1) * sizeof(TCHAR) );
        RegCloseKey(hKey);
    }
    ...
    if (NT_SUCCESS(rc))
    {
        ...
        if (rc == ERROR_SUCCESS)
        {
            if ((rc = RegOpenKeyEx( HKEY_USERS, TEXT("RegionalSettingsTempKey\\Control Panel\\Desktop"), 0L, KEY_READ | KEY_WRITE, &hKey )) == ERROR_SUCCESS)
            {
                rc = RegSetValueEx( hKey, c_szMUIValue, 0L, REG_SZ, (LPBYTE)szData, (lstrlen(szData) + 1) * sizeof(TCHAR) );
                RegCloseKey(hKey);
...
BOOL Region_ApplySettings(HWND hDlg, LPREGDLGDATA pDlgData)
...
//  change flag stored in the property sheet page structure appropriately.
{
        ...
        if (Region_SetupLanguageGroups(hDlg, &bReboot, &fUserCancel) == FALSE)
        {
            ...
            if (fUserCancel)
            {
                ...
                while( iIndex < cCount )
                {
                    ...
                    if (pLG)
                    {
                        if ((pLG->wStatus == ML_INSTALL) || (pLG->wStatus == (ML_REMOVE | ML_ORIG_INSTALLED)))
                        {
    ...
    //  admin privileges.
    if ((pDlgData->Changes & RC_SystemLocale) && (RegSysLocaleID != SysLocaleID) && (pDlgData->Admin_Privileges))
    {
        ...
        if (SetupChangeLocaleEx( hDlg, NewLocale, pSetupSourcePath, (g_bSetupCase) ? SP_INSTALL_FILES_QUIETLY : 0, NULL, 0 ))
        {
            //  If Setup fails, put up a message.
            SetCursor(hcurSave);
            Region_ShowMsg( NULL, IDS_SETUP_STRING, IDS_TITLE_STRING, MB_OK_OOPS, NULL );
    ...
    if (pDlgData->Changes & RC_UserLocale)
    {
        ...
        //  See if the current selections are different from the original selections.
        if ((dwLocale != CB_ERR) && ((dwLocale != pDlgData->dwCurUserLocale) || (dwSorting != pDlgData->dwCurSorting)))
        {
            //  Get the locale id for the current selection.
            ...
            //  Install the new locale by adding the appropriate information to the registry.
            ...
            //  Broadcast the message that the international settings in the registry have changed.
            dwRecipients = BSM_APPLICATIONS | BSM_ALLDESKTOPS;
            BroadcastSystemMessage( BSF_FORCEIFHUNG | BSF_IGNORECURRENTTASK | BSF_NOHANG | BSF_NOTIMEOUTIFNOTHUNG, &dwRecipients, WM_WININICHANGE, 0, (LPARAM)szIntl );
        }
    }
    ...
    if (pDlgData->Changes & RC_UILanguage)
    {
        ...
        //  See if the current selection is different from the original selection.
        if ((dwUILang != CB_ERR) && ((dwUILang != pDlgData->dwCurUILang)))
        {
            ...
            //  If the user has admin privileges, see if they want to have this set for ALL new users of this machine.
            if (pDlgData->Admin_Privileges)
            {
                if (Region_ShowMsg( hDlg, IDS_CHANGE_UI_LANG, IDS_TITLE_STRING, MB_YESNO | MB_ICONQUESTION, NULL ) == IDYES)
                {
                    if (!Region_ChangeUILangForAllUsers(UILang))
                    {
                        Region_ShowMsg( hDlg, IDS_DEFAULT_USER_ERROR, IDS_TITLE_STRING, MB_OK_OOPS, NULL );
                    }
                }
            }
            else
            {
                ...
                //  they log on.
                Region_ShowMsg( hDlg, IDS_CHANGE_UI_LANG_NOT_ADMIN, IDS_TITLE_STRING, MB_OK | MB_ICONINFORMATION, NULL);
            }
        }
    }

    //  If the user locale, system locale, or ui language changed, and we're not running in gui setup, then let's invoke desk.cpl.
    if (!g_bSetupCase && DeskCPLChanges)
    {
        Region_InvokeDeskCPL();
    }

    //  If the system locale changed and we're not running in gui setup, then let's invoke sysocmgr.exe.
    ...
    //  Register the regional change every time so that all other property pages will be updated due to the locale settings change.
    ...
    {
        if (Region_ShowMsg( hDlg, IDS_REBOOT_STRING, IDS_TITLE_STRING, MB_YESNO | MB_ICONQUESTION, NULL ) == IDYES)
        {
...
BOOL Region_ValidatePPS(HWND hDlg, LPREGDLGDATA pDlgData)
...
//  Also, if the user locale has changed, then register the change so that all other property pages will be updated with the new locale settings.
{
    ...
    if (Changes & RC_UserLocale)
    {
        ...
        //  See if the current selections are different from the original selections.
        if ((dwLocale != CB_ERR) && ((dwLocale != pDlgData->dwLastUserLocale) || (dwSorting != pDlgData->dwLastSorting)))
        {
            //  Get the locale id for the current selection.
            NewLocale = (LCID)ComboBox_GetItemData(hUserLocale, dwLocale);
...
BOOL Region_InitPropSheet(HWND hDlg, LPPROPSHEETPAGE psp)
{
    HKEY hKey;
    TCHAR szData[MAX_PATH];
    DWORD cbData;
    ...
    if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, c_szInstalledLocales, 0L, KEY_WRITE, &hKey ) == ERROR_SUCCESS)
    {
    ...
    {
        //  The user does not have admin privileges, so disable the appropriate items in the "Language settings for the system" group.
...
BOOL CALLBACK Region_EnumInstalledCPProc(LPTSTR pString)
{
    UINT CodePage;
    LPCODEPAGE pCP;
    ...
    //  Find the code page in the linked list and mark it as originally installed.
    pCP = pCodePages;
    while (pCP)
    {
...
DWORD Region_GetLanguageGroup(LCID lcid)
//  Reads the Language Group Id of the given language.
{
    ...
    szData[0] = 0;
    if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, c_szInstalledLocales, 0, KEY_READ, &hKey ) == ERROR_SUCCESS)
    {
        cbData = sizeof(szData);
...
BOOL Region_GetUILanguageGroups(PUILANGUAGEGROUP pUILanguageGroup)
//  Reads the language groups of all the UI languages installed on this machine.
{
...
BOOL CALLBACK Region_EnumUILanguagesProc(LPWSTR pwszUILanguage, LONG_PTR lParam)
{
    int Ctr = 0;
    LGRPID lgrp;
...
BOOL Region_EnumInstalledLanguageGroups()
{
    ...
    //  Get the original install language so that we can mark that language group as permanent.
    ...
    //  un-installing them.  MUISETUP makes sure that each installed UI language has its language group installed.
    Region_GetUILanguageGroups(&UILangGroup);

    //  Open the HKLM\SYSTEM\CurrentControlSet\Control\Nls\Language Groups key.
    if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, c_szLanguageGroups, 0, KEY_READ, &hKey ) != ERROR_SUCCESS)
    {
        return (FALSE);
    }
    ...
    while (rc == ERROR_SUCCESS)
    {
        ...
        {
            //  Find the language group in the linked list and mark it as originally installed.
            pLG = pLanguageGroups;
            while (pLG)
            {
                if (pLG->LanguageGroup == LanguageGroup)
                {
                    ...
                    //  installed, then disable the un-installation of this language group.
                    Ctr = 0;
...
BOOL Region_GetLocaleList(LPLANGUAGEGROUP pLG)
{
    ...
    {
        if (SetupGetLineByIndex(g_hIntlInf, szSection, LineNum, &Context) && SetupGetIntField(&Context, 0, &Locale))
        {
            if (SORTIDFROMLCID(Locale))
            {
                //  Add the locale to the alternate sort list for this language group.
            ...
            {
                //  Add the locale to the locale list for this language group.
...
BOOL Region_LanguageGroupDirExist(PTSTR pszLangDir)
{
...
BOOL Region_GetSupportedLanguageGroups(LPREGDLGDATA pDlgData)
{
    UINT LanguageGroup;
    ...
    for (LineNum = 0; LineNum < LineCount; LineNum++)
    {
        if (SetupGetLineByIndex(g_hIntlInf, TEXT("LanguageGroups"), LineNum, &Context) && SetupGetIntField(&Context, 0, &LanguageGroup))
        {
            //  Create the new node.
            ...
            //  appropriate lang\xxx directory does not exist, then disable the language group.
            wItemStatus = 0;
            ...
            {
                if ((ItemCount = SetupGetFieldCount(&Context)) && (ItemCount >= 2))
                {
            ...
            if ((!SetupFindFirstLine( g_hIntlInf, szSection, TEXT("AddReg"), &Context )))
            {
...
BOOL Region_LoadLanguageGroups(HWND hDlg, LPREGDLGDATA pDlgData)
{
    ...
    ListView_SetExtendedListViewStyle( hwndLG, dwExStyle | LVS_EX_CHECKBOXES | LVS_EX_FULLROWSELECT );

    //  Go through the list of language groups and add each one to the list view and set the appropriate state.
    pLG = pLanguageGroups;
    while (pLG)
    {
        ...
        if (iIndex >= 0)
        {
            ListView_SetItemState( hwndLG,
                                   iIndex,
                                   (pLG->wStatus & ML_ORIG_INSTALLED) ? INDEXTOSTATEIMAGEMASK(LVIS_SELECTED) : INDEXTOSTATEIMAGEMASK(LVIS_FOCUSED),
                                   LVIS_STATEIMAGEMASK );
        }
    ...
    while (iIndex > 0)
    {
        ListView_SetItemState( hwndLG, iIndex - 1, 0, LVIS_FOCUSED | LVIS_SELECTED );
        iIndex--;
    }

    //  Select the first one in the list.
    ListView_SetItemState( hwndLG, 0, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED );
...
BOOL Region_GetSupportedCodePages()
{
    ...
    for (LineNum = 0; LineNum < LineCount; LineNum++)
    {
        if (SetupGetLineByIndex(g_hIntlInf, TEXT("CodePages"), LineNum, &Context) && SetupGetIntField(&Context, 0, &CodePage))
        {
            //  Create the new node.
            ...
            if ((CodePage == GetACP()) || (CodePage == GetOEMCP()) || (!SetupFindFirstLine( g_hIntlInf, szSection, TEXT("AddReg"), &Context )))
            {
                //  Mark it as permanent.
                //  Also mark it as originally installed to avoid problems.
...
BOOL Region_LoadCodePages(HWND hDlg)
{
    ...
    ListView_SetExtendedListViewStyle( hwndCP, dwExStyle | LVS_EX_CHECKBOXES | LVS_EX_FULLROWSELECT );
    ...
    while (pCP)
    {
        ...
        //  There's a bug in the list view code such that the check mark
        //  isn't displayed when you set the state through InsertItem, so we have to set it explicitly using SetItemState.
        if (iIndex >= 0)
        {
            ListView_SetItemState( hwndCP,
                                   iIndex,
                                   (pCP->wStatus & ML_ORIG_INSTALLED) ? INDEXTOSTATEIMAGEMASK(LVIS_SELECTED) : INDEXTOSTATEIMAGEMASK(LVIS_FOCUSED),
                                   LVIS_STATEIMAGEMASK );
        }
    ...
    while (iIndex > 0)
    {
        ListView_SetItemState( hwndCP, iIndex - 1, 0, LVIS_FOCUSED | LVIS_SELECTED );
        iIndex--;
    }

    //  Select the first one in the list.
    ListView_SetItemState( hwndCP, 0, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED );
...
void Region_ListViewCustomDraw(HWND hDlg, LPNMLVCUSTOMDRAW pDraw)
{
    ...
    if ((pDraw->nmcd.dwDrawStage & CDDS_ITEMPREPAINT) && (pNode) && (pNode != (LPLANGUAGEGROUP)(LB_ERR)))
    {
        if (pNode->wStatus & (ML_PERMANENT | ML_DISABLE))
        {
            pDraw->clrText = (pDraw->nmcd.uItemState & CDIS_SELECTED) ?
                ((GetSysColor(COLOR_HIGHLIGHT) == GetSysColor(COLOR_GRAYTEXT)) ? GetSysColor(COLOR_HIGHLIGHTTEXT) : GetSysColor(COLOR_GRAYTEXT))
...
BOOL Region_ListViewChanging(HWND hDlg, NM_LISTVIEW *pLV)
{
    LPLANGUAGEGROUP pNode;
    ...
    //  NOTE: Both the language group structure and the code page structure have wStatus as the first field, so just use the language group structure to access the field.
    pNode = (LPLANGUAGEGROUP)pLV->lParam;

    //  Make sure we're not trying to change a permanent or disabled
    //  language group or code page.  If so, return TRUE to prevent the change.
...
BOOL Region_ListViewChanged(HWND hDlg, int iID, NM_LISTVIEW *pLV)
{
    ...
    //        have wStatus as the first field, so just use the language group structure to access the field.
    pNode = (LPLANGUAGEGROUP)pLV->lParam;

    //  Make sure we're not trying to change a permanent or disabled
    //  language group or code page.  If so, set the check box to its appropriate state.
    if (pNode->wStatus & (ML_PERMANENT | ML_DISABLE))
    {
    ...
    while (iCount > 0)
    {
        ListView_SetItemState( hwndLV, iCount - 1, 0, LVIS_FOCUSED | LVIS_SELECTED );
        iCount--;
    }

    //  Make sure this item is selected.
    ListView_SetItemState( hwndLV, pLV->iItem, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED );
...
BOOL Region_InitInf(HWND hDlg, HINF *phIntlInf, LPTSTR pszInf, HSPFILEQ *pFileQueue, PVOID *pQueueContext)
{
    //  Open the Inf file.
    *phIntlInf = SetupOpenInfFile(pszInf, NULL, INF_STYLE_WIN4, NULL);
    ...
    //  Create a setup file queue and initialize default setup copy queue callback context.
    ...
    *pQueueContext = SetupInitDefaultQueueCallbackEx( hDlg, (g_bSetupCase ? INVALID_HANDLE_VALUE : NULL), 0L, 0L, NULL );
    if (!*pQueueContext)
    {
...
void Region_CloseInf(HINF hIntlInf, HSPFILEQ FileQueue, PVOID QueueContext)
{
    //  Terminate the Queue.
...
BOOL Region_InitSystemLocales(HWND hDlg)
{
    ...
    //  Get the list of locales and fill in the system locale combo box.
    Region_EnumLocales(hDlg, hSystemLocale, TRUE);

    //  Get the string for the system default setting.
    //  Special case Spanish.
    if ((SysLocaleID == LCID_SPANISH_TRADITIONAL) || (SysLocaleID == LCID_SPANISH_INTL))
    {
...
BOOL Region_SetDefaultCommandOK(HWND hDlg)
{
    HWND hSystemLocale = GetDlgItem(hDlg, IDC_SYSTEM_LOCALE);
    DWORD dwLocale;
    LCID NewLocale;
...
INT_PTR Region_SetDefaultProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case ( WM_INITDIALOG ) :
        ...
        case ( WM_HELP ) :
        {
            WinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD_PTR)(LPTSTR)aRegionSetDefaultHelpIds );
            break;
        }
        case ( WM_CONTEXTMENU ) :
        {
            WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD_PTR)(LPTSTR)aRegionSetDefaultHelpIds );
            break;
        }
        case ( WM_COMMAND ) :
        {
...
int Region_CommandSetDefault(HWND hDlg, LPREGDLGDATA pDlgData)
{
    int rc = 0;
    ...
    if (pDlgData != NULL)
    {
        ...
        if ((rc = (int)DialogBoxParam( hInstance, MAKEINTRESOURCE(DLG_REGION_SET_DEFAULT), hDlg, Region_SetDefaultProc, (LPARAM)(pDlgData) )) != IDOK)
        {
...
BOOL Region_AdvancedCommandOK(HWND hDlg)
{
    LPCODEPAGE pCP;
    HINF hIntlInf;
    HSPFILEQ FileQueue;
    ...
    //  Go through each code page node to see if anything needs to be done.
    pCP = pCodePages;
    while (pCP)
    {
        //  See if any changes are necessary for this code page.
        if ((pCP->wStatus == ML_INSTALL) || (pCP->wStatus == (ML_ORIG_INSTALLED | ML_REMOVE)))
        {
            //  See if we're installing or removing.
            fAdd = (pCP->wStatus == ML_INSTALL);

            //  Initialize Inf stuff.
            if ((!bInitInf) && (!Region_InitInf(hDlg, &hIntlInf, szIntlInf, &FileQueue, &QueueContext)))
            {
                SetCursor(hcurSave);
                return (FALSE);
            }
            ...
            //  Get the inf section name.
            wsprintf( szSection, TEXT("%ws%d"), fAdd ? szCPInstallPrefix : szCPRemovePrefix, pCP->CodePage );

            //  Enqueue the code page files so that they may be
            //  copied.  This only handles the CopyFiles entries in the inf file.
            if (!SetupInstallFilesFromInfSection( hIntlInf, NULL, FileQueue, szSection, pSetupSourcePath, SP_COPY_NEWER ))
            {
                //  Setup failed to find the code page.
                //  This shouldn't happen - the inf file is messed up.
                Region_ShowMsg( hDlg, IDS_ML_COPY_FAILED, 0, MB_OK_OOPS, pCP->pszName );
                pCP->wStatus = 0;
            }
        }
    ...
    if (bInitInf)
    {
                                 ...
                                 &d )) && (d != 1))
        {
            ...
            if (!SetupCommitFileQueue( hDlg, FileQueue, SetupDefaultQueueCallback, QueueContext ))
            {
                //  This can happen if the user hits Cancel from within
                //  the setup dialog.
                Region_ShowMsg( hDlg, IDS_ML_SETUP_FAILED, 0, MB_OK_OOPS, NULL );
                bRet = FALSE;
        ...
        while (pCP)
        {
            ...
            if ((pCP->wStatus == ML_INSTALL) || (pCP->wStatus == (ML_ORIG_INSTALLED | ML_REMOVE)))
            {
                ...
                //  Call setup to install other inf info for this code page.
                if (!SetupInstallFromInfSection( hDlg, hIntlInf, szSection, SPINST_ALL & ~SPINST_FILES, NULL, pSetupSourcePath, 0, NULL, NULL, NULL, NULL ))
                {
                    //  Setup failed.

                    //  Already copied the code page file, so no need to change the status of the code page info here.

                    //  This shouldn't happen - the inf file is messed up.
                    Region_ShowMsg( hDlg, IDS_ML_INSTALL_FAILED, 0, MB_OK_OOPS, pCP->pszName );
                }

                //  Reset the status to show the new state of this code page.
...
INT_PTR Region_AdvancedProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
        case ( WM_INITDIALOG ) :
        ...
        case ( WM_HELP ) :
        {
            WinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD_PTR)(LPTSTR)aRegionAdvancedHelpIds );
            break;
        }
        case ( WM_CONTEXTMENU ) :
        {
            WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD_PTR)(LPTSTR)aRegionAdvancedHelpIds );
            break;
        }
        case ( WM_NOTIFY ) :
        {
            ...
            switch (psn->code)
            {
                ...
                {
                    return (Region_ListViewChanging( hDlg, (NM_LISTVIEW *)lParam ));
                    break;
                }
                case ( LVN_ITEMCHANGED ) :
                {
                    Region_ListViewChanged( hDlg, IDC_CODEPAGES, (NM_LISTVIEW *)lParam );
                    break;
                }
                case ( NM_CLICK ) :
                case ( NM_DBLCLK ) :
                {
                    ...
                    //  Get where we were hit and then translate it to our window.
                    GetCursorPos(&ht.pt);
                    ...
                    if ((ht.iItem >= 0) && ((ht.flags & LVHT_ONITEM) == LVHT_ONITEMLABEL))
                    {
                        ...
                        //  The user clicked on the item label.  Simulate a state change so we can process it.
                        state = ListView_GetItemState( hwndList, ht.iItem, LVIS_STATEIMAGEMASK );
                        ...
                        ListView_SetItemState( hwndList, ht.iItem, state, LVIS_STATEIMAGEMASK );
                    }
                }
                default :
                {
...
int Region_CommandAdvanced(HWND hDlg, LPREGDLGDATA pDlgData)
{
    int rc = 0;
    ...
    {
        //  Return value can be 1:IDOK, 2:IDCANCEL or -1:Error (from USER)
        if ((rc = (int)DialogBoxParam( hInstance, MAKEINTRESOURCE(DLG_REGION_ADVANCED), hDlg, Region_AdvancedProc, (LPARAM)(pDlgData) )) != IDOK)
        {
            //  Failure, so need to return 0.
            rc = 0;
        }
...
INT_PTR CALLBACK GeneralDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case ( WM_HELP ) :
        {
            WinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD_PTR)(LPTSTR)aRegionHelpIds );
            break;
        }
        case ( WM_CONTEXTMENU ) :      // right mouse click
        {
            WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD_PTR)(LPTSTR)aRegionHelpIds );
            break;
        }
        case ( WM_NOTIFY ) :
        {
            ...
            switch (psn->code)
            {
                ...
                case ( PSN_KILLACTIVE ) :
                {
                    ...
                    if (pDlgData)
                    {
                        SetWindowLongPtr( hDlg, DWLP_MSGRESULT, !Region_ValidatePPS( hDlg, pDlgData ) );
                    }
                    break;
                }
                case ( PSN_APPLY ) :
                ...
                case ( LVN_ITEMCHANGED ) :
                {
                    ...
                    if (Region_ListViewChanged(hDlg, IDC_LANGUAGE_GROUPS, pLV))
                    {
                        ...
                        //  the appropriate locales based on the chosen language groups.
                        Region_SetValues(hDlg, pDlgData, FALSE);

                        //  Note that the language groups have changed and enable the apply button.
                        if (pDlgData)
                        {
    ...
    //  startup.wShowWindow = wShowWindow;
        ...
        for (ctr = 0; ; ctr++)
        {
            ...
            else if (lEnum != ERROR_SUCCESS)
            {
                ...
                //  failure.  We can't recover from any other registry problem anyway.
                break;
            }
            ...
            if (dwType == REG_SZ)
            {
                ...
                if (CreateProcess( NULL, szCmdLine, NULL, NULL, FALSE, CREATE_NEW_PROCESS_GROUP, NULL, NULL, &startup, &pi ))
                {
...
BOOL Region_SetupLanguageGroups(HWND hDlg, LPBOOL pbReboot, LPBOOL pbUserCancel)
{
    ...
    //  Go through each language group node to see if anything needs to be done.
    pLG = pLanguageGroups;
    while (pLG)
    {
        //  See if any changes are necessary for this language group.
        if ((pLG->wStatus == ML_INSTALL) || (pLG->wStatus == (ML_ORIG_INSTALLED | ML_REMOVE)))
        {
            //  See if we're installing or removing.
            ...
            //  Get the inf section name.
            wsprintf( szSection, TEXT("%ws%d"), fAdd ? szLGInstallPrefix : szLGRemovePrefix, pLG->LanguageGroup );
            ...
            if (!SetupInstallFilesFromInfSection( hIntlInf, NULL, FileQueue, szSection, pSetupSourcePath, SP_COPY_NEWER ))
            {
                ...
                //  This shouldn't happen - the inf file is messed up.
                Region_ShowMsg( hDlg, IDS_ML_COPY_FAILED, 0, MB_OK_OOPS, pLG->pszName );
    ...
    if (bInitInf)
    {
        ...
        //  d = 2: User wants to use existing files, but del/ren queues
        //         not empty.  Must commit queue.  The copy queue will
        //         have been emptied, so only del/ren functions will be performed.
        if ((SetupScanFileQueue( FileQueue,
                                 SPQ_SCAN_PRUNE_COPY_QUEUE | SPQ_SCAN_FILE_VALIDITY,
                                 hDlg,
                                 NULL,
                                 ...
                                 &d )) && (d != 1))
        {
            ...
            if (!SetupCommitFileQueue( hDlg, FileQueue, SetupDefaultQueueCallback, QueueContext ))
            {
                //  This can happen if the user hits Cancel from within the setup dialog.
                Region_ShowMsg( hDlg, IDS_ML_SETUP_FAILED, 0, MB_OK_OOPS, NULL );
                *pbUserCancel = TRUE;
                bRet = FALSE;
        ...
        while (pLG)
        {
            ...
            if ((pLG->wStatus == ML_INSTALL) || (pLG->wStatus == (ML_ORIG_INSTALLED | ML_REMOVE)))
            {
                ...
                if (!SetupInstallFromInfSection( hDlg, hIntlInf, szSection, SPINST_ALL & ~SPINST_FILES, NULL, pSetupSourcePath, 0, NULL, NULL, NULL, NULL ))
                {
                    ...
                    //  Already copied the language group file, so no need to change the status of the language group info here.

                    //  This shouldn't happen - the inf file is messed up.
                    Region_ShowMsg( hDlg, IDS_ML_INSTALL_FAILED, 0, MB_OK_OOPS, pLG->pszName );
                }
                else
                {
                    *pbReboot = TRUE;
                ...
                //  Reset the status to show the new state of this language group.
...
//  to reboot the system, and then resets all of the privileges to their old state.
VOID Region_RebootTheSystem()
{
    HANDLE Token = NULL;
...
void Region_UpdateIndicator(BOOL bCreate)
{
    HWND hwndIndicate;
    HKEY hKey;
    ...
    else if (bCreate)
    {
        ...
        if (RegCreateKey( HKEY_CURRENT_USER, REGSTR_PATH_RUN, &hKey ) == ERROR_SUCCESS)
        {
            RegSetValueEx( hKey, szInternat, 0, REG_SZ, (LPBYTE)szInternat, sizeof(szInternat) );
            RegCloseKey(hKey);
        }
    }
}
...
BOOL Region_InstallKeyboardLayout(HWND hDlg, LCID Locale, LPTSTR pszLocale)
{
    ...
    if (!Region_InstallDefaultLayout(Locale, pszLocale))
    {
        ...
        Region_ShowMsg( hDlg, IDS_KBD_LOAD_KBD_FAILED, 0, MB_OK_OOPS, szData );
        return (FALSE);
    }
...
HKL Region_GetHKL(DWORD dwLocale, DWORD dwLayout, LPTSTR pszLayout, HINF hIntlInf)
{
    TCHAR szData[MAX_PATH];
    INFCONTEXT Context;
    ...
    else
    {
        ...
        if (HIWORD(dwLayout) != 0)
        {
            ...
            //  should be.
            if ((SetupFindFirstLine(hIntlInf, szKbdLayoutIds, pszLayout, &Context)) && (SetupGetStringField(&Context, 1, szData, MAX_PATH, NULL)))
            {
...
BOOL Region_ValidateRegistryPreload()
{
    ...
    if (RegOpenKeyEx( HKEY_CURRENT_USER, szKbdPreloadKey, 0, KEY_ALL_ACCESS, &hKeyPreload ) != ERROR_SUCCESS)
    {
        return (FALSE);
    }

    //  Open the HKCU\Keyboard Layout\Substitutes key.
    if (RegOpenKeyEx( HKEY_CURRENT_USER, szKbdSubstKey, 0, KEY_ALL_ACCESS, &hKeySubst ) != ERROR_SUCCESS)
    {
    ...
    cbData = sizeof(szData);
    szData[0] = TEXT('\0');
    rc = RegQueryValueEx( hKeyPreload, szValue, NULL, NULL, (LPBYTE)szData, &cbData );
    while (rc == ERROR_SUCCESS)
    {
        //  Look to see if the input locale is installed and if the layout is valid.
        dwData = TransNum(szData);
        cbData2 = sizeof(szData2);
        szData2[0] = TEXT('\0');
        if ((IsValidLocale((LCID)(LOWORD(dwData)), LCID_INSTALLED)) &&
            ((RegQueryValueEx( hKeySubst, szData, NULL, NULL, (LPBYTE)szData2, &cbData2 ) != ERROR_SUCCESS) || (IsValidLocale((LCID)(LOWORD(TransNum(szData2))), LCID_INSTALLED))))
        {
            ...
            if (dwNum != dwNumSet)
            {
                ...
                RegSetValueEx( hKeyPreload, szValue, 0, REG_SZ, (LPBYTE)szData, (DWORD)(lstrlen(szData) + 1) * sizeof(TCHAR) );
            }
            dwNumSet++;
        }
        else
        {
            if ((szData2[0] != 0) || (RegQueryValueEx( hKeySubst, szData, NULL, NULL, (LPBYTE)szData2, &cbData2 ) == ERROR_SUCCESS))
            {
                UnloadKeyboardLayout(Region_GetHKL( dwData, TransNum(szData2), szData2, hIntlInf ));
...
{
    //  Make sure the locale is valid and then call setup to install the requested locale.
    if (IsValidLocale(Locale, LCID_INSTALLED))
    {
        if (!SetupChangeLocaleEx( HWND_DESKTOP, LOWORD(Locale), pSetupSourcePath, SP_INSTALL_FILES_QUIETLY, NULL, 0 ))
        {
...
void Region_SetLocaleInfo(LCID Locale, LCTYPE LCType, LPTSTR lpIniStr)
{
    TCHAR pBuf[SIZE_128];

    if (GetLocaleInfo( Locale, LCType | LOCALE_NOUSEROVERRIDE, pBuf, SIZE_128 ))
    {
...
BOOL Region_InstallUserLocale(LCID Locale)
{
    ...
    if ((RegOpenKeyEx( HKEY_CURRENT_USER, c_szCPanelIntl, 0L, KEY_READ | KEY_WRITE, &hKey ) != ERROR_SUCCESS) ||
        (RegSetValueEx( hKey, TEXT("Locale"), 0L, REG_SZ, (LPBYTE)szLCID, (lstrlen(szLCID) + 1) * sizeof(TCHAR) ) != ERROR_SUCCESS))
    {
        if (hKey != NULL)
        {
            RegCloseKey(hKey);
...
BOOL Region_IsValidLayout(DWORD dwLayout)
{
    HKEY hKey1, hKey2;
...
void Region_SetFEHotkey(BOOL bThai)
{
    HKEY hKey;
    TCHAR szData[MAX_PATH];
    DWORD cbData;

    //  If there is no hotkey switch, set it to Ctrl+Shift.  Otherwise, the user cannot switch to an IME without setting the value first.
    szData[0] = TEXT('\0');
    if (RegOpenKey( HKEY_CURRENT_USER, szKbdToggleKey, &hKey ) == ERROR_SUCCESS)
    {
        cbData = sizeof(szData);
        RegQueryValueEx( hKey, TEXT("Hotkey"), NULL, NULL, (LPBYTE)szData, &cbData );
        switch (szData[0])
        {
            case TEXT('1'):
            case TEXT('2'):
            ...
            case TEXT('3'):
            {
                ...
                szData[1] = TEXT('\0');
                RegSetValueEx( hKey, TEXT("Hotkey"), 0, REG_SZ, (LPBYTE)szData, (DWORD)(lstrlen(szData) + 1) * sizeof(TCHAR) );
                break;
            }
            case TEXT('4'):
            {
    ...
    else if (RegCreateKey( HKEY_CURRENT_USER, szKbdToggleKey, &hKey ) == ERROR_SUCCESS)
    {
        //  We don't have a Toggle key yet.  Create one and set the correct value.
        szData[0] = bThai? TEXT('4'): TEXT('1');
        szData[1] = 0;
        RegSetValueEx( hKey, TEXT("Hotkey"), 0, REG_SZ, (LPBYTE)szData, (DWORD)(lstrlen(szData) + 1) * sizeof(TCHAR) );
...
void Region_InstallInputLocales(LPLAYOUTLIST pLayoutList, DWORD dwNum)
{
    HKEY hKeyPreload, hKeySubst;
    TCHAR szValue[MAX_PATH];
    TCHAR szData[MAX_PATH];
    ...
    cchValue = sizeof(szValue) / sizeof(TCHAR);
    cbData = sizeof(szData);
    rc = RegEnumValue( hKeyPreload, dwIndex, szValue, &cchValue, NULL, NULL, (LPBYTE)szData, &cbData );
    while (rc == ERROR_SUCCESS)
    {
        //  Save the preload number if it's higher than the highest one found so far.
        ...
        dwData = 0;
        cbData2 = sizeof(szData2);
        if (RegQueryValueEx( hKeySubst, szData, NULL, NULL, (LPBYTE)szData2, &cbData2 ) == ERROR_SUCCESS)
        {
            dwData = TransNum(szData2);
        }

        //  Go through each of the requested input locales and make sure they don't already exist.
        for (dwCtr = 0; dwCtr < dwNum; dwCtr++)
        {
            if (LOWORD(pLayoutList[dwCtr].dwLocale) == LOWORD(dwValue))
            {
                ...
                if (pLayoutList[dwCtr].bIME == FALSE)
                {
                    ...
                    if (((dwData2 & 0xf000) != 0xe000) && (pLayoutList[dwCtr].dwSubst <= dwData2))
                    {
                        if (dwData2 == 0)
                        {
    ...
    //  Increase the maximum preload value by one so that it represents the next available value to use.
    dwPreloadNum++;

    //  Go through the list of layouts and add them.
    for (dwCtr = 0; dwCtr < dwNum; dwCtr++)
    {
        if ((pLayoutList[dwCtr].bLoaded == FALSE) && (IsValidLocale(pLayoutList[dwCtr].dwLocale, LCID_INSTALLED)) && (Region_IsValidLayout(pLayoutList[dwCtr].dwLayout)))
        {
            ...
            //  Save the locale id as a string so that it can be written into the registry.
            ...
            else
            {
                ...
                if (dwCtr != 0)
                {
                    ...
                    {
                        dwCtr2--;
                        if ((pLayoutList[dwCtr2].bLoaded == FALSE) && (pLayoutList[dwCtr].dwLocale == pLayoutList[dwCtr2].dwLocale) && (pLayoutList[dwCtr2].bIME == FALSE))
                        {
            ...
            if (((pLayoutList[dwCtr].dwLocale != pLayoutList[dwCtr].dwLayout) || (pLayoutList[dwCtr].dwSubst != 0)) && (pLayoutList[dwCtr].bIME == FALSE))
            {
                wsprintf(szData2, TEXT("%08x"), pLayoutList[dwCtr].dwLayout);
                RegSetValueEx( hKeySubst, szData, 0, REG_SZ, (LPBYTE)szData2, (DWORD)(lstrlen(szData) + 1) * sizeof(TCHAR) );
            }
    ...
    //  Ctrl+Shift.  Otherwise, the user cannot switch to an IME without setting the value first.
...
BOOL Region_GetInputLocaleList(PINFCONTEXT pContext, DWORD dwStartField, LPDWORD pdwLocale, LPDWORD pdwLayout)
{
    ...
    pLayoutList = (LPLAYOUTLIST)LocalAlloc( LPTR, sizeof(LAYOUTLIST) * dwNumList );
    if (pLayoutList == NULL)
    {
        return (FALSE);
    }
    ...
    {
        if (SetupGetStringField( pContext, dwCtr, szBuffer, MAX_PATH, NULL ))
        {
            //  Find the colon in order to save the input locale and layout values separately.
            pPos = szBuffer;
            while (*pPos)
            {
                if ((*pPos == CHAR_COLON) && (pPos != szBuffer))
                {
                    *pPos = 0;
                    ...
                    if ((pLayoutList[dwCtr2].dwLocale = TransNum(szBuffer)) && (pLayoutList[dwCtr2].dwLayout = TransNum(pPos)))
                    {
                        //  See if it's an IME.
                        if ((HIWORD(pLayoutList[dwCtr2].dwLayout) & 0xf000) == 0xe000)
                        {
                            ...
                            if (LOWORD(pLayoutList[dwCtr2].dwLocale) != LOWORD(pLayoutList[dwCtr2].dwLayout))
                            {
                                break;
                            }
                        }
                        ...
                        {
                            if ((pLayoutList[dwCtr2].dwLocale == pLayoutList[dwCtr3].dwLocale) && (pLayoutList[dwCtr2].dwLayout == pLayoutList[dwCtr3].dwLayout))
                            {
                                break;
                            }
                        }

                        //  Only increment the count if it's not a duplicate.
                        if (dwCtr3 == dwCtr2)
                        {
                            dwCtr2++;
                        }
...
BOOL Region_RemoveActiveImmKey(PTSTR szLayoutKey, PTSTR szLayoutId)
//  This is called during keyboard upgrade to remove the keyboard layout entry if Active IMM has been installed.
//  Active IMM is only installed on NT4 systems and isn't supported in NT5.
{
    HKEY hLayoutKey;
    BOOL bDelete = FALSE;
    DWORD dwLayoutId = TransNum(szLayoutId);

    if ((dwLayoutId == 0x0404) || (dwLayoutId == 0x0804) || (dwLayoutId == 0x0411) || (dwLayoutId == 0x0412))
    {
        if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, szLayoutKey, 0, KEY_ALL_ACCESS, &hLayoutKey ) == ERROR_SUCCESS)
        {
            if ((RegQueryValueEx( hLayoutKey, TEXT("AIMMFlags"), NULL, NULL, NULL, NULL ) == ERROR_SUCCESS))
            {
                bDelete = TRUE;
            }
...
BOOL Region_FileExists(PTSTR szFile)
{
    HANDLE hFile;
...
//  Installs the necessary language groups to enable the installed keyboard layouts.
BOOL Region_UpgradeKeyboardLayoutList(HINF *phIntlInf, HSPFILEQ *pFileQueue, PVOID *pQueueContext)
{
    ...
    //  Let's install language groups for any installed keyboard layout, in case we're doing an upgrade.
    if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, szLayoutPath, 0, KEY_ALL_ACCESS, &hKeyLayouts ) == ERROR_SUCCESS)
    {
        ...
        rc = RegEnumKey( hKeyLayouts, dwIndex, szData, sizeof(szData) / sizeof(TCHAR) );
        SysDirLen = GetSystemDirectory( szSystemDirectory, (sizeof(szSystemDirectory) / sizeof(TCHAR)) );
        if (SysDirLen != 0)
        {
        ...
        while ((rc == ERROR_SUCCESS) && (SysDirLen != 0))
        {
            ...
            if (IsValidLocale((LCID)(LOWORD(dwData)), LCID_SUPPORTED))
            {
                *pszKbdLayoutFileName = TEXT('\0');
                ...
                if (RegOpenKey( HKEY_LOCAL_MACHINE, szKeyboardLayoutFilePath, &hKeyKeyboardLayout ) == ERROR_SUCCESS)
                {
                    cb = sizeof(szValue);
                    if ((RegQueryValueEx( hKeyKeyboardLayout, szLayoutFile, NULL, NULL, (LPBYTE) szValue, &cb ) == ERROR_SUCCESS))
                    {
                        Len = lstrlen(szValue);

                        if ((Len + SysDirLen) < (sizeof(szSystemDirectory) / sizeof(TCHAR)))
                        {
                            *pszFileName = TEXT('\0');
                ...
                if (IsKbdFileInstalled)
                {
                    ...
                    //  ActiveIMM is not supported in W2K, so delete its keys if present and don't install corresponding language group.
                    if (!Region_RemoveActiveImmKey( szKeyboardLayoutFilePath, szData ))
                    {
                        if (SetupFindFirstLine( *phIntlInf, L"Locales", szData, &LineContext ) &&
                            SetupGetStringField( &LineContext, 3, szValue, sizeof(szValue) / sizeof(TCHAR), NULL ))
                        {
                            ...
                            //  user is still no worse off than they were before the upgrade.
                            ...
                            szBuffer[0] = 0;
                            if (SetupFindFirstLine( *phIntlInf, L"LanguageGroups", szValue, &LineContext ))
                            {
                                if (SetupGetStringField( &LineContext, 2, szBuffer, MAX_PATH, NULL ))
                                {
                                    szBuffer[4] = TEXT('\0');
                                }
                            }
                            ...
                            {
                                //  Add it to the list, if not already added.
        ...
        for (dwCtr = 0; dwCtr < dwMaxLayouts; dwCtr++)
        {
            ...
            if (!IsValidLanguageGroup( InstallLanguageGroups[dwCtr], LGRPID_SUPPORTED ))
            {
                InstallLanguageGroups[dwCtr] = LGRPID_WESTERN_EUROPE;
            }
            else if (IsValidLanguageGroup( InstallLanguageGroups[dwCtr], LGRPID_INSTALLED ))
            {
                InstallLanguageGroups[dwCtr] = LGRPID_WESTERN_EUROPE;
            }
        }
        ...
        for (dwCtr = 0; dwCtr < dwMaxLayouts; dwCtr++)
        {
            ...
            if (InstallLanguageGroups[dwCtr] != LGRPID_WESTERN_EUROPE)
            {
                //  Enqueue the language group files so that they may be
                //  copied.  This only handles the CopyFiles entries in the inf file.
                wsprintf( szBuffer, TEXT("%ws%d"), szLGInstallPrefix, InstallLanguageGroups[dwCtr] );

                SetupInstallFilesFromInfSection( *phIntlInf, NULL, *pFileQueue, szBuffer, pSetupSourcePath, SP_COPY_NEWER );
            }
        }
        ...
        {
            //  Copy the files in the queue.
            if (!SetupCommitFileQueue( HWND_DESKTOP, *pFileQueue, SetupDefaultQueueCallback, *pQueueContext ))
            {
        ...
        //  Call setup to install other inf info for the various language groups.
        for (dwCtr = 0; dwCtr < dwMaxLayouts; dwCtr++)
        {
            if (InstallLanguageGroups[dwCtr] != LGRPID_WESTERN_EUROPE)
            {
                ...
                SetupInstallFromInfSection( HWND_DESKTOP, *phIntlInf, szBuffer, SPINST_ALL & ~SPINST_FILES, NULL, pSetupSourcePath, 0, NULL, NULL, NULL, NULL );
            }
        }
...
//  for the current locale (except for the 2-digit vs. 4-digit year).
{
    ...
    //  format setting.
    if ((GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SSHORTDATE, szBufCur, SIZE_64 )) && (GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SSHORTDATE | LOCALE_NOUSEROVERRIDE, szBufDef, SIZE_64 )))
    {
        ...
        while ((*pCur) && (*pCur == *pDef))
        {
            ...
            if (*pCur == CHAR_SML_Y)
            {
                if (((*(pCur + 1)) == CHAR_SML_Y) && ((*(pDef + 1)) == CHAR_SML_Y) && ((*(pDef + 2)) == CHAR_SML_Y) && ((*(pDef + 3)) == CHAR_SML_Y))
                {
                    bChange = TRUE;
                    pCur += 1;
                    pDef += 3;
...
BOOL Region_WinntUpgrade(PCTSTR pUnattendFile)
//  Determines whether NT-Setup is doing an upgrade or a fresh install.
{
    ...
    Buffer[0] = TEXT('\0');
    GetPrivateProfileString( TEXT("data"), TEXT("winntupgrade"), TEXT("no"), Buffer, sizeof(Buffer) / sizeof(TCHAR), pUnattendFile );
...
void Region_DoUnattendModeSetup(LPCTSTR pUnattendFile, BOOL AllowProgressBar)
...
//        do the right thing, so we have to roll our own.
{
    ...
    //  Don't display the FileCopy progress bar unless the user specifies it wants it.
    if (!AllowProgressBar)
    {
        g_bSetupCase = TRUE;
    }
    ...
    //  Now that the INF has been initialized, set this to TRUE to show that we're in setup mode.
    g_bSetupCase = TRUE;
    ...
    if ((SetupFindFirstLine( hFile, szRegionalSettings, szLanguageGroup, &Context )) && (dwNum = SetupGetFieldCount(&Context)))
    {
        bLangGroup = TRUE;
        for (dwCtr = 1; dwCtr <= dwNum; dwCtr++)
        {
            if (SetupGetStringField(&Context, dwCtr, szBuffer, MAX_PATH, NULL))
            {
                ...
                //  Enqueue the language group files so that they may be
                //  copied.  This only handles the CopyFiles entries in the inf file.
                wsprintf( szBuffer, TEXT("%ws%d"), szLGInstallPrefix, LanguageGroup );
                SetupInstallFilesFromInfSection( hIntlInf, NULL, FileQueue, szBuffer, pSetupSourcePath, SP_COPY_NEWER );
            }
        }
        ...
        {
            //  Copy the files in the queue.
            if (!SetupCommitFileQueue( HWND_DESKTOP, FileQueue, SetupDefaultQueueCallback, QueueContext ))
            {
                //  This can happen if the user hits Cancel from
                //  within the setup dialog.
        ...
        for (dwCtr = 1; dwCtr <= dwNum; dwCtr++)
        {
            if (SetupGetStringField(&Context, dwCtr, szBuffer, MAX_PATH, NULL))
            {
                ...
                wsprintf( szBuffer, TEXT("%ws%d"), szLGInstallPrefix, LanguageGroup );
                SetupInstallFromInfSection( HWND_DESKTOP, hIntlInf, szBuffer, SPINST_ALL & ~SPINST_FILES, NULL, pSetupSourcePath, 0, NULL, NULL, NULL, NULL );
            }
        }
    ...
    if (bWinntUpgrade)
    {
        Region_UpgradeKeyboardLayoutList( &hIntlInf, &FileQueue, &QueueContext);
    }

    //  Install the requested Language/Region information.  If a
    //  Language/Region was not specified, then install the requested System Locale, User Locale, and Input Locales.
    if ((SetupFindFirstLine( hFile, szRegionalSettings, szLanguage, &Context )) && (SetupGetStringField(&Context, 1, szBuffer, MAX_PATH, NULL)))
    {
        ...
        //  Install the Language as the System Locale and the User Locale, and then install all layouts associated with the Language.
    ...
    //  Make sure there was a valid Language setting.  If not, then look for the individual keywords.
    if (!bFound)
    {
        ...
        if ((SetupFindFirstLine( hFile, szRegionalSettings, szSystemLocale, &Context )) && (SetupGetStringField(&Context, 1, szBuffer, MAX_PATH, NULL)))
        {
        ...
        if ((SetupFindFirstLine( hFile, szRegionalSettings, szUserLocale, &Context )) && (SetupGetStringField(&Context, 1, szBuffer, MAX_PATH, NULL)))
        {
        ...
        {
            //  No input locales are specified, so install the default input locale for the system locale and/or user locale if they were specified.
            if (SystemLocale != 0)
            {
    ...
    if (!bFound && !bLangGroup)
    {
        ...
        if ((SetupFindFirstLine( hIntlInf, L"DefaultValues", L"Locale", &Context )) && (SetupGetStringField(&Context, 1, szBuffer, MAX_PATH, NULL)))
        {
            ...
            if ((SetupFindFirstLine( hIntlInf, L"Locales", szBuffer, &Context )) && (SetupGetStringField(&Context, 3, szBuffer, MAX_PATH, NULL)))
            {
                ...
                //  Call setup to install other inf info for the language group.
                SetupInstallFromInfSection( HWND_DESKTOP, hIntlInf, szBuffer, SPINST_ALL & ~SPINST_FILES, NULL, pSetupSourcePath, 0, NULL, NULL, NULL, NULL );
    ...
    Region_RunRegApps(c_szSysocmgr);//  Run any necessary apps (for FSVGA/FSNEC installation).

Region_UnattendModeExit:
    Region_CloseInf(hIntlInf, FileQueue, QueueContext);//  Close the inf file.
    SetupCloseInfFile(hFile);//  Close the unattend mode file.
}
...
BOOL Region_ReadDefaultLayoutFromInf(LPTSTR pszLocale, LPDWORD pdwLocale, LPDWORD pdwLayout, LPDWORD pdwLocale2, LPDWORD pdwLayout2, HINF hIntlInf)
{
    INFCONTEXT Context;
    ...
    szPair[0] = 0;
    if (SetupFindFirstLine( hIntlInf, TEXT("Locales"), pszLocale, &Context ))
    {
    ...
    //  Find the colon in the string and then set the position pointer to the next character.
    pPos = szPair;
    while (*pPos)
    {
    ...
    if ((*pPos) && (*pdwLocale = TransNum(szPair)) && (*pdwLayout = TransNum(pPos)))
    {
        if ((!pdwLocale2) || (!pdwLayout2) || (dwLangIn == LANGIDFROMLCID(*pdwLocale)))
        {
            return (TRUE);
        }
        ...
        //  when the Thai locale is chosen.  This is why we have two locales and layouts passed back to the caller.
        iField = 6;
        ...
        //  This should not happen, but do the right thing and only pass back the default layout if it does.
        return (TRUE);
    }
...
BOOL Region_GetDefaultLayoutFromInf(LPTSTR pszLocale, LPDWORD pdwLocale, LPDWORD pdwLayout, LPDWORD pdwLocale2, LPDWORD pdwLayout2)
{
    BOOL bRet = TRUE;
    HINF hIntlInf;
    ...
    {
        bRet = Region_ReadDefaultLayoutFromInf( pszLocale, pdwLocale, pdwLayout, pdwLocale2, pdwLayout2, hIntlInf );
...
BOOL Region_InstallDefaultLayout(LCID Locale, LPTSTR pszLocale)
{
    ...
    if (Region_GetDefaultLayoutFromInf( pszLocale, &dwLocale, &dwLayout, &dwLocale2, &dwLayout2 ) == FALSE)
    {
        //  Try just the language id.
        if (HIWORD(Locale) != 0)
        {
            wsprintf(szLCID, TEXT("%08x"), LANGIDFROMLCID(Locale));
            if (Region_GetDefaultLayoutFromInf( szLCID, &dwLocale, &dwLayout, &dwLocale2, &dwLayout2 ) == FALSE)
            {
                return (FALSE);
            }
        }
    ...
    //  Install the input locale / keyboard layout pair if it's not already installed.
    Region_InstallInputLocales(&LayoutList, 1);

    if (dwLayout2)
    {
        //  The first layout was for a different locale, so also install the next layout.
...
HKL Region_ActivateDefaultKeyboardLayout(DWORD dwLocale, DWORD dwLayout, LPTSTR pszLayout, HINF hIntlInf)
//   Sets the default input layout on the system, and broadcast to all running apps about this change.
{
    HKL hkl;

    hkl = Region_GetHKL( dwLocale, dwLayout, pszLayout, hIntlInf );
    if (hkl)
    {
        if (SystemParametersInfo( SPI_SETDEFAULTINPUTLANG, 0, (LPVOID)((LPDWORD) &hkl), 0 ))
        {
            DWORD dwRecipients = BSM_APPLICATIONS | BSM_ALLDESKTOPS;
            BroadcastSystemMessage( BSF_POSTMESSAGE, &dwRecipients, WM_INPUTLANGCHANGEREQUEST, 1, (LPARAM) hkl );
        }
    }
...
BOOL Region_SetPreloadLocale(LCID Locale, HINF hIntlInf, DWORD dwLayout)
//  Change the preload sort order to ensure that either the default keyboard or given keyboard for the locale is preloaded first.
{
    HKEY hKeyPreload,hKeySubst;
    TCHAR szValue[MAX_PATH];
    ...
    if (dwLayout == 0)
    {
        ...
        if (Region_GetDefaultLayoutFromInf( szValue, &dwLocale, &dwLayout, NULL, NULL ) == FALSE)
        {
            return (FALSE);
        }
    }
    ...
    rc = RegEnumValue( hKeyPreload, dwIndex, szValue, &cchValue, NULL, NULL, (LPBYTE)szData2, &cbData );
    if (rc != ERROR_SUCCESS)
    {
        //  We should never get here.  This means that there were no preload records at all.  Time to bail.
        RegCloseKey(hKeyPreload);
        RegCloseKey(hKeySubst);
        return (FALSE);
    }
    ...
    dwData = 0;
    cbData = sizeof(szData);
    if (RegQueryValueEx( hKeySubst, szData2, NULL, NULL, (LPBYTE)szData, &cbData ) == ERROR_SUCCESS)
    {
        dwData = TransNum(szData);
    }

    //  See if dwLayout matches the layout or substitute value.
    if (((dwData) && (dwData == dwLayout) && (LOWORD(dwLayout2) == dwLocale)) || ((!dwData) && (dwLayout2 == dwLayout) && (LOWORD(dwLayout2) == dwLocale)))
    {
        //  No need to do anything.  The new language is the first preload.
        //  All is happy.  Cleanup and depart.
        Region_ActivateDefaultKeyboardLayout( dwLayout2, dwData, szData, hIntlInf );
        RegCloseKey(hKeyPreload);
        RegCloseKey(hKeySubst);
        return (TRUE);
    }
    ...
    //  Start by putting the new default dwLayout into the first preload record.
    ...
    while (rc == ERROR_SUCCESS)
    {
        ...
        if (((dwData) && (dwData == dwLayout) && (LOWORD(dwLayout2) == dwLocale)) || ((!dwData) && (dwLayout2 == dwLayout) && (LOWORD(dwLayout2) == dwLocale)))
        {
            ...
            //  need to update the first entry, since the HIWORD may contain IME codes and the like.
            wsprintf(szData, TEXT("%08x"), dwLayout2);
            RegSetValueEx( hKeyPreload, szValue0, 0, REG_SZ, (LPBYTE)szData, (DWORD)(lstrlen(szData) + 1) * sizeof(TCHAR) );
            Region_ActivateDefaultKeyboardLayout( dwLayout2, dwData, szTemp, hIntlInf );
            RegFlushKey(hKeyPreload);
            RegCloseKey(hKeyPreload);
            RegCloseKey(hKeySubst);
            return (TRUE);
        ...
        //  Preserve the value from this record to stuff into the next record.
    ...
    //  We should never get here.  This means that the new default layout was not in the preload list at all.
    RegFlushKey(hKeyPreload);
    RegCloseKey(hKeyPreload);
    RegCloseKey(hKeySubst);
    return (TRUE);
...
BOOL Region_InstallAllLayouts(HINF hIntlInf, LCID Locale, LPTSTR pszLocale)
{
    ...
    //  Get the line in the inf file that contains the list of default input locales for the given locale.
    if (SetupFindFirstLine( hIntlInf, TEXT("Locales"), pszLocale, &Context ))
    {
        //  Install the list of input locale / keyboard layout pairs for the given locale.
