...
Abstract:
    This module implements the currency property sheet for the Regional Settings applet.
--*/
...
void Currency_DisplaySample(HWND hDlg)
//  Updates the currency sample.
//  It formats the currency based on the user's current locale settings.
//  It displays either a positive value or a negative value based on the Positive/Negative radio buttons.
{
    TCHAR szBuf[MAX_SAMPLE_SIZE];
    int nCharCount;

    //  Get the string representing the currency format for the positive sample currency and, if the the value is valid, display it.
    //  Perform the same operations for the negative currency sample.
    nCharCount = GetCurrencyFormat( UserLocaleID, 0, szSample_Number, NULL, szBuf, MAX_SAMPLE_SIZE );
    if (nCharCount)
    {
    ...
    nCharCount = GetCurrencyFormat( UserLocaleID, 0, szNegSample_Number, NULL, szBuf, MAX_SAMPLE_SIZE );
    if (nCharCount)
    {
...
void Currency_ClearValues(HWND hDlg)
{
...
void Currency_SetValues(HWND hDlg)
{
    HWND hCtrl1, hCtrl2;
    ...
    if (GetLocaleInfo(UserLocaleID, LOCALE_SMONGROUPING, szBuf, SIZE_128) && (szBuf[0]))
    {
        ...
        else
        {
            ...
            if ((szBuf[0] > CHAR_ZERO) && (szBuf[0] <= CHAR_NINE) && ((szBuf[1] == 0) || (lstrcmp(szBuf + 1, TEXT(";0")) == 0)))
            {
                ...
                if (GetCurrencyFormat(UserLocaleID, 0, szSample, &cfmt, szBuf, SIZE_128))
                {
                    ...
                    if (Index >= 0)
                    {
                        ComboBox_SetItemData( hCtrl2, Index, (LPARAM)((DWORD)cfmt.Grouping) );
                    }
                    else
                    {
                        Index = 0;
    ...
    if (Set_List_Values(hDlg, IDC_POS_CURRENCY_SYM, 0))
    {
        ...
        {
            ComboBox_SetCurSel( GetDlgItem(hDlg, IDC_POS_CURRENCY_SYM), StrToLong(szBuf) );
        }
        else
        {
            MessageBox(hDlg, szLocaleGetError, NULL, MB_OK | MB_ICONINFORMATION);
    ...
    if (Set_List_Values(hDlg, IDC_NEG_NUM_FORMAT, 0))
    {
        ...
        {
            ComboBox_SetCurSel( GetDlgItem(hDlg, IDC_NEG_NUM_FORMAT), StrToLong(szBuf) );
        }
        else
        {
            MessageBox(hDlg, szLocaleGetError, NULL, MB_OK | MB_ICONINFORMATION);
...
BOOL Currency_ApplySettings(HWND hDlg, BOOL bRedisplay)
{
    DWORD dwRecipients;
    ...
    {
        if (!Set_Locale_Values( hDlg, LOCALE_SCURRENCY, IDC_CURRENCY_SYMBOL, TEXT("sCurrency"), FALSE, 0, 0, NULL ))
        {
            return (FALSE);
        }
    }
    ...
    {
        if (!Set_Locale_Values( hDlg, LOCALE_ICURRENCY, IDC_POS_CURRENCY_SYM, TEXT("iCurrency"), TRUE, 0, 0, NULL ))
        {
            return (FALSE);
        }
    }
    ...
    {
        if (!Set_Locale_Values( hDlg, LOCALE_INEGCURR, IDC_NEG_NUM_FORMAT, TEXT("iNegCurr"), TRUE, 0, 0, NULL ))
        {
            return (FALSE);
        }
    }
    ...
    {
        if (!Set_Locale_Values( hDlg, LOCALE_SMONDECIMALSEP, IDC_DECIMAL_SYMBOL, 0, FALSE, 0, 0, NULL ))
        {
            return (FALSE);
        }
    }
    ...
    {
        if (!Set_Locale_Values( hDlg, LOCALE_ICURRDIGITS, IDC_NUM_DECIMAL_DIGITS, TEXT("iCurrDigits"), TRUE, 0, 0, NULL ))
        {
            return (FALSE);
        }
    }
    ...
    {
        if (!Set_Locale_Values( hDlg, LOCALE_SMONTHOUSANDSEP, IDC_DIGIT_GROUP_SYMBOL, 0, FALSE, 0, 0, NULL ))
        {
            return (FALSE);
        }
    }
    ...
    {
        if (!Set_Locale_Values( hDlg, LOCALE_SMONGROUPING, IDC_NUM_DIGITS_GROUP, 0, TRUE, 0, TEXT(";0"), NULL ))
        {
            return (FALSE);
        }
    }
...
BOOL Currency_ValidatePPS(HWND hDlg, LPARAM Changes)
{
    ...
    //  If the currency symbol has changed, ensure that there are no digits contained in the new symbol.
    if ((Changes & CC_SCurrency) && Item_Has_Digits(hDlg, IDC_CURRENCY_SYMBOL, FALSE))
    {
        No_Numerals_Error(hDlg, IDC_CURRENCY_SYMBOL, IDS_LOCALE_CURR_SYM);
        return (FALSE);
    }

    //  If the currency's decimal symbol has changed, ensure that there are no digits contained in the new symbol.
    if ((Changes & CC_SMonDec) && Item_Has_Digits(hDlg, IDC_DECIMAL_SYMBOL, FALSE))
    {
        No_Numerals_Error(hDlg, IDC_DECIMAL_SYMBOL, IDS_LOCALE_CDECIMAL_SYM);
        return (FALSE);
    }

    //  If the currency's thousands grouping symbol has changed, ensure that there are no digits contained in the new symbol.
    if ((Changes & CC_SMonThousand) && Item_Has_Digits(hDlg, IDC_DIGIT_GROUP_SYMBOL, FALSE))
    {
...
void Currency_InitPropSheet(HWND hDlg, LPARAM lParam)
{
...
INT_PTR CALLBACK CurrencyDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case ( WM_NOTIFY ) :
        {
            ...
            switch (lpnm->code)
            {
                ...
                case ( PSN_KILLACTIVE ) :
                {
                    ...
                    SetWindowLongPtr( hDlg, DWLP_MSGRESULT, !Currency_ValidatePPS( hDlg, lpPropSheet->lParam) );
                    break;
                }
                case ( PSN_APPLY ) :
                {
        ...
        case ( WM_HELP ) :
        {
            WinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD_PTR)(LPTSTR)aCurrencyHelpIds );
            break;
        }
        case ( WM_CONTEXTMENU ) :      // right mouse click
        {
            WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD_PTR)(LPTSTR)aCurrencyHelpIds );
            break;
        }
        case ( WM_COMMAND ) :
        {
