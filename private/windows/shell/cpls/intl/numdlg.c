    ...
    HWND hDlg)
{
    ...
    if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, LANGUAGE_GROUPS_KEY, 0L, KEY_READ, &hKey ) == ERROR_SUCCESS)
    {
            ...
            if (c_szDigitsPerLangGroup[Ctr1][0] != 0)
            {
                ...
                if (cbData > sizeof(TCHAR))
                {
                    ...
                    for (Ctr2 = 0; Ctr2 < MAX_DIGITS_PER_LG; Ctr2++)
                    {
                        ...
                        {
                            if (ComboBox_FindStringExact(hCtrl1, -1, c_szNativeDigits[Index] ) == CB_ERR)
                            {
                                ComboBox_AddString( hCtrl1, c_szNativeDigits[Index] );
                            }
                        }
                    }
                }
