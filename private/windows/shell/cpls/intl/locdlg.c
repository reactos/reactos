    ...
    LPLANGNODE pLangNode)
{
    ...
    if (pLangNode->wStatus & LANG_IME)
    {
        ...
        {
            GetAtomName( g_lpLayout[pLangNode->iLayout].atmIMEFile, szFileName, MAX_PATH );
            ExtractIconEx(szFileName, 0, (HICON *)&hIcon, NULL, 1);

            if (hIcon)
            {
                pLangNode->nIconIME = ImageList_AddIcon( g_himIndicators, hIcon );
            }
            else
            {
    ...
    HWND hwnd)
{
    ...
    if (RegOpenKeyEx( HKEY_CURRENT_USER, szKbdLayouts, 0, KEY_ALL_ACCESS, &hkeyLayouts ) == ERROR_SUCCESS)
    {
    ...
    if (RegCreateKey( HKEY_CURRENT_USER, szKbdLayouts, &hkeyLayouts ) == ERROR_SUCCESS)
    {

        //  Create the HKCU\Keyboard Layout\Substitutes key.

        if (RegCreateKey( hkeyLayouts, szSubstKey, &hkeySubst ) == ERROR_SUCCESS)
        {

            //  Create the HKCU\Keyboard Layout\Preload key.

            if (RegCreateKey( hkeyLayouts, szPreloadKey, &hkeyPreload ) == ERROR_SUCCESS)
            {
    ...
    if (RegCreateKey( HKEY_CURRENT_USER, REGSTR_PATH_RUN, &hkeySubst ) != ERROR_SUCCESS)
    {
    ...
    if (IsDlgButtonChecked(hwnd, IDC_KBDL_INDICATOR))
    {
        ...
        if (hkeySubst)
        {
            RegSetValueEx( hkeySubst, szInternat, 0, REG_SZ, (LPBYTE)szInternat, sizeof(szInternat) );
        }
    }
    else
    {
    ...
    if (RegCreateKey( HKEY_CURRENT_USER, szScanCodeKey, &hkeyScanCode ) == ERROR_SUCCESS)
    {
        if (szShiftL[0])
        {
            RegSetValueEx( hkeyScanCode, szValueShiftLeft, 0, REG_SZ, (LPBYTE)szShiftL, (DWORD)(lstrlen(szShiftL) + 1) * sizeof(TCHAR) );
        }
...
BOOL Locale_FileExists(LPTSTR pFileName)
{
...
BOOL Locale_LoadLayouts(HWND hwnd)
{
    HKEY hKey;
    HKEY hkey1;
    DWORD cb;
    ...
    dwIndex = 0;
    dwRetVal = RegEnumKey( hKey, dwIndex, szValue, sizeof(szValue) / sizeof(TCHAR) );
    ...
    LPPROPSHEETPAGE psp)
{
    ...
    if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, szLocaleInfo, 0L, KEY_WRITE, &hKey ) == ERROR_SUCCESS)
    {
