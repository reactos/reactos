...
static const TCHAR c_szLanguages[] = TEXT("System\\CurrentControlSet\\Control\\Nls\\Language");

static const TCHAR c_szControlPanelIntl[] = TEXT("Control Panel\\International");
...
//  Function Prototypes.
void DoProperties(HWND hwnd, LPCTSTR pCmdLine);




//  This routine is called from LibInit to perform any initialization that is required.
...
BOOL CreateGlobals()
{
    ...
    if (RegOpenKeyEx( HKEY_LOCAL_MACHINE, c_szLanguages, 0L, KEY_READ, &hKey ) == ERROR_SUCCESS)
    {
    ...
    if (RegOpenKeyEx( HKEY_CURRENT_USER, c_szControlPanelIntl, 0L, KEY_READ, &hKey ) == ERROR_SUCCESS)
    {
        //  Query the locale id.
        szData[0] = 0;
        cbData = sizeof(szData);
...
BOOL IsRtLLocale(LCID iLCID)
{
    ...
    if (GetLocaleInfo( iLCID, LOCALE_FONTSIGNATURE, (LPTSTR) &wLCIDFontSignature, (sizeof(wLCIDFontSignature) / sizeof(TCHAR)) ))
    {
