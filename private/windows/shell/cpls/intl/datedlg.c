    ...
    HWND hHijriComboBox)
{
    ...
    if (iIndex != CB_ERR)
    {
        ...
        {
            RegSetValueEx( hKey, c_szAddHijriDate, 0, REG_SZ, (LPBYTE)c_szAddHijriDateValues[iIndex], (lstrlen(c_szAddHijriDateValues[iIndex]) + 1) * sizeof(TCHAR) );
            RegCloseKey(hKey);
        }
    }
}
    ...
    HWND hDlg)
{
    ...
    if (RegOpenKeyEx( HKEY_CURRENT_USER, c_szInternational, 0, KEY_READ | KEY_WRITE, &hKey ) == ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueEx( hKey, c_szAddHijriDate, NULL, NULL, (LPBYTE)szCurrentValue, &DataLen ) != ERROR_SUCCESS)
        {
