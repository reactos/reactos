    ...
    )
{
    ...
    case WM_HELP:      // F1
        ...
        (DWORD) (LPSTR) aVirtualMemHelpIds);
        break;
...
void GetCurrRSL( LPINT pcmRSL, LPINT pcmUsed, LPINT pcmPPLim )
{
    ...
    Status = NtQuerySystemInformation(SystemRegistryQuotaInformation, &srqi, sizeof(srqi), NULL);
    if (NT_SUCCESS(Status)) {
...
*/
INT VirtualMemComputeAllocated( HWND hWnd )
{
    BOOL fSuccess = FALSE;
    ...
    __try {
        ...
        result = NtQuerySystemInformation(SystemPageFileInformation, pPagefileInfo, PAGEFILE_INFO_BUFFER_SIZE, NULL);
        ...
        // If the system created a temporary pagefile, ignore it
        // so as not to confuse users
        if ((ERROR_SUCCESS == lResult) && fTempPagefile) {
            // If the system-created temp pagefile is the only
            // pagefile, we can just return zero, otherwise
            // we need to advance to the second pagefile
            // before we start accumulating sizes

            if (!pCurrentPagefile->NextEntryOffset) {
                fSuccess = TRUE;
                __leave;
            } // if (!(pCurrentPagefile->NextEntryOffset))
        ...
        // Add up any remaining pagefile sizes
        ...
        ulPagefileSize *= SysInfo.dwPageSize;

        // Convert bytes to MB
        ulPagefileSize /= ONE_MEG;
    ...
    )
{
    ...
    SetDlgItemMB(hDlg, IDD_VM_ALLOCD, VirtualMemComputeAllocated(hDlg));
}
...
static BOOL VirtualMemSetNewSize(HWND hDlg)
{
    ...
    if (acdfControls[CD_WRITE].u.fValue) {
        nBootPF = -1;
    } else if (acdfControls[CD_LOG].u.fValue || acdfControls[CD_SEND].u.fValue) {
...
)
{
    ...
    __try {
        ...
        lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, szSessionManager, 0L, KEY_READ | KEY_WRITE, &hKey);
        ...
        lResult = RegQueryValueEx(hKey, szPendingRename, 0L, &dwValueType, (LPBYTE) NULL, &cbRegistry);
        if (ERROR_SUCCESS != lResult) {
        ...
        if (ERROR_SUCCESS == lResult) {
            lResult = RegQueryValueEx(hKey, szPendingRename, 0L, &dwValueType, (LPBYTE) szBuffer, &cbRegistry);
            if (ERROR_SUCCESS != lResult) {
