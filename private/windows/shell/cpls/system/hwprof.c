...
BOOL CopyHardwareProfile(HWND   hDlg, ULONG  ulIndex, ULONG  ulProfile, LPTSTR szNewFriendlyName);
BOOL RenameHardwareProfile(HWND   hDlg, ULONG  ulIndex, ULONG  ulProfile, LPTSTR szNewFriendlyName);
BOOL DeleteHardwareProfile(HWND   hDlg, ULONG  ulIndex);
...
BOOL InsertRank(PHWPROF_INFO   pInfo, ULONG          ulRank);
...
BOOL SwapPreferenceOrder(HWND  hDlg, HWND  hList, ULONG ulIndex1, ULONG ulIndex2);
BOOL DeleteProfileDependentTree(ULONG ulProfile);
BOOL CopyRegistryNode(HKEY     hSrcKey, HKEY     hDestKey);
BOOL DeleteRegistryNode(HKEY     hNodeKey, LPTSTR   szSubKey);
BOOL StripCurrentTag(
...

{
   ...
   switch (uMessage)
   {
      case WM_INITDIALOG:
         ...
         LoadString(hInstance, HWP_DEF_FRIENDLYNAME, pszRegDefaultFriendlyName, MAX_FRIENDLYNAME_LEN);
         ...
         SendDlgItemMessage(hDlg, IDD_HWP_ORDERUP, BM_SETIMAGE, (WPARAM)IMAGE_ICON, (LPARAM)LoadIcon(hInstance, MAKEINTRESOURCE(UP_ICON)));
         SendDlgItemMessage(hDlg, IDD_HWP_ORDERDOWN, BM_SETIMAGE, (WPARAM)IMAGE_ICON, (LPARAM)LoadIcon(hInstance, MAKEINTRESOURCE(DOWN_ICON)));
         ...
         SendDlgItemMessage(hDlg, IDD_HWP_SECSCROLL, UDM_SETRANGE, 0, MAKELONG((SHORT)MAX_USER_WAIT, (SHORT)MIN_USER_WAIT));
         SendDlgItemMessage(hDlg, IDD_HWP_SECONDS, EM_LIMITTEXT, 3, 0L);
         GetUserWaitInterval(&ulWait);
         if (ulWait == 0xFFFFFFFF) {
             CheckRadioButton(hDlg, IDD_HWP_WAITFOREVER, IDD_HWP_WAITUSER, IDD_HWP_WAITFOREVER);
            SendDlgItemMessage(hDlg, IDD_HWP_SECSCROLL, UDM_SETPOS, 0, DEFAULT_USER_WAIT);
            EnableWindow(GetDlgItem(hDlg, IDD_HWP_SECONDS), FALSE);
            EnableWindow(GetDlgItem(hDlg, IDD_HWP_SECSCROLL), FALSE);
         }
         else {
            CheckRadioButton(hDlg, IDD_HWP_WAITFOREVER, IDD_HWP_WAITUSER, IDD_HWP_WAITUSER);
         ...
         return 0;
      case WM_HELP:
         WinHelp((HWND)((LPHELPINFO)lParam)->hItemHandle, HELP_FILE, HELP_WM_HELP, (DWORD)(LPTSTR)HwProfileHelpIds);
         break;
      case WM_CONTEXTMENU:
         WinHelp((HWND)wParam, HELP_FILE, HELP_CONTEXTMENU, (DWORD)(LPTSTR)HwProfileHelpIds);
         break;
      case WM_DESTROY:
          ...
          if (pInfo) {
              ...
              hIcon = (HICON)SendDlgItemMessage(hDlg, IDD_HWP_ORDERUP, BM_GETIMAGE, 0, 0);
              if (hIcon) {
                 DeleteObject(hIcon);
              }

              hIcon = (HICON)SendDlgItemMessage(hDlg, IDD_HWP_ORDERDOWN, BM_GETIMAGE, 0, 0);
              if (hIcon) {
                 DeleteObject(hIcon);
              }
          }
      ...
      case WM_COMMAND:
      {
         switch (LOWORD(wParam))
         {
            case IDOK:
                if (bAdmin) {
                    ...
                    if (IsDlgButtonChecked(hDlg, IDD_HWP_WAITFOREVER)) {
                        ...
                        ulWait = GetDlgItemInt(hDlg, IDD_HWP_SECONDS, &Status, FALSE);
                        if (!Status  ||  ulWait > MAX_USER_WAIT) {
                            ...
                            MessageBox(hDlg, szMsg, szCaption, MB_OK | MB_ICONEXCLAMATION);
            ...
            case IDD_HWP_PROFILES:
               ...
               if (bAdmin) {
                   if (HIWORD(wParam) == LBN_DBLCLK) {
                      ...
                      if ((ulIndex = (ULONG)SendMessage((HWND)lParam, LB_GETCURSEL, 0, 0)) == LB_ERR) {
                         break;
                      }

                      if ((lValue = SendMessage((HWND)lParam, LB_GETITEMDATA, ulIndex, 0)) == LB_ERR) {
                         break;
                      }
            ...
            case IDD_HWP_PROPERTIES:
               ...
               pInfo->ulSelectedProfile = SendMessage(hList, LB_GETITEMDATA, ulIndex, 0);
            ...
            case IDD_HWP_COPY:
               ...
               SendMessage(hList, LB_GETTEXT, ulIndex, (LPARAM)(LPCTSTR)HwSelectedProfile.szFromFriendlyName);
               HwSelectedProfile.ulFromProfileID = SendMessage(hList, LB_GETITEMDATA, ulIndex, 0);

               // strip off the current tag if it exists "(Current)"
               GetCurrentProfile(&ulCurrentProfile);

               StripCurrentTag(HwSelectedProfile.szFromFriendlyName, HwSelectedProfile.ulFromProfileID, ulCurrentProfile);

               // pass selected profile info to copy profile dialog box
               HwSelectedProfile.hParent = hDlg;

               if (!DialogBoxParam(hInstance, (LPTSTR)MAKEINTRESOURCE(DLG_HWP_COPY), hDlg, (DLGPROC)CopyProfileDlg, (LPARAM)&HwSelectedProfile)) {
               ...
               CopyHardwareProfile(hDlg, ulIndex, HwSelectedProfile.ulFromProfileID, HwSelectedProfile.szToFriendlyName);
            ...
            case IDD_HWP_RENAME:
               ...
               HwSelectedProfile.hParent = hDlg;
               if (!DialogBoxParam(hInstance, (LPTSTR)MAKEINTRESOURCE(DLG_HWP_RENAME), hDlg, (DLGPROC)RenameProfileDlg, (LPARAM)&HwSelectedProfile)) {
               ...
               // and update the display
               RenameHardwareProfile(hDlg, ulIndex, HwSelectedProfile.ulFromProfileID, HwSelectedProfile.szToFriendlyName);
            ...
            case IDD_HWP_DELETE: {
               ...
               // profile name)
               SendMessage(hList, LB_GETTEXT, ulIndex, (LPARAM)(LPCTSTR)szProfileName);
      ...
      )
{
   ...
   switch (uMessage)
   {
      case WM_INITDIALOG:
         ...
         SendDlgItemMessage(hDlg, IDD_HWP_COPYTO, EM_LIMITTEXT, MAX_FRIENDLYNAME_LEN-1, 0L);
      ...
      case WM_COMMAND:
      {
         switch (LOWORD(wParam))
         {
            case IDOK:
               ...
               GetDlgItemText(hDlg, IDD_HWP_COPYTO, pHwProfile->szToFriendlyName, MAX_FRIENDLYNAME_LEN);

               if (pHwProfile->szToFriendlyName == NULL || *pHwProfile->szToFriendlyName == '\0') {
      ...
      case WM_COMMAND:
      {
         switch (LOWORD(wParam))
         {
            case IDOK:
               ...
               ulReturn = GetDlgItemText(hDlg, IDD_HWP_RENAMETO, pHwProfile->szToFriendlyName, MAX_FRIENDLYNAME_LEN);
               if (pHwProfile->szToFriendlyName == NULL || *pHwProfile->szToFriendlyName == '\0') {
                  // accept request to copy to zero-length string but
                  // do nothing (return FALSE from DialogBox call)
                  EndDialog(hDlg, FALSE);
               ...
               if (lstrcmpi(pHwProfile->szToFriendlyName, pHwProfile->szFromFriendlyName) == 0) {
   ...
   while (RegStatus != ERROR_NO_MORE_ITEMS) {
      ...
      if (RegStatus == ERROR_SUCCESS) {
         ...
         if (RegQueryValueEx(
            ...
            if (bAdmin) {
                lstrcpy(pInfo->pHwProfValues[ulIndex].szFriendlyName, pszRegDefaultFriendlyName);
         ...
         ulSize = sizeof(ULONG);
         if (RegQueryValueEx(hCfgKey, pszRegPreferenceOrder, NULL, NULL, (LPBYTE)&pInfo->pHwProfValues[ulIndex].ulPreferenceOrder, &ulSize) != ERROR_SUCCESS) {
   ...
   if (RegOpenKeyEx(
       ...
       if (RegQueryValueEx(hKey, pszRegIsPortable, NULL, NULL,
          ...
          if (bAdmin) {
              ...
              RegSetValueEx(hKey, pszRegIsPortable, 0, REG_DWORD, (LPBYTE)&pInfo->bPortable, sizeof(DWORD));
          }
       }
       RegCloseKey(hKey);
   }
      ...
      )
{
   ...
   if (bAdmin) {
       ...
       if (RegSetValueEx(hKey, pszRegUserWaitInterval, 0, REG_DWORD, (LPBYTE)&ulWait, sizeof(ULONG)) != ERROR_SUCCESS) {
          RegCloseKey(hKey);
          return FALSE;
       }
      ...
      )
{
   ...
   if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, pszRegIDConfigDB, 0, KEY_SET_VALUE, &hKey) == ERROR_SUCCESS) {
       RegSetValueEx(hKey, pszRegIsPortable, 0, REG_DWORD, (LPBYTE)&pInfo->bPortable, sizeof(DWORD));
       RegCloseKey(hKey);
   }
...
BOOL WriteProfileInfo(PHWPROF_VALUES pProfValues)
{
   ...
   wsprintf(RegStr, TEXT("%s\\%s\\%04u"), pszRegIDConfigDB, pszRegKnownDockingStates, pProfValues->ulProfile);

   // create the profile key if it's a new profile.
   // Don't worry about security because the profile subkey always inherits the security of the parent Hardware Profiles key.
   if (pProfValues->ulAction & HWP_CREATE) {
      if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, RegStr, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, NULL) != ERROR_SUCCESS) {
         return FALSE;
      }
      ...
      RegSetValueEx(hKey, pszRegHwProfileGuid, 0, REG_SZ, (LPBYTE)UuidString, (lstrlen(UuidString) + 1) * sizeof(WCHAR));
      RpcStringFree(&UuidString);
      UuidString = NULL;
   } else {// if not a create, just open the existing key
      if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, RegStr, 0, KEY_SET_VALUE, &hKey) != ERROR_SUCCESS) {
         return FALSE;
      }
   }

   // update preference order if modified
   if ((pProfValues->ulAction & HWP_REORDER) || (pProfValues->ulAction & HWP_CREATE)) {
      RegSetValueEx(hKey, pszRegPreferenceOrder, 0, REG_DWORD, (LPBYTE)&pProfValues->ulPreferenceOrder, sizeof(ULONG));
      pProfValues->ulAction &= ~HWP_REORDER;    // clear action
   }

   // update friendly name if modified
   if ((pProfValues->ulAction & HWP_RENAME) || (pProfValues->ulAction & HWP_CREATE)) {
      RegSetValueEx(hKey, pszRegFriendlyName, 0, REG_SZ, (LPBYTE)pProfValues->szFriendlyName, (lstrlen(pProfValues->szFriendlyName)+1) * sizeof(TCHAR));
      pProfValues->ulAction &= ~HWP_RENAME;     // clear action
   }

   // update property values if modified
   if ((pProfValues->ulAction & HWP_PROPERTIES) || (pProfValues->ulAction & HWP_CREATE)) {
      RegSetValueEx(hKey, pszRegDockState, 0, REG_DWORD, (LPBYTE)&pProfValues->ulDockState, sizeof(ULONG));
      RegSetValueEx(hKey, pszRegAliasable, 0, REG_DWORD, (LPBYTE)&pProfValues->bAliasable, sizeof(DWORD));

      //RegSetValueEx(hKey, pszRegIsPortable, 0, REG_DWORD, (LPBYTE)&pProfValues->bPortable, sizeof(BOOL));

      RegSetValueEx(hKey, pszRegDockID, 0, REG_SZ, (LPBYTE)pProfValues->szDockID, (lstrlen(pProfValues->szDockID)+1) * sizeof(TCHAR));
      RegSetValueEx(hKey, pszRegSerialNumber, 0, REG_SZ, (LPBYTE)pProfValues->szSerialNumber, (lstrlen(pProfValues->szSerialNumber)+1) * sizeof(TCHAR));

      pProfValues->ulAction &= ~HWP_PROPERTIES;    // clear action
   }

   if (pProfValues->ulAction & HWP_CREATE) {
      // copy the profile enum info. Don't worry about security on this createkey because the profile key always inherits the security of the parent Hardware Profiles key.
      wsprintf(RegStr, TEXT("%s\\%04u"), pszRegHwProfiles, pProfValues->ulProfile);
      RegCreateKeyEx(HKEY_LOCAL_MACHINE, RegStr, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hDestKey, NULL);
      wsprintf(RegStr, TEXT("%s\\%04u"), pszRegHwProfiles, pProfValues->ulCreatedFrom);
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, RegStr, 0, KEY_ALL_ACCESS, &hSrcKey);
   ...
   )
{
   ...
   for (i=0; RegStatus == ERROR_SUCCESS; i++) {
      ...
      RegStatus = RegEnumValue(hSrcKey, i, szString, &ulLength, NULL, &ulType, Data, &ulDataSize);
        if (RegStatus == ERROR_SUCCESS) {
...
VOID DisplaySystemMessage(HWND  hWnd, UINT  uiSystemError)
{
   ...
   FormatMessage(FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM, NULL, uiSystemError, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), szMessage, MAX_PATH, NULL);
...
BOOL ValidateAsciiString(IN LPTSTR  pszString)
{
   LPTSTR p = NULL;
...
BOOL UpdateOrderButtonState(HWND  hDlg)
{
   ...
   pInfo = (PHWPROF_INFO)GetWindowLong(hDlg, DWL_USER);
   hList = GetDlgItem(hDlg, IDD_HWP_PROFILES);
   if ((ulIndex = (ULONG)SendMessage(hList, LB_GETCURSEL, 0, 0)) == LB_ERR) {
      return FALSE;
   }
