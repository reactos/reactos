...
VOID InitGeneralDlg(HWND hDlg)
{
    ...
    if( GetPrivateProfileString( c_szOemGenSection, c_szOemName, c_szEmpty, _scr1, ARRAYSIZE( _scr1 ), oemfile ) )
    {
        SetDlgItemText( hDlg, ctlid++, _scr1 );

        if( GetPrivateProfileString( c_szOemGenSection, c_szOemModel, c_szEmpty, _scr1, ARRAYSIZE( _scr1 ), oemfile ) )
        {
            SetDlgItemText( hDlg, ctlid++, _scr1 );
        }
        ...
        if( GetPrivateProfileString( c_szOemSupportSection, _scr2, c_szEmpty, _scr1, ARRAYSIZE( _scr1 ), oemfile ) )
        {
        ...
        if( SetClearBitmap( GetDlgItem( hDlg, IDC_GEN_OEM_IMAGE ), oemfile, SCB_FROMFILE ) )
        {
                               ...
                               WPARAM wParam, LPARAM lParam )
{
    switch( uMsg ) {
        ...
        case WM_INITDIALOG:
        {
            ...
            GetPrivateProfileString( c_szOemGenSection, c_szOemName, c_szEmpty, text, sizeof( text ), oemfile );
            ...
            for( ;; i++ )
            {
                ...
                GetPrivateProfileString( c_szOemSupportSection, line, c_szDefSupportLineText, text, sizeof( text ) - 2,
                    oemfile );
