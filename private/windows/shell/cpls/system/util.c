...
Routine Description:
    SkipWhiteSpace
    For the purposes of this fuction, whitespace is space, tab, cr, or lf.
Arguments:
    sz -
...
--*/
{
    ...
    do {
        ...
        if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            // We found a file.  Set the file attributes, and try to delete it.
            if ((fd.dwFileAttributes & FILE_ATTRIBUTE_READONLY) || (fd.dwFileAttributes & FILE_ATTRIBUTE_SYSTEM)) {
...
--*/
{
    ...
    if (!AdjustTokenPrivileges( hToken, FALSE, &tp, dwSize, lpPrevPrivilages, &dwSize )) {
...
--*/
{
    ...
    // First, see if we can create the directory without having to build parent directories.
    ...
    if (szDirectory[1] == TEXT(':')) {
        ...
        // Skip the slash, and find the slash between the share name and the directory name.
        lpEnd++;
...
Arguments:
    hKey -
        Supplies a handle to a registry key which will be the parent of the created key.
    lpSubKey -
...
--*/
{
    ...
    if (n != LB_ERR)
    {
        for (i = 0; i < n; i++)
        {
            if (SendMessage (hCtrl, LVM_GETITEMSTATE, i, (LPARAM) LVIS_SELECTED) == LVIS_SELECTED) {
                return i;
            }
        }
    }
...
Arguments:
    hWnd -
        Supplies parent window handle
    wText -
        Supplies ID of a printf-like format string to display as the message box text
    wCaption -
