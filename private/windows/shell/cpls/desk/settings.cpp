...
#pragma message( __FILE__"(37): warning : testing with one mon" )
#   undef EnumDisplaySettingsEx
#   define EnumDisplaySettingsEx(a,b,c,d) EnumDisplaySettings(a,b,c)
#endif
**/
...
void CList::Process()
{
    ...
    // to show 16 color modes, or small modes.
...
BOOL CDEVMODELST::BuildList(LPTSTR pszDevName, HWND hwnd ) {
    ...
    DeskDebugPrint(("Desk.cpl: Root of linked list of modes = %08lx\n", &cdmHead));
...
// This code a quick hack to call the above reg object from the
// multimon page code to get the display description if the
// EnumDisplayDevices string comes back empty.
...
BOOL CDLGCHGADAPTOR::InitDlg(HWND hwndFocus) {
    ...
    LPTSTR pfileVersions = fileVersions;
    HDC hdc;
    ULONG DrivVer;
    ...
    wsprintf(driverName, TEXT("drivers\\%s.sys"), crvo.GetMiniPort());


    // Open the file version resource for the driver


    CFILEVER cfv(driverName);
    ...
    pfileVersions += wsprintf(pfileVersions, TEXT("%s"), cfv.GetFileVer());
    ...
    if (psz) {
        ...
        // for each display driver, get the version info.


        while (next && (*pszTmp != NULL)) {


            // Go to the next name


            while ((*pszTmp != NULL) && (*pszTmp != TEXT(','))) {
                pszTmp++;
            }
            ...
            *pszTmp = NULL;
            ...
            CFILEVER cfv(pszTmp1);
            pfileVersions += wsprintf(pfileVersions, TEXT(", %s"), cfv.GetFileVer());

            // Skip over the comma and the space characters.
    ...
    // Get the version number from all files, and append "," and the
...
//    }
    ...
    // 0> The display passed in lParam
    // 1) The primary - unless VGA
    // 2) A non-primary, to avoid VGA.
    // 3) Anything


    this->fUsingDefault = HIWORD(this->lParam);
    ...
    if (this->fUsingDefault) {
        ...
        DISPLAY_DEVICE displayDevice;
        int iDevNum = LOWORD(this->lParam);
        ...
        if (EnumDisplayDevices(NULL, iDevNum, &displayDevice, 0))
        {
            ...
            //BUGBUG - what should we do if we can't open the requested driver
        } else {
            bBadDriver = TRUE;
        }
    }
...
{
    HDC hdc = GetDC(NULL);
    BOOL fPalette = GetDeviceCaps(hdc, RASTERCAPS) & RC_PALETTE;
    ReleaseDC(NULL, hdc);
    return fPalette;
}
    ...
    case ID_DSP_FONTSIZE:
        ...
        case CBN_SELCHANGE:
            ...
            if (gbExecMode == EXEC_SETUP) {
                ...
                int iCurSel;
                iCurSel = this->SendCtlMsg(ID_DSP_FONTSIZE, CB_GETCURSEL, 0, 0);

                if (iCurSel == this->idCustomFonts) {
                    int cdpi;
...
\**/
PCDISPLAYDLG NewDisplayDialogBox(HINSTANCE hmod, LPARAM lParam) {
    ...
    )
{
    ...
    switch (msg) {
    case WM_INITDIALOG:
        pddDialog = NewDisplayDialogBox(hInstance, ((LPPROPSHEETPAGE)lParam)->lParam);
    ...
    DWORD dwTextID, ... )
{
    ...
    if (!FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_MAX_WIDTH_MASK, ghmod, dwTextID, 0, (LPTSTR)&pszMsg, 1, &marker)) {
        pszMsg = gpszError;
    }
    ...
    if (fSound) {
        MessageBeep(fuStyle & (MB_ICONASTERISK | MB_ICONEXCLAMATION | MB_ICONHAND | MB_ICONQUESTION | MB_OK));
    }
...
LPTSTR FmtSprint(DWORD id, ... ) {
    ...
    if (!FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_MAX_WIDTH_MASK, ghmod, id, 0, (LPTSTR)&pszMsg, 1, &marker)) {
        GetLastError();
        pszMsg = TEXT("...");
    }
    va_end(marker);
    ...
    if (hdskDefault != NULL) {
        ...
        if (hdsk != NULL) {
            ...
            if (SetThreadDesktop(hdsk)) {
                ...
                Sleep(7000);// Sleep for some seconds so you have time to look at the screen.
            }
        }
