    ...
    )
{
    ...
    if (SCMHandle = OpenSCManager(NULL, NULL, SC_MANAGER_ALL_ACCESS))
    {
        if (ServiceHandle = OpenService(SCMHandle, ServiceName, SERVICE_ALL_ACCESS))
        {
            ...
            if (ServiceConfig = (LPQUERY_SERVICE_CONFIG) LocalAlloc(LPTR, ServiceConfigSize))
            {
...
DWORD DeskSCStartService(LPWSTR ServiceName)
{
    SC_HANDLE SCMHandle;
    SC_HANDLE ServiceHandle;
...
BOOL DeskSCStopService(LPWSTR ServiceName)
{
    SC_HANDLE      SCMHandle;
    SC_HANDLE      ServiceHandle;
    BOOL           bRet = FALSE;
    ...
    )
{
    ...
    if ((maxmem = maxmem * numDev) > 10)
    {
                           ...
                           &disposition) == ERROR_SUCCESS)
        {
            ...
            if ( (RegQueryValueEx(hkey, TEXT("SystemPages"), NULL, NULL, (LPBYTE)(&data), &cb) != ERROR_SUCCESS) || (data < maxmem) )
            {
                ...
                SetupDiGetDeviceInstallParams(hDevInfo, pDeviceInfoData, &DeviceInstallParams);
    ...
    do {
        ...
        else
        {
            ...
            wsprintf(keyName, TEXT("System\\CurrentControlSet\\Services\\%ws\\Device%d"), pServiceName, numDev);
...
#if ANDREVA_DBG
    hkey = NULL;
    RegCreateKeyEx(HKEY_LOCAL_MACHINE, regstring, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hkey, &disposition);
    if (hkey) {
        RegCloseKey(hkey);
    }
#endif
