...
detect_next:
        ...
        wsprintf(DeviceInst, TEXT("Root\\LEGACY_%ws\\0000"), szServiceName);
        ...
        {
            // Set the parameters for the installation to do no file copies
            ...
            {
                SetupDiDestroyDriverInfoList(hDevInfo, &DeviceInfoData, SPDIT_CLASSDRIVER);
            }
        }
        else
        {
                                                               ...
                                                               "ExtensionPropSheetPageProc")))
                        {
                                                 ...
                                                 &hPage))
                            {
                                ...
                                else
                                {
                                    ...
                                    CM_Get_First_Log_Conf(&LogConfig, DeviceInfoData.DevInst, FORCED_LOG_CONF);
    ...
    )
{
    ...
    else if (err != ERROR_CANCELLED)
    {
        ...
        FmtMessageBox(hwnd, MB_ICONSTOP | MB_OK, FALSE, ID_DSP_TXT_INSTALL_DRIVER, ID_DSP_TXT_DRIVER_INSTALLED_FAILED);
    }
