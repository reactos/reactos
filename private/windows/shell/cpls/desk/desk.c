...
PAGEINFO aPageInfo[] = {
    ...
    { DLG_MONITOR,     DisplayPageProc,    szNoSettingsPage }       // SETTINGS MUST BE LAST PAGE!!!
};
...
// GetDisplaySettingsPage

BOOL
GetDisplaySettingsPage(LPVOID lpv, LPFNADDPROPSHEETPAGE lpfnAdd, LPARAM lparam)
{
    ...
    psp.lParam = (LPARAM)MAKELPARAM(0, TRUE);
    ...
    if (gbExecMode == EXEC_NORMAL) {
        for( i = 0; i < C_PAGES_DESK; i++ ) {
            ...
            if (psp.pfnDlgProc == DisplayPageProc ) {
                ...
                if (!GetDisplaySettingsPage(NULL, _AddDisplayPropSheetPage, (LPARAM)&psh)) {
                    /*
                     * We something went wrong with multi-monitor stuff.  Fall back
                     * to old style code
                     */

                    psp.pfnDlgProc = DisplayPageProc;
                    psp.lParam = MAKELPARAM(1,TRUE);    // Tell settings page to look for default adaptor
...
LRESULT CALLBACK CoverWindowProc( HWND window, UINT message, WPARAM wparam, LPARAM lparam )
{
    switch( message )
    {
        case WM_CREATE:
...
BOOL APIENTRY DisplayPageProc(   HWND hDlg, UINT msg, UINT wParam, LONG lParam) {
    return DeskDefPropPageProc( hDlg, msg, wParam, lParam );
}
#endif
