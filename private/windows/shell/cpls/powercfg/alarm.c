...
void EditWorkItem(HWND hWnd, LPTSTR pszTaskName)
{
    ...
    if (SUCCEEDED(hr)) {
        hr = pISchedAgent->lpVtbl->Activate(pISchedAgent, pszTaskName, &IID_ITask, &(IUnknown *)pITask);
        if (SUCCEEDED(hr)) {
      ...
      if (g_gpp.user.DischargePolicy[uiIndex].PowerPolicy.EventCode &
         POWER_LEVEL_USER_NOTIFY_EXEC) {
         {
            ...
            if (SUCCEEDED(hr)) {
               ...
               hr = CoCreateInstance( &CLSID_CSchedulingAgent, NULL, CLSCTX_INPROC_SERVER, &IID_ISchedulingAgent, (LPVOID*)&pISchedAgent);
               if (SUCCEEDED(hr)) {
                  hr = pISchedAgent->lpVtbl->Activate(pISchedAgent, g_szAlarmTaskName [uiIndex], &IID_ITask, &(IUnknown *)pITask);
                  if (SUCCEEDED(hr)) {
