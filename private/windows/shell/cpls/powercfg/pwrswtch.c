...
VOID SetAdvancedDlgProcData(HWND hWnd, BOOL bRestoreCurrent)
{
    ...
    if (g_SysPwrCapabilities.SystemS1 || g_SysPwrCapabilities.SystemS2 || g_SysPwrCapabilities.SystemS3) {
...
*/
INT_PTR CALLBACK AdvancedDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    NMHDR FAR   *lpnm;
    UINT        uiID;
    ...
    case WM_NOTIFY:
        ...
        case PSN_APPLY:
            ...
            if (bDirty) {
                GetControls(hWnd, NUM_BUTTON_POL_CONTROLS, g_pcButtonPol);
                g_gpp.user.LidCloseAc.Action = g_gpp.user.LidCloseDc.Action;
                g_gpp.user.PowerButtonAc.Action = g_gpp.user.PowerButtonDc.Action;
                g_gpp.user.SleepButtonAc.Action = g_gpp.user.SleepButtonDc.Action;
                ...
                SysTray_EnableService(STSERVICE_POWER, g_gpp.user.GlobalFlags & g_uiEnableSysTrayFlag);
                bDirty = FALSE;
            }
            break;
        case PSN_SETACTIVE:
