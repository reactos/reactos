...
VOID DisableControls(HWND hWnd, UINT uiNumControls, POWER_CONTROLS  pc[])
/*
*  DESCRIPTION:
...
VOID HideControls(HWND hWnd, UINT uiNumControls, POWER_CONTROLS  pc[])
/*
*  DESCRIPTION:
        ...
        switch (pc[i].uiType) {
            ...
            case EDIT_UINT:
                if (pc[i].lpdwParam) {
                    ...
                    SendDlgItemMessage(hWnd, psd->uiSpinId, UDM_SETRANGE, 0, (LPARAM) *(psd->puiRange));
                }
...
*/
{
    ...
    for (i = 0; i < uiNumControls; i++) {
        ...
        switch (pc[i].uiType) {
            ...
            case CHECK_BOX_ENABLE:
                // lpdwParam points to a flag mask.
                DoGetCheckBoxEnable(hWnd, pc[i].uiID, &uiDataOut, pc[i].lpdwParam, pc[i].lpdwState);
                break;
            case EDIT_UINT:
                ...
                break;
            case EDIT_TEXT:
                GetDlgItemText(hWnd, pc[i].uiID, pc[i].lpvData, (pc[i].dwSize / sizeof(TCHAR)) - 1);
                break;
            case SLIDER:
...
/*
*  DESCRIPTION:
*   Limit the range of values in a ID array based on the passed in Min and Max values.
*/
{
    UINT i, j;
...
LPTSTR IDtoStr(UINT uiCount, UINT uiStrID[])
/*
*  DESCRIPTION:
...
/*
*  DESCRIPTION:
*   Reset and populate a combo box with the data pointed to by uiId. Select the item pointed to by uiData.
*/
{
    ...
    while (uiId[i]) {
        ...
        if (lpsz) {
            if (uiIndex != (UINT) SendDlgItemMessage(hWnd, uiID, CB_ADDSTRING, 0, (LPARAM)lpsz)) {
            ...
            if (SendDlgItemMessage(hWnd, uiID, CB_SETITEMDATA, uiIndex++, (LPARAM)uiId[i++]) == CB_ERR) {
    ...
    if (bFoundSel) {
        if ((UINT)SendDlgItemMessage(hWnd, uiID, CB_SETCURSEL, (WPARAM)uiSelIndex, 0) != uiSelIndex) {
