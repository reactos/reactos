...
LPTSTR CDECL LoadDynamicString( UINT StringID, ... )
{
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ALLOCATE_BUFFER, (LPVOID) (LPTSTR) Buffer, 0, 0, (LPTSTR) &pStr, 0, &Marker);
    ...
    UINT    uiTitleID)
{
   ...
   if (dwErr != NO_ERROR) {
      if (!FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwErr, 0, (LPTSTR)&pszErr, 1, NULL)) {
         if (LoadString(g_hInstance, IDS_UNKNOWN_ERROR, szUnknownErr, sizeof(szUnknownErr))) {
            pszErr = szUnknownErr;
         }
      }
   }
...
BOOL InitCapabilities(PSYSTEM_POWER_CAPABILITIES pspc)
{
    ...
    if (g_dwNumSleepStates > 1) {
        g_dwSleepStatesMaxMin = MAKELONG((short) 0, (short) g_dwNumSleepStates - 1);
    }
