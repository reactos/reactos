...
//    6 = Sunday
{
    ...
    if (iDay < 0)
    {
        ...
        *ch = TEXT('0') + DEF_FIRST_WEEKDAY;
        ...
        iDay = ( ((*ch >= TEXT('0')) && (*ch <= TEXT('6'))) ? ((int)*ch - TEXT('0')) : DEF_FIRST_WEEKDAY );
    }
...
int GetDaysOfTheMonth(int iMonth)
{
    int cDays;
    int nYear;
...
void AdjustDeltaDay(HWND hwnd, int iDay)
//  Alters the variables in wDeltaDateTime, allowing a CANCEL button to perform its job by resetting the time as if it had never been touched.
//  GetTime() and GetDate() should already have been called.
{
    ...
    if (wDateTime[DAY] != iDay)
    {
        ...
        FORWARD_WM_COMMAND( GetParent(hwnd), GetWindowLong(hwnd, GWL_ID), hwnd, CBN_SELCHANGE, SendMessage );
    }
}


void ChangeCurrentDate(PCALINFO pci, int iNewCol, int iNewRow)
...
BOOL CalendarPaint(PCALINFO pci, HWND hwnd)
{
    RECT rc, rcT;
    PAINTSTRUCT ps;
    HDC hdc;
    ...
    {
        for (iWeekDay = iFirstDayOfMonth; (iWeekDay < 7) && (iDay <= iMaxDays); iWeekDay++)
        {
            ...
            ExtTextOut( hdc, rcT.left, rcT.top, 0, &rcT, (LPTSTR)pszDate, 2, NULL );
    ...
    if (!IsFELang)
    {
        ...
        Locale = (PRIMARYLANGID(LangID) == LANG_ARABIC) ? MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT), SORT_DEFAULT) : LOCALE_USER_DEFAULT;

        for (iWeekDay = 0; (iWeekDay < 7); iWeekDay++)
        {
            GetLocaleInfo( Locale, LOCALE_SABBREVDAYNAME1 + (iWeekDay + iFirstDayOfWeek) % 7, szShortDay, sizeof(szShortDay) / sizeof(TCHAR) );
            if (*szShortDay)
            {
                *szShortDay = (TCHAR)CharUpper((LPTSTR)(DWORD)(TBYTE)*szShortDay);
            }

            TextOut( hdc, (((rc.right - rc.left) * iWeekDay) / 7) + cBorderX, cBorderY, szShortDay, 1 );
        }
    }
    else
    {
        ...
        for (iWeekDay = 0; (iWeekDay < 7); iWeekDay++)
        {
            GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SABBREVDAYNAME1 + (iWeekDay + iFirstDayOfWeek) % 7, szShortDay, sizeof(szShortDay) / sizeof(TCHAR) );
                ...
                (lstrlen(szShortDay) == 3 * NUM_DBCS_CHARS))
            {
                TextOut( hdc,
                         ...
                         cBorderY,
                         (LangID == MAKELANGID( LANG_CHINESE, SUBLANG_CHINESE_HONGKONG )) ? szShortDay : szShortDay + (2 * NUM_DBCS_CHARS),
                         1 * NUM_DBCS_CHARS );
            }
            else
            {
                TextOut( hdc, (((rc.right - rc.left) * iWeekDay) / 7) + cBorderX, cBorderY, szShortDay, lstrlen(szShortDay) );
            }
        }
    }
...
BOOL IsValidClick(HWND hwnd, int x, int y)
{
    int iT;
...
BOOL HandleDateChange(PCALINFO pci, int x, int y)
{
    RECT rc, rcT;
    int ix, iy;
    ...
    if (IsValidClick(pci->hwnd, ix, iy))
    {
        ...
        InvalidateRect(pci->hwnd, &rcT, FALSE);
        ChangeCurrentDate( pci, (x * 7) / (rc.right - rc.left), (y * 7) / (rc.bottom - rc.top) );
        return (TRUE);
    }
    else
    {
...
void HandleKeyDown(PCALINFO pci, int vk, LPARAM lParam)
{
    RECT rcT;
...
BOOL CalendarInit(HANDLE hInstance)
{
    WNDCLASS wc;
