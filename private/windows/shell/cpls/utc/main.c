...
BOOL EnableTimePrivilege(PTOKEN_PRIVILEGES *pPreviousState, ULONG *pPreviousStateLength);
BOOL ResetTimePrivilege(PTOKEN_PRIVILEGES PreviousState, ULONG PreviousStateLength);
int DoMessageBox(HWND hWnd, DWORD wText, DWORD wCaption, DWORD wType);
    ...
    switch (uMsg)
    {
        ...
        case ( CPL_NEWINQUIRE ) :
        {
            ...
            switch (lParam1)
            {
                case ( 0 ) :
                {
                    lpCPlInfo->hIcon = LoadIcon( g_hInst, MAKEINTRESOURCE(IDI_TIMEDATE) );
                    LoadString( g_hInst, IDS_TIMEDATE, lpCPlInfo->szName, sizeof(lpCPlInfo->szName) );
                    LoadString( g_hInst, IDS_TIMEDATEINFO, lpCPlInfo->szInfo, sizeof(lpCPlInfo->szInfo) );
        ...
        case ( CPL_STARTWPARMS ) :
        {
            ...
            switch (lParam1)
            {
                case ( 0 ) :
                {
                    ...
                    {
                        DoMessageBox( hwnd, IDS_NOTIMEERROR, IDS_CAPTION, MB_OK | MB_ICONINFORMATION );
                    }
                    break;
                }
            }
