...
BOOL ClockInit(HINSTANCE hInstance)
...
void GetTimeClock(TIME *pt, PCLOCKSTR np)
//  Gets the time that we should display on the clock.
//  The client could have specified a function to call to get this or an HWND to pass a message to to get it from.
{
    SYSTEMTIME st;
    ...
    if (np->hwndGetTime)
    {
        SendMessage( np->hwndGetTime, CLM_UPDATETIME, CLF_GETTIME, (LPARAM)(LPSYSTEMTIME)&st );
        pt->hour = st.wHour % 12;
...
void CompClockDim(HWND hWnd, PCLOCKSTR np)//  Calculates the clock dimensions.
{
    int i;
    int tWidth;
    int tHeight;
...
void ClockSize(PCLOCKSTR np, int newWidth, int newHeight)//  Sizes the clock to the specified size.
{
...
void DrawFace(HDC hDC, PCLOCKSTR np)//  Draws the clock face.
{
    int i;
    RECT tRect;
    LPPOINT ppt;
    ...
    blobWidth = MulDiv( MAXBLOBWIDTH, (np->clockRect.right - np->clockRect.left), np->HorzRes );
    ...
    np->clockCenter.y = np->clockRect.top + ((np->clockRect.bottom - np->clockRect.top) / 2) - 1;
...
//  It assumes you have set nTime already.  This allows it to be called by the timer or by the client.
{
    ...
    if (hint == REPAINT)
    {
        ...
        //  Otherwise we will create artifacts when there is a clipping region.
        DrawFace(hDC, np);
...
//  Update the clock.  Called on a timer tick.
{
    ...
    if ((np->nTime.second == np->oTime.second) && (np->nTime.minute == np->oTime.minute) && (np->nTime.hour == np->oTime.hour))
    {
        return;
    }
...
//  Deals with the clock messages.
{
    ...
    switch (message)
    {
        ...
        case ( WM_CREATE ) :
        {
            ...
            } while (np->nTime.second == np->oTime.second && np->nTime.minute == np->oTime.minute && np->nTime.hour == np->oTime.hour);

            SetTimer(hWnd, TIMER_ID, OPEN_TLEN, 0L);

            ClockSize( np, ((LPCREATESTRUCT)lParam)->cx, ((LPCREATESTRUCT)lParam)->cy );
            break;
        }
        case ( WM_SIZE ) :
        {
        ...
        case ( WM_TIMECHANGE ) :
        {
            ...
            if (np->hwndGetTime)
            {
                ...
                SendMessage( np->hwndGetTime, CLM_UPDATETIME, CLF_SETTIME, (LPARAM)(LPSYSTEMTIME)&System );
            }
