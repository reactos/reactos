...
static BOOL CreateCDC(LPCDC cdc, HBITMAP bitmap)
{
...
static void DestroyCDC(LPCDC cdc)
{
    if (cdc->dc)
    {
        if (cdc->defbitmap)
...
BOOL LoadWorldMap(LPWORLDMAP map, HINSTANCE instance, LPCTSTR resource)
{
    ...
    if (tempdc)
    {
        if (CreateCDC( &map->original, LoadImage( instance, resource, IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION ) ))
        {
            DIBSECTION ds;
            ...
            if (GetObject(map->original.bitmap, sizeof(DIBSECTION), &ds))
            {
                ...
                if (( (GetDeviceCaps(tempdc, BITSPIXEL) * GetDeviceCaps(tempdc, PLANES)) > 4 ) || CreateCDC( &map->prepared, CreateCompatibleBitmap( tempdc, ds.dsBm.bmWidth, ds.dsBm.bmHeight ) ))
                {
                    ...
                    map->source = (map->prepared.dc) ? map->prepared.dc : map->original.dc;
                    result = TRUE;
                }
            }
            else
...
void FreeWorldMap(LPWORLDMAP map)
{
...
void SetWorldMapRotation(LPWORLDMAP map, int rotation)
{
...
void RotateWorldMap(LPWORLDMAP map, int delta)
{
    SetWorldMapRotation(map, map->rotation + delta);
}


int WorldMapGetDisplayedLocation(LPWORLDMAP map, int pos)
{
    return ( (pos + map->rotation) % map->size.cx );
}


void EnumWorldMapDirtySpans(LPWORLDMAP map, ENUMSPANPROC proc, DWORD data, BOOL rotate)
{
    DIRTYSPAN *span = map->dirty.spans;
...
int GetWorldMapColorIndex(LPWORLDMAP map, int x, int y)
{
    //  Protect against faulting.
    if ( !map->bits || (x < 0) || (x >= map->size.cx) || (y < 0) || (y >= map->size.cy) )
    {
        return (-1);
    }
    ...
    y = (LONG)map->size.cy - 1 - y;//  Correct for dib origin.

    return ( map->bits[map->scanbytes * y + x] );
}


DIRTYSPAN *NewSpan(DIRTYSTUFF *dirty, DIRTYSPAN *a, DIRTYSPAN *b)
{
...
void DeleteSpan(DIRTYSTUFF *dirty, DIRTYSPAN *a, DIRTYSPAN *b)
{
    if (a)
    {
        a->next = b->next;
...
void AddDirtySpan(DIRTYSTUFF *dirty, int left, int cx)
{
    int right = left + cx;
...
void ChangeWorldMapColor(LPWORLDMAP map, int index, const RGBQUAD *color, int x, int cx)
{
    if ((index >= 0) && (index < WORLDMAP_MAX_COLORS))
    {
        map->dirty.colors[index] = *color;//  Store the new color.
...
void CommitChanges(LPWORLDMAP map)
{
    if (map->dirty.last >= 0)
    {
        SetDIBColorTable( map->original.dc, map->dirty.first, 1 + map->dirty.last - map->dirty.first, map->dirty.colors + map->dirty.first );
    ...
    while (map->dirty.spans)
    {
        ...
        if (map->prepared.dc)
        {
            BitBlt( map->prepared.dc, span->left, 0, span->right - span->left, (int)map->size.cy, map->original.dc, span->left, 0, SRCCOPY );
        }
...
void DrawWorldMap(HDC dc, int xdst, int ydst, int cx, int cy, LPWORLDMAP map, int xmap, int ymap, DWORD rop)
{
    CommitChanges(map);
    ...
    {
        int firstcx = (int)map->size.cx - xmap;//  Compute the width of the first blt.
        BitBlt(dc, xdst, ydst, firstcx, cy, map->source, xmap, ymap, rop);//  See bits.  See bits blt.
    ...
    BitBlt(dc, xdst, ydst, cx, cy, map->source, xmap, ymap, rop);//  blt bits blt!
}