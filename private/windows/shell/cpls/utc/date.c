...
//  When you click on cancel, wDeltaDateTime is subtracted from the current values.
//  You cannot just revert to what it was when it started as time continues and dates could change.
...
int WINAPI WideStrToStr(LPSTR psz, LPWSTR pwsz)
{
...
int AnsiWideStrCmpI(LPCSTR pszAnsi, WCHAR *pwszWide)
{
    char szAnsi[64];
...
//  'M', 'd', or 'y', and checks if the string indicates a leading zero or century.
//  The return value is a pointer to the next character, which should be a separator or NULL.
...
int _fastcall MonthUpperBound(int nMonth, int nYear)
/*
加下注释吧！
功能：根据年和月返回这个月有多少天。
返回值：28/29/30/31
...
void GetDateTime()
{
    ...
    if (!g_ptziCurrent || !SystemTimeToTzSpecificLocalTime( g_ptziCurrent, &UniversalSystemTime, &SystemTime ))
#endif
    {
...
void AdjustDelta(HWND hDlg, int nIndex)
...
//  GetTime() & GetDate() should already have been called.
{
    int nDelta;
    ...
    nDelta = (int)SendDlgItemMessage( hDlg,
                                      nIndex <= SECOND ? DATETIME_TARROW : DATETIME_YARROW,
                                      UDM_GETPOS,
                                      0,
                                      0L );
    ...
    //  If our current recording of the time/date is not what we have now, do deltas.
    if (wDateTime[nIndex] != nDelta)
    {
        wDeltaDateTime[nIndex] += nDelta - wDateTime[nIndex];//  Delta is previous delta + (users setting versus system time)
        wPrevDateTime[nIndex] = wDateTime[nIndex] = nDelta;//  Previous value is current user's settings.
...
void AdjustDeltaMonth(int iMonth)
//  Alters the variables in wDeltaDateTime, allowing a CANCEL button to perform its job by resetting the time as if it had never been touched.
//  GetTime() & GetDate() should already have been called.
{
    GetDate();
...
int ReadShortDate(LPTSTR pszDate, BOOL *pbMonth, BOOL *pbDay, BOOL *pbYear)
...
void ReflectAMPM(HWND hDlg, int nNum)
...
void DateTimeInit(HWND hDlg, WORD nBaseID, WORD nSepID, LPTSTR pszSep, int nMaxDigitWidth, BOOL bDate)
//  Determine the widest digit (safety against variable pitch fonts).
{
    HWND hAMPMList;
    ...
    GetWindowRect( GetDlgItem( hDlg, bDate ? DATETIME_CALENDAR : DATETIME_CLOCK ), (LPRECT)&Rect );
    ScreenToClient(hDlg, (LPPOINT)&Rect.left);
    ScreenToClient(hDlg, (LPPOINT)&Rect.right);
...
void myitoa(int intValue, LPTSTR lpStr)
{
    LPTSTR lpString;
    TCHAR c;
...
void UpdateItem(HWND hDlg, int i)
//  This displays the information in the control from the array of global values.
//  Also selects the control.
//  Also adds leading 0's as well as rounding years to 2 digits and 24 or AM/PM hours.
{
    TCHAR szNum[5];
    ...
    SetDlgItemText(hDlg, DATETIME_HOUR + i, szNum);//  Reflect the value in the appropriate control.
    SendDlgItemMessage(hDlg, DATETIME_HOUR + i, EM_SETSEL, 0, MAKELONG(0, 32767));//  Select the field too.

    //  If we changed year or month, then we may have altered the leap year state.
    if (i == MONTH || i == YEAR)
    {
        g_sDateInfo[DAY].nMax = MonthUpperBound( wDateTime[MONTH], wDateTime[YEAR] );
    }
...
void _ShowTZ(HWND hDlg)
{
    ...
    else
    {
        ...
        lstrcpy( name, (TimeZoneId == TIME_ZONE_ID_STANDARD) ? info.StandardName : info.DaylightName );
        ...
        if (*name && (lstrcmpi(name, c_szFirstBootTZ) != 0))
        {
            ...
            if (!*format)
            {
                GetWindowText( ctl, format, sizeof(format) / sizeof(format[0]) );
            }
...
void TimeProvider(LPSYSTEMTIME lpSystemTime, HWND hDlg)
//  Called by the clock to find out what time it is.
{
    short wTemp[7];

    //  If the user has modified the time, the clock should display the edit controls, otherwise its just the SystemTime.
    if (g_Modified)
    {
    ...
    else
    {
        ...
        if ((wDateTime[DAY] != wTemp[DAY]) || (wDateTime[MONTH] != wTemp[MONTH]) || (wDateTime[YEAR] != wTemp[YEAR]))
        {
            InvalidateRect (GetDlgItem (hDlg, DATETIME_CALENDAR), NULL, TRUE);

            if (wDateTime[MONTH] != wTemp[MONTH])
            {
                ComboBox_SetCurSel (GetDlgItem (hDlg, DATETIME_MONTHNAME), wDateTime[MONTH] - 1);
            }
...
void InitDateTimeDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
//  Called to init the dialog.
{
    ...
    for (i = 0; i < 12; i++)
    {
        GetLocaleInfo( LOCALE_USER_DEFAULT, LOCALE_SMONTHNAME1 + i, szMonth, sizeof(szMonth) );
    ...
    SendMessage( hwndScroll, UDM_SETRANGE, 0, MAKELPARAM(g_sDateInfo[YEAR].nMax, g_sDateInfo[YEAR].nMin) );
    ...
    SetWindowLong( hwndTBorder, GWL_EXSTYLE, GetWindowLong(hwndTBorder, GWL_EXSTYLE) | WS_EX_CLIENTEDGE );
...
LRESULT CheckNum(HWND hDlg, UINT nScrollID, HWND hCtl)
{
    static int cReenter = 0;

    LRESULT lRet;

    //  If this is an illegal value, (but not blank), then kill the last char that was entered.
    lRet = SendDlgItemMessage(hDlg, nScrollID, UDM_GETPOS, 0, 0L);

    ++cReenter;//  Guard against re-entrance.

    if (cReenter <= 4)
    {
        SendMessage( hCtl, HIWORD(lRet) && GetWindowTextLength(hCtl) ? EM_UNDO : EM_EMPTYUNDOBUFFER, 0, 0L );
    }
...
BOOL CALLBACK DateTimeDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
//  Main dialog proc.
{
    int i;
    ...
    switch (uMsg)
    {
        ...
        case ( WM_DESTROY ) :
        {
            ...
            {
                ImmAssociateContext( GetDlgItem(hDlg, DATETIME_YEAR), g_PrevIMCForDateField );
            }
            UnhookTimer(hDlg);
            break;
        }
        ...
        case ( WM_CTLCOLOR ) :
        {
            //  Set the background color of the time controls to the the color of the edit controls.
            if ((GET_WM_CTLCOLOR_HWND(wParam, lParam, uMsg) == GetDlgItem(hDlg, DATETIME_TSEP1)) ||
                (GET_WM_CTLCOLOR_HWND(wParam, lParam, uMsg) == GetDlgItem(hDlg, DATETIME_TSEP2)) ||
                (GET_WM_CTLCOLOR_HWND(wParam, lParam, uMsg) == GetDlgItem(hDlg, DATETIME_TBORDER)))
            {
#ifndef WIN32
        ...
        case ( WM_VSCROLL ) :
        {
            switch (GET_WM_VSCROLL_CODE(wParam, lParam))
            {
                ...
                {
                    SYSTEMTIME SystemTime;
                    HWND hBuddy = (HWND)SendMessage(GET_WM_VSCROLL_HWND(wParam, lParam), UDM_GETBUDDY, 0, 0L );
            ...
            switch (GET_WM_COMMAND_ID(wParam, lParam))
            {
                case ( DATETIME_AMPM ) :
                {
                    ...
                    switch (GET_WM_COMMAND_CMD(wParam, lParam))
                    {
                        case ( LBN_SETFOCUS ) :
                        {
                            ...
                            SendMessage( hwndScroll, UDM_SETRANGE, 0, MAKELPARAM(1, 0) );
                            udAccel.nSec = 0;
                            udAccel.nInc = 1;
                            SendMessage( hwndScroll, UDM_SETACCEL, 1, (LPARAM)(LPUDACCEL)&udAccel );
                            SendMessage( hwndScroll, UDM_SETBUDDY, (WPARAM)hwndThisCtl, 0 );
                            break;
                        }
                        case ( LBN_KILLFOCUS ) :
                        {
                            ...
                            if ((HWND)SendMessage( hwndScroll, UDM_GETBUDDY, 0, 0 ) == hwndThisCtl)
                            {
                        ...
                        case ( LBN_SELCHANGE ) :
                        {
                            if ((g_Modified == FALSE) && (g_bPM == (BOOL)ListBox_GetTopIndex(hwndThisCtl)))
                            {
                                break;
                            }
                ...
                case ( DATETIME_SECOND ) :
                {
                    switch (GET_WM_COMMAND_CMD(wParam, lParam))
                    {
                        case ( EN_CHANGE ) :
                        {
                            ...
                            //  Force the clock to reflect this setting.
                            TimeProvider(&SystemTime, hDlg);
                            SendDlgItemMessage( hDlg, DATETIME_CLOCK, CLM_UPDATETIME, 0, (LPARAM)(LPSYSTEMTIME)&SystemTime );
                            break;
                        }
                    }
                ...
                case ( DATETIME_DAY ) :
                {
                    switch (GET_WM_COMMAND_CMD(wParam, lParam))
                    {
                        ...
                        {
                            CheckNum( hDlg,
                                      GET_WM_COMMAND_ID(wParam, lParam) <= DATETIME_SECOND ? DATETIME_TARROW : DATETIME_YARROW,
                                      GET_WM_COMMAND_HWND(wParam, lParam) );

                            // Changing the year may alter the number of days in February.
                            // Yes this is a hack, but this entire applet is a giant broken hack and I want to change it as little as possible.
                        ...
                        case ( EN_SETFOCUS ) :
                        {
                            ...
                            if (id <= SECOND)
                            {
                                ...
                                SendMessage( hwndScroll, UDM_SETRANGE, 0, MAKELPARAM( g_sDateInfo[id].nMax, g_sDateInfo[id].nMin) );
                                udAccel[0].nSec = 0;
                                udAccel[0].nInc = 1;
                                udAccel[1].nSec = 2;
                                ...
                                SendMessage( hwndScroll, UDM_SETBUDDY, (WPARAM)GET_WM_COMMAND_HWND(wParam, lParam), 0 );
                            }
                            break;
                        }
                        case ( EN_KILLFOCUS ) :
                        {
                            ...
                            if ((id <= SECOND) && (HWND)SendMessage( hwndScroll, UDM_GETBUDDY, 0, 0 ) == GET_WM_COMMAND_HWND(wParam, lParam))
                            {
                                SendMessage(hwndScroll, UDM_SETBUDDY, 0, 0);
                            }
                ...
                case ( DATETIME_MONTHNAME ) :
                {
                    ...
                    {
                        int nIndex = 1 + (int)ComboBox_GetCurSel(GetDlgItem( hDlg, DATETIME_MONTHNAME ));
                ...
                {
                    //  If the calendar sent us a change, we will assume that it is to allow the apply now to work.
                    PropSheet_Changed(GetParent(hDlg), hDlg);
                    break;
                }
            }
        ...
        case ( WM_HELP ) :             // F1
        {
            WinHelp( (HWND)((LPHELPINFO)lParam)->hItemHandle, NULL, HELP_WM_HELP, (DWORD)(LPTSTR)aDateTimeHelpIds );
            break;
        }
        case ( WM_CONTEXTMENU ) :      // right mouse click
        {
            WinHelp( (HWND)wParam, NULL, HELP_CONTEXTMENU, (DWORD)(LPTSTR)aDateTimeHelpIds );
            break;
        }
        default :
        {
...
void SetZoneState(HWND map, PTZINFO zone, BOOL highlight)
//  Sets the display state of a time zone in the map control.
{
    if (zone)
    {
        if (zone->SeaIndex >= 0)
        {
            MapControlSetSeaRegionHighlight( map, zone->SeaIndex, highlight, zone->MapLeft, zone->MapWidth );
        }

        if (zone->LandIndex >= 0)
        {
            MapControlSetLandRegionHighlight( map, zone->LandIndex, highlight, zone->MapLeft, zone->MapWidth );
        }
    }
}


void SetZoneFamilyState(HWND map, PTZINFO family, BOOL highlight)
...
void ParseMapInfo(PTZINFO zone, const TCHAR *text)
...
//  Reads the data for a time zone from the registry.
{
    ...
    if (RegQueryValueEx( key, c_szTZDisplayName, 0, NULL, (LPBYTE)zone->szDisplayName, &len ) != ERROR_SUCCESS)
    {
        return (FALSE);
    }

    //  Under NT, the keyname is the "Standard" name.
    //  Values stored under the keyname contain the other strings and binary info related to the time zone.
    //  Every time zone must have a standard name, therefore, we save registry space by using the Standard name as the subkey name under the "Time Zones" key.
    ...
    if (RegQueryValueEx( key, c_szTZDaylightName, 0, NULL, (LPBYTE)zone->szDaylightName, &len ) != ERROR_SUCCESS)
    {
        return (FALSE);
    }

    len = sizeof(zone->Bias) + sizeof(zone->StandardBias) + sizeof(zone->DaylightBias) + sizeof(zone->StandardDate) + sizeof(zone->DaylightDate);
    if (RegQueryValueEx( key, c_szTZI, 0, NULL, (LPBYTE)&zone->Bias, &len ) != ERROR_SUCCESS)
    {
        return (FALSE);
    }

    len = sizeof(mapinfo);
    if (RegQueryValueEx( key, c_szTZMapInfo, 0, NULL, (LPBYTE)mapinfo, &len ) != ERROR_SUCCESS)
    {
        *mapinfo = TEXT('\0');
    }
    ...
    //  Generate phony MapLeft and MapRight until they show up in the registry.
...
//  Inserts a new time zone into a list, sorted by bias and then name.
{
    if (*list)
    {
        ...
        while (next && zone->Bias <= next->Bias)
        {
            ...
            {
                if (CompareString( GetUserDefaultLCID(), 0, zone->szDisplayName, -1, next->szDisplayName, -1 ) == CSTR_LESS_THAN)
                {
                    break;
                }
            }
...
//  Returns num read, -1 on failure.
{
    ...
    *list = NULL;
    ...
    if (RegOpenKey( HKEY_LOCAL_MACHINE, c_szTimeZones, &key ) == ERROR_SUCCESS)
    {
        TCHAR name[TZNAME_SIZE];
        PTZINFO zone = NULL;
        int i;
        ...
        for (i = 0; RegEnumKey(key, i, name, TZNAME_SIZE) == ERROR_SUCCESS; i++)
        {
            ...
            if (!zone && ((zone = (PTZINFO)LocalAlloc(LPTR, sizeof(TZINFO))) == NULL))
            {
                zone = *list;
                *list = NULL;
                count = -1;
            ...
            {
                //  Each sub key name under the Time Zones key is the "Standard" name for the Time Zone.
...
void InitZoneMapping(PTZINFO *lookup, PTZINFO list, HWND map)
...
void BreakZonesIntoFamilies(PTZINFO head)
...
//  Assumes the passed list is sorted by bias.
{
    ...
    if ((subhead) && (subhead->Bias == BIAS_PLUS_12) && (head->Bias == BIAS_MINUS_12))
    {
        PTZINFO next = head;
...
BOOL InitTimezones(HWND page, PTZINFO *lookup)
...
void ChangeZone(HWND page, TZPAGE_STATE *state, PTZINFO zone)
//  Updates the current zone, making sure new zone's family is highlighted.
{
    if (zone || state->zone)
    {
        BOOL newfamily = (!zone || !state->zone || (zone->Bias != state->zone->Bias));
...
void HotTrackZone(HWND page, TZPAGE_STATE *state, int index)
...
//  Expects to be called with dups.
{
    ...
    {
        ComboBox_SetCurSel( GetDlgItem(page, IDD_TIMEZONES), (zone ? zone->ComboIndex : -1) );
        ChangeZone(page, state, zone);
    }
}


void CenterZone(HWND page, TZPAGE_STATE *state, BOOL animate)
//  Updates the map highlighting and combo selection for a given map index.
//  Expects to be called with dups.
{
...
PTZINFO GetPTZ(HWND hDlg, int iItem)
...
void SetAllowLocalTimeChange(BOOL fAllow)
{
    ...
    else
    {
        ...
        if (RegCreateKey( HKEY_LOCAL_MACHINE, c_szRegPathTZControl, &key ) == ERROR_SUCCESS)
        {
            DWORD value = 1;
...
BOOL InitTimeZonePage(HWND hDlg, TZPAGE_STATE *state)
//  This function initializes everything to do with the Time Zones.
{
    ...
    {
        //  The 'default' value of the time zone key specifies the default zone.
        TCHAR szDefaultName[TZNAME_SIZE];
        LONG len = sizeof(szDefaultName);

        if (RegQueryValue( HKEY_LOCAL_MACHINE, c_szTimeZones, szDefaultName, &len ) == ERROR_SUCCESS)
        {
#ifdef UNICODE
    ...
    if (iCurrentTZ == g_nTimeZones)
    {
        ...
        for (j = 0; j < g_nTimeZones; j++)
        {
            ...
            {
                int nHitCount = TZ_HIT_BASE + ((ptzi->StandardBias == tziCurrent.StandardBias) + (ptzi->DaylightBias == tziCurrent.DaylightBias));
    ...
    if ((tziCurrent.StandardDate.wMonth == 0) || (tziCurrent.DaylightDate.wMonth == 0))
    {
...
void SetTheTimezone(BOOL bAutoMagicTimeChange, BOOL bAutoMagicEnabled, PTZINFO ptzi)
...
#ifdef WINNT
    if ((bAutoMagicEnabled == 0) || (bAutoMagicTimeChange == 0) || (ptzi->StandardDate.wMonth == 0))
    {
        //  Standard Only.
...
BOOL CALLBACK TimeZoneDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
                ...
                case ( 0 ) :
                {
                    switch (((NMHDR *)lParam)->code)
                    {
                        case ( PSN_APPLY ) :
                        {
                            ...
                            SetTheTimezone(IsDlgButtonChecked(hDlg, IDD_AUTOMAGIC), IsWindowEnabled(GetDlgItem(hDlg, IDD_AUTOMAGIC)), GetPTZ(hDlg, -1) );
                            break;
                        }
                    }
                    break;
...
static int GetClInt(const TCHAR *p)
...
BOOL SelectZoneByName(LPCTSTR cmdline)
{
    BOOL result = FALSE;
    HKEY key = NULL;
        ...
        else
        {
            ...
            for (i = 0; RegEnumKey(key, i, name, TZNAME_SIZE) == ERROR_SUCCESS; i++)
            {
                if (RegOpenKey(key, name, &subkey) == ERROR_SUCCESS)
                {
                    ...
                    if ((RegQueryValueEx( subkey, c_szTZDisplayName, 0, NULL, (LPBYTE)&zone.szDisplayName, &len ) == ERROR_SUCCESS) &&
                        (CompareString( GetUserDefaultLCID(),
                                        NORM_IGNORECASE  | NORM_IGNOREKANATYPE | NORM_IGNOREWIDTH | NORM_IGNORENONSPACE,
                                        zone.szDisplayName,
                                        (CmdLen < 15) ? -1 : min(lstrlen(zone.szDisplayName), CmdLen), cmdline, -1 ) == CSTR_EQUAL))
                    {
                        if (ReadZoneData(&zone, subkey, name))
                        {
                            result = TRUE;
...
BOOL OpenDateTimePropertySheet(HWND hwnd, LPCTSTR cmdline)
