...
HRESULT HandleQueryInterface(REFIID riid, PVOID *ppvInterface, INTERFACES *Interfaces, ULONG ArraySize)
/*++
Routine Description:
...
VOID PopulateListView(HWND ListView, PACKAGEDISPINFO *Packages, ULONG NumberOfPackages)
/*++
Routine Description:
    This routine puts the information about the applications into the list view.
Arguments:
...
*/
{
    ...
    for (i = 0; i < NumberOfPackages; i++) {
        ...
        *PackageInfo = Packages[i];
        ...
        wsprintf((PWCHAR)((PCHAR)PackageInfo + sizeof(PACKAGEDISPINFO)), L"%d.%d", Packages[i].dwVersionHi, Packages[i].dwVersionLo);
