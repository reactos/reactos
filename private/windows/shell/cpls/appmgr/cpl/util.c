...
BOOL ScreenRectToClientRect(PRECT Rect, HWND Window)
/*++
Routine Description:
    This routine takes a rectangle in screen coordinates and converts it to a rectangle relative to the specified window.
Arguments:
...
VOID UnimplementedFeature(HWND Dialog, HWND ToolTip, UINT ControlId)
/*++
Routine Description:
    This routine attaches the unimplemented feature tool tip to the specified control.
Arguments:
...
*/
{
    ...
    // Get the rectangle for the internet radio button and convert to dialog relative coordinates
...
BOOL VerifySetupName(PWCHAR FileName)
/*++
Routine Description:
    This routine verifies that the specified file exists.
    BUGBUG We should probably verify that it could actually be an install program?
Arguments:
...
#if 0
PWCHAR StrStr(PWCHAR Source, PWCHAR SearchString)
/*++
Routine Description:
    This routine determines if the specified string is a sub-string of the source string.
Arguments:
...
*/
{
    ...
    // If the string to search for is longer than the string to search in, we won't find a match
...
VOID CreateApplicationListViewColumns(HWND ListViewWindow)
/*++
Routine Description:
    This routine creates the columns for the standar application list view in the programs wizard.
Arguments:
...
VOID PopulateApplicationListView(HWND ListViewWindow, DWORD DesiredCapabilities)
/*++
Routine Description:
    This routine enumerates the installed applications on the machine, and populates the list view with the information.
Arguments:
...
*/
{
    ...
    do {
        ...
        RetVal = PipFindNextProduct(Pip, &Capabilities, &Actions, &AppInfo);
        if ((RetVal == ERROR_SUCCESS) && (Capabilities & DesiredCapabilities)) {
            // Save the relavent information
            Descriptor = HeapAlloc(GetProcessHeap(), 0, sizeof(APPLICATIONDESCRIPTOR));
    ...
    do {
        ...
        AppInfo.InstalledVersion = VersionString;
        AppInfo.InstalledVersionSize = 256;
        RetVal = PipDarwinFindNextProduct(Pip, &Capabilities, &Actions, &AppInfo);
        if ((RetVal == ERROR_SUCCESS) && (Capabilities & DesiredCapabilities)) {
            // Save the relavent information
            Descriptor = HeapAlloc(GetProcessHeap(), 0, sizeof(APPLICATIONDESCRIPTOR));
            Descriptor->Actions = Actions;
...
BOOL HandleQueryCancel(HWND Dialog, UINT NextPageID)
/*++
Routine Description:
    This function is a generic handler for PSN_QUERYCANCEL.
    Nearly every page has a cancel button and in nearly every case, we set the DWL_MSGRESULT to prevent the cancel, and change to the appropriate cancel page.
Arguments:
...
LONG CheckSetWindowLong(HWND hWnd, INT nIndex, LONG dwNewLong)
/*++
Routine Description:
    This function checks that the SetWindowLong succeeded.
    There are a large number of SetWindowLongCalls, so it made sense to create a function rather than putting them all inline.
Arguments:
...
BOOL ItemSelected(HWND DialogWindow, PULONG Index)
/*++
Routine Description:
...
BOOL SetUpFonts(HINSTANCE Instance, HWND Window, HFONT *LargeFont, HFONT *SmallFont)
/*++
Routine Description:
    This routine creates the bold logfonts for the exterior pages of the wizard.
Arguments:
...
*/
{
    ...
    Success = SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICS), &NonClientMetrics, FALSE);
    if (!Success) {
        return FALSE;
    }
    ...
    FontNameSize = LoadString(Instance, IDS_LARGEFONTNAME, Large.lfFaceName, LF_FACESIZE);
    if (FontNameSize == 0) {
        return FALSE;
    }

    FontSizeSize = LoadString(Instance, IDS_LARGEFONTSIZE, FontSizeString, 24);
    if (FontSizeSize == 0) {
        return FALSE;
    }
...
VOID SetExteriorTitleFont(HWND Dialog)
/*++
Routine Description:
...
*/
{
    ...
    SetWindowFont(Control, SmallTitleFont, TRUE);
    Control = GetDlgItem(Dialog, IDC_TITLE_LARGE);
    SetWindowFont(Control, LargeTitleFont, TRUE);
}


BOOL CancelInitProc(HWND Dialog, UINT Msg, WPARAM wParam, LPARAM lParam)
/*++
Routine Description:
...
INT CancelNotifyProc(HWND DialogWindow, WPARAM wParam, LPNMHDR NotifyHeader, UINT PageIndex)
/*++
Routine Description:
...
Return Value:
    non-zero if the message was handled.  DWL_MSGRESULT is set to the return value for the message
*/
{
    HWND Cancel;
...
/*++
Routine Description:
    This routine posts the update message to all of the interested windows.
*/
{
    ULONG PageIndex;
    ...
    for (PageIndex = 0; PageIndex < NUM_PAGES; PageIndex++) {
        if (PageInfo[PageIndex].Dialog != NULL) {
            PostMessage(PageInfo[PageIndex].Dialog, WM_UPDATELISTVIEW, 0, (LPARAM)0);
        }
    }
}


VOID UpdateApplicationLists(HWND ListView, DWORD DesiredCapabilities)
/*++
Routine Description:
...
*/
{
    ...
    NumberOfItems = ListView_GetItemCount(ListView);
...
LONG ReadAndExpandRegString(HKEY RegKey, HANDLE Heap, PWCHAR ValueName, PWCHAR *Value)
/*++
Routine Description:
...
Arguments:
    ...
    Heap - Supplies the handle of the heap to allocate the result string from.
...
*/
{
    ...
    // Get the Value
    ValueBufferSize = 256;
    RetVal = RegQueryValueEx(RegKey, ValueName, 0, &Type, (PBYTE)ValueBuffer, &ValueBufferSize);

    // If the value was longer than our buffer, allocate a buffer of appropriate length and try again
    if (RetVal == ERROR_MORE_DATA) {
        ValueBufferPointer = HeapAlloc(Heap, 0, ValueBufferSize);
        RetVal = RegQueryValueEx(RegKey, ValueName, 0, &Type, (PBYTE)ValueBuffer, &ValueBufferSize);
        if (RetVal != ERROR_SUCCESS) {
            // We've got some sort of problem
            HeapFree(Heap, 0, ValueBufferPointer);
            return RetVal;
        }
    } else {
    ...
    if (Type == REG_EXPAND_SZ) {
        ...
        DesiredStringLength = ExpandEnvironmentStrings(ValueBufferPointer, NULL, 0);

        StringBuffer = HeapAlloc(Heap, 0, DesiredStringLength * sizeof(WCHAR));
        ...
        // Expand it for real
        ExpandEnvironmentStrings(ValueBufferPointer, StringBuffer, DesiredStringLength);
    } else if (Type == REG_SZ) {
        // We just need to get the length and copy it (maybe)
        if (ValueBufferPointer != ValueBuffer) {
            // We already allocated space in the heap. We can just return that
...
VOID DisableCancelButton(HWND Dialog)
{
    HWND Cancel;

    Cancel = GetDlgItem(GetParent(Dialog), IDCANCEL);
