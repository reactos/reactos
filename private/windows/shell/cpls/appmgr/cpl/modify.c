...
Notes:
    Bugbug daveh  What do we do about error handling?  Suppose the ListView_InsertColumn fails for example.
    Do we put up a message box?
--*/
...
VOID RemoveApplication(PAPPLICATIONDESCRIPTOR AppDescriptor);
VOID ModifyApplication(PAPPLICATIONDESCRIPTOR AppDescriptor);
static VOID PopulateRemoveListView(HWND ListViewWindow);
static ULONG SelectedProgram = 0xFFFFFFFF;
static HWND ApplicationListView;


BOOL ModifyProgramInitProc(HWND DialogWindow,
    UINT Msg,
    WPARAM wParam,
    LPARAM lParam
    )
