...
BOOL PipDarUninstall(HANDLE Information);
BOOL PipDarModify(HANDLE Information);
BOOL PipDarRepair(HANDLE Information, DWORD Action);
BOOL PipDarUpgrade(HANDLE Information);
VOID PipDarFreeHandle(HANDLE Handle);
...
HANDLE PipDarwinInitialize(VOID)
/*++
Routine Description:
    This routine performs initialization for enumerating Darwin applications.
Return Value:
...
*/
{
    ...
    // Allocate our structure and initialize (NB. We have to zero the memory to get EnumIndex properly initialized)
...
Return Value:
    ERROR_SUCCESS for Success, ERROR_NO_MORE_ITEMS when all products have been enumerated.
Notes:
...
*/
{
    ...
    RetVal = MsiEnumProducts(PipInfo->EnumIndex, ProductId);
    ...
    ProductNameSize = 256;
    RetVal = MsiGetProductInfo(ProductId, INSTALLPROPERTY_PRODUCTNAME, ProductName, &ProductNameSize);
    ...
    *Capabilities = 0;
    ...
    DarwinInfo = HeapAlloc(GetProcessHeap(), 0, sizeof(DARWINAPPINFO));
    DarwinInfo->ProductId = HeapAlloc(GetProcessHeap(), 0, 39 * sizeof(WCHAR));
    ...
    *Capabilities |= PIP_UNINSTALL | PIP_MODIFY | PIP_REPAIR;
    ...
    // will result in success, but that piece of information won't be returned

    // Look up upgrade URL in registry
    lstrcpy(KeyName, L"software\\Installer\\Products\\");
    lstrcat(KeyName, ProductId);

    RetVal = RegOpenKeyEx(HKEY_LOCAL_MACHINE, KeyName, 0, KEY_READ, &SubKey);
    if (RetVal != ERROR_SUCCESS) {
        return ERROR_SUCCESS;
    }

    UrlStringLength = MAX_PATH * sizeof(WCHAR);
    RetVal = RegQueryValueEx(SubKey, L"URLUpdateInfo", NULL, &ValueType, (PBYTE)UrlString, &UrlStringLength);
    RegCloseKey(SubKey);
    if (RetVal != ERROR_SUCCESS) {
        return ERROR_SUCCESS;
    }

    DarwinInfo->UpgradeUrl = HeapAlloc(GetProcessHeap(), 0, (lstrlen(UrlString) + 1) * sizeof(WCHAR));
...
BOOL PipDarwinUninitialize(HANDLE IN Handle)
/*++
Routine Description:
    This function closes all handles, and frees all memory associated with the enumeration.
Arguments:
...
*/
{
    ...
    AppInfo = Information;
    RetVal = MsiConfigureProduct(AppInfo->ProductId, INSTALLLEVEL_DEFAULT, INSTALLSTATE_ABSENT);
    if (RetVal == ERROR_SUCCESS) {
        // From appmgmt.h.
...
*/
{
    ...
    RetVal = CreateProcess(NULL, CommandLine, NULL, NULL, FALSE, 0, NULL, NULL, &StartupInfo, &ProcessInformation);
...
Arguments:
    ...
    Action - Supplies the actual repair action to perform (PIP_REPAIR_REPAIR, or PIP_REPAIR_REINSTALL).
Return Value:
    TRUE for Success
*/
{
    ...
    if (Action == PIP_REPAIR_REINSTALL) {
        ReinstallAction = REINSTALLMODE_REPAIR | REINSTALLMODE_FILEREPLACE | REINSTALLMODE_USERDATA | REINSTALLMODE_MACHINEDATA | REINSTALLMODE_SHORTCUT;
    ...
    RetVal = MsiReinstallProduct(AppInfo->ProductId, ReinstallAction);
    if (RetVal == ERROR_SUCCESS) {
        return TRUE;
    } else {
        return FALSE;
...
*/
{
    ...
    ShellExecute(NULL, NULL, AppInfo->UpgradeUrl, NULL, NULL, SW_SHOWDEFAULT);
...
/*++
Routine Description:
    This function frees the type specific information for Darwin apps.
Arguments:
...
*/
{
    ...
    HeapFree(GetProcessHeap(), 0, DarwinInfo->ProductId);

    if (DarwinInfo->UpgradeUrl) {
        HeapFree(GetProcessHeap(), 0, DarwinInfo->UpgradeUrl);
    }

    HeapFree(GetProcessHeap(), 0, DarwinInfo);
}