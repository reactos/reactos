...
Notes:
bugbug need to adjust dialog contents based on hardware availability.
...
static LPWSTR AddStartRadioButtonsSelected(HWND DialogWindow);
static VOID DisableControls(HWND Dialog);
static BOOL MediaSelectRadioButtonsSelected(HWND DialogWindow);
static VOID PopulateAddListView(HWND ListViewWindow);
BOOL MediaChangeProc(HWND Dialog, UINT Msg, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK SubClassProc(HWND PropertySheet, UINT Msg, WPARAM wParam, LPARAM lParam);
...
BOOL AddSourceInitProc(HWND DialogWindow, UINT Msg, WPARAM wParam, LPARAM lParam)
/*++
Routine Description:
This function handles initialization for IDD_ADD_SOURCE.
Currently, it just creates the tool tip for the unimplemented functionality.
Arguments:
