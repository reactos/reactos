...
BOOL Pip95Uninstall(HANDLE Information);
BOOL Pip95Modify(HANDLE Information);
VOID Pip95FreeHandle(HANDLE Handle);
VOID Pip95GetProperties(HANDLE Handle, PPIPAPPLICATIONPROPERTIES Properties);
...
HANDLE PipInitialize(VOID)
/*++
Routine Description:
    This routine performs initialization for enumerating Logo95/Logo3 applications.
Arguments:
    None
Return Value:
...
TryTheNextOne:
    ...
    KeyNameSize = 256;
    RetVal = RegEnumKeyEx(PipInfo->Key, PipInfo->EnumIndex, KeyName, &KeyNameSize, NULL, NULL, NULL, &FileTime);
    PipInfo->EnumIndex++;
    if (RetVal != ERROR_SUCCESS) {
        return RetVal;
    }
    ...
    RetVal = RegOpenKeyEx(PipInfo->Key, KeyName, 0, KEY_READ, &SubKey);
    if (RetVal != ERROR_SUCCESS) {
        return RetVal;
    }
    ...
    RetVal = RegQueryValueEx(SubKey, L"DisplayName", 0, &Type, (PBYTE)ProductName, &ProductNameSize);
    ...
    RetVal = ReadAndExpandRegString(SubKey, Heap, L"UninstallString", &UninstallString);
    if (RetVal != ERROR_SUCCESS) {
        // If there is no uninstall string, the we just try to find the ModifyPath
        if (RetVal != ERROR_FILE_NOT_FOUND) {
            goto cleanup;
        }
    } else {
    ...
    // Get the ModifyPath
    RetVal = ReadAndExpandRegString(SubKey, Heap, L"ModifyPath", &ModifyPathString);
    if (RetVal != ERROR_SUCCESS) {
    ...
    if (Caps == 0) {
        ...
        Logo95Info = HeapAlloc(Heap, 0, sizeof(LOGO95INFO));
        ...
        Logo95Info->RegKeyName = HeapAlloc(Heap, 0, (KeyNameSize + 1) * sizeof(WCHAR));
...
cleanup:
    if (ModifyPathString != NULL) {
        HeapFree(Heap, 0, ModifyPathString);
    }
    ...
    RetVal = ReadAndExpandRegString(SubKey, GetProcessHeap(), L"ModifyPath", &ModifyPath);
    if (RetVal != ERROR_SUCCESS) {
        *Capabilities = 0;
        ApplicationInformation->h = NULL;
        if (RetVal != ERROR_FILE_NOT_FOUND) {
        ...
        *Capabilities = PIP_UNINSTALL;
        ...
        Logo95Info = HeapAlloc(GetProcessHeap(), 0, sizeof(LOGO95INFO));
        ApplicationInformation->h = (HANDLE)Logo95Info;
        Logo95Info->UninstallString = HeapAlloc(GetProcessHeap(), 0, UninstallStringSize * sizeof(WCHAR));

        Logo95Info->RegKeyName = HeapAlloc(GetProcessHeap(), 0, (KeyNameSize + 1) * sizeof(WCHAR));
        lstrcpy(Logo95Info->RegKeyName, KeyName);
        Actions->Uninstall = Pip95Uninstall;
        Actions->GetProperties = Pip95GetProperties;
    }
...
BOOL PipUninitialize(HANDLE IN Handle)
/*++
Routine Description:
...
*/
{
    ...
    RetVal = CreateProcess(NULL, Logo95Info->UninstallString, NULL, NULL, FALSE, 0, NULL, NULL, &StartupInfo, &ProcessInformation);

    // Wait for the uninstall to finish.  Otherwise, when we
    // update the listbox of installed apps, it looks like the one we're uninstalling is still installed

    WaitHandles[0] = ProcessInformation.hProcess;

    do {
        WaitRetVal = MsgWaitForMultipleObjects(1, WaitHandles, FALSE, INFINITE, QS_ALLEVENTS);
...
*/
{
    ...
    RetVal = CreateProcess(NULL, Logo95Info->ModifyPath, NULL, NULL, FALSE, 0, NULL, NULL, &StartupInfo, &ProcessInformation);
    CloseHandle(ProcessInformation.hProcess);
    CloseHandle(ProcessInformation.hThread);
    return RetVal;
}
...
/*++
Routine Description:
    This function frees the type specific information for logo95 apps.
Arguments:
...
VOID Pip95GetProperties(HANDLE Handle, PPIPAPPLICATIONPROPERTIES Properties)
{
    ...
    RetVal = RegOpenKeyEx(RegKey, Logo95Info->RegKeyName, 0, KEY_READ, &SubKey);
    RegCloseKey(RegKey);
    if (RetVal != ERROR_SUCCESS) {
        return;
    }
    ...
    RegQueryValueEx(SubKey, L"DisplayName", 0, &Type, (LPBYTE)Properties->ProductName, &(Properties->ProductNameSize));
    RegQueryValueEx(SubKey, L"Publisher", 0, &Type, (LPBYTE)Properties->Publisher, &(Properties->PublisherSize));
    RegQueryValueEx(SubKey, L"ProductID", 0, &Type, (LPBYTE)Properties->ProductID, &(Properties->ProductIDSize));
    RegQueryValueEx(SubKey, L"RegOwner", 0, &Type, (LPBYTE)Properties->RegisteredOwner, &(Properties->RegisteredOwnerSize));
    RegQueryValueEx(SubKey, L"RegCompany", 0, &Type, (LPBYTE)Properties->RegisteredCompany, &(Properties->RegisteredCompanySize));
    RegQueryValueEx(SubKey, L"HelpTelephone", 0, &Type, (LPBYTE)Properties->SupportTelephone, &(Properties->SupportTelephoneSize));
    RegQueryValueEx(SubKey, L"InstallLocation", 0, &Type, (LPBYTE)Properties->InstallLocation, &(Properties->InstallLocationSize));
    RegQueryValueEx(SubKey, L"InstallSource", 0, &Type, (LPBYTE)Properties->InstallSource, &(Properties->InstallSourceSize));
}