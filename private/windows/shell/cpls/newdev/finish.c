...
BOOL IsNullDriverInstalled(HDEVINFO hDeviceInfo, PSP_DEVINFO_DATA DeviceInfoData)
/*
Routine Description:
    This routine determines whether a null driver, or no driver at all, is installed for this device instance.
    Currently the test is that I know a null driver was installed if the "Driver" value entry doesn't exist.
Arguments:
...
--*/
{
    ...
    if (SetupDiGetDeviceRegistryProperty(hDeviceInfo, DeviceInfoData, SPDRP_DRIVER, NULL, pvBuffer, sizeof(Buffer), NULL)) {
        return FALSE;
    } else {
        return TRUE;
    }
...
--*/
{
    ...
    try {
        ...
        if (!SetupDiGetDriverInfoDetail(NewDevWiz->hDeviceInfo, &NewDevWiz->DeviceInfoData, &DriverInfoData, pDriverInfoDetailData, Size, &Size)) {
            SavedError = GetLastError();
            goto clean0;
        }

        Error = RegCreateKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Reinstall"), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKeyReinstall, &RegCreated);
        ...
        *ptchDst = TEXT('\0');
        ...
        Error = RegCreateKeyEx(hKeyReinstall, StringId, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKeyDeviceInstance, &RegCreated);
        ...
        Error = RegSetValueEx(hKeyDeviceInstance, TEXT("DisplayName"), 0, REG_SZ, (PVOID)DisplayName, lstrlen(DisplayName)*sizeof(TCHAR)+sizeof(TCHAR));
        ...
        Error = RegSetValueEx(hKeyDeviceInstance, TEXT("StringId"), 0, REG_SZ, (PVOID)pDriverInfoDetailData->HardwareID, lstrlen(pDriverInfoDetailData->HardwareID)*sizeof(TCHAR)+sizeof(TCHAR));
        if (SavedError == ERROR_SUCCESS)
        {
            SavedError = Error;
        }

        Error = RegSetValueEx(hKeyDeviceInstance, TEXT("ReinstallString"), 0, REG_SZ, (PVOID)BackupPath, lstrlen(BackupPath)*sizeof(TCHAR)+sizeof(TCHAR));
        if (SavedError == ERROR_SUCCESS)
        {
            SavedError = Error;
        }
                                          ...
                                          ))
            {
                if (BackupOldDrivers)
                {
                    ...
                    if (MessageHandlerContext)
                    {
                        ...
                        if ((Error == ERROR_SUCCESS) && (BackupPath[0] != TEXT('\0'))) {
    ...
    )
{
    ...
    *TextBuffer = TEXT('\0');
    ...
    if (Error != ERROR_SUCCESS)
    {
        ...
        if (FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, HRESULT_FROM_SETUPAPI(Error), 0, (LPTSTR)&ErrorMsg, 0, NULL))
        {
       ...
       switch (((NMHDR FAR *)lParam)->code) {
           ...
           case PSN_RESET:
            ...
            case PSN_WIZFINISH:
            {
                ...
                if (RegCreateKeyEx(HKEY_CURRENT_USER, REGSTR_PATH_DEVICEINSTALLER, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKeyDeviceInstaller, &RegCreated) == ERROR_SUCCESS) {
                    RegSetValueEx(hKeyDeviceInstaller, REGSTR_VAL_SEARCHOPTIONS, 0, REG_DWORD, (PVOID)&(NewDevWiz->SearchOptions), sizeof(DWORD));
