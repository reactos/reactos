...
void InstallSilentChilds(HWND hwdnParent, PNEWDEVWIZ NewDevWiz);


BOOL RegistryDeviceName(DEVINST DevInst, PTCHAR  Buffer, DWORD   cbBuffer)
{
    ULONG ulSize;
    CONFIGRET ConfigRet;
...
BOOL IntializeDeviceMapInfo(void)
{
    ...
    Status = NtQueryInformationProcess(NtCurrentProcess(), ProcessDeviceMap, &ProcessDeviceMapInfo.Query, sizeof(ProcessDeviceMapInfo.Query), NULL);
   ...
   )
{
    ...
    switch (message) {
        ...
        case WUM_SEARCHDRIVERS: {
            ...
            if (NewDevWiz->WizardType == NDWTYPE_FOUNDNEW && !(NewDevWiz->Capabilities & CM_DEVCAP_SILENTINSTALL)) {
                InstallSilentChilds(hDlg, NewDevWiz);
            }
