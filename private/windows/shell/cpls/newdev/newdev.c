...
typedef BOOL(*PINSTALLNEWDEVICE)(HWND hwndParent, LPGUID ClassGuid, PDWORD pReboot);
...
)
{
    ...
    // (HdwWiz, devmgr, cpl applets etc. and so don't have a proper intro page.
...
BOOL InstallDeviceWizard(HWND hWnd, PNEWDEVWIZ NewDevWiz)
{
    int  PropSheetResult;
    PNDWPROPERTYSHEET NdwPropertySheet;
...
// compatible drivers.
BOOL UpdateDeviceWizard(HWND hWnd, PNEWDEVWIZ NewDevWiz)
{
    int  PropSheetResult;
    PNDWPROPERTYSHEET NdwPropertySheet;
...
BOOL InstallSelectedDriver(HWND hwndParent, HDEVINFO hDeviceInfo, LPCWSTR DisplayName, BOOL Backup, PDWORD pReboot)
/*
Routine Description:
...
Arguments:
   ...
   HDEVINFO hDeviceInfo - DeviceInfoList which supplies the selected device to install the selected driver on.
   DisplayName - Friendly backup string
   Backup - BOOL that indicates whether or not we should back up the current drivers before installing the new ones.
...
Return Value:
    BOOL    TRUE if the driver was installed
            FALSE if the driver was not installed.  Check GetLastError() to see if the specific error.
--*/
{
    NEWDEVWIZ  NewDevWiz;
    ...
    try {
        ...
        if (!SetupDiGetSelectedDevice(NewDevWiz.hDeviceInfo, &NewDevWiz.DeviceInfoData))
        {
...
--*/
{
    ...
    try {
        ...
        if (SetupDiGetDeviceInstallParams(NewDevWiz.hDeviceInfo, &NewDevWiz.DeviceInfoData, &DeviceInstallParams))
        {
        ...
        // If we still can't find a driver, then start at the driver search page.

        // otherwise go straight to the finish page and install.
        // To preserve drivers preselected by the caller (legacy detect)
        // the currently SelectedDriver is used, but if there is no selected driver the highest ranking  driver is used.

        DriverInfoData.cbSize = sizeof(SP_DRVINFO_DATA);
        DriversFound = SetupDiEnumDriverInfo(NewDevWiz.hDeviceInfo, &NewDevWiz.DeviceInfoData, SPDIT_COMPATDRIVER, 0, &DriverInfoData);
        if (!DriversFound) {
            ...
            if (SetupDiBuildDriverInfoList(NewDevWiz.hDeviceInfo, &NewDevWiz.DeviceInfoData, SPDIT_COMPATDRIVER))
            {
                SetupDiCallClassInstaller(DIF_SELECTBESTCOMPATDRV, NewDevWiz.hDeviceInfo, &NewDevWiz.DeviceInfoData);
            }
        ...
        if (DriversFound) {
            ...
            if (!SetupDiGetSelectedDriver(NewDevWiz.hDeviceInfo, &NewDevWiz.DeviceInfoData, &SelectedDriverInfo))
            {
                SetupDiSetSelectedDriver(NewDevWiz.hDeviceInfo, &NewDevWiz.DeviceInfoData, &DriverInfoData);
            }
...
Routine Description:
   ...
   functions now).  This entry point just forwards the function call onto hdwwiz.cpl now.
