    ...
    )
{
    ...
    ConfigRet = CM_Get_Child_Ex(&DeviceInstance, ParentNode->DevInst, 0, DeviceTree->hMachine);
    ...
    while (ConfigRet == CR_SUCCESS) {
        ...
        Len = sizeof(Buffer);
        ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_CLASSGUID, NULL, &Buffer, &Len, 0, DeviceTree->hMachine);

        tvi.item.mask = TVIF_TEXT | TVIF_PARAM;
        if (ConfigRet == CR_SUCCESS) {
            pSetupGuidFromString(Buffer, &DeviceTreeNode.ClassGuid);
            if (SetupDiGetClassImageIndex(&DeviceTree->ClassImageList, &DeviceTreeNode.ClassGuid, &tvi.item.iImage))
            {
        ...
        *pDeviceTreeNode = DeviceTreeNode;
        ...
        Len = sizeof(Buffer);
        ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_DEVICEDESC, NULL, (PVOID) Buffer, &Len, 0, DeviceTree->hMachine);
        if (ConfigRet != CR_SUCCESS) {
    ...
    ACD_SkipIt:
        ...
        // Next sibling ...
        ConfigRet = CM_Get_Sibling_Ex(&DeviceInstance, DeviceInstance, 0, DeviceTree->hMachine);
    }
...
void AddEditLine(HWND hwndEdit, PTCHAR LineBuffer)
{
   wcscat(LineBuffer, L"\r\n");
...
void MultiSzToCommas(PWCHAR Buffer)
{
   PWCHAR pwch;
...
void FillRegKeyValues(HWND hwndEdit, HKEY hKey, PTCHAR LineBuffer, PVOID Buffer, int    LenBuffer)
{
    ...
    while (TRUE) {
        ...
        LenData = LenBuffer;
        Error = RegEnumValue(hKey, Index++, NameBuffer, &Len, NULL, &Type, Buffer, &LenData);
...
void FillDeviceStatus(PDEVICETREE DeviceTree, PDEVTREENODE DeviceTreeNode)
{
   ...
   *Buffer= '\0';
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_Device_ID_Ex(DeviceInstance, (PVOID)Buffer, Len, 0, DeviceTree->hMachine);
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_FRIENDLYNAME, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_MFG, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_CLASS, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_CLASSGUID, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_DRIVER, NULL, (PVOID)DeviceTreeNode->Driver, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_SERVICE, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_HARDWAREID, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Buffer);
   memset(Buffer, 0 , sizeof(Buffer));
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_COMPATIBLEIDS, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_PHYSICAL_DEVICE_OBJECT_NAME, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Buffer);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_LOCATION_INFORMATION, NULL, (PVOID)Buffer, &Len, 0, DeviceTree->hMachine);
   if (ConfigRet != CR_SUCCESS) {
       wcscpy(Buffer, TEXT("?"));
       }
   ...
   Len = sizeof(Problem);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_CAPABILITIES, NULL, (PVOID)&Problem, &Len, 0, NULL);
   if (ConfigRet != CR_SUCCESS) {
       Problem = (ULONG)-1;
       }
   ...
   Len = sizeof(Problem);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_CONFIGFLAGS, NULL, (PVOID)&Problem, &Len, 0, NULL);
   if (ConfigRet != CR_SUCCESS) {
       Problem = (ULONG)-1;
       }
   ...
   Len = sizeof(Problem);
   ConfigRet = CM_Get_DevNode_Registry_Property_Ex(DeviceInstance, CM_DRP_UI_NUMBER, NULL, (PVOID)&Problem, &Len, 0, NULL);
   if (ConfigRet != CR_SUCCESS) {
       Problem = (ULONG)-1;
       }
   ...
   ConfigRet = CM_Get_DevNode_Status_Ex(&DevNodeStatus, &Problem, DeviceInstance, 0, DeviceTree->hMachine);
   ...
   ConfigRet = CM_Open_Class_Key_ExW(&DeviceTreeNode->ClassGuid, NULL, KEY_READ, RegDisposition_OpenExisting, &hKeyClass, 0, DeviceTree->hMachine);
   if (ConfigRet == CR_SUCCESS) {
       FillRegKeyValues(DeviceTree->hwndEdit, hKeyClass, LineBuffer, Buffer, sizeof(Buffer));
       ...
       if (DeviceTreeNode->Driver && (pwch = wcschr(DeviceTreeNode->Driver, L'\\'))) {
           ...
           AddEditLine(DeviceTree->hwndEdit,LineBuffer);
           Error = RegOpenKeyEx(hKeyClass, ++pwch, 0, KEY_READ, &hKeyDriver);
           if (Error == CR_SUCCESS) {
               FillRegKeyValues(DeviceTree->hwndEdit, hKeyDriver, LineBuffer, Buffer, sizeof(Buffer));
...
BOOL InitDevTreeDlgProc(HWND hDlg, PDEVICETREE DeviceTree)
{
    CONFIGRET ConfigRet;
    HWND hwndTree;
    HTREEITEM hTreeItem;
...
#if 0
    // Create an empty classless DeviceInfoList. We will use this list for easy access to the information of each devnode.
    // TBD: make remotable.
    ...
    ConfigRet = CM_Locate_DevNode_Ex(&DeviceTree->RootNode.DevInst, NULL, CM_LOCATE_DEVNODE_NORMAL, DeviceTree->hMachine);
    if (ConfigRet != CR_SUCCESS) {
