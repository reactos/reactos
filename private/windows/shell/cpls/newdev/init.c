   ...
   )
{
    ...
    Error = RegQueryValueEx(hKey, REGSTR_VAL_DEVICEPATH, NULL, &Type, (LPBYTE)ValueDevicePath, &cbData);
    ...
    )
{
    ...
    while (Result &&
        ...
        SetupDiGetDeviceRegistryProperty(hDevInfo, &DeviceInfoData, SPDRP_HARDWAREID, NULL, NULL, 0, &Size);

        HardwareIdList = malloc(Size + 2);
        if (HardwareIdList) {
            if (SetupDiGetDeviceRegistryProperty(hDevInfo, &DeviceInfoData, SPDRP_HARDWAREID, NULL, (PBYTE)HardwareIdList, Size, &Size)) {
                // If any of the devices HardwareIds match the given ID then we have a match and need to upgrade the drivers on this device.
                for (SingleHardwareId = HardwareIdList; *SingleHardwareId; SingleHardwareId += lstrlen(SingleHardwareId) + 1) {
    ...
    int nShow)
{
    ...
    hThread = CreateThread(NULL, 0, DevInstallUiOnlyThread, (PVOID)hPipeRead, 0, &ThreadId);
    if (!hThread) {
