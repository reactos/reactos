...
BOOLEAN ReadSource(void)
{
    ...
    fh = CreateFile(SRC_NAME, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
VOID GetDischargePolicies(UINT uiLine, UINT uiNum, UINT uiIndex)
{
    UINT    i, uiOne = 1;
    ...
    GetNAToken();
    g_gupp.DischargePolicy[uiIndex].PowerPolicy.EventCode |= GetFlagToken(POWER_LEVEL_USER_NOTIFY_SOUND);

    GetSleepActionFlagsGlobal(uiLine + DP_SLEEP_ACT_FLAGS, NULL, &(g_gupp.DischargePolicy[uiIndex].PowerPolicy));
...
*/
{
    ...
    GetSleepActionFlagsGlobal(POWER_BUTTON_SLEEP_ACTION_FLAGS_LINE, &(g_gupp.PowerButtonAc), &(g_gupp.PowerButtonDc));
    ...
    GetSleepActionFlagsGlobal(SLEEP_BUTTON_SLEEP_ACTION_FLAGS_LINE, &(g_gupp.SleepButtonAc), &(g_gupp.SleepButtonDc));
    ...
    GetSleepActionFlagsGlobal(LID_CLOSE_SLEEP_ACTION_FLAGS_LINE, &(g_gupp.LidCloseAc), &(g_gupp.LidCloseDc));
...
void __cdecl main(int argc, char **argv)
{
    ...
    if (uiNameCount) {
        ...
        if (uiDescCount == uiNameCount) {
            ...
            GetPolicies();// Get the power policies, schemes            
            GetGlobalPolicies();// Get the global power policies           
            WriteRegSpec(fIni, pszName, pszDesc, g_uiPoliciesCount); // Write the registry specification file.
