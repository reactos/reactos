    ...
    case DLL_PROCESS_ATTACH:
        ...
        if (!g_hmodNTDLL) {
            ...
            g_pfnNtPowerInformation =
                (PFNNTPOWERINFORMATION)GetProcAddress(g_hmodNTDLL, "NtPowerInformation");
            if (!g_pfnNtPowerInformation) {
                DebugPrint("Unable to resolve address: NtPowerInformation");
                return FALSE;
            }
            g_pfnNtInitiatePwrAction = (PFNNTINITIATEPWRACTION)GetProcAddress(g_hmodNTDLL, "NtInitiatePowerAction");
            if (!g_pfnNtPowerInformation) {
                DebugPrint("Unable to resolve address: NtPowerInformation");
                return FALSE;
            }
...
BOOLEAN CanUserWritePwrScheme(VOID)
{
    ...
    dwSize = sizeof(szNum);
    if (ReadWritePowerValue(HKEY_LOCAL_MACHINE, c_szREGSTR_PATH_MACHINE_POWERCFG, c_szREGSTR_VAL_LASTID, szNum, &dwSize, FALSE, TRUE)) {
        // Write the value back out, this may fail if user doesn't have write access.
        if (ReadWritePowerValue(HKEY_LOCAL_MACHINE, c_szREGSTR_PATH_MACHINE_POWERCFG, c_szREGSTR_VAL_LASTID, szNum, &dwSize, TRUE, TRUE)) {
            return TRUE;
        }
        else {
            lErr = GetLastError();
...
BOOLEAN GetPwrDiskSpindownRange(PUINT puiMax, PUINT puiMin)
{
    ...
    if (ReadPowerIntOptional(HKEY_LOCAL_MACHINE, c_szREGSTR_PATH_MACHINE_POWERCFG, c_szREGSTR_VAL_SPINDOWNMAX, puiMax) &&
        ReadPowerIntOptional(HKEY_LOCAL_MACHINE, c_szREGSTR_PATH_MACHINE_POWERCFG, c_szREGSTR_VAL_SPINDOWNMIN, puiMin)) {
        return TRUE;
    }
...
*/
{
    ...
    if (!OpenMachineUserKeys(c_szREGSTR_PATH_USER_POWERCFG_POLICIES, c_szREGSTR_PATH_MACHINE_POWERCFG_POLICIES, &hKeyUser, &hKeyMachine)) {
    ...
    while (lRet == ERROR_SUCCESS) {
        ...
        if ((lRet = RegEnumKeyEx(hKeyUser, dwIndex, g_szRegValue, &dwSize, NULL, NULL, NULL, &ft)) == ERROR_SUCCESS) {
            // Open the Policies Key. The key name is the policies ID.
            lpszDesc = NULL;
            if (MyStrToInt(g_szRegValue, &uiID)) {
                if ((lRet = RegOpenKeyEx(hKeyUser, g_szRegValue, 0, KEY_READ, &hKeyPolicyUser)) == ERROR_SUCCESS) {
                    if ((lRet = RegOpenKeyEx(hKeyMachine, g_szRegValue, 0, KEY_READ, &hKeyPolicyMachine)) == ERROR_SUCCESS) {
                        // Get the friendly name..
                        dwNameSize = MAX_NAME_SIZE;
                        if ((lRet = RegQueryValueEx(hKeyPolicyUser, TEXT("Name"), NULL, NULL, (PBYTE)szNameBuf, &dwNameSize)) == ERROR_SUCCESS) {
                            // Descriptions are optional.
                            dwDescSize = MAX_DESC_SIZE;
                            if ((lRet = RegQueryValueEx(hKeyPolicyUser, TEXT("Description"), NULL, NULL, (PBYTE)lpszDescBuf, &dwDescSize)) == ERROR_SUCCESS) {
                                lpszDesc = lpszDescBuf;
                            }

                            // Read the user and machine policies.
                            dwSize = sizeof(upp);
                            if ((lRet = RegQueryValueEx(hKeyPolicyUser, TEXT("Policies"), NULL, NULL, (PBYTE)&upp, &dwSize)) == ERROR_SUCCESS) {
                                dwSize = sizeof(mpp);
                                if ((lRet = RegQueryValueEx(hKeyPolicyMachine, TEXT("Policies"), NULL, NULL, (PBYTE)&mpp, &dwSize)) == ERROR_SUCCESS) {
                                    // Merge the user and machine policies.
                                    if (MergePolicies(&upp, &mpp, &pp)) {
                                        // Call the enumerate proc.
                                        if (!lpfn(uiID, dwNameSize, szNameBuf, dwDescSize, lpszDesc, &pp, lParam)) {
...
BOOLEAN ReadGlobalPwrPolicy(PGLOBAL_POWER_POLICY  pgpp)
{
...
DWORD SetPrivilegeAttribute(LPCTSTR PrivilegeName, DWORD   NewPrivilegeAttribute, DWORD   *OldPrivilegeAttribute)
{
    ...
    if (!AdjustTokenPrivileges(TokenHandle, FALSE, &TokenPrivileges, sizeof(TOKEN_PRIVILEGES), &OldTokenPrivileges, &ReturnLength)) {
        ...
        if (OldPrivilegeAttribute != NULL) {
            //  If the privilege changed, store the old value.  If it did not change, store the value passed in.
...
NTSTATUS CallNtSetValidateAcDc(BOOLEAN bValidate, PVOID InputBufferAc OPTIONAL, PVOID OutputBufferAc OPTIONAL, PVOID InputBufferDc OPTIONAL, PVOID OutputBufferDc OPTIONAL)
{
    NTSTATUS                ntsRetVal;
    ...
    ntsRetVal = g_pfnNtPowerInformation(pilAc, InputBufferAc, sizeof(SYSTEM_POWER_POLICY), OutputBufferAc, sizeof(SYSTEM_POWER_POLICY));
    if (ntsRetVal == STATUS_SUCCESS) {
        pil = pilDc;
        ntsRetVal = g_pfnNtPowerInformation(pilDc, InputBufferDc, sizeof(SYSTEM_POWER_POLICY), OutputBufferDc, sizeof(SYSTEM_POWER_POLICY));
    }
...
VOID InitAdmin(PADMINISTRATOR_POWER_POLICY papp)
{
    ...
    if (g_bAdminOverrideActive) {
        ntsRetVal = CallNtPowerInformation(AdministratorPowerPolicy, &g_app, sizeof(ADMINISTRATOR_POWER_POLICY), &g_app, sizeof(ADMINISTRATOR_POWER_POLICY));
        if (ntsRetVal != STATUS_SUCCESS) {
