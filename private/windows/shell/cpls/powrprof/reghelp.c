...
)
{
    ...
    if ((lRet = RegCreateKeyEx(hKeyUser, lpszSchemeName, 0, TEXT(""), REG_OPTION_NON_VOLATILE, sam, NULL, phKeyUser, &dwDisposition)) == ERROR_SUCCESS) {
        if (!bMustExist || (dwDisposition == REG_OPENED_EXISTING_KEY)) {
            if ((lRet = RegCreateKeyEx(hKeyMachine, lpszSchemeName, 0, TEXT(""), REG_OPTION_NON_VOLATILE, sam, NULL, phKeyMachine, &dwDisposition)) == ERROR_SUCCESS) {
                if (!bMustExist || (dwDisposition == REG_OPENED_EXISTING_KEY)) {
                    bRet = TRUE;
                }
            }
...
)
{
   ...
   LONG     lRet;
    ...
    if ((lRet = RegCreateKeyEx(hKey, lpszPath, 0, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKeyPath, &dwDisposition)) == ERROR_SUCCESS) {
       if (lpszValue) {
           dwSize = (lstrlen(lpszValue) + 1) * sizeof(TCHAR);
           if ((lRet = RegSetValueEx(hKeyPath, lpszValueName, 0, REG_SZ, (PBYTE) lpszValue, dwSize)) == ERROR_SUCCESS) {
               bRet = TRUE;
           }
       }
       else {
...
)
{
    ...
    if (bWrite) {
        ...
        // Read current case.
        if ((lRet = RegQueryValueEx(hKeyPath, lpszValueName, NULL, NULL, (PBYTE) lpszValue, lpdwSize)) == ERROR_SUCCESS) {
            bRet = TRUE;
        }
    }
...
)
{
    ...
    if ((lRet = RegSetValueEx(hKeyUser, TEXT("Policies"), 0, REG_BINARY, (PBYTE) lpvUser, dwcbUserSize)) == ERROR_SUCCESS) {
        // Write the binary policies data
        if ((lRet = RegSetValueEx(hKeyMachine, TEXT("Policies"), 0, REG_BINARY, (PBYTE) lpvMachine, dwcbMachineSize)) == ERROR_SUCCESS) {
