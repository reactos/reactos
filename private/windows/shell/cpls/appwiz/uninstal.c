...
void Uninstall_UninstallCurrentItem( HWND dlg, LPWIZDATA lpwd )
{
    ...
    if( item )
    {
        ...
        {
            DWORD idthread = 0;
            HANDLE thread = CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE)Uninstall_TrackingThread, info, CREATE_SUSPENDED, &idthread );
            if( thread )
            {
                STARTUPINFO startup;
                BOOL bRet;
                ...
                if (bWx86Enabled && bForceX86Env) {
                    Len = GetEnvironmentVariableW(ProcArchName, ProcArchValue, sizeof(ProcArchValue));
                ...
                bRet = CreateProcess(NULL, item->command, NULL, NULL, FALSE, dwCreationFlags, NULL, NULL, &startup, &info->uninstaller);
#ifdef WX86
                if (bWx86Enabled && bForceX86Env) {
