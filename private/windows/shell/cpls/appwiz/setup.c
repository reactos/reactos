...
BOOL FindBestSetupPrg(LPTSTR lpszExeName, LPTSTR lpszDriveRoot, LPTSTR lpszSpecialCase, LPTSTR lpszAppNames, LPTSTR lpszExtensions)
{
    ...
    *lpszExeName = 0;
    ...
    while (*lpszCurApp)
    {
        ...
        while (*lpszCurExt)
        {
            ...
            if (ProgramExists(szThisOne))
            {
                BOOL fIsWinApp = HIWORD(SHGetFileInfo(szThisOne, 0, NULL, 0, SHGFI_EXETYPE)) > 0;
...
void _inline UpdateFileInfo(LPWIZDATA lpwd, LPTSTR lpszFileName)
{
    ...
    SHGetFileInfo(lpszFileName, 0, &fi, sizeof(fi), SHGFI_ICON | SHGFI_DISPLAYNAME | SHGFI_LARGEICON);
...
BOOL SetupNextPressed(LPWIZDATA lpwd)
{
    ...
    for (iDrive = 0; (!fFoundExe) && (iDrive < 26); iDrive++)
    {
        ...
        if ((iDrvType == DRIVE_REMOVABLE) || (iDrvType == DRIVE_CDROM))
        {
            ...
            {
                //fFoundDisk = TRUE;
                fFoundExe = FindBestSetupPrg(lpwd->szExeName, szDriveRoot, szSpecialCase, szAppNames, szExtensions);
            }
        }
    }
