                  ...
                  DWORD  PreferredValue)
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, m_szRegParallelMap, 0, KEY_QUERY_VALUE, &parallelMap) != ERROR_SUCCESS) {
        return;
    }

    valueSize = sizeof(valueName);
    lptSize = sizeof(lptName);
    while (ERROR_SUCCESS == RegEnumValue(parallelMap, i++, valueName, &valueSize, NULL, &regDataType, (LPBYTE)lptName, &lptSize)) {
...
// @@END_DDKSPLIT
    ...
    if ((hDeviceKey = SetupDiCreateDevRegKey(DeviceInfoSet,
        ...
        if (RegQueryValueEx(hDeviceKey,
            ...
            if (RegQueryValueEx(hDeviceKey,
                ...
                if (SetupDiGetDeviceRegistryProperty(DeviceInfoSet, DeviceInfoData, SPDRP_ENUMERATOR_NAME, NULL, (PBYTE)charBuffer, sizeof(charBuffer), NULL)) {
        ...
        if (!SetupDiGetDeviceRegistryProperty(DeviceInfoSet, DeviceInfoData, SPDRP_SERVICE, NULL, (LPBYTE)lpszService, MAX_PATH * sizeof(TCHAR), NULL)) {
            if (13L == GetLastError())
            {
    ...
    if (LoadString(g_hInst, IDS_FRIENDLY_FORMAT, friendlyNameFormat, CharSizeOf(friendlyNameFormat)) &&
        SetupDiGetDeviceRegistryProperty(DeviceInfoSet, DeviceInfoData, SPDRP_DEVICEDESC, NULL, (PBYTE)deviceDesc, sizeof(deviceDesc), NULL)) {
        wsprintf(charBuffer, friendlyNameFormat, deviceDesc, lptPortName);
    }
    else {
        lstrcpy(charBuffer, lptPortName);// Simply use LPT port name.
    }
