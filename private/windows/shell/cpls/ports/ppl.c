...
Routine Description: FindParams
    Finds the device instance in the linked list.
    If the device is not in the list, then a new node is allocated, inserted into the head of the list and returned to the caller.
Arguments:
    DevInst - Unique Number identifying the device
Return Value:
    POUR_PROP_PARAMS - The node in the list that it represented by the DevInst or NULL if not enough memory is available
--*/
{
    POUR_PROP_PARAMS tmp, current;

    // No need for Enter()/Leave() because this func is called from within  PplAddParams exclusively

    // Iterate through the list and try to find the node with the same DevInst
    for (current = Head; current; current = current->Next) {
        if (current->DeviceInfoData->DevInst == DevInst) {
            OutputDebugString(_T("found duplicate!"));
            // This node in the list represents this DevInst.
            // Clear out all previous information, keeping the following in minde
    ...
    // We didn't find that particular DevInst in the list, so we create one and insert it at the head of the list
...
Routine Description: PplRemoveParams
    Removes Params from the linked list.
    Params is no longer valid after the function returns.
Arguments:
