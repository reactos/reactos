...
--*/
{
    ...
    if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, szComDBPath, 0, (TCHAR *) NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS | KEY_NOTIFY, (LPSECURITY_ATTRIBUTES) NULL, &dbInfo->DBKey, &disposition) != ERROR_SUCCESS) {

        // Try again w/out notification caps

        if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, szComDBPath, 0, (TCHAR *) NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, (LPSECURITY_ATTRIBUTES) NULL, &dbInfo->DBKey, &disposition) != ERROR_SUCCESS) {
            return CreationFailure(PHComDB, dbInfo);
        }
    ...
    if (disposition == REG_CREATED_NEW_KEY) {
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szComDBPathOld, 0, KEY_ALL_ACCESS, &hOldDB) == ERROR_SUCCESS &&
            RegQueryValueEx(hOldDB, szComDBName, 0, &type, NULL, &dbInfo->PortsLength) == ERROR_SUCCESS) {
            ...
            res = RegQueryValueEx(hOldDB, szComDBName, 0, &type, (PBYTE) dbInfo->Ports, &size);
            RegDeleteValue(hOldDB, szComDBName);


            // The value does not exist, write it out

            if (RegSetValueEx(dbInfo->DBKey, szComDBName, 0, REG_BINARY, dbInfo->Ports, dbInfo->PortsLength) != ERROR_SUCCESS) {
                RegCloseKey(hOldDB);
                return CreationFailure(PHComDB, dbInfo);
            }
    ...
    if (!migrated) {
        res = RegQueryValueEx(dbInfo->DBKey, szComDBName, 0, &type, NULL, &dbInfo->PortsLength);
        if (res == ERROR_FILE_NOT_FOUND) {
            ResizeDatabase(dbInfo, COMDB_MIN_PORTS_ARBITRATED);

            // The value does not exist, write it out
            res = RegSetValueEx(dbInfo->DBKey, szComDBName, 0, REG_BINARY, dbInfo->Ports, dbInfo->PortsLength);
    ...
    if (RegQueryValueEx(dbInfo->DBKey,
        ...
        RegSetValueEx(dbInfo->DBKey, szComDBName, 0, REG_BINARY, dbInfo->Ports, dbInfo->PortsLength);
    }
