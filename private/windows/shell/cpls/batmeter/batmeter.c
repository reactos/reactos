...
*/
{
   ...
   if (g_uiBatCount) {
      ...
      g_hwndBatMeter = CreateDialog(g_hInstance, MAKEINTRESOURCE(IDD_BATMETER), hwndParent, BatMeterDlgProc);
      ...
      if ((g_hwndBatMeter) && (hwndFrame)) {
         ...
         if (!MoveWindow(g_hwndBatMeter, rFrame.left, rFrame.top, iWidth, iHeight, FALSE)) {
    ...
    if (GetSystemPowerStatus(&sps) && hWnd) {
        ...
        if (g_hwndBatMeter) {
...
#ifndef SIM_BATTERY
           WalkBatteryState(DEVICES, (WALKENUMPROC)UpdateBatInfoProc, NULL, (LPARAM)NULL, (LPARAM)NULL);
#else
           WalkBatteryState(DEVICES, (WALKENUMPROC)SimUpdateBatInfoProc, NULL, (LPARAM)NULL, (LPARAM)NULL);
#endif
           ...
           if (g_bShowingMulti) {
               ...
               WalkBatteryState(ALL, (WALKENUMPROC)UpdateBatMeterProc, hWnd, (LPARAM)g_bShowingMulti, (LPARAM)bForceUpdate);
           }
           else {
               // Display only the comosite battery information.
               UpdateBatMeterProc(&g_bs, hWnd, (LPARAM)g_bShowingMulti, (LPARAM)bForceUpdate);
           }
           bRet = TRUE;
        }
    }
