...
void OnTimerDeviceChange(PDEVICETREE DeviceTree)
{
...
BOOL RefreshTree(PDEVICETREE DeviceTree)
{
    CONFIGRET ConfigRet;
    ...
    SelectedDevInst = DeviceTree->SelectedTreeNode ? DeviceTree->SelectedTreeNode->DevInst : 0;
    ...
    ConfigRet = CM_Get_Child_Ex(&DeviceInstance, DeviceTree->DevInst, 0, DeviceTree->hMachine);
    if (ConfigRet == CR_SUCCESS) {
        AddChildSiblings(DeviceTree, NULL, DeviceInstance, 0, TRUE);
    }

    DisplayChildSiblings(DeviceTree, &DeviceTree->ChildSiblingList, NULL, FALSE);

    // restore treeview redraw state, and reset the selected item
    DevTreeNode = DevTreeNodeByDevInst(SelectedDevInst, &DeviceTree->ChildSiblingList);
