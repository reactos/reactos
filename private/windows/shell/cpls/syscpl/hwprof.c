...

{
   ...
   while (RegStatus != ERROR_NO_MORE_ITEMS) {
      ...
      if (RegStatus == ERROR_SUCCESS) {
         ...
         if (RegQueryValueEx(hCfgKey, pszRegFriendlyName, NULL, NULL, (LPBYTE)&pInfo->pHwProfValues[ulIndex].szFriendlyName, &ulSize) != ERROR_SUCCESS) {
         ...
         // dock state
         ulSize = sizeof(ULONG);
         if (RegQueryValueEx(hCfgKey, pszRegDockState, NULL, NULL, (LPBYTE)&pInfo->pHwProfValues[ulIndex].ulDockState, &ulSize) != ERROR_SUCCESS) {
            pInfo->pHwProfValues[ulIndex].ulDockState = DOCKINFO_USER_SUPPLIED | DOCKINFO_DOCKED | DOCKINFO_UNDOCKED;
         }

         // portable computer flag
         ulSize = sizeof(ULONG);
         if (RegQueryValueEx(hCfgKey, pszRegIsPortable, NULL, NULL, (LPBYTE)&pInfo->pHwProfValues[ulIndex].bPortable, &ulSize) != ERROR_SUCCESS) {
            pInfo->pHwProfValues[ulIndex].bPortable = FALSE;
         }

         // Dock ID
         ulSize = MAX_DOCKID_LEN * sizeof(TCHAR);
         if (RegQueryValueEx(hCfgKey, pszRegDockID, NULL, NULL, (LPBYTE)&pInfo->pHwProfValues[ulIndex].szDockID, &ulSize) != ERROR_SUCCESS) {
            pInfo->pHwProfValues[ulIndex].szDockID[0] = TEXT('\0');
         }

         // Serial Number
         ulSize = MAX_SERIALNUM_LEN * sizeof(TCHAR);
         if (RegQueryValueEx(hCfgKey, pszRegSerialNumber, NULL, NULL, (LPBYTE)&pInfo->pHwProfValues[ulIndex].szSerialNumber, &ulSize) != ERROR_SUCCESS) {
         ...
         SendMessage(hList, LB_DELETESTRING, pInfo->pHwProfValues[ulIndex].ulPreferenceOrder, 0);
         AppendCurrentTag(
         ...
         lReturn = SendMessage(hList, LB_INSERTSTRING, pInfo->pHwProfValues[ulIndex].ulPreferenceOrder, (LPARAM)(LPCTSTR)szName);

         // store the profile id along with the entry so we can associate the string and the profile id later
      ...
      )
{
   ...
   while (ulBufferIndex < pInfo->ulNumProfiles) {
      if (!(pInfo->pHwProfValues[ulBufferIndex].ulAction & HWP_DELETE)) {
         if (lstrcmpi(pInfo->pHwProfValues[ulBufferIndex].szFriendlyName, pszFriendlyName) == 0) {
...
VOID UnicodeToUL(LPWSTR pString, PULONG pulValue)
{
    ULONG i;
    ...
    *pulValue = 0;
    for (i = 0; i < 4; i++) {
        *pulValue *= 10;
        *pulValue += (pString[i] <= '9') ? (pString[i] - '0') : (pString[i]-'A'+10);
    }
} // UnicodeToUL


BOOL CopyHardwareProfile(
      HWND   hDlg,
      ULONG  ulIndex,
      ULONG  ulProfile,
      LPTSTR szNewFriendlyName
   ...
   )
{
   ...
   if (RegDeleteKey(hParentKey, szKey) != ERROR_SUCCESS) {
      if (RegOpenKeyEx(hParentKey, szKey, 0, KEY_ENUMERATE_SUB_KEYS | KEY_WRITE, &hKey) != ERROR_SUCCESS) {
         return FALSE;
      }
      ...
      while (RegStatus == ERROR_SUCCESS) {
         ...
         ulSize = MAX_PATH;
         RegStatus = RegEnumKeyEx(hKey, 0, szSubKey, &ulSize, NULL, NULL, NULL, NULL);
         if (RegStatus == ERROR_SUCCESS) {
...
VOID DisplayPrivateMessage(HWND  hWnd, UINT  uiPrivateError)
{
...
VOID DisplaySystemMessage(HWND  hWnd, UINT  uiSystemError)
{
   ...
   MessageBox(hWnd, szMessage, pszErrorCaption, MB_OK | MB_ICONSTOP);// display a message box with this error
} // DisplaySystemMessage
   ...
   switch (uMessage)
   {
      case WM_INITDIALOG:
         ...
         if (pProfInfo->ulDockState == 0) {
            pProfInfo->ulDockState = DOCKINFO_USER_SUPPLIED | DOCKINFO_DOCKED | DOCKINFO_UNDOCKED;
         }

         // initialize the dock state radio buttons
         if ((pProfInfo->ulDockState & DOCKINFO_DOCKED) && (pProfInfo->ulDockState & DOCKINFO_UNDOCKED)) {
...
--*/
{
    ...
    if(!GetTokenInformation(Token,TokenGroups,NULL,0,&BytesRequired) && (GetLastError() == ERROR_INSUFFICIENT_BUFFER) && (Groups = LocalAlloc(LPTR, BytesRequired))
