...
BOOL  TaskingDlg( HWND  hdlg, UINT  uMessage, WPARAM  wParam, LPARAM  lParam )
{
    ...
    switch( uMessage )
    {
       case WM_INITDIALOG:
          ...
          RegRes = RegOpenKeyEx( HKEY_LOCAL_MACHINE, m_szRegPriKey, 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &m_Hkey );
          if( RegRes == ERROR_SUCCESS )
          {
              Length = sizeof( Value );
              RegRes = RegQueryValueEx( m_Hkey, m_szRegPriority, NULL, &Type, (LPBYTE) &Value, &Length );
              if( RegRes == ERROR_SUCCESS )
              {
                  switch( Value )
                  {
