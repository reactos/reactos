...
*/
{
    ...
    case WM_MOUSEMOVE:
        ...
        dwState = dwSpinnerState;//  Save copy of original state
    ...
    case WM_RBUTTONUP:
        ...
        if ((UINT) (bRight - WM_LBUTTONDOWN + WM_LBUTTONUP) == message)
        {
            ...
            {
                ClickedRectCalc(hArrow, dwSpinnerState, &rect);//  Calculate the rectangle before clearing states.
                StateClear(dwSpinnerState, SPINNERSTATE_CLICKED | SPINNERSTATE_MOUSEOUT);//  Clear the states so we repaint properly.
