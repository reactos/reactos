...
VOID MakePathName(CHAR ARG_PTR *pszPath, CHAR ARG_PTR *pszFileName)
{
   ...
   if (*pszFileName != '\0' &&
       ! (! IsDBCSLeadByte(pszFileName[0]) && pszFileName[0] == PERIOD && ! IsDBCSLeadByte(pszFileName[1]) && pszFileName[1] == '\0'))
   {
   ...
   if ((pszExt = ExtractExtension(pszFileName)) != NULL)
   {
      ...
      if ((nExtLen = STRLEN(pszExt)) > 0)
      {
         ...
         else
         {
            ...
            if (! IsDBCSLeadByte(*pszPrevious) && *pszPrevious == chEXTENSION_CHAR)
...
** N.b., stream-style I/O routines like fopen() and fclose() may counter the intended effect of this function.
** fclose() writes the current date to any file it's called with which was opened in write "w" or append "a" mode.
** One way to get around this in order to modify the date of a file opened for writing or appending by fopen() is to fclose() the file and fopen() it again in read "r" mode.
** Then set its date and time stamp with CopyDateTimeStamp().
*/
INT CopyDateTimeStamp(INT_PTR doshFrom, INT_PTR doshTo)
{
#ifdef ORGCODE
   ...
   if(!GetFileTime((HANDLE) doshFrom, &lpCreationTime, &lpLastAccessTime, &lpLastWriteTime)){
      return((INT)LZERROR_BADINHANDLE);
   }
   if(!SetFileTime((HANDLE) doshTo, &lpCreationTime, &lpLastAccessTime, &lpLastWriteTime)){
      return((INT)LZERROR_BADINHANDLE);
   }
