...
static BOOL ActuallyTheSameFile(CHAR ARG_PTR *pszFile1, CHAR ARG_PTR *pszFile2);
static BOOL ProcessNotification(CHAR ARG_PTR *pszSource, CHAR ARG_PTR *pszDest, WORD wNotification);
...
static VOID MakeDestFileName(CHAR ARG_PTR *argv [], CHAR ARG_PTR *pszDest)
{
    ...
    {
        STRCPY(pszDest, pszTargetName);// Prepend output file name with destination directory path name.
        pszDestFile = ExtractFileName(pszInFileName);// Isolate source file name from source file specification.
        MakePathName(pszDest, pszDestFile);// Add destination file name to destination directory path specification.
    }
    else
...
static BOOL ActuallyTheSameFile(CHAR ARG_PTR *pszFile1, CHAR ARG_PTR *pszFile2)
{
    ...
    if (GetCanonicalName(pszFile1, szCanonicalName1) && GetCanonicalName(pszFile2, szCanonicalName2))
    {
...
static BOOL ProcessNotification(CHAR ARG_PTR *pszSource, CHAR ARG_PTR *pszDest, WORD wNotification)
{
    ...
    case NOTIFY_START_COMPRESS:
    {
        ...
        {
        case LZX_ALG:
            LoadString(NULL, SID_COMPRESSING_LZX, ErrorMsg, 1024);
            printf(ErrorMsg, pszSource, pszDest, CompressionMemoryFromTCOMP(DiamondCompressionType));
            break;
        case QUANTUM_ALG:
            LoadString(NULL, SID_COMPRESSING_QUANTUM, ErrorMsg, 1024);
            printf(ErrorMsg, pszSource, pszDest, CompressionLevelFromTCOMP(DiamondCompressionType), CompressionMemoryFromTCOMP(DiamondCompressionType));
            break;
        default:
            LoadString(
                NULL,
...
//  Return value is TRUE if time stamp on pszFile1 is newer than the time stamp on pszFile2.
//  If either of the two files do not exist, the return value is also TRUE (for indicating that pszFile2 should be update from pszFile1).
...
static BOOL FileTimeIsNewer(const char* pszFile1, const char* pszFile2)
{
    ...
    if ((_stat(pszFile2, &StatBufDest)) || (_stat(pszFile1, &StatBufSource)) || (StatBufSource.st_mtime > StatBufDest.st_mtime))
        return TRUE;
...
INT __cdecl main(INT argc, CHAR *argv [])
{
    ...
    if ((LANG_JAPANESE == PRIMARYLANGID(wLanguageId)) || (LANG_KOREAN == PRIMARYLANGID(wLanguageId)) || (LANG_CHINESE == PRIMARYLANGID(wLanguageId)))
    {
        ...
        case 949:
        case 950:
            SetThreadLocale(MAKELCID(MAKELANGID(PRIMARYLANGID(GetSystemDefaultLangID()), SUBLANG_ENGLISH_US), SORT_DEFAULT));
            break;
        default:
    ...
    {
        pszInFileName = CharLowerA(argv[iSourceFileName]);// Set up global input file name.
        ...
        if ((!bUpdateOnly) || (FileTimeIsNewer(pszInFileName, chTargetFileName))) {
            if (DiamondCompressionType) {
                fError = DiamondCompressFile(ProcessNotification, pszInFileName, pszOutFileName, bDoRename, pLZI);
            }
            else {
                fError = Compress(ProcessNotification, pszInFileName, pszOutFileName, byteAlgorithm, bDoRename, pLZI);
            }
            ...
            else
            {
                ...
                if (pLZI && pLZI->cblInSize && pLZI->cblOutSize) {
                    ...
                    printf(ErrorMsg, pszInFileName, pLZI->cblInSize, pLZI->cblOutSize, (INT) (100 - 100 * pLZI->cblOutSize / pLZI->cblInSize));
                }
                else {
    ...
    FreeGlobalBuffers(pLZI);// Free memory used by ring buffer and I/O buffers.
    ...
    if (nTotalFiles > 1) {
        ...
        printf(ErrorMsg, nTotalFiles, cblTotInSize, cblTotOutSize, (INT) (100 - 100 * cblAdjOutSize / cblAdjInSize));
    }
