...
{
    // Although not the most efficient possible algoeithm in each case, this algorithm is correct for unicode, sbcs, or dbcs.
...
BOOL PatternMatch(PCSTR pszString, PCSTR pszPattern, IN BOOL fImplyDotAtEnd)
{
    /* RECURSIVE */
    ...
    for ( ; ; )
    {
        ...
        default:
            ...
            {
                if ((*pszPattern == DOT) && (*pszString == END) && (fImplyDotAtEnd == TRUE))
                {
                    fImplyDotAtEnd = FALSE;
                }
                else
    ...
    )
{
    ...
    case fdintCOPY_FILE:
...

        {
            ...
            if (Info->SelectiveFilesSpec != NULL) {
                ...
                if (PatternMatch(Parameters->psz1, Info->SelectiveFilesSpec, fAllowImpliedDot) == FALSE) {
    ...
    case fdintCLOSE_FILE_INFO:
...

        {
            ...
            {
                if (DosDateTimeToFileTime(Parameters->date, Parameters->time, &ftLocal) && LocalFileTimeToFileTime(&ftLocal, &ftUTC))
                {
    ...
    )
{
    ...
    // Get a handle to the source to use to copy the date and time stamp.
...
BOOL IsDiamondFile(IN PSTR FileName, OUT PBOOL ContainsMultipleFiles)
{
