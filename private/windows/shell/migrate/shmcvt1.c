...
void CvtDeskCPL_Win95ToSUR( void ) {
    ...
    for(;;) {
        ...
        RegSetValueEx(hk, pszVName, 0L, dwType, (LPBYTE)&sdw, SIZEOF(sdw));// write the new data back out
    }
...
void CvtCursorsCPL_DaytonaToSUR( void ) {
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szWinCsrSchemes, 0, TEXT(""), REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE, NULL, &hkOut, &dwTmp) != ERROR_SUCCESS) {
        goto ErrorExit;
    }

    // for each value in the source key
    cchClass = ARRAYSIZE(szClass);
    erc = RegQueryInfoKey(hkIn, szClass, &cchClass, NULL, &cSubk, &cchMaxSubk, &cchMaxCls, &iVal, &cchMaxVName, &cbMaxVData, &cbSecDes, &ftLstWr);
    if( erc != ERROR_SUCCESS && erc != ERROR_MORE_DATA)
        goto ErrorExit;
    ...
    for(;;) {
        ...
        *(pszOut-1) = TEXT('\0');
        ...
        RegSetValueEx(hkOut, pszVName, 0L, REG_EXPAND_SZ, (LPBYTE)szOut, (DWORD)(sizeof(TCHAR)*(pszOut - szOut)));// write the new data back out
    }
                        ...
                        == ERROR_SUCCESS )
                {
                    ...
                    if ( lstrcmpi(szOldKeyValue, szNewKeyValue) == 0 )
                    {
                        ...
                        if ( lstrcmp(szOldKeyName, szDefaultScheme) == 0 )
                        {
                            ...
                            RegSetValueEx( hkOldCursors, szSchemeSource, 0, REG_DWORD, (unsigned char *)&iSchemeLocation, sizeof( iSchemeLocation ) );
                        }
                        // remove the user key
