...
DWORD WINAPI QueryDosDeviceW(PCWSTR lpDeviceName, PWSTR lpTargetPath, DWORD ucchMax)
{
    NTSTATUS Status;
    ...
    try {
        if (ARGUMENT_PRESENT(lpDeviceName)) {
            ...
            while (TRUE) {
                Status = NtQueryDirectoryObject(DirectoryHandle, (PVOID) DirInfo, sizeof(DirInfoBuffer), TRUE, RestartScan, &Context, &ReturnedLength);
                if (!NT_SUCCESS(Status)) {//  Check the status of the operation.
