...
Abstract:
    This module contains private function prototypes and types for the 32-bit windows base APIs.
...
// Include message definitions for communicating between client and server portions of the Base portion of the Windows subsystem
#include "basemsg.h"
#include "winuserp.h"
...
{
    return Count > DATA_ATTRIBUTE_LENGTH && !_wcsnicmp( &Name[(Count - DATA_ATTRIBUTE_LENGTH) / sizeof( WCHAR )], BasepDataAttributeType, DATA_ATTRIBUTE_LENGTH / sizeof( WCHAR ));
}

__inline BOOL BasepIsPropertySetAttribute(ULONG Count, const WCHAR *Name)
{
    return Count > PROPERTY_ATTRIBUTE_LENGTH && !_wcsnicmp( &Name[(Count - PROPERTY_ATTRIBUTE_LENGTH) / sizeof( WCHAR )], BasepPropertySetAttributeType, PROPERTY_ATTRIBUTE_LENGTH / sizeof( WCHAR ));
}

PUNICODE_STRING Basep8BitStringToStaticUnicodeString(IN LPCSTR SourceString);
BOOL Basep8BitStringToDynamicUnicodeString(OUT PUNICODE_STRING UnicodeString, IN LPCSTR lpSourceString);
NTSTATUS (*Basep8BitStringToUnicodeString)(PUNICODE_STRING DestinationString, PANSI_STRING SourceString, BOOLEAN AllocateDestinationString);
NTSTATUS (*BasepUnicodeStringTo8BitString)(PANSI_STRING DestinationString, PUNICODE_STRING SourceString, BOOLEAN AllocateDestinationString);
...
typedef NTSTATUS (*BASECLIENTCONNECTROUTINE)(PVOID MustBeNull, PVOID ConnectionInformation, PULONG ConnectionInformationLength);
POBJECT_ATTRIBUTES BaseFormatObjectAttributes(POBJECT_ATTRIBUTES ObjectAttributes, PSECURITY_ATTRIBUTES SecurityAttributes, PUNICODE_STRING ObjectName);
...
NTSTATUS BaseCreateStack(HANDLE Process, SIZE_T StackSize, SIZE_T MaximumStackSize, PINITIAL_TEB InitialTeb);
...
LPWSTR BaseComputeProcessDllPath(LPCWSTR ApplicationName, LPVOID Environment);
...
typedef struct _INIFILE_PARAMETERS {
    ...
    // Remaining fields only valid when parsing an on disk .INI file mapped into memory.
    PVOID TextCurrent;
    PVOID TextStart;
    PVOID TextEnd;
...
// Handles are 32-bit pointers to the u.Object field of a BASE_HANDLE_TABLE_ENTRY.
// Since this field is 4 bytes into the structure and the structures are always on 8 byte boundaries, we can test the 0x4 bit to see if it is a handle.

// In Sundown, these handles are 64-bit pointers to the u.Object field which is 8 bytes into the structure.
// Therefore, we should test the 0x8 bit to see if it is a handle.
...
NTSTATUS CsrBaseClientConnectToServer(PWSTR szSessionDir, PHANDLE phMutant, PBOOLEAN pServerProcess);
NTSTATUS CsrBasepRefreshIniFileMapping(PUNICODE_STRING BaseFileName);
NTSTATUS CsrBasepDefineDosDevice(DWORD dwFlags, PUNICODE_STRING pDeviceName, PUNICODE_STRING pTargetPath);
...
NTSTATUS CsrBasepSetProcessShutdownParam(DWORD dwLevel, DWORD dwFlags);
NTSTATUS CsrBasepGetProcessShutdownParam(LPDWORD lpdwLevel, LPDWORD lpdwFlags);
NTSTATUS CsrBasepSetTermsrvAppInstallMode(BOOL bState);
NTSTATUS CsrBasepCreateThread(HANDLE ThreadHandle, CLIENT_ID ClientId);
NTSTATUS CsrBasepDebugProcess(CLIENT_ID DebuggerClientId, DWORD dwProcessId, PVOID AttachCompleteRoutine);
