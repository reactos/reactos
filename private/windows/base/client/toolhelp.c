...
// private functions
NTSTATUS ThpCreateRawSnap(IN ULONG dwFlags, IN ULONG th32ProcessID, PUCHAR *RawProcess, PRTL_DEBUG_INFORMATION *RawModule, PRTL_DEBUG_INFORMATION *RawDebugInfo);
...
HANDLE WINAPI CreateToolhelp32Snapshot(IN DWORD dwFlags, IN DWORD th32ProcessID)
/*++
Routine Description:
    Takes a snapshot of the Win32 processes, heaps, modules, and threads used by the Win32 processes.
    Returns an open handle to the specified snapshot if successful or  -1 otherwise.

    NOTE that all of the snapshots are global except for the heap and module lists which are process specific.
    To enumerate the heap or module state for all WIN32 processes call with TH32CS_SNAPALL and the current process.
    Then for each process in the TH32CS_SNAPPROCESS list that isn't the current process, do a call with just TH32CS_SNAPHEAPLIST and/or TH32CS_SNAPMODULE.
...
Arguments:
    ...
    th32ProcessID - Supplies a Win32 process identifier. This parameter can be zero to indicate the current process.
        This parameter is used when the TH32CS_SNAPHEAPLIST or TH32CS_SNAPMODULE value is specified.
        ...
        The snapshot handle acts like a Win32 object handle and is subject to the same rules regarding which processes and threads it is valid in.
Return Value:
...
/*++
Routine Description:
    Retrieves information about the first heap that has been allocated by a specified Win32 process.
Arguments:
    SnapSection - Supplies the handle of the snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lphl - Returns a HEAPLIST32 structure.  
...
--*/
{
    ...
    ViewSize = 0;
    SnapshotBase = 0;
    Status = NtMapViewOfSection(SnapSection, NtCurrentProcess(), &SnapshotBase, 0L, 0L, &SectionOffset, &ViewSize, ViewShare, 0L, PAGE_READWRITE);
    if (!NT_SUCCESS(Status)) {
        BaseSetLastNTError(Status);
        return FALSE;
    }
    ...
    if (SnapshotBase->HeapListCount == 0) {
        ...
        memcpy(lphl, (LPHEAPLIST32)((ULONG_PTR)SnapshotBase + (ULONG_PTR)SnapshotBase->HeapListHead), sizeof(HEAPLIST32));
        retv = TRUE;
...
/*++
Routine Description:
    Retrieves information about the next heap that has been allocated by a Win32 process.
Arguments:
    SnapSection - Supplies the handle of the snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lphl - Returns a HEAPLIST32 structure. The calling application must set the dwSize member of HEAPLIST32 to the size, in bytes, of the structure.
Return Value:
...
--*/
{
    ...
    if (SnapshotBase->HeapListIndex < SnapshotBase->HeapListCount) {
        memcpy(lphl, (LPHEAPLIST32)((ULONG_PTR)SnapshotBase + (ULONG_PTR)(&SnapshotBase->HeapListHead[SnapshotBase->HeapListIndex++])), sizeof(HEAPLIST32));
        retv = TRUE;
    } else {
        SetLastError(ERROR_NO_MORE_FILES);
    }
...
/*++
Routine Description:
    Retrieves information about the first block of a heap that has been allocated by a Win32 process.
    Also, create a snapshot of that heap so the Heap32Next functions can walk them.
Arguments:
    lphe - Returns a HEAPENTRY32 structure.  The calling application must set the dwSize member to the size, in bytes, of the structure.
    th32ProcessID - Supplies the identifier of the Win32 process context that owns the heap.
...
Notes:
    since there is no way to save heap snapshots or delete heap snapshots in any way that makes sense
    (because this info is not associated with snapshots etc), the information needs to be generated completely for each call.
    This is very expensive, it would be a good idea to redesign this API...
    We could try and cheat by preserving snapshots until they are completyely iterated or until a new one is taken, etc, but we will just end up with a bunch of memory leaks
--*/
{
    ...
    Status = RtlQueryProcessDebugInformation((HANDLE)LongToHandle(th32ProcessID), RTL_QUERY_PROCESS_HEAP_SUMMARY | RTL_QUERY_PROCESS_HEAP_ENTRIES, ThRawHeapDebugInfo);
    if (!NT_SUCCESS(Status))    {
    ...
    RtlDestroyQueryDebugBuffer(ThRawHeapDebugInfo);// free snapshot
    return retv;
}
...
/*++
Routine Description:
    Retrieves information about the next block of a heap that has been allocated by a Win32 process.
Arguments:
    lphe - Returns a HEAPENTRY32 structure.  The calling application must set the dwSize member to the size, in bytes, of the structure.
Return Value:
    Returns TRUE if information about the next block in the heap has been copied to the buffer or FALSE otherwise.
    The ERROR_NO_MORE_FILES error value is returned by the GetLastError function when no more objects in the heap exist.
note:
    this function can be prone to error since the heap can change between calls to get heaplists, Heap32First, etc.
    There is no good way to manage a snapshot using this model, so we just get to live with it.
--*/
{
    PRTL_DEBUG_INFORMATION ThRawHeapDebugInfo;
    PRTL_HEAP_ENTRY p;
    ...
    Status = RtlQueryProcessDebugInformation((HANDLE)LongToHandle(lphe->th32ProcessID), RTL_QUERY_PROCESS_HEAP_SUMMARY | RTL_QUERY_PROCESS_HEAP_ENTRIES, ThRawHeapDebugInfo);
    if (!NT_SUCCESS(Status))    {
        RtlDestroyQueryDebugBuffer(ThRawHeapDebugInfo);
        BaseSetLastNTError(Status);
        return FALSE;
    ...
    // by the time we reach this point, either we have just exited from scannin a segment, or p-> is pointing at a non-segment entry.
    if(hit_seg == FALSE)
    {
...
BOOL WINAPI Toolhelp32ReadProcessMemory(IN DWORD th32ProcessID, IN LPCVOID lpBaseAddress, OUT PUCHAR lpBuffer, IN DWORD cbRead, OUT LPDWORD lpNumberOfBytesRead)
/*++
Routine Description:
    Copies memory allocated to another process into an application-supplied buffer.
    This function is for people who are too stupid or lazy to open the process themselves.
Arguments:
    th32ProcessID - Supplies the Identifier of the Win32 process whose memory is being copied.
                    This parameter can be zero to copy the memory of the current process.
    lpBaseAddress - Supplies the base address in the specified process to read.
        Before transferring any data, the system verifies that all data in the base address and memory of the specified size is accessible for read access.
    ...
    lpNumberOfBytesRead - Returns the number of bytes copied to the buffer. If this parameter is NULL, it is ignored.
Return Value:
...
/*++
Routine Description:
    Retrieves information about the first Win32 process encountered in a system snapshot.
Arguments:
    SnapSection - Supplies the handle of a snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lppe - Returns a PROCESSENTRY32W structure.  The caller must set the dwSize member to the size, in bytes, of the structure.
Return Value:
...
--*/
{
    ...
    if (SnapshotBase->ProcessCount == 0) {
        ...
        memcpy(lppe, (LPPROCESSENTRY32W)((ULONG_PTR)SnapshotBase + (ULONG_PTR)SnapshotBase->ProcessListHead), sizeof(PROCESSENTRY32W));
...
Arguments:
    SnapSection - Supplies the handle of a snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lppe - Returns a PROCESSENTRY32 structure.  The caller must set the dwSize member to the size, in bytes, of the structure.
Return Value:
    Returns TRUE if the first entry of the process list has been copied to the buffer or FALSE otherwise.
    The ERROR_NO_MORE_FILES error value is returned by the GetLastError function if no processes exist or the snapshot does not contain process information.
--*/
...
/*++
Routine Description:
    Retrieves information about the first thread of any Win32 process encountered in a system snapshot.
Arguments:
    SnapSection - Supplies the handle of a snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lpte - Returns a THREADENTRY32 structure.  The caller must set the dwSize member to the size, in bytes, of the structure.
Return Value:
...
--*/
{
    ...
    if (SnapshotBase->ThreadCount == 0) {
        ...
        memcpy(lpte, (LPTHREADENTRY32) ((ULONG_PTR) SnapshotBase + (ULONG_PTR) SnapshotBase->ThreadListHead), sizeof(THREADENTRY32));
...
/*++
Routine Description:
    Retrieves information about the next thread of any Win32 process encountered in the system memory snapshot.
Arguments:
    SnapSection - Supplies the handle of a snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lpte - Reeturns a THREADENTRY32 structure.  The caller must set the dwSize member to the size, in bytes, of the structure.
Return Value:
...
Arguments:
    SnapSection - Supplies the handle of a snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lpme - Returns a buffer containing a MODULEENTRY32W structure.  The caller must set the dwSize member to the size, in bytes, of the structure.
Return Value:
...
--*/
{
    ...
    if (SnapshotBase->ModuleCount == 0) {
        ...
        memcpy(lpme, (PMODULEENTRY32W)((ULONG_PTR)SnapshotBase + (ULONG_PTR)SnapshotBase->ModuleListHead), sizeof(MODULEENTRY32W));
...
Arguments:
    SnapSection - Supplies the handle of the snapshot returned from a previous call to the CreateToolhelp32Snapshot function.
    lpme - Returns a buffer containing a MODULEENTRY32 structure.  The caller must set the dwSize member to the size, in bytes, of the structure.
Return Value:
    Returns TRUE if the first entry of the module list has been copied to the buffer or FALSE otherwise.
    The ERROR_NO_MORE_FILES error value is returned by the GetLastError function if no modules exist or the snapshot does not contain module information.
--*/
...
Arguments:
    SnapSection - Supplies the handle of a snapshot returned from a previous call to CreateToolhelp32Snapshot.
    lpme - Returns a MODULEENTRY32W structure.  The calling application must set the dwSize member to the size, in bytes, of the structure.
Return Value:
...
Arguments:
    SnapSection - Supplies the handle of a snapshot returned from a previous call to CreateToolhelp32Snapshot.
    lpme - Returns a MODULEENTRY32 structure.  The calling application must set the dwSize member to the size, in bytes, of the structure.
Return Value:
    Returns TRUE if the next entry of the module list has been copied to the buffer or FALSE otherwise.
    The ERROR_NO_MORE_FILES error value is returned by the GetLastError function if no more modules exist.
--*/
...
Arguments:
    th32ProcessID - Supplies a WIN32 process ID.  See CreateToolhelp32Snapshot for full description.
    dwFlags - Supplies switches requesting various data.  See CreateToolhelp32Snapshot for full description
Return Value:
...
--*/
{
    ...
    *RawDebugInfo = NULL;
    if((dwFlags & TH32CS_SNAPPROCESS) || (dwFlags & TH32CS_SNAPTHREAD)){
        do {
            try {
                ...
                Status = NtAllocateVirtualMemory(NtCurrentProcess(), RawProcess, 0, &stBufferSize, MEM_COMMIT, PAGE_READWRITE);
    ...
    if(dwFlags & TH32CS_SNAPMODULE)
    {
        ...
        if (NT_SUCCESS(Status)) {
            Status = RtlQueryProcessDebugInformation((HANDLE)LongToHandle(th32ProcessID), RTL_QUERY_PROCESS_MODULES, *RawModule);
        }
    }
    ...
    if (dwFlags & TH32CS_SNAPHEAPLIST) {
        ...
        if (NT_SUCCESS(Status)) {
            Status = RtlQueryProcessDebugInformation((HANDLE)LongToHandle(th32ProcessID), RTL_QUERY_PROCESS_HEAP_SUMMARY, *RawDebugInfo);
        }
    }
...
NTSTATUS ThpAllocateSnapshotSection(OUT PHANDLE SnapSection,
IN DWORD dwFlags,
...
/*++
Routine Description:
    This function calculates the size of the snapshot and allocates a file mapping object for it in the pagefile.
    Also, initialize snapshot information in the header
Arguments:
    th32ProcessID - Supplies a WIN32 process ID.  See CreateToolhelp32Snapshot for full description.
    dwFlags - Supplies switches describing requested data.  See CreateToolhelp32Snapshot for full description.
    th32ProcessID -
    RawProcess -
    RawDebugInfo -
Return Value:
...
--*/
{
    ...
    Status = NtCreateSection(SnapSection, STANDARD_RIGHTS_REQUIRED | SECTION_QUERY | SECTION_MAP_READ | SECTION_MAP_WRITE, pObja, &SectionSize, PAGE_READWRITE, SEC_COMMIT, NULL);
    if ( !NT_SUCCESS(Status) ) {
        return Status;
    }
...
NTSTATUS ThpCopyAnsiToUnicode(PWCHAR Dest, PUCHAR Src, USHORT Max)
{
...
NTSTATUS ThpProcessToSnap(IN DWORD dwFlags,
                          IN DWORD th32ProcessID,
...
/*++
Routine Description:
    This function processes the data in the raw dumps specified by dwFlage into a mapped file.
Arguments:
    dwFlags - Supplies switches describing the data requested.  See CreateToolhelp32Snapshot for full description.
    th32ProcessID - Supplies a WIN32 process ID. See CreateToolhelp32Snapshot for full description.
...
--*/
{
    ...
    if (dwFlags & TH32CS_SNAPMODULE) {
        ...
        for (mCount = 0; mCount < RawModule->Modules->NumberOfModules; mCount++)   {
            ...
            pModule->hModule = ModuleInfo->ImageBase;// base == handle
            pModule->modBaseAddr = ModuleInfo->ImageBase;// Base address of module in th32ProcessID's context
            ThpCopyAnsiToUnicode(pModule->szExePath, ModuleInfo->FullPathName, sizeof(pModule->szExePath)); // Path
            ThpCopyAnsiToUnicode(pModule->szModule, &ModuleInfo->FullPathName[ModuleInfo->OffsetToFileName], sizeof(pModule->szModule));// module name
            pModule->modBaseSize = ModuleInfo->ImageSize;// Size in bytes of module starting at modBaseAddr

            // these are meaningless on NT
            // but some apps may care... Gruntz (bugid 327009)
            // was failing because th32ModuleID was 0, so now we stick in the address of the Module descriptor

            // However it turns out that a pointer doesn't fit in a DWORD, so we stick in the value 1 instead.
    ...
    if (dwFlags & TH32CS_SNAPPROCESS) {
        ...
        BufferWriteAddr = (PCHAR)(BufferWriteAddr + cProcess * sizeof(PROCESSENTRY32W));// update the pointer to the write area
    }
