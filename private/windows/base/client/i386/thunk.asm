...
;    thunk.asm
; Abstract:
;   This module implements all Win32 thunks.
;   This includes the first level thread starter...
; Author:
...
; Routine Description:
;    This function calls to the portable thread starter after moving its arguments from registers to the stack.
; Arguments:
;    EAX - StartRoutine
;    EBX - ThreadParameter
; Return Value:
...
; VOID BaseProcessStartThunk(IN LPVOID lpProcessStartAddress, IN LPVOID lpParameter);
; Routine Description:
;    This function calls the process starter after moving its arguments from registers to the stack.
; Arguments:
;    EAX - StartRoutine
...
; VOID BaseSwitchStackThenTerminate(IN PVOID StackLimit, IN PVOID NewStack, IN DWORD ExitCode)
; Routine Description:
;     This API is called during thread termination to delete a thread's stack, switch to a stack in the thread's TEB, and then terminate.
; Arguments:
;     StackLimit (esp+4) - Supplies the address of the stack to be freed.
;     NewStack (esp+8) - Supplies an address within the terminating threads TEB that is to be used as its temporary stack while exiting.
;     ExitCode (esp+12) - Supplies the termination status that the thread is to exit with.
...
; VOID BaseAttachCompleteThunk(VOID)
; Routine Description:
;     This function is called after a successful debug attach.
;     Its purpose is to call portable code that does a breakpoint, followed by an NtContinue.
...
; ULONG BaseGetTickCount (IN LARGE_INTEGER CurrentTime, IN LARGE_INTEGER BootTime)
; Routine Description:
;    This function computes the number of milliseconds that have transpired since the system was booted.
; Arguments:
;    CurrentTime (TOS+4) - Supplies the current time in 100ns units.
;    BootTime (TOS+12) - Supplies the boot time in 100ns units.
; Return Value:
;    The number of milliseconds since system boot is returned as the function value.
...
; Routine Description:
;    This function saves the state of the current fiber and switches to the new fiber.
; Arguments:
