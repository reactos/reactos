 ...
 *   STATUS_SUCCESS - no error
*/
WINBASEAPI BOOL WINAPI SetTermsrvAppInstallMode( BOOL bState )
{
    NTSTATUS Status;
    ...
    c->bState = bState;
    Status = CsrClientCallServer((PCSR_API_MSG)&m, NULL, CSR_MAKE_API_NUMBER(BASESRV_SERVERDLL_INDEX, BasepSetTermsrvAppInstallMode), sizeof( *c ));
#endif
    ...
    if (IsTerminalServer() && IsTSAppCompatEnabled()) {
        ...
        if (gpTermsrvUpdateAllUserMenu == NULL) {
            ...
            if (dllHandle) {
                gpTermsrvUpdateAllUserMenu = (PTERMSRVUPDATEALLUSERMENU)GetProcAddress(dllHandle,"TermsrvUpdateAllUserMenu");
            }
        }
...
/* External Functions */
NTSTATUS BaseDllOpenIniFileOnDisk(PINIFILE_PARAMETERS a);
...
NTSTATUS TermsrvIniSyncLoop( HANDLE SrcHandle, PINIFILE_PARAMETERS a, PBOOLEAN pfIniUpdated);
NTSTATUS TermsrvGetSyncTime( PUNICODE_STRING pSysIniPath, PUNICODE_STRING pUserBasePath, PLARGE_INTEGER  pLastSyncTime);
