    ...
    )
{
    ...
    n = GetPrivateProfileString( TEXT("a"), TEXT("\0"), TEXT("Default"), KeyValue, 1024 * sizeof( TCHAR ), TEXT("test.ini"));
#ifdef UNICODE
...
#if 0
    if (n != 36 || strcmp( SectionValue, "Keyword11=Value11" ) || strcmp( SectionValue+18, "Keyword12=Value12" )) {
        printf( "** test.ini[ Section1 ] is incorrect (Length == %d)\n", n );
        DumpSection( "test.ini", "Section1"  );
        }

    n = GetPrivateProfileString( "Section2", "Keyword25", "Default25", KeyValue, 1024, "test.ini");
    if (n != 9 || strcmp( KeyValue, "Default25" )) {
        printf( "** test.ini[ Section2 ].Keyword25 is incorrect (Length == %d)\n", n );
        DumpSection( "test.ini", "Section2"  );
        }

    n = GetPrivateProfileString( "Section2", "Keyword24", NULL, KeyValue, 1024, "test.ini");
    if (n || strcmp( KeyValue, "" )) {
        printf( "** test.ini[ Section2 ].Keyword24 is incorrect (Length == %d)\n", n );
        DumpSection( "test.ini", "Section2"  );
        }

    n = GetPrivateProfileString( "Section2", "Keyword23", NULL, KeyValue, 1024, "test.ini");
    if (n != 7 || strcmp( KeyValue, "Value23" )) {
        printf( "** test.ini[ Section2 ].Keyword23 is incorrect (Length == %d)\n", n );
        DumpSection( "test.ini", "Section2"  );
        }

    n = GetPrivateProfileString( "Section2", "Keyword22", NULL, KeyValue, 1024, "test.ini");
    if (n != 7 || strcmp( KeyValue, "Value22" )) {
        printf( "** test.ini[ Section2 ].Keyword22 is incorrect (Length == %d)\n", n );
        DumpSection( "test.ini", "Section2"  );
        }

    n = GetPrivateProfileString( "Section2", "Keyword21", NULL, KeyValue, 1024, "test.ini");
