...
--*/
{
    ...
    RtlZeroMemory((PVOID)Context, sizeof(CONTEXT));// Initialize the Context
    ...
    if (ContextType == BaseContextTypeThread) {
        ...
        // when SwitchToFiber restores context and calls return, the contorl goes to this routine
        Context->BrRp = *((ULONGLONG *) ((PUCHAR) BaseFiberStart));
        Context->RsPFS |= 0x60;// set up sof = 96 in pfs. This will be used to set up CFM for above routine
    } else {
...
Arguments:
   StartRoutine - User Target start program counter ThreadParameter
Return Value:
   Never Returns
--*/
{
