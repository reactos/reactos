...
HGLOBAL WINAPI GlobalReAlloc(HANDLE hMem, SIZE_T dwBytes, UINT uFlags)
{
    PBASE_HANDLE_TABLE_ENTRY HandleEntry;
    HANDLE Handle;
    LPSTR p;
...
HANDLE WINAPI GlobalHandle(LPCVOID pMem)
{
    HANDLE Handle;
    ULONG Flags;
    ...
    if ((ULONG_PTR) hMem & BASE_HANDLE_MARK_BIT) {
        ...
        try {
            ...
            if (!RtlIsValidHandle(&BaseHeapHandleTable, (PRTL_HANDLE_TABLE_ENTRY) HandleEntry)) {
                ...
                // If passed address is NOT part of an image file, then display a debug message.
                // This prevents apps that call GlobalUnlock with the return value of LockResource from displaying the message.
...
SIZE_T WINAPI GlobalSize(HANDLE hMem)
{
    PBASE_HANDLE_TABLE_ENTRY HandleEntry;
    PVOID Handle;
    ULONG Flags;
...
SIZE_T WINAPI GlobalCompact(DWORD dwMinFree)
{
...
VOID WINAPI GlobalMemoryStatus(LPMEMORYSTATUS lpBuffer)
{
    ...
    // Determine the memory load.  < 100 available pages is 100 Otherwise load is ((TotalPhys - AvailPhys) * 100) / TotalPhys
    if (PerfInfo.AvailablePages < 100) {
        lpBuffer->dwMemoryLoad = 100;
    } else {
        lpBuffer->dwMemoryLoad = ((DWORD) (BASE_SYSINFO.NumberOfPhysicalPages - PerfInfo.AvailablePages) * 100) / BASE_SYSINFO.NumberOfPhysicalPages;
    }
    ...
    Memory64 = __min(PerfInfo.CommitLimit - PerfInfo.CommittedPages, QuotaLimits.PagefileLimit - VmCounters.PagefileUsage);
    Memory64 *= BASE_SYSINFO.PageSize;
...
PVOID WINAPI VirtualAllocEx(HANDLE hProcess, PVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect)
{
    NTSTATUS Status;
...
DWORD WINAPI VirtualQueryEx(HANDLE hProcess, LPCVOID lpAddress, PMEMORY_BASIC_INFORMATION lpBuffer, DWORD dwLength)
{
    NTSTATUS Status;
    ULONG ReturnLength;

    Status = NtQueryVirtualMemory(hProcess, (LPVOID) lpAddress, MemoryBasicInformation, (PMEMORY_BASIC_INFORMATION) lpBuffer, dwLength, &ReturnLength);
    if (NT_SUCCESS(Status)) {
        return(ReturnLength);
    } else {
        BaseSetLastNTError(Status);
...
/*++
Routine Description:
    This API may be used to lock the specified range of the processes address space into memory.
    This range is present whenever the application is running.
    All pages covered by the range must be commited.
    VirtialLock is in now way related to LocalLock or GlobalLock.
    It does not perform a handle translation.
    Its function is to lock memory in the "working set" of the calling process.

    Note that the specified range is used to compute the range of pages covered by the lock.
    A 2 byte lock that straddles a page boundry ends up locking both of the pages covered by the range.
    Also note that calls to VirtualLock do not nest.
Arguments:
...
/*++
Routine Description:
    This API may be used to unlock the specified range of the processes address space from memory.
    This call is used to reveres the effects of a previous call to VirtualLock.
    The range specified need not match a range passed to a previous VirtualLock call, but it must specify a locked range" for this API to be successful.
    Note that the specified range is used to compute the range of pages covered by the unlock.
    A 2 byte unlock that straddles a page boundry ends up unlocking both of the pages covered by the range.
Arguments:
...
Arguments:
    hProcess - Supplies a handle to the process in which the instruction cache is to be flushed.
    lpBaseAddress - Supplies an optional pointer to base of the region that is flushed.
    dwSize - Supplies the length of the region that is flushed if the base address is specified.
Return Value:
    TRUE - The operation was was successful.
    FALSE - The operation failed.  Extended error status is available using GetLastError.
--*/
...
WINBASEAPI UINT WINAPI GetWriteWatch(DWORD dwFlags,    PVOID lpBaseAddress, SIZE_T dwRegionSize, PVOID *addresses, ULONG_PTR *count, LPDWORD granularity)
{
    NTSTATUS Status;

    Status = NtGetWriteWatch(NtCurrentProcess(), dwFlags, lpBaseAddress, dwRegionSize, addresses, count, granularity);
    if (NT_SUCCESS(Status)) {// Note these return codes are taken straight from Win9x.
        return(0);
    } else {
        BaseSetLastNTError(Status);
        return (UINT) -1;
