...
Arguments:
    ...
    ppOutPath - Output path that accesses the specified session.
                If pIniPath is "C:" and SessionId is 6, the output would be "GLOBALROOT\Sessions\6\DosDevices\C:".
Return Value:
    TRUE - Path returned in *ppOutPath in newly allocated memory from LocalAlloc.
...
--*/
{
    ...
    // if the input path is null or the pointer is a bad pointer, return an error.
