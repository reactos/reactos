...
Arguments:
    lpLocalPortion - Supplies the address of a per-app local portion of the global critical section.
Return Value:
    TRUE - The operation was successful.
    FALSE - The operation failed.
--*/
    ...
    )
{
    ...
    if (--GlobalPortion->RecursionCount) {
        ...
        // We are really leaving, so give up ownership and decrement the lock count
