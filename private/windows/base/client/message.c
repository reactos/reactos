...
Abstract:This module contains the Win32 Message Management APIs
Author:Steve Wood (stevewo) 24-Jan-1991
...
DWORD APIENTRY FormatMessageA(DWORD dwFlags, LPCVOID lpSource, DWORD dwMessageId, DWORD dwLanguageId, LPSTR lpBuffer, DWORD nSize, va_list *lpArguments)
{
    NTSTATUS Status;
    DWORD Result;
    PWSTR UnicodeSource;
    ...
    if (UnicodeBuffer != NULL) {
        Result = BaseDllFormatMessage(TRUE, dwFlags, (LPVOID)UnicodeSource, dwMessageId, dwLanguageId, UnicodeBuffer, nSize, lpArguments);
    }
    else {
    ...
    if (Result != 0) {
        ...
        if (!NT_SUCCESS(Status)) {
            ...
            // Ajust return value, since Result contains Unicode char counts, we have to adjust it to ANSI char counts
...
DWORD APIENTRY FormatMessageW(DWORD dwFlags, LPCVOID lpSource, DWORD dwMessageId, DWORD dwLanguageId, PWSTR lpBuffer, DWORD nSize, va_list *lpArguments)
{
    return BaseDllFormatMessage(FALSE, dwFlags, (LPVOID)lpSource, dwMessageId, dwLanguageId, lpBuffer, nSize, lpArguments);
}


BOOLEAN CreateVirtualBuffer(OUT PVIRTUAL_BUFFER Buffer, IN ULONG CommitSize OPTIONAL, IN ULONG ReserveSize OPTIONAL)
{
...
BOOLEAN ExtendVirtualBuffer(IN PVIRTUAL_BUFFER Buffer, IN PVOID Address)
{
...
BOOLEAN TrimVirtualBuffer(IN PVIRTUAL_BUFFER Buffer)
{
...
BOOLEAN FreeVirtualBuffer(IN PVIRTUAL_BUFFER Buffer)
{
    return (BOOLEAN)VirtualFree(Buffer->Base, 0, MEM_RELEASE);
}


int VirtualBufferExceptionHandler(IN DWORD ExceptionCode, IN PEXCEPTION_POINTERS ExceptionInfo, IN OUT PVIRTUAL_BUFFER Buffer)
{
    PVOID BadAddress;

    // If this is an access violation touching memory within our reserved buffer, but outside of the committed portion of the buffer, then we are going to take this exception.
    if (ExceptionCode == STATUS_ACCESS_VIOLATION) {
        ...
        if (BadAddress >= Buffer->CommitLimit && BadAddress < Buffer->ReserveLimit) {
            ...
            // If there is room to commit more memory, try to do so.
            // If no room or unable to commit, then execute the exception handler.
            // Otherwise we were able to commit the additional buffer space, so update the commit limit on the caller's stack and retry the faulting instruction.
    ...
    return EXCEPTION_CONTINUE_SEARCH;// Not an exception we care about, so pass it up the chain.
}
...
DWORD APIENTRY BaseDllFormatMessage(BOOLEAN ArgumentsAreAnsi, DWORD dwFlags, LPVOID lpSource, DWORD dwMessageId, DWORD dwLanguageId, PWSTR lpBuffer, DWORD nSize, va_list *arglist)
{
    ...
    if (((dwMessageId & 0xffff0000) == 0x80070000) && (dwFlags & FORMAT_MESSAGE_FROM_SYSTEM) && !(dwFlags & FORMAT_MESSAGE_FROM_HMODULE) && !(dwFlags & FORMAT_MESSAGE_FROM_STRING))
    {
    ...
    if (CreateVirtualBuffer(&Buffer, nSize + 1, 0))
        ...
        retrySystem2:
            Status = RtlFindMessage(DllHandle, PtrToUlong(RT_MESSAGETABLE), (ULONG)dwLanguageId, dwMessageId, &MessageEntry);
            if (!NT_SUCCESS(Status)) {
                if (Status == STATUS_MESSAGE_NOT_FOUND) {
                    ...
                    if (dwFlags & FORMAT_MESSAGE_FROM_SYSTEM && DllHandle == (PVOID)BaseDllHandle) {
                        // The message isn't in kernel32.dll, conditionally load netmsg.dll to see if the message is there.
