...
HANDLE APIENTRY CreateNamedPipeA(
    LPCSTR lpName,
    DWORD dwOpenMode,
    DWORD dwPipeMode,
    DWORD nMaxInstances,
...
HANDLE APIENTRY CreateNamedPipeW(
    LPCWSTR lpName,
    DWORD dwOpenMode,
    DWORD dwPipeMode,
    DWORD nMaxInstances,
...
Parameters:
    ...
    dwOpenMode --Supplies the set of flags that define the mode which the pipe is to be opened with.
        The open mode consists of access flags (one of three values) logically ORed with a writethrough flag (one of two values) and an overlapped flag (one of two values), as described below.
        ...
        PIPE_ACCESS_INBOUND --Data goes from client to server only.
            (This is semantically equivalent to calling CreateFile with access flags of GENERIC_READ.)

        PIPE_ACCESS_OUTBOUND --Data goes from server to client only.
            (This is semantically equivalent to calling CreateFile with access flags of GENERIC_WRITE.)

        PIPE_WRITETHROUGH --The redirector is not permitted to delay the transmission of data to the named pipe buffer on the remote server.
            This disables a performance enhancement for applications that need synchronization with every write operation.

        FILE_FLAG_OVERLAPPED --Indicates that the system should initialize the file so that ReadFile, WriteFile and other
            operations that may take a significant time to process will return ERROR_IO_PENDING.
            An event will be set to the signalled state when the operation completes.
    ...
    dwPipeMode --Supplies the pipe-specific modes (as flags) of the pipe.
        This parameter is a combination of a read-mode flag, a type flag, and a wait flag.
    ...
    nMaxInstances --Gives the maximum number of instances for this pipe.
        Acceptable values are 1 to PIPE_UNLIMITED_INSTANCES-1 and PIPE_UNLIMITED_INSTANCES.
    ...
    nDefaultTimeOut -- Specifies an optional pointer to a timeout value that is to be used if a timeout value is not specified when waiting for an instance of a named pipe.
        This parameter is only meaningful when the first instance of a named pipe is created.
        If neither CreateNamedPipe or WaitNamedPipe specify a timeout 50 milliseconds will be used.

    lpSecurityAttributes --An optional parameter that, if present and supported on the target system, supplies a security descriptor for the named pipe.
        This parameter includes an inheritance flag for the handle.
        If this parameter is not present, the handle is not inherited by child processes.
...
--*/
{
    ...
    MaxInstances = (nMaxInstances == PIPE_UNLIMITED_INSTANCES)? 0xffffffff : nMaxInstances;// Convert Win32 maximum Instances to Nt maximum instances.
    ...
    if (nDefaultTimeOut) {
        ...
        Timeout.QuadPart = -10 * 1000 * 50;//  Default timeout is 50 Milliseconds
    }

    //  Check no reserved bits are set by mistake.
    if ((dwOpenMode & ~(PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED | FILE_FLAG_WRITE_THROUGH | WRITE_DAC | WRITE_OWNER | ACCESS_SYSTEM_SECURITY)) ||
    ...
    //  Translate the open mode into a sharemode to restrict the clients access and derive the appropriate local desired access.
    ...
    if (Status == STATUS_NOT_SUPPORTED || Status == STATUS_INVALID_DEVICE_REQUEST) {
        // The request must have been processed by some other device driver (other than NPFS).  Map the error to something reasonable.
...
/*++
Routine Description:
    The ConnectNamedPipe function is used by the server side of a named pipe to wait for a client to connect to the named pipe with a CreateFile request.
    The handle provided with the call to ConnectNamedPipe must have been previously returned by a successful call to CreateNamedPipe.
    The pipe must be in the disconnected, listening or connected states for ConnectNamedPipe to succeed.
    ...
    When a client connects with a CreateFile, the state will be changed from listening to connected and the ConnectNamedPipe returns TRUE.
    When the file handle is created with FILE_FLAG_OVERLAPPED on a blocking mode pipe, the lpOverlapped parameter can be specified.
    This allows the caller to continue processing while the ConnectNamedPipe API awaits a connection.
    When the pipe enters the signalled state the event is set to the signalled state.

    When nonblocking is specified ConnectNamedPipe will not block.
    On the first call the state will change from disconnected to listening.
    When a client connects with an Open the state will be changed from listening to connected.
    The ConnectNamedPipe will return FALSE (with GetLastError returning ERROR_PIPE_LISTENING) until the state is changed to the listening state.
...
Arguments:
    ...
    lpOverlapped - Supplies an overlap structure to be used with the request.
        If NULL then the API will not return until the operation completes.
        When FILE_FLAG_OVERLAPPED is specified when the handle was created, ConnectNamedPipe may return ERROR_IO_PENDING to allow the caller to continue processing while the operation completes.
        The event (or File handle if hEvent=NULL) will be set to the not signalled state before ERROR_IO_PENDING is returned. The event will be set to the signalled state upon completion of the request.
        GetOverlappedResult is used to determine the error status.
Return Value:
...
--*/
{
    ...
    if (lpOverlapped == NULL && Status == STATUS_PENDING) {
        Status = NtWaitForSingleObject(hNamedPipe, FALSE, NULL);// Operation must complete before return & Iosb destroyed
        if (NT_SUCCESS(Status)) {
            Status = Iosb.Status;
        }
    }
...
/*++
Routine Description:
    The DisconnectNamedPipe function can be used by the server side of a named pipe to force the client side to close the client side's handle.
    (Note that the client side must still call CloseFile to do this.)  The client will receive an error the next time it attempts to access the pipe.
    Disconnecting the pipe may cause data to be lost before the client reads it.
    (If the application wants to make sure that data is not lost, the serving side should call FlushFileBuffers before calling DisconnectNamedPipe.)
Arguments:
    hNamedPipe - Supplies a Handle to the server side of a named pipe.
Return Value:
...
BOOL APIENTRY GetNamedPipeHandleStateA(
    HANDLE hNamedPipe,
    LPDWORD lpState,
...
*/
{
    if ( ARGUMENT_PRESENT( lpUserName ) ) {
        ...
        return GetNamedPipeHandleStateW(hNamedPipe, lpState, lpCurInstances, lpMaxCollectionCount, lpCollectDataTimeout, NULL, 0);
    }
}


BOOL APIENTRY GetNamedPipeHandleStateW(
    HANDLE hNamedPipe,
    LPDWORD lpState,
    LPDWORD lpCurInstances,
    LPDWORD lpMaxCollectionCount,
...
/*++
Routine Description:
The GetNamedPipeHandleState function retrieves information about a given named pipe handle.
The information returned by this function can vary during the lifetime of an instance of a named pipe.
The handle must be created with the GENERIC_READ access rights.

Arguments:

    hNamedPipe - Supplies the handle of an opened named pipe.

    lpState - An optional parameter that if non-null, points to a DWORD which will be set with flags indicating the current state of the handle.
        ...
        PIPE_READMODE_MESSAGE
            Read the pipe as a message stream. If this flag is not set, the pipe is read as a byte stream.
    ...
    lpMaxCollectionCount - If non-null, this points to a DWORD which will be set to the maximum number of bytes that will be collected on the clients machine before transmission to the server.
        This parameter must be NULL on a handle to the server end of a named pipe or when client and server applications are on the same machine.

    lpCollectDataTimeout - If non-null, this points to a DWORD which will be set to the maximum time (in milliseconds) that can pass before a remote named pipe transfers information over the network.
        This parameter must be NULL if the handle is for the server end of a named pipe or when client and server applications are on the same machine.

    lpUserName - An optional parameter on the server end of a named pipe.
        Points to an area which will be filled-in with the null-terminated string containing the name of the username of the client application.
        This parameter is invalid if not NULL on a handle to a client end of a named pipe.
...
--*/
{
    ...
    if (ARGUMENT_PRESENT(lpMaxCollectionCount) || ARGUMENT_PRESENT(lpCollectDataTimeout)) {
        ...
        if (ARGUMENT_PRESENT(lpCollectDataTimeout)) {
            ...
            TimeWorkspace = RtlExtendedLargeIntegerDivide(LiTemporary, 10000, NULL); // Not interested in any remainder
...
Arguments:
    hNamedPipe - Supplies the handle of an opened named pipe.
    lpUserName - Points to an area which will be filled-in with the null-terminated string containing the name of the username of the client application.
        This parameter is invalid if not NULL on a handle to a client end of a named pipe.
    nMaxUserNameSize - Size in characters of the memory allocated at lpUserName.
Return Value:
    TRUE -- The operation was successful.
...
--*/
{
    ...
    if (!NT_SUCCESS(Status)) {
        (RevertToSelfp) ();//  We were not impersonating anyone
    } else {
...
/*++
Routine Description:
    The SetNamedPipeHandleState function is used to set the read mode and the blocking mode of a named pipe.
    On the client end of a remote named pipe this function can also control local buffering.
    The handle must be created with the GENERIC_WRITE access rights.
Arguments:
    hNamedPipe - Supplies a handle to a named pipe.
    lpMode - If non-null, this points to a DWORD which supplies the new mode.
        The mode is a combination of a read-mode flag and a wait flag.
    ...
    lpMaxCollectionCount - If non-null, this points to a DWORD which supplies the maximum number of bytes that will be collected on the client machine before transmission to the server.
        This parameter must be NULL on a handle to the server end of a named pipe or when client and server applications are on the same machine.
        This parameter is ignored if the client specified write through when the handle was created.

    lpCollectDataTimeout - If non-null, this points to a DWORD which supplies the maximum time (in milliseconds) that can pass before a remote named pipe transfers information over the network.
        This parameter must be NULL if the handle is for the server end of a named pipe or when client and server applications are on the same machine.
        This parameter is ignored if the client specified write through when the handle was created.
...
--*/
{
    ...
    if (ARGUMENT_PRESENT(lpMaxCollectionCount) || ARGUMENT_PRESENT(lpCollectDataTimeout)){
        ...
        if ((lpMaxCollectionCount == NULL) || (lpCollectDataTimeout == NULL)){
            // User is setting only one of the two parameters so read the other value.
            // There is a small window where another thread using the same handle could set the other value.
...
/*++
Routine Description:
    The GetNamedPipeInfo function retrieves information about a named pipe.
    The information returned by this API is preserved the lifetime of an instance of a named pipe.
    The handle must be created with the GENERIC_READ access rights.
Arguments:
    hNamedPipe - Supplies a handle to a named pipe.
    ...
    lpMaxInstances - An optional parameter that if non-null, points to a DWORD which will be set with the maximum number of pipe instances that can be created.
        Besides various numeric values, a special value may be returned for this.

        PIPE_UNLIMITED_INSTANCES
            Unlimited instances of the pipe can be created. This is an indicator that the maximum is requested;
            the value of the equate may be higher or lower than the actual implementation's limit, which may vary over time.
...
/*++
Routine Description:
    The PeekNamedPipe function copies a named pipe's data into a buffer for preview without removing it.
    The results of a PeekNamedPipe are similar to a ReadFile on the pipe except more information is returned, the function never blocks and if the pipe handle is reading in message mode, a partial message can be returned.
    ...
    However, there is no reason for calling it this way.
...
--*/
{
    ...
    try {
        ...
        if (Status == STATUS_PENDING) {
            Status = NtWaitForSingleObject(hNamedPipe, FALSE, NULL);// Operation must complete before return & IoStatusBlock destroyed
            if (NT_SUCCESS(Status)) {
                Status = Iosb.Status;
            }
        }
        ...
        //  Peek is complete, package up data for caller ensuring that the PeekBuffer is deleted even if an invalid pointer was given.
        if (NT_SUCCESS(Status)) {
            try {
                ...
                if (ARGUMENT_PRESENT(lpBytesLeftThisMessage)) {
                    *lpBytesLeftThisMessage = PeekBuffer->MessageLength - (ULONG) (Iosb.Information - FIELD_OFFSET(FILE_PIPE_PEEK_BUFFER, Data[0]));
                }

                if (ARGUMENT_PRESENT(lpBuffer)) {
                    RtlCopyMemory(lpBuffer, PeekBuffer->Data, Iosb.Information - FIELD_OFFSET(FILE_PIPE_PEEK_BUFFER, Data[0]));
                }
...
BOOL APIENTRY TransactNamedPipe(
    HANDLE hNamedPipe,
    LPVOID lpInBuffer,
    DWORD nInBufferSize,
    LPVOID lpOutBuffer,
...
Routine Description:
    ...
    This function fails if the named pipe contains any unread data or if the named pipe is not in message mode.
    A named pipe's blocking state has no effect on the TransactNamedPipe function.
    This API does not complete until data is written into the InBuffer buffer.
    The lpOverlapped parameter is available to allow an application to continue processing while the operation takes place.
Arguments:
    hNamedPipe - Supplies a handle to a named pipe.
    ...
    lpOverlapped - An optional parameter that supplies an overlap structure to be used with the request.
        If NULL or the handle was created without FILE_FLAG_OVERLAPPED then the TransactNamedPipe will not return until the operation completes.

        When lpOverlapped is supplied and FILE_FLAG_OVERLAPPED was specified when the handle was created, TransactNamedPipeFile may return ERROR_IO_PENDING to allow the caller to continue processing while the operation completes.
        The event (or File handle if hEvent == NULL) will be set to the not signalled state before ERROR_IO_PENDING is returned.
        The event will be set to the signalled state upon completion of the request.
        GetOverlappedResult is used to determine the result when ERROR_IO_PENDING is returned.
Return Value:
    TRUE -- The operation was successful.
    FALSE -- The operation failed. Extended error status is available using GetLastError.
--*/
...
/*++
Routine Description:
    CallNamedPipe is equivalent to a series of calls to CreateFile, perhaps WaitNamedPipe (if CreateFile can't open the pipe immediately),
    SetNamedPipeHandleState, TransactNamedPipe, and CloseFile. Refer to the documentation for those APIs for more information.
Arguments:
...
--*/
{
    ...
    while (1) {
        ...
        if (FirstChance == FALSE) {
            return FALSE;//  Already called WaitNamedPipe once so give up.
        }
    ...
    try {
        ...
        Result = SetNamedPipeHandleState(Pipe, &ReadMode, NULL, NULL);//  Default open is readmode byte stream- change to message mode.
        if (Result == TRUE) {
...
Arguments:
    lpNamedPipeName - Supplies the name of the named pipe.
    nTimeOut - Gives a value (in milliseconds) that is the amount of time this function should wait for the pipe to become available.
        (Note that the function may take longer than that to execute, due to various factors.)
...
--*/
{
    ...
    //  Open a handle either to the redirector or the NPFS depending on the start of the pipe name.
    //  Split lpNamedPipeName into two halves as follows:
    ...
    case RtlPathTypeUncAbsolute:
        ...
        for (Pwc = &PipeName.Buffer[2]; *Pwc != 0; Pwc++) {
            if (*Pwc == L'\\') {
                break;//  Found backslash after servername
            }
        }
        ...
        RtlInitUnicodeString(&PipeName, (PWCH) Pwc + 1);// Set up pipe name, skip leading backslashes.
        break;
    default:
        BaseSetLastNTError(STATUS_OBJECT_PATH_SYNTAX_BAD);
        RtlFreeUnicodeString(&OriginalPipeName);
