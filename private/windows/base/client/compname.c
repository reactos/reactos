...
#define NON_VOLATILE_COMPUTERNAME_NODE     L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\ComputerName\\ComputerName"
#define VOLATILE_COMPUTERNAME_NODE         L"\\Registry\\Machine\\System\\CurrentControlSet\\Control\\ComputerName\\ActiveComputerName"
...
// Allow the cluster guys to override the returned names with their own virtual names
...
#define VALUE_BUFFER_SIZE (sizeof(KEY_VALUE_FULL_INFORMATION) + \
    ...
    if (NT_SUCCESS(NtStatus)) {
        ...
        if (NT_SUCCESS(NtStatus)) {
            ...
            // First see if it's null terminated.  If it is, pretend like it's not.
            pTerminator = (PWCHAR)((PBYTE) pKeyValueInformation + pKeyValueInformation->DataOffset + pKeyValueInformation->DataLength);
            pTerminator--;
            if (*pTerminator == L'\0') {
...
--*/
{
    ...
    // Find out the required size of the ANSI buffer and validate it against the passed in buffer size
