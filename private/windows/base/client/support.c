...
Routine Description:
    ...
    It returns the address of the resulting structure (or NULL if SecurityAttributes was not specified).
Arguments:
...
PLARGE_INTEGER BaseFormatTimeOut(OUT PLARGE_INTEGER TimeOut, IN DWORD Milliseconds)
/*++
Routine Description:
...
Arguments:
    ...
    MaximumStackSize - Supplies the maximum size for the new threads stack.
        If this parameter is not specified, then the reserve size of the current images stack descriptor is used.
...
--*/
{
    ...
    if (!StackSize) {
        ...
        // Now Compute how much additional stack space is to be reserved.
        // This is done by...  If the StackSize is <= Reserved size in the image, then reserve whatever the image specifies.
        // Otherwise, round up to 1Mb.
        if (StackSize >= MaximumStackSize) {
            MaximumStackSize = ROUND_UP(StackSize, (1024 * 1024));
        }
    }

    // Align the stack size to a page boundry and the reserved size to an allocation granularity boundry.
...
/*++
Routine Description:
    This function is called to start a Win32 thread.
    Its purpose is to call the thread, and if the thread returns, to terminate the thread and delete it's stack.
Arguments:
...
--*/
{
    try {// test for fiber start or new thread
        // WARNING WARNING DO NOT CHANGE INIT OF NtTib.Version. There is external code depending on this initialization !
...
{
    try {
        NtSetInformationThread(NtCurrentThread(), ThreadQuerySetWin32StartAddress, &lpStartAddress, sizeof(lpStartAddress));
...
--*/
{
    ...
    // x86 Borland C++ 4.1 (and perhaps other versions) Rudely assumes that it can use the top of the stack.
    // Even tho this is completly bogus, leave some space on the top of the stack, to avoid problems.
    ...
    if (MaximumStackSize < 256 * 1024) {
        // Enforce a minimum stack size of 256k since the CPU emulator grabs several pages of the x86 stack for itself
...
BOOL BasePushProcessParameters(HANDLE Process,
                               PPEB Peb,
...
Routine Description:
    ...
    The parameter record is then written into the address space of the specified process.
Arguments:
    ...
    dwSubsystem - if non-zero, then value will be stored in child process PEB.
        Only non-zero for separate VDM applications, where the child process has NTVDM.EXE subsystem type,
        not the 16-bit application type, which is what we want.
Return Value:
...
LPWSTR BaseComputeProcessDllPath(IN LPCWSTR ApplicationName, IN LPVOID Environment)
/*++
Routine Description:
    This function computes a process DLL path.
Arguments:
    ApplicationName - An optional argument that specifies the name of the application.
                      If this parameter is not specified, then the current application is used.
...
--*/
{
    ...
    Status = RtlQueryEnvironmentVariable_U(Environment, &BasePathVariableName, &BaseDefaultPathAppend);
    if (NT_SUCCESS(Status)) {
        ...
        if (DynamicAppendBuffer) {
            ...
            Status = RtlQueryEnvironmentVariable_U(Environment, &BasePathVariableName, &PathEnv);
            if (NT_SUCCESS(Status)) {
...
retry:
    ...
    try {
        ...
        if (ApplicationPathLength == 0 && ARGUMENT_PRESENT(ApplicationName) && RetryOccured == FALSE) {
            // We were called with an ApplicationName, BUT it did not contain a path.
            // It was malformed. Retry, but this time do it with application name == NULL to force using the processes .exe dir
            RetryOccured = TRUE;
    ...
    if (!DynamicAppendBuffer) {
        RtlMoveMemory(pw, BaseDefaultPath.Buffer, DefaultPathLength);
    } else {
        RtlMoveMemory(pw, BaseDefaultPath.Buffer, BaseDefaultPath.Length);
        RtlMoveMemory(&pw[BaseDefaultPath.Length >> 1], DynamicAppendBuffer, PathEnv.Length);
        RtlFreeHeap(RtlProcessHeap(), 0, DynamicAppendBuffer);
    }
...
PUNICODE_STRING Basep8BitStringToStaticUnicodeString(IN LPCSTR lpSourceString)
/*++
Routine Description:
...
Return Value:
    Pointer to the Teb static string if conversion was successful, NULL otherwise.
    If a failure occurred, the last error is set.
--*/
{
    ...
    StaticUnicode = &NtCurrentTeb()->StaticUnicodeString;//  Get pointer to static per-thread string
    ...
    if (!NT_SUCCESS(Status)) {//  If we couldn't convert the string
...
Arguments:
    ...
    lpSourceString - string in OEM or ANSI
Return Value:
    TRUE if string is correctly stored, FALSE if an error occurred.
    In the error case, the last error is correctly set.
--*/
{
    ANSI_STRING AnsiString;
    NTSTATUS Status;
    ...
    if (!NT_SUCCESS(Status)) {//  If we couldn't do this, fail
        if (Status == STATUS_BUFFER_OVERFLOW) {
            SetLastError(ERROR_FILENAME_EXCED_RANGE);
        } else {
            BaseSetLastNTError(Status);
...
ULONG BasepAnsiStringToUnicodeSize(PANSI_STRING AnsiString)
/*++
Routine Description:
...
ULONG BasepOemStringToUnicodeSize(PANSI_STRING OemString)
/*++
Routine Description:
...
ULONG BasepUnicodeStringToOemSize(PUNICODE_STRING UnicodeString)
/*++
Routine Description:
...
ULONG BasepUnicodeStringToAnsiSize(PUNICODE_STRING UnicodeString)
/*++
Routine Description:
...
// Mike Swift 5/19/95.
{
    ...
    *ReturnedState = NULL;
    ...
    Status = NtOpenProcessToken(NtCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &State->Token);
    if (!NT_SUCCESS(Status)) {
    ...
    // STATUS_NOT_ALL_ASSIGNED means that the privilege isn't in the token, so we can't proceed.
