...
//  SetProcessDefaultCredentials accepts handle to the process and various AcquireCredentialsHandle.
//  It calls CreateRemoteThread to invoke a thread in the remote process which executes SetpProcessDefaultCredentialsThreadProc
...
//  *  KERNEL32.DLL will always be mapped in exactly the same address locations in all processes.
//  *  Writing to the address location based on one process will write to same place in the other process
//  *  Invoking the function at address location based on one process will invoke the same function in the other process.
...
DWORD PingProcess(LPVOID  Param) {
    return(ERROR_SUCCESS);
}


DWORD SetpProcessDefaultCredentialsThreadProc(LPVOID  Param)
{
    ...
    if( NULL != (hMod = LoadLibraryA("Secur32.dll")) ) {
        ...
        // It is assumed that SetProcessDefaultCredentials has already setup all the memory correctly.
        //  we just call the API.
        SecStatus = pfn(acqCredInfo.wszPrincipalName, acqCredInfo.wszPackageName, acqCredInfo.fCredentials,
            acqCredInfo.pLogonID, &acqCredInfo.AuthIdentity, acqCredInfo.pGetKeyFunc, acqCredInfo.KeyFuncArg, &hCredentials, &Expiry);
        }
    }
...
// we can't free the module because this will clean up the credential state.
// But this is only in the spawned app, so when the app dies it will get cleaned up
//    if(NULL != hMod)
//     FreeLibrary(hMod);
...
BOOL WINAPI SetProcessDefaultCredentials(
    HANDLE  hProcess,
    LPWSTR  lpPrincipal,
    LPWSTR  lpPackage,
    ULONG   fCredentials,
    ...
    )
{
    ...
    if( pAuthData->UserLength        > MAX_PATH    ||    pAuthData->PasswordLength   > MAX_PATH    || pAuthData->DomainLength     > MAX_PATH  ) {
        SetLastError(ERROR_BUFFER_OVERFLOW);
        return(FALSE);
    }

    // we do the following:
    //  *   WriteProcess Memory:  Write the arguments to the exact same address locations in the remote process as  in current process.
    //  *   Call CreateRemoteThread and pass the same address location of SetpProcessDefaultCredentialsThreadProc as in current process.
    ...
    hThread = CreateRemoteThread(hProcess, NULL, 0, PingProcess, NULL, 0, NULL);
    if(hThread) {
    ...
    if(!WriteProcessMemory( hProcess, &acqCredInfo, &acqCredInfo, sizeof(acqCredInfo), &BytesWritten))
        return(FALSE);

    // so, after lot of painful work, we should have written all the parameters such that calling the ThreadProc will do the right thing (TM).
    hThread = CreateRemoteThread( hProcess, NULL, 0, SetpProcessDefaultCredentialsThreadProc, NULL, 0, NULL);
    if(hThread)
    {
