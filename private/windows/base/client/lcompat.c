...
int WINAPI _lopen(LPCSTR lpPathName, int iReadWrite)
{
    ...
    ShareMode = BasepOfShareToWin32Share((DWORD)iReadWrite);// Compute ShareMode
...
HFILE WINAPI _lcreat(LPCSTR lpPathName, int  iAttribute)
{
    ...
    DesiredAccess = (GENERIC_READ | GENERIC_WRITE);// Compute Desired Access
    ShareMode = FILE_SHARE_READ | FILE_SHARE_WRITE;;
    CreateDisposition = CREATE_ALWAYS;// Compute Create Disposition
...
int WINAPI MulDiv (int nNumber, int nNumerator, int nDenominator)
{
    ...
    Negate = nNumber ^ nNumerator ^ nDenominator;// Compute the size of the result.
    ...
    // Compute the 64-bit product of the multiplier and multiplicand values and round.
    u.Product.QuadPart = Int32x32To64(nNumber, nNumerator) + ((ULONG)nDenominator / 2);

    // If there are any high order product bits, then the quotient has overflowed.
    if ((ULONG)nDenominator > u.Remainder) {
        // Divide the 64-bit product by the 32-bit divisor forming a 32-bit quotient and a 32-bit remainder.
...
int APIENTRY lstrcmpA(LPCSTR lpString1, LPCSTR lpString2)
{
    ...
    if (retval == 0)
    {
        // The caller is not expecting failure.  Try the system default locale id.
    ...
    if (retval == 0)
    {
        ...
        {
            // The caller is not expecting failure.
            // We've never had a failure indicator before.
            // We'll do a best guess by calling the C runtimes to do a non-locale sensitive compare.
...
int APIENTRY lstrcmpiA(LPCSTR lpString1, LPCSTR lpString2)
{
    ...
    {
        // The caller is not expecting failure.  Try the system default locale id.
        retval = CompareStringA( GetSystemDefaultLCID(), LOCALE_USE_CP_ACP | NORM_IGNORECASE, lpString1, -1, lpString2, -1 );
    }
    if (retval == 0)
    {
        if (lpString1 && lpString2)
...
LPSTR APIENTRY lstrcpyA(LPSTR lpString1, LPCSTR lpString2)
{
    __try {
...
LPSTR APIENTRY lstrcpynA(LPSTR lpString1, LPCSTR lpString2, int iMaxLength)
{
    LPSTR src,dst;
...
LPSTR APIENTRY lstrcatA(LPSTR lpString1, LPCSTR lpString2)
{
    __try {
...
LPWSTR APIENTRY lstrcpyW(LPWSTR lpString1, LPCWSTR lpString2)
{
    __try {
...
LPWSTR APIENTRY lstrcpynW(LPWSTR lpString1, LPCWSTR lpString2, int iMaxLength)
{
    LPWSTR src,dst;
...
LPWSTR APIENTRY lstrcatW(LPWSTR lpString1, LPCWSTR lpString2)
{
    __try {
