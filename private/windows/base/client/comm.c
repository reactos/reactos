...
static NTSTATUS GetFriendlyMatchComm(
    IN PWSTR ValueName,
    IN ULONG ValueType,
    IN PVOID ValueData,
    IN ULONG ValueLength,
...
VOID GetFriendlyUi(LPCWSTR FriendlyName, PUNICODE_STRING DllToInvoke)
{
        ...
        try {
            ...
            if (getSize && getList && locateDevNode && openDevKey) {
                ...
                if (getSize(&sizeOfBuffer, L"MODEM", CM_GETIDLIST_FILTER_SERVICE) == CR_SUCCESS) {
                    ...
                    bufferForList = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG( TMP_TAG ), (sizeOfBuffer*sizeof(WCHAR)) +(sizeof(WCHAR)*2));
                    if (bufferForList) {
                        PWCHAR currentId;

                        try {
                            if (getList(L"modem", bufferForList, sizeOfBuffer, CM_GETIDLIST_FILTER_SERVICE) == CR_SUCCESS) {
                                for (currentId = bufferForList; *currentId; currentId += wcslen(currentId)+1) {
                                    ...
                                    if (locateDevNode(&devInst, currentId, CM_LOCATE_DEVINST_NORMAL) == CR_SUCCESS) {
                                        ...
                                        if (openDevKey(devInst, KEY_ALL_ACCESS, 0, RegDisposition_OpenAlways, &handleToDev, CM_REGISTRY_SOFTWARE) == CR_SUCCESS) {
                                            ...
                                            if (!NT_SUCCESS(
                                                     RtlQueryRegistryValues(RTL_REGISTRY_HANDLE, handleToDev, &paramTable[0], &localMatch, NULL)
                                                     )) {
                                            ...
                                            statusOfQuery = RtlQueryRegistryValues(RTL_REGISTRY_HANDLE, handleToDev, &paramTable[0], DllToInvoke, NULL);
                                            paramTable[0].QueryRoutine = GetFriendlyMatchComm;
    ...
    if (!DllToInvoke->Buffer) {
        ...
        // See if the value is a valid comm port name.  If it is, default return serialui.dll
        ...
        if (localMatch.FoundIt) {
            ...
            DllToInvoke->Buffer = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG( TMP_TAG ), (ansiString.Length+2)*sizeof(WCHAR));
            if (!DllToInvoke->Buffer) {
...
/*++
Routine Description:
    This function establishes the timeout characteristics for all read and write operations on the handle specified by hFile.
Arguments:
    hFile - Specifies the communication device to receive the settings.
            The CreateFile function returns this value.
...
Routine Description:
    ...
    for immediate transmission, by placing it at the head of the transmit queue.
Arguments:
...
--*/
{
    ...
    if (ARGUMENT_PRESENT(lpOverlapped)) {
        ...
        if (!(Event = CreateEvent(NULL, TRUE, FALSE, NULL))) {
            ...
            Status = NtDeviceIoControlFile(hFile, Event, NULL, NULL, &Iosb, IOCTL_SERIAL_WAIT_ON_MASK, NULL, 0, lpEvtMask, sizeof(ULONG));
            if ( Status == STATUS_PENDING) {
                // Operation must complete before return & IoStatusBlock destroyed
                Status = NtWaitForSingleObject( Event, FALSE, NULL );
                if ( NT_SUCCESS(Status)) {
