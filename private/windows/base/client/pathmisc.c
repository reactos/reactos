...

{
    ...
    if ( PathType != RtlPathTypeRelative ) {
        if (RtlDoesFileExists_U(lpFileName) ) {
            ...
            if ( ARGUMENT_PRESENT(lpExtension) ) {
                ...
                AllocatedPathString.Buffer = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG( TMP_TAG ), AllocatedPathString.MaximumLength);
    ...
    ComputedFileName = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG( TMP_TAG ), ComputedLength);
    if ( !ComputedFileName ) {
        if ( AllocatedPath ) {
            RtlFreeHeap(RtlProcessHeap(), 0, AllocatedPath);
            }
...

{
    ...
    if ( UnicodeString.Length > (USHORT)(UnicodeString.MaximumLength-sizeof(UNICODE_NULL)) ) {
        ...
        UnicodeString.Buffer = RtlAllocateHeap ( RtlProcessHeap(), MAKE_TAG( TMP_TAG ), UnicodeString.Length+ sizeof(UNICODE_NULL));
        if ( !UnicodeString.Buffer ) {
            BaseSetLastNTError(STATUS_NO_MEMORY);
            return 0;
            }
...

{
    ...
    try {
        if ( ARGUMENT_PRESENT(lpVolumeNameBuffer) ) {
            ...
            UnicodeVolumeName.Buffer = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG( TMP_TAG ), UnicodeVolumeName.MaximumLength);
        ...
        if ( ARGUMENT_PRESENT(lpFileSystemNameBuffer) ) {
            ...
            UnicodeFileSystemName.Buffer = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG( TMP_TAG ), UnicodeFileSystemName.MaximumLength);
...

{
    ...
    TranslationStatus = RtlDosPathNameToNtPathName_U(ARGUMENT_PRESENT(lpRootPathName) ? lpRootPathName : DefaultPath, &FileName, NULL, NULL);
    if ( !TranslationStatus ) {
        SetLastError(ERROR_PATH_NOT_FOUND);
        return FALSE;
        }
...
bail_gzsd:
    ...
    InitializeObjectAttributes( &ObjectAttributes, &KeyName, OBJ_CASE_INSENSITIVE, CurrentUserKey, NULL);
    Status = NtOpenKey( &DirKey, KEY_READ | KEY_NOTIFY | KEY_WRITE, &ObjectAttributes);
    ...
    Status = NtQueryValueKey( DirKey, &KeyValueName, KeyValuePartialInformation, ValueInfo, sizeof(ValueInfoBuffer), &DataLength);
    NtClose(DirKey);
    if ( !NT_SUCCESS(Status) ) {
        goto bail_gzsd;
        }
