...
BOOL BuildSubSysCommandLine(LPWSTR SubSysName, LPCWSTR lpApplicationName, LPCWSTR lpCommandLine, PUNICODE_STRING SubSysCommandLine);
PVOID BasepIsRealtimeAllowed(BOOLEAN LeaveEnabled);


#ifdef WX86
PWCHAR BasepWx86KnownExe(LPCWSTR ExeName)
/*++
Routine Description:
    Checks for Wx86 Known Exes which wx86 applications must run compatible binaries. We currently have only one Known exe, regedit.exe.
Arguments:
     ExeName - name to check for match.
Return Value:
     if the name needs to be swapped, a buffer is allocated off of the process heap filled with new name and returned.
...
BOOL BasepIsImageVersionOk(IN ULONG ImageMajorVersion, IN ULONG ImageMinorVersion)
{
    ...
    if ( ( ImageMajorVersion > USER_SHARED_DATA->NtMajorVersion ) || ( ImageMajorVersion == USER_SHARED_DATA->NtMajorVersion && ImageMinorVersion > USER_SHARED_DATA->NtMinorVersion)) {
        return FALSE;
    }
...
NTSTATUS BasepIsProcessAllowed(LPCWSTR lpApplicationName)
/*++
...
--*/
{
    ...
    // - Certification is OFF, and dwCertifyErrorCode indicates whether this is because no certification is needed, or due to an initialization error.
...
InitDone:
    if ( fInitialized ) {
       ...
       ULONG Reason;
        ...
        // Two phase notification scheme.
        // In the first phase we get every dll to vote whether the process should be created.
        // In the second phase we let them know if the process is going to get created or not.
    ...
    RtlEnterCriticalSection(&gcsAppCert);// Start initialization
    ...
    if (IsEmbeddedNT()) {
        ...
        // When we are called from CreateProcessA (e.g. the debuggers), the application command line is stored in this area.
        // Therefore, we need to save / restore it around the call to LoadLibrary
        ...
        if (hDll == NULL) {// The library was not loaded, return.
            Status = STATUS_UNSUCCESSFUL;
            goto Cleanup;
        }
        ...
        if (fEmbeddedCertFunc == NULL) {// Unable to retrieve routine address, fail.
            Status = STATUS_UNSUCCESSFUL;
        }
        ...
        if ( !NT_SUCCESS(NtOpenKey(&hKey, KEY_QUERY_VALUE, &obja))) {
            goto Cleanup;
        } else {
            NtClose(hKey);
        }
    ...
    if (!NT_SUCCESS(Status)) {
       if (Status == STATUS_OBJECT_NAME_NOT_FOUND) {
          Status = STATUS_SUCCESS;// If the registry Key is missing AppCert is turned off
       }
    }
...
--*/
{
    ...
    if (ARGUMENT_PRESENT (lpCommandLine)) {
        if ( (strlen( lpCommandLine ) + 1) * sizeof( WCHAR ) < NtCurrentTeb()->StaticUnicodeString.MaximumLength ) {
    ...
    try {
        try {
            ...
            if (ARGUMENT_PRESENT(lpStartupInfo->lpReserved)) {
                // Win95 does not touch reserved, and Intergraph Voxtel passes garbage for this. Handle this by probing lpReserved, and if the pointer is bad, ignore it
                try {
...
{
    // Soft link in the USER call back for the routine needed for WinExec() synchronization.
    // The only reason this is a soft link is so we can run char mode without gui.
...
void StuffStdHandle(HANDLE ProcessHandle, HANDLE StdHandle, PHANDLE TargetHandleAddress)
{
    ...
    Status = NtDuplicateObject( NtCurrentProcess(), StdHandle, ProcessHandle, &TargetStdHandle, DUPLICATE_SAME_ACCESS | DUPLICATE_SAME_ATTRIBUTES, 0, 0);
    if (!NT_SUCCESS( Status )) {
        return;
    }

    Status = NtWriteVirtualMemory( ProcessHandle, TargetHandleAddress, &TargetStdHandle, sizeof( TargetStdHandle ), &NumberOfBytesWritten);
}
...
/*++
Routine Description:
    A process and thread object are created and a handle opened to each object using CreateProcess.
    Note that WinExec and LoadModule are still supported, but are implemented as a call to CreateProcess.
Arguments:
    lpApplicationName - Supplies an optional pointer to a null terminated character string that contains the name of the image file to execute.
                        This is a fully qualified DOS path name.
                        If not specified, then the image file name is the first whitespace delimited token on the command line.

    lpCommandLine - Supplies a null terminated character string that contains the command line for the application to be executed.
        The entire command line is made available to the new process using GetCommandLine.
        If the lpApplicationName parameter was not specified, then the first token of the command line specifies file name of the application (note that this token
        begins at the beginning of the command line and ends at the first "white space" character).
        If the file name does not contain an extension (the presence of a "."), then .EXE is assumed.
        If the file name does not contain a directory path, Windows will search for the executable file in:
          - The current directory
          - The windows directory
          - The windows system directory
          - The directories listed in the path environment variable

        This parameter is optional onlu if the lpApplicationName parameter is specified.
        In this case the command line the application receives will be the application name.

    lpProcessAttributes - An optional parameter that may be used to specify the attributes of the new process.
        If the parameter is not specified, then the process is created without a security descriptor, and the resulting handle is not inherited on process creation:

        SECURITY_ATTRIBUTES Structure:
        DWORD nLength - Specifies the length of this structure.  Must be set to sizeof( SECURITY_ATTRUBUTES ).
        LPVOID lpSecurityDescriptor - Points to a security descriptor for the object (must be NULL for Win32, used on NT/Win32).
                                      The security descriptor controls the sharing of an object.
        BOOL bInheritHandle - Supplies a flag that indicates whether or not the returned handle is to be inherited by a new process during process creation.
                              A value of TRUE indicates that the new process will inherit the handle.
    lpThreadAttributes - An optional parameter that may be used to specify the attributes of the new thread.
                         If the parameter is not specified, then the thread is created without a security descriptor, and the resulting handle is not inherited on process creation.

    dwCreationFlags - Supplies additional flags that control the creation of the process.

        dwCreationFlags Flags:

        DEBUG_PROCESS - If this flag bit is set, then the creating process is treated as a debugger, and the process being created is created as a debugee.
            All debug events occuring in the debugee are reported to the debugger.
            If this bit is clear, but the calling process is a debugee, then the process becomes a debugee of the calling processes debugger.
            If this bit is clear and the calling processes is not a debugee then no debug related actions occur.
        DEBUG_ONLY_THIS_PROCESS - If this flag is set, then the DEBUG_PROCESS flag bit must also be set.
            The calling process is is treated as a debugger, and the new process is created as its debuggee.
            If the new process creates additional processes, no debug related activities (with respect to the debugger) occur.
        CREATE_SUSPENDED - The process is created, but the initial thread of the process remains suspended.
            The creator can resume this thread using ResumeThread.
            Until this is done, code in the process will not execute.
    ...
    bInheritHandles - Supplies a flag that specifies whether or not the new process is to inherit handles to objects visible to the calling process.
        A value of TRUE causes handles to be inherited by the new process.
        If TRUE was specified, then for each handle visible to the calling process, if the handle was created with the inherit handle option, the handle is inherited to the new process.
        The handle has the same granted access in the new process as it has in the calling process, and the value of the handle is the same.
    lpEnvironment - An optional parameter, that if specified, supplies a pointer to an environment block.
        If the parameter is not specified, the environment block of the current process is used.
        This environment block is made available to the new process using GetEnvironmentStrings.
    lpCurrentDirectory - An optional parameter, that if specified, supplies a string representing the current drive and directory for the new process.
        The string must be a fully qualified pathname that includes a drive letter.
        If the parameter is not specified, then the new process is created with the same current drive and directory as the calling process.
        This option is provided primarily for shells that want to start an application and specify its initial drive and working directory.
    lpStartupInfo - Supplies information that specified how the applications window is to be shown.
        This structure is described in the Win32 User Interface API Book.
    ...
    PROCESS_INFORMATION Structure:
        HANDLE hProcess - Returns a handle to the newly created process.
            Through the handle, all operations on process objects are allowed.
        HANDLE hThread - Returns a handle to the newly created thread.
            Through the handle, all operations on thread objects are allowed.
        DWORD dwProcessId - Returns a global process id that may be used to identify a process.
                            The value is valid from the time the process is created until the time the process is terminated.
        DWORD dwThreadId - Returns a global thread id that may be used to identify a thread.
...
--*/
{
    ...
    // CREATE_WITH_USERPROFILE is the new Create Flag that is used only by CreateProcessWithLogonW.
    // If this flags ends up getting passed to CreateProcess, we must reject it.
    ...
    if ((dwCreationFlags & CREATE_SEPARATE_WOW_VDM) == 0) {
        // If the creator is running inside a job object, always set SEPERATE_WOW_VDM so the VDM is part of the job.
        JOBOBJECT_BASIC_UI_RESTRICTIONS UiRestrictions;
        Status = NtQueryInformationJobObject(NULL, JobObjectBasicUIRestrictions, &UiRestrictions, sizeof(UiRestrictions), NULL);
        if (Status != STATUS_ACCESS_DENIED) {
            // Anything other than STATUS_ACCESS_DENIED indicates the current process is inside a job.
    ...
    if (lpEnvironment && !(dwCreationFlags & CREATE_UNICODE_ENVIRONMENT)) {
        ...
        Unicode.Buffer = NULL;
        Status = NtAllocateVirtualMemory(NtCurrentProcess(), &Unicode.Buffer, 0, &MemoryInformation.RegionSize, MEM_COMMIT, PAGE_READWRITE);
        if (!NT_SUCCESS(Status)) {
            BaseSetLastNTError(Status);
            return FALSE;
        }
    ...
    try {
        ...
        // STARTF_HASSHELLDATA means std handles are used for shell-private data.
        // This flag is used if an icon is passed to ShellExecuteEx.
    ...
    VdmRetry:
        ...
        if (!ARGUMENT_PRESENT(lpApplicationName)) {
            ...
            *TempNull = UNICODE_NULL;
...
#ifdef WX86
            // Wx86 applications must use x86 version of known exes for compatibility.
            if (UseKnownWx86Dll) {
                LPWSTR KnownName;
                NtCurrentTeb()->Wx86Thread.UseKnownWx86Dll = FALSE;
            ...
            Length = SearchPathW(NULL, lpApplicationName, (PWSTR) L".exe", MAX_PATH, NameBuffer, NULL) * 2;
            if (Length != 0 && Length < MAX_PATH * sizeof(WCHAR)) {
                // SearchPathW worked, but file might be a directory if this happens, we need to keep trying
            ...
            if (!Length || Length >= MAX_PATH << 1) {
                ...
                *TempNull = TempChar;
                ...
                // If we still have command line left, then keep going the point is to march through the command line looking for whitespace so we can try to find an image name launches of things like:
        ...
        Status = NtOpenFile(&FileHandle, SYNCHRONIZE | FILE_EXECUTE, &Obja, &IoStatusBlock, FILE_SHARE_READ | FILE_SHARE_DELETE, FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE);
        if (!NT_SUCCESS(Status)) {// if we failed, see if this is a device. If it is a device, then just return invalid image format
        ...
        // If no desktop has been specified, use the caller's desktop.
        if (StartupInfo.lpDesktop == NULL) {
            StartupInfo.lpDesktop = (LPWSTR) ((PRTL_USER_PROCESS_PARAMETERS) NtCurrentPeb()->ProcessParameters)->DesktopInfo.Buffer;
        }
        ...
        if (!NT_SUCCESS(Status)) {
...
#ifdef i386
                // Use OS/2 if x86 (OS/2 not supported on risc), and CreationFlags don't have forcedos bit and Registry didn't specify ForceDos
...
#endif
                // Falls into Dos case, so that stub message will be printed, and bound apps will run w/o OS/2 subsytem
            ...
            ForceDos :
            {
                         ...
                         if (Status == STATUS_INVALID_IMAGE_PROTECT || Status == STATUS_INVALID_IMAGE_NE_FORMAT || (BinarySubType = BaseIsDosApplication(&PathName, Status)))
                         {
                             VdmBinaryType = BINARY_TYPE_DOS;

                             // create the environment before going to the server.
                             // This was done becuase we want NTVDM to have the new environment when it gets created.
                             if (!BaseCreateVDMEnvironment(lpEnvironment, &AnsiStringVDMEnv, &UnicodeStringVDMEnv))
                                 return FALSE;

                             if (!BaseCheckVDM(VdmBinaryType | BinarySubType, lpApplicationName, lpCommandLine, lpCurrentDirectory, &AnsiStringVDMEnv, &m, &iTask, dwCreationFlags, &StartupInfo))
                                 return FALSE;
                             ...
                             case VDM_NOT_PRESENT:
                                 // mark this so the server can undo creation if something goes wrong.
                                 // We marked it "partitially created" because the NTVDM has yet not been fully created.
                                 // a call to UpdateVdmEntry to update process handle will signal the NTVDM process completed creation
                                 VDMCreationState = VDM_PARTIALLY_CREATED;
                                 // fail the call if NTVDM process is being created DETACHED.
                                 // note that, we let it go if NTVDM process is already running.
                         ...
                         } else {//  must be a .bat or .cmd file
            ...
            case STATUS_INVALID_IMAGE_WIN_16:// 16 bit windows exe
#if defined(BUILD_WOW6432) || defined(_WIN64)
                if (lpOriginalApplicationName == NULL) {
                    // pass in the part of the command line after the exe name including whitespace
            ...
            RetrySepWow:
                ...
                if (!BaseCheckVDM(VdmBinaryType, lpApplicationName, lpCommandLine, lpCurrentDirectory, &AnsiStringVDMEnv, &m, &iTask, dwCreationFlags, &StartupInfo))
                {
                    // If we failed with access denied, caller may not be allowed allowed to access the shared wow's desktop, so retry as a separate wow
                ...
                case VDM_NOT_PRESENT:
                    ...
                    // Throw away DETACHED_PROCESS flag which isn't meaningful for Win16 apps.
                    dwCreationFlags |= CREATE_NO_WINDOW;
                    dwCreationFlags &= ~(CREATE_NEW_CONSOLE | DETACHED_PROCESS);

                    // We're starting a WOW VDM, turn on feedback unless the creator passed STARTF_FORCEOFFFEEDBACK.
        ...
        // Query the section to determine the stack parameters and image entrypoint.
        Status = NtQuerySection(SectionHandle, SectionImageInformation, &ImageInformation, sizeof(ImageInformation), NULL);
        if (!NT_SUCCESS(Status)) {
            BaseSetLastNTError(Status);
            RtlFreeHeap(RtlProcessHeap(), 0, FreeBuffer);
            FreeBuffer = NULL;
        ...
        if (!(dwCreationFlags & (DEBUG_PROCESS | DEBUG_ONLY_THIS_PROCESS)) || NtCurrentPeb()->ReadImageFileExecOptions) {
            LdrQueryImageFileExecutionOptions(&PathName, L"Debugger", REG_SZ, ImageFileDebuggerCommand, sizeof(ImageFileDebuggerCommand), NULL);
        }

        if ((ImageInformation.Machine < USER_SHARED_DATA->ImageNumberLow) || (ImageInformation.Machine > USER_SHARED_DATA->ImageNumberHigh)) {
#if defined(WX86) || defined(_AXP64_)
        ...
        Status = NtCreateProcess(&ProcessHandle, PROCESS_ALL_ACCESS, pObja, NtCurrentProcess(), (BOOLEAN) bInheritHandles, SectionHandle, NULL, NULL);// Create the process object
        if (!NT_SUCCESS(Status)) {
            BaseSetLastNTError(Status);
            return FALSE;
        }

        // NtCreateProcess will set to normal OR inherit if parent is IDLE or Below only override if a mask is given during the create.
        ...
        if (dwCreationFlags & CREATE_DEFAULT_ERROR_MODE) {
            ...
            NtSetInformationProcess(ProcessHandle, ProcessDefaultHardErrorMode, (PVOID) &NewMode, sizeof(NewMode));
        }

        // If the process is being created for a VDM call the server with process handle.
        if (VdmBinaryType) {
            ...
            VDMCreationState |= VDM_FULLY_CREATED;// For Sep wow the VdmWaitHandle = NULL (there is none!)
        }

        // if we're a detached priority, we don't have the focus, so don't create with boosted priority.
        if (dwCreationFlags & DETACHED_PROCESS) {
            KPRIORITY SetBasePriority;

            SetBasePriority = (KPRIORITY) NORMAL_BASE_PRIORITY;
            Status = NtSetInformationProcess(ProcessHandle, ProcessBasePriority, (PVOID) &SetBasePriority, sizeof(SetBasePriority));
            ASSERT(NT_SUCCESS(Status));
        }

#if defined(i386) || defined(_IA64_)
        // Reserve memory in the new process' address space if necessary (for vdms). This is required only for x86 system.
        if (VdmReserve) {
            BigVdmReserve = VdmReserve;
            Status = NtAllocateVirtualMemory(ProcessHandle, &BaseAddress, 0L, &BigVdmReserve, MEM_RESERVE, PAGE_EXECUTE_READWRITE);
            if (!NT_SUCCESS(Status)){
                BaseSetLastNTError(Status);
                return FALSE;
            }
        ...
        // Determine the location of the processes PEB.
        Status = NtQueryInformationProcess(ProcessHandle, ProcessBasicInformation, &ProcessInfo, sizeof(ProcessInfo), NULL);
        if (!NT_SUCCESS(Status)) {
            BaseSetLastNTError(Status);
            return FALSE;
        }
        ...
        if (!BasePushProcessParameters(ProcessHandle, Peb, lpApplicationName, CurdirBuffer,
            QuoteInsert || QuoteCmdLine ? QuotedBuffer : lpCommandLine, lpEnvironment, &StartupInfo, dwCreationFlags | dwNoWindow, bInheritHandles, IsWowBinary ? IMAGE_SUBSYSTEM_WINDOWS_GUI : 0)) {
            return FALSE;
        }
        ...
        if (!VdmBinaryType && !bInheritHandles && !(StartupInfo.dwFlags & STARTF_USESTDHANDLES) && !(dwCreationFlags & (DETACHED_PROCESS | CREATE_NEW_CONSOLE | CREATE_NO_WINDOW)) &&
            ImageInformation.SubSystemType == IMAGE_SUBSYSTEM_WINDOWS_CUI)
        {
            PRTL_USER_PROCESS_PARAMETERS ParametersInNewProcess;

            Status = NtReadVirtualMemory(ProcessHandle, &Peb->ProcessParameters, &ParametersInNewProcess, sizeof(ParametersInNewProcess), NULL);
            if (NT_SUCCESS(Status)) {
                if (!CONSOLE_HANDLE(NtCurrentPeb()->ProcessParameters->StandardInput)) {
                    StuffStdHandle(ProcessHandle, NtCurrentPeb()->ProcessParameters->StandardInput, &ParametersInNewProcess->StandardInput);
                }
                if (!CONSOLE_HANDLE(NtCurrentPeb()->ProcessParameters->StandardOutput)) {
                    StuffStdHandle(ProcessHandle, NtCurrentPeb()->ProcessParameters->StandardOutput, &ParametersInNewProcess->StandardOutput);
                }
                if (!CONSOLE_HANDLE(NtCurrentPeb()->ProcessParameters->StandardError)) {
                    StuffStdHandle(ProcessHandle, NtCurrentPeb()->ProcessParameters->StandardError, &ParametersInNewProcess->StandardError);
                }
            }
        }
        ...
        // Allocate a stack for this thread in the address space of the target process.
        ...
        BaseInitializeContext(&ThreadContext, Peb, ImageInformation.TransferAddress, InitialTeb.StackBase, BaseContextTypeProcess);// Create an initial context for the new thread.
        pObja = BaseFormatObjectAttributes(&Obja, lpThreadAttributes, NULL);
        Status = NtCreateThread(&ThreadHandle, THREAD_ALL_ACCESS, pObja, ProcessHandle, &ClientId, &ThreadContext, &InitialTeb, TRUE);// Create the actual thread object
        if (!NT_SUCCESS(Status)) {
            BaseSetLastNTError(Status);
            return FALSE;
        }

        // From here on out, do not modify the address space of the new process.
        // WOW64's implementation of NtCreateThread() reshuffles the new process' address space if the current process is 32-bit and the new process is 64-bit.
#if DBG
        Peb = NULL;
#endif
        ...
        if (Wx86Info) {
            ...
            Status = BaseCreateWx86Tib(ProcessHandle, ThreadHandle, (ULONG) ((ULONG_PTR) ImageInformation.TransferAddress),
        ...
        // Call the Windows server to let it know about the process.
        ...
        // Set the 2 bit if a gui app is starting.
        // The window manager needs to know this so it can synchronize the startup of this app (WaitForInputIdle api).
        // This info is passed using the process handle tag bits.
        // The 1 bit asks the window manager to turn on or turn off the application start cursor (hourglass/pointer).

        // When starting a WOW process, lie and tell UserSrv NTVDM.EXE is a GUI process.
        // We also turn on bit 0x8 so that UserSrv can ignore the UserNotifyConsoleApplication call made by the console during startup.
        if (ImageInformation.SubSystemType == IMAGE_SUBSYSTEM_WINDOWS_GUI || IsWowBinary) {
            ...
            // If the creating process is a GUI app, turn on the app. start cursor by default.  This can be overridden by STARTF_FORCEOFFFEEDBACK.
...
#else
        CsrClientCallServer((PCSR_API_MSG) &m, NULL, CSR_MAKE_API_NUMBER(BASESRV_SERVERDLL_INDEX, BasepCreateProcess), sizeof(*a));
#endif
    ...
    VdmExists:
        ...
        try {
            if (VdmWaitHandle) {
                // tag Shared WOW VDM handles so that wait for input idle has a chance to work.
                // Shared WOW VDM "process" handles are actually event handles,  Separate WOW VDM handles are real process handles.
                // Also mark DOS handles with 0x1 so WaitForInputIdle has a way to distinguish DOS apps and not block forever.
                if (VdmBinaryType == BINARY_TYPE_WIN16)  {
                    lpProcessInformation->hProcess = (HANDLE) ((ULONG_PTR) VdmWaitHandle | 0x2);

                    // Shared WOW doesn't always start a process, so we don't have a process ID or thread ID to return if the VDM already existed.
                ...
                // Close the ProcessHandle, since we are returning the VdmProcessHandle instead.
...
Routine Description:
    ...
    Associated with the process handle is a set of access rights that may be performed using the process handle.
    The caller specifies the desired access to the process using the DesiredAccess parameter.
Arguments:
    mDesiredAccess - Supplies the desired access to the process object.
        For NT/Win32, this access is checked against any security descriptor on the target process.
        The following object type specific access flags can be specified in addition to the STANDARD_RIGHTS_REQUIRED access flags.
        DesiredAccess Flags:
    ...
    bInheritHandle - Supplies a flag that indicates whether or not the returned handle is to be inherited by a new process during process creation.
         A value of TRUE indicates that the new process will inherit the handle.
    dwProcessId - Supplies the process id of the process to open.
Return Value:
    NON-NULL - Returns an open handle to the specified process.
        The handle may be used by the calling process in any API that requires a handle to a process.
        If the open is successful, the handle is granted access to the process object only to the extent that it requested access through the DesiredAccess parameter.
...
Routine Description:
    ...
    ExitProcess is the prefered method of exiting an application.
    This API provides a clean application shutdown.
    This includes calling all attached DLLs at their instance termination entrypoint.
    If an application terminates by any other method:
        - TerminateProcess
...
--*/
{
    ...
    if ( BaseRunningInServerProcess ) {
        ...
        try {
            ...
            CsrClientCallServer( (PCSR_API_MSG)&m, NULL, CSR_MAKE_API_NUMBER( BASESRV_SERVERDLL_INDEX, BasepExitProcess), sizeof( *a ));
#endif
...
Routine Description:
    ...
    DLLs that the process is attached to that the process is terminating.
    TerminateProcess is used to unconditionally cause a process to exit.
    It should only be used in extreme circumstances.
    The state of global data maintained by DLLs may be comprimised if TerminateProcess is used rather that ExitProcess.

    Once all of the threads have terminated, the process attains a state of signaled satisfying any waits on the process.
    The process's termination status is updated from its initial value of STATUS_PENDING to the termination status of the last thread in the process to terminate (usually this is the same value as the
    TerminationStatus parameter).
    Terminating a process does not remove a process from the system.
    It simply causes all of the threads in the process to terminate their execution, and causes all of the object handles opened by the process to be closed.
    The process is not removed from the system until the last handle to the process is closed.
...
Routine Description:
    ...
    If a process is in the signaled state, calling this function returns the termination status of the process.
    If the process is not yet signaled, the termination status returned is STILL_ACTIVE.
Arguments:
...
Return Value:
    ...
    The block is opaque and must only be interpreted via the environment variable access functions.
--*/
{
...
Return Value:
    The actual number of bytes stored in the memory pointed to by the lpBuffer parameter.
    The return value is zero if the environment variable name was not found in the current process's environment.
    On successful return (returned value < nSize) the returned value does not include the null terminator byte.
    On buffer overflow failure (returned value > nSize), the returned value does include the null terminator byte.
--*/
{
    NTSTATUS Status;
    NTSTATUS Status2;
...
Arguments:
    lpName - Pointer to a null terminate string that is the name of the environment variable whose value is being requested.
    lpValue - An optional pointer to a null terminated string that is to be the new value of the specified variable name.
              If this parameter is NULL, then the variable will be deleted from the current process's environment.
Return Value:
    TRUE - The operation was successful
    FALSE/NULL - The operation failed. Extended error status is available using GetLastError.
--*/
...
DWORD WINAPI ExpandEnvironmentStringsA(LPCSTR lpSrc, LPSTR lpDst, DWORD nSize)
{
    ...
    Length = 0;
    Status = RtlExpandEnvironmentStrings_U(NULL, (PUNICODE_STRING) &UnicodeSource, (PUNICODE_STRING) &UnicodeDest, &Length);
    if (NT_SUCCESS(Status)) {
        if (nSize > MAXUSHORT - 2) {
            iSize = MAXUSHORT - 2;
        } else {
...
/*++
Routine Description:
    This function executes the Windows or non-Windows application identified by the lpCmdLine parameter.
    The uCmdShow parameter specifies the initial state of the application's main window when it is created.
    The WinExec function is obsolete.
    CreateProcess is the prefered mechanism for creating a process to run an application.
    The Win32 implementation of WinExec is layered on top of CreateProcess.
    For each parameter to CreateProcess, the following section describes how the parameter is formed, and its meaning with respect to WinExec.
        lpApplicationName - NULL
...
Arguments:
    lpCmdLine - Points to a null-terminated character string that contains the command line (filename plus optional parameters) for the application to be executed.
        If the lpCmdLine string does not contain a directory path, Windows will search for the executable file in this order:
        1.  The current directory
        2.  The Windows directory (the directory containing WIN.COM);
            the GetWindowsDirectory function obtains the pathname of this directory
    ...
    uCmdShow - Specifies how a Windows application window is to be shown.
        See the description of the ShowWindow function for a list of the acceptable values for the uCmdShow parameter.
        For a non-Windows application, the PIF file, if any, for the application determines the window state.
Return Value:
...
retry:
    ...
    CreateProcessStatus = CreateProcess(NULL, (LPSTR)lpCmdLine, NULL, NULL, FALSE, 0, NULL, NULL, &StartupInfo, &ProcessInformation);
    if ( CreateProcessStatus ) {
        // Wait for the started process to go idle. If it doesn't go idle in 10 seconds, return anyway.
        ...
        return 33;
    } else {// If CreateProcess failed, then look at GetLastError to determine appropriate return code.
        // Take a closer look at CreateProcess errors. For instance, Claris Works 5.0 launches hyperterm.exe as hyperterm.exe"<null> the trailing " is causing problems so nuke it and then retry.
...
Routine Description:
    ...
    The LoadModule function is obsolete.  CreateProcess is the prefered mechanism for creating a process to run an application.
    The Win32 implementation of LoadModule is layered on top of CreateProcess.
    For each parameter to CreateProcess, the following section describes how the parameter is formed, and its meaning with respect to LoadModule.
        ...
        lpCurrentDirectory - A value of NULL is used.
        lpStartupInfo - The structure is initialized to NULL.
                        The cb field is initialized, and the wShowWindow field is set to the value of second word of the lpCmdShow field of the parameter block is used.
        lpProcessInformation.hProcess - The handle is immediately closed.
        lpProcessInformation.hThread - The handle is immediately closed.
Arguments:
    lpModuleName - Points to a null-terminated string that contains the filename of the application to be run.
                   If the lpModuleName string does not contain a directory path, Windows will search for the executable file in this order:
    ...
    lpParameterBlock - Points to a data structure consisting of four fields that defines a parameter block.  This data structure consists of the following fields:
        ...
        lpEnvAddress - Points to an array of NULL terminated strings that supply the environment strings for the new process.
            The array has a value of NULL as its last entry.
            A value of NULL for this parameter causes the new process to start with the same environment as the calling process.
        ...
        lpCmdShow - Points to a structure containing two WORD values.
            ...
            The second value specifies how the application window is to be shown and is used to supply the dwShowWindow parameter to CreateProcess.
            See the description of the <uCmdShow> paramter of the ShowWindow function for a list of the acceptable values.
    ...
    try {
        ...
        if (!Length || Length >= MAX_PATH) {
            ...
            if (PathType != RtlPathTypeRelative) {
                ...
                // The failed open should set get last error properly.
                hFile = CreateFile(lpModuleName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
                RtlFreeHeap(RtlProcessHeap(), 0, NameBuffer);
                if (hFile != INVALID_HANDLE_VALUE) {
                    CloseHandle(hFile);
                    return 2;
        ...
        CreateProcessStatus = CreateProcess(NameBuffer, CommandLineBuffer, NULL, NULL, FALSE, CreateFlag, LoadModuleParams->lpEnvAddress, NULL, &StartupInfo, &ProcessInformation);
        RtlFreeHeap(RtlProcessHeap(), 0, NameBuffer);
        NameBuffer = NULL;
...
Routine Description:
    ...
    On NT/Win32, this handle has the maximum access allowed by any security descriptor placed on the current process.
Return Value:
...
Return Value:
    Returns a unique value representing the process ID of the currently executing process.
    The return value may be used to open a handle to a process.
--*/
{
...
BOOL WINAPI ReadProcessMemory(HANDLE hProcess, LPCVOID lpBaseAddress, LPVOID lpBuffer, DWORD nSize, LPDWORD lpNumberOfBytesRead)
/*++
Routine Description:
    Memory within a specified process can be read using ReadProcessMemory.

    This function copies the data in the specified address range from the specified process into the specified buffer of the current process.
    The specified process does not have to be being debugged in order for this API to operate.
    The caller must simply have a handle to the process that was created with PROCESS_VM_READ access.
Arguments:
    hProcess - Supplies an open handle to a process whose memory is to be read.
               The handle must have been created with PROCESS_VM_READ access to the process.

    lpBaseAddress - Supplies the base address in the specified process to be read.
        Before any data transfer occurs, the system verifies that all data within the base address and the specified size is accessible for read access.
        If this is the case, then the API proceeds.  Otherwise the API fail.
    ...
    lpNumberOfBytesRead - An optional parameter, that if supplied receives the actual number of bytes transferred into the specified buffer.
        This can be different than the value of nSize if the requested read crosses into an area of the process that
        is inaccessible (and that was made inaccessible during the data transfer).
        If this occurs a value of FALSE is returned and GetLastError returns a "short read" error indicator.
...
{
    NTSTATUS Status;
    Status = NtReadVirtualMemory(hProcess, (PVOID)lpBaseAddress, lpBuffer, nSize, lpNumberOfBytesRead);
    if ( !NT_SUCCESS(Status) ) {
        BaseSetLastNTError(Status);
        return FALSE;
    } else {
...
Routine Description:
    ...
    This function copies the from the specified buffer in the current process to the address range of the specified process.
    The specified process does not have to be being debugged in order for this API to operate.
    The caller must simply have a handle to the process that was created with PROCESS_VM_WRITE access.
Arguments:
    ...
    lpBaseAddress - Supplies the base address in the specified process to be written.
        Before any data transfer occurs, the system verifies that all data within the base address and the specified size is accessible for write access.
        If this is the case, then the API proceeds.
        Otherwise the API fail.
    ...
    lpNumberOfBytesWritten - An optional parameter, that if supplied receives the actual number of bytes transferred into the specified process.
        This can be different than the value of nSize if the requested write crosses into an area of the process that is inaccessible (and that was made inaccessible during the data transfer).  .
        If this occurs a value of FALSE is returned and GetLastError returns a "short write" error indicator.
Return Value:
    TRUE - The operation was successful.
    FALSE/NULL - The operation failed. Extended error status is available using GetLastError.
--*/
{
    ...
    RegionSize = nSize;
    Base = lpBaseAddress;
    Status = NtProtectVirtualMemory(hProcess, &Base, &RegionSize, PAGE_EXECUTE_READWRITE, &OldProtect);
    if (NT_SUCCESS(Status)) {
        // See if previous protection was writable. If so, then reset protection and do the write.
        // Otherwise, see if previous protection was read-only or no access. In this case, don't do the write, just fail
            ...
            (OldProtect & PAGE_EXECUTE_WRITECOPY) == PAGE_EXECUTE_WRITECOPY)
        {
            ...
            Status = NtWriteVirtualMemory(hProcess, lpBaseAddress, lpBuffer, nSize, lpNumberOfBytesWritten);
            if (!NT_SUCCESS(Status)) {
                BaseSetLastNTError(Status);
                return FALSE;
            }
            ...
            // See if the previous protection was read only or no access. If this is the case, restore the previous protection and return an access violation error.
            if ((OldProtect & PAGE_NOACCESS) == PAGE_NOACCESS ||  (OldProtect & PAGE_READONLY) == PAGE_READONLY) {
                ...
                // The previous protection must have been code and the caller is trying to set a breakpoint or edit the code. Do the write and then restore the previous protection.
                Status = NtWriteVirtualMemory(hProcess, lpBaseAddress, lpBuffer, nSize, lpNumberOfBytesWritten);
...
/*++
Routine Description:
    The GetSystemInfo function is used to return information about the current system.
    This includes the processor type, page size, oem id, and other interesting pieces of information.
Arguments:
    lpSystemInfo - Returns information about the current system.
        ...
        DWORD dwActiveProcessorMask - Returns a mask representing the set of processors configured into the system.
                                      Bit 0 is processor 0, bit 31 is processor 31.
        DWORD dwNumberOfProcessors - Returns the number of processors in the system.
        WORD wProcessorLevel - Returns the level of the processors in the system.
                               All processors are assumed to be of the same level, stepping, and are configured with the same options.
        WORD wProcessorRevision - Returns the revision or stepping of the processors in the system.
                                  All processors are assumed to be of the same level, stepping, and are configured with the same options.
--*/
{
    NTSTATUS Status;
    ...
    // for apps less than 3.51, then return 0 in dwReserved.
    // This allows borlands debugger to continue to run since it mistakenly used dwReserved as AllocationGranularity
...
--*/
{
    ...
    case SystemInfoRemoteBootServerPath:
        ...
        } else {// This is not a remote boot client. Return success with a zero-length buffer.
            *nSize = 0;
            return TRUE;
        }
        break;
    default:
        SetLastError(ERROR_INVALID_PARAMETER);// Unrecognized information class.
        return FALSE;
    }

    // If the buffer isn't big enough, tell the caller how big the buffer needs to be and return an error.
...
/*++
Routine Description:
    The GetSystemInfoEx function is used to return information about the current system.
    It returns different information depending on the requested class.
Arguments:
...
Return Value:
    ...
    ERROR_BUFFER_OVERFLOW, then the supplied buffer was too small too contain all of the information, and nSize returns the required buffer size.
--*/
{
    DWORD requiredSize;
    BOOL isRemoteBoot;
...
BOOL BuildSubSysCommandLine(LPWSTR  lpSubSysName, LPCWSTR lpApplicationName, LPCWSTR lpCommandLine, PUNICODE_STRING SubSysCommandLine)
{
    UNICODE_STRING Args;
    ...
    RtlInitUnicodeString(&Command, lpApplicationName);// Get application name length
    RtlInitUnicodeString(&Args, lpCommandLine);// get lpCommandLine length
    ...
    if ( SubSysCommandLine->Buffer ) {// New command line begins with either L"OS2 /P " or L"POSIX /P "
        RtlAppendUnicodeToString(SubSysCommandLine, lpSubSysName);
        RtlAppendUnicodeStringToString(SubSysCommandLine, &Command);// append full path name
        RtlAppendUnicodeToString(SubSysCommandLine, L" /C ");
        RtlAppendUnicodeStringToString(SubSysCommandLine, &Args);// and append to new command line
    } else {
        BaseSetLastNTError(STATUS_NO_MEMORY);
        ReturnStatus = FALSE;
    }
...
Routine Description:
    ...
    PROCESS_SET_INFORMATION and PROCESS_QUERY_INFORMATION access is required to the process in order to call this API.
    Using this API has a dramatic impact on the scheduling characteristics of the effected process.
    Applications should use this API carefully and understand the impact of making a process run in either the Idle or High priority classes.
Arguments:
    ...
    dwPriorityClass - Supplies the new priority class for the process.
        The priority class constants are described above.
        If more than one priority class is specified, the lowest specified priority class is used.
Return Value:
...
--*/
{
    ...
    PriClass.Foreground = FALSE;
    Status = NtSetInformationProcess(hProcess, ProcessPriorityClass, (PVOID)&PriClass, sizeof(PriClass));
    if ( State ) {
        BasepReleasePrivilege( State );
    }
...
Routine Description:
    ...
    PROCESS_QUERY_INFORMATION access is required to the process in order to call this API.
Arguments:
...
--*/
{
    ...
    Status = NtQueryInformationProcess(hProcess, ProcessPriorityClass, &PriClass, sizeof(PriClass), NULL);
    if (!NT_SUCCESS(Status)) {
        BaseSetLastNTError(Status);
        return 0;
    }
...
Routine Description:
    ...
    Note that since Win32 is a pre-emptive multi-tasking environment, the results of this test are only meaningful if the other threads in
    the process do not manipulate the range of memory being tested by this call.
    Even after a pointer validation, an application should use the structured exception handling capabilities present in the system to gaurd access through pointers that it does not control.
Arguments:
...
--*/
{
    ...
    // If the structure has zero length, then do not probe the structure for read accessibility or alignment.
    if (cb != 0) {
        ...
        // Compute the ending address of the structure and probe for read accessibility.
...
Routine Description:
    ...
    Note that since Win32 is a pre-emptive multi-tasking environment, the results of this test are only meaningful if the other threads in
    the process do not manipulate the range of memory being tested by this call.
    Even after a pointer validation, an application should use the structured exception handling capabilities present in the
...
--*/
{
    ...
    // If the structure has zero length, then do not probe the structure for write accessibility.
    if (cb != 0) {
        ...
        // Compute the ending address of the structure and probe for write accessibility.
        EndAddress = StartAddress + cb - 1;
        if (EndAddress < StartAddress) {
            return TRUE;
        } else {
...
--*/
{
    ...
    // If the structure has zero length, then do not probe the structure for read accessibility.
    if (cchMax != 0) {
...
/*++
Routine Description:
    This function sets shutdown parameters for the currently calling process.
    dwLevel is the field that defines this processes shutdown order relative to the other processes in the system.
    Higher levels shutdown first, lower levels shutdown last.
Arguments:
...
Notes:
    Applications running in the system security context do not get shut down by the system.
    They will get notified of shutdown or logoff through the callback installable via SetConsoleCtrlRoutine() (see that for more info).
    They also will get notified in the order specified by the dwLevel parameter.
Return Value
...
Arguments:
    ...
    lpdwFlags - Pointer to the DWORD where the shutdown flags information should be put.
Return Value
...
/*++
Routine Description:
    This function is used to return the processor affinity mask for the selected process and for the system.
    The process affinity mask is a bit vector where each bit represents the processors that the process is allowed to run on.
    The system affinity mask is a bit vector where each bit represents the processors configured into the system
...
/*++
Routine Description:
    This function allows the caller to determine the minimum and maximum working set sizes of the specified process.
    The working set sizes effect the virtual memory paging behavior for the process.
Arguments:
    hProcess - Supplies an open handle to the specified process.  The handle must have been created with PROCESS_QUERY_INFORMATION access.

    lpMinimumWorkingSetSize - Supplies the address of the variable that will receive the minimum working set size of the specified process.
        The virtual memory manager will attempt to keep at least this much memory resident in the process whenever the process is active.

    lpMaximumWorkingSetSize - Supplies the address of the variable that will receive the maximum working set size of the specified process.
        In tight memory situations, the virtual memory manager will attempt to keep at no more than this much memory resident in the process whenever the process is active.
...
/*++
Routine Description:
    This function allows the caller to set the minimum and maximum working set sizes of the specified process.
    The working set sizes effect the virtual memory paging behavior for the process.
    The specified process's working set be emptied (essentially swapping out the process) by specifying the distinguished values 0xffffffff for both the minimum and maximum working set sizes.
...
Arguments:
    ...
    dwMinimumWorkingSetSize - Supplies the minimum working set size for the specified process.
        The virtual memory manager will attempt to keep at least this much memory resident in the process whenever the process is active.
        A value of (SIZE_T)-1 and the same value in dwMaximumWorkingSetSize will temporarily trim the working set of the specified process (essentially out swap the process).

    dwMaximumWorkingSetSize - Supplies the maximum working set size for the specified process.
        In tight memory situations, the virtual memory manager will attempt to keep at no more than this much memory resident in the process whenever the process is active.
        A value of (SIZE_T)-1 and the same value in dwMinimumWorkingSetSize will temporarily trim the working set of the specified process (essentially out swap the process).
...
--*/
{
    ...
    if (dwMinimumWorkingSetSize == 0 || dwMaximumWorkingSetSize == 0) {
        ...
        if (dwMaximumWorkingSetSize == dwMinimumWorkingSetSize && dwMaximumWorkingSetSize == (SIZE_T) -1) {
...
DWORD WINAPI GetProcessVersion(DWORD ProcessId)
{
    ...
    try {
        if (ProcessId == 0 || ProcessId == GetCurrentProcessId()) {
            ...
            b = ReadProcessMemory(hProcess, &NtHeader->OptionalHeader.MajorSubsystemVersion, &SwappedVersion, sizeof(SwappedVersion), NULL);
            if (!b) {
                goto finally_exit;
            }
...
BOOL WINAPI SetProcessAffinityMask(HANDLE hProcess, DWORD_PTR dwProcessAffinityMask)
{
    ...
    Status = NtSetInformationProcess(hProcess, ProcessAffinityMask, &dwProcessAffinityMask, sizeof(dwProcessAffinityMask));
    if (!NT_SUCCESS(Status)) {
        BaseSetLastNTError(Status);
        return FALSE;
    }
...
NTSTATUS BasepConfigureAppCertDlls(IN PWSTR ValueName, IN ULONG ValueType, IN PVOID ValueData, IN ULONG ValueLength, IN PVOID Context, IN PVOID EntryContext)
{
...
NTSTATUS BasepSaveAppCertRegistryValue(IN OUT PLIST_ENTRY ListHead, IN PWSTR Name, IN PWSTR Value OPTIONAL)
{
    PLIST_ENTRY Next;
    PBASEP_APPCERT_ENTRY p;
    UNICODE_STRING UnicodeName;
    ...
    InsertTailList(ListHead, &p->Entry);
     ...
     if (ARGUMENT_PRESENT(Value)) {
         HINSTANCE hDll = LoadLibraryW(Value);// load certification DLL
         if (hDll == NULL) {// The library was not loaded, return.
...
Arguments:
    fPrefixMappedApplicationName
        - TRUE means that the original lpApplicationName was NULL.
               The application name was stripped from the head of lpCommandLine.
               The mapped application name needs to be added to the head of the mapped command line.
        - FALSE means that the original lpApplicationName was non-NULL.
               the lpCommandLine argument is identical to the original lpCommandLine argument.
...
--*/
{
    ...
    // lpCommandLine sits in the StaticUnicodeBuffer in the Teb and also a high probability that LoadLibrary will trash that buffer in a bad way
    ...
    result = (*lpfn)(fPrefixMappedApplicationName, lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes,
