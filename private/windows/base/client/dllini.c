...
NTSTATUS BaseDllInitializeIniFileMappings(PBASE_STATIC_SERVER_DATA StaticServerData)
{
...
DWORD APIENTRY GetProfileStringW(
                 LPCWSTR lpAppName,
                 LPCWSTR lpKeyName,
                 LPCWSTR lpDefault,
...
DWORD APIENTRY GetProfileSectionW(LPCWSTR lpAppName, LPWSTR lpReturnedString, DWORD nSize)
{
...
UINT APIENTRY GetPrivateProfileIntW(LPCWSTR lpAppName, LPCWSTR lpKeyName, INT nDefault, LPCWSTR lpFileName)
{
    ...
    ReturnValue = 0;
    cb = GetPrivateProfileStringW( lpAppName, lpKeyName, NULL, ValueBuffer, sizeof( ValueBuffer ) / sizeof( WCHAR ), lpFileName);
    if (cb == 0) {
        ReturnValue = nDefault;
    } else {
...
DWORD APIENTRY GetPrivateProfileSectionW(LPCWSTR lpAppName, LPWSTR lpReturnedString, DWORD nSize, LPCWSTR lpFileName)
{
    NTSTATUS Status;
    ULONG n;
...
NTSTATUS BaseDllReadWriteIniFileViaMapping(IN PINIFILE_PARAMETERS a);
NTSTATUS BaseDllReadWriteIniFileOnDisk(IN PINIFILE_PARAMETERS a);
NTSTATUS BaseDllOpenIniFileOnDisk(IN PINIFILE_PARAMETERS a);
NTSTATUS BaseDllCloseIniFileOnDisk(IN PINIFILE_PARAMETERS a);
NTSTATUS BaseDllAppendNullToResultBuffer(IN PINIFILE_PARAMETERS a);
NTSTATUS BaseDllAppendStringToResultBuffer(IN PINIFILE_PARAMETERS a, IN PANSI_STRING String OPTIONAL, IN PUNICODE_STRING StringU OPTIONAL, IN BOOLEAN IncludeNull);
NTSTATUS BaseDllAppendBufferToResultBuffer(IN PINIFILE_PARAMETERS a, IN PBYTE Buffer OPTIONAL, IN PWSTR BufferU OPTIONAL, IN ULONG Chars, IN BOOLEAN IncludeNull);
                       ...
                       )
{
    ...
    Status = BaseDllCaptureIniFileParameters( Unicode, Operation, WriteOperation, MultiValueStrings, FileName, ApplicationName, VariableName, VariableValue, VariableValueLength, &a);
    if (!NT_SUCCESS( Status )) {
        return Status;
    }
    ...
    if (a->Operation == RefreshIniFileMapping) {
        ...
        if (a->BaseFileName.Length > (MAX_PATH * sizeof( WCHAR ))) {
            ...
            if (CaptureBuffer == NULL) {
                ...
                CsrCaptureMessageString( CaptureBuffer, (PCHAR)a->BaseFileName.Buffer, a->BaseFileName.Length, a->BaseFileName.MaximumLength, (PSTRING)&ap->IniFileName);
...
ULONG BaseDllIniFileNameLength(IN BOOLEAN Unicode, IN PVOID *Name);
NTSTATUS BaseDllFindIniFileNameMapping(IN PUNICODE_STRING FileName, IN PUNICODE_STRING BaseFileName, OUT PINIFILE_MAPPING_FILENAME *ReturnedFileNameMapping);
BOOLEAN BaseDllGetApplicationName(IN PINIFILE_PARAMETERS a, OUT PANSI_STRING *ApplicationName OPTIONAL, OUT PUNICODE_STRING *ApplicationNameU OPTIONAL);
BOOLEAN BaseDllGetVariableName(IN PINIFILE_PARAMETERS a, OUT PANSI_STRING *VariableName OPTIONAL, OUT PUNICODE_STRING *VariableNameU OPTIONAL);
BOOLEAN BaseDllGetVariableValue(IN PINIFILE_PARAMETERS a, OUT PBYTE *VariableValue OPTIONAL, OUT PWSTR *VariableValueU OPTIONAL, OUT PULONG VariableValueLength);
    ...
    *Key = INVALID_HANDLE_VALUE;
    ...
    if (WriteAccess) {
        ...
        if ( Status == STATUS_OBJECT_NAME_NOT_FOUND && OperationRetried == FALSE ) {
            ...
            InitializeObjectAttributes( &RetryObjectAttributes, &RetryRegistryPath, OBJ_CASE_INSENSITIVE | OBJ_OPENIF, NULL, NULL);
...
NTSTATUS BaseDllCheckKeyNotEmpty(IN HANDLE Key, IN PUNICODE_STRING SubKeyName)
{
    NTSTATUS Status;
    OBJECT_ATTRIBUTES ObjectAttributes;
    HANDLE SubKey;
                        ...
                        )
{
    ...
    Status = NtQueryValueKey( Key, VariableName, KeyValuePartialInformation, &KeyValueInformation, sizeof( KeyValueInformation ), &ResultLength);
    if (!NT_SUCCESS( Status )) {
        if (Status != STATUS_BUFFER_OVERFLOW) {
            return Status;
        }
    ...
    try {
        ...
        while (CurrentValueLength) {
            if (a->Unicode) {
                ...
                a->VariableNameU.Length = 0;
                FreeBuffer = RtlAllocateHeap( RtlProcessHeap(), MAKE_TAG( TMP_TAG ), a->VariableNameU.MaximumLength + ((a->ValueLength+1) * sizeof( WCHAR )));
                if (FreeBuffer == NULL) {
                    Status = STATUS_NO_MEMORY;
                    break;
                }
                         ...
                         )
{
    ...
    Status = NtQueryValueKey( Key, VariableName, KeyValueBasicInformation, &KeyValueInformation, sizeof( KeyValueInformation ), &ResultLength);
    if (NT_SUCCESS( Status ) || Status == STATUS_BUFFER_OVERFLOW) {
                        ...
                        )
{
    ...
    if (NT_SUCCESS( Status )) {
        if (RtlDosPathNameToNtPathName_U( a->NtFileName.Buffer, &FullFileName, &FilePart, NULL)) {
            ...
            IniFile = RtlAllocateHeap( RtlProcessHeap(), MAKE_TAG( INI_TAG ) | HEAP_ZERO_MEMORY, sizeof( *IniFile ) + a->NtFileName.MaximumLength);
            if (IniFile == NULL) {
                return STATUS_NO_MEMORY;
            }
    ...
    Status = NtAllocateVirtualMemory( NtCurrentProcess(), &IniFile->BaseAddress, 0, &IniFile->RegionSize, MEM_RESERVE, PAGE_READWRITE);
    if (NT_SUCCESS( Status )) {
        Status = NtAllocateVirtualMemory( NtCurrentProcess(), &IniFile->BaseAddress, 0, &IniFile->CommitSize, MEM_COMMIT, PAGE_READWRITE);
        if (NT_SUCCESS( Status )) {
                       ...
                       )
{
    ...
    if (NewEndOfFile > a->IniFile->CommitSize) {
        ...
        Status = NtAllocateVirtualMemory( NtCurrentProcess(), &a->IniFile->BaseAddress, 0, &a->IniFile->CommitSize, MEM_COMMIT, PAGE_READWRITE);
        if (!NT_SUCCESS( Status )) {
    ...
    if (SizeToRemove > InsertAmount) {
        ...
        // Delete remaining text after insertion string by moving it up
