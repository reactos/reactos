...
ULONG BasepOfShareToWin32Share(IN ULONG OfShare)
{
    DWORD ShareMode;
...
PUNICODE_STRING BaseIsThisAConsoleName(PUNICODE_STRING FileNameString, DWORD dwDesiredAccess)
{
    ...
    if (FileNameString->Length)
    {
        ...
        if (sch == (WCHAR)'c' || sch == (WCHAR)'C' || sch == (WCHAR)'\\' || ech == (WCHAR)'n' || ech == (WCHAR)'N' || ech == (WCHAR)':' || ech == (WCHAR)'$')
        {
...
DWORD WINAPI CopyReparsePoint(HANDLE hSourceFile, HANDLE hDestinationFile)
/*++
Routine Description:
...
/*++
Routine Description:
    This is an internal routine that copies a name grafting file/directory preserving its characteristics.
Arguments:
    hSourceFile - Provides a handle to the source file.
    lpExistingFileName - Provides the name of the existing, source file.
    lpNewFileName - Provides a name for the target file/stream. This must not be a UNC path name.
    lpProgressRoutine - Optionally supplies the address of a callback routine to be called as the copy operation progresses.
    lpData - Optionally supplies a context to be passed to the progress callback routine.
    pbCancel - Optionally supplies the address of a boolean to be set to TRUE if the caller would like the copy to abort.
    lpCopyFlags - Provides flags that modify how the copy is to proceed.  See CopyFileEx for details.
Return Value:
    TRUE - The operation was successful.
    FALSE - The operation failed. Extended error status is available using GetLastError.
--*/
{   // CopyNameGraftNow
    ...
    try
    {
        ...
        */
        ...
        InitializeObjectAttributes(&Obja, &DestFileName, OBJ_CASE_INSENSITIVE, DestRelativeName.ContainingDirectory, NULL);
        ...
        if ( !(*lpCopyFlags & COPY_FILE_FAIL_IF_EXISTS) && ((Status == STATUS_EAS_NOT_SUPPORTED) || (Status == STATUS_IO_REPARSE_TAG_MISMATCH)) ) {
            // In either of these error conditions, the correct behavior is to first delete the destination file and then copy the name graft.

            // Re-open the destination for the deletion without inhibiting the reparse behavior.
            ...
            /*
            DestFile = CreateFileW(lpNewFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_NEW, FILE_FLAG_OPEN_REPARSE_POINT, NULL);
        ...
        Status = NtQueryInformationFile(hSourceFile, &IoStatusBlock, (PVOID) &BasicInformation, sizeof(BasicInformation), FileBasicInformation);
        if ( !NT_SUCCESS(Status) ) {
            BaseSetLastNTError(Status);
            leave;
        }
        ...
        // If the time cannot be set for whatever reason, we still return TRUE.
        Status = NtSetInformationFile(DestFile, &IoStatusBlock, &BasicInformation, sizeof(BasicInformation), FileBasicInformation);
        if ( Status == STATUS_SHARING_VIOLATION ) {
            ...
            // Re-Open the destination file inhibiting the reparse behavior as we know that it is a symbolic link.  Please note that we do this
            // using the CreateFileW API.  The CreateFileW API allows you to
            // pass NT native desired access flags, even though it is not documented to work in this manner.

            /*
            DestFile = CreateFileW(lpNewFileName, FILE_WRITE_ATTRIBUTES, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OPEN_REPARSE_POINT, NULL);
            */
            Status = NtCreateFile( &DestFile, FILE_WRITE_ATTRIBUTES, &Obja, &IoStatus, NULL, 0, 0, FILE_OPEN, FILE_OPEN_REPARSE_POINT | CreateOptions, NULL, 0 );
            if ( NT_SUCCESS( Status )) {
                // If the open succeeded, we update the file information on the new file.
...
--*/
{
    ...
    b = CopyFileExW((LPCWSTR)StaticUnicode->Buffer, (LPCWSTR)DynamicUnicode.Buffer, (LPPROGRESS_ROUTINE)NULL, (LPVOID)NULL, (LPBOOL)NULL, bFailIfExists ? COPY_FILE_FAIL_IF_EXISTS : 0);
...
/*++
Routine Description:
    A file, its extended attributes, alternate data streams, and any other attributes can be copied using CopyFile.
Arguments:
    lpExistingFileName - Supplies the name of an existing file that is to be copied.
    lpNewFileName - Supplies the name where a copy of the existing files data and attributes are to be stored.
    bFailIfExists - Supplies a flag that indicates how this operation is to proceed if the specified new file already exists.
        A value of TRUE specifies that this call is to fail.
        A value of FALSE causes the call to the function to succeed whether or not the specified new file exists.
Return Value:
    TRUE - The operation was successful.
...
--*/
{
    ...
    b = CopyFileExW(lpExistingFileName, lpNewFileName, (LPPROGRESS_ROUTINE)NULL, (LPVOID)NULL, (LPBOOL)NULL, bFailIfExists ? COPY_FILE_FAIL_IF_EXISTS : 0);
    return b;
}


BOOL WINAPI CopyFileExA(LPCSTR lpExistingFileName, LPCSTR lpNewFileName, LPPROGRESS_ROUTINE lpProgressRoutine OPTIONAL, LPVOID lpData OPTIONAL, LPBOOL pbCancel OPTIONAL, DWORD dwCopyFlags)
/*++
Routine Description:
...
--*/
{
    ...
    b = CopyFileExW((LPCWSTR)StaticUnicode->Buffer, (LPCWSTR)DynamicUnicode.Buffer, lpProgressRoutine, lpData, pbCancel, dwCopyFlags);
    RtlFreeUnicodeString(&DynamicUnicode);
    return b;
}
...
/*++
Routine Description:
    During CopyFile, check to see if the source is a symlink which requires special processing during copy.
Arguments:
    SourceFile - Handle for the source of the copy.
    IsNameGrafting - If true on return, the source file is grafted.
    bCopyRawSourceFile - If true on return, the source file needn't be reopened.
        If false, the file should be reopened without the FILE_OPEN_REPARSE_POINT flag.
    bOpenFilesAsReparsePoint - If true on return, source/dest named streams should be opened/created with FILE_OPEN_REPARSE_POINT specified.
    bIsNssFile - If true on return, the source file is a Native Structured Storage file.
    FileTagInformation - Pointer to location to hold the results of NtQueryInformationFile(FileAttributeTagInformation).
Return Value:
    NTSTATUS
--*/
{
    ...
    Status = NtQueryInformationFile(SourceFile, &IoStatus, (PVOID) FileTagInformation, sizeof(*FileTagInformation), FileAttributeTagInformation);
    if ( !NT_SUCCESS(Status) ) {
        //  Not all File Systems implement all information classes.
        //  The value STATUS_INVALID_PARAMETER is returned when a non-supported information class is requested to a back-level File System. As all
        //  the parameters to NtQueryInformationFile are correct, we can infer in this case that we found a back-level system.
        if ( (Status != STATUS_INVALID_PARAMETER) && (Status != STATUS_NOT_IMPLEMENTED) ) {
            return( Status );
        }
        Status = STATUS_SUCCESS;

        //  If FileAttributeTagInformation is not supported, we assume that the file at hand is not a reparse point nor a symbolic link.
        //  The copy of these files is the same as the raw copy of a file.
        //  The target file is opened without inhibiting the reparse point behavior.
        *bCopyRawSourceFile = TRUE;
    } else {
       //  The source file is opened and the file system supports the FileAttributeTagInformation information class.
       if ( FileTagInformation->FileAttributes & FILE_ATTRIBUTE_REPARSE_POINT ) {
           ...
           if (( FileTagInformation->ReparseTag == IO_REPARSE_TAG_NSS ) || ( FileTagInformation->ReparseTag == IO_REPARSE_TAG_NSSRECOVER)) {
               //  It is safe to copy the reparse point raw. Whenever possible
               //  inhibit the reparse behavior at the source and at the destination file systems.
               *bCopyRawSourceFile = TRUE;
    ...
    )
{
    ...
    try {
        ...
        //  (3) establish whether a reparse point that is not a symbolic link is to be copied in raw format or re-enabling the reparse point behavior
...
retry_open_SourceFile:
        ...
        while( TRUE ) {
            SourceFile = CreateFileW(lpExistingFileName, SourceFileAccess, SourceFileSharing, NULL, OPEN_EXISTING, SourceFileFlagsAndAttributes, NULL);
            if ( SourceFile == INVALID_HANDLE_VALUE ) {
                // If we tried to get ACCESS_SYSTEM_SECURITY access, that might cause an access or privilege error.
                if( ( GetLastError() == ERROR_PRIVILEGE_NOT_HELD || GetLastError() == ERROR_ACCESS_DENIED) && (SourceFileAccess & ACCESS_SYSTEM_SECURITY) ) {
                    // Turn it off
                    SourceFileAccess &= ~ACCESS_SYSTEM_SECURITY;
                }

                // Maybe we should stop requesting write access (done for COPYFILE_OPEN_SOURCE_FOR_WRITE
                else if( GetLastError() == ERROR_ACCESS_DENIED && (GENERIC_WRITE & SourceFileAccess) ) {
                    // Turn it off, but if originally requested, turn access_system_security back on.
                ...
                // Try sharing for writing.
                    ...
                    SourceFileSharing |= FILE_SHARE_WRITE;// Add write-sharing
                    SourceFileAccess = SourceFileAccessDefault;// Start back over wrt the access flags
                }
                ...
                else {// Otherwise there's nothing more we can try.
                    leave;
                }
            }   // if ( SourceFile == INVALID_HANDLE_VALUE )

            // We've opened the source file.
            // If we haven't yet checked for name grafting (symbolic links), do so now.
            else if( !CheckedForNameGrafting ) {
                CheckedForNameGrafting = TRUE;
                //  Find out whether the file is a symbolic link and whether a reparse point can be copied with the reparse behavior inhibited.
                Status = BasepProcessNameGrafting( SourceFile, &IsNameGrafting, &bCopyRawSourceFile, &bOpenFilesAsReparsePoint, &bIsNssFile, &FileTagInformation );
                if( !NT_SUCCESS(Status) ) {
                ...
                if ( IsNameGrafting ) {
                    ...
                    Status = CopyNameGraftNow(
                        ...
                        lpExistingFileName,
                        lpNewFileName,
                        (PRIVCOPY_FILE_DIRECTORY & dwPrivCopyFlags) ? (FILE_DIRECTORY_FILE | FILE_OPEN_FOR_BACKUP_INTENT) : 0,
                        lpProgressRoutine,
                        lpData,
                        pbCancel,
                        &dwCopyFlags
                ...
                // If we're doing a raw copy, we can start doing the copy with this SourceFile handle.
        ...
        Status = NtQueryInformationFile(SourceFile, &IoStatus, (PVOID) &FileInformation, sizeof(FileInformation), FileStandardInformation);
        if ( !NT_SUCCESS(Status) ) {
            BaseSetLastNTError(Status);
            leave;
        }
        ...
        Status = NtQueryInformationFile(SourceFile, &IoStatus, (PVOID) &BasicInformation, sizeof(BasicInformation), FileBasicInformation);
        if ( !NT_SUCCESS(Status) ) {
            BaseSetLastNTError(Status);
            leave;
        }
        ...
        //  with a NULL buffer.  This is acceptable since we have at least a default data stream,
        StreamInfoSize = 4096;
        do {
            ...
            if ( !NT_SUCCESS(Status) ) {
                //  We failed the call.  Free up the previous buffer and set up for another pass with a buffer twice as large
        ...
        if (bIsNssFile && bOpenFilesAsReparsePoint && !NT_SUCCESS(Status))
        {
            ...
            //access them for some reason (most likely because there is more than 64k of stream data and the source is across the network), try to downgrade to docfile.
            if (SourceFile != INVALID_HANDLE_VALUE) {
                CloseHandle( SourceFile );
                SourceFile = INVALID_HANDLE_VALUE;
            }
            ...
            // first CreateFile attempt will succeed, because we've already set the correct SourceFileAccess/Sharing values.
            SourceFileFlagsAndAttributes &= ~FILE_FLAG_OPEN_REPARSE_POINT;

            goto retry_open_SourceFile;
        }

        //  If a progress routine or a restartable copy was requested, obtain the full size of the entire file, including its alternate data streams, etc.
        if ( ARGUMENT_PRESENT(lpProgressRoutine) || (dwCopyFlags & COPY_FILE_RESTARTABLE) ) {
            ...
            if ( StreamInfoBase != NULL ) {
                ...
                while (TRUE) {
                    // Account for the size of this stream in the overall size of the file.
        ...
        // Determine whether or not the copy operation should really be restartable based on the actual, total file size.
        if ( (dwCopyFlags & COPY_FILE_RESTARTABLE) && ( RestartState.FileSize.QuadPart < (512 * 1024) || (SourceFileAttributes & FILE_ATTRIBUTE_DIRECTORY) )) {
        ...
        if ( bOpenFilesAsReparsePoint && !b && !((GetLastError() == ERROR_FILE_EXISTS) && (dwCopyFlags & COPY_FILE_FAIL_IF_EXISTS)) ) {
            //  Clean up.
            ...
            if (SourceFileAttributes & FILE_ATTRIBUTE_READONLY) {
                ...
                //  Some servers (like Win9x) won't let us set file attributes on the handle we already have opened.
                //  SetFileAttributesW can do the job nicely, so we'll call that to make sure that the read-only bit isn't set.
                //  We had to close DestFile before doing this because it was possibly opened share-exclusive.
            ...
            //  Try again the copy operation without inhibiting the reparse behavior for the source.
            SourceFileFlagsAndAttributes &= ~FILE_FLAG_OPEN_REPARSE_POINT;
            bOpenFilesAsReparsePoint = FALSE;

            //  Go to re-open the source file without inhibiting the reparse point behavior and try the copy again.
            goto retry_open_SourceFile;
        }

        if ( b ) {
            // Attempt to determine whether or not this file has any alternate data streams associated with it.
            // If it does, attempt to copy each to the output file.
            // Note that the stream information may have already been obtained if a progress routine was requested.
            if ( StreamInfoBase != NULL ) {
                DWORD StreamCount = 0;
                ...
                while (TRUE) {
                    ...
                    //  it.  Alas, this code is NTFS specific and documented nowhere in the Io spec.
                    ...
                    // If we haven't already, verify that both the source and destination are really stream capable.
                    if( !CheckedForStreamCapable ) {
                        ...
                        Status = NtQueryVolumeInformationFile( DestFile, &IoStatus, &FileFsAttrInfoBuffer.Info, sizeof(FileFsAttrInfoBuffer), FileFsAttributeInformation );
                        if( NT_SUCCESS(Status) && (FileFsAttrInfoBuffer.Info.FileSystemAttributes & FILE_NAMED_STREAMS) ) {
                            ...
                            // SMB servers (SCO VisionFS) return success but don't really support streams.
                            Status = NtQueryVolumeInformationFile( SourceFile, &IoStatus, &FileFsAttrInfoBuffer.Info, sizeof(FileFsAttrInfoBuffer), FileFsAttributeInformation );
                        }
                        ...
                        if( !NT_SUCCESS(Status) || !(FileFsAttrInfoBuffer.Info.FileSystemAttributes & FILE_NAMED_STREAMS) ) {
                            ...
                            if( dwCopyFlags & PRIVCOPY_FILE_VALID_FLAGS ) {
                                if( !BasepCopyFileCallback( TRUE,    // Continue by default
                                                            ...
                                                            DestFile,
                                                            NULL )) {
                                    // LastError has been set, but we need it in Status for compatibility with the rest of this routine.
                                ...
                                // Ignore the named stream loss.
                                // We'll still try to copy the streams, though, since the target might be NT4 which didn't support the FILE_NAMED_STREAMS bit.
                                // But since IsStreamCapable is FALSE, if there's an error, we'll ignore it.
                                Status = STATUS_SUCCESS;
                            }
                        }
                        else {
                    ...
                    if ( b == TRUE || (b == SUCCESS_RETURNED_STATE && RestartState.CurrentStream == StreamCount) ) {
                        ...
                        Status = NtCreateFile(&StreamHandle, GENERIC_READ | SYNCHRONIZE, &ObjectAttributes, &IoStatus, NULL, 0, FILE_SHARE_READ, FILE_OPEN, FlagsAndAttributes, NULL, 0);

                        //If we got a share violation, try again with FILE_SHARE_WRITE.
                        if ( Status == STATUS_SHARING_VIOLATION ) {
                            ...
                            Status = NtCreateFile(&StreamHandle, GENERIC_READ | SYNCHRONIZE, &ObjectAttributes, &IoStatus, NULL, 0, dwShare, FILE_OPEN, FlagsAndAttributes, NULL, 0);
                        }
                        ...
                        if ( NT_SUCCESS(Status) ) {
                            ...
                            // For named streams, ignore the fail-if-exists flag.
                            // If the dest file already existed at the time the copy started, then we would have failed on the copy of the unnamed stream.
                            // So if a named stream exists, that means that it was created by some other process while we were copying the unnamed stream.
                            // The assumption is that such a stream should be overwritten (this scenario can occur with SFM).
                            ...
                            if ( OutputStream ) {
                                //  We set the last write time on all streams since there is a problem with RDR caching open handles and closing them out of order.
                                if ( b ) {
                                    Status = NtSetInformationFile(OutputStream, &IoStatus, &BasicInformation, sizeof(BasicInformation), FileBasicInformation);
                                }
                    ...
                    if ( !b ) {
                        // If the target is known to be capable of multi-stream files, then this is a fatal error.  Otherwise we'll ignore it.
                        if( IsStreamCapable ) {
        ...
        //  If the copy operation was successful, and it was restartable, and the output file was large enough that it was actually copied in a
        //  restartable manner, then copy the initial part of the file to its output.

        //  Restartability is accomplished by placing a restart header at the head of the default data stream.  When the copy is complete, we
            ...
            if ( b && (BytesRead == BytesToRead) ) {
                ...
                if ( b && (BytesRead == BytesWritten) ) {
                    if ( BytesRead < sizeof(RESTART_STATE) ) {
                        ...
                        Status = NtSetInformationFile(DestFile, &IoStatus, &EofInformation, sizeof(EofInformation), FileEndOfFileInformation);
                        if ( !NT_SUCCESS(Status) ) {
                            BaseMarkFileForDelete(DestFile,0);
                            b = FALSE;
                        }
        ...
        // If the copy operation was successful, set the last write time for the default steam so that it matches the input file.
        if ( b ) {
            Status = NtSetInformationFile(DestFile, &IoStatus, &BasicInformation, sizeof(BasicInformation), FileBasicInformation);
            if ( Status == STATUS_SHARING_VIOLATION ) {
                // IBM PC Lan Program (and other MS-NET servers) return STATUS_SHARING_VIOLATION if an application attempts to perform
                // an NtSetInformationFile on a file handle opened for GENERIC_READ or GENERIC_WRITE.
                ...
                DestFile = CreateFileW(lpNewFileName, FILE_WRITE_ATTRIBUTES, 0, NULL, OPEN_EXISTING, FlagsAndAttributes | FileFlagBackupSemantics, NULL);
                if ( DestFile != INVALID_HANDLE_VALUE ) {
                    //  If the open succeeded, we update the file information on the new file.

                    //  Note that we ignore any errors from this point on.
                    NtSetInformationFile(DestFile, &IoStatus, &BasicInformation, sizeof(BasicInformation), FileBasicInformation);
                }
            }
        }
    } finally {
...
BOOL CopyFileExW(LPCWSTR lpExistingFileName,
    LPCWSTR lpNewFileName,
    LPPROGRESS_ROUTINE lpProgressRoutine OPTIONAL,
    LPVOID lpData OPTIONAL,
    LPBOOL pbCancel OPTIONAL,
...
/*
Routine Description:
    A file, its extended attributes, alternate data streams, and any other attributes can be copied using CopyFileEx.
    CopyFileEx also provides callbacks and cancellability.
Arguments:
    lpExistingFileName - Supplies the name of an existing file that is to be copied.
    lpNewFileName - Supplies the name where a copy of the existing files data and attributes are to be stored.
    lpProgressRoutine - Optionally supplies the address of a callback routine to be called as the copy operation progresses.
    lpData - Optionally supplies a context to be passed to the progress callback routine.
    lpCancel - Optionally supplies the address of a boolean to be set to TRUE if the caller would like the copy to abort.
    dwCopyFlags - Specifies flags that modify how the file is to be copied:
        COPY_FILE_FAIL_IF_EXISTS - Indicates that the copy operation should fail immediately if the target file already exists.
        COPY_FILE_RESTARTABLE - Indicates that the file should be copied in restartable mode; i.e., progress of the copy should be tracked in
            the target file in case the copy fails for some reason.  It can then be restarted at a later date.
Return Value:
    TRUE - The operation was successful.
    FALSE/NULL - The operation failed. Extended error status is available using GetLastError.
*/
...
DWORD BasepChecksum(PUSHORT Source, ULONG Length)
/*++
Routine Description:
    Compute a partial checksum on a structure.
Arguments:
    Source - Supplies a pointer to the array of words for which the checksum is computed.
...
BOOL BasepRemoteFile(HANDLE SourceFile, HANDLE DestinationFile)
{
    ...
    Status = NtQueryVolumeInformationFile(SourceFile, &IoStatus, &DeviceInformation, sizeof(DeviceInformation), FileFsDeviceInformation);
    if ( NT_SUCCESS(Status) && (DeviceInformation.Characteristics & FILE_REMOTE_DEVICE) ) {
        return TRUE;
    }

    Status = NtQueryVolumeInformationFile(DestinationFile, &IoStatus, &DeviceInformation, sizeof(DeviceInformation), FileFsDeviceInformation);
    if ( NT_SUCCESS(Status) && DeviceInformation.Characteristics & FILE_REMOTE_DEVICE ) {
        return TRUE;
    }
...
{   // BasepRestartCopyFile
    ...
    try {
        // Note that setting the sequential scan flag is an optimization here that works because of the way that the Cache Manager on
        // the target works vis-a-vis unmapping segments of the file behind write operations.  This eventually allows the restart
        // section and the end of the file to both be mapped, which is the desired result.

        // Inhibit reparse behavior when appropriate.
        FlagsAndAttributes |= FILE_FLAG_SEQUENTIAL_SCAN;
        if ( OpenAsReparsePoint ) {
            // The target has to be opened as reparse point. If
            // this fails the source is to be closed and re-opened without inhibiting the reparse point behavior.
            FlagsAndAttributes |= FILE_FLAG_OPEN_REPARSE_POINT;
        }
        ...
        *DestFile = CreateFileW(
        ...
        //  The target file already exists, so determine whether a restartable copy was already proceeding.  If so,
        ...
        if ( !b || BytesRead != sizeof(RESTART_STATE) ) {
            // The file could not be read, or there were not enough bytes to contain a restart record.  In
            // either case, if the output file cannot be replaced, simply return an error now.
        ...
        // Check the contents of the restart state just read against the known contents of what would be there if this were the same copy operation.
        ...
        // the appropriate values into the internal restart state so the operation can continue from there.
        ...
        if ( !RestartState.CurrentStream ) {
            ...
            // We'll leave the handle in *DestFile, and the caller and pick up the copy of this stream.
            b = TRUE;
        } else {
...
/*++
Routine Description:
    This is an internal routine that copies the compression state during a copyfile.
    If the source is compressed, that same compression algorithm is copied to the dest.
    If that fails, an attempt is made to set the default compression.
    Depending on the copy flags, it may alternatively be necessary to decompress the destination.
Arguments:
    hSourceFile - Provides a handle to the source file.
    DestFile - Provides a handle to the destination file.
    SourceFileAttributes - FileBasicInformation attributes queried from the source file.
    DestFileAttributes - FileBasicInformation attributes for the current state of the destination file.
    DestFileFsAttributes - FileFsAttributeInformation.FileSystemAttributes for the file system of the dest file.
    CopyFlags - Provides flags that modify how the copy is to proceed.  See CopyFileEx for details.
    lpCopyFileContext - Provides a pointer to a pointer to the context information to track callbacks, file sizes, etc. across streams during the copy operation.
Return Value:
    TRUE - The operation was successful.
    FALSE/NULL - The operation failed. Extended error status is available using GetLastError.  The DestFile has already been marked for delete.
--*/
{   // BasepCopyCompression
    ...
    try
    {
        if( !(SourceFileAttributes & FILE_ATTRIBUTE_COMPRESSED) ) {
            // The source file is not compressed.  If necessary, decompress the target.
            if( (DestFileAttributes & FILE_ATTRIBUTE_COMPRESSED) && (CopyFlags & PRIVCOPY_FILE_SUPERSEDE) ) {
                // The source isn't compressed, but the dest is, and we don't want to acquire attributes from the dest.
                // So we need to manually decompress it.
            ...
            // Does the target filesystem even support compression?
            if( !(FILE_FILE_COMPRESSION & DestFileFsAttributes) ) {
                ...
                // Target volume supports compression.  Compress the target file if it's not already.
...
/*++
Routine Description:
    This is an internal routine that copies the encryption state during a copyfile.
    Depending on the copy flags, it may be necessary to decompress the destination.
    To encrypt/decrypt a file it is necessary to close the current handle, encrypt/decrypt, and reopen.
Arguments:
    hSourceFile - Provides a handle to the source file.
    ...
    SourceFileAttributes - FileBasicInformation attributes queried from the source file.
    SourceFileAttributesMask - the attributes from the source that are intended to be set on the dest.
    DestFileAttributes - FileBasicInformation attributes for the current state of the destination file.
        This value is updated to reflect changes to the encryption state of the dest file.
    DestFileFsAttributes - FileFsAttributeInformation.FileSystemAttributes for the file system of the dest file.
    CopyFlags - Provides flags that modify how the copy is to proceed.  See CopyFileEx for details.
    lpCopyFileContext - Provides a pointer to a pointer to the context information to track callbacks, file sizes, etc. across streams during the copy operation.
Return Value:
...
{   // BasepCopyEncryption
    ...
    try
    {
        if( (SourceFileAttributes & FILE_ATTRIBUTE_ENCRYPTED) && (SourceFileAttributesMask & FILE_ATTRIBUTE_ENCRYPTED) && !(*DestFileAttributes & FILE_ATTRIBUTE_ENCRYPTED) ) {
            // We tried to copy over encryption, but it didn't stick:
            // *  This may be a system file, encryption is not supported on system files.
            // *  If this is a non-directory file, then encryption is not supported on the target file system.
            // *  If this is a directory file, then we must try to encrypt
            //    it manually (since we opened it, rather than creating it).
            //    It still may not be possible but we'll have to try to find out.
            if( (SourceFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && !(*DestFileAttributes & FILE_ATTRIBUTE_SYSTEM) ) {
                EncryptFile = TRUE;
            }
        } else if( !(SourceFileAttributes & FILE_ATTRIBUTE_ENCRYPTED) && (*DestFileAttributes & FILE_ATTRIBUTE_ENCRYPTED) && (CopyFlags & PRIVCOPY_FILE_SUPERSEDE) ) {
            // The source is decrypted, the destination was encrypted, and the caller specified that the source should be copied as-is.
            // So we must manually decrypt the destination.
            // This can happen if the dest file already existed and was encrypted.
            DecryptFile = TRUE;
        }
        ...
        if( (SourceFileAttributes & FILE_ATTRIBUTE_ENCRYPTED) && !(*DestFileAttributes & FILE_ATTRIBUTE_ENCRYPTED) ) {
            ...
            // or the target just doesn't support encryption.  See if it's OK to continue with the copy.
            if( DestFileFsAttributes & FILE_SUPPORTS_ENCRYPTION ) {
                if( !(SourceFileAttributesMask & FILE_ATTRIBUTE_ENCRYPTED) ) {
                    // We opened the file with encryption turned off, so we must have gotten an access-denied on the first try.
...
/*++
Routine Description:
    This is an internal routine that copies an entire file (default data stream only), or a single stream of a file.
    If the hTargetFile parameter is present, then only a single stream of the output file is copied.
...
Arguments:
    ...
    hTargetFile - Optionally provides a handle to the target file.  If the stream being copied is an alternate data stream, then this handle must be provided.
    ...
    lpCopySize - Provides variable to store size of copy chunks to be used in copying the streams.
        This is set for the file, and then reused on alternate streams.

    lpCopyFileContext - Provides a pointer to a pointer to the context information to track callbacks, file sizes, etc. across streams during the copy operation.

    lpRestartState - Optionally provides storage to maintain restart state during the copy operation.
        This pointer is only valid if the caller has specified the COPY_FILE_RESTARTABLE flag in the lpCopyFlags word.

    OpenFileAsReparsePoint - Flag to indicate whether the target file is to be opened as a reparse point or not.

    DestFileFsAttributes - If hTargetFile is present, provides a location to store the destination file's filesystem attributes.
        If hTargetFile is not present, provides those attributes to this routine.

Return Value:
    TRUE - The operation was successful.
    SUCCESS_RETURNED_STATE - The operation was successful, but extended information was returned in the restart state structure.
    FALSE/NULL - The operation failed. Extended error status is available using GetLastError.
--*/
{   // BaseCopyStream
    HANDLE DestFile = INVALID_HANDLE_VALUE;
    ...
    DWORD CreateDisposition;
    BOOL Canceled = FALSE;
    BOOL fIsNssFile = (dwReparseTag == IO_REPARSE_TAG_NSS || dwReparseTag == IO_REPARSE_TAG_NSSRECOVER);
    DWORD SourceFileAttributes;
    DWORD SourceFileAttributesMask;
    BOOL fSparseSource;
    DWORD BlockSize;
    ...
    *lpCopySize = BASE_COPY_FILE_CHUNK;
    ...
    //  Get times and attributes for the file if the entire file is being copied
    Status = NtQueryInformationFile(hSourceFile, &IoStatus, (PVOID) &FileBasicInformationData, sizeof(FileBasicInformationData), FileBasicInformation);
    ...
    // For now, only maintain sparse information on NSS files that are not being downgraded.
    ...
    try {
        ...
        if ( !ARGUMENT_PRESENT(hTargetFile) ) {
            ...
            // We're being called to copy the unnamed stream of the file, and we need to create the file itself.
            ...
            // Begin by determining how the target file is to be opened based on whether or not the copy operation is to be restartable.
            if ( Restartable ) {
            ...
            if( DestFile == INVALID_HANDLE_VALUE ) {
                ...
                // contained files, so copying of a directory file should also have merge semantics.
                if( SourceFileAttributes & FILE_ATTRIBUTE_DIRECTORY )
                ...
                if( SourceFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) {
                    // We may or may not be able to get FILE_WRITE_DATA access, necessary for setting compression.
                ...
                EaBuffer = NULL;
                EaSize = 0;
                Status = NtQueryInformationFile(hSourceFile, &IoStatusBlock, &EaInfo, sizeof(EaInfo), FileEaInformation);
                if ( NT_SUCCESS(Status) && EaInfo.EaSize ) {
                    ...
                    do {
                        ...
                        Status = NtQueryEaFile(hSourceFile, &IoStatusBlock, EaBuffer, EaSize, FALSE, (PVOID)NULL, 0, (PULONG)NULL, TRUE);
                        if ( !NT_SUCCESS(Status) ) {
                ...
                // open as such, otherwise loop until we find a way to open it with NtCreateFile.
                ...
                while( DestFile == NULL || DestFile == INVALID_HANDLE_VALUE ) {
                    ...
                    if( NT_SUCCESS(Status) ) {
                        if ( fIsNssFile && !OpenFileAsReparsePoint && (CreateOptions & FILE_COPY_STRUCTURED_STORAGE) ) {
                            ...
                            nssc.param=0;
                            Status = NtFsControlFile(DestFile, NULL, NULL, NULL, &IoStatusBlock, FSCTL_NSS_RCONTROL, &nssc, sizeof(NSS_CONTROL), NULL, 0);
                            if ( !NT_SUCCESS(Status) ) {
                                CreateOptions &= ~FILE_COPY_STRUCTURED_STORAGE;
                        ...
                        else if( (SourceFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && CreateDisposition == FILE_OPEN_IF && lpConsoleName == NULL ) {
                            // Compatibility hack:  We successfully created the target, but some servers (SCO VisionFS) get confused by the FILE_OPEN_IF
                            // flag and create a non-directory file instead.  Check to see if this hapenned, and if so deleted it and re-create with FILE_CREATE
                            // instead.  This is a perf hit that we have to query the file attributes,
                            // but at least it is not a net round-trip because the rdr caches the file attributes in Create&X.
                            FILE_BASIC_INFORMATION NewDestInfo;
                            Status = NtQueryInformationFile( DestFile, &IoStatus, &NewDestInfo, sizeof(NewDestInfo), FileBasicInformation );
                            if( !NT_SUCCESS(Status) ) {
                                BaseMarkFileForDelete( DestFile, 0 );
                                BaseSetLastNTError(Status);
                                leave;
                            ...
                            if( !(NewDestInfo.FileAttributes & FILE_ATTRIBUTE_DIRECTORY) ) {
                                ...
                                // is a very VisionFS-specific workaround, we'll just turn off that bit
                    ...
                    // file already exists by that name, we need to manually delete it (FILE_OVERWRITE isn't valid for a directory file).
                    if( (*lpCopyFlags & PRIVCOPY_FILE_DIRECTORY) && Status == STATUS_NOT_A_DIRECTORY && !(*lpCopyFlags & COPY_FILE_FAIL_IF_EXISTS) ) {
                        Status = NtCreateFile(
                                    &DestFile,
                                    DELETE|SYNCHRONIZE,
                                    &Obja,
                    ...
                    // was denied, attempt to open the file and allow other readers and writers.
                    if( GetLastError() == ERROR_SHARING_VIOLATION || GetLastError() == ERROR_ACCESS_DENIED ) {
                        ...
                        if ( !NT_SUCCESS(Status) && (DestFileAccess & DELETE) && (Status == STATUS_SHARING_VIOLATION || Status == STATUS_ACCESS_DENIED ) ) {
                    ...
                    if(( GetLastError() == ERROR_ACCESS_DENIED  ) && (DestFileAccess & (WRITE_DAC | WRITE_OWNER)) ) {
                        ...
                        // if this still doesn't work, then the next iteration will turn WRITE_DAC back off, thus covering both scenarios.
                    ...
                    // point and we failed, try again without FILE_COPY_STRUCTURED_STORAGE
                    ...
                    // call the caller back and get permission to copy the data into an unencrypted file).
                    ...
                    // NT4 returns invalid-parameter error on an attempt to open a directory file with both FILE_WRITE_DATA and FILE_OPEN_IF.
                    // Samba 2.x returns ERROR_ALREADY_EXISTS, even though the semantics of FILE_OPEN_IF says that it should open the existing directory.
                ...
                if ( OpenFileAsReparsePoint && (DestFile != INVALID_HANDLE_VALUE)) {
                    ...
                    if ( !CopyResult ) {
                        ...
                        // will re-start the copy without inhibiting the reparse behavior.
                        ...
                        // further retries will fail.  Therefore we need to try to delete the new file here.
                        if (*lpCopyFlags & COPY_FILE_FAIL_IF_EXISTS)
                        {
                            FILE_DISPOSITION_INFORMATION Disposition = {TRUE};

                            Status = NtSetInformationFile(DestFile, &IoStatus, &Disposition, sizeof(Disposition), FileDispositionInformation);
                ...
                *DestFileFsAttributes = 0;
                ...
                if( *lpCopyFlags & PRIVCOPY_FILE_VALID_FLAGS ) {
                    ...
                    if( lpConsoleName == NULL ) {
                        Status = NtQueryInformationFile( DestFile, &IoStatus, &DestBasicInformation, sizeof(DestBasicInformation), FileBasicInformation );
                        if( NT_SUCCESS(Status) ) {
                    ...
                    Status = NtQueryVolumeInformationFile( hSourceFile, &IoStatus, &FileFsAttrInfoBuffer.Info, sizeof(FileFsAttrInfoBuffer), FileFsAttributeInformation );
                    if( NT_SUCCESS(Status) ) {
                ...
                if( (SourceFileFsAttributes & FILE_PERSISTENT_ACLS) && (*lpCopyFlags & (PRIVCOPY_FILE_METADATA | PRIVCOPY_FILE_SACL | PRIVCOPY_FILE_OWNER_GROUP)) ) {
                ...
                if( (*lpCopyFlags & PRIVCOPY_FILE_METADATA) ) {
                    ...
                    // uncompressed/encrypted file, we must decrypt the dest before attempting to compress it.
                    if( DestFileAttributes & FILE_ATTRIBUTE_COMPRESSED ) {
                    ...
                    for( i = 0; i < 2; i++ ) {
                        if( DoCompression ) {
                            DoCompression = FALSE;
                            b = BasepCopyCompression( hSourceFile, DestFile, SourceFileAttributes, DestFileAttributes, *DestFileFsAttributes, *lpCopyFlags, &Context );
                        } else {
                            DoCompression = TRUE;
                ...
                // If copying a directory file, handle the supersede case and attributes.
                if( SourceFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) {
                    // In the supersede case, the target's named streams should be removed.
                    if( *lpCopyFlags & PRIVCOPY_FILE_SUPERSEDE ) {
                        ...
                        do {
                            ...
                            Status = NtQueryInformationFile(DestFile, &IoStatus, (PVOID) StreamInfoBase, StreamInfoSize, FileStreamInformation);
                            if ( !NT_SUCCESS(Status) ) {
                    ...
                    if( SourceFileAttributes != DestFileAttributes ) {
                        ...
                        Status = NtSetInformationFile( DestFile, &IoStatus, &DestBasicInformation, sizeof(DestBasicInformation), FileBasicInformation );
                        if( !NT_SUCCESS(Status) ) {
                            BaseMarkFileForDelete( DestFile, 0 );
                            BaseSetLastNTError(Status);
                            leave;
            ...
            if ( OpenFileAsReparsePoint ) {
                ...
                FlagsAndAttributes |= FILE_OPEN_REPARSE_POINT;
                DesiredAccess |= GENERIC_READ;
                if ( !(*lpCopyFlags & COPY_FILE_FAIL_IF_EXISTS) || !(Restartable && (lpRestartState->LastKnownGoodOffset.QuadPart)) ) {
            ...
            if ( !NT_SUCCESS(Status) ) {
                ...
                if( Status == STATUS_OBJECT_NAME_INVALID && StreamName.Buffer[1] <= 0x1f && StreamName.Buffer[1] >= 1 ) {
                    ...
                    // the cause of the problem, and will silently fail the copy of this stream just as NT4 did.
                    NTSTATUS StatusT = STATUS_SUCCESS;
                    IO_STATUS_BLOCK Iosb;
                    FILE_FS_ATTRIBUTE_INFORMATION FsAttrInfo;

                    StatusT = NtQueryVolumeInformationFile( hTargetFile, &Iosb, &FsAttrInfo, sizeof(FsAttrInfo), FileFsAttributeInformation );
                    ...
                    // we don't need it (status_buffer_overflow is just a warning, so the rest of the data is good).
                ...
                // is a readonly file.  If so, change it to read/write, re-attempt the open, and set it back to readonly again.
                Status = NtQueryInformationFile(hTargetFile, &IoStatus, (PVOID) &FileBasicInformationData, sizeof(FileBasicInformationData), FileBasicInformation);
                if ( !NT_SUCCESS(Status) ) {
                    leave;
                }
                ...
                if ( FileBasicInformationData.FileAttributes & FILE_ATTRIBUTE_READONLY ) {
                    ...
                    (VOID) NtSetInformationFile(hTargetFile, &IoStatus, &FileBasicInformationData, sizeof(FileBasicInformationData), FileBasicInformation);
                    Status = NtCreateFile(
                                &DestFile,
                                DesiredAccess,
                                &ObjectAttributes,
            ...
            // reparse behavior inhibited. This is needed because of the incompatibility between FILE_OPEN_REPARSE_POINT and FILE_OVERWRITE_IF.
            if ( OpenFileAsReparsePoint ) {
               if ( !(*lpCopyFlags & COPY_FILE_FAIL_IF_EXISTS) ||
        ...
        // Adjust the notion of restartability and chunk size based on whether or not one of the files is remote.
        ...
        if (fSparseSource) {
            ...
            Status = NtFsControlFile(DestFile, NULL, NULL, NULL, &IoStatus, FSCTL_SET_SPARSE, NULL, 0, NULL, 0);

            // If we couldn't make the target sparse, see if it's OK to carry on.
            if( !NT_SUCCESS(Status) && (*lpCopyFlags & PRIVCOPY_FILE_VALID_FLAGS) ) {
                if( !BasepCopyFileCallback( TRUE,    // Continue (ignore the error) by default
                                            RtlNtStatusToDosError(Status),
                                            Context,
                                            NULL,
                                            (STATUS_INVALID_DEVICE_REQUEST != Status) ? PRIVCALLBACK_SPARSE_FAILED : PRIVCALLBACK_SPARSE_NOT_SUPPORTED,
                                            hSourceFile,
                                            DestFile,
                                            &Canceled )) {
                    if( Canceled ) {
        ...
        // Preallocate the size of this file/stream so that extends do not occur.
        if ( !(Restartable && lpRestartState->LastKnownGoodOffset.QuadPart) && lpFileSize->QuadPart) {
            EndOfFileInformation.EndOfFile = *lpFileSize;
            Status = NtSetInformationFile(DestFile, &IoStatus, &EndOfFileInformation, sizeof(EndOfFileInformation), FileEndOfFileInformation);
        ...
        // output file or alternate data stream has been created.  Note that a stream number of 1 means that the file itself has been created.
        ...
        if (!Restartable && !fSparseSource) {
            while (!lpFileSize->HighPart && (lpFileSize->LowPart < TWO56K)) {
                // If there's nothing to copy, then we're done (this happens when copying directory files, as there's no unnamed data stream).
                if( lpFileSize->LowPart == 0 ) {
                    ReturnValue = TRUE;
                    leave;
                }

                // Create a section and map the source file.  If anything fails, then drop into an I/O system copy mode.
                Status = NtCreateSection(&Section, SECTION_ALL_ACCESS, NULL, NULL, PAGE_READONLY, SEC_COMMIT, hSourceFile);
                if ( !NT_SUCCESS(Status) ) {
                    break;
                }
                ...
                Status = NtMapViewOfSection(Section, NtCurrentProcess(), &SourceBase, 0L, 0L, &SectionOffset, &BigViewSize, ViewShare, 0L, PAGE_READONLY);
                NtClose(Section);
                Section = NULL;
                if ( !NT_SUCCESS(Status) ) {
                    break;
                ...
                try {
                    while (BytesToWrite) {
                        ...
                        // If the caller has a progress routine, invoke it for this chunk's completion.
                        if ( Context ) {
                            if ( Context->lpProgressRoutine ) {
        ...
        if ( Restartable ) {
            ...
            // of the copy to the last known good offset that was written to the output file to continue the operation.
            SetFilePointer(hSourceFile, lpRestartState->LastKnownGoodOffset.LowPart, &lpRestartState->LastKnownGoodOffset.HighPart, FILE_BEGIN);
            SetFilePointer(DestFile, lpRestartState->LastKnownGoodOffset.LowPart, &lpRestartState->LastKnownGoodOffset.HighPart, FILE_BEGIN);
        ...
        do {
            ...
            if (fSparseSource)
            {
                ...
                Status = NtFsControlFile(hSourceFile, NULL, NULL, NULL, &IoStatus, FSCTL_QUERY_ALLOCATED_RANGES, &arbIn, sizeof(arbIn), &arbOut, sizeof(arbOut));
                if (NT_SUCCESS(Status) || (Status == STATUS_BUFFER_OVERFLOW))
                {
                    ...
                    liAllocatedEnd.QuadPart = arbOut.FileOffset.QuadPart + arbOut.Length.QuadPart - 1;

                    //Determine if the current block is in an allocated
                    //range.  If it is not, then skip the block.
#if DBG == 1
                    if ((liBlockStart.QuadPart > liAllocatedStart.QuadPart) && (arbOut.Length.QuadPart != 0))
                    {
                    ...
                    if (arbOut.Length.QuadPart == 0) {
                        ...
                        fSkipBlock = TRUE;
                    } else if ((liBlockStart.QuadPart < liAllocatedStart.QuadPart) && (liBlockEnd.QuadPart < liAllocatedStart.QuadPart)) {
                        //The entire block is zeroed, so skip it
                        fSkipBlock = TRUE;
                    }
                    else if (liBlockStart.QuadPart < liAllocatedStart.QuadPart) {
                        //Only the tail end of the block is valid.
                        //Skip forward to that and copy to the end of
                        //the block.
                        if ((SetFilePointer(hSourceFile, liAllocatedStart.LowPart, &(liAllocatedStart.HighPart), FILE_BEGIN) != 0xFFFFFFFF) &&
                            (SetFilePointer(DestFile, liAllocatedStart.LowPart, &(liAllocatedStart.HighPart), FILE_BEGIN) != 0xFFFFFFFF)) {
                            //BUGBUG:  What if one of those SetFilePointers
                            //fails?
                            BytesWritten = liAllocatedStart;
                            BlockSize = (DWORD)((liBlockEnd.QuadPart - liAllocatedStart.QuadPart));
                        }
                    }
                    else {
            ...
            {
                BytesWritten.QuadPart += ViewSize;
                if (( SetFilePointer(DestFile, BytesWritten.LowPart, &BytesWritten.HighPart, FILE_BEGIN) == 0xffffffff ) && ( GetLastError() != NO_ERROR ))
                {
                    b = FALSE;
                    break;
                }
            ...
            if ( Restartable && (((WriteCount & 3) == 0 && BytesWritten.QuadPart ) || BytesWritten.QuadPart == lpFileSize->QuadPart) ) {
                ...
                // this stream of the file has been completely copied, so update the restart state in the output file accordingly.
...
--*/
{
    ...
    return ( CreateFileW( Unicode->Buffer, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile));
}
...
Return Value:
    Not -1 - Returns an open handle to the specified file.  Subsequent access to the file is controlled by the DesiredAccess parameter.
...
--*/
{
    ...
    if ( ARGUMENT_PRESENT(hTemplateFile) ) {
        ...
        if ( NT_SUCCESS(Status) && EaInfo.EaSize ) {
            ...
            do {
                ...
                Status = NtQueryEaFile(hTemplateFile, &IoStatusBlock, EaBuffer, EaSize, FALSE, (PVOID)NULL, 0, (PULONG)NULL, TRUE);
                if ( !NT_SUCCESS(Status) ) {
                    RtlFreeHeap(RtlProcessHeap(), 0,EaBuffer);
                    EaBuffer = NULL;
                    IoStatusBlock.Information = 0;
    ...
    if ( !(dwFlagsAndAttributes & FILE_FLAG_BACKUP_SEMANTICS) ) {
        ...
        if ( (dwFlagsAndAttributes & FILE_ATTRIBUTE_DIRECTORY  ) && (dwFlagsAndAttributes & FILE_FLAG_POSIX_SEMANTICS ) && (CreateDisposition == FILE_CREATE) ) {
    ...
    return Handle;// Deal with hTemplateFile
}
...
HFILE WINAPI OpenFile(LPCSTR lpFileName, LPOFSTRUCT lpReOpenBuff, UINT uStyle)
{
    ...
    retry_open:
            ...
            {
                {
                    ...
                    HardErrorStatus = NtRaiseHardError(HardErrorStatus | HARDERROR_OVERRIDE_ERRORMODE, 1, 1, &ErrorParameter, OptionRetryCancel, &ErrorResponse);
        ...
        // The re-open buffer is completely filled in.
        // Now see if we are quitting (parsing), verifying, or just returning with the file opened.
        if (uStyle & OF_VERIFY)
        {
            if (OriginalReOpenBuff.Reserved1 == lpReOpenBuff->Reserved1 && OriginalReOpenBuff.Reserved2 == lpReOpenBuff->Reserved2 && !strcmp(OriginalReOpenBuff.szPathName, lpReOpenBuff->szPathName))
            {
                goto finally_exit;
            }
            else
...

{
    ...
    if( GetNamedSecurityInfoWPtr == NULL || SetNamedSecurityInfoWPtr == NULL ) {
    ...
    dwError = GetNamedSecurityInfoWPtr( lpExistingFileName, SE_FILE_OBJECT, SecurityInformation, &Owner, &Group, &Dacl, &Sacl, &SecurityDescriptor );
    ...
    while( TRUE && SecurityInformation != 0 ) {
        dwError = SetNamedSecurityInfoWPtr( lpNewFileName, SE_FILE_OBJECT, SecurityInformation, Owner, Group, Dacl, Sacl );
...
BOOL WINAPI ReplaceFileA(LPCSTR lpReplacedFileName, LPCSTR lpReplacementFileName, LPCSTR lpBackupFileName OPTIONAL, DWORD dwReplaceFlags, LPVOID lpExclude, LPVOID lpReserved)
/*++
Routine Description:
...
--*/
{
    ...
    if(NULL == lpReplacedFileName || NULL == lpReplacementFileName || NULL != lpExclude || NULL != lpReserved || dwReplaceFlags & ~(REPLACEFILE_WRITE_THROUGH | REPLACEFILE_IGNORE_MERGE_ERRORS)) {
        SetLastError(ERROR_INVALID_PARAMETER);
        return FALSE;
    }
    ...
    if(lpBackupFileName) {
        b = ReplaceFileW((LPCWSTR)DynamicUnicodeReplaced.Buffer, (LPCWSTR)DynamicUnicodeReplacement.Buffer, (LPCWSTR)DynamicUnicodeBackup.Buffer, dwReplaceFlags, lpExclude, lpReserved);
    } else {
        b = ReplaceFileW((LPCWSTR)DynamicUnicodeReplaced.Buffer, (LPCWSTR)DynamicUnicodeReplacement.Buffer, NULL, dwReplaceFlags, lpExclude, lpReserved);
    }
...
Error Code:
    ...
    ERROR_UNABLE_TO_REMOVE_REPLACED     The replacement file has inherited the replaced file's attributes and streams.
                                        the replaced file is unchanged. Both
                                        files still exist under their original
                                        names. No backup file exists.

    ERROR_UNABLE_TO_MOVE_REPLACEMENT    Same as above. Except that backup file exists if requested.

    ERROR_UNABLE_TO_MOVE_REPLACEMENT_2  The replacement file has inherited the replaced file's attributes and streams.
                                        It's still under its original name.
                                        Replaced file exists under the name of the backup file.
    ...
    All other error codes               Both replacement file and replaced file
                                        ...
                                        none of, or part of, or all of the replaced file's attributes and streams.
...
--*/
{
    ...
    try
    {
        ...
        InitializeObjectAttributes(&ReplacedObjAttr, &ReplacedFileNTName, OBJ_CASE_INSENSITIVE, ReplacedRelativeName.ContainingDirectory, NULL);
        ...
        if(!RtlDosPathNameToNtPathName_U(lpReplacementFileName, &ReplacementFileNTName, NULL, &ReplacementRelativeName)) {
            SetLastError(ERROR_PATH_NOT_FOUND);
            leave;
        }
        ...
        InitializeObjectAttributes(&ReplacementObjAttr, &ReplacementFileNTName, OBJ_CASE_INSENSITIVE, ReplacementRelativeName.ContainingDirectory, NULL);
        ...
        // but can't be set by NtSetInformationFile. Compression and encryption will be handled later.
        status = NtQueryInformationFile(ReplacedFile, &IoStatusBlock, &ReplacedBasicInfo, sizeof(ReplacedBasicInfo), FileBasicInformation);
        if(!NT_SUCCESS(status)) {
            ...
            status = NtQueryInformationFile(ReplacementFile, &IoStatusBlock, &ReplacementBasicInfo, sizeof(ReplacementBasicInfo), FileBasicInformation);
            if(!NT_SUCCESS(status)) {
                if(!(dwReplaceFlags & REPLACEFILE_IGNORE_MERGE_ERRORS)) {
                    BaseSetLastNTError(status);
                    leave;
            ...
            // Creation time is the only time we want to preserve. So zero out all the other times.
            ...
            status = NtSetInformationFile(ReplacementFile, &IoStatusBlock, &ReplacedBasicInfo, sizeof(ReplacedBasicInfo), FileBasicInformation);
        ...
        status = NtQueryVolumeInformationFile(ReplacementFile, &IoStatusBlock, &ReplacementFsAttrInfoBuffer.Info, sizeof(ReplacementFsAttrInfoBuffer), FileFsAttributeInformation);
        if(!NT_SUCCESS(status)) {
            if(!(dwReplaceFlags & REPLACEFILE_IGNORE_MERGE_ERRORS)) {
                BaseSetLastNTError(status);
                leave;
        ...
        // If the to-be-replaced file has alternate data streams, and they do not exist in the replacement file, copy them into the replacement file.
        cInfo = 4096;
        do {
            ...
            status = NtQueryInformationFile(ReplacedFile, &IoStatusBlock, ReplacedStreamInfo, cInfo, FileStreamInformation);
            if (!NT_SUCCESS(status)) {
        ...
        if(NULL == ReplacedStreamInfo) {
            ...
            if(!NT_SUCCESS(status)) {
                ...
                while(TRUE) {
                    ...
                    if(ScannerStreamInfoReplaced->StreamNameLength <= sizeof(WCHAR) || ScannerStreamInfoReplaced->StreamName[1] == ':') {
                    ...
                    // We wait until now to do this query because we don't want to do it unless it's absolutely necessary.
                    if(NULL == ReplacementStreamInfo) {
                        ...
                        do {
                            ...
                            status = NtQueryInformationFile(ReplacementFile, &IoStatusBlock, ReplacementStreamInfo, cInfo, FileStreamInformation);
                            if (!NT_SUCCESS(status)) {
        ...
        if(!fQueryReplacedFileFail) {
            ...
            if(fReplacedFileIsEncrypted && !fReplacementFileIsEncrypted) {
                ...
                status = NtOpenFile(&ReplacementFile,
                                    SYNCHRONIZE | GENERIC_READ | GENERIC_WRITE | WRITE_DAC | DELETE,
                                    &ReplacementObjAttr,
                                    &IoStatusBlock,
                                    0,
                                    FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT);
                if(STATUS_ACCESS_DENIED == status &&
                   dwReplaceFlags & REPLACEFILE_IGNORE_MERGE_ERRORS) {
                    status = NtOpenFile(&ReplacementFile,
                                        SYNCHRONIZE | GENERIC_READ | DELETE | WRITE_DAC,
                                        &ReplacementObjAttr,
                                        &IoStatusBlock,
                                        0,
                                        FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT);
        ...
        // Otherwise delete it.
        if(lpBackupFileName) {
            if(!RtlDosPathNameToNtPathName_U(lpBackupFileName, &BackupNTFileName, NULL, NULL)) {
                SetLastError(ERROR_PATH_NOT_FOUND);
                leave;
            }

            BackupReplaceRenameInfo = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TMP_TAG), BackupNTFileName.Length + sizeof(*BackupReplaceRenameInfo));
        ...
        ReplaceRenameInfo = RtlAllocateHeap(RtlProcessHeap(), MAKE_TAG(TMP_TAG), ReplacedFileNTName.Length + sizeof(*ReplaceRenameInfo));
...
/*++
Routine Description:
    This routine marks a file for delete, so that when the supplied handle is closed, the file will actually be deleted.
Arguments:
    File - Supplies a handle to the file that is to be marked for delete.
    FileAttributes - Attributes for the file, if known.  Zero indicates they are unknown.
--*/
{
#undef DeleteFile
    ...
    NtSetInformationFile(File, &IoStatus, &DispositionInformation, sizeof(DispositionInformation), FileDispositionInformation);
}