...
--*/
{
   ...
   try {
      ...
      // Look in CurrentDockInfo for a hardware determined DockingState value.
      ulSize = sizeof(ULONG);
      if ((RegQueryValueEx(hCurrentDockInfoKey, pszRegDockingState, NULL, NULL, (LPBYTE)&lpHwProfileInfo->dwDockInfo, &ulSize) != ERROR_SUCCESS)
          || (!lpHwProfileInfo->dwDockInfo)
          || ((lpHwProfileInfo->dwDockInfo & DOCKINFO_UNDOCKED) && (lpHwProfileInfo->dwDockInfo & DOCKINFO_DOCKED))) {
          // If there's no such value, or the value was set to 0 (unspported), or if the value is "unknown", resort to user supplied docking info.
          ...
          if ((RegQueryValueEx(hCfgKey, pszRegDockState, NULL, NULL, (LPBYTE)&lpHwProfileInfo->dwDockInfo, &ulSize) != ERROR_SUCCESS) || (!lpHwProfileInfo->dwDockInfo)) {
              // If there's no such value, or the value was set to 0, there is no user specified docking state to resort to;
      ...
      if (RegQueryValueEx(hCfgKey, pszRegHwProfileGuid, NULL, NULL, (LPBYTE)&lpHwProfileInfo->szHwProfileGuid, &ulSize) != ERROR_SUCCESS) {
      ...
      if (RegQueryValueEx(hCfgKey, pszRegFriendlyName, NULL, NULL, (LPBYTE)&lpHwProfileInfo->szHwProfileName, &ulSize) != ERROR_SUCCESS) {
...
--*/
{
   ...
   try {
      ...
      // on successful return, convert unicode form of struct to ANSI and copy struct members to callers struct
...
--*/
{
    ...
    // If every unicode character is a double-byte character, then the buffer needs to be the same size as the unicode string.
    // Otherwise it might be smaller, as some unicode characters will translate to single-byte characters.
    ...
    // If the realloc fails for some reason the original buffer is not freed.
...
--*/
{
    ...
    // Allocate maximally sized buffer.
    // If every character is a single-byte character, then the buffer needs to be twice the size as the 8bit string.
    // Otherwise it might be smaller, as some characters are 2 bytes in their unicode and 8bit representations.
...
RPC_STATUS GuidToString(UUID   *Uuid, LPTSTR StringGuid)
{
   ...
   if (Status == RPC_S_OK) {
      // the form we want is all uppercase and with curly brackets around, like what OLE does
