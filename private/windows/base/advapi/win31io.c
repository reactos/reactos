...
BOOL InitializeWin31State(IN WIN31IO_EVENT EventType, OUT PWIN31IO_STATE State)
{
    ...
    cch = GetWindowsDirectoryW(State->szWindowsDirectory, sizeof(State->szWindowsDirectory));
    ...
    *State->FileNamePart++ = OBJ_NAME_PATH_SEPARATOR;
    ...
    if (EventType == Win31SystemStartEvent) {
        ...
        if (OpenThreadToken(GetCurrentThread(), TOKEN_READ, TRUE, &TokenHandle) || OpenProcessToken(GetCurrentProcess(), TOKEN_READ, &TokenHandle)) {
            if (GetTokenInformation(TokenHandle, TokenUser, &State->TokenInformation, sizeof(State->TokenInformation), &ReturnLength)) {
    ...
    State->Win31IOKey = OpenCreateKey(EventType == Win31SystemStartEvent ? State->SoftwareRoot : State->UserRoot, L"Windows 3.1 Migration Status", TRUE);
...
)
{
    ...
    cch = GetPrivateProfileStringW(ApplicationName, KeyName, NULL, State->WorkBuffer, State->cchWorkBuffer, TempFileName);
    if (cch != 0) {
...
)
{
    ...
    if (GetFileAttributesW(State->WindowsPath) != 0xFFFFFFFF) {
        cchGroupNames = GetPrivateProfileStringW(L"Groups", NULL, L"", State->WorkBuffer, State->cchWorkBuffer, State->WindowsPath);
        if (cchGroupNames != 0) {
            Result = TRUE;
        }
    }
...
HANDLE OpenCreateKey(IN HANDLE Root, IN PWSTR Path, IN BOOL WriteAccess)
{
    ...
    Status = NtOpenKey(&Key,
                       WriteAccess ? (STANDARD_RIGHTS_WRITE | KEY_QUERY_VALUE | KEY_ENUMERATE_SUB_KEYS | KEY_SET_VALUE | KEY_CREATE_SUB_KEY) : GENERIC_READ,
                       &ObjectAttributes
    );
    if (NT_SUCCESS(Status)) {
        return Key;
