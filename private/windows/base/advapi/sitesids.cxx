...
GetSiteNameFromSid(PSID pSid, LPWSTR *pwsSite)
{
    ...
    *pwsSite = NULL;
    ...
    error = RegCreateKeyExW(HKEY_LOCAL_MACHINE, SITE_SID_CACHE_REG_KEY, 0, NULL, 0, KEY_ALL_ACCESS, NULL, &hCacheKey, &dwDisposition);
    if (ERROR_SUCCESS != error)
    {
        // Can't write, try read-only.  We won't be able to update the cache,
        // but we might be able to return the site name

        error = RegCreateKeyExW(HKEY_LOCAL_MACHINE, SITE_SID_CACHE_REG_KEY, 0, NULL, 0, KEY_QUERY_VALUE, NULL, &hCacheKey, &dwDisposition);
        if (ERROR_SUCCESS != error) {
...
GetSiteSidFromUrl(LPCWSTR wsUrl)
{
    ...
    hCacheKey = NULL;
    if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, SITE_SID_CACHE_REG_KEY, 0, NULL, 0, KEY_ALL_ACCESS, NULL, &hCacheKey, &dwDisposition) != ERROR_SUCCESS)
    {
...
*/
{
  ...
  if (pUser == NULL) {
      //        DebugMsg((DM_WARNING, TEXT("GetUserSid: Failed to allocate %d bytes"), BytesRequired));
      return NULL;
  }
  ...
  if (!NT_SUCCESS(status)) {
      //        DebugMsg((DM_WARNING, TEXT("GetUserSid: Failed to query user info from user token, status = 0x%x"), status));
      LocalFree(pUser);
      return NULL;
  }
  ...
  LocalFree(pUser);
  if (!NT_SUCCESS(status)) {
      //        DebugMsg((DM_WARNING, TEXT("GetUserSid: RtlCopySid Failed. status = %d"), status));
      LocalFree(pSid);
      pSid = NULL;
  }
...
Return Value:
    ...
    FALSE if it isn't, or if the handle of the current process cannot be obtained
--*/
{
...
WINADVAPI BOOL WINAPI IsInSandbox(VOID)
{
...
STDAPI CoInternetCreateSecurityManager(IN  IServiceProvider *pSP, OUT IInternetSecurityManager **ppSM, IN DWORD dwReserved)
{
    HRESULT hr = E_FAIL;
