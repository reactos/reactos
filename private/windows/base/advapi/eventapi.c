...
--*/
{
    ...
    // Convert the array of strings to an array of PUNICODE_STRINGs before calling ElfReportEventW.
    pUStrings = RtlAllocateHeap(RtlProcessHeap(), 0, wNumStrings * sizeof(PUNICODE_STRING));
    if (pUStrings) {
        // Guard the memory allocation above while we peruse the user's buffer. If not, we'd leak it on an exception.
        try {
...
--*/
{
    ...
    if (pAStrings) {
        ...
        try {
            // For each string passed in, allocate an ANSI_STRING structure and fill it in with the string.
            for (AllocatedStrings = 0; AllocatedStrings < wNumStrings;
              AllocatedStrings++) {
