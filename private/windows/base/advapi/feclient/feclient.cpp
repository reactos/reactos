...
--*/
{
    ...
    if (DirPath) {
        ...
        IniFilePath = (LPWSTR)RtlAllocateHeap(RtlProcessHeap(), 0, (wcslen(DirPath)+1+wcslen(INIFILENAME))*sizeof(WCHAR));
        if (IniFilePath) {
            if (Disable) {
...
--*/
{
    ...
    ValueLength = GetPrivateProfileString(SectionName, KeyName, L"0", ResultString, sizeof(ResultString), IniFileName);
...
BOOL DirEfsDisabled(IN LPCWSTR  DirName)
/*++
Routine Description:
    ...
    if (Result) {
        ...
        if ( (FileAttributes & FILE_ATTRIBUTE_ENCRYPTED) ||
            ...
            if ( FullFileNameU.Length >= MAX_PATH * sizeof(WCHAR)){
                ...
                FullPathName = (LPWSTR)RtlAllocateHeap(RtlProcessHeap(), 0, FullPathLen);
                TmpBuffer = (LPWSTR)RtlAllocateHeap(RtlProcessHeap(), 0, TmpBuffLen);
                            ...
                            *lpStatus = FILE_UNKNOWN ;
                            ...
                            if ( ( FullFileNameU.Length < PathLength * sizeof (WCHAR) ) ||
                                ...
                                if ( _wcsnicmp(TmpBuffer, FullFileNameU.Buffer, PathLength)){
                                    ...
                                    if (!bRet){
                                        RtlFreeHeap(RtlProcessHeap(), 0, TmpBuffer);
                                        TmpBuffer = (LPWSTR)RtlAllocateHeap(RtlProcessHeap(), 0, allowPathLen);
                                        if (TmpBuffer){
                                            bRet = GetProfilesDirectory(TmpBuffer, &allowPathLen);
                                        } else {
                                            *lpStatus = ERROR_OUTOFMEMORY;
