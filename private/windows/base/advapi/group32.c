    ...
    )
{
    ...
    for (NumberOfPersonalGroups=1; ; NumberOfPersonalGroups++) {
        ...
        Status = NtQueryValueKey( *GroupNamesKey, &ValueName, KeyValuePartialInformation, (PVOID)&KeyValueInformation, sizeof( KeyValueInformation ), &ResultLength);
...
BOOL DoesExistGroup(HANDLE GroupsKey, PWSTR GroupName)
{
    ...
    Status = NtQueryValueKey( Key, &ValueName, KeyValueFullInformation, &KeyValueInformation, sizeof( KeyValueInformation ), &ResultLength);
    NtClose( Key );
    ...
    )
{
    ...
    Status = NtQueryValueKey( Key, &ValueName, KeyValueFullInformation, &KeyValueInformation, KeyValueInformation.DataLength, &ResultLength);
    NtClose( Key );
    if (!NT_SUCCESS( Status )) {
        VirtualFree( Base, 0, MEM_RELEASE );
        return NULL;
