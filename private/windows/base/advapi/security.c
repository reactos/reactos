...
BOOL APIENTRY DuplicateToken(HANDLE ExistingTokenHandle, SECURITY_IMPERSONATION_LEVEL ImpersonationLevel, PHANDLE DuplicateTokenHandle)
/*++
Routine Description:
    Create a new token that is a duplicate of an existing token.
    The new token will be an impersonation token of the supplied level.
Arguments:
...
BOOL APIENTRY AccessCheck(PSECURITY_DESCRIPTOR pSecurityDescriptor,
                          HANDLE ClientToken,
                          DWORD DesiredAccess,
...
/*++
Routine Description:
    This routine compares the input Security Descriptor against the input token and indicates by its return value if access is granted
    or denied.  If access is granted then the desired access mask becomes the granted access mask for the object.

    The semantics of the access check routine is described in the DSA
    Security Architecture workbook.  Note that during an access check only the discretionary ACL is examined.
...
Arguments:
    ...
    AccessStatus - Status value that may be returned indicating the reason why access was denied.  Routines should avoid hardcoding a
         return value of STATUS_ACCESS_DENIED so that a different value can be returned when mandatory access control is implemented.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
BOOL APIENTRY AccessCheckByType(PSECURITY_DESCRIPTOR pSecurityDescriptor,
...
Arguments:
    ...
    ObjectTypeList - Supplies a list of GUIDs representing the object (and sub-objects) being accessed.  If no list is present, AccessCheckByType
...
BOOL APIENTRY AccessCheckByTypeResultList(PSECURITY_DESCRIPTOR pSecurityDescriptor,
                                          PSID PrincipalSelfSid,
                                          HANDLE ClientToken,
                                          DWORD DesiredAccess,
                                          POBJECT_TYPE_LIST ObjectTypeList,
...
Arguments:
    ...
    AccessStatusList - Status value that may be returned indicating the reason why access was denied.  Routines should avoid hardcoding a
         return value of STATUS_ACCESS_DENIED so that a different value can be returned when mandatory access control is implemented.
...
Arguments:
    ...
    DesiredAccess - Is an access mask indicating which access types are desired to the token.  These access types are reconciled
...
Arguments:
    ...
    OpenAsSelf - Is a boolean value indicating whether the access should be made using the calling thread's current security context, which
        ...
        caller's current context should be used un-modified.  A value of TRUE indicates the request should be fulfilled using the process
...
BOOL APIENTRY GetTokenInformation(HANDLE TokenHandle,
...
Arguments:
    ...
    ReturnLength - This parameter receives the actual length of the requested information.  If this value is larger than that
        provided by the TokenInformationLength parameter, then the buffer provided to receive the requested information is not
        large enough to hold that data and no data is returned.

        If the queried class is TokenDefaultDacl and there is no default Dacl established for the token, then the return
...
BOOL APIENTRY SetTokenInformation(HANDLE TokenHandle,
                                  TOKEN_INFORMATION_CLASS TokenInformationClass,
                                  PVOID TokenInformation,
                                  DWORD TokenInformationLength)
/*++
...
BOOL APIENTRY AdjustTokenPrivileges(HANDLE TokenHandle,
...
Arguments:
    ...
    ReturnLength - Indicates the actual number of bytes needed to contain the previous privilege state information.
        This parameter is ignored if the PreviousState argument is not passed.
...
BOOL APIENTRY AdjustTokenGroups(HANDLE TokenHandle,
                                BOOL ResetToDefault,
...
BOOL APIENTRY AccessCheckAndAuditAlarmW(LPCWSTR SubsystemName,
                                        PVOID HandleId,
...
BOOL APIENTRY AccessCheckByTypeAndAuditAlarmW(LPCWSTR SubsystemName,
                                              LPVOID HandleId,
...
Routine Description:
    ...
    denied.  If access is granted then the desired access mask becomes
    the granted access mask for the object.  The semantics of the
    access check routine is described in the DSA Security Architecture workbook.
...
Arguments:
    ...
    pfGenerateOnClose - Points to a boolean that is set by the audity
        generation routine and must be passed to ObjectCloseAuditAlarm when the object handle is closed.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
BOOL APIENTRY AccessCheckByTypeResultListAndAuditAlarmW(LPCWSTR SubsystemName,
                                                        LPVOID HandleId,
                                                        LPCWSTR ObjectTypeName,
                                                        LPCWSTR ObjectName,
                                                        PSECURITY_DESCRIPTOR SecurityDescriptor,
...
BOOL APIENTRY AccessCheckByTypeResultListAndAuditAlarmByHandleW(LPCWSTR SubsystemName,
                                                                LPVOID HandleId,
                                                                HANDLE ClientToken,
                                                                LPCWSTR ObjectTypeName,
                                                                LPCWSTR ObjectName,
...
BOOL APIENTRY AccessCheckAndAuditAlarmA(LPCSTR SubsystemName,
                                        PVOID HandleId,
...
BOOL APIENTRY AccessCheckByTypeAndAuditAlarmA(LPCSTR SubsystemName,
                                              PVOID HandleId,
...
WINADVAPI BOOL WINAPI AccessCheckByTypeResultListAndAuditAlarmA(LPCSTR SubsystemName,
                                                                LPVOID HandleId,
                                                                LPCSTR ObjectTypeName,
                                                                LPCSTR ObjectName,
                                                                PSECURITY_DESCRIPTOR SecurityDescriptor,
...
WINADVAPI BOOL WINAPI AccessCheckByTypeResultListAndAuditAlarmByHandleA(LPCSTR SubsystemName,
                                                                        LPVOID HandleId,
                                                                        HANDLE ClientToken,
                                                                        LPCSTR ObjectTypeName,
                                                                        LPCSTR ObjectName,
...
BOOL APIENTRY ObjectOpenAuditAlarmA(LPCSTR SubsystemName,
                                    PVOID HandleId,
                                    LPSTR ObjectTypeName,
                                    LPSTR ObjectName,
...
BOOL APIENTRY ObjectOpenAuditAlarmW(LPCWSTR SubsystemName,
                                    PVOID HandleId,
                                    LPWSTR ObjectTypeName,
                                    LPWSTR ObjectName,
                                    PSECURITY_DESCRIPTOR pSecurityDescriptor,
...
/*++
    Routine Description:
    This routine is used to generate audit and alarm messages when an attempt is made to access an existing protected subsystem object or
    create a new one.  This routine may result in several messages being generated and sent to Port objects.  This may result in a significant
    latency before returning.  Design of routines that must call this routine must take this potential latency into account.  This may have
    an impact on the approach taken for data structure mutex locking, for example.
...
Arguments:
    ...
    HandleId - A unique value representing the client's handle to the
        object.  If the access attempt was not successful (AccessGranted is FALSE), then this parameter is ignored.
    ...
    ClientToken - A handle to a token object representing the client that
        requested the operation.  This handle must be obtained from a
        communication session layer, such as from an LPC Port or Local Named Pipe, to prevent possible security policy violations.
    ...
    Privileges - Optionally points to a set of privileges that were 
        ...
        by the subject are marked using the UsedForAccess flag of the attributes associated with each privilege.

    ObjectCreation - A boolean flag indicating whether the access will
        result in a new object being created if granted.  A value of TRUE
        indicates an object will be created, FALSE indicates an existing object will be opened.

    AccessGranted - Indicates whether the requested access was granted or
        not.  A value of TRUE indicates the access was granted.  A value of FALSE indicates the access was not granted.
    GenerateOnClose - Points to a boolean that is set by the audit
        generation routine and must be passed to NtCloseObjectAuditAlarm() when the object handle is closed.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
BOOL APIENTRY ObjectPrivilegeAuditAlarmA(LPCSTR SubsystemName,
                                         PVOID HandleId,
                                         HANDLE ClientToken,
                                         DWORD DesiredAccess,
...
BOOL APIENTRY ObjectPrivilegeAuditAlarmW(LPCWSTR SubsystemName,
                                         PVOID HandleId,
                                         HANDLE ClientToken,
                                         DWORD DesiredAccess,
                                         PPRIVILEGE_SET Privileges,
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when an attempt is made to perform privileged operations on a protected
    subsystem object after the object is already opened.  This routine may result in several messages being generated and sent to Port
    objects.  This may result in a significant latency before returning.  Design of routines that must call this routine must
    take this potential latency into account.  This may have an impact on the approach taken for data structure mutex locking, for example.

    This API requires the caller have SeSecurityPrivilege privilege.
    The test for this privilege is always against the primary token of the calling process, allowing the caller to be impersonating a
...
Routine Description:
    ...
    Design of routines that must call this routine must take this potential latency into
    account.  This may have an impact on the approach taken for data structure mutex locking, for example.

    This API requires the caller have SeSecurityPrivilege privilege.
    The test for this privilege is always against the primary token of the calling
    process, allowing the caller to be impersonating a client during the call with no ill effects.
...
Arguments:
    ...
    GenerateOnClose - Is a boolean value returned from a corresponding
        AccessCheckAndAuditAlarm() call or ObjectOpenAuditAlarm() call when the object handle was created.
Return value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
/*++
Routine Description:
    This routine is used to generate audit and alarm messages when an object in a protected subsystem is deleted. 
    This routine may result in several messages being generated and sent to Port objects.  
    This may result in a significant latency before returning.  
...
BOOL APIENTRY PrivilegedServiceAuditAlarmA(LPCSTR SubsystemName, LPCSTR ServiceName, HANDLE ClientToken, PPRIVILEGE_SET Privileges, BOOL AccessGranted)
/*++
Routine Description:
...
BOOL APIENTRY PrivilegedServiceAuditAlarmW(LPCWSTR SubsystemName, LPCWSTR ServiceName, HANDLE ClientToken, PPRIVILEGE_SET Privileges, BOOL AccessGranted)
/*++
Routine Description:
...
Arguments:
    ...
    Privileges - Points to a set of privileges required to perform the
        ...
        subject are marked using the UsedForAccess flag of the attributes associated with each privilege.
...
/*++
Routine Description:
    This function initializes an SID data structure.
    It does not, however, set the sub-authority values.
...
PVOID APIENTRY FreeSid(PSID pSid)
/*++
Routine Description:
...
BOOL APIENTRY AllocateAndInitializeSid(PSID_IDENTIFIER_AUTHORITY pIdentifierAuthority,
...
/*++
Routine Description:
    This function allocates and initializes a sid with the specified number of sub-authorities (up to 8). 
    A sid allocated with this routine must be freed using FreeSid().
Arguments:
    ...
    nSubAuthorityCount - The number of sub-authorities to place in the SID.
        ...
        have meaningful values.  This must contain a value from 0 through 8.
...
Arguments:
    ...
    nSubAuthority - An index indicating which sub-authority is being
        specified.  This value is not compared against the number of sub-authorities in the SID for validity.
Return Value:
...
DWORD APIENTRY GetLengthSid(PSID pSid)
/*++
Routine Description:
...
Return Value:
    BOOL - TRUE if the GrantedAccess mask has all the bits set that
        the DesiredAccess mask has set.  That is, TRUE is returned if all of the desired accesses have been granted.
--*/
{
...
Routine Description:
    ...
    set.  If bits other than the generic bits are provided on input, they will not be cleared bt the mapping.
Arguments:
...
Routine Description:
    ...
    that the number of ACEs specified in the AceCount fit in the space specified by the AclSize field of the ACL header.
Arguments:
...
/*++
Routine Description:
    InitializeAcl creates a new ACL in the caller supplied memory buffer.  
    The ACL contains zero ACEs; therefore, it is an empty ACL as opposed to a nonexistent ACL.
    That is, if the ACL is now set to an object it will implicitly deny access to everyone.
Arguments:
...
BOOL APIENTRY GetAclInformation(PACL pAcl, PVOID pAclInformation, DWORD nAclInformationLength, ACL_INFORMATION_CLASS dwAclInformationClass)
/*++
Routine Description:
...
BOOL APIENTRY SetAclInformation(PACL pAcl, PVOID pAclInformation, DWORD nAclInformationLength, ACL_INFORMATION_CLASS dwAclInformationClass)
/*++
Routine Description:
...
Arguments:
    ...
    dwStartingAceIndex - Supplies the ACE index which will be the index of the first ace inserted in the acl.  0 for the
...
Routine Description:
    ...
    A very bland ACE header is placed in the ACE.  The AceFlags and inheritance are specified by the AceFlags parameter.
Arguments:
    PAcl - Supplies the Acl being modified
    dwAceRevision - Supplies the Acl/Ace revision of the ACE being added
...
BOOL APIENTRY AddAuditAccessAce(PACL pAcl, DWORD dwAceRevision, DWORD dwAccessMask, PSID pSid, BOOL bAuditSuccess, BOOL bAuditFailure)
/*++
Routine Description:
...
BOOL APIENTRY AddAuditAccessAceEx(PACL pAcl,
                                  DWORD dwAceRevision,
                                  DWORD AceFlags,
                                  DWORD dwAccessMask,
                                  PSID pSid,
...
--*/
{
    ...
    Status = RtlAddAuditAccessAceEx(pAcl, dwAceRevision, AceFlags, dwAccessMask, pSid, (BOOLEAN)bAuditSuccess, (BOOLEAN)bAuditFailure);
    if (!NT_SUCCESS(Status)) {
        if (Status == STATUS_INVALID_PARAMETER) {
            SetLastError(ERROR_INVALID_FLAGS);
        } else {
...
BOOL APIENTRY AddAccessAllowedObjectAce(PACL pAcl,
                                        DWORD dwAceRevision,
                                        DWORD AceFlags,
                                        DWORD AccessMask,
                                        GUID *ObjectTypeGuid,
...
Arguments:
    ...
    InheritedObjectTypeGuid - Supplies the GUID of the object type that will
        inherit this ACE.  If NULL, no inherited object type GUID is placed in the ACE.
    pSid - Pointer to the SID being granted access.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
    ...
    Status = RtlAddAccessAllowedObjectAce(pAcl, dwAceRevision, AceFlags, AccessMask, ObjectTypeGuid, InheritedObjectTypeGuid, pSid);
    if (!NT_SUCCESS(Status)) {
        if (Status == STATUS_INVALID_PARAMETER) {
            SetLastError(ERROR_INVALID_FLAGS);
        } else {
...
BOOL APIENTRY AddAccessDeniedObjectAce(PACL pAcl,
                                       DWORD dwAceRevision,
                                       DWORD AceFlags,
                                       DWORD AccessMask,
                                       GUID *ObjectTypeGuid,
...
--*/
{
    ...
    Status = RtlAddAccessDeniedObjectAce(pAcl, dwAceRevision, AceFlags, AccessMask, ObjectTypeGuid, InheritedObjectTypeGuid, pSid);
    if (!NT_SUCCESS(Status)) {
        if (Status == STATUS_INVALID_PARAMETER) {
            SetLastError(ERROR_INVALID_FLAGS);
        } else {
...
BOOL APIENTRY AddAuditAccessObjectAce(PACL pAcl,
                                      DWORD dwAceRevision,
                                      DWORD AceFlags,
                                      DWORD dwAccessMask,
                                      GUID *ObjectTypeGuid,
...
Routine Description:
    ...
    return pointer is to the byte immediately following the acl, and TRUE will be returned.
Arguments:
...
Routine Description:
    ...
    system ACL, no discretionary ACL, no owner, no primary group and all control flags set to false (null).
Arguments:
...
DWORD APIENTRY GetSecurityDescriptorLength(PSECURITY_DESCRIPTOR pSecurityDescriptor)
/*++
Routine Description:
    This routine returns the length, in bytes, necessary to capture a structurally valid SECURITY_DESCRIPTOR.  The length includes the length
    of all associated data structures (like SIDs and ACLs).  The length also takes into account the alignment requirements of each component.
...
Arguments:
    pSecurityDescriptor - Points to the SECURITY_DESCRIPTOR whose
        length is to be returned.  The SECURITY_DESCRIPTOR's structure is assumed to be valid.
Return Value:
...
BOOL APIENTRY GetSecurityDescriptorControl(PSECURITY_DESCRIPTOR pSecurityDescriptor, PSECURITY_DESCRIPTOR_CONTROL pControl, LPDWORD lpdwRevision)
/*++
Routine Description:
...
Arguments:
    ...
    lpdwRevision - Receives the revision of the security descriptor.
        This value will always be returned, even if an error is returned by this routine.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
Routine Description:
    ...
    marks the DACL on the security descriptor as protected. And
        ...
        SetSecurityDescriptorControl( &SecDesc, SE_DACL_PROTECTED, 0 );
...
Arguments:
    ...
    ControlBitsOfInterest - A mask of the control bits being changed, set,
        or reset by this call.  The mask is the logical OR of one or more of the following flags:
            SE_DACL_UNTRUSTED
            SE_SERVER_SECURITY
    ...
    ControlBitsToSet - A mask indicating what the bits specified by ControlBitsOfInterest should be set to.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
BOOL APIENTRY SetSecurityDescriptorDacl(PSECURITY_DESCRIPTOR pSecurityDescriptor,
                                        BOOL bDaclPresent,
                                        PACL pDacl OPTIONAL,
                                        BOOL bDaclDefaulted OPTIONAL)
/*++
Routine Description:
    This procedure sets the discretionary ACL information of an absolute format security descriptor.  
    If there is already a discretionary ACL present in the security descriptor, it is superseded.
Arguments:
    pSecurityDescriptor - Supplies the security descriptor to be which the discretionary ACL is to be added.
    bDaclPresent - If FALSE, indicates the DaclPresent flag in the security descriptor should be set to FALSE.  
        In this case, the remaining optional parameters are ignored.  
        Otherwise, the DaclPresent control flag in the security descriptor is set to
    ...
    pDacl - Supplies the discretionary ACL for the security
        ...
        null ACL is assigned to the security descriptor. 
        A null discretionary ACL unconditionally grants access.
        The ACL is referenced by, not copied into, by the security descriptor.

    bDaclDefaulted - When set, indicates the discretionary ACL was picked up from some default mechanism (rather than explicitly
        specified by a user).  This value is set in the DaclDefaulted control flag in the security descriptor.  
        If this optional parameter is not passed, then the DaclDefaulted flag will be cleared.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
BOOL APIENTRY GetSecurityDescriptorDacl(PSECURITY_DESCRIPTOR pSecurityDescriptor,
...
Arguments:
    pSecurityDescriptor - Supplies the security descriptor.
    lpbDaclPresent - If TRUE, indicates that the security descriptor does contain a discretionary ACL.
        In this case, the remaining OUT parameters will receive valid values.
        Otherwise, the security descriptor does not contain a discretionary ACL and the remaining OUT parameters will not receive valid values.
    pDacl - This value is returned only if the value returned for the DaclPresent flag is TRUE.  
        In this case, the Dacl parameter receives the address of the security descriptor's discretionary ACL.  
        If this value is returned as null, then the security descriptor has a null discretionary ACL.
    lpbDaclDefaulted - This value is returned only if the value returned for the DaclPresent flag is TRUE.  
        In this case, the DaclDefaulted parameter receives the value of the security descriptor's DaclDefaulted control flag.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
BOOL APIENTRY SetSecurityDescriptorSacl(PSECURITY_DESCRIPTOR pSecurityDescriptor, BOOL bSaclPresent, PACL pSacl OPTIONAL, BOOL bSaclDefaulted)
/*++
Routine Description:
    This procedure sets the system ACL information of an absolute security
    descriptor.  If there is already a system ACL present in the security descriptor, it is superseded.
Arguments:
    pSecurityDescriptor - Supplies the security descriptor to be which the system ACL is to be added.
    bSaclPresent - If FALSE, indicates the SaclPresent flag in the security descriptor should be set to FALSE.  
        In this case, the remaining optional parameters are ignored. 
        Otherwise, the SaclPresent control flag in the security descriptor is
        set to TRUE and the remaining optional parameters are not ignored.
    pSacl - Supplies the system ACL for the security descriptor.  If
        this optional parameter is not passed, then a null ACL is
        assigned to the security descriptor.  The ACL is referenced by, not copied into, by the security descriptor.
    bSaclDefaulted - When set, indicates the system ACL was picked up from some default mechanism (rather than explicitly specified
        by a user).  This value is set in the SaclDefaulted control flag in the security descriptor.  If this optional parameter
...
BOOL APIENTRY GetSecurityDescriptorSacl(PSECURITY_DESCRIPTOR pSecurityDescriptor, LPBOOL lpbSaclPresent, PACL *pSacl, LPBOOL lpbSaclDefaulted)
/*++
Routine Description:
...
Arguments:
    ...
    lpbSaclPresent - If TRUE, indicates that the security descriptor
        does contain a system ACL.  In this case, the remaining OUT parameters will receive valid values.  Otherwise, the
        security descriptor does not contain a system ACL and the remaining OUT parameters will not receive valid values.

    pSacl - This value is returned only if the value returned for the
        SaclPresent flag is TRUE.  In this case, the Sacl parameter receives the address of the security descriptor's system ACL.
        If this value is returned as null, then the security descriptor has a null system ACL.

    lpbSaclDefaulted - This value is returned only if the value
        returned for the SaclPresent flag is TRUE.  In this case, the
        SaclDefaulted parameter receives the value of the security descriptor's SaclDefaulted control flag.
...
BOOL APIENTRY SetSecurityDescriptorOwner(PSECURITY_DESCRIPTOR pSecurityDescriptor, PSID pOwner OPTIONAL, BOOL bOwnerDefaulted OPTIONAL)
/*++
Routine Description:
...
Arguments:
    ...
    pSecurityDescriptor - Supplies the security descriptor in which
        the owner is to be set.  If the security descriptor already includes an owner, it will be superseded by the new owner.

    pOwner - Supplies the owner SID for the security descriptor.  If
        this optional parameter is not passed, then the owner is cleared (indicating the security descriptor has no owner).
        The SID is referenced by, not copied into, the security descriptor.

    bOwnerDefaulted - When set, indicates the owner was picked up from some default mechanism (rather than explicitly specified by a
        user).  This value is set in the OwnerDefaulted control flag in the security descriptor.  If this optional parameter is
...
BOOL APIENTRY GetSecurityDescriptorOwner(PSECURITY_DESCRIPTOR pSecurityDescriptor, PSID *pOwner, LPBOOL lpbOwnerDefaulted)
/*++
Routine Description:
...
Arguments:
    ...
    pOwner - Receives a pointer to the owner SID.  If the security
        descriptor does not currently contain an owner, then this value will be returned as null.  In this case, the remaining
        OUT parameters are not given valid return values.  Otherwise, this parameter points to an SID and the remaining OUT
...
BOOL APIENTRY SetSecurityDescriptorGroup(PSECURITY_DESCRIPTOR pSecurityDescriptor, PSID pGroup OPTIONAL, BOOL bGroupDefaulted OPTIONAL)
/*++
Routine Description:
    This procedure sets the primary group information of an absolute security descriptor.  
    If there is already an primary group present in the security descriptor, it is superseded.
Arguments:

    pSecurityDescriptor - Supplies the security descriptor in which the primary group is to be set.
        If the security descriptor already includes a primary group, it will be superseded by the new group.

    pGroup - Supplies the primary group SID for the security descriptor.  
        If this optional parameter is not passed, then the primary group is cleared (indicating the security
        descriptor has no primary group).  The SID is referenced by, not copied into, the security descriptor.

    bGroupDefaulted - When set, indicates the owner was picked up from some default mechanism (rather than explicitly specified by a
        user).  This value is set in the OwnerDefaulted control flag in the security descriptor.  If this optional parameter is
        not passed, then the SaclDefaulted flag will be cleared.
...
BOOL APIENTRY GetSecurityDescriptorGroup(PSECURITY_DESCRIPTOR pSecurityDescriptor, PSID *pGroup, LPBOOL lpbGroupDefaulted)
/*++
Routine Description:
...
Arguments:
    ...
    pGroup - Receives a pointer to the primary group SID.  If the
        security descriptor does not currently contain a primary group, then this value will be returned as null.  
        In this case, the remaining OUT parameters are not given valid return values.  
        Otherwise, this parameter points to an SID and the remaining OUT parameters are provided valid return values.

    lpbGroupDefaulted - This value is returned only if the value returned for the Group parameter is not null.  
        In this case, the GroupDefaulted parameter receives the value of the security descriptor's GroupDefaulted control flag.
...
Arguments:
    ...
    CreatorDescriptor - (Optionally) Points to a security descriptor presented by the creator of the object. 
        If the creator of the object did not explicitly pass security information for the new
    ...
    IsDirectoryObject - Specifies if the new object is going to be a
        directory object.  A value of TRUE indicates the object is a container of other objects.

    Token - Supplies the token for the client on whose behalf the object is being created.  If it is an impersonation token,
        then it must be at SecurityIdentification level or higher.
        If it is not an impersonation token, the operation proceeds normally.

        A client token is used to retrieve default security information for the new object, such as default owner, primary
        group, and discretionary access control.  The token must be open for TOKEN_QUERY access.
...
Arguments:
    ...
    ParentDescriptor - Supplies the Security Descriptor for the parent
        directory under which a object exists.  If there is no parent directory, then this argument is specified as NULL.
    ...
    NewSecurityDescriptor Points to a pointer that is to be made to point to the
        ...
        longer needed, this descriptor must be freed using DestroyPrivateObjectSecurity().

    ObjectType - GUID of the object type being created.  If the object being
        created has no GUID associated with it, then this argument is specified as NULL.
...
--*/
{
    ...
    Status = RtlConvertToAutoInheritSecurityObject(ParentDescriptor,
...
--*/
{
    ...
    Status = RtlNewSecurityObjectEx(ParentDescriptor,
                                    CreatorDescriptor,
                                    NewDescriptor,
                                    ObjectType,
...
--*/
{
    ...
    Status = RtlSetSecurityObjectEx(SecurityInformation, ModificationDescriptor, ObjectsSecurityDescriptor, AutoInheritFlags, GenericMapping, Token);
    if (!NT_SUCCESS(Status)) {
        BaseSetLastNTError(Status);
        return FALSE;
    }
...
BOOL APIENTRY GetPrivateObjectSecurity(PSECURITY_DESCRIPTOR ObjectDescriptor,
...
Arguments:
    ...
    ResultantDescriptor - Points to buffer to receive the resultant security descriptor.
        The resultant security descriptor will contain all information requested by the SecurityInformation parameter.
...
Routine Description:
    ...
    deletion.  The input descriptor is expected to be one created via a call to CreatePrivateObjectSecurity.
Arguments:
...
Arguments:
    ...
    pAbsoluteSecurityDescriptor - A pointer to a buffer in which will
        be placed the main body of the Absolute format security descriptor.
    ...
    pDacl - Supplies a pointer to a buffer that will contain the Dacl
        of the output descriptor.  This pointer will be referenced by, not copied into, the output descriptor.
    lpdwDaclSize - Supplies the size of the buffer pointed to by Dacl.
        In case of error, it will return the minimum size necessary to contain the Dacl.
    ...
    lpdwSaclSize - Supplies the size of the buffer pointed to by Sacl.
        In case of error, it will return the minimum size necessary to contain the Sacl.
    pOwner - Supplies a pointer to a buffer that will contain the
        Owner of the output descriptor.  This pointer will be referenced by, not copied into, the output descriptor.
    lpdwOwnerSize - Supplies the size of the buffer pointed to by
        Owner.  In case of error, it will return the minimum size necessary to contain the Owner.
    pPrimaryGroup - Supplies a pointer to a buffer that will contain
        the PrimaryGroup of the output descriptor.  This pointer will be referenced by, not copied into, the output descriptor.
    lpdwPrimaryGroupSize - Supplies the size of the buffer pointed to
        by PrimaryGroup.  In case of error, it will return the minimum size necessary to contain the PrimaryGroup.
Return Value:
    Returns TRUE for success, FALSE for failure.  Extended error status is available using GetLastError.
--*/
{
...
Arguments:
    ...
    DesiredAccess - Points to an access mask to be set to represent the
        accesses necessary to modify the information specified in the SecurityInformation parameter.
Return Value:
    None.
--*/
{
...
Arguments:
    ...
    DesiredAccess - Points to an access mask to be set to represent the
        accesses necessary to query the information specified in the SecurityInformation parameter.
Return Value:
    None.
--*/
{
...
BOOL APIENTRY SetFileSecurityW(LPCWSTR lpFileName, SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor)
/*++
Routine Description:
    This API can be used to set the security of a file or directory (process, file, event, etc.).
...
Arguments:
    lpFileName - Supplies the file name of the file to open.  Depending on
        the value of the FailIfExists parameter, this name may or may not already exist.
...
BOOL APIENTRY SetFileSecurityA(LPCSTR lpFileName, SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor)
/*++
Routine Description:
...
BOOL APIENTRY GetFileSecurityW(LPCWSTR lpFileName,
...
Routine Description:
    ...
    rights and privileges, this procedure will return a security descriptor containing the requested security descriptor fields.
    To read the handle's security descriptor the caller must be granted READ_CONTROL access or be the owner of the object.  
    In addition, the caller must have SeSecurityPrivilege privilege to read the system ACL.
Arguments:
    ...
    pSecurityDescriptor - A pointer to the buffer to receive a copy of
        ...
        has the rigth to view.  The security descriptor is returned in self-relative format.

    nLength - The size, in bytes, of the security descriptor buffer.

    lpnLengthNeeded - A pointer to the variable to receive the number of bytes needed to store the complete secruity descriptor.
        If returned number of bytes is less than or equal to nLength then
...
BOOL APIENTRY GetFileSecurityA(LPCSTR lpFileName,
                               SECURITY_INFORMATION RequestedInformation,
                               PSECURITY_DESCRIPTOR pSecurityDescriptor,
                               DWORD nLength,
                               LPDWORD lpnLengthNeeded)
...
--*/
{
    ...
    return (GetFileSecurityW((LPCWSTR)Unicode->Buffer, RequestedInformation, pSecurityDescriptor, nLength, lpnLengthNeeded));
}


BOOL APIENTRY SetKernelObjectSecurity(HANDLE Handle, SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR SecurityDescriptor)
/*++
Routine Description:
    This API can be used to set the security of a kernel object (process, file, event, etc.).
    This call is only successful if the following conditions are met:
    o If the object's owner or group is to be set, the caller must have WRITE_OWNER permission or have SeTakeOwnershipPrivilege.
    o If the object's DACL is to be set, the caller must have WRITE_DAC permission or be the object's owner.
    o If the object's SACL is to be set, the caller must have SeSecurityPrivilege.
...
BOOL APIENTRY GetKernelObjectSecurity(HANDLE Handle,
                                      SECURITY_INFORMATION RequestedInformation,
                                      PSECURITY_DESCRIPTOR pSecurityDescriptor,
                                      DWORD nLength,
                                      LPDWORD lpnLengthNeeded)
/*++
Routine Description:
    This API returns top the caller a copy of the security descriptor protecting a kernel object.
    Based on the caller's access rights and privileges, this procedure will return a security descriptor
    containing the requested security descriptor fields.
    To read the handle's security descriptor the caller must be granted READ_CONTROL access or be the owner of the object.
    In addition, the caller must have SeSecurityPrivilege privilege to read the system ACL.
...
/*++
Routine Description:
    This routine may be used to obtain an Impersonation token representing your own process's context.
    This may be useful for enabling a privilege for a single thread rather than for the entire process;
    or changing the default DACL for a single thread.
...
BOOL LookupAccountNameInternal(LPCWSTR lpSystemName,
...
/*++
Routine Description:
    Translates a passed name into an account SID.
    It will also return the name and SID of the first domain in which this name was found.
Arguments:
    lpSystemName - Supplies the name of the system at which the lookup is to be performed.
                   If the null string is provided, the local system is assumed.
    lpAccountName - Supplies the account name.
    Sid - Returns the SID corresponding to the passed account name.
    cbSid - Supplies the size of the buffer passed in for Sid.  
            If the buffer size is not big enough, this parameter will return the size necessary to hold the output Sid.
    ReferencedDomainName - Returns the name of the domain in which the name was found.
    cbReferencedDomainName - Supplies the size (in Wide characters) of the ReferencedDomainName buffer.
        If the buffer size is not large enough, this parameter will return the size necessary to hold the null-terminated output domain name.
        If the buffer size is large enough, tis parameter will return the size (in Ansi characters, excluding the terminating null) of the Referenced Domain name.
...
--*/
{
    ...
    Status = LsaLookupNames(PolicyHandle, 1, &Name, &ReferencedDomains, &TranslatedSid);// Attempt to translate the Name to a Sid.

#if DBG
// This code is useful for tracking down components that call Lookup code before the system is initialized
    ...
    TmpStatus = LsaClose(PolicyHandle);// Close the Policy Handle,  which is not needed after here.
//    ASSERT( NT_SUCCESS( TmpStatus ));

    // If an error was returned, check specifically for STATUS_NONE_MAPPED.
    // In this case, we may need to dispose of the returned Referenced Domain List and Translated Sid structures.
    // For all other errors, LsaLookupNames() frees these structures prior to exit.
    if (!NT_SUCCESS(Status)) {
    ...
    // There should be exactly one Referenced Domain and its DomainIndex should be zero.
    ...
    if (!fUnicode) {
        RtlUnicodeToMultiByteSize(&ReturnedDomainNameSize, ReferencedDomains->Domains->Name.Buffer, ReferencedDomains->Domains->Name.Length);
    } else {
    ...
    // Check if buffer sizes are too small.
    // For the returned domain, the size in Wide characters provided must allow for the null terminator that will be appended to the returned name.
    if ((SidLengthRequired > *cbSid) || (ReturnedDomainNameSize + 1 > *cbReferencedDomainName)) {
        ...
        *cbReferencedDomainName = ReturnedDomainNameSize + 1;
        ...
        // Build the SID in the return buffer by combining the Domain Sid returned by LsaLookupNames() with the Relative Id returned.
        // The final SID will have one more SubAuthority than the Domain Sid.
        ...
        *peUse = TranslatedSid->Use;// Copy the Sid Use field.
...
BOOL APIENTRY LookupAccountNameA(LPCSTR lpSystemName,
...
Arguments:
    ...
    ReferencedDomainName - Returns the name of the domain in which the name was found.
    cbReferencedDomainName - Supplies the size (in Ansi characters) of the ReferencedDomainName buffer.
        If the buffer size is not large enough, this parameter will return the size necessary to hold the null-terminated output domain name.
        If the buffer size is large enough, tis parameter will return the size (in Ansi characters, excluding the terminating null) of the Referenced Domain name.
...
--*/
{
    ...
    // Convert the passed lpAccountName to a WCHAR string to be passed to the ..W routine.
    // Note that we cannot use the StaticUnicodeString in the Thread Environment Block because this is used by LdrpWalkImportDescriptor,
    // called from the client RPC stub code of the LsaOpenPolicy() call in LookupAccountNameW.
    ...
    if (rc) {
        rc = LookupAccountNameInternal((LPCWSTR)pSystemName,
...
BOOL APIENTRY LookupAccountNameW(LPCWSTR lpSystemName,
                                 LPCWSTR lpAccountName,
                                 PSID Sid,
                                 LPDWORD cbSid,
                                 LPWSTR ReferencedDomainName,
...
BOOL APIENTRY LookupAccountSidInternal(LPCWSTR lpSystemName,
                                       PSID lpSid,
                                       LPWSTR lpName,
                                       LPDWORD cbName,
...
/*++
Routine Description:
    Translates a passed SID into an account name.
    It will also return the name and SID of the first domain in which this SID was found.
Arguments:
    lpSystemName - Supplies the name of the system at which the lookup is to be performed.
        If the null string is provided, the local system is assumed.
    lpSid - Supplies the account Sid.
    lpName - Returns the name corresponding to the passed account SID.
    cbName - Supplies the size (in Wide characters) of the buffer passed in for lpName.
        This size must allow one character for the null terminator that will be appended to the returned name.
        If the buffer size is not large enough, this parameter will return the size necessary to hold the null-terminated output name.
        If the buffer size is large enough, this parameter will return the size (in Ansi characters, excluding the null terminator) of the name returned.
    lpReferencedDomainName - Returns the name of the domain in which the name was found.
    cbReferencedDomainName - Supplies the size (in Wide characters) of the ReferencedDomainName buffer.
        This size must allow one charcter for the null terminator that will be appended to the returned name.
        If the buffer size is not large enough, this parameter will return the size necessary to hold the output null-terminated domain name.
        If the buffer size is large enough, the size of the returned name, excluding the terminating null will be returned.
    peUse - Returns an enumerated type inidicating the type of the account.
    fUnicode - indicates whether the caller wants a count of unicode or ansi characters.
Return Value:
...
--*/
{
    ...
    // If an error was returned, check specifically for STATUS_NONE_MAPPED.
    // In this case, we may need to dispose of the returned Referenced Domain List and Names structures.
    // For all other errors, LsaLookupSids() frees these structures prior to exit.
    if (!NT_SUCCESS(Status)) {
        if (Status == STATUS_NONE_MAPPED) {
            if (ReferencedDomains != NULL) {
                TmpStatus = LsaFreeMemory(ReferencedDomains);
    ...
    // Check if buffer sizes  for the Name and Referenced Domain Name are too small.
    // The sizes in Wide characters provided must allow for the null terminator that will be appended to the returned names.
    if ((ReturnedNameSize + 1 > *cbName) || (ReturnedDomainNameSize + 1 > *cbReferencedDomainName)) {
        // One or both buffers are too small.
        // Return sizes required for both buffers, allowing one character for the null terminator.
        *cbReferencedDomainName = ReturnedDomainNameSize + 1;
        ...
        *peUse = Names->Use;// Copy in the Use field.
        Rc = TRUE;
    }
...
BOOL APIENTRY LookupAccountSidA(LPCSTR lpSystemName,
                                PSID lpSid,
                                LPSTR lpName,
                                LPDWORD cbName,
...
Arguments:
    ...
    lpSid - Supplies the account Sid.
    lpName - Returns the name corresponding to the passed account SID.
    cbName - Supplies the size (in Ansi characters) of the buffer passed in for lpName.
        This size must allow one character for the null terminator that will be appended to the returned name.
        If the buffer size is not large enough, this parameter will return the size necessary to hold the null-terminated output name.
        If the buffer size is large enough, this parameter will return the size (in Ansi characters, excluding the null terminator) of the name returned.
    lpReferencedDomainName - Returns the name of the domain in which the name was found.
...
--*/
{
    ...
    BoolStatus = LookupAccountSidInternal((LPCWSTR)pSystemName,
                                          lpSid,
                                          WName,
                                          cbName,
                                          WReferencedDomainName,
...
BOOL APIENTRY LookupAccountSidW(LPCWSTR lpSystemName,
                                PSID lpSid,
                                LPWSTR lpName,
                                LPDWORD cbName,
                                LPWSTR lpReferencedDomainName,
...
--*/
{
    return(LookupAccountSidInternal(lpSystemName,
                                    lpSid,
                                    lpName,
                                    cbName,
...
/*++
Routine Description:
    This function retrieves the value used on the target system to locally represent the specified privilege.
    The privilege is specified by programmatic name.
Arguments:
    lpSystemName - Supplies the name of the system at which the lookup is to be performed.
        If the null string is provided, the local system is assumed.
...
Arguments:
    ...
    cchName - indicates how large the buffer is (in characters).
              This count does not include the null-terminator that is added at the end of the string.
--*/
{
    NTSTATUS                    Status, TmpStatus;
    LSA_HANDLE                  PolicyHandle;
...
Arguments:
    lpSystemName - Supplies the name of the system at which the lookup
        is to be performed.  If the null string is provided, the local system is assumed.
    lpName - provides the privilege's programmatic name.
    lpDisplayName - Receives the privilege's displayable name.
    cchDisplayName - indicates how large the buffer is (in characters).
                     This count does not include the null-terminator that is added at the end of the string.
...
VOID SepFormatAccountSid(PSID Sid, LPWSTR OutputBuffer)
{
    ...
    // Do everything as ANSI for the time being, and then convert to wide-char at the bottom.

    // We need to do this until we have more complete c-runtime support for w-char strings.
...
BOOL APIENTRY CreateRestrictedToken(IN HANDLE ExistingTokenHandle,
                                    IN DWORD Flags,
                                    ...
                                    OUT PHANDLE NewTokenHandle)
{
    ...
    if (DisableSidCount != 0) {
        ...
        DisabledSids = (PTOKEN_GROUPS)LocalAlloc(0, sizeof(TOKEN_GROUPS) + (DisableSidCount - 1) * sizeof(SID_AND_ATTRIBUTES));
    ...
    if (DeletePrivilegeCount != 0) {
        ...
        DeletedPrivileges = (PTOKEN_PRIVILEGES)LocalAlloc(0, sizeof(TOKEN_PRIVILEGES) + (DeletePrivilegeCount - 1) * sizeof(LUID_AND_ATTRIBUTES));
        if (DeletedPrivileges == NULL) {
            Status = STATUS_INSUFFICIENT_RESOURCES;
            goto Cleanup;
        }
        DeletedPrivileges->PrivilegeCount = DeletePrivilegeCount;
        RtlCopyMemory(DeletedPrivileges->Privileges, PrivilegesToDelete, DeletePrivilegeCount * sizeof(LUID_AND_ATTRIBUTES));
    }
    ...
    if (RestrictedSidCount != 0) {
        ...
        RestrictedSids = (PTOKEN_GROUPS)LocalAlloc(0, sizeof(TOKEN_GROUPS) + (RestrictedSidCount - 1) * sizeof(SID_AND_ATTRIBUTES));
...
Arguments:
    TokenHandle - If present, this token is checked for the sid. If not
        present then the current effective token will be used. This must be an impersonation token.
...
#define MEMBER_ACCESS 1
    ...
    * IsMember = FALSE;
    ...
    if (ARGUMENT_PRESENT(TokenHandle)) {
        ...
        if (Status == STATUS_NO_TOKEN)// if there is no thread token, try the process token
        {
            Status = NtOpenProcessToken(NtCurrentProcess(), TOKEN_QUERY | TOKEN_DUPLICATE, &ProcessToken);
            // If we have a process token, we need to convert it to an impersonation token
            if (NT_SUCCESS(Status)) {
                BOOL Result;
    ...
    // ACE, which is an ACE + length of ths SID.
    ...
    Status = NtAccessCheck(SecDesc, EffectiveToken, MEMBER_ACCESS, &GenericMapping, PrivilegeSet, &PrivilegeSetLength, &AccessGranted, &AccessStatus);
    if (!NT_SUCCESS(Status)) {
        goto Cleanup;
    }

    // if the access check failed, then the sid is not a member of the token
...
--*/
{
    ...
    // MakeAbsoluteSD2() has the same prototype as RtlSelfRelativeToAbsoluteSD2() so the parameters check returns the same parameter order if the caller passes invalid parameter.
...
Return Value:
    ERROR_INVALID_DATA      if the SE_RM_CONTROL_VALID flag is not present in the security descriptor
