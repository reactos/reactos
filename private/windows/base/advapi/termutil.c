...
BOOL IsSystemLUID(VOID)
{
    ...
    if ( CurrentLUID.LowPart == 0 && CurrentLUID.HighPart == 0 ) {
        ...
        NtQueryInformationToken( TokenHandle, TokenStatistics, &TokenInformation, sizeof(TokenInformation), &ReturnLength );
        NtClose( TokenHandle );
...
void InitializeTermsrvFpns(void)
{
    ...
    if (IsSystemLUID()) {
        if ( (dwCompatFlags & (TERMSRV_COMPAT_SYSREGMAP | TERMSRV_COMPAT_WIN32)) != (TERMSRV_COMPAT_SYSREGMAP | TERMSRV_COMPAT_WIN32) ) {
            // Process is running as SYSTEM and we don't have an app
            // compatibility flag telling us to do the regmap stuff.
            return;
        }
    } else if ( (dwCompatFlags & (TERMSRV_COMPAT_NOREGMAP | TERMSRV_COMPAT_WIN32)) == (TERMSRV_COMPAT_NOREGMAP | TERMSRV_COMPAT_WIN32) ) {
        return;// We don't want to do registry mapping for this user process
    }
