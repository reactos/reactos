...
DWORD AllowDesktopAccessToUser(LPTSTR  WinstaName, LPTSTR  DeskName, LPCTSTR  UserName, LPCTSTR  DomainName)
/*++
Routine Description:
   This routine opens the desktop defined in lpStartupInfo, modifies DACL on it to grant user in the Token full control
Arguments:
...
DWORD RemoveDesktopAccessFromUser(LPTSTR  WinstaName, LPTSTR  DeskName, LPCTSTR  UserName, LPCTSTR  DomainName)
/*++
Routine Description:
   This routine opens the desktop defined in lpStartupInfo, modifies DACL on it to remove user ACE.
Arguments:
   hToken -- handle to the user's token
   lpStartupInfo -- process startup structure that contains the desktop name
*/
...
static BOOL WINAPI FreePackedStructAndData(PVOID pvPackedStructAndData)
{
    return(HeapFree(GetProcessHeap(), 0, pvPackedStructAndData));
}


static PVOID __cdecl PackStructAndDataW(PDWORD pcbBytesToSend, PVOID pvStruct, int cbStruct, int nDataPtrOffset0, int nDataLen0, ...)
{
    ...
    LPPROCESS_INFORMATION lpProcessInformation)
{
    ...
    __try {
        ...
        // We are overloading CREATE_NEW_CONSOLE to CREATE_WITH_NETWORK_LOGON
        ...
        if (lpApplicationName == NULL)
        {
            if (lpCommandLine != NULL)
            {
                ...
                LPWSTR  ApplName = (LPWSTR)HeapAlloc(hheap, 0, sizeof(WCHAR) * (MAX_PATH + 1));
