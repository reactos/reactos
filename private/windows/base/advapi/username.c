...
BOOL WINAPI GetUserNameInternal (LPWSTR pBuffer, LPDWORD pcbBuffer, BOOLEAN fUnicode)
/*++
Routine Description:
  This returns the name of the user currently being impersonated.
Arguments:
    pBuffer - Points to the buffer that is to receive the null-terminated character string containing the user name.
    pcbBuffer - Specifies the size (in characters) of the buffer. The length of the string is returned in pcbBuffer.
Return Value:
...
--*/
{
    ...
    Status = LsaGetUserName(&UserName, NULL);
    ...
    if (fUnicode) {
        ...
        RtlUnicodeToMultiByteSize(&ReturnedNameSize, UserName->Buffer, UserName->Length);
        // Add one for a null terminator
        ReturnedNameSize++;
    }

    // Check that the buffer is big enough. If it not, let the caller know how big it should be.
    if (ReturnedNameSize > *pcbBuffer) {
        *pcbBuffer = ReturnedNameSize;
        BaseSetLastNTError( STATUS_BUFFER_TOO_SMALL );
        ReturnValue = FALSE;
    } else {
        CopyMemory(pBuffer, UserName->Buffer, UserName->Length);
        pBuffer[UserName->Length / sizeof(WCHAR)] = L'\0';// Zero terminate the returned buffer.
        *pcbBuffer = ReturnedNameSize;// Return the length of the name in characters.
        ReturnValue = TRUE;
    }
