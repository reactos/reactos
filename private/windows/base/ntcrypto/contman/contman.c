    ...
    )
{
    ...
    while (1)
    {
        fResult = CryptProtectData(pDataIn, szDataDescr, pOptionalEntropy, pvReserved, pPromptStruct, dwFlags, pDataOut);
        if(!fResult)
        {
            dwErr = GetLastError();
            if ((RPC_S_SERVER_TOO_BUSY == dwErr) && (MAX_DPAPI_RETRY_COUNT > dwRetryCount))
            {
...
DWORD GetHashOfContainer(LPCSTR pszContainer, LPWSTR pszHash)
{
    MD5_CTX     MD5;
    ...
    if (NULL == (pszLowerContainer = (LPSTR)ContInfoAlloc(strlen(pszContainer) + sizeof(CHAR))))
    {
...
DWORD GetMachineGUID(LPWSTR *ppwszUuid)
{
    HKEY    hRegKey = 0;
//  UUID    Uuid;
    LPSTR   pszUuid = NULL;
    ...
    *ppwszUuid = NULL;
    ...
    dwSts = RegOpenKeyEx(HKEY_LOCAL_MACHINE, SZLOCALMACHINECRYPTO, 0, KEY_READ, &hRegKey);
    ...
    dwSts = RegQueryValueEx(hRegKey, SZCRYPTOMACHINEGUID, 0, NULL, NULL, &cbUuid);
    if (ERROR_FILE_NOT_FOUND == dwSts)
    {
        goto Ret;   // Return a success code, but a null GUID.
    }
    ...
    if (ERROR_SUCCESS != RegQueryValueEx(hRegKey, SZCRYPTOMACHINEGUID, 0, NULL, pszUuid, &cbUuid))
    {
        dwErr = (DWORD)NTE_FAIL;
        goto Ret;
    }
    ...
    if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszUuid, -1, NULL, cch)))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    ...
    if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszUuid, -1, pwszUuid, cch)))
    {
         dwErr = GetLastError();
         goto Ret;
    }
...
DWORD SetMachineGUID()
{
    ...
    // read the GUID from the Local Machine portion of the registry
    if (ERROR_SUCCESS != RegCreateKeyEx(HKEY_LOCAL_MACHINE, SZLOCALMACHINECRYPTO, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hRegKey, &dwResult))
    {
        dwErr = (DWORD)NTE_FAIL;
        goto Ret;
    }

    if (ERROR_SUCCESS == RegQueryValueEx(hRegKey, SZCRYPTOMACHINEGUID, 0, NULL, NULL, &cbUuid))
    {
        goto Ret;
    }
    ...
    if (ERROR_SUCCESS != RegSetValueEx(hRegKey, SZCRYPTOMACHINEGUID, 0, REG_SZ, (BYTE*)pszUuid, strlen(pszUuid) + 1))
    {
        dwErr = (DWORD)NTE_FAIL;
        goto Ret;
    }
...
DWORD AddMachineGuidToContainerName(LPSTR pszContainer, LPWSTR pwszNewContainer)
{
    WCHAR   rgwszHash[33];
    LPWSTR  pwszUuid = NULL;
    DWORD   dwErr = 0;
...
DWORD TryDPAPI()
{
    ...
    if (!MyCryptProtectData(&DataIn, L"Export Flag", &ExtraEntropy, NULL, &PromptStruct, 0, &DataOut))
    {
...
DWORD ProtectExportabilityFlag(IN BOOL fExportable, IN BOOL fMachineKeyset, OUT BYTE **ppbProtectedExportability, OUT DWORD *pcbProtectedExportability)
{
    CRYPTPROTECT_PROMPTSTRUCT   PromptStruct;
    CRYPT_DATA_BLOB             DataIn;
    CRYPT_DATA_BLOB             DataOut;
    ...
    ExtraEntropy.cbData = sizeof(STUFF_TO_GO_INTO_MIX);
    ExtraEntropy.pbData = STUFF_TO_GO_INTO_MIX;
    if (!MyCryptProtectData(&DataIn, L"Export Flag", &ExtraEntropy, NULL, &PromptStruct, dwProtectFlags, &DataOut))
    {
        dwErr = (DWORD)NTE_FAIL;
        goto Ret;
    }
...
DWORD UnprotectExportabilityFlag(IN BOOL fMachineKeyset, IN BYTE *pbProtectedExportability, IN DWORD cbProtectedExportability, IN BOOL *pfExportable)
{
    CRYPTPROTECT_PROMPTSTRUCT   PromptStruct;
    CRYPT_DATA_BLOB             DataIn;
    CRYPT_DATA_BLOB             DataOut;
    ...
    ExtraEntropy.cbData = sizeof(STUFF_TO_GO_INTO_MIX);
    ExtraEntropy.pbData = STUFF_TO_GO_INTO_MIX;
    if (!MyCryptUnprotectData(&DataIn, NULL, &ExtraEntropy, NULL, &PromptStruct, dwProtectFlags, &DataOut))
    {
        dwErr = (DWORD)NTE_BAD_KEYSET;
        goto Ret;
    }
...
DWORD GetMachineKeysetDirDACL(IN OUT PACL *ppDacl)
/*
    Creates a DACL for the MachineKeys directory for machine keysets so that Everyone may create machine keys.
*/
{
    ...
    if(!AllocateAndInitializeSid(&siaWorld, 1, SECURITY_WORLD_RID, 0, 0, 0, 0, 0, 0, 0, &pEveryoneSid))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    if(!AllocateAndInitializeSid(&siaNTAuth, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, &pAdminsSid))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    ...
    dwAclSize = sizeof(ACL) + 2 * ( sizeof(ACCESS_ALLOWED_ACE) - sizeof(DWORD) ) + GetLengthSid(pEveryoneSid) + GetLengthSid(pAdminsSid);
    ...
    if(!AddAccessAllowedAce(*ppDacl, ACL_REVISION, (FILE_GENERIC_WRITE | FILE_GENERIC_READ) & (~WRITE_DAC), pEveryoneSid))
    {
        dwErr = GetLastError();
        goto Ret;
    }

    if(!AddAccessAllowedAce(*ppDacl, ACL_REVISION, FILE_ALL_ACCESS, pAdminsSid))
    {
        dwErr = GetLastError();
        goto Ret;
    }
...
*/
{
    ...
    if( wszCreationStartPoint < wszFullPath || wszCreationStartPoint  > (wcslen(wszFullPath) + wszFullPath))
    {
...
BOOL IsLocalSystem(BOOL *pfIsLocalSystem)
/*
...
//  UINT x;
    ...
    *pfIsLocalSystem = FALSE;
    ...
    if (TRUE == OpenThreadToken(GetCurrentThread(), MAXIMUM_ALLOWED, TRUE, &hThreadToken))
    {
    ...
    bSuccess = GetTokenInformation(hToken, TokenUser, pTokenUser, dwInfoBufferSize, &dwInfoBufferSize);
    ...
    if (FALSE == AllocateAndInitializeSid(&siaNtAuthority, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0, 0, 0, 0, 0, &psidLocalSystem))
        goto Ret;
...
BOOL IsThreadLocalSystem(BOOL *pfIsLocalSystem)
/*
    This function determines if the user associated with the specified token is the Local System account.
*/
{
    HANDLE  hToken = 0;
    UCHAR InfoBuffer[1024];
    ...
    )
{
    ...
    if ( (psia->Value[0] != 0) || (psia->Value[1] != 0) ) {
        dwSidSize += wsprintfA(TextualSid + dwSidSize, "0x%02hx%02hx%02hx%02hx%02hx%02hx",
                    (USHORT)psia->Value[0],
                    (USHORT)psia->Value[1],
                    (USHORT)psia->Value[2],
                    (USHORT)psia->Value[3],
        ...
        dwSidSize += wsprintfA(TextualSid + dwSidSize, "%lu", (ULONG)(psia->Value[5]) + (ULONG)(psia->Value[4] << 8) + (ULONG)(psia->Value[3] << 16) + (ULONG)(psia->Value[2] << 24));
    }

    // loop through SidSubAuthorities
    for (dwCounter = 0 ; dwCounter < dwSubAuthorities ; dwCounter++) {
        dwSidSize += wsprintfA(TextualSid + dwSidSize, "-%lu", *GetSidSubAuthority(pSid, dwCounter) );
    }
    ...
    )
{
    ...
    // prepare SidIdentifierAuthority
    if ( (psia->Value[0] != 0) || (psia->Value[1] != 0) ) {
        dwSidSize += wsprintfW(wszTextualSid + dwSidSize, L"0x%02hx%02hx%02hx%02hx%02hx%02hx",
                    (USHORT)psia->Value[0],
                    (USHORT)psia->Value[1],
                    (USHORT)psia->Value[2],
                    (USHORT)psia->Value[3],
        ...
        dwSidSize += wsprintfW(wszTextualSid + dwSidSize, L"%lu", (ULONG)(psia->Value[5]) + (ULONG)(psia->Value[4] << 8) + (ULONG)(psia->Value[3] << 16) + (ULONG)(psia->Value[2] << 24));
    }

    // loop through SidSubAuthorities
    for (dwCounter = 0 ; dwCounter < dwSubAuthorities ; dwCounter++) {
        dwSidSize += wsprintfW(wszTextualSid + dwSidSize, L"-%lu", *GetSidSubAuthority(pSid, dwCounter) );
    }
...
BOOL GetUserSid(PTOKEN_USER *pptgUser, DWORD *pcbUser, BOOL *pfAlloced)
{
    HANDLE      hToken = 0;
    BOOL        bSuccess;
    BOOL        fRet = FALSE;
...
BOOL GetUserTextualSidA(LPSTR lpBuffer, LPDWORD nSize)
{
...
BOOL GetUserTextualSidW(LPWSTR lpBuffer, LPDWORD nSize)
{
    BYTE        FastBuffer[FAST_BUF_SIZE];
    PTOKEN_USER ptgUser;
    DWORD       cbUser;
...
DWORD GetUserDirectory(IN BOOL fMachineKeyset, OUT LPWSTR pwszUser, OUT DWORD *pcbUser)
{
    DWORD   dwErr = 0;
...
typedef HRESULT (WINAPI *SHGETFOLDERPATHW)(HWND hwnd, int csidl, HANDLE hToken, DWORD dwFlags, LPWSTR pwszPath);
    ...
    )
{
    ...
    *pfIsLocalSystem = FALSE;
    ...
    if ((PROV_RSA_SIG == dwProvType) || (PROV_RSA_FULL == dwProvType) || (PROV_RSA_SCHANNEL == dwProvType))
    {
        wszProductString = WSZRSAPRODUCTSTRING;
    }
    else if ((PROV_DSS == dwProvType) || (PROV_DSS_DH == dwProvType) || (PROV_DH_SCHANNEL == dwProvType))
    {
    ...
    if( fMachineKeyset || *pfIsLocalSystem )
    {
        if (!fOldWin2KMachineKeyPath)
        {
            ...
            dwLastError = (DWORD)_SHGetFolderPathW( NULL, CSIDL_COMMON_APPDATA | CSIDL_FLAG_CREATE, hToken, 0, wszUserStorageRoot );
            CloseHandle( hToken );
        ...
        {
            cbUserStorageRoot = GetSystemDirectoryW(wszUserStorageRoot, MAX_PATH);
    ...
    else
    {
        ...
        if ((dwTempProfileFlags & PT_TEMPORARY) || (dwTempProfileFlags & PT_MANDATORY))
        {
        ...
        dwLastError = (DWORD)_SHGetFolderPathW( NULL, CSIDL_APPDATA | CSIDL_FLAG_CREATE, hToken, 0, wszUserStorageRoot );
        CloseHandle( hToken );
        if( dwLastError != ERROR_SUCCESS )
            goto Ret;
    ...
    *ppwszUserStorageArea = (LPWSTR)ContInfoAlloc(
    ...
    if (*ppwszUserStorageArea) {
        ...
        dwLastError = CreateNestedDirectories(*ppwszUserStorageArea, (LPWSTR)((LPBYTE)*ppwszUserStorageArea + cbUserStorageRoot + sizeof(WCHAR)), fMachineKeyset);
    }
    else
...
DWORD GetFilePath(IN      LPCWSTR  pwszUserStorageArea, IN      LPCWSTR  pwszFileName, IN OUT  LPWSTR   *ppwszFilePath)
{
...
)
{
    ...
    hFile = CreateFileW(wszFilePath, dwDesiredAccess, dwShareMode, NULL, dwCreationDisposition, dwAttribs, NULL);
    if (INVALID_HANDLE_VALUE == hFile)
    {
        ...
        if (fMachineKeyset)
        {
            ...
            // open a token handle
            if (FALSE == OpenThreadToken(GetCurrentThread(), MAXIMUM_ALLOWED, TRUE, &hToken))
            {
                if (FALSE == OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &hToken))
                {
                    goto Ret;
            ...
            {
                if(!LookupPrivilegeValue(NULL, SE_BACKUP_NAME, &(pPriv->Privilege[0].Luid)))
                {
                    goto Ret;
                }
            }
            ...
            {
                if(!LookupPrivilegeValue(NULL, SE_RESTORE_NAME, &(pPriv->Privilege[0].Luid)))
                {
                    goto Ret;
                }
            }
            ...
            if (fPrivSet)
            {
                hFile = CreateFileW(wszFilePath, dwDesiredAccess, dwShareMode, NULL, dwCreationDisposition, dwAttribs | FILE_FLAG_BACKUP_SEMANTICS, NULL);
                if (INVALID_HANDLE_VALUE != hFile)
                {
                    fSetLastError = FALSE;
                }
...
//  DWORD cbFileName;
    ...
    *phFile = INVALID_HANDLE_VALUE;
    ...
    if (0 != (dwLastError = GetFilePath(wszUserStorageArea, wszFileName, &wszFilePath)))
    {
        goto Ret;
    }
    ...
    while (1)
    {
        *phFile = MyCreateFile(fMachineKeyset, wszFilePath, dwDesiredAccess, dwShareMode, dwCreationDistribution, dwAttribs | FILE_FLAG_SEQUENTIAL_SCAN);
        if( *phFile == INVALID_HANDLE_VALUE )
        {
            dwLastError = GetLastError();
            if (((ERROR_SHARING_VIOLATION == dwLastError) || (ERROR_ACCESS_DENIED == dwLastError)) && (MAX_CREATE_FILE_RETRY_COUNT > dwRetryCount))
            {
...
//  DWORD cbFileName;
    ...
    *phFile = INVALID_HANDLE_VALUE;
    ...
    if (0 != (dwLastError = GetFilePath(pwszUserStorageArea, rgwszNewFileName, &pwszFilePath)))
    {
        goto Ret;
    }

    hFind = FindFirstFileExW(pwszFilePath, FindExInfoStandard, &FindData, FindExSearchNameMatch, NULL, 0);
    ...
    if (0 != (dwLastError = GetFilePath(pwszUserStorageArea, FindData.cFileName, &pwszFilePath)))
    {
        goto Ret;
    }

    *phFile = CreateFileW(pwszFilePath, GENERIC_READ, dwShareMode, NULL, dwCreationDistribution, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
    if( *phFile == INVALID_HANDLE_VALUE )
    {
        dwLastError = NTE_BAD_KEYSET;
        goto Ret;
    ...
    wcscpy(pwszNewFileName, FindData.cFileName);// allocate and copy in the real file name to be returned
...
//  specified token is the Local System account.
DWORD ZeroizeFile(IN      LPCWSTR  wszFilePath)
{
    HANDLE  hFile = INVALID_HANDLE_VALUE;
    BYTE    *pb = NULL;
    DWORD   cb;
    ...
    if (INVALID_HANDLE_VALUE == (hFile = CreateFileW(wszFilePath, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_SYSTEM, NULL)))
    {
        dwErr = (DWORD)NTE_BAD_KEYSET;
        goto Ret;
    }
...
DWORD DeleteFileInStorageArea(IN      LPCWSTR  wszUserStorageArea, IN      LPCWSTR  wszFileName)
{
    LPWSTR wszFilePath = NULL;
    DWORD cbUserStorageArea;
    DWORD cbFileName;
...
DWORD SetContainerUserName(IN LPSTR pszUserName, IN PKEY_CONTAINER_INFO pContInfo)
{
    ...
    if (NULL == (pContInfo->pszUserName = (LPSTR)ContInfoAlloc((strlen(pszUserName) + 1) * sizeof(CHAR))))
    {
        dwErr = ERROR_NOT_ENOUGH_MEMORY;
        goto Ret;
    }
...
DWORD NT5ToNT5Migration(IN DWORD dwProvType, IN BOOL fMachineKeyset, IN LPSTR pszContainer, IN LPWSTR pwszFilePath, IN DWORD dwFlags, OUT PKEY_CONTAINER_INFO pContInfo, OUT HANDLE *phFile)
{
    ...
    if (0 != (dwErr = GetUserStorageArea(dwProvType, fMachineKeyset, TRUE, &fIsLocalSystem, &pwszOldFilePath)))
    {
        goto Ret;
    }
    ...
    if (69 == strlen(pszContainer))
    {
        ...
        if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszContainer, -1, NULL, cch)))
        {
            dwErr = GetLastError();
            goto Ret;
        }
        ...
        if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszContainer, -1, pwszFileName, cch)))
        {
             dwErr = GetLastError();
             goto Ret;
        }

        if (ERROR_SUCCESS != OpenFileInStorageArea(fMachineKeyset, GENERIC_READ, pwszOldFilePath, pwszFileName, phFile))
        {
    ...
    if (fRetryWithHashedName)
    {
        ...
        if(ERROR_SUCCESS != (dwErr = OpenFileInStorageArea(fMachineKeyset, GENERIC_READ, pwszOldFilePath, pwszFileName, phFile)))
        {
    ...
    *phFile = INVALID_HANDLE_VALUE;
    ...
    if(ERROR_SUCCESS != (dwErr = OpenFileInStorageArea(fMachineKeyset, GENERIC_READ, pwszFilePath, pwszFileName, phFile)))
    {
        goto Ret;
    }
...
//  DWORD                   cbFileName;
    ...
    // get the correct storage area (directory)
    if (0 != (dwErr = GetUserStorageArea(dwProvType, fMachineKeyset, FALSE, &fIsLocalSystem, &pwszFilePath)))
    {
        goto Ret;
    }
    ...
    if (69 == strlen(pszContainerName))
    {
        ...
        if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszContainerName, -1, NULL, cch)))
        {
            dwErr = GetLastError();
            goto Ret;
        }
        ...
        if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszContainerName, -1, pwszFileName, cch)))
        {
             dwErr = GetLastError();
             goto Ret;
        }

        if (ERROR_SUCCESS == OpenFileInStorageArea(fMachineKeyset, GENERIC_READ, pwszFilePath, pwszFileName, &hFile))
        {
    ...
    if (fRetryWithHashedName)
    {
        if (0 != (dwErr = AddMachineGuidToContainerName(pszContainerName, pContInfo->rgwszFileName)))
        {
            goto Ret;
        }
        if(ERROR_SUCCESS != (dwErrOpen = OpenFileInStorageArea(fMachineKeyset, GENERIC_READ, pwszFilePath, pContInfo->rgwszFileName, &hFile)))
        {
            ...
            {
                if (0 != (dwErr = NT5ToNT5Migration(dwProvType, fMachineKeyset, pszContainerName, pwszFilePath, dwFlags, pContInfo, &hFile)))
                {
            ...
            else
            {
                ...
                if(ERROR_SUCCESS != FindClosestFileInStorageArea(pwszFilePath, pszContainerName, rgwszOtherMachineFileName, &hFile))
                {
                    dwErr = (DWORD)NTE_BAD_KEYSET;
                    goto Ret;
                }
    ...
    if (NULL == (pbFile = (BYTE*)MapViewOfFile(hMap, FILE_MAP_READ, 0, 0, 0 )))
    {
        dwErr = (DWORD)NTE_BAD_KEYSET;
        goto Ret;
    }
    ...
    // get the user name
    if (NULL == (pContInfo->pszUserName = ContInfoAlloc(pContInfo->ContLens.cbName)))
    {
        dwErr = ERROR_NOT_ENOUGH_MEMORY;
        goto Ret;
    }
    ...
    // get the random seed
    if (NULL == (pContInfo->pbRandom = ContInfoAlloc(pContInfo->ContLens.cbRandom)))
    {
        dwErr = ERROR_NOT_ENOUGH_MEMORY;
        goto Ret;
    }
    ...
    {
        if (NULL == (pContInfo->pbSigPub = ContInfoAlloc(pContInfo->ContLens.cbSigPub)))
        {
            dwErr = ERROR_NOT_ENOUGH_MEMORY;
            goto Ret;
        }
        ...
        if (NULL == (pContInfo->pbSigEncPriv = ContInfoAlloc(pContInfo->ContLens.cbSigEncPriv)))
        {
            dwErr = ERROR_NOT_ENOUGH_MEMORY;
            goto Ret;
        }
        memcpy(pContInfo->pbSigEncPriv, pbFile + cb, pContInfo->ContLens.cbSigEncPriv);
        cb += pContInfo->ContLens.cbSigEncPriv;

        // get the exportability info for the sig key
        if (0 != (dwErr = UnprotectExportabilityFlag(fMachineKeyset, pbFile + cb, Exportability.cbSigExportability, &pContInfo->fSigExportable)))
        {
            dwErr = (DWORD)NTE_BAD_KEYSET;
            goto Ret;
        }
    ...
    {
        if (NULL == (pContInfo->pbExchPub = ContInfoAlloc(pContInfo->ContLens.cbExchPub)))
        {
            dwErr = ERROR_NOT_ENOUGH_MEMORY;
            goto Ret;
        }
        memcpy(pContInfo->pbExchPub, pbFile + cb, pContInfo->ContLens.cbExchPub);
        cb += pContInfo->ContLens.cbExchPub;

        if (NULL == (pContInfo->pbExchEncPriv = ContInfoAlloc(pContInfo->ContLens.cbExchEncPriv)))
        {
            dwErr = ERROR_NOT_ENOUGH_MEMORY;
            goto Ret;
        }
        memcpy(pContInfo->pbExchEncPriv, pbFile + cb, pContInfo->ContLens.cbExchEncPriv);
        cb += pContInfo->ContLens.cbExchEncPriv;

        // get the exportability info for the sig key
        if (0 != (dwErr = UnprotectExportabilityFlag(fMachineKeyset, pbFile + cb, Exportability.cbExchExportability, &pContInfo->fExchExportable)))
        {
            dwErr = (DWORD)NTE_BAD_KEYSET;
            goto Ret;
        }
...
//  DWORD                   cbProtectedExchExportFlag;
    ...
    if (pContInfo->ContLens.cbSigPub && pContInfo->ContLens.cbSigEncPriv)
    {
        if (0 != (dwErr = ProtectExportabilityFlag(pContInfo->fSigExportable, fMachineKeyset, &pbProtectedSigExportFlag, &ExportabilityLens.cbSigExportability)))
        {
            goto Ret;
        }
    }
    ...
    if (pContInfo->ContLens.cbExchPub && pContInfo->ContLens.cbExchEncPriv)
    {
        if (0 != (dwErr = ProtectExportabilityFlag(pContInfo->fExchExportable, fMachineKeyset, &pbProtectedExchExportFlag, &ExportabilityLens.cbExchExportability)))
        {
            goto Ret;
        }
    }
    ...
    if (pContInfo->ContLens.cbSigPub || pContInfo->ContLens.cbSigEncPriv)
    {
        ...
        memcpy(pb + cb, pContInfo->pbSigEncPriv, pContInfo->ContLens.cbSigEncPriv);
        cb += pContInfo->ContLens.cbSigEncPriv;

        // write the exportability info for the sig key
        memcpy(pb + cb, pbProtectedSigExportFlag, ExportabilityLens.cbSigExportability);
    ...
    if (pContInfo->ContLens.cbExchPub || pContInfo->ContLens.cbExchEncPriv)
    {
        ...
        memcpy(pb + cb, pContInfo->pbExchEncPriv, pContInfo->ContLens.cbExchEncPriv);
        cb += pContInfo->ContLens.cbExchEncPriv;

        // write the exportability info for the sig key
        memcpy(pb + cb, pbProtectedExchExportFlag, ExportabilityLens.cbExchExportability);
    ...
    if(ERROR_SUCCESS != OpenFileInStorageArea(fMachineKeyset, GENERIC_WRITE, pwszFilePath, pwszFileName, &hFile))
    {
        dwErr = (DWORD)NTE_FAIL;
        goto Ret;
    }
...
DWORD DeleteKeyContainer(IN LPWSTR pwszFilePath, IN LPSTR pszContainer)
{
    LPWSTR  pwszFileName = NULL;
    ...
    if (fRetryWithHashedName)
    {
        if (0 != (dwErr = AddMachineGuidToContainerName(pszContainer, rgwchNewFileName)))
        {
            goto Ret;
        }
...
DWORD DeleteContainerInfo(IN DWORD dwProvType, IN LPSTR pszContainer, IN BOOL fMachineKeyset)
{
    LPWSTR  pwszFilePath = NULL;
    HANDLE  hFile = INVALID_HANDLE_VALUE;
    BOOL    fIsLocalSystem = FALSE;
    ...
    if (0 != (dwErr = DeleteKeyContainer(pwszFilePath, pszContainer)))
    {
        ...
        if (fMachineKeyset)
        {
            ...
            if (0 != (dwErr = GetUserStorageArea(dwProvType, fMachineKeyset, TRUE, &fIsLocalSystem, &pwszFilePath)))
            {
                goto Ret;
            }
    ...
    while (1)
    {
        ...
        if (0 != FindClosestFileInStorageArea(pwszFilePath, pszContainer, rgwchNewFileName, &hFile))
        {
            break;
        }
...
DWORD ReadContainerNameFromFile(IN BOOL fMachineKeyset, IN LPWSTR pwszFileName, IN LPWSTR pwszFilePath, OUT LPSTR pszNextContainer, IN OUT DWORD *pcbNextContainer)
{
    HANDLE              hMap = NULL;
    BYTE                *pbFile = NULL;
    HANDLE              hFile = INVALID_HANDLE_VALUE;
    ...
    if ((sizeof(DWORD) + sizeof(KEY_CONTAINER_LENS) + sizeof(KEY_EXPORTABILITY_LENS) + pContLens->cbName) > cbFile)
    {
        dwErr = (DWORD)NTE_BAD_KEYSET;
        goto Ret;
    }
    ...
    memcpy(pszNextContainer, pbFile + sizeof(DWORD) + sizeof(KEY_CONTAINER_LENS) + sizeof(KEY_EXPORTABILITY_LENS), pContLens->cbName);
...
DWORD GetUniqueContainerName(IN KEY_CONTAINER_INFO *pContInfo, OUT BYTE *pbData, OUT DWORD *pcbData)
{
    ...
    if (0 == (cch = WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, pContInfo->rgwszFileName, -1, NULL, cch, NULL, NULL)))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    ...
    if (0 == (cch = WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, pContInfo->rgwszFileName, -1, pszUniqueContainer, cch, NULL, NULL)))
    {
        dwErr = GetLastError();
        goto Ret;
    }
...
BOOL MachineGuidInFilename(LPWSTR pwszFileName, LPWSTR pwszMachineGuid)
{
    DWORD   cbFileName;
    BOOL    fRet = FALSE;
    ...
    if (0 == memcmp(pwszMachineGuid, &(pwszFileName[cbFileName - 36]), 36 * sizeof(WCHAR)))
        fRet = TRUE;
    ...
    )
{
    ...
    // get the correct storage area (directory)
    if (0 != (dwLastError = GetUserStorageArea(dwProvType, fMachineKeyset, FALSE, &fIsLocalSystem, &pwszFilePath)))
    {
        goto Ret;
    }
    ...
    if (dwFlags & CRYPT_FIRST)
    {
        *phFind = INVALID_HANDLE_VALUE;
        ...
        if (NULL == (pwszEnumFilePath = (LPWSTR)ContInfoAlloc((wcslen(pwszFilePath) + 2) * sizeof(WCHAR))))
        {
            dwLastError = ERROR_NOT_ENOUGH_MEMORY;
            goto Ret;
        }
        ...
        *phFind = FindFirstFileExW(pwszEnumFilePath, FindExInfoStandard, &FindData, FindExSearchNameMatch, NULL, 0);
...
GetNextFile:
    ...
    // we try to get the next file if failure occurs in case the file was deleted since the FindNextFile
    if (NTE_BAD_KEYSET == (dwLastError = ReadContainerNameFromFile(fMachineKeyset, FindData.cFileName, pwszFilePath, pszNextContainer, pcbNextContainer)))
    {
        goto GetNextFile;
    }
...
DWORD MyRegOpenKeyEx(IN HKEY hRegKey, IN LPSTR pszKeyName, IN DWORD dwReserved, IN REGSAM SAMDesired, OUT HKEY *phNewRegKey)
{
    ...
    if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszKeyName, -1, NULL, cch)))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    ...
    if (0 == (cch = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, pszKeyName, -1, pwsz, cch)))
    {
        dwErr = GetLastError();
        goto Ret;
    }

    dwErr = RegOpenKeyExW(hRegKey, pwsz, dwReserved, SAMDesired, phNewRegKey);
...
DWORD AllocAndSetLocationBuff(BOOL fMachineKeySet, DWORD dwProvType, CONST char *pszUserID, HKEY *phTopRegKey, TCHAR **ppszLocBuff, BOOL fUserKeys, BOOL *pfLeaveOldKeys)
{
    CHAR    szSID[MAX_PATH];
...
//  DWORD   i;
    ...
    if (fMachineKeySet)
    {
        *phTopRegKey = HKEY_LOCAL_MACHINE;
        if ((PROV_RSA_FULL == dwProvType) || (PROV_RSA_SCHANNEL == dwProvType))
        {
...
//  DWORD               cb;
    ...
    if (0 != (dwErr = GetUserStorageArea(dwProvType, TRUE, TRUE, &fIsLocalSystem, &pwszUserStorageArea)))
    {
    ...
    if (INVALID_HANDLE_VALUE == (hFind = FindFirstFileExW(pwszTmp, FindExInfoStandard, &FindData, FindExSearchNameMatch, NULL, 0)))
    {
        dwErr = ERROR_NO_MORE_ITEMS;
        goto Ret;
    }
    ...
    if (NULL == (pContInfo->pchEnumOldMachKeyEntries = ContInfoAlloc(i * (MAX_PATH + 1))))
    {
        dwErr = ERROR_NOT_ENOUGH_MEMORY;
        goto Ret;
    }
    ...
    for (i = 0; i < pContInfo->cMaxOldMachKeyEntry; i++)
    {
        ...
        // return the container name, in order to do that we need to open the
        // file and pull out the container name
        if (0 != (dwErr = ReadContainerNameFromFile(TRUE, FindData.cFileName, pwszUserStorageArea, pszNextContainer, &cbNextContainer)))
        {
...
DWORD GetNextEnumedOldMachKeys(IN PKEY_CONTAINER_INFO pContInfo, IN BOOL fMachineKeyset, IN DWORD dwProvType, OUT BYTE *pbData, OUT DWORD *pcbData)
{
    CHAR    *psz;
    DWORD   dwErr = 0;
    ...
    if ((NULL == pContInfo->pchEnumOldMachKeyEntries) || (pContInfo->dwiOldMachKeyEntry >= pContInfo->cMaxOldMachKeyEntry))
    {
        dwErr = ERROR_NO_MORE_ITEMS;
        goto Ret;
    }
    ...
    psz = pContInfo->pchEnumOldMachKeyEntries + (pContInfo->dwiOldMachKeyEntry * pContInfo->cbOldMachKeyEntry);
...
DWORD EnumRegKeys(IN OUT PKEY_CONTAINER_INFO pContInfo, IN BOOL fMachineKeySet, IN DWORD dwProvType, OUT BYTE *pbData, IN OUT DWORD *pcbData)
{
    ...
    if (0 == (cbBuff = AllocAndSetLocationBuff(fMachineKeySet, dwProvType, pContInfo->pszUserName, &hTopRegKey, &pszBuff, TRUE, &fLeaveOldKeys)))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    ...
    // open the reg key
    if ((dwErr = MyRegOpenKeyEx(hTopRegKey, pszBuff, 0, KEY_READ, &hKey)) != ERROR_SUCCESS)
    {
        dwErr = ERROR_NO_MORE_ITEMS;
        goto Ret;
    }
    ...
    if ((dwErr = RegQueryInfoKey(hKey, NULL, NULL, NULL, &cSubKeys, &cchMaxSubkey, &cchMaxClass, &cValues, &cchMaxValueName, &cbMaxValueData, &cbSecurityDesriptor, &ftLastWriteTime)) != ERROR_SUCCESS)
    {
        goto Ret;
    }
    ...
    if (0 != cSubKeys)
    {
        ...
        if (NULL == (pContInfo->pchEnumRegEntries = ContInfoAlloc(pContInfo->cMaxRegEntry * pContInfo->cbRegEntry * sizeof(CHAR))))
        {
            dwErr = ERROR_NOT_ENOUGH_MEMORY;
            goto Ret;
        }
        ...
        for (i = 0; i < pContInfo->cMaxRegEntry; i++)
        {
            ...
            if ((dwErr = RegEnumKey(hKey, i, psz, pContInfo->cbRegEntry)) != ERROR_SUCCESS)
            {
                goto Ret;
            }
        }
...
Ret:
    if (hTopRegKey && (HKEY_CURRENT_USER != hTopRegKey) && (HKEY_LOCAL_MACHINE != hTopRegKey))
    {
...
DWORD GetNextEnumedRegKeys(IN PKEY_CONTAINER_INFO pContInfo, IN BOOL fMachineKeySet, IN DWORD dwProvType, OUT BYTE *pbData, OUT DWORD *pcbData)
{
    CHAR    *psz;
    DWORD   dwErr = 0;

    if ((NULL == pContInfo->pchEnumRegEntries) || (pContInfo->dwiRegEntry >= pContInfo->cMaxRegEntry))
    {
        dwErr = ERROR_NO_MORE_ITEMS;
        goto Ret;
    }
    ...
    psz = pContInfo->pchEnumRegEntries + (pContInfo->dwiRegEntry * pContInfo->cbRegEntry);
...
BOOL OpenRegKeyWithTokenPriviledges(IN HKEY hTopRegKey, IN LPSTR pszRegKey, OUT HKEY *phRegKey, OUT DWORD *pdwFlags)
{
    TOKEN_PRIVILEGES    tp;
    ...
    if (ERROR_SUCCESS != MyRegOpenKeyEx(hTopRegKey, pszRegKey, 0, KEY_ALL_ACCESS, &hRegKey))
    {
        goto Ret;
    }
    RegCloseKey(hRegKey);
    ...
    if (FALSE == (fStatus = OpenThreadToken(GetCurrentThread(), MAXIMUM_ALLOWED, TRUE, &hToken)))
    {
        ...
        fStatus = OpenThreadToken(GetCurrentThread(), MAXIMUM_ALLOWED, TRUE, &hToken);
    }
    ...
    if (fStatus)
    {
        ...
        fStatus = LookupPrivilegeValueA(NULL, SE_SECURITY_NAME, &luid);
        if (fStatus)
        {
            ...
            // adjust privilege
            fStatus = AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), &tpPrevious, &cbPrevious);
    ...
    if (ERROR_SUCCESS != MyRegOpenKeyEx(hTopRegKey, pszRegKey, 0, KEY_ALL_ACCESS | dwAccessFlags, phRegKey))
    {
        goto Ret;
    }
...
Ret:
    ...
    if (fAdjusted)
    {
        ...
        fStatus = AdjustTokenPrivileges(hToken, FALSE, &tpPrevious, sizeof(TOKEN_PRIVILEGES), NULL, NULL);
    }
    ...
    if (dwOldKeyFlags & PRIVILEDGE_FOR_SACL)
    {
        ...
        if (fStatus)
        {
            ...
            if (fStatus)
            {
                ...
                // adjust privilege
                fAdjusted = AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), &tpPrevious, &cbPrevious);
            }
        }
    }

    if (ERROR_SUCCESS != (dwErr = SetSecurityOnContainer(wszFileName, dwProvType, fMachineKeyset, SecurityInformation, pSecurityDescriptor)))
    {
        goto Ret;
    }
...
// for registry keys and converts the access mask so generic access bits are used
DWORD CheckAndChangeAccessMasks(IN PACL pAcl)
{
    ...
    for (i=0;i<AclSizeInfo.AceCount;i++)
    {
        ...
        if ((pAce->Mask & KEY_SET_VALUE) || (pAce->Mask & GENERIC_ALL) || (pAce->Mask & GENERIC_WRITE))
        {
            NewMask |= GENERIC_ALL;
        }
...
DWORD ConvertContainerSecurityDescriptor(IN PSECURITY_DESCRIPTOR pSecurityDescriptor, OUT PSECURITY_DESCRIPTOR *ppNewSD, OUT DWORD *pcbNewSD)
{
    DWORD                       cbSD;
    ...
    if (!GetSecurityDescriptorControl(pSecurityDescriptor, &Control, &dwRevision))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    ...
    if (NULL == (*ppNewSD = (PSECURITY_DESCRIPTOR)ContInfoAlloc(cbSD)))
    {
        dwErr = ERROR_NOT_ENOUGH_MEMORY;
        goto Ret;
    }
    ...
    if (!GetSecurityDescriptorDacl(*ppNewSD, &fDACLPresent, &pDacl, &fDaclDefaulted))
    {
        dwErr = GetLastError();
        goto Ret;
    }
    ...
    if (!GetSecurityDescriptorSacl(*ppNewSD, &fSACLPresent, &pSacl, &fSaclDefaulted))
    {
        dwErr = GetLastError();
        goto Ret;
    }
...
DWORD SetSecurityOnContainer(IN LPCWSTR wszFileName, IN DWORD dwProvType, IN DWORD fMachineKeyset, IN SECURITY_INFORMATION SecurityInformation, IN PSECURITY_DESCRIPTOR pSecurityDescriptor)
{
    ...
    if (0 != (dwErr = ConvertContainerSecurityDescriptor(pSecurityDescriptor, &pSD, &cbSD)))
    {
        goto Ret;
    }

    // get the correct storage area (directory)
    if (0 != (dwErr = GetUserStorageArea(dwProvType, fMachineKeyset, FALSE, &fIsLocalSystem, &wszUserStorageArea)))
    {
        goto Ret;
    }
                             ...
                             )
{
    ...
    cbSD = sizeof(cbSD);
    pSD = &cbSD;
    if (!GetFileSecurityW(wszFilePath, RequestedInformation, pSD, cbSD, &cbSD))
    {
...
*/
{
    ...
    *pfInFrance = FALSE;
    ...
    // Check if the default language is Standard French or if the users's country is set to FRANCE
...
BOOL InitExpOffloadInfo(IN OUT PEXPO_OFFLOAD_STRUCT *ppOffloadInfo)
{
    ...
    __try
    {
        ...
        if (ERROR_SUCCESS != (dwErr = RegQueryValueEx(hOffloadRegKey, EXPO_OFFLOAD_REG_VALUE, 0, NULL, rgbModule, &cbModule)))
        {
            if (ERROR_MORE_DATA == dwErr)
            {
                ...
                fAlloc = TRUE;
                if (ERROR_SUCCESS != (dwErr = RegQueryValueEx(HKEY_LOCAL_MACHINE, EXPO_OFFLOAD_REG_VALUE, 0, NULL, pbModule, &cbModule)))
                {
                    goto Ret;
                }
            }
        ...
        if (NULL == (pTmpOffloadInfo = (PEXPO_OFFLOAD_STRUCT)ContInfoAlloc(sizeof(EXPO_OFFLOAD_STRUCT))))
        {
            goto Ret;
        }
        ...
        if (NULL == (pTmpOffloadInfo->hInst = LoadLibraryEx(pbModule, NULL, 0)))
        {
            goto Ret;
        }

        if (NULL == (pTmpOffloadInfo->pExpoFunc = GetProcAddress(pTmpOffloadInfo->hInst, EXPO_OFFLOAD_FUNC_NAME)))
        {
            goto Ret;
        }
                       ...
                       )
{
    ...
    __try
    {
        ...
        if (!pOffloadInfo->pExpoFunc(pbBase, pbExpo, cbExpo, pbModulus, cbModulus, pbResult, pReserved, dwFlags))
        {
            goto Ret;
        }
...
// allows the resources to be localize even though the CSPs themselves are signed.
    ...
    )
{
    ...
    if (0 == (cchMessage = LoadStringW(hModule, dwResourceId, szMessage, MAX_STRING_RSC_SIZE)))
    {
        goto Ret;
    }
    ...
    if(*pdwRemainingBufferSize < ((cchMessage + 1) * sizeof(WCHAR)))
    {
        ...
        if (NULL == (*ppStringBlock = (BYTE*)ContInfoReAlloc(*ppStringBlock, dwNewSize)))
        {
            goto Ret;
        }
    ...
    *ppString = (LPWSTR)(*ppStringBlock + *pdwBufferSize - *pdwRemainingBufferSize);
...
BOOL LoadStrings()
{
    ...
    if (NULL == g_pbStringBlock)
    {
        ...
        if(!FetchString(hMod, IDS_CSP_RSA_SIG_DESCR, &g_Strings.pwszRSASigDescr, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_RSA_EXCH_DESCR, &g_Strings.pwszRSAExchDescr, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_IMPORT_SIMPLE, &g_Strings.pwszImportSimple, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_SIGNING_E, &g_Strings.pwszSignWExch, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_CREATE_RSA_SIG, &g_Strings.pwszCreateRSASig, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_CREATE_RSA_EXCH, &g_Strings.pwszCreateRSAExch, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_DSS_SIG_DESCR, &g_Strings.pwszDSSSigDescr, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_DSS_EXCH_DESCR, &g_Strings.pwszDHExchDescr, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_CREATE_DSS_SIG, &g_Strings.pwszCreateDSS, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_CREATE_DH_EXCH, &g_Strings.pwszCreateDH, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_IMPORT_E_PUB, &g_Strings.pwszImportDHPub, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_MIGR, &g_Strings.pwszMigrKeys, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_DELETE_SIG, &g_Strings.pwszDeleteSig, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_DELETE_KEYX, &g_Strings.pwszDeleteExch, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_DELETE_SIG_MIGR, &g_Strings.pwszDeleteMigrSig, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_DELETE_KEYX_MIGR, &g_Strings.pwszDeleteMigrExch, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_SIGNING_S, &g_Strings.pwszSigning, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_EXPORT_E_PRIV, &g_Strings.pwszExportPrivExch, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_EXPORT_S_PRIV, &g_Strings.pwszExportPrivSig, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_IMPORT_E_PRIV, &g_Strings.pwszImportPrivExch, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }

        if(!FetchString(hMod, IDS_CSP_IMPORT_S_PRIV, &g_Strings.pwszImportPrivSig, &g_pbStringBlock, &dwBufferSize, &dwRemainingBufferSize))
        {
            goto Ret;
        }
    }
...
BOOL GetRNGDriverHandle(IN OUT HANDLE *phDriver)
{
    ...
    if (INVALID_HANDLE_VALUE == (hDriver = CreateFileA(szDeviceName, FILE_SHARE_READ | FILE_SHARE_WRITE | GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)))
    {
        SetLastError((DWORD) NTE_FAIL);
        goto Ret;
    }
    ...
    fReturnCode = DeviceIoControl(hDriver, IOCTL_ISD_GetCapability, &ISD_Cap, sizeof(ISD_Cap), &ISD_Cap, sizeof(ISD_Cap), &dwBytesReturned, NULL);
...
// Description : Checks if the INTEL RNG driver is available, if so then checks if the chipset supports the hardware RNG.
...
//               is also mixed in.
unsigned int HWRNGGenRandom(IN HANDLE hRNGDriver, IN OUT BYTE *pbBuffer, IN DWORD dwLen)
{
    ...
    for (i = 0; i < (dwLen / sizeof(DWORD)); i++)
    {
        ...
        fReturnCode = DeviceIoControl(hRNGDriver, IOCTL_ISD_GetRandomNumber, &ISD_Random, sizeof(ISD_Random), &ISD_Random, sizeof(ISD_Random), &dwBytesReturned, NULL);
...
//               is to be used.  If the registry entry is there then it attempts to get the HW RNG driver handle.
BOOL SetupHWRNGIfRegistered(OUT HANDLE *phRNGDriver)
{
    HKEY    hRegKey = 0;
    BOOL    fRet = FALSE;
...
void MACBytes(IN DESTable *pDESKeyTable, IN BYTE *pbData, IN DWORD cbData, IN OUT BYTE *pbTmp, IN OUT DWORD *pcbTmp, IN OUT BYTE *pbMAC, IN BOOL fFinal)
{
    DWORD   cb = cbData;
    DWORD   cbMACed = 0;
    ...
    while (cb)
    {
        ...
        else
        {
            ...
            CBC(des, DES_BLOCKLEN, pbMAC, pbTmp, pDESKeyTable, ENCRYPT, pbMAC);
...
// Given hInst, allocs and returns pointers to MAC pulled from resource
BOOL GetResourcePtr(IN HMODULE hInst, IN LPSTR pszRsrcName, OUT BYTE **ppbRsrcMAC, OUT DWORD *pcbRsrcMAC)
{
    HRSRC   hRsrc;
    BOOL    fRet = FALSE;

    // Nab resource handle for our signature
    if (NULL == (hRsrc = FindResourceA(hInst, pszRsrcName, RT_RCDATA)))
        goto Ret;
...
BOOL MACBytesOfFile(IN HANDLE hFile, IN DWORD cbToBeMACed, IN DESTable *pDESKeyTable, IN BYTE *pbTmp, IN DWORD *pcbTmp, IN BYTE *pbMAC, IN BYTE fFinal)
{
    ...
    while (cbRemaining > 0)
    {
        ...
        MACBytes(pDESKeyTable, rgbChunk, dwBytesRead, pbTmp, pcbTmp, pbMAC, fFinal);
...
BOOL MACTheFile(LPCSTR pszImage, DWORD cbImage)
{
    HMODULE                     hInst = 0;
    ...
    hMapping = CreateFileMapping((HANDLE)IntToPtr(hFile), NULL, PAGE_READONLY, 0, 0, NULL);
    if(hMapping == NULL)
    {
        goto Ret;
    }

    hInst = MapViewOfFile(hMapping, FILE_MAP_READ, 0, 0, 0);
    if(hInst == NULL)
    {
        goto Ret;
    }
    ...
    *((ULONG_PTR*)&hInst) |= 0x00000001;
    ...
    // MAC up to the CRC
    if (!MACBytesOfFile((HANDLE)IntToPtr(hFile), dwCRCOffset, &DESKeyTable, rgbTmp, &cbTmp, rgbMAC, FALSE))
    {
        goto Ret;
    }

    // pretend CRC is zeroed
    MACBytes(&DESKeyTable, (BYTE*)&dwZeroCRC, sizeof(DWORD), rgbTmp, &cbTmp, rgbMAC, FALSE);
    ...
    if (!MACBytesOfFile((HANDLE)IntToPtr(hFile), cbCRCToRsrc1, &DESKeyTable, rgbTmp, &cbTmp, rgbMAC, FALSE))
    {
        goto Ret;
    }

    // pretend image has zeroed first resource
    MACBytes(&DESKeyTable, (BYTE*)pbZeroRsrc1, cbZeroRsrc1, rgbTmp, &cbTmp, rgbMAC, FALSE);
    ...
    if (!MACBytesOfFile((HANDLE)IntToPtr(hFile), cbRsrc1ToRsrc2, &DESKeyTable, rgbTmp, &cbTmp, rgbMAC, FALSE))
    {
        goto Ret;
    }

    // pretend image has zeroed second Resource
    MACBytes(&DESKeyTable, (BYTE*)pbZeroRsrc2, cbZeroRsrc2, rgbTmp, &cbTmp, rgbMAC, FALSE);
    ...
    if (!MACBytesOfFile((HANDLE)IntToPtr(hFile), cbPostRsrc, &DESKeyTable, rgbTmp, &cbTmp, rgbMAC, TRUE))
    {
        goto Ret;
    }
...
BOOL SelfMACCheck(IN LPSTR pszImage)
{
...
BOOL CSPCheckIfShouldDisplayOutput()
{
    ...
    if (ERROR_SUCCESS != RegOpenKeyEx(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Cryptography\\DebugInfo", 0, KEY_READ, &hRegKey))
    {
        goto Ret;
    }
...
void CSPDebugOutput(IN char *szOutString)
{
/*
    DWORD dwWritten;
    ...
    if (NULL != g_hfLogFile)
    {
        WriteFile(g_hfLogFile, szOutString, lstrlen(szOutString), &dwWritten, NULL);
    }
*/
...
void CSPDumpAddress(IN BYTE *pb, IN OUT LPSTR psz)
{
...
void CSPDumpHexString(IN BYTE *pb, IN DWORD cb)
{
    DWORD   i;
    CHAR    pszLine[MAX_PATH];
...
void OutputDebugHeader(IN LPSTR pszFunc)
{
    SYSTEMTIME  stTime;
    DWORD       Level = 0;
//  ULONG       ClientProcess;
//  ULONG       ClientThread;
    ...
    cbOut = wsprintf(szOutString, "[%2d/%2d %02d:%02d:%02d.%03d] %d.%d\n", stTime.wMonth, stTime.wDay, stTime.wHour, stTime.wMinute, stTime.wSecond, stTime.wMilliseconds, GetCurrentProcessId(), GetCurrentThreadId());
    CSPDebugOutput(szOutString);
}


void CSPDebugOutputFuncReturn(IN BOOL fReturn)
{
    CHAR    pszTmp[MAX_PATH];
...
void CSPDebugOutputAcqCtxt(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV *phProv, IN CHAR *pUserID, IN DWORD dwFlags, IN PVTableProvStruc pVTable)
{
...
void CSPDebugOutputReleaseCtxt(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputSetProvParam(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN DWORD dwParam, IN BYTE *pbData, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputGetProvParam(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN DWORD dwParam, IN BYTE *pbData, IN DWORD *pdwDataLen, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputDeriveKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN ALG_ID Algid, IN HCRYPTHASH hHash, IN DWORD dwFlags, IN HCRYPTKEY *phKey)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputDestroyKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTKEY hProv, IN HCRYPTKEY hKey)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputGenKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTKEY hProv, IN ALG_ID Algid, IN DWORD dwFlags, IN HCRYPTKEY *phKey)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputGetKeyParam(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTKEY hKey, IN DWORD dwParam, IN BYTE *pbData, IN DWORD *pdwDataLen, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputGetUserKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN DWORD dwKeySpec, IN HCRYPTKEY *phKey)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputSetKeyParam(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTKEY hKey, IN DWORD dwParam, IN BYTE *pbData, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputGenRandom(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN DWORD dwLen, IN BYTE *pbBuffer)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputExportKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTKEY hKey, IN HCRYPTKEY hExpKey, IN DWORD dwBlobType, IN DWORD dwFlags, IN BYTE *pbData, IN DWORD *pdwDataLen)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputImportKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN BYTE *pbData, IN DWORD dwDataLen, IN HCRYPTKEY hImpKey, IN DWORD dwFlags, IN HCRYPTKEY *phKey)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputDuplicateKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTKEY hKey, IN DWORD *pdwReserved, IN DWORD dwFlags, IN HCRYPTKEY *phKey)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputGetHashParam(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash, IN DWORD dwParam, IN BYTE *pbData, IN DWORD *pdwDataLen, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputSetHashParam(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash, IN DWORD dwParam, IN BYTE *pbData, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputEncrypt(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hKey, IN HCRYPTHASH hHash, IN BOOL Final, IN DWORD dwFlags, IN BYTE *pbData, IN DWORD *pdwDataLen, IN DWORD dwBufLen)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputDecrypt(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hKey, IN HCRYPTHASH hHash, IN BOOL Final, IN DWORD dwFlags, IN BYTE *pbData, IN DWORD *pdwDataLen)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputSignHash(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash, IN DWORD dwKeySpec, IN LPCWSTR pszDescription, IN DWORD dwFlags, IN BYTE *pbSignature, IN DWORD *pdwSigLen)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputVerifySignature(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash, IN BYTE *pbSignature, IN DWORD dwSigLen, IN HCRYPTKEY hPubKey, IN LPCWSTR pszDescription, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputCreateHash(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN ALG_ID Algid, IN HCRYPTKEY hKey, IN DWORD dwFlags, IN HCRYPTHASH *phHash)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputDestroyHash(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputHashData(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash, IN BYTE *pbData, IN DWORD dwDataLen, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputHashSessionKey(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash, IN HCRYPTKEY hKey, IN DWORD dwFlags)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
...
void CSPDebugOutputDuplicateHash(IN BOOL fEnter, IN BOOL fReturn, IN HCRYPTPROV hProv, IN HCRYPTHASH hHash, IN DWORD *pdwReserved, IN DWORD dwFlags, IN HCRYPTHASH *phHash)
{
    if (CSPCheckIfShouldDisplayOutput())
    {
        if (fEnter)
