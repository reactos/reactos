...
BOOL SmallTest(DWORD dw)
{
    ...
    if (ERROR_SUCCESS != RegCreateKeyEx(HKEY_CURRENT_USER, MY_TEST_REG_ENTRY, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hRegKey, &dwDisposition))
        goto Ret;

    if (ERROR_SUCCESS != RegSetValueEx(hRegKey, "AutoEnrollTest", 0, REG_BINARY, (BYTE*)&dw, sizeof(dw)))
        goto Ret;
...
BOOL EnrollForACert(IN BOOL fMachineEnrollment, IN BOOL fRenewalRequired, IN PAUTO_ENROLL_INFO pInfo)
{
    ...
    if (fMachineEnrollment)
    {
        ...
        if (0 == GetComputerNameW(rgwszMachineName, &cMachineName))
        {
            goto Ret;
        }
    ...
    if (!CryptUIWizCertRequest(CRYPTUI_WIZ_NO_UI, 0, NULL, &CertRequestInfo, &pCertContext, &dwCAStatus))
    {
    ...
    if (CRYPTUI_WIZ_CERT_REQUEST_STATUS_SUCCEEDED == dwCAStatus)
    {
        ...
        if(!CertGetCertificateContextProperty(pCertContext, CERT_SHA1_HASH_PROP_ID, blobHash.pbData, &blobHash.cbData))
        {
            AutoEnrollErrorLogging(GetLastError());
            goto Ret;
        }

        pCert =  CertFindCertificateInStore(pInfo->hMYStore, pCertContext->dwCertEncodingType, 0, CERT_FIND_SHA1_HASH, &blobHash, NULL);
        if(pCert == NULL)
        {
            AutoEnrollErrorLogging(GetLastError());
            goto Ret;
        ...
        if (!CertSetCertificateContextProperty(pCert, CERT_AUTO_ENROLL_PROP_ID, 0, &CryptData))
        {
            AutoEnrollErrorLogging(GetLastError());
            goto Ret;
        }
...
BOOL AutoEnrollWait(VOID)
/*
    This routine determines if the protected storage service is pending start.
    If the service is pending start, this routine waits until the service is running before returning to the caller.
    ...
    When the return value is FALSE, the value is only advisory, and may not indicate the current state of the service.
    The reasoning here is that if the service did not start the first time this call is made,
    is will not likely be running the next time around, and hence we avoid checking on subsequent calls.

    For current situations, the caller should ignore the return value;
    when the return value is FALSE, the caller should just try making the call into the service.
    If the service is still down, the call into it will fail appropriately.
...
--*/
{
    ...
    // open the protected storage service so we can query it's current state.
    ...
    if( OpenProcessToken( GetCurrentProcess(), TOKEN_QUERY, &hToken ) ) {
        do {
            ...
            if(!GetTokenInformation(hToken, TokenUser, TokenInformation, cbTokenInformation, &cbTokenInformation))
                break;

            if(!AllocateAndInitializeSid(&sia, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0, 0, 0, 0, 0, &psidLocalSystem))
                break;
...
#define SLEEP_SECONDS (5)
    ...
    if( fSystemAccount ) {
        dwStopCount = 900 / SLEEP_SECONDS;// 15 minutes for SYSTEM account.
    } else {
        // loop checking the service status every 5 seconds, for up to 2 minutes total (120 seconds, 5*24=120)
    ...
    for( ; dwStopCount != 0 ; dwStopCount--, Sleep(SLEEP_SECONDS*1000) ) {
        ...
        if( sServiceStatus.dwCurrentState == SERVICE_STOPPED && sServiceStatus.dwWin32ExitCode == ERROR_SERVICE_NEVER_STARTED ) {
            continue;
        }
...
//          pInfo - information needed to enroll (see AUTO_ENROLL_INFO struct in autoenrl.h)

//  History:    01-12-98   jeffspel   Created
BOOL ProvAutoEnrollment(IN BOOL fMachineEnrollment, IN PAUTO_ENROLL_INFO pInfo)
{
