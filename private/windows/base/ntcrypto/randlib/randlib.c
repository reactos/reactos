...
typedef NTSYSAPI VOID (NTAPI *RTLINITUNICODESTRING) (PUNICODE_STRING DestinationString, PCWSTR SourceString);
typedef BOOL (WINAPI *GETCURSORPOS)(LPPOINT lpPoint);
...
//    *pdwTmp = NtGetTickCount();
...
#else
    {
        ...
        _NtQuerySystemInformation(SystemTimeOfDayInformation, pTimeOfDay, sizeof(*pTimeOfDay), &cbSystemInfo);
...
//        ZwQueryPerformanceCounter(pliPerfCount, &liPerfFreq);
...
#endif  // !KMODE_RNG
    {
        ...
        if( (void*)_NtQuerySystemInformation ) {
            ...
            Status = _NtQuerySystemInformation(SystemProcessorPerformanceInformation, pSystemProcessorPerformanceInfo, sizeof(*pSystemProcessorPerformanceInfo), &cbSystemInfo);
            if ( NT_SUCCESS(Status) ) {
                UPDATE_BUFFER( cbSystemInfo );
            }
            ...
            Status = _NtQuerySystemInformation(SystemPerformanceInformation, pSystemPerformanceInfo, sizeof(*pSystemPerformanceInfo), &cbSystemInfo);
            if ( NT_SUCCESS(Status) ) {
                UPDATE_BUFFER(  cbSystemInfo );
            }
            ...
            Status = _NtQuerySystemInformation(SystemExceptionInformation, pSystemExceptionInfo, sizeof(*pSystemExceptionInfo), &cbSystemInfo);
            if ( NT_SUCCESS(Status) ) {
                UPDATE_BUFFER( cbSystemInfo );
            }
            ...
            Status = _NtQuerySystemInformation(SystemLookasideInformation, pSystemLookasideInfo, sizeof(*pSystemLookasideInfo), &cbSystemInfo);
            if ( NT_SUCCESS(Status) ) {
                UPDATE_BUFFER( cbSystemInfo );
            }
            ...
            Status = _NtQuerySystemInformation(SystemInterruptInformation, pSystemInterruptInfo, cbSystemInfo, &cbSystemInfo);
            if ( NT_SUCCESS(Status) ) {
                UPDATE_BUFFER( cbSystemInfo );
            }
            ...
            Status = _NtQuerySystemInformation(SystemProcessInformation, pSystemProcessInfo, cbSystemInfo, &cbSystemInfo);
            if ( NT_SUCCESS(Status) ) {
                UPDATE_BUFFER( cbSystemInfo );
            }
...
--*/
{
    ...
    if( hFile == NULL ) {
        ...
        // have to use the Nt flavor of the file open call because it's a base device not aliased to \DosDevices
        ...
        hPreviousValue = INTERLOCKEDCOMPAREEXCHANGEPOINTER(&g_hKsecDD, hFile, NULL);
...
BOOL IsRNGWinNT(VOID)
/*++
    This function determines if we are running on Windows NT and furthermore,
    if it is appropriate to make use of certain user operations where the code is running.

    If the function returns TRUE, the caller cannot make calls to user
    based function and should use an alternative approach such as NtQuerySystemInformation.
...
--*/
{
    ...
    if( GetVersionEx(&osVer) ) {
        ...
        if( fIsWinNT ) {
            ...
            if( hNTDll ) {
                _NtQuerySystemInformation = (NTQUERYSYSTEMINFORMATION)GetProcAddress(hNTDll, "NtQuerySystemInformation");
                ...
                _RtlInitUnicodeString = (RTLINITUNICODESTRING)GetProcAddress(hNTDll, "RtlInitUnicodeString");
            }
        } else {
            ...
            if( hUser32 ) {
                _GetCursorPos = (GETCURSORPOS)GetProcAddress(hUser32, "GetCursorPos");
                _GetMessageTime = (GETMESSAGETIME)GetProcAddress(hUser32, "GetMessageTime");
            }
        }
    }
