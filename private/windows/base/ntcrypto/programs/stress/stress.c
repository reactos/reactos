...
int __cdecl main(int cArg, char *rgszArg[])
{
    ...
    if ((bRanbuf = VirtualAlloc(0, dRandom, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)) == 0)
        {
            if (cArg > 1)
        {
    ...
    if ((bcryptbuf = VirtualAlloc(0, dRandom + 8, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)) == 0)
    {
            if (cArg > 1)
        {
            printf("malloc failed = %x\n", GetLastError());
