...
int UpdateChanges(LPSTR szScanFile);
int ScanChanges(LPSTR szScanFile);
BOOL CheckHash(IN LPSTR szFileToCheck, IN BYTE HashExpected[A_SHA_DIGEST_LEN], IN OUT BOOL *pfMatch);
BOOL GetHash(IN LPSTR szFileToHash, IN BYTE HashCurrent[A_SHA_DIGEST_LEN]);
...
BOOL TranslateToMapped(IN LPSTR szFileCandidate, IN OUT  LPSTR szFileMapped);
...
int UpdateChanges(LPSTR szScanFile)
{
    ...
    // this is done so we can just append all updated hashes to the end of the filtered input file.
    ...
    // if everything succeeded, append the intermediate hash entry file to the intermediate temporary file.
    if( iRet == RTN_OK ) {
        rewind( streamHashes );
    ...
    )
{
    ...
    *pfMatch = FALSE;
    ...
    hFile = CreateFileA(szFileToCheck, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
    ...
    )
{
    ...
    hFile = CreateFileA(szFileToHash, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
    if(hFile == INVALID_HANDLE_VALUE)
        return FALSE;
