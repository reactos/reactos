...
int __cdecl main(int cArg, char *rgszArg[])
{
    ...
    for (iLoopCount=0; iLoopCount<( sizeof(provList)/sizeof(PROVENTRY)); iLoopCount++)
    {
                       ...
                       0)) != INVALID_HANDLE_VALUE)
        {
            ...
            if ((lpvAddress = VirtualAlloc(NULL, NumBytes, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE)) == NULL)
            {
                CloseHandle(hFileSig);
            ...
            if (!ReadFile((HANDLE) hFileSig, lpvAddress, NumBytes, &NumBytesRead, 0))
            {
                CloseHandle(hFileSig);
        ...
        // Create or open in local machine for provider:
            ...
            if ((err = RegCreateKeyEx(HKEY_LOCAL_MACHINE, (const char *) provList[iLoopCount].szProvider, 0L, "", REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwIgn)) != ERROR_SUCCESS)
            {
        ...
        // Set Image path to dll
            ...
            if ((err = RegSetValueEx(hKey, "Image Path", 0L, REG_SZ, provList[iLoopCount].szImagePath, strlen(provList[iLoopCount].szImagePath)+1)) != ERROR_SUCCESS)
            {
        ...
        // Set Type
            ...
            if ((err = RegSetValueEx(hKey, "Type", 0L, REG_DWORD, (LPTSTR) &dwValue, sizeof(DWORD))) != ERROR_SUCCESS)
            {
        ...
        // Place signature
            ...
            if ((err = RegSetValueEx(hKey, "Signature", 0L, REG_BINARY, (LPTSTR) lpvAddress, NumBytes)) != ERROR_SUCCESS)
            {
        ...
        // Create or open in local machine for provider type: Type
            ...
            if ((err = RegCreateKeyEx(HKEY_LOCAL_MACHINE, (const char *) provList[iLoopCount].szType, 0L, "", REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwIgn)) != ERROR_SUCCESS)
            {
                printf("Install failed: Registry entry existed: %x\n", err);
            }

            // always set MS_DEF_PROV to be the default provider
            if ((err = RegSetValueEx(hKey, "Name", 0L, REG_SZ, MS_DEF_PROV, strlen(MS_DEF_PROV)+1)) != ERROR_SUCCESS)
            {
                printf("Install failed: Setting Default type: %x\n", err);
                return(FALSE);
            }

            if ((err = RegSetValueEx(hKey, "TypeName", 0L, REG_SZ, MS_RSA_TYPE, strlen(MS_RSA_TYPE)+1)) != ERROR_SUCCESS)
            {
