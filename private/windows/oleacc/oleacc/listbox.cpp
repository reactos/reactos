    ...
    else
    {
        ...
        if (cch)
        {
            ...
            if( IsTridentControl( m_hwnd, m_fComboBox, m_fComboBox ) )
            {
                ...
                lpszUnicodeText = (OLECHAR *)SharedAlloc((cch+1)*sizeof(OLECHAR), hwndAsk, &hProcess);
...
STDMETHODIMP CListBox::accLocation(long* pxLeft, long *pyTop, long* pcxWidth, long* pcyHeight, VARIANT varChild)
{
    RECT    rc;
...
STDMETHODIMP CListBoxFrame::accNavigate(VARIANT varStart, long dwNavDir, VARIANT* pvarEnd)
{
    COMBOBOXINFO    cbi;
    ...
    return(GetParentToNavigate(INDEX_COMBOBOX_LIST, cbi.hwndCombo, OBJID_CLIENT, dwNavDir, pvarEnd));
}
...
BOOL IsComboEx(HWND hwnd)
{
    ...
    return MyGetWindowClass(hwnd, szClass, ARRAYSIZE(szClass) ) && ! lstrcmpi(szClass, szComboExName);
}


//  Returns the COMBOEX window if the combo is embedded in a COMBOEX (like on the toolbar).
