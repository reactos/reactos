...
//  This allocates out of the shared heap on Win '95. On NT, we need to use VirtualAllocEx to allocate memory in the other process.
//  The caller of SharedAlloc will need to then use ReadProcessMemory to read the data from the VirtualAlloc'ed memory.
//  What I am going to do is create 2 new functions - SharedRead and SharedWrite, that will read and write shared memory.
//  On Win95, they will just use CopyMemory, but on NT they will use ReadProcessMemory and WriteProcessMemory.

//  Parameters:
//      UINT    cbSize      Size of the memory block required
//      HWND    hwnd        Window handle in the process to allocate the shared memory in.
//      HANDLE* pProcHandle Pointer to a handle that has the process handle filled in on return.
//                          This must be saved for use in calls to SharedRead, SharedWrite, and SharedFree.
//  Returns:
//      Pointer to the allocated memory, or NULL if it fails.
//      Access to the memory must be done using SharedRead and SharedWrite.
//      On success, pProcHandle is filled in as well.
...
void *  __cdecl operator new(unsigned int nSize)
{
    ...
    InterlockedIncrement( & g_MemInfo.m_NumAlloc );// Update statistics...
    return pv;// return pointer to alloc'd space...
}
...
void UninitMemChk()
{
    DBPRINTF( TEXT("Total objects: %d, unfreed: %d\n"), g_MemInfo.m_NumAlloc, g_MemInfo.m_NumAlloc - g_MemInfo.m_NumFree );
    DBPRINTF( TEXT("Total Shared objects: %d, unfreed: %d\n"), g_SharedMemInfo.m_NumAlloc, g_SharedMemInfo.m_NumAlloc - g_SharedMemInfo.m_NumFree );
}
