...
#define MAX_TRIES 20
    ...
    else
    {
        ...
        if (GetSubMenu(m_hMenu, 0) && (GetMenuState(m_hMenu, 0, MF_BYPOSITION) & MF_BITMAP))
            n++;
        ...
        // try several times -
        nTries = 0;
        while (((GetMenuState(m_hMenu, varChild.lVal - 1, MF_BYPOSITION) & MF_HILITE) == 0) && (nTries < MAX_TRIES))
        {
            Sleep(55);
            nTries++;
        }
                     ...
                     LPTSTR lpszBuf, UINT cchMax)
{
    ...
    *lpszBuf = 0;
    ...
    if (ReadProcessMemory(hProcess, (LPCVOID)mii.dwItemData, &fmi, sizeof(fmi), &cbRead) && (cbRead == sizeof(fmi)))
    {
        ...
        else if (fmi.pidl)
        {
            ...
            if (ReadProcessMemory(hProcess, fmi.pidl, &id, sizeof(ITEMIDLIST), &cbRead) && (cbRead == sizeof(id)))
            {
                id.cbTotal -= OFFSET_SZFRIENDLYNAME;
                cchMax = min((DWORD)id.cbTotal, cchMax);
                cchMax = max(cchMax, 1);

                ReadProcessMemory(hProcess, (LPBYTE)fmi.pidl + OFFSET_SZFRIENDLYNAME, lpszBuf, cchMax, &cbRead);
                lpszBuf[cchMax - 1] = 0;
                     ...
                     UINT           cchMax)
{
    ...
    if (hProcess)
    {
        ...
        if (ReadProcessMemory(hProcess, (LPCVOID)dwItemData, (LPVOID)& menuinfo, sizeof(menuinfo), &cbRead) && (cbRead == sizeof(menuinfo)))
        {
            // Check signature...
...
//  This allows us to navigate up through menupopups which have no hwnd (return NULL), but which do have parents, which eventually leads us to the owning hWnd.
STDAPI WindowFromAccessibleObjectEx(IAccessible* pacc, HWND* phwnd)
{
    IAccessible* paccT;
    IOleWindow* polewnd;
