...
//  We implement this ourself to avoid pulling in CRT gunk.  We don't need to do anything when a pure virtual function isn't redefined.
...
//  Some code will also respond to IText.  That code must override our QueryInterface() implementation.
...
STDMETHODIMP CAccessible::GetTypeInfo(UINT itInfo, LCID lcid, ITypeInfo ** ppITypeInfo)
{
    HRESULT hr;
...
STDMETHODIMP CAccessible::GetIDsOfNames(REFIID riid, OLECHAR** rgszNames, UINT cNames, LCID lcid, DISPID* rgDispID)
{
    HRESULT hr;
...
STDMETHODIMP CAccessible::Invoke(DISPID dispID, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS * pDispParams, VARIANT* pvarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr)
{
    HRESULT hr;
...
//  NOTE:  Not only is this the default handler, it can also serve as parameter checking for overriding implementations.
STDMETHODIMP CAccessible::get_accParent(IDispatch ** ppdispParent)
{
    InitPv(ppdispParent);

    if (m_hwnd)
        return(AccessibleObjectFromWindow(m_hwnd, OBJID_WINDOW, IID_IDispatch, (void **)ppdispParent));
    else
        return(S_FALSE);
}
...
STDMETHODIMP CAccessible::get_accKeyboardShortcut(VARIANT varChild, BSTR* pszShortcut)
{
    InitPv(pszShortcut);
                                 ...
                                 LPVOID *  ppvObject )
{
    ...
    hr = CoCreateInstance( rgRegisteredTypes[ iHandlerIndex ], NULL, CLSCTX_INPROC_SERVER, IID_IAccessibleHandler, &pv );
    if ( SUCCEEDED( hr ) )
    {
    ...
    else
    {
        ...
        hr = CoCreateInstance( rgRegisteredTypes[ iHandlerIndex ], NULL, CLSCTX_INPROC_SERVER, riid, ppvObject );
    }
...
#ifdef _DEBUG
// This new PrintIt function takes the arguments and then uses wvsprintf to format the string into a buffer.
// It then uses OutputDebugString to show the string.
