...
STDMETHODIMP CStatusBar32::accLocation(long* pxLeft, long* pyTop, long* pcxWidth, long* pcyHeight, VARIANT varChild)
{
    LPRECT  lprc;
    RECT    rcLocal;
    HANDLE  hProcess;
...
STDMETHODIMP CStatusBar32::accNavigate(long dwNavDir, VARIANT varStart, VARIANT* pvarEnd)
{
    LONG    lEnd;
    ...
    if ((!ValidateChild(&varStart) && !ValidateHwnd(&varStart)) || !ValidateNavDir(dwNavDir, varStart.lVal))
        return(E_INVALIDARG);

    // If the action is any navigation other than first or last and the child is 0
    ...
    // HWNDID, then we have to convert from the HWND to the child ID by seeing which child area has coordinates that contain the HWND.
    ...
    if (lEnd)
    {
        ...
        // We need to check all our child windows to see if they are contained within that section and return a dispatch interface if so.
        lEnd = FindChildWindowFromID (lEnd);
        if (IsHWNDID(lEnd))
        {
            pvarEnd->vt = VT_DISPATCH;
            return (AccessibleObjectFromWindow(HwndFromHWNDID(lEnd),OBJID_WINDOW, IID_IDispatch, (void**)&pvarEnd->pdispVal));
        }
        pvarEnd->vt = VT_I4;
        pvarEnd->lVal = lEnd;
        return(S_OK);
...
STDMETHODIMP CStatusBar32::accHitTest(long x, long y, VARIANT* pvarEnd)
{
    ...
    if ((hr != S_OK) || (pvarEnd->vt != VT_I4) || (pvarEnd->lVal != 0) || !m_cChildren)
        return(hr);
...
// This tries to find which section of the status bar the window child is in.

//  Parameters:
//      long    HwndID - this should be an HWNDID - an HWND with the high bit turned on
//  Returns:
//      long indicating which section the window is in. We will return a 1-based number, unless we didn't find anything, when we return 0.
...
//  Parameters:
//      long    ID - this should be a 1-based child ID indicating which part of the status bar to check for a child window.
//  Returns:
//      long that actually contains an HWNDID, or just the value passed in if there was no child window.
