...
//  In reality, have to also override other things, mainly for the Start button.
...
void CButton::SetupChildren(void)
{
    ...
    // check to see if the start button has focus and a menu is shown. if so, then there is one child
...
//  HACK for start button. If the menu is visible then we'll give that back, otherwise we'll just fall back on CClient
...
STDMETHODIMP CButton::accNavigate(long dwNavDir, VARIANT varStart, VARIANT * pvarEnd)
{
    ...
    if (!ValidateHwnd(&varStart) || !ValidateNavDir(dwNavDir, varStart.lVal))
        return(E_INVALIDARG);
    ...
    SetupChildren();
    if ((m_cChildren > 0) && (dwNavDir == NAVDIR_FIRSTCHILD || dwNavDir == NAVDIR_LASTCHILD))
    {
        hwndChild = FindWindow(TEXT("#32768"),NULL);
        if (!hwndChild)
            return(S_FALSE);
...
STDMETHODIMP CButton::get_accDefaultAction(VARIANT varChild, BSTR* pszDefAction)
{
    ...
    case BS_PUSHBOX:
    case BS_OWNERDRAW:
    case BS_USERBUTTON:// Pushing a push button is the default
        return(HrCreateString(STR_BUTTON_PUSH, pszDefAction));
    case BS_CHECKBOX:
    case BS_AUTOCHECKBOX:// Toggling a checkbox is the default
        ...
        break;
    case BS_RADIOBUTTON:
    case BS_AUTORADIOBUTTON:// Checking a radio button is the default
        return(HrCreateString(STR_BUTTON_CHECK, pszDefAction));
    case BS_3STATE:
    case BS_AUTO3STATE:
...
STDMETHODIMP CButton::accDoDefaultAction(VARIANT varChild)
{
    ...
    case BS_DEFPUSHBUTTON:
        if (InTheShell(m_hwnd, SHELL_TRAY))
        {
            // You can't just click the start button; it won't do anything if the tray isn't active except take focus
...
STDMETHODIMP CButton::Next(ULONG celt, VARIANT *rgvar, ULONG* pceltFetched)
{
    ...
    m_idChildCur = 1;// Advance the current position

    // Fill in the number fetched
    if (pceltFetched)
        *pceltFetched += cFetched;

    return((cFetched < (long)celt) ? S_FALSE : S_OK);// Return S_FALSE if we grabbed fewer items than requested
}
