...
--*/
{
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (pValueInformation != NULL) {
            while ((Status = NtQueryValueKey(hPerflibKey, &ValueNameString, KeyValuePartialInformation, pValueInformation, ValueBufferLength, &ResultLength)) == STATUS_BUFFER_OVERFLOW) {
                pValueInformation = REALLOCMEM(pValueInformation, ResultLength);
                if (pValueInformation == NULL) {
                    break;
                } else {
            ...
            if (NT_SUCCESS(Status)) {
                ...
                if (pValueInformation->Type == dwRegType) {
                    ...
                    if (pValueInformation->DataLength <= dwMaxSize) {
                        memcpy(pReturnBuffer, &pValueInformation->Data[0], pValueInformation->DataLength);
                        bUseDefault = FALSE;
                        lReturn = STATUS_SUCCESS;
                    }
                }
            } else {
                lReturn = Status;// return the default value
            }

            FREEMEM(pValueInformation);// release temp buffer
        } else {
            // unable to allocate memory for this operation so
            // just return the default value
        }

        NtClose(hPerflibKey);// close the registry key
    } else {
...
// Timer functions
DWORD PerflibTimerFunction(LPDWORD dwArg)
/*++
...
--*/
{
    ...
    if (Status == ERROR_SUCCESS) {
        ...
        if (hPerflibTimingThread == NULL) {
            ...
            hPerflibTimingThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)PerflibTimerFunction, NULL, 0, NULL);
...
*/
{
    ...
    if (bStatus) {
        ...
        if (pValueInformation != NULL) {
            ntStatus = NtQueryValueKey(hKey, &usLocal, KeyValuePartialInformation, pValueInformation, ValueBufferLength, &ResultLength);
            if ((NT_SUCCESS(ntStatus) || ntStatus == STATUS_BUFFER_OVERFLOW)) {
                ...
                if (NT_SUCCESS(ntStatus)) {
                    if (ARGUMENT_PRESENT(lpData)) {
                        if (!bUnicode && (pValueInformation->Type == REG_SZ || pValueInformation->Type == REG_EXPAND_SZ || pValueInformation->Type == REG_MULTI_SZ)) {
