...
LONG GetProcessNameColMeth (VOID)
{
    ...
    RtlInitUnicodeString (&PerflibSubKeyString, L"\\Registry\\Machine\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Perflib");
    ...
    if (NT_SUCCESS (Status)) {
        ...
        if (pKeyInfo != NULL) {
            RtlInitUnicodeString (&NameInfoValueString, L"CollectUnicodeProcessNames");// initialize value name string
            dwRetBufLen = 0;
...
LONG GetPerfDataAccess (VOID)
{
    ...
    if (NT_SUCCESS (Status)) {
        ...
        if (pKeyInfo != NULL) {
            ...
            RtlInitUnicodeString (&NameInfoValueString, L"CheckProfileSystemRight");// init value name string
...
*/
{
    ...
    dwStatus = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Perflib", 0L, KEY_READ, & hKeyPerflib);
