...
)
{
    ...
    if (status == ERROR_SUCCESS) {
        ...
        WmiBuffer = (LPBYTE)HeapAlloc (hProcessHeap, HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, WmiBufSize);
        if (WmiBuffer  == NULL) {
            ...
            DebugPrint((2, "BuildPhysDisk: WmiQueryAllDataW status1=%d\n", status));
#if DBG
            if (!HeapValidate(hProcessHeap, 0, WmiBuffer)) {
                DebugPrint((2, "BuildPhysDisk: WmiQueryAllDataW corrupted WmiBuffer\n"));
                DbgBreakPoint();
            }
#endif
            ...
            if ((WmiBufSize > 0) && (WmiBufSize != WmiAllocSize)) {
                WmiBuffer = (LPBYTE)HeapReAlloc (hProcessHeap, HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, WmiBuffer, WmiBufSize);
                if (WmiBuffer == NULL) {
...
#if DBG
                if (!HeapValidate(hProcessHeap, 0, WmiBuffer)) {
                    DebugPrint((2, "BuildPhysDisk: WmiQueryAllDataW2 corrupted WmiBuffer\n")); 
                    DbgBreakPoint();
                }
#endif
                DebugPrint((2, "BuildPhysDisk: WmiQueryAllDataW status2=%d\n", status));
            } else {
        ...
        if (status == ERROR_SUCCESS) {
            ...
            while (bNotDone) {
                ...
                if ((PCHAR) WmiDiskInfo > (PCHAR) WmiBuffer + WmiAllocSize) {
                    DebugPrint((2, "BuildPhysDisk: WmiDiskInfo %d exceeded %d + %d\n", WmiDiskInfo, WmiBuffer, WmiAllocSize));
                }
#endif
                pDiskPerformance = (PDISK_PERFORMANCE)((PUCHAR)WmiDiskInfo +  WmiDiskInfo->DataBlockOffset);
                ...
                dwInstanceNameOffset = WmiDiskInfo->DataBlockOffset + ((sizeof(DISK_PERFORMANCE) + 1) & ~1) ;

#if DBG
                if ((dwInstanceNameOffset+(PCHAR)WmiDiskInfo) > (PCHAR) WmiBuffer + WmiAllocSize) {
                    DebugPrint((2, "BuildPhysDisk: dwInstanceNameOffset %d exceeded %d + %d\n", dwInstanceNameOffset, WmiBuffer, WmiAllocSize));
                }
#endif
...
#if DBG
                if ((wNameLen + (PCHAR)WmiDiskInfo + dwInstanceNameOffset) > (PCHAR) WmiBuffer + WmiAllocSize) {
                    DebugPrint((2, "BuildPhysDisk: wNameLen %d exceeded %d + %d\n", wNameLen, WmiBuffer, WmiAllocSize));
                }
#endif
                if (wNameLen > 0) {
...
*/
{
    ...
    while ((bNeedMoreData) && (dwRetryCount)) {
        ...
        pMountPoints = (PMOUNTMGR_MOUNT_POINTS) HeapAlloc (hProcessHeap, HEAP_FLAGS, dwBufferSize);
        if (pMountPoints == NULL) {
...
#if DBG
        HeapUsed += dwBufferSize;
        DebugPrint((4, "\tAdded MountPoints %d to %d\n", dwBufferSize, HeapUsed));
#endif
        dwReturnSize = 0;
    ...
    if (!dwRetryCount)  {
        ...
        dwOrigErrorMode = SetErrorMode (SEM_FAILCRITICALERRORS      | SEM_NOALIGNMENTFAULTEXCEPT  | SEM_NOGPFAULTERRORBOX       | SEM_NOOPENFILEERRORBOX);
        for (dwBufEntry=0, dwListEntry = 0;  dwBufEntry < pMountPoints->NumberOfMountPoints;  dwBufEntry++) {
            ...
            if (point->SymbolicLinkNameLength) {
                ...
                if (*(LONGLONG *)pDriveLetter == llDosDevicesId) {
                    ...
                    if (((*pDriveLetter >= L'A') && (*pDriveLetter <= L'Z')) || ((*pDriveLetter >= L'a') && (*pDriveLetter <= L'z'))) {
                        pList[dwListEntry].wcDriveLetter = towupper(*pDriveLetter);

                        memset(&objectAttributes, 0, sizeof(OBJECT_ATTRIBUTES));
                        memcpy (szTempPath,  (LPVOID)((PCHAR) pMountPoints + point->SymbolicLinkNameOffset), point->SymbolicLinkNameLength);
                        ...
                        *pThisChar = 0;
                        ...
                        if (bIsHardDisk) {
                            ...
                            InitializeObjectAttributes(&objectAttributes, &usVolume, OBJ_CASE_INSENSITIVE, NULL, NULL);
                            status = NtOpenFile(&pList[dwListEntry].hVolume, (ACCESS_MASK)FILE_LIST_DIRECTORY | SYNCHRONIZE, &objectAttributes, &status_block,
                                FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_SYNCHRONOUS_IO_NONALERT | FILE_DIRECTORY_FILE);
            ...
            if (dwDone == 0) {
               ...
               swprintf (szDbgBuffer, L"\nPERFDISK:VolumeList -- Added %s as drive %c", pList[dwListEntry].wszInstanceName, pList[dwListEntry].wcDriveLetter);
               OutputDebugStringW (szDbgBuffer);
#endif
                // then the data fields have been satisfied so this entry is done and we can now go to the next entry in the caller's buffer
                dwListEntry++;
            }
        }

        SetErrorMode (dwOrigErrorMode);
        *pdwNumEntries = dwListEntry;// return the number of entries actually used here
    }
