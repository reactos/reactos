...
--*/
{
    ...
    while ( TRUE ) {
        if ( ProcessInfo->ImageName.Buffer != NULL || ProcessInfo->NumberOfThreads > 0 ) {
            NullProcess = FALSE;
        ...
        while ( !NullProcess && ThreadNumber < ProcessInfo->NumberOfThreads ) {
            ...
            Status = NtOpenThread(&hThread, THREAD_GET_CONTEXT, &Obja, &ThreadInfo->ClientId);
        ...
        ProcessInfo = (PSYSTEM_PROCESS_INFORMATION) &pProcessBuffer[ProcessBufferOffset];
    ...
    //  Thread definition, so we can update the offset to the next object definition
    *lpcbTotalBytes = pThreadDetailDataDefinition->ThreadDetailsObjectType.TotalByteLength = (DWORD)((PCHAR) pPerfInstanceDefinition - (PCHAR) pThreadDetailDataDefinition);
