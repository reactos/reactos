...
BOOL GetProcessExeName(HANDLE  hProcessID, PUNICODE_STRING pusName)
{
    HANDLE                  hProcess;
        ...
        if (!ReadProcessMemory(hProcess, &Peb->Ldr, &Ldr, sizeof(Ldr), NULL)) {
            ...
            if (!ReadProcessMemory(hProcess, &LdrHead->Flink, &LdrNext, sizeof(LdrNext), NULL)) {
                ...
                if (!ReadProcessMemory(hProcess, LdrEntry, &LdrEntryData, sizeof(LdrEntryData), NULL)) {
                    ...
                    if (!ReadProcessMemory(hProcess, LdrEntryData.BaseDllName.Buffer, (LPVOID)&wszDllName[0], sizeof(wszDllName), NULL)) {
                        // unable to read DLL buffer
                        bReturn = FALSE;
                    } else {
                        // copy the short name to the caller's buffer
                        RtlInitUnicodeString (pusName, wszDllName);
...
LONG GetProcessNameColMeth (VOID)
{
    NTSTATUS            Status;
    HANDLE              hPerflibKey;
    ...
    RtlInitUnicodeString (&PerflibSubKeyString, (LPCWSTR)L"\\Registry\\Machine\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Perflib");
    ...
    if (NT_SUCCESS (Status)) {
        ...
        if (pKeyInfo != NULL) {
            RtlInitUnicodeString (&NameInfoValueString, (LPCWSTR)L"CollectUnicodeProcessNames");// initialize value name string

            dwRetBufLen = 0;
            Status = NtQueryValueKey (hPerflibKey, &NameInfoValueString, KeyValuePartialInformation, (PVOID)pKeyInfo, dwBufLen, &dwRetBufLen);
            if (NT_SUCCESS(Status)) {
        ...
        NtClose (hPerflibKey);// close handle
    }
...
--*/
{
    ...
    wStringSize =  sizeof (UNICODE_STRING) + MAX_INSTANCE_NAME * sizeof(WCHAR) + sizeof (UNICODE_NULL);
        ...
        *pPeriod = 0; // null terminate buffer
        ...
        switch (HandleToUlong(pProcess->UniqueProcessId)) {
            case IDLE_PROCESS_ID:
                RtlAppendUnicodeToString (pusLocalProcessNameBuffer, (LPWSTR)IDLE_PROCESS);
                break;
            case SYSTEM_PROCESS_ID:
                RtlAppendUnicodeToString (pusLocalProcessNameBuffer, (LPWSTR)SYSTEM_PROCESS);
                break;
...
static DWORD GetSystemProcessData ()
{
    DWORD   dwReturnedBufferSize;
    NTSTATUS Status;

    //  Get process data from system.
    //  if bGotProcessInfo is TRUE, that means we have the process info.
    //  collected earlier when we are checking for costly object types.
    ...
    if ( !NT_SUCCESS(Status) ) {
        ...
        // get system time
        Status = NtQuerySystemInformation(SystemTimeOfDayInformation, &SysTimeInfo, sizeof(SysTimeInfo), &dwReturnedBufferSize);
        if (!NT_SUCCESS(Status)) {
            Status = (DWORD)RtlNtStatusToDosError(Status);
        }
    }
