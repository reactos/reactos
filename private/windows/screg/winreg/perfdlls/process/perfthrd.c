...
--*/
{
    ...
    pPerfInstanceDefinition = (PPERF_INSTANCE_DEFINITION)&pThreadDataDefinition[1];
    ...
    while ( TRUE ) {
        ...
        while ( !NullProcess && ThreadNumber < ProcessInfo->NumberOfThreads ) {
            ...
            tcdTotal.ProcessorTime +=
                pTCD->ProcessorTime = ThreadInfo->KernelTime.QuadPart + ThreadInfo->UserTime.QuadPart;
            ...
            pTCD->ThreadState = (DWORD)((ThreadInfo->ThreadState > 7) ? 7 : ThreadInfo->ThreadState);
