...
PPROCESS_VA_INFO GetSystemVaData (IN PSYSTEM_PROCESS_INFORMATION pFirstProcess)
    /*++
    GetSystemVaData
    Obtains the Process and Image Virtual Address information for all processes running on the system.
    (note that the routines called by this function allocate data structures consequently the corresponding
    ...
    if (pFirstProcess != NULL) {
        ...
        while (TRUE) {  // loop exit is at bottom of loop
            ...
            if ( pThisProcess->NextEntryOffset == 0 ) {
                ...
                pThisProcess = (PSYSTEM_PROCESS_INFORMATION) ((PBYTE)pThisProcess + pThisProcess->NextEntryOffset);
            }
        }
...
PPROCESS_VA_INFO GetProcessVaData (IN PSYSTEM_PROCESS_INFORMATION     pProcess)
    ...
    Return Value
    Pointer to completed Process VA info structure or NULL if unable to collect data
    --*/
{
    NTSTATUS                Status;
    HANDLE                  hProcess;
    ...
    // allocate structure
    pThisProcess = ALLOCMEM (hLibHeap, HEAP_ZERO_MEMORY, sizeof (PROCESS_VA_INFO));
    if (pThisProcess) { // allocation successful
        // initialize fields
        pThisProcess->BasicInfo =  ALLOCMEM (hLibHeap, HEAP_ZERO_MEMORY, sizeof (PROCESS_BASIC_INFORMATION));
        ...
        // buffer
        pThisProcess->pProcessName = ALLOCMEM (hLibHeap, HEAP_ZERO_MEMORY, (sizeof(UNICODE_STRING) + MAX_PROCESS_NAME_LENGTH));
        ...
        if (hProcess) {
            ...
            pPeb = pThisProcess->BasicInfo->PebBaseAddress;// get pointer to the Process Environment Block
            Status = NtReadVirtualMemory (hProcess, &pPeb->Ldr, &Ldr, sizeof (Ldr), NULL);// read address of loader information structure
            if (!NT_SUCCESS(Status)){// bail out if unable to read information
                goto SuccessExit;// if error reading data, then bail out
            }

            LdrHead = &Ldr->InMemoryOrderModuleList;// get head pointer to linked list of memory modules used by this process
            Status = NtReadVirtualMemory (hProcess, &LdrHead->Flink, &LdrNext, sizeof (LdrNext), NULL);// Get address of next list entry
            if (!NT_SUCCESS(Status)){// bail out if unable to read information
                goto SuccessExit;// if error reading data, then bail out
            }
            ...
            while ( LdrNext != LdrHead ) {
                ...
                Status = NtReadVirtualMemory(hProcess, pLdrEntry, &LdrEntryData, sizeof(LdrEntryData), NULL);
                ...
                pNewModule = GetModuleVaData (&LdrEntryData, pThisProcess);
            ...
            // now that we have a list of all images, query the process' virtual memory for the list of memory blocks in use by this process and assign them to the appropriate category of memory
...
PBailOut:
    if (pThisProcess->BasicInfo) {
        FREEMEM (hLibHeap, 0, pThisProcess->BasicInfo);
    }
    FREEMEM (hLibHeap, 0, pThisProcess);
    if (hProcess) CloseHandle(hProcess);
    return NULL;
}
    ...
    --*/
{
    ...
    pThisModule = ALLOCMEM (hLibHeap, HEAP_ZERO_MEMORY, sizeof (MODINFO));
    if (!pThisModule) {
        return NULL;
    }
    ...
    pusInstanceName = ALLOCMEM (hLibHeap, HEAP_ZERO_MEMORY, wStringSize + sizeof(UNICODE_STRING))  ;
    if (!pusInstanceName) {
        goto MBailOut;
    }
    ...
    pusLongInstanceName = ALLOCMEM (hLibHeap, HEAP_ZERO_MEMORY, wStringSize + sizeof (UNICODE_STRING));
    ...
    ImageNameBuffer = usImageFileName.Buffer = ALLOCMEM(hLibHeap, HEAP_ZERO_MEMORY, usImageFileName.MaximumLength);
    ...
    usExeFileName.Buffer = ALLOCMEM(hLibHeap, HEAP_ZERO_MEMORY, usExeFileName.MaximumLength);
    if ( !usExeFileName.Buffer ) {
        goto MBailOut;
    }

    // read base .exe/.dll name of image
    Status = NtReadVirtualMemory(pProcess->hProcess, ModuleListEntry->BaseDllName.Buffer, usExeFileName.Buffer, usExeFileName.MaximumLength, NULL);
    if ( !NT_SUCCESS(Status) ) {
        goto MBailOut;
    }
    ...
    //    FREEMEM (hLibHeap, 0, RelativeName.RelativeName.Buffer);
    FREEMEM (hLibHeap, 0, ImageNameBuffer);
    FREEMEM (hLibHeap, 0, usExeFileName.Buffer);
    return (pThisModule);   // return pointer to completed module structure
    //  Module bail out point, called when the routine is unable to continue for some reason. This cleans up any allocated memory, etc.
MBailOut:
    if (pThisModule) {
        FREEMEM (hLibHeap, 0, pThisModule);
    }

    if (usNtFileName.Buffer) {
        FREEMEM (RtlProcessHeap(),   // this is allocated by an RTL function
            0, usNtFileName.Buffer);
    }
    ...
    if (pusInstanceName) {
        FREEMEM (hLibHeap, 0, pusInstanceName);
    }

    if (pusLongInstanceName) {
        FREEMEM (hLibHeap, 0, pusLongInstanceName);
    }
    ...
    --*/
{
    ...
    while (pThisMod)  { // go to end of list or match is found
        ...
        if (pAddress >= pThisMod->BaseAddress) {
            if ((PVOID)((PDWORD)pAddress + dwExtent) <= (PVOID)((ULONG_PTR)pThisMod->BaseAddress+pThisMod->VirtualSize)) {
                return (pThisMod);
            }
        }
...
BOOL FreeSystemVaData (IN PPROCESS_VA_INFO pFirstProcess)
{
...
BOOL FreeProcessVaData (IN PPROCESS_VA_INFO pProcess)
{
    PMODINFO    pThisModule, pNextModule;

    if (pProcess) {
        if (pProcess->pProcessName) {
            FREEMEM (hLibHeap, 0, pProcess->pProcessName);
            pProcess->pProcessName = NULL;
        }
        if (pProcess->BasicInfo) {
            FREEMEM (hLibHeap, 0, pProcess->BasicInfo);
...
BOOL FreeModuleVaData (IN PMODINFO pModule)
{
    if (pModule) {
        if (pModule->InstanceName) {
            FREEMEM(hLibHeap, 0, pModule->InstanceName);
            pModule->InstanceName = NULL;
        }
        if (pModule->LongInstanceName) {
            FREEMEM(hLibHeap, 0, pModule->LongInstanceName);
            pModule->LongInstanceName = NULL;
        }
        FREEMEM (hLibHeap, 0, pModule);
    }
