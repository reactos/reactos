...
DWORD APIENTRY OpenServerQueueObject (IN  LPWSTR  szValueName)
{
    NTSTATUS    status;
    ...
    pSrvQueueStatistics = (SRV_QUEUE_STATISTICS *)ALLOCMEM (hLibHeap, HEAP_ZERO_MEMORY, dwDataBufferLength);

    // if memory allocation failed, then no server queue stats will be returned.
