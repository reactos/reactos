...
DWORD APIENTRY CollectProcessorObjectData(IN OUT  LPVOID  *lppData,
...
--*/
{
    ...
    if (ProcessorBufSize) {
        ntStatus = NtQuerySystemInformation(SystemProcessorPerformanceInformation, pProcessorBuffer, ProcessorBufSize, &dwReturnedBufferSize);
    ...
    if (!NT_SUCCESS(ntStatus)) {
        ...
        memset(pProcessorInterruptInformation, 0, (BasicInfo.NumberOfProcessors * sizeof(SYSTEM_INTERRUPT_INFORMATION)));
    }

    memset(&pcdTotalData, 0, sizeof(pcdTotalData));// clear the "Total" instance
    ...
    // point to the first processor in the returned array of interrupt information. data is returned as an array of structures.
    ...
    for (CurProc = 0; CurProc < (ULONG)BasicInfo.NumberOfProcessors; CurProc++) {
        ...
        pcdTotalData.ProcessorTime += pPCD->ProcessorTime = pProcessorInformation->IdleTime.QuadPart;
        pcdTotalData.UserTime += pPCD->UserTime = pProcessorInformation->UserTime.QuadPart;
        // kernel time is total kernel time less the time spent in the idle thread for that processor
        pcdTotalData.KernelTime += pPCD->KernelTime = pProcessorInformation->KernelTime.QuadPart - pPCD->ProcessorTime;

        pcdTotalData.Interrupts += pPCD->Interrupts = pProcessorInformation->InterruptCount;
        pcdTotalData.DpcTime += pPCD->DpcTime = pProcessorInformation->DpcTime.QuadPart;
        pcdTotalData.InterruptTime += pPCD->InterruptTime = pProcessorInformation->InterruptTime.QuadPart;
        ...
        pcdTotalData.DpcBypassRate += pPCD->DpcBypassRate = pThisProcessorInterruptInformation->DpcBypassCount;
        pcdTotalData.ApcBypassRate += pPCD->ApcBypassRate = pThisProcessorInterruptInformation->ApcBypassCount;
    ...
    *lpcbTotalBytes =
        pProcessorDataDefinition->ProcessorObjectType.TotalByteLength = (DWORD)((LPBYTE)pPerfInstanceDefinition - (LPBYTE)pProcessorDataDefinition);
