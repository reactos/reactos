...
--*/
{
    ...
    while ((status = NtQuerySystemInformation(
        ...
        pSysPageFileInfo = REALLOCMEM (hLibHeap, 0, pSysPageFileInfo, dwSysPageFileInfoSize);
        if (pSysPageFileInfo == NULL) {
            status = ERROR_OUTOFMEMORY;
            break;
        }
    ...
    if ( !NT_SUCCESS(status) ) {
        ReportEvent (hEventLog, EVENTLOG_ERROR_TYPE, 0, PERFOS_UNABLE_QUERY_PAGEFILE_INFO, NULL, 0, sizeof(DWORD), NULL, &status);
        *lpcbTotalBytes = (DWORD) 0;
        *lpNumObjectTypes = (DWORD) 0;
