...
static DWORD GetSystemThreadInfo (PPERFSYS_THREAD_DATA_BLOCK pTDB)
{
    NTSTATUS    status;
    ...
    memset (pTDB, 0, sizeof (PERFSYS_THREAD_DATA_BLOCK));// reset the caller's buffer
    ...
    if ( NT_SUCCESS(status) ) {
        ...
        while ( TRUE ) {
            ...
            while ( !NullProcess && ThreadNumber < ProcessInfo->NumberOfThreads ) {
                pTDB->dwThreadCount++;//  Format and collect Thread data

                // update thread state counters
                if (ThreadInfo->ThreadState == 1) {// then it's READY
                    pTDB->dwReadyThreads++;
                } else if (ThreadInfo->ThreadState == 4) {// then it's TERMINATED
                    pTDB->dwTerminatedThreads++;
                } else if (ThreadInfo->ThreadState == 5) {// then it's WAITING
                    pTDB->dwWaitingThreads++;
                } else if (ThreadInfo->ThreadState == 6) {// then it's in TRANSITION
            ...
            if (ProcessInfo->NextEntryOffset == 0) {
                break;// that was the last process
            }
        ...
        ReportEvent (hEventLog, EVENTLOG_WARNING_TYPE, 0, PERFOS_UNABLE_QUERY_PROCESS_INFO, NULL, 0, sizeof(DWORD), NULL, (LPVOID)&status);
    }
...
--*/
{
    ...
    if (!NT_SUCCESS(ntStatus)) {
        // unable to collect the data from the system so clear the return data structure to prevent bogus data from being returned
        ReportEvent (hEventLog, EVENTLOG_WARNING_TYPE, 0, PERFOS_UNABLE_QUERY_EXCEPTION_INFO, NULL, 0, sizeof(DWORD), NULL, (LPVOID)&ntStatus);
    ...
    ntStatus = NtQuerySystemInformation (SystemRegistryQuotaInformation, (PVOID)&RegistryInfo, sizeof(RegistryInfo), NULL);
    if (ntStatus != STATUS_SUCCESS) {
        ReportEvent (hEventLog, EVENTLOG_WARNING_TYPE, 0, PERFOS_UNABLE_QUERY_REGISTRY_QUOTA_INFO, NULL, 0, sizeof(DWORD), NULL, (LPVOID)&ntStatus);
    ...
    if (!NT_SUCCESS(ntStatus)) {
        ReportEvent (hEventLog, EVENTLOG_WARNING_TYPE, 0, PERFOS_UNABLE_QUERY_SYSTEM_TIME_INFO, NULL, 0, sizeof(DWORD), NULL, (LPVOID)&ntStatus);
