...

{
    ...
    Error = RegNotifyChangeKeyValue(RootHandle, FALSE, REG_LEGAL_CHANGE_FILTER, EventHandle, TRUE);
...

{
    ...
    Error = RegOpenKeyEx(PredefinedHandle, ".Default", REG_OPTION_RESERVED, MAXIMUM_ALLOWED, &RootHandle);
    ...
    Error = RegCloseKey(PredefinedHandle);
    REG_API_SUCCESS( RegCloseKey );
    ...
    _EventHandle1 = CreateEvent(NULL, FALSE, FALSE, NULL);
    ASSERT( _EventHandle1 != NULL );

    _EventHandle2 = CreateEvent(NULL, FALSE, FALSE, NULL);
    ASSERT( _EventHandle2 != NULL );

    // Create the notify thread.
    NotifyThreadHandle = CreateThread(NULL, 0, NotifyThread, NULL, 0, &ThreadID);
    ...
    // Use Win 3.1 compatible APIs to create/close, open/close and delete the key TestUser1.
    Error = RegCreateKey(RootHandle, KEY_NAME_1, &Handle1);
    REG_API_SUCCESS( RegCreateKey );
    Error = RegCloseKey(Handle1);
    REG_API_SUCCESS( RegCloseKey );
    ...
    Error = RegCreateKeyEx(RootHandle, KEY_NAME_1, 0, KEY_NAME_1_CLASS, REG_OPTION_RESERVED, KEY_ALL_ACCESS, &SecurityAttributes, &Handle1, &Disposition);
    ...
    Error = RegOpenKeyEx(RootHandle, KEY_NAME_1, REG_OPTION_RESERVED, KEY_ALL_ACCESS, &Handle1);
    REG_API_SUCCESS( RegOpenKeyEx );

    Error = RegCloseKey(Handle1);
    REG_API_SUCCESS( RegCloseKey );

    Error = RegCreateKeyEx(RootHandle, KEY_NAME_1, 0, KEY_NAME_1_CLASS, REG_OPTION_RESERVED, KEY_ALL_ACCESS, NULL, &Handle1, &Disposition);
    REG_API_SUCCESS( RegCreateKeyEx );
    ...
    Error = RegCreateKeyEx(Handle1, KEY_NAME_1_1, 0, KEY_NAME_1_1_CLASS, REG_OPTION_RESERVED, KEY_ALL_ACCESS, &SecurityAttributes, &Handle1_1, &Disposition);
    REG_API_SUCCESS( RegCreateKeyEx );
    ASSERT( Disposition == REG_CREATED_NEW_KEY );
    Error = RegCreateKeyEx(Handle1, KEY_NAME_1_2, 0, KEY_NAME_1_2_CLASS, 0, KEY_ALL_ACCESS, &SecurityAttributes, &Handle1_2, &Disposition);
    REG_API_SUCCESS( RegCreateKeyEx );
    ...
    Error = RegCreateKey(RootHandle, KEY_NAME_1, &Handle1);
    REG_API_SUCCESS( RegCreateKey );
    Error = RegRestoreKey(Handle1, SAVE_RESTORE_FILE, 0);
    ...
    Error = RegSetValue(RootHandle, KEY_NAME_1,
                VALUE_DATA_1_TYPE,
                Data_1,
                VALUE_DATA_1_LENGTH
                );
    ...
    Error = RegSetValueEx(Handle1, VALUE_NAME_2, 0, VALUE_DATA_2_TYPE, ( PVOID ) &Data_2, VALUE_DATA_2_LENGTH);
    ...
    Error = RegQueryValue(RootHandle, KEY_NAME_1, Data, &DataLength);
    ...
    DataLength = MAX_DATA_LENGTH;
    Error = RegQueryValueEx(Handle1, VALUE_NAME_2, NULL, &Type, Data, &DataLength);
    ...
    for( Index = 0; Index < 2; Index++ ) {
        ...
        DataLength      = MAX_DATA_LENGTH;
        Error = RegEnumValue(Handle1, Index, ValueName, &ValueNameLength, NULL, &Type, Data, &DataLength);
    ...
    Error = RegDeleteKey(RootHandle, KEY_NAME_1);
    REG_API_SUCCESS( RegDeleteKey );
    Error = RegCloseKey(Handle1);
    REG_API_SUCCESS( RegCloseKey );
}