...

{
    ...
    for( Index = 0; Index < NumberOfSubKeys; Index++ ) {
        ...
        Error = RegEnumKey(KeyHandle, 0, KeyName, KeyNameLength);
        REG_API_SUCCESS( RegEnumKey );

        Error = RegOpenKeyEx(KeyHandle, KeyName, REG_OPTION_RESERVED, KEY_ALL_ACCESS, &ChildHandle);
        ...
        Error = RegDeleteKey(KeyHandle, KeyName);
        REG_API_SUCCESS( RegDeleteKey );
    }
}

VOID DeleteKey(IN PKEY     Key)
{
    LONG    Error;
