...
void * __cdecl ::operator new(unsigned int dwBytes)
{
    void *p;
...
BOOL ReadMemoryUserMode( HANDLE hProcess, const void* pAddress, void* pBuffer, DWORD dwSize, DWORD* pdwRead )
{
    return ReadProcessMemory( hProcess, pAddress, pBuffer, dwSize, pdwRead );
}


BOOL ReadMemoryKernelMode( HANDLE, const void* pAddress, void* pBuffer, DWORD dwSize, DWORD* pdwRead )
{
    return ReadMemory( (ULONG) pAddress, pBuffer, dwSize, pdwRead );
}


BOOL WriteMemoryUserMode( HANDLE hProcess, const void* pAddress, void* pBuffer, DWORD dwSize, DWORD* pdwRead )
{
    return WriteProcessMemory( hProcess, (void*) pAddress, pBuffer, dwSize, pdwRead );
}


BOOL WriteMemoryKernelMode( HANDLE, const void* pAddress, void* pBuffer, DWORD dwSize, DWORD* pdwRead )
{
    return WriteMemory( (ULONG) pAddress, pBuffer, dwSize, pdwRead );
}


BOOL GetData(IN DWORD dwAddress,  IN LPVOID ptr, IN ULONG size, IN PCSTR type )
{
    BOOL b;
    ULONG BytesRead;
    ULONG count;
...
WCHAR *ReadProcessChar(unsigned short * Address)
{
...
void TrackObjectDataPrint(TrackObjectData* pKeyData)
{
    ...
    if (!GetData(StackAddress, &pStack, sizeof(PVOID), NULL)) {
    ...
    if (!GetData(pStack, rgStack, sizeof(*rgStack) * dwStackDepth, NULL)) {
        dprintf("Error reading stack frames at 0x%x\n", StackAddress);
...
NTSTATUS PrintObjectInfo(HANDLE Handle)
{
    ...
    if (!NT_SUCCESS(Status)) {
        if (STATUS_INFO_LENGTH_MISMATCH == Status) {
            ...
            pNameInfo = (POBJECT_NAME_INFORMATION) RtlAllocateHeap(RtlProcessHeap(), 0, dwRequired);
            if (pNameInfo) {
