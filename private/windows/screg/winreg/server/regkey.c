...
Notes:
    ...
    On the client side, modules contain RPC wrappers for both the new Win32 and compatible Win 3.1 APIs.
    The Win 3.1 wrappers generally supply default parameters before calling the Win32 wrappers.
    In some cases they may need to call multiple Win32 wrappers in order to function correctly (e.g.  RegSetValue sometimes needs to call RegCreateKeyEx).
    The Win32 wrappers are quite thin and usually do nothing more than map a predefined handle to a real handle and perform ANSI<->Unicode translations.
    In some cases (e.g.  RegCreateKeyEx) the wrapper also converts some argument (e.g.  SECURITY_ATTRIBUTES) to an RPCable representation.
    In both the Win 3.1 and Win32 cases ANSI and Unicode implementations are provided.

    On the server side, there is one entry point for each of the Win32 APIs.
    Each contains an identical interface with the client side wrapper with the exception that all string / count arguments are passed as a single counted Unicode string.  Pictorially, for an API named "F":
    ...
    This yields smaller code (as the string conversion is done only once per API) at the cost of slightly higher maintenance (i.e. Win 3.1
    default parameter replacement and Win32 string conversions must be manually kept in synch).
...

{
    ...
    *phkResult = NULL;
    ...
    if( lpSubKey->Buffer[0] == ( WCHAR )'\\' ) {
        ...
        // Do not accept a key name that starts with '\', even though the code below would handle it.
        // This is to ensure that RegCreateKeyEx and RegOpenKeyEx will behave in the same way
    ...
    for (;;) {
        ...
        if (!HKEY_RestrictedSite && fTrySingleCreate)
        {
            ...
            InitializeObjectAttributes(&Obja,
                                       ...
                                       Attributes,
                                       hkDestKey,
                                       ARGUMENT_PRESENT( pRpcSecurityAttributes ) ? pRpcSecurityAttributes->RpcSecurityDescriptor.lpSecurityDescriptor : NULL);
            Status = NtCreateKey(phkResult, samDesired, &Obja, 0, lpClass, dwOptions, &dwDisposition);
            ...
            if (gpfnTermsrvCreateRegEntry && NT_SUCCESS(Status) && (dwDisposition == REG_CREATED_NEW_KEY)) {
                ...
                gpfnTermsrvCreateRegEntry(*phkResult, &Obja, 0, lpClass, dwOptions);
            }
        }
...
// local-only check for restricted
...
#endif // local-only check for restricted
        {
            ...
            Status = NtCreateKey(&TempHandle1, ( i == NumberOfSubKeys - 1 )? samDesired : MAXIMUM_ALLOWED, &Obja, 0, lpClass, dwOptions, &dwDisposition);
            if (NT_SUCCESS(Status) && lpdwDisposition) {
                *lpdwDisposition = dwDisposition;
            }
            ...
            if (IsTerminalServer()) {
                ...
                if (Status == STATUS_ACCESS_DENIED) {
                    ...
                    Status = NtCreateKey(&TempHandle1, (samDesired & ~(WRITE_DAC | WRITE_OWNER | KEY_CREATE_LINK)), &Obja, 0, lpClass, dwOptions, &dwDisposition);
...
error_status_t BaseRegOpenKey(IN HKEY hKey, IN PUNICODE_STRING lpSubKey, IN DWORD dwOptions, IN REGSAM samDesired, OUT PHKEY phkResult)
/*
Routine Description:
    Open a key for access, returning a handle to the key.  If the key is not present, it is not created (see RegCreateKeyExW).
Arguments:
    hKey - Supplies a handle to an open key.  The lpSubKey pathname parameter is relative to this key handle.
        Any of the predefined reserved handle values or a previously opened key handle may be used for hKey.
...
Return Value:
    Returns ERROR_SUCCESS (0) for success; error-code for failure.
    If successful, RegOpenKeyEx will return the handle to the newly opened key in phkResult.
--*/
{
    OBJECT_ATTRIBUTES   Obja;
    ...
    // Need to NULL this out param for compat with NT4, even though SDK does not define this out param on api failure -- bad apps were written
    ...
    // were actually passing in a bad pointer and ignoring the error returned by the api as part of their normal operation.
    __try {
        *phkResult = NULL;
    ...
    //  This was added on the client side so that the NULL was transmited by RPC.
    lpSubKey->Length -= sizeof( UNICODE_NULL );

    // If this is a restricted site, first try to open the per-site version of this key
...
// local-only check for restricted
    ...
    if (HKEY_RestrictedSite)
    {
        ...
        if (NULL != MappedKey)
        {
            InitializeObjectAttributes(
                &Obja,
                &NewSubKey,
                dwOptions & REG_OPTION_OPEN_LINK ? (OBJ_OPENLINK | OBJ_CASE_INSENSITIVE) : OBJ_CASE_INSENSITIVE,
                MappedKey,
                NULL
                );
        ...
        {
            // The key is a class registration parent key (e.g. CLSID)
            // We need to always return the global version of these keys to ensure global data under them is accessable.
            // Munge the samDesired to ensure that the key can be opened.
            samDesired = MAXIMUM_ALLOWED;
        }
    }
#endif
...
#ifdef LOCAL
    if (REG_CLASS_IS_SPECIAL_KEY(hKey)) {
        Status = BaseRegOpenClassKey(hKey, lpSubKey, dwOptions, samDesired, phkResult);
    } else
#endif // LOCAL
    {
        Status = NtOpenKey(phkResult, samDesired, &Obja);// Obja was initialized above
    }
    ...
    // so if we get here with an access denied, and this is a mappable key, then try to create a per-site version of the key
...
// local-only check for restricted
    ...
    if (NULL != MappedKey && STATUS_ACCESS_DENIED == Status)
    {
        ...
        lpSubKey->Length += sizeof(UNICODE_NULL);
        ret = BaseRegCreateKey(hKey, lpSubKey, &Class, 0, MAXIMUM_ALLOWED, NULL, phkResult, NULL);
    }

    if ((!REG_CLASS_IS_SPECIAL_KEY(hKey)) && !NT_SUCCESS(Status) && gpfnTermsrvOpenRegEntry) {
        // Obja was initialized above
        if (gpfnTermsrvOpenRegEntry(phkResult, samDesired, &Obja)) {
...
--*/
{
    ...
    InitializeObjectAttributes(&attributes, pKeyName, OBJ_CASE_INSENSITIVE, HKEY_RestrictedSite, NULL);
    ...
    NewSubKey->MaximumLength = pKeyName->Length - (USHORT)((BYTE *) LastElement  - (BYTE *) pKeyName->Buffer);
    NewSubKey->Buffer        = RtlAllocateHeap(RtlProcessHeap(), 0, NewSubKey->MaximumLength);
    ...
    if (0 != pKeyName->Length)
    {
        ...
        pKeyName->Length -= sizeof(L'\\');
        InitializeObjectAttributes(&attributes, pKeyName, OBJ_CASE_INSENSITIVE, BaseKey, NULL);
