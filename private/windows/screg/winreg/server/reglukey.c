...
--*/
{
    ...
    InitializeObjectAttributes(&ObjaKey, SubKey, OBJ_CASE_INSENSITIVE, hKey, NULL);

    // Convert the DOS path name to a canonical Nt path name.
    ErrorFlag = RtlDosPathNameToNtPathName_U(lpFile->Buffer, &FileName, NULL, &RelativeName);
    ...
    InitializeObjectAttributes(&ObjaFile, &FileName, OBJ_CASE_INSENSITIVE, RelativeName.ContainingDirectory, NULL);

#if DBG
    //OutputDebugString( "WINREG: Before NtLoadKey\n" );
#endif

    NtStatus = NtLoadKey(&ObjaKey, &ObjaFile);
...
Arguments:
    hKey - Supplies a handle to an open key. lpSubKey is relative to this handle.
    lpSubKey - Supplies a path name to the key that is to be unloaded.
        The combination of hKey and lpSubKey must refer to a hive in the Registry created with RegRestoreKey or RegLoadKey.
...
--*/
{
    ...
    //  Remove terminating NULLs from Length counts. These were added on the client side so that RPC would transmit the whole thing.
    if ( lpSubKey && lpSubKey->Length > 0 ) {
        lpSubKey->Length -= sizeof( UNICODE_NULL );
    }

    InitializeObjectAttributes(&ObjaKey, lpSubKey, OBJ_CASE_INSENSITIVE, hKey, NULL);
...
Arguments:
    ...
    lpSubKey - Supplies a path name to the key that is to be replaced.
        The combination of hKey and lpSubKey must refer to a hive in the Registry.
        This parameter may be NULL.
...
--*/
{
    ...
    NtStatus = NtCreateKey(&HiveHandle, MAXIMUM_ALLOWED, &ObjaKey, 0, NULL, REG_OPTION_BACKUP_RESTORE, NULL);
    ...
    ErrorFlag = RtlDosPathNameToNtPathName_U(lpNewFile->Buffer, &NewFileName, NULL, &RelativeName);
    ...
    InitializeObjectAttributes(&ObjaNewFile, &NewFileName, OBJ_CASE_INSENSITIVE, RelativeName.ContainingDirectory, NULL);

    // Convert the old DOS path name to a canonical Nt path name.
    ErrorFlag = RtlDosPathNameToNtPathName_U(lpOldFile->Buffer, &OldFileName, NULL, &RelativeName);
    ...
    // Initialize the Obja structure for the new file.
    InitializeObjectAttributes(&ObjaOldFile, &OldFileName, OBJ_CASE_INSENSITIVE, RelativeName.ContainingDirectory, NULL);

    NtStatus = NtReplaceKey(&ObjaNewFile, HiveHandle, &ObjaOldFile);
