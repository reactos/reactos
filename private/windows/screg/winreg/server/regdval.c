...
error_status_t BaseRegDeleteValue (HKEY hKey, PUNICODE_STRING lpValueName)
/*++
Routine Description:
    Remove the named value from the specified key.
Arguments:
    hKey - Supplies a handle to the open key.
    lpValueName - Supplies the name of the value to remove.  lpValueName may be NULL.
Return Value:
...
--*/
{
    ...
    //  Subtract the NULL from the string length. This was added by the client so that RPC would transmit the whole thing.
    lpValueName->Length -= sizeof( UNICODE_NULL );

    // Call the Nt Api to delete the value, map the NTSTATUS code to a Win32 Registry error code and return.
    ...
    if (REG_CLASS_IS_SPECIAL_KEY(hKey)) {
        Status = BaseRegGetUserAndMachineClass(NULL, hkDelete, MAXIMUM_ALLOWED, &hkMachineClasses, &hkUserClasses);
    ...
    Status = NtDeleteValueKey(hkDelete, lpValueName);

#ifdef LOCAL
    // For class keys, try again with machine if there were two keys of the same name
    if (REG_CLASS_IS_SPECIAL_KEY(hKey)) {
        if ((STATUS_OBJECT_NAME_NOT_FOUND == Status) && (hkUserClasses && hkMachineClasses)) {
            Status = NtDeleteValueKey(hkMachineClasses, lpValueName);
        }
    }
