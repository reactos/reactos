...

{
...
#endif // LOCAL
    {
        ...
        InitializeObjectAttributes(&Obja, lpSubKey, OBJ_CASE_INSENSITIVE, hKey, NULL);
        Status = NtOpenKey(&KeyHandle, DELETE, &Obja);
    }
...
// local-only check for restricted
    ...
    if (!NT_SUCCESS(Status) && HKEY_RestrictedSite)
    {
        ...
        if (NULL != MappedKey)
        {
            ...
            InitializeObjectAttributes(&Obja, &NewSubKey, OBJ_CASE_INSENSITIVE, MappedKey, NULL);
            Status2 = NtOpenKey(&KeyHandle, DELETE, &Obja);
            RtlFreeUnicodeString(&NewSubKey);
            NtClose(MappedKey);
            Status = (NT_SUCCESS(Status2) || STATUS_OBJECT_NAME_NOT_FOUND == Status) ? Status2 : Status;
        }
    }
