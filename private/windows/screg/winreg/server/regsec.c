...
Abstract:
    This module contains code to apply security to the otherwise unsecured top level keys, in fashion that will allow existing consumers access to the keys that they need (print, srvmgr, etc.).
    ...
    if (Status == STATUS_BUFFER_TOO_SMALL)
    {
        ...
        if (pSD)
        {
            ...
            if (NT_SUCCESS(Status))
            {
                ...
                if ( NT_SUCCESS( Status ) )
                {
                    pSidCopy = RtlAllocateHeap( RtlProcessHeap(), 0, RtlLengthSid( pSid ) );
                    if ( pSidCopy )
                    {
...
GetSDFromKey_BadAcl:
        ...
        pSD = RtlAllocateHeap(RtlProcessHeap(), 0, sizeof(SECURITY_DESCRIPTOR) );
        if (pSD)
        {
...

{
    ...
    if ( ( Status == STATUS_NO_IMPERSONATION_TOKEN ) || ( Status == STATUS_NO_TOKEN ) )
    {
        Token = NULL ;
    }
    else if ( Status == STATUS_SUCCESS )
    {
        ...
        Status = NtSetInformationThread( NtCurrentThread(), ThreadImpersonationToken, (PVOID) &NullHandle, (ULONG) sizeof( NullHandle ) );
    }
    else
    {
        return Status ;
    ...
    Status = NtOpenKey( &hKey, KEY_READ, &ObjAttr);

    if ( Token )
    {
        NTSTATUS RestoreStatus;

        RestoreStatus = NtSetInformationThread( NtCurrentThread(), ThreadImpersonationToken, (PVOID) &Token, sizeof( NullHandle ) );
        NtClose( Token );
    ...
    PULONG              ListCount)
{
    ...
    Status = NtQueryValueKey(   hKey, &UniString, KeyValuePartialInformation, NULL, 0, &Size );
    ...
    if ( pValue )
    {
        Status = NtQueryValueKey(   hKey, &UniString, KeyValuePartialInformation, pValue, Size, &Size );
    }
    ...
    Paths = RtlAllocateHeap( RtlProcessHeap(), 0, StringCount * sizeof(UNICODE_STRING) );
    if ( !Paths )
    {
        RtlFreeHeap( RtlProcessHeap(), 0, pValue );
        return( FALSE );
    ...
    *ListCount = StringCount;
    ...
    while ( *Scan )
    {
        RtlInitUnicodeString( &Paths[ StringCount ], Scan );
    ...
    )
{
    ...
    Status = NtOpenKey( &hKey, KEY_READ, &ObjAttr);// Open the key in local system context
