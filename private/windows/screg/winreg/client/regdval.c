...
Routine Description:
    ...
    RegDeleteValueA converts the lpValueName argument to a counted Unicode string and then calls BaseRegDeleteValue.
--*/
{
    ...
    ValueName->Length += sizeof( UNICODE_NULL );//  Add terminating NULL to Length so that RPC transmits it

    // Call the Base API, passing it the supplied parameters and the counted Unicode strings.
    if( IsLocalHandle( hKey )) {
        Result = (LONG)LocalBaseRegDeleteValue (hKey, ValueName);
    } else {
        Result = (LONG)BaseRegDeleteValue (DereferenceRemoteHandle( hKey ), ValueName);
    }
...
Routine Description:
    ...
    RegDeleteValueW converts the lpValueName argument to a counted Unicode string and then calls BaseRegDeleteValue.
--*/
{
