...
// Macros to manage local versus remote handles (HKEYs), as well as class registration keys from HKEY_CLASSES_ROOT
...
//  BOOL IsLocalHandle(IN HKEY Handle);
#define IsLocalHandle( Handle )      ( ! ((( DWORD_PTR )( Handle )) & REMOTE_HANDLE_TAG ))


//  BOOL IsSpeciaClassesHandle(IN HKEY Handle);
#define IsSpecialClassesHandle( Handle )                                 \
    ( ((( DWORD_PTR )( Handle )) & REG_CLASSES_SPECIAL_TAG ))

//  VOID TagRemoteHandle(IN PHKEY Handle);
#define TagRemoteHandle( Handle )                                       \
    ASSERT( IsLocalHandle( *Handle ));                                  \
    ( *Handle = (( HKEY )((( DWORD_PTR )( *Handle )) | REMOTE_HANDLE_TAG )))


//  HKEY DereferenceRemoteHandle(IN HKEY Handle);
#define DereferenceRemoteHandle( Handle )                               \
    (( HKEY )((( DWORD_PTR )( Handle )) & ~REMOTE_HANDLE_TAG ))


//  HKEY TagSpecialClassesHandle (IN HKEY Handle);
...
// ConvertKey is a temporary work around to convert predefined registry keys that are not sign extended to their sign extended form for 64-bit systems.
#if defined(_WIN64)
__inline VOID ConvertKey(HKEY *Handle)
{
