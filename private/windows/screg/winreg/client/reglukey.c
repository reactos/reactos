...
--*/
{
    ...
    //  Add the NULL to the length so that RPC will transmit the entire thing
    if ( SubKey.Length > 0 ) {
        SubKey.Length += sizeof( UNICODE_NULL );
    }

    RtlInitUnicodeString(&File, lpFile);
    ...
    // Call the server
    if( IsLocalHandle( Handle )) {
        Error = (LONG)LocalBaseRegLoadKey(Handle, &SubKey, &File);
    } else {
        Error = (LONG)BaseRegLoadKey(DereferenceRemoteHandle( Handle ), &SubKey, &File);
    }
...

{
    ...
    RtlInitUnicodeString(&SubKey, lpSubKey);
    RtlInitUnicodeString(&NewFile, lpNewFile);
    RtlInitUnicodeString(&OldFile, lpOldFile);
