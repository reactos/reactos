...
Abstract:
    This module contains the client side wrappers for the Win32 Registry query key APIs.
    That is:
        - RegQueryInfoKeyA
        - RegQueryInfoKeyW
...
--*/
{
    ...
    if( ARGUMENT_PRESENT( lpReserved ) || (ARGUMENT_PRESENT( lpClass ) && ( ! ARGUMENT_PRESENT( lpcbClass )))) {
    ...
    //  case we use the unicode string allocated on the stack, because we must not change the Buffer or MaximumLength fields of the static unicode string in the TEB.
    if ( !ARGUMENT_PRESENT( lpClass ) || *lpcbClass == 0 ) {
        ...
        // Use the static Unicode string in the TEB as a temporary for the key's class.
        Class = &NtCurrentTeb( )->StaticUnicodeString;
        ASSERT( Class != NULL );
        Class->Length = 0;
    }

    // Call the Base API passing it a pointer to a counted Unicode string for the class string.
    if( IsLocalHandle( hKey )) {
        ...
        if (Error == ERROR_SUCCESS) {
            ...
            // Check for a downlevel Win95 server, which requires us to work around their BaseRegQueryInfoKey bugs.
            // They do not account for Unicode correctly.
            if (IsWin95Server(DereferenceRemoteHandle(hKey),dwVersion)) {
                // This is a Win95 server.
                // Double the maximum value name length and maximum value data length to account for the Unicode translation that Win95 forgot to account for.
    ...
    //  MaxSubKeyLen, MaxClassLen, and MaxValueNameLen should be in number of characters, without counting the NULL.
    //  Note that the server side will return the number of bytes, without counting the NUL
    cbMaxSubKeyLen /= sizeof( WCHAR );
    cbMaxClassLen /= sizeof( WCHAR );
    cbMaxValueNameLen /= sizeof( WCHAR );

    //  Subtract the NULL from the Length. This was added on the server side so that RPC would transmit it.
    if ( Class->Length > 0 ) {
        Class->Length -= sizeof( UNICODE_NULL );
    }

    // If all the information was succesfully queried from the key convert the class name to ANSI and update the class length value.
    if( ( Error == ERROR_SUCCESS ) && ARGUMENT_PRESENT( lpClass ) && ( *lpcbClass != 0 ) ) {
        ...
        Status = RtlUnicodeStringToAnsiString(&AnsiString, Class, FALSE);
        ASSERTMSG( "Unicode->ANSI conversion of Class ", NT_SUCCESS( Status ));
        ...
        *lpcbClass = AnsiString.Length;
        ...
        // Not all of the information was succesfully queried, or Class doesn't have to be converted from UNICODE to ANSI
...
--*/
{
    ...
    if( IsLocalHandle( hKey )) {
        ...
        if (Error == ERROR_SUCCESS) {
            ...
            if (IsWin95Server(DereferenceRemoteHandle(hKey),dwVersion)) {
                // This is a Win95 server.
                // Double the maximum value name length and maximum value data length to account for
                // the Unicode translation that Win95 forgot to account for.
                cbMaxValueNameLen *= sizeof(WCHAR);
                cbMaxValueLen *= sizeof(WCHAR);
            }
        }
