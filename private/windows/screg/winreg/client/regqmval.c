...
--*/
{
    ...
    // so that any Unicode value data will fit before it is converted to Ansi.
    ...
    if (IsLocalHandle(hKey)) {
        Error = (LONG)LocalBaseRegQueryMultipleValues(hKey, Values, num_vals, NewValueBuf, pTotalSize);
    } else {
        DWORD dwVersion;
        Error = (LONG)BaseRegQueryMultipleValues(DereferenceRemoteHandle( hKey ), Values, num_vals, NewValueBuf, pTotalSize);
        if ((Error == ERROR_SUCCESS) && (IsWin95Server(DereferenceRemoteHandle(hKey),dwVersion))) {
            // Win95's RegQueryMultipleValues doesn't return Unicode value data, so do not try and convert it back to Ansi.
            for (i=0; i<num_vals; i++) {
    ...
    if (Error == ERROR_SUCCESS) {
        ...
        for (i=0; i < num_vals; i++) {
            ...
            if ((val_list[i].ve_type == REG_SZ) || (val_list[i].ve_type == REG_EXPAND_SZ) || (val_list[i].ve_type == REG_MULTI_SZ)) {
                Status = RtlUnicodeToMultiByteN(lpValueBuf + DataOffset, Values[i].rv_valuelen/sizeof(WCHAR), &AnsiLength, (PWCH)(NewValueBuf + Values[i].rv_valueptr), Values[i].rv_valuelen);
                if (!NT_SUCCESS(Status)) {
                    Error =  RtlNtStatusToDosError( Status );
                }
                ...
                CopyMemory(lpValueBuf + DataOffset, NewValueBuf + Values[i].rv_valueptr, Values[i].rv_valuelen);
        ...
        // available. So just return the required bytes for the Unicode data, as this will always be enough.
        if (ldwTotsize != NULL) {
...
Return value:
    ...
    dynamic key, it will return ERROR_CANTREAD.  If the total length of the
    requested data (valListLength + ldwTotSize) is more than the system limit of one
    megabytes, then the function returns ERROR_TRANSFER_TOO_LONG and only the first megabyte of data is returned.
--*/
{
    NTSTATUS Status;
    PRVALENT Values;
    ...
    // Note that we have to copy the value names because RPC tromps on them.
    for (i=0; i<num_vals; i++) {
    ...
    // Call the Base API, passing it the supplied parameters and the counted Unicode strings.
    if (IsLocalHandle(hKey)) {
        Error = (LONG)LocalBaseRegQueryMultipleValues(hKey, Values, num_vals, (LPSTR)lpValueBuf, pTotalSize);
    } else {
        DWORD dwVersion;
        if (IsWin95Server(DereferenceRemoteHandle(hKey),dwVersion)) {
            // We cannot support RegQueryMultipleValuesW to Win95 servers since they do not return Unicode value data.
            Error = ERROR_CALL_NOT_IMPLEMENTED;
        } else {
            Error = (LONG)BaseRegQueryMultipleValues(DereferenceRemoteHandle( hKey ), Values, num_vals, (LPSTR)lpValueBuf, pTotalSize);
        }
    }
    if (Error == ERROR_SUCCESS) {
        // Convert results back.
