...
--*/
{
    ...
    // If the sub-key is NULL or points to an empty string then the value is to be queried from this key (i.e.  hKey) otherwise the sub-key needs to be opened.
    if(( lpSubKey == NULL ) || ( *lpSubKey == '\0' )) {
        ChildKey = hKey;
    } else {
        // The sub-key was supplied so impersonate the client and attempt to open it.
    ...
    // ChildKey contains an HKEY which may be the one supplied (hKey) or returned from RegOpenKeyExA. Query the value using the special value name NULL.
    ...
    // If the type of the value is not a null terminate string, then return an error. (Win 3.1 compatibility)
...
--*/
{
    ...
    if( ! NT_SUCCESS( Status )) {
        ...
        Status = RtlAnsiStringToUnicodeString(&StubValueName, &AnsiString, TRUE);
        if( ! NT_SUCCESS( Status )) {
    ...
    ValueName->Length += sizeof( UNICODE_NULL );//  Add the terminating NULL to the Length so that RPC transmits it.

    // Call the Base API, passing it the supplied parameters and the counted Unicode strings. Note that zero bytes are transmitted (i.e. InputLength = 0) for the data.
    ...
    if( IsLocalHandle( hKey )) {
        ...
        ASSERT( ValueName->Buffer );//  Make sure that the local side didn't destroy the Buffer in the StaticUnicodeString
    } else {
        Error = (LONG)BaseRegQueryValue (DereferenceRemoteHandle( hKey ), ValueName, &ValueType, lpData, &ValueLength, &InputLength);
    }

    // If no error or callers buffer too small, and type is one of the null terminated string types, then do the UNICODE to ANSI translation.
    // We handle the buffer too small case, because the callers buffer may be big enough for the ANSI representation, but not the UNICODE one.
    // In this case, we need to allocate a buffer big enough, do the query again and then the translation into the callers buffer.  We only do
    // this if the caller actually wants the value data (lpData != NULL)
    if ((Error == ERROR_SUCCESS || Error == ERROR_MORE_DATA) && (ARGUMENT_PRESENT( lpData ) || ARGUMENT_PRESENT( lpcbData ))&& (ValueType == REG_SZ || ValueType == REG_EXPAND_SZ || ValueType == REG_MULTI_SZ)) {
        ...
        if (UnicodeValueBuffer == NULL) {
            ...
            if( IsLocalHandle( hKey )) {
                //  Add the terminating NULL to the Length (remember that in the local case, ValueName->Length
                //  was decremented by sizeof( UNICODE_NULL ) in the first call to LocalBaseRegQueryValue).
                //  This won't happen in the remote case, since the server side will decrement ValueName->Length on
                //  the transmitted structure (a copy of ValueName), and the new Valuename->Length won't be transmitted back to the client.
                ...
                Error = (LONG)BaseRegQueryValue (DereferenceRemoteHandle( hKey ), ValueName, &ValueType, (LPBYTE)UnicodeValueBuffer, &ValueLength, &InputLength);
            }
            // Compute needed buffer size , cbAnsi will keeps the byte counts to keep MBCS string after following step.
        ...
        if ((Error == ERROR_SUCCESS) && (AnsiValueBuffer != NULL) ) {
            // We have a UNICODE value, so translate it to ANSI in the callers buffer.  In the case where the caller's buffer was big enough
            // for the UNICODE version, we do the conversion in place, which works since the ANSI version is smaller than the UNICODE version.
            Index = 0;
            Status = RtlUnicodeToMultiByteN( AnsiValueBuffer, AnsiValueLength, &Index, UnicodeValueBuffer, UnicodeValueLength);
            if (!NT_SUCCESS( Status )) {
                Error = RtlNtStatusToDosError( Status );
            }

            cbAnsi = Index;// Now Index keeps Byte counts of MBCS string in AnsiValueBuffer
        }
        ...
        ValueLength = cbAnsi;// Return the length of the ANSI version to the caller.
        ...
        // If the last character of the buffer is not a NULL and there is enough space left in the caller's buffer, slap a NULL in there to prevent him from going nuts trying to do a strlen().
        if (ARGUMENT_PRESENT( lpData ) && (*lpcbData > ValueLength)  && (ValueLength > 0) && (lpData[ValueLength-1] != '\0')) {
            lpData[ValueLength] = '\0';
        }
    }

    // Stored the returned length in the caller specified location and return the error code.
    if (lpdwType != NULL) {
        *lpdwType = ValueType;
    }
    ...
    RtlFreeUnicodeString(&StubValueName);// Free the temporary Unicode string stub allocated for the ValueName
...
--*/
{
    ...
    // Validate dependency between lpData and lpcbData parameters.
    if( ARGUMENT_PRESENT( lpReserved ) || (ARGUMENT_PRESENT( lpData ) && ( ! ARGUMENT_PRESENT( lpcbData )))) {
        return ERROR_INVALID_PARAMETER;
    }
    ...
    if ( (Error == ERROR_SUCCESS) && ARGUMENT_PRESENT( lpData ) && ( (ValueType == REG_SZ) || (ValueType == REG_EXPAND_SZ) || (ValueType==REG_MULTI_SZ)) && ( ValueLength > sizeof(WCHAR))) {
        ...
        if ((String[Length-1] != UNICODE_NULL) && (ValueLength+sizeof(WCHAR) <= *lpcbData)) {
