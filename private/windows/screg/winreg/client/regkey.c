...
LONG APIENTRY RegOverridePredefKey (IN HKEY hKey, IN HKEY hNewKey)
/*
Routine Description:
...
LONG APIENTRY RegCreateKeyA (HKEY hKey, LPCSTR lpSubKey, PHKEY phkResult)
/*
Routine Description:
...
--*/
{
    ...
    // Win3.1ism - Win 3.1 allows the predefined handle to be opened by specifying a pointer to an empty or NULL string for the sub-key.
    // If the subkey is NULL or points to a NUL string and the handle is predefined, just return the predefined handle (a virtual open) otherwise it's an error.
    ConvertKey(&hKey);
...
LONG APIENTRY RegCreateKeyW (HKEY hKey, LPCWSTR lpSubKey, PHKEY phkResult)
/*
Routine Description:
...
--*/
{
    ...
    Status = RtlAnsiStringToUnicodeString(SubKey, &AnsiString, FALSE);
    ...
    if (ARGUMENT_PRESENT( lpClass )) {
        ...
        Status = RtlAnsiStringToUnicodeString(&ClassUnicode, &AnsiString, TRUE);
        if( ! NT_SUCCESS( Status )) {
            Error = RtlNtStatusToDosError( Status );
            goto ExitCleanup;
        }
    ...
    // If the caller supplied a LPSECURITY_ATTRIBUTES argument, map it to the RPCable version.
    ...
    } else {// No PSECURITY_ATTRIBUTES argument, therefore no mapping was done.
        pRpcSA = NULL;
    }
    ...
    if( IsLocalHandle( hKey )) {
        Error = (LONG)LocalBaseRegCreateKey (hKey, SubKey, Class, dwOptions, samDesired, pRpcSA, phkResult, lpdwDisposition);
    } else {
    ...
    // Free the counted Unicode string allocated by RtlAnsiStringToUnicodeString.
    if (Class != NULL) {
        RtlFreeUnicodeString( Class );
    }

    // Free the RPC_SECURITY_DESCRIPTOR buffer and return the Registry return value.
    if( pRpcSA != NULL ) {
...
--*/
{
    ...
    SubKey.Length += sizeof( UNICODE_NULL );//  Add terminating NULL to Length so that RPC transmits it.
    ...
    // If the caller supplied a LPSECURITY_ATTRIBUTES argument, map it and call the private version of the create key API.
    if( ARGUMENT_PRESENT( lpSecurityAttributes )) {
        pRpcSA = &RpcSA;
        Error = MapSAToRpcSA( lpSecurityAttributes, pRpcSA );
        if( Error != ERROR_SUCCESS ) {
...
--*/
{
    ...
    //  Caller must pass pointer to the variable where the opened handle will be returned
    if( phkResult == NULL ) {
        return ERROR_INVALID_PARAMETER;
    }
    ...
    //  If hKey is a predefined key, and lpSubKey is either a NULL pointer or a NUL string, close the predefined key and clear the associated entry
    //  in the PredefinedHandleTable  (RegCloseKey will do the job).
    if( IsPredefinedRegistryHandle( hKey ) && ( ( lpSubKey == NULL ) || ( *lpSubKey == '\0' ) ) ) {
        if ( HKEY_CLASSES_ROOT != hKey ) {
            ...
            //  Create a handle and save it in the appropriate entry  in PredefinedHandleTable.
    ...
    // Call the Base API, passing it the supplied parameters and the counted Unicode strings.
    if( IsLocalHandle( hKey )) {
        Error = (LONG)LocalBaseRegOpenKey (hKey, SubKey, dwOptions, samDesired, phkResult);
    } else {
...
--*/
{
    ...
    //  If hKey is a predefined key, and lpSubKey is either a NULL pointer or a NUL string, close the predefined key and clear the associated entry in the PredefinedHandleTable  (RegCloseKey will do the job).
    if( IsPredefinedRegistryHandle( hKey ) && ( ( lpSubKey == NULL ) || ( *lpSubKey == ( WCHAR )'\0' ) ) ) {
        if ( HKEY_CLASSES_ROOT != hKey ) {
            Error = RegCloseKey( hKey );
            if( Error != ERROR_SUCCESS ) {
                return( Error );
...
Routine Description:
    ...
    process is impersonating multiple users, this allows access to the appropriate key.
Arguments:
