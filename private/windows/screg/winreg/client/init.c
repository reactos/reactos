...
Abstract:
    This module contains the entry point for the Win32 Registry APIs client side DLL.
...
BOOL RegInitialize (IN HANDLE   Handle, IN DWORD    Reason, IN PVOID    Reserved)
/*++
Routine Description:
    Returns TRUE.
Arguments:
...
--*/
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        if( !InitializeRegNotifyChangeKeyValue( ) || !LocalInitializeRegCreateKey() || !InitializePredefinedHandlesTable() ) {
            return( FALSE );
        }
#else
#if defined(_REGCLASS_MALLOC_INSTRUMENTED_)
        ...
        if( !LocalInitializeRegCreateKey() || !InitializePredefinedHandlesTable() || !InitializeHKEYRestrictedSite() || !InitializeClassesRoot()) {
            return( FALSE );
        }
#endif
#if defined(LEAK_TRACK)
    ...
    case DLL_PROCESS_DETACH:
        ...
        if( Reserved == NULL && !CleanupPredefinedHandles()) {
            return( FALSE );
        }
...
#ifndef REMOTE_NOTIFICATION_DISABLED
        if( !CleanupRegNotifyChangeKeyValue( ) || !LocalCleanupRegCreateKey() || !CleanupPredefinedHandlesTable() || !CleanupClassesRoot( FALSE ) {
            return( FALSE );
        }
#else
        if( !LocalCleanupRegCreateKey() || !CleanupPredefinedHandlesTable() || !CleanupClassesRoot( FALSE )) {
            return( FALSE );
        }
#if defined(LEAK_TRACK)
...
/*++
Routine Description:
    If the current process has a restricted token then some registry accesses get remapped to HKCU\RestrictedSites\<site sid>
Arguments:
    None
Return Value:
...
--*/
{
    ...
    if (STATUS_SUCCESS == status)
    {
        MaximumLength = (USHORT) (CurrentUserPath.Length + sizeof(L"\\RestrictedSites\\") + lstrlenW(SiteDirectory) * sizeof(WCHAR));
        RestrictedSitesPath.Length        = 0;
        RestrictedSitesPath.MaximumLength = MaximumLength;
        RestrictedSitesPath.Buffer        = RtlAllocateHeap(RtlProcessHeap(), 0, MaximumLength);
        if (NULL != RestrictedSitesPath.Buffer)
        {
            RtlCopyUnicodeString(&RestrictedSitesPath, &CurrentUserPath);
            RtlAppendUnicodeToString(&RestrictedSitesPath, L"\\RestrictedSites\\");
            RtlAppendUnicodeToString(&RestrictedSitesPath, SiteDirectory);
            InitializeObjectAttributes(&attributes, &RestrictedSitesPath, OBJ_CASE_INSENSITIVE, NULL, NULL)
            status = NtOpenKey(&HKEY_RestrictedSite, MAXIMUM_ALLOWED, &attributes);
