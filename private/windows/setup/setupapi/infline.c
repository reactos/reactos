    ...
    )
{
    ...
    // Traverse the linked list of loaded INFs, looking for the specified section.
    try {
...
BOOL SetupFindNextLine(IN  PINFCONTEXT ContextIn, OUT PINFCONTEXT ContextOut)
{
...
#ifdef UNICODE
// ANSI version
LONG SetupGetLineCountA(IN HINF  InfHandle, IN PCSTR Section)
{
    PCWSTR section;
    LONG l;
    DWORD d;
...
#else
// Unicode stub
LONG SetupGetLineCountW(IN HINF   InfHandle, IN PCWSTR Section)
{
    UNREFERENCED_PARAMETER(InfHandle);
    UNREFERENCED_PARAMETER(Section);
    SetLastError(ERROR_CALL_NOT_IMPLEMENTED);
...
LONG SetupGetLineCount(IN HINF   InfHandle, IN PCTSTR Section)
{
    PLOADED_INF CurInf;
    PINF_SECTION InfSection;
    LONG LineCount;
