...
BOOL _WouldNeverLog(IN PSETUP_LOG_CONTEXT LogContext, IN DWORD Level)
/*++
Routine Description:
    ...
    no locking performed - safe to call if locked, or if we know we won't decrement usage count below 1.
Arguments:
    LogContext - optionally supplies a pointer to the SETUP_LOG_CONTEXT to be used for logging. If not supplied always return FALSE.
...
--*/
{
    ...
    if (((LogContext->LogLevel & SETUP_LOG_LEVELMASK) <= SETUP_LOG_NOLOG) &&((LogContext->LogLevel & DRIVER_LOG_LEVELMASK) <= DRIVER_LOG_NOLOG)) {
...
__inline // we want to always optimize this out
BOOL _WouldLog(IN PSETUP_LOG_CONTEXT LogContext, IN DWORD Level)
/*++
Routine Description:
...
--*/
{
    ...
    // Win9x provides no way to wait for a file to become unlocked, so we have to poll.
    // Putting this Sleep(0) allows others to have a chance at the file.
    Sleep(0);
}
