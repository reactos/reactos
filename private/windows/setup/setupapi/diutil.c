...
BOOL RemoveCDMDirectory(IN PTSTR FullPathName)
/*++
Routine Description:
    This routine deletes the Code Download Manager temporary directory.
    It will only delete the directory and all of it's contents if:
    ...
    We will strip off the filename and remove the entire directory where this INF file is located.
Arguments:
...
--*/
{
    ...
    if (_tcsnicmp(TempPath, Directory, sizeof(TempPath) / sizeof(TCHAR))) {
        return _RemoveCDMDirectory(Directory);//Remove the directory
    } else {
            TCHAR Debug[512];
...
VOID DestroyDriverNodes(IN PDRIVER_NODE DriverNode, IN PDEVICE_INFO_SET pDeviceInfoSet)
/*++
Routine Description:
...
--*/
{
    ...
    while(DriverNode) {
        ...
        // If this driver was from the Internet then we want to delete the directory where it lives.
...
DWORD AcquireSCMLock(IN SC_HANDLE SCMHandle, OUT SC_LOCK *pSCMLock)
/*++
Routine Description:
    This routine attempts to lock the SCM database.
    If it is already locked it will retry ACQUIRE_SCM_LOCK_ATTEMPTS times at intervals of ACQUIRE_SCM_LOCK_INTERVAL.
Arguments:
    ...
    *pSCMLock = NULL;
    ...
    while((*pSCMLock = LockServiceDatabase(SCMHandle)) == NULL && Attempts > 0) {
        ...
        if((Err = GetLastError()) == ERROR_SERVICE_DATABASE_LOCKED) {
            Attempts--;
            Sleep(ACQUIRE_SCM_LOCK_INTERVAL);// Sleep for specified time
        } else {
...
--*/
{
    ...
    // OK, next try to retrieve the DeviceDesc property (if we're working on a device information element.
    if(DeviceInfoData) {
        if(SetupDiGetDeviceRegistryProperty(DeviceInfoSet, DeviceInfoData, SPDRP_DEVICEDESC, NULL, (PBYTE)DeviceDescBuffer, LINE_LEN * sizeof(TCHAR), NULL)) {
            return TRUE;
        }
    }
