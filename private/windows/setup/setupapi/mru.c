...
VOID pSetupStripTrailingPlatformComponent(IN OUT PTSTR  *Paths, IN OUT PDWORD  NumPaths);


BOOL _SetupSetSourceList(IN DWORD   Flags, IN PCTSTR *SourceList, IN UINT    SourceCount)
/*++
Routine Description:
    This routine allows the caller to set the list of installation
    sources for either the current user or the system (common to all users).

Arguments:

    Flags - a combination of the following values:

        SRCLIST_SYSTEM - specify that the list is to become the per-system list. The caller must be administrator.

        SRCLIST_USER - specify that the list is to become the per-user list.

        SRCLIST_TEMPORARY - specify that the list is to become the entire list for the duration of the current process,
            or until this routine is called again to change the behavior.

        Exactly one of SRCLIST_SYSTEM, SRCLIST_USER, and SRCLIST_TEMPORARY must be specified.

        SRCLIST_NOBROWSE - specify that the user is not allowed to add or change sources when the SetupPromptForDisk API is used.
            Typically used in combination with SRCLIST_TEMPORARY.

    SourceList - supplies array of strings that are to become the source list, as described by the Flags parameter.
...
BOOL SetupSetSourceListA(IN DWORD   Flags, IN PCSTR  *SourceList, IN UINT    SourceCount)
{
    PCWSTR *sourceList;
    UINT u;
    DWORD rc;
...
/*++
Routine Description:
    This routine cancels any temporary list and no-browse behavior and reverts to standard list behavior.
Arguments:
    None.
Return Value:
...
BOOL _SetupAddToSourceList(IN DWORD  Flags, IN PCTSTR Source)
/*++
Routine Description:
    This routine allows the caller to append a value to the list of installation sources for either the current user or the system.
    If the value already exists it is removed first.
Arguments:
    Flags - a combination of the following values:

        SRCLIST_SYSTEM - specify that the source is to added to the per-system list. The caller must be administrator.

        SRCLIST_USER - specify that the list is to be added to the per-user list.

        SRCLIST_SYSIFADMIN - specifies that if the caller is administrator, then the source is added to the system list; if the caller
            is not administrator then the source is added to the per-user list for the current user.
        ...
        SRCLIST_APPEND - specify that the source is to be added to the end of the given list. Otherwise it is added to the beginning.
...
--*/
{
    ...
    if (TemporarySourceList) {
        ...
        if (Flags & SRCLIST_SYSTEM) {
            if (IsUserAdmin()) {
                d = QueryMultiSzValueToArray(HKEY_LOCAL_MACHINE, pszPerSystemKey, pszPerSystemVal, &Lists[0], &Counts[0], FALSE);
        ...
        if ((Flags & SRCLIST_USER) && (d == NO_ERROR)) {
            d = QueryMultiSzValueToArray(HKEY_LOCAL_MACHINE, pszPerSystemKey, pszPerSystemVal, &Lists[NumberOfLists], &Counts[NumberOfLists], FALSE);
            if (d == NO_ERROR) {
            ...
            if (p = DuplicateString(Source)) {
                if (pTmp = MyRealloc(Lists[u], (Counts[u] + 1) * sizeof(PTSTR))) {
                    ...
                    if (TemporarySourceList) {
                        ...
                        d = SetArrayToMultiSzValue(RootKeys[u], SubKeys[u], Vals[u], Lists[u], Counts[u]);
                        if (NeedToFree[u]) {
...
BOOL SetupAddToSourceListA(IN DWORD  Flags, IN PCSTR  Source)
{
    BOOL b;
    DWORD rc;
    PCWSTR source;
...
BOOL SetupAddToSourceListW(IN DWORD  Flags, IN PCWSTR Source)
{
    UNREFERENCED_PARAMETER(Flags);
...
BOOL SetupAddToSourceList(IN DWORD  Flags, IN PCTSTR Source)
{
    BOOL b;
    DWORD rc;
    PCTSTR source;
...
BOOL _SetupRemoveFromSourceList(IN DWORD  Flags, IN PCTSTR Source)
/*++
Routine Description:
    This routine allows the caller to remove a value from the list of installation sources for either the current user or the system.
...
Arguments:
    ...
    Flags - a combination of the following values:
        ...
        SRCLIST_SYSTEM - specify that the source is to removed from the per-system list. The caller must be administrator.

        SRCLIST_USER - specify that the list is to be removed from the per-user list.

        SRCLIST_SYSIFADMIN - specifies that if the caller is administrator, then the source is removed from the system list; if the caller
            is not administrator then the source is removed from the per-user list for the current user.

        Any combination of these flags may be specified on a single call.

        If a temporary list is currently in use (see SetupSetSourceList), these 3 flags are ignored and the source is removed from the temporary list.

        SRCLIST_SUBDIRS - specify that all subdirectories of Source are also
            to be removed. The determination of subdirectories is done based on a simple prefix scan.
...
--*/
{
    ...
    if (d == NO_ERROR) {
        ...
        for (u = 0; u < NumberOfLists; u++) {
            ...
            for (v = 0; v < Counts[u]; v++) {
                ...
                if (Match) {
                    ...
                    MoveMemory(&Lists[u][v], &Lists[u][v + 1], (Counts[u] - (v + 1)) * sizeof(PTSTR));
        ...
        if (TemporarySourceList) {
            ...
            u = 0;
            if (Flags & SRCLIST_SYSTEM) {
                d = SetArrayToMultiSzValue(HKEY_LOCAL_MACHINE, pszPerSystemKey, pszPerSystemVal, Lists[0], Counts[0]);
                u++;
            }

            if ((d == NO_ERROR) && (Flags & SRCLIST_USER)) {
                d = SetArrayToMultiSzValue(HKEY_CURRENT_USER, pszPerUserKey, pszPerUserVal, Lists[u], Counts[u]);
                u++;
            }
        }
    }
...
BOOL SetupRemoveFromSourceListA(IN DWORD  Flags, IN PCSTR  Source)
{
    PCWSTR source;
    BOOL b;
    DWORD rc;
...
BOOL SetupRemoveFromSourceListW(IN DWORD  Flags, IN PCWSTR Source)
{
    UNREFERENCED_PARAMETER(Flags);
    UNREFERENCED_PARAMETER(Source);
    SetLastError(ERROR_CALL_NOT_IMPLEMENTED);
...
BOOL SetupRemoveFromSourceList(IN DWORD  Flags, IN PCTSTR Source)
{
    PCTSTR source;
    BOOL b;
    DWORD rc;
...
BOOL SetupQuerySourceListA(IN  DWORD   Flags, OUT PCSTR **List, OUT PUINT   Count)
{
    PCWSTR *list;
    UINT count;
    BOOL b;
