...
// Define the context structure used by the default device comparison callback (used by SetupDiRegisterDeviceInfo).
...
Routine Description:
    ...
    This set may be associated with an optionally-specified class GUID.
Arguments:
    ClassGuid - Optionally, supplies a pointer to the class GUID that is to be associated with this set.
    hwndParent - Optionally, supplies the window handle of the top-level window to use for any UI related to installation of a class driver contained in this set's global class driver list (if it has one).
Return Value:
...
Arguments:
    ...
    hwndParent - Optionally, supplies the window handle of the top-level window
        to use for any UI related to installation of a class driver contained in this set's global class driver list (if it has one).
    ...
    MachineName - Optionally, supplies the name of the machine for which this
        ...
        may be opened/created.  If this parameter is NULL, then the local machine is used.
...
clean0:     ;   // nothing to do.
    ...
    return (Err == NO_ERROR) ? (HDEVINFO)DeviceInfoSet : (HDEVINFO)INVALID_HANDLE_VALUE;
}
...
/*++
Routine Description:
    This API retrieves the class GUID associated with a device information set (if it has an associated class).
Arguments:
    DeviceInfoSet - Supplies a handle to a device information set whose associated class is being queried.
    ClassGuid - Supplies a pointer to a variable that receives the GUID for the associated class.
Return Value:
    If the function succeeds, the return value is TRUE.

    If the function fails, the return value is FALSE.
    To get extended error information, call GetLastError.
    If the set has no associated class, then GetLastError will return ERROR_NO_ASSOCIATED_CLASS.
--*/
{
...
Arguments:
    DeviceInfoSet - Supplies a handle to the device information set to retrieve detailed information for.

    DeviceInfoSetDetailData - Supplies the address of a structure that receives
        information about the specified device information set.  This structure is defined as follows:
...
Remarks:
    The cbSize field of the output structure must be set to sizeof(SP_DEVINFO_LIST_DETAIL_DATA) or the  call will fail with ERROR_INVALID_USER_BUFFER.
--*/
{
    PDEVICE_INFO_SET pDeviceInfoSet;
    DWORD Err;
...
Arguments:
    DeviceInfoSet - Supplies a handle to a device information set to which this new device information element is to be added.
    ...
    DeviceName - Supplies either a full device instance ID (e.g., Root\*PNP0500\0000)
        ...
        ID suffix (e.g., *PNP0500).  The latter may only be specified if the DICD_GENERATE_ID flag is specified in the CreationFlags parameter.

    ClassGuid - Supplies a pointer to the GUID for this device's class.  If the class is not yet known, this value should be GUID_NULL.

    DeviceDescription - Optionally, supplies a textual description of the device.

    hwndParent - Optionally, supplies the window handle of the top-level window to use for any UI related to installing the device.

    CreationFlags - Supplies flags controlling how the device information element is to be created.  May be a combination of the following values:
...
Remarks:
    ...
    of the existing list, the enumeration ordering is preserved, thus we don't need to invalidate our enumeration hint.
--*/
{
    PDEVICE_INFO_SET pDeviceInfoSet;
    DWORD Err, StringLen;
    ...
    try {
        ...
        // set, so that we can easily lop off the new node if we encounter an error after insertion.
...
Routine Description:
    ...
    If a device information element already exists for this device instance, the existing element is returned.
Arguments:
    DeviceInfoSet - Supplies a handle to a device information set to which the opened device information element is to be added.
    ...
    try {
        ...
        if(OpenFlags & DIOD_INHERIT_CLASSDRVS) {
            ...
            if(pDeviceInfoSet->ClassDriverHead) {
                ...
                // We found the driver list object, now increment its refcount, and do the inheritance.
                CurDrvListObject->RefCount++;
...
--*/
{
    return SetupDiGetClassDevsEx(ClassGuid, Enumerator, hwndParent, Flags, NULL, NULL, NULL);
}
...
--*/
{
    ...
    if(!DeviceInfoSet || (DeviceInfoSet == INVALID_HANDLE_VALUE)) {
        ...
        // a copy of it, and work with that one, so that if something fails, we haven't screwed up the original one.
    ...
    try {
        ...
        if(hDevInfo) {
            ...
            if(pDeviceInfoSet->HasClassGuid) {
                ...
                // device interfaces based on the underlying devices' setup class.
                ExistingClassGuid = NULL;
            }
        }
        ...
        do {
            if(GetInterfaceList) {
                if(Flags & DIGCF_ALLCLASSES) {
                    ...
                    pSetupStringFromGuid(ClassGuid, InterfaceGuidString, SIZECHARS(InterfaceGuidString));
                    // We only need to go through this list once.
                    GetNextInterfaceClass = FALSE;
                }

                // We'll be using the same character buffer to store each device instance ID we're opening below.
            ...
            if(GetInterfaceList) {
                if(RegOpenKeyEx(hKeyDevClassRoot, InterfaceGuidString, 0, KEY_READ, &hKeyCurDevClass) != ERROR_SUCCESS) {
                    ...
                    continue;// Skip this interface class.
                }
            }
            ...
            for(CurDevId = DevIdBuffer; *CurDevId; CurDevId += lstrlen(CurDevId) + 1) {
                ...
                // AccessDeviceInfoSet, CloneDeviceInfoSet, and RollbackDeviceInfoSet.)
                Err = pSetupOpenAndAddNewDevInfoElem(pDeviceInfoSet,
                                                     DeviceInstanceToOpen,
                                                     !(Flags & DIGCF_PRESENT),
                                                     ((Flags & (DIGCF_ALLCLASSES | DIGCF_DEVICEINTERFACE)) ? ExistingClassGuid : ClassGuid),
                                                     hwndParent,
                                                     &DevInfoElem,
...
clean0:
        ...
        // Access the following variables, so the compiler will respect the statement ordering in the try clause.
...
Arguments:
    ...
    Description - Optionally, supplies the description of the element to be added.

    hwndParent - Optionally, supplies the handle to the top level window for UI relating to this element.

    DiElemFlags - Specifies flags pertaining to the device information element being created.
    ...
    ContainingDeviceInfoSet - Supplies a pointer to the device information set
        ...
        different from the pDeviceInfoSet parameter if we're working against a cloned devinfo set (i.e., to facilitate rollback).

    DeviceInfoElement - Supplies the address of the variable that receives a pointer to the newly-allocated device information element.

Return Value:
    If the function succeeds, the return value is NO_ERROR, otherwise the ERROR_* code is returned.
Remarks:
    Since the new element is added onto the end of the existing list, our enumeration hint isn't invalidated.
--*/
{
    DWORD Err = NO_ERROR;
    ...
    *DeviceInfoElement = NULL;
    ...
    try {
        ...
        if(Description) {
            ...
            (*DeviceInfoElement)->DeviceDescription = (*DeviceInfoElement)->DeviceDescriptionDisplayName = -1;
        }
...
--*/
{
    return pSetupDiGetOrSetDeviceInfoContext(DeviceInfoSet, DeviceInfoData, Context, NULL);
}
