...
VOID SpFreeTemporaryParseStructures(IN PX_INF Inf)
/*++
Routine Description:
...
BOOL SpAppendSection(IN PPARSE_CONTEXT Context, IN PTCHAR         SectionName)
/*++
Routine Description:
    This appends a new section to the section list in the current INF.
    All further lines and values pertain to this new section, so it resets the line list and value lists too.
Arguments:
...
BOOL SpAppendLine(IN PPARSE_CONTEXT Context, IN PTCHAR         LineKey)
/*++
Routine Description:
    This appends a new line to the line list in the current section.
    All further values pertain to this new line, so it resets the value list too.
Arguments:
    Context - supplies the parse context.
...
BOOL SpAppendValue(IN PPARSE_CONTEXT Context, IN PTCHAR         ValueString)
/*++
Routine Description:
...
Arguments:
    ...
    StreamEnd - Supplies the memory address immediately following the character stream.
Return Value:
    The next token
--*/
{
    ...
    if ((pch < StreamEnd) && ((*pch == TEXT(';')) || (*pch == TEXT('#')) || ((*pch == TEXT('/')) && (*(pch + 1) == TEXT('/'))))) {
        do {
            pch++;
