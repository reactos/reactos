...
BOOL WINAPI SetupDiGetDeviceRegistryProperty(
    IN  HDEVINFO         DeviceInfoSet,
    IN  PSP_DEVINFO_DATA DeviceInfoData,
    IN  DWORD            Property,
    OUT PDWORD           PropertyRegDataType, OPTIONAL
...
Arguments:
    ...
    PropertyRegDataType - Optionally, supplies the address of a variable that will receive the data type of the property being retrieved.
        This will be one of the standard registry data types (REG_SZ, REG_BINARY, etc.)
...
Return Value:
    ...
    If the supplied buffer was not large enough to hold the requested property, the error will be ERROR_INSUFFICIENT_BUFFER, and RequiredSize will specify how large the buffer needs to be.
--*/
{
    BOOL b;
...
clean0:
        ...
        Err = ERROR_INVALID_PARAMETER;
        // Access the following registry handle so the compiler will respect statement ordering w.r.t. its assignment.
        hk = hk;
    }
...
Arguments:
    ...
    Create - Specifies whether the key should be created if doesn't already exist.
    samDesired - Specifies the access you require for this key.
    hInterfaceDeviceKey - Supplies the address of a variable that receives a handle to the requested registry key.
        (This variable will only be written to if the handle is successfully opened.)
Return Value:
...
--*/
{
    ...
    try {
        // Get the interface device node, and verify that its class matches what the caller passed us.
