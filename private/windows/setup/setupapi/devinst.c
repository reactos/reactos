    ...
    try {
        ...
        if(Flags & CALLCI_CALL_HELPERS) {
            if(InstallParamBlock->CoInstallerCount > 0) {
                ...
                ZeroMemory(CoInstallerInternalContext, sizeof(COINSTALLER_INTERNAL_CONTEXT) * InstallParamBlock->CoInstallerCount);
...
clean0: ;   // nothing to do.
    ...
    // If we just did a DIF_REGISTER_COINSTALLERS, then we invalidated our current list of co-installers.
...
--*/
{
    ...
    try {
        ...
        Err = pSetupInstallFiles(hInf,
                                 ...
                                 NULL,
                                 NULL,
                                 SP_COPY_NEWER_OR_SAME | SP_COPY_LANGUAGEAWARE | ((Flags & DI_NOBROWSE) ? SP_COPY_NOBROWSE : 0),
                                 NULL,
                                 FileQueue,
                                 // This flag is ignored by pSetupInstallFiles because we don't pass a callback here and we pass a user-defined file queue.
...
clean0: ;   // nothing to do.
        ...
        // Reference the following variables so that the compiler will respect our statement order w.r.t. assignment.
        hKey = hKey;
...
BOOL WINAPI SetupDiGetHwProfileList(
...
--*/
{
    ...
    // Get the string form of the class GUID, because that will be the name of the multi-sz value entry under HKLM\System\CCS\Control\CoDeviceInstallers
