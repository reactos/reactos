...
Return Value:
    ...
    If logging is not enabled or the text needs to be ignored because of the logging level, TRUE is returned.
--*/
{
    DWORD d;
    LPTSTR text;
    ...
    // We don't use FormatMessageA since that implies ANSI substitutions, and ours are Unicode.
    text = NULL;
    d = FormatMessage(FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_ALLOCATE_BUFFER, MyDllModuleHandle, MessageId, 0, (LPTSTR)&text, 0, &arglist);
    va_end(arglist);
