...
Abstract:
    Private top-level header file for Windows NT Setup services Dll.
...
#if MEM_DBG
VOID SetTrackFileAndLine (PCSTR File, UINT Line);
VOID ClrTrackFileAndLine (VOID);
...
extern BOOL GuiSetupInProgress;// are we inside gui setup? determined at process attach
extern DWORD GlobalSetupFlags;// various other global flags
extern UINT g_uQueryCancelAutoPlay;// global window message for cancelling autoplay.
extern PCTSTR InfSearchPaths;// Static multi-sz list of directories to be searched for INFs.
extern PCTSTR PlatformName;// Name of platform (mips, alpha, x86, ppc, etc)
extern HANDLE g_hHeap;// Debug memory functions and wrappers to track allocations


#if MEM_DBG
PVOID MyDebugMalloc(IN DWORD Size, IN PCSTR Filename, IN DWORD Line);
...
PTSTR TrackedDuplicateString(IN TRACK_ARG_DECLARE, IN PCTSTR String);
...
VOID SetDlgText(IN HWND hwndDlg, IN INT  iControl, IN UINT nStartString, IN UINT nEndString);

#define SDT_MAX_TEXT    1000        // Max SetDlgText() combined text size

PTSTR MyLoadString(IN UINT StringId);
PTSTR FormatStringMessage(IN UINT FormatStringId, ...);
PTSTR FormatStringMessageV(IN UINT     FormatStringId, IN va_list *ArgumentList);
PTSTR FormatStringMessageFromString(IN PTSTR FormatString, ...);
PTSTR FormatStringMessageFromStringV(IN PTSTR    FormatString, IN va_list *ArgumentList);
PTSTR RetreiveAndFormatMessage(IN UINT MessageId, ...);
PTSTR RetreiveAndFormatMessageV(IN UINT     MessageId, IN va_list *ArgumentList);
INT FormatMessageBox(IN HANDLE hinst, IN HWND   hwndParent, IN UINT   TextMessageId, IN PCTSTR Title, IN UINT   Style, ...);

// This is in shell32.dll and in windows\inc16\shlsemip.h but
// that file cannot be #include'd here as it has macros that clash with our own, etc.
int RestartDialog(IN HWND hwnd, IN PCTSTR Prompt, IN DWORD Return);

// Decompression/filename manupilation routines in decomp.c.
PTSTR SetupGenerateCompressedName(IN PCTSTR Filename);
...
// Diamond functions. The Process and Thread Attach routines are called by the DLL entry point routine and should not be called by anyone else.
BOOL DiamondProcessAttach(IN BOOL Attach);
VOID DiamondThreadAttach(IN BOOL Attach);
BOOL DiamondIsCabinet(IN PCTSTR FileName);
DWORD DiamondProcessCabinet(IN PCTSTR CabinetFile, IN DWORD  Flags, IN PVOID  MsgHandler, IN PVOID  Context, IN BOOL   IsUnicodeMsgHandler);

// Misc routines
VOID DiskPromptGetDriveType(IN  PCTSTR PathToSource, OUT PUINT  DriveType, OUT PBOOL  IsRemovable);
BOOL SetTruncatedDlgItemText(HWND hdlg, UINT CtlId, PCTSTR TextIn);
LPTSTR CompactFileName(LPCTSTR FileNameIn, DWORD CharsToRemove);
DWORD ExtraChars(HWND hwnd, LPCTSTR TextBuffer);
VOID pSetupInitPlatformPathOverrideSupport(IN BOOL Init);
VOID pSetupInitSourceListSupport(IN BOOL Init);
DWORD pSetupDecompressOrCopyFile(
    IN  PCTSTR SourceFileName,
...
PTSTR AllocAndReturnDriverSearchList(IN DWORD SearchControl);
pSetupGetSecurityInfo(IN HINF Inf, IN PCTSTR SectionName, OUT PCTSTR *SecDesc );
...
VOID pSetupInitNetConnectionList(IN BOOL Init);
BOOL _SetupGetSourceFileSize(
    IN  HINF        InfHandle,
    IN  PINFCONTEXT InfContext,     OPTIONAL
...
PTSTR pSetupGetDefaultSourcePath(IN  HINF   InfHandle, IN  DWORD  Flags, OUT PDWORD InfSourceMediaType);
VOID InfSourcePathFromFileName(IN  PCTSTR  InfFileName, OUT PTSTR  *SourcePath,  OPTIONAL
    OUT PBOOL   TryPnf);
...
// Private flags are added from MSB down; public flags are added from LSB up.
#define STRTAB_ALREADY_LOWERCASE 0x80000000

// Private string table functions that don't do locking.  These are
// to be used for optimization purposes by components that already have a locking mechanism (e.g., HINF, HDEVINFO).
LONG
...
BOOL pStringTableGetExtraData(IN  PVOID StringTable, IN  LONG  StringId, OUT PVOID ExtraData, IN  UINT  ExtraDataBufferSize);
BOOL pStringTableSetExtraData(IN PVOID StringTable, IN LONG  StringId, IN PVOID ExtraData, IN UINT  ExtraDataSize);
...
PTSTR pStringTableStringFromId(IN PVOID StringTable, IN LONG  StringId);
PVOID pStringTableDuplicate(IN PVOID StringTable);
VOID pStringTableDestroy(IN PVOID StringTable);
VOID pStringTableTrim(IN PVOID StringTable);
PVOID pStringTableInitialize(IN UINT ExtraDataSize   OPTIONAL);
DWORD pStringTableGetDataBlock(IN  PVOID  StringTable, OUT PVOID *StringTableBlock);

// PNF String table routines
PVOID InitializeStringTableFromPNF(IN PPNF_HEADER PnfHeader, IN LCID Locale);
...
VOID InitLogApi (VOID);
VOID TerminateLogApi (VOID);
...
BOOL WaitForPostedThreadMessage(OUT LPMSG MessageData, IN  UINT  Message);

// Macro to make ansi vs unicode string handling a little easier
#ifdef UNICODE
...
DWORD MapFileForRead(IN  HANDLE   FileHandle, OUT PDWORD   FileSize, OUT PHANDLE  MappingHandle, OUT PVOID   *BaseAddress);
BOOL DoMove(IN PCTSTR CurrentName, IN PCTSTR NewName);
BOOL DelayedMove(IN PCTSTR CurrentName, IN PCTSTR NewName       OPTIONAL);
...
PCTSTR GetSystemSourcePath(TRACK_ARG_DECLARE);
PCTSTR GetServicePackSourcePath(TRACK_ARG_DECLARE);
