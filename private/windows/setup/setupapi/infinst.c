...
// Mapping between registry key specs in an inf file and predefined registry handles.
...
STRING_TO_DATA InfRegValToDevRegProp[] = {
    pszDeviceDesc,           (UINT)CM_DRP_DEVICEDESC,
...
STRING_TO_DATA InfRegValToClassRegProp[] = {
    TEXT(""),            (UINT)0,
    TEXT(""),            (UINT)0,
    TEXT(""),            (UINT)0,
    TEXT(""),            (UINT)0,
                    ...
                    if(RootKeySpec = pSetupGetField(&FirstLineContext,1)) {
                        ...
                        if ((SceRegUpdate = GetProcAddress(Sce_Dll,"SceSetupUpdateSecurityKey"))){
                            ...
                            if((error = (DWORD)SceRegUpdate( RootKey, (PWSTR)SubKeyName, 0, (PWSTR)SecDesc))) {
                                ...
                                WriteLogError(((PLOADED_INF) Inf)->LogContext, SETUP_LOG_ERROR, error);
...
DWORD_PTR pSetupProcessUpdateInisLine(IN PVOID Inf, IN PINFCONTEXT InfLineContext, IN PVOID Context)
/*
Routine Description:
...
BOOL pSetupFieldPresentInIniFileLine(IN  PTCHAR  Line, IN PCTSTR Field, OUT PTCHAR *Start, OUT PTCHAR *End)
{
    TCHAR c;
    PTCHAR p,q;
    BOOL b;
    ...
    while(*Line) {
        ...
        // Make sure the field is 0-terminated and see if we have what we're looking for.
        c = *p;
        *p = 0;
        b = (lstrcmpi(Line,Field) == 0);
        *p = c;
        // Skip separators so p points to first char in next field, or to the terminating 0.
...
DWORD_PTR pSetupProcessUpdateIniFieldsLine(IN PVOID Inf, IN PINFCONTEXT InfLineContext, IN PVOID Context)
/*
Routine Description:
...
--*/
{
    ...
    if((RootKeySpec = pSetupGetField(InfLineContext,1)) && (SubKeyName = pSetupGetField(InfLineContext,2))) {
        ValueName = pSetupGetField(InfLineContext,3);

        RootKey = pSetupInfRegSpecToKeyHandle(RootKeySpec, RegModContext->UserRootKey);
        if(RootKey) {
            // Make an information log entry saying we are deleting a key.
            // Note that we must allow for the fact that some parts of the name may be missing.
            if (slot_regop == 0) {
                slot_regop = AllocLogInfoSlot(((PLOADED_INF) Inf)->LogContext,FALSE);
            }
            WriteLogEntry(
            ...
            if(ValueName) {
                if(*ValueName && !(*SubKeyName)) {
                    // If the key being used is HKR with no subkey specified, and if we are doing the DelReg for a hardware key (i.e., DevInst is non-NULL,
                    // then we need to check to see whether the value entry is the name of a device registry property.
                    if ((RegModContext->Flags & INF_PFLAG_CLASSPROP) != 0 && LookUpStringInTable(InfRegValToClassRegProp, ValueName, &CmPropertyCode)) {
                        // This value is a class registry property--we must delete the property by calling a CM API.
                        CM_Set_Class_Registry_Property(RegModContext->ClassGuid, CmPropertyCode, NULL, 0, 0, RegModContext->hMachine);
                    } else if ((RegModContext->Flags & INF_PFLAG_DEVPROP) != 0 && LookUpStringInTable(InfRegValToDevRegProp, ValueName, &CmPropertyCode)) {
                        // This value is a device registry property--we must delete the property by calling a CM API.
                        CM_Set_DevInst_Registry_Property(RegModContext->DevInst, CmPropertyCode, NULL, 0, 0);
                    }
                }

                // Open subkey for delete.
                d = RegOpenKeyEx(RootKey, SubKeyName, 0, KEY_SET_VALUE, &Key);
                if(d == NO_ERROR) {
                    // Do delete.
            ...
            if (d != NO_ERROR) {
                // Log that an error occurred, but I don't think that it matters if it was from open or delete.
                WriteLogError(((PLOADED_INF) Inf)->LogContext, SETUP_LOG_ERROR, d);
            }
        } else {
            d = ERROR_BADKEY;
        }
...
DWORD_PTR pSetupProcessAddRegLine(IN PVOID Inf, IN PINFCONTEXT InfLineContext, IN PVOID Context)
/*
Routine Description:
...
--*/
{
    ...
    if(RootKeySpec = pSetupGetField(InfLineContext,1)) {
        ...
        if(SubKeyName && *SubKeyName) {
            ...
            // If we're running in non-interactive mode, we cannot allow RunOnce processing to happen in that context (typically, in TCB)
            // because we have no control over what crap gets registered for RunOnce.  Also, we can't kick off RunOnce in the context of a
            // logged-on user, because if it's a non-administrator, some of the operations may fail, and be lost forever (SWENUM is a prime example of this).

            // Therefore, when we're in non-interactive mode, we "swallow" any AddReg directives for RunOnce entries that use rundll32, and
            // squirrel them away in a global list.  The caller (i.e., umpnpmgr) can retrieve this list and do the job of rundll32
            // manually for these entries.  If we encounter any other kinds of runonce entries, we bail.
                            ...
                            *(q++) = TEXT('\0'); // separate 2nd part of string
                            ...
                            if(p == MyGetFileTitle(p)) {
                                ...
                                if(!DllPathSize || (DllPathSize >= SIZECHARS(szBuffer))) {
                                    // If we start failing because MAX_PATH isn't big enough anymore, we wanna know about it!
                            ...
                            // OK, now that we have the full path of the DLL, verify its digital signature.
                            d = VerifyFile(((PLOADED_INF)Inf)->LogContext, NULL, NULL, 0, MyGetFileTitle(DllFullPath), DllFullPath, NULL, NULL, FALSE, NULL, NULL, NULL);
                            if(d != NO_ERROR) {
                                MyFree(DllFullPath);
                                goto clean0;
                            }
                            ...
                            // name (in ANSI), as well as the argument string to be passed to the DLL.
                            p = _tcschr(q, TEXT(' '));
                            if(p) {
                                *(p++) = TEXT('\0');
                            ...
                            // If we get to this point, we have the full DLL path, the DLL entrypoint (always in ANSI, since
                            // that's what GetProcAddress wants), and the DLL argument string.  Before we create a new node
                            // to add to our global list, scan the list to see if the node is already in there (if so, we don't need to add it again).
                            ...
                            if(RunOnceListHead) {
                                ...
                                do {
                                    ...
                                    if(!lstrcmpi(DllFullPath, CurNode->DllFullPath) && !lstrcmpiA(DllEntryPointName, CurNode->DllEntryPointName) && !lstrcmpi(DllParams, CurNode->DllParams)) {
            ...
            // we need to set the Key equal to the RootKey (So we won't think it's a newly-opened key and try to close it later.
            Key = RootKey;
        }
...
DWORD_PTR pSetupProcessBitRegLine(IN PVOID Inf, IN PINFCONTEXT InfLineContext, IN PVOID Context)
/*
Routine Description:
...
--*/
{
    ...
    if(RootKeySpec = pSetupGetField(InfLineContext,1)) {
        ...
        SubKeyName = pSetupGetField(InfLineContext,2);// SubKeyName is optional.
        ValueName = pSetupGetField(InfLineContext,3);// ValueName is optional. Either NULL or "" are acceptable to pass to RegSetValueEx.
        SetupGetIntField(InfLineContext,4,&Flags);// get the flags
        ...
        SetupGetIntField(InfLineContext,6,&ByteNumber);// get the byte number to modify
        ...
        if(SubKeyName && *SubKeyName) {
            ...
            if(d == NO_ERROR) {
                ...
                // Work around hacked nonsense on Win95 where the unnamed value behaves differently.
                if( ((ValueName == NULL) || (*ValueName == 0)) && (OSVersionInfo.dwPlatformId != VER_PLATFORM_WIN32_NT)) {
                    d = 0;
            ...
            // If the key being used is HKR with no subkey specified, and if we are doing the BitReg for a hardware key or for a ClassInstall32
            // entry, then we need to check to see whether the value entry we have is the name of a device or class registry property.
            if((RegModContext->Flags & INF_PFLAG_CLASSPROP) && ValueName && *ValueName && LookUpStringInTable(InfRegValToClassRegProp, ValueName, &CmPropertyCode)) {
                // Retrieve the existing class property.

                // BUGBUG (lonnym)--There's presently a bug in CM_Get_Class_Registry_Property that succeeds a call with a
                // null buffer of zero-length (the proper behavior is to fail
                // the call with CR_BUFFER_SMALL).  We'll handle both cases here, in case that bug doesn't get fixed.
                cb = 0;
                cr = CM_Get_Class_Registry_Property(RegModContext->ClassGuid, CmPropertyCode, &ValueType, NULL, &cb, 0, RegModContext->hMachine);
                if((cr == CR_SUCCESS) || (cr == CR_BUFFER_SMALL)) {
                    ...
                    if(cb) {
                        ...
                        if(d == NO_ERROR) {
                            cr = CM_Get_Class_Registry_Property(RegModContext->ClassGuid, CmPropertyCode, &ValueType, Data, &cb, 0, RegModContext->hMachine);
                            if(cr == CR_SUCCESS) {
                                DevOrClassProp = TRUE;
                            } else {
                    ...
                    // We can't access the property (probably because it doesn't exist.
                    // We return ERROR_INVALID_DATA for consistency with the return code used by SetupDiGetDeviceRegistryProperty.
                    d = ERROR_INVALID_DATA;
                }
            } else if((RegModContext->Flags & INF_PFLAG_DEVPROP) && ValueName && *ValueName && (b = LookUpStringInTable(InfRegValToDevRegProp, ValueName, &CmPropertyCode))) {
                // Retrieve the existing device property.
                cb = 0;
                cr = CM_Get_DevInst_Registry_Property(RegModContext->DevInst, CmPropertyCode, &ValueType, NULL, &cb, 0);
                if(cr == CR_BUFFER_SMALL) {
                    ...
                    if(d == NO_ERROR) {
                        cr = CM_Get_DevInst_Registry_Property(RegModContext->DevInst, CmPropertyCode, &ValueType, Data, &cb, 0);
                        if(cr == CR_SUCCESS) {
                            DevOrClassProp = TRUE;
                        } else {
                            d = MapCrToSpError(cr, ERROR_INVALID_DATA);
            ...
            // property, we need to set the Key equal to the RootKey (So we won't think it's a newly-opened key and try to close it later.
            Key = RootKey;
        }
            ...
            if (d == NO_ERROR) {
                ...
                if(DevOrClassProp) {
                    if(RegModContext->Flags & INF_PFLAG_CLASSPROP) {
                        cr = CM_Set_Class_Registry_Property(RegModContext->ClassGuid, CmPropertyCode, Data, cb, 0, RegModContext->hMachine);
                        if(cr != CR_SUCCESS) {
                            d = MapCrToSpError(cr, ERROR_INVALID_DATA);
                        }
                    } else {
                        MYASSERT(RegModContext->Flags & INF_PFLAG_DEVPROP);
                        cr = CM_Set_DevInst_Registry_Property(RegModContext->DevInst, CmPropertyCode, Data, cb, 0);
                        if(cr != CR_SUCCESS) {
                            d = MapCrToSpError(cr, ERROR_INVALID_DATA);
                        }
                    }
...
DWORD_PTR pSetupProcessIni2RegLine(IN PVOID Inf, IN PINFCONTEXT InfLineContext, IN PVOID Context)
{
    ...
    // Note that we must allow for the fact that the subkey name may be missing.
    if (slot_regop == 0) {
        slot_regop = AllocLogInfoSlot(((PLOADED_INF) Inf)->LogContext,FALSE);
    }
    WriteLogEntry(
    ...
    if(SubkeyPath) {
        d = RegCreateKeyEx(RootKey, SubkeyPath, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hKey, &Disposition);
    } else {
        hKey = RootKey;
    }
    ...
    for(Line=Buffer; (d==NO_ERROR) && *Line; Line+=lstrlen(Line)+1) {
        ...
        WriteLogEntry(((PLOADED_INF) Inf)->LogContext, slot_subop, MSG_LOG_SETTING_REG_VALUE, NULL, key, value);

        // Now key points to the value name and value to the value.
        d = RegSetValueEx(hKey, key, 0, REG_SZ, (CONST BYTE *)value, (lstrlen(value)+1)*sizeof(TCHAR));
    }

    if (d != NO_ERROR) {
        // Log that an error occurred, but I don't think that it matters if it was from create or set.
        WriteLogError(((PLOADED_INF) Inf)->LogContext, SETUP_LOG_ERROR, d);
    }
...
DWORD pSetupInstallUpdateIniFiles(IN HINF   Inf, IN PCTSTR SectionName)
/*
Routine Description:
...
--*/
{
    ...
    if(SetupFindFirstLine(Inf,Section,NULL,&InfLine)) {
        ...
        if (OleControlData.hWx86Dll != NULL) {
            OleControlData.pWx86LoadX86Dll = (WX86LOADX86DLL_ROUTINE) GetProcAddress(OleControlData.hWx86Dll, "Wx86LoadX86Dll");
            OleControlData.pWx86EmulateX86 = ( WX86EMULATEX86 ) GetProcAddress(OleControlData.hWx86Dll, "Wx86EmulateX86");
            if ((OleControlData.pWx86LoadX86Dll == NULL) || (OleControlData.pWx86EmulateX86 == NULL)) {
            ...
            for(Field=1; b && (Field<=FieldCount); Field++) {
                if(SectionSpec = pSetupGetField(&LineContext,Field)) {
                    ...
                    if((i == 0) && (*SectionSpec == TEXT('@'))) {
                        b = SetupQueueDefaultCopy(FileQueue, LayoutInf, SourceRootPath, SectionSpec + 1, SectionSpec + 1, CopyStyle);
                        if (!b) {
                            rc = GetLastError();
    ...
    )
{
    ...
    // If the caller wants us to run a specific section, then they'd better have told us what kind of section it is (i.e., one and only one of the install action types must be specified).

    // Presently, only LogConfig sections are allowed, since the other flags flags encompass multiple actions (e.g., AddReg _and_ DelReg).
    if((Flags & SPINST_SINGLESECTION) && ((Flags & SPINST_ALL) != SPINST_LOGCONFIG)) {
        d = ERROR_INVALID_FLAGS;
        goto clean1;
    }

    // You can (optionally) specify SPINST_LOGCONFIG_IS_FORCED or SPINST_LOGCONFIGS_ARE_OVERRIDES, but not both.
    if((Flags & (SPINST_LOGCONFIG_IS_FORCED | SPINST_LOGCONFIGS_ARE_OVERRIDES)) == (SPINST_LOGCONFIG_IS_FORCED | SPINST_LOGCONFIGS_ARE_OVERRIDES)) {
        d = ERROR_INVALID_FLAGS;
        goto clean1;
    }

    // We only want to acquire the HDEVINFO lock if we're supposed to do some install actions against a device instance.
    if((Flags & (SPINST_REGISTRY | SPINST_BITREG | SPINST_INI2REG | SPINST_LOGCONFIG)) && DeviceInfoSet && (DeviceInfoSet != INVALID_HANDLE_VALUE) && DeviceInfoData) {
    ...
    try {
        ...
        if((Flags & (SPINST_REGISTRY | SPINST_BITREG | SPINST_INI2REG)) && DevInfoElem) {
            // If the caller supplied a device information set and element, then this is a device installation, and the registry modifications should be made to the device instance's hardware registry key.
            if((RelativeKeyRoot = SetupDiCreateDevRegKey(DeviceInfoSet, DeviceInfoData, DICS_FLAG_GLOBAL, 0, DIREG_DEV, NULL, NULL)) == INVALID_HANDLE_VALUE) {
                d = GetLastError();
                goto RegModsDone;
            }
        ...
        if((Flags & SPINST_LOGCONFIG) && DevInfoElem) {
            d = pSetupInstallLogConfig(InfHandle, SectionName, DevInfoElem->DevInst, Flags, pDeviceInfoSet->hMachine);
            if(d != NO_ERROR) {
                goto RegModsDone;
            }
        }
        ...
        if(Flags & (SPINST_REGISTRY | SPINST_BITREG)) {
            ...
            // We check for the INF_PFLAG_HKR flag in case the caller supplied us with a context that included a UserRootKey they wanted us to use (i.e., instead of the RelativeKeyRoot)...
            if(!(RegContext->Flags & INF_PFLAG_HKR)) {
                RegContext->UserRootKey = RelativeKeyRoot;
            }

            if(Flags & SPINST_REGISTRY) {
                d = pSetupInstallRegistry(InfHandle, SectionName, RegContext);
            }

            if((d == NO_ERROR) && (Flags & SPINST_BITREG)) {
                d = pSetupInstallBitReg(InfHandle, SectionName, RegContext);
            }
...
RegModsDone:
        ...
        d = ERROR_INVALID_PARAMETER;
        // Access the following variable, so that the compiler will respect statement ordering w.r.t. its assignment.
    ...
    if(d == NO_ERROR) {
        if(Flags & SPINST_FILES) {
            d = pSetupInstallFiles(InfHandle, NULL, SectionName, SourceRootPath, MsgHandler, Context, CopyFlags, Owner, NULL, IsMsgHandlerNativeCharWidth);
        }
    }
    ...
    )
{
    ...
    for(p=Section->SectionData; *p; p+=lstrlen(p)+1) {
        ...
        // If we need to match by right hand side, attempt that here.
        if(RightHandSide) {
    ...
    )
{
    ...
    // Locate the line.
    LineInBuffer = pSetupFindLineInSection(Section, KeyName, MatchRHS ? RightHandSide : NULL);
    if(LineInBuffer) {
        ...
        if((BufferUsedDelta > 0) && ((Section->BufferSize - Section->BufferUsed) < BufferUsedDelta)) {
            p = MyRealloc(Section->SectionData, (Section->BufferUsed + BufferUsedDelta)*sizeof(TCHAR));
            if(p) {
        ...
        MoveMemory(
            // Leave exactly enough space for the new line. Since the new line will start at the same place the existing line is at now, the
...
--*/
{
    ...
    // Make sure that a device information set and element were specified if the SPSVCINST_ASSOCSERVICE flag is set.
    ...
    try {
        ...
        if(SetupFindFirstLine(InfHandle, SectionName, NULL, &InfContext)) {
            ...
            // one AddService entry in this service install section.  If not, then clear this flag.
            if((Flags & SPSVCINST_ASSOCSERVICE) && SetupFindFirstLine(InfHandle, SectionName, pszAddService, &InfContext) && SetupFindNextMatchLine(&InfContext, pszAddService, &InfContext))
            {
                Flags &= ~SPSVCINST_ASSOCSERVICE;
            }

            d = InstallNtService(DevInfoElem, InfHandle, NULL, SectionName, NULL, Flags | SPSVCINST_NO_DEVINST_CHECK, &DontCare);
            ...
            try {
                if (InfHandle != NULL && InfHandle != INVALID_HANDLE_VALUE && LockInf((PLOADED_INF)InfHandle)) {
                    WriteLogEntry(((PLOADED_INF)InfHandle)->LogContext, SETUP_LOG_ERROR, MSG_LOG_NOSECTION_MIN, NULL, SectionName);
                    UnlockInf((PLOADED_INF)InfHandle);
                }
            } except(EXCEPTION_EXECUTE_HANDLER) {
            }
...
--*/
{
    ...
    try {
        ...
        *szInfFile++ = TEXT('\0');
        ...
        SetupDiGetActualSectionToInstall(InfHandle, szSectionName, ActualSection, SIZECHARS(ActualSection), &ActualSectionLength, NULL);
        ...
        if(SetupFindFirstLine(InfHandle, ActualSection, pszReboot, &InfContext)) {
            if(iHow == HOW_SILENT_REBOOT) {
                ...
                if(GlobalSetupFlags & PSPGF_NONINTERACTIVE) {
                    ...
                    // In this case, we assert (we should never be running INFs non-interactively that require a reboot unless we've
                    // specified one of the silent reboot flags).  We then
                    // ignore the reboot flag, since the caller obviously doesn't want it/isn't prepared to deal with it.
                    MYASSERT(0);
                } else {
                    // In the interactive case, we want to force the (non-
                    // silent) reboot prompt (i.e., the INF flag overrides the caller).
                    iHow = HOW_ALWAYS_PROMPT_REBOOT;
                }
            }
        }

        SetupOpenAppendInfFile(NULL, InfHandle, NULL);// Assume there is only one layout file and load it.
        AppendLoadIncludedInfs(InfHandle, InfSearchPath, ActualSection, TRUE);// Append-load any included INFs specified in an "include=" line in our install section.
        ...
        InheritLogContext(((PLOADED_INF) InfHandle)->LogContext, &((PSP_FILE_QUEUE) FileQueue)->LogContext);

        NoProgressUI = (GuiSetupInProgress || (GlobalSetupFlags & PSPGF_NONINTERACTIVE));

        QueueContext = SetupInitDefaultQueueCallbackEx(Window, (NoProgressUI ? INVALID_HANDLE_VALUE : NULL), 0, 0, 0);
        if(!QueueContext) {
            goto c2;
        }
        ...
        WriteLogEntry(((PSP_FILE_QUEUE) FileQueue)->LogContext, slot_section, MSG_LOG_INSTALLING_SECTION_FROM, NULL, ActualSection, szInfFile);
...
--*/
{
    ...
    // If we're batching up RunOnce operations for server-side processing, then return immediately without doing a thing.
    if(GlobalSetupFlags & PSPGF_NONINTERACTIVE) {
        return NO_ERROR;
    }
    ...
    if(!(Flags & INSTALLSTOP_NO_GRPCONV)) {
        ...
        if(RegOpenKeyEx(hKey,
            ...
            Error = (DWORD)RegSetValueEx(hKey, REGSTR_VAL_WRAPPER, 0, REG_SZ, (PBYTE)pszRunOnceExe, sizeof(pszRunOnceExe));
        } else {
            // We're OK so far.
            Error = NO_ERROR;
        }
        ...
        if(Flags & INSTALLSTOP_NO_UI) {
            l = RegSetValueEx(hKey, TEXT("GrpConv"), 0, REG_SZ, (PBYTE)pszGrpConvNoUi, sizeof(pszGrpConvNoUi));
        } else {
            l = RegSetValueEx(hKey, TEXT("GrpConv"), 0, REG_SZ, (PBYTE)pszGrpConv, sizeof(pszGrpConv));
        }
    }
    ...
    if (DoRunOnce && (GlobalSetupFlags & PSPGF_NO_RUNONCE)==0) {
        ...
        l = RegQueryInfoKey(hKey,NULL,NULL,NULL, NULL,NULL,NULL, &nValues, NULL, NULL, NULL, NULL);
        if ( l != ERROR_SUCCESS ) {
            nValues = 5;
        } else {
            nValues += 5;
        ...
        if (lc) {
            ...
            WriteLogEntry(lc, SETUP_LOG_INFO, MSG_LOG_RUNONCE_CALL, NULL, nValues);
        }
