    ...
    )
{
    ...
    case WM_PAINT:
        ...
        if (g_LogOpen) {
            DrawText (hdc, TEXT("Log is open!"), -1, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
        } else {
            DrawText (hdc, TEXT("Log is not open!"), -1, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
        }
...
static BOOL InitMainWindow (VOID)
{
    ...
    hWndMain = CreateWindow (pszAppName, TEXT("The Hello Program"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, (HINSTANCE) hModule, NULL);
    if (!hWndMain) {
        return FALSE;
    }
