...
Arguments:
    ...
    MappingHandle - receives the win32 handle for the file mapping object.  This object will be for read access.
    BaseAddress - receives the address where the file is mapped.
Return Value:

    NO_ERROR if the file was opened and mapped successfully.
        The caller must unmap the file with UnmapAndCloseFile when access to the file is no longer desired.
...
Arguments:
    FileHandle - supplies the handle of the opened file to be mapped. This handle must have been opened with at least read access.
    FileSize - receives the size in bytes of the file.
    MappingHandle - receives the win32 handle for the file mapping object.  This object will be for read access.
    BaseAddress - receives the address where the file is mapped.
Return Value:
    NO_ERROR if the file was mapped successfully.  The caller must unmap the file with UnmapAndCloseFile when access to the file is no longer desired.
...
BOOL UnmapAndCloseFile(IN HANDLE FileHandle, IN HANDLE MappingHandle, IN PVOID  BaseAddress)
/*++
Routine Description:
...
#ifdef UNICODE
    If the file is ascii, it is assumed to be ANSI format and is converted to Unicode.
#else
    If the file is unicode, it will be converted to ascii using the ANSI codepage.

    NOTE:  On Windows 95, the IsTextUnicode API is not implemented, thus
    the unicode file must have a byte order mark (BOM) in order for us to recognize it under Win95.
#endif
...
--*/
{
    ...
    if(rc != NO_ERROR) {
        ...
        // Determine whether the file is unicode.  Guard with try/except in case we get an inpage error.
        try {
...
#endif
                        NativeCharCount = MultiByteToWideChar(SysCodePage, MB_PRECOMPOSED, TextStartAddress, FileSize, Buffer, FileSize);
#else
...
BOOL DestroyTextFileReadBuffer(IN PTEXTFILE_READ_BUFFER ReadBuffer)
/*++
Routine Description:
    Destroy a textfile read buffer created by ReadAsciiOrUnicodeTextFile.
Arguments:
    ReadBuffer - supplies the address of a TEXTFILE_READ_BUFFER structure for the buffer to be destroyed.
Return Value:
    BOOLean value indicating success or failure.
--*/
{
...
Routine Description:
    ...
    For the language we look at the translation table in the version resources and assume
    ...
    the function fails. The function does not fail if we are able to retrieve the version but not the language.
        ...
        if(VersionBlock) {
            ...
            if(GetFileVersionInfo((PTSTR)FileName,0,d*sizeof(TCHAR),VersionBlock)) {
                ...
                if(VerQueryValue(VersionBlock,TEXT("\\"),&FixedVersionInfo,&DataLength)) {
                    ...
                    *Version = (((DWORDLONG)FixedVersionInfo->dwFileVersionMS) << 32) + FixedVersionInfo->dwFileVersionLS;
...
Routine Description:
    ...
    Errormode is set (and then restored) so the user will not see any pop-ups.
Arguments:
...
Arguments:
    ...
    CreateTime - if specified and the underlying filesystem supports it, receives the creation time of the file.
    AccessTime - if specified and the underlying filesystem supports it, receives the last access time of the file.
    WriteTime - if specified, receives the last write time of the file.
Return Value:
    If successful, returns NO_ERROR, otherwise returns the Win32 error indicating the cause of failure.
--*/
{
    HANDLE h;
    DWORD d;
...
Arguments:
    ...
    SecurityDescriptor - If the function is successful, receives pointer to buffer containing security information for the file.
        The pointer may be NULL, indicating that there is no security information
        associated with the file or that the underlying filesystem does not support file security.
Return Value:
    Win32 error code indicating outcome. If NO_ERROR check the value returned in SecurityDescriptor.
...
DWORD TakeOwnershipOfFile(IN PCTSTR Filename)
/*++
Routine Description:
    Sets the owner of a given file to the default owner specified in the current process token.
Arguments:
...
Arguments:
    ...
    NewBaseName - optionally specifies the new base name to use when the catalog file is copied into the catalog store.
        If not specified, the basename will be generated by CryptCATAdminAddCatalog.
...
Return Value:
    ...
    If failure, the return value is a Win32 error code indicating the cause of the failure.
--*/
{
    DWORD Err = NO_ERROR;
    HCATADMIN hCatAdmin;
...
#else
                        WideCharToMultiByte(CP_ACP, 0, CatalogInfo.wszCatalogFile, -1, NewCatalogFullPath, MAX_PATH, NULL, NULL);
#endif
                    }
                }
...
#else
                            WideCharToMultiByte(CP_ACP, 0, CatInfo.wszCatalogFile, -1, AnsiBuffer, sizeof(AnsiBuffer), NULL, NULL);
                                    ...
                                    if(FoundMatchingImage) {
                                        if(OpenAndMapFileForRead(CatalogFullPath,
                                            ...
                                            try {
                                                ...
                                                FoundMatchingImage = !memcmp(CatalogBaseAddress, CurCatBaseAddress, CatalogImageSize);
                                            } except(EXCEPTION_EXECUTE_HANDLER) {
                                                FoundMatchingImage = FALSE;
                                            }

                                            UnmapAndCloseFile(CurCatFileHandle, CurCatMappingHandle, CurCatBaseAddress);
                                        } else {
                                            FoundMatchingImage = FALSE;
                                        }
                                    }
...
Arguments:
    ...
    LogContext - supplies the log context to be used in logging an error if we're unable to open an SFC handle.
    phSfp - optionally, supplies the address of a handle variable that will be filled in with a handle to the SFC server.
        This will only be supplied when the routine returns TRUE (i.e., the file is SFP-protected).
Return Value:
