...
//  Return:     0
{
    ...
    while (TRUE) {
        ...
        // Setup the timer
        if (dwTimeout >= 60000) {
            DebugMsg((DM_VERBOSE, TEXT("GPOThread:  Next refresh will happen in %d minutes"), ((dwTimeout / 1000) / 60)));
        } else {
...
//              FALSE if an error occurs
{
    ...
    if (!MakeRegKeySecure((lpGPOInfo->dwFlags & GP_MACHINE) ? NULL : lpGPOInfo->hToken, lpGPOInfo->hKeyRoot, TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Group Policy"))) {
    ...
    if (lpGPOInfo->dwFlags & GP_APPLY_DS_POLICY) {
        ...
        if (!lpDomain) {
            ...
            DebugMsg((DM_WARNING, TEXT("ProcessGPOs: MyGetDomainName failed with %d."), dwResult));
            goto Exit;
        }
    ...
    bRetVal = TRUE; // Set the return value
...
//              FALSE if an error occurs
{
    ...
    while ( lpGPO ) {
        ...
        lpGPO = lpGPO->pNext;// Advance to next GPO
    } // while lpgpo
...
DWORD GetCurTime()
//  Purpose:    Returns current time in minutes, or 0 if there is a failure
{
    DWORD dwCurTime = 0;
    ...
    *ppDeletedGPOList = NULL;
    ...
    if (bTemp) {
        ...
        if (CompareGPOLists (lpOldGPOList, lpGPOInfo->lpGPOList) && !bMembershipChanged && (!(lpGPOInfo->bSidChanged))) {
            ...
            // Exception: Even if nothing has changed but the user's sid changes we need to call the extensions so that they can update their settings
            DWORD dwStatus = 0;
            DWORD dwPrevTime = 0;
                        ...
                        if (RegQueryValueEx (hKeyExt, TEXT("DllName"), NULL, &dwType, (LPBYTE) szDllName, &dwSize) == ERROR_SUCCESS) {
                            ...
                            if ( bFuncFound ) {
                                ...
                                if (lpExt) {
                                    ...
                                    DebugMsg((DM_WARNING, TEXT("ReadGPExtensions: Failed to allocate memory with %d"), GetLastError()));
                                }
                            } else {       // if bFuncFound
...
BOOL ArchiveRegistryValue(HANDLE hFile, LPWSTR lpKeyName, LPWSTR lpValueName, DWORD dwType, DWORD dwDataLength, LPBYTE lpData)
...
//              FALSE if an error occurs
{
    ...
    while (TRUE)
    {
        ...
        if (hArchive) {
            if (!ArchiveRegistryValue(hArchive, lpKeyName, lpValueName, dwType, dwDataLength, lpData)) {
...
//              FALSE if not
{
    ...
    DebugMsg((DM_VERBOSE, TEXT("EnterCriticalPolicySection: %s critical section has been claimed.  Handle = 0x%x"), (bMachine ? TEXT("Machine") : TEXT("User")), hSection));
    return hSection;
}
...
//              FALSE if an error occurs.
{
    ...
    *pbAccessGranted = FALSE;
    ...
    if (!pSize) {
        DebugMsg((DM_WARNING, TEXT("CheckGPOAccess:  ldap_get_values_len failed with 0x%x"), pld->ld_errno));
        goto Exit;
    }
    ...
    if (!pSD) {
        DebugMsg((DM_WARNING, TEXT("CheckGPOAccess:  Failed to allocate memory for SD with  %d"), GetLastError()));
        goto Exit;
    }
    ...
    // Now we use AccessCheckByType to determine if the user / machine should have this GPO applied to them
...
//              FALSE if an error occurs
{
    ...
    *lpGPOList = NULL;
    ...
    // If we don't have a DS server or user / machine name, we can skip the DS stuff and only check for a local GPO
    ...
    if (bMachineTokenOk && (dwFlags & GPO_LIST_FLAG_MACHINE)) {
        ...
        if (!hToken) {
            DebugMsg((DM_WARNING, TEXT("GetGPOInfo:  Failed to get the machine token with  %d"), GetLastError()));
            goto Exit;
        }
    } else {
...
//              FALSE if an error occurs
{
    ...
    pNetAPI32 = LoadNetAPI32();
    if (!pNetAPI32) {
        DebugMsg((DM_WARNING, TEXT("GetGPOList:  Failed to load netapi32 with %d."), GetLastError()));
        goto Exit;
    }

    // If an hToken was offered, then we need to get the name and domain DN name
    if (hToken) {
        ...
        if (*lpTemp != TEXT('\\')) {
            DebugMsg((DM_WARNING, TEXT("GetGPOList:  Failed to find slash in NT4 style name:  <%s>"), lpDomain));
            goto Exit;
        }
        ...
        *lpTemp = TEXT('\0');
        ...
        if (!(pDCI->Flags & DS_DS_FLAG)) {
            pNetAPI32->pfnNetApiBufferFree(pDCI);
            DebugMsg((DM_WARNING, TEXT("GetGPOList:  The domain <%s> does not have a DS"), lpDomain));
            goto Exit;
        }
...
//  Returns:    Win32 error code
{
    ...
    if ( ReadGPOList( szExtension, lpGPOInfo->hKeyRoot, HKEY_LOCAL_MACHINE, lpwszSidUser, TRUE, &pGPOList ) ) {
        ...
        DebugMsg((DM_WARNING, TEXT("Extension %s asynchronous completion, failed to read shadow GPOList"), szExtension));
    }
...
//              0 if an error occurs
{
    ...
    if ( dwResult == ERROR_SUCCESS )
    {
        ...
        // For machine policies specifically ask for Kerberos as the only authentication mechanism.
        // Otherwise if Kerberos were to fail for some reason, then NTLM is used
        // and localsystem context has no real credentials, which means that we won't get any GPOs back.
        secIdentity.Version = SEC_WINNT_AUTH_IDENTITY_VERSION;
        secIdentity.Length = sizeof(SEC_WINNT_AUTH_IDENTITY_EXW);
        secIdentity.User = 0;
