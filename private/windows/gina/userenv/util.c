...
BOOL Delnode (LPTSTR lpDir)
{
    ...
    if (GetCurrentDirectory(MAX_PATH, szCurWorkingDir)) {
        ...
        if (!RemoveDirectory (lpDir)) {
            ...
            if ((dwError != ERROR_FILE_NOT_FOUND) && (dwError != ERROR_PATH_NOT_FOUND)) {
                DebugMsg((DM_VERBOSE, TEXT("Delnode: Failed to delete directory <%s>.  Error = %d"), lpDir, dwError));
            }
#endif
...
LONG RegRenameKey(HKEY hKeyRoot, LPTSTR lpSrcKey, LPTSTR lpDestKey)
{
    ...
    lResult = RegQueryInfoKey(hSrcKey, NULL, NULL, NULL, NULL, NULL, NULL, &dwValues, &dwMaxValueNameLen, &dwMaxValueLen, &dwSDSize, NULL);
    if (lResult != ERROR_SUCCESS) {
...
BOOL MakeRegKeySecure(HANDLE hToken, HKEY hKeyRoot, LPTSTR lpKeyName)
{
    ...
    Error = RegCreateKeyEx(hKeyRoot, lpKeyName, 0, NULL, REG_OPTION_NON_VOLATILE, WRITE_DAC | KEY_ENUMERATE_SUB_KEYS | READ_CONTROL, NULL, &hSubKey, &dwDisp);
...
LPTSTR MyGetDomainName (VOID)
{
    ...
    if (!lpDomain) {
        DebugMsg((DM_WARNING, TEXT("MyGetDomainName:  MyGetUserName failed for NT4 style name with %d"), GetLastError()));
        return NULL;
    }
...
LPTSTR MyGetUserName (EXTENDED_NAME_FORMAT  NameFormat)
{
    ...
    if (!lpUserName) {
        dwError = GetLastError();
        DebugMsg((DM_WARNING, TEXT("MyGetUserName:  Failed to allocate memory with %d"), dwError));
        goto Exit;
    }
    ...
    while (TRUE) {
        ...
        if (pSecur32->pfnGetUserNameEx (NameFormat, lpUserName, &ulUserNameSize)) {
            ...
            if ((dwError == ERROR_INSUFFICIENT_BUFFER) || (dwError == ERROR_MORE_DATA)) {
                ...
                if (!lpTemp) {
                    dwError = GetLastError();
                    DebugMsg((DM_WARNING, TEXT("MyGetUserName:  Failed to realloc memory with %d"), dwError));
...
void StringToGuid( TCHAR * szValue, GUID * pGuid )
{
    ...
    // Since szValue may be used again, no permanent modification to it is be made.
...
HANDLE GetMachineToken (void)
{
    ...
    if (!pvBuffer) {
        DebugMsg((DM_WARNING, TEXT("GetMachineToken:  LocalAlloc failed with %d"), GetLastError()));
...
BOOL GetMachineRole (LPINT piRole)
{
    ...
    if (!pNetAPI32) {
        DebugMsg((DM_WARNING, TEXT("GetMachineRole:  Failed to load netapi32 with %d."), GetLastError()));
    ...
    if (dwResult != ERROR_SUCCESS) {
        DebugMsg((DM_WARNING, TEXT("GetMachineRole:  DsRoleGetPrimaryDomainInformation failed with %d."), dwResult));
        return FALSE;
    }
        ...
        if (pBasic->Flags & DSROLE_PRIMARY_DOMAIN_GUID_PRESENT) {
            if (!IsNullGUID(&pBasic->DomainGuid)) {
                *piRole = 2;   // machine is a member of a domain with DS support
                if ((pBasic->MachineRole == DsRole_RoleBackupDomainController) || (pBasic->MachineRole == DsRole_RolePrimaryDomainController)) {
