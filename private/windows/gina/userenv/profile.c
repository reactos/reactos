...
BOOL IsCentralProfileReachable(LPPROFILE lpProfile, BOOL *bCreateCentralProfile, BOOL *bMandatory);
BOOL IssueDefaultProfile (LPPROFILE lpProfile, LPTSTR lpDefaultProfile, LPTSTR lpLocalProfile, LPTSTR lpSidString, BOOL bMandatory);
...
BOOL GetExclusionList (LPPROFILE lpProfile)
{
    ...
    GetPrivateProfileString (PROFILE_GENERAL_SECTION, PROFILE_EXCLUSION_LIST, TEXT(""), szExcludeListServer, ARRAYSIZE(szExcludeListServer), szNTUserIni);
    ...
    GetPrivateProfileString (PROFILE_GENERAL_SECTION, PROFILE_EXCLUSION_LIST, TEXT(""), szExcludeListLocal, ARRAYSIZE(szExcludeListLocal), szNTUserIni);
                               ...
                               BOOL *bMandatory)
{
    ...
    *bCreateCentralProfile = FALSE;
    ...
    dwError = GetLastError();
    DebugMsg((DM_VERBOSE, TEXT("IsCentralProfileReachable:  Profile is not reachable, error = %d"), dwError));

    // If we received an error other than file not
    // found, bail now because we won't be able to access this location.
    if (dwError != ERROR_FILE_NOT_FOUND) {
        DebugMsg((DM_WARNING, TEXT("IsCentralProfileReachable:  Profile path <%s> is not reachable, error = %d"), szProfile, dwError));
        goto Exit;
    }
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (error == ERROR_ACCESS_DENIED) {
            ...
            if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_READ, &hSubKey) == ERROR_SUCCESS) {
                ...
                RegQueryValueEx(hSubKey, PROFILE_UNLOAD_TIMEOUT, NULL, &dwType, (LPBYTE) &dwMaxWait, &dwSize);

                dwSize = sizeof(bDisableProfileUnloadMsg);
                RegQueryValueEx(hSubKey, DISABLE_PROFILE_UNLOAD_MSG, NULL, &dwType, (LPBYTE) &bDisableProfileUnloadMsg, &dwSize);
            ...
            if (dwMaxWait) {
                DebugMsg((DM_WARNING, TEXT("MyRegUnLoadKey: Hive unload for %s failed due to open registry key.  Windows will try unloading the registry hive once a second for the next %d seconds (max)."), lpSubKey, dwMaxWait));
...
BOOL CreateLocalProfileKey (LPPROFILE lpProfile, PHKEY phKey, BOOL *bKeyExists)
{
    ...
    if (SidString != NULL) {
        ...
        RegErr = RegCreateKeyEx(HKEY_LOCAL_MACHINE, LocalProfileKey, 0, 0, 0, KEY_READ | KEY_WRITE, NULL, phKey, &Disposition);
...
BOOL GetExistingLocalProfileImage(LPPROFILE lpProfile)
{
    ...
    if (bKeyExists) {
        ...
        if (err == ERROR_SUCCESS && cbExpProfileImage) {
            ...
            fh = CreateFile(lpExpProfileImage, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (ComputeLocalProfileName(lpProfile, lpBaseName, lpProfileImage, MAX_PATH, lpExpProfileImage, MAX_PATH, NULL, FALSE)) {
        ...
        if (err == ERROR_SUCCESS) {
            ...
            if (UserSid != NULL) {
                ...
                err = RegSetValueEx(hKey, TEXT("Sid"), 0, REG_BINARY, UserSid, RtlLengthSid(UserSid));
                if (err != ERROR_SUCCESS) {
    ...
    if (bBackup) {
        ...
        if (lResult == ERROR_SUCCESS) {
            ...
            if (lResult == ERROR_SUCCESS) {
                ...
                lResult = RegSetValueEx (hKey, PROFILE_STATE, 0, REG_DWORD, (LPBYTE) &dwInternalFlags, sizeof(dwInternalFlags));
            }
            else {
            ...
            if (lResult == ERROR_SUCCESS) {
                ...
                lResult = RegSetValueEx (hKey, PROFILE_IMAGE_VALUE_NAME, 0, dwType, (LPBYTE) szProfilePath, sizeof(TCHAR)*(lstrlen(szProfilePath)+1));
            }
            else {
...
BOOL UpgradeProfile (LPPROFILE lpProfile)
{
    ...
    lResult = RegQueryValueEx (hKey, PROFILE_BUILD_NUMBER, NULL, &dwType, (LPBYTE)&dwBuildNumber, &dwSize);
    if (lResult == ERROR_SUCCESS) {
...
BOOL SetProfileTime(LPPROFILE lpProfile)
{
    ...
    if (hFileCentral == INVALID_HANDLE_VALUE) {
        DebugMsg((DM_WARNING, TEXT("SetProfileTime:  couldn't open central profile <%s>, error = %d"), szProfile, GetLastError()));
        if (!RevertToUser(&hOldToken)) {
                ...
                if (!MoveFileEx(lpBakProfileImage, lpExpProfileImage, 0)) {
                    ...
                    if (lResult == ERROR_SUCCESS) {
                        ...
                        if (lResult == ERROR_SUCCESS) {
                            ...
                            lResult = RegSetValueEx (hKey, PROFILE_IMAGE_VALUE_NAME, 0, dwType, (LPBYTE) lpExpProfileImage, sizeof(TCHAR)*(lstrlen(lpExpProfileImage)+1));
                        }
...
BOOL PatchNewProfileIfRequired(HANDLE hToken)
{
    ...
    if (UserSid != NULL) {
        // Store the user sid under the Sid key of the local profile
        lResult = RegSetValueEx(hNewKey, TEXT("Sid"), 0, REG_BINARY, UserSid, RtlLengthSid(UserSid));
        if (lResult != ERROR_SUCCESS) {
...
BOOL SaveProfileInfo(LPPROFILE lpProfile)
{
    ...
    lResult = RegSetValueEx (hKey, PROFILE_CENTRAL_PROFILE, 0, REG_SZ, (LPBYTE) lpProfile->lpRoamingProfile, (lstrlen(lpProfile->lpRoamingProfile) + 1) * sizeof(TCHAR));
    if (lResult != ERROR_SUCCESS) {
    ...
    if (szUserGuid) {
        lResult = RegSetValueEx (hKey, PROFILE_GUID, 0, REG_SZ, (LPBYTE) szUserGuid, (lstrlen(szUserGuid)+1)*sizeof(TCHAR));
        if (lResult != ERROR_SUCCESS) {
...
//              FALSE if an error occurs
{
    ...
    while (RegEnumKeyEx (hKeyProfileList, dwIndex, szName, &dwSize, NULL, NULL, NULL, &ftWrite) == ERROR_SUCCESS) {
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szTemp, 0, KEY_READ | KEY_WRITE, &hKeyProfile) == ERROR_SUCCESS) {
            ...
            // If the profile is located in the source directory, move it to the new profiles directory.
            if (CompareString (LOCALE_USER_DEFAULT, NORM_IGNORECASE, szOldProfilePath, iSrcDirLen, lpSrcDir, iSrcDirLen) == CSTR_EQUAL) {
                ...
                // If the user's name has a .000, .001, etc at the end, remove that.
                ...
                // Call ComputeLocalProfileName to get the new profile directory (this also creates the directory)
                lstrcpy (szNewProfilePath, lpDestDir);

                if (!ComputeLocalProfileName (NULL, szName, szNewProfilePath, ARRAYSIZE(szNewProfilePath), szExpNewProfilePath, ARRAYSIZE(szExpNewProfilePath), NULL, FALSE)) {
                    DebugMsg((DM_WARNING, TEXT("MoveUserProfiles:  Failed to generate unique directory name for <%s>"), szName));
                    goto LoopAgain;
                }
                ...
                if (pSD) {
                    if (GetFileSecurity (szExpOldProfilePath, DACL_SECURITY_INFORMATION, pSD, dwLength, &dwLengthNeeded) && (dwLengthNeeded == 0)) {
                        SetFileSecurity (szExpNewProfilePath, DACL_SECURITY_INFORMATION, pSD);
                    } else {
                        DebugMsg((DM_WARNING, TEXT("MoveUserProfiles:  Failed to allocate get security descriptor with %d.  dwLengthNeeded = %d"), GetLastError(), dwLengthNeeded));
                    }

                    LocalFree (pSD);
                } else {
                    DebugMsg((DM_WARNING, TEXT("MoveUserProfiles:  Failed to allocate memory for SD with %d."), GetLastError()));
                }
...
//              FALSE if an error occurs
{
    ...
    if (RegOpenKeyEx (lpProfile->hKeyCurrentUser, USER_SHELL_FOLDERS, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        for (i=0; i < g_dwNumShellFolders; i++) {
            ...
            if (RegQueryValueEx (hKey, c_ShellFolders[i].lpFolderName, NULL, &dwType, (LPBYTE) szTemp, &dwSize) == ERROR_SUCCESS) {
                ...
                // If this is a local directory, create it and set the hidden bit if appropriate
                if (c_ShellFolders[i].bLocal) {
                    if (CompareString (LOCALE_USER_DEFAULT, NORM_IGNORECASE, TEXT("%USERPROFILE%"), dwStrLen, szTemp, dwStrLen) == CSTR_EQUAL) {
                        if (CreateNestedDirectory (szExpTemp, NULL)) {
                            ...
                            DebugMsg((DM_WARNING, TEXT("PrepareProfileForUse:  Failed to create directory <%s> with %d."), szExpTemp, GetLastError()));
                        }
                    }
                }

                // Set the expanded path in the Shell Folders key.
                // This helps some apps that look at the Shell Folders key directly instead of using the shell api
                if (hKeyShellFolders) {
    ...
    if (RegOpenKeyEx (lpProfile->hKeyCurrentUser, TEXT("Environment"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        szTemp[0] = TEXT('\0');
        dwSize = sizeof(szTemp);
        if (RegQueryValueEx (hKey, TEXT("TEMP"), NULL, &dwType, (LPBYTE) szTemp, &dwSize) == ERROR_SUCCESS) {
            if (CompareString (LOCALE_USER_DEFAULT, NORM_IGNORECASE, TEXT("%USERPROFILE%"), dwStrLen, szTemp, dwStrLen) == CSTR_EQUAL) {
                ExpandEnvironmentStrings (szTemp, szExpTemp, ARRAYSIZE(szExpTemp));
                if (!CreateNestedDirectory (szExpTemp, NULL)) {
                    DebugMsg((DM_WARNING, TEXT("PrepareProfileForUse:  Failed to create temp directory <%s> with %d."), szExpTemp, GetLastError()));
                }
            }
        }
        ...
        szTemp[0] = TEXT('\0');
        dwSize = sizeof(szTemp);
        if (RegQueryValueEx (hKey, TEXT("TMP"), NULL, &dwType, (LPBYTE) szTemp, &dwSize) == ERROR_SUCCESS) {
            if (CompareString (LOCALE_USER_DEFAULT, NORM_IGNORECASE, TEXT("%USERPROFILE%"), dwStrLen, szTemp, dwStrLen) == CSTR_EQUAL) {
                ExpandEnvironmentStrings (szTemp, szExpTemp, ARRAYSIZE(szExpTemp));
                if (!CreateNestedDirectory (szExpTemp, NULL)) {
                    DebugMsg((DM_WARNING, TEXT("PrepareProfileForUse:  Failed to create temp directory with %d."), GetLastError()));
                }
            }
        }
...
BOOL DeleteProfile (LPCTSTR lpSidString, LPCTSTR lpProfilePath, LPCTSTR szComputerName)
...
// TBD: Change some of the DeleteProfileEx calls to DeleteProfile
{
    ...
    if (!lpProfilePath) {
        ...
        dwSize = sizeof(szTemp);
        lResult = RegQueryValueEx (hKey, PROFILE_IMAGE_VALUE_NAME, NULL, &dwType, (LPBYTE) szTemp, &dwSize);
        dwSize = sizeof(DWORD);
        lResult = RegQueryValueEx (hKey, PROFILE_STATE, NULL, &dwType, (LPBYTE)&dwInternalFlags, &dwSize);
    ...
    for (tDrive = TEXT('A'); tDrive <= TEXT('Z'); tDrive++) {
        *lpEnd = tDrive;
        ...
        if (!GetVolumeInformation(szShareName, NULL, 0, NULL, &MaxCompLen, &FileSysFlags, szFileSystemName, MAX_PATH))
            continue;
