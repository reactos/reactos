...
/*
Abstract:
    This module contains the code executed at logon for creating a user classes hive and mapping it into the standard user hive.
    The user classes hive and its machine classes counterpart make up the registry subtree known as HKEY_CLASSES_ROOT.
...
Notes:
    Starting with NT5, the HKEY_CLASSES_ROOT key is per-user instead of per-machine -- previously, HKCR was an alias for HKLM\Software\Classes.

    The per-user HKCR combines machine classes stored it the traditional HKLM\Software\Classes location with classes stored in HKCU\Software\Classes.

    Certain keys, such as CLSID, will have subkeys that come from both the machine and user locations.
    When there is a conflict in key names, the user oriented key overrides the other one --
    only the user key is seen in that case.

    Originally, the code in this module was responsible for creating this combined view.
    That responsibility has moved to the win32 registry api's, so the main responsibility of this module is the mapping of the user-specific classes into the registry.

    It should be noted that HKCU\Software\Classes is not the true location of the user-only class data.
    If it were, all the class data would be in ntuser.dat, which roams with the user.
    Since class data can get very large, installation of a few apps would cause HKCU (ntuser.dat) to grow from a few hundred thousand K to several megabytes.
    Since user-only class data comes from the directory, it does not need to roam and therefore it was separated from HKCU (ntuser.dat) and stored in another hive mounted under HKEY_USERS.

    It is still desirable to allow access to this hive through HKCU\Software\Classes, so we use some trickery (symlinks) to make it seem as if the user class data exists there.
--*/
...
LONG CreateRegLink(HKEY hkDest, LPTSTR SubKeyName, LPTSTR lpSourceName)
//  Purpose:    Create a link from the hkDest + SubKeyName pointing to lpSourceRootName
...
//              other NTSTATUS if an error occurs
{
    ...
    // See if this link already exists -- this is necessary because NtCreateKey fails with STATUS_OBJECT_NAME_COLLISION if a link
    // already exists and will not return a handle to the existing link.
    InitializeObjectAttributes(&Attributes, &SubKey, OBJ_CASE_INSENSITIVE | OBJ_OPENLINK, hkDest, NULL);

    // If this call succeeds, we get a handle to the existing link
    Status = NtOpenKey( &hkInternal, MAXIMUM_ALLOWED, &Attributes );
    if (STATUS_OBJECT_NAME_NOT_FOUND == Status) {
        // There is no existing link, so use NtCreateKey to make a new one
        Status = NtCreateKey( &hkInternal, KEY_CREATE_LINK | KEY_SET_VALUE, &Attributes, 0, NULL, REG_OPTION_VOLATILE | REG_OPTION_CREATE_LINK, NULL);
    }

    // Whether the link existed already or not, we should still set the value which determines the link target
    if (NT_SUCCESS(Status)) {
        Status = NtSetValueKey( hkInternal, &SymbolicLinkValueName, 0, REG_LINK, LinkTarget.Buffer, LinkTarget.Length);
        NtClose(hkInternal);
    }
...
//  Purpose:    Deletes a registry key (or link) without using the advapi32 registry apis
...
//              3/6/98      adamed     Created
{
    ...
    InitializeObjectAttributes(&Attributes, &Subtree, OBJ_CASE_INSENSITIVE | OBJ_OPENLINK, hkRoot, NULL);

    Status = NtOpenKey( &hKey, MAXIMUM_ALLOWED, &Attributes );// Open the link
    if (NT_SUCCESS(Status)) {// If we succeeded in opening it, delete it
...
LONG MapUserClassesIntoUserHive(LPPROFILE lpProfile, LPTSTR lpSidString)
{
    LONG   Error;
    LPTSTR lpClassesKeyName;

    // get memory for user classes keyname
    lpClassesKeyName = alloca( lstrlen(lpSidString) * sizeof(TCHAR) + sizeof( USER_CLASSES_HIVE_SUFFIX ) + sizeof( USER_KEY_PREFIX ) + sizeof(TCHAR) );
    ...
    // It's ok if the deletion fails because the classes key, link or nonlink, doesn't exist.
    // It's also ok if it fails because the key exists but is not a link and has children -- in this case, the key and its children will
    ...
    // This key may exist from previous unreleased versions of NT5, or from someone playing around with hive files and adding bogus keys
    if (!RegDelnode (lpProfile->hKeyCurrentUser, CLASSES_SUBTREE)) {
        Error = GetLastError();

        // It's ok if this fails because the key doesn't exist, since nonexistence is our goal.
        if (ERROR_FILE_NOT_FOUND != Error) {
            return Error;
        }
    }

    // At this point, we know that no HKCU\Software\Classes exists, so we should be able to make a link there which points to the hive with the user class data.
    Error = CreateRegLink(lpProfile->hKeyCurrentUser, CLASSES_SUBTREE, lpClassesKeyName);
    return Error;
}
...
//              szLocalHiveDir - out param for location of classes hive folder.
...
//              other error if an error occurs
{
    ...
    // This is a subdir of the user profile which does not roam.

    // Need to do this to fix up a localisation prob. in NT4
    PatchLocalAppData(pProfile->hToken);

    fGotLocalData = GetFolderPath (CSIDL_LOCAL_APPDATA, pProfile->hToken, szLocalHiveDir);
    if (!fGotLocalData) {
        return ERROR_NOT_ENOUGH_MEMORY;
    }

    // append the terminating pathsep so we can add more paths to the newly retrieved subdir
    lstrcat(szLocalHiveDir, CLASSES_SUBDIRECTORY);

    // We will now create our own subdir, CLASSES_SUBDIRECTORY, inside the local appdata subdir we just received above.
...
LONG UnloadClassHive(LPTSTR lpSidString, LPTSTR lpSuffix)
//  Purpose:    unmounts a classes hive

//  Parameters: lpSidString     -   string representing user's sid
//              lpSuffix        -   hive name suffix

//  Return:     ERROR_SUCCESS if successful, other error if not
...
//              3/6/98      adamed     Created
{
    ...
    lpHiveName = alloca( lstrlen(lpSidString) * sizeof(TCHAR) + sizeof(USER_KEY_PREFIX) + (lstrlen(lpSuffix) + 1) *sizeof( TCHAR ) + sizeof(TCHAR) );
    ...
    InitializeObjectAttributes(&Attributes, &ClassesFullPath, OBJ_CASE_INSENSITIVE, NULL, NULL);
    ...
    if (NT_SUCCESS(Status)) {
        ...
        if (MyRegUnLoadKey(HKEY_USERS, lpHiveName + ((sizeof(USER_KEY_PREFIX) / sizeof(TCHAR))-1))) {
            error = ERROR_SUCCESS;
        } else {
            error = GetLastError();
        }
...
//              FALSE if not
{
    ...
    Error = UnloadClassHive(lpSidString, USER_CLASSES_HIVE_SUFFIX);
...
//              lpSuffix    -   Suffix to follow the user's sid when naming the hive
//              lpHiveFileName - full path for backing hive file of user classes
//              phkResult      - root of created hive on success
...
//              other NTSTATUS if an error occurs
{
    ...
    lpHiveKeyName = alloca( lstrlen(lpSidString) * sizeof(TCHAR) + (lstrlen(lpSuffix) + 1) * sizeof(TCHAR) + sizeof(TCHAR) );
    if ( !lpHiveKeyName ) {
        return ERROR_NOT_ENOUGH_MEMORY;
    }
    ...
    // RegLoadKey create or load the existing hive because if the hive is new, we need to apply security.
    fHiveExists = GetFileAttributesEx(lpHiveFilename, GetFileExInfoStandard, &fd );
    ...
    if (ERROR_SUCCESS == res) {
        res = RegOpenKeyEx( HKEY_USERS, lpHiveKeyName, 0, WRITE_DAC | KEY_ENUMERATE_SUB_KEYS | READ_CONTROL, &hkRoot);
        if (ERROR_SUCCESS != res) {
            return res;
        }
    }
    ...
    if (!fHiveExists) {
        ...
        // An existing hive was not found before we created this hive, so we need to set security on the new hive
...
LONG CreateUserClassesHive(LPPROFILE lpProfile, LPTSTR SidString, LPTSTR szLocalHiveDir)
//  Purpose:    create the user-specific classes hive

//  Parameters: lpProfile -   Profile information
//              SidString -   User's Sid as a string
//              szLocalHiveDir - directory in userprofile where hive should be located
...
//              3/6/98      adamed     Created
{
    ...
    lpHiveFilename = alloca( lstrlen(szLocalHiveDir) * sizeof(TCHAR) + sizeof( USER_CLASSES_HIVE_NAME ) + sizeof(TCHAR) * 2);
    if ( !lpHiveFilename ) {
        return ERROR_NOT_ENOUGH_MEMORY;
    }
    ...
    res = CreateClassHive(lpProfile, SidString, USER_CLASSES_HIVE_SUFFIX, lpHiveFilename);
    if (ERROR_SUCCESS != res) {
        return res;
    }
...
LONG MoveUserClassesBeforeMerge(LPPROFILE lpProfile, LPCTSTR lpcszLocalHiveDir)
//  Purpose:    move HKCU\Software\Classes before MapUserClassesIntoUserHive() deletes it.
...
//              5/6/99      vtan       Created
{
    ...
    if (ERROR_SUCCESS == res)
    {
        ...
        if ((ERROR_SUCCESS == RegQueryInfoKey(hKeySource, NULL, NULL, NULL, &dwSubKeyCount, NULL, NULL, NULL, NULL, NULL, NULL, NULL)) && (dwSubKeyCount > 0))
        {
            LPTSTR  pszLocalTempHive;

            // Allocate enough space for the local hive directory and the temp hive filename.
            pszLocalTempHive = alloca((lstrlen(lpcszLocalHiveDir) * sizeof(TCHAR)) + sizeof(TEMPHIVE_FILENAME) + (sizeof('\0') * sizeof(TCHAR)));
...
//  Purpose:    Combines the HKLM\Software\Classes subtree with the HKCU\Software\Classes subtree
...
//              other NTSTATUS if an error occurs
{
    ...
    // first, we will create a directory for the user-specific classes hive -- we need memory for it:
    ...
    if (ERROR_SUCCESS != error) {
        DebugMsg((DM_WARNING, TEXT("LoadUserClasses: Failed to create folder for combined hive (%d)."), error));
        return error;
    }

    // Move HKCU\Software\Classes before merging the two branches.
    // Ignore any errors here as this branch is about to be deleted by the merge anyway.
    // The reason for this move is because NT4 stores customized shell icons in HKCU\Software\Classes\CLSID\{CLSID_x} and NT5 stores this at HKCU\Software\Microsoft\Windows\
    // CurrentVersion\Explorer\CLSID\{CLSID_x} and must be moved now before being deleted.
    error = MoveUserClassesBeforeMerge(lpProfile, szLocalHiveDir);
    if (ERROR_SUCCESS != error) {
        DebugMsg((DM_WARNING, TEXT("MoveUserClassesBeforeMerge: Failed unexpectedly (%d)."), error));
    }
