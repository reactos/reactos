...
LRESULT CALLBACK LogonDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        if (hBmp) {
            SendDlgItemMessage(hDlg, IDD_ICON, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM) hBmp);
        }
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IDOK)
        {
            ...
            if (LogonUser(szUserName, szDomainName, szPassword, LOGON32_LOGON_INTERACTIVE, LOGON32_PROVIDER_DEFAULT, &hUserToken)) {
                EndDialog(hDlg, TRUE);
                return TRUE;
            }
            else {
                ...
                wsprintf(szBuffer, TEXT("Logon Failed with Error %d"), GetLastError());
