...
LPTSTR GetDCList (BOOL bZero)
{
    ...
    if (RegOpenKeyEx (HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Poltest"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        dwSize = 256 * sizeof(TCHAR);
        RegQueryValueEx (hKey, TEXT("DCList"), NULL, &dwType, (LPBYTE) lpList, &dwSize);
        RegCloseKey (hKey);
    }
...
LPTSTR GetDomainName (BOOL bZero)
{
    ...
    if (RegOpenKeyEx (HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Poltest"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        dwSize = 256 * sizeof(TCHAR);
        RegQueryValueEx (hKey, TEXT("DomainName"), NULL, &dwType, (LPBYTE) lpList, &dwSize);
        RegCloseKey (hKey);
    }
    ...
    switch (message)
    {
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == IDOK) {
                ...
                if (RegCreateKeyEx (HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Poltest"), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &dwDisp) == ERROR_SUCCESS) {
                    GetDlgItemText (hDlg, IDC_NAME, szBuffer, 512);
                    RegSetValueEx (hKey, TEXT("DomainName"), NULL, REG_SZ, (LPBYTE) szBuffer, (lstrlen(szBuffer) + 1) *sizeof(TCHAR));

                    GetDlgItemText (hDlg, IDC_DCLIST, szBuffer, 512);
                    RegSetValueEx (hKey, TEXT("DCList"), NULL, REG_SZ, (LPBYTE) szBuffer, (lstrlen(szBuffer) + 1) *sizeof(TCHAR));
    ...
    switch (message)
    {
        case WM_INITDIALOG:
            ...
            if (RegOpenKeyEx (HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Poltest"), 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
                ...
                RegQueryValueEx (hKey, TEXT("GPOName"), NULL, &dwType, (LPBYTE) szGPOName, &dwSize);
                dwSize = sizeof(dwTemp);
                dwTemp = 0;
                RegQueryValueEx (hKey, TEXT("GPONameOptions"), NULL, &dwType, (LPBYTE) &dwTemp, &dwSize);
                RegCloseKey (hKey);
            }
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == IDOK) {
                ...
                if (RegCreateKeyEx (HKEY_CURRENT_USER, TEXT("Software\\Microsoft\\Poltest"), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, &dwDisp) == ERROR_SUCCESS) {
                    ...
                    RegSetValueEx (hKey, TEXT("GPOName"), NULL, REG_SZ, (LPBYTE) szGPOName, (lstrlen(szGPOName) + 1) *sizeof(TCHAR));
                    RegSetValueEx (hKey, TEXT("GPONameOptions"), NULL, REG_SZ, (LPBYTE) &dwTemp, sizeof(dwTemp));
                    RegCloseKey (hKey);
                }
                ...
                if (lpInfo) {
                    ...
                    hThread = CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE)CheckGPOThread, lpInfo, 0, &dwID);
                    if (hThread) {
                        CloseHandle (hThread);
                    }
                }
            ...
            if (LOWORD(wParam) == IDC_BROWSE) {
                ...
                if (SUCCEEDED(BrowseForGPO (&info))) {
                    if (SUCCEEDED(CoCreateInstance (CLSID_GroupPolicyObject, NULL, CLSCTX_SERVER, IID_IGroupPolicyObject, (void**)&pGPO))) {
