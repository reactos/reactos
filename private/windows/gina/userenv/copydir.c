                             ...
                             LPCTSTR lpExclusionList)
{
    ...
    if (dwFlags & CPD_SHOWSTATUS) {
        ...
        if (ThreadInfo.hStatusInitEvent && ThreadInfo.hStatusTermEvent) {
            ...
            hStatusThread = CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE) CopyStatusThread, (LPVOID) &ThreadInfo, 0, &dwThreadId);
            if (hStatusThread) {
        ...
        if (dwFlags & CPD_SLOWCOPY) {
            ...
            if (lpSrcFiles) {
                ...
                for (i = 0; i < NUM_COPY_THREADS; i++) {
                    ...
                    if (hThreads[dwThreadCount] = CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE) CopyFileFunc, (LPVOID) &ThreadInfo, CREATE_SUSPENDED, &dwThreadId)) {
                        dwThreadCount++;
                    }
                }
