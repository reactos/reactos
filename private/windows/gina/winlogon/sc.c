...
// This file contains a small number of functions to listen for smart card events
...
BOOL ScAddEvent(SC_EVENT_TYPE   Type, PSC_DATA        Data)
{
    PSC_EVENT Event ;
...
BOOL ScRemoveEvent(SC_EVENT_TYPE * Type, PSC_DATA *      Data)
{
    PSC_EVENT Event ;
    PLIST_ENTRY Head ;
...
VOID ScFreeEventData(PSC_DATA Data)
{
...
VOID SetLogonReader(LPCTSTR szReader)
{
    HKEY hKey;

    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_WRITE, &hKey) == ERROR_SUCCESS) {
        RegSetValueEx(hKey, TEXT("ScLogonReader"), 0, REG_SZ, (PBYTE) szReader, wcslen(szReader) * sizeof(WCHAR));
        RegCloseKey(hKey);
    }
}


VOID PostSmartCardEvent(PTERMINAL pTerm, SCARDCONTEXT SCContext, SCARD_READERSTATE * Reader)
{
    SC_DATA * Data ;
    ...
    DebugLog(( DEB_TRACE_SC, "Reader %ws reports card %s\n", Reader->szReader, ( EventType == ScRemove ? "removal" : "insertion" ) ));
    ...
    if ( EventType == ScInsert && (Reader->dwEventState & SCARD_STATE_MUTE) == 0)
    {
        // get the card name from the ATR
        StringLen = SCARD_AUTOALLOCATE ;
        Result = pSCardListCards(SCContext, Reader->rgbAtr, NULL, 0, (LPTSTR) &String, &StringLen );
        if ( Result == SCARD_S_SUCCESS )
        {
            ...
            StringLen = SCARD_AUTOALLOCATE ;
            Result = pSCardGetCardTypeProviderName(SCContext, ScInfo.pszCard, SCARD_PROVIDER_CSP, (LPTSTR) &String, &StringLen );
            if ( Result == SCARD_S_SUCCESS )
            {
    ...
    if ( Data )
    {
        ...
        {
            PostMessage( pTerm->hwndSAS, WLX_WM_SAS, WLX_SAS_INTERNAL_SC_EVENT, 0 );
            return ;
        }
...
DWORD WINAPI SCWorkerThread(PVOID   Param)
{
    ...
    // synch up with main thread.
    // If the main thread needs to kill this thread, then it will reset the Tid stored in the global data.
    // Since the call to the s/c resource mgr is blocking, we check this frequently.
    // On startup, we will spin until a non-zero tid is stored there.
    ...
    __try {
        do {
            ...
            Result = pSCardEstablishContext(SCARD_SCOPE_SYSTEM, NULL, NULL, &SCContext );
            if (Result != SCARD_S_SUCCESS || SCContext == 0) {
                __leave;
            }

            ReaderSize = SCARD_AUTOALLOCATE;
            Result = pSCardListReaders(SCContext, TEXT("SCard$AllReaders"), (LPTSTR) &ReaderNames, &ReaderSize );
            if ( pTerm->SmartCardTid != MyTid)
            {
                __leave;
            }
            ...
            Result = pSCardGetStatusChange(SCContext, 1000, ReaderStates, ReaderCount );
            if ( Result != SCARD_S_SUCCESS )
            {
                __leave;
            }
            ...
            while ( pTerm->SmartCardTid == MyTid )
            {
                ...
                Result = pSCardGetStatusChange(SCContext, INFINITE, ReaderStates, ReaderCount );
                ...
                if (Result == SCARD_E_SYSTEM_CANCELLED) {// the sc system has been stopped
                    ...
                    do
                    {
                        Result = WaitForSingleObjectEx(hCalaisStarted, 15000, FALSE);
                        if ( pTerm->SmartCardTid != MyTid )
                        {
                            __leave;
                        }
                ...
                else
                {
                    ...
                    {
                        if ( (ReaderStates[ i ].dwEventState & SCARD_STATE_CHANGED ) )
                        {
                            // This reader changed its state.  Could be an insertion, could be a deletion.
                            DebugLog(( DEB_TRACE_SC, "Posting event for reader %ws\n", ReaderStates[i].szReader ));
                            PostSmartCardEvent( pTerm, SCContext, &ReaderStates[ i ]);
                        }
...
VOID ScThreadDeathCallback(PVOID Context, BOOLEAN Timeout)
{
    PSC_THREAD_CONTROL Control ;
...
VOID StartScThread(PTERMINAL pTerm)
{
    PSC_THREAD_CONTROL Control ;
    ...
    Control = (PSC_THREAD_CONTROL) LocalAlloc( LMEM_FIXED, sizeof( SC_THREAD_CONTROL ) );
    if ( Control )
    {
        pTerm->SmartCardTid = 0 ;

        Control->pTerm = pTerm ;
        Control->Thread = CreateThread( NULL, 0, SCWorkerThread, Control, 0, &pTerm->SmartCardTid );
        if ( Control->Thread )
        {
            RegisterWaitForSingleObject(&Control->Callback, Control->Thread, ScThreadDeathCallback, Control, INFINITE, WT_EXECUTEONLYONCE );
        }
    }
}
...
BOOL IsSmartCardReaderPresent(PTERMINAL pTerm)
{
    ...
    __try {
        ...
        // this way we don't have to call any resource manager function which could potentially start the service
        lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, CALAIS_PATH, 0, KEY_READ, &hKey);
        if ( lResult != 0 ) {
        ...
        lResult = pSCardEstablishContext(SCARD_SCOPE_SYSTEM, NULL, NULL, &hCtx);
        if (lResult != SCARD_S_SUCCESS) {
            leave;
        }

        lResult = pSCardListReaders(hCtx, TEXT("SCard$DefaultReaders"), (LPTSTR) &pReaders, &dwAutoAllocate);
        if (lResult != SCARD_S_SUCCESS) {
            leave;
        }
...
BOOL StartListeningForSC(PTERMINAL pTerm)
{
    NTSTATUS Status ;
...
BOOL StopListeningForSC(PTERMINAL pTerm)
{
