    ...
    )
{
    ...
    SetMyAce(&(Ace[AceCount++]), UserSid, PROCESS_ALL_ACCESS, 0);// Define the User ACEs
    ASSERT((sizeof(Ace) / sizeof(MYACE)) >= AceCount);// Check we didn't goof
    ...
    )
{
    ...
    psd = CreateUserProcessTokenSD( pTerm->pWinStaWinlogon->UserProcessData.UserSid, g_WinlogonSid );
    if ( !psd )
    {
        return FALSE ;
    }
    ...
    Status = RtlAdjustPrivilege(SE_ASSIGNPRIMARYTOKEN_PRIVILEGE, TRUE, FALSE, &WasEnabled);
    if (NT_SUCCESS(Status)) {
        ...
        AdjustStatus = RtlAdjustPrivilege(SE_ASSIGNPRIMARYTOKEN_PRIVILEGE, WasEnabled, FALSE, &WasEnabled);
