...
Abstract:
    This module contains the code necessary to initialize the OS/2 SS entries in the registry.
    It migrates information from os/2's config.sys file to the NT registry.
...
Revision History:
    The code was originally in the os/2 subsystem, but was moved to winlogon so the registry initialization can be done during boot time.
    This way the registry info will be correct for the first os/2 program to run.
--*/
...
// The following constants define the meaning of the Flags value in the config sys key entry:
...
static WCHAR Os2ConfigSysKeyName[] = L"\\REGISTRY\\MACHINE\\SOFTWARE\\Microsoft\\OS/2 Subsystem for NT\\1.0\\config.sys";
static WCHAR Os2EnvironmentDirectory[] = L"\\REGISTRY\\MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment";
...
VOID Os2RegSetFlags(IN HANDLE ConfigSysKeyHandle, IN ULONG FlagsValue)
/*++
Routine Description:
...
--*/
{
    ...
    Status = NtSetValueKey(ConfigSysKeyHandle, &String_U, 0L, REG_DWORD, &FlagsValue, sizeof(DWORD));
...
VOID Os2RegSetStamp(IN HANDLE ConfigSysKeyHandle, IN PLARGE_INTEGER pTimeStamp, IN PLARGE_INTEGER pSizeStamp)
/*++
Routine Description:
...
--*/
{
    ...
    Status = NtSetValueKey(ConfigSysKeyHandle, &String_U, 0L, REG_BINARY, (PVOID) Buffer, sizeof(Buffer));
    if (!NT_SUCCESS(Status)) {
#if DBG
        KdPrint(("Os2RegSetFlags: NetSetValueKey failed, Status = %lx\n", Status));
#endif
...
Arguments:
    ConfigSysKeyHandle -- supplies a read/write handle to the config.sys reg key this is used to update the flags/stamp values
    FlagsValue -- The previously existing "Flags" value that was read in from the registry.  If this is -1, then no previous Flags value existed.
    ConfigSysFileHandle -- An optional handle to an already open config.sys file pTimeStamp, pSizeStamp -- info about the open config.sys file.

    Note:  The parameters ConfigSysFileHandle + pTimeStamp + pSizeStamp are optional, and come as a packet.
        If the config.sys file was previously opened, the handle will be non-null, and all 3 params must be valid.
        If the config.sys file was not previously opened (successfully), the handle should be NULL, and the other 2 params need not be valid.
...
--*/
{
    ...
    if (FlagsValue != (ULONG)-1) {
        if (FlagsValue & FLAGS_DISABLEMIGRATION) {
            ...
            // so there's no need to close it.  Also, no need to update flags/stamp.
            return(FALSE);
        }
        ...
        // at all (not even 1st-time), then set the FlagsValue similarly, and also do a { Os2RegSetFlags(..), return FALSE }
    }
    ...
    if (ConfigSysFileHandle == NULL) {
        ...
        if (IsNEC_98) {
            ...
            if (GetSystemRootDrive(&chTemp)) {
                wsprintf(Os2OriginalCanonicalConfigSys_NEC_PC9800, L"\\DosDevices\\%c:\\CONFIG.SYS", chTemp);
            }
        ...
        InitializeObjectAttributes(&Obja, &CanonicalConfigDotSys_U, OBJ_CASE_INSENSITIVE, NULL, NULL);

        Status = NtOpenFile(&ConfigSysFileHandle, FILE_GENERIC_READ, &Obja, &IoStatus, FILE_SHARE_READ, FILE_SYNCHRONOUS_IO_NONALERT);
        if (!NT_SUCCESS(Status)) {
        ...
        Status = Or2GetFileStamps(ConfigSysFileHandle, &TimeStamp, &SizeStamp);
        if (!NT_SUCCESS(Status)) {
#if DBG
...
//      Or2UnicodeEqualCI(VarName_U.Buffer, L"PATH=", 5) ||
...
#if 0
    Status = NtQueryValueKey(Os2EnvironmentKeyHandle, &VarName_U, KeyValuePartialInformation, &KeyValueInfo, sizeof(KeyValueInfo), &ResultLength);
#else
    ...
    if (Status == STATUS_OBJECT_NAME_NOT_FOUND) {
        ...
        Status = NtSetValueKey(Os2EnvironmentKeyHandle, &VarName_U, (ULONG)0, REG_EXPAND_SZ, VarValue_U.Buffer, VarValue_U.Length + sizeof(WCHAR));
        Value[ValueLen] = wch;
        if (!NT_SUCCESS(Status))
        {
#if DBG
...

{
    ...
    *phEnvironmentKeyHandle = NULL;
    ...
    InitializeObjectAttributes(&Obja, &SoftwareDirectory_U, OBJ_CASE_INSENSITIVE, NULL, NULL);
    Status = NtOpenKey(&SoftwareKeyHandle, KEY_CREATE_SUB_KEY, &Obja);
    if (!NT_SUCCESS(Status)) {
#if DBG
        IF_OS2_DEBUG( INIT ) {
    ...
    InitializeObjectAttributes(&Obja, &ProductDirectory_U, OBJ_CASE_INSENSITIVE, SoftwareKeyHandle, securityDescriptor);
    Status = NtCreateKey(&ProductKeyHandle, KEY_CREATE_SUB_KEY, &Obja, 0, &Class_U, REG_OPTION_NON_VOLATILE, &Disposition);
    ...
    InitializeObjectAttributes(&Obja, &VersionDirectory_U, OBJ_CASE_INSENSITIVE, ProductKeyHandle, securityDescriptor);
    Status = NtCreateKey(&VersionKeyHandle, KEY_CREATE_SUB_KEY, &Obja, 0, &Class_U, REG_OPTION_NON_VOLATILE, &Disposition);
    ...
    Status = NtCreateKey(&Os2IniKeyHandle, KEY_CREATE_SUB_KEY, &Obja, 0, &Class_U, REG_OPTION_NON_VOLATILE, &Disposition);
    if (!NT_SUCCESS(Status))
    {
#if DBG
        IF_OS2_DEBUG( INIT ) {
    ...
    InitializeObjectAttributes(&Obja, &ConfigSysName_U, OBJ_CASE_INSENSITIVE, VersionKeyHandle, securityDescriptor);
    Status = NtCreateKey(&ConfigSysKeyHandle, KEY_READ | KEY_WRITE, &Obja, 0, &Class_U, REG_OPTION_NON_VOLATILE, &Disposition);
    if (Dacl != NULL) {
        RtlFreeHeap(Os2Heap, 0, Dacl);
    }
    NtClose(VersionKeyHandle);
    ...
    InitializeObjectAttributes(&Obja, &EnvRegDir_U, OBJ_CASE_INSENSITIVE, NULL, NULL);
    Status = NtOpenKey(&EnvironmentKeyHandle, KEY_READ | KEY_WRITE, &Obja);
    if (!NT_SUCCESS(Status)) {
#if DBG
        IF_OS2_DEBUG( INIT ) {
...

{
    ...
    InitializeObjectAttributes(&Obja, &String_U, OBJ_CASE_INSENSITIVE, NULL, NULL);
    Status = NtOpenKey(&ConfigSysKeyHandle, KEY_READ, &Obja);
    if (!NT_SUCCESS(Status)) {
        if (Status != STATUS_OBJECT_PATH_NOT_FOUND && Status != STATUS_OBJECT_NAME_NOT_FOUND && Status != STATUS_OBJECT_PATH_INVALID) {
#if DBG
    ...
    RtlInitUnicodeString(&String_U, Os2ConfigFlagName);
    Status = NtQueryValueKey(ConfigSysKeyHandle, &String_U, KeyValuePartialInformation, pInfo, sizeof(RegBuffer), &ResultLength);
    if (!NT_SUCCESS(Status)) {
        NtClose(ConfigSysKeyHandle);
        if (Status != STATUS_OBJECT_PATH_NOT_FOUND && Status != STATUS_OBJECT_NAME_NOT_FOUND && Status != STATUS_OBJECT_PATH_INVALID) {
#if DBG
    ...
    Status = NtQueryValueKey(ConfigSysKeyHandle, &String_U, KeyValuePartialInformation, NULL, 0, &ResultLength);
    ...
    if (Status == STATUS_OBJECT_NAME_NOT_FOUND || Status == STATUS_OBJECT_PATH_NOT_FOUND || Status == STATUS_OBJECT_PATH_INVALID) {
        NtClose(ConfigSysKeyHandle);
        goto BuildRegistry;
    }
...
--*/
{
    ...
    lRet = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\Windows NT\\CurrentVersion"), 0, KEY_QUERY_VALUE, &hSubKey);
    if (ERROR_SUCCESS == lRet) {
        lRet = RegQueryValueEx(hSubKey, TEXT("SystemRoot"), NULL, &dwType, (LPBYTE)szTemp, &cbData);
    } else {
        return FALSE;
    }
