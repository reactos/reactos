...
DWORD WaiterThread(PVOID hProcess);
VOID SetWin9xUpgradePasswords (PTERMINAL pTerm);


VOID UpdateRegistryItem(PREG_CHANGE Change)
{
    HKEY Key ;
    int err ;
    DWORD Type ;
    ...
    if ( Percent )
    {
        ...
        Size = ExpandEnvironmentStrings( Change->SubString, Percent, MAX_PATH );
        if ( (Size == 0) || (Size > MAX_PATH) )
        {
            ...
            Size = ExpandEnvironmentStrings( Change->SubString, Percent, Size );
            if ( Size == 0  )
            {
                LocalFree( Percent );
                return ;
    ...
    Size = sizeof( Scratch );
    String = Scratch ;
    err = RegQueryValueEx(Key, Change->ValueName, 0, &Type, (PUCHAR) String, &Size );
    if ( err )
    {
        if ( ( err == ERROR_PATH_NOT_FOUND ) || ( err == ERROR_FILE_NOT_FOUND ) )
        {
            if ( ( Change->Operation == RegOpAppendSubString ) || ( Change->Operation == RegOpInsertSubString ) )
            {
                RegSetValueEx(Key, Change->ValueName, 0, REG_SZ, (PUCHAR) Change->SubString, ( wcslen( Change->SubString ) + 1 ) * sizeof( WCHAR ) );
            }
            RegCloseKey( Key );
            return;
        }
    ...
    while ( Scan && (*Scan != L'\0') )
    {
        ...
        if ( _wcsicmp( Trail, Change->SubString ) == 0 )
        {
            ...
            if ( Change->Operation == RegOpRemoveSubString )
            {
                ...
                if ( Scan )
                {
                    RtlMoveMemory( Trail, Scan + 1 , ( wcslen( Scan + 1 ) + 1 ) * sizeof( WCHAR ) );
                }
                else
                {
                    // Easy case:
    ...
    if ( ( Change->Operation == RegOpInsertSubString ) || ( Change->Operation == RegOpAppendSubString ) )
    {
        Changed = LocalAlloc( LMEM_FIXED, Size + ( wcslen( Change->SubString ) + 2 ) * sizeof( WCHAR ) );
        if ( Changed )
        {
            *Changed = L'\0';
            ...
            {
                wcscpy( Changed, Change->SubString );
                wcscat( Changed, L"," );
            }
    ...
    if ( StringChanged )
    {
        RegSetValueEx(Key, Change->ValueName, 0, REG_SZ, (PUCHAR) String, (wcslen( String ) + 1 ) * sizeof( WCHAR ) );
    }
...
--*/
{
    ...
    Status = NtCreateEvent( &EventHandle, 0, &EventAttributes, NotificationEvent, FALSE);
...
--*/
{
   ...
   if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, KEYNAME_SETUP, 0, KEY_READ, &hKeySetup) == ERROR_SUCCESS) {
       dwSize = sizeof(SetupType);
       RegQueryValueEx (hKeySetup, VARNAME_SETUPTYPE, NULL, &dwType, (LPBYTE) &SetupType, &dwSize);
...
BOOL SetSetupType (DWORD dwType)
/*++
Routine Description:
...
--*/
{
   ...
   if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, KEYNAME_SETUP, 0, KEY_WRITE, &hKeySetup) == ERROR_SUCCESS) {
       lResult = RegSetValueEx (hKeySetup, VARNAME_SETUPTYPE, 0, REG_DWORD, (LPBYTE) &dwType, sizeof(dwType));
       RegCloseKey (hKeySetup);
   }
...
VOID ExecuteSetup(PTERMINAL pTerm)
/*++
Routine Description:
    Execute setup.exe.  The command line to be passed to setup is obtained from HKEY_LOCAL_MACHINE\system\setup:cmdline.
...
--*/
{
    ...
    FixupRegistry();
    if((Result = RegOpenKey( HKEY_LOCAL_MACHINE, KEYNAME_SETUP, &hKey)) == NO_ERROR) {
        DataSize = sizeof(CmdLineBuffer);
        Result = RegQueryValueEx( hKey, VARNAME_SETUPCMD, NULL, NULL, (LPBYTE)CmdLineBuffer, &DataSize);
        if(Result == NO_ERROR) {
    ...
    {
        wsprintf( DebugCmdLineBuffer, TEXT( "ntsd %s %s" ), (RShift & 0x8000) ? TEXT("-d") : TEXT(""), CmdLine );
                            ...
                            NULL) )
    {
        if (hProcess)
        {
            ...
            hThread = CreateThread( NULL, 0, WaiterThread, (LPVOID)hProcess, 0, &ThreadId);
            if(hThread) {
...
VOID CheckForIncompleteSetup (PTERMINAL pTerm)
/*++
Routine Description:
...
--*/
{
   ...
   if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, KEYNAME_SETUP, 0, KEY_READ, &hKeySetup) == ERROR_SUCCESS) {
       ...
       RegQueryValueEx (hKeySetup, VARNAME_SETUPINPROGRESS, NULL, &dwType, (LPBYTE) &SetupInProgress, &dwSize);
       RegCloseKey (hKeySetup);
   }

    // If setup did not complete then make them reboot
    if (SetupInProgress) {
        TimeoutMessageBox(pTerm, NULL, IDS_SETUP_INCOMPLETE, IDS_WINDOWS_MESSAGE, MB_ICONSTOP | MB_OK);
...
VOID CheckForRepairRequest (void)
{
    HKEY hkeyWinlogon;
    LONG lResult;
    DWORD dwSize, dwType;
    ...
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_READ | KEY_WRITE, &hkeyWinlogon) == ERROR_SUCCESS) {
        dwSize = sizeof(bRunRepair);
        if (RegQueryValueEx (hkeyWinlogon, L"Repair", NULL, &dwType, (LPBYTE) &bRunRepair, &dwSize) == ERROR_SUCCESS) {
    ...
    if (bRunRepair) {
        ...
        if (hSysSetup) {
            RepairStartMenuItems = (REPAIRSTARTMENUITEMS)GetProcAddress(hSysSetup, "RepairStartMenuItems");
...
BOOL CheckForNetAccessWizard (PTERMINAL pTerm)
{
    BOOL fReboot = FALSE;
    ...
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_READ | KEY_WRITE, &hkeyWinlogon) == ERROR_SUCCESS) {
        ...
        RegQueryValueEx (hkeyWinlogon, L"RunNetAccessWizard", NULL, &dwType, (LPBYTE) &uWizardType, &dwSize);
        dwSize = sizeof(dwWin9xUpgrade);
        RegQueryValueEx (hkeyWinlogon, L"SetWin9xUpgradePasswords", NULL, &dwType, (LPBYTE) &dwWin9xUpgrade, &dwSize);
    }

    // check the wizard type, if its != NAW_NETID then we assume that we should be starting the wizard for post setup.
    if ( uWizardType != NAW_NETID || dwWin9xUpgrade )
    {
        // Wait on the font loading thread here, so we don't inadvertantly re-enter the stuff in user that gets confused.
        if ( hFontThread )
        {
...
VOID SetWin9xUpgradePasswords (PTERMINAL pTerm)
{
    ...
    rc = RegOpenKeyEx (HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", 0, KEY_READ, &Key);
    if (rc == ERROR_SUCCESS) {
        rc = RegQueryValueEx (Key, L"MigPwd", NULL, NULL, NULL, &Size);
    ...
    if (rc == ERROR_SUCCESS) {
        ...
        ProcessResult = CreateProcess (CmdLine, NULL, NULL, NULL, FALSE, CREATE_DEFAULT_ERROR_MODE, NULL, SysDir, &si, &pi);
        if (ProcessResult) {
            // Wait for the process to complete.  The machine stays awake until it is done.
            ...
            rc = RegOpenKeyEx (HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_READ | KEY_WRITE, &Key);
            if (rc == ERROR_SUCCESS) {
