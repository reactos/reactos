...
BOOL InitializeMultiUserFunctionsPtrs (void)
{
    ...
    //_WinStationCallback
    gpfnWinStationCallback = (PWINSTATION_CALLBACK) GetProcAddress(dllHandle, "_WinStationCallback");
    ...
    gpfnWinStationDisconnect = (PWINSTATION_DISCONNECT) GetProcAddress(dllHandle, "WinStationDisconnect");
    ...
    //WinStationConnectW
     gpfnWinStationConnect = (PWINSTATION_CONNECT) GetProcAddress(dllHandle, "WinStationConnectW");
    ...
    //WinStationQueryInformationW
     gpfnWinStationQueryInformation = (PWINSTATION_QUERY_INFORMATION) GetProcAddress(dllHandle, "WinStationQueryInformationW");
    ...
    //ServerQueryInetConnectorInformationW
     gpfnServerQueryInetConnectorInformation = (PSERVER_QUERY_IC_INFORMATION) GetProcAddress(dllHandle, "ServerQueryInetConnectorInformationW");
    ...
    gpfnWinStationEnumerate_Indexed = (PWINSTATION_ENUMERATE_INDEXED) GetProcAddress(dllHandle, "WinStationEnumerate_IndexedW");
    ...
    //WinStationNameFromLogonIdW
     gpfnWinStationNameFromSessionId = (PWINSTATION_NAME_FROM_SESSIONID) GetProcAddress(dllHandle, "WinStationNameFromLogonIdW");
    ...
    //WinStationShutdownSystem
     gpfnWinStationShutdownSystem = (PWINSTATION_SHUTDOWN_SYSTEM) GetProcAddress(dllHandle, "WinStationShutdownSystem");
    ...
    //_WinStationWaitForConnect
     gpfnWinStationWaitForConnect = (PWINSTATION_WAIT_FOR_CONNECT) GetProcAddress(dllHandle, "_WinStationWaitForConnect");
    ...
    //WinStationSetInformationW
     gpfnWinStationSetInformation = (PWINSTATION_SET_INFORMATION) GetProcAddress(dllHandle, "WinStationSetInformationW");
    ...
    //_WinStationNotifyLogon
     gpfnWinStationNotifyLogon = (PWINSTATION_NOTIFY_LOGON) GetProcAddress(dllHandle, "_WinStationNotifyLogon");
    ...
    //_WinStationInitNewSession
     gpfnWinStationNotifyNewSession = (PWINSTATION_NOTIFY_NEW_SESSION) GetProcAddress(dllHandle, "_WinStationNotifyNewSession");
    ...
    )
{
    ...
    switch (Status)
    {
        case ERROR_CTX_LOGON_DISABLED:
            Result = TimeoutMessageBox(pTerm, hDlg, IDS_MULTIUSER_LOGON_DISABLED, IDS_LOGON_MESSAGE, MB_OK | MB_ICONEXCLAMATION);
            break;
        case ERROR_CTX_WINSTATION_ACCESS_DENIED:
            Result = TimeoutMessageBox(pTerm, hDlg, IDS_MULTIUSER_WINSTATION_ACCESS_DENIED, IDS_LOGON_MESSAGE, MB_OK | MB_ICONEXCLAMATION);
            break;
        default:
#if DBG
            ...
            Result = WlxMessageBox(pTerm, hDlg, Buffer2, Buffer1, MB_OK | MB_ICONEXCLAMATION);
            break;
    }
...
* HISTORY:
*/
INT_PTR WINAPI LogonDisabledDlgProc(
    HWND    hDlg,
    UINT    message,
    WPARAM  wParam,
    LPARAM  lParam
    )
{
    ...
    switch (message)
    {
        ...
        case WLX_WM_SAS:
            ...
            {
                // If this was a timeout, return false, and let winlogon kill us later
                return(FALSE);
            }
...
*/
LONG DefaultUserConfigQuery( WCHAR * pServerName, PUSERCONFIGW pUserConfig, ULONG UserConfigLength, PULONG pReturnLength )
{
