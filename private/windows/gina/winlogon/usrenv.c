...
BOOL SetGINAEnvVars (PWINDOWSTATION pWS);


BOOL SetupBasicEnvironment(PVOID * ppEnv)
{
    HKEY hKey;
...
*/
BOOL SetupUserEnvironment(PTERMINAL pTerm)
{
    PVOID pEnv = NULL;
     ...
     */
    SetHomeDirectoryEnvVars(&pEnv, lpHomeDirectory, lpHomeDrive, lpHomeShare, lpHomePath);
    ...
    if (GetUserProfileDirectory (pWS->UserProcessData.UserToken, lpUserProfile, &dwSize)) {
        SetUserEnvironmentVariable(&pEnv, USERPROFILE_VARIABLE, lpUserProfile, TRUE);
    }

    pWS->UserProcessData.pEnvironment = pEnv;

    if (pWS->UserProcessData.CurrentDirectory = (LPTSTR)Alloc(sizeof(TCHAR)*(lstrlen(lpHomeDirectory)+1)))
...
* 2-28-92  Johannec     Created
*/
VOID ResetEnvironment(PTERMINAL pTerm)
{
    PWINDOWSTATION pWS = pTerm->pWinStaWinlogon;
...
*/
BOOL OpenHKeyCurrentUser(PWINDOWSTATION pWS)
{
    DWORD err;
    ...
    Status = RtlOpenCurrentUser(MAXIMUM_ALLOWED, &pWS->UserProcessData.hCurrentUser );
...
*/
VOID CloseHKeyCurrentUser(PWINDOWSTATION pWS)
{
    DWORD err;
...
*/
BOOL SetEnvironmentULong(PVOID * Env, LPTSTR Variable, ULONG_PTR Value)
{
    WCHAR Buffer[16];
    int Result;
    NTSTATUS Status ;
...
#if defined(_WIN64)
    if ((Value >> 32) != 0) {
        Result = _snwprintf(Buffer, sizeof(Buffer)/sizeof(TCHAR), TEXT("%x"), (ULONG)(Value >> 32));
        Result += _snwprintf(Buffer + Result/sizeof(TCHAR), (sizeof(Buffer) - Result)/sizeof(TCHAR), TEXT("%08x"), (ULONG)Value);
    } else {
        Result = _snwprintf(Buffer, sizeof(Buffer)/sizeof(TCHAR), TEXT("%x"), (ULONG)Value);
    }
#else
    ...
    Status = RtlSetEnvironmentVariable(Env, &Var, &Val );
    return NT_SUCCESS( Status );
}
...
BOOL SetEnvironmentLargeInt(PVOID * Env, LPTSTR Variable, LARGE_INTEGER Value)
{
    TCHAR Buffer[20];
    int Result;
    UNICODE_STRING Var ;
...
BOOL SetGINAEnvVars (PWINDOWSTATION pWS)
{
    ...
    if (RegCreateKeyEx(pWS->UserProcessData.hCurrentUser, TEXT("Volatile Environment"), 0, NULL, REG_OPTION_VOLATILE, KEY_WRITE, NULL, &hKey, &dwDisp) != ERROR_SUCCESS) {
        goto Exit;
    }
    ...
    for (;;) {
       ...
       *lpTemp = TEXT('\0');
       ...
       RegSetValueEx (hKey, szValueName, 0, (cPercent >= 2) ? REG_EXPAND_SZ : REG_SZ, (LPBYTE) szValue, (len + 1) * sizeof(TCHAR));
       lpEnd++;
       if (!*lpEnd) {
           break;
       }
    ...
    if (g_IsTerminalServer && gpfnWinStationNameFromSessionId) {
        if (gpfnWinStationNameFromSessionId(SERVERNAME_CURRENT, g_SessionId, szValue)) {
            RegSetValueEx (hKey, SESSIONNAME_VARIABLE, 0, REG_SZ, (LPBYTE) szValue, (lstrlen(szValue) + 1) * sizeof(TCHAR));
        }
    }
    ...
    if ( hinstShell32 )
    {
        ...
        if ( pfnSHGetFolderPath && SUCCEEDED(pfnSHGetFolderPath(NULL, CSIDL_APPDATA | CSIDL_FLAG_DONT_VERIFY, pWS->hToken, 0, szValue)) )
        {
            RegSetValueEx (hKey, APPDATA_VARAIBLE, 0, REG_SZ, (LPBYTE)szValue, (lstrlen(szValue) + 1) * sizeof(TCHAR));
        }
