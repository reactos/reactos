    ...
    LPARAM lParam )    // additional information
{
    ...
    case WM_DESTROY:  // message: window being destroyed
        ...
        SendNotifyMessage( (HWND)-1, wMsgNddeAgntDying, (WPARAM) g_hNetDDEMainWnd, 0);
        PostQuitMessage(0);
    ...
    PCOPYDATASTRUCT pCopyDataStruct )
{
    ...
    case NDDEAGT_CMD_WINEXEC:
         ...
         * us he trusted it
         */
        if( CompareNddeModifyId( lpszShareName, &pNddeAgtCmd->qwModifyId[0], &dwOptions ) )
        {
            /*
    ...
    LPDWORD lpdwId)
{
    ...
    nRet = NDdeShareGetInfo(lpszServer, lpszShareName, 2, (LPBYTE)NULL, 0, &avail, &items);
    if (nRet == NDDE_BUF_TOO_SMALL) {
        ...
        if (lpDdeI == NULL) {
            ...
            items = 0;
            nRet = NDdeShareGetInfo(lpszServer, lpszShareName, 2, (LPBYTE)lpDdeI, avail, &avail, &items);
...
BOOL CompareModifyIds(LPTSTR lpszShareName)
{
    DWORD       dwIdNdde[2];
    DWORD       dwIdTrusted[2];
    DWORD       dwOptions;
    ...
    RetCode = NDdeGetTrustedShare( lpszServer,lpszShareName, &dwOptions, &dwIdTrusted[0], &dwIdTrusted[1] );
...
VOID CleanupTrustedShares(void)
{
    ...
    RetCode = NDdeTrustedShareEnum ( lpszServer, 0, lpBuf, avail, &entries, &avail );
    if (RetCode != NDDE_NO_ERROR) {
        NDDETRACE(("NetddeAgnt: Unable to enumerate trusted shares: %d\n", RetCode));
        LocalFree(lpBuf);
        return;
    }
    for ( lpShareName = (LPTSTR)lpBuf; *lpShareName; lpShareName += lstrlen(lpShareName) + 1 ) {
    ...
    LPDWORD     lpdwOptions )
{
    ...
    RetCode = NDdeGetTrustedShare(lpszServer, lpszShareName, lpdwOptions, &dwIdTrusted[0], &dwIdTrusted[1]);
    if (RetCode != NDDE_NO_ERROR) {
        NDDETRACE(("NetddeAgnt: Unable to access trusted share \"%s\" info: %d\n", lpszShareName, RetCode));
        return FALSE;
    }

    NDDETRACE(("NetddeAgnt: CompareNddeModifyId() returns: NddeId: %08X%08X, TrustedId: %08X%08X\n", lpdwIdNdde[0], lpdwIdNdde[1], dwIdTrusted[0], dwIdTrusted[1]));

    if( (lpdwIdNdde[0] == dwIdTrusted[0]) && (lpdwIdNdde[1] == dwIdTrusted[1]) )  {
            bMatch = TRUE;
    }
...
//              FALSE if not
{
    ...
    hAppDesktopThread = CreateThread(NULL, 0, ApplicationDesktopThread, (LPVOID)pTerm, 0, &dwThreadId);
