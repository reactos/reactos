...
HANDLE ExecLogoffThread(PTERMINAL pTerm, DWORD     Flags);
DWORD LogoffThreadProc(LPVOID Parameter);
DWORD KillComProcesses(LPVOID Parameter);
INT_PTR WINAPI ShutdownWaitDlgProc(HWND    hDlg, UINT    message, WPARAM  wParam, LPARAM  lParam);
BOOL WINAPI ShutdownDlgProc(HWND    hDlg, UINT    message, WPARAM  wParam, LPARAM  lParam);
BOOL DeleteNetworkConnections(PTERMINAL pTerm);
BOOL DeleteRasConnections(PTERMINAL pTerm);
INT_PTR WINAPI DeleteNetConnectionsDlgProc(HWND    hDlg, UINT    message, WPARAM  wParam, LPARAM  lParam);
...
*/
int InitiateLogoff(PTERMINAL pTerm, LONG      Flags)
{
    BOOL IgnoreResult;
    HANDLE ThreadHandle;
    HANDLE Handle;
    ...
    // If this is a shutdown operation, call ExitWindowsEx from another thread.
    if (Flags & (EWX_SHUTDOWN | EWX_REBOOT | EWX_POWEROFF)) {
        ...
        // We don't want to start another thread to perform logoffs in case the system is out of memory and unable to create any more threads.
        ...
        if (Handle == NULL) {
            ...
            // Let the thread run
            if ((pTerm->UserLoggedOn) && (pTerm->LastGinaRet != WLX_SAS_ACTION_FORCE_LOGOFF) )
            {
...
HANDLE ExecLogoffThread(PTERMINAL pTerm, DWORD Flags)
{
    ...
    if ( Flags & EWX_SYSTEM_CALLER )
    {
        ...
        ThreadHandle = CreateThread(NULL, 0, LogoffThreadProc, &Startup, 0, &ThreadId );
    }
    else
    {
        DebugLog(( DEB_TRACE, "Starting user thread for Logoff, flags = %x\n", Flags ));
        ThreadHandle = ExecUserThread(pTerm, LogoffThreadProc, &Startup, 0,          // Thread creation flags
                            &ThreadId);
    }
...
DWORD LogoffThreadProc(LPVOID Parameter)
{
    ...
    // If this logoff is a result of the InitiateSystemShutdown API, put up a dialog warning the user.
    ...
    if ( Result ) {
        ...
        if (LogoffFlags & (EWX_SHUTDOWN | EWX_REBOOT | EWX_POWEROFF))
        {
            if ( LogoffFlags & EWX_WINLOGON_API_SHUTDOWN )
            {
                // Turn off the flags for this call.
                // They are already in the other bits, so it will come through correctly.
            }
            else
            {
        ...
        if (Result) {
            ...
            {
                // If another thread is doing an ExitWindows, we would corrupt the flags.
                // This can happen if (a) a remote shutdown is happening during a logoff, (b) a system process has died, (c) someone uses c-a-d and the API at the same time to shutdown.
                DebugLog(( DEB_TRACE, "Spinning while exit windows in progress\n" ));
                Sleep( 1000 );
            }

            LogoffLockTest();// Check to see if the logoff processing is going on in a different thread

            DebugLog((DEB_TRACE, "Calling ExitWindowsEx(%#x, 0)\n", LogoffFlags));

            ExitWindowsInProgress = TRUE ;// Set global flag indicating an ExitWindows is in progress.
...
VOID RebootMachine(PTERMINAL pTerm)
/*
...
VOID PowerdownMachine(PTERMINAL pTerm)
/*
...
BOOL ShutdownMachine(PTERMINAL pTerm, int Flags)
{
    int Result;
    ...
    FoundDialogHandle = FindResource(NULL, (LPTSTR) MAKEINTRESOURCE(IDD_SHUTDOWN), (LPTSTR) MAKEINTRESOURCE(RT_DIALOG));
    ...
    if (g_IsTerminalServer) {
        gpfnWinStationShutdownSystem( SERVERNAME_CURRENT, WSD_LOGOFF );// Shutdown the WinStations
    }

    (void) pTerm->Gina.pWlxShutdown(pTerm->Gina.pGinaContext, Flags);

    // If we haven't shut down already (via the Remote shutdown path), then we  start it here, and wait for it to complete.
    // Otherwise, skip straight down to the cool stuff.
    if (pTerm->WinlogonState != Winsta_Shutdown)
    {
        // Call windows to do the windows part of shutdown
        // We make this a force operation so it is guaranteed to work and can not be interrupted.
        ...
        Result = WlxDialogBoxParam( pTerm, g_hInstance, (LPTSTR)IDD_SHUTDOWN_WAIT, NULL, ShutdownWaitDlgProc, (LPARAM)pTerm);
    }
    else
    {
    ...
    // Enable the shutdown privilege
    // This should always succeed - we are either system or a user who successfully passed the privilege check in ExitWindowsEx.
...
*/
BOOL Logoff(PTERMINAL pTerm, int LoggedOnResult)
{
    NTSTATUS Status;
    LUID luidNone = { 0, 0 };
    PWINDOWSTATION pWS = pTerm->pWinStaWinlogon;
    ...
    if (!g_IsTerminalServer || g_Console) {
        SendMessage(g_hwndAppDesktopThread, WM_NOTIFY, (WPARAM)g_hwndAppDesktopThread, (LPARAM)g_hwndAppDesktopThread);
    }
#endif
    ...
    try
    {
        ...
        // otherwise we end up playing the default rather than the user specified sound.
...
DWORD KillComProcesses(PVOID Parameter)
{
    PTERMINAL pTerm = (PTERMINAL) Parameter ;

    DebugLog(( DEB_TRACE, "ExitWindowsEx called to shut down COM processes\n" ));

    ExitWindowsEx(EWX_FORCE | EWX_LOGOFF | EWX_NONOTIFY | EWX_WINLOGON_CALLER, 0 );
    return 0 ;
}
...
BOOL DeleteNetworkConnections(PTERMINAL    pTerm)
/*
* PURPOSE:  Calls WNetNukeConnections in the client context to delete any connections they may have had.
* RETURNS:  TRUE on success, FALSE on failure
* HISTORY:
...
*/
{
    ...
    lpfnWNetOpenEnum = (PWNETOPENENUM) GetProcAddress(hMPR, (LPSTR)szWNetOpenEnum);
    lpfnWNetEnumResource = (PWNETENUMRESOURCE) GetProcAddress(hMPR, (LPSTR)szWNetEnumResource);
    lpfnWNetCloseEnum = (PWNETCLOSEENUM) GetProcAddress(hMPR, (LPSTR)szWNetCloseEnum);
    lpfnWNetNukeConn = (PWNETNUKECONN) GetProcAddress(hMPR, (LPSTR)szWNetNukeConn);

    // Check for NULL return values
    if ( !lpfnWNetOpenEnum || !lpfnWNetEnumResource || !lpfnWNetCloseEnum || !lpfnWNetNukeConn ) {
    ...
    if ( (*lpfnWNetOpenEnum)(RESOURCE_CONNECTED, RESOURCETYPE_ANY, 0, NULL, &hEnum) == NO_ERROR) {
        if ((*lpfnWNetEnumResource)(hEnum, &dwNumEntries, &NetRes, &dwEntrySize) == ERROR_NO_MORE_ITEMS) {
...
BOOL DeleteRasConnections(PTERMINAL    pTerm)
{
    ...
    hService = OpenService(hServiceMgr, RASMAN_SERVICE_NAME, SERVICE_QUERY_STATUS);
    if (hService == NULL)
    {
    ...
    pRasEnumConnectionsW = (PRASENUMCONNECTIONSW) GetProcAddress(hRasApi, "RasEnumConnectionsW" );
    pRasHangUpW = (PRASHANGUPW) GetProcAddress( hRasApi, "RasHangUpW" );
    if ( (!pRasEnumConnectionsW) || (!pRasHangUpW) )
    {
        return( FALSE );
    }
