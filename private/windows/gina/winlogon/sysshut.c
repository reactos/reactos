...
//  Synopsis:   Loops getting the locallized name for system (NT_AUTHORITY\SYSTEM) from the LSA as soon as it initializes
//  History:    3-04-98   RichardW   Created
VOID ShutdownGetSystemName(PVOID Ignored, BOOLEAN Timeout)
{
    NTSTATUS Status ;
...
BOOL InitializeShutdownModule(VOID)
/*
Routine Description:
...
--*/
{
    ...
    ShutdownInProgress = FALSE;// Initialize global variables

    Status = RtlInitializeCriticalSection(&ShutdownCriticalSection);// Initialize critical section to protect globals
#if DBG
    if (!NT_SUCCESS(Status)) {
    ...
    ShutdownNameWait = SetTimerQueueTimer(NULL, ShutdownGetSystemName, NULL, 10 * 1000, INFINITE, FALSE );
    ...
    )
{
    ...
    if (Error == ERROR_SUCCESS) {
        ...
        Result = InitiateLogoff( pShutDownTerm, EWX_WINLOGON_API_SHUTDOWN | ExitWindowsFlags );
...
ULONG BaseInitiateShutdownEx(IN PREGISTRY_SERVER_NAME ServerName, IN PREG_UNICODE_STRING lpMessage OPTIONAL, IN DWORD dwTimeout, IN BOOLEAN bForceAppsClosed, IN BOOLEAN bRebootAfterShutdown, IN DWORD dwReason)
/*
Routine Description:
    Initiates the shutdown of this machine.
Arguments:
    ...
    bRebootAfterShutdown - TRUE if the system should reboot. FALSE if it should
              - be left in a shutdown state.
    dwReason    - Reason for initiating the shutdown.  This reason is logged in the eventlog #6006 event.
Return Value:
    Returns ERROR_SUCCESS (0) for success; error-code for failure.
--*/
{
    ...
    rc = RegCreateKeyExW (HKEY_LOCAL_MACHINE, REGSTR_PATH_RELIABILITY, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, NULL);
    if (rc == ERROR_SUCCESS) {
    ...
    return StartSystemShutdown( FALSE, (PUNICODE_STRING)lpMessage, dwTimeout, bForceAppsClosed, bRebootAfterShutdown );
    ...
    )
{
    ...
    // Get our name.  Note:  we will leak this memory, but we're shutting down, so who cares?
    if ( ShutdownSystemName )
    {
    ...
    {
        Status = LsaGetUserName(&Name, &Domain );
    }
...
--*/
{
    ...
    ExitWindowsFlags |= bRebootAfterShutdown ? (EWX_REBOOT | EWX_WINLOGON_OLD_REBOOT) : 0;
...
BOOLEAN ShutdownThread(PULONG Flags)
/*
Routine Description:
    Handles the display of a shutdown dialog and coordinating with the AbortShutdown API.
Arguments:
    None
Return Value:
...
--*/
{
    ...
    switch (message) {
    case WM_INITDIALOG:
        InsertClientId(hDlg, IDD_SYSTEM_MESSAGE, UserName, UserDomain);// Add the caller's id to the main message text
        SetDlgItemText(hDlg, IDD_MESSAGE, ShutdownMessage);// Setup the client's message
    ...
    case WLX_WM_SAS:
        ...
        if ((wParam == WLX_SAS_TYPE_SCRNSVR_TIMEOUT) && (message == WLX_WM_SAS)) {
           // Don't end the dialog if it's just a screen saver timeout
           return(TRUE);
        } else if ((wParam == WLX_SAS_TYPE_CTRL_ALT_DEL) && (message == WLX_WM_SAS)) {
...
BOOL UpdateTimeToShutdown(HWND    hDlg)
/*
Routine Description:
...
ULONG BaseAbortShutdown(IN PREGISTRY_SERVER_NAME ServerName)
/*
Routine Description:
...
DWORD TestClientPrivilege(VOID)
/*
Routine Description:
...
--*/
{
    ...
    return(ERROR_SUCCESS);// They passed muster
}


DWORD GetClientId(PTSTR *UserName, PTSTR *UserDomain)
/*
Routine Description:
    Gets the name and domain of the caller, allocates and returns pointers to the information.
    Note we have RPC impersonate the client to discover their ID.
Arguments:
    UserName - a pointer to a NULL terminated string containing the client's user name is returned here.
    DomainName - a pointer to a NULL terminated string containing the client's domain name is returned here.
    ...
    if (OpenThreadToken(GetCurrentThread(), TOKEN_QUERY, FALSE, &TokenHandle)) {
        ...
        if (!GetTokenInformation(TokenHandle, TokenUser,  (PVOID)NULL, 0, &cbNeeded)) {
            if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
                ...
                if (pUserToken != NULL) {
                    if (GetTokenInformation(TokenHandle, TokenUser,  pUserToken, cbNeeded, &cbNeeded)) {
                        // Convert User Sid to name/domain
                        cbName = 0;
                        cbDomain = 0;

                        if (!LookupAccountSid(NULL, pUserToken->User.Sid, NULL, &cbName, NULL, &cbDomain, &SidNameUse)) {
                            if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
                                ...
                                *UserName = (PTSTR)LocalAlloc(LPTR, cbName*sizeof(TCHAR));
                                if ((*UserDomain != NULL) && (*UserName != NULL)) {
                                    ReturnValue = LookupAccountSid(NULL, pUserToken->User.Sid, *UserName, &cbName, *UserDomain, &cbDomain, &SidNameUse);
                                }
                            }
                        }
                    }
...
VOID DeleteClientId(PTSTR UserName, PTSTR UserDomain)
/*
Routine Description:
...
BOOL InsertClientId(HWND hDlg, int ControlId, PTSTR UserName, PTSTR UserDomain)
/*
Routine Description:
    Takes the text from the specified dialog control, treats it as a printf formatting string and inserts the user name and domain as the first 2 string identifiers (%s)
Arguments:
    UserName - a pointer to the username returned by GetClientId.
    DomainName - a pointer to the domain name eturned by GetClientId
Return Value:
...
--*/
{
    ...
    SetDlgItemText(hDlg, ControlId, Buffer);// Replace the control text with our formatted result
