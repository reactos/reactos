...
typedef BOOL(*PFNRESTORENETCON)(HWND hwndParent, LPTSTR lpDevice);
...
TCHAR szAdminName[MAX_STRING_BYTES];// For migration
VOID StartAppDesktopThread(PTERMINAL pTerm);// application desktop thread declaration
...
typedef (WINAPI *PFNINITCOMMONCONTROLSEX)(LPINITCOMMONCONTROLSEX);


// Local Prototypes
int DoScreenSaver(PTERMINAL pTerm, BOOL WkstaLocked);
void WinsrvNotify(PTERMINAL pTerm, DWORD SasType);
BOOL LoggedonDlgInit(HWND hDlg);
int MultiUserLogonAttempt(PTERMINAL pTerm, PWLX_MPR_NOTIFY_INFO pMprInfo, HANDLE hToken);


VOID NetworkProviderTask(PVOID pv, BOOLEAN timeout)
{
    PTERMINAL Terminal;
    PWINDOWSTATION WS;
    ...
    if ((!IsLocked(Terminal->WinlogonState)) && (!Terminal->ScreenSaverActive) && (!Terminal->ShutdownStarted))
    {
        DebugLog((DEB_TRACE, "NetworkProviderTask:  Event set, calling gina\n"));

        if (Terminal->Gina.pWlxNetworkProviderLoad(Terminal->Gina.pGinaContext, &MprInfo))
        {
            DebugLog((DEB_TRACE, "NPTask:  Gina returned success, notifying providers\n"));

            MprLogonNotify(Terminal, NULL, MprInfo.pszUserName, MprInfo.pszDomain, MprInfo.pszPassword, MprInfo.pszOldPassword, &WS->LogonId, &LogonScripts);
...
//  History:    11-04-94   RichardW   Created
{
    ...
    Status = NtQueryInformationProcess(NtCurrentProcess(), ProcessQuotaLimits, &Quota, sizeof(QUOTA_LIMITS), NULL);
    if (NT_SUCCESS(Status))
    {
        Quota.MinimumWorkingSetSize = (SIZE_T)-1;
        Quota.MaximumWorkingSetSize = (SIZE_T)-1;

        NtSetInformationProcess(NtCurrentProcess(), ProcessQuotaLimits, &Quota, sizeof(QUOTA_LIMITS));
    }
}


DWORD PokeComCtl32(PVOID ignored)// Code from the shell guys to flush comctl32's notion of the atom table.
{
    HINSTANCE ComCtl;
...
BOOL InitializeGinaDll(PTERMINAL  pTerm)
...
//  History:    10-17-94   RichardW   Created
{
    ...
    // Perversely, this may not return.  The GINA may in fact call SASNotify immediately, so update the state before we go in:
    pTerm->WinlogonState = Winsta_NoOne;
    DebugLog((DEB_TRACE_STATE, "InitGina:  State is %d %s\n", Winsta_NoOne, GetState(Winsta_NoOne)));

    if (!pTerm->Gina.pWlxInitialize(pTerm->pWinStaWinlogon->lpWinstaName, pTerm, NULL, (PVOID)&WlxDispatchTable, &pTerm->Gina.pGinaContext))
    {
        ExitProcess(EXIT_GINA_INIT_ERROR);// If the GINA failed to init, we're dead.  bugcheck time:
    }
...
void SASRouter(PTERMINAL pTerm, DWORD SasType)
...
//  History:    8-24-94   RichardW   Created
{
    ...
    {
        if (IsDisplayState(pTerm->WinlogonState) || (pTerm->WinlogonState == Winsta_WaitForShutdown) || (pTerm->WinlogonState == Winsta_Locked))
        {
        ...
        if ((SasType == WLX_SAS_TYPE_SCRNSVR_TIMEOUT) || (SasType == WLX_SAS_TYPE_TIMEOUT))
        {
...
//              [SasType]  --
//  History:    10-17-94   RichardW   Created
void CADNotify(PTERMINAL pTerm, DWORD SasType)
{
...
PVOID CopyEnvironment(PVOID pEnv)
{
    ...
    if (VirtualQueryEx(GetCurrentProcess(), pEnv, &mbi, sizeof(mbi)))
    {
        pNew = VirtualAlloc(NULL, mbi.RegionSize, MEM_COMMIT, PAGE_READWRITE);
        if (pNew)
        {
...
//  History:    10-17-94   RichardW   Created
int LogonAttempt(PTERMINAL pTerm)
{
    DWORD               WlxResult;
    WLX_MPR_NOTIFY_INFO MprInfo;
    ...
    if (pWS->Domain) {
        ...
        lResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, LOCAL_USERS_KEY, 0, KEY_READ | KEY_WRITE, &hKey);
        if (lResult == ERROR_SUCCESS) {
            ...
            lResult = RegQueryValueEx(hKey, pWS->UserName, NULL, &dwType, (PBYTE)&dwVal, &dwSize);
            if (lResult == ERROR_SUCCESS) {
...
)
{
    ...
    if (PageFilePopup) {
        ...
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szMemMan, 0, KEY_READ, &hkeyMM) == ERROR_SUCCESS) {
            ...
            if (RegQueryValueEx(hkeyMM, szNoPageFile, NULL, &dwType, (LPBYTE)&dwTempFile, &cbTempFile) != ERROR_SUCCESS || dwType != REG_DWORD || cbTempFile != sizeof(dwTempFile)) {
                dwTempFile = 0;
            }
...
VOID SuspendComputer(BOOL bHibernate, BOOL bDisableWakeupEvents)
{
    ...
    hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SleepThread, (LPVOID)pStateInfo, 0, &dwID);
    if (hThread)
        CloseHandle(hThread);
    else
        LocalFree(pStateInfo);
...
//  Synopsis:   Blocks, waiting for the interactive user to do something, or a SAS to come in from the gina.
//  History:    10-17-94   RichardW   Created
int BlockWaitForUserAction(PTERMINAL pTerm)
{
    int res;
    ...
    SetActiveDesktop(pTerm, Desktop_Application);
    WlxSetTimeout(pTerm, TIMEOUT_NONE);
    res = WlxDialogBoxParam(pTerm, g_hInstance, MAKEINTRESOURCE(IDD_CONTROL), NULL, LoggedonDlgProc, (LPARAM)pTerm);
#if DBG
    if (res == -1)
    {
...
//  History:    10-17-94   RichardW   Created
{
    ...
    do {
        ...
        Result = CreateProcessW(NULL, InitialCommand, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi);
        if (!Result) {
    ...
    while (WlxResult == WLX_SAS_ACTION_NONE)
    {
        ...
        if (pTerm->WinlogonState == Winsta_NoOne)
        {
            ...
            // If we got a user logoff notify, that means that WE HAVE ALREADY SHUT DOWN.
            // A remote shutdown has taken place, and it has been started by sysshut.c.
            if (pTerm->SasType == WLX_SAS_TYPE_USER_LOGOFF)
            {
                // We are *done*
            ...
            if (pTerm->SasType == WLX_SAS_TYPE_SCRNSVR_TIMEOUT)
            {
                ...
                {
                    if ((ScreenSaverResult == 0) && (pTerm->SasType == WLX_SAS_TYPE_USER_LOGOFF))
                    {
                    ...
                    // Remember, we're in Winsta_NoOne_Display right now, so we reset this so that we'll drop back into this at the top of the loop.
                    DebugLog((DEB_TRACE_STATE, "Resetting to %s\n", GetState(Winsta_NoOne)));
                    pTerm->WinlogonState = Winsta_NoOne;
                    continue;
                }

                // If we got a user logoff notify, that means that WE HAVE ALREADY SHUT DOWN.  A remote shutdown has taken place, and it has been started by sysshut.c.
                if (pTerm->SasType == WLX_SAS_TYPE_USER_LOGOFF)
                {
                    // We are *done*
        ...
        {
            SuspendComputer((WlxResult == WLX_SAS_ACTION_SHUTDOWN_HIBERNATE) ? TRUE : FALSE, (WlxResult == WLX_SAS_ACTION_SHUTDOWN_SLEEP2) ? TRUE : FALSE);
...
void WinsrvNotify(PTERMINAL pTerm, DWORD SasType)
{
    ...
    if (((pTerm->LogoffFlags & (EWX_SYSTEM_CALLER)) == 0) && (pTerm->UserLoggedOn == FALSE))
    {
        DebugLog((DEB_TRACE, "Discarding notice from winsrv!\n"));
        return;
    }

    // If the caller isn't system, and the workstation is locked, and force isn't set, reject it:
    if (!(pTerm->LogoffFlags & EWX_WINLOGON_CALLER) && ((pTerm->WinlogonState == Winsta_Locked_Display) ||
        (pTerm->WinlogonState == Winsta_Locked) ||
                                                        (pTerm->WinlogonState == Winsta_Locked_SAS)) && !(pTerm->LogoffFlags & EWX_FORCE))
    {
        DebugLog((DEB_TRACE, "Discarding exit request 'cause we're locked\n"));
        return;
    }

    // If this indicates that winlogon initiated this message (by calling InitiateLogoff() somewhere else, or we're in a wait state, then pass the message along.  This is what will kill our LoggedOnDlg.
    if ((pTerm->LogoffFlags & EWX_WINLOGON_CALLER) || (pTerm->WinlogonState == Winsta_WaitForLogoff) || (pTerm->WinlogonState == Winsta_WaitForShutdown))
    {
        SASRouter(pTerm, SasType);
    ...
    HANDLE hToken)
{
    ...
    if (!g_Console) {
        ...
        if (gpfnWinStationQueryInformation(SERVERNAME_CURRENT,
            ...
            if (!ConfigData.User.fDisableEncryption) {
                gpfnWinStationSetInformation(SERVERNAME_CURRENT, LOGONID_CURRENT, WinStationEncryptionPerm, NULL, 0);
            }
            else {
                // Turn off encryption, we don't want the overhead
                gpfnWinStationSetInformation(SERVERNAME_CURRENT, LOGONID_CURRENT, WinStationEncryptionOff, NULL, 0);
            }
        }
    }
    ...
    if (CtxConnectSession(pTerm)) {
        ...
        // Tell terminal service that a new terminal user session has been created.
        if ((!g_Console) && (NULL != gpfnWinStationNotifyNewSession)) {
            if (!gpfnWinStationNotifyNewSession(SERVERNAME_CURRENT, LOGONID_CURRENT))
            {
                HandleFailedLogon(pTerm, NULL, (NTSTATUS)GetLastError(), 0, pMprInfo->pszUserName, pMprInfo->pszDomain);
                WlxResult = WLX_SAS_ACTION_LOGOFF;
            }
        }
    }
