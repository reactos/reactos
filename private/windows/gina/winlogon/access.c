...
// Spawn the correct help
            ...
            StartApplication(pAccessInfo->pTerm, szDesktop, pAccessInfo->pTerm->pWinStaWinlogon->UserProcessData.pEnvironment, buf);
            return TRUE;
            break;
       default:
            return FALSE;
...
DWORD WINAPI StickyKeysNotificationThread(LPVOID lpv) {
    ...
    if (iRet) {
        ...
        if (dwS != sticky.dwFlags) {
           ...
           SendMessageTimeout(HWND_BROADCAST, WM_SETTINGCHANGE, SPI_SETSTICKYKEYS, FALSE, SMTO_ABORTIFHUNG, 5000, NULL);
        }
...
BOOL IsNotifReq(ACCESSINFO* pAccessInfo, PWINDOWSTATION pWS)
{
    ...
    {
        if (ERROR_SUCCESS == RegOpenKeyEx(pWS->UserProcessData.hCurrentUser, NOTIF_KEY, 0, KEY_READ, &hKey) )
        {
