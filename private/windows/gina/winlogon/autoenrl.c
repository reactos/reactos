...
// Description: This function logs the result of a certificate test into the AE_CERT_TEST_ARRAY
void AELogTestResult(PAE_CERT_TEST_ARRAY *ppAEData, DWORD idTest, ...)
{
    va_list ArgList;
    va_start(ArgList, idTest);

    if ((*ppAEData == NULL) || ((*ppAEData)->cTests == (*ppAEData)->cMaxTests))
    {
        PAE_CERT_TEST_ARRAY pAENew = NULL;
        DWORD               cAENew = ((*ppAEData) ? (*ppAEData)->cMaxTests : 0) + AE_CERT_TEST_SIZE_INCREMENT;
        // We need to grow the array

        pAENew = LocalAlloc(LMEM_FIXED, sizeof(AE_CERT_TEST_ARRAY) + (cAENew - ANYSIZE_ARRAY)*sizeof(AE_CERT_TEST));
        if (pAENew == NULL)
        {
            return;
        }
...
// Description: This function registers an event in the event log of the local machine.
void LogAutoEnrollmentEvent(IN DWORD  dwEventId, IN HANDLE hToken, ...)
{
...
// Description: This function registers an event in the event log of the local machine.
void LogAutoEnrollmentError(IN HRESULT hr, IN DWORD   dwEventId, IN BOOL fMachineEnrollment, IN HANDLE hToken, IN LPWSTR wszCertType, IN LPWSTR wszCA)
{
    HKEY        hRegKey = 0;
    WCHAR       szMsg[512];
    HANDLE      hEventSource = 0;
...
// Description: This function loads the specified Auto Enrollment provider, and calls the entry point to that provider.
//              It then unloads the provider.
BOOL LoadAndCallEnrollmentProvider(IN BOOL fMachineEnrollment, IN PAUTO_ENROLL_INFO pEnrollmentInfo)
{
    ...
    if (NULL == (hAutoEnrollProv = LoadLibraryA(pEnrollmentInfo->pszAutoEnrollProvider)))
    {
        AE_DEBUG((AE_ERROR, L"Could not load auto-enrollment provider %ls\n\r", pEnrollmentInfo->pszAutoEnrollProvider));
        goto Ret;
    }

    if (NULL == (pEntryPoint = GetProcAddress(hAutoEnrollProv, "ProvAutoEnrollment")))
    {
    ...
    if (FALSE == pEntryPoint(fMachineEnrollment, pEnrollmentInfo))
    {
        ...
        LogAutoEnrollmentError(HRESULT_FROM_WIN32(GetLastError()), EVENT_AE_ENROLLMENT_FAILED, fMachineEnrollment, NULL, pEnrollmentInfo->pwszCertType, pEnrollmentInfo->pwszCAAuthority);
        goto Ret;
    }
...
// Description: This function initializes information needed to proceed with auto enrollment.
HRESULT InitInternalInfo(LDAP *pld, IN BOOL fMachineEnrollment, IN HANDLE hToken, OUT PINTERNAL_INFO pInternalInfo)
{
    ...
    if (NULL == (pInternalInfo->hRootStore = CertOpenStore(CERT_STORE_PROV_SYSTEM_W, 0, 0, dwOpenStoreFlags | CERT_STORE_READONLY_FLAG, L"ROOT")))
    {
    ...
    if (NULL == (pInternalInfo->hCAStore = CertOpenStore(CERT_STORE_PROV_SYSTEM_W, 0, 0, dwOpenStoreFlags | CERT_STORE_READONLY_FLAG, L"CA")))
    {
        hrLocal = HRESULT_FROM_WIN32(GetLastError());
    ...
    if (NULL == (pInternalInfo->hMYStore = CertOpenStore(CERT_STORE_PROV_SYSTEM_W, 0, 0, dwOpenStoreFlags, L"MY")))
    {
        hrLocal = HRESULT_FROM_WIN32(GetLastError());
    ...
    // Normalize the directory DN into a real BER encoded name
    pInternalInfo->blobDN.cbData = 0;
    CertStrToName(X509_ASN_ENCODING, pInternalInfo->wszDN, CERT_X500_NAME_STR, NULL, NULL, &pInternalInfo->blobDN.cbData, NULL);
    ...
    if (!CertStrToName(X509_ASN_ENCODING, pInternalInfo->wszDN, CERT_X500_NAME_STR, NULL, pInternalInfo->blobDN.pbData, &pInternalInfo->blobDN.cbData, NULL))
    {
        hrLocal = HRESULT_FROM_WIN32(GetLastError());
    ...
    if (fMachineEnrollment)
    {
        pInternalInfo->awszldap_UPN = g_pfnldap_get_values(pld, PrincipalAttributes, DS_ATTR_DNS_NAME);
        if ((pInternalInfo->awszldap_UPN) && (*pInternalInfo->awszldap_UPN))
        {
    ...
    {
        pInternalInfo->awszldap_UPN = g_pfnldap_get_values(pld, PrincipalAttributes, DS_ATTR_UPN);
        if ((pInternalInfo->awszldap_UPN == NULL) || (*pInternalInfo->awszldap_UPN == NULL))
        {
    ...
    if ((pInternalInfo->awszEmail) && (*pInternalInfo->awszEmail))
    {
        AE_DEBUG((AE_INFO, L"E-mail name %ls\n\r", *pInternalInfo->awszEmail));
    }
    // Build up the list of CA's
    // Build up the list of CA's
    hrNetwork = CAEnumFirstCA((LPCWSTR) pld, CA_FLAG_SCOPE_IS_LDAP_HANDLE | (fMachineEnrollment ? CA_FIND_LOCAL_SYSTEM : 0), &hCACurrent);
    if (hrNetwork != S_OK)
    {
        goto Ret;
    }
    ...
    for (iCAIndex = 0; iCAIndex < cCA; iCAIndex++)
    {
        ...
        {
            CAFreeCAProperty(pInternalInfo->acaList[pInternalInfo->ccaList].hCAInfo, pInternalInfo->acaList[iCAIndex].awszCertificateTemplates);
        }
        ...
        if ((hrNetwork != S_OK) || (hrLocal != S_OK))
        {
            break;
        }
        ...
        hrNetwork = CAGetCAProperty(hCACurrent, CA_PROP_NAME, &awszName);
        if (hrNetwork != S_OK)
        {
        ...
        CAFreeCAProperty(hCACurrent, awszName);
        hrNetwork = CAGetCAProperty(hCACurrent, CA_PROP_DNSNAME, &awszName);
        if (hrNetwork != S_OK)
        {
        ...
        CAFreeCAProperty(hCACurrent, awszName);
        hrNetwork = CAGetCAProperty(hCACurrent, CA_PROP_CERT_TYPES, &pInternalInfo->acaList[pInternalInfo->ccaList].awszCertificateTemplates);
        if (hrNetwork != S_OK)
        {
        ...
        if (!CertGetCertificateContextProperty(pCert, CERT_SHA1_HASH_PROP_ID, pInternalInfo->acaList[pInternalInfo->ccaList].CACertHash, &cbHash))
        {
            continue;
        }
        CertFreeCertificateContext(pCert);
        AE_DEBUG((AE_INFO, L"CA %ls\\%ls available\n\r", pInternalInfo->acaList[pInternalInfo->ccaList].wszName, pInternalInfo->acaList[pInternalInfo->ccaList].wszDNSName));
...
Ret:
    if (hrLocal != S_OK)
    {
        LogAutoEnrollmentError(hrLocal, EVENT_AE_LOCAL_CYCLE_INIT_FAILED, pInternalInfo->fMachineEnrollment, pInternalInfo->hToken, NULL, NULL);
    }
    if (hrNetwork != S_OK)
    {
        LogAutoEnrollmentError(hrNetwork, EVENT_AE_NETWORK_CYCLE_INIT_FAILED, pInternalInfo->fMachineEnrollment, pInternalInfo->hToken, NULL, NULL);
    }
...
// Description: This function frees and resources which were needed for auto enrollment.
void FreeInternalInfo(IN PINTERNAL_INFO pInternalInfo)
{
    DWORD i;
    ...
    if (pInternalInfo->acaList)
    {
        for (i = 0; i < pInternalInfo->ccaList; i++)
        {
            ...
            {
                CAFreeCAProperty(pInternalInfo->acaList[i].hCAInfo, pInternalInfo->acaList[i].awszCertificateTemplates);
            }
...
// Description: This function initializes information needed to proceed with auto enrollment.
BOOL InitInstance(IN PCCTL_CONTEXT pCTLContext, IN PINTERNAL_INFO pInternalInfo, OUT PAE_INSTANCE_INFO pInstance)
{
    FILETIME    ft;
    ...
    {
        pInstance->dwRandomIndex = ft.dwLowDateTime % pInternalInfo->ccaList;
    }
    else
    {
...
// Description: This function frees and resources which were needed for auto enrollment.
void FreeInstance(IN PAE_INSTANCE_INFO pInstance)
{
...
// Description: This function retrieves additional enrollment information needed to enroll for a cert.
BOOL SetEnrollmentCertType(IN PAE_INSTANCE_INFO pInstance, OUT PAUTO_ENROLL_INFO pEnrollmentInfo)
{
    BOOL    fRet = FALSE;
    ...
    pEnrollmentInfo->CertExtensions.cExtension = pInstance->pCertTypeExtensions->cExtension;
    pEnrollmentInfo->CertExtensions.rgExtension = pInstance->pCertTypeExtensions->rgExtension;

    // copy over the cert type name, this is freed by the FreeInternalInfo
    // function, but after being used in the EnrollmentInfo struct
    pEnrollmentInfo->pwszCertType = pInstance->pwszCertType;

    // The auto enrollment ID is used to uniquely tie an autoenrolled cert to it's autoenrollment object.
...
BOOL GetCertTypeInfo(IN OUT PAE_INSTANCE_INFO pInstance, IN LDAP * pld, OUT BOOL *pfPermissionToEnroll)
{
    HRESULT     hr = S_OK;
    ...
    *pfPermissionToEnroll = FALSE;
    ...
    if (S_OK != (hr = CAFindCertTypeByName(wszCertTypeName,
        ...
        CT_FLAG_SCOPE_IS_LDAP_HANDLE | (pInstance->pInternalInfo->fMachineEnrollment ? CT_ENUM_MACHINE_TYPES | CT_FIND_LOCAL_SYSTEM : CT_ENUM_USER_TYPES),
        &hCertType)))
    {
    ...
    if (S_OK != (hr = CAGetCertTypeProperty(hCertType, CERTTYPE_PROP_DN, &awszName)))
    {
    ...
    if (NULL == (pInstance->pwszCertType = (LPWSTR) AEAlloc((wcslen(*awszName) + 1)*sizeof(WCHAR))))
    {
        hr = E_OUTOFMEMORY;
        goto Ret;
    }
    ...
    if (NULL == (pInstance->pwszAEIdentifier = (LPWSTR) AEAlloc((wcslen((LPWSTR) pInstance->pCTLContext->pCtlInfo->ListIdentifier.pbData) + 1)*sizeof(WCHAR))))
    {
        hr = E_OUTOFMEMORY;
        goto Ret;
    }
    ...
    // get the extensions for the cert type
    if (S_OK != (hr = CAGetCertTypeExtensions(hCertType, &pInstance->pCertTypeExtensions)))
    {
    ...
    // get the extensions for the cert type
    if (S_OK != (hr = CAGetCertTypeFlags(hCertType, &pInstance->dwCertTypeFlags)))
    {
    ...
    if (S_OK != (hr = CAGetCertTypeExpiration(hCertType, NULL, (LPFILETIME) &pInstance->ftExpirationOffset)))
    {
...
Ret:
    if (hr != S_OK)
    {
        LogAutoEnrollmentError(hr, EVENT_UAE_UNKNOWN_CERT_TYPE, pInstance->pInternalInfo->fMachineEnrollment, pInstance->pInternalInfo->hToken, (LPWSTR) pInstance->pCTLContext->pCtlInfo->ListIdentifier.pbData, NULL);
    }
...
//              in a certificate contain the enhanced key usage extensions from the auto enrollment object (CTL),
    ...
    )
{
    ...
    if (pCertUsageExt)
    {
        if (!CryptDecodeObject(CRYPT_ASN_ENCODING, szOID_ENHANCED_KEY_USAGE, pCertUsageExt->Value.pbData, pCertUsageExt->Value.cbData, 0, NULL, &cbCertUsage))
        {
        ...
        if (!CryptDecodeObject(CRYPT_ASN_ENCODING, szOID_ENHANCED_KEY_USAGE, pCertUsageExt->Value.pbData, pCertUsageExt->Value.cbData, 0, pCertUsage, &cbCertUsage))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Ret;
        }
    ...
    if (pAEObjUsageExt)
    {
        if (!CryptDecodeObject(CRYPT_ASN_ENCODING, szOID_ENHANCED_KEY_USAGE, pAEObjUsageExt->Value.pbData, pAEObjUsageExt->Value.cbData, 0, NULL, &cbAEObjUsage))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Ret;
        }
        ...
        if (!CryptDecodeObject(CRYPT_ASN_ENCODING, szOID_ENHANCED_KEY_USAGE, pAEObjUsageExt->Value.pbData, pAEObjUsageExt->Value.cbData, 0, pAEObjUsage, &cbAEObjUsage))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Ret;
        }
    ...
    {
        // The template requires no usage extension, so because the cert has a usage extension, we fail the test.
        goto Failed;
    }

    // check if the number of usages is smaller in the cert then in the auto enrollment object
    if (pCertUsage->cUsageIdentifier < pAEObjUsage->cUsageIdentifier)
    {
        goto Failed;
    }

    // check if all the usages found in the auto enrollment object are in the cert
    for (i = 0; i < pAEObjUsage->cUsageIdentifier; i++)
    {
        for (j = 0; j < pCertUsage->cUsageIdentifier; j++)
        {
            if (0 == strcmp(pCertUsage->rgpszUsageIdentifier[j], pAEObjUsage->rgpszUsageIdentifier[i]))
            {
                break;
            }
        }
...
Failed:
    ...
    AELogTestResult(ppAEData, AE_TEST_EXTENSION_EKU, wszCertEKU, wszTemplateEKU);
    goto Ret;
}
...
//              in a certificate are a superset of the key usages from the auto enrollment object (CTL),
HRESULT CompareKeyUsageExtensions(IN PAE_INSTANCE_INFO pInstance, IN PCCERT_CONTEXT pCertContext, IN OUT PAE_CERT_TEST_ARRAY  *ppAEData)
{
    HRESULT             hr = S_OK;
    PCERT_EXTENSION     pCertUsageExt;
    PCERT_EXTENSION     pAEObjUsageExt;
    ...
    if (!CryptDecodeObject(X509_ASN_ENCODING, X509_KEY_USAGE, pCertUsageExt->Value.pbData, pCertUsageExt->Value.cbData, 0, (PVOID *) pCertUsage, &dwKeyUsage))
    {
        hr = HRESULT_FROM_WIN32(GetLastError());
        goto Ret;
    }
    ...
    // Decode the key usage into their basic bit's
    dwKeyUsage = MAX_KEY_USAGE_SIZE;
    if (!CryptDecodeObject(X509_ASN_ENCODING, X509_KEY_USAGE, pAEObjUsageExt->Value.pbData, pAEObjUsageExt->Value.cbData, 0, (PVOID *) pAEObjUsage, &dwKeyUsage))
    {
        hr = HRESULT_FROM_WIN32(GetLastError());
        goto Ret;
    }
    ...
    for (i = 0; i < pAEObjUsage->cbData; i++)
    {
        ...
        if ((pAEObjUsage->pbData[i] & bMask) != ((pAEObjUsage->pbData[i] & bMask)  & pCertUsage->pbData[i]))
        {
            goto Failed;
        }
    }
...
Failed:
    ...
    AELogTestResult(ppAEData, AE_TEST_EXTENSION_KU, wszCertKU, wszTemplateKU);
    goto Ret;
}


// Description: This function checks if the basic constraints
//              in a certificate are a superset of the basic constraints from the auto enrollment object (CTL),
HRESULT CompareBasicConstraints(IN PAE_INSTANCE_INFO pInstance, IN PCCERT_CONTEXT pCertContext, IN OUT PAE_CERT_TEST_ARRAY  *ppAEData)
{
    HRESULT                         hr = S_OK;
    ...
    if (pCertConstraints)
    {
        ...
        if (!CryptDecodeObject(CRYPT_ASN_ENCODING, szOID_BASIC_CONSTRAINTS2, pCertConstraints->Value.pbData, pCertConstraints->Value.cbData, 0, &CertConstraintInfo, &cb))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Ret;
        }
    ...
    if (pAEObjConstraints)
    {
        ...
        if (!CryptDecodeObject(CRYPT_ASN_ENCODING, szOID_BASIC_CONSTRAINTS2, pAEObjConstraints->Value.pbData, pAEObjConstraints->Value.cbData, 0, &AEObjConstraintInfo, &cb))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Ret;
        }
    ...
    {
        if (CertConstraintInfo.fPathLenConstraint != AEObjConstraintInfo.fPathLenConstraint)
        {
            goto Failed;
        }
        if (CertConstraintInfo.fPathLenConstraint)
        {
            if (CertConstraintInfo.dwPathLenConstraint > AEObjConstraintInfo.dwPathLenConstraint)
            {
                goto Failed;
            }
        }
...
Failed:
    ...
    AELogTestResult(ppAEData, AE_TEST_EXTENSION_BC, wszCertBC, wszTemplateBC);
    goto Ret;
}


// Name:    VerifyTemplateName
HRESULT VerifyTemplateName(IN PAE_INSTANCE_INFO pInstance, IN PCCERT_CONTEXT pCertContext, IN OUT PAE_CERT_TEST_ARRAY  *ppAEData)
{
    HRESULT hr = S_OK;
    BOOL   fMatch = FALSE;
    ...
    if (pInstance->dwCertTypeFlags & CT_FLAG_ADD_TEMPLATE_NAME)
    {
        ...
        if (!CryptDecodeObject(X509_ASN_ENCODING, X509_UNICODE_ANY_STRING, pCertType->Value.pbData, pCertType->Value.cbData, 0, pbName, &cbTemplateName))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
            goto Ret;
        }
...
// Description: This function checks if a the extensions in a certificate contain the appropriate extensions from the certificate template
    ...
    )
{
    ...
    if (S_OK != (hr = CompareEnhancedKeyUsageExtensions(pInstance, pCertContext, ppAEData)))
    {
        goto Ret;
    }

    // check key usages
    if (S_OK != (hr = CompareKeyUsageExtensions(pInstance, pCertContext, ppAEData)))
    {
        goto Ret;
    }

    // check basic constraints
    if (S_OK != (hr = CompareBasicConstraints(pInstance, pCertContext, ppAEData)))
    {
        goto Ret;
    }
    ...
    // check basic constraints
    if (S_OK != (hr = VerifyTemplateName(pInstance, pCertContext, ppAEData)))
    {
        goto Ret;
    }
...
// Description: Determine whether the acutal naming information for the user matches that in the certificate.
{
    HRESULT             hr = S_OK;
    ...
    for (iRDN = 0; iRDN < pInfo->cRDN; iRDN++)
    {
        for (iATTR = 0; iATTR < pInfo->rgRDN[iRDN].cRDNAttr; iATTR++)
        {
            ...
            cszRDNAttr = CertRDNValueToStr(pInfo->rgRDN[iRDN].rgRDNAttr[iATTR].dwValueType, &pInfo->rgRDN[iRDN].rgRDNAttr[iATTR].Value, NULL, 0);
            if (cszRDNAttr == 0)
            {
                continue;
            }
            ...
            cszRDNAttr = CertRDNValueToStr(pInfo->rgRDN[iRDN].rgRDNAttr[iATTR].dwValueType, &pInfo->rgRDN[iRDN].rgRDNAttr[iATTR].Value, wszRDNAttr, cszRDNAttr);
            if (cszRDNAttr == 0)
            {
            ...
            {
                // If there are published UPN's, then we should check against those for a match, otherwise, we check against the generated UPN
                if ((pInstance->pInternalInfo->awszldap_UPN != NULL) && (*pInstance->pInternalInfo->awszldap_UPN != NULL))
                {
            ...
            {
                // If there are published e-mails, then we should check against those for a match
                if ((pInstance->pInternalInfo->awszEmail != NULL) && (*pInstance->pInternalInfo->awszEmail != NULL))
                {
                ...
                {
                    // We have no e-mail name for this subject, yet there was one in the cert.
    ...
    for (iExtension = 0; iExtension < pCert->pCertInfo->cExtension; iExtension++)
    {
            ...
            (strcmp(pCert->pCertInfo->rgExtension[iExtension].pszObjId, szOID_SUBJECT_ALT_NAME2) == 0))
        {
            ...
            for (iAltName = 0; iAltName < pAltName->cAltEntry; iAltName++)
            {
                ...
                {
                    if (CertCompareCertificateName(pCert->dwCertEncodingType, &pInstance->pInternalInfo->blobDN, &pAltName->rgAltEntry[iAltName].DirectoryName))
                    {
                        fDNMatch = TRUE;
                    }
                }
                ...
                case CERT_ALT_NAME_OTHER_NAME:
                    if (strcmp(pAltName->rgAltEntry[iAltName].pOtherName->pszObjId, szOID_ALT_NAME_OBJECT_GUID) == 0)
                    {
                        if (pInstance->dwCertTypeFlags & CT_FLAG_ADD_OBJ_GUID)
                        {
                            // Object ID's should always be the same, so don't compare them for now.
                            fObjIDMatch = TRUE;
                        }
                        else
                        {
                    ...
                    else if (strcmp(pAltName->rgAltEntry[iAltName].pOtherName->pszObjId, szOID_NT_PRINCIPAL_NAME) == 0)
                    {
                            ...
                            &PrincipalNameBlobSize))
                        {
                            ...
                            if ((pInstance->pInternalInfo->awszldap_UPN != NULL) && (*pInstance->pInternalInfo->awszldap_UPN != NULL))
                            {
                                ...
                                while (*pwszCurrentName)
                                {
                                    if (_tcscmp(*pwszCurrentName, (LPWSTR) PrincipalNameBlob->Value.pbData) == 0)
                                    {
                            ...
                            {
                                if (_tcscmp(pInstance->pInternalInfo->wszConstructedUPN, (LPWSTR) PrincipalNameBlob->Value.pbData) == 0)
                                {
                                    fAltSubjectUPNMatch = TRUE;
                                }
                            }
    ...
    if (((pInstance->pInternalInfo->fMachineEnrollment) ? ((!fSubjectUPNMatch) || (!fDNSMatch)) : ((!fSubjectUPNMatch) && (!fAltSubjectUPNMatch))))
    {
        ...
        AELogTestResult(ppAEData, pInstance->pInternalInfo->fMachineEnrollment ? AE_TEST_NAME_SUBJECT_DNS : AE_TEST_NAME_UPN, wszUPN);
        AEFree(wszUPN);
        fDisplaySubjectName = TRUE;
        fDisplayAltSubjectName = TRUE;
    }

    if ((pInstance->dwCertTypeFlags & CT_FLAG_ADD_EMAIL) && ((!fSubjectEmailMatch) || (!fAltSubjectEmailMatch)))
    {
        // We didn't find an appropriate UPN in either the subject or alt subject
        DWORD cEmailChars = 0;
        ...
        {
            AELogTestResult(ppAEData, AE_TEST_NAME_SUBJECT_EMAIL, wszEmail);
        }
        else if (!fAltSubjectEmailMatch && fSubjectEmailMatch)
        {
            AELogTestResult(ppAEData, AE_TEST_NAME_ALT_SUBJECT_EMAIL, wszEmail);
        }
        else if ((!fAltSubjectEmailMatch) && (!fSubjectEmailMatch))
        {
            AELogTestResult(ppAEData, AE_TEST_NAME_BOTH_SUBJECT_EMAIL, wszEmail);
        }
    ...
    if ((pInstance->dwCertTypeFlags & CT_FLAG_ADD_DIRECTORY_PATH) && (!fDNMatch))
    {
        AELogTestResult(ppAEData, AE_TEST_NAME_DIRECTORY_NAME, pInstance->pInternalInfo->wszDN);
        fDisplayAltSubjectName = TRUE;
    }

    if (!fObjIDMatch)
    {
        AELogTestResult(ppAEData, (pInstance->dwCertTypeFlags & CT_FLAG_ADD_OBJ_GUID) ? AE_TEST_NAME_NO_OBJID : AE_TEST_NAME_OBJID);
        fDisplayAltSubjectName = TRUE;
    }
    ...
    if (fDisplaySubjectName)
    {
        ...
        cNameStr = CertNameToStr(X509_ASN_ENCODING, &pCert->pCertInfo->Subject, CERT_X500_NAME_STR, NULL, 0);
        if (cNameStr)
        {
            ...
            AELogTestResult(ppAEData, AT_TEST_SUBJECT_NAME, wszNameStr);
            AEFree(wszNameStr);
        }
    }
    ...
    if (fDisplayAltSubjectName)
    {
        ...
        for (iExtension = 0; iExtension < pCert->pCertInfo->cExtension; iExtension++)
        {
                ...
                (strcmp(pCert->pCertInfo->rgExtension[iExtension].pszObjId, szOID_SUBJECT_ALT_NAME2) == 0))
            {
                ...
                AELogTestResult(ppAEData, AT_TEST_ALT_SUBJECT_NAME, wszFormat);
                LocalFree(wszFormat);
            }
        }
    }
...
// Description: This function checks if if the certificate has expired, or has been revoked
{
    ...
    if (!CertGetCertificateChain(pInstance->pInternalInfo->fMachineEnrollment ? HCCE_LOCAL_MACHINE : HCCE_CURRENT_USER,
        ...
        NULL,
        &ChainParams,
        CERT_CHAIN_REVOCATION_CHECK_END_CERT | CERT_CHAIN_REVOCATION_CHECK_CHAIN,
        NULL,
        &pChainContext))
    {
        hr = HRESULT_FROM_WIN32(GetLastError());
    ...
    if ((S_OK == hrChainStatus) || (CRYPT_E_NO_REVOCATION_CHECK == hrChainStatus) || (CRYPT_E_REVOCATION_OFFLINE == hrChainStatus))
    {
        // The cert is still currently acceptable by trust standards, so we can renew it.
        if (NULL == (*ppAEData))
        {
            (*ppAEData) = (PAE_CERT_TEST_ARRAY) LocalAlloc(LMEM_FIXED, sizeof(AE_CERT_TEST_ARRAY) + (AE_CERT_TEST_SIZE_INCREMENT - ANYSIZE_ARRAY)*sizeof(AE_CERT_TEST));
            if ((*ppAEData) == NULL)
            {
                hr = E_OUTOFMEMORY;
                goto Ret;
    ...
    else
    {
        ...
        // we must re-enroll
        AELogTestResult(ppAEData, AE_TEST_CHAIN_FAIL, hrChainStatus, wszChainStatus);
        AEFree(wszChainStatus);
    }
    // Verify that the immediate CA of the cert exists in the
    // Autoenrollment Object.  Empty ctl's imply any CA is ok.
    if (pCTLInfo->cCTLEntry)
    {
        ...
        if (!CertGetCertificateContextProperty(pChainContext->rgpChain[0]->rgpElement[1]->pCertContext, CERT_SHA1_HASH_PROP_ID, pHash, &cbHash))
        {
            hr = HRESULT_FROM_WIN32(GetLastError());
    ...
    {
        // Nudge the evaluation of the cert chain by the expiration offset so we know if is expired by that time in the future.
        GetSystemTimeAsFileTime((LPFILETIME) &ftTime);
        // Build the certificate chain for trust
        // operations
        ChainParams.cbSize = sizeof(ChainParams);
        ...
        if (pInstance->ftExpirationOffset.QuadPart < 0)
        {
            ...
            ftHalfLife.QuadPart = (((LARGE_INTEGER *) &pCert->pCertInfo->NotAfter)->QuadPart - ((LARGE_INTEGER *) &pCert->pCertInfo->NotBefore)->QuadPart) / 2;
        ...
        if ((S_OK != hrChainStatus) && (CRYPT_E_NO_REVOCATION_CHECK != hrChainStatus) && (CRYPT_E_REVOCATION_OFFLINE != hrChainStatus))
        {
            // The cert has expired or has been revoked or something, we must re-enroll
...
HRESULT VerifyAutoenrolledCertificate(IN PAE_INSTANCE_INFO pInstance, IN PCCERT_CONTEXT pCert, IN OUT PAE_CERT_TEST_ARRAY  * ppAEData)
{
    HRESULT hr = S_OK;
...
//              member is valid to satisfy this auto-enrollment request, or if it should be renewed.
{
    HRESULT hr = S_OK;
    ...
    *pfNeedNewCert = TRUE;
    ...
    if ((pInstance == NULL) || (pInstance->pCTLContext == NULL) || (pInstance->pCTLContext->pCtlInfo == NULL))
    {
        goto Ret;
    }
    ...
    if ((pAEData) && (pAEData->cTests > 0))
    {
        ...
        if (CertGetCertificateContextProperty(pInstance->pOldCert, AE_CERT_TEST_ARRAY_PROPID, NULL, &cbTestArray))
        {
            aidTestArray = (DWORD *) AEAlloc(cbTestArray);
            CertGetCertificateContextProperty(pInstance->pOldCert, AE_CERT_TEST_ARRAY_PROPID, aidTestArray, &cbTestArray);
        ...
        if (fFailed)
        {
            ...
            LogAutoEnrollmentEvent(pInstance->fRenewalOK ? EVENT_OLD_CERT_VERIFY_RENEW_WARNING : EVENT_OLD_CERT_VERIFY_REENROLL_WARNING,
...
Ret:
    ...
    if (hr != S_OK)
    {
        LogAutoEnrollmentError(hr, EVENT_UAE_VERIFICATION_FAILURE, pInstance->pInternalInfo->fMachineEnrollment, pInstance->pInternalInfo->hToken, pInstance->pwszCertType, NULL);
    }
...
//              member is valid to satisfy this auto-enrollment request, or if it should be renewed.
{
    ...
    if ((pAEData) && (pAEData->cTests > 0))
    {
        ...
        LogAutoEnrollmentEvent(EVENT_ENROLLED_CERT_VERIFY_WARNING, pInstance->pInternalInfo->hToken, pInstance->pwszCertType, wszFailureMessage, NULL);
        AEFree(wszFailureMessage);
        // Report the event here
    }
...
BOOL FindExistingEnrolledCertificate(IN PAE_INSTANCE_INFO pInstance, OUT PCCERT_CONTEXT  *ppCert)
// Description: This function searches for an existing certificate enrolled with this auto-enrollment object.
{
    ...
    *ppCert = NULL;
    ...
    while (pCertContext = CertFindCertificateInStore(pInstance->pInternalInfo->hMYStore, X509_ASN_ENCODING, 0, CERT_FIND_PROPERTY, &dwEnrollPropId, pPrevContext))
    {
        ...
        // check if this is an auto enroll cert and if the cert type is correct

        cbCurrentId = cbEnrollmentId;
        if (!CertGetCertificateContextProperty(pCertContext, CERT_AUTO_ENROLL_PROP_ID, wszEnrollmentId, &cbCurrentId))
        {
            continue;
        }
...
void EnrollmentWithCTL(IN PAE_INSTANCE_INFO pInstance, IN LDAP *            pld)
...
//              different CAs until either an enrollment is successful or the list of CAs is exhausted.
{
    ...
    if (!GetCertTypeInfo(pInstance, pld, &fPermitted))
    {
        goto Ret;
    }
    if (!fPermitted)
    ...
    do
    {
        ...
        for (i = 0; i < pInstance->pInternalInfo->ccaList; i++)
        {
            ...
            if (pInstance->pCTLContext->pCtlInfo->cCTLEntry > 0)
            {
                for (iCTL = 0; iCTL < pInstance->pCTLContext->pCtlInfo->cCTLEntry; iCTL++)
                {
                    ...
                    if (memcmp(pEntry->SubjectIdentifier.pbData, pInstance->pInternalInfo->acaList[dwIndex].CACertHash, pEntry->SubjectIdentifier.cbData) == 0)
                    {
                        break;
                    }
                }
            ...
            fAnyAcceptableCAs = TRUE;// Yes, we may enroll at this CA!
            ...
            if (LoadAndCallEnrollmentProvider(pInstance->pInternalInfo->fMachineEnrollment, &EnrollmentInfo))
            {
                PCCERT_CONTEXT pNewCert = NULL;
                // Succeeded, verify the cert that was retrieved
        ...
        {
            // If we have a preexisting cert, then we may need to try twice, first to renew, and then to re-enroll
            if (pInstance->pOldCert)
            {
...
PCCERT_CONTEXT FindCertificateInOtherStore(IN HCERTSTORE hOtherStore, IN PCCERT_CONTEXT pCert)
{
    ...
    if (!CertGetCertificateContextProperty(pCert, CERT_SHA1_HASH_PROP_ID, rgbHash, &HashBlob.cbData) || SHA1_HASH_LENGTH != HashBlob.cbData)
        return NULL;
...
// enterprise root store, and moves them into the local machine root store.
{
    ...
    hRootStore = CertOpenStore(CERT_STORE_PROV_SYSTEM_REGISTRY_W, 0, 0, CERT_SYSTEM_STORE_LOCAL_MACHINE_ENTERPRISE, L"ROOT");
    if (hRootStore == NULL)
    {
        hr = HRESULT_FROM_WIN32(GetLastError());
        AE_DEBUG((AE_ERROR, L"Unable to open ROOT store (%lx)\n\r", hr));
    ...
    hEnterpriseRoots = CertOpenStore(CERT_STORE_PROV_LDAP, 0, 0, CERT_STORE_READONLY_FLAG, wszLdapRootStore);
    if (hEnterpriseRoots == NULL)
    {
        DWORD err = GetLastError();

        // BUGBUG: Checking against NO_SUCH_OBJECT is a workaround for the fact that the ldap provider returns LDAP errors
        if ((err == LDAP_NO_SUCH_OBJECT) || (err == ERROR_FILE_NOT_FOUND))
        {
    ...
    while (pContext = CertEnumCertificatesInStore(hEnterpriseRoots, pContext))
    {
        ...
        {
            CertAddCertificateContextToStore(hRootStore, pContext, CERT_STORE_ADD_ALWAYS, NULL);
        }
    }
...
error:
    if (hr != S_OK)
    {
        LogAutoEnrollmentError(hr, EVENT_UPDATE_ENTERPRISE_ROOT_FAILURE, TRUE, NULL, NULL, NULL);
    }
...
// NTAuth store, and moves them into the local machine NTAuth.
{
    ...
    // BUGBUG:  Make the enterprise root store later
    hAuthStore = CertOpenStore(CERT_STORE_PROV_SYSTEM_REGISTRY_W, 0, 0, CERT_SYSTEM_STORE_LOCAL_MACHINE_ENTERPRISE, L"NTAuth");
    if (hAuthStore == NULL)
    {
        hr = HRESULT_FROM_WIN32(GetLastError());
    ...
    hDSAuthRoots = CertOpenStore(CERT_STORE_PROV_LDAP, 0, 0, CERT_STORE_READONLY_FLAG, wszNTAuth);
    ...
    while (pContext = CertEnumCertificatesInStore(hDSAuthRoots, pContext))
    {
        ...
        {
            CertAddCertificateContextToStore(hAuthStore, pContext, CERT_STORE_ADD_ALWAYS, NULL);
        }
    }
...
error:
    if (hr != S_OK)
    {
        LogAutoEnrollmentError(hr, EVENT_UPDATE_NTAUTH_FAILURE, TRUE, NULL, NULL, NULL);
    }
...
DWORD WINAPI ProcessAutoEnrollment(BOOL   fMachineEnrollment, HANDLE hToken)
// Description: This function retrieves the appropriate auto enrollment
//              objects (CTLs) and then calls a function to proceed with auto enrollment for each of the objects.
{
    DWORD                       i;
    ...
    __try
    {
        ...
        if (!fMachineEnrollment)
        {
            if (hToken)
            {
                if (!ImpersonateLoggedOnUser(hToken))
                {
                    ...
                    LogAutoEnrollmentError(HRESULT_FROM_WIN32(dwRet), EVENT_AE_SECURITY_INIT_FAILED, fMachineEnrollment, hToken, NULL, NULL);
                    goto Ret;
                }
            }
        }
        ...
        // from the local machine store so that we don't try and enroll for certs which are meant to be for the machine
        if (!fMachineEnrollment)
        {
            ...
            if (!CertRegisterPhysicalStore(L"ACRS", CERT_SYSTEM_STORE_CURRENT_USER, CERT_PHYSICAL_STORE_LOCAL_MACHINE_NAME, &PhysicalStoreInfo, NULL))
            {
                dwRet = GetLastError();
                AE_DEBUG((AE_ERROR, L"Could not register ACRS store: (%lx)\n\r", dwRet));
                LogAutoEnrollmentError(HRESULT_FROM_WIN32(dwRet), EVENT_AE_LOCAL_CYCLE_INIT_FAILED, fMachineEnrollment, hToken, NULL, NULL);
                goto Ret;
            }
        }

        // open the ACRS store and fine the CTL based on the auto enrollment usage
        if (0 == (hACRSStore = CertOpenStore(CERT_STORE_PROV_SYSTEM_W, 0, 0, dwOpenStoreFlags, L"ACRS")))
        {
            dwRet = GetLastError();
        ...
        for (i = 0;; i++)
        {
            ...
            if (NULL == (pCTLContext = CertFindCTLInStore(hACRSStore, X509_ASN_ENCODING, CTL_FIND_SAME_USAGE_FLAG, CTL_FIND_USAGE, &CTLFindUsage, pPrevCTLContext)))
            {
            ...
            if (!fInitialized)
            {
                ...
                if (S_OK != (dwRet = InitInternalInfo(pld, fMachineEnrollment, hToken, &InternalInfo)))
                {
                    break;
                }
            ...
            if (!InitInstance(pPrevCTLContext, &InternalInfo, &Instance))
            {
                dwRet = E_FAIL;
                break;
            }
            // UNDONE - perform a WinVerifyTrust check on this auto enrollment object (CTL) to make sure it is trusted
...
//  Parameters: pInfo   - AutoEnrollment info
{
    ...
    __try
    {
        ...
        // Build a timer event to ping us in about 8 hours if we don't get notified.
        lTimeout = AE_DEFAULT_REFRESH_RATE;

        hCurrent = HKEY_LOCAL_MACHINE;// Query for the refresh timer value
        ...
        lTimeout = lTimeout * 60 * 60 * 1000;// Convert hours to milliseconds
...
HANDLE RegisterAutoEnrollmentProcessing(IN BOOL fMachineEnrollment, IN HANDLE hToken)
...
//  History:    01-11-98   jeffspel   Created
{
    ...
    __try
    {
        ...
        if ((g_hInstSecur32 == NULL) || (g_hInstWldap32 == NULL))
        {
            goto error;
        }
        ...
        {
            if (!DuplicateToken(hToken, SecurityImpersonation, &pThreadInfo->hToken))
            {
        ...
        if (!RegisterGPNotification(pThreadInfo->hNotifyEvent, pThreadInfo->fMachineEnrollment))
        {
        ...
        if (pThreadInfo->fMachineEnrollment)
        {
            wsprintf(szEventName, TEXT("AUTOENRL: machine refresh timer for %d:%d"), GetCurrentProcessId(), GetCurrentThreadId());
        }
        else
        {
            wsprintf(szEventName, TEXT("AUTOENRL: user refresh timer for %d:%d"), GetCurrentProcessId(), GetCurrentThreadId());
        }
        ...
        if (!RegisterWaitForSingleObject(&pThreadInfo->hNotifyWait, pThreadInfo->hNotifyEvent, AutoEnrollmentThread, (PVOID) pThreadInfo, INFINITE, 0))
        {
            AE_DEBUG((AE_ERROR, L"RegisterWait failed: (%lx)\n\r", GetLastError()));
            goto error;
        }

        if (!RegisterWaitForSingleObject(&pThreadInfo->hTimerWait, pThreadInfo->hTimer, AutoEnrollmentThread, (void*) pThreadInfo, INFINITE, 0))
        {
            AE_DEBUG((AE_ERROR, L"RegisterWait failed: (%lx)\n\r", GetLastError()));
            goto error;
        }

        // Seed the timer with about 5 minutes, so we can come back and run an auto-enroll later without blocking this thread.
        DueTime.QuadPart = Int32x32To64(-10000, (fMachineEnrollment ? MACHINE_AUTOENROLL_INITIAL_DELAY : USER_AUTOENROLL_INITIAL_DELAY) * 1000);
...
{
    // NOTE, auto-enrollment is registered by the GPO download startup code.
    ...
    g_pfnldap_first_entry = (PFNLDAP_FIRST_ENTRY) GetProcAddress(g_hInstWldap32, "ldap_first_entry");
...
void AEDebugLog(long Mask, LPCWSTR Format, ...)
{
    ...
    if (Mask & g_AutoenrollDebugLevel)
    {
        ...
        if (wvsprintf(&wszOutString[iOut], Format, ArgList) < 0)
        {
            ...
            // Less than zero indicates that the string would not fit into the buffer.  Output a special message indicating overflow.
            wcscpy(&wszOutString[(sizeof(wszOutString) - sizeof(wszOverFlow)) / sizeof(WCHAR)], wszOverFlow);
        }
        va_end(ArgList);
...
HRESULT myGetConfigDN(IN  LDAP *pld, OUT LPWSTR *pwszConfigDn)
{
    ...
    timeout.tv_sec = 300;
    timeout.tv_usec = 0;
    // Query for the ldap server oerational attributes to obtain the default naming context.
...
LPWSTR HelperExtensionToString(PCERT_EXTENSION Extension)
{
    ...
    CryptFormatObject(X509_ASN_ENCODING, 0, 0, NULL, Extension->pszObjId, Extension->Value.pbData, Extension->Value.cbData, NULL, &cbFormat);
    if (cbFormat)
    {
...
//              4/12/99     NishadM    Created
{
    ...
    // Get the Admin sid
    if (!AllocateAndInitializeSid(&authNT, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, &psidAdmin)) {
        goto Exit;
    }
    ...
    do {
        ...
        __try
        {
            ...
            dwErr = DsGetDcName(NULL,     // Delayload wrapped
                NULL, NULL, NULL, dwGetDCFlags, &pDomainInfo);
        }
        ...
        if ((pDomainInfo == NULL) || ((pDomainInfo->Flags && DS_GC_FLAG) == 0) || ((pDomainInfo->Flags && DS_DNS_CONTROLLER_FLAG) == 0) || (pDomainInfo->DomainControllerName == NULL))
        {
            if (!fForceRediscovery)
            {
