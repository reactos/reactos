...
NTSTATUS SleepSystem(IN PTERMINAL pTerm, IN HWND hWnd, IN PPOWERSTATEPARAMS pPSP);
INT_PTR WINAPI SleepDlgProc(HWND    hDlg, UINT    message, WPARAM  wParam, LPARAM  lParam);
...
BOOL SASInit(PTERMINAL pTerm)
{
    ...
    // Register this window with windows so we get notified for screen-saver startup and user log-off
    ...
    )
{
    ...
    if (wEventType & POWER_LEVEL_USER_NOTIFY_SOUND) {
        ...
        Status = RegOpenKeyEx(hCurrent, gaPowerEventSounds[wEventLevel], 0, KEY_READ, &hkeyMM);
        RegCloseKey( hCurrent );
        if (Status == ERROR_SUCCESS) {
            Status = RegQueryValueEx(hkeyMM, NULL, NULL, &dwType, (LPBYTE)wcSound, &cbData);
            if ((Status == ERROR_SUCCESS) && (dwType == REG_SZ) && (wcSound[(cbData/sizeof(WCHAR))-1] == '\0')) {
                __try {
                    b = PlaySound(wcSound, NULL, SND_FILENAME | SND_NODEFAULT | SND_ASYNC);
                } __except (EXCEPTION_EXECUTE_HANDLER) {
                }
            }
    ...
    if (wEventType & POWER_LEVEL_USER_NOTIFY_EXEC) {
        ...
        // OLE is delay loaded, so this try/except is to catch any delay load failures and drop the call
        __try {
    ...
    )
{
    ...
    SpawnProcess:
        ...
        if ( Job )
        {
            ...
            // Delete the job.
            // The job will self-destruct if it hasn't completed after 5 minutes.
    ...
    switch (message)
    {
        ...
        case WM_LOGONNOTIFY: // A private notification from Windows
            ...
            switch (wParam)
            {
                ...
                case LOGON_LOGOFF:
                    ...
                    if ( ExitWindowsInProgress && ( !( lParam & EWX_WINLOGON_CALLER ) ) )
                    {
                        break;
                    }
                ...
                case LOGON_INPUT_TIMEOUT:
                {
                    ...
                    if ( OpenHKeyCurrentUser( pTerm->pWinStaWinlogon ) )
                    {
                        ...
                        err = RegOpenKeyEx( pTerm->pWinStaWinlogon->UserProcessData.hCurrentUser, SCREENSAVER_KEY, 0, KEY_READ, &Desktop );
                        if ( err == 0 )
                        {
                            dwSize = sizeof( Value );
                            err = RegQueryValueExA(Desktop, SCREEN_SAVER_SECURE_KEY, 0, &dwType, Value, &dwSize );
                            if ( err == 0 )
                            {
                                bSecure = atoi( Value );
                            }
                ...
                case LOGON_RESTARTSHELL:
                    ...
                    if (!pTerm->UserLoggedOn  || pTerm->LogoffFlags    || (lParam == 1)            || (pTerm->TickCount > (DWORD)GetMessageTime())) {
                        break;
                    }
                ...
                case LOGON_POWERSTATE:
                    return SleepSystem(pTerm, hwnd, (PPOWERSTATEPARAMS)lParam);
                case SESSION_RECONNECTED:
                    if ( !g_Console && gpfnWinStationQueryInformation( SERVERNAME_CURRENT, LOGONID_CURRENT, WinStationInformation, &InfoData, sizeof(InfoData), &Length ) ) {
                        SetUserEnvironmentVariable(&pTerm->pWinStaWinlogon->UserProcessData.pEnvironment, WINSTATIONNAME_VARIABLE, InfoData.WinStationName, TRUE );
                    }
                    break;
                case LOGON_LOCKWORKSTATION:
                    if (pTerm->UserLoggedOn && pTerm->Gina.pWlxIsLockOk(pTerm->Gina.pGinaContext) && (!IsLocked(pTerm->WinlogonState))) {
                        SetActiveDesktop(pTerm, Desktop_Winlogon);
        ...
        case WM_TIMER:
            ...
            if (lResult == ERROR_SUCCESS) {
                ...
                RegQueryValueEx (hKey, TEXT("AutoRestartShell"), NULL, &dwType, (LPBYTE) &bRestart, &dwSize);
                RegCloseKey (hKey);
            }
            ...
            if (bRestart) {
                ...
                wsprintfW (szDesktop, L"%s\\%s", pTerm->pWinStaWinlogon->lpWinstaName, APPLICATION_DESKTOP_NAME);
...
/*++
Routine Description:
    This routine actually calls the kernel to invoke the sleeping state.
    This is also responsible for displaying the relevant sleep progress dialog and locking the desktop on return.
Arguments:
...
--*/
{
    ...
    EnablePrivilege(SE_SHUTDOWN_PRIVILEGE, TRUE);
    NtStatus = NtSetSystemPowerState (pPSP->SystemAction, pPSP->MinSystemState, pPSP->Flags);
    ...
    if ((NT_SUCCESS(NtStatus)) && (pPSP->Flags & POWER_ACTION_LOCK_CONSOLE) && (pTerm->UserLoggedOn) && (pTerm->Gina.pWlxIsLockOk(pTerm->Gina.pGinaContext)) && (!IsLocked(pTerm->WinlogonState))) {
        // This will ensure the display will be reenabled before the dialog is put up.
        ...
        // If we're going back to full screen mode, give the system a chance to stabilize first.
