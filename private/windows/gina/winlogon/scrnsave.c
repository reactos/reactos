    ...
    )
{
    ...
    if ( OpenHKeyCurrentUser( pWS ))
    {
        ...
        err = RegOpenKeyEx(pWS->UserProcessData.hCurrentUser, SCREENSAVER_KEY, 0, KEY_READ, &UserControl );
        if ( err == 0 )
        {
            dwSize = sizeof( Buffer );

            err = RegQueryValueExA(UserControl, SCREEN_SAVER_SECURE_KEY, NULL, &dwType, Buffer, &dwSize );
            if ( err == 0 )
            {
                if ( dwType == REG_SZ )
                {
            ...
            err = RegQueryValueExA(UserControl, SCREEN_SAVER_ENABLED_KEY, NULL, &dwType, Buffer, &dwSize );
            if ( err == 0 )
            {
                if ( dwType == REG_SZ )
                {
            ...
            err = RegQueryValueExW(UserControl, SCREEN_SAVER_FILENAME_KEY, NULL, &dwType, NULL, &dwSize );
            if ( err != ERROR_FILE_NOT_FOUND )
            {
                ...
                {
                    err = RegQueryValueExW(UserControl, SCREEN_SAVER_FILENAME_KEY, NULL, &dwType, (PUCHAR) ScreenSaverData->ScreenSaverName, &dwSize );
                    if ( err )
                    {
        ...
        if (RegOpenKeyEx (pWS->UserProcessData.hCurrentUser, SCREENSAVER_POLICY_KEY, 0, KEY_READ, &hKeyPolicy) == ERROR_SUCCESS)
        {
            DWORD   dwBuffer = 0;
            ...
            */
            ...
            err = RegQueryValueExW(hKeyPolicy, SCREEN_SAVER_FILENAME_KEY, NULL, &dwType, NULL, &dwSize );
            if ( err != ERROR_FILE_NOT_FOUND )
            {
                    if ( ScreenSaverData->ScreenSaverName )
                    {
                ...
                if ( ScreenSaverData->ScreenSaverName )
                {
                    err = RegQueryValueExW(hKeyPolicy, SCREEN_SAVER_FILENAME_KEY, NULL, &dwType, (PUCHAR) ScreenSaverData->ScreenSaverName, &dwSize );
                    if ( err )
                    {
                        LocalFree( ScreenSaverData->ScreenSaverName );
