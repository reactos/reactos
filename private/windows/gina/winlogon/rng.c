...
// This file exports four functions: InitializeRNG, ShutdownRNG, GenRandom, and GenerateRandomBits, which are used to generate random sequences of bytes.
...
VOID STGenerateRandomBits(PUCHAR pbBuffer, ULONG  dwLength)
{
...
static BOOL RandomFillBuffer(BYTE *pbBuffer, DWORD *pdwLength)
{
    ...
    if(g_dwRC4BytesUsed >= RC4_REKEY_PARAM) {
        ...
        // Mix all bits
        {
            ...
            A_SHAUpdate(&SHACtx, g_rgbStaticBits, A_SHA_DIGEST_LEN);// SHA the static bits
            A_SHAUpdate(&SHACtx, randContext.rgbBitBuffer, randContext.dwBitsFilled);// SHA the gathered bits
            A_SHAUpdate(&SHACtx, pbBuffer, *pdwLength);// SHA the user-supplied bits
            A_SHAFinal(&SHACtx, g_rgbStaticBits);// output back out to static bits
        }
