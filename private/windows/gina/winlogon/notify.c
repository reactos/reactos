...
VOID WlpDeleteNotificationObject(PWLP_NOTIFICATION_OBJECT Notify)
{
    if ( Notify->DllName )
    {
...
BOOL WlpSnapNotifyDll(PWLP_NOTIFICATION_OBJECT Notify, DWORD OperationMask)
{
    int err ;
    ...
    for ( Index = 0 ; Index < sizeof( WlpNotifyStrings ) / sizeof( PSTR ) ; Index++ )
    {
        dwSize = sizeof( FunctionName );

        err = RegQueryValueExA(Notify->Key, WlpNotifyStrings[ Index ], 0, &dwType, FunctionName, &dwSize );
        if ( (err == 0) && (dwType == REG_SZ) )
        {
            Notify->EntryPoints[ Index ] = (LPTHREAD_START_ROUTINE) GetProcAddress(Dll, FunctionName );
    ...
    err = RegQueryValueEx( Notify->Key, NOTIFY_ASYNCHRONOUS, 0, &dwType, (PUCHAR) &Value, &dwSize );
    ...
    err = RegQueryValueEx( Notify->Key, NOTIFY_IMPERSONATE, 0, &dwType, (PUCHAR) &Value, &dwSize );
    if ( (err == 0) && (dwType == REG_DWORD) )
    {
        if ( Value )
        {
    ...
    if ( (Type == 0) || ( ( Type & OperationMask) == 0 ) )
    {
        Ret = FALSE ;
        goto Snap_ErrorCleanup;
    }
...
//  History:    11-14-97   RichardW   Created
PWLP_NOTIFICATION_OBJECT WlpReadNotificationParams(HKEY NotifyKey, PWSTR DllName, BOOL SafeMode)
{
    PWLP_NOTIFICATION_OBJECT Notify ;
    HKEY hKey ;
    DWORD dwType ;
    ...
    Notify = (PWLP_NOTIFICATION_OBJECT) LocalAlloc( LMEM_FIXED | LMEM_ZEROINIT, sizeof( WLP_NOTIFICATION_OBJECT ) );
    if ( !Notify )
    {
        return NULL ;
    }

    err = RegOpenKeyEx( NotifyKey, DllName, 0, KEY_READ, &hKey );
    if ( err )
    {
        goto Read_Cleanup ;
    }
    ...
    if ( SafeMode )
    {
        IsSafe = 0 ;
        dwSize = sizeof( BOOL );
        err = RegQueryValueEx( hKey, NOTIFY_SAFEMODE, 0, &dwType, (LPBYTE) &IsSafe, &dwSize );
        if ( err )
        {
            IsSafe = FALSE ;
        }
    ...
    err = RegQueryValueEx( hKey, NOTIFY_DLLNAME, 0, &dwType, (LPBYTE) szTemp, &dwSize );
    if ( err )
    {
        goto Read_Cleanup ;
    }
    ...
    if ( SearchPath(NULL, szDllName, NULL, MAX_PATH, szTemp, &FileName ) == 0 )
    {
        goto Read_Cleanup ;
    }
    ...
    RegQueryValueEx( hKey, NOTIFY_MAXWAIT, 0, &dwType, (LPBYTE) &Notify->dwMaxWait, &dwSize );
...
PWLP_NOTIFICATION_OBJECT WlpAddSfcNotify(VOID)
{
    PWLP_NOTIFICATION_OBJECT Notify ;
    CHAR FunctionName[ MAX_PATH ];
    TCHAR szTemp [ MAX_PATH ];
    ...
    Notify->DllName = AllocAndDuplicateString( szTemp );
    Notify->Type = WLP_NOTIFY_LOGOFF | WLP_NOTIFY_STARTSHELL | WLP_NOTIFY_ASYNCHRONOUS ;
...
//  History:    11-14-97   RichardW   Created
BOOL WlpInitializeNotifyList(PTERMINAL pTerm)
{
    HKEY NotifyKey ;
    ...
    err = RegOpenKeyEx( HKEY_LOCAL_MACHINE, NOTIFY_KEY, 0, KEY_READ, &NotifyKey );
    ...
    while (RegEnumKeyEx ( NotifyKey, dwIndex, szDllName, &dwSize, NULL, NULL, NULL, &ftTime ) == ERROR_SUCCESS)
    {
        if ( _wcsicmp( szDllName, TEXT("sfc.dll") ) != 0 )
        {
            Notify = WlpReadNotificationParams(NotifyKey, szDllName, pTerm->SafeMode);
            if ( Notify )
            {
                InsertTailList( &NotifyList, &Notify->List );
            }
...
int WlpNotifyExceptionFilter(PEXCEPTION_POINTERS Exception, PWLP_NOTIFY_BOOTSTRAP Boot)
{
    ...
    if ( KernelDebuggerPresent )
    {
        KdPrint(("WINLOGON:  Notification DLL %ws hit exception %x\n    while executing %s notify function\n",
                Boot->Notify->DllName, Exr->ExceptionCode, WlpNotifyStrings[ Boot->Operation ] ));
    }
...
//  History:    11-14-97   RichardW   Created
DWORD WlpExecuteNotify(PWLP_NOTIFY_BOOTSTRAP Boot)
{
...
//  History:    11-14-97   RichardW   Created
VOID WlWalkNotifyList(PTERMINAL Terminal, DWORD Operation)
{
    PLIST_ENTRY Scan ;
    PWLP_NOTIFICATION_OBJECT Notify;
    ...
    while ( Scan != &NotifyList )
    {
        ...
        if ( Notify->Type & OperationMask )
        {
            ...
            if ( (Notify->Dll == NULL) && ((Notify->Type & WLP_NOTIFY_INTERNAL) == 0) )
            {
            ...
            Boot = (PWLP_NOTIFY_BOOTSTRAP) LocalAlloc( LMEM_FIXED, sizeof( WLP_NOTIFY_BOOTSTRAP ) );
            if ( Boot == NULL )
            {
                Scan = Scan->Flink ;
                continue;
            ...
            ThreadHandle = CreateThread( 0, 0, WlpExecuteNotify, Boot, 0, &tid );
            if ( ThreadHandle )
            {
                if ( !bAsync )
                {
