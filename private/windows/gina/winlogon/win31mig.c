...
BOOL IsWin9xUpgrade (VOID)
{
    ...
    rc = RegOpenKeyEx (HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_READ, &Key);
    if (rc != ERROR_SUCCESS) {
    ...
    rc = RegQueryValueEx (Key, TEXT("win9xupg"), NULL, NULL, (PBYTE) &ValueData, &Size);
...
BOOL Windows31Migration(PTERMINAL pTerm)
{
                ...
                if (pDialogInfo->Win31MigrationFlags != 0) {
                    ...
                    if (pDialogInfo->Win31MigrationFlags & WIN31_MIGRATE_GROUPS) {
                        ...
                        if (OpenHKeyCurrentUser(pDialogInfo->pTerm->pWinStaWinlogon)) {
                            if (RegCreateKeyEx (pDialogInfo->pTerm->pWinStaWinlogon->UserProcessData.hCurrentUser, WINLOGON_KEY, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDisp) == ERROR_SUCCESS) {
                                bRunGrpConv = TRUE;
                                RegSetValueEx (hKey, TEXT("RunGrpConv"), 0, REG_DWORD, (LPBYTE) &bRunGrpConv, sizeof(bRunGrpConv));
                                RegCloseKey (hKey);
                            }
