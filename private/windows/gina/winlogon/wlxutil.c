...
PWindowMapper LocateTopMappedWindow(PTERMINAL pTerm)
{
    int i;
...
PWindowMapper AllocWindowMapper(PTERMINAL pTerm)
{
    int i;
    PWindowMapper   pMap;
...
PWindowMapper LocateWindowMapper(HWND hWnd, PTERMINAL pTerm)
{
    int i;
...
void FreeWindowMapper(PWindowMapper  pMap, PTERMINAL pTerm)
{
    pMap->hWnd = NULL;
    pMap->DlgProc = NULL;
...
HWND LocateTopWindow(PTERMINAL pTerm)
{
    PWindowMapper   pMap;
...
BOOL SetMapperFlag(HWND hWnd, DWORD Flag, PTERMINAL pTerm)
{
    PWindowMapper   pMap;
...
BOOL QueueSasEvent(DWORD dwSasType, PTERMINAL pTerm)
{
...
BOOL FetchPendingSas(PDWORD pSasType, PTERMINAL pTerm)
{
...
VOID EnableSasMessages(HWND  hWnd, PTERMINAL pTerm)
{
    DWORD   SasType;
    SasMessages = TRUE;
...
INT_PTR CALLBACK RootDlgProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    PWindowMapper   pMap;
    ...
    if (Message == WLX_WM_SAS && ((pMap->fMapper & MAPPERFLAG_WINLOGON) == 0))
    {
...
VOID ChangeStateForSAS(PTERMINAL  pTerm)
{
#if DBG
...
#if DBG
    DebugLog((DEB_TRACE, "ChangeStateForSAS: Went from %d (%s) to %d (%s)\n", State, GetState(State), pTerm->WinlogonState, GetState(pTerm->WinlogonState)));
#endif
}


BOOL SendSasToTopWindow(PTERMINAL pTerm, DWORD SasType)
{
    PWindowMapper   pMap;
#if DBG
    WCHAR           WindowName[32];
    ...
    if (pTerm->cActiveWindow)
    {
        ...
        if ( ( SasType > WLX_SAS_TYPE_MAX_MSFT_VALUE) || ( SasType == WLX_SAS_TYPE_SCRNSVR_TIMEOUT) ||  ( SasType == WLX_SAS_TYPE_TIMEOUT) )
        {
            // Either a timeout (which we have to forward), or a private which we have to forward.  Kill any message boxes
    ...
    {
        // No windows active.  See if there is a single message box going currently, like maybe a legal notice box.  If there is, kill it for any SAS
...
VOID DestroyMprInfo(PWLX_MPR_NOTIFY_INFO pMprInfo)
{
...
//  History:    10-14-97   RichardW   Created
VOID WINAPI WlxSasNotify(HANDLE  hWlx, DWORD   SasType)
{
    PTERMINAL    pTerm;
...
BOOL WINAPI WlxSetTimeout(HANDLE  hWlx, DWORD   Timeout)
{
    ...
    if (pTerm = VerifyHandle(hWlx))
    {
        if ((pTerm->WinlogonState == Winsta_NoOne_Display) || (pTerm->WinlogonState == Winsta_Locked_Display))
        {
            if (Timeout)
            {
...
int WINAPI WlxMessageBox(HANDLE hWlx, HWND hWnd, LPWSTR lpsz1, LPWSTR lpsz2, UINT fmb)
{
    ...
    ret = TimeoutMessageBoxlpstr(pTerm, hWnd, lpsz1, lpsz2, fmb, pTerm->Gina.cTimeout | TIMEOUT_SS_NOTIFY );
    pTerm->MessageBoxActive = FALSE ;
    return ret;
}


int WINAPI WlxDialogBox(HANDLE hWlx, HANDLE hInstance, LPWSTR lpsz1, HWND hWnd, DLGPROC dlgproc)
{
    return(WlxDialogBoxParam(hWlx, hInstance, lpsz1, hWnd, dlgproc, 0));
}


int WINAPI WlxDialogBoxIndirect(HANDLE hWlx, HANDLE hInstance, LPCDLGTEMPLATE lpTemplate, HWND hWnd, DLGPROC dlgproc)
{
    return(WlxDialogBoxIndirectParam(hWlx, hInstance, lpTemplate, hWnd, dlgproc, 0));
}


int WINAPI WlxDialogBoxParam(HANDLE hWlx, HANDLE hInstance, LPWSTR lpsz1, HWND hWnd, DLGPROC dlgproc, LPARAM lParam)
{
    PWindowMapper   pMap;
    PTERMINAL       pTerm;
    int res;
    ...
    res = TimeoutDialogBoxParam(pTerm, hInstance, lpsz1, hWnd, RootDlgProc, (LPARAM) &MapTerminal, pTerm->Gina.cTimeout | TIMEOUT_SS_NOTIFY);
    FreeWindowMapper(pMap, pTerm);
    return(res);
}


int WINAPI WlxDialogBoxIndirectParam(HANDLE hWlx, HANDLE hInstance, LPCDLGTEMPLATE lpTemplate, HWND hWnd, DLGPROC dlgproc, LPARAM lParam)
{
    PWindowMapper   pMap;
    int res;
    PTERMINAL        pTerm;
...
//  Synopsis:   Allows a GINA to notify all or one network provider of a password change.
//  History:    10-14-97   RichardW   Created
int WINAPI WlxChangePasswordNotifyEx(HANDLE hWlx, PWLX_MPR_NOTIFY_INFO pMprInfo, DWORD dwChangeInfo, PWSTR pszProvider, PVOID pvReserved)
{
    PTERMINAL    pTerm;
    int         Result;
    ...
    Result = MprChangePasswordNotify(pTerm, LocateTopWindow(pTerm), pszProvider, pMprInfo->pszUserName, pMprInfo->pszDomain, pMprInfo->pszPassword, pMprInfo->pszOldPassword, dwChangeInfo, FALSE);
...
BOOL WINAPI WlxSetReturnDesktop(HANDLE hWlx, PWLX_DESKTOP pDesktop)
{
    ...
    if ((pDesktop->Size != sizeof(WLX_DESKTOP)) || ((pDesktop->Flags & (WLX_DESKTOP_HANDLE | WLX_DESKTOP_NAME)) == 0) )
    {
...
BOOL WINAPI WlxCreateUserDesktop(HANDLE hWlx, HANDLE hToken, DWORD Flags, PWSTR pszDesktopName, PWLX_DESKTOP * ppDesktop)
{
    ...
    if (((Flags & (WLX_CREATE_INSTANCE_ONLY | WLX_CREATE_USER)) == 0 ) || ((Flags & (WLX_CREATE_INSTANCE_ONLY | WLX_CREATE_USER)) == (WLX_CREATE_INSTANCE_ONLY | WLX_CREATE_USER) ) )
    {
    ...
    {
        Status = NtQueryInformationToken(   hToken, TokenGroups, NULL, 0, &Needed );
        ...
        Status = NtQueryInformationToken(hToken, TokenGroups, pGroups, Needed, &Needed );
        if ( !NT_SUCCESS( Status ) )
        {
            LocalFree( pGroups );
            SetLastError( RtlNtStatusToDosError( Status ) );
    ...
    {
        Status = NtQueryInformationToken(hToken, TokenUser, NULL, 0, &Needed );
        if ( Status != STATUS_BUFFER_TOO_SMALL )
        {
            SetLastError( RtlNtStatusToDosError( Status ) );
            return( FALSE );
        ...
        Status = NtQueryInformationToken(hToken, TokenUser, pUser, Needed, &Needed );
        if ( !NT_SUCCESS( Status ) )
        {
            LocalFree( pUser );
            SetLastError( RtlNtStatusToDosError( Status ) );
    ...
    pDesktop->hDesktop = CreateDesktopW (pszDesktopName, NULL, NULL, 0, MAXIMUM_ALLOWED, NULL);
    if ( !pDesktop->hDesktop )
    {
        goto CleanUp;
    }

    if (!SetUserDesktopSecurity(pDesktop->hDesktop, pSid, g_WinlogonSid ) )
    {
        goto CleanUp;
    }

    if (!AddUserToWinsta( pTerm->pWinStaWinlogon, pSid, hToken ) )
    {
        goto CleanUp;
    }
...
*/
BOOL WlxQueryClientCredentials(PWLX_CLIENT_CREDENTIALS_INFO_V1_0 pInfo)
{
    ULONG Length;
    ...
    if ( !gpfnWinStationQueryInformation( SERVERNAME_CURRENT, LOGONID_CURRENT, WinStationConfiguration, &ConfigData, sizeof(ConfigData), &Length ) ) {
        return( FALSE );
    }

    if ( ConfigData.User.UserName[0] || ConfigData.User.Domain[0] ) {
        ret = InitializeAutoLogonInfo(pInfo, ConfigData.User.UserName, ConfigData.User.Domain, ConfigData.User.Password, ConfigData.User.fPromptForPassword);
    }
...
*/
BOOL WlxQueryInetConnectorCredentials(PWLX_CLIENT_CREDENTIALS_INFO_V1_0 pInfo)
{
    ULONG Length;
    WINSTATIONCONFIG ConfigData;
    BOOL ret = FALSE;
    ...
    if ( !gpfnServerQueryInetConnectorInformation(SERVERNAME_CURRENT, &ConfigData, sizeof(ConfigData), &Length ) ) {
        return( FALSE );
    }
 ...
 *   This allows GINA's to optionally support a Disconnect... dialog item.
...
BOOL WlxDisconnect( void )
{
    ...
    rc = gpfnWinStationDisconnect(SERVERNAME_CURRENT, LOGONID_CURRENT, TRUE);
    return( rc );
}
...
VOID WINAPI WlxWin31Migrate(HANDLE  hWlx)
{
    ...
    if (pTerm = VerifyHandle(hWlx))
    {
        // If not logging in as Guest, System or Administrator then check for migration of Windows 3.1 configuration information.
        LoadString(NULL, IDS_ADMIN_ACCOUNT_NAME, szAdminName, sizeof(szAdminName));
        if (!IsUserAGuest(pTerm->pWinStaWinlogon) && _wcsicmp(pTerm->pWinStaWinlogon->UserName, szAdminName))
        {
...
*/
BOOL InitializeAutoLogonInfo(PWLX_CLIENT_CREDENTIALS_INFO_V1_0 pAutoLogon, LPTSTR pUserName, LPTSTR pDomain, LPTSTR pPassword, BOOL fPromptForPassword)
{
...
BOOL StartApplication(PTERMINAL pTerm, PWSTR pszDesktop, PVOID pEnvironment, PWSTR pszCmdLine)
{
    BOOL  bResult;
    ...
    bResult = pTerm->Gina.pWlxStartApplication(pTerm->Gina.pGinaContext, pszDesktop, pNewEnvironment, pszCmdLine);
...
VOID QueryVerboseStatus(VOID)
{
    ...
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        RegQueryValueEx (hKey, VERBOSE_STATUS, NULL, &dwType, (LPBYTE) &bVerbose, &dwSize);
        RegCloseKey (hKey);
    }
    ...
    g_bVerboseStatus = bVerbose;// Save the results
}
...
VOID StatusMessage(BOOL bVerbose, DWORD dwOptions, UINT idMsg,  ...)
{
    ...
    // If this is a verbose message and we are not in verbose mode, exit now
    ...
    g_pTerminals->Gina.pWlxDisplayStatusMessage(g_pTerminals->Gina.pGinaContext, hDesktop, dwOptions, NULL, lpMessage);
...
DWORD StatusMessage2(BOOL bVerbose, LPWSTR lpMessage)
{
    ...
    // Update the status message
    g_pTerminals->Gina.pWlxDisplayStatusMessage(g_pTerminals->Gina.pGinaContext, hDesktop, 0, NULL, lpMessage);
...
BOOL WaitForServiceToStart (LPTSTR lpServiceName, DWORD dwMaxWait)
{
    ...
    hService = OpenService(hScManager, lpServiceName, SERVICE_QUERY_CONFIG | SERVICE_QUERY_STATUS);
    if (!hService) {
        goto Exit;
    }
    ...
    // Loop until the service starts or we think it never will start or we've exceeded our maximum time delay.
...
BOOL WaitForMUP (DWORD dwMaxWait)
{
    ...
    do {
        hEvent = OpenEvent (SYNCHRONIZE, FALSE, TEXT("wkssvc:  MUP finished initializing event"));
        if (hEvent) {
            break;
        }
    ...
    bResult = (WaitForSingleObject (hEvent, dwMaxWait) == WAIT_OBJECT_0);// Wait for the event to be signalled
...
VOID WaitForServices(PTERMINAL pTerm)
{
    ...
    if ( pTerm->SafeMode )
    {
        // In safe mode, no sense waiting for services that are disabled to start.
        return;
    }
    hDsReindexEvent = OpenEvent( SYNCHRONIZE, FALSE, L"NTDS.IndexRecreateEvent" );
    if ( hDsReindexEvent ) {
...
DWORD WINAPI WlxQueryTerminalServicesData(HANDLE hWlx, PWLX_TERMINAL_SERVICES_DATA pTSData, WCHAR *UserName, WCHAR *Domain)
{
    ...
    if (pTerm != NULL) {
        ...
        if (rc == STATUS_SUCCESS) {
            ...
            wcscpy(pTSData->ProfilePath, pTerm->MuGlobals.UserConfig.WFProfilePath);
            wcscpy(pTSData->HomeDir, pTerm->MuGlobals.UserConfig.WFHomeDir);
            wcscpy(pTSData->HomeDirDrive, pTerm->MuGlobals.UserConfig.WFHomeDirDrive);
        }
    } else {
...
DWORD WINAPI QueryTerminalServicesDataWorker(PTERMINAL pTerm, WCHAR *UserName, WCHAR *Domain)
{
    ...
    if (_wcsicmp(ComputerName, Domain)) {
        __try {
            // Delay load netapi32.dll. We need a try except around this call in case the LoadLibrary/GetProcAddress fails during runtime
    ...
    if (dllHandle != NULL) {
        pfnRegUserConfigQuery = (PREGUSERCONFIGQUERY)GetProcAddress(dllHandle, "RegUserConfigQuery");
        pfnRegDefaultUserConfigQuery = (PREGDEFAULTUSERCONFIGQUERY) GetProcAddress(dllHandle, "RegDefaultUserConfigQueryW");
        if (pfnRegUserConfigQuery == NULL || pfnRegDefaultUserConfigQuery == NULL) {
    ...
    Error = pfnRegUserConfigQuery(DcNameBuffer, UserName, &pTerm->MuGlobals.UserConfig, sizeof(USERCONFIG), &Length);
    if (Error != ERROR_SUCCESS) {
        DebugLog((DEB_TRACE,"WlxQueryTSData: RegUserConfigQuery returned %u\n", Error));
        Error = pfnRegDefaultUserConfigQuery(DcNameBuffer, &pTerm->MuGlobals.UserConfig, sizeof(USERCONFIG), &Length);
