...
VOID InitSystemParametersInfo(PTERMINAL pTerm, BOOL bUserLoggedOn)
/*
...
*/
{
    ...
    // Impersonate the user so if the user server side has to reference any ini files etc, it will do it in the correct place.
...
BOOL CreateUserEnvironment(PVOID *pEnv)
/*
* History:
...
BOOL UpdateUserEnvironment(PVOID *pEnv)
{
    ...
    Result = RegOpenKeyEx (HKEY_LOCAL_MACHINE, KEY_NAME, 0, KEY_QUERY_VALUE, &KeyHandle);
    ...
    for (i=0; i<cValues ; i++) {
        ...
        switch ( Type ) {
            case REG_SZ:
                {
                    Bool = SetUserEnvironmentVariable(pEnv, ValueName, ValueData, TRUE);
                    if ( !Bool ) {
                        DebugLog((DEB_ERROR, "UpdateUserEnvironment:  Failed to set <%s> to <%s> with %d.", ValueName, ValueData, GetLastError()));
                    }
            ...
            case REG_EXPAND_SZ:
                {
                    ...
                    Bool = SetUserEnvironmentVariable(pEnv, ValueName, ExpandedValue, TRUE);
                    Free( ExpandedValue );
                    if ( !Bool ) {
                        DebugLog((DEB_ERROR, "UpdateUserEnvironment:  Failed to set <%s> to <%s> with %d.", ValueName, ValueData, GetLastError()));
                    }
