...
DWORD WINAPI WinlogonUnhandledExceptionFilter(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
    CHAR Text[80];
...
VOID UpdateTcpIpParameters(VOID)
{
    HKEY Key ;
    int err ;
    WCHAR LocalSpace[ 64 ];
    ...
    err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("System\\CurrentControlSet\\Services\\Tcpip\\Parameters"), 0, KEY_READ | KEY_WRITE, &Key );
    if ( err == 0 )
    {
        err = RegQueryValueEx(Key, TEXT("NV Hostname"), 0, &Type, (PBYTE) Buffer, &Size );
        ...
        if ( err == 0 )
        {
            RegSetValueEx(Key, TEXT("Hostname"), 0, REG_SZ, (PBYTE) Buffer, Size );
        }

        // now, do the same for DnsDomain
        err = RegQueryValueEx(Key, TEXT("NV Domain"), 0, &Type, (PBYTE) Buffer, &Size );
        if ( (err == ERROR_INSUFFICIENT_BUFFER) || (err == ERROR_MORE_DATA ))
        {
        ...
        if ( err == 0 )
        {
            RegSetValueEx(Key, TEXT("Domain"), 0, REG_SZ, (PBYTE) Buffer, Size );
        }
    ...
    err = RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("Software\\Policies\\Microsoft\\System\\DNSclient"), 0, KEY_READ | KEY_WRITE, &Key );
    if ( err == 0 )
    {
        // now, do the same for the suffix that comes down through policy
        err = RegQueryValueEx(Key, TEXT("NV PrimaryDnsSuffix"), 0, &Type, (PBYTE) Buffer, &Size );
        if ( (err == ERROR_INSUFFICIENT_BUFFER) || (err == ERROR_MORE_DATA ))
        {
            if ( Buffer != LocalSpace )
            {
        ...
        if ( err == 0 ) {
            RegSetValueEx(Key, TEXT("PrimaryDnsSuffix"), 0, REG_SZ, (PBYTE) Buffer, Size );
        } else {
            RegDeleteValue(Key, TEXT("PrimaryDnsSuffix" ) );
        }
...
DWORD InitializeSlowStuff (LPVOID dummy)
{
    ...
    // call the winmm guys so they can start their message window and whatever else they need.
...
BOOL InitializeGlobals(HINSTANCE hInstance)
{
    ...
    if (g_IsTerminalServer) {
        ...
        if (!NT_SUCCESS(NtQueryInformationProcess(NtCurrentProcess(), ProcessSessionInformation, &SessionInfo, sizeof(SessionInfo), NULL))) {
            ASSERT(FALSE);
    ...
    *(RtlSubAuthoritySid(g_WinlogonSid, 0)) = SECURITY_LOCAL_SYSTEM_RID;
    ...
    g_fExecuteSetup = (g_uSetupType == SETUPTYPE_FULL) || (g_uSetupType == SETUPTYPE_UPGRADE);
    if (!g_fExecuteSetup) {
    ...
    // Get a copy of the computer name in *my* environment, so that we can look at it later.
...
void Breakin(ULONG BreakinProcessId)
{
    ...
    if (OpenProcessToken( GetCurrentProcess(), MAXIMUM_ALLOWED, &Token ))
    {
        ...
        if (AdjustTokenPrivileges( Token, FALSE, Privs, sizeof( Buf2 ), NewPrivs, &size )) {
            Process = OpenProcess(PROCESS_ALL_ACCESS, FALSE, BreakinProcessId);
            if (Process) {
                ...
                if (ntdll) {
                    ...
                    Thread = CreateRemoteThread(Process, NULL, STACKSIZE, DbgBreakPoint, NULL, CREATE_SUSPENDED, &ThreadId);
                    if (Thread) {
            ...
            // Once the remote thread is started, return to the old privileges so that nothing else gets screwed.
            AdjustTokenPrivileges( Token, FALSE, NewPrivs, 0, NULL, NULL );
        }
...
ULONG NTAPI WlpPeriodicBreak(PVOID Param, BOOLEAN Timeout)
{
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    if ( KdInfo.KernelDebuggerEnabled || NtCurrentPeb()->BeingDebugged )
    {
        ...
        SetTimerQueueTimer( NULL, WlpPeriodicBreak, NULL, 60 * 1000, 60 * 1000, FALSE );
    }

    InitDebugSupport();// Initialize debug support and logging
    ...
    UpdateTcpIpParameters();// Map the TCPIP information
    ...
    // Check for safemode:
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, SAFEBOOT_OPTION_KEY, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        dwSize = sizeof(dwOptionValue);
        RegQueryValueEx (hKey, OPTION_VALUE, NULL, &dwType, (LPBYTE) &dwOptionValue, &dwSize);
        RegCloseKey (hKey);

        // Check if the options value is minimal, if so exit now so Group Policy is not applied.
    ...
    // For the console winlogon, set the event that is toggled when the network provider list changes
    if (g_Console) {
        CreateNetworkProviderEvent();
    }

    if ( !WlpInitializeNotifyList( pTerm ) || !InitializeJobControl() || !LogoffLockInit() )
    {
    ...
    pTerm->ErrorMode = SetErrorMode( SEM_FAILCRITICALERRORS | SEM_NOOPENFILEERRORBOX );
    ...
    if (g_Console) {
        hThread = CreateThread (NULL, 0, (LPTHREAD_START_ROUTINE) InitializeSlowStuff, NULL, 0, &dwThreadID);
        if (hThread) {
    ...
    if ( pTerm->SafeMode )
    {
        ExpandEnvironmentStringsW(TEXT("%SystemRoot%\\system32\\msgina.dll"), pszGinaName, MAX_PATH );
    }
    else
    {
        GetProfileString(APPLICATION_NAME, GINA_KEY, TEXT("msgina.dll"), pszGinaName, MAX_PATH);
    }
    ...
    WlAddInternalNotify(PokeComCtl32, WL_NOTIFY_LOGOFF, FALSE, FALSE, TEXT("Reset ComCtl32"), 15 );

    MainLoop (pTerm);// Main loop

    // Shutdown the machine
    if (g_IsTerminalServer) {
        // Standard NT never exits the MainLoop above unless shutdown is requested.
        // HYDRA exits MainLoop for all non-console WinStations, therefore we must check if shutdown is desired.
        if ( IsShutdown(pTerm->LastGinaRet) )
            ShutdownMachine(pTerm, pTerm->LastGinaRet);

         // If its the console, and another WinStation did the shutdown, we must wait for the systems demise. If we exit here, we bluescreen while the shutdown is in process.
         if( g_Console ) {
...
BOOL CreatePrimaryTerminal (void)
{
    ...
    pTerm = LocalAlloc (LPTR, sizeof(TERMINAL) + (lstrlenW(DEFAULT_TERMINAL_NAME) + 1) * sizeof(WCHAR));
    if (!pTerm) {
    ...
    // For non-console winlogon's, make them point to the session specific DeviceMap.
    if (!g_Console) {
    ...
    pWS = LocalAlloc (LPTR, sizeof(WINDOWSTATION) + (lstrlenW(WINDOW_STATION_NAME) + 1) * sizeof(WCHAR));
    if (!pWS) {
    ...
    pWS->hdeskWinlogon = CreateDesktopW (WINLOGON_DESKTOP_NAME, NULL, NULL, 0, MAXIMUM_ALLOWED, NULL);
    ...
    pWS->hdeskApplication = CreateDesktopW (APPLICATION_DESKTOP_NAME, NULL, NULL, 0, MAXIMUM_ALLOWED, NULL);
    ...
    pShutDownTerm = pTerm;// Set the shutdown terminal now so we won't AV when upgrade.
    RtlZeroMemory( &pTerm->MuGlobals, sizeof(pTerm->MuGlobals));// Initialize Multi-User Globals
    pTerm->MuGlobals.ConfigQueryResult = ERROR_INVALID_DATA;// We have not retrieved the USERCONFIG yet.
    if (g_IsTerminalServer) {
...
//  Purpose:    Misc initialization that needs to be done after the first terminal is created.
...
VOID MiscInitialization (PTERMINAL pTerm)
{
    ...
    {
        // In the case of running setup in mini-setup mode, we want to be able to continue through and let the user logon.
        if(g_uSetupType == SETUPTYPE_NOREBOOT) {
            ExecuteSetup(pTerm);// go execute setup
        }

        // Don't go any further if setup didn't complete fully.
        // If this machine has not completed setup correctly, this will not return.
        CheckForIncompleteSetup(g_pTerminals);
    }

    if (!IsWin9xUpgrade()) {
        // Check to see if there is any WIN.INI or REG.DAT to migrate into Windows/NT registry.
        ...
        if (Win31MigrationFlags != 0) {
            SynchronizeWindows31FilesAndWindowsNTRegistry( Win31SystemStartEvent, Win31MigrationFlags, NULL, NULL);
            InitSystemFontInfo();
        }
    }
    ...
    hFontThread = StartLoadingFonts();// Load those pesky fonts:
    CheckForRepairRequest ();// Check if we need to run setup's GUI repair code
}