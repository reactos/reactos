    ...
    )
{
    ...
    rc = FormatMessageW (FORMAT_MESSAGE_ALLOCATE_BUFFER| FORMAT_MESSAGE_ARGUMENT_ARRAY| FORMAT_MESSAGE_FROM_HMODULE,
...
LPTSTR MyGetUserName (EXTENDED_NAME_FORMAT  NameFormat)
{
    ...
    while (TRUE) {
        if (GetUserNameEx (NameFormat, lpUserName, &ulUserNameSize)) {
            ...
            // Otherwise, check the pass count and retry if appropriate.
...
BOOL DoesUserDNSDomainExist(PTERMINAL pTerm)
{
    ...
    while (lpTemp && *lpTemp)
    {
        ...
        if (CompareString(LOCALE_USER_DEFAULT, NORM_IGNORECASE, lpTemp, 13, TEXT("USERDNSDOMAIN"), 13) == CSTR_EQUAL) {
            return TRUE;
        }
...
DWORD GetUserAccountInfo (PTERMINAL pTerm, BOOL *bLocalUser, BOOL *bNT4User)
{
    ...
    *lpTemp = TEXT('\0');
    ...
    dwError = DsGetDcName (NULL, lpDomain, NULL, NULL, DS_DIRECTORY_SERVICE_PREFERRED, &pDCI);
...
BOOL CheckForGPOScripts (HKEY hKeyRoot, LPTSTR lpScriptType)
{
    ...
    if (RegOpenKeyEx (hKeyRoot, TEXT("Software\\Policies\\Microsoft\\Windows\\System\\Scripts") , 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        if (RegQueryValueEx (hKey, lpScriptType, NULL, &dwType, NULL, &dwSize) == ERROR_SUCCESS) {
            bResult = TRUE;
        }
...
BOOL ExecuteGPOScripts (PTERMINAL pTerm, BOOL bUser, BOOL bSync, LPWSTR lpDesktop, PVOID pEnvironment)
{
    BOOL Result = FALSE;
    ...
    // If we are executing the scripts synchronously, then put userinit in a job object so we can kill it
    if (bSync) {
        ...
        if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, WINLOGON_KEY, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
            ...
            RegQueryValueEx (hKey, MAX_GPO_SCRIPT_WAIT, NULL, &dwType, (LPBYTE) &dwMaxWait, &dwSize);
            RegCloseKey (hKey);
        }
        ...
        Result = StartProcessInJob(pTerm, (bUser ? ProcessAsUser : ProcessAsSystem), lpDesktop, pEnvironment, szCmdLine, CREATE_UNICODE_ENVIRONMENT, 0, Job );

        // If the app was started, assign the process to the job
        if (Result) {
            BOOL bSwitchedDesktop = FALSE;

            // user scripts run on application desktop application desktop needs to be made visible.
            if ( bUser )
            {
        ...
        if (bUser) {
            ...
            StartSystemProcess(szCmdLine, lpDesktop, 0, 0, pEnvironment, 0, NULL, NULL);
        }
    }
