...
static void InitDCI()
{
    ...
    if (pdci->dwDCICaps & DCI_1632_ACCESS) {
        ...
        if (pdci->wSelSurface == 0 || pdci->dwOffSurface >= 0x10000) {
            ...
            if (pdci->wSelSurface)
                SetSelectorBase(ScreenSel, GetSelectorBase(pdci->wSelSurface) + pdci->dwOffSurface);
            else
    ...
    DPF(("DCI Surface: biCompression= %ld '%4.4hs' Masks: %04lX %04lX %04lX",
         biScreen.bi.biCompression,
         (LPSTR)&biScreen.bi.biCompression,
         biScreen.dwMask[0],
         biScreen.dwMask[1],
         biScreen.dwMask[2]));
...
* @rdesc Returns TRUE if the library is initialized properly, otherwise it returns FALSE.
...
BOOL VFWAPI DrawDibInit()
{
    ...
    if (gfInit) {
        ...
        if (gwScreenWidth == wScreenWidth && gwScreenHeight == wScreenHeight && gwScreenBitDepth == wScreenBitDepth) {
            return TRUE;
        }

        DPF(("Screen has changed from %dx%dx%d to %dx%dx%d",
             gwScreenWidth,
             gwScreenHeight,
             gwScreenBitDepth,
             wScreenWidth,
...
#ifdef DEBUG
    {
        ...
        case BM_24RGB:
        case BM_32RGB:
            DPF(("display format: %d-bits RGB", ((wType == BM_24RGB) ? 24 : 32)));
            break;
        default:
...
*/
BOOL DrawDibQueryBitmapX(PDD pdd)
{
                         ...
                         UINT     wFlags)
{
    ...
    if (wFlags & DDF_ANIMATE) {
        if (!(gwRasterCaps & RC_PALETTE) || (int)lpbi->biBitCount > 8 || pdd->hpalDraw)
            wFlags &= ~DDF_ANIMATE;
    }
...
try_again:
    ...
    //  regardless of what the display can do
        ...
        if (pdd->hic == NULL) {
            ...
            pdd->hic = ICLocate(ICTYPE_VIDEO, fccHandler, lpbi, NULL, ICMODE_FASTDECOMPRESS);
            if (pdd->hic == NULL)
                pdd->hic = ICDecompressOpen(ICTYPE_VIDEO, fccHandler, lpbi, NULL);
...
no_decomp:
    ...
    if (pdd->hpal && pdd->lpbi->biBitCount > 8 && ((pdd->ulFlags ^ ulFlagsSave) & (DDF_DITHER))) {
    ...
    if ((gwRasterCaps & RC_PALETTE) && pdd->biDraw.biBitCount <= 8 && pdd->hpal == NULL) {
...
//      pdd->hbmDraw = CreateCompatibleBitmap(hdc, (int)pdd->biDraw.biWidth, (int)pdd->biDraw.biHeight);
    ...
    if (ScreenBitDepth > 4 &&
        ...
        if (pdd->hpalDraw || pdd->hpal) {
            hpalOld = SelectPalette(hdc, pdd->hpalDraw ? pdd->hpalDraw : pdd->hpal, TRUE);
        }
    ...
    // SendSetPalette(pdd);
        ...
        for (i = 0; i < 256; i++) {
            if (pdd->biBuffer.biBitCount == 8) {
                ...
                pdd->aw[i] = (WORD)GetNearestPaletteIndex(pdd->hpalDraw, RGB(pe.peRed, pe.peGreen, pe.peBlue));
            }
        }
