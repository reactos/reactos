...
#pragma message("Remove the expiration code after Beta ships")
    ...
    if (hInst)
                GetModuleFileName(hInst, achMod, sizeof(achMod));
    ...
    LPMCI_DGV_STEP_PARMS lpStep)
{
    ...
    if (fSeekExactOff) {
        ...
        */
        dwRet = DeviceSeek(npMCI, lFrames + lFrameCur, dwFlags | MCI_WAIT, lpStep->dwCallback);
        npMCI->dwOptionFlags &= ~(MCIAVIO_SEEKEXACT);
    } else {
        dwRet = DeviceSeek(npMCI, lFrames + lFrameCur, dwFlags, lpStep->dwCallback);
    }
...
*/
DWORD NEAR PASCAL GraphicStop (NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_GENERIC_PARMS lpParms)
{
    if (!IsWindow(npMCI->hwndPlayback))
        return MCIERR_NO_WINDOW;
...
*/
DWORD NEAR PASCAL GraphicPause(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_GENERIC_PARMS lpParms)
{
    LPARAM dwCallback;
    if (!IsWindow(npMCI->hwndPlayback))
        return MCIERR_NO_WINDOW;
...
*/
DWORD NEAR PASCAL GraphicStatus (NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_STATUS_PARMS lpStatus)
{
    DWORD dwRet = 0L;
    ...
    if (dwFlags & MCI_STATUS_ITEM) {
        ...
        if ((dwFlags & MCI_TRACK) && !((lpStatus->dwItem == MCI_STATUS_POSITION) || (lpStatus->dwItem == MCI_STATUS_LENGTH)))
            return MCIERR_FLAGS_NOT_COMPATIBLE;

        if ((dwFlags & MCI_STATUS_START) && (lpStatus->dwItem != MCI_STATUS_POSITION))
            return MCIERR_FLAGS_NOT_COMPATIBLE;
...
*/
DWORD NEAR PASCAL GraphicInfo(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_INFO_PARMS lpInfo)
{
    DWORD       dwRet = 0L;
    TCHAR       ch = TEXT('\0');
    BOOL        fTest = FALSE;
...
*/
DWORD NEAR PASCAL GraphicSet (NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_SET_PARMS lpSet)
{
    DWORD       dwRet = 0L;
    DWORD       dwAction;
...
*/
DWORD NEAR PASCAL GraphicSignal(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_SIGNAL_PARMS lpSignal)
{
    DWORD       dwRet = 0L;
    DWORD       dwUser;
    DWORD       dwPosition;
...
*/
DWORD NEAR PASCAL GraphicWindow (NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_WINDOW_PARMS lpWindow)
{
    DWORD   dwRet = 0L;
    int     i = 0;
    HWND    hWndNew;
...
*/
DWORD NEAR PASCAL GraphicPut ( NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_RECT_PARMS lpParms)
{
    BOOL        frc;
    RECT        rc;
    ...
    switch (dwFlags & (MCI_DGV_PUT_SOURCE | MCI_DGV_PUT_DESTINATION | MCI_DGV_PUT_WINDOW))
    {
        case 0L:
            return MCIERR_MISSING_PARAMETER;
    ...
    LPMCI_DGV_RECT_PARMS lpParms)
{
    ...
    switch (dwFlags & (MCI_DGV_WHERE_SOURCE | MCI_DGV_WHERE_DESTINATION | MCI_DGV_WHERE_WINDOW))
    {
        case 0L:
            return MCIERR_MISSING_PARAMETER;
...
*/
DWORD NEAR PASCAL GraphicUpdate(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_UPDATE_PARMS lpParms)
{
    RECT    rc;
...
DWORD PASCAL mciDriverEntry (UINT wDeviceID, UINT wMessage, DWORD dwFlags, LPMCI_GENERIC_PARMS lpParms)
{
    ...
    */
    if ( IsNTWOW() ) {
          ...
          if ( IsWindow(npMCI->hwndDefault) ) {
              WNDPROC wndProc = (WNDPROC)GetWindowLongPtr( npMCI->hwndDefault, GWLP_WNDPROC );
              if ( wndProc != GraphicWndProc ) {
                  DPF2(( "WOW app has subclassed AVIWnd32 window - correcting\n" ));
                  SetWindowLongPtr( npMCI->hwndDefault, GWLP_WNDPROC, (LONG_PTR)GraphicWndProc );
              }
