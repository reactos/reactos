...
void FAR PASCAL _LOADDS mciaviTask(DWORD_PTR dwInst)
{
    ...
    // own the winproc.
#if 0
    if (mmTaskCreate((LPTASKCALLBACK) aviWinProcTask, &npMCI->hThreadWinproc, (DWORD)(UINT)npMCI) == 0)
#else
    if (npMCI->hThreadWinproc = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)aviWinProcTask, (LPVOID)npMCI, 0, &dwThreadId ))
#endif
    {
    ...
    if (WaitForMultipleObjects(2, &npMCI->hThreadWinproc,
        ...
        npMCI->dwTaskError = MCIERR_CREATEWINDOW;// thread aborted
    ...
    TaskReturns(npMCI, 0);// signal that the open is complete
...
#ifdef DEBUG
    // A complex assertion.  If we have stopped temporarily, then we do not want to go back to sleep.
    if ((npMCI->dwFlags & MCIAVI_UPDATING) && (WAIT_TIMEOUT    == WaitForMultipleObjects(IDLEWAITFOR, &npMCI->hEventSend, FALSE, 0))) {
