...
void FAR PASCAL GraphicImmediateNotify(UINT wDevID, LPMCI_GENERIC_PARMS lpParms, DWORD dwFlags, DWORD dwErr);
DWORD PASCAL  GraphicClose(NPMCIGRAPHIC npMCI);
DWORD PASCAL GraphicOpen(NPMCIGRAPHIC FAR * lpnpMCI, DWORD dwFlags, LPMCI_DGV_OPEN_PARMS lpOpen, UINT wDeviceID);
DWORD FAR PASCAL GraphicInfo(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_INFO_PARMS lpInfo);
DWORD FAR PASCAL GraphicPlay(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_PLAY_PARMS lpPlay);
DWORD FAR PASCAL GraphicCue(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_CUE_PARMS lpCue);
DWORD FAR PASCAL GraphicStep(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_STEP_PARMS lpStep);
DWORD FAR PASCAL GraphicStop(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_GENERIC_PARMS lpParms);
DWORD FAR PASCAL GraphicSeek(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_SEEK_PARMS lpSeek);
DWORD FAR PASCAL GraphicPause(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_GENERIC_PARMS lpParms);
DWORD FAR PASCAL GraphicStatus(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_STATUS_PARMS lpStatus);
DWORD FAR PASCAL GraphicSet(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_SET_PARMS lpSet);
DWORD FAR PASCAL GraphicResume(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_GENERIC_PARMS lpParms);
DWORD FAR PASCAL GraphicRealize(NPMCIGRAPHIC npMCI, DWORD dwFlags);
DWORD FAR PASCAL GraphicUpdate(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_UPDATE_PARMS lpParms);
DWORD FAR PASCAL GraphicWindow(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_WINDOW_PARMS lpWindow);
DWORD FAR PASCAL GraphicConfig(NPMCIGRAPHIC npMCI, DWORD dwFlags);
DWORD FAR PASCAL GraphicSetAudio(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_SETAUDIO_PARMS lpSet);
DWORD FAR PASCAL GraphicSetVideo(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_SETVIDEO_PARMS lpSet);
DWORD FAR PASCAL GraphicSignal(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_SIGNAL_PARMS lpSignal);
DWORD FAR PASCAL GraphicWhere(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_RECT_PARMS lpParms);
DWORD FAR PASCAL GraphicPut(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_RECT_PARMS lpParms);
 ...
 *      gets a DRV_OPEN message. This happens each time that a new movie is opened thru MCI.

 * @parm LPMCI_OPEN_DRIVER_PARMS | lpOpen | Far pointer to the standard MCI open parameters
...
DWORD NEAR PASCAL GraphicInfo16(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_INFO_PARMS lpInfo)
/*
 * @doc INTERNAL MCIAVI
...
*/
{
    ...
    case 0L:
        ...
        LoadString(ghModule, MCIAVI_PRODUCTNAME, lpInfo->lpstrReturn, (UINT)lpInfo->dwRetSize);
        break;
    case MCI_INFO_VERSION:
        /* !!! Not returning PARAM_OVERFLOW here but I am above - lazy eh */
        LoadString(ghModule, MCIAVI_VERSION, lpInfo->lpstrReturn, (UINT)lpInfo->dwRetSize);
        break;
...
DWORD NEAR PASCAL GraphicList(NPMCIGRAPHIC npMCI, DWORD dwFlags, LPMCI_DGV_LIST_PARMS lpList)
/*
 * @doc INTERNAL MCIAVI
...
/*
 * @doc INTERNAL MCIAVI
 * @api DWORD | GraphicGetDevCaps | This function returns  device capabilities
 * @parm NPMCIGRAPHIC | npMCI | Near pointer to instance data block
 * @parm DWORD | dwFlags | Flags for the GetDevCaps message.
 * @parm LPMCI_GETDEVCAPS_PARMS | lpCaps | Parameters for the GetDevCaps message.
 * @rdesc Returns an MCI error code.
*/
{
    DWORD dwRet = 0L;
    ...
    if (dwFlags & MCI_GETDEVCAPS_ITEM)
    {
        ...
        case MCI_GETDEVCAPS_DEVICE_TYPE:
            lpCaps->dwReturn = MAKEMCIRESOURCE(MCI_DEVTYPE_DIGITAL_VIDEO, MCI_DEVTYPE_DIGITAL_VIDEO);
            dwRet = MCI_RESOURCE_RETURNED;
            break;
...
/*
 * @doc INTERNAL MCIAVI
 * @api DWORD | mciSpecial | This function handles all the MCI commands that don't require instance data such as open.
...
*/
{
    ...
    case MCI_OPEN_DRIVER:
        ...
        break;
    case MCI_GETDEVCAPS:
        dwRet = GraphicGetDevCaps(NULL, dwFlags, (LPMCI_GETDEVCAPS_PARMS)lpParms);
        break;
    case MCI_CONFIGURE:
        if (!(dwFlags & MCI_TEST))
...
*/
{
    ...
    if (npMCI->wMessageCurrent) {
        ...
        if (wMessage != MCI_STATUS && wMessage != MCI_GETDEVCAPS && wMessage != MCI_INFO) {
            //DPF(("Warning!!!!!\n"));
    ...
    case MCI_RESUME:
        ...
        break;
    case MCI_SET:
        dwRet = GraphicSet(npMCI, dwFlags, (LPMCI_DGV_SET_PARMS)lpParms);
        break;
    case MCI_STATUS:
        dwRet = GraphicStatus(npMCI, dwFlags, (LPMCI_DGV_STATUS_PARMS)lpParms);
        break;
    case MCI_INFO:
    ...
    case MCI_CONFIGURE:
        ...
        break;
    case MCI_SETAUDIO:
        dwRet = GraphicSetAudio(npMCI, dwFlags, (LPMCI_DGV_SETAUDIO_PARMS)lpParms);
        break;
    case MCI_SETVIDEO:
        dwRet = GraphicSetVideo(npMCI, dwFlags, (LPMCI_DGV_SETVIDEO_PARMS)lpParms);
        break;
    case MCI_SIGNAL:
        dwRet = GraphicSignal(npMCI, dwFlags, (LPMCI_DGV_SIGNAL_PARMS)lpParms);
        break;
    case MCI_LIST:
        dwRet = GraphicList(npMCI, dwFlags, (LPMCI_DGV_LIST_PARMS)lpParms);
        break;
#if 0
    case MCI_LOAD:
        dwRet = GraphicLoad(npMCI, dwFlags, (LPMCI_DGV_LOAD_PARMS)lpParms);
        break;
#endif
    case MCI_RECORD:
    case MCI_SAVE:
 ...
 *          defined driver portocols. Messages above DRV_USER are used for driver specific messages.
 * @parm    DWORD | dwParam1 | Data for this message.  Defined separately for each message
 * @parm    DWORD | dwParam2 | Data for this message.  Defined separately for each message
 * @rdesc Defined separately for each message.
*/
DWORD FAR PASCAL _LOADDS DriverProc(DWORD dwDriverID, HANDLE hDriver, UINT wMessage, DWORD dwParam1, DWORD dwParam2)
{
    DWORD dwRes = 0L;
     ...
     * also held in mciDriverEntry, in GraphicWndProc, and around all worker thread draw functions.
     */
...
#define GET_MAPPING_MODULE_NAME         TEXT("wow32.dll")
    ...
    case DRV_CONFIGURE:
        ...
        break;
    default:
        if (!HIWORD(dwDriverID) && wMessage >= DRV_MCI_FIRST && wMessage <= DRV_MCI_LAST)
            dwRes = mciDriverEntry((UINT)dwDriverID, wMessage, dwParam1, (LPMCI_GENERIC_PARMS)dwParam2);
        else
            dwRes = mmDefDriverProc(dwDriverID, hDriver, wMessage, dwParam1, dwParam2);
        break;
    }
