...
void FAR PASCAL _LOADDS mciaviTask(DWORD dwInst)
{
    ...
    SetErrorMode(npMCI->uErrorMode);// Set this task's error mode to the same as the parent's.
...
void NEAR PASCAL mciaviMessage(NPMCIGRAPHIC npMCI, UINT msg)
{
    ...
    case TASKSTARTING:
        ...
        if ((wNotification != MCI_NOTIFY_FAILURE) || ((npMCI->dwFlags & MCIAVI_WAITING) == 0))
...
void NEAR PASCAL aviTaskYield(void)
{
    ...
    //  getting anything your priority is lowered which would mess up our tinkering with the priority here.
    PrioritySeparation = GetPrioritySeparation();

    if (PrioritySeparation != 0) {
        SetThreadPriority(GetCurrentThread(), PrioritySeparation == 1 ?
