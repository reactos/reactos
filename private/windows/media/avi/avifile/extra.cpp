...
HRESULT ReadExtra(LPEXTRA extra, DWORD ckid, LPVOID lpData, LONG FAR *lpcbData)
{
...
HRESULT WriteExtra(LPEXTRA extra, DWORD ckid, LPVOID lpData, LONG cbData)
{
    HPBYTE lp;
    ...
    hmemcpy(lp + extra->cb + sizeof(DWORD) * 2, lpData, cbData - sizeof(DWORD) * 2);
...
HRESULT ReadIntoExtra(LPEXTRA extra, HSHFILE hshfile, MMCKINFO FAR * lpck)
{
    HPBYTE lp;
    ...
    if (shfileRead(hshfile, (HPSTR) lp + extra->cb + sizeof(DWORD) * 2, lpck->cksize) != (LONG) lpck->cksize)
...
LONG FindChunkAndKeepExtras(LPEXTRA extra, HSHFILE hshfile, MMCKINFO FAR* lpck, MMCKINFO FAR* lpckParent, UINT uFlags)
{
    ...
    for (;;) {
        ...
        if ((!ckidFind || lpck->ckid == ckidFind) && (!fccTypeFind || lpck->fccType == fccTypeFind))
            return 0;
