...
CTask FAR* CTask::LookupTask(HTASK FAR&    hTask)
{
    ...
    HINSTANCE    hInstance)
{
    ...
    for (pDllEntry = pTask->m_pDllEntry; pDllEntry; pDllEntry = pDllEntry->pNextDll) {
        if ((hInstance == pDllEntry->hInstance) && (rclsid == pDllEntry->clsid))
...
STDAPI  CoGetClassObject(REFCLSID    rclsid, DWORD    dwClsContext, LPVOID    pvReserved, REFIID    riid, void FAR* FAR*    ppv)
{
    char    aszServer[256];
    HTASK    htask;
    CTask FAR*    pTask;
...
STDAPI CoCreateInstance(REFCLSID    rclsid, IUnknown FAR* pUnkOuter, DWORD    dwClsContext, REFIID    riid, LPVOID FAR*    ppv)
{
    HRESULT    hr;
    IClassFactory FAR*    pFactory;
...
STDAPI GetStandardTaskMalloc(IMalloc FAR* FAR* ppMalloc)
{
    v_stdMalloc.AddRef();
...
STDAPI CoGetMalloc(DWORD    dwMemContext, IMalloc FAR* FAR* ppMalloc)
{
    HTASK    htask;
    CTask FAR*    pTask;
    IMalloc FAR*    pMalloc;
