...
HRESULT CAVIFileCF::Create(const CLSID FAR&    rclsid, REFIID    riid, LPVOID FAR*    ppv)
{
...
CAVIFileCF::CAVIFileCF(const CLSID FAR&    rclsid, IUnknown FAR* FAR*    ppUnknown) : m_Unknown(this), m_Factory(this)
{
    m_clsid = rclsid;
    *ppUnknown = &m_Unknown;
}
...
CAVIFileCF::CUnknownImpl::CUnknownImpl(CAVIFileCF FAR*    pAVIFileCF)
{
...
STDMETHODIMP CAVIFileCF::CUnknownImpl::QueryInterface(REFIID    iid, LPVOID FAR*    ppv)
{
    if (iid == IID_IUnknown)
...
CAVIFileCF::CFactoryImpl::CFactoryImpl(CAVIFileCF FAR*    pAVIFileCF)
{
    m_pAVIFileCF = pAVIFileCF;
}


STDMETHODIMP CAVIFileCF::CFactoryImpl::QueryInterface(REFIID    iid, LPVOID FAR*    ppv)
{
...
STDMETHODIMP CAVIFileCF::CFactoryImpl::CreateInstance(IUnknown FAR*    pUnknownOuter, REFIID    riid, LPVOID FAR*    ppv)
{
...
STDMETHODIMP CAVIFileCF::CFactoryImpl::LockServer(BOOL    fLock)
{
    fLocked = fLock;
    return AVIERR_OK;
}