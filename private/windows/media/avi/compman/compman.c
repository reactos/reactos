...
BOOL ReadDriversInfo()
{
    ...
    for (;;)
    {
        ...
        cchBuffer += (UINT)myGetPrivateProfileString(gszIniSect, NULL, szNull, (LPTSTR)lpICInfoMem + cchBuffer, (cbBuffer/sizeof(TCHAR)) - cchBuffer);

        // if all of the INI data fit, we can leave the loop
        if (cchBuffer < ((cbBuffer/sizeof(TCHAR)) - 5))
            break;
        }
...
LONG OpenUserKey(PHKEY pKey, LPCTSTR lp, LPCTSTR pSection)
{
    ...
    return(RegCreateKeyExW(HKEY_CURRENT_USER, section, 0, NULL, 0, KEY_SET_VALUE, NULL, pKey, &disposition));
    } else {
...
LONG OpenSystemKey(PHKEY pKey, LPCTSTR lp, LPCTSTR pSection)
{
    ...
    lstrcat(section, pSection);
    if (lp) {
    return(RegCreateKeyExW(HKEY_LOCAL_MACHINE, section, 0, NULL, 0, KEY_QUERY_VALUE | KEY_SET_VALUE, NULL, pKey, &disposition));
    } else {
    // We are only reading what is there...
...
)
{
    if (IsAdmin()) {
        ...
        if (lpszString) {
            LPCTSTR lpStr;
            RegSetValueEx(key, lpszKeyName, 0, REG_SZ, (LPCVOID)lpszString, sizeof(TCHAR)*(lstrlen(lpszString)+1));
            ...
            if (ERROR_SUCCESS == (RegCreateKeyExW(HKEY_CURRENT_USER, section, 0, NULL, 0, KEY_SET_VALUE, NULL, &key2, &disposition))) {
                RegSetValueEx(key2, lpszString, 0, REG_SZ, (LPCVOID)lpStr, sizeof(TCHAR)*(lstrlen(lpStr)+1));
                RegCloseKey(key2);
            }
            }
        } else {
...
// correct.
    ...
    if (ERROR_SUCCESS == OpenUserKey(&key, NULL, lpszSection)) {
        nSize = (DWORD) (DWORD_PTR) ((LPBYTE)lpEnd-(LPBYTE)lpBuf);// Calculate - in bytes - how much space is in the buffer
        nRet = RegQueryValueEx(key, lpszKey, NULL, &dwType, (LPBYTE)lpBuf, &nSize);// Get the data
        RegCloseKey(key);
    }
