...
LPVOID FAR PASCAL AllocSectorAlignedMem (DWORD dwRequest, DWORD dwAlign)
{
    ...
    pbuf = VirtualAlloc (NULL, dwRequest, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    ...
    LPBOOL       lpbPending)
{
    ...
    *lpbPending = FALSE;
    ...
    #ifdef JMK_HACK_CHECKHDR
     if (IsBadWritePtr (lptvh, sizeof(*lptvh)) || HIWORD(lptvh->vh.lpData) != HIWORD(lptvh->p16Alloc))
        {
    ...
    LPUINT      lpuError)
{
    ...
    *lpuError = 0;
    ...
    while(lpcs->iNextAsync != lpcs->iLastAsync)
    {
        ...
        // as complete, then return.  Thought..call GetQueuedCompletionStatus in a loop, until there are no more blocks pending.  This way we
        ...
        if (lpah->uType & ASYNCIOPENDING) {
            ...
            LPOVERLAPPED povl;
            BOOL fResult =
            pfnGetQueuedCompletionStatus(lpcs->hCompletionPort, &dwWritten, &key, &povl, 0);
            if (fResult) {
