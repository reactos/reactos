...
//  Allow this file to build without warnings when the DUnicode switch is turned off.
#undef MAKEINTRESOURCE
#define MAKEINTRESOURCE MAKEINTRESOURCEW


//  Forward Declarations.
int GetLocalizedLanguageName(LANGID Language, LPWSTR *ppLangName);
BOOL GetLocalizedCountryName(LANGID Language, LPWSTR *ppCtryName);
BOOL GetLocalizedSortName(LCID Locale, LPWSTR *ppSortName);
...
BOOL SetMultipleUserInfo(DWORD dwFlags, int cchData, LPCWSTR pPicture, LPCWSTR pSeparator, LPCWSTR pOrder, LPCWSTR pTLZero, LPCWSTR pTimeMarkPosn);
BOOL SetTwoDigitYearInfo(CALID Calendar, LPCWSTR pYearInfo, int cchData);
void GetInstallLanguageFromRegistry();
...
{
    // If the thread isn't impersonating, then re-read the process locale from the current user's registry
...
BOOL WINAPI IsValidLanguageGroup(LGRPID LanguageGroup, DWORD dwFlags)
//  Determines whether or not a language group is installed in the system
//  if the LGRPID_INSTALLED flag is set, or whether or not a language group is supported in the system if the LGRPID_SUPPORTED flag is set.
...
//  03-10-98    JulieB    Created.
{
    ...
    if ((dwFlags & IVLG_INVALID_FLAG) || (MORE_THAN_ONE(dwFlags, IVLG_SINGLE_FLAG)))
    {
        return (FALSE);
    }

    OPEN_LANG_GROUPS_KEY(FALSE);//  Open the Language Groups registry key.
    ...
    if ((QueryRegValue( hLangGroupsKey, pTmpBuf, &pKeyValueFull, MAX_KEY_VALUE_FULLINFO, NULL ) != NO_ERROR))
    {
        return (FALSE);
    }

    //  Language Group is SUPPORTED.  If the INSTALLED flag is NOT set, then return success.
...
//  Determines whether or not a locale is installed in the system if the LCID_INSTALLED flag is set,
//  or whether or not a locale is supported in the system if the LCID_SUPPORTED flag is set.
...
//  07-26-93    JulieB    Created.
{
    ...
    {
        //  The ME release of NT 4 did a really bad thing and allowed 0x39 to be passed in as a valid flag value for Arabic and Hebrew.
        //  As a result, we need to allow this flag combination for the Arabic and Hebrew locales.
        if ((dwFlags == 0x39) && ((Locale == MAKELCID(MAKELANGID(LANG_ARABIC, SUBLANG_DEFAULT), SORT_DEFAULT)) || (Locale == MAKELCID(MAKELANGID(LANG_HEBREW, SUBLANG_DEFAULT), SORT_DEFAULT))))
        {
    ...
    //  See if the LOCALE information is in the system for the given locale.
    if (GetLocHashNode(Locale) == NULL)
    {
        return (FALSE);//  Return failure.
    }

    //  Locale is SUPPORTED.  If the INSTALLED flag is NOT set, then return success.
    if (!(dwFlags & LCID_INSTALLED))
    {
        return (TRUE);
    }

    //  Open the Locale, the Alternate Sorts, and the Language Groups registry keys.
    ...
    if (((QueryRegValue( hLocaleKey, pTmpBuf, &pKeyValueFull, MAX_KEY_VALUE_FULLINFO, NULL ) == NO_ERROR) || (QueryRegValue( hAltSortsKey, pTmpBuf, &pKeyValueFull, MAX_KEY_VALUE_FULLINFO, NULL ) == NO_ERROR))
        && (pKeyValueFull->DataLength > 2))
    {
        RtlInitUnicodeString(&ObUnicodeStr, GET_VALUE_DATA_PTR(pKeyValueFull));
        if ((RtlUnicodeStringToInteger(&ObUnicodeStr, 16, &Data) == NO_ERROR) && (Data != 0))
        {
            pKeyValueFull = (PKEY_VALUE_FULL_INFORMATION)pStatic2;
            if ((QueryRegValue( hLangGroupsKey, ObUnicodeStr.Buffer, &pKeyValueFull, MAX_KEY_VALUE_FULLINFO, NULL ) == NO_ERROR) && (pKeyValueFull->DataLength > 2))
            {
                pData = GET_VALUE_DATA_PTR(pKeyValueFull);
                if ((pData[0] == L'1') && (pData[1] == 0))
                {
...
//  If none of the special case locales was given, the given locale id is returned.

//  09-01-93    JulieB    Created.
{
    CHECK_SPECIAL_LOCALES(Locale, FALSE);//  Check for the special locale values.
    return (Locale);//  Return the locale id.
}
...
{
    return ((LCID)(NtCurrentTeb()->CurrentLocale));//  Return the locale id stored in the TEB.
}


BOOL WINAPI SetThreadLocale(LCID Locale)
//  Resets the locale id for the current thread.
//  Any locale-dependent functions will reflect the new locale.
//  If the locale passed in is not a valid locale id, then FALSE is returned.
...
//  03-11-93    JulieB    Moved from base\client; Added Locale Validation.
{
    ...
    NtCurrentTeb()->CurrentLocale = (ULONG)Locale;//  Set the locale id in the TEB.
...
//  Returns the current User's UI language selection.
//  If the UI language is not available, then the chosen default UI language is used (NLS_DEFAULT_UILANG).
...
//  If the registry value is not readable, then the chosen default language is used
//  (NLS_DEFAULT_LANGID).

//  05-31-91    JulieB    Created.
{
    return (LANGIDFROMLCID(gSystemLocale));//  Get the language id from the locale id stored in the cache and return it.
}


LANGID WINAPI GetUserDefaultLangID()
//  Returns the default language for the current user.
//  If the current user's language is not set, then the system default language id is returned.

//  05-31-91    JulieB    Created.
{
    return (LANGIDFROMLCID(GetUserDefaultLCID()));//  Get the language id from the locale id stored in the cache and return it.
}
...
//  05-31-91    JulieB    Created.
{
    return (gSystemLocale);//  Return the locale id stored in the cache.
}


LCID WINAPI GetUserDefaultLCID()
//  Returns the default locale for the current user.
//  If current user's locale is not set, then the system default locale id is returned.
...
//  05-31-91    JulieB    Created.
{
    ...
    switch (Lcid)
    {
        case ( -1 ) :
        {
            ...
            {
                // If we can't get it from the registry, then let's use the system locale since it won't be resolved by calling GetUserDefaultLCID() again.
        ...
        case ( 0 ) :
        {
            ...
            //  If we are running in the interactive logged on user, then use the one cached in CSRSS if the cache is valid.
            //  Otherwise, use the process cached locale.
            if (gInteractiveLogonUserProcess == (BOOL) -1)
            {
                NlsIsInteractiveUserProcess();
            }

            if ((gInteractiveLogonUserProcess != FALSE) && (pNlsUserInfo->fCacheValid != FALSE))
            {
...
DWORD WINAPI VerLanguageNameW(DWORD wLang, LPWSTR szLang, DWORD wSize)
//  Returns the language name of the given language id in the language of the current user.
...
//  05-31-91    JulieB    Moved and Rewrote from Version Library.
{
    ...
    {
        //  Can't get the name of the language id passed in, so get the "language neutral" name.
        Length = GetLocalizedLanguageName((LANGID)LANG_NEUTRAL, &pString);
    }

    //  If the length is too big for the buffer, then reset the length to the size of the given buffer.
    if (Length >= wSize)
    {
        Length = wSize - 1;
    }
    ...
    return (Length);//  Return the number of characters in the string, NOT including the null termination.
}


DWORD WINAPI VerLanguageNameA(DWORD wLang, LPSTR szLang, DWORD wSize)
//  Returns the language name of the given language id in the language of the current user.
...
//  05-31-91    JulieB    Moved from Version Library.
{
    ...
    if (Language.Buffer == NULL)//  Make sure the allocation succeeded.
    {
        return (FALSE);
    }
    ...
    return (Status);//  Return the value returned from VerLanguageNameW.
}


int WINAPI GetLocaleInfoW(LCID Locale, LCTYPE LCType, LPWSTR lpLCData, int cchData)
//  Returns one of the various pieces of information about a particular locale by querying the configuration registry.
...
//  05-31-91    JulieB    Created.
{
    ...
    if ((pHashN == NULL) || (cchData < 0) || ((lpLCData == NULL) && (cchData != 0)))
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return (0);
    }

    //  Set the base value to add to in order to get the variable length strings.
    ...
    {
        //  Flag is set, so set the boolean value and remove the flag from the LCType parameter (for switch statement).
        UserOverride = FALSE;
    }
    ...
    //  If user information exists for the given LCTYPE, then the user default is returned instead of the system default.
    switch (LCType)
    {
    ...
    case (LOCALE_SLANGUAGE) :
    {
        ...
        //  The pString pointer is set after GetLocalizedLanguageName is called.
        Length = GetLocalizedLanguageName(LANGIDFROMLCID(Locale), &pString);
        if (Length == 0)
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (0);
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sAbbrevLangName, NLS_VALUE_SLANGUAGE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iCountry, NLS_VALUE_ICOUNTRY, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sCountry, NLS_VALUE_SCOUNTRY, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
        {
            ...
            //  The pString pointer is set after GetLocalizedCountryName is called.
            if (!GetLocalizedCountryName(LANGIDFROMLCID(Locale), &pString))
            {
                SetLastError(ERROR_INVALID_PARAMETER);
                return (0);
            }
    ...
    case (LOCALE_SSORTNAME) :
    {
        ...
        //  The pString pointer is set after GetLocalizedSortName is called.
    ...
    case (LOCALE_SLIST) :
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sList, NLS_VALUE_SLIST, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iMeasure, NLS_VALUE_IMEASURE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iPaperSize, NLS_VALUE_IPAPERSIZE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sDecimal, NLS_VALUE_SDECIMAL, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sThousand, NLS_VALUE_STHOUSAND, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sGrouping, NLS_VALUE_SGROUPING, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iDigits, NLS_VALUE_IDIGITS, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iLZero, NLS_VALUE_ILZERO, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iNegNumber, NLS_VALUE_INEGNUMBER, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sNativeDigits, NLS_VALUE_SNATIVEDIGITS, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iDigitSubstitution, NLS_VALUE_IDIGITSUBST, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sCurrency, NLS_VALUE_SCURRENCY, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sMonDecSep, NLS_VALUE_SMONDECIMALSEP, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sMonThouSep, NLS_VALUE_SMONTHOUSANDSEP, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sMonGrouping, NLS_VALUE_SMONGROUPING, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iCurrDigits, NLS_VALUE_ICURRDIGITS, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iCurrency, NLS_VALUE_ICURRENCY, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iNegCurr, NLS_VALUE_INEGCURR, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sPosSign, NLS_VALUE_SPOSITIVESIGN, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sNegSign, NLS_VALUE_SNEGATIVESIGN, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        //  Since there is no positive sign in any of the ICURRENCY options, use the INEGCURR options instead.  All known
        //  locales would use the positive sign in the same position as the negative sign.

        //  NOTE:  For the 2 options that use parenthesis, put the positive sign at the beginning of the string
    ...
    {
        //  Use the INEGCURR value from the user portion of the registry, if it exists.
    ...
    {
        //  Use the ICURRENCY value from the user portion of the registry, if it exists.
    ...
    case (LOCALE_INEGSYMPRECEDES) :
    {
        ...
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iNegCurr, NLS_VALUE_INEGCURR, pTemp, TRUE))
        {
            ...
            case (L'1') :
            {
                if ((*(pString + 1) == L'0') || (*(pString + 1) == L'3') || (*(pString + 1) == L'5'))
                {
                    *pString = L'0';
                    *(pString + 1) = 0;
                }
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sTimeFormat, NLS_VALUE_STIMEFORMAT, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    case (LOCALE_STIME) :
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sTime, NLS_VALUE_STIME, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iTime, NLS_VALUE_ITIME, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iTLZero, NLS_VALUE_ITLZERO, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iTimeMarkPosn, NLS_VALUE_ITIMEMARKPOSN, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    case (LOCALE_S1159) :
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->s1159, NLS_VALUE_S1159, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    case (LOCALE_S2359) :
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->s2359, NLS_VALUE_S2359, pTemp, FALSE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sShortDate, NLS_VALUE_SSHORTDATE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    case (LOCALE_SDATE) :
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sDate, NLS_VALUE_SDATE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iDate, NLS_VALUE_IDATE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sYearMonth, NLS_VALUE_SYEARMONTH, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sLongDate, NLS_VALUE_SLONGDATE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    case (LOCALE_ILDATE) :
    {
        ...
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->sLongDate, NLS_VALUE_SLONGDATE, pTemp, TRUE))
        {
            ...
            while (pTmp = wcspbrk(pTmp, L"dMy'"))
            {
                ...
                {
                    //  Found one of the values, so break out of while loop.
                    break;
                }
            }
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iCalType, NLS_VALUE_ICALENDARTYPE, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iFirstDay, NLS_VALUE_IFIRSTDAYOFWEEK, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    {
        Base = 10;
        if (UserOverride && GetUserInfo(Locale, LCType, pNlsUserInfo->iFirstWeek, NLS_VALUE_IFIRSTWEEKOFYEAR, pTemp, TRUE))
        {
            pString = pTemp;
        }
        else
    ...
    case (LOCALE_FONTSIGNATURE) :
    {
        ...
        //  Copy the string to lpLCData and return the number of characters copied.
    ...
    //  See if the caller wants the value in the form of a number instead of a string.
    if (ReturnNum)
    {
    ...
    //  Add one for the null termination.  All strings should be null terminated.
    Length++;
...
//  Sets one of the various pieces of information about a particular locale by making an entry in the user's portion of the configuration registry.
//  This will only affect the user override portion of the locale settings.
//  The system defaults will never be reset.
...
//  07-14-93    JulieB    Created.
{
    ...
    case (LOCALE_SLIST) :
    {
        ...
        return (SetUserInfo(LOCALE_SLIST, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_IMEASURE) :
    {
        //  Validate the new value.  It should be no longer than MAX_IMEASURE wide characters in length.
        ...
        if ((cchData != MAX_IMEASURE) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_IMEASURE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_IMEASURE, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_IPAPERSIZE) :
    {
        ...
        if ((cchData < 2) || (RtlUnicodeStringToInteger(&ObUnicodeStr, 10, &Value)) || (Value < DMPAPER_LETTER) || (Value > DMPAPER_LAST))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_IPAPERSIZE, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SDECIMAL) :
    {
        //  Validate the new value.  It should be no longer than MAX_SDECIMAL wide characters in length and should not
        //  contain any integer values (L'0' thru L'9').
        if (!IsValidSeparatorString(lpLCData, MAX_SDECIMAL, FALSE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SDECIMAL, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_STHOUSAND) :
    {
        //  Validate the new value.  It should be no longer than MAX_STHOUSAND wide characters in length and should not contain any integer values (L'0' thru L'9').
        if (!IsValidSeparatorString(lpLCData, MAX_STHOUSAND, FALSE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_STHOUSAND, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SGROUPING) :
    {
        //  Validate the new value.  It should be no longer than
        //  MAX_SGROUPING wide characters in length and should contain alternating integer and semicolon values.
        //       (eg. 3;2;0  or  3;0  or  0)

        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).
        if (!IsValidGroupingString(lpLCData, MAX_SGROUPING, TRUE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SGROUPING, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_IDIGITS) :
    {
        ...
        if ((cchData != MAX_IDIGITS) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_IDIGITS))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_IDIGITS, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_ILZERO) :
    {
        ...
        if ((cchData != MAX_ILZERO) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_ILZERO))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_ILZERO, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_INEGNUMBER) :
    {
        //  Validate the new value.  It should be no longer than MAX_INEGNUMBER wide characters in length.
        ...
        if ((cchData != MAX_INEGNUMBER) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_INEGNUMBER))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_INEGNUMBER, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SNATIVEDIGITS) :
    {
        //  Validate the new value.  It should be exactly MAX_SNATIVEDIGITS wide characters in length.
        ...
        return (SetUserInfo(LOCALE_SNATIVEDIGITS, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_IDIGITSUBSTITUTION) :
    {
        //  Validate the new value.  It should be no longer than MAX_IDIGITSUBST wide characters in length.
        ...
        if ((cchData != MAX_IDIGITSUBST) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_IDIGITSUBST))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_IDIGITSUBSTITUTION, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SCURRENCY) :
    {
        //  Validate the new value.  It should be no longer than
        //  MAX_SCURRENCY wide characters in length and should not contain any integer values (L'0' thru L'9').
        if (!IsValidSeparatorString(lpLCData, MAX_SCURRENCY, FALSE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SCURRENCY, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SMONDECIMALSEP) :
    {
        //  Validate the new value.  It should be no longer than
        //  MAX_SMONDECSEP wide characters in length and should not contain any integer values (L'0' thru L'9').
        if (!IsValidSeparatorString(lpLCData, MAX_SMONDECSEP, FALSE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SMONDECIMALSEP, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SMONTHOUSANDSEP) :
    {
        //  Validate the new value.  It should be no longer than MAX_SMONTHOUSEP wide characters in length and should not
        //  contain any integer values (L'0' thru L'9').
        if (!IsValidSeparatorString(lpLCData, MAX_SMONTHOUSEP, FALSE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SMONTHOUSANDSEP, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SMONGROUPING) :
    {
        //  Validate the new value.  It should be no longer than MAX_SMONGROUPING wide characters in length and should contain alternating integer and semicolon values.
        //       (eg. 3;2;0  or  3;0  or  0)

        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).
        if (!IsValidGroupingString(lpLCData, MAX_SMONGROUPING, TRUE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SMONGROUPING, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_ICURRDIGITS) :
    {
        ...
        //        2 chars long (includes null).
        RtlInitUnicodeString(&ObUnicodeStr, lpLCData);
        if ((cchData < 2) || (RtlUnicodeStringToInteger(&ObUnicodeStr, 10, &Value)) || (Value < 0) || (Value > MAX_VALUE_ICURRDIGITS) || ((Value == 0) && ((*lpLCData != NLS_CHAR_ZERO) || (cchData != 2))))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_ICURRDIGITS, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_ICURRENCY) :
    {
        //  Validate the new value.  It should be no longer than MAX_ICURRENCY wide characters in length.
        ...
        if ((cchData != MAX_ICURRENCY) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_ICURRENCY))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_ICURRENCY, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_INEGCURR) :
    {
        ...
        //        2 chars long (includes null).
        RtlInitUnicodeString(&ObUnicodeStr, lpLCData);
        if ((cchData < 2) || (RtlUnicodeStringToInteger(&ObUnicodeStr, 10, &Value)) || (Value < 0) || (Value > MAX_VALUE_INEGCURR) || ((Value == 0) && ((*lpLCData != NLS_CHAR_ZERO) || (cchData != 2))))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_INEGCURR, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SPOSITIVESIGN) :
    {
        //  Validate the new value.  It should be no longer than MAX_SPOSSIGN wide characters in length and should not contain any integer values (L'0' thru L'9').
        if (!IsValidSeparatorString(lpLCData, MAX_SPOSSIGN, FALSE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SPOSITIVESIGN, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SNEGATIVESIGN) :
    {
        //  Validate the new value.  It should be no longer than MAX_SNEGSIGN wide characters in length and should not contain any integer values (L'0' thru L'9').
        if (!IsValidSeparatorString(lpLCData, MAX_SNEGSIGN, FALSE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_SNEGATIVESIGN, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_STIMEFORMAT) :
    {
        //  Validate the new value.  It should be no longer than MAX_STIMEFORMAT wide characters in length.

        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).  This is checked below
        //        in the check for whether or not there is an hour delimeter.
        ...
        //  NOTE: Must link the STIME, ITIME, ITLZERO, and ITIMEMARKPOSN values in the registry.

        //  Search for H or h, so that iTime and iTLZero can be set.  If no H or h exists, return an error.
        ...
        //  Search for tt, so that ITIMEMARKPOSN can be set.  If no tt exists, do not change the value.
        ...
        while (pPos = wcspbrk(pPos, L"Hhms"))
        {
            ...
            if (*pPos)
            {
                ...
                if (pPos2)
                {
                    ...
                    {
                        //  Found end of separator, so break out of while loop.
                        break;
                    }
                }
            }
        ...
        //  Validate the new value.  It should be no longer than MAX_STIME wide characters in length and should not contain any integer values (L'0' thru L'9').

        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).
        if (!IsValidSeparatorString(pTemp, MAX_STIME, TRUE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }

        //  Make sure that the time separator does NOT contain any of the special time picture characters - h, H, m, s, t, '.
    ...
    case (LOCALE_STIME) :
    {
        ...
        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).
        if (!IsValidSeparatorString(lpLCData, MAX_STIME, TRUE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        if (GetUserInfo(Locale, LOCALE_STIMEFORMAT, pNlsUserInfo->sTimeFormat, NLS_VALUE_STIMEFORMAT, pOutput, TRUE))
        {
            pString = pOutput;
        }
        else
        {
            pString = (LPWORD) (pHashN->pLocaleHdr) + pHashN->pLocaleHdr->STimeFormat;
        }

        //  Get the current setting for STIME.
        if (GetUserInfo(Locale, LCType, pNlsUserInfo->sTime, NLS_VALUE_STIME, pOutput2, TRUE))
        {
            pSep = pOutput2;
        }
        else
        {
            pSep = (LPWORD) (pHashN->pLocaleHdr) + pHashN->pLocaleHdr->STime;
        }

        //  Get the length of the separator string.
        SepLen = NlsStrLenW(pSep);

        //  Setup the string containing the characters to find in the timeformat string.
        pFind[0] = NLS_CHAR_QUOTE;
        pFind[1] = *pSep;
        pFind[2] = 0;

        //  Find the time separator in the STIMEFORMAT string and replace it with the new time separator.

        //  The new separator may be a different length than the old one, so must use a static buffer for the new time format string.
        pPos = pTemp;
        ...
        *pPos = 0;
        ...
        //  Call the server to set the registry.
        return (SetMultipleUserInfo(LCType, cchData, pTemp, lpLCData, NULL, NULL, NULL));
        break;
    }
    case (LOCALE_ITIME) :
    {
        ...
        //  Validate the new value.  It should be no longer than MAX_ITIME wide characters in length.
        ...
        if ((cchData != MAX_ITIME) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_ITIME))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        else
        {
            ...
            NlsStrCpyW(pTemp, (LPWORD) (pHashN->pLocaleHdr) + pHashN->pLocaleHdr->STimeFormat);
            pString = pTemp;
        }
        ...
        //  Call the server to set the registry.
        return (SetMultipleUserInfo(LCType, cchData, pString, NULL, lpLCData, NULL, NULL));
        break;
    }
    case (LOCALE_S1159) :
    {
        //  Validate the new value.  It should be no longer than MAX_S1159 wide characters in length.
        ...
        return (SetUserInfo(LOCALE_S1159, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_S2359) :
    {
        //  Validate the new value.  It should be no longer than MAX_S2359 wide characters in length.
        ...
        return (SetUserInfo(LOCALE_S2359, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SSHORTDATE) :
    {
        //  Validate the new value.  It should be no longer than MAX_SSHORTDATE wide characters in length.
        ...
        //  Search for the 'd' or 'M' or 'y' sequence in the date format string to set the new IDATE value.

        //  If none of these symbols exist in the date format string, then return an error.
        ...
        *pTemp = 0;
        ...
        // Since the date separator (LOCALE_SDATE) is being set here, we should do the same validation as LOCALE_SDATE.
        if (!IsValidSeparatorString(pTemp, MAX_SDATE, TRUE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }

        //  Make sure that the date separator does NOT contain any of the special date picture characters - d, M, y, g, '.
    ...
    case (LOCALE_SDATE) :
    {
        ...
        //  Validate the new value.  It should be no longer than MAX_SDATE wide characters in length and should not contain any integer values (L'0' thru L'9').

        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).
        if (!IsValidSeparatorString(lpLCData, MAX_SDATE, TRUE))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        if (GetUserInfo(Locale, LOCALE_SSHORTDATE, pNlsUserInfo->sShortDate, NLS_VALUE_SSHORTDATE, pOutput, TRUE))
        {
            pString = pOutput;
        }
        else
        {
            pString = (LPWORD) (pHashN->pLocaleHdr) + pHashN->pLocaleHdr->SShortDate;
        }

        //  Get the current setting for SDATE.
        if (GetUserInfo(Locale, LCType, pNlsUserInfo->sDate, NLS_VALUE_SDATE, pOutput2, TRUE))
        {
            pSep = pOutput2;
        }
        else
        {
            pSep = (LPWORD) (pHashN->pLocaleHdr) + pHashN->pLocaleHdr->SDate;
        }

        //  Get the length of the separator string.
        SepLen = NlsStrLenW(pSep);

        //  Setup the string containing the characters to find in the shortdate string.
        pFind[0] = NLS_CHAR_QUOTE;
        pFind[1] = *pSep;
        pFind[2] = 0;

        //  Find the date separator in the SSHORTDATE string and replace it with the new date separator.

        //  The new separator may be a different length than the old one, so must use a static buffer for the new short date format string.
        pPos = pTemp;
        while (pPos2 = wcspbrk(pString, pFind))
        {
            //  Copy format string up to pPos2.
    ...
    case (LOCALE_SYEARMONTH) :
    {
        //  Validate the new value.  It should be no longer than MAX_SYEARMONTH wide characters in length.

        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).  This is checked below
        //        in the check for whether or not there is a date, month, or year delimeter.
        ...
        return (SetUserInfo(LOCALE_SYEARMONTH, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_SLONGDATE) :
    {
        //  Validate the new value.  It should be no longer than MAX_SLONGDATE wide characters in length.
        ...
        //  Make sure one of 'd' or 'M' or 'y' exists in the date format string.  If it does not, then return an error.
        pPos = wcspbrk(lpLCData, L"dMy");
        if (!pPos)
        {
            SetLastError(ERROR_INVALID_PARAMETER);
        ...
        return (SetUserInfo(LOCALE_SLONGDATE, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_ICALENDARTYPE) :
    {
        //  Validate the new value.  It should be no longer than MAX_ICALTYPE wide characters in length.

        //  NOTE: The string may not be NULL, so it must be at least
        //        2 chars long (includes null).
        if ((cchData < 2) || (cchData > MAX_ICALTYPE) || (!IsValidCalendarTypeStr(pHashN, lpLCData)))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_ICALENDARTYPE, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_IFIRSTDAYOFWEEK) :
    {
        //  Validate the new value.  It should be no longer than MAX_IFIRSTDAY wide characters in length.
        ...
        if ((cchData != MAX_IFIRSTDAY) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_IFIRSTDAY))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_IFIRSTDAYOFWEEK, (LPWSTR) lpLCData, cchData));
        break;
    }
    case (LOCALE_IFIRSTWEEKOFYEAR) :
    {
        //  Validate the new value.  It should be no longer than MAX_IFIRSTWEEK wide characters in length.
        ...
        if ((cchData != MAX_IFIRSTWEEK) || (*lpLCData < NLS_CHAR_ZERO) || (*lpLCData > MAX_CHAR_IFIRSTWEEK))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (FALSE);
        }
        ...
        return (SetUserInfo(LOCALE_IFIRSTWEEKOFYEAR, (LPWSTR) lpLCData, cchData));
        break;
    }
    default:
    {
...
//  Returns one of the various pieces of information about a particular calendar by querying the configuration registry.
//  This call also indicates how much memory is necessary to contain the desired information.
...
//  12-17-97    JulieB    Created.
{
    ...
    //    - NULL data pointer AND count is not zero
    VALIDATE_LOCALE(Locale, pHashN, FALSE);
    if ((pHashN == NULL) || (cchData < 0) || ((lpCalData == NULL) && (cchData != 0)))
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return (0);
    }
    ...
    {
        if ((lpValue != NULL) || (cchData < 0) || ((lpCalData == NULL) && (cchData != 0)))
        {
            SetLastError(ERROR_INVALID_PARAMETER);
            return (0);
        }
    ...
    {
        //  Flag is set, so set the boolean value and remove the flag from the CalType parameter (for switch statement).
        UserOverride = FALSE;
    }
    if (CalType & CAL_RETURN_NUMBER)
    {
    ...
    if (((CalType != CAL_ITWODIGITYEARMAX) && ((pOptCal = IsValidCalendarType(pHashN, Calendar)) == NULL)) || (GetCalendar(Calendar, &pCalInfo) != NO_ERROR))
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return (0);
    }
    ...
    //  If user information exists for the given CALTYPE, then the user default is returned instead of the system default.
    switch (CalType)
    {
        ...
        case ( CAL_ITWODIGITYEARMAX ) :
        {
            ...
            // Check if a policy is enforced for the current user, and if so, let's use it.
            ...
            {
                if (UserOverride && GetTwoDigitYearInfo(Calendar, pTemp, NLS_TWO_DIGIT_YEAR_KEY))
                {
                    pString = pTemp;
                }
                else
        ...
        case ( CAL_SERASTRING ) :
        {
            ...
            //  Make sure the string is NOT empty.  If it is, return the empty string.
            if (*pString)
            {
                pString = ((PERA_RANGE)pString)->pYearStr + NlsStrLenW(((PERA_RANGE)pString)->pYearStr) + 1;
            }
        ...
        case ( CAL_SSHORTDATE ) :
        {
            ...
            //  Make sure the string is NOT empty.  If it is, use the locale's short date string.
            if (*pString == 0)
            {
                pString = (LPWORD)(pHashN->pLocaleHdr) + pHashN->pLocaleHdr->SShortDate;
            }
        ...
        case ( CAL_SLONGDATE ) :
        {
            ...
            //  Make sure the string is NOT empty.  If it is, use the locale's long date string.
            if (*pString == 0)
            {
                pString = (LPWORD)(pHashN->pLocaleHdr) + pHashN->pLocaleHdr->SLongDate;
            }
        ...
        case ( CAL_SYEARMONTH ) :
        {
            ...
            //  Make sure the string is NOT empty.  If it is, use the locale's year month string.
            if (*pString == 0)
            {
                pString = (LPWORD)(pHashN->pLocaleHdr) + pHashN->pLocaleHdr->SYearMonth;
            }
        ...
        case ( CAL_SABBREVMONTHNAME13 ) :
        {
            ...
            {
                pString = (LPWORD)pCalInfo + *((LPWORD)((LPBYTE)(pCalInfo) + (FIELD_OFFSET(CALENDAR_VAR, SDayName1) + ((CalType - CAL_SDAYNAME1) * sizeof(WORD)))));
            }

            //  Make sure the string is NOT empty.  If it is, use the locale's string.
            if ((pString == NULL) || (*pString == 0))
            {
                pString = (LPWORD)(pHashN->pLocaleHdr) + *((LPWORD)((LPBYTE)(pHashN->pLocaleHdr) + (FIELD_OFFSET(LOCALE_VAR, SDayName1) + ((CalType - CAL_SDAYNAME1) * sizeof(WORD)))));
            }
    ...
    if (ReturnNum)
    {
        //  Make sure the flags are valid and that the DWORD buffer is not NULL.
        if (Base == 0)
        {
            SetLastError(ERROR_INVALID_FLAGS);
            return (0);
...
//  calendar by making an entry in the user's portion of the configuration registry.
//  This will only affect the user override portion of the calendar settings.
//  The system defaults will never be reset.
...
//  12-17-97    JulieB    Created.
{
    ...
    cchData = NlsStrLenW(lpCalData) + 1;//  Get the length of the buffer.
    ...
    switch (CalType)
    {
        case ( CAL_ITWODIGITYEARMAX ) :
        {
            //  Get the default value to make sure the calendar is allowed to be set.  Things like the Japanese Era calendar may not be set.
            RtlInitUnicodeString( &ObUnicodeStr, ((LPWORD)pCalInfo + (((PCALENDAR_VAR)pCalInfo)->STwoDigitYearMax)) );
            ...
            //  Validate the new value.  It should be no longer than MAX_ITWODIGITYEAR wide characters in length.
            //  It should be between 99 and 9999.
            if ((cchData > MAX_ITWODIGITYEAR) || (cchData < 3) || ((cchData == 3) && ((*lpCalData != NLS_CHAR_NINE) || (*(lpCalData + 1) != NLS_CHAR_NINE))))
            {
                SetLastError(ERROR_INVALID_PARAMETER);
                return (FALSE);
            }

            return (SetTwoDigitYearInfo(Calendar, lpCalData, cchData));//  Set the registry with the new TwoDigitYearMax string.
            break;
        }
        default :
        {
...
int GetLocalizedLanguageName(LANGID Language, LPWSTR *ppLangName)
//  Returns the localized version of the language name for the given language id.
//  It gets the information from the resource file in the language that the current user is using.

//  05-31-91    JulieB    Created.
{
    //  Make sure we have a language id that is either 0x0000 or greater than 0x0400.
    if ((Language == 0) || (Language >= LANG_USER_DEFAULT))
    {
        return (GetNameFromStringTable(Language, ppLangName));//  Get the language name from the string table.
    }

    return (0);//  If we get here, there is an invalid language id, so return 0.
}


BOOL GetLocalizedCountryName(LANGID Language, LPWSTR *ppCtryName)
//  Returns the localized version of the country name for the given language id.
//  It gets the information from the resource file in the language that the current user is using.
...
//  05-31-91    JulieB    Created.
{
    ...
    if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, MAKEINTRESOURCEW(Language), (WORD)LangId ))))
    {
        //   Could not find resource.  Try NEUTRAL language id.
        if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, MAKEINTRESOURCEW(Language), (WORD)0 ))))
        {
    ...
    {
        //  Lock the resource.  Store the found pointer in the given country name pointer.
...
BOOL GetLocalizedSortName(LCID Locale, LPWSTR *ppSortName)
//  Returns the localized version of the sort name for the given locale id.
//  It gets the information from the resource file in the language that the current user is using.
...
//  11-15-96    JulieB    Created.
{
    ...
    //  Find the resource.
    LangId = LANGIDFROMLCID(GetUserDefaultLCID());
    if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, pName, (WORD)LangId ))))
    {
        //  Could not find resource.  Try NEUTRAL language id.
        if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, pName, (WORD)0 ))))
        {
            ...
            if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, pName, (WORD)LangId ))))
            {
                ...
                if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, pName, (WORD)0 ))))
                {
                    ...
                    if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, NLS_SORT_RES_DEFAULT, (WORD)LangId ))))
                    {
                        //  Could not find resource.  Try NEUTRAL language id.
                        if ((!(hFindRes = FindResourceExW( hModule, RT_RCDATA, NLS_SORT_RES_DEFAULT, (WORD)0 ))))
                        {
                            return (FALSE);//  Could not find resource.  Return failure.
                        }
                    }
                }
            }
    ...
    if (hLoadRes = LoadResource(hModule, hFindRes))
    {
        //  Lock the resource.  Store the found pointer in the given sort name pointer.
...
//  07-14-93    JulieB    Created.
{
    ...
    DataLength *= sizeof(WCHAR);//  Get the length of the value string.

    // If there is no logged on user or the current security context
    // isn't the logged-on interactive user, then set the registry value directly.
    if ((! pNlsUserInfo->fCacheValid ) || (! NT_SUCCESS( NlsCheckForInteractiveUser() )))
    {
    ...
    {
        //  We got a failure.  Try using just the registry apis to set the registry.
        //  It's possible that the cache is not valid yet if this is called from setup or winlogon.
        return (SetCurrentUserRegValue(LCType, pData, DataLength));
    }
...
BOOL SetCurrentUserRegValue(LCTYPE   LCType, LPWSTR pData, ULONG DataLength)
//  Set the registry value for the current security context.
//  This routine is called when the current security context is different from the logged on user.
...
//  12-26-98    SamerA    Created.
{
    ...
    CLOSE_REG_KEY(hKey);
    NlsFlushProcessCache(LCType);// Flush the process cache entry, if needed.
    return (TRUE);
}


BOOL SetMultipleUserInfoInRegistry(DWORD dwFlags, int cchData, LPCWSTR pPicture, LPCWSTR pSeparator, LPCWSTR pOrder, LPCWSTR pTLZero, LPCWSTR pTimeMarkPosn)
//  This routine sets the given multiple values in the registry with the given data.
//  All values must be of the type REG_SZ.
...
//  06-11-98    JulieB    Created.
{
    ...
    OPEN_CPANEL_INTL_KEY(hKey, FALSE, KEY_READ | KEY_WRITE);//  Open the Control Panel International registry key.
    ...
    case (LOCALE_STIMEFORMAT) :
    {
        rc = SetRegValue(hKey, NLS_VALUE_STIMEFORMAT, pPicture, cchData * sizeof(WCHAR));
        if (NT_SUCCESS(rc))
        {
            NlsFlushProcessCache(LOCALE_STIMEFORMAT);
            rc = SetRegValue(hKey, NLS_VALUE_STIME, pSeparator, (lstrlen(pSeparator) + 1) * sizeof(WCHAR));
        }
        if (NT_SUCCESS(rc))
        {
            NlsFlushProcessCache(LOCALE_STIME);
            rc = SetRegValue(hKey, NLS_VALUE_ITIME, pOrder, (lstrlen(pOrder) + 1) * sizeof(WCHAR));
        }
        if (NT_SUCCESS(rc))
        {
            NlsFlushProcessCache(LOCALE_ITIME);
            rc = SetRegValue(hKey, NLS_VALUE_ITLZERO, pTLZero, (lstrlen(pTLZero) + 1) * sizeof(WCHAR));
        }
        if (NT_SUCCESS(rc))
        {
    ...
    case (LOCALE_STIME) :
    {
        rc = SetRegValue(hKey, NLS_VALUE_STIME, pSeparator, cchData * sizeof(WCHAR));
        if (NT_SUCCESS(rc))
        {
            NlsFlushProcessCache(LOCALE_STIME);
    ...
    case (LOCALE_ITIME) :
    {
        rc = SetRegValue(hKey, NLS_VALUE_ITIME, pOrder, cchData * sizeof(WCHAR));
        if (NT_SUCCESS(rc))
        {
            NlsFlushProcessCache(LOCALE_ITIME);
            rc = SetRegValue(hKey, NLS_VALUE_STIMEFORMAT, pPicture, (lstrlen(pPicture) + 1) * sizeof(WCHAR));
    ...
    case (LOCALE_SSHORTDATE) :
    {
        rc = SetRegValue(hKey, NLS_VALUE_SSHORTDATE, pPicture, cchData * sizeof(WCHAR));
        if (NT_SUCCESS(rc))
        {
            NlsFlushProcessCache(LOCALE_SSHORTDATE);
            rc = SetRegValue(hKey, NLS_VALUE_SDATE, pSeparator, (lstrlen(pSeparator) + 1) * sizeof(WCHAR));
        }
        if (NT_SUCCESS(rc))
        {
    ...
    case (LOCALE_SDATE) :
    {
        rc = SetRegValue(hKey, NLS_VALUE_SDATE, pSeparator, cchData * sizeof(WCHAR));
        if (NT_SUCCESS(rc))
        {
            NlsFlushProcessCache(LOCALE_SDATE);
    ...
    return (rc == NO_ERROR);//  Return the result.
}


BOOL SetMultipleUserInfo(DWORD dwFlags, int cchData, LPCWSTR pPicture, LPCWSTR pSeparator, LPCWSTR pOrder, LPCWSTR pTLZero, LPCWSTR pTimeMarkPosn)
//  This routine calls the server to set multiple registry values.
//  This way, only one client/server transition is necessary.

//  08-19-94    JulieB    Created.
{
    NTSTATUS Status;

    // If there is no logged on user or the current security context isn't the logged-on interactive user, then set the registry value directly.
    if ((! pNlsUserInfo->fCacheValid ) || (! NT_SUCCESS( NlsCheckForInteractiveUser() )))
    {
        if (SetMultipleUserInfoInRegistry( dwFlags, cchData, pPicture, pSeparator, pOrder, pTLZero, pTimeMarkPosn ) == FALSE)
        {
            SetLastError(ERROR_INVALID_ACCESS);
            return (FALSE);
        }
    ...
    if (!NT_SUCCESS(Status))
    {
        ...
        //  registry.  It's possible that the cache is not valid yet if this is called from setup or winlogon.
        if (SetMultipleUserInfoInRegistry( dwFlags, cchData, pPicture, pSeparator, pOrder, pTLZero, pTimeMarkPosn ) == FALSE)
        {
            SetLastError(ERROR_INVALID_ACCESS);
            return (FALSE);
...
BOOL GetTwoDigitYearInfo(CALID Calendar, LPWSTR pYearInfo, PWSTR pwszKeyPath)
...
//  12-17-97    JulieB    Created.
{
    ...
    if (OpenRegKey( &hKey, NULL, pwszKeyPath, KEY_READ ) != NO_ERROR)
    {
        return (FALSE);
    }
    ...
    pKeyValueFull = (PKEY_VALUE_FULL_INFORMATION)pStatic;
    rc = QueryRegValue( hKey, pCalStr, &pKeyValueFull, MAX_KEY_VALUE_FULLINFO, &IfAlloc );

    NtClose(hKey);//  Close the registry key.
    ...
    if (pKeyValueFull->DataLength > 2)
    {
        NlsStrCpyW(pYearInfo, GET_VALUE_DATA_PTR(pKeyValueFull));//  Copy the info.

        //  Make sure the value is between 99 and 9999.
        RtlInitUnicodeString(&ObUnicodeStr, pYearInfo);
        if ((RtlUnicodeStringToInteger(&ObUnicodeStr, 10, &Year) == NO_ERROR) && (Year >= 99) && (Year <= 9999))
        {
            bResult = TRUE;
        }
    }
    ...
    return (bResult);//  Return the result.
}
...
//  12-17-97    JulieB    Created.
{
    ...
    //  Open the Control Panel International registry key.
    //  If it doesn't exist, then we have to create each subkey separately.
    if (OpenRegKey( &hKey, NULL, NLS_TWO_DIGIT_YEAR_KEY, KEY_READ | KEY_WRITE ) != NO_ERROR)
    {
        //  Registry key does not exist, so create each subkey separately.
        if (CreateRegKey( &hKey, NULL, NLS_CALENDARS_KEY, KEY_READ | KEY_WRITE ) == NO_ERROR)
        {
            if (CreateRegKey( &hKey, NULL, NLS_TWO_DIGIT_YEAR_KEY, KEY_READ | KEY_WRITE ) != NO_ERROR)
            {
                return (FALSE);
            }
        }
    ...
    switch (Calendar)
    {
        ...
        default :
        {
            ...
            rc = SetRegValue(hKey, pCalStr, pYearInfo, (ULONG)cchData * sizeof(WCHAR));//  Set the TwoDigitYearMax value in the registry.
            break;
        }
    }
