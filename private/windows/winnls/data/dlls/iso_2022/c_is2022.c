...
//  10-30-96    JulieB    Created.
BOOL DllEntry(HANDLE hModule, DWORD dwReason, LPVOID lpRes)
{
    switch (dwReason)
    {
    ...
    LPCPINFO lpCPInfo)
{
    ...
    switch (dwFlags)
    {
        ...
        case ( NLS_CP_MBTOWC ) :
        {
            ...
            switch (CodePage)
            {
                ...
                case (50229) :
                {
                    return (MBToWC_CP5022X( lpMultiByteStr, cchMultiByte, lpWideCharStr, cchWideChar ));
                }
                case (52936) :
                {
                    return (MBToWC_CP52936( lpMultiByteStr, cchMultiByte, lpWideCharStr, cchWideChar ));
                }
            }
            break;
        }
        case ( NLS_CP_WCTOMB ) :
        {
            ...
            cchMBCount = WideCharToMultiByte( NlsCodePage, WC_NO_BEST_FIT_CHARS, lpWideCharStr, cchWideChar, lpMBNoEscStr, cchMultiByte, NULL, NULL );
            if (cchMBCount != 0)
            {
                switch (CodePage)
                {
                    ...
                    case (50222) :
                    {
                        cchMBCount = ParseMB_CP5022J( CodePage, lpMultiByteStr, cchMultiByte, lpMBNoEscStr, cchMBCount );
                        break;
                    }
                    case (50225) :
                    case (50227) :
                    case (50229) :
                    {
                        cchMBCount = ParseMB_CP5022_579( CodePage, lpMultiByteStr, cchMultiByte, lpMBNoEscStr, cchMBCount );
                        break;
                    }
                    case (52936) :
                    {
                        cchMBCount = ParseMB_CP52936( lpMultiByteStr, cchMultiByte, lpMBNoEscStr, cchMBCount );
                        break;
                    }
                }
            }
    ...
    //  This shouldn't happen since this function gets called by the NLS API routines.
    SetLastError(ERROR_INVALID_PARAMETER);
    return (0);
}
...
DWORD MBToWC_CP5022X(LPSTR lpMultiByteStr, int cchMultiByte, LPWSTR lpWideCharStr, int cchWideChar)
{
