...
//  Time format buffers must be in line with the pAllLocales global buffer.
LPWSTR pTimeAM[] =
{
    L"03:45:25",                                 //  0x0402
...
int TestGetTimeFormat()
{
    ...
    ErrCount += GTF_BadParamCheck();//  Test bad parameters.
    ErrCount += GTF_NormalCase();//  Test normal cases.
    ErrCount += GTF_Ansi();//  Test Ansi version.
...
int GTF_BadParamCheck()
{
    ...
    rc = GetTimeFormatW( (LCID)333, 0, NULL, NULL, lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_PARAMETER, "Bad Locale", &NumErrors );

    //  Null Pointers.

    //  Variation 1  -  lpTimeStr = NULL
    rc = GetTimeFormatW( Locale, 0, NULL, NULL, NULL, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_PARAMETER, "lpTimeStr NULL", &NumErrors );

    //  Bad Count.

    //  Variation 1  -  cchTime < 0
    rc = GetTimeFormatW( Locale, 0, NULL, NULL, lpTimeStr, -1 );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_PARAMETER, "cchTime < 0", &NumErrors );

    //  Invalid Flag.

    //  Variation 1  -  LCType = invalid
    rc = GetTimeFormatW( Locale, GTF_INVALID_FLAGS, NULL, NULL, lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_FLAGS, "Flag invalid", &NumErrors );

    //  Variation 2  -  lpFormat and NoUserOverride
    rc = GetTimeFormatW( Locale, LOCALE_NOUSEROVERRIDE, NULL, L"tt hh:mm:ss", lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_FLAGS, "lpFormat and NoUserOverride", &NumErrors );

    //  Variation 3  -  Use CP ACP, lpFormat and NoUserOverride
    rc = GetTimeFormatW( Locale, LOCALE_USE_CP_ACP | LOCALE_NOUSEROVERRIDE, NULL, L"tt hh:mm:ss", lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_FLAGS, "Use CP ACP, lpFormat and NoUserOverride", &NumErrors );

    //  Buffer Too Small.

    //  Variation 1  -  cchTime = too small
    rc = GetTimeFormatW( Locale, 0, NULL, NULL, lpTimeStr, 2 );
    CheckReturnBadParam( rc, 0, ERROR_INSUFFICIENT_BUFFER, "cchTime too small", &NumErrors );
    ...
    rc = GetTimeFormatW( Locale, 0, &MyTime, NULL, lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_PARAMETER, "bad wHour", &NumErrors );
    ...
    MyTime.wMilliseconds = 13;
    rc = GetTimeFormatW( Locale, 0, &MyTime, NULL, lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_PARAMETER, "bad wMinute", &NumErrors );
    ...
    MyTime.wMilliseconds = 13;
    rc = GetTimeFormatW( Locale, 0, &MyTime, NULL, lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_PARAMETER, "bad wSecond", &NumErrors );
    ...
    rc = GetTimeFormatW( Locale, 0, &MyTime, NULL, lpTimeStr, BUFSIZE );
    CheckReturnBadParam( rc, 0, ERROR_INVALID_PARAMETER, "bad wMilliseconds", &NumErrors );
