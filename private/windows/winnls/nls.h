...
Abstract:
    This file contains the header information shared by all of the modules of NLS.
...
#define MAX_KEY_VALUE_FULLINFO                                             ( FIELD_OFFSET( KEY_VALUE_FULL_INFORMATION, Name ) + MAX_PATH_LEN )
...
#define GET_DWORD_WEIGHT(pHashN, wch)                                          ( MAKE_SORTKEY_DWORD(((pHashN)->pSortkey)[wch]) )
#define GET_EXPANSION_1(pwt)                                                   ( ((pTblPtrs->pExpansion)[GET_EXPAND_INDEX(pwt)]).UCP1 )
#define GET_EXPANSION_2(pwt)                                                   ( ((pTblPtrs->pExpansion)[GET_EXPAND_INDEX(pwt)]).UCP2 )
...
#define IS_KOREAN(lcid)                                                        ( LANGIDFROMLCID(lcid) == MAKELANGID(LANG_KOREAN, SUBLANG_KOREAN) )
...
ULONG AllocTables(void);
ULONG GetUnicodeFileInfo(void);
ULONG GetCTypeFileInfo(void);
ULONG GetDefaultSortkeyFileInfo(void);
ULONG GetDefaultSortTablesFileInfo(void);
...
ULONG CreateNlsObjectDirectory(void);
...
ULONG QueryRegValue(HANDLE hKeyHandle, LPWSTR pValue, PKEY_VALUE_FULL_INFORMATION *ppKeyValueFull, ULONG Length, LPBOOL pIfAlloc);
ULONG SetRegValue(HANDLE hKeyHandle, LPCWSTR pValue, LPCWSTR pData, ULONG DataLength);
ULONG CreateSectionTemp(HANDLE *phSec, LPWSTR pwszFileName);
ULONG OpenSection(HANDLE *phSec, PUNICODE_STRING pObSectionName, PVOID *ppBaseAddr, ULONG AccessMask, BOOL bCloseHandle);
ULONG MapSection(HANDLE hSec, PVOID *ppBaseAddr, ULONG PageProtection, BOOL bCloseHandle);
ULONG UnMapSection(PVOID pBaseAddr);
ULONG GetNlsSectionName(UINT Value, UINT Base, UINT Padding, LPWSTR pwszPrefix, LPWSTR pwszSecName);
ULONG GetCodePageDLLPathName(UINT CodePage, LPWSTR pDllName, USHORT cchLen);

//  Utility Routines - util.c.
BOOL GetUserInfoFromRegistry(LPWSTR pValue, LPWSTR pOutput);
int GetNameFromStringTable(WORD ResourceID, LPWSTR *ppName);
BOOL IsValidSeparatorString(LPCWSTR pString, ULONG MaxLength, BOOL fCheckZeroLen);
BOOL IsValidGroupingString(LPCWSTR pString, ULONG MaxLength, BOOL fCheckZeroLen);
LPWORD IsValidCalendarType(PLOC_HASH pHashN, CALID CalId);
LPWORD IsValidCalendarTypeStr(PLOC_HASH pHashN, LPCWSTR pCalStr);
BOOL GetUserInfo(LCID Locale, LCTYPE LCType , LPWSTR pCacheString, LPWSTR pValue, LPWSTR pOutput, BOOL fCheckNull);
WCHAR FASTCALL GetPreComposedChar(WCHAR wcNonSp, WCHAR wcBase);
BOOL FASTCALL GetCompositeChars(WCHAR wch, WCHAR *pNonSp, WCHAR *pBase);
int FASTCALL InsertPreComposedForm(LPCWSTR pWCStr, LPWSTR pEndWCStr, LPWSTR pPreComp);
int FASTCALL InsertFullWidthPreComposedForm(LPCWSTR pWCStr, LPWSTR pEndWCStr, LPWSTR pPreComp, PCASE pCase);
int FASTCALL InsertCompositeForm(LPWSTR pWCStr, LPWSTR pEndWCStr);
ULONG NlsConvertIntegerToString(UINT Value, UINT Base, UINT Padding, LPWSTR pResultBuf, UINT Size);
LPWSTR FASTCALL NlsStrCpyW(LPWSTR pwszDest, LPCWSTR pwszSrc);
LPWSTR FASTCALL NlsStrCatW(LPWSTR pwsz1, LPCWSTR pwsz2);
int FASTCALL NlsStrLenW(LPCWSTR pwsz);
LPWSTR FASTCALL NlsStrNCatW(LPWSTR pwszFront, LPCWSTR pwszBack, int Count);
int FASTCALL NlsStrEqualW(LPCWSTR pwszFirst, LPCWSTR pwszSecond);
int FASTCALL NlsStrNEqualW(LPCWSTR pwszFirst, LPCWSTR pwszSecond, int Count);

//  Security Routines - security.c.
NTSTATUS NlsCheckForInteractiveUser();
NTSTATUS NlsIsInteractiveUserProcess();
NTSTATUS NlsGetUserLocale(LCID *Lcid);
NTSTATUS NlsGetCurrentUserNlsInfo(LCID Locale, LCTYPE LCType, PWSTR RegistryValue, PWSTR pOutputBuffer, BOOL IgnoreLocaleValue);
NTSTATUS NlsQueryCurrentUserInfo(PNLS_LOCAL_CACHE pNlsCache, LPWSTR pValue, LPWSTR pOutput);
NTSTATUS NlsFlushProcessCache(LCTYPE LCType);

//  Internal Enumeration routines - enum.c.
BOOL Internal_EnumSystemLanguageGroups(NLS_ENUMPROC lpLanguageGroupEnumProc, DWORD dwFlags, LONG_PTR lParam, BOOL fUnicodeVer);
BOOL Internal_EnumLanguageGroupLocales(NLS_ENUMPROC lpLangGroupLocaleEnumProc, LGRPID LanguageGroup, DWORD dwFlags, LONG_PTR lParam, BOOL fUnicodeVer);
BOOL Internal_EnumUILanguages(NLS_ENUMPROC lpUILanguageEnumProc, DWORD dwFlags, LONG_PTR lParam, BOOL fUnicodeVer);
...
BOOL Internal_EnumCalendarInfo(NLS_ENUMPROC lpCalInfoEnumProc, LCID Locale, CALID Calendar, CALTYPE CalType, BOOL fUnicodeVer, BOOL fExVersion);
BOOL Internal_EnumTimeFormats(NLS_ENUMPROC lpTimeFmtEnumProc, LCID Locale, DWORD dwFlags, BOOL fUnicodeVer);
BOOL Internal_EnumDateFormats(NLS_ENUMPROC lpDateFmtEnumProc, LCID Locale, DWORD dwFlags, BOOL fUnicodeVer, BOOL fExVersion);
...
//  all instance-specific.  In general, there shouldn't be much reason to create instance globals.

//  Globals are included last because they may require some of the types being defined above.
...
NTSTATUS CsrBasepNlsUpdateCacheCount(VOID);
#endif   // _NLS_