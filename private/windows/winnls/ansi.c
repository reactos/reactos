    ...
    int cchTime)
{
    ...
    while (1)
    {
        ResultLen = GetTimeFormatW( Locale, dwFlags, lpTime, pUnicode, pBuf, ResultLen );
    ...
    int *pUnicodeLength)
{
    ...
    if ((*ppUnicodeBuffer == NULL) || (AnsiLength > (MAX_STRING_LEN - 1)))
    {
        ...
        if ((pUnicode = (LPWSTR)NLS_ALLOC_MEM((UnicodeLength + 1) * sizeof(WCHAR) )) == NULL)
        {
            SetLastError(ERROR_OUTOFMEMORY);
            return (FALSE);
        }
    ...
    if (ResultLength == 0)
    {
        ...
        {
            //  Get the size of the buffer needed to hold the Unicode string.
    ...
    ASSERT(ResultLength <= (int)UnicodeLength);//  Make sure there is room in the buffer for the null terminator.
    pUnicode[ResultLength] = UNICODE_NULL;//  Null terminate the string.
...
//  Ansi buffer passed in (unless AnsiLength is 0) and checks for buffer overflow.

//  11-10-93    JulieB    Created.
int NlsUnicodeToAnsi(PCP_HASH pHashN, LPCWSTR pUnicodeBuffer, int UnicodeLength, LPSTR pAnsiBuffer, int AnsiLength)
{
    //  Convert the Unicode string to an Ansi string and return the result.
    //  The last error will be set appropriately by WideCharToMultiByte.
...
BOOL NlsEnumUnicodeToAnsi(PCP_HASH pHashN, LPCWSTR pUnicodeBuffer, LPSTR *ppAnsiBuffer)
{
    ...
    //  Get the length of the Unicode string (in bytes), including the null terminator.
    ...
    if (ResultLength == 0)
    {
        ...
        if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
        {
            //  Get the size of the buffer needed to hold the ansi string.
    ...
    BOOL fVersion)
{
    ...
    //  Convert the null-terminated Unicode string to a null-terminated Ansi string.
    if (!NlsEnumUnicodeToAnsi( pHashN, pUnicodeBuffer1, &pAnsiBuffer1 ))
    {
        return (FALSE);
    }

    if ((pUnicodeBuffer2 != NULL) && (!NlsEnumUnicodeToAnsi( pHashN, pUnicodeBuffer2, &pAnsiBuffer2 )))
    {
    ...
    switch (fVersion)
    {
        ...
        case ( 2 ) :
        {
            rc = (*((NLS_ENUMPROC2)pNlsEnumProc))( dwValue1, dwValue2, pAnsiBuffer1, lParam );
            break;
        }
        case ( 3 ) :
        {
            rc = (*((NLS_ENUMPROC3)pNlsEnumProc))( dwValue1, pAnsiBuffer1, pAnsiBuffer2, dwValue2, lParam );
            break;
        }
        case  ( 4 ) :
        {
            rc = (*((NLS_ENUMPROC4)pNlsEnumProc))( pAnsiBuffer1, lParam );
            break;
