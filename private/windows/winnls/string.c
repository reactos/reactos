...
int LongCompareStringW(PLOC_HASH pHashN, DWORD dwCmpFlags, LPCWSTR lpString1, int cchCount1, LPCWSTR lpString2, int cchCount2, BOOL fModify);
...
//  It returns TRUE if the counter is not at zero (counting backwards) and the null termination has not been reached (if -1 was passed in the count parameter.

//  11-04-92    JulieB    Created.
#define NOT_END_STRING(ct, ptr, cchIn)     ((ct != 0) && (!((*(ptr) == 0) && (cchIn == -2))))


//  Checks to see if the pointer is at the end of the string.
//  It returns TRUE if the counter is zero or if the null termination has been reached (if -2 was passed in the count parameter).

//  11-04-92    JulieB    Created.
#define AT_STRING_END(ct, ptr, cchIn)             ((ct == 0) || ((*(ptr) == 0) && (cchIn == -2)))


//  Removes the current state from the state table.
//  This should only be called when the current state should not be entered for the remainder of the comparison.
//  It decrements the counter going through the state table and decrements the number of states in the table.
...
//  11-04-92    JulieB    Created.
#define SCAN_LONGER_STRING( ct, ptr, cchIn, ret )                          \
{                                                                          \
    /*                                                                     \
...
//  11-04-92    JulieB    Created.
#define QUICK_SCAN_LONGER_STRING( ptr, ret )                               \
{                                                                          \
    /*                                                                     \
     *  Search through the rest of the longer string to make sure          \
     *  all characters are not to be ignored.  If find a character that    \
...
//  Returns the weight for the far east special case in "wt".
//  This currently includes the Cho-on, the Repeat, and the Kana characters.

//  08-19-93    JulieB    Created.
#define GET_FAREAST_WEIGHT( wt, uw, mask, pBegin, pCur, ExtraWt, fModify ) \
{                                                                          \
    int ct;                       /* loop counter */                       \
            ...
            {                                                              \
                ...
                {                                                          \
                    ...
                    continue;                                              \
                                                                           ...
                                                                           \
                UNICODE_WT(&PrevWt) = MAKE_UNICODE_WT(KANA, PrevAW, fModify); \
                                                                           \
                /*                                                         \
...
//  Compares two wide character strings of the same locale according to the supplied locale handle.
...
//  05-31-91    JulieB    Created.
{
    ...
    if ( (pHashN == NULL) || (pHashN->IfCompression) || (cchCount1 > -1) || (cchCount2 > -1) || (dwCmpFlags > NORM_IGNORECASE) || (fModify == TRUE) )
    {
        return (LongCompareStringW( pHashN, dwCmpFlags, lpString1, ((cchCount1 <= -1) ? -2 : cchCount1), lpString2, ((cchCount2 <= -1) ? -2 : cchCount2), fModify ));
    }
    ...
    //  Switch on the different flag options.  This will speed up the comparisons of two strings that are different.

    //  The only two possibilities in this optimized section are no flags and the ignore case flag.
    if (dwCmpFlags == 0)
    {
    ...
    while ((*pString1 != 0) && (*pString2 != 0))
    {
        ...
        if (Weight1 != Weight2)
        {
            ...
            //  If Unicode Weights are different and no special cases, then we're done.  Otherwise, we need to do extra checking.
            ...
            if ((uw1 != uw2) || (sm1 == FAREAST_SPECIAL))
            {
                ...
                switch (sm1)
                {
                    ...
                    case ( FAREAST_SPECIAL ) :
                    {
                        ...
                        GET_FAREAST_WEIGHT( Weight1, uw1, Mask, lpString1, pString1, ExtraWt1, FALSE );
                ...
                switch (sm2)
                {
                    ...
                    case ( PUNCTUATION ) :
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                WhichPunct1 = (((GET_ALPHA_NUMERIC(&Weight1) < GET_ALPHA_NUMERIC(&Weight2))) ? CSTR_LESS_THAN : CSTR_GREATER_THAN);
                            }
                    ...
                    case ( FAREAST_SPECIAL ) :
                    {
                        ...
                        GET_FAREAST_WEIGHT( Weight2, uw2, Mask, lpString2, pString2, ExtraWt2, FALSE );
                        ...
                        else
                        {
                            ...
                            {
                                GET_WT_FOUR(&WhichExtra) = ( GET_WT_FOUR(&ExtraWt1) < GET_WT_FOUR(&ExtraWt2) ) ? CSTR_LESS_THAN : CSTR_GREATER_THAN;
                            }
                            if ( (GET_WT_FIVE(&WhichExtra) == 0) && ( GET_WT_FIVE(&ExtraWt1) != GET_WT_FIVE(&ExtraWt2) ) )
                            {
                                GET_WT_FIVE(&WhichExtra) = ( GET_WT_FIVE(&ExtraWt1) < GET_WT_FIVE(&ExtraWt2) ) ? CSTR_LESS_THAN : CSTR_GREATER_THAN;
                            }
                            if ( (GET_WT_SIX(&WhichExtra) == 0) && ( GET_WT_SIX(&ExtraWt1) != GET_WT_SIX(&ExtraWt2) ) )
                            {
                                GET_WT_SIX(&WhichExtra) = ( GET_WT_SIX(&ExtraWt1) < GET_WT_SIX(&ExtraWt2) ) ? CSTR_LESS_THAN : CSTR_GREATER_THAN;
                            }
                            if ( (GET_WT_SEVEN(&WhichExtra) == 0) && ( GET_WT_SEVEN(&ExtraWt1) != GET_WT_SEVEN(&ExtraWt2) ) )
                            {
                                GET_WT_SEVEN(&WhichExtra) = ( GET_WT_SEVEN(&ExtraWt1) < GET_WT_SEVEN(&ExtraWt2) ) ? CSTR_LESS_THAN : CSTR_GREATER_THAN;
                            }
                        }
            ...
            if (State & STATE_CW)
            {
                ...
                if (GET_CASE(&Weight1) != GET_CASE(&Weight2))
                {
                    ...
                    WhichCase = (GET_CASE(&Weight1) < GET_CASE(&Weight2)) ? CSTR_LESS_THAN : CSTR_GREATER_THAN;
    ...
    QUICK_SCAN_LONGER_STRING( pString1, ((*pString2 == 0) ? CSTR_GREATER_THAN : CSTR_LESS_THAN) );
}
...
//  05-31-91    JulieB    Created.
{
    ...
    if ( (lpSrcStr == NULL) || (cchSrc == 0) || (lpCharType == NULL) || (lpSrcStr == lpCharType) )
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return (FALSE);
    }

    //  If cchSrc is -1, then the source string is null terminated and we need to get the length of the source string.
    //  Add one to the length to include the null termination.
    ...
    //  Return the appropriate information in the lpCharType parameter based on the dwInfoType parameter.
    switch (dwInfoType)
    {
        case ( CT_CTYPE1 ) :
        {
...
//  05-31-91    JulieB    Created.
{
    ...
    //  See if we should stop on the null terminator regardless of the count values.  The original count values are stored in ctr1 and ctr2
    ...
    //  Check if compression in the given locale.
    //  If not, then try a wchar by wchar compare.
    //  If strings are equal, this will be quick.
    if ((IfCompress = pHashN->IfCompression) == FALSE)
    {
        ...
        while ( NOT_END_STRING(ctr1, pString1, cchCount1) && NOT_END_STRING(ctr2, pString2, cchCount2) )
        {
            ...
            if (*pString1 == *pString2)
            {
                //  Characters are equal, so increment pointers, decrement counters, and continue string compare.
                pString1++;
                pString2++;
                ctr1--;
                ctr2--;
            ...
            {
                //  Difference was found.  Fall into the sortkey check below.
                break;
            }
        }

        //  If the end of BOTH strings has been reached, then the strings match exactly.  Return success.
    ...
    //  Set the weights to be invalid.
    //  This flags whether or not to recompute the weights next time through the loop.
    //  It also flags whether or not to start over (continue) in the loop.
    Weight1 = CMP_INVALID_WEIGHT;
    Weight2 = CMP_INVALID_WEIGHT;

    //  Switch on the different flag options.
    //  This will speed up the comparisons of two strings that are different.
    State = STATE_CW;
    ...
    while ( NOT_END_STRING(ctr1, pString1, cchCount1) && NOT_END_STRING(ctr2, pString2, cchCount2) )
    {
        ...
        //  If compression locale, then need to check for compression characters even if the weights are equal.
        //  If it's not a compression locale, then we don't need to check anything if the weights are equal.
        if ( (IfCompress) && (GET_COMPRESSION(&Weight1) || GET_COMPRESSION(&Weight2)) )
        {
            ...
            if (pHashN->IfDblCompression == FALSE)
            {
                ...
                switch (CompVal)
                {
                    ...
                    case ( COMPRESS_3_MASK ) :
                    {
                        ...
                        if ( ((If1) && (!IfEnd1) && !AT_STRING_END(ctr1 - 2, pString1 + 2, cchCount1)) || ((If2) && (!IfEnd2) && !AT_STRING_END(ctr2 - 2, pString2 + 2, cchCount2)) )
                        {
                            ...
                            for (; ctr > 0; ctr--, pComp3++)
                            {
                                ...
                                if ( (If1) && (!IfEnd1) && !AT_STRING_END(ctr1 - 2, pString1 + 2, cchCount1) && (pComp3->UCP1 == *pString1) && (pComp3->UCP2 == *(pString1 + 1)) && (pComp3->UCP3 == *(pString1 + 2)) )
                                {
                                    ...
                                    If1 = 0;//  Set boolean for string 1 - search is complete.

                                    //  Break out of loop if both searches are done.
                                    if (If2 == 0)
                                        break;
                                }

                                //  Check character in string 2.
                                if ( (If2) && (!IfEnd2) && !AT_STRING_END(ctr2 - 2, pString2 + 2, cchCount2) && (pComp3->UCP1 == *pString2) && (pComp3->UCP2 == *(pString2 + 1)) && (pComp3->UCP3 == *(pString2 + 2)) )
                                {
                                    ...
                                    If2 = 0;//  Set boolean for string 2 - search is complete.
                    ...
                    case ( COMPRESS_2_MASK ) :
                    {
                        ...
                        if ( ((If1) && (!IfEnd1)) || ((If2) && (!IfEnd2)) )
                        {
                            ...
                            {
                                //  Check character in string 1.
                                if ( (If1) && (!IfEnd1) && (pComp2->UCP1 == *pString1) && (pComp2->UCP2 == *(pString1 + 1)) )
                                {
                                    //  Found compression for string 1.
                                    //  Get new weight and mask it.
                                    //  Increment pointer and decrement counter.
                                ...
                                //  Check character in string 2.
                                if ( (If2) && (!IfEnd2) && (pComp2->UCP1 == *pString2) && (pComp2->UCP2 == *(pString2 + 1)) )
                                {
                                    //  Found compression for string 2.
                                    //  Get new weight and mask it.
                                    //  Increment pointer and decrement counter.
            ...
            else if (!IfEnd1 && !IfEnd2)
            {
                ...
                if (IfDblCompress1 = ((GET_DWORD_WEIGHT(pHashN, *pString1) & CMP_MASKOFF_CW) == (GET_DWORD_WEIGHT(pHashN, *(pString1 + 1)) & CMP_MASKOFF_CW)))
                {
                    //  Advance past the first code point to get to the compression character.
                    pString1++;
                    ctr1--;
                    IfEnd1 = AT_STRING_END(ctr1 - 1, pString1 + 1, cchCount1);
                }

                if (IfDblCompress2 = ((GET_DWORD_WEIGHT(pHashN, *pString2) & CMP_MASKOFF_CW) == (GET_DWORD_WEIGHT(pHashN, *(pString2 + 1)) & CMP_MASKOFF_CW)))
                {
                    //  Advance past the first code point to get to the compression character.
                    pString2++;
                    ctr2--;
                ...
                //  compression.  Pointer fixup at the end will advance them correctly.

                //  If double compression, we advanced the pointer at the beginning of the switch statement.
                //  If double compression character was actually found, the pointer was NOT advanced.
                //  We now want to decrement the pointer to put it back to where it was.

                //  The next time through, the pointer will be pointing to the regular compression part of the string.
                if (IfDblCompress1)
                {
                    pString1--;
                    ctr1++;
        ...
        if (Weight1 != Weight2)
        {
            ...
            //  Must check ENTIRE string for any possibility of Unicode Weight differences.
            //  As soon as a Unicode Weight difference is found, then we're done.
            //  If no UW difference is found, then the first Diacritic Weight difference is used.
            //  If no DW difference is found, then use the first Case Difference.
            //  If no CW difference is found, then use the first Extra Weight difference.
            //  If no EW difference is found, then use the first Special Weight difference.
            if ((uw1 != uw2) || ((sm1 <= SYMBOL_5) && (sm1 >= FAREAST_SPECIAL)))
            {
                ...
                //  Check for Ignore Nonspace and Ignore Symbol.
                //  If Ignore Nonspace is set and either character is a nonspace mark only, then we need to advance the pointer to skip over the character and continue.
                //  If Ignore Symbol is set and either character is a punctuation char, then we need to advance the pointer to skip over the character and continue.

                //  This step is necessary so that a string with a nonspace mark and a punctuation char following one another are properly ignored when one or both of the ignore flags is set.
                if (fIgnoreDiacritic)
                {
                ...
                //  Switch on the script member of string 1 and take care of any special cases.
                switch (sm1)
                {
                    case ( NONSPACE_MARK ) :
                    {
                    ...
                    case ( PUNCTUATION ) :
                    {
                        ...
                        else if (!fStringSort)
                        {
                            ...
                            if (sm2 != PUNCTUATION)
                            {
                                ...
                                if (WhichPunct2)
                                {
                                    //  Set WP 2 to show that string 2 is smaller, since a punctuation char had already been found at an earlier position in string 2.

                                    //  Set the Ignore Punctuation flag so we just skip over any other punctuation chars in the string.
                                    WhichPunct2 = CSTR_GREATER_THAN;
                                    fIgnorePunct = TRUE;
                                }
                                else
                                {
                                    //  Set WP 1 to show that string 2 is smaller, and that string 1 has had a punctuation char - since no punctuation chars have been found
                                    //  in string 2.
                                    WhichPunct1 = CSTR_GREATER_THAN;
                                }
                    ...
                    case ( EXPANSION ) :
                    {
                        ...
                        //  Add one to counter so that subtraction doesn't end comparison prematurely.
                        ctr1++;
                        ...
                        //  Set cExpChar1 to the number of expansion characters stored.
                        cExpChar1 = MAX_TBL_EXPANSION;
                    ...
                    case ( FAREAST_SPECIAL ) :
                    {
                        //  Get the weight for the far east special case and store it in Weight1.
                        GET_FAREAST_WEIGHT( Weight1, uw1, Mask, lpString1, pString1, ExtraWt1, fModify );
                ...
                //  Switch on the script member of string 2 and take care of any special cases.
                switch (sm2)
                {
                    case ( NONSPACE_MARK ) :
                    {
                    ...
                    case ( SYMBOL_5 ) :
                    {
                        //  If the ignore symbol flag is set, then skip over the symbol.
                        if (fIgnoreSymbol)
                        {
                            pString2++;
                            ctr2--;
                    ...
                    case ( PUNCTUATION ) :
                    {
                        //  If the ignore punctuation flag is set, then skip over the punctuation char.
                        if (fIgnorePunct)
                        {
                        ...
                        else if (!fStringSort)
                        {
                            ...
                            if (sm1 != PUNCTUATION)
                            {
                                ...
                                if (WhichPunct1)
                                {
                                    //  Set WP 1 to show that string 1 is smaller, since a punctuation char had already been found at an earlier position in string 1.
                                ...
                                {
                                    //  Set WP 2 to show that string 1 is smaller, and that string 2 has had a punctuation char - since no
                                    //  punctuation chars have been found in string 1.
                                    WhichPunct2 = CSTR_LESS_THAN;
                                }
                            ...
                            else
                            {
                                ...
                                //  See if either of the strings has encountered punctuation chars previous to this.
                                if (WhichPunct1)
                                {
                                    //  String 1 has had a punctuation char, so it should be the smaller string (since both have punctuation chars).
                                    WhichPunct1 = CSTR_LESS_THAN;
                                }
                                else if (WhichPunct2)
                                {
                                    //  String 2 has had a punctuation char, so it should be the smaller string (since both have punctuation chars).
                                    WhichPunct2 = CSTR_GREATER_THAN;
                                }
                                else
                                {
                                ...
                                //  Advance pointer 1 and decrement counter 1.
                                //  Pointer 2 and counter 2 will be updated after if-else statement.
                                pString1++;
                                ctr1--;
                                Weight1 = CMP_INVALID_WEIGHT;
                            }
                    ...
                    case ( EXPANSION ) :
                    {
                        ...
                        //  Set cExpChar2 to the number of expansion characters stored.
                        cExpChar2 = MAX_TBL_EXPANSION;
                    ...
                    case ( FAREAST_SPECIAL ) :
                    {
                        //  Get the weight for the far east special case and store it in Weight2.
                        GET_FAREAST_WEIGHT( Weight2, uw2, Mask, lpString2, pString2, ExtraWt2, fModify );
                ...
                if ((Weight1 == CMP_INVALID_WEIGHT) || (Weight2 == CMP_INVALID_WEIGHT))
                {
                    ...
                    //  (if applicable) so that it doesn't get modified again.
                    if (fModify == TRUE)
                    {
                        if (sm1 == FAREAST_SPECIAL)
                        {
                ...
                //  We're not supposed to drop down into the state table if the unicode weights are different, so stop comparison and return result of unicode weight comparison.
                if (uw1 != uw2)
                {
                    return ((uw1 < uw2) ? CSTR_LESS_THAN : CSTR_GREATER_THAN);
                }
            ...
            //  For each state in the state table, do the appropriate comparisons.
            if (State & (STATE_DW | STATE_REVERSE_DW))
            {
                //  Get the diacritic weights.
                dw1 = GET_DIACRITIC(&Weight1);
                ...
                if (dw1 != dw2)
                {
                    //  Look ahead to see if diacritic follows a minimum diacritic weight.
                    //  If so, get the diacritic weight of the nonspace mark.
                    ...
                    //  Save which string has the smaller diacritic weight if the diacritic weights are still different.
                    if (dw1 != dw2)
                    {
                        WhichDiacritic = (dw1 < dw2) ? CSTR_LESS_THAN : CSTR_GREATER_THAN;
                        REMOVE_STATE(STATE_DW);//  Remove state from state machine.
                    }
                }
            }
            if (State & STATE_CW)
            {
                ...
                if (GET_CASE(&Weight1) != GET_CASE(&Weight2))
                {
                    WhichCase = (GET_CASE(&Weight1) < GET_CASE(&Weight2)) ? CSTR_LESS_THAN : CSTR_GREATER_THAN;//  Save which string has the smaller case weight.
                    REMOVE_STATE(STATE_CW);//  Remove state from state machine.
                }
            }
        }
    ...
    //  If the end of BOTH strings has been reached, then the unicode weights match exactly.
    //  Check the diacritic, case and special weights.
    //  If all are zero, then return success.
    //  Otherwise, return the result of the weight difference.
    ...
    SCAN_LONGER_STRING( ctr1, pString1, cchCount1, fEnd1 );//  Scan to the end of the longer string.
}