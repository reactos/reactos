...
#define SEM_NOERROR   (SEM_FAILCRITICALERRORS | SEM_NOGPFAULTERRORBOX  | SEM_NOOPENFILEERRORBOX)
...
PCP_HASH FASTCALL GetCPHashNode(UINT CodePage)
...
PLOC_HASH FASTCALL GetLocHashNode(LCID Locale)
//  Returns a pointer to the appropriate LOC hash node given the locale.
//  If no table could be found for the given locale, NULL is returned.
...
//  05-31-91    JulieB    Created.
{
    ...
    if (pIdeograph)
    {
        ...
        {
            pEntryIG = (PIDEOGRAPH_EXCEPT)( ((LPBYTE)pIdeograph) + sizeof(IDEOGRAPH_EXCEPT_HDR) );
