...
int FoldCZone(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int FoldDigits(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int FoldCZone_Digits(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int FoldLigatures(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int FoldPreComposed(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int FoldComposite(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int MapCase(PLOC_HASH pHashN, LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest, PCASE pCaseTbl);
int MapSortKey(PLOC_HASH pHashN, DWORD dwFlags, LPCWSTR pSrc, int cchSrc, LPBYTE pDest, int cchDest, BOOL fModify);
int MapNormalization(PLOC_HASH pHashN, DWORD dwFlags, LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int MapKanaWidth(PLOC_HASH pHashN, DWORD dwFlags, LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest);
int MapHalfKana(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest, PKANA pKana, PCASE pCase);
int MapFullKana(LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest, PKANA pKana, PCASE pCase);
int MapTraditionalSimplified(PLOC_HASH pHashN, DWORD dwFlags, LPCWSTR pSrc, int cchSrc, LPWSTR pDest, int cchDest, PCHINESE pChinese);
...
int WINAPI FoldStringW(DWORD dwMapFlags, LPCWSTR lpSrcStr, int cchSrc, LPWSTR lpDestStr, int cchDest)
//  Maps one wide character string to another performing the specified translation.
//  This mapping routine only takes flags that are locale independent.
...
//  05-31-91    JulieB    Created.
{
    ...
    if ((cchSrc == 0) || (cchDest < 0) || (lpSrcStr == NULL) || ((cchDest != 0) && (lpDestStr == NULL)) || (lpSrcStr == lpDestStr))
    {
    ...
    switch (dwMapFlags)
    {
        case ( MAP_FOLDCZONE ) :
        {
            ...
            Count = FoldCZone( lpSrcStr, cchSrc, lpDestStr, cchDest );
            break;
        }
        case ( MAP_FOLDDIGITS ) :
        {
            //  Map the string to fold the Ascii Digits.
            Count = FoldDigits( lpSrcStr, cchSrc, lpDestStr, cchDest );
            break;
        }
        case ( MAP_EXPAND_LIGATURES ) :
        {
            //  Map the string to expand all Ligatures.
            Count = FoldLigatures( lpSrcStr, cchSrc, lpDestStr, cchDest );
            break;
        }
        case ( MAP_PRECOMPOSED ) :
        {
            //  Map the string to compress all composite forms of characters to their precomposed form.
            Count = FoldPreComposed( lpSrcStr, cchSrc, lpDestStr, cchDest );
            break;
        }
        case ( MAP_COMPOSITE ) :
        {
            //  Map the string to expand out all precomposed characters to their composite form.
            Count = FoldComposite( lpSrcStr, cchSrc, lpDestStr, cchDest );
            break;
        }
        case ( MAP_FOLDCZONE | MAP_FOLDDIGITS ) :
        {
            //  Map the string to fold the Compatibility Zone and fold the Ascii Digits.
            Count = FoldCZone_Digits( lpSrcStr, cchSrc, lpDestStr, cchDest );
            break;
        }
        case ( MAP_EXPAND_LIGATURES | MAP_FOLDCZONE ) :
        {
            //  Map the string to expand the ligatures and fold the Compatibility Zone.
            Count = FoldLigatures( lpSrcStr, cchSrc, lpDestStr, cchDest );
            Count = FoldCZone( lpDestStr, Count, lpDestStr, cchDest );
            break;
        }
        case ( MAP_EXPAND_LIGATURES | MAP_FOLDDIGITS ) :
        {
            //  Map the string to expand the ligatures and fold the Ascii Digits.
            Count = FoldLigatures( lpSrcStr, cchSrc, lpDestStr, cchDest );
            Count = FoldDigits( lpDestStr, Count, lpDestStr, cchDest );
            break;
        }
        case ( MAP_EXPAND_LIGATURES | MAP_FOLDCZONE | MAP_FOLDDIGITS ) :
        {
            ...
            Count = FoldLigatures( lpSrcStr, cchSrc, lpDestStr, cchDest );
            Count = FoldCZone_Digits( lpDestStr, Count, lpDestStr, cchDest );
            break;
        }
        case ( MAP_PRECOMPOSED | MAP_FOLDCZONE ) :
        {
            //  Map the string to convert to precomposed forms and to fold the Compatibility Zone.
            Count = FoldPreComposed( lpSrcStr, cchSrc, lpDestStr, cchDest );
            Count = FoldCZone( lpDestStr, Count, lpDestStr, cchDest );
            break;
        }
        ...
        {
            //  Map the string to convert to precomposed forms and to fold the Ascii Digits.
            Count = FoldPreComposed( lpSrcStr, cchSrc, lpDestStr, cchDest );
            Count = FoldDigits( lpDestStr, Count, lpDestStr, cchDest );
            break;
        }
        ...
        {
            //  Map the string to convert to composite forms and to fold the Compatibility Zone.
            Count = FoldComposite( lpSrcStr, cchSrc, lpDestStr, cchDest );
            Count = FoldCZone( lpDestStr, Count, lpDestStr, cchDest );
            break;
        }
        ...
        {
            //  Map the string to convert to composite forms and to fold the Ascii Digits.
            Count = FoldComposite( lpSrcStr, cchSrc, lpDestStr, cchDest );
            Count = FoldDigits( lpDestStr, Count, lpDestStr, cchDest );
            break;
        }
    ...
    //  Or, if cchDest == 0, then return the number of characters that would have been written to the buffer.
    return (Count);
}
...
//  Maps one wide character string to another performing the specified translation.
//  This mapping routine only takes flags that are locale dependent.
...
//  07-26-93    JulieB    Added new flags for NT-J.
{
    ...
    if ( (pHashN == NULL) ||
         ...
         ((lpSrcStr == lpDestStr) &&
          ((!(dwMapFlags & (LCMAP_UPPERCASE | LCMAP_LOWERCASE))) ||
           (dwMapFlags & (LCMAP_HIRAGANA | LCMAP_KATAKANA | LCMAP_HALFWIDTH | LCMAP_FULLWIDTH)))) )
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return (0);
    }
    ...
    if ( (dwMapFlags & LCMS_INVALID_FLAG) || (dwMapFlags == 0) ||
         ((dwMapFlags & (NORM_ALL | SORT_STRINGSORT)) && (dwMapFlags & LCMAP_NO_NORM)) ||
         ((dwMapFlags & NORM_SORTKEY_ONLY) && (!(dwMapFlags & LCMAP_SORTKEY))) ||
         ...
         ((dwMapFlags & LCMAP_LINGUISTIC_CASING) && (!(dwMapFlags & (LCMAP_UPPERCASE | LCMAP_LOWERCASE)))) )
    {
        SetLastError(ERROR_INVALID_FLAGS);
        return (0);
    }
    ...
    if (dwMapFlags & LCMAP_SORTKEY)
    {
        ...
        Count = MapSortKey( pHashN, dwMapFlags, lpSrcStr, cchSrc, (LPBYTE)lpDestStr, cchDest, IS_KOREAN(Locale) );
    }
    else
    {
        switch (dwMapFlags & ~(LCMAP_BYTEREV | LCMAP_LINGUISTIC_CASING))
        {
            case ( LCMAP_LOWERCASE ) :
            {
                ...
                Count = MapCase( pHashN,
                                 ...
                                 lpDestStr,
                                 cchDest,
                                 (dwMapFlags & LCMAP_LINGUISTIC_CASING) ? pHashN->pLowerLinguist : pHashN->pLowerCase );
                break;
            }
            case ( LCMAP_UPPERCASE ) :
            {
                ...
                Count = MapCase( pHashN,
                                 ...
                                 lpDestStr,
                                 cchDest,
                                 (dwMapFlags & LCMAP_LINGUISTIC_CASING) ? pHashN->pUpperLinguist : pHashN->pUpperCase );
                break;
            }
            ...
            case ( LCMAP_SIMPLIFIED_CHINESE | LCMAP_UPPERCASE )  :
            {
                ...
                Count = MapTraditionalSimplified(
                    pHashN,
                    dwMapFlags & ~LCMAP_BYTEREV,
                    lpSrcStr,
                    cchSrc,
            ...
            default :
            {
                //  Make sure the Chinese flags are not used with the Japanese flags.
                if (dwMapFlags & (LCMAP_TRADITIONAL_CHINESE | LCMAP_SIMPLIFIED_CHINESE))
                {
                    SetLastError(ERROR_INVALID_FLAGS);
                    return (0);
                }
                ...
                Count = MapKanaWidth( pHashN, dwMapFlags & ~LCMAP_BYTEREV, lpSrcStr, cchSrc, lpDestStr, cchDest );
                break;
            }
        }
    }
    ...
    if (dwMapFlags & LCMAP_BYTEREV)
    {
        ...
        if (dwMapFlags == LCMAP_BYTEREV)
        {
            ...
            //  Make sure that the size of the destination buffer is larger than zero.
            //  If it is zero, return the size of the source string only.
            //  Do NOT touch lpDestStr.
            if (cchDest != 0)
            {
                //  Flag is used by itself.  Reverse the bytes from
                //  the source string and store them in the destination string.
                if (cchSrc > cchDest)
                {
                ...
                {
                    lpDestStr[ctr] = MAKEWORD( HIBYTE(lpSrcStr[ctr]), LOBYTE(lpSrcStr[ctr]) );
                }
            }
        ...
        else
        {
            ...
            if (cchDest != 0)
            {
                ...
                if (dwMapFlags & LCMAP_SORTKEY)
                {
                    ...
                    {
                        lpDestStr[ctr] = MAKEWORD( HIBYTE(lpDestStr[ctr]), LOBYTE(lpDestStr[ctr]) );
                    }
                }
                else
                {
    ...
    //  Return the number of characters (or number of bytes for sortkey) written to the buffer.
    return (Count);
}
...
//  destination buffer, and returns the number of wide characters written to the buffer.
...
//  02-01-93    JulieB    Created.
{
    ...
    return (ctr);//  Return the number of wide characters written.
}
...
//  string in the destination buffer, and returns the number of wide characters written to the buffer.
...
//  02-01-93    JulieB    Created.
{
    ...
    //  Fold the compatibility zone and the ascii digits values and store it in the destination string.
    for (ctr = 0; ctr < cchSrc; ctr++)
    {
        pDest[ctr] = GET_FOLD_CZONE(pTblPtrs->pCZone, pSrc[ctr]);
...
//  10-15-96    JulieB    Created.
{
    ...
    if (cchDest == 0)
    {
        ...
        //  the number of characters that would have been written to a destination buffer.
        while (ctr < cchSrc)
        {
            ...
            if (GET_SCRIPT_MEMBER(&Weight) == EXPANSION)
            {
                ...
                {
                    ctr2++;
                    Weight = MAKE_SORTKEY_DWORD((pTblPtrs->pDefaultSortkey)[GET_EXPANSION_2(&Weight)]);
    ...
    {
        //  Convert the source string to expand all ligatures and store the result in the destination buffer.
        while ((ctr < cchSrc) && (ctr2 < cchDest))
        {
            ...
            if (GET_SCRIPT_MEMBER(&Weight) == EXPANSION)
            {
                do
                {
                    ...
                    Weight = MAKE_SORTKEY_DWORD((pTblPtrs->pDefaultSortkey)[pDest[ctr2]]);
                } while (GET_SCRIPT_MEMBER(&Weight) == EXPANSION);
    ...
    return (ctr2);//  Return the number of wide characters written.
}
...
//  02-01-93    JulieB    Created.
{
    ...
    if (cchDest == 0)
    {
        ...
        //  number of characters that would have been written to a destination buffer.
        while (ctr < cchSrc)
        {
            if ((ctr2 != 0) && (IS_NONSPACE_ONLY(pTblPtrs->pDefaultSortkey, pSrc[ctr])))
            {
                ...
                //  character count (the precomposed character was written over the previous character).
                if (wch)
                {
                    if ((wch = GetPreComposedChar(pSrc[ctr], wch)) == 0)
                    {
                        //  No translation for composite form, so just increment the destination counter.
                        ctr2++;
                    }
                }
                else
                {
                    if ((wch = GetPreComposedChar( pSrc[ctr], pSrc[ctr - 1] )) == 0)
                    {
            ...
            {
                //  Not part of a composite character, so just increment the destination counter.
                wch = 0;
                ctr2++;
            }
            ctr++;
    ...
    {
        //  Convert the source string to precomposed and store it in the destination string.
        while ((ctr < cchSrc) && (ctr2 < cchDest))
        {
            if ((ctr2 != 0) && (IS_NONSPACE_ONLY(pTblPtrs->pDefaultSortkey, pSrc[ctr])))
            {
                ...
                //  character count (the precomposed character was written over the previous character).
                wch = pDest[ctr2 - 1];
                if ((pDest[ctr2 - 1] = GetPreComposedChar( pSrc[ctr], pDest[ctr2 - 1] )) == 0)
                {
                    //  No translation for composite form, so must
                    //  rewrite the base character and write the composite character.
                    pDest[ctr2 - 1] = wch;
                    pDest[ctr2] = pSrc[ctr];
                    ctr2++;
                }
            ...
            {
                //  Not part of a composite character, so just write the character to the destination string.
                pDest[ctr2] = pSrc[ctr];
                ctr2++;
            }
            ctr++;
...
//  02-01-93    JulieB    Created.
{
    ...
    if (cchDest == 0)
    {
        pEndDest = (LPWSTR)pTmp + MAX_COMPOSITE;//  Get the end of the tmp buffer.

        //  Convert the source string to precomposed and calculate the
        //  number of characters that would have been written to a destination buffer.
        while (ctr < cchSrc)
        {
            *pTmp = pSrc[ctr];//  Write the character to the destination string.

            //  See if it needs to be expanded to its composite form.

            //  If no composite form is found, the routine returns 1 for the base character.
            //  Simply increment by the return value.
            ctr2 += InsertCompositeForm(pTmp, pEndDest);

            ctr++;//  Increment the source string counter.
        }
    }
    else
    {
        pEndDest = (LPWSTR)pDest + cchDest;//  Get the end of the destination string.

        //  Convert the source string to precomposed and store it in the destination string.
        while ((ctr < cchSrc) && (ctr2 < cchDest))
        {
            pDest[ctr2] = pSrc[ctr];//  Write the character to the destination string.
...
//  05-31-91    JulieB    Created.
{
    ...
    //  If cchSrc is greater than cchDest, then the destination buffer
    //  is too small to hold the lower or upper case string.  Return an error.
    if (cchSrc > cchDest)
    {
        SetLastError(ERROR_INSUFFICIENT_BUFFER);
        return (0);
    ...
    //  Lower or Upper case the source string and store it in the destination string.
    for (ctr = 0; ctr < cchSrc; ctr++)
    {
...
#define SPECIAL_CASE_HANDLER( SM, pWeight, pSortkey, pExpand, Position, fStringSort, fIgnoreSymbols, pCur, pBegin, fModify ) \
{                                                                           \
                ...
                {                                                           \
                    ...
                    {                                                       \
                        ...
                        *pPosUW = MAKE_UNICODE_WT(KANA, PrevAW, fModify);   \
                         ...
                         *                                                  \
                         */                                                 \
                        PrevCW = (GET_CASE(&PrevWt) & CaseMask) |  CASE_XW_MASK; \
...
int MapSortKey(PLOC_HASH pHashN, DWORD dwFlags, LPCWSTR pSrc, int cchSrc, LPBYTE pDest, int cbDest, BOOL fModify)
...
//  11-04-92    JulieB    Created.
{
    ...
    if (cchSrc > MAX_STRING_LEN)
    {
        ...
        if ((pUW = (LPWSTR)NLS_ALLOC_MEM( WeightLen * MAX_WEIGHTS * sizeof(WCHAR) )) == NULL)
        {
    ...
    //  If none or all of the ignore case flags are set, then just leave the mask as 0xff.
    CaseMask = 0xff;
    ...
    else if (pHashN->IfDblCompression == FALSE)
    {
        ...
        //  Compressions DO exist in the given locale, so check for them.

        //  No double compressions exist in the given locale, so DO NOT check for them.
        for (; PosCtr <= cchSrc; PosCtr++, pPos++)
        {
            //  Get weights.
            pWeight = &((pHashN->pSortkey)[*pPos]);
            ...
            if (SM > MAX_SPECIAL_CASE)
            {
                //  No special case on character, but must check for compression characters.
                switch (GET_COMPRESSION(pWeight))
                {
                    case ( COMPRESS_3_MASK ) :
                    {
                        if ((PosCtr + 2) <= cchSrc)
                        {
                            ...
                            {
                                if ((pComp3->UCP1 == *pPos) && (pComp3->UCP2 == *(pPos + 1)) && (pComp3->UCP3 == *(pPos + 2)))
                                {
    ...
    else
    {
        ...
        //  Double Compressions also exist in the given locale, so check for them.
        for (; PosCtr <= cchSrc; PosCtr++, pPos++)
        {
            //  Get weights.
            pWeight = &((pHashN->pSortkey)[*pPos]);
            ...
            if (SM > MAX_SPECIAL_CASE)
            {
                ...
                //  compression characters and double compression characters.
                IfDblCompress =
                  (((PosCtr + 1) <= cchSrc) &&
                    ...
                    case ( COMPRESS_3_MASK ) :
                    {
                        if (IfDblCompress)
                        {
                            if ((PosCtr + 3) <= cchSrc)
                            {
                                ...
                                for (; ctr > 0; ctr--, pComp3++)
                                {
                                    if ((pComp3->UCP1 == *(pPos + 1)) && (pComp3->UCP2 == *(pPos + 2)) && (pComp3->UCP3 == *(pPos + 3)))
                                    {
                                        pWeight = &(pComp3->Weights);
                                        *pPosUW = GET_UNICODE_MOD(pWeight, fModify);
                                        *pPosDW = GET_DIACRITIC(pWeight);
                    ...
                    case ( COMPRESS_2_MASK ) :
                    {
                        if (IfDblCompress)
                        {
                            if ((PosCtr + 2) <= cchSrc)
                            {
                                ...
                                for (; ctr > 0; ctr--, pComp2++)
                                {
                                    if ((pComp2->UCP1 == *(pPos + 1)) && (pComp2->UCP2 == *(pPos + 2)))
                                    {
                                        pWeight = &(pComp2->Weights);
                                        *pPosUW = GET_UNICODE_MOD(pWeight, fModify);
                                        *pPosDW = GET_DIACRITIC(pWeight);
                        ...
                        if ((PosCtr + 1) <= cchSrc)
                        {
                            ...
                            for (; ctr > 0; ctr--, pComp2++)
                            {
                                if ((pComp2->UCP1 == *pPos) && (pComp2->UCP2 == *(pPos + 1)))
                                {
                                    pWeight = &(pComp2->Weights);
                                    *pPosUW = GET_UNICODE_MOD(pWeight, fModify);
                                    *pPosDW = GET_DIACRITIC(pWeight);
    ...
    if (cbDest == 0)
    {
        PosCtr += (int)((LPBYTE)pPosUW - (LPBYTE)pUW);//  Count the Unicode Weights.
        PosCtr++;//  Count the Separator.
        ...
        if (pXW < pPosXW)
        {
            if (dwFlags & NORM_IGNORENONSPACE)
            {
                //  Ignore 4W and 5W.  Must count separators for 4W and 5W, though.
                PosCtr += 2;
                ctr = 2;
            }
            else
    ...
    else
    {
        ...
        //  Copy Separator to destination buffer if the destination buffer is large enough.
        if (PosCtr == cbDest)
        {
            NLS_FREE_TMP_BUFFER(pUW, pBuffer);
            SetLastError(ERROR_INSUFFICIENT_BUFFER);
        ...
        //  Copy Terminator to destination buffer if the destination buffer is large enough.
        if (PosCtr == cbDest)
        {
            NLS_FREE_TMP_BUFFER(pUW, pBuffer);
            SetLastError(ERROR_INSUFFICIENT_BUFFER);
    ...
    NLS_FREE_TMP_BUFFER(pUW, pBuffer);//  Free the buffer used for the weights, if one was allocated.
    return (PosCtr);//  Return number of BYTES written to destination buffer.
}
...
//  11-04-92    JulieB    Created.
{
    ...
    switch (dwFlags)
    {
        case ( NORM_IGNORENONSPACE ) :
        {
            ...
            if (cchDest == 0)
            {
                //  Count the number of characters that would be written to the destination buffer.
        ...
        case ( NORM_IGNORENONSPACE | NORM_IGNORESYMBOLS ) :
        {
            //  If the destination value is zero, then only return the count of characters.  Do NOT touch pDest.
            if (cchDest == 0)
            {
                //  Count the number of characters that would be written to the destination buffer.
                for (ctr = 0, ctr2 = 0; ctr < cchSrc; ctr++)
...
//  given string in the destination buffer, and returns the number of wide characters written to the buffer.
...
//  07-26-93    JulieB    Created.
{
    ...
    {
        pCase = (dwFlags & LCMAP_LINGUISTIC_CASING) ? pHashN->pLowerLinguist : pHashN->pLowerCase;
    }
    else if (dwFlags & LCMAP_UPPERCASE)
    {
        pCase = (dwFlags & LCMAP_LINGUISTIC_CASING) ? pHashN->pUpperLinguist : pHashN->pUpperCase;
    }
    else
    {
        pCase = NULL;
    ...
    switch (dwFlags)
    {
        ...
        case ( LCMAP_KATAKANA ) :
        {
            ...
            return (cchSrc);//  Return the number of characters mapped.
            break;
        }
        ...
        {
            //  This combination of flags is strange, because Hiragana is only full width.
            //  So, the Hiragana flag is the most important.
            //  Full width Katakana will be mapped to full width Hiragana, not half width Katakana.
        ...
        case ( LCMAP_HIRAGANA | LCMAP_FULLWIDTH ) :
        {
            ...
            //  Katakana to full width Katakana before trying to map to Hiragana.
...
//  in the destination buffer, and returns the number of wide characters written to the buffer.

//  This first converts the precomposed characters to their composite forms,
//  and then maps all characters to their half width forms.
//  This handles the case where the full width precomposed form should map to TWO half width
//  code points (composite form).  The half width precomposed forms do not exist in Unicode.
...
//  11-04-93    JulieB    Created.
{
        ...
        if (Count > 1)
        {
            ...
            if (ct > 1)
            {
                ...
                else
                {
                    ...
                    //  the rest of the composite chars from the tmp buffer to the destination buffer.
                    (*ppIncr)++;
                    Count -= ct;
            ...
            else
            {
                ...
                //  composite form from the tmp buffer to the destination buffer.
                if (pBuf + Count > pEndBuf)
                {
                    break;
                }
    ...
    {
        //  Make sure the given buffer was large enough to hold the mapping.
        if (ctr < cchSrc)
        {
            SetLastError(ERROR_INSUFFICIENT_BUFFER);
            return (0);
        ...
        return ((int)((*ppIncr) - pDest));//  Return the number of characters written to the buffer.
    }
}
...
//  This first converts the characters to full width, and then maps all composite characters to their precomposed forms.
//  This handles the case where the half width composite form (TWO code points) should map to a
//  full width precomposed form (ONE full width code point).
//  The half width precomposed forms do not exist in Unicode and we need the full
//  width precomposed forms to round trip with the TWO half width code points.
...
//  11-04-93    JulieB    Created.
{
    ...
    {
        //  Convert a composite form to its full width precomposed form (if exists).
        //  Also, convert to case if necessary.
        Count = InsertFullWidthPreComposedForm( pPosSrc, pEndSrc, pBuf, pCase );
        pPosSrc += Count;
        ...
        (*ppIncr)++;//  Increment the destination pointer.
    }
...
//  05-07-96    JulieB    Created.
{
    ...
    //  If the destination value is zero, then just return the length of the source string.
    //  Do NOT touch pDest.
    if (cchDest == 0)
    {
        return (cchSrc);
    ...
    //  If cchSrc is greater than cchDest, then the destination buffer is too small to hold the new string.
    //  Return an error.
    if (cchSrc > cchDest)
    {
        SetLastError(ERROR_INSUFFICIENT_BUFFER);
