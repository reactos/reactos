...
Abstract:
    This file contains the main function, the main parsing functions, and all helper functions used by the various modules.
...
//  12-10-91    JulieB    Modified for new table format.
{
    ...
    while (fscanf(pInputFile, "%s", pszKeyWord) == 1)
    {
        ...
        else if (_strcmpi(pszKeyWord, "LOCALE") == 0)
        {
            ...
            if (ParseWriteLocale( &LocHdr, &LocStat, &LocVar, pszKeyWord ))
            {
                return (1);
            }
        }
...
int GetKeyParam(PSZ pszParam)
//  This routine gets the parameter for the keyword from the input file.
//  If the parameter is not there, then an error is returned.

//  12-10-91    JulieB    Created.
{
    //  Read the parameter from the input file.
    if (fscanf( pInputFile, "%s ;%*[^\n]", pszParam ) != 1)
    {
...
//  This routine gets the size of the table from the input file.
//  If the size is not there, then an error is returned.
