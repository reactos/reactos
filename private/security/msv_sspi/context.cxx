...

PSSP_CONTEXT SspContextReferenceContext(IN ULONG_PTR ContextHandle,IN BOOLEAN RemoveContext)
/*++
Routine Description:
    This routine checks to see if the Context is for the specified Client Connection, and references the Context if it is valid.

    The caller may optionally request that the Context be removed from the list of valid Contexts - preventing future requests from finding this Context.
Arguments:
    ContextHandle - Points to the ContextHandle of the Context to be referenced.
    RemoveContext - This boolean value indicates whether the caller wants the Context to be removed from the list of Contexts.
        TRUE indicates the Context is to be removed.
...
--*/
{
    ...
    for ( ListEntry = SspContextList.Flink; ListEntry != &SspContextList; ListEntry = ListEntry->Flink ) {
        Context = CONTAINING_RECORD( ListEntry, SSP_CONTEXT, Next );

        // Found a match ... reference this Context (if the Context is being removed, we would increment and then decrement the reference, so don't bother doing either - since they cancel each other out).
        if ( Context == (PSSP_CONTEXT) ContextHandle)
        {
            ...
            if (!RemoveContext)
            {
                ...
                // We only timeout contexts that are being setup, not fully authenticated contexts.
                if (SspTimeHasElapsed( Context->StartTime, Context->Interval))
                {
                    if ((Context->State != AuthenticatedState) && (Context->State != AuthenticateSentState) && (Context->State != PassedToServiceState))
                    {
...
VOID SspContextDereferenceContext(PSSP_CONTEXT Context)
/*++
Routine Description:
...
PSSP_CONTEXT SspContextAllocateContext()
/*++
Routine Description:
    This routine allocates the security context block, initializes it and links it onto the specified credential.
Return Value:
...
--*/
{
    ...
    // The reference count is set to 2.  1 to indicate it is on the valid Context list, and one for the our own reference.
...
NTSTATUS SspContextGetMessage(IN PVOID InputMessage, IN ULONG InputMessageSize, IN NTLM_MESSAGE_TYPE ExpectedMessageType, OUT PVOID* OutputMessage)
/*++
Routine Description:
...
Arguments:
    ...
    ExpectedMessageType - The type of message the should be in the message header.
    OutputMessage - Returns a pointer to an allocated buffer that contains the message.  The buffer should be freed using NtLmFree.
Return Value:
...
--*/
{
    ...
    RtlCopyMemory( TypicalMessage, InputMessage, InputMessageSize );

    // Validate the message header.
    if ( strncmp( (const char *)TypicalMessage->Signature, NTLMSSP_SIGNATURE, sizeof(NTLMSSP_SIGNATURE)) != 0 || TypicalMessage->MessageType != ExpectedMessageType ) {
...
VOID SspContextCopyString(IN PVOID MessageBuffer, OUT PSTRING32 OutString, IN PSTRING InString, IN OUT PCHAR *Where)
/*++
Routine Description:
    ...
    The descriptor 'address' is an offset from the MessageBuffer.

    Where is updated to point to the next available space in the MessageBuffer.

    The caller is responsible for any alignment requirements and for ensuring there is room in the buffer for the string.

Arguments:
    MessageBuffer - Specifies the base address of the buffer being copied into.
    OutString - Returns a descriptor for the copied string.  The descriptor is relative to the begining of the buffer.
...
--*/
{
    ...
    *Where += InString->Length;// Update Where to point past the copied data.
}


VOID SspContextCopyStringAbsolute(IN PVOID MessageBuffer,OUT PSTRING OutString,IN PSTRING InString,IN OUT PCHAR *Where)
/*++
Routine Description:
    This routine copies the InString into the MessageBuffer at Where.
    It then updates OutString to be a descriptor for the copied string.
...
BOOLEAN SspConvertRelativeToAbsolute (
...
Arguments:
    MessageBase - a pointer to the base of the buffer that the string is relative to.
        The MaximumLength field of the descriptor is forced to be the same as the Length field.
    MessageSize - Size of the message buffer (in bytes).
    StringToRelocate - A pointer to the string descriptor to make absolute.
    AlignToWchar - If TRUE the passed in StringToRelocate must describe a buffer that is WCHAR aligned.  If not, an error is returned.
    AllowNullString - If TRUE, the passed in StringToRelocate may be a zero length string.
Return Value:
...
--*/
{
    ...
    // If the buffer is allowed to be null, check that special case.
    if ( AllowNullString ) {
    ...
    if ( Offset >= MessageSize || Offset + StringToRelocate->Length > MessageSize ) {
        return FALSE;
    }
...

TimeStamp SspContextGetTimeStamp(IN PSSP_CONTEXT Context,IN BOOLEAN GetExpirationTime)
/*++
Routine Description:
...
VOID SspContextSetTimeStamp(IN PSSP_CONTEXT Context, IN LARGE_INTEGER ExpirationTime)
/*++
Routine Description:
...
--*/
{
    ...
    // If the expiration time is infinite, so is the interval
    if ( ExpirationTime.HighPart == 0x7FFFFFFF && ExpirationTime.LowPart == 0xFFFFFFFF ) {
        Context->Interval = INFINITE;

    // Handle non-infinite expiration times
    } else {
        // Compute the time remaining before the expiration time
        TimeRemaining.QuadPart = ExpirationTime.QuadPart - Context->StartTime.QuadPart;

        // If the time has already expired, indicate so.
        if ( TimeRemaining.QuadPart < 0 ) {
            Context->Interval = 0;

        // If the time hasn't expired, compute the number of milliseconds remaining.
        } else {
            MillisecondsRemaining = RtlExtendedMagicDivide(TimeRemaining, BaseGetTickMagicDivisor, BaseGetTickMagicShiftCount );
            if ( MillisecondsRemaining.HighPart == 0 && MillisecondsRemaining.LowPart < 0x7fffffff ) {
...

BOOL SsprCheckMinimumSecurity(IN ULONG NegotiateFlags, IN ULONG MinimumSecurityFlags)
/*++
Routine Description:
...
Return Value:
    ...
    FALSE   otherwise
Notes:
    The MinimumSecurityFlags can contain features that only apply if a key is needed when doing signing or sealing.
    These have to be removed if SIGN or SEAL is not in the NegotiateFlags.
--*/
{
    ...
    // if SIGN or SEAL is not negotiated, then remove all key related requirements, since they're not relevant when a key isn't needed
    if ((NegotiateFlags & (NTLMSSP_NEGOTIATE_SIGN | NTLMSSP_NEGOTIATE_SEAL)) == 0)
    {
        EffFlags &= ~(NTLMSSP_NEGOTIATE_128 | NTLMSSP_NEGOTIATE_56 | NTLMSSP_NEGOTIATE_KEY_EXCH);
    } else if ((NegotiateFlags & NTLMSSP_NEGOTIATE_SEAL) == 0) {
        // If SIGN is negotiated, but SEAL isn't, then remove flags that aren't relevant to encryption
...

SECURITY_STATUS SsprMakeSessionKey(
...
//        return(SEC_E_OK);
    ...
    if ((Context->NegotiateFlags & (NTLMSSP_NEGOTIATE_SIGN| NTLMSSP_NEGOTIATE_SEAL)) == 0)
    {
        RtlCopyMemory(Context->SessionKey,NtUserSessionKey,sizeof(LocalSessionKey));
        return SEC_E_OK;
    }

    if (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_NTLM2)
    {
        // when using NTLM2, LSA gets passed flags that cause it to make good session keys -- nothing for us to do
        RtlCopyMemory(LocalSessionKey,NtUserSessionKey,sizeof(LocalSessionKey));
    }
    else if( Context->NegotiateFlags & NTLMSSP_NEGOTIATE_LM_KEY )
    {
        ...
        RtlZeroMemory(LocalSessionKey,MSV1_0_USER_SESSION_KEY_LENGTH);
        ...
        // The LM session key is made by taking the LM sesion key given to us by the LSA, extending it to LM_OWF_LENGTH with our salt, and then producing a new challenge-response with it and the original challenge response.
        // The key is made from the first 8 bytes of the key.
        RtlCopyMemory(  &LmKey, LanmanSessionKey, MSV1_0_LANMAN_SESSION_KEY_LENGTH );
        memset( (PUCHAR)(&LmKey) + MSV1_0_LANMAN_SESSION_KEY_LENGTH, NTLMSSP_KEY_SALT, LM_OWF_PASSWORD_LENGTH - MSV1_0_LANMAN_SESSION_KEY_LENGTH );
        Status = RtlCalculateLmResponse((PLM_CHALLENGE) TemporaryResponse,&LmKey,&LmResponseKey);
        ZeroMemory( TemporaryResponse, sizeof(TemporaryResponse) );
        if (!NT_SUCCESS(Status))
            return(SspNtStatusToSecStatus(Status, SEC_E_NO_CREDENTIALS));

        RtlCopyMemory(LocalSessionKey,&LmResponseKey,MSV1_0_USER_SESSION_KEY_LENGTH);
    } else {
        RtlCopyMemory(LocalSessionKey,NtUserSessionKey,MSV1_0_USER_SESSION_KEY_LENGTH);
    }

    // If we aren't doing key exchange, store the session key in the context.
    // Otherwise encrypt the session key to send to the server.
    if (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_KEY_EXCH) {
        struct RC4_KEYSTRUCT Rc4Key;

        // make a key schedule from the temp key to form key exchange key
        rc4_key(&Rc4Key,MSV1_0_USER_SESSION_KEY_LENGTH,LocalSessionKey);
        if (DatagramSessionKey == NULL)
        {
            rc4(&Rc4Key,MSV1_0_USER_SESSION_KEY_LENGTH,Context->SessionKey);// decrypt what's in Context->SessionKey, leave it there
        } else {
            DatagramSessionKey->Length = DatagramSessionKey->MaximumLength = MSV1_0_USER_SESSION_KEY_LENGTH;// set the proper length so client will send something (length was 0)
            RtlCopyMemory(DatagramSessionKey->Buffer,Context->SessionKey,MSV1_0_USER_SESSION_KEY_LENGTH);// copy randomly generated key to buffer to send to server
            rc4(&Rc4Key,MSV1_0_USER_SESSION_KEY_LENGTH,(unsigned char*)DatagramSessionKey->Buffer);// encrypt it with the key exchange key
        }
    } else {
        // just make the temp key into the real one
        RtlCopyMemory(Context->SessionKey,LocalSessionKey,MSV1_0_USER_SESSION_KEY_LENGTH);
    }
...
--*/
{
    ...
    *NegotiateFlags = 0;
    ...
    Status = SspCredentialReferenceCredential(CredentialHandle,FALSE,FALSE,&Credential );
    if ( !NT_SUCCESS( Status ) )
    {
    ...
    *ContextHandle = (LSA_SEC_HANDLE) Context;
    ...
    if ( (ContextReqFlags & ISC_REQ_PROMPT_FOR_CREDS) != 0 ) {
        ...
        SspPrint(( SSP_CRITICAL, "SsprHandleFirstCall: invalid ContextReqFlags 0x%lx.\n", ContextReqFlags ));
        goto Cleanup;
    }

    // Capture the default credentials from the credential structure.
    if ( Credential->DomainName.Buffer != NULL ) {
        Status = NtLmDuplicateUnicodeString(&Context->DomainName, &Credential->DomainName );
        if (!NT_SUCCESS(Status)) {
            SspPrint(( SSP_CRITICAL, "SsprHandleFirstCall: NtLmDuplicateUnicodeString (DomainName) returned %d\n",Status));
            goto Cleanup;
        }
    }
    if ( Credential->UserName.Buffer != NULL ) {
        Status = NtLmDuplicateUnicodeString(&Context->UserName, &Credential->UserName);
        if (!NT_SUCCESS(Status)) {
            SspPrint(( SSP_CRITICAL, "SsprHandleFirstCall: NtLmDuplicateUnicodeString (UserName) returned %d\n", Status ));
            goto Cleanup;
        }
    }
    ...
    // If the caller specified INTEGRITY, SEQUENCE_DETECT or REPLAY_DETECT, that means they want to use the MakeSignature/VerifySignature calls.  Add this to the negotiate.
    if (ContextReqFlags & (ISC_REQ_INTEGRITY | ISC_REQ_SEQUENCE_DETECT | ISC_REQ_REPLAY_DETECT))
    {
        Context->NegotiateFlags |= NTLMSSP_NEGOTIATE_SIGN | NTLMSSP_NEGOTIATE_KEY_EXCH | NTLMSSP_NEGOTIATE_LM_KEY ;
    }
    ...
    if ((ContextReqFlags & ISC_REQ_CONFIDENTIALITY) != 0) {
        if (NtLmGlobalEncryptionEnabled) {
            ...
            *ContextAttributes |= ISC_RET_CONFIDENTIALITY;
            ...
            SspPrint(( SSP_CRITICAL, "SsprHandleFirstCall: NtLmGlobalEncryptionEnabled is FALSE\n"));
            goto Cleanup;
        }
    }
    ...
    // For connection oriented security, we send a negotiate message to the server.  For datagram, we get back the server's capabilities in the challenge message.
    if ((ContextReqFlags & ISC_REQ_DATAGRAM) == 0) {
        BOOLEAN CheckForLocal;

        if ( (Credential->DomainName.Buffer == NULL && Credential->UserName.Buffer == NULL && Credential->Password.Buffer == NULL ) && (Credential->ClientTokenHandle != NULL) )
        {
            CheckForLocal = TRUE;
        } else {
            CheckForLocal = FALSE;
        ...
        if( CheckForLocal ) {
            ...
            // the old way took the critsect twice, once to read sizes, second time to grab buffers - bad news if the global got bigger in between.
            ...
            if( NtLmGlobalOemComputerNameString.Buffer == NULL || NtLmGlobalOemPrimaryDomainNameString.Buffer == NULL ) {
                // user has picked a computer name or domain name that failed to convert to OEM.  disable the loopback detection.
                // Sometime beyond Win2k, Negotiate package should have a general, robust scheme for detecting loopback.
                CheckForLocal = FALSE;
            } else {
                Status = NtLmDuplicateString(&NtLmLocalOemComputerNameString, &NtLmGlobalOemComputerNameString);
                if( NT_SUCCESS(Status) ) {
                    Status = NtLmDuplicateString(&NtLmLocalOemPrimaryDomainNameString, &NtLmGlobalOemPrimaryDomainNameString );
                }
            }
        ...
        if ((ContextReqFlags & ISC_REQ_ALLOCATE_MEMORY) == 0)
        {
            if ( NegotiateMessageSize > *OutputTokenSize ) {
                ...
                SspPrint(( SSP_CRITICAL, "SsprHandleFirstCall: OutputTokenSize is %d\n", *OutputTokenSize));
                goto Cleanup;
            }
        }

        NegotiateMessage = (PNEGOTIATE_MESSAGE) NtLmAllocate( NegotiateMessageSize );
        ...
        // If this is the first call, build a Negotiate message.
        ...
        // Copy the DomainName and ComputerName into the negotiate message so the other side can determine if this is a call from the local system.

        // Pass the names in the OEM character set since the character set hasn't been negotiated yet.

        // Skip passing the workstation name if credentials were specified.
        // This ensures the other side doesn't fall into the case that this is the local system.
        // We wan't to ensure the new credentials are authenticated.
        ...
        if ( CheckForLocal ) {
            ...
            // OEM_DOMAIN_SUPPLIED used to always be supplied - but the only case it is ever used is when NTLMSSP_NEGOTIATE_LOCAL_CALL is set.
        ...
        if ((ContextReqFlags & ISC_REQ_ALLOCATE_MEMORY) == 0)
        {
            RtlCopyMemory( *OutputToken, NegotiateMessage, NegotiateMessageSize );
        }
        else
        {
    ...
    if ((ContextReqFlags & ISC_REQ_DATAGRAM) != 0) {
        ...
        *OutputTokenSize = 0;
        ...
        // Generate a session key for this context if sign or seal was requested.
        if ((Context->NegotiateFlags & (NTLMSSP_NEGOTIATE_SIGN | NTLMSSP_NEGOTIATE_SEAL)) != 0) {
        ...
        // Unless client wants to force its use, Turn off strong crypt, because we can't negotiate it.
        if (!NtLmGlobalDatagramUse128BitEncryption) {
            Context->NegotiateFlags &= ~NTLMSSP_NEGOTIATE_128;
        }

        // likewise for 56bit.  note that package init handles turning off 56bit if 128bit is configured for datagram.
        if(!NtLmGlobalDatagramUse56BitEncryption) {
            Context->NegotiateFlags &= ~NTLMSSP_NEGOTIATE_56;
        }

        //  Unless client wants to require NTLM2, can't use its message processing features because we start using MD5 sigs, full duplex mode, and datagram rekey before we know if the server supports NTLM2.
...
Cleanup:
    if ( Context != NULL ) {
        ...
        SspContextDereferenceContext( Context );// Always dereference it.
    }
...
--*/
{
    ...
    *ContextAttributes = 0;
    ...
    Status = SspCredentialReferenceCredential(CredentialHandle,FALSE,FALSE,&Credential );
    if ( !NT_SUCCESS( Status ) ) {
        SspPrint(( SSP_CRITICAL,"SsprHandleNegotiateMessage: invalid credential handle.\n" ));
        goto Cleanup;
    }

    if ( (Credential->CredentialUseFlags & SECPKG_CRED_INBOUND) == 0 ) {
        Status = SEC_E_INVALID_CREDENTIAL_USE;
        SspPrint(( SSP_CRITICAL,"SsprHandleNegotiateMessage: invalid credential use.\n" ));
        goto Cleanup;
    }
    ...
    if ( Context == NULL ) {
        Status = STATUS_NO_MEMORY;
        SspPrint(( SSP_CRITICAL, "SsprHandleNegotiateMessage: SspContextAllocateContext() returned NULL.\n" ));
        goto Cleanup;
    }
    ...
    *ContextHandle = (ULONG_PTR) Context;
    ...
    // Get the NegotiateMessage.  If we are re-establishing a datagram context then there may not be one.
    if ( InputTokenSize >= sizeof(OLD_NEGOTIATE_MESSAGE) ) {
        ...
        if ((ContextReqFlags & ASC_REQ_ALLOCATE_MEMORY) == 0)
        {
            if ( ChallengeMessageSize > *OutputTokenSize ) {
                SspPrint(( SSP_CRITICAL, "SsprHandleNegotiateMessage: invalid ChallengeMessageSize\n"));
                Status = SEC_E_BUFFER_TOO_SMALL;
                goto Cleanup;
            }
        }

        ChallengeMessage = (PCHALLENGE_MESSAGE) NtLmAllocate( ChallengeMessageSize );
        if ( ChallengeMessage == NULL ) {
            SspPrint(( SSP_CRITICAL, "SsprHandleNegotiateMessage: Error allocating ChallengeMessage.\n" ));
            Status = STATUS_NO_MEMORY;
            goto Cleanup;
        }
        ...
        // Check that both sides can use the same authentication model.
        // For compatibility with beta 1 and 2 (builds 612 and 683), no requested authentication type is assumed to be NTLM.
        // If NetWare is explicitly asked for, it is assumed that NTLM would have been also, so if it wasn't, return an error.
        ...
        if( (NegotiateMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_56) && (NegotiateFlagsKeyStrength & NTLMSSP_NEGOTIATE_56) )
        {
            ChallengeMessage->NegotiateFlags |= NTLMSSP_NEGOTIATE_56;
        }

        if( (NegotiateMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_128) && (NegotiateFlagsKeyStrength & NTLMSSP_NEGOTIATE_128) )
        {
            ChallengeMessage->NegotiateFlags |= NTLMSSP_NEGOTIATE_128;
        }

        // If the client supplied the Domain Name and User Name, and did not request datagram, see if the client is running on this local machine.
        if ( ( (NegotiateMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) == 0) &&
            ( (NegotiateMessage->NegotiateFlags & (NTLMSSP_NEGOTIATE_OEM_DOMAIN_SUPPLIED| NTLMSSP_NEGOTIATE_OEM_WORKSTATION_SUPPLIED)) == (NTLMSSP_NEGOTIATE_OEM_DOMAIN_SUPPLIED| NTLMSSP_NEGOTIATE_OEM_WORKSTATION_SUPPLIED) ) ) {
            // The client must pass the new negotiate message if they pass these flags
            ...
            // If both strings match, this is a local call.
            // The strings have already been uppercased.
            EnterCriticalSection (&NtLmGlobalCritSect);
            if ( RtlEqualString( &OemWorkstationName, &NtLmGlobalOemComputerNameString, FALSE ) && RtlEqualString( &OemDomainName, &NtLmGlobalOemPrimaryDomainNameString, FALSE ) ) {
                ChallengeMessage->NegotiateFlags |= NTLMSSP_NEGOTIATE_LOCAL_CALL;
                SspPrint(( SSP_MISC, "SsprHandleNegotiateMessage: Local Call.\n" ));
        ...
        if ( (NegotiateMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) == NTLMSSP_NEGOTIATE_DATAGRAM) {
            ChallengeMessage->NegotiateFlags |= NTLMSSP_NEGOTIATE_DATAGRAM;
        }
    } else {
        // No negotiate message.  We need to check if the caller is asking for datagram.
        if ((ContextReqFlags & ASC_REQ_DATAGRAM) == 0 ) {
            SspPrint(( SSP_CRITICAL,
                      "SsprHandleNegotiateMessage: "
        ...
        // Record in the context that we are doing datagram.  We will tell the client everything we can negotiate and let it decide what to negotiate.
        ChallengeMessage->NegotiateFlags = NTLMSSP_NEGOTIATE_DATAGRAM |
    ...
    Status = SspGenerateRandomBits( (UCHAR*)ChallengeMessage->Challenge, MSV1_0_CHALLENGE_LENGTH );
    if( !NT_SUCCESS( Status ) ) {
        SspPrint(( SSP_CRITICAL, "SsprHandleNegotiateMessage: SspGenerateRandomBits failed\n"));
        goto Cleanup;
    }
    ...
    // Save the Challenge and Negotiate Flags in the Context so it is available when the authenticate message comes in.
    ...
    if ((ContextReqFlags & ASC_REQ_ALLOCATE_MEMORY) == 0)
    {
        RtlCopyMemory( *OutputToken, ChallengeMessage, ChallengeMessageSize );
    }
    else
    {
...

NTSTATUS SsprHandleChallengeMessage(
    IN LSA_SEC_HANDLE CredentialHandle,
    IN OUT PLSA_SEC_HANDLE ContextHandle,
...
Arguments:
    ClientTokenHandle - Optionally passes in a handle to an impersonation token of the client.
        This impersonation token will be passed directly to the server if the server is running on the same machine.
        In that case, this routine will NULL the ClientTokenHandle letting the caller know that it need not close the handle.
        The server will close the handle when it's done with it.
...
--*/
{
    ...
    LPBYTE GetChallengeResponseBuffer[sizeof(MSV1_0_GETCHALLENRESP_REQUEST) + TARGET_INFO_LEN + (PWLEN+UNLEN+DNLEN+CNLEN+DNLEN+5) * sizeof(WCHAR) ];
    ...
    *NegotiateFlags = 0;
    ...
    GetChallengeResponse = (PMSV1_0_GETCHALLENRESP_REQUEST) GetChallengeResponseBuffer;
    ...
    if (*ContextHandle == NULL)
    {
        ...
        SecStatus = SspCredentialReferenceCredential(CredentialHandle, FALSE, FALSE, &Credential );
        if ( !NT_SUCCESS( SecStatus ) )
        {
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: SspCredentialReferenceCredential returns %x.\n", SecStatus ));
            goto Cleanup;
        }
        ...
        if (Context == NULL)
        {
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: SspContextAllocateContext returns NULL.\n" ));
            SecStatus = STATUS_NO_MEMORY;
            goto Cleanup;
        }

        // We've just added a context, we don't nornally add and then reference it.
        SspContextDereferenceContext( Context );
        ...
        *ContextHandle = (LSA_SEC_HANDLE) Context;
        ...
        if ( Credential->DomainName.Buffer != NULL ) {
            Status = NtLmDuplicateUnicodeString(&Context->DomainName, &Credential->DomainName );
            if (!NT_SUCCESS(Status)) {
                SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: NtLmDuplicateUnicodeString (DomainName) returned %d\n",Status));
                SecStatus = SspNtStatusToSecStatus( Status, SEC_E_INSUFFICIENT_MEMORY);
                goto Cleanup;
            }
        }
        if ( Credential->UserName.Buffer != NULL ) {
            Status = NtLmDuplicateUnicodeString(&Context->UserName, &Credential->UserName);
            if (!NT_SUCCESS(Status)) {
                SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: NtLmDuplicateUnicodeString (UserName) returned %d\n", Status ));
                SecStatus = SspNtStatusToSecStatus( Status, SEC_E_INSUFFICIENT_MEMORY);
                goto Cleanup;
            }
        }
        ...
        if (!NT_SUCCESS(SecStatus)) {
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: SspCredentialGetPassword returned %d\n", SecStatus ));
            goto Cleanup;
        }
        ...
        // fake it
        Context->NegotiateFlags = NTLMSSP_NEGOTIATE_UNICODE | NTLMSSP_NEGOTIATE_OEM | NTLMSSP_REQUEST_TARGET | NTLMSSP_REQUEST_INIT_RESPONSE | NTLMSSP_TARGET_TYPE_SERVER ;
        ...
        *ExpirationTime = SspContextGetTimeStamp(Context, TRUE);
        ...
        if (UseSuppliedCreds)
        {
            ...
            {
                SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: Invalid SecondInputTokensize.\n" ));
            ...
            {
                SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: Error while allocating NtLmChallengeMessage\n" ));
                SecStatus = STATUS_NO_MEMORY;
                goto Cleanup;
            }
            ...
            if ((NtLmChallengeMessage->Password.Length == 0) && (NtLmChallengeMessage->UserName.Length == 0) && (NtLmChallengeMessage->DomainName.Length == 0))
            {
                ...
                if (Context->Password.Buffer == NULL)
                {
                    SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: NtLmAllocate(Password) returns NULL.\n"));
                ...
                *(Context->Password.Buffer) = L'\0';
                ...
                if (Context->UserName.Buffer == NULL)
                {
                    SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: NtLmAllocate(UserName) returns NULL.\n"));
                    SecStatus = SEC_E_INSUFFICIENT_MEMORY;
                    goto Cleanup;
                }
                ...
                *(Context->UserName.Buffer) = L'\0';
                ...
                if (Context->DomainName.Buffer == NULL)
                {
                    SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: NtLmAllocate(DomainName) returns NULL.\n"));
                    SecStatus = SEC_E_INSUFFICIENT_MEMORY;
                    goto Cleanup;
                }
            ...
            else
            {
                ...
                if (NtLmChallengeMessage->Password.Buffer != 0)
                {
                    ...
                    SecStatus = NtLmDuplicateUnicodeString(&Context->Password, &AbsoluteString);
                    if (!NT_SUCCESS(SecStatus))
                    {
                        SspPrint(( SSP_CRITICAL,
                ...
                if (NtLmChallengeMessage->UserName.Length != 0)
                {
                    ...
                    SecStatus = NtLmDuplicateUnicodeString(&Context->UserName, &AbsoluteString);
                    if (!NT_SUCCESS(SecStatus))
                    {
                        SspPrint(( SSP_CRITICAL,
                ...
                if (NtLmChallengeMessage->DomainName.Length != 0)
                {
                    ...
                    SecStatus = NtLmDuplicateUnicodeString(&Context->DomainName, &AbsoluteString);
                    if (!NT_SUCCESS(SecStatus))
                    {
                        SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: NtLmDuplicateUnicodeString(DomainName) returns 0x%lx.\n",SecStatus ));
                        goto Cleanup;
                    }
                }
            }
    ...
    if ( Context == NULL ) {
        SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: invalid context handle.\n" ));
        ...
        if( Context->Credential == NULL ) {
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: invalid context handle, missing credential.\n" ));
            ASSERT( Context->Credential );
            SecStatus = STATUS_INVALID_HANDLE;
            goto Cleanup;
        }

        // If this is not reauthentication (or is datagram reauthentication) pull the context out of the associated credential.
    ...
    // If we have already sent the authenticate message, then this must be RPC calling Initialize a third time to re-authenticate a connection.
    // This happens when a new interface is called over an existing connection.
    // What we do here is build a NULL authenticate message that the server will recognize and also ignore.

    // That being said, if we are doing datagram style authentication then the story is different.
    // The server may have dropped this security context and then the client sent another packet over.
    // The server will then be trying to restore the context, so we need to build another authenticate message.
    if ( Context->State == AuthenticateSentState ) {
        ...
        if (((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) == NTLMSSP_NEGOTIATE_DATAGRAM) && (InputTokenSize != 0) && (InputToken != NULL) ) {
            // we are doing a reauthentication for datagram, so let this through.  We don't want the security.dll remapping this context.
            *ContextAttributes |= SSP_RET_REAUTHENTICATION;
        } else {
            // To make sure this is the intended meaning of the call, check that the input token is NULL.
            if ( (InputTokenSize != 0) || (InputToken != NULL) ) {
                SecStatus = SEC_E_INVALID_TOKEN;
                SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: (re-auth) invalid InputTokenSize.\n" ));
                goto Cleanup;
            }

            if ( (*OutputTokenSize < sizeof(NullMessage))  && ((ContextReqFlags & ISC_REQ_ALLOCATE_MEMORY) == 0))
            {
                ...
                if ((ContextReqFlags & ISC_REQ_ALLOCATE_MEMORY) == 0)
                {
                    RtlCopyMemory( *OutputToken, &NullMessage, sizeof(NullMessage));
                }
                else
                {
                    PAUTHENTICATE_MESSAGE NewNullMessage = (PAUTHENTICATE_MESSAGE) NtLmAllocate(sizeof(NullMessage));
    ...
    // Since this is the second call, the credential is implied by the Context.
    // We could double check that the Credential Handle is either NULL or correct.  However, our implementation doesn't maintain a close
    // association between the two (actually no association) so checking would require a lot of overhead.
    ...
    // make sure KEY_EXCH is always set if DATAGRAM negotiated and we need a key this is for local internal use; its now safe because we've got the bits to go on the wire copied...
    if ((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) && (Context->NegotiateFlags & (NTLMSSP_NEGOTIATE_SIGN |NTLMSSP_NEGOTIATE_SEAL)))
    {
        Context->NegotiateFlags |= NTLMSSP_NEGOTIATE_KEY_EXCH;
    }

    // allow negotiate of certain options such as sign/seal when server asked for it, but client didn't.
#if 0
    if( ChallengeMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_SEAL ) {
        if( (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_SEAL) == 0 ) {
            SspPrint(( SSP_API_MORE, "SsprHandleChallengeMessage: client didn't request SEAL but server did, adding SEAL.\n"));
        }
    ...
    if( ChallengeMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_SIGN ) {
        if( (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_SIGN) == 0 ) {
            SspPrint(( SSP_API_MORE, "SsprHandleChallengeMessage: client didn't request SIGN but server did, adding SIGN.\n"));
        }
    ...
    // if server didn't support certain crypto strengths, insure they are disabled.
    if( (ChallengeMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_56) == 0 ) {
        if( Context->NegotiateFlags & NTLMSSP_NEGOTIATE_56 ) {
            SspPrint(( SSP_NEGOTIATE_FLAGS, "SsprHandleChallengeMessage: Client supported 56, but server didn't.\n"));
        }
    ...
    if( (ChallengeMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_128) == 0 ) {
        if( Context->NegotiateFlags & NTLMSSP_NEGOTIATE_128 ) {
            SspPrint(( SSP_NEGOTIATE_FLAGS, "SsprHandleChallengeMessage: Client supported 128, but server didn't.\n"));
        }
    ...
    // not done for legacy down-level case, as, NegotiateFlags are constructed from incomplete information.
    if( !fCallFromRedir )
    {
        if (!SsprCheckMinimumSecurity(Context->NegotiateFlags, NtLmGlobalMinimumClientSecurity)) {
    ...
    // Determine that the caller negotiated to NTLM or nothing, but not NetWare.
    ...
    // If the server is running on this same machine, just duplicate our caller's token and use it.
    if ( ChallengeMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_LOCAL_CALL ) {
        ...
        // Require the new challenge message if we are going to access the server context handle
        if ( InputTokenSize < sizeof(CHALLENGE_MESSAGE) ) {
            SecStatus = SEC_E_INVALID_TOKEN;
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: invalid InputTokenSize.\n"));
            goto Cleanup;
        }

        // Open the server's context here within this process.
        ServerContextHandle = (ULONG_PTR)(ChallengeMessage->ServerContextHandle);
        ServerContext = SspContextReferenceContext(ServerContextHandle, FALSE );
        if ( ServerContext == NULL ) {
            // This means the server has lied about this being a local call or the server process has exitted.
            SspPrint(( SSP_CRITICAL,
                      "SsprHandleChallengeMessage: "
        ...
        // give local call a hard-coded session key so calls into RDR to fetch a session key succeed (eg: RtlGetUserSessionKeyClient)
    ...
    // If the server is running on a diffent machine, determine the caller's DomainName, UserName and ChallengeResponse to pass back in the AuthenicateMessage.
    } else {
        ...
        if ( (ChallengeMessage->NegotiateFlags & NTLMSSP_REQUEST_NON_NT_SESSION_KEY ) || (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_LM_KEY))
        {
        ...
        GetChallengeResponseSize += GetChallengeResponse->ServerName.Length + GetChallengeResponse->LogonDomainName.Length + GetChallengeResponse->UserName.Length;

        // Check for null session. This is the case if the caller supplies an empty username, domainname, and password.

        // duplicate the hidden password string, then reveal it into new buffer.  This avoids thread race conditions during hide/reveal and also allows us to avoid re-hiding the material.
...
#define NULL_SESSION_REQUESTED RETURN_RESERVED_PARAMETER
            ...
            // We aren't doing a null session intentionally. MSV may choose to do a null session if we have no credentials available.
            if ( Context->DomainName.Buffer == NULL ) {
            ...
            if ( Context->Password.Buffer == NULL ) {
                ...
                // Check to make sure the client's impersonation level is less than or equal to what they are asking for on the server. We only do this if we actually have their token.
                if (ClientTokenHandle != NULL) {
                    Status = NtQueryInformationToken(ClientTokenHandle,TokenImpersonationLevel,(PVOID) &ImpersonationLevel,TokenInformationSize,&TokenInformationSize);

                    // If the token is a primary token the return code will be STATUS_INVALID_INFO_CLASS because the token is forced to be equivalent to SecurityImpersonation.
                    if (NT_SUCCESS(Status)) {
                        if ((ImpersonationLevel ==  SecurityIdentification) && ((Context->ContextFlags & NTLMSSP_NEGOTIATE_IDENTIFY) == 0)) {
                            SecStatus = SEC_E_NO_CREDENTIALS;
                            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: Error 1, 0x%lx.\n", SecStatus));
                            goto Cleanup;
                        } else if (ImpersonationLevel != SecurityImpersonation) {
                            SecStatus = SEC_E_NO_CREDENTIALS;
                            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: Error 2, 0x%lx.\n", SecStatus));
                        }
                    } else if (Status != STATUS_INVALID_INFO_CLASS) {
                        SecStatus = SspNtStatusToSecStatus(Status, SEC_E_NO_CREDENTIALS );
                        SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: Error 3, 0x%lx.\n", SecStatus));
                        goto Cleanup;
                    }
                }
            } else {
                // MSV needs the password to be 'in' the passed in buffer.
                RtlCopyMemory(MarshalPtr, GetChallengeResponse->Password.Buffer, GetChallengeResponse->Password.Length + sizeof(WCHAR));
                GetChallengeResponse->Password.Buffer = (LPWSTR)(MarshalPtr);
                GetChallengeResponseSize += GetChallengeResponse->Password.Length + sizeof(WCHAR);
            }
        }

        // scrub the cleartext password now to avoid pagefile exposure during lengthy processing.
        ...
        if ((ChallengeResponseMessage->CaseSensitiveChallengeResponse.Length == 0) && (ChallengeResponseMessage->CaseInsensitiveChallengeResponse.Length == 1)) {
            *ContextAttributes |= ISC_RET_NULL_SESSION;
            ...
            // Normalize things by copying the default domain name and user name into the ChallengeResponseMessage structure.
            if ( Context->DomainName.Buffer != NULL ) {
        ...
        if ( DoUnicode ) {
            ...
            Status = RtlUpcaseUnicodeStringToOemString(&DomainName,&ChallengeResponseMessage->LogonDomainName,TRUE);
            if ( !NT_SUCCESS(Status) ) {
                SspPrint(( SSP_CRITICAL,"SsprHandleChallengeMessage: RtlUpcaseUnicodeToOemString (DomainName) returned 0x%lx.\n", Status));
                SecStatus = SspNtStatusToSecStatus( Status,SEC_E_INSUFFICIENT_MEMORY);
                goto Cleanup;
            }
            ...
            if ( !NT_SUCCESS(Status) ) {
                SspPrint(( SSP_CRITICAL,"SsprHandleChallengeMessage: RtlUpcaseUnicodeToOemString (UserName) returned 0x%lx.\n", Status));
                SecStatus = SspNtStatusToSecStatus( Status,SEC_E_INSUFFICIENT_MEMORY);
                goto Cleanup;
            }
        ...
        // Generate the session key, or encrypt the previosly generated random one, from various bits of info. Fill in session key if needed.
        ...
        {
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: SsprMakeSessionKey\n"));
            goto Cleanup;
        }
    }

    // If the caller specified SEQUENCE_DETECT or REPLAY_DETECT, that means they want to use the MakeSignature/VerifySignature calls.
    // Add this to the returned attributes and the context negotiate flags.
    ...
    if ((Context->NegotiateFlags & ChallengeMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_SEAL) || (ContextReqFlags & ISC_REQ_CONFIDENTIALITY)) {
        if (NtLmGlobalEncryptionEnabled) {
            Context->NegotiateFlags |= NTLMSSP_NEGOTIATE_SEAL;
            ...
            *ContextAttributes |= ISC_REQ_CONFIDENTIALITY;
            ...
            SecStatus = STATUS_NOT_SUPPORTED;
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: NtLmGlobalEncryption not enabled.\n"));
            goto Cleanup;
        }
    }

    if ((Context->NegotiateFlags & ChallengeMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) == NTLMSSP_NEGOTIATE_DATAGRAM ) {
        *ContextAttributes |= ISC_RET_DATAGRAM;
        Context->ContextFlags |= ISC_RET_DATAGRAM;
        Context->NegotiateFlags |= NTLMSSP_NEGOTIATE_DATAGRAM;
    }
    ...
    AuthenticateMessage = (PAUTHENTICATE_MESSAGE) NtLmAllocate(AuthenticateMessageSize );
    if ( AuthenticateMessage == NULL ) {
        SecStatus = STATUS_NO_MEMORY;
        SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: Error allocating AuthenticateMessage.\n" ));
        goto Cleanup;
    }
    ...
    if ((ContextReqFlags & ISC_REQ_ALLOCATE_MEMORY) == 0)
    {
        RtlCopyMemory( *OutputToken,AuthenticateMessage,AuthenticateMessageSize );
    }
    else
    {
    ...
    *OutputTokenSize = AuthenticateMessageSize;
    ...
    if (fCallFromRedir)
    {
        NtLmInitializeResponse = (PNTLM_INITIALIZE_RESPONSE) NtLmAllocate(sizeof(NTLM_INITIALIZE_RESPONSE));
        if ( NtLmInitializeResponse == NULL ) {
            SecStatus = STATUS_NO_MEMORY;
            SspPrint(( SSP_CRITICAL, "SsprHandleChallengeMessage: Error allocating NtLmInitializeResponse.\n" ));
            goto Cleanup;
        }
        RtlCopyMemory(NtLmInitializeResponse->UserSessionKey,ChallengeResponseMessage->UserSessionKey,MSV1_0_USER_SESSION_KEY_LENGTH);
        RtlCopyMemory(NtLmInitializeResponse->LanmanSessionKey,ChallengeResponseMessage->LanmanSessionKey,MSV1_0_LANMAN_SESSION_KEY_LENGTH);
...
Cleanup:
    ...
    if ( Context != NULL ) {
        ...
        *NegotiateFlags = Context->NegotiateFlags;
        ...
        // If we just created the context (because rdr may be talking to a pre NT 5.0 server,, we need to dereference it again.
...
--*/
{
    ...
    *UserFlags = 0;
    ...
    if (*ContextHandle == NULL)
    {
        ...
        if ( !NT_SUCCESS( SecStatus ) )
        {
            SspPrint(( SSP_CRITICAL, "SsprHandleAuthenticateMessage: SspCredentialReferenceCredential returns %x.\n", SecStatus ));
            goto Cleanup;
        }
        ...
        {
            SspPrint(( SSP_CRITICAL,"SsprHandleAuthenticateMessage: NtlmAuthenticateMessage size if bogus.\n" ));
            SecStatus = SEC_E_INVALID_TOKEN;
            goto Cleanup;
        }
        ...
        if (ContextReqFlags  & ISC_REQ_ALLOCATE_MEMORY)
        {
            SspPrint(( SSP_CRITICAL,"SsprHandleAuthenticateMessage: ContextReqFlags has ISC_REQ_ALLOCATE_MEMORY.\n" ));
            SecStatus = STATUS_NOT_SUPPORTED;
            goto Cleanup;
        }
        ...
        {
            SspPrint(( SSP_CRITICAL,"SsprHandleAuthenticateMessage: NtlmAcceptResponse size if bogus.\n" ));
            SecStatus = SEC_E_INVALID_TOKEN;
            goto Cleanup;
        }
        ...
        *ContextHandle = (LSA_SEC_HANDLE) Context;
        ...
        {
            SspPrint(( SSP_CRITICAL, "SsprHandleAuthenticateMessage: Error while assigning NtLmAuthenticateMessage\n" ));
            SecStatus = STATUS_NO_MEMORY;
            goto Cleanup;
        }
        ...
        {
            fSubAuth = TRUE;
            SubAuthPackageId = (NtLmAuthenticateMessage->ParameterControl >> MSV1_0_SUBAUTHENTICATION_DLL_SHIFT);
        }
        Context->State = ChallengeSentState;
        Context->NegotiateFlags = NTLMSSP_NEGOTIATE_UNICODE ;

        // The server may request this option with a <= 4.0 client, in which case HandleNegotiateMessage, which normally sets this flag, won't have been called.
        if ( (ContextReqFlags & ASC_REQ_ALLOW_NON_USER_LOGONS ) != 0) {
            *ContextAttributes |= ASC_RET_ALLOW_NON_USER_LOGONS;
            Context->ContextFlags |= ASC_RET_ALLOW_NON_USER_LOGONS;
        }
    ...
    if ( Context == NULL ) {
        SecStatus = STATUS_INVALID_HANDLE;
        SspPrint(( SSP_CRITICAL, "SsprHandleAuthenticateMessage: Error from SspContextReferenceContext.\n" ));
        goto Cleanup;
    }

    if ( ( Context->State != ChallengeSentState) && ( Context->State != AuthenticatedState) ) {
        SspPrint(( SSP_CRITICAL,
    ...
    // If the call comes and we have already authenticated, then it is probably RPC trying to reauthenticate, which happens when someone calls two interfaces on the same connection.
    // In this case we don't have to do anything - we just return success and let them get on with it.
    // We do want to check that the input token is all zeros, though.
    if ( Context->State == AuthenticatedState ) {
        ...
        *OutputTokenSize = 0;
        ...
        if (memcmp(&AuthenticateMessage->LmChallengeResponse, &NullMessage.LmChallengeResponse, sizeof(STRING32) * 5) ) {
            SecStatus = SEC_E_INVALID_TOKEN;
            SspPrint(( SSP_CRITICAL,
                  "SsprHandleAuthenticateMessage: "
    ...
    // If we are re-establishing a datagram context, get the negotiate flags out of this message.
    if ((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) != 0) {
        if ((InputTokenSize < sizeof(AUTHENTICATE_MESSAGE)) || ((AuthenticateMessage->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) == 0) ) {
            SecStatus = SEC_E_INVALID_TOKEN;
            goto Cleanup;
        }
    ...
    if( !fCallFromSrv )
    {
        if (!SsprCheckMinimumSecurity(AuthenticateMessage->NegotiateFlags, NtLmGlobalMinimumServerSecurity)) {
            SecStatus = SEC_E_UNSUPPORTED_FUNCTION;
            SspPrint(( SSP_CRITICAL,
                      "SsprHandleAuthenticateMessage: "
    ...
    if ( !SspConvertRelativeToAbsolute( AuthenticateMessage,
                                        ...
                                        DoUnicode, // Unicode alignment
#ifdef notdef
        // Allow null sessions.  The server should guard against them if it doesn't want them.
...
///    if ((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_DATAGRAM) != 0) {
    if ((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_KEY_EXCH) != 0) {
        ...
        if (((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_LOCAL_CALL) == 0) && (SessionKeyString.Buffer == NULL)) {
            SecStatus = SEC_E_INVALID_TOKEN;
            goto Cleanup;
        }
    ...
    // If the client is on the same machine as we are, just use the token the client has already placed in our context structure,
    if ( (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_LOCAL_CALL ) &&
    ...
    // If the client is on a different machine than we are, use LsaLogonUser to create a token for the client.
    } else {
        //  Store user name and domain name
        SecStatus = NtLmDuplicateUnicodeString(&Context->UserName, &UserName);
        if (!NT_SUCCESS(SecStatus)) {
            goto Cleanup;
        }

        SecStatus = NtLmDuplicateUnicodeString(&Context->DomainName, &DomainName);
        if (!NT_SUCCESS(SecStatus)) {
            goto Cleanup;
        }
        ...
        if (!fSubAuth)
        {
            ...
            MsvLogonMessage = (PMSV1_0_LM20_LOGON) NtLmAllocate(MsvLogonMessageSize );
            if ( MsvLogonMessage == NULL ) {
                SecStatus = STATUS_NO_MEMORY;
                SspPrint(( SSP_CRITICAL, "SsprHandleAuthenticateMessage: Error allocating MsvLogonMessage"));
                goto Cleanup;
            }
            ...
            SspContextCopyStringAbsolute( MsvLogonMessage,&MsvLogonMessage->CaseSensitiveChallengeResponse,&NtChallengeResponse,&Where );
            SspContextCopyStringAbsolute(MsvLogonMessage,&MsvLogonMessage->CaseInsensitiveChallengeResponse,&LmChallengeResponse,&Where );
            ...
            // By passing in the RETURN_PASSWORD_EXPIRY flag, the password expiration time is returned in the logoff time
        ...
        else
        {
            ...
            MsvSubAuthLogonMessage = (PMSV1_0_SUBAUTH_LOGON) NtLmAllocate(MsvSubAuthLogonMessageSize );
            if ( MsvSubAuthLogonMessage == NULL ) {
                SecStatus = STATUS_NO_MEMORY;
                SspPrint(( SSP_CRITICAL, "SsprHandleAuthenticateMessage: Error allocating MsvSubAuthLogonMessage"));
                goto Cleanup;
            }
            ...
            SspContextCopyStringAbsolute( MsvSubAuthLogonMessage,&MsvSubAuthLogonMessage->AuthenticationInfo1,&LmChallengeResponse,&Where );
            SspContextCopyStringAbsolute(MsvSubAuthLogonMessage,&MsvSubAuthLogonMessage->AuthenticationInfo2,&NtChallengeResponse,&Where );
        ...
        // But, special case for null sessions. since we already negotiated NTLM2, but the LmChallengeResponse field is actually used here.
        // REVIEW -- maybe don't negotiate NTLM2 if NULL session
        if((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_NTLM2)  && (AuthenticateMessage->LmChallengeResponse.Length >= MSV1_0_CHALLENGE_LENGTH))
        {
        ...
        // be LsaTokenInformationNull if it is. If so, we may need to fail the logon.
        if (TokenInformationType == LsaTokenInformationNull)
        {
// RESTRICT_NULL_SESSIONS deemed too risky because legacy behavior of package allows null sessions from SYSTEM.
        ...
        else
        {
            ...
            if ( AuditSid )
            {
                RtlCopyMemory( AuditSid, TokenInfoV2->User.User.Sid, RtlLengthSid( TokenInfoV2->User.User.Sid ) );
            }
        }
        ...
        Status = LsaFunctions->CreateToken(
                    ...
                    &SourceContext,
                    Network,
                    (((Context->NegotiateFlags & NTLMSSP_NEGOTIATE_IDENTIFY) != 0) ? SecurityIdentification : SecurityImpersonation),
                    TokenInformationType,
                    TokenInformation,
                    NULL,
                    AccountName,
...
//                (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_SIGN) ||
        ...
        *UserFlags = LogonProfileMessage->UserFlags;
        ...
        if (Context->NegotiateFlags & NTLMSSP_NEGOTIATE_KEY_EXCH && AuthenticateMessage->SessionKey.Length == MSV1_0_USER_SESSION_KEY_LENGTH)
        {
            RtlCopyMemory(Context->SessionKey, SessionKeyString.Buffer, MSV1_0_USER_SESSION_KEY_LENGTH);
        }

        // Generate the session key, or decrypt the generated random one sent to us by the client, from various bits of info
    ...
    // from the one the caller passed in. This may happen with temp duplicate accounts and local account
    ...
    *ExpirationTime = SspContextGetTimeStamp( Context, TRUE );
    ...
    if ( Context->NegotiateFlags & NTLMSSP_NEGOTIATE_SIGN ) {
        *ContextAttributes |= ASC_RET_REPLAY_DETECT | ASC_RET_SEQUENCE_DETECT | ASC_RET_INTEGRITY;
    }
...
Cleanup:
    ...
    // Audit this logon
    if (NT_SUCCESS(SecStatus)) {
        // If we don't have an account name, this was a local connection and we didn't build a new token, so don't bother auditing.
        // also, don't bother auditing logons that fellback to guest.
        if ( (AccountName != NULL) && (AccountName->Length != 0) && !fAvoidGuestAudit ) {
            LsaFunctions->AuditLogon(STATUS_SUCCESS,STATUS_SUCCESS,AccountName,AuthenticatingAuthority,WorkstationName,AuditSid,Network,&SourceContext,&LogonId);
        }
    } else {
    ...
    if ( Context != NULL ) {
        ...
        if ( NT_SUCCESS(SecStatus) ) {
            ...
            *NegotiateFlags = Context->NegotiateFlags;
            ...
            // if caller wants only INTEGRITY, then wants application supplied sequence numbers...
            if ((Context->ContextFlags & (ASC_REQ_INTEGRITY | ASC_REQ_REPLAY_DETECT | ASC_REQ_SEQUENCE_DETECT)) == ASC_REQ_INTEGRITY)
            {
                *NegotiateFlags |= NTLMSSP_APP_SEQ;
            }
        } else {
        ...
        // If we just created this context, then we need to dereference it once more with feeling
...
NTSTATUS SsprDeleteSecurityContext (IN OUT ULONG_PTR ContextHandle)
/*++
Routine Description:
    Deletes the local data structures associated with the specified security context and generates a token which is passed to a remote peer so it too can remove the corresponding security context.

    This API terminates a context on the local machine, and optionally provides a token to be sent to the other machine.
    The OutputToken generated by this call is to be sent to the remote peer (initiator or acceptor).
    If the context was created with the I _REQ_ALLOCATE_MEMORY flag, then the package will allocate a buffer for the output token.
...
NTSTATUS SspContextInitialize(VOID)
/*++
Routine Description:
...
--*/
{
    ...
    while ( !IsListEmpty( &SspContextList ) ) {
        ...
        ContextHandle = (LSA_SEC_HANDLE) CONTAINING_RECORD( SspContextList.Flink, SSP_CONTEXT, Next );
