...
// If the registry key does not exist, then this default constant defines how many logon cache entries will be active.
// The max constant places an upper limit on how many cache entries we will support.
// If the user specifies more than the max value, we will use the max value instead.
...
// This structure is saved on disk and provides information about the rest of the cache.
// This structure is in a value named "NL$Control" under the cache registry key.
typedef struct _NLP_CACHE_CONTROL
{
    ULONG       Revision;// Revision of the cache on-disk structure
    ULONG       Entries;// The current on-disk size of the cache (number of entries)
...
typedef struct _NLP_CTE{
    // CTEs are linked on either an invalid list (in any order) or on a valid list (in ascending order of time).
    // This makes it easy to figure out which entry is to be flushed when adding to the cache.
    LIST_ENTRY Link;

    // Time the cache entry was established.
    // This is used to determine which cache entry is the oldest, and therefore will be flushed from the cache first to make room for new entries.
    LARGE_INTEGER       Time;

    // This field contains the index of the CTE within the CTE table.
    // This index is used to generate the names of the entrie's secret key and cache key in the registry.
    // This field is valid even if the entry is marked Inactive.
    ULONG               Index;

    // Normally, we walk the active and inactive lists to find entries.
    // When growing or shrinking the cache, however, it is nice to be able to walk the table using indexes.
    // In this case, it is nice to have a local way of determining whether an entry is on the active or inactive list.
    // This field provides that capability.
...
// This structure is used for keeping track of all information that is stored on backing store.
...
NTSTATUS NlpConvert1_0To1_0B(VOID);
NTSTATUS NlpOpen_Nt1_0_Secret(VOID);
...
NTSTATUS NlpBuildCacheEntry(IN PNETLOGON_INTERACTIVE_INFO LogonInfo,
...
NTSTATUS NlpAddSupplementalCacheData(IN PVOID SupplementalCacheData,
...
NTSTATUS NlpMakeSecretPassword(OUT PLSAPR_CR_CIPHER_VALUE Passwords,
                               IN PUNICODE_STRING UserName,
                               IN PNT_OWF_PASSWORD NtOwfPassword OPTIONAL,
                               IN PLM_OWF_PASSWORD LmOwfPassword OPTIONAL);
NTSTATUS NlpMakeSecretPasswordNT5(IN OUT PCACHE_PASSWORDS Passwords,
                                  IN PUNICODE_STRING UserName,
                                  IN PNT_OWF_PASSWORD NtOwfPassword OPTIONAL,
                                  IN PLM_OWF_PASSWORD LmOwfPassword OPTIONAL);
NTSTATUS NlpReadCacheEntry(IN PUNICODE_STRING DomainName,
                           IN PUNICODE_STRING UserName,
                           OUT PULONG Index,
...
NTSTATUS NlpBuildAccountInfo(IN  PLOGON_CACHE_ENTRY pCacheEntry, IN  ULONG EntryLength, OUT PNETLOGON_VALIDATION_SAM_INFO2* AccountInfo);
...
VOID DumpAccountInfo(IN  PNETLOGON_VALIDATION_SAM_INFO2 AccountInfo);
VOID DumpCacheEntry(IN  ULONG              Index, IN  PLOGON_CACHE_ENTRY pEntry);
#endif //DBG
...
// This boolean indicates whether or not we have been able to initialize caching yet.
// It turn out that during authentication package load time, we can't do everything we would like to (like call LSA RPC routines).
// So, we delay initializing until we can call LSA.
// All publicly exposed interfaces must check this value before assuming work can be done.
...
// It also indicates what time each entry was established so we know which order to discard them in.

//  This field is a pointer to an array of CTEs.
//  The number of CTEs in the array is in NlpCacheControl.Entries.
//  This structure is allocated at initialization time.
PNLP_CTE            NlpCteTable;

// The Cache Table Entries in NlpCteTable are linked on either an active or inactive list.
// The entries on the active list are in ascending time order - so the last one on the list is the first
// one to be discarded when a flush is needed to add a new entry.
LIST_ENTRY          NlpActiveCtes;
LIST_ENTRY          NlpInactiveCtes;

// global, per-machine key used for encrypting NT_5_0 version cache entries.
...
Routine Description:
    ...
    (we can't open LSA, for example).  So, defer initialization until later.
--*/
{
...
NTSTATUS NlpGetCacheEntry(IN  PNETLOGON_LOGON_IDENTITY_INFO LogonInfo,
...
/*++
Routine Description:
    If the user logging on has information stored in the cache, then it is retrieved.
    Also returns the cached password from 'secret' storage
Arguments:
    LogonInfo   - pointer to NETLOGON_IDENTITY_INFO structure which contains the domain name, user name for this user
    AccountInfo - pointer to NETLOGON_VALIDATION_SAM_INFO2 structure to receive this user's specific interactive logon information
    Passwords   - pointer to CACHE_PASSWORDS structure to receive passwords returned from secret storage
Return Value:
    NTSTATUS
        Success = STATUS_SUCCESS
                    *AccountInfo points to a NETLOGON_VALIDATION_SAM_INFO2 structure. This must be freed by caller

                    *Passwords contain USER_INTERNAL1_INFORMATION structure which contains NT OWF password and LM OWF password.
                    These must be used to validate the logon
...
{
    NTSTATUS         NtStatus;
    PNETLOGON_VALIDATION_SAM_INFO2 SamInfo = NULL;
    ...
    *AccountInfo = NULL;
    ...
    NtStatus = NlpReadCacheEntry(&LogonInfo->LogonDomainName, &LogonInfo->UserName, &Index, &CacheEntry, &EntrySize);
    if (!NT_SUCCESS(NtStatus)) {
        LEAVE_CACHE();
        return (NtStatus);
    }
    ...
    if (CacheEntry->Revision >= NLP_CACHE_REVISION_NT_5_0) {
        // for NT5, we can release the cache lock now, since all data stored in one place.
        LEAVE_CACHE();
        fCacheLocked = FALSE;
        ...
        if (ppSupplementalCacheData  && SupplementalCacheDataLength) {
            ...
            *ppSupplementalCacheData = MIDL_user_allocate(*SupplementalCacheDataLength);
            if (*ppSupplementalCacheData == NULL) {
                ...
                // note: the decrypt operation that occurred during the ReadCacheEntry validates any data and pointers through
                // integrity checking via HMAC.  Having said that, we can be lazy and not do boundry checking.
                Source = ((LPBYTE)CacheEntry + CacheEntry->SupplementalCacheDataOffset);
                CopyMemory(*ppSupplementalCacheData, Source, *SupplementalCacheDataLength);
            }
        }
    }
    ...
    if (NT_SUCCESS(NtStatus)) {
        if (CacheEntry->Revision >= NLP_CACHE_REVISION_NT_5_0) {
            ...
            // prior to NT5, the Passwords are stored separately in their own LSA secret.
            ...
            if (NT_SUCCESS(NtStatus)) {
                if (CurrentSecret) {
                    // Check to see which version of the passwords are stored here - the normal or the salted.
                    RtlCopyMemory((PVOID)Passwords, (PVOID)CurrentSecret->Buffer, (ULONG)CurrentSecret->Length);
                    if (CacheEntry->Revision < NLP_CACHE_REVISION_NT_4_SP4) {
                        if (Passwords->SecretPasswords.NtPasswordPresent) {
                            NtStatus = NlpComputeSaltedHashedPassword(&Passwords->SecretPasswords.NtOwfPassword,
                        ...
                        if (NT_SUCCESS(NtStatus) && Passwords->SecretPasswords.LmPasswordPresent) {
                            NtStatus = NlpComputeSaltedHashedPassword(&Passwords->SecretPasswords.LmOwfPassword,
...
NTSTATUS NlpAddCacheEntry(IN  PNETLOGON_INTERACTIVE_INFO LogonInfo,
                          IN  PNETLOGON_VALIDATION_SAM_INFO2 AccountInfo,
    ...
    Arguments:
        LogonInfo   - pointer to NETLOGON_INTERACTIVE_INFO structure which contains the domain name, user name and password for this user.
                      These are what the user typed to WinLogon
        AccountInfo - pointer to NETLOGON_VALIDATION_SAM_INFO2 structure which contains this user's specific interactive logon information
    Return Value:
        NTSTATUS
            Success = STATUS_SUCCESS
    ...
    --*/
{
    ...
    ULONG         EntrySize, EntrySizeExisting, Index;
    ...
    // (this is new for NT5).
    NtStatus = NlpAddSupplementalCacheData(SupplementalCacheData, SupplementalCacheDataLength, &CacheEntry, &EntrySize);
    if (!NT_SUCCESS(NtStatus)) {
        goto Cleanup;
    }
    ...
    NtStatus = NlpReadCacheEntry(&LogonInfo->Identity.LogonDomainName, &LogonInfo->Identity.UserName, &Index, &CacheEntryExisting, &EntrySizeExisting);
    if (!NT_SUCCESS(NtStatus)) {// If we didn't find an entry, then we need to allocate an entry.
        NlpGetFreeEntryIndex(&Index);
        CacheEntryExisting = NULL;
    } else {
        // We already have an entry for this user.
        // Discard the structure we got back but use the same index.
...
--*/
{
    ...
    CopyMemory(((PBYTE)(NewCacheEntry)+*pEntryLength), SupplementalCacheData, SupplementalCacheDataLength);
...
/*++
Routine Description:
    Deletes a user account from the local user account cache, if the corresponding entry can be found.
    We actually just null out the current contents instead of destroying the storage - this should save us some time when we next come to add an entry to the cache
Arguments:
    LogonInfo   - pointer to NETLOGON_INTERACTIVE_INFO structure which contains the domain name, user name and password for this user
Return Value:
    NTSTATUS
...
VOID NlpChangeCachePassword(IN PUNICODE_STRING DomainName,
                            IN PUNICODE_STRING UserName,
    ...
    --*/
{
    ...
    NtStatus = NlpReadCacheEntry(DomainName, UserName, &Index, &CacheEntry, &EntrySize);
    if (!NT_SUCCESS(NtStatus)) {
        LEAVE_CACHE();
        return;
    }
    ...
    if (CacheEntry->Revision >= NLP_CACHE_REVISION_NT_5_0) {
        ...
        CachedUser.Length = CachedUser.MaximumLength = CacheEntry->UserNameLength;
        CachedUser.Buffer = (PWSTR)((PBYTE)CacheEntry + sizeof(LOGON_CACHE_ENTRY));
        NtStatus = NlpMakeSecretPasswordNT5(&CacheEntry->CachePasswords, &CachedUser, NtOwfPassword, LmOwfPassword);
        if (NT_SUCCESS(NtStatus)) {
            // encrypt the entry...
            NtStatus = NlpEncryptCacheEntry(CacheEntry, EntrySize);
        }
        ...
        if (NT_SUCCESS(NtStatus)) {
            ...
            if (NT_SUCCESS(NtStatus)) {
                ...
                NtStatus = NlpMakeSecretPassword(&Passwords, &CachedUser, NtOwfPassword, LmOwfPassword);
                if (NT_SUCCESS(NtStatus)) {
                    NtStatus = NlpWriteSecret(&Passwords, CurrentSecret);
                    FreeToHeap(Passwords.Buffer);// free the buffer allocated to store the passwords
                }
...
--*/
{
    ...
    // Upon return from this routine, if logon caching is enabled, the following will be true:
    //      A handle to the LsaPolicy object will be open (NlpLsaHandle).
    //      A handle to the registry key in which all cache entries are held will be open (NlpCacheHandle).
    //      A global structure defining how many cache entries there are will be initialized (NlpCacheControl).
    //      The Cache Table Entry table (CTE table) will be initialized (NlpCteTable).
    //      The active and inactive CTE lists will be built (NlpActiveCtes and NlpInactiveCtes).
    ...
    if (NlpInitializationNotYetPerformed) {
        ...
        if (NT_SUCCESS(NtStatus)) {
            // Successfully, or unsucessfully, The definition of "initialized" is we could call LSA's RPC routines.
            ...
            if (NT_SUCCESS(NtStatus)) {
                // Get information on the current cache structure (number of entries, et cetera).
                // This information is placed in a global variable for use throughout this module.
                ...
                // If we were successful, then see if we need to change the cache due to new user-specified cache size.
                if (NT_SUCCESS(NtStatus)) {
        ...
        // If we had an error, then set our entry count to zero to prevent using any cache information.
        if (!NT_SUCCESS(NtStatus)) {
...
/*++
Routine Description:
    Initializes the Global variable NlpCacheEncryptionKey with a per-machine cache encryption key.
    If the per-machine key does not exist as an LSA secret, it is created.
--*/
{
    ...
    NtStatus = I_LsarOpenSecret(NlpLsaHandle, (PLSAPR_UNICODE_STRING)&ValueName, SECRET_QUERY_VALUE | SECRET_SET_VALUE, &SecretHandle);
    if (!NT_SUCCESS(NtStatus)) {
        ...
        NtStatus = I_LsarCreateSecret(NlpLsaHandle, (PLSAPR_UNICODE_STRING)&ValueName, SECRET_SET_VALUE, &SecretHandle);
        if (!NT_SUCCESS(NtStatus)) {
            return (NtStatus);
        }
...
--*/
{
    ...
    // derive encryption key from global machine LSA secret, and random cache entry key.
...
NTSTATUS NlpBuildCacheEntry(IN  PNETLOGON_INTERACTIVE_INFO LogonInfo,
    ...
    Arguments:
        LogonInfo       - pointer to NETLOGON_INTERACTIVE_INFO structure containing user's name and logon domain name
        AccountInfo     - pointer to NETLOGON_VALIDATION_SAM_INFO2 from successful logon
    ...
    Return Value:
        NTSTATUS
            Success = STATUS_SUCCESS
                        *ppCacheEntry contains pointer to allocated LOGON_CACHE_ENTRY structure
            Failure = STATUS_NO_MEMORY
    ...
    --*/
{
    ...
    *pEntryLength = length;
    ...
    // each of these (unicode) strings and other structures are copied to the end of the fixed LOGON_CACHE_ENTRY structure, each aligned on DWORD boundaries
    ...
    NtStatus = RtlCopySid(pEntry->LogonDomainIdLength, (PSID)dataptr, AccountInfo->LogonDomainId);
...
/*++
Routine Description:
    Opens the registry node for read or write (depending on Switch) and opens the secret storage in the same mode.
    If successful, the NlpCacheHandle is valid.
Return Value:
    NTSTATUS
        Success = STATUS_SUCCESS
                    NlpCacheHandle contains handle to use for reading/writing registry
        Failure =
--*/
{
    NTSTATUS NtStatus;
...
/*++
Routine Description:
    Opens a cache entry's secret storage object for read (in order to LsaQuerySecret) and write (in order to LsaSetSecret).
    If successful, the handle value is placed in the global variable NlpSecretHandle.
...
--*/
{
    ...
    NtStatus = I_LsarOpenSecret(NlpLsaHandle, (PLSAPR_UNICODE_STRING)&ValueName, SECRET_QUERY_VALUE | SECRET_SET_VALUE, &NlpSecretHandle);
    if (!NT_SUCCESS(NtStatus)) {
        if (NtStatus == STATUS_OBJECT_NAME_NOT_FOUND) {
            NtStatus = I_LsarCreateSecret(NlpLsaHandle, (PLSAPR_UNICODE_STRING)&ValueName, SECRET_SET_VALUE | SECRET_QUERY_VALUE, &NlpSecretHandle);
            if (!NT_SUCCESS(NtStatus)) {
...
#if DBG
    {
        ...
        if (DumpCacheInfo) {
            DbgPrint("NlpReadSecret: NewSecret.Nt = \"%s\"\n"
                     ...
                     "            OldSecret.Nt = \"%s\"\n"
                     "            OldSecret.Lm = \"%s\"\n",
                     *NewSecret ? DumpOwfPasswordToString(newNt, (PLM_OWF_PASSWORD)((*NewSecret)->Buffer)) : "",
                     *NewSecret ? DumpOwfPasswordToString(newLm, (PLM_OWF_PASSWORD)((*NewSecret)->Buffer) + 1) : "",
                     *OldSecret ? DumpOwfPasswordToString(oldNt, (PLM_OWF_PASSWORD)((*OldSecret)->Buffer)) : "",
                     *OldSecret ? DumpOwfPasswordToString(oldLm, (PLM_OWF_PASSWORD)((*OldSecret)->Buffer) + 1) : ""
            );
        }
    }
#endif
...
NTSTATUS NlpMakeSecretPassword(OUT PLSAPR_CR_CIPHER_VALUE Passwords,
    ...
    /*++
    Routine Description:
        Converts a (fixed length structure) NT_OWF_PASSWORD and a LM_OWF_PASSWORD to a UNICODE_STRING.
        Allocates memory for the unicode string in this function

        The calling function must free up the string buffer allocated in this routine.
        The caller uses FreeToHeap (RtlFreeHeap)

    Arguments:
        Passwords       - returned UNICODE_STRING which actually contains a CACHE_PASSWORDS structure
    ...
    --*/
{
    ...
    // concatenate the fixed length NT_OWF_PASSWORD and LM_OWF_PASSWORD structures into a buffer which we then use as a UNICODE_STRING
    if (ARGUMENT_PRESENT(NtOwfPassword)) {
        Status = NlpComputeSaltedHashedPassword(&pwd->SecretPasswords.NtOwfPassword, NtOwfPassword, UserName);
        if (!NT_SUCCESS(Status)) {
            goto Cleanup;
        }
        ...
        RtlZeroMemory((PVOID)&pwd->SecretPasswords.NtOwfPassword, sizeof(pwd->SecretPasswords.NtOwfPassword));
        pwd->SecretPasswords.NtPasswordPresent = FALSE;
    }

    if (ARGUMENT_PRESENT(LmOwfPassword)) {
        Status = NlpComputeSaltedHashedPassword(&pwd->SecretPasswords.LmOwfPassword, LmOwfPassword, UserName);
        if (!NT_SUCCESS(Status)) {
            goto Cleanup;
        }
        ...
        RtlZeroMemory((PVOID)&pwd->SecretPasswords.LmOwfPassword, sizeof(pwd->SecretPasswords.LmOwfPassword));
    ...
    --*/
{
    ...
    // Windows2000:
    // never store LMOWF -- since we never need it, and, this would be the first thing attacked once a cache entry is unwrapped.
...
NTSTATUS NlpReadCacheEntry(IN PUNICODE_STRING DomainName,
    ...
    Arguments:
        ...
        UserName - The name of the account whose password is to be changed. This can be the Sam Account Name.
    ...
    --*/
{
    ...
    while (Next != (PNLP_CTE)&NlpActiveCtes) {
        NtStatus = NlpReadCacheEntryByIndex(Next->Index, CacheEntry, EntrySize);
        if (!NT_SUCCESS(NtStatus)) {
        ...
        if (!NT_SUCCESS(NtStatus)) {
            ...
            // the reason for this is because the decrypt does an integrity check.  
            // We don't want one corrupt cache entry to cause (possibly) the whole cache to be invalidated.
            FreeToHeap((*CacheEntry));
        ...
        CachedDomain.Length = CachedDomain.MaximumLength = (*CacheEntry)->DomainNameLength;
        CachedDomain.Buffer = (PWSTR)((LPBYTE)CachedUser.Buffer + ROUND_UP_COUNT((*CacheEntry)->UserNameLength, sizeof(ULONG)));
        CachedDnsDomain.Length = CachedDnsDomain.MaximumLength = (*CacheEntry)->DnsDomainNameLength;
        CachedDnsDomain.Buffer = (PWSTR)((LPBYTE)CachedDomain.Buffer + ROUND_UP_COUNT((*CacheEntry)->DomainNameLength, sizeof(ULONG)));
        CachedUpn.Length = CachedUpn.MaximumLength = (*CacheEntry)->UpnLength;
        CachedUpn.Buffer = (PWSTR)((LPBYTE)CachedDnsDomain.Buffer + ROUND_UP_COUNT((*CacheEntry)->DnsDomainNameLength, sizeof(ULONG)));

        // If the caller passed in a domain name, the user name is the SamAccountName, and the domain name is either the Netbios or Dns Domain Name.
        if (DomainName->Length != 0) {
            if (RtlEqualUnicodeString(UserName, &CachedUser, (BOOLEAN)TRUE)) {
                if (RtlEqualDomainName(DomainName, &CachedDomain) || RtlEqualUnicodeString(DomainName, &CachedDnsDomain, (BOOLEAN)TRUE)) {
                    // found it !
                    break; // out of while-loop
                }
            }

            // If no domain name was passed in, the user name is the UPN.
        } else {
        ...
        // Not the right entry, free the registry structure and go on to the next one.
        FreeToHeap((*CacheEntry));
        Next = (PNLP_CTE)(Next->Link.Flink);
    }
...
Return Value:
    NTSTATUS
        Success = STATUS_SUCCESS
                    The LOGON_CACHE_ENTRY is now in the registry (hopefully on disk)
...
Return Value:
    None.
    if string was copied, *Source and *Dest are updated to point to the next naturally aligned (DWORD) positions in the input and output buffers resp.
--*/
{
...
/*++
Routine Description:
    Performs the reverse of NlpBuildCacheEntry - creates a NETLOGON_VALIDATION_SAM_INFO2 structure from a cache entry
Arguments:
...
--*/
{
    ...
    // will GetComputerName ever fail??? Its only used to fake the logon server name when we logon using the cached information, so its
    ...
    // commonBits is the size of the variable data area common to both the LOGON_CACHE_ENTRY and NETLOGON_VALIDATION_SAM_INFO2 structures
    ...
    // length is the required amount of buffer in which to build a working NETLOGON_VALIDATION_SAM_INFO2 structure
    length = ROUND_UP_COUNT(sizeof(NETLOGON_VALIDATION_SAM_INFO2), sizeof(ULONG)) + commonBits + sidLength + computerNameLength * sizeof(WCHAR);
    ...
    // point source at the first string to be copied out of the variable length data area at the end of the cache entry
    ASSERT(pCacheEntry->Revision >= NLP_CACHE_REVISION_NT_1_0B);
    ...
    // point dest at the first (aligned) byte at the start of the variable data area at the end of the sam info structure
    dest = (PUCHAR)(pSamInfo + 1);

    // pull out the variable length data from the end of the LOGON_CACHE_ENTRY and stick them at the end of the NETLOGON_VALIDATION_SAM_INFO2 structure.
    // These must be copied out IN THE SAME ORDER as NlpBuildCacheEntry put them in.
    // If we want to change the order of things in the buffer, the order must be changed in both routines (this & NlpBuildCacheEntry)

    // create the UNICODE_STRING structures in the NETLOGON_VALIDATION_SAM_INFO2 structure and copy the strings to the end of the buffer.
    // 0 length strings will get a pointer which should be ignored
    NlpCopyAndUpdateAccountInfo(pCacheEntry->EffectiveNameLength, &pSamInfo->EffectiveName, &source, &dest);
    ...
    // final UNICODE_STRING. This one from stack. Note that we have finished with source
    source = (PUCHAR)computerName;
    NlpCopyAndUpdateAccountInfo((USHORT)(computerNameLength * sizeof(WCHAR)), &pSamInfo->LogonServer, &source, &dest);
...
/*++
Routine Description:
    This function retrieves the cache control information from the registry.
    This information is placed in global data for use throughout this module.
    The Cache Table Entry table will also be initialized.
...
--*/
{
    ...
    ULONG RequiredSize;
    PKEY_VALUE_PARTIAL_INFORMATION RegInfo = NULL;

    // read the current control info, if it is there.
    // If it is not there, then we may be dealing with a down-level system and might have a single cache entry in the registry.
    RtlInitUnicodeString(&CacheControlValueName, L"NL$Control");
    NtStatus = NtQueryValueKey(NlpCacheHandle, &CacheControlValueName, KeyValuePartialInformation, NULL, 0, &RequiredSize);
    if (NT_SUCCESS(NtStatus) || NtStatus == STATUS_OBJECT_NAME_NOT_FOUND) {
        NTSTATUS TempStatus;

        // Hmmm - no entry, that means we are dealing with a first release system here (that didn't have this value).
        ...
        NtStatus = NtQueryValueKey(NlpCacheHandle, &CacheControlValueName, KeyValuePartialInformation, (PVOID)RegInfo, RequiredSize, &RequiredSize);
        if (!NT_SUCCESS(NtStatus)) {
            goto Cleanup;
        }
        ...
        if (NlpCacheControl.Revision != NLP_CACHE_REVISION) {
            // There is no conversion. All the version of cache control have been the same.
            NlpCacheControl.Revision = NLP_CACHE_REVISION;
...
--*/
{
    ...
    NlpCteTable = AllocateFromHeap(sizeof(NLP_CTE) * NlpCacheControl.Entries);// Allocate a CTE table
    if (NlpCteTable == NULL) {// Can't allocate table, disable caching
        NlpCacheControl.Entries = 0;    // Disable cache
        return(STATUS_NO_MEMORY);
    }

    for (i = 0; i < NlpCacheControl.Entries; i++) {
        NtStatus = NlpReadCacheEntryByIndex(i, &CacheEntry, &EntrySize);
        if (!NT_SUCCESS(NtStatus)) {
            NlpCacheControl.Entries = 0;    // Disable cache
            return(NtStatus);
        }
...
Routine Description:
    ...
    If this succeeds, then the global cache control information is updated appropriately.
    If it fails then one of two things will happen:
...
--*/
{
    ...
    ULONG ErrorCacheSize, EntrySize, i, j;

    // Find out how many logons to cache.
    // This is a user setable value and it may be different than the last time we booted.
    CachedLogonsCount = GetProfileInt(TEXT("Winlogon"), TEXT("CachedLogonsCount"), NLP_DEFAULT_LOGON_CACHE_COUNT      // Default value
    );

    // Minimize the user-supplied value with the maximum allowable value.
    if (CachedLogonsCount > NLP_MAX_LOGON_CACHE_COUNT) {
        CachedLogonsCount = NLP_MAX_LOGON_CACHE_COUNT;
    }

    // Compare it to what we already have and see if we need to change the size of the cache
    if (CachedLogonsCount == NlpCacheControl.Entries) {
        // No change
        return(STATUS_SUCCESS);
    }

    // Set the size of the cache to be used in case of error changing the size.
    // If we are trying to grow the cache, then use the existing cache on error.
    // If we are trying to shrink the cache, then disable caching on error.
    if (CachedLogonsCount > NlpCacheControl.Entries) {
    ...
    NewCteTable = AllocateFromHeap(sizeof(NLP_CTE) * CachedLogonsCount);
    if (NewCteTable == NULL) {
    ...
    if (CachedLogonsCount > NlpCacheControl.Entries) {
        ...
        for (Next = (PNLP_CTE)NlpActiveCtes.Flink; Next != (PNLP_CTE)(&NlpActiveCtes); Next = (PNLP_CTE)Next->Link.Flink) {
            InsertTailList(&NewActive, &NewCteTable[Next->Index].Link);
            NewCteTable[Next->Index].Active = TRUE;
        }

        InitializeListHead(&NewInactive);
        for (Next = (PNLP_CTE)NlpInactiveCtes.Flink; Next != (PNLP_CTE)(&NlpInactiveCtes); Next = (PNLP_CTE)Next->Link.Flink) {
        ...
        if (CachedLogonsCount != 0) {
            ...
            // That is a degenerate case of shrinking that requires only the last few steps of shrinking.

            // Allocate an array of pointers for reading registry and secret info into.  Clear it to assist in cleanup.
            CacheAndSecrets = (PNLP_CACHE_AND_SECRETS)AllocateFromHeap(sizeof(NLP_CACHE_AND_SECRETS) * CachedLogonsCount);
            if (CacheAndSecrets == NULL) {
                FreeToHeap(NlpCteTable);
                NlpCacheControl.Entries = ErrorCacheSize;
                return(STATUS_NO_MEMORY);
            }
            RtlZeroMemory(CacheAndSecrets, (sizeof(NLP_CACHE_AND_SECRETS) * CachedLogonsCount));
            ...
            while (Next != (PNLP_CTE)&NlpActiveCtes && i < CachedLogonsCount) {
                NtStatus = NlpReadCacheEntryByIndex(Next->Index, &CacheAndSecrets[i].CacheEntry, &CacheAndSecrets[i].EntrySize
                                                    // &EntrySize
                );
                if (NT_SUCCESS(NtStatus)) {
                    ...
                    if (CacheAndSecrets[i].CacheEntry->Revision < NLP_CACHE_REVISION_NT_5_0) {
                        NtStatus = NlpOpenSecret(Next->Index);
                        if (NT_SUCCESS(NtStatus)) {
                            NtStatus = NlpReadSecret(&CacheAndSecrets[i].NewSecret, &CacheAndSecrets[i].OldSecret);
                            NlpCloseSecret();
                        }
                    }

                    if (NT_SUCCESS(NtStatus)) {
                        // Only make this entry active if everything was successfully read in.
            ...
            // At this point "i" indicates how many CacheAndSecrets entries are active.
            // Furthermore, the entries were assembled in the CacheAndSecrets array in ascending time order,
            // which is the order they need to be placed in the new CTE table.
            for (j = 0; j < i; j++) {
                Next = &NewCteTable[j];

                // The Time field in the original cache entry is not aligned properly, so copy each field individually.
                ...
                NtStatus = NlpWriteCacheEntry(j, CacheAndSecrets[j].CacheEntry, CacheAndSecrets[j].EntrySize);
                if (NT_SUCCESS(NtStatus)) {
                    if (CacheAndSecrets[j].CacheEntry->Revision < NLP_CACHE_REVISION_NT_5_0) {
                        ...
                        // note: we don't bother to try to migrate pre-win2000 -> Win2000 here, because this will happen later,
                        //       as a side-effect of updating cache entry during successful DC validated logon.
                        NtStatus = NlpOpenSecret(j);
                        if (NT_SUCCESS(NtStatus)) {
                            NtStatus = NlpWriteSecret(CacheAndSecrets[j].NewSecret, CacheAndSecrets[j].OldSecret);
                        }
                    }

                    if (NT_SUCCESS(NtStatus)) {
                        // move the corresponding entry into the new CTEs active list.
                        Next->Active = TRUE;
    ...
    //      Filled the CTE table with copies of the currently active CTEs (including putting each CTE on an active or inactive list).

    //      Established new CTE entries, including the corresponding secrets and cache keys in the registry, for the new CTEs.

    // All we have left to do is:

    //      Update the cache control structure in the registry to indicate we have a new length

    //      move the new CTE over to the real Active and Inactive list heads (rather than the local ones we've used so far)

    //      deallocate the old CTE table.

    //      Re-set the entries count in the in-memory cache-control structure NlpCacheControl.
...
Routine Description:
    ...
    Note:
        When lengthening the cache, call this routine after the cache entries and corresponding secrets have been established for the new length.
...
/*++
Routine Description:
    This function builds a name of a cache entry value or secret name for a cached entry.
    The name is based upon the index of the cache entry.
...
--*/
{
    ...
    NtStatus = I_LsarOpenSecret(NlpLsaHandle, (PLSAPR_UNICODE_STRING)&ValueName, DELETE, &SecretHandle);
    if (NT_SUCCESS(NtStatus)) {
        // for Windows2000, we remove old style cache entry related LSA secrets.

        I_LsarDelete(SecretHandle);// Deleting and object causes its handle to be closed
...
Arguments:
    Index - The index of the entry whose value and secret are to be deleted.
            This value is used only to build a name with (not to reference the CTE table).
--*/
{
    NTSTATUS NtStatus;
    UNICODE_STRING ValueName;
    ...
    NtStatus = I_LsarOpenSecret(NlpLsaHandle, (PLSAPR_UNICODE_STRING)&ValueName, DELETE, &SecretHandle);
    if (NT_SUCCESS(NtStatus)) {
        NtStatus = I_LsarDelete(SecretHandle);// Deleting and object causes its handle to be closed
        NtStatus = NtDeleteValueKey(NlpCacheHandle, &ValueName);// Now delete the registry value
    }
...
/*++
Routine Description:
    This function retrieves the cache entry used in NT1.0 systems and stores it (if found) in the zero'th CTE entry.
    It also copies the secrets from 1.0 storage format to 1.0B format.
Return Value:
    STATUS_SUCCESS - if the entry was successfully upgraded, or if it didn't exist.
    STATUS_NO_MEMORY - if we couldn't allocate memory from heap.
    other  - unexpected error.
--*/
{
    NTSTATUS NtStatus;
    PKEY_VALUE_FULL_INFORMATION RegistryStructure;
    PLOGON_CACHE_ENTRY_1_0 CacheEntry;
    PLOGON_CACHE_ENTRY NewCacheEntry;
    UNICODE_STRING NullName;
    PLSAPR_CR_CIPHER_VALUE CurrentSecret = NULL, OldSecret = NULL;
    ULONG RequiredSize, VariableSize, EntrySize, NewSize;
    PUCHAR Source, Dest;

    // This should always try to return at least the KEY_VALUE_FULL_INFORMATION structure, even if there isn't data available.
    RtlInitUnicodeString(&NullName, NULL);
    NtStatus = NtQueryValueKey(NlpCacheHandle, &NullName, KeyValueFullInformation, NULL, 0, &RequiredSize);
    ASSERT(!NT_SUCCESS(NtStatus));
    ...
    if (RegistryStructure == NULL) {
        ...
        NtStatus = NtQueryValueKey(NlpCacheHandle, &NullName, KeyValueFullInformation, RegistryStructure, RequiredSize, &RequiredSize);
        if (NT_SUCCESS(NtStatus)) {
            // If we didn't get any data in the query, then there wasn't a cache entry, don't do anything.
            // Otherwise, copy it to the new scheme.
            if (RequiredSize > sizeof(KEY_VALUE_FULL_INFORMATION)) {
                // OK, we now have a NT1_0 cache entry.
                // This is the same as a NT1_0B cache entry, except that the fields from SidCount onward are new to NT1_0B and so aren't present in the structure we just read in.

                // Now the challange is to build a new Registry Structure that looks just like the one we just read in, but adds in these new fields.

                // Warning - the fields of CacheEntry are not necessarily aligned nicely because the structure starts at a random offset inside a registry header.
                //           Avoid referencing CacheEntry fields.
                CacheEntry = (PLOGON_CACHE_ENTRY_1_0)((PCHAR)(RegistryStructure)+(RegistryStructure->DataOffset));
                EntrySize = RegistryStructure->DataLength;
                Source = (PUCHAR)(((PLOGON_CACHE_ENTRY_1_0)CacheEntry) + 1);
                VariableSize = EntrySize - ROUND_UP_COUNT(sizeof(LOGON_CACHE_ENTRY_1_0), sizeof(ULONG));
                NewSize = ROUND_UP_COUNT(sizeof(LOGON_CACHE_ENTRY), sizeof(ULONG)) + VariableSize;
                ...
                if (NewCacheEntry == NULL) {
                    ...
                    // Copy the fixed-length aspects of the original CacheEntry into the new cache entry.
                    RtlMoveMemory(NewCacheEntry, CacheEntry, sizeof(LOGON_CACHE_ENTRY_1_0));
                    ...
                    // put out the secrets first so that if it fails we haven't already validated the cache entry.
                    ...
                    if (NT_SUCCESS(NtStatus)) {
                        ...
                        if (NT_SUCCESS(NtStatus)) {
                            ...
                            if (NT_SUCCESS(NtStatus)) {
                                ...
                                if (NT_SUCCESS(NtStatus)) {
                                    NtStatus = NlpWriteCacheEntry(0, NewCacheEntry, NewSize);
                                    // The CTE table isn't built yet, so don't try to update this entry in the CTE.
                                }
                            }
...
Routine Description:
    ...
    This is used to upgrade the system.  The secret is opened for query access.

    If the secret does not exist, it is NOT created.
Return Value:
    STATUS_SUCCESS - The secret was successfully openned.
    STATUS_OBJECT_NAME_NOT_FOUND - The secret didn't exist.  The handle value is not valid.
...
--*/
{
    ...
    NtStatus = I_LsarOpenSecret(NlpLsaHandle, (PLSAPR_UNICODE_STRING)&SecretName, SECRET_QUERY_VALUE, &NlpSecretHandle);
    return(NtStatus);
}
        ...
        if (RegInfo->DataLength == 0) {
            ...
            *EntrySize = RegInfo->DataLength;
            ...
            if ((*CacheEntry) == NULL) {
                ...
                RtlCopyMemory((*CacheEntry), RCacheEntry, (*EntrySize));
            }
        }
    }
...
Routine Description:
    ...
    This requires placing the entry in the right location in the list chronologically.
    The beginning of the list is the most recently updated (or referenced) cache entry.
    The end of the list is the oldest active cache entry.

    Note - The entry may be already in the active list (but in the wrong place), or may be on the inactive list.
...
--*/
{
    ...
    // Remove the entry from its current list, and then place it in the active list.
    RemoveEntryList(&NlpCteTable[Index].Link);

    // Now walk the active list until we find a place to insert the entry.  It must follow all entries with more recent time stamps.
    Next = (PNLP_CTE)NlpActiveCtes.Flink;
    while (Next != (PNLP_CTE)&NlpActiveCtes) {
...
--*/
{
    // Remove the entry from its current list, and then place it in the inactive list.
    RemoveEntryList(&NlpCteTable[Index].Link);
...
Routine Description:
    ...
    The entry is left on the list it is already on.
    If it is used by the caller, then the caller must ensure it is re-assigned to the active list (using NlpAddEntryToActiveList()).
