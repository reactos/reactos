...
Abstract:
    ...
    These routines are shared by the MSV authentication package and the Netlogon service.
...
NTSTATUS Msv1_0SubAuthenticationRoutineZero(
...
BOOLEAN MsvpLm3ValidateResponse (IN PNT_OWF_PASSWORD pNtOwfPassword, IN PUNICODE_STRING pUserName, IN PUNICODE_STRING pLogonDomainName, IN UCHAR ChallengeToClient[MSV1_0_CHALLENGE_LENGTH], IN PMSV1_0_LM3_RESPONSE pLm3Response)
{
    UCHAR Response[MSV1_0_NTLM3_RESPONSE_LENGTH];
    ULONG i;

    MsvpLm3Response (pNtOwfPassword, pUserName, pLogonDomainName, ChallengeToClient, pLm3Response, Response);// compute the response again
    i = (ULONG)RtlCompareMemory(pLm3Response->Response, Response, MSV1_0_NTLM3_RESPONSE_LENGTH);// compare with what we were passed

    return (i == MSV1_0_NTLM3_RESPONSE_LENGTH);
}


BOOLEAN MsvpNtlm3ValidateResponse (
    ...
    )
{
    ...
    if (pNtlm3Response->RespType > 1 || pNtlm3Response->HiRespType < 1)
        return FALSE;
    ...
    if (Time.QuadPart > (LONGLONG)pNtlm3Response->TimeStamp) {
        if (Time.QuadPart - (LONGLONG)pNtlm3Response->TimeStamp > (MSV1_0_MAX_NTLM3_LIFE*TicksPerSecond))
            return FALSE;
    } else if ((LONGLONG)pNtlm3Response->TimeStamp - Time.QuadPart > (MSV1_0_MAX_NTLM3_LIFE*TicksPerSecond)) {
        return FALSE;
    }
#endif
    ...
    i = (ULONG)RtlCompareMemory(pNtlm3Response->Response, Response, (size_t)MSV1_0_NTLM3_RESPONSE_LENGTH);

    return (i == MSV1_0_NTLM3_RESPONSE_LENGTH);
}


BOOLEAN MsvpPasswordValidate (
...
/*++
Routine Description:
    Process an interactive, network, or session logon.
    It calls SamIUserValidation, validates the passed in credentials, updates the logon statistics and packages the result for return to the caller.

    This routine is called directly from the MSV Authentication package on any system where LanMan is not installed.
    This routine is called from the Netlogon Service otherwise.
...
Arguments:
    ...
    LogonInformation -- Specifies the description for the user logging on.
        The LogonDomainName field should be ignored.
    ...
    UserSessionKey -- Returns the NT User session key for this network logon session.

    LmSessionKey -- Returns the LM compatible session key for this network logon session.
...
--*/
{
    ...
    *UserFlags = LOGON_NTLMV2_ENABLED;
    ...
    if ( !Passwords->NtPasswordPresent ){
        RtlCopyMemory( &Passwords->NtOwfPassword, &NlpNullNtOwfPassword, sizeof(Passwords->NtOwfPassword) );
    }

    if ( !Passwords->LmPasswordPresent ){
        RtlCopyMemory( &Passwords->LmOwfPassword, &NlpNullLmOwfPassword, sizeof(Passwords->LmOwfPassword) );
    }

    // Handle interactive/service validation.

    // Simply compare the OWF password passed in with the one from the SAM database.
    switch ( LogonLevel )
    {
    case NetlogonInteractiveInformation:
    case NetlogonServiceInformation:
        ASSERT( offsetof( NETLOGON_INTERACTIVE_INFO, LmOwfPassword)  ==  offsetof( NETLOGON_SERVICE_INFO, LmOwfPassword) );
        ASSERT( offsetof( NETLOGON_INTERACTIVE_INFO, NtOwfPassword)  ==  offsetof( NETLOGON_SERVICE_INFO, NtOwfPassword) );

        LogonInteractiveInfo = (PNETLOGON_INTERACTIVE_INFO) LogonInformation;
        ...
        if ( UasCompatibilityRequired && !Passwords->NtPasswordPresent && Passwords->LmPasswordPresent ) {
            if ( RtlCompareMemory( &Passwords->LmOwfPassword, &LogonInteractiveInfo->LmOwfPassword, LM_OWF_PASSWORD_LENGTH ) != LM_OWF_PASSWORD_LENGTH ) {
                return FALSE;
            }
        ...
        //  This enforces case sensitivity.
            ...
            if ( RtlCompareMemory( &Passwords->NtOwfPassword, &LogonInteractiveInfo->NtOwfPassword, NT_OWF_PASSWORD_LENGTH ) != NT_OWF_PASSWORD_LENGTH ) {
                return FALSE;
            }
        }
    ...
    case NetlogonNetworkInformation:
        // First, assume the passed password information is a challenge response.
        ...
        if (LogonNetworkInfo->NtChallengeResponse.Length > NT_RESPONSE_LENGTH) {
            ...
            // because a Subauth may have been used, we will only return failure here if we know the request was NTLMv2.
            if( AlreadyValidated || (LogonNetworkInfo->Identity.ParameterControl & MSV1_0_USE_CLIENT_CHALLENGE) ) {
        ...
        if (!AlreadyValidated &&
            ...
            (LogonNetworkInfo->NtChallengeResponse.Length == NT_RESPONSE_LENGTH || LogonNetworkInfo->LmChallengeResponse.Length == NT_RESPONSE_LENGTH))
        {
            NT_RESPONSE NtResponse;

            // Compute what the response should be.
            Status = RtlCalculateNtResponse(&LogonNetworkInfo->LmChallenge, &Passwords->NtOwfPassword, &NtResponse );
            if ( NT_SUCCESS(Status) ) {
                // If the responses match, the passwords are valid.
                //  Try the NT response first, then the LM response
                if ( RtlCompareMemory(LogonNetworkInfo->NtChallengeResponse.Buffer, &NtResponse, LogonNetworkInfo->NtChallengeResponse.Length ) == NT_RESPONSE_LENGTH ) {
                    AlreadyValidated = TRUE;
                } else if ( RtlCompareMemory(LogonNetworkInfo->LmChallengeResponse.Buffer, &NtResponse, LogonNetworkInfo->LmChallengeResponse.Length ) == NT_RESPONSE_LENGTH ) {
                    AlreadyValidated = TRUE;
                }
            }
        }
            ...
            )
        {
            ...
            // Compute what the response should be.
            Status = RtlCalculateLmResponse(&LogonNetworkInfo->LmChallenge, &Passwords->LmOwfPassword, &LmResponse );
            if ( NT_SUCCESS(Status) ) {
                // If the responses match, the passwords are valid.
                if ( RtlCompareMemory(LogonNetworkInfo->LmChallengeResponse.Buffer, &LmResponse, LM_RESPONSE_LENGTH ) == LM_RESPONSE_LENGTH ) {
                    AlreadyValidated = TRUE;
                    *UserFlags |= LOGON_USED_LM_PASSWORD;
                }
            }
        ...
        if ( !AlreadyValidated ) {
            ...
            // Compute the OWF password for the specified Cleartext password and compare that to the OWF password retrieved from SAM.


            // If we're in UasCompatibilityMode, and we don't have the NT password in SAM or we don't have the NT password supplied by the caller.
            //  validate against the LM version of the password.
            ...
            if ( UasCompatibilityRequired &&
                 ((Passwords->LmPasswordPresent) || (!Passwords->LmPasswordPresent && !Passwords->NtPasswordPresent)) &&
                 (!Passwords->NtPasswordPresent || LogonNetworkInfo->NtChallengeResponse.Length == 0 ) )
            {
                ...
                for (i = 0; i < LogonNetworkInfo->LmChallengeResponse.Length; i++) {
                    LmPassword[i] = RtlUpperChar(LogonNetworkInfo->LmChallengeResponse.Buffer[i]);
                }

                (VOID) RtlCalculateLmOwfPassword( LmPassword, &LmOwfPassword );

                if ( RtlCompareMemory( &Passwords->LmOwfPassword, &LmOwfPassword, LM_OWF_PASSWORD_LENGTH ) != LM_OWF_PASSWORD_LENGTH ) {
                    // Try the case preserved clear text password, too.
                    //  (I know of no client that does this, but it is compatible with the LM 2.x server.)
                    RtlZeroMemory( &LmPassword, sizeof(LmPassword) );
                    RtlCopyMemory(&LmPassword, LogonNetworkInfo->LmChallengeResponse.Buffer, LogonNetworkInfo->LmChallengeResponse.Length);
                    (VOID) RtlCalculateLmOwfPassword( LmPassword, &LmOwfPassword );
                    if ( RtlCompareMemory( &Passwords->LmOwfPassword, &LmOwfPassword, LM_OWF_PASSWORD_LENGTH ) != LM_OWF_PASSWORD_LENGTH ) {
                        return FALSE;
                    }
            ...
            //  This enforces case sensitivity.
                ...
                Status = RtlCalculateNtOwfPassword( (PUNICODE_STRING) &LogonNetworkInfo->NtChallengeResponse, &NtOwfPassword );
                if ( RtlCompareMemory( &Passwords->NtOwfPassword, &NtOwfPassword, NT_OWF_PASSWORD_LENGTH ) != NT_OWF_PASSWORD_LENGTH ) {
                    return FALSE;
                }
            }
        ...
        if ( LogonNetworkInfo->NtChallengeResponse.Length == 0 ) {
            ...
            RtlCopyMemory( UserSessionKey, &Passwords->LmOwfPassword, sizeof(*LmSessionKey) );
        } else {
        ...
        if ( Passwords->LmPasswordPresent || !Passwords->NtPasswordPresent ) {
            RtlCopyMemory( LmSessionKey, &Passwords->LmOwfPassword, sizeof(*LmSessionKey) );
        }
...
BOOLEAN MsvpEqualSidPrefix(IN PSID DomainSid, IN PSID GroupSid)
/*++
Routine Description:
    This routine checks to see if the specified group sid came from the specified domain by verifying that the domain portion of the group sid is equal to the domain sid.
Arguments:
...
--*/
{
    ...
    if ((LocalGroupSid->SubAuthorityCount == LocalDomainSid->SubAuthorityCount + 1) &&
        RtlEqualMemory(RtlIdentifierAuthoritySid(LocalDomainSid), RtlIdentifierAuthoritySid(LocalGroupSid), RtlLengthRequiredSid(LocalDomainSid->SubAuthorityCount) - FIELD_OFFSET(SID,IdentifierAuthority)))
    {
        return(TRUE);
    }
...
NTSTATUS MsvpFilterGroupMembership(
...
/*++
Routine Description:
    This routine separates the complete transitive group membership into portions from this domain and portions from others.
Arguments:
    CompleteMembership - The complete transitive membership.
    LogonDomainId - SID of the logon domain, used for compressing group membership.
    LocalMembership - Receives a list of rids corresponding to groups in this domain. The list should be freed with MIDL_user_free.
    GlobalMembership - Recevies a list of sids corresponding to groups in other domain. The list, but not the sids, should be free with MIDL_user_free.
    GlobalMembershipSize - Size, in bytes, of the sids in the global membership and the size of the SID_AND_ATTRIBUTES structures.
Returns:
...
#define MSVP_LOCAL_GROUP_ATTR 0x20000000
    ...
    for (Index = 0; Index < CompleteMembership->Count ; Index++ ) {
        ...
        if (MsvpEqualSidPrefix(LogonDomainId, CompleteMembership->SidAndAttributes[Index].Sid)) {
    ...
    for (Index = 0; Index < CompleteMembership->Count ; Index++ ) {
        if ((CompleteMembership->SidAndAttributes[Index].Attributes & MSVP_LOCAL_GROUP_ATTR) != 0) {
            ...
            LocalMembership->Groups[LocalCount].RelativeId = *RtlSubAuthoritySid(CompleteMembership->SidAndAttributes[Index].Sid, *RtlSubAuthorityCountSid(CompleteMembership->SidAndAttributes[Index].Sid) - 1);
            LocalCount++;
        } else {
...
Arguments:
    DomainHandle -- Specifies a handle to the SamDomain to use to validate the request.
    ...
    GuestRelativeId - If non-zero, specifies the relative ID of the account to validate against.

    ValidationLevel -- Specifies the level of information returned in ValidationInformation.
        Must be NetlogonValidationSamInfo or NetlogonValidationSamInfo2.

    ValidationInformation -- Returns the requested validation information.
        This buffer must be freed user MIDL_user_free.
    ...
    BadPasswordCountZeroed - Returns TRUE iff we zeroed the BadPasswordCount field of this user.
...
--*/
{
    ...
    if( LogonInfo->ParameterControl & MSV1_0_TRY_SPECIFIED_DOMAIN_ONLY && LogonInfo->LogonDomainName.Length ) {

        // common case is a match for LogonDomainName, so avoid taking locks until mis-match occurs.
        if(!RtlEqualDomainName( &LogonInfo->LogonDomainName, LogonDomainName )) {
            ...
            // pickup the local target name, based on whether this computer is a domain controller.
            EnterCriticalSection(&NtLmGlobalCritSect);
            if( NlpWorkstation ) {
                if( (NtLmGlobalUnicodeDnsComputerNameString.Length + sizeof(WCHAR)) <= sizeof( LocalTarget ) ) {
                    RtlCopyMemory(LocalTarget, NtLmGlobalUnicodeDnsComputerName, NtLmGlobalUnicodeDnsComputerNameString.Length);
                    cchLocalTarget = (NtLmGlobalUnicodeDnsComputerNameString.Length) / sizeof(WCHAR);
                }
            } else {
                if( (NtLmGlobalUnicodeDnsDomainNameString.Length + sizeof(WCHAR)) <= sizeof( LocalTarget ) ) {
                    RtlCopyMemory(LocalTarget, NtLmGlobalUnicodeDnsDomainName, NtLmGlobalUnicodeDnsDomainNameString.Length);
                    cchLocalTarget = (NtLmGlobalUnicodeDnsDomainNameString.Length) / sizeof(WCHAR);
                }
            }
            ...
            if( (LogonInfo->LogonDomainName.Length + sizeof(WCHAR)) <= sizeof( SpecifiedTarget ) ) {
                ...
                RtlCopyMemory(SpecifiedTarget, LogonInfo->LogonDomainName.Buffer, LogonInfo->LogonDomainName.Length);
            }
    ...
    // Initialization.
    RtlZeroMemory(&SubAuthValidationInformation, sizeof(MSV1_0_VALIDATION_INFO));
    ...
    *Authoritative = TRUE;
    ...
    case NetlogonNetworkInformation:
        ...
        UserAccountControl |= USER_INTERDOMAIN_TRUST_ACCOUNT | USER_WORKSTATION_TRUST_ACCOUNT | USER_SERVER_TRUST_ACCOUNT;
        break;
    default:
        *Authoritative = TRUE;
    ...
    Status = I_SamIGetUserLogonInformation(DomainHandle, SamFlags, &LocalUserName, &UserAllInfo, &GroupMembership, &UserHandle);
    if ( !NT_SUCCESS(Status) ) {
        UserHandle = NULL;
        *Authoritative = FALSE;
    ...
    // SubAuthentication packages can be more specific than this test but they can't become less restrictive.
    ...
    // determine if machine account, if so, certain failures are treated as Authoritative, to prevent fallback to guest and returning incorrect error codes.
    ...
    // If there is a SubAuthentication DLL, call it to do all the authentication work.
    if ( (LogonInfo->ParameterControl & MSV1_0_SUBAUTHENTICATION_DLL) && (!(LogonInfo->ParameterControl & MSV1_0_SUBAUTHENTICATION_DLL_EX))) {
        ...
        // But, for those who call with old SubAuth packages, they will expect us to do the dirty work.
        ...
        if ( (LogonInfo->ParameterControl & MSV1_0_SUBAUTHENTICATION_DLL_EX))
        {
            ...
            Status = Msv1_0SubAuthenticationRoutineEx(LogonLevel,LogonInformation,Flags,(PUSER_ALL_INFORMATION) UserAll,(SAM_HANDLE)UserHandle,&SubAuthValidationInformation,&ActionsPerformed );
            *Authoritative = SubAuthValidationInformation.Authoritative;
            if ( !NT_SUCCESS(Status) ) {
                goto Cleanup;
            }

            // We need to do this because even if any of the following checks fail, ARAP stills wants the returned blobs from teh subauth package to be returned to the caller.
            fSubAuthEx = TRUE;
        }
        ...
        // Check the password if there's no subauth or if subauth did not already check password.
        if ((ActionsPerformed & MSV1_0_SUBAUTH_PASSWORD) == 0)
        {
            if ( SecureChannelType != NullSecureChannel ) {
                ...
                if ( UserAll->NtPasswordPresent ) {
                    Passwords.NtOwfPassword = *((PNT_OWF_PASSWORD)(UserAll->NtOwfPassword.Buffer));
                }

                Passwords.LmPasswordPresent = UserAll->LmPasswordPresent;
                if ( UserAll->LmPasswordPresent ) {
                    Passwords.LmOwfPassword = *((PLM_OWF_PASSWORD)(UserAll->LmOwfPassword.Buffer));
                }
                ...
                if ( ! MsvpPasswordValidate (UasCompatibilityRequired,LogonLevel,LogonInformation,&Passwords,&UserFlags,&UserSessionKey,&LmSessionKey ) ) {
                    ...
                    // This special case check is after the MsvpPasswordValidate to give MsvpPasswordValidate every opportunity to compute the correct values for UserSessionKey and LmSessionKey.

                    if ( GuestRelativeId != 0 && !UserAll->NtPasswordPresent && !UserAll->LmPasswordPresent ) {
                        RtlZeroMemory( &UserSessionKey, sizeof(UserSessionKey) );
                        RtlZeroMemory( &LmSessionKey, sizeof(LmSessionKey) );
        ...
        if (RelativeId != DOMAIN_USER_RID_ADMIN) {
            // Check if the account is disabled if there's no subauth or if subauth has not already checked.
    ...
    // But, for NT 5.0, we must allow accounts with account control USER_WORKSTATION_TRUST_ACCOUNT for remote boot clients who will logon with their machine accounts
    if ( (UserAll->UserAccountControl & USER_MACHINE_ACCOUNT_MASK) != 0 ) {
        if (UserAll->UserAccountControl & USER_INTERDOMAIN_TRUST_ACCOUNT) {
            ...
            if ( (LogonInfo->ParameterControl & MSV1_0_ALLOW_WORKSTATION_TRUST_ACCOUNT) == 0) {
                ...
                // if the password was correct, and it happened to match the machine name, dis-allow it regardless.

                // Compute the lower case user name.
                TempStatus = RtlDowncaseUnicodeString( &MachineAccountName, (PUNICODE_STRING)&UserAll->UserName, TRUE );
                if( NT_SUCCESS( TempStatus ) )
                {
                    ...
                    if ( UserAll->NtPasswordPresent ) {
                        ...
                        RtlCalculateNtOwfPassword( &MachineAccountName, &NtOwfMachineName );
                        if ( RtlCompareMemory( pOwf, &NtOwfMachineName, NT_OWF_PASSWORD_LENGTH ) == NT_OWF_PASSWORD_LENGTH ) {
                            Status = STATUS_NOLOGON_WORKSTATION_TRUST_ACCOUNT;
                        }
                    }
            ...
            if ( (LogonInfo->ParameterControl & MSV1_0_ALLOW_SERVER_TRUST_ACCOUNT) == 0) {
                ...
                // this will be merged when a new Rtl has been added for computing initial machine password from machine acct name.
    ...
    // Filter the groups into global groups (from other domains) and local groups (from this domain).
    Status = MsvpFilterGroupMembership(&GroupMembership, LogonDomainId, &GroupsBuffer, &GlobalGroupMembership, &GlobalMembershipSize);
    if ( !NT_SUCCESS(Status) ) {
        *Authoritative = FALSE;
        goto Cleanup;
    }
...
Cleanup:
    if (NT_SUCCESS(Status) || fSubAuthEx)
    {
        ...
        // for network logon of the guest account, return the account name so the correct name ends up in the session list.
        if ( LogonLevel == NetlogonNetworkInformation && GuestRelativeId != 0 ) {
            ValidationSamSize += UserAll->UserName.Length + sizeof(WCHAR) ;
        }

        if ( LogonInfo->ParameterControl & MSV1_0_RETURN_USER_PARAMETERS ) {
            ValidationSamSize += UserAll->Parameters.Length + sizeof(WCHAR);
        } else if ( LogonInfo->ParameterControl & MSV1_0_RETURN_PROFILE_PATH ) {
            ValidationSamSize += UserAll->ProfilePath.Length + sizeof(WCHAR);
        }

        // If the caller can handle extra groups, let them have the groups from other domains.
        ...
        RtlCopyMemory( &ValidationSam->ExpansionRoom[SAMINFO_LM_SESSION_KEY], &LmSessionKey, SAMINFO_LM_SESSION_KEY_SIZE );
        ...
        RtlCopyMemory(Where, GroupsBuffer.Groups, GroupsBuffer.MembershipCount * sizeof(GROUP_MEMBERSHIP) );
        ValidationSam->GroupIds = (PGROUP_MEMBERSHIP) Where;
        Where += GroupsBuffer.MembershipCount * sizeof(GROUP_MEMBERSHIP);

        RtlCopyMemory(Where, LogonDomainId, RtlLengthSid( LogonDomainId ) );
        ...
        if (ValidationLevel == NetlogonValidationSamInfo2) {
            ...
            if (GlobalMembershipSize != 0) {
                ...
                for (Index = 0; Index < ValidationSam->SidCount ; Index++ ) {
                    ...
                    RtlCopyMemory(ValidationSam->ExtraSids[Index].Sid, GlobalGroupMembership.SidAndAttributes[Index].Sid, SidLength);
                    Where += SidLength;
                }
            }
        }
        ...
        if ( LogonLevel != NetlogonNetworkInformation ) {
            NlpPutString( &ValidationSam->EffectiveName, (PUNICODE_STRING)&UserAll->UserName, &Where );
            NlpPutString( &ValidationSam->FullName, (PUNICODE_STRING)&UserAll->FullName, &Where );
            NlpPutString( &ValidationSam->LogonScript, (PUNICODE_STRING)&UserAll->ScriptPath, &Where );
            NlpPutString( &ValidationSam->ProfilePath, (PUNICODE_STRING)&UserAll->ProfilePath, &Where );
            NlpPutString( &ValidationSam->HomeDirectory, (PUNICODE_STRING)&UserAll->HomeDirectory, &Where );
            NlpPutString( &ValidationSam->HomeDirectoryDrive, (PUNICODE_STRING)&UserAll->HomeDirectoryDrive, &Where );
        }
        ...
        if ((SubAuthValidationInformation.WhichFields & MSV1_0_VALIDATION_LOGON_SERVER) != 0) {
            NlpPutString( &ValidationSam->LogonServer, &SubAuthValidationInformation.LogonServer, &Where );
        } else {
            NlpPutString( &ValidationSam->LogonServer, LogonServer, &Where );
        }

        if ((SubAuthValidationInformation.WhichFields & MSV1_0_VALIDATION_LOGON_DOMAIN) != 0) {
            NlpPutString( &ValidationSam->LogonDomainName, &SubAuthValidationInformation.LogonDomainName, &Where );
        } else {
            NlpPutString( &ValidationSam->LogonDomainName, LogonDomainName, &Where );
        }

        // Kludge: Pass back UserParameters in HomeDirectoryDrive since we can't change the NETLOGON_VALIDATION_SAM_INFO2 structure between releases NT 1.0 and NT 1.0A.
        // HomeDirectoryDrive was NULL for release 1.0A
        // so we'll use that field.
        if ( LogonInfo->ParameterControl & MSV1_0_RETURN_USER_PARAMETERS ) {
            NlpPutString( &ValidationSam->HomeDirectoryDrive, (PUNICODE_STRING)&UserAll->Parameters, &Where );
        } else if ( LogonInfo->ParameterControl & MSV1_0_RETURN_PROFILE_PATH ) {
            NlpPutString( &ValidationSam->HomeDirectoryDrive, (PUNICODE_STRING)&UserAll->ProfilePath, &Where );
            ValidationSam->UserFlags |= LOGON_PROFILE_PATH_RETURNED;
        }

        *Authoritative = TRUE;

        // For SubAuthEx, we save away the original Status to make decisions later on about additional processing to perform.
        if( fSubAuthEx ) {
    ...
    // If the User Parameters have been changed, write them back to SAM.
    if ( NT_SUCCESS(Status) && (WhichFields & USER_ALL_PARAMETERS) ) {
        SAMPR_USER_INFO_BUFFER UserInfo;

        UserInfo.Parameters.Parameters = UserAll->Parameters;
        Status = I_SamrSetInformationUser(UserHandle, UserParametersInformation, &UserInfo );
    }

    // Update the logon statistics.
    if ( NT_SUCCESS( SubAuthExStatus ) && (NT_SUCCESS(Status) || Status == STATUS_WRONG_PASSWORD) ) {
        SAMPR_USER_INFO_BUFFER UserInfo;

        if ( NT_SUCCESS( Status ) ) {
            if ( LogonLevel == NetlogonInteractiveInformation ) {
                UserInfo.Internal2.StatisticsToApply = USER_LOGON_INTER_SUCCESS_LOGON;
            } else {
                // On network logons,
                //  only update the statistics on 'success' if explicitly asked, or the Bad Password count will be zeroed.
                if ( (LogonInfo->ParameterControl & MSV1_0_UPDATE_LOGON_STATISTICS) || UserAll->BadPasswordCount != 0 ) {
            ...
            UserInfo.Internal2.StatisticsToApply = USER_LOGON_BAD_PASSWORD | USER_LOGON_BAD_PASSWORD_WKSTA;
        ...
        if ( UserInfo.Internal2.StatisticsToApply != 0 ) {
            ...
            LogonStatus = I_SamrSetInformationUser(UserHandle, UserInternal2Information, &UserInfo );
        }
    }

    // Audit this logon. We don't audit failures for the guest account because they are so frequent.
...
Routine Description:
    ...
    This routine is called directly from the MSV Authentication package if the account is defined locally.
    This routine is called from the Netlogon Service otherwise.
...
Arguments:
    ...
    AccountsToTry -- Specifies whether the username specified in LogonInformation is to be used to logon, whether to guest account is to be used to logon, or both serially.
...
--*/
{
    ...
    *BadPasswordCountZeroed = FALSE;// Validate the specified user.

    if ( AccountsToTry & MSVSAM_SPECIFIED ) {
        // Keep track of the total number of logons attempted.
        I_SamIIncrementPerformanceCounter(MsvLogonCounter);
        ...
        // If the SAM database authoritatively handled this logon attempt, just return.
        if ( *Authoritative ) {
            return Status;
        }
    ...
    // If guest accounts are not allowed, return now.
    LogonInfo = (PNETLOGON_LOGON_IDENTITY_INFO) LogonInformation;
    if ( LogonLevel != NetlogonNetworkInformation ||
    ...
    if ( NT_SUCCESS(GuestStatus) ) {
        ...
        ASSERT ((ValidationLevel == NetlogonValidationSamInfo) || (ValidationLevel == NetlogonValidationSamInfo2) );
        ValidationInfo = (PNETLOGON_VALIDATION_SAM_INFO2) *ValidationInformation;
        ValidationInfo->UserFlags |= LOGON_GUEST;
        return GuestStatus;
    }

    // Failed Guest logon attempts are never authoritative and the status from the original logon attempt is more significant than the Guest logon status.
    *Authoritative = FALSE;
    return Status;
}
...
/*++
Routine Description:
    Process an interactive, network, or session logoff.
    It simply updates the logon statistics for the user account.

    This routine is called directly from the MSV Authentication package if the user was logged on not using the Netlogon service.
    This routine is called from the Netlogon Service otherwise.
Arguments:
    DomainHandle -- Specifies a handle to the SamDomain containing the user to logoff.
    LogonLevel -- Specifies the level of information given in LogonInformation.
    LogonInformation -- Specifies the description for the user logging on.  The LogonDomainName field should be ignored. The caller is responsible for validating this field.
Return Value:
    STATUS_SUCCESS: if there was no error.
    STATUS_INVALID_INFO_CLASS: LogonLevel or ValidationLevel are invalid.
    STATUS_NO_SUCH_USER: The specified user has no account.
...
ULONG MsvGetLogonAttemptCount (VOID)
/*++
Routine Description:
...
--*/
{
    return NlpLogonAttemptCount;// Keep track of the total number of logons attempted.
}