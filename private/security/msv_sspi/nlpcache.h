...
// current hack registry can only handles names valid on underlying file system.
// Therefore can't have long names if registry on FAT - use LSD or, alternatively, LCD
...
// CACHE_PASSWORDS - passwords are stored (in secret storage) as two encrypted one way function (OWF) passwords concatenated together.
...
// <GroupCount> GROUP_MEMBERSHIP structures, followed by a SID which is the LogonDomainId.
// The rest of the data in the entry is the buffer areas for the UNICODE_STRING fields
typedef struct _LOGON_CACHE_ENTRY {
    ...
    // The following fields are present in NT1.0A release and later systems.
    ...
    // this improves performance and eliminates problems with storing data in 2 locations.

    // note: data from this point forward is encrypted and protected from
    // tampering via HMAC.  This includes the data marshalled beyond the structure.
...
// pre-NT5 versions of the LOGON_CACHE_ENTRY structure, for sizing and field mapping purposes for backwards compatibility.
...
NTSTATUS NlpAddCacheEntry(IN  PNETLOGON_INTERACTIVE_INFO LogonInfo, IN  PNETLOGON_VALIDATION_SAM_INFO2 AccountInfo, IN  PVOID SupplementalCacheData, IN  ULONG SupplementalCacheDataLength);
NTSTATUS
NlpGetCacheEntry(
