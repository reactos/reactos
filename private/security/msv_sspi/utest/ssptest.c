...
--*/
{
    ...
    do {
        ...
        pDsGetDcNameA = (DWORD (WINAPI *)(LPCSTR, LPCSTR, GUID *, LPCSTR, ULONG, PDOMAIN_CONTROLLER_INFOA *)) GetProcAddress(hLogonsrv, "DsGetDcNameA");
        ...
        NetStatus = (*pDsGetDcNameA)(NULL, "NTDEV", NULL, NULL, DS_DIRECTORY_SERVICE_REQUIRED | DS_RETURN_FLAT_NAME, &ControllerInfo);

        if (hLogonsrv)
            FreeLibrary(hLogonsrv);
    ...
    if ( !QuietMode ) {
        printf( "CredentialHandle1: 0x%lx 0x%lx   ", CredentialHandle1.dwLower, CredentialHandle1.dwUpper );
...
// #define DO_OEM 1
    ...
    SecStatus = AcquireCredentialsHandle(
                    ...
                    NULL,
                    (DomainName == NULL && UserName == NULL && Password == NULL) ? NULL : &AuthIdentity,
                    NULL,
                    NULL,
                    &CredentialHandle2,
                    &Lifetime );
    ...
    if ( !QuietMode ) {
        printf( "CredentialHandle2: 0x%lx 0x%lx   ", CredentialHandle2.dwLower, CredentialHandle2.dwUpper );
        PrintTime( "Lifetime: ", Lifetime );
    }
    ...
    if ( !QuietMode ) {
        printf( "\n\nNegotiate Message:\n" );
        printf( "ClientContextHandle: 0x%lx 0x%lx   Attributes: 0x%lx ", ClientContextHandle.dwLower, ClientContextHandle.dwUpper, ContextAttributes );
        PrintTime( "Lifetime: ", Lifetime );
    ...
    if ( !QuietMode ) {
        printf( "\n\nChallenge Message:\n" );
        printf( "ServerContextHandle: 0x%lx 0x%lx   Attributes: 0x%lx ", ServerContextHandle.dwLower, ServerContextHandle.dwUpper, ContextAttributes );
        PrintTime( "Lifetime: ", Lifetime );
...
VOID TestLogonRoutine(IN LPWSTR UserName, IN LPWSTR DomainName, IN LPWSTR Password)
{
    NTSTATUS Status;
    ...
    Status = LsaRegisterLogonProcess(&TempName, &LogonHandle, &Dummy);
    if (!NT_SUCCESS(Status))
    {
    ...
    strncpy(SourceContext.SourceName, "ssptest        ", sizeof(SourceContext.SourceName));
    NtAllocateLocallyUniqueId(&SourceContext.SourceIdentifier);

    RtlInitString(&TempName, NTLMSP_NAME_A);
    Status = LsaLookupAuthenticationPackage(LogonHandle, &TempName, &PackageId);
    if (!NT_SUCCESS(Status))
    {
...
#define QUIET_PARAM "/Q"
    ...
    for ( i=1; i<argc; i++ ) {
        ...
        // Handle /TestSsp:
        } else if ( _strnicmp( argument, TESTSSP2_PARAM, sizeof(TESTSSP2_PARAM)-1 ) == 0 ){
            char *end;
            if ( Action != NoAction ) {
                goto Usage;
            }
