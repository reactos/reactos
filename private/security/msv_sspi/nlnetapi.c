...
Abstract:
   This module loads Netapi.dll at runtime and sets up pointers to the APIs called by Msv1_0.
...
typedef NTSTATUS (*PI_NetNotifyNetlogonDllHandle) (IN PHANDLE Role);


VOID NlpLoadNetlogonDll (VOID)
/*++
Routine Description:
    Uses Win32 LoadLibrary and GetProcAddress to get pointers to functions in Netlogon.dll that are called by Msv1_0.
Return Value:
    None.  If successful, NlpNetlogonDllHandle is set to non-null and function pointers are setup.
--*/
{
    HANDLE hModule = NULL;
    ...
    NlpNetLogonSamLogon = (PNETLOGON_SAM_LOGON_PROCEDURE) GetProcAddress(hModule, "NetrLogonSamLogon");
    ...
    NlpNetLogonSamLogoff = (PNETLOGON_SAM_LOGOFF_PROCEDURE) GetProcAddress(hModule, "NetrLogonSamLogoff");
    ...
    pI_NetNotifyNetlogonDllHandle = (PI_NetNotifyNetlogonDllHandle) GetProcAddress( hModule, "I_NetNotifyNetlogonDllHandle" );
