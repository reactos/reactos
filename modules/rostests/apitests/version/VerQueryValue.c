...
static const TEST_VERSIONINFO g_VersionInfo =
{
    ...
    .Value =
    {
        ...
        .dwFileVersionLS = 0x0ECE0F77,
        .dwProductVersionMS = 0x00040009,
        .dwProductVersionLS = 0x00040004,
...
Test_StaticVersionnfo(void)
{
    ...
    result = VerQueryValueW(&g_VersionInfo,
                            L"\\StringFileInfo\\000004B0\\CompanyName",
                            &pvData,
                            &cbLen);
    ok(result, "VerQueryValueW failed\n");
    ok_eq_int(cbLen, 16);
    ok(wcscmp((WCHAR*)pvData, L"ReactOS Project") == 0, "Unexpected value\n");
    ExpectedOffset = FIELD_OFFSET(TEST_VERSIONINFO, StringFileInfo.StringTable.CompanyName.Value);
    ok(pvData == (PVOID)((ULONG_PTR)&g_VersionInfo + ExpectedOffset), "Unexpected offset\n");
    ...
    ok(result, "VerQueryValueW failed\n");
    ok_eq_int(cbLen, 0);
    ok(wcscmp((WCHAR*)pvData, L"") == 0, "Unexpected value\n");
