...
void Test_File(void)
{
    ...
    Result = GetModuleFileNameW(NULL, FilePath, MAX_PATH);
    ...
    if (!Success)
    {
        trace("GetVolumeNameForVolumeMountPointW failed: %ld\n", GetLastError());
        return;
    }
...
{
    HMODULE hmodKerne32 = GetModuleHandleW(L"kernel32.dll");
    pGetFinalPathNameByHandleW = (FN_GetFinalPathNameByHandleW*)
        GetProcAddress(hmodKerne32, "GetFinalPathNameByHandleW");
