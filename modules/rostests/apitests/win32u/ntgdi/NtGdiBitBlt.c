...
START_TEST(NtGdiBitBlt)
{
    ...
    ok_int(bRet, FALSE);
    ok_long(GetLastError(), GetNTVersion() >= _WIN32_WINNT_WIN10 ? 0xDEADBEEF : ERROR_INVALID_PARAMETER);

    /* Test invalid dc */
    SetLastError(0xDEADBEEF);
    bRet = NtGdiBitBlt((HDC)0x123456, 0, 0, 10, 10, (HDC)0x123456, 10, 10, SRCCOPY, 0, 0);
    ok_int(bRet, FALSE);
    ok_long(GetLastError(), GetNTVersion() >= _WIN32_WINNT_WIN10 ? 0xDEADBEEF : ERROR_INVALID_HANDLE);
