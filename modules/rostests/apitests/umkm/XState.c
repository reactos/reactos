...
AdjustAffinity(VOID)
{
    ...
    if (OldAffinity == 0)
    {
        skip("SetThreadAffinityMask failed\n");
        return;
    }
}
...
void __fastcall set_SSE_state(const M128U64 data[REG_COUNT]);
void __fastcall get_SSE_state(M128U64 data[REG_COUNT]);

void __fastcall set_AVX_state(const M256U64 data[REG_COUNT]);
void __fastcall get_AVX_state(M256U64 data[REG_COUNT]);

void __fastcall set_AVX512_state(const M512U64 data[REG_COUNT]);
void __fastcall get_AVX512_state(M512U64 data[REG_COUNT]);
...
void Test_SSE(void)
{
    ...
    M128U64 InData[REG_COUNT];
    RandomFill128(InData, REG_COUNT);
...
static DWORD WINAPI Thread_AVX(LPVOID Parameter)
{
    ...
    // Set a new "random" state
    RandomFill256(Data256, REG_COUNT);
...
void Test_AVX(void)
{
    ...
    // Fill the array with random numbers
    M256U64 InData[REG_COUNT];
    RandomFill256(InData, REG_COUNT);
...
static DWORD WINAPI Thread_AVX512(LPVOID Parameter)
{
    ...
    // Set a new "random" state
    RandomFill512(Data512, REG_COUNT);
...
void Test_AVX512(void)
{
    ...
    // Fill the array with random numbers
    M512U64 InData[REG_COUNT];
    RandomFill512(InData, REG_COUNT);
