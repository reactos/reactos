...
static DWORD WINAPI Thread_SSE(LPVOID Parameter)
{
    ...
    M128U64 Data128[REG_COUNT_SSE] = { 0 };
    get_SSE_state(Data128);

    // Make sure it's all zero
    static const M128U64 Zero128 = { 0 };
    for (ULONG i = 0; i < REG_COUNT_SSE; i++)
    {
        ok_eq_m128i(Data128[i], Zero128);
    }

    // Set a new "random" state
    RandomFill128(Data128, REG_COUNT_SSE);
    set_SSE_state(Data128);
    return 0;
}
...
void Test_SSE(void)
{
    ...
    M128U64 InData[REG_COUNT_SSE];
    RandomFill128(InData, REG_COUNT_SSE);

    _SEH2_TRY
    {
        // Set the state
        set_SSE_state(InData);
    }
    ...
    // Get the state
    M128U64 OutData[REG_COUNT_SSE] = { 0 };
    get_SSE_state(OutData);

    // Validte the state of the non-volatile registers
    for (ULONG i = 6; i < REG_COUNT_SSE; i++)
    {
        ok_eq_m128i(InData[i], OutData[i]);
    }
}
...
static DWORD WINAPI Thread_AVX(LPVOID Parameter)
{
    ...
    // Get the current (fresh) state
    M256U64 Data256[REG_COUNT_AVX];
    get_AVX_state(Data256);

    // Make sure it's all zero
    static const M256U64 Zero256 = { 0 };
    for (ULONG i = 0; i < REG_COUNT_AVX; i++)
    {
        ok_eq_m256i(Data256[i], Zero256);
    }

    // Set a new "random" state
    RandomFill256(Data256, REG_COUNT_AVX);
    set_AVX_state(Data256);
    return 0;
}
...
void Test_AVX(void)
{
    ...
    // Fill the array with random numbers
    M256U64 InData[REG_COUNT_AVX];
    RandomFill256(InData, REG_COUNT_AVX);

    _SEH2_TRY
    {
        // Set the state
        set_AVX_state(InData);
    }
    _SEH2_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
    {
    ...
    // Get the state
    M256U64 OutData[REG_COUNT_AVX] = { 0 };
    get_AVX_state(OutData);

    // Validte the state of the non-volatile registers
    for (ULONG i = 6; i < REG_COUNT_AVX; i++)
    {
        ok_eq_m256i(InData[i], OutData[i]);
    }
}
...
static DWORD WINAPI Thread_AVX512(LPVOID Parameter)
{
    ...
    // Get the current (fresh) state
    M512U64 Data512[REG_COUNT_AVX512];
    get_AVX512_state(Data512);

    // Make sure it's all zero
    static const M512U64 Zero512 = { 0 };
    for (ULONG i = 0; i < REG_COUNT_AVX512; i++)
    {
        ok_eq_m512i(Data512[i], Zero512);
    }

    // Set a new "random" state
    RandomFill512(Data512, REG_COUNT_AVX512);
    set_AVX512_state(Data512);
    return 0;
}
...
void Test_AVX512(void)
{
    ...
    // Fill the array with random numbers
    M512U64 InData[REG_COUNT_AVX512];
    RandomFill512(InData, REG_COUNT_AVX512);

    _SEH2_TRY
    {
        // Set the state
        set_AVX512_state(InData);
    }
    _SEH2_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
    {
    ...
    // Get the state
    M512U64 OutData[REG_COUNT_AVX512] = { 0 };
    get_AVX512_state(OutData);

    // Validte the state of the non-volatile registers
    for (ULONG i = 6; i < REG_COUNT_AVX512; i++)
    {
        ok_eq_m512i(InData[i], OutData[i]);
    }
}
