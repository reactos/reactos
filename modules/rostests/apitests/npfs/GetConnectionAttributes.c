 ...
 * PURPOSE:     Named Pipe File System (NPFS) Extended Attributes test
 * COPYRIGHT:   Copyright 2026 Mohammad Amin Mollazadeh <madamin@proton.me>
 */
...
static char NonExistentAttributeName[] = "NonExistentAttribute";
...
static void NamedPipeClientChildProcess(void)
{
    ...
    /* Keep the child process alive while the pipe is still connected. */
...
START_TEST(NpfsGetConnectionAttributes)
{
    NTSTATUS Status;
    HANDLE hPipe = INVALID_HANDLE_VALUE;
    char Buffer[256]; /* For extended attribute data */
    ...
    hPipe = CreateNamedPipeW(L"\\\\.\\pipe\\NpfsApiTestPipe",
                             ...
                             1,
                             256, 256, /* Pipe Read/Write buffer size */
                             0, NULL);
    if (hPipe == INVALID_HANDLE_VALUE)
    {
    ...
    /* Query for a non-existence attribute */
    Status = NtFsControlFile(hPipe, NULL, NULL, NULL,
                             &IoStatusBlock,
                             FSCTL_PIPE_GET_CONNECTION_ATTRIBUTE,
                             NonExistentAttributeName, sizeof(NonExistentAttributeName),
    ...
    *(DWORD*)Buffer = 0xdeadbeef;
    ...
    ok_eq_hex(Status, STATUS_SUCCESS);
    ok_eq_ulong(IoStatusBlock.Information, sizeof(DWORD));
    ok_eq_ulong(*(DWORD*)Buffer, SessionId);
