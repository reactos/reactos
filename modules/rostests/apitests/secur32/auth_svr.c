 ...
 * PURPOSE:         This is a server-side SSPI Windows Sockets program.
...
svr_GenContext(
    ...
    OUT DWORD *pcbOut,
    OUT BOOL *pfDone,
    IN BOOL fNewConversation);

BOOL svr_DoAuthentication(
    IN PSVR_PARAMS psp,
    IN int ServerPort,
    IN SOCKET AuthSocket);

BOOL svr_AcceptAuthSocket(
    IN PSVR_PARAMS psp,
    IN int ServerPort,
    OUT SOCKET *ServerSocket)
{
    ...
    *ServerSocket = sockClient;
    ...
    return svr_DoAuthentication(psp, ServerPort, sockClient);
}
    ...
    IN SOCKET AuthSocket)
{
    ...
    while (!done)
    {
        ...
        if (!svr_GenContext(psp,
                            ...
                            fNewConversation))
        {
            sync_err("svr_GenContext failed.\n");
            return FALSE;
        }
    ...
    IN BOOL fNewConversation)
{
    ...
    ULONG             Attribs = 0;
    PAUTH_TEST_DATA_SVR ptest = psp->ptest;
...
printf("XXX \n %lx \n", ss);
    ...
    if (fNewConversation)
    {
        sync_ok(Attribs == ptest->ASCContextRETAttr1, "Attribs 0x%x wrong, expected 0x%x.\n",
            Attribs, ptest->ASCContextRETAttr1);
        sync_ok(OutSecBuff.cbBuffer != 0, "OutSecBuff.cbBuffer == 0\n");
    }
    else
    {
        sync_ok(Attribs == ptest->ASCContextRETAttr2, "Attribs 0x%x wrong, expected 0x%x.\n",
            Attribs, ptest->ASCContextRETAttr2);
        sync_ok(OutSecBuff.cbBuffer == 0, "OutSecBuff.cbBuffer != 0\n");
        /* avoid more errors */
    ...
    IN int ServerPort)
{
    ...
    //while (TRUE)
    {
        ...
        Success = svr_AcceptAuthSocket(psp, ServerPort, &Server_Socket);
        sync_ok(Success, "svr_AcceptAuthSocket failed\n");
        if (!Success)
        {
...
int svr_auth_main(int argc, WCHAR** argv)
{
    ...
    testDataIdx = _wtoi(argv[1]);
    if (testDataIdx > AUTH_TEST_DATA_SIZE)
    {
        sync_err("internal error - testindex out of bounds!\n");
        goto done;
    }
    sp.PackageName = authtestdata[testDataIdx].PackageName;
    sp.ptest = &authtestdata[testDataIdx].svr;
