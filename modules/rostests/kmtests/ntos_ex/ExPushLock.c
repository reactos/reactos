 ...
 * PURPOSE:     ReactOS push lock tests
...
static
LARGE_INTEGER
PushLockRelativeTimeout(
    _In_ ULONG Milliseconds)
{
    LARGE_INTEGER Timeout;

    Timeout.QuadPart = -((LONGLONG)Milliseconds * 10 * 1000);
    ...
    _In_ PKEVENT Event)
{
    ...
    Timeout = PushLockRelativeTimeout(PUSH_LOCK_TIMEOUT_MS);
...
PushLockDelay(VOID)
{
    ...
    Delay = PushLockRelativeTimeout(PUSH_LOCK_POLL_INTERVAL_MS);
...
static
BOOLEAN
PushLockStartControlledThread(_Inout_ PPUSH_LOCK_THREAD_CONTEXT Context,
...
TestPushLockWaiterSelection(VOID)
{
    ...
    PKTHREAD Threads[RTL_NUMBER_OF(Modes)];
    NTSTATUS Status;
    ULONG Started;
    ULONG Index;
...
TestPushLockWaiterArrivalDuringRelease(VOID)
{
    ...
    for (Iteration = 0; Iteration < PUSH_LOCK_RACE_ITERATIONS; Iteration++)
    {
        ...
        /* Make the new waiter ready immediately before the final shared release.
           The scheduler may insert it before or during the release. This camn't
...
TestPushLockContention(VOID)
{
    ...
    PKTHREAD Threads[PUSH_LOCK_CONTENTION_THREADS];
    KEVENT StartGate;
    NTSTATUS Status;
    ULONG Index;
    ULONG Started;
