...
#ifdef __REACTOS__
    ok(hkResult == NULL || broken(LOBYTE(LOWORD(GetVersion())) < 6), "expected hkResult == NULL\n");
#else
    ok(hkResult == NULL, "expected hkResult == NULL\n");
#endif
...
static void test_redirection(void)
{
    ...
    if (LOBYTE(LOWORD(GetVersion())) == 6 && HIBYTE(LOWORD(GetVersion())) == 0) {
...
static void test_performance_keys(void)
{
    ...
    if (LOBYTE(LOWORD(GetVersion())) < 6 ||
        (LOBYTE(LOWORD(GetVersion())) == 6 && HIBYTE(LOWORD(GetVersion())) == 0)) {
#else
    if (LOBYTE(LOWORD(GetVersion())) < 6) {
#endif
