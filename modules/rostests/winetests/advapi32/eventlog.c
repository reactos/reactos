...
static void test_eventlog_start(void)
{
    ...
    while (!found && ret)
    {
        ...
        if (ret && record->EventID == EVENT_EventlogStarted)
        {
...
#ifdef __REACTOS__
            ok(record->NumStrings == 0 || broken(_ntMajor < 6), "Expected 0, got %d\n", record->NumStrings);
#else
            ...
            ok(record->UserSidLength == 0, "Expected 0, got %ld\n", record->UserSidLength);
#ifdef __REACTOS__
            ok(record->DataLength == 24 || broken(_ntMajor < 6), "Expected 24, got %ld\n", record->DataLength);
            ok(record->DataOffset == record->UserSidOffset || broken(_ntMajor < 6), "Expected offsets to be the same\n");
#else
            ...
            size = (size + 7) & ~7;
#ifdef __REACTOS__
            ok(record->DataOffset == size || broken(_ntMajor < 6) ||
...
#ifdef __REACTOS__
    ok(!ret || broken(_ntMajor == 6 && _ntMinor == 0), "Expected failure\n");
