...
static void test_lcmapstring_unicode(lcmapstring_wrapper func_ptr, const char *func_name)
{
...
#ifdef __REACTOS__
    ok(!lstrcmpW(buf, math_result) ||
       broken(!lstrcmpW(buf, math_text)) /* WS03 */, "%s string compare mismatch %s\n", func_name, debugstr_w(buf));
#else
    ...
    ok(ret == lstrlenW(buf) + 1, "%s ret %d, error %ld, expected value %d\n", func_name,
       ret, GetLastError(), lstrlenW(buf) + 1);
#ifdef __REACTOS__
    ok(!lstrcmpW(buf, cjk_compat_result) ||
       broken(!lstrcmpW(buf, cjk_compat_text)) /* WS03 */, "%s string compare mismatch %s\n", func_name, debugstr_w(buf));
#else
