...
static void test_dos_devices(void)
{
    ...
    BOOL ret;
#ifdef __REACTOS__
    char root_path[MAX_PATH] = {0}, buf3[MAX_PATH] = {0};
...
#ifdef __REACTOS__
    strcpy(buf3, root_path);
    strcat(buf3, "/");
    for (char *p = buf3; *p; p++) {
        // Replace backslashes with forward slashes
        if (*p == '\\') *p = '/';
        // Lowercase everything except the first character
        else if (p != buf3) *p = (char)tolower((unsigned char)*p);
    }
    ...
    strcat(buf3, "\\??\\");
    strcat(buf3, root_path);
    for (char *p = buf3; *p; p++) {
        // Lowercase everything except "C:\"
        if (p != (buf3 + 4)) *p = (char)tolower((unsigned char)*p);
    }
    strcat(buf3, "\\");
...
#ifdef __REACTOS__
    strcpy(buf, "C:\\winetest_mnt\\");
    strcat(buf, sysroot + 3);
...
#ifdef __REACTOS__
        strcpy(buf, "C:\\winetest_link");
        strcat(buf, sysroot + 2);
        ret = GetVolumePathNameA( buf, path, sizeof(path) );
#else
...
#ifdef __REACTOS__
        strcpy(buf, "C:\\winetest_link\\winetest_mnt\\winetest_link");
        strcat(buf, sysroot + 2);
        strcat(buf, "\\");
        ret = GetVolumePathNameA( buf, path, sizeof(path) );
#else
...
static void test_GetVolumeInformationByHandle(void)
{
    ...
    for (char *p = sysroot; *p; p++) {
        ...
        if (*p == '\\') *p = '/';
        // Lowercase everything except "C:"
        else if (p != sysroot) *p = (char)tolower((unsigned char)*p);
    }
#endif
