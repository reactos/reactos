...
/* NOTE: Pre-Vista does not recognize the "all processors" flag (all bits set) */
   ...
   if (retMask == processMask && sizeof(ULONG_PTR) > sizeof(ULONG))
   {
    ...
    else
    {
        ...
        ok(ret == ~0u || broken(ret == 0) /* x86_64 */, "Unexpected return value %lu.\n", ret);
        ok(GetLastError() == ERROR_INVALID_PARAMETER || broken(GetLastError() == 0xdeadbeef) /* x86_64 */, "Unexpected error %ld.\n", GetLastError());
#else
        ok(ret == ~0u, "Unexpected return value %lu.\n", ret);
        ok(GetLastError() == ERROR_INVALID_PARAMETER, "Unexpected error %ld.\n", GetLastError());
#endif
...
static inline unsigned long get_fpu_cw(void)
{
    ...
    extern void get_fpu_cw_raw(WORD *cw, unsigned int *sse);
    get_fpu_cw_raw(&cw, &sse);
#else
    __asm { fnstcw [cw] }
    __asm { stmxcsr [sse] }
#endif
