                                 ...
                                 const void *section_data )
{
    ...
    {
#ifdef __REACTOS__
        ok( image.ComPlusILOnly || broken(LOBYTE(LOWORD(GetVersion())) < 6),
#else
...
#ifdef __REACTOS__
            ok( image.ComPlusNativeReady || broken(LOBYTE(LOWORD(GetVersion())) < 6),
#else
...
#ifdef __REACTOS__
        ok( image.ImageMappedFlat || broken(LOBYTE(LOWORD(GetVersion())) < 6),
#else
...
#ifdef __REACTOS__
        ok( image.ImageDynamicallyRelocated || broken(LOBYTE(LOWORD(GetVersion())) < 6),
#else
...
#ifdef __REACTOS__
            if (!hlib_as_data_file && GetLastError() == ERROR_INVALID_PARAMETER) {
                skip("LOAD_LIBRARY_AS_DATAFILE_EXCLUSIVE not supported\n");
...
#ifdef __REACTOS__
            if (!hlib_as_data_file && GetLastError() == ERROR_INVALID_PARAMETER) {
                skip("LOAD_LIBRARY_AS_IMAGE_RESOURCE not supported\n");
                FreeLibrary(hlib_as_data_file);
            }
            else {
#endif // __REACTOS__
...
#ifdef __REACTOS__
        if (LOBYTE(LOWORD(GetVersion())) >= 6) { // Crashes on WS03
#endif // __REACTOS__
...
#define ADD_RELOC(field) data.rel.type_off[nb_rel++] = (IMAGE_REL_BASED_HIGHLOW << 12) + offsetof( struct imports, field )
...
#ifdef __REACTOS__
            ok( ptr->tls_index < 32 || broken(ptr->tls_index == 9999) /* WS03 */, "wrong tls index %d\n", ptr->tls_index );
...
#ifdef __REACTOS__
            ok( ptr->tls_index < 32 || broken(ptr->tls_index == 9999) /* WS03 */, "wrong tls index %d\n", ptr->tls_index );
            if (sizeof(tls_init_code) > 1 && ptr->tls_index != 9999)
#else
            ok( ptr->tls_index < 32, "wrong tls index %d\n", ptr->tls_index );
        ...
        case 6:  /* map with ntdll, without IMAGE_DLLCHARACTERISTICS_DYNAMIC_BASE */
...
#ifdef __REACTOS__
            ok( test == 6 ? !image.ImageDynamicallyRelocated : image.ImageDynamicallyRelocated || broken(LOBYTE(LOWORD(GetVersion())) < 6),
#else
            ...
            todo_wine_if (test == 5)
#ifdef __REACTOS__
            ok( status == (test == 6 ? STATUS_IMAGE_NOT_AT_BASE : STATUS_SUCCESS) || broken(LOBYTE(LOWORD(GetVersion())) < 6),
#else
...
static BOOL WINAPI dll_entry_point(HINSTANCE hinst, DWORD reason, LPVOID param)
{
    ...
    case DLL_PROCESS_DETACH:
    {
...
#ifdef __REACTOS__
                ok(!ret || broken(LOBYTE(LOWORD(GetVersion())) < 6), "RtlDllShutdownInProgress returned %ld\n", ret);
#else
                ok(!ret, "RtlDllShutdownInProgress returned %ld\n", ret);
#endif // __REACTOS__
        }
        ...
        if (param && pFlsGetValue)
        {
            ...
            value = pFlsGetValue(fls_index);
#ifdef __REACTOS__
            ok(value == NULL || broken(value == (void*)0x31415) /* WS03 */, "FlsGetValue returned %p, expected NULL\n", value);
#else
            ok(value == NULL, "FlsGetValue returned %p, expected NULL\n", value);
#endif // __REACTOS__
            ok(GetLastError() == ERROR_SUCCESS, "FlsGetValue failed with error %lu\n", GetLastError());
#ifdef __REACTOS__
            ok(fls_callback_count == thread_detach_count + 1 || broken(LOBYTE(LOWORD(GetVersion())) < 6),
        ...
        {
#ifdef __REACTOS__
            ok(!handle || broken(handle != 0) /* Pre-Win7 */, "CreateThread should fail\n");
        ...
        {
#ifdef __REACTOS__
            ok(!handle || broken(handle != 0) /* Pre-Win7 */, "CreateRemoteThread should fail\n");
    ...
    case DLL_THREAD_DETACH:
        ...
        if (pFlsGetValue && fls_index != FLS_OUT_OF_INDEXES)
        {
            ...
            value = pFlsGetValue(fls_index);
#ifdef __REACTOS__
            ok(!value || broken(value == (void*)0x31415) /* WS03 */, "FlsGetValue returned %p, expected NULL\n", value);
#else
            ok(!value, "FlsGetValue returned %p, expected NULL\n", value);
#endif // __REACTOS__
            ok(GetLastError() == ERROR_SUCCESS, "FlsGetValue failed with error %lu\n", GetLastError());
...
#include "poppack.h"
...
#ifdef __REACTOS__
    ok(ret == 195 || broken(ret == 0xc0000005) /* WS03 */, "expected exit code 195, got %lu\n", ret);
#else
    ...
    *child_failures = -1;
    ...
    todo_wine
#ifdef __REACTOS__
    ok(ret == WAIT_TIMEOUT || broken(ret == WAIT_OBJECT_0) /* WS03 */, "child process should fail to terminate\n");
#else
    ok(ret == WAIT_TIMEOUT, "child process should fail to terminate\n");
#endif // __REACTOS__
    if (ret != WAIT_OBJECT_0)
    ...
    todo_wine
#ifdef __REACTOS__
    ok(ret == 201 || broken(ret == 1) /* WS03 */, "expected exit code 201, got %lu\n", ret);
#else
