...
#ifdef __REACTOS__
            ok( !expect_status || broken(is_win64) /* Broken on Vista, 8.1 x64 */, "%u: got fallback but expected failure %lx\n", line, expect_status );
            ok( expect_fallback || broken(is_win64) /* Broken on Vista, 8.1 x64 */ ||
#else
