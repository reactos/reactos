...
#ifdef __REACTOS__
/* Under GCC our ARRAYSIZE() expands to RTL_NUMBER_OF_V2(), which wraps the
   count in a statement expression to type-check the argument. That is not a
   constant expression, so it cannot size a struct member: the surrounding
   struct becomes variably modified and can no longer be initialized. Use the
   plain sizeof division from wine/test.h instead. */
#undef ARRAYSIZE
...
static DWORD CALLBACK enumSessionsProc( void *p )
{
...
static DWORD CALLBACK openProc( void *p )
{
    OpenParam *param = p;
...
static DWORD CALLBACK createPlayerProc( void *p )
{
...
static void test_COM(void)
{
    ...
    IDirectPlay3 *dp3;
    IDirectPlay4A *dp4A;
#ifdef __REACTOS__ /* MSVC C4312: widen the sentinel so the cast is not narrowing on Win64 */
