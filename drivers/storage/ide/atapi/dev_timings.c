    ...
    _In_ BOOLEAN ForceCompatibleTimings)
{
    ...
    if (ForceCompatibleTimings)
    {
        for (i = 0; i < ATA_MAX_DEVICE; ++i)
        {
            ...
            Config[i].CurrentModes = 0;
            Config[i].PioCycleTime = AtapTimingTable[PIO_MODE(0)];
            Config[i].SwDmaCycleTime = AtapTimingTable[SWDMA_MODE(0)];
            Config[i].MwDmaCycleTime = AtapTimingTable[MWDMA_MODE(0)];
    ...
    for (i = 0; i < ATA_MAX_DEVICE; ++i)
    {
        ...
        AllowedModesMask = DevExt->TransferModeUserAllowedMask & DevExt->TransferModeAllowedMask;
        ...
        Config[i].PioCycleTime = DevExt->PioCycleTime;
        Config[i].SwDmaCycleTime = DevExt->SingleWordDmaCycleTime;
        Config[i].MwDmaCycleTime = DevExt->MultiWordDmaCycleTime;
