    ...
    _In_ ULONG OutputBufferLength)
{
     ...
     */
    Irp = IoBuildDeviceIoControlRequest(IOCTL_ACPI_ASYNC_EVAL_METHOD,
                                        TopDeviceObject,
                                        InputBuffer,
                                        InputBufferLength,
                                        OutputBuffer,
    ...
    _In_opt_ PIDENTIFY_DEVICE_DATA IdBlock2)
{
    ...
    InputBuffer = ExAllocatePoolUninitialized(NonPagedPool, InputSize, ATAPORT_TAG);
    if (!InputBuffer)
    {
    ...
    Status = AtaAcpiEvaluateObject(ChanExt->Common.Self, InputBuffer, InputSize, NULL, 0);
    if (!NT_SUCCESS(Status))
    {
    ...
    _In_ PIDENTIFY_DEVICE_DATA IdBlock)
{
    ...
    Status = AtaAcpiEvaluateObject(DevExt->Common.Self, InputBuffer, InputSize, NULL, 0);
    if (!NT_SUCCESS(Status))
    {
...
AtaAcpiSetupNextGtfRequest(
    ...
    _In_ PULONG Index)
{
    PACPI_EVAL_OUTPUT_BUFFER OutputBuffer = DevExt->GtfDataBuffer;

    ASSERT(OutputBuffer && OutputBuffer->Signature == ACPI_EVAL_OUTPUT_BUFFER_SIGNATURE);

    while (*Index < OutputBuffer->Argument[0].DataLength)
    {
        PATA_ACPI_TASK_FILE AcpiTaskFile;

        AcpiTaskFile = (PATA_ACPI_TASK_FILE)OutputBuffer->Argument[0].Data + (*Index++);

        if (!AtaAcpiFilterTaskFile(AcpiTaskFile))
            continue;

        AtaAcpiTaskFileToInternalTaskFile(AcpiTaskFile, Request);
        return TRUE;
    }
