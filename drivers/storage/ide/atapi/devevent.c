    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    if (IsPowerOfTwo(DevExt->ActiveSlotsBitmap))
    {
        ...
        NT_VERIFY(_BitScanForward(&Slot, DevExt->ActiveSlotsBitmap) != 0);
    ...
    DevExt->WorkerContext.PausedSlotsBitmap |= DevExt->ActiveSlotsBitmap;
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    DevExt->WorkerContext.OldRequest = DevExt->Slots[Request->Slot];
    DevExt->Slots[Request->Slot] = Request;
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    FailedRequest = DevExt->Slots[LogPage->NcqTag];
...
AtaDeviceDisableDma(
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    WARN("DMA failures, disabling DMA\n");
    ...
    switch (FailedRequest->SrbStatus)
    {
        ...
        case SRB_STATUS_ERROR:
        {
            if (AtaDeviceIsDmaCrcError(DevExt, FailedRequest) &&
                !(FailedRequest->Flags & REQUEST_FLAG_PASSTHROUGH))
            {
                /* AtaDeviceLogEvent(DevExt, EVENT_CODE_CRC_ERROR); */
    ...
    _In_ PATA_DEVICE_REQUEST FailedRequest)
{
    ...
    {
        /*
         * We usually do not want to retry an request, because the upper class driver
    ...
    AtaReqStartCompletionDpc(DevExt, FailedRequest);
}
...
AtaDeviceHandleEnum(
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    if (!AtaAhciEnumerateDevice(DevExt))
    {
        AtaDeviceSetState(DevExt, DEVICE_STATE_RESET);
        return;
    }
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    if (!IS_AHCI(DevExt) &&
        !(DevExt->ChanExt->Flags & (CHANNEL_IO_MODE_DETECTED | CHANNEL_CBUS_IDE)))
    {
        /* Switch the controller over to 32-bit I/O mode */
        DevExt->ChanExt->Flags |= CHANNEL_IO32 | CHANNEL_IO_MODE_DETECTED;
        ...
        else
        {
            ...
            DevExt->ChanExt->Flags &= ~CHANNEL_IO32;
        }
    }
    ...
    if (!IS_ATAPI(DevExt))
    {
        ...
        if (DevExt->MultiSectorTransfer != 0)
        {
            ...
            TaskFile->Feature = IDE_COMMAND_SET_MULTIPLE;
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    DevExt->Slots[Request->Slot] = DevExt->WorkerContext.OldRequest;
