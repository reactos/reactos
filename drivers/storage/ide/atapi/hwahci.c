    ...
    _In_ ULONG Timeout)
{
    ...
    for (i = Timeout; i > 0; --i)
    {
        Data = AHCI_PORT_READ(DevExt->IoBase, Register);
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    SataControl = AHCI_PORT_READ(DevExt->IoBase, PxSataControl);
    SataControl &= ~AHCI_PXCTL_DET_MASK;
    SataControl |= AHCI_PXCTL_DET_RESET;
    AHCI_PORT_WRITE(DevExt->IoBase, PxSataControl, SataControl);
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    AHCI_PORT_WRITE(DevExt->IoBase, PxCommandListBaseLow, (ULONG)PortData->CommandListPhys);
    if (DevExt->ChanExt->AhciCapabilities & AHCI_CAP_S64A)
    {
        AHCI_PORT_WRITE(DevExt->IoBase,
                        PxCommandListBaseHigh,
                        (ULONG)(PortData->CommandListPhys >> 32));
    }

    /* Physical address of the allocated FIS receive area */
    AHCI_PORT_WRITE(DevExt->IoBase, PxFisBaseLow, (ULONG)PortData->ReceivedFisPhys);
    if (DevExt->ChanExt->AhciCapabilities & AHCI_CAP_S64A)
    {
        AHCI_PORT_WRITE(DevExt->IoBase,
                        PxFisBaseHigh,
                        (ULONG)(PortData->ReceivedFisPhys >> 32));
    }
}
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    Success = AtaAhciPollRegister(DevExt,
                                  PxCmdStatus,
                                  AHCI_PXCMD_CR,
                                  0,
                                  AHCI_DELAY_CR_START);
    if (!Success)
    {
        WARN("Failed to stop the command list DMA engine %08lx\n",
             AHCI_PORT_READ(DevExt->IoBase, PxCmdStatus));
    }
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    if (!(DevExt->ChanExt->AhciCapabilities & AHCI_CAP_SCLO))
        return TRUE;
    ...
    return AtaAhciPollRegister(DevExt,
                               PxCmdStatus,
                               AHCI_PXCMD_CLO,
                               0,
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    if (!AtaAhciWaitStopCommandListProcess(DevExt))
        return FALSE;

    AtaAhciCommandListOverride(DevExt);
    ...
    if (!AtaAhciWaitStopFisReceiveProcess(DevExt))
        return FALSE;
    ...
    _In_ ULONG InterruptStatus)
{
    ...
    FirstTaskFileData = AHCI_PORT_READ(DevExt->IoBase, PxTaskFileData);
    CmdStatus = AHCI_PORT_READ(DevExt->IoBase, PxCmdStatus);
    ...
    SecondTaskFileData = AHCI_PORT_READ(DevExt->IoBase, PxTaskFileData);
    ...
    if (DevExt->PortFlags & PORT_NCQ)
    {
        AtaAhciStartCommandListProcess(DevExt);
        return ACTION_ERROR;
    }
    ...
    if (IsPowerOfTwo(DevExt->ActiveSlotsBitmap))
    {
        ...
        SlotsBitmap = DevExt->ActiveSlotsBitmap;
    }
    else if (DevExt->ActiveSlotsBitmap & (1 << CurrentCommandSlot))
    {
    ...
    else
    {
        ...
        if (DevExt->ActiveSlotsBitmap != 0)
        {
            ERR("Invalid slot received from the HBA %08lx --> %08lx\n",
                DevExt->ActiveSlotsBitmap, 1 << CurrentCommandSlot);
        }
        else
        {
    ...
    Request = DevExt->Slots[FailedSlot];
    ASSERT(Request);
    ...
    {
        AtaAhciSaveTaskFile(DevExt, Request);
    }
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    PAHCI_FIS_DEVICE_TO_HOST Fis;

    Fis = &DevExt->PortData->ReceivedFis->DeviceToHostFis;
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    PortData = DevExt->PortData;
    SlotData = &PortData->Slot[Request->Slot];
    ...
    _In_ PSCATTER_GATHER_LIST SgList)
{
    ...
    PrdTableEntry = DevExt->PortData->Slot[Request->Slot].CommandTable->PrdTable;
...
AtaAhciStartIo(
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ASSERT(!(DevExt->ActiveSlotsBitmap & (1 << Request->Slot)));
    ASSERT(!(DevExt->ActiveQueuedSlotsBitmap & (1 << Request->Slot)));
    ASSERT(DevExt->Slots[Request->Slot] == Request);

    AtaAhciBuildCommandHeader(DevExt, Request);

    DevExt->TimerCount[Request->Slot] = Request->TimeOut;
    DevExt->ActiveSlotsBitmap |= 1 << Request->Slot;

    if (Request->Flags & REQUEST_FLAG_NCQ)
    {
        DevExt->ActiveQueuedSlotsBitmap |= 1 << Request->Slot;

        AHCI_PORT_WRITE(DevExt->IoBase, PxSataActive, 1 << Request->Slot);
    }
    AHCI_PORT_WRITE(DevExt->IoBase, PxCommandIssue, 1 << Request->Slot);
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    if (!AtaAhciIdleState(DevExt))
        return FALSE;

    if (DevExt->Flags & DEVICE_ENUM)
        AtaAhciSetupDmaMemoryAddress(DevExt);
    ...
    SataStatus = AHCI_PORT_READ(DevExt->IoBase, PxSataStatus);

    if ((SataStatus & AHCI_PXSSTS_DET_MASK) != AHCI_PXSSTS_DET_DEVICE_PHY)
    {
        AtaAhciSendComReset(DevExt);

        if (!AtaAhciPollRegister(DevExt,
                                 PxSataStatus,
    ...
    Signature = AHCI_PORT_READ(DevExt->IoBase, PxSignature);
...
Done:
    ...
    /* Clear interrupts */
    AHCI_PORT_WRITE(DevExt->IoBase, PxInterruptStatus, 0xFFFFFFFF);
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    AHCI_PORT_WRITE(DevExt->IoBase, PxInterruptEnable, 0);
    ...
    _In_ PATAPORT_DEVICE_EXTENSION DevExt)
{
    ...
    SataSpeed = AHCI_PORT_READ(DevExt->IoBase, PxSataStatus) & AHCI_PXSSTS_SPD_MASK;
    ...
    _In_opt_ PVOID SystemArgument2)
{
    ...
    if (DevExt->ActiveQueuedSlotsBitmap != 0)
        CommandRegister = PxSataActive;
    else
        CommandRegister = PxCommandIssue;
    CommandsIssued = AHCI_PORT_READ(DevExt->IoBase, CommandRegister);
    ...
    if (InterruptStatus & (AHCI_PXIRQ_FATAL_ERROR | AHCI_PXIRQ_PORT_STATUS))
    {
        ...
        {
            Action = AtaAhciHandleFatalError(DevExt, InterruptStatus);
        }

        /* Perform recovery actions */
        if (Action != ACTION_NONE)
        {
            AtaDeviceQueueAction(DevExt, Action);
        }
    }
    ...
    AtaAhciPortCompleteCommands(DevExt, CommandsCompleted);
    ...
    _In_ PATAPORT_AHCI_PORT_INFO PortInfo)
{
    ...
    PortData->LocalSgList.Elements[0].Address.QuadPart = BufferPa;
