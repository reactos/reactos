    ...
    _In_ PIRP Irp)
{
    ...
    DeviceDescriptor->BusType = IS_AHCI(DevExt) ? BusTypeSata : BusTypeAta;
    ...
    _In_ PIRP Irp)
{
    ...
    AccessAlignmentDescriptor->BytesOffsetForSectorAlignment =
        DevExt->SectorSize * AtaDevLogicalSectorAlignment(&DevExt->IdentifyDeviceData);
    ...
    _In_ ULONG BufferSize)
{
    ...
    Srb->TargetId = DevExt->AtaScsiAddress.TargetId;
    Srb->Lun = DevExt->AtaScsiAddress.Lun;
    Srb->PathId = DevExt->AtaScsiAddress.PathId;
    ...
    return Irp->IoStatus.Status; // TODO: Return the "Status" var instead?
}
    ...
    _In_ PIRP Irp)
{
    ...
    AdapterDescriptor->AdapterUsesPio = !IS_AHCI(ChanExt) && !IS_PCIIDE(ChanExt);
    AdapterDescriptor->AdapterScansDown = FALSE;
    AdapterDescriptor->CommandQueueing = FALSE; /* Disable request tagging */
    AdapterDescriptor->AcceleratedTransfer = FALSE;
    AdapterDescriptor->BusType = IS_AHCI(ChanExt) ? BusTypeSata : BusTypeAta;
    ...
    _In_ PIO_STACK_LOCATION IoStack)
{
    ...
    Capabilities->AdapterUsesPio = !IS_AHCI(ChanExt) && !IS_PCIIDE(ChanExt);
