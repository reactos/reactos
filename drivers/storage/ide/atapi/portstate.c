    ...
    _In_ ULONG TargetId)
{
     ...
     */
    _InterlockedOr(&PortData->Worker.EventsPending, ACTION_ENUM_DEVICE_INT);
    ...
    _In_ PATAPORT_PORT_DATA PortData)
{
    ...
    if (DevExt)
    {
        ...
        if (PortData->PortFlags & PORT_FLAG_IS_PMP)
            DevExt->Device.PmpNumber = DevExt->Device.AtaScsiAddress.TargetId;
        else
            DevExt->Device.PmpNumber = 0;
