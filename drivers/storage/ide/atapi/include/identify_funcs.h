 ...
 * PURPOSE:     IDENTIFY DEVICE and IDENTIFY PACKET DEVICE data helper functions
    ...
    _In_ PIDENTIFY_DEVICE_DATA IdentifyData)
{
    ...
    /* Verify the checksum (excludes the crc byte) */
    Crc = 0;
    for (i = 0; i < sizeof(*IdentifyData) - 1; ++i)
    {
...
AtaDevDefaultChsTranslation(
    _In_ PIDENTIFY_DEVICE_DATA IdentifyData,
    _In_ PUSHORT Cylinders,
    _In_ PUSHORT Heads,
    _In_ PUSHORT SectorsPerTrack)
{
    /* Word 1 */
...
AtaDevHasNcqAutosense(
    _In_ PIDENTIFY_DEVICE_DATA IdentifyData)
{
    USHORT Word76 = ((PUSHORT)IdentifyData)[76]; // SerialAtaCapabilities
...
FORCEINLINE
BOOLEAN
AtaDevIsStandyByTimerPeriodsValid(
    _In_ PIDENTIFY_DEVICE_DATA IdentifyData)
{
    /* Bit 13 of word 49 */
