    ...
    _In_ UCHAR Type)
{
    ...
    ChanExt->Common.DevicePowerState = PowerDeviceD0;
    ChanExt->Common.SystemPowerState = PowerSystemWorking;
    ...
    Fdo->AlignmentRequirement = max(ChanExt->Ldo->AlignmentRequirement, FILE_WORD_ALIGNMENT);

    IoInitializeRemoveLock(&ChanExt->Common.RemoveLock, ATAPORT_TAG, 0, 0);
    KeInitializeSpinLock(&ChanExt->PdoListLock);
    KeInitializeDpc(&ChanExt->CompletionDpc, AtaReqCompletionDpc, ChanExt);
    InitializeSListHead(&ChanExt->CompletionQueueList);

    if (!IS_LEGACY_IDE_EXT(ChanExt) && AtaFdoDetectAhciController(ChanExt))
    {
        ChanExt->Common.Flags |= DO_IS_AHCI;
    }
