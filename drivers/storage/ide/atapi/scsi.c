    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    {
        PDMA_ADAPTER AdapterObject = Device->ChanExt->AdapterObject;
    ...
    {
        MmUnmapReservedMapping(Device->ChanExt->ReservedVaSpace, ATAPORT_TAG, Request->Mdl);
        _InterlockedExchange(&Device->ChanExt->ReservedMappingLock, 0);
    }
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    if (Request->InternalState != REQUEST_STATE_NOT_STARTED)
    {
        ...
        if (PortData->AllocatedSlots == 0)
        {
            ...
            {
                PortData->QueueFlags &= ~PORT_QUEUE_FLAG_LAST_TARGET_MASK;
                PortData->QueueFlags |= 0xFF;
            }
        }
        ...
        if ((PortData->QueueFlags & PORT_QUEUE_FLAG_SIGNAL_STOP) && AtaPortQueueEmpty(PortData))
        {
            ...
            AtaPortQueueEmptyEvent(&PortData->Worker);
        }
    ...
    {
        CompletionAction = AtaReqRequeueRequest(Device, Request, &SrbStatus);
    }
...
DbgPrint("************************************** FREEZE \n");
/* ASSERT(0); */
        if (!(Request->SrbStatus & SRB_STATUS_AUTOSENSE_VALID))
            ASSERT(0);
    }
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    PortData->PrepareIo(PortData, Request);
    ...
    {
        if (!PortData->StartIo(PortData, Request))
        {
            ...
            if (Request->Flags & REQUEST_FLAG_POLL)
            {
                ...
                KeInsertQueueDpc(&PortData->PollingTimerDpc, NULL, NULL);
            }
        }
    ...
    _In_ PVOID Context)
{
    ...
    PortData->PreparePrdTable(PortData, Request, SgList);
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    if (Request->Flags & REQUEST_FLAG_HAS_LOCAL_BUFFER)
    {
         ...
         * because this would require the PCIIDEX driver to allocate a common buffer for us.
         */
        ASSERT(IS_AHCI(Device) || !(Request->Flags & REQUEST_FLAG_DMA));
        ...
        {
            /* DMA transfer */
            AtaReqPreparePrdTable(NULL, NULL, &Device->PortData->Ahci.LocalSgList, Request);
        }
        else
        {
            /* PIO data transfer */
    ...
    {
        // TODO what to do for IDE when SgList == NULL?
        AtaReqStartIo(Request);
        return;
    }
    ...
    if (Request->Flags & REQUEST_FLAG_PROGRAM_DMA)
    {
        if (AtaReqGetScatterGatherList(Device->ChanExt, Request, AtaReqPreparePrdTable))
            return;
        ...
        /* S/G list construction failed, attempt to fall back to PATA PIO mode */
        if (!AtaPataDmaTransferToPioTransfer(Request))
            goto CompleteNoMemory;
    }

    /* PATA PIO data transfer path */
    ASSERT(!IS_AHCI(Device));
...
{
    PATAPORT_CHANNEL_EXTENSION ChanExt = DeferredContext;
    ...
    CurrentEntry = ExInterlockedFlushSList(&ChanExt->CompletionQueueList);
    while (CurrentEntry)
    {
        PATA_DEVICE_REQUEST Request;
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    ChanExt = Request->Device->ChanExt;
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    if (Request->Flags & REQUEST_FLAG_NCQ)
    {
        ...
        SlotsBitmap &= Device->MaxQueuedSlotsBitmap;
    }
    else
    {
    ...
    {
        UCHAR LastPmpTarget = PortData->QueueFlags & PORT_QUEUE_FLAG_LAST_TARGET_MASK;

        if ((LastPmpTarget != 0xFF) && (LastPmpTarget != Device->AtaScsiAddress.TargetId))
            return FALSE;

        PortData->QueueFlags &= ~PORT_QUEUE_FLAG_LAST_TARGET_MASK;
        PortData->QueueFlags |= Device->AtaScsiAddress.TargetId;
...
{
    if (&QueueEntry->ListEntry == Device->NextDeviceQueueEntry)
    {
        /* C-LOOK: Move the head position to service other request */
        Device->NextDeviceQueueEntry = Device->NextDeviceQueueEntry->Flink;
    }

    if (RemoveEntryList(&QueueEntry->ListEntry))
    {
        Device->NextDeviceQueueEntry = NULL;
    }
}
    ...
    _In_ PIRP Irp)
{
    ...
    if (!Device->NextDeviceQueueEntry)
        Device->NextDeviceQueueEntry = &QueueEntry->ListEntry;
}
    ...
    _In_ PATAPORT_IO_CONTEXT Device)
{
    ...
    CurrentEntry = Device->NextDeviceQueueEntry;
    while (CurrentEntry)
    {
        PREQUEST_QUEUE_ENTRY QueueEntry;
        PIRP Irp;
    ...
    _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    KeWaitForSingleObject(&Device->QueueStoppedEvent,
                                   Executive,
                                   KernelMode,
                                   FALSE,
                                   NULL);
    ...
    _In_ PATAPORT_IO_CONTEXT Device)
{
    ...
    while (TRUE)
    {
        ...
        QueueEntry = CONTAINING_RECORD(Device->NextDeviceQueueEntry,
                                       REQUEST_QUEUE_ENTRY,
                                       ListEntry);
        ASSERT(QueueEntry->Context == Device);
    ...
    _In_opt_ PVOID SystemArgument2)
{
    ...
    while (_BitScanForward(&Slot, ActiveTimersBitmap) != 0)
    {
        ...
        if (IS_AHCI(Request->Device))
            AtaAhciPortHandleInterrupt(PortData);
        else
            AtaPataPoll(PortData);

        if (PortData->ActiveSlotsBitmap & (1 << Request->Slot))
            DoProcessAgain = TRUE;
    }
    KeReleaseInterruptSpinLock(PortData->ChanExt->InterruptObject, OldIrql);
