    ...
    _Out_ AHCI_FIS_HOST_TO_DEVICE* __restrict Fis)
{
    ...
    Fis->Flags = UPDATE_COMMAND | Request->Device->PmpNumber;
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    if (PortData->PortFlags & PORT_FLAG_FBS_ENABLED)
    {
        AHCI_PORT_WRITE(IoBase,
                        ...
                        (Request->Device->PmpNumber << AHCI_FBS_ISSUE_SHIFT) | AHCI_FBS_ENABLE);
    }
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    Control |= Request->Device->PmpNumber << AHCI_COMMAND_HEADER_PMP_SHIFT;
