    ...
    _In_ USHORT Register)
{
    ...
    PortData->Worker.InternalDevice.AtaScsiAddress.TargetId = AHCI_PMP_CONTROL_PORT;
    ...
    _In_ PATAPORT_PORT_DATA PortData)
{
    ...
    return (ULONG)TaskFile->SectorCount |
           ((ULONG)TaskFile->LowLba << 8) |
           ((ULONG)TaskFile->MidLba << 16) |
           ((ULONG)TaskFile->HighLba << 24);
}
    ...
    _In_ BOOLEAN AssertSRST)
{
    ...
    PortData->Worker.InternalDevice.AtaScsiAddress.TargetId = PortNumber;
    ...
    _In_ BOOLEAN ProcessErrorStatus)
{
    ...
    if (PortData->PortFlags & PORT_FLAG_FBS_ENABLED)
        ReceivedFis += Request->Device->AtaScsiAddress.TargetId;
    ...
    _In_ PATAPORT_PORT_DATA PortData)
{
    ...
    INFO("Unable to downgrade interface speed %08lx\n", SataSpeed);
    return FALSE;
}
