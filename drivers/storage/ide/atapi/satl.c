    ...
    _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    else
    {
        ...
        if (!(DevExt->Device.DeviceFlags & DEVICE_PIO_ONLY) &&
            !(SRB_GET_FLAGS(Srb) & SRB_FLAG_PIO_ONLY))
        {
    ...
    _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    if ((DevExt->Device.DeviceFlags & DEVICE_LBA48) && AtaDevHasFlushCacheExt(&DevExt->IdentifyDeviceData))
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    // TODO if (SrbStatus != SRB_STATUS_SUCCESS) return;
