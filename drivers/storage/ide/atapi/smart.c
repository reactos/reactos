    ...
    _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    if (IS_AHCI(&DevExt->Device))
    {
        ...
        DeviceBitmap = IS_ATAPI(&DevExt->Device) ? (1 << 4) : (1 << 0);
    }
    else
    {
        ...
        while (TRUE)
        {
            ...
            DevExt = AtaFdoFindNextDeviceByPath(ChanExt, &AtaScsiAddress, SEARCH_FDO_DEV, Srb);
            if (!DevExt)
                break;
            ...
            if (AtaScsiAddress.Lun == 0)
            {
                if (IS_ATAPI(&DevExt->Device))
                    DeviceBitmap |= (1 << AtaScsiAddress.TargetId) << 4;
                else
                    DeviceBitmap |= (1 << AtaScsiAddress.TargetId) << 0;
            }
        ...
        if ((ChanExt->Flags & CHANNEL_SECONDARY_ADDRESS_CLAIMED) &&
            !(ChanExt->PortData->PortFlags & PORT_FLAG_CBUS_IDE))
        {
            DeviceBitmap <<= 2;
        }
    }
