    ...
    _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    TaskFile->Command = IDE_COMMAND_SMART;

    /* Set the master/slave bit to the correct value */
    TaskFile->DriveSelect = Buffer.CmdIn->irDriveRegs.bDriveHeadReg & ~IDE_DRIVE_SELECT_SLAVE;
    TaskFile->DriveSelect |= DevExt->DeviceSelect & IDE_DRIVE_SELECT_SLAVE;
    ...
    _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    if (IS_ATAPI(DevExt) || (Buffer.CmdIn->irDriveRegs.bCommandReg != ID_CMD))
    {
        Srb->SrbStatus = SRB_STATUS_INVALID_REQUEST;
    ...
    _In_ PSCSI_REQUEST_BLOCK Srb)
{
    ...
    if (IS_AHCI(DevExt))
    {
        ...
        DeviceBitmap = IS_ATAPI(DevExt) ? (1 << 4) : (1 << 0);
    }
    else
    {
        PATAPORT_CHANNEL_EXTENSION ChanExt = DevExt->ChanExt;
        ULONG DeviceNumber, MaxDevices;
        ...
        for (DeviceNumber = 0; DeviceNumber < MaxDevices; ++DeviceNumber)
        {
            ...
            AtaScsiAddress = AtaMarshallScsiAddress(ChanExt->PathId, DeviceNumber, 0);

            DeviceExtension = AtaFdoFindDeviceByPath(ChanExt, AtaScsiAddress);
            if (DeviceExtension && !DeviceExtension->ReportedMissing)
            {
                if (IS_ATAPI(DeviceExtension))
                    DeviceBitmap |= (1 << DeviceNumber) << 4;
                else
                    DeviceBitmap |= (1 << DeviceNumber) << 0;
            }
        }
        ...
        /* Set the secondary channel bits */
        if ((ChanExt->PathId == 1) && (!(ChanExt->Flags & CHANNEL_CBUS_IDE)))
            DeviceBitmap <<= 2;
    }
