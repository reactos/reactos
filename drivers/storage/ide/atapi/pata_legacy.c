...
AtaLegacyShouldDetectChannels(VOID)
{
    ...
    Status = AtaOpenRegistryKey(&SoftKeyHandle, NULL, &AtapDriverRegistryPath, FALSE);
    if (!NT_SUCCESS(Status))
    {
    ...
    Status = AtaOpenRegistryKey(&ParamsKeyHandle, SoftKeyHandle, &ParametersKeyName, FALSE);
    if (!NT_SUCCESS(Status))
    {
