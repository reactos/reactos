    ...
    for (TargetId = 0; TargetId < ChanExt->MaxTargetId; ++TargetId)
    {
        ...
        for (Lun = 0; Lun < MaxLun; ++Lun)
        {
            ...
            switch (DevExt->Worker.EnumStatus)
            {
                ...
                {
                    // TODO: This could be optimized to not allocate EnumDevExt blindly
                ...
                case DEV_STATUS_NEW_DEVICE:
                {
                    ...
                    if (!(DevExt->Device.DeviceFlags & DEVICE_UNINITIALIZED))
                    {
                         ...
                         * This would cause a little overhead,
                         * but will provide an easy way to manage the PDO state.
                         */
                        --Lun; // Retry
                        continue;
                    }
            ...
            if (Lun == 0)
            {
                 ...
                 * (For example: TEAC PD-1 PD-518E and Nakamichi MJ-5.16)
                 */
                if (IS_ATAPI(&DevExt->Device))
                ...
                // TODO: Ignore multi-lun devices for now
                if (MaxLun > 1)
                {
                    WARN("Ignored %lu-lun device\n", MaxLun);
                    MaxLun = 1;
                }
            }
    ...
    _In_ PDEVICE_RELATIONS DeviceRelations)
{
    ...
    while (TRUE)
    {
        DevExt = AtaFdoFindNextDeviceByPath(ChanExt,
                                            &AtaScsiAddress,
                                            SEARCH_FDO_DEV | SEARCH_REMOVED_DEV,
                                            NULL);
        if (!DevExt)
            break;
