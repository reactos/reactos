    ...
    _Inout_ PATA_DEVICE_REQUEST Request)
{
    ...
    TaskFile->Error       = ATA_READ(ChanData->Regs.Error, ChanData, MRES_TF);
    TaskFile->SectorCount = ATA_READ(ChanData->Regs.SectorCount, ChanData, MRES_TF);
    TaskFile->LowLba      = ATA_READ(ChanData->Regs.LbaLow, ChanData, MRES_TF);
    TaskFile->MidLba      = ATA_READ(ChanData->Regs.LbaMid, ChanData, MRES_TF);
    TaskFile->HighLba     = ATA_READ(ChanData->Regs.LbaHigh, ChanData, MRES_TF);
    TaskFile->DriveSelect = ATA_READ(ChanData->Regs.Device, ChanData, MRES_TF);
    TaskFile->Command     = ATA_READ(ChanData->Regs.Command, ChanData, MRES_TF);

    if (Request->Flags & REQUEST_FLAG_LBA48)
    {
        UCHAR Control = ATA_READ(ChanData->Regs.Control, ChanData, MRES_CTRL) | IDE_DC_ALWAYS;

        /* Read the extra information from the second byte of FIFO */
        ATA_WRITE(ChanData->Regs.Control, Control | IDE_HIGH_ORDER_BYTE, ChanData, MRES_CTRL);

        TaskFile->FeatureEx     = ATA_READ(ChanData->Regs.Features, ChanData, MRES_TF);
        TaskFile->SectorCountEx = ATA_READ(ChanData->Regs.SectorCount, ChanData, MRES_TF);
        TaskFile->LowLbaEx      = ATA_READ(ChanData->Regs.LbaLow, ChanData, MRES_TF);
        TaskFile->MidLbaEx      = ATA_READ(ChanData->Regs.LbaMid, ChanData, MRES_TF);
        TaskFile->HighLbaEx     = ATA_READ(ChanData->Regs.LbaHigh, ChanData, MRES_TF);

        ATA_WRITE(ChanData->Regs.Control, Control, ChanData, MRES_CTRL);
    }
}
...
PataReadStatus(
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    return ATA_READ(ChanData->Regs.Status, ChanData, MRES_TF);
}
    ...
    _In_ UCHAR SrbStatus)
{
    ...
    if (SrbStatus != SRB_STATUS_SUCCESS && SrbStatus != SRB_STATUS_DATA_OVERRUN)
    {
        ...
        else
        {
            ...
            Request->Output.Error = ATA_READ(ChanData->Regs.Error, ChanData, MRES_TF);
        }
...
PciIdeDmaReadStatus(
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    return ATA_READ(ChanData->Regs.Dma + PCIIDE_DMA_STATUS, ChanData, MRES_DMA);
}
    ...
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    ...
    ATA_WRITE(IoBase + PCIIDE_DMA_COMMAND,
              PCIIDE_DMA_COMMAND_STOP, ChanData, MRES_DMA);
    ATA_WRITE(IoBase + PCIIDE_DMA_STATUS,
              PCIIDE_DMA_STATUS_INTERRUPT | PCIIDE_DMA_STATUS_ERROR, ChanData, MRES_DMA);
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    /* Begin transaction */
    ATA_WRITE(ChanData->Regs.Dma + PCIIDE_DMA_COMMAND, Command, ChanData, MRES_DMA);
}
    ...
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    ...
    (VOID)ATA_READ(IoBase + PCIIDE_DMA_STATUS, ChanData, MRES_DMA);

    /* Clear the interrupt bit in the status register */
    ATA_WRITE(IoBase + PCIIDE_DMA_COMMAND, PCIIDE_DMA_COMMAND_STOP, ChanData, MRES_DMA);
    ATA_WRITE(IoBase + PCIIDE_DMA_STATUS, PCIIDE_DMA_STATUS_INTERRUPT, ChanData, MRES_DMA);
}
...
{
    ATA_WRITE(ChanData->Regs.Dma + PCIIDE_DMA_STATUS, DmaStatus, ChanData, MRES_DMA);
}
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    ATA_WRITE(ChanData->Regs.Control, Control | IDE_DC_ALWAYS, ChanData, MRES_CTRL);
    ...
    _In_ UCHAR IdeStatus)
{
    ...
    switch (InterruptReason)
    {
        ...
        case ATAPI_INT_REASON_DATA_IN:
        {
            ...
            ByteCount = ATA_READ(ChanData->Regs.ByteCountLow, ChanData, MRES_TF);
            ByteCount |= ATA_READ(ChanData->Regs.ByteCountHigh, ChanData, MRES_TF) << 8;
    ...
    _In_ UCHAR DmaStatus)
{
    ...
    TRACE("%u Status 0x%02x, DMA 0x%02x, REQ 0x%lx\n",
          DEV_NUMBER(Request->Device),
          IdeStatus,
          DmaStatus,
          ChanData->CommandFlags);
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    if (Request->Flags & REQUEST_FLAG_LBA48)
    {
        ATA_WRITE(ChanData->Regs.Features, Request->TaskFile.FeatureEx, ChanData, MRES_TF);
        ATA_WRITE(ChanData->Regs.SectorCount, Request->TaskFile.SectorCountEx, ChanData, MRES_TF);
        ATA_WRITE(ChanData->Regs.LbaLow, Request->TaskFile.LowLbaEx, ChanData, MRES_TF);
        ATA_WRITE(ChanData->Regs.LbaMid, Request->TaskFile.MidLbaEx, ChanData, MRES_TF);
        ATA_WRITE(ChanData->Regs.LbaHigh, Request->TaskFile.HighLbaEx, ChanData, MRES_TF);

        /* Store the extra information in the second byte of FIFO: */
    }
    ATA_WRITE(ChanData->Regs.Features, Request->TaskFile.Feature, ChanData, MRES_TF);
    ATA_WRITE(ChanData->Regs.SectorCount, Request->TaskFile.SectorCount, ChanData, MRES_TF);
    ATA_WRITE(ChanData->Regs.LbaLow, Request->TaskFile.LowLba, ChanData, MRES_TF);
    ATA_WRITE(ChanData->Regs.LbaMid, Request->TaskFile.MidLba, ChanData, MRES_TF);
    ATA_WRITE(ChanData->Regs.LbaHigh, Request->TaskFile.HighLba, ChanData, MRES_TF);

    if (Request->Flags & REQUEST_FLAG_SET_DEVICE_REGISTER)
    {
        ATA_WRITE(ChanData->Regs.Device, Request->TaskFile.DriveSelect, ChanData, MRES_TF);
    }
}
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    ATA_WRITE(ChanData->Regs.ByteCountLow, (UCHAR)(ByteCount >> 0), ChanData, MRES_TF);
    ATA_WRITE(ChanData->Regs.ByteCountHigh, (UCHAR)(ByteCount >> 8), ChanData, MRES_TF);
    ATA_WRITE(ChanData->Regs.Features, Features, ChanData, MRES_TF);
    ATA_WRITE(ChanData->Regs.Command, IDE_COMMAND_ATAPI_PACKET, ChanData, MRES_TF);
    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    ATA_WRITE(ChanData->Regs.Command, Request->TaskFile.Command, ChanData, MRES_TF);
    ...
    _In_ SCATTER_GATHER_LIST* __restrict SgList)
{
    ...
    DBG_UNREFERENCED_LOCAL_VARIABLE(DescriptorNumber);
...
PciIdeCheckInterrupt(
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    UCHAR DmaStatus = PciIdeDmaReadStatus(ChanData);

    /*
     * On some PATA controllers the DMA interrupt bit reflects the state
     * of the IDE interrupt pin. Such controllers assert the DMA interrupt
     * even if the current command is a PIO command.
     * We use this capability for better handling of PCI shared interrupts.
     */
    return !!(DmaStatus & PCIIDE_DMA_STATUS_INTERRUPT);
}
