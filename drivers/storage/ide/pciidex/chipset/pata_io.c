    ...
    _In_ PATA_DEVICE_REQUEST Request)
{
    ...
    if (ChanData->ChanInfo & CHANNEL_FLAG_FLAG_IO32)
    {
    ...
    _In_ ULONG ByteCount)
{
    ...
    else
    {
        ...
        {
            PUCHAR Buffer = ChanData->DataBuffer + ByteCount - 1;

            *Buffer = ATA_READ(ChanData->Regs.Data);
        }
    }
    ...
    _In_ ULONG ByteCount)
{
    ...
    else
    {
        ...
        if (ByteCount & (sizeof(USHORT) - 1))
        {
            ...
            ATA_WRITE(ChanData->Regs.Data, *Buffer);
        }
    }
