    ...
    _In_reads_(MAX_IDE_DEVICE) PCHANNEL_DEVICE_DATA* DeviceList)
{
    ...
    for (i = 0; i < MAX_IDE_DEVICE; ++i)
    {
        ...
        PciRead(Controller, &TimingReg, PICCOLO_REG_PIO_TIMING, sizeof(TimingReg));
        TimingReg &= PICCOLO_PIO_MASK;
        TimingReg |= PiccoloPioTimings[Device->PioMode];
        PciWrite(Controller, &TimingReg, PICCOLO_REG_PIO_TIMING, sizeof(TimingReg));

        if (Device->DmaMode != 0)
        {
            PciRead(Controller, &TimingReg, PICCOLO_REG_DMA_TIMING, sizeof(TimingReg));
            ...
            PciWrite(Controller, &TimingReg, PICCOLO_REG_DMA_TIMING, sizeof(TimingReg));
        }
    }
}
    ...
    _Inout_ PATA_CONTROLLER Controller)
{
    ...
    ChanData->TransferModeSupportedBitmap = PIO_ALL | MWDMA_ALL | UDMA_MODES(0, 2);
