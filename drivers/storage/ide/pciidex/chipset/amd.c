...
} AmdControllerList[] =
{
    ...
    { PCI_VEN_NVIDIA, PCI_DEV_MCP89_SATA,     HW_FLAGS_SATA | HW_FLAGS_AHCI },
};
...
static const UCHAR AmdTimingControlShift[MAX_IDE_CHANNEL][MAX_IDE_DEVICE] =
{
    // M  S
    { 24, 16 }, // Pri
    { 8,  0  }, // Sec
};
static const UCHAR AmdAddressSetupShift[MAX_IDE_CHANNEL][MAX_IDE_DEVICE] =
{
    // M  S
    { 6, 4 }, // Pri
    { 2, 0 }, // Sec
};
static const UCHAR AmdCmdTimeShift[MAX_IDE_CHANNEL] =
{
    24, // Pri
    16  // Sec
};
    ...
    _In_reads_(MAX_IDE_DEVICE) PCHANNEL_DEVICE_CONFIG* DeviceList)
{
    ...
    ULONG i, DataTimReg, CmdTimReg, UdmaTimReg;

    AtaSelectTimings(DeviceList, DeviceTimings, AMD_PCI_CLOCK, SHARED_CMD_TIMINGS);

    DataTimReg = PciRead32(Controller, AMD_REG_TIMING_CTRL(IoBase));
    CmdTimReg = PciRead32(Controller, AMD_REG_ADDRESS_SETUP(IoBase));
    ...
    INFO("CH %lu: Config (before)\n"
         ...
         "UDMA %08lX\n"
         "CFG  %08lX\n",
         Channel, DataTimReg, CmdTimReg, UdmaTimReg, PciRead32(Controller, IoBase));
    ...
    for (i = 0; i < MAX_IDE_DEVICE; ++i)
    {
        ...
        if (Controller->Pci.VendorID == PCI_VEN_AMD)
            UdmaTimReg |= AMD_UDMA_TIME(3) << AmdTimingControlShift[Channel][i];
        ...
        Timing->AddressSetup = CLAMP_TIMING(Timing->CmdRecovery, 1, 4) - 1;
        ...
        CmdTimReg &= ~(0x03 << AmdAddressSetupShift[Channel][i]);
        CmdTimReg |= Value << AmdAddressSetupShift[Channel][i];

        Value = (Timing->CmdActive << 4) | Timing->CmdRecovery;
        CmdTimReg &= ~(0xFF << AmdCmdTimeShift[Channel]);
        CmdTimReg |= Value << AmdCmdTimeShift[Channel];

        Value = (Timing->DataActive << 4) | Timing->DataRecovery;
        DataTimReg &= ~(0xFF << AmdTimingControlShift[Channel][i]);
        DataTimReg |= Value << AmdTimingControlShift[Channel][i];
    }

    AmdEnablePostedWriteBuffer(Controller, ChanData, Channel, DeviceList, IoBase);

    PciWrite32(Controller, AMD_REG_TIMING_CTRL(IoBase), DataTimReg);
    PciWrite32(Controller, AMD_REG_ADDRESS_SETUP(IoBase), CmdTimReg);
