    ...
    _Out_ PULONG BestMode)
{
    ...
    for (i = MaximumMode; i >= MinimumMode; i--)
    {
        ...
        {
            *BestCycleTime = AtapTimingTable[i];
            *BestMode = i;
            return;
        }
    }
    ...
    {
        ASSERT(*BestMode < RTL_NUMBER_OF(AtapTimingTable));
        *BestCycleTime = AtapTimingTable[*BestMode];
    }
    else
    {
        *BestCycleTime = IDE_ACPI_TIMING_MODE_NOT_SUPPORTED;
    ...
    _In_reads_(MAX_IDE_DEVICE) PCHANNEL_DEVICE_CONFIG* DeviceList)
{
     ...
     *
     * It is possible to snoop an ATA command by software but an ACPI evaluation request can fail.
    ...
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    ...
    if (!IoBase)
    {
        if (Controller->AccessRange[PCIIDE_DMA_IO_BAR].Flags & RANGE_IS_VALID)
            ERR("CH %lu: No bus master registers", ChanData->Channel);
        else
    ...
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    ...
    CommandPortBase = AtaCtrlPciGetBar(Controller, BarIndex, 8);
    if (!CommandPortBase)
        return STATUS_INSUFFICIENT_RESOURCES;

    ControlPortBase = AtaCtrlPciGetBar(Controller, BarIndex + 1, 4);
    if (!ControlPortBase)
        return STATUS_INSUFFICIENT_RESOURCES;
