    ...
    _In_ ULONG ClockPeriodPs)
{
    ...
    if (Mode >= MWDMA_MODE(0))
        CycleTime = Device->MinMwDmaCycleTime;
    else if (Mode >= SWDMA_MODE(0))
        CycleTime = Device->MinSwDmaCycleTime;
    else
    {
        CycleTime = Device->MinPioCycleTime;

        /* The t0 for register transfer is the same except for mode 2 */
        CmdCycleTime = CycleTime;
        if (Mode == PIO_MODE(2))
            CmdCycleTime = min(CmdCycleTime, 330);
    }
     ...
     */
    CycleTime = AtaGetClocks(CycleTime, ClockPeriodPs);
    if ((Timing->DataActive + Timing->DataRecovery) < CycleTime)
    {
        Timing->DataActive += (CycleTime - (Timing->DataActive + Timing->DataRecovery)) / 2;
        Timing->DataRecovery = CycleTime - Timing->DataActive;
    }
    ...
    _Out_ PULONG BestMode)
{
    ...
    /*
     * This can fail due to the first _GTM result may return compatible timings for a device,
    ...
    _In_ PCM_RESOURCE_LIST ResourcesTranslated)
{
    ...
    if (!PciIdeInitDma(Controller, ChanData))
    {
        /* Fall back to PIO */
    ...
    _In_ BOOLEAN Enable)
{
    ...
    if (ChanData->ChanInfo & CHANNEL_FLAG_CBUS)
    {
        ...
        ChanData->ReadStatus(ChanData); // TODO: Move to ClearInterrupts()
    ...
    _In_ ULONG HwExtensionSize)
{
    ...
    for (i = 0; i < Controller->MaxChannels; ++i)
    {
        ...
        /* Set default values */
        ChanData->ChanInfo = CHANNEL_FLAG_HAS_CHANNEL_INTERRUPT;
        ...
        ChanData->Info.TransferModeSupported = PIO_ALL | SWDMA_ALL | MWDMA_ALL | UDMA_ALL;
