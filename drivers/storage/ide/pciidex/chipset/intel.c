 ...
 * PURPOSE:     Intel I/O controller minidriver
...
#define PCI_DEV_ICH5_82801EB_1              0x24D1
#define PCI_DEV_ICH5_82801EB_2              0x24DB
#define PCI_DEV_ICH5_R_82801ER              0x24DF
#define PCI_DEV_ICH5_6300ESB_1              0x25A2
#define PCI_DEV_ICH5_6300ESB_2              0x25A3
#define PCI_DEV_ICH5_6300ESB_RAID           0x25B0

#define PCI_DEV_ICH6_82801FB_1              0x2651
#define PCI_DEV_ICH6_R_82801FB              0x2652
#define PCI_DEV_ICH6_M_82801FBM             0x2653
#define PCI_DEV_ICH6_82801FB_2              0x266F
...
#define ESB2_63XXESB_REV_A1       0xD9
...
static const INTEL_CONTROLLER_INFO IntelControllerList[] =
{
    ...
    { PCI_DEV_ICH5_82801EB_1,    TYPE_SATA,    },
    { PCI_DEV_ICH5_82801EB_2,    TYPE_ICH,     },
    { PCI_DEV_ICH5_R_82801ER,    TYPE_SATA,    },
    { PCI_DEV_ICH5_6300ESB_1,    TYPE_ICH,     },
    { PCI_DEV_ICH5_6300ESB_2,    TYPE_SATA,    },
    { PCI_DEV_ICH5_6300ESB_RAID, TYPE_SATA,    },
    { PCI_DEV_ICH6_82801FB_1,    TYPE_SATA,    },
    { PCI_DEV_ICH6_R_82801FB,    TYPE_AHCI,    },
    { PCI_DEV_ICH6_M_82801FBM,   TYPE_AHCI,    },
    { PCI_DEV_ICH6_82801FB_2,    TYPE_ICH,     },
...
PCIIDEX_PAGED_DATA
static const ATA_PCI_ENABLE_BITS IntelPiixEnableBits[2] =
{
    { 0x41, 0x80, 0x80 },
    { 0x43, 0x80, 0x80 },
};
...
PCIIDEX_PAGED_DATA
static const ATA_PCI_ENABLE_BITS IntelMpiixEnableBits[2] =
{
    { 0x6D, 0xC0, 0x80 },
    { 0x6D, 0xC0, 0xC0 },
};
...
static const ULONG IntelDmaModeToTimingMode[] = {
    ...
    PIO_MODE(0), // Unused, MW0 is not supported by the chip
...
IntelPiixChooseDeviceSpeed(
    _In_ ULONG Channel,
    _In_ PCHANNEL_DEVICE_DATA Device)
{
    ULONG Mode;
    ...
    for (Mode = Device->PioMode; Mode >= PIO_MODE(0); Mode--)
    {
        ...
        if (Device->PioCycleTime <= IntelTimingModeToCycleTime[Mode])
            break;
    }
    ...
    /* DMA speed */
    for (Mode = Device->DmaMode; Mode >= SWDMA_MODE(1); Mode--)
    {
        ULONG RecommendedCycleTime;
        ULONG TimingMode;

        /* Last entry, disable DMA */
        if (Mode == SWDMA_MODE(1))
        {
            Mode = 0;
            break;
        }
        ...
        if (Device->DmaMode >= MWDMA_MODE(0))
            RecommendedCycleTime = Device->MwDmaCycleTime;
        else
            RecommendedCycleTime = Device->SwDmaCycleTime;

        ASSERT((Mode == SWDMA_MODE(2)) || (Mode == MWDMA_MODE(1)) || (Mode == MWDMA_MODE(2)));

        TimingMode = IntelDmaModeToTimingMode[Mode - SWDMA_MODE(2)];

        if (RecommendedCycleTime <= IntelTimingModeToCycleTime[TimingMode])
            break;
    }
    ...
    _In_reads_(MAX_IDE_DEVICE) PCHANNEL_DEVICE_DATA* DeviceList)
{
    ...
    UCHAR IdeSlaveTimReg, IdeUdmaControlReg, IdeUdmaTimReg;
    ULONG i;
    ...
    PciRead(Controller, &IdeTimReg, PIIX_REG_IDETIM(Channel), sizeof(IdeTimReg));
    PciRead(Controller, &IdeSlaveTimReg, PIIX_REG_SIDETIM, sizeof(IdeSlaveTimReg));
    if (ChanData->HwFlags & HW_FLAGS_HAS_UDMA_REG)
    {
        PciRead(Controller, &IdeUdmaControlReg, PIIX_REG_UDMACTL, sizeof(IdeUdmaControlReg));
        PciRead(Controller, &IdeUdmaTimReg, PIIX_REG_UDMATIM(Channel), sizeof(IdeUdmaTimReg));
        if (ChanData->HwFlags & HW_FLAGS_HAS_CFG_REG)
        {
            PciRead(Controller, &IdeConfigReg, PIIX_REG_CONFIG, sizeof(IdeConfigReg));
        }
    }
    ...
    IdeUdmaControlReg &= ~PIIX_UDMACTL_EN_MASK(Channel);
    ...
    PciWrite(Controller, &IdeSlaveTimReg, PIIX_REG_SIDETIM, sizeof(IdeSlaveTimReg));
    PciWrite(Controller, &IdeTimReg, PIIX_REG_IDETIM(Channel), sizeof(IdeTimReg));
    if (ChanData->HwFlags & HW_FLAGS_HAS_UDMA_REG)
    {
        PciWrite(Controller, &IdeUdmaControlReg, PIIX_REG_UDMACTL, sizeof(IdeUdmaControlReg));
        PciWrite(Controller, &IdeUdmaTimReg, PIIX_REG_UDMATIM(Channel), sizeof(IdeUdmaTimReg));
        if (ChanData->HwFlags & HW_FLAGS_HAS_CFG_REG)
        {
            /* Enable this feature for performance enhancement */
            IdeConfigReg |= PIIX_CONFIG_WR_PING_PONG;

            PciWrite(Controller, &IdeConfigReg, PIIX_REG_CONFIG, sizeof(IdeConfigReg));
        }
    }
    ...
    INFO("CH %lu: Config\n"
         ...
         "UDMATIM   %02X\n"
         "CONFIG  %04X\n",
         Channel, IdeTimReg, IdeSlaveTimReg, IdeUdmaControlReg, IdeUdmaTimReg, IdeConfigReg);
}
    ...
    _In_reads_(MAX_IDE_DEVICE) PCHANNEL_DEVICE_DATA* DeviceList)
{
    ...
    for (i = 0; i < MAX_IDE_DEVICE; ++i)
    {
        ...
        PciRead(Controller, &DeviceTimingReg, SCH_REG_DTIM(i), sizeof(DeviceTimingReg));
        ...
        PciWrite(Controller, &DeviceTimingReg, SCH_REG_DTIM(i), sizeof(DeviceTimingReg));
    }
}
    ...
    _In_ BOOLEAN UseDma)
{
    ...
    USHORT NewTiming = HwExt->Device[TargetId].Timing[UseDma ? 1 : 0];

    if (HwExt->CurrentTiming != NewTiming)
    {
        USHORT IdeTimReg;

        HwExt->CurrentTiming = NewTiming;

        /* Set the proper device timings */
        PciRead(ChanData->Controller, &IdeTimReg, HwExt->Register, sizeof(IdeTimReg));
        IdeTimReg &= ~(PIIX_IDETIM_RCT_MASK |
                       ...
                       PIIX_IDETIM_IE(1));
        IdeTimReg |= NewTiming;
        PciWrite(ChanData->Controller, &IdeTimReg, HwExt->Register, sizeof(IdeTimReg));

        TRACE("CH %lu: IDETIM %04X\n", ChanData->Channel, IdeTimReg);
    }
}
 ...
 * 2) Snoop the ATA command by software and run with the proper timings for that device.
 *
    ...
    _In_reads_(MAX_IDE_DEVICE) PCHANNEL_DEVICE_DATA* DeviceList)
{
    ...
    PciRead(Controller, &IdeTimReg, HwExt->Register, sizeof(IdeTimReg));
    ...
    for (i = 0; i < MAX_IDE_DEVICE; ++i)
    {
        ...
        /* Enable only the bits that should not be changed by mode change */
        if (Device->FixedDisk)
            IdeTimReg |= PIIX_IDETIM_PPE(i);
    ...
    PciWrite(Controller, &IdeTimReg, HwExt->Register, sizeof(IdeTimReg));
}
    ...
    _In_ PATA_CHANNEL_DATA ChanData)
{
    ...
    switch (ControllerInfo->Type)
    {
        ...
        default:
            ChanData->SetTransferMode = PciIdeSataSetTransferMode;
            break;
    }
    ...
    switch (ControllerInfo->Type)
    {
        ...
        case TYPE_ICH:
        {
            ...
            if (SupportedMode & UDMA_MODES(3, 5))
            {
                USHORT IdeConfigReg;

                PciRead(Controller, &IdeConfigReg, PIIX_REG_CONFIG, sizeof(IdeConfigReg));
                if (PIIX_CONFIG_CR(ChanData->Channel) == 0)
                {
                    INFO("CH %lu: BIOS detected 40-conductor cable\n", ChanData->Channel);
                    SupportedMode &= ~UDMA_MODES(3, 5);
                }
            }
            break;
        }
    ...
    ChanData->TransferModeSupportedBitmap = SupportedMode;

    ChanData->ChanInfo |= ATA_CHANNEL_FLAG_FLAG_IO32;
}
    ...
    _Inout_ PATA_CONTROLLER Controller)
{
    ...
    for (i = 0; i < RTL_NUMBER_OF(IntelControllerList); ++i)
    {
        ...
        if (ControllerInfo->DeviceID == Controller->Pci.DeviceID)
            break;
    }
    if (i == RTL_NUMBER_OF(IntelControllerList))
        return STATUS_NOT_SUPPORTED;

    if (ControllerInfo->Type == TYPE_AHCI)
    {
    ...
    {
        PATA_CHANNEL_DATA ChanData = Controller->Channels[i];
