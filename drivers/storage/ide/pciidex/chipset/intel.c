    ...
    _In_reads_(MAX_IDE_DEVICE) PCHANNEL_DEVICE_CONFIG* DeviceList)
{
    ...
    {
        IdeUdmaControlReg = PciRead8(Controller, PIIX_REG_UDMACTL);
    ...
    if (ChanData->HwFlags & HW_FLAGS_HAS_UDMA_REG)
    {
        PciWrite8(Controller, PIIX_REG_UDMACTL, IdeUdmaControlReg);
    ...
    _In_ PCHANNEL_DATA_PATA ChanData)
{
    ...
    switch (ControllerInfo->Type)
    {
        ...
        case TYPE_AHCI:
        {
            ...
            /* Errata: Setting SRST does not set the BSY bit */
            if ((Controller->Pci.DeviceID == PCI_DEV_ESB2_63XXESB) &&
                (Controller->Pci.RevisionID < ESB2_63XXESB_REV_A1))
            {
                // FIXME Implement a fix
            }
            break;
        }
    ...
    _Inout_ PATA_CONTROLLER Controller)
{
    ...
    for (i = 0; i < Controller->MaxChannels; ++i)
    {
        ...
        if (HwFlags & HW_FLAGS_DISABLE_DMA)
            ChanData->Info.TransferModeSupported &= PIO_ALL;
    }
