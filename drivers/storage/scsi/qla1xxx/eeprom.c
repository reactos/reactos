    ...
    _In_ ULONG BitCount)
{
    ...
    {
        USHORT DataIn = ((Sequence >> i) & 1) << QL_NVRAM_DATAIN_SHIFT;

        NVRAM_WRITE(HwExt, DataIn | QL_NVRAM_SELECT);
        NVRAM_WRITE(HwExt, DataIn | QL_NVRAM_SELECT | QL_NVRAM_CLOCK);
        NVRAM_WRITE(HwExt, DataIn | QL_NVRAM_SELECT);
    }
}
    ...
    _In_ PISP_HW_EXTENSION HwExt)
{
    ...
    for (i = 0; i < RTL_BITS_OF(USHORT); ++i)
    {
        ...
        NVRAM_READ(HwExt, &DataOut);
        SerialData = (SerialData << 1) | ((DataOut >> QL_NVRAM_DATAOUT_SHIFT) & 1);
    ...
    _Out_ PUSHORT NvramImage)
{
    ...
    for (Address = 0; Address < NvramWords; ++Address)
    {
        ...
        NvramImage[Address] = letoh16(IspNvramShiftIn(HwExt));

        /* Verify the ID header */
        if (Address == 4)
        {
            PQL_NVRAM_HEADER Header = (PQL_NVRAM_HEADER)NvramImage;
            ...
            {
                ERR("Bad NVRAM header %02X.%02X.%02X.%02X ver %02X\n",
                    Header->Id[0],
                    Header->Id[1],
                    Header->Id[2],
                    Header->Id[3],
        ...
        TRACE("Data %02X: %4X\n", Address, NvramImage[Address]);
    }

    /* Validate the NVRAM checksum */
    if (!IspNvramChecksumValid(NvramImage, NvramWords * 2))
    {
...
IspNvramParse1040(
    _In_ PISP_HW_EXTENSION HwExt,
    _In_ PUSHORT NvramImage)
{
    PQL_NVRAM_ISP1040 Nvram = (PQL_NVRAM_ISP1040)NvramImage;
    PISP_BUS_DATA BusData = &HwExt->BusData[0];
    ULONG TargetId;

    HwExt->TerminationControl = Nvram->Flags & 0x03;
    HwExt->IspParameter = letoh16(Nvram->IspParameter);
    HwExt->FwFeatures = letoh16(Nvram->FwFeatures);
    ...
    BusData->SelectionTimeout = letoh16(Nvram->SelectionTimeout);
    BusData->MaxQueueDepth = letoh16(Nvram->MaxQueueDepth);
...
IspNvramParse1080(
    ...
    _In_ PUSHORT NvramImage)
{
    PQL_NVRAM_ISP1080 Nvram = (PQL_NVRAM_ISP1080)NvramImage;
    ...
    for (PathId = 0; PathId < HwExt->ScsiBusCount; ++PathId)
    {
        ...
        BusData->SelectionTimeout = letoh16(NvramBusData->SelectionTimeout);
        BusData->MaxQueueDepth = letoh16(NvramBusData->MaxQueueDepth);
...
IspReadEeprom(
    _In_ PISP_HW_EXTENSION HwExt)
{
    USHORT NvramImage[128];
    BOOLEAN Success;

    Success = IspNvramRead(HwExt, NvramImage);
    if (!Success)
    {
    ...
    if (HwExt->IspFamily == QL_ISP_FAMILY_1040)
        IspNvramParse1040(HwExt, NvramImage);
    else
        IspNvramParse1080(HwExt, NvramImage);
}
