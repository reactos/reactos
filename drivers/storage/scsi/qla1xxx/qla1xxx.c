...
VOID
IspPutDataSegment32(
    _Out_ QL_IOCB_DATA_SEGMENT_32* __restrict DataSegment,
    _In_ STOR_SCATTER_GATHER_ELEMENT* __restrict SgElement)
{
    ...
    DataSegment->Address = htole32(SgElement->PhysicalAddress.LowPart);
    DataSegment->Length = htole32(SgElement->Length);
}

static
VOID
IspPutDataSegment64(
    _Out_ QL_IOCB_DATA_SEGMENT_64* __restrict DataSegment,
    _In_ STOR_SCATTER_GATHER_ELEMENT* __restrict SgElement)
{
    ASSERT(SgElement->Length != 0);

    DataSegment->AddressLow = htole32(SgElement->PhysicalAddress.LowPart);
    DataSegment->AddressHigh = htole32(SgElement->PhysicalAddress.HighPart);
    DataSegment->Length = htole32(SgElement->Length);
}
    ...
    _In_ ULONG CodeSize)
{
    ...
    for (i = 0; i < CodeSize; ++i)
    {
        ...
        HwExt->Mailbox[2] = htole16(Image[i]);
        if (!IspMailboxSend(HwExt))
            return FALSE;
    }
    ...
    _In_ SCSI_REQUEST_BLOCK* __restrict Srb)
{
    ...
    if ((letoh16(Response->CompletionStatus) == RQCS_COMPLETE) &&
        (letoh16(Response->ScsiStatus) == SCSISTAT_GOOD))
    {
        Srb->SrbStatus = SRB_STATUS_SUCCESS;
        return;
    }
    ...
    TRACE("RESP h 0x%lX scsi 0x%X comp 0x%X statefl 0x%X statfl 0x%X time %u sens %u resid %lu\n",
          letoh32(Response->Handle),
          letoh16(Response->ScsiStatus),
          letoh16(Response->CompletionStatus),
          letoh16(Response->StateFlags),
          letoh16(Response->StatusFlags),
          letoh16(Response->Time),
          letoh16(Response->SenseLength),
          letoh32(Response->ResidualLength));

    if ((letoh16(Response->CompletionStatus) == RQCS_RESET_OCCURRED) ||
        (letoh16(Response->CompletionStatus) == RQCS_ABORTED))
    {
        HwExt->MarkerNeeded[Srb->PathId] = TRUE;
    }

    if (letoh16(Response->CompletionStatus) == RQCS_QUEUE_FULL)
        Srb->ScsiStatus = SCSISTAT_BUSY;
    else
        Srb->ScsiStatus = letoh16(Response->ScsiStatus);

    if (letoh16(Response->CompletionStatus) < RTL_NUMBER_OF(IspCompletionStatusToSrbStatus))
        Srb->SrbStatus = IspCompletionStatusToSrbStatus[letoh16(Response->CompletionStatus)];
    else
        Srb->SrbStatus = SRB_STATUS_ERROR;

    /* Residual underrun */
    if (letoh16(Response->CompletionStatus) == RQCS_DATA_UNDERRUN)
    {
        ASSERT(Srb->DataTransferLength >= letoh32(Response->ResidualLength));
        Srb->DataTransferLength -= letoh32(Response->ResidualLength);
    }

    /* Auto request sense */
    if ((letoh16(Response->StateFlags) & RQSF_GOT_SENSE) && (Srb->SenseInfoBufferLength != 0))
    {
        Srb->SrbStatus |= SRB_STATUS_AUTOSENSE_VALID;
        Srb->SenseInfoBufferLength = min(Srb->SenseInfoBufferLength,
                                         letoh16(Response->SenseLength));

        ASSERT(letoh16(Response->SenseLength) <= sizeof(Response->SenseData));
    ...
    _In_ PISP_HW_EXTENSION HwExt)
{
    ...
    while (ResponseCons != ResponseProd)
    {
        ...
        if (Response->Header.EntryType == RQSTYPE_RESPONSE)
        {
            ...
            Srb = IspGetSrbFromHandle(HwExt, letoh32(Response->Handle));
            if (!Srb)
            {
                ERR("Invalid handle 0x%08lX\n", letoh32(Response->Handle));
    ...
    _In_ ULONG EntryCount)
{
    ...
    ASSERT(Packet != NULL);
    __assume(Packet != NULL); // This removes a NULL check after being inlined
    ...
    _In_ ULONG RequestProd)
{
    while (SegmentCount > 0)
    {
        ...
        {
            IspPutDataSegment64(&Continuation->DataSegment[i], SgElement);
    ...
    _In_ ULONG RequestProd)
{
    while (SegmentCount > 0)
    {
        ...
        for (i = 0; i < MaxSegments; ++i)
        {
            IspPutDataSegment32(&Continuation->DataSegment[i], SgElement);
    ...
    _In_ PISP_SRB_EXTENSION SrbExt)
{
    ...
    Packet->Handle = htole32(RequestProd);
    ...
    SegmentCount = letoh16(SrbExt->Packet.SegmentCount);
    if (HwExt->Flags & ISP_FLAG_64BIT_ADDRESS)
    {
    ...
    _Out_ QL_IOCB_REQUEST* __restrict Packet)
{
    ...
    Packet->Flags = htole16(Flags);
    Packet->Timeout = htole16(min(Srb->TimeOutValue, 0xFFFF));
    Packet->CdbLength = htole16(Srb->CdbLength);
    Packet->Lun = Srb->Lun;
    ...
    if (HwExt->Flags & ISP_FLAG_64BIT_ADDRESS)
    {
        ...
        if (Sgl)
        {
            ...
            for (i = 0; i < MaxSegments; ++i)
            {
                IspPutDataSegment64(&Packet->x64.DataSegment[i], &Sgl->List[i]);
            }
        }
    }
    else
    {
        ...
        if (Sgl)
        {
            ...
            for (i = 0; i < MaxSegments; ++i)
            {
                IspPutDataSegment32(&Packet->x32.DataSegment[i], &Sgl->List[i]);
            }
        }
    }
    Packet->SegmentCount = htole16(SegmentCount);
}
    ...
    _In_opt_ PVOID SystemArgument2)
{
    ...
    else
    {
        ...
        HwExt->InterruptFlags &= ~(ISP_INT_FLAG_NEED_RESET_BUS1 |
                                   ISP_INT_FLAG_NEED_RESET_BUS2);
    }
    ...
    _In_ PISP_HW_EXTENSION HwExt)
{
    ...
    HwExt->InterruptFlags &= ~ISP_INT_FLAG_STOPPED;
