    ...
    _In_ PIO_STACK_LOCATION IrpSp)
{
    ...
    if (PdoExt->Common.Signature == IsaPnpReadDataPort)
    {
        ResourceList = IsaPnpCreateReadPortDOResources(PdoExt);
        if (!ResourceList)
            return STATUS_NO_MEMORY;
    ...
    _In_ PIO_STACK_LOCATION IrpSp)
{
    ...
    if (PdoExt->Common.Signature == IsaPnpReadDataPort)
    {
        RequirementsList = IsaPnpCreateReadPortDORequirements(PdoExt, PdoExt->SelectedPort);
        if (!RequirementsList)
            return STATUS_NO_MEMORY;
    ...
    _In_ PDEVICE_OBJECT Pdo)
{
    ...
    if (LogDev->Descriptors)
        ExFreePoolWithTag(LogDev->Descriptors, TAG_ISAPNP);
