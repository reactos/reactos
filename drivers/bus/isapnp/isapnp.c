 ...
 * 0x47, 0x01, 0x30, 0x03, 0x30, 0x03, 0x04, 0x04, // IO
 * 0x30,                                           // Start DF
 * 0x22, 0x04, 0x00,                               // IRQ
 * 0x30,                                           // Start DF
 * 0x22, 0xC0, 0x00,                               // IRQ
    ...
    _In_ PISAPNP_PDO_EXTENSION PdoExt)
{
    ...
    ULONG ResourceCount, AltListCount, ListSize, i, j;
    ...
    ResourceCount = 0;
    AltListCount = 1;
    Resource = &PdoExt->IsaPnpDevice->Descriptors->Resources[0];
    while (Resource->Type != ISAPNP_RESOURCE_TYPE_END)
    {
        switch (Resource->Type)
        {
            case ISAPNP_RESOURCE_TYPE_START_DEPEDENT:
            {
            ...
            case ISAPNP_RESOURCE_TYPE_END_DEPEDENT:
            {
                DfState = dfDone;
                break;
            }
    ...
    if (!ResourceCount && !FirstFixedDescriptors && !LastFixedDescriptors)
    ...
    while (Resource->Type != ISAPNP_RESOURCE_TYPE_END)
    {
        switch (Resource->Type)
        {
            case ISAPNP_RESOURCE_TYPE_START_DEPEDENT:
            {
                if (!IsFirstAltList)
                {
                    ...
                    RtlCopyMemory(&AltList->Descriptors,
                                  &RequirementsList->List[0].Descriptors[0],
            ...
            case ISAPNP_RESOURCE_TYPE_IRQ:
            {
                ...
                for (j = 0; j < RTL_BITS_OF(Resource->IrqDescription.Mask); j++)
                {
                    if (!(Resource->IrqDescription.Mask & (1 << j)))
                        continue;
            ...
            case ISAPNP_RESOURCE_TYPE_DMA:
            {
                ...
                for (j = 0; j < RTL_BITS_OF(Resource->DmaDescription.Mask); j++)
                {
                    if (!(Resource->DmaDescription.Mask & (1 << j)))
                        continue;
    ...
    if (LastFixedDescriptors)
    {
        ...
        for (i = 0; i < RequirementsList->AlternativeLists - 1; i++)
        {
    ...
    _Out_opt_ PULONG Length)
{
    ...
    Resource = &LogDevice->Descriptors->Resources[0];
    while (Resource->Type != ISAPNP_RESOURCE_TYPE_END)
    {
    ...
    _In_ PISAPNP_PDO_EXTENSION PdoExt)
{
    ...
    for (i = 0; i < RTL_NUMBER_OF(LogDev->Irq); i++)
    {
        ...
        if (!FindIrqDescriptor(LogDev, LogDev->Irq[i].CurrentNo))
        {
            DPRINT1("IRQ entry %lx not found\n", LogDev->Irq[i].CurrentNo);
        }
    ...
    for (i = 0; i < RTL_NUMBER_OF(LogDev->Dma); i++)
    {
        ...
        if (!FindDmaDescriptor(LogDev, LogDev->Dma[i].CurrentChannel))
        {
            DPRINT1("DMA entry %u not found\n", LogDev->Dma[i].CurrentChannel);
        }
    ...
    for (i = 0; i < RTL_NUMBER_OF(LogDev->MemRange); i++)
    {
        ...
        if (!FindMemoryDescriptor(LogDev,
                                  ...
                                  &Information))
        {
            DPRINT1("MEM entry %lx %lx not found\n",
                    LogDev->MemRange[i].CurrentBase,
                    LogDev->MemRange[i].CurrentLength);
            goto InvalidBiosResources;
        }
