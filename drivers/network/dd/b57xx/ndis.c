...
NDIS_STATUS
NTAPI
MiniportReset(OUT PBOOLEAN AddressingReset,
                   ...
                   IN NDIS_HANDLE WrapperConfigurationContext)
{
    ...
    Status = NdisAllocateMemoryWithTag((PVOID*)&Adapter, sizeof(*Adapter), B57XX_TAG);
    ...
    Status = NdisMInitializeScatterGatherDma(MiniportAdapterHandle, FALSE,  MAXIMUM_FRAME_SIZE);
    if (Status != NDIS_STATUS_SUCCESS)
    {
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        NDIS_MinDbgPrint("Unable to allocate resources\n");
        Status = NDIS_STATUS_RESOURCES;
        goto Cleanup;
    }
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        NDIS_MinDbgPrint("Unable to reset the NIC (0x%x)\n", Status);
        goto Cleanup;
    }
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        NDIS_MinDbgPrint("Unable to get the fixed MAC address (0x%x)\n", Status);
        goto Cleanup;
    }
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        NDIS_MinDbgPrint("Unable to enable TX and RX (0x%x)\n", Status);
        goto Cleanup;
    }
