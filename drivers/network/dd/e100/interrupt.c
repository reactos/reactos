    ...
    _In_ PE100_ADAPTER Adapter)
{
    ...
    /*
     * Since we also check the FXP_RFD_STATUS_RNR bit to prevent restart latency
     * we cannot assume that the RU is currently in the NORESOURCES state.
     * Thus, make sure the RU has actually entered the no resource condition.
     */
    ...
    _Out_ PBOOLEAN OutOfBuffers)
{
    ...
    while (PacketsToIndicate < RTL_NUMBER_OF(ReceiveArray))
    {
        ...
        {
            /*
             * We enable saving of bad packets on the 82557
             * in order to receive VLAN frames with the default MTU.
    ...
    _Inout_ PLIST_ENTRY SendReadyList)
{
    ...
    for (TxContext = Adapter->TxFirst; Adapter->TxPending > 0; TxContext = TxContext->Next)
    {
        ...
        if (!(letoh16(Tcb->Header.Status) & FXP_CB_STATUS_C))
            break;
