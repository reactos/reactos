    ...
    _In_ PE100_TX_CONTEXT TxContext)
{
    ...
    {
        ULONG TcbOffset, TcbPhys;

        TRACE("Starting CU %02X\n", FXP_SCB_CUS(CSR_READ_8(Adapter, FXP_CSR_SCB_RUSCUS)));

        Adapter->CommandUnitActive = TRUE;

        TcbOffset = (ULONG)((ULONG_PTR)TxContext->Tcb - (ULONG_PTR)Adapter->HeadTcb);
        TcbPhys = Adapter->HeadTcbPa + TcbOffset;

        EeCommandUnitExecuteCommandAddr(Adapter, FXP_SCB_COMMAND_CU_START, TcbPhys);
    }

    /* Set a watchdog timer (6 seconds) */
    Adapter->TxTimeOut = 3;
}
    ...
    _In_ UINT NumberOfPackets)
{
    ...
    if (NumberOfPackets == 0)
    {
        ...
        do
        {
            PLIST_ENTRY Entry = RemoveHeadList(&Adapter->SendQueueList);
    ...
    if (Adapter->TxPending != TxPending)
    {
