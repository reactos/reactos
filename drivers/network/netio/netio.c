...
typedef struct _WSK_SOCKET_INTERNAL
{
    ...
    LONG RefCount;                       /* See SocketGet/SocketPut */
...
static void
SocketPutSynchronous(_In_ PWSK_SOCKET_INTERNAL Socket)
{
    ...
    while (PutSocketsThreadShouldRun)
    {
        ...
        while (SocketsToPut != NULL)
        {
            ...
            while (NumSocketPuts > 0)
            {
                ...
                SocketPutSynchronous(SocketToPut);
                KeAcquireSpinLock(&SocketsToPutListLock, &OldIrql);
            }
        }
        KeReleaseSpinLock(&SocketsToPutListLock, OldIrql);
    ...
    _Inout_ PIRP Irp)
{
    ...
    Socket->RefCount = 1;            /* SocketPut() is in WskCloseSocket */
