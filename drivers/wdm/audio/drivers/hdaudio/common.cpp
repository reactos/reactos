    ...
    OUT PUNKNOWN *OutPortUnknown)
{
    ...
    if (bTopology)
    {
        ...
        Status =
            HDAUDIO_NewMiniportTopology(&MiniportTopology, AssociatedPinsCount, AssociatedPinIds, OutNode, this, FilterDescription);
        if (!NT_SUCCESS(Status))
        {
    ...
    else
    {
        ...
        if (bOutput)
        {
            Status = OutNode->GetNodesWithType(0x00, &NodesCount, &Nodes);
        }
        else
        {
            Status = OutNode->GetNodesWithType(0x01, &NodesCount, &Nodes);
        }
        if (!NT_SUCCESS(Status))
        {
        ...
        Status =
            HDAUDIO_NewMiniportWaveRT(&MiniportWaveRT, NodesCount, Nodes, OutNode, this, FilterDescription);
        if (!NT_SUCCESS(Status))
        {
    ...
    for (ULONG NodeIndex = 0; NodeIndex < PinNodeCount; NodeIndex++)
    {
        ...
        if (NT_SUCCESS(Status))
        {
            ...
            if (PinCaps.EAPDCapable)
            {
                // enable EAPD
                DPRINT1("Enable EAPD...\n");
                Status = OutNode->SetEAPD(PinNodes[NodeIndex], 0x02);
                if (!NT_SUCCESS(Status))
                {
    ...
    IN UCHAR Digital)
{
    ...
    for (ULONG Index = 0; Index < AssociatedPinCount; Index++)
    {
        ...
        if (PinConfiguration.PortConnectivity == 0x1) // pin is not connected
            continue;
    ...
    if (bSet)
    {
        ...
        Status = InstallDevice(
            DeviceObject, Irp, SubdeviceWave, bOutput, FALSE, CLSID_PortWaveRT, ResourceList, AssociatedPinCount, AssociatedPins, (PVOID)OutNode,
        ...
        DPRINT1("HDAUDIO: SubdeviceName %S\n", SubdeviceTopology);
        Status = InstallDevice(
            DeviceObject, Irp, SubdeviceTopology, bOutput, TRUE, CLSID_PortTopology, ResourceList, AssociatedPinCount, AssociatedPins, (PVOID)OutNode,
    ...
    {
        DPRINT1("HDAUDIO: Pin is not connected\n");
    }
    ...
    IN UCHAR Digital)
{
    ...
    if (NT_SUCCESS(Status))
    {
        Status = BuildInstallFilter(DeviceObject, Irp, TRUE, OutNode, ResourceList, AssociatedPinCount, AssociatedPins, Digital);
        if (NT_SUCCESS(Status))
        {
