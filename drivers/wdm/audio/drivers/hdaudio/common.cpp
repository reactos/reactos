...
CAdapterCommon::Initialize(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp, IN PRESOURCELIST ResourceList)
{
    ...
    DPRINT1("ProcessDacNodes----------------------------------\n");
    Status = ProcessDacNodes(DeviceObject, Irp, ResourceList, OutputNodeCount, OutputNodes, (PVOID)OutNode);
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("HDAUDIO: ProcessDacNodes failed for output node with %x\n", Status);
        delete OutNode;
        ExFreePool(InputNodes);
        ExFreePool(OutputNodes);
        ExFreePool(PinNodes);
    ...
    OutNode->ClearVisitedState();
    DPRINT1("ProcessAdcNodes----------------------------------\n");
    Status = ProcessAdcNodes(DeviceObject, Irp, ResourceList, InputNodeCount, InputNodes, (PVOID)OutNode);
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("HDAUDIO: ProcessAdcNodes failed for output node with %x\n", Status);
        delete OutNode;
        ExFreePool(InputNodes);
        ExFreePool(OutputNodes);
        ExFreePool(PinNodes);
    ...
    IN PVOID Node)
{
    ...
    Status = ChooseSubdeviceName(0x01, TRUE, &SubdeviceWave);
    if (!NT_SUCCESS(Status))
    {
    ...
    IN PVOID Node)
{
    ...
    // lets build a subdevice name
    LPWSTR SubdeviceWave = NULL;
    Status = ChooseSubdeviceName(0x0A, TRUE, &SubdeviceWave);
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("HDAUDIO: ChooseSubdeviceName failed with %x\n", Status);
        return Status;
