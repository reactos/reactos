    ...
    IN BOOLEAN Capture)
{
    ...
    if (Capture)
        WaveInPins[0].KsPinDescriptor.DataRangesCount = 0;
    else
        WaveOutPins[0].KsPinDescriptor.DataRangesCount = 0;
    ...
    if (Capture)
        WaveInPins[0].KsPinDescriptor.DataRangesCount = FormatsSupported;
    else
        WaveOutPins[0].KsPinDescriptor.DataRangesCount = FormatsSupported;
    ...
    IN PVOID Node)
{
    ...
    for (ULONG NodeIndex = 0; NodeIndex < PinNodeCount; NodeIndex++)
    {
        ...
        Status = OutNode->GetAmplifierDetails(PinNodes[NodeIndex], 0, &AmplifierCapabilities);
        if (!NT_SUCCESS(Status))
        {
            DPRINT1("Warning: GetAmplifierDetails failed with %x for node %u\n", Status, PinNodes[NodeIndex]);
        }
        ...
        Set.Index = 0;
        Set.Mute = 0;
        Set.Gain = AmplifierCapabilities.NumSteps;
        Status = OutNode->SetAmplifierGainMute(PinNodes[NodeIndex], &Set);
        if (!NT_SUCCESS(Status))
        {
            DPRINT1("Warning: GetAmplifierDetails failed with %x for node %u\n", Status, PinNodes[NodeIndex]);
        }
    ...
    IN PULONG InputNodes)
{
    ...
    for (ULONG NodeIndex = 0; NodeIndex < PinNodeCount; NodeIndex++)
    {
        ...
        AMPLIFIER_CAPABILITIES AmplifierCapabilities;
        Status = OutNode->GetAmplifierDetails(PinNodes[NodeIndex], 1, &AmplifierCapabilities);
        if (!NT_SUCCESS(Status))
        {
            DPRINT1("Warning: GetAmplifierDetails failed with %x for node %u\n", Status, PinNodes[NodeIndex]);
        }
