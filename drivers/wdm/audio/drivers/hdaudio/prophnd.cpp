...
PropertyHandler_Volume(IN PPCPROPERTY_REQUEST PropertyRequest)
{
    ...
    for (ULONG NodeIndex = 0; NodeIndex < OutputNodeCount; NodeIndex++)
    {
        ...
        Status = Node->GetAmplifierDetails(OutputNodes[NodeIndex], FALSE, &AmplifierCapabilities);
        if (!NT_SUCCESS(Status))
        {
        ...
        if (AmplifierCapabilities.NumSteps == 0 || AmplifierCapabilities.Steps == 0)
        {
        ...
        if (PropertyRequest->Verb & KSPROPERTY_TYPE_BASICSUPPORT)
        {
            if (PropertyRequest->ValueSize >= sizeof(KSPROPERTY_DESCRIPTION))
            {
                ...
                if (PropertyRequest->ValueSize >= BasicPropertySize)
                {
                    ...
                    Range->Bounds.SignedMinimum = DBMin * 0x10000;
                    Range->Bounds.SignedMaximum = DBMax * 0x10000;
                    Range->SteppingDelta = DBStep * 0x10000;
                    Range->Reserved = 0;
        ...
        else if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
        {
            ...
            DPRINT("GetAmplifierGainMute Status %x, Node %d, Mute %x, Gain %d\n", Status, OutputNodes[NodeIndex], Mute, Gain);
        ...
        else if (PropertyRequest->Verb & KSPROPERTY_TYPE_SET)
        {
            ...
            DPRINT("SetAmplifierGainMute Status %x, Node %d, Gain %d\n", Status, OutputNodes[NodeIndex], Gain);
        }
    }
    Miniport->Release();
    return Status;
...
PropertyHandler_Mute(IN PPCPROPERTY_REQUEST PropertyRequest)
{
    ...
    for (ULONG NodeIndex = 0; NodeIndex < OutputNodeCount; NodeIndex++)
    {
        ...
        else if (PropertyRequest->Verb & KSPROPERTY_TYPE_SET)
        {
            ...
            Status = Node->SetAmplifierGainMute(OutputNodes[NodeIndex], &Set);
            DPRINT("SetAmplifierGainMute Status %x, Node %d, Mute %x\n", Status, OutputNodes[NodeIndex], Mute);
        }
    }
    Miniport->Release();
    return Status;
