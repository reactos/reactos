    ...
    OUT PUCHAR Gain)
{
    ...
    Input ? AC_AMP_GET_INPUT : AC_AMP_GET_OUTPUT | Right ? AC_AMP_GET_RIGHT : AC_AMP_GET_LEFT | AC_AMP_GET_INDEX;
    Status = m_Adapter->TransferVerb(Verb, &Response);
    if (!NT_SUCCESS(Status))
    {
    ...
    IN UCHAR Gain)
{
    ...
    Input ? AC_AMP_SET_INPUT : AC_AMP_SET_OUTPUT | Right ? AC_AMP_SET_RIGHT : AC_AMP_SET_LEFT | Mute | Gain;
    Status = m_Adapter->TransferVerb(Verb, &Response);
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("HDAUDIO: GetAmplifierGainMute failed with %x\n", Status);
