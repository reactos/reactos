	...
	_Out_ PHDAUDIO_BUFFER_DESCRIPTOR* BdlBuffer)
{
	...
	PVOID buf = MmAllocateContiguousMemorySpecifyCache(RequestedBufferSize, zeroAddr, maxAddr, RtlConvertUlongToLargeInteger(0), MmWriteCombined);
	if (!buf)
	{
		ASSERT(FALSE);
		return STATUS_INSUFFICIENT_RESOURCES;
	}
	RtlZeroMemory(buf, RequestedBufferSize);

	PHDAC_BDLENTRY bdl = (PHDAC_BDLENTRY)MmAllocateContiguousMemorySpecifyCache(BDL_SIZE, zeroAddr, maxAddr, RtlConvertUlongToLargeInteger(0), MmWriteCombined);
	if (!bdl)
	{
		ASSERT(FALSE);
		MmFreeContiguousMemorySpecifyCache(buf, RequestedBufferSize, MmWriteCombined);
		return STATUS_INSUFFICIENT_RESOURCES;
	}
	RtlZeroMemory(bdl, BDL_SIZE);
