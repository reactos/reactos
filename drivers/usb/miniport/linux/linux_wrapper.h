...
struct pci_driver {
        ...
        int  NTAPI (*probe)  (struct pci_dev *dev, const struct pci_device_id *id);   /* New device inserted */
        void NTAPI (*remove) (struct pci_dev *dev);   /* Device removed (NULL if not a hot-plug capable driver) */
...
//#define snprintf(a,b,format, arg...) swprintf((a),format, ##arg)
...
/* Function prototypes */
/*------------------------------------------------------------------------*/ 
void NTAPI usb_hcd_pci_remove (struct pci_dev *dev);
