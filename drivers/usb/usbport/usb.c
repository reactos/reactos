 ...
 */
struct usb_device NTAPI *usb_alloc_dev(struct usb_device *parent, struct usb_bus *bus)
{
	struct usb_device *dev;
 ...
 */
struct usb_device NTAPI *usb_get_dev (struct usb_device *dev)
{
	struct device *tmp;
 ...
 */
void NTAPI usb_put_dev(struct usb_device *dev)
{
	if (dev)
		put_device(&dev->dev);
}
 ...
 * controllers) should ever call this.
 */
void NTAPI usb_connect(struct usb_device *dev)
{
	int devnum;
 ...
 * Init
 */
int NTAPI __init usb_init(void)
{
	if (nousb) {
 ...
 * Cleanup
 */
void NTAPI __exit usb_exit(void)
{
