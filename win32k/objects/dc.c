  ...
  if (hDC == NULL)
    {
      ...
      DisplayDC = IntGdiCreateDC(&DriverName, NULL, NULL, NULL, TRUE, FALSE);
      if (NULL == DisplayDC)
        {
          return NULL;
        }
  ...
  if (NULL == Driver || 0 == RtlCompareUnicodeString(Driver, &StdDriver, TRUE))
    {
      if (CreateAsIC)
        ...
        //FIXME
//      else if (! UserGraphicsCheck(TRUE))
        else
        {
          BOOL inUser = UserIsEntered();
          if (!inUser){
             UserEnterExclusive();
          }
          if (! UserGraphicsCheck(TRUE))
          {
            if (!inUser){
               UserLeave(); 
            }
              ...
              CONST PDEVMODEW InitData)
{
  ...
  Ret = IntGdiCreateDC(&SafeDriver, &SafeDevice, NULL, &SafeInitData, FALSE, FALSE);
              ...
              CONST PDEVMODEW InitData)
{
  ...
  Ret = IntGdiCreateDC(NULL == Driver ? NULL : &SafeDriver,
                       ...
                       NULL == InitData ? NULL : &SafeInitData, TRUE, FALSE);
