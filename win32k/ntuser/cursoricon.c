...
HCURSOR FASTCALL
UserSetCursor(PCURICON_OBJECT NewCursor, BOOL ForceChange)
{
   BITMAPOBJ *BitmapObj;
   SURFOBJ *SurfObj;
   PDEVINFO DevInfo;
...
BOOL FASTCALL
UserSetupCurIconHandles(PWINSTATION_OBJECT WinStaObject)
{
...
UserReferenceCurIconByProcess(PCURICON_OBJECT Object)
{
...
VOID FASTCALL
UserReferenceCurIcon(PCURICON_OBJECT CurIcon)
{
   //FIXME   
}
VOID FASTCALL
...
VOID FASTCALL CursorDereference(PCURICON_OBJECT Cursor)
{
   //FIXME
}
...
UserDestroyCurIconObject(PCURICON_OBJECT Cursor, BOOL ProcessCleanup)
{
   ...
   if (NULL == Cursor->hModule)
   {
      ASSERT(Cursor->ProcessList.Flink->Flink == &Cursor->ProcessList);
      Current = CONTAINING_RECORD(Cursor->ProcessList.Flink, CURICON_PROCESS, ListEntry);
   ...
   Search = Cursor->ProcessList.Flink;
   while (Search != &Cursor->ProcessList)
   {
      Current = CONTAINING_RECORD(Search, CURICON_PROCESS, ListEntry);
      if (Current->Process == W32Process)
      {
   ...
   bmpMask = Cursor->IconInfo.hbmMask;
   bmpColor = Cursor->IconInfo.hbmColor;
...
VOID FASTCALL
UserCleanupCurIcons(struct _EPROCESS *Process, PW32PROCESS Win32Process)
{
   PLIST_ENTRY CurrentEntry;
   ...
   while (CurrentEntry != &CurIconList)
   {
      Cursor = CONTAINING_RECORD(CurrentEntry, CURICON_OBJECT, ListEntry);
      CurrentEntry = CurrentEntry->Flink;

      ProcessEntry = Cursor->ProcessList.Flink;
      while (ProcessEntry != &Cursor->ProcessList)
      {
         ...
         if (Win32Process == ProcessData->Process)
         {
            ...
            RemoveEntryList(&Cursor->ListEntry);
            UserDestroyCurIconObject(Cursor, TRUE);
            break;
         }
         ProcessEntry = ProcessEntry->Flink;
      }
      if (ProcessEntry == &Cursor->ProcessList)
      {
...
//      ObmCloseHandle(WinStaObject->HandleTable, Handle);
   ...
   if(IconInfo)
   {
      ...
      if(Indirect)
      {
         Cursor->IconInfo.hbmMask = BITMAPOBJ_CopyBitmap(Cursor->IconInfo.hbmMask);
         Cursor->IconInfo.hbmColor = BITMAPOBJ_CopyBitmap(Cursor->IconInfo.hbmColor);
      }
      if(Cursor->IconInfo.hbmColor &&
            (bmp = BITMAPOBJ_LockBitmap(Cursor->IconInfo.hbmColor)))
      {
         Cursor->Size.cx = bmp->SurfObj.sizlBitmap.cx;
         Cursor->Size.cy = bmp->SurfObj.sizlBitmap.cy;
         BITMAPOBJ_UnlockBitmap(bmp);
         GDIOBJ_SetOwnership(Cursor->IconInfo.hbmColor, NULL);
      }
      if(Cursor->IconInfo.hbmMask &&
            (bmp = BITMAPOBJ_LockBitmap(Cursor->IconInfo.hbmMask)))
      {
         if (Cursor->IconInfo.hbmColor == NULL)
         {
            Cursor->Size.cx = bmp->SurfObj.sizlBitmap.cx;
            Cursor->Size.cy = bmp->SurfObj.sizlBitmap.cy / 2;
         }
         BITMAPOBJ_UnlockBitmap(bmp);
         GDIOBJ_SetOwnership(Cursor->IconInfo.hbmMask, NULL);
      }
   }
   ...
   PICONINFO IconInfo)
{
   ...
   RtlCopyMemory(&ii, &Cursor->IconInfo, sizeof(ICONINFO));
   ...
   SIZE *Size)
{
   ...
   /* Copy fields */
   Status = MmCopyToCaller(fIcon, &Cursor->IconInfo.fIcon, sizeof(BOOL));
   if(!NT_SUCCESS(Status))
   {
      SetLastNtError(Status);
      goto done;
...
BOOL
STDCALL
NtUserClipCursor(RECT *UnsafeRect)
{
   ...
   POINT MousePos;
   DECLARE_RETURN(BOOL);
   PDESKTOP_OBJECT CurrentDesktop;
 ...
 * -you shouldnt/cant destroy a shared cursor
 */
BOOL
STDCALL
   ...
   DWORD Unknown)
{
   ...
   if(UserDestroyCurIconObject(Cursor, FALSE))
   {
      CursorDereference(Cursor);
      RETURN( TRUE);
   }
...
NtUserSetCursor(HCURSOR hCursor)
{
   ...
   hOldCursor = UserSetCursor(Cursor, FALSE);
   ...
   PICONINFO IconInfo)
{
   ...
   CurIconObject = UserGetCursorObject(hCursor);
   if(CurIconObject)
   {
      /* Copy fields */
   ...
   HRSRC hGroupRsrc)
{
   ...
   CurIconObject = UserGetCursorObject(Handle);
   if(CurIconObject)
   {
   ...
   DWORD Unknown1)
{
   ...
   CurIconObject = UserGetCursorObject(hIcon);
   if(CurIconObject)
   {
