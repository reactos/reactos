...
{
   return PsGetWin32Thread()->Desktop;
}
...
PWINDOW_OBJECT FASTCALL UserGetDesktopWindow(VOID)
{
   ...
   return GetWnd(pdo->DesktopWindow);
}
...
NtUserPaintDesktop(HDC hDC)
{
   ...
   DesktopBrush = (HBRUSH)UserGetClassLong(WndDesktop, GCL_HBRBACKGROUND, FALSE);
   ...
   if(WinSta->hbmWallpaper != NULL)
   {
      ...
      if((DeskWin = WndDesktop))//(DeskWin = IntGetWindowObject(hWndDesktop)))
      {
         SIZE sz;
         int x, y;
         HDC hWallpaperDC;
