...
static BOOLEAN CallMmInitSystem(IN ULONG Phase, IN PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    /* Get the function address and call through pointer */
    PFN_MM_INIT_SYSTEM pfnMmInit = (PFN_MM_INIT_SYSTEM)MmInitSystem;
    
    /* Use volatile to prevent optimization */
    volatile PFN_MM_INIT_SYSTEM volatilePfn = pfnMmInit;
    
    if (volatilePfn)
    {
        /* Try to call through the pointer */
        return volatilePfn(Phase, LoaderBlock);
    }
    return FALSE;
}
...
KiSystemStartupBootStack(VOID)
{
    ...
    /* SKIP - causes hang (cross-module call) */
    /* PoInitializePrcb(Prcb); */
    {
        const char msg[] = "*** KERNEL: PoInitializePrcb skipped (cross-module) ***\n";
        const char *p = msg;
        while (*p) { while ((__inbyte(COM1_PORT + 5) & 0x20) == 0); __outbyte(COM1_PORT, *p++); }
    }
    ...
    /* Skip MmInitSystem to avoid hang */
    {
        const char msg[] = "*** KERNEL: Skipping MmInitSystem Phase 1 (causes hang) ***\n";
        const char *p = msg;
        while (*p) { while ((__inbyte(COM1_PORT + 5) & 0x20) == 0); __outbyte(COM1_PORT, *p++); }
    }
    ...
    /* Initialize graphics subsystem for UEFI GOP */
    {
        ...
        if (LoaderBlock && LoaderBlock->Extension)
        {
            ...
            if (Extension->Size >= sizeof(LOADER_PARAMETER_EXTENSION))
            {
                ...
                /* Map framebuffer for direct access */
                /* For AMD64, physical = virtual in early boot */
                ...
                if (FrameBufferBase.QuadPart != 0 && FrameBufferSize > 0)
                {
                    ...
                    /* Skip actual pixel writing to avoid crashes */
                    {
                        const char msg[] = "*** KERNEL: Skipping pixel writes (crash avoidance) ***\n";
                        const char *p = msg;
                        while (*p) { while ((__inbyte(COM1_PORT + 5) & 0x20) == 0); __outbyte(COM1_PORT, *p++); }
                    }
                   ...
                   IN PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    /* Inline minimal executive initialization to avoid cross-module call */
    {
        ...
        /* With small memory model, we can call ObInitSystem directly */
        {
            const char msg[] = "*** KERNEL: Skipping ObInitSystem (may hang) ***\n";
            const char *p = msg;
            while (*p) { while ((__inbyte(COM1_PORT + 5) & 0x20) == 0); __outbyte(COM1_PORT, *p++); }
        }
        
        /* Skip for now to avoid hang
        BOOLEAN Result = ObInitSystem();
        if (Result)
        {
            const char msg[] = "*** KERNEL: Object Manager initialized successfully! ***\n";
            const char *p = msg;
            while (*p) { while ((__inbyte(COM1_PORT + 5) & 0x20) == 0); __outbyte(COM1_PORT, *p++); }
        }
        */
