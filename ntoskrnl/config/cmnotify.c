               ...
               _In_ PCM_KEY_CONTROL_BLOCK SubKey)
{
    ...
    /* Subkey should be deeper than the parent key in the keys tree */
    if (SubKey->TotalLevels <= ParentKey->TotalLevels)
        return FALSE;

    /* Walk up the key tree from the subkey towards the parent key */
    SubKeyParent = SubKey->ParentKcb;
    Depth = SubKey->TotalLevels - ParentKey->TotalLevels;
    while (SubKeyParent != NULL &&
           SubKeyParent != ParentKey &&
           Depth-- > 0)
                ...
                IN ULONG Filter)
{
    ...
    /*
     * Avoid double-signal and double-free of PostBlocks.
     * The critical section only includes non-blocking tasks,
     * so it shouldn't slow down the CM that much.
     */
    ExAcquirePushLockExclusive(&KeyHive->HiveLock);
    KeyHive->HiveLockOwner = KeGetCurrentThread();

    if (IsListEmpty(ListHead)) goto Unlock;
...
Unlock:
    KeyHive->HiveLockOwner = NULL;
    ExReleasePushLockExclusive(&KeyHive->HiveLock);
}
               ...
               IN BOOLEAN LockHeld)
{
    ...
    PCMHIVE KeyHive = CONTAINING_RECORD(KeyBody->KeyControlBlock->KeyHive, CMHIVE, Hive);
    ...
    /* Serialize against CmpReportNotify walking/removing entries from the same lists */
    ExAcquirePushLockExclusive(&KeyHive->HiveLock);
    KeyHive->HiveLockOwner = KeGetCurrentThread();
