...
//
// Ensures the device object is physical (PDO)
//
#define POP_ASSERT_DO_IS_PHYSICAL(DeviceExtension)             \
    ASSERT(DeviceExtension->CommonExtension.IsFdo == FALSE)
...
{
    ASSERT(PopIrpOwnerLockThread == NULL);
...
//
// Retrieve common functional extensions (FDO Exts) of a device
//
FORCEINLINE
PVOID
PopGetFdoExtensions(
...
PopAcquirePowerPolicyLock(VOID)
{
    ...
    ASSERT(PopPowerPolicyOwnerLockThread == NULL);
