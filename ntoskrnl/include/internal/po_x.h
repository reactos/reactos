...
//
// Ensures the current IRQL of the current calling thread is at
// passive level (mainly used by power setting callbacks)
//
#define POP_ASSERT_IRQL_PASSIVE()                                        \
    ASSERT(KeGetCurrentIrql() == PASSIVE_LEVEL)

//
// Ensures the setting callback hasn't yet entered into the driver's
// supplied callback function
//
#define POP_ASSERT_NO_RUNNING_CALLBACK(SettingCallback)                   \
    ASSERT((SettingCallback->Flags & POP_PSC_ENTERING_CALLBACK) == 0)
...
//
// Power setting callback inliners
//
FORCEINLINE
VOID
PopPowerSettingApplyAttribute(
    _In_ PPOP_POWER_SETTING_CALLBACK SettingCallback,
    _In_ ULONG AttributeFlag)
{
    SettingCallback->Flags |= AttributeFlag;
}

FORCEINLINE
VOID
PopPowerSettingClearAttribute(
    _In_ PPOP_POWER_SETTING_CALLBACK SettingCallback,
    _In_ ULONG AttributeFlag)
{
    SettingCallback->Flags &= ~AttributeFlag;
...
FORCEINLINE
BOOLEAN
PopPowerSettingReadAttribute(
    _In_ PPOP_POWER_SETTING_CALLBACK SettingCallback,
    _In_ ULONG AttributeFlag)
{
    if (SettingCallback->Flags & AttributeFlag)
    {
        return TRUE;
    }
...
FORCEINLINE
VOID
PopAlertCallbackReturned(
    _In_ PPOP_POWER_SETTING_CALLBACK SettingCallback)
{
    KeSetEvent(&SettingCallback->CallbackReturned, IO_NO_INCREMENT, FALSE);
}
