...
CcpUnmapCache(PVOID Context)
{
	...
	MmUnmapViewInSystemSpace(WorkItem->ToUnmap);
...
VOID CcpDereferenceCache(ULONG Start, BOOLEAN Immediate)
{
	...
	if (Immediate)
	{
		PVOID ToUnmap = Bcb->BaseAddress;
		PSECTION_OBJECT ToDeref = Bcb->SectionObject;
		...
		CcpUnlock();
		MmUnmapViewInSystemSpace(ToUnmap);
