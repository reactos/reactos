    ...
    _Out_ PUNICODE_STRING SymbolicLinkName)
{
    ...
    Length = PrefixString->Length + DeviceString->Length
 ...
 * Receives device string with '##?#' prefix at the start. For example, ##?#ACPI#PNP0501#1#.
 *
    ...
    _Out_opt_ LPGUID InterfaceClassGuid)
{
    ...
    static const UNICODE_STRING MungedPrefix = RTL_CONSTANT_STRING(L"##?#");
    ...
    _In_ BOOLEAN Create)
{
    ...
    if (ReferenceStringPresent)
    {
        SymbolicLink.MaximumLength = SymbolicLink.Length = (USHORT)((ULONG_PTR)(ReferenceString.Buffer) - (ULONG_PTR)SymbolicLink.Buffer);
    }
    else
    {
        ReferenceString.Buffer = &Buffer;
        ReferenceString.MaximumLength = ReferenceString.Length = sizeof(WCHAR);
    }

    /* Fix prefix: '\??\' -> '##?#' */
    SymbolicLink.Buffer[0] = L'#';
    SymbolicLink.Buffer[1] = L'#';
    SymbolicLink.Buffer[2] = L'?';
    SymbolicLink.Buffer[3] = L'#';
    ...
    /* Replace first '\' by '#' in reference string */
    ...
    _Out_ PUNICODE_STRING AliasSymbolicLinkName)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("Failed to separate '%wZ', Status 0x%08lx\n", SymbolicLinkName, Status);
        goto Quit;
    }
