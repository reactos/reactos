...
{
    const UNICODE_STRING PathSep = RTL_CONSTANT_STRING(L"\\");
    ...
    {
        DPRINT1("RtlUnicodeStringPrintf() failed with status 0x%08lx\n", Status);
        return Status;
    }
...
{
    const UNICODE_STRING KernelModePrefix = RTL_CONSTANT_STRING(L"\\??\\");
    const UNICODE_STRING UserModePrefix = RTL_CONSTANT_STRING(L"\\\\?\\");
    const UNICODE_STRING MungedPrefix = RTL_CONSTANT_STRING(L"##?#");
