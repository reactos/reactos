 ...
 *
 * @remarks
 * The caller assumes responsibility that they lock the DOPE
 * list before calling this function to walk down over the list.
 */
static
BOOLEAN
    ...
    _In_ DEVICE_POWER_STATE State)
{
    ...
    if (!ConservationIdleTime && !PerformanceIdleTime)
    {
        ...
        if (Dope)
        {
            ...
            if (PopIsDeviceRegisteredForIdleDetection(Dope))
            {
                /*
    ...
    if (ConservationIdleTime == -1 && PerformanceIdleTime == -1)
    {
        ...
        {
            DPRINT1("Default idle times requested for an unsupported device type (%lu) of DO (0x%p)\n", DeviceObject->DeviceType, DeviceObject);
            return NULL;
        }
    ...
    _Inout_ PVOID Handle)
{
    ...
    /* Mark the callback for un-registration so that it can be deleted later */
    PopPowerSettingApplyAttribute(SettingCallback, POP_PSC_UNREGISTERED);
    PopReleasePowerSettingLock();

    /*
     * At the time of marking this callback for unregistration, there may be a
     * power worker already executing into driver's supplied callback routine or
    ...
    /* The callback has finished its job, unregister it now */
    PopAcquirePowerSettingLock();
    PopReleasePowerSettingCallback(SettingCallback);
    ...
    _Inout_ PULONG IdlePointer)
{
    ...
    InterlockedIncrementUL(&Dope->BusyCount);
}
    ...
    _Inout_ PULONG IdlePointer)
{
    ...
    Dope = CONTAINING_RECORD(IdlePointer, DEVICE_OBJECT_POWER_EXTENSION, IdleCount);
    InterlockedIncrementUL(&Dope->BusyReference);
}
    ...
    _Inout_ PULONG IdlePointer)
{
    ...
    Dope = CONTAINING_RECORD(IdlePointer, DEVICE_OBJECT_POWER_EXTENSION, IdleCount);
    InterlockedDecrementUL(&Dope->BusyReference);
}
