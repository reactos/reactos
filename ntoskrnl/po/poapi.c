    ...
    _Outptr_opt_ PVOID *Handle)
{
    ...
    /* Make sure the provided GUID is one of the power setting GUIDs */
    if (!PopIsPowerSettingValid(SettingGuid))
    {
        /* Display the offending GUID to the debugger if needed */
        Status = RtlStringFromGUID(SettingGuid, &GuidString);
        if (NT_SUCCESS(Status))
        {
            DPRINT1("The following GUID %wZ is not a valid power setting GUID\n", &GuidString);
    ...
    /*
     * The callback was never registered, allocate a notification entry
     * for this callback as we will notify the power setting worker
     * immediately to set the value to the driver's callback for the
     * first time.
     */
    ...
    if (NotifyEntry == NULL)
    {
        DPRINT1("Failed to allocate memory for the power setting notification entry\n");
        PopReleasePowerSettingLock();
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    /* Allocate memory for this power setting callback and register it */
    ...
    /* Cache the setting callback to the notification entry for deployment */
    NotifyEntry->SettingCallback = SettingCallback;

    /* Setup the notification worker item */
    ExInitializeWorkItem(&NotifyEntry->NotifyWorkItem,
                         Routine,
                         NotifyEntry);

    /*
     * Now this callback is pretty much ready to be deployed to the
     * power setting worker, release the lock and enqueue the worker.
     */
    PopReleasePowerSettingLock();
    ...
    /* The callback has been notified, we no longer need the notify entry anymore */
    *Handle = SettingCallback;
    PopFreePool(NotifyEntry, TAG_PO_POWER_SETTING_NOTIFY_ENTRY);
    return Status;
}
