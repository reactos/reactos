    ...
    _In_ PVOID SystemArgument2)
{
    ...
    PPOP_IRP_WATCHDOG_DATABASE Watchdog;
    ...
    for (Entry = PopIrpWatchdogList.Flink;
         ...
         Entry = Entry->Flink)
    {
        Watchdog = CONTAINING_RECORD(Entry, POP_IRP_WATCHDOG_DATABASE, Link);
        if (Watchdog->Irp == Irp)
        {
            /* We found it */
            Found = TRUE;
    ...
    _In_ PIRP Irp)
{
     ...
     * allocation is fatal.
     */
    Watchdog = PopAllocatePool(sizeof(POP_IRP_WATCHDOG_DATABASE),
                               FALSE,
                               TAG_IRP_WATCHDOG);
    ...
    _In_ PKSTART_ROUTINE WorkerRoutine)
{
    ...
    if (!NT_SUCCESS(Status))
    {
        DPRINT1("Failed to set the thread base priority of IRP power worker thread (Status 0x%lx)", Status);
        return Status;
    }
