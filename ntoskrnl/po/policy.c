    ...
    _In_ POWER_POLICY_DEVICE_TYPE PolicyDeviceType)
{
    ...
    {
        /*
         * If we reach this path the Power Policy Manager has been called
         * two or multiple times of new upcoming batteries. This is a typical
         * behavior on multi-battery systems. As the Power Manager is currently
         * initializing the composite battery, make the thread wait until
         * the Power Manager has finished initializing the composite battery and
         * then re-issue a tag read request. This is so we avoid weird bugs where
         * multiple instances are trying to initialize the composite battery.
         */
        if (PopBattery->Flags & POP_CB_INITIALIZING)
        {
            KeWaitForSingleObject(&PopCbInitialized,
                                  Executive,
                                  KernelMode,
                                  FALSE,
                                  NULL);
