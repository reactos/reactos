...
/**
 * @brief
 * Checks if the system is draining electrical power from
 * AC source and acknowledges the Power Manager if it's so.
 *
 * @param[in] PowerState
 * The current power state draining situation of the system,
 * provided by the caller.
 *
 * @param[out] Changed
 * A boolean returned to the caller which indicates if the
 * power policy has changed due to change of a state in events
 * of power draining.
 */
static
VOID
    ...
    _Out_ PBOOLEAN Changed)
{
    ...
    *Changed = FALSE;
     ...
     * the event of the system draining AC power is occurring just now. Notify
     * every registered power setting callback as well.
     */
...
/**
 * @brief
 * The main Composite Battery (CB) IRP completion handler.
 * It handles information provided by the Composite Battery
 * driver (COMPBATT) and then it forwads the said information
 * back to the Composite Battery main handler of the Power Manager.
 *
 * @param[in] DeviceObject
 * A pointer to a device object, which refers to the Composite
 * Battery driver itself. This parameter is unused.
 *
 * @param[in] Irp
 * A pointer to the IRP passed by the Composite Battery driver.
 * This parameter is unused.
 *
 * @param[in] Context
 * A pointer to a context supplied by the Composite Battery driver.
 * This parameter is unused.
 *
 * @return
 ...
 * STATUS_NO_SUCH_DEVICE is returned if the composite battery has dissapeared
 * or it hasn't been connected yet.
 */
static
NTSTATUS
NTAPI
    ...
    _In_ PVOID Context)
{
        ...
        Status != STATUS_CANCELLED)
    {
        ...
        {
            DPRINT1("Battery not yet connected or it has disappeared, waiting for the battery to appear\n");
        ...
        !PopBattery->Irp->Cancel)
    {
        ...
        switch (Mode)
        {
            ...
            case POP_CB_QUERY_STATUS_MODE:
            {
                 ...
                 * on the new status. Notify every callback that are registered
                 * for notifications for GUID_BATTERY_PERCENTAGE_REMAINING power
                 * setting.
                 */
                if (PopBattery->Status.Capacity != BattStatus.Capacity)
                {
                    PopBattery->Mode = POP_CB_QUERY_BATTERY_ESTIMATION_TIME_MODE;
                    PopNotifyPowerSettingChange(&GUID_BATTERY_PERCENTAGE_REMAINING);
                }
...
/**
 * @brief
 * Calculates the battery wait status thresholds. The high and low capacity wait
 * thresholds are determined so that the Power Manager gets notified at the
 * exact capacity change of the battery.
 *
 * @param[in,out] WaitStatus
 * A pointer to the wait status capacity thresholds. This structure contains
 * the high and low thresholds of which the Power Manager gets notified when
 * the battery has hit the exact capacity.
 */
static
VOID
...
/**
 * @brief
 * Allocates memory space for the buffer so that the battery
 * tag can be read from the Composite Battery driver. It also
 * tells the Composite Battery driver how should it wait for
 * the battery tag to appear.
 *
 * @param[in] WaitOnTag
 * If set to TRUE, the driver must wait indefinitely on the
 * battery tag to appear so that we can read it. If set to
 * FALSE, the battery tag is read immediately.
 *
 * @return
 * Returns a pointer to allocated memory to hold up the battery
 * tag from the driver. NULL is returned otherwise.
 */
static
PULONG
PopReadBatteryTag(
    _In_ BOOLEAN WaitOnTag)
{
     ...
     * otherwise we have to wait indefinitely on the battery to get a tag.
     */
    Timeout = (WaitOnTag == TRUE) ? -1 : 0;
...
/**
 * @brief
 * Allocates memory space for the buffer so that the static
 * composite battery information can be queried.
 *
 * @param[in] InfoType
 * The type of battery information to be read. If set to BatteryInfo,
 * the Power Manager will inquire the Composite Battery driver to
 * read the static battery information. If set to BatteryEstTime
 * the estimation time of the battery will be read. If set to
 * BatteryTemp, the overall temperature of the battery will be read.
 *
 * @return
 * Returns a pointer to allocated memory to hold up the battery
 * information from the driver. NULL is returned otherwise.
 */
static
 ...
 * @brief
 * Allocates memory space for the buffer so that the battery
 * status such as power charge state and capacity can be read
 * from the driver.
 *
 * @return
 * Returns a pointer to allocated memory to hold up the battery
 * status. NULL is returned otherwise.
 */
static
...
PopReadBatteryStatus(VOID)
{
     ...
     * status of the battery. Set an interval to POP_CB_STATUS_WAIT_INTERVAL
     * (== 3 seconds) as timeout for battery status to be read because our
     * ACPI driver lacks in ACPI notifications.
     *
     * As a result, if we were to wait indefinitely we may not get notified
     * when the battery's capacity changes. Therefore having a timeout helps
     * keep pooling for battery status, even if we get the same battery
     * capacity.
     */
...
/**
 * @brief
 * Acknowledges the Power Manager that a new battery is appearing.
 * All the information regarding the composite battery and status
 * are invalidated.
 *
 * @param[in] BatteryName
 * A pointer to a Unicode string that points to the name of the
 * new battery.
 */
VOID
NTAPI
...
/**
 * @brief
 * Queries the battery state, such as state charge, capacity and whatnot.
 *
 * @param[out] BatteryState
 * A pointer to system battery state of which this function fills
 * data in to the caller.
 */
VOID
NTAPI
    ...
    _Out_ PSYSTEM_BATTERY_STATE BatteryState)
{
    ...
    /*
     * There never was a battery present with the Power Manager,
...
/**
 * @brief
 * The main core policy device handler of the Composite Battery.
 * This handler communicates with the Composite Battery driver
 * via IOCTLs of which the handler sends command requests to
 * the driver to gather data about the battery such as its tag,
 * information, status and whatnot.
 *
 * @param[in] Parameter
 * A pointer to arbitrary data. This usually points to a policy
 * work item, submitted by either the PPM (Power Policy Manager)
 * or the battery IRP completion routine of the kernel.
 */
...
/**
 * @brief
 * Disconnects the composite battery from the Power Policy Manager.
 */
NTSTATUS
NTAPI
PopDisconnectCompositeBattery(VOID)
{
    ...
    /* Acknowledge the Power Manager the composite battery is now disconnected */
    PopCbConected = FALSE;
...
/**
 * @brief
 * Connects the composite battery with the Power Policy Manager.
 * This is usually triggered when a battery comes in we're being
 * notified of it.
 *
 * @param[in] BatteryDevice
 * A pointer to the battery device object. This usually points to
 * the Composite Battery driver itself, as the kernel will cache
 * the object into the dedicated Power Manager structure.
 */
NTSTATUS
NTAPI
    ...
    _In_ PDEVICE_OBJECT BatteryDevice)
{
    ...
    /*
     * Mark the composite battery as being currently connected. This helps avoiding
     * instances of connecting the composite battery each time a new battery is
     * coming. This also helps avoiding race conditions where we get two upcoming
     * batteries simultaneously upon connecting with the policy device.
     */
    PopCbConected = TRUE;
    ...
    /*
     * The power policy has changed, notify every setting callback who has
