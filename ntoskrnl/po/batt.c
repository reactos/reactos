        ...
        !PopBattery->Irp->Cancel)
    {
        ...
        switch (Mode)
        {
            case POP_CB_READ_TAG_MODE:
            {
                ...
                /*
                 * Also take away the initialization mark as the composite battery
                 * is pretty much set up. Alert other threads that the composite
                 * battery is initialized in case the system has multiple batteries
                 * so that the Power Manager will re-issue a read tag request again.
                 */
                if (PopBattery->Flags & POP_CB_INITIALIZING)
                {
                    PopBattery->Flags &= ~POP_CB_INITIALIZING;
                    KeSetEvent(&PopCbInitialized, IO_NO_INCREMENT, FALSE);
                }
...
PopDisconnectCompositeBattery(VOID)
{
    ...
    PopBattery->Flags &= ~(POP_CB_PENDING_NEW_BATTERY | POP_CB_PROCESSING_MODE_REQUEST | POP_CB_INITIALIZING | POP_CB_WAIT_ON_BATTERY_TAG | POP_CB_REMOVE_BATTERY);
    PopBattery->Flags |= POP_CB_NO_BATTERY;

    /* Reset the composite battery initialization event, we no longer have batteries */
    KeClearEvent(&PopCbInitialized);
    ...
    _In_ PDEVICE_OBJECT BatteryDevice)
{
    ...
    /* Mark the composite battery for initialization */
    PopBattery->Flags |= POP_CB_INITIALIZING;
