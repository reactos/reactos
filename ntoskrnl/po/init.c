...
PopInitSystemPhase0(VOID)
{
    ...
    KeInitializeDpc(&PopIdleScanDevicesDpc, PopScanForIdleDevicesDpcRoutine, NULL);
     ...
     */
    PopDefaultPowerStateHandlers[PowerStateShutdownOff].Type = PowerStateShutdownOff;
    PopDefaultPowerStateHandlers[PowerStateShutdownOff].RtcWake = FALSE;
    PopDefaultPowerStateHandlers[PowerStateShutdownOff].Handler = (PVOID)PopShutdownHandler;
    PopDefaultPowerStateHandlers[PowerStateShutdownOff].Context = NULL;
...
PopInitSystemPhase1(VOID)
{
    ...
    ULONG IsPowerSimulateEnabled = 0;
    ...
    // FIXME: Initialize the Power Registry Database

    /*
     * Check if power simulation is triggered for this system.
     * Apply some capabilities in this case. We do not care if
     * we fail to query the PowerSimulate key value, because
     * it is not that important to us.
     */
    Status = PopReadPowerSettings(&PopPowerSimulateRegValue,
                                  REG_DWORD,
                                  &KeyInfo);
    if (NT_SUCCESS(Status))
    {
        IsPowerSimulateEnabled = *(PULONG)KeyInfo->Data;
        PopFreePool(KeyInfo, TAG_PO_REGISTRY);
        if (IsPowerSimulateEnabled)
        {
    ...
    /* Read the AC/DC and administrator policies and apply them where necessary */
    ...
    if (!NT_SUCCESS(Status))
    {
        /* If this fails, then sacrifice the system... */
