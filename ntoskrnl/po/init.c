...
/**
 * @brief
 * Initializes the shutdown queue entries list.
 */
static
VOID
...
/**
 * @brief
 * Initializes the Power Policy Manager of the Power Manager.
 */
static
VOID
...
/**
 * @brief
 * Initializes the default administrator power policy settings
 * overrides.
 *
 * @param[out] AdminPolicy
 * A pointer to the defaulted administrator power policy settings.
 */
static
VOID
...
/**
 * @brief
 * Initializes the power setting callbacks mechanism.
 */
static
VOID
...
/**
 * @brief
 * Initializes the composite battery mechanism.
 *
 * @return
 * Returns TRUE if the component of the Power Manager has
...
/**
 * @brief
 * Initializes the power request mechanism.
 *
 * @return
 * Returns STATUS_SUCCESS if the component of the Power Manager
 * has initialized successfully. Otherwise a failure NTSTATUS
 * code is returned.
 */
CODE_SEG("INIT")
NTSTATUS
...
/**
 * @brief
 * Initialize the core Power Manager engine and its components at
 * Phase 0 boot initialization.
 *
 * @return
 * Returns TRUE if all the components of the core engine have initialized
...
PopInitSystemPhase0(VOID)
{
     ...
     *
     * NOTE: We could decode the ACPI_TABLE buffer and look for the presence of
     * system states like _S4 within FADT, so that we can determine the system state
     * capabilities earlier. Thing is, in principle it's not the job of the kernel
     * to do this but rather the ACPI HAL must be responsible of this and call
     * ZwPowerInformation(SystemPowerStateHandler) to register system state handlers
     * with us. In this way we can fully trust that we can determine the power capabilities
     * of this system.
     */
    ...
    Status = PopCreateWorkerThread(&PopMasterDispatchIrp,
                                   NULL,
                                   POP_IRP_MASTER_DISPATCHER_THREAD_PRIORITY);
    if (!NT_SUCCESS(Status))
    {
...
PopInitSystemPhase1(VOID)
{
    ...
    /* Read the AC/DC, administrator and miscellanea policies from the registry and apply them where necessary */
 ...
 *
 * @return
 * Returns TRUE if the subsystem has initialized successfully,
 * FALSE otherwise.
 */
CODE_SEG("INIT")
BOOLEAN
...
/**
 * @brief
 * Initialize the power modules of this processor such as
 * CPU idling, performance and whatnot.
 *
 * @param[out] Prcb
 * A pointer to the defaulted administrator power policy settings.
 */
