...
/**
 * @brief
 * The main master worker thread that flushes volumes down to the backing
 * storage disk on power down.
 *
 * @param[in] Context
 * A pointer to a flush context, that contains flush data such as the volume
...
/**
 * @brief
 * Allocates or retrieves a device object power extension (DOPE)
 * of a device.
 *
 * @param[in] DeviceObject
 * A pointer to a device object that is to be given a DOPE. If this device
 * already has a DOPE, it's returned that instead.
 *
 * @return
 * Returns a pointer to an allocated DOPE assigned to the device for the
 * first time. The existing DOPE is returned if the device already has it.
    ...
    _In_ PDEVICE_OBJECT DeviceObject)
{
    ...
    /* Initialize the cooling extension of this device */
    Dope->CoolingExtension = NULL;
...
/**
 * @brief
 * Delists the volume entry of the device and frees up the
 * allocated DOPE.
 *
 * @param[in] DeviceObject
 * A pointer to a device object of which its volume entry is to be
 * removed from the list and DOPE freed from memory.
 */
VOID
NTAPI
...
/**
 * @brief
 * Flushes volumes upon a power down of the system.
 *
 * @param[in] ShuttingDown
 * If set to TRUE, the function will flush both the whole registry database
 * as well as the non-removable devices. If set to FALSE, the function
 * will flush volumes depending on what the flush policy allows.
 */
VOID
NTAPI
PopFlushVolumes(
...
/**
 * @brief
 * Initializes the device power extensions of a device object.
 *
 * @param[in,out] DeviceObjectExtension
 * A pointer to a device object extensions of a device where the
 * power extension properties are to be initialized.
 */
VOID
NTAPI
...
/**
 * @brief
 * Registers a volume device with the Power Manager. Volumes that are
 * registered will then be flushed down upon a power of the system.
 *
 * @param[in] DeviceObject
 * A pointer to a device object extensions of a device where the
 * power extension properties are to be initialized.
