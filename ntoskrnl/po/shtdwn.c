...
/**
 * @brief
 * Registers a thread to be notified of the shutdown event when the
 * system is powering down.
 *
 * @param[in] Thread
 * A pointer to a Executive-side thread that requests a shutdown event
 * notification. The thread will be alerted when shutdown is imminent
 * so it can process shutdown related stuff accordingly.
 *
 * @return
 * Returns STATUS_SUCCESS if the thread has successfully requested to
 * receive a shutdown event notification. STATUS_NO_MEMORY is returned
 * if there's no enough memory to be allocated for the shutdown entry
 * for the thread. STATUS_UNSUCCESSFUL is returned if the thread could
 * not be requested for receiving shutdown events because shutdown is
 * already occurring.
 */
static
NTSTATUS
...
/**
 * @brief
 * Gives chance to each process alive to process down their shutdown
 * tasks when the system is shutting down.
 */
static
VOID
PopProcessShutDownLists(VOID)
{
    ...
    /* Signal the shutdown event so that everybody knows the system is shutting down */
     ...
     * of the system. It is not a problem if we release the lock that early because
    ...
    while (PopShutdownThreadList != NULL)
    {
        ...
        KeWaitForSingleObject(ShutDownWaitEntry->Thread, Executive, KernelMode, FALSE, NULL);
...
/**
 * @brief
 * Computes the number of processes that are currently alive at
 * the time of shutting down.
 *
 * @return
 * Returns a number to the count of active live processes.
 */
static
ULONG
...
/**
 * @brief
 * The last stage of shutting down the system. The system state handler is
 * invoked and processors are put in a halt. There's no way of returning back
 * at this point.
 *
 * @param[in] SystemAction
 * A power action to be taken when shutting down the system. The following
 * supported actions are:
 *
 * PowerActionShutdownReset -- Shuts down the system and instructs the CPU
 *                             to reset itself.
 *
 * PowerActionShutdown -- Shuts down the system but without powering OFF
 *                        the system. This is possible only if the "no power OFF"
 *                        policy is currently in force. Otherwise this power action
 *                        acts similarly to PowerActionShutdownOff.
 *
 * PowerActionShutdownOff -- Shuts down the system and mechanically powers it OFF (G3).
 */
static
VOID
PopShutdownSystem(
...
/**
 * @brief
 * Shuts down the system gracefully, thereby peforming a shutdown of each kernel
 * subsystem, before inquiring state handlers or HALs to shutdown the metal.
 * This is the first stage of the shutdown procedure.
 */
...
/**
 * @brief
 * The default Power Manager state handler. The handler is executed on each
 * logical processor by PopInvokeSystemStateHandler.
 *
 * @param[in] Context
 * A pointer to a context that is supplied by the HAL who registers a state
 * handler. The context is for the handler itself. This parameter is optional.
 * For the default state handler, we don't use it.
 *
 * @param[in] SystemHandler
 * A pointer to a system handler that is supplied by the HAL who registers a
 * state handler. The handler invokes this subsequent handler which passes
 * control to the system shutdown to that handler. This parameter is optional.
 * For the default state handler, we don't use it.
 *
 * @param[in] SystemContext
 * A pointer to a system context that is supplied by the HAL who registers a state
 * handler. The context is for the system state handler. This parameter is optional.
 * For the default state handler, we don't use it.
 *
 * @param[in] NumberProcessors
 * A number to the total active processors, passed by PopInvokeSystemStateHandler.
 * For the default state handler, we don't use it.
 *
 * @param[in] Number
 * The number that represents the identification of the logical processor region,
 * passed by PopInvokeSystemStateHandler. We use this parameter to figure out
 * if this handler was called from the the boot processor (aka CPU 0) and if so,
 * a shutdown banner and text is displayed.
 */
NTSTATUS
NTAPI
PopShutdownHandler(
...
/**
 * @brief
 * Puts a shutdown workitem into the shutdown queue list. This workitem is processed
 * once the shutdown operation has been triggered, of which the process that queues
 * a shutdown workitem can perform shutdown related tasks.
 *
 * @param[in] WorkItem
 * A pointer to a workitem to be put in the shutdown queue for later processing.
 *
 * @return
 * Returns STATUS_SUCCESS if the workitem has been queued, otherwise STATUS_SYSTEM_SHUTDOWN
 * is returned if shutdown is already occurring.
 */
NTSTATUS
NTAPI
...
/**
 * @brief
 * The calling thread uses this function to request notifications of
 * upcoming shutdown event.
 *
 * @param[out] Event
 * A pointer to an event setup by the Power Manager, returned to the caller.
 * The calling thread shall wait on this event to be alerted of an upcoming
 * shutdown event.
 *
 * @return
 * Returns STATUS_SUCCESS if the thread requested a shutdown event successfully.
