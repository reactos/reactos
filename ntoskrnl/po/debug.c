    ...
    _In_ PVOID SystemArgument2)
{
         ...
         NextEntry = NextEntry->Flink)
    {
        ...
        DbgPrint("Power request is coming from %s\n", PowerRequest->ComingFromUserMode ? "user-mode" : "kernel-mode");
        ...
        {
            /* TODO: SimpleString information logging for now */
            if (PowerRequest->Context->Flags & DIAGNOSTIC_REASON_SIMPLE_STRING)
            {
                DbgPrint("Reason for this power request:\n");
                DbgPrint("       %wZ\n\n", PowerRequest->Context->SimpleString);
            }
        }
        else
        {
            DbgPrint("Reason Context is NULL\n\n");
        }
