...
/**
 * @brief
 * Starts the power requests reaper mechanism. The reaper will terminate all the
 * present and active power requests after 5 minutes. The reaper is activated
 * only if the system has AoAc (modern standby) capability.
 */
static
VOID
...
/**
 * @brief
 * Adjusts the execution state flags of a legacy power request.
 *
 * @param[in] EsFlags
 * The new execution state flags that are to replace the previous
 * ones, provided by the caller.
 *
 * @param[in,out] StateHandle
 * A pointer to a power request state handle of which its execution
 * state flags are to be changed.
 */
static
VOID
...
/**
 * @brief
 * Creates a legacy power request object. Legacy power requests are
 * used primarily to register system states, specifically PoRegisterSystemState
 * and PoUnregisterSystemState functions.
 *
 * @param[in] EsFlags
 * The execution state flags to pass down to the newly created
 * legacy power request object.
 *
 * @param[in] NewRegister
 * If set to TRUE, this power request was never created before and
 * it'll be registered for the first time. If set to FALSE, this
 * indicates the caller simply wants to set new execution state flags.
 *
 * @param[in,out] Handle
 * A pointer to the newly created legacy power request object,
 * returned to the caller.
 *
 * @return
 * Returns STATUS_SUCCESS if the legacy power request has been created
...
/**
 * @brief
 * Checks if the resource name is present when the caller passes
 * a reason context when asking for a power request to be created.
 *
 * @param[in] ResourceFileName
 * A pointer to a Unicode string that points to the resource filename.
 *
 * @return
 * Returns TRUE if the resource filename is present, FALSE otherwise.
 */
static
BOOLEAN
...
/**
 * @brief
 * Validates a COUNTED_REASON_CONTEXT buffer passed by the caller.
 * The buffer coming from UM is unsafe, severe checks are performed.
 * Contents inside the reason context are validated to ensure they
 * do make sense, regardless if the buffer was coming from UM or KM.
 *
 * @param[in] Context
 * A pointer to a reason context buffer to be validated.
 *
 * @param[in] PreviousMode
 * The requestor mode execution.
 *
 * @param[out] CapturedContext
 * A pointer to the captured reason context that's been validated,
 * returned to the caller.
 *
 * @return
 * Returns STATUS_SUCCESS if the validation has completed and the
 * reason context buffer is valid. STATUS_INVALID_PARAMETER is
 * returned if at least one of the contents inside the reason context
 * is not valid. STATUS_INSUFFICIENT_RESOURCES is returned if there's
 * no enough memory to be allocated to hold the captured reason context.
 */
static
NTSTATUS
...
/**
 * @brief
 * Frees a captured reason context buffer previously allocated
 * from a call to PopValidateAndCaptureReasonContext.
 *
 * @param[in] Context
 * A pointer to a reason context buffer to be freed.
 *
 * @param[in] PreviousMode
 * The requestor mode execution.
 */
...
/**
 * @brief
 * Creates a power request object.
 *
 * @param[in] DeviceObject
 * A pointer to a device object that is creating the power request.
 *
 * @param[in] Context
 * A pointer to a reason context that describes the reasons why this
 * device is creating a power request.
 *
 * @param[out] PowerRequestObject
 * A pointer to a created power request object, returned to the caller.
 *
 * @return
 * Returns STATUS_SUCCESS if the power request object has been
 * created. A failure NTSTATUS code is returned otherwise.
 */
static
NTSTATUS
...
/**
 * @brief
 * The power request reaper DPC routine that is executed by the kernel
 * once the timer has expired. This will invoke the reaper routine
 * to terminate every active power request.
 */
...
/**
 * @brief
 * Terminates every active power request currently registered.
 *
 * @remarks
 * The action of terminating a power request implies by signing the
 * "Terminate" mark to it rather than totally invalidate the power request
 * from memory. This gives the opportunity to every driver to free their
 * power requests on cleanup. If we were to free the objects in our end
 * we might end up with invalid pointers on drivers' end.
 */
VOID
NTAPI
...
/**
 * @brief
 * Increments or decrements a power request of the specified
 * type from the power request object. The act of incrementing or
 * decrementing power requests makes the Power Manager override certain
 * power policy settings such as sleep options, idle counters, etc.
 *
 * @param[in] PowerRequest
 * A pointer to a power request object of which its power request
 * properties are to be changed.
 *
 * @param[in] RequestType
 * The type of power request of which its properties are to be changed.
 *
 * @param[in] PreviousMode
 * The requestor mode execution.
 *
 * @param[in] ClearRequest
 * If set to TRUE, the power request of the specified type will be
 * decremented. Otherwise the counter of the power request type will
 * be incremented.
 *
 * @return
 * Returns STATUS_SUCCESS if the properties of the target power request
 * have been changed properly. STATUS_NOT_SUPPORTED is returned if the
 * specified power request type is not supported when attempting to
 * change the power request type state properties from KM.
 */
NTSTATUS
NTAPI
...
/**
 * @brief
 * Called by the Object Manager whenever a power request object
 * is to be closed after no longer being used.
 */
VOID
NTAPI
...
/**
 * @brief
 * A helper function that abstracts the registration of a power request
 * object in two types, legacy and Vista+, depending on what the caller
 * wants.
 *
 * @param[in] DeviceObject
 * A pointer to a device object that registers for a power request.
 * This parameter is optional for legacy requests.
 *
 * @param[in] Request
 * The type of request the calling thread is inquiring. The following types are:
 *
 * RegisterLegacyRequest -- The caller inquires that a legacy power request is
 *                          to be created.
 *
 * RegisterALaVistaRequest -- The caller inquires that a Vista+ power request
 *                            is to be created.
 *
 * @param[in] NewRegister
 * If set to TRUE, this is a new registration of a power request that was
 * never created before. This only applies for legacy requests.
 *
 * @param[in] EsFlags
 * The execution state flags to be passed to the power request upon
 * creation. This is only for legacy requests.
 *
 * @param[in] Context
 * A pointer to a reason context describing the reasons why this device
 * is creating a power request. This parameter is optional for legacy requests
 * and in general, if the caller doesn't want to provide any reason.
 *
 * @param[in,out] PowerRequestHandle
 * A handle to a power request object that is returned to the caller.
 *
 * @return
 * Returns STATUS_SUCCESS if the power request object has been successfully
 * registered. Otherwise a failure NTSTATUS code.
 *
 * @remarks
 * The helpers abstracts the creation of power requests in two subsequent
 * helpers, legacy and Vista+ power requests creation. The legacy requests are
 * created due to calls made by PoRegisterSystemState and Vista+ requests made
 * due to calls by PoCreatePowerRequest.
 *
 * It's important to note that prior Windows 7 (which in that edition it's where
 * power requests got introduced), Windows 2000, XP and Server 2003 never had a
 * concept of power requests, at least in API form. These versions of Windows
 * provided PoRegisterSystemState as a way for drivers to inquire power requests
 * to make the system busy due to activities. In Windows 7, this API function has
 * been deprecated with a new function, PoCreatePowerRequest. Technically power
 * requests are mere objects created by the Object Manager. In ReactOS,
 * PoRegisterSystemState and NtSetThreadExecutionState do make use of the power request
 * object mechanism in order to avoid duplicate mechanism implementations.
 */
NTSTATUS
NTAPI
 ...
 *
 * @param[in] Thread
 * A pointer to a Executive-side thread of which its power request object
 * is to be deleted.
 */
VOID
NTAPI
