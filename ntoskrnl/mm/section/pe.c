...
//		DIE(("PointerToRawData[%u] is not aligned\n", i));
	...
	ASSERT(Intsafe_CanAddLong64(pssSegments[i].Image.FileOffset, pssSegments[i].RawLength.u.LowPart));
	...
	if(!AlignUp(&pssSegments[i].Length.u.LowPart, pssSegments[i].Length.u.LowPart, nSectionAlignment))
	    DIE(("Cannot align the virtual size of section %u\n", i));

	ASSERT(IsAligned(pssSegments[i].Length.QuadPart, nSectionAlignment));
	...
	if(!Intsafe_AddULong32(&nPrevVirtualEndOfSegment, pssSegments[i].Image.VirtualAddress, pssSegments[i].Length.u.LowPart))
