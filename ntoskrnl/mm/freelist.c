...
MmLockPageUnsafe(PFN_TYPE Pfn)
{
   ...
   Page = MiGetPfnEntry(Pfn);
   DPRINT("MmLockPageUnsafe(PhysicalAddress %x) from %x count %d\n", Pfn << PAGE_SHIFT, __builtin_return_address(1), Page->LockCount);
...
MmUnlockPage(PFN_TYPE Pfn)
{
   ...
   Page->LockCount--;
   DPRINT("MmUnlockPage(PhysicalAddress %x) from %x count %d\n", Pfn << PAGE_SHIFT, __builtin_return_address(1), Page->LockCount);
   KeReleaseQueuedSpinLock(LockQueuePfnLock, oldIrql);
}
